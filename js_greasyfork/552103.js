...
// @downloadURL https://update.greasyfork.org/scripts/552103/Catwar%20-%20%D0%9A%D0%B0%D1%80%D1%82%D0%B0%20%D0%B4%D0%BB%D1%8F%207%D0%94%D0%9B.user.js
// @updateURL https://update.greasyfork.org/scripts/552103/Catwar%20-%20%D0%9A%D0%B0%D1%80%D1%82%D0%B0%20%D0%B4%D0%BB%D1%8F%207%D0%94%D0%9B.meta.js
// ==/UserScript==
...
(function () {
  ...
  function updateLabyrinthSelector() {
    ...
    if (!selector) return;
    if (mapsLoaded) {
      expandedSections = GM_getValue('expandedSections', {}) || {};
...
function toggleUI() {
  ...
  const content = document.querySelector('.cw-ui-content');
  if (!ui || !content) return;

  ui.classList.toggle('cw-minimized', isUIMinimized);
...
function toggleSettings() {
  ...
  // Включаем/выключаем класс-обёртку
  ui.classList.toggle('cw-settings-visible', !isSettingsMinimized);

  // Убираем инлайн display, чтобы управлял CSS
  content.style.removeProperty('display');

  // Форс-рефлоу (на всякий)
