...
// @downloadURL https://update.greasyfork.org/scripts/37212/Uber%20Memsource%20project%20creation%20tweak.user.js
// @updateURL https://update.greasyfork.org/scripts/37212/Uber%20Memsource%20project%20creation%20tweak.meta.js
// ==/UserScript==

var workflows = [
    {"Type":"None", "Tooltip": ""},
    {"Type":"TR Only", "Tooltip": ""},
    {"Type":"RV Only", "Tooltip": ""},
    {"Type":"DTR", "Tooltip": ""},
    {"Type":"TR", "Tooltip": "", "Type2":"RV"},
];
...
workflows.forEach(function(element) {
    ...
    if(element.Type != "None") {
        $('#' + id).parent().append(get_urgent_dropdown(id,urgNone));
    }
    if(element.Type2) {
        $('#' + id).parent().append('&nbsp;+&nbsp;<label for="' + id + '"'+ tooltipHTML +'>' + element.Type2 + '</label>' + get_urgent_dropdown(sanitize_id_name(element.Type2)));
    }
});
...
$(document).on("click keyup change", ".uber_input, .urgent_types", event => {
    ...
    workflow = workflow.replace(/\sNon-URG/g,""); // workaround for DTR
...
function get_urgent_dropdown(field,none) {
