// ==UserScript==
// @name         GoztePlace - Enhanced Wplace Overlay
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  Enhanced fork of Wplace Overlay Pro with additional features. Overlays tiles on wplace.live. Can also resize, and color-match your overlay to wplace's palette. Make sure to comply with the site's Terms of Service, and rules! This script is not affiliated with Wplace.live in any way, use at your own risk.
// @author       gozteplace (based on shinkonet's Wplace Overlay Pro)
...
// @downloadURL https://update.greasyfork.org/scripts/545928/GoztePlace%20-%20Enhanced%20Wplace%20Overlay.user.js
// @updateURL https://update.greasyfork.org/scripts/545928/GoztePlace%20-%20Enhanced%20Wplace%20Overlay.meta.js
// ==/UserScript==
(() => {
  /*
   * GOZTEPLACE - Enhanced Wplace Overlay Userscript
   * Based on Wplace Overlay Pro by shinkonet
   * 
   * This userscript provides overlay functionality for wplace.live with additional enhancements.
   * 
   * ARCHITECTURE OVERVIEW:
   * =====================
   * - Core Modules: Fundamental functionality (storage, canvas, palette, etc.)
   * - UI Modules: User interface components (panels, modals, styling)
   * - Application: Bootstrap and main entry point
   * 
   * MAIN FEATURES:
   * ==============
   * - Overlay image placement on wplace.live canvas
   * - Color matching to wplace palette
   * - Multiple overlay support with opacity control
   * - Resize and positioning tools
   * - Template loading from URLs
   * - Cross-platform GreaseMonkey compatibility
   * 
   * The code is organized into modular sections as indicated by the comments below.
   */

  // =============================================================================
  // CORE MODULE: GreaseMonkey API Wrappers (src/core/gm.ts)
  // =============================================================================
  // This module provides cross-platform compatibility for GreaseMonkey APIs
  // Handles differences between GM and GM_ prefixed functions
  
  var NATIVE_FETCH = window.fetch; // Store reference to original fetch function
  // Cross-platform getter for GreaseMonkey storage
  // Tries modern GM.getValue first, falls back to legacy GM_getValue
  var gmGet = (key, def) => {
    try {
    ...
    return Promise.resolve(def); // Return default value if all methods fail
  };
  // Cross-platform setter for GreaseMonkey storage
  // Tries modern GM.setValue first, falls back to legacy GM_setValue
  var gmSet = (key, value) => {
    try {
    ...
    return Promise.resolve(); // Silent fail if no storage method available
  };
  // Fetch URL as blob using GreaseMonkey's xmlhttpRequest
  // This bypasses CORS restrictions that regular fetch() might encounter
    ...
    return new Promise((resolve, reject) => {
      try {
        GM_xmlhttpRequest({
          method: "GET",
          url,
          responseType: "blob", // Request binary data as blob
          onload: (res) => {
  ...
  // Convert blob to base64 data URL for embedding in HTML/Canvas
  function blobToDataURL(blob) {
    return new Promise((resolve, reject) => {
      const fr = new FileReader();
      fr.onload = () => resolve(String(fr.result)); // Result is data:image/type;base64,data
      fr.onerror = reject;
      fr.readAsDataURL(blob); // Reads blob as base64-encoded data URL
    });
  }
  ...
  // =============================================================================
  // CORE MODULE: Color Palette Management (src/core/palette.ts)
  // =============================================================================
  // This module handles color palettes for wplace.live and provides color matching functionality
  
  // Free tier color palette for wplace.live (RGB values)
  // These are the exact colors available to users on the free plan
  var WPLACE_FREE = [
    [0, 0, 0],
    [60, 60, 60],
    [120, 120, 120],
  ...
  // =============================================================================
  // CORE MODULE: Configuration Store (src/core/store.ts)
  // =============================================================================
  // This module manages application configuration and state persistence
  
  // Main configuration object that holds all application state
  // This gets saved to/loaded from GreaseMonkey storage
  var config = {
    overlays: [],
  ...
  // Load configuration from GreaseMonkey storage
  // Loads all config keys and applies default values if not found
  ...
  // Save configuration to GreaseMonkey storage
  // Optionally specify which keys to save (defaults to all)
  ...
  // =============================================================================
  // UI MODULE: CSS Styles (src/ui/styles.ts)
  // =============================================================================
  // This module contains all CSS styling for the userscript interface
  ...
  // =============================================================================
  // UI MODULE: Color Correction Modal (src/ui/ccModal.ts)
  // =============================================================================
  // This module provides the color correction interface for matching overlay colors to wplace palette
  ...
  // =============================================================================
  // UI MODULE: Resize Modal (src/ui/rsModal.ts)
  // =============================================================================
  // This module provides the resize/scale interface for adjusting overlay dimensions
  ...
  // =============================================================================
  // UI MODULE: Main Control Panel (src/ui/panel.ts)
  // =============================================================================
  // This module creates and manages the main overlay control panel interface
  var panelEl = null;
  function $(id) {
  ...
  // =============================================================================
  // APPLICATION MODULE: Main App Bootstrap (src/app.ts)
  // =============================================================================
  // This module handles application initialization and template loading from URLs
  ...
  // =============================================================================
  // ENTRY POINT: Main Application Entry (src/main.ts)
  // =============================================================================
  // This is the main entry point that starts the application when the page loads
  (function() {
    "use strict";
    window.onload = () => {
      bootstrapApp().catch(
