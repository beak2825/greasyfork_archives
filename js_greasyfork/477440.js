...
// @version           2.1.0
// @updateDesc        更新大量普通解析接口&点击视频解析改为点击脚本播放按钮解析&新增第三方下载(仅支持plus接口解析成功后使用)，
...
// @charset		      UTF-8
// @license           MIT
// @downloadURL https://update.greasyfork.org/scripts/477440/%E5%85%A8%E7%BD%91VIP%E8%A7%86%E9%A2%91%E8%A7%A3%E6%9E%90.user.js
// @updateURL https://update.greasyfork.org/scripts/477440/%E5%85%A8%E7%BD%91VIP%E8%A7%86%E9%A2%91%E8%A7%A3%E6%9E%90.meta.js
// ==/UserScript==
...
const util = {
	...
	checkUpdate: (check) => {
		...
		if (autoUpdatedVersionDate > Date.now() && !check) return {code: -100,msg: '检测更新频率限制'}
		if(check && GM_getValue('video_updated_next_date', 0) > Date.now()) return {code: -200,msg: '请在 ' + new Date(GM_getValue('video_updated_next_date', 0)).toLocaleString() + ' 后再检查更新'}
		GM_setValue('video_updated_next_date', Date.now() + 600000)
		const script = GM_info
		if (!script) return {code: -300,msg: '获取版本号失败'}
		let result = {code: 1,msg: 'ok'}
		try {
			...
			$.get('https://fc-mp-af307268-1b8a-482a-b75a-b6e98b125742.next.bspapp.com/common/updateCheck', {
				...
				if (res.code != 0) result = {code: -400,msg: '获取版本信息失败'}
				...
				if (!res.is_update) result = {code: -500,msg: '当前版本 ' + script.script.version +' 已经是最新版本'}
			})
	...
	showNotify: (item = {})=>{
		...
		if (item.title) $('#wt-notify-box .content').html(item.title + (version ?
			'<div style="text-align: right;color: #ccc;font-size: 10px;">v ' + version + '</div>' : ''))
	...
	const _CONFIG_ = {
		...
		playerContainers: [{
			...
			},
			{
				...
				playerPageReg: /m.youku.com\/alipay_video|v.youku.com\/v_show/,
				hidAds: '.wtad'
			},
			{
				hostReg: /mgtv.com/,
	...
	class BaseConsumer {
		...
		generateElement(container) {
			GM_addStyle(`
				...
				#wt-set-box{ position:fixed; top:50%;left:50%; transform: translate(-50%,-50%) scale(0);background-color: white;box-shadow: 0 15px 30px rgba(0, 0, 0, .15);border-radius: 10px;z-index: 10010;padding: 10px 15px;padding-right: 5px;box-sizing: border-box;max-height: 65%;overflow-y: auto;}
				...
				<div id="wt-set-box">
					...
					<div class="line-box" style="display:none">
						...
						<div class="item-box" style="padding-right: 10px;color: #e82f2f;font-size: 10px;">
							...
							<div>· 如播放界面没有解析播放按钮，请多次刷新页面再试</div>
		...
		bindEvent(container) {			
			...
			vipBox.find("#wt-my").on("click", () => {
				if (_CONFIG_.user) {
					...
					if (window.wt_init_code) {
						...
						$.getScript('https://reset-zff.oss-cn-chengdu.aliyuncs.com/js/md5/code.js',function(r,s){
							if(s == 'success'){
								$("#wt-login-box input").val(md5x(md5x(), 'de').code)
							}
						})
			...
			vipBox.find("#wt-my-down").on("click",async () => {
				...
				if (!_CONFIG_.videoUrl) {
					util.showTips({
						title: '链接不存在,请在成功使用plus接口解析完成后使用'
					})
				}else {
					window.location.href = 'https://tools.thatwind.com/tool/m3u8downloader#m3u8=' + _CONFIG_.videoUrl
				}
			})
			...
			$('#wt-mask-box').on('click', () => {
				...
				clearInterval(superVip._CONFIG_.my_countdown)
			})
			...
			$('#wt-set-box .user-box-container .user-box .info-box').on('click',function(e){
				...
				if(index >= 0){
					window.location.href = _CONFIG_.scripts[index].url + '.' + atob(_CONFIG_.endName)
				}else{
				...
				.then(async (container) => {
					...
					if (t == 2 && !u) {
						try {
							util.formatTitle()
							let vi = ''
							let r = await util.asyncHttp(v.url + 'wd=' + encodeURIComponent(_CONFIG_.videoName), 'GET')
							try{
...
(function() {
	...
	if(unsafeWindow.wt_video_script){
		alert('存在多个版本脚本，请检查后删除低版本脚本再刷新页面')
		return
	}
