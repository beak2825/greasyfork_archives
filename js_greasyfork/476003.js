...
// @grant        GM_listValues
// @grant        GM_info
// @downloadURL https://update.greasyfork.org/scripts/476003/%E6%94%B9%20%E8%B4%B4%E5%90%A7%E9%A1%B5%E9%9D%A2%E7%B2%BE%E7%AE%80.user.js
// @updateURL https://update.greasyfork.org/scripts/476003/%E6%94%B9%20%E8%B4%B4%E5%90%A7%E9%A1%B5%E9%9D%A2%E7%B2%BE%E7%AE%80.meta.js
// ==/UserScript==
...
(function() {
    ...
    function appendCSS(){
        ...
        if (homePageProcess){
            if (homePageMatch.test(window.location.href)){
                ...
                //贴吧首页右侧广告、辅助模式
                cssText += '#right_wrap {display:none !important;} #spage-tbshare-container{display:none !important;} ul.tbui_aside_float_bar{display:none !important;}';
                //头部导航内容
                cssText += '.search_top{visibility:hidden !important;}'
                //左边贴吧分类
                cssText +='.u-f-t.ufw-gap, #f-d-w{display:none !important;}'
                //搜索前6个隐藏(.operation_item)、个性动态
                cssText +='li:has(> div.operation_item),li.break_title:has(> span.break_tip),  .sub_nav_list>li:nth-child(4){display:none !important;}'
                //隐藏个性动态
                //cssText +='li:has(> div.operation_item),li.break_title:has(> span.break_tip), .sub_nav_list>li:nth-child(-n+2), .sub_nav_list>li:nth-child(4){display:none !important;}'
                //左边个人信息
                cssText += '.left-sec{display:none !important;} .page-container .content-sec{background:none!important;}'
                //热门吧改ui
                //cssText +='.page-container .main-sec{width:90% !important;}'
                cssText += '.page-container .content-sec, .page-container .right-sec, #forum_rcmd{width:auto !important;}'
                //个性动态改ui
                cssText += '.page-container .r-left-sec{width:100% !important;}'
                cssText += '.sub_nav_wrap{background:none!important;width:auto !important;display:flex;justify-content:center;}'
                cssText += '.new_list li{display:flex;justify-content:center;font-size:16px;} .n_right{width:600px !important;}'
                //贴吧名 字体大小
                cssText += '.n_right .title-tag-wraper .n_name{font-size:20px !important;}'
                //帖子标题 字体大小
                cssText += '.n_right .thread-name-wraper .title{font-size:18px !important;}'
                //帖子内容 字体大小
                cssText += '.n_right  .n_txt{font-size:16px !important;}'
                //帖子作者与时间 字体大小
                cssText += '.n_right .n_reply{font-size:14px !important;}'
            }
        }
   ...
   function cleanAD(){
        var ads = document.querySelectorAll('.clearfix.j_feed_li.home-place-item');
        var count = ads.length;
        addCss(`
        .clearfix.j_feed_li.home-place-item {
          display:none;
          visibility: hidden;
        }
        `)
        console.log("My贴吧页面精简: 清理了"+count+"条动态内的广告");
    }
