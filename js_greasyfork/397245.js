// ==UserScript==
// @name        imdb_everywhere
// @name:zh-CN         imdb_everywhere
// @name:zh-TW         imdb_everywhere
// @name:ja        imdb_everywhere
// @namespace   imdb_everywhere
// @supportURL  https://github.com/zhuzemin
// @description:zh-CN  if page has imdb link, display content from that imdb link.
// @description:zh-TW  if page has imdb link, display content from that imdb link.
// @description:ja  if page has imdb link, display content from that imdb link.
// @description if page has imdb link, display content from that imdb link.
// @include     https://*
// @include     http://*
// @exclude     https://www.imdb.com/*
// @exclude     *://*.jpg
// @exclude     *://*.gif
// @exclude     *://*.png
// @exclude     *://*.mp4
...
// @connect-src www.imdb.com

// @downloadURL https://update.greasyfork.org/scripts/397245/imdb_everywhere.user.js
// @updateURL https://update.greasyfork.org/scripts/397245/imdb_everywhere.meta.js
// ==/UserScript==
var config = {
    'debug':true
}
...
var init = function () {
    if(window.self === window.top) {
        ...
        for(var a of aList) {
            if(/(https:\/\/www\.imdb\.com\/name\/nm\d*)|(https:\/\/www\.imdb\.com\/title\/tt\d*\/?$)/.test(a.href)){
                debug('a.href: ' + a.href);
                let reqObj=new ObjectRequest(a.href);
                reqObj.package=a;
                request(reqObj,handle_imdb);
            }
...
function handle_imdb(response,object) {
    var dom = new DOMParser().parseFromString(response.responseText, "text/html");
    let content;
    if(/(https:\/\/www\.imdb\.com\/name\/nm\d*)/.test(response.finalUrl)){
        let div=dom.querySelector('div[data-category="actor"]')||dom.querySelector('div[data-category="actress"]');
        content=div.nextElementSibling;
    }
    else if(/(https:\/\/www\.imdb\.com\/title\/tt\d*\/?$)/.test(response.finalUrl)){
        content=dom.querySelector('div.title_block');

    }
    if(content!=null){
        content.style = 'background-color:#D8E0E0;border: 3px solid green;';
        for (var a of content.querySelectorAll('a')) {
            a.href = a.href.replace(getLocation(window.location.href).hostname, 'www.imdb.com');
        }
        let targetElem=object.package;
        targetElem.parentElement.insertBefore(content,targetElem.nextElementSibling);
    }
}
