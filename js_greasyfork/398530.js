// ==UserScript==
// @name           	GLB Ticket Price Assistant - pabst edit - SM Tweak https
// @namespace      	http://mmofootball.tk/
// @description    	Helps you set prices of tickets.
// @include       	http://glb.warriorgeneral.com/game/stadium.pl?team_id=*
// @include       	https://glb.warriorgeneral.com/game/stadium.pl?team_id=*
// @include             http://glb.warriorgeneral.com/game/team_finances.pl?team_id=*
// @include             https://glb.warriorgeneral.com/game/team_finances.pl?team_id=*
// @copyright           2013, AirMcMVP
// @version		2014.05.05
// @downloadURL https://update.greasyfork.org/scripts/398530/GLB%20Ticket%20Price%20Assistant%20-%20pabst%20edit%20-%20SM%20Tweak%20https.user.js
// @updateURL https://update.greasyfork.org/scripts/398530/GLB%20Ticket%20Price%20Assistant%20-%20pabst%20edit%20-%20SM%20Tweak%20https.meta.js
// ==/UserScript==

// pabst fixed stuff 4/25/2014
...
function setTicketPrices() {
    ...
    var loc = new Array();
    if (window.location.href.indexOf('stadium.pl') != -1) {
        loc = document.getElementById('content_pricing').getElementsByTagName('tbody')[1].getElementsByClassName('seating_chart_value');
    } 
    else {
    ...
    for (i = 0; i < 10; i++) {
        if (document.getElementsByClassName('percentHolder')[i].id != 'locked') {
            loc[i].firstChild.value = (suggested[i] * percent).toFixed(2);
            loc[i].firstChild.textContent = (suggested[i] * percent).toFixed(2);
        }
    }
    updatePercentages();
}
...
function updatePercentages() {
    ...
    for (i = 0; i < 10; i++) {
        percent[i] = parseInt((loc[i].firstChild.textContent.replace('$', '') / suggested[i]) * 100);
        percent[i] = parseInt((loc[i].firstChild.value.replace('$', '') / suggested[i]) * 100);
    }
