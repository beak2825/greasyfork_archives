...
(function () {
  ...
  let selectedFilesList = []; // 保存已选择的文件列表（用于文件选择上传时合并）
  let fileUrlMap = new Map(); // 存储文件对象到URL的映射（用于抓取时去重）
  ...
  function removeAttachmentByIndex(index) {
    ...
    // 使用 selectedFilesList 作为源，如果没有则从 input 获取
    const files = selectedFilesList.length > 0
      ? selectedFilesList
      : (attachmentInput.files ? Array.from(attachmentInput.files) : []);

    if (files.length === 0 || index < 0 || index >= files.length) return;

    // 获取要删除的文件，并从 fileUrlMap 中移除
    const fileToRemove = files[index];
    if (fileToRemove) {
      fileUrlMap.delete(fileToRemove);
    }
    ...
    // 更新保存的文件列表
    selectedFilesList = Array.from(dataTransfer.files);
  ...
  function bindWorkOrderEvents() {
    ...
    // 重命名文件为 image_时间戳 格式
    function renameFileWithTimestamp(file, index = 0) {
      const timestamp = Date.now();
      // 获取文件扩展名
      const originalName = file.name;
      const lastDotIndex = originalName.lastIndexOf(".");
      const extension = lastDotIndex > 0 ? originalName.substring(lastDotIndex) : "";
      // 生成新文件名：image_时间戳.扩展名（多个文件时添加索引）
      const newFileName = index > 0
        ? `image_${timestamp}_${index}${extension}`
    ...
    function handleFileSelect(newFiles) {
      ...
      // 获取现有文件（优先使用 selectedFilesList，如果没有则从 input 获取）
      const existingFiles = selectedFilesList.length > 0
        ? selectedFilesList
        : (attachmentInput.files ? Array.from(attachmentInput.files) : []);
    ...
    // 附件输入框变化事件（支持多文件选择，合并而不是覆盖，保持原文件名）
    attachmentInput.onchange = function (e) {
      if (e.target.files && e.target.files.length > 0) {
        ...
        // 合并文件（避免重复）
        const allFiles = [...selectedFilesList];
        newFiles.forEach((newFile) => {
          // 检查是否已存在同名文件（只判断文件名）
          const isDuplicate = selectedFilesList.some(
            (existing) => existing.name === newFile.name
          );
          if (!isDuplicate) {
            allFiles.push(newFile);
    ...
    attachmentArea.addEventListener("drop", function (e) {
      ...
      if (files && files.length > 0) {
        ...
        const renamedFiles = filesArray.map((file, index) => renameFileWithTimestamp(file, index));
        handleFileSelect(renamedFiles);
      }
    });
    ...
    document.addEventListener("paste", function (e) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (item.kind === "file") {
          ...
          if (file) {
            // 使用重命名函数
            const renamedFile = renameFileWithTimestamp(file, pastedFiles.length);
  ...
  async function getFormData() {
    ...
    // 保存附件信息（转换为 base64）
    const attachmentInput = document.getElementById("attachment");
    // 优先使用 selectedFilesList（包含修改后的文件名），如果没有则使用 attachmentInput.files
    const files = selectedFilesList.length > 0
      ? selectedFilesList
      : (attachmentInput && attachmentInput.files ? Array.from(attachmentInput.files) : []);
    if (files.length > 0) {
      ...
      if (draftData) {
        ...
        if (
          ...
          if (attachmentInput) {
            ...
            attachmentInput.files = dataTransfer.files;
            // 同时更新 selectedFilesList，确保恢复的文件名被正确保存
            selectedFilesList = Array.from(dataTransfer.files);
            updateAttachmentPreview();
          }
        }
  ...
  async function submitWorkOrder() {
    ...
    if (hasAttachment) {
      ...
      // 优先使用 selectedFilesList（包含修改后的文件名），如果没有则使用 attachmentField.files
      const files = selectedFilesList.length > 0
        ? selectedFilesList
  ...
  async function captureShulexPageInfo() {
    ...
    try {
      ...
      let orderNumberPart1 = null;
      let orderNumberPart2 = null;

      // 尝试从标准位置获取订单号第一部分
      if (infoRowValueElements.length >= 3) {
        orderNumberPart1 = infoRowValueElements[2].textContent.trim();
        console.log('orderNumberPart1 from infoRowValueElements:', orderNumberPart1);
      }

      // 如果第一部分为空或者是 '- -'，尝试从店铺标签获取
      if (!orderNumberPart1 || orderNumberPart1 === '- -') {
        const shopDiv = document.querySelector('.pHlXRyo');
        console.log('shopDiv:', shopDiv);
        const shulex_shop = shopDiv?.querySelector('.ant-tag');
        console.log(shulex_shop?.textContent || '未找到店铺标签');
        if (shulex_shop?.textContent) {
          orderNumberPart1 = shulex_shop.textContent.trim();
          console.log('orderNumberPart1 from shop tag:', orderNumberPart1);
        }
      }

      // 尝试获取订单号第二部分
      if (copyClickableElements.length >= 2) {
        orderNumberPart2 = copyClickableElements[1].textContent.trim();
        console.log('orderNumberPart2:', orderNumberPart2);
      }

      // 如果两部分都有值，组合成完整订单号
      if (orderNumberPart1 && orderNumberPart2) {
        ...
        console.log("未找到完整的订单号信息，继续处理其他信息");
        if (orderNumberPart1) {
          console.log("找到订单号第一部分:", orderNumberPart1);
        }
        if (orderNumberPart2) {
          console.log("找到订单号第二部分:", orderNumberPart2);
        }

        // 如果抓取不到订单号，尝试获取SKU值填充到SKU字段中
        try {
          const typographyElements = document.querySelectorAll('.ant-typography');
          if (typographyElements.length >= 2) {
            const skuValue = typographyElements[1].textContent.trim();
            if (skuValue) {
              const skuInput = document.getElementById("sku");
              if (skuInput && !skuInput.value.trim()) {
                skuInput.value = skuValue;
                console.log("从shulex页面抓取到SKU:", skuValue);
                successMessages.push(`成功抓取SKU: ${skuValue}`);
              }
            }
          }
        } catch (error) {
          console.error("抓取SKU失败:", error);
        }
      }
    } catch (error) {
  ...
  async function extractOrderInfoFromIframe() {
    try {
      ...
      if (!orderId) {
        try {
          const spans = document.querySelectorAll('.product-detail .text');
          const targetSpan = Array.from(spans).find(span =>
            span.textContent.includes('仓库SKU：')
          );
          const a = targetSpan ? targetSpan.querySelector('a') : null;
          skuValue = a ? a.textContent.trim() : null;
          if (skuValue) {
            console.log("从易仓iframe页面抓取到SKU:", skuValue);
          }
        } catch (error) {
          console.error("在iframe中抓取SKU失败:", error);
        }
      }
      ...
      const orderInfo = {
        ...
        success: !!orderId || !!skuValue || (imageUrls && imageUrls.length > 0),
      };
    ...
    try {
      ...
      if (
        ...
        if (orderIdInput) {
          ...
          // 触发获取订单信息并填充SKU
          getOrderSkusByOrderId();
        } else if (orderInfo.skuValue) {
          // 如果没有订单号但有SKU值，填充SKU
          const skuInput = document.getElementById("sku");
          if (skuInput && !skuInput.value.trim()) {
            skuInput.value = orderInfo.skuValue;
            console.log("从易仓缓存信息获取到SKU:", orderInfo.skuValue);
          }
        } else {
          // 如果既没有订单号也没有从缓存获取到SKU，尝试从当前页面获取SKU
          try {
            const spans = document.querySelectorAll('.product-detail .text');
            const targetSpan = Array.from(spans).find(span =>
              span.textContent.includes('仓库SKU：')
            ...
            const a = targetSpan ? targetSpan.querySelector('a') : null;
            const skuValue = a ? a.textContent.trim() : null;
            if (skuValue) {
              const skuInput = document.getElementById("sku");
              if (skuInput && !skuInput.value.trim()) {
                skuInput.value = skuValue;
                console.log("从易仓当前页面抓取到SKU:", skuValue);
              }
            }
          } catch (error) {
            console.error("从当前页面抓取SKU失败:", error);
          }
        }
      ...
      return new Promise((resolve, reject) => {
        ...
        const messageHandler = (event) => {
          if (event.data && event.data.type === "ORDER_INFO_EXTRACTED") {
            ...
            // 如果既没有订单号也没有图片，尝试获取SKU，如果也没有SKU则认为失败
            if (
              !orderInfo.success ||
              (!orderInfo.orderId &&
                !orderInfo.skuValue &&
              ...
              try {
                ...
                if (skuValue) {
                  ...
                  if (skuInput && !skuInput.value.trim()) {
                    ...
                    console.log("从易仓当前页面抓取到SKU:", skuValue);
                    hasSku = true;
                    showResultTips(`未找到订单号，但成功抓取SKU: ${skuValue}`);
                    if (capturePageInfoBtn) {
                      capturePageInfoBtn.disabled = false;
                      capturePageInfoBtn.textContent = "捕获当前页面信息";
                    }
              ...
              if (!hasSku) {
                const errorMsg = "无法从iframe中提取订单信息、SKU或图片";
            ...
            if (orderInfo.orderId) {
              ...
              if (skuInput && !skuInput.value.trim()) {
                skuInput.value = orderInfo.skuValue;
                console.log("从易仓iframe获取到SKU:", orderInfo.skuValue);
              }
            } else {
              // 如果既没有订单号也没有从iframe获取到SKU，尝试从当前页面获取SKU
              try {
                const spans = document.querySelectorAll('.product-detail .text');
                const targetSpan = Array.from(spans).find(span =>
                  span.textContent.includes('仓库SKU：')
  ...
  async function addImagesToAttachments(imageUrls) {
    ...
    // 获取现有文件（优先使用 selectedFilesList，如果没有则从 input 获取）
    let existingFiles = selectedFilesList.length > 0
      ? selectedFilesList
      : (attachmentInput.files ? Array.from(attachmentInput.files) : []);

    // 先移除所有来自相同URL的文件（确保再次抓取时显示完整图片）
    const urlsToRemove = new Set(imageUrls);
    existingFiles = existingFiles.filter((file) => {
      const fileUrl = fileUrlMap.get(file);
      if (fileUrl && urlsToRemove.has(fileUrl)) {
        // 从映射中移除
    ...
    // 合并现有文件和新文件（抓取的文件已经通过URL去重，这里只需要合并）
    ...
    // 更新 selectedFilesList，确保手动上传时能保留抓取的文件
    selectedFilesList = allFiles;
