...
(function() {
    ...
    // Function to copy place
    function pressPieCopyPlaceButton() {
        notifyAction("Copy Place");
        const sel = WazeWrap.getSelectedFeatures();
        if (!sel.length || sel[0].WW.getType() !== "venue") {
            console.warn("WME Shortcuts+: No venue selected.");
            return;
        }
        ...
        const oldPlace = sel[0].WW.getObjectModel();

        const NewPlace = new PlaceObject({
            geoJSONGeometry: W.userscripts.toGeoJSONGeometry(
                oldPlace.getOLGeometry().clone()
            )
        });

        /* ---------- basic attributes ---------- */
        Object.assign(NewPlace.attributes, {
            name: oldPlace.attributes.name + " (kopija)",
            phone: oldPlace.attributes.phone,
            url: oldPlace.attributes.url,
            categories: [].concat(oldPlace.attributes.categories),
            aliases: [].concat(oldPlace.attributes.aliases),
            description: oldPlace.attributes.description,
            houseNumber: oldPlace.attributes.houseNumber,
            lockRank: oldPlace.attributes.lockRank,
            services: [].concat(oldPlace.attributes.services),
            openingHours: [].concat(oldPlace.attributes.openingHours),
            streetID: oldPlace.attributes.streetID
        });

        /* ---------- offset geometry ---------- */
        let c;
        const geom = NewPlace.getOLGeometry();

        if (geom.toString().startsWith("POLYGON")) {
            geom.components[0].components.forEach(p => {
                c = WazeWrap.Geometry.ConvertTo4326(p.x, p.y);
                c.lon += WazeWrap.Geometry.CalculateLongOffsetGPS(5, c.long, c.lat);
                p.x = WazeWrap.Geometry.ConvertTo900913(c.lon, c.lat).lon;
            });
        } else {
            c = WazeWrap.Geometry.ConvertTo4326(geom.x, geom.y);
            c.lon += WazeWrap.Geometry.CalculateLongOffsetGPS(5, c.long, c.lat);
            geom.x = WazeWrap.Geometry.ConvertTo900913(c.lon, c.lat).lon;
        }
        ...
        if (_.includes(NewPlace.attributes.categories, "PARKING_LOT")) {
            NewPlace.attributes.categoryAttributes.PARKING_LOT =
                _.cloneDeep(oldPlace.attributes.categoryAttributes.PARKING_LOT);
        }
        ...
        /* ---------- copy address ---------- */
        const address = oldPlace.getAddress();
        ...
        const UFA = new UpdateFeatureAddress(NewPlace, addrAttrs);
        ...
        console.log("WME Shortcuts+: Place copied.");
    }
        ...
        for (var mark in W.model.venues.objects) {
            ...
            if (venue.attributes) {
                try {
                    ...
                    if (metersArea < 500) {
                        highlightAPlace(venue, "#33ff66", "#33ff66");
    ...
    function addShortcuts() {
        ...
        new WazeWrap.Interface.Shortcut(
            'pressPieCopyPlaceButton',
            'Kopēt vietu',
            'shortcuts_plus',
            'WME Shortcuts+',
            EXTERNAL_SETTINGS.copyPlace,
            pressPieCopyPlaceButton,
            null
        ).add();
    ...
    function sandboxLoadSettings() {
        ...
        EXTERNAL_SETTINGS.clickOtherPointShortcut = loadedSettings.clickOtherPointShortcut || 'A+5';
        EXTERNAL_SETTINGS.clickOtherPolygonShortcut = loadedSettings.clickOtherPolygonShortcut || 'A+2';
        ...
        EXTERNAL_SETTINGS.copyPlace = loadedSettings.copyPlace || 'A+3'
        addShortcuts();
    ...
    function sandboxSaveSettings() {
        ...
        EXTERNAL_SETTINGS.highlightSmallVenuesShortcut = getShortcutKeys(W.accelerators.Actions.highlightSmallVenues);
        EXTERNAL_SETTINGS.wmeutkadastrsShortcut = getShortcutKeys(W.accelerators.Actions.wmeutkadastrsShortcut);
        EXTERNAL_SETTINGS.copyPlace = getShortcutKeys(W.accelerators.Actions.copyPlace);
