// ==UserScript==
// @name   The most advanced hack sploop.io
// @author Creator
// @description Xxx  mod the most advanced hack of 2025
...
// @downloadURL https://update.greasyfork.org/scripts/522299/The%20most%20advanced%20hack%20sploopio.user.js
// @updateURL https://update.greasyfork.org/scripts/522299/The%20most%20advanced%20hack%20sploopio.meta.js
// ==/UserScript==
...
class Variables {
  constructor() {
    this.loaded = false;
    const dx = {
      logo: "https://imgur.com/nc9HoL0.png"
    };
    this.images = dx;
    this.mouse = {};
    this.angles = [];
...
class RadarManager {
  ...
  ["frame"]() {
    ...
    kk.beginPath();
    kk.strokeStyle = "#f8f9f1";
    kk.shadowColor = "#f8f9f1";
    kk.shadowBlur = 15;
    kk.lineWidth = 3;
    kk.globalAlpha = 0.8;
    kk.moveTo(0, 0);
...
const mo = {
  name: "VISIT GEANE",
  icon: "bi bi-youtube",
  title: "Support sploop io god in YouTube for more mods <3",
  enabled: true
};
const Menu_Switchs = [lh, li, lj, mn, mo];
const mp = {
  name: "indicator_enemy",
...
const ms = {
  name: "Auto replace",
  title: "Auto place helper",
  enabled: true
};
const Visual_Switchs = [mp, mq, mr, ms];
const mt = {
  name: "Auto Heal",
  title: "Heals you fast",
  enabled: true
};
const mu = {
  name: "Auto break",
  title: "Break traps aotomactilly",
  enabled: true
};
const mv = {
  name: "Auto place",
...
function pageHooks(nk) {
  ...
  nl.replace("Clan Colors V2", /\w\(\w+\),"#404040"\):null/, "'#000000', '##000000') : null");
  nl.replace("Age Color", /(\("AGE 0",24,)"#fff"\)/, "$1'##000000', '##000000')");
  nl.replace("Age Body", /(background\:\w+\(\)\.\w+\([^,]+,[^,]+,[^,]+,)[^)]+\)/, "$1 '#000000')");
  nl.replace("Age Fill", /(,[^=]+=)[^,]+(,this\.\w+&&)/, "$1 '#000000' $2");
  ...
  nl.replace("Health Color & Visuals", /"#a4cc4f":"#cc5151"/, "'#000000' : '#000000',\n\n        Hooks.Tracers(...arguments)\n        Hooks.PlaceHelper(...arguments)\n    ");
  const nq = {
    values: np
  };
