// ==UserScript==
// @name         CloudFaucet.io - LTC (Pescador de Cripto)
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/413572/CloudFaucetio%20-%20LTC%20%28Pescador%20de%20Cripto%29.user.js
// @updateURL https://update.greasyfork.org/scripts/413572/CloudFaucetio%20-%20LTC%20%28Pescador%20de%20Cripto%29.meta.js
// ==/UserScript==
...
(function() {
        ...
        if($('.jconfirm-content').find('div').text().indexOf('0') > -1){
            $.ajax({
                ...
                success: function(data){
                    balance = $(data).find('.dashboard-value').eq(1).text();
                    console.log(balance);
                }
            });
            setTimeout(function(){
                message = "CloudFaucet.io\n- Sucesso: " + $('.jconfirm-content').find('div').text() + "\n- Sua balança: " + balance + "LTC";
                ntb(message);
                setTimeout(function(){
                    window.close();
                    window.history.go(0);
    ...
    function cloudFaucet(){
        setTimeout(function(){
            if($('.litecoin-faucet-button.deactive.tipso_style').attr('class').indexOf('deactive')){
                $('.litecoin-faucet-button.deactive.tipso_style').click();
    ...
    function open(){
        if (window.location.href == "https://pescadordecripto.com/dashboard/") {
            var win_cloudfaucet = window.open("https://cloudfaucet.io/faucet", "CloudFaucetLtc","width=10,height=10,left=-3000,top=-3000");
        }
    ...
    if (window.location.href == "https://pescadordecripto.com/dashboard/") {
        ...
        a.id = 'cloudfaucet-io-ltc';
        a.className = 'faucet-link faucet-active';
        a.innerHTML = 'CLOUDFAUCET.IO - LTC';
    ...
    if (window.location.href == "https://pescadordecripto.com/install/") {
        document.getElementById('cloudfaucet-io-ltc').classList.add("faucet-active");
    }
})();