// ==UserScript==
// @name Скрипт для технических проблем
// @namespace https://forum.blackrussia.online
// @version 0.1.8.0
// @description Для борьбы с техническими проблемами
// @author absolutedurak
...
// @downloadURL https://update.greasyfork.org/scripts/456279/%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D1%82%D0%B5%D1%85%D0%BD%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D1%85%20%D0%BF%D1%80%D0%BE%D0%B1%D0%BB%D0%B5%D0%BC.user.js
// @updateURL https://update.greasyfork.org/scripts/456279/%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D1%82%D0%B5%D1%85%D0%BD%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D1%85%20%D0%BF%D1%80%D0%BE%D0%B1%D0%BB%D0%B5%D0%BC.meta.js
// ==/UserScript==
...
},
{
	...
	content:
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	'[CENTER]Произошла ошибка, будете разблокированы.[/CENTER][/FONT][/SIZE]<br>'+
...
},
{
	...
	content:
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	"[CENTER]Ваша тема не как не относится к жалобам на технических специалистов, обратитесь с данной темой в <u>технический раздел вашего сервера</u> - [URL='https://forum.blackrussia.online/index.php?forums/Технический-раздел.22/']клик[/URL]<br>" +
	'[CENTER]Отказано.[/CENTER][/FONT][/SIZE]',
	prefix: UNACCEPT_PREFIX,
	status: false,
},
{
	...
	content: 
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	"[CENTER]Ваша блокировка обжалованию не подлежит вообще.<br>Вы получили блокировку за серьезное нарушение, которое невозможно обжаловать/обнулить.<br>" +
	'[CENTER]Закрыто.[/CENTER][/FONT][/SIZE]',
	prefix: UNACCEPT_PREFIX,
	status: false,
},
{
	...
	content: 
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	"[CENTER]Проверив вашу историю наказаний, было принято решение о снижении срока блокировки аккаунта.<br>Будьте аккуратнее в следующие разы, ведь на встречу пойти мы врядли сможем.<br>" +
	'[CENTER]Закрыто.[/CENTER][/FONT][/SIZE]',
	prefix: UNACCEPT_PREFIX,
	status: false,
},
{
	...
	"[CENTER]Подробнее в данной теме - [URL='https://forum.blackrussia.online/index.php?threads/lime-Защита-игрового-аккаунта.1201253/']клик[/URL][/center]<br><br>" +
...
},
{
	...
	content:
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	"[CENTER]Пожалуйста, убедительная просьба, ознакомьтесь с правилами восстановлений: https://clck.ru/NeHEQ.<br> Вы создали тему, которая не относится к технической проблеме.[/CENTER]<br><br>" +
	'[CENTER]Отказано.[/CENTER][/FONT][/SIZE]',
    prefix: UNACCEPT_PREFIX,
	status: false,
},
{
	...
	'[CENTER]В остальных же случаях, <b>если не были зачислены BLACK COINS — вероятнее всего, была допущена ошибка при вводе реквизитов</b>. К нашему сожалению, из-за большого количества попыток обмана, мы перестали рассматривать подобные жалобы. Вам необходимо быть внимательными при осуществлении покупок.<br>' +
...
},
{
	...
	content:
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	"[LEFT]Мы рассмотрели ваш запрос о возврате денежных средств и сообщаем следующее. Запуская игру, пользователь соглашается с правилами её использования, которые изложены в Пользовательском соглашении, что по смыслу ст. 435 и 438 Гражданского кодекса РФ является принятием (акцептом) оферты Компании https://blackrussia.online/oferta.php, а равно заключением договора.  Согласно Пользовательскому соглашению «Внутриигровая валюта» – виртуальная внутриигровая валюта, являющаяся неактивированными данными и командами, которая не имеет денежной стоимости и не подлежит денежной оценке, хотя и имеет цену на момент приобретения. Согласно п. Политика возврата платежей, Пользовательского соглашения денежные средства за внутриигровые товары не подлежат возврату с момента появления Внутриигровой валюты на счете аккаунта. Согласно п. Политика возврата платежей, Пользовательского соглашения пользователь самостоятельно следит за безопасностью своего аккаунта, сам несет ответственность за все действия, которые выполняются в сервисах с помощью его аккаунта, а также в нем самом. Согласно п. Политика возврата платежей, Пользовательского соглашения пользователь гарантирует, что он имеет право использовать выбранные им платежные средства, не нарушая при этом законодательства РФ и/или законодательства иной страны, гражданином которой является пользователь, и прав третьих лиц. Компания не несет ответственности за возможный ущерб третьим лицам, причиненный в результате использования пользователем не принадлежащих ему средств оплаты. Совершая покупки внутри игры, а также предоставляя платежную информацию, Вы гарантируете, что являетесь законным владельцем платёжного средства и аккаунта, связанного с данным платежом. Все действия с картой считаются совершенными с Вашего ведома и согласия, то есть лично владельцем карты. Кроме того, в соответствии законодательством РФ родители несут имущественную ответственность по сделкам малолетнего, в том числе по сделкам, совершенным им самостоятельно. Таким образом, если Вы являетесь законными представителем, Вы отвечаете за действия ребёнка внутри игры. Компания не может отслеживать действия несовершеннолетнего и нести за них ответственность." +
	'[CENTER]Закрыто.[/CENTER][/FONT][/SIZE]',
	prefix: CLOSE_PREFIX,
	status: false,
},
{
    title: 'Смена статуса/имени на ФА',
	content:
	'[SIZE=4][FONT=Veranda][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	"[CENTER]Если вы хотите изменить статус на форуме или Имя, то обратитесь в данную тему - [URL='https://forum.blackrussia.online/index.php?threads/Смена-имени-статуса-форумного-аккаунта-lime.4083573/\']клик[/URL]<br>[I][CENTER][COLOR=rgb(139, 0, 0)]Закрыто[/COLOR][/CENTER][/I].[/FONT][/SIZE]",
	prefix: CLOSE_PREFIX,
	status: false,
},
{
	...
	content:
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	"[CENTER]Ваше игровое имущество/денежные средства будут восстановлены в течение месяца. <br>Убедительная просьба, <b><u>не менять никнейм до момента восстановления</u></b>.<br>" +
...
},
{
	...
	"[CENTER]Если данная проблема все ещё актуальна, оставьте новую заявку в данном разделе.<br>" +
	'[CENTER]Закрыто.[/CENTER][/FONT][/SIZE]',
	prefix: CLOSE_PREFIX,
	status: false,
},
...
},
{
	...
	content:
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	"[CENTER]По техническим причинам данное действие невозможно, пожалуйста воспользуйтесь копированием ссылки от сюда:<br>[img]https://i.ibb.co/SX77Fgw/photo-2022-08-20-16-31-57.jpg[/img]<br>Если данный способ не помогает, то используйте сервис сокращения ссылок https://clck.ru[/CENTER]"+
	'[CENTER]Закрыто.[/CENTER][/FONT][/SIZE]',
	prefix: CLOSE_PREFIX,
	status: false,
},
...
},
{
	...
	content:
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	'[CENTER]Если вы были кикнуты из игры за "Подозрительное ПО"<br>Это могло произойти по одной из причин:<br><b>Вы полностью не остановились на метке для разгрузки/загрузки на работе дальнобойщика</b><br>При стрельбе вы постоянно нажимали на кнопку стрельбы.<br>Старайтесь избегать данных действий.<br>Решено, закрыто.[/CENTER][/FONT][/SIZE]',
	prefix: DECIDED_PREFIX,
	status: false,
},
{
...
},
{
	...
	content:
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	"[CENTER]Данная недоработка будет проверена и исправлена.<br> Спасибо, ценим Ваш вклад в развитие проекта.<br>" +
	'[CENTER]Рассмотрено.[/CENTER][/FONT][/SIZE]',
	prefix: WATCHED_PREFIX,
	status: false,
},
{
	...
	content:
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	"[CENTER]Данная недоработка была исправлена.<br>Спасибо, ценим Ваш вклад в развитие проекта.<br>" +
	'[CENTER]Рассмотрено.[/CENTER][/FONT][/SIZE]',
	status: false,
	prefix: WATCHED_PREFIX,
},
...
},
{
	...
	content:
	'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' + 
	'[CENTER]Будете выпущены, ожидайте.[/CENTER][/FONT][/SIZE]',
	prefix: CLOSE_PREFIX,
	status: false,
},
{
