// ==UserScript==
// @name         ChatGPT助手（对话大纲+提示词管理）
// @namespace    https://greasyfork.org/zh-CN/scripts/559726-chatgpt%E5%8A%A9%E6%89%8B-%E5%AF%B9%E8%AF%9D%E5%A4%A7%E7%BA%B2-%E6%8F%90%E7%A4%BA%E8%AF%8D%E7%AE%A1%E7%90%86
// @version      1.0.0
// @description  ChatGPT助手：支持聊天对话大纲、提示词管理、导入JSON提示词、标签页自动重命名。
// @description:en ChatGPT Assistant: Supports outline navigation, prompt management, JSON prompt import, tab auto-rename, scroll to top/bottom, reading history, page width adjustment.
// @author       shao51920（青野）
// @homepage     https://ncnra455tpou.feishu.cn/wiki/space/7583662503437651144?ccm_open_type=lark_wiki_spaceLink&open_tab_from=wiki_home
...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/559726/ChatGPT%E5%8A%A9%E6%89%8B%EF%BC%88%E5%AF%B9%E8%AF%9D%E5%A4%A7%E7%BA%B2%2B%E6%8F%90%E7%A4%BA%E8%AF%8D%E7%AE%A1%E7%90%86%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/559726/ChatGPT%E5%8A%A9%E6%89%8B%EF%BC%88%E5%AF%B9%E8%AF%9D%E5%A4%A7%E7%BA%B2%2B%E6%8F%90%E7%A4%BA%E8%AF%8D%E7%AE%A1%E7%90%86%EF%BC%89.meta.js
// ==/UserScript==
...
(function () {
    ...
    const SETTING_KEYS = {
        CLEAR_TEXTAREA_ON_SEND: 'chatgpt_clear_on_send',
        LANGUAGE: 'chatgpt_language',
        PAGE_WIDTH: 'chatgpt_page_width',
        OUTLINE: 'chatgpt_outline_settings',
        TAB_ORDER: 'chatgpt_tab_order',
        MODEL_LOCK: 'chatgpt_model_lock',
        PROMPTS_SETTINGS: 'chatgpt_prompts_settings',
        READING_HISTORY: 'chatgpt_reading_history_settings',
        TAB_SETTINGS: 'chatgpt_tab_settings',
    };
    ...
    const DEFAULT_TAB_SETTINGS = {
        ...
        privacyTitle: 'ChatGPT',    // 隐私模式下的伪装标题
    ...
    const DEFAULT_WIDTH_SETTINGS = {
        'chatgpt': { enabled: false, value: '1400', unit: 'px' }
    };
    ...
    class SiteAdapter {
        ...
        extractOutline(maxLevel = 6) {
            // 默认实现：从滚动容器或整个文档中提取标题
            const outline = [];
            const container = this.getScrollContainer() || document.body;
            if (!container) {
                console.log('AI超级助手: extractOutline - 未找到滚动容器');
                return outline;
            }

            console.log('AI超级助手: extractOutline - 滚动容器:', container);
            ...
            // 策略1: 优先在聊天消息内查找标题 (更精准)
            const chatSelectors = this.getChatContentSelectors();
            console.log('AI超级助手: extractOutline - 聊天选择器:', chatSelectors);

            if (chatSelectors && chatSelectors.length > 0) {
                const messages = container.querySelectorAll(chatSelectors.join(', '));
                console.log('AI超级助手: extractOutline - 找到消息数:', messages.length);

                if (messages.length > 0) {
                    messages.forEach(msg => {
                        const headings = msg.querySelectorAll(headingSelectors.join(', '));
                    ...
                    if (outline.length > 0) {
                        console.log('AI超级助手: extractOutline - 从消息中提取大纲:', outline.length, '项');
                        return outline;
                    }
                }
            }

            // 策略2: 如果消息内没找到,直接在容器中查找所有标题
            const headings = container.querySelectorAll(headingSelectors.join(', '));
            console.log('AI超级助手: extractOutline - 容器中标题数:', headings.length);
            ...
            console.log('AI超级助手: extractOutline - 最终提取大纲:', outline.length, '项');
            return outline;
        }
    ...
    /**
     * ChatGPT 适配器 (chatgpt.com / chat.openai.com)
     */
    class ChatGPTAdapter extends SiteAdapter {
        ...
        getThemeColors() {
            return { primary: '#10a37f', secondary: '#19c37d' };
        }
        ...
        getSessionId() {
            // ChatGPT URL 格式: /c/{conversation_id}
        ...
        getSessionName() {
            // 尝试从页面标题获取对话名称
            const title = document.title;
            if (title && !title.includes('ChatGPT')) {
                return title.trim();
            }
            // 回退到基类默认实现
        ...
        insertPrompt(content) {
            ...
            try {
                ...
                console.log('ChatGPT Assistant: execCommand failed', e);
            }
        ...
        extractOutline(maxLevel = 6) {
            ...
            headings.forEach(heading => {
                ...
                // 创建唯一标识(文本 + 在DOM中的位置)
                const uniqueKey = `${text}-${level}-${heading.offsetTop}`;

                if (text && level <= maxLevel && !seen.has(uniqueKey)) {
                    seen.add(uniqueKey);
                    outline.push({
                        level: level,
                        text: text,
        ...
        isGenerating() {
            ...
            // 检查加载动画
            const spinner = document.querySelector('.result-streaming') ||
        ...
        getModelName() {
            // 尝试从 UI 提取当前模型名称
            const modelButton = document.querySelector('button[id*="headlessui"]') ||
                document.querySelector('[class*="model-switcher"]');

            if (modelButton && modelButton.textContent) {
                const text = modelButton.textContent.trim();
                // 匹配 GPT-4, GPT-3.5 等
                const match = text.match(/(GPT-[\d.]+|ChatGPT|o1|o3)/i);
                if (match) {
                    return match[0];
                }
    ...
    class GeminiHelper {
        ...
        loadPrompts() {
            ...
            console.log('AI助手: 加载提示词', saved ? `${saved.length} 条` : '使用默认');
            if (!saved) {
        ...
        savePrompts() {
            console.log('AI助手: 保存提示词', `${this.prompts.length} 条`);
        ...
        createStyles() {
            ...
            style.textContent = `
                ...
                .add-prompt-btn {
                    margin: 0; padding: 10px; background: ${gradient};
        ...
        createUI() {
            ...
            // 导入 JSON 按钮
            const importBtn = createElement('button', { className: 'add-prompt-btn', id: 'import-json-btn' });
            importBtn.appendChild(createElement('span', {}, '📥'));
            importBtn.appendChild(createElement('span', {}, '导入JSON'));
            ...
            const buttonContainer = createElement('div', {
                style: 'display: flex; gap: 8px; padding: 12px; margin: 0;'
            });
            ...
            promptsContent.appendChild(buttonContainer);
            promptsContent.appendChild(fileInput);
        ...
        createSettingsContent(container) {
            ...
            // 7. 其他设置 (折叠面板) - ChatGPT 暂无特定设置
            // 保留此部分以便将来扩展 // ========== 统一管理分类顺序 ==========
            // 1. 通用设置（语言）- 已在上方添加
            // 2. 标签页设置
            ...
            // 8. 其他设置 (此部分已无内容，但保留结构以备将来扩展)
            // content.appendChild(otherSettingsSection); // 移除，因为 otherSettingsSection 不再创建
            ...
            reader.onload = (e) => {
                try {
                    ...
                    if (!Array.isArray(jsonData)) {
                        this.showToast('❌ JSON 格式错误：应为数组');
                        return;
                    }

                    let importCount = 0;
                    jsonData.forEach(item => {
                        // 验证每个提示词对象,支持 content 或 text 字段
                        const content = item.content || item.text;
                        if (item && typeof item === 'object' && item.title && content) {
                            this.addPrompt({
                                title: item.title,
                    ...
                    if (importCount > 0) {
                        this.showToast(`✅ 成功导入 ${importCount} 个提示词`);
                        this.refreshPromptList();
                        this.refreshCategories();
                    } else {
                        this.showToast('⚠️ 未找到有效的提示词数据');
                    }
                } catch (error) {
                    console.error('JSON 导入失败:', error);
                    this.showToast('❌ JSON 解析失败：' + error.message);
                }
            };
        ...
        bindEvents() {
            ...
            document.getElementById('json-file-input')?.addEventListener('change', (e) => {
                ...
                if (file) {
                    this.importPromptsFromJSON(file);
        ...
        makeDraggable() {
            ...
            document.addEventListener('mousemove', (e) => {
                if (isDragging) {
                    ...
                    xOffset = currentX;
                    yOffset = currentY;
                    panel.style.transform = `translate(${currentX}px, ${currentY}px) translateY(-50%)`;
                }
            });
    ...
    function init() {
        try {
            console.log('AI超级助手: 正在初始化...');
            // 初始化站点注册表
            const siteRegistry = new SiteRegistry();
            siteRegistry.register(new ChatGPTAdapter());

            let currentAdapter = siteRegistry.detect();

            // 如果没有匹配到特定适配器,创建通用适配器
            if (!currentAdapter) {
                console.log('AI超级助手: 未匹配到特定站点,使用通用适配器');
                // 创建一个简单的通用适配器
                currentAdapter = {
                    match: () => true,
                    getSiteId: () => 'universal',
                    getName: () => {
                        ...
                        if (host.includes('qianwen.com') || host.includes('tongyi.aliyun.com')) return '通义千问';
                        if (host.includes('yiyan.baidu.com')) return '文心一言';
                        if (host.includes('poe.com')) return 'Poe';
                        if (host.includes('metaso.cn')) return '秘塔搜索';
                        if (host.includes('felo.ai')) return 'Felo';
                        if (host.includes('huggingface.co')) return 'HuggingFace';
                        if (host.includes('lmarena.ai') || host.includes('lmsys.org')) return 'LM Arena';
                        return 'AI平台';
                    },
                    getThemeColors: () => ({ primary: '#4285f4', secondary: '#34a853' }),
                    // 扩充选择器以覆盖更多平台
                    getTextareaSelectors: () => [
                        '#prompt-textarea', // ChatGPT
                        'div[contenteditable="true"]', // 通用 contenteditable
                        'textarea[placeholder*="输入"]', // 中文通用
                        'textarea[placeholder*="Type"]', // 英文通用
                        'textarea[placeholder*="Ask"]', // 英文通用
                        'textarea[data-id]', // 某些特定属性
                        'textarea.ant-input', // Ant Design
                        'textarea.el-textarea__inner', // Element UI
                        'textarea', // 最通用
                        '.ProseMirror', // Claude, etc.
                        '[role="textbox"]' // 无障碍属性
                    ],
                    getSubmitButtonSelectors: () => ['button[type="submit"]', 'button[aria-label*="Send"]', 'button[aria-label*="发送"]', 'button:has(svg)'],
                        ...
                        for (const selector of selectors) {
                            ...
                            for (const element of elements) {
                                if (element.offsetParent !== null) { // 检查元素是否可见
                                    this.textarea = element;
                                    console.log('AI超级助手: 找到输入框', selector, element);
                                    return element;
                                }
                            }
                        }
                    ...
                    insertPrompt: function (text) {
                        if (!this.textarea) this.findTextarea();
                        if (!this.textarea) {
                            console.warn('AI超级助手: 未找到输入框');
                            return false;
                        }
                        ...
                        // 策略1: execCommand (对 contenteditable 最有效)
                        if (document.execCommand) {
                            try {
                                ...
                                console.log('AI超级助手: execCommand 失败', e);
                            }
                        }

                        // 策略2: 原生属性赋值 + 事件触发 (React/Vue 需要)
                        ...
                        if (element.tagName === 'TEXTAREA' && nativeInputValueSetter) {
                            ...
                            element.value = text;
                            element.textContent = text; // 对于 contenteditable
                        }
                    ...
                    isValidTextarea: function (element) {
                        return element && (element.tagName === 'TEXTAREA' || element.contentEditable === 'true');
                    },
                    // 添加所有缺失的 SiteAdapter 方法
                    // 改进滚动容器检测，支持更多 SPA 网站，排除侧边栏
                    getScrollContainer: () => {
                        // 尝试使用 DOMToolkit 查找
                        if (typeof DOMToolkit !== 'undefined' && DOMToolkit.findScrollContainer) {
                            const container = DOMToolkit.findScrollContainer({
                                selectors: [
                                    'main div[class*="scroll"]', // 通用滚动容器
                                    '[role="presentation"] > div[class*="scroll"]', // ChatGPT
                                    '.react-scroll-to-bottom--css-root', // 常见 React 聊天库
                                    'main',
                                    '[role="main"]',
                                    '.chat-content',
                                    '.conversation-container',
                                    '#chat-history',
                                    'div[class*="overflow-y-auto"]', // Tailwind
                            ...
                            // 额外验证：确保容器不在侧边栏内
                            if (container) {
                                const isInSidebar = container.closest('aside, nav, [class*="sidebar"], [class*="history"], [class*="drawer"]');
                                if (!isInSidebar) {
                                    console.log('AI超级助手: 找到主对话滚动容器', container);
                                    return container;
                                }
                            }
                        }
                    ...
                    // getChatContentSelectors 和 extractOutline 使用基类的默认实现
                    getSessionId: () => window.location.pathname,
                    getWidthSelectors: () => [],
                    supportsTabRename: () => false,
                    getDefaultLockSettings: () => ({ enabled: false, modelKeyword: '' }),
                    supportsNewTab: () => false,
                    supportsScrollLock: () => false,
                    isNewConversation: () => false,
                    isGenerating: () => false,
                    getModelName: () => null,
                    getNetworkMonitorConfig: () => null,
                    afterPropertiesSet: () => { },  // 空实现
                    getVisibleAnchorElement: () => null,
                    restoreScroll: () => false
                };
                // 使用 register() 方法注册通用适配器，而不是直接设置 .current
                // 这样 siteRegistry.detect() 就能找到它
                siteRegistry.register(currentAdapter);
                currentAdapter = siteRegistry.detect();
            }

            console.log(`AI超级助手: 已匹配站点 - ${currentAdapter.getName()}`);

            setTimeout(() => {
                try {
                    console.log('AI超级助手: 创建实例...');
                    window.geminiHelper = new GeminiHelper(siteRegistry);
                    console.log('AI超级助手: 实例创建成功');
                } catch (error) {
                    console.error('AI超级助手: 启动失败 (Constructor Error)', error);
                }
            }, 2000);
        } catch (e) {
            console.error('AI超级助手: 初始化失败 (Init Error)', e);
        }
    }
