...
// @downloadURL https://update.greasyfork.org/scripts/541911/123%E4%BA%91%E7%9B%98%E7%A7%92%E4%BC%A0%E9%93%BE%E6%8E%A5%EF%BC%88%E9%AD%94%E6%94%B9%E7%89%88%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/541911/123%E4%BA%91%E7%9B%98%E7%A7%92%E4%BC%A0%E9%93%BE%E6%8E%A5%EF%BC%88%E9%AD%94%E6%94%B9%E7%89%88%EF%BC%89.meta.js
// ==/UserScript==
...
(function() {
    ...
    const coreLogic = {
        ...
        batchTransferAllPublicRepoResources: async function(startIdx, endIdx) {
            processStateManager.reset();
            uiManager.showModal("📥 批量转存公共资源库", `
                <div class="fastlink-progress-container"><div class="fastlink-progress-bar" style="width: 0%"></div></div>
                <div class="fastlink-status"><p>正在获取公共资源库列表...</p></div>
                <div class="fastlink-stats"><span class="success-count">✅ 成功：0</span><span class="failed-count">❌ 失败：0</span></div>
                <div class="fastlink-current-file"><p class="file-name">准备中...</p></div>
            `, 'progress_stoppable', false);

            let page = 1, allShares = [], end = false, successes = 0, failures = 0, processed = 0;
            let metaStatus = 'running', metaReason = '', metaTime = Date.now();
            // 1. 拉取所有资源
            while (!end && !processStateManager.isStopRequested()) {
                try {
                    const data = await publicRepoApiHelper.listPublicShares(page);
                    if (data && data.success && Array.isArray(data.files)) {
                        allShares = allShares.concat(data.files);
                        end = data.end;
                        page++;
                        processStateManager.updateProgressUI(allShares.length, 0, successes, failures, `已获取${allShares.length}条资源`, "拉取中...");
                    } else {
                        end = true;
                    }
                } catch (e) {
                    processStateManager.appendLogMessage(`❌ 拉取第${page}页资源失败: ${e.message}`, true);
                    failures++;
                    metaStatus = 'paused'; metaReason = 'network'; metaTime = Date.now();
                    break;
                }
            }
            if (allShares.length === 0) {
                processStateManager.appendLogMessage("❌ 未获取到任何公共资源库资源", true);
                // 记录meta
                const metaKey = `fastlink_batch_meta_${startIdx||1}_${endIdx||0}_${0}`;
                localStorage.setItem(metaKey, JSON.stringify({start:startIdx||1,end:endIdx||0,total:0,finished:0,status:'failed',reason:'empty',time:Date.now()}));
                return;
            }
            allShares.sort((a, b) => {
            ...
            allShares.forEach((item, idx) => { item._sortedIndex = idx + 1; });
            let sharesToTransfer = allShares;
            if (typeof startIdx === 'number' && typeof endIdx === 'number' && startIdx > 0 && endIdx >= startIdx) {
                sharesToTransfer = allShares.slice(startIdx - 1, endIdx);
            }
            const totalCount = allShares.length;
            const rangeKey = `fastlink_batch_success_${startIdx||1}_${endIdx||allShares.length}_${totalCount}`;
            const metaKey = `fastlink_batch_meta_${startIdx||1}_${endIdx||allShares.length}_${totalCount}`;
            let finishedIdxArr = [];
            try {
                const saved = localStorage.getItem(rangeKey);
                if (saved) finishedIdxArr = JSON.parse(saved);
                if (!Array.isArray(finishedIdxArr)) finishedIdxArr = [];
            } catch(e) { finishedIdxArr = []; }
            // 记录任务启动meta
            localStorage.setItem(metaKey, JSON.stringify({start:startIdx||1,end:endIdx||allShares.length,total:totalCount,finished:finishedIdxArr.length,status:'running',reason:'',time:Date.now()}));
            // 2. 依次转存
            let interrupted = false;
            for (let i = 0; i < sharesToTransfer.length; i++) {
                if (processStateManager.isStopRequested()) { interrupted = true; metaStatus = 'paused'; metaReason = 'manual'; metaTime = Date.now(); break; }
                const share = sharesToTransfer[i];
                const globalIdx = share._sortedIndex;
                if (finishedIdxArr.includes(globalIdx)) {
                    processStateManager.appendLogMessage(`⏭️ 跳过已完成序号${globalIdx}：${share.name}`);
                    successes++;
                    processed++;
                    processStateManager.updateProgressUI(processed, sharesToTransfer.length, successes, failures, share.name);
                    continue;
                }
                try {
                    processStateManager.updateProgressUI(i, sharesToTransfer.length, successes, failures, share.name, "获取分享码...");
                    const shareCodeData = await publicRepoApiHelper.getShareCode(share.codeHash);
                    if (!shareCodeData.isFinish || !shareCodeData.message) throw new Error("获取分享码失败");
                    const longShareCode = shareCodeData.message;
                    processStateManager.updateProgressUI(i, sharesToTransfer.length, successes, failures, share.name, "转换格式...");
                    const fastLinkJsonData = await publicRepoApiHelper.transformShareCodeTo123FastLinkJson(longShareCode, share.name);
                    if (!fastLinkJsonData.isFinish || typeof fastLinkJsonData.message !== 'object') throw new Error("分享码转换为JSON失败");
                    processStateManager.updateProgressUI(i, sharesToTransfer.length, successes, failures, share.name, "转存中...");
                    await coreLogic.transferImportedJsonData(fastLinkJsonData.message, "");
                    successes++;
                    finishedIdxArr.push(globalIdx);
                    localStorage.setItem(rangeKey, JSON.stringify(finishedIdxArr));
                    // 实时更新meta
                    localStorage.setItem(metaKey, JSON.stringify({start:startIdx||1,end:endIdx||allShares.length,total:totalCount,finished:finishedIdxArr.length,status:'running',reason:'',time:Date.now()}));
                    processStateManager.appendLogMessage(`✅ 转存 "${share.name}" 成功（序号${globalIdx}）`);
                } catch (e) {
                    failures++;
                    // 检查是否网络异常
                    if (e.message && (e.message.includes('网络') || e.message.includes('超时') || e.message.includes('连接'))) {
                        interrupted = true; metaStatus = 'paused'; metaReason = 'network'; metaTime = Date.now();
                        processStateManager.appendLogMessage(`⚠️ 检测到网络异常，任务已暂停，可在任务管理中续传。`, true);
                        break;
                    }
                    processStateManager.appendLogMessage(`❌ 转存 "${share.name}" 失败: ${e.message}`, true);
                }
                processed++;
                processStateManager.updateProgressUI(processed, sharesToTransfer.length, successes, failures, share.name);
            }
            // 任务完成/暂停/中断后，写入最终meta
            let finalStatus = 'finished', finalReason = '', finalTime = Date.now();
            if (interrupted) { finalStatus = 'paused'; finalReason = metaReason; finalTime = metaTime; }
            else if (successes === sharesToTransfer.length) { finalStatus = 'finished'; finalReason = ''; finalTime = Date.now(); }
            else if (failures > 0) { finalStatus = 'failed'; finalReason = 'error'; finalTime = Date.now(); }
            localStorage.setItem(metaKey, JSON.stringify({start:startIdx||1,end:endIdx||allShares.length,total:totalCount,finished:finishedIdxArr.length,status:finalStatus,reason:finalReason,time:finalTime}));
            processStateManager.updateProgressUI(sharesToTransfer.length, sharesToTransfer.length, successes, failures, "处理完成", "");
            uiManager.showAlert(`批量转存完成：成功${successes}，失败${failures}`);
        }
    };
    ...
    const uiManager = {
        ...
        createDropdownButton: function() {
            ...
            dropdownMenu.innerHTML = `
                <ul class="ant-dropdown-menu ant-dropdown-menu-root ant-dropdown-menu-vertical ant-dropdown-menu-light" role="menu" tabindex="0" data-menu-list="true" style="border-radius: 10px;">
                    ...
                    <li class="ant-dropdown-menu-item ant-dropdown-menu-item-only-child" role="separator" style="border-top: 1px solid #eee; margin: 3px 0; padding: 0;"></li>
                    <li id="fastlink-task-manager" class="ant-dropdown-menu-item ant-dropdown-menu-item-only-child" role="menuitem" tabindex="-1" style="padding: 5px 12px;">🗂️ 任务管理</li>
            ...
            dropdownMenu.querySelector('#fastlink-task-manager').addEventListener('click', (e) => { e.stopPropagation(); dropdownMenu.style.display = 'none'; uiManager.showTaskManagerModal(); });
        ...
        showPublicRepoBrowserModal: async function() {
            ...
            // 标序号
            allFiles.forEach((item, idx) => { item._sortedIndex = idx + 1; });
            // 统计总数
            const totalCount = allFiles.length;

            // 构建内容
            let listHtml = `<div style='margin-bottom:8px;font-weight:bold;'>资源总数：${totalCount}</div>`;
            listHtml += `<div class=\"public-repo-content-area\" id=\"fl-m-public-repo-content-area\">`;
            if (totalCount === 0) {
                listHtml += `<p class=\"public-repo-empty\">暂无公共资源。</p>`;
            } else {
                listHtml += allFiles.map(item =>
                    `<div class=\"public-repo-item\" data-codehash=\"${item.codeHash}\" data-name=\"${item.name}\">`
                    + `<span style='font-weight:bold;margin-right:8px;'>${item._sortedIndex}.</span>`
                    + `<span class=\"public-repo-item-name\">${item.name}</span>`
                    + `<span class=\"public-repo-item-timestamp\" style='margin-left:8px;font-size:0.9em;color:#888;'>更新时间: ${new Date(item.timestamp).toLocaleString()}</span>`
                    + `</div>`
                ).join('');
            }
            listHtml += `</div><p id=\"fl-m-public-repo-import-status\" class=\"public-repo-import-status\"></p>`;

            this.showModal("📦 公共资源库", listHtml, 'publicRepoBrowser');

            // 事件绑定（点击选中、导入、批量转存等）
            const contentArea = this.modalElement.querySelector('#fl-m-public-repo-content-area');
            if (contentArea) {
                contentArea.querySelectorAll('.public-repo-item').forEach(itemDiv => {
                    itemDiv.onclick = () => {
                        document.querySelectorAll('.public-repo-item.selected').forEach(sel => sel.classList.remove('selected'));
                        itemDiv.classList.add('selected');
                        this.publicRepoState.selectedShareForImport = { codeHash: itemDiv.dataset.codehash, name: itemDiv.dataset.name };
                        const importStatusEl = this.modalElement.querySelector('#fl-m-public-repo-import-status');
                        if (importStatusEl) importStatusEl.textContent = `已选择 "${itemDiv.dataset.name}"`;
                    };
                });
            }
            // 绑定按钮事件（与原来一致）
            ...
            const batchBtn = this.modalElement.querySelector('#fl-m-batch-transfer-public-repo');
            if (batchBtn) {
                batchBtn.onclick = () => {
                    this.hideModal();
                    uiManager.showModal("📥 批量转存公共资源库", "将批量转存所有公共资源库资源到当前目录，可能耗时较长，是否继续？", 'confirmBatchTransferPublicRepo');
                };
            }
            // 新增：序号范围批量转存
            const batchRangeBtn = this.modalElement.querySelector('#fl-m-batch-transfer-range');
            if (batchRangeBtn) {
                batchRangeBtn.onclick = () => {
                    const startInput = this.modalElement.querySelector('#fl-m-batch-range-start');
                    const endInput = this.modalElement.querySelector('#fl-m-batch-range-end');
                    const start = parseInt(startInput.value, 10);
                    const end = parseInt(endInput.value, 10);
                    if (!start || !end || start < 1 || end < start) {
                        uiManager.showAlert('请输入有效的起止序号（起始>=1，结束>=起始）');
                        return;
                    }
                    this.hideModal();
                    coreLogic.batchTransferAllPublicRepoResources(start, end);
                };
            }
        },
        ...
        showModal: function(title, content, type = 'info', closable = true, pureLinkForClipboard = null, jsonDataForExport = null, preprocessingFailuresForLog = null, customModalElement = null) {
            ...
            if (type === 'inputLink') { htmlContent += `<button id="fl-m-confirm" class="confirm-btn">➡️ 转存</button><button id="fl-m-cancel" class="cancel-btn">取消</button>`; }
                ...
                htmlContent += `<button id="fl-m-public-repo-import-btn" class="confirm-btn">📥 导入选中项</button>`;
                htmlContent += `<button id="fl-m-batch-transfer-public-repo" class="confirm-btn" style="margin-left:10px;">📥 批量转存全部资源</button>`;
                // 新增：序号范围批量转存
                htmlContent += `<input id="fl-m-batch-range-start" type="number" min="1" style="width:60px;margin-left:10px;" placeholder="起始">-<input id="fl-m-batch-range-end" type="number" min="1" style="width:60px;" placeholder="结束">`;
                htmlContent += `<button id="fl-m-batch-transfer-range" class="confirm-btn" style="margin-left:5px;">�� 批量转存范围</button>`;
                htmlContent += `<button id="fl-m-cancel" class="cancel-btn">关闭</button>`;
            }
            ...
            else if (type === 'confirmBatchTransferPublicRepo') { htmlContent += `<button id="fl-m-confirm-batch-transfer" class="confirm-btn">确定</button><button id="fl-m-cancel" class="cancel-btn">取消</button>`; }
           ...
           const copyPreprocessingLogBtn = this.modalElement.querySelector('#fl-m-copy-preprocessing-log'); if(copyPreprocessingLogBtn && preprocessingFailuresForLog) { copyPreprocessingLogBtn.onclick = () => { const logText = preprocessingFailuresForLog.map(pf => `文件: ${pf.fileName || (pf.originalEntry&&pf.originalEntry.path)||'未知路径'}\n${(pf.originalEntry&&pf.originalEntry.etag)?('原始ETag: '+pf.originalEntry.etag+'\n'):(pf.etag?'处理后ETag: '+pf.etag+'\n':'')}${(pf.originalEntry&&pf.originalEntry.size)?('大小: '+pf.originalEntry.size+'\n'):(pf.size?'大小: '+pf.size+'\n':'')}错误: ${pf.error||'未知错误'}`).join('\n\n'); GM_setClipboard(logText); this.showAlert("预处理失败日志已复制到剪贴板！", 1500); };}
            ...
            // === 批量转存公共资源库弹窗按钮事件绑定 ===
            if (type === 'confirmBatchTransferPublicRepo') {
                const confirmBtn = this.modalElement.querySelector('#fl-m-confirm-batch-transfer');
                if (confirmBtn) {
                    confirmBtn.onclick = () => {
                        this.hideModal();
                        coreLogic.batchTransferAllPublicRepoResources();
                    };
                }
        ...
        // 新增任务管理弹窗
        showTaskManagerModal: function() {
            // 获取所有断点任务key和meta
            const tasks = [];
            for (let i = 0; i < localStorage.length; i++) {
                const key = localStorage.key(i);
                if (key && key.startsWith('fastlink_batch_meta_')) {
                    try {
                        const meta = JSON.parse(localStorage.getItem(key));
                        if (meta && typeof meta === 'object') {
                            // 查找对应的 finishedIdxArr
                            const rangeKey = `fastlink_batch_success_${meta.start}_${meta.end}_${meta.total}`;
                            let finished = 0;
                            try {
                                const arr = JSON.parse(localStorage.getItem(rangeKey));
                                if (Array.isArray(arr)) finished = arr.length;
                            } catch(e){}
                            tasks.push({ key, ...meta, finished });
                        }
                    } catch(e){}
                }
            }
            ...
            let html = `<div style='font-weight:bold;margin-bottom:8px;'>断点续传任务管理</div>`;
            if (tasks.length === 0) {
                html += `<div style='color:#888;'>暂无断点任务</div>`;
            } else {
                html += `<table style='width:100%;font-size:0.95em;'><tr><th>范围</th><th>总数</th><th>已完成</th><th>状态</th><th>原因</th><th>时间</th><th>操作</th></tr>`;
                tasks.forEach(t => {
                    let statusTxt = t.status==='running'?'进行中':(t.status==='paused'?'已暂停':(t.status==='finished'?'已完成':'失败'));
                    let reasonTxt = t.reason==='manual'?'手动暂停':(t.reason==='network'?'断网暂停':(t.reason==='error'?'异常':'-'));
                    let timeTxt = t.time?new Date(t.time).toLocaleString():'-';
                    html += `<tr><td>${t.start}-${t.end}</td><td>${t.total}</td><td>${t.finished}</td><td>${statusTxt}</td><td>${reasonTxt}</td><td>${timeTxt}</td><td>
                        <button class='task-continue-btn' data-key='${t.key}' data-start='${t.start}' data-end='${t.end}' data-total='${t.total}'>继续</button>
                        <button class='task-del-btn' data-key='${t.key}'>删除</button>
                    </td></tr>`;
                });
                html += `</table>`;
            }
            this.showModal('🗂️ 任务管理', html, 'taskManager');
            // 事件绑定
            const modal = this.modalElement;
            if (!modal) return;
            modal.querySelectorAll('.task-continue-btn').forEach(btn => {
                btn.onclick = () => {
            ...
            modal.querySelectorAll('.task-del-btn').forEach(btn => {
                btn.onclick = () => {
                    const key = btn.getAttribute('data-key');
                    if (confirm('确定要删除该任务记录吗？')) {
                        localStorage.removeItem(key);
                        // 同时删除 success 记录
                        const m = key.match(/^fastlink_batch_meta_(\d+)_(\d+)_(\d+)$/);
                        if (m) {
                            const rangeKey = `fastlink_batch_success_${m[1]}_${m[2]}_${m[3]}`;
  ...
  function formatBytes(bytes, decimals = 2) { if (bytes === 0) return '0 Bytes'; const k = 1024; const dm = decimals < 0 ? 0 : decimals; const sizes = ['Bytes', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB']; const i = Math.floor(Math.log(bytes) / Math.log(k)); return parseFloat((bytes / Math.pow(k, i)).toFixed(dm)) + ' ' + sizes[i]; }
    ...
    // === 新增：批量转存确认弹窗事件 ===
    const oldShowModal = uiManager.showModal;
    uiManager.showModal = function(title, content, type = 'info', closable = true, pureLinkForClipboard = null, jsonDataForExport = null, preprocessingFailuresForLog = null, customModalElement = null) {
        oldShowModal.call(this, title, content, type, closable, pureLinkForClipboard, jsonDataForExport, preprocessingFailuresForLog, customModalElement);
        if (type === 'confirmBatchTransferPublicRepo') {
            const confirmBtn = this.modalElement.querySelector('#fl-m-confirm-batch-transfer');
            if (confirmBtn) {
                confirmBtn.onclick = () => {
