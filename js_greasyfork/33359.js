...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/33359/ACGN%E8%82%A1%E7%A5%A8%E7%B3%BB%E7%B5%B1%E6%AF%8F%E8%82%A1%E7%87%9F%E5%88%A9%E5%A4%96%E6%8E%9B.user.js
// @updateURL https://update.greasyfork.org/scripts/33359/ACGN%E8%82%A1%E7%A5%A8%E7%B3%BB%E7%B5%B1%E6%AF%8F%E8%82%A1%E7%87%9F%E5%88%A9%E5%A4%96%E6%8E%9B.meta.js
// ==/UserScript==
...
const companyPattern = /company\/detail/;
const stockSummaryPattern = /company\/[0-9]+/;
const accountInfoPattern = /accountInfo/;
const foundationPlanPattern = /foundation\/[0-9]+/;
...
(function mainfunction() {
  ...
  setTimeout(addEvent, 5000); // 監聽main的變化並呼叫對應事件
  //setTimeout(blockAD, 500);;
})();

function addCompanyEvent() {
  addCompanyClickListener(); // 數據資訊
  // addSubscribeListener(); // 訂閱按鈕
}

function addAIEvent() {
  AddTaxListener(); // 稅率資料夾
  // addShowSubscribesListener(); // 訂閱資料夾
  detectHoldStockInfo(); // 持股資訊資料夾
}

function addSSEvent() {
  // computeAssets(); // 持股總額計算
  addStockSummaryListener(); //
}
//Header新增按鈕並監聽
function addNavItem() {
  /*
...
function addEvent() {
  $("#main").bind("DOMNodeInserted DOMNodeRemoved", function() {
    ...
    if (oldUrl === document.location.href) return;

    //設定狀態
    const wasCompany = document.location.href.search(companyPattern) !== -1;
    const wasStockSummary = document.location.href.search(stockSummaryPattern) !== -1;
    const wasAccountInfo = document.location.href.search(accountInfoPattern) !== -1;
    const wasFoundationPlan = document.location.href.search(foundationPlanPattern) !== -1;
