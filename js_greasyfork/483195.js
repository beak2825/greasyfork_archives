// ==UserScript==
// @name         B站成分查询·贴标签·三相之力·超多项指示器功能修复
// @namespace    Lss
// @version      1.2
// @description  老脚本不能用了，做了一些修改，勉强能用。由于B站API限制，‘频繁刷新页面’或者‘评论区用户过多’可能会导致插件暂时失效，等待几分钟即可。有问题请留言，理性讨论。
// @author       xulaupuz&nightswan&DLjun&Lss
...
// @license MIT
// @run-at document-end
// @downloadURL https://update.greasyfork.org/scripts/483195/B%E7%AB%99%E6%88%90%E5%88%86%E6%9F%A5%E8%AF%A2%C2%B7%E8%B4%B4%E6%A0%87%E7%AD%BE%C2%B7%E4%B8%89%E7%9B%B8%E4%B9%8B%E5%8A%9B%C2%B7%E8%B6%85%E5%A4%9A%E9%A1%B9%E6%8C%87%E7%A4%BA%E5%99%A8%E5%8A%9F%E8%83%BD%E4%BF%AE%E5%A4%8D.user.js
// @updateURL https://update.greasyfork.org/scripts/483195/B%E7%AB%99%E6%88%90%E5%88%86%E6%9F%A5%E8%AF%A2%C2%B7%E8%B4%B4%E6%A0%87%E7%AD%BE%C2%B7%E4%B8%89%E7%9B%B8%E4%B9%8B%E5%8A%9B%C2%B7%E8%B6%85%E5%A4%9A%E9%A1%B9%E6%8C%87%E7%A4%BA%E5%99%A8%E5%8A%9F%E8%83%BD%E4%BF%AE%E5%A4%8D.meta.js
// ==/UserScript==
...
(function () {
    ...
    const keyword_gzlj = "公主连结"
    const keyword_yys = "阴阳师"
    ...
    const is_new = document.getElementsByClassName('mini-header__title').length != 0 // 检测是不是新版
    ...
    const tag_blhx_Inner = "<b style='color: #4169E1'>" + tag_blhx + "</b>"
    const tag_gzlj_Inner = "<b style='color: #FF00FF'>" + tag_gzlj + "</b>"
    const tag_fgo_Inner = "<b style='color: #FF7F50'>" + tag_fgo + "</b>"
    const tag_cj_Inner = "<b style='color: #FF0000'>" + tag_cj + "</b>"
    const tag_yys_Inner = "<b style='color: #E0CC8E'>" + tag_yys + "</b>"
    ...
    const get_pid = (c) => {
        if (is_new) {
            ...
            return c.children[0].dataset.userId
        } else {
    ...
    let jiance = setInterval(() => {
        ...
        if (commentlist != 0) {
            ...
            commentlist.forEach(c => {
                ...
                if (huanta.has(pid)) {
                    if (c.textContent.includes(tag_huanta) === false) {
                        c.innerHTML += tag_huanta_Inner
                    }
                    return
                } else if (no_huanta.has(pid)) {
                    // do nothing
                    return
                }
                if (yys.has(pid)) {
                    if (c.textContent.includes(tag_yys) === false) {
                        c.innerHTML += tag_yys_Inner
                    }
                    return
                } else if (no_yys.has(pid)) {
                    // do nothing
                    return
                }
                unknown.add(pid)
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function (res) {
                        if (res.status === 200) {
                            ...
                            //vtuber
                            if (vtuber && (st.includes(keyword_jr) || st.includes(keyword_xw) || st.includes(keyword_bl) || st.includes(keyword_jl) || st.includes(keyword_nl)|| st.includes(keyword_lty) || st.includes(keyword_cy))) {
                                ...
                                c.innerHTML += "<b style='color: #FF6347' >></b>"
                            }
                            ...
                            //添加阴阳师标签
                            if (shouyou && (st.includes(keyword_yys))) {
                                c.innerHTML += tag_yys_Inner
                                yys.add(pid)
                            } else {
                                no_yys.add(pid)
                            ...
                            //添加路人标签
                            if (!st.includes(keyword_gzlj) && !st.includes(keyword_xjqx) && !st.includes(keyword_gjqt) && !st.includes(keyword_nsh) && !st.includes(keyword_zxsj) && !st.includes(keyword_jw) && !st.includes(keyword_hsh) && !st.includes(keyword_lol) && !st.includes(keyword_cod) && !st.includes(keyword_cod1) && !st.includes(keyword_yuan) && !st.includes(keyword_yuan1) && !st.includes(keyword_zhou) && !st.includes(keyword_nong) && !st.includes(keyword_huanta) && !st.includes(keyword_zhanshuang) && !st.includes(keyword_mingchao) && !st.includes(keyword_Zzz) && !st.includes(keyword_bbb) && !st.includes(keyword_bbb2) && !st.includes(keyword_bbb3) && !st.includes(keyword_td) && !st.includes(keyword_gy) && !st.includes(keyword_blhx) && !st.includes(keyword_fgo) && !st.includes(keyword_fgo1) && !st.includes(keyword_fgo2) && !st.includes(keyword_sed) && !st.includes(keyword_sed1) && !st.includes(keyword_gwlr) && !st.includes(keyword_gwlr1) && !st.includes(keyword_jr) && !st.includes(keyword_xw) && !st.includes(keyword_bl) && !st.includes(keyword_jl) && !st.includes(keyword_nl) && !st.includes(keyword_cy) && !st.includes(keyword_lty) && !st.includes(keyword_yys)) {
                                //添加小号标签
                            ...
                            //添加小号标签
                            if (c.innerHTML == "" || c.innerHTML.length < 3) {
                                c.innerHTML += tag_xh_Inner
                                xh.add(pid)
                                return
                            }
