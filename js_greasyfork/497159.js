// ==UserScript==
// @name BLACK RUSSIA TECH CURATORS SCRIPT
// @namespace https://forum.blackrussia.online
// @version 1.2
// @description Официальный скрипт на форум для руководства технического отдела BLACK RUSSIA
...
// @downloadURL https://update.greasyfork.org/scripts/497159/BLACK%20RUSSIA%20TECH%20CURATORS%20SCRIPT.user.js
// @updateURL https://update.greasyfork.org/scripts/497159/BLACK%20RUSSIA%20TECH%20CURATORS%20SCRIPT.meta.js
// ==/UserScript==
...
(function () {
	...
	{
        title: 'ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ⠀ᅠ ᅠ ᅠᅠДля ответов в жалобах на специалистов ᅠᅠ ⠀ᅠᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ',
	...
	{
		title: 'Игровой аккаунт будет обжалован',
		content:
		'[SIZE=4][FONT=Veranda][CENTER]Доброго времени суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
		"[CENTER]Блокировка вашего игрового аккаунта будет обжалована, наказание будет снято/снижено в течение 4-х часов.</u>.<br><br>[/CENTER]" +
		'[CENTER][CENTER][COLOR=rgb(127, 255, 0)]Одобрено[/COLOR], закрыто.<br>[/CENTER][/CENTER][/FONT][/SIZE]',
		prefix: DECIDED_PREFIX,
		status: false,
	},
		{
		title: 'Связался в личных сообщениях форума',
		content:
		'[SIZE=4][FONT=Veranda][CENTER]Доброго времени суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
		'[CENTER]Связался с вами в личных сообщениях форума для дальнейшего рассмотрения.[/CENTER][/FONT][/SIZE]',
		prefix: PIN_PREFIX,
		status: true,
	},
                {
		...
		content:
		'[SIZE=4][FONT=Veranda][CENTER]Доброго времени суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
		"[CENTER]После проверки доказательств от технического специалиста и системы логирования был принят вывод, что наказание выдано верно.<br><br>" +
		'[CENTER][COLOR=rgb(255, 0, 0)]Отказано[/COLOR], закрыто.[/CENTER][/FONT][/SIZE]',
		prefix: UNACCEPT_PREFIX,
		status: false,
	},
	{
		title: 'Бан за обход системы с комиссией (Perm)',
		content:
		'[SIZE=4][FONT=Veranda][CENTER]Доброго времени суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
		'[CENTER]Ваш игровой аккаунт был заблокирован в связи с тем, что вы совершили обмен автомобилей ZAZ на ZAZ с доплатой свыше 10.000.000 игровой валюты, подчёркиваю тот факт, что передача игровой валюты таким образом запрещена.<br>[/CENTER]<br>'+
		'[CENTER]Тема передана на окончательный вердикт моему руководству.[/CENTER][/FONT][/SIZE]',
		prefix: PIN_PREFIX,
		status: true,
	},
	{
	...
	{
		title: 'Аккаунт будет разблокирован (ошибка теха)',
		content:
		'[SIZE=4][FONT=Veranda][CENTER]Доброго времени суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
		'[CENTER]Ваш игровой аккаунт будет разблокирован в течение 4-х часов.<br>С техническим специалистом будут проведены профилактические меры.<br>[/CENTER]<br>'+
		'[CENTER][COLOR=rgb(127, 255, 0)]Одобрено[/COLOR], закрыто.[/CENTER][/FONT][/SIZE]',
		prefix: DECIDED_PREFIX,
		status: false,
	},
	{
	...
	{
		title: 'ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ         ᅠ⠀⠀ ⠀⠀ᅠ ᅠᅠДля ответов в техническом разделе ᅠᅠ ᅠᅠ⠀⠀⠀ ⠀ᅠ     ᅠ ᅠ     ᅠ ᅠ ᅠ ',
        dpstyle: 'oswald: 3px;     color: #fff; background: #db2309; box-shadow: 0 0 2px 0 rgba(0,0,0,0.14),0 2px 2px 0 rgba(0,0,0,0.12),0 1px 3px 0 rgba(0,0,0,0.2); border: none; border-color: #f53317',
	},
	{
		title: 'Имущество не пропало (передано игроку)',
		content:
		'[SIZE=4][FONT=Veranda][CENTER]Доброго времени суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
		"[CENTER]После детальной проверки системы логирования не подтвердилась пропажа имущества, вы передачи имущество трейдами/обменами другому-(им) игроку-(ам).<br>" +
		'[CENTER][I][COLOR=rgb(255,0,0)]Закрыто[/COLOR][/I].[/CENTER][/FONT][/SIZE]',
		prefix: UNACCEPT_PREFIX,
		status: false,
	},
