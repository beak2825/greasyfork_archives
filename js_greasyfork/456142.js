...
// @grant        none
// @unwrap
// @downloadURL https://update.greasyfork.org/scripts/456142/Bonk%20Commands.user.js
// @updateURL https://update.greasyfork.org/scripts/456142/Bonk%20Commands.meta.js
// ==/UserScript==
...
scope.censor = function(text) {
    return text.replace(RegExp('@'+Gdocument.getElementById('pretty_top_name').innerHTML), function(url) {
    return '<a style = "color:#0060C0">' + url + '</a>';
  })
};
scope.urlify = function(text) {
    ...
    return '<a href="' + url + '" target="_blank" style = "color:#00C000">' + url + '</a>';
  })
};
...
scope.displayInChat = function(message, LobbyColor, InGameColor, options, message2) {
            ...
            B.innerHTML+=censor(message2.replace(/&/g, '&amp;').replace(/>/g, '&gt;').replace(/</g, '&lt;'));
            ...
            D.innerHTML+=censor(message2.replace(/&/g, '&amp;').replace(/>/g, '&gt;').replace(/</g, '&lt;'));
...
scope.commandhandle = function(chat_val){
    if (chat_val.substring(1,6)=="echo " && chat_val.replace(/^\s+|\s+$/g, '').length>=7){
        ...
        if(scroll==false){
            ...
            Gdocument.getElementById("ingamechatcontent").style["overflow-x"]="scroll";
        }
        else if(scroll==true){
            scroll = false;
        ...
        try{
            ...
            if(typeof(lm[0].parentElement.style["parsed"])=='undefined'){
                if(lm[0].className == "ingamechatname"){
                    ...
                    lm[1].innerHTML = censor(lm[1].innerHTML);
                    Laster_message = lastmessage();
                    lm[0].parentElement.style["parsed"] = true;
                }
                if(lm[0].className == ""){
