...
// @downloadURL https://update.greasyfork.org/scripts/402625/turkopticon.user.js
// @updateURL https://update.greasyfork.org/scripts/402625/turkopticon.meta.js
// ==/UserScript==
...
function attr_html_max(n, i, max, size, bucket) {
	return pad(long_word(n), 7) + ": " + visualize_buckets(i, max, size, bucket); }
...
function insertInlineCss() {
    ...
    css += ".tob > .tom > .to_pay_buckets { display: flex; padding-left: 20px; padding-right: 10px; }\n";
...
function insertDropDowns(rai, resp) {
	for(var rid in rai) {
        ...
        if (resp[rid]["attrs"]) {
          resp[rid]["pay_buckets"] = [["$0-$4", 1], ["$4-$7", 3], ["$7-$10", 6], ["$10-$15", 10], ["$15+", 3]];
          // resp[rid]["pay_buckets"] = [["$0-$4", 0], ["$4-$7", 0], ["$7-$10", 0], ["$10-$15", 0], ["$15+", 0]];
        }
