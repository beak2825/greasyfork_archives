...
// @grant          GM_setValue
// @grant          GM_getValue
// @downloadURL https://update.greasyfork.org/scripts/476482/ani%20gamer%20video.user.js
// @updateURL https://update.greasyfork.org/scripts/476482/ani%20gamer%20video.meta.js
// ==/UserScript==
...
(function() {
   ...
   */    var hasRequiredReactDom_production_min;
    ...
    var App = function App() {
        ...
        useGmMenu("設定跳過觸發鍵", (function() {
            var key = prompt("請輸入跳過觸發鍵", gamerSkipKey);
            if (key) {
                if (key.length > 1) {
                    alert("只能輸入一個字");
                    return;
                ...
                alert("已設定跳過觸發鍵為 ".concat(key));
            }
        }));
        var defaultGamerOPTime = 90;
        var _useGmValue2 = useGmValue("gamerOPTime", defaultGamerOPTime), gamerOPTime = _useGmValue2.data, updateGamerOPTime = _useGmValue2.updateData;
        ...
        var gamerVideoRatePool = [ .5, .75, 1, 1.25, 1.5, 1.75, 2 ];
        var defaultGamerVideoRate = 1.5;
        var _useGmValue3 = useGmValue("gamerVideoRate", defaultGamerVideoRate), gamerVideoRate = _useGmValue3.data, updateGamerVideoRate = _useGmValue3.updateData;
        useGmMenu("設定影片速度", (function() {
            ...
            if (rate) {
                ...
                if (gamerVideoRatePool.includes(rateNumber)) {
                    ...
                    alert("請輸入正確的速度 [".concat(gamerVideoRatePool.toString(), "]"));
                }
            }
        }));
        var defaultFullScanKey = "f";
        var _useGmValue4 = useGmValue("fullScanKey", defaultFullScanKey), fullScanKey = _useGmValue4.data, updateFullScanKey = _useGmValue4.updateData;
        useGmMenu("設定全螢幕切換鍵", (function() {
            var key = prompt("請輸入全螢幕切換鍵", fullScanKey);
            if (key) {
                if (key.length > 1) {
                    alert("只能輸入一個字");
                    return;
                ...
                alert("已設定全螢幕切換鍵為 ".concat(key));
            }
        }));
        var defaultAutoNext = true;
        var _useGmValue5 = useGmValue("autoNext", defaultAutoNext), autoNext = _useGmValue5.data, updateAutoNext = _useGmValue5.updateData;
        useGmMenu("設定自動切換下一集", (function() {
            var isAutoNext = confirm("是否啟用自動切換下一集");
            updateAutoNext(isAutoNext);
            alert("已設定自動切換下一集功能 ".concat(isAutoNext));
        }));
        var defaultHideHistoryIds = [];
        var _useGmValue6 = useGmValue("hideHistoryIds", defaultHideHistoryIds), hideHistoryIds = _useGmValue6.data, updateHideHistoryIds = _useGmValue6.updateData;
        useGmMenu("設定隱藏觀看紀錄", (function() {
            var ids = prompt("請輸入要隱藏的觀看紀錄 影片 Sn , 分隔, EX : 12345, 54321", hideHistoryIds.join(","));
            if (ids === null) {
                return;
            }
            if (ids) {
        ...
        reactExports.useEffect((function() {
            function triggerHideHistoryIds() {
                var historyList = unsafeWindow.$(".user-watch-list .click-area");
