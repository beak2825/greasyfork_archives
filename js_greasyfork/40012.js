...
// @grant       GM_addStyle
// @connect <value>
// @downloadURL https://update.greasyfork.org/scripts/40012/Elo-Rang%20AT.user.js
// @updateURL https://update.greasyfork.org/scripts/40012/Elo-Rang%20AT.meta.js
// ==/UserScript==
...
function appendRank(rank){
	// create html frame
	  var s ='<li><strong id="rankshow" <style=top:-10px; class="player-name">Elo-Rang:&nbsp';
	s+="";{
        s+=rank = "<font color='yellow'>"+ String(rank) + "&nbsp</font></strong></b>";
            }
	var div = document.createElement('div');
	div.innerHTML = s;

	// insert
	var elementsProfileBox = document.getElementsByClassName('player-name');
...
function searchELORang(name, toCall){
	...
	httpGetAsync('http://fussballcup.at/index.php?club=' + name + '&_qf__form=&module=rating&action=index&area=user&league=&path=index.php&layout=none', fetchELOResults , params);
}
