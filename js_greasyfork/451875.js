// ==UserScript==
// @name         NovemaDownloader
// @namespace    https://github.com/Timesient/manga-download-scripts
// @version      0.5
// @license      GPL-3.0
// @author       Timesient
// @description  Manga downloader for novema.jp
// @icon         https://www.google.com/s2/favicons?sz=64&domain=novema.jp
// @homepageURL  https://greasyfork.org/scripts/451875-novemadownloader
...
// @downloadURL https://update.greasyfork.org/scripts/451875/NovemaDownloader.user.js
// @updateURL https://update.greasyfork.org/scripts/451875/NovemaDownloader.meta.js
// ==/UserScript==

(async function(axios, JSZip, saveAs, ImageDownloader) {
  'use strict';

  // import seedrandom library directly
  ...
  // get image data
  const { serialNumber, chapterNumber } = window.location.href.match(/serial\/n(?<serialNumber>\d*)\/n(?<chapterNumber>\d*)/).groups;
  const jsonURL = `https://novema.jp/img/serial-comic/${serialNumber}/${chapterNumber}/content/index.json`;
  const imageData = await axios({
    method: 'GET',
    url: jsonURL
  }).then(res => res.data.map(item => ({
    url: `https://novema.jp/img/serial-comic/${serialNumber}/${chapterNumber}/content/${item.name}`,
    seed: item.seed
  })));
  ...
  function getDecryptedImage(data) {
    return new Promise(async resolve => {
      // scramble code extracted from comic-viewer.min.js
      const t = 480;
      const n = data.seed;
      const e = new Image;
      e.src = data.url;
