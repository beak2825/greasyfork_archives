...
// @downloadURL https://update.greasyfork.org/scripts/20320/WME%20URComments%20Custom%20List.user.js
// @updateURL https://update.greasyfork.org/scripts/20320/WME%20URComments%20Custom%20List.meta.js
// ==/UserScript==
/* Changelog
...
window.UrcommentsCustomReminderPosistion = 0;
...
window.UrcommentsCustomCloseNotIdentifiedPosistion = 3;
...
window.UrcommentsCustomArray2 = [
    ...
    "Just a reminder: We cannot investigate your map problem further without additional details; if we don't hear back from you soon, we will close the report.  Thanks!", //GizmoGuy, t0cableguy, rickzabel 1/14/2015
    "Open",
    ...
    "Volunteer responding - You reported \"$URD\" -- ",
    "Open",

    "Long Delay",
    "Sorry about the long delay since you submitted your report.  There's a paucity of volunteer editors in this area to help with feedback requests.  If you recall any details of this report, please let us know by responding, and if it's too long ago, we'd also like to know that so that we can close the report.  Thanks! ",
    "Open",

    "Navigate Button",
    "What did you enter as your desired destination in the Waze app clicking on the Navigate button? If you can provide this information we can confirm the location within the Waze database, and possibly link a Google place point with the Waze location as well.",
    "Open",

    "Address Adjustments and refresh",
    ...
    "The address has been adjusted. This should reach mobile devices within a few days. On rare occasions it can take closer to a week.  To get an updated result, remove the location from your navigation history (including favorites) and then search for the location again.", //GizmoGuy, t0cableguy, rickzabel 1/14/2015
    "Solved",

    "Confusing Do Not Enter sign",
    "These \"Do Not Enter\" signs are a little confusing.  The white bar within the red circle sometimes has the restricted hours, and your time of travel seems to be outside this time range.",
    "Open",

    "Road closure advice",
    "For closures that last only a few hours to days, it is better to use the Report > Closure feature built into the Waze app. For closures longer than that, especially if there is some online reference to the closure, report it as a map problem so we can address it on the map.  We frequently will not be able to respond within a couple days so it's not as useful to report shorter road closures.  Thanks!",
    "Open",

    "Charlesgate Storrow",
    "Unfortunately it's tough keeping track of the ramp closures here. MassDOT publishes some of them which we try to enter, but some of them slip through. When you encounter road closures at this site in the future, please use the Report > Closure feature built into the Waze app which will help alter other users more rapidly than we can as we only go through working on user reports when the volunteers have time. Thanks!",
    "Solved",

    "I-93 Surface street overlap",
    "Unfortunately, this is an ongoing problem that we don't have a solution for yet.  The surface streets have such close overlap with I-93 which runs underground, and Waze doesn't know the difference between above- and underground roads.  (It doesn't understand 3D).  So in this case the GPS thought you were on I-93 going south, coming off the exit ramp, from which you are actually allowed to turn left on  Oliver.  Unfortunately there's no immediate solution to this problem, we complain to the company to work on it but have received no response as if it's even in their priority list, or when/whether this issue will (ever) be addressed. Sorry!",
    "Open",

    "No GPS in tunnel",
    "Unfortunately this is a known ongoing problem with long tunnels which contain exits, and we don't yet have a solution since there is no GPS reception underground.  Since the smartphones aren't integrated with cars computers, they doesn't have the ability like some car GPS systems to use 'dead reckoning' to estimate travel when without GPS reception.   Even after coming above ground again, waze app usually takes at less 3-5 seconds to reaquire a GPS lock.  Sorry I can't add much now but we're still exploring what options we may have for improving these situations.",
    "Open",

    "Confirmed Temporary Closure",
    "Unfortunately waze (and the volunteers who maintain the maps) can't keep track of intermittent short term road closures which are not publicly announced, and frequently we miss them even when they are (if it's not a big event).  In these cases the best option to help your fellow drivers is to click on Report -> Closure to indicate a temporarily closed road.  If it's a longer term closure where you can provide or we can find documentation like a web site indicating the duration of the closure, those sorts of changes can be made to the master map.",
    "Solved",

    "Other data source",
    "What is probably happening here is that there are multiple data sources which feed into the Waze app.  These include an internal Waze database, as well as other data sources such as Google Maps, Yelp, yp, and foursquare.  You can see the options of data sources at the bottom of your search screen when searching for a destination/place name.  We only have control over the Waze internal database (the waze icon) and can't readily alter the search results from the other services.  If you use a composite search (the pin icon), you also might be receiving a GPS point which is from a non-waze data source.  Even if the Waze data is correct the other search results may still take you to the wrong place.",
    "Open",
    
    "Uber/Lyft",
    "Are you an Uber/Lyft driver?  The most common problem we have in this situation is when drivers use their app and then use Waze to route.  In this case, the Uber/Lyft app transfers the GPS end point to Waze, and then Waze simply routes to the closest street.  In the case of the Prudential, Waze sometimes takes you to I-90 since it the tunnel runs directly under the Pru.  The only suggestion we can make is to set the Pru as a favorite within waze and do the location lookup directly in Waze which should then usually route correctly.    We frequently see this issue in the back bay issue because the GPS point of businesses within the Uber/Lyft apps is usually in the middle of buildings, and frequently closer to a back alley than the main street.  In this case the only solution would be to enter your destination directly within Waze, which would more likely route correctly.",
    "Open",
    ...
    "Volunteer responding. Thank you for your Speed Limit report.  You were in a parking lot at the time of reporting. To update the speed limit I need to know what the speed limit is, where it starts and where it ends.",
    "Open",

    "Can't find reported limit, need verification",
    "Volunteer responding. Thank you for your report of incorrect Speed Limit. I have looked at Google StreetView in the region where you posted your report and I don't see any signs indicating 40 MPH speed limit. To update the speed limit I need to know where the speed limit changes, and will either need to verify with StreetView or if you can provide an image.",
    "Open",
    
    "Duplicate report",
    "Hello, you sent x duplicate reports at the same location/same time, we are closing this duplicate report.",
    "NotIdentified",

    "Speed limit discrepant",
    "[NOTE] Volunteer responding. Thank you for your report. I am unable to verify that speed limit through official sources.  Google street view from XXX shows a XXX mph speed limit at your location.  If you would like to send a photo of the new sign, please send to masswazers@gmail.com, 'Attn jaywazin XXXXX'.  We will continue to monitor.",
    "Open",

    "Speed limit transition",
    "Thanks for the speed limit report.  We currently have the correct speed limit transition posted. The speed limit will not change on the Waze display until after you pass the speed limit sign.  Due to GPS lag, your device may take 4-6 sec for the new speed limit to show up on your screen.",
    "Solved",

    "Speed limit advisory",
    "Thanks for the speed limit report.   Based on Street View images, you were reporting a yellow advisory sign.  Within waze we only post the legal speed limit on WHITE Speed Limit signs. ",
    "Solved",

    "Speed limit confirmed",
    "Thanks for the speed limit report.   We have confirmed that the speed limit which you have marked is set appropriately in the map. ",
    "Solved",
    ...
    //"Just a reminder: We have not received a response on your report. If we don't hear back from you soon we will infer everything is okay and close the report. Thanks!", //karlcr9911 12/7/14  //this has been getting me the most responses, but it probably is the special link i am including with the UR layer enabled rickzabel 12/7/14 t0cableguy 12/8/14
    "We try in some cases to put in surround road closures during thes events, but can't keep track of all of them as the closures aren't published in advance.  For these sorts of closures, advise that you use the Report > Closure feature built into the Waze app.   This tells your app to reroute, and after a few users mark a road closed, the server is supposed to recognize it as closed for all.  Thanks!",
    "NotIdentified",
