// ==UserScript==
// @name              [ALL SERVERS] | КаСтОмИзИрОвАнНыЙ Скрипт для Кураторов Форума [Для телефона]
// @namespace         https://forum.blackrussia.online
// @version           1.0.0
// @author            Dany_Forbs from ORANGE
// @connection        https://vk.com/kwaazzi
// @updateversion     Создан 06.12.2025
...
// @downloadURL https://update.greasyfork.org/scripts/558132/%5BALL%20SERVERS%5D%20%7C%20%D0%9A%D0%B0%D0%A1%D1%82%D0%9E%D0%BC%D0%98%D0%B7%D0%98%D1%80%D0%9E%D0%B2%D0%90%D0%BD%D0%9D%D1%8B%D0%99%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%20%D0%A4%D0%BE%D1%80%D1%83%D0%BC%D0%B0%20%5B%D0%94%D0%BB%D1%8F%20%D1%82%D0%B5%D0%BB%D0%B5%D1%84%D0%BE%D0%BD%D0%B0%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/558132/%5BALL%20SERVERS%5D%20%7C%20%D0%9A%D0%B0%D0%A1%D1%82%D0%9E%D0%BC%D0%98%D0%B7%D0%98%D1%80%D0%9E%D0%B2%D0%90%D0%BD%D0%9D%D1%8B%D0%99%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%20%D0%A4%D0%BE%D1%80%D1%83%D0%BC%D0%B0%20%5B%D0%94%D0%BB%D1%8F%20%D1%82%D0%B5%D0%BB%D0%B5%D1%84%D0%BE%D0%BD%D0%B0%5D.meta.js
// ==/UserScript==
 
(function () {
    'use strict';
    ...
    let CUSTOM_COLORS = {
        ...
        textColor: '#FFD700',
        shadowColor: 'rgba(50, 205, 50, 0.6)',
        insetGlow: 'rgba(255, 215, 0, 0.2)',
    ...
    const PINBIO_PREFIX = 15;
    
    // ==================== ФУНКЦИЯ ДЛЯ СОЗДАНИЯ СТИЛЕЙ ====================
    function createButtonStyle(isHeader = false) {
        if (isHeader) {
            return `
                padding: 8px 20px;
                ...
                text-shadow: 0 2px 5px rgba(0, 0, 0, 0.9), 0 0 10px ${CUSTOM_COLORS.textColor.replace(')', ', 0.5)').replace('rgb', 'rgba')};
                background: linear-gradient(135deg, 
                    ${CUSTOM_COLORS.headerGradient.start} 0%, 
                    ${CUSTOM_COLORS.headerGradient.middle1} 30%, 
                    ${CUSTOM_COLORS.headerGradient.middle2} 70%, 
                    ${CUSTOM_COLORS.headerGradient.end} 100%);
                border: 3px solid ${CUSTOM_COLORS.borderColor};
                border-radius: 12px;
                box-shadow: 0 0 25px ${CUSTOM_COLORS.headerGradient.start.replace(')', ', 0.8)').replace('rgb', 'rgba')}, 
                            0 0 35px ${CUSTOM_COLORS.textColor.replace(')', ', 0.6)').replace('rgb', 'rgba')}, 
                            inset 0 1px 3px rgba(255, 255, 255, 0.4), 
                            inset 0 -1px 3px rgba(0, 0, 0, 0.3), 
                            0 6px 0 ${CUSTOM_COLORS.headerGradient.end}, 
            ...
            return `
                border-radius: 13px;
                margin-right: 5px;
                border: 2px solid ${CUSTOM_COLORS.borderColor};
                font-family: UtromPressKachat;
                background: linear-gradient(to bottom, 
                    ${CUSTOM_COLORS.buttonGradient.top}, 
                    ${CUSTOM_COLORS.buttonGradient.bottom});
                color: ${CUSTOM_COLORS.textColor};
                font-weight: bold;
                box-shadow: 0 0 10px ${CUSTOM_COLORS.shadowColor}, 
                            0 2px 4px ${CUSTOM_COLORS.buttonGradient.bottom.replace(')', ', 0.4)').replace('rgb', 'rgba')}, 
                            inset 0 1px 1px ${CUSTOM_COLORS.insetGlow};
                text-shadow: ${CUSTOM_COLORS.textShadow};
            `;
        }
    }
    ...
    dpstyle: createButtonStyle(true)
},
{
...
{
    title: 'В ЖБ на АДМ',
    dpstyle: createButtonStyle(false),
    content:  
          '[CENTER][SIZE=4][FONT=Verdana][color=#7FFFD4]Доброго времени суток, уважаемый(-ая)[/COLOR]<br> {{ user.mention }}.<br><br>' +
          '[url=https://ibb.co/51gRYCr][img]https://i.ibb.co/grLRvQS/image.png[/img][/url]<br><br>' +
...
},
{
    ...
    // ==================== ПАНЕЛЬ НАСТРОЕК (Вариант 2) ====================
    function createSettingsPanel() {
        const panelHTML = `
            <div id="customScriptSettings" style="
                position: fixed;
                top: 100px;
                right: 20px;
                width: 300px;
            ...
            ">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                    <h3 style="margin: 0; color: #FFD700;">🎨 Настройки скрипта</h3>
                ...
                <div style="margin-bottom: 10px;">
                    <label style="display: block; margin-bottom: 5px; font-weight: bold;">Цвет границы:</label>
                    <input type="color" id="borderColor" value="#FFD700" style="width: 100%;">
                    <input type="text" id="borderColorText" value="#FFD700" style="width: 100%; margin-top: 5px; padding: 5px;">
                </div>
                
                <div style="margin-bottom: 10px;">
                    <label style="display: block; margin-bottom: 5px; font-weight: bold;">Верх градиента:</label>
                    <input type="color" id="gradientTop" value="#228B22" style="width: 100%;">
                    <input type="text" id="gradientTopText" value="#228B22" style="width: 100%; margin-top: 5px; padding: 5px;">
                </div>
                
                <div style="margin-bottom: 10px;">
                    <label style="display: block; margin-bottom: 5px; font-weight: bold;">Низ градиента:</label>
                    <input type="color" id="gradientBottom" value="#006400" style="width: 100%;">
                    <input type="text" id="gradientBottomText" value="#006400" style="width: 100%; margin-top: 5px; padding: 5px;">
                </div>
                
                <div style="margin-bottom: 10px;">
                    <label style="display: block; margin-bottom: 5px; font-weight: bold;">Цвет текста:</label>
                    <input type="color" id="textColor" value="#FFD700" style="width: 100%;">
                    <input type="text" id="textColorText" value="#FFD700" style="width: 100%; margin-top: 5px; padding: 5px;">
                </div>
        ...
        // Обработчики событий
        $('#openSettings').click(() => {
            $('#customScriptSettings').show();
        });
        
        $('#closeSettings').click(() => {
            $('#customScriptSettings').hide();
        });
        
        // Синхронизация color picker и текстового поля
        $('#borderColor').on('input', function() {
            $('#borderColorText').val($(this).val());
        });
        
        $('#borderColorText').on('input', function() {
            const value = $(this).val();
            if (/^#[0-9A-F]{6}$/i.test(value) || /^rgb/i.test(value)) {
                $('#borderColor').val(value);
            }
        });
        
        $('#gradientTop').on('input', function() {
            $('#gradientTopText').val($(this).val());
        });
        
        $('#gradientTopText').on('input', function() {
            const value = $(this).val();
            if (/^#[0-9A-F]{6}$/i.test(value) || /^rgb/i.test(value)) {
                $('#gradientTop').val(value);
            }
        });
        
        $('#gradientBottom').on('input', function() {
            $('#gradientBottomText').val($(this).val());
        });
        
        $('#gradientBottomText').on('input', function() {
            const value = $(this).val();
            if (/^#[0-9A-F]{6}$/i.test(value) || /^rgb/i.test(value)) {
                $('#gradientBottom').val(value);
            }
        });
        ...
        $('#applySettings').click(() => {
            ...
            updateButtonStyles();
            $('#customScriptSettings').hide();
            XF.alert('Настройки применены! Кнопки обновлены.', null, 'Успех');
        });
        
        $('#resetSettings').click(() => {
            resetSettings();
            XF.alert('Настройки сброшены к значениям по умолчанию.', null, 'Сброс');
        });
    }
    ...
    function saveSettings() {
        const settings = {
            ...
            gradientTop: $('#gradientTop').val(),
            gradientBottom: $('#gradientBottom').val(),
            textColor: $('#textColor').val()
        };
        
        localStorage.setItem('customScriptColors', JSON.stringify(settings));
    }
    
    function loadSettings() {
        const saved = localStorage.getItem('customScriptColors');
        if (saved) {
            try {
                const settings = JSON.parse(saved);
                $('#borderColor').val(settings.borderColor || '#FFD700');
                $('#borderColorText').val(settings.borderColor || '#FFD700');
                $('#gradientTop').val(settings.gradientTop || '#228B22');
                $('#gradientTopText').val(settings.gradientTop || '#228B22');
                $('#gradientBottom').val(settings.gradientBottom || '#006400');
                $('#gradientBottomText').val(settings.gradientBottom || '#006400');
                $('#textColor').val(settings.textColor || '#FFD700');
                $('#textColorText').val(settings.textColor || '#FFD700');
                
                updateCUSTOM_COLORS(settings);
            } catch (e) {
    ...
    function updateCUSTOM_COLORS(settings) {
        CUSTOM_COLORS.borderColor = settings.borderColor || '#FFD700';
        CUSTOM_COLORS.buttonGradient.top = settings.gradientTop || '#228B22';
        CUSTOM_COLORS.buttonGradient.bottom = settings.gradientBottom || '#006400';
        CUSTOM_COLORS.textColor = settings.textColor || '#FFD700';
    }
    ...
    function resetSettings() {
        const defaultSettings = {
            borderColor: '#FFD700',
            gradientTop: '#228B22',
            gradientBottom: '#006400',
            textColor: '#FFD700'
        };
        
        localStorage.setItem('customScriptColors', JSON.stringify(defaultSettings));
        loadSettings();
        updateButtonStyles();
    }
    
    function updateButtonStyles() {
        // Обновляем массив кнопок с новыми стилями
        buttons = initializeButtons();
        
        // Если панель ответов открыта - перерисовываем её
        if ($('.select_answer').length) {
            $('.overlay-container').remove();
            XF.alert(buttonsMarkup(buttons), null, 'ОТВЕТЫ');
            attachButtonHandlers();
        }
        
        // Также обновляем кнопку "Префиксы"
        $('#prefixesToggle').attr('style', 
            'border-radius: 13px; margin-right: 5px; border: 2px solid ' + CUSTOM_COLORS.borderColor + 
            '; font-family: UtromPressKachat; padding: 8px 16px; background: linear-gradient(to bottom, #B22222, #8B0000); color: ' + 
            CUSTOM_COLORS.textColor + '; font-weight: bold; box-shadow: 0 0 10px ' + 
            CUSTOM_COLORS.textColor.replace(')', ', 0.6)').replace('rgb', 'rgba') + ';'
        );
    }
 
$(document).ready(() => {
