...
// @downloadURL https://update.greasyfork.org/scripts/563799/Geoguessr%20head-to-head%20unranked%20duel%20statistics.user.js
// @updateURL https://update.greasyfork.org/scripts/563799/Geoguessr%20head-to-head%20unranked%20duel%20statistics.meta.js
// ==/UserScript==
...
const USERSCRIPT_DATA_BLOCK_CLASS = '__userscript_h2h_unranked_data_block';
const USERSCRIPT_DATA_ELEMENT_CLASS = '__userscript_h2h_unranked_data_element';
const USERSCRIPT_GAME_LIST_CLASS = '__userscript_h2h_unranked_game_list';
const USERSCRIPT_GAME_LIST_CLASS_INACTIVE = '__userscript_h2h_unranked_game_list_inactive';
const USERSCRIPT_UPDATE_NOTIFICATION_CLASS = '__userscript_h2h_unranked_update_notification';
const USERSCRIPT_CUSTOM_SPACER_CLASS = '__userscript_h2h_unranked_custom_spacer';
...
updateMessageReceiver.addEventListener('message', (event) => {
    ...
    if (event.data === "stop") {
        ...
        else if (location.pathname.match(/^\/(..\/)?me\/profile$/)) {
            runOnProfilePage(true);
        }
    }
});
...
const fetchStats = async (userId, opponents, partner = null) => {
    ...
    for (const duel of duels) {
        if (partner && duel.partner !== partner) {
            continue;
        }
        if (partner && ! opponents.every((opponent) => duel.opponents.includes(opponent))) {
            continue;
        }
...
const fetchDuelList = async (userId, opponentId, gameMode = 'overall', partner = null) => {
    const db = await openDB(userId);
    const tx = db.transaction(`duels_${userId}`, 'readonly');
    const duelsStore = tx.objectStore(`duels_${userId}`);
...
const fetchDuelsByAllModes = async (userId, opponentId, partner = null) => {
    console.log("Grabbing games from opponent: " + opponentId);
    const [allGames, movingGames, noMoveGames, nmpzGames] = await Promise.all([
        fetchDuelList(userId, opponentId, 'overall', partner),
        fetchDuelList(userId, opponentId, 'StandardDuels', partner),
        fetchDuelList(userId, opponentId, 'NoMoveDuels', partner),
        fetchDuelList(userId, opponentId, 'NmpzDuels', partner)]);
    return {
        overall: allGames,
...
const runOnUserPage = async (forceRun = false) => {
    ...
    navigator.locks.request("userscript_unranked_duels_history", async (lock) => {
        ...
        if (!forceRun && opponentId === lastOpponentId && dataBlock) {
            return
        }
...
const createDownload = async (userId) => {
    ...
    for (const storeName of storeNames) {
        ...
        data.stores[storeName] = await fetchResult(objStore.getAll());
    }
...
const runOnProfilePage = async (forceRun = false) => {
    ...
    navigator.locks.request("userscript_unranked_duels_history_database_download", async () => {
