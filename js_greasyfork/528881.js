...
// @version      1.7
// @description  抓取小红书页面上的帖子标题、链接和点赞数(本地数据库版本)
// @author       稳稳
// @match        https://www.xiaohongshu.com/*
// @grant        GM_openInTab
// @grant        GM_xmlhttpRequest
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/528881/%E5%B0%8F%E7%BA%A2%E4%B9%A6%E5%B8%96%E5%AD%90%E6%8A%93%E5%8F%96%E5%99%A8%28%E6%9C%AC%E5%9C%B0%E7%89%88%29.user.js
// @updateURL https://update.greasyfork.org/scripts/528881/%E5%B0%8F%E7%BA%A2%E4%B9%A6%E5%B8%96%E5%AD%90%E6%8A%93%E5%8F%96%E5%99%A8%28%E6%9C%AC%E5%9C%B0%E7%89%88%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    const CONFIG = {
        ...
        DETAIL_DELAY: {
            MIN: 3000,    // 增加详情页最小等待时间
            MAX: 8000     // 增加详情页最大等待时间
        },
        ...
        LOAD_CHECK_DELAY: {
            MIN: 500,     // 最小等待时间500ms
            MAX: 5000     // 最大等待时间5000ms
        },

        DETAIL_TIMEOUT: 30000,

        BOTTOM_CHECK_RETRIES: 10,  // 从5改为10次
    ...
    async function getPostDetail(postData) {
        return new Promise((resolve) => {
            ...
            }, getRandomDelay(CONFIG.LOAD_CHECK_DELAY.MIN, CONFIG.LOAD_CHECK_DELAY.MAX));
    ...
    async function uploadToServer(data, retryCount = 0) {
        const MAX_RETRIES = 3;
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                GM_xmlhttpRequest({
                    method: 'POST',
                    url: 'http://localhost:8888/redbook-server/save_notes.php', // 改用8888端口
                    headers: {
                    ...
                    onerror: async function(error) {
                        ...
                        if (retryCount < MAX_RETRIES) {
                            document.getElementById('uploadStatus').textContent = 
                                `上传失败，${RETRY_DELAY/1000}秒后重试(${retryCount + 1}/${MAX_RETRIES})`;
                            await new Promise(resolve => setTimeout(resolve, RETRY_DELAY));
                            try {
                                const result = await uploadToServer(data, retryCount + 1);
                                resolve(result);
                            } catch (e) {
                                reject(e);
                            }
                        } else {
                            document.getElementById('uploadStatus').textContent = '上传失败: 达到最大重试次数';
                            reject(new Error('上传失败: 达到最大重试次数'));
                        }
                    },
                    ontimeout: async function() {
                        if (retryCount < MAX_RETRIES) {
                            document.getElementById('uploadStatus').textContent = 
                                `上传超时，${RETRY_DELAY/1000}秒后重试(${retryCount + 1}/${MAX_RETRIES})`;
                            await new Promise(resolve => setTimeout(resolve, RETRY_DELAY));
                            try {
                                const result = await uploadToServer(data, retryCount + 1);
                                resolve(result);
    ...
    async function checkNoteExists(noteId) {
        return new Promise((resolve) => {
            GM_xmlhttpRequest({
                method: 'GET',
                url: `http://localhost:8888/redbook-server/check_note.php?note_id=${noteId}`, // 改用8888端口
                headers: {
                    'Accept': 'application/json',
                    'Access-Control-Allow-Origin': '*'  // 添加CORS头
                },
    ...
    async function extractPostInfo() {
        ...
        for (const item of noteItems) {
            ...
            // 修改链接匹配模式
            const noteMatch = coverLink.match(/\/explore\/([^?]+)/);
            if (!noteMatch) continue;

            const noteId = noteMatch[1];
            const fullLink = `https://www.xiaohongshu.com/explore/${noteId}`;
