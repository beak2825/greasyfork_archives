// ==UserScript==
// @name         提问小助手fdl定制版v1.0.1
// @namespace    bob@fanruan.com
// @version      1.0.0.2
...
// @downloadURL https://update.greasyfork.org/scripts/521082/%E6%8F%90%E9%97%AE%E5%B0%8F%E5%8A%A9%E6%89%8Bfdl%E5%AE%9A%E5%88%B6%E7%89%88v101.user.js
// @updateURL https://update.greasyfork.org/scripts/521082/%E6%8F%90%E9%97%AE%E5%B0%8F%E5%8A%A9%E6%89%8Bfdl%E5%AE%9A%E5%88%B6%E7%89%88v101.meta.js
// ==/UserScript==
...
(function () {
    ...
    window.MakeTags = function () {
      ...
      tags = ['FDL-基础模块','FDL-管道任务','FDL-定时任务','FDL-数据服务','FDL-实时任务','FDL-产品融合','FDL-数据管理治理','FDL-安全漏洞','FDL-数据源', '未知-找Bob'];
      ...
      function categoryTags(ts) {
        ...
        var result = cats.map(g => {
          return {
            ...
            tags: ts.filter(h => g ? h.startsWith(g + '-') : !h.includes('-')).map(t => t.match(/(?:([^a-zA-Z0-9]+|FDL)-)?(.+)/).slice(0, 3))
          }
        });
        return result;
      }
    ...
    if (window.location.href.indexOf("ask") != -1) {
      ...
      $('#postSpaceKey').val('finetube').trigger('change');
      document.getElementById('postSpaceKey').value = "finetube";
      document.getElementById('postSpaceName').value = "2.3 FineDataLink";
  //描述预填充
  const preFillContent = ['【客户名称】：', '【现象&问题描述】：xx操作后 在xx时间 xx任务异常(具体异常现象)', '【jar包信息】：FDL4.1.12.x 独立部署/容器化部署/..','【数据库类型&版本】' , '【截图&日志】：现象截图&INFO级别日志&.. '].map(e => `<p>${e}</p>`).join('');
  ...
  */
      ...
      function setNewTitleLeader(text){
        const titleLeaders = ['报表', 'BI', 'FDL', '图表', '移动端', '平台', '专家'];
      ...
      //addRadioOptions('module', '请选择提问模块', [['report', '报表'], ['bi', 'BI'], ['finetube', 'FDL'], ['chart', '图表'], ['mobile', '移动端'], ['dec', '平台']], radioOptionCallback4module, 'div.form-item.ask-title');
