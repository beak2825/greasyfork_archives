// ==UserScript==
// @name           JV_Chat_MP
// @description    JVChat Premium pour les MP de JVC
...
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/472006/JV_Chat_MP.user.js
// @updateURL https://update.greasyfork.org/scripts/472006/JV_Chat_MP.meta.js
// ==/UserScript==
...
.header,
.btn-poster-msg,
.form-post-topic > div:nth-child(3),
.titre-discut,
#destinataires,
.action-bar,
.form-harassment,
...
#repondre-mp > .form-post-topic > .row {
    margin: 0;
}
...
/*
.messageEditor__containerEdit > .messageEditor__buttonEdit {
    border: 0;
}
*/
...
/* .jvchat-night-mode #jvchat-main, */
.jvchat-night-mode .col-lg-9,
    ...
    <div class='panel panel-jv-forum'>
        ...
        <div class='panel-body'>
            <div id='jvchat-info'>
                ...
                <div id='jvchat-topic'>
                    <h4 class='titre-info-fofo'>MP-PRIVE</h4>
                    <div id="jvchat-topic-info">
                        <div><strong><a title="Ouvrir le MP" id="jvchat-topic-title"></a></strong></div>
...
function getForm(doc) {
    return doc.getElementsByClassName('form-post-topic')[0];
}
...
function toggleTextarea() {
    ...
    //document.getElementById("repondre-mp").getElementsByClassName("jv-editor-toolbar")[0].classList.toggle("jvchat-hide");
    document.getElementById("repondre-mp").getElementsByClassName("messageEditor__buttonEdit")[0].classList.toggle("jvchat-hide");
    ...
    document.getElementById("repondre-mp").classList.toggle("jvchat-reduced");
...
function parseURL(url) {
    const baseURL = new URL(url, location.origin); /*Parser URL Fectché */
    const domain = baseURL.origin + baseURL.pathname; /*URL sans parametre */
    const ids = `id=${baseURL.searchParams.get("id")}`;
    const offsetString = offsetNumberString();
    return { domain: domain, ids: ids, offsetString: offsetString };
}

function offsetNumberString() {
    let offsetNumber = 1;
    let offsetMaxNumber = nombresMessage();
    offsetNumber = (offsetMaxNumber <= 20) ? 1 : offsetMaxNumber - 20;
    return `offset=${offsetNumber}`;
}

function buildURL(dict) {
    return `${dict.domain}?${dict.ids}&${dict.offsetString}`;
}

function buildURLreturnMP(dict) {
    return `${dict.domain}?${dict.ids}${dict.folder}#last_msg`;
}
...
function clearPage(document) {
    ...
    //let previsu = document.getElementById("repondre-mp").getElementsByClassName("previsu-editor")[0];
    let previsu = document.getElementById("repondre-mp").getElementsByClassName("messageEditor__containerPreview")[0];
    if (previsu) {
    ...
    document.getElementsByClassName("col-lg-9")[0].insertAdjacentHTML("afterbegin", "<div id='jvchat-main'><hr class='jvchat-ruler'></div>");
    document.getElementsByClassName("row")[0].insertAdjacentHTML("afterbegin", "<div id='jvchat-alerts'><div id='jvchat-fixed-alert' class='jvchat-hide'><div class='alert-row'></div></div><div id='jvchat-turbo-warning' class='jvchat-hide'><button class='close jvchat-alert-hide' aria-hidden='true' data-dismiss='alert' type='button'>×</button><div class='alert-row'></div></div><div id='jvchat-degraded-refresh-warning' class='jvchat-hide'><div class='alert-row'></div></div></div>");

    document.getElementsByClassName("layout__content")[0].insertAdjacentHTML("afterbegin", PANEL);
    document.getElementsByClassName("layout__content")[0].insertAdjacentHTML("beforeend", "<div id='jvchat-right-padding'></div>");

    document.getElementById("mp").classList.add("jvchat-root");

    document.getElementById("repondre-mp").classList.add("jvchat-reduced");
    document.getElementById("repondre-mp").classList.add("jvchat-hide");

    let toolbar = document.getElementById("repondre-mp").getElementsByClassName("messageEditor__buttonEdit")[0];
    if (toolbar) {
    ...
    if (configuration["night_mode"]) {
        document.getElementById("mp").classList.add("jvchat-night-mode");
    }
...
function toggleNightModeOption(event) {
    ...
    saveConfig();
    document.getElementById("mp").classList.toggle("jvchat-night-mode");
}
...
function adjustMaxWidth(maxWidth) {
    //document.getElementById("discussion").style["flex-grow"] = maxWidth;
    document.querySelector(".row.mb-5").style["flex-grow"] = maxWidth;
...
function postMessage() {
    ...
    let formulaire = document.getElementById("repondre-mp");
...
function makeMessage(message) {
    ...
    let html =
        `<div class="jvchat-bloc-message">
            <div class="jvchat-message" jvchat-id="${author}">
                <div>
...
function addMessages(messages, editing, requestTimestamp) {
    ...
    for (let message of messages) {
        ...
        let idMP = message.author + message.dateString;
        message.id = idMP;
...
function setUser(document, user) {
    ...
    if ((userConnected === undefined && isConnected) || (userConnected !== undefined && isConnected !== userConnected)) {
        ...
        let isDown = isScrollDown();
        document.getElementById("repondre-mp").classList.toggle("jvchat-hide");
        if (isDown) {
            setScrollDown();
        }
    }
...
function triggerJVChat() {
    //change_URL_et_savefolder_pour_garder_lancre________
    const url = new URL(window.location.href);
    var folderURL = `&folder=${url.searchParams.get("folder") || 0}`;

    url.searchParams.delete("folder"); url.searchParams.delete("offset");
    url.hash = "last_msg";
    ...
    let topic = document.querySelector('.titre-discut').textContent.trim();;
    ...
    urlToFetch.page = 1;
    urlToFetch.anchor = "";
    urlToFetch.folder = folderURL //garder_dossier_mp_en_memoire
...
function nombresMessage () {
    let offsetRegex = /\d+(?=\D*$)/i;
    let offsetText = document.querySelector('.action-bar.pagination .action-left').innerText;

    return parseInt(offsetText.match(offsetRegex), 10);
}
...
function parsePage(res, requestTimestamp) {
    ...
    let nbMessages = nombresMessage();
    if (nbMessages !== lastNbMessages) {
...
function makeJVChatButton() {
    ...
    let btn = `<button class="btn ${cls}">${text}</button>`;
    return btn;
}
