// ==UserScript==
// @name         SOOP(숲) 타임 스탬프 (예비용)
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  SOOP(숲) 라이브 및 다시보기 영상에서 단축키 'Y'를 눌러 메모를 작성하고 저장할 수 있습니다.
// @author       멍멍이
...
// @run-at       document-end
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/545215/SOOP%28%EC%88%B2%29%20%ED%83%80%EC%9E%84%20%EC%8A%A4%ED%83%AC%ED%94%84%20%28%EC%98%88%EB%B9%84%EC%9A%A9%29.user.js
// @updateURL https://update.greasyfork.org/scripts/545215/SOOP%28%EC%88%B2%29%20%ED%83%80%EC%9E%84%20%EC%8A%A4%ED%83%AC%ED%94%84%20%28%EC%98%88%EB%B9%84%EC%9A%A9%29.meta.js
// ==/UserScript==
...
(function () {
    ...
    function showMemoInputPopup(time, onSave) {
        ...
        popup.innerHTML = `
            <div style="font-weight: bold; margin-bottom: 6px; font-size: 14px;">메모 입력 (${time})</div>
            <input type="text" id="memoInput" style="width: 100%; padding: 6px; font-size: 14px;" value="여기에 문구를 입력하세요" />
    ...
    if (window.location.href.includes('vod.sooplive.co.kr')) {
        ...
        document.addEventListener('keypress', (event) => {
            if (event.key.toUpperCase() === hotkey) {
                ...
                if (timeElement) {
                    const time = timeElement.innerText;
                    showMemoInputPopup(time, (memo) => {
                        navigator.clipboard.writeText(`${time}, ${memo}`);
                        saveTimestampLog(time, memo);
                        showToastMessage('저장 완료');
                    });
                } else {
                    showToastMessage('시간 정보를 찾을 수 없습니다.', true);
