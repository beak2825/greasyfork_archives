// ==UserScript==
// @name         ValueEdge: do not wrap text in code blocks
// @namespace    https://github.com/ahr-huber/monkey-scripts/
// @version      2025-03-28
// @description  Fixes code blocks to not wrap text in code blocks. It also adds a grey background to make them stand out.
// @license      MIT
// @author       Andreas Huber
// @match        https://*.saas.microfocus.com/*
// @icon         data:image/png;base64,
// @grant        none
// @sandbox      DOM
// @downloadURL https://update.greasyfork.org/scripts/531140/ValueEdge%3A%20do%20not%20wrap%20text%20in%20code%20blocks.user.js
// @updateURL https://update.greasyfork.org/scripts/531140/ValueEdge%3A%20do%20not%20wrap%20text%20in%20code%20blocks.meta.js
// ==/UserScript==

(function() {
    'use strict';

    function cssFix_codeBlocks() {
         const style = document.createElement("style");
        style.textContent = `
        .fr-wrapper pre,
        .alm-entity-form-comment-entry-text pre{
            border: 1px solid #ccc;
  	        background: #efefef;
  	        border-radius: 5px;
            white-space: pre;
            overflow: auto;
        }
        `;
        document.body.appendChild(style);
    }

    cssFix_codeBlocks(); // code blocks use whitespace: no-wrap and get a grey background
})();