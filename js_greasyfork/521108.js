// ==UserScript==
// @name         BLACK RUSSIA TOMSK || Скрипт для ГСХ/ЗГСХ/СХ
// @namespace    https://forum.blackrussia.online
// @version      1.1
// @description  Специально для BlackRussia || TOMSK D.Skyxrockedz
// @author       Dozza
// @match        https://forum.blackrussia.online/index.php?threads/*
// @include      https://forum.blackrussia.online/index.php?threads/
// @grant        none
// @license      MIT
// @collaborator QuenkM
// @icon data:image/jpeg;base64,
// @downloadURL https://update.greasyfork.org/scripts/521108/BLACK%20RUSSIA%20TOMSK%20%7C%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%93%D0%A1%D0%A5%D0%97%D0%93%D0%A1%D0%A5%D0%A1%D0%A5.user.js
// @updateURL https://update.greasyfork.org/scripts/521108/BLACK%20RUSSIA%20TOMSK%20%7C%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%93%D0%A1%D0%A5%D0%97%D0%93%D0%A1%D0%A5%D0%A1%D0%A5.meta.js
// ==/UserScript==
(function () {
  'use strict';
const UNACCСEPT_PREFIX = 4; // Prefix that will be set when thread closes
const ACCСEPT_PREFIX = 8; // Prefix that will be set when thread accepted
const RESHENO_PREFIX = 6; // Prefix that will be set when solving the problem
const NARASSMOTRENIIRP_PREFIX = 2; // Prefix that will be set when thread pins
const GA_PREFIX = 12; // Prefix that will be set when thread send to ga
const COMMAND_PREFIX = 10; // Prefix that will be set when thread send to project team
const WATCHED_PREFIX = 9;
const CLOSE_PREFIX = 7;
const SPECY_PREFIX = 11;
const TEXY_PREFIX = 13;
const OTKAZBIO_PREFIX = 4;
const ODOBRENOBIO_PREFIX = 8;
const NARASSMOTRENIIBIO_PREFIX = 2;
const OTKAZRP_PREFIX = 4;
const ODOBRENORP_PREFIX = 8;
const OTKAZORG_PREFIX = 4;
const ODOBRENOORG_PREFIX = 8;
const NARASSMOTRENIIORG_PREFIX = 2;
const buttons = [
     {
	  title: '-----> ╴╴╴╴╴╴╴╴╴╴╴╴Набор на пост хелпера(click)╴╴╴╴╴╴╴╴╴╴╴╴<------',
         content:
`[CENTER][B][FONT=times new roman][SIZE=5][I][COLOR=rgb(235, 107, 86)]Заявления на пост "Агент Поддержки"[/COLOR][/I][/FONT]<br><br>` +
`[COLOR=rgb(204, 255, 0)][U][SIZE=4][FONT=times new roman][I]Критерии:[/I][/FONT][/U]<br><br>`+
`[I][SIZE=4][FONT=times new roman]1. Иметь возраст не ниже 15 лет. (исключение до 14)[/FONT]<br>`+
`[SIZE=4][FONT=times new roman]2. Быть грамотным.[/FONT]<br>`+
`[SIZE=4][FONT=times new roman]3. Иметь хороший микрофон.[/FONT]<br>`+
`[SIZE=4][FONT=times new roman]4. Иметь уровень не ниже 8 (без исключений)[/FONT]<br>`+
`[SIZE=4][FONT=times new roman]5. Иметь желание помогать проекту.[/FONT]<br>`+
`[SIZE=4][FONT=times new roman]6. Знать команды, /gps сервера.[/FONT]<br>`+
`[SIZE=4][FONT=times new roman]7. Открыть личные сообщения во ВКонтакте.[/FONT]<br>`+
`[SIZE=4][FONT=times new roman]ВАЖНО: | Пост хелпера не игрушка. За неутстойку 7 дневного срока будет наказание.[/FONT]<br>`+
`[SIZE=4][FONT=times new roman]ВАЖНО: | Главный хелпер в праве отказать в вашей заявке, не объясняя причины[/FONT]<br>`+
`[SIZE=4][FONT=times new roman]ВАЖНО: | Если ваша заявка одобрена то вам напишут в VK и позже добавят в беседу кандидатов.[/FONT]<br>`+
`[SIZE=4][FONT=times new roman]ВАЖНО: | Ваши заявки будут периодически удаляться.[/FONT]<br>`+
`[SIZE=4][FONT=times new roman]ВАЖНО: | Иметь активную и не пустую страницу в ВКонтакте.[/FONT][/I]<br>`+
`[B][SIZE=4][FONT=times new roman][I]ВАЖНО: | Иметь активный и не пустой ФА, которому минимум 30 дней.[/I][/FONT][/B][/COLOR][/B]<br><br>`+


`[COLOR=rgb(153, 204, 0)][B][FONT=times new roman][SIZE=4][I][U]ФОРМА ПОДАЧИ ЗАЯВЛЕНИЯ:[/U][/I][/FONT][/B][/COLOR]<br>`+

`[B][FONT=times new roman][SIZE=4][COLOR=rgb(153, 204, 0)][I]1. Ваш игровой никнейм:<br>`+
`2. Ваш игровой уровень:<br>`+
`3. Скриншот вашей статистики (ОБЯЗАТЕЛЬНО с /time):<br>`+
`4. Почему именно вы должны занять пост хелпера (подробно):<br>`+
`5. Были ли у вас баны/варны, если да то за что:<br>`+
`6. Что такое по вашему "блат" в общем?:<br>`+
`7. Ваш реальный возраст:<br>`+
`8. Страна/город в котором живёте:<br>`+
`9. Ваш часовой пояс:<br>`+
`10. Ваше реальное имя:<br>`+
`11. Ссылка на ваш ВКонтакте:<br>`+
`12. Логин Discord:<br>`+
`13. Были ли вы ранее хелпером / администратором:[/I][/COLOR][/FONT][/B]<br>`+

`[QUOTE][FONT=times new roman][SIZE=4][I][B][COLOR=rgb(247, 218, 100)]Примечание: После одобрение, с вами свяжется Старшая Администрация. Вам будет необходимо добавить представителя старшей администрации в друзья, после вас добавят в специальную беседу. Никто из состава администрации не будет просить у вас все различные пароли, пин-коды, информация о привязках и так далее. Не видитесь на обманы[/COLOR][/B][/I][/FONT][/QUOTE]<br>`+
`[COLOR=rgb(250, 197, 28)][B][SIZE=5][FONT=times new roman][I][U]Контакты:[/U][/I][/FONT][/B][/COLOR]<br>`+
`[COLOR=rgb(209, 213, 216)][B][I][SIZE=4][FONT=times new roman]Главный Администратор[B] —[/B] [/FONT][/I][/B][I][SIZE=4][FONT=times new roman][URL='https://vk.com/id313215658'][B][B]*КЛИК*[/B][/B][/URL][/FONT]<br>`+
`[SIZE=4][FONT=times new roman][B]Основной ЗГА[B] —[/B] [/B][URL='https://vk.com/id471376695'][B]*КЛИК*[/B][/URL][/FONT]<br>`+
`[SIZE=4][FONT=times new roman][B]Заместитель ГА [I][B]— [URL='https://vk.com/larshopeless']*КЛИК*[/URL][/B][/I][/B][/FONT]<br>`+
`[SIZE=4][FONT=times new roman][B][I][B]Куратор Администрации [I][B][I][B]— [URL='https://vk.com/id551260629']*КЛИК*[/URL][/B][/I][/B][/I][/B][/I][/B][/FONT]<br>`+
`[SIZE=4][FONT=times new roman][B][I][B][I][B][I][B][I][B][I][B]Куратор Администрации [I][B][I][B]—
[URL='https://vk.com/id558869279']*КЛИК*[/URL][/B][/I][/B][/I][/B][/I][/B][/I][/B][/I][/B][/I][/B][/I][/B][/FONT]<br>`+
`[SIZE=4][FONT=times new roman][B]Главный Следящий за АП — [/B][URL='https://vk.com/id659131672'][B][B]*КЛИК*[/B][/B][/URL][/FONT]<br>`+
`[SIZE=4][FONT=times new roman][B]Заместитель [/B]Главного Следящего[B] за АП — [/B][/FONT][/I][URL='https://vk.com/id631993243'][B][B][SIZE=4][FONT=times new roman][I]*КЛИК*[/I][/FONT][/B][/B][/URL][/COLOR]<br>`+
` <br>`+
`[SIZE=4][FONT=times new roman][COLOR=rgb(209, 213, 216)][I]Контакты даны с целью избежания обманов со стороны людей, которые представляются как ГС АП, ЗГС АП и т.п.[/I][/COLOR]<br>`+
`[I][COLOR=rgb(209, 213, 216)]Не нужно писать в ЛС с вопросами по типу: "Проверьте заявки", "Когда проверят заявки".[/COLOR][/I][/FONT]<br>`+
` <br>`+
`[B][FONT=times new roman][SIZE=5][COLOR=rgb(255, 0, 0)][I]ОСТЕРЕГАЙТЕСЬ МОШЕННИКОВ, НИКАКОЙ АДМИНИСТРАТОР НЕ ПОПРОСИТ ПАРОЛЬ ОТ ВАШЕГО АККАУНТА![/I][/COLOR]<br>`+
` <br>`+
"[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY [/B][/COLOR]",
      status: false,
        },
{
title: `На рассмотрении`,
content:  '[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
`[CENTER] Ваша жалоба закреплена и находится на рассмотрении.<br><br>` +
`Пожалуйста ожидайте ответа.<br>` +
`[COLOR=orange]На рассмотрение.[/color][/CENTER][/FONT]` +
      "[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY[/B][/COLOR]",
      status: false,
},
{
  title: '╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴Одобрено ╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴',
    },
    {
    title: `Профилактическая беседа`,
content: '[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
` Благодарим за ваше обращение!<br>` +
` С Агентом Поддержки будет проведена профилактическая беседа.<br>` +
` [COLOR=lightgreen]Одобрено[/color],закрыто[/CENTER]` +
      "[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY[/B][/COLOR]",
      status: false,
},
{
title: `Агенту Поддержки выдано наказание`,
content: '[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
`Благодарим за ваше обращение!<br>` +
`Лидер получит соответствующие наказание` +
`[COLOR=lightgreen]Одобрено[/color],закрыто [/CENTER]` +
      "[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY[/B][/COLOR]",
      status: false,
},
{
 title: '╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴Передачи жалобы ╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴',
    },
    {
    title: `Передана ГСХ`,
content:  '[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
`[CENTER] Ваша жалоба передана на рассмотрение Главному Следящему за АП.<br><br>` +
`Пожалуйста ожидайте ответа.<br>` +
`[COLOR=orange]На рассмотрение.[/color][/CENTER]` +
      "[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY[/B][/COLOR]",
      status: false,
},
{
     title: '╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴Отказано ╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴',
    },
    {
title: `Не в тот раздел`,
content: '[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
`[CENTER]Ошиблись разделом,пожалуйста напишите свою жалобу в раздел «РАЗДЕЛ»<br>` +
`[CENTER][[COLOR=red]Отказано[/color],закрыто[/CENTER]` +
      "[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY[/B][/COLOR]",
      status: false,
},
{
title: `Не является АП`,
content: '[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
`[CENTER]Данный игрок не являеться Агентом Поддержки.<br>` +
`[CENTER][COLOR=red] Отказано[/color],закрыто.[/CENTER]` +
  "[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY[/B][/COLOR]",
      status: false,
},
{
title: `Агент Поддержки снят`,
content: '[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
` [CENTER] Благодарим за ваше обращение!<br>` +
` [CENTER] Агент Поддержки снят со своего поста. <br>` +
` [CENTER] [COLOR=lightgreen]Одобрено[/color],закрыто.[/CENTER]` +
      "[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY[/B][/COLOR]",
      status: false,
},
{
title: `Недостаточно доказательств`,
content:'[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
`[CENTER] Недостаточно доказательств, которые потверждают нарушение Агента Поддержки.<br>`+
`[CENTER][[COLOR=red]Отказано[/color],закрыто.[/CENTER]` +
      "[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY[/B][/COLOR]",
      status: false,
},
{
title: `Нарушений нет`,
content:'[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
`[CENTER] Не увидел нарушений со сттороны Агента Поддержки!<br>`+
`[CENTER] [COLOR=red] Отказано,[/color] закрыто.[/CENTER]` +
      "[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY[/B][/COLOR]",
      status: false,
},
{
title:
`Пересмотрена`,
content:         '[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
`Ваша жалоба была пересмотрена!<br>` +
`Агент Поддержки получит соответствующие наказание` +
`[COLOR=lightgreen]Одобрено[/color],закрыто [/CENTER]` +
      "[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY[/B][/COLOR]",
      status: false,
},
{
title: `Правила раздела`,
content:'[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
`[CENTER]Пожалуйста, убедительная просьба, ознакомится с назначением данного раздела в котором Вы создали тему, так как ваш запрос никоим образом не относится к предназначению данного раздела.[/CENTER]`+
`[CENTER][COLOR=red] Отказано[/color], закрыто.[/CENTER]<br><br>` +
      "[I][COLOR=rgb(255, 255, 255)]Приятной игры на[/COLOR][COLOR=rgb(255, 0, 0)][B] Black Russia[/B][/COLOR] [/I][COLOR=rgb(0, 100, 0)][B]GROZNY[/B][/COLOR]",
      status: false,
   },
  ];

 $(document).ready(() => {
    // Загрузка скрипта для обработки шаблонов
    $('body').append('<script src="https://cdn.jsdelivr.net/npm/handlebars@latest/dist/handlebars.js"></script>');

    // Добавление кнопок при загрузке страницы
    addButton('На рассмотрение', 'pin');
    addButton('Одобрено', 'odobreno');
    addButton ('Отказано', 'otkaz');
    addButton('Закрыто', 'close');
    addButton('Click me', 'selectAnswer');


    // Поиск информации о теме
    const threadData = getThreadData();

     $('button#otkaz').click(() => editThreadData(OTKAZRP_PREFIX, true));
    $('button#pin').click(() => editThreadData( NARASSMOTRENIIRP_PREFIX, true));
    $('button#close').click(() => editThreadData(CLOSE_PREFIX, true));
    $('button#odobreno').click(() => editThreadData(ODOBRENORP_PREFIX, true));



     $(`button#selectAnswer`).click(() => {
      XF.alert(buttonsMarkup(buttons), null, 'Выберите ответ:');
      buttons.forEach((btn, id) => {
        if (id > 0) {
          $(`button#answers-${id}`).click(() => pasteContent(id, threadData, true));
        }
        else {
          $(`button#answers-${id}`).click(() => pasteContent(id, threadData, false));
        }
      });
    });
  });

  function addButton(name, id) {
    $('.button--icon--reply').before(
      `<button type="button" class="button rippleButton" id="${id}" style="margin: 3px;">${name}</button>`,
    );
  }

  function buttonsMarkup(buttons) {
    return `<div class="select_answer">${buttons
  .map(
  (btn, i) =>
    `<button id="answers-${i}" class="button--primary button ` +
    `rippleButton" style="margin:5px"><span class="button-text">${btn.title}</span></button>`,
  )
  .join('')}</div>`;
  }

  function pasteContent(id, data = {}, send = false) {
    const template = Handlebars.compile(buttons[id].content);
    if ($('.fr-element.fr-view p').text() === '') $('.fr-element.fr-view p').empty();

    $('span.fr-placeholder').empty();
    $('div.fr-element.fr-view p').append(template(data));
    $('a.overlay-titleCloser').trigger('click');

    if (send == true) {
      editThreadData(buttons[id].prefix, buttons[id].status);
      $('.button--icon.button--icon--reply.rippleButton').trigger('click');
    }
  }

  function getThreadData() {
    const authorID = $('a.username')[0].attributes['data-user-id'].nodeValue;
    const authorName = $('a.username').html();
    const hours = new Date().getHours();
    return {
      user: {
        id: authorID,
        name: authorName,
        mention: `[USER=${authorID}]${authorName}[/USER]`,
      },
      greeting: () =>
        4 < hours && hours <= 11 ?
        'Доброе утро' :
        11 < hours && hours <= 15 ?
        'Добрый день' :
        15 < hours && hours <= 21 ?
        'Добрый вечер' :
        'Доброй ночи',
    };
  }

    function editThreadData(prefix, pin = false) {
// Получаем заголовок темы, так как он необходим при запросе
    const threadTitle = $('.p-title-value')[0].lastChild.textContent;

    if(pin == false){
        fetch(`${document.URL}edit`, {
          method: 'POST',
          body: getFormData({
            prefix_id: prefix,
            title: threadTitle,
            _xfToken: XF.config.csrf,
            _xfRequestUri: document.URL.split(XF.config.url.fullBase)[1],
            _xfWithData: 1,
            _xfResponseType: 'json',
          }),
        }).then(() => location.reload());
    } else  {
        fetch(`${document.URL}edit`, {
          method: 'POST',
          body: getFormData({
            prefix_id: prefix,
            title: threadTitle,
            pin: 1,
            _xfToken: XF.config.csrf,
            _xfRequestUri: document.URL.split(XF.config.url.fullBase)[1],
            _xfWithData: 1,
            _xfResponseType: 'json',
          }),
        }).then(() => location.reload());
    }




 if(pin == false){
        fetch(`${document.URL}edit`, {
          method: 'POST',
          body: getFormData({
            prefix_id: prefix,
            title: threadTitle,
            _xfToken: XF.config.csrf,
            _xfRequestUri: document.URL.split(XF.config.url.fullBase)[1],
            _xfWithData: 1,
            _xfResponseType: 'json',
          }),
        }).then(() => location.reload());
    } else  {
        fetch(`${document.URL}edit`, {
          method: 'POST',
          body: getFormData({
            prefix_id: prefix,
            title: threadTitle,
            pin: 1,
            _xfToken: XF.config.csrf,
            _xfRequestUri: document.URL.split(XF.config.url.fullBase)[1],
            _xfWithData: 1,
            _xfResponseType: 'json',
          }),
        }).then(() => location.reload());
           }


function moveThread(prefix, type) {
// Получаем заголовок темы, так как он необходим при запросе
const threadTitle = $('.p-title-value')[0].lastChild.textContent;

fetch(`${document.URL}move`, {
  method: 'POST',
  body: getFormData({
    prefix_id: prefix,
    title: threadTitle,
    target_node_id: type,
    redirect_type: 'none',
    notify_watchers: 1,
    starter_alert: 1,
    starter_alert_reason: "",
    _xfToken: XF.config.csrf,
    _xfRequestUri: document.URL.split(XF.config.url.fullBase)[1],
    _xfWithData: 1,
    _xfResponseType: 'json',
  }),
}).then(() => location.reload());
}

function getFormData(data) {
    const formData = new FormData();
    Object.entries(data).forEach(i => formData.append(i[0], i[1]));
    return formData;
  }
    }
})();