...
(function () {
    ...
    var seasonIds = [];
    var allSeasonIds = []; // Lưu tất cả các mùa giải
    $.ajaxSetup({
        async: false
    });
    $.ajax('https://trophymanager.com/history/club/transfers/' + clubId, {
        ...
        success: function (response) {
            ...
            for (let i = 0; i < comboSeason.length; i++) {
                ...
                allSeasonIds.push(comboSeason[i].value);
            }
        },
        error: function (e) {}
    });
    ...
    var displaySeasonIds = []; // Các mùa để hiển thị
    if (seasonCount > 0 && seasonCount < seasonIds.length) {
        ...
        displaySeasonIds = [...seasonIds]; // Lưu các mùa để hiển thị

        // Mở rộng seasonIds để load thêm một số mùa trước đó
        // Điều này giúp xác định đúng trade vs academy
        let oldestDisplaySeason = parseInt(seasonIds[seasonIds.length - 1]);
        let extendCount = 0;
        for (let i = 0; i < allSeasonIds.length && extendCount < 10; i++) {
            let s = parseInt(allSeasonIds[i]);
            if (s < oldestDisplaySeason && !seasonIds.includes(allSeasonIds[i])) {
                seasonIds.push(allSeasonIds[i]);
                extendCount++;
            }
        }
    } else {
        ...
        seasonIds.forEach((seasonId) => {
            $.ajax('https://trophymanager.com/history/club/transfers/' + clubId + '/' + seasonId, {
                ...
                success: function (response) {
                    ...
                    // Chỉ tính summary cho các mùa trong displaySeasonIds
                    if (displaySeasonIds.includes(seasonId)) {
    ...
    function processPlayer() {
        ...
        for (let [key, value] of playerMap) {
            ...
            value.Transaction.forEach(tran => {
                ...
                if (temp[1] == 1) {
                    // Chỉ thêm vào boughtArr nếu mùa nằm trong displaySeasonIds
                    if (displaySeasonIds.includes(temp[0])) {
                        boughtArr.push({
                            Id: value.Id,
                            Name: value.Name,
                            Season: temp[0],
                    ...
                    if (waitSellForTrade) {
                        ...
                        if (tradeProfit >= minProfit && tradeProfit <= maxProfit) {
                            // Chỉ thêm vào tradeArr nếu cả buy và sell đều trong displaySeasonIds
                            if (displaySeasonIds.includes(temp[0])) {
                                tradeArr.push({
                                    Id: value.Id,
                                    Name: value.Name,
                        ...
                        // Chỉ thêm vào soldArr nếu mùa nằm trong displaySeasonIds
                        if (displaySeasonIds.includes(temp[0])) {
                            soldArr.push({
                                Id: value.Id,
                                Name: value.Name,
                        ...
                        // Chỉ thêm vào soldArr và academy nếu mùa nằm trong displaySeasonIds
                        if (displaySeasonIds.includes(temp[0])) {
                            soldArr.push({
                                Id: value.Id,
                                Name: value.Name,
    ...
    function processAcademy() {
        displaySeasonIds.forEach((seasonId) => {
    ...
    function showSummaryTransfer() {
        ...
        if (rowCount > APPLICATION_PARAM.SEASON_SHOW) {
            let buttonLabel = "Show all season " + displaySeasonIds[displaySeasonIds.length - 1] + " - " + displaySeasonIds[0];
