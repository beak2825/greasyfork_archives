...
// @grant GM_log
// @version 0.0.1.20150715222622
// @description KOC clicker
// @downloadURL https://update.greasyfork.org/scripts/10959/Yet%20Another%20Recruiter.user.js
// @updateURL https://update.greasyfork.org/scripts/10959/Yet%20Another%20Recruiter.meta.js
// ==/UserScript==
*/
...
function updateStats(response) {
  try {
    ...
    if (moraleText) {
      moraleStatusBox.textContent = AddCommas(moraleText.replace(/[^0-9\.]+/g,''));
    }
	//Hid this, not really necessary! Baigo
...
function handleSecurityRecaptcha(responseDetails) {
  if (responseDetails.responseText.indexOf(
    ...
    // This recaptcha trumps all the others.
     if (recaptchaType < RecaptchaType_SECURITY || recaptchaType == RecaptchaType_PROCESSING) {
...
function handleMisclickRecaptcha(responseDetails) {
  if (responseDetails.responseText.indexOf('Invalid selection...') >= 0) {
     if (recaptchaType < RecaptchaType_MISCLICK || recaptchaType == RecaptchaType_PROCESSING) {
...
function maybeSendRecaptchaResponse(e) {
  ...
  if (0xd == e.keyCode) {
    ...
    var responseDetails = new XMLHttpRequest();
    if (RecaptchaType_MISCLICK == recaptchaType) {
      var url = 'http://www.kingsofchaos.com/recruit.php?';
      data += '&uniqid=' + misclickedUniqid;
    } else if (RecaptchaType_SECURITY == recaptchaType) {
      var url = 'http://www.kingsofchaos.com/security.php';
    } else {
      GM_log('critical error!');
    }
	responseDetails.open("POST", url, false);
	responseDetails.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	 responseDetails.send(data);
...
function getNextCaptchaImage() {
  ...
  var responseDetails = new XMLHttpRequest();
  var url = 'http://www.kingsofchaos.com/recruit.php?uniqid=' + uniqid;
  responseDetails.open("GET", url, false);
  responseDetails.send();
    --captchaFetchesInProgress;
    ...
    var myGold = extractText(responseDetails.responseText,
    						 'Gold:<font color="#250202">', 
    						 '</font>').replace(/\D/g, '');
    document.getElementById('currentGold').textContent = AddCommas(myGold);
...
function sendRecruiterResponse(captchaObject, letter) {
  ...
  responseDetails.open("POST", url, false);
  responseDetails.setRequestHeader("Content-type","application/x-www-form-urlencoded");
  responseDetails.send( 'iuniqid=' + captchaObject.iuniqid
...
function initializeRecruiter() {
  ...
  document.body.innerHTML =
      ...
      '  </div>' +
      ' <div style="float:left;position:relative;width:200px;height: 100%;' +
      '              border-right: 1px solid #777;text-align: center; vertical-align: top">' +
      '    Gold: <span id="currentGold">0</span><br><br>' +
      ...
      '      </div>' +
	  '      <div>' +
      '        <button>Skip &#xbb;</button><button>Stop Clicking</button>' +
      '      </div>' +
      '    </div>' +
      '  </div>' +
      '</div>' +
      // Removed by Baigo '<div id="debugBar"></div>' +
...
*/
 ...
 //                                         false);
  ...
  buttons[9].addEventListener('click', maybeSendRecruiterResponseHelper(' '),
                               false);
  buttons[0].addEventListener('click', maybeSendRecruiterResponseHelper('K'),
                              false);
  buttons[1].addEventListener('click', maybeSendRecruiterResponseHelper('I'),
                              false);
  buttons[2].addEventListener('click', maybeSendRecruiterResponseHelper('N'),
                              false);
  buttons[3].addEventListener('click', maybeSendRecruiterResponseHelper('G'),
                              false);
  buttons[4].addEventListener('click', maybeSendRecruiterResponseHelper('C'),
                              false);
  buttons[5].addEventListener('click', maybeSendRecruiterResponseHelper('H'),
                              false);
  buttons[6].addEventListener('click', maybeSendRecruiterResponseHelper('A'),
                              false);
  buttons[7].addEventListener('click', maybeSendRecruiterResponseHelper('O'),
                              false);
  buttons[8].addEventListener('click', maybeSendRecruiterResponseHelper('S'),
                              false);
...
function whoAmI() {
  ...
  var URL = "http://www.kingsofchaos.com/base.php";
  details.open("GET", URL, false);
  details.send();
    myUniqid = extractText(details.responseText,
        '<a href="http://www.kingsofchaos.com/recruit.php?uniqid=', '">');
    if (!myUniqid) {
      showModalBox('1Something weird happened\u2026 try refreshing and ' +
                   'launching YAR again.');
      return;
    }
    myStatsid = extractText(details.responseText,
        '<a href="stats.php?id=', '">');

    var myGold = extractText(details.responseText,
    						 'Gold:<font color="#250202">', 
    						 '</font>').replace(/\D/g, '');
    document.getElementById('currentGold').textContent = AddCommas(myGold);

    if (!myStatsid) {
      showModalBox('2Something weird happened\u2026 try refreshing and ' +
                   'launching YAR again.');
      return;
    }
    loadClickList();
...
function requestQuery(){
	...
	var details = {};
	var urll = mainURL+'/yarstats.php?id='+myStatsid;
	GM_xmlhttpRequest({
		method: "GET",
		url: urll,
		onload: function(responseDetails) {
			var datas = responseDetails.responseText.split(",");
			document.getElementById('totalclickStatusBox').textContent = AddCommas(datas[0]);
			document.getElementById('todayclickStatusBox').textContent = AddCommas(datas[1]) + "/" + AddCommas(datas[2]);
			document.getElementById('clickleftStatusBox').textContent = AddCommas(datas[3]);
		}
	});
}
