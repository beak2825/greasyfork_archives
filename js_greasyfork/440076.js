...
// @name Virtonomica:City Festifal
// @description Сбор информации о дате запускм фестиваля
// @description Передача этой информации на сервер
// @include     https://virtonomica.ru/vera/main/politics/mayor/*/projects
// @version  0.01
// @grant    none
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/440076/Virtonomica%3ACity%20Festifal.user.js
// @updateURL https://update.greasyfork.org/scripts/440076/Virtonomica%3ACity%20Festifal.meta.js
// ==/UserScript==
var run = function() {
    ...
    // API
    let API_ADD_CITY_INFO = "https://test.pbliga.com/virta/polit.php?action=fest";
    ...
    // контрол с кнопкой
    let wc = $('<li class="my_btn" title="Отправить данные о Фестивале на сервер">' + ico_retail +'</li>');
    wc.click( function () {
        var send = new Object();
        // имя города
        ...
        let tr = $("div#mainContent table.list tr:contains('Проводить фестиваль можно не чаще, чем четыре раза в виртогод (один раз в 13 пересчетов)')");
        let td = $("td", tr).eq(3).html();
        //console.info( td );
        if ( td.indexOf("Проект на текущий момент недоступен.") > -1 ) {
            $("#polt_info").text('Фестиваль запущен!!!').css('color','green');
            send.fest = true;
            ...
            if ( str != '') {
                send.fest_date = str;
                $("#polt_info").append('<p>' + str );
            }
            // число дней до запуска проекта
            var str = get_substring(td, template_info_begin, tempate_info_end );
            send.days = -1;
            if ( str != '') {
                send.days = parseInt( str );
            };

        } else {
            $("#polt_info").text('Фестиваль отсутсвует!!!').css('color','red');
            send.fest = false;
        }
        console.info( send );
        // отаправка на сервер
        $("#polt_info").append('<p>Отправка данных на сервер');
