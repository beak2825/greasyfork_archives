// ==UserScript==
// @name         Wallhaven一键下载&收藏
// @version      2.0
// @description  在Wallhaven缩略图上添加下载按钮和复选框，支持单张下载、逐个下载和打包下载，同时加入收藏夹。首次运行时可输入 collectionId 并保存。
// @match        *://wallhaven.cc/*
// @require      https://cdnjs.cloudflare.com/ajax/libs/jszip/3.7.1/jszip.min.js
// @require      https://cdnjs.cloudflare.com/ajax/libs/FileSaver.js/2.0.5/FileSaver.min.js
// @grant        GM_addStyle
// @license      MIT
// @author       Silencestardust
// @namespace 略
// @downloadURL https://update.greasyfork.org/scripts/531812/Wallhaven%E4%B8%80%E9%94%AE%E4%B8%8B%E8%BD%BD%E6%94%B6%E8%97%8F.user.js
// @updateURL https://update.greasyfork.org/scripts/531812/Wallhaven%E4%B8%80%E9%94%AE%E4%B8%8B%E8%BD%BD%E6%94%B6%E8%97%8F.meta.js
// ==/UserScript==
...
(function() {
    ...
    const getToken = () => {
        ...
        for (let i = 0; i < forms.length; i++) {
            const input = forms[i].querySelector('input[name="_token"]');
            if (input) {
                token = input.value;
                break;
            }
    ...
    // 获取或设置 collectionId
    let collectionId = localStorage.getItem('wallhaven_collectionId');
    if (!collectionId) {
        collectionId = prompt('请输入 collectionId：');
        if (collectionId) {
            localStorage.setItem('wallhaven_collectionId', collectionId);
        } else {
            showProgress('未输入有效的 collectionId，部分功能可能无法正常使用。');
            return;
        }
    }
    ...
    if (thumbListing) {
        ...
        thumbListing.addEventListener('click', (e) => {
            ...
            if (btn) {
                ...
                const id = thumb.dataset.wallpaperId;
                const url = getImageUrl(id,!!thumb.querySelector('.thumb-info .png'));
                fetchWithRetry(url)
                   .then(blob => saveAs(blob, url.split('/').pop()))
                   .then(() => {
                        const token = getToken();
                        if (!token) {
                            throw new Error('无法获取 _token');
                        }
                        const extraCommandUrl = `https://wallhaven.cc/favorites/add?wallHashid=${id}&collectionId=${collectionId}&_token=${token}`;
                        return fetch(extraCommandUrl);
                    })
                   .then(response => {
                        if (!response.ok) {
                            throw new Error('执行额外命令失败');
                        }
                        return response.text();
                    })
                   .then(data => {
                        console.log('额外命令执行成功:', data);
                    })
                   .catch(err => {
                        showProgress('下载失败或执行额外命令失败，请重试。');
