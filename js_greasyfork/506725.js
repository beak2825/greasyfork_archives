...
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/506725/pz3D%EF%BC%88Dev%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/506725/pz3D%EF%BC%88Dev%EF%BC%89.meta.js
// ==/UserScript==
...
function awaitLoad() {
    ...
    return new Promise((res, rej) => {
        ...
        const groups_turnLight = [
            ['左转向灯亮', '左转向灯熄灭', '左转向灯不确定'],
            ['右转向灯亮', '右转向灯熄灭', '右转向灯不确定'],
        ]
        ...
        Obs(document, (mrs) => {
            ...
            mrs.some((mr) => {
                [...mr.addedNodes].some((an) => {
                            ...
                            mrs.forEach(mr => {
                                ...
                                if(mr.attributeName == 'class' && target.matches('.ant-checkbox-wrapper.ant-checkbox-wrapper-checked')) {
                                    ...
                                    groups.forEach(group => {
                                        ...
                                        group.some(option => {
                                            if(option == target.textContent) return false

                                            const optionLabel = labels.find(label => label.textContent.includes(option))
                                            if(optionLabel) {
                                                if(optionLabel.matches('.ant-checkbox-wrapper-checked')) optionLabel.click()
                                            }
                                        })
                                        const label_notAno = labels.find(label => label.textContent.includes('无需标注'))
                                        if(label_notAno.matches('.ant-checkbox-wrapper-checked')) label_notAno.click()
                                    })
                    ...
                    if(_ds.isSetHidden && an.matches?.('.DP-view') && an.textContent.includes('允许绘制到图像外')) {
                        const settingPanel = $('.ant-modal.index_TheStopModal__RTmQK')
                        const targetBtn = settingPanel.$$('.DP-row').find(item => item.textContent.includes('允许绘制到图像外'))?.$('button')
                        targetBtn.click()
                        settingPanel.$$('.DP-bottom button').find(btn => btn.textContent == '应 用')?.click()
