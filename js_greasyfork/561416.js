...
(function() {
    ...
    function applyPremiumLogo() {
        const logoSelectors = ["ytd-logo #logo-icon", "ytd-topbar-logo-renderer #logo", "a#logo svg"];
        logoSelectors.forEach(selector => {
            const container = document.querySelector(selector);
            if (container) {
                const svg = container.tagName === 'svg' ? container : container.querySelector("svg");
                if (svg && !svg.getAttribute('data-is-premium')) {
    ...
    // SVG Pfad angepasst für perfekte Zentrierung im 60px Container
    dwnldBtn.innerHTML = `
    ...
    dwnldBtn.addEventListener('click', (e) => {
        ...
        sources.forEach(src => {
            ...
            // Menü beim Klick auf einen Link automatisch schließen
    ...
    function setupSpeedInterface() {
     ...
     // --- 10. HANDLE ADS (Feinschliff für Direktlinks & Kaltstarts) ---
    function handleAds() {
        const video = document.querySelector('video');
        ...
        if (video && ad) {
            ...
            if (!video.muted) {
                video.muted = true;
                video.volume = 0; // Zusätzliche Sicherheit
            }

            // Geschwindigkeit bleibt wie gewünscht bei 2.0x
            ...
            // Lautstärke aus Speicher wiederherstellen
            const savedVol = sessionStorage.getItem('custom-player-volume') || 0.8;
            video.volume = savedVol;

            // Zurück auf die manuell gewählte Geschwindigkeit
            video.playbackRate = manualSpeed;
