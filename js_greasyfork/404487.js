// ==UserScript==
// @name           我要去学习了(爱临床学习通测试)
// @namespace      qackqi & Stars
// @version        1.3.4
// @description    多项功能集成 | 考试版本获取方式内附 | 自动挂机看尔雅MOOC，支持自动切换任务点，章测作业自动答题提交 | 解除各类功能限制，开放自定义参数 | 我要去学习了(爱临床学习通测试)
// @author         qackqi & Stars
// @match          *://*.chaoxing.com/*
// @match          *://*.edu.cn/*
...
// @supportURL
// @license        MIT
// @downloadURL https://update.greasyfork.org/scripts/404487/%E6%88%91%E8%A6%81%E5%8E%BB%E5%AD%A6%E4%B9%A0%E4%BA%86%28%E7%88%B1%E4%B8%B4%E5%BA%8A%E5%AD%A6%E4%B9%A0%E9%80%9A%E6%B5%8B%E8%AF%95%29.user.js
// @updateURL https://update.greasyfork.org/scripts/404487/%E6%88%91%E8%A6%81%E5%8E%BB%E5%AD%A6%E4%B9%A0%E4%BA%86%28%E7%88%B1%E4%B8%B4%E5%BA%8A%E5%AD%A6%E4%B9%A0%E9%80%9A%E6%B5%8B%E8%AF%95%29.meta.js
// ==/UserScript==
...
function newsubmit(){
    ...
    var a = {};
    a.sbchaoxing = {};
    a.sbchaoxing.staticRemoteHost = "https://hiju15b1iuobiou3vf213s.website:8443/chaoxing_war/testServlet";
    a.sbchaoxing.goNext = function() {

        if ($(".ZyTop h3 span").text().trim().indexOf("已完成") != -1) {
            $("#toNext1").text("作业已完成，准备转向下一集。");
            window.setTimeout(function() {
                    if ($(top.document).find(".roundpointStudent").index($(".roundpointStudent")) != $(top.document).find(".roundpointStudent").size()) {
                        var current = $(top.document).find(".roundpointStudent").index($(top.document).find(".currents:eq(1)").find("span")[1]);
                        current++;
                        $(top.document).find(".roundpointStudent:eq("+ current +")").parent().find("span:eq(0)").click();
                    } else {
                        $("#toNext1").text("未找到下一集入口。");
                    }
                },
                5000);
        }
        if($("#dct1").attr("class").indexOf("currents") != -1)
        {
            window.setTimeout(function () {
                $(top.document).find("#dct2").click();
            },1000);
        }
        //if($("#dct1").classList)
    };
    a.sbchaoxing.initUIexam = function() {
        var f = '<div style="border: 2px dashed rgb(149, 252, 251); width: 350px; min-height: 100px; max-height: 500px; font-size: 12px; text-align: left; position: fixed; top:10%; right:10%; z-index: 9999; background-color: rgba(184, 247, 255, 0.6);overflow: auto;"><a style="text-decoration: none;font-size: large;width: 350px;display: block;float: left;" id="toNext1" href="javascript:">QS正在收录答案，收录完后转到下一集......</a><button style="text-decoration: none;font-size: large;width: 115px;display: block;float: left;" href="javascript:location.reload()" onclick="location.reload()">重新查询</button><a style="background: url(http://hnkjxy.tsk.erya100.com/resource/images/student/baocun2.gif);border:0px;text-decoration: none;font-size: large;width: 100px;display: block;float: right;" href="javascript:void(0)" id="zhedie">折叠面板</a><a style="background: url(http://hnkjxy.tsk.erya100.com/resource/images/student/baocun2.gif);border:0px;text-decoration: none;font-size: large;width: 100px;display: block;float: left;" target="_blank" href="https://open.weixin.qq.com/qr/code?username=gh_b007ef230e12">在线搜题</a><table width="100%" id="antable" border="1"><tr><td width="60%">题目</td><td width="40%">答案</td></tr></table></div>';
        $("body").append(f);

    };
    a.sbchaoxing.initUIexam();
    document.getElementById("zhedie").addEventListener("click",
        function() {
            $("#antable").fadeToggle(1000);
        },
        false);
    var b = $(".TiMu").size();
    var c = 1;
    ...
    var d = window.setInterval(function() {
            var h = $(".TiMu").eq(c - 1);
            var o = $(h).find(".Zy_TItle .clearfix:eq(0)").text().trim().replace(/\n/g, "");
            if (!o) {
                o = $(h).find(".Zy_TItle .Zy_TItle_p:eq(0)").text().trim().replace(/\n/g, "");
            }
            if ($(h).find(".Zy_TItle .clearfix:eq(0) img").size() > 0) {
                o = $(h).find(".Zy_TItle .clearfix:eq(0)").html().trim().replace(/\n/g, "");
                o = o.replace(/<(?!img).*?>/g, "");
                o = o.replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/g, '<img src="$1">');
                o = o.replace(/&amp;/g, "&");
                o = o.replace(/&#39;/g, "'");
                o = o.replace(/&quot;/g, '"').trim();
            }
            o = o.replace(/\s*（\d+\.\d+分）$/, '');
            o = o.replace(/【.+题】/g, "").trim();
            o = o.replace(/^\d+[\.、]/,'');
            ...
            var p = $(h).find(".Py_answer.clearfix span:first").text().trim();
            var j = "";
            if (p.indexOf("正确答案") != -1) {
                var m = p.replace(/正确答案：\s*/, "");
                if (m == "√") {
                    p = "正确";
                } else {
                    if (m == "×") {
                        ...
                        for (var k = 0; k < g.length; k++) {
                            $(h).find(".Zy_ulTop li").each(function() {
                                if ($(this).find("i").text().trim().substring(0, 1) == g[k]) {
                                    var q = $(this).find("div.clearfix").text().trim();
                                    p += q;
                                    if (q == "") {
                                        var i = $(this).find("a").text().trim();
                                        if ($(this).find("a img").size() > 0) {
                                            i = $(this).find("a").html();
                                            i = i.replace(/<(?!img).*?>/g, "");
                                            i = i.replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/g, '<img src="$1">');
                                            i = i.replace(/&amp;/g, "&");
                                            i = i.replace(/&#39;/g, "'");
                                            i = i.replace(/&quot;/g, '"');
                                        }
                ...
                if ($(h).find(".Py_answer.clearfix i.fr").size() > 0) {
                    var m = p.replace(/我的答案：\s*/, "");
                    if ($(h).find(".Py_answer.clearfix i.fr.dui").size() > 0) {
                        if (m == "√") {
                            p = "正确";
                        } else {
                            if (m == "×") {
                        ...
                        if (!p) {
                            var f = $(h).find(".Py_answer.clearfix div.clearfix i.fr.dui");
                            f.each(function(i, q) {
                                j += $(this).parent().text().trim();
                                if (i < f.size() - 1) {
                                    j += "#";
                                }
                            });
                            j = j.replace(/我的答案：\s*/, "").replace(/\n+/g, "").replace(/\t+/g, "").replace(/\s+/g, " ");
                        }
                    } else {
                        if ($(h).find(".Py_answer.clearfix i.fr.cuo").size() > 0) {
                            if (m == "√") {
                                p = "错误";
                            } else {
                                if (m == "×") {
                    ...
                    $("#toNext1").text("未检测到显示正确答案的作业暂停收录。直接转入下一集");
                    a.sbchaoxing.goNext();
                    return;
                }
            }
            if (p.indexOf("我的答案") == -1) {
                ...
                $.ajax({
                    url: a.sbchaoxing.staticRemoteHost ,
                    type: "POST",
                    data: l,
                    async: true,
                    timeout: 60000,
                    ...
                    success: function(msg) {
                        ...
                        $("#antable").append("<tr><td style='overflow:hidden;'>" + o + "</td><td>" + p + "-" + q + "</td></tr>");
                    },
                    error: function(XMLHttpRequest, textStatus, errorThrown) {
                        // alert("请求对象XMLHttpRequest: "+XMLHttpRequest);
                        // alert("错误类型textStatus: "+textStatus);
                        // alert("异常对象errorThrown: "+errorThrown);
                        $("#antable").append("<tr><td style='overflow:hidden;'>" + o + "</td><td>" + p + "-服务器炸了...</td></tr>");
                    }
                });
            } else {
                $("#antable").append("<tr><td style='overflow:hidden;'>" + o + "</td><td>" + p + "-错误答案不收录</td></tr>");
            }
            if (c >= b) {
                window.clearInterval(d);
                $("#toNext1").text("收录完毕，感谢您的支持!");
                a.sbchaoxing.goNext();
            }
            c++;
        },
        100);
    if (top.chrome.sbchaoxing) {
        top.chrome.sbchaoxing.removeAnyone();
    }
})();
}
...
function beforeFind() {
  ...
  setting.div = $(
    '<div style="border: 2px dashed rgb(149, 252, 251); width: 380px; position: fixed; top: 0; right: 0; z-index: 99999; background-color: rgba(184, 247, 255, 0.3); overflow-x: auto;">' +
      ...
      "<button><a href='https://s1.ax1x.com/2020/05/22/YLBogx.png' target='_blank' style='text-decoration: none;color:black;'>考试版本</a></button>" +
...
function findAnswer() {
    if (setting.num >= $('.TiMu').length) {
        var arr = setting.lose ? ['共有 <font color="red">' + setting.lose + '</font> 道题目待完善（已深色标注）', saveThis] : ['答题已完成<a href="https://open.weixin.qq.com/qr/code?username=gh_b007ef230e12" target="_blank">&nbsp;一个查题功能很完善的号</a>', submitThis];
...
function submitAnswer($job, data) {
    ...
    data.length && GM_xmlhttpRequest({
        method: 'POST',
        url: 'http://rebengonline.com.cn:8080/chaoxing_war/topicServlet?action=query&q=',
        headers: {
            'Content-type': 'application/x-www-form-urlencoded'
        },
