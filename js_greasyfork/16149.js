...
// @version     4.0.033
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/16149/HIT%20Scraper%20%5Bdev%5D%20FORK.user.js
// @updateURL https://update.greasyfork.org/scripts/16149/HIT%20Scraper%20%5Bdev%5D%20FORK.meta.js
// ==/UserScript==
  ...
  var ico = 'data:image/png;base64,',
    ...
    defaults = { //{{{
      themes: {//{{{
        ...
        classic: { 
          ...
          hitDB       :'#66CC66', nohitDB    :'#FF3030', unqualified :'#9F9F9F', reqmaster :'#FF3030', nomaster     :'#000000',
    ...
    Settings = {//{{{
      ...
      user: JSON.parse(localStorage.getItem('scraper_settings')) || {//{{{
        ...
        toWeights: { comm: '0', pay: '3', fair: '3', fast: '0' },
          ...
          _general = //{{{
           `<div>
             ...
             <div style="float:left; margin-left:15px">
              ...
              <input id="comm" type="number" min="0" max="5" step="0.5" value=${this.user.toWeights.comm} style="width:40px"/></p>
              ...
              <input id="fast" type="number" min="0" max="5" step="0.5" value=${this.user.toWeights.fast} style="width:40px"/></p>
             </div>
    ...
    Interface = {//{{{
      ...
      draw: function() {//{{{
        ...
        //{{{ body
        body = `
          <audio src=${_audio0} id="ding" volume="1"></audio><audio src=${_audio1} id="squee" volume="1"></audio>
          ...
          <div id="controlpanel" class="controlpanel cpdefault">
           ...
           <table id="resultsTable" style="width:100%">
            ...
            <thead><tr style="font-weight:800;font-size:0.87em;text-align:center">
             <td style="width:15px">M?</td><td style="width:15px">Q?</td><td>Reward</td><td>Requester</td><td style="width:30px">Avg TO</td><td style="width:30px">TO Pay</td><td style="width:30px"># Rev</td><td>Title</td><td style="width:35px"># Avail</td><td>Time</td>
...
/*              AAtime: get('a[id^="autoapprove_data"]').parentNode.nextElementSibling.textContent, */
      ...
      meld: function() {//{{{
        var reviews = arguments.length ? arguments[0] : null,
          ...
          addRowHTML = r => {//{{{
            ...
            return `<tr class="${r.blocked ? 'blocklisted' : ''} ${shouldHide ? 'ignored hidden' : ''} ` +
              `${r.included ? 'includelisted' : ''} ${r.rowColor} ${r.shine ? 'shine' : ''}">` +
            `<td class="${r.masters ? 'reqmaster' : 'nomaster'}" style="text-align:center">${r.masters ? 'M' : ''}</td>` +
            `<td class="${r.qualified ? 'tooweak' : 'reqmaster'}" style="text-align:center">${r.qualified ? 'Q' : 'N'}</td>` +
            /* `${r.qualified ? '' : '<td class="tooweak" title="Not qualified to work on this HIT">NQ</td>'}` + */
            `<td style="text-align:center"><a target="_blank" ${r.hit.panda ? 'href="'+r.hit.panda+'"' : ''}>${r.pay}</td>` +
            ...
            `<td style="text-align:center"><a class="static toLink" target="_blank" data-rid="${r.requester.id ? r.requester.id : 'null'}" ` +
              ...
              (r.TO ? Math.round(r.TO.attrs.qual*10)/10 : 'n/a') + createTooltip('to',r.TO) + '</a></td>' +
            `<td style="text-align:center">${r.TO ? Math.round(r.TO.attrs.pay*10)/10 : 'n/a'}</td>` +
            `<td style="text-align:center">${r.TO ? (r.TO.tos_flags ? r.TO.reviews + '*' : r.TO.reviews) : 'n/a'}</td>` +
            ...
            `<td style="text-align:center" >${r.time.replace(/([a-z])\w+|\s+/g, "$1")}</td>` +
           /* `<td style="text-align:center" >${r.AAtime}</td>` + */
        ...
        results = scraperHistory.filter(v => { 
          ...
          // BOTTLES (to fix: 1. make Q column show M for masters 2. add AA? 3. make # Avail column into TM links!)
            ...
            templateVars = {//{{{
              ...
              toText: (function(){//{{{
                var _to = this.record.TO, txt = '', color, _attr, sym = Settings.user.vbSym,
                  _long = { comm: 'Comm', pay: 'Pay', fair: 'Fair', fast: 'Prompt' };
  ...
  function createTooltip(type,obj) {//{{{
    var html, reason = typeof obj === 'object' || Settings.user.disableTO ? ': TO disabled in user settings' 
      ...
      _genMeters = function() {
        var attrmap = { comm: 'Comm', pay: 'Pay', fair: 'Fair', fast: 'Prompt' }, html = [];
    ...
    if (!obj) {
      ...
      html = `<div class="tooltip" style="width:100px">
        ${_genMeters()}</div>`;
