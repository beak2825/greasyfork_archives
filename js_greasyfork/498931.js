...
// @downloadURL https://update.greasyfork.org/scripts/498931/runnum-tools-new.user.js
// @updateURL https://update.greasyfork.org/scripts/498931/runnum-tools-new.meta.js
// ==/UserScript==
...
const monkey_url = "http://18.142.199.10:5000/sendData";
let isLoaded = false
let data_url = "";
let post_data = {};
let uuid = {};
...
function listenForRequests() {
    ...
    XMLHttpRequest.prototype.open = function (method, url) {
        ...
        if (url.includes("/share/share/")) {
            ...
            data_url = "http://" + window.location.hostname + ":" + window.location.port + "/share/share/api_yinliu_count.html?page=1&limit=500&id=&class_id=&is_repet=1&start_time=&end_time=";
        }

        console.log("window.location.hostname --- " + window.location.hostname)
        if (window.location.hostname.includes("haiwang.app")) {
            console.log('海王 工单-----' + url);
            //https://www.haiwang.app/web/accountshow/list?page=1&limit=10&sort=acclist_id%20DESC&sharekey=F0EmlgDy3&shareid=23812&password=
            var sharekey = url.match(/sharekey=([^&]+)/);
            var shareid = url.match(/shareid=([^&]+)/);
            var password = url.match(/password=([^&]+)/);
            data_url = "https://" + window.location.hostname + "web/accountshow/list?page=1&limit=500&sort=acclist_id%20DESC&sharekey="+ sharekey + "&shareid=" + shareid + "&password=" + password;
            haiwangCode = sharekey + "-" + shareid
        }
    ...
    if (window.location.href.includes("qihang") || window.location.href.includes("sihai") || window.location.href.includes("yinchao")) {
        ...
        if (data_url.includes("/share/share/")) {
            headers = {
                ...
                "Referer": "http://" + window.location.hostname + ":" + window.location.port + "/share/share/api_yinliu_count.html?token=" + shareToken,
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (response) {
                ...
                if (window.location.host == "kf.007.tools") {
                    ...
                    dataStr = JSON.stringify(aa)
                } else if (data_url.includes("haiwang.app")) {
                    let aa = getHaiwangData(response)
                    dataStr = JSON.stringify(aa)
                }
...
function getHaiwangData(response) {
    var text = response.responseText;
    console.log("text --- "  +text)
    var new_data = {};
    ...
    for (var i = 0; i < data.data.length; i++) {
        ...
        // 这里减去了重复的fans
        intoAllFans += item.acclist_todaynum;
        var new_item = {
            numId: item.acclist_username,
            state: acclist_status,
            intoFans: parseInt(item.acclist_todaynum),
            repeatFans: 0
        };
...
function createBtn() {
    ...
    if (window.location.hostname == "www.haiwang.app") {
        var buttons = document.querySelectorAll('button');
        // Iterate through the buttons to find the "查询" button
        let resetBtn = null;
        buttons.forEach(function (button) {
        ...
        if (resetBtn != null) {
            // var parentButton = resetBtn.closest('.ant-btn'); // 找到包含按钮图标的父按钮
            var parentDiv = resetBtn.closest('.el-button');
            if (parentDiv) {
                let extractButton = parentDiv.cloneNode(true);
                extractButton.textContent = '提取号码'; // 修改按钮文本
                ...
                extractButton.addEventListener('click', function () {
                    ...
                    if (extractButton.textContent == '复制提取码') {
                        ...
                        copyToClipboard(haiwangCode)
 
                        // 显示提示信息
                        messageElement.textContent = '复制成功';
                        extractButton.parentNode.parentNode.parentNode.appendChild(messageElement);
                        // 延迟一定时间后清除提示信息
                        setTimeout(function () {
                            extractButton.parentNode.parentNode.parentNode.removeChild(messageElement);
                        }, 2000); // 2秒后清除提示信息
                    } else {
                        startSend();
                    }
                ...
                // 在按钮后插入提取号码按钮
                resetBtn.parentNode.appendChild(extractButton);
            }
        }
    }
