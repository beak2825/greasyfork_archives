...
// @version      2.1.0
// @description  Скрипт для кураторов форума сервера Podolsk
// @author       Waldemar_Doxbinov / Alnaro_Tessa
...
// @downloadURL
// @updateURL
// @downloadURL https://update.greasyfork.org/scripts/552831/Podolsk%20%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%BA%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%20%D1%84%D0%BE%D1%80%D1%83%D0%BC%D0%B0.user.js
// @updateURL https://update.greasyfork.org/scripts/552831/Podolsk%20%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%BA%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%20%D1%84%D0%BE%D1%80%D1%83%D0%BC%D0%B0.meta.js
// ==/UserScript==
  ...
  const buttons = [
      ...
      },
      {
         ...
         `${START_COLOR_1}{{ greeting }}, уважаемый(-ая) {{ user.mention}}${END_COLOR + START_COLOR_2}.${END_COLOR}<br><br>` +
          ...
          `${START_COLOR_2}Просьба не создавать дубликаты данной темы, иначе ваш форумный аккаунт может быть заблокирован.${END_COLOR}<br><br>` +
        `${START_DECOR}<img src="https://i.ibb.co/5g8DHg4j/1000039734.pnghttps://i.ibb.co/grLRvQS/image.png"><br>` +
          `${START_COLOR_1}Ожидайте ответа${END_COLOR}${START_COLOR_2}.${END_COLOR}${END_DECOR}`,
          prefix: WAIT_PREFIX,
          status: true,
          move: 0,
    ...
    },
    {
     ...
     },
     {
         title: 'Обход системы / Багоюз',
          content:
          `${START_DECOR}<img src="https://i.ibb.co/Nd2GMnyr/1000039677.png://i.ibb.co/grLRvQS/image.png"><br>` +
          `${START_COLOR_1}{{ greeting }}, уважаемый(-ая) {{ user.mention}}${END_COLOR + START_COLOR_2}.${END_COLOR}<br><br>` +
...
Пример: в итогах игрок описывает, что его персонаж стал психически больным и теперь убивает всех, кого видит.)[/QUOTE]${END_COLOR}<br><br>` +
    ...
    const organizations = [
        ...
        },
        {
          ...
          `${START_COLOR_1}{{ greeting }}, уважаемый(-ая) {{ user.mention}}${END_COLOR + START_COLOR_2}.${END_COLOR}<br><br>` +
          `${START_COLOR_2}Ваша Roleplay организация составлена${END_COLOR} ${START_COLOR_1}не по форме${END_COLOR}${START_COLOR_2}.<br>Ознакомиться с правилами составления Roleplay организаций вы можете в этой теме - *<a href='https://forum.blackrussia.online/threads/Podolsk-Правила-создания-roleplay-организаций.13684213/'>Кликабельно</a>*.${END_COLOR}<br><br>` +
          `${START_COLOR_2}Просьба не создавать дубликаты данной темы, иначе ваш форумный аккаунт может быть заблокирован.${END_COLOR}<br><br>` +
