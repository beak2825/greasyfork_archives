...
// @namespace https://greasyfork.org/users/2503
// ==/UserScript==
...
TW_Calc.craft.updateLastCraft = function(){
	$.get("game.php", {window: "crafting"}, function(d){
		TW_Calc.craft.dataLastCraft = new Object();
...
TW_Calc.craft.launch = function(id, input){
	try{
		if(TW_Calc.craft.dataLastCraft == undefined){
			TW_Calc.craft.dataLastCraft = new Object();
		}
		...
		while(new Boolean(TW_Calc.craft.craft[id][i])!=false){
				...
				var craftText = (TW_Calc.craft.dataLastCraft[reciepeId] == null ? TW_Calc.lang.lang_177 : '<span style="color:yellow; cursor:default;">'+TW_Calc.craft.dataLastCraft[reciepeId].formatDurationBuffWay()+'</span>');
				var selector = ".TW-CALC-QUEST > div.tw2gui_window_content_pane > #tab_"+u+" > #craft_content > #TWCalc_craft_content.tw2gui_groupframe > .tw2gui_groupframe_content_pane > div > .tw2gui_scrollpane_clipper > div";
		...
		switch(d){
			case 2:
				...
				if(new Boolean(data.quests.series[id].ready)!=false){
					if(data.quests.series[id].ready=="false"){TW_Calc.q='<span style="display:inline-block;width:100%"><b>Táto úloha ešte nebola spracovaná<b></span>';};
				}else{
			...
			default:
				...
				while(new Boolean(data.quests.series[i])!=false){
					...
					switch(data.quests.series[i].clas){
						case "Všetky": var questclass = TW_Calc.lang.quest.lang_4; break;
						case "Pracovník": var questclass = TW_Calc.lang.lang_42; break;
...
TW_Calc.TombolaExporter.createData = function(a,b){
	...
	if(b == 2){
		if(new Boolean(a.outcome) != false){
			var prize = a.outcome.itemId;
			var c = a.outcome.itemEnhance;
			switch(c){
				case 25: var category = 1; break;
				case 150: var category = 2; break;
				case 800: var category = 3; break;
				default: var category = 0; break;
			}
			TW_Calc.TombolaExporter.exportData(prize,b,category);
			TW_Calc.TombolaExporter.saveData(prize,b,category);
		}
...
TW_Calc.TombolaExporter.Tab.launch = function(){
	try{
		...
		var o = TW_Calc.TombolaExporter.createObjectFromStorage(2);
		if(o==null){
			o = [];
		}else{
			$('#tab_twcalc8>.tw2gui_scrollpane>.tw2gui_scrollpane_clipper>.tw2gui_scrollpane_clipper_contentpane').append('<h2 style="margin-left:0px;padding-top:0px;">Easter:&nbsp;<span id="TW_Calc_TombolaExporter_Tab_Groupframe_ID"></span></h2>');
		}
		var i = 0;
		for(i;i < o.length;i++){
			var k = 0;
			if(o[i].length!=0){
				...
				var html = '<div id="TW_Calc_TombolaExporter_Tab_Easter_Items_'+i+'" style="background:'+bg+';float:left;width:636px;margin:5px;padding:10px;border: 3px solid #a49e97; border-radius: 8px; box-shadow: 0 0 20px inset; opacity: 0.9; left: 0; right: 0; top: 0; bottom: 0;"></div>';
				$('#tab_twcalc8>.tw2gui_scrollpane>.tw2gui_scrollpane_clipper>.tw2gui_scrollpane_clipper_contentpane').append(html);			
			}
			for(k;k < o[i].length;k++){
				var itemObj = ItemManager.get(Number(o[i][k].id));
				...
				$('.count',item).css('display','block');
				$('.count>p',item).html(count)
				$('#TW_Calc_TombolaExporter_Tab_Easter_Items_'+i).append(item);
			}
		}
		var totalSpins = 0;
		var freeSpins = 0;
