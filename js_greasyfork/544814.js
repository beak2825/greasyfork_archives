...
// @version      v1.91
// @description  🚀新功能上线，不浪费一秒，会员随时停启💰。付费内容无限次观看、便捷下载和即时分享链接。内容自动展开和完全无广告的环境。🚀
// @description:zh-TW   🚀新功能上線，不浪費一秒，會員隨時停啟💰。付費內容無限次觀看、便捷下載和即時分享連結受益於，內容自動展開和完全無廣告的環境。🚀
// @author            lx
// @match        *://h5dfsg.anwangjd1.com/*
// @match        *://d3df6hjcjf7ng5.cloudfront.net/*
...
// @downloadURL https://update.greasyfork.org/scripts/544814/%E9%BA%BB%E8%B1%86%E5%82%B3%E5%AA%92.user.js
// @updateURL https://update.greasyfork.org/scripts/544814/%E9%BA%BB%E8%B1%86%E5%82%B3%E5%AA%92.meta.js
// ==/UserScript==
...
(function () {
  ...
  async function togglePlay() {
    ...
    if (m3u8Url == "") {
      showApiMessage("请先获取视频链接", "error");
      return;
    }
    m3u8Url = substringAfter(m3u8Url, "media/m3u8/");
    if (!m3u8Url.includes("m3u8ex")) {
      m3u8Url = replaceAfterMarker(prevM3u8, "m3u8ex", m3u8Url);
    }
    ...
    m3u8Content = m3u8Content.replace(
      "/api/app/media/enkey",
      "https://d1kek4wgeaw03m.cloudfront.net/api/app/media/enkey"
    );
          ...
          if (xhr.status === 200) {
            try {
              ...
              Object.keys(urlM3u8Patterns).forEach((pattern) => {
                if (new RegExp(pattern).test(requestURL)) {
                  prevM3u8 = window.location.origin + requestURL;
                }
              });
            } catch (error) {
  ...
  // 初始化播放器
  function initPlayer(container, videoUrl) {
    waitForElement(".mine-ad").then((e) => {
      console.log("e", e);
      e.remove();
    });
    ...
    dp.on("play", () => {
      waitForElement(".dplayer-mobile-play").then((res) => {
    ...
    waitForElement(".dplayer-play-icon").then((res) => {
      res.style.display = "inline";
    });

    window.dp = dp; // 挂载到 window 方便调试
  }
  async function jljdRequest() {
    ...
    unsafeWindow.fetch = async function (url, options) {
      ...
      if (url.endsWith("/media/play") || url.endsWith("/onepost/detail")) {
        slefVideo.id = window.location.href.split("/").pop();
        const clone = response.clone();
        let responseJson = await clone.json();
        slefVideo.signData = responseJson.data;
        await serializeVideo();
        if (slefVideo.id.includes("video-detail")) {
          waitForElement("#player")
            .then((container) => {
        ...
        `;
          ...
          `;
              notice.textContent =
                "温馨提示：请用右边悬浮导航的播放按钮进行观看！";
              container.parentNode.insertBefore(noticeContainer, container);
              noticeContainer.appendChild(container);
              noticeContainer.appendChild(notice);
            })
            .catch(console.error);

          waitForElement(".swiper_main_ad").then((res) => {
            res.style.display = "none";
          });
          // waitForElement(".mine-ad").then((res) => {
          //   res.style.display = "none";
          // });
        } else {
          waitForElement(".md-video-play").then((res) => {
  ...
  function replaceAfterMarker(url, marker, replacement) {
    let parsedUrl;
    try {
    ...
    const markerIndex = pathSegments.findIndex((seg) => seg.includes(marker));

    if (markerIndex === -1) {
      throw new Error(
        `Marker "${marker}" not found in path: ${parsedUrl.pathname}`
      );
    }
    const endIndex = markerIndex + 1;

    if (endIndex >= pathSegments.length) {
      pathSegments.push(replacement);
    } else {
      pathSegments.splice(
        endIndex,
        pathSegments.length - endIndex,
        replacement
      );
    }
