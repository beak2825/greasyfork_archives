...
// @description         Stable WME Suite: Roundabout Editor, City/Place/User Explorers
...
// @downloadURL https://update.greasyfork.org/scripts/561418/Abdullah%20Abbas%20WME%20Tools.user.js
// @updateURL https://update.greasyfork.org/scripts/561418/Abdullah%20Abbas%20WME%20Tools.meta.js
// ==/UserScript==

/*
 * Abdullah Abbas WME Tools
 * Version: 2026.01.04.1
 * Stable Release: Roundabout Editor, Search Tools, Lock Indicator.
 * Name updated in UI to "Abdullah Abbas WME Tools".
 */
...
(function() {
    ...
    const SCRIPT_VERSION = "2026.01.04.1";
    const DEFAULT_W = "340px";
    const DEFAULT_H = "460px";

    const STRINGS = {
        'ar-IQ': {
            main_title: 'Abdullah Abbas WME Tools', // الاسم الجديد
        ...
        'en-US': {
            main_title: 'Abdullah Abbas WME Tools', // New Name
    ...
    const RoundaboutEditor = {
        init: () => {
            const html = `
                <div style="text-align:center; padding:10px;">
                    <div style="margin-bottom:15px; background:#fff; border:2px solid #333; padding:10px; border-radius:8px;">
                        <span style="font-size:16px; font-weight:bold; color:#000;">${_t('unit_m')}: </span>
                        <input type="number" id="ra-val" class="aa-input" value="1" style="width:80px; display:inline-block; font-size:18px; font-weight:bold; text-align:center; border:1px solid #000;">
                    </div>

                    <div style="font-size:14px; font-weight:bold; margin-bottom:5px; color:#000;">تحريك (Move)</div>
                    <div class="aa-ra-controls">
                        <div></div><button id="ra_up" class="aa-btn aa-green aa-big-icon">▲</button><div></div>
                        <button id="ra_left" class="aa-btn aa-green aa-big-icon">◄</button><button id="ra_down" class="aa-btn aa-green aa-big-icon">▼</button><button id="ra_right" class="aa-btn aa-green aa-big-icon">►</button>
                    </div>

                    <div style="margin-top:20px;">
                        <div style="font-size:14px; font-weight:bold; margin-bottom:5px; color:#000;">تدوير (Rotate)</div>
                        <div class="aa-btn-group">
                            <button id="ra_rot_l" class="aa-btn aa-bg-red aa-huge-icon">↺</button>
                            <button id="ra_rot_r" class="aa-btn aa-bg-blue aa-huge-icon">↻</button>
                        </div>
                    </div>

                    <div style="margin-top:15px;">
                        <div style="font-size:14px; font-weight:bold; margin-bottom:5px; color:#000;">حجم (Size)</div>
                        <div class="aa-btn-group">
                            <button id="ra_shrink" class="aa-btn aa-teal" style="font-size:16px;">${_t('ra_in')}</button>
                            <button id="ra_expand" class="aa-btn aa-teal" style="font-size:16px;">${_t('ra_out')}</button>
                        </div>
                    </div>
                </div>
                <div id="ra_status" style="margin-top:15px; text-align:center; font-weight:bold; font-size:16px; color:red; border-top:2px solid #000; padding-top:10px;">${_t('ra_err')}</div>
            `;
            UIBuilder.createFloatingWindow('AA_RAWin', 'win_ra', 'aa-bg-green', html);
            W.selectionManager.events.register("selectionchanged", null, RoundaboutEditor.checkSelection);
        ...
        checkSelection: () => {
            ...
            let isRA = false;
            if(sel.length > 0 && sel[0].model.type === 'segment') if(WazeWrap.Model.isRoundaboutSegmentID(sel[0].model.attributes.id)) isRA = true;
        ...
        run: (action, multiplier) => {
            ...
            try { WazeActionUpdateSegmentGeometry = require('Waze/Action/UpdateSegmentGeometry'); WazeActionMoveNode = require('Waze/Action/MoveNode'); WazeActionMultiAction = require('Waze/Action/MultiAction'); } catch(e) { console.error("WME Modules not found", e); return; }
            ...
            try {
                if(action === 'ShiftLong' || action === 'ShiftLat') {
                    ...
                    for(let i=0; i<RASegs.length; i++){
                        ...
                        if(isLat) offset = WazeWrap.Geometry.CalculateLatOffsetGPS(val, c.lon, c.lat); else offset = WazeWrap.Geometry.CalculateLongOffsetGPS(val, c.lon, c.lat);
            ...
            } catch (e) { console.error("RA Operation Failed", e); }
        }
    };
    ...
    function injectCSS() {
        const css = `
            ...
            .aa-grid-3 { display:grid; grid-template-columns:1fr 1fr 1fr; gap:5px; margin:5px 0; }
            .aa-badge { background:#eee; padding:2px 6px; text-align:center; border-radius:10px; font-size:11px; font-weight:bold; color: #000 !important; }
            .aa-lock-opt { font-size:12px; font-weight:bold; display:flex; align-items:center; gap:5px; background:#fff; padding:5px; border-radius:4px; border:1px solid #ccc; }
    ...
    function buildSidebar() {
        ...
        // UPDATE HERE: Full Name in Tab Link
