...
// @downloadURL https://update.greasyfork.org/scripts/378164/tiebaAllsign.user.js
// @updateURL https://update.greasyfork.org/scripts/378164/tiebaAllsign.meta.js
// ==/UserScript==
...
function resetData() {
	GM_deleteValue('todaySigned');
	if (!GM_getValue('todaySigned')) {
...
function jjuds() {
	...
	function getTieba(nn) { //获取第n页的贴吧列表
		//console.log(nn);
		if (nn > 0 && nn <= 100) {
			var addTail = "&pn=" + nn;
				...
				onload : function (reText) {
					...
					if (ww!=null) {
						for (s = 0; s < ww.length; s++) {
							if (maxSign === 0 || allAncs.length < maxSign) {
...
GM_registerMenuCommand('全签到', jjuds);
GM_registerMenuCommand('重設已簽到記錄(調試用)', resetData);
