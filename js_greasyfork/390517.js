// ==UserScript==
// @name         Send love from Spotify to last.fm
// @author       CennoxX
// @contact      cesar.bernard@gmx.de
// @namespace    https://greasyfork.org/users/21515
// @description  Love or Unlove song on last.fm if it is loved at Spotify
// @version      0.1.0
// @match        https://open.spotify.com/*
// @grant        GM.xmlHttpRequest
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/390517/Send%20love%20from%20Spotify%20to%20lastfm.user.js
// @updateURL https://update.greasyfork.org/scripts/390517/Send%20love%20from%20Spotify%20to%20lastfm.meta.js
// ==/UserScript==
(function() {
    ...
    if (!sk){
        ...
        {
            window.location.replace("https://www.last.fm/de/api/auth?api_key="+api_key+"&cb=https://open.spotify.com/");
        }
    }
    var oldTitle = '';
    var track;
    ...
    function setLoveButtons(){
        var playingBar = document.querySelector("[class='now-playing-bar']");
        unloveButton = playingBar.querySelector("[class*='spoticon-heart-'][class*='control-button--active']");
        if (unloveButton != null)
        {
            unloveButton.addEventListener("click", function() {requestLastfm("track.unlove", track, artist);});
        } else {
            loveButton = document.querySelector("[class='now-playing-bar']").querySelector("[class*='spoticon-heart-']");
            loveButton.addEventListener("click", function() {requestLastfm("track.love", track, artist);});
        }
    }
    function requestLastfm(method, track, artist){
        //get song
        GM.xmlHttpRequest({
            method: 'GET',
            ...
            GM.xmlHttpRequest({
                ...
                onload: function(response) {
                    if (response.responseText.length > 0 && response.responseText.includes('<lfm status="ok" />')) {
                        ...
                        var reg = new RegExp(artist+"\\|"+track+".*");
                        //console.log(artist+": "+track +(method=="track.love"?"\t💗":"") + (method=="track.unlove"?"\t💔":""));
                        setLoveButtons();
                    }
                },
                onerror: function(response) {
