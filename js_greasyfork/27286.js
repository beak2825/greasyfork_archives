...
// @namespace		https://greasyfork.org/en/scripts/27286-drakbot
// @downloadURL https://update.greasyfork.org/scripts/27286/drakbot.user.js
// @updateURL https://update.greasyfork.org/scripts/27286/drakbot.meta.js
// ==/UserScript==
...
function emjack() {
	...
	ej.lbot=[
			...
			},
		{
			...
			callback: function(data) {
				sock.chat("Most recent command: @drakahn chance (of ___)", data.user);
				}
			},
		{
            name: "Scriptcheck",
			...
			},
		{
            ...
            regex: /^Greetings./i,
            callback: function(data){
                var msgArray = ["", "/me i'm feeling happy...", "/me play with me!", "/me i'm feeling robotic...", "/me play with me!", "/me i'm feeling stressed...", "/me play with me!", "/me i'm feeling bored...", "/me play with me!", "/me drakahn is feeling angry...", "/me play with me!", "/me play with me!", "/me play with me!", "/me i'm feeling angry...", "/me play with me!", "/me play with me!", "/me play with me!"]; 
...
name: "Sodoku", regex: /kys/i, callback: function(data) { if(data.user === master && king){ sock.chat("", data.user); sock.cmd("leave"); } else{ sock.chat("Fucking kill yourself", data.user); 
			...
			},
		{
			...
			responses: [
				"drakahn cooks an electronic steak, but it fails."
				]
			},
		{
            name: "Advice",
...
"Your freedom to swing your arm ends where my nose begins.",
			...
			callback: function(data) {
				...
				sock.chat(sformat(arand(this.responses)));
                    ...
                    kingofthehill=!kingofthehill 
                        ...
                        sock.chat("@drakahn You begin to fight "+kingofthehill);
			...
			},
		{
			...
			responses: [
				"The first one to say '@drakahn attack' wins. ⚔",
				]
			},
		{
            name: "Advice",
			...
			},
		{
            ...
            regex: /^inferno/i,
            callback: function(data){
                var msgArray = ["inferno running...", "when drakahn says 'inferno', say '1'. best out of 5 wins. you need to keep track of score.", "ready, set... Go!", "inferno", "inferno", "inferno",
...
"inferno"]; 
			...
			},
		{
			...
			responses: [
                ...
                "The best user on epicmafia.com is drakahn",
			...
			},
		{
			...
			responses: [
                ...
                "The winner of the lottery is... $1! Prize: A compliment. Type '@drakahn compliment' for a compliment.",
                ...
                "The winner of the lottery is... $1! Prize: A insult. Type '@drakahn insult' to recieve your free insult.",
                ...
                "The winner of the lottery is... $1! Prize: A free book. Type '@drakahn library' to recieve your free book!",
                ...
                "The winner of the lottery is... $1! Prize:  Your very own command! PM drakahn '87739571' and specify your command with what you want drakahn to say in response.",
                "The winner of the lottery is... $1! Prize:  Your very own command! PM drakahn '92835780' and specify your command with what you want drakahn to say in response.",
                ...
                "The winner of the lottery is... $1! Prize:  Another lottery ticket to a knock-off lottery called 'lootery', type @drakahn lootery to access.",
			...
			},
		{
			...
			callback: function(data) {
					...
					skunk=data.user;
					sock.chat("you peasant...", data.user);
			...
			},
		{
			...
			regex: /^story/i,
			callback: function(data) {
					sock.chat("You have just been born. Congratulations(?) To continue, type @drakahn next.", data.user);
				}
			},
		{
            name: "Bow",
			regex: /^next/i,
			callback: function(data) {
					sock.chat("You have just died. Sorry for your loss(?) To continue, type @drakahn 678.", data.user);
				},
			},
		{
            name: "Sing",
            ...
            regex: /^(678)/i,
            callback: function(data){
                var msgArray = ["After being born, you became a person with choices. This was inevitable, and now you have to make them.", "You are free to make them. Type @drakahn makechoice"]; 
            ...
            },
          {
            ...
            callback: function(data){
                ...
                var msgArray = ["Because you are two years old, there are many things you don't understand. One night, your mother feeds the cat.", "This upsets you. Why would she do that? Surely, you're more important to her than the cat is.", "To throw a fit, type '@drakahn fit'", "To throw your food across the kitchen, type '@drakahn throw'",]; 
                if(busy === 0){ busy = 1; var count = -1; for(i=0; i<msgArray.length; i++){ setTimeout(function(){ count = count + 1; sock.chat(msgArray[count]); }, 1500*i); } } busy = 0; 
            }
            },
          {
            ...
            callback: function(data){
                ...
                var msgArray = ["You throw your food accross the room","This should get you the attention you deserve","Type '@drakahn continue'",]; 
                if(busy === 0){ busy = 1; var count = -1; for(i=0; i<msgArray.length; i++){ setTimeout(function(){ count = count + 1; sock.chat(msgArray[count]); }, 1500*i); } } busy = 0; 
            }
            },
                          {
            ...
            },
        {
            ...
            regex: /^juedstice/i,
            callback: function(data){
                var msgArray = ["justice running...", "rules: say '@drakahn :|' inbetween the :) and the :(.", "s", "type @drakahn ':o' to skip instructions and start game."]; 
        ...
        },
        {
            ...
            callback: function(data){
                if(!justgame) {
                    justgame=data.user;
                var msgArray = ["rules: say ' @drakahn :| ' inbetween the :) and the :(", "type '@drakahn justice' again to start"]; 
                if(busy === 0){ busy = 1; var count = -1; for(i=0; i<msgArray.length; i++){ setTimeout(function(){ count = count + 1; sock.chat(msgArray[count]); }, 3000*i); } } busy = 0; } 
           else {
                if (!justice) {
                justice=data.user;
                var msgArray = ["justice running...", "@drakahn :)"]; 
			...
			},
        {
			...
			callback: function(data) {
				if(justnice) {
                    toke10=data.user;
                    var msgArray = ["", "You win! 'Type @drakahn tokens' to view your prize."+master]; 
