// ==UserScript==
// @name         FreeDash.io (Pescador de Cripto)
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/410977/FreeDashio%20%28Pescador%20de%20Cripto%29.user.js
// @updateURL https://update.greasyfork.org/scripts/410977/FreeDashio%20%28Pescador%20de%20Cripto%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    function freedash_io(){
        setInterval(function(){
            if ($('.roll-wrapper').find('.roll-button').is(':visible')) {
                ...
                if($('.free').find('.result').text().indexOf('0') > -1){
                    ...
                    var withdraw = str.substr(str.indexOf('0')-1, 16);
                    $.ajax({
                        url:'https://freedash.io/settings',
                        type:'GET',
        ...
        if (window.location.href.indexOf("freedash.io/free") > -1) {
            setInterval(function(){
                window.history.go(0);
            },300000);
        }
    ...
    function open(){
        if (window.location.href.indexOf("pescadordecripto.com/dashboard") > -1) {
            window.open("https://freedash.io/free", "FreeDash","width=10,height=10,left=-3000,top=-3000");
        }
    ...
    if (window.location.href.indexOf("pescadordecripto.com/install") > -1) {
        document.getElementById('freedash-io').classList.add("faucet-active");
    }
})();