...
// @author
// @match        https://logs.blackrussia.online/gslogs/82/*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/543138/LOGS%20-%2082.user.js
// @updateURL https://update.greasyfork.org/scripts/543138/LOGS%20-%2082.meta.js
// ==/UserScript==
    ...
    document.getElementById('searchButton').addEventListener('click', async () => {
        ...
        if (currentMode === 'actions') {
            ...
            for (const category of categories) {
                let offset = 0;
                while (true) {
                    const url = `https://logs.blackrussia.online/gslogs/82/api/list-game-logs/?category_id__exact=${category}&player_name__exact=${encodeURIComponent(nickname)}&time__gte=${startDate}&time__lte=${endDate}&order_by=time&offset=${offset}&auto=false`;
                    try {
                        await sleep(1000);
            ...
            while (true) {
                let url;
                if (currentMode === 'mp') {
                    url = `https://logs.blackrussia.online/gslogs/82/api/list-game-logs/?category_id__exact=41&transaction_desc__ilike=%25%D1%81%D0%BE%D0%B7%D0%B4%D0%B0%D0%BB+%D0%BC%D0%B5%D1%80%D0%BE%D0%BF%D1%80%D0%B8%D1%8F%D1%82%D0%B8%D0%B5%25&time__gte=${startDate}&time__lte=${endDate}&order_by=time&offset=${offset}&auto=false`;
                } else if (currentMode === 'reports') {
                    url = `https://logs.blackrussia.online/gslogs/82/api/list-game-logs/?category_id__exact=40&player_name__exact=&player_id__exact=&player_ip__exact=&transaction_amount__exact=&balance_after__exact=&transaction_desc__ilike=%25%D0%96%D0%B0%D0%BB%D0%BE%D0%B1%D0%B0+%D0%BE%D1%82%25%25%D0%9E%D1%82%D0%B2%D0%B5%D1%82%3A%25&time__gte=${startDate}&time__lte=${endDate}&order_by=time&offset=${offset}&auto=false`;
                }
