// ==UserScript==
// @name          Deepseek Awesome Code
// @namespace     http://tampermonkey.net/
// @version       V1.0
// @description   Transforming Deepseek Codeblock into a specialized artifact similar to Claude.
// @author        Reda Elsayed
...
// @downloadURL https://update.greasyfork.org/scripts/525195/Deepseek%20Awesome%20Code.user.js
// @updateURL https://update.greasyfork.org/scripts/525195/Deepseek%20Awesome%20Code.meta.js
// ==/UserScript==
...
(function () {
    ...
    const config = {
        panelHeightRatio: 0.8, debounceTimeout: 150, maxCodeSize: 100000, // 100KB
        mobileBreakpoint: 768, zIndex: 2147483647
    };
    ...
    const mutationQueue = [];
    const state = {
        isPanelOpen: false, lastFocusedElement: null, prismLoaded: false, isInitialized: false
    };
    ...
    function processPreElement(pre) {
        ...
        placeholder.textContent = 'View Code';
    ...
    function detectLanguage(pre) {
        ...
        const lang = languageElement?.textContent.trim().toLowerCase() || Array.from(pre.classList).find(c => c.startsWith('language-'))?.split('-')[1] || 'plaintext';
    ...
    function showNotification(message, isError = false) {
        if (typeof GM_notification === 'function') {
            GM_notification({
                title: isError ? 'Error' : 'Success', text: message, timeout: 2000
            });
        } else {
    ...
    if (!window[SCRIPT_ID]) {
        ...
        window.addEventListener('unload', () => cleanup(true));
