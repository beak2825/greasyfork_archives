// ==UserScript==
// @name         WME Larimer GIS Map
// @namespace    https://greasyfork.org/users/45389
// @version      0.3.2
// @description  Open a Larimer County GIS map in another window, at the same location as the WME map.  Keeps the location of the GIS map synced to WME.
// @author       jcloudm for localization, MapOMatic original
// @include     /^https:\/\/(www|beta)\.waze\.com\/(?!user\/)(.{2,6}\/)?editor.*$/
// @include     /^http:\/\/www\.arcgis\.com\/home\/webmap\/viewer\.html\?*/
// @license      GNU GPLv3
// @downloadURL https://update.greasyfork.org/scripts/28037/WME%20Larimer%20GIS%20Map.user.js
// @updateURL https://update.greasyfork.org/scripts/28037/WME%20Larimer%20GIS%20Map.meta.js
// ==/UserScript==
...
(function() {
    ...
    function log(message, level) {
        if (message && level <= debugLevel) {
            console.log('LC GIS:', message);
        }
    }

    function onButtonClick() {
        var wazeExt = W.map.getExtent();
        var url = 'http://www.arcgis.com/home/webmap/viewer.html?urls=http%3A%2F%2Fmaps1.larimer.org%2Farcgis%2Frest%2Fservices%2FBasemaps%2Fbasemap%2FMapServer%2Chttp%3A%2F%2Fmaps1.larimer.org%2Farcgis%2Frest%2Fservices%2FMapServices%2FParcels%2FMapServer&source=sd&extent=';
        url += wazeExt.left + '%2C' + wazeExt.bottom + '%2C' + wazeExt.right + '%2C' + wazeExt.top + '%2C102113';
        if (!mapWindow || mapWindow.closed) {
            mapWindow = window.open(null, 'lc_gis_map');
            try {
    ...
    function init() {
        $('.WazeControlPermalink').prepend(
            $('<div>').css({float:'left',display:'inline-block', padding:'0px 5px 0px 3px'}).append(
                $('<a>',{id:'lc-gis-button',title:'Open the LC GIS map in a new window', href:'javascript:void(0)'})
                .text('LCGIS')
