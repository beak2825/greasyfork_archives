...
// @author      fyxie
// @downloadURL https://update.greasyfork.org/scripts/4004/EH%20Plus.user.js
// @updateURL https://update.greasyfork.org/scripts/4004/EH%20Plus.meta.js
// ==/UserScript==
...
$('div.ido').prepend('<div class="fxt"><div class="minitab" id="toggleseen">Seen - <span id="seencount">0</span></div>'+
'</div>');
...
'<label><input id="stars"   type="checkbox" '+($cfg_stars?'checked':'')  +'>Show Stars, Etc</label><br>'+
...
'<label><input id="fadevisited"   type="checkbox" '+($cfg_fadevisited?'checked':'')  +'>Fade visited galleries</label><br>'+
'<label><input id="hidevisited"   type="checkbox" '+($cfg_hidevisited?'checked':'')  +'>Hide visited galleries</label><br>'+
...
$('div.fxs span#migratebtn').click(function (e) {
	try {
		if (confirm("Mirgate old database? (this may take a while, will confirm on complete)")) {
			console.log("Migrating old db...");
			var i = migrateOldVisitedDB();
			if (confirm("Migration completed. "+i+" galleries marked as visited.\r\nDelete old visisted DB? (No longer needed in newest version, but shouldn't hurt to keep it.)"))	{
				console.log("Deleting old db...");
				deleteOldVisitedDB();
			}
		}
	}
	catch(e) {alert("error in migration: "+e);}
});
...
'div.id2 a {display: block; vertical-align:top;  padding: 5px 7px; height:100%;} div.id2{display:block}'+ 
...
function saveGalState() {
	...
	try{ $body_html = document.getElementsByClassName('ido')[0].innerHTML;	}
	catch(e){} //class not present on non-gal pages, needed when called out of index case from i(); at very bottom can probably refactor and remove try catch
} saveGalState();
...
function getAppendTag($tagbox,$class,$text) {
	...
	if ($flagLookupC[$text]) {
		return '<div class="floatflag stroke2" style="color:'+$flagLookupC[$text]+'">'+$stext+'</div>&nbsp;';
	}
	else return '<div class="floatflag stroke2 '+$class+'">'+$stext+'</div>&nbsp;';
}
...
function fixuptags() {
	...
	$(this).find(".id44").each(function(index) {
		...
		var gal_idx = gal_href.match(/^http:\/\/(?:g\.e-|e[^-])hentai\.org\/g\/(\d*)\/.*$/);
		if (!isNaN(gal_idx[1])) {
			...
			if (isGalVisited(gal_id)) {
				$seencount++;
				$tagContainer.closest('.id1').find('div.id2 a').first().attr('href');
				if ($cfg_hidevisited) {
					$hidden = true;
...
function sort_compact() {
	...
	if ($cfg_group)  { 
		...
		for(var $i=0; $i<$list_hg.length; $i++) {
			...
			$list_hg[$i] = $list_hg[$i].replace(/[^a-zA-Z0-9]/g,"").toLowerCase();
		}

		//0 prefix single digit numbers
		for(var $i=0; $i<$list_hg.length; $i++) {
			$list_hg[$i] = $list_hg[$i].replace(/([^\d])(\d)([^\d])/g,"$10$2$3") //mid string
			$list_hg[$i] = $list_hg[$i].replace(/([^\d])(\d)([\d]{9})$/,"$10$2$3") //end of string
		}
...
function applychanges() {
	...
	if ($cfg_hidevisited) { $('#toggleseen').parent().addClass('thidden'); }
...
$(  '<style type="text/css">'+
	...
	' p#nb { position:relative; height:0; top: 56px; right:'+ $linkx +'px; display:inline; color:#b1b1b1 !important; } ' + 
	' p#nb a { color:#b1b1b1; } ' + 
	' p#nb img { display:none; } ' + 
	...
	' p.ip { display:none;}'+
	//' p.ip { position:relative; top:-20px; padding:0; margin:-20px; color: #b8b8b8;}'+
	'</style>').appendTo("head");
...
if (document.URL.match(/^http:\/\/g.e-hentai\.org\/[gs]\/.*$/)) { //gallery or image
	if ($("div.d p:first-child").text() === "This gallery has been removed, and is unavailable.") {
...
if (document.URL.match(/^http:\/\/(g\.e-|e[^-])hentai\.org\/g\/.*$/)) { //gallery case
	...
	var gal_idx = document.URL.match(/^http:\/\/(?:g\.e-|e[^-])hentai\.org\/g\/(\d*)\/.*$/);
	if (!isNaN(gal_idx[1])) {
		var gal_id = parseInt(gal_idx[1],10);
...
else if (document.URL.match(/^http:\/\/(g\.e-|e[^-])hentai\.org\/(?!s).*$/) && self == top) { //search / index case, exclude iframe 
	i();
}
else {
	showPage();
