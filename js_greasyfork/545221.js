// ==UserScript==
// @name         CHZZK(치지직) 타임 스탬프 (예비용)
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  CHZZK(치지직) 라이브 및 다시보기 영상에서 단축키 'Y'를 눌러 메모를 작성하고 저장할 수 있습니다.
// @author       멍멍이
...
// @run-at       document-end
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/545221/CHZZK%28%EC%B9%98%EC%A7%80%EC%A7%81%29%20%ED%83%80%EC%9E%84%20%EC%8A%A4%ED%83%AC%ED%94%84%20%28%EC%98%88%EB%B9%84%EC%9A%A9%29.user.js
// @updateURL https://update.greasyfork.org/scripts/545221/CHZZK%28%EC%B9%98%EC%A7%80%EC%A7%81%29%20%ED%83%80%EC%9E%84%20%EC%8A%A4%ED%83%AC%ED%94%84%20%28%EC%98%88%EB%B9%84%EC%9A%A9%29.meta.js
// ==/UserScript==
...
(function () {
    ...
    GM_registerMenuCommand('📁 타임스탬프 파일로 저장', () => {
        const log = GM_getValue('timestampLog', []);
        if (!log || log.length === 0) return showToastMessage("저장된 내용이 없습니다.");
    ...
    function showMemoInputPopup(time, onSave) {
        ...
        popup.innerHTML = `
            <div style="font-weight: bold; margin-bottom: 6px; font-size: 14px; color: black;">메모 입력 (${time})</div>
            <input type="text" id="memoInput" style="width: 100%; padding: 6px; font-size: 14px; color: black;" value="여기에 문구를 입력하세요" />
        ...
        function saveAndClose() {
            ...
            if (memo === "여기에 문구를 입력하세요" || memo === "") memo = "";
            onSave(memo);
            popup.remove();
    ...
    function findTimeElement() {
        for (const sel of TIME_SELECTORS) {
            ...
            if (el && el.innerText) {
                const match = el.innerText.match(/\b\d{1,2}:\d{2}(?::\d{2})?\b/);
        ...
        for (const n of candidates) {
            ...
            const match = n.innerText.trim().match(/\b\d{1,2}:\d{2}(?::\d{2})?\b/);
    ...
    function handleChzzkTimestamp() {
        ...
        if (!found) return showToastMessage('시간 정보를 찾을 수 없습니다.');
