...
// @include     https://www.recaptcha.net/recaptcha/api2/bframe?*&k=6LeRrhIUAAAAAC0_WQguxdrrcHztb8RLS03lFgE2
// @namespace   thisisausername190
// @license     NA
// @downloadURL https://update.greasyfork.org/scripts/439545/PCI%2B.user.js
// @updateURL https://update.greasyfork.org/scripts/439545/PCI%2B.meta.js
// ==/UserScript==
window.pciPlus_version = "1.17.0";
...
(() => {
    ...
    (() => {
        ...
        function clearCoverage(unselectTower, forceFullRemove) {
            ...
            if (window.lookupCircleSource && forceFullRemove) {
                for (const feature of window.lookupCircleSource.getFeatures()) {
                    window.lookupCircleSource.removeFeature(feature);
                }
            }
        }
        ...
        function genRandomColor(seed) {
            ...
            if (seed) {
                ...
                let colorAsHex = hslToHex(parseInt(MD5(String(.2515656165189135 * seed)), 16) % 360, window.darkMode ? default_saturation_dark : default_saturation, window.darkMode ? default_lightness_dark : default_lightness);
                return colorAsHex;
            }
            let randomHue = Math.floor(Math.random() * 360 * 16777215);
        ...
        function generateIconStyle(towerType, verified, towerLabel, towerRegion) {
            const {ol} = window;
            const verifiedText = verified ? "verified" : "unverified";
            const iconStyle = window.showIcons ? "badge" : "dot";
            let tacIcon;
            if (window.pciPlus_colorizeByTac) {
                tacIcon = new ol.style.Icon({
                    anchor: [ .5, .5 ],
                    color: genRandomColor(towerRegion),
                    src: "images/dot.png"
                });
            }
            if (window.showTowerLabels == true) {
                ...
                return new ol.style.Style({
                    ...
                    image: window.pciPlus_colorizeByTac ? tacIcon : window.iconCache[iconStyle][verifiedText][towerType]
                });
            } else {
                return window.pciPlus_colorizeByTac ? new ol.style.Style({
                    image: tacIcon
                }) : window.iconStyleCache[iconStyle][verifiedText][towerType];
            }
        }
        function getCmgmCheckmark(siteID) {
            if (window.cmgm_verifiedSiteList == undefined || window.pciPlus_showCmgmCheckmark == "off" || window.pciPlus_enableCmgmIntegration == false) {
                return "";
            } else {
                return window.cmgm_verifiedSiteList[`${window.MCC}${window.MNC}`].includes(siteID) ? "​✓" : "";
            }
        }
        function generateTowerLabel(siteID, regionID, towerType, towerBandArray, estimatedBandData, towerAttributes) {
            ...
            }) + (window.pciPlus_showCmgmCheckmark == "afterId" ? getCmgmCheckmark(siteID) : "");
            ...
            if (towerBands.length > 1) {
                towerText += "\nBands " + towerBands + (window.pciPlus_showCmgmCheckmark == "afterBands" ? getCmgmCheckmark(siteID) : "");
        ...
        function styleFeature(feature, bypassCache) {
            if (window.savedFeatures && window.savedFeatures.get(feature) !== undefined && !bypassCache) {
            ...
            let iconStyle = generateIconStyle(towerType, verified, towerLabel, parseInt(tower.regionID));
            if (window.showIcons) {
        ...
        function refreshTowerStylesOnly() {
            for (let tower in window.Towers) {
                window.Towers[tower].setStyle(styleFeature(window.Towers[tower], true));
            }
        }
        ...
        const cmgmKnown = (cmgmConcealed, cmgmSiteType, cmgmEvLinks, cmgmPhotoLinks, cmgmSvLinks, cmgmTags) => src_elements.a("span", null, src_elements.a("tr", null, src_elements.a("td", null, "Build"), src_elements.a("td", null, cmgmConcealed, " ", cmgmSiteType)), src_elements.a("tr", null, src_elements.a("td", null, "Evidence"), src_elements.a("td", null, cmgmEvLinks)), src_elements.a("tr", null, src_elements.a("td", null, "Photos"), src_elements.a("td", null, cmgmPhotoLinks)), src_elements.a("tr", null, src_elements.a("td", null, "Street View", src_elements.a("a", {
            style: "font-size: 10px; vertical-align: super; display: none;",
            id: "bingMapsLink",
            target: "_blank"
        }, "BM")), src_elements.a("td", null, cmgmSvLinks)), src_elements.a("tr", null, src_elements.a("td", null, "Tags"), src_elements.a("td", null, cmgmTags)));
        ...
        async function getInitialVerifiedSites() {
            if (!window.pciPlus_enableCmgmIntegration) {
                devLogger("CMGM is disabled, not checking verified API");
                return;
            }
            const res = await fetch("https://cmgm.us/api/pciplus/getAllTowers.php?plmn=310260");
            const json = await res.json();
            const data = parseCmgmSitesResponse(json);
            window.cmgm_verifiedSiteList = data;
            refreshTowerStylesOnly();
        }
        async function getNewVerifiedSites() {
            const currentCarrier = `${window.MCC}${window.MNC}`;
            const currentCmgmCarrier = Object.keys(cmgmCarriers).find((key => cmgmCarriers[key] === currentCarrier));
            const [lat, lng] = getMapCenter();
            const res = await fetch(`https://cmgm.us/api/cmgm/getTowersV3.php?latitude=${lat}&longitude=${lng}&carrier=${currentCmgmCarrier}&properties=LTE_1,LTE_2,LTE_3,LTE_4,LTE_5,LTE_6,LTE_7,LTE_8,LTE_9,NR_1,NR_2,NR_3,NR_4,carrier&max_distance=10`);
            const json = await res.json();
            const data = parseCmgmSitesResponse(json);
            window.cmgm_verifiedSiteList = data;
            refreshTowerStylesOnly();
        }
        function parseCmgmSitesResponse(cmgmResposne) {
            let data = window.cmgm_verifiedSiteList || {};
            for (const carrier of Object.keys(cmgmResposne)) {
                if (data[cmgmCarriers[carrier]] == undefined) {
                    data[cmgmCarriers[carrier]] = [];
                }
                for (const cmgmSite of cmgmResposne[carrier]) {
                    for (const enbid of Object.values(cmgmSite)) {
                        if (!data[cmgmCarriers[carrier]][enbid]) {
                            data[cmgmCarriers[carrier]].push(enbid);
                        }
                    }
                }
            }
        ...
        async function refreshMap(hideToast, noDelay) {
            if (!hideToast) {
            ...
            if (window.pciPlus_enableCmgmIntegration && window.pciPlus_showCmgmCheckmark != "off") {
                getNewVerifiedSites();
            }
        }
        ...
        function enableDarkMode() {
            ...
            mapSource.on("prerender", (evt => {
                if (!window.darkMode || window.mapType.includes("aerial") || window.mapType.includes("satellite")) {
                    return;
                }
                if (evt.context) {
            ...
            $(darkModeStylesheet).insertBefore(".navbar.fixed-top");
        ...
        function disableDarkMode() {
            ...
            $(lightStylesheet).insertBefore(".navbar.fixed-top");
            window.updateLinkback();
            refreshTowerStylesOnly();
        }
        function reloadPage() {
        ...
        function showTiles(inMCC, inMNC) {
            ...
            var tileLayer = new ol.layer.Tile({
                visible: true,
                name: "SignalTrails",
                preload: window.pciPlus_dontPreloadTiles ? 0 : 2,
                transition: 0,
        ...
        function genColorStyle(signal, showAllStrengths, showText, carrier) {
            const {ol} = window;
            let color;
            if (signal >= -62) {
                color = [ 4, 48, 3 ];
            ...
            return new ol.style.Style({
                ...
                fill: new ol.style.Fill({
                    color: `rgba(${color[0]}, ${color[1]}, ${color[2]}, ${window.pciPlus_showFccPolygons == "minimal" ? "0.50" : "0.40"})`
                }),
                text
            });
        }
        function createFCCTileLayer(carrier, rat, showAllStrengths, showText) {
            ...
            const vectorTileLayer = new ol.layer.VectorTile({
                source: new ol.source.VectorTile({
                    ...
                    url: `https://pp-p.tallyfre.workers.dev/?https://map.coveragemap.com/api/v1/fcc/tiles?version=5&z={z}&x={x}&y={y}&technology=${rat == "NR" ? "5g" : "lte"}&provider=${carrier}`,
                    maxZoom: 11
                }),
                style: a => {
                    const signalStrength = a.get("signal") | -100;
                    return genColorStyle(signalStrength, showAllStrengths, showText, carrier);
                }
            });
            return vectorTileLayer;
        }
        function toggleFccLayer(state, inMCC, inMNC, inRAT) {
            const setting = window.pciPlus_showFccPolygons;
            let {map, fccPolygonLayer} = window;
            let MCC = inMCC || window.MCC;
            let MNC = inMNC || window.MNC;
            let netType = inRAT || window.netType;
            if (state == false) {
        ...
        async function getUserHistory(userId, currentOffset, continuingSession, oldOffset) {
            ...
            footerElements["Previous"] = src_elements.a("div", {
                ...
                class: "dropdown-item btn-prev",
                type: "button"
            }, "x5000 (250)"), totalLocatedTowers > 1e4 ? src_elements.a("button", {
                "data-mult": "10000",
                id: "next-1000",
                class: "dropdown-item btn-next",
                type: "button"
            }, "x10000 (500)") : "", totalLocatedTowers > 2e4 ? src_elements.a("button", {
                "data-mult": "10000",
                id: "next-1000",
                class: "dropdown-item btn-next",
                type: "button"
        ...
        const cmgm_login_store_api = "https://cmgm.us/settings/pciplus-login/?pciplusIdent=";
        const cmgm_login_get_token_api = "https://cmgm.us/api/pciplus/login-auth.php?pciplusIdent=";
        ...
        const cmgm_stats_api_url = "https://cmgm.us/api/pciplus/fun.php";
        ...
        const cmgm_view_history_name = "View CMGM History⁺";
        const cmgm_view_history_icon = "📜";
        const cmgm_view_history_description = "View CMGM edit history for the current site";
        async function finishLogin(randomId, enableSetting) {
            const res = await fetch(cmgm_login_get_token_api + randomId);
            const toast = new PPToast("cmgmSigninToast", 5e3);
            toast.setHeader("CMGM");
            if (res.ok) {
                const userid = (await res.json())["msg"];
                devLogger("Recieved User ID from CMGM:", userid);
                $.cookie("pciPlus_cmgmUserId", userid, {
                    expires: window.isMobileDevice ? 3600 * 6.99 : 9999999
                });
                window.pciPlus_cmgmUserId = userid;
                toast.setContents("Login successful!");
            } else {
                toast.setContents("Login was unsuccessful! Please try again later.");
                toggleSetting("pciPlus_enableCmgmIntegration", false);
            }
            toast.showToast();
            if (enableSetting) {
                toggleSetting("pciPlus_enableCmgmIntegration", true);
            }
        }
        function showLoginPrompt(enableSetting) {
            const randomId = self.crypto.randomUUID().replaceAll("-", "");
            const message = src_elements.a("span", null, "Welcome to CMGM! In order to use CMGM with PCI+, an account is required.", src_elements.a("br", null), "Please click the link below to sign in.", src_elements.a("br", null), src_elements.a("br", null), src_elements.a("b", null, src_elements.a("a", {
                style: "font-size: x-large;",
                target: "_blank",
                href: cmgm_login_store_api + randomId
            }, "Login here!")), src_elements.a("br", null), src_elements.a("br", null), "Once you're returned here, click the button below to finish logging in.", src_elements.a("br", null), src_elements.a("br", null), src_elements.a("button", {
                class: "btn btn-info",
                id: "cmgmCompleteLoginBtn"
            }, "Complete Login"));
            const loginPrompt = bootbox.alert({
                message,
                size: "large",
                closeButton: false,
                onEscape: () => {
            ...
            loginPrompt.on("shown.bs.modal", (() => {
                $("#cmgmCompleteLoginBtn").on("click", (() => {
                    window.bootbox.hideAll();
                    finishLogin(randomId, true);
                }));
            }));
        }
        function loginAndEnableCmgm(requireLogin) {
            if (window.pciPlus_cmgmUserId) {
                return true;
            }
            const cookieVal = $.cookie("pciPlus_cmgmUserId");
            if (cookieVal == undefined || cookieVal == "") {
                ...
                window.pciPlus_cmgmUserId = cookieVal;
                return true;
            }
        }
        function patchModifyMapRotation(state) {
            $(".ol-rotate-reset").css("margin-top", "10px");
            $(".ol-rotate-reset").insertBefore("#measureButton");
            if (state != undefined) {
                if (state == true) {
                    disableRotate();
                } else {
            ...
            if (window.pciPlus_disableMapRotation == true) {
                disableRotate();
            } else {}
        }
        function disableRotate() {
            const {map, ol} = window;
            if (window.pciPlus_disableMapRotation_interactionList == undefined) {
                window.pciPlus_disableMapRotation_interactionList = [];
            }
            map.getView().setRotation(0);
            $(".ol-rotate-reset").css("display", "none");
            map.getInteractions().getArray().forEach((function(interaction) {
                if (interaction instanceof ol.interaction.DragRotate || interaction instanceof ol.interaction.KeyboardPan || interaction instanceof ol.interaction.PinchRotate) {
                    window.pciPlus_disableMapRotation_interactionList.push(interaction);
        ...
        function enableRotate() {
            const {map} = window;
            for (const interaction of window.pciPlus_disableMapRotation_interactionList) {
                map.addInteraction(interaction);
            }
            $(".ol-rotate-reset").css("display", "");
        }
        let changelog = [ {
            version: "1.17.0",
            releaseDate: "xxxx-xx-xx",
            changes: {
                new: [ "<b>PCI+ Nightly</b>: Recieve 'nightly' updates from PCI+, in between full releases", "Add Google Geolocation API support to 'Request Tower Lookup'", "Enable changing NR tower type (@RaccoonCast)", "Show 'Wiki Contributors' below Tower Contributors (@RaccoonCast)", "CMGM: Show Bing Maps link in table", 'CMGM: Add "CMGM History" action button', "Move 'Location History' and 'Metadata History' to actions table", "Add 'Force Update' toggle to Last Generated date (@RaccoonCast)", "CMGM: Add framework to show stats within CellMapper", "CMGM: Require login to access CMGM APIs within PCI+", "CMGM: Add action button to show edit history", "Add option to disable new CellMapper rotation interaction", "Add total count of all CAPTCHA challenges completed (@RaccoonCast)", "Show estimated precision circles for 'Request Tower Lookup'", "Add Shift-A shortcut to toggle Aerial view (@mystica555)", "Add larger user history multipliers for users with over 10k pins (@RaccoonCast)", "Add current PCI+ version to Browser Extension menu (@lukeiamyourda)", "Add Shift-D shortcut to toggle FCC Polygons (@RaccoonCast)", "Check for unpinned FirstNet eNBs when pinning AT&T (@mystica555)", "Display county in address for records with CMGM", "Add option to show checkmark on verified CMGM sites (@codyingaround, @RaccoonCast)" ],
                improvement: [ "Hide CMGM table when site does not have CMGM record", "CMGM: Execute tower move without asking for permission", "Improve dark theme support for text inputs", "Show error when ppT/ppL lookup fails (@RaccoonCast)", "Reimplement colorization by TAC when enabled, after removal in a prior version of PCI+", "Reduce visual interruptions and improve speed during CellMapper load (@RaccoonCast)", "Correct position of color randomizer after initial unlock (@RaccoonCast)", "FCC Polygons may not reflect correct provider/RAT when values were updated from sidebar (@RaccoonCast)", "Improve loading of tiles at zoom level 15, with optional setting to disable tile preloading", "Update LARIAC imagery from v6 to v7", "Improve adding CMGM splits on mobile (@RaccoonCast)", "Switch primary host from GreasyFork to Cloudflare Pages to improve reliability", "Fix issue which could cause duplicate 'Add Data' buttons", "Persist CMGM TAC prompt for 30 seconds", "Implement faster client-side refresh where possible", "Remove 'Report A Problem' shortcut", "Prevent color inversion of aerial/satellite maps in Dark Mode (@RaccoonCast)", "Show CMGM data and add TAC/PCIs for added splits immediately, without requiring that they be re-clicked", "Improve support for CellMapper iOS app" ],
                patch: [ "<b>Replace buggy sector highlighting with bug-free native solution</b>", "Reduce wrapped text in settings for sidebar users (c/o @RaccoonCast)", "Move CellMapper rotation selector into same container as zoom icons", "Update URL on map move, even if towers are disabled", "Prevent CellMapper from clearing map after CAPTCHA is solved (@RaccoonCast)", "Prevent CellMapper issue which could cause multiple carriers / RATs to appear on the map simultaneously" ],
                bug: [ "PCI+ may not recognize redirect Wiki redirect links containing spaces", "PCI Search may not be linked correctly in details table", "Sector Area may not appear if a site does not have Direction data", "Some site types may not appear immediately after change", "'Copy Tower Link' could use incorrect MCC/MNC (@RaccoonCast)", "Change Maprad links to match new standard (c/o @RaccoonCast)", "PCI+ may pass null PCI to CMGM (@RaccoonCast)", "UL frequency may not appear in details table", "Update FCC polygons to render more effectively at all resolutions", "PCIs may be unlinked or incorrectly cropped in tower details", "Fix issue which could cause 'Hide Country In Select' to malfunction", "Fix issue which could cause Shift-B shortcut to fail, requiring refresh", "Signal Trails persistence may behave incorrectly if set by a URL", "Google Maps: 'Measure Distance' broken after site update" ]
            }
        }, {
            version: "1.16.1",
            releaseDate: "2023-12-04",
            changes: {
                new: [ "Click 'x' button on PCI Search toast to cancel lookup (@RaccoonCast)", "Show/hide regional imagery basemaps based on map location (@RaccoonCast)" ],
                improvement: [ "Add title attribute to Custom Band Selector (@RaccoonCast)", "Increase transparency of CM polygons in 'Detailed' mode (@RaccoonCast)", "Add class identifier for Select Providers dialog", "Add class name for 'Notes' table cell" ],
                patch: [ "Fix <code>navigator.geolocation</code> support on cellmapper.net", "Don't clear coverage polygons when toggling Tower Labels (@mystica555)" ],
                bug: [ "CMGM split could be added multiple times in the event of high latency (@RaccoonCast)", "Band Selector Filters Sectors may not work correctly with T-Mobile US n2/n25 (@RaccoonCast)", "CM / Bootstrap styles could take precedence over PCI+ styles (@codyingaround, @mystica555)", "'Show More' contributor toggle could fail to expand non-premium contributors" ]
            }
        }, {
            version: "1.16.0",
        ...
        function popupChangelog(updated, versionInView) {
            ...
            let versionName = versionInView == 0 ? window.pciPlus_version : changelog[versionInView].version;
            ...
            window.bootbox.dialog({
                ...
                callback: () => {},
                title: `<center>${updated ? "PCI+ Updated!" : "PCI+ " + versionName + ' <span style="font-size: 0.7em;">(' + changelog[versionInView].releaseDate + ")</span>"} </center>`,
        ...
        async function handleUpdates() {
            let newVersionUrl;
            let versionCheckApiUrl;
            if (window.pciPlus_enableNightlyUpdates) {
                newVersionUrl = "https://pci-plus-nightly.pages.dev/script.user.js";
                versionCheckApiUrl = "https://pci-plus-nightly.pages.dev/version.json";
            } else {
                newVersionUrl = "https://pci-plus.pages.dev/script.user.js";
                versionCheckApiUrl = "https://pci-plus.pages.dev/version.json";
            }
            ...
            let latestVersion;
            try {
                installedVersion = window.pciPlus_version;
                secondInstalledVersion = changelog[1].version;
                let updateApiResponse = await fetch(versionCheckApiUrl);
                let pciPlusApiResults = await updateApiResponse.json();
            ...
            if (installedVersion.endsWith("~dev")) {
                devLogger("You're using a development build; skipping update prompt");
            } else if ($.cookie("pciPlus_skipUpdateCheck") === "true") {
                devLogger("Cookie present, skipping update check");
            } else if (window.ppBrowserExtension === true || window.CMApp != undefined) {
                devLogger("Using browser extension or mobile app, so skipping update check");
            } else if (latestVersion != installedVersion) {
                let out = src_elements.a("div", {
                    style: "text-align: center; font-size: medium;"
                }, "Please ", latestVersion == secondInstalledVersion ? "downgrade" : "update", " to PCI+", " ", src_elements.a("b", null, `${latestVersion}.`), window.pciPlus_enableNightlyUpdates ? "<br/><br/> If you wish to disable Nightly updates, you can do so from PCI+ Settings." : "");
                bootbox.confirm({
                    centerVertical: true,
                    closeButton: false,
                    size: "extra-large",
                    ...
                    title: `<center>${latestVersion == secondInstalledVersion ? "Downgrade Pending" : "Update Available"}</center>`,
                    message: out
                });
                return;
            }
        ...
        let settings = [ {
            key: "pciPlus_enableNightlyUpdates",
            name: '<b class="warningText">PCI+ Nightly⁺</b>',
            description: "Enable 'nightly' updates from PCI+, in between regular releases. ",
            category: "extras",
            settingType: "boolean",
            defaultValue: false,
            selectOptions: undefined,
            callbackShouldShowSetting: () => {
                if (window.ppBrowserExtension || window.CMApp) {
                    $.cookie("pciPlus_enableNightlyUpdates", false);
                    return false;
                } else {
                    return true;
                }
            ...
            warning: {
                ...
                warningHtml: src_elements.a("span", null, src_elements.a("b", null, "PCI+ Nightly"), " builds are generated automatically, and do not go through the same testing as a regular PCI+ release. Bugs and unstable behavior are expected.", src_elements.a("br", null), " ", src_elements.a("br", null), "Please report all bugs you encounter in", " ", src_elements.a("a", {
                    href: "https://discord.com/channels/980652545938702376/980652545938702379"
                }, "#feedback"), ", along with your version number.")
            }
        }, {
            ...
            callbackOnSettingChanged: state => {
                if (state == true) {
                    loginAndEnableCmgm(true);
                    getInitialVerifiedSites();
                } else {
                    refreshTowerStylesOnly();
                }
            ...
            key: "pciPlus_disableMapRotation",
            name: "Disable Map Rotation⁺",
            description: "Disable map rotation, and hide the rotation control on the top right",
            category: "map",
            settingType: "boolean",
            defaultValue: false,
            selectOptions: undefined,
            callbackOnSettingChanged(state, _originalEvent) {
                patchModifyMapRotation(state);
            }
        }, {
            ...
            key: "pciPlus_showLabelsInCoverageEstimateCircles",
            name: "Show Label in Coverage Estimate Circle⁺",
            description: "Show a label with the site information inside of the Coverage estimation circle",
            category: "extras",
            settingType: "boolean",
            defaultValue: true,
            selectOptions: undefined
        }, {
            key: "pciPlus_promptFirstnetWhenPinningAtt",
            name: "Check for FirstNet while pinning AT&T⁺",
            description: "Check if an equivalent FirstNet (313-100) site is unpinned while pinning AT&T (310-410)",
            category: "extras",
            settingType: "boolean",
            defaultValue: true,
            selectOptions: undefined
            ...
            key: "pciPlus_dontPreloadTiles",
            name: "Load Tiles On Demand⁺",
            description: "This will make signal trails load more consistently, but tiles will not load at unsupported zoom levels (including z15).",
            category: "map",
            settingType: "boolean",
            selectOptions: undefined,
            defaultValue: false,
            callbackOnSettingChanged: () => {
                showTiles(window.MCC, window.MNC);
            }
        }, {
            ...
            callbackOnSettingChanged: state => {
                if (state === false) {
                    ...
                    let sectorColorsSetting = $("#pciPlus_useStylingMethod")[0].parentNode.parentNode;
                    sectorColorsSetting.parentNode.insertBefore(element, sectorColorsSetting);
                }
            }
        }, {
            key: "pciPlus_showCmgmCheckmark",
            name: "Show CMGM Verified⁺",
            description: "Show a check mark on sites which have been verified on CMGM",
            category: "map",
            selectOptions: [ {
                value: "afterId",
                name: "After Tower ID"
            ...
            callbackShouldShowSetting: () => {
                let shouldEnable = $.cookie("pciPlus_enableCmgmIntegration") == "true";
                return shouldEnable;
            }
        }, {
            ...
            key: "pciPlus_exportSettings",
            name: "Export PCI+ Settings⁺",
            description: "Export settings for use in another PCI+ instance",
            category: "extras",
            settingType: "PPToggleLink",
            defaultValue: false,
            selectOptions: undefined,
            toggleLinkName: "Export",
            dontUseCookie: true,
            callbackOnSettingChanged: () => {
                let settings = {};
                document.cookie.split(";").forEach((item => {
                    ...
                    const val = itemArr[1];
                    if (name == "pciPlus_exportSettings" || name == "pciPlus_importSettings" || name == "pciPlus_randomizeSectorColors" || name == "pciPlus_version" || name == "hash" || name == "selectedProvider" || name.startsWith("ph_")) {
                        return;
                    }
                    settings[name] = val;
                }));
                const settingsText = btoa(JSON.stringify(settings));
                copyToClipboard(settingsText);
                const toast = new PPToast("settingsCopiedText", 5e3);
                toast.setContents("Settings have been copied to clipboard.");
                toast.showToast();
            }
        }, {
            key: "pciPlus_importSettings",
            name: "Import PCI+ settings⁺",
            description: "Import PCI+ settings from another instance",
            category: "extras",
            settingType: "PPToggleLink",
            defaultValue: false,
            selectOptions: undefined,
            toggleLinkName: "Import",
            dontUseCookie: true,
            callbackOnSettingChanged: () => {
                var dialog = bootbox.prompt({
                    title: "Paste your settings code below.",
                    closeButton: false,
                    size: "extra-large",
                    inputType: "textarea",
                ...
                function dialogCallback(text) {
                    const failToast = new PPToast("importFailedToast", 5e3);
                    let settingsContent = {};
                    try {
                        settingsContent = JSON.parse(atob(text));
                    } catch (err) {
                        devLogger("Failed to parse clipboard contents as JSON");
                        failToast.setContents("PCI+ couldn't understand your clipboard! Make sure that you've correctly copied the settings text.");
                        failToast.showToast();
                        return;
                    }
                    window.dontReload = true;
                    for (let i of Object.keys(settingsContent)) {
                        let settingValue = settingsContent[i];
                        if (i == "pciPlus_cmgmUserId") {
                            $.cookie("pciPlus_cmgmUserId", settingValue, {
                                expires: 9999999
                            });
                            window.pciPlus_cmgmUserId = settingValue;
                            continue;
                        }
                        ...
                        if (i.startsWith("pciPlus")) {
                            toggleSetting(i, settingValue);
                        } else {
                            window[i] = settingValue;
                            $(`${i}`).prop("checked", settingValue);
                            $.cookie(i, settingValue, {
                                expires: 3600
                            });
                        }
                    }
            ...
            settingType: "PPMultiSetting",
            defaultValue: "off",
            selectOptions: fccPolygonRenderOptions,
            callbackOnSettingChanged: state => {
                if (state === "off") {
        ...
        async function pp_getTowersInView(inMCC, inMNC, clear, inNetType) {
            ...
            const towersRes = fetch(API_URL + "getTowers?" + new URLSearchParams(getTowersParams), {
                ...
                if (window.MCC != inMCC || window.MNC != inMNC || window.netType != inNetType) {
                    devLogger("PLMN or RAT changed, don't render");
                    return;
                }
                if (clear) {
                    if (window.skipNextClear) {
        ...
        async function showProvidersDialog(latitude, longitude, altLocation) {
            ...
            const alert = bootbox.alert(providersDiv, (() => {
                getNetworkInfo(MCC, MNC);
                return true;
            }));
            alert.on("shown.bs.modal", (() => {
                $(".modal").addClass("selectProvidersModal");
            }));
        }
        ...
        function changeMapType(newType) {
            ...
            switch (newType) {
              ...
              case "losangelescounty_aerial":
                baseLayer = new ol.source.XYZ({
                    attributions: [ baseAttrib, 'Tiles courtesy of <a href="https://lariac-lacounty.hub.arcgis.com/">Los Angeles Region Imagery Acquisition Consortium</a>' ],
                    url: "https://svc.pictometry.com/Image/BCC27E3E-766E-CE0B-7D11-AA4760AC43ED/wmts?SERVICE=WMTS&VERSION=1.0.0&REQUEST=GetTile&LAYER=PICT-LARIAC7--KCrSFBeqgG&STYLE=default&FORMAT=image/png&TILEMATRIXSET=GoogleMapsCompatible&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}",
                    maxZoom: 21
                });
                break;
        ...
        function kb_switchBasemap() {
            const currentGroup = document.querySelector("#map_select_layer > optgroup > option:checked").parentElement.label;
            devLogger("current group:", currentGroup);
            let toSwitchTo = "";
            if (currentGroup == "Maps") {
                let aerialGroup = document.querySelector('optgroup[label="Aerial"]').children;
                for (let option of aerialGroup) {
                    if (option?.id?.endsWith("_aerial") && [ ...option.classList ].length == 0) {
                        devLogger("Switching to:", option);
                        toSwitchTo = option.value;
                        break;
                    }
                }
                if (toSwitchTo.length == 0) {
                    devLogger("No local satellite available, switching to esri");
                    toSwitchTo = "esri_satellite";
                }
            } else if (currentGroup == "Aerial") {
                toSwitchTo = "osm_street";
            } else {
                return;
            }
            changeMapType(toSwitchTo);
            document.getElementById("map_select_layer").value = toSwitchTo;
            const optionFullName = document.querySelector(`option[value='${toSwitchTo}']`).innerText;
            const toast = new PPToast("switchedBasemapKeyboardShortcutToast", 3e3);
            toast.setContents(`Changed basemap to <b>${optionFullName}</b>.`);
            toast.showToast();
        }
        function kb_toggleFccTiles() {
            const options = [ "off", "minimal", "detailed" ];
            const currentSetting = window.pciPlus_showFccPolygons;
            const index = options.indexOf(currentSetting);
            let newIndex = 0;
            if (index == -1 || index >= 2) {
                newIndex = 0;
            } else {
            ...
            toggleSetting("pciPlus_showFccPolygons", options[newIndex]);
            const newValue = options[newIndex];
            const newProperName = fccPolygonRenderOptions.find((i => i.value == newValue)).name;
            const toast = new PPToast("switchFccTilesKeyboardShortcutToast", 3e3);
            toast.setContents("Show FCC Polygons: <b>" + newProperName + "</b>");
            toast.showToast();
        }
        const keyboardShortcuts = [ {
            ...
            name: "Toggle Aerial View",
            triggers: [ "a", "A" ],
            requiresCtrl: false,
            requiresShift: true,
            showInList: true,
            callback: () => kb_switchBasemap()
        }, {
            name: "Toggle Trails",
            triggers: [ "r", "R" ],
            requiresCtrl: false,
            ...
            name: "Toggle FCC Polygons",
            triggers: [ "d", "D" ],
            requiresCtrl: false,
            requiresShift: true,
            showInList: true,
            callback: () => kb_toggleFccTiles()
        }, {
            name: "Scroll Sidebar",
        ...
        function createToggleLinkSetting(name, id, explanation, toggleLinkName) {
            const node = textToHtml(src_elements.a("tr", {
                style: "border-top: 1px solid #dee2e6"
            }, src_elements.a("td", {
                style: "border: 0; vertical-align: middle;",
                ...
                href: "#",
                id
            }, toggleLinkName ? `(${toggleLinkName})` : "(Toggle)"))));
        ...
        function hookSettingsMenu() {
            ...
            for (let i of settings) {
                ...
                if (i.settingType == "boolean") {
                    ...
                    element = createToggleLinkSetting(i.name, i.key, i.description, i.toggleLinkName || "");
                } else {
 ...
 */
        ...
        function patchIosAppOnly() {
            console.log("[PCI+] Adding patches for iOS App");
            $(".navbar").css("display", "none");
            $("#location.marker").css("display", "none");
            const docNodeOne = document.body.childNodes[0];
            const docNodeTwo = document.body.childNodes[1];
            if (docNodeOne.nodeName == "#text") {
                $(docNodeOne).remove();
            } else if (docNodeTwo.nodeName == "#text") {
                $(docNodeTwo).remove();
            }
            ...
            iosStylesheet.textContent = `\n  \n    #pciPlusActionsTableSection {\n      margin-bottom: 0px !important;\n    }\n      \n    #cmgmTable {\n      margin-bottom: 0px !important;\n    }\n  \n  `;
            $(iosStylesheet).insertBefore("body");
            $("#modal_tower_details").addClass("hidden");
            $("#modal_tower_details").css({
                left: "0px !important",
                right: "0px !important"
            });
            ...
            window.toggleModal = () => {};
            window.toggleTowerInfo = bool => {
                if (bool == undefined) bool = true;
                devLogger("toggling:", bool);
                bool ? showModal("modal_tower_details") : showModal("map_canvas");
            };
            const modalIds = {
                0: "map_canvas",
        ...
        function getCommaSeparatedPcis(cells) {
            ...
            for (let i of Object.values(cells)) {
                if (!pciList.includes(i.PCI) && i.PCI != null) {
                    pciList.push(i.PCI);
                }
            }
        ...
        async function getSite(requestSearchParams) {
            let res = await fetch(window.API_URL + "getTowerInformation?" + new URLSearchParams(requestSearchParams), {
                credentials: "include"
            });
            let resJson = await res.json();
            devLogger("res:", res, resJson);
            if (resJson?.statusCode == "NEED_RECAPTCHA") {
                window.handleResponse(resJson);
            }
        ...
        async function offerFirstnetPin(RAT, regionId, towerId, latitude, longitude) {
            (async () => {
                const potentialFnSite = await getSite({
                    MCC: "313",
                    MNC: "100",
                    RAT,
                ...
                if (potentialFnSite && potentialFnSite.towerMover == 0) {
                    devLogger("we better prompt");
                    const toast = new PPToast("firstnetOfferAttPinToast");
                    toast.setContents(src_elements.a("div", null, src_elements.a("p", null, "PCI+ detected that FirstNet ", src_elements.a("b", null, genNodeTitle(RAT) + " " + towerId), " also belongs at this location. Would you like to pin it here?"), src_elements.a("br", null), src_elements.a("button", {
                        id: "firstnetOfferYesButton",
                        class: "btn btn-danger"
                    }, "Yes"), " ", src_elements.a("button", {
                        id: "firstnetOfferNoButton",
                        class: "btn btn-success"
                    }, "No"), src_elements.a("button", {
                        id: "firstnetMapSelectButton",
                        class: "btn btn-info",
                        style: "float: right;"
                    }, "View")));
                    toast.showToast();
                    $("#firstnetOfferYesButton").on("click", (() => {
                        doOverride(313, 100, RAT, regionId, towerId, undefined, latitude, longitude, undefined, undefined, undefined);
                        toast.hideToast();
                    }));
                    $("#firstnetOfferNoButton").on("click", (() => {
                        toast.hideToast();
                    }));
                    $("#firstnetMapSelectButton").on("click", (() => {
                        let plmn = window.MCC == 313 && window.MNC == 100 ? [ 310, 410 ] : [ 313, 100 ];
                        selectProvider(plmn[0], plmn[1]);
                        if ($("#firstnetMapSelectButton")[0].innerText == "View") {
                            $("#firstnetMapSelectButton").text("Back");
                        } else {
                            $("#firstnetMapSelectButton").text("View");
                        }
                    }));
                }
            })();
        }
        async function doOverride(inMCC, inMNC, inRAT, inLAC, inBase, inCellId, lat, lng, inAction, inAttribute, inValue, towerProperName, copyLinkOnly, executeSilently) {
            ...
            if (overrideResponseText == "OKAY") {
                if (!(inAttribute == "TOWER_TYPE" && inValue == "DAS") && !window.isMobileDevice && !executeSilently) {
                ...
                if (inAction && inValue && inAttribute && !executeSilently) {
                ...
                if (lat != 0 && lng != 0 && !executeSilently) {
                    ...
                    if (window.pciPlus_promptFirstnetWhenPinningAtt && inMCC == 310 && inMNC == 410) {
                        offerFirstnetPin(inRAT, inLAC, inBase, lat, lng);
                    }
                }
            }
        }
        ...
        async function addSplitTower(position, splitId, cmgmId) {
            const {userID, pciPlus_cmgmUserId} = window;
            ...
            reqForm.set("cmgm_session_id", pciPlus_cmgmUserId);
            reqForm.set("username", userName);
            reqForm.set("id", String(cmgmId));
            reqForm.set("splitId", String(splitId));
            reqForm.set("pos", position);
            devLogger("form:", reqForm, pciPlus_cmgmUserId, userName, cmgmId, splitId, position);
 ...
 * @param cmgmId
 * @param editHistory
 * @author tiau, RaccoonCast
 */
        function showEditHistory(cmgmId, editHistory) {
            const editHistoryArr = editHistory.split("\n");
            for (let item in editHistoryArr) {
                if (editHistoryArr[item].startsWith("—")) {
                    editHistoryArr[item] = editHistoryArr[item].replaceAll(/—(.*)—/g, "\n$1");
                }
            }
            const editHistoryString = editHistoryArr.join("\n");
            const style = `\n    resize: none; \n    text-wrap: nowrap;\n    width: 100%;\n    line-height: 115%;\n    border: none;\n    outline: none;\n    font-family: monospace, monospace;\n    `;
            const content = src_elements.a("textarea", {
                readonly: "",
                rows: "35",
                cols: "120",
                style
            }, editHistoryString.replaceAll("\n", "&#013;"));
            const dialog = bootbox.dialog({
                title: `Edit History for #${cmgmId}`,
                size: "large",
                closeButton: false,
                message: content
            });
            dialog.on("shown.bs.open", (e => {
                $(dialog)[0].querySelector(".modal-content > .modal-header").appendChild($(dialog)[0].querySelector(".bootbox-close-button"));
            }));
            const modalDialog = $(dialog)[0].querySelector(".modal-dialog");
            $(dialog)[0].querySelector(".modal-body").style.maxHeight = "50em";
            modalDialog.style.width = "60%";
            modalDialog.style.maxWidth = "60%";
            modalDialog.style.height = "50%";
            modalDialog.style.maxHeight = "50%";
            const closeButton = textToHtml(src_elements.a("button", {
                type: "button",
                class: "bootbox-close-button close",
                "data-dismiss": "modal",
                "aria-hidden": "true"
            }, "×"));
            $(dialog)[0].querySelector(".modal-header").append(closeButton);
        }
        function buildCmgmComponents() {
            ...
            if (document.getElementById("cmgmTable") == null) {
 ...
 */
        async function organizeCmgmData(cmgmResponse) {
            ...
            for (let cmgm_field of Object.keys(cmgmResponse)) {
                if (cmgmResponse[cmgm_field] !== null && cmgmResponse[cmgm_field] != "") {
                    ...
                    if (cmgm_field == "bingmaps_a") {
                        cmgmBingLink = cmgmResponse[cmgm_field];
                    }
            ...
            return {
                ...
                cmgmBingLink: cmgmBingLink || null,
        ...
        async function updateTacDeny(cmgmId) {
            const {pciPlus_cmgmUserId} = window;
            const reqForm = new FormData;
            reqForm.set("cmgm_session_id", pciPlus_cmgmUserId);
            reqForm.set("id", String(cmgmId));
        ...
        function checkCmgmTac(cmgmResponse, currentSite) {
            try {
                ...
                if (currentTac != cmgmTac) {
                    ...
                    let toast = new PPToast("CMGMTacToast", 3e4, "CMGM");
        ...
        function updateSidebarAddress(cmgmResponse) {
            try {
                const {address, city, county, state, zip} = cmgmResponse;
                const addressSlices = [ address, city, county, state, zip ];
                ...
                const normalizedAddr = `${address}, ${city}, ${county}, ${state} ${zip}`;
        ...
        async function cmgmInit(liveReload) {
            try {
                const {currentSite, pciPlus_enableCmgmIntegration} = window;
                ...
                if (pciPlus_enableCmgmIntegration) {
                    ...
                    let {ok, cmgmId, cmgmConcealed, cmgmSiteType, cmgmEvLinks, cmgmPhotoLinks, cmgmSvLinks, cmgmBingLink, cmgmTags} = await organizeCmgmData(cmgmTower);
                    if (ok == false) {
                        ...
                        $("#cmgmTable").css("display", "");
                        $("#cmgmTableBody").html(cmgmKnown(cmgmConcealed, cmgmSiteType, cmgmEvLinks, cmgmPhotoLinks, cmgmSvLinks, cmgmTags));
                        if (cmgmBingLink != null) {
                            const bingEl = $("#cmgmTableBody")[0].querySelector("#bingMapsLink");
                            bingEl.style.display = "";
                            bingEl.href = cmgmBingLink;
                        }
                    }
                    const cmgmStatusEl = liveReload ? document.getElementById("cmgmNameAndId") ?? document.getElementById("cmgmStatus") : document.getElementById("cmgmStatus");
                    ...
                    if (liveReload) {
                        $(".cmgmCreateLink").parent().addClass("d-none");
                    }
                    ...
                    if (!siteNotSupportedByCmgm) {
                        addCmgmActionButton(ok, cmgmTower, currentSite);
                    }
                }
            } catch (err) {
                devLogger(err);
        ...
        function addCmgmActionButton(hasCmgmRecord, cmgmTower, currentSite) {
            if (!hasCmgmRecord && (currentSite.towerMover == undefined || currentSite.towerMover < 1)) {
            ...
            } else if (hasCmgmRecord == true) {
                addToTable(cmgm_view_history_name, cmgm_view_history_description, cmgm_view_history_icon, "#", "cmgmMarkSplitAction", (() => {
                    showEditHistory(parseInt(cmgmTower.id), cmgmTower.edit_history);
                }), true);
            }
        }
            ...
            $(`.cmgmSplitLocButton`).on("click", (async event => {
                ...
                if (res && res.ok) {
                    if (res.pinCoordsIncluded && (originSite.towerMover == undefined || originSite.towerMover < 1)) {
                        const {currentSite} = window;
                        window.doOverride(currentSite.Provider.countryID, currentSite.Provider.providerID, currentSite.RAT, parseInt(currentSite.regionID), parseInt(currentSite.siteID), null, res.coordsToPin.latitude, res.coordsToPin.longitude, null, null, null, currentSite.siteID);
                    } else {
        ...
        function overrideSelectInteractionEventListener() {
            ...
            select_interaction.features_.listeners_.add[0] = async e => {
                ...
                if (marker.get("base") == undefined && marker.get("CID") != undefined) {
                    ...
                    $('[id^="detailsTable"]').css("border-style", "solid");
                ...
                if (marker.get("name") == "ApproxCoverageCircle") {
                    e.stopPropagation();
                    return false;
                }
        ...
        async function getTowerCoverage(cMCC, cMNC, cLAC, cTowerId, color, highlight) {
            let {API_URL, netType, deg2dir, handleResponse, ol, map, currentSite, pciPlus_allowSelectMultipleTowers, pciPlus_selectCidRenderType, pciPlus_showTrueSectors, pciPlus_customSectorColors, pciPlus_doColorsBySite, MCC, MNC} = window;
            ...
            for (let cellIndex of Object.keys(cellsCoverage)) {
                ...
                if (window.pciPlus_onlyShowSelectedBandSectors) {
                    ...
                    if (window.showBand != 0 && window.showBand != cellBand && !(currentSite.Provider.countryID == 310 && currentSite.Provider.providerID == 260 && currentSite.RAT == "NR" && cellBand == 2 && window.showBand == 25)) {} else {
            ...
            if (window.pciPlus_allowSelectMultipleTowers) {
        ...
        async function handlePCIPSCSearch(input_pci, input_tac) {
            ...
            let pciCancelled = false;
            pciLoadToast.onClose((() => {
                devLogger(`PCI Search for ${pciPsc} was cancelled!`);
                pciCancelled = true;
                endSearchCleanup();
            }));
        ...
        async function addJumpCoverage() {
            ...
            for (let i of cellNodes) {
                try {
                    ...
                    cellIdNode.innerHTML = `<td width="50%">Cell Identifier</td><td style="color:${inline_link_color}; cursor:pointer;" href='#' onclick='if (isMobileDevice) { toggleTowerInfo(false); } centreMap(${lat},${long}, true, ${zoom > 18 ? zoom : 18}); highlightByCid(${cidMatchOutput})'>${cidMatchOutput}</td>`;
                } catch (err) {
        ...
        function drawCircle(point, radius, dir) {
            ...
            var points = Math.floor(Math.random() * 200) == 1 ? 4 : resolution;
            ...
            for (var i = start; dir == 1 ? i < end : i > end; i = i + dir) {
                ...
                let ey = point[0] + rlng * Math.cos(theta);
                let ex = point[1] + rlat * Math.sin(theta);
                extp.push([ ey, ex ]);
            }
            return extp;
        }
        function addCircleToMap(lat, lon, radiusInMeters, labelText) {
            const {ol, map} = window;
            var ring = drawCircle([ lon, lat ], radiusInMeters, 1);
            var polygon = new ol.geom.Polygon([ ring ]);
            polygon.transform("EPSG:4326", "EPSG:3857");
            if (!window.lookupCircleSource) {
                createCircleSource();
            }
            let locationName = `${lat}-${lon}-${radiusInMeters}`;
            const features = window.lookupCircleSource.getFeatures();
            if (features.length > 0) {
                for (let feat of features) {
                    if (feat.get("loc") == locationName) {
                        devLogger("feature already present!", features);
                        return;
                    }
                }
            }
            var feature = new window.ol.Feature(polygon);
            feature.set("name", "ApproxCoverageCircle");
            feature.set("loc", locationName);
            if (labelText && window.pciPlus_showLabelsInCoverageEstimateCircles) {
                feature.set("textLabel", new ol.style.Text({
                    font: "13px Calibri,sans-serif",
                    fill: new ol.style.Fill({
                        color: "#ededed"
                    }),
            ...
            window.lookupCircleSource.addFeature(feature);
        }
        function createCircleSource() {
            const {ol, map} = window;
            var circlePolygonVectorSource = new ol.source.Vector;
            window.lookupCircleSource = circlePolygonVectorSource;
            var circleLayer = new ol.layer.Vector({
                source: circlePolygonVectorSource,
                style: feature => new ol.style.Style({
                    ...
                    text: feature.get("textLabel")
                })
            });
            map.addLayer(circleLayer);
        }
        function searchCachedLookups(siteId) {
            const {globalThirdPartyLookupList} = window;
            if (!globalThirdPartyLookupList[siteId]) {
                return {
                    ok: false
                };
            } else {
                let result = globalThirdPartyLookupList[siteId];
                result.cached = true;
                return result;
            }
        }
        async function searchGoogleGeolocation(GOOGLE_GEOLOCATION_TOKEN) {
            let siteLocation = [ 0, 0 ];
            const {currentSite} = window;
            ...
            for (let i of currentSiteKeys) {
                let newObj = {
                    locationAreaCode: Number(currentSite.regionID),
                    cellId: Number(i),
                    mobileCountryCode: currentSite.Provider.countryID,
                    mobileNetworkCode: currentSite.Provider.providerID,
                    age: 1,
                    signal: "-50",
                    timingAdvance: 15
                };
            ...
            if (currentSite.Provider.countryID == 310 && currentSite.Provider.providerID == 260 && currentSite.RAT == "LTE") {
                for (let i of [ [ 1, 40 ], [ 2, 40 ], [ 3, 50 ] ]) {
                    const cid = (parseInt(currentSite.siteID) << 8) + Number(i[0]);
                    if (!currentSiteKeys.includes(String(cid))) {
                        let newObj = {
                            locationAreaCode: Number(currentSite.regionID),
                            cellId: cid,
                            mobileCountryCode: currentSite.Provider.countryID,
            ...
            if (currentSite.Provider.countryID == 310 && currentSite.Provider.providerID == 410 && currentSite.RAT == "LTE") {
                for (let i of [ [ 8, 40 ], [ 9, 40 ], [ 10, 50 ], [ 15, 40 ], [ 16, 40 ], [ 17, 40 ], [ 22, 40 ], [ 23, 40 ], [ 24, 50 ] ]) {
                    const cid = (parseInt(currentSite.siteID) << 8) + Number(i[0]);
                    if (!currentSiteKeys.includes(String(cid))) {
                        let newObj = {
                            locationAreaCode: Number(currentSite.regionID),
            ...
            if (currentSite.Provider.countryID == 311 && currentSite.Provider.providerID == 480 && currentSite.RAT == "LTE") {
                for (let i of [ [ 1, 40 ], [ 2, 40 ], [ 3, 50 ] ]) {
                    const cid = (parseInt(currentSite.siteID) << 8) + Number(i[0]);
                    if (!currentSiteKeys.includes(String(cid))) {
                        let newObj = {
            ...
            let currentCarrierName = window.MNCSelectBox.options["" + currentSite.Provider.countryID + currentSite.Provider.providerID].text.split(" - ")[0].trim() || "";
            let res = await fetch(`https://www.googleapis.com/geolocation/v1/geolocate?key=${GOOGLE_GEOLOCATION_TOKEN}`, {
                method: "POST",
                body: JSON.stringify({
                    homeMobileCountryCode: currentSite.Provider.countryID,
                    homeMobileNetworkCode: currentSite.Provider.providerID,
                    radioType: currentSite.RAT,
                    carrier: currentCarrierName,
                    considerIp: false,
                    cellTowers: cellsToPass
                })
            });
            let returnedRes = await res.json();
            devLogger("Google responded:", returnedRes);
            if (returnedRes.accuracy == undefined) {
                return {
                    ok: false
                };
            } else {
                siteLocation = [ returnedRes.location.lat, returnedRes.location.lng ];
                return {
                    ok: true,
        ...
        async function searchUnwiredLabs(UNWIREDLABS_TOKEN) {
            ...
            if (returnedRes.lat == undefined || returnedRes.lon == undefined) {
                ...
                return {
                    ...
                    coords: siteLocation,
                    accuracy: returnedRes.accuracy,
                    remainingRequests: returnedRes.balance
                };
            }
        }
        async function doCalculate() {
            ...
            const GOOGLE_GEOLOCATION_KEY = "AIzaSyB2rfY-ihTBTNYmW437vzegflcXkwU7Rh8";
            const {currentSite} = window;
            ...
            providers.set(searchCachedLookups, [ parseInt(window.currentSite.siteID) ]);
            providers.set(searchGoogleGeolocation, [ GOOGLE_GEOLOCATION_KEY ]);
            providers.set(searchUnwiredLabs, [ UNWIREDLABS_KEY ]);
            providers.set(searchOpenCellid, [ OPENCELLID_KEY ]);
            for (let providerFunc of Array.from(providers.keys())) {
                if (currentSite.RAT != "LTE" && providerFunc == searchGoogleGeolocation) {
                    continue;
                }
                let result = await providerFunc(...providers.get(providerFunc));
                devLogger("result:", result);
                if (result.ok == false && result.cached == true) {
                    break;
                }
                if (result.ok == false) {
                    continue;
                ...
                centreMap(result.coords[0], result.coords[1], true, isDish ? 20 : 16);
                let humanAccuracy = "";
                if (window.pciPlus_distanceUnit == "km") {
                    humanAccuracy = result.accuracy + "m";
                } else if (window.pciPlus_distanceUnit == "mi") {
                    humanAccuracy = (result.accuracy / 1609).toFixed(1) + "mi";
                }
                const fullSiteName = `${genNodeTitle(currentSite.RAT)} ${getFullSiteName(currentSite)}`;
                let message = `According to ${result.name}, <b>${fullSiteName}</b> is ${result.accuracy ? `within ${humanAccuracy} of` : "near"} (${result.coords[0].toFixed(3)}, ${result.coords[1].toFixed(3)}).`;
                if (!result.cached && result.name == "Unwired Labs") {
                    message += `<br/> <br/> PCI+ has ${result.remainingRequests} requests remaining today.`;
                }
                toast.setContents(message);
                toast.setupTimeout(1e4);
                toast.showToast();
                if (result.accuracy && !isDish) {
                    addCircleToMap(result.coords[0], result.coords[1], result.accuracy, fullSiteName);
                }
                const siteId = parseInt(currentSite.siteID);
                window.globalThirdPartyLookupList[siteId] = result;
                return;
            }
            toast.setupTimeout(5e3);
            toast.setContents("Tower could not be found.");
            toast.showToast();
            window.globalThirdPartyLookupList[parseInt(currentSite.siteID)] = {
                ok: false,
                cached: true
            };
        }
        ...
        const copyTowerLink = () => {
            ...
            let copyTowerOnclick = () => {
                let newLink = `https://www.cellmapper.net/map?MCC=${currentSite.Provider.countryID}&MNC=${currentSite.Provider.providerID}&type=${currentSite.RAT}&latitude=${currentSite.latitude.toFixed(5)}&longitude=${currentSite.longitude.toFixed(5)}&zoom=17&ppT=${currentSite.siteID}&ppL=${currentSite.regionID}`;
        ...
        const doTowerLookup = () => addToTable("Request Tower Lookup⁺", "Look up tower location using a third-party provider", "🔎", "#", "pciPlusLookupTower", (() => doCalculate()));
        ...
        function transformCmActionsLinks() {
            ...
            for (const i of Object.keys(emojiMap)) {
                ...
                if (cmElement == undefined) {
                    ...
                    if (window.currentSite.RAT == "NR" && i == "Change Tower Type") {
                        doNrChanges();
                    }
        ...
        function doNrChanges() {
            let dialogPage = $("#dialog-form-tower-changetypedialog")[0];
            let dasCell = dialogPage.querySelector('.aProviderCell[title="DAS"]');
            dasCell.classList.add("cellDisabled");
            let dasItem = dialogPage.querySelector('option[value="DAS"]');
            dasItem.disabled = true;
            if (dasItem.selected) {
                $("#towertype_input").val("MACRO");
            }
            $("#nrTowerTypeChangeWarning").removeClass("d-none");
            $("#modifyTowerTypeForm").find(".modal-body").find("p").addClass("d-none");
        }
        function revertNrChanges() {
            let dialogPage = $("#dialog-form-tower-changetypedialog")[0];
            let dasCell = dialogPage.querySelector('.aProviderCell[title="DAS"]');
            dasCell.classList.remove("cellDisabled");
            let dasItem = dialogPage.querySelector('option[value="DAS"]');
            dasItem.disabled = false;
            $("#nrTowerTypeChangeWarning").addClass("d-none");
            $("#modifyTowerTypeForm").find(".modal-body").find("p").removeClass("d-none");
        }
        const addDisabledCmgmSplit = () => addToTable(cmgm_view_history_name, "", cmgm_view_history_icon, "#", "cmgmMarkSplitAction", (() => {}));
        const towerMetadataHistory = () => addToTable("View Meta History", "View historical tower metadata (eg. Tower Type)", "🏷️", "#", "cmViewTowerMetadataHistory", (() => {
            const {currentSite, getTowerOverrideHistory} = window;
            getTowerOverrideHistory(currentSite.Provider.countryID, currentSite.Provider.providerID, currentSite.RAT, currentSite.regionID, currentSite.siteID, 0, true);
        }));
        const towerMoveHistory = () => addToTable("View Move History", "View historical tower locations", "📝", "#", "cmViewTowerLocationHistory", (() => {
            const {currentSite, getTowerOverrideHistory} = window;
            getTowerOverrideHistory(currentSite.Provider.countryID, currentSite.Provider.providerID, currentSite.RAT, currentSite.regionID, currentSite.siteID, 0);
        }));
        function addActions(actionFuncs, requireUnverified) {
            const {currentSite} = window;
            function dontAddAction(func, funcsArray) {
                return func == addDisabledCmgmSplit && funcsArray.indexOf(addDisabledCmgmSplit) == funcsArray.length - 1 && !window.pciPlus_enableCmgmIntegration;
            }
        ...
        function initActionsTable() {
            ...
            const actionsPriorityOne = [ moveToTowerLocation, copyTowerLink, towerMoveHistory, addDisabledCmgmSplit, towerMetadataHistory ];
            addActions(actionsPriorityOne, false);
            const actionsRequireUnverified = [ doTowerLookup ];
        ...
        function addCellArea() {
            ...
            function executeAreaAdd() {
                for (let cell of Object.keys(currentSite.cells)) {
                    ...
                    let directionCell = $(table).find("tr:contains(Direction)")[0] || $(table).find('tr:contains("First Seen")')[0];
        ...
        function patchGetBaseStation() {
            ...
            window.getBaseStation = async (MCC, MNC, LAC, site, marker, customReadyCount) => {
                ...
                const longLoadingTimeToast = new PPToast("towerTakingTimeToast", 8e3);
                longLoadingTimeToast.setContents("This request is taking a while! Please hold...");
                devLogger("got base station");
                let notReadyCount = 0;
                async function checkifReady() {
                    if (notReadyCount > 50) {
                        longLoadingTimeToast.showToast();
                    }
                    if (notReadyCount > (customReadyCount || 300)) {
                        longLoadingTimeToast.hideToast();
                ...
                async function executePatches() {
                    ...
                    const firstSeen = detailsTable.find('td:contains("First Seen")');
                    const lastSeen = detailsTable.find('td:contains("Last Seen")');
                    try {
                        firstSeen.siblings().text(new Date(currentSite.firstseendate).toLocaleDateString(void 0, window.dateOptions));
                        lastSeen.siblings().text(new Date(currentSite.lastseendate).toLocaleDateString(void 0, window.dateOptions));
                    } catch (err) {
                        firstSeen.text("First Seen (Parent)");
                        lastSeen.text("Last Seen (Parent)");
                    }
                    ...
                    const detailRowClasses = {
                        ...
                        "Max RSRQ": "detailsTableMaxRSRQ",
                        "Max SNR": "detailsTableMaxSNR",
                        "Max SS RSRQ": "detailsTableMaxSSRSRQ",
                        "Max SS SNR": "detailsTableMaxSSSNR",
                        "Max CSI RSRP": "detailsTableMaxCSIRSRP",
                        "Max CSI RSRQ": "detailsTableMaxCSIRSRQ",
                        "Max CSI SNR": "detailsTableMaxCSISNR",
                        ...
                        Notes: "detailsTableCellNumberUsedForNotes",
                    ...
                    let removals = 0;
                    for (let el of document.getElementById("divcontentdata").children) {
                        if (removals == 4) {
                            break;
                        }
                        if (el.tagName === "B" && el.innerText.includes("History:")) {
                            $(el).css("display", "none");
                        }
                        if (el.tagName === "A" && el.innerText == "View History") {
                            $(el).css("display", "none");
                            removals++;
                        }
                        if (el.tagName == "BR") {
            ...
            const toggleNonPremiumHandler = event => {
                devLogger("clicked");
            ...
            window.pp_toggleNonPremium = toggleNonPremiumHandler;
        }
                ...
                if (clipboard.match(regex)) {
                    ...
                    pinButton.onclick = () => {
                        ...
                        if (window.currentSite != undefined) {
                            ...
                            const noSiteToast = new PPToast("noSiteDefinedToast", 5e3);
                            noSiteToast.setContents("You haven't selected a site to pin!");
                            noSiteToast.showToast();
                        }
                        jumpClipboardCoordsToast.clearTimeout();
                        jumpClipboardCoordsToast.hideToast();
                    };
        ...
        function addEventListeners() {
            ...
            window.MNCSelectBox.on("item_add", (item => {
                ...
                if (window.pciPlus_showFccPolygons !== "off") {
                    toggleFccLayer(true, updatedMCC, updatedMNC);
                }
            ...
            window.NetSelectBox.on("item_add", (() => {
                if (window.pciPlus_showFccPolygons !== "off") {
                    toggleFccLayer(true);
                }
            }));
            window.select_interaction.listeners_.select[0] = evt => {
                devLogger("map select fired, pci+ intercepting");
                if (evt?.deselected) {
                    for (const deselectedEl of evt.deselected) {
                        if (deselectedEl.get("CID") != undefined) {
                            $(`#detailsTable${deselectedEl.get("CID")}`).css("border-style", "solid");
                        }
                    }
                }
            };
        ...
        async function q_gotoAndLoadTower(tower, tac, MCC, MNC, cid, latitude, longitude) {
            ...
            if (serverLoggedIn) {
                waitForMapLoggedIn();
                const result = await getBaseStation(MCC, MNC, parseInt(tac), parseInt(tower), undefined, 100);
                if (result == false) {
                    const toast = new PPToast("baseStationNotFoundToast", 5e3);
                    toast.setContents(`No site was found with ${genNodeTitle(window.netType)} ${tower}.`);
                    toast.showToast();
                }
                ...
                if (data.length > 1) {
                    ...
                    devLogger("No towers were found! Show popup");
                    return;
                }
                if (window.pciPlus_showSidebar) {
                    toggleSection("Details");
        ...
        function modifySidebarUx() {
            ...
            towerDetailsSyle.textContent = `\n    /* [PCI+] Set bottom margin for sidebar elements */\n    #modal_tower_details_content > table {\n      margin-bottom: 1rem !important;\n    }\n\n    /* [PCI+] Override 50% separator, since sidebar has a minimum width */\n    #settings > tbody > tr > td,\n    #ppSettings > tbody > tr > td,\n    #ppSettings > tbody > table > tbody > tr > td,\n    #filters > tbody > tr > td {\n      white-space: nowrap !important; \n    }\n    `;
            ...
            try {
                ...
                bootbox.confirm({
                    ...
                    callback: confirmed => {
                        ...
                        getSite(requestSearchParams).then((res => {
                            if (res !== false) {
        ...
        async function addWikiContributors(title, isDas) {
            devLogger("finding wiki name");
            const wikiNames = await getWikiContributors(title);
            if (wikiNames.length == 0) {
                return;
            }
            let containerAlreadyPresent = document.getElementById("wikiContributorsContainer") !== null;
            const container = src_elements.a("span", {
                id: "wikiContributorsContainer"
            }, src_elements.a("br", null), src_elements.a("b", null, "Wiki Contributors:"), src_elements.a("span", {
                id: "wikiContributors"
            }));
            if (!containerAlreadyPresent) {
                $(container).insertAfter("#towerContributors");
            }
            for (const name of wikiNames) {
                let userElement;
                const nameRegex = new RegExp(`\\W${name}\\W`, "i");
                devLogger("name:", nameRegex);
                if ($("#wikiContributors").html().match(nameRegex)) {
                    devLogger(`Skipping wiki contributor add of ${name} as that user is already present`);
                    return;
                }
                const userStats = getUserByName(name);
                if (userStats != null) {
                    window._renderUserStats(userStats.id, "wikiContributors");
                } else {
                    $("#wikiContributors").append(src_elements.a("span", null, name.toLocaleLowerCase()));
                }
                devLogger("user add:", userElement);
                $("#wikiContributors").append(" ");
            }
        }
        function getUserByName(userName) {
            for (let user of Object.values(window.userCache)) {
        ...
        async function getWikiContributors(pageId) {
            const res = await fetch(`https://docs.cellmapper.net/mw/api.php?action=query&titles=${pageId}&prop=revisions&rvprop=user&rvlimit=max&format=json`);
            const revisions = await res.json();
            let pageContributors = [];
            const page = Object.values(revisions.query.pages)[0];
            for (let userBlock of page.revisions) {
                const user = userBlock.user;
                if (!pageContributors.includes(user)) {
                    pageContributors.push(user);
                }
            }
            devLogger("contributors:", pageContributors);
        ...
        async function generateWikiPage(MCC, MNC, LAC, RAT, towerId) {
            ...
            for (let network of networks) {
                ...
                for (let wikiPage of [ await genWikiType(`${MCC}_${network}_${das_parent_id}`, true, undefined, das_parent_id), await genWikiType(`${MCC}_${network}_${LAC}_${towerId}`, false), await genWikiType(`${MCC}_${network}_${towerId}`, false) ]) {
                    ...
                    await addWikiContributors(title, wikiPage.isDas);
                    pagesAdded++;
                    if (wikiPage.isDas) {
                        dasPagesAdded++;
                    }
                ...
                if (!(window.currentSite.siteID == towerId)) {
                    devLogger(`Detected site deviation; cancelling Add-Data button render for ${towerId}`);
                    return;
                }
                if (document.querySelector("#divcontentdata > tr > td > div > a > b") != null) {
                    devLogger("Wiki 'Add Data' already present, exit!");
                    return;
                }
        ...
        function resizeElements() {
            ...
            $(".mw-parser-output a").each(((i, item) => {
                item.target = "_blank";
            }));
        }
        function wikiPageIsEmpty(html) {
            const doc = (new DOMParser).parseFromString(html, "text/html");
            if (doc.getElementsByClassName("mw-parser-output") && doc.getElementsByClassName("mw-parser-output")[0].children.length === 0) {
                return true;
            } else {
                return false;
            }
        ...
        function toggleTrails() {
            ...
            updateLinkback();
            $.cookie("pciPlus_tilesEnabled", window.tilesEnabled, {
                expires: 3600
            });
        ...
        function persistSignalTrails(queryParams) {
            ...
            let tilesEnabledCookie = $.cookie("pciPlus_tilesEnabled") || "true";
            ...
            $("#doTrails").prop("checked", window.tilesEnabled);
            $.cookie("tilesEnabled", tilesEnabledCookie, {
                expires: 3600
            });
            updateLinkback();
        ...
        function patchRemoveZoomInToast() {
            (function($) {
                ...
                $.fn.toast = function() {
                    ...
                    if (this[0].id == "toastMessage" && (toastMessage === "Zoom in to display more towers" || toastMessage === "Attempting to retrieve your location")) {
                        return;
                    }
        ...
        function patchHandleResponse() {
            ...
            window.handleResponse = response => {
                if (response.statusCode && response.statusCode == "NEED_RECAPTCHA") {
                    ...
                    $("#totalCaptchasSolvedNum").text($.cookie("pciPlus_captchaCounter") || 0);
                }
                if (response.responseData && response.responseData == "RECAPTCHA_OK") {
                    ...
                    let counter = $.cookie("pciPlus_captchaCounter");
                    if (counter == undefined) {
                        counter = 0;
                    }
                    const newVal = parseInt(counter) + 1;
                    $.cookie("pciPlus_captchaCounter", newVal, {
                        expires: 1e6
                    });
                }
        ...
        function addFilterCustomBand() {
            ...
            spChildren[indexToPutItIn].outerHTML = src_elements.a("span", null, " ", src_elements.a("a", {
                title: "Select a band that's not in the list",
        ...
        function moveTower(inMCC, inMNC, inSystem, inLAC, inBase, inCell, inLat, inLng, setVerified, newTowerType) {
            ...
            if (setVerified != undefined) {
                ...
                if (newTowerType == "MICRO" && window.pciPlusFilter_hideMicro || newTowerType == "DAS" && window.pciPlusFilter_hideDas == "hideDas" || newTowerType == "DECOMMISSIONED" && window.pciPlusFilter_hideDecom || newTowerType == "MACRO" && window.pciPlusFilter_hideMacro == "hideAll" || newTowerType == "PICO" && window.pciPlusFilter_hidePico) {
        ...
        function moveLastGeneratedDate() {
            try {
                const regex = /(<b>Last Generated:<\/b> (.*)<br>)/;
                const innerHTML = document.getElementById("divcontentdata").innerHTML;
                const match = innerHTML.match(regex);
                if (match == null) {
                    devLogger("Could not find last generated date. Exiting");
                    return false;
                }
                const dateForTower = window.currentSite.towerAttributes.TIME;
                document.getElementById("divcontentdata").innerHTML = innerHTML.replace(match[1], "");
                const genDateTableRow = textToHtml(src_elements.a("tr", null, src_elements.a("td", null, "Last Generated"), src_elements.a("td", {
                    title: new Date(parseInt(dateForTower)).toLocaleString()
                }, new Date(parseInt(dateForTower)).toLocaleDateString(void 0, window.dateOptions), src_elements.a("a", {
                    href: "#",
                    style: "user-select: none;",
                ...
                const forceUpdateToggle = genDateTableRow.querySelector("#forceUpdateToggle");
                forceUpdateToggle.onclick = async () => {
                    devLogger("Force update requested");
                    forceUpdateToggle.style.display = "none";
                    const {currentSite} = window;
                    if ((currentSite.RAT == "LTE" || currentSite.RAT == "NR") && currentSite.towerAttributes.TOWER_TYPE !== "DAS") {
                        await doOverride(currentSite.Provider.countryID, currentSite.Provider.providerID, currentSite.RAT, parseInt(currentSite.regionID), parseInt(currentSite.siteID), null, null, null, "setAttribute", "TOWER_TYPE", currentSite.towerAttributes.TOWER_TYPE, null, null, true);
                    } else {}
                    const toast = new PPToast("forceUpdateSuccessfulToast", 5e3);
                    toast.setContents(src_elements.a("span", null, src_elements.a("b", null, genNodeTitle(currentSite.RAT), " ", getFullSiteName(currentSite)), " ", "was sucessfully updated."));
                    toast.showToast();
                };
                const addressRow = $('#modal_tower_details_content > table:nth-child(1) > tbody > tr:contains("Address")');
                const cmgmRow = $('#modal_tower_details_content > table:nth-child(1) > tbody > tr:contains("CMGM")');
                const lastRow = "#modal_tower_details_content > table:nth-child(1) > tbody > tr:last-child";
                if (addressRow[0]) {
                    $(genDateTableRow).insertBefore(addressRow);
                } else if (cmgmRow[0]) {
                    $(genDateTableRow).insertBefore(cmgmRow);
                } else {
                    $(genDateTableRow).insertBefore(lastRow);
                }
            } catch (err) {
                devLogger('Error moving "last generated" date!', err);
                return false;
            }
        }
        function patchMoveToCurrentLocation() {
            const originalFunction = window.moveToCurrentLocation;
            window.moveToCurrentLocation = prevent_move => {
                if (typeof window.CMApp == "undefined" && navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition((pos => {
                        const coords = [ pos.coords.latitude, pos.coords.longitude ];
                        if (!prevent_move) {
                            centreMap(coords[0], coords[1], true);
                        }
                        return coords;
                    }));
                } else {
                    originalFunction(prevent_move);
                }
            };
        }
        function patchMapBoundsChanged() {
            const {ol} = window;
            const ogFunction = window.mapBoundsChanged;
            window.mapBoundsChanged = () => {
                ogFunction();
                function mapCenterWithinExtent(extent) {
                    let mapCenter = getMapCenter();
                    return ol.extent.containsCoordinate(extent, [ mapCenter[1], mapCenter[0] ]);
                }
                const massExtent = ol.extent.boundingExtent([ [ -73.508142, 41.187054 ], [ -69.858861, 42.886589 ] ]);
                if (mapCenterWithinExtent(massExtent)) {
                    $("#massachusetts_aerial").removeClass("d-none");
                } else {
                    $("#massachusetts_aerial").addClass("d-none");
                }
                const lacExtent = ol.extent.boundingExtent([ [ -119.1644, 32.616 ], [ -117.3715, 34.8233 ] ]);
                if (mapCenterWithinExtent(lacExtent)) {
                    $("#losangelescounty_aerial").removeClass("d-none");
                } else {
                    $("#losangelescounty_aerial").addClass("d-none");
                }
                if (!window.showTowers) {
                    window.updateLinkback();
                }
        ...
        function linkPciInSidebar(jNode) {
            try {
                ...
                const pciNumber = $(jNode)[0].querySelector(".detailsTablePci > td:last-child");
                pciNumber.outerHTML = src_elements.a("td", {
                    class: `pciLink${pulledPci}`,
                    style: "color:#18BC9C; cursor: pointer;",
                    onclick: `window.handlePCIPSCSearch(${pulledPci}, parseInt(${currentSite.regionID}))`
                }, pulledPci);
        ...
        function changeCellDateTime(jNode) {
            try {
                let {currentSite} = window;
                ...
                const fsTime = currentSite.cells[cid].FirstSeen;
                let fsTimeCell = new Date(fsTime).toLocaleString();
                let fsTimeCellDate = new Date(fsTime).toLocaleDateString(void 0, window.dateOptions);
                const lsTime = currentSite.cells[cid].LastSeen;
                let lsTimeCell = new Date(lsTime).toLocaleString();
                let lsTimeCellDate = new Date(lsTime).toLocaleDateString(void 0, window.dateOptions);
                let table = document.getElementById(`detailsTable${cid}`);
                if (typeof table !== "undefined") {
                    let fsRow = table.rows[fsIndex + 1].children[1];
                    fsRow.innerText = fsTimeCellDate;
                    fsRow.setAttribute("title", fsTimeCell);
                    let lsRow = table.rows[lsIndex + 1].children[1];
                    lsRow.innerText = lsTimeCellDate;
                    lsRow.setAttribute("title", lsTimeCell);
                }
            } catch (err) {
                devLogger("Error adding time to sidebar:", err);
            }
        }
        function addCaptchaCounter() {
            const el = getCaptchaStatsElement();
            const footerEl = $("#dialog-requests-exceeded > .modal-dialog > .modal-content > .modal-footer");
            footerEl.css("justify-content", "left");
            footerEl.append(el);
            window.addEventListener("message", (event => {
                const data = event.data.target;
                if (data?.id == "recaptcha-verify-button") {
                    incrementCounter("pciPlus_captchaChallengesSolvedCounter");
                } else if (data?.class?.startsWith("rc-image-tile")) {
                    incrementCounter("pciPlus_captchaSquaresClickedCounter");
                } else {
                    devLogger("recieved a message we don't understand!", event.data);
                }
            }), false);
            function incrementCounter(cookieName) {
                let counter = $.cookie(cookieName);
                if (counter == undefined) {
                    counter = 0;
                }
                const newVal = parseInt(counter) + 1;
                ...
                if (document.getElementById("captchaStats")) {
                    $("#captchaStats").replaceWith(getCaptchaStatsElement());
                }
            }
        }
        const extendedCaptchaSupported = window.ppBrowserExtension || window.navigator.userAgent.match(/CellMapperIos/i);
        function getCaptchaStatsElement() {
            return src_elements.a("span", {
                id: "captchaStats"
            }, src_elements.a("span", {
                title: "Total CAPTCHA challenges solved and accepted"
            }, "Total CAPTCHA Solves:", " ", src_elements.a("b", {
                id: "totalCaptchasSolvedNum"
            }, $.cookie("pciPlus_captchaCounter") || 0)), src_elements.a("br", null), src_elements.a("span", {
                class: extendedCaptchaSupported ? "d-none" : "",
                title: "Total CAPTCHA challenges submitted, including those that were not accepted"
            }, "Total CAPTCHA Attempts:", " ", src_elements.a("b", {
                id: "totalCaptchaChallengesSolvedNum"
            }, $.cookie("pciPlus_captchaChallengesSolvedCounter") || 0)), src_elements.a("br", null), src_elements.a("span", {
                class: extendedCaptchaSupported ? "d-none" : "",
                title: "Total images selected inside of CAPTCHA challenges"
            }, "Total CAPTCHA Interactions:", " ", src_elements.a("b", {
                id: "totalCaptchaSquaresClickedNum"
            }, $.cookie("pciPlus_captchaSquaresClickedCounter") || 0)));
        }
        ...
        function patchGetNetworkInfo() {
            const origGetNetworkInfo = window.getNetworkInfo;
            window.getNetworkInfo = (MCC, MNC) => {
                setupCarrierSwitchback();
                hideCountryInSelect();
                origGetNetworkInfo(MCC, MNC);
            };
        }
        async function patchCellMapper(queryParams) {
            ...
            hookSettingsMenu();
            handleUpdates();
            patchMoveToCurrentLocation();
            patchStopJqueryWikiGet();
            if (window.pciPlus_enableCmgmIntegration && window.pciPlus_showCmgmCheckmark) {
                getInitialVerifiedSites();
            }
        ...
        function addMapradLink() {
            ...
            const el = textToHtml(src_elements.a("a", {
                href: `https://maprad.io/us/search/coordinates/1000/${lat},${lng}?source=US&coordStr=${lat},${lng}&radius=1000`,
                target: "_blank",
                style: "\n    margin-left: auto;\n    padding-left: 2.5rem;\n    padding-top: 0px;\n    margin-top: 0px;\n    font-weight: bold;\n    color: deeppink;\n"
            }, "(Maprad.io)"));
        ...
        function patchMenu() {
            ...
            newNode.innerHTML += `<li aria-checked="false" data-index="10" role="menuitemradio" tabindex="0" jsaction="click: actionmenu.select; keydown: actionmenu.keydown;" jstcache="316" jsinstance="*10" class="nbpPqf-menu-x3Eknd fxNQSd" jsan="0.aria-checked,7.nbpPqf-menu-x3Eknd,0.data-index,0.role,0.tabindex,0.jsaction"><div jstcache="317" style="display:none"></div><div jstcache="318" style="display:none"></div><span jstcache="319" style="display:none"></span><div jstcache="320" class="nbpPqf-menu-x3Eknd-text-haAclf" jsan="7.nbpPqf-menu-x3Eknd-text-haAclf,t-nsjBiGFs4q0"><div jstcache="336" class="nbpPqf-menu-x3Eknd-text" jsan="7.nbpPqf-menu-x3Eknd-text">Measure distance</div><div jstcache="337" style="display:none"></div></div></li>`;
        ...
        function addRedirectLink() {
            ...
            const redirectMatch = /\#REDIRECT\s\[\[([A-z0-9\-\_\:\s]+)\]\]/;
        ...
        function patchCellmapperCaptcha() {
        ...
        const patchedSites = {
            ...
            cellmapperCaptcha: /https:\/\/www\.recaptcha\.net\/recaptcha\/api2\/bframe\?.*&k=6LeRrhIUAAAAAC0_WQguxdrrcHztb8RLS03lFgE2$/,
        ...
        function init(queryParams) {
            ...
            } else if (currentUrl.match(patchedSites.cellmapperCaptcha)) {
                patchCellmapperCaptcha();
                console.log("[PCI+] Patched CellMapper Captcha");
        ...
        if (window.location.href.match(patchedSites.cellmapper)) {
            console.log("[PCI+] Checking CellMapper preload conditions");
            document.cookie = "tilesEnabled: false;";
        }
        if (window.location.href.match(patchedSites.bingMaps)) {
            init(qp);
        } else if (window.location.href.match(patchedSites.cellmapper)) {
