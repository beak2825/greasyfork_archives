// ==UserScript==
// @name         Old Payment Page
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  Old Payment Page For ROBLOX
// @author       Vue2016
// @match        *https://www.roblox.com/upgrades/paymentmethods?ap=48*
// @icon        data:image/png;base64,
// @license MIT
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/534558/Old%20Payment%20Page.user.js
// @updateURL https://update.greasyfork.org/scripts/534558/Old%20Payment%20Page.meta.js
// ==/UserScript==

(function() {
    'use strict';
    document.querySelector("#payment-methods-container").innerHTML += `
   <ol class="progress">
  <li class="is-active" data-step="1">
  </li>
  <li data-step="2">
  </li>
  <li data-step="3" class="progress__last">
  </li>
</ol>`;

     document.querySelector("#payment-methods-container").innerHTML += `
<div class="wrapper">

    <ul>
        <li>
            <p>Supercharge your Purchase!</p>
        </li>
        <li>
            <input type="checkbox" name="check-box" /> <span>Add 1000 ROBUX for $9.95</span>

        </li>
        <li>
            <input type="checkbox" name="check-box" /> <span>Add 2750 ROBUX for $24.95</span>

        </li>
        <li>
            <input type="checkbox" name="check-box" /> <span>Add 6000 ROBUX for $49.95</span>

        </li>

    </ul>

<h2>Builders Club</h2><h1>$5.95</h1>
<h3>Total Due:</h3><h4>$5.95</h4>
<a type="button" class="btn-primary-md btn-fixed-width" id="CreditButton">Continue</a>
</div>`;
})();