// ==UserScript==
// @name                WME Validator BR Localization - Oficial
// @version             0.0.0.2
// @description         Este script traduz o WME Validator para o Brasil, adaptando às regras de edição da comunidade. Você também deve ter o paconte principal, WME Validator, instalado. Essa versão é baseada no trabalho do usuário mikenit (Vidal).
// @match               https://editor-beta.waze.com/*editor/*
// @match               https://www.waze.com/*editor/*
// @grant               none
// @run-at              document-start
// @namespace           https://greasyfork.org/pt-BR/users/14639
// @downloadURL https://update.greasyfork.org/scripts/12010/WME%20Validator%20BR%20Localization%20-%20Oficial.user.js
// @updateURL https://update.greasyfork.org/scripts/12010/WME%20Validator%20BR%20Localization%20-%20Oficial.meta.js
// ==/UserScript==
//
...
window.WME_Validator_Brazil = {
  ...
  "130.enabled": true,
  "130.params": {"template": "${altStreet[#]}:${type}:${street}","regexp": "/[A-Z]{2,3}-\\d{3}:([125]|[12]\\d)|:([125]|[12]\\d):[A-Z]{2,3}-\\d{3}/i"},
  ...
  "130.solution.en": "Change to the local language and Solve the issue",
  ...
  "131.enabled": true,
  "131.params": {"template": "${type}:${altStreet[#]}#${street}:${length}:${roundabout}","regexp": "!/^([1258]|1[06789]|2[01]):|^[3467]:(.*#)?((BRN?|A[CLPM]|AR[ARS]|BA|CE|DF|EMC|ES|GCR|GO|GRP|HRT|ICR|IDT|IRS|BT[CV]|IPR|IT[GNU]|IVA|JGS|LGS|MA|MBC|MOR|MT|MS|[AL]?MG[CT]?|O(S|UR)|P(A|B|DD|E|I|L[NS]|RJ|RD?)|R([JNOR]|PD|CD)|SB|V?RS|R?SC|SCD|SJ|SL[RT]|SM[AR]|SPV?|SE|TBO|TCI|TM?O|TOP|VRD)-\\d{3}([:#]| [A-Z])|:\\d(\\d?:|\\d{2,}:1)|(Ac[\\.e])|[Rr]etorno|SP[ADI]-\\d{3}\\/\\d{3}|Contorno|Linha|Marg(\\.|inal)|PIR-\\d{3}(N|\\/(SP-)?\\d{3})?|AMG-\\d{4})|^4:#(|(Saída|Ponte|Viaduto|Vd\\.) .*):|^15:#Travessia /"},
  ...
  "131.problem.en": "Brazil: This highway has nonstandard name",
  "131.problem": "Esta rodovia tem nome fora do padrão. Leia a",
  "131.solution.en": "Change to the local language and Solve the issue",
  ...
  "132.enabled": true,
  "132.params": {"template": "${deadEndA}:${UturnA}:${deadEndB}:${UturnB}","regexp": "^0:1|0:1$"},
  "132.problemLink": "F:f=299&t=87202",
  "132.title.en": "U-turn",
  "132.title": "Rua com retorno ativado (U-turn)",
  "132.problem.en": "The drivable road has a U-turn enabled",
  "132.problem": "A rua trafegável tem um retorno (U-turn) habilitado",
  "132.solution.en": "Disable U-turn",
  "132.solution": "Desabilite o retorno (U-turn)",
  "133.enabled": true,
  "133.params": {"template": "${city}#${street}#${altStreet[#]}##${type}#","regexp": "/^#(\\S+)( (.+)?)?(?=#).*\\1 ?(.+)?##[3467]#|(\\S+#\\S+\\.? )(.+)(?=#).*\\6##|(\\S+#\\S+\\.? \\S+\\. )(.+)(?=#).*\\8##/i"},
  "133.solutionLink": "W:Como_categorizar_e_nomear_vias",
  "133.title.en": "Alternate street names equal to the street name",
  "133.title": "Nomes iguais no alternativo e no principal",
  "133.problem.en": "Repeated names just increases the search time and the number of equal results",
  "133.problem": "Ter nomes repetidos apenas aumenta o tempo de busca e o número de resultados iguais.",
  "133.solution.en": "Change to the local language and Solve the issue",
  "134.enabled": true,
  "134.params": {"template": "${type}:${street}","regexp": "/^[3467]:Rod/"},
  ...
  "134.problem.en": "Brazil: This highway has nonstandard name",
  "134.problem": "Esta rodovia tem nome fora do padrão. Leia a",
  "134.solution.en": "Change to the local language and Solve the issue",
  "134.solution": "Verifique o nome da rodovia. Falta prefixo no nome principal. Se federal: BR-XXX; se estadual: UF-XXX\nOU;\nSe não for rodovia, troque a categoria da via. Detalhes no", "133.solution": "Apague o(s) nome(s) alternativo igual(is) ou similar(es) ao nome principal da via",
  ...
  "173.params": { "regexp": "/((?!N\\.Sra?\\.)[^\\s]\\.[^\\s0-9-][^\\s0-9\\.])|[^\\s]\\.N\\.Sra?\\.|([0-9][^\\s0-9]+\\.[^0-9-])/" },
  ...
  "193.params": {"regexp": "/((?!N\\.Sra?\\.)[^\\s]\\.[^\\s0-9-][^\\s0-9\\.])|[^\\s]\\.N\\.Sra?\\.|([0-9][^\\s0-9]+\\.[^0-9-])/"},
