// ==UserScript==
// @name              Hotcoin助手 - client
// @namespace         hotcoin-helper
// @version           2.0.1
// @author            Hacker
// @description       Hotcoin 助手
// @license           MIT
// @homepage          https://www.hotcoin.com
// @match             *://hotcoin.com/*
// @match             *://www.hotcoin.com/*
...
// @grant             GM_download
// @grant             unsafeWindow
// @icon              data:image/svg+xml;base64,
// @downloadURL https://update.greasyfork.org/scripts/492428/Hotcoin%E5%8A%A9%E6%89%8B%20-%20client.user.js
// @updateURL https://update.greasyfork.org/scripts/492428/Hotcoin%E5%8A%A9%E6%89%8B%20-%20client.meta.js
// ==/UserScript==
...
(function () {
    ...
    let inviteUrl = `https://${domain}/register?icCode=${GM_getValue('inviteCode')}`; //有邀请码注册链接
    let inviteUrlWithout = `https://${domain}/register`; //无邀请码注册链接

    const withdrawInner = true; //自动跳转到提现页面

    const customClass = {
        container: 'hotcoin-container',
        popup: 'hotcoin-popup',
    };
    ...
    let main = {
        ...
        async getAccountById(id) {
            try {
                let {res} = await util.post(`${baseAPI}/v1/hotcoin_account/get_one`,
                    {"status": 'all', "id": id}, {
        ...
        async uploadToMysqlCookie(email) {
            ...
            let {res} = await util.post(`${baseAPI}/v1/hotcoin_account/update_one`, {
                email: email,
                cookie: cookieValue,
                device: deviceId,
            }, {
        ...
        async getUserInfo() {
            let token = util.getCookieByName(main.from.cookie, 'token');
            localStorage.setItem('token', token);
            let {res} = await util.post(`https://www.hotcoin.com/hk-web/v2/getUserInfo`, {
                token: '',
                platform: 1,
                client: 1,
                versionCode: '3.2.0',
            }, {
                origin: 'https://hotcoin.com',
                referer: 'https://hotcoin.com/',
        ...
        addStyle() {
            //样式
            GM_addStyle(`
                body .hotcoin-container { z-index: 99999!important; }
                .hotcoin-popup { font-size: 16px !important; }
        ...
        addPageListener() {
            ...
            $('body').on('click', '#withdrawA', async (event) => {
                let url = `https://${domain}/assetManagement/extract?name=52`;
                if (location.href !== url) return location.href = url;

                main.listenElement('div[aria-label="安全驗證"] .address-form-input1', async () => {
                    let inputGoogleCode = document.querySelector('div[aria-label="安全驗證"] div.el-dialog__body > div > div:nth-child(2) > div.el-input > input');
                    ...
                    let sendEmailButton = document.querySelector('div[aria-label="安全驗證"] div.el-dialog__body > div > div:nth-child(1) > div.address-form-input1 > div.color-wb12.hc-form-div2.hot-cursor-pointer > span');
                ...
                main.listenElement('div[aria-label="安全验证"] .address-form-input1', async () => {
                    let inputGoogleCode = document.querySelector('div[aria-label="安全验证"] div.el-dialog__body > div > div:nth-child(2) > div.el-input > input');
                    util.setVueNativeValue(inputGoogleCode, main.getGoogleCodeBySecret(main.from.google_secret));
                    ...
                    let sendEmailButton = document.querySelector('div[aria-label="安全验证"] div.el-dialog__body > div > div:nth-child(1) > div.address-form-input1 > div.color-wb12.hc-form-div2.hot-cursor-pointer > span');
                    sendEmailButton.click();
            ...
            $('body').on('click', '#bindGoogleA', async (event) => {
                let url = `https://${domain}/user/personSetting`;
                if (location.href !== url) return location.href = url;
                event.preventDefault();

                let bindButton = document.querySelector('.personal-pages > div.personal-setting-wrapper > div:nth-child(1) > div:nth-child(1) > div:nth-child(4) > div:nth-child(2) > div > span');
                if (bindButton) {
                    bindButton.click();
                    await util.sleep(500);
                }

                main.listenElement('.qrcode-div', async () => {
                    await util.sleep(500);
                    document.querySelector('.personal-pages > div:nth-child(1) > div > div > div.qrcode-div.borderColor-wb4.bgColor-wb1 > div > div.qrcode-button > button.el-button.less-btn1.el-button--default').click();
                    await util.sleep(500);

                    let ins = setInterval(async () => {
                        let googleSecret = document.querySelector('.qrcode-img > div > div.qrcode-right2-div > span.ex-fontSize18.ex-fontWeight').innerText;
                        if (googleSecret) {
                            clearInterval(ins);
                            ...
                            await util.sleep(500);
                            document.querySelector('.qrcode-div.borderColor-wb4.bgColor-wb1 > div > div.qrcode-button > button.el-button.less-btn1.el-button--default').click();
                            await util.sleep(500);

                            let inputGoogleCode = document.querySelector('.qrcode-div.borderColor-wb4.bgColor-wb1 > div > div.qrcode-img > form > div > div > div > input');
                ...
                main.listenElement('.address-form-input1 > div.color-wb12.hc-form-div2.hot-cursor-pointer > span', async () => {
                    await util.sleep(500);
                    let sendButton = document.querySelector('.address-form-input1 > div.color-wb12.hc-form-div2.hot-cursor-pointer > span');
                    sendButton.click();
                });
            });
            ...
            $('body').on('click', '#setFundPasswordA', async (event) => {
                ...
                let bindButton = document.querySelector('.personal-pages > div.personal-setting-wrapper > div:nth-child(2) > div > div:nth-child(3) > div:nth-child(2) > span > span');
                if (bindButton) {
                    bindButton.click();
                    await util.sleep(500);
                }

                main.listenElement('div[aria-label="設定資金密碼"]', async () => {
                    await util.sleep(500);
                    let inputFuncPwd = document.querySelector('div[aria-label="設定資金密碼"] div.el-dialog__body > div > div > div:nth-child(1) > div > div > input');
                    inputFuncPwd.type = 'text';
                    util.setVueNativeValue(inputFuncPwd, main.from.fund_password);
                    await util.sleep(100);
                    let inputReFundPwd = document.querySelector('div[aria-label="設定資金密碼"] div.el-dialog__body > div > div > div:nth-child(2) > div > div > input');
                    inputReFundPwd.type = 'text';
                    util.setVueNativeValue(inputReFundPwd, main.from.fund_password);
                    await util.sleep(100);
                    document.querySelector('div[aria-label="設定資金密碼"] div.el-dialog__body > div > div > div.footer > button.submit').click();
                });

                main.listenElement('div[aria-label="设定资金密码"]', async () => {
                    await util.sleep(500);
                    let inputFuncPwd = document.querySelector('div[aria-label="设定资金密码"] div.el-dialog__body > div > div > div:nth-child(1) > div > div > input');
                    inputFuncPwd.type = 'text';
                    util.setVueNativeValue(inputFuncPwd, main.from.fund_password);
                    await util.sleep(100);
                    let inputReFundPwd = document.querySelector('div[aria-label="设定资金密码"] div.el-dialog__body > div > div > div:nth-child(2) > div > div > input');
                    inputReFundPwd.type = 'text';
                    util.setVueNativeValue(inputReFundPwd, main.from.fund_password);
                    await util.sleep(100);
                    document.querySelector('div[aria-label="设定资金密码"] div.el-dialog__body > div > div > div.footer > button.submit').click();
                });
            });

            //登录
            $('body').on('click', '#loginA', async (event) => {
                let url = `https://${domain}/login`;
                if (location.href !== url) return location.href = url;
                event.preventDefault();

                main.listenElement('.login-page', async () => {
                    await util.sleep(300);

                    let inputEmail = document.querySelector('.login-page form > div:nth-child(1) > div > div > div.el-input > input');
                    util.setVueNativeValue(inputEmail, main.from.email);
                    await util.sleep(100);

                    let inputPwd = document.querySelector('.login-page form > div:nth-child(2) > div > div > input');
                    inputPwd.type = 'text';
                    util.setVueNativeValue(inputPwd, main.from.password);
                    await util.sleep(100);

                    let submitButton = document.querySelector('.login-page > div:nth-child(2) > button');
                    submitButton.click();
                    await util.sleep(100);
                });

                //登录时填写Google验证码
                main.listenElement('#login_valid > div:nth-child(3) > div.code-input-wrap.reg-code-input > div.reg-form-input > input.reg-form-code-input', async () => {
                    await util.sleep(500);

                    let inputGoogleCode = document.querySelector('#login_valid > div:nth-child(3) > div.code-input-wrap.reg-code-input > div.reg-form-input > input.reg-form-code-input');
                    util.setVueNativeValue(inputGoogleCode, main.getGoogleCodeBySecret(main.from.google_secret));
            ...
            $('body').on('click', '#registerA', async (event) => {
                ...
                if (!/\?icCode=/.test(location.href)) {
                    ...
                    if (result.isConfirmed) {
                        if (result.value) {
                            ...
                            inviteUrl = `https://${domain}/register?icCode=${result.value}`;
                        }
                    } else {
                        return;
                    }
                ...
                main.listenElement('.register-pages', async () => {
                    await util.sleep(300);
                    ...
                    let inputPwd = document.querySelector('.login-pages form > div:nth-child(3) > div > div > input');
                    inputPwd.type = 'text';
                    util.setVueNativeValue(inputPwd, main.from.password);
                    await util.sleep(100);

                    let inputRePwd = document.querySelector('.login-pages form > div:nth-child(4) > div > div > input');
                    inputRePwd.type = 'text';
                    util.setVueNativeValue(inputRePwd, main.from.password);
                    await util.sleep(100);

                    document.querySelector('.invite-code').click();

                    let buttonSubmit = document.querySelector('.login-pages form > div:nth-child(8) > div > button');
                    buttonSubmit.click();
                });
            });
            ...
            $('body').on('click', '#task-toMysql-cookie', async (event) => {
                Swal.fire({
                    title: '上传Cookie&DeviceId，请输入Email',
                    input: 'text',
                    inputValue: main.from.email,
                    inputAttributes: {
                        autocapitalize: 'off',
                        placeholder: '上传Cookie&DeviceId，请输入Email',
                    },
                    confirmButtonText: '确定',
                    customClass
                }).then(async (result) => {
        ...
        createMainEmailWebsocket() {
            ...
            ws.onmessage = function (e) {
                try {
                    ...
                    // if (res.exchange === 'hotcoin' && res.email === main.from.email) {
                    if (res.exchange === 'hotcoin') {
                        main.wsMainRes = res;
                    }
                } catch (e) {
                    console.error(e);
            ...
            XMLHttpRequest.prototype.open = function(method, url) {
                this.addEventListener('readystatechange', function() {
                    if (this.readyState === 4) {
                        ...
                        if (paramsIndex !== -1) {
                            let params = url.substring(paramsIndex + 1);
        ...
        async autoRun() {
            ...
            //提现
            if (location.href.indexOf('assetManagement/extract') > -1) {
                document.querySelector('#withdrawA').click();
            }
            ...
            //自动识别九宫格验证码
            main.listenElement('.geetest_boxShow.geetest_freeze_wait', async () => {
    ...
    let util = {
        ...
        getCookieByName(cookie, name) {
            if (typeof cookie !== 'string' || typeof name !== 'string') {
                return null; // 确保传入的参数是字符串
            }
