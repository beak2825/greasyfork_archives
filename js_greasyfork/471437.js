// ==UserScript==
// @name         Autoverify-mod-ck
// @license      No License
// @namespace    https://www.like996.icu:1205/
// @version      20231226
// @description  autoverify captcha
// @author       crab
// @match        http://*/*
// @match        https://*/*
// @connect      like996.icu
// @connect      https://like996.icu:1205
// @connect      *
...
// @nocompat     Chrome
// @downloadURL https://update.greasyfork.org/scripts/471437/Autoverify-mod-ck.user.js
// @updateURL https://update.greasyfork.org/scripts/471437/Autoverify-mod-ck.meta.js
// ==/UserScript==
...
class CaptchaWrite {
    ...
    // 打开帮助页面
    openHelp() {
        return GM_openInTab("https://www.like996.icu:1205/help.html", 'active');
    }
                    ...
                    if (img && IdentifyResult) {
                        ...
                        $("input").each(function () {
                            $(this).click(function () {
                                ...
                                ////////console.log(AddRule);
                                that.delCapFoowwLocalStorage(window.location.href);
                            });
                        });
                    }
                });
    ...
    CalcPickUp() {
        ...
        $("canvas,img,input[type='image']").each(function () {
            $(this).on("contextmenu mousedown", function (e) {// 为了避免某些hook的拦截
                ...
                that.Hint('等待识别')
                IdentifyResult = that.CalcImgPathToResult(img, function ManualRule(img, IdentifyResult) {
                    if (img && IdentifyResult) {
                        console.log('记录信息' + img + IdentifyResult);
                        AddRule['img'] = img;
                        $("img").each(function () {
        ...
        that.sendPostMessage("CalcPickUp")
    }
        ...
        if (tagName === "img") {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (tagName === "canvas") {
                    ...
                    if ((that.checkClassName(curEl, "canvas_bg") || that.checkClassName(curEl.parentNode, "captcha_basic_bg")) || (position != "absolute" && (eleWidth > 300 && eleWidth >= eleHeight * 1.2 && eleWidth <= eleHeight * 5))) {
                        ruleCache['big_image'] = that.Aimed(el);
                        that.Hint('您已成功选择大图片。', 5000);
                        that.setCapFoowwLocalStorage("slidePathCache", ruleCache, new Date().getTime() + 1000 * 60);
                        that.checkTargetNeedZIndex(ruleCache, curEl);
                    ...
                    } else if (that.checkClassName(curEl, "slide")) {
                        ruleCache['small_image'] = that.Aimed(el);
                        that.Hint('您已成功选择小图片。', 5000);
                        that.setCapFoowwLocalStorage("slidePathCache", ruleCache, new Date().getTime() + 1000 * 60);
                        that.checkTargetNeedZIndex(ruleCache, curEl);
    ...
    checkSlideCaptcha(slideCache) {
        ...
        const check = async () => {
            ...
            var img_json = {"target_img": small_base64,'bg_img':big_base64}
            var imgjson_str = JSON.stringify(img_json);
            var postData_dddd = {
              img: btoa(imgjson_str),
              ocr_type: 4
            }
            //console.log("postData: "+postData_dddd);

            //that.Identify_Crab(null, postData, function Slide(data) {
            that.Identify_Crab(null, postData_dddd, function Slide(data) {
                //console.log("等待滑动距离：" + "20")
    ...
    checkSlideBehaviorCaptcha(slideCache) {
        ...
        const check = async () => {
            ...
            //that.Identify_Crab_origin(null, postData, function Slide(data) {
            that.Identify_Crab(null, postData, function Slide(data) {
                console.log("等待滑动距离：" + data.data)
    ...
    moveSideCaptcha(bigImg,smallImg, moveItem, data) {
        ...
        // 两个元素距离屏幕左侧的差距(用于后期取不到滑动距离切换参照物的差值)
        //let eledifference=moveItem.getBoundingClientRect().x-targetImg.getBoundingClientRect().x;
        //let eledifference = moveItem.getBoundingClientRect().x - targetImg.getBoundingClientRect().x;
    ...
    Query(Json, callback) {
        var that = this;
        var QueryRule = '';
        var LocalStorageData = this.getCapFoowwLocalStorage(Json.method + "_" + Json.data.href);
        if (Json.method == 'captchaHostAdd') {
            //that.delCapFoowwLocalStorage("captchaHostQuery_" + Json.data.host);
            that.delCapFoowwLocalStorage("captchaHostQuery_" + Json.data.href);
        ...
        GM_xmlhttpRequest({
            ...
            onload: obj => {
                ...
                ////console.log('captchaHostQuery_查询返回的结果 ' + data.data[0]);
                //////////if (data.description != undefined) {
                //////////    that.Hint(data.description)
                //////////}
                QueryRule = data;
                that.setCapFoowwLocalStorage(Json.method + "_" + Json.data.path, data, new Date().getTime() + 1000 * 60)
                if (callback != null) {
                    callback(QueryRule);
                }
    ...
    Start() {
        ...
        that.Query({
            ...
            ///////////////////////////////////////console.log("captchaHostQuery查询返回的规则: " + JSON.stringify(Rule))
            if (Rule.code == 531 || Rule.code == 532) {
                ...
                ////////////////////////////////////////console.log(writeResultIntervals);
    ...
    WriteResultsInterval() {
        for (var i = 0; i < writeResultIntervals.length; i++) {
            ...
            if (!ocrType) {ocrType = writeResultIntervals[i].ocr_type;}
            //console.log(ocrType);
            if (!ocrType || ocrType == 1 || ocrType == 10) {
                // 英数验证码
                var imgAddr = writeResultIntervals[i].img;
                if (imgAddr && imgAddr.indexOf('this.src') !=-1) {imgAddr = writeResultIntervals[i].img.replaceAll("'",'"').replace('"&"',"'&'").replace('"?"',"'?'");}
                //console.log(imgAddr);
                ...
                try {
                    if (this.getCapFoowwLocalStorage("err_" + writeResultIntervals[i].img) == null) {// 写入识别规则之前，先判断她是否有错误
                        if (ocrType == 10) {this.CalcRuleBindingElement(imgAddr, inputAddr);} else {this.RuleBindingElement(imgAddr, inputAddr);}
                    }
                } catch (e) {
                ...
                var small_image = writeResultIntervals[i].small_image;
                if (document.querySelector(big_image) == null) {
                    continue;
                }
                if (document.querySelector(small_image) == null) {
                    continue;
                }
    ...
    Identify_Crab(img, postData, callback) {
        ...
        postData["version"] = "5.1";
        var duration = postData["small_image_width"]
        that.setCapFoowwLocalStorage("识别结果缓存:" + postDataHash, "识别中..", new Date().getTime() + (9999999 * 9999999));//同一个验证码只识别一次
        ////////////////////////////////////////console.log("调用接口提交的data: " + postData);
        var url = that.getCaptchaServerUrl() + "ocr/b64/json";
        if (postData['ocr_type'] == 4) {url = that.getCaptchaServerUrl() + "slide/match/b64/json"}
        if (postData['ocr_type'] == 10) {url = that.getCaptchaServerUrl() + "calc/b64/json"}
        ///////////////////////////////////////////console.log(url);
        //var url = that.getCaptchaServerUrl() + "ocr/b64/json";
        //var url = that.getCaptchaServerUrl() + "/hello";
        console.log("验证码变动，开始识别");
        var imgb64 = postData['img'];
        //console.log(imgb64);
        GM_xmlhttpRequest({
            url: url,
            method: 'POST',
            ...
            onload: obj => {
                ...
                /////////////////////////////////////console.log("调用接口获取的结果: " + Results);
                if (postData['ocr_type'] == 4) {Results = JSON.stringify(data.result.target);data['data'] = data['result']['target'][0];data["description"] = "识别完成";}
                if (postData['ocr_type'] == 5) {data['data'] = duration + 20 ;data['result'] = 'slide';data['msg'] = null;data["description"] = "识别完成";}
                //console.log("data: " + data.data);
                //var Results_dddd = Results;
                //console.log('hello的返回' + JSON.stringify(data.result));
                //if (postData['ocr_type'] == 4) {console.log('hello的返回' + JSON.stringify(data.result.target));}
                //if (!data.valid) {
                if (data.msg) {
                    //if (data.description != undefined) {
                        that.Hint('识别请求发生错误： ' + data.msg, 5000);
                    //}
                    that.setCapFoowwLocalStorage("识别结果缓存:" + postDataHash, data.result, new Date().getTime() + (9999999 * 9999999))

                } else {

                    that.setCapFoowwLocalStorage("识别结果缓存:" + postDataHash, data.result, new Date().getTime() + (9999999 * 9999999))
                    //var Results = JSON.stringify(data.result);
                    //if (postData['ocr_type'] == 4) {Results = JSON.stringify(data.result.target);data['data'] = 20;}
                    //var Results_dd = Results
                    //console.log("Results1111111: " + Results );
                    if (callback != null) {
                        if (callback.name == 'Slide') {
                            ...
                            if (Results.length < 0) {
                                ...
                                that.Hint(data.msg, data.showTime)
                            } else {
                            ...
                            if (callback.name == 'WriteRule') {
                                ...
                                //callback(JSON.stringify(data.result.target))
                            } else if (callback.name == 'ManualRule') {
                                // 手动添加规则
                                //callback(img, data.data);
                                callback(img, data.result);
                            }
                        }
                    }
                }
        ...
        //console.log("Results2222222: " + Results_dddd );
        //return Results_dddd;
        return Results;
    }
    ...
    //根据规则提取验证码base64并识别
    async CalcImgPathToResult(imgElement, callback) {
        var that = this;
        var imgObj = $(imgElement);
        if (!imgObj.is(":visible")) {
            console.log("验证码不可见，本次不识别");
        ...
        var postData = {img: imgBase64, ocr_type: 10};
        that.Identify_Crab(imgElement, postData, callback);
    }
    ...
    //绑定规则到元素，并尝试识别
    CalcRuleBindingElement(img, input) {
        var that = this;
        //创建一个触发操作
        if (document.querySelector(img) == null) {
            return;
        ...
        document.querySelector(img).onload = function () {
            that.CalcRuleBindingElement(img, input)
        }

        this.CalcImgPathToResult(img, function WriteRule(vcode) {
            that.WriteImgCodeResult(vcode, input)
        })
    ...
    WriteImgCodeResult(ImgCodeResult, WriteInput) {
        ...
        if (typeof (InputEvent) !== 'undefined') {
            ...
            if (that.FireForReact(WriteInput, "change")) {
                return;
            }
            let eventNames = ["keypress", "keydown", "input", "keydown", "keyup", "select"];
    ...
    AutoRules() {
        ...
        $("canvas,img,input[type='image'],div").each(function () {
            ...
            for (let i = 0; i < checkList.length; i++) {
                if (checkList[i].toString().toLowerCase().indexOf("logo") != -1 || checkList[i].toString().toLowerCase().indexOf("btn") != -1 ) {
                    //如果元素内包含logo字符串，则直接跳过
                    checkList.splice(i, 1);
                    i--; // 因为删除了一个元素，所以需要将 i 减 1，以便继续遍历
                    //return true;
                    //break;
                }
            }

            checkList = checkList.filter(item => item.toString().toLowerCase().indexOf("logo") == -1 &&  item.toString().toLowerCase().indexOf("btnlogin") == -1 ) ;
            ...
            let imgRules = "code,captcha,yzm,check,random,veri,vcodeimg,验证码,看不清,换一张,login,点击,verify,yanzhengma,idpicture".split(",");
            let isHave = false;
                ...
                for (let j = 0; j < imgRules.length; j++) {
                    if (checkList[i] != undefined) {
                        if (elemAttributeData.indexOf(imgRules[j]) != -1
                            && ((imgTagName == "img" && !isInvalid) || imgTagName != "img") && imgWidth > 29 && imgWidth < 161
        ...
        //captchaMap = captchaMap.filter(item => !item.img.toLowerCase().includes("btn")) ;
        ...
        captchaMap.forEach((item) => {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                if (inputTags.length) {
                    ...
                    if (!type || (type === "text" && inputWidth > 29)) {
                        // 给目标元素添加边框，证明自动规则选中得
        ...
        MatchList = MatchList.filter(item => JSON.stringify(item).toLowerCase().indexOf("logo") == -1 &&  JSON.stringify(item).toLowerCase().indexOf("btn") == -1) ;
        return MatchList;
    }
    ...
    Aimed(Element) {
        ...
        // 如果有name且只有一个，则直接返回
        var selectElement = that.getElementName(Element); var test = that.getElementId
        if (selectElement != null) {
            return selectElement;
        }

        // 如果有id且只有一个，则直接返回
        //var selectElement = that.getElementId(Element);
        //if (selectElement != null) {
        //    return selectElement;
        //}
            ...
            if (key == "src" && value.indexOf("data:image") != 0) {
                ...
                if (/\d/.test(value)) {
                    ...
                    if (lastSlashIndex !== -1) {
                        ...
                        if (truncateURL.indexOf("http") != 1) {
                            truncateURL = "http:" + truncateURL;
                        }
                        const url = new URL(truncateURL);
                        if (url.pathname != "/") {
                            value = truncateURL;
                        }
                    }
    ...
    captchaHostBad(img, input) {
        ...
        this.delCapFoowwLocalStorage("captchaHostQuery_" + window.location.href);
        this.Query({
                ...
                list.addEventListener("click", e => {
                    ...
                    if(menuList[menuId].type=="button"){
                        ...
                        if (!checkbox.checked) {
                            ...
                            crabCaptcha.Hint(menuList[menuId].hintOpen);
                            //let doWork = targetElem.attr("doWork");
