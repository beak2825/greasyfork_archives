...
// @version      1
// @author       white space
// @description  press [q] for heal, it clowns somewhat easily, press [n] for triple mills (this is part 2 of Go@d m@Cr0 prt 1)
// @match        *://moomoo.io/*
// @match        *://*.moomoo.io/*
...
// @downloadURL https://update.greasyfork.org/scripts/479482/Go%40d%20m%40Cr0%202.user.js
// @updateURL https://update.greasyfork.org/scripts/479482/Go%40d%20m%40Cr0%202.meta.js
// ==/UserScript==
(() => {
  ...
  document.addEventListener("keydown", event => {
    if (event.keyCode === 78 && document.activeElement.id.toLowerCase() !== 'chatbox') {
      autoMill = !autoMill;
      chat(`white's Ass Mill ${autoMill ? 'on' : 'off'}`);
    }
  });
  ...
  WebSocket.prototype.send = function (event) {
    ...
    // Event listener for [q] key to toggle autoHeal
  document.addEventListener("keydown", event => {
    if (event.keyCode === 81 && document.activeElement.id.toLowerCase() !== 'chatbox') {
      autoHeal = !autoHeal;
      chat(`white's Ass Heal ${autoHeal ? 'on' : 'off'}`);
    }
  });
})();