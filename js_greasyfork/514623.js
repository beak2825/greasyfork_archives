// ==UserScript==
// @name         哔哩哔哩 - 屏蔽指定内容 - 改1
// @namespace    https://greasyfork.org/zh-CN/users/1387998-slimee
// @homepage     https://greasyfork.org/zh-CN/users/1387998-slimee
// @version      5.1.4
// @description  实现对评论区中无意义的At或分别按用户名、关键字或正则表达式对视频(或直播间/相薄)和评论(或回复)进行屏蔽; 将鼠标移至网页右下角弹出悬浮按钮
// @author       pana,ztelliot,slimeE
...
// @noframes
// @note         更新记录:
// @note         ver.5.1.4  更新主页规则。
// @note         ver.5.1.3  增加屏蔽条目的输出信息，正则匹配前会将'...'替换成'.{0,10}',原程序中因为有逗号的原因无法限定长度，可以用形如'x...y'屏蔽'xy'中间有空格的情况。
// @note         ver.5.1.2  修正隐藏评论回复框。
// @note         ver.5.1.1  添加隐藏评论回复框选项。
// @note         ver.5.1.0  添加搜索页面，视频右边栏，个人动态页面的匹配。添加用户名正则匹配选项，添加正则忽略大小写选项，修复一些用户名提取不正常的问题。
...
// @downloadURL https://update.greasyfork.org/scripts/514623/%E5%93%94%E5%93%A9%E5%93%94%E5%93%A9%20-%20%E5%B1%8F%E8%94%BD%E6%8C%87%E5%AE%9A%E5%86%85%E5%AE%B9%20-%20%E6%94%B91.user.js
// @updateURL https://update.greasyfork.org/scripts/514623/%E5%93%94%E5%93%A9%E5%93%94%E5%93%A9%20-%20%E5%B1%8F%E8%94%BD%E6%8C%87%E5%AE%9A%E5%86%85%E5%AE%B9%20-%20%E6%94%B91.meta.js
// ==/UserScript==
...
(async function () {
  ...
  `;
    const handler = [
        ...
        {
            index: 'div.mb_x40.col_xl_1_7.col_md_2.col_xs_1_5.col_3',
            user: '.bili-video-card__info--author',
            text: '.bili-video-card__info--tit',
        },
        {
            index: 'body > div.home-redesign-base > div > div > div.default-content > div > div.minimal-home-tab-cards > a',
            user: '.name',
            text: '.title',
        },
        {
            index: '.__scale-wrap.bili-video-card__wrap',
            user: '.bili-video-card__info--author',
            text: '.bili-video-card__info--tit',
        },
        {
        ...
        {
            index: '.bili-dyn-item__main',
            user: 'span',
            text: '.bili-dyn-item__body',
            url: 't.bilibili.com',
        },
        {
            clientInformation: 0,
    ...
    function decideText(
        ...
        let ignoreCase = bilibiliConfig.regexIgnoreCase ? 'i' : '';
        ...
        if (isDecide) {
            if (bilibiliConfig.keywordEnable) {
                for (let k of ruleList) {
                    if (k) {
                        if (typeof k === 'string' && textValue.includes(k)) {
                            printDecideInfo(textValue, k);
                            return true;
                        } else {
                            try {
                                let kw = limitKwLong(k);
                                if (textValue.match(new RegExp(kw, ignoreCase))) {
                                    printDecideInfo(textValue, kw);
                                    return true;
                                }
                            } catch (e) {
                ...
                for (let i of bilibiliConfig.commentArray) {
                    if (i) {
                        if (typeof i === 'string') {
                            ...
                            try {
                                let kw = limitKwLong(i);
                                if (textValue.match(new RegExp(kw, ignoreCase))) {
                                    printDecideInfo(textValue, kw);
                                    return true;
                                } else if (sourceText.match(new RegExp(kw, ignoreCase))) {
                                    printDecideInfo(sourceText, kw);
                                    return true;
                                }
                            } catch (e) {
                                console.error('存在错误的正则表达式: ', e);
            ...
            if (isDecideDynamicTitle) {
                if (bilibiliConfig.keywordEnable) {
                    for (let o of ruleList) {
                        if (o) {
                            if (typeof o === 'string' && dynamicVideo.includes(o)) {
                                printDecideInfo(dynamicVideo, o);
                                dynamicStatus = true;
                                break;
                            } else {
                                try {
                                    let kw = limitKwLong(o);
                                    if (dynamicVideo.match(new RegExp(kw, ignoreCase))) {
                                        printDecideInfo(dynamicVideo, kw);
                                        dynamicStatus = true;
                                        break;
                                    }
                                } catch (e) {
                    ...
                    for (const q of bilibiliConfig.commentArray) {
                        if (q) {
                            if (typeof q === 'string') {
                                if (dynamic.content.includes(q)) {
                                    if (dynamic.sourceContent) {
                                        if (dynamic.sourceContent.includes(q)) {
                                            pirntDecideInfo(dynamic.sourceContent, q);
                                            dynamicStatus = true;
                                            break;
                                        } else if (/\[.*\]/i.test(q)) {
                                            printDecideInfo(dynamic.sourceContent, q);
                                            dynamicStatus = true;
                                            break;
                                        }
                                    } else {
                                ...
                                try {
                                    let kw = limitKwLong(q);
                                    if (dynamic.content.match(new RegExp(kw, ignoreCase))) {
                                        printDecideInfo(dynamic.content, kw);
                                        dynamicStatus = true;
                                        break;
                                    } else if (dynamic.sourceContent.match(new RegExp(kw, ignoreCase))) {
                                        printDecideInfo(dynamic.sourceContent, kw);
                                        dynamicStatus = true;
                                        break;
                                    }
                                } catch (e) {
                    ...
                    for (const r of bilibiliConfig.commentArray) {
                        if (r) {
                            if (typeof r === 'string') {
                                if (repost.content.includes(r)) {
                                    if (repost.sourceContent) {
                                        if (repost.sourceContent.includes(r)) {
                                            printDecideInfo(repost.sourceContent, r);
                                            dynamicStatus = true;
                                            break;
                                        } else if (/\[.*\]/i.test(r)) {
                                            printDecideInfo(repost.sourceContent, r);
                                ...
                                try {
                                    let kw = limitKwLong(r);
                                    if (repost.content.match(new RegExp(kw, ignoreCase))) {
                                        printDecideInfo(repost.content, kw);
                                        dynamicStatus = true;
                                        break;
                                    } else if (repost.sourceContent.match(new RegExp(kw, ignoreCase))) {
                                        printDecideInfo(repost.sourceContent, kw);
                                        dynamicStatus = true;
                                        break;
                                    }
                                } catch (e) {
    ...
    function hideHandler(itemNode, username, textValue, method = 0, type = {}, remove = false) {
        if (username) {
            if (typeof username === 'object') {
                username = username.textContent.replaceAll('\n', "").trim();
            }
        ...
        } else if (bilibiliConfig.usernameRegexEnabled ? decideText(username, bilibiliConfig.usernameArray, isComment, isLive, isPic, sourceText, isMessageReply, dynamicVideo, dynamic, repost, onlyAt) : decideUsername(username, isComment, isLive, isPic, isMessageReply, trueLove, repostUser)) {
            hideStatus = true;
        } else if (decideText(textValue, bilibiliConfig.keywordArray, isComment, isLive, isPic, sourceText, isMessageReply, dynamicVideo, dynamic, repost, onlyAt)) {
            hideStatus = true;
        } else {
            hideStatus = false;
        }
    ...
    function hideEvent() {
        handler.forEach(item => {
            ...
            if (url) {
                ...
                status = OLD_URL.indexOf('www.bilibili.com') !== -1 || OLD_URL.indexOf('search.bilibili.com') !== -1;
            }
            if (status) {
    ...
    function hideComment() {
        ...
        //顶部评论框
        try {
            let n = document.querySelector("#commentapp > bili-comments").shadowRoot.querySelector("#header");
            Block_Obj.fn.hideOperation(n, bilibiliConfig.hideReplyBox, 2);
        } catch (error) {

        }
        //底部评论框
        try {
            let n = document.querySelector("#commentapp > bili-comments").shadowRoot.querySelector("#header > bili-comments-header-renderer").shadowRoot.querySelector("div.bili-comments-bottom-fixed-wrapper > div > bili-comment-box");
            Block_Obj.fn.hideOperation(n, bilibiliConfig.hideReplyBox, 2);
        } catch (error) {
        ...
        //弹幕输入框
        try {
            let n = document.querySelector("#bilibili-player > div > div > div.bpx-player-primary-area > div.bpx-player-sending-area > div > div.bpx-player-dm-root > div.bpx-player-video-inputbar.disabled");
            Block_Obj.fn.hideOperation(n, bilibiliConfig.hideReplyBox);
        } catch (error) {
    ...
    function printDecideInfo(textValue, k) {
        if (bilibiliConfig.maskInfo) {
            console.log("%c屏蔽字符串: ", "color: #ff735a; font - weight: bold;", textValue);
            console.log("%c命中关键词: %c" + k, "color: #7DDA58; font - weight: bold;", "color: #FE9900;");
        }
    }
    function limitKwLong(k) {
        let kw = k.replace('...', '.{0,10}');
        return kw;
    }
})();
