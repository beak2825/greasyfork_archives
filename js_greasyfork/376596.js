...
// @downloadURL https://update.greasyfork.org/scripts/376596/HWM_TransferSearch.user.js
// @updateURL https://update.greasyfork.org/scripts/376596/HWM_TransferSearch.meta.js
// ==/UserScript==
...
(function(){
  ...
  function search( id, elem, type ) {
    ...
    request(url+'pl_transfers.php?page=9999&id=' + id, search_1_last_page, type);
  }

  function search_1_last_page(objXMLHttpReq, type){
...
/*!*/	  reg = new RegExp( '[0-9]{2}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}: .* ["\'].*' + rep( search_str ) + '.*["\']' );
  ...
  function startSearch( pg, id, reg, lastPg, elem, type, search_str ) {
    if ( $('stop').value != '1' && pg <= lastPg ) {
      var args = {pg:pg, id:id, reg:reg, lastPg:lastPg, elem:elem, type:type, search_str:search_str};
  ...
  function search_2_pages(objXMLHttpReq, args){
    if (objXMLHttpReq.readyState == 4 && objXMLHttpReq.status == 200) {
      ...
      $('percent').innerHTML = ( Math.round( $('viewed').innerHTML * 100 / args.lastPg ) );
      startSearch( args.pg+1, args.id, args.reg, args.lastPg, args.elem, args.type, args.search_str );
    }
  }
  ...
  function create_header(elem){
    ...
    div.innerHTML =
      ...
      '<tr><td>Поиск штрафов:</td><td><div id="HWM_transfer_search_checkbox_label">Блоки / Разблоки</div></td>'+
