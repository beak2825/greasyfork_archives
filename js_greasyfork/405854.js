// ==UserScript==
// @name         iTEST助手-坤少
// @namespace    http://sloving.gitee.io/
// @version      1.0.0
// @description  私人用途
// @author       SlovinG
// @match        https://itestcloud.unipus.cn/itest-api/itest/s/answer/**
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/405854/iTEST%E5%8A%A9%E6%89%8B-%E5%9D%A4%E5%B0%91.user.js
// @updateURL https://update.greasyfork.org/scripts/405854/iTEST%E5%8A%A9%E6%89%8B-%E5%9D%A4%E5%B0%91.meta.js
// ==/UserScript==

(function () {
    'use strict';

    document.write("<script type='text/javascript' src='http://www.veryhuo.com/uploads/Common/js/jQuery.md5.js'></script>");//引入jquery的md5加密组件

    const trans_appid = '20200603000484959'; //百度翻译开放平台上申请的APPID
    const trans_key = 'Fz6UttgcMXATCddMMZW1'; //百度翻译开放平台上申请的密钥
    ...
    function writing() {
        ...
        $.each($('.itest-section'), function (index, value) {
            if ($(value).attr('part1') == "Part I Writing    ") {
                $.each($(value).children('.itest-ques-set'), function (index, value) {
                    $(value).find('.word-count').prepend("<br><button class='writing_translate_btn'>中翻英</button>");
        ...
        $(document).on("click", ".writing_translate_btn", function () {
            ...
            var textarea_trans = $(this).parent().parent().parent().find('textarea');
            $.ajax({
                type: "post",
                async: false,
    ...
    function translate_option() {
        ...
        //为听力添加按钮
        $.each($('.itest-section'), function (index, value) {
            if ($(value).attr('part1') == "Part II Listening Co...") {
                $.each($(value).children('.itest-ques-set'), function (index, value) {
                    $(value).find('.css-danxuan').prepend("<button class='option_translate_btn'>翻译选项</button>");
                })
            }
        })
        ...
        $(document).on("click", ".option_translate_btn", function () {
            $(this).removeClass('option_translate_btn').addClass('option_hide_btn').html("清空翻译")
            var context_list = "";
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(options, function (index, value) {
                        $(value).append(res_word[index * 2 + 1])
                    })
                },
                error: function () {
                    alert('百度翻译接口测试失败，请检测appid和key！');
        ...
        $(document).on("click", ".option_hide_btn", function () {
            $(this).removeClass('option_hide_btn').addClass('option_translate_btn').html("翻译选项")
    ...
    function allow_multiple_play() {
        ...
        $(".itest-hear-reslist").each(function () {
            var option_musics = $(this).text().match(/(http.*?\.mp3)/g);
            if (option_musics) {
    ...
    function select_word() {
        //为选词填空添加按钮
        $.each($('.itest-section'), function (index, value) {
            if ($(value).attr('part1') == "Part III Reading Com..." && $(value).attr('part2') == "Section A    ") {
                $.each($(value).children('.itest-ques-set'), function (index, value) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.error_code == "54003") {
                        ...
                        data.trans_result.forEach(function (e) {
                            res_word.push("<label style='color: blue;'>" + e.dst + "</label>")
                        })
                    } else if (data.error_code) {
        ...
        $(document).on("click", ".select_word_content_btn", function () {
            ...
            var re = new RegExp("\t", "g");
            var re2 = new RegExp("\n", "g");
            $(this).next().children('div[style="text-align:justify"]').each(function () {
            ...
            var trans_str = trans_appid + context_list + trans_salt + trans_key;
            var trans_sign = $.md5(trans_str);
            var options = $(this).next().find('div[style="text-align:justify"]');
            $.ajax({
                type: "post",
                async: false,
                url: "https://api.fanyi.baidu.com/api/trans/vip/translate",
    ...
    function long_reading() {
        //为长篇阅读添加翻译按钮
        $.each($('.itest-section'), function (index, value) {
            if ($(value).attr('part1') == "Part III Reading Com..." && $(value).attr('part2') == "Section B    ") {
                $.each($(value).children('.itest-ques-set'), function (index, value) {
                    $(value).prepend("<br><button class='long_reading_translate_btn'>翻译全文</button>");
                    $(value).find('.con-right').before("<br><button class='long_reading_option_btn'>翻译选项</button>");
                })
            }
        })
        ...
        $(document).on("click", ".long_reading_translate_btn", function () {
            $(this).removeClass('long_reading_translate_btn').addClass('long_reading_hide_btn').html("清空翻译");
            var context_list = [];
            $(this).next().find("div[style='text-align:justify']").each(function () {
                //将偶数的br替换为hr
                $(this).children('br:even').replaceWith(' ')
                context_list = $(this).html().split('<br>');
                var re = new RegExp("\n", "g");
                var re1 = new RegExp("&nbsp;", "g");
                $.each(context_list, function (index, value) {
                    context_list[index] = value.replace(re, " ").replace(re1, " ");
                })
        ...
        $(document).on("click", ".long_reading_hide_btn", function () {
            $(this).removeClass('long_reading_hide_btn').addClass('long_reading_translate_btn').html("翻译全文");
            $(this).next().find("div[style='text-align:justify']").each(function () {
        ...
        $(document).on("click", ".long_reading_option_btn", function () {
            $(this).removeClass('long_reading_option_btn').addClass('long_reading_option_hide_btn').html("清空翻译")
            var context_list = "";
        ...
        $(document).on("click", ".long_reading_option_hide_btn", function () {
            $(this).removeClass('long_reading_option_hide_btn').addClass('long_reading_option_btn').html("翻译选项")
    ...
    function reading_comprehension() {
        //为阅读理解添加翻译按钮
        $.each($('.itest-section'), function (index, value) {
            if ($(value).attr('part1') == "Part III Reading Com..." && $(value).attr('part2') == "Section C    ") {
                $.each($(value).children('.itest-ques-set'), function (index, value) {
                    $(value).prepend("<br><button class='reading_comprehension_translate_btn'>翻译全文</button>");
                    $(value).find('.con-right').prepend("</br><button class='reading_comprehension_option_btn'>翻译选项</button>");
                })
            }
        })
        ...
        $(document).on("click", ".reading_comprehension_translate_btn", function () {
            $(this).removeClass('reading_comprehension_translate_btn').addClass('reading_comprehension_hide_btn').html("清空翻译");
            var context_list = [];
            $(this).next().find("div[style='text-align:justify']").each(function () {
                //将偶数的br替换为hr
                $(this).children('br:even').replaceWith(' ')
        ...
        $(document).on("click", ".reading_comprehension_hide_btn", function () {
            $(this).removeClass('reading_comprehension_hide_btn').addClass('reading_comprehension_translate_btn').html("翻译选项")
        ...
        $(document).on("click", ".reading_comprehension_option_btn", function () {
            $(this).removeClass('reading_comprehension_option_btn').addClass('reading_comprehension_option_hide_btn').html("清空翻译")
            var context_list = "";
            $(this).parent().find('label').each(function () {
                context_list += $(this).text().replace(/\s/g, " ") + '\n';
            });
            ...
            var trans_str = trans_appid + context_list + trans_salt + trans_key;
            var trans_sign = $.md5(trans_str);
            var options = $(this).parent().children().children('.option')
            $.ajax({
                type: "post",
                async: false,
                url: "https://api.fanyi.baidu.com/api/trans/vip/translate",
        ...
        $(document).on("click", ".reading_comprehension_option_hide_btn", function () {
            $(this).removeClass('reading_comprehension_option_hide_btn').addClass('reading_comprehension_option_btn').html("翻译选项")
    ...
    function paragraph_translate() {
        //为段落翻译添加按钮
        $.each($('.itest-section'), function (index, value) {
            if ($(value).attr('part1') == "Part IV Translation    ") {
                $.each($(value).children('.itest-ques-set'), function (index, value) {
                    $(value).find('.word-count').prepend("</br><button class='paragraph_translate_btn'>翻译全文</button>");
                })
            }
        })
        ...
        $(document).on("click", ".paragraph_translate_btn", function () {
            $(this).removeClass('paragraph_translate_btn').addClass('paragraph_translate_hide_btn').html("清空翻译");
            var context_list = "";
            var re = new RegExp("\\(.*?\\)", "g");
            var re2 = new RegExp("（.*?）", "g");
            var re3 = new RegExp("\n", "g");
            $(this).parent().parent().find('div[style="text-align:justify"]').each(function () {
        ...
        $(document).on("click", ".paragraph_translate_hide_btn", function () {
            $(this).removeClass('paragraph_translate_hide_btn').addClass('paragraph_translate_btn').html("翻译全文")
            $(this).parent().parent().parent().find('textarea').each(function () {
                $(this).val("");
            });
        });
    ...
    function set_style() {
        ...
        $('.con-right.itest-need-layout').css({
            "margin-bottom": "100px"
        })
        $('.writing_translate_btn').css({
            "background": "#2befea",
            "cursor": "pointer",
        })
