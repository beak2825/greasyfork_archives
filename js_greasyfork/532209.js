// ==UserScript==
// @name         山东教师教育网，课程学习助手
// @namespace    http://tampermonkey.net/zzzzzzys_山东教师教育网
// @version      1.0.0
// @copyright    zzzzzzys.All Rights Reserved.
// @description  山东教师教育网，第一期 安庆市岳西县2023-2025年中小学幼儿园教师全员远程培训。其他项目也可试用下是否生效。课程可极速秒过，测试也可自动答题！课程播放页面脚本生效！
// @author       zzzzzzys
...
// @downloadURL https://update.greasyfork.org/scripts/532209/%E5%B1%B1%E4%B8%9C%E6%95%99%E5%B8%88%E6%95%99%E8%82%B2%E7%BD%91%EF%BC%8C%E8%AF%BE%E7%A8%8B%E5%AD%A6%E4%B9%A0%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/532209/%E5%B1%B1%E4%B8%9C%E6%95%99%E5%B8%88%E6%95%99%E8%82%B2%E7%BD%91%EF%BC%8C%E8%AF%BE%E7%A8%8B%E5%AD%A6%E4%B9%A0%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
class Runner {
    constructor() {
        this.runner = null
    ...
    initAjaxHooker() {
        ...
        ajaxHooker.hook(request => {
            ...
            if (request.url.includes('api/learning/') && request.url.includes('activity')) {
                request.response = res => {
                    const json = JSON.parse(res.responseText)
                    if(json.data?.activity?.type === "test"){
                        window.examList=json.data?.activity?.items
                        console.log("activity examList：",window.examList)
                        return
                    }
                    const actToken=json.data?.actToken
                    const currentTime=json.data?.duration
                    const duration=json.data?.activity?.length
                    const standard=json.data?.activity?.standard
                    console.log("activity actToken：",actToken);
                    console.log("activity currentTIme：",currentTime)
                    console.log("activity duration：",duration)
                    console.log("activity standard：",standard)
                    window.actToken = actToken
    ...
    run() {
        ...
        if (url.includes("learning")) {
            this.runner = new Course("channel-qlteacher")
            // this.runner.run()
        } else if (url.includes("mineCourse")) {
            this.runner = new Index("channel-qlteacher")
        }
    }
}
...
class Index {
    ...
    async run() {
        ...
        try {
            ...
            const catalogSelector = '.list_title'
            ...
            for (let i = 0; i < pageList.length; i++) {
                ...
                for (let catalog of catalogList) {
                    console.log(catalog.querySelector('.list_title_text').innerText)
                    ...
                    await sleep(1000)
                    const waitForUrlChange = () => new Promise(resolve => {
                        let checkCount = 0;
                        const interval = setInterval(() => {
                            // 如果URL变化或超时（5秒），则继续执行
                            if (location.href !== initialHref || checkCount >= 50) {
...
class Utils {
    ...
    static flag = 'qlteacher_VIP'
    static js_Flag = 'qlteacher_jsCode'
    static vipText = '高级功能已启用！(山东教师教育)'
    static baseText = '您正在使用基础版本，功能可能存在限制'
    static vipSign = 'qlteacher_vipSign'
    ...
    static async validateCode(data) {
        try {
            ...
            data.bindInfo = value.name + "_" + value.id
            data.website = "67f4e049286f7cabf082c3da"
            console.log(data);
            // return
