...
(function () {
    ...
    function renderGifs() {
        ...
        if (myGifs.size === 0) {
            ...
            const pinnedUrls = Array.from(pinnedGifs);
            const unpinnedUrls = Array.from(myGifs).filter(url => !pinnedGifs.has(url));
            pinnedUrls.forEach(url => addGifItem(url, true));
            unpinnedUrls.forEach(url => addGifItem(url, false));
        }
    }
