// ==UserScript==
// @name           Today's Projected Earnings Plus Graph
...
// @downloadURL https://update.greasyfork.org/scripts/3208/Today%27s%20Projected%20Earnings%20Plus%20Graph.user.js
// @updateURL https://update.greasyfork.org/scripts/3208/Today%27s%20Projected%20Earnings%20Plus%20Graph.meta.js
// ==/UserScript==
...
// ******* CHANGE THIS NUMBER TO YOUR DAILY GOAL *******
var dailyGoal = 30;
// ******* CHANGE THIS NUMBER TO YOUR DAILY GOAL *******
var imgData = '';
...
function earningsChart(TodayChart){
        $(".container-content:eq(0)").append('<div id="projectedEarnings" style="height:75px;"></div>');
        var plot2 = $.jqplot('projectedEarnings', [[[TodayChart,'']],], {
                seriesColors: [((TodayChart < dailyGoal) ? ((TodayChart < (dailyGoal - 5)) ? '#FF3300' : '#FFCC00') : '#33CC33')],
                seriesDefaults: {
                        renderer:$.jqplot.BarRenderer,
                        pointLabels: {
                            show: true,
                            location: ((TodayChart < (dailyGoal / 3)) ? 'e' : 'w'),
                            edgeTolerance: -15,
                            formatString: '$' + TodayChart.toFixed(2) + '/$' + dailyGoal
                        },
                        shadowAngle: 135,
                        rendererOptions: {
                ...
                axes: {
                        xaxis: {
                                max: ((TodayChart < dailyGoal) ? dailyGoal: (Math.floor(TodayChart) + 10)),
                                min: 0
                        },
                        yaxis: {
        ...
        var imgData = $('#projectedEarnings').jqplotToImageStr({});
        $('#projectedEarnings').on('click',function () {
             $.ajax({
