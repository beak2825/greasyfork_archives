...
// @grant       GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/3118/mmmturkeybacon%20Color%20Coded%20Search%20with%20Checkpoints.user.js
// @updateURL https://update.greasyfork.org/scripts/3118/mmmturkeybacon%20Color%20Coded%20Search%20with%20Checkpoints.meta.js
// ==/UserScript==
...
//To do: Handle the following error:
//Could Not Connect: (Too many connections) 

// URLs for testing, learning parameters
//https://turkopticon.ucsd.edu/api/multi-attrs.php?ids=ABSF8UXFZYEK6
//https://mturk-api.istrack.in/multi-attrs.php?ids=ABSF8UXFZYEK6
//https://data.istrack.in/turkopticon.php?data=2.57,2.31,2.89,2.71
...
var REVIEWS_BASE = 'https://turkopticon.ucsd.edu/';
...
function turkopticon_error_handler($parent_tables, api_base, requester_IDs_csv)
{
    ...
    else
    {
        $parent_tables.each(function()
        {
            ...
            // create checkpoints
            var $preview_link = $(this).find('a:contains("View a HIT in this group")');
            var groupId = $preview_link.attr('href')
            groupId = (groupId) ? groupId.split('=')[1] : $(this).find('a[href^="/mturk/notqualified?hitGroupId="]').attr('href').split(/=|&/)[1];
            ...
            $preview_link.after(checkbox);
            // mark checkpoints
            if (checkbox.checked == true)
            {
                var checkpoint_date = GM_getValue(checkbox.name+'_date');
                $preview_link.text(checkpoint_date+CHECKPOINT_MESSAGE);
    ...
    GM_xmlhttpRequest(
    {
        ...
        onload: function (response)
        {
            ...
            $parent_tables.each(function()
            {
                ...
                //var $preview_link = $(this).find('span[class="capsulelink"] > a');
                var groupId = $preview_link.attr('href')
                //groupId = (groupId) ? groupId.split('=')[1] : $(this).find('a[href^="/mturk/notqualified?hitGroupId="]').attr('href').split(/=|&/)[1];
                groupId = (groupId) ? groupId.split('=')[1] : $(this).find('a[href^="/mturk/notqualified"]').attr('href').split('?hitGroupId=')[1];
