...
// @version 2023.2.7
// @description 这个脚本帮助你导出京东的订单列表页中的订单,仅限本页.v2023新增：导出收件人一列，导出文件名加上年份和页码。基于somereason修改。
// @author somereason,mixuezj
...
(function () {
    ...
    $("#srBtnExport").click(function (e) {
        ...
        var fn=`JD_${title}[${page}].csv`
        createAndDownloadFile(fn, str);
    })
    //获取订单
    function getOrderListStr() {
        ...
        $(".tr-th").each(function () {//订单首行,就是有时间,订单号那行,订单和子订单都有
            ...
            var ele = {
                ...
                consignee: $(this).next().find(".consignee>.txt").text().trim(),
            };
            //子订单的明细,可能有多个商品
            ...
            ele.title = str1;
            str += `"${ele.billId}","${ele.time}","${ele.consignee}","${ele.status}","${ele.amount}","${ele.title}"`;//输出,只支持ES6
            str += "\n";
        })
