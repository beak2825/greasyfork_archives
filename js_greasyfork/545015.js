...
// @grant        GM_setValue
// @grant        GM_getValue
// @downloadURL https://update.greasyfork.org/scripts/545015/UploadAPI%20%E5%B7%A5%E5%85%B7.user.js
// @updateURL https://update.greasyfork.org/scripts/545015/UploadAPI%20%E5%B7%A5%E5%85%B7.meta.js
// ==/UserScript==
...
(() => {
    ...
    function uploadNetWorthToGitee() {
        ...
        const path = "api.json";
        const token = "6edd85f8a3e748dfe7ba0edcc3e65886";
                        ...
                        if (hashValue) {
                            try {
                                ...
                                // 保存回localStorage
                                        ...
                                        console.log(`数据已成功保存到localStorage.MYAPI，${flagName}已设置为1`);
                            } catch (e) {
                            ...
                            if (tbody) {
                                ...
                                if (hashValue && Object.keys(allRowsData).length > 0) {
                                    try {
                                        ...
                                        const flagName = `flag_click${characterId}`;
                                        localStorage.setItem(flagName, '1');
                                        console.log(`所有表格数据已成功保存到localStorage.MYAPI: #后字符=${hashValue}，${flagName}已设置为1`);

                                        // 等待flag_click变为0
                                        async function waitForFlagClick() {
                                            const characterId = getCharacterId();
                                            const flagName = `flag_click${characterId}`;
                                            while (localStorage.getItem(flagName) === '1') {
                                                await new Promise(resolve => setTimeout(resolve, 500));
                                            }
                                            console.log(`${flagName}已变为0，继续执行`);
                                        }
                                        waitForFlagClick();
                                    } catch (e) {
                                        console.error('保存数据到localStorage失败:', e);
