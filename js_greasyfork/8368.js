...
// @downloadURL https://update.greasyfork.org/scripts/8368/Super_preloaderPlus_one.user.js
// @updateURL https://update.greasyfork.org/scripts/8368/Super_preloaderPlus_one.meta.js
// ==/UserScript==
	...
	},
	{
		...
		autopager: {
			...
			documentFilter:	function(doc) {
				...
				var	pres = doc.querySelectorAll('pre[class^="brush:"]'); [].forEach.call(pres,
				function(pre) {
...
var	setup =	function(){
	...
	var	SP = {
		init: function() {
			...
			var	hashSite = _.find(HashchangeSites,
			function(x)	{
	...
	function init(window, document)	{
		...
		';
			...
			a_starti.addEventListener('click',
			function() {
		...
		function autopager(SSS,	floatWO) {
			...
			function createSep(lastUrl,	currentUrl,	nextUrl) {
				...
				if (SSS.a_separator) {
					...
					var	pageStr	= '第 <span style="color:red!important;">' + curNumber + '</span> 页' + (SSS.a_separatorReal ? getRalativePageStr(lastUrl, currentUrl, nextUrl) : '');
					div.appendChild($C('a',	{
	...
	function getRalativePageStr(lastUrl, currentUrl, nextUrl) {
		...
		if (!realPageSiteMatch)	{ //不满足以上条件，再根据地址特征来匹配
			var	sitePattern;
			for	(var i = 0,
			length = REALPAGE_SITE_PATTERN.length; i < length; i++)	{
	...
	function handleLazyImgSrc(rule,	doc) {
		...
		imgAttrs.forEach(function(attr)	{
			attr = attr.trim();	[].forEach.call(doc.querySelectorAll("img["	+ attr + "]"),
			function(img) {
	...
	function wildcardToRegExpStr(urlstr) {
		...
		var	reg	= urlstr.replace(/[()\[\]{}|+.,^$?\\]/g, "\\$&").replace(/\*+/g,
		function(str) {
			return str === "*" ? ".*": "[^/]*";
		});
		return "^" + reg + "$";
