...
// @version      4.0
// @downloadURL https://update.greasyfork.org/scripts/456395/7%20Cups%20-%20Tea%20%20Cake%20BETA.user.js
// @updateURL https://update.greasyfork.org/scripts/456395/7%20Cups%20-%20Tea%20%20Cake%20BETA.meta.js
// ==/UserScript==
{
...
<div class="col-12 col-md-9 form-wrap">
	...
	insensitive (a, b) { // case-insensitive comparison
		...
		async compile () {
			// number of pages…
			var base = location.href.replace(/\?p=\d+/, ''),
		...
		obey (post) { // do the thing the post wants {id, by, body}
		...
		op: { // operations...
			...
			removeme: {
				...
				admin: false,
				how: function (tag) {
					this.list = this.list.filter(u => u != tag[0])
					}
				},
			...
			remove: {
				...
				admin: true,
				how: function (tag) {
					this.list = this.list.filter(u => !tag.includes(u))
					}
				},
		...
		async read (base, pagenum) { // fetch a page and extract its posts
...
Tags: <span id="tc-taglist-n" style="display: inline-block; margin-right: 2em; font-size: 12px;">—</span>
