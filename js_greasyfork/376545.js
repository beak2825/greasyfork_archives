// ==UserScript==
// @name           Kamerka
// @namespace      https://greasyfork.org
// @author         2014, suchar, 2020, woland
// @description    Skrypt dodaje ikonę kamerki na profilu - link do transmisji.
// @homepage       https://greasyfork.org/pl/scripts/376545-kamerka
// @include        https://fotka.com/profil/*
// @version        1.7
// @grant          unsafeWindow
// @grant          GM_getValue
// @grant          GM_setValue
// @downloadURL https://update.greasyfork.org/scripts/376545/Kamerka.user.js
// @updateURL https://update.greasyfork.org/scripts/376545/Kamerka.meta.js
// ==/UserScript==
var $ = unsafeWindow.$;
	var l = $("#profile-box-activity-container");
	if (l != null){
        var a = document.createElement("A");
        a.href = "/kamerka/" + unsafeWindow._currentProfile.login;

        var p = document.createElement("IMG");
        if (unsafeWindow._currentProfile.cam.isTransmitting) {
           p.src = "data:image/png;base64,";
        } else {
           p.src = "data:image/png;base64,";
        }
        p.title = "Kamerka"; 
		p.style.position = "relative";
		p.style.top = "3px";
		p.style.marginLeft = "4px";
		a.appendChild(p);
        l.append(a);
	}
