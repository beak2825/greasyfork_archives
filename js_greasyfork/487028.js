...
// @version      1.81
// @description  FriendTasks
// @author       xxxx
// @include https://*.the-west.*/game.php*
// @icon         data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/487028/FriendTasks99.user.js
// @updateURL https://update.greasyfork.org/scripts/487028/FriendTasks99.meta.js
// ==/UserScript==
...
(function() {
        ...
        var findHealthConsume = function(consumes) {
            ...
            for(var i = 0 ; i < consumes.length;i++) {
                if(consumes[i].health != 0) {
                    ...
                    if(consumes[i].energy <= energyMissing) {
                        if(consumes[i].energy == 0 && consumes[i].health < consumeToChoose.health){
                            consumeToChoose = consumes[i];
                        } else {
                            return consumes[i];
                        }
    ...
    Dobby.createConsumablesTable = function() {
        ...
        if(Dobby.consumableUsed.length === 1) {
            lastConsumUsed = Dobby.consumableUsed[Dobby.consumableUsed.length - 1].name + " |";
        }
        if(Dobby.consumableUsed.length > 1) {
            lastConsumUsed = Dobby.consumableUsed[Dobby.consumableUsed.length - 1].name + ", " + Dobby.consumableUsed[Dobby.consumableUsed.length - 2].name;
        }
