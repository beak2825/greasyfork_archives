// ==UserScript==
// @name         CloudFaucet.io - BTC (Pescador de Cripto)
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/413570/CloudFaucetio%20-%20BTC%20%28Pescador%20de%20Cripto%29.user.js
// @updateURL https://update.greasyfork.org/scripts/413570/CloudFaucetio%20-%20BTC%20%28Pescador%20de%20Cripto%29.meta.js
// ==/UserScript==
...
(function() {
        ...
        if($('.jconfirm-content').find('div').text().indexOf('0') > -1){
            $.ajax({
                ...
                success: function(data){
                    balance = $(data).find('.dashboard-value').eq(0).text();
                    console.log(balance);
                }
            });
            setTimeout(function(){
    ...
    function cloudFaucet(){
        setTimeout(function(){
            if($('.bitcoin-faucet-button.deactive.tipso_style').attr('class').indexOf('deactive')){
                $('.bitcoin-faucet-button.deactive.tipso_style').click();
    ...
    function open(){
        if (window.location.href == "https://pescadordecripto.com/dashboard/") {
            var win_cloudfaucet = window.open("https://cloudfaucet.io/faucet", "CloudFaucetBtc","width=10,height=10,left=-3000,top=-3000");
        }
    ...
    if (window.location.href == "https://pescadordecripto.com/dashboard/") {
        ...
        a.id = 'cloudfaucet-io-btc';
        a.className = 'faucet-link faucet-active';
        a.innerHTML = 'CLOUDFAUCET.IO - BTC';
    ...
    if (window.location.href == "https://pescadordecripto.com/install/") {
        document.getElementById('cloudfaucet-io-btc').classList.add("faucet-active");
    }
})();