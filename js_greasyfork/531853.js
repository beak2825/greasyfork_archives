// ==UserScript==
// @name         Incomplete GGn itch.io Uploady
...
// @downloadURL https://update.greasyfork.org/scripts/531853/Incomplete%20GGn%20itchio%20Uploady.user.js
// @updateURL https://update.greasyfork.org/scripts/531853/Incomplete%20GGn%20itchio%20Uploady.meta.js
// ==/UserScript==
...
const tags_mapping = {
    ...
    "Puzzle": [ "puzzle" ],
    "Racing": [ "racing" ],
    "Real time strategy": [ "real.time.strategy", "strategy" ],
    "Rhythm": [ "rhythm" ],
    "Roguelike": [ "roguelike" ],
    "Roguelite": [ "roguelike" ],
    "Romance": [ "romance" ],
    ...
    "Turn-based Strategy": [ "strategy", "turn.based.strategy", "turn.based" ],
...
function fill_form(response) {
    ...
    if (steamLinkElem) {
        ...
        const steamFillWinButton = $('#fill_win')
...
function fill_form_from_itch_scrape(page) {
    ...
    const screenshots = []
    screenshotElems.forEach(elem => screenshots.push(elem.src))
    ...
    tags.forEach(tag => {
        if (tags_mapping[tag]) {
            ...
            tags_mapping[tag.toLowerCase()].forEach(mappedTag => appedTags.add(mappedTag))
        }
    })
