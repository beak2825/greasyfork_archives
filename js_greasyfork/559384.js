...
// @version      4.3
// @author       Tian Jialin
// @description  后台随机“刷新”检查 + 页面内检测；触发强力警报；测试警报可多次；实时倒计时显示
// @match        *://msc.engg.hku.hk/online/enrolment/enrolmentrecord_add.asp*
// @run-at       document-end
// @grant        GM_notification
// @grant        GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/559384/HKU%20Enrolment%20Loud%20Alarm%20%2B%20Live%20Countdown.user.js
// @updateURL https://update.greasyfork.org/scripts/559384/HKU%20Enrolment%20Loud%20Alarm%20%2B%20Live%20Countdown.meta.js
// ==/UserScript==
...
(() => {
    ...
    p.innerHTML = `
      ...
      <details id="tm-help" open>
        ...
        <div class="help-body">
          ...
          <ul>
            <li><b>页面内检测</b>（每 ${DOM_CHECK_MS}ms）：直接扫描当前页面文字，适合捕捉页面动态更新。</li>
            <li><b>后台刷新检查</b>（每 ${REMOTE_MIN_MS/1000}–${REMOTE_MAX_MS/1000}s 随机）：用 <code>fetch</code> 拉取最新 HTML，不会真的刷新页面，所以更稳定、也不容易丢状态。</li>
            <li><b>倒计时</b>：上面两行分别显示两种检测的下一次执行时间。</li>
          </ul>

          <div style="margin-top:6px;"><b>按钮说明：</b></div>
          <ul>
            <li><b>启用声音(一次)</b>：浏览器通常禁止“未交互自动出声”。点一次后会“滴”一下，解锁音频；只要你不手动刷新/关闭标签页，后续警报可直接响。</li>
            <li><b>测试警报</b>：随时可点，用于验证遮罩/通知/声音是否正常；<b>不会暂停监控</b>。</li>
            <li><b>恢复监控</b>：真正警报触发后脚本会自动暂停（避免刷屏/误触发连环警报），点它继续监控。</li>
          </ul>

          <div style="margin-top:6px;"><b>警报触发后怎么停？</b> 点击屏幕中央的「停止警报」。</div>
          <div style="margin-top:6px;">
            <b>想更敏感/更慢一点？</b> 修改顶部参数：<code>REMOTE_MIN_MS</code>/<code>REMOTE_MAX_MS</code>/<code>DOM_CHECK_MS</code>/<code>ALARM_VOLUME</code>。
          </div>
          <div style="margin-top:6px;">
            <b>调试：</b>F12 → Console，可看到 <code>[TM]</code> 相关日志。
          </div>
        </div>
      </details>
  ...
  async function unlockAudioOnce() {
    try {
      ...
      audioUnlocked = true;
      setText('#tm-note', '✅ 声音已启用：只要不手动刷新/关闭标签页，就不需要再点。');
    } catch (e) {
    ...
    overlay.innerHTML = `
      <div id="tm-alarm-box">
        ...
        <p id="tm-alarm-desc">
          ...
          ${audioUnlocked ? '正在持续鸣叫。' : '你还没“启用声音(一次)”，所以可能不会出声（浏览器限制）。'}
        </p>
  ...
  async function fetchClosedFound() {
    const resp = await fetch(location.href, { cache: 'no-store', credentials: 'include' });
  ...
  function pauseMonitoring() {
    ...
    if (domTimer) clearTimeout(domTimer);
    if (remoteTimer) clearTimeout(remoteTimer);
    domTimer = remoteTimer = null;
    nextDomAt = nextRemoteAt = 0;
  }
