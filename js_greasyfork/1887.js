...
// @downloadURL https://update.greasyfork.org/scripts/1887/DotD%20Raid%20Catcher.user.js
// @updateURL https://update.greasyfork.org/scripts/1887/DotD%20Raid%20Catcher.meta.js
// ==/UserScript==
...
function shared(sv) {
	...
	window.RaidCatcher = {
			...
			updateCheck: function () {
				if ((RaidCatcher.settings.lastUpdateCheck||0) < (new Date().getTime() - 14400000)) {
					RaidCatcher.request.req({
						eventName: "rc.chkupd",
						url: 'https://greasyfork.org/scripts/1887-dotd-raid-catcher/code',
						method: "GET",
						timeout: 30000
					});
		...
		ui:{//raid tab creation/interaction
			...
			getRaidSearchBossRegex: function(v){
				...
				if(splits.length>0) {
					for(i=0;i<splits.length;i+=2) {
						if (splits[i].trim() != "" && splits[i].trim()[0] != "!") {
							var currentVal = splits[i].trim();

							for (j in DotDRaidInfo.searchKeywords) {
								if (DotDRaidInfo.searchKeywords.hasOwnProperty(j)) {
									var keyWord = DotDRaidInfo.searchKeywords[j];
									if(DotDRaidInfo.classifications.hasOwnProperty(j)){
										keyWord.sub = DotDRaidInfo.classifications[j].toString().replace(/,/g, '|');
									}
									if (keyWord.reg && keyWord.sub && keyWord.reg.test(currentVal)) {
										currentVal = keyWord.sub;
									}
								}
							}
							...
							if (currentVal.trim() != "") {
								reString += (i==0)?"":"|";
								reString += "(";
								reString += currentVal.trim().replace(/\./g, "\\.");
								reString += ")";		
							}		
						} 
					}
			...
			getRaidSearchBossAntiRegex: function(v){
				var bossSearch = v||RaidCatcher.$("#RaidBossNameSearch").val();
				var splits = bossSearch.split(/\||,|(\sor\s)/i);
				var reString = "";
				if(splits.length>0) {
					for(i=0;i<splits.length;i+=2) {
						if (splits[i].trim() != "" && splits[i].trim()[0] == "!") {
							var currentVal = splits[i].trim().substring(1);
				...
				if (reString == "") {
					reString = "$qwertyuiopadsf^"; // This value should never match anything unless Cleo loses his mind
				}
				//console.log("[RaidCatcher] Boss search anti regex: " + reString)
				return new RegExp(reString, "i");
			},
			...
			raidSearch: function (str){
				...
				var anti_re = RaidCatcher.ui.getRaidSearchBossAntiRegex();
				var p_re = new RegExp(posterSearch, "i");
				for(i=0; i< raidList.length; i++) {
					var el = raidList[i];
					if(el && el.nodeType != 3){
						var f = RaidCatcher.ui.filterRaidSingle(el, re, p_re, anti_re, posterSwitch, roomSearch, roomSwitch, ageTime, isBefore);
			...
			filterRaidSingle: function(el, re, p_re, anti_re, posterSwitch, roomFilter, roomSwitch, age, isBefore){
				...
				//console.log("filterRaidSingle, re: " + re)
				//console.log("anti_re: " + anti_re)
				
				if(typeof r == 'object') {
					return ((re.test(e.innerHTML) || re.test(e.lastChild.innerHTML)) &&  //Name
							(!(anti_re.test(e.innerHTML) || anti_re.test(e.lastChild.innerHTML))) && // Anti-matching on name
							...
							<div class="tab_pane"> \
								<div id="ImportingExpirationTimesDiv" class="collapsible_panel"> \
									...
									<div id="ImportingExpirationTimes"><hr> \
										<table><!--<tr><td colspan="2" align="left" helptext="<p>Importing raids requires that you share raids in chat.  After sharing a raid, you will be able to import raids of that size for a certain amount of time.  All raids should expire six hours after your summon is available again.</p> <p>If a raid size has expired, simply summon a raid of that size, and directly submit it or post it in chat, then try importing again.</p>">\
										...
										<tr><td align="right" id="DotD_LabelG">Gigantic:&nbsp;</td><td align="left"><span id="DotD_ExpG">Unknown</span></td></tr>\
							...
							<div class="tab_pane" style="overflow:auto"> \
								<div id="dotd_raid_filters" class="collapsible_panel"> \
									...
									<div id="RaidFilters"> \
										...
										<div> \
											<a href="#" onclick="RaidCatcher.ui.toggleDisplay(\'DotDFilterGigantic\',this,null,\'Gigantic Raids [-]\');return false;">Gigantic Raids [+]</a><br> \
											<table id="DotDFilterGigantic" style="display:none">\
												<tr><td><b>Raid</td><td><b>N</b></td><td><b>H</b></td><td><b>L</b></td><td><b>NM</b></td><td><b>All</b></td></tr> \
											</table>\
										</div> \
								...
								<div id="dotd_leon_filters" class="collapsible_panel"> \
									...
									<div id="LEONFilter"> \
										...
										<div> \
											<a href="#" onclick="RaidCatcher.ui.toggleDisplay(\'LEONFilterGigantic\',this,null,\'Gigantic Raids [-]\');return false;">Gigantic Raids [+]</a><br> \
											<table id="LEONFilterGigantic" style="display:none">\
												<tr><td><b>Raid</td><td><b>1/5</b></td><td><b>1/6</b></td><td><b>1/8</b></td><td><b>2/9</b></td><td><b>2-3</b></td><td><b>3/9</b></td><td><b>3-4</b></td><td><b>All</b></td></tr> \
											</table>\
										</div> \
										<div> \
			...
			getSelectedRaids: function (s, sort) {
				...
				if(s != ""){
					...
					var re = RaidCatcher.ui.getRaidSearchBossRegex(),
						anti_re = RaidCatcher.ui.getRaidSearchBossAntiRegex(),
						...
						if(raid && raid.ele){
							if (!(typeof raid === 'undefined') && !RaidCatcher.settings.getFilter(raid.boss, raid.diff-1) && (
								...
								(((/visited/.test(s) && raid.visited) || (/new/.test(s) && !raid.visited)) &&
									RaidCatcher.ui.filterRaidSingle(raid.ele, re, pRe, anti_re, pSw, room, roomSw,ageTime,isBefore))
							)) {
								try {
									r.push(raid);
			...
			standardMessage: function (){
				if(RaidCatcher.version.needsUpdate) return 'Script version is out of date.  <a href="https://greasyfork.org/scripts/1887-dotd-raid-catcher" target="_blank" onclick="RaidCatcher.version.needsUpdate=false;RaidCatcher.ui.updateMessage()">Update</a> <a href="#" onclick="RaidCatcher.version.needsUpdate=false;RaidCatcher.ui.updateMessage(); return false;">Dismiss</a>'
		...
		chat: {
			...
			init: function(){
				...
				holodeck.addChatCommand("version", function(deck,text) {
					...
					d += '<a href="https://greasyfork.org/scripts/1887-dotd-raid-catcher" target="_blank">Go to script page</a>';
					RaidCatcher.chat.echo(d);
					return false;
				});
				holodeck.addChatCommand("update", function(deck,text) {
					window.open("https://greasyfork.org/scripts/1887-dotd-raid-catcher");
				...
				holodeck.addChatCommand("ad", function(deck, text) {
					RaidCatcher.chat.output("Get your raids here! Please read the information on the script page https://greasyfork.org/scripts/1887-dotd-raid-catcher");
					return false;
				});
			...
			output: function (txt) {
				// Quick hack so the reload button will work on the AG toolbar
				if (txt == "/reload") {
					RaidCatcher.game.reload();
				} else {
					var elems= document.getElementsByClassName('chat_input');
		...
		settings: (function(){
			...
			tmp.init = function(){
				...
				this.importAliveG = (typeof this.importAliveG == 'boolean'?this.importAliveG:false);
				...
				this.importExpireG = (typeof this.importExpireG == 'number'?this.importExpireG:0);
				...
				this.importSummonG = (typeof this.importSummonG == 'number'?this.importSummonG:0);
			...
			tmp.updateImportInfo = function (r){
				if(r){
					...
					RaidCatcher.settings.importExpireG = r.dateG;
					...
					RaidCatcher.settings.importSummonG = r.summG;
					...
					RaidCatcher.settings.importAliveG = r.activeG;
				...
				var html1 = makeHtml(RaidCatcher.settings.importAliveS,RaidCatcher.settings.importExpireS, RaidCatcher.settings.importSummonS, 'Small'), 
					...
					html6 = makeHtml(RaidCatcher.settings.importAliveG,RaidCatcher.settings.importExpireG, RaidCatcher.settings.importSummonG, 'Gigantic');
				...
				RaidCatcher.$('#DotD_ExpG').html(html6, true)
		...
		util: {
			...
			getRaidSizes: {
				...
				800:{ name: 'Gigantic', visible: 'Yes', pruneTimers: [86400000,172800000,259200000]} // 24h, 48h, 72h
			},
			ctrlHeld:false,
...
function amain(sv){//AG
	...
	window.initAGDotD = function (tries) {
		...
		if (tries > 12 && !(typeof DotDRaidInfo == 'object')) {
			// Load cached raid info
			console.log("[RaidCatcher] Attempting to load cached raid info");
			if (typeof DotDRaidInfo != 'object') {
				try{ 
		...
		if(tries > 60){ // Increased to 60 because of the pre-stitial ad system they added
			console.log('[RaidCatcher] AG resources not found. Aborting');
			return;
		}
		if(typeof RaidCatcher == "object" && typeof document.getElementById('gamefilearea') != 'null' && typeof document.getElementById('gamefilearea').getElementsByTagName('iframe')[0] != 'null' && typeof document.getElementById('gamefilearea').getElementsByTagName('iframe')[0] != 'undefined' && typeof DotDRaidInfo == 'object'){
			...
			var gameframe = document.getElementById('gamefilearea').getElementsByTagName('iframe')[0];
