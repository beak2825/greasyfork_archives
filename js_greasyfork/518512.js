...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/518512/spawnpad%20%2B%20Mill.user.js
// @updateURL https://update.greasyfork.org/scripts/518512/spawnpad%20%2B%20Mill.meta.js
// ==/UserScript==
(() => {
...
const place = (thing, angle) => {
      ...
      emit("F", 1, angle);
      emit("F", 0, angle);
      emit("z", myPlayer.weaponIndex, true);
    };
  /**
...
var element36 = document.getElementById('actionBarItem36');
if (element36 && element36.offsetParent !== null) {
    inventory.spawnpad = 36 - 16;
}
  };
  ...
  WebSocket.prototype.send = function (event) {
   ...
   window.setInterval(() => {
    if (Math.atan2(y - myPlayer.y, x - myPlayer.x) < (scale + placeOffsetr) * 2) {
        if (autospawn) {
            var centerX = myPlayer.x;
            var centerY = myPlayer.y;
            place(inventory.spawnpad, 0);
            place(inventory.spawnpad, - Math.PI / 2);
            place(inventory.spawnpad, Math.PI / 2);
            place(inventory.spawnpad, Math.PI);
        }
    }
  }, 200);

  // Event listener for [N] key to toggle autoSpawn
  document.addEventListener("keydown", event => {
    if (event.code === 'KeyN' && document.activeElement.id.toLowerCase() !== 'chatbox') {
        autospawn = !autospawn;
    }
  });
  ...
  document.addEventListener("keydown", event => {
    if (event.code === 'KeyM' && document.activeElement.id.toLowerCase() !== 'chatbox') {
        autoMill = !autoMill;
    }
  });
})();
