// ==UserScript==
// @name         01傑(20180904)
// @namespace    http://swordman-s1.yytou.com/?id=6208387&time=1513298597083&key=4252c5b7dc18cf870a0e68867c14468f&s_line=1
// @version      2.1
// @description  try to take over the world!
// @author       MARBO&jj
// @match        http://swordman-s1.yytou.com/?id=6208387&time=1513298597083&key=4252c5b7dc18cf870a0e68867c14468f&s_line=1
// @grant        note
// @downloadURL https://update.greasyfork.org/scripts/372751/01%E5%82%91%2820180904%29.user.js
// @updateURL https://update.greasyfork.org/scripts/372751/01%E5%82%91%2820180904%29.meta.js
// ==/UserScript==
...
var playerName = '千門傑';
var playerMaxHp = '347920';
var enforcePoints = '1375';
var mySkillLists = '九天龍吟劍法;排雲掌法';
var mySkillLists1 = '乾坤大挪移';
skillLists = '';
buttonWidth = '80px';
buttonHeight = '20px';
currentPos = 35;
...
//	   BiShi2Button.style.visibility="hidden";//切小號
	   ...
	   pozhaoInButton1.style.visibility="hidden";// 破招守
...
/*  clickButton('clan buy 601');//空識卷軸
    ...
    clickButton('enable fonxansword', 1);//封山劍法
    clickButton('practice fonxansword', 1);
    r3();//技能準備
...
function killDrunkManFunc(){
	if (killDrunkManButton.innerText == '刷碎片') {
        ...
        clickButton('enableskill enable paiyun-zhang no_attack_select', 1);//取消排雲掌法必刷
        clickButton('enableskill enable jiutian-sword no_attack_select', 1);//取消九天龍吟劍法必刷
        clickButton('enableskill enable dzxinmojing no_attack_select', 1);//取消道種心魔經必刷
        clickButton('enableskill enable force budongmwj', 1);//不動明王訣
        clickButton('enableskill enable budongmwj no_attack_select', 1);//取消不動明王訣必刷
   ...
   var bm = 0,
    ...
    function QiXiaTalkFunc() {
        // if (userqu == 15 || userqu == 47 || userqu == 51 || userqu == 56 || userqu == 57 || userqu == 64 || userqu == 66 || userqu == 68) {
            var a = prompt("請輸入撩奇俠順序（中文分號隔開）", "郭濟；浪喚雨；火雲邪神；步驚鴻；宇文無敵；逆風舞；李宇飛；王蓉；龐統；風南；玄月研；狼居胥；烈九州；李玄霸；八部龍將；夏嶽卿；妙無心；巫夜姬");
            qxList = a.split("；");
            qxOrder = 0;
...
//破招守--------------------------------------------------------------------------------------------------------------------------------------------
	var pozhaoInButton1 = document.createElement('button');
		...
		function CheckCeshi1 () {
		  if (pozhaoInButton1.innerText == '破招守') {
		    pozhaoInButton1.innerText = '停破招守';
...
function autoBattleFunc(){
    ...
    if(!skillstr){
        //skillstr = prompt("請輸入要釋放的技能,以英文逗號分隔,前兩種武功為攻擊技能,第3個參數為釋放時需要的氣數,第4個參數為加血技能:","九天龍吟劍法,排雲掌法,8,道種心魔經");
        var skillstr = '如來神掌,覆雨劍法,8,道種心魔經';
...
//				    var aaa = ['悟性裝','寶箱3000','通靈','上香','打鳥','冰月谷','突破丹禮包','狂暴丹','掌門手諭','投資','煉藥','拼圖','簽到','禦寒衣','刻刀'];
				...
				if(qmjname2=='準備銅人'){
                    ...
                    clickButton('enableskill enable sword yitian-sword', 1);
				...
				if(qmjname2=='打銅人'){clickButton('enable mapped_skills restore go 1', 1);r4();clickButton('enforce '+ enforcePoints);clickButton('golook_room');}
                ...
                if(qmjname2=='全任務'){
                    ...
                    for (var i=0; i<5; i++){clickButton('vip finish_bad 2', 0);}//邪氣5次
...
function baoguoZhengli1Func() {
    ...
    zhengli("蒼狼護臂","equip_barcer7", "put_store");
    zhengli("青鸞護臂","equip_barcer8", "put_store");
    zhengli("翎眼赤護","equip_barcer9", "put_store");
...
function r2(){//青龍技能
    ...
    clickButton('enableskill enable dodge qiankun-danuoyi', 1);//乾坤大挪移
    clickButton('enable unmap_all', 1);//一鍵取消
    clickButton('enableskill enable unarmed paiyun-zhang', 1);//排雲掌法
    clickButton('enableskill enable paiyun-zhang attack_select', 1);//排雲掌法必刷
    ...
    clickButton('enableskill enable parry xianglong-zhang', 1);//降龍十八掌
    clickButton('golook_room');
}
function r3(){//技能準備
    ...
    clickButton('enableskill enable dodge wanliuguiyi', 1);//萬流歸一
    clickButton('enableskill enable dodge qiankun-danuoyi', 1);//乾坤大挪移
    clickButton('enableskill enable qiankun-danuoyi attack_select', 1);//乾坤大挪移必刷
    clickButton('enableskill enable dodge yyhuanxubu', 1);//幽影幻虛步
    clickButton('enableskill enable parry liumai-shenjian', 1);//六脈神劍
    clickButton('enableskill enable parry xianglong-zhang', 1);//降龍十八掌*/
    clickButton('golook_room');
...
function r4(){//正常裝
            ...
            go("unwield sword of windspring;wield weapon_sb_sword11;wield weapon_sb_unarmed11;wear equip_moke_head10;wear equip_moke_finger10");
}
       function s1(){//悟性裝
            go("unwield weapon_sb_sword10;wield sword of windspring;wear dream hat;wear longyuan banzhi moke");
  }
       function s2(){//寶箱3000
	   ...
	   function s4(){//上線香
		...
		function s7(){// 冰月谷
		    go("jh 14;w;n;n;n;n;by_upgrade 2 equip_by_yupei10;by_upgrade 2 equip_by_yupei10;event_1_32682066;event_1_83460220;kill bingyuegu_longwenshou");
}
        function s8(){// 突破丹禮包
...
gSocketMsg.addMessage2 = function(cc) {
        ...
        for (this._msg += cc; 0 < this._msg.length; ) {
            ...
            if(this._msg_line.indexOf("月朗天門陣升級完畢！") != -1){clickButton('hhjz xiulian go', 1);}
            ...
            if(this._msg_line.indexOf("你的天波九轉成功向前突破了，請繼續努力！") != -1){tupostart();}
            0 < this._msg_line.length && this.recvNetWork(this._msg_line);
            this._msg_len = this._msg_start = 0
        }
    }
                    function tupostart(){
                    clickButton('enable tbjz', 1);//天波九轉 21/9->15
                    clickButton('tupo go,tbjz', 1);
                    clickButton('tupo_speedup tbjz go', 0);

                    clickButton('enable jxdyq', 1);//九溪斷月槍 7/10->20
                    clickButton('tupo go,jxdyq', 1);
                    clickButton('tupo_speedup2 jxdyq go', 0);

                    clickButton('enable douzhangundian', 1);//鬥戰棍典 25/9->13
                    clickButton('tupo go,douzhangundian', 1);
                    clickButton('tupo_speedup douzhangundian go', 0);

                    //clickButton('enable king-sword', 1);// 帝王劍法 5/10->10
