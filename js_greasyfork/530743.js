// ==UserScript==
// @name           MooMoo.io | 🐺FΣΓΔΠΨZΣΓΛΓ🐺
// @namespace      https://greasyfork.org/users/163730
// @version        ╳F╳Δ╳Z╳Σ╳
// @description    【 ČESKY 】 ➜ 〖 AUTOMATICKÝ ŽIVOT 〗〖 DVOJITÝ ÚTOK 〗〖 INFORMACE O VYLEPŠENÍ / KLÁVESNICI 〗〖 MĚNĚNÍ ČEPICE / DOPLŇKU V TABULCE 〗〖 MĚNĚNÍ ITEMŮ 〗〖 MĚNĚNÍ ČEPICE + KUPOVÁNÍ 〗〖 MĚNIČ ČEPIC 〗〖 LEPŠÍ MINIMAPA 〗〖 ODSTRANĚNÍ ZBYTEČNOSTÍ 〗〖 DALŠÍ VYLEPŠENÍ 〗ㅤ【 ENGLISH 】 ➜〖 AUTO HEAL 〗〖 DOUBLE ATTACK 〗〖 UPGRADE / KEYBOARD INFORMATION 〗〖 CHANGE HATS / ACCESSORIES IN TOOLBAR 〗〖 CHANGE ITEMS 〗〖 CHANGE HAT + BUYING 〗〖 CHANGER HATS 〗〖 BETTER MINIMAP 〗〖 REMOVING NEEDLESSNESS 〗〖 OTHER IMPROVEMENTS 〗
// @author         🐺FΣΓΔΠΨZΣΓΛΓ🐺
...
// @downloadURL https://update.greasyfork.org/scripts/530743/MooMooio%20%7C%20%F0%9F%90%BAF%CE%A3%CE%93%CE%94%CE%A0%CE%A8Z%CE%A3%CE%93%CE%9B%CE%93%F0%9F%90%BA.user.js
// @updateURL https://update.greasyfork.org/scripts/530743/MooMooio%20%7C%20%F0%9F%90%BAF%CE%A3%CE%93%CE%94%CE%A0%CE%A8Z%CE%A3%CE%93%CE%9B%CE%93%F0%9F%90%BA.meta.js
// ==/UserScript==
...
// AUTOMATICKÝ ŽIVOT [ F4 ─ ☑ / ☐ ] | DVOJTÝ ÚTOK [ Pravé Tlačítko ] | INFORMACE O VYLEPŠENÍ [ F2 ] | MĚNĚNÍ ČEPIC A DOPLŇKY V TABULCE [ F4 ] | INFORMACE O KLÁVESNICI [ F7 ] | INFORMACE O KLÁVESNICI 2 [ F8 ] | MĚNĚNÍ ITEMŮ [ Větrný Mlýn ─ L / Špice ─ O / Jámová Past ─ P / Zrychlovací Podložka ─ P ] \\
// AUTO HEAL [ F4 ─ ☑ / ☐ ] | DOUBLE ATTACK [ Right Button ] | UPGRADE TREE [ F2 ] | CHANGE HATS / ACCESSORIES IN TOOLBAR [ F4 ] | KEYBOARD INFORMATION [ F7 ] | KEYBOARD INFORMATION 2 [ F8 ] | CHANGE ITEMS [ Windmill ─ L / Spikes ─ O / Pit Trap ─ P / Boost Pad ─ P ] \\
...
$('body').prepend('<img class= "InfoOKlávesnice" src= "https://i.imgur.com/SgmFhMB.png" alt= "Info O Vylepšení" width= "977" height= "319" style= "position: absolute ; left: 14.2752562225475827517584548331797122955322265625% ; top: 19.23177083333333570180911920033395290374755859375% ; display: none ; pointer-events: none ; "/>' );
...
document.addEventListener('keydown', function(kfc) {
    if(!$(':focus').length) {
        switch (kfc.keyCode) {
            ...
            case 221: Rt(14); kfc.preventDefault(); break;            // Bear Head                           = ) [ close bracket / å / 221 ]
            case 191: Rt(15); kfc.preventDefault(); break;            // Monkey Head                         = ´ [ forward slash / ç / 191 ]
            case 219: Rt(16); kfc.preventDefault(); break;            // Polar Head                          = Ú [ open bracket / 219 ]
            ...
            case 186: Rt(19); kfc.preventDefault(); break;            // Blitz Hat                           = Ů [ semi-colon / ñ / 186 ]
            ...
            case 73: Rt(36); kfc.preventDefault(); break;             // Scavenger Gear                      = I [ 73 ]
          }
	}
});
...
$('#mapDisplay').css({'background': 'url("https://i.imgur.com/iFM3MOv.png")'});
...
document.getElementById('gameName').innerHTML = '🐺FERANYZERIR🐺';
