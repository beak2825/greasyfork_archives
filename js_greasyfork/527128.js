// ==UserScript==
// @name        Control Panel for Twitter Custom 
// @description 以降verだとsafriで動かなくなってるので更新停止 /　Gives you more control over Twitter and adds missing features and UI improvements
...
// @license     MIT
// @version     186.2_end
// @downloadURL https://update.greasyfork.org/scripts/527128/Control%20Panel%20for%20Twitter%20Custom.user.js
// @updateURL https://update.greasyfork.org/scripts/527128/Control%20Panel%20for%20Twitter%20Custom.meta.js
// ==/UserScript==
void function() {
...
const config = {
  ...
  // Shared
  addAddMutedWordMenuItem: true, // ミュートワード追加メニューアイテムを追加する
  alwaysUseLatestTweets: true, // 最新ツイートを常に使用する
  defaultToLatestSearch: false, // デフォルトで最新の検索結果を使用しない
  disableHomeTimeline: false, // ホームタイムラインを無効にしない
  disabledHomeTimelineRedirect: 'notifications', // ホームタイムライン無効時のリダイレクト先を通知に設定
  disableTweetTextFormatting: false, // ツイートテキストのフォーマットを無効にしない
  dontUseChirpFont: true, // Chirpフォントを使用しない
  dropdownMenuFontWeight: true, // ドロップダウンメニューのフォントウェイトを有効にする
  fastBlock: true, // 高速ブロック機能を有効にする
  followButtonStyle: 'monochrome', // フォローボタンのスタイルをモノクロに設定
  hideAdsNav: true, // 広告ナビゲーションを非表示にする
  hideBlueReplyFollowedBy: false, // フォローしている人からの青い返信を非表示にしない
  hideBlueReplyFollowing: false, // 自分がフォローしている人からの青い返信を非表示にしない
  hideBookmarkButton: false, // ブックマークボタンを非表示にしない
  hideBookmarkMetrics: false, // ブックマークの指標を非表示にしない
  hideBookmarksNav: false, // ブックマークナビゲーションを非表示にしない
  hideCommunitiesNav: true, // コミュニティナビゲーションを非表示にする
  hideComposeTweet: false, // ツイート作成ボタンを非表示にしない
  hideExplorePageContents: false, // 探索ページの内容を非表示にしない
  hideFollowingMetrics: true, // フォロー数の指標を非表示にする
  hideForYouTimeline: true, // 「あなたへのおすすめ」タイムラインを非表示にする
  hideGrokNav: false, // Grokナビゲーションを非表示にしない
  hideInlinePrompts: true, // インラインプロンプトを非表示にする
  hideJobsNav: true, // ジョブナビゲーションを非表示にする
  hideLikeMetrics: false, // いいね数の指標を非表示にしない
  hideQuotesFrom: [], // 特定のユーザーからの引用ツイートを非表示（リスト指定）
  hideListsNav: false, // リストナビゲーションを非表示にしない
  hideMetrics: false, // 全ての指標を非表示にしない
  hideMonetizationNav: true, // 収益化ナビゲーションを非表示にする
  hideMoreTweets: true, // 「もっと見る」ツイートを非表示にする
  hideNotifications: 'ignore', // 通知を無視（非表示）
  hideProfileRetweets: false, // プロフィールのリツイートを非表示にしない
  hideQuoteTweetMetrics: false, // 引用ツイートの指標を非表示にしない
  hideReplyMetrics: false, // 返信数の指標を非表示にしない
  hideRetweetMetrics: true, // リツイート数の指標を非表示にする
  hideSeeNewTweets: false, // 「新しいツイートを見る」を非表示にしない
  hideShareTweetButton: false, // ツイートの共有ボタンを非表示にしない
  hideSubscriptions: true, // 購読を非表示にする
  hideTimelineTweetBox: false, // タイムラインのツイートボックスを非表示にしない
  hideTotalTweetsMetrics: false, // ツイート総数の指標を非表示にしない
  hideTweetAnalyticsLinks: false, // ツイートの分析リンクを非表示にしない
  hideTwitterBlueReplies: true, // Twitter Blueの返信を非表示にする
  hideTwitterBlueUpsells: true, // Twitter Blueのアップセルを非表示にする
  hideUnavailableQuoteTweets: false, // 利用できない引用ツイートを非表示にしない
  hideVerifiedNotificationsTab: true, // 認証済み通知タブを非表示にする
  hideViews: false, // 閲覧数を非表示にしない
  hideWhoToFollowEtc: true, // 「フォローすべき人」などを非表示にする
  listRetweets: 'ignore', // リストのリツイートを無視する
  mutableQuoteTweets: true, // 引用ツイートをミュート可能にする
  mutedQuotes: [], // ミュートされた引用ツイートのリスト
  quoteTweets: 'ignore', // 引用ツイートを無視する
  redirectToTwitter: false, // 別のサイトからTwitterへのリダイレクトを無効化しない
  reducedInteractionMode: false, // インタラクションモードを制限しない
  restoreLinkHeadlines: true, // リンクの見出しを復元する
  replaceLogo: true, // ロゴを置き換える
  restoreOtherInteractionLinks: false, // その他のインタラクションリンクを復元しない
  restoreQuoteTweetsLink: false, // 引用ツイートのリンクを復元しない
  retweets: 'separate', // リツイートを分ける
  showBlueReplyFollowersCountAmount: '1000000', // 青い返信のフォロワー数の表示量
  showBlueReplyFollowersCount: false, // 青い返信のフォロワー数を表示しない
  showBlueReplyVerifiedAccounts: false, // 青い返信の認証済みアカウントを表示しない
  showBookmarkButtonUnderFocusedTweets: true, // フォーカスされたツイートの下にブックマークボタンを表示する
  sortReplies: 'relevant', // 返信を関連性順にソート
  tweakNewLayout: false, // 新しいレイアウトの調整を行わない
  tweakQuoteTweetsPage: false, // 引用ツイートページを調整しない
  twitterBlueChecks: 'replace', // Twitter Blueのチェックを置き換える
  unblurSensitiveContent: true, // センシティブコンテンツのぼかしを解除する
  uninvertFollowButtons: true, // フォローボタンの反転を解除する
  // Experiments（試験的機能）	
  customCss: '', // カスタムCSSを適用（空欄の場合なし）	
  // Desktop only（デスクトップ版のみ）
  fullWidthContent: false, // コンテンツを全幅表示にしない
  fullWidthMedia: false, // メディアを全幅表示にしない
  hideAccountSwitcher: false, // アカウント切り替えを非表示にしない
  hideExploreNav: false, // 探索ナビゲーションを非表示にしない
  hideExploreNavWithSidebar: true, // サイドバーと一緒に探索ナビゲーションを非表示にする
  hideMessagesDrawer: true, // メッセージドロワーを非表示にする
  hideProNav: true, // プロナビゲーションを非表示にする
  hideSidebarContent: true, // サイドバーの内容を非表示にする
  hideSpacesNav: true, // スペースナビゲーションを非表示にする
  hideToggleNavigation: false, // ナビゲーション切り替えボタンを非表示にしない
  navBaseFontSize: false, // ナビゲーションの基本フォントサイズを有効にしない
  navDensity: 'default', // ナビゲーションの密度をデフォルトに設定
  showRelevantPeople: false, // 関連する人を表示しない
  // Mobile only（モバイル版のみ）
  hideMessagesBottomNavItem: false, // 下部ナビゲーションのメッセージアイコンを非表示にしない
}
//#endregion
