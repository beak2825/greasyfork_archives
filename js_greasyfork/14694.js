// ==UserScript==
// @name        Not bloomed - Late bloomer - 18/4.5 Potentiel+
...
// @grant        all
// @downloadURL https://update.greasyfork.org/scripts/14694/Not%20bloomed%20-%20Late%20bloomer%20-%201845%20Potentiel%2B.user.js
// @updateURL https://update.greasyfork.org/scripts/14694/Not%20bloomed%20-%20Late%20bloomer%20-%201845%20Potentiel%2B.meta.js
// ==/UserScript==
		...
		{ 
			if (xhr.readyState == 4)
			{
                ...
                return ((Math.pow(2,Math.log(weight*SI)/Math.log(Math.pow(2,7))) - Math.pow(2,Math.log(weight*wage/(wage_rate))/Math.log(Math.pow(2,7)))).toFixed(1)*10) - 1 ;
                 ...
                 function return_starting_blooming(Last_Season_Avg_Ti,Season_Avg,SI,age)
                        {var Age = parseInt(age);
                         var SIN = parseInt(SI.replace(",", "")) ;
                         if (Age == 21) {Season_Avg = Season_Avg + 1 ;} 
                   
                         if ((Last_Season_Avg_Ti < 9) && (Last_Season_Avg_Ti > -1))
                             if ((Season_Avg < 9) && (Season_Avg > 7))
                                 if ((SIN > 6999) && (SIN < 17000))
                                     return "YES" ;
                                 else
                                     return "----";
                            else
                ...
                $("a[player_link=" + resultado.player.player_id + "]").closest("tr").append("<td class='right align_right SB' style='padding-right:5px;'><span>" + return_starting_blooming(Last_Season_Avg_Ti(return_TI((arrary[id_index(resultado.player.player_id,arrary)][1]),resultado.player.skill_index,player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary))),Season_TI(player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary)),SI(resultado.player.skill_index),return_wage(resultado.player.wage),24.795),session,parseInt(resultado.player.months)),return_TI((arrary[id_index(resultado.player.player_id,arrary)][1]),resultado.player.skill_index,player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary))),resultado.player.skill_index,resultado.player.age)  + "</span></td>");
