...
// @version        2.12.23
// @downloadURL https://update.greasyfork.org/scripts/35932/Travian%20Resource%20bar%20plus.user.js
// @updateURL https://update.greasyfork.org/scripts/35932/Travian%20Resource%20bar%20plus.meta.js
// ==/UserScript==
...
function marketTradeRoutes() {
	...
	function createTradeLoop() {
		...
		if(err.length==0) {
			...
			for(var i=0; i<e.length; i++) {
				if(e[i].checked) {
					setTimeout(function(k){createTrade(k);}, 300*count++, e[i].value);
				}
				if (i==e.length-1) {
	...
	function merUpd () {
		var aR = 0;
		for (var i = 1; i < 5; i++ ) { var val = $g("r"+i,routesTable).value; aR += isNumeric(val) ? parseInt(val) : 0; }
	...
	routesTable.tBodies[0].appendChild(m1);
	...
	for(var i=0; i<24; i++) {
		...
		var t = $t(i>9?i:"0"+i)
		var c = $e('INPUT',[['type','checkbox'],['style','margin-'+docDir[0]+':15px;'],['class','rbcheckbox'],['value',i],['checked',true]]);
		p.appendChild(c);
		p.appendChild(t);
	}
	p.appendChild($e('BR'));
	var btnText = $g("tradeSaveButton",routesTable);
...
function detectEgyptiansAndHuns () {
	...
	if( aText ) { 
		...
		if (T4_feature_flags.tribesEgyptiansAndHuns == true) {
			RB.Setup[47] = 1;
			RB.Setup[48] = 401; //hardcoded map size. only Travian Fire and Sand has 400 map size.
			saveCookie( 'RBSetup', 'Setup' );
		} else {
			RB.Setup[47] = 2;
			RB.Setup[48] = 801; //hardcoded map size.
			saveCookie( 'RBSetup', 'Setup' );
		}
	}
}
...
function detectMapSize () {
	var fullScr = $xf('//script[contains(text(),"TravianDefaults")]').textContent;
	if ( /"width":(\d+)/.test(fullScr) ) {
		RB.Setup[48] = fullScr.match(/"width":(\d+)/)[1];
		saveCookie( 'RBSetup', 'Setup' );
	}
}
...
function addReport () {
	...
	form.appendChild($e('input',[['type','hidden'],['name','design'],['value',RB.Setup[47]==1?0:1]]));
...
function displayWhatIsNew () {
	if ($g('whatsnew')) { 
		...
		var box = $e('div',[['id','whatsnew'],['style','width:410px;position:fixed;top:50%;left:50%;transform: translate(-50%,-50%);color:black;background-color:#FFFFFF;padding:5px 5px;border-radius:1em;z-index:999;opacity:0.88;']]);
		...
		content.innerHTML = "What's new in Version "+version+" - Nov 26, 2017:<p></p><ui><li>Firefox Quantum 57.0 with Greasemonkey 4.0 compatibility (Greasemonkey 4.0 is in early development and does not support auto-update yet)</li><li>Fix script error in marketplace for Persian language</li></ui>";
