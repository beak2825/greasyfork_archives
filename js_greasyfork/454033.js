...
// @author       afk
// @grant        none
// @version 0.0.1.20221031134034
// @downloadURL https://update.greasyfork.org/scripts/454033/A-ware.user.js
// @updateURL https://update.greasyfork.org/scripts/454033/A-ware.meta.js
// ==/UserScript==
...
function handleMessage(m){
 ...
 if ("/log" == moduel[2].substring(0, 4)) {
    ...
    if(FunctionID == "2" && moduel[1] != myPlayer.id && Hacks && log) {// supermod stuff
        if(null != names[moduel[1][1]]){
            if(names[moduel[1][1]] != moduel[1][2]){
                ...
                console.log("{", names[moduel[1][1]], "} returned");
                chat("{" + names[moduel[1][1]] + "} returned");
            }
        }else{
    ...
    if(FunctionID == "13" && moduel[1] != myPlayer.id && Hacks && log) { // supermod stuff
        ...
        if(names[moduel[1]]) {
            console.log("{", names[moduel[1]], "} raged");
            chat("{" + names[moduel[1]] + "} raged");
            names[moduel[1]] = void 0;
        }else{
            console.log("[", moduel[1], "] raged");
...
let message = {
    ...
    chats: [
        ...
        "Aye, I don't wanna die young",
        "so I stay with a stick",
        ...
        "That's a fact and no cap", "Me and Sheff-",
        "had to run up them racks", "If you miss then we doublin",
        "back 2018", "I was stuck in the trap", "Fell in love",
        "and I got too attached", "Heart got broke", "it ain't no comin' back",
        "If we up it", "ain't no comin back", "Fuck with my opps",
        "said they no comin' back", "Is you with it?",
        "If you up it, I up it", "I up it, let's get it",
        ...
        "Cause she ain't", "feel the strokes in a min",
        "You know me", "I be hittin' it different",
        "So you can't get it", "when I'm finished"]
};
setInterval(() => {
    if(messageToggle) {
        message.count++
...
!(function(e) {
    ...
    function(e, t, n) {
        ...
        var at = document.getElementById("featuredYoutube"),
            ot = [{
                name: "AFK",
                link: "https://www.youtube.com/channel/UCKMRr-UjkAaTAtGj54OpEsQ",
            },
                 ],
    ...
    var teamDetect = window.teamDetect = function(id){
        ...
        function Bots(C, name){
            Removing = false;
            let Projects = ['cyclic-reflective-bream','enshrined-gusty-guanaco','dust-branch-exception','blush-holy-carob','economic-quilled-spandex','citrine-eastern-mercury','stitch-cute-mare','autumn-bright-beach','cyan-abrasive-impulse','lacy-vintage-spatula']
                    ...
                    e.tick = setInterval(async () => {
                        if(e.able && document.wsURL && !e.sent && R){
                            async function Connect(){
                                ...
                                e.send(JSON.stringify({url: Link, name: "Burek", skin: 1, own: R, mode: {zombie: false, defense: false, multi: false, farm: false, chill: false, passive: false, gold: false, chat: false, sandbox: false}}));
console.log('Connecting bots');
                            ...
                            e.send(JSON.stringify({zombie: true, defense: false, multi: false, farm: false, chill: false, passive: false, gold: false, chat: false, sandbox: false}));
                        };
                    }, 500);
                };
            });
...
//    new lineBreak(ui);
    ...
    for(let i = 0; i < 10; i++) {
       ...
       if(e.key == "u" && document.activeElement.id.toLowerCase() !== 'chatbox' && document.getElementById("mainMenu").style.display == "none"){
           Removing = true;
           hasBot = false;
           r.send('ch', "Warning[Bots-Removed] </>");
       }
       if(e.key == "i" && document.activeElement.id.toLowerCase() !== 'chatbox' && document.getElementById("mainMenu").style.display == "none"){
           ...
           Bots(botCount, "afkbot-a");
           hasBot = true;
           r.send('ch', `Warning[Connect-[${botCount}]Bots] </>`);
       }
