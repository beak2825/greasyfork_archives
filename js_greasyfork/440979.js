...
// @downloadURL https://update.greasyfork.org/scripts/440979/%E6%88%98%E6%96%97%E5%90%A7%E6%AD%8C%E5%A7%AC%EF%BC%81%E3%80%90%E5%86%85%E7%BE%A4%E3%80%91.user.js
// @updateURL https://update.greasyfork.org/scripts/440979/%E6%88%98%E6%96%97%E5%90%A7%E6%AD%8C%E5%A7%AC%EF%BC%81%E3%80%90%E5%86%85%E7%BE%A4%E3%80%91.meta.js
// ==/UserScript==
/*
脚本框架不能加载时可尝试替换库源网址：
第12行（选一个）：
...
// @require      https://gitee.com/flyxiu/flyx/raw/master/ku/jQuery3.4.1.js
第27-30行：
// @require      https://gitee.com/flyxiu/flyx/raw/master/ku/libBilibiliToken_copy.js
// @require      https://gitee.com/flyxiu/flyx/raw/master/ku/libWasmHash.js
// @require      https://gitee.com/flyxiu/flyx/raw/master/ku/crypto-js.js
// @require      https://gitee.com/flyxiu/flyx/raw/master/ku/pako.min.js
*/
...
`);
                ...
                tj.find('div[data-toggle="lottery_result_uid_list"] [data-action="save1"]').click(async function () {
                    ...
                    for(let i =1;i<w_num[1]+1;i++){
                        jindu.innerHTML = `获取云数据：${lottery_result_data.length}`
                        let w_lottery_result_data = await getMyJson(`https://gitee.com/flyxiu/flyx/raw/master/lottery_result_data/1/${i}.json`);
                        await sleep(3000)
                        if(w_lottery_result_data[0]== undefined){
                            MY_API.chatLog(`【批量检查官方抽奖】无云数据或获取异常！`);
                        }else{
                        ...
                        return BAPI.get_sessions(end_ts).then(async function (data) {
                            if(data.code==0){
                                ...
                                for(let i=0;i<session_list.length;i++){
                                    ...
                                    if(ts_ms()*1000-session_list[i].session_ts < MY_API.CONFIG.auto_get_sessions_day * 24 * 3600 * 1000 * 1000)talker_id_list.push(session_list[i].talker_id)
                                }
                        ...
                        await BAPI.getMsg(talker_id_list[i]).then(async(data) => {//私信内容
                            if(data.code==0){
                                ...
                                for(let n=0;n<msg.length;n++){
                                    if(msg[n].timestamp > get_sessions_end_ts_last/1000 && msg[n].sender_uid==talker_id_list[i] && MY_API.CONFIG.get_sessions_keyword.some(v => msg[n].content.toLowerCase().indexOf(v) > -1) && ts_s()-msg[n].timestamp < MY_API.CONFIG.auto_get_sessions_day * 24 *3600 && !un_keyword.some(v => msg[n].content.toLowerCase().indexOf(v) > -1)){
                                        ...
                                        //私信推送点
                                        await sleep(2000)
                                        let msgmsg = `【私信提取推送】${msg[n].content}`
                                        ...
                                        if(MY_API.CONFIG.qqbot && qq_run_mark){
                                            qq(MY_API.CONFIG.qq,msgmsg,qun_server[1])
                                        }
...
4px;border: none;padding: 5px;cursor: pointer;box-shadow: 1px 1px 2px #00000075;">本地：${timestampToTime(ts_s())}</br>网络：${timestampToTime(ts_s_correct())}</button>`);
        ...
        let not_office_dynamic_do = async function (did) {
            ...
            await BAPI.dynamic_lottery_notice(did).then(async function (dat) {//判断是否是自动转发的未过期的抽奖，否，则加入中奖检查
                console.log('判断是否是自动转发的未过期的抽奖',dat)
                if(dat.code==0){
            ...
            if(lottery_time != undefined && lottery_time < ts_s())return
            if(breakmark)return
