...
// @grant		GM_info
// @run-at		document-end
// @downloadURL https://update.greasyfork.org/scripts/466026/MouseHunt%20AutoBot%20UPDATED.user.js
// @updateURL https://update.greasyfork.org/scripts/466026/MouseHunt%20AutoBot%20UPDATED.meta.js
// ==/UserScript==
...
function submitPuzzle(str) {
	var url = "/managers/ajax/users/puzzle.php";
	var param =`sn=Hitgrab&hg_is_ajax=1&action=solve&code=${str}&last_read_journal_entry_id=${lastReadJournalEntryId}&uh=${user.unique_hash}`;
		...
		if (request.readyState === 4)
		{
			if (request.status == 200)
			{
				try
				{
					...
					// document.getElementsByClassName('mousehuntPage-puzzle-form-description')[0].textContent += str + " - " + tmpRespondJSON.success;
					window.localStorage.UOP_puzzleLastResult = str + " - Attempt: " + kingsRewardRetry + " - Result: " + tmpRespondJSON.success;
					
					if(tmpRespondJSON.success){
                        kingsRewardRetry = 0;
			...
			else
			{
				...
				puzzleSubmitErrorHash = user.unique_hash;
...
function requestPuzzle() {
	var url = "/managers/ajax/users/puzzle.php";
	var param =`sn=Hitgrab&hg_is_ajax=1&action=request_new_code&last_read_journal_entry_id=${lastReadJournalEntryId}&uh=${user.unique_hash}`;
	var request = new XMLHttpRequest();
	request.open("POST", url, true);
	request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	request.onreadystatechange = function()
			...
			if (request.status == 200)
			{
				...
				if(tmpRespondJSON.success){
                    ...
                    window.setTimeout(function () {
                        CallKRSolver('/images/puzzleimage.php?t=' + new Date().getTime() + '&user_id=' + user.user_id + '&hash='+user.unique_hash);
                    }, 6000);
                }
			}
		}
...
function FinalizePuzzleImageAnswer(answer) {
    ...
    if (answer.length != 5) {
        ...
        submitPuzzle(answer.toLowerCase());
...
function CallKRSolver(src) {
    ...
    if (debugKR) {
        ...
        img = document.getElementsByClassName('puzzleView__imageContainer')[0];
        if (debug) console.log("Captcha Image fetched:")
        if (debug) console.log(img);

        frame.src = src? src: img.querySelector('img').src;
        /*} else {
...
function CheckKRAnswerCorrectness() {
    var puzzleForm = document.getElementsByClassName("puzzleView__formContainer")[0];
...
function countdownTimer() {
    if (isKingReward) {
        ...
        //TODO: Reload page to trigger KRSolver()
...
function afterSoundingHorn() {
    ...
    var huntersMessage = document.getElementsByClassName("huntersHornView__message--active");

    if(huntersMessage.length) {
        var huntersMessageTitle = document.getElementsByClassName('huntersHornMessageView__title')[0];
        if(huntersMessageTitle.textContent == "You have a King's Reward!") {
            location.reload(true);
        }
    }
...
function kingRewardAction() {
    ...
    document.getElementsByClassName('puzzleView__code')[0].focus();
    ...
    //         if (inputElementList[i].getAttribute('class') == "puzzleView__code") {
    ...
    if (isNewUI) {
        ...
        if (linkElementList) {
            ...
            for (i = 0; i < linkElementList.length; ++i) {
                // check if it is a resume button
                if (linkElementList[1].getAttribute('src') && linkElementList[i].getAttribute('src').indexOf("resume_hunting_blue.gif") != -1) {
                    // found resume button
    ...
    try {
        ...
        if (browser == 'chrome') {
            ...
            if (name == "user.next_activeturn_seconds") {
                ...
                return unsafeWindow.user.username;
            } else {
