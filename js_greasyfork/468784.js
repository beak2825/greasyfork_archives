// ==UserScript==
// @name         网赚盘
// @namespace    https://pandown.huajicloud.ml
// @version      1.5
// @description  支持快速下载网赚盘
// @author       Hoooooo
// @match        *://www.kufile.net/*
// @match        *://www.feimaoyun.com/*
// @match        *://h5.jingfile.com/*
// @match        *://www.xingyaopan.com/*
// @match        *://www.xywpan.com/*
// @match        *://www.567yun.cn/*
...
// @downloadURL https://update.greasyfork.org/scripts/468784/%E7%BD%91%E8%B5%9A%E7%9B%98.user.js
// @updateURL https://update.greasyfork.org/scripts/468784/%E7%BD%91%E8%B5%9A%E7%9B%98.meta.js
// ==/UserScript==
...
(function() {
	...
	tishi("建议收藏官网 防止失联 官网链接：https://pandown.huajicloud.ml 请手动复制本网站", "guanwang")
	...
	if (window.location.host == "www.kufile.net") {
		if (window.location.pathname.split("/")[1].indexOf("vip.php") == 0) {
			...
			downurl = "https://pandown.huajicloud.ml/api/kufile?file=" + fid
	...
	if (window.location.host == "www.feimaoyun.com") {
		tishi("维护中", "feimaotishi")
		return;
	}
	//判断网盘 并执行相应事件
	if (window.location.host == "www.xingyaopan.com" || window.location.host == "www.xywpan.com") {
		if (window.location.pathname.split("/")[1].indexOf("vip.php") == 0) {
			document.querySelector("html")
				.innerHTML = "<head></head><body><p><font size=\"24\"><font color=\"#FF0000\">本人技术有限 无法屏蔽弹出页面 弹出本页面属于正常现象 请手动关闭</font> </font></p><br><br><p></p></body>";
		} else {
			...
			downurl = "https://pandown.huajicloud.ml/api/starpan?file=" + fid
			getdown.open("get", downurl);
			getdown.send(null);
		}
		return;
	...
	//判断网盘 并执行相应事件
	if (window.location.host == "www.567yun.cn" || window.location.host == "www.rarwp.com") {
		if (window.location.pathname.split("/")[1].indexOf("vip.php") == 0) {
			document.querySelector("html")
				.innerHTML = "<head></head><body><p><font size=\"24\"><font color=\"#FF0000\">本人技术有限 无法屏蔽弹出页面 弹出本页面属于正常现象 请手动关闭</font> </font></p><br><br><p></p></body>";
		} else {
			...
			downurl = "https://pandown.huajicloud.ml/api/567yun?file=" + fid
			getdown.open("get", downurl);
			getdown.send(null);
		}
		return;
	...
	//判断网盘 并执行相应事件
	if (window.location.host == "www.5678yun.cn" || window.location.host == "www.567file.com") {
		...
		downurl = "https://pandown.huajicloud.ml/api/5678yun?file=" + fid
		getdown.open("get", downurl);
		getdown.send(null);
		return;
	}
	...
	if (window.location.host == "www.expfile.com") {
		...
		downurl = "https://pandown.huajicloud.ml/api/expfile?file=" + fid
		getdown.open("get", downurl);
		getdown.send(null);
		return;
	}
	...
	const action_id = {
		...
		'www.567yun.cn': 'load_down_addr10',
		...
		"www.52-vip.com": "load_down_addr1",
	}
	...
	if (typeof(getAction()) != "undefined") {
		if (/www.77file.com\/wap.php/.test(window.location.href)) {
			...
			document.getElementsByClassName("dump")[3].onclick = function() {
				window.open('https://github.com/huajiqaq/link-helper/issues')
			}
