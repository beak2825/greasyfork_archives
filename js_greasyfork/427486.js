...
// @author          Duke
// @description     国抽平台功能增强-普通
// @include         https://spcjinsp.gsxt.gov.cn/test_platform/foodTest/search/0?v=4.0
...
// @version         1.3.2
// @grant           none
// @downloadURL https://update.greasyfork.org/scripts/427486/%E5%9B%BD%E6%8A%BD%E5%B9%B3%E5%8F%B0%E5%8A%9F%E8%83%BD%E5%A2%9E%E5%BC%BA-%E6%99%AE%E9%80%9A%E9%A3%9F%E5%93%81.user.js
// @updateURL https://update.greasyfork.org/scripts/427486/%E5%9B%BD%E6%8A%BD%E5%B9%B3%E5%8F%B0%E5%8A%9F%E8%83%BD%E5%A2%9E%E5%BC%BA-%E6%99%AE%E9%80%9A%E9%A3%9F%E5%93%81.meta.js
// ==/UserScript==
...
(function() {
    ...
    function editAll(arr,issave){
        function getSampleid(XMDATA,num) {
            ...
            var theURL = 'https://spcjinsp.gsxt.gov.cn/test_platform/api/food/getFood?order=desc&offset=0&limit=10&dataType=1&startDate=' + startYear + '-01-01&endDate=' + formatDate(myDate) + '&taskFrom=&samplingUnit=&testUnit=&enterprise=&sampledUnit=&foodName=&province=&reportNo=&bsfla=&bsflb=&sampleNo=' +  XMDATA["抽样编号"] + '&foodType1=&foodType4=&sampleNo_index=1&_=' + myDate.getTime();
                    ...
                    if (httpRequest.readyState == 4 && httpRequest.status == 200) {
                        ...
                        if(json.length!=0){
                            ...
                            if(!json.match(reg0)){
                                ...
                                //打开填报页面
                                parent.addTabs("/test_platform/foodTest/foodDetail/" + id_sampleNo[0],id_sampleNo[1]);
                                ...
                                load_timeout=setTimeout(function(){
                                    ...
                                    for(var m = 0;m < XMDATA.testitem.length; m++){
                                        var xmmc = XMDATA.testitem[m]["检验项目"];
                                        resarr.push([num,"页面打开超时" +  XMDATA["抽样编号"],xmmc]);
                                    }
                                    //关闭当前页
                                ...
                                $('iframe[name=' + name1 + ']',parent.document).load(function () {
                                    ...
                                    //gettestinfo获取成功
                                    this.contentWindow.$(this.contentWindow.document).ajaxComplete(function (event, request, settings) {
                                        if ( settings.url == "/test_platform/api/food/getTestInfo" ) {
                                            console.log('Complete_yes');
                                            //清除超时
                                            ...
                                            setTimeout(function(){
                                                ...
                                                for(var m = 0;m < XMDATA.testitem.length;m++){
                                                    ...
                                                    if($('iframe[name=' + name1 + ']',parent.document).contents().find('body').find('input[value="' + xmmc +'"]').length < 1) {
                                                        ...
                                                        var str11 = $('iframe[name=' + name1 + ']',parent.document).contents().find('body').find('input[value="' + xmmc +'"][type="text"]').attr("id");
                                                        var reg2='([\\s\\S]*?)_item';
        ...
        async function getAllSampleid(arr1) {
            ...
            setTimeout(function(){
                $('a[data-id="/test_platform/foodTest/search/0"]',parent.document).find(".fa-times-circle").click();
            },3000)
        }
        getAllSampleid(arr);
    };
    ...
    $("#open-btn").click(function () {
        ...
        function openSelectedItem(sampleId,sampleNo) {
            return new Promise((resolve) => {
                foodManager.openDetail(sampleId,sampleNo);
                var name1 = $(".J_mainContent iframe:visible", parent.document).attr("name");
                $('iframe[name=' + name1 + ']',parent.document).load(function () {
                    setTimeout(function(){
                        resolve();
