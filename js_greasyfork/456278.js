...
// @grant        none
// @unwrap
// @downloadURL https://update.greasyfork.org/scripts/456278/Bonk%20Commands.user.js
// @updateURL https://update.greasyfork.org/scripts/456278/Bonk%20Commands.meta.js
// ==/UserScript==
...
if(typeof(scope.originalmanualscroll)=='undefined'){scope.originalmanualscroll = null;}
...
Gdocument.getElementById("ingamechatinputtext").style["display"] = "";};
    ...
    debuggereval.addEventListener("keypress",function(e){if(e.repeat){return;}if(e.code == "Enter"){if(debuggersendrecieve.value == 0){SEND(this.value);}else{RECIEVE(this.value);}}});
...
Gwindow.XMLHttpRequest.prototype.send = function(data) {
    ...
    if (this.isSearchMap) {
        this.onreadystatechange = function () {
            if (this.readyState == 4 && searchrequested) {
...
Gwindow.WebSocket.prototype.send = function(args) {
    if(this.url.includes(".bonk.io/socket.io/?EIO=3&transport=websocket&sid=")){
        ...
        this.onmessage = function(args){return;};
        return;
...
scope.urlify = function(text) {
    return text.replace(/(?:https?:\/\/)?(?:[A-Za-z0-9-ßàÁâãóôþüúðæåïçèõöÿýòäœêëìíøùîûñé]+)(?:\.[A-Za-z0-9-ßàÁâãóôþüúðæåïçèõöÿýòäœêëìíøùîûñé]+)+(?:\/(?:[A-Za-z0-9-._~:/?#\[\]@!$&'()*+,;=]|%[0-9a-fA-F]{2})*)?(?:\?(?:[^=]+=[^&](?:&[^=]+=[^&])*)?)?/g, function(url) {
        if(url.startsWith('https://')){return '<a href="' + url + '" target="_blank" style = "color:orange">' + url + '</a>';}
...
scope.commandhandle = function(chat_val){
    if (chat_val.substring(1,6)=="echo " && chat_val.replace(/^\s+|\s+$/g, '').length>=7){
    ...
    else if (chat_val.substring(1,6)=="audio"){
        if(chat_val.substring(6,9)=="add"){
            audio = new Audio(chat_val.substring(10))
            displayInChat(audio.src + ' added.')
            return "";
        }
        else if(chat_val.substring(6)=="play"){
            audio.play()
            displayInChat('Sound played.')
            return "";
        }
        else if(chat_val.substring(6)=="loop"){
            audio.loop = !audio.loop
            if(audio.loop){
                displayInChat('Sound looped.')
            }
            else{
            displayInChat('Sound unlooped.')
            }
            return "";
        }
        else if(chat_val.substring(6,10)=="seek"){
            audio.currentTime = chat_val.substring(11,chat_val.indexOf(':'))*60+(+chat_val.substring(chat_val.indexOf(':')+1))
            displayInChat('Seeked to '+chat_val.substring(11)+'.')
            return "";
        }
        else if(chat_val.substring(6,11)=="speed"){
            audio.playbackRate = chat_val.substring(12)
            displayInChat('Speed set to '+chat_val.substring(12)+'.')
            return "";
        }
        else if(chat_val.substring(6,11)=="pitch"){
            if(chat_val.substring(12)!="1"&&chat_val.substring(12)!="0"){
                displayInChat('1 for true or 0 for false.')
                return "";
            }
            else{
                audio.preservesPitch = !chat_val.substring(12)
                displayInChat('Pitch set to '+(chat_val.substring=='1'?"false":"true")+".")
                return "";
            }
        }
        else if(chat_val.substring(6)=="pause"){
            audio.pause()
            displayInChat('Sound paused.')
            return "";
        }
    }
        ...
        if(scope.comandosplus){
            ...
            chat(flag_manage('Commands disabled.'))
            return "";
        }
    }
...
function timeout123() {
    updateWssLog();
    if(originalmanualscroll == null){
        originalmanualscroll = Gdocument.getElementById("maploadwindowmapscontainer").onscroll;
    }
    if(Gdocument.getElementById("maploadwindowmapscontainer").onscroll != null){
        if(Gdocument.getElementById("maploadwindowmapscontainer").onscroll.toString() == originalmanualscroll.toString()){
            Gdocument.getElementById("maploadwindowmapscontainer").onscroll = function(...arguments) {
                if(Gdocument.getElementById("maploadtypedropdowntitle").textContent == "MAP REQUESTS"){
                    return;
                }
                return originalmanualscroll.call(this,...arguments);
            }
        }
    }
    if(ishost && Gdocument.getElementById("maploadtypedropdowntitle").textContent == "MAP REQUESTS"){
    ...
    if(new_message){
        ...
        if(scope.comandosplus){
            if(Last_message.substring(Last_message.substring(0,Last_message.indexOf(':')).length+3,Last_message.substring(0,Last_message.indexOf(':')).length+14).toLowerCase() == "!percentage"){
                var a=Last_message.substring(Last_message.indexOf('(')+1,Last_message.indexOf(')'))
                var b=Last_message.substring(Last_message.lastIndexOf('(')+1,Last_message.lastIndexOf(')'))
            ...
            if(Last_message.substring(Last_message.substring(0,Last_message.indexOf(':')).length+3,Last_message.substring(0,Last_message.indexOf(':')).length+7).toLowerCase() == "!say"){
                if(Last_message.substring(0,Last_message.indexOf(':')).toLowerCase() != username.toLowerCase()){
            ...
            if(Last_message.startsWith("* ")&&Last_message.endsWith("has joined the game")){
                chat('Hello, '+Last_message.substring(2,Last_message.length-20)+'! Type !help to continue.')
            }
        }
