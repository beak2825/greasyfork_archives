...
// @version     1.0.5
// @downloadURL https://update.greasyfork.org/scripts/438831/GN_TradeHelper.user.js
// @updateURL https://update.greasyfork.org/scripts/438831/GN_TradeHelper.meta.js
// ==/UserScript==
...
//----------------------------------------------------------------------------//
    ...
    function get_obj_prices(obj){
        ...
        xhr.onreadystatechange = function(){
            if(xhr.readyState == 4)
                if(xhr.status == 200){
                    let obj_id = obj.value.includes("@")?obj.value.split("@")[0]:null
                    var objects = parse_page(xhr.response, obj.type, obj_id);
                    if(!objects.length)
                        remove_table();
                    else{
                        draw_lot_table(objects, obj.type);
                        set_form_values(objects, obj, obj_id);
                    }
...
//----------------------------------------------------------------------------//
    ...
    function parse_page(response, type, id){
        ...
        switch(type){
            ...
            case enum_ot.art:
                return get_art_objects(objects, id);
...
//----------------------------------------------------------------------------//
    ...
    function getOptPrice(cur_dur, max_dur, repair, price) {
        let dur1, totalAmountOfBattles = parseInt(cur_dur);
        let dur2 = parseInt(max_dur);
        let repairInt = parseInt(repair);
        let totalPrice = parseFloat(price);
        let optPrice = totalPrice / totalAmountOfBattles;
        while (true) {
            dur1 = Math.floor(dur2 * 0.9);
            dur2 -= 1;
            totalAmountOfBattles += dur1;
            totalPrice += repairInt * 1.01 * 1.01;
            let tempOptPrice = totalPrice / totalAmountOfBattles;
            if (tempOptPrice > optPrice) {
                return optPrice.toFixed(2)
            }
            optPrice = tempOptPrice;
        }
    }

    function get_art_objects(objects, id){
        var cleared_objects = [];
        ...
        objects.forEach(function(current){
            ...
            if(matches){
                ...
                obj.ppb = getOptPrice(obj.cur_dur, obj.max_dur, CV.get_artefact(id).repair_cost, obj.price)
...
//----------------------------------------------------------------------------//
    ...
    function remove_table(){
        var parent = document.querySelector('form[name="f"]');
...
//----------------------------------------------------------------------------//
    ...
    function set_form_values(objects, obj, obj_id){
        if(obj.type == enum_ot.any)
        ...
        switch(obj.type){
            ...
            case enum_ot.art:
                var select = document.querySelector('select[name="item"]');
                var option = select.options[select.selectedIndex];
                var cur_toughness = option.textContent.match(/(\d{1,3})\/(\d{1,3})/);
                let cur_dur = cur_toughness[1]-0
                let max_dur = cur_toughness[2]-0
                let repair_price = CV.get_artefact(obj_id).repair_cost
                let item_price = smith(cur_dur, max_dur, repair_price, objects[0].ppb - 0.1).zena
                price_el.value = parseInt(item_price) - 2;

                let change_ppb_counter = 0;
                let change_ppb_container = $("change_ppb_container")
                if (change_ppb_container) {
                    change_ppb_container.remove()
                }
                price_el.insertAdjacentHTML("afterend", `
                <div style="display: flex" id="change_ppb_container">
                    <div>ЦЗБ:</div>
                    <div id="curr_ppb">${objects[0].ppb - 0.1}</div>
                    <div id="minus_ppb"><div style="background-color: red; border: 1px solid black; border-radius: 5px; cursor: pointer; color: white; font-weight: bold;">-1 ЦЗБ</div></div>
                    <div id="plus_ppb"><div style="background-color: green; border: 1px solid black; border-radius: 5px; cursor: pointer; color: white; font-weight: bold;">+1 ЦЗБ</div></div>
                </div>
                `)
                $("minus_ppb").addEventListener("click", () => {
                    change_ppb_counter--
                    item_price = smith(cur_dur, max_dur, repair_price, objects[0].ppb - 0.1 + change_ppb_counter).zena
                    price_el.value = parseInt(item_price) - 2;
                    $("curr_ppb").innerText = objects[0].ppb - 0.1 + change_ppb_counter
                })
                $("plus_ppb").addEventListener("click", () => {
                    change_ppb_counter++
                    item_price = smith(cur_dur, max_dur, repair_price, objects[0].ppb - 0.1 + change_ppb_counter).zena
                    price_el.value = parseInt(item_price) - 2;
                    $("curr_ppb").innerText = objects[0].ppb - 0.1 + change_ppb_counter
