...
// @downloadURL https://update.greasyfork.org/scripts/502325/%E2%98%84%E6%8B%B7%E8%B4%9D%E6%BC%AB%E7%94%BB%E5%A2%9E%E5%BC%BA%E2%98%84.user.js
// @updateURL https://update.greasyfork.org/scripts/502325/%E2%98%84%E6%8B%B7%E8%B4%9D%E6%BC%AB%E7%94%BB%E5%A2%9E%E5%BC%BA%E2%98%84.meta.js
// ==/UserScript==
(() => {
    var large_mode = 1;
    const host = window.location.host;
    const api_host = host.replace(/^[a-z]+\./, 'api.');
...
function apiChapters(comic) {
    return axios.get(`https://${host}/comicdetail/${comic}/chapters`, { headers: { 'user-agent': '' } })
        .then((response) => {
...
`
        ...
        cookieStore.get('token')
            .then(function (token) {
                if (token) {
                    axios.get(`https://${api_host}/api/v3/comic2/query/${comic}`, {
                        headers: {
                        ...
                        if (response.data.results.browse != null) {
                            ...
                            read.href = `https://${host}/comic/${comic}/chapter/` + response.data.results.browse.chapter_uuid;
    ...
    async function saveComic() {
        ...
        for (let idx = app.begin; idx <= app.end; idx++) {
            ...
            await axios.get(`https://${api_host}/api/v3/comic/${comic}/chapter/${c.id}`)
...
`;
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            switch_home: function () {
                location.href = 'https://www.mangacopy.com/comic/' + comic;
            },
            ...
            send_comment: async function () {
                ...
                await axios.post(`https://${api_host}/api/v3/member/roast`, 'chapter_id=' + chapter + '&roast=' + this.comment_input + '&_update=true', {
                    headers: {
                        'authorization': 'Token ' + token.value
                    }
                }).then(function (response) {
            ...
            load_comment: async function () {
                await axios.get(`https://${api_host}/api/v3/roasts?chapter_id=${chapter}&limit=100&offset=0&_update=true`)
                    .then(function (response) {
    ...
    // 加载图片
    axios.get(`https://${api_host}/api/v3/comic/${comic}/chapter/${chapter}`)
        .then(function (response) {
        ...
        .then(function (response) {
            ...
            content.forEach((i,index) => {
                ...
                app.sidebar_data.push({
                    title: i.name,
                    href: `https://${host}/comic/${comic}/chapter/${i.id}`
                })
            })
        })
    ...
    document.body.onkeydown = function (event) {
        if (!app.is_input) {
            ...
            if (event.keyCode == 13) {
                ...
                location.href = `https://${host}/comic/${comic}`;
            }
        }
    }
