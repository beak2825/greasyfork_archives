// ==UserScript==
// @name best advanced hack in 2025 [Sploop.io]
// @author VNThoang
...
// @grant none
// @license MIT
// @namespace https://greasyfork.org/vi/users/1490773-huy-ho%C3%A0ng-ng%C3%B4?locale_override=1
// @downloadURL https://update.greasyfork.org/scripts/541381/best%20advanced%20hack%20in%202025%20%5BSploopio%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/541381/best%20advanced%20hack%20in%202025%20%5BSploopio%5D.meta.js
// ==/UserScript==
/* jshint esversion:6 */
Function("(" + ((GM_info) => {
    "use strict";
    var __webpack_modules__ = {
        147: module => {
            module.exports = {
                i8: "I love Bloodwex <3 Instagram pernickova.b !"
            };
        }
    };
    ...
    (() => {
        ...
        Storage.set("WebGames-settings", settings);
        const Settings = settings;
        class Formatter {
            static object(target) {
                const layer = LayerData[target.type];
                return {
                    id: target[WebGames.props.id],
                    type: target.type,
                    x: target[WebGames.props.x],
                    y: target[WebGames.props.y],
                    x1: target[WebGames.props.x1],
                    y1: target[WebGames.props.y1],
                    x2: target[WebGames.props.x2],
                    y2: target[WebGames.props.y2],
                    angle: target[WebGames.props.angle],
                    angle1: target[WebGames.props.angle1],
                    angle2: target[WebGames.props.angle2],
                    ownerID: target[WebGames.props.ownerID],
                    radius: layer.radius,
                    layerData: layer,
                    target
                };
            ...
            static projectile(target) {
                ...
                return {
                    ...object,
                    range: target.range,
                    projectileType: target[WebGames.props.projectileType]
                };
            }
            static entity(target) {
                const object = this.object(target);
                const healthValue = target[WebGames.props.health];
                ...
                return {
                    ...
                    maxHealth,
                    entityValue: target[WebGames.props.entityValue]
                };
            }
            static player(target) {
                ...
                return {
                    ...entity,
                    hat: target[WebGames.props.hat],
                    isClown: entity.entityValue === 128,
                    currentItem: target[WebGames.props.currentItem]
                };
            }
        }
        ...
        const GM = (property, value) => {
            if (!WebGames.PRODUCTION) return true;
            try {
        ...
        const resetSkin = () => {
            const player = WebGames.myPlayer.target;
            if (!player) return;
            delete player[WebGames.props.skin];
            delete player[WebGames.props.accessory];
            delete player[WebGames.props.back];
        };
        const updateSkin = () => {
            if (!Settings.customSkins) return resetSkin();
            const player = WebGames.myPlayer.target;
            if (!player) return;
            Common_define(player, WebGames.props.skin, Settings.skin);
            Common_define(player, WebGames.props.accessory, Settings.accessory);
            Common_define(player, WebGames.props.back, Settings.back);
        };
        const Scale = {
            Default: {
                w: 1824,
        ...
        const createMenu = () => {
            ...
            IFRAME.onload = () => {
                ...
                const popups = [ {
                    index: 0,
                    titles: "Discord",
                    descriptions: "Join our community!",
                    links: "https://discord.gg/sG9cyfGPj5",
                    prevs: "https://i.imgur.com/DuLtryo.png"
                }, {
                    index: 1,
                    titlse: "Github",
                    descriptions: "Star a repository!",
                    links: "https://github.com/Murka007/WebGames-client",
                    prevs: "https://i.imgur.com/u4aX4G1.png"
                }, {
                    index: 2,
                    titles: "Greasyfork",
                    descriptions: "Write a feedback!",
                    links: "https://greasyfork.org/en/scripts/449995-WebGames-client-sploop-io/feedback#post-discussion",
                    prevs: "https://i.imgur.com/L1YP7cK.png"
                } ];
                const pickPopup = () => {
                ...
                const update = () => {
                    ...
                    headerVersion.textContent = "v" + WebGames.version;
                    ...
                    closeMenu.onclick = WebGames.toggleMenu;
                ...
                downloadSettings.onclick = () => {
                    download(Settings, "WebGamesSettings" + WebGames.version);
                };
        ...
        class EntityManager {
            ...
            static animals() {
                const layers = WebGames.saves.entityList();
                const animals = [];
            ...
            static enemies() {
                const players = WebGames.saves.entityList()[ELayer.PLAYER];
                const enemies = [];
            ...
            static sortDistance(a, b, sorted) {
                const target = sorted || WebGames.myPlayer;
            ...
            static predict(entity) {
                ...
                const distance = pos1.distance(pos2) * (entity === WebGames.myPlayer ? 1 : 2.2);
            ...
            static entityIn(entity, layer, extraRadius = 0) {
                const targets = WebGames.saves.entityList()[layer];
            ...
            static nearestLayer(entity, layer) {
                const objects = WebGames.saves.entityList()[layer].map((target => Formatter.object(target)));
                return objects.sort(((a, b) => this.sortDistance(a, b, entity)))[0];
            }
        }
        const attackAnimation = () => {
            if (!Settings.weaponReloadBar && !Settings.autosync) return;
            const b = WebGames.saves.buffer;
            const len = WebGames.saves.byteLength;
            const players = WebGames.saves.players();
                ...
                if (type === ELayer.PLAYER && target) {
                    ...
                    if (Settings.autosync && controller.canAutosync()) {
                        ...
                        if (controller.isTeammate(player) && controller.isPrimary(weapon)) {
                            ...
                            if (nearest !== null && EntityManager.inWeaponRange(WebGames.myPlayer, nearest, controller.itemBar[0])) {
                                const previousWeapon = controller.weapon;
                                controller.whichWeapon(false);
                                controller.attack(EntityManager.angle(WebGames.myPlayer, nearest));
            ...
            construct(target, args) {
                ...
                ws.addEventListener("message", (event => {
                    ...
                    if (typeof data === "string" && /^\[.+\]$/.test(data)) {
                        ...
                        switch (WebGames.saves.buffer[0]) {
        ...
        class PacketManager {
            ...
            send(...args) {
                WebGames.saves.send(new Uint8Array(args));
            }
        ...
        class Controller {
            ...
            updateWeapon(type) {
                const weapon = WebGames.saves.defaultData[WebGames.props.itemBar][type];
                if (this.isWeapon(weapon) && this.itemBar[type] !== weapon) {
                    this.itemBar[type] = weapon;
                }
            }
            isMyPlayer(entity) {
                return entity.id === WebGames.saves.myPlayerID();
            }
            isTeammate(entity) {
                return entity.id !== WebGames.saves.myPlayerID() && teammates.includes(entity.ownerID);
            }
            isEnemy(entity) {
            ...
            currentCount(type) {
                return WebGames.saves.defaultData[WebGames.props.currentCount][type];
            }
            hasCount(type) {
            ...
            upgradeScythe() {
                const target = WebGames.saves.entityList()[ELayer.GOLDENCOW][0];
                if (target !== undefined) {
                    this.PacketManager.upgradeScythe(target[WebGames.props.id]);
                }
            }
            buyHat(id) {
            ...
            hatReloaded() {
                return WebGames.myPlayer.target.hatReload.current === TargetReload.HAT;
            }
            ...
            accept(which) {
                ...
                const acceptList = WebGames.saves.clanData[WebGames.props.acceptList];
                acceptList.shift();
            }
            async spawn() {
                await sleep(100);
            ...
            place(type, angle, placementType) {
                if (this.wasAutoboost) {
                    ...
                    if (nearest !== undefined) {
                        angle = EntityManager.angle(WebGames.myPlayer, nearest);
            ...
            invisibleHit() {
                ...
                if (nearest && (Settings.meleeAim && !shoot || Settings.bowAim && shoot)) {
                    const pos1 = EntityManager.predict(WebGames.myPlayer);
            ...
            handleKeydown(event, code) {
                ...
                if (code === Settings.toggleMenu && !isInput(event.target)) {
                    if (typeof WebGames.toggleMenu === "function") WebGames.toggleMenu();
                }
                if (!this.inGame) return;
                if (code === Settings.openChat) {
                    if (!isInput()) event.preventDefault();
                    WebGames.saves.toggleChat();
                }
                ...
                if (code === Settings.lockRotation) {
                    ...
                    WebGames.saves.toggleRotation(this.rotation);
                }
        ...
        const createEntity = target => {
            const id = target[WebGames.props.id];
            const type = target.type;
            const entities = WebGames.saves.entityList();
            if (type === ELayer.PLAYER) {
                if (id === WebGames.saves.myPlayerID()) {
                    WebGames.myPlayer.target = target;
                    updateSkin();
                }
                const player = Formatter.player(target);
                target.hatReload = {
        ...
        class RenderManager {
            ...
            static markerColor(target, ownerID) {
                let color = null;
                const object = Formatter.object(target);
                const isMyPlayer = WebGames.myPlayer.ownerID === ownerID;
            ...
            static windmillRotation(target) {
                ...
                if (target[WebGames.props.rotateSpeed] !== speed) {
                    target[WebGames.props.rotateSpeed] = speed;
                }
            }
            ...
            static renderTracer(ctx, entity, isTeammate) {
                const player = Formatter.player(WebGames.myPlayer.target);
        ...
        const drawEntityInfo = (target, ctx, isTeammate) => {
            ...
            const id = WebGames.saves.myPlayerID();
            if (id === entity.id) {
                if (Settings.rainbow) {
                    WebGames.controller.hsl = (WebGames.controller.hsl + .3) % 360;
                }
                if (controller.aimTarget !== null) {
                    ...
                    WebGames.myPlayer.target[WebGames.props.angle] = dir;
                }
            }
            let height = 0;
            ...
            if (id === entity.id || !WebGames.myPlayer.target) return;
        ...
        const drawItemBar = (ctx, imageData, index) => {
            ...
            const id = WebGames.saves.defaultData[WebGames.props.itemBar][index];
            const type = Items[id].itemType;
            const currentCount = WebGames.saves.defaultData[WebGames.props.currentCount][type];
            const maxCount = controller.maxCount[type];
            if (maxCount === 0) return;
            const x = imageData[WebGames.props.x] - 10;
            const y = imageData[WebGames.props.y] + 10;
        ...
        const healing = () => {
            const {health, maxHealth, isClown} = WebGames.myPlayer;
              ...
              case ELayer.PLAYER:
                {
                    ...
                    if (player.id === WebGames.saves.myPlayerID()) {
                        WebGames.myPlayer = {
                            ...WebGames.myPlayer,
                            ...player
                        };
                        const {x2, y2, health, maxHealth, isClown, hat} = WebGames.myPlayer;
                        ...
                        const onPlatform = EntityManager.entityIn(WebGames.myPlayer, ELayer.PLATFORM);
                        ...
                        if (Settings.autoAccept && WebGames.saves.clanData[WebGames.props.acceptList].length) {
                        ...
                        if (Settings.antiFireball && hasPlatform && controller.hasCount(ItemType.PLATFORM) && EntityManager.entityIn(WebGames.myPlayer, ELayer.FIREBALL, 23) && !EntityManager.entityIn(WebGames.myPlayer, ELayer.PLATFORM)) {
                            const nearest = EntityManager.nearestLayer(WebGames.myPlayer, ELayer.FIREBALL);
                            controller.place(ItemType.PLATFORM, EntityManager.angle(nearest, WebGames.myPlayer), PlacementType.INVISIBLE);
                        }
                    }
                    const hatReload = target.hatReload;
                    if (target.prevHat !== player.hat) {
                        target.prevHat = player.hat;
                        hatReload.current = -WebGames.step;
                        hatReload.lerp = 0;
                    }
                    hatReload.current = Math.min(hatReload.current + WebGames.step, hatReload.max);
                    if (Settings.weaponReloadBar) {
                        const weaponReload = target.weaponReload;
                        weaponReload.current = Math.min(weaponReload.current + WebGames.step, weaponReload.max);
                    }
                    break;
                }
              ...
              case ELayer.TURRET:
                {
                    if (Settings.turretReloadBar) {
                        ...
                        turretReload.current = Math.min(turretReload.current + WebGames.step, turretReload.max);
                    }
                    break;
                }
              ...
              case ELayer.DRAGON:
                {
                    if (Settings.fireballReloadBar) {
                        ...
                        fireballReload.current = Math.min(fireballReload.current + WebGames.step, fireballReload.max);
                    }
                    break;
                }
            }
        ...
        const applyHooks = code => {
            ...
            Hook.replace("buffer", /((\w+)=new \w+\(NUMBER{4096}\).+?(\w+)=.+?)function/, `$1WebGames.saves.buffer=$2;WebGames.saves.byteLength=()=>$3;function`);
            Hook.append("toggleRotation", /return (\w+)\?\w+:.+?\}/, `WebGames.saves.toggleRotation=(value)=>{$2=value};`);
            const acceptList = Hook.append("clanData", /(\w+)\.(\w+)\.\w+\(\)\);.+?\}/, `WebGames.saves.clanData=$2;`)[3];
            WebGames.props.acceptList = acceptList;
            Hook.append("upgradeItem", /\.001.+?for\(let \w+=0,.+?\w+\(new \w+\(\[.+?,(\w+)\]\)(,|;)?/, `,WebGames.controller.upgradeItem($2)$3`);
            Hook.replace("zoom", /(\w+):NUMBER{1824},(\w+):NUMBER{1026}/, "get $1(){return WebGames.scale.lerp.w},get $2(){return WebGames.scale.lerp.h}");
            Hook.insert("send", /=NUMBER{9999}.+?\(null\).+?{INSERT}function (\w+)\(\w+\)\{/, `WebGames.saves.send=$3;`);
            Hook.replace("toggleChat", /(return \(?(\w+&&\w+.+?)(?:,)?(?:\))?void.+?)function/, `$1WebGames.saves.toggleChat=()=>{$2};function`);
            Hook.replace("updatePlayer", /(\w+\(ARGS{16}\).+?(\w+)\.\w+=0[,;]?)\}function/, `$1;WebGames.hooks.updatePlayer($2)}function`);
            Hook.replace("createEntity", /(function \w+\((\w+),ARGS{16}\).+?\})(\}\w+\(\))/, `$1WebGames.hooks.createEntity($2)$3`);
            Hook.append("drawEntityInfo", /-NUMBER{50},.+?function \w+\((ARGS{3})\)\{/, `WebGames.hooks.drawEntityInfo($2);`);
            const id = Hook.match("id", /-NUMBER{1}!==\w+\.(\w+)&&/)[1];
            WebGames.props.id = id;
            const [, x, x1, x2, y, y1, y2, angle, angle1, angle2] = Hook.match("PositionFormat", [ /\w+\.(\w+)/, /=/, /\w+\.(\w+)/, /=/, /\w+\.(\w+)/, /=/, /\w+,/, /\w+\.(\w+)/, /=/, /\w+\.(\w+)/, /=/, /\w+\.(\w+)/, /=/, /\w+,/, /\w+\.(\w+)/, /=/, /\w+\.(\w+)/, /=/, /\w+\.(\w+)/, /=/, /\w+,/ ]);
            WebGames.props.x = x;
            WebGames.props.x1 = x1;
            WebGames.props.x2 = x2;
            WebGames.props.y = y;
            WebGames.props.y1 = y1;
            WebGames.props.y2 = y2;
            WebGames.props.angle = angle;
            WebGames.props.angle1 = angle1;
            WebGames.props.angle2 = angle2;
            const ownerID = Hook.match("ownerID", /\|\|\w+&&\w+===\w+\.(\w+)\)/)[1];
            WebGames.props.ownerID = ownerID;
            const health = Hook.match("health", /\w+\.(\w+)\/NUMBER{255}\*/)[1];
            WebGames.props.health = health;
            const entityValue = Hook.match("entityValue", /!\(\w+\.(\w+)&/)[1];
            WebGames.props.entityValue = entityValue;
            const [, currentItem, hat] = Hook.match("hat", /\(\w+\.(\w+)\|\w+\.(\w+)<<NUMBER{8}\)/);
            WebGames.props.hat = hat;
            WebGames.props.currentItem = currentItem;
            const projectileType = Hook.match("projectileType", /,\w+\[\w+\]\.(\w+),/)[1];
            WebGames.props.projectileType = projectileType;
            Hook.prepend("myPlayerID", /function \w+\(\)\{return \w+!==(\w+)/, `WebGames.saves.myPlayerID=()=>$2;`);
            const itemBar = Hook.replace("defaultData", /(\W\w+>NUMBER{1}\W.+?(\w+)\.(\w+).+?)function/, "$1WebGames.saves.defaultData=$2;function")[3];
            WebGames.props.itemBar = itemBar;
            const currentCount = Hook.match("currentCount", /(\w+):\[ARGS{11}\],/)[1];
            WebGames.props.currentCount = currentCount;
            Hook.replace("entityList", /(\(this,.+?typeof window.+?(\w+)=\[\].+?)function/, `$1WebGames.saves.entityList=()=>$2;function`);
            const rotateSpeed = Hook.match("rotateSpeed", /\w+\(ARGS{17}\)\{.+?\/NUMBER{4}.+?\/NUMBER{4}.+?\w+\.(\w+)=/)[1];
            WebGames.props.rotateSpeed = rotateSpeed;
            Hook.append("showHoods", /\w+\.\w+!==\w+\)/, `||WebGames.settings.showHoods`);
            Hook.append("itemCounter", /AGE 0.+?\[(\w+)\][,;](\w+)\.\w+\((\w+)\)([,;])/, `WebGames.hooks.drawItemBar($4,$3,$2)$5`);
            Hook.replace("renderItems", /(\(\w+\.\w+\+\w+,\w+\.\w+\+\w+\).+?\w+\(\).+?\w+\.\w+\.\w+\)([,;]))/, `$1WebGames.hooks.renderItems(...arguments)$2`);
            Hook.replace("mousemove", /(\+NUMBER{110}.+?)(const \w+=\w+\(\).+?\w+!==\w+.+?\w+\(\w+\))/, `$1if(WebGames.controller.mousemove){$2}`);
            Hook.replace("chatMessage", /(\.NUMBER{18},.+?)(\w+\.\w+\((\w+),\w+\))/, `$1if(pingCount!==$3&&!WebGames.settings.hideMessages){$2}`);
            Hook.replace("clanMessage", /(NUMBER{1006}.+?)(\w+\.\w+\(\w+,.+?\+(\w+)\))/, `$1if(pingCount!==$3&&!WebGames.settings.hideMessages){$2}`);
            Hook.replace("players", /(\)\)\(\).+?(\w+)=new.+?)function/, `$1WebGames.saves.players=()=>$2;function`);
            const [skin, accessory, back] = Hook.matchAll("skins", /=\w+\.(\w+)\|\|NUMBER{0}/).map((a => a[1]));
            WebGames.props.skin = skin;
            WebGames.props.accessory = accessory;
            WebGames.props.back = back;
        ...
        const WebGames = window.WebGames;
        ...
        setTimeout(() => {
            ...
            function sendToAllKey(Down,E){
                ...
                for(let i = 0; i < I.length; i++){
                    //I[i].contentWindow.WebGames.controller.mouse.angle = controller.mouse.angle;
            ...
            if (!inIframe()) {
                ...
                window.addEventListener("mousedown",(function(e){
                    ...
                    WebGameshit.mainMouseDown = true;
                    WebGameshit.mainMouseNum = e.which;
                }));
                window.addEventListener("mouseup",(function(e){
                    sendToAllMouse(false,e)
                    WebGameshit.mainMouseDown = false;
                    WebGameshit.mainMouseNum = false;
                }));
            ...
            setInterval(() => {
                if (!inIframe()) {
                    ...
                    WebGameshit.mainX = WebGames.myPlayer.x;
                    WebGameshit.mainY = WebGames.myPlayer.y;
                    WebGameshit.mainMouseY = getpos.x / window.innerWidth;
                    WebGameshit.mainMouseX = getpos.y / window.innerHeight;
                    WebGameshit.mainMouseX1 = getpos.x - (window.innerWidth / 2);
                    WebGameshit.mainMouseY1 = getpos.y - (window.innerHeight / 2);
                    WebGameshit.mainAngle = WebGames.myPlayer.angle;
                    if(WebGames.controller.weapon == false){
                        WebGameshit.mainWeapon = 0;
                    } else {
                        WebGameshit.mainWeapon = 1;
                    }
                    WebGameshit.mainClanName = document.querySelector("#clan-title").innerText;
                    localStorage.WebGameshit = JSON.stringify(WebGameshit);
                }
                if (inIframe()) {
                    let WebGameshit = JSON.parse(localStorage.WebGameshit)
                    function mouseup(target) {
                        document.querySelector('#clan-menu').style.display = "flex";
                        target.onmouseup(new Proxy(new MouseEvent("mouseup"), {
                            get(target, prop) {
                    ...
                    if(document.querySelector("#clan-title").innerText == "Clans"){
                        for(let i = 0; i <= document.querySelector("#clan_menu_content").childElementCount; i++){
                            try{
                                if(document.querySelectorAll("#clan_menu_content > div > p")[i].innerText == WebGameshit.mainClanName){
                    ...
                    if(WebGameshit.mainMouseDown && WebGameshit.mainMouseNum == 1){
                        WebGames.controller.attack(Math.atan2(WebGameshit.mainY - (WebGames.myPlayer.y - WebGameshit.mainMouseY1), WebGameshit.mainX - (WebGames.myPlayer.x - WebGameshit.mainMouseX1)));
                        WebGames.controller.PacketManager.stopAttack();
                    }
                    document.querySelector("#play").click()
                    let someX = Math.pow(((WebGames.myPlayer.x - WebGameshit.mainMouseX1) - WebGameshit.mainX), 2);
                    let someY = Math.pow(((WebGames.myPlayer.y - WebGameshit.mainMouseY1) - WebGameshit.mainY), 2);
                    if (Math.sqrt(someX + someY) > 85) {
                        WebGames.controller.PacketManager.moveAngle(Math.atan2(WebGameshit.mainY - (WebGames.myPlayer.y - WebGameshit.mainMouseY1), WebGameshit.mainX - (WebGames.myPlayer.x - WebGameshit.mainMouseX1)));
                        WebGames.controller.PacketManager.changeAngle(Math.atan2(WebGameshit.mainY - (WebGames.myPlayer.y - WebGameshit.mainMouseY1), WebGameshit.mainX - (WebGames.myPlayer.x - WebGameshit.mainMouseX1)));
                        WebGames.controller.mouse.angle = Math.atan2(WebGameshit.mainY - (WebGames.myPlayer.y - WebGameshit.mainMouseY1), WebGameshit.mainX - (WebGames.myPlayer.x - WebGameshit.mainMouseX1));
                    } else {
                        WebGames.controller.PacketManager.moveByBitmask(0);
                    }
                    if(WebGameshit.mainWeapon == 0){
                        WebGames.controller.whichWeapon(false);
                    } else {
                        WebGames.controller.whichWeapon(true);
                    }
                    if(Set == "bow"){
                        [1,18,9,19,26,8,17,27,12].forEach(i => WebGames.controller.PacketManager.upgrade(i))
                    } else if(Set == "Polearm"){
                        [2,18,9,19,0,8,28,27,12].forEach(i => WebGames.controller.PacketManager.upgrade(i))
                    } else if(Set == "katana") {
                        [1,18,9,19,0,8,17,27,12].forEach(i => WebGames.controller.PacketManager.upgrade(i))
                    }
                }
            },100)
            if (!inIframe()) {
