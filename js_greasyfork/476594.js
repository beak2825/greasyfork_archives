...
// @downloadURL https://update.greasyfork.org/scripts/476594/%E8%80%83%E5%8F%A4%E5%AE%B6%E6%B5%8B%E8%AF%95.user.js
// @updateURL https://update.greasyfork.org/scripts/476594/%E8%80%83%E5%8F%A4%E5%AE%B6%E6%B5%8B%E8%AF%95.meta.js
// ==/UserScript==
...
new_element.setAttribute('href', document.currentScript.src.replace('kgj.js', '') + '../css/element-ui@2.14.1_lib_theme-chalk_index.css');
...
var videoListFunction = function() {
	...
	if ($('.pc-icon-player') && $('.pc-icon-player').forEach) {
		...
		//全局变量
		originOpen = window.open;
		copyOpen = function(url) {
			//将链接转化成原视频链接
		...
		//添加浮动的选择按钮
		$('body').append(`<div class="select-all" style='width: 100px;
		height: 50px;
		line-height: 50px;
		position: fixed;
		bottom: 15px;
	...
	setInterval(() => {
		$('.list-export-item').forEach(pro => {
			...
			if (is_has.length == 0) {
				...
				//添加相关视频按钮
				$(pro).parent().append(
					`<div style="position: absolute;right: -60px;bottom: 28px;" flex="cross:center main:center" class="cover-click search-video"> <a style="color: #4975e9;text-decoration: none;" href="/darenSquare/videoList?goods_name=` +
	...
	setInterval(() => {
		$('.goods-export').forEach(pro => {
			...
			if (is_has.length == 0) {
				...
				//添加相关视频按钮
				$(pro).parent().append(
					`<div style="position: absolute;right: -28px;bottom: 15px;" flex="cross:center main:center" class="cover-click search-video"> <a style="color: #4975e9;text-decoration: none;" href="/darenSquare/videoList?goods_name=` +
					goods_name + `" target="_blank">关联视频</a><div>`
				)
			}
		})
