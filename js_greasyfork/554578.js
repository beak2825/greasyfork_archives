...
let cmds = {
	...
	volume: {
		...
		func: (...args) => {
			...
			if (args.length === 1) {
				send(`Please specify a volume to set to. ${sep} Range: \`${minVol.toFixed(1)} to ${maxVol.toFixed(1)}\` ${sep} Usage: \`${ogcmd} <volume from ${minVol.toFixed(1)} to ${maxVol.toFixed(1)}>\``)
			} else {
	...
	speed: {
		aliases: ['sp', 'ps'],
		about: "Adjusts playback speed.",
		func: (...args) => {
			let ogcmd = args[0]
			const minSpeed = 0.1, maxSpeed = 2;
			if (args.length === 1) {
				send(`Please specify a speed to set to. ${sep} Range: \`${minSpeed.toFixed(1)}x (${minSpeed * 100}%) to ${maxSpeed.toFixed(1)}x (${maxSpeed * 100}%)\` ${sep} Usage: \`${ogcmd} <value from ${minSpeed.toFixed(1)} to ${maxSpeed.toFixed(1)}>\``)
			} else {
				args[1] = parseFloat(args[1])
				if (args[1] >= minSpeed && args[1] <= maxSpeed) {
					player.playbackSpeed = args[1]
					send(`Playback speed set to \`${player.playbackSpeed}x (${Math.trunc(player.playbackSpeed * 100)}%)\`.`)
				} else {
	...
	pause: {
		aliases: ['pa', 'toggle', 'to'],
		about: "Pauses or resumes a track.",
		func: () => {
	...
	prefix: {
		aliases: ['pre'],
		about: "Sets the prefix of the MIDI player.",
		func: (...args) => {
			if (args.length <= 1) {
				send('Please specify a prefix to set to.')
			} else {
				prefix = args[1]
				send(`Prefix now set to ${args[1] === '' ? 'nothing' : `\`${prefix}\``}. Use \`${prefix}help\` to see commands.`)
			}
			localStorage.tmp_prefix = prefix
		}
	}
}
...
notif.innerHTML = `<b>${name} by ${author} loaded.</b>\n<b>Use ${prefix}help to see commands.</b>\n<small><i>(click to close popup)</i></small>`
notif.style.cssText = `
    color: #fff;
...
notif.onmousedown = hidenotif
document.body.prepend(notif)
setTimeout(()=>{
    notif.style.transform = "translate(-50%, 0%)"
    setTimeout(hidenotif, 5e3)
}, 50)
...
async function checkUpdate() {
	...
	try {
		// try using main url
		r = await fetch('https://update.greasyfork.org/scripts/554578/JMIDIPlayer.user.js')
	} catch {
		// use backup url if blocked
		r = await fetch('https://update.greasyfork.org/scripts/554578/TealMIDIPlayer%20%28JMP%20only%29.user.js')
	}
                ...
                if (parseInt(splitNewNum[i]) !== parseInt(splitMyNum[i])) {
					const callback2 = ()=>{
						...
						notif2.style.cssText = `
							...
							cursor: default;
						`
						document.body.prepend(notif2)
						setTimeout(()=>{
							notif2.style.transform = "translate(-50%, 0%)"

							setTimeout(()=>{
								if (!notif2.isConnected) return
								notif2.style.opacity = "0%"
								notif2.style.transform = "translate(-50%, -100%)"
								setTimeout(()=>{
									notif2.remove()
								}, 1.5e3)
							}, parseInt(splitNewNum[i]) > parseInt(splitMyNum[i]) ? 30e3 : 5e3)
						}, 50)
					}
