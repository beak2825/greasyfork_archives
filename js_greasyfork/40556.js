...
// @description    TBR Map v3.2.29 voor Tribalwars.nl
// @grant		   none
// @downloadURL https://update.greasyfork.org/scripts/40556/TBR%20Map.user.js
// @updateURL https://update.greasyfork.org/scripts/40556/TBR%20Map.meta.js
// ==/UserScript==
 ...
 * v3.2.29	fix door nik12111
 */

if (location.href.match(/^https:\/\/((nl|zz|svs|en).*\.tribalwars\.(nl|se|net)\/(game.php|public_report).*)/)) {
  ...
  (function (f) {
    ...
    var settings = {
        ...
        "showAge": false,			// show age of report with the thickness of the border
        "maxAge": 9999,				// show reports older than maxAge using thin lines (days)
    ...
    /* would be stored in cookie/localStorage */
      ...
      config = {
        "opslaan": "https://twtools.tk/TBR_test/index.php/opslaan.php?value=opslaan&",
        "note": "https://twtools.tk/TBR_test/index.php/note.php?value=note&",
        "info": "https://twtools.tk/TBR_test/index.php/info.php?value=info&",
        "map": "https://twtools.tk/TBR_test/index.php/map.php?value=map&",
        "version": "3.2.26",
        "farmrule": {
          "nl5": 1200,
          "nl10": 1800,
      ...
      txtDB = {
        "se": { /* textmessages, should be translated */
          ...
          "haul": "Byte",
          "loyalty": "Lojalitet",
          "player_map_re": /(.+)\s\([0-9.]+ Poäng\)/,
          "tribe_map_re": /(.+)\s\([0-9.]+ Poäng\)/,
      ...
      do_request = function (req) {
        if (req.cmd == 'opslaan') {
          ...
          if (req.info!='') {
            ...
            $.post('https://tbrmap.twmerge.nl/debuginfo.php?error=' + req.info.error + '&bugid=' + req.info.bugid + '&v='+req.version+'&world='+req.world+'&player='+req.player, {desc: 'full report html', url: host, report: req.info});
          }
          ...
          TspawnSector: function(data,sector){
            ...
            for (var d in data.villages) {
              ...
              if (vil.note && TWMap.context._showPremium) {
                sector.appendElement($('<img src="https://tbrmap.twmerge.nl/note_trans.gif">').css({
                  position: 'absolute',
                  'z-index': 4,
                  'margin-top': 13,
                  'margin-left': 18,
              ...
              if (vil.troops) {
                ...
                if (box.getContext) {
                  ...
                  el_img.src = "https://tbrmap.twmerge.nl/images/spacer.gif";
