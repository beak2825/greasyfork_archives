...
// @version      1.201
// @description  Baixe imagens, GIFs, vídeos e mensagens de voz no aplicativo Telegram de canais privados que desativam o download e restringem o salvamento de conteúdo
// @author       Diego Melo
// @license      GNU GPLv3
...
// @namespace https://greasyfork.org/users/1330811
// @downloadURL https://update.greasyfork.org/scripts/500115/TGDownload.user.js
// @updateURL https://update.greasyfork.org/scripts/500115/TGDownload.meta.js
// ==/UserScript==
...
(function () {
  ...
  const completeProgress = (videoId) => {
    ...
    progressBar.querySelector("p").innerText = "Concluído";
  ...
  const AbortProgress = (videoId) => {
    const progressBar = document
      .getElementById("tel-downloader-progress-" + videoId)
      .querySelector("div.progress");
    progressBar.querySelector("p").innerText = "Abortado";
  ...
  const tel_download_video = (url) => {
      ...
      fetch(url, {
        ...
        .then((res) => {
          if (![200, 206].includes(res.status)) {
            throw new Error("Foi recebida resposta diferente de 200/206: " + res.status);
          }
          const mime = res.headers.get("Content-Type").split(";")[0];
          if (!mime.startsWith("video/")) {
            throw new Error("Obtenha resposta sem vídeo com tipo MIME " + mime);
          }
          ...
          if (startOffset !== _next_offset) {
            logger.error("Lacuna detectada entre as respostas.", fileName);
            logger.info("Último deslocamento: " + _next_offset, fileName);
            logger.info("Novo deslocamento inicial " + match[1], fileName);
            throw "Lacuna detectada entre as respostas.";
          }
          if (_total_size && totalSize !== _total_size) {
            logger.error("O tamanho total difere", fileName);
            throw "O tamanho total difere";
          }
          ...
          logger.info(
            `Obtenha resposta: ${res.headers.get(
              "Content-Length"
            )} bytes de dados de ${res.headers.get("Content-Range")}`,
            fileName
          );
          logger.info(
            `Progresso: ${((_next_offset * 100) / _total_size).toFixed(0)}%`,
            fileName
          );
          updateProgress(
            videoId,
    ...
    const save = () => {
      logger.info("Concluir o download dos blobs", fileName);
      logger.info("Concatenando blobs e baixando...", fileName);

      const blob = new Blob(_blobs, { type: "video/mp4" });
      const blobUrl = window.URL.createObjectURL(blob);

      logger.info("Tamanho final do blob: " + blob.size + " bytes", fileName);
      ...
      logger.info("Download acionado", fileName);
    };
  ...
  const tel_download_audio = (url) => {
      ...
      fetch(url, {
        ...
        .then((res) => {
          if (res.status !== 206 && res.status !== 200) {
            logger.error(
              "Foi recebida resposta diferente de 200/206: " + res.status,
              fileName
            );
            return;
          }
          ...
          if (!mime.startsWith("audio/")) {
            logger.error(
              "Obtenha resposta sem áudio com tipo MIME " + mime,
              fileName
            );
            throw "Obtenha resposta sem áudio com tipo MIME " + mime;
          }
          ...
          try {
            ...
            if (startOffset !== _next_offset) {
              logger.error("Lacuna detectada entre as respostas.");
              logger.info("Último deslocamento: " + _next_offset);
              logger.info("Novo deslocamento inicial " + match[1]);
              throw "Lacuna detectada entre as respostas.";
            }
            if (_total_size && totalSize !== _total_size) {
              logger.error("O tamanho total difere");
    ...
    const save = () => {
      logger.info(
        "Conclua o download dos blobs. Concatenando blobs e baixando...",
        fileName
      );
      ...
      logger.info("Tamanho final do blob em bytes: " + blob.size, fileName);
