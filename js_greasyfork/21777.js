...
// @run-at      document-end
// @version     2.5.0
// @downloadURL https://update.greasyfork.org/scripts/21777/%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/21777/%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
 ...
 *@转载重用请保留此信息
 *@2016年07月29日 修复百度网盘助手会自动消失的问题（修改特征码，防止百度将其删除）
 *
 *
 * */
...
(function (){
    ...
    var helperMenuBtns=(function(){
        ...
        function createHelperBtn(btn) {
            ...
            $(newnode).attr('id','').attr('href','javascript:void(0)').attr('data-key','yunhelper').attr('node-type','yun-helper').attr('onclick','').css({"padding-left":8}).html(html.replace(/[\u4E00-\u9FA5]{2,4}(\(.*\)|\uff08.*\uff09)?/,'\u7f51\u76d8\u52a9\u624b')).unbind();
            $(newnode).find("em").remove();
            $(newnode).find("b").css({"padding-right":8});
            var o=$('<div class="yunHelper" style="display:inline-block;">').append(newnode)[0];
            btn.parentNode.insertBefore(o, btn.nextSibling);
            return o;
        }
        var helperBtn = $('.yunHelper'),helperMenu = $('#panHelperMenu'),
        menuFun = function() {
        ...
        helperBtn.click(menuFun).mouseenter(function() {
            ...
            helperMenu.children('ul').css('width', "70px");
