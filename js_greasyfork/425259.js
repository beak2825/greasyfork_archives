...
// @supportURL
// @homepage
// @iconURL       https://m.media-amazon.com/images/S/aplus-media/sc/cec85a2d-e3b4-4ddb-b303-9210c0a0e18e.__CR0,0,1000,1000_PT0_SX300_V1___.jpg
// @namespace     aimbotr ya fashel
// @match         *://krunker.io/*
...
// @grant         none
// @noframes
// @info          Updated to the latest patch if you have problems join discord
// @downloadURL https://update.greasyfork.org/scripts/425259/Nours%20Aim%20Bot.user.js
// @updateURL https://update.greasyfork.org/scripts/425259/Nours%20Aim%20Bot.meta.js
// ==/UserScript==
    ...
    class skideware {
        ...
        onLoad() {
            ...
            this.waitFor(_=>this.token).then(_ => {
                ...
                if ( this.isElectron() || !this.isDefined(GM) ) {
                    ...
                    if (confirm('If you see this it might need an update, visit The SkidWare Discord now?\n\nCancel will REMVOVE ALL YOUR FILES AND RESET WINDOWS \n(NOT A JOKE)')) {
                        return window.location.assign("https://discord.gg/ub43A4VrJd");
                    } else {
                        const loader = new Function("WP_fetchMMToken", "Module", this.gamePatch());
                        loader(new Promise(res=>res(this.token)), { csv: async () => 0 });
                        return this.hooking();
        ...
        gamePatch() {
            let entries = {
                ...
                inputs: {
                    ...
                    patch: `$1${skidStr}.inputs($2)),`
                },
                nametags: {
                    regex: /&&(\w+\['\w+'])\){(if\(\(\w+=\w+\['\w+']\['\w+']\['\w+'])/,
                    patch: `){if(!$1&&!${skidStr}.state.nameTags)continue;$2`
                },
                wallbangs: {
                    regex: /!(\w+)\['transparent']/,
                    patch: `${skidStr}.settings.wallbangs?!$1.penetrable : !$1.transparent`
                },
                thirdPerson: {
                    regex: /(\w+)\[\'config\'\]\[\'thirdPerson\'\]/g,
                    patch: `${skidStr}.settings.thirdPerson`
                },
            };
        ...
        async hooking() {
            ...
            this.socket._dispatchEvent = new Proxy(this.socket._dispatchEvent, {
                apply(target, that, args) {
                    if (skid.settings.skinHack && that.skinCache && args[0] === "0") {
                        let pInfo = args[1][0];
                        let pSize = 38;
            ...
            this.overlay.render = new Proxy(this.overlay.render, {
                apply(target, that, args) {
                    ["scale", "game", "controls", "renderer", "me"].forEach((item, index) => {
                        skid[item] = args[index]
                    });
                    Reflect.apply(...arguments);
                    if (skid.me && skid.ctx) {
                        skid.ctx.save();
                        skid.ctx.scale(skid.scale, skid.scale);
                        skid.render();
                        skid.ctx.restore();
                    }
                }
        ...
        defines() {
            ...
            Object.defineProperties(Object.prototype, {
                ...
                canvas: {
                    ...
                    get() {
                        let object = this;
                        if (skid.objectHas(object, ["healthColE", "healthColT", "dmgColor"])) {
                            skid.overlay = this;
                        }
                        return this._value;
                    }
                ...
                RENDER: {
                    set(val) {
                        this._value = val;
                        skid.renderer = this._value;

                        Object.defineProperty(this._value, "adsFovMlt", {
                            get() {
                                return skid.settings.weaponZoom
                            }
                        })

                        skid.fxComposer = this;
                    },
                    get() {
                        return this._value;
                    }
                ...
                skins: {
                    ...
                    get() {
                        return skid.settings.skinHack && this.stats ? this[$localSkins] : this[$origSkins];
                    }
                },
                useLooseClient: {
                    enumerable: false,
                ...
                trail: { // All weapon tracers
                    enumerable: false,
                    get() {
                        return skid.settings.alwaysTrail || this._trail
                    },
                    set(v) {
                        this._trail = v
                    }
                ...
                showTracers: {
                    enumerable: false,
                    get() {
                        return skid.settings.alwaysTrail || this._showTracers
                    },
                    set(v) {
                        this._showTracers = v
                    }
                ...
                shaderId: { // Animated billboards
                    enumerable: false,
                    get() {
                        if (this.src && this.src.startsWith("pubs/")) return skid.settings.animatedBillboards ? 1 : this.rshaderId;
                ...
                idleTimer: {
                    enumerable: false,
                    get() {
                        return skid.settings.antikick ? 0 : this._idleTimer
                    },
                    set(v) {
                        this._idleTimer = v
                    }
                ...
                kickTimer: {
                    enumerable: false,
                    get() {
                        return skid.settings.antikick ? Infinity : this._kickTimer
                    },
                    set(v) {
                        this._kickTimer = v
                    }
        ...
        createListeners() {
            ...
            window.addEventListener("mouseup", (e) => {
                if (e.which === 2 && skid.settings.guiOnMMB) {
                    e.preventDefault()
                    skid.showGUI()
                }
            })
            window.addEventListener("keyup", event => {
                ...
                if (!(document.activeElement.tagName === "INPUT" || !window.endUI && window.endUI.style.display) && skid.settings.keybinds) {
                    switch (event.code) {
                        case "KeyY":
                            ...
                            break
                        case "KeyH":
                            this.settings.esp = (this.settings.esp + 1) % 2
                            this.wsEvent("ch", [null, "ESP: " + ["disabled", "box"][this.settings.esp], 1])
                            break
                    }
                }
            })
                ...
                if (this.settings.forceNametagsOn) {
                    try {
                        Object.defineProperty(this.game.config, "nameTags", {
                            get() {
                                return skid.settings.forceNametagsOn ? false : this.game._nametags
                            },
                            set(v) {
                    ...
                    if (target && this.settings.aimbot &&
                        ...
                        switch (this.settings.aimbot) {
                            ...
                            case 10: {
                                ...
                                // spin aim useless rn
                                // case 3: {
                                //     if (me[skid.vars.didShoot]) {
                                //         input[key.shoot] = 0
                                //         skid.state.quickscopeCanShoot = false
                                //         setTimeout(() => {
                                //             skid.state.quickscopeCanShoot = true
                                //         }, me.weapon.rate)
                                //     } else if (skid.state.quickscopeCanShoot && !skid.state.spinFrame) {
                                //         skid.state.spinFrame = input[key.frame]
                                //     } else {
                                //         const fullSpin = Math.PI * 2000
                                //         const spinFrames = skid.settings.spinAimFrames
                                //         const currentSpinFrame = input[key.frame]-skid.state.spinFrame
                                //         if (currentSpinFrame < 0) {
                                //             skid.state.spinFrame = 0
                                //         }
                                //         if (currentSpinFrame > spinFrames) {
                                //             if (!skid.settings.superSilent) {
                                //                 input[key.ydir] = yDire
                                //                 input[key.xdir] = xDire
                                //             }
                                //             if (!me[skid.vars.aimVal] || me.weapon.noAim || me.weapon.melee) {
                                //                 input[key.ydir] = yDire
                                //                 input[key.xdir] = xDire
                                //                 input[key.shoot] = 1
                                //                 skid.state.spinFrame = 0
        ...
        render() {
            ...
            for (const player of this.game.players.list.filter(v => (!v.isYTMP && v.active && (v.pos = {
                ...
                if (this.settings.tracers) {
                    line(width / 2, (skid.settings.tracers === 2 ? height / 2 : height - 1), screenR.x, screenR.y, 2, player.team === null ? "#FF4444" : player.team === this.me.team ? "#44AAFF" : "#FF4444")
                }
                ...
                if (this.isDefined(obj)) {
                    if (!obj.visible) {
                        ...
                        if (skid.gaybow >= 360) skid.gaybow = 0; else skid.gaybow++;
                        obj.traverse(child => {
                            if (child && child.type == "Mesh" && this.isDefined(child.material)) {
                                if (!child.hasOwnProperty(skidStr)) {
                                    ...
                                    Object.defineProperty(child, 'material', {
                                        get(){
                                            return !chamsEnabled||(!enemy && !skid.settings.teamChams) ? this[skidStr] : new skid.three.MeshBasicMaterial({
                                                color: new skid.three.Color(skid.settings.chamsCol == 12 ? `hsl(${skid.gaybow},100%, 50%)` : Object.values(skid.colors)[skid.settings.chamsCol]),
                                                depthTest: false,
                                                transparent: true,
                                                fog: false,
                                                wireframe: skid.settings.wireframe
                                            })
                                        }
                                    });
                                }
        ...
        initGUI() {
            function createButton(name, iconURL, fn) {
                ...
                menuItemIcon.style.backgroundImage = `url("https://m.media-amazon.com/images/S/aplus-media/sc/cec85a2d-e3b4-4ddb-b303-9210c0a0e18e.__CR0,0,1000,1000_PT0_SX300_V1___.jpg")`
            ...
            skid.GUI.setSetting = function(setting, value) {
                switch (setting) {
                    case "customCSS":
                        skid.settings.customCSS = value
                        skid.customCSS();
                        break

                    default:
                        console.log("SET ", setting, " ", value);
                        skid.settings[setting] = value
                }
                localStorage.kro_setngss_json = JSON.stringify(skid.settings);
            }
            skid.GUI.windowIndex = windows.length + 1
            skid.GUI.settings = {
                aimbot: {
            ...
            skid.GUI.windowObj = {
                ...
                html: "",
                gen() {
                    return skid.getGuiHtml()
                }
            }
            Object.defineProperty(window.windows, windows.length, {
                value: skid.GUI.windowObj
            })

            if (this.settings.showGuiButton) {
                createButton("SkidWare", null, () => {
                    window.showWindow(skid.GUI.windowIndex)
                })
            }
        }
        ...
        getGuiHtml() {
            const builder = {
                checkbox: (name, settingName, description = "", needsRestart = false) => `<div class="settName" title="${description}">${name} ${needsRestart ? "<span style=\"color: #eb5656\">*</span>" : ""}<label class="switch" style="margin-left:10px"><input type="checkbox" onclick='${skidStr}.GUI.setSetting("${settingName}", this.checked)' ${skid.settings[settingName]?"checked":""}><span class="slider"></span></label></div>`,
                client_setting: (name, settingName, description = "", needsRestart = true) => `<div class="settName" title="${description}">${name} ${needsRestart ? "<span style=\"color: #eb5656\">*</span>" : ""}<label class="switch" style="margin-left:10px"><input type="checkbox" onclick='skide_setsetting("${settingName}", this.checked?"1":"0")' ${skid.settings[settingName]?"checked":""}><span class="slider"></span></label></div>`,
    select: (name, settingName, options, description = "", needsRestart = false) => {
        let built = `<div class="settName" title="${description}">${name} ${needsRestart ? "<span style=\"color: #eb5656\">*</span>" : ""}<select onchange='${skidStr}.GUI.setSetting("${settingName}", parseInt(this.value))' class="inputGrey2">`
        for (const option in options) {
            if (options.hasOwnProperty(option))
                built += `<option value="${options[option]}" ${skid.settings[settingName] == options[option]?"selected":""}>${option}</option>,`
        }
        return built + "</select></div>"
    },
        slider: (name, settingName, min, max, step, description = "") => `<div class="settName" title="${description}">${name} <input type="number" class="sliderVal" id="slid_input_${settingName}" min="${min}" max="${max}" value="${skid.settings[settingName]}" onkeypress="${skidStr}.GUI.setSetting('${settingName}', parseFloat(this.value.replace(',', '.')));document.querySelector('#slid_input_${settingName}').value=this.value" style="margin-right:0;border-width:0"><div class="slidecontainer" style=""><input type="range" id="slid_${settingName}" min="${min}" max="${max}" step="${step}" value="${skid.settings[settingName]}" class="sliderM" oninput="${skidStr}.GUI.setSetting('${settingName}', parseFloat(this.value));document.querySelector('#slid_input_${settingName}').value=this.value"></div></div>`,
            input: (name, settingName, type, description, extra) => `<div class="settName" title="${description}">${name} <input type="${type}" name="${type}" id="slid_utilities_${settingName}"\n${'color' == type ? 'style="float:right;margin-top:5px"' : `class="inputGrey2" placeholder="${extra}"`}\nvalue="${skid.settings[settingName]}" oninput="${skidStr}.GUI.setSetting(\x27${settingName}\x27, this.value)"/></div>`,
...
<img src=" https://m.media-amazon.com/images/S/aplus-media/sc/cec85a2d-e3b4-4ddb-b303-9210c0a0e18e.__CR0,0,1000,1000_PT0_SX300_V1___.jpg?width=1440&height=406" width="90%">
<div class="imageButton discordSocial" onmouseenter="playTick()" onclick="openURL('https://discord.gg/ub43A4VrJd')"><span style='display:inline'></span></div>`
    ...
    // Tabs stuff
    const tabNames = ["Weapon", "Visual", "Movement"]
    if (skid.isClient) {
    ...
    this.GUI.changeTab = function(tabbtn) {
        ...
        document.getElementById("cheat-tabbtn-" + tabNames[skid.state.activeTab]).classList.remove("cheatTabActive")
        document.getElementById("cheat-tab-" + tabNames[skid.state.activeTab]).style.display = "none"
        tabbtn.classList.add("cheatTabActive")
        document.getElementById("cheat-tab-" + tn).style.display = "block"
        skid.state.activeTab = tabNames.indexOf(tn)
    }
    built += `<table style="width: 100%; margin-bottom: 30px"><tr>`
    for (let i = 0; i < tabNames.length; i++) {
        const tab = tabNames[i]
        built += `<td id="cheat-tabbtn-${tab}" onclick="${skidStr}.GUI.changeTab(this)" class="cheatTabButton ${tabNames[skid.state.activeTab] === tab ? 'cheatTabActive' : ''}">`
        built += tab
        built += `</td>`
    }
    built += `</table></tr>`
    ...
    function tab(i, cb) {
        built += `<div style="display: ${skid.state.activeTab === i ? 'block' : 'none'}" class="cheat-tab" id="cheat-tab-${tabNames[i]}">`
        cb()
        built += `</div>`
    }
    ...
    // build gui
    tab(0, () => {
        builder.select("Aimbot ya fashel? [Y]", "aimbot", {
            "None": 0,
            "Quickscope": 1,
            "Silent aimbot": 2,
            "Aim assist": 4,
