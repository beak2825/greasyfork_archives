// ==UserScript==
// @name         Auto_Award_Plus
// @namespace    https://blog.chrxw.com
// @version      2.2
// @description  Steam自动打赏黄金增强版
// @author       Chr_
// @include      /https://steamcommunity\.com/(id|profiles)/[^\/]+/(recommended|reviews)\/?(\?p=\d+)?$/
...
// @downloadURL https://update.greasyfork.org/scripts/420525/Auto_Award_Plus.user.js
// @updateURL https://update.greasyfork.org/scripts/420525/Auto_Award_Plus.meta.js
// ==/UserScript==
...
function addPanel() {
    ...
    let pTips1 = genLabel('自动打赏Plus - V' + Version + ' - By Chr_');

    let pLink1 = genLink('打赏作者（点数）', 'https://steamcommunity.com/id/Chr_/recommended/', '感谢支持');
    ...
    // 设置排序条件会导致打赏面板消失
    let s = document.getElementById('sort_by_reviews');
    let r = document.querySelector('.es-sortbox__reverse');
    if (s != null && r != null) {
        if (s.value != 'default' || r.textContent != '↓') {
            let btnResetSort = genButton('打赏面板消失？点这里解决', resetSort);
            let btnArea2 = document.querySelector('.es-sortbox--reviews');
            btnArea2.insertBefore(genSpace(), btnArea2.children[0]);
            btnArea2.insertBefore(btnResetSort, btnArea2.children[0]);
        }
    }
}
// 重置排序
function resetSort() {
    let s = document.getElementById('sort_by_reviews_default');
    s.click();
    let r = document.querySelector('.es-sortbox__reverse');
    if (r.textContent != '↓') {
        r.click();
...
function autoAward() {
    const max = 50; // 最大尝试次数
    let tries = 0; // 当前次数
    let curr = 0; // 当前处于第几篇评测
    retry(reviewAward, 50);
    // 处理评测上的奖励按钮
    function reviewAward() {
        let reward = document.querySelectorAll('.control_block>span:last-child');
        if (reward) {
            if (curr < reward.length) {
                reward[curr].click();
                tries = 0;
                retry(waitLoad, 600);
            } else {//翻页
            ...
            setTimeout(() => {
                try {
                    ...
                    if (e.toString().search('操作结束') != -1) {
                        ...
                        saveConf();
                        closePanel();
                    } else if (e.toString().search('更换评测') != -1) {
                        closePanel();
                        curr++;
                        retry(reviewAward, 100);
                    } else {
...
function setGoal() {
    ...
    saveConf();
    updateProcess();
    ShowAlertDialog('提示', '目标已设置，等待开始运行。<br><br>点击【开始打赏】开始运行！<br><br>关闭打赏提示框后，点击【重置】中断运行（手动刷新页面不会中断脚本运行）！<br><br>如果遇到意外错误中断运行，手动点击【开始打赏】可以继续运行。');
}
// 清除目标
function resetGoal() {
    Vmode = false;
...
function setPage(page) {
    window.location.href = window.location.origin + window.location.pathname + '?p=' + page.toString()
}
// 修复打赏图标
function fixAward() {
