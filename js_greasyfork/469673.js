// ==UserScript==
// @name     kbin enhancer script
// @description Few small changes to the kbin UI while they still develop some features
// @namespace com.sirpsychomantis > personalized by uselessartifact
// @license MIT
// @version  1.9.1
// @grant    none
// @run-at document-end
...
// @downloadURL https://update.greasyfork.org/scripts/469673/kbin%20enhancer%20script.user.js
// @updateURL https://update.greasyfork.org/scripts/469673/kbin%20enhancer%20script.meta.js
// ==/UserScript==


(function(){

  /*var figures = document.getElementsByTagName("blockquote figure");
  var allfigures = Array.prototype.slice.call(figures);
      allfigures.forEach(function(item){item.style.display = "none"});*/
  ...
  function collapseComment(blockquote) {
    ...
    const username = blockquote.querySelector("header a[class='user-inline']").innerHTML;
    ...
    newBlockquote.innerHTML = '<header><a href="javascript:;">&#x25B7;&emsp;' + username + ', ' + time + '</a></header>';
  ...
  function addCollapseLinks() {
    if (location.pathname.startsWith('/m')) {
      ...
      comments.forEach(blockquote => {
      	const header = blockquote.querySelector("header a");
...
/* Start User Functions */
  ...
  //$("article footer menu").css({"border":"1px solid red"});
  //$("article footer menu").append($("article aside form.vote__up[0]"));
//   var allfigures = Array.prototype.slice.call(articlebuttons);
//       allfigures.forEach(function(item){item.style.display = "none"});
