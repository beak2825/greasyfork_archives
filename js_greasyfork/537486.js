// ==UserScript==
// @name         FF Scouter V2 + BS Estimates (modded)
// @namespace    Violentmonkey Scripts
// @match        https://www.torn.com/*
// @version      2.44
// @author       rDacted, Weav3r - modded by GFOUR
...
// @downloadURL https://update.greasyfork.org/scripts/537486/FF%20Scouter%20V2%20%2B%20BS%20Estimates%20%28modded%29.user.js
// @updateURL https://update.greasyfork.org/scripts/537486/FF%20Scouter%20V2%20%2B%20BS%20Estimates%20%28modded%29.meta.js
// ==/UserScript==

const FF_VERSION = 2.43;
...
if (!singleton) {
    ...
    function create_text_location() {
        ...
        // Check if h4 exists before trying to use it
        if (!h4) {
            // For pages without h4 (like competition pages), just append to body hidden
            // The info_line is mainly used for profile/attack pages
            info_line.style.display = 'none';
            document.body.appendChild(info_line);
            return info_line;
        }
    ...
    function set_message(message, error = false) {
        ...
        // Make sure it's visible when we have a message
        info_line.style.display = 'block';
    }
    ...
    // Create the singleton element - this is needed to prevent multiple instances
    ...
    function get_player_id_in_element(element) {
        ...
        // Check parent elements for competition pages where the anchor wraps the honor-text-wrap
    ...
    function isCompactLayout(element) {
        // Check if we're in a compact grid layout (like competition pages)
    ...
    function show_cached_values(elements) {
        for (const [player_id, element] of elements) {
            ...
            if (!element.classList.contains('indicator-lines')) {
                ...
                // Use smaller arrow for compact layouts
                const compact = isCompactLayout(element);
                element.style.setProperty("--arrow-width", compact ? "12px" : "15px");

                // Only remove these classes for non-compact layouts
                if (!compact) {
                ...
                if (ff) {
                    ...
                    if (response.bs_estimate || response.bs_estimate_human) {
                        ...
                        if (bsValue) {
                            ...
                            // Get the same background color as the FF triangle
                            let backgroundColor;
                            if (percent < 33) {
    ...
    const ff_gauge_observer = new MutationObserver(async function () {
        ...
        if (honor_bars.length > 0) {
            ...
            } else if (window.location.href.startsWith("https://www.torn.com/page.php?sid=competition")) {
                // Competition/Elimination pages - try multiple selectors
                await apply_ff_gauge($('[class*="name___"] .honor-text-wrap').toArray());
                await apply_ff_gauge($('[class*="name___"] a').toArray());
            }
        }
