...
// @connect      gitlab.com
// @downloadURL https://update.greasyfork.org/scripts/424600/SimpleDiscordCrypt.user.js
// @updateURL https://update.greasyfork.org/scripts/424600/SimpleDiscordCrypt.meta.js
// ==/UserScript==
...
async function decryptMessage(message, payload, ignoreAttachments) {
    ...
    if(payloadBuffer.byteLength === 16) {
        if(!differentKey) message.content = `<:ENC:${PNG}>`; //invisible separator at the end to make the emoji smaller
        ...
        if(!differentKey) message.content = `<:ENC:${PNG}>` + content;
