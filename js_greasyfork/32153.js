...
// @namespace      PTP-Send-Points-Inel
// @downloadURL https://update.greasyfork.org/scripts/32153/PTP%20Send%20Points.user.js
// @updateURL https://update.greasyfork.org/scripts/32153/PTP%20Send%20Points.meta.js
// ==/UserScript==
...
authKey = document.head.innerHTML.split('authkey=')[1].split('"')[0];
token = document.body.getAttribute('data-anticsrftoken');
...
if(!unsafeWindow.localStorage.amnt)
  unsafeWindow.localStorage.amnt = false;
if(!unsafeWindow.localStorage.msg)
  unsafeWindow.localStorage.msg = false;
if(!unsafeWindow.localStorage.hat)
  unsafeWindow.localStorage.hat = false;
...
for (var post=0; post<fPT.length; post++)// in fPT)
{
  ...
  sendSomethingDiv.setAttribute('style', 'text-align: center; font-size: 0.8em;');
  sendSomethingDiv.appendChild(document.createElement('br'));
  ...
  else
  {
    ...
    amount = '';
    if(JSON.parse(unsafeWindow.localStorage.amnt))
    {
    ...
    sPAmount.setAttribute('style', 'background: #555; border-color: #000; width: 72px; height: 18px; text-align: center; border-width: thin; font-size: 1em;');
    sPAmount.setAttribute('title', 'Amount');
    sPAmount.setAttribute('placeholder', 'Amount');
    sPAmount.value = amount;
    sPMessage = document.createElement('textarea');
    sPMessage.setAttribute('style', 'background: #555; border-color: #000; width: 72px; height: 18px; text-align: center; float: right; font-size: 1em;');
    sPMessage.setAttribute('title', 'Message');
    sPMessage.setAttribute('placeholder', 'Message');
    if(JSON.parse(unsafeWindow.localStorage.msg))
    ...
    sendPointsSpan.appendChild(document.createTextNode('\u00A0'));
    ...
    if(JSON.parse(unsafeWindow.localStorage.hat))
    {
      ...
      anonLabel = document.createElement('label');
      anonLabel.setAttribute('style', 'height: 16px; text-align: center; font-size: 1em;');

      anonCheck = document.createElement('input');
      anonCheck.type = 'checkbox';
      anonCheck.setAttribute('style', 'position: relative; vertical-align: bottom; bottom: 1px;');
      anonLabel.appendChild(anonCheck);
      anonLabel.appendChild(document.createTextNode(': Anonymous'));

      sendHatsDiv.appendChild(anonLabel);
      //sendHatsDiv.appendChild(document.createElement('br'));
      sendHatsDiv.appendChild(document.createTextNode('\u00A0\u00A0\u00A0'));

      /*anonCheck = document.createElement('input');
      anonCheck.type = 'checkbox';
      anonCheck.setAttribute('style', 'background: #555; border-color: #000;');
      ...
      days.setAttribute('style', 'background: #555; border-color: #000; width: 4em; height: 16px; text-align: center; font-size: 1em;');
      ...
      hatMessage = document.createElement('input');
      hatMessage.setAttribute('style', 'background: #555; border-color: #000; height: 16px; width: 130px; text-align: center; font-size: 1em;');
      hatMessage.setAttribute('title', 'Message');
      hatMessage.setAttribute('class', 'hat_message');
      hatMessage.setAttribute('placeholder', 'Message');
...
function settings(span)
{
  ...
  amnt = JSON.parse(unsafeWindow.localStorage.amnt);
  ...
  msg = JSON.parse(unsafeWindow.localStorage.msg);
  ...
  hat = JSON.parse(unsafeWindow.localStorage.hat);
...
function save_settings(span)
{
  ...
  unsafeWindow.localStorage.amnt = checks[0].checked;
  unsafeWindow.localStorage.msg = checks[1].checked;
  unsafeWindow.localStorage.hat = checks[2].checked;
  span.innerHTML = '';
  settings_fill(span);
}

function response(span, text)
{
    mess = this.responseText.split('color: red;text-align:center;">');
    endsplit = '</p>';
    if(mess.length < 2)
    {
      mess = this.responseText.split('alert--warning text--center">');
      endsplit = '</div>';
      if(mess.length < 2)
      {
        document.body.innerHTML = '<p>Failed to send points, hopefully this page has the error message as to why. If the message is that you need cookies enabled, turn on 3rd party cookies.</p>' + this.responseText;
        return;
      }
    }
	span.innerHTML = text;
	summary.innerHTML += mess[1].split(endsplit)[0] + '<br />';
}
...
function send_points(username, span)
{
    ...
    var data = "auth=" + authKey + "&username=" + username + "&amount=" + amount + "&message=" + message + "&AntiCsrfToken=" + token + "&action=send";
    var x = new XMLHttpRequest();
    x.onload = response.bind(x, span, '[Points Sent]');
    //x.open("post", "bonus.php?action=send", true);
    x.open("post", "bonus.php", true);
	...
	/*GM_xmlhttpRequest({
		...
		data: "auth=" + authKey + "&username=" + username + "&amount=" + amount + "&message=" + message + "&AntiCsrfToken=" + token + "&action=send",
		headers: {
...
function send_hat(userID, span)
{
  ...
  days = inputs[1].value;
  //message = encodeURIComponent(span.getElementsByTagName('textarea')[0].value);
  message = encodeURIComponent(span.getElementsByClassName('hat_message')[0].value);
  postData = "auth=" + authKey + "&AntiCsrfToken=" + token + '&extra={"mult":' + days + ',"anon":' + anonymous + ',"user":' + userID;
  if(message == '')
    postData += ',"comm": false}';
  else
  ...
  var x = new XMLHttpRequest();
  x.onload = response.bind(x, span, '[Hat Sent]');
  x.open("post", "bonus.php?action=purchase&type=9&itemid=26&confirm=1", true);
  x.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
