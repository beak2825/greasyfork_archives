...
// @downloadURL https://update.greasyfork.org/scripts/453058/%E7%AE%97%E5%88%86%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/453058/%E7%AE%97%E5%88%86%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
...
(function() {
    ...
    css += '#pf_board {font-size:14px; position: fixed; right: 10px; bottom: 150px;background: #FFF;border: 1px solid #A6CBE7;padding: 3px;} #pf_board .item {padding: 5px 0;} #pf_board .item .tm {display: inline-block;} #pf_board .item .code_short {width: 40px; margin-left: 3px;} #pf_board .item .code_medium {width: 60px; margin-left: 3px;}';
    ...
    if (localHref.match(/(cl\.|t66y)/) != null ){
        pageInit();
    }
    ...
    function save_game()
    {
        if((/竞猜/).test(localTitle)){
            copy_bets();
            console.log('save_game_copy_bets');
            return false;
        }
        win_item = [];
        new_data = [];
    ...
    function copy_bets(){
        let bets_copy = '';
        let copy_tample = '{floor}	{author}	{points}';//'{0}	{1}	{2}\r\n';
        $.each($("div[id^='cont']:not(#conttpc)"), function(i, bet) {
            ...
            let mp = points_item.floor.match(/(?<floor>\d+)/);
            if(mp != null)
            {
            ...
            bets_copy += copy_tample.format(points_item);//(points,points,points);
            ...
            $.each(ms, function(k, m){
                ...
                let mc = m.match(/(：|:)\s*(?<team>.*)/);//m.match(/\d+\.(\S)(：|:)\s*(?<team>.*)<br/g);
                if(mc != null)
                {
    ...
    function settle_up(){
        ...
        let copy_tample = '{floor}	{author}	{points}\r\n';//'{0}	{1}	{2}\r\n';
        $.each($("div[id^='cont']:not(#conttpc)"), function(i, bet) {
            var ratio = 0;
            var points = 0;
            var bet_points = 0;
            ...
            let points_item = {};
            points_item.author = $('.r_two b:first', $(bet).parent().parent()).text();
            points_item.floor = $('.tipad .s3', $(bet).parent().parent().parent()).text();
