// ==UserScript==
// @name   🌱【免费版】中国海峡人才市场_福建省人才培训平台|如需自动下一集、自动换课程等高级功能见收费版本：https://doc.zhanyc.cn/pages/zghxrcsc/
// @namespace    http://jb.zhanyc.cn/
// @icon    https://js.zhanyc.cn/img/js-logo.svg
// @version      1.1
// @description  当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制、自动答题功能。如需自动下一集、自动换课程等高级功能可升级付费版本，一杯咖啡钱，保你无忧学习，且永久使用|接各类脚本开发、代挂工作，微信：zhanyc_cn 备用微信:zhanfengkuo 个人网站：http://doc.zhanyc.cn
// @author       zfk
// @include    *://*rcpxpt.com*
// @grant       GM_getValue
// @grant       GM_setValue
...
// @antifeature 
// @license GPL
// @downloadURL https://update.greasyfork.org/scripts/514040/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E4%B8%AD%E5%9B%BD%E6%B5%B7%E5%B3%A1%E4%BA%BA%E6%89%8D%E5%B8%82%E5%9C%BA_%E7%A6%8F%E5%BB%BA%E7%9C%81%E4%BA%BA%E6%89%8D%E5%9F%B9%E8%AE%AD%E5%B9%B3%E5%8F%B0%7C%E5%A6%82%E9%9C%80%E8%87%AA%E5%8A%A8%E4%B8%8B%E4%B8%80%E9%9B%86%E3%80%81%E8%87%AA%E5%8A%A8%E6%8D%A2%E8%AF%BE%E7%A8%8B%E7%AD%89%E9%AB%98%E7%BA%A7%E5%8A%9F%E8%83%BD%E8%A7%81%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%EF%BC%9Ahttps%3Adoczhanyccnpageszghxrcsc.user.js
// @updateURL https://update.greasyfork.org/scripts/514040/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E4%B8%AD%E5%9B%BD%E6%B5%B7%E5%B3%A1%E4%BA%BA%E6%89%8D%E5%B8%82%E5%9C%BA_%E7%A6%8F%E5%BB%BA%E7%9C%81%E4%BA%BA%E6%89%8D%E5%9F%B9%E8%AE%AD%E5%B9%B3%E5%8F%B0%7C%E5%A6%82%E9%9C%80%E8%87%AA%E5%8A%A8%E4%B8%8B%E4%B8%80%E9%9B%86%E3%80%81%E8%87%AA%E5%8A%A8%E6%8D%A2%E8%AF%BE%E7%A8%8B%E7%AD%89%E9%AB%98%E7%BA%A7%E5%8A%9F%E8%83%BD%E8%A7%81%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%EF%BC%9Ahttps%3Adoczhanyccnpageszghxrcsc.meta.js
// ==/UserScript==
(function () {
    ...
    let freeTips = " 当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制、自动答题功能。如需自动下一集、自动换课程等高级功能，可点击下方按钮查看付费版本"
    let docUrl = "https://doc.zhanyc.cn/pages/zghxrcsc/";
    let plugMain = Object.assign(baseConfig, {
        ...
        pageData: {
            ...
            qaData: {
                "19282": ["C", "D", "D", "B", "B", "B", "A", "正确", "正确", "正确"],
                "19281": ["B", "D", "D", "A", "B", "A", "ABCD", "ABCDE", "ABCDEFG", "ACD"],
                "19280": ["A", "B", "A", "B", "D", "D", "B", "C", "ABCD", "BD"],
                "19279": ["B", "C", "A", "C", "A", "B", "BCD", "ACD", "ABC", "ABCD"],
                "19278": ["B", "C", "D", "B", "ABCDE", "BCD", "ABC", "CD", "ABCD", "错误"],
        ...
        async runByUrl(url) {
            ...
            if (url.toLocaleLowerCase().includes("sysConfigItem/selectDetail".toLocaleLowerCase())) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
            } else if (url.toLocaleLowerCase().includes("/classModule/video/".toLocaleLowerCase())) {
                plugMain.page_video();
            } else if (url.toLocaleLowerCase().includes("tikuUserBatch/keepTopic".toLocaleLowerCase())) {
                plugMain.page_keepTopic();
            } else if (url.toLocaleLowerCase().includes("classPackage/classPackageDetail".toLocaleLowerCase())) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
            } else if (url.toLocaleLowerCase().includes("usersCenter".toLocaleLowerCase())) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
            }
        },

        async page_keepTopic(isCallback = false) {
            console.log("%c page_keepTopic", "background:rgb(0,0,0);color:#fff");

            if (!isCallback) {
                plugMain.closeWaitConfrimWin()
                await plugMain.waitOf(a => $(".issueTitle").text() != "")
                await plugMain.waitTimeout(1000)
            }
            let oldNo = $(".questionStatus strong.current:visible").text()
            let id = plugMain.parseQueryString().testId
            let qaData = plugMain.pageData.qaData[id]
            if (!qaData) {
                plugMain.alertMsg("没有配置答案，请联系脚本客服处理")
                return
            }
            let curNo = qaData[Number(oldNo) - 1]
            let answerArr = [curNo]
            if (curNo.length > 0 && /^[A-Z]+$/.test(curNo)) {
                answerArr = curNo.split('')
            }
            for (let i = 0; i < answerArr.length; i++) {
                const item = answerArr[i];
                if (!$(`.issueChoice [optionno='${item}']`).is("[checked]")) {
                    $(`.issueChoice [optionno='${item}']`).click()
                    await plugMain.waitTimeout(100)
                }
            }

            await plugMain.waitTimeout(200)
            if ($("button.next").text() == '交卷') {
                plugMain.confirmRun("5秒后交卷，如果不想交卷，请下下面的【取消执行】按钮", 5000).then(a => {
                    $("button.next").click()
                })
                return
            }
            $("button.next").click()
            await plugMain.waitOf(a => $(".questionStatus strong.current:visible").text() != oldNo)
            await plugMain.waitTimeout(1000)
            plugMain.page_keepTopic(true)
        },
        async page_video() {
            plugMain.closeWaitConfrimWin()
            console.log("%c page_video", "background:rgb(0,0,0);color:#fff");
            await plugMain.waitOf(a => $(".zjlieb").length > 0)
            await plugMain.waitTimeout(1000)
            let timeout = 2;
            let lastTime = null;
            ...
            plugMain.pageData.videoKZLIndex = setTimeout(async () => {
