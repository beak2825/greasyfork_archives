// ==UserScript==
// @name         Torn Properties Manager dev
...
// @run-at       document-end
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/547712/Torn%20Properties%20Manager%20dev.user.js
// @updateURL https://update.greasyfork.org/scripts/547712/Torn%20Properties%20Manager%20dev.meta.js
// ==/UserScript==

(function() {
    'use strict';
    
    alert('DEBUG: Script started loading');
    alert(`DEBUG: Page title - ${document.title}`);
    alert(`DEBUG: User agent - ${navigator.userAgent}`);
    ...
    function observeOfferSubmissions() {
        alert('DEBUG: observeOfferSubmissions function called');
        
        const url = new URL(window.location.href);
        alert(`DEBUG: Current URL - ${window.location.href}`);
        alert(`DEBUG: URL hash - ${url.hash}`);
        
        if (url.hash.includes('tab=offerExtension') || url.hash.includes('tab=lease')) {
            alert('DEBUG: URL contains offer or lease tab');
            
            const propertyId = url.hash.match(/ID=(\d+)/)?.[1];
            alert(`DEBUG: Extracted property ID - ${propertyId}`);
            
            if (!propertyId) {
                alert('DEBUG: No property ID found, exiting function');
                return;
            }

            console.log('Auto-filling form for property:', propertyId);
            alert(`DEBUG: Starting auto-fill for property ${propertyId}`);

            const observer = new MutationObserver((mutations, obs) => {
                alert('DEBUG: MutationObserver triggered');
                
                const offerExtensionUl = document.querySelector('ul.offerExtension-input');
                alert(`DEBUG: Found offerExtension-input ul: ${!!offerExtensionUl}`);
                
                if (offerExtensionUl) {
                    alert('DEBUG: offerExtension-input ul found, looking for cost and amount elements');
                    
                    const costLi = offerExtensionUl.querySelector('li.cost');
                    const amountLi = offerExtensionUl.querySelector('li.amount');
                    
                    alert(`DEBUG: Found cost li: ${!!costLi}, Found amount li: ${!!amountLi}`);
                    
                    if (costLi && amountLi && !costLi.dataset.listenerAttached) {
                        alert('DEBUG: Both elements found and not already processed');
                        ...
                        alert(`DEBUG: Default values - Period: ${defaultPeriod}, Amount: ${defaultAmount}`);
                        
                        // Only run if we haven't processed these inputs yet
                        if (!costLi.dataset.processed) {
                            alert('DEBUG: Starting processing of cost inputs');
                            setTimeout(() => {
                                alert('DEBUG: Inside setTimeout, processing form inputs');
                                
                                // Set the cost inputs
                                const costInputs = costLi.querySelectorAll('input.offerExtension.input-money:not([data-processed])');
                                alert(`DEBUG: Found ${costInputs.length} cost inputs`);
                                
                                costInputs.forEach((input, index) => {
                                    if (!input.dataset.processed) {
                                        alert(`DEBUG: Setting cost input ${index + 1} to ${defaultAmount}`);
                                        input.value = defaultAmount;
                                        input.dispatchEvent(new Event('input', { bubbles: true }));
                                        input.dispatchEvent(new Event('change', { bubbles: true }));
                                        input.dataset.processed = 'true';
                                        alert(`DEBUG: Cost input ${index + 1} set successfully`);
                                    }
                                });

                                // Set the amount input
                                const amountInput = amountLi.querySelector('input.input-money:not([data-processed])');
                                alert(`DEBUG: Found amount input: ${!!amountInput}`);
                                
                                if (amountInput && !amountInput.dataset.processed) {
                                    alert(`DEBUG: Setting amount input to ${defaultPeriod}`);
                                    amountInput.value = defaultPeriod.toString();
                                    amountInput.dispatchEvent(new Event('input', { bubbles: true }));
                                    amountInput.dispatchEvent(new Event('change', { bubbles: true }));
                                    amountInput.dataset.processed = 'true';
                                    alert('DEBUG: Amount input set successfully');
                                } else if (!amountInput) {
                                    alert('DEBUG: Amount input not found');
                                } else {
                                    alert('DEBUG: Amount input already processed');
                                }
                                
                                costLi.dataset.processed = 'true';
                                alert('DEBUG: Marked costLi as processed');
                                
                                if (costLi.dataset.processed && amountInput?.dataset.processed) {
                                    alert('DEBUG: Both inputs processed, disconnecting observer');
                                    observer.disconnect();
                                } else {
                                    alert('DEBUG: Not all inputs processed yet, observer still active');
                                }
                            }, 500);
                        } else {
                            alert('DEBUG: costLi already processed, skipping');
                        }
                        
                        costLi.dataset.listenerAttached = 'true';
                        alert('DEBUG: Marked listener as attached');
                    } else {
                        alert('DEBUG: Required elements not found or already attached');
                    }
                } else {
                    alert('DEBUG: offerExtension-input ul not found');
                }
            });

            alert('DEBUG: Creating MutationObserver');
            observer.observe(document.body, {
                childList: true,
                subtree: true
            });
            alert('DEBUG: MutationObserver started');
        } else {
            alert('DEBUG: URL does not contain offer or lease tab, function exiting');
        }
    }

    // Initialize the script
    alert(`DEBUG: Current URL - ${window.location.href}`);
    alert('DEBUG: Setting up load event listener');
    window.addEventListener('load', function() {
        alert('DEBUG: Window load event fired');
        let attempts = 0;
        
        const checkForElement = setInterval(() => {
            attempts++;
            alert(`DEBUG: Attempt ${attempts} - Looking for #properties-page-wrap`);
            const targetElement = document.querySelector('#properties-page-wrap');
            
            if (targetElement) {
                alert('DEBUG: Found target element, initializing script');
                ...
                alert(`DEBUG: Failed to find target element after ${CONFIG.MAX_RETRIES} attempts`);
                clearInterval(checkForElement);
                console.error('Properties Manager: Failed to initialize');
            } else {
                // Also check what elements are available that contain "properties"
                const allElements = document.querySelectorAll('[id*="properties"], [class*="properties"]');
                if (attempts === 1 || attempts % 10 === 0) { // Only show this every 10 attempts to avoid spam
                    alert(`DEBUG: Target element not found, attempt ${attempts}/${CONFIG.MAX_RETRIES}. Found ${allElements.length} elements containing 'properties'`);
                }
            }
        }, CONFIG.RETRY_DELAY);
    });
    ...
    alert('DEBUG: Setting up hashchange listener');
    ...
    alert('DEBUG: Script finished loading completely');
})();