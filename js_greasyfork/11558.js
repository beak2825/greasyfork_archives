...
// @downloadURL https://update.greasyfork.org/scripts/11558/FrenchUnMod.user.js
// @updateURL https://update.greasyfork.org/scripts/11558/FrenchUnMod.meta.js
// ==/UserScript==
...
if (a=="?a=settings") {
	...
	document.getElementById('content-mid').style.minHeight="1820px"; // Height of settings
...
if (GM_getValue(id+"UM_OP_unmodon", true)) {
	...
	if (a.substring(0,8)=="?a=equip") {
		...
		if (GM_getValue(id+"UM_OP_fastzk", true)) {
			...
			for (var i=0; i<itemS.length; i++) {
				...
				krew = false;
				if (GM_getValue(id+"UM_OP_zkkrew", true)) if (ta[0].innerHTML.search('Ours')>0 || ta[0].innerHTML.search('Chamanis')>0 || ta[0].innerHTML.search('Osseu')>0 || ta[0].innerHTML.search('Dracula')>0 || ta[0].innerHTML.search('Élastique')>0 || ta[0].innerHTML.search('Astral')>0 || ta[0].innerHTML.search('Du Sang')>0) { 
		...
		if (GM_getValue(id+"UM_OP_sellLowItem", true)) {
			// mod clean-zk
	...
	if (a.substring(0,11)=="?a=auction&" && a.search('&t=69')>0 && GM_getValue(id+"UM_OP_auction", true)) {
		...
		for (i=1; i<tr.length; i++) {
			...
			sztuk = td[1+correction].innerHTML.substring(td[1+correction].innerHTML.search(':')+1).replace('</span>','');
			oferta = td[3+correction].innerHTML.replace(' ','');
	...
	if ("t"!=a[a.length-1] && "?a=profile&uid="==a.substring(0,15)) {
		...
		if (GM_getValue(id+"UM_OP_showHideArenaStats", false)) {
			var added = false;
			var endIndex = document.getElementsByClassName('profile-stats').length;
			for(var i = 0; i < endIndex; i++) {
				opis = document.getElementsByClassName('profile-stats')[i];
