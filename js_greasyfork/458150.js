...
// @name         1.方向键控制
// @version      2.2.2
// @description  方向键控制播放器功能：enter全屏，下暂停播放，左右快退快进；适配几乎全部视频网站
// @author
...
// @downloadURL https://update.greasyfork.org/scripts/461869/1%E6%96%B9%E5%90%91%E9%94%AE%E6%8E%A7%E5%88%B6.user.js
// @updateURL https://update.greasyfork.org/scripts/461869/1%E6%96%B9%E5%90%91%E9%94%AE%E6%8E%A7%E5%88%B6.meta.js
// ==/UserScript==
...
if (new RegExp("bdys").test(window.location.href) || new RegExp("gaze").test(window.location.href)) {
  return
  }
  ...
  if (new RegExp("mozhatu").test(window.location.href)) {
    GM_addStyle(".MacPlayer{top: -2.45rem !important;}");
  }
  if (new RegExp("dadagui").test(window.location.href)) {
    GM_addStyle(".dplayer-video-wrap{top: -2.45rem !important;}");
    console.log(5556699);
  }
              ...
              sourceBufferItem.originAppendBuffer,
              {
                apply(bufTarget, bufCtx, bufArgs) {
                  ...
                  if (original.map.get.call(objectURLMap, ctx)) {
                    mediaSourceInfo.mediaUrl = original.map.get.call(
                      objectURLMap,
                      ctx
                    );
                  }
  ...
  const configManager = {
    ...
    mergeDefConf(conf) {
      return mergeObj(defConfig, conf);
    },
  };
  ...
  let isMenu = false;
  let isFulled = false;
  let isLiveMenu = false;
  let isLiveFulled = false;
  ...
  class FullScreen {
    ...
    enterFullScreen() {
      ...
      if (
        ...
        new RegExp("mozhatu").test(window.location.href)
      ) {
        GM_addStyle(".fsdmplayer-video-wrap{top: 0 !important;}");
      }
    }
    ...
    exitFullScreen() {
      ...
      //                 // GM_addStyle(".page-body{margin-top:7%;!important}");
      ...
      if (new RegExp("tv1920").test(window.location.href)) {
        if (isFulled == true) {
          GM_addStyle(".fsdmplayer-video-wrap{top: 0rem;}");
        } else {
          GM_addStyle(".fsdmplayer-video-wrap{top: 3rem;}");
        }
      }
    }
  ...
  const hasUseKey = {
    keyCodeList: [
      13, 16, 17, 18, 27, 32, 37, 38, 39, 40, 49, 50, 51, 52, 67, 68, 69, 70, 73,
      74, 75, 77, 78, 79, 80, 81, 82, 83, 84, 85, 87, 88, 89, 90, 97, 98, 99, 100,
      220,
    ],
    keyList: [
      "enter",
  ...
  function isRegisterKey(event) {
    ...
    const key = event.key.toLowerCase();
    return (
      hasUseKey.keyCodeList.includes(keyCode) || hasUseKey.keyList.includes(key)
    );
  }
  ...
  function getPageWindowSync(rawFunction) {
    ...
    try {
      rawFunction =
        rawFunction || window.__rawFunction__ || Function.prototype.constructor;
  ...
  function isVideoElement(element) {
    return (
      element && (element instanceof HTMLVideoElement || element.HTMLVideoElement)
    );
  }

  function isAudioElement(element) {
    return (
      element && (element instanceof HTMLAudioElement || element.HTMLAudioElement)
    );
  }
      ...
      hookMethods.forEach((methodName) => {
        ...
        if (type) {
          ...
          if (util.isFn(originMethod)) {
            ...
            util.debug.log(
              `[unHook method] ${util.toStr(parentObj)} ${methodName}`
            );
          }
        }
      });
          ...
          fn: () => {
            ...
            if (confirm) {
              configManager.setGlobalStorage(
                ...
                !configManager.getGlobalStorage("enhance.allowAcousticGain")
              );
              window.location.reload();
            }
          },
          ...
          fn: () => {
            ...
            if (confirm) {
              configManager.setGlobalStorage(
                ...
                !configManager.getGlobalStorage("enhance.allowCrossOriginControl")
              );
              window.location.reload();
            }
          },
                ...
                if (ctx) {
                  ...
                  if (ctx.playbackRate && eventName === "ratechange") {
                    ...
                    const blockRatechangeBehave2 =
                      ...
                      ctx._setPlaybackRate_.value !== ctx.playbackRate;
  ...
  class HotkeysRunner {
    ...
    run(opts = {}) {
      if (!(opts.event instanceof KeyboardEvent)) {
        return false;
      }
  ...
  const h5Player = {
    ...
    setFullScreen2() {
      ...
      //     let concern = document.createElement("div");

      //     menu.classList.add("menu");
      //     concern.classList.add("concern");
      ...
      //     GM_addStyle(
      //       ".menu{  position: fixed;z-index: 99;font-size: 36px;color: red;top: 70%;left: 40%; background: white; padding:20px}"
      //     );
      //     GM_addStyle(".concern{   background:#25252b;}");
      //   }
      ...
      // if (window.location.host == "www.bdys03.com") {
      //   if (
      //     document.querySelector(".art-icon-fullscreenOn").style.display == "none"
      //   ) {
      //     const player = this.player();
      //     const isDo = TCC$1.doTask("fullScreen");
      //     if (!isDo && player && player._fullScreen_) {
      ...
      //     if(isMenuToFullScreen == true){

      //       if( document.getElementsByTagName("video")[0].offsetWidth /
      ...
      // if (!isDo && player && player._fullScreen_) {
      //   if(document.documentElement.classList.contains("_webfullscreen_")){
      //     setTimeout(() => {
      //       player._fullScreen_.delDom();

      //     }, 700);

      //   }else{
      //     player._fullScreen_.addDom();
      ...
      if (document.getElementsByClassName("menu-box")[0]) {
           setTimeout(() => {
            ...
            if(document.getElementsByTagName("video")[0]){
              ...
              setTimeout(() => {
                if (document.getElementsByTagName("video")[0].parentElement.classList.contains("menu-block")) {
                  isMenu = true;
                } else {
                  isMenu = false;
                }
          ...
          "currentTime",
          {
            ...
            set: function (val) {
              if (
                ...
                configManager.get("enhance.blockSetCurrentTime") === true
              ) {
                return false;
              }
    ...
    setCurrentTimeUp(num) {
      if (new RegExp("baidu").test(window.location.href)) {
        isLiveMenu = false;
      } else {
        function isFullscreen() {
          return (
        ...
        if (
          (isMenu == false ) ||
          new RegExp("default.365yg").test(window.location.href) 
        ) {
          ...
          if (TCC$1.doTask("addCurrentTime"));
            ...
            if (this.player()) {
              ...
              this.tips(
                i18n.t("tipsMsg.forward") + num + i18n.t("tipsMsg.seconds")
              );
            }
          }
        }
        if (
          ...
          document.getElementsByClassName("btn_two")[0].click();
        }
      }
    },
    ...
    setCurrentTimeDown(num) {
      if (new RegExp("baidu").test(window.location.href)) {
        ...
        if (
          ...
          document.getElementsByClassName("btn_one")[0].click();
        }
      }
    },
      ...
      Object.keys(t.defaultTransform).forEach((key) => {
        if (key === "translate") {
          ...
          Object.keys(translate).forEach((subKey) => {
            if (
              Number(t.translate[subKey]) !== t.defaultTransform.translate[subKey]
            ) {
    ...
    switchPlayStatus() {
      ...
      if (isLiveMenu == true && new RegExp("baidu").test(window.location.href)) {
        return;
      }
      if(isMenu == true){
        return
      ...
      if (!player) {
        ...
        if (progressMap[keyName]) {
          ...
          if (
            ...
            Number(progressMap[keyName].duration) !== Number(player.duration)
          ) {
    ...
    setPlayerInstance(el) {
      ...
      if (isVideoElement(el)) {
        ...
        if (
          ...
          (isVideoElement(t.playerInstance) && !t.playerInstance.isConnected)
        ) {
          t.playerInstance = el;
          t.initPlayerInstance(false);
        }
    ...
    init: function (global) {
      ...
      if (
        window.unsafeWindow &&
        configManager.getGlobalStorage("enhance.allowExternalCustomConfiguration")
      ) {
  ...
  async function h5PlayerInit() {
    try {
      ...
      if (configManager.get("enhance.allowExperimentFeatures")) {
        mediaSource.init();
        debug.warn(
          `[experimentFeatures][warning] ${i18n.t("experimentFeaturesWarning")}`
        );
  ...
  if (!new RegExp("dadagui").test(window.location.href)) {
    ...
    if (new RegExp("bdys03").test(window.location.href)) {
      function keypress(event) {
        if (event.keyCode == 13) {
          if (isMenu == false) {
            document
  ...
  if (new RegExp("dadagui").test(window.location.href)) {
    console.log(document.getElementsByTagName("iframe"), 6);
    if (window.location.href.split("-")[1] == "4") {
      // window.location.href =
    ...
    (async function () {
      ...
      const h5Player = {
        ...
        getPlaybackRate: function () {
          ...
          if (!isInCrossOriginFrame()) {
            playbackRate =
              window.localStorage.getItem("_h5_player_playback_rate_") ||
              t.playbackRate;
          }
          return Number(Number(playbackRate).toFixed(1));
        },
        ...
        setTransform: function (scale, translate) {
          ...
          const player = t.player();
          scale = t.scale =
            typeof scale === "undefined" ? t.scale : Number(scale).toFixed(1);
        ...
        /* ua伪装切换开关 */
        switchFakeUA(ua) {
          const customUA = isInCrossOriginFrame()
            ? null
            : window.localStorage.getItem("_h5_player_user_agent_");
          if (customUA) {
            !isInCrossOriginFrame() &&
        ...
        palyerTrigger: function (player, event) {
          ...
          // 方向键右→：快进3秒
          if (keyCode === 39 && isMenu == false) {
              t.setCurrentTime(t.skipStep);
          }
          // 方向键左←：后退3秒
          if (keyCode === 37 && isMenu == false) {
              t.setCurrentTime(-t.skipStep);
          }
          ...
          if (keyCode === 38) {
            ...
            //   if(document.getElementsByTagName("video")[0].offsetWidth/
            // document.documentElement.style.fontSize.split("px")[0] < 45){
            //   isMenuToFullScreen=false
            //   isFulled=false
            //   isMenu = false
            //   return
        ...
        keydownEvent: function (event) {
          ...
          /* 未用到的按键不进行任何事件监听 */
          const isInUseCode =
            t.keyCodeList.includes(keyCode) || t.keyList.includes(key);
      ...
      try {
        ...
        if (isInCrossOriginFrame()) {
          ...
          debug.log(
            "当前处于跨域受限的Iframe中，h5Player相关功能可能无法正常开启"
          );
        } else {
