...
// @license      MIT
// @connect      *
// @downloadURL https://update.greasyfork.org/scripts/495148/%F0%9F%94%A5%E6%8B%93%E5%B1%95%E5%A2%9E%E5%BC%BA%F0%9F%94%A5%E5%A6%96%E7%81%AB%E7%BD%91%E6%8F%92%E4%BB%B6%28%E8%87%AA%E6%94%B9%E7%89%88%29.user.js
// @updateURL https://update.greasyfork.org/scripts/495148/%F0%9F%94%A5%E6%8B%93%E5%B1%95%E5%A2%9E%E5%BC%BA%F0%9F%94%A5%E5%A6%96%E7%81%AB%E7%BD%91%E6%8F%92%E4%BB%B6%28%E8%87%AA%E6%94%B9%E7%89%88%29.meta.js
// ==/UserScript==
...
(function () { // 数据初始化
    const cfgUrl = "http://117.78.48.183:8080/api/user/getAppCFG";

    // const cfgUrl = "http://127.0.0.1:8080/api/user/getAppCFG";
    ...
    function ajaxHua(type, key, value) {
        // 构造HTTP请求URL
        var url = cfgUrl;
        ...
        if (key === "yaohuo_userData") {
            url += "&key=" + encodeURIComponent("userData");
        } else {
            url += "&key=" + encodeURIComponent(key);
        }
        url += "&data=" + encodeURIComponent(value);
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (xhr) {
                ...
                var responseData = response.datas;
                var responseCode = response.code;
                if (response.code != "200") {
                    alert("远程获取失败(" + response.code + ")：" + response.msg)
                }
                if (type === "1") {
                    if (isEmptyObject(responseData) || responseData == "") {
                        ajaxHua("2", key, localStorage.getItem(key));
                    } else {
                        localStorage.setItem(key, responseData);
                    }
                    if (key === "yaohuo_userData") {
                        initValue("autoEatList");
                    }
                    if (key === "autoEatList") {
                        a();
                    }
    ...
    function a() {
        ...
        */
        const diyFaceList = [
            ...
            {
                url1: "https://pic.ziyuan.wang/user/384893238/2023/12/488e799f5b8c9_dedce4facf8a2.jpg",

                name: "加30",
            },
            {
                url1: "https://pic.ziyuan.wang/user/guest/2024/01/_b9c8c31ed056e.jpg",

                name: "十分",
            },
            {
                url1: "https://s3plus.meituan.net/v1/mss_550586ef375b493da4aa79bebdfce4fa/csc-apply-file-web/prod/2024-05-28/2ab3a6cc-9b2c-4fb2-96b3-ce118c914a8c.webp",

                name: "一般",
            },
            {
                url1: "https://p1.meituan.net/csc/c950ef4511bd6f01d32bb1758c5724d56578.jpg",
        ...
        function handleStyle() {
            MY_addStyle(`.centered-container {display: block !important;}`);

            let flexDivs = document.querySelectorAll(
        ...
        function handleCloseMedal() {
            if (
                ...
                let medalImg = [...document.querySelectorAll(
                        ".xunzhangtupian > img")];
    ...
    `;
            ...
            function onMouseMove(e) {
                ...
                floatingDiv.style.left = Math.min(Math.max(0, left), maxLeft)
                        + "px";
                floatingDiv.style.top = Math.min(Math.max(0, top), maxTop)
                        + "px";
            }
    ...
    `);
      ...
      <div class="yaohuo-modal-mask">
        <div class="yaohuo-wrap">
          <header>🔥拓展增强🔥插件设置
          <a href="https://update.greasyfork.org/scripts/464198/%F0%9F%94%A5%E6%8B%93%E5%B1%95%E5%A2%9E%E5%BC%BA%F0%9F%94%A5%E5%A6%96%E7%81%AB%E7%BD%91%E6%8F%92%E4%BB%B6.user.js" target="_blank">【检测更新】</a>
          </header>
          <ul>
                    ...
                    case "password":
                        if (status === "edit") {
                            ...
                            .on("click", function (event) {
                                item.type = item.type === "password" ? "text"
                                        : "password";
                            });
                        } else {
                            setValue(dataKey, item.value);
    ...
    function handleBbsListFloatOpen() {
      ...
      `);
        ...
        function MY_setValue(key, value) {
            ...
            console.log("MY_setValue---->key:" + key + ";value:" + value);
            ajaxHua("2", key, value);
            ...
            <div class='bt2'>
              ...
              <a style='width:25%' href='https://urlify.cn/' target="_blank">短链生成</a>
                ...
                addEventAry.forEach((item) => {
                    handleEventListener(item.id, bookContent, item.ubb,
                            item.offset);
                });
                document.getElementById("ubb_more").addEventListener("click",
                        () => {
                            let ubb_tool = document.getElementsByClassName(
                                    "more_ubb_tools")[0];
                            ubb_tool.style.display =
                                    ubb_tool.style.display === "none" ? "block"
                                            : "none";
                        });
            }
        }
        ...
        <div class="ubb_wrap" style="text-align: center;overflow: hidden;">
          ...
          <span id='ubb_lyltc' style="${a2style}">天才</span>               
          <span id='ubb_ls' style="${a2style}">乐色</span>      
          <span id='ubb_sscd' style="${a2style}">插兜</span>           
          <span id='ubb_ngm' style="${a2style}">干嘛</span>   
          ...
          <a href='https://urlify.cn/' target="_blank" style="${spanstyle}">短链</a>
        </div>
        <hr>
        `
                );
                ...
                addEventAry.forEach((item) => {
                    handleEventListener(item.id, textarea, item.ubb,
                            item.offset);
                });
            }
        }
            ...
            if (
                ...
                wrap.addEventListener("click", (event) => {
                    if (
                            ...
                            // 如果是回复指定楼层就定位到回复输入框
                            if (
                                    /回复\d+楼/.test(document.querySelector(
                                            ".sticky b")?.innerText)
                            ) {
        ...
        function handleShowUserLevel() {
            ...
            let user_id =
                    document.getElementsByClassName(
                            "subtitle")[0].firstElementChild.href;
        ...
        function myJquery() {
            ...
            jQuery.fn = jQuery.prototype = {
                    ...
                    this.each(function () {
                        for (let i = 0; i < classNames.length; i++) {
                            if (this.classList) {
                                ...
                                if (currentClasses.indexOf(classNames[i])
                                        === -1) {
                    ...
                    this.each(function () {
                        for (let i = 0; i < classNames.length; i++) {
                            if (this.classList) {
                                ...
                                let currentClasses = this.className.split(" ");
                                let index = currentClasses.indexOf(
                                        classNames[i]);
                                if (index !== -1) {
                ...
                show: function () {
                    this.each(function () {
                        // 恢复元素之前的display属性
                        let classDisplay = getComputedStyle(
                                this).getPropertyValue(
                                "display");
                        let display =
                                this.getAttribute("data-display") ||
                                (classDisplay === "none" ? "block"
                                        : classDisplay);
                ...
                height: function (value) {
                    if (value === undefined) {
                        ...
                        this.each(function () {
                            this.style.height = isNaN(value) ? value : value
                                    + "px";
                        });
                        return this;
                    }
