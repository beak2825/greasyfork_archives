...
// @version      08.08
// @description  try to control the world!
// @author       You
// @match        https://www.erepublik.com/en
// @icon         https://www.google.com/s2/favicons?sz=64&domain=erepublik.com
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/520861/NEW%20NEW%20NEW.user.js
// @updateURL https://update.greasyfork.org/scripts/520861/NEW%20NEW%20NEW.meta.js
// ==/UserScript==
(function() {
    'use strict';
    const force = 65;
    ...
    const low = Number(getLocalStorageData('dmgL'));
    const high = Number(getLocalStorageData('dmgH'));
    ...
    if (captcha) {
        const mCapth = `${cName}\nverify Capthca https://www.erepublik.com/en`;
        sendMessage(mCapth);
        // Delay of 2 seconds
        setTimeout(function() {
            reloadPage();
    ...
    async function getBattleInfo(){
        ...
        const filteredInfo = data.division.filter(div => div.division === 11);
    ...
    async function checkZone(timeDiff){
        let weapQu = Number(getLocalStorageData('weapQ'));
        ...
        if(timeDiff < 7200){
            ...
            if(started == 1 && finished == 0){
                ...
                if(targetEnemy == force && deffend == 1){
                ...
                if(!empty){
                    ...
                    const hit = Math.floor(damage/dph) * 10;
                    logMessage(`DPH: ${dph}`);
...
/*                         if(deffend == 1 && damage > 45000){
    ...
    async function scanBattle() {
        ...
        if (list.length > 0) {
            ...
            if(energy > maxPower || (!defff && energy < maxPower && energy > minPower)) {
                saveToLocalStorage('weapQ', 0);
            } else {
                saveToLocalStorage('weapQ', powerPlus);
            }
            ...
            <div style="background-color:white; margin:15% auto; padding:20px; border:1px solid #888; width:900px; max-width:90%; position:relative; display:flex;">
                <div style="flex:1; padding:10px;">
                    ...
                    <form id="dataForm" style="width:200px;">
                        ...
                        <input type="number" id="dmgL" value="${getLocalStorageData(keys.dmgL)}" style="border:none; border-bottom:1px solid #ccc; outline:none; width:50%; padding:5px; margin-bottom:10px;"><br>
                        <label for="dmgH">HIGH DAMAGE:</label><br>
                        <input type="number" id="dmgH" value="${getLocalStorageData(keys.dmgH)}" style="border:none; border-bottom:1px solid #ccc; outline:none; width:50%; padding:5px; margin-bottom:10px;"><br>
                        <label for="weapQ">WEAPON QUALITY:</label><br>
                        <input type="number" id="weapQ" value="${getLocalStorageData(keys.weapQ)}" style="border:none; border-bottom:1px solid #ccc; outline:none; width:20%; padding:5px; margin-bottom:10px;" max="10"><br>
    ...
    `;
        ...
        saveButton.onclick = function() {
            ...
            saveToLocalStorage('targetCountry', targetCountry.value);

            // Save other form data
            saveToLocalStorage('dmgL', document.getElementById('dmgL').value);
            saveToLocalStorage('dmgH', document.getElementById('dmgH').value);
            saveToLocalStorage('weapQ', document.getElementById('weapQ').value);
            saveToLocalStorage('round', 0);
    ...
    function defaultDamage(){
        ...
        if(deffend == 1){
            ...
            damage = low + plus - random1 - random2;
        }
        return damage;
    ...
    function compareDamage(opposite){
        ...
        if(deffend == 1){
            if(opposite > low && opposite < high && dom < 18){
                damage = high2;
            ...
            //lose
            if(opposite > high && opposite < dhigh){
                damage = limit;
            } else if(opposite > mid && opposite < high){
                damage = mid;
            } else if(opposite > low && opposite < mid){
                damage = lower;
            } else {
                damage = low;
            }
...
/*         else {
    ...
    async function checkAllBattle(data){
        ...
        var list = [{
            ...
            region: 665, //Lithuanian Highland
            region_name: "Lithuanian Highland"
        }, {
            war_id: 215758,
            id: 669680,
            country_id: 166, //UAE
            ...
            war_id: 216333,
            id: 685462,
            country_id: 42, //Bulgaria
            region: 445, //Uttar Pradesh
