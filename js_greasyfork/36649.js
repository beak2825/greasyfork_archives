// ==UserScript==
// @name          Eternity Tower Enemy Vitals
// @icon          https://www.eternitytower.net/favicon.png
// @namespace     http://mean.cloud/
// @version       1.19.5
// @description   Shows the vital statistics of enemies in battle
// @match         *://eternitytower.net/*
// @match         *://www.eternitytower.net/*
// @author        psouza4@gmail.com
// @copyright     2017-2023, Peter Souza
// @run-at        document-end
// @grant         GM_getValue
// @grant         GM_setValue
// @downloadURL https://update.greasyfork.org/scripts/36649/Eternity%20Tower%20Enemy%20Vitals.user.js
// @updateURL https://update.greasyfork.org/scripts/36649/Eternity%20Tower%20Enemy%20Vitals.meta.js
// ==/UserScript==


////////////////////////////////////////////////////////////////
////////////// ** SCRIPT GLOBAL INITIALIZATION ** //////////////
ET_EnemyVitalsMod_Settings = {};
function startup() { ET_EnemyVitalsMod(); }
////////////////////////////////////////////////////////////////


ET_EnemyVitalsMod = function()
{
	//ET.MCMF.WantDebug = true;
	
    ET.MCMF.Ready(function()
    {
        ET_EnemyVitalsMod_Settings.DisplayMode = CInt(GM_getValue("PETEnemyDetails_DisplayMode")); // default 0
    });
    
    ET.MCMF.EventSubscribe("ET:firstBattleFrame", function()
    {
        if (ET.MCMF.WantDebug)
        {
            //ET.MCMF.Log("new BattleFirstFrame data:");
            //console.log(ET.MCMF.BattleFirstFrame);
            
            //ET.MCMF.Log("current enemy unit ID list: " + ET.MCMF.LastUnitIDList);
        }
    });
    ...
    ET.MCMF.EventSubscribe("ET:combatTick", function()
    {
        try
        {
            if (ET.MCMF.BattleUITemplate !== undefined)
            {
                let battleData = ET.MCMF.LiveBattleData();

                if (jQ(".PETEnemyDetailsOptionsB1").length === 0)
                {
                    //jQ(".PETEnemyDetailsOptionsB1").remove();
                    jQ("div.buff-icon-container > img[src=\"/icons/amulet.svg\"").parent().parent().prepend
                        ("<div id=\"PETEnemyDetailsOptionsB1\" class=\"PETEnemyDetailsOptionsB1 buff-icon-container icon-box medium-icon btn btn-primary\" style=\"text-muted\"></div>");

                    ET_EnemyVitalsMod_CreateTooltip();
                    
                    if (ET_EnemyVitalsMod_Settings.DisplayMode === 0)
                        jQ(".PETEnemyDetailsOptionsB1").html("<span style=\"font-size: 8pt; line-height: 10pt;\">Details<br />Standard</span>");
                    else if (ET_EnemyVitalsMod_Settings.DisplayMode === 1)
                        jQ(".PETEnemyDetailsOptionsB1").html("<span style=\"font-size: 8pt; line-height: 10pt;\">Details<br />Full</span>");
                    else if (ET_EnemyVitalsMod_Settings.DisplayMode === 2)
                        jQ(".PETEnemyDetailsOptionsB1").html("<span style=\"font-size: 8pt; line-height: 10pt;\">Details<br />Minimal</span>");
                    else if (ET_EnemyVitalsMod_Settings.DisplayMode === 3)
                        jQ(".PETEnemyDetailsOptionsB1").html("<span style=\"font-size: 8pt; line-height: 10pt;\">Details<br />None</span>");
                        
                    jQ(".PETEnemyDetailsOptionsB1").click(function()
                    {
                        ET_EnemyVitalsMod_Settings.DisplayMode = (ET_EnemyVitalsMod_Settings.DisplayMode + 1) % 4;
                        
                        GM_setValue("PETEnemyDetails_DisplayMode", ET_EnemyVitalsMod_Settings.DisplayMode);
                ...
                //console.log(ET.MCMF.BattleUnitList);
                jQ.makeArray(battleData.enemies).forEach(function(currentMonster)
                {
                    //ET.MCMF.Log(JSON.stringify(currentMonster));
                    //ET.MCMF.Log(JSON.stringify(window.ET.MCMF.PlayerUnitData));

                    sThisMonsterID = currentMonster.id;
                    oOriginalMonsterData = ET_EnemyVitalsMod_MatchOriginalMonster(sThisMonsterID);
                    dThisMonsterArmor = parseFloat(currentMonster.stats.armor);
                    if (dThisMonsterArmor < parseFloat(0))
                        dThisMonsterArmor = parseFloat(0);
                    dThisMonsterOriginalArmor = parseFloat(0);
                    if (oOriginalMonsterData !== undefined)
                    {
                        dThisMonsterOriginalArmor = parseFloat(oOriginalMonsterData.stats.armor);
                        //ET.MCMF.Log("Original monster vs current monsterr data for this monster: " + sThisMonsterID);
                        //console.log(oOriginalMonsterData);
                        //console.log(currentMonster);
                        
                        dThisMonsterArmor = dThisMonsterOriginalArmor;
                        jQ.makeArray(currentMonster.buffs).forEach(function(oThisBuff)
                        {
                            if (CDbl(oThisBuff.data.totalArmorReduction) > 0)
                                dThisMonsterArmor -= CDbl(oThisBuff.data.totalArmorReduction);
                        });
                        
                        if (dThisMonsterArmor < 0.0)
                            dThisMonsterArmor = 0.0; 

                        if (dThisMonsterArmor === dThisMonsterOriginalArmor)
                            dThisMonsterArmor = CDbl(currentMonster.stats.armor);
                    }
                    //else
                    //{
                    //    ET.MCMF.Log("Couldn't find original monster data for this monster: " + sThisMonsterID);
                    //    console.log(currentMonster);
                    //}
                    ...
                    jQ("div.battle-unit-container").each(function()
                    {
                        try
                        {
                            var icon = jQ(this).find("img.enemy-icon");
                            ...
                            if (icon.length > 0)
                            {
                                ...
                                {
                                    sMonsterArmorText = "";
                                    dThisMonsterArmor = CDbl(dThisMonsterArmor);
                                    dThisMonsterOriginalArmor = CDbl(dThisMonsterOriginalArmor);

                                    if (ET_EnemyVitalsMod_Settings.DisplayMode === 2)
                                    {
                                        if ((Math.abs(dThisMonsterOriginalArmor - dThisMonsterArmor) < 1) || (dThisMonsterOriginalArmor === 0))
                                            sMonsterArmorText = dThisMonsterArmor.toFixed(0);
                                        else
                                            sMonsterArmorText = "<span style=\"color: red;\">" + dThisMonsterArmor.toFixed(0) + "</span>/" + dThisMonsterOriginalArmor.toFixed(0);
                                    }
                                    else
                                    {
                                        if ((Math.abs(dThisMonsterOriginalArmor - dThisMonsterArmor) < 1) || (dThisMonsterOriginalArmor === 0))
                                            ...
                                            sMonsterArmorText = "<span style=\"color: red;\">" + dThisMonsterArmor.toFixed(0) + "</span>&nbsp;of&nbsp;" + dThisMonsterOriginalArmor.toFixed(0);
                                    }

                                    jQ(this).parent().find(".PETE_EnemyDetails").remove();

                                    let dDPS = 0.0;
                                    try {
                                        dDPS = (currentMonster.stats.attack + ((CDbl(currentMonster.stats.attackMax) - CDbl(currentMonster.stats.attack)) / 2.0)) * currentMonster.stats.attackSpeed; } catch (err) { }

                                    if (ET_EnemyVitalsMod_Settings.DisplayMode === 0)
                                    {
                                        jQ(this).parent().find(".buff-container").after(
                                            "<div class=\"PETE_EnemyDetails\">" +
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attackSpeed-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/attackSpeed.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16dpx; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;DPS:</b> &nbsp;" + dDPS.toFixed(0) + "</div>" +
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attack-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/defense.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16px; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;Defense:</b> &nbsp;" + CDbl(currentMonster.stats.defense).toFixed(0) + "</div>" +
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attack-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/armor.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16px; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;Armor:</b> &nbsp;" + sMonsterArmorText + "</div>" +
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attack-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/magicArmor.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16px; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;M. Armor:</b> &nbsp;" + CDbl(currentMonster.stats.magicArmor).toFixed(0) + "</div>" +                                            
                                            "</div>");
                                    }
                                    else if (ET_EnemyVitalsMod_Settings.DisplayMode === 1)
                                    {
                                        jQ(this).parent().find(".buff-container").after(
                                            ...
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attackSpeed-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/attackSpeed.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16dpx; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;DPS:</b> &nbsp;" + dDPS.toFixed(0) + "</div>" +
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attack-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/attack.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16dpx; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;Attack:</b> &nbsp;" + CDbl(currentMonster.stats.attack).toFixed(0) + " - " + CDbl(currentMonster.stats.attackMax).toFixed(0) + "</div>" +
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attack-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/accuracy.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16px; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;Speed:</b> &nbsp;" + CDbl(currentMonster.stats.attackSpeed).toFixed(1) + "</div>" +
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attackSpeed-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/attackSpeed.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16px; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;Accuracy:</b> &nbsp;" + CDbl(currentMonster.stats.accuracy).toFixed(0) + "</div>" +
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attack-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/defense.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16px; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;Defense:</b> &nbsp;" + CDbl(currentMonster.stats.defense).toFixed(0) + "</div>" +
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attack-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/armor.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16px; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;Armor:</b> &nbsp;" + sMonsterArmorText + "</div>" +
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attack-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/magicPower.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16px; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;M. Power:</b> &nbsp;" + CDbl(currentMonster.stats.magicPower).toFixed(0) + "</div>" +
                                            "<div class=\"d-flex flex-row mb-1\"><div class=\"d-flex align-items-center attack-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/magicArmor.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16px; border: none !important;\">\r\n</div>" +
                                            "<b>&nbsp;M. Armor:</b> &nbsp;" + CDbl(currentMonster.stats.magicArmor).toFixed(0) + "</div>" +                                            
                                            "</div>");
                                    }
                                    else if (ET_EnemyVitalsMod_Settings.DisplayMode === 2)
                                    {
                                        jQ(this).parent().find(".buff-container").after(
                                            ...
                                            "<div class=\"d-flex align-items-center attackSpeed-tooltip-container drop-target drop-abutted drop-abutted-left drop-abutted-top drop-element-attached-bottom drop-element-attached-left drop-target-attached-top drop-target-attached-left\">\r\n<img src=\"/icons/attackSpeed.svg\" class=\"extra-small-icon\" style=\"width: 16px; height: 16dpx; border: none !important;\">\r\n</div>" +
                                            "&nbsp;" + dDPS.toFixed(0) + "&nbsp;&nbsp;&nbsp;" +
                        ...
                        catch (err) { ET.MCMF.Log("Enemy Vitals::Error in div.battle-unit-container.each():"); console.log(err); }
                    });
                });
            }
        }
        catch (err) { ET.MCMF.Log("Enemy Vitals::Error in ET:combatTick event:"); console.log(err); }
    });
};

ET_EnemyVitalsMod_CreateTooltip = function()
{
    oEl = jQ("#PETEnemyDetailsOptionsB1").get(0);
    if (!oEl)
        return;
    ...
    jQ("#tooltip-PETEnemyDetailsOptionsB1").remove();
    jQ("body").append("<div class=\"item-tooltip-content my-tooltip-inner\" id=\"tooltip-PETEnemyDetailsOptionsB1\"></div>");

    jQ("#tooltip-PETEnemyDetailsOptionsB1").html
    (
        "    <h3 class=\"popover-title\">" +
        "        Enemy Details Addon" +
        "    </h3>" +
        "    <div class=\"popover-content\">" +
        "        Click to cycle through enemy details display modes.<br />" +
        "    </div>"
    );    

    tippy("#PETEnemyDetailsOptionsB1",
    {
        html: jQ("#tooltip-PETEnemyDetailsOptionsB1")[0],
        performance: !0,
        animateFill: !1,
        distance: 5
    });
...
ET_EnemyVitalsMod_MatchOriginalMonster = function(id)
{
    if (ET.MCMF.BattleFirstFrame !== undefined)
    {
        try
        {
            originalEnemies = jQ.makeArray(ET.MCMF.BattleFirstFrame.enemies);

            for (let i = 0; i < originalEnemies.length; i++)
                if (originalEnemies[i].id === id)
                    return originalEnemies[i];
        }
        catch (err) { }
    }
