...
// @grant        http://code.jquery.com/jquery-3.5.1.min.js
// @downloadURL https://update.greasyfork.org/scripts/422376/Coconut%20Shy%20%20Bagatelle.user.js
// @updateURL https://update.greasyfork.org/scripts/422376/Coconut%20Shy%20%20Bagatelle.meta.js
// ==/UserScript==
	...
	(function () {
		...
		function playCS(count) {
			return new Promise(resolve => {
				setTimeout(() => {
					console.log("Playing game #" + count);
					$.ajax({
						url: "http://www.neopets.com/halloween/process_cocoshy.phtml?coconut=3",
						async: false,
						success: function (result) {
							let stats = result.match(/\d+/g);
							let msg = format_ascii(result.split("error=")[1]);
							resolve({
								"status": true,
								"points": stats[0],
								"np": stats[1],
			...
			return new Promise(resolve => {
				setTimeout(() => {
					...
					$.ajax({
						...
						async: false,
						success: function (result) {
							let points = result.split("points=")[1].split("&")[0];
							let np = result.split("np=")[1].split("&")[0];
							let slot = calc_slot(result.split("success=")[1].split("&")[0]);
							resolve({
								"status": true,
								"points": points,
								"np": np,
...
function format_ascii(str) {
	let ascii = str.match(/%../g);
	for (let i = 0; i < ascii.length; i++) {
		let cc = parseInt(`0x${ascii[i].replace("%", "")}`);
...
function calc_slot(string) {
	...
	-----------------------------------------------------*/
	let num = string.replace(/R/g, "0").replace(/L/g, "1").split("");
	let digits = [];
	for (let i = 0; i < 4; i++) {
		digits.push(parseInt(num[i]));
	}
	let binary = parseInt(digits.join("")); // remove leading 0
