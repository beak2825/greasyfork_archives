// ==UserScript==
// @name         Projection Viewer Demo
// @namespace    http://tampermonkey.net/
// @version      0.1.14
// @description  Projection Viewer for annotation tool
// @author       You
...
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/450536/Projection%20Viewer%20Demo.user.js
// @updateURL https://update.greasyfork.org/scripts/450536/Projection%20Viewer%20Demo.meta.js
// ==/UserScript==
...
async function mappingUrl(side, imageUrl) {
    // const mapping = (await (
    //   await fetch(
    //     "https://echo-apps.oss-cn-beijing.aliyuncs.com/notions/chrome_extension_shangqi_viewer.json"
    //   )
    // ).json()) as { id: string; mapping: number; name: string }[];
    // const apiVersion =
    //   mapping.find((s) => imageUrl.includes(s.name))?.mapping ?? 1;
    const apiVersion = 4;
...
function getBirdEyeImage(attachment) {
    if (attachment.includes('测试批/fish/')) {
        return attachment.replace("测试批/fish/", "测试批/bird/").replace('.jpg', '.png');
    }
