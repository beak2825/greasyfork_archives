// ==UserScript==
// @name         01奶(20180904)
// @namespace    http://swordman-s1.yytou.com/?id=6733667&time=1518582711721&key=1debae1c0da58d68a07059359bfc7f41&s_line=1
// @version      2.1
// @description  try to take over the world!
// @author       MARBO&jj
// @match        http://swordman-s1.yytou.com/?id=6733667&time=1518582711721&key=1debae1c0da58d68a07059359bfc7f41&s_line=1
// @grant        note
// @downloadURL https://update.greasyfork.org/scripts/372752/01%E5%A5%B6%2820180904%29.user.js
// @updateURL https://update.greasyfork.org/scripts/372752/01%E5%A5%B6%2820180904%29.meta.js
// ==/UserScript==
...
var playerName = '千門牛奶';
var playerMaxHp = '186572';
var enforcePoints = '1175';
...
/*  clickButton('clan buy 601');//空識卷軸
    ...
    clickButton('enable mo-zhang-jue', 1);//魔掌訣
    clickButton('practice mo-zhang-jue', 1);
    clickButton('enable mystsword', 1);//小步玄劍
    clickButton('practice mystsword', 1);
    clickButton('enable jinyu-quan', 1);//金玉拳
    clickButton('practice jinyu-quan', 1);
    clickButton('enable cloudstaff', 1);//白雲杖法
    clickButton('practice cloudstaff', 1);
    r3();//技能準備
   ...
   var bm = 0,
    ...
    function QiXiaTalkFunc() {
        // if (userqu == 15 || userqu == 47 || userqu == 51 || userqu == 56 || userqu == 57 || userqu == 64 || userqu == 66 || userqu == 68) {
            var a = prompt("請輸入撩奇俠順序（中文分號隔開）", "步驚鴻；火雲邪神；郭濟；浪喚雨；宇文無敵；王蓉；逆風舞；李宇飛；風南；龐統；玄月研；狼居胥；烈九州；穆妙羽；李玄霸；八部龍將；風無痕；厲滄若；夏嶽卿；妙無心；巫夜姬");
            qxList = a.split("；");
            qxOrder = 0;
...
function autoBattleFunc(){
    ...
    if(!skillstr){
        ...
        var skillstr = '九天龍吟劍法,排雲掌法,8,道種心魔經';
...
//				    var aaa = ['悟性裝','寶箱3000','通靈','上香','打鳥','冰月谷','突破丹禮包','狂暴丹','掌門手諭','投資','煉藥','拼圖','簽到','禦寒衣','刻刀'];
				    var aaa = ['掌門手諭','吃藥','清包包','搜朱果','刻刀','禦寒衣','青龍技能','技能準備','幫派繳圖','悟性裝','正常裝','師門幫派','茅山天師','取消茅山','準備銅人','打銅人','冰月谷','打鳥','突破','破障除魔','2倍包月','全任務'];
			    var bbb = [];
				...
				if(qmjname2=='取消茅山'){clickButton('enable mapped_skills restore go 1', 1);clickButton('golook_room');}
				if(qmjname2=='不動明王'){r6();}
				if(qmjname2=='準備銅人'){
                    ...
                    //clickButton('enableskill enable dodge mysterrier', 1);
...
function r2(){//青龍技能
    ...
    clickButton('enableskill enable force yijinjing', 1);//易筋經神功
    clickButton('enable unmap_all', 1);//一鍵取消
    ...
    clickButton('enableskill enable force bahuang-gong', 1);//八荒六合惟我獨尊功
    clickButton('enableskill enable force yijinjing', 1);//易筋經神功
    clickButton('enableskill enable lingboweibu', 1);//淩波微步
    clickButton('enableskill enable dodge qiankun-danuoyi', 1);//乾坤大挪移
    clickButton('enableskill enable parry zhenwu-jian', 1);//真武七截劍
    clickButton('enableskill enable parry dugu-jiujian', 1);//獨孤九劍
    clickButton('golook_room');
}
function r3(){//技能準備
    ...
    clickButton('enableskill enable unarmed anran-zhang', 1);//黯然銷魂掌
    clickButton('enableskill enable unarmed liumai-shenjian', 1);//六脈神劍
    clickButton('enableskill enable liumai-shenjian attack_select', 1);//六脈神劍必刷
    clickButton('enableskill enable sword iron-sword', 1);//玄鐵劍法
    clickButton('enableskill enable sword jiutian-sword', 1);//九天龍吟劍法
    clickButton('enableskill enable jiutian-sword attack_select', 1);//九天龍吟劍法必刷
    ...
    clickButton('enableskill enable force bahuang-gong', 1);//八荒六合惟我獨尊功
    clickButton('enableskill enable force yijinjing', 1);//易筋經神功
    clickButton('enableskill enable yijinjing attack_select', 1);//易筋經神功必刷
    clickButton('enableskill enable lingboweibu', 1);//淩波微步
    clickButton('enableskill enable dodge qiankun-danuoyi', 1);//乾坤大挪移
...
gSocketMsg.addMessage2 = function(cc) {
        ...
        for (this._msg += cc; 0 < this._msg.length; ) {
            ...
            if(this._msg_line.indexOf("你的八荒功成功向前突破了，請繼續努力！") != -1){tupostart();}
            if(this._msg_line.indexOf("你的降龍十八掌成功向前突破了，請繼續努力！") != -1){tupostart();}
            if(this._msg_line.indexOf("你的帝王劍法成功向前突破了，請繼續努力！") != -1){tupostart();}
            if(this._msg_line.indexOf("你的闢邪劍法成功向前突破了，請繼續努力！") != -1){tupostart();}
            0 < this._msg_line.length && this.recvNetWork(this._msg_line);
            this._msg_len = this._msg_start = 0
        }
    }
                    function tupostart(){
                    clickButton('enable pixie-sword', 1);//闢邪劍法 6/9->13
                    clickButton('tupo go,pixie-sword', 1);
                    clickButton('tupo_speedup pixie-sword go', 0);

                    clickButton('enable sszaohuagong', 1);//生生造化功 10/9->20
                    clickButton('tupo go,sszaohuagong', 1);
                    clickButton('tupo_speedup2 sszaohuagong go', 0);

                    clickButton('enable king-sword', 1);//帝王劍法 11/9->10
                    clickButton('tupo go,king-sword', 1);
                    clickButton('tupo_speedup king-sword go', 0);

                    clickButton('enable fuyu-sword', 1);//覆雨劍法 31/9->20
                    clickButton('tupo go,fuyu-sword', 1);
                    clickButton('tupo_speedup2 fuyu-sword go', 0);

                    clickButton('enable xianglong-zhang', 1);//降龍十八掌 24/9->13
                    clickButton('tupo go,xianglong-zhang', 1);
                    clickButton('tupo_speedup xianglong-zhang go', 0);

                    clickButton('enable bahuang-gong', 1);//八荒功 9/10->15
                    clickButton('tupo go,bahuang-gong', 1);
                    clickButton('tupo_speedup bahuang-gong go', 0);
