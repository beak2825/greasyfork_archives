// ==UserScript==
// @name         Rotten Tomatoes export CSV button with Reviews- update -2020
// @namespace
// @version      0.5
...
// @downloadURL https://update.greasyfork.org/scripts/395957/Rotten%20Tomatoes%20export%20CSV%20button%20with%20Reviews-%20update%20-2020.user.js
// @updateURL https://update.greasyfork.org/scripts/395957/Rotten%20Tomatoes%20export%20CSV%20button%20with%20Reviews-%20update%20-2020.meta.js
// ==/UserScript==
debugger;
...
var ratingBoxes = document.getElementsByClassName("ratings__user-rating-review");
var ratingsArray = [];

for (var i = ratingBoxes.length - 1; i >= 0; i--) {
	var boxBody = ratingBoxes[i].getElementsByClassName("bottom_divider ratings__user-rating")[0];

	var boxHeading = boxBody.getElementsByClassName("ratings__review-top")[0];
	...
	var ratingDiv = boxBody.getElementsByTagName("div")[1];
    var ratinghalf = 0;
    var ratingfull = ratingBoxes[i].getElementsByClassName("star-display__filled ").length;
    ratinghalf = ratingBoxes[i].getElementsByClassName("star-display__half ").length;
	if (ratinghalf == 1) {
		ratingValue = ratingfull + 0.5;
	}
	else {
		ratingValue = ratingfull;
	}

    var ratingLog = boxBody.getElementsByClassName("ratings__age")[0].textContent;
    ...
    var reviewRaw_full = boxBody.getElementsByClassName("ratings__comment")[0].textContent;
    var reviewRaw = reviewRaw_full.replaceAll("\"", "\"\""); //replace double quotes with double double quotes to prepare for comma sanitziation for CSV format
    reviewRaw = reviewRaw.replace(/\n/g, " ");
    reviewRaw = reviewRaw.replace(/[`~!@#$%^&*()]/gi, '');
...
var pageBody = document.getElementsByClassName("panel-heading")[0]
//debugger;
function download() {
	link.click();
	return;
