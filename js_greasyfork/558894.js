// ==UserScript==
// @name            HDrezka Improvement для ноута NEW
// @name:en         HDrezka Improvement для ноута NEW
// @name:uk         HDrezka Improvement для ноута NEW
// @name:ru         HDrezka Improvement для ноута NEW
...
// @version         2.1.7.2
// @include         /^https?:\/\/.*rezk.*\/.*$/
// @icon            https://img.icons8.com/?size=100&id=123725&format=png&color=000000
// @grant           GM.info
// @grant           GM.addStyle
...
// @license         MIT
// @downloadURL https://update.greasyfork.org/scripts/558894/HDrezka%20Improvement%20%D0%B4%D0%BB%D1%8F%20%D0%BD%D0%BE%D1%83%D1%82%D0%B0%20NEW.user.js
// @updateURL https://update.greasyfork.org/scripts/558894/HDrezka%20Improvement%20%D0%B4%D0%BB%D1%8F%20%D0%BD%D0%BE%D1%83%D1%82%D0%B0%20NEW.meta.js
// ==/UserScript==
...
(function () {
    ...
    .b-wrapper {
        ...
        box-sizing: content-box;*/ /* my_edit */
    }
    .glory {
    ...
    .b-content__inline_items {
        ...
        /*width: 100%;*/ /* my_edit */
        width: auto;
        /*padding-right: 16px;*/ /* my_edit */
        box-sizing: border-box;
    }

    /* Remove extra right padding for content page */
    .b-content__columns {
        /*padding-right: 0 !important;*/ /* my_edit */
        /*padding-right: 280px;*/ /* my_edit */
        width: 75%;
        margin: 0 auto;
    }
    ...
    .b-content__inline_inner_mainprobar .b-content__inline_item {
        margin-left: 6px !important; */ /* my_edit */
    }
    ...
    function initContentSizeTumbler() {
        ...
        body.hc-content-size-mobile .b-sidelist__holder {
        overflow: auto;
        width: 105%; */ /* my_edit */
        }
        ...
        body.hc-content-size-default .b-content__columns {
        /*padding-right: 0 !important;*/ /* my_edit */
        /*padding-right: 280px;*/ /* my_edit */
        width: 75%;
        margin: 0 auto;
        }
        body.hc-content-size-default .b-sidelist__holder {
        overflow: auto;
        width: 95%;
        }
        body.hc-content-size-default .b-newest_slider {
        ...
        body.hc-content-size-wide .b-content__columns {
        /*padding-right: 0 !important;*/ /* my_edit */
        /*padding-right: 280px;*/ /* my_edit */
        width: 80%;
        margin: 0 auto;
        }
        body.hc-content-size-wide .b-sidelist__holder {
        overflow: auto;
        width: 100%;
        }
        body.hc-content-size-wide .b-sidelist__holder .b-sidelist {
        width: 115% !important;
        }
        ...
        body.hc-content-size-ultrawide .b-content__columns {
        /*padding-right: 0 !important;*/ /* my_edit */
        /*padding-right: 280px;*/ /* my_edit */
        width: 90%;
        margin: 0 auto;
        }
        body.hc-content-size-ultrawide .b-sidelist__holder {
        overflow: auto;
        width: 100%;
        }
        body.hc-content-size-ultrawide .b-sidelist__holder .b-sidelist {
        width: 100% !important;
        }
        ...
        body.hc-content-size-ultraultrawide .b-content__columns {
        /*padding-right: 0 !important;*/ /* my_edit */
        /*padding-right: 280px;*/ /* my_edit */
        width: 98%;
        margin: 0 auto;
        }
        body.hc-content-size-ultraultrawide .b-sidelist__holder .b-sidelist {
        width: 100% !important;
        }
        ...
        body.hc-content-size-full .b-content__columns {
        /*padding-right: 0 !important;*/ /* my_edit */
        /*padding-right: 280px;*/ /* my_edit */
        width: 100%;
        margin: 0 auto;
        }
        body.hc-content-size-full .b-sidelist__holder .b-sidelist {
        width: 100% !important;
        }
    ...
    function initIMDbRating() {
        ...
        /* === КиноПоиск цвета === */
        .b-content__inline_item-link > .rating .rating-value.kp.low {
            filter: invert(39%) sepia(94%) saturate(672%) hue-rotate(346deg) brightness(90%) contrast(102%);
        }
        .b-content__inline_item-link > .rating .rating-value.kp.medium {
            filter: invert(66%) sepia(85%) saturate(348%) hue-rotate(347deg) brightness(99%) contrast(91%);
        }
        .b-content__inline_item-link > .rating .rating-value.kp.high {
            filter: invert(79%) sepia(54%) saturate(3658%) hue-rotate(98deg) brightness(92%) contrast(101%);
        }
        /* !css */
        ...
        // === ДОБАВЛЕНО: КИНОПОИСК ===
        // ✅ Новая функция для получения рейтинга Кинопоиска по аналогии с IMDb
        function getKPRating(id) {
            return new Promise((resolve, reject) => {
                console.debug(`KP Rating: request quick content for id=${id}.`);
                GM.xmlHttpRequest({
                    method: "POST",
                    url: location.origin + "/engine/ajax/quick_content.php",
                    data: `id=${id}&is_touch=1`,
                    ...
                    onload(response) {
                        console.debug(`Response ${response.status} for ${response.finalUrl}`, {response});
                        const ttl = 7 * 24 * 60 * 60 * 1000; // One week TTL
                        if (response.status !== 200) {
                            console.debug(
                                `KP Rating: request quick content for id=${id} failed with ${response.status} status code.`
                            );
                            reject();
                            return;
                        }

                        // ✅ Ищем блок рейтинга Кинопоиска
                        const ratingPattern =
                              /<span class="kp">Кинопоиск: <b>(?<rating>.*)<\/b> <i>\((?<votes>.*)\)<\/i><\/span>/;
                        const ratingResult = ratingPattern.exec(response.responseText);

                        if (!ratingResult?.groups?.rating) {
                            // ❌ Рейтинг не найден — сохраняем пустое значение
                            const data = { rating: "", votes: "", id };
                            setWithExpiry(`kp_${id}`, data, ttl);
                            resolve(data);
                            console.debug(
                                `KP Rating: request quick content for id=${id} success, but no correct data found.`
                            );
                            return;
                        }
                        ...
                        setWithExpiry(`kp_${id}`, data, ttl);
                        resolve(data);
                        console.debug(`KP Rating: request quick content for id=${id} success.`);
                    },
                    onerror(e) {
                        console.debug(`KP Rating: request quick content for id=${id} failed.`);
                        console.debug("Error:", {e});
                        reject();
                    },
                });
        ...
        function showRating(ratingObject) {
            if (!ratingObject || ratingObject.id == null) return; // только проверяем id, а не рейтинг
            ...
            document
                ...
                // IMDb блок
                let imdbHTML = "";
                if (ratingObject.rating && ratingObject.rating !== "") {
                    const rating = parseFloat(ratingObject.rating).toFixed(1);
                    let votes;
                    try {
                    ...
                    let levelIMDB = "";
                    if (votes >= 1000) {
                        if (rating < 5) levelIMDB = "low";
                        else if (rating < 7) levelIMDB = "medium";
                        else levelIMDB = "high";
                    }

                    imdbHTML = `
                    <span class="rating">
                        <span class="rating-value ${levelIMDB}"><b>${rating}</b></span>
                        <span> / </span>
                ...
                // === ДОБАВЛЕНО: КИНОПОИСК ===
                const kpData = getWithExpiry(`kp_${ratingObject.id}`);
                let kpHTML = "";

                if (kpData && kpData.rating) {
                    const kpRating = parseFloat(kpData.rating);
                    const kpVotes = Number(kpData.votes.replace(/\D/g, ""));

                    let levelKP = "";
                    if (kpVotes >= 1000) {
                        if (kpRating < 5) levelKP = "low";
                        else if (kpRating < 7) levelKP = "medium";
                        else levelKP = "high";
                    }

                    kpHTML = `
                    <span class="rating" style="margin-left:4px">
                        <span class="rating-value kp ${levelKP}"
                              style="background-image:url(https://img.icons8.com/?size=100&id=87537&format=png&color=000000);
                                     background-repeat:no-repeat;
                        ...
                        <span> / </span>
                        <span class="rating-votes">${Math.floor(kpVotes / 1000)} k</span>
                    </span>`;
                }
                // === /ДОБАВЛЕНО: КИНОПОИСК ===

                // вставляем оба блока, если есть хоть один
                if (imdbHTML || kpHTML) {
                    contentItemLinkElement.innerHTML += imdbHTML + kpHTML;
                }
            });
        }
        ...
        async function getAndShowRating(contentItemElement, callback = () => {}, retries = 0) {
            ...
            if (
                ...
                // === ДОБАВЛЕНО: КИНОПОИСК ===
                // Запрашиваем рейтинг Кинопоиска параллельно с IMDb
                getKPRating(id).then((kpData) => {
                        if (kpData && kpData.rating) {
                            setWithExpiry(`kp_${id}`, kpData, 7 * 24 * 60 * 60 * 1000);
                            showRating(ratingObject);
                        }
                });
                // === /ДОБАВЛЕНО: КИНОПОИСК ===
    ...
    r4.settings = R4Settings({
        script_homepage: "https://greasyfork.org/en/scripts/558894",
        version_text: "Версия",
