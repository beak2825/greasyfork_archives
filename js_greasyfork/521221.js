...
// @icon        https://131aiclub.github.io/img/favicon.ico
// @downloadURL https://update.greasyfork.org/scripts/521221/%E4%B8%9C%E5%8D%97%E5%A4%A7%E5%AD%A6%E6%8A%A2%E8%AF%BE%E5%8A%A9%E6%89%8B131%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/521221/%E4%B8%9C%E5%8D%97%E5%A4%A7%E5%AD%A6%E6%8A%A2%E8%AF%BE%E5%8A%A9%E6%89%8B131%E7%89%88.meta.js
// ==/UserScript==
(function () {
    //版本
    let version = [1, 0, 3]
                ...
                `
                        ...
                        ev: {
                            "click": () => {
                                ...
                                checkbox.checked = settings.auto;
                            }
                        }
                    }),
                    self.createNode({
        ...
        self.createPopUp = (title, node, width, height) => app.appendChild(self.createNode({
            ...
            children: [
                ...
                self.createNode({
                    tagName: "button",
                    obj: {
                        "class": "el-button--default el-button--large is-round",
                        "style": `
                    margin: 20px;
                    position: absolute;
                    right:10%;
                ...
                `
                    ...
                    ev: {
                        "click": () => {
                            methods.saveCourse(); // 保存设置
                            if (settings.auto) {
                                methods.startAutoEnroll(); // Start auto enroll
                            } else {
                                methods.stopAutoEnroll(); // Stop auto enroll
                            }
                            ...
                            children: [
                                ...
                                self.createNode({
                                    ...
                                    obj: {
                                        "id": "interval",
                                        "type": "number",
                                        "value": settings.interval || 3000, // 默认间隔 3000 毫秒（3秒）
                                        "min": 1000, // 最小间隔 1000 毫秒
                                        "style": "width: 60%; margin-left: 10px;"
                                    },
                                    ev: {
                                        "change": (e) => {
                                            let interval = parseInt(e.target.value, 10);
                                            if (interval >= 100) { // 确保间隔不小于 100 毫秒
                                                settings.interval = interval;
                                                methods.saveCourse();
                                                if (settings.auto) {
                                                    methods.startAutoEnroll(); // 重新启动自动抢课，应用新的间隔
                                                }
                                            }
                                        }
                                    }
    ...
    let methods = {
        ...
        // Method to start automatic enrollment
        startAutoEnroll() {
            // Ensure no multiple intervals are running at once
            if (enrollInterval) {
                clearInterval(enrollInterval);
            }

            // Trigger enroll every 3 seconds (3000 ms)
            enrollInterval = setInterval(() => {
                methods.enroll();
            }, settings.interval);
        },

        // Method to stop automatic enrollment
        stopAutoEnroll() {
            if (enrollInterval) {
                clearInterval(enrollInterval);
                enrollInterval = null;
