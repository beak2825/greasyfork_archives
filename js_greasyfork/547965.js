...
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/547965/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29.user.js
// @updateURL https://update.greasyfork.org/scripts/547965/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29.meta.js
// ==/UserScript==

/*
  Changes (5.8.5):
  - FLAM prompt (Ask each time): now fully mobile-safe.
    * Overlay uses a SAFE_INSET and sizes panel with calc(100vw/100vh - 2*SAFE_INSET).
    * Prevents right/left (and top/bottom) cut-offs; keeps borders equidistant from screen edges.
    * Keeps previous visual design: buttons default GREY; hover/touch/click goes WHITE; thin white panel border.
*/
...
(async function () {
  ...
  // ────────────────────────────────────────────────────────────────────────
  // Action chooser — FLAM-styled (MOBILE-SAFE)
  // ────────────────────────────────────────────────────────────────────────
  function askForActionOverlay() {
    return new Promise((resolve) => {
      const overlay = document.createElement('div');
      overlay.id = 'flam-action-overlay';
      const SAFE_INSET = 16; // px on each side (keeps equal breathing room)
      ...
      Object.assign(panel.style, {
        position: 'relative',
        width: `min(760px, calc(100vw - ${SAFE_INSET * 2}px))`,   // never wider than the padded viewport
        maxWidth: `calc(100vw - ${SAFE_INSET * 2}px)`,
        maxHeight: `calc(100vh - ${SAFE_INSET * 2}px)`,           // never taller than the padded viewport
        overflow: 'auto',                                         // scroll if content taller on small screens
  ...
  async function handleShowUrl(btnEl) {
    ...
    panel.innerHTML = `
      ...
      <textarea id="kodishowurl_text" style="width:600px; max-width:95vw; height:200px; font-size:14px; box-sizing:border-box; white-space:pre-wrap;">${info.url}</textarea>
