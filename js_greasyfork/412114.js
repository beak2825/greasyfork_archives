...
// @namespace    https://greasyfork.org/en/scripts/412114-hospital-filter-modified
// @version      0.3
// @description  Enables filters to remove/hide people from the hospital. modification: adds self hosped and mugged filters
// @author       Cryosis7 [926640], modified by Xeno2
// @match        *.torn.com/hospitalview.php*
// @grant        GM_addStyle
// @grant        GM_setValue
// @grant        GM_getValue
// @downloadURL https://update.greasyfork.org/scripts/412114/Hospital%20Filter%20%5Bmodified%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/412114/Hospital%20Filter%20%5Bmodified%5D.meta.js
// ==/UserScript==
...
function filterMember() {
	...
	if (filters.chk.time && compareTime(filters.time, player.time))
		show = false;
	...
	if (filters.chk.selfHosped && !selfHospedStatus.includes(player.status))
		show = false;
	if (filters.chk.mugged && !player.status.includes("Mugged by"))
		show = false;
	if (show)
		$(this).show();
	else
...
function addFilterElements(element) {
	...
	$(element).children(".cont-gray").append(`
		  <span style="padding-right: 15px">
			<label>Mugged:
			<input type="checkbox" name="mugged" style="transform:translateY(25%)"/>
			</label>
		  </span>
		  `);
...
function setInitialValue() {
	for (let filter in filters) {
		if (filter == 'chk') {
			...
			$(".filter-container").find(`[name="mugged"]`).prop('checked', filters.chk.mugged);
			continue;
		}
