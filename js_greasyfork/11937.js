// ==UserScript==
// @name           Amazon Tag Remove https (All)
// @namespace      Tag remove for links of Amazon
// @description    Elimina la variable tag de los vínculos hacia Amazon
// @include        *
// @author         Basado en WeSo
// @version        1
// @run-at         document-end

// @downloadURL https://update.greasyfork.org/scripts/11937/Amazon%20Tag%20Remove%20https%20%28All%29.user.js
// @updateURL https://update.greasyfork.org/scripts/11937/Amazon%20Tag%20Remove%20https%20%28All%29.meta.js
// ==/UserScript==
...
function clearText (text,site) {
  ...
  if (result = exp.exec(tmp))
    tmp = "https://www."+site+"/dp/B"+result[2]+"/"+result[1]+"/";
  ...
  exp = new RegExp(pattern, "gi");
  if (result = exp.exec(tmp))
    tmp = "https://www."+site+"/gp/product/"+result[1]+"/";
  ...
  pattern = ".*http://[w]{0,3}.?"+site+"/(.*)";
  exp = new RegExp(pattern, "gi");
  if (result = exp.exec(tmp))
    tmp = "https://www."+site+"/"+result[1];
