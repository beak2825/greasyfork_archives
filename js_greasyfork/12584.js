...
// -- NSFW END
// @downloadURL https://update.greasyfork.org/scripts/12584/Manga%20Loader.user.js
// @updateURL https://update.greasyfork.org/scripts/12584/Manga%20Loader.meta.js
// ==/UserScript==
...
var getViewer = function(prevChapter, nextChapter) {
  ...
  // add main styles
  addStyle('main', true, viewerCss, imagesCss, imageCss, counterCss, navCss, navHoverCss, statsCss, statsCollapseCss, statsHoverCss, boxCss, floatingMsgCss, buttonCss, keySettingCss);
  // add user styles
  ...
  UI.btnSettings.addEventListener('click', function() {
    if(UI.floatingMsg.textContent) {
      ...
      // Keybindings
      settings += 'Keybindings:<br><table class="ml-setting-key">' +
        '<tr><td>Previous Chapter</td><td><input data-ignore="true" data-key="PREV_CHAP" type="text" value="' + (storeGet('ml-setting-key-prev-chap') || UI.keys.PREV_CHAP) + '"/></td></tr>' +
        '<tr><td>Exit</td><td><input data-ignore="true" data-key="EXIT" type="text" value="' + (storeGet('ml-setting-key-exit') || UI.keys.EXIT) + '"/></td></tr>' +
        '<tr><td>Next Chapter</td><td><input data-ignore="true" data-key="NEXT_CHAP" type="text" value="' + (storeGet('ml-setting-key-next-chap') || UI.keys.NEXT_CHAP) + '" /></td></tr>' +
        '</table><br>';
      // Save button
  ...
  // keybindings
  UI.keys = storeGet('ml-setting-key') || {
  ...
  var ex = extractInfo.bind(imp),
      ...
      loadNextPage = function(url) {
        ...
        if (imp.pages) {
          ...
          if(location.protocol === 'https:' && url.indexOf('https:') !== 0) url = 'https' + url.slice(url.indexOf(':'));
          xhr.open('get', url);
