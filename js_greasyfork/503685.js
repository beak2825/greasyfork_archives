// ==UserScript==
// @name         Технический специалист Novosibirsk 34
// @namespace    https://forum.blackrussia.online
// @version      1.1.7
// @description  Для люпими Novosibirsk 34
...
// @downloadURL https://update.greasyfork.org/scripts/503685/%D0%A2%D0%B5%D1%85%D0%BD%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B9%20%D1%81%D0%BF%D0%B5%D1%86%D0%B8%D0%B0%D0%BB%D0%B8%D1%81%D1%82%20Novosibirsk%2034.user.js
// @updateURL https://update.greasyfork.org/scripts/503685/%D0%A2%D0%B5%D1%85%D0%BD%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B9%20%D1%81%D0%BF%D0%B5%D1%86%D0%B8%D0%B0%D0%BB%D0%B8%D1%81%D1%82%20Novosibirsk%2034.meta.js
// ==/UserScript==
    ...
    const PIN_PREFIXX = 11; //  префикс передачи админам
    const PIN_PREFIXXX = 3; //  префикс передачи в обжалования
    const PIN_PREFIXXXXX = 8; //  префикс передачи в жб на игроков
    const PIN_PREFIXXXX = 5; //  префикс передачи в тех раздел
    const PIN_PREFIXXXXXX = 1 ; //  префикс передачи в жб на тех
...
},
{
	...
	status: false,
        ...
        {
	title: 'В обжалование наказаний 34',
	color: '',
	content:
	'[SIZE=4][FONT=Verdana][CENTER]Доброго времени суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	"[CENTER]Вы ошиблись разделом. Переношу ваше обращение в соответствующий для этого раздел.[/CENTER]<br>",
	...
	status: false,
       ...
       {
	title: 'В тех раздел 34',
	color: '',
	content:
	'[SIZE=4][FONT=Verdana][CENTER]Доброго времени суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	"[CENTER]Вы ошиблись разделом. Переношу ваше обращение в соответствующий для этого раздел.[/CENTER]<br>",
	prefix: PIN_PREFIXXXX,
	status: false,
},
{
	title: 'В жалобы на тех 34',
	color: '',
	content:
	'[SIZE=4][FONT=Verdana][CENTER]Доброго времени суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	"[CENTER]Вы ошиблись разделом. Переношу ваше обращение в соответствующий для этого раздел.[/CENTER]<br>",
	prefix: PIN_PREFIXXXXXX,
	status: false,
},
	{
	title: 'В жб на игроков 34 ',
	color: '',
	content:
	'[SIZE=4][FONT=Verdana][CENTER]Доброго времени суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	"[CENTER]Вы ошиблись разделом. Переношу ваше обращение в соответствующий для этого раздел.[/CENTER]<br>",
	prefix: PIN_PREFIXXXXX,
	status: false,
},
];
	...
	addButton('Закрытие без перемещения', 'closed_complaint');
  addButton ('Убрать префикс', 'without_a_prefix');
    ...
    $('button#pin').click(() => editThreadData(PIN_PREFIXX, false));
    $('button#pin').click(() => editThreadData(PIN_PREFIXXX, false));
    $('button#pin').click(() => editThreadData(PIN_PREFIXXXX, false));
    $('button#pin').click(() => editThreadData(PIN_PREFIXXXXXX, false));
    $('button#pin').click(() => editThreadData(PIN_PREFIXXXXX, false));
...
const Button54 = buttonConfig("Игр 34", "https://forum.blackrussia.online/forums/Жалобы-на-игроков.1572/");
const ButtonTech54 = buttonConfig("ЖБ 34", "https://forum.blackrussia.online/forums/Сервер-№34-novosibirsk.1543/")
const ButtonComp54 = buttonConfig("ТР 34", "https://forum.blackrussia.online/forums/Технический-раздел-novosibirsk.1544/")
