...
// @version      0.1
// @description  Life is hard enough. Don't download tests for hours
// @author       You
// @match        https://yayin.eduasist.com/fernus/index
// @icon         https://www.google.com/s2/favicons?sz=64&domain=eduasist.com
// @grant        none
// @license      GNU GPLv3
// @downloadURL https://update.greasyfork.org/scripts/479346/Fe.user.js
// @updateURL https://update.greasyfork.org/scripts/479346/Fe.meta.js
// ==/UserScript==
...
var select = document.getElementById("kaynakContainer");
select.addEventListener("change", (event) => {
 instantKonu();
});
//*[@id="kaynakContainer"]/select

let downButton = document.createElement("BUTTON");
    downButton.innerHTML = "Herşey!";
...
let liteButton = document.createElement("BUTTON");
    liteButton.innerHTML = "Sadece İndir!";
    stickTo.appendChild(liteButton);
...
let konuButton = document.createElement("BUTTON");
    konuButton.innerHTML = "Konu Seç!";
    stickTo.appendChild(konuButton);

    konuButton.onclick = () => {
 instantKonu();
};
//const konuSelectionTimeout = setTimeout(sinifSelection, 1000);

//const bransSelectionTimeout = setTimeout(bransSelection, 1000);

 //setTimeout(konuSelection, 1000);
...
function sinifSelection(){
  ...
  if(sinif.includes("TYT") || sinif.includes("AYT")) sinif = "12. Sınıf";
    else sinif = sinif.substring(0, sinif.indexOf(".")+7);
...
function konuSelection(){
  setTimeout(function() {
    ...
    konuSec.value = optionFinder(konuSec,done);
      ...
      var chosen = optionFinder(konuSec,done);
      if(chosen){
        konuSec.value = optionFinder(konuSec,done);
      }else{
        var newAttempt = document.getElementById("isimler");
