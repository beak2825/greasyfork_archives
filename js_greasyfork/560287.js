// ==UserScript==
// @name         点击提示
// @namespace    http://tampermonkey.net/
// @version      1.2
// @icon         data:image/png;base64,
// @description  点击指定文本元素后将其修改为"已点击"（目前仅支持专利电子系统缴费板块）
// @author       You
// @match        *://interactive.cponline.cnipa.gov.cn/public-app-zljffw/*
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/560287/%E7%82%B9%E5%87%BB%E6%8F%90%E7%A4%BA.user.js
// @updateURL https://update.greasyfork.org/scripts/560287/%E7%82%B9%E5%87%BB%E6%8F%90%E7%A4%BA.meta.js
// ==/UserScript==

(function() {
    'use strict';

    // 自定义需要监听的文本内容配置
    // 在这里添加你想要监听的文本内容
    const TARGET_TEXTS = [
        '详细信息',
        '电子票据',
        '下载订单',
        // 可以在这里添加更多需要监听的文本内容
        // '其他按钮文本',
        // '更多选项',
    ];

    // 创建一个 MutationObserver 来监听 DOM 变化
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            mutation.addedNodes.forEach(function(node) {
                if (node.nodeType === 1) { // 元素节点
                    attachClickListener(node);
                    // 检查新添加节点的子节点
                    const allChildren = node.querySelectorAll('button, a, span, div');
                    allChildren.forEach(attachClickListener);
                }
            });
        });
    });

    // 配置观察选项
    const config = {
        childList: true,
        subtree: true
    };

    // 开始观察
    observer.observe(document.body, config);

    // 检查文本是否在目标列表中
    function isTargetText(text) {
        if (!text) return false;
        const trimmedText = text.trim();
        return TARGET_TEXTS.some(target =>
            trimmedText.includes(target) || target.includes(trimmedText)
        );
    }

    // 为元素添加点击监听器
    function attachClickListener(element) {
        // 检查元素是否包含目标文本
        if (element.tagName === 'BUTTON' || element.tagName === 'A' ||
            element.classList.contains('el-button')) {

            // 检查元素本身是否包含目标文本
            const elementText = element.textContent.trim();
            if (isTargetText(elementText)) {
                if (!element.hasAttribute('data-click-listener')) {
                    element.addEventListener('click', handleClick, true); // 使用捕获阶段
                    element.setAttribute('data-click-listener', 'true');
                }
            }

            // 检查元素内的子元素是否包含目标文本
            const childElements = element.querySelectorAll('span, div');
            childElements.forEach(child => {
                const childText = child.textContent.trim();
                if (isTargetText(childText) && !child.hasAttribute('data-click-listener')) {
                    child.addEventListener('click', handleClick, true); // 使用捕获阶段
                    child.setAttribute('data-click-listener', 'true');
                }
            });
        }
    }

    // 点击处理函数
    function handleClick(event) {
        const target = event.target;
        let textElement = null;
        let originalEvent = null;

        // 查找包含目标文本的元素
        if (target.tagName === 'BUTTON' || target.tagName === 'A') {
            const elementText = target.textContent.trim();
            if (isTargetText(elementText)) {
                textElement = target.querySelector('span') || target.querySelector('div') || target;
                originalEvent = event;
            }
        } else if (target.tagName === 'SPAN' || target.tagName === 'DIV') {
            const elementText = target.textContent.trim();
            if (isTargetText(elementText)) {
                textElement = target;
                originalEvent = event;
            } else {
                // 向上查找父级按钮或链接
                const parentBtn = target.closest('button, a');
                if (parentBtn) {
                    const parentText = parentBtn.textContent.trim();
                    if (isTargetText(parentText)) {
                        textElement = target;
                        originalEvent = event;
                    }
                }
            }
        } else {
            // 向上查找最近的按钮或链接
            const parentBtn = target.closest('button, a');
            if (parentBtn) {
                const parentText = parentBtn.textContent.trim();
                if (isTargetText(parentText)) {
                    textElement = parentBtn.querySelector('span') || parentBtn.querySelector('div') || parentBtn;
                    originalEvent = event;
                }
            }
        }

        // 如果找到了匹配的文本元素，则修改文本
        if (textElement && originalEvent) {
            // 保存原始文本内容
            if (!textElement.hasAttribute('data-original-text')) {
                textElement.setAttribute('data-original-text', textElement.textContent.trim());

                // 修改文本为"已点击"
                textElement.textContent = '已点击';

                // 添加样式表示已点击
                textElement.style.color = '#909399';
                textElement.style.textDecoration = 'line-through';

                // 在一小段时间后移除事件监听，避免重复执行
                setTimeout(() => {
                    // 不移除事件监听器，而是标记元素已被处理
                    textElement.setAttribute('data-text-modified', 'true');
                }, 100);
            }

            // 重要：不阻止事件冒泡，让原有功能正常执行
            // event.stopPropagation(); // 移除了这行
        }
    }

    // 页面加载完成后为现有元素添加监听器
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() {
            const allElements = document.querySelectorAll('button, a, span, div');
            allElements.forEach(attachClickListener);
        });
    } else {
        const allElements = document.querySelectorAll('button, a, span, div');
        allElements.forEach(attachClickListener);
    }

    console.log('自定义文本元素点击修改脚本（修复版）已加载，监听文本：', TARGET_TEXTS);
})();



