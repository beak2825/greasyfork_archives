...
// @downloadURL https://update.greasyfork.org/scripts/387644/idle.user.js
// @updateURL https://update.greasyfork.org/scripts/387644/idle.meta.js
// ==/UserScript==
...
const idMatch = location.href.match(/\d+$/);
const configId = 'idle-ui-config-' + (idMatch ? idMatch[0] : 'default');
let localConfig = localStorage.getItem(configId);
if (localConfig) {
...
function idleInit() {
    ...
    const input = `<div id="idle-ui-quicksearch">${itemTypes}<input placeholder="搜索..." class="form-control"/><div class="popover" style="display: none; right: 60px; top: 28px;"><div class="popover-content"></div></div></div>`;

    // 将 input 内容插入到 <a class="navbar-brand"> 标签的后面
    navbarBrand.insertAdjacentHTML('afterend', input);
    ...
    if (location.href.indexOf('Character/Detail') || location.href.indexOf('Character/BmDetail') >= 0) {
    ...
    function saveStats(statsData) {
        const id = idMatch[0];
        let stats = localStorage.getItem('idle-ui-stats' + idMatch[0]);
        ...
        statsData.time = time;
        stats[id] = statsData;
        localStorage.setItem('idle-ui-stats' + idMatch[0], JSON.stringify(stats));
    }
    ...
    function displayStats(id, timeSpan, uniqueChange, setChange) {
        ...
        $('.navbar.navbar-inverse.navbar-fixed-top').next().prepend(message);
    ...
    function addConfig() {
        ...
        const configHtml = renderConfigHtml();
        const html = `
              ...
              <div class="modal-dialog modal-large" role="">
                  <div class="modal-content model-inverse">
                      ...
                      <div class="modal-body">
                        ...
                        <div class="panel-header state">配置项开关（配置具体含义请参考<a href="https://greasyfork.org/zh-CN/scripts/387644-idle" target="_blank">脚本介绍</a>，点击即可启用/禁用，变更后请刷新）</div>
    ...
    if (config.mapHack && location.href.indexOf('Map/Dungeon') >= 0) {
        ...
        function mapMove() {
            ...
            if (nextBlockIndex === null && (moveBlocks.length > 0)) {
                const randomIndex = Math.floor(Math.random() * moveBlocks.length);  // 生成一个随机索引
                nextBlockIndex = moveBlocks[randomIndex];  // 选择随机的元素
            }
            ...
            console.log("下一个点击区域" + nextBlockIndex);
        ...
        function clickBlock(block) {
            ...
            const lastTime = localStorage.getItem('idle-ui-last-time');
            if (lastTime) {
                if (seconds - lastTime < 500) {
                    setTimeout(() => {
                        clickBlock(block);
                    }, 300);
                    return;
            ...
            localStorage.setItem('idle-ui-last-time', seconds);
            ...
            if (f.hasClass("monster")) {
                ...
                $.ajax({
                    ...
                    success: function (a) {
                        ...
                        setTimeout(() => {
                            mapMove();
                        }, Math.round(config.userNumber * 600));
                    },
    ...
    if (config.mapHack && (location.href.indexOf('Map/Detail') >= 0 || location.href.indexOf('BmDetail') >= 0)) {
        const btn = renderButton('idle-ui-maplog', '自动秘境日志');
        if (location.href.indexOf('Map/Detail') >= 0) {
            $('.btn.btn-xs').eq(1).before(btn);
        } else if (location.href.indexOf('Character/BmDetail') >= 0) {
            $('.btn.btn-xs.btn-success').eq(2).before(btn);
        }
    ...
    function inBattlePage() {
        const battePages = ['Battle/Simulate', 'Battle/InDungeon', 'Battle/WithChar', 'Battle/OnMap'];
