// ==UserScript==
// @name        MouseHunt AutoBot Enhanced Edition UPDATED 2025
// @author      NobodyRandom
// @author      tigerting98
// @version    	2.3.9
// @description Script automating MH horn sounding and areas. Fixed nevocaine's version. Original scripts from CnN and Ooi. Includes updates from nevocaine and NobodyRandom. Fixes by tigerting98 in 2025.
...
// @grant		GM_info
// @run-at		document-end
// @downloadURL https://update.greasyfork.org/scripts/550728/MouseHunt%20AutoBot%20Enhanced%20Edition%20UPDATED%202025.user.js
// @updateURL https://update.greasyfork.org/scripts/550728/MouseHunt%20AutoBot%20Enhanced%20Edition%20UPDATED%202025.meta.js
// ==/UserScript==
...
function check() {
    ...
    if (true) {
        ...
        if (user.environment_name == "Cantera Quarry"){
            ...
            for (const check_bait of qg_bait){f
                console.log("checking", check_bait);
                if (findCommonElements(magic_dict_cq[check_bait], remaining_mice)){
                    checkThenArm(null, 'bait', check_bait);
                    console.log(check_bait, "present")
...
function receiveMessage(event) {
    ...
    if (debug) console.debug("Event data: ", event.data);
    if (!debugKR && !isAutoSolve)
        return;

    if (event && (event.origin.indexOf("mhcdn") > -1 || event.origin.indexOf("mousehuntgame") > -1 || event.origin.indexOf("dropbox") > -1) && typeof event.data === "string") {
        try {
...
var getMapPort;
try {
    if (chrome.runtime && !isNullOrUndefined(chrome.runtime.id)) {
        ...
        if (!objFW.retreating && objFW.commanderClearStreak && objFW.streak > 2 && bCurrentStreakZeroPopulation && !bWrongSoldierTypeStreak) {
            ...
            if (index == objPopulation.ARTILLERY && charmArmed.indexOf('Warpath') > -1)
                ...
                if (objFW.charmType[objFW.streak].indexOf('None') > -1)
                    ...
                    charmName = ['Super Warpath ' + objPopulation.name[index], 'Warpath ' + objPopulation.name[index]];
                else
                    charmName = 'Warpath ' + objPopulation.name[index];
            }
    ...
    if (objFW.disarmAfterSupportRetreat && sumData(objFW.population.all) <= g_arrFWSupportRetreat[wave]) {
        ...
        if (!isNullOrUndefined(charmName)) {
            if (typeof charmName === 'string' || charmName instanceof String)
                ...
                // charmName === array
                charmName.map(function(x){return x.replace(/Warpath Warpath/g, 'Warpath');});
            }
...
function getKingRewardStatus() {
    ...
    if (debug) console.debug('in getKingRewardStatus() - user.has_puzzle:', strValue, ", type of user.has_puzzle value", typeof strValue);
...
function doubleCheckLocation() { //return true if location is camp page (this is to combat ajax loads)
    ...
    if (thePage) {
        return (( thePage.dataset.page && thePage.dataset.page.indexOf("PageCamp") > -1) || thePage.className.indexOf("PageCamp") > -1);
    } else {
        return false;
    }
}
        ...
        if (!aggressiveMode) {
            ...
            if (headerElement) {
                ...
                if (isvisible(headerStatus)) {
                    ...
                    if (debug) console.log("soundHorn: Horn ready state not visible. Synchronizing data to prevent loop.");

                    // update timer
                    displayTimer("Synchronizing Data...", "Hunter horn is missing. Synchronizing data...", "Hunter horn is missing. Synchronizing data...");

                    // Instead of trying to sound the horn and checking again,
                    // which can cause a loop, let's retrieve fresh data.
                    retrieveData();
                    ...
                    // The countdown will be restarted by retrieveData -> action -> countdownTimer
                    // This breaks the potential loop.
                    window.setTimeout(function () {
                        countdownTimer();
                    }, timerRefreshInterval * 1000);
                }
...
function afterSoundingHorn() {
    ...
    var headerElement = document.querySelector('.huntersHornView');
    if (headerElement) {
        console.log ("checking KR status in afterSoundingHorn()", getKingRewardStatus(), "header check", checkHeaderKingsReward());
        if (getKingRewardStatus() || checkHeaderKingsReward()) {
            isKingReward = true;
            console.log("is King Reward, running kingRewardAction");
            kingRewardAction();
    ...
    try {
        ...
        if (browser == 'chrome') {
            ...
            if (name == "user.next_activeturn_seconds") {
                ...
                console.log("type of user.has_puzzle", typeof unsafeWindow.user.has_puzzle);
