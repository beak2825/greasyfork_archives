...
(function() {
    ...
    // --- 1. 状态提示器 (3秒自动清理) ---
    const addStatus = (msg, color = "#3498db", duration = 3000) => {
        let container = document.getElementById('debug-container');
        if (!container) {
            container = document.createElement('div');
            container.id = 'debug-container';
            container.style = "position:fixed;top:0;left:0;width:100%;z-index:99999;pointer-events:none;display:flex;flex-direction:column;";
            document.documentElement.appendChild(container);
        }
        
        const line = document.createElement('div');
        line.style = `padding:8px; color:white; text-align:center; font-weight:bold; font-size:14px; margin-bottom:1px; transition:opacity 0.5s; background-color:${color};`;
        line.innerText = msg;
        
        container.appendChild(line);

        // 统一设置：所有提示 duration 毫秒后消失
        setTimeout(() => {
            line.style.opacity = '0';
            setTimeout(() => line.remove(), 500);
        }, duration);
        
        return line; // 返回引用以便手动控制
    };

    // --- 2. 注入环境 ---
    window.nw = {}; 
    console.log("NW 环境已预注入");
    addStatus("🔍 [监控] 脚本已加载，NW 环境预注入成功");
    ...
    const observer = new MutationObserver(() => {
        ...
        if (bodyText.includes("请使用国家传染病")) {
            isReloading = true;
            
            addStatus("⚠️ [检测] 判定失败：检测到拦截文字", "#e67e22");

            // 倒计时逻辑
            let count = 3;
            const countdownLine = addStatus(`⏳ [倒计时] 强制刷新还剩 ${count} 秒...`, "#d35400", 3500);
            
            const timer = setInterval(() => {
                count--;
                if (count > 0) {
                    countdownLine.innerText = `⏳ [倒计时] 强制刷新还剩 ${count} 秒...`;
                } else {
                    clearInterval(timer);
                }
            }, 1000);
            ...
            // 维度 C 方案：3秒后执行硬核刷新
            setTimeout(async () => {
                addStatus("🚀 [执行] 启动维度 C：Fetch 强制回源...", "#c0392b");
                try {
                    ...
                    addStatus("❌ 刷新异常，尝试普通 reload", "#7f8c8d");
    ...
    setTimeout(() => {
        if (!isReloading) {
            addStatus("✅ [结束] 页面加载平稳，未发现拦截", "#27ae60");
            observer.disconnect();
        }
    }, 10000);
