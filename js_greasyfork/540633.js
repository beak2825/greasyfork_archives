...
// @downloadURL https://update.greasyfork.org/scripts/540633/Lyra%20Exporter%20Fetch%20dev.user.js
// @updateURL https://update.greasyfork.org/scripts/540633/Lyra%20Exporter%20Fetch%20dev.meta.js
// ==/UserScript==
    ...
    // Helper function to fetch images via GM_xmlhttpRequest (bypass CORS)
    function fetchViaGM(url, headers = {}) {
        console.log('[Grok] fetchViaGM: Entry, URL =', url);
        console.log('[Grok] fetchViaGM: GM_xmlhttpRequest available?', typeof GM_xmlhttpRequest !== 'undefined');

        return new Promise((resolve, reject) => {
            if (typeof GM_xmlhttpRequest === 'undefined') {
                console.log('[Grok] fetchViaGM: Using fallback fetch()');
                fetch(url, { headers }).then(r => {
                    console.log('[Grok] fetchViaGM: Fetch response status =', r.status);
                    if (r.ok) return r.blob();
                    return Promise.reject(new Error(`Status: ${r.status}`));
                }).then(blob => {
                    console.log('[Grok] fetchViaGM: Fetch successful, blob size =', blob.size);
                    resolve(blob);
                }).catch(err => {
                    console.error('[Grok] fetchViaGM: Fetch failed:', err);
                    reject(err);
                });
                return;
            }

            console.log('[Grok] fetchViaGM: Using GM_xmlhttpRequest');
            GM_xmlhttpRequest({
                method: "GET",
                url,
                headers,
                responseType: "blob",
                onload: r => {
                    console.log('[Grok] fetchViaGM: GM onload, status =', r.status);
                    if (r.status >= 200 && r.status < 300) {
                        console.log('[Grok] fetchViaGM: GM success, blob size =', r.response?.size);
                        resolve(r.response);
                    } else {
                        console.error('[Grok] fetchViaGM: GM bad status:', r.status);
                        reject(new Error(`Status: ${r.status}`));
                    }
                },
                onerror: e => {
                    console.error('[Grok] fetchViaGM: GM onerror:', e);
                    reject(new Error(e.statusText || 'Network error'));
                },
                ontimeout: () => {
                    console.error('[Grok] fetchViaGM: GM timeout');
    ...
    async function processImageElement(imgElement) {
        console.log('[Grok] processImageElement: Entry');

        if (!imgElement) {
            console.log('[Grok] processImageElement: imgElement is null/undefined');
            return null;
        }

        const url = imgElement.src;
        console.log('[Grok] processImageElement: URL =', url);

        if (!url || url.startsWith('data:')) {
            console.log('[Grok] processImageElement: URL is empty or data URI, skipping');
            return null;
        }
        ...
        try {
            ...
            if (url.startsWith('blob:')) {
                console.log('[Grok] processImageElement: Processing blob URL');
                try {
                    const blob = await fetch(url).then(r => r.ok ? r.blob() : Promise.reject());
                    base64Data = await Utils.blobToBase64(blob);
                    mimeType = blob.type;
                    size = blob.size;
                    console.log('[Grok] processImageElement: Blob processed successfully');
                } catch (blobError) {
                    console.log('[Grok] processImageElement: Blob fetch failed, trying canvas fallback');
                    // Canvas fallback
                    const canvas = document.createElement('canvas');
                    canvas.width = imgElement.naturalWidth || imgElement.width;
                    canvas.height = imgElement.naturalHeight || imgElement.height;
                    ...
                    base64Data = dataURL.split(',')[1];
                    size = Math.round((base64Data.length * 3) / 4);
                    console.log('[Grok] processImageElement: Canvas fallback successful');
                }
            } else {
                console.log('[Grok] processImageElement: Processing HTTP URL');

                // Try Canvas method first (more reliable for already-loaded images)
                console.log('[Grok] processImageElement: Trying Canvas method');
                console.log('[Grok] processImageElement: Image crossOrigin =', imgElement.crossOrigin);
                console.log('[Grok] processImageElement: Image complete =', imgElement.complete);
                ...
                try {
                    ...
                    console.log('[Grok] processImageElement: Canvas size =', canvas.width, 'x', canvas.height);

                    if (canvas.width === 0 || canvas.height === 0) {
                        throw new Error('Image not loaded or has zero dimensions');
                    }

                    console.log('[Grok] processImageElement: Drawing image to canvas...');
                    const ctx = canvas.getContext('2d');
                    ctx.drawImage(imgElement, 0, 0);
                    console.log('[Grok] processImageElement: Image drawn successfully');

                    const isPhoto = canvas.width * canvas.height > 50000;
                    console.log('[Grok] processImageElement: Converting to data URL (isPhoto =', isPhoto, ')...');
                    const dataURL = isPhoto ? canvas.toDataURL('image/jpeg', 0.85) : canvas.toDataURL('image/png');
                    console.log('[Grok] processImageElement: toDataURL successful');

                    mimeType = isPhoto ? 'image/jpeg' : 'image/png';
                    base64Data = dataURL.split(',')[1];
                    size = Math.round((base64Data.length * 3) / 4);

                    console.log('[Grok] processImageElement: Canvas method successful, base64 length =', base64Data.length);
                } catch (canvasError) {
                    console.error('[Grok] processImageElement: Canvas method failed:', canvasError);
                    console.error('[Grok] processImageElement: Error name:', canvasError.name);
                    console.error('[Grok] processImageElement: Error message:', canvasError.message);
                    console.log('[Grok] processImageElement: Falling back to GM_xmlhttpRequest');

                    // Fallback to GM_xmlhttpRequest
                    let blob;
                    try {
                        blob = await fetchViaGM(url);
                        console.log('[Grok] processImageElement: Blob fetched, size =', blob.size);
                    } catch (fetchError) {
                        console.error('[Grok] processImageElement: fetchViaGM also failed:', fetchError);
                        throw fetchError;
                    }

                    base64Data = await Utils.blobToBase64(blob);
                    console.log('[Grok] processImageElement: Base64 conversion complete, length =', base64Data.length);
                ...
                // Fix MIME type if it's octet-stream or empty
                if (!mimeType || mimeType === 'application/octet-stream' || !mimeType.startsWith('image/')) {
                    console.log('[Grok] processImageElement: Fixing MIME type from', mimeType);
                    if (url.includes('.jpg') || url.includes('.jpeg')) {
                        mimeType = 'image/jpeg';
                    } else if (url.includes('.png')) {
                        mimeType = 'image/png';
                    ...
                    console.log('[Grok] processImageElement: MIME type fixed to', mimeType);
                }
            }

            console.log('[Grok] processImageElement: Success! Returning data');
            return { type: 'image', format: mimeType, size, data: base64Data, original_src: url };
        } catch (e) {
            console.error('[Grok] processImageElement: Exception caught:', e);
            console.error('[Grok] processImageElement: Stack trace:', e.stack);
            return null;
        }
    }
    ...
    const GrokHandler = {
        ...
        getConversation: async (conversationId) => {
            try {
                ...
                if (State.includeImages) {
                    console.log('[Grok] Capturing images from DOM...');

                    // Create a map to track which images we've already processed (by URL)
                    ...
                    console.log(`[Grok] Found ${messageContainers.length} response containers`);
                    ...
                    for (const container of messageContainers) {
                        ...
                        console.log(`[Grok] Response ${responseId}: Found ${generatedImgs.length} AI-generated images`);

                        for (const img of generatedImgs) {
                            console.log(`[Grok] Processing AI image: ${img.src.substring(0, 80)}...`);
                            ...
                            if (parentStyle && parentStyle.filter && parentStyle.filter.includes('blur')) {
                                console.log('[Grok] Skipped: blurred background image');
                                continue;
                            }

                            // Skip if already processed
                            if (processedUrls.has(img.src)) {
                                console.log('[Grok] Skipped: duplicate image');
                                continue;
                            }
                            processedUrls.add(img.src);

                            try {
                                console.log('[Grok] Calling processImageElement...');
                                const imageData = await processImageElement(img);
                                console.log('[Grok] processImageElement returned:', imageData ? 'success' : 'null');
                                ...
                                if (imageData) {
                                    ...
                                    console.log('[Grok] Added to capturedImages array');
                                }
                            } catch (e) {
                                console.error('[Grok] Exception in AI image processing:', e);
                            }
                        }
                        ...
                        if (capturedImages.length > 0) {
                            ...
                            console.log(`[Grok] Captured ${capturedImages.length} AI images for response ${responseId}`);
                        }
                    }
                    ...
                    console.log(`[Grok] Found ${allUserImages.length} user-uploaded images`);

                    for (const img of allUserImages) {
                        console.log(`[Grok] Processing user image: ${img.src.substring(0, 80)}...`);

                        if (processedUrls.has(img.src)) {
                            console.log('[Grok] Skipped: duplicate user image');
                            continue;
                        }
                        processedUrls.add(img.src);

                        try {
                            console.log('[Grok] Calling processImageElement for user image...');
                            const imageData = await processImageElement(img);
                            console.log('[Grok] processImageElement returned:', imageData ? 'success' : 'null');
                            ...
                            if (imageData) {
                                ...
                                console.log(`[Grok] Found ${humanResponses.length} human messages with file attachments`);
                                ...
                                if (humanResponses.length > 0) {
                                    ...
                                    console.log(`[Grok] Captured user image for response ${targetResponse.responseId}`);
                                } else {
                                    console.warn('[Grok] No human messages with file attachments found to attach user image');
                                }
                            }
                        } catch (e) {
                            console.error('[Grok] Exception in user image processing:', e);
                        }
                    }
                }
        ...
        exportAll: async (btn, controlsArea) => {
            ...
            try {
                ...
                const zipFilename = `grok_export_all_${new Date().toISOString().slice(0, 10)}.zip`;
                Utils.downloadFile(zipBlob, zipFilename);
                alert(`${i18n.t('successExported')} ${exported} ${i18n.t('conversations')}`);
            } catch (error) {
                ErrorHandler.handle(error, 'Export all conversations');
