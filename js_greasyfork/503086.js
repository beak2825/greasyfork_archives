...
// @downloadURL https://update.greasyfork.org/scripts/503086/pixels%20tree%20not%20drink.user.js
// @updateURL https://update.greasyfork.org/scripts/503086/pixels%20tree%20not%20drink.meta.js
// ==/UserScript==
  ...
  */
        ...
        let interval = setInterval(() => {
            ...
            if (unsafeWindow.spacepack) {
                ...
                let position_y = 0
                let orders = []
                let mine_map = 'pixelsNFTFarm-2781'
                let apiary_time = {}
                let ElementsName = []
                let cur_coins = 0
                ...
                let sell_info = ["Popberry", "Grainbow", "Queen Bee", "Honey", "Wax","Whittlewood Log","Sap"]
                ...
                async function sendMssage() {
                    ...
                    while (room){
                        ...
                        if(state.id.includes("terravilla")){
                            ...
                            entities = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].entities;
                            for (const [key, value] of entities) {
                                if(value.gameEntity&&(value.gameEntity.id.includes('ent_treeLand1')||value.gameEntity.id.includes('ent_treeWater1'))){
                                    if (energy < 10 ) {
                                        let gostore=true
                                        break;
                                    }
                                    while (true){
                                        if (value.currentState.state === 'mature'){
                                            ...
                                            // info['tree_mv'][keys]={"x":value.position.x,"y":value.position.y}
                                        }else {
                                            break
                                        }
                                    }
                            ...
                            entities = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].entities;
                            for (const [key, value] of entities) {
                                if (value.gameEntity&&(value.gameEntity.id==='ent_sap'||value.gameEntity.id==='ent_wood_01')){
                                    await move_distance(value.propCache.position.x,value.propCache.position.y)
                                    await sleep(1000)
                                }
                            }
