...
// @version         1.4.1
// @date            2016-05-14
// @namespace https://greasyfork.org/users/90801
// @downloadURL https://update.greasyfork.org/scripts/26159/Gazelle%20Snatched.user.js
// @updateURL https://update.greasyfork.org/scripts/26159/Gazelle%20Snatched.meta.js
// ==/UserScript==
...
(function () {
	...
	var T_SNATCHED = 'color: #E5B244 !important; text-decoration:line-through !important; display:inline; background:url(https://ptpimg.me/9782nn.png) top right no-repeat; padding:0 18px 1px 0;';
	var UPLOADED = 'color: #63b708 !important; text-decoration:line-through !important; display:inline; background:url(https://ptpimg.me/tgwtts.png) top right no-repeat; padding:0 18px 1px 0;';
	var LEECHING = 'color: #F70000 !important; display:inline; background:url(https://ptpimg.me/xpf985.png) top right no-repeat; padding:0 18px 1px 0;';
	var SEEDING = 'font-style:italic; text-decoration:none !important;';
	var BOOKMARKED = 'background:url(https://ptpimg.me/w3p5wx.png) top right no-repeat; padding:0 18px 1px 0;'; // or use https://ptpimg.me/l7vfh5.png for red bookmark
	...
	function StatusBox(title) {
		...
		return {
			...
			show: function(t) {
				if (global_hideStatusBox != 'true' || /apollo\.rip\/torrents\.php.type/.test(document.URL)) {
					t = t || 0;
					update_timer(t);
					set_visible(true);
				}
	...
	function registerMenuCommand(oText, oFunc) {
		...
		MenuCommandArray[MenuCommandArray.length] = [oText.replace("apollo.rip Snatched: ",""),oFunc,oText.replace("apollo.rip Snatched: ","").replace(" ","_")];
	}
	...
	/* Get apollo.rip base URL */
	var whatcd_url_base = document.URL.match(/^(https:\/\/apollo\.rip|https:\/\/apollo\.rip)/)[1];
	...
	var status = StatusBox('apollo.rip Snatched');
	...
	/* Reset command */
	registerMenuCommand('apollo.rip Snatched: Reset Snatched', function() { snatch_cache.clear(); bookmark_cache.clear(); GM_setLSValue('last_update', '0'); GM_setLSValue('full_update','1'); GM_setLSValue('fullUpdateStarted','1'); location.reload(); });
	/* Update w/o clear */
	registerMenuCommand('apollo.rip Snatched: Update', function() { GM_setLSValue('last_update', '0'); GM_setLSValue('full_update','1'); GM_setLSValue('force_all','1'); GM_setLSValue('fullUpdateStarted','1'); location.reload(); });
	registerMenuCommand('apollo.rip Snatched: Options', function() { DisplaySlideMenu(true); });
	...
	function scan_torrent_page(doc, type) {
		...
		torrent_table.find('div.group_info').each(function(i) {
			...
			if (m) {
				...
				log("Not sure how apollo.rip Snatched got here. Please notify MordredKLB with what you were doing");
			...
			if (group_id && torrent_id) {
				// we are saving a type of "snatched" but when applying that class we have to apply it as "wcds_snatched" due to a apollo.rip having it's own .snatched style now
	...
	function mark_snatched_links() {
		...
		$('#content a').each(function(i) {
			...
			if (href) {
				...
				if (group_id && b.groups[group_id] && !(/apollo\.rip\/bookmarks\.php/.test(document.URL)) &&
						!(/apollo\.rip\/user\.php/.test(document.URL)) &&
						(!torrent_id || !$(this).parent().parent().is('.group_torrent')) && !$(this).is('.post_id')) {
		...
		if (/apollo\.rip\/torrents\.php/.test(document.URL)) {
			/* Parse search */
			var search = {};
		...
		if (/apollo\.rip\/torrents\.php\?id/.test(document.URL)) {
	...
	/* This function was hacked from a generic one and converted to jQuery to work better with apollo.rip Snatched.
	...
	if (/apollo\.rip\/bookmarks\.php(?!.action=edit)/i.test(document.URL)) {
		var scan_status = $('<div>Scanning current page... <span></span></div>');
		status.contents().append(scan_status);
		status.show();
	...
	// 	location.reload();
	// }
	if (full_update == 1 /*&& !(/apollo\.rip\/torrents\.php/.test(document.URL))*/) {
