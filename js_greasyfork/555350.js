// ==UserScript==
// @name            HDrezka Improvement forked
// @name:en         HDrezka Improvement forked
// @name:uk         HDrezka Improvement forked
// @name:ru         HDrezka Improvement forked
...
// @author          rub4ek & ruff3d
// @namespace       http://tampermonkey.net/
// @version         2.1.7-forked
// @include         http*://*rezka*/*
...
// @license         MIT
// @downloadURL https://update.greasyfork.org/scripts/555350/HDrezka%20Improvement%20forked.user.js
// @updateURL https://update.greasyfork.org/scripts/555350/HDrezka%20Improvement%20forked.meta.js
// ==/UserScript==

$(document).ready(function() {
  let titleEl = $('title');
  let title = titleEl.text()
    .replace('онлайн бесплатно в хорошем качестве', '')
    .replace('Смотреть фильмы и сериалы онлайн в хорошем качестве 720p hd и без регистрации', 'HD-Rezka')
    .replace(/Смотреть (.+)/, '$1')
  title = title.charAt(0).toUpperCase() + title.slice(1);
  console.log(title);
  titleEl.text(title);
  let headerEl = $('.b-content__htitle h1');
  let headerText = headerEl.text().replace(/Смотреть (.+) в HD онлайн/, '$1')
  headerText = headerText.charAt(0).toUpperCase()  + headerText.slice(1);;
  headerEl.text(headerText);
  $('#wrapper').css('background-color', '#000!important');
  $('#addcomment-title').hide();
  $('#oframecdnplayer > pjsdiv:nth-child(5) > pjsdiv:nth-child(1)').css('background-color','rgba(23, 35, 34, 0.2)');
});
...
(function () {
    ...
    */
    ...
    .clearfix.b-wrapper.b-footer__inner,
    .b-footer,
    .b-disable-advertising {
      display: none !important;
    }
    
    #oframecdnplayer > pjsdiv:nth-child(5) > pjsdiv:nth-child(1) {
        background-color: rgba(23, 35, 34, 0.3);
    }

    .b-theme__template__night #wrapper {
      background-color: #000 !important;
    }

    .b-content__inline_item-cover .hd-label-row input[type=checkbox]+label:after {
      top: -22px !important;
    }

    .b-content__inline_item-cover .hd-label-row input[type=checkbox]+label:before {
      top: -20px !important;
    }
