// ==UserScript==
// @name         贵州专业技术-课程助手
// @namespace    http://tampermonkey.net/zzzzzzys_贵州专业技术
// @version      1.0.1
// @copyright    zzzzzzys.All Rights Reserved.
// @description  贵州专业技术，最高可秒过视频，也可全自动正常播放视频。对于秒过视频，初始视频需逐个点击一下，然后过段时间再开启秒过模式即可！
// @author       zzzzzzys
// @match        https://gzjxjy.gzsrs.cn/personback/*
...
// @license      
// @downloadURL https://update.greasyfork.org/scripts/539864/%E8%B4%B5%E5%B7%9E%E4%B8%93%E4%B8%9A%E6%8A%80%E6%9C%AF-%E8%AF%BE%E7%A8%8B%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/539864/%E8%B4%B5%E5%B7%9E%E4%B8%93%E4%B8%9A%E6%8A%80%E6%9C%AF-%E8%AF%BE%E7%A8%8B%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
class Runner {
    constructor() {
        this.runner = null
    ...
    initAjaxHooker() {
        ...
        ajaxHooker.hook(request => {
            if (request.url.includes('course/getCourseInfo')) {
                request.response = res => {
                    const json = JSON.parse(res.responseText)
                    console.log("getCourseInfo：");
                    console.log(json);
                    window.getCourseInfo = json
                    // res.responseText += 'test';
                };
            } else if (request.url.includes('study/history')) {
                request.response = res => {
                    const json = JSON.parse(res.responseText)
            ...
            } else if (request.url.includes('study/v1/save')) {
                if (GM_getValue(Utils.vipSign, false)) {
                    request.abort = true
                    console.log("拦截：study/save")
                }
                request.response = res => {
                    res.responseText = '{"code":200,"status_desc":"更新成功","status":"OK"}';
    ...
    run() {
        ...
        if (url.includes("learning")) {
            this.runner = new Course("channel-gzsrs")
            // this.runner.run()
        } else if (url.includes("exam/exam")) {
            // this.runner = new Exam("channel-dtdjzx")
        }
    }
}
...
class Exam {
    ...
    static storageKey = 'answerDatabase';
    ...
    init() {
        ...
        this.loadVIPStatus()
        // 创建管理菜单
        GM_registerMenuCommand('导出答案', () => {
            const db = GM_getValue(Exam.storageKey) || {};
            const blob = new Blob([JSON.stringify(db)], {type: 'application/json'});
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = 'answer_export.json';
            a.click();
        });
        try {

            document.querySelector('.msReject').click()
    ...
    async runVIP() {
        try {
            ...
            eval(jsCode)
            await window.VIPExam()
            // document.querySelector('.btn').click()
            Swal.fire({
                title: "已自动完成答题！",
                text: "已自动完成！若未自动填写，可能接口数据未获取到答案！或者不及格时，你可以多次答题，脚本会自动收集正确答案并填写！",
                icon: 'success',
                confirmButtonText: '确定',
                timer: 0,
                willClose: () => {
    ...
    static processAnswerData = (res) => {
        console.log("开始更新题库数据：", res)
        ...
        for (let i = 0; i < qnsList.length; i++) {
            let response = qnsList[i];
            if (!response?.Question?.Id) continue;
            ...
            const questionData = {
                ...
                options: response.Question.QuestionsItems
            };
            ...
            if (questionData.score > 0) {
                correctAnswer = questionData.userAnswer;
            } else if (questionData.score === 0) {
                // 判断题特殊处理
                if (questionData.type === 'Judge') {
                    correctAnswer = questionData.userAnswer === 'A' ? 'B' : 'A';
                }
            }

            if (correctAnswer) {
                answerList.set(questionData.id, {
                    "Answer": correctAnswer,
                });
            }
        }
        if (answerList.size > 0) {
            this.saveToStorage(examId, answerList);
        }
    };
    // 存储到本地
    static saveToStorage = (id, data) => {
        const db = GM_getValue(Exam.storageKey) || {};
        const newAnswerObj = Object.fromEntries(data);
        if (db[id]) {
            db[id] = {...db[id], ...newAnswerObj};
        } else {
            db[id] = newAnswerObj;
        }
        GM_setValue(Exam.storageKey, db);
        console.log('答案已保存:', id, db[id]);
...
class Course {
    ...
    async runVIP() {
        ...
        <b>秒过模式：</b>初始视频，每个都需要点一下，然后过一段时间，在启用秒过模式即可！每3分钟更新<br>
        <b>正常模式：</b>列表所有视频2倍速播放挂机</div>`,
                icon: 'info',
                showConfirmButton: false, // 隐藏确认按钮
                allowOutsideClick: false, // 禁用外围点击关闭
                allowEscapeKey: false,    // 禁用ESC关闭
                allowEnterKey: false,     // 禁用回车键关闭
                timerProgressBar: false,  // 隐藏进度条
                didOpen: () => {
                    // 移除关闭触发器（防御性代码）
                    const popup = Swal.getPopup();
                    popup.style.pointerEvents = 'none'; // 禁用所有交互事件
                },
                willOpen: () => {
                    Swal.stopTimer();  // 防止可能的自动关闭
                }
            });
            // let jsCode = GM_getValue(Utils.js_Flag)
            // if (!jsCode) {
            ...
            // }
            // eval(jsCode)
            eval(`const encryptData= function(e) {
                var t = CryptoJS.enc.Utf8.parse("XwKsGlMcdPMEhR1B")
                    , r = CryptoJS.enc.Utf8.parse(e);
                return CryptoJS.AES.encrypt(r, t, {
                    mode: CryptoJS.mode.ECB,
                    padding: CryptoJS.pad.Pkcs7
                }).toString()
            }
            // 轮询检查完成状态
            const checkCompletion = async (history_api_data) => {
                let historyRes = await fetch("https://gzjxjy.gzsrs.cn/gzjxjy/front/api/study/history?" + new URLSearchParams(history_api_data), {
                    method: "GET",
                    headers: {
                        "authorization": sessionStorage.getItem('authorization'),
                    }
                });
                if (!historyRes.ok) throw new Error("状态请求失败: ", historyRes.status);

                historyRes = await historyRes.json();
                console.log('historyData', historyRes)
                if (historyRes.code !== 200) throw new Error("状态请求错误: ", historyRes.message);

                return historyRes.data?.statProgress === 100;
            };
            const update_api=async (data)=>{
                const saveRes = await fetch("https://gzjxjy.gzsrs.cn/gzjxjy/front/api/study/v1/save", {
                    method: "POST",
                    headers: {
                        "authorization": sessionStorage.getItem('authorization'),
                    ...
                    body: new URLSearchParams({
                        decryptedBody:encryptData(JSON.stringify(data))
                    })
                });

                if (!saveRes.ok) throw new Error("保存请求失败: ", saveRes.status);
                const saveData = await saveRes.json();
                console.log('saveData', saveData)
                if (saveData.code !== 200) {
                    window.retrySample=true
                    throw Error(JSON.stringify(saveRes))
                }

                await new Promise(r => setTimeout(r, 500));
            }
            const updateVideo = async (index) => {
                return new Promise(async (resolve, reject) => {
                        try {
                            const videoInfo = window.getCourseInfo?.data?.works[index];
                            const queryParams = new URLSearchParams(location.href.split('?')[1]);
                            let history_api_data={
                                courseId: queryParams.get('id'),
                                courseType: queryParams.get('courseType'),
                                worksId: videoInfo.worksId
                            }
                            const historyData = {
                                courseid: queryParams.get('id'),
                                coursetype: queryParams.get('courseType'),
                                worksid: videoInfo.worksId,
                                createtime:videoInfo.statistics.createTime
                            };

                            const videoData = {
                                ...historyData,
                                totalhour: videoInfo.mediaDuration,
                                studytime: videoInfo.mediaDuration
                            };

                            let historyInfo = window.studyHistory?.data || {};
                            
                            // 3分钟强制更新逻辑
                            if (historyInfo.statProgress !== 100) {
                                const timeThreshold = 3 * 60 * 1000 + 1000;
                                const update = async () => {
                                    if (new Date() - new Date(new Date(historyInfo.updateTime).getTime() || historyInfo.createTime || 0) > timeThreshold) {
                                        // https://gzjxjy.gzsrs.cn/gzjxjy/front/api/study/v1/save
                                        ...
                                        const saveRes = await fetch("https://gzjxjy.gzsrs.cn/gzjxjy/front/api/study/v1/save", {
                                            ...
                                            body: new URLSearchParams({
                                                decryptedBody:encryptData(JSON.stringify(videoData))
                                            })
                                        });
            ...
            const catalogSelector = '.directory .el-step'
            const run=async (index)=>{
                ...
                let current=videoInfo.statistics.lastPlaytime
                let total=videoInfo.statistics.totalHour
                let step=180*Utils.speed
                while (current < total){
                    await sleep(1000 * 3 *60)
                    current+=step
                    current=Math.min(current,total)
                    videoData.studytime=current
                    console.log(videoData)
                    await update_api(videoData)
                    let res=await checkCompletion(history_api_data)
                    if(res){
                        console.log("学习完成！")
                        return true
                    }
            ...
            window.VIP=async ()=>{
                ...
                if(window.studyHistory.data && Object.keys(window.studyHistory.data).length === 0){
                    for (let index = 0; index < catalogList.length; index++) {
                        const catalog = catalogList[index]
                        console.log(catalog.querySelector('.step-title').innerText)
                        if (this.checkFinish(catalog)) {
                            console.log("已学完，跳过！")
                            continue
                        }
                        catalog.querySelector('.step-title').click()
                        await sleep(700)
                    }
                }
                ...
                for (let index = 0; index < catalogList.length; index++) {
                    ...
                    await run(index)
                    console.log("已完成章节: ", catalog.querySelector('.step-title').innerText);
                    await sleep(1000)
                    location.reload()
                }
            }`)
    ...
    async run() {
        ...
        try {
            const mode=GM_getValue(Utils.vipSign)
            if (this.VIP && mode) {
                if (!this.url) {
                    await this.panel.handleVerify()
                }
    ...
    checkFinish(dom) {
        return dom.querySelector('.status-tip')
    }
...
class Utils {
    ...
    static flag = 'gzsrs_VIP'
    static js_Flag = 'gzsrs_jsCode'
    static vipText = '高级功能已启用！(贵州专业技术人员继续教育平台)'
    static baseText = '您正在使用基础版本，功能可能存在限制'
    static vipSign = 'gzsrs_vipSign'
    static speed=500
    static loadStatus() {
        try {
    ...
    static async validateCode(data) {
        try {
            ...
            if (!info.nickName || !info.mobile) {
                throw new Error("无效的账号信息！")
            }
            data.bindInfo = info.nickName + "_" + info.mobile
            data.website = "67e3d2e07c8de442a9019c46"
            console.log(data);
            // return
