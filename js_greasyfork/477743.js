// ==UserScript==
// @name         专业化团队-Rat签收工具
// @namespace    http://192.168.207.136:8881/epoint-rwgl-web
// @version      1.2
// @description  Rat签收工具
// @author       wl
...
// @grant        none
// @updateurl  https://greasyfork.org/en/scripts/477743-%E4%B8%93%E4%B8%9A%E5%8C%96%E5%9B%A2%E9%98%9F-rat%E7%AD%BE%E6%94%B6%E5%B7%A5%E5%85%B7
// @downloadURL https://update.greasyfork.org/scripts/477743/%E4%B8%93%E4%B8%9A%E5%8C%96%E5%9B%A2%E9%98%9F-Rat%E7%AD%BE%E6%94%B6%E5%B7%A5%E5%85%B7.user.js
// @updateURL https://update.greasyfork.org/scripts/477743/%E4%B8%93%E4%B8%9A%E5%8C%96%E5%9B%A2%E9%98%9F-Rat%E7%AD%BE%E6%94%B6%E5%B7%A5%E5%85%B7.meta.js
// ==/UserScript==
...
(function () {
    ...
    //#region 需求流程处理页面
    if (window_url.indexOf("demandbasicinfo_detail?") != -1 ||window_url.indexOf("demandbasicinfonew_detail?") != -1) {
        // 需求信息
        ...
        function loadBtn() {
             signBtn = createBtn("Rat签收", function () {
                                loadDemandInfo();
                                var datagrid_projectgroup = mini.get('datagrid_projectgroup');
                                var datagrid_group = mini.get('datagrid_group');
                                console.log(mini.get('transactionhistory1'));
                                var worklist = mini.get('transactionhistory1').controls.workitemlist.data;
                                if (worklist.length == 0) {
                                    epoint.alert('因为处理历史列表还未加载出来，请再点击一次！');
				                    return;
                                }
                                var itemdate="";
                                for(var i = 0; i < worklist.length; i++){
                                    if(worklist[i].activityName=="开发负责人阅读"){
                                        itemdate = worklist[i];
                                    }
                                }
                                console.log(itemdate);
				                var projectname = demandInfo.projectname;
				                var username = itemdate.transactorName;
                                var userguid = itemdate.operatorfordisplayGuid;
				                var ouname = "";
				                var json = {};
				                json.projectname = projectname.replace(/&/g,"111");
				                json.username = username;
                                json.userguid = userguid;
				                json.ouname = ouname;
                                json.demandname = mini.get('demandname').getValue().replace(/&/g,"111").replace(/·/g,"222");
                                json.functionreadtag = mini.get('functionreadtag').getValue();
                                json.hopefinishdate = mini.get('hopefinishdate').getValue();
                                json.operatedate = $('.form-control.span1[label="登记日期"]').children().html();
                                json.demandStorynum =$('.form-control.span1[label="需求编号"]').children().html();
                                json.windowurl =parent.window.location.href.replace(/&/g,"替换符号");
                                    //218.4.136.126:8881
                                      window.open("http://192.168.207.136:8881/epoint-rwgl-web/jxgl/performancerequire/performancerequireadd?data="+epoint.encodeUtf8(JSON.stringify(json)),
					                    "_blank");
