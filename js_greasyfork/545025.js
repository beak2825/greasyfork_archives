...
// @name              糖心Vlog
// @version           1.0.5
// @description       🔥不限次看站内所有付费视频，下载视频，复制播放链接，屏蔽广告
...
// @antifeature       payment
// @license           MIT
// @downloadURL https://update.greasyfork.org/scripts/545025/%E7%B3%96%E5%BF%83Vlog.user.js
// @updateURL https://update.greasyfork.org/scripts/545025/%E7%B3%96%E5%BF%83Vlog.meta.js
// ==/UserScript==
...
function init($){
	...
	const initPlayerUrl = async function(that, url, params){
		if (superVip._CONFIG_.user && superVip._CONFIG_.user.token) {		
			...
			try{
				let res = await util.asyncHttp(superVip._CONFIG_.apiBaseUrl + '/s' + (Math.floor(Math.random() * 3) + 1) + '00/tangxinSign?code=' + encodeURIComponent(params[0]) + '&version=' + superVip._CONFIG_.version)
					...
					const interceptMedia = (element) => {
						if(element.src && element.src.match(/\.mp4$/)){
							if(!superVip._CONFIG_.videoUrl.playerUrl || superVip._CONFIG_.videoUrl.playerUrl != element.src){
								superVip._CONFIG_.videoUrl.downloadUrlSign = element.src
							...
							success: async (confirm) => {
								if (confirm) {
									try {
										...
										$.ajax({
											url: superVip._CONFIG_.apiBaseUrl + '/d' + (Math.floor(Math.random() * 3) + 1) +'00/downloadTangxin',
											method: 'POST',
