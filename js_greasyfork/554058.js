// ==UserScript==
// @name         Ultimate Hand History Collector - v0.7.2
...
// @license MIT
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/554058/Ultimate%20Hand%20History%20Collector%20-%20v072.user.js
// @updateURL https://update.greasyfork.org/scripts/554058/Ultimate%20Hand%20History%20Collector%20-%20v072.meta.js
// ==/UserScript==
...
(function() {
    ...
    function validateHand(hand) {
        ...
        // Check 4: Mixed format detection ($ and BB in same hand)
    let hasDollarFormat = false;
    let hasBBFormat = false;

    hand.messages.forEach(msg => {
        // Check for $ amounts (excluding "won $" at end)
        if (msg.text.match(/(?:posted|called|bet|raised).*\$/)) {
            hasDollarFormat = true;
        }
        // Check for BB amounts
        if (msg.text.match(/\d+\.?\d*\s+BB/)) {
            hasBBFormat = true;
        }
    });

    if (hasDollarFormat && hasBBFormat) {
        warnings.push('⚠️ MIXED FORMAT: Hand contains both $ and BB betting - possible corruption!');
    }
        ...
        panel.innerHTML = `
            ...
            <div class="hh-content" id="hhContent">
                <div class="hh-settings-panel" id="hhSettingsPanel" style="display: none;">
                   ...
                   <div class="hh-setting-row" style="margin-top: 10px;">
                       <label>Data Management:</label>
                   </div>
                   <div class="hh-setting-actions">
                       <button class="hh-btn hh-btn-danger" id="hhClearHands">Clear Hands</button>
                       <button class="hh-btn hh-btn-danger" id="hhClearStats">Clear Stats</button>
                       <button class="hh-btn hh-btn-danger" id="hhClearNotes">Clear Notes</button>
                   </div>
        ...
        helpModal.innerHTML = `
            <div class="hh-modal-content">
                ...
                <div class="hh-modal-body">
                    <h3>✨ Fully Automatic System</h3>
                    <div class="hh-modal-body">
    <h3>✨ Fully Automatic System</h3>
    <p>Simply play poker - everything happens automatically:</p>
    <p>• Hands are collected and archived in real-time</p>
    <p>• Stats calculate after every hand</p>
    <p>• HUD updates automatically</p>
    <p>• Hand list refreshes automatically</p>
    <p><strong>No manual clicking required!</strong></p>

    <h3>📦 Two-Database Architecture</h3>
    <p><strong>Archive Database (What You See):</strong> Stores all your hands for review and analysis</p>
    <p><strong>Temp Database (Hidden):</strong> Briefly holds each hand for stat calculation, then auto-clears</p>
    <p>This keeps the system fast while preserving your full hand history!</p>

    <h3>🎯 Hand Detection</h3>
    <p>Hands are detected elegantly using game messages:</p>
    <p>• Collection starts when "Game [id] started" appears</p>
    <p>• Collection continues through all betting rounds</p>
    <p>• Hand ends when next "Game started" message appears</p>
    <p>• This captures split pots, reveals, and all-in situations perfectly</p>

    <h3>💾 Backup & Export</h3>
    <p><strong>Export JSON:</strong> Creates a backup of ALL archived hands</p>
    <p>• Use for safekeeping or sharing hand histories</p>
    <p>• Can be imported back later to restore your archive</p>
    <p><strong>Recommended:</strong> Export backups regularly</p>

    <h3>📥 Importing Hands</h3>
    <p>Click <strong>Import JSON</strong> to restore from backup or add old hands:</p>
    <p>• Hands are added to archive database</p>
    <p>• Duplicates are automatically skipped</p>
    <p>• Imported hands appear in your view immediately</p>

    <h3>🔍 Reviewing Hands</h3>
    <p>Navigate through your complete hand history:</p>
    <p>• <strong>◄ Previous / Next ►</strong> - Move one hand at a time</p>
    <p>• <strong>← Previous 50 / Next 50 →</strong> - Jump through pages</p>
    <p>• <strong>🏷️ Add Tag</strong> - Label hands for future reference</p>
    <p>• <strong>📋 Copy</strong> - Copy hand details to clipboard</p>
    <p>• <strong>🗑️ Delete This Hand</strong> - Remove single hand from archive</p>

    <h3>🧹 Data Management</h3>
    <p>Control your poker data from Settings:</p>
    <p><strong>Clear Hands:</strong> Deletes all archived hands</p>
    <p><strong>Clear Stats:</strong> Deletes all player statistics (VPIP, PFR, etc.)</p>
    <p><strong>Clear Notes:</strong> Deletes all player notes</p>
    <p><em>⚠️ All clearing requires confirmation and cannot be undone!</em></p>

    <h3>🏥 Health Check</h3>
    <p>Validates all archived hands and reports issues:</p>
    <p>• Checks for proper hand structure</p>
    <p>• Detects incomplete hands</p>
    <p>• Identifies mixed $ and BB format corruption</p>
    <p>• Full report appears in browser console</p>

    <h3>💡 Pro Tips</h3>
    <p>• Archive persists forever (unless you clear it)</p>
    <p>• Stats accumulate across all sessions automatically</p>
    <p>• Split pots and all-in situations fully captured</p>
    <p>• Export backups before clearing any data</p>
    <p>• Use Stats Calculator to process hands into statistics</p>
    <p>• Use Stats Display to view live HUD overlay</p>
</div>
                </div>
            </div>
        `;
    ...
    function setupEventListeners() {
        ...
        const clearHandsBtn = document.getElementById('hhClearHands');
        const clearStatsBtn = document.getElementById('hhClearStats');
        const clearNotesBtn = document.getElementById('hhClearNotes');
        ...
        clearHandsBtn.addEventListener('click', clearHandsConfirm);
        clearStatsBtn.addEventListener('click', clearStatsConfirm);
        clearNotesBtn.addEventListener('click', clearNotesConfirm);
    ...
    async function loadHandMetadata() {
        return new Promise((resolve, reject) => {
            if (!archiveDb) {
                reject(new Error('Archive database not initialized'));
                return;
            }
            const transaction = archiveDb.transaction(["hands"], "readonly");
    ...
    try {
        ...
        dbSizeSpan.textContent = `${count} hands`;
    } catch (error) {
        dbSizeSpan.textContent = 'Error loading count';
        console.error('Failed to get hand count:', error);
    }
}
...
console.log('🔍 First imported hand:', importData.hands[0]);
console.log('🔍 First imported handId:', importData.hands[0]?.handId);
            ...
            const existingHands = await loadAllHandsFromArchive();
    ...
    function loadAllHandsFromArchive() {
        return new Promise((resolve, reject) => {
            if (!archiveDb) {
                reject(new Error('Archive database not initialized'));
                return;
    ...
    async function exportDatabase() {
        try {
            const allHands = await loadAllHandsFromArchive();
            const exportData = {
                version: '0.7.0',
    ...
    function clearArchiveHands() {
        return new Promise((resolve, reject) => {
            if (!archiveDb) {
                reject(new Error('Archive database not initialized'));
                return;
            }
            const transaction = archiveDb.transaction(["hands"], "readwrite");
            const store = transaction.objectStore("hands");
    ...
    async function deleteDatabase() {
        ...
        const confirmed = confirm(
            `⚠️ DELETE ARCHIVE DATABASE?\n\n` +
            `This will permanently delete ALL ${count} archived hands.\n\n` +
    ...
    async function clearHandsConfirm() {
    const count = await getHandCount();
    if (count === 0) {
        alert('No hands to clear - database is already empty.');
        return;
    }
    const confirmed = confirm(
        `⚠️ CLEAR HAND HISTORY?\n\n` +
        `This will permanently delete ${count} archived hands.\n\n` +
        `Consider exporting to JSON first as a backup.`
    );
    if (!confirmed) return;
    try {
        ...
        updatePaginationUI();
        updateDatabaseSize();
        console.log('✅ Cleared hand history');
        alert('Hand history cleared successfully.');
    } catch (error) {
        console.error('Failed to clear hands:', error);
        alert('Failed to clear hands. Check console for details.');
    }
}

async function clearStatsConfirm() {
    const confirmed = confirm(
        `⚠️ CLEAR ALL PLAYER STATS?\n\n` +
        `This will delete all player statistics (VPIP, PFR, etc.).\n\n` +
        `The HUD will show no stats until new hands are played.`
    );
    if (!confirmed) return;
    try {
        await clearPlayerStats();
        alert('Player stats cleared successfully.');
    } catch (error) {
...
async function clearNotesConfirm() {
    const confirmed = confirm(
        `⚠️ CLEAR ALL PLAYER NOTES?\n\n` +
        `This will delete all your notes on players.\n\n` +
        `This action cannot be undone.`
    );
    if (!confirmed) return;
    try {
        await clearPlayerNotes();
        alert('Player notes cleared successfully.');
    } catch (error) {
        console.error('Failed to clear notes:', error);
        alert('Failed to clear notes. Check console for details.');
    }
}
    ...
    // Add this function anywhere in the Collector script (near other utility functions)
async function performanceCheck() {
    console.log('🔍 === PERFORMANCE CHECK ===');

    // 1. Database sizes
    const archiveCount = await getHandCount();
    console.log(`📦 Archive: ${archiveCount} hands`);
    ...
    // 3. Check for intervals/timers
    const highestInterval = setInterval(() => {}, 999999);
    clearInterval(highestInterval);
    console.log(`⏱️ Active timers: ~${highestInterval} (lower is better)`);

    // 4. DOM element count
    const allElements = document.querySelectorAll('*').length;
    const hudElements = document.querySelectorAll('.poker-hud-overlay').length;
    console.log(`🌳 DOM elements: ${allElements} total, ${hudElements} HUD overlays`);

    // 5. Event listeners (approximate)
    const pokerTable = document.querySelector('.holdemWrapper___D71Gy');
    console.log(`👂 Poker table listeners: ${pokerTable ? 'attached' : 'not found'}`);

    // 6. Current state
    const state = window.pokerHistoryDB.getState();
    console.log(`🎮 Collecting: ${state.isCollecting}, Current hand: #${state.currentHandNumber}`);
    ...
    function updateUI() {
        ...
        navLabel.textContent = `Hand ${globalIndex}/${handMetadataList.length}`;
    ...
    // Remember current hand being viewed
    const currentHandId = currentPageHands.length > 0 && historyIndex >= 0 ?
        currentPageHands[historyIndex].handId : null;

    handMetadataList = await loadHandMetadata();

    // Try to find and restore the hand we were viewing
    if (currentHandId) {
        const foundIndex = handMetadataList.findIndex(h => h.handId === currentHandId);
        if (foundIndex >= 0) {
            // Calculate which page this hand is on
            metadataOffset = Math.floor(foundIndex / PAGE_SIZE) * PAGE_SIZE;
            await loadCurrentPage();
            // Set historyIndex to the position within the current page
            historyIndex = foundIndex - metadataOffset;
            updateUI();
            updatePaginationUI();
            return;
        }
    ...
    // If we couldn't find the hand, just stay at current position
    // (Don't reset to 0)
    await loadCurrentPage();
}
    ...
    function detectHeroName() {
        // Method 1: Hero element with name child
        const heroElement = document.querySelector('.playerMeGateway___AEI5_ .name___cESdZ');
        if (heroElement && heroElement.textContent.trim()) {
            heroName = heroElement.textContent.trim();
            console.log(`📊 Collector: Hero detected (hero element) as "${heroName}"`);
            return;
        }

        // Method 2: Torn's hidden input field (most reliable!)
        ...
        if (tornUserInput && tornUserInput.value) {
            try {
                ...
                if (userData && userData.playername) {
                    heroName = userData.playername;
                    console.log(`📊 Collector: Hero detected (torn-user data) as "${heroName}"`);
                    return;
                }
            } catch (e) {
                console.log('📊 Collector: Could not parse torn-user data');
            }
        }
        ...
        for (const selector of selectors) {
            ...
            if (element && element.textContent.trim()) {
                heroName = element.textContent.trim();
                console.log(`📊 Collector: Hero detected (fallback selector) as "${heroName}"`);
                return;
            }
        }

        // Method 4: Look for "Dealt to [name]" in recent messages
        ...
        for (let i = messages.length - 1; i >= Math.max(0, messages.length - 20); i--) {
            ...
            if (dealtMatch) {
                ...
                console.log(`📊 Collector: Hero detected (dealt message) as "${heroName}"`);
                return;
            }
        }

        console.warn('📊 Collector: Could not detect hero name');
    }
    ...
    function captureSupplementaryData() {
        return new Promise((resolve) => {
            ...
            // Hero name detection FIRST (before capturing tableData)
            if (hasCards) {
                // Have cards = actively playing
                if (!heroName) {
                    detectHeroName();
                }
            } else {
                // No cards = not playing
                if (heroName) {
                    console.log(`📊 Collector: No hero cards - clearing name`);
                    heroName = null;
                }
            }

            // NOW capture tableData (with correct heroName)
    ...
    return new Promise((resolve, reject) => {
        ...
        openRequest.onupgradeneeded = function(event) {
            ...
            if (!db.objectStoreNames.contains("playerStats")) {
                ...
                // Create playerNotes store for display script
            if (!db.objectStoreNames.contains("playerNotes")) {
                db.createObjectStore("playerNotes", {
                    keyPath: "playerName"
                });
            }
            }
    ...
    function initArchiveIndexedDB() {
        return new Promise((resolve, reject) => {
            const openRequest = indexedDB.open("pokerHandArchiveDB", 1);

            openRequest.onerror = () => {
                console.error('❌ Failed to open archive database');
                reject(openRequest.error);
            };

            openRequest.onsuccess = () => {
                archiveDb = openRequest.result;
                console.log('✅ Archive database connected');
                resolve(archiveDb);
            };
            ...
            openRequest.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains('hands')) {
                    const objectStore = db.createObjectStore('hands', { keyPath: 'handId' });
                    objectStore.createIndex('handNumber', 'handNumber', { unique: false });
                    objectStore.createIndex('startTime', 'startTime', { unique: false });
                    console.log('📦 Archive database object store created');
                }
            };
        });
    }
    ...
    function saveHandToDB(hand) {
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = function() {
                console.log('✅ Hand saved to temp DB:', hand.handId);

                // Also save to ARCHIVE database
                saveToArchive(handToSave)
                    .then(() => {
                        console.log('📦 Hand archived:', hand.handId);

                        // Fire event for auto-calculation
                        window.postMessage({
                            type: "COMMAND",
                            target: "PokerStatsCalculator",
                            action: "AUTO_CALCULATE",
                            source: "HandHistoryCollector"
                        }, "*");
                    // Fire event to refresh hand list display
                    window.postMessage({
                        type: "COMMAND",
                        target: "HandHistoryCollector",
                    ...
                    .catch(err => {
                        console.error('⚠️ Archive save failed (non-critical):', err);
                        resolve(request.result); // Still resolve - archive failure shouldn't break main flow
                    });
            };
    ...
    function clearPlayerStats() {
        return new Promise((resolve, reject) => {
            ...
            const request = store.clear();
            request.onsuccess = function() {
                console.log('✅ Cleared playerStats store');
                resolve();
            };
            request.onerror = function() {
                reject(request.error);
    ...
    function clearPlayerNotes() {
        return new Promise((resolve, reject) => {
            ...
            const transaction = db.transaction(["playerNotes"], "readwrite");
            const store = transaction.objectStore("playerNotes");
            const request = store.clear();
            request.onsuccess = function() {
                console.log('✅ Cleared playerNotes store');
                resolve();
            };
            request.onerror = function() {
                reject(request.error);
    ...
    window.pokerHistoryDB = {
        ...
        clearPlayerNotes: clearPlayerNotes,
    ...
    async function init() {
        ...
        try {
            await initIndexedDB();
            await initArchiveIndexedDB();
             // Listen for display refresh trigger
            window.addEventListener('message', (event) => {
                if (event.data.type === 'COMMAND' &&
                    event.data.target === 'HandHistoryCollector' &&
                    event.data.action === 'REFRESH_DISPLAY') {

                    console.log('🔄 Auto-refreshing hand list display');

                    // Wait a moment for archive DB to settle
                    setTimeout(() => {
                        refreshHistory();
                    }, 500);
                }
            ...
            if (count > 0) {
    // Query just the highest handNumber, not all hands
    const maxHandNumber = await new Promise((resolve, reject) => {
        const tx = archiveDb.transaction("hands", "readonly");
        const index = tx.objectStore("hands").index("handNumber");
        const request = index.openCursor(null, "prev");

        request.onsuccess = (e) => {
            const cursor = e.target.result;
            if (cursor) {
                resolve(cursor.value.handNumber || 0);
            } else {
                resolve(0);
            }
        };
