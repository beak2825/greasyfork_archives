// ==UserScript==
// @name           Season 100 updated VPB with Player Value
// @namespace      gashouse/RyanCane/slughead42/mandyross/monsterkill
...
// @grant          GM_addStyle
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/498629/Season%20100%20updated%20VPB%20with%20Player%20Value.user.js
// @updateURL https://update.greasyfork.org/scripts/498629/Season%20100%20updated%20VPB%20with%20Player%20Value.meta.js
// ==/UserScript==
...
// gashouse - bug corrections: Jan, 2024
//      updated to remove global defines that caused values to wander if you went to Previous Day or stopped to update or enhance
//      some 'if-then-else' constructs became out-of scope with vars causing variables to become stale or overwritten
//      tried to correct all the syntax and LINT notices
//         once the previous LINT corrections were done it created other syntax opportunities that required further debugging
// gashouse - additions: April, 2024
//      included the Player Value on Skill Point Page, but this script had syntax, scope and LINT issues also
//
...
function enhanceTraining() {
    ...
    var a;
    for ( a=0; a<attributeTrainingOptions.length; a++) {
    ...
    while (!found) {
        chosenAttribute = prompt(promptMsg);
...
function updateTrainingPrediction() {
    ...
    if (trainingType==TRAININGTYPE.MULTI) {
        ...
        logMsg = '<b>Multi Train Prediction</b><br/>';
        ...
        logMsg = '<b>Single Training Prediction</b><br/>'+singleTrainSelect.value+'<br/>';
...
// level up
    if (xp >= 1000) {
        ...
        var k;
        var new_value;
        for (k=0; k<positionData[position].majors.length; k++) {
            new_value = parseFloat(document.getElementById(positionData[position].majors[k]).innerHTML);
        ...
        for (k=0; k<positionData[position].minors.length; k++) {
            new_value = parseFloat(document.getElementById(positionData[position].minors[k]).innerHTML);
