// ==UserScript==
// @name         Caption Inspector + Kairos - pogdeviv
...
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/503748/Caption%20Inspector%20%2B%20Kairos%20-%20pogdeviv.user.js
// @updateURL https://update.greasyfork.org/scripts/503748/Caption%20Inspector%20%2B%20Kairos%20-%20pogdeviv.meta.js
// ==/UserScript==
...
// v4.0 added more buttons (IMDB, CSM, AppleTV, MAL, Lyrics, Translated lyrics) to metadata with option selection.
(function() {
    'use strict';
    ...
    let options = {};
    let urls = {};
    let pageId = (document.URL.includes('external-content-id/')
                  ? document.URL.split('external-content-id/')[1]?.split('&')[0]
                  : document.URL.split('sourcePackage=')[1]);

       GM_addStyle (` #root: { margin-top:40px; }
.mybutton:hover {
...
.mybutton {
  ...
  z-index: 0;
    ...
    function intcpt() {
        if (pageId !== (document.URL.includes('external-content-id/')
                         ? document.URL.split('external-content-id/')[1]?.split('&')[0]
                         : document.URL.split('sourcePackage=')[1])) {
            $('#cptInsptdiv').remove();
            pageId = (document.URL.includes('external-content-id/')
                      ? document.URL.split('external-content-id/')[1]?.split('&')[0]
                      : document.URL.split('sourcePackage=')[1]);
            resetVars(false);
            captions = [];
                ...
                if (data.length > 0 && !!data[0].territory) {
                    ...
                    if (data[0].staticMetadata.contentType == 'SEASON' || data[0].staticMetadata.contentType == 'EPISODE') {
                        if ($('p:contains("Series:")').length > 0) {
                            ...
                            urls.CSM = ''; urls.AppleTV = ''; urls.Lyrics = ''; urls.TranslatedLyrics = ''; urls.IMDb = ''; urls.MAL = '';
    ...
    function displayButton() {
        $('#app-layout-content-1').each(function() { this.children[0].children[0].style.paddingTop = '60px'; });
        $('#cptInsptdiv button').remove();
        if ($('#cptInsptdiv').length == 0) {
            $('#root')[0].insertAdjacentHTML('beforebegin', `<div id="cptInsptdiv" style="position:fixed; margin-top: 5%;width:100%;z-index:10000;">
                         <svg xmlns="http://www.w3.org/2000/svg" style="float:right; height:30px; width:30px;" width="16" height="16" fill="rgb(7, 115, 152)" class="bi bi-list" viewBox="0 0 16 16"><title>Options</title>
