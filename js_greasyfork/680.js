...
// @namespace      Uranos
// @description    Erweitert das Browserspiel Dragosien (dragosien.de) mit zusätzlichen Funktionen, die die Bedienung erleichtern sollen.
// @icon           https://i.ibb.co/FBpHwRK/pimpmydragosien.png
...
// @author         Uranos
// @version        2.14
// @new            Fehlerbehebungen
// @downloadURL https://update.greasyfork.org/scripts/680/Pimp%20My%20Dragosien.user.js
// @updateURL https://update.greasyfork.org/scripts/680/Pimp%20My%20Dragosien.meta.js
// ==/UserScript==
...
var SUC_script_num = 'https://greasyfork.org/scripts/680-pimp-my-dragosien/';
  function updateCheck(forced) {
    if ((forced) || (parseInt(GM_getValue('SUC_last_update', '0')) + 3600000 <= (new Date().getTime()))) {
      try {
        GM_xmlhttpRequest({
          method: 'GET',
          url: SUC_script_num+'code/Pimp%20My%20Dragosien.meta.js?'+new Date().getTime(),
...
+script_name+'\".\nM\u00f6chtest du es installieren?\n\nNeuerungen:\n'+neues)) {
                  ...
                  document.location.href=SUC_script_num+'code/Pimp%20My%20Dragosien.user.js';
                }
              }
              else if (forced) {
...
function platz() {
  ...
  var bmp1=parseInt(platz[5]+platz[6])-parseInt(platz[3]+platz[4]);
  if (mtgl==1) { var pl=""; } else { var pl="er" }
  bmp1=bmp1.toString().replace(/(\d+)(\d{3})\b/g,"$1.$2");
  mtgl_seite.innerHTML=mtgl_seite.innerHTML.replace(/(Baumeisterpunkte<\/b>)/, "$1 <span class='annotation'>"
+"\(Noch Platz f&uuml;r "+mtgl+" Mitglied"+pl+", "+bmp1+" Baumeisterpunkte\)<\/span>");
...
function lager_stufe() {
  ...
  GM_xmlhttpRequest({
    method: 'GET',
    url: 'https://'+welt+'/?t=market&tab=14',
    headers: {'Cache-Control': 'no-cache'},
    onload: function(resp) {
      rt=resp.responseText;
...
function orakel_ga(neu) {
  ...
  var url='https://'+window.location.host+'/?t=guild_arena&tab=5';
...
//weitere Infos
  ...
  var bmp2=line.parentNode.getElementsByTagName("div")[3].innerHTML.match(/(\d+)\D*\/\D*(\d+) Mitgliede?r?,\s*(\d*)\D?(\d+)\D*\/\D*(\d*)\D?(\d+)/);
  ...
  if (seite.innerHTML.match(/zur Gildenarena gehen/)) {
    ...
    if(typeof(GM_getValue("ga_stufe_"+user+"_"+location.host))!="undefined") {
      ...
      vitr=(parseFloat(ges_p)-(5*(parseFloat(bmp2[2])-1))-((parseFloat(bmp2[5])-5)/3)-(5*parseFloat(ga_stufe)))/20;
...
//Gildenhalle
  ...
  +'    <h3>Gildenhalle Stufe '+(parseInt(bmp2[2])-1)+'<\/h3>\n'
  +'    <div>und Chat<\/div>\n'
  +'    <div><\/div>\n'
  +'    <div>'+bmp2[1]+'\/'+bmp2[2]+' Mitglieder<\/div>\n'
  +'  <\/div>\n'
  +'<\/div>\n'
...
//Gildenturm
  if(gt!="") {
    ...
    +'    <h3>Gildenturm Stufe '+((parseInt(bmp2[5])-5)/3)+'<\/h3>\n'
    +'    <div>und News<\/div>\n'
    +'    <div><\/div>\n'
    +'    <div>'+bmp2[3]+'.'+bmp2[4]+'\/'+bmp2[5]+'.'+bmp2[6]+' Baumeisterpunkte<\/div>\n'
    +'  <\/div>\n'
    +'<\/div>\n'
    ...
    +'    <div>und News<\/div>\n'
    +'    <div><\/div>\n'
    +'    <div>'+bmp2[3]+'.'+bmp2[4]+'\/5.000 Baumeisterpunkte<\/div>\n'
    +'  <\/div>\n'
    +'<\/div>\n'
    +'<a onmouseover="showBuildingTooltip(elm(\'tooltip21\'));"  onmouseout="hideBuildingTooltip(elm(\'tooltip21\'));" class="building pos21"  href="?t=guild_hall&city=1&tab=11"><\/a>\n';
  }
...
//Mitglieder
  var hierhin=line.parentNode.getElementsByTagName("div")[3];
  var bmp3=hierhin.innerHTML.match(/<b>Gesamt:.*Mitgliede?r?\,\s*.*Baumeisterpunkte.*>/);
//var bmp3=hierhin.innerHTML.match(/(\d+)\D*\/\D*(\d+) Mitgliede?r?,\s*(\d*)\D?(\d+)\D*\/\D*(\d*)\D?(\d+)/);
  ...
  if(dr==true) tabelle+='5">'+bmp3[0]; else tabelle+='4">'+bmp3[0].replace(/(<\/b>) (<span)/,'$1<br>$2');
  tabelle+='<\/td><\/tr><\/table>';
...
//serverzeit=Tag,Monat,Jahr,Stunde,Minute,Sekunde
      ...
      if(((parseInt(zeit[3])>17)||(parseInt(zeit[3])==17&&parseInt(zeit[4])>=45))&&
        ...
        document.location.href="https:\/\/"+document.location.host+"\/?t=highscore&chapter=games&pos="+optionNext;
      }
      if((parseInt(zeit[3])>20)||(parseInt(zeit[3])==20&&parseInt(zeit[4])>=15)) {
        document.location.href="https:\/\/"+document.location.host+"\/?t=highscore&chapter=games&pos="+optionNextNext;
      }
...
function g_tab() {
  if (line) {
    if (line.innerHTML.match(/Gildenhalle/)&&line.parentNode&&line.parentNode.getElementsByTagName("ul")[0]) {
      if (line.parentNode.getElementsByTagName("h3")[0]) {
...
if(document.URL.match(/overview=0/)) { zeigen(); }
