...
function settings() { // 设置按钮
    if ($(this).attr('effect') === 'uncheck') {
        ...
        for (var i = 1; i <= 7; i++) {
            var effectValue = getScriptSettings(i).effect === true ? 'checked' : 'uncheck';// 确定复选框的effect属性
            ...
            var $checkbox = $('<input type="checkbox" num=' + i + ' use="setbox" effect="' + effectValue + '"' + check + ' style="margin-left: 10px; margin-top: 10px;" class="setbox">');
...
function setbox(){
    ...
    saveScriptSettings(num,$(this).attr('effect') === 'checked' ? false:true);
    if($(this).attr('effect') === 'uncheck') $(this).attr('effect', 'checked');
    else $(this).attr('effect', 'uncheck');
}
...
(function (){
    ...
    function runScript() {// 脚本主逻辑
        ...
        $('span.header2-text:contains("字幕组列表")').each(function(){
            var $settings_icon = $('<i class="fas fa-cog" id="settings-icon" effect="uncheck" style="color:gray; margin-left: 5px; cursor: pointer; "><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css"></i>');
