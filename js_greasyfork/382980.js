// ==UserScript==
// @name                WME Route Speeds (HOV development again)
// @description         Shows segment speeds in a route.
// @include             /^https:\/\/(www|beta)\.waze\.com\/(?!user\/)(.{2,6}\/)?editor\/?.*$/
// @version             2019.05.16.001.hov.tm
// @grant               none
...
// @downloadURL https://update.greasyfork.org/scripts/382980/WME%20Route%20Speeds%20%28HOV%20development%20again%29.user.js
// @updateURL https://update.greasyfork.org/scripts/382980/WME%20Route%20Speeds%20%28HOV%20development%20again%29.meta.js
// ==/UserScript==
...
(function () {
	...
	function addLabel(lines, speedtekst, routespeedsoption2, odctime, odclen, routespeedsoption4, id) {
        ...
        if (routespeedsoption4) speedtekst = Math.round(speedtekst / 1.60934) + " m "
           else speedtekst = odctime + " k " ;
...
//                rtsummarysec += routeodc[i].crossTimeWithoutRealTime;
			}
		}
		else {
			for (i = 0; i < routeodc.length; i++) {
...
//		var rtDif = (summarysec !== rtsummarysec);
        ...
        if (id==1 && routehovlane1) html = html + "<b>  \u25ca</b>";
        if (id==2 && routehovlane2) html = html + "<b>  \u25ca</b>";
        if (id==3 && routehovlane3) html = html + "<b>  \u25ca</b>";
        if (id==4 && routehovlane4) html = html + "<b>  \u25ca</b>";
        if (id==5 && routehovlane5) html = html + "<b>  \u25ca</b>";
	...
	function requestRouteFromLiveMap(x1, y1, x2, y2) {
		...
		$.ajax({
			...
			success: function (json) {
                ...
                console.log("Route type 1: " + routehovlane1);  // attempt to get HOV lane type from existing results
                console.log("Route type 2: " + routehovlane2);
                console.log("Route type 3: " + routehovlane3);
                console.log("Route type 4: " + routehovlane4);
                console.log("Route type 5: " + routehovlane5);
	...
	function initialiseWMERouteSpeeds() {
		...
		addon.innerHTML = '' +
			...
			'<div>' +
			getCheckboxHtml('option5', 'Max routes to display', '', { display: 'inline-block' }) +
			...
			getCheckboxHtml('option7', 'Real-Time Traffic', 'note: this only takes effect for routes within the last 30-60 minutes') +
			...
			'<option value="3">Fastest (Avg Segment Speed)</option>' +
			'</select>' +
			'<br>' +
