...
// @name         网站签到工具(有手动检查菜单）
// @version      1.0.0
// @description  用于各种论坛和网站自动签到 论坛签到工具
// @author       alonewinds
...
// @downloadURL https://update.greasyfork.org/scripts/562816/%E7%BD%91%E7%AB%99%E7%AD%BE%E5%88%B0%E5%B7%A5%E5%85%B7%28%E6%9C%89%E6%89%8B%E5%8A%A8%E6%A3%80%E6%9F%A5%E8%8F%9C%E5%8D%95%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/562816/%E7%BD%91%E7%AB%99%E7%AD%BE%E5%88%B0%E5%B7%A5%E5%85%B7%28%E6%9C%89%E6%89%8B%E5%8A%A8%E6%A3%80%E6%9F%A5%E8%8F%9C%E5%8D%95%EF%BC%89.meta.js
// ==/UserScript==

(function () {
    // --- 每日(xsijishe.net) 全局自动后台签到逻辑 开始 ---
    // 逻辑说明：
    // 1. 全局运行：匹配所有网站，确保浏览器打开即运行。
    // 2. 性能保护：仅在顶层窗口运行，避免iframe重复执行。
    // 3. 静默签到：通过后台请求检查和签到，用户无感知。
    // 4. 跨天检测：每分钟检查一次，由定时器触发。

    if (window.top === window.self) {
        function runXsijisheSign(force, retryCount) {
            retryCount = retryCount || 0;
            var MAX_RETRIES = 3;
            var today = new Date().toLocaleDateString();
            var lastSign = GM_getValue('xsijishe_last_signin_date', '');

            if (force || lastSign !== today) {
                console.log("【每日自动签到】检查签到状态..." + (retryCount > 0 ? " (重试第 " + retryCount + " 次)" : ""));
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function (response) {
                        if (response.status === 200) {
                            ...
                            // 检查是否已签到
                            if (responseText.includes("今日已签到") || responseText.includes("您的签到排名")) {
                                console.log("【每日自动签到】检测到今日已签到。");
                                GM_setValue('xsijishe_last_signin_date', today);
                                if (force) alert("【每日自动签到】测试成功：检测到今日已签到！");
                            } else {
                                // 未签到，尝试寻找签到链接
                                var hrefMatch = responseText.match(/id="JD_sign"[^>]*href="([^"]+)"/);
                                if (!hrefMatch) {
                                    hrefMatch = responseText.match(/href="([^"]*plugin\.php\?id=k_misign:sign[^"]*)"/);
                                }

                                if (hrefMatch && hrefMatch[1]) {
                                    var signUrl = hrefMatch[1];
                                    if (!signUrl.startsWith('http')) {
                                        signUrl = 'https://xsijishe.net/' + signUrl;
                                    }
                                    signUrl = signUrl.replace(/&amp;/g, '&');

                                    console.log("【每日自动签到】找到签到链接，正在请求:", signUrl);
                                    GM_xmlhttpRequest({
                                        method: "GET",
                                        url: signUrl,
                                        timeout: 10000,
                                        onload: function (r) {
                                            if (r.status === 200) {
                                                console.log("【每日自动签到】签到请求已发送，状态码:", r.status);
                                                GM_setValue('xsijishe_last_signin_date', today);
                                                if (force) alert("【每日自动签到】测试成功：已发送签到请求！");
                                            } else {
                                                handleError("签到请求返回错误状态码: " + r.status);
                                            }
                                        },
                                        onerror: function (e) {
                                            handleError("签到请求网络错误");
                                        },
                                        ontimeout: function () {
                                            handleError("签到请求超时");
                                        }
                                    ...
                                    console.log("【每日自动签到】未找到签到按钮链接，可能是页面结构变更或未登录。");
                                    if (force) alert("【每日自动签到】测试结果：未找到签到按钮 (可能已签到或未登录)");
                                }
                            }
                        } else {
                            handleError("主页面请求返回错误状态码: " + response.status);
                        }
                    },
                    onerror: function (e) {
                        handleError("主页面请求网络错误");
                    },
                    ontimeout: function () {
                        handleError("主页面请求超时");
                    }
                ...
                console.log("【每日自动签到】今日已执行过签到，跳过检查。");
            }

            function handleError(msg) {
                console.error("【每日自动签到】错误: " + msg);
                if (retryCount < MAX_RETRIES) {
                    var nextRetry = retryCount + 1;
                    var delay = 5000 * nextRetry; // 递增延迟: 5s, 10s, 15s
                    console.log("【每日自动签到】将在 " + (delay / 1000) + " 秒后尝试第 " + nextRetry + " 次重试...");
                    setTimeout(function () {
                        runXsijisheSign(force, nextRetry);
                    }, delay);
                } else {
                    console.error("【每日自动签到】已达到最大重试次数，今日不再尝试。");
                    var failMsg = "【每日自动签到】失败：" + msg + "。请手动检查网站状态。";
                    if (force) {
                        alert(failMsg);
                    } else {
                        GM_notification({
                            text: "尝试 3 次均失败，请检查网络或手动签到。",
                            title: "每日自动签到失败",
                            timeout: 0,
                            onclick: function () {
                                window.open("https://xsijishe.net/k_misign-sign.html", "_blank");
                            }
                        });
                    }
                }
        ...
        // 注册菜单命令，方便用户点击测试
        GM_registerMenuCommand("测试：立即检查每日签到", function () {
            runXsijisheSign(true);
        });

        // 启动时检查一次
        setTimeout(function () { runXsijisheSign(false); }, 5000); // 延迟5秒，避免拖慢页面首屏加载

        // 智能调度：不在每分钟轮询，而是计算距离午夜还有多久，直接休眠到那一刻
        function scheduleMidnightCheck() {
            var now = new Date();
            var midnight = new Date(now);
            midnight.setHours(24, 0, 0, 100); // 设为今晚 24:00:00.100 (即明天 00:00:00.100)

            var delay = midnight - now; // 计算毫秒差值

            // 如果计算出的时间甚至是负数（极少见情况），加一天
            if (delay < 0) {
                delay += 24 * 60 * 60 * 1000;
            }

            console.log("【每日自动签到】已进入低功耗待机模式。下一次唤醒时间：" + new Date(now.getTime() + delay).toLocaleString());

            setTimeout(function () {
                // 时间到，执行签到检查
                runXsijisheSign(false);
                // 递归调用，安排明天的检查
    ...
    //知轩藏书 梦楠分享 道言分享网
        ...
        window.setTimeout(function () {
            ...
            checkinLinks.forEach(function (link) {
                if (link.textContent.includes('今日奖励') || link.textContent.includes('每日')) {
                    link.click();
                    return false;
                }
            });
    ...
    //布谷TV 马克喵 咸鱼单机 小黑资源库
        ...
        window.setTimeout(function () {
            ...
            if (button && button.textContent.includes('每日')) {
                button.click();
            }
        }, 2000);
        return;
    ...
    //MineBBS我的世界中文论坛
        ...
        window.setTimeout(function () {
            ...
            if (button_cta && button_cta.textContent.indexOf("每日") == 0) {
                button_cta.click();
            }
        }, 2000);
        return;
    ...
    //鲲Galgame
        ...
        window.setTimeout(() => {
            ...
            if (signInBtn?.disabled === false) {
                ...
                console.log('鲲Galgame未找到每日按钮，可能是:\n1.已签到完成；\n2.菜单没有展开；\n3.被弹窗遮挡；\n4.文本代码没加载；\n5.网站代码更新了,给我提交反馈。');
            }
        }, 2000);
        return;
    }
