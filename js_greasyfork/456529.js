...
// @license MIT
// @run-at document-end
// @downloadURL https://update.greasyfork.org/scripts/456529/%E4%B8%89%E7%9B%B8%E4%B9%8B%E5%8A%9B%C2%B7%E4%B8%89%E5%8F%8B%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/456529/%E4%B8%89%E7%9B%B8%E4%B9%8B%E5%8A%9B%C2%B7%E4%B8%89%E5%8F%8B%E7%89%88.meta.js
// ==/UserScript==
...
(function () {
    ...
    const keyword_gcb = "罕见"
    const keyword_jn = "七海"
    const keyword_mml = "猫雷"
    const keyword_hjqs = "珈乐"
    ...
    const tag_hjqs_Inner = "<b style='color: #B8A6D9'>" + tag_hjqs + "</b>"
    ...
    let jiance = setInterval(() => {
        ...
        if (commentlist.length != 0) {
            ...
            commentlist.forEach(c => {
                ...
                if (hjqs.has(pid)) {
                    if (c.textContent.includes(tag_hjqs) === false) {
                        c.innerHTML += tag_hjqs_Inner
                    }
                    return
                } else if (no_hjqs.has(pid)) {
                    // do nothing
                    return
                }
                if (dwr.has(pid)) {
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function (res) {
                        if (res.status === 200) {
                            ...
                            unknown.delete(pid)
                            //vtuber
                            if (bshow && (st.includes(keyword_jxt) || st.includes(keyword_ccj) || st.includes(keyword_gcb)||st.includes(keyword_jn)||st.includes(keyword_mml)||st.includes(keyword_hjqs)||st.includes(keyword_dwr)||st.includes(keyword_bjx)||st.includes(keyword_nql)||st.includes(keyword_xxx)||st.includes(keyword_xhz))) {
