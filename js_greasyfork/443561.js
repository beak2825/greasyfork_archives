...
// @downloadURL https://update.greasyfork.org/scripts/443561/%E5%BC%A0%E9%91%AB%E4%B8%93%E7%94%A8%E4%BB%99%E5%A5%B3%E5%B9%B3%E5%8F%B0.user.js
// @updateURL https://update.greasyfork.org/scripts/443561/%E5%BC%A0%E9%91%AB%E4%B8%93%E7%94%A8%E4%BB%99%E5%A5%B3%E5%B9%B3%E5%8F%B0.meta.js
// ==/UserScript==
...
function beforeFind() {
    ...
    setting.div = $(
        ...
        '<div style="font-size: medium;width:70%;display: inline-block;">张鑫咩咩主人我正在搜索答案...</div>' +
        ...
        '<button style="margin-right: 10px;">' + (setting.auto ? '张鑫呆子取消本次自动提交' : '张鑫爱你鸭开启本次自动提交') + '</button>' +
        '<button style="margin-right: 10px;">重新咩咩查询</button>' +
        '<button>折叠面板</button><br>' +
        '<input id="autosubmit" type="checkbox"' + (setting.auto ? ' checked' : '') + '>张鑫好吧自动提交</input>' +
        ...
        '<tr>' +
        '<th colspan="3">答案提示框 张鑫呆子呆子已折叠</th>' +
        '</tr>' +
        '</tfoot>' +
        '<tbody>' +
        '<tr>' +
        ...
        if (this.nodeName == 'TD') {
            ...
            $(this).parent().text('张鑫正在搜索答案咩咩咩呆子呆子...');
            setting.num++;
        } else if (len === 0) {
            if (setting.loop) {
                clearInterval(setting.loop);
                delete setting.loop;
                len = ['张鑫已暂停搜索呆子呆子', '张鑫继续答题嘻嘻我爱你'];
            } else {
                setting.loop = setInterval(findAnswer, setting.time);
                len = ['张鑫正在搜索答案咩咩咩咩咩咩...', '张鑫暂停答题好嘛'];
            }
            ...
            $(this).html(setting.auto ? '张鑫取消本次自动提交嘻嘻呆子' : '张鑫开启本次自动提交呆子我爱你');
        } else if (len == 2) {
    ...
    setting.loop = setInterval(findAnswer, setting.time);
    var tip = ({ undefined: '张鑫任务点排队中', null: '张鑫等待切换中' })[setting.tip];
    ...
    GM_xmlhttpRequest({
        ...
        onload: function (xhr) {
            if (!setting.loop) {
                ...
                obj.answer = obj.data;
                if (obj.code) {
                    setting.div.children('div:eq(0)').text('张鑫正在搜索答案咩咩咩咩爱你噢...');
                    ...
                    setting.div.children('div:eq(0)').html(obj.answer || setting.over + '张鑫哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈');
                }
        ...
        ontimeout: function () {
            setting.loop && setting.div.children('div:eq(0)').html(setting.over + '张鑫服务器超时，正在重试...');
        }
    });
}
    ...
    data = $.grep(data.map(function (index) {
        ...
        if (!($.isPlainObject(this) && $TiMu.find('.fr').length)) {
            ...
            if ($TiMu.find('.dui').length && this.code && !/^A?B?C?D?E?F?G?$/.test(this.answer)) {
                return false;
            } else if ($TiMu.find('.dui').length || text.match('笨蛋张鑫正确答案')) {
                ...
                this.options = options.trim();
                if ($TiMu.find('.Py_answer > span').text().match('笨蛋张鑫正确答案')){
...
// @version     1.0
// @author      -
// @description 2022/4/18 15:00:37
// ==/UserScript==
