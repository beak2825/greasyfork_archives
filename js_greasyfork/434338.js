// ==UserScript==
// @name baiyunweideminahuatang
// @description wudan
// @author 吴丹
// @version 20170403.913
...
// @downloadURL https://update.greasyfork.org/scripts/434338/baiyunweideminahuatang.user.js
// @updateURL https://update.greasyfork.org/scripts/434338/baiyunweideminahuatang.meta.js
// ==/UserScript==
			...
			if (location.href.match(/\.163\..+?\/store/)) {} else {
				try {
					...
					function jxbvipobj() {
						...
						const jsonvipbybfqa = "https://www.ikukk.com/?ac=0&url=", bfqlwobj = /\.(?:youku|mgtv)\./, bfqzl = /(?:DP|[7七]牛|记忆)播放器/i, jsonviptha = /^\s*?(?![\s\S]*?(?:["']\s*?urls?\s*?["']\s*?:\s*?["']http|<script))/i, jsonvipthb = /{[\s\S]*?["']\s*?urls?\s*?["']\s*?:\s*?["'](?![\s\S]+?(?:\.html|(?:&llq|\/\?[^=#]+?[=#])["']|\.[a-z]{2,5}\?[a-z]{1,10}=))\s*?(http[\s\S]+?)["']/i, jsonvipthc = /{[\s\S]*?["']\s*?urls?\s*?["']\s*?:\s*?["'](?![\s\S]+?(?:&llq|\/\?[^=#]+?[=#]|\/[^\/]+?\.php\?(?![^"'=#]+?[=#]\w+?\.m(?:3u8|p4))[\s\S]+?)["'])\s*?(http(?![^"']+?\.html)[\s\S]+?\.[a-z]{2,5}[\s\S]+?)["']/i, jsonvipthd = /{[\s\S]*?["']\s*?urls?\s*?["']\s*?:\s*?["'](?![\s\S]+?(?:&llq|\/\?[^=#]+?[=#])["'])\s*?(http[\s\S]+?(?:\/[^\/]+?\.php\?(?![^"'=#]+?[=#]\w+?\.m(?:3u8|p4))|\/[^\/]+?\.php\?key=[^\/"']+?|=http)[\s\S]+?)["']/i, jsonvipthe = /^\s*?http(?!.+?http).+?\.m(?:3u8|p4)/i, jsonvipthf = /(?:{[\s\S]*?["']\s*?urls?\s*?["']\s*?:\s*?["']http(?!.+?http).+?shouquan[^"']*?\.m(?:3u8|p4)[^"']*?)["']/i, jsonvipthg = /^\s*?<\s*?[^>]*?html\s*?>[\s\S]+?<script[\s\S]+?(?:\s+?urls?\s*?=|,(?:\s*?video\s*?:\s*?\{[^\}]*?)?\s*?urls?\s*?:|=\s*?{[\s\S]*?"urls?"\s*?:)\s*?["']\s*?(https?:\/\/(?![^"']+?\.php\?(?![^"'=#]+?[=#]\w+?\.m(?:3u8|p4))[^"']*?)[^"']+?\.m(?:3u8|p4)[^"']*?)["']/i, jsonvipthh = /^\s*?<\s*?[^>]*?html\s*?>[\s\S]+?<h\d+?>[\s\S]*?解[^析]*?析[^失]*?失[^败]*?败/i, jsonvipthi = /^\s*?<\s*?[^>]*?html\s*?>[\s\S]+?<script/i, jsonvipthj = /^\s*?http.+?(?:\/[^\/]+?\.php\?|=http)/i, jsonvipthk = /["']\s*?urls?\s*?["']\s*?:\s*?["']([^"']+?)["']/i, jsonvipthl = /{(?![\s\S]*?["']\s*?urls?\s*?["']\s*?:\s*?["']http)/i, jsonvipthn = /^\s*?<\s*?[^>]*?html\s*?>[\s\S]+?<script[\s\S]+?(?:\s+?urls?\s*?=|,\s*?urls?\s*?:|=\s*?{[\s\S]*?"urls?"\s*?:)\s*?["'](?:\s*?|不匹配|\\u4e0d\\u5339\\u914d|充值|\\u5145\\u503c|https?:\/\/(?:(?!.+?http).+?shouquan|[^"']+?\/https?:\/\/)[^"']+?\.m(?:3u8|p4)[^"']*?)["']/i, jsonvipthm = /^\s*?(?:1{1,}|(?:原|(?:镶?嵌|内?置)入?|当前)网?页?播?放?)\s*?$/, jsonviptho = /^\s*?http(?!.+?http).+?\/[^\/]+?\.php\?key=[^\/"']+?/i, jsonvipthp = /{[\s\S]*?["']\s*?urls?\s*?["']\s*?:\s*?["'](http[^"']+?\?\w+?[=#])\s*?["']/i, jsonvipjkcsc = '<ul id="Dany" style="overflow:auto;margin:0;padding:0px;border:0;font-size:large;"><h1 style="text-align:center;overflow:auto;margin:0;padding:0px;border:0;font-size:large;color:forestgreen;height:33px;">提取结果</h1><h2 style="text-align:center;"><h2 style="text-align:center;"><a href="', jsonvipjkcsd = '" style="color:lightsalmon;font-size:large;text-decoration:none;font-family:monospace!important;" title="点击 打开该资源" target="_blank">', jsonvipjklxa = /^\s*?(?:https|aHR0cH|(?!http)[^\/]+?:\/\/)/, jxbjxqptck = "allowfullscreen=true,allowfullscreen=allowfullscreen,esizable=1,scrollbars=1,toolbar=0,status=0,width=1050,height=600,left=" + (screen.availWidth - 1050) / 2 + ",top=" + (screen.availHeight - 600) / 2, jxbjxqptdc = "allowfullscreen=true,allowfullscreen=allowfullscreen,esizable=1,scrollbars=1,toolbar=0,status=0,width=" + (screen.availWidth - screen.availWidth * 0.0066) + ",height=" + (screen.availHeight - 55);
						...
						cssobj('恢复声音,替换自动下一集,计时开始,计时结束,Dany,我是Dany,Dany是我,我是Dany,强制刷新,万能元素解析,Dany标题,Dany电脑,Dany手机,Dany电脑网页,Dany手机网页,我是Dany生活很简单,Dany监控视频播放结束,我是电脑芒果,Dany自动点击接口,跳过广告,Dany自动判断多余,Dany全屏,qqjstzobj,zddj,jxqvipimg,jxbsjqq,jxbvideo');
						async function zdbt() {
							setTimeout(function() {
								...
								if (document.querySelector("\u0044\u0061\u006e\u0079\u81ea\u5b9a\u4e49\u63a5\u53e3") != null) {} else {
									...
									if (localStorage.getItem("jkqhmsobj") == '2') {
										...
										httpsaobj = [{
											...
											name: "遇见",
											url: "https%3A%2F%2Fjx.yujiannk.com%2F%3Furl%3D",
											jj: "1"
										}, {
											name: "地衣",
											url: "https%3A%2F%2Fjiexi.us%2F%3Furl%3D",
											...
											name: "毅常",
											url: "https%3A%2F%2Fjh.zjhoude.cn%2Fm3u8.php%3Furl%3D",
											jj: "1"
										}, {
											name: "段友",
													...
													try {
														if (sessionStorage.getItem('首次解析') == null || sessionStorage.getItem('首次解析') == undefined || sessionStorage.getItem('首次解析') == '') {
															...
															try {
																...
																var jxbjxurl, jjxxaa, jjxxbb, jjxxcc, wudan;
																...
																let tctha = '<div id="Dany" ' + jxbvipjxyyystyle + '><img id="Dany" src="https://api.dujin.org/bing/1920.php" ' + jxbvipjximg + '></img></div>';
																let tcthb = '<div id="Dany" ' + jxbvipjxyyystyle + '><img id="Dany" src="' + jxbimg + '" ' + jxbvipjximg + '></img></div>';
																try {
																	...
																	try {
																		...
																		function vipjxdmaobj() {
																			setTimeout(function() {
																				function tcthaobj() {
																					if (document.querySelector(vip)) {
																						if ((https[num].tp != null || https[num].tp != undefined) && (https[num].tp.match(/^\s*?https:\/\//))) {
																							document.querySelector(vip).innerHTML = '<div id="Dany" ' + jxbvipjxyyystyle + '><img id="Dany" src="' + https[num].tp.replace(/(?:^\s*|\s*$)/, "") + '" ' + jxbvipjximg + '></img></div>';
																						} else {
																				...
																				try {
																					if (wudan.match(new RegExp("^https:\/\/(?!.+?(?:强制(?:弹[小大]窗|跳转|搜索)|播放器))")) && !(wudan.match(new RegExp("(?:透明无接口|布米米|视频选集|Youtube|麻花影视|旋风视频)")))) {
																						if (objj.match('acfun') || (pcliwaiobj && localStorage.getItem("jxywyfsgbobj") != null & !(sessionStorage.getItem("jxywygdfsgbobj") == '1'))) {
																							if (objj.match('acfun') || localStorage.getItem("jxywyfsgbobj") == '1') {
																								tcthaobj();
																								window.open(wudan.replace(bfqzl, ""), "bkmk_popup", jxbjxqptdc)
																								...
																								tcthaobj();
																								window.open(wudan.replace(bfqzl, ""), "bkmk_popup", jxbjxqptck)
																							}
																						} else {
																							try {
																							...
																							if (document.querySelector(vip)) {
																								function tcthcobj() {
																									...
																									document.querySelector(vip).innerHTML = jxbvipjxyyy + wudan + '" ' + jxbvipjximg + '></iframe></div>';
																									...
																									tcthcobj();
																									window.open(wudan.replace(/强制弹大窗.*?$/, "").replace(bfqzl, ""), "bkmk_popup", jxbjxqptdc);
																									...
																									if (sessionStorage.getItem("jxyxkdfsgbobj") == '1') {
																										...
																										tcthcobj();
																										window.open(wudan.replace(/强制弹小窗.*?$/, "").replace(bfqzl, ""), "bkmk_popup", jxbjxqptck);
																									} else if (localStorage.getItem("jxcxkfsgbobj") == '1') {
																										tcthcobj();
																										window.open(wudan.replace(/强制弹小窗.*?$/, "").replace(bfqzl, ""), "bkmk_popup", jxbjxqptdc);
																						...
																						if (wudan.match(new RegExp("透明无接口"))) {
																							try {
																								...
																								if (document.querySelector(vip)) {
																									document.querySelector(vip).innerHTML = '<div id="Dany" ' + jxbvipjxyyystyle + '><img id="Dany" src="https://ceo30.coding.net/p/ceo30/d/sjtp/git/raw/master/1.png" ' + jxbvipjximg + ' onclick="document.getElementById(\'jxbhttps\').click();"></img></div>';
																									jxqzsxobj()
																								} else {}
																							}
																						...
																						} else if (wudan.match(new RegExp("Youtube"))) {
																							tcthbobj();
																							try {
																								if (sessionStorage.getItem('搜索关键词') != null) {} else {
																									zdbt();
																						...
																						} else if (wudan.match(new RegExp("麻花影视"))) {
																							tcthbobj();
																							try {
																								if (sessionStorage.getItem('搜索关键词') != null) {} else {
																									zdbt();
																		...
																		if ((https[num].json == null || https[num].json == undefined || https[num].json == '' || https[num].json == '0' || https[num].json.match(new RegExp("(?:跳转?|^\s*?(?<![非不])套壳浏?览?器?|^\s*?(?<!开)关\s*?$)")) || localStorage.getItem("objjbo") != '1') || (https[num].json != null || https[num].json != undefined || https[num].json != '') && (localStorage.getItem("objjbo") != '1')) {
																			...
																			if (https[num].json == null || https[num].json == undefined || https[num].json == '' || https[num].json == '0') {
																				...
																				if (document.querySelector(vip)) {
																					if ((https[num].tp != null || https[num].tp != undefined) && (https[num].tp.match(/^\s*?https:\/\//))) {
																						...
																						document.querySelector(vip).innerHTML = '<div id="Dany" ' + jxbvipjxyyystyle + '><img id="Dany" src="' + jxbimg + '" ' + jxbvipjximg + '></img></div>';
																					}
																				} else {}
																				jxqzsxobj();
																				window.open(wudan.replace(bfqzl, ""), "bkmk_popup");
																			...
																			if (localStorage.getItem("objjbo") == '1') {
																				...
																				if (document.querySelector(vip)) {
																					document.querySelector(vip).innerHTML = '<div id="Dany" ' + jxbvipjxidstyle + '><div id="Dany" style="letter-spacing:2px;color:#f8964c;' + jxbvipjxstrongstyle + '><img src="' + jxbtupiand + '" style="width:80%;"></div></div>';
																					jxqzsxobj();
																				} else {}
																				try {
																					var flag = 'true',
									...
									if (jxbpcobj) {
										...
										function createMenuhttps() {
											...
											jxbhttps.id = "jxbhttps";
											jxbhttps.innerHTML = '<svg id ="vipsvg" style="font-weight:bold;cursor:pointer;bottom:50px;width:40px;height:40px;position:fixed;right:0.5vmin;z-index:100000;text-align:center;border-radius:10px;"</svg><svg xmlns="http://www.w3.org/2000/svg" version="1.1"><text x="0" y="45" fill="red" transform="rotate(30 20,40)">❤</text></svg';
										...
										(function() {
											try {
												...
												if (localStorage.getItem("jkqhmsobj") == '2') {
													...
													sjqjpbzdylwaobj = /(?:M19|邦宁|网吧|B站)/i, sjyyyzdypbobj = /(?:PAR|地衣|爱跟|老版|[淘白][a-z0-9]|看剧|酷看|久播)/i, sjqjpbzdylwbobj = /(?:八九|斯卡|思云|萌欣|诺讯|麦田|木穆|依齐|民俗|玻璃|七彩|扫描|神马|白玉|依托|垫片|乐多|Youtube|和巴|[方文学提教]案|布米米|视频选集|透明.*?接口)/i;
													for (let jxbqcsjjki = 0; jxbqcsjjki < jxbqcsjjk.length; jxbqcsjjki++) {
										...
										(function() {
											let jxbewj = '', jxbwjstyle = 'float:left;border-radius:3.5px;color:#000!important;box-shadow:inset rgba(255,254,255,.6) 0 0.3em 0.3em,inset rgba(0,0,0,.15) 0 -0.1em 0.3em,#d82661 0 0.1em 3px,#b7144a 0 0.3em 1px,rgba(0,0,0,.2) 0 0.5em 5px;background:0 0;font-weight:700;font-size:16px;padding:5px 2.5px 5px 2.5px;border:2.5px outset buttonface;text-decoration:none;background-color:#4CAF50;', jxbhttpsew = [{
												name: jxbewj + '<a style="' + jxbwjstyle + '" onclick="document.getElementById(\'jjxxkka\').style.display=\'none\';document.getElementById(\'jxbhttps\').click();" href="' + atob("aHR0cHM6Ly9naXRlZS5jb20vd2QxNzA0MDMvaXQtd2FzLW1lYW50LXRvLWJlLXlvdS9yYXcvbWFzdGVyLzIwMTcwNDAzLTEuanBn") + '" target="_blank">\u543e\u4e39</a>'
											}, {
												name: jxbewj + '<a style="' + jxbwjstyle + '" onclick="document.getElementById(\'jjxxkka\').style.display=\'none\';document.getElementById(\'jxbhttps\').click();" href="\u0068\u0074\u0074\u0070\u003a\u002f\u002f\u0077\u0077\u0077\u002e\u0061\u0073\u0079\u0073\u002e\u0076\u0069\u0070\u002f\u006b\u0075\u0061\u0069\u0073\u0068\u006f\u0075\u002f" target="_httpblank">\u5feb\u624b</a>'
											}, {
									...
									if (document.querySelector("\u0044\u0061\u006e\u0079\u7535\u8111\u7f51\u9875")) {
										...
										jxqvipimg.addEventListener('click', function() {
											if (document.querySelector("img#Dany")) {} else {
												...
												try {
													document.querySelector("div#Dany").innerHTML = '<div id="Dany" ' + jxbvipjxyyystyle + '><img id="Dany" src="https://api.dujin.org/bing/1920.php" ' + jxbvipjximg + '></img></div>';
												} catch (e) {}
												tcthbobj();
												jxqzsxobj();
											}
