...
// ==UserScript==
// @name         尔雅满分助手 支持gtpchart3.5答题和众多免费题库，自动静音播放，倍速需谨慎
// @namespace    askAuto
// @version      2.0.4
// @author      yifeng
// @description  挂机解放时间，无需任何操作自动完成所有任务点。汇集全网免费、付费题库接口支持一键对接，答案更全更靠谱。
...
// @match        *://*.hnsyu.net/*
// @match        *://*.gdhkmooc.com/*
// @require      https://cdn.staticfile.org/vue/3.2.47/vue.global.prod.js
...
// @require      https://cdn.staticfile.org/pinia/2.0.34/pinia.iife.prod.js
// @require      https://cdn.staticfile.org/element-plus/2.3.3/index.full.min.js
...
// @resource     element-plus  https://cdn.staticfile.org/element-plus/2.3.3/index.css
...
// @downloadURL https://update.greasyfork.org/scripts/473302/%E5%B0%94%E9%9B%85%E6%BB%A1%E5%88%86%E5%8A%A9%E6%89%8B%20%E6%94%AF%E6%8C%81gtpchart35%E7%AD%94%E9%A2%98%E5%92%8C%E4%BC%97%E5%A4%9A%E5%85%8D%E8%B4%B9%E9%A2%98%E5%BA%93%EF%BC%8C%E8%87%AA%E5%8A%A8%E9%9D%99%E9%9F%B3%E6%92%AD%E6%94%BE%EF%BC%8C%E5%80%8D%E9%80%9F%E9%9C%80%E8%B0%A8%E6%85%8E.user.js
// @updateURL https://update.greasyfork.org/scripts/473302/%E5%B0%94%E9%9B%85%E6%BB%A1%E5%88%86%E5%8A%A9%E6%89%8B%20%E6%94%AF%E6%8C%81gtpchart35%E7%AD%94%E9%A2%98%E5%92%8C%E4%BC%97%E5%A4%9A%E5%85%8D%E8%B4%B9%E9%A2%98%E5%BA%93%EF%BC%8C%E8%87%AA%E5%8A%A8%E9%9D%99%E9%9F%B3%E6%92%AD%E6%94%BE%EF%BC%8C%E5%80%8D%E9%80%9F%E9%9C%80%E8%B0%A8%E6%85%8E.meta.js
// ==/UserScript==
 
(t=>{const a=document.createElement("style");a.dataset.source="vite-plugin-monkey",a.textContent=t,document.head.append(a)})(" .dialog-footer button[data-v-468a4ec0]:first-child{margin-right:10px}#csbutton[data-v-468a4ec0]{position:fixed;bottom:20px;right:20px;z-index:99999}#zeokdjg[data-v-aae16a8b]{position:fixed;left:10px;bottom:50vh;z-index:9999}.question_btn[data-v-aae16a8b]{width:40px;height:40px;border-radius:5px;margin:5px}.question_div[data-v-aae16a8b]{height:200px}.question_ti[data-v-aae16a8b]{margin:10px 0 20px}.cx_log[data-v-aae16a8b]{margin:2px 0}.status_log[data-v-aae16a8b]{margin-top:10px} ");
...
(async function (vue, pinia$1, ElementPlus, md5, $$1) {
  ...
  class ServerApi {
    ...
    async m() {
      return new Promise(async (resolve) => {
        this.defaultRequest(`${this.api1}/def/askMsg`, "GET", null, {}).then((response) => {
  ...
  var typr_js = Typr;
  const questionType = { "单选题": "0", "多选题": "1", "填空题": "2", "判断题": "3", "简答题": "4", "名词解释": "5", "论述题": "6", "计算题": "7" }, log = (data, type = "info") => {
    var _a;
  ...
  }), removeHtml = (html) => null == html ? "" : html.replace(/<((?!img|sub|sup|br)[^>]+)>/g, "").replace(/&nbsp;/g, " ").replace(/\s+/g, " ").replace(/<br\s*\/?>/g, "\n").replace(/<img.*?src="(.*?)".*?>/g, '<img src="$1"/>').trim(), cl = (str) => str.replace(/^【.*?】\s*/, "").replace(/\s*（\d+\.\d+分）$/, ""), ls = (key, value, time) => {
    const ls2 = window.localStorage;
    if (void 0 === value) {
      const data = ls2.getItem(key);
      if (data) {
        const { value: v, time: t } = JSON.parse(data);
        return time && (/* @__PURE__ */ new Date()).getTime() - t > 1e3 * time ? (ls2.removeItem(key), null) : v;
      }
      return null;
    }
    ls2.setItem(key, JSON.stringify({ value, time: (/* @__PURE__ */ new Date()).getTime() }));
  }, getQuestion = (type, html) => {
    ...
    const fontData = ((base64) => {
      ...
      return array;
    })(fontMatch[1]), font = typr_js.parse(fontData), table = JSON.parse(_GM_getResourceText("ttf"));
    let text = {};
    for (let i = 19968; i < 40870; i++) {
      let t = typr_js.U.codeToGlyph(font, i);
      t && (t = typr_js.U.glyphToPath(font, t), t = md5(JSON.stringify(t)).slice(24), text[i] = table[t]);
    }
    ...
    switch (questionData.type) {
      ...
      case "1":
        ...
        for (var i = 0; i < matchArr.length; i++)
          $$1(html).find("ul:eq(0) li :radio,:checkbox,textarea").eq(matchArr[i]).click(), $$1(html).find(".answerBg").eq(matchArr[i]).click();
    ...
    const askstore = useAskStore(), { dialogVisible, count, questionList, task } = pinia$1.storeToRefs(askstore), activeName = vue.ref("first"), activeNames = vue.ref(["1"]);
    if (!ls("autoaskc")) {
      new ServerApi().m().then((res) => {
        ls("autoaskc", res, 86400);
      }).catch((res) => {
        ls("autoaskc", res, 86400);
      });
    }
    return { count, dialogVisible, questionList, activeName, activeNames, task, msg: ls("autoaskc"), Aim: aim_default, handleClick: (e) => {
      askstore.select(e);
    } };
