// ==UserScript==
// @name         2022年青椒课堂自动答题程序(终版)
// @namespace    李飞
// @version      2022.10.17
// @description  全自动
// @author       lee
// @license      MIT
// @match        *://nx.2-class.com/competition
// @match        *://nx.2-class.com/competition
// @match        *://www.2-class.com/competition
// @match        *://www.2-class.com/competition
// @grant        none
// @contributionAmount wxp://f2f0mQewTSpQRKupHG4MxG1LyLBoHlQux_K3K-Db84qVxr0
// @downloadURL https://update.greasyfork.org/scripts/453464/2022%E5%B9%B4%E9%9D%92%E6%A4%92%E8%AF%BE%E5%A0%82%E8%87%AA%E5%8A%A8%E7%AD%94%E9%A2%98%E7%A8%8B%E5%BA%8F%28%E7%BB%88%E7%89%88%29.user.js
// @updateURL https://update.greasyfork.org/scripts/453464/2022%E5%B9%B4%E9%9D%92%E6%A4%92%E8%AF%BE%E5%A0%82%E8%87%AA%E5%8A%A8%E7%AD%94%E9%A2%98%E7%A8%8B%E5%BA%8F%28%E7%BB%88%E7%89%88%29.meta.js
// ==/UserScript==

(async function() {
    'use strict';

    let questions = [
        [
            '1.根据《中华人民共和国刑法》，毒品是指鸦片、海洛因、甲基苯丙胺（冰毒）、吗啡、大麻、可卡因，以及国家规定管制的其他能够使人形成瘾癖的（  ）和精神药品。',
            4
        ],
        [
            '2.毒品的分类方式多种多样，从毒品的来源来看，毒品可分为天然毒品、（  ）、合成毒品。',
            1
        ],
        [
            '3. 禁毒是全社会的共同责任。对于中学生而言，下列做法正确的是：',
            2
        ],
        [
            '4. 阿片类毒品是从罂粟中提取的生物碱及体内外的衍生物。下列毒品中不属于阿片类毒品的是：',
            4
        ],
        [
            '5. 毒品祸害无穷，不仅严重损害吸毒者本人的身体健康，对吸毒者的家庭和整个社会秩序都造成了严重的打击，吸毒的危害包括：',
            8
        ],
        [
            '6. 鸦片，又叫阿片，俗称大烟，源于罂粟植物的（  ），其所含主要生物碱是吗啡。鸦片因产地不同，呈黑色或褐色；有氨味或陈旧尿味，味苦，气味强烈。鸦片最初是作为药用，主要用于镇咳、止泻等。吸食者初吸时会感到头晕目眩、恶心或头痛，多次吸食就会上瘾。',
            2
        ],
        [
            '7. 新精神活性物质（New Psychoactive Substances，简称NPS），是不法分子为逃避打击而对列管毒品进行化学结构修饰所得到的毒品类似物，具有与管制毒品相似或更强的兴奋、致幻、麻醉等效果，这一概念于2013年在《世界毒品报告》中被首次提出，下列关于新精神活性物质的说法正确的是：',
            8
        ],
        [
            '8.下列哪个选项全部是新精神活性物质？',
            1
        ],
        [
            '9. LSD, 化学名称为D-麦角二乙胺，于1938年被首次合成，是一种强效（  ）。',
            4
        ],
        [
            '10. 加强禁毒工作，治理毒品问题，对深入推进平安中国、法治中国建设，维护国家长治久安，保障人民群众健康幸福，实现“两个一百年”奋斗目标和中华民族伟大复兴的中国梦，具有十分重要的意义。作为一名中学生，可以为禁毒工作做些什么？',
            8
        ],
        [
            '11. “国际禁毒日”是每年的（  ）。',
            2
        ],
        [
            '12.珍爱生命，远离毒品。青少年要积极学习毒品知识，增强防毒拒毒意识，主动对毒品说“不”。下列有关毒品的说法正确的是？',
            8
        ],
        [
            '13. 我国的“毒品”一词有着深远的历史渊源，是国人在百年来与毒品不懈抗争中的创造成果。我国首次对 “毒品”一词作出清晰定义的法律文本是：',
            1
        ],
        [
            '14.毒品问题不是一个国家、一个民族的问题，而是全人类所面临的挑战。各国应当紧密团结在一起，携手应对毒品问题。目前，我国已加入的三大禁毒国际公约是：',
            8
        ],
        [
            '15. 可卡因是用（  ）制成的。',
            2
        ],
        [
            '16. 丧尸药、浴盐、土冰，这些都是同一类毒品的俗称，它的真正名称是（  ）',
            2
        ],
        [
            '17. 王某在某地购得15克海洛因藏在家中准备自己吸食，后被公安机关查获，王某的行为构成了什么罪？',
            2
        ],
        [
            '18. （  ）要依法加强涉毒演艺人员参加演出管理，推进娱乐服务场所毒品预防工作，支持创作优秀禁毒文化作品。',
            2
        ],
        [
            '19. 吸毒，也称“药物滥用”，就是出于（  ）目的，通过注射、口服、鼻吸或其他方式将毒品摄入人体的行为。',
            1
        ],
        [
            '20. 麻黄碱是从（  ）中提取的 。',
            2
        ],
        [
            '21. 不满十六周岁的未成年人吸毒成瘾严重的，可以不适用（  ）。',
            1
        ],
        [
            '22.第一次将毒品犯罪规定为国际犯罪的公约是（  ）。',
            8
        ],
        [
            '23. “金新月”国际毒源地是指以下哪几个国家的交界地带？',
            2
        ],
        [
            '24. 世界上第一个国际禁毒公约是（  ）。',
            2
        ],
        [
            '25.下列有关毒品的认识错误的是：',
            8
        ],
        [
            '26. 下面关于大麻的说法哪个是错误的？（  ）',
            8
        ],
        [
            '27. γ-羟基丁酸，又称GHB，是一种无色无味的液体，由于它可以被轻易地放入饮料之中交给不知情的受害人使用，且受害人经常会不记得遭受过攻击，所以被称为“液体迷魂药”“迷奸水”。',
            1
        ],
        [
            '28.（  ）是迷幻蘑菇中的主要成分，这种物质是一种血清素受体激动剂。在血清素缺席的场合，能够刺激一些受体，使人产生做梦一样的幻觉。',
            1
        ],
        [
            '29.（  ），又称“阿拉伯茶”“东非罂粟”等。原产于非洲及阿拉伯半岛，主要活性成分为卡西酮、去甲伪麻黄碱等，具有兴奋和轻微致幻作用。滥用会出现抑郁、烦躁等症状。',
            1
        ],
        [
            '30. 毒品是使用后能够产生（  ）的物质。',
            2
        ],
        [
            '31. 身体脱毒只是戒毒过程的第一步，最根本上的是要彻底摆脱（  ），才能达到彻底康复。',
            4
        ],
        [
            '32. 合成毒品直接作用于人的（  ）。',
            4
        ],
        [
            '33. 毒品预防教育的首要重点对象是（  ）。',
            2
        ],
        [
            '34. 当发现有人可能正在吸毒或实施涉及毒品的违法犯罪行为时，应该（  ）。',
            1
        ],
        [
            '35. 当有人威胁我们吸毒时，要将情况主动告知家长和学校，或者打110报警，寻求帮助。',
            1
        ],
        [
            '36. 一个完整的戒毒过程，包含（  ）。',
            1
        ],
        [
            '37. 当在你身边出现毒品时，正确的做法是（  ）。',
            4
        ],
        [
            '38. 吸毒人员既是病人又是（  ）。',
            2
        ],
        [
            '39. 国家鼓励公民、组织开展公益性的禁毒宣传活动。',
            1
        ],
        [
            '40. 国家严格管制戒毒药品的研究、生产、供应和使用。',
            1
        ],
        [
            '41. 海洛因的滥用方式有（  ）。',
            8
        ],
        [
            '42. 吸食海洛因，如采取静脉注射的方式，1至2次就可能成瘾。',
            1
        ],
        [
            '43. 你的好朋友在娱乐场所给了你一种样子像糖果一样的东西，说特别好玩，让你尝尝。你的选择应该是：',
            4
        ],
        [
            '44. 你的同学或者好朋友把你介绍给他的朋友们，这些人私密地聚在某人家里或者类似娱乐场所的地方，使用一些粉末、片剂或者类似香烟的东西，还请你品尝，你恰当的拒绝方式应该是（  ）。',
            8
        ],
        [
            '45. 你认为一个家庭如何才能“远离毒品”？',
            1
        ],
        [
            '46. 贩卖毒品负刑事责任的年龄是（  ）。',
            4
        ],
        [
            '47. 青少年如何防止吸毒？',
            8
        ],
        [
            '48. 青少年吸毒的原因是什么？',
            8
        ],
        [
            '49. 如果有人在我们周围大量生产制造冰毒，很容易产生大量刺鼻的刺激性气味。这种说法正确吗？',
            1
        ],
        [
            '50. 吸毒人员的毒品滥用方式多样，有更大的艾滋病传播风险。为了引起人们更广泛的关注，世界卫生组织将每年（  ）定为“世界艾滋病日”。',
            8
        ],
        [
            '51. 在机场或火车站安检之前，有陌生人很着急地请你帮忙带个小书包，说书包里是“救命药”，飞机抵达或火车到站后，有人会来取这个小书包，还要给你一些酬金。你该怎么做？',
            2
        ],
        [
            '52. 合成大麻素类物质是（  ）',
            4
        ],
        [
            '53. 近几年，经过持续开展对青少年的毒品预防教育，我国35岁以下吸毒人员逐年减少。',
            1
        ],
        [
            '54. 健康生活方式有？',
            8
        ],
        [
            '55. 2019年5月1日起，我国将（  ）类物质列入非药用类麻醉药品和精神药品管制品种增补目录。',
            4
        ],
        [
            '56. 止咳水通常含有可待因、麻黄碱等成分，（  ）服用可形成心理依赖，戒断症状类似海洛因。',
            1
        ],
        [
            '57. 要构筑良好的拒毒心理防线只需做到正确把握好奇心，抵制不良诱惑即可。',
            2
        ],
        [
            '58. 田某发现，在美国留学期间结识的朋友劳伦，经常在微信朋友圈出售一些号称化学合成的糖果、巧克力和“叶子”味的烟弹，这些很有可能含有毒品（  ）。',
            8
        ],
        [
            '59. 远离毒品的自我保护方法有：',
            8
        ],
        [
            '60. 制定《中华人民共和国禁毒法》的目的是：',
            8
        ],
        [
            '61.中学生应该怎样做才能远离毒品、拒绝毒品？',
            8
        ],
        [
            '62. 身边的亲戚、朋友、同学或者家长都吸烟，他们递烟给你，你不要，有人说偶尔体验一下没关系，你最好的应对方式应该是（  ）。',
            8
        ],
        [
            '63. 安定有助于睡眠，可以擅自服用，不需医生指导使用，也不会对身体造成什么危害。',
            2
        ],
        [
            '64. 为吸毒者提供房间、打火机、吸管等吸毒场所、工具，但自己并不吸毒，所以不用承担法律责任。这种说法：',
            2
        ],
        [
            '65. 近年来市场上不断涌现一些“新精神活性物质”，它们的使用效果和某些毒品相似，但却没有被国家管制为毒品。由于未被国家管制，所以我们可以尝试使用这种物质。',
            2
        ],
        [
            '66. 当前要不断推进禁毒宣传教育的理念思路、内容形式、方法手段改革创新，提高传播力、引导力和影响力，增强（  ）。',
            8
        ],
        [
            '67. 当前要坚持关口前移、预防为先，从根本上遏制吸毒人员滋生，从源头上（  ）。',
            2
        ],
        [
            '68. 为防范利用寄递渠道贩毒问题，国家邮政局要求物流寄递企业严格落实“（  ）、寄件验视、过机安检”三项制度。',
            4
        ],
        [
            '69. 下面表述正确的是？（  ）',
            8
        ],
        [
            '70. 禁毒民警检查某娱乐场所时，被两男子的交谈内容“吸引”，不时可以听到“壶”“溜了几口”等话语，通过这些敏感词，民警即时判断出来，这极有可能是两名吸食（  ）人员。',
            1
        ],
        [
            '71. （  ）地区是我国境外合成毒品的主要来源。',
            1
        ],
        [
            '72. 2019年8月29日，青岛海关首次查获了10盒1000粒被误传为“聪明药”的一类管制精神药物（  ）。',
            1
        ],
        [
            '73. 下列选项中属于吸毒成瘾表现的是（  ）。',
            8
        ],
        [
            '74. 下列选项中属于毒品的是（  ）。',
            8
        ],
        [
            '75. 下列选项中不属于毒品特征的是（  ）。',
            8
        ],
        [
            '76. 不是艾滋病传播途径的是（  ）',
            8
        ],
        [
            '77. 下列属于合成毒品的是（  ）。',
            4
        ],
        [
            '78. 下列说法正确的是（  ）。',
            1
        ],
        [
            '79. 下列物品中不属于毒品的是（  ）。',
            4
        ],
        [
            '80. 下列关于戒毒说法正确的是（  ）。',
            4
        ],
        [
            '81. 吸食过量大麻可发生意识不清、产生幻觉等。',
            1
        ],
        [
            '82. 吸食合成毒品危害比传统毒品要小。',
            2
        ],
        [
            '83. 吸食冰毒以后驾驶车辆，容易造成情绪冲动及过度兴奋，从而导致行为失控极易引发严重交通事故。',
            1
        ],
        [
            '84. 吸毒者在今后的人生道路上应该选择的正确生活态度是（  ）。',
            8
        ],
        [
            '85. 吸毒者不健康的心理有盲目从众、好奇、爱慕虚荣、赶时髦、追求刺激和享乐、赌气或逆反、无知和轻信、自暴自弃等。',
            1
        ],
        [
            '86. 我们的爸爸、爷爷都可以喝酒，所以我们未成年人也可以喝酒。',
            2
        ],
        [
            '87. 我们在网络上和陌生人交流和交友的时候，应该保持警惕心，不能轻易泄露自己的个人信息。',
            1
        ],
        [
            '88. 吸毒成瘾不仅是身体上会对毒品产生依赖，更严重的是在心理上对毒品产生依赖。',
            1
        ],
        [
            '89. 我国禁毒工作的治本之策是（  ）。',
            1
        ],
        [
            '90. 长期吸食以后，情绪亢奋冲动。就算是停止复吸，戒断很久以后，都容易出现幻听、幻觉、被害妄想等稽延性戒断症状。这是（　）毒品的精神病态特征。',
            4
        ],
        [
            '91. 禁毒工作要不断巩固深化（  ）成果，助力乡村振兴。',
            1
        ],
        [
            '92. 关于“上头电子烟”，下列说法正确的是（  ）。',
            8
        ],
        [
            '93. 有人劝说小明一起“嗨气球”，并且告诉小明吸了之后会有“上头”的感觉。对于该案例，下列说法正确的是（）。',
            8
        ],
        [
            '94. 小明随家人出国旅游，在国外的时候，有人邀请小明一起抽大麻烟，并且告诉小明在国外抽大麻是合法的。对于该案例，下列说法正确的是（）。',
            8
        ],
        [
            '95. 小明随家人搭乘飞机外出旅游，进机场的时候，有陌生人请小明帮忙，让小明偷偷把一小包“小树枝”贴身隐藏带过安检，并承诺事后给小明一大笔钱。对于该案例，下列说法正确的是（）。',
            8
        ],
        [
            '96. 关于毒品犯罪，下列说法正确的是（）。',
            8
        ],
        [
            '97. 2021年我国新列管（  ）易制毒化学品',
            4
        ],
        [
            '98. 下列属于我国2021年新列管易制毒化学品的有（  ）',
            2
        ],
        [
            '99. 下列属于非药用类麻醉药品和精神药品的有（ ）',
            4
        ],
        [
            '100. 国家鼓励吸毒成瘾人员自行戒除毒瘾。对自愿接受戒毒治疗的吸毒人员，公安机关对其原吸毒行为不予处罚。（  ）',
            1
        ],
        [
            '101. 戒毒条例》第十八条明确规定，乡（镇）人民政府、城市街道办事处和社区戒毒工作应当采取下列措施管理、帮助社区戒毒人员：',
            8
        ],
        [
            '102. 《中华人民共和国禁毒法》第三十八条规定。吸毒成瘾人员，有（  ）情形之一的，由县级以上人民政府公安机关作出强制隔离戒毒的决定。',
            8
        ],
        [
            '103. 《中华人民共和国禁毒法》第三十一条规定，吸毒成瘾的认定办法，由国务院（）部门、（）部门、（）部门规定。',
            8
        ],
        [
            '104. 20世纪末的奥林匹克竞技比赛中，为了赢得更好的比赛成绩，曾经一度被美国短跑、拳击等项目的运动员广泛滥用，并造成部分运动员最终患上急性心脏病或脑血管疾病引发猝死的物质是(  )。',
            2
        ],
        [
            '105. 对下列涉毒人员，可以不予处罚的是（ ）。',
            2
        ],
        [
            '106. 2021年3月，国家禁毒委员会命名了首批（  ）个全国禁毒示范城市。',
            8
        ],
        [
            '107. 目前，我国共有（   ）个公安院校设有禁毒学本科专业。',
            4
        ],
        [
            '108. 你的同学以认识朋友的名义邀请你参加聚会，在聚会现场让你尝试一下装在瓶子里的不明液体，说“不是毒品”“比较刺激”“不会上瘾”“大家都玩”。你应该选择怎么办？',
            8
        ],
         [
            '109. 下列哪一项不属于我国禁毒工作方针。',
            4
        ],
        [
            '110. 下列哪一项不属于我国刑法规定的罪名。',
            8
        ],
        [
            '111. 易制毒化学品的生产、经营、购买、运输无须经过许可即可开展。',
            2
        ],
        [
            '112. 县级以上各级人民政府＿＿将禁毒工作纳入国民经济和社会发展规划，并将禁毒经费列入本级财政预算。',
            2
        ],
        [
            '113. 未成年人的父母或者其他监护人＿＿对未成年人进行毒品危害的教育，防止其吸食、注射毒品或者进行其他毒品违法犯罪活动。',
            2
        ],
        [
            '114. 吸毒成瘾人员应当进行戒毒治疗。',
            1
        ],[
            '115. 对吸毒成瘾人员，公安机关可以责令其接受社区戒毒。社区戒毒期限为（  ）。',
            8
        ],
        [
            '116. 下列哪种毒品滥用历史悠久，地域广泛，全球滥用者最多？',
            8
        ],
        [
            '117. 大麻类毒品的主要活性成分是四氢大麻酚。',
            1
        ],
        [
            '118. 大剂量吸食大麻可造成幻觉、妄想、精神失常，长期使用大麻容易导致痴呆等。',
           1
        ],
        [
            '119. 世界上一些国家实行娱乐大麻合法化，我们国家也应该推行娱乐大麻合法化。',
           2
        ],
        [
            '120. 长期吸食大麻才会上瘾，偶尔吸一次不会上瘾。',
           2
        ],
        [
            '1.	“国际禁毒日”是每年的（  ）。',
        ],
        [
            '2.	传统毒品一般是指鸦片、海洛因、大麻等流行较早的毒品。',
            1
        ],
        [
            '3.	药品可以随意服用，不需要遵医嘱。',
            4
        ],
        [
            '4.	“金三角”是指泰国、缅甸、（  ）三国交界的地方（一个区域）。',
            1
        ],
        [
            '5.	 “摇头丸”是苯丙胺类的衍生物，属中枢神经（  ）。',
            2
        ],
        [
            '6.	甲基苯丙胺因其纯品无色透明，像冰一样，故俗称“冰毒”。',
            1
        ],
        [
            '7.	吸食毒品是违法行为，不是犯罪行为。',
            1
        ],
        [
            '8.	《中华人民共和国禁毒法》规定，教育行政部门、（  ）应当将禁毒知识纳入教育、教学内容，对学生进行禁毒宣传教育。',
            1
        ],
        [
            '9.	《中华人民共和国禁毒法》自（  ）起施行。',
            4
        ],
        [
            '10.	K粉呈白色结晶粉末状，易溶于水，可勾兑进饮料和酒水中。',
            1
        ],
        [
            '11.	从毒品流行的时间顺序看，可分为传统毒品和新型毒品。传统毒品一般是指鸦片、海洛因、大麻等流行较早的毒品。',
            1
        ],
        [
            '12.	从医学角度看，吸毒成瘾是一种疾病，是（  ）。',
            8
        ],
        [
            '13.	当发现有人可能正在吸毒或实施涉及毒品的违法犯罪行为时，应该（  ）。',
            1
        ],
        [
            '14.	当有人威胁我们吸毒时，要将情况主动告知家长和学校，或者打110报警，寻求帮助。',
            1
        ],
        [
            '15.	当有人以各种借口引诱你吸食毒品或尝试可能是毒品的药丸时，正确的做法是（  ）。',
            1
        ],
        [
            '16.	当在你身边出现毒品时，正确的做法是（　）。',
            4
        ],
        [
            '17.	毒品区别于其他毒物的自然属性是（  ）。',
            2
        ],
        [
            '18.	毒品是使用后能够产生依赖性的物质。',
            1
        ],
        [
            '19.	《中华人民共和国禁毒法》第四条规定：禁毒工作实行（  ）为主，综合治理，禁种、禁制、禁贩、禁吸并举的方针。',
            1
        ],
        [
            '20.	毒品与药品，往往具有双重属性，合法为人解除病痛的就是药品。',
            1
        ],
        [
            '21.	止咳水不能随便乱用，需要遵医嘱。',
            1
        ],
        [
            '22.	二十世纪（  ），中国获得“无毒国”美誉近三十年。',
            2
        ],
        [
            '23.	各级人民政府应当建立毒品违法犯罪举报制度。',
            1
        ],
        [
            '24.	根据（  ）需要，依法可以生产、经营、使用、储存、运输麻醉药品和精神药品。',
            1
        ],
        [
            '25.	在日常生活中防毒要做到：一是不要听人蛊惑，受人引诱；二是不要与吸毒、贩毒者为伍；三是不要随意接受陌生人的馈赠； 四是（  ）。',
            2
        ],
        [
            '26.	医学上习惯称吸毒为药物滥用。',
            1
        ],
        [
            '27.	国家鼓励公民、组织开展公益性的禁毒宣传活动。',
            1
        ],
        [
            '28.	合成毒品直接作用于人的（  ）。',
            4
        ],
        [
            '29.	各级各类学校必须开设禁毒专题教育课，将禁毒教育列为学校教育的内容。',
            1
        ],
        [
            '30.	戒毒人员在（  ）等方面不受歧视。有关部门、组织和人员应当在这些方面对戒毒人员给予必要的指导和帮助。',
            8
        ],
        [
            '31.	戒毒是一个长期的过程，包括生理脱毒与医学治疗、（  ）、善后辅导与回归社会三个阶段。',
            2
        ],
        [
            '32.	“金新月”国际毒源地是指以下哪几个国家的交界地带？',
            2
        ],
        [
            '33.	咖啡因是从茶叶、咖啡果中提炼出来的一种生物碱，是国家管制的精神药品，所以人们喝茶、喝咖啡的行为是吸毒。',
            2
        ],
        [
            '34.	麦角二乙胺（LSD），俗称“邮票”、“贴纸”，是一种强烈的致幻剂。',
            1
        ],
        [
            '35、关于“笑气”，下列说法正确的是：（  ）',
            15
        ],
        [
            '36.	南美的（　）、秘鲁、玻利维亚是可卡因的最大生产基地，俗称“银三角”。',
            4
        ],
        [
            '37.	李某发现儿子小强吸毒后，便将其关在家中，并与家人轮流看守令其戒毒。起初，断了毒品的小强呼天喊地，半个月后，小强又恢复了正常。试问小强是否已全部戒除毒瘾？（  ）',
            2
        ],
        [
            '38.	你的好朋友在娱乐场所给你一种样子像糖果一样的东西，说特别好玩，让你尝尝。你的选择应该是：（  ）',
            4
        ],
        [
            '39.	有人引诱你吸食一些不明来源的小零食、饮料等，你应该如何应对？',
            2
        ],
        [
            '40.	你认为一个家庭如何才能“远离毒品”？（  ）',
            1
        ],
        [
            '41.	青少年如何避免吸毒？（  ）',
            8
        ],
        [
            '42.	青少年吸毒的原因是什么？（  ）',
            8
        ],
        [
            '43.	珍爱生命，远离毒品，要做到（  ）',
            8
        ],
        [
            '44.	人们常说“毒品猛于虎”，毒品的危害除了对身心的危害，严重摧残吸毒者的身体之外，还包括（  ）。',
            8
        ],
        [
            '45.	如果有同学或好朋友吃了一些东西以后，发生昏厥、呕吐或是抽搐等不适症状，我们可以拨打120急救电话。',
            1
        ],
        [
            '46.	身体脱毒只是戒毒过程的第一步，最根本上的是要彻底摆脱（　），才能达到彻底康复。',
            4
        ],
        [
            '47.	王某在自己花盆里种植5株罂粟用来欣赏美丽花朵，这是允许的 。',
            2
        ],
        [
            '48.	世界卫生组织将每年（  ）定为“世界艾滋病日”。',
            8
        ],
        [
            '49.	毒品预防教育的重点对象是（　）。',
            1
        ],
        [
            '50.	吸毒的危害有哪些方面？（  ）',
            8
        ],
        [
            '51.	吸毒会损害人的呼吸系统、消化系统、心血管系统、免疫系统和神经系统，感染各种疾病。',
            1
        ],
        [
            '52.	吸毒行为可以通过采集吸毒嫌疑人的血液、尿液、毛发等检测出来。',
            1
        ],
        [
            '53.	吸毒人员是违法者，也是（  ）。',
            1
        ],
        [
            '54.	大剂量吸食大麻可造成幻觉、妄想、精神失常。',
            1
        ],
        [
            '55.	小明今年上五年级，沉迷于网络游戏，然后到黑网吧去上网，结交了不良的朋友，最后染上了烟瘾和学会了吸毒。这个故事告诉我们什么道理？（  ）',
            8
        ],
        [
            '56.	摇头丸主要出现在慢摇吧、迪厅等娱乐场所，青少年应拒绝去这些场所。（  ）',
            1
        ],
        [
            '57.	远离毒品的自我保护方法有（　）。',
            8
        ],
        [
            '58.	跳跳糖、奶茶、咖啡包等不是毒品，但是毒贩会把毒品伪装成“跳跳糖、奶茶、咖啡包”等，我们要保持警惕。',
            1
        ],
        [
            '59.	小学生应该怎样做才能远离毒品、拒绝毒品？（ ）',
            8
        ],
        [
            '60.	身边的亲戚、朋友、同学或者家长都吸烟，他们递烟给你，你不要，有人说偶尔体验一下没关系，你最好的应对方式应该是（　）。',
            8
        ],
        [
            '61.	世界上三大毒品产地中哪一个不在亚洲？' ,
            4
        ],
        [
            '62.	大麻是目前世界上滥用人数最多的毒品。',
            1
        ],
        [
            '63.	为了安全起见，我们应该拒绝陌生人给的糖果、点心或任何饮料。（  ）',
            1
        ],
        [
            '64.	“虎门销烟”是哪一天开始的（  ）。',
            1
        ],
        [
            '65.	未成年人的父母或者其他监护人应当对未成年人进行毒品危害的教育，防止其吸食、注射毒品或者进行其他毒品违法犯罪活动。',
            1
        ],
        [
            '66.	我国近代史中的第一次“鸦片战争”是哪国发起的？',
            4
        ],
        [
            '67.	我国禁毒工作的治本之策是（　）。',
            1
        ],
        [
            '68.	我们的爸爸、爷爷都可以喝酒，所以作为小学生我们也可以喝酒。（  ）',
            2
        ],
        [
            '69.	吸毒会败坏社会风气，腐蚀人的灵魂，摧毁民族精神。',
            1
        ],
        [
            '70.	吸毒人群的意外死亡率较一般人群高。',
            1
        ],
        [
            '71.	吸毒人员身体消瘦是一种常态，而非病态，所以吸食毒品能有效减肥。',
            2
        ],
        [
            '72.	吸毒如果仅仅偶尔吸一两次，一般都不会上瘾。这种说法（  ）',
            2
        ],
        [
            '73.	吸毒者不健康的心理有盲目从众、好奇、爱慕虚荣、赶时髦、追求刺激和享乐、赌气或逆反、无知和轻信、自暴自弃等。',
            1
        ],
        [
            '74.	吸食冰毒以后马上驾驶车辆，容易造成情绪冲动及过度兴奋，从而极易引发严重交通事故。',
            1
        ],
        [
            '75.	吸食注射毒品成瘾的，应当戒除毒瘾。',
            1
        ],
        [
            '76.	吸烟会使肺癌的发生几率增加，诱发呼吸系统疾病，从而威胁人们的身体健康。',
            1
        ],
        [
            '77.	下面表述正确的是？（  ）',
            8
        ],
        [
            '78.	学生也有一份禁毒的责任。',
            1
        ],
        [
            '79.	要拒绝毒品，我们除了要知道什么是毒品、知道毒品极易成瘾、知道毒品的危害，还要（　）。',
            8
        ],
        [
            '80.	以下属于生活技能教育主要内容的是（　）。',
            8
        ],
        [
            '81.	长期抽烟、喝酒也会产生生理依赖和心理依赖',
            1
        ],
        [
            '82.	出于观赏的目的，种植大麻、古柯或罂粟就是合法的。以上说法正确吗?',
            2
        ],
        [
            '83.	止咳露（或止咳水）只是一种常见的中成药，大量服用不会形成药物依赖。这种说法（  ）。',
            2
        ],
        [
            '84.	制定《中华人民共和国禁毒法》的目的是：预防和惩治毒品违法犯罪行为，保护公民身心健康，维护社会秩序。',
            1
        ],
        [
            '85.	学校毒品预防教育的目标是（  ）',
            4
        ],
        [
            '86.	学校是毒品预防教育的主阵地，课堂是主渠道',
            1
        ],
        [
            '87.	新精神活性物质又称：',
            8
        ],
        [
            '88.	不是艾滋病的传播途径的是（  ）',
            8
        ],
        [
            '89.	不健康的生活方式有？',
            8
        ],
        [
            '90.	国家禁毒办依托全国青少年毒品预防教育数字化平台，建立科学评比遴选机制，鼓励各地开发制作各类禁毒宣传教育资料，推送全国（  ）。',
            1
        ],
        [
            '91.	结交朋友越多越好吗？（  ）',
            4
        ],
        [
            '92.	高雅情趣是（  ）向上的生活情趣，能催人上进，改变人的精神面貌，提高人的文化修养，使生活更加充实且富有意义。',
            8
        ],
        [
            '93、目前，（  ）地区是对我国危害最大的毒品来源地。',
            1
        ],
        [
            '94、麻醉药品和精神药品属于列管物质，具有（  ）属性。①药品属性；②受管制性；③成瘾性；④滥用危害性。',
            4
        ],
        [
            '95、下列属于我国整类列管的非药用类麻醉药品和精神药品的有（  ）①合成大麻素类物质；②色胺类物质；③卡西酮类物质；④芬太尼类物质。',
            4
        ],
        [
            '96、芬太尼属于哪种药品？',
            2
        ],
        [
            '97、今年6月至11月，中央宣传部、中央网信办、教育部、国家禁毒办等十个部门联合制定方案，在全国集中组织开展防范毒品滥用宣传教育活动。方案明确指出，我国目前列管的麻精药品数量和整类列管的物质数量分别是（  ）。',
            8
        ],
        [
            '98、列入下列哪个目录的物质如果被滥用就是吸毒？',
            8
        ],
        [
            '99、合成毒品“麻古”是泰语的音译，其主要成分是(  )',
            1
        ],
        [
            '100、可卡因的原植物是(  )，曾经是古代美洲原住民的提神草。',
            8
        ],
        [
            '101、1909年2月1日，中、日、英、法、俄、德、美、葡等国召开禁毒会议，拉开了国际性禁毒活动的序幕，这次会议的举办地是(  )。',
            1
        ],
        [
            '102、近年来，各地禁毒部门积极推进现代科技应用，已经广泛应用了（  ）、（  ）等手段进行毒情监测预警。①污水监测；②毛发检测；③空气监测；④土壤监测。',
            1
        ],
        [
            '103、国家禁毒委员会成立于（   ）年，现有公安部、教育部等（   ）个成员单位。',
            1
        ],
        [
            '104、第一届全国青少年禁毒知识竞赛于（   ）年举办。',
            2
        ],
        [
            '105.   下列哪一项不属于我国禁毒工作方针。',
            4
       ],
        [
            '106.   易制毒化学品的生产、经营、购买、运输无须经过许可即可开展。',
            2
        ],
         [
            '107.   县级以上各级人民政府＿＿将禁毒工作纳入国民经济和社会发展规划，并将禁毒经费列入本级财政预算。',
            2
        ],
         [
            '108.   未成年人的父母或者其他监护人＿＿对未成年人进行毒品危害的教育，防止其吸食、注射毒品或者进行其他毒品违法犯罪活动。',
            2
        ],
        [
            '109.   长期吸食大麻才会上瘾，偶尔吸一次不会上瘾。',
            2
        ],
         [
            '110.	小学生应该怎样做才能远离毒品、拒绝毒品？（ ）',
            8
        ],
        ];
    const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));

    const stdString = (str) => str.replace(/\n.*/gu, '').replace(/①.*/gu, '').replace(/[^\u{4E00}-\u{9FA5}A-Za-z0-9]/gu, '');

    const next = () => {
        let length = document.getElementsByClassName('ant-btn').length;
        if (document.getElementsByClassName('ant-btn')[length - 1]) {
            document.getElementsByClassName('ant-btn')[length - 1].click();
        };
    };

    const answer = () => {
        let done = false;
        for (let [question, answer] of questions) {
            let input = document.getElementsByClassName('exam-content-question')[0].innerHTML;
            if (stdString(question).includes(stdString(input))) {
                let length = document.getElementsByTagName('label').length;
                let option = 0;
                while (option < length) {
                    // 如果有多选题的话，按位判断就很有用了
                    // 比方说 ACD 就是 13
                    if (document.getElementsByTagName('label')[option] && answer >> option & 1) {
                        document.getElementsByTagName('label')[option].click();
                    };
                    option += 1;
                };
                done = true;
                break;
            };
        };
        if (!done) {
            alert('题库未查询到此题，请按F5键刷新页面。');
        };
    };

    await sleep(800);
    let offset = 0;
    while (offset < 20) {
        next();
        await sleep(800);
        answer();
        await sleep(820);
        offset += 1;
    };
    await sleep(500 + Math.random() * 500);
    next();
})();