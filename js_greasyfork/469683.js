...
// @downloadURL https://update.greasyfork.org/scripts/469683/%E3%80%90%E7%A7%BB%E5%8A%A8%E7%AB%AF%E3%80%91%E7%99%BE%E5%BA%A6%E7%B3%BB%E4%BC%98%E5%8C%96-beta.user.js
// @updateURL https://update.greasyfork.org/scripts/469683/%E3%80%90%E7%A7%BB%E5%8A%A8%E7%AB%AF%E3%80%91%E7%99%BE%E5%BA%A6%E7%B3%BB%E4%BC%98%E5%8C%96-beta.meta.js
// ==/UserScript==
...
try {
  if (typeof Vue !== "object") {
    if (typeof window.Vue !== "object") {
      window.Vue = Vue;
    } else {
      console.warn("window.Vue is define");
    }
  } else {
    console.warn("Vue is define");
  }
} catch (error) {
  console.warn("Failed to register global variables 【window.Vue】: " + error);
}

/* 注册VueRouter */
try {
  if (typeof VueRouter !== "object") {
    if (typeof window.VueRouter !== "object") {
      window.VueRouter = VueRouter;
    } else {
      console.warn("window.VueRouter is define");
    }
  } else {
    console.warn("VueRouter is define");
  }
} catch (error) {
  console.warn(
    "Failed to register global variables 【window.VueRouter】: " + error
  );
}

/* 注册ElementPlus */
try {
  if (typeof ElementPlus !== "object") {
    if (typeof window.ElementPlus !== "object") {
      window.ElementPlus = ElementPlus;
    } else {
      console.warn("window.ElementPlus is define");
    }
  } else {
    console.warn("ElementPlus is define");
  }
} catch (error) {
  console.error(
    "Failed to register global variables 【window.ElementPlus】: " + error
  );
}

/* 注册ElementPlusIconsVue】 */
try {
  if (typeof ElementPlusIconsVue !== "function") {
    if (typeof window.ElementPlusIconsVue !== "function") {
      window.ElementPlusIconsVue = ElementPlusIconsVue;
    } else {
      console.warn("ElementPlusIconsVue is define");
    }
  } else {
    console.error(
      "Failed to register global variables 【window.ElementPlusIconsVue】: " +
        error
    );
  }
} catch (error) {
