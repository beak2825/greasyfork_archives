...
// @grant        GM_setValue
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/542131/ClickFit.user.js
// @updateURL https://update.greasyfork.org/scripts/542131/ClickFit.meta.js
// ==/UserScript==
  ...
  if (window.location.href !== currentUrl) {
    ...
    setTimeout(() => {
      if (currentUrl.includes('/file/')) {
        console.log('Nouvelle page de dossier détectée, ajout des boutons...');
        addCalculationButtonsToHeader();
      }
    }, 1000);
  }
}, 1000); // Vérifier l'URL toutes les secondes
...
function addCalculationButtonsToHeader() {
  ...
  if (!isFilePage) {
    console.log('ℹ️ Pas sur une page de dossier (/file/), boutons de calcul non ajoutés');
  }

  console.log('✅ Page de dossier détectée, ajout des boutons...');
  ...
  allElements.forEach((el, index) => {
    if (index < 20) { // Limiter à 20 éléments pour éviter le spam
      console.log(`  - ${el.tagName}.${el.className} (${el.id ? '#' + el.id : 'no-id'})`);
    }
  });
  ...
  async function quickSave() {
    ...
    for (const btn of buttons) {
      if (btn.textContent?.includes('Enregistrer') && !btn.disabled) {
        btn.click();
        clickCount++;
        console.log(`💾 Clic sur bouton Enregistrer ${clickCount}`);
      }
    }

    if (clickCount > 0) {
      console.log(`${clickCount} bouton(s) Enregistrer cliqué(s)`);
    }
  ...
  function createButtons() {
    ...
    if (!headerActions) {
      console.log('❌ Aucun header trouvé, retry dans 1000ms');
    ...
    console.log('✅ Boutons LRPG et Ortho-K ajoutés au header !');
  }
  ...
  function createSplitViewButton() {
    ...
    if (!nav) {
      console.log('❌ Navigation non trouvée, retry dans 1000ms');
    ...
    function handleSplitViewClick(e) {
      ...
      console.log('🔀 SplitView cliqué depuis le header');
      ...
      if (typeof activateSplitView === 'function') {
        try {
          activateSplitView();
          console.log('✅ SplitView activé depuis le header');
        } catch (error) {
          console.error('❌ Erreur lors de l\'activation SplitView:', error);
        }
      } else {
        console.warn('⚠️ Fonction activateSplitView non trouvée, recherche alternative...');

        // Chercher la fonction dans window
        if (typeof window.activateSplitView === 'function') {
          console.log('✅ Fonction trouvée dans window');
          window.activateSplitView();
        } else {
          console.warn('⚠️ Aucune fonction SplitView trouvée');
          // Toast de fallback
          const toast = document.createElement('div');
    ...
    console.log('✅ Bouton SplitView ajouté à la navigation !');
  }
...
delay(addCalculationButtonsToHeader, DELAYS.LONG);
  ...
  setInterval(syncHeaderButtonsWithRoute, 1500);
...
setInterval(recolorAstigmatisme, 2000);
  ...
  function createFloatingButton() {
    ...
    console.log('✅ Bouton flottant créé et ajouté au DOM');

    // Clic direct sur le bouton flottant = Import Topographies
    fab.addEventListener('click', async () => {
      console.log('📁 Clic bouton flottant - Import Topographies');

      // Protection contre les doublons
      if (window.DesktopImportModule && window.DesktopImportModule.isModalOpen) {
        console.log('⚠️ Modal déjà ouvert, on évite le doublon');
        return;
      }
      ...
      while (!window.DesktopImportModule && attempts < 10) {
        console.log(`⏳ Attente initialisation module... (${attempts + 1}/10)`);
        await wait(500);
        attempts++;
      }
        ...
        try {
          ...
          console.log('✅ Modal d\'import ouvert');
        } catch (error) {
        ...
        console.warn('⚠️ Module d\'import non chargé après 5 secondes');
        // Toast de fallback
        const toast = document.createElement('div');
  ...
  const DesktopImportModule = {
        ...
        async parseSaveIndexFile(filePath) {
            try {
                ...
                for (let i = 3; i < lines.length; i++) {
                    ...
                    console.log(`👁️ Œil trouvé: ${eye}, Patient: ${lastName} ${firstName}`);
                    ...
                    if (eye === 'OD' || eye === 'OS') {
                        ...
                        try {
                            ...
                            console.warn(`⚠️ Erreur parsing valeurs kératométriques pour ${eye}:`, e);
                        }
                    }
                }

                console.log(`✅ SaveIndex parsé:`, { eyes, infos });
                console.log(`📊 Détail des infos:`, JSON.stringify(infos, null, 2));
                return { eyes, infos };
        ...
        async parseXrefFile(filePath) {
          try {
              ...
              for (let i = 2; i < lines.length; i++) {
                  ...
                  console.log(`👁️ Œil trouvé: ${eye}, Fichier: ${filename}`);
                      ...
                      try {
                          ...
                          if (columns.length > 8 && columns[8] && columns[8].trim()) {
                              ...
                              if (simk1Val > 0) {
                                  ...
                                  console.log(`📊 SimK1: ${simk1Val}mm → ${infos[eye].simk1}D`);
                              }
                          }
                          ...
                          if (columns.length > 10 && columns[10] && columns[10].trim()) {
                              ...
                              if (simk2Val > 0) {
                                  ...
                                  console.log(`📊 SimK2: ${simk2Val}mm → ${infos[eye].simk2}D`);
                              }
                          }
                          ...
                          console.warn(`⚠️ Erreur parsing valeurs SimK pour ${eye}:`, e);
                      }
                  }
              }

              console.log(`✅ XREF parsé:`, { eyes, infos });
              console.log(`📊 Détail des infos:`, JSON.stringify(infos, null, 2));
              return { eyes, infos };
      ...
      parsePentacamFilename(filePath) {
          try {
              ...
              if (!match) {
                  console.warn(`⚠️ Format de nom Pentacam non reconnu: ${filename}`);
                  return null;
              }
              ...
              console.log(`👁️ Œil trouvé: ${eye}, Patient: ${fullName}`);
              ...
              console.log(`✅ Pentacam parsé:`, result);
              return result;
      ...
      createSimpleModal() {
          ...
          setTimeout(() => {
              ...
              if (overlay) {
                  ...
                  console.log('🔧 Overlay forcé visible');
              }
          }, 100);
                  ...
                  try {
                      ...
                      if (xrefData && xrefData.infos) {
                          ...
                          if (groupIndex !== undefined && this.currentGroups[groupIndex]) {
                              ...
                              const firstEyeData = Object.values(xrefData.infos)[0];
                              if (firstEyeData) {
                                  this.currentGroups[groupIndex].parsedData = firstEyeData;
                                  console.log(`📋 Données patient stockées dans le groupe ${groupIndex}:`, firstEyeData);
                                  console.log(`📋 Groupe mis à jour:`, this.currentGroups[groupIndex]);
                              } else {
                                  console.warn('⚠️ Aucune donnée œil trouvée dans xrefData.infos');
                              }
                          } else {
                              console.warn(`⚠️ Groupe ${groupIndex} non trouvé ou invalide`);
                          }

                          // Afficher les informations patient
                          let patientHtml = '';

                          Object.entries(xrefData.infos).forEach(([eye, info]) => {
                              ...
                              patientHtml += `
                                  ...
                                  ">
                                      ...
                                      <div style="margin-bottom: 4px;">
                                          <strong style="color: #333;">👤 Nom:</strong> <span style="color: #555;">${info.lastName || 'N/A'}</span>
                                      </div>

                                      <div style="margin-bottom: 4px;">
                                          <strong style="color: #333;">👤 Prénom:</strong> <span style="color: #555;">${info.firstName || 'N/A'}</span>
                                      </div>
                                  </div>
                              `;
                          });
                      ...
                      if (saveIndexData && saveIndexData.infos) {
                          ...
                          if (groupIndex !== undefined && this.currentGroups[groupIndex]) {
                              ...
                              if (firstEyeData) {
                                  ...
                                  console.warn('⚠️ Aucune donnée œil trouvée dans saveIndexData.infos');
                              }
                          } else {
                              console.warn(`⚠️ Groupe ${groupIndex} non trouvé ou invalide`);
                          }
                      ...
                      if (pentacamData && pentacamData.infos) {
                          ...
                          if (groupIndex !== undefined && this.currentGroups[groupIndex]) {
                              ...
                              if (firstEyeData) {
                                  ...
                                  console.warn('⚠️ Aucune donnée œil trouvée dans pentacamData.infos');
                              }
                          } else {
                              console.warn(`⚠️ Groupe ${groupIndex} non trouvé ou invalide`);
                          }
    ...
    async performRealImport(group, eye) {
      ...
      if (window.location.href === 'https://click-fit.precilens.com/' ||
          ...
          console.log('🏠 Page d\'accueil détectée - Lancement auto-création porteur + fichier avant import');
          await this.autoCreateWearerAndFile(group);
          console.log('✅ Auto-création terminée, arrêt de l\'import normal');
          return; // Sortir APRÈS la création automatique - IMPORTANT !
                  } else {
          console.log('ℹ️ Pas sur la page d\'accueil, import normal');
      }
    ...
    async autoCreateWearerAndFile(group) {
        console.log('🏠 Début auto-création porteur + fichier...');

        // Récupérer les données patient parsées depuis le modal
        let patientData = null;
        if (group && group.parsedData) {
            patientData = group.parsedData;
            console.log('📋 Données patient récupérées:', patientData);
            console.log('📋 Nom:', patientData.lastName);
            console.log('📋 Prénom:', patientData.firstName);
            console.log('📋 Date:', patientData.date);
        } else {
            console.warn('⚠️ Aucune donnée patient trouvée dans le groupe:', group);
            console.warn('⚠️ Group.parsedData:', group?.parsedData);
        }

      try {
                 // 1. Ouvrir le modal "Ajouter porteur"
                 console.log('👤 Étape 1: Ouverture du modal Ajouter porteur');
                 ...
                 for (const selector of wearerSelectors) {
                     ...
                     if (addWearerBtn) {
                         console.log(`✅ Bouton "Ajouter porteur" trouvé avec sélecteur: ${selector}`);
                         break;
                     }
                 }
                 ...
                 if (!addWearerBtn) {
                     ...
                     for (const btn of allButtons) {
                         ...
                         if (text.includes('Ajouter') && text.includes('porteur')) {
                             addWearerBtn = btn;
                             console.log(`✅ Bouton "Ajouter porteur" trouvé par texte: "${text}"`);
                             break;
                         }
                     }
                 }
                 ...
                 try {
                     addWearerBtn.click();
                     console.log('✅ Modal Ajouter porteur ouvert');
                 } catch (error) {
                     console.error('❌ Erreur lors du clic sur "Ajouter porteur":', error);
                     throw error;
                 }

                 // 2. Attendre que le modal apparaisse et remplir les champs
                 console.log('⏳ Attente de 500ms que le modal apparaisse...');
                 await wait(500);

                 // Vérifier que le modal est bien ouvert
                 const modal = document.querySelector('#wrapper > app-modals-injector > div > div > app-modal');
                 if (!modal) {
                     console.warn('⚠️ Modal non détecté, on continue quand même...');
                 } else {
                     console.log('✅ Modal détecté et ouvert');
                 }

                 console.log('📝 Étape 2: Remplissage automatique des champs patient');

          // Debug: Lister tous les inputs disponibles
          const allInputs = document.querySelectorAll('input');
          allInputs.forEach((input, index) => {
              if (index < 10) { // Limiter à 10 pour éviter le spam
                  console.log(`  Input ${index}: type="${input.type}", name="${input.name}", placeholder="${input.placeholder}", id="${input.id}"`);
              }
          });

                 // Remplir le nom de famille avec le sélecteur exact
                 if (patientData && patientData.lastName) {

                     const lastNameInput = document.querySelector('#input-lastName');

                     if (lastNameInput) {
                         console.log(`✅ Champ nom trouvé avec #input-lastName`);
                         lastNameInput.value = patientData.lastName;
                         lastNameInput.dispatchEvent(new Event('input', { bubbles: true }));
                         lastNameInput.dispatchEvent(new Event('change', { bubbles: true }));
                         lastNameInput.dispatchEvent(new Event('blur', { bubbles: true }));
                         console.log(`✅ Nom de famille rempli: ${patientData.lastName}`);
                     } else {
                         console.warn('⚠️ Champ #input-lastName non trouvé');
                     }
                 }

                 // Remplir le prénom avec le sélecteur exact
                 if (patientData && patientData.firstName) {

                     const firstNameInput = document.querySelector('#input-firstName');

                     if (firstNameInput) {
                         console.log(`✅ Champ prénom trouvé avec #input-firstName`);
                         firstNameInput.value = patientData.firstName;
                         firstNameInput.dispatchEvent(new Event('input', { bubbles: true }));
                         firstNameInput.dispatchEvent(new Event('change', { bubbles: true }));
                         firstNameInput.dispatchEvent(new Event('blur', { bubbles: true }));
                         console.log(`✅ Prénom rempli: ${patientData.firstName}`);
                     } else {
                         console.warn('⚠️ Champ #input-firstName non trouvé');
                     }
                 }

          // La date de naissance sera remplie manuellement par l'utilisateur
          console.log('📅 Date de naissance: à remplir manuellement par l\'utilisateur');

                 // 3. Attendre que l'utilisateur clique manuellement sur "Ajouter"
                 console.log('⏳ Étape 3: Attente du clic manuel sur "Ajouter" par l\'utilisateur...');
                 console.log('👤 Veuillez vérifier les informations et cliquer sur "Ajouter" quand vous êtes prêt');

                 // Attendre que l'utilisateur clique sur "Ajouter" puis cliquer automatiquement sur "Confirmer"
                 await new Promise((resolve) => {
                     const checkInterval = setInterval(() => {
                         // Vérifier si le modal est toujours ouvert
                         const modal = document.querySelector('#wrapper > app-modals-injector > div > div > app-modal');
                         const modalContainer = document.querySelector('#wrapper > app-modals-injector > div > div');

                         // Chercher le bouton "Confirmer" qui apparaît après le clic sur "Ajouter"
                         const confirmBtn = document.querySelector('#wrapper > app-modals-injector > div > div > app-modal > div.modal-container > div > div.modal__footer > div > div:nth-child(2) > amds-button.modal-submit-btn.ng-star-inserted.hydrated > button');

                         // Si le bouton "Confirmer" existe, cliquer dessus automatiquement
                         if (confirmBtn && confirmBtn.textContent.trim() === 'Confirmer') {
                             console.log('✅ Bouton "Confirmer" trouvé, clic automatique...');

                             try {
                                 confirmBtn.click();
                                 console.log('✅ Clic automatique sur "Confirmer" effectué');

                                 // Attendre un peu puis vérifier que le modal est fermé
                                 setTimeout(() => {
                                     clearInterval(checkInterval);
                                     console.log('✅ Modal fermé après confirmation');
                                     resolve();
                                 }, 1500); // Augmenté à 1.5s pour plus de stabilité
                             } catch (error) {
                                 console.error('❌ Erreur lors du clic sur "Confirmer":', error);
                                 clearInterval(checkInterval);
                                 resolve();
                             }
                             return;
                         ...
                         // Le modal est fermé si :
                         // 1. Le modal n'existe plus
                         // 2. Le modal est caché (display: none)
                         // 3. Le conteneur modal n'existe plus
                         // 4. Le conteneur modal est caché
                         if (!modal ||
                             ...
                             clearInterval(checkInterval);
                             console.log('✅ Utilisateur a cliqué sur "Ajouter" - Modal fermé détecté');
                             resolve();
                         }
                     }, 500);
                     ...
                     setTimeout(() => {
                         clearInterval(checkInterval);
                         console.log('⏰ Timeout: Attente trop longue, on continue...');
                         resolve();
                     }, 60000);
                 });

          // 4. Attendre 500ms puis cliquer sur "Ajouter fichier"
          console.log('⏳ Attente de 500ms avant de chercher le bouton "Ajouter fichier"...');
          await wait(500);

                 console.log('📁 Étape 4: Clic sur le bouton Ajouter fichier');

                 // Essayer plusieurs sélecteurs pour le bouton "Ajouter fichier"
                 let addFileBtn = null;

                 // Sélecteurs possibles pour le bouton "Ajouter fichier"
                 const selectors = [
                     ...
                     'button[class*="file"]',
                     'button[class*="dossier"]'
                 ];

                 for (const selector of selectors) {
                     addFileBtn = document.querySelector(selector);
                     if (addFileBtn) {
                 ...
                 if (!addFileBtn) {
                     ...
                     for (const btn of allButtons) {
                         ...
                         if (text.includes('Ajouter') && (text.includes('dossier') || text.includes('fichier') || text.includes('file'))) {
                             addFileBtn = btn;
                 ...
                 if (!addFileBtn) {
                     console.warn('⚠️ Bouton "Ajouter fichier" non trouvé, on attend encore 500ms...');

                     // Attendre encore 500ms et réessayer
                     await wait(500);

                     // Réessayer avec les mêmes sélecteurs
                     for (const selector of selectors) {
                         addFileBtn = document.querySelector(selector);
                         if (addFileBtn) {
                             console.log(`✅ Bouton trouvé au 2ème essai avec sélecteur: ${selector}`);
                             break;
                         }
                     }

                     if (!addFileBtn) {
                         console.error('❌ Bouton "Ajouter fichier" toujours non trouvé après 2 tentatives');
                         throw new Error('Bouton "Ajouter fichier" non trouvé après 2 tentatives');
                     }
                 }
          ...
          console.log('⏳ Étape 5: Attente du changement d\'URL...');
                 ...
                 const urlCheckInterval = setInterval(() => {
                     if (window.location.href !== currentUrl && window.location.href.includes('/file/')) {
                         ...
                         console.log(`✅ Nouvelle URL détectée: ${window.location.href}`);

                         // Attendre 1 seconde puis déclencher l'import topographie
                         setTimeout(async () => {
                             console.log('🚀 Déclenchement de l\'import topographie...');

                             // Protection contre les doublons
                             if (window.DesktopImportModule && window.DesktopImportModule.isImporting) {
                                 console.log('⚠️ Import déjà en cours, on évite le doublon');
                                 return;
                             }
                             ...
                             try {
                                 console.log('🚀 Déclenchement de l\'import direct...');
                 ...
                 await new Promise((resolve) => {
                     ...
                     setTimeout(() => {
                         clearInterval(urlCheckInterval);
                         console.log('✅ Processus d\'auto-création terminé');
                         resolve();
                     }, 30000);
                 });
    ...
    async triggerDirectImport(group) {
        console.log('🎯 Déclenchement import direct pour groupe:', group);
        ...
        try {
            ...
            console.log(`👁️ Import ciblé pour l'œil: ${targetEye}`);
            ...
            console.log(`🚀 Lancement de l'import avec performRealImport pour ${targetEye}`);
  ...
  function injectRefractionDuplicateButton() {
    try {
      ...
      console.log('🔧 Injection bouton sur entête Réfraction OD');
      ...
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        console.log('🔁 Clic bouton duplication OD→OG');
      ...
      console.log('✅ Bouton de duplication injecté sur OD');
        ...
        const obs = new MutationObserver(() => {
          setTimeout(() => {
            ...
            if (missing && !missing.querySelector('.cf-dup-refraction-btn')) {
              console.log('🔄 Réinjection: entête OD manquante');
              injectRefractionDuplicateButton();
            }
          }, 500);
        });
        obs.observe(document.body, { childList: true, subtree: true });
        window.__cfDupRefractionObserver = obs;
      }
    } catch (e) {
      console.warn('Injection bouton duplication réfraction échouée:', e);
    }
  }
    ...
    function protectFocus() {
      ...
      HTMLElement.prototype.blur = function() {
        try {
          ...
          console.warn('⚠️ Erreur dans override blur:', error);
          return originalBlur.apply(this, arguments);
        }
      };
      ...
      document.addEventListener('blur', function(e) {
        try {
          ...
          if (target && target === currentFocusedElement &&
            ...
            setTimeout(() => {
              ...
              if (newFocus && newFocus !== target &&
                ...
                console.log('🖱️ Défocus volontaire détecté - désactivation protection focus');
          ...
          console.warn('⚠️ Erreur dans gestionnaire blur:', error);
        }
      }, true); // Capture phase
      ...
      document.addEventListener('click', function(e) {
        try {
          ...
          if (shouldMaintainFocus && currentFocusedElement &&
            ...
            console.log('🖱️ Clic sur élément non-interactif - désactivation protection focus');
            shouldMaintainFocus = false;
            currentFocusedElement = null;
          }
        } catch (error) {
          console.warn('⚠️ Erreur dans gestionnaire clic non-interactif:', error);
        }
      }, false);
      ...
      document.addEventListener('click', function(e) {
        try {
          ...
          if (target.tagName === 'BUTTON' ||
            ...
            console.log('🖱️ Clic bouton détecté - désactivation temporaire protection focus');
            shouldMaintainFocus = false;

            setTimeout(() => {
              try {
                console.log('⏰ Délai écoulé - protection focus reste désactivée');
              } catch (error) {
                console.warn('⚠️ Erreur lors de la réactivation focus:', error);
              }
            }, 2000);
          }
        } catch (error) {
          console.warn('⚠️ Erreur dans gestionnaire clic bouton:', error);
        }
      }, false);

      // Observer les modals qui s'ouvrent pour désactiver complètement la protection
      const modalObserver = new MutationObserver((mutations) => {
        try {
          mutations.forEach((mutation) => {
            mutation.addedNodes.forEach((node) => {
              if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                if (node.classList?.contains('modal') ||
                  ...
                  console.log('🚪 Modal détecté - désactivation complète protection focus');
                  shouldMaintainFocus = false;
                  ...
                  console.log('🚪 Modal fermé - protection focus désactivée définitivement');
                }
              }
            });
          });
        } catch (error) {
          console.warn('⚠️ Erreur dans observer modals:', error);
        }
      });
      ...
      document.addEventListener('keydown', (e) => {
        ...
        // Gérer aussi Shift+Tab (navigation arrière)
          ...
          if (currentIndex !== -1) {
            ...
            if (prevElement) {
              console.log(`Navigation arrière vers: ${fieldsOrder[prevIndex]}`);
              setTimeout(() => {
                prevElement.focus();
                prevElement.select();
              }, isAutoSaving ? 100 : 0);
...
function launch() {
  ...
  setTimeout(() => {
    console.log('Tentative d\'ajout des boutons de calcul...');
    addCalculationButtonsToHeader();

    // Retry après 3 secondes si les boutons ne sont pas encore là
    setTimeout(() => {
      if (!document.querySelector('.calc-buttons-container') && window.location.href.includes('/file/')) {
        console.log('Retry ajout boutons de calcul...');
        addCalculationButtonsToHeader();
      }
    }, 3000);
  }, 1000);
