...
(() => {
  ...
  // 正则表达式匹配专辑链接格式
  const REG_ALBUM = /^https:\/\/music\.amazon\.(?:com\.[a-z]{2,3}|co\.[a-z]{2}|com|[a-z]{2,3})\/albums\/B0[A-Z0-9]{8}$/;

  // 正则表达式匹配专辑页面路径（用于检测当前是否在专辑页面）
  const REG_ALBUM_PAGE = /^https:\/\/music\.amazon\.(?:com\.[a-z]{2,3}|co\.[a-z]{2}|com|[a-z]{2,3})(?:\/[a-z]{2})?\/albums\//;
