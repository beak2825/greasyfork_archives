...
// @grant        all
// @downloadURL https://update.greasyfork.org/scripts/391106/Mid%20Bloom%20-%20Profit%2035.user.js
// @updateURL https://update.greasyfork.org/scripts/391106/Mid%20Bloom%20-%20Profit%2035.meta.js
// ==/UserScript==
...
function calcularRotinas()
{
	...
	$("tr.header").append("<th class='LB' title='' style='width: 40px; cursor: pointer;''>MB</th>");
	...
	$("div.player_name > a").each(function(e, player)
	{
		...
		xhr.onreadystatechange = function()
		{
			if (xhr.readyState == 4)
			{
                ...
                function mid_bloom_func(info,TI) {
                    var ASI = info[1]
                    var AGE = info[3]
                    var BID_Full = info[4]
                    ...
                    if ((age[0] > 18 && age[0] < 23) && (TI > 14) && (REC == 7) && (age[1] < 5) && (Bid < 42)) {
                      return "⚔⚔⚔";
                    }
                    else {
                       return "----";
                ...
                var mid_bloom = mid_bloom_func(arrary[id_index(resultado.player.player_id,arrary)],TI);
                ...
                $("a[player_link=" + resultado.player.player_id + "]").closest("tr").append("<td class='right align_right MB' style='padding-right:5px;'><span>" + mid_bloom   +"</span></td>");
