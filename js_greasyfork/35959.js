...
// @name        豆瓣电影资源搜索
// @namespace   *
// @description 在豆瓣电影详情页，根据标题获取磁链接并自动离线下载
// @match       https://movie.douban.com/subject/*
// @comment     Modified based on 挊-随时开车 https://greasyfork.org/zh-CN/scripts/26779-%E6%8C%8A-%E9%9A%8F%E6%97%B6%E5%BC%80%E8%BD%A6
...
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/35959/%E8%B1%86%E7%93%A3%E7%94%B5%E5%BD%B1%E8%B5%84%E6%BA%90%E6%90%9C%E7%B4%A2.user.js
// @updateURL https://update.greasyfork.org/scripts/35959/%E8%B1%86%E7%93%A3%E7%94%B5%E5%BD%B1%E8%B5%84%E6%BA%90%E6%90%9C%E7%B4%A2.meta.js
// ==/UserScript==

let douban_movie_resource = {
    start: function () {
        this.add_css();
        this.get_moviename();
        this.add_btn();
    ...
    get_moviename: function(){
        ...
        var movie_name = t.substring(0,t.length-4);
        return movie_name;
    },
    add_btn: function () {
        this.selection = this.get_moviename();
        console.log(this.selection);
        var search_btn_html = '<li>';
        search_btn_html += '<div class="search-btn">';
        search_btn_html += '<a class="search" href="javascript:void(0)" onclick="">';
        search_btn_html += '<i></i>';
        search_btn_html += '搜索资源';
        search_btn_html += '</a>';
        search_btn_html += '</div>';
        search_btn_html += '</li>';
        var ul_tag = $("div.gtleft>ul");
        if(ul_tag){
            ul_tag.append(search_btn_html);
        }

        sbtn = document.getElementsByClassName("search")[0];
...
let select_popup = function () {
    ...
    if (enable) {
        ...
        GM_registerMenuCommand("豆瓣电影资源搜索-关闭选中时弹出图标", function () {
            GM_setValue("enable_select_popup", false);        });
    } else {
        GM_registerMenuCommand("豆瓣电影资源搜索-开启选中时弹出图标", function () {
            GM_setValue("enable_select_popup", true);
        });
    }
};
GM_registerMenuCommand("豆瓣电影资源搜索-标题长度", set_max_title_length);

select_popup();
run();

if (window === window.top) {
    douban_movie_resource.start();
}