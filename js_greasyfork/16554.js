// ==UserScript==
// @name        Reddit Secure Delete - Only Comments More Than 7 Days Old 
// @namespace   Reddit
// @include     https://*.reddit.com/*
// @include     http://*.reddit.com/*
// @version     1.10
// @description Securely delete your reddit posts. Now with the ability to only securely delete comments more than 7 days old. Originally by https://greasyfork.org/en/users/5868-mrazavi
// @downloadURL https://update.greasyfork.org/scripts/16554/Reddit%20Secure%20Delete%20-%20Only%20Comments%20More%20Than%207%20Days%20Old.user.js
// @updateURL https://update.greasyfork.org/scripts/16554/Reddit%20Secure%20Delete%20-%20Only%20Comments%20More%20Than%207%20Days%20Old.meta.js
// ==/UserScript==
...
unsafeWindow.span = '';
unsafeWindow.user = '';
unsafeWindow.days_to_go_back = 7;
...
unsafeWindow.delete_all = function()
{
 try{
	...
	for (var i = 0; i < comments.length; i++)
	{
        ...
        //If first word in there is "submitted" then this is a reddit submission/post, not a user comment
        var postMetaData = comments[i].parentNode.innerHTML.toLowerCase();
        var firstWord = postMetaData.substr(0, postMetaData.indexOf(" "));
        if (firstWord == "submitted") continue;
        
        //Check if it is more than X number of days ago before deleting it
        var main_parent = comments[i].parentNode.parentNode;
        var timestamp = main_parent.querySelector("time").innerHTML;
        if (timestamp.toLowerCase().indexOf("day") == -1) continue; 
        var number_of_days_ago = timestamp.substr(0, timestamp.indexOf(" "));
        if (number_of_days_ago <= unsafeWindow.days_to_go_back) continue;
    ...
    if (unsafeWindow.num_user_comments > 0) 
    {
    ...
    {
        alert("There were no comments older than " + unsafeWindow.days_to_go_back + " day(s) old.");
    }
 } catch(e){
...
function UpdateDeleteAllSpan()
{
	if (unsafeWindow.num_user_comments){
		...
		dlink.setAttribute('href', 'javascript:void(0)');
		dlink.appendChild(document.createTextNode('SECURE DELETE visible comment(s) on this page that are over ' + unsafeWindow.days_to_go_back + ' day(s) old.'));
...
unsafeWindow.addEventListener("DOMContentLoaded",add_delete_links, false);