...
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/459590/Rutracker%20autosort%20by%20downloads%2C%20seeds.user.js
// @updateURL https://update.greasyfork.org/scripts/459590/Rutracker%20autosort%20by%20downloads%2C%20seeds.meta.js
// ==/UserScript==
...
function sortBySeeds(){
    const topics = Array.from(document.querySelectorAll('.hl-tr'))
    const newTopics = Array.from(topics)
    for(let i = 0; i < newTopics.length; i++){
        const peersBlock = newTopics[i].querySelector('.vf-col-tor.tCenter.nowrap .seedmed b')
        if(peersBlock){
...
function sortByDonwloads(){
    ...
    const newTopics = Array.from(topics)
    for(let i = 0; i < newTopics.length; i++){
        const downloadsBlock = newTopics[i].querySelector('.vf-col-replies.tCenter.small.nowrap .med b')
        if(downloadsBlock){
