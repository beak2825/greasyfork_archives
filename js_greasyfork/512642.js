...
// @updateURL
// @downloadURL https://update.greasyfork.org/scripts/512642/Own%20script.user.js
// @updateURL https://update.greasyfork.org/scripts/512642/Own%20script.meta.js
// ==/UserScript==
...
(function () {
  ...
  const buttons = [
        ...
        },
      {
          ...
          content:
              `${START_DECOR}Здравствуйте, {{ user.mention }}.<br><br>` +
              "Ваша тема передана на рассмотрение команде проекта.<br><br>" +
              `Ожидайте ответа.${END_DECOR}`,
          prefix: CP_PREFIX,
          status: true,
        },
        {
          ...
          content:
              `${START_DECOR}Здравствуйте, {{ user.mention }}.<br><br>` +
              "После проверки доказательств было принято решение, что наказание выдано верно.<br><br>" +
              `Закрыто.${END_DECOR}`,
          prefix: CLOSE_PREFIX,
          status: false,
        },
        ...
        },
      {
          ...
          content:
              `${START_DECOR}Здравствуйте, {{ user.mention }}.<br><br>` +
              "Если вы не согласны с наказанием, его можно было оспорить в разделе жалоб на администрацию в течение 48-ми часов.<br><br>" +
              `Закрыто.${END_DECOR}`,
          prefix: CLOSE_PREFIX,
          status: false,
        },
      {
        ...
        },
        {
          ...
          content:
              `${START_DECOR}Здравствуйте, {{ user.mention }}.<br><br>` +
              "Пересоздайте тему в разделе жалоб на технических специалистов → *[URL='https://forum.blackrussia.online/forums/%D0%A2%D0%B5%D1%85%D0%BD%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B9-%D1%80%D0%B0%D0%B7%D0%B4%D0%B5%D0%BB.22/']Кликабельно[/URL]*<br><br>" +
          `Закрыто.${END_DECOR}`,
          prefix: CLOSE_PREFIX,
          status: false,
        },
        ...
        },
      {
          ...
          content:
              `${START_DECOR}Здравствуйте, {{ user.mention }}.<br><br>` +
              "Ваше обжалование одобрено, наказание будет полностью снято. Ждем вас на посту администратора сервера TOLYATTI<br><br>" +
              `Одобрено.${END_DECOR}`,
          prefix: OKAY_PREFIX,
          status: false,
        },
  ...
  $(document).ready(() => {
    ...
    // addButton('Pin', 'ras', 'fa-thumbtack', COLOR);
    addButton('', 'deny', 'fa-times', COLOR);
    addButton('', 'lvlup', 'fa-level-up');
    addButton('', 'time', 'fa-clock');
