...
// @downloadURL https://update.greasyfork.org/scripts/551999/%E7%A7%8D%E5%AD%90%E4%B8%8B%E8%BD%BD%E5%B7%A5%E5%85%B7-%28%E5%85%BC%E5%AE%B9trae%E6%B5%8F%E8%A7%88%E5%99%A8%29.user.js
// @updateURL https://update.greasyfork.org/scripts/551999/%E7%A7%8D%E5%AD%90%E4%B8%8B%E8%BD%BD%E5%B7%A5%E5%85%B7-%28%E5%85%BC%E5%AE%B9trae%E6%B5%8F%E8%A7%88%E5%99%A8%29.meta.js
// ==/UserScript==
...
(function () {
  ...
  const siteStrategies = {
    new_mteam: {
      ...
      getDownloadButtonMountPoint: () => {
        if (document.querySelector(".mt-4.app-content__inner")) {
          return document
            .querySelector(
              "button.ant-btn.ant-btn-link.ant-btn-sm.ant-dropdown-trigger"
            )
            ?.closest("td");
        }
    ...
    mteam: {
      getTorrentUrl: () => {
        return document
          .evaluate("//a[text()='[IPv4+https]']", document)
          .iterateNext().href;
      },
    },
    ptlsp: {
      getTorrentUrl: () =>
        document.querySelector(`#download_pkey`).getAttribute(`href`),
    },
    tjupt: {
      getTorrentUrl: () =>
        document.querySelector(`#direct_link`).getAttribute(`href`),
    },
    springsunday: {
      getTorrentTitle: () => document.querySelector(`#torrent-name`).innerText,
      getDownloadButtonMountPoint: () =>
        document.querySelector('a[title="下载种子"]').closest("td"),
    },
    hhanclub: {
      getTorrentTitle: () => {
        return document
          .evaluate("//div[text()='标题']", document)
          .iterateNext()
      ...
      getTorrentSubTitle: () =>
        document.evaluate("//div[text()='副标题']", document).iterateNext()
          ?.nextElementSibling.innerText,
      getDownloadButtonMountPoint: () =>
        document.querySelector(".flex.gap-x-5"),
    },
    hdsky: {
      getTorrentName: () => {
        let str = document
          .evaluate("//td[text()='下载']", document)
          .iterateNext()
      ...
      getDownloadButtonMountPoint: () =>
        document.querySelector("#outer .dt_download")?.closest("td"),
    },
    hdhome: {
      getTorrentUrl: () => {
        const linkElement = document.querySelector(
          'td.rowfollow a[href*="/download.php?id="]'
        );
    ...
    totheglory: {
      getTorrentName: () =>
        document
          .querySelector("td.rowhead")
          .nextElementSibling.querySelector("a")
          .textContent.replace("[TTG]", ""),
      getTorrentTitle: () => document.querySelector("h1").textContent,
      getTorrentSubTitle: () => "",
      getTorrentUrl: () =>
        document.querySelector("td[valign='top'] a").getAttribute("href"),
      getDownloadButtonMountPoint: () =>
        document
          .querySelector('a[href^="https://totheglory.im/dl/"]')
          .closest("td"),
    },
    hddolby: {
      getTorrentUrl: () => {
        const currentDomain = window.location.origin;
        const relativeUrl = [...document.querySelectorAll("a.faqlink")]
          .find((element) => element.textContent.includes("右键复制种子链接"))
          .getAttribute("href");
    ...
    defaultStrategy: {
      getTorrentUrl: () => {
        ...
        let firstMatchingLink = Array.from(allLinks).find(function (link) {
          return (
            /download.php\?id=[0-9]+&passkey=.+$/.test(link.href) ||
      ...
      getTorrentName: () => {
        let str = document
          .querySelector("#outer td.rowfollow > a.index")
          .innerText.trim();
        console.log("原始种子名:", str);
        return /\.(.+)\./.exec(str)[1];
      },
      getTorrentSubTitle: () => {
        let subTitleTd =
          document.evaluate("//td[text()='副標題']", document).iterateNext() ||
          document.evaluate("//td[text()='副标题']", document).iterateNext() ||
          document
            .evaluate("//td[text()='Small Description']", document)
            .iterateNext();
        return subTitleTd.nextElementSibling.innerText;
      },
      getDownloadButtonMountPoint: () =>
        document.querySelector("#outer img.dt_download")?.closest("td"),
    },
  };
  ...
  function execMethodName(methodName) {
    try {
      let strategy =
        (getSite() && siteStrategies[getSite()]) ||
        siteStrategies.defaultStrategy;
      let execMethodName =
        strategy[methodName] || siteStrategies.defaultStrategy[methodName];
      let flag =
        getSite() &&
        siteStrategies[getSite()] &&
        siteStrategies[getSite()][methodName]
          ? getSite()
          : "defaultStrategy";
  ...
  const PT = {
    ...
    getTorrentTitle: () => {
      let v = replaceUnsupportedCharacters(
        execMethodName("getTorrentTitle")
      ).trim();
    ...
    getTorrentSubTitle: () => {
      let v = replaceUnsupportedCharacters(
        execMethodName("getTorrentSubTitle")
      ).trim();
      console.log("副标题: ", v);
      return v;
    },
  ...
  let qbittorrent = (function () {
          ...
          GM_xmlhttpRequest({
            ...
            onload: function (response) {
              ...
              if (data && data.length === 1) {
                ...
                if (!oldFilePath.startsWith(config.separator))
                  oldFilePath = config.separator + oldFilePath;
            ...
            onload: function (response) {
              ...
              dataArr.forEach((info) => {
                if (info.name === newTorrentName) {
                  ...
                  // 下载目录下面第一级
                  let oldFilePath = info.content_path.replace(
                    info.save_path,
                    ""
                  );
    ...
    function renameFileOrFolder(hash, oldPath, newPath) {
      return new Promise((resolve, reject) => {
        ...
        const endpoint = isFolder(oldPath)
    ...
    function addTorrent(rename, savePath, torrentUrl) {
      return new Promise((resolve, reject) => {
        ...
        // let downloadMsg = cocoMessage.loading("下载中！", 10000, true);
        const url = `${config.address}/api/v2/torrents/add`;
        console.log('url:', url)
        GM_xmlhttpRequest({
          method: "POST",
          headers: {
        ...
        request(
          ...
          async function (response) {
            ...
            if (response.status === 409) {
              // X-Transmission-Session-Id 失效
              sessionId = response.responseHeaders.match(
                /X-Transmission-Session-Id:\s*(\S+)/i
              )[1];
          ...
          .then((base64Torrent) => {
            request(
              ...
              function (response) {
                ...
                let torrent = duplicate
                  ? data.arguments["torrent-duplicate"]
                  : data.arguments["torrent-added"];
                resolve({
                  message: "添加种子成功.",
                  id: torrent.id,
    ...
    let app = new Vue({
      ...
      data: {
        ...
        config: {
          ...
          saveLocations: GM_getValue("saveLocations", [
            { label: "默认", value: "" },
          ]), // 下载目录 默认 savePath 兼容老版本
      ...
      methods: {
        ...
        download(inputValue) {
          ...
          // 原来文件是单文件 当前文件名未加后缀
          if (!isFolderFlag && isFolder(inputValue))
            inputValue += getSuffix(torrentName);
          ...
          if (byteCount > 255) {
            ...
            cocoMessage.error(
              `字节数超过255，一个中文占用3字节，当前字节数：${byteCount}`,
              10000,
              true
            );
            return;
          ...
          if (
            this.config.saveLocations.length === 0 ||
            this.selectedLabel >= this.config.saveLocations.length
          ) {
            cocoMessage.error(
              "必须选择下载位置，如果没有下载位置请点击脚本图标进行配置",
              10000,
              true
            );
            return;
          ...
          Client[config.client].download(
            inputValue,
            savePath,
            hash,
            torrentUrl,
            this.config.autoCloseWindow
          );
        },
        addLine() {
        ...
        startDragging(e) {
          ...
          if (
            e.target === this.$el.querySelector("#popup") ||
            e.target === this.$el.querySelector("#download-title")
          ) {
            // 只有在鼠标在popup上时才允许拖动,外圈
