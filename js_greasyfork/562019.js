...
// @downloadURL https://update.greasyfork.org/scripts/562019/OGLight%20Ninja%20%28CellMaster%27s%20Patcher%29.user.js
// @updateURL https://update.greasyfork.org/scripts/562019/OGLight%20Ninja%20%28CellMaster%27s%20Patcher%29.meta.js
// ==/UserScript==
...
class OGLight
{
    ...
    init(cache)
    {
        ...
        this.ptreKey = localStorage.getItem(localStoragePrefix+'ogl-ptreTK') || false;
...
class DomManager extends Manager
{
    ...
    loadBase(reload)
    {
        ...
        if(LFComponent) Util.addDom('a', { child:'genetics', href:`?page=ingame&component=lfbonuses`, class:'ogl_lfBonusesLink tooltip', title:'Lifeforms Bonuses', parent:LFComponent });
    }
    ...
    updateTop()
    {
        ...
        if(wreckfield)
        {
            wreckfield.setAttribute('href', `?page=ingame&component=facilities&openTech=36`);
        ...
        Object.values(this.ogl._dom.planet).forEach(planet =>
        {
            ...
            this.ogl._dom.redraw(div, fragment, () =>
            {
                if(!planet.classList.contains('tooltipLeft'))
                {
                    ...
                    {
                        const icon = Util.addDom('a', { class:'wreckFieldIcon tooltip js_hideTipOnMobile ogl_active ogl_wreckfieldIcon', parent:planet.parentNode, href:`?page=ingame&component=facilities&cp=${id}&openTech=36`, child:'<span class="icon icon_wreck_field"></span>' });
...
class FetchManager extends Manager
{
    ...
    async fetchEmpire(type, callback, ignoreDelay)
    {
        ...
        return fetch(`?page=ajax&component=empire&ajax=1&planetType=${type}&asJson=1`,
        {
    ...
    async fetchLFBonuses(callback, ignoreDelay)
    {
        ...
        return fetch(`?page=ajax&component=lfbonuses`,
        {
            headers: { 'X-Requested-With': 'XMLHttpRequest' },
            signal: controller.signal
        })
    ...
    async fetchProductionQueue(callback, ignoreDelay)
    {
        ...
        return fetch(`?page=ajax&component=productionqueue&ajax=1`,
        {
            headers: { 'X-Requested-With': 'XMLHttpRequest' },
            signal: controller.signal
        })
...
class UIManager extends Manager
{
                ...
                || event.target.closest('.planetlink, .moonlink'))
                {
                    if(document.body.classList.contains('ogl_destinationPicker'))
                    {
                        ...
                        if(document.body.classList.contains('ogl_initHarvest'))
                        {
                            ...
                            window.location.href = `?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${type}&oglmode=1&ogldestinationid=${id}&oglfirstsourceid=${firstSourceId}&ogldestinationtype=${type == 3 ? 'moon' : 'planet'}`;
                        }
                        else
                        {
    ...
    getPlayerTooltip(id)
    {
        ...
        const container = Util.addDom('div',
        {
            ...
            child:`
            <h1 class="${status}">${player.name} <a href="${window.location.protocol}//${window.location.host}${window.location.pathname}?page=highscore&site=${page}&searchRelId=${id}">#${player.score.globalRanking}</a></h1>
            <div class="ogl_grid">
        ...
        // buddy
        Util.addDom('a', { child:'person_add', class:'material-icons ogl_button overlay', parent:container.querySelector('.ogl_actions'), href:`?page=ingame&component=buddies&action=7&id=${player.uid}&ajax=1`, onclick:() =>
        {
            tippy.hideAll();
        }});
...
class TopbarManager extends Manager
{
    ...
    openSettings(buttonSource)
    {
        ...
        options.forEach(opt =>
        {
            ...
            else if(opt == 'ptreTeamKey')
            {
                ...
                const input = Util.addDom('input', { type:'password', placeholder:'TM-XXXX-XXXX-XXXX-XXXX', value:localStorage.getItem(localStoragePrefix+'ogl-ptreTK') || '', parent:label,
                oninput:() =>
                {
                    ...
                    {
                        input.classList.remove('ogl_danger');
                        localStorage.setItem(localStoragePrefix+'ogl-ptreTK', input.value);
    ...
    openPinnedDetail(id, update)
    {
        ...
        const updateSide = () =>
        {
            ...
            // buddy
            Util.addDom('a', { child:'person_add', class:'material-icons ogl_button overlay', parent:container.querySelector('.ogl_actions'), href:`?page=ingame&component=buddies&action=7&id=${id}&ajax=1`, onclick:() =>
            {
                tippy.hideAll();
            }});
...
class MessageManager extends Manager
{
    ...
    loadSpyTable(isOutdated, forceClean)
    {
        ...
        list.forEach(message =>
        {
            ...
            const name =  Util.addDom('a', { child:message.playerName, parent:nameContainer, class:'overlay', href:`${window.location.protocol}//${window.location.host}${window.location.pathname}?page=componentOnly&component=messagedetails&messageId=${message.id}` });
            const pin = Util.addDom('div', { parent:nameContainer });
            const loot = Util.addDom('a', { child:Util.formatToUnits(lootValue), parent:content, class:'ogl_textRight ogl_loot', href:`?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${message.targetTypeID}&mission=1&am${this.ogl.db.options.defaultShip}=${requiredShips}&oglmode=4&oglmsg=${message.id}` });
        ...
        if(id == 36)
        {
            ...
            if(wreckfieldButton)
            {
                const checkWreckfield = tries =>
                {
                    ...
                    fetch(`?page=ajax&component=repairlayer&asJson=1`,
                    {
                        headers: { 'X-Requested-With': 'XMLHttpRequest' }
                    })
                    .then(response =>
