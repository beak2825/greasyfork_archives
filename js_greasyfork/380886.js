...
// @downloadURL https://update.greasyfork.org/scripts/380886/HWM_WH_Helper.user.js
// @updateURL https://update.greasyfork.org/scripts/380886/HWM_WH_Helper.meta.js
// ==/UserScript==
...
for(var i = 0; i < buttons.length; i++){
    buttons[i].onclick = function(e){
        ...
        var blimit = form.parentNode.querySelector('select[name="blimit"]');
        var bcost = form.parentNode.querySelector('input[name="bcost"]');
        var al = form.parentNode.querySelector('select[name="al"]');
        var saved = JSON.parse(GM_getValue('saved' + artId, '{}'));
        saved.blimit = blimit.value;
        saved.bcost = bcost.value;
        saved.al = al.value;
...
if(document.location.href.indexOf('cat=4') == -1){
    var cell = document.querySelector('td[align="left"][colspan="5"]');
