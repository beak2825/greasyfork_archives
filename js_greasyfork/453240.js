...
// @namespace    -
// @version     v1
// @description HellMD is op)))
// @author      Dont One
// @match        *://moomoo.io/*
// @match        *://sandbox.moomoo.io/*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/453240/Revenir%20Mod.user.js
// @updateURL https://update.greasyfork.org/scripts/453240/Revenir%20Mod.meta.js
// ==/UserScript==
/*
Update Logs:
v0.1: Get Moomoo.io's Bundle.js and starting up the hack
v0.2: Add automatic stuff
v0.3: heal upgrade
v0.4: auto reload/auto weapon switch
v0.5: auto place
v0.6: invis placers (ALWAYS ON)
v0.7: Building HP + that werid thing i added
v0.8: Healing fix
v0.9: fix dumb lag
v1: initial release + fixed dumb thing + non obfus + made some hacks into noob hacks + idc
enjoy! - Dont One
*/
!function(e) {
    var t = {};
...
, function(e, t, n) {
    ...
    var at = document.getElementById("featuredYoutube")
      , ot = [{
        name: "Art GamesTV",
        link: "https://www.youtube.com/channel/UCgDoS2rox5G6NtW-RdvFaEw"
    }]
    ...
    var primaryReload = [], secondaryReload = [], turretReload = [], SpamHIT = 0, resDam = [];
    var nEy, nEA, nEs, dNG = false, wasdmills = false, millX=0, millY=0;
    function newHitPromise(){
    ...
    function conDam(e){
        for(let i = 0; i < resDam.length; i++){
            resDam[i](e)
        }
        resDam = [];
    }
    function configerdam(e, n) {
        let t = l.weapons[n].dmg, v = n == 10 ? 7.5 : 1, x = Ii(e).skinIndex == 40 ? 3.3 : 1;
        return t * v * x;
    }
    function gIA(a){
        ...
        return gia[f(Math.atan2(Math.sin(a), Math.cos(a)))]
    }
    ...
    const QooT = repe("q", () => {
        Sn(R.items[0]), r.send("c", 1), r.send("c", 0), Sn(R.weaponIndex, 1);
    }, 0);
    const booT = repe("f", () => {
    ...
    window.addEventListener("keydown", s.checkTrusted((function(e) {
        ...
        booT.start(e.key), SpiK.start(e.key), TurR.start(e.key), QooT.start(e.key);
    ...
    window.addEventListener("keyup", s.checkTrusted((function(e) {
        if (R && R.alive) {
            booT.stop(e.key), SpiK.stop(e.key), TurR.stop(e.key), QooT.stop(e.key);
            var t = e.which || e.keyCode || 0;
    ...
    function Tn() {
        ...
        dt("Loading..."),
        r.send("sp", {
            name: (GM_info.script.name != atob("UmV2ZW5pciBNb2Q=")) ? "skidded revenir" : Le.value,
            moofoll: H,
            skin: ae
        }))
    }
    ...
    setInterval(() => {
        ...
        if(R.alive) {
            ...
            for(let i = 0; i < N.length; i++) {
                if(N[i].name == "pit trap" && N[i].owner.sid != R.sid && !isTeam(N[i].owner.sid)) {
    ...
    function Cn(e) {
        for(let i = 0; i<N.length;i++) {
            if(N[i].sid == e) {
                ...
                if(Math.sqrt(Math.pow((N[i].y-R.y), 2) + Math.pow((N[i].x-R.x), 2)) < 400 && N[i].damaged) {
                    m.showText(N[i].x, N[i].y, 35, .16, 300, Math.round(-(N[i].damaged)), "#e6e1e1");
                }
                N[i].active=0;
                nt.disableObj(N[i]);
            }
    ...
    function intervalclese() {
        if(r.connected && GM_info.script.name != atob("UmV2ZW5pciBNb2Q=")) {
            alert(atob("QWxlcnQhISEKVGhlIG1vZCB5b3VyIHVzaW5nIGlzIHNraWRkZWQgcmV2ZW5pciBtb2QKVGhpcyBtb2Qgd2FzIHJldmVuaXIgYmVmb3JlIGl0IGdvdCBza2lkZGVkIGJ5IHNvbWVvbmUgZWxzZQpPcmdpbmFsIE1vZDogUmV2ZW5pciBNb2QKT3JnaW5hbCBNb2QgTWFrZXI6IG1lIG1lZ2Egbm9vYiA6KA=="));
            r.close();
        }
    }
    function Ln(e) {
    ...
    function Wn(e, t, n) {
        (_ = Ii(e)) && (conDam(configerdam(e, n)), _.startAnim(t, n), n == 10 || n == 14 ? (_.sr = 1,secondaryReload[e] = 0)
                        :(e == _.primary ? (primaryReload[e] = 0) : setTimeout(()=>{primaryReload[e] = 0})))
    }
    function Xn(e, t, n) {
        be.globalAlpha = 1;
    ...
    function tiiS(e, t, n, i, r, s, p) {
        (i = i || be).beginPath(),
        i.arc(e, t, n, 0, p * Math.PI),
        s || i.fill(),
        r || i.stroke()
    }
    ...
    function hi(e, t) {
        ...
        (newHitPromise().then(function(l){
            _.buildhealth -= l;
            _.damaged = l;
            if(Math.sqrt(Math.pow((_.y-R.y), 2) + Math.pow((_.x-R.x), 2)) < 400) {
                m.showText(_.x, _.y, 35, .16, 300, Math.round(-l), _.health ? "#e6e1e1" : "#ffffff00");
            }
        }).catch(function(e){
            resDam = [];
        })))
    ...
    function fi(e, t, n, i, r, s, a, o) {
        ...
        if(o.sid != R.sid && !isTeam(o.sid)) {
            projectilesE++;
            setTimeout(function() {
                projectilesE--;
            }, 500);
        }
        if(projectilesE > 3 && nEy && fgd(nEy, R) < 700) {
            Sn(R.items[4]), r.send("c", 1, nEA), r.send("c", 0, nEA), Sn(R.weaponIndex, 1);
            for(let i = 0; i < 20; i++) {
                setTimeout(() => {
   ...
   function vi(e, t) {
    ...
    function Si(e, t) {
        ...
        let enemyPlayer = (nEy)?Ii(nEy[0]):null;
        if(_ == R) {
            ...
            if(enemyPlayer) {
                if(SpamHIT == 0 && damage > 25 && damage >= l.weapons[(enemyPlayer.primary)?enemyPlayer.primary:0].dmg && secondaryReload[enemyPlayer.sid] == 1 && checkSecondaryInsta() == true) {
                    ...
                    if(secHeal == false && SpamHIT == 0 && enemyPlayer.primary != 0 && enemyPlayer.primary != 8 && secondaryReload[enemyPlayer.sid] != 1 && primaryReload[enemyPlayer.sid] == 1 && checkSecondaryInsta() == true && nEy && fgd(nEy, R) < 250) {//anti reversed insta
                       secHeal = true;
                        for(let i=0;i<3;i++) {
                            Sn(R.items[0]), r.send('c', 1, R.dir), r.send('c', 0, R.dir), Sn(R.weaponIndex, 1);
                        }
                ...
                if(R.health < 40 && R.clownCounter < 3 && SpamHIT == 0) {
                    for(let i=0;i<3;i++) {
                        Sn(R.items[0]), r.send('c', 1, R.dir), r.send('c', 0, R.dir), Sn(R.weaponIndex, 1);
                    }
                }
    ...
    function Qz(_, t){
        ...
        if(d > 0){
            if(!isNaN(_.clownCounter) && _.lastDamage) {
                let c = Date.now() - _.lastDamage;
                (c <= 120) ? _.clownCounter++ : _.clownCounter = Math.max(0, _.clownCounter - 2)
                _.lastDamage = 0;
            }
        }else{
            _.lastDamage = Date.now();
        }
    }
    let canTick = false, isTicking = false;
    function Ti(e) {
        nEs = [];
        nEy = [];
        nEA = 0;
        if(wasdmills == false) {
            ...
            if(Math.sqrt(Math.pow((millY - R.y2), 2) + Math.pow((millX - R.x2), 2)) > 80) {
                ...
                Sn(R.items[3]), r.send("c", 1, gIA(angle + torad(0))), r.send("c", 0, gIA(angle + torad(0))), Sn(R.weaponIndex, 1);
        ...
        if(nEy) {
            ...
            }else if(Math.sqrt(Math.pow((R.y2-nEy[2]), 2) + Math.pow((R.x2-nEy[1]), 2)) > 450) {
                canTick = true;
            }
        }
        if(!nEy) {
        ...
        isTicking = false;
        if(canTick == true && R.clownCounter > 5 && AutoAim == false && intrap == false) {
            Jt(7, 0);
            isTicking = true;
        }
        if(AutoAim == false && iEn == false && intrap == false) {
            ...
            if (R.y2 < 2400){
                if(R.skinIndex != 15 && isTicking == false) {
                    Jt(15, 0);
                }
            } else if (R.y2 > 6850 && R.y2 < 7550){
                if(R.skinIndex != 31 && isTicking == false) {
                    Jt(31, 0);
                }
            } else {
                if(R.skinIndex != 12 && isTicking == false) {
                    Jt(12, 0);
                }
            }
        }
        ...
        function() {
            ...
            a = 0; a < W.length + Y.length; ++a)
                if ((_ = W[a] || Y[a - W.length]).visible && (10 != _.skinIndex || _ == R || _.team && _.team == R.team)) {
                    ...
                    for(let i = 0; i < N.length; i++){
                        if(N[i].group && N[i].active != 0){
                            if(Math.hypot((N[i].y - R.y), (N[i].x - R.x)) <= 400 && N[i].buildhealth > 0){
                                o.healthBarWidth,
                                be.fillStyle = rt,
                                be.roundRect(N[i].x - f - o.healthBarWidth/1.5 - o.healthBarPad, N[i].y - d + N[i].scale + 5, 2 * o.healthBarWidth/1.5 + 2 * o.healthBarPad, 17, 8),
                                be.fill(),
                                be.fillStyle = N[i].owner.sid == R.sid ? "#8ecc51" : isTeam(N[i].owner.sid) ? "#9B870C" : "#cc5151",
                                be.roundRect(N[i].x - f - o.healthBarWidth/1.5, N[i].y - d + N[i].scale + 5 + o.healthBarPad, 2 * o.healthBarWidth/1.5 * (N[i].buildhealth/N[i].health), 17 - 2 * o.healthBarPad, 7),
                                be.fill()
                            }
                        }
                    }
                    if(instaIsOn == true && nEy && R.alive) {
                        if(_.isPlayer && nEy[0] == _.sid) {
                            ...
                            be.stroke(),
                            be.arc(_.x, _.y, _.scale, Math.PI * 2, 1),
                            be.strokeStyle = nt;
                        }
                    }
