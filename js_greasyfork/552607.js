// ==UserScript==
// @name         TM - Buscador de Jogadores da Liga + NT
// @version      3.6 - Adicionada marcação de jogadores da NT Shortlist
// @description  Na página da liga, busca jogadores com R5, com filtros e marca jogadores da lista de observação NT.
// @author       douglaskraft-club/970949/ (modificado por Gemini)
...
// @downloadURL https://update.greasyfork.org/scripts/552607/TM%20-%20Buscador%20de%20Jogadores%20da%20Liga%20%2B%20NT.user.js
// @updateURL https://update.greasyfork.org/scripts/552607/TM%20-%20Buscador%20de%20Jogadores%20da%20Liga%20%2B%20NT.meta.js
// ==/UserScript==
...
(function() {
    ...
    // --- PARTE 1: FUNCIONALIDADE NA PÁGINA DA NT SHORTLIST ---
    if (window.location.href.includes('nt-shortlist')) {
        const btn = document.createElement('button');
        btn.textContent = '💾 Salvar Lista de Observação';
        btn.style.cssText = "position: fixed; top: 10px; left: 10px; z-index: 9999; padding: 6px; background: #28a745; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;";
        btn.addEventListener('click', () => {
            ...
            rows.forEach(row => {
                ...
                if (link) {
                    ...
                    if (match) listaNT.push(match[1]); // Salva apenas o ID do jogador
                }
            });

            GM_setValue('listaNT', JSON.stringify(listaNT));
            alert(`✅ Lista de Observação salva com ${listaNT.length} jogadores!`);
        });
        document.body.appendChild(btn);
        return; // Impede que o resto do script (buscador) rode nesta página
    }


    // --- PARTE 2: FUNCIONALIDADE NA PÁGINA DA LIGA (BUSCADOR) ---
    ...
    async function findHighR5Players() {
        ...
        // Carrega a lista de observação salva
        const listaNT = JSON.parse(GM_getValue('listaNT', '[]'));

        // Captura dos valores dos filtros
        const angolanOnly = $('#r5-angolan-only-filter').is(':checked');
        const minAge = parseInt($('#r5-min-age-filter').val(), 10);
        const maxAge = parseInt($('#r5-max-age-filter').val(), 10);
        let minR5 = parseInt($('#r5-value-filter').val(), 10);
        if (isNaN(minR5)) {
            minR5 = 100;
        }
            ...
            try {
                ...
                if (playersData) {
                    for (const playerId in playersData) {
                        ...
                        if (angolanOnly && playerInfo.player_country.toLowerCase() !== 'ao') {
                            continue;
                        }
                        const playerAge = parseInt(playerInfo.age, 10);
                        if (!isNaN(minAge) && playerAge < minAge) {
                            continue;
                        }
                        if (!isNaN(maxAge) && playerAge > maxAge) {
                            continue;
                        }
                        ...
                        if (max_r5 > minR5) {
                            // Verifica se o jogador está na lista NT
                            ...
                            foundPlayers.push({
                                ...
                                isNT: isNT // Adiciona a informação ao objeto do jogador
                            });
                        }
                    }
                }
        ...
        displayResults(foundPlayers, minR5);
        findButton.prop('disabled', false).text('Buscar Jogadores');
    }

    function displayResults(players, minR5) {
        let html = `
            <div class="r5-results-header">
                <h2>Jogadores Encontrados com R5 > ${minR5}</h2>
                <button id="r5-close-btn" title="Fechar">X</button>
            </div>`;

        if (players.length === 0) {
            html += `<p style="padding: 15px;">Nenhum jogador encontrado que satisfaça os critérios selecionados.</p>`;
        } else {
            ...
            players.forEach(p => {
                ...
                // Prepara o nome e o estilo para jogadores NT
                let playerName = p.playerName;
                let playerStyle = '';
                if (p.isNT) {
                    playerName = `[NT] ${p.playerName}`;
                    playerStyle = 'style="color: #00FF00; font-weight: bold;"';
                }

                html += `
                    <tr>
                        <td><a href="/club/${p.clubId}/" target="_blank">${p.clubName}</a></td>
                        <td><a href="/players/${p.playerId}/#/" target="_blank" ${playerStyle}>${playerName}</a></td>
    ...
    GM_addStyle(`
        #r5-controls-container { position: fixed; bottom: 20px; right: 20px; z-index: 9998; background-color: rgba(42, 42, 42, 0.9); border: 1px solid #444; border-radius: 5px; padding: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.2); display: flex; flex-direction: column; gap: 10px; }
        .r5-filter-group { display: flex; flex-direction: column; gap: 5px; color: #ccc; }
        .r5-filter-group label { font-size: 12px; font-weight: bold; }
        .r5-filter-group input[type="checkbox"] { margin-right: 5px; vertical-align: middle; }
        .r5-filter-group .age-inputs { display: flex; gap: 5px; }
        .r5-filter-group input[type="number"] { width: 60px; background-color: #333; border: 1px solid #555; color: #fff; border-radius: 3px; padding: 2px 5px; }
        #r5-finder-btn { padding: 10px 15px; font-size: 14px; font-weight: bold; color: white; background-color: #28a745; border: none; border-radius: 5px; cursor: pointer; }
    ...
    function setupUI() {
        const controlsContainer = $(`
            <div id="r5-controls-container">
                <div class="r5-filter-group">
                    <label>
                        <input type="checkbox" id="r5-angolan-only-filter">
                        Apenas Angolanos
                    </label>
                </div>
                <div class="r5-filter-group">
                    <label>Filtrar Idade:</label>
                    <div class="age-inputs">
                        <input type="number" id="r5-min-age-filter" placeholder="Mín">
                        <input type="number" id="r5-max-age-filter" placeholder="Máx">
                    </div>
                </div>
                <div class="r5-filter-group">
                    <label for="r5-value-filter">R5 Mínimo:</label>
                    <input type="number" id="r5-value-filter" value="100" placeholder="Ex: 100">
                </div>
                <button id="r5-finder-btn">Buscar Jogadores</button>
            </div>
        `);
        const resultsContainer = $('<div id="r5-results-container"></div>');
        $('body').append(controlsContainer).append(resultsContainer);
        $('#r5-finder-btn').on('click', findHighR5Players);
    }

    $(document).ready(function() {
        // Roda o setup do buscador apenas se não estiver na página da shortlist
        if (!window.location.href.includes('nt-shortlist')) {
            setupUI();
        }
    });
