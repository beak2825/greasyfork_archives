...
function listenForRequests() {
    ...
    XMLHttpRequest.prototype.open = function (method, url) {
        console.log("访问的 ,method : " +  method + "  : URL ==" + url )
        // 找出请求数据的接口
        //https://admin.scrmchampion.com/counter-api/detail/user-detail/get_share_list
        //https://admin.scrmchampion.com/shared-order/3c66a4e4bddf1b9f267144215ba4297b90298095e52d6890b603368a25be560c?u=cab225b52884473aa1700c03de26df13&code=GXhJAkTp
    ...
    XMLHttpRequest.prototype.send = function (data) {
        ...
        if (window.location.hostname.includes("haixiang")) {
            console.log('post haixiang 数据--- ' + data);
...
function getDataAndSendToMonkey() {
    ...
    if (window.location.hostname.includes("haixiang") || window.location.href.includes("007.mn/work-order-sharing") || window.location.href.includes("imx.chat/work-order-sharing")
        ...
        if (window.location.href.includes("007.mn/work-order-sharing") || window.location.href.includes("imx.chat/work-order-sharing")
            || window.location.href.includes("yf.lu/work-order-sharing") ) {
            //{"page":1,"per_page":10,"workId":"1701124542770438145"}
            //将per_page的值改为500
        ...
        GM_xmlhttpRequest({
            ...
            url: data_url,
            data: requestMethod === "POST" ? post_data : undefined,
            headers: {
                "Content-Type": requestMethod === "POST" ? "application/json;charset=UTF-8" : "",
                "Uuid": uuid
            },
            onload: function (response) {
                ...
                if (window.location.href.includes("007.mn/work-order-sharing") || window.location.href.includes("imx.chat/work-order-sharing")
                     || window.location.href.includes("yf.lu/work-order-sharing") ) {
                    console.log('海象 007.mn imx.chat yf.lu 33333 scrmchampion get007MNPostData')
                    let aa = get007MNPostData(response)
                    dataStr = JSON.stringify(aa)
                } else if (window.location.href.includes("haixiang.app")) {
                    console.log('haixiang.app开始处理')
                    let aa = getHaiXiangPostData(response)
                    dataStr = JSON.stringify(aa)
                }else if( window.location.href.includes("scrmchampion.com/shared-order")){
                    console.log('scrmchampion007开始处理')
