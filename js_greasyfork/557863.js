...
// @downloadURL https://update.greasyfork.org/scripts/557863/%E7%BD%91%E9%A1%B5%E8%B0%83%E8%AF%95%20with%20CookieCopy.user.js
// @updateURL https://update.greasyfork.org/scripts/557863/%E7%BD%91%E9%A1%B5%E8%B0%83%E8%AF%95%20with%20CookieCopy.meta.js
// ==/UserScript==
...
`;
    ...
    const fetchAndCopyCookies = async (format) => {
        // 步骤 1: 检查 GM_cookie 权限
        if (typeof GM_cookie === 'undefined' || typeof GM_cookie.list !== 'function') {
            Qmsg.error("GM_cookie API 不可用。请确认在脚本头部添加了 @grant GM_cookie 权限，且脚本管理器支持该 API。");
            return;
        }
        ...
        const queryDomain = rootDomain || hostname;

        // 步骤 2: 使用 GM_cookie.list 获取所有 Cookies（包括 HttpOnly）
        try {
            // 将查询参数设置为目标域名
            const cookies = await new Promise((resolve, reject) => {
                GM_cookie.list({ domain: queryDomain }, (cookies, error) => {
                    if (error) {
                        reject(new Error(Array.isArray(error) ? error.join(', ') : (error.message || JSON.stringify(error))));
                    } else {
                        // 对数据进行预处理：将时间戳转换为可读格式
                        const processedCookies = cookies.map(c => {
                            const item = { ...c };

                            // 转换时间戳为 ISO 字符串，如果是 Session Cookie 则不设置 expires
                            if (item.expirationDate && !item.session) {
                                item.expires = new Date(item.expirationDate * 1000).toISOString();
                            } else {
                                item.expires = ""; // 会话 Cookie 留空
                            }

                            //   delete item.storeId;
                            //   delete item.expirationDate; // 删除原始的时间戳
            ...
            if (!cookies || cookies.length === 0) {
                Qmsg.warning(`在域名 ${queryDomain} 下未找到任何 Cookies。`);
                return;
            }
            ...
            if (format === 'raw') {
                // 原始字符串格式
                copiedContent = cookies.map(c => `${c.name}=${c.value}`).join('; ');
            } else if (format === 'json') {
                // JSON 对象格式
                copiedContent = JSON.stringify(cookies, null, 2);
            } else if (format === 'tsv') {
                // TSV 表格格式逻辑
                const headers = [
                    "name", "value", "domain", "path", "Expires/MaxAge",
                    "Size", "HttpOnly", "Secure", "SameSite",
                    "Partition Key", "Site", "Cross Site", "Priority"
                ];

                const tsvRows = cookies.map(c => {
                    const fields = [];
                    ...
                    fields.push(c.value || "");
                    // 3. domain
                    fields.push(c.domain || "");
                    // 4. path
                    fields.push(c.path || "");
                    // 5. Expires/MaxAge (使用预处理的 expires 字段)
                    fields.push(c.expires || "");
                    // 6. Size (API 不提供，留空)
                    fields.push("");
                    // 7. HttpOnly (打勾标记)
                    fields.push(c.httpOnly ? "✓" : "");
                    // 8. Secure (打勾标记)
                    fields.push(c.secure ? "✓" : "");
                    // 9. SameSite (如果不是 'unspecified' 则使用)
                    fields.push(c.sameSite && c.sameSite !== 'unspecified' ? c.sameSite : "");
                    // 10-13. Partition Key, Site, Cross Site, Priority (API 不提供，留空)
                    fields.push(""); // Partition Key
                    fields.push(""); // Site
                    fields.push(""); // Cross Site
                    fields.push(""); // Priority

                    return fields.join('\t'); // 使用制表符分隔
                });

                // 组合表头和内容，使用换行符分隔
                copiedContent = [headers.join('\t'), ...tsvRows].join('\n');
            ...
            // 步骤 3: 复制内容
            utils.copy(copiedContent);
            Qmsg.success(`所有 Cookies (${format.toUpperCase()} 格式, 域名: ${queryDomain}) 已复制到剪贴板！`);

        } catch (e) {
            Qmsg.error(`复制 Cookies 时发生错误: ${e.message}`);
        }
    };
        ...
        views: [
            {
                ...
                type: "container",
                views: [
                    // 按钮 1: 原始字符串格式（包含 HttpOnly）
                    UIButton(
                        "复制所有 Cookies (Raw)", // 左侧标题
                        "以 key=value; 格式复制所有 Cookie (包括 HttpOnly)", // 描述
                        "一键复制", // 按钮文字
                        void 0,
                        false,
                        false,
                        "info", // 蓝色
                        (event) => {
                            Qmsg.info("正在获取并复制原始字符串格式 Cookies...", 2000);
                            fetchAndCopyCookies('raw');
                        }
                    ),
                    // 按钮 2: JSON 对象格式（包含 HttpOnly）
                    UIButton(
                        "复制所有 Cookies (JSON)", // 左侧标题
                        "以 JSON 格式复制所有 Cookie (包括 HttpOnly)", // 描述
                        "一键复制", // 按钮文字
                        void 0,
                        false,
                        false,
                        "info", // 绿色
                        (event) => {
                            Qmsg.info("正在获取并复制 JSON 格式 Cookies...", 2000);
                            fetchAndCopyCookies('json');
                        }
                    ),
                    // 按钮 3: TSV 表格格式（包含 HttpOnly 和元数据）
                    UIButton(
                        "复制 Cookies (TSV 表格)", // 左侧标题
                        "以制表符分隔的表格格式复制所有 Cookie 和元数据", // 描述
                        "一键复制", // 按钮文字
                        void 0,
                        false,
                        false,
                        "info", // 橙色
                        (event) => {
                            Qmsg.info("正在获取并复制 TSV 表格格式 Cookies...", 2000);
                            fetchAndCopyCookies('tsv'); // 触发新的 TSV 格式
                        }
                    ),
                    UISelect(
                        "调试工具",
