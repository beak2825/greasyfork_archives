...
// @version      1.6.0
// @description  Trajets simple
// @author       vok2022 (discord)
// @license      Voko
...
// @grant        GM_setValue
// @grant        GM_getValue
// @downloadURL https://update.greasyfork.org/scripts/491204/OWL%20Retn.user.js
// @updateURL https://update.greasyfork.org/scripts/491204/OWL%20Retn.meta.js
// ==/UserScript==
...
function onMapClick() {
    ...
    if ($('.fa-eraser.toggled').length > 0) {
        ...
        for (let key in bankData) {
            console.log(key + ', map : ' + bankData[key].map + ', Up : ' + bankData[key].top.path + ', Down : ' + bankData[key].bottom.path + ', Right : ' + bankData[key].right.path + ', Left : ' + bankData[key].left.path + ', havenbag : ' + bankData[key].havenbag.path);
        }
        ...
        for (let key in phenixData) {
            console.log(key + ', map : ' + phenixData[key].map + ', Up : ' + phenixData[key].top.path + ', Down : ' + phenixData[key].bottom.path + ', Right : ' + phenixData[key].right.path + ', Left : ' + phenixData[key].left.path + ', havenbag : ' + phenixData[key].havenbag.path);
        }
    } else {
        if (!checkMaps(actualMap, pathData)) {
            if ($(".fa-suitcase.toggled").length > 0) {
        ...
        for (let key in pathData) {
            console.log(key + ', map : ' + pathData[key].map + ', Up : ' + pathData[key].top.path + ', Down : ' + pathData[key].bottom.path + ', Right : ' + pathData[key].right.path + ', Left : ' + pathData[key].left.path + ', havenbag : ' + pathData[key].havenbag.path + ', gather : ' + pathData[key].gather + ', fight : ' + pathData[key].fight);
        }
    }
}
    ...
    if (index !== undefined) {
        if (array.length !== 0) {
            if (array[index].havenbag.path)
                    // && !array[index].bank)
                path = "                    await Map.GoToHavenBagAsync();";
            else {
                if (array[index].top.path)
                    path = "                    await Map.ChangeToAsync(ChangeMapSide.Up);"; //Directions
...
function gatherFigh(index, array) {
    ...
    if (array[index].gather)
        path = "                    await Map.GatherAsync([" + ($('#ressources-selecter').val() ? $('#ressources-selecter').val() : "") + "]);\r\n";
    if (array[index].fight)
        path += "                    await Map.FightAsync(new FightOptionsBuilder().WithMinMonsters(" + $('#min-monster').val() + ").WithMaxMonsters(" + $('#max-monster').val() + ").Build());";
    return path;
}
...
$('#path-creator-bar').on('click', '.fa-save', function () {
    var firstIf = false;
    var txt = "// Generated On Dofus-Map with Vok Retn Script Maker and OWL --\r\n" +
    ...
    "public class ScriptGenerated(IScript script)\r\n" +
    "{\r\n" +
    ...
    if (pathLinearData.length !== 0) {
        ...
        '------------------\r\n' +
        '\r\n' +
        '   [Handler]\r\npublic Task OnLoadedAsync()\r\n   {\r\n    Bank.SetWeightLimit(80);\r\n    Common.Log("Script chargé !");\r\n' +
        ...
        if (havenbagCheck('move')) {
            ...
            for (let index = 0; index < havenBagData.length; index++) {
                if (havenBagData[index].move) {
                    if (!firstIf) {
                        txt += "\t\tif actualMap == \"" + havenBagData[index].map + "\" then\r\n\t\t\tmap:changeMap(\"test3(" + havenBagData[index].mapidToGo + ")\")\r\n";
                        firstIf = true;
                    } else
                    txt += "\t\telseif actualMap == \"" + havenBagData[index].map + "\" then\r\n\t\t\tmap:changeMap(\"test4(" + havenBagData[index].mapidToGo + ")\")\r\n";
                }
            }
            txt += '\t\tend\r\n' +
            '\telse\r\n' +
        ...
        for (let key in pathLinearData) {
            ...
            txt += '\t[(' + pathLinearData[key].map.replace(' ', '') + ')] =\r\n      [\r\n      asaync () =>\r\n      {\r\n' + gatherFigh(key, pathLinearData) + '\r\nawait Map.ChangeToAsync(ChangeMapSide.' + getLinearAction(1, key) + '"' + (pathLinearData[key].index >= 2 ? ', "' + getLinearAction(2, key) + '"' : '') + (pathLinearData[key].index >= 3 ? ', "' + getLinearAction(3, key) + '"' : '') + (pathLinearData[key].index >= 4 ? ', "' + getLinearAction(4, key) + '"' : '') + ');\r\n       }\r\n       ],\r\n';
        }
        for (let key in pathData) {
            txt += '\t[(' + pathData[key].map.replace(" ", "") + ')] =\r\n[\r\nasaync () =>\r\n      {      \r\n' + gatherFigh(key, pathData) + '\r\nawait Map.ChangeToAsync(ChangeMapSide.' + pathGenerator(key, pathData) +');\r\n }\r\n ],\r\n\r\n';
        }
        txt += '}\r\n' +
        '\r\n' +
        'function processMap()\r\n' +
        ...
        if (havenbagCheck('move')) {
            txt += '            [(0, 0)] =\r\n            [\r\n';
            //await _wrapper.UseZaapAsync(' + havenBagData[index].mapidToGo + ');\r\n}\r\n],\r\n';
            for (let index = 0; index < havenBagData.length; index++) {
                if (havenBagData[index].move) {                   
                    txt += "                async() =>\r\n                {\r\n                    await Map.UseZaapAsync(" + havenBagData[index].mapidToGo + ");\r\n                },\r\n";
                    //" + havenBagData[index].mapidToGo + ")\")\r\n";
                }
            } 
            txt += '            ],\r\n';          
        }
    ...
    if (havenbagCheck('bank')) {
        ...
        for (let index = 0; index < havenBagData.length; index++) {
            if (havenBagData[index].bank) {
                if (!firstIf) {
                    txt += "\t\tif actualMap == \"" + havenBagData[index].map + "\" then\r\n\t\t\tmap:changeMap(\"test(" + havenBagData[index].mapidToGo + ")\")\r\n";
                    firstIf = true;
                } else
                txt += "\t\telseif actualMap == \"" + havenBagData[index].map + "\" then\r\n\t\t\tmap:changeMap(\"test2(" + havenBagData[index].mapidToGo + ")\")\r\n";
            }
        }
        txt += "\t\tend\r\n" +
        "\telse\r\n" +
    ...
    // }
    txt +='\t\tif (Map.Id == ' +  $('#banque-map').val() + ')\r\n        {\r\n' +
        '\t\t    await Npc.SpeakToAsync(520);\r\n            return; \r\n        }\r\n' +
        '\t\tawait Map.AutoTripAsync(' +  $('#banque-map').val() + ');\r\n    }\r\n\r\n' +
        //Fin banque
    ...
    '        else if (question.MessageId == 30638)\r\n        {\r\n' +
    '           await Npc.AnswerAsync(question.Replies[0]);\r\n        }\r\n' +
    '        else if (question.MessageId == 30637)\r\n        {\r\n' +
    '           await Npc.AnswerAsync(question.Replies[0]);\r\n        }\r\n' +
    '    }\r\n\r\n' +
    ...
    '        await Fight.SetBonusAsync(ChallengeBonusEnum.ChallengeDropBonus); // ChallengeExperienceBonus\r\n' +
    '        await Fight.SetPositionAsync(FightPosition.Nearest); // Fuite Furthest\r\n    }\r\n\r\n';

    // IA
    txt +='    [Handler]\r\n    public async Task OnFightTurnAsync()\r\n    {\r\n' +
    '    if (Character.Breed == PlayableBreedEnum.Cra)\r\n        {\r\n' +
    '        await Fight.ExecuteAsync([\r\n' +
    '        new FightActionMove(FightMove.Nearest), // Déplacement\r\n' +
    '        new FightActionCast(13047, CastTarget.Enemy), // Optique 3\r\n' +
    '        new FightActionCast(13047, CastTarget.Enemy), // Optique 3\r\n' +
    '        new FightActionFinishTurn() // Passe\r\n' +
    '        ]);\r\n' +
    '    }\r\n' +
    '   if (Character.Breed == PlayableBreedEnum.Ecaflip)\r\n' +
    '    {\r\n' +
    '        await Fight.ExecuteAsync([\r\n' +
    '        new FightActionMove(FightMove.Nearest), // Déplacement\r\n' +
    ...
    '        ]);\r\n' +
    '    }\r\n' +
    '   if (Character.Breed == PlayableBreedEnum.Sacrieur)\r\n' +
    '    {\r\n' +
    '        await Fight.ExecuteAsync([\r\n' +
    '        new FightActionMove(FightMove.Nearest), // Déplacement\r\n' +
    '        new FightActionCast(12748, CastTarget.Enemy), // Hémorragie\r\n' +
    '        new FightActionCast(12748, CastTarget.Enemy), // Hémorragie\r\n' +
    '        new FightActionFinishTurn() // Passe\r\n' +
    '        ]);\r\n' +
    '    }\r\n' +
    '   if (Character.Breed == PlayableBreedEnum.Feca)\r\n' +
    '    {\r\n' +
    '        await Fight.ExecuteAsync([\r\n' +
    '        new FightActionMove(FightMove.Nearest), // Déplacement\r\n' +
    '        new FightActionCast(12978, CastTarget.Enemy), // Attaque naturelle\r\n' +
    '        new FightActionCast(12978, CastTarget.Enemy), // Attaque naturelle\r\n' +
    '        new FightActionFinishTurn() // Passe\r\n' +
    '        ]);\r\n' +
    '     }\r\n' +
...
function saveTextAsFile(txt) {
    ...
    var fileNameToSaveAs = '[' + ($('#type-field').val() ? $('#type-field').val() : "") + '][' + $('#area-field').val() + ']' + $('#file-name-field').val() + '.cs';
            ...
            for (let index = 0; index < result.length; index++) {
                ...
                if (/(-|)[0-9]+?,\s(-|)[0-9]+?/g.exec(actualMap)) {
                    let loadAddLinearAction = function (param) {
                        ...
                        if (matche !== null) {
                            if (matche[1] == "Up") $(".fa-arrow-up").addClass('toggled');
                            if (matche[1] == "Left") $(".fa-arrow-left").addClass('toggled');
                            if (matche[1] == "Down") $(".fa-arrow-down").addClass('toggled');
