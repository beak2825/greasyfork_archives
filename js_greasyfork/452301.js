...
// @version         2.4.4-edited2
// @date            2023-06-06
// @downloadURL https://update.greasyfork.org/scripts/452301/Gazelle%20Snatched.user.js
// @updateURL https://update.greasyfork.org/scripts/452301/Gazelle%20Snatched.meta.js
// ==/UserScript==
...
(function () {
	...
	var releaseTypes = ['Album', 'EP', 'Soundtrack', 'Compilation', 'Remix', 'Anthology', 'DJ Mix', 'Single',
						'Live album', 'Mixtape', 'Unknown', 'Bootleg', 'Interview', 'Demo'];
	...
	function scan_torrent_page(doc, type) {
		...
		torrent_table.find('div.group_info').each(function(i) {
			...
			var m = link.attr('href').match(/torrents\.php\?(?:id|groupId)=(\d+)&torrentid=(\d+)/);
			if (m) {
				group_id = m[1];
				torrent_id = m[2];
			} else {
				/* I don't know if we can ever get here! */
				console.log('Not sure how Gazelle Snatched got here. Please notify Mordred with what you were doing');
			...
			if (group_id && torrent_id) {
				...
				if (!d.torrents[torrent_id] ||
					...
					var nm = reg[1].trim();
					if (domain_abbr == 'o'){
						//We want to start after span.torrent_links_block and end after the last link (to torrents.php)
						var firsttext = $(this).contents().index($(this).contents('span.torrent_links_block'))+1
						var lastLink = $(this).contents().index($(this).contents('a:last'))+1
						//Extract and format the text in the selected range
						nm = $(this).contents().slice(firsttext, lastLink).text().trim().replace('–', '-')
					}
	...
	function mark_snatched_links() {
		...
		$('#content a').each(function(i) {
			...
			if (href) {
				...
				if (domain_abbr == 'o' && /\/artist\.php\?id/.test(document.URL) && this.innerText == 'PL') {
           return;
        }
		...
		if (/\/torrents\.php\?id/.test(document.URL)) {
			...
			if (($('a.add_bookmark').length || $('a.remove_bookmark').length || $('span.add_bookmark').length || $('span.remove_bookmark').length) &&
				...
				mark_snatched.on('click', function () {
					...
					if (g.groups[group_id]) {
						...
						if (domain_abbr == 'o'){
							var lastLink = $('#content .header h2').contents().index($('#content .header h2').contents('a:last'))+1
							key = $('#content .header h2').contents().slice(0, lastLink).text().trim().replace('–', '-').toLowerCase()
						}
		...
		if (/\/artist\.php\?id/.test(document.URL)) {
			...
			$('#content a[title="View torrent group"]').each(function () {
		...
		if (/\/top10\.php/.test(document.URL) || /\/torrents\.php\?action=notify/.test(document.URL)) {
			$('td div.group_info > strong').each(function () {
				if (domain_abbr == 'o'){
					var lastLink = $(this).contents().index($(this).contents('a:last'))+1
					key = $(this).contents().slice(0, lastLink).text().trim().replace('–', '-').toLowerCase()
				}
				else{
					var key = this.innerText.replace(releaseTypeRegex,'').replace(/\[\d*\]/, '').trim().toLowerCase();
				}
				if (snatched_groups[key]) {
					addGroupSnatched(key, snatched_groups[key], $(this).contents().filter('a').slice(-1)[0]);
				}
			});
		}
		...
		if (/\/torrents.php\?id=/.test(document.URL)) {
			if (domain_abbr == 'o'){
				var albumNameHeader = $('#content .header h2 a').slice(-1)[0]
			}
			else{
				var albumNameHeader = $('#content .header h2 span')[0]
			}
			...
			if (keys.length) {
				addGroupSnatched(keys[0], snatched_groups[keys[0]], albumNameHeader);
			} else {
				var header = $('#content .header h2').text();
				...
				if (snatched_groups[key]) {
					addGroupSnatched(key, snatched_groups[key], albumNameHeader);
				}
			}
		}
