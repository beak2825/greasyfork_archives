...
// @downloadURL https://update.greasyfork.org/scripts/473083/%E5%9B%BD%E5%AE%B6%E7%94%B5%E6%8A%95%E7%BD%91%E7%BB%9C%E5%AD%A6%E9%99%A2.user.js
// @updateURL https://update.greasyfork.org/scripts/473083/%E5%9B%BD%E5%AE%B6%E7%94%B5%E6%8A%95%E7%BD%91%E7%BB%9C%E5%AD%A6%E9%99%A2.meta.js
// ==/UserScript==
...
(function() {
    ...
    function saveHome(url){
        GM_setValue('homeURL',url);
    }

    function getHome(){
        let oldUrl = GM_getValue('homeURL');

        if(!oldUrl){
            saveHome('https://e-learning.spicti.com/#/study/course/index');
            return 'https://e-learning.spicti.com/#/study/course/index';
        }
    ...
    const panel = function(){
        ...
        var ddds4 = $('<div style="position: absolute;top: 85%;width:94%;height:13%;padding: 3px;background: #ffedf0;border-radius: 5px;"><button id="clearbtn" style="position: absolute;width:88px;right: 269px;background-color: #ffe5e5;border-radius: 4px;border-color: #ffc0c0;color: grey;">清理缓存</button><a href="http://8.130.116.135/?article/" style="position: absolute;right: 10px;text-decoration: none;color: pink;">\u003e\u003e\u003e\</a></div>');
    ...
    `);
       ...
       $('#clearbtn').on('click',function(){
           ddds3.children().remove();
           addMessage('已清空缓存');
           GM_deleteValue('pageID');
           GM_deleteValue('table');
       });
    ...
    function loginNow(){
        ...
        if (targetElement) {
            ...
            // 创建一个点击事件（通常不是必需的，因为mousedown + mouseup会触发click）
            ...
            // 触发click事件（可选，因为mousedown + mouseup通常已经足够）
            targetElement.dispatchEvent(clickEvent);

            // 触发mouseup事件
            targetElement.dispatchEvent(mouseupEvent);

            console.log('Simulated click on element with ID:', targetElementId);
        } else {
            console.log('Element with ID', targetElementId, 'not found.');
        }
    }
    ...
    function startTimer() {
        wait = setInterval(function (){
            ...
            if(window.location.href === "https://e-learning.spicti.com/#/home"){
            ...
            }else if(window.location.href === 'https://e-learning.spicti.com/#/branch-list-v/dd8fe673-d97a-48f7-b79e-2688300d5049'){
                saveHome(window.location.href);
                clearInterval(wait);
                tipsWin().then((result) => {
                    if(!result){
                        addMessage("新版课程页，无需操作，将自动播放");
                        clearInterval(wait);
                        if(document.querySelector("#D203registerMask") != null){
                            document.querySelector("#D203registerMask").click();
                        }
            ...
            }else if(window.location.href === 'https://e-learning.spicti.com/#/branch-list-v/52be0af0-f82a-4a3a-8289-2af2c4fe6eff'){
                saveHome(window.location.href);
                clearInterval(wait);
                tipsWin().then((result) => {
                    if(!result){
                        addMessage("新版专题页，请选择要学习的专题课程");
                        clearInterval(wait);
                        if(document.querySelector("#D203registerMask") != null){
                            document.querySelector("#D203registerMask").click();
                        }
                ...
                setTimeout(function(){
                    if(document.getElementsByTagName('video')[0] != undefined){
                        document.getElementsByTagName('video')[0].play();
                        searchFC = setInterval(searchVD,9863);
                    }else{
                        moveTXT();
                    }
                },9863);
                ...
                addMessage("登录或前往课程、专题");
                if(window.location.href === 'https://e-learning.spicti.com/#/home-v?id=111893'){
                    document.querySelector("#app-header-wrapper > div > div > div > div.app-header-block.app-header-block-1 > div.app-header-block-1-left > div.app-header-nav > div:nth-child(2) > div > div").click();
                }
            }
        }, 2000)
    }
    ...
    function moveTXT(){
        ...
        function scrollToBottom(container) {
            var startTime = null;
            var scrollDistance = container.scrollHeight - container.scrollTop;

            function step(timestamp) {
                if (!startTime) startTime = timestamp;
                var progress = timestamp - startTime;
                var fraction = Math.min(progress / 60000, 1); // 10秒

                var currentScroll = Math.floor(easeInOutQuad(fraction) * scrollDistance);
                container.scrollTop = currentScroll;

                // 检查是否滚动到底部
                if (container.scrollTop + container.clientHeight >= container.scrollHeight) {
                    // 停止滚动（其实这里不需要显式调用什么函数，因为条件不满足时不会继续调用requestAnimationFrame）
                    console.log('Scrolled to bottom111111111111111111111111111111111111111111111111111');
                } else {
                    // 如果还没到底部，继续动画
        ...
        // 获取目标容器并调用滚动函数
        var container = document.getElementsByClassName("relative fullScreen-content pdf-wrap")[0].children[1];
    ...
    function selectPageNEW(oldPID){
        let nowPID = getNowPidNew();
        if(nowPID < oldPID){
            console.log('nowPID',nowPID);
            let btnCount = document.querySelector("body > div.vue-app > div > div.vue-main-page > div > div > div > div:nth-child(2) > div > div > div.page-main > div.study-list-page.up-down > div.content-area > div > div.e-pagination-box > ul").childElementCount;
            document.querySelector("body > div.vue-app > div > div.vue-main-page > div > div > div > div:nth-child(2) > div > div > div.page-main > div.study-list-page.up-down > div.content-area > div > div.e-pagination-box > ul").children[btnCount-1].click();
            setTimeout(function(){
                selectPageNEW(oldPID)
            },800);
        }
            ...
            setTimeout(function(){
                setTimeout(function(){
                    uL = document.querySelector("body > div.vue-app > div > div.vue-main-page > div > div > div > div:nth-child(2) > div > div > div.page-main > div.study-list-page.up-down > div.content-area > div");
                    courseCount = uL.childElementCount;
                    courseIndex = 0;
                    setTimeout(function(){
                        selectStudyingNew();
    ...
    function getNowPidNew(){
        let nowPIDSTR = document.querySelector("body > div.vue-app > div > div.vue-main-page > div > div > div > div:nth-child(2) > div > div > div.page-main > div.study-list-page.up-down > div.content-area > div > div.e-pagination-box > ul").getElementsByClassName('active themeColor-border-color themeColor-background-color zxy-pagination-item')[0].innerText;
        let nowPID = parseInt(nowPIDSTR);
        return nowPID;
    }
...
//                         }
    ...
    function selectStudyingNew(){
        if(courseIndex < courseCount){

            let uL = document.querySelector("body > div.vue-app > div > div.vue-main-page > div > div > div > div:nth-child(2) > div > div > div.page-main > div.study-list-page.up-down > div.content-area > div");
            let a = uL.children[courseIndex].children[0].children[0].children[0].childElementCount;
            let title = uL.children[courseIndex].getElementsByClassName('content-area')[0].children[0].title;
            let b = document.querySelector("body > div.vue-app > div > div.vue-main-page > div > div > div > div:nth-child(2) > div > div > div.page-main > div.study-list-page.up-down > div.content-area > div").children[0].getElementsByClassName('detail-item')[0].innerText.split(': ')[1];

            if(!isNaN(parseInt(b))){

                if(a != 1){
                    let status = uL.children[courseIndex].getElementsByClassName('status')[0].innerText;
                    addMessage(status+"status");
                    if(status === '已完成'){
                        addMessage(title+"|已完成");
                        courseIndex++;
...
//                         }
                    ...
                    if(imSrc != 'url("images/default-cover/default_course.jpg")'){
                        ...
                        if(getTable() === '全部'){
                            savePID(getNowPidNew());
                        }else{
                            saveTablePID(getTable(),getNowPidNew())
                        }

                        uL.children[courseIndex].children[0].click();
                        setTimeout(function(){
                            window.close();
                        },3000);
                    }else{
