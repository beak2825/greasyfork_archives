   // ==UserScript==
// @name       Hybrid - Categorize races in photos
// @version    2.7
// @author     jawz
...
// @downloadURL https://update.greasyfork.org/scripts/28495/Hybrid%20-%20Categorize%20races%20in%20photos.user.js
// @updateURL https://update.greasyfork.org/scripts/28495/Hybrid%20-%20Categorize%20races%20in%20photos.meta.js
// ==/UserScript==
 
if ($('li:contains("3/27")').length) {
    for (i=0;i<20;i++)
        $('p:contains("Person")').eq(i).find('a').attr('href', "http://www.google.com/search?q=" + $('p:contains("Person")').eq(i).find('a').text());
    ...
    $(document).keyup(function (event) {
        ...
        if (key=='13') {
            ...
            answer(5,5);
        else if (key == 110) //Decimal = No Person
            comboAnswer(0,5);
        else if ([103].indexOf(key) >= 0) //Numpad 7 = One White
            answer(6,7);
