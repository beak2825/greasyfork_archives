...
// @namespace    NightSwan
// @version      0.1
// @description  B站评论区自动标注三相玩家和社死玩家（基于原神指示器和原三相一些小的修改）
// @author       xulaupuz&nightswan&xhskjjmc
...
// @license MIT
// @run-at document-end
// @downloadURL https://update.greasyfork.org/scripts/451276/dys%E9%89%B4%E5%AE%9A%E5%99%A8.user.js
// @updateURL https://update.greasyfork.org/scripts/451276/dys%E9%89%B4%E5%AE%9A%E5%99%A8.meta.js
// ==/UserScript==
...
(function () {
    ...
    const keyword_jn = "七海"
    const keyword_mml = "喵喵露"
    const keyword_dys = "德云色"
    ...
    //标签颜色，可自定义，默认为B站会员色
    const tag_nor_Inner = "<b style='color: #FFD700'>" + tag_nor + "</b>"
    ...
    const tag_dys_Inner = "<b style='color: #11DD77'>" + tag_dys + "</b>"
    ...
    let jiance = setInterval(() => {
        ...
        if (commentlist.length != 0) {
            ...
            commentlist.forEach(c => {
                let pid = get_pid(c)
                if (dys.has(pid)) {
                    if (c.textContent.includes(tag_dys) === false) {
                        c.innerHTML += tag_dys_Inner
                    }
                    return
                } else if (no_dys.has(pid)) {
                    // do nothing
                    return
                }
                if (cj.has(pid)) {
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function (res) {
                        if (res.status === 200) {
                            ...
                            //添加隐藏标签
                            if (st.includes(keyword_cj)&& !st.includes(keyword_dys)) {
                                c.innerHTML += tag_cj_Inner
                                cj.add(pid)
                                return
                            } else {
                            ...
                            //添加纯良标签
                            if (!st.includes(keyword_nong) && !st.includes(keyword_yuan) && !st.includes(keyword_zhou)&& !st.includes(keyword_dys)) {
                                c.innerHTML += tag_nor_Inner
                                nor.add(pid)
                            } else {
                                no_nor.add(pid)
