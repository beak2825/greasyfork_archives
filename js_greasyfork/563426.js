// ==UserScript==
// @name         Veyra Enemy Wave Farming Bot222
...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/563426/Veyra%20Enemy%20Wave%20Farming%20Bot222.user.js
// @updateURL https://update.greasyfork.org/scripts/563426/Veyra%20Enemy%20Wave%20Farming%20Bot222.meta.js
// ==/UserScript==
...
(function() {
    ...
    `;
        ...
        <div id="hud-header" style="text-align: center; margin-bottom: 10px; cursor: move; display: flex; justify-content: space-between; align-items: center;">
            <h3 style="margin: 0; color: #3498db; flex-grow: 1; text-align:center;">🤖 Veyra Farming Bot v3.3.0</h3>
...
<div style="margin-bottom: 8px; display: flex; align-items: center; gap: 8px;">
  ...
  >
    ...
    async function fspConsume() {
        ...
        let potionBtn = document.querySelector(
            '.potion-use-btn[data-name="Large Stamina Potion"]'
        );

        // fallback if not found or empty
        if (!potionBtn || Number(potionBtn.dataset.max) === 0) {
            potionBtn = document.querySelector(
        ...
        // final guard
        if (!potionBtn || Number(potionBtn.dataset.max) === 0) {
            console.warn("[BOT] No usable stamina potion found");
            return;
        }
    ...
    const ABC = "__a4";
    const ABC_REFRESH_MS = 24 * 60 * 60 * 1000;
    function getCurrentUsername() {
        return document.querySelector(".small-user .small-name")
            ?.textContent
            ?.trim() || "";
    }
    function hasValidCache(username) {
        try {
            const raw = localStorage.getItem(ABC);
            if (!raw) return false;

            const data = JSON.parse(raw);
            if (!data.ok) return false;
            if (Date.now() > data.nextCheck) return false;
    ...
    const TAG = PARTYS.split(" ")[1];
    function SUPERSIZEYOU(username) {
        const r = new RegExp(
            "^\\s*\\[[^\\]]*" + TAG + "[^\\]]*\\]",
            "i"
        );
    ...
    function POLICE(username) {
        return fetch(SUPERSIZEME, {
            method: "GET",
            credentials: "include"
        })
            .then(res => {
            ...
            const doc = new DOMParser().parseFromString(html, "text/html");
            const title = doc.querySelector(".panel .hdr h1.title")?.textContent || "";

            const valid =
                  title.includes(PARTYS) &&
                  SUPERSIZEYOU(username);
            ...
            localStorage.setItem(ABC, JSON.stringify({
                ok: true,
                user: username,
                nextCheck: Date.now() + ABC_REFRESH_MS
            }));
    ...
    async function ensure123() {
        const username = getCurrentUsername();
        if (!username) return false;
        if (!SUPERSIZEYOU(username)) return false;
        if (hasValidCache(username)) {
            return true;
        }
        return await POLICE(username);
    }
    async function init() {
        ...
        if (!(await ensure123())) {
            console.warn("[Init] access denied");
            return;
        }
        createHUD();
