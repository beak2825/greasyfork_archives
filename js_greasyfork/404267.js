...
// @match        https://www.easports.com/fifa/ultimate-team/web-app/*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/404267/FUT20%20Autobuyer%20Menu.user.js
// @updateURL https://update.greasyfork.org/scripts/404267/FUT20%20Autobuyer%20Menu.meta.js
// ==/UserScript==
...
(function() {
    ...
    */
    ...
    function createAutoBuyerInterface()
    {
        ...
        if (window.hasLoadedAll && getAppMain().getRootViewController().getPresentedViewController().getCurrentViewController().getCurrentController()._jsClassName) {
            if (!jQuery('.SearchWrapper').length) {
                ...
                jQuery(view.__root.parentElement).append('<div id="SearchWrapper" style="width: 40%; right: 40%"><textarea readonly id="progressAutobuyer" style="font-size: 15px; width: 100%;height: 58%;"></textarea><label>Search Results:</label><br/><textarea readonly id="autoBuyerFoundLog" style="font-size: 10px; width: 100%;height: 26%;"></textarea></div>');
            ...
            if (jQuery('.search-prices').first().length) {
                if (!jQuery('#ab_buy_price').length) {
                    jQuery('.search-prices').first().append(
                        ...
                        '</div>' +
                        '<p1 class="secondary">Minimum profit per purchase: <span id="profit">0</span></br>' +
                        '</br>' +
                        '</p1>' +
                        ...
                        '<div class="price-filter">' +
                        '   <div class="info">' +
                        '       <span class="secondary label" style="color: #434853;">Wait Time:<br/><small style="color: #888c94">(Delay between searches )</small></span>' +
                        '   </div>' +
                        '   <div class="buttonInfo">' +
                        '       <div class="inputBox">' +
                        '           <input type="tel" class="numericInput" id="ab_wait_time" placeholder="3-5">' +
                        '       </div>' +
                        '   </div>' +
                        '</div>' +
                        '<div class="price-filter">' +
                        ...
                        '<div class="price-filter">' +
                        '   <div class="info">' +
                        '       <span class="secondary label" style="color: #000;">Purchases Until Stop<br/><small style="color: #888c94">Leave this empty if you want it to keep buying players</small></span>' +
                        '   </div>' +
                        '   <div class="buttonInfo">' +
                        '       <div class="inputBox">' +
    ...
    window.getRandomWait = function() {
        var searchNumber = 15;
        if (jQuery('#ab_search_number').val() !== '') {
            searchNumber = parseInt(jQuery('#ab_search_number').val());
        }
        var addedTime = 0;
        var abRandomDelay = (parseInt(jQuery('#ab_random_delay').val()) *1000);
        if (window.searchCount % searchNumber === 0) {
            if (!Number.isNaN(abRandomDelay)){
                addedTime = abRandomDelay;
            } else {
                addedTime = 10000;
            }
        }
        if (jQuery('#ab_wait_time').val().includes("-")) {
            var wait = [3, 5];
            ...
            var delay = 3;
            if (jQuery('#ab_wait_time').val() !== '') {
                delay = parseInt(jQuery('#ab_wait_time').val());
            }
            window.searchCount++;
            return ((delay * 1000) + addedTime);
        }
    };
    ...
    window.updateAttempts = function() {
        window.attempts++;
        jQuery('#ab-buy-attempts').html(window.attempts);
    }

    window.updateSuccessfulBuys = function() {
        window.successfulBuys++;
        jQuery('#ab-successful-buys').html(window.successfulBuys);
    }
