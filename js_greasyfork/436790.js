...
// @grant        GM_info
// @license		 lly
// @downloadURL https://update.greasyfork.org/scripts/436790/mserscript.user.js
// @updateURL https://update.greasyfork.org/scripts/436790/mserscript.meta.js
// ==/UserScript==
	...
	function loadData() {
		...
		GM_xmlhttpRequest({
			...
			onload: function (xhr) {
				...
				//加密
				//var desMessage = encryptByDES("PJFPGMQAW04=", "dhdxdhdx");
				//console.log(desMessage);
				//解密
				// var message = decryptByDES("PJFPGMQAW04=", "dhdxdhdx")

				// 解密
				//console.log(CryptoJS.enc.Utf8.parse("dhdxdhdx")) //C033
				//let enres =  encryptByDES("C033","dhdxdhdx","dhdxdhdx")
				//console.log(enres)
				//console.log(toBase641(enres))
				//var str = toBase641(enres);
				//var data = base64toHEX(str);
				//console.log(data)
				//let result  = decryptByDESModeEBC(data,"dhdxdhdx","dhdxdhdx");
				//console.log(result)
				//console.log(message);


				/* 	var ipdata = JSON.parse(xhr.responseText.replace("var returnCitySN = ","").replace(";","").replace(/\s/g,""));
					var cip = ipdata.cip
					console.info('ip地址：',cip)
					if(cip){
						GM_xmlhttpRequest({
							url:"https://weleader5.oss-cn-shenzhen.aliyuncs.com/APP/clipdata.json?tt"+Date.parse(new Date()).toString(),
							method :"GET",
							data:"",
							headers: {
								"Content-type": "application/x-www-form-urlencoded"
							},
							onload:function(xhrdata){
								//console.info('ip地址：',xhrdata.responseText)
								var clipdatajsons = JSON.parse(xhrdata.responseText);
								for (var j=0;j<clipdatajsons.length;j++)
								{
									if(clipdatajsons[j].ip == cip){
										var clipdata = clipdatajsons[j];
										$('#selfservername').html(clipdata.name);
										$('#selfserverIp').html(clipdata.ip);
										$('#selfclemail').html(clipdata.email);https://coinlist.co/queue/enter_queue/braintrustoption1
										break;
									}
								}
							}
