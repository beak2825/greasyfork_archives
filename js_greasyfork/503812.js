...
// @require https://update.greasyfork.org/scripts/524935/1687440/rd.js

// @downloadURL https://update.greasyfork.org/scripts/503812/%EB%91%98%ED%81%AC%EB%A6%BD%ED%8A%B8%20-%20%EC%9C%84%EB%B2%84%EC%8A%A4%20-%20%EB%B0%B0%ED%8F%AC%EC%9A%A9.user.js
// @updateURL https://update.greasyfork.org/scripts/503812/%EB%91%98%ED%81%AC%EB%A6%BD%ED%8A%B8%20-%20%EC%9C%84%EB%B2%84%EC%8A%A4%20-%20%EB%B0%B0%ED%8F%AC%EC%9A%A9.meta.js
// ==/UserScript==
...
(function () {
	...
	const displayRemainingTime = (targetDate) => {
		const updateRemainingTime = () => {
			...
			const remainingSeconds =
				((remainingTime / 1000) % 60).toFixed(3) + getRD();
	...
	const scheduleButtonClick = (targetTime) => {
		...
		targetDate.setMilliseconds(targetTime.milliseconds + getRD());
...
3. 'cbc(hours, minutes, seconds, milliseconds)'을 호출하여 버튼 클릭을 예약할 수 있습니다.
 시간이 입력되지 않으면 5초 뒤에 버튼이 클릭됩니다.
  ...
  <p>둘크립트 동작중...</p>
	...
	document
		.getElementById("prayButton")
		.addEventListener("click", () => handlePrayButtonClick());

	// prayButton 클릭 시 실행되는 함수
