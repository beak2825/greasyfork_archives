...
v.1.7.4 - 19 Jul 2017 - by linus linus
 - Bug fix: Settings link re-added
 - Bug fix: Inline numbered re-added
 - Bug fix: Rich cards results re-added
...
EndHistory */
// @downloadURL https://update.greasyfork.org/scripts/31607/GoogleMonkeyR%20Fix%20%28Jul%202017%29.user.js
// @updateURL https://update.greasyfork.org/scripts/31607/GoogleMonkeyR%20Fix%20%28Jul%202017%29.meta.js
// ==/UserScript==
...
//			style += ("#rcnt{margin-top:1em} #sfcnt,#sftr,#searchform{display:none!important;}#cnt{padding:0}#cnt .mw:first-child{position:absolute;top:4.5em;right:0}#rshdr .sfcc{position:absolute;top:2em;right:0}");
		...
		// +TA - style fix
		style += ("#center_col .kp-blk {max-width: 60%} #GTR li.g, #GTR div.g {margin:.25em !important;" + BGBorder + " "+ hue +" ! important; padding:0 0 .5em ! important; } li.g {list-style:none outside none;"+imagePreview+"};");
...
//			style += ("#rcnt{margin-top:1em} #sfcnt,#sftr,#searchform{display:none!important;}#cnt{padding:0}#cnt .mw:first-child{position:absolute;top:4.5em;right:0}#rshdr .sfcc{position:absolute;top:2em;right:0}");
			...
			this.whackDomTimeOut = setTimeout(function()
			{
				...
				if(ires){
					...
					if(extrares && res && this.autoLoad){
						extrares.parentNode.insertBefore(res, extrares);
					}
					if(this.hideSearch)
					{
						// this.resizeWatcher(true);
	...
	buildResultTable : function()
	{
		...
		if(div)
		{
			...
			// var list = document.getElementsByXPath("//div[@id='ires']//li[contains(@class,'g')] | //div[@id='ires']//div[contains(@class,'obcontainer')] | //div[@id='ires']//li/div[@class='g'] | //div[@id='ires']//div[@class='g']");
			// var list = document.getElementsByXPath("//div[@id='ires']//div[contains(@class,'kp-blk')] | //div[@id='ires']//li[contains(@class,'g')] | //div[@id='ires']//div[contains(@class,'obcontainer')] | //div[@id='ires']//li/div[@class='g'] | //div[@id='ires']//div[@class='g']");
	...
	resultsToTable : function(list, i, resLength, initial)
	{
		var link = list[i], div;
		// monkeyr.log(link)
		if(this.numResults && (div = document.getElementByXPath(".//div[@class='rc']/h3[@class='r'] | ./h3[@class='r'] | ./h3[1] | ./div[1]/h3[1] | .//td[@class='tsw']/h3[1]", link)))
		{
	...
	processResults : function(responseText)
	{
		...
		var list = document.getElementsByXPath(".//div[@id='ires']/ol/div[starts-with(@class,'srg')]/li | //div[@id='ires']//div[contains(@class,'obcontainer')] | //div[@id='ires']//div[@class='g'] | //div[@id='ires']//li[contains(@class,'g')]", nextResult);
		var length = list.length;
		if((ires = document.getElementById('ires')) && (cnt = document.getElementById('cnt'))){
			ires.style.display = ((length==1) ? 'block' : 'none');
	...
	registerControls : function()
	{
		...
		if (controls && !link)
		{
			...
			// var parent = document.getElementById('abar_button_opt');
			// var parent = document.getElementByXPath("//a[@id='hdtb-tls']");
			// var parent = controls.parentNode;
			var gheadermenu = document.getElementById('ab_options');
