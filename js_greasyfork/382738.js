...
// @version      null
// @description  World best zombs.io mod
// @author       DemostanisYt & Ultimate Mod
// @match        http://zombs.io/*
// @downloadURL https://update.greasyfork.org/scripts/382738/Ultimate%20bots.user.js
// @updateURL https://update.greasyfork.org/scripts/382738/Ultimate%20bots.meta.js
// ==/UserScript==
...
`)
        ...
        Ultimate.menu = Ultimate.createMenu("<h3>Ultimate bots</h3><div class='menu-grid'><div class='bots'></div><button class='ui-button ui-widget ui-corner-all add-bot'>Add bot</button></div>", "menu")
        _(".hud-top-center").innerHTML += "<button class='ultimate-button'>Ultimate mod</button>"
...
if(typeof unsafeWindow == "undefined" || "null") {
    window.myBot = new Bot("botName", "botPrefix", {
        profileName: "Bot name",
        profilePic: "img.png",
        profileInfo: "I am bot name, the famous bot!"
    })
} else {
    unsafeWindow.myBot = new Bot("botName", "botPrefix", {
        profileName: "Bot name",
        profilePic: "img.png",
        profileInfo: "I am bot name, the famous bot!"
    })
...
myBot.sendMessage("Hello dear user!")`
            ...
            function loadBots() {
                ...
                for (let i = 0; i < storedBots.length; i++) {
                    try {
                        ...
                        Game.currentGame.ui.getComponent("Chat").onMessageReceived({
                            displayName: "[CONSOLE]",
                            message: "Must delete bot number " + i + ", because it has an error. " + err + " Please reload your page.</br><button class='ui-button ui-widget ui-corner-all delete-notworking-bot'>Click here</button>"
                        })
                        var messages = document.getElementsByClassName("hud-chat-message")
                        var length = messages.length - 1
                        messages[length].innerHTML = messages[length].innerHTML.replace(/&lt;/g, "<").replace(/&gt;/g, ">")
        ...
        _(".add-bot").addEventListener("click", function() {
            let editorMenu = Ultimate.createMenu("<textarea class='editor'></textarea><button class='ui-button ui-widget ui-corner-all save-bot'>Save</button>", "menu")
            editorMenu.show()
        ...
        document.addEventListener("click", function(e) {
            if (e.target.classList[3] == "delete-bot") {
                ...
                let menu = Ultimate.createMenu('<textarea class="edit-bot-textarea"></textarea><button class="ui-button ui-widget ui-corner-all save-changes">Save changes</button>', "menu")
                menu.show()
