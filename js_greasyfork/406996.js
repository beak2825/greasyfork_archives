...
// @downloadURL https://update.greasyfork.org/scripts/406996/Virtonomica%3AGosSnab%2010%25.user.js
// @updateURL https://update.greasyfork.org/scripts/406996/Virtonomica%3AGosSnab%2010%25.meta.js
// ==/UserScript==
...
*/
    ...
    function SetAmount( up) {
        ...
        $("tr.product_row").each( function() {
            ...
            var set = sale; // закупить то, что было продано
            ...
            if( !( (sale == 0) && (amount==0) ) ) {
                ...
                if (limit > 0) {
                    ...
                    if (prc > limit) {
                        prc_count++;
                        var all = Math.round(100*sale/prc);
                        // Не закупать больше рынка
                        if (set > all*0.1) {
                            set = Math.round(all*0.1);
                            console.log ("0.1all.set = "+ set);
                        }
                        //                 else  if ( (col+set -sale) > all*0.1 ) {
                        //                                         // прогноз тгго, что будет на складе через персчет
                        //                                        var new_sklad = col+set - sale;
    ...
    function calculateAdd( up )
    {
          ...
          var  prc = market[ img.attr("src") ];
            var sum = price[ img.attr("src") ];
            prc_count++;
            var all = Math.round(100*sale/prc);
           var set = Math.round(all*0.1);
        ...
        if (limit > 0) {
            ...
            $.get(url, function(data) {
                ...
                for (i=1; i< td.length; i++) {
                    ...
                    //	console.log(market[ img.attr('src') ]);
