...
// @include https://*t66y*
// @grant  GM_addStyle
// @2019.10.06 新增adblock检测屏蔽，新增指定用户发帖高亮，屏蔽详情页广告
// @2019.12.12 更新屏蔽规则
// @2019.12.19 更新屏蔽规则


// @downloadURL https://update.greasyfork.org/scripts/390949/%5BHAN%5DCL1024%20%E7%BB%88%E6%9E%81%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/390949/%5BHAN%5DCL1024%20%E7%BB%88%E6%9E%81%E7%89%88.meta.js
// ==/UserScript==
...
(function() {
    if (document.title.indexOf('草榴') != -1) {
        ...
        var PLANETWORK = {
            ...
            Earth: function() { //所有新增功能都在这里
                $('.sptable').remove();
                $('.sptable_do_not_remove').remove();
                $('#footer').remove();//移除footer广告，直接搞掉footer吧
                //去掉详情页广告
                var html = document.getElementsByClassName('t t2')[0].innerHTML
                document.getElementsByClassName('t t2')[0].innerHTML=html.replace('<div class="tips" style="width:auto">', '<div class="tips" style="width:auto;display:none">')
                //改掉站名
                var head = document.getElementsByClassName('banner')[0].innerHTML
                document.getElementsByClassName('banner')[0].innerHTML=head.replace('草榴社區', '看什么看！')
            },
            Mars: function() {
                ...
                $('div.t2').each(function() {
                    ...
                    if (html == '1024'||html == '感谢分享'||html == '谢谢分享') {
...
*/
            ...
            Western: function() { //新增关键词高亮
                //改掉站名
                var head = document.getElementsByClassName('banner')[0].innerHTML
                document.getElementsByClassName('banner')[0].innerHTML=head.replace('草榴社區', '看什么看！')
                /*var searchKey = 'Western'
                var html = document.getElementsByClassName('t')[0].innerHTML
                document.getElementsByClassName('t')[0].innerHTML=html.replace(/Western/g, '<span style="color:white;background-color:#0000ff8f">' + searchKey + '</span>')*/
                $('.tr3').each(function(){
                    var tr3 = $(this);
                    keyword = $(this).find('a[class=bl]').text();
                    if(keyword.startsWith('Western'))
                    {
                        tr3.find('td:first').parents("tr").css('background', '#fbff00ad');
                    }
                    else if(keyword.startsWith('一夜抱妇'))
                    {
                        tr3.find('td:first').next().html('<h3>不看你的收费网盘，给老子爬</h3>').css('color', '#d8dde4').css('text-decoration', 'line-through');
                        tr3.find('td.tal').nextAll().html('<h3>爬</h3>').css('color', '#d8dde4').css('text-decoration', 'line-through');
                    }
                });
            },
            Saturn: function() {
