...
// @downloadURL https://update.greasyfork.org/scripts/459843/%E5%97%A8%E7%9A%AE%E6%BC%AB%E7%95%AB%E9%96%B1%E8%AE%80%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/459843/%E5%97%A8%E7%9A%AE%E6%BC%AB%E7%95%AB%E9%96%B1%E8%AE%80%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
...
(async () => {
    ...
    const isLogged = localStorage.getItem("_u")?.includes("token") || document.cookie.includes("sf_token");
    const nextSelector = "//article//button[text()='下一话' or text()='下一話'][starts-with(@data-href,'/mangaread/')]";
    const prevSelector = "//article//button[text()='上一话' or text()='上一話'][starts-with(@data-href,'/mangaread/')]";
        ...
        fetch("/v2.0/apis/manga/reading?" + params, getHeaders()).then(res => res.json()).then(async jsonData => {
            try {
                if (jsonData.status == 0) {
                    ...
                    const srcs = jsonData.data.scans.map(({
                        url
                    }) => {
                        if (srcNoParams) {
                            return url.replace(/\?q=\d+$/, "");
                        } else {
                            return url;
                        }
                    });
...
body {
    overflow: auto !important;
        ...
        srcNoParams = !src.includes("?");
        console.log("頁面圖片網址參數？", srcNoParams ? "沒有\n" : "有\n", img);

        const footerEle = await waitEle("#root footer");
        new IntersectionObserver((entries, observer) => {
            if (entries[0].isIntersecting) {
                const nextE = ge("//article//button[text()='下一话' or text()='下一話']");
                const prevE = ge("//article//button[text()='上一话' or text()='上一話']");
                if (nextE?.dataset?.href?.includes("/readMore/")) {
                    nextE.style.color = "rgb(33, 33, 33)";
                    nextE.style.backgroundColor = "rgb(245, 245, 245)";
                    nextE.innerText = "（╯﹏╰）已经没有下一话了哦~";
                }
                if (prevE?.dataset?.href?.includes("/mangaread/")) {
                    prevE.style.color = "rgb(255, 255, 255)";
                    prevE.style.backgroundColor = "rgb(103, 58, 183)";
                }
            }
        }).observe(footerEle);
    }
    ...
    if (configs.autoNext == 1 && isReadPage) {
        ...
        const observeE = ge("#root footer article");
        if (observeE) {
            let timeId;
            new IntersectionObserver(entries => {
                if (entries[0].isIntersecting) {
                    timeId = setTimeout(() => {
                        let nextE = ge(nextSelector);
                        if (nextE) {
                            _unsafeWindow.location.href = nextE.dataset.href;
                        }
    ...
    if (configs.preload == 1 && isReadPage && configs.infiniteScroll != 1) {
        ...
        if (nextE) {
            preload(nextE.dataset.href, "嗨皮漫畫下一話數據\n");
        }
    }
...
.hide {
    display: none !important;
        ...
        let [, , mangaCode, currentChapterId] = lp.split("/");
        //const mangaCode = await waitEle("button[data-href]:has([data-testid='DescriptionIcon'])").then(b => b.dataset.href.split("/").at(-1));
        //const localStorageHistory = JSON.parse(localStorage.getItem("_ht")).find(h => h.serie_code == mangaCode);
        ...
        const getNextCid = () => chapterListData?.find((_, index, array) => array[index - 1]?.id === currentData?.current_cid)?.id;
        const getPrevCid = () => chapterListData?.find((_, index, array) => array[index + 1]?.id === currentData?.current_cid)?.id;
            ...
            const get = () => {
                ...
                return fetch("/v2.0/apis/manga/chapterByPage?" + params, getHeaders()).then(res => res.json()).then(json => {
                    ...
                    if (json?.msg === "success") {
                        ...
                        const find_current_chapter_data = json.data.items.some(e => e.id == currentChapterId);
                        if (json?.data?.isEnd == 1 || chapterListData.length >= json?.data?.total || find_current_chapter_data) {
                            isObtainedAllChapters = true;
                            chapterListData = chapterListData.reverse();
                            console.log("無限滾動需要用到的章節資料", chapterListData);
                        } else {
                            page++;
                            return get();
                        }
        ...
        const getReadData = async (current_cid, isNext = 0) => {
            let loading;
            if (isNext == 1) {
            ...
            try {
                ...
                if (isNext == 1) {
                    ...
                    const index = localStorageHistoryArray.findIndex(h => h.serie_code == mangaCode);
                    const object = Object.assign(localStorageHistoryArray[index], {
                        read_chapter_id: readJson.data.id,
                        read_chapter_name: readJson.data.chapter_name,
                        add_time: new Date().getTime(),
                        update_time: new Date().getTime()
                    });
                    localStorageHistoryArray[index] = object;
                    localStorage.setItem("_ht", JSON.stringify(localStorageHistoryArray));

                    loading?.remove();
                }
                return {
                    current_cid: Number(current_cid),
                    ...readJson.data
                };
            } catch (error) {
                loading?.remove();
        ...
        const addBrowsingHistory = (data, id) => {
            ...
            const url = _unsafeWindow.location.origin + "/mangaread/" + mangaCode + "/" + id;
...
<div style="background-color:#fff;padding:10px;">
  ...
  <button id="reload-comments" class="hide" style="font-size: 1rem; border: 1px solid rgb(0, 0, 0); border-radius: 4px;">${i18n.button.reloadComments}</button>
</div>
            `;
            ...
            const reload = ge("#reload-comments", div);
    ...
    <div class="MuiListItemText-root MuiListItemText-multiline" style="min-width: 0px; margin-top: 6px; margin-bottom: 6px; font-weight: bolder; color: rgba(0, 0, 0, 0.87);">
        ...
        const createPageElement = (data, isFirst = 0) => {
            ...
            let srcs = data.scans.map(obj => {
                let src;
                if (srcNoParams || configs.highQuality == 1) {
            ...
            const there_is_a_next = !!getNextCid();

            if (srcs.length == 2 && (("next_cid" in data) || there_is_a_next)) {
                srcs = srcs.slice(0, -1);
            }
                ...
                if (configs.autoReload == 1) {
                    ...
                    img.onerror = error => {
                        ...
                        if (num < 10) {
                            ...
                            setTimeout(() => {
                                if (num >= 1) {
                                    error.target.src = error.target.dataset.src.replace(/\?q=\d+$/, "");
                                } else {
        ...
        const preloadNext = async (cid) => {
            const data = await getReadData(cid);
        ...
        const infiniteScroll = async () => {
            ...
            const chapters_next_cid = getNextCid();

            if (("next_cid" in currentData) || !!chapters_next_cid) {
                ...
                if (nextDataJSon == "ERROR") {
                    ...
                    const chapters_next_cid = getNextCid();
                    const chapters_pre_cid = getPrevCid();
                    ...
                    if (("next_cid" in currentData) || !!chapters_next_cid) {
                        ...
                        const nextUrl = "/mangaread/" + mangaCode + "/" + nid;
                    ...
                    if (("pre_cid" in currentData) || !!chapters_pre_cid) {
                        ...
                        const prevUrl = "/mangaread/" + mangaCode + "/" + pid;
        ...
        const readData = await getReadData(currentChapterId);
