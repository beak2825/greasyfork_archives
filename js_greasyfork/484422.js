// ==UserScript==
// @name                Save Twitter Images and Videos to Eagle
// @name:zh             导入 Twitter 图片视频到 Eagle
// @name:zh-CN          导入 Twitter 图片视频到 Eagle

// @description         Save images and videos from Twitter to Eagle.
// @description:zh      在Twitter网页上添加下载按钮直接导入Eagle
// @description:zh-CN   在Twitter网页上添加下载按钮直接导入Eagle
...
// @version             0.2.1
// @namespace https://greasyfork.org/users/1245918
// @downloadURL https://update.greasyfork.org/scripts/484422/Save%20Twitter%20Images%20and%20Videos%20to%20Eagle.user.js
// @updateURL https://update.greasyfork.org/scripts/484422/Save%20Twitter%20Images%20and%20Videos%20to%20Eagle.meta.js
// ==/UserScript==

const SAVE_TXT = true; // 是否保存帖子文本
const SAVE_TAGS = true; // 是否保存帖子文本中的标签
const TAG_ACCOUNT_NAME = false; // 是否将用户名作为标签保存
const TAG_NICK_NAME = true; // 是否将昵称作为标签保存
const PARENT_DIR_NAME = ""; // 用于存放文件的文件夹名，为空则表示不需要
const PATT = / *[@＠◆■◇☆⭐️🌟🦇💎🔞🍅🌱🐻🍬：:\\\/].*/; // 处理昵称多余后缀的正则

// 读取已存储设置
var patt = new RegExp(GM_getValue("patt", PATT.source));
var saveTags = GM_getValue("saveTags", SAVE_TAGS);
var saveTxt = GM_getValue("saveTxt", SAVE_TXT);
var tagAccountName = GM_getValue("tagAccountName", TAG_ACCOUNT_NAME);
var tagNickName = GM_getValue("tagNickName", TAG_NICK_NAME);
var parentDirName = GM_getValue("parentDirName", PARENT_DIR_NAME);
...
function isDarkMode(){
    // return document.getElementsByTagName("html")[0].getAttribute("color-scheme") === "dark";
    // 获取 name 为 theme-color 的 meta 元素
    var themeColorMeta = document.querySelector('meta[name="theme-color"]');

    // 检查是否找到了 meta 元素
    if (themeColorMeta) {
        // 获取 meta 元素的 content 属性值
        var themeColor = themeColorMeta.getAttribute('content');
...
function download(url, filename, accountName, id, dirId){
    if(tagAccountName) tweet_tags.push(accountName);
    if(tagNickName) tweet_tags.push(nick_name);
    let data = {
        "url": url,
        "name": filename,
        "website": `https://x.com/${accountName}/status/${id}`,
        "tags": tweet_tags,
        "annotation": tweet_txt,
        "folderId": dirId,
        //"headers": HEADERS
    }
    GM_xmlhttpRequest({
...
async function getParentFolderId(){
    // 搜索同名文件夹
    function searchFolder(folders){
        for(let folder of folders){
            if(folder.name === parentDirName){
                return folder;
            }
        }
        for(let folder of folders){
            let target = searchFolder(folder.children);
    ...
    let parentFolder;
    if(folders){
        parentFolder = searchFolder(folders);
    }
    else{
    ...
    if(!parentFolder){
        parentFolder = await createParentFolder();
    }
...
function createParentFolder() {
    return new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
            ...
            method: "POST",
            data: JSON.stringify({ folderName: parentDirName }),
            onload: function(response) {
...
// 格式化昵称，删除多余后缀，为避免误伤，同时匹配到多次不作处理
function nickNameTrim(nickName){
    let test = nickName.match(patt);
    if(test && test.length === 1){
        let tmp = nickName.replace(test[0],"");
        nickName = tmp === "" ? nickName : tmp;
    }
    // 删掉“接稿中”三个字
    nickName = nickName.replace(/接稿中$/, "");
    return nickName
}

function saveTxtAndTags(btn){
    var ancestorDiv = btn;
    while(true) {
        if (!ancestorDiv.parentNode) {
            break; // 如果已经没有更多的祖先元素，则跳出循环
        }
        if (ancestorDiv.parentNode.childNodes.length >= 4) {
            break; // 子节点大于等于4时跳出循环
        }
        ancestorDiv = ancestorDiv.parentNode;
    }
    if (ancestorDiv) {
        var previousDiv = ancestorDiv.previousElementSibling;
        if (previousDiv) {
            let txtNode = previousDiv;
            while(true) {
                if (txtNode.childNodes.length == 0) {
                    break;
                }
                txtNode = txtNode.childNodes[0];
                if (txtNode.lang == "" || txtNode.lang == null || txtNode.lang == undefined) {
                    continue;
                }

                // 遍历 `<div>` 的子节点
                for (var j = 0; j < txtNode.childNodes.length; j++) {
                    var childNode = txtNode.childNodes[j];

                    // 如果子节点是文本节点，将其内容添加到 tweet_txt
                    if (childNode.nodeType === 1 || childNode.nodeType === 3) {
                        if (childNode.textContent == "") {
                            tweet_txt += childNode.alt;
                        }
                        else {
                            tweet_txt += childNode.textContent;
                        }
                        // 如果子节点是元素节点，并且包含 "#"，将链接中的文本添加到 tweet_tags
                        if (childNode.textContent.includes('#')) {
                            tweet_tags.push(childNode.textContent.trim().replaceAll("#", ""));
                        }
                    }
                }
            }
        ...
        if (previousDiv.previousElementSibling != null) {
            childNode = previousDiv.previousElementSibling;
            while(childNode.childNodes.length > 0){
                childNode = childNode.childNodes[0];
            }
            nick_name = nickNameTrim(childNode.textContent);
        }
        else{
            previousDiv = ancestorDiv.parentNode.previousElementSibling;
            if (previousDiv) {
                if (previousDiv.childNodes.length >= 2) {
                    childNode = previousDiv.childNodes[1];
                    while(childNode.childNodes.length > 0){
                        childNode = childNode.childNodes[0];
                    }
                    nick_name = nickNameTrim(childNode.textContent);
...
<div class="ujs-btn-common ujs-btn-error-text"></div>`.slice(1);
        ...
        static async _downloadPhotoMediaEntry(id, author, url, btn) {
            ...
            const filename = `${nick_name}_${lastModifiedDate}_${id}`;
            //downloadBlob(blob, filename, url);

            let dirId = await getParentFolderId();
            download(handleImgUrl(url), filename, author, id, dirId);
            clearCache();
        ...
        static async _downloadVideoMediaEntry(mediaEntry, btn, id /* of original tweet */) {
            ...
            let dirId = await getParentFolderId();
            download(url, filename, author, id, dirId);
            clearCache();
...
function createConfigPage(){
    ...
    // 布尔值
    let saveTxt_input = createNewConfig("是否保存帖子文本", "checkbox", saveTxt);
    let saveTags_input = createNewConfig("是否保存帖子文本中的标签", "checkbox", saveTags);
    let tagAccountName_input = createNewConfig("是否将用户名作为标签保存", "checkbox", tagAccountName);
    let tagNickName_input = createNewConfig("是否将昵称作为标签保存", "checkbox", tagNickName);
    // 文本
    let parentDirName_input = createNewConfig("用于存放文件的文件夹名，为空则表示不需要", "text", parentDirName);
    let patt_input = createNewConfig("正则表达式，处理昵称多余后缀，匹配到的内容将被删除：", "text", patt.source);
    ...
    button_save.addEventListener("click", ()=>{
        saveTxt = saveTxt_input.checked;
        saveTags = saveTags_input.checked;
        tagAccountName = tagAccountName_input.checked;
        tagNickName = tagNickName_input.checked;
        parentDirName = parentDirName_input.value;
        patt = new RegExp(patt_input.value);
        GM_setValue("saveTags", saveTags);
        GM_setValue("saveTxt", saveTxt);
        GM_setValue("tagAccountName", tagAccountName);
        GM_setValue("tagNickName", tagNickName);
        GM_setValue("parentDirName", parentDirName);
