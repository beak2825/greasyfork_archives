...
// @downloadURL https://update.greasyfork.org/scripts/42473/Pixiv%20-%20Manga%20Viewer.user.js
// @updateURL https://update.greasyfork.org/scripts/42473/Pixiv%20-%20Manga%20Viewer.meta.js
// ==/UserScript==
...
(function() {
    ...
    function multiple() {
        ...
        imgCount = document.querySelector('figure > div > div > div > div').innerHTML.match(/\d+$/)[0];
        var maxHeight = maxMangaHeight == 'default' ? maxHeight = 'calc(100vh - 48px)' : maxHeight = maxMangaHeight;
        style.innerHTML = `
            ._1rR5dTX { margin: 16px 0 0; background-color: #fff; position: relative; }
            ...
            ._gVu_bev { position: absolute; top: 8px; right: 8px; background: #aaa; border-radius: 8px; color: #fff; font-size: 10px; font-weight: 700; line-height: 1; padding: 3px 8px; }
            ._pmv-image-1 { margin-top: 0 !important; }
              figure > div { min-height: 480px; }
        `;
        let isCountNull = imgCount.match(/^\d+$/) ? document.querySelectorAll('figure a[href*="mode=manga"]')[1].innerText += ' (' + imgCount + ')' : imgCount = 'null';
        request();
    }
    ...
    function createImage(src) {
        ...
        layout.className = '_1rR5dTX _pmv-image-' + index;
        layout.innerHTML = `<div class="_7tR0cGH"><span class="_gVu_bev"><div>${index + ' / ' + imgCount}</div></span><div class="_2rca-ib"><a class="_2dxMuQZ" href=${src} target="_blank"><img class="_2dnVuLX" alt="Failed!" src=${defaultResolution}></a></div></div>`;
