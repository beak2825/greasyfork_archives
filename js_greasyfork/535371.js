...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/535371/MWI%20QQShow.user.js
// @updateURL https://update.greasyfork.org/scripts/535371/MWI%20QQShow.meta.js
// ==/UserScript==
...
(function () {
            ...
            GM_xmlhttpRequest({
                ...
                onload: function (response) {
                    try {
                        ...
                        localStorage.setItem("magic_qqshow_cache", JSON.stringify(globalVariable.qqShow.replacementTargets));
                        showToast("已更新QQ秀缓存");
                        resolve();
                    } catch (jsonError) {
    ...
    function gameMain() {
        ...
        hookWebSocket();
        // 优先从缓存加载QQ秀
        if ('magic_qqshow_cache' in localStorage) {
            globalVariable.qqShow.replacementTargets = JSON.parse(localStorage.getItem("magic_qqshow_cache"));
        }
        if ('magic_qqshow_cache_timestamp' in localStorage) {
            // 若缓存过期（有效期 1 天） => 立即更新缓存
            if (Math.abs(Date.now() - localStorage.getItem("magic_qqshow_cache_timestamp")) > 86400000) getQQShowData();
        } else {
            // 未找到缓存  => 立即更新缓存
            getQQShowData();
        }
