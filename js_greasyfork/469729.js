...
// @grant        GM_info
// @downloadURL https://update.greasyfork.org/scripts/469729/GanganonlineDownloader.user.js
// @updateURL https://update.greasyfork.org/scripts/469729/GanganonlineDownloader.meta.js
// ==/UserScript==
    ...
    const timer = setInterval(() => {
      try {
        ...
        if (chapterData) {
          ...
          // 过滤掉包含/secure/extra_manga_page/的图片
          const filteredImageURLs = chapterData.pages
            .map(page => window.location.origin + (page.image || page.linkImage).imageUrl)
            .filter(url => !url.includes('/secure/extra_manga_page/'));
