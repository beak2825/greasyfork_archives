...
// @version      3.1.5
// @description  Christmas Town Helper : Highlights Items, Chests, NPCs, Games Cheat, Extended View, Joystick and Chest Decoder
// @author       Hardy [2131687], zstorm [2268511]
...
// @downloadURL https://update.greasyfork.org/scripts/559603/Christmas%20Town%20Helper%20%2B.user.js
// @updateURL https://update.greasyfork.org/scripts/559603/Christmas%20Town%20Helper%20%2B.meta.js
// ==/UserScript==
(function () {
    'use strict';    
    throwRed(` v${GM_info.script?.version || null} | started`);
    function throwRed(data) {
        console.log(`%cChristmas Town Helper+${(typeof data !== 'object') ? '' + data : ''}`, 'background: #9c161fff; color: white;padding:10px; border-radius:3px;', (typeof data === 'object') ? data : '');
    }
    if (window.hardyScriptLoaded) {
        console.log("CT Helper+ already active. Blocking duplicate load...");
        return; 
    }
    window.hardyScriptLoaded = true;
    console.log("CT Helper+ initialized successfully.");
    let hardyObserver = null;
    function startItemObserver() {        
        if (hardyObserver) {
            hardyObserver.disconnect();
            hardyObserver = null;
        }
        const targetNode = document.querySelector('.items-layer');
        if (!targetNode) return;
        hardyObserver = new MutationObserver((mutations) => {
            clearTimeout(window.highlightTimeout);
            window.highlightTimeout = setTimeout(() => {
                highlightItems();
            }, 100); 
        });
        hardyObserver.observe(targetNode, { childList: true, subtree: true });
    }
    const waitObj = {};
    ...
    let saved;
	//const ctVibrate = localStorage.getItem('wb_ct_vibrate') === 'true';
	let possibilities = [];
    ...
    const colors = {
        ...
        orange: '#d6891080',
        green: '#1e844980',
        default: 'rgba(20, 30, 40, 0.5)',
        panelBg: 'rgba(23, 32, 42, 0.85)',
        border: '#2e4053',
        text: '#d5d8dc'
    };
    const nextColor = {
    ...
    const options = {
		"checkbox":
			{
				"items": { "name": "Highlight Items", "def": "yes", "color": "#e4e461" },
				"gold_chest": { "name": "Highlight Golden Chests", "def": "yes", "color": "#e4e461" },
				"silver_chest": { "name": "Highlight Silver Chests", "def": "yes", "color": "#e4e461" },
				"bronze_chest": { "name": "Highlight Bronze Chests", "def": "yes", "color": "#e4e461" },
				"combo_chest": { "name": "Highlight Combination Chests", "def": "yes", "color": "#e4e461" },
				"chest_keys": { "name": "Highlight Keys", "def": "yes", "color": "#e4e461" },
				"highlight_santa": { "name": "Highlight Santa", "def": "yes", "color": "#ff6200" },
				"highlight_npc": { "name": "Highlight Other NPCs", "def": "yes", "color": "#ff6200" },
        ...
        "garlandAssembleSolve": function (gridData) {
            ...
            instance.solve()
                .then(solution => {
                    ...
                    garlandColor(clicks);
                    gameHelper.html = `<label class="ctHelperSuccess">Click on yellow tiles until no longer yellow.</label> However, click slowly to avoid unnecessary clicks.`;
                    gameHelper.update();
                }).catch(error => {
    ...
    const resetLogic = () => {
        possibilities = [];
        guessedDigits = new Set();
        for (let i = 1; i <= 9; i++) {
            for (let j = 1; j <= 9; j++) {
                for (let k = 1; k <= 9; k++) {
                    if (i !== j && j !== k && i !== k) possibilities.push([i, j, k]);
                }
            }
        }
        ...
        [1, 2, 3].forEach(i => {
            ...
            if (btn) {
                btn.setAttribute('data-val', 'red');
                btn.style.backgroundColor = colors.red;
                btn.innerText = 'R';
            }
            if (digitBox) {
                digitBox.style.backgroundColor = colors.default;
            }
        });

        document.getElementById('ct-suggestion').innerText = "TRY: 1 5 9";
        document.getElementById('real-input').value = "";
        document.getElementById('ct-suggestion').classList.remove('suggestionGlowing');
        document.getElementById('ct-suggestion').classList.add('suggestionNormal');
        document.getElementById('poss-count').innerText = "CHEST HELPER";
        document.getElementById('history-log').innerHTML = "";
        updateVisualInput("");
    };

    const updateVisualInput = (val) => {
        const digits = val.split('');
        const suggestionText = document.getElementById('ct-suggestion').innerText;
        const suggestionMatch = suggestionText.match(/\d \d \d/);
        const suggestedDigits = suggestionMatch ? suggestionMatch[0].split(' ') : [];

        for (let i = 0; i < 3; i++) {
            const box = document.getElementById(`digit-${i}`);
            if (digits[i]) { box.innerText = digits[i]; box.style.opacity = "1"; }
            else if (suggestedDigits[i]) { box.innerText = suggestedDigits[i]; box.style.opacity = "0.3"; }
            else { box.innerText = "-"; box.style.opacity = "1"; }
        }
    };
                    ...
                    if (data.mapData.items) {
                        ...
                        if (items.length > 0) {
                            ...
                            if (metadata.settings.count == 1) {
								document.querySelector(".hardyNearbyChests").innerHTML = '<div class="content"></div>';
                                document.querySelector(".hardyNearbyItems").innerHTML = '<div class="content"></div>';
    ...
    function ctHelperChangeHTML(array, selector, label) {
        ...
        const content = length ? array.map(element => `<p class="nearby-item-chests-p">${element[0]} at ${element[1]}, ${element[2]}&nbsp;</p>`).join("") : "";
        const el = document.querySelector(`.${selector}`);
        if (el) el.innerHTML = `<div class="content">${content}</div>`;
    }
    function initiate() {
        if (window.location.href.includes("page=ctitemsFound")) {
            ...
            createStorage();
            localStorage.setItem('wb_ct_chest', false);
            addBox();
            highlighter_css()
            gamesHelper_css();
    ...
    function updateModifierText() {
        setTimeout(() => {
            const els = document.querySelectorAll(".rateBonus");
            els.forEach(el => {
                el.innerHTML = `Spawn Rate Bonus : ${metadata.cache.spawn_rate}%    |    Speed Rate Bonus : ${metadata.cache.speed_rate}%`;
            });
        }, 3000);
    }
    function addBox() {
        document.querySelectorAll(".hardyCTBox, .ct-joystick-container, #ct-chest-panel").forEach(el => el.remove());        
        waitForElement("#christmastownroot div[class^='appCTContainer']", 900, 999, "dhjdvefvasvduqwdufdevshacqweu").then((element) => {
            ...
            newBox.innerHTML = `<div class="wb-ct-title hardyCTHeader">
                                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 0 10px; height: 100%;">
                                        <div id="titlePanel" style="width:20%; display: flex; align-items: center; white-space: nowrap;">
                                            <span>CT Helper</span>
                                        </div>
                                        <div id="settingPanel" style="width:80%;">
                                            <button class="settingBtn cherrypick" title="CT Helper Settings">⚙️</button>
                                        </div>
                                    </div>
                                </div>
                                <div class="hardyCTContent">
                                            <div class="nearby-items-chests">
                                                <div class="hardyNearbyItems"> <div class="content"></div> </div>
                                                <div class="hardyNearbyChests"> <div class="content"></div> </div>
                                            </div>
                                </div>`;
            element.insertBefore(newBox, element.firstChild.nextSibling);
            newBox.querySelector(".settingBtn").addEventListener("click", (event) => {
                createPreferencesBox('#hardy_pref_box_q-links');
            })
            metadata.settings.spawn = 1;
            metadata.settings.speed = 1;

            //////////////////////////////////// // CTMAP - Extended View & Joystick ////////////////////////////////////
            const panel = newBox.querySelector("#settingPanel");
            panel.scrollLeft = panel.scrollWidth;
            const btnHtml = `
                <button id="toggleJoystick" class="settingBtn cherrypick"" title="Joystick">🎮</button>
                <button id="toggleChest" class="settingBtn cherrypick"" title="Chest Helper">🗃️</button>
                <button id="toggleEx" class="settingBtn regular-btn" title="Bigger Map">🏞️</button>
                <button id="toggleFog" class="settingBtn regular-btn" title="Fog or Box" >📦</button>
            `;
            panel.insertAdjacentHTML('afterbegin', btnHtml);

            // Joystick
            const createJoystick = () => {
                if (document.querySelector('.ct-joystick-container')) return null;
                const container = document.createElement('div');
                container.className = 'ct-joystick-container';
                container.style.display = localStorage.getItem('wb_ct_joystick') === 'true' ? 'flex' : 'none';

                const base = document.createElement('div');
                base.className = 'ct-joystick-base';
                const indicators = {
                    'ArrowUp': Object.assign(document.createElement('div'), { className: 'joy-indicator joy-up' }),
                    'ArrowDown': Object.assign(document.createElement('div'), { className: 'joy-indicator joy-down' }),
                    'ArrowLeft': Object.assign(document.createElement('div'), { className: 'joy-indicator joy-left' }),
                    'ArrowRight': Object.assign(document.createElement('div'), { className: 'joy-indicator joy-right' })
                };
                Object.values(indicators).forEach(el => base.appendChild(el));

                const stick = document.createElement('div');
                stick.className = 'ct-joystick-stick';
                ...
                const triggerKey = (key, type) => {
                    ...
                    const event = new KeyboardEvent(type, { key, code: key, keyCode, which: keyCode, bubbles: true });
                    document.dispatchEvent(event);
                    if (indicators[key]) {
                        indicators[key].classList.toggle('joy-active', type === 'keydown');
                    }
                    const vibrationEnabled = localStorage.getItem('wb_ct_vibrate') !== 'false';
                    //if (type === 'keydown' && vibrationEnabled && navigator.vibrate) {
                    //	navigator.vibrate(15); // 15ms
                    //}
                ...
                const handleMove = (e) => {
                    ...
                    if (distance > 15) {
                        ...
                        if (deg > -157.5 && deg < -22.5) newKeys.add('ArrowUp');
                        if (deg > 22.5 && deg < 157.5) newKeys.add('ArrowDown');
                        if (Math.abs(deg) > 112.5) newKeys.add('ArrowLeft');
                        if (Math.abs(deg) < 67.5) newKeys.add('ArrowRight');
                    }
                    activeKeys.forEach(k => { if (!newKeys.has(k)) triggerKey(k, 'keyup'); });
                    newKeys.forEach(k => { if (!activeKeys.has(k)) triggerKey(k, 'keydown'); });
                    activeKeys = newKeys;
                };

                const stopMove = () => {
                    activeKeys.forEach(k => triggerKey(k, 'keyup'));
                    activeKeys.clear();
            ...
            const hardyContent = newBox.querySelector(".hardyCTContent");
            const joy = createJoystick();
            if (joy) document.body.appendChild(joy);

            ///////////////////////////////////////////////////  CHEST DECODER  ////////////////////
            const createChestPanel = () => {
                if (document.querySelector('.ct-chest-panel')) return null;
                const chestPanel = document.createElement('div');
                chestPanel.id = "ct-chest-panel";
                chestPanel.style = `
                    position: fixed;
                    top: 50px;
                    right: 20px;
                ...
                chestPanel.innerHTML = `
                        <div id="panel-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 10px; cursor: move; padding: 5px 0;">
                            <div id="headInfo">
                                <span id="poss-count" style="font-weight:bold; color: #5dade2; font-size: 12px;">CHEST HELPER</span>
                            </div>
                            <div id="headButton" style="display:flex; justify-content:space-between; gap:5px;">
                                <div id="miniChestPanel" style="background: ${colors.blue}; border-radius: 50%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center;cursor: pointer; padding: 5px;">↕️</div>
                                <div id="closeChestPanel" style="background: ${colors.red}; border-radius: 50%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center;cursor: pointer; padding: 5px;">❌</div>
                                </div>								
                            </div>
                        <div id="ct-suggestion" class="suggestionNormal">TRY: 1 5 9</div>
                        <div id="lock-container" style="background-image: url('https://profileimages.torn.com/1e8a4f5b-6d07-457c-a410-56dd300f8e5b-2268511.png'); 
                            ...
                            margin-bottom: 10px; 
                            cursor: text;">
                            <input type="text" id="real-input" inputmode="numeric" maxlength="3" style="position: absolute; opacity: 0; pointer-events: auto; top: 0; left: 0;">
                            <div style="display: flex; justify-content: center; gap: 6px;">
                                <div id="digit-0" style="width: 35px; height: 40px; background: ${colors.default}80; border: 1px solid #34495e; border-radius: 5px; display: flex; align-items: center; justify-content: center; font-size: 22px; color: #fff;text-shadow: 1px 1px 2px rgba(0,0,0,0.8);">-</div>
                                <div id="digit-1" style="width: 35px; height: 40px; background: ${colors.default}80; border: 1px solid #34495e; border-radius: 5px; display: flex; align-items: center; justify-content: center; font-size: 22px; color: #fff;text-shadow: 1px 1px 2px rgba(0,0,0,0.8);">-</div>
                                <div id="digit-2" style="width: 35px; height: 40px; background: ${colors.default}80; border: 1px solid #34495e; border-radius: 5px; display: flex; align-items: center; justify-content: center; font-size: 22px; color: #fff;text-shadow: 1px 1px 2px rgba(0,0,0,0.8);">-</div>
                            </div>
                        </div>
                        <div style="display:flex; justify-content:space-between; gap:8px; margin-bottom: 10px;">
                            <button id="btn-res1" data-val="red" style="flex:1; background:${colors.red}80; height:35px; border:none; color:white; font-weight:bold; border-radius:5px;text-shadow: 1px 1px 2px rgba(0,0,0,0.8);">R</button>
                            <button id="btn-res2" data-val="red" style="flex:1; background:${colors.red}80; height:35px; border:none; color:white; font-weight:bold; border-radius:5px;text-shadow: 1px 1px 2px rgba(0,0,0,0.8);">R</button>
                            <button id="btn-res3" data-val="red" style="flex:1; background:${colors.red}80; height:35px; border:none; color:white; font-weight:bold; border-radius:5px;text-shadow: 1px 1px 2px rgba(0,0,0,0.8);">R</button>
                        </div>
                        <div style="display:flex; justify-content:space-between; gap:8px;">
                            <button id="reset-ct" style="width:30%; background:transparent; border:1px solid #c0392b; color:#c0392b; font-size:9px; height:35px; border-radius:5px;">RESET</button>
                            <button id="calc-btn" style="width:70%; background: #2980b9; color:#fff; border:none; height:35px; font-weight:bold; border-radius:5px; font-size:12px;">CHECK</button>
                        </div>
                        <div id="history-log" style="
                ...
                const dStart = (e) => {
                    ...
                    initialX = ev.clientX - xOffset; initialY = ev.clientY - yOffset;
                    if (e.target.closest('#panel-header')) active = true;
                };
                const dEnd = () => active = false;
                const dMove = (e) => {
                    if (!active) return;
                    e.preventDefault();
                    let ev = e.type === "touchmove" ? e.touches[0] : e;
                    currentX = ev.clientX - initialX; currentY = ev.clientY - initialY;
                    xOffset = currentX; yOffset = currentY;
                    chestPanel.style.transform = `translate3d(${currentX}px, ${currentY}px, 0)`;
                };
                ...
                document.addEventListener("mousedown", dStart);
                ...
                lockContainer.onclick = () => {
                    realInput.focus();
                };

                realInput.oninput = () => {
                    realInput.value = realInput.value.replace(/[^1-9]/g, '');
                    updateVisualInput(realInput.value);
                };
                ...
                [1, 2, 3].forEach(i => {
                    ...
                    btn.addEventListener('click', () => {
                        ...
                        btn.innerText = next.charAt(0).toUpperCase();
                        digitBox.style.backgroundColor = colors[next];
                    });
                });

                const getBestSuggestion = () => {
                    const sugEl = document.getElementById('ct-suggestion');
                    const log = document.getElementById('history-log');
                    let turnCount = log.children.length + 1;

                    if (possibilities.length === 0) return "STH WRONG";
                    if (possibilities.length === 1) {
                        sugEl.classList.add('suggestionGlowing');
                        return `MUST BE: ${possibilities[0].join(' ')}`;
                    }
                    // Map out G and possible-G
                    let lockedSlots = [null, null, null];
                    let knownDigits = new Set();
                    
                    for (let i = 0; i < 3; i++) {
                        let firstVal = possibilities[0][i];
                        if (possibilities.every(p => p[i] === firstVal)) {
                            lockedSlots[i] = firstVal;
                            knownDigits.add(firstVal);
                        }
                    }

                    // If we have G/possible-G, skip them, hunt the unknowns
                    if (turnCount === 3) {
                        let candidates = [1, 2, 3, 4, 5, 6, 7, 8, 9].filter(n => !knownDigits.has(n));                        
                        // Prioritize untested numbers in Alive pool
                        candidates.sort((a, b) => (guessedDigits.has(a) ? 1 : 0) - (guessedDigits.has(b) ? 1 : 0));

                        if (candidates.length >= 3) {
                            return `SCAN: ${candidates[0]} ${candidates[1]} ${candidates[2]}`;
                        }
                    }

                    // Fallback to standard Solver for Turn 2 or Turn 4
                    let best = possibilities[0], maxScore = -1;
                    for (let p of possibilities) {
                        let score = 0;
                        p.forEach((digit, index) => {
                            if (lockedSlots[index] === digit) score += 100;
                            if (!guessedDigits.has(digit)) score += 50;
                        });
                        if (score > maxScore) { maxScore = score; best = p; }
                    }
                    return `TRY: ${best.join(' ')}`;
                };

                document.getElementById('ct-suggestion').onclick = function() {
                    const m = this.innerText.match(/\d \d \d/);
                    if (m) { 
                ...
                document.getElementById('history-log').onclick = function() {
                    const log = document.getElementById('history-log');
                    if (log.style.height === 'var(--log-height)'){
                        log.style.height = '15px';
                        chestPanel.style.paddingBottom = '0px';
                    } else {
                        log.style.height = 'var(--log-height)';
                        chestPanel.style.paddingBottom = '12px';
                    }
                    

                };

                document.getElementById('miniChestPanel').onclick = function() {
                    chestPanel.style.width = chestPanel.style.width === 'var(--chest-width)' ? '200px' : 'var(--chest-width)';
                    document.getElementById('history-log').click();
                    document.getElementById('lock-container').style.borderRadius === '5px' ? "border-radius:0px !important; padding: 15px 3px !important; margin-bottom: 5px !important;" : "border-radius:5px; padding: 15px 5px; margin-bottom: 10px;";
                ...
                document.getElementById('calc-btn').onclick = () => {
                    ...
                    const fb = [
                        document.getElementById('btn-res1').getAttribute('data-val'),
                        document.getElementById('btn-res2').getAttribute('data-val'),
                        document.getElementById('btn-res3').getAttribute('data-val')
                    ];
                    const log = document.getElementById('history-log');
                    const wr = document.createElement('div');
                    wr.style = "display: flex; justify-content: space-between; align-items: center; margin-top: 6px; padding: 0 4px;";

                    let turnCount = log.children.length + 1;
                    const turn = document.createElement('span');
                    turn.innerText = "Turn" + turnCount;
                    turn.style = `width: 30px; color: ${turnCount >= 2 ? '#ff4757' : '#85929e'}; font-weight: bold; font-size: 13px;`;

                    const entry = document.createElement('div');
                    entry.style = "display: flex; gap: 4px;";

                    val.split('').forEach((d, i) => {
                        const s = document.createElement('span');
                        s.innerText = d;
                        const bgColor = colors[fb[i]] || colors.default;
                        s.style = `width: 26px; height: 26px; line-height: 26px; text-align: center; background: ${bgColor}; border-radius: 3px; color: #fff; font-weight: bold; font-size: 15px; text-shadow: 1px 1px 1px rgba(0,0,0,0.7);`;
                        entry.appendChild(s);
                    });

                    wr.appendChild(turn);
                    wr.appendChild(entry);
                    log.prepend(wr);

                    val.split('').forEach(d => guessedDigits.add(Number(d)));
                    possibilities = possibilities.filter(p => {
                        let g = val.split('').map(Number);
                        for (let i = 0; i < 3; i++) {
                            if (fb[i] === 'green' && p[i] !== g[i]) return false;
                            if (fb[i] === 'orange' && (p[i] === g[i] || !p.includes(g[i]))) return false;
                            if (fb[i] === 'red' && p.includes(g[i])) return false;
                        }
                        return true;
                    });

                    document.getElementById('poss-count').innerText = `Possibilties: ${possibilities.length}`;
                    document.getElementById('ct-suggestion').innerText = getBestSuggestion();
                    ...
                    [1, 2, 3].forEach(i => {
                        ...
                        btn.style.backgroundColor = colors.red;
                        btn.innerText = 'R';
                        digitBox.style.backgroundColor = "rgba(20,30,40,0.8)";
                    });
                };
                document.getElementById('reset-ct').onclick = resetLogic;
                resetLogic();
                return chestPanel;
            };
            const chest = createChestPanel();
            if (chest) document.body.appendChild(chest);
            resetLogic();

            applyAllChanges();
            const mapEl = document.querySelector('#map');
            if (mapEl && !mapEl.querySelector('.wb_fog')) {
                mapEl.insertAdjacentHTML('afterbegin', '<div class="wb_fog left"></div><div class="wb_fog right"></div><div class="wb_fog top"></div><div class="wb_fog bottom"></div><div class="wb_fog rightborderfix"></div><div class="wb_fog leftborderfix"></div>');
            }
            ////////////////////////
        }).catch(error => {
            console.log(error);
	...
	document.addEventListener('click', (e) => {
		const btn = e.target.closest('#toggleEx, #toggleFog, #toggleJoystick, #toggleChest');
		if (!btn) return;

		const keys = {
			'toggleEx': 'wb_ct_ex_on',
			'toggleFog': 'wb_ct_ex_fog',
			'toggleJoystick': 'wb_ct_joystick',
			'toggleChest': 'wb_ct_chest',
		};
		const storageKey = keys[btn.id];
		const newState = localStorage.getItem(storageKey) !== 'true';
		localStorage.setItem(storageKey, newState);
		applyAllChanges();
	});
    document.addEventListener('click', (e) => {
		const close = e.target.closest('#closeChestPanel');
        if (!close) return;
		const newState = localStorage.getItem('wb_ct_chest') !== 'true';
		localStorage.setItem('wb_ct_chest', newState);
		applyAllChanges();
	});
	const masterButtonToggle = (forceState) => {
		const keys = {
			'toggleEx': 'wb_ct_ex_on',
			'toggleFog': 'wb_ct_ex_fog',
			'toggleJoystick': 'wb_ct_joystick',
		...
		const memoryKey = 'wb_ct_helper_snapshot';
		if (forceState === false) {
			const snapshot = {};
			Object.keys(keys).forEach(id => {
				snapshot[keys[id]] = localStorage.getItem(keys[id]);
			});
			localStorage.setItem(memoryKey, JSON.stringify(snapshot));
			...
			if (savedData) {
				...
				Object.keys(snapshot).forEach(storageKey => {
					localStorage.setItem(storageKey, snapshot[storageKey]);
				});
			} else {
				Object.values(keys).forEach(storageKey => {
	...
	const initGameObserver = () => {
		...
		const callback = () => {
            ...
            const isCurrentlyOn = localStorage.getItem('wb_ct_ex_on') === 'true';
            const isCurrentlyOff = localStorage.getItem('wb_ct_ex_on') === 'false';
            const hasSnapshot = localStorage.getItem('wb_ct_helper_snapshot');
			if (titleTextClean === lastTitle) return;
			lastTitle = titleTextClean;
			const isMiniGame = miniGames.find(game =>
				titleTextClean.includes(game.toLowerCase())
			);
			if (isMiniGame) {
				if ( isCurrentlyOn || localStorage.getItem('wb_ct_joystick') === 'true') {
					console.log(`[Z] ${titleTextClean}! Minimize all!`);
					masterButtonToggle(false);
				}
			} else {
				if (isCurrentlyOff && hasSnapshot) {
					console.log("[Z] Restoring settings back.");
					masterButtonToggle(true);
				}
			}
             const santaDiv = document.querySelector('.santaMessageWrapper___MV6M3 p.paragraph'); //santa___ox0lO
             if (!santaDiv) {
                //console.log("No santaMessageWrapper___MV6M3 div found.");
                return;
            }
            if (santaDiv && isCurrentlyOn) {
                if (santaDiv.innerText.includes('There are no more presents for you today')) {
                    if (canFixSantaToday()) {
                        ...
                        setTimeout(() => {
                            localStorage.setItem('wb_ct_ex_on', isCurrentlyOff);
                            applyAllChanges();
                        }, 3000);
                        localStorage.setItem('wb_ct_ex_on', isCurrentlyOn);
                        masterButtonToggle(true);
                    } else {
                        console.log("Santa is already fixed for today. I hope he would not stuck again.");
                    }
                }
            }
		};
	...
	const applyAllChanges = () => {
		...
		if (wrap) {
			wrap.classList.toggle('wb-extended', localStorage.getItem('wb_ct_ex_on') === 'true');
			wrap.classList.toggle('wb-extended-fog', localStorage.getItem('wb_ct_ex_fog') === 'true');
		}
		const toggleDisplay = (selector, key, displayType = 'block') => {
			const el = document.querySelector(selector);
			if (el) el.style.display = localStorage.getItem(key) === 'true' ? displayType : 'none';
		};
		toggleDisplay('.ct-joystick-container', 'wb_ct_joystick', 'flex');
		toggleDisplay('#ct-chest-panel', 'wb_ct_chest', 'block');  //findme
	};

	function highlightItems() {
        const items = document.querySelectorAll(".items-layer .ct-item");
        const improvedGiftUrl = "https://profileimages.torn.com/09283301-ee91-4edc-b8df-ccfe03fa646b-2268511.gif";
        const obj = {   "Mystery Gift": "items",
                        "Gold Chest": "gold_chest",
                        "Silver Chest": "silver_chest",
                        "Bronze Chest": "bronze_chest",
                        "Combination Chest": "combo_chest",
                        "Golden Key": "chest_keys",
                        "Silver Key": "chest_keys",
                        "Bronze Key": "chest_keys" };
        for (const item of items) {
            ...
            const key = obj[name];
            if (name === "Mystery Gift" && imgElement.src !== improvedGiftUrl) {
                imgElement.src = improvedGiftUrl;
            }
    ...
    function highlighter_css() {
        ...
        if (to_be_highlighted_npcs.length > 0) {
            ...
            const styleAll = '{z-index:10}';
            let styleString = `${elementsAll} ${styleAll}`;
            styleString += `
			.ct-user[npctype='santa']::before,
			.ct-user[npctype='other']::before `;
            styleString += `
				{content:"";
                    position:absolute;
                    top:15px;left:15px;
                    ...
                    animation:1.5s ease-in-out infinite pulse-santa;
                    pointer-events:none
                    }
            ...
            styleString += `
				.ct-user[npctype='santa']::before
					{border: 5px solid ${saved.color["color_highlight_santa"]};}
				.ct-user[npctype='other']::before
					{border: 5px solid ${saved.color["color_highlight_npc"]};}`;
            GM_addStyle(styleString);
        }
    }
    ...
    if (!prefBox) {
        ...
        box.innerHTML = `
            <div class="hardy_modal">
                <div class="hardy_modal_header" style="display: flex; justify-content: space-between; align-items: center; padding: 8px;">
                    <label>Christmas Town Helper Preferences</label>
                </div>
                <div style="text-align: right; margin-right: 8px;"><button class="hardy_modal_close">Close</button></div>
                <div style="overflow: auto;">
                    <div style="margin-top: 10px; margin-bottom: 10px; text-align: center; font-size: 1.2em;  font-weight: bold;">
                        <div class="rateBonus" style="color: #4ade80;">Spawn Rate Bonus : ${metadata.cache.spawn_rate}%    ---    Speed Rate Bonus : ${metadata.cache.speed_rate}%</div>
                    </div>
                    <div class="hardy_modal_content"></div>
                    <div class="hardy_modal_msg" style="color: #4ade80; margin-top: 10px; font-weight: bold;"></div>
                </div>
            </div>`;

        box.querySelector(".hardy_modal_close").addEventListener("click", () => box.remove());
        let html = `<p class="text-blue-200">Highlighter</p>`;
        ...
        html += `
            <div style="text-align: right; margin-top: 15px; padding-bottom: 10px;">
                <button class="hardy-save-prefs" style="margin-right: 8px;">Save</button>
                <button class="hardy-ct-itemstable">Items Table</button>
            </div>`;

        box.querySelector(".hardy_modal_content").innerHTML = html;

        const wreathInput = box.querySelector('input[name="wreath"]');
        if (wreathInput) {
            const firstgameHelperLabel = wreathInput.previousSibling;
            const pGames = createElement("p", {"class": "text-blue-200", "style": "margin-top: 15px;"});
            pGames.innerText = "Games Helper";
            firstgameHelperLabel.parentNode.insertBefore(pGames, firstgameHelperLabel);
        }

        const chirpInput = box.querySelector('input[name="chirp_alert_ct"]');
        if (chirpInput) {
            const chirpAlertLabel = chirpInput.previousSibling;
            const pMisc = createElement("p", {"class": "text-blue-200", "style": "margin-top: 15px;"});
            pMisc.innerText = "Miscellaneous";
            chirpAlertLabel.parentNode.insertBefore(pMisc, chirpAlertLabel);
        }

        document.body.insertBefore(box, document.body.firstChild);
        box.querySelectorAll('input[type="color"]').forEach((input) => {
            input.onchange = function () { input.setAttribute("value", this.value); };
        });
        box.querySelector(".hardy-save-prefs").addEventListener("click", () => {
            ...
            inputs.forEach(input => {
                ...
                if (inpType === "checkbox") {
                    saved.checkbox[name] = input.checked ? "yes" : "no";
                } else if (inpType === "color") {
                    saved.color[name] = input.value;
                }
            });

            savePrefs();
            box.querySelector(".hardy_modal_msg").innerHTML = `Preferences saved! Refresh to sync.`;
        });
...
.hardyCTHeader { color: #121212; padding: 6px; font-size: 1.2em; font-weight: bold; text-align: center; }
.hardyCTContent { padding: 10px; font-size: 1.2em; }
.ctRecordLink { color: #121212; border: none; padding: 8px; cursor: pointer; }
.nearby-items-chests { display: flex; justify-content: space-between; }
.nearby-item-chests-p {
...
@keyframes blink-attention {
            50% {
                color: #ffea00ff;
                text-shadow: 0 0 8px #f1ffb0ff, 0 0 12px #fbff7fff;
            }
        }
...
.wb-extended #ct-wrap { display: flex !important; flex-direction: column !important; }
@media screen and (min-width: 785px) {
    .d .wb-extended #ct-wrap .user-map-container,
    .d .wb-extended #ct-wrap .map-overview { width: 784px !important; height: 600px !important; }
    .d .wb-extended #ct-wrap .map-overview #world { top: 135px !important; left: 225px !important; }
}
.wb-extended.wb-extended-fog .wb_fog { z-index: 99; position: absolute; background: url(/images/v2/christmas_town/snow.jpg) repeat; pointer-events: none; }
/* Add fog positioning (.left, .right, etc.) from script 1 here if you want the borders */

body.dark-mode .hardyCTHeader { color: #f0f0f0; }
...
.hardy_modal_header { background-color: #000; text-align: center; color: #4fff98ff; border-radius: 6px 6px 0 0; padding: 5px; width: 100%; }
...
.helcostrDoesntLikeGreenCommas { color: black; }
.hardyCTBox { background-color: #f2f2f2; border: 1px solid #ccc; border-radius: 10px; max-height: 250px; }
.hardyCTHeader { color: #121212; padding: 6px; font-size: 1.2em; font-weight: bold; text-align: center; border-radius: 10px 10px 0 0; }
.hardyCTContent { padding: 10px; font-size: 1.2em; }
.ctRecordLink { color: #121212; border: none; padding: 8px; cursor: pointer; }
.nearby-items-chests { display: flex; justify-content: space-between;  }
.nearby-item-chests-p {
...
.wb-ct-title {
    ...
    border-radius: 10px 10px 0 0 !important;
        ...
        /* --- CT Extended Map Layout --- */
.wb-extended #ct-wrap { display: flex !important; flex-direction: column !important; }
.wb-extended.wb-extended-fog .wb_fog {
    z-index: 99; position: absolute; background: url(/images/v2/christmas_town/snow.jpg) 0 0 repeat;
    pointer-events: none; border: 1px solid rgba(167, 190, 201, 0.3);
}
/* Desktop Map Size (Min-width 785px) */
@media screen and (min-width: 785px) {
    :root {
        --chest-width: 280px;
        --log-height: 100%;
    ...
    .d .wb-extended #ct-wrap .map-overview { width: 784px !important; height: 600px !important; }
    .d .wb-extended #ct-wrap .map-overview #world { top: 135px !important; left: 225px !important; }
    .wb-extended.wb-extended-fog .wb_fog.left, .wb-extended.wb-extended-fog .wb_fog.leftborderfix { width: 165px; height: 600px; }
    .wb-extended.wb-extended-fog .wb_fog.right, .wb-extended.wb-extended-fog .wb_fog.rightborderfix { left: 619px; width: 165px; height: 600px; }
    .wb-extended.wb-extended-fog .wb_fog.top { width: 784px; height: 75px; }
    .wb-extended.wb-extended-fog .wb_fog.bottom { top: 525px; width: 784px; height: 75px; }
}
/* Mobile Map Size (Under 784px) */
@media screen and (max-width: 784px) {
    ...
    .d .wb-extended #ct-wrap .user-map-container { width: 100% !important; }
    .d .wb-extended #ct-wrap .map-overview { width: 384px !important; height: 385px !important; top: -27px; }
    .d .wb-extended #ct-wrap .map-overview #world { top: 122px; left: 122px; zoom: 67%; }
    .wb-extended.wb-extended-fog .wb_fog.left { width: 21px; top: -27px; left: -1px; height: 384px; }
    .wb-extended.wb-extended-fog .wb_fog.right { width: 21px; top: -6.5px; left: 364px; height: 344px; }
    .wb-extended.wb-extended-fog .wb_fog.top { width: 385px; height: 21px; top: -27px; }
    .wb-extended.wb-extended-fog .wb_fog.bottom { top: 337px; width: 385px; height: 21px; }
}
...
.wb-extended.wb-extended-fog .wb_fog {
    ...
    pointer-events: none;
    border: 1px solid rgba(167, 190, 201, 0.3);
}
#settingPanel {
...
/* Ensure the SVG gear icon matches the emoji sizing */
#settingPanel button svg {
    display: block;
...
@keyframes pulse-glow {
    0% { box-shadow: 0 0 5px rgba(46, 204, 113, 0.2); border-color: #2e4053; }
    50% { box-shadow: 0 0 15px rgba(46, 204, 113, 0.6); border-color: #2ecc71; }
    100% { box-shadow: 0 0 5px rgba(46, 204, 113, 0.2); border-color: #2e4053; }
}
.suggestionGlowing {
    background: rgba(0,0,0,0.3);
    border-top: 1px solid #2e4053;
    ...
    transition: all 0.3s ease;
        ...
        `);
		 ///////////////////////// Joystick
        GM_addStyle(`
.ct-joystick-container {
    position: fixed !important;
    bottom: 40px !important; /* Thumb height */
    left: 30px !important;   /* Thumb reach */
    width: 120px;
    height: 120px;
...
.joy-up { top: 10px; left: 47px; }
.joy-down { bottom: 10px; left: 47px; }
.joy-left { left: 10px; top: 47px; }
.joy-right { right: 10px; top: 47px; }
.joy-active { opacity: 0.9; box-shadow: 0 0 10px #00ff00; background: #00ff00; }
