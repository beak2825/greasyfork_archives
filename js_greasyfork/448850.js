...
// @downloadURL https://update.greasyfork.org/scripts/448850/%E5%9B%9B%E7%89%B9%E6%9D%B1%E5%96%B0%E8%B5%A4%E9%80%9F%E6%9A%97.user.js
// @updateURL https://update.greasyfork.org/scripts/448850/%E5%9B%9B%E7%89%B9%E6%9D%B1%E5%96%B0%E8%B5%A4%E9%80%9F%E6%9A%97.meta.js
// ==/UserScript==
...
var MARK_TSUMOGIRI = true;
//HAND EVALUATION CONSTANTS
var EFFICIENCY = 0.9; // Lower: Slower and more expensive hands. Higher: Faster and cheaper hands. Default: 1.0, Minimum: 0
var SAFETY = 0.8; // Lower: The bot will not pay much attention to safety. Higher: The bot will try to play safer. Default: 1.0, Minimum: 0
var SAKIGIRI = 1.0; //Lower: Don't place much importance on Sakigiri. Higher: Try to Sakigiri more often. Default: 1.0, Minimum: 0

//CALL CONSTANTS
var CALL_PON_CHI = 1.2; //Lower: Call Pon/Chi less often. Higher: Call Pon/Chi more often. Default: 1.0, Minimum: 0
...
var DEBUG_BUTTON = true; //Display a Debug Button in the GUI
...
function initGui() {
	...
	hideButton.innerHTML = "チェック";
	hideButton.onclick = function () {
		displaytwo("下家聴牌確率: " + Number(isPlayerTenpai(1) * 100).toFixed(1) + "%, 期待打点:" + Number(getExpectedHandValue(1).toFixed(0)) + ", 進攻性：" + Number(isPlayerPushing(1)).toFixed(2));
        displaythree("対面聴牌確率: " + Number(isPlayerTenpai(2) * 100).toFixed(1) + "%, 期待打点: " + Number(getExpectedHandValue(2).toFixed(0)) + ", 進攻性：" + Number(isPlayerPushing(2)).toFixed(2));
        displayone("上家聴牌確率: " + Number(isPlayerTenpai(3) * 100).toFixed(1) + "%, 期待打点: " + Number(getExpectedHandValue(3).toFixed(0)) + ", 進攻性：" + Number(isPlayerPushing(3)).toFixed(2));
        showCrtStrategyMsg(getExpectedHandValue(player));
	};
...
function shouldFold(tile, highestPrio = false) {
	if (tile.shanten * 4 > tilesLeft) {
		if (highestPrio) {
			showCrtActionMsg("Hand is too far from tenpai before end of game. Fold!");
	...
	if (tile.danger > foldThreshold) {
		if (highestPrio) {
			showCrtActionMsg("Tile Danger " + Number(tile.danger).toFixed(2) + " of " + getTileName(tile.tile, false) + " is too dangerous.");
...
function shouldRiichi(tilePrio) {
	...
	if (strategy == STRATEGIES.CHIITOITSU) {
		if (tilePrio.shape == 0) {
			showCrtActionMsg("Decline Riichi because of chiitoitsu wait that can be improved!");
			return false;
		}
	...
	if (strategy == STRATEGIES.THIRTEEN_ORPHANS) {
		showCrtActionMsg("Decline Riichi because of Thirteen Orphan strategy.");
		return false;
	}

	//Close to end of game
	if (tilesLeft <= 7 - RIICHI) {
		showCrtActionMsg("Decline Riichi because close to end of game.");
		return false;
	}

	//No waits
	if (tilePrio.waits < 1) {
		showCrtActionMsg("Decline Riichi because of no waits.");
		return false;
	}
	...
	if (isLastGame() && getDistanceToLast() > 0 && getDistanceToLast() < tilePrio.score.riichi) {
		showCrtActionMsg("Accept Riichi because of last place in last game.");
		return true;
	}
	...
	if (isLastGame() && (getDistanceToFirst() < -10000 || (tilePrio.yaku.closed >= 1 && getDistanceToFirst() < 0))) {
		showCrtActionMsg("Decline Riichi because of huge lead in last game.");
		return false;
	}
	...
	if (seatWind != 1 && badWait && tilePrio.score.riichi < 4000 - (RIICHI * 1000) && !lotsOfDoraIndicators && tilePrio.shape > 0.4) {
		showCrtActionMsg("Decline Riichi because of worthless hand, bad waits and not dealer.");
		return false;
	}

	// High Danger and hand not worth much or bad wait
	if (tilePrio.score.riichi < (getCurrentDangerLevel() - (RIICHI * 1000)) * (1 + badWait)) {
		showCrtActionMsg("Decline Riichi because of worthless hand and high danger.");
		return false;
	}
	...
	if (tilePrio.yaku.closed >= 1 && tilePrio.score.closed / (seatWind == 1 ? 1.5 : 1) > 4000 + (RIICHI * 1000) + (tilePrio.waits * 500)) {
		showCrtActionMsg("Decline Riichi because of high value hand with enough yaku.");
		return false;
	}

	// Hand already has high value and no yaku
	if (tilePrio.yaku.closed < 0.9 && tilePrio.score.riichi > 5000 - (RIICHI * 1000)) {
		showCrtActionMsg("Accept Riichi because of high value hand without yaku.");
		return true;
	}

	// Number of Kans(Dora Indicators) -> more are higher chance for uradora
	if (lotsOfDoraIndicators) {
		showCrtActionMsg("Accept Riichi because of multiple dora indicators.");
		return true;
	}
	...
	if (isLastGame() && badWait && ((getDistanceToPlayer(1) >= -1000 && getDistanceToPlayer(1) <= 0) ||
		...
		showCrtActionMsg("Decline Riichi because distance to next player is < 1000 in last game.");
		return false;
	}

	// Default: Just do it.
	showCrtActionMsg("Accept Riichi by default.");
	return true;
}
...
async function callTriple(combinations, operation) {
	...
	if (isSameTile(nextDiscard, getTileForCall()) ||
		...
		declineCall(operation);
		showCrtActionMsg("Next discard would be the same tile. Call declined!");
		return false;
	}

	if (strategy == STRATEGIES.FOLD || tilePrios.filter(t => t.safe).length == 0) {
		showCrtActionMsg("Would fold next discard! Declined!");
		declineCall(operation);
		return false;
	}

	if (tilesLeft <= 4 && handValue.shanten == 1 && newHandValue.shanten == 0) { //Call to get tenpai at end of game
		showCrtActionMsg("Accept call to be tenpai at end of game!");
	...
	if (newHandValue.yaku.open < 0.15 && //Yaku chance is too bad
		...
		showCrtActionMsg("Not enough Yaku! Declined! " + newHandValue.yaku.open + " < 0.15");
		declineCall(operation);
		return false;
	}

	if (handValue.waits > 0 && newHandValue.waits < handValue.waits + 1) { //Call results in worse waits 
		showCrtActionMsg("Call would result in less waits! Declined!");
		declineCall(operation);
		return false;
	}
	...
	if (isClosed && newHandValue.score.open < 1500 - (CALL_PON_CHI * 200) && newHandValue.shanten >= 2 + CALL_PON_CHI && seatWind != 1 &&// Hand is worthless and slow and not dealer. Should prevent cheap yakuhai or tanyao calls
		...
		showCrtActionMsg("Hand is cheap and slow! Declined!");
		declineCall(operation);
		return false;
	}
	...
	if (newHandValue.shanten > handValue.shanten) { //Call would make shanten worse
		showCrtActionMsg("Call would increase shanten! Declined!");
		declineCall(operation);
		return false;
	}
		...
		if (!isClosed && newHandValue.priority > handValue.priority * 1.5) { //When the call improves the hand
			showCrtActionMsg("Call accepted because hand is already open and it improves the hand!");
		}
		else {
			declineCall(operation);
			showCrtActionMsg("Call declined because it does not benefit the hand!");
			return false;
		}
	}
		...
		if (handValue.shanten >= 5 - CALL_PON_CHI && seatWind == 1) { //Very slow hand & dealer? -> Go for a fast win
			showCrtActionMsg("Call accepted because of slow hand and dealer position!");
		}
			...
			showCrtActionMsg("Call accepted because hand is already open!");
		}
			...
			showCrtActionMsg("Call accepted because of high value hand!");
		}
			...
			showCrtActionMsg("Call accepted because it boosts the value of the hand!");
		}
			...
			showCrtActionMsg("Call accepted because it reduces shanten!");
		}
			...
			showCrtActionMsg("Call accepted because it eliminates a bad wait and makes the hand ready!");
		}
			...
			showCrtActionMsg("Call accepted because it's good in multiple aspects");
		}
		else { //Decline
			declineCall(operation);
			showCrtActionMsg("Call declined because it does not benefit the hand!");
...
function discardFold(tiles) {
	if (strategy != STRATEGIES.FOLD) { //Not in full Fold mode yet: Discard a relatively safe tile with high priority
		...
		showCrtActionMsg("Hand is very dangerous, full fold.");
		strategyAllowsCalls = false;
	}
