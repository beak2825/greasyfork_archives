...
// @grant        none
// @description  wow
// @downloadURL https://update.greasyfork.org/scripts/374124/LuckerAlt.user.js
// @updateURL https://update.greasyfork.org/scripts/374124/LuckerAlt.meta.js
// ==/UserScript==
//
...
setTimeout(()=>{
	...
	$(document).ready(() => {
		...
		function PickPhrase(TextArray,UsedArray){
		...
		function DoShit(){
			let msgtime = randomInteger(50,75);
			var Typer = setInterval(function Typer(){
				if(fiftycounter>=msgtime){
					fiftycounter = 0;
					msgtime = randomInteger(50,75);

					let Choosen = PickPhrase(TextArr,UsedArr);
			...
			$(document).arrive(".messages li:last-child", function() {
			    ...
			    UnderText = lastmsg.children(".msg-body");//chat_cont for 2.0 
			    var KekUndText = UnderText.text();
			    var MessageUndText = UnderText.text().toLowerCase();
			    if((msgnickname.includes("Rainbot") && MessageUndText.includes("пользователям") && filebool==true))
			    {
			    	let Choosen = PickPhrase(TextArr,UsedArr);
			    	if(sleepmoscow==false)
			    	{
			    ...
			    if(!lastmsg.hasClass("msg-moderator-message")){
			    	...
			    	console.log(`сообщения : ${fiftycounter}`);
			    }	
