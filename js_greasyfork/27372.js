// ==UserScript==
// @name        HIT Scraper WITH EXPORT - With next accept
// @author      feihtality, saqfish
...
// @version     4.0.0703
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/27372/HIT%20Scraper%20WITH%20EXPORT%20-%20With%20next%20accept.user.js
// @updateURL https://update.greasyfork.org/scripts/27372/HIT%20Scraper%20WITH%20EXPORT%20-%20With%20next%20accept.meta.js
// ==/UserScript==
...
(function() {
    ...
    Object.deepAssign = function(target, src) {
        for (var key in src) { if (src.hasOwnProperty(key)) {
            if (src[key] && typeof src[key] === 'object')
                target[key] = Object.deepAssign({}, src[key]);
            else
                target[key] = src[key];
        }}
        return target;
    ...
    var ico = 'data:image/png;base64,',
        ...
        Settings = {//{{{
            defaults: {//{{{
                ...
                searchBy: 0, invert: false, shine: '300', minTOPay: '',minAcceptTO: '', hideNoTO: false, onlyViable: false,
                disableTO: false, sortPay: false, sortAll: false, search: '', hideBlock: true, 
                onlyIncludes: false, acceptNext: false, shineInc: true, sortAsc: false, sortDsc: true, gbatch: false, bubbleNew: false,
...
${cellFix.row('toPoor')}    ${cellFix.text('toPoor')}    ${cellFix.export('toPoor')}    ${cellFix.vlink('toPoor')}`;
        ...
        aaTing={
            box:null, bigbox:null, showing:false
        },
            ...
            draw: function() {//{{{
                var user = this.user = Settings.user, _cb = user.showCheckboxes ? '' : 'hidden',
                    ...
                    titles = {//{{{
                        ...
                        minAcceptTO: "After getting search results, Don't auto accept results below this average Turkopticon rating.\n" +
                        "Minimum is 1, maximum is 5, decimals up to 2 places, such as 3.25",
                        ...
                        acceptNext: "Accept the next hit that comes in.",
...
<label class="${user.ff ? 'checked' : ''}" title="${titles.onlyIncludes}" for="onlyIncludes">Restrict to includelist</label>
<input id="onlyIncludes" class="${_cb}" type="checkbox" title="${titles.onlyIncludes}" ${user.onlyIncludes ? 'checked' : ''} /><i></i>            

<label class="${user.acceptNext ? 'checked' : ''}" title="${titles.acceptNext}" for="acceptNext">Accept Next</label>
<input id="acceptNext" class="${_cb}" type="checkbox" title="${titles.acceptNext}" ${user.acceptNext ? 'checked' : ''} /><i></i>
Min Accept TO: <input id="minAcceptTO" type="number" title="${titles.minAcceptTO}" min="1" max="5" step="0.25" value="${user.minAcceptTO}" /><i></i>
...
<tbody></tbody>
            ...
            init: function() {//{{{
                ...
                $('#acceptNext').click(() => {if(!Settings.user.acceptNext){
                    $('body').append("<div id='dialogBox' style='padding-top: 5px; padding-bottom: 5px; padding-left: 5px; padding-right: 5px; background-color:black;'><div id='stuffArea'><table id='aaTable'></table></div></div>");
                    $('#dialogBox').css({"position": "absolute","top": 60,"left": 260, "border" :" 3px solid green"});
                    $('#stuffArea').css({"height":"300px", "width":"600px", "overflow":"auto"});
                    $('#dialogBox').append("<div id='buttonsTab' style='background-color:grey;'><button id='dialogCancel'>Close</button> Click to move</div>");
                    $('#dialogCancel').css({"border" :" 3px solid red"});
                    aaTing.box = $('#aaTable');
                    aaTing.bigbox = $('#dialogBox');
                    aaTing.showing = true;
                    $('#dialogCancel').click(()=>{aaTing.showing = false; aaTing.bigbox.remove();});

                    function eM() {$('#dialogBox').removeClass('mov');}
                    function sM() {
                        $('.mov').on('mousemove', function(event) {
                            var tX = event.pageX - $('#dialogBox').width() / 2,
                                tY = event.pageY - $('#dialogBox').height();
                            $('.mov').offset({
                                left: tX,
                                top: tY
                            });
                    ...
                    $("#buttonsTab").on('mousedown', function() {
                        $('#dialogBox').addClass('mov');
                        sM();
                    }).on('mouseup', function() {
                        $('#dialogBox').removeClass('mov');
                        eM();
                    });
                }});
...
data-cmp-index="requesterName" style="text-align:center;cursor:default">T</td>` +
                ...
                for (var r of (Settings.user.bubbleNew ? bubbleNewHits(results) : results)) { 
                    var shouldHide = Boolean((Settings.user.hideBlock && r.blocked) || (Settings.user.hideNoTO && !r.TO) || 
                                             ...
                                             (Settings.user.minAcceptTO && r.TO && +r.TO.attrs.qual < +Settings.user.minAcceptTO));
                    ...
                    var _st = Interface.isLoggedout ? 'disabled' : '';
                    var _sh = ex => Settings.user['export'+ex] ? '' : 'hidden';
                    if (Settings.user.acceptNext) {
                        if(r.isNew && !shouldHide){
                            var xhr2 = new XMLHttpRequest();
                            xhr2.open('GET', r.hit.panda, true);
                            xhr2.send();
                            var getTime = () => {var dt = new Date(); return dt.getHours() + ":" + dt.getMinutes()};
                            if(aaTing.showing) aaTing.box.prepend(`<tr><td><div><button class="ex vb ${_st} ${_sh('Vb')}" style="width:30px" data-gid="${r.groupId}">vB</button></td> <td class="aaHit">${getTime()}</td><td class="aaHit">${r.pay}</td><td class="aaHit">${r.requester.name}</td><td class="aaHit"><a href="${r.hit.panda}">${r.title.replace(/"/g,'&quot;')}</a></td></tr>`);
                            $('.aaHit').css({"border" :" 1px solid #566573", "padding": "4px"});
                            //v => v.onclick = e => new Exporter(e);
                        }
                    }
                }
                ...
                Array.from(document.querySelectorAll('.ex')).forEach(v => v.onclick = e => new Exporter(e));
                ...
                if ((Date.now() - Interface.time)/1000 > 1200) scraperHistory.prune();
            },//}}}
...
<dt>qualifications</dt><dd>${obj.quals}</dd></dl>`;
                ...
                function Archive() {//{{{
                    var _ratings = { global: { comm: 0, pay: 0, fair: 0, fast: 0, reviews: 0 } };
                    this.hitGroups = {};
                    this.ratings = Object.deepAssign({},_ratings);
                    this.prune = function() {
                        if (/\d{2}/.exec(new Date())[0] !== /\d{2}/.exec(new Date(Interface.time))[0]) {
                            this.ratings = Object.deepAssign({},_ratings);
                        }
                        Interface.time = Date.now();
                    ...
                    this.addReviews = function(obj) {
                        ...
                        for (var k of rids) {
                            ...
                            for (attr of Object.keys(Settings.user.toWeights)) this.ratings.global[attr] += obj[k].attrs[attr] * obj[k].reviews;
                        }
                        for (var g of groups) {
