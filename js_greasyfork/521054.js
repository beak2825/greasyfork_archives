// ==UserScript==
// @name         Save TEMU Keyword Search XHR
...
// @license     MIT
// @downloadURL https://update.greasyfork.org/scripts/521054/Save%20TEMU%20Keyword%20Search%20XHR.user.js
// @updateURL https://update.greasyfork.org/scripts/521054/Save%20TEMU%20Keyword%20Search%20XHR.meta.js
// ==/UserScript==

(function() {
    'use strict';

    let targetUrl = "api/poppy/v1/search?scene=search";
    let targetUrlBottom = "/api/poppy/v1/opt?scene=opt_landing_bottom_rec";

    let savedData = GM_getValue('savedData', {});
    if (!window.location.pathname.includes('login')) {
        let searchParams = new URLSearchParams(window.location.search);
        let searchKey = searchParams.get('search_key') || 'default'; // 如果没有 search_key，则设置为默认值
        searchKey = decodeURIComponent(searchKey).replace(/\s+/g, '_'); // 解码并将空格替换为下划线
        GM_setValue('fileName', searchKey);
    }
    ...
    function updateCounterCountryDisplay() {
        ...
        if (lowerPriceRange != "") {
            ...
            priceDisplay.textContent = "NO PRICE RANGE IS SELECTED.";
        }
