    ...
    const App = {
        data() {
            return {
                ...
                configSetting: Object.assign({
                    ...
                    waitSeconds: 5,
                    abandonPriceRule: [],
                }, GM_getValue("configSetting"), { "Cookie": document.cookie }),
        ...
        methods: {
            ...
            getUserInfo: function () {
                ...
                // 定义请求的URL
                const url = 'https://seller.kuajingmaihuo.com/bg/quiet/api/mms/userInfo';
                // 定义要发送的数据
                const data = {};
                // 使用fetch API发起POST请求
                fetch(url, {
            ...
            SMZQ_isAbandonPriceRule: function (supplyPrice, suggestSupplyPrice) {
                ...
                for (let index = 0; index < this.configSetting.abandonPriceRule.length; index++) {
                    let curRule = this.configSetting.abandonPriceRule[index];
                    if (supplyPrice >= curRule.price && suggestSupplyPrice <= curRule.abandonPrice) {
                        isAbandon = true;
                        break;
                    }
                }
            ...
            SMZQ_searchForChainSupplier: function (pageNum, pageSize) {
                const configSetting = this.configSetting;
                // 定义请求的URL
                const url = 'https://seller.kuajingmaihuo.com/marvel-mms/cn/api/kiana/xmen/select/searchForChainSupplier';
            ...
            SMZQ_rejectRemark: function (orderId) {
                const configSetting = this.configSetting;
                // 定义请求的URL
                const url = 'https://seller.kuajingmaihuo.com/gmp/bg/magneto/api/price-review-order/no-bom/reject-remark';
                // 定义要发送的数据
                const data = {
                    orderId: orderId
                };
            ...
            SMZQ_reviewNoBom: function (priceOrderId) {
                const configSetting = this.configSetting;
                // 定义请求的URL
                const url = 'https://seller.kuajingmaihuo.com/gmp/bg/magneto/api/price-review-order/no-bom/review';
                // 定义要发送的数据
                const data = {
            ...
            SMZQ_bargainNoBom: function (postData) {
                const configSetting = this.configSetting;
                // 定义请求的URL
                const url = 'https://seller.kuajingmaihuo.com/marvel-mms/cn/api/kiana/magneto/price/bargain-no-bom';
            ...
            SMZQ_execute: async function () {
                ...
                if (configSetting.autoPage) {
                    await getDataList(1, 1).then(data => {
                ...
                Promise.all(arrDataList).then(async resource => {
                    let dataList = [];
                    ...
                    for (let i = 0; i < dataList.length; i++) {
                        ...
                        for (let j = 0; j < curData.skcList.length; j++) {
                            ...
                            for (let m = 0; m < curSkc.supplierPriceReviewInfoList.length; m++) {
                                ...
                                await rejectRemark(curSupplier.priceOrderId).then(data => {
                                    console.log("rejectRemark: ", data);

                                    if (data.success) {
                                        if (isAbandonPriceRule(data.result.supplyPrice, data.result.suggestSupplyPrice)) {
                                            IsChangePrice = false;
                                            curSuggestSupplyPrice = data.result.suggestSupplyPrice;
                                        }
                                    } else {
                                ...
                                if (!IsChangePrice) {
                                    ...
                                    await reviewPrice(curSupplier.priceOrderId).then(data => {
                                ...
                                await revisePrice(postData).then(data => {
            ...
            SPLB_pageQuery: function (pageNum, pageSize) {
                const configSetting = this.configSetting;
                // 定义请求的URL
                const url = 'https://seller.kuajingmaihuo.com/bg-visage-mms/product/skc/pageQuery';
            ...
            SPLB_signJIT: function (postData) {
                const configSetting = this.configSetting;
                // 定义请求的URL
                const url = 'https://seller.kuajingmaihuo.com/bg-visage-mms/product/agreement/batch/sign';
                ...
                Promise.all(arrDataList).then(async resource => {
                    ...
                    if (postGroup.length) {
                        ...
                        for (let index = 0; index < postGroup.length; index++) {
                            ...
                            _Vue.logList.push({
                                text: `等待${configSetting.waitSeconds}秒请求: 第${index + 1}次签署,签署${curPostData.length}个`
                            });
                            await waitSeconds(configSetting.waitSeconds);
                            await signJIT(curPostData).then(data => {
                                if (data.success) {
                                    _Vue.logList.push({
                                    ...
                                    if (data.result.failedDetailList.length) {
                                        for (let i = 0; i < data.result.failedDetailList.length; i++) {
