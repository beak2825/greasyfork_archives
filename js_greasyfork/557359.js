...
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/557359/YouTube%20Player%20Controls.user.js
// @updateURL https://update.greasyfork.org/scripts/557359/YouTube%20Player%20Controls.meta.js
// ==/UserScript==
...
// --- Redirección automática de /live/xxxx a /watch?v=xxxx ---
(function () {
    try {
        const path = window.location.pathname;

        // Coincide con /live/VIDEOID
        ...
        console.error("Live redirect error:", e);
    }
})();
...
function cinema(start_count) {
    ...
    var view_height = H - (hide ? 0 : search_height);
    var view_width = W;
    ...
    var pwidth = Number(pl.style.width.replace(/[^\d\.\-]/g, ''));
    var pheight = Number(pl.style.height.replace(/[^\d\.\-]/g, ''));
    ...
    // Ajuste para Touch Window from Inside: Ajustamos el video a la ventana sin cortarlo
    if (pratio < view_ratio) {
        // El video es más estrecho, llenamos la altura y centramos horizontalmente
        height = view_height;
        width = height * pratio;
        left = (view_width - width) / 2;
    } else {
        // El video es más ancho, llenamos el ancho y centramos verticalmente
        width = view_width;
        height = width / pratio;
        top = (view_height - height) / 2;
    }
