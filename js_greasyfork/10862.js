// ==UserScript==
// @name            Reddit/Voat Image Hover Zoom
...
// @grant           GM_log
// @version 0.0.1.20150710183236
// @downloadURL https://update.greasyfork.org/scripts/10862/RedditVoat%20Image%20Hover%20Zoom.user.js
// @updateURL https://update.greasyfork.org/scripts/10862/RedditVoat%20Image%20Hover%20Zoom.meta.js
// ==/UserScript==
try {
    var loading_gif =       "https://i.imgur.com/vSBeLww.gif";
    var css_overrides =     false;
    var img =               $('<img>').attr('src', loading_gif).attr('id', 'hover-popup');
    var thumb_selector =    'a.thumbnail';
    
	if (window.location.host.indexOf("voat") >= 0) {
		var static_parent = $('#container');
	} else {
	...
	function getSrc(src) {
		src = src.trim();
		var imgur = new RegExp(/^https?:\/\/(?:i\.)?imgur\.com\/(\w+)$/i);
		src = src.replace(imgur, "https://i.imgur.com/$1.png");
		var qkme = new RegExp(/^http:\/\/qkme\.me\/(\w+)$/i);
		src = src.replace(qkme, "http://i.qkme.me/$1.jpg");
		return src;
	}
