...
// @downloadURL https://update.greasyfork.org/scripts/40918/bangumi%20%E8%AF%84%E5%88%86%E5%8A%A9%E6%89%8B%20%CE%B1.user.js
// @updateURL https://update.greasyfork.org/scripts/40918/bangumi%20%E8%AF%84%E5%88%86%E5%8A%A9%E6%89%8B%20%CE%B1.meta.js
// ==/UserScript==
...
(function() {
    ...
    // 用户主页的正则表达式
    let _homepageRE = new RegExp("https?:\/\/(bgm\.tv|bangumi\.tv|chii\.in)\/user\/" + _user + "$");
    // 初始化评分的参考
    let reference = {};
    ...
    if(location.href.match(/https?:\/\/(bgm\.tv|bangumi\.tv|chii\.in)\/subject.*/)) {
        // 获取在简评中的评分数据
        // 各项评分
        let variousScores = $(".SidePanel").text().match(/\$(.*)\$/);
        let singleValues = [];
        // 分数（原先的提交的分数或者计算后所得的分数）
        let realScore = $("input[name=rating]:checked").val() ? $("input[name=rating]:checked").val() : 0;
        // console.log(realScore);
        if (variousScores) {
            variousScores = variousScores[1];
        }
        //添加评分助手面板
        let addRA = function(PreviousElement) {
            let MAINBOX = $("<div><div class='head'></div></div>");
            // 创建 range
            ...
            let rangeList = [MAINBOX.find(".head")];
            let i = 0;
            // 添加 range, button 等内容
            for(let key in reference) {
                if (variousScores) {
                    let singleScore = variousScores[i] == "a" ? 10 : parseInt(variousScores[i]);
            ...
            calculateScore = function(round) {
                ...
                let i = 0;
                for(let key in reference) {
                    let singleValue = MAINBOX.find("input[name=" + key + "]").val();
                    let weight = reference[key];
            ...
            setvalueboxvalue = function() {
                let ranges = MAINBOX.find(".RA-range");
                if(ranges) {
            ...
            getRangeValue = function() {
                let ranges = MAINBOX.find(".RA-range");
                for(let i = 0;i < ranges.length;i++) {
                    singleValues[i] = ranges.eq(i).val() == 10 ? "a" : ranges.eq(i).val();
                }
            };
            // let rangeList = [$("#rate-tip").parent()];
            setvalueboxvalue();
            getRangeValue();
            ...
            setscoreBox = function(rangeinput) {
                console.log(variousScores);
                if (variousScores || rangeinput) {
                    let color = "green";
                    let _score = calculateScore();
                    // console.log(_score);
            ...
            setscoreBox();
            // 获取 range 评分
            MAINBOX.find(".RA-range").mousemove(function(){
                setscoreBox(true);
                getRangeValue();
                let comment = $("textarea[name=comment]").val();
                let singleValues_str = singleValues.join("");
            ...
            scoreBox.click(function() {
                ...
                let formhash = $("html").html().match(/formhash" value="(.*?)"/)[1];
                let interest = $("input[name=interest]:checked").val();
                let rate = realScore || $("input[name=rating]:checked").val();
                let tags = $("input[name=tags]").val();
                let comment = $("textarea[name=comment]").val();
                let privacy = $("input[name=privacy]").attr("checked") ? 1 : 0;
                let postDatas = {
                    formhash : formhash,
                    referer : "subject",
                    interest : interest,
            ...
            reset.click(function() {
                if(variousScores) {
                    ...
                    for(let key in reference) {
                        ...
                        MAINBOX.find("input[name=" + key + "]").val(tempScore);
                        i++;
                    }
                } else {
                    MAINBOX.find("input.RA-range").val(5);
                }
                setvalueboxvalue();
                setscoreBox(true);
            });
            scoreBox.after(reset);
            PreviousElement.after(MAINBOX);
        };
        addRA($("textarea#comment"));
        // 同时出现在两处时有 bug，未解决。
        // addRA($("#rate-tip").parent());
        $("input.inputBtn[name=update]").css("display","none");
    } else if(location.href.match(_homepageRE)) {
        /// 如果在个人主页
        // 评分助手设置
        let RAbox = $("<div><p class='RA-title'>bangumi 评分助手设置</p><div class='content'></div></div>");
        // 创建 input
        let createInput = function(val1, val2) {
        ...
        let subbmitBtn = $("<button>提交</button>");
        subbmitBtn.css({
            "width" : "100%",
            "height" : "25px",
