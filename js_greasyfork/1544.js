...
// @version        1.44
// @include        https://virtonomic*.*/*/window/unit/employees/engage/*
// @include        http://igra.aup.*/*/window/unit/employees/engage/*
// @downloadURL https://update.greasyfork.org/scripts/1544/virtonomica%3A%D0%97%D0%B0%D1%80%D0%BF%D0%BB%D0%B0%D1%82%D1%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/1544/virtonomica%3A%D0%97%D0%B0%D1%80%D0%BF%D0%BB%D0%B0%D1%82%D1%8B.meta.js
// ==/UserScript==
...
var input_f = $("<input> ").prop({type: 'text', value: '', size: '6'}).change(function() {
  ...
  container.append( $('<table id=btr>').append('<td>').append(input_95).append('<td>').append(input_f).append('<td>').append(input_plus).append('<td>').append(input_100).append('<td>').append(input_ret) );
  
  // проверка на наличие требований от профсоюзов
  var prof = $(".fa-bell");
  if ( prof.length > 0 ) {
    //console.log('Профсоюзы!!!');  
    var input_prof = $('<td><button id=prof style="font-size:125%" title="Как хочет профсоюз"><i class="fa fa-bell"></i></button>').click(function() {
      // с удаление пробелов
      var str = $.trim( $(".fa-bell").parent().text().replace(/\s/g, '') );
      var pos = str.indexOf('$');
      if ( pos == -1) return;
      
      var prof_treb = parseFloat( str.substr(pos+1) );
      //console.log( prof_treb );
      var inp_sal = $("#salary");
      //console.log( inp_sal.eq(0) );
      inp_sal.val(prof_treb).blur();
      
      
    });
    $('#btr').append( input_prof );
  }
