// ==UserScript==
// @name         奥鹏教师教育网
// @namespace    https://greasyfork.org/zh-CN/scripts/奥鹏教师教育网
// @version      1.0.4
// @description  奥鹏教师教育网，学习界面自动挂机刷课
// @author       ZouYS
...
// @downloadURL https://update.greasyfork.org/scripts/523355/%E5%A5%A5%E9%B9%8F%E6%95%99%E5%B8%88%E6%95%99%E8%82%B2%E7%BD%91.user.js
// @updateURL https://update.greasyfork.org/scripts/523355/%E5%A5%A5%E9%B9%8F%E6%95%99%E5%B8%88%E6%95%99%E8%82%B2%E7%BD%91.meta.js
// ==/UserScript==
(function () {
    'use strict';
    // 只管时间而不管节点是否全部完成
    ...
    window.onload = function () {
        console.log("页面定时器捕捉：", window[stime])
        // 学习界面  目录节点
        const studyNodes = document.querySelectorAll(catalogSelecter);

        const study = async function (nodes) {
            ...
            for (const node of list) {
                ...
                if(onlyTime){
                    try {
                        const studyTime = getStudyTime(timeID)
                        console.log("学习时间:",studyTime)
                        const maxTime = getMaxStudyTime(timeID)
                ...
                node.click()
                const nodeType = getStudyNodesType(node)
                console.log("节点类型：", nodeType)
                if (nodeType !== 2) {
                    continue
                }
                const video = await getStudyVideoNode(videoNodeID)
                // 未找到视频
                if (!video) {
                    ...
                    await waitForVideoEnd(video); // 等待视频播放完毕
            ...
            if (studyTime < maxTime) {
                ...
                const intervalId = setInterval(async () => {
                    ...
                    // 再次检查学习时间
                    const newStudyTime = getStudyTime(timeID);
     ...
     * @returns {number}
     */
    const getStudyTime = function (timeId) {
     ...
     * @returns {number}
     */
    const getMaxStudyTime = function (timeId) {
     ...
     */
    const checkIframeForVideo = function (iframe) {
        return new Promise((resolve) => {
            // 监听 iframe 的加载完成事件
     ...
     */
    const getStudyVideoNode = async function (videoNodeID, timeout = 10000) {
        return new Promise(async (resolve, reject) => {
            ...
            // 检查 iframe 是否包含视频
            const hasVideo = await checkIframeForVideo(iframe);
            if (!hasVideo) {
    ...
    /**
     * 获取视频节点
     * @param {string} id - 视频元素的 ID
     * @param {number} timeout - 超时时间
     * @returns {Promise<HTMLElement>}
     */
    /*const getStudyVideoNode = function (id, timeout = 10000) {
        return new Promise((resolve, reject) => {
            // 视频容器
            const videoContainer = document.querySelector('.Course-video')
            if (!videoContainer) {
                reject(new Error("未找到视频容器"));
                return;
            }
            // 视频
     ...
     * @returns {number}
     */
    const getStudyNodesType = function (studyNode) {
        if (!studyNode.id) {
            return -1
        }
