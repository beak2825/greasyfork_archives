// ==UserScript==
// @name       				OCS 网课助手 *魔改版*
// @version    				4.7.32.5-魔改版-by信安2303-盛夏
// @description				OCS(online-course-script) 网课助手魔改版，支持  【知到智慧树】 【超星学习通】 【智慧职教】 【职教云】 【中国大学MOOC】 等网课的学习，作业。具体的功能请查看脚本悬浮窗中的教程页面。
// @author     				盛夏
// @license    				MIT
// @match      				*://*.zhihuishu.com/*
// @match      				*://*.chaoxing.com/*
...
// @icon       				http://q1.qlogo.cn/g?b=qq&nk=2539159006&s=640
// @connect           enncy.cn
// @connect           icodef.com
// @connect           ocsjs.com
// @connect           localhost
// @connect           every-api.com
// @connect           tikuhai.com
// @connect           lemtk.xyz
// @antifeature				payment
// @downloadURL https://update.greasyfork.org/scripts/483189/OCS%20%E7%BD%91%E8%AF%BE%E5%8A%A9%E6%89%8B%20%2A%E9%AD%94%E6%94%B9%E7%89%88%2A.user.js
// @updateURL https://update.greasyfork.org/scripts/483189/OCS%20%E7%BD%91%E8%AF%BE%E5%8A%A9%E6%89%8B%20%2A%E9%AD%94%E6%94%B9%E7%89%88%2A.meta.js
// ==/UserScript==
...
(function(global2, factory) {
        ...
        if (Object.prototype.hasOwnProperty.call(configs, key)) {
          ...
          if (config.label !== void 0) {
            const element = el("config-element", {
              ...
              sync: config.sync,
              attrs: config.attrs,
              _onload: function(el2) {
                var _a;
                (_a = config.onload) == null ? void 0 : _a.call(this, el2);
                onload == null ? void 0 : onload(el2);
              },
  ...
  function domSearchAll(wrapper, root2 = window.document) {
    ...
    Reflect.ownKeys(wrapper).forEach((key) => {
      const item = wrapper[key.toString()];
      // alert("item= " + typeof item === "string" ? Array.from(root2.querySelectorAll(item)));
      Reflect.set(
        obj,
        key,
  ...
  function splitAnswer(answer) {
    /*拆分答案*/
    try {
      const json = JSON.parse(typeof answer == 'object' ? answer : answer.replace(/\\n/g, ""));
      if (Array.isArray(json)) {
        return json.map(String).filter((el2) => el2.replace(/"/g, "").replace(/,/g, "").trim().length > 0);
      }
      if (typeof json === 'object' && json != null) {
        const values = Object.values(json);
        if (Array.isArray(values)) {
            return values.map(String).filter((el2) => el2.replace(/\"/g, "").replace(/\,/g, "").trim().length > 0);
        }
      }
    } catch {
      ...
      for (const sep of seprators) {
        if (answer.split(sep).length > 1) {
          return answer.split(sep).filter((el2) => el2.replace(/"/g, "").replace(/,/g, "").trim().trim().length > 0);
        }
      }
    }
    return [answer];
  ...
  function defaultQuestionResolve(ctx) {
    return {
      async single(infos, options, handler) {
        var allAnswer = infos.map((res) => res.results.map((res2) => splitAnswer(res2.answer)).flat()).flat();
        if (infos[0].url.includes("api.tikuhai.com") && typeof allAnswer == 'object'){
          allAnswer[0] =  allAnswer[0].match(/<img[^>]*\ssrc=["']([^"']+)["']/i)[1];
        }
    ...
    await Promise.all(
      temp.map(async (wrapper) => {
        ...
        try {
          ...
          let requestData;
          let url;
          const currentDomain = wrapper.url.match(/^(?:https?:\/\/)?(?:[^@\n]+@)?(?:www\.)?([^:/\n?]+)?/)[1];
          if (method.toLocaleLowerCase() === "get") {
            if(currentDomain == "go.every-api.com"){
              headers.Authorization = headers.Authorization.replace("${AiToken}", CommonProject.scripts.settings.cfg["AIToken"]); // 设置token
              const options_match = decodeURIComponent(decodeURIComponent(env["titleAi"]));
              if(options_match.match(/https:\/\/[^\s]*\.png/g)  != null && options_match.match(/https:\/\/[^\s]*\.mp3/g) != null){
                url = new URL(resolvePlaceHolder(wrapper.url, "gpt-4", { encodeURI: true }, true, true, true));
              }else if(options_match.match(/https:\/\/[^\s]*\.png/g) != null){
                url = new URL(resolvePlaceHolder(wrapper.url, "gpt-4", { encodeURI: true }, true, true, false));
              }else if(options_match.match(/https:\/\/[^\s]*\.mp3/g) != null){
                url = new URL(resolvePlaceHolder(wrapper.url, "gpt-4", { encodeURI: true }, true, false, true));
              }else{
                url = new URL(resolvePlaceHolder(wrapper.url, CommonProject.scripts.settings.cfg.GPTModel, { encodeURI: true }, true, false, false));
              }
            }else{
              url = new URL(resolvePlaceHolder(wrapper.url, null, { encodeURI: true }, false, false, false)); // 封装url，替换字符串中的占位符
            }
            ...
            Object.keys(wrapperData).forEach((key) => {
              if (typeof wrapperData[key] === "object" && Reflect.has(wrapperData[key], "handler")) {
                ...
                if (result) {
                  Reflect.set(data, key, handler2(env));
                }
              } else {
            ...
            requestData = data;
            if (currentDomain == "api.tikuhai.com" && env["titleImg"].length > 0){
              if (typeof env["titleImg"] == 'object'){
                const i = 0;
                const title_url = requestData["question"].match(/(https?:\/\/[^\s,]+\.png)/gi);
                while(i <= env["titleImg"].length){
                  const title_image_url = env["titleImg"][i].match(/<img[^>]*\ssrc=["']([^"']+)["']/i)[1];
                  if(title_image_url == title_url[i]){
                    var question = requestData["question"];
                    question = question.replace(title_url[i], env["titleImg"]);
                    requestData["question"] = question;
                  }
                  i++;
                };
              }else if(typeof env["titleImg"] == 'string'){
                const title_url = requestData["question"].match(/(https?:\/\/[^\s,]+\.png)/gi)[0];
                const title_image_url = env["titleImg"].match(/<img[^>]*\ssrc=["']([^"']+)["']/i)[1];
                var question = requestData["question"];
                question = question.replace(title_url, env["titleImg"]);
                requestData["question"] = question;
              }
              
              requestData["options"] = JSON.parse(requestData["options"]);
            }
          ...
          }else if(currentDomain == "go.every-api.com" && responseData.code === -1){
            $console.error(responseData.msg);
          }else if(currentDomain == "go.every-api.com" && responseData.choices[0].index === 0){
            $console.info("当前模式: " + responseData.model);
            $console.info("扣除点数: " + responseData.usage.total_tokens);
          }
          ...
          searchInfos.push({
            ...
            response: void 0,
            data: void 0,
            error: (error == null ? void 0 : (error.message ? error.message : (error.msg ? error.msg : void 0))) || "\u9898\u5E93\u8FDE\u63A5\u5931\u8D25"
          });
        }
      })
    );
    function resolvePlaceHolder(str, model, options, Ai, image, audio) {
      let value;
      if (typeof str === "string") {
        ...
        matches.forEach((placeHolder) => {
          if(Ai){
            // Ai题库
            const type = env["type"];
            if(type == "single" || type == "multiple" || type == "line" || type == "completion"){
              const title_options = decodeURIComponent(decodeURIComponent(env["options"]));
              const titleAi = decodeURIComponent(decodeURIComponent(env["titleAi"]));
              // const urlRegex = /(https:\/\/[^\s]*\.png)/g;
              // var matchs = urlRegex.exec(titleAi);
              // // alert(matchs);
              // const titleAi_result = (matchs ? matchs[0] : "") + titleAi.replace(urlRegex, "").trim();
              env["options"] = options.replace(/\//g, "%2F").replace(/\t/,"").replace(/×/g, "");
              env["titleAi"] = titleAi_result.replace(/\//g, "%2F").replace(/_/g, "%5f").replace(/\t/,"");
              if(image && audio){
                value = "不要说多的只把答案告诉我,这是听力题在图片url里请解析图片里的题目和解析音频url里的内容并给出答案,如果是英语题目则给出英文答案(选择题和判断题除外)否则中文答案,并且把答案输出成json格式(必须单层json),题目:" + env["titleAi"] + " " + env["options"];
              }
              if(image){
                value = "不要说多的只把答案告诉我,请用url编码解码图片url并读取图片里的题目并给出答案,如果是英语题目则给出英文答案(选择题和判断题除外)否则中文答案,并且把答案输出成json格式(必须单层json),题目:" + env["titleAi"] + " " + env["options"];
              }
              if(audio){
                value = "不要说多的只把答案告诉我,这是听力题请解析音频url里的内容并根据题目给出答案,如果是英语题目则给出英文答案(选择题和判断题除外)否则中文答案,并且把答案输出成json格式(必须单层json),题目:" + env["titleAi"] + " " + env["options"];
              }
              if(image === false && audio === false){
                value = "不要说多的只把答案告诉我,如果是英语题目则给出英文答案(选择题和判断题除外)否则中文答案,并且把答案输出成json格式(必须单层json),题目:" + env["titleAi"] + " " + env["options"];
              }
            }else{
              value = "不要说多的只把答案告诉我,如果是英语题目则给出英文答案,否则中文答案,并且把答案输出成json格式(必须单层json),题目:" + env["titleAi"] + " " + env["options"];
            }
          }else{
            //普通题库
            if(str.match(/^(?:https?:\/\/)?(?:[^@\n]+@)?(?:www\.)?([^:/\n?]+)?/)[1] === "q.icodef.com"){
              value = decodeURIComponent(decodeURIComponent(env["title"])).replace(/\t/g, "").replace(/\//g, "%2F");
            }else {
              value = decodeURIComponent(decodeURIComponent(env["title"])).replace(/\t/g, "");    // placeHolder.replace(/\${(.*)}/, "$1")]
            };
          }
          if(placeHolder === "\${model}"){
            str = str.replace(placeHolder, model);
          }
          if(placeHolder === "\${title}"){
            str = str.replace(placeHolder, (options == null ? void 0 : options.encodeURI) ? encodeURIComponent(value) : value);
          }
          if(placeHolder == "\${options_array}"){
            var i = 0;
            ...
            const str_array = JSON.stringify(options);
            str = str.replace(placeHolder, str_array);
          }
        });
      }
      return str;
    ...
    scripts: {
      guide: new Script({
        ...
        onrender({ panel: panel2 }) {
          ...
          panel2.body.replaceChildren(el("div", { className: "card" }, [gotoHome(), contactUs, changeLog]), guide);
        }
      }),
      settings: new Script({
        name: "\u2699\uFE0F \u5168\u5C40\u8BBE\u7F6E",
        ...
        configs: {
                    ...
                    el("div", { style: { display: "flex", flexWrap: "wrap", marginTop: "12px", fontSize: "12px" } }, [
                      ...
                      el("div", { style: { flex: "1", display: "flex", flexWrap: "wrap", justifyContent: "end" } }, [
                        ...
                        el("button", "\u786E\u5B9A", (btn) => {
                          ...
                          btn.onclick = async () => {
                            var _a;
                            const connects = (_a = $gm.getInfos()) == null ? void 0 : _a.script.connects;
                              ...
                              try {
                                ...
                                if (select.value === "TikuAdapter") {
                                  if (value.startsWith("http") === false) {
                                    $modal("alert", {
                                      content: "\u683C\u5F0F\u9519\u8BEF\uFF0CTikuAdapter\u89E3\u6790\u5668\u53EA\u80FD\u89E3\u6790 url \u94FE\u63A5\uFF0C\u8BF7\u91CD\u65B0\u8F93\u5165\uFF01\u6216\u8005\u67E5\u770B https://github.com/DokiDoki1103/tikuAdapter \u6587\u6863\u8BF4\u660E"
                                    });
                                    return;
                                  }
                                  select.value = "\u9ED8\u8BA4";
          ...
          GPTModel:{
            ...
            options: [
                  ["gpt-3.5-turbo", "GPT3.5-turbo", "\u4EF7\u683C: 50\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"],
                  ["gpt-3.5-turbo-0301", "GPT3.5-turbo-0301", "\u4EF7\u683C: 50\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"],
                  ["gpt-3.5-turbo-0613", "GPT3.5-turbo-0613", "\u4EF7\u683C: 50\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"],
                  ["gpt-3.5-turbo-1106", "GPT3.5-turbo-1106", "\u4EF7\u683C: 50\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"],
                  ["gpt-3.5-turbo-16k", "GPT3.5-turbo-16k", "\u4EF7\u683C: 50\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"],
                  ["gpt-3.5-turbo-16k-0613", "GPT3.5-turbo-16k-0613", "\u4EF7\u683C: 50\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"],
                  ["gpt-4", "GPT4", "\u4EF7\u683C: 750\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"],
                  ["gpt-4-0314", "GPT4-0314", "\u4EF7\u683C: 750\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"],
                  ["gpt-4-0613", "GPT4-0613", "\u4EF7\u683C: 750\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"],
                  ["gpt-4-1106-preview", "GPT4-1106-preview", "\u4EF7\u683C: 300\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"],
                  ["gpt-4-32k", "GPT4-32k", "\u4EF7\u683C: \u672A\u77E5\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"],
                  ["gpt-4-32k-0314", "GPT4-32k-0314", "\u4EF7\u683C: \u672A\u77E5\u70B9\u6570, \u6587\u672C\u6570\u91CF: 1000Token"]
                  ]
          },
          "AIToken":{
            defaultValue: "",
            ...
            notificationBySetting: (content, opts) => {
              ...
              if (this.cfg.notification !== "no-notify") {
                $gm.notification(content, {
                  ...
                  duration: (opts == null ? void 0 : opts.duration) || 30,
        ...
        onrender({ panel: panel2 }) {
          if ($gm.getInfos() !== void 0) {
            ...
            panel2.body.append(el("div", { style: { display: "flex" } }, [testNotification, refresh]), tableContainer);
      ...
      onlineSearch: new Script({
        ...
        onrender({ panel: panel2 }) {
          ...
          panel2.body.append(el("div", [content, searchContainer]));
        }
      }),
      render: RenderScript,
      hack: new Script({
...
\u4E3B\u9875\uFF1A${c.homepage || "\u672A\u77E5\u4E3B\u9875"}`,
    ...
    scripts: {
      console: new Script({
        ...
        onrender({ panel: panel2 }) {
          ...
          this.offConfigChange(state$5.console.listenerIds.logs);
          state$5.console.listenerIds.logs = this.onConfigChange("logs", (logs2) => {
      ...
      appConfigSync: new Script({
        ...
        onrender({ panel: panel2 }) {
          ...
          update();
          this.offConfigChange(state$5.app.listenerIds.sync);
          this.offConfigChange(state$5.app.listenerIds.connected);
          this.offConfigChange(state$5.app.listenerIds.closeSync);
          state$5.app.listenerIds.sync = this.onConfigChange("sync", update);
          state$5.app.listenerIds.connected = this.onConfigChange("connected", update);
          state$5.app.listenerIds.closeSync = this.onConfigChange("closeSync", (closeSync) => {
            if (closeSync) {
              this.cfg.sync = false;
                    ...
                    if (Object.prototype.hasOwnProperty.call(project.scripts, key)) {
                      ...
                      for (const ck in script.configs) {
                        if (Object.prototype.hasOwnProperty.call(script.configs, ck)) {
                          if (((_a = script.configs[ck].extra) == null ? void 0 : _a.appConfigSync) === false) {
                    ...
                    if (Object.prototype.hasOwnProperty.call(project.scripts, key)) {
                      ...
                      script.onrender = ({ panel: panel2, header }) => {
                        ...
                        if (panel2.configsContainer.children.length) {
                          ...
                          panel2.lockWrapper.style.width = ((_a2 = panel2.configsContainer.clientWidth) != null ? _a2 : panel2.clientWidth) + "px";
                          panel2.lockWrapper.style.height = ((_b = panel2.configsContainer.clientHeight) != null ? _b : panel2.clientHeight) + "px";
            ...
            (_b = this.panel) == null ? void 0 : _b.body.append(
              el("div", { className: "card" }, [
                el("div", { style: { padding: "8px 0px", textAlign: "end" } }, [
                  ...
                  el("button", { className: "base-style-button", innerText: "\u{1F50D}\u6267\u884C\u641C\u7D22" }, (btn) => {
                    btn.onclick = () => {
                ...
                el(
                  ...
                  [
                    ...list.length === 0 ? [el("div", { style: { color: "white", textAlign: "center" } }, "\u6682\u65E0\u6570\u636E")] : [],
                    ...list.map(
                      (item) => el(
                        "div",
                        {
        ...
        onstart() {
          ...
          fetch = (input, init) => {
            if (this.cfg.enable) {
              ...
              res.then((result) => result.text()).then((result) => {
                setItem(id, result, "");
  ...
  function commonWork(script, options) {
    ...
    const start2 = async () => {
      ...
      (_d = (_c = script.panel) == null ? void 0 : _c.body) == null ? void 0 : _d.replaceChildren(container, workResultPanel());
  ...
  const $app_actions = {
    ...
    waitForResponse: async (url, options) => {
      try {
      ...
      return new Promise((resolve, reject) => {
        const interval = setInterval(async () => {
          try {
            const res = await appActionRequest(
              ...
              { baseUrl: "https://onlineexamh5new.zhihuishu.com/", responseType: (options == null ? void 0 : options.responseType) || "text" }
            );
            if (res) {
              clearInterval(interval);
              resolve(res);
        ...
        }, (options == null ? void 0 : options.period) || 3e3);
        setTimeout(() => {
          clearInterval(interval);
        }, (options == null ? void 0 : options.timeout) || 60 * 1e3);
      });
    },
    mouseMove: async (x, y) => {
    ...
    mouseClick: async (elementOrSelector, count = 1) => {
  ...
  async function appActionRequest(data, options) {
    ...
    try {
      const res = await request(
        ((options == null ? void 0 : options.baseUrl) || "http://localhost:15319") + "/ocs-script-actions?" + new URLSearchParams(data).toString(),
        {
          type: "fetch",
          method: "get",
          responseType: (options == null ? void 0 : options.responseType) || "text",
          headers: {
    ...
    scripts: {
      ...
      "gxk-study": new Script({
        ...
        async oncomplete() {
          ...
          const closeTestDialog = async () => {
            ...
            if ((_b = (_a = tip == null ? void 0 : tip.querySelector(".el-message-box__message")) == null ? void 0 : _a.textContent) == null ? void 0 : _b.includes("\u672A\u505A\u7B54\u7684\u5F39\u9898\u4E0D\u80FD\u5173\u95ED")) {
          ...
          const recordStudyTimeLoop = () => {
            var _a;
            this.methods.increaseStudyTime(((_a = $el(".source-name")) == null ? void 0 : _a.innerText) || "\u65E0\u540D\u79F0", this.cfg.playbackRate);
      ...
      "gxk-work": new Script({
        ...
        async onstart() {
          ...
          state$4.work.workInfo = await $app_actions.waitForResponse(url, {
            responseType: "json"
          });
        },
        async oncomplete() {
  ...
  async function watch(options, onended) {
    ...
    const set = async () => {
      ...
      state$4.study.currentMedia = video2;
      if (video2) {
  ...
  async function switchLine(definition2 = "line1bq") {
    const controlsBar = $el(".controlsBar");
    const dl = $el(".definiLines");
    if (controlsBar && dl) {
      controlsBar.style.display = "block";
      dl.style.display = "block";
      await $app_actions.mouseClick(`.definiLines .${definition2}`);
    }
  }
  async function switchPlaybackRate(playbackRate2) {
    const controlsBar = $el(".controlsBar");
    const sl = $el(".speedList");
    if (controlsBar && sl) {
      controlsBar.style.display = "block";
      sl.style.display = "block";
      await $app_actions.mouseClick(`.speedList [rate="${playbackRate2 === 1 ? "1.0" : playbackRate2}"]`);
    }
  }
  ...
  function waitForWorkInfo() {
    return new Promise((resolve, reject) => {
      const interval = setInterval(() => {
        if (state$4.work.workInfo) {
          clearInterval(interval);
          resolve(state$4.work.workInfo);
        }
      }, 1e3);
    });
  }
  function gxkWorkAndExam({
    ...
    const allExamParts = ((_e = ((_d = (_c = (_b = (_a = state$4 == null ? void 0 : state$4.work) == null ? void 0 : _a.workInfo) == null ? void 0 : _b.rt) == null ? void 0 : _c.examBase) == null ? void 0 : _d.workExamParts) || []) == null ? void 0 : _e.map((p) => p.questionDtos).flat()) || [];
    const titleTransform = (_, index2) => {
      var _a2;
      const div = el("div");
      div.innerHTML = ((_a2 = allExamParts[index2]) == null ? void 0 : _a2.name) || "\u9898\u76EE\u8BFB\u53D6\u5931\u8D25";
    ...
    const worker = new OCSWorker({
      ...
      answerer: (elements, type, ctx) => {
        const title = titleTransform(void 0, index++);
        if (title) {
      ...
      work: {
        type(ctx) {
          var _a2, _b2, _c2, _d2;
          const type = (_d2 = (_c2 = (_b2 = (_a2 = ctx.elements.title[0].parentElement) == null ? void 0 : _a2.parentElement) == null ? void 0 : _b2.querySelector(".subject_type")) == null ? void 0 : _c2.textContent) == null ? void 0 : _d2.trim();
      ...
      onResolveUpdate(res) {
        var _a2;
        if ((_a2 = res.result) == null ? void 0 : _a2.finish) {
    ...
    worker.doWork().then(async () => {
      ...
      for (let index2 = 0; index2 < worker.totalQuestionCount; index2++) {
        const modal = $modal("alert", { content: "\u6B63\u5728\u4FDD\u5B58\u9898\u76EE\u4E2D\uFF0C\u8BF7\u52FF\u64CD\u4F5C...", confirmButton: null });
        await waitForCaptcha();
        await $.sleep(2e3);
        (_a2 = document.querySelectorAll(".answerCard_list ul li").item(index2)) == null ? void 0 : _a2.click();
        await $.sleep(200);
        const next2 = $el("div.examPaper_box > div.switch-btn-box > button:nth-child(2)");
        if (next2) {
          next2.click();
        } else {
  ...
  function autoStop(stopTime) {
    ...
    if (stopTime !== "0") {
      ...
      state$4.study.stopMessage = $message("info", {
        duration: 0,
        ...
        onrender({ panel: panel2 }) {
          ...
          if (!((_a = CommonProject.scripts.settings.cfg.answererWrappers) == null ? void 0 : _a.length)) {
            ...
            if (state$3.study.answererWrapperUnsetMessage === void 0) {
              state$3.study.answererWrapperUnsetMessage = $message("warn", {
  ...
  function workOrExam(type = "work", { answererWrappers, period, thread, redundanceWordsText }) {
    ...
    const workOrExamQuestionTitleTransformImg = (titles) => {
      const optimizationTitle = titles.map((titleElement) => {
        if (titleElement) {
          ...
          for(var i=0; i < childNodes.length; i++){
            if (childNodes[i].nodeName == 'IMG'){
              optimizationElementWithImage(titleCloneEl).innerText;
              childNodesArray.push(childNodes[i].outerHTML);
            }
          }
    ...
    const workOrExamQuestionTitleTransformAi = (titles) => {
      const optimizationTitle = titles.map((titleElement) => {
        if (titleElement) {
          const titleCloneEl = titleElement.cloneNode(true);
          // const childNodes = titleCloneEl.childNodes;
    ...
    const worker = new OCSWorker({
      ...
      elements: {
        ...
        options: 'div[class="stem_answer"] div[class="clearfix answerBg"] div[class="fl answer_p"] p > img,.answerBg .answer_p, .textDIV, .eidtDiv',
      ...
      answerer: (elements, type2, ctx) => {
        if (elements.title) {
          ...
          const titleImg = workOrExamQuestionTitleTransformImg(elements.title);
          console.info(titleImg);
          const titleAi = workOrExamQuestionTitleTransformAi(elements.title);
          if (title && titleAi) {
            const typeInput = elements.type[0];
            return CommonProject.scripts.apps.methods.searchAnswerInCaches(title, () => {
              return defaultAnswerWrapperHandler(answererWrappers, {
                type: typeInput ? getQuestionType(parseInt(typeInput.value)) : void 0,
                title,
                titleImg,
                titleAi,
                options: ctx.elements.options.map((o) => o.innerText == null || o.innerText == "\n" ?  o.outerHTML : o.innerText).join("\n")
              });
            });
          } else {
            throw new Error("\u9898\u76EE\u4E3A\u7A7A\uFF0C\u8BF7\u67E5\u770B\u9898\u76EE\u662F\u5426\u4E3A\u7A7A\uFF0C\u6216\u8005\u5FFD\u7565\u6B64\u9898");
            ...
            if (!CXProject.scripts.study.cfg.enableMedia) {
              ...
              if (opts.restudy || attachment.job) {
                func = () => {
                  ...
                  return mediaTask(opts, media, doc);
                };
              }
            }
          } else if (chapterTest) {
              ...
              if (status == null ? void 0 : status.classList.contains("testTit_status_complete")) {
                ...
                if (attachment.job || CommonProject.scripts.settings.cfg["work-when-no-job"]) {
                  func = () => {
                    ...
                    return chapterTestTask(root2, opts.workOptions);
                  };
                }
                if (attachment.job === void 0 && CommonProject.scripts.settings.cfg["work-when-no-job"] === false) {
                  $console.warn(
  ...
  async function chapterTestTask(frame, { answererWrappers, period, upload, thread, stopSecondWhenFinish, redundanceWordsText }) {
    ...
    const chapterTestTaskQuestionTitleTransformAi = (titles) => {
      /*处理Ai题库题目标题*/
      const transformed = StringUtils.of(
        titles.map((t2) => t2 ? optimizationElementWithImage(t2).innerText : "").join(",")
        ).nowrap().nospace().toString().trim().replace(/^\d+[。、.]/, "").replace(/（\d+.\d+分）/, "").trim();
      // alert(createdLinks.size);
      return removeRedundantWords(transformed, redundanceWordsText.split("\n")); // 删除沉余字符
    };
    const worker = new OCSWorker({
      root: TiMu,
      elements: {
        ...
        options: '.groupTitile,.secondUlList li div p,ul li label span[class^="num_option "],.index,ul li .after,ul li textarea,ul textarea,ul li label:not(.before)',
      ...
      answerer: (elements, type, ctx) => {
        ...
        const titleAi = chapterTestTaskQuestionTitleTransformAi(elements.title);
        if (title && titleAi) {
          const typeInput = elements.type[0];
          return CommonProject.scripts.apps.methods.searchAnswerInCaches(title, () => {
            return defaultAnswerWrapperHandler(answererWrappers, {
          ...
          const handler = (type2, answer, option, ctx2) => {
            ...
            if (type2 === "judgement" || type2 === "single" || type2 === "multiple") {
              ...
              if ((_f = option == null ? void 0 : option.parentElement) == null ? void 0 : _f.parentElement) {
                (_g = $el("[onclick*=saveQuestion]", option.parentElement.parentElement)) == null ? void 0 : _g.click();
              }
            }
          };
          return await resolver(
  ...
  function work$1({ answererWrappers, period, thread }) {
    $message("info", { content: "\u5F00\u59CB\u4F5C\u4E1A" });
    CommonProject.scripts.workResults.methods.init();
    const titleTransform = (titles) => {
        ...
        methods() {
          return {
            main: async () => {
              ...
              setTimeout(() => {
                var _a2, _b;
                (_a2 = $el(".el-message-box__wrapper")) == null ? void 0 : _a2.remove();
                (_b = $el(".v-modal")) == null ? void 0 : _b.remove();
              }, 3e3);
              await waitForLoad();
              ...
              if (["ppt", "doc", "pptx", "docx", "pdf"].some((i) => courseInfo.fileType === i)) {
                ...
                if ((_a = $el(".guide")) == null ? void 0 : _a.innerHTML.includes("\u5F88\u62B1\u6B49\uFF0C\u60A8\u7684\u6D4F\u89C8\u5668\u4E0D\u652F\u6301\u64AD\u653E\u6B64\u7C7B\u6587\u4EF6")) {
                  $console.error(`\u4EFB\u52A1\u70B9 ${courseInfo.name}\uFF0C\u4E0D\u652F\u6301\u64AD\u653E\u3002`);
                } else {
                  await watchMedia$1(this.cfg.volume);
                }
              } else {
  ...
  async function watchMedia$1(volume2) {
    const media = await waitForMedia();
    media.volume = volume2;
  ...
  async function watchFile() {
    var _a;
    const vue = (_a = $el(".el-carousel")) == null ? void 0 : _a.__vue__;
    if (!vue) {
      return;
    }
  ...
  async function start() {
    var _a, _b;
    const vue = (_a = $el(".guide")) == null ? void 0 : _a.__vue__;
    const info = ((_b = vue == null ? void 0 : vue.courseList) == null ? void 0 : _b.id) ? vue == null ? void 0 : vue.courseList : vue == null ? void 0 : vue.designData;
    if ((info == null ? void 0 : info.id) !== void 0) {
      return info;
    } else {
      return void 0;
    }
  ...
  function getNextObject() {
    var _a, _b;
    return (_b = (_a = $el(".guide")) == null ? void 0 : _a.__vue__) == null ? void 0 : _b.nextObj;
  }
  async function next() {
    var _a;
    ...
    if (id && (nextObject == null ? void 0 : nextObject.id) !== void 0) {
      if (["\u6D4B\u9A8C", "\u8BA8\u8BBA"].some((i) => nextObject.fileType === i)) {
        ...
        (_a = $el(".preOrNext .next .el-link")) == null ? void 0 : _a.click();
      }
    } else {
      $message("success", {
        duration: 0,
  ...
  async function waitForLoad() {
    return new Promise((resolve, reject) => {
      const interval = setInterval(() => {
        var _a;
        if (((_a = $el(".guide")) == null ? void 0 : _a.__vue__) !== void 0) {
          clearInterval(interval);
          resolve();
        }
      }, 1e3);
  ...
  function getCourseInfo(id) {
    ...
    return fetch("https://zyk.icve.com.cn/prod-api/teacher/courseContent/" + id, {
      headers: {
        ...
        authorization: "Bearer " + ((_a = document.cookie.match(/Token=([^;]+)/)) == null ? void 0 : _a[1])
      },
      method: "GET"
    }).then((res) => res.json());
  }
  ...
  const ICourseProject = Project.create({
    name: "\u4E2D\u56FD\u5927\u5B66MOOC",
          ...
          setInterval(() => {
            ...
            for (const key in ICourseProject.scripts) {
              if (Object.prototype.hasOwnProperty.call(ICourseProject.scripts, key)) {
                const script = ICourseProject.scripts[key];
                ...
                if (runAtHash.length && runAtHash.some((h) => state.currentUrlHash.includes(h))) {
                  if (state.currentRunningScriptName !== script.name) {
                    ...
                    (_b = (_a = script.methods) == null ? void 0 : _a.main) == null ? void 0 : _b.call(_a, () => {
            ...
            main: async (canRun) => {
              ...
              async function gotoNextJob() {
                ...
                for (const item of list) {
                  const el2 = typeof item === "function" ? item() : item;
                  if (el2) {
                    await $app_actions.mouseClick(el2);
                  }
                }
                if (list.length === 0) {
  ...
  function workAndExam(type, { answererWrappers, period, thread, redundanceWordsText, upload, stopSecondWhenFinish }) {
    CommonProject.scripts.workResults.methods.init({
      questionPositionSyncHandlerType: "icourse"
    });
    const titleTransform = (titles) => {
      return removeRedundantWords(
        titles.map((t2) => t2 ? optimizationElementWithImage(t2).innerText : "").filter((t2) => t2.trim() !== "").join(",").replace(/[\u200A-\u200F]/g, ""),
        redundanceWordsText.split("\n")
      );
    };
    const worker = new OCSWorker({
      ...
      work: {
        async handler(type2, answer, option) {
          if (type2 === "judgement" || type2 === "single" || type2 === "multiple") {
            ...
            if (input && !(input == null ? void 0 : input.checked)) {
      ...
      onElementSearched(elements, root2) {
        elements.options.forEach((el2) => {
          const correct = el2.querySelector(".u-icon-correct");
          const wrong = el2.querySelector(".u-icon-wrong");
          if (correct) {
      ...
      if (type === "chapter-test") {
        ...
        await worker.uploadHandler({
          ...
          async callback(finishedRate, uploadable) {
            ...
            $message("success", { content, duration: 0 });
            await $.sleep(3e3);
            if (uploadable) {
  ...
  async function readPPT(readSpeed) {
    var _a, _b, _c, _d;
    ...
    if (reader) {
      ...
      for (let index = start2; index < total + 1; index++) {
        ...
        await $app_actions.mouseClick(next2);
        await $.sleep(readSpeed * 1e3);
      }
    }
  }
  async function discussion(discussionStrategy) {
    ...
    const p = (_g = (_f = document.querySelector("[id*=ueditor]")) == null ? void 0 : _f.contentDocument) == null ? void 0 : _g.querySelector("body p");
    if (p) {
      p.innerText = res;
      await $.sleep(1e3);
