// ==UserScript==
// @name                WME Validator Localization for New Jersey with Buttons
// @namespace           https://greasyfork.org/en/users/15052
// @version             1.2
// @author              New Jersey Editors
// @credit              Credit to XanderB for the original Validator base template, and to joyriding, and those whose work preceded this one. Credit also to PesachZ for creating the New York version which was forked for NJ. (You must have the main Validator installed to use this, get it at https://greasyfork.org/en/scripts/1571-wme-validator )
// @description         This adds a permalink button to the NJ GIS web map with parcel layers on, and localizes Validator for use in NJ with updated checks for the USA.
// @include             /^https:\/\/(www|beta)\.waze\.com\/(?!user\/)(.{2,6}\/)?editor.*$/
// @grant               none
// @run-at              document-start
// @require             https://greasyfork.org/scripts/13097-proj4js/code/Proj4js.js
// ^^^^^^^^         needed for the NJ FC Viewer PL to convert coordinated to proper projection
...
// @downloadURL https://update.greasyfork.org/scripts/370462/WME%20Validator%20Localization%20for%20New%20Jersey%20with%20Buttons.user.js
// @updateURL https://update.greasyfork.org/scripts/370462/WME%20Validator%20Localization%20for%20New%20Jersey%20with%20Buttons.meta.js
// ==/UserScript==
//
//######################################################################################################
...
*/
  "131.enabled": true,
  "131.params": {
    "titleEN": "Not New Jersey",
    "problemEN": "The segment is assigned to another state",
    "solutionEN": "Make sure you are editing in NJ and change it. If you are editing outside NJ you can disable this check by entering \"!131, *\" (without the quotes) in the Reported As: field of Validators search settings (magnifying glass)",
    "template": "${state}",
    "regexp": "!/^New Jersey/"
  },
  "131.solutionLink": "W:Creating_and_editing_road_segments#Address_Properties",
  "131.title.en": "Custom check",
  "131.title": "Not New Jersey",
  ...
  "131.solution": "Make sure you are editing in NJ and change it. If you are editing outside NJ you can disable this check by entering \"!131, *\" (without the quotes) in the Reported As: field of Validators search settings (magnifying glass)",
  "132.enabled": true,
  "132.params": {
 ...
 /* "138.enabled": true,
  "138.params": {
    ...
    "problemEN": "NJ follows national guidelines for hwy naming prefixes (I-##, US-##, CR-##, SR-##)",
    "solutionEN": "Rename the Street or Alt Street",
    "template": "${state}:${street}#${altStreet[#]}",
    "regexp": "/^New Jersey:.*\\b((([Uu]\\.? ?( S|S(?!-\\d)[- ]|S\\.|s\\.?)|[Nn](?! \\d)(ew )?(j|J- |[Jj]ersey)? ?|[Ss]tate |[Ss] ?[RrHh]|[Cc]o(unty)? ?|[Rr]o?u?[Tt]e?|[Ff](s|S(?!-\\d)|[Rr]))([Ss][Pp][Uu][Rr]|[Rr]((ou)?(te)?|(oa)?d)|[Hh]((igh)?[Ww]a?[Yy])?)?|Sr|SR=|I- |[Cc]([HhrSs]|R(?!-\\d))|([Ii]|[SsCcUu][RrRrSs])[ =])[- ]{0,2}\\d+|([Uu] ?[Ss][- ]?)?([Hh](igh)?[Ww]a?[Yy] )?[Ff] ?([Ss] ?|[Ss]? ?[Rr])(oa)?d? )/"
  },
  "138.solutionLink": "W:NJ#Major_roads",
  "138.title.en": "Custom check",
  "138.title": "Incorrect Hwy Name Prefix",
  "138.problem.en": "The segment matched custom conditions",
  "138.problem": "NJ follows national guidelines for hwy naming prefixes (I-##, US-##, CR-##, SR-##)",
  ...
  "153.params": {
    "n": 3,
    "problemEN": "Ramps generally connect to Minor Highways or higher, so they should be locked to Lvl ${n}. If not connected to a freeway, verify that this meets the criteria to be a ramp, and lock to the highest connected segment",
...
// --------------------------- Modifications for permalink buttons to Maps -------------------------------------------------

//  add NJ Parcels PL button
function  open_map(server) {
  ...
  var NJ_GIS = 'https://newjersey.maps.arcgis.com/apps/webappviewer/index.html?id=bae6f833507745d287a15497d233fbbc';

  window.open(NJ_GIS,'NYSOrthos');
}
...
setTimeout(function() {
    ...
    map_links.innerHTML = '<img src="https://newjersey.maps.arcgis.com/apps/webappviewer/images/shortcut.ico" alt="NJ GIS" width="18" height="18" id="NJ_GIS" title="NYS Orthos Permalink" style="cursor: pointer; float: left; display: inline-block; margin: 2px 5px 0 3px;"> ';
    map_links.innerHTML += '<style>.olControlAttribution {display: none;}</style>';


    WazePermalink.appendChild(map_links);

    document.getElementById("NJ_GIS")
