// ==UserScript==
// @name         MWI-Enhanced-Hit-Tracker-Complete
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  Attack/Heal With Effects & Chat Filtering
// @author       Grogu2484
// @license      MIT
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/541680/MWI-Enhanced-Hit-Tracker-Complete.user.js
// @updateURL https://update.greasyfork.org/scripts/541680/MWI-Enhanced-Hit-Tracker-Complete.meta.js
// ==/UserScript==

(function() {
    'use strict';

    // Enhanced settings with new visual options and chat filtering
    let settings = {
        tracker0: { enabled: true, heal: true, r: 255, g: 99, b: 132, name: "Player 1" },
        tracker1: { enabled: true, heal: true, r: 54, g: 162, b: 235, name: "Player 2" },
        tracker2: { enabled: true, heal: true, r: 255, g: 206, b: 86, name: "Player 3" },
        tracker3: { enabled: true, heal: true, r: 75, g: 192, b: 192, name: "Player 4" },
        tracker4: { enabled: true, heal: true, r: 153, g: 102, b: 255, name: "Player 5" },
        tracker6: { enabled: true, heal: true, r: 255, g: 0, b: 0, name: "Enemies" },
        missedLine: { enabled: true },
        moreEffect: { enabled: true },
        animationStyle: { value: 'smooth' }, // smooth, lightning, pulse
    ...
    // Enhanced settings panel with better styling
    function createSettingsPanel() {
        ...
        // Create player tracker settings
        ...
        // Create chat filtering settings
    ...
    function openEnhancedColorPicker(id, element) {
        ...
        modal.innerHTML = `
            ...
            ">
                ...
                ">
                    ...
                    <svg width="250" height="60" style="margin: 10px 0;">
                        <defs>
                            <linearGradient id="grad1" x1="0%" y1="0%" x2="100%" y2="0%">
                                <stop offset="0%" style="stop-color:rgb(${s.r},${s.g},${s.b});stop-opacity:1" />
                                <stop offset="100%" style="stop-color:rgb(${s.r},${s.g},${s.b});stop-opacity:0.3" />
                            </linearGradient>
                        </defs>
                        <path d="M 20 40 Q 125 10 230 40" stroke="url(#grad1)" 
                              stroke-width="4" fill="none" stroke-linecap="round" id="preview-line"
                              filter="drop-shadow(0 0 8px rgba(${s.r},${s.g},${s.b},0.8))"/>
                    </svg>
                </div>
                ...
                <div style="margin-bottom: 20px;">
                    <label style="display: block; margin-bottom: 8px; color: #ffd93d;">Presets:</label>
                    <div style="display: flex; gap: 10px; justify-content: center;">
                        ${[
                            { name: 'Red', r: 255, g: 0, b: 0 },
                            { name: 'Blue', r: 0, g: 100, b: 255 },
                            { name: 'Green', r: 0, g: 255, b: 100 },
                            { name: 'Purple', r: 150, g: 0, b: 255 },
                            { name: 'Orange', r: 255, g: 150, b: 0 },
                            { name: 'Pink', r: 255, g: 100, b: 150 }
                        ].map(preset => `
                            <div class="preset-color" data-r="${preset.r}" data-g="${preset.g}" data-b="${preset.b}" style="
                                width: 25px;
                                height: 25px;
                                ...
                                cursor: pointer;
                                transition: all 0.3s ease;
                            " title="${preset.name}" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'"></div>
                        `).join('')}
                    </div>
                </div>
        ...
        const previewLine = modal.querySelector('#preview-line');
        const gradient = modal.querySelector('#grad1');
        ...
        function updatePreview() {
            ...
            `;
            previewLine.setAttribute('filter', `drop-shadow(0 0 8px rgba(${s.r},${s.g},${s.b},0.8))`);
        }
        ...
        modal.querySelectorAll('.preset-color').forEach(preset => {
            preset.addEventListener('click', () => {
                s.r = parseInt(preset.dataset.r);
                s.g = parseInt(preset.dataset.g);
                s.b = parseInt(preset.dataset.b);
                inputs.forEach(input => {
                    input.value = s[input.dataset.color];
                });
                updatePreview();
            });
        });
    ...
    const ChatFilter = {
        ...
        shouldHideMessage(textContent, username = '') {
            ...
            if (settings.chatFilter.hideSpam.enabled) {
                const repeatedChars = /(.)\1{4,}/.test(text); // 5+ repeated characters
        ...
        processTextNode(node) {
            ...
            if (this.shouldHideMessage(text, username)) {
                // Find the message container to hide
                ...
                for (const mutation of mutations) {
                    for (const node of mutation.addedNodes) {
                        if (node.nodeType === Node.TEXT_NODE) {
                            ...
                            // Process all text nodes in the added element
    ...
    function createEnhancedPath(start, end, reverse = false) {
        ...
        const style = settings.animationStyle.value;
    ...
    function createEnhancedEffect(startEl, endEl, hpDiff, index, reverse = false) {
        ...
        const color = `rgba(${s.r}, ${s.g}, ${s.b}, 1)`;
        
        // Enhanced styling with multiple effects
        ...
        // Enhanced animation based on style
        ...
        setTimeout(cleanup, 6000); // Failsafe
    }
    ...
    function createEnhancedHitEffect(point, container, path, target, size, damage, color) {
        ...
        // Enhanced shockwave with gradient
        ...
        // Enhanced screen shake with direction
        if (target) {
    ...
    function initializeEnhancedTracker() {
        ...
        // Monitor settings changes for chat filter
        const originalSaveSettings = saveSettings;
        saveSettings = function() {
            originalSaveSettings();
            
            // Reinitialize chat filter if settings changed
