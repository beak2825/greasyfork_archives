...
// ==UserScript==
// @name         WAZEPT Segments (日本対応 - A/B進行方向切替)
// @version      2024.06.30
// @description  セグメントの標準化を支援するスクリプト（日本語対応、分割セグメントA/Bの進行方向切替）
// @author       J0N4S13 (jonathanserrario@gmail.com), 日本語対応: xAI
// @include      /^https:\/\/(www|beta)\.waze\.com\/(?!user\/)(.{2,6}\/)?editor.*$/
// @exclude      https://www.waze.com/user/*editor/*
// @exclude      https://www.waze.com/*/user/*editor/*
// @grant        none
// @namespace    https://greasyfork.org/users/218406
// @downloadURL https://update.greasyfork.org/scripts/543489/WAZEPT%20Segments%20%28%E6%97%A5%E6%9C%AC%E5%AF%BE%E5%BF%9C%20-%20AB%E9%80%B2%E8%A1%8C%E6%96%B9%E5%90%91%E5%88%87%E6%9B%BF%29.user.js
// @updateURL https://update.greasyfork.org/scripts/543489/WAZEPT%20Segments%20%28%E6%97%A5%E6%9C%AC%E5%AF%BE%E5%BF%9C%20-%20AB%E9%80%B2%E8%A1%8C%E6%96%B9%E5%90%91%E5%88%87%E6%9B%BF%29.meta.js
// ==/UserScript==

(function() {
    var version = GM_info.script.version;
    // 日本の道路タイプID（Wazeの道路タイプに基づく）
    var roads_id = [1, 2, 3, 4, 6, 7, 8, 17, 18, 19, 20, 22];
    var pedonal_id = [5, 10, 16]; // 歩行者用道路
    ...
    var language = {
        ...
        strMeters: "メートル",
        strDistance: "距離",
        strSelMoreSeg: "複数のセグメントが選択されています。続行しますか？"
    };
    var indexselected = "";
    var valueselected = "";
    var array_roads = {};
    ...
    function selectedFeature() {
        ...
        setTimeout(() => {
            ...
            if (typeData == "segment") {
                myTimer();
                if (W.loginManager.getUserRank() >= 3) // レベル3以上の編集者のみ
                    insertButtons();
            }
        }, 100);
    }
                ...
                .then(text => {
                    ...
                    $(json.table.rows).each(function() {
                        if (verifyNull(this["c"][0]) == true) {
                            var elem = [
                                verifyNull(this["c"][4]), // ロックレベル
                                verifyNull(this["c"][5]), // 速度制限
                                verifyNull(this["c"][6]), // 道路タイプ
                                verifyNull(this["c"][7]), // 住所設定
                                verifyNull(this["c"][8])  // その他
                            ];
            ...
            fetch('https://docs.google.com/spreadsheets/d/1qkWpUItMINWmtn7HtrpmZbRPtAnz0x0zjrIWB7lnOqI/gviz/tq?tqx=out:json')
                ...
                .then(text => {
                    ...
                    $(json.table.rows).each(function() {
                        ...
                        if (verifyNull(this["c"][0]).toLowerCase() == "ja") { // 日本語(ja)を優先
                            array_language_country["btnSplit"] = "セグメント分割";
                            array_language_country["strMeters"] = "メートル";
                            array_language_country["strDistance"] = "距離";
                            array_language_country["strSelMoreSeg"] = "複数のセグメントが選択されています。続行しますか？";
                        }
                    });
                });
                ...
                addsign.onclick = function() {
                    ...
                    if (indexselected == 'DATA_TOLLS') {
                        ...
                        if (myRoad.fwdDirection == true && myRoad.revDirection == true) {
                            // A→B/B→Aボタンを削除したため、双方向の場合は直接処理しない
                            return;
                        } else {
                            if (myRoad.fwdDirection == true) {
                                from = myRoad.fromNodeID;
                                to = myRoad.toNodeID;
                                sentido = "TRUE";
                            } else if (myRoad.revDirection == true) {
                            ...
                            var $temp = $("<input>");
                            $("body").append($temp);
                            $temp.val(center.lon.toString().slice(0, 8) + "," + center.lat.toString().slice(0, 8) + "|" +
                                      W.selectionManager.getSelectedFeatures()[0]["attributes"]["wazeFeature"]["_wmeObject"]["attributes"]["id"] + "|" +
                                      sentido + "|" + getPermalink() + "|" + from + "|" + to).select();
                    ...
                    $.each(W.selectionManager.getSelectedFeatures(), function(i, segment) {
                        ...
                        if (pedonal_id.includes(seg.attributes.roadType) && roads_id.includes(parseInt(array_config_country[indexselected][2]))) {
                            ...
                            address = false;
                        } else if (roads_id.includes(seg.attributes.roadType) && pedonal_id.includes(parseInt(array_config_country[indexselected][2]))) {
    ...
    function mainSplitSegments() {
        ...
        $.each(seg_esquerda, function(i, segId) {
            selectedSegments.push(W.model.segments.getObjectById(segId));
        });
        $.each(seg_direita, function(i, segId) {
            selectedSegments.push(W.model.segments.getObjectById(segId));
        });
        W.selectionManager.setSelectedModels(selectedSegments);
    ...
    function createSegments(sel, displacement, no) {
        ...
        // A/Bセグメントの進行方向を切り替える（左右を入れ替え）
        let tempPoints = esquerdaPoints;
        esquerdaPoints = direitaPoints.reverse();
        direitaPoints = tempPoints.reverse();
        ...
        if (last_coord_esquerda_first != null && last_coord_esquerda_last != null && last_coord_direita_last != null && last_coord_direita_first != null) {
            if (no == "AB") {
                esquerdaPoints.pop();
                esquerdaPoints.push(last_coord_direita_first);
                direitaPoints.pop();
                direitaPoints.push(last_coord_esquerda_first);
            }
            if (no == "BA") {
                esquerdaPoints.shift();
                esquerdaPoints.unshift(last_coord_direita_last);
                direitaPoints.shift();
                direitaPoints.unshift(last_coord_esquerda_last);
            }
            if (no == "AA") {
                esquerdaPoints.pop();
                esquerdaPoints.push(last_coord_direita_first);
        ...
        // A/Bセグメントの進行方向を切り替える
        if (no == "AA" || no == "BB") {
            W.model.actionManager.add(new UpdateObject(segmentoEsquerdo, {
                'fwdDirection': false,
                'revDirection': true,
                'fwdMaxSpeed': segmentoEsquerdo.attributes.revMaxSpeed,
                'revMaxSpeed': segmentoEsquerdo.attributes.fwdMaxSpeed
            }));
            W.model.actionManager.add(new UpdateObject(segmentoDireito, {
                'fwdDirection': false,
                'revDirection': true,
                'fwdMaxSpeed': segmentoDireito.attributes.revMaxSpeed,
                'revMaxSpeed': segmentoDireito.attributes.fwdMaxSpeed
            }));
        } else {
            W.model.actionManager.add(new UpdateObject(segmentoEsquerdo, {
                'fwdDirection': false,
