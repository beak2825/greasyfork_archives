// ==UserScript==
// @name         Zelenka Auto-Scanner (Clean Fork)
// @version      3.0.1
// @license MIT
// @description  Автоматический анализ общих IP на форуме (clean fork)
// @author       vuchaev2015
...
// @downloadURL https://update.greasyfork.org/scripts/557923/Zelenka%20Auto-Scanner%20%28Clean%20Fork%29.user.js
// @updateURL https://update.greasyfork.org/scripts/557923/Zelenka%20Auto-Scanner%20%28Clean%20Fork%29.meta.js
// ==/UserScript==
...
function setupMenuButton() {
    ...
    if (linksList && linksList.length) {
        linksList.append(`<li><a href="javascript:void(0)" id="${buttonId}">Scanner</a></li>`);
...
async function openSettings(page = 1, lastId = generateRandomString(10)) {
    ...
    XenForo.alert(content, 'Zelenka Auto-Scanner');
...
function logWithPrefix(text) {
    console.log(`[Zelenka Auto-Scanner] ${text}`)
}
...
async function checkUser(link, source, gifId, style = null) {
    ...
    if (notToCheckPreviouslyChecked && checkedItem) {
        ...
        if (gifElement) {
            ...
            xenforoLogAndAlert(`${output}`, `Zelenka Auto-Scanner`);
        }
        return;
    }
