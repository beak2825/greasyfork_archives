...
// @run-at			document-end
// @grant			none
// @author			Raphael, updated by 1121, modified by Sluimerstand
// @downloadURL https://update.greasyfork.org/scripts/383314/Hentai%20Heroes%2B%2B%20%28OCD%20edit%29.user.js
// @updateURL https://update.greasyfork.org/scripts/383314/Hentai%20Heroes%2B%2B%20%28OCD%20edit%29.meta.js
// ==/UserScript==
...
// Rounding to K, M, G and T
function nRounding(num, digits) {
	var si = [
		{ value: 1, symbol: "" },
		{ value: 1E3, symbol: "K" },
		{ value: 1E6, symbol: "M" },
...
texts.en = {
	...
	minute:"m",
	rank:"Rank",
	demote:"To <u>demote</u> you can have a maximum of ",
	promote:"To <u>not promote</u> you can have a maximum of ",
	points:"points",
...
texts.es = {
	...
	minute:"m",
	rank:"Rango",
	demote:"Para <u>degradar</u> puedes tener un máximo de ",
	promote:"Para <u>no promocionarte</u> puedes tener un máximo de ",
	points:"puntos",
...
texts.fr = {
	...
	minute:"m",
	rank:"Rang",
	demote:"Pour <u>rétrograder</u> vous pouvez avoir un maximum de ",
	promote:"Pour <u>ne pas monter</u> vous pouvez avoir un maximum de ",
	points:"points",
...
if (CurrentPage.indexOf('shop') != -1) ModifyMarket();					//Market
else if (CurrentPage.indexOf('harem') != -1) ModifyHarem();				//Harem
else if (CurrentPage.indexOf('quest') != -1) ModifyScenes();			//Scenes
else if (CurrentPage.indexOf('tower-of-fame') != -1) ModifyLeagues();	//Leagues
else if (CurrentPage.indexOf('home') != -1) home();						//Home
else if (CurrentPage.indexOf('log_in') != -1) home();

eventTrolls = JSON.parse(localStorage.getItem("eventTrolls"));			//Fight A Villain Colours
FightATroll();															//Fight A Villain Menu
moneyXP();																//Better XP and money
...
function moneyXP() {
	setInterval(function() {
		...
		$('span[hero="xp_sep"]').empty().append(nThousand(Hero.infos.Xp.left, 1));
		$('span[hero="xp_max"]').empty().append(" XP");
		$('div[hero="soft_currency"]').empty().append(nRounding(Hero.infos.soft_currency, 1));
	}, 3000)
};
...
sheet.insertRule('[hero="hard_currency"] { margin-left: -13px !important;}');

function FightATroll() {
	var Trolls = ['Dark Lord', 'Ninja Spy', 'Gruntt', 'Edwarda', 'Donatien', 'Silvanus', 'Bremen', 'Finalmecia', 'Roko Senseï', 'Karole', 'Jackson&#8217;s Crew'];
	...
	for (var i = 0; i < CurrentWorld; i++) {
		...
		TrollsMenu += '<a class="' + type + '" href="/battle.html?id_troll=' + (i+1) + '">' + TrollName + '</a><br />';
	}

	$('#contains_all > header').children('[type=energy_fight]').append('<div class="scriptInfo" id="FightTroll">' + texts[lang].fight_villain + '<span class="Arrow"></span><div class="TrollsMenu">' + TrollsMenu + '</div></div>');
	fightTrollCss();
}
	...
	if (typeof event_object_data !== 'undefined') {
		event_object_data.girls.forEach(function(girl) {
			if(girl.hasOwnProperty('troll')) {
				if(!girl.hasOwnProperty('owned_girl')) {
...
function ModifyHarem() {
	...
	+ document.getElementsByClassName('focus_text')[0].innerHTML + '/' + nThousand(Hero.infos.level*stats.girls) + ' harem level (' + nThousand(Hero.infos.level*stats.girls-document.getElementsByClassName('focus_text')[0].innerHTML.replace(/,/g, '')) + ' to go)<br />- '
	+ stats.unlockedScenes + '/' + stats.allScenes + ' ' + texts[lang].unlocked_scenes + ' (' + nThousand(stats.allScenes-stats.unlockedScenes) + ' to go)'
	...
	$('#harem_left').append('<div id="CustomBar">'
							+ '<img f="stats" src="https://i.harem-battle.club/images/2018/08/14/uyC.png">'
							+ '</div>'
	...
	function addPriceRow(rowName, affection, money, kobans) {
		return '<b>' + rowName + '</b> ' +
			nThousand(affection) + ' ' + texts[lang].affection + ' (' + nThousand(affection*417) + ' <span class="imgMoney"></span>) and ' +
...
function ModifyScenes() {
	if (new URL(window.location.href).searchParams.get('hh_scenes') == null)
		return;
...
function ModifyLeagues() {
	...
	var playerCurrent;
	var maxDemotionPoints;
	var maxDemotion;
	var maxStagnatePoints;
	var maxStagnate;

	function calculatePower() {
		//Get all the numbers safely
		playerEgo = parseInt($('div#leagues_left div.lead_ego div:nth-child(2)').text().replace(/[^0-9]/gi, ''));

		playerDefHC = $('div#leagues_left div.stats_wrap div:nth-child(2)').text();
		if (playerDefHC.includes('.') || playerDefHC.includes(',')) {
			playerDefHC = parseInt($('div#leagues_left div.stats_wrap div:nth-child(2)').text().replace('K', '00').replace(/[^0-9]/gi, ''));
		}
		else {
			playerDefHC = parseInt($('div#leagues_left div.stats_wrap div:nth-child(2)').text().replace('K', '000').replace(/[^0-9]/gi, ''));
		}

		playerDefCH = $('div#leagues_left div.stats_wrap div:nth-child(4)').text();
		if (playerDefCH.includes('.') || playerDefCH.includes(',')) {
			playerDefCH = parseInt($('div#leagues_left div.stats_wrap div:nth-child(4)').text().replace('K', '00').replace(/[^0-9]/gi, ''));
		}
		else {
			playerDefCH = parseInt($('div#leagues_left div.stats_wrap div:nth-child(4)').text().replace('K', '000').replace(/[^0-9]/gi, ''));
		}

		playerDefKH = $('div#leagues_left div.stats_wrap div:nth-child(6)').text();
		if (playerDefKH.includes('.') || playerDefKH.includes(',')) {
			playerDefKH = parseInt($('div#leagues_left div.stats_wrap div:nth-child(6)').text().replace('K', '00').replace(/[^0-9]/gi, ''));
		}
		else {
			playerDefKH = parseInt($('div#leagues_left div.stats_wrap div:nth-child(6)').text().replace('K', '000').replace(/[^0-9]/gi, ''));
		}

		playerAtk = $('div#leagues_left div.stats_wrap div:nth-child(8)').text();
		if (playerAtk.includes('.') || playerAtk.includes(',')) {
			playerAtk = parseInt($('div#leagues_left div.stats_wrap div:nth-child(8)').text().replace('K', '00').replace(/[^0-9]/gi, ''));
		}
		else {
			playerAtk = parseInt($('div#leagues_left div.stats_wrap div:nth-child(8)').text().replace('K', '000').replace(/[^0-9]/gi, ''));
		}

		playerExcitement = ($('div#leagues_left .girls_wrapper div:nth-child(1)').attr('girl-tooltip-data')).match(/[+-]?\d+(?:\.\d+)?/g);
		playerExcitement = (parseInt(playerExcitement[2])+parseInt(playerExcitement[4])+parseInt(playerExcitement[6]))*28;

		opponentEgo = parseInt($('div#leagues_right div.lead_ego div:nth-child(2)').text().replace(/[^0-9]/gi, ''));
		opponentEgoBase = opponentEgo;

		opponentDefHC = $('div#leagues_right div.stats_wrap div:nth-child(2)').text();
		if (opponentDefHC.includes('.') || opponentDefHC.includes(',')) {
			opponentDefHC = parseInt($('div#leagues_right div.stats_wrap div:nth-child(2)').text().replace('K', '00').replace(/[^0-9]/gi, ''));
		}
		else {
			opponentDefHC = parseInt($('div#leagues_right div.stats_wrap div:nth-child(2)').text().replace('K', '000').replace(/[^0-9]/gi, ''));
		}

		opponentDefCH = $('div#leagues_right div.stats_wrap div:nth-child(4)').text();
		if (opponentDefCH.includes('.') || opponentDefCH.includes(',')) {
			opponentDefCH = parseInt($('div#leagues_right div.stats_wrap div:nth-child(4)').text().replace('K', '00').replace(/[^0-9]/gi, ''));
		}
		else {
			opponentDefCH = parseInt($('div#leagues_right div.stats_wrap div:nth-child(4)').text().replace('K', '000').replace(/[^0-9]/gi, ''));
		}

		opponentDefKH = $('div#leagues_right div.stats_wrap div:nth-child(6)').text();
		if (opponentDefKH.includes('.') || opponentDefKH.includes(',')) {
			opponentDefKH = parseInt($('div#leagues_right div.stats_wrap div:nth-child(6)').text().replace('K', '00').replace(/[^0-9]/gi, ''));
		}
		else {
			opponentDefKH = parseInt($('div#leagues_right div.stats_wrap div:nth-child(6)').text().replace('K', '000').replace(/[^0-9]/gi, ''));
		}

		opponentAtk = $('div#leagues_right div.stats_wrap div:nth-child(8)').text();
		if (opponentAtk.includes('.') || opponentAtk.includes(',')) {
			opponentAtk = parseInt($('div#leagues_right div.stats_wrap div:nth-child(8)').text().replace('K', '00').replace(/[^0-9]/gi, ''));
		}
		else {
			opponentAtk = parseInt($('div#leagues_right div.stats_wrap div:nth-child(8)').text().replace('K', '000').replace(/[^0-9]/gi, ''));
		}

		opponentExcitement = ($('div#leagues_right .girls_wrapper div:nth-child(1)').attr('girl-tooltip-data')).match(/[+-]?\d+(?:\.\d+)?/g);
		opponentExcitement = (parseInt(opponentExcitement[1])+parseInt(opponentExcitement[3])+parseInt(opponentExcitement[5]))*28;

		//Log opponent name
		console.log("Simulation log for: " + $('div#leagues_right div.player_block div.title').text());

		//Determine class defence
		...
		function playerTurn() {
			...
			if (playerOrgasm >= playerExcitement) {
				...
				//Log results
				console.log("Player orgasm!");

				//Orgasm 1
				if (playerOrgasmCount == 1) {
					//Add Beta stats
					girlStat = $('div#leagues_left .girls_wrapper .team_girl[g=2]').attr('girl-tooltip-data');
					girlStat = girlStat.substring(girlStat.indexOf("caracs") + 1).match(/[+-]?\d+(?:\.\d+)?/g);
					girlStat2 = $('div#leagues_right .girls_wrapper .team_girl[g=2]').attr('girl-tooltip-data').match(/[+-]?\d+(?:\.\d+)?/g);

					if (playerClass == 'class1') { //HC
						playerAtk += Math.round(1.3 * girlStat[1]);
					}
					if (playerClass == 'class2') { //CH
						playerAtk += Math.round(1.3 * girlStat[3]);
					}
					if (playerClass == 'class3') { //KH
						playerAtk += Math.round(1.3 * girlStat[5]);
					}
					if (opponentClass == 'class1') { //HC
						opponentDef += Math.round(1.75 * girlStat2[1]);
					}
					if (opponentClass == 'class2') { //CH
						opponentDef += Math.round(1.75 * girlStat2[3]);
					}
					if (opponentClass == 'class3') { //KH
						opponentDef += Math.round(1.75 * girlStat2[5]);
					}
				}
				...
				if (playerOrgasmCount == 2) {
					//Add Omega stats
					girlStat = $('div#leagues_left .girls_wrapper .team_girl[g=3]').attr('girl-tooltip-data');
					girlStat = girlStat.substring(girlStat.indexOf("caracs") + 1).match(/[+-]?\d+(?:\.\d+)?/g);
					girlStat2 = $('div#leagues_right .girls_wrapper .team_girl[g=3]').attr('girl-tooltip-data').match(/[+-]?\d+(?:\.\d+)?/g);

					if (playerClass == 'class1') { //HC
						playerAtk += Math.round(girlStat[1]);
					}
					if (playerClass == 'class2') { //CH
						playerAtk += Math.round(girlStat[3]);
					}
					if (playerClass == 'class3') { //KH
						playerAtk += Math.round(girlStat[5]);
					}
					if (opponentClass == 'class1') { //HC
						opponentDef += Math.round(girlStat2[1]);
					}
					if (opponentClass == 'class2') { //CH
						opponentDef += Math.round(girlStat2[3]);
					}
					if (opponentClass == 'class3') { //KH
						opponentDef += Math.round(girlStat2[5]);
					}
				}
			...
			//No orgasm
				...
				opponentEgo -= Math.round(playerAtk - opponentDef);
		...
		function opponentTurn() {
			...
			if (opponentOrgasm >= opponentExcitement) {
				...
				if (opponentOrgasmCount == 1) {
					//Orgasm damage
					girlStat = $('div#leagues_right .girls_wrapper .team_girl[g=1]').attr('girl-tooltip-data');
					girlStat = parseInt(girlStat.substring(girlStat.indexOf('class')).match(/[+-]?\d+(?:\.\d+)?/g)[0]);

					if (girlStat == 1) {
						playerEgo -= Math.round(opponentAtk * 2.25 - playerDef);

						//Log results
						console.log("Opponent orgasm + HC Harmony proc!");
					}
					else {
						playerEgo -= Math.round(opponentAtk * 1.5 - playerDef);

						if (girlStat == 2) {
							opponentEgo += Math.round(opponentDefBase);

							//Log results
							console.log("Opponent orgasm + CH Harmony proc!");
						}
						if (girlStat == 3) {
							opponentEgo += Math.round(opponentEgoBase * 0.1);

							//Log results
							console.log("Opponent orgasm + KH Harmony proc!");
						}
					}

					//Add Beta stats
					girlStat = $('div#leagues_right .girls_wrapper .team_girl[g=2]').attr('girl-tooltip-data').match(/[+-]?\d+(?:\.\d+)?/g);
					girlStat2 = $('div#leagues_left .girls_wrapper .team_girl[g=2]').attr('girl-tooltip-data');
					girlStat2 = girlStat2.substring(girlStat2.indexOf("caracs") + 1).match(/[+-]?\d+(?:\.\d+)?/g);

					if (opponentClass == 'class1') { //HC
						opponentAtk += Math.round(1.3 * girlStat[1]);
					}
					if (opponentClass == 'class2') { //CH
						opponentAtk += Math.round(1.3 * girlStat[3]);
					}
					if (opponentClass == 'class3') { //KH
						opponentAtk += Math.round(1.3 * girlStat[5]);
					}
					if (playerClass == 'class1') { //HC
						playerDef += Math.round(1.75 * girlStat2[1]);
					}
					if (playerClass == 'class2') { //CH
						playerDef += Math.round(1.75 * girlStat2[3]);
					}
					if (playerClass == 'class3') { //KH
						playerDef += Math.round(1.75 * girlStat2[5]);
					}
				}
				...
				if (opponentOrgasmCount == 2) {
					...
					//Add Omega stats
					girlStat = $('div#leagues_right .girls_wrapper .team_girl[g=3]').attr('girl-tooltip-data').match(/[+-]?\d+(?:\.\d+)?/g);
					girlStat2 = $('div#leagues_left .girls_wrapper .team_girl[g=3]').attr('girl-tooltip-data');
					girlStat2 = girlStat2.substring(girlStat2.indexOf("caracs") + 1).match(/[+-]?\d+(?:\.\d+)?/g);

					if (opponentClass == 'class1') { //HC
						opponentAtk += Math.round(girlStat[1]);
					}
					if (opponentClass == 'class2') { //CH
						opponentAtk += Math.round(girlStat[3]);
					}
					if (opponentClass == 'class3') { //KH
						opponentAtk += Math.round(girlStat[5]);
					}
					if (playerClass == 'class1') { //HC
						playerDef += Math.round(girlStat2[1]);
					}
					if (playerClass == 'class2') { //CH
						playerDef += Math.round(girlStat2[3]);
					}
					if (playerClass == 'class3') { //KH
						playerDef += Math.round(girlStat2[5]);
					}
				}
			...
			//No orgasm
				...
				playerEgo -= Math.round(opponentAtk - playerDef);
		...
		if (matchRating >= 0) {
			...
			$('div#leagues_right .lead_player_profile').append('<div class="matchRating plus"><img id="powerLevelScouter" src="https://i.harem-battle.club/images/2019/01/14/uaM.png">' + matchRating + '</div>');
		}
		else {
			matchRating = nThousand(matchRating);
			$('div#leagues_right .lead_player_profile').append('<div class="matchRating minus"><img id="powerLevelScouter" src="https://i.harem-battle.club/images/2019/03/07/yYt.png">' + matchRating + '</div>');
		}

		//Replace opponent excitement with the correct value
		$('div#leagues_right div.stats_wrap div:nth-child(9) span:nth-child(2)').empty().append(nRounding(opponentExcitement,0));
	}

	calculatePower();

	//Add extra League info
	challengesPossible = $('div.bar-wrap div.over span:nth-child(3) span[rel=count]').text();
	if (challengesPossible.indexOf('s') > -1) {
		challengesPossible = 1;
	}
	else {
		challengesPossible = parseInt($('div.bar-wrap div.over span:nth-child(3) span[rel=count]').text().match(/[+-]?\d+(?:\.\d+)?/g));
	}

	seasonEnds = $('div.league_end_in span[rel=timer]').text();
	if (seasonEnds.indexOf(texts[lang].day) > -1) {
		challengesPossible += $('div.league_end_in span[rel=timer]').text().match(/[+-]?\d+(?:\.\d+)?/g)[0] * 1440;
		if (seasonEnds.indexOf(texts[lang].hour) > -1) {
			challengesPossible += $('div.league_end_in span[rel=timer]').text().match(/[+-]?\d+(?:\.\d+)?/g)[1] * 60;
			if (seasonEnds.indexOf(texts[lang].minute) > -1) {
				challengesPossible += $('div.league_end_in span[rel=timer]').text().match(/[+-]?\d+(?:\.\d+)?/g)[2] * 1;
			}
		}
		if (seasonEnds.indexOf(texts[lang].hour) === -1) {
			if (seasonEnds.indexOf(texts[lang].minute) > -1) {
				challengesPossible += $('div.league_end_in span[rel=timer]').text().match(/[+-]?\d+(?:\.\d+)?/g)[1] * 1;
			}
		}
	}
	if (seasonEnds.indexOf(texts[lang].day) === -1 && seasonEnds.indexOf(texts[lang].hour) > -1) {
		challengesPossible += $('div.league_end_in span[rel=timer]').text().match(/[+-]?\d+(?:\.\d+)?/g)[0] * 60;
		if (seasonEnds.indexOf(texts[lang].minute) > -1) {
			challengesPossible += $('div.league_end_in span[rel=timer]').text().match(/[+-]?\d+(?:\.\d+)?/g)[1] * 1;
		}
	}
	if (seasonEnds.indexOf(texts[lang].day) === -1 && seasonEnds.indexOf(texts[lang].hour) === -1 && seasonEnds.indexOf(texts[lang].minute) > -1) {
		challengesPossible += $('div.league_end_in span[rel=timer]').text().match(/[+-]?\d+(?:\.\d+)?/g) * 1;
	}
	...
	challengesTotal = 3 * ((' '+$('div.leagues_table table tbody.leadTable').text()+' ').split('/3').length-1);
	challengesPossible = Math.floor(challengesPossible/35) + parseInt($('div.bar-wrap div.over span[energy]').text());
	playerCurrent = $('div.leagues_table table tr td:nth-child(4):contains(-)').parent().text().match(/[+-]?\d+(?:\.\d+)?/g);

	maxDemotionPoints = $('div.leagues_table table tr td:nth-child(1):contains(' + (challengesTotal / 3 - 14) + ')').parent().text().match(/[+-]?\d+(?:\.\d+)?/g);
	maxDemotionPoints = maxDemotionPoints[maxDemotionPoints.length-1];
	if (maxDemotionPoints - playerCurrent[playerCurrent.length-1] > 0) {
		maxDemotion = '+' + (maxDemotionPoints - playerCurrent[playerCurrent.length-1]);
	}
	else {
		maxDemotion = maxDemotionPoints - playerCurrent[playerCurrent.length-1];
	}

	maxStagnatePoints = $('div.leagues_table table tr td:nth-child(1):contains(15)').parent().text().match(/[+-]?\d+(?:\.\d+)?/g);
	maxStagnatePoints = maxStagnatePoints[maxStagnatePoints.length-1];
	if (maxStagnatePoints - playerCurrent[playerCurrent.length-1] > 0) {
		maxStagnate = '+' + (maxStagnatePoints - playerCurrent[playerCurrent.length-1]);
	}
	else {
		maxStagnate = maxStagnatePoints - playerCurrent[playerCurrent.length-1];
	}
	...
	+ "<span class='possibleChallenges'><img src='https://i.harem-battle.club/images/2019/05/06/yWk.png' style='margin-bottom: 2px;'> " + challengesPossible + "/" + challengesLeft
	...
	+ "<span class='maxStagnate'><img src='https://i.harem-battle.club/images/2019/05/06/yWG.png' style='margin-left: 10px; margin-bottom: 2px;'> " + maxStagnate
	+ "<span class='scriptLeagueInfoTooltip maxStagnateTooltip'>" + texts[lang].promote + maxStagnatePoints + " " + texts[lang].points + ".</span></span>"
	+ "<span class='maxDemote'><img src='https://i.harem-battle.club/images/2019/05/06/yWv.png' style='margin-left: 10px; margin-bottom: 2px;'> " + maxDemotion
	+ "<span class='scriptLeagueInfoTooltip maxDemoteTooltip'>" + texts[lang].demote + maxDemotionPoints + " " + texts[lang].points + ".</span></span>"
	+ "</span>");

	//Replace player excitement with the correct value
	$('div#leagues_left div.stats_wrap div:nth-child(9) span:nth-child(2)').empty().append(nRounding(playerExcitement,0));

	//Add current points to current position
	$('div#leagues_left div.current_position').empty().append(texts[lang].rank + ": " + playerCurrent[0] + " (" + playerCurrent[playerCurrent.length-1] + " " + texts[lang].points +")");

	// Refresh sim on new opponent selection
