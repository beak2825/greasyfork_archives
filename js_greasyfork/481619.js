// ==UserScript==
// @name         考古加助手
// @namespace    https://greasyfork.org/zh-CN/scripts/481619-考古加助手
// @version      1.5
// @license      GPL-3.0
// @description  增强考古加能力
// @author       wzhjm
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/481619/%E8%80%83%E5%8F%A4%E5%8A%A0%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/481619/%E8%80%83%E5%8F%A4%E5%8A%A0%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
(function () {
    'use strict';
    var t, s;
        ...
        setInterval(() => {
            $('.list-export-item').forEach(pro => {
                ...
                if (is_has.length == 0) {
                    //添加相关视频按钮
                    $(pro).parent().append(
                        `<div style="cursor: pointer;color: #4975e9;float: left;" flex="cross:center main:center" class="btn_video over-click search-video">关联视频<div>`
                    )
                    $(pro).parent().find('.btn_video').click(function(e){
                        let goods_name =$(e.target).parent().find('.title').text()
                        console.log(goods_name)
                        window.open(`/darenSquare/videoList?goods_name=` + goods_name,'_blank')
                    });
                    //添加1688按钮
                    $(pro).parent().append(
                        `<div style="cursor: pointer;color: #4975e9;margin-left: 10px;float: left;" flex="cross:center main:center" class="btn_1688 cover-click search-video">跳转1688<div>`
                    )
                    $(pro).parent().find('.btn_1688').click(function(e){
                        let goods_name =$(e.target).parent().find('.title').text()
                        goods_name= goods_name.replace(/【.*?】/g, "");
                        console.log(goods_name)
                        window.open(`https://s.1688.com/selloffer/offer_search.htm?keywords=` +GBK.URI.encodeURI(goods_name),'_blank')
                    });
                }
            })
        }, 1000)
        ...
        setInterval(() => {
            $('.goods-export').forEach(pro => {
                ...
                if (is_has.length == 0) {
                    ...
                    $(pro).parent().find('.btn_video').click(function(e){
                        let goods_name =$(e.target).parent().find('.desc').text()
                        console.log(goods_name)
                        window.open(`/darenSquare/videoList?goods_name=` + goods_name,'_blank')
                    });
                    //添加1688按钮
