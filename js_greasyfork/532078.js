...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/532078/libulib%E6%8F%90%E7%A4%BA%E8%AF%8D%E7%94%9F%E6%88%90%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/532078/libulib%E6%8F%90%E7%A4%BA%E8%AF%8D%E7%94%9F%E6%88%90%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
  ...
  "negative": "cropped, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer digits, bad feet, fewer toes, extra toes, [other negative prompts]"
    ...
    function createUserInputModal() {
        ...
        closeButton.addEventListener('click', (e) => {
            e.stopPropagation();
            // 检查是否有未保存的更改
            const inputField = document.querySelector('#libulib-prompt-modal textarea');
        ...
        generateButton.addEventListener('click', () => {
            ...
            if (activeTabId === 'b-icon') {
                // 删除底座不变相关代码
                // const keepBase = document.getElementById('keep-base').checked;
                // if (keepBase) {
                //     additionalPrompt = '（底座不变，底部保持不变，base unchanged）';
                // }
            }
            ...
            // 调用DeepSeek API
            callDeepSeekAPI(finalInput, loadingContainer, activeTabId)
                .then(data => {
                    // 关闭模态框
                    document.body.removeChild(overlay);
    ...
    // 调用DeepSeek API
    async function callDeepSeekAPI(input, loadingContainer, tabId = 'general') {
        const systemPrompt = getSystemPrompt(tabId);
...
3. 保留所有关于UI设计场景的提示词，如"clean background"、"studio lighting"等`;
   ...
   d. 新主体应获得最高权重，但整体风格不变`;
    ...
    function createMainButton() {
        ...
        function checkForContainer() {
            ...
            requestAnimationFrame(() => {
                ...
                // 设置全局标记，防止重复创建
                window.libulib_button_created = true;
                logDebug('提示词助手按钮已添加到physton-gradio-container');
            });
        }
    ...
    function createAuthModal() {
        ...
        passwordInput.placeholder = '密码（@limayao.com邮箱可使用邮箱前缀作为密码）';
        ...
        regUsernameInput.placeholder = '设置用户名';
    ...
    function createUserManagementModal() {
        ...
        backButton.textContent = '返回提示词助手';
...
返回格式示例：
{
  ...
  "negative": "cropped, lowres, worst quality, low quality, blurry, jpeg artifacts, deformed, mutation, mutilated, bad anatomy, bad hands, missing fingers, extra fingers, liquid fingers, bad feet, extra limbs, missing limbs, disconnected limbs, realistic, photorealistic, realistic face, text, signature, watermark, username, fused with background"
    ...
    function getSystemPrompt(tabId) {
        return SYSTEM_PROMPTS[tabId === 'b-icon' ? 'bIcon' : 
                             tabId === 'chan-ip' ? 'chanIp' : 
                             'general'].systemPrompt;
    }

    // 提示词优化系统提示
    const PROMPT_OPTIMIZATION_SYSTEM_PROMPT = `你是一个专业的Stable Diffusion F.1模型提示词优化专家。我会提供给你三个内容：
1. 用户的优化需求
2. 当前的正向提示词
3. 当前的负向提示词
...
1. **保持现有框架**：不要完全重写提示词，保留原有的关键核心词汇和整体结构
2. **精细化优化**：只在必要时进行小范围、精细化的调整
3. **权重微调**：可以适当调整关键词的权重，但谨慎使用高于1.3的权重，避免过度拟合导致细节失真
4. **用户意图优先**：根据用户描述的优化需求进行针对性调整
5. **提示词位置**：关键词的位置很重要，重要概念应放在前面
6. **输出格式**：必须返回JSON格式，包含优化后的"positive"和"negative"字段
7. **不要解释**：不要提供解释，直接返回JSON结果
...
返回格式示例：
{
  ...
  "negative": "优化后的负向提示词"
    ...
    // 优化提示词的函数
    async function optimizePrompts(userOptimizationRequest, currentPositive, currentNegative) {
        const loadingContainer = document.getElementById('optimization-loading-container');
        loadingContainer.style.display = 'block';
        loadingContainer.innerHTML = '<div>正在优化提示词...</div>';
        
        try {
            // 构建用户请求
            const userPrompt = `
用户的优化需求: "${userOptimizationRequest}"

当前正向提示词: "${currentPositive}"

当前负向提示词: "${currentNegative}"

请根据用户需求对提示词进行优化，保持原有框架，进行精细化调整。`;
            ...
            const response = await fetch('https://api.deepseek.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    temperature: 0.4, // 降低温度以获得更确定性的调整
                    max_tokens: 1000
                })
            });
            ...
            try {
                ...
                if (jsonMatch) {
                    ...
                    throw new Error("无法解析优化后的提示词");
                }
            }
            
            loadingContainer.innerHTML += '<div style="color:green">提示词优化完成!</div>';
            return result;
    ...
    function handleOptimizeButtonClick() {
        // 获取优化需求
        const optimizationRequest = document.getElementById('prompt-optimization-request').value;
        ...
        // 获取当前提示词文本框中的内容
        const currentPositive = document.querySelector('.prompt-positive-textarea').value;
        const currentNegative = document.querySelector('.prompt-negative-textarea').value;
        ...
        // 调用优化函数
        optimizePrompts(optimizationRequest, currentPositive, currentNegative)
            .then(result => {
                // 更新提示词文本框
                document.querySelector('.prompt-positive-textarea').value = result.positive;
                document.querySelector('.prompt-negative-textarea').value = result.negative;
                
                // 触发输入事件以确保UI更新
                document.querySelector('.prompt-positive-textarea').dispatchEvent(new Event('input', { bubbles: true }));
                document.querySelector('.prompt-negative-textarea').dispatchEvent(new Event('input', { bubbles: true }));
            })
            .catch(error => {
    ...
    // 在现有对话框中添加优化需求输入区域
    function createOptimizationInterface(modal) {
        const optimizationContainer = document.createElement('div');
        optimizationContainer.style.marginTop = '15px';
        optimizationContainer.style.padding = '15px';
        optimizationContainer.style.borderTop = '1px solid #eee';
        ...
        // 优化需求输入框
        const inputLabel = document.createElement('div');
        inputLabel.textContent = '请描述您的优化需求:';
        inputLabel.style.fontSize = '14px';
        inputLabel.style.marginBottom = '5px';
        optimizationContainer.appendChild(inputLabel);
        
        const optimizationInput = document.createElement('textarea');
        optimizationInput.id = 'prompt-optimization-request';
        optimizationInput.placeholder = '例如：增强主体细节，调整光照效果，突出某个特定元素...';
        optimizationInput.style.width = '100%';
        optimizationInput.style.padding = '10px';
        optimizationInput.style.borderRadius = '6px';
        optimizationInput.style.border = '1px solid #ddd';
        optimizationInput.style.minHeight = '60px';
        optimizationInput.style.resize = 'vertical';
        optimizationInput.style.marginBottom = '10px';
        optimizationContainer.appendChild(optimizationInput);
        
        // 优化按钮
        const optimizeButton = document.createElement('button');
        optimizeButton.textContent = '优化提示词';
        optimizeButton.style.backgroundColor = '#4caf50';
        optimizeButton.style.color = 'white';
        optimizeButton.style.border = 'none';
        optimizeButton.style.padding = '8px 15px';
        optimizeButton.style.borderRadius = '6px';
        optimizeButton.style.cursor = 'pointer';
        optimizeButton.style.fontSize = '14px';
        optimizeButton.style.fontWeight = 'bold';
        optimizeButton.style.transition = 'all 0.2s ease';
        
        optimizeButton.addEventListener('mouseover', () => {
            optimizeButton.style.backgroundColor = '#45a049';
        });
        ...
        optimizeButton.addEventListener('click', handleOptimizeButtonClick);
        optimizationContainer.appendChild(optimizeButton);
        
        // 加载动画容器
        const loadingContainer = document.createElement('div');
        loadingContainer.id = 'optimization-loading-container';
        loadingContainer.style.marginTop = '10px';
        loadingContainer.style.padding = '10px';
        loadingContainer.style.border = '1px solid #eee';
        loadingContainer.style.borderRadius = '6px';
        loadingContainer.style.backgroundColor = '#f9f9f9';
        loadingContainer.style.fontSize = '14px';
        loadingContainer.style.display = 'none';
        optimizationContainer.appendChild(loadingContainer);
        
        // 将整个容器添加到模态框
        modal.appendChild(optimizationContainer);
    }
})(); 