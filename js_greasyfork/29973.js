...
// @downloadURL https://update.greasyfork.org/scripts/29973/virtonomica%3A%D0%97%D0%B0%D1%80%D0%BF%D0%BB%D0%B0%D1%82%D1%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/29973/virtonomica%3A%D0%97%D0%B0%D1%80%D0%BF%D0%BB%D0%B0%D1%82%D1%8B.meta.js
// ==/UserScript==
...
//	alert("k="+ k);
  ...
  var input_100 = $('<button id=b100>100%</button>').click(function() {
	...
	appriseEmployeeLevel();
	  ...
	  var input_top100 = $('<button id=top100>top1 = 100%</button>').click(function() {
				current_kv = $("#apprisedEmployeeLevel").text();
	my_salary = new Number($("#salary").val());

			var maxqq = $.urlParam('maxq');
			var quantity = $('#quantity').val();
			var find_kv = Math.log(maxqq / quantity) / Math.log(1.4);
//			console.log(maxqq, quantity, maxq);
			k = my_salary/prize;
	k1 = k*k;
	if ( k < 1) {
		base = (current_kv/k1) ;
...
var input_f = $("<input> ").prop({type: 'text', value: '', size: '6'}).change(function() {
  ...
  container.append( $('<table><tr>').append('<td>').append(input_95).append('<td>').append(input_f).append('<td>').append(input_plus).append('<td>').append(input_100).append('<td>').append(input_top100).append('<td>').append(input_ret) );
}
