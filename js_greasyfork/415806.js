...
// @version      1.9
// @description  FUT21 Autobuyer (JHU)
// @author       Rastor
// @co-author    Tiebe_V
// @match        https://www.easports.com/*/fifa/ultimate-team/web-app/*
// @match        https://www.ea.com/*/fifa/ultimate-team/web-app/*
// @grant       GM_notification
// @downloadURL https://update.greasyfork.org/scripts/415806/FUT21%20Autobuyer.user.js
// @updateURL https://update.greasyfork.org/scripts/415806/FUT21%20Autobuyer.meta.js
// ==/UserScript==
...
(function() {
    ...
    function addTabItem() {
        if (jQuery('h1.title').html() == 'Home' || jQuery('h1.title').html() == 'Accueil') {
    ...
    function createAutoBuyerInterface()
    {
        ...
        if (window.hasLoadedAll && getAppMain().getRootViewController().getPresentedViewController().getCurrentViewController().getCurrentController()._jsClassName) {
            ...
            if (!jQuery('.search_cancel_button').length) {
                    jQuery('#InfoWrapper').next().find('.button-container button').first().after('<button class="btn-standard search_cancel_button">Stop</button>')
            }

            if (!jQuery('.search_cancel_button_top').length) {
                    jQuery('.ut-fifa-header-view a').first().after('<a class="search_cancel_button_top" style="width: 155px;text-align: center;display: inline-block;line-height: 40px;">Stop Autobuyer</a>');
            }
        } else {
    ...
    jQuery(document).on('click', '.search_cancel_button_top', deactivateAutoBuyer);
    jQuery(document).on('touchstart', '.search_cancel_button', deactivateAutoBuyer);
    jQuery(document).on('touchstart', '.search_cancel_button_top', deactivateAutoBuyer);
