// ==UserScript==
// @name         ComicearthstarSpider
// @namespace    https://comic-earthstar.jp/
// @version      0.1
// @description  Image spider for comic-earthstar.jp
// @author       DD1969
...
// @downloadURL https://update.greasyfork.org/scripts/445115/ComicearthstarSpider.user.js
// @updateURL https://update.greasyfork.org/scripts/445115/ComicearthstarSpider.meta.js
// ==/UserScript==
...
(async function(axios, JSZip, saveAs) {
  ...
  async function start() {
    ...
    const cURL = `https://api.comic-earthstar.jp/c.php?${cid}`;
    const baseURL = await new Promise(resolve => {
      GM_xmlhttpRequest ( {
        method: 'GET',
        url: cURL,
        responseType: 'json',
        onload: res => resolve(res.response.url)
      });
    });

    const configURL = `${baseURL}configuration_pack.json`;
    ...
    const title = document.getElementById('pagetitle')?.title || `pack_${Date.now()}`;

    const imageData = [];
    for (let i = 0; i < config.configuration.contents.length; i++) {
      imageData.push({
        url: `${baseURL}${config.configuration.contents[i].file}/0.jpeg`,
