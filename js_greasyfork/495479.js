(function() {
    ...
    function playVoide(){
        capterCount = document.getElementsByClassName('courseItem___9OZvO course-item').length;
        ...
        for(let i=0;i<=capterCount;i++){
            ...
            let prevClassTxt = document.getElementsByClassName('courseItem___9OZvO course-item')[i].getElementsByClassName('ant-tag')[0].innerText;
            if(prevClassTxt === '已完成学习'){
                ...
                let vdHref = document.getElementsByClassName('courseItem___9OZvO course-item')[i].getElementsByTagName('a')[0].href;

                if(vdHref != 'javascript:;'){
                    if(saveVDHrefTow(vdHref)){
                        saveVDHref(vdHref);
                    }
                }
                else{
                    console.log("xigua:",i,"javascript:;");
                    sTime();
                }
    ...
    function getVDHref(){
        let oldData = GM_getValue('vDHrefList');
    ...
    function saveVDHref(href){
        let answerList = getVDHref();

        let juge = answerList.includes(href);

        if (juge) {
            console.log("视频记录已存在",href);
            return false;
        }

        answerList.push(href);

        console.log("保存连接",href);

        GM_setValue('vDHrefList',answerList);

        return true;
    }

    function getVDHrefTow(){
        let oldDataTow = GM_getValue('vDHrefListTow');
        if (Array.isArray(oldDataTow)) {
            return oldDataTow;
        }
        return [];
    }

    function saveVDHrefTow(href){
        let answerListTow = getVDHrefTow();
        let jugeTow = answerListTow.includes(href);

        if (jugeTow) {
            addMessage("视频已完成或正在学习中",href);
            return false;
        }


        answerListTow.push(href);

        console.log("保存连接",href);

        GM_setValue('vDHrefListTow',answerListTow);
    ...
    var oldCounter = null;
    function getCounter(){
        oldCounter = GM_getValue('counter');
        if (!oldCounter) {
            GM_setValue('counter',1);
            return GM_getValue('counter');
        }
        return oldCounter;
    }

    function subtract(){
        GM_setValue('counter',(getCounter() - 1));
    }

    function addCounter(count){
        GM_setValue('counter',(count + 1));
    }
    ...
    function listener(){
        localHref = getVDHref();
        console.log("待学列表",localHref);
        if(localHref.length < 1){
            console.log("学习完毕");
            return;
        }
        else{
            windowCount = getCounter();
            console.log("已开窗口数",windowCount);
            if(windowCount < 1){

                console.log("打开一个新窗口",localHref[0]);
                addCounter(windowCount);

                window.open(localHref[0], '_blank', 'width=500,height=500');

                localHref.shift();
                GM_setValue('vDHrefList',localHref);

            }
            else{
                console.log("window数量已达上限",windowCount);
            }
    ...
    var wait = setInterval(function (){
        ...
        if(nowUrl === 'https://zyscjj.org.cn/'){
            addMessage("首页");
        }
        else if(nowUrl === 'https://zyscjj.org.cn/train/special'){
            try {
                addMessage("课程列表...");
        ...
        else if(nowUrl.split('=')[0] === 'https://zyscjj.org.cn/detail?partnerClassId' || nowUrl.split('=')[0] === 'https://zyscjj.org.cn/detail?courseId'){
            try {
                addMessage("学习页...");
        ...
        else if(nowUrl.substring(0,26) === 'https://zyscjj.org.cn/exam'){
            try {
                addMessage("考试页...");
                trueAnswrItem = document.querySelector("#root > section > main > div > div > div.flex-box > div.flex-1.exam-drawer-question > div > form");
                examAllCount = trueAnswrItem.childElementCount - 1;
                let wahtDo = document.querySelector("#root > section > main > div > div > div.flex-box > div.info > div > div:nth-child(1) > div.exam-box-continer > span:nth-child(1)").className;
                if(wahtDo === 'card'){
    ...
    function juTorF(rIndex){
        let a = trueAnswrItem.children[rIndex].className;
        if(a === 'choiceItem___2XGZn choiceItemError___3CKth'){
            return false;
        }else{
            return true;
        }
    ...
    function getAnswerError(){
        let oldData = GM_getValue('answerErrorList');
        if (Array.isArray(oldData)) {
            return oldData;
        }
        return [];
    ...
    function saveAnswerError(answer_key,answer){
        let oldAnswer = getAnswerError();

        let existIndex = oldAnswer.findIndex(obj => answer_key in obj);

        if (existIndex !== -1) {
            // 更新值为['1','2','3']
            let arr = oldAnswer[existIndex][answer_key];
            if(arr.includes(answer)){
                return false;
            }
            arr.push(answer);
            oldAnswer[existIndex][answer_key] = arr;
        } else {
            // push新集合
            let tempMap = {};
            tempMap[answer_key] = [answer];
            oldAnswer.push(tempMap);
        }

        GM_setValue('answerErrorList',oldAnswer);
    ...
    function examRadio(){
        if(radioIndex < examAllCount){
            let answer_key = trueAnswrItem.children[radioIndex].getAttribute('ques_key');
            if(!juTorF(radioIndex)){
                // let answerStr = trueAnswrItem.children[radioIndex].getElementsByClassName("true-answer")[0].innerText;
                // let regex = /[A-Za-z]+/g;
                // let answer = answerStr.match(regex)[0];
                console.log("第",(radioIndex+1),"错误，下一题");
            ...
            if (result !== undefined) {
                ...
                switch(result)
                {
                    case 'A':
                        ...
                        trueAnswrItem.children[radioIndex].getElementsByClassName("ant-radio-group ant-radio-group-outline")[0].children[0].click();
                        break;
                    case 'B':
                        console.log("第"+(radioIndex+1)+"题选","B");
                        trueAnswrItem.children[radioIndex].getElementsByClassName("ant-radio-group ant-radio-group-outline")[0].children[1].click();
                        break;
                    case 'C':
                        console.log("第"+(radioIndex+1)+"题选","C");
                        trueAnswrItem.children[radioIndex].getElementsByClassName("ant-radio-group ant-radio-group-outline")[0].children[2].click();
                        break;
                    case 'D':
                        console.log("第"+(radioIndex+1)+"题选","D");
                        trueAnswrItem.children[radioIndex].getElementsByClassName("ant-radio-group ant-radio-group-outline")[0].children[3].click();
                        break;
                    case 'E':
                        console.log("第"+(radioIndex+1)+"题选","E");
                        trueAnswrItem.children[radioIndex].getElementsByClassName("ant-radio-group ant-radio-group-outline")[0].children[4].click();
                        break;
                }
                radioIndex++;
                examAnswerNew();
        ...
        if(radioIndex < examAllCount){
            ...
            if(!juTorF(radioIndex)){
                ...
                if (result !== undefined) {
                    ...
                    addMessage("抱歉，第"+(radioIndex+1)+"题未找到答案(c)");
                    // errorTest(answer_key);
                    // trueAnswrItem.children[radioIndex].getElementsByClassName("ant-radio-group ant-radio-group-outline")[0].children[3].click();
                    // radioIndex++;
                    // examAnswer();
                    catchError(0);
    ...
    function optionTrue(radioIndex){
        var parentElement = trueAnswrItem.children[radioIndex].getElementsByClassName("ant-radio-group ant-radio-group-outline")[0];

        var childElement = trueAnswrItem.children[radioIndex].getElementsByClassName("ant-radio-group ant-radio-group-outline")[0].getElementsByClassName("ant-radio-wrapper ant-radio-wrapper-checked")[0];
    ...
    function catchError(ind){
        if(ind > 4){
            console.log('超出可选范围ind');
            return;
        }
        let quesId = trueAnswrItem.children[radioIndex].getAttribute('ques_key');
        // let quesAnswer = optionTrue(radioIndex);

        if(saveAnswerError(quesId,ind)){
            trueAnswrItem.children[radioIndex].getElementsByClassName("ant-radio-group ant-radio-group-outline")[0].children[ind].click();
            console.log("test选项记录成功...");
            radioIndex++;
            examAnswer();
        }else{
            console.log("第"+radioIndex+"已选过"+ind);
            catchError(++ind);
        }
    }

    function catchErrorNew(ind){
        if(ind > 4){
            console.log('超出可选范围ind');
            return;
        }
    ...
    function errorTest(aw_key){
        let arrError = getAnswerError();
        let result = arrError.reduce((accumulator, obj) => {
            if (aw_key in obj) {
                return obj[aw_key];
            }
            return accumulator;
    ...
    function opindex(resl,op,idex){
        console.log(resl,op,idex)
        if (!resl.includes(op)) {
            trueAnswrItem.children[radioIndex].getElementsByClassName("ant-radio-group ant-radio-group-outline")[0].children[idex].click();
            setTimeout(function(){
                radioIndex++;
                examAnswer();
            },236);
    ...
    `);
        ...
        $("#speedxgone").on('click',function(){
            ...
            GM_deleteValue("answerErrorList");
            GM_deleteValue('counter');
            GM_deleteValue('vDHrefList');
            GM_deleteValue('vDHrefListTow');
            ddds3.children().remove();
            addMessage("已清空缓存");
        });
