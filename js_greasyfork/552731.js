...
// @author       dml19 (修改自南信院-新版正方教务系统导出课程表)
// @supportURL   https://github.com/31415926535x/CollegeProjectBackup/blob/master/ZhengfangClassScheduleToICS/Readme.md
// @compatible   chrome
// @compatible   firefox
// @license      MIT
// @include      *://jwxt.wmu.edu.cn/*
// @include      *://jwxt.wmu.cn/*
// @include      https://jwxt.wmu.edu.cn/jwglxt/kbcx/xskbcx_cxXskbcxIndex.html*
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/552731/WMU%E6%96%B0%E7%89%88%E6%AD%A3%E6%96%B9%E6%95%99%E5%8A%A1%E7%B3%BB%E7%BB%9F%E5%AF%BC%E5%87%BA%E8%AF%BE%E7%A8%8B%E8%A1%A8.user.js
// @updateURL https://update.greasyfork.org/scripts/552731/WMU%E6%96%B0%E7%89%88%E6%AD%A3%E6%96%B9%E6%95%99%E5%8A%A1%E7%B3%BB%E7%BB%9F%E5%AF%BC%E5%87%BA%E8%AF%BE%E7%A8%8B%E8%A1%A8.meta.js
// ==/UserScript==
...
// 课堂时间表（按你提供的）
var classtime = {
    '1':  { 'start': '0800', 'end': '0840' },
    '2':  { 'start': '0845', 'end': '0925' },
    '3':  { 'start': '0940', 'end': '1020' },
    '4':  { 'start': '1025', 'end': '1105' },
    '5':  { 'start': '1110', 'end': '1150' },
    '6':  { 'start': '1155', 'end': '1235' },
    '7':  { 'start': '1240', 'end': '1320' },
    '8':  { 'start': '1330', 'end': '1410' },
    '9':  { 'start': '1415', 'end': '1455' },
    '10': { 'start': '1500', 'end': '1540' },
    '11': { 'start': '1545', 'end': '1625' },
    '12': { 'start': '1630', 'end': '1710' },
    '13': { 'start': '1715', 'end': '1755' },
    '14': { 'start': '1820', 'end': '1900' },
    '15': { 'start': '1905', 'end': '1945' },
    '16': { 'start': '1950', 'end': '2030' },
    '17': { 'start': '2035', 'end': '2115' },
    '18': { 'start': '2110', 'end': '2150' },
    '19': { 'start': '2205', 'end': '2245' }
};
...
(function() {
    ...
    // document-end 已可直接运行；仍保留以防宿主限制
    (window.unsafeWindow || window).addEventListener("load", main);
})();

// ---- 工具：安全 trim（替代 jQuery 的 $.trim） ----
function safeTrim(x){ return (x && typeof x === 'string') ? x.trim() : String(x || '').trim(); }
...
function main() {
    ...
    if (windowURL.indexOf(ClassScheduleToICSURL) != -1) {
        ...
        var yd = document.getElementById("btn_yd");
        if (yd) {
            yd.onclick = function() {
...
function ClassScheduleToICS() {
    ...
    function pageFullyLoaded() {
        ...
        btn.onclick = function() {
            ...
            generateCalendar(parseCourses(parseTable()));
    ...
    class Course {
        constructor(course) {
            if (course) {
                ...
                this.week = course.week;       // 星期几
                this.info = course.info;       // 该时段信息
                this.startTime = course.startTime;
                this.endTime   = course.endTime;
                this.startWeek = course.startWeek;
                this.endWeek   = course.endWeek;
                this.isSingleOrDouble = course.isSingleOrDouble; // 是否单双周
                this.location = course.location; // 上课地点
                this.teacher  = course.teacher;  // 任课老师
            }
        }
    }
    ...
    function parseCourses(data) {
        ...
        for (let i = 0; i < data.divs.length; ++i) {
            ...
            // 更稳健地取课程名
            let span0 = data.divs[i].getElementsByTagName("span")[0];
            if (span0) {
                let f0 = span0.getElementsByTagName("font")[0];
                course.name = safeTrim((f0 ? f0.innerText : span0.textContent) || "");
                if (course.name && course.name.endsWith("◆")) {
                    course.name = course.name.substring(0, course.name.length - 1);
                }
            } else {
                course.name = "未命名课程";
            }
            ...
            data.divs[i].querySelectorAll("p").forEach(p => {
                let sp0 = p.getElementsByTagName("span")[0];
                if (!sp0) return;
                let title = sp0.getAttribute("title");
                ...
                if (title == "节/周") {
                    (function(str = (p.getElementsByTagName("font")[1] || {}).innerText) {
                        ...
                        let time = str.substring(str.indexOf("(") + 1, str.indexOf(")"));
                        let wk = str.substring(str.indexOf(")") + 1).split(",");
                    ...
                    let f = p.getElementsByTagName("font")[1];
                    course.location = safeTrim(f ? f.innerText : p.textContent);

                } else if (title == "教师") {
                    let f = p.getElementsByTagName("font")[1];
                    course.teacher = safeTrim(f ? f.innerText : p.textContent);
                }
            });
    ...
    // 通过节次确定时间（强化：未知节次给出兜底，不报错）
    function getTime(num, StartOrEnd) {
        num = String(num);
        const slot = classtime[num];
        if (!slot) {
            console.warn('未定义的节次：', num);
            return (StartOrEnd == 0 ? '080000' : '090000');
        }
        const time = (StartOrEnd == 0 ? slot["start"] : slot["end"]);
        return time + '00';
    }
        ...
        courses.forEach(course => {
            for (let i = 0; i < course.isSingleOrDouble.length; ++i) {
                let e = new ICSEvent(
                    ...
                    "" + course.name + " " + (course.location ? course.location.substring(5) : "") + " " + (course.info ? course.info.substring(1, 5) : ""),
                    "" + (course.location || ""),
                    "" + (course.info ? course.info.substring(1, 5) : "") + " " + (course.teacher || "")
                );
                e.setRRULE(
                    "WEEKLY",
                    res.Calendar.WKST,
