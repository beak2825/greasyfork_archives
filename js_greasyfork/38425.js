...
// @downloadURL https://update.greasyfork.org/scripts/38425/HIT%20Forker%20%28Beta%29.user.js
// @updateURL https://update.greasyfork.org/scripts/38425/HIT%20Forker%20%28Beta%29.meta.js
// ==/UserScript==
...
function _getTVReviews( reqlist ) {
    ...
    if ( reqlist.length > 0 && config.tv ){
        var tvurl = 'https://turkerview.com/api/v1/requesters/?ids=' + reqlist.join() + '&from=tpg';
        var newreviews;
        GM_xmlhttpRequest ({
            method: "GET",
            url: tvurl,
...
function _build (keys, log_keys, timeis) {
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        rowcolor = hit.tv != 'N/A' ? _color_tv(hit) : _color_to(hit)
...
function _panda(term, reqname, reqid, title, value, name, button) {
    ...
    var once = term == "panda" ? false : true;
...
function _export_vb (key) {
  ...
  var exportcode = '[table][tr][td]'+
      ...
      '[b]Requester:[/b] [URL=' + pandaurl + hit.reqlink + ']' + hit.reqname + '[/URL] [' + hit.reqid + '] ([URL=' + pandaurl + hit.conlink + ']Contact[/URL]) ([URL=' + hit.tvlink +']Req TV[/URL]): [B][COLOR=' + _tvhourly + ']' + tv + '[/COLOR][/B]\n' +
