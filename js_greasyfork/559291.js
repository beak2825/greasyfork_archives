...
// @downloadURL https://update.greasyfork.org/scripts/559291/%E5%A6%96%E7%81%AB%E7%BD%91%E5%A2%9E%E5%BC%BA%E8%84%9A%E6%9C%ACPlus.user.js
// @updateURL https://update.greasyfork.org/scripts/559291/%E5%A6%96%E7%81%AB%E7%BD%91%E5%A2%9E%E5%BC%BA%E8%84%9A%E6%9C%ACPlus.meta.js
// ==/UserScript==
...
function useRightNextBtn() {
  waitForElement(".btBox .bt2", () => {
    ...
    // 【核心修复】防止重复执行：检查是否已经标记过
    if (btBox.attr("data-v2jun-swapped") === "true") return;

    const links = btBox.children("a");
    if (links.length < 2) return;

    // 获取按钮文本，确保只在顺序是 [上一页][下一页] 时才交换
    const firstText = links.eq(0).text().trim();
    const secondText = links.eq(1).text().trim();

    if (firstText === "上一页" && secondText === "下一页") {
        // 克隆元素以保留事件绑定
        ...
        // 标记为已处理
        btBox.attr("data-v2jun-swapped", "true");
        console.log("已将翻页按钮位置互换");
    } else if (firstText === "下一页" && secondText === "上一页") {
        // 如果已经是想要的状态，也标记为已处理，防止后续逻辑干扰
        btBox.attr("data-v2jun-swapped", "true");
    }
  });
}
