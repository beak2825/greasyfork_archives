// ==UserScript==
// @name         Pornhub User Tagger
// @namespace    http://tampermonkey.net/
// @version      17.1
// @description  Made the input field and tag display area more compact (200px width).
// @author       GPT-4 & Gemini Pro
// @match        *://*.pornhub.com/model/*
// @match        *://*.pornhub.com/users/*
// @match        *://*.pornhub.com/channels/*
// @match        *://*.pornhub.com/pornstar/*
// @match        *://*.pornhub.org/model/*
// @match        *://*.pornhub.org/users/*
// @match        *://*.pornhub.org/channels/*
// @match        *://*.pornhub.org/pornstar/*
// @grant        GM.setValue
// @grant        GM.getValue
// @grant        GM.addValueChangeListener
// @grant        GM_addStyle
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/546471/Pornhub%20User%20Tagger.user.js
// @updateURL https://update.greasyfork.org/scripts/546471/Pornhub%20User%20Tagger.meta.js
// ==/UserScript==
...
(async () => {
    ...
    const UI = {
        ...
        backupData() { const dataStr = JSON.stringify(DB.data, null, 2); const blob = new Blob([dataStr], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `pornhub-tags-backup-${new Date().toISOString().slice(0, 10)}.json`; a.click(); URL.revokeObjectURL(url); alert('Backup created.'); },
        restoreData() { if (!confirm('This will overwrite ALL current tags. Are you sure?')) return; const input = document.createElement('input'); input.type = 'file'; input.accept = '.json'; input.onchange = e => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = async (event) => { try { const restoredData = JSON.parse(event.target.result); if (typeof restoredData === 'object' && restoredData !== null) { DB.data = restoredData; await DB.save(); alert('Data restored!'); this.refreshManagementPanel(); if (this.currentUserInfo) this.renderUserTags(); } else throw new Error('Invalid file format.'); } catch (err) { alert('Error restoring file: ' + err.message); } }; reader.readAsText(file); };input.click(); },
    };
