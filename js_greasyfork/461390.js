...
// @supportURL   http://forumfrat.forumfree.it/
// @version      2.1
// @description  Script Vendetta2X
// @include      *vendetta2x.servegame.com/*
...
// @downloadURL https://update.greasyfork.org/scripts/461390/ML_PLUS2X.user.js
// @updateURL https://update.greasyfork.org/scripts/461390/ML_PLUS2X.meta.js
// ==/UserScript==

(function() {
    'use strict';
    var _VERS = "2.1.23",serverID,_PAGE,ELE_CLSS,refCLSS,_ORARIO,_DATA;

    $(document).ready(function(){
        if($("iframe", parent.document).length){var frmMenu = ($("iframe", parent.document)[2].contentWindow ||$("iframe", parent.document)[2].contentDocument);}
        var EleDiv = $('div', parent.document.body);
       ...
       /*################ GreaseMonkey pre 4.0 Version    ###############*/
        ...
        var LISTENER = {
            ...
            EVNT_SETTING: function(name){
                    try{
                        if(JS.getById(document,'chkNOTEPAD')){
                            JS.getById(document,'chkNOTEPAD').addEventListener('change',function(){
                                if (this.checked) {
                                    if($('#NOTEPAD',parent.document).length){GM.Scrivi('noNOTEPAD',true);$('#NOTEPAD',parent.document)[0].parentNode.removeChild($('#NOTEPAD',parent.document)[0])}
                                } else {
                                    if($('#NOTEPAD',parent.document).length){}else{GM.Cancella('noNOTEPAD');CHANGE.AddNotePad();}
                                }
                            });
                        }
                        /*if(JS.getById(document,'btnSaveConfig')){
                            JS.getById(document,'btnSaveConfig').addEventListener('click',function(){
                                if(JS.getById(document,'chkEDIF').checked == true){GM.Scrivi('noIMG_RIEP',true);}else{GM.Cancella('noIMG_RIEP');}
                                if(JS.getById(document,'chkNOTEPAD').checked == true){GM.Scrivi('noNOTEPAD',true);}else{GM.Cancella('noNOTEPAD');}
                            });
                        }*/
                    }catch(e){
            ...
            EVNT_MISSIONI: function(){
                try{
                    //Header SendPrevBuild
                    if(JS.getById(document,'SendPrevEd')){
                    JS.getById(document,'SendPrevEd').addEventListener("click", function() {
                        GM.Scrivi('AUTO_MISSIONE','PREV');
                        JS.getById(document,'subbt').click();
                    })
                    }
                    //Header SendNextBuild
                    if(JS.getById(document,'SendNextEd')){
                    JS.getById(document,'SendNextEd').addEventListener("click", function() {
                        GM.Scrivi('AUTO_MISSIONE','NEXT');
                        JS.getById(document,'subbt').click();
                    })
                    }
                    //SINGLE SAVE TRP
                    var btnS_TRP=document.getElementsByName('btnSaveTrp');
                    for(var u_S=0; u_S < btnS_TRP.length; u_S++){
                        btnS_TRP[u_S].addEventListener('click', function(){
                            var id_unit = this.id.split('_')[1];
                            if(parseInt(JS.getById(document, id_unit).value) >0){GM.Scrivi(this.id,parseInt(JS.getById(document, id_unit).value));}
                            this.style.display="none";JS.getById(document,'DEL_' + id_unit ).style.display = "";
                        },false);
                    }
                    //SINGLE DELETE TRP
                    var btnD_TRP=document.getElementsByName('btnDelTrp');
                    for(var u_D=0; u_D < btnD_TRP.length; u_D++){
                        btnD_TRP[u_D].addEventListener('click', function(){
                            var id_unit = this.id.split('_')[1];
                            GM.Cancella("SAVE_"+id_unit);
                            this.style.display="none";JS.getById(document,'SAVE_' + id_unit ).style.display = "";
                            JS.getById(document, id_unit).value=0;
                        },false);
                    }
                    //SALVA COORDINATE
                    JS.getById(document,'SAVE_COORDS').addEventListener("click", function() {
                        var _coord =JS.getById(document,'C0').value+':'+JS.getById(document,'C1').value+':'+JS.getById(document,'C2').value;
                        GM.Scrivi('SAVE_COORDS',_coord);
                        this.style.display="none";
                        JS.getById(document,'DEL_COORDS').style.display="";
                    })
                    //DELETE COORDINATE
                    JS.getById(document,'DEL_COORDS').addEventListener("click", function() {
                        GM.Cancella('SAVE_COORDS');
                        this.style.display="none";
                        JS.getById(document,'SAVE_COORDS').style.display="";
                        //Update actual coord build
                        if($('#selEDIFICI',frmMenu.document).length) {
                        var tmpCoord = $('#selEDIFICI',frmMenu.document)[0].options[$('#selEDIFICI',frmMenu.document)[0].selectedIndex].innerHTML
                        //console.log(tmpCoord)
                        JS.getById(document,'C0').value=parseInt(tmpCoord.split(':')[0]);
                        JS.getById(document,'C1').value=parseInt(tmpCoord.split(':')[1]);
                        JS.getById(document,'C2').value=parseInt(tmpCoord.split(':')[2]);
                        }
                    })
                    //SALVA TIPO MISSIONE
                    JS.getById(document,'SAVE_TYP_MISS').addEventListener("click", function() {
                        GM.Scrivi('SAVE_TYP_MISS',JS.getById(document,'typ').selectedIndex);
                        this.style.display="none";
                        JS.getById(document,'DEL_TYP_MISS').style.display="";
                    })
                    //DELETE TIPO MISSIONE
                    JS.getById(document,'DEL_TYP_MISS').addEventListener("click", function() {
                        GM.Cancella('SAVE_TYP_MISS');JS.getById(document,'typ').selectedIndex=0;
                        this.style.display="none";
                        JS.getById(document,'SAVE_TYP_MISS').style.display="";
                        transport()
                    })
                         //SALVA VEL MISSIONE
                        JS.getById(document,'SAVE_VEL_MISS').addEventListener("click", function() {
                            GM.Scrivi('SAVE_VEL_MISS',JS.getById(document,'spr').selectedIndex);
                            this.style.display="none";
                            JS.getById(document,'DEL_VEL_MISS').style.display="";
                        })
                        //DELETE TIPO MISSIONE
                        JS.getById(document,'DEL_VEL_MISS').addEventListener("click", function() {
                            GM.Cancella('SAVE_VEL_MISS');JS.getById(document,'spr').selectedIndex=0;
                            this.style.display="none";
                            JS.getById(document,'SAVE_VEL_MISS').style.display="";
                        })
                    //RISORSE
                    ////btnSaveRESS
                    var btnS_RESS=document.getElementsByName('btnSaveRESS');
                    for(var r_S=0; r_S < btnS_RESS.length; r_S++){
                        btnS_RESS[r_S].addEventListener('click', function(){
                            var id_ress =this.id.split('_')[2];
                            GM.Scrivi(this.id,JS.getById(document,id_ress).value);
                            this.style.display="none";
                            JS.getById(document,'DEL_RESS_'+id_ress).style.display="";
                        },false);
                    }
                    ////btnDelRESS
                    var btnD_RESS=document.getElementsByName('btnDelRESS');
                    for(var r_D=0; r_D < btnD_RESS.length; r_D++){
                        btnD_RESS[r_D].addEventListener('click', function(){
                            var id_ress =this.id.split('_')[2];
                            GM.Cancella('SAVE_RESS_'+id_ress);
                            this.style.display="none";
                            JS.getById(document,'SAVE_RESS_'+id_ress).style.display="";
                            JS.getById(document,id_ress).value=0;
                        },false);
                    }
                    JS.getById(document,'subbt').addEventListener("click", function() {
                       GM.Scrivi('BACKTOMISS');
                    })
                }catch(e){
        ...
        var CHANGE = {
            AddNotePad: function(){
                    try{
                        var notepad = document.createElement('textarea');notepad.id="NOTEPAD";
                        if($('.div_page', parent.document.body).length){if($("#NOTEPAD",parent.document.body).length){}else{
                                if(GM.Esiste('noNOTEPAD',false) == false){
                                   EleDiv[1].parentNode.parentNode.insertBefore(notepad,EleDiv[0]);
                                   if(GM.Esiste('NOTE')){notepad.value=GM.Esiste('NOTE');}
                                   notepad.addEventListener('focusout', function(e) {GM.Scrivi('NOTE',this.value);});
                                }
                            }
                        }
                    }catch(e){
                        console.log('[ xCHANGE.AddNotePad() ]  :  ' + e.name + '{ - Linea: ' + e.lineNumber + ' }\n');
                        console.log('  --------->  ' + e.message);
                    }
                },
            CalcolaCR: function(){
                    try{
                        // $('th:contains("Tutte le truppe")')[0].style="colspan=2";
                        //$('th:contains("Tutte le truppe")')[0].parentNode.appendChild('<th>PT TRUPPE</th>');
                        //
                        //-----------------> pt['Mercenario']
                        //datiCR = [CapA | CapM | CapAl | CapD]
                        var datiCR = [],_tmp=0,ptSpiata=0;
                        var pt={'Picchiatore':6,'Buttafuori':6,'Bandito':4,'Pistolero':21,'Truppa di Occupazione':251,'Spia':3,'Imballatore':9,
                                'Agente CIA':87,'Agente FBI':48,'Trasportatore':51,'Esperto Tattico':93,'Tiratore scelto':28,'Killer professionista':176,
                                'Ninja':236,'Esperto in Demolizioni':281,'Mercenario':1176,'Lavoratore illegale':4,'Sentinella':21,'Polizia':54,
                               'Guardia del corpo':43,'Guardia d`onore':388}
                        var nUn={'Picchiatore':0,'Buttafuori':0,'Bandito':0,'Pistolero':0,'Truppa di Occupazione':0,'Spia':0,'Imballatore':0,
                                'Agente CIA':0,'Agente FBI':0,'Trasportatore':0,'Esperto Tattico':0,'Tiratore scelto':0,'Killer professionista':0,
                                'Ninja':0,'Esperto in Demolizioni':0,'Mercenario':0,'Lavoratore illegale':0,'Sentinella':0,'Polizia':0,
                               'Guardia del corpo':0,'Guardia d`onore':0}
                        var ElencoRowCR = $('tr',$('th:contains("Tutte le truppe")')[0].parentNode.parentNode);
                        //Trovo Indice REPORT
                        for (var dp=3; dp < ElencoRowCR.length;dp++){
                            if( $('th',ElencoRowCR[dp]).length>4){
                               var nUnit=$('th',ElencoRowCR[dp])[0].textContent.trim();
                               if(nUn[nUnit] != null){
                                   nUn[nUnit]=parseInt($('th',ElencoRowCR[dp])[3].textContent)-parseInt($('th',ElencoRowCR[dp])[4].textContent);
                                }
                            }
                        ...
                        var totPT=0,eleNomi = ['Picchiatore','Buttafuori','Bandito','Pistolero','Truppa di Occupazione','Spia','Imballatore',
                                'Agente CIA','Agente FBI','Trasportatore','Esperto Tattico','Tiratore scelto','Killer professionista',
                                'Ninja','Esperto in Demolizioni','Mercenario','Lavoratore illegale','Sentinella','Polizia',
                               'Guardia del corpo','Guardia d`onore']
                        for(var _pt=0;_pt<eleNomi.length;_pt++){
                           totPT+= nUn[eleNomi[_pt]]*pt[eleNomi[_pt]]
                        }
                        if(totPT>0){
                            //Aggiungo calc pt truppe spiata
                            var tr_PtTruppe = document.createElement('tr');
                            tr_PtTruppe.innerHTML='<th>TOTALE PUNTI TRUPPE </th><th colspan="4" style="text-align:center">' + totPT.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") +'</th>';
                            console.log($('th:contains("Possibilità di spionaggio")')[0].parentNode.parentNode);
                           $('th:contains("Possibilità di spionaggio")')[0].parentNode.parentNode.insertBefore(tr_PtTruppe,$('th:contains("Possibilità di spionaggio")')[0].parentNode);
                        }
            ...
            ConfrontaCLSS : function(){
                try {
                    ...
                    //document.getElementById('tabSCORE').style ="width: 100%;overflow-y: scroll;";
            ...
            mainSCORE: function(){
                try{
                    //document.getElementsByTagName('table')[document.getElementsByTagName('table').length-2].style.width="90%"
                     if(!frmMenu){$('table')[$('table').length-2].style.width="90%" ;}
            ...
            MISSIONI: function(){
                try{
                    //Controllo !CAMBIA EDIFICIO ON
                        var tabMissioni = JS.getTagName(document,'table',2);
                        if(tabMissioni.innerHTML.indexOf('<td colspan="3" class="f">')!=-1){GM.Cancella('BACKTOMISS');GM.Cancella('AUTO_MISSIONE');}
                        if(tabMissioni.textContent.indexOf('All Prepara una nuova mission')!= -1){
                            //Pulsanti < <<  > >>
                            if($("iframe", parent.document).length){
                                if($('#BtnPrevEd',frmMenu.document).length){
                                    var btnSendPrevEd = document.createElement('button');btnSendPrevEd.id="SendPrevEd";btnSendPrevEd.name="btnSendPrevEd";btnSendPrevEd.textContent=" << ";
                                    btnSendPrevEd.style="padding-top:0;padding-bottom:0;font-weight:normal;height:20px;";
                                    $(".c[colspan='3']")[1].append(btnSendPrevEd)
                                }
                                if($('#BtnNextEd',frmMenu.document).length){
                                    var btnSendNextEd = document.createElement('button');btnSendNextEd.id="SendNextEd";btnSendNextEd.name="btnSendNextEd";btnSendNextEd.textContent=" >> ";
                                    btnSendNextEd.style="padding-top:0;padding-bottom:0;font-weight:normal;height:20px;";
                                    $(".c[colspan='3']")[1].append(btnSendNextEd);
                                }
                            }
                            //SAVE TRUPPE
                            var tabUnit = JS.getTagName(document,'table',3);tabUnit.id="tabUnit";
                            var tr_tabUnit = JS.getTagName(tabUnit,'tr',-1);
                            for(var uN=1;uN < tr_tabUnit.length;uN++){
                                if(tr_tabUnit[uN].innerHTML.indexOf('info.php')!=-1){
                                    var _index=JS.getTagName(tr_tabUnit[uN],'input',2).id;
                                    var maxUnit=parseInt(JS.getById(document,_index +'M').value);
                                    //TRUPPE
                                    JS.getTagName(tr_tabUnit[uN],'th',0).innerHTML += '&nbsp&nbsp'+
                                        '<a id="SAVE_' + _index + '" name="btnSaveTrp" type="submit"><img style="width:20px;heigth:20px" src="https://i.ibb.co/6YFmxHR/save-file-disk-open-searsh-loading-clipboard-1513.png" alt="buttonpng" border="0" /></a>'+
                                        '<a id="DEL_' + _index + '" name="btnDelTrp" style="display:none"><img style="width:30px;heigth:30px;" src="https://i.ibb.co/s5Rxszk/2303123-bin-delete-garbage-remove-trash-icon.png" alt="CANCELLA SAVE NOME TRUPPA" border="0"></a>';
                                    if(GM.Esiste('SAVE_'+_index)){
                                        JS.getById(document,'SAVE_' + _index ).style.display = "none";
                                        JS.getById(document,'DEL_' + _index ).style.display = "";
                                        JS.getById(document,_index ).value = Math.min(parseInt(GM.Esiste('SAVE_'+_index)),parseInt(JS.getById(document,_index+'M').value));
                                    }
                                }
                            }
                            //COORDINATE
                            var tr_coords=JS.getById(document,'C0').parentNode.parentNode;
                            JS.getTagName(tr_coords,'th',0).innerHTML += '&nbsp&nbsp'+
                                    '<a id="SAVE_COORDS" name="btnSaveCoord" type="submit"><img style="width:20px;heigth:20px" src="https://i.ibb.co/6YFmxHR/save-file-disk-open-searsh-loading-clipboard-1513.png" alt="buttonpng" border="0" /></a>'+
                                    '<a id="DEL_COORDS" name="btnDelCoord" style="display:none"><img style="width:30px;heigth:30px;" src="https://i.ibb.co/s5Rxszk/2303123-bin-delete-garbage-remove-trash-icon.png" alt="CANCELLA SAVE NOME TRUPPA" border="0"></a>';
                            if(GM.Esiste('SAVE_COORDS')){
                                JS.getById(document,'C0').value=GM.Esiste('SAVE_COORDS').split(':')[0]
                                JS.getById(document,'C1').value=GM.Esiste('SAVE_COORDS').split(':')[1]
                                JS.getById(document,'C2').value=GM.Esiste('SAVE_COORDS').split(':')[2]
                                JS.getById(document,'SAVE_COORDS').style.display="none";
                                JS.getById(document,'DEL_COORDS').style.display="";
                            }
                            //TIPO MISSIONE
                            var tr_typMiss = JS.getById(document,'typ').parentNode.parentNode;
                            JS.getTagName(tr_typMiss,'th',0).innerHTML += '&nbsp&nbsp'+
                                    '<a id="SAVE_TYP_MISS" name="btnSaveCoord" type="submit"><img style="width:20px;heigth:20px" src="https://i.ibb.co/6YFmxHR/save-file-disk-open-searsh-loading-clipboard-1513.png" alt="buttonpng" border="0" /></a>'+
                                    '<a id="DEL_TYP_MISS" name="btnDelCoord" style="display:none"><img style="width:30px;heigth:30px;" src="https://i.ibb.co/s5Rxszk/2303123-bin-delete-garbage-remove-trash-icon.png" alt="CANCELLA SAVE NOME TRUPPA" border="0"></a>';
                            if(GM.Esiste('SAVE_TYP_MISS')){
                                JS.getById(document,'typ').selectedIndex=parseInt(GM.Esiste('SAVE_TYP_MISS'))
                                JS.getById(document,'SAVE_TYP_MISS').style.display="none";
                                JS.getById(document,'DEL_TYP_MISS').style.display="";
                                transport();
                            }
                            //VEL MISSIONE
                                $('th',$('th:contains("Arrivo stimato")')[0].parentNode)[2].innerHTML +='&nbsp&nbsp'+
                                        '<a id="SAVE_VEL_MISS" name="btnSaveVMiss" type="submit"><img style="width:20px;heigth:20px" src="https://i.ibb.co/6YFmxHR/save-file-disk-open-searsh-loading-clipboard-1513.png" alt="buttonpng" border="0" /></a>'+
                                        '<a id="DEL_VEL_MISS" name="btnDelVMiss" style="display:none"><img style="width:30px;heigth:30px;" src="https://i.ibb.co/s5Rxszk/2303123-bin-delete-garbage-remove-trash-icon.png" alt="CANCELLA SAVE NOME TRUPPA" border="0"></a>';
                                if(GM.Esiste('SAVE_VEL_MISS')){
                                    JS.getById(document,'spr').selectedIndex=parseInt(GM.Esiste('SAVE_VEL_MISS'));
                                    JS.getById(document,'SAVE_VEL_MISS').style.display="none";
                                    JS.getById(document,'DEL_VEL_MISS').style.display="";
                                }
                            //RISORSE
                            var btnRESS='', tabRisorse=JS.getById(document,'retra');
                            var tr_btnRisorse = JS.getTagName(tabRisorse,'tr',-1)
                            for(var btnR=0;btnR<4;btnR++){
                                JS.getTagName(tr_btnRisorse[2],'th',-1)[btnR].innerHTML += '&nbsp&nbsp'+
                                    '<a id="SAVE_RESS_R'+ btnR +'" name="btnSaveRESS" type="submit"><img style="width:20px;heigth:20px" src="https://i.ibb.co/6YFmxHR/save-file-disk-open-searsh-loading-clipboard-1513.png" alt="buttonpng" border="0" /></a>'+
                                    '<a id="DEL_RESS_R'+ btnR +'" name="btnDelRESS" style="display:none"><img style="width:30px;heigth:30px;" src="https://i.ibb.co/s5Rxszk/2303123-bin-delete-garbage-remove-trash-icon.png" alt="CANCELLA SAVE NOME TRUPPA" border="0"></a>';
                                if(GM.Esiste("SAVE_RESS_R"+ btnR)){
                                    JS.getById(document,'R'+btnR).value=parseInt(GM.Esiste("SAVE_RESS_R"+ btnR));
                                    JS.getById(document,'SAVE_RESS_R'+btnR).style.display="none";
                                    JS.getById(document,'DEL_RESS_R'+btnR).style.display="";
                                }
                            }
                            updatepageinfo();
                            LISTENER.EVNT_MISSIONI();
                        }

                }catch(e){
                    console.error('[ --> CHANGE.MISSIONI() <-- ]  :  ' + e.name + '{ - Linea: ' + e.lineNumber + ' }\n');
                    console.error('  --------->  ' + e.message);
                }
            },
            RIEP_MISSIONI: function(){
                try {
                       if(GM.Esiste('AUTO_MISSIONE')){
                           if(GM.Esiste('AUTO_MISSIONE')=='PREV'){GM.Cancella('AUTO_MISSIONE');$('#BtnPrevEd',frmMenu.document).click();}
                           if(GM.Esiste('AUTO_MISSIONE')=='NEXT'){GM.Cancella('AUTO_MISSIONE');$('#BtnNextEd',frmMenu.document).click();}
                          }
                    if(GM.Esiste('BACKTOMISS')){GM.Cancella('BACKTOMISS');$('[href="fleet.php"] ',frmMenu.document).click()}

                } catch (e) {
                    console.error('[ --> CHANGE.RIEP_MISSIONI() <-- ]  :  ' + e.name + '{ - Linea: ' + e.lineNumber + ' }\n');
                    console.error('  --------->  ' + e.message);
                }
            },
            MENU: function(){
                    try{
                        ...
                        var tr_tabMenu = $('tr', $('table',document));
                        var MenuHead=document.createElement('tr');MenuHead.innerHTML='<td class="VSCRIPT">Script Vers.' + _VERS + '</td>';MenuHead.id="VERS"
                        tr_tabMenu[0].parentNode.insertBefore(MenuHead,tr_tabMenu[0])
                        //tr_tabMenu[tr_tabMenu.length-3] <--- OPZIONI
                        //<LNK> COSTI GIOCO
                        /*var MenuCosti=document.createElement('tr');MenuCosti.id="lnkCOSTI"
                        MenuCosti.innerHTML='<th height="18"><a href="costi.php" class="l1" onclick="x("costi.php")">COSTI GIOCO</a></th>';
                        tr_tabMenu[tr_tabMenu.length-2].parentNode.insertBefore(MenuCosti,tr_tabMenu[tr_tabMenu.length-2]);*/
                        //<LNK> IMPOSTAZIONI SCRIPT
                        var MenuConfScrpt=document.createElement('tr');MenuConfScrpt.id="lnkCONF_SCRIPT"
                        MenuConfScrpt.innerHTML='<th height="18"><a href="s_config.php" class="l1" onclick="x("s_config.php")">IMP SCRIPT</a></th>';
                        tr_tabMenu[tr_tabMenu.length-2].parentNode.insertBefore(MenuConfScrpt,tr_tabMenu[tr_tabMenu.length-2]);
                    }catch(e){
                        console.error('[ --> CHANGE.MENU() <-- ]  :  ' + e.name + '{ - Linea: ' + e.lineNumber + ' }\n');
                        console.error('  --------->  ' + e.message);
                    }
                },
            CONFIG: function(){
                    try{
                        if($("iframe", parent.document).length){var frm = ($("iframe", parent.document)[3].contentWindow ||$("iframe", parent.document)[3].contentDocument);}
                        var lang = ['IMPOSTAZIONI', 'ESCI', 'AGGIORNA', 'GENERALE', 'SCELTA VISTA GLOBALE : ', 'VISTA GLOBALE', 'NASCONDI LOGO GAME ', 'NASCONDI IMG RIEPILOGO',
                       'IMPOSTA x PER CALCOLO LIV SUCCESSIVI : ', 'SALVA', 'CONFIGURAZIONI SALVATATE ... ']
                        $('body',frm.document)[0].innerHTML='<div style="background-color: Black">' +
                           '<header style="background-color:#0070C0;color:Yellow;font-family: GulimChe, monospace;text-align: center;font-weight:bold;font-size:18px">' +
                           'ML_PLUS2X Vers.  ' + _VERS + '</header><br />' +
                           '<table width="100%">' +
                           '<tr style="background-color:#95B3D7;font-family:Kristen ITC;text-align:center;font-weight:bold;font-size:16px">' +
                           '<td colspan="2">GENERALE</td></tr>'+
                           '<tr style="display:none"><td style="width:80%;text-align:center;font-weight:bold"><p style="color:beige;font-size:14px">NASCONDI IMG RIEPILOGO</p></td>' +
                           '<td style="border:solid;text-align:center;background-color:black"><input type="checkbox" id="chkEDIF" name="chkEDIF" /></td></tr>' +
                            '<tr><td style="width:80%;text-align:center;font-weight:bold"><p style="color:beige;font-size:14px">NASCONDI NOTEPAD</p></td>' +
                           '<td style="border:solid;text-align:center;background-color:black"><input type="checkbox" id="chkNOTEPAD" name="chkNOTEPAD" /></td></tr>' +
                           '</table>'

                        //Controllo prec save config
                        if(GM.Esiste('noIMG_RIEP')){JS.getById(document,'chkEDIF').checked=true;}
                        if(GM.Esiste('noNOTEPAD')){JS.getById(document,'chkNOTEPAD').checked=true;}
                        LISTENER.EVNT_SETTING();

                    }catch(e){
                        console.error('[ --> CHANGE.CONFIG() <-- ]  :  ' + e.name + '{ - Linea: ' + e.lineNumber + ' }\n');
                        console.error('  --------->  ' + e.message);
                    }
                },
        };
        ...
        var GAME = {
            myCSS: function(){
                try {
                     /* New CSS */
                     GM_addStyle('#NOTEPAD  {width: 300px; height: 400px;float: left;}'+
                                '.VSCRIPT {FONT-SIZE: 11px !Important;}' +
                                '  .title_DettP{color:whitesmoke;font-size:24pt; border: solid 2px orange; font-family:\'AR CENA\';text-align:center;width:1000px}' +
                                '   .MenuCLSS{visibility:visible; align:left;font-family: Verdana, Geneva, sans-serif; color: white}' +
                                '   .tabRP_title{color: white;font-size: 14pt;font-weight:400;font-style: normal;text-decoration: none;font-family: Calibri, sans-serif;text-align: center;'+
                                               'vertical-align: middle;white-space: nowrap;border-style: none;border-color: inherit;border-width: medium;padding: 0px;background: #6E0000;}'+
            ...
            startMAIN: function(){
                try{
                    //Aggiunta NOTEPAD LATERALE
                    CHANGE.AddNotePad();
                    //IMPOSTO ID BTN_CAMBIO_ED/SELECT NEL MENU
                    if($("iframe", parent.document).length){
                        $('select',frmMenu.document)[0].id="selEDIFICI";
                        for(var b=0;b<$('input',frmMenu.document).length;b++){
                           if($('input',frmMenu.document)[b].value=='˄'){$('input',frmMenu.document)[b].id="BtnPrevEd";}
                            if($('input',frmMenu.document)[b].value=='˅'){$('input',frmMenu.document)[b].id="BtnNextEd";}
                        }
                    }
                    GAME.ACTION(_PAGE);
                }catch(e){
            ...
            ACTION : function(name){
                switch(name){
                    case 'fleet.php' : CHANGE.MISSIONI();break;
                    case 'unitview.php': CHANGE.RIEP_MISSIONI();break;
                    case 'player.php' : CHANGE.PLAYER();break;
                    case 'highscore.php': CHANGE.mainSCORE(); break;
                    case 'report.php' : CHANGE.REPORT();break;
                    case 's_config.php' : CHANGE.CONFIG();break;
                }
            }
        };
        /*################          MAIN SCRIPT            ###############*/
        try{
            ...
            if(_PAGE){if(_PAGE != 'logout.php'){if(_PAGE=='nav.php'){CHANGE.MENU();}else{GAME.startMAIN();}}}else{console.log('{ MAIN : _INDEX } Non riesco a ricavare url/name page dal gioco !!!!')}
            //if(_PAGE){if(_PAGE != 'logout.php'){ GAME.startMAIN(); }}else{console.log('{ MAIN : _INDEX } Non riesco a ricavare url/name page dal gioco !!!!')}
        }catch(e){
