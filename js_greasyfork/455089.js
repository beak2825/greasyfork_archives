// ==UserScript==
// @name               nga_wd_helper2 - SelfAdvanced
// @name:zh            NGA版主助手2 - SelfAdvanced
// @name:zh-CN         NGA版主助手2 - SelfAdvanced
...
// @grant              GM.setValue
// @grant              GM.getValue
// @downloadURL https://update.greasyfork.org/scripts/455089/nga_wd_helper2%20-%20SelfAdvanced.user.js
// @updateURL https://update.greasyfork.org/scripts/455089/nga_wd_helper2%20-%20SelfAdvanced.meta.js
// ==/UserScript==

/* View original-version Code in:
 *  - https://greasyfork.org/zh-CN/scripts/438912-nga-wd-helper2/code
 *  - Last update: v1.03, 20231015.
*/
...
(async function() {
    ...
    commonui.mainMenu && commonui.mainMenu.addItemOnTheFly('WD助手', null, () => {
        ...
        w._.addContent(null);
        w._.addContent(
            '如有疑问，请前往阅读',_$('/a','class','teal b','href','https://shimo.im/docs/QhJd3dKVvWh9Cx9W','target','_blank','innerHTML','[帮助文档]'),_$('/br'),_$('/br'),
    ...
    if (location.pathname == '/') {
        ...
        if (page.__GP.admincheck & 2 && !$('nga_wd_helper_pids_add')) {
                                ...
                                page.__NUKE.doRequest({
                                    ...
                                    f: d => {
                                        if (!d?.error && d?.data?.[0]) {
                                            ...
                                            const result = `id:${ids_i} ${d.error[0]}`;
                                            results += result + '\n';
                                            mas(ids);
                                        }
                                        else {
                                            alert(`[${ids_i}/${ids_i2}] Request Failed!`);
                                            console.log(d);
                                        }
                                    },
                                });
            ...
            setTimeout(()=>{
                ...
                td_pids_lessernuke.addEventListener('click', () => {
                    const fid = page.__CURRENT_FID;
                    const tid = page.__CURRENT_TID;
                    //var myLesserNuke = commonui.oldLesserNuke ? commonui.oldLesserNuke : commonui.lessernuke;
                    if(commonui.oldLesserNuke) {
                        commonui.oldLesserNuke(this, 1, 1, null);
                    }
                    else{
                        commonui.lesserNuke(this, 1, 1, null);
                    }
                    ...
                    page.__NUKE.doRequest({
                        u: { u: '/nuke.php?__lib=modify_forum&__act=get_rule&raw=3', a: { tid: 1, pid: 1, fid: page.__CURRENT_FID, ffid: '' } },
                        f: function (d) {
                            const e = page.__NUKE.doRequestIfErr(d);
                            if (e || !d.data[0]) {
                                return;
                            }
                        ...
                        if (confirm('即将进入批量循环操作\n请再次检查参数设置\n批量操作过程中不要关闭窗口或离开本页面\n操作完成后会有弹窗提示')) {
                            ...
                            const mas = function (pids) {
                                if (pids.length) {
                                    ...
                                    if (pid) {
                                        page.__NUKE.doRequest({
                                            u: { u: page.__API._base, a: { __lib: 'nuke', __act: 'lesser_nuke', tid: tid, pid: pid, opt: opt, info: il, infos: ist, infosk: '', raw: 3, nga_wd_helper_pids_lessernuke: 1 } },
                                            f: function (d) {
                                                if (d && !d.error && d.data) {
                ...
                td_pids_add.addEventListener('click', () => {
                    const fid = page.__CURRENT_FID;
                    const tid = page.__CURRENT_TID;
                    page.adminui.addpoint(null, 1, 1, fid);
                    ...
                    new_button.addEventListener('click', () => {
                        ...
                        if (confirm('即将进入批量循环操作\n请再次检查参数设置\n批量操作过程中不要关闭窗口或离开本页面\n操作完成后会有弹窗提示')) {
                            ...
                            commonui.userCache.set('lastTipInfo', info, 86400 * 30);
                            commonui.userCache.set('lastTipAmt', value, 86400 * 30);
                            let results = '';
                            const mas = function (pids) {
                                if (pids.length) {
                                    const pid = pids.shift();
                                    if(pid){
                                        page.__NUKE.doRequest({
                                            u: { u: page.__API._base, a: { __lib: 'add_point_v3', __act: 'add', opt: opt, fid: fid, tid: tid, pid: pid, info: info, value: value, raw: 3, nga_wd_helper_pids_add: 1 } },
                                            f: function (d) {
                                                if (d && !d.error && d.data) {
                                                    const result = `pid:${pid} ${d.error ? d.error[0] : d.data[0]}`;
                                                    results += result + '\n';
        ...
        if ((page.__GP.admincheck & 2) && !(nga_wd_helper_offs & 1)) {
            const ajax_function = () => {
                const floors = commonui.postArg.data;
                for (let i in floors) {
                    if (floors[i].recommendO && !floors[i].recommendO.nga_wd_helper_score && floors[i].pAid != '41369816') {
                        floors[i].recommendO.nga_wd_helper_score = true;
        ...
        if (page.__GP.admincheck & 2) {
            ...
            td_tids_push.addEventListener('click', () => {
                page.adminui.createadminwindow();
                page.adminui.w._.addContent(null)
                page.adminui.w._.addTitle('提前或下沉主题(批量)');
                const push_action = (down = '') =>{
                    const tids_checked = page.commonui.massAdmin.getChecked();
                    if (!tids_checked) {
                        return;
                    }
                    ...
                    if (confirm('即将进入批量循环操作\n请再次检查参数设置\n批量操作过程中不要关闭窗口或离开本页面\n操作完成后会有弹窗提示')) {
                        ...
                        const mas = function (tids) {
                            if (tids.length) {
                                ...
                                page.__NUKE.doRequest({
                                    u: { u: page.__API._base, a: { __lib: 'topic_push', __act: 'push', tid: tid, down: down, raw: 3, nga_wd_helper_tids_push: 1 } },
                                    f: function (d) {
                                        if (d && !d.error && d.data) {
            ...
            const tids_color_interval = setInterval(() => {
                ...
                if (ori_bar) {
                    ...
                    ori_bar.appendChild(td_tids_push);
                }
            }, 100);
            setTimeout(() => {
                clearInterval(tids_color_interval);
