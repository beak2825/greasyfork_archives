...
// @license     PTP
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/462244/PTP%20Trump%20Helper%20056.user.js
// @updateURL https://update.greasyfork.org/scripts/462244/PTP%20Trump%20Helper%20056.meta.js
// ==/UserScript==
...
(function () {
  ...
  const allReportsOnPage = onReportsPage
    ...
    : Array.from(document.querySelectorAll("#all_reports")[0].children).slice(
        1
      );
  ...
  allReportsOnPage.forEach((report) => {
    ...
    if (self.nodeName.toLowerCase() === "div") {
      ...
      const trumpInitSelected = self.querySelector(
        `#resolve_type${reportIdNum} > option:nth-child(${
          onReportsPage ? 12 : 7
        })`
      ).selected;
      const resolveRow = self.querySelector(`#options${reportIdNum}`);
      const tBody = self.querySelector("tbody");
      const selectResolveType = self.querySelector(
        `#resolve_type${reportIdNum}`
      );
      ...
      if (!onReportsPage || !trumpInitSelected) {
        ...
        plInput.addEventListener(
          "input",
          handlePlInputChange.bind(plInput, logMessageField)
        );
      ...
      selectResolveType.addEventListener(
        "change",
        showOrHide.bind(this, self, reportIdNum)
      );
      ...
      if (self.querySelector("#Active_trump") === null) {
        for (let i = 0; i < trumpReasons.length; i++) {
          ...
          childSpan.addEventListener(
            "click",
            handleCheckboxClick.bind(input, logMessageField)
          );
  ...
  function handlePlInputChange(logMessageField) {
    ...
    } else if (this.dataset.lastVal.length !== this.value.length) {
  ...
  function handleCheckboxClick(logMessageField) {
    if (event.target.className === "trumpLabel") {
