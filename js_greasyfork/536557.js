...
// @license    MIT
// @downloadURL https://update.greasyfork.org/scripts/536557/HTML5%E8%A7%86%E9%A2%91%E6%92%AD%E6%94%BE%E5%B7%A5%E5%85%B7.user.js
// @updateURL https://update.greasyfork.org/scripts/536557/HTML5%E8%A7%86%E9%A2%91%E6%92%AD%E6%94%BE%E5%B7%A5%E5%85%B7.meta.js
// ==/UserScript==
...
const cfg = {
	...
	multipleV: !1, //多视频页面
	isNumURL: !1, //网址数字分集
	isDouyinFeed: host === 'www.douyin.com' && location.pathname === '/' //抖音首页推荐
};
const bus = new Vue();
...
.set(80, _ => { //按键P：截图
	try {
		if(cfg.isDouyinFeed) {
			...
			for(const video of videos) {
				if(video.getBoundingClientRect().top >= 0 &&
				   video.getBoundingClientRect().bottom <= window.innerHeight) {
					v = video;
					break;
				}
			}
		...
		canvas.toBlob((blob) => {
			...
			const timestamp = String(Math.floor(v.currentTime)).padStart(3, '0');
			GM_download({
				url: dataURL,
				name: `${formattedDate}_${timestamp}.jpg`,
				onloadend: _ => {
