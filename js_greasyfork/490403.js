...
// @grant GM_setValue
// @grant GM_getValue
// @downloadURL https://update.greasyfork.org/scripts/490403/Noxtrip%20Scripts.user.js
// @updateURL https://update.greasyfork.org/scripts/490403/Noxtrip%20Scripts.meta.js
// ==/UserScript==
...
(function() {
    ...
    var maxPocetZemiPrePredikcie = 16 // Nad tento pocet uz treba zapnut predikcie cez checkbox
    ...
    // 2. Script nevie o obyvateloch cudzej zeme. Pre taketo obchodky sa pouziva bulharska konstanta 10%
    ...
    function process_centrala( ){
        ...
        if (zeme.length > maxPocetZemiPrePredikcie && !GM_getValue("noxtrip_zisk_prediction",false)){
            return;
        }
    ...
    function addCentralaCheckbox(){
        if ( document.getElementById("noxtrip_zisk_prediction")){
            return;
        }
        var checkboxTable = document.createElement('table');
        checkboxTable.style.paddingTop = "10px";
        checkboxTable.style.paddingBottom = "5px";
        var settingTr = document.createElement('tr');
        var centralaCheckbox = document.createElement('input');
        centralaCheckbox.setAttribute("type","checkbox")
        centralaCheckbox.setAttribute("id","noxtrip_zisk_prediction");
        centralaCheckbox.checked = GM_getValue("noxtrip_zisk_prediction",false);
        centralaCheckbox.addEventListener('change', (event) => {
            if (event.currentTarget.checked) {
                GM_setValue("noxtrip_zisk_prediction",true)
                location.reload();
            } else {
                GM_setValue("noxtrip_zisk_prediction",false)
            }
        })
        settingTr.appendChild( centralaCheckbox);
        var checkboxLabel = document.createElement('label');
        checkboxLabel.setAttribute("for","noxtrip_zisk_prediction");
        checkboxLabel.innerText = "Predikcie a návratnosť";
        settingTr.appendChild( checkboxLabel);
        checkboxTable.appendChild(settingTr);
        var centralaDiv = document.getElementById("centrala").getElementsByTagName("DIV");
        document.getElementById("centrala").insertBefore(checkboxTable, centralaDiv[ centralaDiv.length -1] );
    }
            ...
            if (zeme.smlouvy[i].obchodka){
                for (var j = 0; j < vsetkyZeme.length; j++){
                    if (vsetkyZeme[j].id === zeme.smlouvy[i].id ){
                        var pomer = vsetkyZeme[j].obyvateliaPoKolach[kolo] / zeme.obyvateliaPoKolach[kolo];
                        var percento = pomer * 0.18;
                        if (percento > 0.4){
                            percento = 0.4;
                        }
                        var ziskObchodka = 5 * percento * zeme.attributy.magia_zlato * zeme.obyvateliaPoKolach[kolo];
                        mojaZem = true;
                        break;
                    }
                }
    ...
    function calculateZiskCudzieObchodkyZaKolo( zeme, kolo, dilna, lenMojZisk){
        ...
        if (!zeme.obyvCudzieZemkyVPriemere){
            ...
            var percentoZCudzichObchodiek = ziskCudzichObchodiek / ( zeme.obyvateliaPoKolach[kolo] * 5 * zeme.attributy.magia_zlato * dilnaModifier);
            var percentoCudzej = percentoZCudzichObchodiek / zeme.pocetCuzdichObchodiek;
            zeme.obyvCudzieZemkyVPriemere = (percentoCudzej / 0.18) * zeme.obyvateliaPoKolach[kolo];
        }
        var pomer = zeme.obyvCudzieZemkyVPriemere / zeme.obyvateliaPoKolach[kolo];
        var percento = pomer * 0.18;
        if (percento > 0.4){
            percento = 0.4;
        }
        var ziskCudziaObchodka = 5 * zeme.obyvateliaPoKolach[kolo] * percento * zeme.attributy.magia_zlato * dilnaModifier;
