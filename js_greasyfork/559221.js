// ==UserScript==
// @name         OLM Study Assistant Pro v8.1 - Cosmic (Fix q_type 21)
// @namespace    http://tampermonkey.net/
// @version      8.1
// @description  Công cụ hỗ trợ học tập, bắt đáp án thông minh (kể cả tự luận) với giao diện vũ trụ + xác thực key qua server + fix lệch câu / gợi ý TV / sắp xếp câu / đánh số chuẩn với OLM.
// @author       TIỆP GÀ CUI
// @match        https://olm.vn/*
// @grant        unsafeWindow
// @grant        GM_addStyle
...
// @downloadURL https://update.greasyfork.org/scripts/559221/OLM%20Study%20Assistant%20Pro%20v81%20-%20Cosmic%20%28Fix%20q_type%2021%29.user.js
// @updateURL https://update.greasyfork.org/scripts/559221/OLM%20Study%20Assistant%20Pro%20v81%20-%20Cosmic%20%28Fix%20q_type%2021%29.meta.js
// ==/UserScript==
...
(function () {
    ...
    const Config = {
        ...
        API_ENDPOINT: 'https://hotroal.xyz/api_check_key.php',
    ...
    const AuthService = {
        ...
        generateDeviceId() {
            const id = 'olm_study_' + btoa(navigator.userAgent + Date.now()).substring(0, 20);
        ...
        async processValidation(key) {
            ...
            const url = `${Config.API_ENDPOINT}?key=${encodeURIComponent(trimmedKey)}&device_id=${encodeURIComponent(deviceId)}`;
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: (res) => {
                        try {
                            ...
                            console.log('[StudyAssistant] API response:', data);
                            ...
                            console.error('[StudyAssistant] Lỗi parse JSON API:', e, res);
                            this.isKeyValid = false;
                            GM_setValue('keyStatus', 'invalid');
                            resolve(false);
                        }
                    ...
                    onerror: (err) => {
                        console.error('[StudyAssistant] GM_xmlhttpRequest error:', err);
                        this.isKeyValid = false;
                        GM_setValue('keyStatus', 'invalid');
                        resolve(false);
                    }
        ...
        async checkCachedKey() {
            ...
            if (this.currentKey && cachedStatus === 'valid') {
                this.isKeyValid = true;
                return true;
            }
            if (this.currentKey) {
    ...
    class StudyPanel {
        ...
        _createPanelContainer() {
            ...
            settingsButton.onclick = () => {
                const keyMeta = GM_getValue('keyMeta', null);
                let extra = '';
                if (keyMeta) {
                    extra =
                        `\nTrạng thái: ${keyMeta.status}` +
                        `\nThiết bị: ${keyMeta.devices_used}/${keyMeta.max_devices}` +
                        (keyMeta.note ? `\nGhi chú: ${keyMeta.note}` : '');
                }
                alert(
                    `Study Assistant Pro v${Config.VERSION}\n` +
                    `Thiết bị: ${AuthService.deviceId}\n` +
                    `Key: ${AuthService.currentKey || 'Chưa thiết lập'}${extra}`
                );
            };
    ...
    const UIManager = {
        ...
        updateUIState() {
            const isValid = AuthService.isKeyValid;
            ...
            if (isValid) {
                ...
                this.toggleButton.classList.add('invalid');
                this.toggleButton.innerHTML = '🔒';
                this.toggleButton.title = 'Nhập mã kích hoạt để sử dụng';
            }
            this.panel.setVisible(isValid && this.isPanelEnabled);
        },
        ...
        _showKeyModal() {
            const existingModal = document.getElementById('key-modal');
            ...
            const innerHtml = `
                <div style="
                    background: radial-gradient(circle at top left, #4c1d95 0%, transparent 55%),
                                radial-gradient(circle at bottom right, #1d4ed8 0%, transparent 55%),
                                linear-gradient(145deg, #0f172a 0%, #020617 52%, #0b1120 100%);
                    padding: 50px 40px;
                    border-radius: 28px;
                    text-align: center;
                    max-width: 480px;
                    ...
                    overflow: hidden;
                ">
                    <div style="position: absolute; inset: -30%; opacity: 0.12; background:
                        radial-gradient(circle at 10% 0%, #38bdf8 0, transparent 50%),
                        radial-gradient(circle at 90% 100%, #a855f7 0, transparent 50%);
                        filter: blur(8px);
                    "></div>
                    <div style="position: relative; z-index: 1;">
                        <div style="font-size: 64px; margin-bottom: 25px;">🎓</div>
                        <h2 style="color: #f9fafb; margin-bottom: 8px; font-weight: 800; font-size: 28px;">
                            Study Assistant Pro
                        </h2>
                        <p style="color: #9ca3af; margin-bottom: 20px; font-size: 15px;">
                            Nhập mã kích hoạt để mở khóa toàn bộ tính năng hỗ trợ học tập.
                        </p>
                        <p style="color:#6b7280; font-size: 13px; margin-bottom: 20px;">
                            Mặc định: <code style="color:#a855f7;">${Config.DEFAULT_KEY}</code> (bạn có thể đổi thành mã riêng của mình)
                        </p>
                        <input id="key-input" type="password" placeholder="Nhập mã kích hoạt..."
                            style="
                                width: 100%;
                                padding: 16px 18px;
                        ...
                        <div style="font-size: 13px; color: #9ca3af; margin-bottom: 6px;">
                            Mã thiết bị:
                            <code style="color: #60a5fa;">${AuthService.deviceId}</code>
                        </div>
                        <div style="font-size: 12px; color:#6b7280;">
                            Gợi ý: lưu lại mã thiết bị nếu bạn muốn quản lý key sau này.
                        </div>
                    </div>
                </div>
            `;
            ...
            const activateBtn = modal.querySelector('#validate-btn');
            ...
            const submit = async () => {
                ...
                const isValid = await AuthService.processValidation(key);
                if (isValid) {
                    modal.remove();
                    alert('🎉 KÍCH HOẠT THÀNH CÔNG!');
                    this.isPanelEnabled = true;
                    GM_setValue('isScriptEnabled', true);
                    this.updateUIState();
                    ApiInterceptor.init(this.processApiData.bind(this));
                } else {
                    alert('❌ Mã kích hoạt không hợp lệ hoặc đã đạt giới hạn thiết bị!');
                    activateBtn.textContent = '🚀 KÍCH HOẠT NGAY';
                }
            };
            activateBtn.onclick = submit;
    ...
    async function main() {
        try {
            ...
            if (AuthService.isKeyValid) {
                ...
                setTimeout(() => UIManager._showKeyModal(), 1500);
            }
        } catch (e) {
