...
// @version 0.0.1.20140830173008
// @downloadURL https://update.greasyfork.org/scripts/4151/Naltrexone%27s%20KoL%20Scripts%20-%20WikiLink.user.js
// @updateURL https://update.greasyfork.org/scripts/4151/Naltrexone%27s%20KoL%20Scripts%20-%20WikiLink.meta.js
// ==/UserScript==


/********************************** Recent Changes **********************************************
Recent Updates:
2.6  Updated to respect GreaseMonkey 2.0 (or greater) security settings
     This is obviously just a first pass to get it back up and working-- will
     add support for new pages and item types later this weekend.
2.5  Avoided conflict with "Down the Rabbit Hole".  More fixes coming soon.
2.4  Internal build
2.3  Updated the familiar decoder
     Avoided some combat messages that shouldn't be wikified.
2.2  Fixes conflicts with Picklish's scripts
     Avoids extraneous tag in the Desktop Globe fight
...
const familiarDecoder = ["","Mosquito","Leprechaun","Levitating Potato","Angry Goat","Sabre-Toothed Lime","Fuzzy Dice",
	    ...
	    "Scary Death Orb","Jitterbug","Nervous Tick","","Origami Towel Crane","","","Toothsome Rock","","Ancient Yuletide Troll"];
...
const familiarImageNameDecoder = [["hat2","Hovering Sombrero"],["sgfairy","Spooky Gravy Fairy"],["npzr","Ninja Pirate Zombie Robot"],
	    ["slgfairy","Sleazy Gravy Fairy"],["chatteeth","Wind-up Chattering Teeth"],["ghobo","Spirit Hobo"],
...
// Some things look like they should be wikified, but shouldn't.  Here's a list to check against before applying markup.
...
function insertWikiLink(itemElement, wikiEntryName, textToInsertAfter) {
      if ((itemElement.tagName == 'A') || (itemElement.tagName == 'IMG') || (itemElement.getAttribute('id') == 'monname')) {
...
if ((window.location.pathname != "/charpane.php")  &&
    ...
    for (var i=0; i < itemElements.length; i++ ) {
      ...
      if (((curItem.innerHTML.indexOf(':') < 0) || (curItem.innerHTML.indexOf('NOTE:') >= 0)) && // Anything with a : other than a NOTE is a header.
         ...
         (curItem.innerHTML.indexOf('perpendicular') < 0) &&  // avoids wikifying the "hint" bolding in the Giant Desktop Globe fight
        ...
        if (curItem.innerHTML.indexOf('Brrrrr') >=0) { wasSpecialCase = 1; } 
