// ==UserScript==
// @name         FC-Lomo
// @namespace    https://greasyfork.org/es/scripts/20775-fc-Lomo
// @version      1.2
...
// @downloadURL https://update.greasyfork.org/scripts/20775/FC-Lomo.user.js
// @updateURL https://update.greasyfork.org/scripts/20775/FC-Lomo.meta.js
// ==/UserScript==
/* jshint -W097 */
'use strict';
...
{
	var words = {
		politics: ["partido popular", " C'S", "votado", "podemita","pudremitas", "riverita", "peperro","político", "Saenz de Santamaria", "Sáenz de Santamaría", "votantes", "pdr snchz", "partidos politicos", "partidos políticos", "partido político", "partido politico", "Pedro Sánchez", "Pedro Sanchez", "rajoy","soraya", "falange", "pableras", "upyd", "coletas","psoe","debate", "carmena", "voto", "pablemos", "ciudadanos", "ciutadans", "discurso", "albert", "rivera", "iglesias", "elecciones", "votar", "votación", "votacion", "pedro sanchez", "pedro sánchez", "podemos", "podemitas", " pp"],
		football: ["Peña Real Valladolid","Peña Bética", "Peña Athletic", "PEÑA COLCHONERA", "PENYA BLAUGRANA", "piqué", "PEÑA CELTISTA", "real madrid", "futbol", "fútbol", "porra", "apuesta", "penya culer", "peña culer", "culers", "culeros", "culerdos", "FC Barcelona", "barça", "Barsa", "atleti", "PEÑA VALENCIANISTA", "merengue"]
	},
	censorList = [];
	...
	{
		window.$(".cajasprin:nth-of-type(1) tr").eq(1).append('<td><div id="Lomo-config" title="Configuración de Lomo (borrado de temas)">Anti-CiberVoluntarios</div></d>');
			...
			<div id="Lomo-panel">
				...
				<ul>
					...
					<li>
						...
						<label>
							Filtro propio:<br>
							<textarea name="Lomo-custom" placeholder="gore,movistar"></textarea>
						</label>
					</li>
				</ul>
		...
		*/});
		...
		window.$("#Lomo-config").on("click", function () {
			window.$('#Lomo-panel').toggle();
		});

		window.$("#Lomo-panel input").on("change", function () {
			updateCensor(window.$(this).val(), window.$(this).is(":checked"));
		});
		window.$("#Lomo-panel textarea").on("change", function () {
			updateCensor("custom",window.$(this).val());
		});
		
		window.$("#Lomo-panel button").on("click", function () {
			location.reload();
		});
		loadConfig();
	};
	var loadConfig = function ()
	{
		...
		for (k in config) {
			if (config[k]) {
				if (k == "custom") {
					window.$('#Lomo-panel textarea[name="Lomo-custom"]').val(config[k]);
				} else {
					window.$("#Lomo-panel input[value=" + k + "]").prop("checked", true);
				}
			}
		}
	};
	...
	var setConfig = function (config) {
		localStorage.Lomo = JSON.stringify(config);
	};
		...
		if ($mainForum.length > 0)
		{
			...
			$mainForum.append(parseTemplate(function(){/*
				...
				<tr class="Lomo-separator">
					<td></td>
					<td></td>
  					<td><a id="Lomo-next-page" href="#">Siguiente página &gt;</a></td>
				  	<td></td>
				  	<td></td>
				</tr>
				<tr class="Lomo-separator">
				...
				<tr class="Lomo-separator">
					<td class="tcat"></td>
					<td class="tcat"></td>
					<td class="tcat">Posts eliminados por Lomo</td>
				  	<td class="tcat" id="Lomo-count" align="right">0</td>
				  	<td class="tcat"></td>
				</tr>
				<tr>
			...
			*/}));
			...
			// General section
			window.$('#threadbits_forum_2 tr:not(.Lomo-separator)').each(function ()
			{
			...
			window.$('#Lomo-count').text(deletedCount);
			window.$('#Lomo-next-page').attr("href", window.$('a[rel="next"]').eq(0).attr("href"));
		} else {
			// Home
