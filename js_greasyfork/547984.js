...
// @downloadURL https://update.greasyfork.org/scripts/547984/%E6%8A%96%E9%9F%B3%E7%B4%A0%E6%9D%90%E6%95%B0%E6%8D%AE.user.js
// @updateURL https://update.greasyfork.org/scripts/547984/%E6%8A%96%E9%9F%B3%E7%B4%A0%E6%9D%90%E6%95%B0%E6%8D%AE.meta.js
// ==/UserScript==
...
(function () {
  ...
  const API_ENDPOINT = "https://business.oceanengine.com/nbs/api/bm/video/list";
  const DEFAULT_LIMIT = 100;
  const INCLUDE_METRICS = ["show_cnt", "total_play", "conversion_rate", "conversion_cost", "valid_play_rate", "ctr", "stat_cost", "cpm_platform", "click_cnt", "cpc_platform"];
  const concurrency = 3;
  const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
  const fetchPage = async (page, startDate, endDate, csrftoken, GROUP_ID) => {
    var _a, _b;
    const params = {
      group_id: GROUP_ID,
      version: "v2",
      ...
      metrics_start_time: `${startDate} 00:00:00`,
      metrics_end_time: `${endDate} 23:59:59`,
      page,
      limit: DEFAULT_LIMIT,
      owner: {
        owner_id: GROUP_ID,
    ...
    const { data } = await axios$1.post(`${API_ENDPOINT}?group_id=${GROUP_ID}`, params, {
      withCredentials: true,
      headers: {
        "x-csrftoken": csrftoken
      }
    });
    console.log("🚀 第", page, "页获取成功，条数：", (_b = (_a = data == null ? void 0 : data.data) == null ? void 0 : _a.videos) == null ? void 0 : _b.length);
    return data;
  };
  const fetchPageWithRetry = async (page, startDate, endDate, csrftoken, GROUP_ID, retry = 2) => {
    try {
      return await fetchPage(page, startDate, endDate, csrftoken, GROUP_ID);
    } catch (err) {
      if (retry > 0) {
        console.log("🔁 第", page, "页请求失败，重试剩余：", retry);
        await delay(500 + Math.floor(Math.random() * 500));
        return fetchPageWithRetry(page, startDate, endDate, csrftoken, GROUP_ID, retry - 1);
      }
      throw err;
    }
  };
  const fetchAll = async (startDate, endDate, csrftoken, GROUP_ID, onProgress) => {
    var _a, _b, _c, _d, _e;
    const first = await fetchPageWithRetry(1, startDate, endDate, csrftoken, GROUP_ID);
    if ((first == null ? void 0 : first.code) !== 0) {
      console.log("❌ 首次请求失败：", (first == null ? void 0 : first.msg) || "未知错误");
      return { totalPage: 0, totalCount: 0, videos: [] };
    }
    const totalPage = ((_b = (_a = first == null ? void 0 : first.data) == null ? void 0 : _a.pagination) == null ? void 0 : _b.total_page) || 1;
    const totalCount = ((_d = (_c = first == null ? void 0 : first.data) == null ? void 0 : _c.pagination) == null ? void 0 : _d.total_count) || 0;
    const allVideos = [...((_e = first == null ? void 0 : first.data) == null ? void 0 : _e.videos) || []];
    console.log("🚀 首页获取成功，条数：", allVideos.length, "，总页数：", totalPage, "，总条数：", totalCount);
    let donePages = Math.min(1, totalPage);
    onProgress == null ? void 0 : onProgress(donePages, totalPage);
    const remainPages = [];
    for (let p = 2; p <= totalPage; p++)
      remainPages.push(p);
    const queue = new PQueue({ concurrency });
    console.log("📦 计划拉取页数：", remainPages.length, "｜并发：", concurrency);
    const promises = remainPages.map(
      (p) => queue.add(async () => {
        var _a2;
        const res = await fetchPageWithRetry(p, startDate, endDate, csrftoken, GROUP_ID);
        if ((res == null ? void 0 : res.code) === 0) {
          const list = ((_a2 = res == null ? void 0 : res.data) == null ? void 0 : _a2.videos) || [];
          allVideos.push(...list);
        } else {
          console.log("⚠️ 某页返回异常：", (res == null ? void 0 : res.msg) || "未知错误");
        }
        donePages += 1;
        onProgress == null ? void 0 : onProgress(donePages, totalPage);
      })
    ...
    console.log("✅ 全部分页完成，累计条数：", allVideos.length);
    return { totalPage, totalCount, videos: allVideos };
  };
  const exportToXlsx = async (videos, fileName) => {
    const rows = videos.map((v) => {
      var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m, _n;
      const duration = ((_a = v == null ? void 0 : v.video_info) == null ? void 0 : _a.duration) ?? "";
      const showCnt = ((_b = v == null ? void 0 : v.metrics) == null ? void 0 : _b.show_cnt) ?? "";
      const cost = ((_c = v == null ? void 0 : v.metrics) == null ? void 0 : _c.stat_cost) ?? "";
      const playCnt = ((_d = v == null ? void 0 : v.metrics) == null ? void 0 : _d.total_play) ?? "";
      const conversionRate = ((_e = v == null ? void 0 : v.metrics) == null ? void 0 : _e.conversion_rate) ?? "";
      const avgConversionCost = ((_f = v == null ? void 0 : v.metrics) == null ? void 0 : _f.conversion_cost) ?? "";
      const effectivePlayRate = ((_g = v == null ? void 0 : v.metrics) == null ? void 0 : _g.valid_play_rate) ?? "";
      const clickRate = ((_h = v == null ? void 0 : v.metrics) == null ? void 0 : _h.ctr) ?? "";
      const clickCnt = ((_i = v == null ? void 0 : v.metrics) == null ? void 0 : _i.click_cnt) ?? "";
      const avgClickCost = ((_j = v == null ? void 0 : v.metrics) == null ? void 0 : _j.cpc_platform) ?? "";
      const avgShowCost = ((_k = v == null ? void 0 : v.metrics) == null ? void 0 : _k.cpm_platform) ?? "";
      return {
        视频名称: (v == null ? void 0 : v.video_name) || "",
        视频ID: (v == null ? void 0 : v.video_id) || "",
        素材ID: (v == null ? void 0 : v.material_id) || "",
        创建时间: (v == null ? void 0 : v.create_time) ? dayjs(v == null ? void 0 : v.create_time).format("YYYY-MM-DD HH:mm:ss") : "",
        花费: cost,
        时长_秒: duration,
        素材评估: Array.isArray(v == null ? void 0 : v.material_properties) ? (_l = v == null ? void 0 : v.material_properties) == null ? void 0 : _l.join(",") : "",
        标签: Array.isArray(v == null ? void 0 : v.organization_tags) ? (_m = v == null ? void 0 : v.organization_tags) == null ? void 0 : _m.join(",") : "",
        所属账户ID: Array.isArray(v == null ? void 0 : v.source_owners) ? (_n = v == null ? void 0 : v.source_owners) == null ? void 0 : _n.map((item) => item == null ? void 0 : item.id).join(",") : "",
        展示数: showCnt,
        播放量: playCnt,
        转化率: conversionRate,
        平均转化成本: avgConversionCost,
        有效播放率: effectivePlayRate,
        点击率: clickRate,
        消耗: cost,
        千次展现费用: avgShowCost,
        点击数: clickCnt,
    ...
    utils.book_append_sheet(workbook, worksheet, "videos");
    writeFileSync(workbook, fileName);
  };
  const taskGetAll = async (startDate, endDate, csrftoken, GROUP_ID, onProgress) => {
    if (!startDate || !endDate) {
      console.log("❌ 日期范围无效：", startDate, endDate);
      return;
    }
    if (!csrftoken) {
      console.log("❌ 缺少 csrftoken");
      return;
    }
    const all2 = await fetchAll(startDate, endDate, csrftoken, GROUP_ID, onProgress);
    console.log("🎉 全量数据准备完成，视频条数：", all2.videos.length, "，总页数：", all2.totalPage, "，总条数：", all2.totalCount);
    await exportToXlsx(all2.videos, `${startDate} ~ ${endDate}.xlsx`);
    console.log("📤 已导出 Excel：", `${startDate} ~ ${endDate}.xlsx`);
  };
  const containerSelector = ".header-head-right";
  const wrapperId = "sucaiExportWrapper";
  const maxRetry = 20;
  const retryMs = 500;
  function getCsrfToken() {
    var _a;
    return (_a = document.cookie.split("; ").find((row) => row.startsWith("csrftoken="))) == null ? void 0 : _a.split("=")[1];
  }
  function getDateRange() {
    var _a, _b, _c, _d;
    const inputs = document.querySelectorAll(".ovui-custom-input__content input");
    if (!inputs || inputs.length < 2) {
      console.log("❌ 未找到日期输入框");
      return null;
    }
    const startDate = (_b = (_a = inputs[0]) == null ? void 0 : _a.value) == null ? void 0 : _b.trim();
    const endDate = (_d = (_c = inputs[1]) == null ? void 0 : _c.value) == null ? void 0 : _d.trim();
    if (!startDate || !endDate) {
      console.log("❌ 日期范围不完整，请先选择开始与结束日期");
      return null;
    }
  ...
  function getCcId() {
    ...
    const ccId = url.split("?")[1].split("=")[1];
    return ccId;
  }
  function createExportButton() {
    const btn = document.createElement("button");
    btn.textContent = "导出素材数据";
    btn.style.cssText = `
    padding: 4px 8px;
    border: none;
    border-radius: 3px;
  ...
  `;
    ...
    btn.addEventListener("click", async () => {
      ...
      if (!csrftoken) {
        console.log("❌ 未找到 csrftoken");
        return;
      }
      const GROUP_ID = getCcId();
      if (!GROUP_ID) {
        console.log("❌ 未找到 GROUP_ID");
        return;
      }
      console.log("✅ 导出素材点击：开始导出", startDate, endDate);
      const originalText = btn.textContent;
      btn.disabled = true;
      btn.textContent = "导出中...";
      try {
        await taskGetAll(startDate, endDate, csrftoken, GROUP_ID, (done, total) => {
          btn.textContent = `导出中 ${done}/${total}...`;
        });
        console.log("🎉 导出完成：", `${startDate} ~ ${endDate}.xlsx`);
      } catch (error) {
        console.log("❌ 导出失败：", (error == null ? void 0 : error.message) || error);
      } finally {
        btn.disabled = false;
        btn.textContent = originalText || "导出素材数据";
      }
    });
    return btn;
  }
  function mountExportButton(retryCount = 0) {
    if (document.getElementById(wrapperId)) {
      console.log("ℹ️ 按钮已存在，跳过挂载");
      return;
    }
    const container = document.querySelector(containerSelector);
    if (!container) {
      if (retryCount < maxRetry) {
        setTimeout(() => mountExportButton(retryCount + 1), retryMs);
        return;
      }
      console.log("⚠️ 未找到挂载容器，放弃挂载");
      return;
    }
    const wrapper = document.createElement("div");
    wrapper.id = wrapperId;
    wrapper.classList.add("header-head-jumplink", "text-skyBlue-6", "flex", "items-center");
    wrapper.style.cssText = "margin-right: 10px;";
    wrapper.append(createExportButton());
    const firstChild = container.firstElementChild;
    if (firstChild) {
      container.insertBefore(wrapper, firstChild);
      console.log("✅ 导出按钮已作为第一个节点挂载");
    } else {
      container.appendChild(wrapper);
      console.log("✅ 导出按钮已挂载到页面");
    }
  }
  mountExportButton();
