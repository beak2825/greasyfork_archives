...
(function(){
    'use strict';
    // 这里替换成你自己的网站域名，避免误杀，可以为空数组
    const whitelistSites = [];
    if (whitelistSites.some(site => location.hostname.includes(site))) {
        console.log('[广告拦截] 当前站点白名单，跳过拦截');
        return;
    }
    const adDomains = [
        'doubleclick.net',
    ...
    window.fetch = function() {
        if (arguments[0] && isAdUrl(arguments[0])) {
            console.log('[广告拦截] 阻止fetch请求:', arguments[0]);
            return new Promise(() => {});
        }
    ...
    XMLHttpRequest.prototype.open = function(method, url) {
        if (isAdUrl(url)) {
            console.log('[广告拦截] 阻止XHR请求:', url);
            return;
        }
        return originalOpen.apply(this, arguments);
    };
    // 重写adsbygoogle对象push函数，阻止广告加载
    window.adsbygoogle = window.adsbygoogle || [];
    window.adsbygoogle.push = function() {
        console.log('[广告拦截] 阻止adsbygoogle.push调用');
        return [];
    };
    // 删除广告iframe
    function removeAds() {
        try {
            document.querySelectorAll('iframe').forEach(iframe => {
                if (isAdUrl(iframe.src)) {
                    console.log('[广告拦截] 移除广告iframe', iframe.src);
                    iframe.remove();
                }
            });
        } catch(e) {
    ...
    // 初次DOM加载完成后删除广告iframe
    window.addEventListener('DOMContentLoaded', () => {
        removeAds();
    });
    // 监听DOM变动动态删除广告iframe，节流处理
    const observer = new MutationObserver(() => {
        clearTimeout(window._adblockTimeout);
        window._adblockTimeout = setTimeout(() => {
            removeAds();
        }, 500);
    });
