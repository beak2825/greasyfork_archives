// ==UserScript==
// @name         HDKylin-Torrent-bot
// @namespace    http://tampermonkey.net/
// @version      1.2.92
// @description  HDKylin审种机器人
...
// @downloadURL https://update.greasyfork.org/scripts/493972/HDKylin-Torrent-bot.user.js
// @updateURL https://update.greasyfork.org/scripts/493972/HDKylin-Torrent-bot.meta.js
// ==/UserScript==
...
console.log("title_type",title_type);
  ...
  console.log("title_audio:"+title_audio);
    ...
    if(/[^\x00-\xff]+/g.test(title) && !title.includes('￡') && !title.includes('™') && !/[\u2161-\u2169]/g.test(title) && !title.includes('Ⅰ') && !title.includes('白自在') && !title.includes('至尊宝')) {
        ...
        error = true;
      setTimeout(function() {let button = document.querySelector('#approval');
button.click();
}, 500);
    }
    if (!subtitle) {
