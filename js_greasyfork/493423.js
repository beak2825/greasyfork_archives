...
// @description:ja    没入型体験に焦点を当てる、1.人気の写真に直接アクセスする広告をブロックする2.検索する方法を入力するためにユーザーを使用する3.検索pidとuid 4.元の画像とサイズを表示する Zip | multiple map zip 5.アーティストID、アーティストの背景画像を表示します。6.自動ロードコメントを追加します。7.動的マークアップ作業タイプを指定します。8.リダイレクトを削除します。9.シングルページソート10.コントロールパネルを選択します。github：https：/ /github.com/MegaPiggy/Tampermonkey、スターとフォークへようこそ。
// @description:zh-CN 专注沉浸式体验，1.屏蔽广告,直接访问热门图片 2.使用users入り的方式搜索 3.搜索pid和uid 4.显示原图及尺寸，图片重命名，下载原图|gif图|动图帧zip|多图zip 5.显示画师id、画师背景图 6.自动加载评论 7.对动态标记作品类型 8.去除重定向 9.单页排序 10.控制面板选择想要的功能 github:https://github.com/MegaPiggy/Tampermonkey，欢迎star和fork。
// @description:zh-TW 專注沉浸式體驗，1.屏蔽廣告,直接訪問熱門圖片2.使用users入り的方式搜索3.搜索pid和uid 4.顯示原圖及尺寸，圖片重命名，下載原圖|gif圖|動圖幀zip|多圖zip 5.顯示畫師id、畫師背景圖6.自動加載評論7.對動態標記作品類型8.去除重定向9.單頁排序10.控制面板選擇想要的功能github:https:/ /github.com/MegaPiggy/Tampermonkey，歡迎star和fork。
// @author      MegaPiggy
...
// @license     GPL-3.0
// @supportURL  https://github.com/MegaPiggy/Tampermonkey
// @grant       unsafeWindow
...
// @run-at      document-end
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/493423/Pixiv%20Plus.user.js
// @updateURL https://update.greasyfork.org/scripts/493423/Pixiv%20Plus.meta.js
// ==/UserScript==
jQuery(function ($) {
    'use strict';
    // 加载依赖
    ...
    let i18nLib = {
        ...
        en: {
            enhanced_config: 'Pixiv Enhanced Configuration',
            pjax: 'PJAX compatible (recommended)',
            auto_load_comment: 'Auto-loading comments',
            auto_load_multi: 'Automatically load multiple images',
            show_image_size: 'Show image size',
            image_preload: 'Pre-download Gif, Zip (traffic consumption)',
            download_file_name: 'Download file name',
            save: 'Save',
            info_format: '{pid} is the artwork id--------{uid} is the artist id\\n{pname} is the name of the artwork--------{uname} is the name of the artist\\nNote that in the case of multiple images, the index numbers are automatically populated\\nCurrently only GIF and multi-image renaming is supported',
            description: 'Description',
            favorites: 'favorites',
            illegal: 'illegal',
            download: 'download',
            download_multiple: 'Download multiple images',
            download_gif: 'Download gifs',
            download_wait: 'Please wait until the download has completed!',
            download_gif_wait: 'Gif is not loaded, please wait a moment!',
            download_fix: 'Download has stopped, fixing...',
            ...
            id_intialization_fail: 'Search UID and PID initialization failed, form element fetch failed',
            search_enhance_intialization: 'Search Enhancement Initialization',
        },
        ko: {},
        zh: {
            enhanced_config: 'Pixiv增强配置',
            pjax: '兼容PJAX(推荐)',
            auto_load_comment: '自动加载评论',
            auto_load_multi: '自动加载多图',
            show_image_size: '显示图片尺寸大小',
            image_preload: '预下载Gif、Zip(耗流量)',
            download_file_name: '下载文件名',
            save: '保存',
            info_format: '{pid}是作品id--------{uid}是画师id\\n{pname}是作品名--------{uname}是画师名\\n注意, 多图情况下, 会自动填充index索引编号\\n目前只支持GIF和多图的重命名',
            description: '说明',
            favorites: '收藏人数',
            illegal: '不合法',
            download: '下载',
            download_multiple: '下载多图',
            download_gif: '下载gif图',
            download_wait: '请等待下载完成',
            download_gif_wait: 'Gif未加载完毕, 请稍等片刻!',
            download_fix: '下载已停止，正在修复!',
            ...
            id_intialization_fail: '搜索UID和PID 初始化失败, form元素获取失败',
            search_enhance_intialization: '搜索增强 初始化',
        },
        'zh-cn': {},
        'zh-tw': {
            enhanced_config: 'Pixiv增强配置',
    ...
    let isMemberPage = () => isMemberIndexPage() || isMemberIllustPage() || isMemberBookmarkPage() || isMemberFriendPage(),
        isSearchPage = () => /.+\/search\.php.*/.test(location.href) || /.+\/tags\/.*\/artworks.*/.test(location.href);
    ...
    (function () {
        observerFactory(function (mutations, observer) {
            for (let i = 0, len = mutations.length; i < len; i++) {
                ...
                console.log(i18n("search_enhance_intialization"));
                ...
                (function ($form) {
                    let initSearch = function (option) {
                        ...
                        if(!options.$form) {
                            console.log(i18n('id_intialization_fail'));
                        }
    ...
    (async function () {
        ...
        observerFactory(function (mutations, observer) {
            for (let i = 0, len = mutations.length; i < len; i++) {
                ...
                $.ajax({
                    ...
                    success: response => {
                        GM.getValue(GMkeys.downloadName, `{pid}`).then(name => {
                            let $a = $(`<a href="${response.body.originalSrc}" download="${getDownloadName(name)}.pixiv"></a>`);
        ...
        observerFactory(function (mutations, observer) {
            for (let i = 0, len = mutations.length; i < len; i++) {
                ...
                console.log(i18n('download_multiple'));
                ...
                let $zipBtn = initDownloadBtn({
                    ...
                    clickFun: function () {
                        ...
                        let timeFrom = Date.now()-lastDownload >= 10000;
                        let hasNotDownloaded = downloaded < num;
                        if($(this).attr('start') !== 'true' || (timeFrom && hasNotDownloaded)) {
                            if (timeFrom && hasNotDownloaded){
                              alert(i18n('download_fix'));
                            }
                            $(this).attr('start', true);
                            $.each(imgUrls, function (index, url) {
                                if (downloadURLs.includes(url) !== true) {
                                  GM.xmlHttpRequest({
                                      method: 'GET', url: url,
                                      headers: {referer: 'https://www.pixiv.net/'},
                                      overrideMimeType: 'text/plain; charset=x-user-defined',
                        ...
                        GM.getValue(GMkeys.downloadName, `{pid}`).then(name => {
                            zip.generateAsync({type: 'blob', base64: true})
                                .then(content => saveAs(content, getDownloadName(name) + ".pixiv"));
                        });
                    }
                });
    ...
    (function () {
        ...
        let $table = $(`<table style="width: 700px;">
            <tbody>
                <tr><th width="185">${i18n('enhanced_config')}</th><td width="500">
                    <label><input type="checkbox" name="${GMkeys.MO}">${i18n('pjax')}</label><br/>
                    <label><input type="checkbox" name="${GMkeys.switchComment}">${i18n('auto_load_comment')}</label><br/>
                    <label><input type="checkbox" name="${GMkeys.switchImgMulti}">${i18n('auto_load_multi')}</label><br/>
                    <label><input type="checkbox" name="${GMkeys.switchImgSize}">${i18n('show_image_size')}</label><br/>
                    <label><input type="checkbox" name="${GMkeys.switchImgPreload}">${i18n('image_preload')}</label><br/>

                    <label>${i18n('download_file_name')}: <input type="text" name="${GMkeys.downloadName}" placeholder="{pid}-{uid}-{pname}-{uname}"></label>
                    <a>${i18n('save')}</a>
                    <a onclick="alert(i18n('info_format'));">${i18n('description')}</a>
                </td></tr>
            </tbody>
        </table>`);
