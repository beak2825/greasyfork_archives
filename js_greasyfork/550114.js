...
// @downloadURL https://update.greasyfork.org/scripts/550114/%E6%9B%B4%E6%96%B9%E4%BE%BF%E5%9C%B0%E7%9C%8BLivebench.user.js
// @updateURL https://update.greasyfork.org/scripts/550114/%E6%9B%B4%E6%96%B9%E4%BE%BF%E5%9C%B0%E7%9C%8BLivebench.meta.js
// ==/UserScript==
...
(function () {
  ...
  const CATEGORY_GROUPS_DEFAULT = {
    ...
    'Reasoning': ['web_of_lies_v3','zebra_puzzle','spatial']
  };
...
(function(){
  ...
  async function onCSV(url, text) {
    ...
    try {
      ...
      let groups = null;
      try {
        const resp = await fetch('https://livebench.ai/categories_2025_05_30.json', { credentials:'omit' });
        if (resp && resp.ok) groups = await resp.json();
      } catch(_) { groups = CATEGORY_GROUPS_DEFAULT; }
      if (!groups) groups = CATEGORY_GROUPS_DEFAULT;
      const html = buildHTML({ csvText: text, url, parsed, groups });
  ...
  function refetch(url){
    if (!url) return;
    if (seen.has(url) || refetching.has(url)) return; // 若已处理或正在拉取，则不重复
    refetching.add(url);
    ...
    fetch(url)
      ...
      .catch(()=>{
        if (typeof GM_xmlhttpRequest === 'function') {
          GM_xmlhttpRequest({ method:'GET', url, onload: r => onCSV(url, r.responseText), onerror: ()=>{} });
        }
      })
      .finally(()=>{ refetching.delete(url); });
  }
  try {
    if ('PerformanceObserver' in window) {
      const po = new PerformanceObserver(list => {
        for (const e of list.getEntries()) {
          const name=e && e.name;
          if (name && CSV_RX.test(name) && !seen.has(name) && !refetching.has(name)) refetch(name);
        }
      });
