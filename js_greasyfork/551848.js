...
!function() {
  ...
  const registry = new class {
    ...
    matchByHost(hostname) {
      for (const module of this.modules) {
        const pattern = module.config.domain || module.config.domainPattern;
        if (pattern && pattern.test(hostname)) return module;
      }
      return null;
    }
  ...
  class BasePreview {
    constructor(siteConfig) {
      if (this.pendingRequests = 0, this.maxConcurrent = 3, this.requestQueue = [], !siteConfig.preview) throw new Error("PreviewConfig is required");
      this.siteConfig = siteConfig, this.config = siteConfig.preview;
    }
    async processListPage() {
      !function() {
        if (document.getElementById("lm-preview-styles")) return;
        const style = document.createElement("style");
        style.id = "lm-preview-styles", style.textContent = "\n/* 预览容器 */\n.lm-preview-container {\n  display: flex;\n  gap: 8px;\n  width: 100%;\n  margin-top: 10px;\n}\n\n/* 图片包装器 */\n.lm-preview-img-wrapper {\n  flex: 1;\n  min-width: 0;\n  height: 180px;\n  background: #f5f5f5;\n  border-radius: 6px;\n  overflow: hidden;\n  cursor: pointer;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n}\n\n.lm-preview-img-wrapper:hover {\n  box-shadow: 0 2px 8px rgba(0,0,0,0.15);\n}\n\n/* 预览图片 */\n.lm-preview-img {\n  width: 100%;\n  height: 100%;\n  object-fit: contain;\n}\n\n/* 加载占位 */\n.lm-preview-placeholder {\n  color: #999;\n  font-size: 12px;\n}\n\n/* 错误状态 */\n.lm-preview-error {\n  color: #f56c6c;\n  font-size: 12px;\n}\n\n/* 无图提示 */\n.lm-preview-empty {\n  width: 100%;\n  padding: 20px;\n  text-align: center;\n  background: #f9f9f9;\n  border-radius: 6px;\n  color: #999;\n  font-size: 13px;\n}\n\n/* 磁力链接 */\n.lm-magnet-block {\n  margin-top: 10px;\n  padding: 10px 12px;\n  background: #f0f9ff;\n  border: 1px solid #e0f2fe;\n  border-radius: 6px;\n  font-size: 13px;\n  word-break: break-all;\n  cursor: pointer;\n}\n\n.lm-magnet-block:hover {\n  background: #e0f2fe;\n}\n", 
        document.head.appendChild(style);
      }();
      const container = document.querySelector(this.config.listContainer);
      if (!container) return;
      const items = container.querySelectorAll(this.config.itemSelector), tasks = Array.from(items).map(item => this.processItem(item));
      await Promise.allSettled(tasks);
    }
    async processItem(item) {
      if (item.dataset.lmProcessed) return;
      item.dataset.lmProcessed = "1";
      const link = item.querySelector(this.config.linkSelector);
      if (!link?.href) return;
      const url = link.href, previewContainer = document.createElement("div");
      previewContainer.className = "lm-preview-container", previewContainer.innerHTML = '<div class="lm-preview-placeholder">加载中...</div>', 
      item.appendChild(previewContainer);
      try {
        const cached = this.getCache(url);
        if (cached) return void this.renderPreview(previewContainer, cached);
        await this.waitForSlot();
        const data = await this.fetchDetail(url);
        this.releaseSlot(), (data.images.length > 0 || data.magnet) && this.setCache(url, data), 
        this.renderPreview(previewContainer, data);
      } catch (error) {
        this.releaseSlot(), previewContainer.innerHTML = '<div class="lm-preview-error">加载失败</div>';
      }
    }
    async fetchDetail(url) {
      ...
      let images;
      images = this.config.extractImages ? this.config.extractImages(doc) : this.defaultExtractImages(doc);
      let magnet = null;
      return magnet = this.config.extractMagnet ? this.config.extractMagnet(doc) : this.defaultExtractMagnet(doc), 
      {
        images: images,
        magnet: magnet
      };
    }
    defaultExtractImages(doc) {
      const maxImages = this.config.maxImages || 4, imgs = doc.querySelectorAll(this.config.imageSelector);
      return Array.from(imgs).slice(0, maxImages).map(img => {
        const el = img;
        return el.getAttribute("zoomfile") || el.getAttribute("data-src") || el.getAttribute("data-original") || el.src;
      }).filter(src => src && !src.includes("none.gif") && !src.includes("icon"));
    }
    defaultExtractMagnet(doc) {
      const magnetLink = doc.querySelector('a[href^="magnet:?xt=urn:btih:"]');
      if (magnetLink) return magnetLink.href;
      const text = doc.body.innerText || "", match = text.match(/magnet:\?xt=urn:btih:[a-zA-Z0-9]{32,40}/i);
      if (match) return match[0];
      const hashMatch = text.match(/\b[a-fA-F0-9]{40}\b/);
      return hashMatch ? `magnet:?xt=urn:btih:${hashMatch[0]}` : null;
    }
    renderPreview(container, data) {
      if (container.innerHTML = "", 0 !== data.images.length || data.magnet) {
        if (data.images.length > 0) {
          const imagesContainer = document.createElement("div");
          imagesContainer.className = "lm-preview-container", data.images.forEach((src, index) => {
            const wrapper = document.createElement("div");
            wrapper.className = "lm-preview-img-wrapper";
            const img = document.createElement("img");
            img.className = "lm-preview-img", img.src = src, img.loading = "lazy", img.onerror = () => {
              wrapper.innerHTML = '<span class="lm-preview-error">加载失败</span>';
            }, wrapper.appendChild(img), wrapper.onclick = () => Lightbox.show(data.images, index), 
            imagesContainer.appendChild(wrapper);
          }), container.appendChild(imagesContainer);
        }
        if (data.magnet) {
          const magnetDiv = document.createElement("div");
          magnetDiv.className = "lm-magnet-block", magnetDiv.textContent = data.magnet, magnetDiv.title = "点击复制", 
          magnetDiv.onclick = () => {
            navigator.clipboard.writeText(data.magnet), magnetDiv.textContent = "✓ 已复制", setTimeout(() => {
              magnetDiv.textContent = data.magnet;
            }, 1500);
          }, container.appendChild(magnetDiv);
        }
      } else container.innerHTML = '<div class="lm-preview-empty">📝 暂无预览</div>';
    }
    waitForSlot() {
      return this.pendingRequests < this.maxConcurrent ? (this.pendingRequests++, Promise.resolve()) : new Promise(resolve => {
    ...
    releaseSlot() {
      ...
      next && setTimeout(next, 200 + 200 * Math.random());
    }
    getCache(url) {
      const key = this.getCacheKey(url), cached = Storage.get(key, null);
      return cached ? Date.now() - cached.timestamp > 864e5 ? (Storage.delete(key), null) : cached.data : null;
    }
    setCache(url, data) {
    ...
    getCacheKey(url) {
      const match = url.match(/(?:tid|id)=(\d+)/), id = match ? match[1] : btoa(url).slice(0, 16);
      return `lm_preview_${this.siteConfig.type}_${id}`;
    }
    static clearCache(siteType) {
      Storage.listKeys().forEach(key => {
        key.startsWith("lm_preview_") && (siteType && !key.includes(`_${siteType}_`) || Storage.delete(key));
      });
    }
  }
  class CheckInBar {
    constructor(config, stats, onSignClick) {
      this.config = config, this.stats = stats, this.onSignClick = onSignClick;
    }
    render() {
      return function() {
        if (document.getElementById("lm-checkin-styles")) return;
        const style = document.createElement("style");
        style.id = "lm-checkin-styles", style.textContent = "\n/* 签到信息栏 - 完整版 */\n.lm-checkin-bar {\n  margin-bottom: 10px;\n  width: 100%;\n  background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);\n  border-radius: 10px;\n  box-shadow: 0 4px 12px rgba(102, 126, 234, 0.3);\n  box-sizing: border-box;\n}\n\n.lm-checkin-bar-inner {\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  padding: 12px 20px;\n  color: #fff;\n  font-size: 13px;\n}\n\n.lm-checkin-stats {\n  display: flex;\n  align-items: center;\n  gap: 20px;\n}\n\n.lm-checkin-stat-item {\n  text-align: center;\n  border-right: 1px solid rgba(255, 255, 255, 0.25);\n  padding-right: 20px;\n}\n\n.lm-checkin-stat-item:last-child {\n  border-right: none;\n  padding-right: 0;\n}\n\n.lm-checkin-stat-label {\n  font-size: 10px;\n  opacity: 0.85;\n  margin-bottom: 2px;\n}\n\n.lm-checkin-stat-value {\n  font-size: 18px;\n  font-weight: 700;\n}\n\n.lm-checkin-actions {\n  display: flex;\n  align-items: center;\n  gap: 12px;\n}\n\n.lm-checkin-status {\n  font-size: 12px;\n  font-weight: 600;\n}\n\n.lm-checkin-status.signed {\n  color: rgba(144, 238, 144, 0.9);\n}\n\n.lm-checkin-status.unsigned {\n  color: rgba(255, 215, 0, 0.9);\n}\n\n.lm-checkin-btn {\n  padding: 6px 18px;\n  border: none;\n  border-radius: 6px;\n  font-size: 13px;\n  font-weight: 600;\n  cursor: pointer;\n  transition: all 0.2s;\n}\n\n.lm-checkin-btn.active {\n  background: rgba(255, 255, 255, 0.95);\n  color: #667eea;\n}\n\n.lm-checkin-btn.active:hover {\n  background: #fff;\n  transform: translateY(-1px);\n}\n\n.lm-checkin-btn.disabled {\n  background: rgba(255, 255, 255, 0.15);\n  color: rgba(255, 255, 255, 0.6);\n  cursor: not-allowed;\n}\n\n/* 签到信息栏 - 精简版 */\n.lm-checkin-bar-compact {\n  display: inline-block;\n  margin: 0 15px;\n  color: #666;\n  font-size: 12px;\n}\n\n/* 响应式 */\n@media (max-width: 768px) {\n  .lm-checkin-bar-inner {\n    flex-direction: column;\n    gap: 12px;\n    padding: 12px 16px;\n  }\n  .lm-checkin-stats {\n    flex-wrap: wrap;\n    gap: 12px;\n  }\n}\n", 
        document.head.appendChild(style);
      }(), "compact" === (this.config.checkIn?.infoBar?.style || "full") ? this.renderCompact() : this.renderFull();
    }
    renderFull() {
      const bar = document.createElement("div");
      bar.className = "lm-checkin-bar", bar.id = "lm-checkin-bar";
      const inner = document.createElement("div");
      inner.className = "lm-checkin-bar-inner";
      const statsDiv = document.createElement("div");
      statsDiv.className = "lm-checkin-stats", statsDiv.appendChild(this.createStatItem("连续", `${this.stats.lxdays}天`)), 
      statsDiv.appendChild(this.createStatItem("等级", `Lv.${this.stats.lxlevel}`)), statsDiv.appendChild(this.createStatItem("总天数", `${this.stats.lxtdays}天`));
      const actionsDiv = document.createElement("div");
      actionsDiv.className = "lm-checkin-actions";
      const statusDiv = document.createElement("div");
      statusDiv.className = "lm-checkin-status " + (this.stats.alreadySigned ? "signed" : "unsigned"), 
      statusDiv.textContent = this.stats.alreadySigned ? "✅ 今日已签" : "⏰ 待签到";
      const btn = document.createElement("button");
      return btn.className = "lm-checkin-btn " + (this.stats.alreadySigned ? "disabled" : "active"), 
      btn.textContent = this.stats.alreadySigned ? "已签到" : "签到", !this.stats.alreadySigned && this.onSignClick && (btn.onclick = () => this.onSignClick?.()), 
      actionsDiv.appendChild(statusDiv), actionsDiv.appendChild(btn), inner.appendChild(statsDiv), 
      inner.appendChild(actionsDiv), bar.appendChild(inner), bar;
    }
    renderCompact() {
      const bar = document.createElement("div");
      return bar.className = "lm-checkin-bar-compact", bar.id = "lm-checkin-bar", bar.innerHTML = `连续${this.stats.lxdays}天 | 总${this.stats.lxtdays}天 | ${this.stats.alreadySigned ? "✓已签" : "待签"}`, 
      bar;
    }
    ...
    insertTo(target, position = "beforebegin") {
      const bar = this.render();
      target.insertAdjacentElement(position, bar);
    }
    update(stats) {
      this.stats = stats;
      const oldBar = document.getElementById("lm-checkin-bar");
      if (oldBar) {
        const newBar = this.render();
  ...
  class BaseCheckIn {
    constructor(config) {
      this.checkInBarInstance = null, this.statsPromise = null, this.initPromise = null, 
      this.config = config, this.cacheKey = config.checkIn?.cacheKey || `LM_CHECKIN_${config.type.toUpperCase()}`;
    }
    init() {
      return this.config.checkIn?.enabled ? (this.initPromise || (this.initPromise = (async () => {
        try {
          const cachedStats = Storage.get(this.cacheKey);
          cachedStats && this.showCheckInBar(cachedStats);
          const stats = await this.getStats();
          if (!stats) return;
          if (Storage.set(this.cacheKey, stats), "auto" === this.config.checkIn?.type && !stats.alreadySigned && stats.signUrl) return void (await this.autoSign(stats.signUrl));
          await this.showCheckInBar(stats);
        } catch (error) {}
      })()), this.initPromise) : Promise.resolve();
    }
    getStats() {
      if (this.statsPromise) return this.statsPromise;
      const statsUrl = this.config.checkIn?.statsUrl;
      return statsUrl ? (this.statsPromise = (async () => {
        try {
          const fullUrl = statsUrl.startsWith("http") ? statsUrl : `${location.origin}/${statsUrl}`, response = await fetch(fullUrl, {
            credentials: "include"
          }), html = await response.text(), doc = (new DOMParser).parseFromString(html, "text/html");
          return this.config.checkIn?.parseStats ? this.config.checkIn.parseStats(doc) : this.defaultParseStats(doc, html);
        } catch (error) {
          return null;
        }
      })(), this.statsPromise) : Promise.resolve(null);
    }
    defaultParseStats(doc, html) {
      ...
      let signUrl;
      if (signBtn) {
        const href = signBtn.getAttribute("href"), onclick = signBtn.getAttribute("onclick");
        if (href && "javascript:;" !== href) signUrl = href.startsWith("http") ? href : `${location.origin}/${href}`; else if (onclick) {
          const urlMatch = onclick.match(/['"]([^'"]+sign[^'"]*)['"]/i);
          urlMatch && (signUrl = urlMatch[1].startsWith("http") ? urlMatch[1] : `${location.origin}/${urlMatch[1]}`);
        }
      }
      return {
    ...
    async showCheckInBar(stats) {
      if (this.checkInBarInstance) return void this.checkInBarInstance.update(stats);
      if (document.getElementById("lm-checkin-bar")) return this.checkInBarInstance = new CheckInBar(this.config, stats, () => this.execute()), 
      void this.checkInBarInstance.update(stats);
      const infoBarConfig = this.config.checkIn?.infoBar;
      if (!infoBarConfig?.target) return;
      const target = await this.waitForElement(infoBarConfig.target, 5e3);
      ...
      this.checkInBarInstance.insertTo(target, infoBarConfig.position || "beforebegin")));
    }
    async execute() {
      if (this.config.checkIn?.execute) return await this.config.checkIn.execute();
      if ("manual" === this.config.checkIn?.type) {
        const statsUrl = this.config.checkIn.statsUrl;
        return statsUrl && (window.location.href = statsUrl.startsWith("http") ? statsUrl : `${location.origin}/${statsUrl}`), 
        !0;
      }
      const stats = await this.getStats();
      return !!stats?.signUrl && await this.autoSign(stats.signUrl);
    }
    async autoSign(signUrl) {
      try {
        ...
        })).ok ? (Toast.success(`${this.config.name} 签到成功`), setTimeout(() => location.reload(), 1e3), 
        !0) : (Toast.error(`${this.config.name} 签到失败`), !1);
      } catch (error) {
        return Toast.error(`${this.config.name} 签到异常`), !1;
      }
    }
    waitForElement(selector, timeout = 5e3) {
      return new Promise(async resolve => {
        document.body || await new Promise(r => {
          const observer2 = new MutationObserver(() => {
            document.body && (observer2.disconnect(), r());
          });
        ...
        const observer = new MutationObserver(() => {
          ...
          el && (observer.disconnect(), resolve(el));
        });
  ...
  function createSiteModule(config, hooks) {
    let previewProcessor2 = null;
    config.preview && (previewProcessor2 = new BasePreview(config));
    let checkInProcessor = null;
    config.checkIn?.enabled && (checkInProcessor = new BaseCheckIn(config));
    const module = {
      type: config.type,
      name: config.name,
      config: config,
      hasCheckIn: !!config.checkIn?.enabled,
      hasPreview: !!config.preview,
      async init() {
        if (config.styles) {
          const style = document.createElement("style");
          style.id = `lm-styles-${config.type}`, style.textContent = config.styles, document.head.appendChild(style);
        }
        if (checkInProcessor) try {
          "home" === registry.detectPageType(module) && checkInProcessor.init().catch(e => {});
        } catch (e) {}
        hooks?.onInit && await hooks.onInit();
      },
      async handlePage(pageType) {
        switch (pageType) {
         case "home":
          checkInProcessor && await checkInProcessor.init(), hooks?.onHome && await hooks.onHome();
          break;

         case "list":
          hooks?.onList && await hooks.onList(), previewProcessor2 && (await previewProcessor2.processListPage(), 
          hooks?.onPreviewDone && hooks.onPreviewDone());
          break;

         case "search":
          hooks?.onSearch && await hooks.onSearch(), previewProcessor2 && await previewProcessor2.processListPage();
          break;

         case "content":
          hooks?.onContent && await hooks.onContent();
        }
      },
      async checkIn() {
        if (checkInProcessor) {
          const success = await checkInProcessor.execute();
          hooks?.onCheckInDone && hooks.onCheckInDone(success);
        }
      },
      destroy() {
        hooks?.onDestroy && hooks.onDestroy();
        const style = document.getElementById(`lm-styles-${config.type}`);
        style && style.remove();
      }
    };
    return module;
  ...
  const laowangConfig = {
    type: "laowang",
    name: "老王论坛",
    domain: /laowang\.vip|laowangwnr3p\.com|y6ptsn8\.com/,
    pages: {
    ...
    preview: {
      listContainer: "#threadlist .bm_c",
      itemSelector: "li",
      linkSelector: 'div.c.cl a[href*="viewthread"]',
      imageSelector: ".pcb img.zoom[zoomfile], .pattl img.zoom[zoomfile]",
      maxImages: 4
    },
    checkIn: {
      enabled: !0,
      type: "manual",
      statsUrl: "plugin.php?id=k_misign:sign",
  ...
  class LaowangPreview {
    constructor() {
      this.pending = 0, this.maxConcurrent = 3, this.queue = [];
    }
    async processListPage() {
      this.injectStyles(), this.convertToListLayout();
      const items = document.querySelectorAll(".lw-item");
      for (const item of items) this.processItem(item);
    }
    async processSearchPage() {
      this.injectStyles();
      const items = document.querySelectorAll('.result-item[class*="uid_hidden_"]');
      for (const item of items) this.processSearchItem(item);
    }
    async processSearchItem(item) {
      if (item.dataset.lwPreview) return;
      item.dataset.lwPreview = "1";
      const link = item.querySelector('.result-title a[href*="viewthread"]');
      if (!link?.href) return;
      const url = link.href, tid = this.getTid(url);
      if (!tid) return;
      const preview = document.createElement("div");
      preview.className = "lw-preview", preview.innerHTML = '<span class="lw-loading">加载预览...</span>';
      const titleEl = item.querySelector(".result-title");
      titleEl ? titleEl.after(preview) : item.appendChild(preview);
      try {
        const cacheKey = `lw_prev_${tid}`, cached = Storage.get(cacheKey, null);
        if (cached && Date.now() - cached.time < 864e5) return void this.render(preview, cached.images);
        await this.acquireSlot();
        const images = await this.fetchImages(url);
        this.releaseSlot(), images.length > 0 && Storage.set(cacheKey, {
          images: images,
          time: Date.now()
        }), this.render(preview, images);
      } catch {
        preview.innerHTML = '<span class="lw-error">加载失败</span>';
      }
    }
    convertToListLayout() {
      const waterfall = document.querySelector("#waterfall");
      if (!waterfall || waterfall.classList.contains("lw-converted")) return;
      waterfall.classList.add("lw-converted");
      const items = Array.from(waterfall.querySelectorAll('li[class*="uid_hidden_"]'));
      waterfall.innerHTML = "", items.forEach(li => {
        const titleLink = li.querySelector("h3.xw0 a"), coverImg = li.querySelector(".c.cl img");
        if (!titleLink) return;
        const item = document.createElement("div");
        item.className = "lw-item", item.dataset.url = titleLink.href;
        const title = document.createElement("div");
        title.className = "lw-title", title.innerHTML = `<a href="${titleLink.href}">${titleLink.textContent}</a>`;
        const preview = document.createElement("div");
        ...
        item.appendChild(title), item.appendChild(preview), waterfall.appendChild(item);
      });
    }
    async processItem(item) {
      ...
      if (!url) return;
      const tid = this.getTid(url);
      if (!tid) return;
      const preview = item.querySelector(".lw-preview");
      if (preview) try {
        const cacheKey = `lw_prev_${tid}`, cached = Storage.get(cacheKey, null);
        if (cached && Date.now() - cached.time < 864e5) return void this.render(preview, cached.images);
        await this.acquireSlot();
    ...
    async fetchImages(url) {
      const res = await fetch(url, {
        credentials: "include"
      }), html = await res.text(), imgs = (new DOMParser).parseFromString(html, "text/html").querySelectorAll(".t_fsz img[zoomfile], .pcb img[zoomfile]"), images = [];
      return imgs.forEach(img => {
        const el = img, src = el.getAttribute("zoomfile") || el.src;
        !src || src.includes("none.gif") || src.includes("icon") || images.includes(src) || images.push(src);
      }), images.slice(0, 4);
    }
    render(container, images) {
      container.innerHTML = "", 0 !== images.length ? images.forEach((src, idx) => {
        const thumb = document.createElement("div");
        thumb.className = "lw-thumb";
        const img = document.createElement("img");
        img.src = src, img.loading = "lazy", img.onclick = e => {
          e.preventDefault(), e.stopPropagation(), Lightbox.show(images, idx);
        }, img.onerror = () => {
          thumb.remove();
        }, thumb.appendChild(img), container.appendChild(thumb);
      }) : container.innerHTML = '<span class="lw-empty">暂无预览</span>';
    }
    getTid(url) {
    ...
    acquireSlot() {
      return this.pending < this.maxConcurrent ? (this.pending++, Promise.resolve()) : new Promise(r => this.queue.push(() => {
        this.pending++, r();
      }));
    }
    releaseSlot() {
    ...
    injectStyles() {
      ...
      const style = document.createElement("style");
      style.id = "lw-css", style.textContent = "\n      /* 转换瀑布流为列表 */\n      #waterfall.lw-converted {\n        display: block !important;\n        height: auto !important;\n        width: 100% !important;\n      }\n\n      /* 帖子项 */\n      .lw-item {\n        padding: 12px 0;\n        border-bottom: 1px solid #eee;\n      }\n\n      /* 标题 */\n      .lw-title a {\n        font-size: 14px;\n        color: #333;\n        text-decoration: none;\n        line-height: 1.5;\n      }\n      .lw-title a:hover { color: #1890ff; }\n\n      /* 预览区 */\n      .lw-preview {\n        display: flex;\n        gap: 8px;\n        margin: 8px 0;\n      }\n\n      /* 缩略图 */\n      .lw-thumb {\n        width: 200px;\n        height: 180px;\n        border-radius: 4px;\n        cursor: pointer;\n        background: #f5f5f5;\n        display: flex;\n        align-items: center;\n        justify-content: center;\n      }\n      .lw-thumb img {\n        max-width: 100%;\n        max-height: 100%;\n        object-fit: contain;\n      }\n\n\n\n      .lw-loading, .lw-empty, .lw-error {\n        font-size: 12px;\n        color: #ccc;\n      }\n      .lw-error { color: #f56c6c; }\n    ", 
      document.head.appendChild(style);
    }
    static clearCache() {
      Storage.listKeys().forEach(k => {
        k.startsWith("lw_prev_") && Storage.delete(k);
      });
    }
  }
  const _PopupWindow = class {
    static open(url, config = {}) {
      this.currentOverlay && this.close(), this.currentConfig = {
        width: 1200,
        height: 800,
        title: "详情预览",
        closeOnOverlay: !0,
      ...
      overlay.className = `${this.currentConfig.className}-overlay`, overlay.style.cssText = "\n      position: fixed;\n      top: 0;\n      left: 0;\n      right: 0;\n      bottom: 0;\n      background: rgba(0, 0, 0, 0.7);\n      z-index: 99999;\n      display: flex;\n      align-items: center;\n      justify-content: center;\n      animation: fadeIn 0.2s ease-in;\n    ";
      const popup = document.createElement("div");
      popup.className = `${this.currentConfig.className}-container`, popup.style.cssText = `\n      width: ${this.currentConfig.width}px;\n      height: ${this.currentConfig.height}px;\n      max-width: 95vw;\n      max-height: 90vh;\n      background: white;\n      border-radius: 8px;\n      box-shadow: 0 10px 40px rgba(0, 0, 0, 0.3);\n      display: flex;\n      flex-direction: column;\n      overflow: hidden;\n      animation: slideIn 0.3s ease-out;\n    `;
      ...
      closeBtn.onmouseover = () => {
        closeBtn.style.background = "#d0d0d0", closeBtn.style.color = "#333";
      }, closeBtn.onmouseout = () => {
        closeBtn.style.background = "#e0e0e0", closeBtn.style.color = "#666";
      }, closeBtn.onclick = () => this.close(), header.appendChild(title), header.appendChild(closeBtn);
      const iframe = document.createElement("iframe");
      if (iframe.src = url, iframe.style.cssText = "\n      flex: 1;\n      border: none;\n      width: 100%;\n      height: 100%;\n    ", 
      this.currentConfig.onLoad && (iframe.onload = () => {
        this.currentConfig?.onLoad && this.currentConfig.onLoad(iframe);
      }), popup.appendChild(header), popup.appendChild(iframe), overlay.appendChild(popup), 
      this.currentConfig.closeOnOverlay && (overlay.onclick = e => {
        e.target === overlay && this.close();
      }), this.currentConfig.closeOnEsc) {
        const handleEsc = e => {
          "Escape" === e.key && (this.close(), document.removeEventListener("keydown", handleEsc));
        };
        document.addEventListener("keydown", handleEsc);
      }
      document.body.appendChild(overlay), this.currentOverlay = overlay, this.injectStyles();
    }
    static close() {
      this.currentOverlay && (this.currentOverlay.remove(), this.currentOverlay = null, 
      this.currentConfig = null);
    }
    static isOpen() {
      return null !== this.currentOverlay;
    }
    static injectStyles() {
      if (document.getElementById("popup-window-styles")) return;
      const style = document.createElement("style");
      style.id = "popup-window-styles", style.textContent = "\n      @keyframes fadeIn {\n        from { opacity: 0; }\n        to { opacity: 1; }\n      }\n      \n      @keyframes slideIn {\n        from {\n          opacity: 0;\n          transform: scale(0.9) translateY(-20px);\n        }\n        to {\n          opacity: 1;\n          transform: scale(1) translateY(0);\n        }\n      }\n    ", 
      document.head.appendChild(style);
    }
  };
  _PopupWindow.currentOverlay = null, _PopupWindow.currentConfig = null;
  let PopupWindow = _PopupWindow;
  const previewProcessor$1 = new LaowangPreview;
  async function showCheckInStatus$1() {
    try {
      const statsUrl = laowangConfig.checkIn?.statsUrl;
      if (!statsUrl) return;
      const fullUrl = statsUrl.startsWith("http") ? statsUrl : `${location.origin}/${statsUrl}`, response = await fetch(fullUrl, {
        credentials: "include"
      }), html = await response.text(), alreadySigned = html.includes("btnvisted") || html.includes("已签到");
      let stats = {
        lxdays: "0",
        lxlevel: "0",
        lxreward: "0",
      ...
      try {
        ...
        stats.lxdays = doc.querySelector("#lxdays")?.value || "0", stats.lxlevel = doc.querySelector("#lxlevel")?.value || "0", 
        stats.lxreward = doc.querySelector("#lxreward")?.value || "0", stats.lxtdays = doc.querySelector("#lxtdays")?.value || "0";
        const fastSignBtn = doc.querySelector("a.J_chkitot");
        if (fastSignBtn) {
          const href = fastSignBtn.getAttribute("href");
          href && (stats.signUrl = href.startsWith("http") ? href : `${location.origin}/${href}`);
        }
      } catch (e) {}
      const target = await function(selector, timeout = 5e3) {
      ...
      if (target && !document.getElementById("lm-checkin-info")) {
        const info = document.createElement("div");
        info.id = "lm-checkin-info", info.style.cssText = "margin-bottom:15px;padding:12px 16px;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;border-radius:8px;font-size:14px;display:flex;align-items:center;justify-content:space-between;box-shadow:0 4px 6px rgba(0,0,0,0.1);";
        const statsContainer = document.createElement("div");
        statsContainer.style.cssText = "display:flex;gap:16px;align-items:center;";
        ...
        if (statusSpan.innerHTML = alreadySigned ? "✅ 今日已签" : "⏰ 待签到", statusSpan.style.cssText = "font-weight:500;font-size:13px;", 
        rightContainer.appendChild(statusSpan), !alreadySigned) {
          const btn = document.createElement("a");
          btn.href = "javascript:void(0)", btn.textContent = "去签到", btn.style.cssText = "color:#fff;background:rgba(255,255,255,0.2);padding:6px 16px;border-radius:4px;text-decoration:none;cursor:pointer;font-weight:bold;transition:background 0.2s;", 
          btn.onclick = e => {
            var url;
            e.preventDefault(), url = stats.signUrl, PopupWindow.open(url, {
              width: 500,
              height: 400,
              title: "🎯 签到验证",
              onLoad: iframe => {
                try {
                  Toast.success("✅ 请完成验证");
                  let isSubmitting = !1, hasAutoClicked = !1;
                  const checkInterval = setInterval(() => {
                    try {
                      ...
                      if (!hasAutoClicked) {
                        const tncode = doc.querySelector(".tncode");
                        if (tncode) {
                          const textSpan = tncode.querySelector(".tncode-text");
                          textSpan && textSpan.textContent?.includes("点击进行安全验证") && (hasAutoClicked = !0, 
                          tncode.click());
                        }
                      }
                      const captchaInput = doc.querySelector("#clicaptcha-submit-info");
                      if (captchaInput && captchaInput.value && captchaInput.value.length > 20) {
                        clearInterval(checkInterval), isSubmitting = !0, Toast.info("✅ 验证完成，正在签到...");
                        const form = doc.querySelector("#v2_captcha_form"), signBtn = doc.querySelector(".J_chkitot");
                        if (form) form.submit(); else if (signBtn) signBtn.click(); else {
                      ...
                      PopupWindow.close(), window.location.reload());
                    } catch (e) {}
                  }, 500);
          ...
          }, btn.onmouseover = () => btn.style.background = "rgba(255,255,255,0.3)", btn.onmouseout = () => btn.style.background = "rgba(255,255,255,0.2)", 
          rightContainer.appendChild(btn);
        }
        info.appendChild(rightContainer);
        const blockContainer = target.closest(".block");
        blockContainer && blockContainer.parentNode ? blockContainer.parentNode.insertBefore(info, blockContainer) : target.parentNode && target.parentNode.insertBefore(info, target);
      }
    } catch (error) {}
  }
  const LaowangModule = {
    type: "laowang",
    name: "老王论坛",
    config: laowangConfig,
    hasCheckIn: !0,
    hasPreview: !0,
    async init() {
      if (laowangConfig.styles) {
        const style = document.createElement("style");
        style.id = "lm-styles-laowang", style.textContent = laowangConfig.styles, document.head.appendChild(style);
      }
      "undefined" != typeof GM_registerMenuCommand && GM_registerMenuCommand("🗑️ 清空图片缓存", () => {
        LaowangPreview.clearCache(), Toast.success("图片缓存已清空"), setTimeout(() => location.reload(), 500);
      }), function() {
        const remove = () => {
          const viewer = document.getElementById("image-viewer");
          viewer && viewer.remove();
        };
        remove(), new MutationObserver(remove).observe(document.body, {
          childList: !0,
          subtree: !1
        });
      }(), document.querySelectorAll('.deanbkms[id^="forum_rules_"]').forEach(el => {
    ...
    async handlePage(pageType) {
      switch (pageType) {
       ...
       case "list":
        ...
        }(), await previewProcessor$1.processListPage();
        break;

       case "search":
        await previewProcessor$1.processSearchPage();
      }
    },
    async checkIn() {
      await showCheckInStatus$1();
  ...
  class MoxingPreview {
    constructor() {
      this.pending = 0, this.maxConcurrent = 3, this.queue = [], this.imageObserver = null, 
      this.imageObserver = new IntersectionObserver(entries => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            const img = entry.target, lazySrc = img.dataset.lazySrc;
            lazySrc && !img.src && (img.src = lazySrc, this.imageObserver?.unobserve(img));
          }
        });
      }, {
    ...
    async processListPage() {
      if (location.search.includes("forumdefstyle=no")) {
        const url = new URL(location.href);
        return url.searchParams.set("forumdefstyle", "yes"), void location.replace(url.href);
      }
      this.injectStyles();
      const getTable = () => document.querySelectorAll('#threadlisttableid tbody[id^="normalthread_"]');
      let tbodies = getTable();
      if (0 === tbodies.length) for (let i = 0; i < 10 && (await new Promise(r => setTimeout(r, 500)), 
      tbodies = getTable(), !(tbodies.length > 0)); i++) ;
      for (const tbody of tbodies) this.processThreadRow(tbody);
    }
    async processThreadRow(tbody) {
      if (tbody.dataset.mxProcessed) return;
      tbody.dataset.mxProcessed = "1";
      const link = tbody.querySelector("a.s.xst") || tbody.querySelector('a[href^="thread-"]') || tbody.querySelector('th div a[href^="thread-"]');
      if (!link?.href) return;
      const url = link.href, tid = this.getTid(url);
      if (!tid) return;
      const previewTr = document.createElement("tr");
      previewTr.className = "mx-preview-row";
      const previewTd = document.createElement("td");
      previewTd.colSpan = 5, previewTd.className = "mx-preview-td";
      const previewContainer = document.createElement("div");
      previewContainer.className = "mx-preview-container", previewContainer.innerHTML = '<span class="mx-loading">加载预览...</span>', 
      previewTd.appendChild(previewContainer), previewTr.appendChild(previewTd), tbody.appendChild(previewTr), 
      this.loadPreview(previewContainer, url, tid);
    }
    async processSearchPage() {
      this.injectStyles();
      const getItems = () => document.querySelectorAll("#threadlist li.pbw.z");
      let items = getItems();
      if (0 === items.length) for (let i = 0; i < 10 && (await new Promise(r => setTimeout(r, 500)), 
      items = getItems(), !(items.length > 0)); i++) ;
      for (const item of items) this.processSearchItem(item);
    }
    async processSearchItem(item) {
      if (item.dataset.mxProcessed) return;
      item.dataset.mxProcessed = "1";
      const link = item.querySelector("h3.xs3 a");
      if (!link?.href) return;
      const url = link.href, tid = this.getTid(url);
      if (!tid) return;
      const previewContainer = document.createElement("div");
      previewContainer.className = "mx-preview-container mx-search-preview", previewContainer.innerHTML = '<span class="mx-loading">加载预览...</span>', 
      item.appendChild(previewContainer), this.loadPreview(previewContainer, url, tid);
    }
    async loadPreview(container, url, tid) {
      try {
        container.setAttribute("data-url", url);
        const cacheKey = `mx_prev_v2_${tid}`, cached = Storage.get(cacheKey, null);
        if (cached && Date.now() - cached.time < 864e5) return void this.render(container, cached.images, cached.magnet);
        await this.acquireSlot();
        try {
          const data = await this.fetchPreviewData(url);
          (data.images.length > 0 || data.magnet) && Storage.set(cacheKey, {
            ...data,
            time: Date.now()
          }), this.render(container, data.images, data.magnet);
        } finally {
          this.releaseSlot();
        }
      } catch (e) {
        container.innerHTML = '<span class="mx-error">加载失败</span>';
      }
    }
    async fetchPreviewData(url) {
      ...
      imgs.forEach(img => {
        const el = img;
        let src = el.getAttribute("zoomfile") || el.getAttribute("file") || el.getAttribute("data-actualsrc") || el.getAttribute("actualsrc") || el.getAttribute("data-src") || el.getAttribute("src");
        if (!src) return;
        if (!src.startsWith("http") && !src.startsWith("//") && !src.startsWith("data:")) try {
          src = new URL(src, baseUrl.href).href;
        } catch (e) {
          return;
        }
        const lower = src.toLowerCase();
        [ "avatar", "usergroup", "smiley", "common/", "icon", "none.gif", "pixel.png", "uc_server", "online_", "rank" ].some(key => lower.includes(key)) || images.includes(src) || images.push(src);
      });
      let magnet = null;
      const magnetLink = doc.querySelector('a[href^="magnet:?xt=urn:btih:"]');
      if (magnetLink) magnet = magnetLink.href; else {
        const text = doc.body.innerText || "", match = text.match(/magnet:\?xt=urn:btih:[a-zA-Z0-9]{32,40}/i);
        if (match) magnet = match[0]; else {
          const hashMatch = text.match(/\b[a-fA-F0-9]{40}\b/);
          hashMatch && (magnet = `magnet:?xt=urn:btih:${hashMatch[0]}`);
        }
      }
      return {
    ...
    render(container, images, magnet) {
      container.innerHTML = "";
      const hasImages = images.length > 0, hasMagnet = !!magnet;
      if (!hasImages && !hasMagnet) {
        container.innerHTML = "";
        const emptyTip = document.createElement("div");
        emptyTip.className = "mx-empty", emptyTip.innerText = "暂无预览 ";
        const retryBtn = document.createElement("a");
        return retryBtn.innerText = "[重试]", retryBtn.style.cursor = "pointer", retryBtn.onclick = e => {
          e.preventDefault(), container.innerHTML = '<span class="mx-loading">重试中...</span>';
          const url = container.getAttribute("data-url");
          url && this.fetchPreviewData(url).then(data => {
            this.render(container, data.images, data.magnet);
          });
        }, emptyTip.appendChild(retryBtn), void container.appendChild(emptyTip);
      }
      const wrapper = document.createElement("div");
      if (wrapper.style.cssText = "display:flex;flex-direction:column;gap:8px;", hasImages) {
        const imgBox = document.createElement("div");
        imgBox.style.cssText = "display:flex;gap:10px;flex-wrap:nowrap;overflow:hidden;", 
        images.forEach((src, idx) => {
          const thumb = document.createElement("div");
          thumb.className = "mx-thumb";
          let fullSrc = src.startsWith("http") ? src : `${location.origin}/${src}`;
          src.startsWith("//") && (fullSrc = "https:" + src);
          const img = document.createElement("img");
          img.dataset.lazySrc = fullSrc, img.referrerPolicy = "origin", img.style.cssText = "display:block;opacity:0;transition:opacity 0.3s;", 
          img.onclick = e => {
            e.preventDefault(), e.stopPropagation(), Lightbox.show(images.map(s => s.startsWith("http") ? s : s.startsWith("//") ? "https:" + s : `${location.origin}/${s}`), idx);
          }, img.onload = () => {
            thumb.style.background = "none", img.style.opacity = "1";
          }, img.onerror = () => {
            thumb.innerHTML = '<span style="color:#999;font-size:11px;">加载失败</span>', thumb.style.background = "#f5f5f5";
          }, thumb.appendChild(img), imgBox.appendChild(thumb), this.imageObserver ? this.imageObserver.observe(img) : img.src = fullSrc;
      ...
      if (hasMagnet) {
        ...
        magBox.title = "点击复制磁力链接", magBox.onclick = e => {
          e.preventDefault(), e.stopPropagation(), navigator.clipboard.writeText(magnet);
          const tip = magBox.querySelector(".mx-copy-tip");
          tip && (tip.textContent = "✅ 已复制"), setTimeout(() => {
            tip && (tip.textContent = "📋");
          }, 1500);
        }, wrapper.appendChild(magBox);
      }
      container.appendChild(wrapper);
    }
    getTid(url) {
      const m = url.match(/(?:thread-|tid=)(\d+)/);
      return m ? m[1] : null;
    }
    acquireSlot() {
      return this.pending < this.maxConcurrent ? (this.pending++, Promise.resolve()) : new Promise(r => this.queue.push(() => {
    ...
    injectStyles() {
      ...
      const style = document.createElement("style");
      style.id = "mx-css", style.textContent = "\n            .mx-preview-row td {\n                border-bottom: 1px solid #f0f0f0;\n                padding: 5px 10px 15px 10px !important;\n            }\n            .mx-preview-container {\n                display: flex;\n                flex-direction: column;\n                gap: 8px;\n            }\n            .mx-search-preview {\n                margin-top: 10px;\n                padding-top: 10px;\n                border-top: 1px dashed #eee;\n            }\n            .mx-thumb {\n                width: 200px;\n                height: 150px;\n                border-radius: 4px;\n                overflow: hidden;\n                background: #f9f9f9;\n                display: flex;\n                align-items: center;\n                justify-content: center;\n                border: 1px solid #eee;\n                cursor: pointer;\n                flex-shrink: 0;\n            }\n            .mx-thumb img {\n                max-width: 100%;\n                max-height: 100%;\n                object-fit: contain;\n                transition: transform 0.2s;\n            }\n            .mx-thumb:hover img {\n                transform: scale(1.05);\n            }\n            .mx-loading, .mx-empty, .mx-error {\n                font-size: 13px;\n                color: #999;\n                padding: 10px;\n            }\n            .mx-copy-tip {\n                color: #28a745;\n            }\n        ", 
      document.head.appendChild(style);
    }
    static clearCache() {
      Storage.listKeys().forEach(k => {
        k.startsWith("mx_prev_v2_") && Storage.delete(k);
      });
    }
  }
  const previewProcessor = new MoxingPreview;
  function createManualButton(container, url) {
    const btn = document.createElement("a");
    btn.textContent = "去签到", btn.style.cssText = "color:#fff;background:rgba(255,255,255,0.2);padding:6px 16px;border-radius:4px;text-decoration:none;cursor:pointer;font-weight:bold;transition:background 0.2s;", 
    btn.onclick = e => {
      e.preventDefault(), PopupWindow.open(url, {
        width: 500,
        height: 400,
        title: "签到"
      });
  ...
  const MoxingModule = {
    ...createSiteModule({
      type: "moxing",
      name: "魔性论坛",
      domain: /moxing\.app|moxing\.vip|moxing\.lol/,
      pages: {
        home: [ /^\/$/, /^\/index\.php$/, /^\/forum\.php$/ ],
      ...
      preview: {
        listContainer: "#threadlisttableid",
        itemSelector: 'tbody[id^="normalthread_"]',
        linkSelector: "a.s.xst",
        imageSelector: ".t_fsz img.zoom, .t_fsz img.inline-image, .t_fsz img.attach-img, .ql-editor img.inline-image, .ql-snow img.attach-img, .pct img.inline-image",
        maxImages: 4
      },
      checkIn: {
        enabled: !1,
        ...
        infoBar: {
          target: 'a[href*="/home.php?mod=task"]',
          position: "beforebegin",
          style: "compact"
        }
      }
    ...
    init: async () => {
      GM_registerMenuCommand("🧹 清除魔性预览缓存", () => {
        MoxingPreview.clearCache(), Toast.success("缓存已清除");
      });
    },
    handlePage: async pageType => {
      switch (pageType) {
       case "home":
        await async function() {
          try {
            const statsUrl = "plugin.php?id=k_misign:sign", fullUrl = `${location.origin}/${statsUrl}`, response = await fetch(fullUrl, {
              credentials: "include"
            }), html = await response.text(), alreadySigned = html.includes("btnvisted") || html.includes("已签到");
            let stats = {
              lxdays: "0",
            ...
            try {
              ...
              stats.lxdays = doc.querySelector("#lxdays")?.value || "0", stats.lxlevel = doc.querySelector("#lxlevel")?.value || "0", 
              stats.lxreward = doc.querySelector("#lxreward")?.value || "0", stats.lxtdays = doc.querySelector("#lxtdays")?.value || "0";
              const fastSignBtn = doc.querySelector('a.J_chkitot, a[href*="operation=qiandao"]');
              if (fastSignBtn) {
                const href = fastSignBtn.getAttribute("href");
                href && (stats.signUrl = href.startsWith("http") ? href : `${location.origin}/${href}`);
              } else {
                const onClickSignBtn = doc.querySelector('a[onclick*="qiandao"]');
                if (onClickSignBtn) {
                  const urlMatch = (onClickSignBtn.getAttribute("onclick") || "").match(/['"]([^'"]+?plugin\.php[^'"]*?operation=qiandao[^'"]*?)['"]/i);
                  urlMatch && (stats.signUrl = urlMatch[1].startsWith("http") ? urlMatch[1] : `${location.origin}/${urlMatch[1]}`);
                }
              }
            } catch (e) {}
            if (stats.signUrl === fullUrl || !stats.signUrl.includes("operation=qiandao")) {
            ...
            let target = await function(selector, timeout = 1e4) {
              return new Promise(resolve => {
                const el = document.querySelector(selector);
                if (el) return resolve(el);
                const observer = new MutationObserver(() => {
                  const el2 = document.querySelector(selector);
                  el2 && (observer.disconnect(), resolve(el2));
                });
                observer.observe(document.body, {
                  childList: !0,
                  subtree: !0
            ...
            }("#portal_block_305", 1e4);
            if (!target) if (target = document.querySelector("#pt"), target) {
              const parent = target.parentNode;
              if (parent) {
                const placeholder = document.createElement("div");
                parent.insertBefore(placeholder, target.nextSibling), target = placeholder;
              }
            } else target = document.querySelector(".wp");
            if (target && !document.getElementById("mx-checkin-info")) {
              const info = document.createElement("div");
              info.id = "mx-checkin-info", info.style.cssText = "margin-bottom:15px;padding:12px 16px;background:linear-gradient(135deg,#c471ed,#f64f59);color:#fff;border-radius:8px;font-size:14px;display:flex;align-items:center;justify-content:space-between;box-shadow:0 4px 6px rgba(0,0,0,0.1);";
              const createStatItem = (label, value, unit = "") => `\n                <div style="text-align:center;">\n                    <div style="font-size:11px;opacity:0.8;margin-bottom:2px;">${label}</div>\n                    <div style="font-size:15px;font-weight:bold;">${value}<span style="font-size:11px;font-weight:normal;margin-left:1px">${unit}</span></div>\n                </div>\n            `;
              info.innerHTML = `\n                <div style="display:flex;gap:16px;align-items:center;">\n                    ${createStatItem("连续签到", stats.lxdays, "天")}\n                    <div style="width:1px;height:24px;background:rgba(255,255,255,0.2);"></div>\n                    ${createStatItem("签到等级", "Lv." + stats.lxlevel)}\n                    <div style="width:1px;height:24px;background:rgba(255,255,255,0.2);"></div>\n                    ${createStatItem("积分奖励", stats.lxreward)}\n                    <div style="width:1px;height:24px;background:rgba(255,255,255,0.2);"></div>\n                    ${createStatItem("总天数", stats.lxtdays, "天")}\n                </div>\n            `;
              const rightDiv = document.createElement("div");
              rightDiv.style.cssText = "display:flex;align-items:center;gap:12px;";
              const statusSpan = document.createElement("span");
              statusSpan.id = "mx-status-text", statusSpan.textContent = alreadySigned ? "✅ 今日已签" : "🚀 自动签到中...", 
              statusSpan.style.fontWeight = "500", rightDiv.appendChild(statusSpan), !alreadySigned && stats.signUrl && stats.signUrl.includes("operation=qiandao") ? fetch(stats.signUrl, {
                credentials: "include",
                headers: {
                  "X-Requested-With": "XMLHttpRequest"
                }
                ...
                text.includes("签到成功") || text.includes("已签到") || text.includes("恭喜") || text.includes("CDATA") ? (statusSpan.textContent = "✅ 签到成功", 
                Toast.success("✅ 魔性论坛自动签到成功"), setTimeout(() => location.reload(), 2e3)) : (statusSpan.textContent = "❌ 自动失败", 
                createManualButton(rightDiv, stats.signUrl));
              }).catch(err => {
                statusSpan.textContent = "❌ 网络错误", createManualButton(rightDiv, stats.signUrl);
              }) : alreadySigned || (statusSpan.textContent = "⏰ 待签到", createManualButton(rightDiv, fullUrl)), 
              info.appendChild(rightDiv), target.parentNode?.insertBefore(info, target);
            }
          } catch (e) {}
        }();
        break;
       ...
       case "list":
        await previewProcessor.processListPage();
        break;

       case "search":
        await previewProcessor.processSearchPage();
      }
    }
  }, SjsModule = createSiteModule({
    type: "sjs",
    name: "司机社",
    domain: /sjs47\.com|xsijishe\.net/,
    pages: {
      home: [ /^\/$/, /^\/index\.php$/, /^\/forum\.php$/ ],
  ...
  function initSjsLightbox() {
    document.querySelectorAll(".nex_thread_pics a").forEach(el => (el => {
      el.dataset.lmLightboxInit || (el.dataset.lmLightboxInit = "true", el.style.cursor = "zoom-in", 
      el.addEventListener("click", e => {
        e.preventDefault(), e.stopPropagation();
        let groupContainer = el.closest(".nex_forumlist_pics");
        groupContainer || (groupContainer = el.closest("tr") || el.closest("li") || el.parentNode);
        const group = groupContainer?.querySelectorAll(".nex_thread_pics a") || [ el ], images = [];
        let currentIndex = 0;
        group.forEach(item => {
          const htmlItem = item;
          let url = "";
          const bgImg = window.getComputedStyle(htmlItem).backgroundImage;
          url = bgImg && "none" !== bgImg ? bgImg.slice(4, -1).replace(/["']/g, "") : htmlItem.querySelector("img")?.src || "", 
          url && (images.push(url), item === el && (currentIndex = images.length - 1));
        }), images.length > 0 && Lightbox.show(images, currentIndex);
      }, !0));
    })(el));
  }
  const GCBT_CONFIG = {
  ...
  const GcbtModule = createSiteModule({
    type: "gcbt",
    name: "GCBT",
    domain: /gcbt\.net/,
    pages: {
    ...
    onList: async () => {
      try {
        DomManager$1.cleanPage(), await PreviewProcessor$1.processAll();
      } finally {
    ...
    onContent: async () => {
      try {
        DomManager$1.cleanPage(), setTimeout(async () => {
    ...
    styles: {
      ...
      previewRow: '\n            /* ===== 广告隐藏 ===== */\n            \n            /* 隐藏顶部广告栏 */\n            .row.ad {\n                display: none !important;\n            }\n            \n            /* 隐藏"视频聊天"列（表格最后一列，但不影响预览行） */\n            table.torrent-list th:last-child,\n            table.torrent-list tr.default td:last-child {\n                display: none !important;\n            }\n            \n            /* 隐藏 mavrtracktor.com 广告链接 */\n            a[href*="mavrtracktor.com"] {\n                display: none !important;\n            }\n            \n            /* ===== 表格布局 ===== */\n            \n            /* 表格布局稳定 */\n            table.torrent-list {\n                table-layout: auto !important;\n                width: 100% !important;\n            }\n            \n            /* ===== 预览行样式 ===== */\n            \n            .u9a9-preview-row {\n                background-color: #f8f9fa !important;\n            }\n            .u9a9-preview-row td {\n                padding: 10px !important;\n                text-align: center !important;\n            }\n            .u9a9-preview-row img {\n                border-radius: 4px;\n                transition: transform 0.2s ease;\n            }\n            .u9a9-preview-row img:hover {\n                transform: scale(1.05);\n            }\n        '
    },
    ...
    static applyStyles() {
      const existingStyle = document.querySelector('style[data-u9a9-style="true"]');
      existingStyle ? this.styleElement = existingStyle : (this.styleElement = document.createElement("style"), 
      this.styleElement.type = "text/css", this.styleElement.setAttribute("data-u9a9-style", "true"), 
      this.styleElement.textContent = U9A9_CONFIG.styles.previewRow, (document.head || document.getElementsByTagName("head")[0]).appendChild(this.styleElement));
    }
    static startStyleMonitor() {
      this.checkInterval || (this.checkInterval = window.setInterval(() => {
        document.querySelector('style[data-u9a9-style="true"]') || (this.styleElement = null, 
        this.applyStyles());
      }, 2e3)), !this.styleObserver && document.head && (this.styleObserver = new MutationObserver(mutations => {
        for (const mutation of mutations) for (const node of mutation.removedNodes) node instanceof HTMLStyleElement && "true" === node.getAttribute("data-u9a9-style") && (this.styleElement = null, 
    ...
    static destroy() {
      ...
      this.styleObserver && (this.styleObserver.disconnect(), this.styleObserver = null), 
      this.styleElement && (this.styleElement.remove(), this.styleElement = null);
    }
  };
  _StyleManager.styleElement = null, _StyleManager.styleObserver = null, _StyleManager.checkInterval = null;
  ...
  const _PreviewProcessor = class {
    ...
    static async processRow(row) {
      ...
      if (cached) this.insertPreviewRow(row, cached); else if (!this.loading.has(pageUrl)) {
        this.loading.add(pageUrl);
        try {
          const images = await this.fetchImages(pageUrl);
          SimpleCache.set(pageUrl, images), this.insertPreviewRow(row, images);
        } catch (error) {} finally {
    ...
    static insertPreviewRow(row, images) {
      const nextRow = row.nextElementSibling;
      if (nextRow && nextRow.classList.contains("u9a9-preview-row")) return void this.updatePreviewRow(nextRow, images);
    ...
    static updatePreviewRow(previewRow, images) {
  ...
  !async function(modules2) {
    try {
      ...
      !function(module, onNavigate) {
        let lastUrl = location.href;
        const handleNavigation = () => {
          const currentUrl = location.href;
          currentUrl !== lastUrl && (lastUrl = currentUrl, setTimeout(onNavigate, 300));
        };
        window.addEventListener("popstate", handleNavigation), window.addEventListener("hashchange", handleNavigation);
        const originalPushState = history.pushState.bind(history), originalReplaceState = history.replaceState.bind(history);
        history.pushState = function(...args) {
          originalPushState(...args), handleNavigation();
        }, history.replaceState = function(...args) {
          originalReplaceState(...args), handleNavigation();
        };
      }(0, handleCurrentPage), await handleCurrentPage();
    } catch (error) {}
  }([ LaowangModule, MoxingModule, SjsModule, GcbtModule, createSiteModule({
    type: "u9a9",
    name: "U9A9",
    domain: /u9a9\.com$/,
    pages: {
    ...
    onInit: async () => {
      StyleManager.init(), DomManager.init();
    },
    onList: async () => {
      await PreviewProcessor.processAll();
    },
    onHome: async () => {
      await PreviewProcessor.processAll();
    },
