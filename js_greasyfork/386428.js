...
// @match			https://duihuan.smzdm.com/p/*
// @grant			GM.xmlHttpRequest
// @icon 			https://www.smzdm.com/favicon.ico
// @downloadURL https://update.greasyfork.org/scripts/386428/%E4%BB%80%E4%B9%88%E5%80%BC%E5%BE%97%E9%BB%91%20%E5%86%85%E6%B5%8B%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/386428/%E4%BB%80%E4%B9%88%E5%80%BC%E5%BE%97%E9%BB%91%20%E5%86%85%E6%B5%8B%E7%89%88.meta.js
// ==/UserScript==
...
.button-a {
	...
	margin-right: 10px;
		...
		for (let item of $('.info-item-left').toArray()) {
			const leftText = $(item)
				.text()
				.trim();
			if (leftText != '开始时间:') {
		...
		const redeem = (pwd, id) => {
			...
			$.post(`https://duihuan.smzdm.com/quan/lingqu/${id}`, {
				safe_pass: pwd,
				client_type: 'PC',
				sourcePage: 'https://duihuan.smzdm.com/lipin/'
			}).done(res => {
		...
		const $button = $(
			'<button type="button" class="button-a">自动抢兑</button>'
		);
		$('.title-20').prepend($button);

		$button.on('click', e => {
			if (!safePwd) {
				safePwd = prompt('输入安全密码 (6位数字)');
			}

			if (safePwd.trim().length === 6) {
				const itemId = $('#js-ensure1').attr('coupon_id');

				console.log('开始抢兑');
				setTimeout(redeem, 0, safePwd, itemId);
			} else {
				safePwd = null;
				alert('密码是6位数字!');
			}
