...
// @grant        GM_addStyle
// @license      GPLv3
// @downloadURL https://update.greasyfork.org/scripts/528579/MouseHunt%20Mouse%20Tracker.user.js
// @updateURL https://update.greasyfork.org/scripts/528579/MouseHunt%20Mouse%20Tracker.meta.js
// ==/UserScript==
...
/*
    ...
    "Glue"
*/
//TODO: add copyMouseDataToClipboard
//TODO: add exportStartStateToFile


//LATEST TODO: check groupMouseData
//TODO: rewrite how initial mouse population is done (based on transposed version of mouseLocationsData, not mouse caught return)
//TODO: fix why back button doesn't work
//TODO: fix styling
(function() {
  'use strict';
  ...
  */
  ...
  const mouseNameCorrections = {
    ...
    // let currentlySavedEnvironments = []; // Store environments data here

    //HOTFIX
    const currentlySavedEnvironments = Array.from(
      new Set(
        Object.values(mouseLocationsData).flat()
      )
    ).map(name => ({
  ...
  GM_addStyle(`
        ...
        #mh-mouse-tracker-focusContainer_v2 {position: fixed; top: 30px; left: 30px; background-color: #f0f0f0; padding: 10px; border: 1px solid #ccc; z-index: 1000; box-shadow: 0 0 5px rgba(0,0,0,0.2);}
  ...
  const fetchEnvironmentsData = async () => {
    ...
    return
      try {
        ...
        currentlySavedEnvironments = await response.json(); // Store fetched environments data
        return currentlySavedEnvironments;
      } catch (error) {
  ...
  async function fetchMouseDataAndUpdateUI(){
      //HOTFIX
      // currentlySavedEnvironments = await fetchEnvironmentsData();
      const initialAllMiceStats = await getHuntingStatsPromise();
      let mouseData = parse_md(initialAllMiceStats);
  ...
  const groupMouseData = (currentMouseData, currentEnvId) => {
    ...
    if (currentEnvId) {
      const currentEnv = currentlySavedEnvironments.find(env => env.id === currentEnvId);
  ...
  function parse_md(allMiceStats){
      let allMiceData = []
      for (const rawMouseName in eventlessMouseLocationsData){
        ...
        const environmentDetails = loc.map(locationName => {
          const env = currentlySavedEnvironments.find(env => env.name === locationName);
          return env ? {
        ...
        loc.forEach((locationName, index) => {
          ...
          allMiceData.push({
            name: rawMouseName,
            catches: currentMouse.num_catches,
            misses: currentMouse.num_misses,
  ...
  function processMiceStats(initialAllMiceStats){
  ...
  const startNewTrackingSession = async () => {
      try {
        ...
        let allInitialMouseData = processMiceStats(initialAllMiceStats);
        let mouseData = parse_md(initialAllMiceStats);

        ts.initialMouseData = JSON.parse(JSON.stringify(allInitialMouseData));
        ts.startTime = Date.now();
        ts.lifetimeHuntsAtStart = calculateTotalHunts(mouseData);
  ...
  */
  ...
  const updateMouseListUI = (currentMouseData) => {
