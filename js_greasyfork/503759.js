// ==UserScript==
// @name         Beautiful logs for technical specialists
...
// @license      Dmiitirev
// @downloadURL https://update.greasyfork.org/scripts/503759/Beautiful%20logs%20for%20technical%20specialists.user.js
// @updateURL https://update.greasyfork.org/scripts/503759/Beautiful%20logs%20for%20technical%20specialists.meta.js
// ==/UserScript==
(function () {
    'use strict';
    ...
    function createStyleButton(element) {
     ...
     function createStyleContainerBg(element) {
        ...
        styleTitleBadge.style.fontSize = '18px';
        ...
        switchCopyBankBlock.innerHTML = `
            ...
            <span class="tooltip-style-text">Копирование описания транзакций всех категорий (кроме Бизнесы/Фракции) без системной информации.<br>Требуется перезагрузка</span><img src='https://cdn-icons-png.flaticon.com/512/25/25429.png' style='width: 15px; height: 15px; filter: invert(1)'></img></div>
            </span>
        `;
        ...
        bugReportTG.href = 'https://t.me/fergusonq'
        ...
        bugReportVK.href = 'https://vk.com/ok_ferguson'
        ...
        bugReportGF.href = 'https://greasyfork.org/ru/scripts/503759-beautiful-logs-for-technical-specialists'
                ...
                transactionCell.addEventListener('copy', function (event) {
                    ...
                    if (selection.length >= transactionCell.textContent.length) {
                        ...
                        if (categoryName === 'BlackPass') {
                            ...
                            if (originalText) {
                                replaceData(`[Глобальный чат | ${transactionDate}] ${playerName} - Написал "$1"`, /^(.*)$/);
                            }
                        ...
                        } else if (categoryName == 'Маркетплейс') {
                            if (originalText.includes('Выставил предмет')) {
                                replaceData(`[Маркетплейс | ${transactionDate}] ${playerName} - Выставил предмет за ${transactionAmount.replace('-', '')} рублей`);
                            } else if (originalText.includes('выставил на продажу')) {
                                replaceData(`[Маркетплейс | ${transactionDate}] ${playerName} - Выставил предмет на продажу $1`, /^.*продажу (.*)\]$/);
                            } else if (originalText.includes('купил')) {
                                replaceData(`[Маркетплейс | ${transactionDate}] ${playerName} - Купил $1 за $2`, /^.*купил (.*) [.*]$/);
                            }
