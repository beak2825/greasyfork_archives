...
// @downloadURL https://update.greasyfork.org/scripts/491224/Whatsapp%E7%BF%BB%E8%AF%91.user.js
// @updateURL https://update.greasyfork.org/scripts/491224/Whatsapp%E7%BF%BB%E8%AF%91.meta.js
// ==/UserScript==
...
(function() {
    ...
    *************************************************************/
    ...
    var lan_select_1 = '<span style="padding-left:20px;padding-right:5px;color:green;font-size:10pt;">输入:</span><select class="languageSelect1" style="padding-right:5px;width: 126px; text-align-last:center;">' + lan_select + '</select>';
    var lan_select_2 = '<span style="padding-left:20px;padding-right:5px;color:green;font-size:10pt;">输出:</span><select class="languageSelect" style="padding-right:5px;width: 126px; text-align-last:center;border-bottom-width: 0px !important;"><option value="off">OFF</option>' + lan_select + '</select>';
    ...
    var translate1 = function(sl,dl,txt,cb){
        debugMessage('Source:'+ txt);
        GM_xmlhttpRequest({
            method: "GET",
            url: "https://translate.googleapis.com/translate_a/single?client=gtx&sl="+ sl + "&tl=" + dl +"&dt=t&q=" + encodeURI(txt),
            onload: function(response) {
                ...
                debugMessage('Translation:'+translate_string);
                cb.append('<hr style="border: 0; padding-top: 1px; background: linear-gradient(to right, transparent, #d0d0d5, transparent);">');
                cb.append('<span class="myTranslateMsg" style="color:#a4a9ab;" lang="en">'+translate_string+'</span>');
                cb.append('<br>');
            }
        });
    };
    ...
    var addTranslateFunc = function(selectChange){
        ...
        if(TRANSLATED_LANGUAGE !== 'off' && $('.tranlate-bottom').length === 0){
            ...
            //    showError('绑定插件失败，请清空输入框后，再次点击聊天框!');
            //}
            function sleep(ms) {
                return new Promise(resolve => setTimeout(resolve, ms || 1000))
            }
            //翻译历史记录
            var messagelist=$('[role="application"]').children();
            var msgnum=messagelist.length;
            $('[role="application"]').children().each(async function(){
               await sleep(200);
               var father_items=$(this).find('.copyable-text').children();
               if(father_items.length==0){
                 return true;
               }
               for(var i=0;i<father_items.length;i++){
                   var msg_item=father_items.eq(i).find('[dir="ltr"]');
                   var msg_str=msg_item.text();
                   if(msg_str!=""){
                       translate1('auto', SOURCE_LANGUAGE, msg_str,father_items.eq(i));
                   }
               }

            })
            $('._3Uu1_').css({"background-color":"#f0f2f5","border":"0px"});
            //监听原本输入框


            $('[role="application"]').on('click', '.selectable-text', function(){
            ...
            //监听元素变化翻译发来的信息
            addNewmsgInterval=setInterval(function(){
                
                var container = document.querySelector('[role="application"]')
                //container.addEventListener('DOMSubtreeModified',async function () {
                    //console.log('测试')
                    //await sleep(400);
                    var newlist=$('[role="application"]').children();
                    if(newlist.length>msgnum){
                        console.log('信息增加');
                        $('[role="application"]').children().each(function(){
                            var father_items=$(this).find('.copyable-text').children();
                            if(father_items.length==0){
                                return true;
                            }
                            for(var i=0;i<father_items.length;i++){
                                var istranslate=father_items.eq(i).find('.myTranslateMsg');
                                if(istranslate.length>0){
                                    console.log('已经翻译,下一个');
                                    return true;
                                }
                                console.log('开始翻译')
                                var msg_item=father_items.eq(i).find('[dir="ltr"]');
    ...
    addListenerInterval = setInterval(function(){
        ...
        if($_div_chat.length){
            ...
            //归档的处理
            $('#pane-side').on('click','button:first-child', function(){
             console.log('按钮点击')
                $('div[role="listitem"]').on('click','div[role="row"]',function(){
                console.log('点击用户')
                })
            })
            //更新会经常导致这个地方需要修改
            $('#pane-side').on('click','div.'+selector, function(){
                ...
                clearInterval(addNewmsgInterval);
                addTranslateFunc();
            });
