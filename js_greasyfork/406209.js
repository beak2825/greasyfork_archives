...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/406209/MCBBS%20%E8%87%AA%E5%AE%9A%E4%B9%89%E8%83%8C%E6%99%AF%20-%20%E6%8A%A2%E5%85%88%E4%BD%93%E9%AA%8C%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/406209/MCBBS%20%E8%87%AA%E5%AE%9A%E4%B9%89%E8%83%8C%E6%99%AF%20-%20%E6%8A%A2%E5%85%88%E4%BD%93%E9%AA%8C%E7%89%88.meta.js
// ==/UserScript==
...
(function() {
    ...
    var version = "y22v1"; // 不要动版本号

    console.log(" %c MCBBS 自定义背景 %c 抢先体验版", "color: #fff; background: #f8981d; padding:5px;", "color:#fff; background: #000; padding:5px;");
    ...
    // 更新提醒
    if(storage.getItem('mbg_trialversion') != version){
        showDialog('<style>.alert_right {background-image: none;padding-right: 0px;padding-left: 0px;}</style><div class=""><b>MCBBS 自定义背景(抢先体验版)已成功更新至 '+ version +'</b><hr><p>此次更新内容：</p> <p ><ol style="margin-left: 25px"><li>已将测试中特性同步至正式版，<a href="https://greasyfork.org/zh-CN/scripts/397762">点击这里前往安装正式版</li></ol></p><h3>请注意：使用抢先体验版需要关闭正式版脚本，体验版内容可能不会成为正式版中的固定性内容。</h3><img src="https://s1.ax1x.com/2020/06/28/NRrPEV.png"/></div>',
                   'right',
                   '<div style="line-height:30px;"><img src="https://s2.ax1x.com/2020/02/25/3twNzq.png" width="20px"> 自定义背景</div>',
                   function() {
            storage["mbg_trialversion"] = version;
        }
                  );
    }
        ...
        document.getElementById('bg_setting').addEventListener('click',function(){
            showDialog('<style>.alert_right {background-image: none;padding-right: 0px;padding-left: 0px;}</style><div class=""><b>点击确定保存 刷新后生效</b><hr><p>自定义背景图 <font color="gray">- 请填写图片链接(多行即启用随机显示)</font></p><textarea id="mbg_url_input" style="width: 98%;" rows="5">' + storage.getItem('mbg_url') + '</textarea><p>背景X偏移量 <font color="gray">- 当前' + mbg_bg_xoffset + '%</font></p><input min="0" max="100" type="range" value="' + mbg_bg_xoffset + '" id="mbg_xoffset_input" style="width: 98%;"><div style="margin-top: -8px;"><font>0%</font><font style="float: right;">100%</font></div><p>透明度 <font color="gray">- 当前' + mbg_opacity + '%</font></p><input min="0" max="100" type="range" value="' + mbg_opacity + '" id="mbg_opacity_input" style="width: 98%;"><div style="margin-top: -8px;"><font>0%</font><font style="float: right;">100%</font></div><p>自定义挂件 <font color="gray">- 请填写图片链接(留空或null即不显示)</font></p><input id="mbg_border_top_input" value="'+ storage.getItem('mbg_border_top') +'" style="width: 98%;" ></div>',
                ...
                storage["mbg_border_top"] = document.getElementById("mbg_border_top_input").value;
                       ...
                       true,
                       {},
                       '<span onclick="showWindow(\'mbg_box\', \'https://www.mcbbs.net/home.php?mod=space&uid=1970274\')" title="点击查看作者"><span style="background: #f8981d; color: #fff; padding: 5px;">MCBBS 自定义背景 </span><span style="background: #000; color: #fff; padding: 5px;"> 抢先体验版</span></span>'
                      );
        })
    }
