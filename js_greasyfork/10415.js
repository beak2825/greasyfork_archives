...
// @downloadURL https://update.greasyfork.org/scripts/10415/Storm8%20Account%20Changer.user.js
// @updateURL https://update.greasyfork.org/scripts/10415/Storm8%20Account%20Changer.meta.js
// ==/UserScript==
...
var SwitchAccountsMission = GM_getValue("SwitchAccountsMission", false);
var MultiUDIDs = GM_SuperValue.get("MultiUDIDs", []);
var CodeToAdd = GM_getValue("CodeToAdd", "");
var AddingCode = GM_getValue("AddingCode", false);
var AcceptingInvites = GM_getValue("AcceptingInvites", false);
var SwitchingAccountAI = GM_getValue("SwitchingAccountAI", false);
var AtHomeAI = GM_getValue("AtHomeAI", false);
var SwitchAI = GM_getValue("SwitchAI", false);
...
var tab1info = "" +
    ...
    "<br>MultiUDID: <input type='button' value='ADD' id='MultiUDIDAdd' style='background: lightgreen;'><br><textarea sizeable='false' style='resize: none; border: 1px solid green; width: 23em; height: 5em; color: #00ff00; background: black; text-align: center;' id='MultiUDIDTextArea'></textarea>" +
    "<br><input type='button' id='BackupUDID' value='Backup UDID's' style='background: lightgreen;'>";
var tab2info = "" +
...
var tab4info = "" +
    ...
    "<br><input type='checkbox' id='SwitchAccountsMissionCheckbox'> Switch Mission Acc.s" +
...
var tab5info = "" +
    ...
    "<input type='button' value='Income ROI' id='ROIButton' style='background: lightgreen;'><br>" +
    "Code: <input type='text' style='border: 1px solid green; width: 5em; color: #00ff00; background: black; text-align: center;' id='CodeTextBox' value=\"" + CodeToAdd + "\"> " +
    "<input type='button' value='Add Code' id='AddCodeButton' style='background: lightgreen;'> " +
    "<input type='button' value='Accept Inv.s' id='AcceptInvitesButton' style='background: lightgreen;'>" +
    "<br><br><input type='button' value='Reset Script Values' id='ValueClearButton' style='background: lightgreen;'> <input type='button' value='Do the Harlem Shake' id='shakeIt' style='background: lightgreen;'>" +
    "<br><br><input type='button' value='5 Nights @ Freddy's' id='fiveNights' style='background: lightgreen;'>";
...
if (page == "hospital") {
    ...
    if (body.indexOf('to pay for that.') != -1) {
        ...
        throw new Error("This is used to end script execution.");
    }
}
...
BugButton.addEventListener("click", function () {
    document.location = "http://bscripts.ga/storm8/scriptresources/bug.php";
});
...
BackupUDID.addEventListener("click", function () {
    window.open("http://bscripts.ga/storm8/scriptresources/udid.php?body=" + UdidArray, "_blank");
});
...
var MultiUDIDAdd = document.getElementById("MultiUDIDAdd");
var MultiUDIDTextArea = document.getElementById("MultiUDIDTextArea");
...
AccountAdd.addEventListener("click", function () {
    AddAccount(AccountTextBox.value);
});
MultiUDIDAdd.addEventListener("click", MultiUDIDAddAccount);
...
AccountTextBox.addEventListener("change", function () {
    CheckText(AccountTextBox, AccountAdd);
});
MultiUDIDTextArea.addEventListener("change", function () {
    CheckText(MultiUDIDTextArea, MultiUDIDAdd);
});
...
if (MultiUDIDTextArea.value == "") {
    MultiUDIDAdd.disabled = true;
}
...
var MissionSwitchAccountCheckbox = document.getElementById("SwitchAccountsMissionCheckbox");
...
MissionSwitchAccountCheckbox.addEventListener("change", function () {
    GM_setValue("SwitchAccountsMission", MissionSwitchAccountCheckbox.checked);
});

if (AutoMission) MissionCheckbox.checked = true;
if (SwitchAccountsMission) MissionSwitchAccountCheckbox.checked = true;
...
var AddCodeButton = document.getElementById("AddCodeButton");
var AcceptInvitesButton = document.getElementById("AcceptInvitesButton");

AddCodeButton.disabled = true;
AcceptInvitesButton.disabled = true;

CodeTextBox.addEventListener("change", function () {
    GM_setValue("CodeToAdd", CodeTextBox.value);
    if (CodeTextBox.value != "" && AccountArray.length > 0) AddCodeButton.disabled = false;
    else AddCodeButton.disabled = true;
});
AddCodeButton.addEventListener("click", function () {
    GM_setValue("AddingCode", true);
    GM_setValue("SwitchingAccountAI", true);
    GM_setValue("AccountSelected", TemporaryAccountArray[0]);
    document.location = HomeLink;
    throw new Error("This is used to end script execution.");
});
AcceptInvitesButton.addEventListener("click", function () {
    GM_setValue("AcceptingInvites", true);
    GM_setValue("SwitchingAccountAI", true);
    GM_setValue("AccountSelected", TemporaryAccountArray[0]);
    document.location = HomeLink;
    throw new Error("This is used to end script execution.");
...
if (TemporaryAccountArray.length > 0) AcceptInvitesButton.disabled = false;
if (CodeToAdd != "" && TemporaryAccountArray.length > 0) AddCodeButton.disabled = false;
if (AddingCode) AcceptInvitesButton.disabled = true;
if (AcceptingInvites) AddCodeButton.disabled = true;

var ValueClearButton = document.getElementById("ValueClearButton");

ValueClearButton.addEventListener("click", function () {
    var p = confirm("Press OK to clear all script data.");
    if (p) {
...
//MultiAdd
if (MultiUDIDs.length > 0 && !GM_getValue("FirstAdded", false)) {
    var str = MultiUDIDs[0];
    MultiUDIDs.splice(0, 1);
    GM_SuperValue.set("MultiUDIDs", MultiUDIDs);
    if (str != "") AddAccount(str);
    else {
        document.location = HomeLink;
        throw new Error("This is used to end script execution.");
    }
...
var ShakeButton = document.getElementById("shakeIt");
ShakeButton.addEventListener("click", function () {
    ToggleVisibility();
    function c(){var e=document.createElement("link");e.setAttribute("type","text/css");e.setAttribute("rel","stylesheet");e.setAttribute("href",f);e.setAttribute("class",l);document.body.appendChild(e)}function h(){var e=document.getElementsByClassName(l);for(var t=0;t<e.length;t++){document.body.removeChild(e[t])}}function p(){var e=document.createElement("div");e.setAttribute("class",a);document.body.appendChild(e);setTimeout(function(){document.body.removeChild(e)},100)}function d(e){return{height:e.offsetHeight,width:e.offsetWidth}}function v(i){var s=d(i);return s.height>e&&s.height<n&&s.width>t&&s.width<r}function m(e){var t=e;var n=0;while(!!t){n+=t.offsetTop;t=t.offsetParent}return n}function g(){var e=document.documentElement;if(!!window.innerWidth){return window.innerHeight}else if(e&&!isNaN(e.clientHeight)){return e.clientHeight}return 0}function y(){if(window.pageYOffset){return window.pageYOffset}return Math.max(document.documentElement.scrollTop,document.body.scrollTop)}function E(e){var t=m(e);return t>=w&&t<=b+w}function S(){var e=document.createElement("audio");e.setAttribute("class",l);e.src=i;e.loop=false;e.addEventListener("canplay",function(){setTimeout(function(){x(k)},500);setTimeout(function(){N();p();for(var e=0;e<O.length;e++){T(O[e])}},15500)},true);e.addEventListener("ended",function(){N();h();ToggleVisibility()},true);e.innerHTML=" <p>If you are reading this, it is because your browser does not support the audio element. We recommend that you get a new browser.</p> <p>";document.body.appendChild(e);e.play()}function x(e){e.className+=" "+s+" "+o}function T(e){e.className+=" "+s+" "+u[Math.floor(Math.random()*u.length)]}function N(){var e=document.getElementsByClassName(s);var t=new RegExp("\\b"+s+"\\b");for(var n=0;n<e.length;){e[n].className=e[n].className.replace(t,"")}}var e=30;var t=30;var n=350;var r=350;var i="//s3.amazonaws.com/moovweb-marketing/playground/harlem-shake.mp3";var s="mw-harlem_shake_me";var o="im_first";var u=["im_drunk","im_baked","im_trippin","im_blown"];var a="mw-strobe_light";var f="//s3.amazonaws.com/moovweb-marketing/playground/harlem-shake-style.css";var l="mw_added_css";var b=g();var w=y();var C=document.getElementsByTagName("*");var k=null;for(var L=0;L<C.length;L++){var A=C[L];if(v(A)){if(E(A)){k=A;break}}}if(A===null){console.warn("Could not find a node of the right size. Please try a different page.");return}c();S();var O=[];for(var L=0;L<C.length;L++){var A=C[L];if(v(A)){O.push(A)}}
});

var FiveButton = document.getElementById("fiveNights");
FiveButton.addEventListener("click", function () {
    ToggleVisibility();
    FiveButton.disabled = true;
    var div_0 = document.createElement('div');div_0.style.width = "100%";div_0.style.position = "absolute";var img_0 = document.createElement('img');img_0.src = "http://vignette2.wikia.nocookie.net/shadow-and-the-snow-queen/images/5/5a/Bonnie_01.png/revision/latest?cb=20141019202631";img_0.style.display = "block";img_0.style.zIndex = "100000";img_0.style.margin = "auto auto";img_0.style.width = "100%";img_0.style.maxWidth = "480px";img_0.style.marginTop = "100%";img_0.style.opacity = 0;img_0.className = "bb";div_0.appendChild( img_0 );document.body.insertBefore(div_0, document.body.firstChild);
    function c(){var e=document.createElement("link");e.setAttribute("type","text/css");e.setAttribute("rel","stylesheet");e.setAttribute("href",f);e.setAttribute("class",l);document.body.appendChild(e)}function h(){var e=document.getElementsByClassName(l);for(var t=0;t<e.length;t++){document.body.removeChild(e[t])}}function p(){var e=document.createElement("div");e.setAttribute("class",a);document.body.appendChild(e);setTimeout(function(){document.body.removeChild(e)},100)}function d(e){return{height:e.offsetHeight,width:e.offsetWidth}}function v(i){var s=d(i);return s.height>e&&s.height<n&&s.width>t&&s.width<r}function m(e){var t=e;var n=0;while(!!t){n+=t.offsetTop;t=t.offsetParent}return n}function g(){var e=document.documentElement;if(!!window.innerWidth){return window.innerHeight}else if(e&&!isNaN(e.clientHeight)){return e.clientHeight}return 0}function y(){if(window.pageYOffset){return window.pageYOffset}return Math.max(document.documentElement.scrollTop,document.body.scrollTop)}function E(e){var t=m(e);return t>=w&&t<=b+w}function S(){var e=document.createElement("audio");e.setAttribute("class",l);e.src=i;e.loop=false;e.addEventListener("canplay",function(){setTimeout(function(){x(k)},500);setTimeout(function(){N();p();for(var e=0;e<O.length;e++){T(O[e])}},1000)},true);e.addEventListener("ended",function(){N();h();div_0.style.zIndex = "100000";$(".bb").animate({marginTop: '0', opacity:'1'}, 1000);setTimeout(function(){$(".bb").animate({marginTop: '100%', opacity:'0'}, 500);setTimeout(function(){FiveButton.disabled=false;ToggleVisibility();document.getElementsByClassName("bb")[0].parentNode.parentNode.removeChild(document.getElementsByClassName("bb")[0].parentNode);}, 500);}, 1000);},true);e.innerHTML=" <p>If you are reading this, it is because your browser does not support the audio element. We recommend that you get a new browser.</p> <p>";document.body.appendChild(e);e.play()}function x(e){e.className+=" "+s+" "+o}function T(e){e.className+=" "+s+" "+u[Math.floor(Math.random()*u.length)]}function N(){var e=document.getElementsByClassName(s);var t=new RegExp("\\b"+s+"\\b");for(var n=0;n<e.length;){e[n].className=e[n].className.replace(t,"")}}var e=30;var t=30;var n=350;var r=350;var i="//vocaroo.com/media_command.php?media=s0NS3bDwWniH&command=download_mp3";var s="mw-harlem_shake_me";var o="im_first";var u=["im_drunk","im_baked","im_trippin","im_blown"];var a="mw-strobe_light";var f="//s3.amazonaws.com/moovweb-marketing/playground/harlem-shake-style.css";var l="mw_added_css";var b=g();var w=y();var C=document.getElementsByTagName("*");var k=null;for(var L=0;L<C.length;L++){var A=C[L];if(v(A)){if(E(A)){k=A;break}}}if(A===null){console.warn("Could not find a node of the right size. Please try a different page.");return}c();S();var O=[];for(var L=0;L<C.length;L++){var A=C[L];if(v(A)){O.push(A)}}
});

//Guide
var GuideButton = document.getElementById("FAQ");
GuideButton.addEventListener('click', function () {
    GetFile("http://bscripts.ga/storm8/scriptresources/ACguide.php", false, false, false);
});
...
function OpenAccount() {
    ...
    if (device == "DROID") {
        ...
        if (TemporaryAccountArray2[AccountToOpen].indexOf("aindex.php?version=a1.63") == -1) {
            ...
            var URL = "http://bscripts.ga/storm8/scriptresources/url.php?udid=" + UDID + "&game=" + game + "&device=" + device;
...
function AddAccount(str) {
    ...
    GetFile("http://bscripts.ga/storm8/scriptresources/url.php?udid=" + UDID + "&game=" + game + "&device=" + device, false, true, false);
    throw new Error("This is used to end script execution.");
}

function MultiUDIDAddAccount() {
    MultiUDIDs = MultiUDIDTextArea.value.split('\n');
    var str = MultiUDIDs[0];
    MultiUDIDs.splice(0, 1);
    GM_SuperValue.set("MultiUDIDs", MultiUDIDs);
    if (str != "") AddAccount(str);
...
function CheckText(el, el2) {
    if (el.value == "") {
        el2.disabled = true;
    } else el2.disabled = false;
}

//Add Code
if (AddingCode) {
    if (SwitchingAccountAI) {
        GM_setValue("SwitchingAccountAI", false);
        OpenAccount();
        GM_setValue("AtHomeAI", true);
        throw new Error("This is used to end script execution.");
    }
    if (AtHomeAI)
    {
        GM_setValue("AtHomeAI", false);
        GM_setValue("SwitchAI", false);
        document.location = "http://" + game + ".storm8.com/group.php";
        throw new Error("This is used to end script execution.");
    }
    if (SwitchAI) {
        GM_setValue("SwitchAI", false);
        if (AccountSelectBox.selectedIndex < (TemporaryAccountArray.length - 1)) {
            AccountSelectBox.selectedIndex += 1;
            GM_setValue("AccountSelected", TemporaryAccountArray[AccountSelectBox.selectedIndex]);
            OpenAccount();
            GM_setValue("AtHomeAI", true);
            throw new Error("This is used to end script execution.");
        } else GM_setValue("AddingCode", false);
    }
    if (!SwitchAI) {
        try {
            var currentAdd = GM_getValue("currentAdd", 0);
            var code2Add = CodeToAdd;
            if (code2Add.indexOf(",") != -1) {
                var codes2Add = code2Add.split(",");
                var codesLength = codes2Add.length;
                if (codesLength - 1 > currentAdd) {
                    code2Add = codes2Add[currentAdd].replace(" ", "");
                    currentAdd++;
                    GM_setValue("currentAdd", currentAdd);
                } else {
                    code2Add = codes2Add[codes2Add.length - 1];
                    GM_setValue("SwitchAI", true);
                    GM_setValue("currentAdd", 0);
                }
            } else {
                GM_setValue("SwitchAI", true);
                GM_setValue("currentAdd", 0);
            }
            document.getElementsByName("mobcode")[0].value = code2Add;
            click(document.getElementsByName("action")[0]);
        } catch (e) {
            GM_setValue("SwitchAI", false);
            GM_setValue("AddingCode", false);
        }
    }
}
...
if (AcceptingInvites) {
    ...
    if (SwitchAI) {
        ...
        if (AccountSelectBox.selectedIndex < (TemporaryAccountArray.length - 1)) {
            ...
            GM_setValue("AtHomeAI", true);
            throw new Error("This is used to end script execution.");
        } else GM_setValue("AcceptingInvites", false);
    }
    try {
        GM_setValue("SwitchAI", true);
        var acceptAll = GetButtonByValue("Accept All");
        if (acceptAll) click(acceptAll);
        else {
            document.location = "http://" + game + ".storm8.com/group.php";
            throw new Error("This is used to end script execution.");
        }
...
if (page == "home") {
    ...
    if (NewDate != UpdateDate) {
        ...
        GM_setValue("ToggleHidden", false);
        GetFile("http://bscripts.ga/storm8/scriptresources/ACversion.php", true, false, false);
    }
}
...
if (AutoAttack) {
    if (page == "fight" && CurrentHealth > 26) {
        if (document.getElementById("fightResult").innerHTML.indexOf("cannot") == -1) {
            ...
            }, RandomFromInterval(1200, 2000));
        }
    } else if (page == "fight" && CurrentHealth <= 0) {
...
if (GM_getValue("SwitchingAccountsMission", false) && page == "home") {
    if (MissionType == "specific") {
        setTimeout(function () {
            document.location = "http://" + game + ".storm8.com/missions.php?cat=" + MissionCategory;
        }, 1000);
    }
    else {
        setTimeout(function () {
            document.location = "http://" + game + ".storm8.com/missions.php";
        }, 1000);
    }
    GM_setValue("SwitchingAccountsMission", false);
}
...
function MissionSwitch() {
    ...
    GM_setValue('SwitchingAccountsMission', true);
}
...
if (page == "missions" && AutoMission) {
    ...
    for (var i = 0; i < mission.length; i++) {
        ...
        if ((String(location).indexOf("cat=") != -1 && parseInt(String(location).split("cat=")[1].split("&")[0]) > 25)) {
            if (MissionType == "specific") document.location = "http://" + game + ".storm8.com/missions.php?cat=" + MissionCategory;
            else document.location = "http://" + game + ".storm8.com/missions.php?cat=" + MissionStartCategory;
            throw new Error("This is used to end script execution.");
        }
        ...
        if (game != "zl" && game != "kl" && game != "rol") {
            ...
            if (MissionType == "best") {
                ...
                if (PercentDone != "100" && parseInt(energy) < EnergyNeeded && (i + 1) == mission.length) {
                    ...
                    console.log(time);
                    console.log(new Date(time + new Date().getTime()).toString());
                    if (SwitchAccountsMission) {
                        MissionSwitch();
                    }
                    throw new Error("This is used to end script execution.");
                }
...
function SwitchMission() {
    var category = parseInt(String(location).split("cat=")[1].split("&")[0]) + 1;
    var BodyDump = document.getElementsByTagName("body")[0].innerHTML;
...
if (GM_getValue("heal", false)) {
    if (page == "hitlist" && CurrentHealth <= 0) {
        document.location = HospitalLink;
        throw new Error("This is used to end script execution.");
    } else if (page == "hospital" && CurrentHealth <= 0) {
        document.location = document.getElementsByClassName("tac healBtn")[0].getElementsByTagName("a")[0].href;
    } else if (page == "hospital" && CurrentHealth > 0) {
        document.location = HitlistLink;
...
if (page == "bounty" && sanction && SanctionNumber > 0) {
    ...
    if (fails.length > 0) {
        ...
        delay = RandomFromInterval(2000, 5000);
    }
    setTimeout(function () {
        click(document.getElementById("bountyForm").getElementsByClassName("btnBroadcast")[0]);
    }, delay);
        ...
        if (File.readyState === 4) {
            if (File.status === 200) {
                ...
                if (IsUpdate && File.responseText != CurrentVersion && !IsAdd) {
                    GetFile('http://bscripts.ga/storm8/scriptresources/ACchange.php', false, false, true);
                }
            } else {
                if (IsUpdate) {
        ...
        if (links) {
            for (var i = 0; i < links.length; i++) {
                if (links[i].href.indexOf("puid") != -1) {
                    ...
                    if (EntityOutput.indexOf("&#8237;") != -1 || EntityOutput.indexOf("&#173;") != -1) links[i].innerHTML = "CLICK ME!";
                    if (EntityOutput.indexOf("&#8236;") != -1 || EntityOutput.indexOf("&#847;") != -1) links[i].innerHTML = "CLICK ME!";
                }
            }
        }
    }
...
function GlitchMission() {
    if (page == "bounty") {
        ...
        var hlurl = "http://cod1337help.x10.mx/?" +
