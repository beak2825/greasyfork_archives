...
// @match       		*://*.deviantart.com/modal/badge/*
// @author				http://dediggefedde.deviantart.com ,http://nuckchorris0.deviantart.com
// @version				1.05
// @grant				unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/5761/AMALL.user.js
// @updateURL https://update.greasyfork.org/scripts/5761/AMALL.meta.js
// ==/UserScript==
(function(){
try {
	...
	if (unsafeWindow.location.href.indexOf('notifications') !== -1) {
		...
		contentEval(()=>{
			...
			DWait.ready(["jms/pages/messagebox.js"],()=>{
				...
				var Llamacheckall=function(){
					$(".amall-give:not([pruf])").each(function(){
						...
						DiFi.pushPost("User","getGiveMenu",[u],(success, data)=>{
					...
					DiFi.pushPost("User","getGiveMenu",[u],(success, data)=>{
						if (success) {
							if (data.response.content.html.indexOf("Give a <span>Llama Badge")===-1){
								...
								'https://www.deviantart.com/modal/badge/give?badgetype=llama&to_user='+userNum+'&referrer=http://my.deviantart.com&amall=1').css({'height': 0, 'width': 0});
                 var retfunc=function(){
							    s.addClass('amall-success').removeClass('amall-working');
                  $("span.amall-give[pruf='"+s.attr("pruf")+"']").addClass('amall-already').removeClass('amall-working');
				  ...
				  setTimeout(()=>{
                    DiFi.send();
                  },1000); //time for server to register llama.
									//$(this).remove();
								};
                el.get(0).onload=retfunc;
                el.appendTo("body");
							}else{
								s.addClass('amall-error').removeClass('amall-working');
							}
