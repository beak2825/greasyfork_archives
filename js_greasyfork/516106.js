...
// @author      imxitiz
// @license     GNU GPLv3
// @description Nov 06 2024, 11:00:34 PM
// @downloadURL https://update.greasyfork.org/scripts/516106/Highlight%20diff%20in%20patches.user.js
// @updateURL https://update.greasyfork.org/scripts/516106/Highlight%20diff%20in%20patches.meta.js
// ==/UserScript==
...
(function() {
    ...
    function highlightDiff() {
        ...
        content = content.replace(/^(?!\+\+\+|\-\-\-)\+(.*)$/gm, (match, p1) => `<span class="addition">${p1}</span>`);

        // Highlight deletions, ignoring lines that start with --- or +++, and remove the "-"
        content = content.replace(/^(?!\+\+\+|\-\-\-)\-(.*)$/gm, (match, p1) => `<span class="deletion">${p1}</span>`);

        // Highlight file change indicators (--- and +++)
        content = content.replace(/^(--- .+|^\+\+\+ .+)$/gm, (match) => `<span class="file-change">${match}</span>`);

        // Highlight line number changes (starting with @@)
        content = content.replace(/^@@ .+$/gm, (match) => `<span class="line-change">${match}</span>`);
