// ==UserScript==
// @name         AUTOBlocks all steam invites EB with Dota
// @include      *steamcommunity.com/*/home/invites*
// @version      0.152
// @description  Blocks all steam users much like the ignore all steam invites
// @author       Andrew Parkes & Enrico Baravelli
// @namespace    https://greasyfork.org/en/scripts/11272-autoblocks-all-steam-invites-eb-with-dota
// @download     https://greasyfork.org/en/scripts/11272-autoblocks-all-steam-invites-eb-with-dota
// @supportURL   https://github.com/AndrewParkes/BlockAllSteamInvites/issues
// @downloadURL https://update.greasyfork.org/scripts/11272/AUTOBlocks%20all%20steam%20invites%20EB%20with%20Dota.user.js
// @updateURL https://update.greasyfork.org/scripts/11272/AUTOBlocks%20all%20steam%20invites%20EB%20with%20Dota.meta.js
// ==/UserScript==
...
if (typeof(element) != 'undefined' && element !== null)
{
    ...
    document.getElementById('pinvites_ignoreall').innerHTML =document.getElementById('pinvites_ignoreall').innerHTML + '<a id="Block_no_dota" class="btn_small btnv6_blue_hoverfade"><span>Block No Dota</span></a>';
    ...
    var blockNoDota = document.getElementById('Block_no_dota');
    if (blockNoDota) 
    {
        blockNoDota.addEventListener ("click", blockNoDotaAcc , 0);
        blockNoDota.addEventListener ("dblclick", blockNoDotaCheck , 0);
    
        if(getCookie("blockNoDotaBox")==1)
        {
           BlockNoDotaBox=1;
            colorBlue("Block_no_dota");
            setCookie( "blockNoDotaBox", BlockNoDotaBox, 24 * 3600 );
        }
    }
...
function blockNoDotaCheck()
{
    ...
    else if(blockNoDotaBox===1)
    {
        blockNoDotaBox=0;
        uncolorBlue("Block_no_dota");
    }
    setCookie( "blockNoDotaBox", blockNoDotaBox, 36 * 3600 );
}
    ...
    for (var i in elems) 
    {
        ...
        //finds the functions that contain block
            if((elems[i]+"").indexOf("block") !=-1)
            {
                ...
                comments=comments.substring(comments.indexOf('<div class="commentthread_comment_text">')-1);

                var goodComments=(comments.split("+REP").length - 1);
                var badComments=(comments.split("-REP").length - 1)+(comments.split("SCAM").length - 1)+(comments.split("SCAMMER").length - 1)+(comments.split("SPAM").length - 1)+(comments.split("PHISH").length - 1);
                if(badComments-goodComments>=1)
                {
                    FriendAccept(userAccount , 'block');
                }
            }
...
function blockNoDotaAcc(zEvent) 
{
    var elems = document.getElementsByClassName('linkStandard');
    for (var i in elems) 
    {
        ...
        //finds the functions that contain block
            if((elems[i]+"").indexOf("block") !=-1)
            {
                //aquires the users steam id
                var userAccount=((elems[i]+"").substr(26)).substr(0,((elems[i]+"").substr(26)).indexOf(",")-1);
                var url ='http://steamcommunity.com/profiles/' +userAccount+ '/games/?tab=recent';
                var gameName=(httpGet(url)+ "").toUpperCase();

                gameName=gameName.substring(gameName.indexOf('<div class="gameListRowItemName">')-1);

                var Dota=(gameName.split("DOTA").length - 1);
                if(Dota==0)
                {
                    FriendAccept(userAccount , 'block');
                }
