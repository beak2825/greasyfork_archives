// ==UserScript==
// @name        TinyChatScript 03_06_2024
// @version     1.8.58
...
// @exclude     https://tinychat.com/start*
...
// @namespace https://greasyfork.org/users/1392618
// @downloadURL https://update.greasyfork.org/scripts/516292/TinyChatScript%2003_06_2024.user.js
// @updateURL https://update.greasyfork.org/scripts/516292/TinyChatScript%2003_06_2024.meta.js
// ==/UserScript==
...
(function() {
    ...
    function CTSInit() {
        ...
        CTS.ScriptLoading = setInterval(function() {
            ...
            if (document.querySelector("#welcome-wrapper")) {
                document.getElementById('paltalk-bar').style.display = 'none';
    ...
    function CTSRoomLoad() {
        ...
        VideoListElement.querySelector("#Bye").addEventListener("click", function() {
            CommandList.bye();
        }, {
            passive: true
        });
        VideoListElement.querySelector("#Undo").addEventListener("click", function() {
            CommandList.undo();
        }, {
            passive: true
        });
        VideoListElement.querySelector("#Bold").addEventListener("click", function() {
            CommandList.bold();
        }, {
            passive: true
        });
    ...
    const botCommandCheck = (...args) => {
        ...
        if (user && user.canGame) {
            if (isCommand(command)) {
                ...
                } else if ((/^!advice$|^!botcheck$|^!8ball |^!chuck$|^!coin$|^!dad$|^!dark$|^!urb |^!vote |^!whoisbot$/i).test(command)) {
    ...
    function botCommandCheckPUB(user, command) {
        ...
        if (CTS.PublicCommandToggle) {
            ...
            if (/^!8ball [\w\s]*\??/i.test(command)) {
                ...
                if (/^!advice$/i.test(command)) {
                    Advice(user.username);
                } else if (/^!chuck$/i.test(command)) {
                    Chuck(user.username);
    ...
    // Dark Joke API (https://v2.jokeapi.dev/joke/Dark)
    async function Dark() {
        // OPEN REQUEST
        if (isSafeListed(arguments[0])) {
            try {
    ...
    // Urban Dictionary API (https://api.Urb.com/)
    async function Urb() {
        // CHECK TERM
            ...
            if (urban !== null) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        console.error('Failed to fetch data from Urban Dictionary API');
                    }
                } catch (error) {
    ...
    function LineSpam() {
        ...
        if ((LineBreaks >= 10 || /\n\n\n\n/g.test(arguments[0]) || phoneNumberRegex.test(arguments[0]) || arguments[0].length > 300) && arguments[1] === false) {
            if (CTS.Host !== CTS.Me.handle) {
                Alert(GetActiveChat(), `Nick: ${arguments[2].nick}\nUser: ${arguments[2].username}\nSPAM message hidden!`);
            }
            return true;
        }
        return false;
    ...
    function GamePrevention() {
        if (!CTS.CanSeeGames && arguments[1] && arguments[0].match(/^\[(8BALL|ADVICE|COIN FLIP|CHUCK NORRIS|DAD JOKE|DARK|FISH|FISHING BOAT|FISHING BOAT HIGH SCORE|FISHING HIGH SCORE|TRIVIA|TRIVIA RANK|URBAN DICTIONARY|WORDLE)\]/)) return false;
        return true;
    }
    ...
    function replaceCharacters(message) {
        ...
        return message.split('').map(char => charMapping[char] || char).join('');
    }
    ...
    function Send() {
        if (CTS.BoldBotMsg) {
            let boldMsg = replaceCharacters(arguments[1]);
            boldMsg = boldMsg.replace(/!𝘄𝗵𝗼𝗶𝘀𝗯𝗼𝘁/g, "!whoisbot");
            boldMsg = boldMsg.replace(/!𝗯𝗼𝘁/g, "!bot");
            ServerSendList[arguments[0]](arguments[0], boldMsg, arguments[2]);
        } else {
    ...
    const CommandList = {
        ...
        reminderadd() {
            if (arguments[0] === "") {
                ...
                if (reminder === null) {
                    Alert(GetActiveChat(), "X Command Rejected!\n!reminderadd 04:18 PM This is an example you can try!");
                } else {
        ...
        bold() {
            CTS.BoldBotMsg = !CTS.BoldBotMsg;
            Save("BoldBotMsg", JSON.stringify(CTS.BoldBotMsg));
            Alert(GetActiveChat(), "Command Accepted!\n" + ((CTS.BoldBotMsg) ? "BOLD MSGS NOW ON!" : "BOLD MSGS NOW OFF!"));
        },
        clr() {
            CTS.Message[GetActiveChat()] = [];
    ...
    var ServerInList = {
        ...
        msg() {
            if (CTS.ScriptInit) {
                let user = HandleToUser(arguments[0].handle);
                if (user != -1) {
                    if (!LineSpam(arguments[0].text, CTS.UserList[user].mod, CTS.UserList[user])) {
                        if (GamePrevention(arguments[0].text, CTS.UserList[user].mod)) {
                            ...
                            if (!CheckUserIgnore(user) && !CheckUserTempIgnore(user) && IgnoreText(text)) {
                                ...
                                if (CTS.Me.handle !== arguments[0].handle) {
                                    ...
                                    if (window.TinychatApp.BLL.SettingsFeature.prototype.getSettings().enableSound && !window.CTSMuted) {
                                        if (CTS.UserList.length <= 4) { // default 14
    ...
    const FishList = {
        ...
        fishspin(player, lines) {
            if (player !== -1 && fishTimerCheck(player)) {
                // Set default wager
                let defaultWager = Fish.PriceList(player, 6);
                ...
                if (player.Points >= wagerPoints) {
                    ...
                    resultMsg += `BET: ${lines} x $${defaultWager}\n`;
    ...
    function SetLocalValues() {
        if (CTS.StorageSupport) {
            ...
            CTS.BoldBotMsg = JSON.parse(Load("BoldBotMsg", JSON.stringify(false)));
