// ==UserScript==
// @name         TMVN League Result(CN beta)
// @version      4.2023051303
...
// @downloadURL https://update.greasyfork.org/scripts/426391/TMVN%20League%20Result%28CN%20beta%29.user.js
// @updateURL https://update.greasyfork.org/scripts/426391/TMVN%20League%20Result%28CN%20beta%29.meta.js
// ==/UserScript==
		...
		matchIds.forEach(function (matchId) {
			...
			xhr.onreadystatechange = function () {
				if (this.readyState == 4 && this.status == 200) {
					...
					if (attendance !== "") {
						attendance = '<span style="color:Orange;">上座率: </span>' + attendance.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
					}
					possession = '<span style="color:Orange;">控球率: </span>' + matchData.possession.home + " - " + matchData.possession.away;
					...
					var rank = '<span style="color:Orange;">排名: </span>' + homeRank + " - " + awayRank;
					...
					if (goalsReport.length == 0) {
						score = '<span style="color:Orange;">比分: </span><a href="/matches/' + matchId + '">0 - 0</a>';
					} else {
						var homeGoal = 0,
						awayGoal = 0;
						...
						score = '<span style="color:Orange;">比分: </span><a href="/matches/' + matchId + '">' + homeGoal + " - " + awayGoal + '</a>';
						scorer += "</tbody></table>";
					}
					...
					if (goalsReport.length > 0) {
						replaceContent += '<tr><td><span style="color:Orange;">进球者: </span></td></tr>';
