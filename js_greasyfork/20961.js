// ==UserScript==
// @name            Morethan.tv Snatched
// @namespace       Morethan.tv
// @description     Mark snatched torrents.
// @author          Mordred & Hentai
// @include 	    https://*morethan.tv/*
...
// @version         1.4.1
// @date            2016-05-14
// @downloadURL https://update.greasyfork.org/scripts/20961/Morethantv%20Snatched.user.js
// @updateURL https://update.greasyfork.org/scripts/20961/Morethantv%20Snatched.meta.js
// ==/UserScript==

// Modified from https://greasyfork.org/scripts/10274-what-cd-snatched Thanks for Mordred!
...
(function () {
	...
	GM_addStyle('.mtvs_snatched { ' + style_tsnatched + ' }');
	GM_addStyle('.mtvs_uploaded { ' + style_uploaded + ' }');
	GM_addStyle('.mtvs_leeching { ' + style_leeching + ' }');
	GM_addStyle('.mtvs_seeding { ' + style_seeding + ' }');
	GM_addStyle('.mtvs_bookmark { ' + style_bookmarked + ' }');
	GM_addStyle(HEADER_STYLE);
	// GM_addStyle(".mtvs_menu { background-color: rgba(40,40,40,0.96); position: fixed; z-index: 500; font-family: Arial, sans-serif; font-size: 11px !important; }")
	// GM_addStyle(".mtvs_button { cursor: pointer; margin-left:12px; float:right; -webkit-box-shadow:inset 0px 1px 0px 0px #ffffff; box-shadow:inset 0px 1px 0px 0px #ffffff; background:-webkit-gradient( linear, left top, left bottom, color-stop(0.05, #343536), color-stop(1, #a3a3a3) ); background:-moz-linear-gradient( center top, #343536 5%, #a3a3a3 100% ); filter:progid:DXImageTransform.Microsoft.gradient(startColorstr='#343536', endColorstr='#a3a3a3'); background-color:#343536; -webkit-border-radius:6px; border-radius:6px; border:1px solid #a1a1a1; display:inline-block; color:#a8a8a8; font-family:arial; font-size:15px; font-weight:bold; padding:6px 24px; text-decoration:none; text-shadow:1px 1px 0px #575757; }");
	// GM_addStyle(".mtvs_button:hover { background:-webkit-gradient( linear, left top, left bottom, color-stop(0.05, #a3a3a3), color-stop(1, #343536) ); background:-moz-linear-gradient( center top, #a3a3a3 5%, #343536 100% ); filter:progid:DXImageTransform.Microsoft.gradient(startColorstr='#a3a3a3', endColorstr='#343536'); background-color:#a3a3a3; }");
	// GM_addStyle(".mtvs_button:active { position:relative; top:1px; }");
	// GM_addStyle(".mtvs_sm_button { cursor: pointer; margin-right:5px; -webkit-box-shadow:inset 0px 1px 0px 0px #ffffff; box-shadow:inset 0px 1px 0px 0px #ffffff; background:-webkit-gradient( linear, left top, left bottom, color-stop(0.05, #343536), color-stop(1, #a3a3a3) ); background:-moz-linear-gradient( center top, #343536 5%, #a3a3a3 100% ); filter:progid:DXImageTransform.Microsoft.gradient(startColorstr='#343536', endColorstr='#a3a3a3'); background-color:#343536; -webkit-border-radius:3px; border-radius:3px; border:1px solid #a1a1a1; display:inline-block; color:#a8a8a8; font-family:arial; font-size: 10px; font-weight:bold; padding:3px 6px; text-decoration:none; text-shadow:1px 1px 0px #575757; }");
	// GM_addStyle(".mtvs_sm_button:hover { background:-webkit-gradient( linear, left top, left bottom, color-stop(0.05, #a3a3a3), color-stop(1, #343536) ); background:-moz-linear-gradient( center top, #a3a3a3 5%, #343536 100% ); filter:progid:DXImageTransform.Microsoft.gradient(startColorstr='#a3a3a3', endColorstr='#343536'); background-color:#a3a3a3; }");
	// GM_addStyle(".mtvs_sm_button:active { position:relative; top:1px; }");
	// GM_addStyle(".mtvs_subItem { margin: 0px 5px 0px 25px; }");
	// GM_addStyle(".mtvs_numeric { padding: 2px !important; font-size: 9pt !important; }")
	// GM_addStyle('.mtvs_header { color:#ffffff !important; font-size: 11pt; }');//padding: 5px 0px; }');
	// GM_addStyle('.mtvs_text { width: 68% !important; margin-right:10px; padding: 2px !important; font-size: 9pt !important; }');
	// GM_addStyle('.mtvs_small_text { font-size: 10px; }');
	// GM_addStyle('.mtvs_link { margin-left:3px; margin-right:3px; }');
	// GM_addStyle('.mtvs_class { display: inline-block; width:93px; margin-left:25px; margin-bottom:9px; font-size:8pt;}');
	// GM_addStyle('.mtvs_leftCol { width:50%; height:auto; display:table-cell; padding: 10px 0px 10px; }');
	// GM_addStyle('.mtvs_rightCol { width:auto; height:auto; display:table-cell; padding: 10px 0px 10px; }');
	...
	function StatusBox(title) {
		/* Setup status area */
		var status_area = $('#mtvs_greasemonkey_status_area').eq(0);
		if (status_area.length == 0) {
			var statWidth = '20%';
			if (window.innerWidth < 1340)
				statWidth = 268;
			status_area = $('<div id="mtvs_greasemonkey_status_area"></div>').css({
				'position': 'fixed',
		...
		return {
			...
			show: function(t) {
				if (global_hideStatusBox != 'true' || /www\.morethan\.tv\/torrents\.php.type/.test(document.URL)) {
					t = t || 0;
					update_timer(t);
					set_visible(true);
				}
	...
	function doOptionsMenu() {
		var options_menu = $('#mtvs_options_menu').eq(0);
		if (options_menu.length == 0) {
			optHeight = 570;
			optWidth = 820;
			options_menu = $('<div id="mtvs_options_menu" class="mtvs_menu"></div>').css({
			...
			refreshHeader = $('<h3 class="mtvs_header">Update Frequency</h3>');
			refreshInput = $('<input class="mtvs_subItem mtvs_numeric" type="text" name="interval" maxlength="3">Interval between updates in minutes (minimum of 10)<br>').css({'text-align':'right', 'width':'20px'});
			columns_div = $('<div></div>').css({ 'width':'100%', 'margin-top':'-18px', 'display':'table'});
			leftColumn = $('<div class="mtvs_leftCol"></div>');
			leftColumn.append(refreshHeader);
			leftColumn.append(refreshInput);

			hideHeader = $('<h3 class="mtvs_header">Visibility</h3>');
			check_box_hide = $('<input class="mtvs_subItem" type="checkbox" name="visibility">Show the status box on all pages<br>');
			explanation_div = $('<div class="mtvs_small_text mtvs_subItem">The status box will always appear on \'/torrents.php?type=...\' and whenever a script update is available.</div>');
			leftColumn.append(hideHeader);
			leftColumn.append(check_box_hide);
			leftColumn.append(explanation_div);

			positionHeader = $('<h3 class="mtvs_header">Status Box Position</h3>');
			radio_button_tl = $('<input class="mtvs_subItem" type="radio" name="location" id="top_left"/>Top Left<br>');
			radio_button_tr = $('<input class="mtvs_subItem" type="radio" name="location" id="top_right"/>Top Right<br>');
			radio_button_bl = $('<input class="mtvs_subItem" type="radio" name="location" id="bottom_left"/>Bottom Left<br>');
			radio_button_br = $('<input class="mtvs_subItem" type="radio" name="location" id="bottom_right"/>Bottom Right<br>');
			rightColumn = $('<div class="mtvs_rightCol"></div>');
			...
			offsetHeader = $('<h3 class="mtvs_header">Status Box Y-Offset</h3>');
			offsetInput = $('<input class="mtvs_subItem mtvs_numeric" type="text" name="yOffset" maxlength="3">The offset in pixels from the top or bottom of the window<br>').css({'text-align':'right', 'width':'20px'});
			...
			styleHeader = $('<h3 class="mtvs_header">Link Style Settings</h3>');
			full_div.append(styleHeader);
			sampleText = $('<span class="mtvs_class"></span><a href="#" id="sample_gsnatched">Sample Group Snatched Torrent Link</a><br>');
			sampleText.click(function () { return false; });
			snatchedInput = $('<span class="mtvs_class">.group_snatched</span><input class="mtvs_text" type="text" id="input_gsnatched" value="'+ style_groupsnatched +'">');
			applyLink = $('<span class="mtvs_sm_button">Test</span>');
			applyLink.click(function () { ApplyStyle('sample_gsnatched', 'input_gsnatched'); return false; });
			defaultLink = $('<span class="mtvs_sm_button">Default</span>');
			...
			sampleText = $('<span class="mtvs_class"></span><a href="#" id="sample_tsnatched">Sample Snatched Torrent Link</a><br>');
			sampleText.click(function () { return false; });
			snatchedInput = $('<span class="mtvs_class">.mtvs_snatched</span><input class="mtvs_text" type="text" id="input_tsnatched" value="'+ style_tsnatched +'">');
			applyLink = $('<span class="mtvs_sm_button">Test</span>');
			...
			sampleText = $('<span class="mtvs_class"></span><a href="#" id="sample_uploaded">Sample Uploaded Torrent Link</a><br>');
			sampleText.click(function () { return false; });
			snatchedInput = $('<span class="mtvs_class">.mtvs_uploaded</span><input class="mtvs_text" type="text" id="input_uploaded" value="'+ style_uploaded +'">');
			applyLink = $('<span class="mtvs_sm_button">Test</span>');
			...
			//sampleText = $('<span class="mtvs_class"></span><a href="#" id="sample_seeding">Sample Seeding Snatched Torrent Link</a><span>&nbsp;&nbsp;(.mtvs_snatched style is also applied to this link)</span><br>');
			//sampleTxt2 = $('<span class="mtvs_class"></span><a href="#" id="sample_ul_seed">Sample Seeding Uploaded Torrent Link</a><span>&nbsp;&nbsp;(.mtvs_uploaded style is also applied to this link)</span><br>');
			sampleText = $('<span class="mtvs_class"></span>Seeding links will <i>always</i> have either the .mtvs_snatched style or the .mtvs_uploaded style applied<br><span class="mtvs_class"></span>to them, so .mtvs_seeding is commonly used to override those base styles.</br>');
			sampleTxt2 = $('<span class="mtvs_class"></span><a href="#" id="sample_seeding">Sample Seeding Snatched Torrent Link</a>&nbsp;&nbsp;<a href="#" id="sample_ul_seed">Sample Seeding Uploaded Torrent Link</a><br>');
			//sampleText.click(function () { return false; });
			sampleTxt2.click(function () { return false; });
			snatchedInput = $('<span class="mtvs_class">.mtvs_seeding</span><input class="mtvs_text" type="text" id="input_seeding" value="'+ style_seeding +'">');
			applyLink = $('<span class="mtvs_sm_button">Test</span>');
			...
			sampleText = $('<span class="mtvs_class"></span><a href="#" id="sample_leeching">Sample Leeching Torrent Link</a><br>');
			sampleText.click(function () { return false; });
			snatchedInput = $('<span class="mtvs_class">.mtvs_leeching</span><input class="mtvs_text" type="text" id="input_leeching" value="'+ style_leeching +'">');
			applyLink = $('<span class="mtvs_sm_button">Test</span>');
			...
			sampleText = $('<span class="mtvs_class"></span><a href="#" id="sample_bookmarked">Sample Bookmarked Torrent Link</a><br>');
			sampleText.click(function () { return false; });
			snatchedInput = $('<span class="mtvs_class">.mtvs_bookmark</span><input class="mtvs_text" type="text" id="input_bookmarked" value="'+ style_bookmarked +'">');
			applyLink = $('<span class="mtvs_sm_button">Test</span>');
			...
			okay_button = $('<span id="js_ok_button" class="mtvs_button">Submit</span>');
			okay_button.click(function () { CommitOptions(); DisplaySlideMenu(false); });
			cancel_button = $('<span id="js_close_button" class="mtvs_button">Cancel</span>');
	...
	function CommitOptions() {
		...
		AddOrDeleteCustomStyle('input_tsnatched',	T_SNATCHED, 	'style_tsnatched', 		'.mtvs_snatched');
		AddOrDeleteCustomStyle('input_uploaded',	UPLOADED, 		'style_uploaded', 		'.mtvs_uploaded');
		AddOrDeleteCustomStyle('input_leeching', 	LEECHING, 		'style_leeching', 		'.mtvs_leeching');
		AddOrDeleteCustomStyle('input_seeding', 	SEEDING, 		'style_seeding', 		'.mtvs_seeding');
		AddOrDeleteCustomStyle('input_bookmarked', 	BOOKMARKED, 	'style_bookmarked', 	'.mtvs_bookmark');
	}
	...
	function DisplaySlideMenu(showMenu) {
		if (showMenu) {
			if (!slideMenuShowing) {
				slideMenuShowing = 1;
				$('#mtvs_options_menu').show().animate({
					top:'-=' + ($('#mtvs_options_menu').innerHeight()-10) + 'px'
				});
			}
		} else {
			slideMenuShowing = 0;
			$('#mtvs_options_menu').animate({
				top:'+=' + ($('#mtvs_options_menu').innerHeight()-10) + 'px'
			}, function () { $('#mtvs_options_menu').hide(); });
		}
	}
	/*****************************/
	...
	function registerMenuCommand(oText, oFunc) {
		...
		MenuCommandArray[MenuCommandArray.length] = [oText.replace("Morethan.tv Snatched: ",""),oFunc,oText.replace("Morethan.tv Snatched: ","").replace(" ","_")];
	}
	...
	/* Get Morethan.tv base URL */
	var mtv_url_base = document.URL.match(/^(https:\/\/www\.morethan\.tv)/)[1];

	/* Create proxy */
	var mtv_proxy = Proxy(mtv_url_base, 1000);
	...
	if (!mtv_id) return; /* Exceptional condition: User ID not found */

	/* Create status box */
	// var server_version = GM_getLSValue("serverVersion", CURRENT_VERSION);
	var status = StatusBox('Morethan.tv Snatched');
	...
	/* Reset command */
	registerMenuCommand('Morethan.tv Snatched: Reset Snatched', function() { snatch_cache.clear(); bookmark_cache.clear(); GM_setLSValue('last_update', '0'); GM_setLSValue('full_update','1'); GM_setLSValue('fullUpdateStarted','1'); location.reload(); });
	/* Update w/o clear */
	registerMenuCommand('Morethan.tv Snatched: Update', function() { GM_setLSValue('last_update', '0'); GM_setLSValue('full_update','1'); GM_setLSValue('force_all','1'); GM_setLSValue('fullUpdateStarted','1'); location.reload(); });
	registerMenuCommand('Morethan.tv Snatched: Options', function() { DisplaySlideMenu(true); });
	...
	function scan_torrent_page(doc, type) {
		...
		torrent_table.find('div.group_info').each(function(i) {
			...
			if (m) {
				...
				log("Not sure how Morethan.tv Snatched got here. Please notify MordredKLB with what you were doing");
			...
			if (group_id && torrent_id) {
				// we are saving a type of "snatched" but when applying that class we have to apply it as "mtvs_snatched" due to a Morethan.tv having it's own .snatched style now
	...
	function scan_all_torrent_pages(type, page_cb, finish_cb, forced_full) {
		...
		function request_url() {
			if (type == 'bookmark')
				...
				return '/torrents.php?type='+type+'&userid='+mtv_id+'&page='+page;
		}
		...
		function page_handler(response) {
			if (response.status == 200) {
				...
				page += 1;
				mtv_proxy.get({ url: request_url(), callback: page_handler, error: error_handler });
			} else {
	...
	function mark_snatched_links() {
		...
		$('#content a').each(function(i) {
			...
			if (href) {
				...
				if (group_id && b.groups[group_id] && !(/www\.morethan\.tv\/bookmarks\.php/.test(document.URL)) &&
						!(/www\.morethan\.tv\/user\.php/.test(document.URL)) &&
						(!torrent_id || !$(this).parent().parent().is('.group_torrent')) && !$(this).is('.post_id')) {
					$(this).addClass('mtvs_bookmark');
				}
				if (torrent_id && d.torrents[torrent_id]) {
					if (d.torrents[torrent_id].ty == 'snatched')
						$(this).addClass('mtvs_snatched');	// we can't use .snatched anymore because what has now added it's own .snatched class
					else if (d.torrents[torrent_id].ty == 'uploaded')
						$(this).addClass('mtvs_uploaded');
					else if (d.torrents[torrent_id].ty == 'leeching')
						$(this).addClass('mtvs_leeching');
					if (d.torrents[torrent_id].sd) {
						if (d.torrents[torrent_id].ty != 'uploaded')
							$(this).addClass('mtvs_seeding mtvs_snatched');	// we're really just marking seeding here, but you can't seed if you haven't snatched so adding that class as well
						else
							$(this).addClass('mtvs_seeding');
						}
				}
		...
		if (/www\.morethan\.tv\/torrents\.php/.test(document.URL)) {
			/* Parse search */
			var search = {};
			...
			if (search.id) {
				...
				$('#content .torrent_table:first tr.group_torrent').each(function(i) {
					...
					$(this).find('td:first span:first a').each(function(i) {
						...
						if (href) {
							...
							if (m) {
								...
								torrent_id = m[1];
								$(this).removeClass('group_snatched mtvs_snatched mtvs_uploaded mtvs_leeching mtvs_seeding');
								return false;
							}
						}
					});
					...
					if (torrent_id && d.torrents[torrent_id]) {
						...
						if (d.torrents[torrent_id].ty == 'snatched')
							link.addClass('mtvs_snatched');	// we can't use .snatched anymore because what has now added it's own .snatched class
						else if (d.torrents[torrent_id].ty == 'uploaded')
							link.addClass('mtvs_uploaded');
						else if (d.torrents[torrent_id].ty == 'leeching')
							link.addClass('mtvs_leeching');
						if (d.torrents[torrent_id].sd) {
							if (d.torrents[torrent_id].ty != 'uploaded')
								link.addClass('mtvs_seeding mtvs_snatched');	// we're really just marking seeding here, but you can't seed if you haven't snatched so setting that class too
							else
								link.addClass('mtvs_seeding');
							}
					}
				});
			}
		...
		if (/www\.morethan\.tv\/torrents\.php\?id/.test(document.URL)) {
			...
			if (albumName) {
				...
				if (m) {
					...
					if (b.groups[group_id])
						albumName.addClass('mtvs_bookmark');
				}
			}
				...
				if (m) {
					...
					$(this).click(function(event) {
						if (!/remove/i.test($(this).text()) && !/unbookmark/i.test($(this).text())) {
							...
							bookmark_cache.serialize();
							$('#content').find('a.mtvs_bookmark').each(function(i) {
								var href = $(this).attr('href');
								torrentString = 'torrents.php?id='+group_id;
								if (href && href=='torrents.php?id='+group_id) {
									$(this).removeClass('mtvs_bookmark');
								}
							});
							$('#content > .thin > .header > h2 > span').eq(0).removeClass('mtvs_bookmark');
						}
					});
				}
			}
	...
	/* This function was hacked from a generic one and converted to jQuery to work better with Morethan.tv Snatched.
	...
	if (/www\.morethan\.tv\/torrents\.php/.test(document.URL)) {
		...
		if ((search.type == 'snatched' || search.type == 'uploaded' || search.type == 'seeding' || search.type == 'leeching') &&
				search.userid == mtv_id && full_update == 0) {
	...
	if (/www\.morethan\.tv\/bookmarks\.php(?!.action=edit)/i.test(document.URL)) {
		var scan_status = $('<div>Scanning current page... <span></span></div>');
		status.contents().append(scan_status);
		status.show();
	...
	// 	location.reload();
	// }
	if (full_update == 1 /*&& !(/www\.morethan\.tv\/torrents\.php/.test(document.URL))*/) {
