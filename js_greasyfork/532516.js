...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/532516/%E8%9D%89%E4%B9%8B%E5%8A%A9.user.js
// @updateURL https://update.greasyfork.org/scripts/532516/%E8%9D%89%E4%B9%8B%E5%8A%A9.meta.js
// ==/UserScript==
...
4. **元素替换规则**
   - **【重要】当用户要求替换某个元素时(如"更换太空服"或"把猫换成狗")：**
   - 仅在正向提示词中替换相应元素，保持替换元素的位置和相对权重
   - **不要**将被替换的元素(如"太空服"或"猫")添加到负向提示词中
   - 负向提示词仅用于排除低质量元素和不需要的特征，不应包含被替换的正常元素
...
4. **元素替换规则**
   - **【重要】当用户要求替换某个元素时(如"更换太空服"或"把草地换成沙滩")：**
   - 仅在正向提示词中替换相应元素，保持替换元素的位置和相对权重
   - **不要**将被替换的元素(如"太空服"或"草地")添加到负向提示词中
   - 负向提示词仅用于排除低质量元素和不需要的特征，不应包含被替换的正常元素
...
- 用户描述"把蓝色衣服换成红色衣服"→在正向提示词中替换blue clothes为red clothes，不在负向提示词中添加blue clothes`;
