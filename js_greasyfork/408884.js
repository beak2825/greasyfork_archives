...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/408884/WK%20Enhanced%20Search.user.js
// @updateURL https://update.greasyfork.org/scripts/408884/WK%20Enhanced%20Search.meta.js
// ==/UserScript==
...
(function() {
    ...
    */
    ...
    var convertResponseToObj = function(response){
        if (response.error){
            ...
            var results = (response.data||response).map(function(v){
                ...
                let sVGurl = v?.data?.character_images?.filter((i)=>{return i.content_type == "image/svg+xml"})?.[0].url;
                var val = (type === "rad" && v.data && !v.data.characters)?
                    ...
                    '<wk-character-image src="'+sVGurl+'" class="subject-character__character-image" aria-label="'+slug+'"></wk-character-image>':
    ...
    var incrementResultTotalBy = function(searchResultsElem, num){
        ...
        var text = searchResultsElem.childNodes[5].childNodes[1].innerHTML;
        //get current number
        var total;
        let pieces;
        const noResults = text.match(/\<p class="wk-text"\>Nothing was found for \<strong\>(.*?)\<\/strong\>\<\/p\>/);
        if (noResults){
            pieces = ['<p class="wk-text">', num, " results found for <strong>", noResults[1], "</strong></p>"];
        } else {
            total = text.match(/([0-9]+)/)[0];
            //get the text around it
            pieces = text.split(total);
            //get the new total
        ...
        searchResultsElem.childNodes[5].childNodes[1].innerHTML = pieces.join("");
    };
    ...
    var createSearchResultElem =function([response, searchResultsElem]){
        ...
        var ul = searchResultsElem.querySelector(".subject-character-grid__items");
        if (!ul) { //No results from native search, have to add our own element
            const newGrid = Object.assign(document.createElement('div'), { className: 'subject-character-grid' });
            document.querySelector('div.search-results').append(newGrid);
            ul = newGrid.appendChild(Object.assign(document.createElement('ol'), { className: 'subject-character-grid__items' }));
        }
        ...
        .then(function(searchResults){
         searchResults = searchResults || [];
            ...
            .map(function(v,i,a){
                ...
                ul.prepend(htmlToNode('<li '+(type==="oth"?'style="'+otherStyle:
                                 ...
                                 ''
                                )+'class="subject-character-grid__item">'+
                         '<a class = "subject-character subject-character--'+longType+' subject-character--grid subject-character--'+(visualiseLocked?'locked':'unlocked')+'" title="'+ja+'" href="/'+path+'/'+slug+'">'+ // this needs to be slug, not val
                         '<div class="subject-character__content">'+
                         '<span class="subject-character__characters">'+
                         '<span class="subject-character__characters-text" lang="ja">'+val+'</span>'+
                         '</span>'+
                         '<div class="subject-character__info">'+
                         (type === "rad"?'':'<span class="subject-character__reading">'+ja+'</span>')+
                         '<span class="subject-character__meaning">'+en+'</span>'+
                         '</div></div>'+
                         '</a></li>'));
...
/*            new MutationObserver((mutationRecords, observer) => {
    ...
    var initHooks = function(){
        ...
        //document.querySelector("form.search__form").addEventListener('submit', initHooks);
        ...
        document.querySelector(".site-content")?.classList?.add('enhanced');
        /*});*/
//mute || console.log("getSubj", getSubjectsBySlug(["低い","ground","新聞"]));
    ...
    const initializeObserver = function(){
        // Select the html element as the target node
        const targetNode = document.body;

        // Configure the observer to watch for childList and subtree changes
            ...
            for(let mutation of mutationsList) {
                if (mutation.type === 'childList') {
                    for(let addedNode of mutation.addedNodes) {
                        // Check if the added node is the new body element
                        if (addedNode.classList?.contains('site-content')) {
                            if (!addedNode.classList.contains('enhanced')){
                                initHooks();
                                //addedNode.classList.add('enhanced'); //mark the node so we don't keep adding results every time we see it
                            }
                            // Now, search for the .search-results element within the new body
                            const searchResultsElement = addedNode.querySelector('.search-results');
                            if (searchResultsElement) {
                                console.log('New .search-results element found:', searchResultsElement);
                            //    alert("just once eh?");

                                // Your code to process the data goes here
                                // You might want to disconnect the observer if you only need to process it once
                                // observer.disconnect();
                            }
                        }
                    }
