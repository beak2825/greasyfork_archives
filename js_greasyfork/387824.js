// ==UserScript==
// @name           Squad_OverviewTM - ASI_Rou_Months + Tooltip R5_Rec
// @version 	     1.0.0
// @description	   Show ASI Rou months on Club Squad page + Tooltip R5 and Rec ( ქართული )
...
// @grant	      none
// @downloadURL https://update.greasyfork.org/scripts/387824/Squad_OverviewTM%20-%20ASI_Rou_Months%20%2B%20Tooltip%20R5_Rec.user.js
// @updateURL https://update.greasyfork.org/scripts/387824/Squad_OverviewTM%20-%20ASI_Rou_Months%20%2B%20Tooltip%20R5_Rec.meta.js
// ==/UserScript==
...
$("#player_table tr:eq(0) th:eq(4)").after('<th align="center" title="Routine (+ skill bonus)"><b>Rou</b></td>');
$("#player_table tr:eq(0) th:eq(7)").after('<th align="center"><b>ASI</b></td>');
$("#player_table tr > .text_fade > div").not(".text_fade_overlay").find("a[player_link]").each(function(){
$(".column2_a").width("550px");
$(".column3_a").width("236px");
...
parent_select.find("td:eq(2)").append("."+MONTHS);
    ...
    parent_select.find("td:eq(7)").after('<td style="text-align:right;padding: 0 5px 0 5px;">'+asi+'</td>');
...
$("#player_table").find("th:eq(9)").attr('colspan',9);
$("#player_table").find("tr > th:eq(10)").parent().after("<tr><th style='width:30px' align='center'>#</th><th style='width: 160px; padding: 0px 0px 0px 5px;'>Name</th><th align='center'>Age</th><th style='width:70px' class='align_center'>Position</th><th></th><th align='center' title='Routine (+ skill bonus)'><b>Rou</b></th><th align='center'><b>ASI</b></th></tr>");
$(".column3_a").find(".box_body > div > p:first").before("<p>ჯამური გამოცდილება: "+addCommas(Math.round(valoretotale/10000)/100)+"<br>ხელფასების ჯამი: "+addCommas(wagetotale)+"</p>");
...
$('a[player_link]').mouseenter(function() {
	...
	var not20 = 0;					// 20ä»¥å¤ã®ã¹ã­ã«æ°
    for (i = 0; i < weightRb[fp].length; i++) { // weightR[fp].length = n.pesi[pos] cioÃ¨ le skill: 14 o 11
    ...
    if (fp2 != -1 && fp2 != fp) {
	    ...
	    not20 = 0;					// 20ä»¥å¤ã®ã¹ã­ã«æ°

        for (i = 0; i < weightRb[fp2].length; i++) { // weightR[fp].length = n.pesi[pos] cioÃ¨ le skill: 14 o 11
    ...
    var RECname = "<div style='color:white;'>რეკომენდაცია</div>";
    ...
    if (s === "" || string === null) {
        $('#tooltip').find('tr:eq(5)').find('td:eq(0)').append(RECname);
        $('#tooltip').find('tr:eq(7)').find('td:eq(0)').append(RR5name);
        $('#tooltip').find('tr:eq(5)').find('td:eq(1)').append(REC);
