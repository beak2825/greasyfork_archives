...
// @version      1.3975
// @released     2024-03-28_20:14:10_215
// @description  为腾讯、优酷、爱奇艺增加修改样式、视频播放便利、跳转解析网站播放视频的功能，对解析网站进行增强改造，以及避免一些不必要的误触。,识别垃圾广告常用的域名，自动退出广告页面
// @author       果心豆腐酱
...
// @match        https://jx.yangtu.top/*
// @match        https://jx.jsonplayer.com/*

// @match        *://*.iqiyi.com/*
// @match        *://*.youku.com/*
// @match        *://*v.qq.com/*
// @match        https://im1907.top/*
// @grant        GM_xmlhttpRequest
// @icon         https://v.qq.com/favicon.ico
// @grant        GM_setValue
// @grant        GM_getValue
// @run-at       document-start
// @downloadURL https://update.greasyfork.org/scripts/483629/%E8%A7%86%E9%A2%91%E7%BD%91%E7%AB%99%E6%B8%85%E7%88%BD%E6%A8%A1%E5%BC%8F2.user.js
// @updateURL https://update.greasyfork.org/scripts/483629/%E8%A7%86%E9%A2%91%E7%BD%91%E7%AB%99%E6%B8%85%E7%88%BD%E6%A8%A1%E5%BC%8F2.meta.js
// ==/UserScript==


//优质配色：#f6f6f6
let 域名 = location.hostname
if (域名.includes('.info') || 域名.includes('dwja50.xyz') || 域名.includes('ld48dsn.xyz') || 域名.includes('kl38.cc') || 域名.includes('jas040ki.xyz')) {
    location.href = GM_getValue("上次视频")
...
if (!location.href.includes('.xmflv.')) {
    ...
    if(document.querySelector('[class="report-area none"]'))return;
    修改样式();
    function 修改样式() {
    ...
    function 腾讯视频会员到期提示() {
        var viptime = document.querySelector('.vip_now.__tips')?.textContent?.match(/\d{4}-\d{2}-\d{2}/);
        if (viptime) {
            const currentDate = new Date();
            // 获取VIP到期日期
            const vipExpireDate = new Date(viptime);
            // 计算距离到期日的天数
            const daysUntilExpire = Math.ceil((vipExpireDate - currentDate) / (1000 * 60 * 60 * 24));

            if (daysUntilExpire <= 90 && daysUntilExpire > 30) {
                remindOncePerPeriod('1_month');
            } else if (daysUntilExpire <= 7 && daysUntilExpire > 3) {
                remindOncePerPeriod('1_week');
            } else if (daysUntilExpire <= 3) {
                remindOncePerDay();
            }
        }

        function remindOncePerPeriod(period) {
            const lastReminderDate = localStorage.getItem(`lastReminderDate_${period}`);
            const today = (new Date()).toDateString();

            if (lastReminderDate !== today) {
                alert(`您的VIP会员将于${expireDate}到期，请及时续费！`);
                localStorage.setItem(`lastReminderDate_${period}`, today);
            }
        }
        function remindOncePerDay() {
            const today = (new Date()).toDateString();
    ...
    function 创建解析按钮() {
        if (!document.querySelector('.解析站点').checked) {
            return;
        }
        document.querySelector('.解析视频')?.remove()
        ...
        if (优酷剧集按钮) {
            let html = `<div class="解析视频" style="display: inline-block;left: -5px;margin: 0px 15px; position: relative;top: 2px;align-content: space-around;">
              ...
              <span class="解析span" style="/* font-size: 10px; */left: 2px;top: 23px;position: absolute;font-size: 12px;font-weight: 600;height:20px;line-height: 20px;">解析</span>
            </div>`;
            if (优酷剧集按钮) {
        ...
        if (爱奇艺追剧边按钮) {
            ...
            <span class="解析span" style="height: 20px;;left: -14px;top: 9px;position: relative;font-size: 12px;font-weight: 600;">解析</span>
            </div>
        ...
        let 爱奇艺收藏 = document.querySelector('[class="interactiveBlock_interactive__TmxDm"]');
        if (爱奇艺收藏) {
            let html = `
            <div class="interactiveBlock_line__KFfHM"></div>
            <div class="解析视频">
            <img src="${图标}" style="width: 28px;margin: 0px 20px 0px 20px;height: 30px;top: -2px;position: relative;left: -13px;">
            <span style="left: -34px;top: 4px;position: relative;font-size: 12px;font-weight: 600;">解析</span>
            </div>
            `;
            爱奇艺收藏.insertAdjacentHTML("beforeend", html);
            document.querySelector('[id="qyBarrageVue"]')?.insertAdjacentHTML("beforeend", html);
        }
    ...
    function 创建主设置组件() {
        ...
        settingPanel.innerHTML = `
                ...
                <br>
                <input type="checkbox" class="解析站点" style="    -webkit-appearance: button;cursor:pointer;border:0px">
                <label for="options">解析站点：</label>
                <select id="options">
                  <option value="虾米" url="https://jx.xmflv.com/?url=">虾米</option>
                  <option value="M1907" url="https://im1907.top/?jx=">M1907</option>
                  <option value="虾米2" url="https://jx.xmflv.cc/?url=">虾米2</option>
                  <option value="虾米2" url="https://jx.yangtu.top/?url=">虾米2</option>
                </select>
                <br>
                ...
                <form action="/submit-form" method="post">
                    ...
                    <option value="朱金">朱金</option>
                    <option value="春节红">春节红</option>
                    <option value="炎红">炎红</option>
                    </select>
                </form>
                <br>
        ...
        // 添加事件监听器，在选项发生变化时执行相应的操作
        selectBox.addEventListener('change', function () {
            let selectedOption = selectBox.selectedOptions[0];
            let url = selectedOption.getAttribute('url');
            解析站点URL = url;
            localStorage.setItem('解析站点URL', 解析站点URL)
            localStorage.setItem('选择站点', selectBox.selectedIndex)
        });
        // 获取要选择的选项的索引
        var targetIndex = Number(localStorage.getItem('选择站点'))
        if (targetIndex > 0) {
            // 选择选项
        ...
        function 加载cookie(cookieStr, cookieDomain, daysToExpire) {
            if (cookieStr) {
                if (cookieStr !== "") {
                    ...
                    if (cookies.length !== 0) {
                        cookies.forEach(function (cookie) {
                            ...
                            // 设置cookie，包括过期时间
                            document.cookie = key + "=" + encodeURIComponent(value) + "; domain=" + cookieDomain + "; path=/; expires=" + expires.toUTCString();
                        });
                    } else {
        ...
        // }
        var advertisement = localStorage.getItem('advertisement');
        ...
        document.querySelector('.解析站点').checked = eval(localStorage.getItem('解析站点'))
        ...
        saveButton.addEventListener('click', function () {
            ...
            //主题替换功能
            var 主题替换 = document.querySelector("#htmlbackground")?.checked;
            localStorage.setItem('htmlbackground', 主题替换);
            if (主题替换) {
                localStorage.setItem('主题选择', document.querySelector('#cars').value);
                修改样式();
                //保存侧边栏设置
            ...
            //保存解析站带你设置
            localStorage.setItem('解析站点', document.querySelector(".解析站点")?.checked);
            if (document.querySelector('.解析站点').checked) {
                创建解析按钮();
            }
        });
    ...
    function 判断爱奇艺选集按钮() {
        var intervalId = setInterval(function () {
            ...
            let 选集 = document.querySelector('[class="iqp-btn iqp-btn-episode"]');
            if (选集) {
                clearInterval(intervalId);
            ...
            for (var mutation of mutationsList) {
                ...
                for (var addedNode of mutation.addedNodes) {
                    ...
                    if (addedNode.nodeType === Node.ELEMENT_NODE) {
                        ...
                        let videoyuansu = addedNode.querySelector('video')
                        if (videoyuansu) {
                            console.log('videoyuansu', videoyuansu)
                        }
                        ...
                        var 协议 = addedNode.querySelector('[class="selected-wrap"]');
                        if (协议) {
                            协议.click();
                        }
                    }
    ...
    function 监测去除腾讯广告() {
        ...
        var 播放器缩小广告 = document.querySelector('[class="txp_full_screen_pause-close"]');
        if (播放器缩小广告) {
            播放器缩小广告.click();
            console.log("删除播放器缩小广告");
        }
        ...
        var 侧边挂件 = document.querySelector('[class="iwan-gamependant-container"]');
        if (侧边挂件) {
            侧边挂件.remove();
            console.log("删除侧边挂件广告", "iwan-gamependant-container");
        }
    ...
    function 爱奇艺去广告() {
        if (localStorage.getItem('advertisement') === "true") {
            //下载按钮
           document.querySelector('[class="btnDownload_item__7wa4m   "]')?.remove();;
            ...
            if (轮播栏广告.length > 0) {
                for (let index = 轮播栏广告.length - 1; index >= 0; index--) {
            ...
            if (轮播栏广告2) {
                if (轮播栏广告2.length > 0) {
                    for (let index = 轮播栏广告2.length - 1; index >= 0; index--) {
            ...
            // 轮播栏广告
            let 轮播栏广告3 = document.querySelectorAll('.panel-item-link ');
            if (轮播栏广告3.length > 0) {
                for (let index = 轮播栏广告3.length - 1; index >= 0; index--) {
                    let href = 轮播栏广告3[index].href;
                    if (href) {
                        let url = new URL(href).hostname;

                        if (url) {
                            if (!url.includes('.iqiyi') || url.includes('game')) {
                                轮播栏广告3[index].remove();
                                console.log('爱奇艺轮播栏广告3广告去除执行完成');
                            }
                        }
                    }
                }
                ...
                // 轮播栏广告
                let 轮播栏广告4 = document.querySelectorAll('.qy20-h-carousel__link');
                if (轮播栏广告4.length > 0) {
                    for (let index = 轮播栏广告4.length - 1; index >= 0; index--) {
                        let href = 轮播栏广告4[index].href;
                        if (href) {
                            let url = new URL(href).hostname;
                            ...
                            if (url) {
                                if (!url.includes('.iqiyi') || url.includes('game')) {
                                    轮播栏广告4[index].remove();
                                    document.querySelectorAll('.qy20-h-carousel__ul')[index]?.remove()
                                    console.log('爱奇艺轮播栏广告4广告去除执行完成');
                                }
                            }
                        }
                    }
    ...
    function 优酷去广告() {
        if (localStorage.getItem('advertisement') === "true") {
            ...
            //条幅广告
            let 条幅广告 = document.querySelectorAll('[class="channelcommon_container   "]')
            for (let index = 0; index < 条幅广告.length; index++) {
                条幅广告[index].parentElement.remove();
                console.log('优酷条幅广告去除执行完成')
            }
    ...
    function 监测爱奇艺轮播广告() {
        // 目标元素
        const targetElement = document.querySelector('.focus-img-wrap');
        if (targetElement) {
            ...
            const observer = new MutationObserver((mutationsList, observer) => {
                ...
                for (const mutation of mutationsList) {
                    if (mutation.type === 'childList') {
                        ...
                        let 链接 = mutation.target.querySelector('.img-link');
                        if (链接) {
                            let url = new URL(链接.href).hostname;
                            if (!url.includes('.iqiyi') || url.includes('game')) {

                                链接.href = ''
                                链接.style.backgroundImage = 'url("//pic0.iqiyipic.com/lequ/common/lego/20240107/cad725369c6c4c579219ee46f3b4d6d5.jpg?caplist=jpg,webp")'
                                链接 = targetElement.querySelector('.focus_title_inner');
                                if (链接) {
                                    url = new URL(链接.href).hostname;
                                    if (!url.includes('.iqiyi') || url.includes('game')) {
                                        链接.href = ''
                                        链接.querySelector('.caption').textContent = '航海之王';
                                        链接.querySelector('.desc').textContent = '新春观影快乐';
                                        console.log('修改标题', mutation.target);
                                    }
                                }
                            }
                        }
    ...
    function 侧边栏按钮创建() {
        ...
        document.querySelector('.toggle_button')?.remove()
    ...
    function 侧边栏调整() {
        ...
        function toggleSidebar(selectButton) {
            ...
            if (自带收缩按钮) {
                ...
                播放器 = document.querySelector('#player-container');
    ...
    function 函数存放(函数) {
        ...
        if (函数 === "页面样式修改") {
            if (localStorage.getItem('htmlbackground') === "true") {
                var 主题选择 = localStorage.getItem('主题选择');
                if (主题选择) {
                    主题色选择(主题选择);
                } else {
                    let 主题 = "月光色";
                ...
                if (location.hostname.includes('v.qq.')) {
                    ...
                    .small-focus .top-nav-wrap[data-v-063bf321], .top-nav-wrap-black[data-v-063bf321] {
                        background-color: ${主题色}f5 !important;
                    }
                    .at-feed--dark{
                        --C2: ${浅灰字体色} !important;
                    }
                    ...
                    .at-feed.at-feed--dark.at-feed--normal {
                        margin: 0 0 0 10px;
                    }
                    /*播放器异常视频列表*/
                    .episode-panel__scroll-wrapper[data-v-fdb92036] .episode-panel__scroll-wrapper-header {
                        background:${深主题色} !important;
                    }
                    .b-tab[data-v-88ee66b1] {
                        background:${深主题色} !important;
                    ...
                    /*顶栏高度*/
                    [class="top-nav-wrap head_inner"]{
                        height: 50px !important;
                    }
                    /*顶栏按钮*/
                    .channel-nav-text:hover {
                        color: #00ff62;
                    ...
                    [class="page-play view-content"] [class="_footer-wrap_yjuq2_1"]{
                            width: calc(100vw - 475px) !important;
                    }
                    .at-feed-stream-comment-item {
                        border-top: 1px solid ${深主题色} !important;
                    }
                    .at-feed-stream-item-wrapper {
                        border-radius: 10px;
                        background: ${深主题色} !important;
                        border-top: 1px solid ${深主题色} !important;
                    }
                    [class="at-feed at-feed--normal"]{
                        margin: 0 0 0 10px;
                    }
                    .container-main__left {
                        padding-bottom: calc(100vh - 55px) !important;
                    }
                    /*预约按钮*/
                    ...
                    .top-nav-wrap.top-nav-wrap-black {
                        background: linear-gradient(to bottom,${深主题色}f5 ,${主题色}f5);
                    }
                    /*播放界面顶栏设置*/
                    #ssi-header{
                        --background: ${深主题色}7d !important;
                    }
            ...
            /*播放页面*/
                    ...
                    /*播放器*/
                    .container-main__wrapper[data-v-83430311] {
                        position: relative;
                        top: -5px !important;
                    }
                    .play-layout[data-v-83430311] {
                        padding: 0 !important;
                    }
                    ...
                    @media (max-width: 1280px){
                        .page-content .container-player[data-v-e54bf460] {
                    ...
                    @media (max-width: 1840px){
                        .page-content .container-player[data-v-e54bf460] {
                            height: 7100vh !important;
                        }
                    }
                    .container-player[data-v-e54bf460] {
                        height: 100vh !important;
                    }
                    .b-sticky__container[data-v-78461fb4] {
                        top: 50px !important;
                    }
                    /*分享列表*/
            ...
            /*旧播放器页面*/
                ...
                /*备案栏配置*/
                    ...
                    .card.vertical {
                      ...
                      .player-bottom__intro[data-v-00cea49d] {
                        padding-left: 32px  !important;
                    }
                      .episode-container[data-v-fdb92036] {
                        position: absolute !important;
                        
                        top: calc(100vh - 55px) !important;
                        height: 100vh !important;
                    }
                    .container-bottom__inner[data-v-e54bf460] {
                        width: calc(100vw - 96px) !important;

                    }
                    /*.player-bottom[data-v-7a1450c1] {
                        background: ${深主题色}  !important;
                    }*/
                    .container-bottom__left[data-v-e54bf460] {
                        width: calc(100vw - 400px) !important;
                    }
                    .episode-panel__scroll-wrapper[data-v-fdb92036] {
                        background: ${深主题色}  !important;
                    }
                    // .container-main__left[data-v-e54bf460] {
                    //     width: 100vw !important;
                    // }
                    .container-main[data-v-e54bf460] {
                        width: 98vw !important;
                    }
                    .container-episode[data-v-e54bf460] {
                        width: 0px !important;
                    }
                    /*全部剧集关闭按钮*/
                    svg.base-icon.svg-icon.svg-icon__overlay-close {
                        width: 40px !important;
                        height: 40px !important;
                    }
                    svg.base-icon.svg-icon.svg-icon__overlay-close:hover {
                        width: 40px !important;
                        height: 40px !important;
                        color: #570000 !important;
                    }
                    * {
                    scrollbar-color: ${深主题色} transparent !important;
                }

                    .player-bottom__item-download{
                        display: none !important;
                    }
                    .list-page-wrap[data-v-fda3d7da]{
                        top: 50px !important;
                    }
                    .list-base-wrap[data-v-b9f874bd]{
                        top: 50px !important;
                ...
                if (location.hostname.includes('youku.')) {
                    ...
                    #app{
                    background-color: ${主题色} !important;
                    }
                    body {
                        background: ${主题色} !important;
                    }
                    ...
                    [data-theme=dark] .listbox-new:hover {
                        opacity: 1;
                        height: calc(100vh - 290px) !important;
                    }
                    [data-theme=dark] .module-playbox-new .playbox-new .play-paction-wrap-new {
                        width: 100% !important;
                    }

                    vip-layer.kui-vip-layer-0 {
                        display: none !important;
                    }
                    [data-theme=dark] .play-top-container-new {
                    ...
                    .topheadernav_nav_box .topheadernav_nav_item {
                        line-height: 50px !important;
                    }
                    .topheader_top_header_box .topheader_top_box {
                        height: 50px !important;
                    }
                    [data-theme=dark] .l-container-new {
                        padding-top: 50px !important;
                    }
                    .topheader_top_header_box {
                        height: 50px !important;
                    }
                    .search_search_box_nav {
                        top: 5px !important;
                    }
                    ...
                    .topheadernav_nav_box {
                        height: 50px !important;
                    }
                    
                    .ikudownload_kuhuduan_box a {
                        height: 50px !important;
                    }
                    .historyrecord_urecord .historyrecord_recordicon {
                        height: 50px !important;
                    }
                    .vip_vip_wrap {
                    ...
                    .usercenter_user_center_box {
                        margin: 4px 46px 0 9px !important;
                    }
                    #checkout_counter_popup{
                        display: none !important; 
                    }
                    .fee-wrap-new.full-mode {
                        display: none;
                    }
                    /*登录广告*/
                    [class="usercenter_panel usercenter_upanel  "]{
                        display: none;
                    }
                    .search_search_box {
                        width: 465px;
                    ...
                    [data-theme=dark] .module-playbox-new{
                        height: calc(100vh - 55px) !important;
                    }
                    .fixed_bar_2rZMG {
                        opacity: 0 !important;
                    }
                    .historyrecord_urecord .historyrecord_panel {
                        top: 50px !important;
                    }
                    .ikudownload_kuhuduan_box .ikudownload_download_panel {
                        top: 50px !important;
                    }
                    ...
                    @media (max-width: 1440px){
                        [data-theme=dark] .play-paction-wrap-new #player {
                            width: 100% !important;
                ...
                if (location.hostname.includes('.iqiyi.')) {
                    css = `
                     .qy-mod-img.horizon[data-v-4e909722] {
                        background: ${深主题色};
                        border-radius: 8px;
                    }
                    .qy-mod-img .qy-mod-link-wrap[data-v-4e909722], .qy-mod-img .qy-mod-link[data-v-4e909722] {
                        border-radius: 8px;
                    }
                    ...
                    .qy-header.home2020.qy-header--absolute .header-wrap{
                        background: linear-gradient(180deg,${主题色}c9,#ff000000 150px) !important;;
                    }
                    ...
                    .qy-app-wrap .flash-wrap {
                        width: 100%;
                    }
                    .side-cont[data-v-3da728c4] {
                        position: absolute !important;
                    ...
                    .btn-wrap[data-v-554b252e]{
                        display: none;
                    }
                    .iqp-bottom-show .iqp-bottom {
                        background: ${主题色} !important;
                    }
                    .fixed-box[data-v-727893da] {
                        position: relative;
                        background: #fff;
                        z-index: 10003;
    ...
    function 新窗口() {
        ...
        const iframeSrc = 解析站点URL + decodeURIComponent(页面链接);
...
if (location.href.includes('.xmflv.') || location.href.includes('im1907.')||location.href.includes('yangtu.')||location.href.includes('jsonplayer.')) {

    var cid;
    var 当前VID;
    var 当前标题 = '酷炫解析';
    var 阻止弹出新页面 = true;
    var 恢复播放 = eval(localStorage.getItem('恢复播放'));
    var 工具条;
    //先执行判断离开页面时，是否阻断，避免跳入广告网站
    页面关闭监测();
    localStorage.setItem('ICOcVk', new Date().getTime())
    localStorage.setItem('ICOcVk_2', 1)
    样式创建();
    ...
    function 判断视频组件加载() {
        var intervalId = setInterval(function () {
            ...
            if (video) {
                ...
                判断历史记录();
                // 创建下一集按钮();
                工具条.appendChild(添加辅助功能());
                弹幕按钮状态();
                绑定热键();
                创建隐藏工具条按钮();
                
                // 添加用户交互，例如点击按钮来触发视频播放
                工具条.addEventListener('click', function() {
                    if (恢复播放 === true && video.paused) {
    ...
    function 取工具条() {
        工具条 = document.querySelector(".art-controls");
        if (!工具条) {
            工具条 = document.querySelector("#s-controls");
        }
        return 工具条;
    }
    function 创建视频列表主程序() {
    ...
    function 创建隐藏工具条按钮() {
        document.querySelector('.隐藏')?.remove();
        var 隐藏 = document.createElement('div');
        隐藏.className = '隐藏';
        隐藏.textContent = '隐藏';
        隐藏.style.borderRadius = '2px';
        if (判断是否是手机设备()) {
            隐藏.style.bottom = '70px';
        } else {
            var viewportHeight = Math.max(document.documentElement.clientHeight || 0, window.innerHeight || 0);
            var calculatedValue = -1 * viewportHeight + 50;
            隐藏.style.top = calculatedValue + 'px';
        }
    ...
    function 创建下一集按钮() {
        ...
        document.querySelector('.art-control.art-control-btn-next.hint--rounded.hint--top')?.remove();
    ...
    async function 剧集盒子创建() {
        ...
        // 构造一个 Promise 对象来检查目标元素是否存在
        const 工具条检查 = new Promise((resolve, reject) => {
            let 检查次数 = 0;
            const 检查间隔 = setInterval(() => {
                const 工具条 = document.querySelector('[class="art-controls"]');
                if (工具条) {
                    clearInterval(检查间隔);
                    resolve(工具条);
                } else {
        ...
        // 等待目标元素被添加到文档中
        try {
            const 工具条 = await 工具条检查;
            工具条.appendChild(剧集盒子);
            剧集盒子点击判断(剧集盒子);
            return 剧集盒子;
        } catch (error) {
            console.error(error);
    ...
    async function 创建视频列表(剧集JSON) {
        创建剧集选择按钮();
        let 剧集盒子 =await 剧集盒子创建();
        let 当前页数 = 1;
        let 每页数量 = 110;
    ...
    function 创建剧集选择按钮() {
        ...
        var interval = setInterval(function () {
            ...
            if (工具条) {
                ...
                if (location.hostname.includes('im1907.top')) {
                    selectButton.style.position = 'absolute';
                    selectButton.style.right = '55px';
    ...
    function 解析站创建主设置组件() {
        ...
        if (工具条) {
            ...
            if (location.hostname.includes('im1907.top')) {
                ...
                settingButton.style.right = '55px';
                settingButton.style.bottom = '-20px';
            }
    ...
    function 页面关闭监测() {
        ...
        window.addEventListener('beforeunload', function (event) {
            GM_setValue("上次视频", this.location.href);
            if (手机浏览器 && 阻止弹出新页面 === true) {
    ...
    function 点击下一集(video) {
        //video.pause();
        localStorage.setItem('恢复播放', true)
