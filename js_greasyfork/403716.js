...
/*
  修改说明：
    1. 修改 tab 形式所挂载的 <iframe> 元素节点
    2. 将 toNode 生成的父节点修改成默认总是为 <div> 元素
*/
...
function GM_configInit(config, args) {
  ...
  if (args.length == 1 && typeof args[0].id == 'string' && typeof args[0].appendChild != 'function') var settings = args[0];
  else {
...
GM_configStruct.prototype = {
    ...
    switch (arguments.length) {
      ...
      default:
        ...
        for (var b in B) {
          ...
          else if (',style,accesskey,id,name,src,href,which,for'.indexOf(',' + b.toLowerCase()) != -1) A.setAttribute(b, B[b]);
  ...
  toTabs: function () {
    // 转为 tab 的形式
    var body = this.frame.tagName == 'IFRAME' ? this.frame.contentWindow.document : this.frame.ownerDocument,
      configId = this.id;
    var $ = function (id) {
...
GM_configField.prototype = {
  ...
  toNode: function (configId, lastParentNode) {
    ...
    var retNode = create('div', { className: 'config_var', id: configId + '_' + id + '_var', title: field.title }),
      firstProp;
  ...
  reset: function () {
    ...
    switch (type) {
      case 'checkbox':
        ...
        break;
      case 'select':
        for (var i = 0, len = node.options.length; i < len; ++i) if (node.options[i].value == this['default']) node.selectedIndex = i;
        break;
      case 'radio':
        var radios = node.getElementsByTagName('input');
