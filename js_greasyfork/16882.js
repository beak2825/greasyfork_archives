// ==UserScript==
// @name           AutoPagerize modified by godblessyou
...
// @run-at         document-end
// @downloadURL https://update.greasyfork.org/scripts/16882/AutoPagerize%20modified%20by%20godblessyou.user.js
// @updateURL https://update.greasyfork.org/scripts/16882/AutoPagerize%20modified%20by%20godblessyou.meta.js
// ==/UserScript==
//
...
/**
reference : https://github.com/swdyh/autopagerize
/**/
if (typeof(window.AutoPagerize) == 'undefined') {
    ...
    SITEINFO = typeof SITEINFO != 'undefined' ? SITEINFO : [
        /* sample
    {
    ...
    */
    ...
    AutoPager = function(info) {
        this.pageNum = 1;
        this.info = info;
        this.state = AUTO_START ? 'enable' : 'disable';
        var self = this;
        debug('AutoPager() > getNextURL');
        ...
        // debug('isChromeExtension()', isChromeExtension());
        // debug('isSafariExtension()', isSafariExtension());
        // debug('isGreasemonkey()', isGreasemonkey());
        // debug('isJetpack()', isJetpack());
        // debug('gmCompatible()', gmCompatible());
        if (isFirefoxExtension()) {
            debug('isFirefoxExtension()');
            ...
            var u = settings['extension_path'] ? settings['extension_path'] + 'loading.html' : 'http://autopagerize.net/files/loading.html';
            this.messageFrame.src = u;
            document.body.appendChild(frame);
            if (isSafariExtension()) {
                safari.self.tab.dispatchMessage('launched', {
                    url: location.href
                });
    ...
    AutoPager.prototype.initHelp = function() {
        ...
        helpDiv.setAttribute('style', 'padding:5px;position:fixed;' + 'top:-200px;right:3px;font-size:10px;' + 'background:#fff;color:#000;border:1px solid #ccc;' + 'z-index:99999999;text-align:left;font-weight:normal;' + 'line-height:120%;font-family:verdana;');
        ...
        for (var i in COLOR) {
            s += '<div style="float:left;width:1em;height:1em;' + 'margin:0 3px;background-color:' + COLOR[i] + ';' + '"></div><div style="margin:0 3px">' + i + '</div>';
        }
        s += '</div>';
    ...
    AutoPager.prototype.updateIcon = function(state) {
        ...
        if (color) {
            if (isFirefoxExtension()) {
                ...
                chrome.extension.connect({
                    name: "pageActionChannel"
                }).postMessage(color);
            } else if (isSafariExtension() || isJetpack()) {} else {
    ...
    AutoPager.prototype.request = function() {
        ...
        AutoPager.requestFilters.forEach(function(i) {
            i(opt);
        }, this);
        if (opt.stop) {
    ...
    AutoPager.prototype.requestLoad = function(res) {
        ...
        //debug('AutoPager.filters', AutoPager.filters);
        AutoPager.filters.forEach(function(i) {
            i(page);
        });
        //debug('filters executed');
        this.requestURL = url;
    ...
    AutoPager.prototype.getNextURL = function(xpath, doc, url) {
        ...
        debug('getNextURL @param xpath', xpath);
        debug('getNextURL @param doc', doc);
        debug('getNextURL @param url', url);
        debug('getNextURL nextLink', nextLink);
        if (nextLink) {
            var findUrlFromOnclickAttr = function(nextLinkEl){
                var nextLinkOnclickAttr = nextLinkEl.getAttribute('onclick');

                if(nextLinkOnclickAttr){
                    if(nextLinkOnclickAttr.indexOf('location.href') > -1){
                        var statement1 = /='(.[^']+)'/g.exec(nextLinkOnclickAttr);
                        if(statement1.length > 1){
                            return statement1[1];
                        }
                        var statement2 = /="(.[^"]+)"/g.exec(nextLinkOnclickAttr);
                        if(statement2.length > 1){
                            return statement2[1];
                        }
                    }
                }
            ...
            var nextValue = nextLink.getAttribute('href') || nextLink.getAttribute('action') || findUrlFromOnclickAttr(nextLink) || nextLink.value;
            if (nextValue.match(/^http(s)?:/)) {
                var result = nextValue;
                //debug('getNextURL result1', result);
            } else {
                var base = getFirstElementByXPath('//base[@href]', doc);
                //debug('getNextURL base', base);
                //debug('getNextURL nextValue', nextValue);
                //debug('getNextURL (base ? base.href : url)', (base ? base.href : url));
                var result = resolvePath(nextValue, (base ? base.href : url), '✮');
                //debug('getNextURL result2', result);
            }
            return result;
        }
    };
    AutoPager.prototype.terminate = function() {
        console.info('AutoPager.prototype.terminate');
    ...
    AutoPager.prototype.error = function() {
        console.info('AutoPager.prototype.error');
        this.updateIcon('error');
        window.removeEventListener('scroll', this.scroll, false);
        if (isSafariExtension() || isChromeExtension() || isJetpack()) {
            var mf = this.messageFrame;
            var u = settings['extension_path'] ? settings['extension_path'] + 'error.html' : 'http://autopagerize.net/files/error.html';
            mf.src = u;
    ...
    if(typeof AutoPagerFilters !== 'undefined'){
        debug('AutoPagerFilters2: ', AutoPagerFilters);
        AutoPager.filters = AutoPager.filters.concat(AutoPagerFilters);
    }
    debug('AutoPager.filters', AutoPager.filters);
    ...
    var launchAutoPager = function(list) {
        ...
        for (var i = 0; i < list.length; i++) {
            try {
                if (ap) {
                    return;
                } else if (!location.href.match(list[i].url)) {} else if (!getFirstElementByXPath(list[i].nextLink)) {
    ...
    var getCacheCallback = function(res, url) {
        ...
        var info;
        try {
            info = JSON.parse(res.responseText).map(function(i) {
                return i.data;
            });
        } catch (e) {
            info = [];
        ...
        if (!location.href.match(re_exclude)) {
            safari.self.addEventListener('message', function(event) {
                if (event.name === 'settings') {
                    ...
                    safari.self.tab.dispatchMessage('siteinfoChannel', {
                        url: location.href
                    });
    ...
    function getFirstElementByXPath(xpath, node) {
        ...
        //debug('getFirstElementByXPath', result.singleNodeValue);
    ...
    console.info('"debugAutoPager = true;" is AutoPager debug.');
    function debug() {
        if (typeof debugAutoPager !== 'undefined' && debugAutoPager) {
    ...
    function resolvePath(path, base) {
        // debug('resolvePath @param path', path);
        // debug('resolvePath @param base', base);
        if (path.match(/^https?:\/\//)) {
            debug('resolvePath statement 1');
            var result = path;
        }
        if (path.match(/^\?/)) {
            //debug('resolvePath statement 2');
            var result = base.replace(/\?.+$/, '') + path;
        } else if (path.match(/^[^\/]/)) {
            //debug('resolvePath statement 3');
            var result = base.replace(/[^/]+$/, '') + path;
        } else {
            //debug('resolvePath statement 4');
            var result = base.replace(/([^/]+:\/\/[^/]+)\/.*/, '\$1') + path;
        }
        //debug('resolvePath', result);
        return result;
    }

    function fixResolvePath() {
        //debug('fixResolvePath');
        ...
        resolvePath = function resolvePath_workaround(path, base) {
            debug('resolvePath_workaround path:', path);
            debug('resolvePath_workaround base:', base);
            baseElement.setAttributeNS(XML_NS, 'xml:base', base);
            pathElement.setAttributeNS(XML_NS, 'xml:base', path);
            debug('resolvePath_workaround return value:', pathElement.baseURI);
    ...
    function isFirefoxExtension() {
        var result = (typeof chlorine == 'object');
        //debug('isFirefoxExtension', result);
        return result;
    }

    function isChromeExtension() {
        var result = (typeof chrome == 'object') && (typeof chrome.extension == 'object');
        //debug('isChromeExtension', result);
        return result;
    }

    function isSafariExtension() {
        var result = (typeof safari == 'object') && (typeof safari.extension == 'object');
        //debug('isSafariExtension', result);
        return result;
    }

    function isGreasemonkey() {
        var result = (typeof GM_log == 'function');
        //debug('isGreasemonkey', result);
        return result;
    }

    function isJetpack() {
        // isFirefoxExtension is obsolete
        var result = (!isGreasemonkey() && !isSafariExtension() && !isChromeExtension() && !isFirefoxExtension());
        //debug('isJetpack', result);
        return result;
    }
