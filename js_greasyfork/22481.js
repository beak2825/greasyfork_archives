// ==UserScript==
// @name         WME DOT Roads Layer (beta)
// @namespace    https://greasyfork.org/users/45389
// @version      0.1.b6
// @description  Adds a layer to display roads from DOT ArcGIS servers.
// @author       MapOMatic
// @include      /^https:\/\/(www|beta)\.waze\.com\/(?!user\/)(.{2,6}\/)?editor\/?.*$/
// @license      GNU GPLv3
// @grant        GM_xmlhttpRequest
// @connect      ky.gov
// @downloadURL https://update.greasyfork.org/scripts/22481/WME%20DOT%20Roads%20Layer%20%28beta%29.user.js
// @updateURL https://update.greasyfork.org/scripts/22481/WME%20DOT%20Roads%20Layer%20%28beta%29.meta.js
// ==/UserScript==

(function() {
    'use strict';

    var _settingsStoreName = 'wme_dot_roads_layer';
    ...
    var _stateSettings = {
        KY: {
            baseUrl: 'http://maps.kytc.ky.gov/arcgis/rest/services/Apps/GeneralHighwayRoads/MapServer/',
            mapLayers: [
                { layerID:68, type:'local', outFields:['*']}
            ],
            colors: {PR:'#00ffff',PS:'#00ffff',PV:'#ff0000'}
        }
    };
    ...
    function loadSettingsFromStorage() {
        ...
        var defaultActiveStateAbbr = 'IN';
        if(!settings) {
            settings = {
                lastVersion:null,
                layerVisible:true,
                //activeStateAbbr:defaultActiveStateAbbr
            };
        } else {
            settings.layerVisible = (settings.layerVisible === true);
            //settings.activeStateAbbr = (settings.activeStateAbbr ? settings.activeStateAbbr : defaultActiveStateAbbr);
        }
        _settings = settings;
    }
    ...
    function saveSettingsToStorage() {
        if (localStorage) {
            var settings = {
                ...
                //activeStateAbbr: _settings.activeStateAbbr
            };
    ...
    function processFeatures(data,context) {
        ...
        data.features.forEach(function(feature) {
            //var featureFC = stateSettings.getFeatureFC(feature,layer);
            //if(!(stateSettings.skipFeature && stateSettings.skipFeature(feature)) && (featureFC <= context.maxFC))  {
            //console.log(feature.attributes);
            var style = styles[feature.attributes.RT_PREFIX];
            var lineFeatures = [];
    ...
    function fetchFeatures() {
        ...
        visibleStates.forEach(function(stateAbbr) {
            ...
            if (state) {
                //var maxFC = state.zoomSettings.maxFC[zoom];
                //var maxOffset = state.zoomSettings.maxOffset[zoom];
                for(var i=0; i<7; i++) {buckets.push([]);}
                state.mapLayers.forEach(function(layer) {
                    if (!layer.zoomLevels || layer.zoomLevels.indexOf(zoom) !== -1) {
                        //var startingFC = state.fetchAllFC ? 1 : maxFC;
                        //for(var fc=7; fc>0; fc--) {
                        var context = {
                            stateSettings: state,
                            //maxFC: maxFC,
                            layer: layer,
                        ...
                        //if (state.fetchAllFC) {
                        url += 'where=RT_PREFIX%3D%27PR%27+OR+RT_PREFIX%3D%27PV%27+OR+RT_PREFIX%3D%27PS%27';
                        //} else {
                        //    url += 'where=' + layer.fcPropName + '%3D' + (fc) + (state.getWhereClause ? state.getWhereClause(context) : '');
                        //}
    ...
    function initLayer(){
        ...
        _mapLayer = new OpenLayers.Layer.Vector("DOT Roads ", {
