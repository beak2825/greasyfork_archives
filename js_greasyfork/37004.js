...
// @downloadURL https://update.greasyfork.org/scripts/37004/WebAssign%20Genius%20Userscript.user.js
// @updateURL https://update.greasyfork.org/scripts/37004/WebAssign%20Genius%20Userscript.meta.js
// ==/UserScript==
//doesn't work rn 

var y = document.getElementById('webAssign');
var z = String(y);
console.log(localStorage.setItem(y,z));

var x = 'WA'
localStorage.setItem(x,'lol');

while ((localStorage.getItem(y)).includes(localStorage.getItem(x)) !== true) {    
    var x = prompt('What is the Passcode?').toUpperCase();
    var n = x.replace('GENIUS', '');
    console.log(localStorage.setItem(x, n));
    localStorage.getItem(x);
    if ((sessionStorage.getItem(y)).includes(localStorage.getItem(x)) === true) {break}
    else if (z.includes(n) !== true) { alert('password was incorrect')}
} 
