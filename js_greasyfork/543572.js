...
// @downloadURL https://update.greasyfork.org/scripts/543572/Applovin%20Ads%20UTM.user.js
// @updateURL https://update.greasyfork.org/scripts/543572/Applovin%20Ads%20UTM.meta.js
// ==/UserScript==
...
(function() {
    ...
    let act = util.getUrlParam('accountId') || ""
    console.log('act',act);
    ...
    function track(){
        tracking = setInterval(async function(){
            ...
            //日期处理
            var frompicker = $(dateElementClass).find('input').eq(0).val();
            var fromdate = moment(frompicker);

            var topicker = $(dateElementClass).find('input').eq(1).val();
            ...
            //Tab处理=
            let campaignId = util.getUrlParam('campaignId') || ""
            if(campaignId){
                type = "ads"
            }else{
                type = "campaigns"
            ...
            if(type && from && siteId){
                ...
                let arr = []
                let titleobj = null;
                titleobj = $('.arco-table-content-inner').find('thead').find('th');
                console.log('titleobj',titleobj);

                titleobj.each(function(i){
                    let position = $(this).find('span.arco-table-th-item-title').html() || "";
                ...
                console.log('arr',arr);
                if((!arr['campaign_id'] && arr['campaign_id']!=0 && !arr['ad_id'] && arr['ad_id']!=0) || (!arr['results'] && arr['results']!=0) || (!arr['amount_spent'] && arr['amount_spent']!=0) || (!arr['p_roas'] && arr['p_roas']!=0)){
                    return util.message.error('Add columns for "ID, Spend, D0 Checkouts, D0 ROAS" ');
                }

                let col = "";
                let allqty=0,alltotal=0;

                if($(".arco-table-content-inner").length>0){
                    $(".arco-table-content-inner tbody tr").each(function(i){
                        //根据tab类型获取判断依据
                        switch(type){
                            case "ads":
                                col = arr['ad_id'];
...
//                         markname = $('<textarea/>').html(markname).text()
                        ...
                        let markidObj = rowObj.find('.arco-table-td').eq(col).find('span.arco-ellipsis-text')
                        ...
                        $.map( utmdata, function( val, index ) {
                            ...
                            switch(type){
                                ...
                                case "campaigns":
                                    ...
                                    //datamark = decodeURIComponent(val.campaign_name)
                                    datamark = val.campaignId
                                    break;
                                default:
                                    break;
                            }
                            ...
                            if(markid === datamark){
                                ...
                                let spentAmount = Number(spentObj.html().replace(/[$,]/g, '')); //订单总额除广告花费
                                let roas = spentAmount>0?((val.total/spentAmount*100).toFixed(2)):"";
                                proasObj.find('span.showbg').length>0?proasObj.find('span').html(roas+"%"):(roas>0?proasObj.prepend("<span class='showbg roascol'>"+roas+"%</span>"):"")
                            }
                        });
                    })
                }
