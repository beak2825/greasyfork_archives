// ==UserScript==
// @name         Bugtracker extension
// @namespace    http://tampermonkey.net/
// @version      1.0.1
// @description  Парсинг балов за баги и подсчет в разшерении Supertest Assistant
// @author       wandersher
// @match        https://bugs-st.worldoftanks.ru/projects/rmdev/*
// @icon         data:image/png;base64,
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/430758/Bugtracker%20extension.user.js
// @updateURL https://update.greasyfork.org/scripts/430758/Bugtracker%20extension.meta.js
// ==/UserScript==


function getItem(key) {
	return new Promise((resolve) => chrome.runtime.sendMessage("mpjhblapicilagkfkldbjcilghbnmpgj", { action: 'get', key }, resolve))
}

function setItem(key, data) {
	return new Promise((resolve) => chrome.runtime.sendMessage("mpjhblapicilagkfkldbjcilghbnmpgj", { action: 'set', key, data }, resolve))
}


(async function() {
    'use strict';
    const style = document.createElement("style");
	style.innerText = "" +
		".odd.status-9,.odd.status-11,.odd.status-12,.odd.status-22{ background: #ebfcf6 !important; }" +
        ".even.status-9,.even.status-11,even.status-12,.even.status-22{ background: #e8e8e8 !important; }" +
        ".odd.status-9 td,.odd.status-11 td,.odd.status-12 td,.odd.status-22 td{ border-color: #e6f4eb !important; }" +
        ".even.status-9 td,.even.status-11 td,.even.status-12 td,.even.status-22 td{ border-color: #e6f4eb !important; }" +
	document.head.appendChild(style);


    const statuses = ["Подтверждён", "Заморожен", "Открыт", "Обрабатывается"]; // 3-5 points
	const closed = ["Не воспроизводится", "Не будет правиться"]; // 1 point
	const priorities = [
		{priority: "Very High", points: 5},
		{priority: "High", points: 5},
		{priority: "Medium", points: 3},
		{priority: "Low", points: 3},
		{priority: "Postponed", points: 3}
	];

	let data = await getItem("bugtracker")

	Array.from(document.querySelectorAll('.autoscroll tbody tr')).map(row => {
		const matches = new RegExp('^<tr.*?<td class="id"><a.*?>(.*?)<.*?class="status">(.*?)<.*?class="priority">(.*?)<.*?class="author"><a.*?>(.*?)<.*?"updated_on">(.*?)<.*?</tr>', 'gs').exec(row.outerHTML);
		if (matches) {
			const exist = data.find(it => it.id === matches[1]);
			let [original, day, month, year, hour, minutes] = new RegExp('^(..).(..).(....) (..):(..)$').exec(matches[5].trim());
			const date = new Date(parseInt(year), parseInt(month) - 1, parseInt(day), parseInt(hour), parseInt(minutes)).getTime()
			if (!exist) {
				data.push({
					id: matches[1],
					status: matches[2],
					priority: matches[3],
					name: matches[4],
					points: closed.find(it=> it.includes(matches[2])) ? 1 : priorities.find(it => it.priority === matches[3]).points,
					date
				})
			} else {
				exist.preority = matches[3]
				exist.status = matches[2]
				exist.points = closed.find(it=> it.includes(matches[2])) ? 1 : priorities.find(it => it.priority === matches[3]).points
				exist.date = date
			}
		}
	})
    await setItem("bugtracker", data.filter(it => statuses.some(status => it.status.includes(status))))
})();