...
// @grant        GM_getValue
// @connect      weibo.com
// @require https://update.greasyfork.org/scripts/435697/1519276/mylib.js
...
// noframes
// @version 0.0.1.20250124154427
// @namespace http://tampermonkey.net/
// @description .js
// @downloadURL https://update.greasyfork.org/scripts/439400/%E2%9D%97Menu.user.js
// @updateURL https://update.greasyfork.org/scripts/439400/%E2%9D%97Menu.meta.js
// ==/UserScript==
const gsv=GM_setValue,ggv=GM_getValue,gxhr=GM.xmlHttpRequest,gsc=GM.setClipboard,win=unsafeWindow;//右击:拽-切换固定;顶-找到菜单;底-切换编辑
if(top===self) main(win);
else {//iframe
    if(location.protocol.includes('http')){
        ...
        win.myaddas(location.href);
        win.mywaitele('video',5000).then(v => {//视频找到
            v.pause();
            const rate=win.mybtns.myappend(`input`,'','placeholder','倍速','value',ggv('pbRate',1)).ael("click",e=>{
            ...
            document.addEventListener('keydown',function (event) {//快进20s
                if (event.key === ']')v.currentTime+=20;
                else if (event.key === '[')v.currentTime-=20;
            });
            win.mysleep(1000).then(()=>{
                v.play();
            });
        }).catch(err => {
            // al(document.URL+err)
...
function main(win){// 防瞎改标题
    const title0=document.title,host=location.host,href=location.href,path=location.pathname,body=document.body;
    if(!host.includes('mp.weixin')) Object.defineProperty(document,'title',{set:newTitle=>title0,get:newTitle=>title0});
  ...
  height:1em;
    ...
    const z=win.myz,
          ul=z.myappend(`my-ul`,'',`style`,'display:none',`className`,'rwf').ael("click",()=>{ul.style.display='none'}),
          ...
          htm=nav1.myappend(`li`,'htm',`className`,"my-li").ael("click",()=>{
              const win=open('',"_blank");
              win.document.write(tarea.value);
              win.document.close();
          }),events=['mouseup','mousedown'],
          share=nav1.myappend(`li`,'share',`className`,"my-li").ael("click",()=>{//必须登录
              let memo=prompt('解说/分隔符:',''),tv=tarea.value;
              if(memo!==null){
                  if(memo.length>1){
                      memo='【'+memo+'】';
                  }else{//分隔符
                      // href=divideStr(href,memo);
                      tv=divideStr(tv,memo);//含1字符和0字符
                  }
              }else return;//如果取消
              if(tv){
                  tv='「❗'+tv.replaceAll(/\n+/g,"\n")+'」';
              ...
              gxhr({
                  ...
                  },data:"text="+encodeURIComponent(`${href}\n#${document.title}#\n${memo}${tv}\n——————shared by GM.xmlHttpRequestค้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้`),
          ...
          save=nav2.myappend(`li`,'save',`className`,"my-li").ael("click",function savepp(){//保存页面part
              const fileName=title0?decodeURIComponent(title0+'&t='+new Date().getTime()+'.htm'):decodeURIComponent(path.replace(/\//g,'')+'&t='+new Date().toJSON()+'.htm'),
                    finalName=prompt('\n这是下载的文件名:',fileName);//4、download
              if(finalName){
                  //1、remove useless tags
                  win.dqa("script,meta,.rwf").forEach(e=>{e.remove()})
                  //移除所有元素的dom0事件
                  removeOn(win.dqa('*'),'on')
                  //2、replace relative path to absolute
                  removeRelativePaths();
                  if(body.hasAttribute("contentEditable")) body.removeAttribute("contentEditable");//千古神坑:必须关闭编辑状态!!!
                  //添加url
                  const h=decodeURIComponent(href);
                  body.myappend('a',h,'href',h,'style','z-index:0;position:fixed;bottom:0;left:0;opacity:0.7;width:100%')
                  //3、get htm
                  const htm=document.documentElement.innerHTML,
                        a=body.myafter('a','','href','data:text/htm;charset=utf-8,\ufeff'+encodeURIComponent(htm) ,'download',finalName,'style','visibility:hidden');
                  a.click();
              }else{
                  return/*win.gsc(htm)&n({text:'copy成功！'})*/;
              }
          }),saveAll=nav2.myappend(`li`,'saveAll',`className`,"my-li").ael("click",function savepa(){//保存页面all
              const fileName=title0?decodeURIComponent(title0+'&t='+new Date().getTime()+'.htm'):decodeURIComponent(path.replace(/\//g,'')+'&t='+new Date().toJSON()+'.htm'),
                    finalName=prompt('\n这是下载的文件名:',fileName);//4、download
              if(finalName){
              ...
              const t=tarea.value||prompt('内容:',''),char=prompt('分隔符:','');
              win.gsc(divideStr(t,char));
          }),
          rmv隐藏域=nav1.myappend(`li`,'rmv隐藏域',`className`,"my-li").ael("click",()=>{
              win.dqa('input[type="hidden"]').forEach(e=>{e.type=''});
          }),no换行=nav2.myappend(`li`,'no换行',`className`,"my-li").ael("click",()=>{// 将两个以上<br>换为一个
              const newHtmlContent = doc.documentElement.innerHTML.replace(/(<br\s*\/?>)\s*(<br\s*\/?>)+/gi, '<br>'),nwin = win.open('', '_self');
              nwin.document.write(newHtmlContent);nwin.document.close();
          }),
    ...
    document.addEventListener("mouseup",ev=>{
        if(!ul.contains(ev.target)){//先把不连续出现的\r\n去除,然后把连续出现的\r\n统一换为一个\r\n
            let t=document.getSelection().toString().replace(/(?<!\r\n)\r\n(?!\r\n)/g, '').replace(/(\r\n)+/g, '\r\n');
            if (t){
                tarea.value=t;
    ...
    const btns=win.myaddbtns(
        function 顶(){
            ...
            if(winy0===winy1) document.body.scrollTo(0,0);
        },
        function 底(){
            const winy0=win.scrollTop;
            win.scrollTo(0,document.body.scrollHeight);
            const winy1=win.scrollTop;
            if(winy0===winy1) document.body.scrollTo(0,document.body.scrollHeight);
        },
        function X(){win.myz.remove()}
    );
    btns.children[1].oncontextmenu=e=>{//顶 右键召唤菜单
        e.preventDefault();//取消默认右键
        ul.style=`position:fixed;left:50%;top:0;display:block`
    };
    btns.children[2].oncontextmenu=e=>{//底 turnEdit
        e.preventDefault();
    ...
    document.addEventListener('mouseup',e=>{//实现按住alt键在当前页打开a标签
        if(e.altKey){
            e.target.dispatchEvent(new CustomEvent('altdown',{
                bubbles:false,
                cancelable:!0
            }));
            e.stopImmediatePropagation();//停止分发
        }
    },!0);
    document.addEventListener('altdown',e=>{
        // JSON.stringify(1);//锁定线程,效果同alert但不弹窗
        let a= e.target,ahref=a.href;
...
function divideStr(str,char){//以特定字符分隔字符串
    return Array.from(str).join(char);
}
