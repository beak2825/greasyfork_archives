...
;const ImageDownloader = (({ JSZip, saveAs }) => {
  ...
  async function download(getImagePromises, title, imageSuffix, zipOptions) {
    ...
    const zip = new JSZip();
    let newtitle0 = title.replace(/(\?|\~|\/|\:)/gi,  function ($0, $1) {
        return {
            '?':'？',
            '~':'～',
            '/':'／',
            ...
            }).replace(/[Ａ-Ｚａ-ｚ０-９]/g, c => String.fromCharCode(c.charCodeAt(0) - 65248)).replace(/\u3000/g, " ").replace(/\s+/g, " ").trim(); // 替换全角英文数字空格为半角，合并连续空白字符，删除首尾空白字符
    const newtitle = newtitle0.replace(/"([^"]*)"/g, '“$1”'); // 半角双引号替换为全角双引号
    const zipTitle = newtitle.replaceAll(/\\|\*|\<|\>|\|/g, '_'); // remove some characters
