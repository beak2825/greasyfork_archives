// ==UserScript==
// @name         01風(20180904)
// @namespace    http://swordman-s1.yytou.com/?id=4278980&time=1512920545299&key=53d9dd1b6161f11b3be4769dfcd1aaf9&s_line=1
// @version      2.1
// @description  try to take over the world!
// @author       MARBO&jj
// @match        http://swordman-s1.yytou.com/?id=4278980&time=1512920545299&key=53d9dd1b6161f11b3be4769dfcd1aaf9&s_line=1
// @grant        note
// @downloadURL https://update.greasyfork.org/scripts/372754/01%E9%A2%A8%2820180904%29.user.js
// @updateURL https://update.greasyfork.org/scripts/372754/01%E9%A2%A8%2820180904%29.meta.js
// ==/UserScript==
...
var playerName = '千門風';
var playerMaxHp = '505699';
var enforcePoints = '1561';
var mySkillLists = '孔雀翎,天火飛錘';
var mySkillLists1 = '破軍棍訣';
skillLists = '';
buttonWidth = '80px';
buttonHeight = '20px';
currentPos = 35;
...
//	   BiShi2Button.style.visibility="visible";//切小號
	   ...
	   pozhaoInButton1.style.visibility="visible";// 破招棍
...
/*  clickButton('clan buy 601');//空識卷軸
    ...
    clickButton('enable liuh-ken', 1);//柳家拳法
    clickButton('practice liuh-ken', 1);
    clickButton('enable mo-bian-jue', 1);//魔鞭訣
    clickButton('practice mo-bian-jue', 1);
    clickButton('enable snowwhip', 1);//寒雪鞭法
    clickButton('practice snowwhip', 1);
    clickButton('enable tenderzhi', 1);//柔虹指法
    clickButton('practice tenderzhi', 1);
    r3();//技能準備
   ...
   var bm = 0,
    ...
    function QiXiaTalkFunc() {
        // if (userqu == 15 || userqu == 47 || userqu == 51 || userqu == 56 || userqu == 57 || userqu == 64 || userqu == 66 || userqu == 68) {
            var a = prompt("請輸入撩奇俠順序（中文分號隔開）", "逆風舞；郭濟；火雲邪神；浪喚雨；李玄霸；玄月研；狼居胥；烈九州；穆妙羽；八部龍將；風無痕；厲滄若；妙無心；巫夜姬");
            qxList = a.split("；");
            qxOrder = 0;
...
function autoBattleFunc(){
    ...
    if(!skillstr){
        //skillstr = prompt("請輸入要釋放的技能,以英文逗號分隔,前兩種武功為攻擊技能,第3個參數為釋放時需要的氣數,第4個參數為加血技能:","破軍棍訣,孔雀翎,8,道種心魔經");
        var skillstr = '破軍棍訣,孔雀翎,8,道種心魔經';
...
//				    var aaa = ['悟性裝','寶箱3000','通靈','上香','打鳥','冰月谷','突破丹禮包','狂暴丹','掌門手諭','投資','煉藥','拼圖','簽到','禦寒衣','刻刀'];
				...
				if(qmjname2=='準備銅人'){
                    ...
                    clickButton('enableskill enable throwing baoyu-lihua', 1);
                ...
                if(qmjname2=='全任務'){
                    ...
                    for (var i=0; i<10; i++){clickButton('vip finish_bad 1', 0);}//正氣5次
                    for (var i=0; i<10; i++){clickButton('vip finish_bad 2', 0);}//邪氣5次
...
function r2(){//青龍技能
    ...
    clickButton('enable unmap_all', 1);//一鍵取消
    clickButton('enableskill enable unarmed liumai-shenjian', 1);//六脈神劍
    clickButton('enableskill enable unarmed anran-zhang', 1);//黯然銷魂掌
    clickButton('enableskill enable sword binggong-jianfa', 1);//織冰劍法
    clickButton('enableskill enable sword jiutian-sword', 1);//九天龍吟劍法
    clickButton('enableskill enable jiutian-sword attack_select', 1);//九天龍吟劍法必刷
    clickButton('enableskill enable blade tulong-blade', 1);//屠龍刀法
    clickButton('enableskill enable blade jiuyin-blade', 1);//九陰噬骨刀
    clickButton('enableskill enable stick shshisigun', 1);//霜寒十四棍
    clickButton('enableskill enable stick douzhangundian', 1);//鬥戰棍典
    clickButton('enableskill enable staff lingshe-zhangfa', 1);//靈蛇杖法
    clickButton('enableskill enable staff duanjia-sword', 1);//段家劍
    clickButton('enableskill enable whip kunxianbianfa', 1);//捆仙鞭法
    clickButton('enableskill enable whip huimengwuheng', 1);//無歸鞭法
    clickButton('enableskill enable throwing kongqueling', 1);//孔雀翎
    clickButton('enableskill enable throwing feidao', 1);//飛刀絕技
    clickButton('enableskill enable kongqueling attack_select', 1);//孔雀翎必刷
    ...
    clickButton('enableskill enable parry shiyangjian', 1);//十陽滅冥箭
    clickButton('enableskill enable parry baoyu-lihua', 1);//暴雨梨花針
    clickButton('golook_room');
}
function r3(){//技能準備
...
function r5(){//師門幫派
    ...
    for (var i=0; i<20; i++){clickButton('vip finish_clan', 0);}  //幫派20次
    clickButton('golook_room');
}
function r4(){//正常裝
            clickButton('auto_equip on', 0);
            go("unwield sword of windspring;wield weapon_sb_hammer11;wield weapon_sb_throwing11;wear equip_moke_finger10;wear equip_moke_head10");
}
       function s1(){//悟性裝
...
gSocketMsg.addMessage2 = function(cc) {
        ...
        for (this._msg += cc; 0 < this._msg.length; ) {
            ...
            if(this._msg_line.indexOf("紫微貪狼陣升級完畢！") != -1){clickButton('hhjz xiulian go', 1);}
            if(this._msg_line.indexOf("你從寒玉床上爬起，結束了這次練功。") != -1){clickButton('golook_room');clickButton('sleep_hanyuchuang', 0);}
            if(this._msg_line.indexOf("你的碧海潮生劍成功向前突破了，請繼續努力！") != -1){tupostart();}
            if(this._msg_line.indexOf("你的葵花寶典成功向前突破了，請繼續努力！") != -1){tupostart();}
            if(this._msg_line.indexOf("你的帝王劍法成功向前突破了，請繼續努力！") != -1){tupostart();}
            if(this._msg_line.indexOf("你的獨孤九劍成功向前突破了，請繼續努力！") != -1){tupostart();}
            if(this._msg_line.indexOf("你的鬥戰棍典 成功向前突破了，請繼續努力！") != -1){tupostart();}
            if(this._msg_line.indexOf("你的鬥戰棍典成功向前突破了，請繼續努力！") != -1){tupostart();}
            0 < this._msg_line.length && this.recvNetWork(this._msg_line);
            this._msg_len = this._msg_start = 0
        }
    }
                    function tupostart(){
                    clickButton('enable dugu-jiujian', 1);//獨孤九劍 6/9->10
                    clickButton('tupo go,dugu-jiujian', 1);
                    clickButton('tupo_speedup dugu-jiujian go', 0);

                    clickButton('enable thfc', 1);//天火飛錘 15/9->40
                    clickButton('tupo go,thfc', 1);
                    clickButton('tupo_speedup2 thfc go', 0);

                    clickButton('enable king-sword', 1);//帝王劍法 16/9->10
                    clickButton('tupo go,king-sword', 1);
                    clickButton('tupo_speedup king-sword go', 0);

                    clickButton('enable pjgj', 1);//破軍棍訣 5/10->20
                    clickButton('tupo go,pjgj', 1);
                    clickButton('tupo_speedup2 pjgj go', 0);

                    clickButton('enable kuihua-shengong', 1);//葵花寶典 24/9->8
                    clickButton('tupo go,kuihua-shengong', 1);
                    clickButton('tupo_speedup kuihua-shengong go', 0);

                    clickButton('enable bihai-sword', 1);//碧海潮生劍 2/10->8
                    clickButton('tupo go,bihai-sword', 1);
                    clickButton('tupo_speedup bihai-sword go', 0);
