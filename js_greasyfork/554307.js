...
// @compatible        safari
// @compatible        kiwi
// @version           2025.04.27.1400
// @created           2025-03-07 21:14:34
// @modified          2025-03-07 21:14:34
// @downloadURL https://update.greasyfork.org/scripts/554307/MissAV%20Enhanced%20Assistant.user.js
// @updateURL https://update.greasyfork.org/scripts/554307/MissAV%20Enhanced%20Assistant.meta.js
// ==/UserScript==
/**
...
(function () {
    ...
    const videoSettings = {
        ...
        // 后台禁止自动暂停模式
        autoPauseDisable: 0, // 0:默认模式(浏览器同一组标签,播放页面切换到另外标签暂停播放), 1:禁止所有暂停播放,切换到同一组标签不暂停播放
        // 自动静音播放
        autoMutePlay: false
    };
    let plyrInstance = null; // 确保 plyrInstance 在函数外部定义
    let skipTime = parseFloat(localStorage.getItem('missavSkipTime')) || 5; // 默认快进时间
    ...
    let controlPanel;
    let showControlsButton = null; // 显示控制面板按钮
    ...
    var handle = () => {
        ...
        if (videoSettings.playCtrlEnable) {
            ...
            div.innerHTML = '<button id="btnControl" onclick="video.scrollIntoView();" type="button" class="relative inline-flex items-center rounded-md bg-transparent pl-2 pr-2 py-2 font-medium text-white hover:bg-primary focus:z-10" style="position: fixed; top: 60%; right: 10px; transform: translateY(-50%); z-index: 1000; opacity: 1; background-color: transparent; border: 1px solid white; border-radius: 8px;border: none;width: 40px; height: 40px;">🔁</button>'
        ...
        let player = document.querySelector('video.player')
         if (!player) {
            player = typeof unsafeWindow !== 'undefined' ? unsafeWindow.player : window.player;
        }
        plyrInstance = player;
        span.classList.value = 'isolate inline-flex rounded-md shadow-sm'
        span.style = ` 0 ${videoSettings.buttonMargin}`
        span.innerHTML = '<button id="btnPlay" onclick="player.togglePlay();" type="button" class="relative -ml-px inline-flex items-center rounded-md bg-transparent pl-2  py-2 font-medium text-white ring-1 ring-inset ring-white hover:bg-primary focus:z-10">' + videoSettings.htmlPlay + '</button>'
        ...
        const links = document.querySelectorAll('.space-y-2 > div:nth-child(4) a').forEach(link => {
            link.style.fontSize = '0.875rem'; // 文字压缩
            link.style.margin = '0.1rem 0';   // 间距压缩
        });

        if (bar && !bar.querySelector('.vjs-skip-15')) {

        // 创建按钮（复用原生样式体系）
            const btn15 = document.createElement('button');
            btn15.className = 'vjs-control vjs-button vjs-skip-15 px-2'; // 关键样式类
            btn15.title = '快进 15 分钟';
            btn15.innerHTML = '<span aria-hidden="true">15m</span>';

        // 点击逻辑（与你的 +5 min 按钮逻辑一致）
            btn15.onclick = () => {
                player.currentTime = Math.min(player.duration, player.currentTime + 1200);
            };

        // 插入定位逻辑（优先在你的 +5 min 按钮后插入）
            const existingButtons = bar.querySelectorAll('span.isolate'); // 通过你创建的 span 容器查找
    ...
    document.addEventListener('DOMContentLoaded', () => {
        // 在 DOMContentLoaded 中初始化 Control Panel
        createControlPanel();
        removeElements()
        toLink()
    })
    ...
    /**
     * 在初始化完成后创建 Control Panel 以及快进按钮.
     */
    function createControlPanel() {
        if (document.querySelector('.custom-control-panel')) return; // 避免重复创建
        ...
        controlPanel.append(
            createInputGroup('快进/退(秒):', 'number', skipTime, (e) => {
                ...
                localStorage.setItem('missavSkipTime', skipTime); // 保存到 localStorage
            }, 'skipTime', { min: 0.1, step: 0.1 })
        );

        // 快进按钮
        const skipButtonsContainer = document.createElement('div');
        skipButtonsContainer.className = 'button-group';
        skipButtonsContainer.style.marginTop = '5px';

        const createSkipButton = (text, skipValue) => {
            const button = document.createElement('button');
            button.textContent = text;
            button.onclick = () => {
                if (!plyrInstance || typeof plyrInstance.currentTime !== 'number') return;
                plyrInstance.currentTime += skipValue; // 使用当前 skipTime
            };
            return button;
        };

        skipButtonsContainer.append(
            createSkipButton('快进5秒', 5),
            createSkipButton('快进30秒', 30)
        );
        controlPanel.append(skipButtonsContainer);
        ...
        buttonContainer.append(
            ...
            createButton('隐藏面板', hideControlPanel)
        );
    ...
    function createShowControlsButton() {
        ...
        showControlsButton.textContent = '显示控制';
   ...
   function showControlPanel() {
    ...
    function saveSettings() {
        try {
            localStorage.setItem('missavSkipTime', skipTime); // 保存 skipTime
    ...
    const observer = new MutationObserver((mutationsList, obs) => {
        const playerElement = document.querySelector('video#player, video.plyr__video-wrapper video, .player-container video, .plyr--video');
