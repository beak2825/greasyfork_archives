...
// @homepage     https://github.com/bangumi/scripts/blob/master/vickscarlet/scripts/fast_search
// @match        *://bgm.tv/*
// @match        *://chii.in/*
// @match        *://bangumi.tv/*
// @grant        GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/559520/Bangumi%20%E5%BF%AB%E9%80%9F%E6%90%9C%E7%B4%A2.user.js
// @updateURL https://update.greasyfork.org/scripts/559520/Bangumi%20%E5%BF%AB%E9%80%9F%E6%90%9C%E7%B4%A2.meta.js
// ==/UserScript==
...
(function () {
  ...
  function toURLSearchParams(parameters, parameters2, boolValues = ["1", "0"]) {
  ...
  var RymkCommandEnum = ((RymkCommandEnum2) => {
    RymkCommandEnum2["user"] = "user";
    RymkCommandEnum2["group"] = "group";
    RymkCommandEnum2["topic"] = "topic";
    RymkCommandEnum2["reply"] = "reply";
    return RymkCommandEnum2;
  })(RymkCommandEnum || {});
  const SearchUserParametersSchema = {};
  const SearchTopicParametersSchema = {
  ...
  const SearchGroupParametersSchema = {
    sort: {
      type: "enum",
  ...
  const SearchReplyParametersSchema = {
    user: { type: "string", optional: true, description: "筛选用户", rename: "user" },
    group: { type: "string", optional: true, description: "筛选小组", rename: "group" },
  ...
  async function rymkFetch(catalog, api2, params, page = 1, limit = 20) {
    params.append("limit", limit.toString());
    if (page > 1) params.append("offset", (page - 1) * limit + "");
    const url = `${RYMK_API_BASE}/search/${api2}?${params.toString()}`;
    const res = await fetchJson(url);
    if (!res.success) return res;
    if ("detail" in res.data) {
      return {
        success: false,
        banner: jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
jsxRuntimeExports.jsx("h1", { children: "🤔Api出错" }),
jsxRuntimeExports.jsx("h3", { children: jsxRuntimeExports.jsx("code", { children: res.data.detail[0].msg }) })
        ] })
      };
    }
    ...
    return {
      success: true,
      data: {
        items: data.map((item) => ({ ...item, catalog })),
  ...
  const api$2 = {
    user: {
      description: "用户",
      search({ query: { keywords: q }, page, limit }) {
    ...
    topic: {
      description: "小组话题",
      kempty: true,
      search({ query: { keywords: q, parameters }, page, limit }) {
  ...
  var SubjectCommandEnum = ((SubjectCommandEnum2) => {
    SubjectCommandEnum2["subject"] = "subject";
    SubjectCommandEnum2["anime"] = "anime";
    SubjectCommandEnum2["book"] = "book";
    SubjectCommandEnum2["music"] = "music";
    SubjectCommandEnum2["game"] = "game";
    SubjectCommandEnum2["real"] = "real";
    return SubjectCommandEnum2;
  })(SubjectCommandEnum || {});
  var SubjectCatalogEnum = ((SubjectCatalogEnum2) => {
    SubjectCatalogEnum2["subject"] = "all";
    SubjectCatalogEnum2["anime"] = "2";
    SubjectCatalogEnum2["book"] = "1";
    SubjectCatalogEnum2["music"] = "3";
    SubjectCatalogEnum2["game"] = "4";
    SubjectCatalogEnum2["real"] = "6";
    return SubjectCatalogEnum2;
  })(SubjectCatalogEnum || {});
  var MonoCommandEnum = ((MonoCommandEnum2) => {
    MonoCommandEnum2["mono"] = "mono";
    MonoCommandEnum2["character"] = "character";
    MonoCommandEnum2["person"] = "person";
    return MonoCommandEnum2;
  })(MonoCommandEnum || {});
  var MonoCatalogEnum = ((MonoCatalogEnum2) => {
    MonoCatalogEnum2["mono"] = "all";
    MonoCatalogEnum2["character"] = "crt";
    MonoCatalogEnum2["person"] = "prsn";
    return MonoCatalogEnum2;
  })(MonoCatalogEnum || {});
  const SearchSubjectParametersSchema = {
    exact: { type: "boolean", optional: true, description: "开启精准匹配", rename: "legacy" }
  };
  const SearchMonoParametersSchema = {};
  function parsePageNumber(inner) {
    if (!inner) {
      return { current: 1, total: 1 };
    }
    const p_edge = inner.querySelector("span.p_edge")?.innerText;
    if (p_edge) {
      const [current2, total2] = p_edge.match(/(\d+)\s*\/\s*(\d+)/)?.slice(1).map(Number);
      return { current: current2, total: total2 };
    }
    const current = Number(inner.querySelector(".p_cur")?.innerText) || 1;
    const total = Array.from(inner.querySelectorAll("a.p")).map((a) => {
      return Number(a.href.split("page=")[1]?.split("&")[0]);
    }).sort((a, b) => b - a)[0] || current;
    return { current, total };
  }
  async function fetchSearchPage(uri, itemSelector) {
    const res = await fetchText(uri);
    if (!res.success) return res;
    const html = document.createElement("html");
    html.innerHTML = res.data;
    const items = Array.from(html.querySelectorAll(itemSelector));
    const extra = {
      page: parsePageNumber(html.querySelector("#multipage > .page_inner"))
    };
    return { success: true, data: { items, extra } };
  }
  const subjectCatalogMapping = new Map([
    ["1", "book"],
    ["2", "anime"],
    ["3", "music"],
    ["4", "game"],
  ...
  async function parseSubjectPage(keyword, params) {
    ...
    const items = res.data.items.map((item) => {
      ...
      const h3 = item.querySelector(".inner > h3");
      const cat = h3?.querySelector(".ico_subject_type")?.className.match(/subject_type_(\d)/)?.[1];
      const catalog = subjectCatalogMapping.get(cat ?? "");
      const title = h3?.querySelector("a")?.innerText || "";
      const titleSub = h3?.querySelector("small")?.innerText;
      const cover = item.querySelector(".cover img")?.src;
      const extra = item.querySelector(".inner > .info.tip")?.innerText?.split(" / ").map((s) => s.trim()).filter((s) => !!s) || [];
      const rank = item.querySelector(".rank")?.lastChild?.textContent;
      const state = revState(
        item.querySelector(".collectModify a.thickbox")?.innerText
      );
      const rate = item.querySelector("p.rateInfo small.fade")?.innerText;
      const ratePeople = item.querySelector("p.rateInfo span.tip_j")?.innerText;
      return {
        id,
        catalog,
        title,
        ...
        cover,
        extra,
        rank: rank ? Number(rank) : void 0,
        mark: state,
        rate: rate ? Number(rate) : void 0,
        ratePeople
      };
    });
  ...
  const monoCatalogMapping = new Map([
    ["character", "character"],
    ["person", "person"]
  ]);
  async function parseMonoPage(keyword, params) {
    const res = await fetchSearchPage(
    ...
    if (!res.success) return res;
    const items = res.data.items.map((item) => {
      const a = item.querySelector("div>h2>a.l");
      if (!a) return null;
      const u = a.href.split("/").filter((s) => s);
      const id = Number(u.pop());
      const catalog = monoCatalogMapping.get(u.pop() ?? "");
      const chineseName = a.querySelector("span.tip")?.textContent;
      let name = a.childNodes[0].textContent || "";
      if (chineseName) name = name.substring(0, name.length - 3);
      const avatar = item.querySelector("img.avatar")?.src;
      const extra = item.querySelector("div.prsn_info > .tip")?.innerText?.split(" / ").map((s) => s.trim()).filter((s) => !!s) || [];
      let comment = Number(
        item.querySelector("div>small.na")?.innerText.replace("(+", "").replace(")", "")
      ) || 0;
      return {
        catalog,
        id,
  ...
  const api$1 = Object.assign({}, subjectApi, monoApi);
  const CommandTipsCss = 'ul.v-search-command-tips{gap:20px;margin-top:12px;display:grid;grid-template-areas:"p1";>li:first-of-type{grid-area:p1}>li{display:flex;flex-direction:column;gap:6px;span{font-weight:600;font-size:14px}ul{padding:0;margin:0;li{margin-left:16px}}}}ul.v-search-command-tips:has(li:nth-of-type(2)){grid-template-columns:auto auto;grid-template-areas:"p1 p2";>li:last-of-type{grid-area:p2}}ul.v-search-command-tips:has(li:nth-of-type(3)){grid-template-columns:auto auto;grid-template-rows:auto auto;grid-template-areas:"p1 p3" "p2 p3";>li:nth-of-type(2){grid-area:p2}>li:last-of-type{grid-area:p3}}';
  importCSS(CommandTipsCss);
  function Parameters({ schema }) {
    if (!Object.keys(schema).length)
      return jsxRuntimeExports.jsx("ul", { children: jsxRuntimeExports.jsx("li", { children: jsxRuntimeExports.jsx("code", { children: "无" }) }) });
    return jsxRuntimeExports.jsx("ul", { children: Object.entries(schema).map(([name, rule]) => jsxRuntimeExports.jsxs("li", { children: [
jsxRuntimeExports.jsx("code", { children: rule.optional ? "可选" : "必需" }),
jsxRuntimeExports.jsx("code", { children: name }),
      ":",
jsxRuntimeExports.jsx("code", { children: renderType(rule) }),
      rule.description
    ] }, name)) });
  }
  function renderType(rule) {
    switch (rule.type) {
      case "string":
      case "boolean":
      case "number":
  ...
  function CommandTips({ kempty, schema }) {
    return jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
jsxRuntimeExports.jsxs("li", { children: [
jsxRuntimeExports.jsx("span", { children: "关键词:" }),
jsxRuntimeExports.jsx("ul", { children: jsxRuntimeExports.jsx("li", { children: jsxRuntimeExports.jsx("code", { children: kempty ? "可选" : "必需" }) }) })
      ] }),
jsxRuntimeExports.jsxs("li", { children: [
jsxRuntimeExports.jsx("span", { children: "参数:" }),
jsxRuntimeExports.jsx(Parameters, { schema })
      ] })
    ] });
  }
  function MainTips() {
    return jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
jsxRuntimeExports.jsxs("li", { children: [
jsxRuntimeExports.jsx("span", { children: "快捷键" }),
jsxRuntimeExports.jsx("ul", { children: keyShortcut.map(({ shortcuts, description }) => jsxRuntimeExports.jsxs("li", { children: [
          shortcuts.map(({ key, ctrl }) => jsxRuntimeExports.jsxs("code", { children: [
            ctrl ? "Ctrl-" : "",
            key
          ] }, key + (ctrl ? "_ctrl" : ""))),
          description
...
jsxRuntimeExports.jsxs("li", { children: [
            "示例:",
jsxRuntimeExports.jsx("code", { children: ":u=sai" }),
            "或",
jsxRuntimeExports.jsx("code", { children: ":u sai" }),
jsxRuntimeExports.jsx("br", {}),
            "表示",
jsxRuntimeExports.jsx("code", { children: "u" }),
            "参数为",
jsxRuntimeExports.jsx("code", { children: "sai" })
          ] })
        ] })
      ] }),
jsxRuntimeExports.jsxs("li", { children: [
jsxRuntimeExports.jsx("span", { children: "命令" }),
jsxRuntimeExports.jsx("ul", { children: Array.from(commandDescription.entries()).map(
          ([command, { commands, description, isDefault }]) => jsxRuntimeExports.jsxs("li", { children: [
            commands.map((cmd) => jsxRuntimeExports.jsx("code", { children: cmd }, cmd)),
            description,
            " ",
            isDefault && jsxRuntimeExports.jsx("code", { children: "(默认)" })
          ] }, command)
        ) })
      ] })
    ] });
  ...
  function Tips(props) {
    return jsxRuntimeExports.jsx("ul", { className: "v-search-command-tips", children: props.tips });
  }
  const api = Object.assign({}, api$1, api$2);
  ...
  const commandDescription = new Map();
  (() => {
    ...
    Object.values(SubjectCommandEnum).forEach(setCommand);
    Object.values(MonoCommandEnum).forEach(setCommand);
    Object.values(RymkCommandEnum).forEach(setCommand);
  })();
      ...
      switch (part[0]) {
        case "/": {
          ...
          if (command)
            return {
              success: false,
              banner: jsxRuntimeExports.jsx("h1", { children: "🤔多个命令" }),
              tips: jsxRuntimeExports.jsx(MainTips, {})
            };
          if (!matchedCommand)
            return {
              success: false,
              banner: jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
jsxRuntimeExports.jsx("h1", { children: "🤔未知命令" }),
jsxRuntimeExports.jsx("h3", { children: jsxRuntimeExports.jsx("code", { children: part }) })
              ] }),
              tips: jsxRuntimeExports.jsx(MainTips, {})
            };
        ...
        case ":": {
          ...
          if (paramParts.length < 2) {
            raw2[paramParts[0]] = parts[++i] || "";
          } else {
            const paramKey = paramParts.shift();
            raw2[paramKey] = paramParts.join("=");
          }
          break;
        }
        case "\\":
    ...
    if (!command) {
      if (isHelp) {
        return {
          success: false,
          banner: jsxRuntimeExports.jsxs("h1", { children: [
            "✨",
jsxRuntimeExports.jsx("code", { children: "/help" })
          ] }),
          tips: jsxRuntimeExports.jsx(MainTips, {})
        };
      }
    ...
    const tip = jsxRuntimeExports.jsx(CommandTips, { schema, kempty });
    const res = parseParameters(raw2, schema);
    if (!res.success) {
      res.tips = tip;
      return res;
    }
...
jsxRuntimeExports.jsxs("h1", { children: [
            "✨",
jsxRuntimeExports.jsxs("code", { children: [
              "/",
              command
            ] })
          ] }),
jsxRuntimeExports.jsx("h3", { children: api[command].description })
        ] }),
        tips: tip
      };
    }
...
jsxRuntimeExports.jsx("h3", { children: api[command].description })
  ...
  function parseParameters(raw2, schema) {
    const res = parse$1(raw2, schema);
    if (res.success) return res;
    switch (res.err) {
      case "key":
        return {
          success: false,
          banner: jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
jsxRuntimeExports.jsx("h1", { children: "🤔未知参数名" }),
jsxRuntimeExports.jsx("h3", { children: jsxRuntimeExports.jsx("code", { children: res.key }) })
          ] })
        };
      case "duplicate":
        return {
          success: false,
          banner: jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
jsxRuntimeExports.jsx("h1", { children: "🤔重复的参数名" }),
jsxRuntimeExports.jsx("h3", { children: jsxRuntimeExports.jsx("code", { children: res.key }) })
          ] })
        };
      case "required":
        return {
          success: false,
          banner: jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
jsxRuntimeExports.jsx("h1", { children: "🤔缺少必选参数" }),
jsxRuntimeExports.jsx("h3", { children: jsxRuntimeExports.jsx("code", { children: res.key }) })
          ] })
        };
      case "invalid":
        return {
          success: false,
          banner: jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
jsxRuntimeExports.jsx("h1", { children: "🤔无效的参数类型" }),
jsxRuntimeExports.jsxs("h3", { children: [
jsxRuntimeExports.jsx("code", { children: res.key }),
              "应为",
jsxRuntimeExports.jsx("code", { children: res.type })
            ] })
          ] })
        };
      case "enum":
        return {
          success: false,
          banner: jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
jsxRuntimeExports.jsx("h1", { children: "🤔无效的参数值" }),
jsxRuntimeExports.jsxs("h3", { children: [
jsxRuntimeExports.jsx("code", { children: res.key }),
              "应为",
jsxRuntimeExports.jsx("code", { children: res.extra?.map((s) => `"${s}"`).join(" | ") })
            ] })
          ] })
        };
    }
  ...
  const indexCss = 'html[data-theme=dark]{.v-search-item-card{>.background{--opacity: .5;--brightness: 1.2}}--background: 30, 30, 30}.v-search-item-card{display:flex;position:relative;gap:12px;padding:12px;img{border:none;margin:0;padding:0}>.background{position:absolute;top:0;left:0;width:100%;height:100%;overflow:hidden;z-index:-1;background-color:rgb(from var(--background) calc(r - 20) calc(g - 20) calc(b - 20) / .3);box-shadow:0 0 12px #0000001a;border-radius:8px;>img{width:calc(100% + 40px);height:calc(100% + 40px);margin:-20px;object-fit:cover;opacity:var(--opacity, .8);filter:blur(12px) brightness(var(--brightness, 1))}}>.background:after{content:"";position:absolute;top:0;left:0;width:100%;height:100%;background:rgb(from var(--background) calc(r - 20) calc(g - 20) calc(b - 20) / .6);backdrop-filter:blur(4px) brightness(var(--brightness, 1));z-index:1}>.avatar{width:48px;height:48px;border:none;object-fit:cover;border-radius:100%;overflow:hidden;padding:0;img{width:100%;height:100%;object-fit:cover}}>.info{flex:1;display:flex;flex-direction:column;gap:12px;>.title{margin:0;padding:0;font-size:16px;font-weight:600;display:flex;gap:4px;flex-wrap:wrap;align-items:end;small{font-size:12px;color:var(--primary-color, #f09199)}}>.extra{font-size:12px;color:var(--color);display:flex;gap:12px;align-items:center}}}html[data-avatar-size=square]{.v-search-item-user{>img{border-radius:4px}}}';
  importCSS(indexCss);
  const UserCss = ".v-search-item-card[data-catalog=user]{flex-direction:column;align-items:center;text-align:center;width:120px;padding:12px 4px;>.info{align-items:center;>.extra{justify-content:center}}}";
  importCSS(UserCss);
  function UserCard({ item }) {
    const id = item.username ?? item.uid;
    const url = `/user/${id}`;
    return jsxRuntimeExports.jsxs("li", { className: "v-search-item-card", "data-catalog": "user", "data-id": id, children: [
jsxRuntimeExports.jsx("div", { className: "background", children: jsxRuntimeExports.jsx(
        "img",
        {
          className: "avatar",
...
jsxRuntimeExports.jsx("a", { className: "avatar", href: url, children: jsxRuntimeExports.jsx(
        "img",
        {
          className: "avatar",
          src: item.avatar_url || "https://lain.bgm.tv/pic/user/l/icon.jpg",
...
jsxRuntimeExports.jsxs("div", { className: "info", children: [
jsxRuntimeExports.jsx("a", { className: "title", href: url, children: item.nickname }),
jsxRuntimeExports.jsxs("div", { className: "extra", children: [
          "@",
          id
        ] })
      ] })
  ...
  const GroupCss = ".v-search-item-card[data-catalog=group]{flex-direction:column;align-items:center;text-align:center;width:120px;padding:12px 4px;>.info{align-items:center;>.extra{justify-content:center}}}";
  importCSS(GroupCss);
  function GroupCard({ item }) {
    const id = item.slug;
    const url = `/group/${id}`;
    return jsxRuntimeExports.jsxs("li", { className: "v-search-item-card", "data-catalog": "group", "data-id": id, children: [
jsxRuntimeExports.jsx("div", { className: "background", children: jsxRuntimeExports.jsx(
        "img",
        {
          className: "avatar",
...
jsxRuntimeExports.jsxs("div", { className: "info", children: [
jsxRuntimeExports.jsx("a", { className: "title", href: url, children: item.title }),
jsxRuntimeExports.jsxs("div", { className: "extra", children: [
          "@",
          id
        ] }),
        item.nsfw && jsxRuntimeExports.jsx("div", { className: "nsfw-badge", children: "NSFW" }),
        item.description && jsxRuntimeExports.jsx("div", { className: "description", children: item.description })
      ] })
    ] }, id);
  }
  function Time({ time }) {
    const date = new Date(time * 1e3);
    return jsxRuntimeExports.jsxs("time", { dateTime: date.toISOString(), children: [
      date.getFullYear(),
      "-",
  ...
  const TopicCss = ".v-search-item-card[data-catalog=topic]{>.info{>.extra{>.group{color:var(--primary-color, #f09199)}>.creator{display:flex;align-items:center;gap:6px;text-decoration:none;color:inherit;img{width:16px;height:16px;object-fit:cover;border-radius:100%}span{vertical-align:middle}}}}}";
  importCSS(TopicCss);
  function TopicCard({
    item: { id, title, creator, created_at, group, reply_count }
  }) {
    const url = `/group/topic/${id}`;
    return jsxRuntimeExports.jsxs("li", { className: "v-search-item-card", "data-catalog": "topic", "data-id": id, children: [
jsxRuntimeExports.jsx("div", { className: "background", children: jsxRuntimeExports.jsx(
        "img",
        {
...
jsxRuntimeExports.jsx("a", { className: "avatar", href: url, children: group.icon_url && jsxRuntimeExports.jsx("img", { src: group.icon_url, alt: group.title }) }),
jsxRuntimeExports.jsxs("div", { className: "info", children: [
jsxRuntimeExports.jsxs("div", { className: "title", children: [
jsxRuntimeExports.jsx("a", { href: url, children: title }),
...
jsxRuntimeExports.jsxs("div", { className: "extra", children: [
jsxRuntimeExports.jsxs("a", { className: "creator", href: `/user/${creator.uid}`, children: [
...
jsxRuntimeExports.jsx("span", { children: creator.nickname })
          ] }),
jsxRuntimeExports.jsx("a", { className: "group", href: `/group/${group.slug}`, children: jsxRuntimeExports.jsx("span", { children: group.title }) }),
jsxRuntimeExports.jsx(Time, { time: created_at })
        ] })
      ] })
    ] }, id);
  }
  ...
  const getTagAttrs = (tag, params) => {
    ...
    if (uniqAttr) {
      const tagAttr2 = attrValue(tag, uniqAttr);
      const attrs = {
        ...params
      };
      delete attrs[String(uniqAttr)];
      const attrsStr = attrsToString(attrs);
      return `${tagAttr2}${attrsStr}`;
    }
  ...
  const nodeTreeToString = (content2, openTag, closeTag) => {
    if (Array.isArray(content2)) {
      return content2.reduce((r, node) => {
        if (node !== null) {
    ...
    if (content2) {
      return toString(content2, openTag, closeTag);
    }
    return null;
  };
  class TagNode {
    ...
    toString({ openTag = OPEN_BRAKET, closeTag = CLOSE_BRAKET } = {}) {
      const content2 = this.content ? nodeTreeToString(this.content, openTag, closeTag) : "";
      ...
      return `${tagStart}${content2}${this.toTagEnd({
      openTag,
      closeTag
    })}`;
    }
    ...
    static create(tag, attrs = {}, content2 = null, start) {
      return new TagNode(tag, attrs, content2, start);
    }
    static isOf(node, type) {
      return node.tag === type;
    }
    constructor(tag, attrs, content2, start, end) {
      this.tag = tag;
      this.attrs = attrs;
...
substrUntilChar(char) {
  ...
  const createCharGrabber = (source, options2) => new CharGrabber(source, options2);
  ...
  function createLexer(buffer, options2 = {}) {
    let row = 0;
    let prevCol = 0;
    let col = 0;
    let tokenIndex = -1;
    ...
    const openTag = options2.openTag || OPEN_BRAKET;
    const closeTag = options2.closeTag || CLOSE_BRAKET;
    const escapeTags = !!options2.enableEscapeTags;
    const contextFreeTags = (options2.contextFreeTags || []).filter(Boolean).map((tag) => tag.toLowerCase());
    const caseFreeTags = options2.caseFreeTags || false;
    const nestedMap = new Map();
    const onToken = options2.onToken || (() => {
    });
    ...
    function stateTag() {
      ...
      const isSpaceRestricted = hasSpace && options2.whitespaceInTags === false;
  ...
  function parse(input, opts = {}) {
    ...
    const openTag = options2.openTag || OPEN_BRAKET;
    const closeTag = options2.closeTag || CLOSE_BRAKET;
    const onlyAllowTags = (options2.onlyAllowTags || []).filter(Boolean).map((tag) => tag.toLowerCase());
    const caseFreeTags = options2.caseFreeTags || false;
    let tokenizer = null;
    ...
    function tagHandleEnd(token) {
      ...
      } else if (typeof options2.onError === "function") {
    ...
    tokenizer = lexer(input, {
      ...
      openTag,
      closeTag,
      onlyAllowTags: options2.onlyAllowTags,
      contextFreeTags: options2.contextFreeTags,
      caseFreeTags: options2.caseFreeTags,
      enableEscapeTags: options2.enableEscapeTags,
      whitespaceInTags: options2.whitespaceInTags
    });
    tokenizer.tokenize();
    do {
  ...
  let C1 = "C1";
  let C2 = "C2";
  function createTree(tree, options2) {
  ...
  function bbob(plugs) {
    const plugins2 = typeof plugs === "function" ? [
      plugs
    ] : plugs || [];
    const mockRender = () => "";
    return {
      process(input, opts) {
        ...
        const parseFn = options2.parser || parse;
        const renderFn = options2.render;
        const data = options2.data || null;
        if (typeof parseFn !== "function") {
          throw new Error(C1);
        }
        const raw = options2.skipParse && Array.isArray(input) ? input : parseFn(input, options2);
        let tree = options2.skipParse && Array.isArray(input) ? createTree(input || [], options2) : createTree(raw, options2);
        for (let idx = 0; idx < plugins2.length; idx++) {
          const plugin = plugins2[idx];
          if (typeof plugin === "function" && renderFn) {
            ...
            tree = createTree(newTree || tree, options2);
          }
        }
        return {
          get html() {
  ...
  function renderNode(node, options2) {
    const { stripTags = false } = options2 || {};
    ...
    if (Array.isArray(node)) {
      return render$1(node, options2);
    }
    if (isTagNode(node)) {
      if (stripTags) {
        return render$1(node.content, options2);
      }
      const attrs = attrsToString(node.attrs);
      if (node.content === null) {
        return START_TAG + node.tag + attrs + SELFCLOSE_END_TAG;
      }
      return START_TAG + node.tag + attrs + END_TAG + render$1(node.content, options2) + CLOSE_START_TAG + node.tag + END_TAG;
    }
    return "";
  }
  function render$1(nodes, options2) {
    if (nodes && Array.isArray(nodes)) {
      return nodes.reduce((r, node) => r + renderNode(node, options2), "");
    }
    if (nodes) {
      return renderNode(nodes, options2);
    }
    return "";
  }
  const toAST = (source, plugins2, options2) => bbob(plugins2).process(source, {
    ...options2,
    render: (input) => {
      return render$1(input, {
        stripTags: true
      });
    }
  }).tree;
  const isContentEmpty = (content2) => {
    if (!content2) {
      return true;
    }
    if (typeof content2 === "number") {
      return String(content2).length === 0;
    }
    return Array.isArray(content2) ? content2.length === 0 : !content2;
  };
  ...
  function render(source, plugins2, options2) {
    return renderToReactNodes(toAST(source, plugins2, options2));
  }
  const content = (children, plugins2, options2) => React.Children.map(children, (child) => {
    if (typeof child === "string") {
      return render(child, plugins2, options2);
    }
    return child;
  });
  const Component = ({ container: container2 = "span", componentProps = {}, children, plugins: plugins2 = [], options: options2 = {} }) => React.createElement(container2, componentProps, content(children, plugins2, options2));
  function process$1(tags, tree, core, options2) {
    return tree.walk((node) => {
      if (isTagNode(node)) {
        const tag = node.tag;
        const tagCallback = tags[tag];
        if (typeof tagCallback === "function") {
          return tagCallback(node, core, options2);
        }
      }
      return node;
    });
  ...
  const toNode = (tag, attrs, content2) => TagNode.create(tag, attrs, content2);
  const toStyle = (style) => ({
    style
  });
  ...
  const presetReact = presetHTML5.extend((tags) => ({
    ...tags,
    b: (...args) => ({
      ...tags.b(...args),
      ...tagAttr({
    ...
    color: (...args) => ({
      ...tags.color(...args),
      ...tagAttr({
        color: String(getUniqAttr(args[0].attrs))
      })
    })
  }));
  const EMOJI_REG = /\(bgm(\d+)\)/g;
  const range = (start, end) => new Array(end - start + 1).fill(0).map((_, i) => start + i);
  const smileMapping = new Map([
    ...range(1, 10).map((n) => [`(bgm0${n})`, `/img/smiles/bgm/0${n}.png`]),
    ["(bgm11)", "/img/smiles/bgm/11.gif"],
    ...range(12, 22).map((n) => [`(bgm${n})`, `/img/smiles/bgm/${n}.png`]),
    ["(bgm23)", "/img/smiles/bgm/23.gif"],
    ...range(24, 33).map((n) => [`(bgm0${n - 23})`, `/img/smiles/tv/0${n - 23}.gif`]),
    ...range(34, 125).map((n) => [`(bgm${n - 23})`, `/img/smiles/tv/${n - 23}.gif`]),
    ...range(200, 238).map((n) => [`(bgm${n})`, `/img/smiles/tv_vs/bgm_${n}.png`]),
    ["(bgm500)", "https://bgm.tv/img/smiles/tv_500/bgm_500.gif"],
    ["(bgm501)", "https://bgm.tv/img/smiles/tv_500/bgm_501.gif"],
    ...range(502, 504).map((n) => [`(bgm${n})`, `/img/smiles/tv_500/bgm_${n}.png`]),
    ["(bgm505)", "https://bgm.tv/img/smiles/tv_500/bgm_505.gif"],
    ...range(506, 514).map((n) => [`(bgm${n})`, `/img/smiles/tv_500/bgm_${n}.png`]),
    ...range(515, 519).map((n) => [`(bgm${n})`, `/img/smiles/tv_500/bgm_${n}.gif`]),
    ["(bgm520)", "https://bgm.tv/img/smiles/tv_500/bgm_520.png"],
    ...range(521, 523).map((n) => [`(bgm${n})`, `/img/smiles/tv_500/bgm_${n}.gif`]),
    ...range(524, 529).map((n) => [`(bgm${n})`, `/img/smiles/tv_500/bgm_${n}.png`])
  ]);
  const handleEmoji = (node) => {
...
"left",
  ...
  const preset = presetReact.extend((tags) => ({
    ...
    user: (node) => {
      ...
      if (!id) return { tag: "span", content: content2 };
      return { tag: "a", href: `/user/${id}`, content: content2 };
    },
    size: (node) => {
    ...
    spoiler: (node) => {
      const summary = Object.values(node.attrs || {})[0];
      node.attrs = {};
      const content2 = {
        tag: "summary",
        content: [summary || ""]
      };
      if (!node.content) return { tag: "details", content: [content2] };
      if (Array.isArray(node.content))
        return { tag: "details", content: [content2, ...node.content] };
      return { tag: "details", content: [content2, node.content] };
    },
    float: (node) => {
  ...
  const customPlugin = (tree, options2) => {
    const newTree = [];
    ...
    return createTree(newTree, options2);
  };
  const plugins = [preset(), customPlugin];
  const ReplyCss = ".v-search-item-card[data-catalog=reply]{>.info{>.content{background-color:var(--background);padding:4px 8px;border-radius:4px;white-space:pre-line;line-height:1.5;img{max-width:250px;height:auto}blockquote{margin:4px 0;padding:4px 4px 4px 12px;border-left:3px solid var(--primary-color, #f09199);background-color:rgb(from var(--primary-color, #f09199) calc(r - 10) calc(g - 10) calc(b - 10) / .1);border-radius:0 4px 4px 0}a{color:var(--link-color, #61a0ff)}pre{background-color:rgb(from var(--primary-color, #f09199) calc(r - 10) calc(g - 10) calc(b - 10) / .1);padding:8px;border-radius:4px;overflow:auto}}>.extra{gap:4px;align-items:center;>*{flex:0 0 auto}>.creator{color:var(--primary-color, #f09199)}}}}";
  importCSS(ReplyCss);
  function ReplyCard({
    item: { id, content: content2, creator, topic, created_at }
  }) {
    ...
    return jsxRuntimeExports.jsxs("li", { className: "v-search-item-card", "data-catalog": "reply", "data-id": id, children: [
jsxRuntimeExports.jsx("div", { className: "background", children: jsxRuntimeExports.jsx("img", { src: avatar, alt: creator.nickname }) }),
jsxRuntimeExports.jsx("a", { className: "avatar", href: url, children: jsxRuntimeExports.jsx("img", { src: avatar, alt: creator.nickname }) }),
jsxRuntimeExports.jsxs("div", { className: "info", children: [
jsxRuntimeExports.jsxs("div", { className: "title", children: [
jsxRuntimeExports.jsx("a", { className: "topic", href: url, children: jsxRuntimeExports.jsx("span", { children: topic.title }) }),
jsxRuntimeExports.jsx("a", { className: "group", href: `/group/${topic.group_slug}`, children: jsxRuntimeExports.jsxs("small", { children: [
            "@",
            topic.group_slug
          ] }) })
        ] }),
jsxRuntimeExports.jsx("div", { className: "content", children: jsxRuntimeExports.jsx(Component, { plugins, options, children: content2 }) }),
jsxRuntimeExports.jsxs("div", { className: "extra", children: [
jsxRuntimeExports.jsx("a", { className: "creator", href: `/user/${uid}`, children: jsxRuntimeExports.jsx("span", { children: creator.nickname }) }),
...
jsxRuntimeExports.jsx(Time, { time: created_at })
  ...
  const SubjectCss = '.v-search-item-card[data-item=subject]{padding:8px;>.avatar{flex:0 0 auto;width:80px;height:auto;border-radius:4px;img{height:auto}}>.info{flex:1 1 auto;>.title{>a{position:relative;display:inline-flex;align-items:center;gap:2px}>a:before{flex:1 1 auto;content:"";display:inline-block;width:17px;height:15px;filter:brightness(.5);background:url(/img/ico/subject_type_v2.png) no-repeat}}>.extra{>.tips{display:flex;gap:8px;flex-wrap:wrap;margin-right:12px;>li{padding:2px 6px;background-color:rgb(from var(--background) calc(r - 30) calc(g - 30) calc(b - 30) / .3);border-radius:12px;font-size:11px}}>.mark{position:absolute;right:8px;bottom:8px;color:var(--primary-color, #f09199)}}}>.judge{flex:0 0 auto;display:flex;flex-direction:column;align-items:flex-end;font-family:Noto Sans Mono,Consolas,Courier New,Courier,monospace;gap:0;top:0;font-size:12px;background-color:linear-gradient(to bottom,var(--primary-color, #f09199) 0%,rgb(from var(--primary-color, #f09199) / 0) 100%);.rank{font-size:24px;line-height:1;font-weight:900;font-style:italic;color:var(--primary-color, #f09199)}.rank:before{content:"#"}.rate{font-size:16px;line-height:1;font-style:italic}.rate:before{content:"☆";padding-right:3px;font-size:.8em}}}html[data-theme=dark]{.v-search-item-card[data-item=subject]{>.info{>.title{>a:before{filter:brightness(1)}}}}}.v-search-item-card[data-catalog=anime]{>.info{>.title{>a:before{background-position:0 -19px}}}}.v-search-item-card[data-catalog=book]{>.info{>.title{>a:before{background-position:0 0}}}}.v-search-item-card[data-catalog=game]{>.info{>.title{>a:before{background-position:0 -50px}}}}.v-search-item-card[data-catalog=music]{>.info{>.title{>a:before{background-position:0 -34px}}}}.v-search-item-card[data-catalog=real]{>.info{>.title{>a:before{background-position:0 -65px}}}}';
  importCSS(SubjectCss);
  ...
  function SubjectCard({
    ...
    return jsxRuntimeExports.jsxs(
      "li",
      {
        ...
        "data-id": id,
        children: [
jsxRuntimeExports.jsx("div", { className: "background", children: cover && jsxRuntimeExports.jsx("img", { src: cover, alt: "" }) }),
jsxRuntimeExports.jsx("a", { className: "avatar", href: `/subject/${id}`, children: cover && jsxRuntimeExports.jsx("img", { className: "avatar", src: cover, alt: title }) }),
jsxRuntimeExports.jsxs("div", { className: "info", children: [
jsxRuntimeExports.jsxs("div", { className: "title", children: [
jsxRuntimeExports.jsx("a", { href: `/subject/${id}`, children: title }),
jsxRuntimeExports.jsx("small", { children: titleSub })
            ] }),
jsxRuntimeExports.jsxs("div", { className: "extra", children: [
jsxRuntimeExports.jsx("ul", { className: "tips", children: extra.map((tip, i) => jsxRuntimeExports.jsx("li", { children: tip }, i)) }),
jsxRuntimeExports.jsx("div", { className: "mark", children: mark })
            ] })
          ] }),
jsxRuntimeExports.jsxs("div", { className: "judge", children: [
            rank && jsxRuntimeExports.jsx("div", { className: "rank", children: rank }),
            rate && jsxRuntimeExports.jsx("div", { className: "rate", children: rate }),
            ratePeople && jsxRuntimeExports.jsx("div", { className: "cnt", children: formatCnt(ratePeople) })
          ] })
        ]
      },
      id
  ...
  const MonoCss = '.v-search-item-card[data-item=mono]{flex-direction:column;align-items:center;text-align:center;width:260px;padding:12px 4px;>.avatar{width:100px;height:100px;border:none;>img{width:100%;height:100%;object-fit:cover}}>.info{align-items:center;>.title{display:flex;flex-direction:column;justify-content:center;align-items:center}>.extra{>.tips{display:flex;gap:8px;flex-wrap:wrap;margin-right:12px;justify-content:center;align-items:center;>li{padding:2px 6px;background-color:rgb(from var(--background) calc(r - 30) calc(g - 30) calc(b - 30) / .3);border-radius:12px;font-size:11px}}>.comment{font-size:16px;position:absolute;right:8px;top:8px;color:var(--primary-color, #f09199)}>.comment:before{content:"+"}}}}';
  importCSS(MonoCss);
  function MonoCard({
...
jsxRuntimeExports.jsx("a", { className: "avatar", href: url, children: jsxRuntimeExports.jsx("img", { src: avatar || "https://lain.bgm.tv/pic/user/l/icon.jpg", alt: name }) }),
jsxRuntimeExports.jsxs("div", { className: "info", children: [
jsxRuntimeExports.jsxs("div", { className: "title", children: [
jsxRuntimeExports.jsx("a", { href: url, children: name }),
jsxRuntimeExports.jsx("small", { children: chineseName })
            ] }),
jsxRuntimeExports.jsxs("div", { className: "extra", children: [
jsxRuntimeExports.jsx("ul", { className: "tips", children: extra.map((tip, i) => jsxRuntimeExports.jsx("li", { children: tip }, i)) }),
              !!comment && jsxRuntimeExports.jsx("div", { className: "comment wide", children: comment })
            ] })
          ] })
        ]
      },
  ...
  const SearchItemListCss = ".v-search-no-item{font-size:36px;font-weight:900;line-height:1.5}.v-search-item-list{list-style:none;box-sizing:border-box;padding:0;margin:0;display:flex;flex-direction:column;gap:12px;width:100%}.v-search-no-items{text-align:center}.v-search-item-list[data-command=mono],.v-search-item-list[data-command=character],.v-search-item-list[data-command=person],.v-search-item-list[data-command=user],.v-search-item-list[data-command=group]{flex-direction:row;flex-wrap:wrap;justify-content:center}";
  importCSS(SearchItemListCss);
  function SearchItem({ item }) {
    switch (item.catalog) {
      case "user":
        return jsxRuntimeExports.jsx(UserCard, { item });
      case "group":
        return jsxRuntimeExports.jsx(GroupCard, { item });
      case "topic":
        return jsxRuntimeExports.jsx(TopicCard, { item });
      case "reply":
        return jsxRuntimeExports.jsx(ReplyCard, { item });
      case "anime":
      case "book":
      case "music":
      case "game":
      case "real":
        return jsxRuntimeExports.jsx(SubjectCard, { item });
      case "character":
      case "person":
        return jsxRuntimeExports.jsx(MonoCard, { item });
      default:
        return null;
    }
  }
  function SearchItemList({ command, items }) {
    if (!items || items.length === 0) {
      return jsxRuntimeExports.jsx("h1", { className: "v-search-no-items", children: "🙁没有搜索结果" });
    }
    return jsxRuntimeExports.jsx("ul", { className: "v-search-item-list", "data-command": command, children: items.map((item, i) => jsxRuntimeExports.jsx(SearchItem, { item }, item.catalog + i)) });
  }
  const BannerCss = ".v-search-banner{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;h1{padding:0;margin:0;font-size:2em;line-height:1.4em;font-weight:900}h3{padding:0;margin:0;font-size:1.5em;line-height:1.4em;font-weight:700}}";
  importCSS(BannerCss);
  function Banner({ banner }) {
    return jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: banner && jsxRuntimeExports.jsx("div", { className: "v-search-banner wide", children: banner }) });
  }
  async function timeout(task, ms, getCancle) {
  ...
  const AppCss = "html[data-theme=dark]{.v-fast-search-app{--background: #2828284c;--color: #cfcfcf}}.v-fast-search-app{--color: #282828;--background: #ffffff4c;position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:999999;>button{background-color:#0003;backdrop-filter:blur(2px);border:none;position:absolute;top:0;right:0;height:100%;width:100%;cursor:pointer}.wide{font-family:Noto Sans Mono,Consolas,Courier New,Courier,Noto Sans CJK SC}code{background:var(--background);backdrop-filter:blur(4px);padding:2px 6px;border-radius:4px;color:var(--primary-color,#f09199);margin:0 4px}}body.v-fast-search-app-open{overflow:hidden}";
  importCSS(AppCss);
  const SearchToolCss = '@property --angle{syntax: "<angle>"; inherits: false; initial-value: 0deg;}@keyframes flip{0%{--angle: 90deg}to{--angle: 450deg}}.v-search-tool:before,.v-search-tool:after{content:"";display:block;position:absolute;height:100%;width:100%;left:50%;top:50%;transform:translate(-50%,-50%);padding:var(--padding);border-radius:calc(var(--border-radius) + var(--padding));z-index:-1;animation:flip 3s cubic-bezier(.68,-.55,.27,1.55) infinite;--angle: 90deg;--padding: 2px;background-image:conic-gradient(from var(--angle),transparent 0%,var(--primary-color,#f09199) 22.5%,var(--primary-color,#f09199) 27.5%,transparent 50%);--p11: calc(var(--border-radius) * (1 - cos(10deg)) + var(--padding));--d11: calc(100% - var(--p11));--p12: calc(var(--border-radius) * (1 - sin(10deg)) + var(--padding));--d12: calc(100% - var(--p12));--p21: calc(var(--border-radius) * (1 - cos(20deg)) + var(--padding));--d21: calc(100% - var(--p21));--p22: calc(var(--border-radius) * (1 - sin(20deg)) + var(--padding));--d22: calc(100% - var(--p22));--p31: calc(var(--border-radius) * (1 - cos(30deg)) + var(--padding));--d31: calc(100% - var(--p31));--p32: calc(var(--border-radius) * (1 - sin(30deg)) + var(--padding));--d32: calc(100% - var(--p32));--p41: calc(var(--border-radius) * (1 - cos(40deg)) + var(--padding));--d41: calc(100% - var(--p41));--p42: calc(var(--border-radius) * (1 - sin(40deg)) + var(--padding));--d42: calc(100% - var(--p42));--pc: calc(var(--border-radius) * (1 - sin(45deg)) + var(--padding));--dc: calc(100% - var(--pc));clip-path:polygon(-100% -100%,-100% 200%,var(--p11) 200%,var(--p11) var(--p12),var(--p21) var(--p22),var(--p31) var(--p32),var(--p41) var(--p42),var(--pc) var(--pc),var(--p42) var(--p41),var(--p32) var(--p31),var(--p22) var(--p21),var(--p12) var(--p11),var(--d12) var(--p11),var(--d22) var(--p21),var(--d32) var(--p31),var(--d42) var(--p41),var(--dc) var(--pc),var(--d41) var(--p42),var(--d31) var(--p32),var(--d21) var(--p22),var(--d11) var(--p12),var(--d11) var(--d12),var(--d21) var(--d22),var(--d31) var(--d32),var(--d41) var(--d42),var(--dc) var(--dc),var(--d42) var(--d41),var(--d32) var(--d31),var(--d22) var(--d21),var(--d12) var(--d11),var(--p12) var(--d11),var(--p22) var(--d21),var(--p32) var(--d31),var(--p42) var(--d41),var(--pc) var(--dc),var(--p41) var(--d42),var(--p31) var(--d32),var(--p21) var(--d22),var(--p11) var(--d12),var(--p11) 200%,200% 200%,200% -100%)}.v-search-tool:before{filter:blur(24px);opacity:.5}.v-search-tool{--border-radius: 8px;position:absolute;top:80px;left:50%;transform:translate(-50%);border-radius:var(--border-radius);background:var(--background, #ffffff4c);backdrop-filter:blur(10px);box-shadow:0 0 2px var(--primary-color, #f09199);box-sizing:border-box;min-width:600px;max-width:calc(100vw - 40px);max-height:calc(100vh - 160px);gap:12px;height:auto;padding:12px;display:flex;flex-direction:column;align-items:center;>input{flex:0 0 auto;grid-area:input;width:100%;box-sizing:border-box;background:transparent;padding:6px 16px;border-radius:100px;border:2px solid var(--primary-color, #f09199);outline:none;font-size:1rem;transition:all .3s ease}>input:hover,>input:focus{transition:all .3s ease}>.v-search-tool-banner{flex:0 0 auto;box-sizing:border-box;grid-area:banner;width:100%;display:grid;place-items:center}>.v-search-tool-tips{flex:0 0 auto;box-sizing:border-box;grid-area:tips;width:100%;display:flex;justify-content:center;align-items:start}>.v-search-tool-container{flex:1 1 auto;min-height:0;width:100%;display:grid;place-items:center;box-sizing:border-box;grid-area:list;min-height:10px;max-width:1200px;scrollbar-width:none;-ms-overflow-style:none;overflow-x:hidden;overflow-y:scroll;>*{width:100%}h1{font-size:2em;line-height:1;font-weight:900}.v-search-tool-loading{padding:12px 0;text-align:center}}>.v-search-tool-container::-webkit-scrollbar{display:none}}';
  importCSS(SearchToolCss);
  const keyShortcut = [
    {
  ...
  function App(props) {
    const [showSearchTool, setShowSearchTool] = reactExports.useState(false);
    const [pedding, setPadding] = reactExports.useState(true);
    const [inComposition, setInComposition] = reactExports.useState(false);
    const [hasMore, setHasMore] = reactExports.useState(false);
    const [banner, setBanner] = reactExports.useState(null);
    const [tips, setTips] = reactExports.useState(null);
    const [list, setList] = reactExports.useState(null);
    const [input, setInput] = reactExports.useState("/");
    const inputValue = reactExports.useRef("/");
    const lastValue = reactExports.useRef("");
    const totalPages = reactExports.useRef(Infinity);
    const currentPage = reactExports.useRef(0);
    const lastList = reactExports.useRef(null);
    const fetching = reactExports.useRef(false);
    const mq = reactExports.useRef(null);
    const ref = reactExports.useRef(null);
    const cancelSearch = reactExports.useRef(() => {
    });
    const close = () => {
      setShowSearchTool(false);
      cancelSearch.current();
    };
    const setCancelSearch = (rj) => cancelSearch.current = () => {
      rj();
      cancelSearch.current = () => {
      };
    };
    ...
    const next = async ({ key, query }) => {
      ...
      if (lastList.current) {
        ...
        lastList.current = { command: query.command, items };
      }
      setList(lastList.current);
      return currentPage.current < totalPages.current;
    };
    const tryNext = async (query, ms = 50, r = false) => {
      cancelSearch.current();
      if (!query) {
    ...
    reactExports.useEffect(() => {
      document.addEventListener("keydown", (e) => {
        ...
        if (filters.has(e.target.tagName)) return;
        switch (e.key) {
          case "k":
          case "K":
            if (!e.ctrlKey) return;
...
jsxRuntimeExports.jsxs("div", { className: "v-search-tool", children: [
jsxRuntimeExports.jsx(
          "input",
          {
            className: "wide",
        ...
        pedding && banner && jsxRuntimeExports.jsx("div", { className: "v-search-tool-banner", children: jsxRuntimeExports.jsx(Banner, { banner }) }),
        pedding && tips && jsxRuntimeExports.jsx("div", { className: "v-search-tool-tips", children: jsxRuntimeExports.jsx(Tips, { tips }) }),
jsxRuntimeExports.jsxs("div", { className: "v-search-tool-container", ref, children: [
          list && jsxRuntimeExports.jsx(SearchItemList, { command: list.command, items: list.items }),
jsxRuntimeExports.jsx("div", { className: "v-search-tool-loading", children: jsxRuntimeExports.jsx("h3", { children: !pedding && (hasMore ? "✨加载中 ..." : "😊没有更多了") }) })
        ] })
      ] })
    ] });
  }
...
jsxRuntimeExports.jsx(reactExports.StrictMode, { children: jsxRuntimeExports.jsx(App, { timeout: 500 }) })
  );
