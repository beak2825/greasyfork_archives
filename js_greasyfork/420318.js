...
// @downloadURL https://update.greasyfork.org/scripts/420318/%E3%80%90%E6%9F%A5%E7%9C%8B%E7%89%88%E3%80%91%E7%A6%81%E5%90%8C%E5%8F%B7%E5%A4%9A%E7%99%BB%EF%BC%8C%E5%B7%B2%E5%86%85%E5%88%B6%E9%98%B2%E6%A3%80%E6%B5%8B%E7%B3%BB%E7%BB%9F%E3%80%82.user.js
// @updateURL https://update.greasyfork.org/scripts/420318/%E3%80%90%E6%9F%A5%E7%9C%8B%E7%89%88%E3%80%91%E7%A6%81%E5%90%8C%E5%8F%B7%E5%A4%9A%E7%99%BB%EF%BC%8C%E5%B7%B2%E5%86%85%E5%88%B6%E9%98%B2%E6%A3%80%E6%B5%8B%E7%B3%BB%E7%BB%9F%E3%80%82.meta.js
// ==/UserScript==
...
var setting = {
    ...
    ,username: '' // 学号/工号/借书证号(邮箱/手机号/账号)，例如''，默认''
    ,password: '' // 密码，例如''，默认''
},
    _self = unsafeWindow,
...
function ZhiJiaoYunBeforeFind() {
    setting.div = $$(
        ...
        '查看版:<input id="phone" type="text name="phone" value='+ ('系统已默认绑定帐号💯禁同号多登 ') + '>'+

       '<div style="color: bule;background: #B0E2FF;;font-size: 15px;margin-bottom: 1%;">目前正在搜索|'+(userSetting.option ? '收费题库':'作业可刷新网页考试不能')+'<br>更新日期：2021年1月17日20：28分09秒 一人一号，多人登同号系统自动禁<br>通知：系统没有自动填空的，可以看下面的绿色标注！或者公众众号回复问题5！！！</div>' +
        ...
        if (this.nodeName == 'TD') {
            ...
            $$(this).parent().text('正在搜索|通知：2021年1月3日24点停止更新');
            setting.num++;
        } else if (len === 0){
            GM_setValue('phone',$$("#phone").val())
        }else if (len === 1) {
            if (setting.loop) {
                clearInterval(setting.loop);
                delete setting.loop;
                len = ['已暂停搜索'+(+userSetting.option ? '通知：2021年1月3日24点停止更新':'通知：2021年1月3日24点停止更新'), '继续答题'];
            } else {
                ...
                len = ['正在搜索'+(+userSetting.option ? '通知：2021年1月3日24点停止更新':'通知：2021年1月3日24点停止更新'), '暂停答题'];
            }
            setting.div.children('div:eq(0)').html(function() {
                return $$(this).data('html') || len[0];
            }).removeData('html');
            ...
            userSetting.option = !userSetting.option;
            $$(this).html(userSetting.option ? '通知：2021年1月3日24点停止更新' : '通知：2021年1月3日24点停止更新');
            GM_setValue('userSetting',userSetting)
        } else if (len == 4) {
            parent.location.reload();
        }else if (len == 5) {
    ...
    GM_xmlhttpRequest({
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                var obj = $$.parseJSON(xhr.responseText) || {};
                if (obj.code == 200) {
                    userSetting.option ? setting.div.children('div:eq(0)').text('正在搜索收费题库--当前积分剩余：'+obj.data.num):setting.div.children('div:eq(0)').text('正在搜索免费题库|通知：2021年1月3日24点停止更新'+obj.data.qun);
                    ...
                    setting.div.children('div:eq(0)').html('发现未知异常，反馈').siblings('button:eq(1)').click();;
                }
                //setting.div.children('span').html(obj.msg || '');
            } else if (xhr.status == 404) {
                setting.div.children('div:eq(0)').data('html', '最新版已发布！请点击油猴按钮【用户脚本检查更新】完成更新！').siblings('button:eq(1)').click();
            }else {
                setting.div.children('div:eq(0)').html('发现未知异常，请反馈');
            }
        },
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').html(setting.over + '正在链接到云端，请稍后....<br>如果长时间链接不上反馈');
        }
    });
}
...
function GreenZhiJiaoYunFindAnswer(){
    ...
    GM_xmlhttpRequest({
        ...
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').html(setting.over + '正在链接到云端，请稍后....<br>如果长时间链接反馈');
        }
    });
}
