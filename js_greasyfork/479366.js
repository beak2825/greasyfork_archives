// ==UserScript==
// @name         武汉宝业建工职业培训
// @namespace    wuhanbaoyejiangongzhiyepeixun
// @version      0.1.2
// @author       https://github.com/panghujiajia
// @description  自动挂课时，中途有拍照检测，自动获取用户头像进行拍照检测
// @license      GPL
// @match        *.e.eceping.net/
// @match        *.e.hbzclm.com/
// @downloadURL https://update.greasyfork.org/scripts/479366/%E6%AD%A6%E6%B1%89%E5%AE%9D%E4%B8%9A%E5%BB%BA%E5%B7%A5%E8%81%8C%E4%B8%9A%E5%9F%B9%E8%AE%AD.user.js
// @updateURL https://update.greasyfork.org/scripts/479366/%E6%AD%A6%E6%B1%89%E5%AE%9D%E4%B8%9A%E5%BB%BA%E5%B7%A5%E8%81%8C%E4%B8%9A%E5%9F%B9%E8%AE%AD.meta.js
// ==/UserScript==
...
(function () {
  ...
  start();
  facePicture();
  async function wait(time = 2) {
    return new Promise((resolve) => {
      setTimeout(resolve, time * 1e3);
    });
  }
  async function choosePlay() {
    let list = $("#videoList .video");
    if (list.length > 0) {
      list = list.filter(function() {
        return !$("i", this).hasClass("studyOver");
      });
      if (list.length) {
        list[0].click();
        await handleFun();
  ...
  async function handleFun() {
    ...
    video.off("pause", videoPause);
    video.off("ended", videoEnded);
    video.on("pause", videoPause);
  ...
  async function getBtn(text) {
    let target = null;
    await wait();
    const spanList = $(".el-dialog__body").find("span");
  ...
  async function takePicture() {
    ...
    let paizhao = null;
    let jixu = null;
    paizhao = await getBtn("拍照");
    $(paizhao).click();
    await wait();
    $("#courseImg").attr("src", window.localStorage.getItem("userAvatar"));
    jixu = await getBtn("继续看课");
    $(jixu).click();
    await wait();
    const btn = $(".el-message-box__btns").children().eq(1);
    if (btn) {
      $(btn).click();
      facePicture();
    }
  }
  async function doPractice(list) {
    const video = getVideo();
    if (video && $(video).get(0)) {
      $(video).get(0).pause();
    }
    for (let i = 0; i < list.length; i++) {
      const card = $(".answer-sheet-list li");
      $(card).eq(i).click();
      await wait(0.2);
      const item = list[i];
      const { questionType, questionAnswers } = item;
      const answers = questionAnswers.filter((a) => a.isRight);
      const options = $(
        ".questions-stems-content .el-checkbox-group .el-checkbox .el-checkbox__label"
      );
      for (let a of answers) {
        options.each((oIndex, oItem) => {
          if ($(oItem).text().replace(/^[A-Z]\. /g, "") === a.content) {
            $(oItem).click();
            return false;
          }
        });
      ...
      if (questionType === "多选题") {
        $(".questions-stems-content .el-button--primary").click();
      }
    }
    if (video && $(video).get(0)) {
      const isPlaying = video.currentTime > 0 && !video.paused && !video.ended && video.readyState > video.HAVE_CURRENT_DATA;
    ...
    if (location.href.includes("/#/coursePlay")) {
      await wait();
      await choosePlay();
    }
  }
  function hookXHR() {
    ...
    XMLHttpRequest.prototype.send = function(body) {
      if (xhrUrl.indexOf("eceping-course/videoPlay/saveStudyPhoto") > -1) {
        const userAvatar = window.localStorage.getItem("userAvatar");
        let requestBody = JSON.parse(body);
        requestBody.photoUrl = userAvatar;
      ...
      this.addEventListener(
        ...
        async function() {
          if (this.readyState == 4 && this.status == 200) {
            if (this.responseURL.indexOf(
              "employee/getEmployeeInformation"
            ) > -1) {
              const userAvatar = window.localStorage.getItem("userAvatar");
              let { empHeadportrait } = JSON.parse(
                this.response
              ).data;
              if (userAvatar !== empHeadportrait) {
            ...
            if (this.responseURL.indexOf("exercise/exerciseDetails") > -1) {
              const {
                paperEntity: { paperAreaList }
              } = JSON.parse(this.response).data;
              await wait(1);
              const list = [];
              paperAreaList.forEach((item) => {
                list.push(...item.questionList);
              });
              await doPractice(list);
            }
          }
        },
        false
