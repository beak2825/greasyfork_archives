...
// @downloadURL https://update.greasyfork.org/scripts/32425/Hybrid%20-%20Calls%20-%20YRVBNF%20%28Past%29.user.js
// @updateURL https://update.greasyfork.org/scripts/32425/Hybrid%20-%20Calls%20-%20YRVBNF%20%28Past%29.meta.js
// ==/UserScript==
...
if ($('li:contains("Call - Contact")').length) {
    ...
    createButton('Nope',0,0,3,2,'This person does not work there, the person I spoke to was not aware of past employment');
    createButton('Wrong Biz',0,1,3,2,'This number connected to the wrong business');
    number.append('<br>');
    number.append('<br>');
    createButton('Generic VM',2,2,3,2,'This  number connected to a voicemail with information that did not help to answer questions.');
    createButton('Personal VM',2,2,3,2,'Voice mail did identify the contact but no title information','Unsure');
    createButton('Biz on VM',2,0,3,2,'The business is listed on the voicemail but with no information pertaining to the contact');
    createButton('No Contact VM',2,0,3,2,'Voicemail without contact');
    number.append('<br>');
    number.append('<br>');
    createButton('DC',4,2,3,2,'Disconnected- this number is not in service. ');
    createButton('DC - Directory',4,2,3,2,'Disconnected - Directory assistance offered');
    createButton('DC - Fast Busy',4,2,3,2,'Disconnected - fast busy');
    number.append('<br>');
    number.append('<br>');
    createButton('Fax',5,2,3,2,'Fax');

    createButton('Ringing', 1,2,3,2,'This call did connect but no one answered after extended ringing');
    createButton('Silence',1,2,3,2,'Rang until silence','Unsure');
    createButton('Busy',3,2,3,2,'Busy signal repeatedly after multiple attemps at calling');
    createButton('Hung Up',1,2,3,2,'Hung up on me');
    number.append('<br>');
    number.append('<br>');
