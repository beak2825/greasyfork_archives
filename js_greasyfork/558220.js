// ==UserScript==
// @name        SceneNZBs_01  QOL Image Zoom
...
// @downloadURL https://update.greasyfork.org/scripts/558220/SceneNZBs_01%20%20QOL%20Image%20Zoom.user.js
// @updateURL https://update.greasyfork.org/scripts/558220/SceneNZBs_01%20%20QOL%20Image%20Zoom.meta.js
// ==/UserScript==
...
(function () {
    ...
    function navigateGallery(direction) {
        ...
        // Erlauben, dass man den Index auch neu setzt, wenn er gleich ist (für initiales Highlight)
        // if (newIndex === activeIndex) return;

        const targetImg = allImagesCache[newIndex];
        const container = DOM_HELPERS.getContainer(targetImg);

        // 1. SCROLLEN (User Wunsch: "nachscrollen")
        ...
        document.querySelectorAll('.ultra-keyboard-active').forEach(el => {
            el.classList.remove('ultra-keyboard-active');
            el.style.backgroundColor = ''; // Reset inline styles if any
        });

        // Farbe-Reset falls vorher grün (Download Feedback)
        container.style.outlineColor = '';
        container.style.backgroundColor = '';
        container.classList.add('ultra-keyboard-active');
    ...
    function init() {
        ...
        // 1. Keyboard Listener (Navi & Actions)
        document.addEventListener('keydown', (e) => {
            ...
            // Action: Download / Add to Cart
            else if (['Enter', ' ', 'f', 'F'].includes(e.key)) {
                if (isKeyboardMode && activeIndex !== -1) {
                    e.preventDefault();
                    triggerDownload();
                }
            }
