...
(function unifiedAutomator() {
    ...
    function calculateExpectedDungeonDamage(currentHP, maxHP) {
        // Cálculo baseado no HP atual e máximo
    ...
    function calculateExpectedExp(currentHP, maxHP) {
        // Cálculo de EXP esperada baseado no HP
        // Base: 10% do HP atual + bônus proporcional
        const hpRatio = currentHP / maxHP;
    ...
    function ensureFloatingHelpersStyles(doc) {
        ...
        style.textContent = `
            ...
            /* Ajuste do body para não ficar embaixo da toolbar */
            body {
            ...
            /* 🆕 When shell is open, add extra margin for the shell panel */
            ...
            /* Desktop: posicionar à direita da página (canto direito) */
            ...
            @media (max-width: 480px) {
                ...
                #autods-floating-helpers-toggle {
                    order: -1; /* Forçar para primeiro em flex */
                    padding: 6px;
                ...
                /* Garantir que todos os botões apareçam em mobile */
            ...
            /* 🆕 Styling when button is inside autods-floating-helpers */
    ...
    function addDirectJoinButton(originalButton, monsterId, buttonLabel, context, instanceId = null, autoNavigate = false) {
        ...
        directButton.textContent = `⚡ ${buttonLabel}`;
        ...
        directButton.addEventListener('click', async (e) => {
            ...
            // context.logger.info(`[DirectJoin] 🖱️ Button clicked for monster ${monsterId}`);

            // VERIFICAÇÃO INTELIGENTE: Só usar direct join em páginas wave/dungeon
            // Se já estiver em battle.php, usar método tradicional (click)
            ...
            if (isOnBattlePage) {
                // context.logger.info(`[DirectJoin] ⚠️ Já está em página de batalha. Usando método tradicional (click).`);
                originalButton.click();
                return;
            }

            if (!isOnWavePage) {
                // context.logger.warn(`[DirectJoin] ⚠️ Página não reconhecida para direct join. Usando método tradicional.`);
                originalButton.click();
                return;
            }
            ...
            directButton.textContent = '⏳ Joining...';
            ...
            try {
                ...
                if (isDungeon) {
                    ...
                    if (result.success) {
                        context.logger.info(`[DirectJoin] ✅ Join successful: ${result.message}`);
                    ...
                    if (text.toLowerCase().includes('success')) {
                        context.logger.info(`[DirectJoin] ✅ Join successful!`);
                ...
                context.logger.error(`[DirectJoin] ❌ Error: ${error.message}`, error);
                
                // FALLBACK FINAL: Em caso de qualquer erro, usar método tradicional
                ...
                try {
                    ...
                    context.logger.error(`[DirectJoin] Fallback click também falhou:`, clickError);
    ...
    function createLogFloater(config = {}) {
        ...
        container.innerHTML = `
            <div id="autods-log-floater-header">
                ...
                <div id="autods-log-floater-controls">
                    <button id="autods-log-clear" title="Limpar logs">🗑️</button>
                    <button id="autods-log-toggle" title="Minimizar/Expandir">➖</button>
                </div>
            </div>
        ...
        function toggleFloater() {
            ...
            toggleBtn.textContent = logFloaterState.minimized ? '➕' : '➖';
        ...
        async function useItem(invId, itemName) {
            try {
                ...
                if (success) {
                    ...
                    logger.warn(`⚠️ Falha ao usar ${itemName}: ${text.substring(0, 100)}`);
                }
                
                return success;
            } catch (error) {
                logger.error(`❌ Erro ao usar ${itemName}:`, error);
                return false;
            }
        }
        ...
        return {
            ...
            async useSmallStaminaPotion(quantity = 1) {
                ...
                if (!ssp || !ssp.invId || !ssp.quantity || ssp.quantity < quantity) {
                    logger.warn(`💊 Sem Small Stamina Potion disponível (precisa: ${quantity}, tem: ${ssp?.quantity || 0})`);
                    return false;
                }
                
                logger.info(`💊 SSP disponível: ${ssp.quantity}x - usando ${quantity}x`);
            ...
            async useExpPotion() {
                ...
                if (!exp || !exp.invId || !exp.quantity || exp.quantity < 1) {
                    logger.warn('💊 Sem EXP Potion disponível');
                    return false;
                }
                
                logger.info(`💊 EXP Potion disponível: ${exp.quantity}x`);
     ...
     * 
     * ⭐ STRATEGY: Uses fetch API first (less Cloudflare-sensitive) with GM_xmlhttpRequest as fallback
    ...
    function createHttpService(context) {
        ...
        return {
            ...
            async joinBattle(monsterId, userId) {
                if (!monsterId) {
                    logger.error(`❌ joinBattle called with invalid monsterId: ${monsterId}`);
                    return { error: 'Invalid monsterId', status: 400 };
                }
                if (!userId) {
                    logger.error(`❌ joinBattle called with invalid userId: ${userId}`);
                    return { error: 'Invalid userId', status: 400 };
                }
                
                const body = `monster_id=${monsterId}&user_id=${userId}`;
                logger.debug(`📤 Sending JOIN request - monster_id=${monsterId}, user_id=${userId}`);
    ...
    function createNotificationService(context) {
        ...
        return {
            ...
            staminaLow(current, required) {
                this.warn(`⚡ Stamina baixa: ${current}/${required}`);
            },
            ...
            battleComplete(monsterName, damage) {
                this.success(`⚔️ ${monsterName} completado! ${formatNumber(damage)} dano`);
            },
            ...
            itemUsed(itemName, remaining) {
                this.info(`💊 ${itemName} usado! Restam: ${remaining}`);
            },
            ...
            lootClaimed(monsterName, rewards) {
                ...
                this.success(`🎁 Loot obtido: ${monsterName}${rewardStr}`);
            },
             ...
             */
            autoPaused(reason) {
                this.warn(`⏸️ Automação pausada: ${reason}`);
            }
        };
    }
        ...
        return {
            ...
            getCurrent() {
                try {
                    ...
                    // Strategy 3: Look for pattern "❤️ 0 / 40,000,000 HP"
                    if (monsterHp === null) {
                        const hpPattern = Array.from(document.querySelectorAll('*')).filter(el => 
                            el.textContent && /❤️\s*[\d,]+\s*\/\s*[\d,]+\s*HP/.test(el.textContent) && el.children.length === 0
                        );
                        if (hpPattern.length > 0) {
                            const hpMatch = hpPattern[0].textContent.match(/❤️\s*([\d,]+)\s*\/\s*([\d,]+)\s*HP/);
                            if (hpMatch) {
                    ...
                    // Read player HP (from "💚 Your HP 157,750 / 157,750" or "💚 157,750 / 157,750 HP")
                    let playerHp = null;
                    let playerMaxHp = null;
                    
                    const playerHpPattern = Array.from(document.querySelectorAll('*')).filter(el => 
                        el.textContent && /💚.*?[\d,]+\s*\/\s*[\d,]+/.test(el.textContent) && el.children.length === 0
                    );
                    ...
                    // Read monster name (from "🧟 Lizardman Flamecaster" or "🧟 Lizardman Shadowclaw DEAD")
        ...
        return {
            ...
            parseWaveMonster(card) {
                try {
                    ...
                    if (!monsterId) {
                        ...
                        logger.warn(`⚠️ [MonsterScanner] Card sem data-monster-id: id="${cardId}", class="${cardClass}", html="${cardHTML}..."`);
                        return null;
                    }
            ...
            parseDungeonMonster(card) {
                try {
                    ...
                    // Extract current damage (não disponível na lista, precisa buscar na página de batalha)
                    let currentDamage = 0;
            ...
            ensureUserId() {
                try {
                    ...
                    if (detected) {
                        ...
                        logger.info(`✅ User ID auto-detected and saved: ${detected}`);
                        return detected;
                    }
                    
                    logger.warn('⚠️ User ID not found in cookie (not logged in?)');
                    return null;
                } catch (error) {
            ...
            startAutoDetect(intervalMs = 5000) {
                const intervalId = scheduler.addInterval(() => {
                    ...
                    if (!cfg.core?.userId) {
                        ...
                        if (detected) {
                            config.update({ core: { userId: detected } });
                            logger.info(`✅ User ID auto-detected: ${detected}`);
                        }
                    }
     ...
     * numbers.parse('💰 1,000,000');      // 1000000
    ...
    function createLootService(context) {
        ...
        const RARITY_CONFIG = {
            MYTHICAL: { icon: '🔮', color: '#FF1493', label: 'MYTHICAL' },
            LEGENDARY: { icon: '⭐', color: '#FFD700', label: 'LEGENDARY' },
            EPIC: { icon: '💜', color: '#9D5CFF', label: 'EPIC' },
            RARE: { icon: '💙', color: '#4A9EFF', label: 'RARE' },
            UNCOMMON: { icon: '💚', color: '#4CAF50', label: 'UNCOMMON' },
            COMMON: { icon: '⚪', color: '#9E9E9E', label: 'COMMON' }
        };
            ...
            for (let i = 0; i < targets.length; i += batchSize) {
                ...
                if (checkLevelUp && i > 0 && initialExpInfo) {
                    ...
                    if (levelUpInfo.leveledUp) {
                        logger.info(`🎉 LEVEL UP detectado! Parando loot... (Batch ${Math.floor(i / batchSize) + 1})`);
                        summary.messages.push('⚠️ Loot stopped: Level up detected');
                        break;
                    }
                }
                ...
                if (checkStamina) {
                    ...
                    if (staminaFull) {
                        logger.info(`⚡ Stamina recuperada! Parando loot... (Batch ${Math.floor(i / batchSize) + 1})`);
                        summary.messages.push('⚠️ Loot stopped: Stamina full recovered');
                        break;
                    }
                }
                    ...
                    <div style="font-family: Arial, sans-serif; color: #fff;">
                        ...
                        <div style="background: #1a1a1a; padding: 15px; border-radius: 8px; margin: 15px 0;">
                            ...
                            <div style="margin-top: 10px; padding: 8px; background: #2a2a2a; border-radius: 4px; font-size: 11px; color: #888; text-align: center;">
                                📊 Eficiência: <span style="color: #8be9fd;">${(summary.exp / summary.damage).toFixed(3)} XP/HP</span> | <span style="color: #ffb86c;">${(summary.gold / summary.damage).toFixed(5)} Gold/HP</span>
                            </div>
                            ` : ''}
                        </div>
                        ...
                        ${summary.messages.length > 0 ? `
                            <div style="margin: 15px 0; padding: 10px; background: #2a2a2a; border-radius: 5px; border-left: 3px solid #FFD700;">
                                ${summary.messages.map(msg => `<div style="margin: 3px 0;">⚠️ ${msg}</div>`).join('')}
                            </div>
                        ` : ''}
                    </div>
                `;
    ...
    function createDataExtractorService(context) {
        ...
        async function extractStats() {
            ...
            try {
                ...
                const staminaText = bodyText.match(/💪\s*(\d[\d,]*)\s*\/\s*(\d[\d,]*)/);
                if (staminaText) {
                ...
                logger.info(`📊 Extracted stats: ATK ${stats.base_attack}, DEF ${stats.base_defense}, STAM ${stats.max_stamina}`);
                ...
                logger.error('❌ Failed to extract stats', { error });
                return {
                    base_attack: 0,
                    base_defense: 0,
                    max_stamina: 0,
        ...
        async function extractPets() {
            ...
            try {
                ...
                const equippedMatch = bodyText.match(/⚔️ Equipped Pets — Attack Team([\s\S]*?)(?:🐾 Pet Inventory|$)/);
                
                if (equippedMatch) {
                    const equippedSection = equippedMatch[1];
                    
                    // Split by pet cards (using stars as delimiter)
                    const petBlocks = equippedSection.split(/★+/);
                    let petIndex = 0;
                    ...
                    for (const block of petBlocks) {
                        ...
                        // Extract stats: "🧪 Lv. 80 | 🔪 703 ATK | 🛡️ 703 DEF"
                        const statsMatch = block.match(/🧪\s*Lv\.\s*(\d+)\s*\|\s*🔪\s*(\d+)\s*ATK\s*\|\s*🛡️\s*(\d+)\s*DEF/);
                        ...
                        // Extract effects: "⚡ 24.8% Extra Damage To Monsters"
                        const effectsMatch = block.match(/⚡\s*([^\n]+)/);
                ...
                const inventoryMatch = bodyText.match(/🐾 Pet Inventory([\s\S]*?)(?:🥚 Eggs|$)/);
                
                if (inventoryMatch) {
                    const inventorySection = inventoryMatch[1];
                    const petBlocks = inventorySection.split(/★+/);
                ...
                logger.info(`🐾 Extracted pets: ${pets.equipped_attack_team.length} equipped, ${pets.available_pets.length} available (ATK ${pets.total_attack_pets_atk}, DEF ${pets.total_attack_pets_def})`);
                ...
                logger.error('❌ Failed to extract pets', { error });
                return {
                    equipped_attack_team: [],
                    total_attack_pets_atk: 0,
                    total_attack_pets_def: 0,
        ...
        async function extractEquipment() {
            ...
            try {
                ...
                logger.debug(`📦 Equipment bonuses from config: Extra DMG ${bonuses.extra_damage_monsters}%`);
                ...
                const bodyText = doc.body.textContent;
                const equippedMatch = bodyText.match(/🛡️ Equipped Items — Attack Set([\s\S]*?)(?:⚔️ Equipment|$)/);
                
                if (equippedMatch) {
                    const equippedSection = equippedMatch[1];
                    
                    // Parse stats: "🔪 180 ATK · 🛡️ 0 DEF"
                    const statsRegex = /🔪\s*(\d+)\s*ATK\s*·\s*🛡️\s*(\d+)\s*DEF/gi;
                ...
                logger.info(`⚔️ Extracted equipment: ATK ${equipment.total_equipped_atk}, DEF ${equipment.total_equipped_def}, Bonuses ${totalBonus.toFixed(1)}%`);
                ...
                logger.error('❌ Failed to extract equipment', { error });
                return {
                    equipped_attack_set: {},
                    total_equipped_atk: 0,
                    total_equipped_def: 0
        ...
        async function extractAll() {
            logger.info('🔄 Extracting all player data...');
        ...
        function clearCache() {
            ...
            logger.debug('🗑️ Data extractor cache cleared');
        }
        ...
        async function forceRefresh() {
            logger.info('🔄 Force refreshing all player data...');
            clearCache();
            ...
            for (const pet of pets) {
                ...
                if (effects.is_pve_only || effects.is_universal) {
                    ...
                    if (effects.extra_damage_monsters > 0) {
                        ...
                        pvmScore += score;
                        breakdown.push(`${effects.extra_damage_monsters}% Extra Damage (×3.0) = ${score.toFixed(1)}`);
                    ...
                    if (effects.armor_penetration > 0) {
                        ...
                        pvmScore += score;
                        breakdown.push(`${effects.armor_penetration}% Armor Pen (×2.0) = ${score.toFixed(1)}`);
                    }
                    
                    if (effects.heal_on_damage > 0) {
                        const score = effects.heal_on_damage * 1.0;
                        pvmScore += score;
                        breakdown.push(`${effects.heal_on_damage}% Heal (×1.0) = ${score.toFixed(1)}`);
                    }
                    
                    if (effects.crit_rate > 0) {
                        const score = effects.crit_rate * 1.5;
                        pvmScore += score;
                        breakdown.push(`${effects.crit_rate}% Crit Rate (×1.5) = ${score.toFixed(1)}`);
                    }
                    
                    if (effects.crit_damage > 0) {
                        const score = effects.crit_damage * 1.2;
                        pvmScore += score;
                        breakdown.push(`${effects.crit_damage}% Crit Damage (×1.2) = ${score.toFixed(1)}`);
                    }
                    
                    if (effects.stamina_save > 0) {
                        const score = effects.stamina_save * 1.0;
                        pvmScore += score;
                        breakdown.push(`${effects.stamina_save}% Stamina Save (×1.0) = ${score.toFixed(1)}`);
                    }
                    ...
                    if (effects.stamina_cost_mult > 1.0) {
                        ...
                        pvmScore -= penalty;
                        breakdown.push(`×${effects.stamina_cost_mult} Stamina Cost (penalty) = -${penalty.toFixed(1)}`);
                    }
                ...
                if (effects.is_pvp_only || effects.is_universal) {
                    ...
                    if (effects.steal_stat > 0) {
                        ...
                        atkScore += score;
                        breakdown.push(`${effects.steal_stat}% Steal ATK (×3.0) = ${score.toFixed(1)}`);
                    }
                    
                    if (effects.team_conversion > 0) {
                        const score = effects.team_conversion * 2.5;
                        atkScore += score;
                        breakdown.push(`${effects.team_conversion}% Team Conversion (×2.5) = ${score.toFixed(1)}`);
                    }
                    
                    if (effects.atk_def_swap > 0) {
                        const score = effects.atk_def_swap * 2.0;
                        atkScore += score;
                        breakdown.push(`${effects.atk_def_swap}% ATK/DEF Swap (×2.0) = ${score.toFixed(1)}`);
                    }
                ...
                if (effects.is_pvp_only || effects.is_universal) {
                    ...
                    if (effects.steal_stat > 0) {
                        const score = effects.steal_stat * 3.0;
                        defScore += score;
                        breakdown.push(`${effects.steal_stat}% Steal DEF (×3.0) = ${score.toFixed(1)}`);
                    }
                    ...
                    if (effects.def_boost > 0) {
                        ...
                        defScore += score;
                        breakdown.push(`${effects.def_boost}% DEF Boost (×2.0) = ${score.toFixed(1)}`);
                    }
        ...
        function analyzeOptimalBuilds(data, monsterDefense = 0) {
            ...
            logger.info(`🔍 Analyzing ${combinations.length} pet combinations...`);
    ...
    function createDamageCalculatorService(context) {
         ...
         * 
         * Formula:
         * PvM_Damage = (225 + equip_bonus + pet_bonus + user_atk_bonus) × stamina_cost × (1 + pet_effects_percent/100)
         * 
         * Where:
         * - equip_bonus = max(30, equip_atk - m_def/2) × 20
         * - pet_bonus = max(30, pet_atk - m_def/2) × 20
         * - user_atk_bonus = 1000 × max(30, user_atk - m_def)^0.25
         ...
         * 
         * Formula:
         * Power = atk_base + 0.7×def_base + 0.3×pets_atk + 0.4×equip_atk + 0.3×pets_def + 0.2×equip_def
         * 
         * @param {Object} params - Calculation parameters
         * @param {number} params.base_attack - Base attack stat
         ...
         * 
         * Formula:
         * Power = def_base + 0.7×atk_base + 0.3×pets_atk + 0.2×eq_atk + 0.3×pets_def + 0.4×eq_def
         * 
         * @param {Object} params - Calculation parameters
         * @param {number} params.base_attack - Base attack stat
         * @param {number} params.base_defense - Base defense stat
        ...
        async function calculateWithCurrentData(dataExtractor, monster_defense = 50, skill_id = -2) {
            logger.info('🧮 Calculating damage with current player data...');
            ...
            logger.info(`✅ PvM: ${formatNumber(pvm.damage)} | PvP ATK: ${pvp_attacker.power} | PvP DEF: ${pvp_defender.power}`);
    ...
    const ultraFastWaveModule = {
        ...
        async runUltraFastLoop(context) {
            ...
            logger.info('⚡ Ultra Fast Wave iniciado - Loop eterno ativado');
            ...
            if (!isActivePage) {
                logger.error(`❌ [UltraFastLoop] NÃO está em active_wave.php! Pathname: ${initialPathname}`);
                return;
            }
            ...
            while (/active_wave\.php/i.test(context.location.pathname)) {
                ...
                // 🎯 PRIORIDADE: Se Special Boss Farm estiver ativo E processando bosses, pausar Ultra Fast Wave
                if (cfg.specialBossFarm?.enabled) {
                    ...
                    if (specialBossModule?.state?.running && specialBossModule?.state?.activeBosses > 0) {
                        logger.info(`👑 Special Boss Farm processando ${specialBossModule.state.activeBosses} boss(es) - pausando Ultra Fast Wave...`);
                ...
                logger.info(`🚀 PRE-FLIGHT JOIN: Entrando em ${newTargets.length} batalha(s) SIMULTANEAMENTE...`);
                
                const joinedTargets = await this.joinUltraFastBattles(context, newTargets);
                
                logger.info(`📊 Pre-Flight Join: ${joinedTargets.length}/${newTargets.length} joins bem-sucedidos`);
                ...
                if (ongoingCount > 0) {
                    ...
                    if (additionalOngoing.length > 0) {
                        logger.info(`⚔️ Atacando ${additionalOngoing.length} batalha(s) adicionais em andamento...`);
            ...
            logger.info(`⚡ Ultra Fast Wave finalizado - Ataques: ${this.state.ultraFastStats.attackCount}, Dano: ${formatNumber(this.state.ultraFastStats.damageDealt)}, Tempo: ${duration.toFixed(1)}s`);
        },
        ...
        getUnclaimedKillsCount() {
            // Método 1: Procurar por "💀 Unclaimed kills: X" ou "Unclaimed kills: X" no texto
            ...
            // Método 2: Procurar por "X kills" (formato comum)
            ...
            // Método 3: Contar monster cards com HP = 0 (dead)
        ...
        async executeQuickDeadLoot(context) {
            ...
            // Se não encontrou, procurar por HP = 0
            if (cards.length === 0) {
                cards = Array.from(document.querySelectorAll('.monster-card'));
                cards = cards.filter(card => {
                    const hpText = card.textContent || '';
            ...
            logger.info(`💀 [LOOT] Encontrados ${cards.length} monster cards para analisar`);
            
            // Coletar targets válidos (filtrando bosses 100B+)
            const lootTargets = [];
            ...
            for (const card of cards) {
                ...
                // Skip bosses (100B+ HP)
                if (hp && hp >= BOSS_HP_THRESHOLD) {
                    logger.warn(`👑 ${monsterName}: Ignorado (boss com ${formatNumber(hp)} HP)`);
                    bossesFiltered++;
                    continue;
                }
            ...
            if (lootTargets.length === 0) {
                logger.info(`💀 [LOOT] Nenhum dead monster para lootar (${bossesFiltered} bosses filtrados)`);
                return false;
            }
         ...
         * 
         * Strategy:
         * 1. 🚀 Try to read from monster card in current page (fast, no HTTP)
         * 2. 📡 If not found, fetch from battle.php (slower, may be blocked by Cloudflare)
         */
        async fetchBattleDamage(context, monsterId) {
            const { logger, numbers, dom } = context;
            
            // 1️⃣ Try to read damage from monster card in current page (FASTEST - no HTTP)
            try {
            ...
            // 2️⃣ Card not found or no damage attribute - try HTTP fetch (slower, may be blocked)
            try {
                ...
                // Validação: verificar se é HTML válido
                if (html.length < 500 || !html.includes('<body')) {
                    logger.debug(`[FetchDamage] ${monsterId}: Resposta inválida (${html.length} bytes)`);
        ...
        async fetchCurrentDamages(context, targets) {
            ...
            logger.info(`📊 Buscando dano atual de ${targets.length} batalha(s)...`);
            ...
            // logger.info(`📊 Danos atuais obtidos em ${duration}ms (${skippedCount} inválidos, ${zeroCount} com 0 dano)`);
        },
        ...
        async joinUltraFastBattles(context, targets) {
            ...
            if (!userId) {
                logger.warn('⚠️ UserId não configurado. Não é possível usar Ultra Fast Mode sem userId.');
                return [];
            }
            ...
            for (const { target, result } of results) {
                if (result.success) {
                    ...
                    logger.debug(`✅ Joined: ${target.name} (ID: ${target.monsterId})`);
                } else if (result.cloudflareBlock) {
                    // Cloudflare blocked - still count as joined since battle exists
                    joinedTargets.push(target);
                    logger.debug(`⚠️ ${target.name}: Cloudflare block mas tentando atacar mesmo assim`);
                } else {
                    logger.debug(`⚠️ Join falhou: ${target.name} - ${result.message}`);
                }
            }
        ...
        async attackUltraFastBattles(context, targets, cfg) {
            ...
            const validTargets = targets.filter(t => {
                if (t.invalid) {
                    logger.debug(`⏭️ ${t.name}: Batalha inválida, pulando`);
                    return false;
                }
                ...
                if (isSpecialBoss(t.name) && effectiveDamageTarget > baseDamageTarget) {
                    logger.info(`👑 ${t.name}: Boss especial detectado - alvo: ${formatNumber(effectiveDamageTarget)} (mínimo 50M para loot)`);
                }
                
                if ((t.currentDamage || 0) >= effectiveDamageTarget) {
                    logger.info(`✅ ${t.name}: Já atingiu threshold (${formatNumber(t.currentDamage)}/${formatNumber(effectiveDamageTarget)})`);
            ...
            if (validTargets.length === 0) {
                logger.info(`✅ Todos os alvos já atingiram o threshold ou são inválidos`);
                return;
            }
            
            logger.info(`🚀 PRE-FLIGHT ATTACK: ${validTargets.length} monstros precisam de ataque, alvo base ${formatNumber(baseDamageTarget)} dano`);
            ...
            while (rounds < maxRounds) {
                ...
                if (currentStamina < skillCost) {
                    ...
                    if (currentStamina < skillCost) {
                        ...
                        logger.info('⏳ Sem opções de recuperação. Parando ataques.');
                        break;
                    } else {
                        // Stamina recovered by loot dead
                        logger.info(`✅ Stamina recuperada por loot dead (${currentStamina}). Continuando...`);
                        continue;
                    }
                }
                ...
                if (activeTargets.length === 0) {
                    logger.info(`✅ Todos os alvos completados ou mortos`);
                    break;
                }
                ...
                logger.debug(`⚔️ Rodada ${rounds}: Atacando ${batchTargets.length} monstros em PARALELO...`);
                ...
                for (const { targetState, result } of results) {
                    if (result.success) {
                        ...
                        if (targetState.currentDamage >= targetDamage) {
                            ...
                            const specialIcon = isSpecialBoss(targetState.name) ? '👑 ' : '';
                            logger.info(`✅ ${specialIcon}${targetState.name}: Alvo atingido! ${formatNumber(targetState.currentDamage)} dano em ${targetState.attackCount} ataques`);
                            this.state.ultraFastStats.monstersCompleted++;
                        }
                        ...
                        logger.info(`💀 ${targetState.name}: Monstro morto/inválido`);
                        this.state.ultraFastStats.monstersCompleted++;
                    }
                }
                ...
                if (successCount > 0) {
                    ...
                    logger.warn(`⚠️ Rodada ${rounds}: Nenhum ataque bem-sucedido (${batchTargets.length} tentativas)`);
                    ...
                    if (currentStaminaCheck < skillCost) {
                        logger.warn(`⏳ Stamina insuficiente após rodada (${currentStaminaCheck}/${skillCost})`);
                        break;
                    }
                    
                    // Check if stuck in loop without progress
                    if (roundsWithoutProgress >= maxRoundsWithoutProgress) {
                        logger.warn(`❌ ${maxRoundsWithoutProgress} rodadas sem progresso. Parando ataques.`);
                        break;
                    }
                }
            ...
            for (const t of targetStates) {
                if (!t.completed && !t.dead && t.attackCount > 0) {
                    logger.info(`⏸️ ${t.name}: ${formatNumber(t.currentDamage)}/${formatNumber(t.effectiveDamageTarget || baseDamageTarget)} (${t.attackCount} ataques)`);
                }
            }
        },
        ...
        async attackOngoingBattles(context, cfg) {
            ...
            if (validTargets.length === 0) {
                logger.info(`✅ Todas as batalhas em andamento já atingiram o threshold`);
                return;
            }
            
            logger.info(`🚀 PRE-FLIGHT: ${validTargets.length} batalha(s) precisam de ataque...`);
                ...
                for (const { targetState, result } of results) {
                    if (result.success) {
                        ...
                        if (targetState.currentDamage >= targetDamage) {
                            targetState.completed = true;
                            const specialIcon = isSpecialBoss(targetState.name) ? '👑 ' : '';
                            logger.info(`✅ ${specialIcon}${targetState.name}: Alvo atingido! ${formatNumber(targetState.currentDamage)} dano`);
                            this.state.ultraFastStats.monstersCompleted++;
                        }
    ...
    const waveUiModule = {
        ...
        initMonsterFilter(context) {
            ...
            filterContainer.innerHTML = `
                ...
                <select id="monster-sort-order"
                    ...
                    <option value="hp-asc" ${this.state.filterSettings.sortOrder === 'hp-asc' ? 'selected' : ''}>HP: Low → High</option>
                    <option value="hp-desc" ${this.state.filterSettings.sortOrder === 'hp-desc' ? 'selected' : ''}>HP: High → Low</option>
                    <option value="name-asc" ${this.state.filterSettings.sortOrder === 'name-asc' ? 'selected' : ''}>Name: A → Z</option>
                    <option value="name-desc" ${this.state.filterSettings.sortOrder === 'name-desc' ? 'selected' : ''}>Name: Z → A</option>
                </select>
        ...
        applySortOrder(context) {
            ...
            // Ordena monstros dentro de cada seção individualmente
            ...
            sections.forEach(sectionType => {
                ...
                // Busca o container interno dentro da seção
        ...
        initMonsterSorting(context) {
            ...
            continueBattleSection.innerHTML = `
                <div class="monster-section-header" data-section="continue">
                    ...
                    <button class="section-toggle-btn" data-toggle="continue">${this.state.continueBattlesExpanded ? '−' : '+'}</button>
                </div>
            ...
            lootSection.innerHTML = `
                <div class="monster-section-header" data-section="loot">
                    ...
                    <button class="section-toggle-btn" data-toggle="loot">${this.state.lootExpanded ? '−' : '+'}</button>
                </div>
            ...
            monsterCards.forEach(card => {
                ...
                if (buttonText.includes('continue')) {
                    ...
                    lootCards.push(card);
                } else if (buttonText.includes('join') || buttonText.includes('⚡') || buttonText === 'fight') {
                    joinCards.push(card);
                } else {
            ...
            if (continueToggle) {
                continueToggle.addEventListener('click', () => {
                    ...
                    continueToggle.textContent = this.state.continueBattlesExpanded ? '−' : '+';
            ...
            if (lootToggle) {
                lootToggle.addEventListener('click', () => {
                    ...
                    lootToggle.textContent = this.state.lootExpanded ? '−' : '+';
                    context.storage.set('waveSectionStates', {
                        continueBattlesExpanded: this.state.continueBattlesExpanded,
                        lootExpanded: this.state.lootExpanded
                    });
    ...
    const specialBossFarmModule = {
        ...
        init(context) {
            this.state = {
                running: false,
                activeBosses: 0, // 🆕 Count of bosses currently being attacked (not just waiting)
                stats: {
                    bossesFound: 0,
                    bossesCompleted: 0,
                    totalAttacks: 0,
        ...
        activate(context) {
            ...
            context.logger.info('👑 Special Boss Farm ativado! (Ultra Fast Mode)');
        ...
        async runBossFarmLoop(context) {
            ...
            logger.info('👑 Iniciando loop de farm de bosses especiais (Ultra Fast)...');
            ...
            while (/active_wave\.php/i.test(context.location.pathname)) {
                ...
                if (bosses.length === 0) {
                    this.state.activeBosses = 0; // 🆕 No bosses to attack
                ...
                this.state.activeBosses = bosses.length; // 🆕 Mark bosses as being processed
                ...
                if (joinedBosses.length === 0) {
                    logger.warn('👑 Falha ao entrar nas batalhas. Aguardando 30s...');
                    await sleep(30000);
                    continue;
                }
                ...
                this.state.activeBosses = 0; // 🆕 Finished attacking bosses
        ...
        async scanSpecialBosses(context, cfg) {
            ...
            for (const card of cards) {
                ...
                if (!monsterId) {
                    logger.warn(`👑 ${monsterName}: Não encontrou ID do monstro`);
                    continue;
                }
        ...
        async joinBossesViaApi(context, bosses, cfg) {
            ...
            logger.info(`👑 Entrando em ${bosses.length} boss(es) via API...`);

            // Join all bosses in parallel
            const joinPromises = bosses.map(async (boss) => {
                if (boss.hasJoined) {
                    logger.debug(`👑 ${boss.name}: Já está na batalha`);
            ...
            for (const result of results) {
                if (result.success || result.alreadyJoined) {
                    logger.info(`✅ ${result.boss.name}: ${result.alreadyJoined ? 'Já estava na batalha' : 'Entrou via API!'}`);
                    result.boss.hasJoined = true;
                    joinedBosses.push(result.boss);
                } else {
                    logger.warn(`❌ ${result.boss.name}: Falha ao entrar - ${result.message || result.error || 'Erro desconhecido'}`);
                }
            }

            logger.info(`👑 Join Results: ${joinedBosses.length}/${bosses.length} bem-sucedidos`);
            return joinedBosses;
        },
        ...
        async attackBossesUltraFast(context, bosses, cfg) {
            ...
            if (validBosses.length === 0) {
                logger.info('✅ Todos os bosses já atingiram o dano alvo!');
                return { hadWork: false, allCompleted: true };
            }

            logger.info(`🚀 Atacando ${validBosses.length} boss(es) (Ultra Fast Mode) até ${formatNumber(targetDamage)} dano cada...`);

            // 3. Process each boss sequentially (to avoid overdamage)
            for (const boss of validBosses) {
                logger.info(`👑 Processando: ${boss.name} (dano atual: ${formatNumber(boss.currentDamage)})`);
            ...
            // Final Summary
            logger.info('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
            logger.info('👑 RESUMO DO SPECIAL BOSS FARM');
    ...
    const ultraFastAttackModule = {
        ...
        init(context) {
            this.state = {
                running: false,
                manualMode: true,  // 🆕 Flag para indicar modo manual (via botão)
                currentBatch: [],
                stats: {
                    totalBatches: 0,
                    totalMonsters: 0,
        ...
        activate(context) {
            ...
            // 🆕 SEMPRE usar modo manual (só executa via botão)
            // Não executar automaticamente mesmo quando ativado
        ...
        triggerManualRun(context) {
            ...
            if (slotInfo.continueCount > 10) {
                logger.warn(`⚠️ NÃO É SEGURO! ${slotInfo.continueCount} batalhas simultâneas ativas (limite: 10)`);
                logger.warn('💡 Aguarde completar algumas batalhas antes de iniciar Ultra Fast Farm.');
                context.notifications?.notify?.(
                    `⚠️ Muitas batalhas ativas (${slotInfo.continueCount} > 10). Aguarde completar algumas.`,
                    'warning'
                );
                return;
            }
            ...
            logger.info('⚡ Ultra Fast Farm iniciado manualmente!');
        ...
        async runBatchFarm(context) {
            ...
            logger.info(`📊 Configuração: ${cfg.maxParallelBattles} batalhas paralelas, ${cfg.attacksPerMonster} ataques por mob`);
            ...
            if (continueCount > 10) {
                logger.warn(`⚠️ MUITAS BATALHAS ATIVAS (${continueCount} > 10)! Modo seguro: apenas verificando dano dos monstros já joined.`);
                logger.info('💡 Não será feito join em novos monstros para evitar sobrecarga do servidor.');
            ...
            logger.info(`✅ Slots disponíveis: ${availableSlots}/${cfg.maxConcurrentBattles}`);

            // Ajustar maxParallelBattles baseado nos slots disponíveis
            ...
            // 2. Group into batches (usando effectiveMaxBattles ao invés de cfg.maxParallelBattles)
            const batches = [];
            for (let i = 0; i < targets.length; i += effectiveMaxBattles) {
                batches.push(targets.slice(i, i + effectiveMaxBattles));
            }

            logger.info(`🎯 ${targets.length} mobs encontrados → ${batches.length} lotes paralelos (${effectiveMaxBattles} mobs/lote)`);
            ...
            for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
                ...
                logger.info(`⚡ Lote ${batchIndex + 1}/${batches.length}: ${batch.length} mobs`);
                ...
                logger.info(`✅ Entradas: ${successfulJoins.length}/${batch.length} bem-sucedidas`);

                if (successfulJoins.length === 0) {
                    logger.warn('⚠️ Nenhuma entrada bem-sucedida neste lote, pulando...');
                    continue;
                }

                // ⏳ CRITICAL: Aguardar servidor processar os joins antes de atacar
                // Sem esse delay, há race condition onde os attacks chegam antes dos joins serem processados
                const joinServerDelay = cfg.delayAfterJoin || 500; // Default 500ms
                logger.debug(`⏳ Aguardando ${joinServerDelay}ms para servidor processar joins...`);
                ...
                for (let attackRound = 0; attackRound < cfg.attacksPerMonster; attackRound++) {
                    logger.info(`⚔️ Rodada de ataque ${attackRound + 1}/${cfg.attacksPerMonster}`);
                ...
                if (batchIndex < batches.length - 1) {
                    logger.info(`⏳ Aguardando ${cfg.delayBetweenBatches}ms antes do próximo lote...`);
            ...
            logger.info(`✨ Farm ultra rápido completo!`);
            logger.info(`📊 Stats: ${this.state.stats.totalMonsters} mobs, ${this.state.stats.totalAttacks} ataques em ${(duration/1000).toFixed(1)}s`);

            // 🆕 Adicionar duração aos stats
            ...
            // 🆕 Auto-loot OU aguardar usuário fechar modal
            if (triggerLoot) {
                logger.info('🎁 Auto-loot será acionado após fechar o modal...');
                // Loot module vai cuidar do reload
            } else {
                logger.info('✅ Ultra Fast Farm concluído! Veja os resultados no modal.');
            }
        },
        ...
        checkAvailableSlots(context, maxConcurrentBattles) {
            ...
            logger.debug(`🔍 Verificação de slots: ${activeJoins} batalhas ativas, ${availableSlots} slots disponíveis`);
        ...
        async checkAndAttackJoinedMonsters(context) {
            ...
            logger.info('🔍 Verificando dano nos monstros já joined...');
            ...
            if (joinedCards.length === 0) {
                logger.info('❌ Nenhum monstro com "Continue" encontrado');
                return;
            }

            logger.info(`📊 ${joinedCards.length} monstros já joined encontrados`);
            ...
            if (targets.length === 0) {
                logger.info('❌ Nenhum target válido extraído');
                return;
            }

            logger.info(`⚔️ Atacando ${targets.length} monstros já joined com skill ${cfg.skillId}...`);
                ...
                await Promise.all(batch.map(async target => {
                    try {
                        ...
                        if (result.success) {
                            ...
                            logger.debug(`✅ Ataque em ${target.name}: sucesso`);
                        } else {
                            logger.debug(`❌ Ataque em ${target.name}: ${result.message}`);
                        }
                    } catch (error) {
                        logger.error(`❌ Erro ao atacar ${target.name}:`, error);
                    }
                }));
            ...
            logger.info(`✅ Verificação e ataque concluído! ${this.state.stats.totalAttacks} ataques executados`);
        },
        ...
        async scanTargets(context) {
            ...
            logger.debug(`[UltraFastAttack] Encontrados ${monsters.length} monster cards após filtros`);
            ...
            if (monsterNamesList && monsterNamesList.length > 0) {
                ...
                logger.debug(`[UltraFastAttack] ${targets.length} mobs após filtro de nome`);
            }
        ...
        async joinBattlesParallel(context, targets) {
            ...
            if (!userId) {
                logger.error('❌ User ID não configurado! Configure core.userId primeiro.');
                return [];
            }
            ...
            // 🔥 PRE-FLIGHT ATTACK: Preparar TODAS as requisições de join antes de enviar
            logger.info(`🚀 PRE-FLIGHT JOIN: Preparando ${targets.length} requisições simultâneas...`);
            
            // Validar targets - remover qualquer um sem monsterId
            const validTargets = targets.filter(target => {
                if (!target.monsterId) {
                    logger.warn(`⚠️ Skipping target ${target.name || 'unknown'} - monsterId missing!`);
                    return false;
                }
                return true;
            });
            ...
            if (validTargets.length === 0) {
                logger.error('❌ Nenhum target válido para join!');
                return [];
            }
            
            logger.info(`✅ ${validTargets.length}/${targets.length} targets válidos`);
            
            // BLOQUEIO DE NAVEGAÇÃO: Usar listener para prevenir navegação acidental durante joins
            // Não conseguimos reatribuir location methods em navegadores modernos, então usamos um listener
            const navigationBlocker = {
                blocked: true,
                originalClickHandler: null
            };
            
            // Interceptar cliques (não deixa navegar)
            const blockClickHandler = (e) => {
                if (navigationBlocker.blocked) {
                    // Se é um link externo ou botão, bloquear
                    if (e.target?.tagName === 'A' || e.target?.getAttribute?.('onclick')) {
                        logger.warn(`🔒 Clique em link bloqueado durante join: ${e.target?.href || e.target?.textContent}`);
                        e.preventDefault();
                        e.stopPropagation();
                    }
                }
            ...
            // Interceptar navigation via popstate (botão back)
            const blockPopState = (e) => {
                if (navigationBlocker.blocked) {
                    logger.warn(`🔒 Navegação do histórico bloqueada`);
                    e.preventDefault();
                }
            };
            ...
            const cleanupNavigation = () => {
                ...
                logger.debug('🔓 Bloqueio de navegação removido');
            };
            
            // Criar array de funções que executam o join (não executar ainda)
            const joinPromises = validTargets.map(target => ({
                target,
                execute: () => context.http.joinBattle(target.monsterId, userId)
            }));

            logger.info(`⚡ Disparando ${joinPromises.length} joins SIMULTANEAMENTE...`);
            
            // Executar TODAS as requisições ao mesmo tempo (Pre-Flight)
            const startTime = Date.now();
            const results = await Promise.all(
                joinPromises.map(async ({ target, execute }) => {
                    try {
                        ...
                        if (result.success) {
                            ...
                            logger.debug(`  ✅ ${target.name}: Join bem-sucedido`);
                        } else {
                            this.state.stats.failedJoins++;
                            logger.debug(`  ⚠️ ${target.name}: Falha - ${result.message}`);
                        }
                        ...
                        this.state.stats.failedJoins++;
                        logger.error(`  ❌ Erro ao entrar em ${target.name}:`, error);
                        return {
                            ...target,
                            success: false,
                            message: error.message
            ...
            logger.info(`📊 Pre-Flight Join: ${successCount} sucessos, ${failCount} falhas (${duration}ms total)`);
        ...
        async attackBattlesParallel(context, targets, skillId) {
            ...
            const attackPromises = targets.map(async (target) => {
                try {
                    ...
                    if (result.success) {
                        this.state.stats.totalAttacks++;
                        logger.debug(`⚔️ Ataque em: ${target.name}`);
                    } else {
                        logger.debug(`❌ Falha no ataque: ${target.name}`);
                    }
        ...
        showFarmResultsModal(context, stats) {
            ...
            try {
                ...
                summary.messages.push(`⚡ Ultra Fast Farm: ${stats.totalMonsters} mobs atacados em ${durationSec}s`);
                summary.messages.push(`⚔️ ${stats.totalAttacks} ataques realizados em ${stats.totalBatches} lotes paralelos`);
                summary.messages.push(`✅ ${stats.successfulJoins} entradas bem-sucedidas`);
                
                if (stats.failedJoins > 0) {
                    summary.messages.push(`⚠️ ${stats.failedJoins} entradas falharam`);
                }
    ...
    const ultraFastLootModule = {
            ...
            if (autoTrigger) {
                context.events.on('autods:ultraFastAttack:complete', (data) => {
                    if (data.triggerLoot && !this.state.running) {
                        context.logger.info('[UltraFastLoot] Auto-triggered após farm');
        ...
        async runLoot(context) {
            if (this.state.running) {
                context.logger.warn('[UltraFastLoot] Já está rodando');
                return;
            }
            ...
            try {
                logger.info('🎁 Ultra Fast Loot iniciado!');
                ...
                if (targets.length === 0) {
                    logger.info('❌ Nenhum mob elegível para loot');
                    this.state.running = false;
                    return;
                }

                logger.info(`🎯 ${targets.length} mobs para lootar`);
                if (includeNames.length > 0) {
                    logger.debug(`ℹ️  Contagem APÓS filtro 'Incluir Nomes': ${targets.length} mobs elegíveis`);
                    logger.debug(`📋 Filtros incluir: ${includeNames.join(', ')}`);
                }
                if (excludeNames.length > 0) {
                    logger.debug(`📋 Filtros excluir: ${excludeNames.join(', ')}`);
                }
                ...
                // 🆕 ROBUSTO: Implementar loot manual com verificação de level up após cada batch
                ...
                for (let i = 0; i < targets.length; i += batchSize) {
                    if (levelUpDetected) {
                        logger.warn(`⏹️  Level up detectado! Parando o loot automático`);
                        break;
                    }
                ...
                // Update module stats (já foi atualizado durante lootBatchParallel)
                this.state.stats.totalExp = summary.exp;
                this.state.stats.totalGold = summary.gold;

                // Summary
                const skippedCount = targets.length - looted;
                logger.info(`✨ Loot concluído!`);
                logger.info(`📊 ${this.state.stats.successful}/${this.state.stats.attempted} loots bem-sucedidos`);
                if (skippedCount > 0) {
                    logger.info(`⏭️  ${skippedCount} mobs não looteados (parado no level up)`);
                }
                logger.info(`💰 ${numbers.format(summary.gold)} Gold, ${numbers.format(summary.exp)} EXP`);
                if (summary.items.size > 0) {
                    logger.info(`🎁 ${summary.items.size} tipos de itens obtidos`);
                }
                ...
                logger.info('✅ Ultra Fast Loot concluído! Veja os resultados no modal e painel lateral.');
        ...
        async scanLootTargets(context) {
            ...
            if (isGuildDungeon) {
                ...
                for (const card of monsterCards) {
                    ...
                    // 👑 Skip bosses (100B+ HP) - they must be looted manually
                    if (hp && hp >= BOSS_HP_THRESHOLD) {
                        context.logger.debug(`↩️  Pulando boss ${monsterName} (HP: ${formatNumber(hp)}) - deve ser looteado manualmente`);
                        continue;
                    }
            ...
            const lootPromises = batch.map(async (target) => {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        // 🆕 Acumular em summary (compatível com loot tradicional)
                        ...
                        logger.debug(`✅ Loot: ${target.type} ${target.monsterId || target.dgmid} - ${exp} XP, ${gold} Gold`);
                    } else {
                        this.state.stats.failed++;
                        logger.debug(`❌ Falha loot: ${result.message}`);
                    }

                    return result;
                } catch (error) {
                    this.state.stats.failed++;
                    logger.error('❌ Erro no loot:', error);
        ...
        showLootResultsModal(context, summary) {
            context.logger.info('🔍 [showLootResultsModal] Iniciando...');
            
            // Usar a função do floatingHelpersModule se disponível
            const floatingHelpers = context.moduleRegistry?.getModule('floatingHelpers');
            
            context.logger.info(`🔍 [showLootResultsModal] floatingHelpers encontrado: ${!!floatingHelpers}`);
            
            if (floatingHelpers && typeof floatingHelpers.openQuickLootModal === 'function') {
                const isGuildDungeon = /guild_dungeon_location\.php/i.test(context.location.pathname);
                const scopeLabel = isGuildDungeon ? 'Guild' : 'Wave';
                
                context.logger.info(`🔍 [showLootResultsModal] Chamando openQuickLootModal (scope: ${scopeLabel})`);
                
                try {
                    floatingHelpers.openQuickLootModal(context, summary, scopeLabel);
                    context.logger.info('✅ [showLootResultsModal] Modal aberto com sucesso!');
                } catch (error) {
                    context.logger.error('❌ [showLootResultsModal] Erro ao abrir modal:', error);
                }
            } else {
                // Fallback: mostrar log simples
                context.logger.warn('[UltraFastLoot] Modal não disponível, resultados no console');
                context.logger.info(`📊 Total: ${formatNumber(summary.exp)} EXP, ${formatNumber(summary.gold)} Gold`);
                
                if (summary.items.size > 0) {
                    context.logger.info(`🎁 Itens (${summary.items.size} tipos):`);
    ...
    const ultraFastStaminaModule = {
        ...
        init(context) {
            this.state = {
                running: false,
                isLoggingIn: false,  // Lock para evitar múltiplos logins paralelos
        ...
        async runStaminaFarm(context) {
            ...
            logger.info('⚡ Ultra Fast Stamina iniciado!');
            ...
            try {
                ...
                if (chapters.length === 0) {
                    logger.warn('❌ Nenhum capítulo encontrado na página');
                    return;
                }

                logger.info(`📚 ${chapters.length} capítulos encontrados`);
                ...
                logger.info(`✨ ${unreactedChapters.length} capítulos sem reação`);
                
                if (unreactedChapters.length === 0) {
                    logger.info('✅ Todos os capítulos já foram reagidos!');
                ...
                logger.info(`📦 ${batches.length} lotes de ${batchSize} reações`);

                // 5. Process batches
                for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
                    const batch = batches[batchIndex];
                    logger.info(`⚡ Lote ${batchIndex + 1}/${batches.length}: ${batch.length} capítulos`);
                    ...
                    if (batchIndex < batches.length - 1) {
                        ...
                        logger.info(`⏳ Aguardando ${delay}ms antes do próximo lote...`);
                        await sleep(delay);
                    }
                }
        ...
        async reactBatchParallel(context, batch, userUID) {
            const { logger } = context;
            const cfg = context.config.get().ultraFastStamina;
            const reactionType = cfg?.reactionType || '1'; // Default: 👍
            ...
            const promises = batch.map(async (chapter) => {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        logger.debug(`✅ Reação enviada: ${chapter.title} (${chapter.id})`);
                        
                        // Set cookie to mark as reacted
                        this.setCookie(`reacted_chap_${chapter.id}`, reactionType, 365);
                    } else {
                        this.state.stats.failed++;
                        logger.debug(`❌ Falha: ${chapter.title} - ${result.message}`);
                    }

                    return result;
                } catch (error) {
                    this.state.stats.failed++;
                    logger.error(`❌ Erro: ${chapter.title}`, error);
                    return { success: false, message: error.message };
                }
            });
        ...
        async autoSignin(context) {
            ...
            // Se já está logando, aguarda o resultado
            ...
            this.state.loginPromise = (async () => {
                try {
                    ...
                    if (!credentials?.email || !credentials?.password) {
                        logger.warn('❌ Credenciais não configuradas. Configure email e senha em ⚙️ Interface > 🔐 Credenciais de Login');
                        return false;
                    }

                    logger.info('🔄 Fazendo login automático (uma única vez)...');
                    ...
                    // "You are already signed in" = sucesso (já estava logado ou login completou)
                    ...
                    if (isLoginPage && !isAlreadySignedIn) {
                        logger.error('❌ Falha no login - credenciais incorretas?');
                        return false;
                    }
        ...
        async sendReaction(context, chapterId, reactionType, userUID) {
            ...
            try {
                ...
                if (this.isLoggedOut(result.data)) {
                    // Se não está logando, inicia o login (com lock)
                    if (!this.state.isLoggingIn) {
                        context.logger.warn('⚠️ Sessão expirada, iniciando login único...');
                    } else {
                        context.logger.debug('⏳ Sessão expirada, aguardando login em andamento...');
                    }
                    ...
                    if (!loginSuccess) {
                        return {
                            success: false,
                            message: 'Sessão expirada e falha no login automático',
                            statusCode: 401
                        };
                    }

                    // Retry the reaction after login
                    context.logger.debug('🔄 Tentando reação novamente após login...');
        ...
        showResults(context) {
            ...
            logger.info('✨ Ultra Fast Stamina concluído!');
            logger.info(`📊 Stats: ${stats.successful}/${stats.attempted} reações enviadas`);
            logger.info(`⏱️ Duração: ${(stats.duration / 1000).toFixed(1)}s`);
            ...
            const durationSec = (stats.duration / 1000).toFixed(1);
            summary.messages.push(`⚡ Ultra Fast Stamina: ${stats.successful} reações enviadas em ${durationSec}s`);
            summary.messages.push(`✅ ${stats.successful} sucessos, ❌ ${stats.failed} falhas`);
            summary.messages.push(`🔄 ${stats.alreadyReacted} capítulos já reagidos (pulados)`);
    ...
    const ultraFastDungeonModule = {
        ...
        triggerManualRun(context) {
            ...
            logger.info('⚡ Ultra Fast Dungeon iniciado manualmente!');
        ...
        async runDungeonFarm(context) {
            ...
            for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
                ...
                if (joinedTargets.length === 0) {
                    logger.warn('⚠️ Nenhuma entrada bem-sucedida neste lote');
                    continue;
                }
        ...
        async scanDungeonTargets(context) {
            ...
            const monsterFilters = rawMonsterNames.map(entry => {
                ...
                // Formato simples (só nome)
                return { name: str, customDamage: null };
            }).filter(f => f.name.length > 0);
            
            // Log active filters
            if (includeNames.length > 0) {
                logger.info(`[UltraFastDungeon] ✓ Incluindo APENAS: ${includeNames.join(', ')}`);
            }
            if (excludeNames.length > 0) {
                logger.info(`[UltraFastDungeon] ✗ Excluindo: ${excludeNames.join(', ')}`);
            }
            ...
            if (excludeNames.length > 0) {
                ...
                logger.info(`[UltraFastDungeon] Após filtro de exclusão: ${beforeCount} → ${monsters.length} monstros`);
            }
            ...
            const filteredTargets = targets.filter(target => {
                ...
                if (target.customDamageLimit) {
                    ...
                    logger.info(`[UltraFastDungeon] 👑 ${target.name}: Boss especial - alvo: ${formatNumber(damageThreshold)} (mínimo 50M para loot)`);
                } else {
                ...
                logger.debug(`[UltraFastDungeon] ✓ ${target.name}: HP ${formatNumber(target.currentHp)}, Dano ${formatNumber(target.currentDamage)}/${formatNumber(effectiveLimit)} (faltam ${formatNumber(target.remainingDamage)})`);
                return true;
            });
        ...
        async fetchCurrentDamages(context, targets) {
            ...
            logger.info(`🔍 Verificando dano atual de ${joinedTargets.length} batalhas já iniciadas...`);
        ...
        async ensureBattlesJoined(context, targets) {
            ...
            // Separar alvos que já estão joined dos que precisam join
            const alreadyJoined = targets.filter(t => t.hasJoined);
            const needJoin = targets.filter(t => !t.hasJoined);

            // Adicionar os que já estão joined
            alreadyJoined.forEach(target => {
                joinedTargets.push(target);
                logger.debug(`[UltraFastDungeon] ${target.name}: Já está joined`);
            });
            ...
            // 🔥 PRE-FLIGHT ATTACK: Preparar TODAS as requisições de join antes de enviar
            logger.info(`🚀 PRE-FLIGHT JOIN: Preparando ${needJoin.length} requisições simultâneas...`);
            
            // Criar array de promises para joins simultâneos
            const joinPromises = needJoin.map(target => {
                // Retornar uma função que cria a promise (não a promise diretamente)
                return {
                    target,
            ...
            logger.info(`✅ Pre-Flight concluído em ${duration}ms`);
            ...
            results.forEach(({ target, result }) => {
                if (result.success || result.cloudflareBlock) {
                    ...
                    logger.warn(`  ⚠️ ${target.name}: Falha - ${result.message}`);
                    this.state.stats.failedJoins++;
                    failCount++;
                }
            });
        ...
        async attackWithSmartDamage(context, targets) {
            ...
            if (targets.length === 0) {
                logger.warn('⚠️ Nenhum alvo para atacar');
                return;
            }
            ...
            for (let round = 0; round < cfg.attacksPerMonster; round++) {
                ...
                if (activeTargets.length === 0) {
                    logger.info(`✅ Todos os alvos atingiram threshold ou completaram ataques`);
                    break;
                }

                logger.info(`⚔️ Rodada ${round + 1}/${cfg.attacksPerMonster}: Atacando ${activeTargets.length} monstros em PARALELO...`);
                ...
                const attackPromises = activeTargets.map(targetState => {
                    ...
                    if (cfg.useSmartDamage) {
                        // Ultra Fast Dungeon usa smartDamage de battle.smartDamage, mas só precisa de useSmartDamage=true
                        // Se smartDamage não estiver configurado, usar skills padrão
                ...
                for (const { targetState, attackResult, error } of results) {
                    if (error) {
                        logger.error(`  ❌ ${targetState.name}: Erro - ${error.message}`);
                        continue;
                    }
                ...
                logger.info(`📊 Rodada ${round + 1}: ${successCount} ataques OK, ${thresholdCount} thresholds (${duration}ms)`);

                // Pequeno delay entre rodadas (não entre ataques individuais)
        ...
        showResults(context) {
            ...
            logger.info('✨ Ultra Fast Dungeon concluído!');
            logger.info(`📊 Stats: ${stats.totalAttacks} ataques, ${stats.monstersCompleted} mobs`);
            logger.info(`⏱️ Duração: ${(duration / 1000).toFixed(1)}s`);
            ...
            summary.messages.push(`⚡ Ultra Fast Dungeon: ${stats.monstersCompleted} mobs em ${durationSec}s`);
            summary.messages.push(`⚔️ ${stats.totalAttacks} ataques enviados`);
            summary.messages.push(`✅ ${stats.successfulJoins} joins, ❌ ${stats.failedJoins} falhas`);
    ...
    const pvpModule = {
        ...
        async runAutoMatchmakingLoop(context) {
            ...
            logger.info('🏟️ Ultra Fast PvP: Starting auto matchmaking loop...');
            ...
            while (this.state.running && attemptCount < maxAttempts) {
                ...
                if (!tokens) {
                    logger.warn('⚠️ Could not read PvP tokens from page');
                    await sleep(1000);
                    continue;
                }
                
                logger.debug(`⚔️ PvP Tokens: ${tokens.current}/${tokens.max}`);
                
                // Check if we have tokens
                if (tokens.current < 1) {
                    logger.warn('⚠️ No tokens available');
                    logger.info('⏳ Aguardando 1 hora para regenerar tokens...');
                ...
                logger.debug(`[UltraFastPvP] Matchmaking result: ${matchResult.success ? '✅' : '❌'} - ${matchResult.error || matchResult.message}`);
                
                if (!matchResult.success) {
                    logger.warn(`❌ Matchmaking failed: ${matchResult.error || matchResult.message}`);
                    await sleep(2000);
                    continue;
                }
                
                logger.info('✅ Matchmaking successful, waiting for battle page to load...');
                ...
                if (currentPath.includes('pvp_battle')) {
                    logger.info('✨ Battle page loaded, starting battle automation...');
                    ...
                    logger.warn('⚠️ Matchmaking found, but page did not change. Looking for Start Matching button...');
                    ...
                    if (battleBtn) {
                        ...
                        if (currentPath.includes('pvp_battle')) {
                            logger.info('✨ Battle page loaded after button click, starting battle automation...');
                            await this.handleUltraFastBattle(context);
                            await sleep(1000);
                        } else {
                            logger.warn('⚠️ Still not on battle page after button click');
                            await sleep(2000);
                        }
                    } else {
                        logger.warn('⚠️ Could not find Start Matching button on page');
                        await sleep(2000);
                    }
                }
            }
            
            logger.info('🏁 Auto matchmaking loop ended');
        ...
        async handleUltraFastBattle(context) {
            ...
            logger.info(`⚔️ Ultra Fast PvP Battle Started!${rotationMode}`);
            ...
            while (this.state.running) {
                ...
                if (!currentPath.includes('pvp_battle')) {
                    logger.info('🔄 No longer on battle page, battle ended');
                    break;
                }
                
                // Check DOM for battle end
                if (this.isBattleEnded()) {
                    logger.info('🏁 Battle ended (detected via DOM)');
                    break;
                }
                ...
                if (result.success && result.data) {
                    ...
                    if (data.your_damage || data.enemy_damage) {
                        logger.debug(`⚔️ Hit #${attackCount}: You dealt ${data.your_damage} dmg, took ${data.enemy_damage} dmg`);
                    }
                    
                    // Check if battle ended from response
                    if (data.ended === 1) {
                        logger.info(`🏁 Battle ended (response indicated) after ${attackCount} attacks`);
                        // Check who won
                    ...
                    if (data.my_hp !== undefined && data.my_hp <= 0) {
                        logger.info('💀 You died - battle lost');
                        this.state.battlesLost++;
                        break;
                    }
                    if (data.enemy_hp !== undefined && data.enemy_hp <= 0) {
                        logger.info('🏆 Enemy defeated - battle won!');
                        this.state.battlesWon++;
                        break;
                    }
                    ...
                    if (data.attacker_tokens !== undefined) {
                        logger.debug(`📊 Your tokens: ${data.attacker_tokens}`);
                    }
                } else {
                    consecutiveErrors++;
                    logger.warn(`⚠️ Attack error (${consecutiveErrors}/${maxConsecutiveErrors}): ${result.message || result.error}`);
                    
                    if (consecutiveErrors >= maxConsecutiveErrors) {
                        logger.warn('❌ Too many consecutive errors, checking if battle ended via DOM...');
                        await sleep(500);
                        
                        if (this.isBattleEnded()) {
                            logger.info('🏁 Battle ended (confirmed via DOM)');
                            break;
                        }
            ...
            logger.info(`✅ Battle #${this.state.battlesCompleted} completed in ${(battleDuration / 1000).toFixed(1)}s (${attackCount} attacks)`);
         ...
         */
        getPvpTokens() {
            // Look for token display in the info box: "⚔️ 30 /30"
            const tokenElements = Array.from(document.querySelectorAll('*')).filter(el => {
                return el.textContent.match(/⚔️\s*(\d+)\s*\/\s*(\d+)/);
            });
            
            for (const el of tokenElements) {
                const match = el.textContent.match(/⚔️\s*(\d+)\s*\/\s*(\d+)/);
                if (match) {
                    return { 
        ...
        showStats(context) {
            ...
            logger.info(`📊 Ultra Fast PvP Stats:
  ├─ Battles: ${this.state.battlesCompleted} (${this.state.battlesWon}W/${this.state.battlesLost}L) - ${winRate}% win rate
  ├─ Total Attacks: ${this.state.attacksSent}
  └─ Session Duration: ${(duration / 60).toFixed(1)} minutes`);
            
            // Show notification
            context.notifications.info(`🏟️ PvP: ${this.state.battlesCompleted} battles (${winRate}% win)`);
        }
    };
    ...
    const attackUnlockerModule = {
        ...
        activate(context) {
            ...
            if (cfg.core?.unlockAttackButtons === false) {
                context.logger.debug('Attack unlocker desabilitado via configuração');
                this.cleanup();
                return;
            }
            ...
            // context.logger.info('🔓 Ativando desbloqueio permanente de botões de ataque...');
            ...
            context.logger.info('✅ Desbloqueio de ataques ativo! Você pode atacar sem delay de 1s.');
        },
        cleanup() {
        ...
        injectGlobalBypass(context) {
            try {
                ...
                context.logger.info('✅ Bypass global de lockAttackButtons injetado');
            } catch (error) {
        ...
        startButtonUnlocker(context) {
            ...
            this.state.intervalId = setInterval(() => {
                ...
                if (buttons.length > 0) {
                    ...
                    // context.logger.debug(`🔓 Desbloqueados ${buttons.length} botões de ataque`);
                }
            ...
            context.logger.info('✅ Intervalo de desbloqueio automático iniciado (50ms)');
        },
        interceptClicks(context) {
            ...
            document.addEventListener('click', (event) => {
                ...
                if (target &&
                    ...
                    // context.logger.debug('🔓 Botão de ataque desbloqueado via interceptor de clique');
                }
    ...
    const directJoinButtonsModule = {
        ...
        injectButtons(context) {
            ...
            setTimeout(() => {
                ...
                logger.info(`[DirectJoinButtons] ✅ Added ${buttonsAdded} direct join buttons to ${cards.length} cards`);
    ...
    const collectionsOrganizerModule = {
        ...
        reorganizeCollections(context) {
            ...
            context.logger.info('[CollectionsOrganizer] ✓ Collections reorganized by completion %');
            
            // Show notification
            context.notifications.success(`📚 Collections organized: sorted by completion (${claimed.length} claimed at bottom)`);
    ...
    const imageBlockModule = {
        ...
        shouldBlockCurrentPath(context) {
            const pathname = context.location?.pathname || window.location?.pathname || '';
            // Bloquear imagens em todas as páginas exceto guild e dungeon_info
    ...
    const dungeonLootHelperModule = {
        ...
        activate(context) {
            ...
            context.logger.info('🎁 Adicionando botões de loot rápido nas dungeon locations...');
        ...
        injectLootButtons(context) {
            ...
            if (injectedCount > 0) {
                context.logger.info(`✅ ${injectedCount} botões de loot adicionados`);
            } else {
                context.logger.debug('Nenhum monstro elegível para loot encontrado');
            }
        },
            ...
            try {
                ...
                if (!result.success) {
                    ...
                    if (/cloudflare/i.test(msg)) {
                        context.notifications.error('❌ Bloqueio Cloudflare: O loot automático foi bloqueado. Por favor, faça o loot manualmente.', 10000);
                        context.notifications.error('❌ Bloqueio Cloudflare: Faça o loot manualmente clicando no botão "Loot" do monstro.', 10000);
                    }
                ...
                context.logger.info(`✅ Loot claimed for ${monsterName}: +${result.exp} XP, +${result.gold} Gold, ${result.items.length} items`);
    ...
    const floatingHelpersModule = {
        ...
        activate(context) {
            ...
            // 🆕 Listen for config changes to update loot button appearance
            context.events.on('config:change', () => {
                ...
                if (newCfg.floatingHelpers?.enabled && this.state.container) {
                    ...
                    // 🆕 Recreate farm box if Ultra Fast Farm toggle changed
                    this.recreateFarmBox(context);
                }
            });
            
            // 🆕 Listen for autods:config:updated to handle panel config changes
            context.events.on('autods:config:updated', () => {
                const newCfg = context.config.get();
                if (newCfg.floatingHelpers?.enabled && this.state.container) {
        ...
        createFloatingContainer(context, cfg) {
            ...
            // Adicionar botão de toggle minimizar/maximizar (mobile only)
            ...
            toggleBtn.addEventListener('click', (e) => {
                ...
                toggleBtn.textContent = isMinimized ? '▲ Minimizar' : '▼ Expandir';
                console.log('[FloatingHelpers] Toggle:', isMinimized ? 'Expandido' : 'Minimizado');
            });
            container.appendChild(toggleBtn);

            // Adicionar botão de loot rápido
            ...
            // Adicionar 4 botões principais de abas (Farm, Dungeon, PvP, Boss)
            ...
            // Integrar botão nativo de battle drawer (🧪)
            this.integrateBattleDrawerButton(context);

            // Adicionar botão de Damage Calculator (após items)
        ...
        createLootBox(context, cfg) {
            const box = document.createElement('div');
            box.className = 'loot-box autods-floating-loot-btn';  // 🆕 Adicionada classe para seleção
            
            // Verificar se Ultra Fast Loot está ativo
            ...
            icon.textContent = '💰';
            icon.className = 'loot-icon';  // 🆕 Classe para o ícone
            ...
            label.className = 'loot-label';  // 🆕 Classe para o label
        ...
        createStaminaBox(context, cfg) {
            ...
            box.addEventListener('click', async () => {
                context.logger.info('⚡ Iniciando Ultra Fast Stamina...');
                
                // Get module
                const ultraStaminaModule = context.moduleRegistry?.getModule('ultraFastStamina');
                if (!ultraStaminaModule) {
                    context.logger.error('Ultra Fast Stamina module não encontrado');
                    context.notifications.error('Ultra Fast Stamina module não encontrado!');
                    return;
                }
        ...
        createFarmBox(context, cfg) {
            ...
            box.addEventListener('click', () => {
                ...
                if (!/active_wave\.php/i.test(pathname)) {
                    context.logger.warn('[FloatingHelpers] Farm box só funciona em active_wave.php');
                    context.notifications.warn('Farm: acesse uma página de wave!');
                    return;
                }
                
                // Executar ultraFastAttackModule para um batch único de operações
                context.logger.info('[FloatingHelpers] 🚀 Iniciando Ultra Fast Attack (batch único)...');
                
                // Ativar ultraFastAttack config se necessário
                const cfg = context.config.get();
                if (!cfg.ultraFastAttack?.enabled) {
                ...
                if (ultraFastAttackModule && typeof ultraFastAttackModule.triggerManualRun === 'function') {
                    context.logger.info('[FloatingHelpers] ✅ Disparando ultraFastAttackModule.triggerManualRun()');
                    ultraFastAttackModule.triggerManualRun(context);
                } else {
                    context.logger.error('[FloatingHelpers] ❌ Ultra Fast Farm module não encontrado ou não tem triggerManualRun!');
                    context.notifications.error('Farm box: módulo não encontrado!');
                }
            });
        ...
        createMainTabButtons(context, cfg) {
            ...
            buttons.forEach(btn => {
                ...
                button.innerHTML = `<span style="font-size: 16px;">${btn.emoji}</span> ${btn.label}${btn.id === 'dungeon' ? ' ⚡' : ''}`;

                button.addEventListener('click', async () => {
                    // Comportamento especial para Dungeon: executa em vez de toggle
                    if (btn.id === 'dungeon') {
                        context.logger.info('🏰 Executando Ultra Fast Dungeon...');
                        const dungeonModule = context.moduleRegistry?.getModule('ultraFastDungeon');
                        if (!dungeonModule) {
                            context.logger.error('Ultra Fast Dungeon module não encontrado');
                            context.notifications.error('Dungeon: módulo não encontrado!');
                            return;
                        }
                        try {
                            if (typeof dungeonModule.triggerManualRun === 'function') {
                                ...
                                context.logger.error('Dungeon: método triggerManualRun não existe');
                                context.notifications.error('Dungeon não está configurado corretamente!');
                            }
                        } catch (error) {
                    ...
                    if (btn.id === 'farm') {
                        ...
                        // Apenas executar em active_wave.php
                        if (!/active_wave\.php/i.test(pathname)) {
                            context.logger.warn('[FloatingHelpers] Farm só funciona em active_wave.php');
                            context.notifications.warn('Farm: acesse uma página de wave!');
                            return;
                        }
                    ...
                    // Comportamento padrão para PvP, Boss: toggle flag
                    const cfg = context.config.get();
                    const parts = btn.config.split('.');
                    let obj = cfg;
        ...
        recreateFarmBox(context) {
            ...
            // Se Attack não está habilitado, remover o botão
            if (!cfg.ultraFastAttack?.enabled) {
            ...
            // Se Attack está habilitado mas botão não existe, criar
            const existingFarmBox = this.state.container?.querySelector('.autods-floating-farm-btn');
            if (!existingFarmBox) {
                // Inserir após o botão de stamina (se existir) ou ao final
            ...
            // Se já existe, apenas atualizar aparência
            const newFarmBox = this.createFarmBox(context, cfg);
        ...
        integrateBattleDrawerButton(context) {
            // Procurar o botão nativo de battle drawer
            const nativeButton = document.getElementById('openBattleDrawerBtn');
            if (!nativeButton) {
                context.logger.debug('[FloatingHelpers] Botão nativo #openBattleDrawerBtn não encontrado');
                return;
            }

            // Esconder o botão da sua posição original
            nativeButton.style.display = 'none';

            // Criar uma cópia estilizada para o floating helpers
            const box = document.createElement('div');
            ...
            // Ao clicar no box, clicar no botão nativo
            box.addEventListener('click', () => {
                nativeButton.click();
                ...
                if (closeAction) {
                    ...
                    context.logger.debug('[FloatingHelpers] Fechar painel não encontrado após abrir drawer');
                }
            });
            ...
            if (container) {
                ...
                context.logger.debug('[FloatingHelpers] Botão de Battle Drawer integrado');
            }
        },
        openBattleDrawer(context) {
            // Abrir o Battle Drawer nativo da página adicionando a classe 'drawer-open' ao body
            ...
            if (body) {
                ...
                if (drawer) {
                    ...
                    context.logger.warn('[FloatingHelpers] Battle Drawer (#battleDrawer) não encontrado na página');

                    // Tentar usar o botão nativo como fallback
                    const openBtn = context.document.getElementById('openBattleDrawerBtn');
                    if (openBtn) {
                        context.logger.debug('[FloatingHelpers] Clicando no botão nativo de abrir drawer');
                        openBtn.click();
                    }
                }
            }
        ...
        updateUltraFastStats(context, stats, type) {
            ...
            if (type === 'farm') {
                ...
                html = `
                    <div style="margin-bottom: 4px; color: #a6e3a1; font-weight: bold;">🌾 Farm Complete!</div>
                    <div>⏱️ ${duration}s</div>
                    <div>📦 ${stats.totalBatches || 0} lotes</div>
                    <div>👹 ${stats.totalMonsters || 0} mobs</div>
                    <div style="color: #a6e3a1;">✅ ${stats.successfulJoins || 0} entradas</div>
                    ${(stats.failedJoins || 0) > 0 ? `<div style="color: #f38ba8;">❌ ${stats.failedJoins} falhas</div>` : ''}
                    <div>⚔️ ${stats.totalAttacks || 0} ataques</div>
                `;
            } else if (type === 'loot') {
                // 🆕 Usar dados do summary se disponível
                ...
                html = `
                    <div style="margin-bottom: 4px; color: #f9e2af; font-weight: bold;">💰 Loot Complete!</div>
                    <div>📦 ${stats.attempted || 0} mobs</div>
                    <div style="color: #a6e3a1;">✅ ${stats.successful || 0} loots</div>
                    ${(stats.failed || 0) > 0 ? `<div style="color: #f38ba8;">❌ ${stats.failed} falhas</div>` : ''}
                    <div>✨ ${formatNumber(exp)} EXP</div>
                    <div>💰 ${formatNumber(gold)} Gold</div>
                    ${itemCount > 0 ? `<div>🎁 ${itemCount} ${summary ? 'tipos' : 'itens'}</div>` : ''}
                `;
            }
        ...
        hideNativeBattleConsumables(context) {
            ...
            if (nativePopup) {
                ...
                // Tentar encontrar pelo conteúdo (complementary role com "Battle Consumables")
        ...
        // REMOVIDO: startExpPotionTimer() - depende de funções removidas
        // REMOVIDO: updateExpPotionTimer() - depende de funções removidas
        // REMOVIDO: stopExpPotionTimer() - depende de funções removidas
        // REMOVIDO: restoreExpPotionTimer() - depende de funções removidas
        async useExpPotion(context) {
            // Verificar se temos dados do inventário
            if (!this.state.inventoryData || !this.state.inventoryData.exp) {
                context.logger.warn('Dados do inventário não disponíveis. Atualizando...');
                await this.updateAllPotionCounts(context);
            }

            const itemData = this.state.inventoryData.exp;
            if (!itemData || !itemData.invId) {
                context.logger.error('Exp Potion S não encontrada no inventário');
                context.notifications.error('Exp Potion S not found in inventory');
                return;
            }

            if (itemData.quantity < 1) {
                context.logger.warn('Exp Potion S indisponível');
            ...
            try {
                ...
                if (!result.success) {
                    throw new Error(`Falha na requisição: ${result.message || 'Unknown error'}`);
                }
        ...
        getPlayerExpInfo() {
            ...
            // Se não encontrou pela estrutura, tentar via regex no body
        ...
        async performQuickLoot(context) {
            context.logger.info('Executando loot rápido...');

            // Verificar se estamos em página de dungeon/wave ou guild dungeon
            ...
            if (!isWavePage && !isGuildDungeon) {
                context.logger.warn('Loot rápido disponível apenas em páginas de dungeon');
                return;
            }
            ...
            if (ultraFastEnabled) {
                context.logger.info('⚡ Ultra Fast Loot está ativo! Usando modo paralelo...');
                
                // Obter referência ao módulo ultraFastLoot usando o método getModule
                ...
                if (ultraFastLootModule && typeof ultraFastLootModule.runLoot === 'function') {
                    ...
                    return; // Sair - Ultra Fast Loot já cuidou de tudo
                } else {
                    context.logger.warn('⚠️ Ultra Fast Loot module não encontrado, usando loot tradicional...');
                }
            }
            
            // FALLBACK: Loot tradicional (código original)
            ...
            if (!userId) {
                context.logger.error('❌ User ID não detectado. Faça login e tente novamente.');
                context.notifications.error('User ID not found. Please login.');
                return;
            }

            // 🆕 Verificar XP do jogador antes de iniciar loot
            const playerExpInfo = this.getPlayerExpInfo();
            if (playerExpInfo) {
                const expToLevelUp = playerExpInfo.maxExp - playerExpInfo.currentExp;
                context.logger.info(`📊 XP atual: ${formatNumber(playerExpInfo.currentExp)} / ${formatNumber(playerExpInfo.maxExp)} (${playerExpInfo.percent.toFixed(1)}%)`);
                context.logger.info(`📊 Falta ${formatNumber(expToLevelUp)} XP para subir de nível`);
            }

            // Para guild dungeons, buscar botões específicos
            if (isGuildDungeon) {
                const guildLootButtons = Array.from(document.querySelectorAll('.autods-loot-btn'));

                if (guildLootButtons.length === 0) {
                    context.logger.info('Nenhum monstro elegível para loot no guild dungeon');
                    return;
                }

                // 🆕 Feedback visual inicial
                context.logger.info(`🎁 Feedback: Iniciando loot de ${guildLootButtons.length} monstros...`);
                ...
                if (context.ui?.showToast) {
                    ...
                    context.notifications.info(`🎁 Iniciando loot de ${guildLootButtons.length} monstros...`);
                }
                
                // Atualizar botão do floating helper se existir
                ...
                context.logger.debug(`[LootAll] Botão encontrado: ${!!lootHelperBtn}, Icon: ${!!lootIcon}, Label: ${!!lootLabel}`);
                ...
                if (lootHelperBtn && lootIcon && lootLabel) {
                    ...
                    // Guardar referências para restauração
                ...
                try {
                    context.logger.info(`✅ Loot de guild dungeon concluído: ${summary.drops}/${guildLootButtons.length} loots bem-sucedidos`);
                    
                    // Show which monsters were looted
                    const monsterNames = Array.from(summary.monsters.values()).map(m => m.name).join(' | ');
                    context.logger.info(`✅ [Dungeon] Mobs: ${monsterNames}`);
                    
                    const failed = guildLootButtons.length - summary.drops;
                    // Mostrar notificação de erro se todas as tentativas falharam
                    if (failed > 0 && summary.drops === 0) {
                        const errorMsg = `❌ Falha no Loot de Dungeon: Todas as ${failed} tentativas foram bloqueadas pelo Cloudflare. Use o botão individual de loot em cada monstro.`;
                        context.notifications.error(errorMsg);
                        showErrorNotification(errorMsg, 10000, context.logger);
                        context.logger.error(errorMsg);
                    } else if (failed > 0) {
                        const warningMsg = `⚠️ Loot de Dungeon parcialmente concluído: ${summary.drops} sucessos, ${failed} bloqueados pelo Cloudflare.`;
                    ...
                    context.logger.error(`Erro no loot rápido de guild dungeon: ${error.message}`);
                } finally {
                    // 🆕 Restaurar botão do floating helper
                    if (this.state.lootAllButton) {
                        this.state.lootAllButton.textContent = this.state.lootAllOriginalText || '💰 Loot All';
            ...
            for (const card of monsterCards) {
                ...
                // Verificar se o botão é "Loot" (não "Join" ou "Continue")
            ...
            if (eligibleMonsters.length === 0) {
                context.logger.info('Nenhum monstro elegível para loot na wave');
                return;
            }

            // 🆕 Feedback visual inicial para wave
            context.notifications.info(`🎁 Iniciando loot de ${eligibleMonsters.length} monstros...`);
            ...
            if (lootHelperBtn && lootIcon && lootLabel) {
                ...
                // Armazenar referências para restauração
                this.state.lootAllIcon = lootIcon;
                this.state.lootAllLabel = lootLabel;
                this.state.lootAllButton = lootHelperBtn;
            }
            ...
            try {
                context.logger.info(`✅ Loot de wave concluído: ${summary.drops}/${eligibleMonsters.length} loots bem-sucedidos`);
                
                // Show which monsters were looted
                const monsterNames = Array.from(summary.monsters.values()).map(m => m.name).join(' | ');
                context.logger.info(`✅ [Wave] Mobs: ${monsterNames}`);
                
                const failed = eligibleMonsters.length - summary.drops;
                
                // Mostrar notificação de erro se todas as tentativas falharam
                if (failed > 0 && summary.drops === 0) {
                    const errorMsg = `❌ Falha no Loot: Todas as ${failed} tentativas foram bloqueadas pelo Cloudflare. Use o botão individual de loot em cada monstro.`;
                    
                    context.notifications.error(errorMsg);
                    showErrorNotification(errorMsg, 10000, context.logger);
                    context.logger.error(errorMsg);
                } else if (failed > 0) {
                    const warningMsg = `⚠️ Loot parcialmente concluído: ${summary.drops} sucessos, ${failed} bloqueados pelo Cloudflare.`;
                    context.notifications.warn(warningMsg);
                    context.logger.warn(warningMsg);
                }
                ...
                context.logger.error(`Erro no loot rápido de wave: ${error.message}`);
            } finally {
                // 🆕 Restaurar botão do floating helper
                if (this.state.lootAllButton && this.state.lootAllIcon && this.state.lootAllLabel) {
                    this.state.lootAllIcon.textContent = '💰';
        ...
        formatMonsterSummary(summary) {
            ...
            summary.monsters.forEach(monster => {
                ...
                const ratioText = ratioParts.length ? ` (${ratioParts.join(' • ')})` : '';
        ...
        showLootSummary(context, summary, scopeLabel) {
            ...
            const fallbackMessage = summary.messages.length ? summary.messages[summary.messages.length - 1] : 'Loot concluído.';
            const message = parts.length ? parts.join(' • ') : fallbackMessage.replace(/<[^>]+>/g, '');
            ...
            try {
                ...
                if (summary.monsters && summary.monsters.size > 0) {
                    ...
                    summary.monsters.forEach(monster => {
                        ...
                        nameSpan.textContent = monster.count > 1 ? `${monsterName} ×${monster.count}` : monsterName;
                ...
                if (summary.items && summary.items.size > 0) {
                    ...
                    summary.items.forEach(item => {
                        ...
                        titleSpan.textContent = `${formatNumber(item.quantity)}× ${item.displayName}`;
                ...
                <div class="autods-layout">
                    <div class="autods-panel">
                        ...
                        <div class="autods-tabs-main">
                            ...
                            <button class="autods-tab" data-tab="ultrafastloot">🎁 Ultra Fast Loot</button>
                            <button class="autods-tab" data-tab="boss">🐉 Boss</button>
...
Scout" rows="2"></textarea>
                            ...
                            <div class="autods-tab-content" data-tab-content="boss">
                                ...
                                <div style="display: flex; gap: 8px; margin-bottom: 12px;">
                                    ...
                                    <button type="button" class="autods-btn autods-btn-secondary" data-action="boss-stop" style="flex: 1; background: linear-gradient(135deg, #6a2a2a 0%, #4a3a2a 100%); border: 1px solid #f88; color: #f88; padding: 8px 12px; border-radius: 4px; cursor: pointer;">⏹️ Stop</button>
                                    <button type="button" class="autods-btn autods-btn-secondary" data-action="boss-reset" style="flex: 1; background: linear-gradient(135deg, #3a4a2a 0%, #2a4a3a 100%); border: 1px solid #8f8; color: #8f8; padding: 8px 12px; border-radius: 4px; cursor: pointer;">🔄 Reset</button>
                                </div>
...
Magus" rows="2"></textarea>
                            ...
                            <div class="autods-tab-content" data-tab-content="battle">
                                <h4 style="margin-bottom: 16px; color: var(--accent-primary);">🗡️ Battle Config</h4>
                            ...
                            <div class="autods-tab-content" data-tab-content="pvp">
                                ...
                                <div class="autods-subsection">
                                    ...
                                    <label class="autods-checkbox" style="margin-top: 12px; background: linear-gradient(135deg, #1a3a2a 0%, #2a1a3a 100%); padding: 8px; border-radius: 6px; border: 1px solid #8fa;">
                                        ...
                                        <span>🔄 Smart Skill Rotation</span>
                                    </label>
                                    ...
                                    <div id="smartRotationConfig" style="margin-top: 10px; padding: 10px; background: rgba(26, 58, 42, 0.3); border-radius: 6px; border: 1px solid #4a8;">
                                        ...
                                        <div id="rotationItems" style="max-height: 300px; overflow-y: auto;">
                                            <!-- Items de rotação serão inseridos aqui dinamicamente -->
                                        </div>
                                    ...
                                    <label class="autods-checkbox" style="margin-top: 8px;">
                                        ...
                                        <span>Auto-iniciar próxima batalha</span>
                                    </label>
                                    <label class="autods-checkbox">
                            ...
                            <div class="autods-tab-content" data-tab-content="interface">
                                ...
                                <div class="autods-subsection">
                                    ...
                                    <label class="autods-field">
                                        <span>Senha</span>
                                        <input type="password" data-config="credentials.password" data-label="Senha" placeholder="●●●●●●●●" />
                                    </label>
                                    <p class="autods-info" style="font-size: 10px; color: #f88; margin-top: 8px;">
                                        ⚠️ As credenciais são salvas localmente. Nunca compartilhe seu config com outras pessoas.
                                    </p>
                                </div>
        ...
        bind(context) {
            ...
            this.elements.container.addEventListener('click', event => {
                ...
                if (target.dataset.action?.includes('boss') || target.closest('[data-action*="boss"]')) {
                    console.log('[UIShell] 🐉 Boss button detected! Action:', target.dataset.action);
                    console.log('[UIShell] Target element:', target);
                    console.log('[UIShell] Closest selector match:', target.closest('[data-action]'));
                }

                // Handle main tabs (Rápido, Farm, Dungeon, Battle, PvP, Interface)
                ...
                if (target.matches('[data-action="close-panel"]') || target.closest('[data-action="close-panel"]')) {
                    ...
                    if (shell) {
                        ...
                        if (isCollapsed) {
                            ...
                            this.showToast(context, 'info', 'Painel minimizado', 'Pressione Ctrl+Shift+P ou clique no botão flutuante para reabrir.');
                        }
                    }
                    return;
                }
                ...
                if (target.matches('[data-action="toggle-log-view"]') || target.closest('[data-action="toggle-log-view"]')) {
                    ...
                    btn.textContent = this.state.logViewExpanded ? '🔼' : '👁️';
                ...
                if (target.matches('[data-action="boss-start"]') || target.closest('[data-action="boss-start"]')) {
                    event.preventDefault();
                    console.log('[UI] ✅ Boss START button handler triggered!');
                    context.logger.info('[UI] 🐉 Iniciando Ultra Fast Boss...');
                ...
                if (target.matches('[data-action="boss-stop"]') || target.closest('[data-action="boss-stop"]')) {
                    event.preventDefault();
                    console.log('[UI] ✅ Boss STOP button handler triggered!');
                    context.logger.info('[UI] 🛑 Parando Ultra Fast Boss...');
            ...
            this.handlers = {
                ...
                battleComplete: evt => {
                    ...
                    addLog('success', `Batalha concluída (${reasonLabel})`, detail.timestamp);
                    this.showToast(context, 'success', 'Batalha PvE', `Concluída: ${reasonLabel}.`);
                },
                battleBlocked: () => {
                    addLog('warn', 'Limite de batalhas alcançado (5 simultâneas).', Date.now());
                ...
                pvpEnd: evt => {
                    ...
                    addLog('success', 'PvP: batalha concluída.', evt?.detail?.timestamp || Date.now());
                    this.showToast(context, 'success', 'PvP', 'Batalha concluída.');
                },
                pvpSurrender: evt => {
                    this.state.pvp.inBattle = false;
                    this.state.pvp.lastResult = { timestamp: evt?.detail?.timestamp || Date.now(), reason: 'surrender' };
                    addLog('warn', 'PvP: rendição automática executada.', evt?.detail?.timestamp || Date.now());
                    this.showToast(context, 'warn', 'PvP', 'Rendição automática executada.');
                },
                ready: evt => {
                    addLog('success', `Automação pronta (v${evt?.detail?.version || SCRIPT_VERSION}).`, Date.now());
                    this.showToast(context, 'success', 'AutoDS', 'Automação inicializada.');
                },
                farmChapter: evt => {
                    const detail = evt?.detail || {};
                    ...
                    addLog('info', 'Capítulo farm concluído.', detail.timestamp || Date.now());
                    this.showToast(context, 'info', 'Farming', href ? `Avançando para ${href}.` : 'Capítulo concluído.');
                },
                ...
                waveMaxBattles: evt => {
                    ...
                    addLog('warn', `Batalhas simultâneas no limite (${count}/${limit}). Reload em ${interval}s.`, detail.timestamp);
                    this.showToast(context, 'warn', 'Wave', `Limite de batalhas (${count}/${limit}). Aguardando...`);
                },
                waveReloading: evt => {
                    addLog('info', 'Recarregando página para verificar batalhas disponíveis...', evt?.detail?.timestamp || Date.now());
                }
            };
        ...
        addLog(level, message, timestamp = Date.now()) {
            ...
            // Manter até MAX_PERSISTENT_LOGS no storage, mas apenas 8 visíveis por padrão
            if (this.state.log.length > MAX_PERSISTENT_LOGS) {
                this.state.log = this.state.log.slice(0, MAX_PERSISTENT_LOGS);
            }

            // Auto-render após adicionar log para garantir que eventos apareçam imediatamente
            if (this._context) {
        ...
        updateStatusIndicators(context) {
            ...
            if (battleStatus) {
                ...
                battleStatus.textContent = active ? '✅' : '⭕';
            ...
            if (othersStatus) {
                ...
                othersStatus.textContent = active ? '✅' : '⭕';
        ...
        restoreDropdownStates(context) {
            ...
            // Restaurar tab principal (Rápido, Farm, etc)
        ...
        render(context) {
            ...
            if (this.elements.log) {
                ...
                if (!this.state.log.length) {
                    ...
                    // Mostrar apenas 8 mais recentes por padrão, ou todos se expandido
                    ...
                    // Mostrar indicador se há mais logs disponíveis
                    if (!this.state.logViewExpanded && this.state.log.length > 8) {
                        ...
                        more.textContent = `+ ${this.state.log.length - 8} eventos anteriores (clique no 👁️ para ver todos)`;
                        more.title = 'Clique no botão 👁️ no cabeçalho para ver todos os eventos';
            ...
            switch (format) {
                case 'percent': {
                    ...
                    if (!Number.isFinite(parsed)) {
                        return { error: 'Valor percentual inválido.' };
                    }
                ...
                case 'minutes': {
                    ...
                    const parsed = Number(normalised);
                    if (!Number.isFinite(parsed)) {
                        return { error: 'Valor em minutos inválido.' };
                    }
                ...
                case 'int': {
                    ...
                    if (!Number.isFinite(parsed)) {
                        return { error: 'Valor inteiro inválido.' };
                    }
                    return { value: ensureBounds(parsed) };
                }
                case 'string':
        ...
        updateConfigValue(context, path, value, logMessage) {
            ...
            if (logMessage) {
                ...
                // render() não é mais necessário aqui, addLog() já renderiza automaticamente
            }
        ...
        onConfigInputChange(context, element) {
            ...
            if (error) {
                ...
                this.showToast(context, 'error', 'Configurações', error);
            ...
            if (element.dataset.toast !== '0') {
                this.showToast(context, 'success', 'Configurações', message);
            }
        },
        ...
        startUltraFastBoss(context) {
            if (this.ultraFastBossState.running) {
                this.showToast(context, 'warn', 'Ultra Fast Boss', 'Já está em execução!');
                return;
            }
            ...
            if (!battleId || !isBattlePage) {
                this.showToast(context, 'error', 'Ultra Fast Boss', 'Navegue até battle.php ou dungeon_battle.php primeiro!');
                return;
            }
            ...
            if (previousBossId && previousBossId !== battleId) {
                this.addLog('info', `🔄 Boss ID mudou (${previousBossId} → ${battleId}), resetando contadores...`);
            }
            ...
            const battleType = isDungeon ? '🏰 Dungeon Boss' : '🐉 Boss';
        ...
        stopUltraFastBoss(context) {
            if (!this.ultraFastBossState.running) {
                this.showToast(context, 'warn', 'Ultra Fast Boss', 'Não está em execução!');
                return;
            }
            ...
            this.addLog('warn', `🛑 Ultra Fast Boss parado - Dano: ${context.numbers.formatCompact(this.ultraFastBossState.currentDamage)}, Ataques: ${this.ultraFastBossState.attackCount}, Poções: ${this.ultraFastBossState.potionsUsed}`);
        ...
        resetUltraFastBoss(context) {
            ...
            this.addLog('info', '🔄 Ultra Fast Boss - Contadores resetados');
        },
        ...
        async runUltraFastBossLoop(context) {
            ...
            while (state.running) {
                ...
                if (playerHP.current <= 0) {
                    this.addLog('warn', '💀 HP zerado! Tentando usar Heal Potion...');
                    ...
                    if (healResult.success) {
                        ...
                        this.addLog('error', '❌ Não foi possível usar Heal Potion - parando');
                        state.running = false;
                        break;
                    }
                }
                ...
                if (state.currentDamage >= state.targetDamage) {
                    this.showToast(context, 'success', '🎉 Ultra Fast Boss', `META ATINGIDA! ${context.numbers.formatCompact(state.currentDamage)}`);
                    this.addLog('success', `🎉 Ultra Fast Boss - META ATINGIDA: ${context.numbers.formatCompact(state.currentDamage)}`);
                    state.running = false;
                    break;
                }
                ...
                if (state.currentStamina < skillCost) {
                    if (!state.autoPotion) {
                        this.showToast(context, 'warn', 'Ultra Fast Boss', 'Stamina esgotada! Sem poções configuradas.');
                        state.running = false;
                        break;
                    }
                    
                    // Use potion and reload
                    this.addLog('info', '💊 Usando Full Stamina Potion...');
            ...
            this.addLog('success', `✅ Ultra Fast Boss finalizado - Dano: ${context.numbers.formatCompact(state.currentDamage)}, DPS: ${context.numbers.formatCompact(dps)}/s, Tempo: ${elapsed}s`);
         ...
         */
        getPlayerHP(context) {
            // Try #pHpText first: "💚 27,000 / 44,500 HP"
        ...
        async useHealPotion(context) {
            this.addLog('info', '💚 Usando Heal Potion...');
            
            // Use centralized inventory service for Heal Potion
            try {
                const result = await context.inventory.useHealPotion();
                if (result) {
                    this.addLog('success', '✅ Heal Potion usada com sucesso!');
                    // Wait for HP to update
                    await sleep(1000);
                    return { success: true };
                } else {
                    this.addLog('warn', '⚠️ Sem Heal Potions disponíveis!');
                    return { success: false };
                }
            } catch (error) {
                this.addLog('error', `❌ Erro ao usar heal: ${error.message}`);
                return { success: false };
            }
        },
        ...
        updateBossStatusUI(context) {
            ...
            if (runningEl) runningEl.textContent = state.running ? '🟢 Executando' : '🔴 Parado';
            
            const bossIdEl = shadow.querySelector('#autods-boss-id');
            if (bossIdEl) {
                const typeIcon = state.isDungeon ? '🏰' : '🐉';
        ...
        checkAndResumeUltraFastBoss(context) {
            ...
            try {
                ...
                if (progress.autoResume) {
                    ...
                    const battleType = progress.isDungeon ? '🏰 Dungeon Boss' : '🐉 Boss';
                    this.addLog('info', `🔄 Resumindo ${battleType} após reload...`);
                    ...
                    this.showToast(context, 'info', 'Ultra Fast Boss', 'Resumindo após poção...');
                    this.updateBossStatusUI(context);
                    this.runUltraFastBossLoop(context);
                }
            } catch (e) {
        ...
        // REMOVIDO: Modal de Configurações Avançadas (substituído por configurações inline no painel)
        ...
        ensureDesktopToggle(context, panelContainer) {
            ...
            // 🆕 Function to try inserting into floating helpers
    ...
    function addGuildDungeonLootAllButton() {
        // Verificar se estamos em página de guild dungeon
        ...
        // Buscar o elemento "👾 Monsters in this location"
        const monsterHeaders = Array.from(document.querySelectorAll('.h')).filter(h =>
            h.textContent.includes('👾 Monsters in this location')
        );
        ...
        `;
        lootAllBtn.innerHTML = `💰 Loot All (${lootButtons.length})`;
        lootAllBtn.title = 'Fazer loot de todos os monstros elegíveis nesta localização';
            ...
            try {
                ...
                for (let i = 0; i < buttons.length; i++) {
                    ...
                    try {
                        ...
                        success++;
                        lootAllBtn.innerHTML = `⏳ ${i + 1}/${buttons.length}...`;
                    } catch (error) {
                        failed++;
                ...
                // Resultado final
                lootAllBtn.innerHTML = `✅ ${success} looted!`;
    ...
    function addDungeonPills() {
        ...
        monsterCards.forEach(card => {
            ...
            // Extrair HP atual e máximo
            const hpNode = card.querySelector('[data-stat="hp"], .hp-text, .hp-value, .monster-hp, .hp-bar + div, .hp');
            if (!hpNode) return;

            const hpText = hpNode.getAttribute?.('data-current-hp') ?? hpNode.textContent ?? '';

            // Parse HP (formato: "❤️ 123,456 / 1,234,567 HP" ou "123456 / 1234567")
            ...
            if (showDamagePills && expectedDamage > 0) {
                ...
                damagePill.innerHTML = `<span>⚔️</span><span>~${formatNumber(expectedDamage)}</span>`;
            ...
            if (showExpPills && expectedExp > 0) {
                ...
                expPill.style.cssText = `
                    ...
                    cursor: help;
                `;
                expPill.title = `EXP estimado: ${formatNumber(expectedExp)} (base 10% do HP + bônus de ratio)`;
                expPill.innerHTML = `<span>✨</span><span>~${formatNumber(expectedExp)}</span>`;
            ...
            // Inserir pills no card (após HP bar)
            ...
            if (hpBar && hpBar.nextSibling) {
                ...
                // Fallback: adicionar antes dos botões
    ...
    function addSmartDamageDisplay() {
        ...
        // Verificar se já adicionamos (evitar duplicatas)
        ...
        // Pegar valores de defesa da config ou usar padrão
        const cfg = JSON.parse(localStorage.getItem('autods.v1:config') || '{}');
    ...
    const autoBossModule = {
        ...
        activate(context) {
            ...
            if (cfg.battle?.enabled) {
                context.logger.warn('⚠️ AutoBoss: Módulo battle regular está ativado. Desative-o para usar AutoBoss.');
                return;
            }
            ...
            if (!window._autoBossConfirmOverridden) {
                ...
                window.confirm = (message) => {
                    context.logger.debug(`✅ AutoBoss: Auto-aceitando confirm: "${message}"`);
                    return true; // Always accept
                };
                window._autoBossConfirmOverridden = true;
                context.logger.debug('🔧 AutoBoss: window.confirm() override ativado');
            }
            ...
            (async () => {
                try {
                    ...
                    context.logger.error('❌ Falha no AutoBoss', error);
                } finally {
                    this.state.running = false;
                }
            })();
        ...
        selectBestAttackButton(context, buttons, currentStamina) {
            if (!buttons || buttons.length === 0) {
                context.logger.debug('❌ AutoBoss: Nenhum botão disponível para seleção');
                return null;
            }

            // If stamina is 0 or negative, don't try to attack
            if (currentStamina <= 0) {
                context.logger.debug('❌ AutoBoss: Stamina = 0, nenhum ataque possível');
                return null;
            }
            ...
            if (bestButton) {
                ...
                context.logger.debug(`🎯 AutoBoss: Selecionado ${skillName} (custo: ${highestCost} stamina, disponível: ${currentStamina})`);
            } else {
                context.logger.debug(`❌ AutoBoss: Nenhum ataque disponível com stamina ${currentStamina}`);
            }
        ...
        async runAutoBoss(context) {
            ...
            while (true) {
                ...
                if (!currentCfg.core.enabled || !currentCfg.autoBoss?.enabled) {
                    logger.info('⏹️ AutoBoss: Desativado via configuração.');
                    break;
                }
                ...
                if (monsterHp !== null && monsterHp <= 0) {
                    logger.info('💀 AutoBoss: Monstro derrotado!');
                    logger.info(`📊 Estatísticas: ${this.state.attackCount} ataques, ${this.state.potionsUsed} poções usadas`);
                    break;
                }
                ...
                if (currentDamage >= damageTarget) {
                    logger.info(`✅ AutoBoss: Meta de dano atingida! ${formatNumber(currentDamage)} / ${formatNumber(damageTarget)}`);
                    logger.info(`📊 Estatísticas: ${this.state.attackCount} ataques, ${this.state.potionsUsed} poções usadas`);
                    break;
                }
                ...
                if (attackButtons.length === 0) {
                    logger.warn('⚠️ AutoBoss: Nenhum botão de ataque disponível.');
                    await sleep(1000);
                    continue;
                }
                ...
                if (!bestButton) {
                    ...
                    logger.warn(`⚠️ AutoBoss: Stamina insuficiente (${currentStamina}). Tentando usar Full Stamina Potion...`);
                    ...
                    if (!potionUsed) {
                        ...
                        logger.info(`📊 Dano total: ${formatNumber(currentDamage)} / ${formatNumber(damageTarget)}`);
                        logger.info(`📊 Estatísticas: ${this.state.attackCount} ataques, ${this.state.potionsUsed} poções usadas`);
                        break;
                    }
                    ...
                    logger.warn('⚠️ AutoBoss: Esperando reload após usar poção...');
                    await sleep(3000);
                ...
                if (reCheckStamina < skillCost) {
                    logger.warn(`⚠️ AutoBoss: Stamina mudou! Agora: ${reCheckStamina}, necessário: ${skillCost}. Pulando ataque.`);
                    await sleep(500);
                ...
                if (!cooldownCheck.ok) {
                    ...
                    logger.debug(`⏳ AutoBoss: Aguardando cooldown... ${waitSecs}s`);
                ...
                if (bestButton.disabled || bestButton.classList.contains('is-loading')) {
                    logger.debug('⚠️ AutoBoss: Botão ficou desabilitado. Aguardando...');
                    await sleep(500);
                    continue;
                }
                ...
                logger.debug(`⚔️ AutoBoss: ${skillName} executado! (Total: ${this.state.attackCount} ataques, Stamina restante: ~${reCheckStamina - skillCost})`);

                // Wait before next attack
                await sleepRandom(delayBase, variation, randomize);
            }

            logger.info('🏁 AutoBoss: Sessão finalizada.');
        },
        cleanup() {
            if (this.state) {
                this.state.running = false;
    ...
    const damageCalculatorModule = {
        ...
        activate(context) {
            ...
            context.logger.info('🧮 Damage Calculator widget ready (inicialmente oculto)...');
        ...
        show(context) {
            ...
            if (this.state.widget) {
                ...
                context.logger.info('🧮 Damage Calculator mostrado');
            }
        },
        ...
        hide(context) {
            if (this.state.widget) {
                ...
                context.logger.info('🧮 Damage Calculator ocultado');
            }
        },
        ...
        createWidget(context) {
            ...
            header.innerHTML = `
                ...
                <div style="display: flex; gap: 8px;">
                    <button id="autods-calc-refresh" style="
                        ...
                        font-size: 11px;
                        transition: all 0.2s;
                    " title="Refresh data">🔄 Refresh</button>
            ...
            const tabItems = [
                ...
                { id: 'pvm', label: '⚔️ PvM', title: 'PvM Damage' },
                { id: 'pvp', label: '🛡️ PvP', title: 'PvP Power' },
                { id: 'builds', label: '🏆 Builds', title: 'Best Pet Builds' }
            ];
            ...
            logger.info('✅ Damage Calculator widget created');
        },
            ...
            return `
                <div style="display: flex; flex-direction: column; gap: 12px;">
                    ...
                    <div style="background: rgba(20, 30, 50, 0.5); padding: 12px; border-radius: 8px; border: 1px solid rgba(70, 130, 220, 0.3);">
                        <div style="font-weight: 600; margin-bottom: 8px; color: #90caf9;">📊 Base Stats</div>
                    ...
                    <div style="background: rgba(20, 30, 50, 0.5); padding: 12px; border-radius: 8px; border: 1px solid rgba(70, 130, 220, 0.3);">
                        <div style="font-weight: 600; margin-bottom: 8px; color: #a78bfa;">🐾 Pets (Attack Team)</div>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 12px; margin-bottom: 12px;">
                            ...
                            ${pets.pvm_damage_bonus_percent > 0 ? `
                                <div style="grid-column: 1 / -1; padding-top: 6px; border-top: 1px solid rgba(255,255,255,0.1);">
                                    <div style="color: #69db7c; font-weight: 600;">⚡ PvM Bonus: +${pets.pvm_damage_bonus_percent.toFixed(1)}%</div>
                                </div>
                            ` : ''}
                        </div>
                                ...
                                ${pets.equipped_attack_team.map(pet => `
                                    <div style="background: rgba(15, 20, 35, 0.5); padding: 6px 8px; border-radius: 4px; margin-bottom: 4px; font-size: 11px;">
                                        ...
                                        <div style="color: #b8c5d6;">
                                            <span style="color: #ff6b6b;">🔪 ${pet.attack}</span> · 
                                            <span style="color: #4dabf7;">🛡️ ${pet.defense}</span> · 
                                            <span style="color: #a78bfa;">🧪 Lv.${pet.level}</span>
                                        </div>
                                        ${pet.effect ? `
                    ...
                    <div style="background: rgba(20, 30, 50, 0.5); padding: 12px; border-radius: 8px; border: 1px solid rgba(70, 130, 220, 0.3);">
                        <div style="font-weight: 600; margin-bottom: 8px; color: #ffd43b;">⚔️ Equipment (Attack Set)</div>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 12px;">
                    ...
                    <div style="background: rgba(20, 30, 50, 0.5); padding: 12px; border-radius: 8px; border: 1px solid rgba(70, 130, 220, 0.3);">
                        <div style="font-weight: 600; margin-bottom: 12px; color: #f06292;">📊 Pet Impact Analysis</div>
        ...
        renderPvMTab(context) {
            ...
            return `
                <div style="display: flex; flex-direction: column; gap: 12px;">
                    <div style="background: rgba(20, 30, 50, 0.5); padding: 12px; border-radius: 8px; border: 1px solid rgba(70, 130, 220, 0.3);">
                        <div style="font-weight: 600; margin-bottom: 12px; color: #90caf9;">⚙️ Settings</div>
                    ...
                    <div style="background: rgba(70, 180, 70, 0.1); padding: 16px; border-radius: 8px; border: 2px solid rgba(70, 180, 70, 0.4);">
                        ...
                        <div style="font-size: 11px; color: #b8c5d6; border-top: 1px solid rgba(70, 180, 70, 0.3); padding-top: 12px;">
                            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 6px;">
                                ...
                                <div style="color: #ffd43b;">× Stamina:</div><div style="text-align: right; color: #ffd43b;">×${pvm.breakdown.stamina_multiplier}</div>
                                ${pvm.breakdown.total_bonus_percent > 0 ? `
                                    ...
                                    <div style="text-align: right; color: #69db7c; font-weight: 600;">×${pvm.breakdown.bonus_multiplier}</div>
                                ` : ''}
                            </div>
                        </div>
                    </div>
            ...
            return `
                <div style="display: flex; flex-direction: column; gap: 12px;">
                    <div style="background: rgba(220, 70, 70, 0.1); padding: 16px; border-radius: 8px; border: 2px solid rgba(220, 70, 70, 0.4);">
                        ...
                        <div style="font-size: 11px; color: #b8c5d6; border-top: 1px solid rgba(220, 70, 70, 0.3); padding-top: 12px;">
                            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 6px;">
                                ...
                                <div>Base Defense (×0.7):</div><div style="text-align: right;">${pvp_attacker.breakdown.base_defense_contribution.toFixed(1)}</div>
                                <div>Pet Attack (×0.3):</div><div style="text-align: right;">${pvp_attacker.breakdown.pet_attack_contribution.toFixed(1)}</div>
                                <div>Equipment ATK (×0.4):</div><div style="text-align: right;">${pvp_attacker.breakdown.equip_attack_contribution.toFixed(1)}</div>
                                <div>Pet Defense (×0.3):</div><div style="text-align: right;">${pvp_attacker.breakdown.pet_defense_contribution.toFixed(1)}</div>
                                <div>Equipment DEF (×0.2):</div><div style="text-align: right;">${pvp_attacker.breakdown.equip_defense_contribution.toFixed(1)}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="background: rgba(70, 130, 220, 0.1); padding: 16px; border-radius: 8px; border: 2px solid rgba(70, 130, 220, 0.4);">
                        ...
                        <div style="font-size: 11px; color: #b8c5d6; border-top: 1px solid rgba(70, 130, 220, 0.3); padding-top: 12px;">
                            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 6px;">
                                ...
                                <div>Base Attack (×0.7):</div><div style="text-align: right;">${pvp_defender.breakdown.base_attack_contribution.toFixed(1)}</div>
                                <div>Pet Attack (×0.3):</div><div style="text-align: right;">${pvp_defender.breakdown.pet_attack_contribution.toFixed(1)}</div>
                                <div>Equipment ATK (×0.2):</div><div style="text-align: right;">${pvp_defender.breakdown.equip_attack_contribution.toFixed(1)}</div>
                                <div>Pet Defense (×0.3):</div><div style="text-align: right;">${pvp_defender.breakdown.pet_defense_contribution.toFixed(1)}</div>
                                <div>Equipment DEF (×0.4):</div><div style="text-align: right;">${pvp_defender.breakdown.equip_defense_contribution.toFixed(1)}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="background: rgba(255, 215, 0, 0.05); padding: 12px; border-radius: 8px; border: 1px solid rgba(255, 215, 0, 0.3); font-size: 11px; color: #b8c5d6;">
                        <div style="margin-bottom: 6px; color: #ffd43b; font-weight: 600;">💡 PvP Formula Info:</div>
                        <div>Attacker: ATK + 0.7×DEF + 0.3×PetATK + 0.4×EqATK + 0.3×PetDEF + 0.2×EqDEF</div>
                        <div>Defender: DEF + 0.7×ATK + 0.3×PetATK + 0.2×EqATK + 0.3×PetDEF + 0.4×EqDEF</div>
                    </div>
                </div>
            `;
        },
                ...
                <div style="display: flex; flex-direction: column; gap: 12px; max-height: 480px; overflow-y: auto;">
                    ...
                    ${builds.pvm.builds.length > 0 ? `
                        <div style="background: rgba(105, 219, 124, 0.1); padding: 12px; border-radius: 8px; border: 1px solid rgba(105, 219, 124, 0.3);">
                            <div style="font-weight: 600; color: #69db7c; margin-bottom: 10px; font-size: 13px;">🏆 Best PvM Builds</div>
                    ...
                    ${builds.pvp_attacker.builds.length > 0 ? `
                        <div style="background: rgba(255, 107, 107, 0.1); padding: 12px; border-radius: 8px; border: 1px solid rgba(255, 107, 107, 0.3);">
                            <div style="font-weight: 600; color: #ff6b6b; margin-bottom: 10px; font-size: 13px;">⚔️ Best PvP Attacker Builds</div>
                    ...
                    ${builds.pvp_defender.builds.length > 0 ? `
                        <div style="background: rgba(77, 171, 247, 0.1); padding: 12px; border-radius: 8px; border: 1px solid rgba(77, 171, 247, 0.3);">
                            <div style="font-weight: 600; color: #4dabf7; margin-bottom: 10px; font-size: 13px;">🛡️ Best PvP Defender Builds</div>
    ...
    function bootstrap() {
        ...
        // Inicializar Smart Damage Calculation na página de stats
        ...
        // Inicializar Dungeon Pills nas páginas de wave
        ...
        // logger.info('💡 User ID detection: getUserIdFromCookie() always checks "demon" cookie');
