// ==UserScript==
// 基于https://greasyfork.org/zh-CN/scripts/394216-%E7%BD%91%E7%9B%98%E9%93%BE%E6%8E%A5%E6%A3%80%E6%9F%A5
// @license GPL-V2
// @name         网盘检测
// @version      0.0.3
// @icon         https://drive.cookie987.tk/favicon.ico
// @author       Cookie987
// @description  标记百度云、蓝奏云、腾讯微云的链接状态.
// @match        *://**/*
// @connect      lanzoui.com
// @connect      lanzouo.com
...
// @noframes
// @namespace https://greasyfork.org/users/850318
// @downloadURL https://update.greasyfork.org/scripts/436800/%E7%BD%91%E7%9B%98%E6%A3%80%E6%B5%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/436800/%E7%BD%91%E7%9B%98%E6%A3%80%E6%B5%8B.meta.js
// ==/UserScript==
(function () {
    'use strict';
    ...
    container.define("api", ["http", "manifest", "oneData", "constant", "svgCrypt"], function (http, manifest, oneData, constant, svgCrypt) {
        ...
        obj.checkLinkLocal = function (shareSource, shareId, callback) {
            ...
            else if (shareSource == constant.sources.lanzoui) {
                obj.checkLinklanzoui(shareId, callback);
            }
        ...
        obj.checkLinklanzoui = function (shareId, callback) {
            var url;
            if (shareId.indexOf("http") < 0) {
                url = "https://www.lanzoui.com/" + shareId;
            }
            else {
                url = shareId;
            }
        ...
        obj.checkLinkTy189 = function (shareId, callback) {
            var url;
            if (shareId.indexOf("http") < 0) {
                url = "https://cloud.189.cn/web/share?code=" + shareId;
            }
            else {
                url = shareId;
            }
    ...
    container.define("app_check_url", ["router", "constant", "config", "option", "checkManage", "findAndReplaceDOMText", "$"], function (router, constant, config, option, checkManage, findAndReplaceDOMText, $) {
        ...
        obj.runMatch = function () {
            ...
            // 蓝奏云补SPAN
            obj.replaceTextAsLink(/(?:https?:\/\/)?www\.lanzoui\.com\/([a-zA-Z0-9_\-]{5,22})\b/gi, constant.sources.lanzoui, function (match) {
                return match[1];
            });
            ...
            $("a:not([one-link-mark])").each(function () {
                ...
                if (href) {
                    ...
                    } else if ((match = /(?:https?:\/\/)?www\.lanzoui\.com\/([a-zA-Z0-9_\-]{5,22})/gi.exec(href))) {
                        oneId = match[1];
                        oneSource = constant.sources.lanzoui;
                    }
                    else if ((match = /(?:https?:\/\/)?share\.weiyun\.com\/([a-zA-Z0-9_\-]{5,22})/gi.exec(href))) {
                        oneId = match[1];
                        oneSource = constant.sources.WEIYUN;
                    } else if ((match = /(?:https?:\/\/)?cloud\.189\.cn\/web\/share?code%3D\/([a-zA-Z0-9_\-]{8,14})/gi.exec(href))) {
                        oneId = match[1];
        ...
        obj.isTransEnable = function (shareSource) {
            ...
            else if (shareSource == constant.sources.lanzoui && option.isOptionActive(option.constants.lanzoui_TRANS)) {
                return true;
            }
        ...
        obj.isCheckEnable = function (shareSource) {
            ...
            else if (shareSource == constant.sources.lanzoui && option.isOptionActive(option.constants.lanzoui_CHECK)) {
                return true;
            }
        ...
        obj.buildShareUrl = function (shareId, shareSource) {
            ...
            if (shareSource == constant.sources.BAIDU) {
                ...
                shareUrl = obj.prefixs.lanzoui + shareId;
            }
            else if (shareSource == constant.sources.WEIYUN) {
