            ...
            setInterval(() => {
                ...
                for (let i = 0; i < mainlength; i+=2) {
                    ...
                    if(ChatPGT_StyleMode.chat === true){
                        ...
                        main_img.innerHTML = `<img alt="User" loading="lazy" width="24" height="24" decoding="async" data-nimg="1" class="rounded-sm" src="${ChatPGT_StyleMode.chat_avatar}" style="color: transparent;">`
        ...
        registerMenuCommand() {
            GM_registerMenuCommand('设置', () => {
                let html = `<div style="font-size: 1em;">
                              <label class="panai-setting-label">用户<input type="checkbox" id="User"} class="panai-setting-checkbox" ${ChatPGT_StyleMode.user ? 'checked' : ''}></label>
                              <label class="panai-setting-label">用户昵称<input type="text" id="User-Name"} value="${ChatPGT_StyleMode.user_name}"></label>
                              <label class="panai-setting-label">用户头像<input type="text" id="User-Avatar"} value="${ChatPGT_StyleMode.user_avatar}"></label>
                              <label class="panai-setting-label">ChatCPT<input type="checkbox" id="Chat"} class="panai-setting-checkbox" ${ChatPGT_StyleMode.chat ? 'checked' : ''}></label>
                              <label class="panai-setting-label">ChatCPT昵称<input type="text" id="Chat-Name"} value="${ChatPGT_StyleMode.chat_name}"></label>
                              <label class="panai-setting-label">ChatCPT头像<input type="text" id="Chat-Avatar"} value="${ChatPGT_StyleMode.chat_avatar}"></label>
                            </div>`;
                Swal.fire({
                    title: 'ChatGPT 头像更改器',
                    html,
                    ...
                    if (res.isConfirmed) {
                        var config = {
                            ...
                            chat_avatar:document.getElementById('Chat-Avatar').value,
                        }
                        var confied = JSON.stringify(config)
                        localStorage.setItem('ChatPGT_StyleMode', confied);
