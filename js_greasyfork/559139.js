...
`;
  ...
  function addCumulatedVersionsTable() {
    ...
    `;
      ...
      const minorKeys = Object.keys(minorToInfo).filter((k) => k.startsWith(major + ".")).sort((a, b) => {
  ...
  // src/features/fix-issue-pr-count.ts
  var description2 = `Show "Issue" and "Pull Requests" counts in the package sidebar. At the time of writing, npm's own
...
`;
  ...
  async function run2() {
    ...
    if (!sidebar) return;
    const ref = Array.from(sidebar.children).find(
  ...
  async function getIssueAndPrCount(repo) {
    ...
    ).then((res) => res.json()).then((data) => data.total_count).catch(() => 0);
    const pulls = fetch(
  ...
  var description3 = `Fix various style issues on the npm site (mostly the package page at the moment).
`;
  function runPre3() {
  ...
  var description4 = "Add helpful links beside the package name title for convenience.";
  function runPre4() {
    if (!isValidPackagePage()) return;
    addStyle(`
    ...
    for (const a of sidebarLinks) {
      if (a.textContent === "Fund this package") {
        return {
          ...
          url: a.href,
          iconSvg: `<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="#fa5b9b" d="M462.3 62.6C407.5 15.9 326 24.3 275.7 76.2L256 96.5l-19.7-20.3C186.1 24.3 104.5 15.9 49.7 62.6c-62.8 53.6-66.1 149.8-9.9 207.9l193.5 199.8c12.5 12.9 32.8 12.9 45.3 0l193.5-199.8c56.3-58.1 53-154.3-9.8-207.9z"></path></svg>`
        };
      }
    }
  }
  ...
  function getPackagephobiaLinkData(packageName) {
    return {
      ...
      iconSvg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 108 108" width="108" height="108"><defs><linearGradient id="main"><stop offset="0" stop-color="#006838"></stop><stop offset="1" stop-color="#32de85"></stop></linearGradient></defs><path xmlns="http://www.w3.org/2000/svg" fill="url(#main)" d="M21.667 73.809V33.867l28.33-16.188 28.337 16.188V66.13L49.997 82.321 35 73.75V41.604l14.997-8.57L65 41.604v16.788l-15.003 8.571-1.663-.95v-16.67l8.382-4.792-6.719-3.838-8.33 4.763V69.88l8.33 4.762 21.67-12.383V37.737l-21.67-12.379-21.663 12.379v39.88L49.997 90 85 70V30L49.997 10 15 30v40z" transform="translate(-8.75 -7.5)scale(1.25)"></path></svg>`
    };
  }
  ...
  var description5 = `Move the "Fund this package" button to the bottom of the sidebar.
`;
  function run5() {
    if (!isValidPackagePage()) return;
  ...
  var description6 = `Hide the "Beta" label in the package code tab because it has been working for around 3 years now.
`;
  function runPre5() {
    addStyle(`
  ...
  var description7 = `If the package is named \`create-*\`, change the suggested install command in the sidebar as
...
`;
  ...
  var description8 = `Remember the banner at the top of the page when dismissed, so it doesn't keep showing up.
`;
  ...
  var description9 = `Remove the RunKit link as it's dead.
`;
  function run8() {
    if (!isValidPackagePage()) return;
