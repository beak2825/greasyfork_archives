// ==UserScript==
// @name                WME Route Speeds (ok_kh fork)
...
// @downloadURL https://update.greasyfork.org/scripts/434560/WME%20Route%20Speeds%20%28ok_kh%20fork%29.user.js
// @updateURL https://update.greasyfork.org/scripts/434560/WME%20Route%20Speeds%20%28ok_kh%20fork%29.meta.js
// ==/UserScript==
...
(function () {
	...
	function createRouteFeatures(id, routewsp, routeodc) {
		...
		let summarylen = routeodc.map(o => o.length).reduce((total, value) => total + value);
		let summarysec = 0;

		if (routespeedsoption7) {
			summarysec += routeodc.map(o => o.cross_time).reduce((total, value) => total + value);
		}
		else {
			summarysec += routeodc.map(o => o.cross_time_without_real_time).reduce((total, value) => total + value);
		}
			...
			success: function (json) {
				if (json.error !== undefined) {
					...
					if (json.response !== undefined) {
						console.log(`WME Route Speeds: ${json.response.length} routes received ( ${numPaths} requested)`);

						if (!routespeedsoption15) {
							var sortByField = (routespeedsoption13 === 2)
								? "length"
								: (routespeedsoption7
									? "total_route_time"
									: "total_route_time_without_real_time");
					...
					if (routeSelectedLast) {
						routeSelected = routeSelectedLast;
					}
					if (routeSelected > json.response.length) {
						routeSelected = json.response.length;
					}

					routes = [];
					json.response.forEach(response => {
						routes.push({
							routewsp: wktToCoords(response.geom.wkt),
							routeodc: response.result
						});
					});
	...
	function rezoom() {
		...
		routes.forEach((route, index) => {
			createRouteFeatures(index + 1, route.routewsp, route.routeodc);
		});
	}
	//--------------------------------------------------------------------------------------------------------
	function enterAB(ev) {
	...
	function wktToCoords(wkt) {
		const wktFormat = new OpenLayers.Format.WKT();
		let features = wktFormat.read(wkt);

		return features.geometry.components.map(g => { return { x: g.x, y: g.y }; })
	}
