    ...
    // @grant        none
// @downloadURL https://update.greasyfork.org/scripts/543116/GeoGuessr%20Duel%20Location%20Exporter.user.js
// @updateURL https://update.greasyfork.org/scripts/543116/GeoGuessr%20Duel%20Location%20Exporter.meta.js
    // ==/UserScript==
    ...
    (function () {
        ...
        function injectExportButton() {
            ...
            // Days back input
            daysBackInput = document.createElement('input');
            daysBackInput.id = 'days-back-input';
            daysBackInput.type = 'number';
            daysBackInput.min = '1';
            daysBackInput.value = '30';
            daysBackInput.style.position = 'fixed';
            daysBackInput.style.top = '180px';
            daysBackInput.style.right = '20px';
            daysBackInput.style.zIndex = 9999;
            daysBackInput.style.padding = '10px';
            daysBackInput.style.width = '120px';
            daysBackInput.style.border = '1px solid #ccc';
            daysBackInput.style.borderRadius = '5px';
            daysBackInput.style.marginBottom = '10px';
            daysBackInput.placeholder = 'Days back (default 30)';
            document.body.appendChild(daysBackInput);
            ...
            button.onclick = async () => {
                ...
                try {
                    await waitIfPaused();
                    const daysBack = parseInt(daysBackInput.value, 10) || 30;
                    const cutoffDate = new Date();
                    cutoffDate.setDate(cutoffDate.getDate() - daysBack);

                    const duelGames = await fetchAllDuels(cutoffDate);
        ...
        async function fetchAllDuels(cutoffDate) {
            const allGames = [];
            let token = null;
            ...
            while (true) {
                ...
                try {
                    ...
                    if (reachedCutoff) {
                        console.log('[GeoGuessr Exporter] Reached cutoff date, ending feed calls.');
                        break;
                    }
