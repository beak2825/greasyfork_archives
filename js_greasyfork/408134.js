...
// @downloadURL https://update.greasyfork.org/scripts/408134/E-Dziennik%20%2B%20Beta.user.js
// @updateURL https://update.greasyfork.org/scripts/408134/E-Dziennik%20%2B%20Beta.meta.js
// ==/UserScript==

// Copyright (c) 2020 Nikodem Reszka

/* Funkcja zliczajaca srednia ocen */
function srednia() {
  ...
  for (i = 0; i < kol.length; i++) {
   if (kol[i].innerHTML.indexOf("Wartość") > -1){
    ...
    /*
    if (kol[i].innerHTML.indexOf("Ocena") == true){
        kol_ocena = i;
  ...
  for (i = 0; i < rowy.length; i++) {
   ...
   if (rowy[i].cells.item(kol_liczona).innerHTML.indexOf("%") > - 1){
       ilosc_Procentow+=1;
   }
  ...
  var sred = Math.round(l/m*100);
    if(sred>100 || ilosc_Procentow < 3){
      sred = sred/100;
      if (sred < 1.41){
      cell3.innerHTML = 'Weź się do, <span style="color:#ff0000">NAUKI!</span>';
      }
      else if (sred < 2.75){
      ...
      cell3.innerHTML = 'Przebijasz skalę.<br>Masz <b>WINA</b>.';
      }
  }
  else{
      if (sred < 40){
