// ==UserScript==
// @name         ACGN-stock 冰塊營利統計外掛：比例型 (手機排版)
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/38688/ACGN-stock%20%E5%86%B0%E5%A1%8A%E7%87%9F%E5%88%A9%E7%B5%B1%E8%A8%88%E5%A4%96%E6%8E%9B%EF%BC%9A%E6%AF%94%E4%BE%8B%E5%9E%8B%20%28%E6%89%8B%E6%A9%9F%E6%8E%92%E7%89%88%29.user.js
// @updateURL https://update.greasyfork.org/scripts/38688/ACGN-stock%20%E5%86%B0%E5%A1%8A%E7%87%9F%E5%88%A9%E7%B5%B1%E8%A8%88%E5%A4%96%E6%8E%9B%EF%BC%9A%E6%AF%94%E4%BE%8B%E5%9E%8B%20%28%E6%89%8B%E6%A9%9F%E6%8E%92%E7%89%88%29.meta.js
// ==/UserScript==
  ...
  const userHaveStockInfo_FolderBody = $(`
    <div class="col-12 text-right border-grid">
        <table border="1" id="userHaveStockInfo_Table">
            <tr>
                <th width="275px">&nbsp;&nbsp;公司名稱&nbsp;&nbsp;</th>
                <th width="60px">&nbsp;&nbsp;股價&nbsp;&nbsp;</th>
                <th width="60px">&nbsp;&nbsp;股數&nbsp;&nbsp;</th>
                <th width="60px">&nbsp;&nbsp;比例&nbsp;&nbsp;</th>
                <th width="65px">&nbsp;&nbsp;總值&nbsp;&nbsp;</th>
                <th width="65px">&nbsp;&nbsp;分紅&nbsp;&nbsp;</th>
                <th width="70px">&nbsp;&nbsp;益本比&nbsp;&nbsp;</th>
            </tr>
        </table>
    </div>
  `);
    ...
    if (userID !== -1)
    {
        ...
        for (let i = 0 ; i < myStockRanked.length ; i++)
        {
			...
			if (hold != 1)
			{
					...
					if ($(`tr[companyID="${companyID}"]`).length < 1)
					{
						$("#userHaveStockInfo_Table").append(`
							<tr companyID="${companyID}">
								<td title="companyID" width="275px">&nbsp; <a href="/company/detail/${companyID}">${name}</a> &nbsp;</td>
								<td title="price" width="60px">${price}</td>
								<td title="hold" width="60px">${hold}</td>
								<td title="holdPercentage" width="65px">${(hold / release * 100).toFixed(2)} %</td>
								<td title="stockWorth" width="65px">${(price * hold)}</td>
								<td title="dividend" width="65px">${(earnPerShare * hold).toFixed(0)}</td>
								<td title="EP" width="70px">--　</td>
							</tr>
						`);
					}
					else
				...
				else if((earnPerShare / price).toFixed(2) <= 0.12)
				{
					if ($(`tr[companyID="${companyID}"]`).length < 1)
					{
						$("#userHaveStockInfo_Table").append(`
							<tr companyID="${companyID}">
								...
								<td title="stockWorth" width="65px">${(price * hold)}</td>
								<td title="dividend" width="65px">${(earnPerShare * hold).toFixed(0)}</td>
								<td title="EP" width="70px" bgcolor="black"><font color="red"><b>${(earnPerShare / price).toFixed(2)}</b></font></td>
							</tr>
						`);
					}
					else
				...
				else if((earnPerShare / price).toFixed(2) <= 0.15)
				{
					if ($(`tr[companyID="${companyID}"]`).length < 1)
					{
						$("#userHaveStockInfo_Table").append(`
							<tr companyID="${companyID}">
								...
								<td title="stockWorth" width="65px">${(price * hold)}</td>
								<td title="dividend" width="65px">${(earnPerShare * hold).toFixed(0)}</td>
								<td title="EP" width="70px" bgcolor="black"><span style="color:orange;"><b>${(earnPerShare / price).toFixed(2)}</b></span></td>
							</tr>
						`);
					}
					else
				...
				else if((earnPerShare / price).toFixed(2) >= 0.7)
				{
					if ($(`tr[companyID="${companyID}"]`).length < 1)
					{
						$("#userHaveStockInfo_Table").append(`
							<tr companyID="${companyID}">
								...
								<td title="stockWorth" width="65px">${(price * hold)}</td>
								<td title="dividend" width="65px">${(earnPerShare * hold).toFixed(0)}</td>
								<td title="EP" width="70px" bgcolor="yellow">${(earnPerShare / price).toFixed(2)}</td>
							</tr>
						`);
					}
					else
					...
					if ($(`tr[companyID="${companyID}"]`).length < 1)
					{
						$("#userHaveStockInfo_Table").append(`
							<tr companyID="${companyID}">
								...
								<td title="stockWorth" width="65px">${(price * hold)}</td>
								<td title="dividend" width="65px">${(earnPerShare * hold).toFixed(0)}</td>
								<td title="EP" width="70px">${(earnPerShare / price).toFixed(2)}</td>
							</tr>
						`);
					}
					else
