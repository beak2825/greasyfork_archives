// ==UserScript==
// @name           pillage toutouseul
// @namespace      toutouseul
// @version        2.0.3
// @description    Travian Auto pillage toutouseul
// @include        *://*.travian.*/build.php?*tt=99*
// @include        *://*.travian.*/login.php
// @include        *://*.travian.*/dorf3.php*
// 
// @grant GM_addStyle
// @grant GM_getValue
// @grant GM_setValue
// @grant GM_deleteValue
// @downloadURL https://update.greasyfork.org/scripts/37574/pillage%20toutouseul.user.js
// @updateURL https://update.greasyfork.org/scripts/37574/pillage%20toutouseul.meta.js
// ==/UserScript==
var Troops = {
};
Troops.Name = [
...
Config.Board = function () {
  ...
  if (aLists.length > 0) {
    ...
    for (var i = 0; i < aLists.split('$').length; i++) {
      ...
      tdPause = Doc.New('td');
      tdSel = Doc.New('select', [
        ['id',
        'p_' + ListID]
      ]);
      tdPause.appendChild(tdSel);
      tdOp0 = Doc.New('option', [
        ['value',
        0]
      ]);
      tdOp0.innerHTML = 0;
      tdOp1 = Doc.New('option', [
        ['value',
        1]
      ]);
      tdOp1.innerHTML = 1;
...
var Doc = {
  ...
  Tab: function (url) {
    ...
    form.appendChild(this.New('input', [
      ['type',
      'submit'],
      [
        'value',
...
Farm.Send = function (ListIndex) {
  ...
  if (AtLeastChecked == true) {
    ...
    dDif = parseInt(CalcNextSent.split('-') [1]) - parseInt(CalcNextSent.split('-') [0]) //dDif = Config.DMax - Config.DMin;
