// ==UserScript==
// @name:en        Iwara Animation Script Support iwara custom sort
// @name:ja      Iwara Animation Script Support iwara custom sort
// @name  预览Iwara视频 兼容 iwara custom sort
// @namespace    http://tampermonkey.net/
// @version      0.14
// @description:en  Animate Iwara video thumbnail when mouse over. to make iwara easier to see.
// @description:ja  Iwaraの動画サムネをマウスオーバーしたときにサムネをアニメーションさせる。ちょっと見やすくする。
// @description:zh-CN 当鼠标悬浮在视频封面时播放预览，让你大致知道视频内容（量子看片？）
// @author      TheCrown
// @match        *://*.iwara.tv/*
// @grant        none
// @require      http://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js
// @description Animate Iwara video thumbnail when mouse over. to make iwara easier to see.
// @downloadURL https://update.greasyfork.org/scripts/405795/%E9%A2%84%E8%A7%88Iwara%E8%A7%86%E9%A2%91%20%E5%85%BC%E5%AE%B9%20iwara%20custom%20sort.user.js
// @updateURL https://update.greasyfork.org/scripts/405795/%E9%A2%84%E8%A7%88Iwara%E8%A7%86%E9%A2%91%20%E5%85%BC%E5%AE%B9%20iwara%20custom%20sort.meta.js
// ==/UserScript==
...
4.this is a custom version based on Iwara Animation Script, origin version not support iwara custom sort, so I edited it to support it.
*/
...
(function () {
    ...
    $(document).on("mouseenter", ".field-item", function (evt) {
        //# マウスポインターの位置を取得するためにevtを受け取る
        img = undefined; //# 最初に確実に初期化
        if (evt.clientX == clientX && evt.clientY == clientY) {
            //# 前回のマウスポインターと同じ位置の場合は実行しない。（プルプル防止）
            return;
        }
        ...
        if (img !== undefined) {
            if (img.match(/\/photos\//)) {
                //# 動画ページでサムネイル以外も大きくなってしまうので除外。リストページでも除外される
            ...
            if (evt.ctrlKey) {
                ...
                var moveX =
                    ...
                    offset.left;
                var moveY =
                    (window.innerHeight - itemHeight) / 2 +
                    $(window).scrollTop() -
                    offset.top;
            ...
            if (img.match(/\/thumbnail-(\d+)_(\d+).jpg/)) {
                ...
                if (792500 <= videono) {
                    //# 一つずつ動画を確認したところ、このあたりから15個使えるようになっているようである。
                    maxNownum = 15;
                } else {
                    maxNownum = 5;
                }
                ...
                if (
                    $("#" + divid).find(".field-type-image,.field-type-file")
                        .length
                ) {
                    //".Field-type-file" is for confirmation and has no meaning. //「.field-type-file」は確認用で意味は無い。
                    ...
                    if ($("#" + divid).find(".add_ani").length === 0) {
                        $(this).append(
                            '<div class="add_ani"><p class="add_comment_youtube">YouTube</p><div>'
                        );
                        $(this)
                            .find("p")
                            .css(
                    ...
                    if (img.indexOf("/youtube/") >= 0) {
                        //If it does not exist after searching, it becomes -1. //念のために検索 youtube以外ではないはずだが 存在しない場合は-1となる。
                        youtubeimg = $(this).find("img").attr("src");
                        //console.log('this is youtube. youtubeでした '+img);
                        youtubeimg = img.replace(
                            "//i.iwara.tv/sites/default/files/styles/thumbnail/public/video_embed_field_thumbnails/youtube/",
                            "https://img.youtube.com/vi/"
                        );
                        youtubeimg = img.replace(
                            "//i.iwara.tv/sites/default/files/styles/sidebar_preview/public/video_embed_field_thumbnails/youtube/",
                            "https://img.youtube.com/vi/"
                        );
                        index = img.indexOf(".jpg");
    ...
    $(document).on("mouseleave", ".field-item", function () {
        $(this).find("img").off("load"); //# イベント除去

        //Restore appearance by releasing mouse over //マウスオーバー解除で見た目を戻す
        if (img !== undefined) {
            //If img is undefined, it has not been changed and will not be returned //imgがundefinedだったら変更していないので戻さない
    ...
    function act() {
        ...
        var countup = function () {
            ...
            if (img.indexOf("youtube") == -1) {
                //If not youtube then execute //youtubeでなければ次を実行)
                viewimg = img + idx + ".jpg";
                $("#" + divid)
                    .find("img")
                    .attr("src", viewimg);
