// ==UserScript==
// @name         FF Scouter V2 Targets
// @namespace    Violentmonkey Scripts
// @match        https://www.torn.com/*
// @version      2.62
// @author       rDacted, Weav3r, xentac, Glitchey
...
// @license      GPL-3.0
// @downloadURL https://update.greasyfork.org/scripts/551843/FF%20Scouter%20V2%20Targets.user.js
// @updateURL https://update.greasyfork.org/scripts/551843/FF%20Scouter%20V2%20Targets.meta.js
// ==/UserScript==

const FF_VERSION = "2.62";
...
if (!singleton) {
  ...
  function configure_perc() {
    const values = get_ff_perc(true);
    let curSetting = "";
    if (values) {
      curSetting = `${values.low},${values.high},${values.max}`;
    }
    const response = prompt(
      "Enter the low, high, and max % you want to use, separated by commas. Empty resets to default (Default '50,40,10').",
      curSetting,
    );
    // They hit cancel
    if (response == null) {
    ...
    if (split.length != 3) {
      showToast(
        "Incorrect format: FF scouter % should be 3 numbers separated by commas [<low>,<high>,<max>]",
      );
      return;
    }
    let low = null;
    try {
      ...
      showToast("Incorrect format: FF scouter low value must be a INT.");
      return;
    }
    let high = null;
    try {
      ...
      showToast("Incorrect format: FF scouter high value must be a INT.");
      return;
    }
    let max = null;
    try {
      max = parseInt(split[2]);
    } catch (e) {
      showToast("Incorrect format: FF scouter max value must be a INT.");
      return;
    }

    set_ff_perc(low, high, max);
  }
  ...
  function reset_ff_perc() {
    rD_deleteValue("ffscouterv2-perc");
  }
  ...
  function set_ff_perc(low, high, max) {
    rD_setValue(
      "ffscouterv2-perc",
      JSON.stringify({ low: low, high: high, max: max }),
    );
  ...
  function get_ff_perc(noDefault) {
    const defaultRange = { low: 50, high: 40, max: 10 };
    const rangeUnparsed = rD_getValue("ffscouterv2-perc");
    ...
    try {
      ...
      console.log(
        "[FF Scouter V2] Problem parsing configured perc, reseting values.",
      );
      reset_ff_perc();
      if (noDefault) {
        return null;
  ...
  ff_gauge_observer.observe(document, {
    ...
    function get_cached_targets(range, staleok) {
        const key = `${TARGET_KEY}-${range}`; // e.g., "ffscouterv2-target-low"
        const value = rD_getValue(key);
  ...
  function update_ff_targets() {
    ...
    //check low cache assuming they all updated at same time.
    const cached = get_cached_targets("low",false);
    if (cached) {
      return;
    }
    ...
    const stored_values = get_ff_ranges();
      ...
      for (const [range_key, range_value] of Object.entries(stored_values)) {
          ...
          const url = `${BASE_URL}/api/v1/get-targets?key=${key}&inactiveonly=1&maxff=${range_value}&limit=50`;

          console.log("[FF Scouter V2] Refreshing chain list: " + range_value);
          rD_xmlhttpRequest({
              method: "GET",
              url: url,
              onload: function (response) {
                  ...
                  if (response.status == 200) {
                      ...
                      if (ff_response.targets) {
                          ...
                          rD_setValue(TARGET_KEY+'-'+range_key, JSON.stringify(result));
                          console.log("[FF Scouter V2] Chain list updated successfully " +range_value);
                      }
                  } else {
                      try {
                          var err = JSON.parse(response.responseText);
...
function get_random_chain_target() {
    ...
    // Automatically select rangeKey based on percentages
    ...
    if (rand < stored_percentages.low) {
        ...
        rangeKey = 'high';
    } else if (rand < stored_percentages.low + stored_percentages.high + stored_percentages.max) {
        rangeKey = 'max';
    } else {
    ...
    const targets = get_cached_targets(rangeKey, true);
    if (!targets) {
        return null;
    }
  ...
  async function createSettingsPanel() {
    ...
    percDiv.className = "ff-settings-entry ff-settings-entry-large";

    const percLabel = document.createElement("label");
    percLabel.setAttribute("for", "ff-perc");
    percLabel.textContent = "FF % (Low, High, Max):";
    percLabel.className = "ff-settings-label ff-settings-label-inline";
    percDiv.appendChild(percLabel);

    const percInput = document.createElement("input");
    percInput.type = "text";
    percInput.id = "ff-perc";
    percInput.placeholder = "50,40,10";
    percInput.className = "ff-settings-input ff-settings-input-narrow";

    percDiv.appendChild(percInput);
    content.appendChild(percDiv);

    // Set current values
    const currentPerc = get_ff_perc(true);
    if (currentPerc) {
      percInput.value = `${currentPerc.low},${currentPerc.high},${currentPerc.max}`;
    }
    ...
    percInput.addEventListener("change", function () {
      ...
      if (parts.length !== 3) {
        this.style.outline = "1px solid red";
        showToast(
          "Incorrect format: FF % should be exactly 3 numbers separated by commas [low,high,max]",
        );
        return;
      }
      ...
      try {
        ...
        if (low <= 0 || high <= 0 || max <= 0) {
          this.style.outline = "1px solid red";
          showToast("FF % must be positive numbers");
          return;
        }
        ...
        if (low+high+max > 100) {
          this.style.outline = "1px solid red";
          showToast("FF % totals can not be greater than 100");
          return;
        }

        set_ff_perc(low, high, max);
        this.style.outline = "none";
        showToast("FF % updated successfully!");
      } catch (e) {
        this.style.outline = "1px solid red";
        showToast("Invalid numbers in FF %");
      }
    });
