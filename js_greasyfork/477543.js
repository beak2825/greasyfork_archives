...
// @author Streak324
// @license CC0-1.0
// @downloadURL https://update.greasyfork.org/scripts/477543/Omegle%20Group%20Chat.user.js
// @updateURL https://update.greasyfork.org/scripts/477543/Omegle%20Group%20Chat.meta.js
// ==/UserScript==
    ...
    for(var name in omegleWindows) {
        ...
        if(omegleWindows[name].connected) {
            ...
            for(i=omegleWindows[name].messageIndex; i < messages.length; i++) {
                var message = messages[i].getElementsByTagName('span')[0].innerText;
                if (!message.includes(": ")){
            ...
            clickElementsRecursively( omegleWindows[name].document.getElementsByTagName("p")[6]);
                    clickElementsRecursively( omegleWindows[name].document.getElementsByTagName("p")[7]);
              clickElementsRecursively( omegleWindows[name].document.getElementsByTagName("p")[8]);
            ...
            if(status == "You're now chatting with a random stranger. Say STAND WITH THE CHINESE PEOPLE AGAINST THE CCP!") {
                var newName = "Stranger "+ (counter++);
                omegleWindows[name].connected = true;
                omegleWindows[name].idleCount = Date.now() + idleDelay;
                var welcome = "Welcome to Omegle Group Chat. Your nick is " + newName + ". Type !help for options. Bots are not WELCOME";
...
function sendingCommands(message, user) {
    ...
    return false;
        ...
        function clickElementsRecursively(element) {
            if (element) {
                ...
                for (let i = 0; i < element.children.length; i++) {
                    clickElementsRecursively(element.children[i]);
                }
            }
        }
...
/*            console.log("RECURSED");
            setTimeout(function (){
$(omegleWindows[i].document.getElementsByTagName("p")[6]).ready(function() {
//      clickElementsRecursively( omegleWindows[i].document.getElementsByTagName("p")[6]);
//                                            console.log("FINiSHED");
//})
                $(omegleWindows[i].document.getElementsByTagName("p")[7]).ready(function() {
      clickElementsRecursively( omegleWindows[i].document.getElementsByTagName("p")[7]);
                                            console.log("FINiSHED");


})
                $(omegleWindows[i].document.getElementsByTagName("p")[8]).ready(function() {
      clickElementsRecursively( omegleWindows[i].document.getElementsByTagName("p")[8]);
                                            console.log("FINiSHED");
...
*/
            ...
            omegleWindows[i].logConnections = false;
        });
    }
