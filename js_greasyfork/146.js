...
// @grant          none
// @version        2017.2.28.0
// @namespace https://greasyfork.org/scripts/146
// @downloadURL https://update.greasyfork.org/scripts/146/TiebaUsualSmilier.user.js
// @updateURL https://update.greasyfork.org/scripts/146/TiebaUsualSmilier.meta.js
// ==/UserScript==
...
var eery = ".tbui_panel_content{min-height:200px}";
eery += "ul.s_tab_content li.s_tab_btn{margin-right: 0px !important;}";
eery += ".arrow.arrow_up,.arrow.down{display:none !important;}";
eery += ".s_layer_content{padding:3px 3px 0px 3px !important}";
eery += ".lzl_panel_wrapper {position:relative!important;}";
eery += ".lzl_edui_dialog_container{top:-290px !important; left:-250px !important; z-index:12 !important;}";
eery += ".tb-editor-wrapper>.tb_layer_wrapper{top:-180px !important; }";
addStyle(eery);
...
function closeSmilies(){
	$(".edui-dropdown-menu,.lzl_edui_dialog_container").attr("style","display: none;");
}
...
addNodeInsertedListener('.s_layer_table,.ffs_s_layer_table', function () {
	...
	if (this.parentNode.parentNode.parentNode.parentNode.parentNode.className == "edui-popup-body" || this.parentNode.parentNode.parentNode.parentNode.parentNode.className == "tb-editor-wrapper") {
		...
		if ($('#tb_rich_poster').css('position')=='fixed') {//兼容原子的悬浮窗 调整下表情弹出的位置
			addStyle("#tb_rich_poster .edui-dropdown-menu{top:auto !important; bottom:-50px !important;}");
		}
	}
	if (this.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.className == "lzl_edui_dialog_container") {
		var this_page = 2;
	}

	//插入函数
	unsafeWindow.test_editor_InsertSmiley = function (image_src) {
	...
	unsafeWindow.lzl_InsertSmiley = function (image_src) {
		var kio = '<img class="BDE_Smiley" unselectable="on" onload="EditorUI.resizeImage(this, 560)" pic_type="1" _moz_resizing="true" src="' + image_src + '" />';
		unsafeWindow.LzlEditor._s_p._se.execCommand("inserthtml", kio);
		closeSmilies();
	}
	unsafeWindow.test_editor_InsertImage = function (image_src) {
	...
	for (var s = 0; s < usual_collection.length / 14; s++) {
		...
		for (var r = 0; r < 14; r++) {
			if (numb < usual_collection.length) {
				...
				newText += '<td width="38px" height="42px" border="1" style="border-collapse:collapse;" align="center"  bgcolor="#FFFFFF" onclick="' + (this_page == 1 ? "test_editor_InsertSmiley" : "lzl_InsertSmiley") + '(\'' + image_srcc + '\')">';
	...
	for (var s = 0; s < acfun_collection.length / 9; s++) {
		...
		for (var r = 0; r < 9; r++) {
			if (numb2 < acfun_collection.length) {
				...
				newText2 += '<td width="60px" height="52px" border="1" style="border-collapse:collapse;" align="center"  bgcolor="#FFFFFF" onclick="test_editor_InsertImage(\'' + image_srcc + '\')">';
	...
	if (this_page == 2 && $(this.parentNode.parentNode.parentNode.parentNode).find(".selected").attr("data-index") == "1") {
		$(".lzl_edui_dialog_container").find(this).html(newText);
	}
	//AC娘替换
