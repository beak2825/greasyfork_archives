// ==UserScript==
// @name        AO3: Quality score with Load More button
// @description Uses the kudos/hits ratio, number of chapters, and statistical evaluation to score and sort AO3 works. Adds a button to load more works.
// @namespace   
// @author      SirCxyrtyx
// @version     1.0
...
// @license     MIT
// @downloadURL https://update.greasyfork.org/scripts/540799/AO3%3A%20Quality%20score%20with%20Load%20More%20button.user.js
// @updateURL https://update.greasyfork.org/scripts/540799/AO3%3A%20Quality%20score%20with%20Load%20More%20button.meta.js
// ==/UserScript==
...
const CONFIG = {
    ...
    colors: {
        ...
        green: '#00d800'   // background color for high scores
    }
};
...
(($) => {
    ...
    baseUrl = baseUrl.replace(/\&page=\d+/gm, '');
    ...
    const countRatio = () => {
        ...
        $('dl.stats').each(function () {
            ...
            try {
                ...
                // Add ratio stats
                let $ratioValue = $this.find('dd.kudoshits');
                if ($ratioValue.length > 0) {
                    console.log(`replace ${$ratioValue.length}`);
                    $ratioValue.text(`${percents_print}`);
                } else {
                    console.log(`add ${$ratioValue.length}`);
                    const $ratioLabel = $('<dt class="kudoshits">').text('Score:');
                    $ratioValue = $('<dd class="kudoshits">').text(`${percents_print}`);
    ...
    const loadMoreWorks = () => {
        if (pageNum == 0) {
            pageNum = parseInt($('#main > ol.pagination:first-of-type > li > a.current').text());
        }
        pageNum++;
        $.get(`${baseUrl}&page=${pageNum}`, function (data) {
            let dataDom = $(data);
            let works = dataDom.find("#main > ol.work.index.group > li");
            if (works.length == 0) {
                alert("no more works");
                return;
            }
            $("#main > ol.work.index.group").append(works);
        ...
        }, "html").fail(() => {alert("page load failed")});
    };
    ...
    const addRatioMenu = () => {
        ...
        const paginationButtons = $('#main > ol.pagination');
        const $buttonMore = $('<li>').html('<a>Load 20 More</a>');
        $buttonMore.click(() => loadMoreWorks());
        paginationButtons.append($buttonMore);
