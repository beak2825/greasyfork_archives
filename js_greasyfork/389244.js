...
// @downloadURL https://update.greasyfork.org/scripts/389244/%E8%AE%A4%E8%AF%81%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/389244/%E8%AE%A4%E8%AF%81%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
...
//定义助手代码
let cqczs = `
             <form id="CQCZS" style="position:fixed;bottom:0rem;;right:0rem;z-index:9;padding:1px;border:1px solid gray;background:white;">
                 <section style="margin:0.1rem 0.1rem;">
                     <canvas id="canvas_yzm"></canvas>
                 </section>
             </form>
             <form id="CQCZS" style="position:fixed;top:0rem;right:0rem;z-index:99999;padding:1px;border:1px solid gray;background:white;">
                 <section style="margin:0.2rem 0.5rem;">
                     <input type="checkbox" id="XSWJ">
                     <button type="button" id="btn XSWJ" class="btn btn-light">显示文件</button>
                     <input type="checkbox" id="XZBY">
                     <button type="button" id="btn XZBY" class="btn btn-light">选择百页</button>
                     <input type="checkbox" id="CLYZM">
                     <button type="button" id="btn CLYZM" class="btn btn-light">处理验证码</button>
                     <span style="margin:0rem 1.5rem;"></span>
                     <button type="button" id="btn BCYM" class="btn btn-light" style="margin:0rem 1rem;">保存页面</button>
                 </section>
             </form>
        `
...
}else if (window_url.indexOf('webdata.cqccms.com.cn/webdata/query/') != -1) {
    $('body').append(cqczs)
    ZHUSHOU();
    YZMCL();
...
function ZHUSHOU() {
    if(window.localStorage.getItem("xswj_anniu")=="true"){
        document.getElementById("XSWJ").checked =true;
    }else{
        document.getElementById("XSWJ").checked =false;
    };
    ...
    if(window.localStorage.getItem("xzby_anniu")=="true"){
        document.getElementById("XZBY").checked =true;
    }else{
        document.getElementById("XZBY").checked =false;
    };

    document.getElementById("imageID").addEventListener('load', YZMCL);
    document.getElementById("btn XSWJ").addEventListener('click',FXKXSWJ);
    document.getElementById('btn CLYZM').addEventListener('click',FXKCLYZM);
    document.getElementById('btn XZBY').addEventListener('click',FXKXZBY);
...
function FXKXSWJ() {
    if(document.getElementById("XSWJ").checked ==true){
        document.getElementById("XSWJ").checked =false;
        window.localStorage.setItem("xswj_anniu", false);
        document.getElementById("XSWJ").addEventListener('click', CQC_CX);
    }else{
        document.getElementById("XSWJ").checked =true;
        window.localStorage.setItem("xswj_anniu", true);
        document.getElementById("XSWJ").removeEventListener('click', CQC_CX);
    }
}
function FXKXZBY() {
    if(document.getElementById("XZBY").checked ==true){
        document.getElementById("XZBY").checked =false;
        window.localStorage.setItem("xzby_anniu", false);
        document.getElementById("XZBY").addEventListener('lclick', xz100y);
    }else{
        document.getElementById("XZBY").checked =true;
        window.localStorage.setItem("xsby_anniu", true);
        document.getElementById("XZBY").removeEventListener('click', xz100y);
    }
}
function FXKCLYZM() {
...
function CQC_CX(wz) {
    ...
    if(td.length > 0){
        for (var i = 0; i < td.length; i++) {
            if(td[i].innerText == '有效 Valid' || td[i].innerText == '已撤销 Withdrawn' || td[i].innerText == '已注销 Cancelled'){
                if(td[i+3].getElementsByTagName("a").length > 0){
                    var dizi="";
                    var src ="";
                    for (var a_i = 0; a_i < td[i+3].getElementsByTagName("a").length; a_i++) {
                        console.log("a内容",td[i+3].getElementsByTagName("a")[a_i].innerText,a_i);
                        if(td[i+3].getElementsByTagName("a")[a_i].innerText.indexOf('CN') != -1){
                            src = td[i+3].getElementsByTagName("a")[a_i].href;
                        }
                    }
                    if(dizi==""){
                        src = td[i+3].getElementsByTagName("a")[0].href;
                    }
                    dizi='https://ow365.cn/?i=30038&fname='+ td[i-10].innerText+a_i+'&furl='+ src;
                    //dizi='https://api.idocv.com/view/url?url='+ encodeURIComponent(src);
                    console.log("dizi",dizi);
                    var iframe = document.createElement("iframe");
                    iframe.width="850px";
                    iframe.height="600px";
                    iframe.src = dizi;
                    td[i-wz].append(iframe);
                }
            }
        }
    }
...
function YZMCL() {
    ...
    canvas.width = 65;
    canvas.height = 38;
    console.log("验证码高度宽度 :",canvas.width, canvas.height);
                ...
                if(words_result.length == 5 || words_result.length == 6){
                    ...
                    if (isFinite(qian)==true && isFinite(hou)==true){
                        ...
                        if (jieguo == 666){
                            ...
                            cwcs = 0;
                            if (url.indexOf('webdata/query/CCCCerti.do') != -1) {
                                CQC_CX(5);
...
function dianji() {
    var yichang_input = document.getElementById('change');
    console.log(yichang_input)
    if(yichang_input != null){
        yichang_input.click();
    }
};
