// ==UserScript==
// @name		GrinslyCustomBiggles'
// @namespace		vidya4chan
...
// @grant		none
// @author		biggles
// @downloadURL https://update.greasyfork.org/scripts/8977/GrinslyCustomBiggles%27.user.js
// @updateURL https://update.greasyfork.org/scripts/8977/GrinslyCustomBiggles%27.meta.js
// ==/UserScript==
...
window.script = new function(){
	...
	self.cssLinks = {
		...
		"phoenix":["Project Phoenix (created by Krogan, Illusory, Dildoer)", "http://googledrive.com/host/0B5VbuD1Jaw15fnV3bE5XeUNIdUQxc1dUNnZHMjgyb0VFT2c1YWpXU09Pai1sdGUxTEtFWk0/collabbiggles.css"],
	};
...
window.script.overwriteFunctions = function() {
	...
	window.room.addMessage = function(user, message, extraStyles) { //extraStyles = additional classes FOR THE MESSAGE STYLE
		...
		if (message.indexOf(':fa32432444st22276770900{{P}POP{O{OOP}{PP434223cvxcx2e2344242-4234232=22342-42-42320sd-068432234432:') > -1 && typeof($codes.fast) !== undefined) {
			script.fastmsgs++;
...
window.script.setListeners = function() {
	...
	$('#toggle_spin_box').on("change", function() {
		...
		if (checked)
			$codes['spin'] = '<span class="spin" style="-webkit-animation: spin 1s linear 0s 50">';
		else
			delete $codes.spin;
	});
...
window.script.setHTML = function() {
	if (!script.htmlIsSet) {
		...
		$('.container').append('<div id="gmtClock"></div>');
