...
// @run-at         document-end
// @namespace      _
// @downloadURL https://update.greasyfork.org/scripts/528330/RedactedMofo%20denlekke.user.js
// @updateURL https://update.greasyfork.org/scripts/528330/RedactedMofo%20denlekke.meta.js
// ==/UserScript==
...
(() => {
  ...
  const MofoConfig = JSON.parse(localStorage.getItem("mofo_config") || '{"mofo_summary": "Mofo", "discogs_token": "",  "beatport_token": "", "allmusic_review": false, "before_links": false}');
  ...
  if (location.href.includes(`action=edit&userid=${myUID}`)) {
    const updateMofoConfig = (e) => {
      ...
      config.beatport_token = document.getElementById("beatport_token").value;
    ...
    const BeatportTokenRow= $(`
      <tr>
        <td class="label tooltip">
          <strong>Mofo Beatport Token:</strong>
        </td>
      </tr>
    `);
    const BeatportTokenCol = $(`<td></td>`);
    const BeatportTokenTxt = $(`<input type="text" size="40" name="beatport_token" id="beatport_token" value="${MofoConfig.beatport_token}">`)
    ...
    BeatportTokenCol.append(BeatportTokenTxt);
    BeatportTokenRow.append(BeatportTokenCol);
    torGrouping.after(BeatportTokenRow);
  ...
  const gazelleAPI = async groupid => {
    try {
      const res = await fetch(`${window.location.protocol}//${window.location.hostname}/ajax.php?action=torrentgroup&id=${groupid}`);
      return res.json();
    } catch (error) {
      console.log(error);
    }
  ...
  const corsFetch = url => {
    ...
    if (url.includes("beatport")) {
      headers['Authorization'] = `Bearer ${MofoConfig.beatport_token}`;
    }
    if (url.includes("allmusic")) {
      headers['Referer'] = url.replace('/reviewAjax','');
    }
  ...
  const beatSearch = async (artist, album) => {
    try {
      ...
      const doc = await fetchJSON(
        `https://api.beatport.com/v4/catalog/search/?q=${query}`
      );
      const releases = doc.releases
      const found_album = releases.find(release => {
        if (sanitize(release.artists[0].name).includes(sanitize(artist))) {
          return sanitize(release.name).includes(sanitize(album));
        }
      });
      const slug = found_album.slug;
      const releaseid = found_album.url.split("/", 7).pop()
      const purchaseLink = `https://www.beatport.com/release/${slug}/${releaseid}`
      if (purchaseLink) {
        return {source: "Beatport", url: purchaseLink};
      }
    } catch (e) {
  ...
  const allMusicSearch = async (artist, album) => {
    try {
      ...
      const albums = [...doc.querySelectorAll("div#resultsContainer div.album")];
      ...
      const allMusicLink = found_album.querySelector('a').href.replace(`${location.origin}`, "www.allmusic.com");
      if (MofoConfig.allmusic_review) {
        doc = await fetchDOM(allMusicLink + "/reviewAjax");
        const authorElem = doc.querySelector('div#review > h3');
        const reviewElem = [...doc.querySelectorAll('div#review > p')];
        if (authorElem && reviewElem) {
          review = {author: authorElem.innerText.split("Review by ")[1].trim(), text: reviewElem.map((e) => e.innerText.trim()).join("\n\n")}
        }
      }
  ...
  const searchForLinks = async (event) => {
    ...
    // Detect and extract any pre-existing "More info:" links from the description
    let existingLinks = [];
    let descText = groupDesc.value;
    const moreInfoRegex = /\[b\]More info:\[\/b\]\s*(.*)/i;
    const matchExisting = descText.match(moreInfoRegex);
    if(matchExisting) {
       const linksPart = matchExisting[1];
       const urlRegex = /\[url=([^\]]+)\]([^\[]+)\[\/url\]/g;
       let m;
       while((m = urlRegex.exec(linksPart)) !== null){
           existingLinks.push({source: m[2].trim(), url: m[1].trim()});
       }
       // Remove the existing "More info:" section from the description.
       groupDesc.value = descText.replace(moreInfoRegex, "").trim();
    }
    ...
    gazelleAPI(groupid).then(({ response, status }) => {
      ...
      // Build search promises only for sources not already present.
      if(!existingLinks.some(link => link.source.toLowerCase() === "deezer")) {
         promises.push(deezSearch(artist, album, upc));
      }
      if(!existingLinks.some(link => link.source.toLowerCase() === "allmusic")) {
         promises.push(allMusicSearch(artist, album));
      }
      if(!existingLinks.some(link => link.source.toLowerCase() === "apple")) {
         promises.push(appleSearch(artist, album, upc));
      }
      if(!existingLinks.some(link => link.source.toLowerCase() === "bandcamp")) {
         promises.push(bandSearch(artist, album));
      }
      if(!existingLinks.some(link => link.source.toLowerCase() === "bleep")) {
         promises.push(bleepSearch(artist, album));
      }
      if(!existingLinks.some(link => link.source.toLowerCase() === "juno")) {
         promises.push(junoSearch(artist, album));
      }
      if(!existingLinks.some(link => link.source.toLowerCase() === "beatport")) {
         promises.push(beatSearch(artist, album));
      }
      if(!existingLinks.some(link => link.source.toLowerCase() === "qobuz")) {
         promises.push(qobuzSearch(artist,album));
      }
      if(!existingLinks.some(link => link.source.toLowerCase() === "discogs")) {
         promises.push(searchDiscogs(artist,album));
      }
      if(!existingLinks.some(link => link.source.toLowerCase() === "musicbrainz")) {
         promises.push(musicBrainzSearch(artist,album));
      }
      if(!existingLinks.some(link => link.source.toLowerCase() === "spotify")) {
         promises.push(spotifySearch(artist,album));
      }
      if(!existingLinks.some(link => link.source.toLowerCase() === "tidal")) {
         promises.push(tidalSearch(artist, album, upc));
      }
      Promise.all(promises).then((results) => {
         // Filter out any failed (null/undefined) results.
         let newLinks = results.filter(result => result);
         // Combine new search results with pre-existing links and deduplicate by source.
         let combined = {};
         existingLinks.forEach(link => {
            combined[link.source.toLowerCase()] = link;
         });
         newLinks.forEach(link => {
            if (!combined[link.source.toLowerCase()]) {
               combined[link.source.toLowerCase()] = link;
            }
         });
         let combinedLinks = Object.values(combined);
         // Sort the combined links alphabetically by source.
         combinedLinks.sort((a, b) => a.source.localeCompare(b.source));
         const moreInfo = combinedLinks.map(link => `[url=${link.url.replace(/#.*/, '')}]${link.source}[/url]`);
