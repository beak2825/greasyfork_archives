// ==UserScript==
// @name         Google maps 圖片縮放工具
// @namespace    http://tampermonkey.net/
// @version      1.1.2
// @description  Google maps 點擊圖片直接懸浮原圖，可用滾輪放大縮小
// @author       shanlan(grok-4-fast-reasoning)
// @match        https://www.google.com/maps*
// @grant        GM_download
// @run-at       document-end
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/559292/Google%20maps%20%E5%9C%96%E7%89%87%E7%B8%AE%E6%94%BE%E5%B7%A5%E5%85%B7.user.js
// @updateURL https://update.greasyfork.org/scripts/559292/Google%20maps%20%E5%9C%96%E7%89%87%E7%B8%AE%E6%94%BE%E5%B7%A5%E5%85%B7.meta.js
// ==/UserScript==
...
(function() {
    ...
    const getVisibleSrc = img => {
        if (!img.src.includes('googleusercontent.com') || img.src.includes('maps/api')) return null;
        return img.src.replace(/=w\d+(?:-h\d+)?(?:-[kv])?/, '=s0');
    };
    ...
    const openModal = img => {
        ...
        o.observe(img, {attributes: true, childList: true, subtree: true});
    ...
    function addClickHandler(img) {
        if (img.dataset.clickAdded || !getVisibleSrc(img)) return;
        img.style.cursor = 'pointer';
        img.addEventListener('click', (e) => {
            openModal(img);
        });
        img.dataset.clickAdded = 'true';
    }

    function scanImages() {
        document.querySelectorAll('img').forEach(addClickHandler);
    }
