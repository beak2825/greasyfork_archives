// ==UserScript==
// @name         «РУСЬ» ONLINE | Скрипт КП
// @namespace    https://forum.rus-rp.online
// @version      1.1.0
// @description  Специально для «РУСЬ» ONLINE
// @author       Crown
// @match        https://forum.rus-rp.online/threads/*
// @include      https://forum.rus-rp.online/threads/
// @grant        none
// @license 	 MIT
// @collaborator Quenk269
// @icon https://emoji.gg/assets/emoji/9372-blurple-boost-level-9.png
// @downloadURL https://update.greasyfork.org/scripts/494180/%C2%AB%D0%A0%D0%A3%D0%A1%D0%AC%C2%BB%20ONLINE%20%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%9A%D0%9F.user.js
// @updateURL https://update.greasyfork.org/scripts/494180/%C2%AB%D0%A0%D0%A3%D0%A1%D0%AC%C2%BB%20ONLINE%20%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%9A%D0%9F.meta.js
// ==/UserScript==
...
//стиль оформления форума
    ...
    .slider {
        ...
        right: 0;
        bottom: 0;
        border: 1px solid #FFE9B8; // обводка кнопки снега
    ...
    .slider:before {
        ...
        left: 2px;
        bottom: 2px;
        background-color: #FFE3AD; //цвет кнопки снега
    ...
    input:checked + .slider:hover {
        background-color: #FACB89; //цвет при нажатии (2)
    }
    input:focus + .slider {
    ...
    const buttons = [
{
            ...
            style: 'width: 97%; background: #ffbf00', //(подсветка)box-shadow: 0px 0px 5px #fff',
},
...
},
{
    ...
    $(document).ready(() => {
        ...
        addButton('На рассмотрении', 'pin', 'background: #73450088; border: 2px solid #ffad33; border-radius: 10px; font-family: JetBrains Mono; color: #ffad33');
        addButton('Важно', 'Vajno', 'background: #44000088; border: 2px solid #a00; border-radius: 10px; font-family: JetBrains Mono; color: #a00');
        addButton('Команде Проекта', 'teamProject', 'background: #6b600188; border: 2px solid #fff06e; border-radius: 10px; font-family: JetBrains Mono; color: #fff06e');
        addButton('ГА', 'Ga', 'background: #44000088; border: 2px solid #a00; border-radius: 10px; font-family: JetBrains Mono; color: #a00');
        addButton('Спецу', 'Spec', 'background: #44000088; border: 2px solid #a00; border-radius: 10px; font-family: JetBrains Mono; color: #a00');
        addButton('Одобрено', 'accepted', 'background: #00440088; border: 2px solid #0a0; border-radius: 10px; font-family: JetBrains Mono; color: #0a0');
        addButton('Отказано', 'unaccept', 'background: #44000088; border: 2px solid #a00; border-radius: 10px; font-family: JetBrains Mono; color: #a00');
        addButton('Теху', 'Texy', 'background: #00004488; border: 2px solid #00a; border-radius: 10px; font-family: JetBrains Mono; color: #00a');
        addButton('Закрыто', 'Zakrito', 'background: #44000088; border: 2px solid #a00; border-radius: 10px; font-family: JetBrains Mono; color: #a00');
        addButton('Ожидание', 'Ojidanie', 'background: #444444ff; border: 2px solid #aaa; border-radius: 10px; font-family: JetBrains Mono; color: #aaa');
        ...
        $(`button#selectAnswer`).click(() => {
            XF.alert(buttonsMarkup(buttons), null, '(«РУСЬ» ONLINE ) Выберите ответ:');
    ...
    function addAnswers() {
        $('.button--icon--reply').after(`<button type="button" class="button--cta uix_quickReply--button button button--icon button--icon--write rippleButton" id="selectAnswer" style="oswald: 3px; margin-left: 20px; border: 3px solid; border-radius: 20px; background: #FFBD4D; padding: 0px 20px 0px 20px; font-Arial-Black: JetBrains Mono; border-color: #FFB55C;">ОТВЕТЫ</button>`,
        );
    }
    ...
    function infoAlert() {
        return `<div class="bug_tracker" style="display:flex; flex-direction:column; flex-wrap:wrap">` +
            `<span class="button-text" style="margin-bottom: 20px; text-align: center; font-size: 16px; font-weight: 500; background: #34aaeb; padding: 5px; border-radius: 15px">Автором и разработчиком данного скрипта является Андрей - Crown. Выразим ему огромное спасибо за помощь!<br>По всем вопросам и предложениям обращайтесь в телеграмм.</span>` +
