// ==UserScript==
// @name        Personal Turkmaster - Whisper Theme - Right Dock
// @namespace   https://greasyfork.org/en/users/13132-rosesword975
// @author      rosesword975 - Color Theme
// @description Position on Left Side of Screen and Color Theme
// @author		  DonovanM - Turkmaster
...
// @grant       GM_getValue
// @grant       GM_setValue
// @downloadURL https://update.greasyfork.org/scripts/11375/Personal%20Turkmaster%20-%20Whisper%20Theme%20-%20Right%20Dock.user.js
// @updateURL https://update.greasyfork.org/scripts/11375/Personal%20Turkmaster%20-%20Whisper%20Theme%20-%20Right%20Dock.meta.js
// ==/UserScript==
...
// Color Theme based on Hitdatabase Whisper theme by Tia Rutledge (rosesword975 on MTG/GreasyFork)
...
var SettingsDialog = function() {
	...
	function _createDOMElement() {
		...
		DOMElement = $('<div id="settingsDialog"><h2>Settings</h2></div>').append(
			...
			$('<div id="frameSettings"><h3>Show Other Page</h3>\
					<ul>\
						<li id="customURL"><input type="text" style="width: 150px;"/>Enter custom URL<br />for right side of page</li>\
					</ul>\
			   </div>'),
			$('<div id="turkmasterURLSettings"><h3>Force TurkMaster URL</h3>\
					<ul>\
	...
	function _addStyle() {
		addStyle("\
			...
			#settingsDialog .on_off span { color: #AFCCDE; margin: 1px; font-size: 56%; font-weight: bold; border-radius: 1.6em;  }\
			#settingsDialog .on_off span:nth-child(2) { background-color: #1F3847; color: #AFCCDE; padding: 0.4em 0.8em; }\
			#settingsDialog .on_off.on span:nth-child(1) { background-color: #1F3847; color: #AFCCDE; padding: 0.4em 0.8em; }\
			#settingsDialog .on_off.on span:nth-child(2) { background-color: inherit; color: #AFCCDE; padding: 0 0.8em 0 0; }\
			...
			#settingsDialog li input[type='text'] { background-color: #232A2F; color: #AFCCDE; width: 3em; font-size: 80%; margin-right: 0.8em; text-align: center; padding-right: 0.5em }\
			#settingsDialog li .more { width: 24px; border: none; color: #AFCCDE; font: bold 160% inital; line-height: 0%; transform: rotate(90deg); background-color: transparent; position: relative; top: 8px; cursor: pointer; height: 0.7em; padding: 0 0 0.65em; }\
			#settingsDialog li#typeface input { color: #AFCCDE; width: 8em }\
			.dialog-big { color: #AFCCDE; position: fixed; top: 2em; left: 50%; width: 860px; margin-left: -430px; background-color: #232A2F; padding: 2%; border: 1px solid #ddd; font-family: 'Oxygen'; box-shadow: 3px 3px 3px rgba(0, 0, 0, 0.4); border-radius: 10px; }\
			.dialog-big p { color: #AFCCDE; background-color: #394752; padding: 1.5em; height: 500px; overflow: scroll; }\
			.dialog-big h2, #export-box h3 { color: #AFCCDE; font-weight: 400 }\
			.dialog-big h2 { color: #AFCCDE; font-size: 170%; margin-top: 0 }\
			.dialog-big button { background-color: #232A2F; color: #AFCCDE; padding: 3px 10px; font-family: 'Oxygen'; border: none; border-radius: 3px; font-weight: bold; transition: background-color 0.3s; margin-right: 0.5em }\
			.dialog-big button:hover { background-color: #1F3847 }\
			.dialog-big textarea { color: #AFCCDE; background-color: #394752; display: block; width: 100%; height: 500px; margin: 1em 1em 1em 0; }\
		");
	}
	...
	addStyle("\
      ...
      color: #808080;\
		...
		#add_watcher_form h3 {\
			...
			margin: 0 0 15px;\
			background-color: #394752;\
			background-color: rgba(24, 42, 47, 1);\
			padding: 3px;\
			color: #AFCCDE;\
			}\
      ...
      color: #AFCCDE;\
		...
		.watcher_button a:hover { background-color: #1F3847 }\
...
function createDetailsPanel() {
	...
	#details_panel.right { right: 30px }");
...
function showDetailsPanel(watcher) {
	...
	if (watcher !== lastWatcher || (watcher === lastWatcher && watcher.isUpdated)) {
		...
		if (watcher.lastHits.length > 0) {
			...
			$(panel).append($('<div>').append('<h2>').css('text-align', 'center').html("<br />\u2618 \u2618 Sorry!! Good Luck Next Time! \u2618 \u2618 <br /><br />"));
		}
	}
	$(panel).show();
...
var DispatchUI = {
	...
	addStyle: function() {
		addStyle("#dispatcher { background-color: #232A2F; position: fixed; top: 0px; float: right; right: 0; height: 100%;  width: 270px; font-size: 8pt;  margin-right: 0px; transition: right 0.5s ease; }\
			#dispatcher.tm-hidden { right: -240px }\
			#content_container { position: absolute; left: 0; top: 0; right: 270px; border-right: 2px solid #232A2F; transition: left 0.5s ease; background-color: #232A2F }\
			#content_container.full { left: 30px }\
			#dispatcher #controller { text-align: center; font: 160% Candara, sans-serif; color: #AFCCDE; position: relative; padding: 3px 5px; }\
			#dispatcher #controller .on_off { margin: 6px 5px 0 0 }\
			#dispatcher #controller .on_off a { font-size: 80% }\
			#dispatcher #controller #settings { top: 2px; position: absolute; left: 5px; }\
			#dispatcher #controller #settings img { width: 1.5em }\
			#dispatcher #controller .play_container { position: absolute; right: 5px }\
			...
			#dispatcher #watcher_container .error_button a { text-decoration: none; color: #AFCCDE; background-color: #1F3847; padding: 3px 10px; margin: 5px; border: 1px solid #1F3847; border-radius: 2px }\
			#dispatcher #watcher_container .error_button a:hover { background-color: #394752; border-color: #394752 }\
			#dispatcher div { color: #808080; font-size: 7pt }\
			#dispatcher .watcher {\
...
#dispatcher .watcher.running .details { margin-left: 0px; background-color: #7B8F8F; background-color: rgba(123, 143, 143, 1); }\
			#dispatcher .watcher.updated { background-color: #7B8F8F; background-color: rgba(123, 143, 143, 1) }\
			#dispatcher .watcher .details { width: 25px; text-align: center; float: lef; background-color: rgba(31, 56, 71, 1); position: absolute; top: 0; bottom: 0; left: 0; font-size: 90%; color: #1F3847; transition: background-color 0.5s }\
			#dispatcher .watcher .details.updated { background-color: rgba(123, 142, 143, 1); background-color: #7B8F8F; background-color: rgba(123, 143, 143, 1) }\
			#dispatcher .watcher .name { font-size 125%; color: #AFCCDE; text-decoration: none; display: inline-block; margin-top: -3px}\
			#dispatcher .watcher .name:hover { text-decoration: underline }\
			#dispatcher .watcher.dragging .name:hover { text-decoration: none }\
			#dispatcher .watcher .time { color: #7B8F8F; display: block; float: right; font-size: 80% }\
			.on_off { float: right; cursor: pointer }\
			.on_off a { margin: 1px; font-size: 56%; font-weight: bold }\
			.on_off a:nth-child(2) { border-radius: 12px; padding: 3px 6px; }\
			.on_off a:nth-child(1) { background-color: #394752; color: #AFCCDE; border-radius: 12px; padding: 3px 6px; }\
			.on_off.on a:nth-child(1) { background-color: #394752; color: #AFCCDE; border-radius: 12px; padding: 3px 6px; }\
			.on_off.on a:nth-child(2) { background-color: inherit; color: #AFCCDE; border-radius: inherit; padding: inherit; }\
			.watcher .on_off {  }\
			#dispatcher .watcher > .content { margin-right: 25px; padding: 5px 5px 5px 33px;}\
			#dispatcher .watcher .bottom { margin: 0 0 -5px; color: #394752 }\
			#dispatcher .watcher .bottom a:link { color: #232A2F; }\
			#dispatcher .watcher .bottom a:hover { color: #232A2F; }\
			#dispatcher .watcher .details { color: #AFCCDE; font-size: 150%; font-weight: bold }\
			#dispatcher .watcher .last_updated { position: absolute; left: 70px; bottom: 4px; font-size: 80%; color: #CADA95}\
			#dispatcher .watcher .icons { visibility: visiable; margin-left: 0px; bottom: 5px }\
			...
			#dispatcher .watcher .color_code { position: absolute; right: 0; top: 0; bottom: 0; width: 9px; cursor: grab;}\
			#dispatcher .watcher .color_code div { position: absolute; right: 0; top: 0; bottom: 0; width: 5px; transition: width 0.15s; }\
			#dispatcher .watcher.dragging .color_code { cursor: grabbing; }\
			#dispatcher .watcher .color_code:hover div { color: #1F3847; width: 9px }\
			#dispatcher .watcher .color_code.hit div       { background-color: rgba(181, 227, 255, .7); }\
			#dispatcher .watcher .color_code.requester div { background-color: rgba(218, 149, 168, .7); }\
			#dispatcher .watcher .color_code.url div       { background-color: rgba(202, 218, 149, .7); }\
			...
			.play:before, .play_all:before {\
				...
				border-style: solid;\
				border-color: transparent transparent transparent #DA95A8;\
				position: absolute;\
				content: '';\
				top: 3px;\
				left: 0px;\
			...
			.play_all:after {\
				...
				border-width: 8px 11px;\
				border-style: solid;\
				border-color: transparent transparent transparent #CADA95;\
				position: absolute;\
				content: '';\
				top: 3px;\
				left: 5px;\
			...
			.play_select {\
				width: 6px;\
				height: 6px;\
				border: 2px solid #7B8F8F;\
				border-radius: 2px;\
				margin-top: 2px;\
			}\
			.watcher.selected .play_select { background-color: #1F3847; border-color: #1F3847; }\
			");
	},
...
Watcher.prototype.parseHitPage = function(data) {
	...
	if ($(msgbox).length > 0 && ($(msgbox).text()).contains("There are no more available HITs in this group.")) {
		...
		if (this.option.auto && (this.option.stopOnCatch || this.option.stopGroup) && this.state.isRunning)
...
NotificationPanel.prototype.createPanel = function() {
	...
	addStyle("\
		...
		#receiver .notification_group {\
			...
			border-right-color  : #1F3847;\
			border-bottom-color : #1F3847;\
		}\
		...
		#receiver .notification_group h3 { color: #AFCCDE; margin: 3px; font-weight: normal }\
		#receiver .notification_group h3 a:link, #receiver .notification_group h3 a:visited { color: #AFCCDE }\
		#receiver .notification_group h4 a:link,\
		#receiver .notification_group h4 a:visited { margin: 2px 0 0 4px; color: #AFCCDE; }\
		.notification_panel h2, #details_panel h2 { color #AFCCDE; font-size: 100%; font-weight: normal; margin: 0px }\
		.notification {\
		...
		.notification_panel a.hit_link:visited { background-color: #394752; }\
		.notification_panel a.hit_link:hover { background: #1F3847; }\
		.notification_panel p {	margin: 3px 0 6px 0; font-size: 80%; cursor: default }\
		.notification_panel .autoaccept {\
			background-color : rgba(57, 71, 82, .3);\
			background-color : rgba(57, 71, 82, 1);\
			background-color : rgba(57, 71, 82, 1);\
		}\
		.notification.not_qualified { background-color: rgba(35, 42, 47, 1) }\
		.notification_panel .new { background-color: rgba(35, 42, 47, 1); }\
		...
		.notification_panel .ratings-chart td { font-family: 'Oxygen',verdana,sans-serif; font-size: 70%; color: #AFCCDE; padding: 0 2em 0 0; cursor: default; vertical-align: center }\
		...
		.notification_panel .ratings-chart .rating > div { background-color: #394752; height: 0.5em; width: 13em; margin: 0 0.5em 0 0; border-radius: 4px }\
		.notification_panel .ratings-chart .rating > div > div { background-color: #232A2F; height: 100%; border-radius: 4px }\
		");
}
