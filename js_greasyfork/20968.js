// ==UserScript==
// @name         :: BEELATTE :: Pixiv Manga Translation Helper
// @namespace    https://greasyfork.org/en/scripts/20968-beelatte-pixiv-manga-translation-helper
// @version      2.1
// @description  Simple manga translation using tooltip for Pixiv
// @author       Beelatte
// @match        http://www.pixiv.net/member_illust.php?mode=manga&illust_id=*
// @match        http://www.pixiv.net/member_illust.php?mode=manga_big&illust_id=*
// @match        http://touch.pixiv.net/member_illust.php?mode=manga&illust_id=*
// @grant        GM_addStyle
// @grant        GM_getResourceText
// @require      http://code.jquery.com/jquery-1.10.0.min.js
// @require      https://cdn.jsdelivr.net/jquery.tooltipster/4.0.4/js/tooltipster.bundle.min.js
// @require      https://cdnjs.cloudflare.com/ajax/libs/tabletop.js/1.4.3/tabletop.min.js
// @resource tooltip https://cdn.jsdelivr.net/jquery.tooltipster/4.0.4/css/tooltipster.bundle.min.css
// @resource noir https://cdn.jsdelivr.net/jquery.tooltipster/4.0.4/css/plugins/tooltipster/sideTip/themes/tooltipster-sideTip-noir.min.css
// @resource shadow https://cdn.jsdelivr.net/jquery.tooltipster/4.0.4/css/plugins/tooltipster/sideTip/themes/tooltipster-sideTip-shadow.min.css
// @downloadURL https://update.greasyfork.org/scripts/20968/%3A%3A%20BEELATTE%20%3A%3A%20Pixiv%20Manga%20Translation%20Helper.user.js
// @updateURL https://update.greasyfork.org/scripts/20968/%3A%3A%20BEELATTE%20%3A%3A%20Pixiv%20Manga%20Translation%20Helper.meta.js
// ==/UserScript==

GM_addStyle(GM_getResourceText("tooltip"));
GM_addStyle(GM_getResourceText("noir"));

/***************************************************************************************************
LoadingOverlay - A flexible loading overlay jQuery plugin
    Author          : Gaspare Sganga
    Version         : 1.4.0
    License         : MIT
    Documentation   : http://gasparesganga.com/labs/jquery-loading-overlay/
****************************************************************************************************/
(function($, undefined){
    var _defaults = {
        color           : "rgba(255, 255, 255, 0.8)",
        custom          : "",
        fade            : true,
        fontawesome     : "",
        image           : "data:image/gif;base64,",
        imagePosition   : "center center",
        maxSize         : "100px",
        minSize         : "20px",
        resizeInterval  : 0,
        size            : "50%",
        zIndex          : undefined
    };

    $.LoadingOverlaySetup = function(settings){
        $.extend(true, _defaults, settings);
    };

    $.LoadingOverlay = function(action, options){
        switch (action.toLowerCase()) {
            case "show":
                var settings = $.extend(true, {}, _defaults, options);
                _Show("body", settings);
                break;
            case "hide":
                _Hide("body", options);
                break;
        }
    };

    $.fn.LoadingOverlay = function(action, options){
        switch (action.toLowerCase()) {
            case "show":
                var settings = $.extend(true, {}, _defaults, options);
                return this.each(function(){
                    _Show(this, settings);
                });
            case "hide":
                return this.each(function(){
                    _Hide(this, options);
                });
        }
    };


    function _Show(container, settings){
        container = $(container);
        var fixed = container.is("body");
        var count = container.data("LoadingOverlayCount");
        if (count === undefined) count = 0;
        if (count === 0) {
            var overlay = $("<div>", {
                class   : "loadingoverlay",
                css     : {
                    "background-color"  : settings.color,
                    "display"           : "flex",
                    "flex-direction"    : "column",
                    "align-items"       : "center",
                    "justify-content"   : "center"
                }
            });
            if (settings.zIndex !== undefined) overlay.css("z-index", settings.zIndex);
            if (settings.image) overlay.css({
                "background-image"      : "url("+settings.image+")",
                "background-position"   : settings.imagePosition,
                "background-repeat"     : "no-repeat"
            });
            if (settings.fontawesome) $("<div>", {
                class   : "loadingoverlay_fontawesome "+settings.fontawesome
            }).appendTo(overlay);
            if (settings.custom) $(settings.custom).appendTo(overlay);
            if (fixed) {
                overlay.css({
                    "position"  : "fixed",
                    "top"       : 0,
                    "left"      : 0,
                    "width"     : "100%",
                    "height"    : "100%"
                });
            } else {
                overlay.css({
                    "position"  : "absolute",
                    "top"       : 0,
                    "left"      : 0
                });
                if (container.css("position") == "static") {
                    overlay.css({
                        "top"   : container.position().top  + parseInt(container.css("margin-top"))  + parseInt(container.css("border-top-width")),
                        "left"  : container.position().left + parseInt(container.css("margin-left")) + parseInt(container.css("border-left-width"))
                    });
                }
            }
            _Resize(container, overlay, settings, fixed);
            if (settings.resizeInterval > 0) {
                var resizeIntervalId = setInterval(function(){
                    _Resize(container, overlay, settings, fixed);
                }, settings.resizeInterval);
                container.data("LoadingOverlayResizeIntervalId", resizeIntervalId);
            }
            if (!settings.fade) {
                settings.fade = [0, 0];
            } else if (settings.fade === true) {
                settings.fade = [400, 200];
            } else if (typeof settings.fade == "string" || typeof settings.fade == "number") {
                settings.fade = [settings.fade, settings.fade];
            }
            container.data("LoadingOverlayFadeOutDuration", settings.fade[1]);
            overlay.hide().appendTo(container).fadeIn(settings.fade[0]);
        }
        count++;
        container.data("LoadingOverlayCount", count);
    }

    function _Hide(container, force){
        container = $(container);
        var count = container.data("LoadingOverlayCount");
        if (count === undefined) return;
        count--;
        if (force || count <= 0) {
            var resizeIntervalId = container.data("LoadingOverlayResizeIntervalId");
            if (resizeIntervalId) clearInterval(resizeIntervalId);
            container.children(".loadingoverlay").fadeOut(container.data("LoadingOverlayFadeOutDuration"), function(){
                $(this).remove();
            });
            container.removeData(["LoadingOverlayCount", "LoadingOverlayFadeOutDuration", "LoadingOverlayResizeIntervalId"]);
        } else {
            container.data("LoadingOverlayCount", count);
        }
    }

    function _Resize(container, overlay, settings, fixed){
        if (!fixed) overlay.css({
            "width"     : container.innerWidth(),
            "height"    : container.innerHeight()
        });
        var size = "auto";
        if (settings.size && settings.size != "auto") {
            var c = fixed ? $(window) : container;
            size = Math.min(c.innerWidth(), c.innerHeight()) * parseFloat(settings.size) / 100;
            if (settings.maxSize && size > parseInt(settings.maxSize)) size = parseInt(settings.maxSize)+"px";
            if (settings.minSize && size < parseInt(settings.minSize)) size = parseInt(settings.minSize)+"px";
        }
        overlay.css("background-size", size);
        overlay.children(".loadingoverlay_fontawesome").css("font-size", size);
    }

}(jQuery));


/***************************************************************************************************

:: BEELATTE :: Pixiv Manga Translation Helper

****************************************************************************************************/

function isMobileDevice() {
    return window.location.href.indexOf('touch') > - 1;
}

var translation_url = 'https://docs.google.com/spreadsheets/d/1_EN8zdFUMf216DpDV-mQ9y7Bzryc8fgTnV1qEbxeyqk/pubhtml';
var body = $("body");
body.LoadingOverlay("show");
$(document).ready(function() {
    if (isMobileDevice()) {
        Tabletop.init( { key: translation_url, callback: getTranslationMobile, simpleSheet: true } );
    } else {
        Tabletop.init( { key: translation_url, callback: getTranslationDesktop, simpleSheet: true } );
    }
});

function getParameterByName(name, url) {
    if (!url) { url = window.location.href; }
    name = name.replace(/[\[\]]/g, "\\$&");
    var regex = new RegExp("[?&]" + name + "(=([^&#]*)|&|#|$)"),
        results = regex.exec(url);
    if (!results) return null;
    if (!results[2]) return '';
    return decodeURIComponent(results[2].replace(/\+/g, " "));
}

function createBubble(cx, cy, rx, ry, text) {
    var bubble;
    bubble = document.createElement("ELLIPSE");
    bubble.className = 'tooltip';
    bubble.title = '<p style="font-size:15px">' + text + '</p>';
    bubble.setAttribute("cx", cx);
    bubble.setAttribute("cy", cy);
    bubble.setAttribute("rx", rx);
    bubble.setAttribute("ry", ry);
    bubble.style.fill = 'rgba(100, 100, 100, 0)';
    bubble.style.stroke = 'black';
    bubble.style.strokeWidth = 3;
    return bubble.outerHTML;
}

function getBubbles(data, page_number) {
    var bubbles, bubbles_list;
    bubbles = '';
    bubbles_list = data.filter(function(bubble) { return bubble.page == page_number; });
    for (var j = 0; j < bubbles_list.length; j++) {
        bubbles += createBubble(bubbles_list[j].cx, bubbles_list[j].cy, bubbles_list[j].rx, bubbles_list[j].ry, bubbles_list[j].text);
    }
    return bubbles;
}

function updateCanvasSizeMobile() {
    $('.work-image').each(function (index) {
        var canvas_list = document.getElementsByClassName("work-canvas");
        canvas_list[index].style.top = $(this).position().top + 'px';
        canvas_list[index].style.width = $(this).outerWidth() + 'px';
        canvas_list[index].style.height = $(this).outerHeight() + 'px';
    });
}

function updateCanvasSizeDesktop() {
    var canvas_list = document.getElementsByClassName("work-canvas");
    var index = pixiv.mangaViewer.listView.position;
    var list = pixiv.mangaViewer.listView;
    var width, height;
    for (var i = 0; i < canvas_list.length; i++) {
        width = list.imageElements[i].width;
        height = list.imageElements[i].height;
        canvas_list[i].style.width = width;
        canvas_list[i].style.height = height;
    }
}

function getTranslationMobile(data, tabletop) {
    var illust_id = getParameterByName('illust_id', window.location.href);
    var image_list;
    data = tabletop.sheets(illust_id).elements;
    if (typeof data === 'undefined') {
        body.LoadingOverlay("hide");
    } else {
        var viewer = document.getElementsByClassName("viewer-comic");
        var group = viewer[0].getElementsByTagName("UL");
        image_list = group[0].getElementsByTagName("IMG");
        for (var i = 0; i < image_list.length; i++) {
            var canvas = document.createElement("SVG");
            canvas.className = 'work-canvas';
            canvas.style.position = 'absolute';
            canvas.style.margin = '10px 0px 10px 0px';
            var image = data.filter(function(e) { return e.page == i; });
            if (typeof image[0] === 'undefined') {
                // do nothing
            } else {
                canvas.setAttribute("viewBox", '0 0 ' + image[0].w + ' ' + image[0].h);
                var bubbles = getBubbles(data, i);
                canvas.insertAdjacentHTML('afterbegin', bubbles);
                image_list[i].insertAdjacentHTML('beforebegin', canvas.outerHTML);
                image_list[i].className = 'work-image';
            }
        }
        updateCanvasSizeMobile();
        $(window).resize(function() { updateCanvasSizeMobile(); });
        $('.tooltip').tooltipster(
            {animation: 'fade',
             animationDuration: 0,
             theme: 'tooltipster-noir',
             repositionOnScroll: true,
             contentAsHTML: true,
             interactive: true,
             side: ['top','bottom','right'],
             delay: 0,
             maxWidth: 250,
             trigger: 'click'});
        body.LoadingOverlay("hide");
    }
}

function getTranslationDesktop(data, tabletop) {
    var illust_id = getParameterByName('illust_id', window.location.href);
    var image_list, trigger_action;
    data = tabletop.sheets(illust_id);
    if (typeof data === 'undefined') {
        body.LoadingOverlay("hide");
    } else {
        var canvas, image, bubbles;
        data = data.elements;
        if (getParameterByName('mode', window.location.href) == 'manga_big') {
            var page_no = getParameterByName('page', window.location.href);
            var raw = document.getElementsByTagName('IMG');
            image = data.filter(function(e) { return e.page == page_no; });
            if (typeof image[0] === 'undefined') {
                // do nothing
            } else {
                canvas = document.createElement('SVG');
                canvas.className = 'work-canvas';
                canvas.style.position = 'absolute';
                canvas.style.width = raw[0].width + 'px';
                canvas.style.height = raw[0].height + 'px';
                canvas.setAttribute("viewBox", '0 0 ' + image[0].w + ' ' + image[0].h);
                bubbles = getBubbles(data, page_no);
                canvas.insertAdjacentHTML('afterbegin', bubbles);
                raw[0].insertAdjacentHTML('beforebegin', canvas.outerHTML);
            }
        } else {
            image_list = document.getElementsByClassName('image ui-scroll-view');
            for (var i = 0; i < image_list.length; i++) {
                canvas = document.createElement('SVG');
                canvas.className = 'work-canvas';
                canvas.style.position = 'absolute';
                canvas.style.margin = '5px 37px 5px 0';
                image = data.filter(function(e) { return e.page == i; });
                if (typeof image[0] === 'undefined') {
                    // do nothing
                } else {
                    canvas.setAttribute("viewBox", '0 0 ' + image[0].w + ' ' + image[0].h);
                }
                bubbles = getBubbles(data, i);
                canvas.insertAdjacentHTML('afterbegin', bubbles);
                image_list[i].insertAdjacentHTML('beforebegin', canvas.outerHTML);
            }
            updateCanvasSizeDesktop();
            colon.w.on('resize.mangaViewer', colon.debounce(function () { updateCanvasSizeDesktop(); }, this));
            colon.w.on('scroll.mangaViewer', colon.throttle(function () { updateCanvasSizeDesktop(); }, this));
        }
        $('.tooltip').tooltipster(
            {animation: 'fade',
             animationDuration: 0,
             theme: 'tooltipster-noir',
             repositionOnScroll: true,
             contentAsHTML: true,
             interactive: true,
             side: ['top','bottom','right'],
             delay: 0,
             maxWidth: 250,
             trigger: 'hover'});
        body.LoadingOverlay("hide");
    }
}