...
// @downloadURL https://update.greasyfork.org/scripts/521824/DC%E6%97%B6%E9%97%B4%E7%AE%A1%E7%90%86%E8%80%85.user.js
// @updateURL https://update.greasyfork.org/scripts/521824/DC%E6%97%B6%E9%97%B4%E7%AE%A1%E7%90%86%E8%80%85.meta.js
// ==/UserScript==
...
let autoStartInterval = null; // 定时器控制变量
...
function saveSettings() {
  ...
  function startAutoSend() {
    ...
    if (now >= start && now <= end) {
      logMessage(`自动发送启动 (时间: ${now.toLocaleTimeString()})`);
      scheduleNextMessage();
    } else {
      logMessage(`当前时间不在设定时间段内: ${startTime} - ${endTime}`);
    }
  }
  ...
  function stopAutoSend() {
    if (timer) {
      clearTimeout(timer);
      timer = null;
      logMessage(`自动发送已停止 (时间: ${new Date().toLocaleTimeString()})`);
    }
  }
...
function createSettings() {
  ...
  settingsDiv.innerHTML = `
    ...
    <label for='auto--start-time' style='display: block; margin-bottom: 10px;'>
      <div>启动时间（格式 HH:mm）：</div>
      <input id='auto--start-time' style='padding: 5px; width: 100%;' type='time' value='08:00' />
    </label>
    <label for='auto--end-time' style='display: block; margin-bottom: 10px;'>
      <div>停止时间（格式 HH:mm）：</div>
      <input id='auto--end-time' style='padding: 5px; width: 100%;' type='time' value='22:00' />
    </label>
    <div id='auto--message-count' style='margin-top: 20px; color: #FFD700; font-weight: bold; text-align: center;'>
      已发送消息数量：0
    </div>
    <div style='text-align: center; margin-top: 20px;'>
      <button id='auto--start' style='padding: 5px 10px; margin-right: 10px;'>保存设置并启动</button>
