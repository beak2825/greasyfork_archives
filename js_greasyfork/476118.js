// ==UserScript==
// @name         IdlePixel Market Overhaul - Zlef Fork
// @namespace    com.anwinity.idlepixel
// @version      1.0.0
// @description  Overhaul of market UI and functionality. Undercutting update
// @author       Original Author: Anwinity || Modded By: GodofNades || Modded by: Zlef
// @license      MIT
// @match        *://idle-pixel.com/login/play*
// @grant        none
// @require      https://greasyfork.org/scripts/441206-idlepixel/code/IdlePixel+.js?anticache=20220905
// @downloadURL https://update.greasyfork.org/scripts/476118/IdlePixel%20Market%20Overhaul%20-%20Zlef%20Fork.user.js
// @updateURL https://update.greasyfork.org/scripts/476118/IdlePixel%20Market%20Overhaul%20-%20Zlef%20Fork.meta.js
// ==/UserScript==
...
`);
            ...
            const sellPriceInput = sellModal.find("#modal-market-configure-item-to-sell-price-each");

            sellPriceInput.after(`
  ...
  <input type="text" id="purchase-price-input">
  <br />
  Total: <span id="modal-market-configure-item-to-sell-total"></span>
`);

            // Event listener to calculate the "Price Each" based on "Purchase price"
            $("#purchase-price-input").on("input", function() {
                const targetNumber = parseFloat($(this).val());
                if (!isNaN(targetNumber)) {
                    const salePrice = Math.ceil(targetNumber / 1.01);
                    $("#modal-market-configure-item-to-sell-price-each").val(salePrice);
                    IdlePixelPlus.plugins.market.applyTotalSell(); // Update the total
                }
            });

            sellAmountInput.on("input change", () => IdlePixelPlus.plugins.market.applyTotalSell());
            sellPriceInput.on("input change", () => IdlePixelPlus.plugins.market.applyTotalSell());

            const purchasePriceInput = $("#purchase-price-input");
            const totalLabel = $("#modal-market-configure-item-to-sell-total");

            // Set up the mutation observer to clear the new entry box and total label when the modal first opens
            const observer = new MutationObserver(function(mutationsList) {
                for(let mutation of mutationsList) {
                    if (mutation.type === 'attributes' && mutation.attributeName === 'style' && $(mutation.target).is(':visible')) {
                        purchasePriceInput.val(''); // Clear the "Purchase price" input
                        totalLabel.text('0');      // Reset the "Total price" label
                    }
                }
            });

            observer.observe(sellModal[0], { attributes: true, attributeFilter: ['style'] });
