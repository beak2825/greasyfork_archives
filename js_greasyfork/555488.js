...
// @description:am      የYouTube ታምብኔይል ሆቨር ቅድመ-እይታዎችን (WebP እና ቪዲዮ) ያግዳል እና የቻናል ትሬይለሮችን (ቪዲዮ + ኦዲዮ) ያቆማል። የእድገት አሞሌዎችን፣ የፕሌይ ቁልፎችን እና የማይንቀሳቀሱ ምስሎችን ያቆᯫል። በሁሉም ገፆች ላይ ይሰራል፣ ምንም መግባት ወይም ፍቃድ አያስፈልግም፣ በሰነድ-ጅምር ይሰራል።
// @description:ar      يحظر YouTube معاينات الصور المصغرة عند التمرير (WebP وفيديو) ويوقف تريلرات القناة (فيديو + صوت). يحافظ على أشرطة التقدم، أزرار التشغيل، والصور الثابتة. يعمل على جميع الصفحات، بدون تسجيل دخول أو أذونات، يبدأ فور تحميل الصفحة.
// @description:as      YouTube-তে অটোপ্লে থাম্বনেইল হোভার পূর্বদর্শন (WebP আৰু ভিডিঅ') আৰু চেনেল ট্ৰেইলাৰ (ভিডিঅ' + অডিঅ') ব্লক করে। প্ৰগ্ৰেছ বাৰ, প্লে বুটাম আৰু স্থিৰ ছবি ৰাখে। সকলো পৃষ্ঠাত কাম করে, লগইন বা অনুমতিৰ প্ৰয়োজন নাই, পৃষ্ঠা লোড হোৱাৰ লগে লগে চলে।
...
// @description:bg      Блокира предварителни прегледи при навеждане на YouTube миниатюри (WebP и видео) и спира трейлърите на канали (видео + звук). Запазва лентите за напредък, бутоните за възпроизвеждане и статичните изображения. Работи на всички страници, без вход или разрешения, стартира при зареждане на страницата.
// @description:bn      থাম্বনেল হোভার প্রিভিউ YouTube (WebP এবং ভিডিও) ব্লক করে এবং চ্যানেল ট্রেলার (ভিডিও + অডিও) বন্ধ করে। প্রোগ্রেস বার, প্লে বোতাম এবং স্থির ছবি রাখে। সব পৃষ্ঠায় কাজ করে, লগইন বা অনুমতি ছাড়া, পৃষ্ঠা লোড হওয়ার সাথে সাথে চলে এবং ডকুমেন্ট-স্টার্টে রান করে।
// @description:bs      Blokira pregled sličica pri lebdenju kursora na YouTube (WebP i video) i zaustavlja trejlere kanala (video + zvuk). Čuva trake napretka, dugmiće za reprodukciju i statične slike. Radi na svim stranicama, bez prijave ili dozvole, pokreće se pri učitavanju stranice.
// @description:ca      Bloqueja les visualitzacions prèvies de les miniatures de YouTube en passar-hi per sobre (WebP i vídeo) i atura els tràilers del canal (vídeo + àudio). Manté les barres de progrés, botons de reproducció i imatges estàtiques. Sense inici de sessió ni permisos, s’executa al carregament de la pàgina.
// @description:cs      Blokuje náhledy miniaturek YouTube při najetí myší (WebP a video) a zastavuje trailery kanálů (video + zvuk). Zachovává ukazatele postupu, tlačítka přehrávání a statické obrázky. Bez přihlášení nebo oprávnění, spouští se při načtení stránky.
// @description:cy      Yn blocio rhagolwg chwarae awtomatig ar smotynnau YouTube (WebP a fideo) ac yn stopio treilar sianel (fideo + sain). Yn cadw barrau cynnydd, botymau chwarae a delweddau statig. Yn gweithio ar bob tudalen, dim mewngofnodi na chaniatâd, yn rhedeg ar document-start.
// @description:da      Blokerer miniature-hover-forhåndsvisninger på YouTube (WebP og video) og stopper kanaltrailere (video + lyd). Bevarer fremskridtsmærker, afspilningsknapper og statiske billeder. Ingen login eller tilladelser, kører ved sideindlæsning.
// @description:de      Blockiert Hover-Vorschauen von YouTube Thumbnails (WebP & Video) und stoppt Kanaltrailer (Video + Ton). Behält Fortschrittsbalken, Play-Buttons und statische Bilder bei. Kein Login oder Berechtigungen erforderlich, startet beim Seitenladen.
// @description:el      Μπλοκάρει προεπισκοπήσεις εικόνων μικρογραφιών YouTube κατά το hover (WebP και βίντεο) και σταματά τρέιλερ καναλιών (βίντεο + ήχος). Διατηρεί γραμμές προόδου, κουμπιά αναπαραγωγής και στατικές εικόνες. Χωρίς σύνδεση ή άδειες, εκτελείται κατά τη φόρτωση σελίδας.
// @description:eo      Blokas miniaturojn hover antaŭrigardojn de YouTube (WebP & video) kaj haltigas kanalajn antaŭfilmojn (video + sono). Konservas progresajn stangojn, ludbutonojn kaj statikajn bildojn. Sen ensaluto aŭ permesoj, lanĉiĝas ĉe ŝarĝo de paĝo.
// @description:es      Bloquea vistas previas de miniaturas de YouTube al pasar el mouse (WebP y video) y detiene tráilers de canal (video + audio). Mantiene barras de progreso, botones de reproducción y imágenes estáticas. Sin inicio de sesión ni permisos, se ejecuta al cargar la página.
// @description:et      Blokeerib YouTube thumbnail-kursorilähisvaadet (WebP ja video) ja peatab kanalite treilerid (video + heli). Säilitab edenemisribad, esitusnupud ja staatilised pildid. Ilma sisselogimise või lubadeta, käivitub lehe laadimisel.
// @description:eu      Blokeatu YouTube thumbnail hover aurrebista (WebP eta bideoa) eta gelditu kanalaren trailerra (bideoa + audioa). Mantentzen ditu aurrerapen-barrak, erreproduzitzeko botoiak eta irudi estatikoak. Saioa hasi gabe eta baimenik gabe, orria kargatzean exekutatzen da.
// @description:fa      پیش‌نمایش خودکار تصویر بندانگشتی YouTube هنگام هاور (WebP و ویدیو) و تریلر کانال‌ها (ویدیو + صدا) را مسدود می‌کند. نوار پیشرفت، دکمه‌های پخش و تصاویر ثابت را حفظ می‌کند. در تمام صفحات کار می‌کند، نیازی به ورود یا مجوز نیست و در زمان بارگذاری صفحه اجرا می‌شود.
// @description:fi      Estää YouTube thumbnail-kohdistus-esikatselut (WebP ja video) ja pysäyttää kanavan trailerit (video + ääni). Säilyttää edistymispalkit, toistonapit ja staattiset kuvat. Ei kirjautumista tai lupia, käynnistyy sivun latautuessa.
...
// @description:hu      Blokkolja a YouTube miniatűr hover előnézeteket (WebP és video) és megállítja a csatorna előzeteseit (video + hang). Megtartja a folyamatjelzőket, lejátszási gombokat és statikus képeket. Bejelentkezés vagy engedély nélkül, oldal betöltésekor fut.
// @description:id      Memblokir pratinjau hover thumbnail YouTube (WebP & video) dan menghentikan trailer saluran (video + audio). Menjaga bilah kemajuan, tombol putar dan gambar statis. Tanpa login atau izin, berjalan saat halaman dimuat.
// @description:is      Blokkar sjálfvirka forskoðun á smámyndum YouTube (WebP og myndskeið) og stöðvar rásarstiklur (myndskeið + hljóð). Heldur áfram framfara stöng, spila hnöppum og kyrrstæðum myndum. Engin innskráning eða heimild, keyrir við hleðslu síðu.
// @description:it      Blocca anteprime hover su miniature YouTube (WebP e video) e ferma i trailer dei canali (video + audio). Mantiene barre di avanzamento, pulsanti play e immagini statiche. Nessun login o permessi, si avvia al caricamento della pagina.
...
// @description:ka      ბლოკავს YouTube მინიატურების ჰოვერ წინასწარ ჩვენებას (WebP & ვიდეო) და აჩერებს არხის ტრეილერებს (ვიდეო + ხმა). ინახავს პროგრესის ზოლებს, დაკვრის ღილაკებს და სტატიკურ სურათებს. შესვლა ან ნებართვა არაა საჭირო, გვერდის ჩატვირთვისას მუშაობს.
...
// @description:km      ទប់ស្កាត់ការបង្ហាញជាមុនផ្ទាំងរូបភាពតូច YouTube (WebP និងវីដេអូ) នៅពេលរំកិលកណ្តុរ និងបញ្ឈប់វីដេអូផ្សព្វផ្សាយឆានែល (វីដេអូ + សំឡេង)។ រក្សាទុករបារវឌ្ឍនភាព ប៊ូតុងចាក់ និងរូបភាពឋិតិវន្ត។ ដំណើរការលើគ្រប់ទំព័រ មិនតម្រូវឱ្យចូលគណនី ឬសិទ្ធិ ចាប់ផ្តើមនៅពេលផ្ទុកទំព័រ។
...
// @description:lo      ບລັອກການສະແດງຕົວຢ່າງອັດຕະໂນມັດຂອງ YouTube ຮູບຍໍ້ (WebP ແລະ ວິດີໂອ) ແລະ ຢຸດຕົວຢ່າງຊ່ອງ (ວິດີໂອ + ສຽງ). ຮັກສາແຖບຄວາມຄືບໍາ, ປຸ່ມຫຼິ້ນ ແລະ ຮູບພາບຄົງທີ່. ເຮັດວຽກໃນທຸກໜ້າ, ບໍ່ຕ້ອງເຂົ້າສູ່ລະບົບ ຫຼື ອະນຸຍາດ, ເລີ່ມເຮັດວຽກຕອນທີ່ໂຫຼດໜ້າ.
// @description:lt      Blokuoja YouTube miniatiūrų užvedimo peržiūras (WebP ir vaizdo įrašus) ir sustabdo kanalų anonsus (vaizdo įrašas + garsas). Išlaiko pažangos juostas, grojimo mygtukus ir statinius vaizdus. Be prisijungimo ar leidimų, veikia puslapio įkėlimo metu.
// @description:lv      Bloķē YouTube sīktēlu pārslēgšanās priekšskatījumu (WebP un video) un aptur kanāla treilerus (video + audio). Saglabā progresa joslas, atskaņošanas pogas un statiskus attēlus. Bez pieteikšanās vai atļaujām, darbojas lapas ielādes laikā.
// @description:ms      Menghalang pratinjau hover thumbnail YouTube (WebP & video) dan menghentikan treler saluran (video + audio). Menyimpan bar kemajuan, butang main dan imej statik. Tiada log masuk atau kebenaran, dijalankan semasa halaman dimuat.
// @description:nb      Blokkerer forhåndsvisninger av YouTube miniatyrbilder ved svev (WebP og video) og stopper kanaltrailere (video + lyd). Beholder fremdriftslinjer, avspillingsknapper og statiske bilder. Ingen innlogging eller tillatelser, kjører ved sidelasting.
...
// @description:nl      Blokkeert hover-voorbeelden van YouTube thumbnails (WebP en video) en stopt kanaaltrailers (video + audio). Behoudt voortgangsbalken, afspeelknoppen en statische afbeeldingen. Geen login of toestemming, start bij paginalading.
...
// @description:pt      Bloqueia pré-visualizações de miniaturas YouTube ao passar o rato (WebP e vídeo) e interrompe trailers de canal (vídeo + áudio). Mantém barras de progresso, botões de reprodução e imagens estáticas. Sem início de sessão ou permissões, executa ao carregar a página.
// @description:pt-BR   Bloqueia prévias de miniaturas YouTube ao passar o mouse (WebP e vídeo) e interrompe trailers de canal (vídeo + áudio). Mantém barras de progresso, botões de play e imagens estáticas. Sem login ou permissão, executa ao carregar a página.
// @description:ro      Blochează previzualizări la hover pe miniaturi YouTube (WebP și video) și oprește trailerele de canal (video + audio). Păstrează barele de progres, butoanele de redare și imaginile statice. Fără login sau permisiuni, rulează la încărcarea paginii.
...
// @description:sq      Bllokon parapamjen e hover thumbnail YouTube (WebP dhe video) dhe ndalon trailerat e kanalit (video + audio). Mbaj shirita përparimi, butona luajtjeje dhe imazhe statike. Pa hyrje ose leje, ekzekutohet gjatë ngarkimit të faqes.
// @description:sr      Блокира аутоматски преглед минијатура YouTube при лебдећу показивачу (WebP и видео) и зауставља трејлере канала (видео + звук). Чува траке напретка, дугмиће за репродукцију и статичне слике. Без пријаве или дозволе, покреће се при учитавању странице.
...
// @description:te      YouTube థంబ్‌నెయిల్ హోవర్ ప్రివ్యూలను (WebP & వీడియో) బ్లాక్ చేస్తుంది మరియు ఛానెల్ ట్రైలర్‌లను (వీడియో + ఆడియో) ఆపేస్తుంది. ప్రోగ్రెస్ బార్‌లు, ప్లే బటన్‌లు మరియు స్టాటిక్ ఇమేజ్‌లను ఉంచుతుంది. అన్ని పేజీలలో పని చేస్తుంది, లాగిన్ లేదా అనుమతులు అవసరం లేదు, డాక్యుమెంట్-స్టార్ట్ వద్ద రన్ అవుతుంది.
