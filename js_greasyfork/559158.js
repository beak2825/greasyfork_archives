// ==UserScript==
// @name         Open Links in Background Tabs One-Button Toggle (Opt-in)
// @version      2.2
// @description  Default: DISABLED. Use script menu to Enable per host. A movable semi-transparent toggle appears in the corner. ON: links open in background tabs, Shift=foreground, Alt=current
// @author       Te55eract
...
// @noframes
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/559158/Open%20Links%20in%20Background%20Tabs%20One-Button%20Toggle%20%28Opt-in%29.user.js
// @updateURL https://update.greasyfork.org/scripts/559158/Open%20Links%20in%20Background%20Tabs%20One-Button%20Toggle%20%28Opt-in%29.meta.js
// ==/UserScript==
...
(function () {
  ...
  // CHANGED: We now track if it is ENABLED. Default is false (Opt-in).
  const MASTER_ENABLED_PREFIX = 'toggleBgTabs:masterEnabled:';
  ...
  // Keys
  let pageKey = currentPageKey();                 // per-page key
  let hostKey = currentHostKey();                 // per-hostname key

  // NEW LOGIC: Default to FALSE. User must enable it.
  let isMasterEnabled = !!GM_getValue(MASTER_ENABLED_PREFIX + hostKey, false);
  ...
  function updateMasterMenuCommand() {
    ...
    // Logic flipped: If enabled, offer to disable. If disabled (default), offer to enable.
    const menuText = isMasterEnabled
  ...
  function toggleMasterState() {
    ...
    GM_setValue(MASTER_ENABLED_PREFIX + hostKey, isMasterEnabled);

    if (!isMasterEnabled) {
      // User just disabled it
      if (wrap) wrap.style.display = 'none';
      console.log('[BG Tabs Toggle] Script disabled for host:', hostKey);
    } else {
      // User just enabled it
      if (wrap) {
        wrap.style.display = '';
      } else {
        initializeUI();
  ...
  // Initialize the menu command immediately
  updateMasterMenuCommand();
  ...
  function initializeUI() {
    ...
    document.addEventListener('click', function (e) {
      // CHANGED: Check isMasterEnabled (true = running)
      if (!isMasterEnabled || !enabled) return;
    ...
    hookLocationChanges(() => {
      ...
      // Update Master State on Nav
      isMasterEnabled = !!GM_getValue(MASTER_ENABLED_PREFIX + hostKey, false);
      updateMasterMenuCommand();
      ...
      if (!isMasterEnabled) {
        ...
        else initializeUI(); // Ensure UI exists if we navigated into an enabled zone
  ...
  // CHANGED: Only initialize if Master is Enabled
  if (isMasterEnabled) {
    ...
    console.log('[BG Tabs Toggle] Script waiting (Default Disabled). Enable via menu for:', hostKey);
  }
})();