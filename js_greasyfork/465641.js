...
// @version      1.0.0
// @description  通过鼠标右键选择元素
// @author       xuanzai
// @match        *://*/*
// @icon         https://www.google.com/s2/favicons?sz=64&domain=tampermonkey.net
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/465641/%E5%85%83%E7%B4%A0%E9%80%89%E6%8B%A9%E5%99%A8.user.js
// @updateURL https://update.greasyfork.org/scripts/465641/%E5%85%83%E7%B4%A0%E9%80%89%E6%8B%A9%E5%99%A8.meta.js
// ==/UserScript==
...
(function () {
  ...
  function getSelectorRes(el, deep = false) {
    if (!(el instanceof Element)) return
    const path = []
    let deepIndex = 0
    // 仅选三层
    while (el.nodeType === Node.ELEMENT_NODE) {
      if (!deep && deepIndex > 2) break
      ...
      if (el.id) {
        ...
        if (!deep && el.className && typeof el.className === 'string') {
          selector += '.' + el.className.split(' ').filter(val => val.replace(/\s+/, '')).join('.')
        }
        let sib = el,
          nth = 1
      ...
      el = el.parentNode
      deepIndex++
      console.log('deepIndex', deepIndex)
    }
    return !deep ? path.join(' ') : path.join('>')
  }
  // 写入剪贴板
  ...
  function track_mouse(event) {
    if (event.button == 2) {
      ...
      if (elementMouseIsOver !== document.querySelector(selector)) {
        selector = getSelectorRes(elementMouseIsOver, true)
      }
