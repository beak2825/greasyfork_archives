...
// @downloadURL https://update.greasyfork.org/scripts/562180/WME%20EZRoad%20Mod.user.js
// @updateURL https://update.greasyfork.org/scripts/562180/WME%20EZRoad%20Mod.meta.js
// ==/UserScript==
...
- Bug fix: Enhanced "Copy Connected Segment Attribute" logic.<br>`;
  ...
  const constructSettings = () => {
    ...
    wmeSDK.Sidebar.registerScriptTab().then(({ tabLabel, tabPane }) => {
      ...
      const presetSection = $(`<div class="ezroadsmod-section" style="margin-top: 15px; padding: 10px; border: 1px solid #ccc; border-radius: 5px; background-color: #f9f9f9;">
        <h5 style="margin-top: 0;">Custom Lock/Speed Presets</h5>
        <div style="margin-bottom: 10px;">
          <input type="text" id="ezroadsmod-preset-name" placeholder="Enter preset name" style="width: 200px; padding: 5px; margin-right: 5px;">
          <button id="ezroadsmod-save-preset" style="padding: 5px 10px; cursor: pointer;">Save Current Config</button>
        </div>
        <div id="ezroadsmod-preset-list" style="margin-top: 10px;"></div>
      </div>`);
      scriptContentPane.append(presetSection);
      ...
      function updatePresetList() {
        const presetList = $('#ezroadsmod-preset-list');
        presetList.empty();
        const options = getOptions();
        const presets = options.customPresets || [];

        if (presets.length === 0) {
          presetList.append('<div style="color: #888; font-style: italic;">No custom presets saved yet.</div>');
          return;
        }

        presets.forEach((preset, index) => {
          const presetItem = $(`
            <div style="display: flex; align-items: center; margin-bottom: 8px; padding: 8px; border: 1px solid #ddd; border-radius: 4px; background-color: white;">
              <span style="flex-grow: 1; font-weight: bold;">${preset.name}</span>
              <button class="ezroadsmod-load-preset" data-index="${index}" style="padding: 4px 8px; margin-right: 5px; cursor: pointer; background-color: #4CAF50; color: white; border: none; border-radius: 3px;">Load</button>
              <button class="ezroadsmod-delete-preset" data-index="${index}" style="padding: 4px 8px; cursor: pointer; background-color: #f44336; color: white; border: none; border-radius: 3px;">Delete</button>
            </div>
          `);
      ...
      $(document).on('click', '#ezroadsmod-save-preset', function () {
        ...
        const existingIndex = options.customPresets.findIndex(p => p.name === presetName);
        if (existingIndex !== -1) {
          if (!confirm(`Preset "${presetName}" already exists. Do you want to overwrite it?`)) {
            return;
          }
          options.customPresets.splice(existingIndex, 1);
        }

        // Save current lock/speed config as preset
        const preset = {
          name: presetName,
          locks: JSON.parse(JSON.stringify(options.locks)),
          speeds: JSON.parse(JSON.stringify(options.speeds))
        };

        options.customPresets.push(preset);
        saveOptions(options);
        ...
        if (WazeWrap?.Alerts) {
          WazeWrap.Alerts.success('EZRoads Mod', `Preset "${presetName}" saved successfully!`, false, false, 2000);
        } else {
      ...
      $(document).on('click', '.ezroadsmod-load-preset', function () {
        const index = parseInt($(this).data('index'));
        const options = getOptions();
        const preset = options.customPresets[index];
        ...
        options.locks = JSON.parse(JSON.stringify(preset.locks));
        options.speeds = JSON.parse(JSON.stringify(preset.speeds));
        saveOptions(options);

        // Update in-memory localOptions and UI
        localOptions.locks = preset.locks;
        localOptions.speeds = preset.speeds;

        // Update lock dropdowns
        $('.road-lock-level').each(function () {
          const roadId = $(this).data('road-id');
          const lockSetting = preset.locks.find((l) => l.id == roadId);
        ...
        $('.road-speed').each(function () {
          const roadId = $(this).data('road-id');
          const speedSetting = preset.speeds.find((s) => s.id == roadId);
          if (speedSetting) $(this).val(speedSetting.speed);
        });

        if (WazeWrap?.Alerts) {
          WazeWrap.Alerts.success('EZRoads Mod', `Preset "${preset.name}" loaded!`, false, false, 2000);
        } else {
      ...
      $(document).on('click', '.ezroadsmod-delete-preset', function () {
        const index = parseInt($(this).data('index'));
        const options = getOptions();
        const preset = options.customPresets[index];
        ...
        options.customPresets.splice(index, 1);
        saveOptions(options);
        updatePresetList();

        if (WazeWrap?.Alerts) {
          WazeWrap.Alerts.success('EZRoads Mod', `Preset "${preset.name}" deleted!`, false, false, 2000);
        } else {
          alert(`Preset "${preset.name}" deleted!`);
        }
      });
