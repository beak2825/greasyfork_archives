...
// @grant          unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/472929/bilibili%20Setting%20Video%20Quality.user.js
// @updateURL https://update.greasyfork.org/scripts/472929/bilibili%20Setting%20Video%20Quality.meta.js
// ==/UserScript==
...
(function() {
    ...
    var App = function App() {
        ...
        var defaultTargetQualitys = [ 1080, 720, 480, 360 ];
        var _useGmValue = useGmValue("targetQualitys", defaultTargetQualitys), targetQualitys = _useGmValue.data, setTargetQualitys = _useGmValue.updateData;
        var defaultHighBitratePriority = true;
        var _useGmValue2 = useGmValue("highBitratePriority", defaultHighBitratePriority), highBitratePriority = _useGmValue2.data, setHighBitratePriority = _useGmValue2.updateData;
        useGmMenu("設置目標品質", (function() {
            var quality = prompt("請輸入目標品質優先序, 逗點分隔（1080, 720, 480, 360）:", targetQualitys.join(","));
            if (quality) {
                var qualityPriority = quality.split(",").map((function(q) {
                    return parseInt(q, 10);
                }));
                var isLegal = qualityPriority.every((function(q) {
                    return !isNaN(q);
                }));
                if (isLegal) {
        ...
        useGmMenu("設置高比特率優先", (function() {
            var priority = confirm("是否要設置高比特率優先？");
        ...
        reactExports.useEffect((function() {
            ...
            var qualityList = dashPlayer.player.getBitrateInfoListFor("video");
            var priorityQualityList = qualityList.map((function(item, index) {
                var priority = targetQualitys.indexOf(item.height);
            ...
            })).sort((function(currItem, nextItem) {
                return highBitratePriority ? nextItem.bitrate - currItem.bitrate : currItem.bitrate - nextItem.bitrate;
            })).sort((function(currItem, nextItem) {
                return currItem.priority - nextItem.priority;
            }));
            if (priorityQualityList.length === 0) {
                console.log("priorityQualityList not found");
                return;
            }
            var targetIdx = priorityQualityList[0].index;
            dashPlayer.player.setQualityFor("video", targetIdx);
            dashPlayer.player.setDefaultQualityFor("video", targetIdx);
            console.log("set video quality to ".concat(priorityQualityList[0].height));
