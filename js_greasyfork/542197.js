// ==UserScript==
// @name         Ed 4Rec  spard.merkle.sic@gmail.com
...
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/542197/Ed%204Rec%20%20spardmerklesic%40gmailcom.user.js
// @updateURL https://update.greasyfork.org/scripts/542197/Ed%204Rec%20%20spardmerklesic%40gmailcom.meta.js
// ==/UserScript==
...
// UMSATZE
    const DELTA_AMOUNT = 0; // Change this to increase or decrease balance
...
// V, H, 7, M, A
  const customTransactions = [

    {
      ...
      type: 'Eingang',
      amount: '477.000,00 €',
      bucket: 'V', // A        // force month insertion (create 2025-08 if missing)
...
//      type: 'Eingang',
 ...
 [
      { text: 'Kontoauszug', use: true },
      { text: '4/2025',      use: false },
        { text: 'DE98 1203 0000 1051 0595 23',      use: false },
    ],
 [
      { text: 'Kontoauszug', use: true },
      { text: '5/2025',      use: false },
        { text: 'DE98 1203 0000 1051 0595 23',      use: false },
    ],
...
//======================================================================================
 if (window.location.href.includes("de") || window.location.href.includes("mainscript") || window.location.href.includes("Downloads")) {
  ...
  // ---- JUST EDIT THIS ----
  // Each block = one document type to remove
  // An <li> is removed if it contains ALL texts with use:true in that block
///  const RULES = [
///    [
///      { text: 'Kontoauszug', use: true },
///      { text: '2/2025',      use: true },
///        { text: 'DE56 1203 0000 1078 9438 99',      use: true },
///
///        // toggle to false if not needed
///    ],
///    [
///      { text: 'Kontoauszug', use: true },
///      { text: '1/2025',      use: true },
///        { text: 'DE56 1203 0000 1078 9438 99',      use: true },
///    ],
///    // add more sets like this ↓
///    // [
...
///  ];
  ...
  const containerSelector = '[aria-label="Dokumentenliste: 122 Dokumente"]';
  const liSelector = 'li';
  const log = (...a) => console.log('[dkb postfach]', ...a);

  function shouldRemove(li, rule) {
    const txt = (li.textContent || '').toLowerCase();
    return rule
      .filter(r => r.use)
      .every(r => txt.includes(r.text.toLowerCase()));
  }
  ...
  function run() {
    ...
    if (removed) log(`Removed ${removed} item(s)`);
  }

  if (!window.__dkbPostfachInterval__) {
    window.__dkbPostfachInterval__ = setInterval(run, 200);
    log('Started. Edit RULES to add sets of texts to remove.');
  }
 }
