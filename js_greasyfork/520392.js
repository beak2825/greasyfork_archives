...
// @description  星图扩展工具
// @author       zgx
// @match        *://www.xingtu.cn/ad/creator/author/douyin/*/*?*
...
// @downloadURL https://update.greasyfork.org/scripts/520392/%E6%98%9F%E5%9B%BE%E8%BE%BE%E4%BA%BA%E6%89%B9%E9%87%8F%E5%BB%BA%E8%81%94%28%E8%BE%BE%E4%BA%BA%E8%AF%A6%E6%83%85%29.user.js
// @updateURL https://update.greasyfork.org/scripts/520392/%E6%98%9F%E5%9B%BE%E8%BE%BE%E4%BA%BA%E6%89%B9%E9%87%8F%E5%BB%BA%E8%81%94%28%E8%BE%BE%E4%BA%BA%E8%AF%A6%E6%83%85%29.meta.js
// ==/UserScript==
...
(function () {
    ...
    if (window.location.href.slice(0, 28) === 'http://star.yinlimedia.com/') {
        ...
        if (GM_getValue('user_name') && GM_getValue('user_token') && GM_getValue('kolDetail') == 'kolDetail') {
            Qmsg.success({
                content: `星图达人批量建联：星推用户信息获取成功`,
                timeout: 5000
            });
            GM_setValue('kolDetail','')
            window.close()
        }else if(GM_getValue('kolDetail') == 'kolDetail'){
            Qmsg.error({
    ...
    `;
        ...
        $("#inly_submit").click(function (e) {
            ...
            if(!name_1.trim()){
                ...
                GM_setValue('kolDetail','kolDetail')
    ...
    text.textContent =
        "点击自动建联后，自动添加达人微信，添加结果请在核力星推中的业务管理>达人建联中查看。";
    ...
    getBalance()
    function appendDoc() {
        const likeComment = document.querySelector(".title-wrapper");
        if (likeComment) {
            likeComment.style.position = 'relative'
            likeComment.append(quota); //把按钮加入到 x 的子节点中
    ...
    function listen() {
        ...
        function replaceFn(obj) {
            if (this?._url?.slice(0, 35) == "/gw/api/author/get_author_base_info") {
    ...
    async function urlClick() {
        if (target_data) {
            ...
            // 大于建联额度时，提示用户
            if (1 > JSON.parse(resBalance.response).harass_balance) {
    ...
    async function openReq(key) {
        // 获取用户信息
        let account_id =
            document.querySelector(".copy-text").childNodes[0].textContent;
        let account_name = document.querySelector(
            ".navigator-user__nickname.text-ellipsis"
        ).innerHTML;
        let info_name = document.querySelector(".author-info .name").innerHTML
        let info_id = document.querySelector(".author-info .copy-text").childNodes[0].textContent
        let info_avata = document.querySelector(".author-info .user-avatar-image").src
        console.log(info_avata)
        let param = {
            task_kols: [{ dy_kol_id: info_id, dy_kol_name: info_name, dy_kol_avatar: info_avata}],
            subscribe_name: key,
