// ==UserScript==
// @name         linux.do.cq.auto.guaji
// @namespace    https://linux.do/u/io.oi/cq.auto.fire
// @version      1.3.7
// @author       io.oi
// @description  auto guaji
// @license      MIT
...
// @grant        GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/525941/linuxdocqautoguaji.user.js
// @updateURL https://update.greasyfork.org/scripts/525941/linuxdocqautoguaji.meta.js
// ==/UserScript==
...
注：以上功能全部基于飞鞋，请多充些飞鞋点数">${Icons.startIcon}</button>
    ...
    <button id="auto-experience" class="button" title="自动吃经验瓶+5倍券">${Icons.startIcon}</button>
</div>`;
  ...
  window.addEventListener("load", () => {
    ...
    function useItem(id,count) {
        ...
        //获取人物背包是否有物品
        const item = app.ThgMu.ins().getItemById(id);
        if (item) {
            // 使用物品，前提是 item 不等于空，使用数量小于 item 的 btCount，btCount 最大是一组物品的数量，不是背包的所有数量
            app.pWFTj.ins().useItem(item.series, id, count === 1 || (count > 1 && count < item.btCount) ? count : item.btCount);
        }
    }
    function useExpItem(count) {
      toast(`使用5倍经验券+多倍经验瓶${count}个`);
      useItem(300, count);  // 使用多倍经验瓶
      useItem(462, 1);  // 使用5倍经验券
      checkEdcwsp();    // 检查是否在挂机状态
    }
    // 监听键盘事件
