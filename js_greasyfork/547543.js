...
// @version      1.5.0
// @description  123云盘提取码管理脚本：自动保存、表格管理、跳转链接、复制提取码、导入导出、防止自动写入剪贴板
// @description:zh-CN  自动保存123云盘提取码与文件名，支持表格管理、一键复制、导入导出及 Tampermonkey 云同步。
...
// @downloadURL https://update.greasyfork.org/scripts/547543/123Pan%20CV.user.js
// @updateURL https://update.greasyfork.org/scripts/547543/123Pan%20CV.meta.js
// ==/UserScript==

// 拦截 navigator.clipboard.writeText，防止 123云盘静默写入剪贴板
(function () {
  'use strict';

  const originalWriteText = navigator.clipboard.writeText;
  navigator.clipboard.writeText = async function(text) {
    console.log('[PanCV] 已阻止123云盘自动写入剪贴板:', text);
  ...
  function addRecord(name, code) {
    ...
    if (!/^[A-Za-z0-9]{4}$/.test(code) || /^\d{3}$/.test(code)) {
      console.log('[PanCV] 跳过无效提取码:', code);
      return;
    }
    ...
    ? `https://${domain}/s/${shareKey}`: location.href;
  ...
  function findCodes() {
    ...
    document.cookie.split(';').forEach(c => {
      ...
      if (kv.length === 2 && /^[A-Za-z0-9]{4}$/.test(kv[1]) && !/^\d{3}$/.test(kv[1])) out.add(kv[1]);
    });

    document.querySelectorAll('input').forEach(el => {
      const v = (el.value || '').trim();
      if (/^[A-Za-z0-9]{4}$/.test(v) && !/^\d{3}$/.test(v)) out.add(v);
    });

    return [...out];
  }

// 双重策略获取完整文件名：优先 aria-label，其次拼接 .file-name-line
  ...
  function findFileNames() {
    ...
    document.querySelectorAll('[aria-label]').forEach(el => {
      ...
      if (ariaLabel && ariaLabel.includes('.') && ariaLabel.length < 200) {
        names.add(ariaLabel);
      }
    });

    // 方法2
    document.querySelectorAll('.file-name-display, [class*="file-name"]').forEach(el => {
      const nameParts = Array.from(el.querySelectorAll('.file-name-line')).map(part =>
        (part.textContent || '').trim()
      ).filter(part => part.length > 0);

      if (nameParts.length > 0) {
        const fullFileName = nameParts.join('');
        if (fullFileName.length > 0 && fullFileName.length < 200) {
  ...
  function showTable() {
    ...
    ">📁</div>
    <div>
    <div style="font-size: 18px; font-weight: 500;">提取码管理</div>
    <div style="font-size: 14px; opacity: 0.9; margin-top: 2px;">共 ${data.length} 条记录</div>
    </div>
    </div>
    <div style="display: flex; align-items: center; gap: 8px;">
    <a href="https://github.com/Chumor/123PanCV" target="_blank" style="
    display: flex;
    align-items: center;
    justify-content: center;
    width: 36px;
    ...
    ${data.length ? data.map((d, i) => `
      <tr style="transition: background 0.2s; ${i % 2 === 0 ? 'background: #fafbfc;': ''}">
      <td style="
      padding: 16px 20px;
      ...
      " title="${d.name}">${d.name.length > 10 ? d.name.substring(0, 10) + '...': d.name}</a>
      </td>
      <td class="copy-code" style="
      padding: 16px 20px;
      border-bottom: 1px solid #f1f3f4;
      cursor: pointer;
      color: #1a73e8;
      ...
      transition: all 0.2s;
      text-align: center;
      " title="点击复制提取码">${d.code}</td>
      <td style="
      padding: 16px 20px;
      border-bottom: 1px solid #f1f3f4;
      text-align: center;
      ">
      <button class="delete-btn" data-url="${d.url}" style="
      padding: 6px 12px;
    ...
    <div style="font-size: 16px; margin-bottom: 8px;">暂无记录</div>
    <div style="font-size: 14px; color: #90939a;">访问123云盘分享链接时自动保存</div>
    </td>
    </tr>
    `}
    </tbody>
    ...
    // 添加删除记录函数到全局作用域
    window.removeRecord = function(url) {
      let data = loadData();
      data = data.filter(d => d.url !== url);
      saveData(data);
      console.log('[PanCV] 删除记录:', url);
    };
    ...
    overlay.addEventListener('click', (e) => {
      ...
      // 关闭按钮
      if (target.id === 'panCV-close' || target === overlay) {
        overlay.remove();
        return;
      }
      ...
      if (target.classList.contains('copy-code')) {
        GM_setClipboard(target.textContent.trim());
        const originalColor = target.style.color;
        const originalBg = target.style.background;
        target.style.color = '#0f9d58';
        target.style.background = '#e6f4ea';
        target.textContent = '✅ 已复制';
        setTimeout(() => {
          target.style.color = originalColor;
          target.style.background = originalBg;
          target.textContent = target.getAttribute('data-code');
        }, 1000);
        return;
      }
      ...
      // 功能按钮
      if (target.id === 'panCV-export') exportData();
      if (target.id === 'panCV-import') importData();
      if (target.id === 'panCV-clear') clearData();
    });
    ...
    overlay.addEventListener('mouseover',
      (e) => {
        ...
        if (target.tagName === 'BUTTON') {
          if (target.id === 'panCV-close') {
            target.style.background = 'rgba(255,255,255,0.3)';
          } else if (target.id === 'panCV-clear') {
        ...
        // GitHub图标悬停效果
        if (target.closest('a[href="https://github.com/Chumor/123PanCV"]')) {
          target.style.background = 'rgba(255,255,255,0.3)';
        }
      });
    ...
    // 保存提取码原始值
    overlay.querySelectorAll('.copy-code').forEach(el => {
  ...
  function exportData() {
    ...
    a.download = `123PanCV-export-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
  }
        ...
        try {
          ...
          if (Array.isArray(imported)) {
            ...
            document.querySelectorAll('div').forEach(el => {
              if (el.style.zIndex === '999999') el.remove();
            });
            setTimeout(showTable,
              300);
          }
        } catch (err) {
  ...
  function clearData() {
    if (confirm('⚠️ 确定要清空所有记录吗？此操作不可撤销。')) {
      saveData([]);
      showNotification('🗑️ 所有记录已清空');
      document.querySelectorAll('div').forEach(el => {
        if (el.style.zIndex === '999999') el.remove();
      });
      setTimeout(showTable,
  ...
  GM_registerMenuCommand('📑 查看提取码记录',
    showTable);
