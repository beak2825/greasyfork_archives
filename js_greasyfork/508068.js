var script_version = "1.1.3"; //内置版本!!!!!!!
// ==UserScript==
// @name         🐔【超星学习通挂科助手】
// @namespace    FuckSuperStarLearning
// @author       倪爸爸
// @version      1.1.3
// @description  [ 1.1.3 ] 修复了部分场景可能提交失败的BUG，详情见(https://github.com/NiButCrazy/FuckSuperStarLearing/blob/main/CHANGELOG.md)
...
(async function (vue, pinia$1, ElementPlus, md5, $$1) {
                ...
                for (let i = 0; i < jobList.length; i++) {
                    ...
                    if ((_a = item.parentElement) == null ? void 0 : _a.classList.contains("ans-job-finished")) {
                        ...
                        if (cxModel.askStore.log(`正在完成任务:${otherInfo.name || otherInfo.title}`), iframe == null ? void 0 : iframe.src.match(/\/ananas\/modules\/video\/index\.html/)) {
                            ...
                            const refresh_time_interval = 1000 * 60 *5 

                            if (formStore.forminput.autoJump){
                                custom_notification("检测到直播任务，正在尝试自动跳转", "由于脚本处于测试阶段，可能会失败")

                            }else {
                                custom_notification("检测到直播任务", "由于未开启自动切换，请手动观看")
                            }
                            is_live = true
                            ...
                            const time_iframe = $('#iframe')[0]
                            time_iframe.onload = function () {
                                const time_iframe = $('#iframe')
                                let iframeDocument = time_iframe.contents().find("iframe")[0].contentDocument;
                                let progress_element = iframeDocument.querySelector(".LiveInfo2")
                                let text = progress_element.innerHTML
                                const regex = /当前观看进度为(\d+\.\d+)%/;
                                const match = text.match(regex);
                                if (formStore.forminput.autoJump){
                                    const is_finish = time_iframe.contents().find(".ans-job-finished")
                                    if (is_finish.length > 0){
                                        custom_notification("直播任务完成", "请关闭直播界面")
                                        top.document.querySelector(".nextChapter").click()
                                    }
                                    if (!formStore.forminput.autoJump){
                                        custom_notification("直播任务完成", "未开启自动切换，请手动切换下一个任务")
                                    }
                                }
                                if (match && match[1]) {
                                    const progress = parseFloat(match[1]);
                                    custom_notification("直播观看进度: " + progress + '%')
                                } else {
                                    console.log('未找到观看进度');
                                }
                            ...
                            setInterval(function (){
                                const time_iframe = $('#iframe')[0]
                                time_iframe.contentWindow.location.reload()
    ...
    let img_list=[
        "https://i1.hdslb.com/bfs/archive/16b7a506a6fd6903a447feae1fe674577f94aa63.jpg",
        "https://img2.baidu.com/it/u=2409820303,500267521&fm=253&fmt=auto&app=138&f=JPEG?w=852&h=800",
        "https://img2.baidu.com/it/u=17365825,359275951&fm=253&fmt=auto&app=138&f=JPEG?w=569&h=427",
        "https://img1.baidu.com/it/u=686210520,1150034579&fm=253&fmt=auto&app=138&f=JPEG?w=260&h=246",
        "https://img1.baidu.com/it/u=3424540889,2720920857&fm=253&fmt=auto&app=138&f=JPEG?w=463&h=290",
    ...
    function nbc_function(){
        Vue.nextTick(() => {
            ...
            if (switch_exam.find('img').length === 0) {
                var new_img=$('<img id="ikun" src="https://i1.hdslb.com/bfs/archive/16b7a506a6fd6903a447feae1fe674577f94aa63.jpg" width="100%" title="小黑子别戳我" style="border-radius:12px">')
                ...
                function switch_img(){
                    ...
                    for (let index = 0; index < img_list.length; index++) {
                        ...
                        if (element === ikun[0].src) {

                            if (index === img_list.length-1) {
                                index=-1
                            }
                            ikun[0].src=img_list[ index + 1 ]
                            break
                        }
                    }
                }
