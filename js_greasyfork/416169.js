...
// @downloadURL https://update.greasyfork.org/scripts/416169/SwaggerUI%20Extends.user.js
// @updateURL https://update.greasyfork.org/scripts/416169/SwaggerUI%20Extends.meta.js
// ==/UserScript==
    ...
    module.exports = function(path) {
        return function(e) {
            ...
            content += params.map((param => {
                ...
                return "data" === param.name && "body" === param.in && item.description.includes("<table") ? $("<div>" + item.description + "</div>").find("tbody tr").map((function() {
                    $tr = $(this);
                    let model = $tr.find("td:eq(1)").text().trim(), name = $tr.find("td:eq(0)").text().trim(), label = "" + name, placeholder = "" + name, rule = "是" === $tr.find("td:eq(2)").text().trim() ? [ {
                        required: !0,
                        message: label + "是必填的"
                    } ] : [];
                    return JSON.stringify({
