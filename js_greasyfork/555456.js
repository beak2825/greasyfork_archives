// ==UserScript==
// @name         Auto selling resources
// @namespace    http://tampermonkey.net/
// @version      0.1.1
// @description  Напівавтоматично крутимо реси
// @author       Riba
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/555456/Auto%20selling%20resources.user.js
// @updateURL https://update.greasyfork.org/scripts/555456/Auto%20selling%20resources.meta.js
// ==/UserScript==
...
(function() {
    ...
    window.addEventListener('load', () => {
        if (location.pathname.includes("map.php")) {
            ...
            const parent = document.querySelector('#map_right_block_inside');
            parent.style.removeProperty('height');
            const parent2 = document.querySelector('#map_right_block');
            parent2.style.removeProperty('height');

            const panel_container = create_el('div', 'z-index: 10000;display: flex;; justify-content: space-around; width: 250px; height: 40px;', '');
            const start_selling_btn = create_el('div', styled_btn, 'Start selling 🚀');
            parent.appendChild(panel_container);
            parent.appendChild(start_selling_btn);
            ...
            let my_coordinate = window.capt;

            const found = map_data.find(obj => obj.name.toLowerCase() === my_coordinate.toLowerCase());
            const factories = found.factories;

            start_selling_btn.addEventListener('click', () => {
                factories.forEach(el => {
                    if (el.resources.includes(resource)) {
                        window.open(`${link}/object-info.php?id=${el.id}`, '_blank').focus();
                    }
                })
            })
        }
