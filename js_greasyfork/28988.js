// ==UserScript==
// @name           TehConnection :: Send torrent to (u/ru/w)Torrent WebUI
// @version        1.0
// @namespace      DesolatedSpace
// @description    Upload directly to rutorrent/wtorrent/utorrent.
// @include        http*://*tehconnection.eu/*
// @grant GM_registerMenuCommand
// @grant GM_setValue
// @grant GM_getValue
// @downloadURL https://update.greasyfork.org/scripts/28988/TehConnection%20%3A%3A%20Send%20torrent%20to%20%28uruw%29Torrent%20WebUI.user.js
// @updateURL https://update.greasyfork.org/scripts/28988/TehConnection%20%3A%3A%20Send%20torrent%20to%20%28uruw%29Torrent%20WebUI.meta.js
// ==/UserScript==
// Modified What.CD :: Send torrent to uTorrent WebUI for TehConnection
...
if (webUIType == '4')
{
  ...
  if (wSSL == '1') {
    wtorrent_connect_string = 'https://' + wHost + ':' + wPort + '/';
  } else {
    wtorrent_connect_string = 'http://' + wHost + ':' + wPort + '/';
  }
  post(wtorrent_connect_string, 'userf=' + wUser + '&passwdf=' + wPass + '&user_login=Login', function (info) {
    var isLoggedIn = true
  });
}
//Rest of script
...
function makeUTorrentLink(link) {
  ...
  if (webUIType == '1' || webUIType == '2' || webUIType == '3')
  {
    uTorrentLink.addEventListener('click', function (e) {
...
function wTorrentClick(sent)
{
  ...
  post(wtorrent_connect_string + 'index.php?cls=AddT', sent, function (addedResponce) {
    var match_added = /Torrent added correctly/gim;
    var already_exists = /Error: File already exists in torrent directory, can't create .torrent/gim;
...
function match(url) {
  for (var i = 0; i < siteArray.length; i++)
  {
    ...
    {
      if (siteArray[i][1] == '1' && webUIType != '2' && webUIType != '3') {
        stripssl = '1';
      }
      if (siteArray[i][2] == '1') {
        useCookie = '1';
      }
      if (url.match('ssl.what.cd')) {
        isWhat = '1';
      }
      if (siteArray[i][6] != '') {
        innerSep = siteArray[i][6];
      }
      if (siteArray[i][5] != '') {
        linkText = siteArray[i][5];
      }
      return 1;
    }
  }
