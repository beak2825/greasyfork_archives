...
// @grant        GM_setValue
// @grant        GM_getValue
// @downloadURL https://update.greasyfork.org/scripts/545409/Details%21%20debug.user.js
// @updateURL https://update.greasyfork.org/scripts/545409/Details%21%20debug.meta.js
// ==/UserScript==
...
function initializeEmptyCombatData() {
    return {
        damage: {}, healing: {}, taken: {}, mitigated: {}, crits: {}, missed: {}, intercepted: {},
        detailedDamage: {}, detailedHealing: {}, detailedTaken: {}, detailedMitigated: {},
        detailedCrits: {},
...
function ensureCompleteDataStructure(dataObject) {
    ...
    // Lazy validation - only run if not already validated
    if (dataObject._structureValidated) return;

    const rootKeys = ['damage', 'healing', 'taken', 'mitigated', 'crits', 'missed', 'intercepted', 'projectedGains', 'initiativeStats'];
    const detailedKeys = ['detailedDamage', 'detailedHealing', 'detailedTaken', 'detailedMitigated', 'detailedCrits', 'detailedMissed', 'detailedIntercepted'];
    ...
    // Mark as validated to avoid repeated processing
...
function ensureProjectedGainsStructure(dataObject) {
    ...
    const dataSectionsForEntityDiscovery = ['damage', 'healing', 'taken', 'mitigated', 'crits', 'missed', 'intercepted'];
...
function combatDataHasMeaningfulEvents(data) {
    ...
    const meaningfulEventKeys = ['damage', 'healing', 'taken', 'mitigated', 'crits', 'missed', 'intercepted'];
...
function attachUserInterfaceEventListeners() {
     ...
     console.log('[Details!] Adding global keydown event listener');
     document.addEventListener('keydown', handleGlobalKeyDown);
     console.log('[Details!] Global keydown event listener added successfully');
}
...
function handleGlobalKeyDown(event) {
    // Add comprehensive logging for all key events
    console.log('[Details!] Key event detected:', {
        key: event.key,
        code: event.code,
        altKey: event.altKey,
    ...
    if (event.altKey && event.key.toLowerCase() === 'n') {
        console.log('[Details!] Alt+N combination detected!');
        console.log('[Details!] Current UI state before toggle:', {
            isUserInterfaceVisible: isUserInterfaceVisible,
            uiContainerElement: !!uiContainerElement,
            uiContainerDisplay: uiContainerElement ? uiContainerElement.style.display : 'N/A'
        });
        
        event.preventDefault();
        console.log('[Details!] Event.preventDefault() called');
        
        if (!isUserInterfaceVisible) {
            console.log('[Details!] UI is not visible, calling showUserInterface(true)...');
            showUserInterface(true);
            console.log('[Details!] showUserInterface(true) completed');
        } else {
            console.log('[Details!] UI is visible, calling hideUserInterface()...');
            hideUserInterface();
            console.log('[Details!] hideUserInterface() completed');
        }
        
        console.log('[Details!] Final UI state after toggle:', {
            isUserInterfaceVisible: isUserInterfaceVisible,
            uiContainerElement: !!uiContainerElement,
            uiContainerDisplay: uiContainerElement ? uiContainerElement.style.display : 'N/A'
        });
...
function getDisplayDataAndInfo() {
    ...
    const simpleModes = ['damage', 'healing', 'taken', 'mitigated', 'crits', 'missed', 'intercepted'];
    const detailedModesMap = {
        damage: 'detailedDamage', healing: 'detailedHealing', taken: 'detailedTaken',
        mitigated: 'detailedMitigated', crits: 'detailedCrits', missed: 'detailedMissed', intercepted: 'detailedIntercepted'
    };
...
function hideUserInterface() {
    console.log('[Details!] hideUserInterface() called, current state:', {
        isUserInterfaceVisible: isUserInterfaceVisible,
        uiContainerElement: !!uiContainerElement
    });
    
    if (!isUserInterfaceVisible) {
        console.log('[Details!] UI already hidden, returning early');
        return;
    }
    
    isUserInterfaceVisible = false;
    console.log('[Details!] Set isUserInterfaceVisible to false');
    
    if (uiContainerElement) {
        console.log('[Details!] Setting uiContainerElement display to none');
...
function showUserInterface(resetPosition = false) {
    console.log('[Details!] showUserInterface() called with resetPosition:', resetPosition);
    console.log('[Details!] Current state:', {
        isUserInterfaceVisible: isUserInterfaceVisible,
        uiContainerElement: !!uiContainerElement,
        savedUserInterfacePosition: savedUserInterfacePosition
    });
    
    if (resetPosition) {
        console.log('[Details!] Resetting UI position to defaults');
        ...
        if (uiContainerElement) {
            console.log('[Details!] Applying reset position to existing container');
            ...
            if (isUserInterfaceMinimized) {
                console.log('[Details!] Un-minimizing interface');
    ...
    if (isUserInterfaceVisible && uiContainerElement && !resetPosition) {
        console.log('[Details!] UI already visible and container exists, just showing');
        uiContainerElement.style.display = 'flex';
        return;
    }

    console.log('[Details!] Setting isUserInterfaceVisible to true');
    isUserInterfaceVisible = true;
    
    if (!uiContainerElement) {
        console.log('[Details!] No UI container exists, creating interface');
        createUserInterface();
    } else {
        console.log('[Details!] UI container exists, showing and updating');
        uiContainerElement.style.display = 'flex';
        updateUserInterface();
    }
    
    saveApplicationState();
    console.log('[Details!] showUserInterface() completed, final display:', 
        uiContainerElement ? uiContainerElement.style.display : 'N/A');
}
...
function aggregateCombatData(data1, data2) {
    ...
    // Mark the merged data as needing validation since we'll be modifying it
...
function parseCombatLogLine(logHtml) {
    ...
    // Mark structure as potentially invalid since we're about to modify data
    ...
    if (interceptionMatch) {
        ...
        // Handle weapon name construction for dual wielding
...
function formatTooltipContent(primaryName, breakdownData, totalValue, mode, maxCritValue = undefined) {
    ...
    breakdownEntries.forEach(entry => {
        ...
        if ((mode === 'damage' || mode === 'taken') && entry.weapons && Object.keys(entry.weapons).length > 0) {
            html += `<div class="tooltip-weapon-breakdown">`;
