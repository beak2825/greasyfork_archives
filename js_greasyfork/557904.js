...
// @version      2026-01-15
// @description  アイヌ語会話辞典の 表記を 変換します。
// @author       トラネコマン
// @match        https://ainu.ninjal.ac.jp/topic/dictionary/*
// @icon         data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/557904/%E3%82%A2%E3%82%A4%E3%83%8C%E8%AA%9E%E4%BC%9A%E8%A9%B1%E8%BE%9E%E5%85%B8%E3%81%AE%E8%A1%A8%E8%A8%98%E5%A4%89%E6%8F%9B.user.js
// @updateURL https://update.greasyfork.org/scripts/557904/%E3%82%A2%E3%82%A4%E3%83%8C%E8%AA%9E%E4%BC%9A%E8%A9%B1%E8%BE%9E%E5%85%B8%E3%81%AE%E8%A1%A8%E8%A8%98%E5%A4%89%E6%8F%9B.meta.js
// ==/UserScript==
...
function correctSpell() {
    let ainu = document.getElementsByClassName("ainu");
    if (ainu.length == 0) return;
    for (let i = 0; i < ainu.length; i++) {
        if (ainu[i].textContent in correct) {
            ainu[i].textContent = correct[ainu[i].textContent];
        }
    }
}
...
function changeNotation(notation) {
    ...
    if (notation == "roman") {
        ...
        let add_display_css = "";
        if (c[notation].m != "") add_display_css = `.ainu{font-family:"${c[notation].m}";}`;
...
function mainProc() {
    //表記オプションの作成
    let toolbar = document.getElementById("header-panel");
    let html = `
<form name="notationForm">`;
    for (let ch in c) {
        html += `
	<label><input type="radio" name="notation" value="${ch}">${c[ch].n[page_language]}</label>
	<a class="js-modal-link" href="#" data-modal-content="#modal-word-${ch}">?</a>
	<div id="modal-word-${ch}" style="display: none;">${c[ch].t[page_language]}</div>`;
    }
    html += `
</form>`;
    ...
    //監視
    let o2 = new MutationObserver((m) => {
        correctSpell();
        changeNotation(document.notationForm.notation.value);
    });
    o2.observe(document.getElementById("gridview-1025"), { childList: true });
    o2.observe(document.getElementById("gridview-1028"), { childList: true });
}
...
`;
    ...
    modal_content = document.querySelector(".js-modal-content");
