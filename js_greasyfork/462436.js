...
// @exclude /^(https?:\/\/)(.+)?(ctr.sh|sinonimos.de|quesignifi.ca|clks.pro|exeo.app|exego.app|exe.io|oii.io|fc-lc.com|fc-lc.xyz|fc.lc|cuty.io|cuty.app|cuty|cutsy|cutado.com|clicksfly.com|yogablogfit.com|financerites.com|vocalley.com|howifx.com|healthfirstweb.com|tmearn.net|mdn.lol|clks.pro|themezon.net|linksly.co|ez4short.com|tech5s.co|ez4mods.com|newassets.hcaptcha.com)(\/.*)/
// @exclude /^(https?:\/\/)(.+)?(mediafire.com|linkjust.com|3rabsports.com|fx-22.com|gold-24.net|forexrw7.com|try2link.com|mobi2c.com|forexit.online|bluetechno.net|hightrip.net|healthy4pepole.com|fx-gd.net)(\/.*)/
// @downloadURL https://update.greasyfork.org/scripts/462436/Bypass%20All%20Shortlinks.user.js
// @updateURL https://update.greasyfork.org/scripts/462436/Bypass%20All%20Shortlinks.meta.js
// ==/UserScript==
// ================================================================================================================================================================
...
(function() { // Please Remember , My Scripts is better with Firefox Browser and Violentmonkey Extension
  'use strict';
  const valdelay = 15; // Change the number from 5 to 10 or 20 if you have issues like Your action marked Suspicious,Don't try to bypass ,Don't use Speedster, etc
  ...
  BlockRead('(/adblock|/ad-now.php|/bypass)', 'setInterval');
  ...
  BloggerPemula('financeandinsurance.xyz', 'link', 'https://blog.theconomy.me/');
  ...
  BypassedByBloggerPemula(/(awgrow|fadedfeet|kenzo-flowertag|homeculina|ineedskin).com|mdn.lol|worldtanr.xyz|lawyex.co/, () => {EnableRCF();window.addEventListener('DOMContentLoaded', () => {BypassHD('form[id]', 17);});});
  ...
  function onHtmlLoaded() {
    ...
    BypassedByBloggerPemula(/ctr.sh/, function() {if (/^\/(.+)/.test(location.pathname) && !BpParams.has('token')) {location = 'https://tiktokrealtime.com/tech/?url8j=' + location.href;}});
    BypassedByBloggerPemula(/coingraph.us|trendzilla.it|horoscop.info/, function() {EnableRCF();SubmitIfExists("form[name='dsb']", 3);ClickIfExists('#wpsafe-link8 > a', 4);});
    ...
    BypassedByBloggerPemula(/shrinke.me|shrinke.us|shrinkme.info|pwrpa.cc|shrinkme.org|paylinks.cloud/, function() {let shk = setInterval(() => {
    ...
    BypassedByBloggerPemula(/blog24.me/, function() {EnableRCF();
      if (location.host.includes('news') || location.host.includes('game') || elementExists('div > div > b > p') || elementExists('.h-captcha')) {window.addEventListener('DOMContentLoaded', () => {BypassHD('form[id]', 19);});} else {window.onscroll = BpBlock();window.check2();
    ...
    BypassedByBloggerPemula(/(remixsounds|helpowi|helpdeep).com|uprwssp.org|remixone.in|webseriesreel.in/, function() {
    ...
    BypassedByBloggerPemula(/theconomy.me|imagenesderopaparaperros.com|happy-living.online/, function() {
    ...
    BypassedByBloggerPemula(/wellness4live.com/, () => {waitForElm('#wpsafe-link > a > .btn-primary.btn', wlBtn => wlBtn.click());});
    ...
    BypassedByBloggerPemula(/(cdrab|tinybc|tinycmd|financenube).com|datacheap.io/, () => {waitForElm('#wpsafe-link a', cdr => redirect(strBetween(cdr.onclick.toString(), `window.open('`, `', '_self')`), false));});
