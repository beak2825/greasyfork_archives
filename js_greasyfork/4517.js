...
// @version 1.0.4
// @downloadURL https://update.greasyfork.org/scripts/4517/Urentool%20UI.user.js
// @updateURL https://update.greasyfork.org/scripts/4517/Urentool%20UI.meta.js
// ==/UserScript==    
...
(function($) {
	...
	*/
	var setInputFocus = function() {
		$form.find('tr').each(function(i, elm) {
			var $tr = $(this),
				$input = $tr.find('input[type="text"].project');
			$input.removeAttr('onfocus');//remove current onfocus behaviour - it added empty lines which gave errors upon submitting
	...
	/**
	* improved version of selecteerDagsoort
	* only asks for confirmation if hours have been booked
	* @returns {undefined}
	*/
	var improvedSelecteerDagsoort = function() {

		var numRows = $('a[onclick^="editRegel"').length,
			hoursAlreadyBooked = (numRows > 0);

		var dagType = document.frm_urenregels.dagsoort.value;
		
		// dagtypen zonder uurregels.
		if(hoursAlreadyBooked && (dagType === 'volledigvakantie' || dagType === 'volledigziek' || dagType === 'vastevrijedag' || dagType === 'feestdag')) {
		   // waarschuwen voor deleten regels.
		   if(confirm("Are you sure you want to change the day type?\nAll of your booked hours on this day will be deleted!")) {
		      document.frm_urenregels.submit();  
		   }
		} else {
		   // terug naar gewone dag, dus posten anders wordt er geen nieuwe uurregel gemaakt.
		   document.frm_urenregels.submit();
		}
	};
	...
	/**
	* initialize improved dagsoort function:
	* remove old function and attach new one
	* @returns {undefined}
	*/
	var initImprovedSelecteerDagsoort = function() {
		$('select[name="dagsoort"]').removeAttr('onchange')
			.on('change', improvedSelecteerDagsoort);
	};
