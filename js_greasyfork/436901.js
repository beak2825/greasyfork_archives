...
// @downloadURL https://update.greasyfork.org/scripts/436901/%E9%80%9A%E7%94%A8%E9%98%85%E8%AF%BB%E5%99%A8Waltz_adapt.user.js
// @updateURL https://update.greasyfork.org/scripts/436901/%E9%80%9A%E7%94%A8%E9%98%85%E8%AF%BB%E5%99%A8Waltz_adapt.meta.js
// ==/UserScript==
...
(function() {
    ...
    var word_length=50;  //匹配的最小字符长度
    var blackList = {"www.baidu.com": true, "www.sogou.com": true, "www.so.com": true, "www.bing.com":true, "cn.bing.com": true, "www.google.com": true, "m.sm.cn": true}; //黑名单
    ...
    //尝试修改通用解析模板
    function newparseContentAndTitle(){
        var mainDom = null;
        var txt=null;
        function findMainDom(doms){
            var docSize = document.body.scrollHeight * document.body.scrollWidth;
            for(var i=0;i<doms.length;i++){
                ...
                // 计算dom尺寸百分比并输出
                var size = (dom.scrollWidth * dom.scrollHeight) / docSize;
                if (dom.getAttribute("class")=="t_f" ){ // pre和code是代码常用元素块，忽略不认为是正文内容
                    mainDom=dom;
                    if (dom.innerText.length>word_length)
                    {
                        txt=txt+dom.innerText
                    }
                }
                findMainDom(dom.children||[]);
            }
        ...
        if (mainDom){
            ...
            // findTitleDom(mainDom.parentNode.children, titleList);
            // 查找标题逻辑bug较多,暂时改为查找网页title作为标题,后续重写逻辑进行优化
            return {content: txt, title: document.title};
        }
    }
    ...
    function checkAndCreateReader(notAlert){
        ...
        var content=null
        if (location.host=="giantesswaltz.org" || location.host=="www.giantesswaltz.org")
        {
            content = newparseContentAndTitle();
        }
        else
        {
            content = parseContentAndTitle();
        }
