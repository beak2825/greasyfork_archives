...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/436598/NameTyper.user.js
// @updateURL https://update.greasyfork.org/scripts/436598/NameTyper.meta.js
// ==/UserScript==
...
(async function() {
    ...
    async function checkNameDB(name){
        let response = await fetch(`https://tishka.xyz/sdt/nametyper.php?action=getCleanName&typedName=${name}`);
        ...
        if (evt.altKey && evt.keyCode == 81) {
            ...
            if(checkDB != 0){
                if(!document.querySelectorAll('[placeholder="Type a message..."]')[0]){
                    document.querySelectorAll('[placeholder="Private messages will be visible to agents only…"]')[0].value += checkDB.cleanName+", ";
                }
                else{
                    document.querySelectorAll('[placeholder="Type a message..."]')[0].value += checkDB.cleanName+", ";
                }
                ...
                if(strippedString[1]){
                    ...
                    if(isKyr(strippedString[0].trim()))
                    {
                        ...
                        if(!document.querySelectorAll('[placeholder="Type a message..."]')[0]){
                            document.querySelectorAll('[placeholder="Private messages will be visible to agents only…"]')[0].value += nameToType+", ";
                        }
                        else{
                            document.querySelectorAll('[placeholder="Type a message..."]')[0].value += nameToType+", ";
                        }
                    }
                    else
                    {
