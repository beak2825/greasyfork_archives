// ==UserScript==
// @name         DeepCo Statistics (Original Layout) + DC/hr Toast + Range Buttons (Stable)
// @namespace    https://www.deepco.app/
// @version      2025-12-21r7q
// @description  Full stable: original layout + legend toggles + range buttons + AUTO + SPA-safe. DC/hr from toast with robust dedupe + instant updates. DC/hr warmup (10s). Live values HUD top-center (only visible series). Title/subtitle aligned left.
...
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/559647/DeepCo%20Statistics%20%28Original%20Layout%29%20%2B%20DChr%20Toast%20%2B%20Range%20Buttons%20%28Stable%29.user.js
// @updateURL https://update.greasyfork.org/scripts/559647/DeepCo%20Statistics%20%28Original%20Layout%29%20%2B%20DChr%20Toast%20%2B%20Range%20Buttons%20%28Stable%29.meta.js
// ==/UserScript==
...
(async function () {
  ...
  function clampSeries(series) {
    try {
      while (series.data.length > MAX_CHART_POINTS) series.removePoint(0, false);
    } catch (_) {}
  }
...
function ensureHud() {
    ...
    __toastObserver = new MutationObserver((mutations) => {
      ...
      for (const m of mutations) {
        for (const node of m.addedNodes || []) {
          ...
          if (!text || !text.includes('[DC]')) continue;
  ...
  function pushInstantBlockPoint(ts) {
    try {
      ...
      myChart.redraw();
      if (rangeMode !== 'all') applyRangeExtremes(t);
      updateHud();
    } catch (_) {}
  }
...
#deepco-action-buttons .deepco-action-btn:active{
  ...
  async function logSnapshotAndUpdate() {
    ...
    myChart.redraw();
    if (rangeMode !== 'all') applyRangeExtremes(ts);
      ...
      const walker = document.createTreeWalker(
        ...
        NodeFilter.SHOW_TEXT,
        {
          acceptNode(node) {
            ...
            return /CS/i.test(t) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT;
          }
        }
      );
