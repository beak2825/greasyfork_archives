...
// @author       MateusFagnds, XFaires, Diego
...
// @downloadURL https://update.greasyfork.org/scripts/487736/GGUtils.user.js
// @updateURL https://update.greasyfork.org/scripts/487736/GGUtils.meta.js
// ==/UserScript==

// DEFINA AQUI A MENSAGEM QUE SERÁ ENVIADA NO BATE-PAPO
var mensagemchat = GM_getValue('mensagemchat', "Olá, você sabia que se você avaliar essa compra positivamente você recebe GGPOINTS e com isso você pode trocar por outros produtos aqui na plataforma? Vai perder essa oportunidade? Deixe logo sua avaliação positiva aqui nessa compra.");
// DEFINA AQUI A MENSAGEM QUE SERÁ COLOCADA NA AVALIAÇÃO
var mensagemavaliacao = GM_getValue('mensagemavaliacao', "O cliente é excepcional, sempre muito cortês e generoso. Agradeço por me escolher e estou certo de que você ficará satisfeito com o produto adquirido. Muito obrigado pela sua confiança em nós.");
...
`;
    ...
    if (element) {
        ...
        element.textContent = 'V' + GM_info.script.version;
    }
...
(function () {
    // à ser executado na mudança de página
    const alterarHTML = function () {
        // ---- Botão para ver token ---- //
        try {
            document.querySelector('.resume-profile-module').innerHTML += `<a class="btn btn-primary" style="margin-left: 5px; background-color: rgb(17, 17, 17);border-color: rgb(17, 17, 17);" onclick="prompt('Seu Token:',document.cookie.split('; ').find(cookie => cookie.startsWith('auth._token.local=')).split('=')[1].split('Bearer%20')[1])">🔑</a>`;
        } catch (e) { }


        // Botão para pedir mensagem de avaliação //
        if (window.location.href.includes('conta/vendas')) {

            document.querySelector(".account-settings-menu").outerHTML += `<div class="d-none d-xl-block d-lg-block account-settings-menu ggutils_rmenu" style="padding-top: 0px; display: flex; flex-direction: row; margin-bottom: 32px;" data-v-3dd8454c="">
            <style>
            ...
            <h3 style="font-size:12px; text-align: center;">ggUtils - Avaliações</h3>
            <div style="width: 100%" class="v-popper filter--single-select v-popper--theme-dropdown"><label> Mensagem De Avaliação</label><textarea class="filter__button" style="width: 100%" placeholder="Mensagem de Avaliação" id="reviewInput">${mensagemavaliacao}</textarea></div><br>
            <div style="width: 100%" class="v-popper filter--single-select v-popper--theme-dropdown"><label> Mensagem De Avaliação no Chat </label><textarea class="filter__button" style="width: 100%" placeholder="Mensagem de Avaliação" id="chatreviewInput">${mensagemchat}</textarea></div>
            </div>`

            document.querySelector("#reviewInput").addEventListener('change', function () {
                GM_setValue('mensagemavaliacao', document.querySelector("#reviewInput").value);
            });

            document.querySelector("#chatreviewInput").addEventListener('change', function () {
                GM_setValue('mensagemchat', document.querySelector("#chatreviewInput").value);
            });
    ...
    // Executa a função na mudança de página
    alterarHTML();

    let urlAnterior = window.location.href;
    setInterval(function () {
        if (urlAnterior !== window.location.href) {
            urlAnterior = window.location.href;
            alterarHTML();
        }
    }, 100);

    // onhashchange é executado quando a URL muda
