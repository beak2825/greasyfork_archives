...
// @namespace    ineluctable-vote-counting
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/28570/PTP%20mafia%20gather%20votes.user.js
// @updateURL https://update.greasyfork.org/scripts/28570/PTP%20mafia%20gather%20votes.meta.js
// ==/UserScript==
...
function doneCollecting(message, votes, m, users)
{
    ...
    if (!uT)
    {
        ...
        uT.setAttribute('style', 'width: 580px; height: 100px; position: initial !important;');
    ...
    uT.innerHTML += '[/hide]';
    //resize('userPostCount'); -- I removed this to make the height of the post count smaller, as it's not very useful to me

    /*var a1 = document.createElement('a');
    d.appendChild(a1);
    a1.innerHTML = 'Removed striked votes';
    a1.href = 'javascript:void(0);';
    a1.addEventListener('click', showDialog.bind(undefined, a1), false);*/
    ...
    t.innerHTML = '[quote][size=4]';
    for (var i = 0; i < done.length; i++)
    {
        var d1 = done[i];
        t.innerHTML += '[b][u]' + d1.votedFor + ': ' + d1.count + '/' + m + '[/u][/b]' + '\n';
        for (var j = 0; j < tally.length; j++)
        {
            var t1 = tally[j];
        ...
        var next_item = i+1;
        if (next_item < done.length)
        {
            t.innerHTML += '\n';
        }
    }
    t.innerHTML += '[/size][/quote]';
    //t.innerHTML += '[/align]';
    resize('collecting');
}
