...
// @downloadURL https://update.greasyfork.org/scripts/426889/%E6%96%B0-%E8%AF%95%E5%AE%A2%E5%B7%B4%E9%A2%86%E5%8F%96%E9%A1%B5%E8%8E%B7%E5%8F%96%E5%BA%97%E5%90%8D.user.js
// @updateURL https://update.greasyfork.org/scripts/426889/%E6%96%B0-%E8%AF%95%E5%AE%A2%E5%B7%B4%E9%A2%86%E5%8F%96%E9%A1%B5%E8%8E%B7%E5%8F%96%E5%BA%97%E5%90%8D.meta.js
// ==/UserScript==
...
(function() {
    ...
    $.post('https://wx.shike8888.com/apply/searchInfo?aid='+aid,function(ret){
        ...
        $.post('https://wx.shike8888.com/activity/getBaseDetail?act_id='+aid,function(res){
            console.log('下单链接：'+res.activityDto['product_link']);
    ...
    function copy(keyword){
        var clipboard = new ClipboardJS('#'+keyword, {
            text: function() {
                return $('#'+keyword).html();
            }
        });
    ...
    //----------------------试用礼包修改程序-----------------------
     ...
     var order_ids=getUrlParam('id');
    //var act_ids = window.Utils.request("aid");
    //var isContent = window.Utils.request("isContent");
    //var platform = navigator.platform;
    //var imgpath = "http://image.shike8888.com/";
    //var act_type = window.Utils.request("act_type");
    //var type = window.Utils.request("type");
    var duobaobi,gifts,shouxufei;
    $('#zhekou').append('<p style="width:120px;height:30px;margin:0 auto;color:white;background-color:red; z-index:999;text-align:center;line-height:30px;"id="gifts" >提前选择试用礼包</p>');
    $("#gifts").off().on('tap',function(){
        gifts =prompt("选择礼包内容：夺宝币(0) 免手续费(1) 10元试用金(2) ；(默认0)");
        if(gifts==''||gifts==null){
            duobaobi =prompt("输入夺宝币数量：");
            if(duobaobi==''||duobaobi==null){
                mui.toast('未输入数值!',{ duration:'short', type:'div' })
            }else{
                edit(order_ids,duobaobi,1);
            }
        }else if(gifts=='1'){
            shouxufei =prompt("输入免手续费金额：");
            if(shouxufei==''||shouxufei==null){
                mui.toast('未输入金额!',{ duration:'short', type:'div' })
            }else{
                edit(order_ids,shouxufei,2);
            }
        }else if(gifts=='2'){

            $.post('/DiscountTryGift/preGiveDiscountTryMoney',{orderId:order_ids},function(data){
                if(data.code == 1){

                     mui.toast('已修改礼包为10元折扣试用金',{ duration:'short', type:'div' })
                }
            },'json');
    ...
    function edit(order_ids,money,type){
        var dataObj = {
            relationId:order_ids,
            money:money,
            type:type
        }
        $.post('/DiscountTryGift/preGiveDiscountGift',dataObj,function(data){
           if(type=='1'){

             mui.toast('已修改礼包为'+money+'夺宝币！',{ duration:'short', type:'div' })
           }else{

            mui.toast('已修改礼包为'+money+'元免手续费！',{ duration:'short', type:'div' })
