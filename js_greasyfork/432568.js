...
// @name        MonkiiMod
// @version      v.10.3.3.2
// @description  This is a sad copy of Pancake Mod lol 
// @author       Me, lol
// @match        *://moomoo.io/*
// @match        *://dev.moomoo.io/*
// @match        *://*.moomoo.io/*
...
// @grant        none
// @namespace    https://greasyfork.org/en/users/752105-wait-4-it
// @downloadURL https://update.greasyfork.org/scripts/432568/MonkiiMod.user.js
// @updateURL https://update.greasyfork.org/scripts/432568/MonkiiMod.meta.js
// ==/UserScript==
...
document.getElementById('linksContainer2').innerHTML = ' Monkiiz!!! ' ;
document.getElementById('gameName').innerHTML = '💪🐒Monkiiz :D 🐒';
document.getElementById('loadingText').innerHTML = ' Monkiz Eatin'
document.getElementById('diedText').innerHTML ="Sheesh, Almost...";
document.getElementById('diedText').style.color = "#fe3200";
document.title='monkii mod';
document.getElementById("leaderboard").append ('Monkiiz');
            ...
            typeof pingTime !== "undefined" &&
                ...
                if (oldPing >= 101 && oldPing <= 250) {
                    ...
                    if(document.getElementById("pingy").checked){
                        newSend(["ch", ["Sh#t, " + oldPing + " ms!"]])
                    }
                }
                if (oldPing >= 251) {
                    pingSpan.style.color = "red";
                    if(document.getElementById("pingy").checked){
                        newSend(["ch", ["Crap, " + oldPing + " ms"]])
                    }
 
                }
                if(oldPing >= 200 && document.getElementById('autoq').checked && isEnemyNear){
                    q = true;
                } else if(oldPing <170 || isEnemyNear == false){
                    q = false;
                }
            })());
        });
...
function handleMessage(_x17) {
    ...
    if (token == 'h' && node[1] == myPlayer.id) {
        ...
        if (node[2] <= 47 && antiinsta2 == true) {
            place(foodType);
            place(foodType);
            place(foodType);
        }
        if (node[2] <= 35 && antiinsta2 == true) {
            place(foodType);
            place(foodType);
            place(foodType);
            hat(6);
...
document['addEventListener']('keydown', function(a) {
    ...
    if(a.keyCode == 82 && document.activeElement.id.toLowerCase() !== 'chatbox') {
        ...
        setTimeout( () => {
            ...
            newSend(["13c", [0, 53, 0]]);
            newSend(["5", [secondary, true]]);
        }, 35);//50 for a bit laggy pc//80 for a bit more laggier pc//100-120 for more laggier pc//you can use all speeds if you have good pc. find your speed..35 is stack
        setTimeout( () => {
            if (pikeinsta == true) {
    ...
    if (a.keyCode == 89 && document.activeElement.id.toLowerCase() !== 'chatbox') {
        ...
        autoaim = true;
        doinsta = true;
        newSend(["ch", ["reverse insta | atsni esrever"]]);
        newSend(["13c", [0, 0, 1]]);
        newSend(["5", [secondary, true]]);
        newSend(["7", [1]]);
        newSend(["13c", [1, 53, 0]]);
...
menuChange.innerHTML = `
        ...
        <label type="radio" class="container">Anti Insta 2(The other AntiInsta will be deactivated automatically)(idk if it´s good)<input type="checkbox" id="antiinsta2">
        <span class="checkmark"></span></label>
 ...
 <label type="radio" class="container">360° hit<input type="checkbox" id="360hit">
        ...
        <span class="checkmark"></span></label>
         <label class="container">AutoBreak Pit-Trap?<input type="checkbox" id="autobreaktrap">
            ...
            <p class="tree">2.  Now press the key 'delete(not on Chromebook)'.</p>
...
var ai2 = document.querySelector("#antiinsta2")
ai2.addEventListener('change', function() {
    if (this.checked) {
        antiinsta2 = true;
        antiinsta = false;
        newSend(["ch", ["0n"]]);
