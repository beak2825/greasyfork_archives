...
// @downloadURL https://update.greasyfork.org/scripts/467837/NEON%27s%20bot%2Bgamemodes.user.js
// @updateURL https://update.greasyfork.org/scripts/467837/NEON%27s%20bot%2Bgamemodes.meta.js
// ==/UserScript==
    ...
    apply( target, thisArgs, args ) {
        ...
        if (ctx){
            ...
            for (let user of users){
                ...
                ctx.strokeStyle = 'black';
                if (user.alive){
                     if (user.arrowAngle && user.id == botId && user.special && aimAssist){
                    ctx.globalAlpha = 0.4;
                    let sx = user.x;
                    let sy = user.y;
        ...
        window.addEventListener("keydown",function(e){
            if (document.activeElement !== chatInput2 && document.activeElement !== chatInput1){
               ...
               if (e.altKey && e.keyCode == 81){
                    aimAssist = !aimAssist;
                    displayOnChat("AimAssist is now "+((aimAssist)? "Activated" : "Deactivated"));
                    e.preventDefault();
                }
