// ==UserScript==
// @name   🌱【免费版】金华市专业技术人员继续教育学习|金华市公务员|如需自动下一集、自动换课程、免播放刷课等高级功能见收费版本：https://doc.zhanyc.cn/pages/jhsgwy/
// @namespace    http://jb.zhanyc.cn/
// @icon    https://js.zhanyc.cn/img/js-logo.svg
// @version      1.0
// @description  当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制功能。如需自动下一集、自动换课程、免播放刷课、全自动无人值守高级功能可升级付费版本，一杯咖啡钱，保你无忧学习，且永久使用|接各类脚本开发、代挂工作，微信：zhanyc_cn 备用微信:zhanfengkuo 个人网站：http://doc.zhanyc.cn
// @author       zfk
// @include    *://*jinhua.gov.cn*
// @include    *://*.jinhua.gov.cn/*
// @grant       GM_getValue
// @grant       GM_setValue
// @grant       GM_addStyle
...
// @antifeature 
// @license GPL
// @downloadURL https://update.greasyfork.org/scripts/533899/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E9%87%91%E5%8D%8E%E5%B8%82%E4%B8%93%E4%B8%9A%E6%8A%80%E6%9C%AF%E4%BA%BA%E5%91%98%E7%BB%A7%E7%BB%AD%E6%95%99%E8%82%B2%E5%AD%A6%E4%B9%A0%7C%E9%87%91%E5%8D%8E%E5%B8%82%E5%85%AC%E5%8A%A1%E5%91%98%7C%E5%A6%82%E9%9C%80%E8%87%AA%E5%8A%A8%E4%B8%8B%E4%B8%80%E9%9B%86%E3%80%81%E8%87%AA%E5%8A%A8%E6%8D%A2%E8%AF%BE%E7%A8%8B%E3%80%81%E5%85%8D%E6%92%AD%E6%94%BE%E5%88%B7%E8%AF%BE%E7%AD%89%E9%AB%98%E7%BA%A7%E5%8A%9F%E8%83%BD%E8%A7%81%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%EF%BC%9Ahttps%3Adoczhanyccnpagesjhsgwy.user.js
// @updateURL https://update.greasyfork.org/scripts/533899/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E9%87%91%E5%8D%8E%E5%B8%82%E4%B8%93%E4%B8%9A%E6%8A%80%E6%9C%AF%E4%BA%BA%E5%91%98%E7%BB%A7%E7%BB%AD%E6%95%99%E8%82%B2%E5%AD%A6%E4%B9%A0%7C%E9%87%91%E5%8D%8E%E5%B8%82%E5%85%AC%E5%8A%A1%E5%91%98%7C%E5%A6%82%E9%9C%80%E8%87%AA%E5%8A%A8%E4%B8%8B%E4%B8%80%E9%9B%86%E3%80%81%E8%87%AA%E5%8A%A8%E6%8D%A2%E8%AF%BE%E7%A8%8B%E3%80%81%E5%85%8D%E6%92%AD%E6%94%BE%E5%88%B7%E8%AF%BE%E7%AD%89%E9%AB%98%E7%BA%A7%E5%8A%9F%E8%83%BD%E8%A7%81%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%EF%BC%9Ahttps%3Adoczhanyccnpagesjhsgwy.meta.js
// ==/UserScript==
(function () {
    let $jq = $;
    unsafeWindow.$jq = $;
    ...
    let freeTips = "当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制功能。如需自动下一集、自动换课程、免播放刷课、全自动无人值守高级功能，可点击下方按钮查看付费版本"
    let docUrl = "https://doc.zhanyc.cn/pages/jhsgwy/";
    let plugMain = Object.assign(baseConfig, {
        ...
        async runByUrl(url) {
            if (plugMain.matchUrl("Course/Default.aspx")) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
                return;
            } else if (plugMain.matchUrl("course/Course.aspx")) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
                return;
            } else if (plugMain.matchUrl("play/right")) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
            } else if (plugMain.matchUrl("my/Default")) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
            } else if (plugMain.matchUrl("/play/play?")) {
                plugMain.page_play();
            } else if (plugMain.matchUrl("/personalCenter")) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
                return;
            } else if (plugMain.matchUrl("/courseCenter/courseDetails?")) {
                plugMain.page_courseDetails();
                return;
            }

        },
        async page_courseDetails() {
            console.log("%c page_courseDetails", "background:rgb(0,0,0);color:#fff");
            plugMain.closeWaitConfrimWin()
            let index = setTimeout(() => {
                plugMain.confirmRun("超过90秒还存在当前页面，准备刷新").then(a => {
                    location.reload()
                })
            }, 90 * 1000);
            await plugMain.waitOf(a => $(`[role="progressbar"]`) != null).then(async a => {

                await plugMain.waitOf(a => plugMain.getElByText($("a"), "点击播放") != null)
                let jd = Number($(`[role="progressbar"]`).attr('aria-valuenow'))
                if (jd >= 100) {
                    plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
                    return
                }
                plugMain.confirmRun("5秒后开始播放", 5000).then(a => {
                    plugMain.getElByText($("a"), "点击播放")[0].click()
                    plugMain.openLjTips(config.studyMode == '1')
                    // 30秒之后如果没有跳转成功就刷新页面 跳转了这个页面会自动关闭就不执行下面
                    setTimeout(() => {
                        if (!plugMain.pageData.videoPageInit) {
                            location.reload()
                        }
                    }, 60 * 1000);
                })
        ...
        async page_play() {
            console.log("%c page_play", "background:rgb(0,0,0);color:#fff");

            GM_addValueChangeListener('closeVideoByDetail', function (name, old_value, new_value, remote) {
                window.close()
            })
            setTimeout(() => {
                GM_addValueChangeListener('closeVideo', function (name, old_value, new_value, remote) {
                    if (remote) {
                        plugMain.tipsAndClose()
                    }
                })
            }, 10 * 1000);
            plugMain.waitOf(a => $("#drag:visible").length > 0).then(a => {
                setTimeout(() => {
                    document.querySelector('#drag > div.handler.handler_bg').style.left = '300px'
                    // 创建一个 MouseEvent 对象
                    var event = new MouseEvent('mousedown', {
                        bubbles: true,
                        cancelable: true,
                        pageX: 800, // 设置 pageX 参数
                        // 其他可选参数，根据需要设置
                    });

                    document.querySelector('#drag > div.handler.handler_bg').dispatchEvent(event)
                    // 创建一个 MouseEvent 对象
                    var event2 = new MouseEvent('mousemove', {
                        bubbles: true,
                        cancelable: true,
                        pageX: 1200, // 设置 pageX 参数
                        // 其他可选参数，根据需要设置
                    });

                    document.querySelector('#drag > div.handler.handler_bg').dispatchEvent(event2)
                }, 3000);
            }, err => {
                location.reload()
            })
            ...
            plugMain.pageData.video.index = setInterval(async () => {
                try {
                    ...
                    plugMain.setGMData("closeVideo", plugMain.now())
