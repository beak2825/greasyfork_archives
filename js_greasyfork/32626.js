...
// @license     AGPLv3
// @author      jcunews
// @description Disable YouTube video & screen based ads at home page, and right before or in the middle of the main video playback. Also disable YouTube's anti-adblocker popup dialog. As of now, it only work on embedded player.
  ...
  fn = (a, ipse, haia, hca, rpo, et) => {
    ...
    window.fetch = async function(u, o) {
      ...
      if (/\.googlevideo\.com\/videoplayback/.test(u.url) && u.args_dyva?.[1]?.body) {
        ...
        // while (r.ok && (b.size < 1000)) {
        //   //console.log('vstream-invalid', u.args_dyva);
        //   (o = o || {}).bnri = true;
        //   r = await ftca(new rq(...u.args_dyva), o);
        //   b = await r.clone().blob();
        // }
      }
      return r
    };
        ...
        window.ytcfg.set = function(ytConfig, ytValue){
          ...
          if (ytConfig) {
            ...
            if (a = ytConfig.EXPERIMENT_FLAGS) {
              ...
              if (a.enable_interstitial_entity_check) a.enable_interstitial_entity_check = false;
