// ==UserScript==
// @name         战斗吧歌姬！【内群】
// @homepage     https://greasyfork.org/zh-CN/scripts/430750-%E6%88%98%E6%96%97%E5%90%A7%E6%AD%8C%E5%A7%AC-%E5%86%85%E7%BE%A4
// @version      8.0.34
// @description  Bilibili直播间，小心心、天选时刻、实物抽奖、活动轮盘抽奖、动态抽奖
// @author       风绫丨钰袖、荒年
...
// @downloadURL https://update.greasyfork.org/scripts/431294/%E6%88%98%E6%96%97%E5%90%A7%E6%AD%8C%E5%A7%AC%EF%BC%81%E3%80%90%E5%86%85%E7%BE%A4%E3%80%91.user.js
// @updateURL https://update.greasyfork.org/scripts/431294/%E6%88%98%E6%96%97%E5%90%A7%E6%AD%8C%E5%A7%AC%EF%BC%81%E3%80%90%E5%86%85%E7%BE%A4%E3%80%91.meta.js
// ==/UserScript==
    ...
    function init() { //API初始化
        ...
        const MY_API = {
            CONFIG_DEFAULT: {
                ...
                detail_by_lid_reset:64000,//直播预约抽奖、动态抽奖起始LID序号
                detail_by_lid_flash:180,//直播预约抽奖、动态抽奖间隔
                gitee_url:0,//云屏蔽词数据地址
                ...
                medal_change: false,
                anchor_postdiscuss: 0, //屏蔽关键词、屏蔽房间号专栏CV号
                ...
                get_sessions_keyword:["中奖","二维码"],//私信关键词
                sendLiveDanmu_dm_type:false,//表情打卡开关
                sendLiveDanmu_dm_type_value:15,//表情表代号
...
<input style="vertical-align: text-top;" type="checkbox" >内群QQ机器人<br/>你的QQ号：<br/><input class="num" style="width:80px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
<button data-action="save1" style="font-size: 100%;color:  #000000">测试</button>
</div>
`);
                qq_run_mark = GM_info.script.homepage.indexOf("430750")>-1
                $('.chat-history-panel').append(qqrunmark);
                $('.zdbgjqqrunmark').hide()
                if(qq_run_mark)$('.zdbgjqqrunmark').show()
                if (MY_API.CONFIG.qqbot)
                    qqrunmark.find('div[data-toggle="qqbot"] input').attr('checked', '');
                ...
                qqrunmark.find('div[data-toggle="qqbot"] [data-action="save1"]').click(function () {
                    if(!qqbotmark)return MY_API.chatLog(`60秒CD中！`);
                    if(MY_API.CONFIG.qq && qqbotmark){
                        qq(MY_API.CONFIG.qq,`【天选众】${Live_info.uname}：群主QQ机器人推送测试消息！${year()}年${month()}月${day()}日${hour()}点${minute()}分！`).then(async function (data) {
                            if(data.retcode==0){
...
<button data-action="save2" style="font-size: 100%;color:  #000000">轮盘类</button>
...
<button data-action="save7" style="font-size: 100%;color:  #000000">中奖</button>
</div>
</fieldset>
...
<input class="hour" style="width:20px;vertical-align: top;" type="text">点推送<input class="days" style="width:20px;vertical-align: top;" type="text">日内中奖信息
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
</div>

<div data-toggle="ServerChan_SCKEY1">
<apend style="font-size: 100%; color:  #000000" title="从点击按钮测试">点击测试：
...
<input style="vertical-align: text-top;" type="checkbox" >Server酱Turbo版<br/><input class="num" style="width:61px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
...
<input style="vertical-align: text-top;" type="checkbox" >PUSH微信推送<br/><input class="num" style="width:61px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
...
<input style="vertical-align: text-top;" type="checkbox" >Qmsg丘丘推送<br/><input class="num" style="width:61px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
...
手动实物宝箱：<input class="num" style="width: 36px;vertical-align: top;" type="text">
...
<apend style="font-size: 100%; color: #000000" title="自动参与已关注的UP最新的动态抽奖">
<input id="dynamic_lottery" style="vertical-align: text-top;" type="checkbox">自动参与已关注的动态抽奖
</div>

<div data-toggle="detail_by_lid_dynamic">
<apend style="font-size: 100%; color: #000000" title="自动参与全部最新的动态抽奖">
<input id="detail_by_lid_dynamic" style="vertical-align: text-top;" type="checkbox" >自动参与近期全部动态抽奖
</div>

<div data-toggle="detail_by_lid_live">
<apend style="font-size: 100%; color: #000000" title="自动参与直播预约抽奖">
<input style="vertical-align: text-top;" type="checkbox" >自动参与近期直播预约抽奖
</div>

<div data-toggle="not_office_dynamic_go">
<apend style="font-size: 100%; color: #000000" title="自动参与近期大锦鲤专栏的动态抽奖">
<input style="vertical-align: text-top;" type="checkbox" >参与大锦鲤专栏的动态抽奖
</div>

<div data-toggle="detail_by_lid_flash">
<apend style="font-size: 100%; color:  #000000" title="直播预约抽奖、转发抽奖的最小间隔！">
转发参与间隔：<input class="num" style="width: 36px;vertical-align: top;" type="text">秒
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
</div>

<div data-toggle="detail_by_lid_reset">
<apend style="font-size: 100%; color:  #000000" title="重置直播预约抽奖、转发抽奖的ID序号！">
动态预约序号：<input class="num" style="width: 36px;vertical-align: top;" type="text">
...
<input style="vertical-align: text-top;" type="checkbox" >每隔<input class="num" style="width:20px;vertical-align: top;" type="text">分发一条动态
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
</div>

<div data-toggle="AUTO_dynamic_del">
<apend style="font-size: 100%; color:  #000000" title="删除旧动态">
删除<input class="num" style="width:20px;vertical-align: top;" type="text">天前的动态
<button data-action="save" style="font-size: 100%;color:  #000000">立即执行</button>
</div>
</fieldset>

<fieldset>
<legend  style="font-size: 100%;color:#000000;">【活动轮盘】</legend>
<div data-toggle="AUTO_activity_lottery">
<apend style="font-size: 100%; color: #000000" title="自动开始活动轮盘抽奖">
<input style="vertical-align: text-top;" type="checkbox" title="自动开始活动轮盘抽奖">自动开始活动轮盘抽奖
<button data-action="save" style="font-size: 100%;color: #000000">立即</button>
</div>

<div data-toggle="AUTO_activity_lottery_time">
<apend style="font-size: 100%; color: #000000" title="定时活动轮盘抽奖">
轮盘定时：<input class="hour" style="width:19px;vertical-align: top;" type="text">点
...
<append style="font-size: 100%;color: blue" ><data-toggle="AnchorFLASH" title="检索天选房间数据间隔/延迟，不含代码运行时间，范围：0-" >检索直播间间隔
<input class="AnchorFLASH" style="width: 30px;vertical-align: top;" type="text">毫秒
<button data-action="save" style="font-size: 100%;color: blue" title="范围：0-100，">保存</button>
</div>

<div data-toggle="AnchorserverFLASH" title="范围：8-50">
<append style="font-size: 100%;color: blue" title="获取群主专栏、简介天选数据的间隔，范围：8-50" >获取群主天选数据间隔
<input class="AnchorserverFLASH" style="width: 30px;vertical-align: top;" type="text">秒
<button data-action="save" style="font-size: 100%;color: blue" title="范围：8-50，">保存</button>
</div>

<div data-toggle="AnchorcheckFLASH" title="范围：10-5000">
<append style="font-size: 100%;color: blue" title="检索天选房时间间隔，范围：10-5000，建议检索耗时+检索间隔≤300秒，即一个5分钟天选的时间" >检索天选房列表时间间隔
<input class="AnchorcheckFLASH" style="width: 30px;vertical-align: top;" type="text">秒
<button data-action="save" style="font-size: 100%;color: blue" title="范围：10-5000">保存</button>
</div>

<div data-toggle="Anchor_room_send" >
<append style="font-size: 100%;color: blue" title="手动推送服务器房间，如果发现漏掉的天选，可以手动推送到专栏" >手动推送天选房
<input class="Anchor_room_send" style="width: 65px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color: blue">发送</button>
</div>

<div data-toggle="Anchor_room_get">
<input style="vertical-align: text-top;" type="checkbox" ><append style="font-size: 100%;color: blue">获取下列分区加入检索
</div>
<div data-toggle="tu50room">
<input style="vertical-align: text-top;" type="checkbox" ><append style="font-size: 100%;color: blue">分区直播间数增加到50
</div>

<div style="font-size: 100%;color: blue" data-toggle="parent_area_id">
...
<input style="vertical-align: text-top;" type="checkbox" ><append style="font-size: 100%;color: blue">把最新天选房加入常驻
</div>

<div data-toggle="Anchor_always_room_switch">
<input style="vertical-align: text-top;" type="checkbox" ><append style="font-size: 100%;color: blue" title="勾选后自动添加有天选的房间到常驻房">启用常驻房（上限：
<input class="Anchor_always_room_num" style="width: 28px;vertical-align: top;" type="text">）
<button data-action="save" style="font-size: 100%;color: blue">保存</button>
</div>

<div data-toggle="Anchor_room_go_switch">
<input style="vertical-align: text-top;" type="checkbox" ><append style="font-size: 100%;color: blue" title=循环20秒检索加急直播间，不需要时请关闭该选项，以免浪费检索资源，受检索模块限制">添加加急房
<input class="Anchor_room_go" style="width: 50px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color: blue">保存</button>
</div>

<div data-toggle="Anchor_always_room_add" >
<append style="font-size: 100%;color: blue" title="手动添加房间号到常驻天选房" >手动添加常驻房
<input class="Anchor_always_room_add" style="width: 67px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color: blue">添加</button>
</div>

<div data-toggle="fans_min">
<apend style="font-size: 100%; color:  #000000" title="抽奖主播粉丝数下限，低于下限不抽奖">
<input style="vertical-align: text-top;" type="checkbox">粉丝下限：<input class="num" style="width:50px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
</div>

<div data-toggle="money_min">
<apend style="font-size: 100%; color:  #000000" title="抽奖奖品金额下限（单位：元），低于下限不抽奖">
<input style="vertical-align: text-top;" type="checkbox">奖金下限：<input class="num" style="width:30px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
</div>
...
<input style="vertical-align: text-top;" type="checkbox">无勋章：奖金下限：<input class="num" style="width:20px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
</div>
...
<input style="vertical-align: text-top;" type="checkbox" >从专栏评论自动屏蔽词屏蔽房<br/>专栏号：<input class="num" style="width:60px;vertical-align: top;" type="text">
<br/>云地址：<input class="string" style="width:60px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
</div>

<div data-toggle="anchor_danmu_content">
<apend style="font-size: 100%; color:  #000000" title="中奖自动发送随机弹幕，英文逗号隔开，不会发送弹幕！">
<input style="vertical-align: text-top;" type="checkbox" >中奖后自动发送随机弹幕<br/><input class="num" style="width:124px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
</div>

<div data-toggle="anchor_msg_content">
<apend style="font-size: 100%; color:  #000000" title="中奖自动发送随机私信，英文逗号隔开">
<input style="vertical-align: text-top;" type="checkbox" >中奖后自动发送随机私信<br/><input class="num" style="width:124px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
</div>

<div data-toggle="Anchor_ignore_keyword">
<apend style="font-size: 100%;color: #000000" title="没兴趣的奖品，可以按格式加进去,注意是英文逗号！">
奖品名称屏蔽词设置<button data-action="save" style="font-size: 100%;color: #000000">保存</button><input class="keyword" style="width: 156px;vertical-align: top;" type="text">
</div>
...
奖品名称正则关键词<button data-action="save" style="font-size: 100%;color: #000000">保存</button><input class="keyword" style="width: 156px;vertical-align: top;" type="text">
</div>

<div data-toggle="Anchor_ignore_uid">
<apend style="font-size: 100%;color: #000000" title="主播黑名单，可以按格式加进去,注意是英文逗号！">
直播间真实房间号屏蔽列表<button data-action="save" style="font-size: 100%;color: #000000">保存</button><input class="keyword" style="width: 156px;vertical-align: top;" type="text"><br>
...
`);
              ...
              /*  ohb.append(`
<fieldset>
<legend append style="font-size: 100%;color: #000000;text-align: left;"><span>天选众部分奖品展示</legend>
<div id="ohb" style="font-size: 100%;color:#000000;">
<append">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp可喜可贺！！！！！！<br/>
&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp一加9RT手机、一加 Buds Z2、C202 GT跑鞋、XIAOMI CIVI、PlayStation5、VIVO X70系列手机、中兴Axon30手机3部（奇迹之夜包揽全场）、苹果手机iphone 12、Ipad、Z30 Pro手机3部、领克04电动滑板车、OPPO Reno6 Pro一台、Note 10新品、
真我GT Neo闪速版、现金红包1000、648等现金红包、红米K40增强版、华为Nova8 Pro、华为路由AX3、cherry键盘8.0、价值千元汉服套装、NS游戏机、联想平板电脑、各种大礼包、各种手办等。
<br>
注：欢迎加群欧气互吸，群内大量欧皇。
</div>
</fieldset>
...
<fieldset>
<legend append style="font-size: 100%;color: #000000;text-align: left;"><span>感谢打赏天选众名单</legend>
<div id="ohb" style="font-size: 100%;color:#000000;">
<append">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp感谢打赏！祝各位姥爷幸运满满天天中奖！<br/>
&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp芜湖机长、铃仙·优昙华院·因幡、王胡子、Period、宋大宝、想去更高的地方看、微风不会运球🏀、*海、夜里lossDES、Alice秋洛、黑之刃、98、锦程、99、hokma、噜啦、黏玉米、林同学想瘦、Stranger丶、_程序洋_、籍嘉树、
Vv、直播大喇叭、有缘来相聚、勺子、喵内、云烟゚、莫邪、♑momo、杀嗷嗷、哭泣、溯夜、
林同学想瘦、格林、阳光下的蘑菇君°、十万个不知道/kb、Revne Starlight、WALLsuai、珂学、枯苏只为伊莉雅、
阿巴阿巴、　、木ノ葉の金色の光、空格、末那、ExEcution、由崎司、蛇喵~、玄鸟、a'ゞ安静、**麒、a 坤。、11、
HN_GG、让孩子中一次奖吧、Chhro、十年轻音、猫之欧皇、飘飘然、舞铲阶级佳星总淑妓、荒年、恩惠、
天都黑了、我不得好死、🌸、冫挽歌朽年彡、机枪阵地前推十五米、花背淘现 五个点、flames@観測者、枔殇ぉ、
鲁小蛋、Aw、龙腾丶凤舞、西 瓜、霸道、鏌鋣、再也不会喜欢你了、Time、自由、柯梓、雀啄 、The only one、
Echo 、eno ylno shT、22554348、ゞ夏夜雨御゜ 、奕手！！
</div>
</fieldset>

<fieldset>
<legend append style="font-size: 100%;color: #000000;text-align: left;"><span>支付宝、微信打赏作者！感谢您的支持！</legend>
<img id="img3" src="https://greasyfork.s3.us-east-2.amazonaws.com/ephhbs5zbdle9ftgk6vpr2kdy69z" width="150" height="130" />
<img id="img4" src="https://greasyfork.s3.us-east-2.amazonaws.com/ij2ogkfnrx8ml27zdwqo1ne3t0t2" width="150" height="130" />
</fieldset>
`);*/

                div.append(`
<fieldset>
<legend append style="font-size: 100%;color: #000000;text-align: left;">欢迎来到<span>${ZBJ}</span>的直播间</legend>
...
<fieldset>
<legend id="CJ" style="font-size: 100%;color: #000000;text-align: left;">我欲成仙丨快乐无边【<span>${CJ}</span>】</legend>
...
<fieldset>
<legend append style="font-size: 100%;color: #000000">修仙插件参数设置</legend>
...
<input style="vertical-align: text-top;" type="checkbox" ><append style="font-size: 100%;color: Orange">隐藏抽奖等反馈信息
</div>

<div data-toggle="AUTO_BOX">
<apend style="font-size: 100%; color: Orange">
...
<apend style="font-size: 100%; color: Orange" title="获取主站登陆、观看、转发（不显示在动态）经验">
...
<apend style="font-size: 100%; color: Orange" title="不占网络内存，加速倍率与勋章数有关，手机端可能无效">
...
<input style="font-size: 100%;color: #000000;vertical-align: text-top;" type="checkbox"><apend style="font-size: 100%; color: purple">表情包弹幕签到：
...
<option value="https://i0.hdslb.com/bfs/live/8471cbcaae6f397c6942bec6dbc5d9f9b07d18a1.png@.webp" id="nalaibani">拿来吧你</option>
...
<option value="https://i0.hdslb.com/bfs/live/4fca02aec57f53c610b863a25e8725ce29275890.png@.webp" id="xiaochangmian">小场面</option>
</select>
...
签到弹幕<input class="num" style="width:124px;vertical-align: top;" type="text">
...
勋章打卡屏蔽列表（填写真实房号）：<br>
<input class="num" style="width:160px;vertical-align: top;" type="text"><button data-action="save" style="font-size: 100%;color:  #000000">保存</button>
</div>

<div data-toggle="TIMEAREADISABLE">
<apend style="font-size: 100%; color: purple">
<input style="vertical-align: text-top;" type="checkbox">启用
<input class="start" style="width: 15px;vertical-align: top;" type="text">点至
<input class="end" style="width: 15px;vertical-align: top;" type="text">点不抽奖（0-23）
<button data-action="save" style="font-size: 100%;color: purple">保存</button>
</div>

<div data-toggle="onedayLT">
<apend style="font-size: 100%; color: purple" title="当天过期辣条、小心心，24点后会自然消失！">
<input style="vertical-align: text-top;" type="checkbox">当天过期礼物投喂房间号：
<input class="start" style="width: 75px;vertical-align: top;" type="text">
<button data-action="save" style="font-size: 100%;color: purple">保存</button>
</div>
...
<fieldset>
<legend  style="font-size: 100%;color:#000000;">【异常判断通知】</legend>
...
<input style="font-size: 100%;color: #000000;vertical-align: text-top;" type="checkbox">参与天选出现未关注异常时自动推送提醒<br>
接收UID：<input class="uid" style="width: 80px;vertical-align: top;" type="text">
...
关键词设置：<input class="keyword" style="width: 156px;vertical-align: top;" type="text"><button data-action="save2" style="font-size: 100%;color: #000000">保存</button>
</div>
</fieldset>
...
<input style="font-size: 100%;color: #000000;vertical-align: text-top;" type="checkbox"><apend style="font-size: 100%;color: purple;vertical-align: top;">随机鸡汤文弹幕<br>
<apend style="font-size: 100%;color: purple;vertical-align: top;">发送弹幕：
<input class="start" style="width: 160px;vertical-align: top;" type="text"><br>到直播间：
<input class="end" style="width: 80px;vertical-align: top;" type="text"><br>每
<input class="number1" style="width: 26px;vertical-align: top;" type="text">秒，共发送
<input class="number2" style="width: 26px;vertical-align: top;" type="text">次
<button data-action="save" style="font-size: 100%;color: purple">开始</button>
<button data-action="save1" style="font-size: 100%;color: purple">停止</button>
</div>
</fieldset>

<fieldset>
<legend append style="font-size: 100%;color: #000000;text-align: left;"><span>配置参数操作界面</legend>
...
<span>${end_time1}</span></br>
<span>${anchor_name2}</span>：
<span>${award_name2}</span>
<span>${end_time2}</span></br>
...
`);
                ...
                sessions.append(`
<fieldset>
<legend  style="font-size: 100%;color:#000000;">【私信提取列表】</legend>
...
`);
                ...
                journal2.append(`
<fieldset>
<legend  style="font-size: 100%;color:#000000;">【活动轮盘抽奖日志】</legend>
<div data-toggle="freejournal_clear">
<button data-action="save" style="font-size: 100%;color: #000000;" >清空日志</button>
</div>
...
`);
                ...
                journal7.find('div[data-toggle="freejournal_clear"] [data-action="save"]').click(async function () {
                    MY_API.CONFIG.freejournal7 = []
                    MY_API.saveConfig()
...
<input style="vertical-align: text-top;" type="checkbox">低粉丝量主播：粉丝数量低于<input class="num" style="width: 50px;vertical-align: top;" type="text">的主播
<button data-action="save" style="font-size: 100%;color: #000000" >保存</button>
<button data-action="save1" style="font-size: 100%;color: #000000" >一键取关！</button>
</div>

<div data-toggle="tags4">
<input style="vertical-align: text-top;" type="checkbox">天选鸽子主播：天选时刻超过<input class="num" style="width: 16px;vertical-align: top;" type="text">天未开的主播
<button data-action="save" style="font-size: 100%;color: #000000" >保存</button>
<button data-action="save1" style="font-size: 100%;color: #000000" >一键取关！</button><br>
</div>

<div data-toggle="tags5">
<input style="vertical-align: text-top;" type="checkbox">动态鸽子主播：超过<input class="num" style="width: 16px;vertical-align: top;" type="text">天未发动态的主播
<button data-action="save" style="font-size: 100%;color: #000000" >保存</button>
<button data-action="save1" style="font-size: 100%;color: #000000" >一键取关！</button><br>
</div>
...
<input style="vertical-align: text-top;" type="checkbox">关注直播主播数量大于<input class="num" style="width: 50px;vertical-align: top;" type="text">时自动取关无私信主播
...
`);
                ...
                if (MY_API.CONFIG.AUTO_activity_lottery)
                    tj.find('div[data-toggle="AUTO_activity_lottery"] input').attr('checked', '');
                tj.find('div[data-toggle="AUTO_activity_lottery"] input').change(async function () { //
                    MY_API.CONFIG.AUTO_activity_lottery = $(this).prop('checked');
                    MY_API.saveConfig()
                    MY_API.chatLog(`【活动轮盘抽奖】开关设置：${MY_API.CONFIG.AUTO_activity_lottery}！`);
                });
                ...
                tj.find('div[data-toggle="AUTO_activity_lottery_time"] [data-action="save"]').click(function () {
                    ...
                    MY_API.saveConfig()
                    MY_API.chatLog(`自动轮盘设置【时-分】：${MY_API.CONFIG.AUTO_activity_lottery_time_hour}-${MY_API.CONFIG.AUTO_activity_lottery_time_min}`);
                });
                        ...
                        if(ignore_max_mark){//黑名单已满
                            if (MY_API.CONFIG.room_ruid.indexOf(ignore_room_array[i]) > -1 ) {
                                ...
                                BAPI.modify(MY_API.CONFIG.room_ruid[num + 1], 2).then(async(data) => {
                                    console.log(data)
                                    if (data.code == 0) {
                                        MY_API.chatLog(`【一键拉黑取关】${ignore_room_array[i]}取关成功！`);
                                    } else {
                ...
                tags.find('div[data-toggle="tags1"] [data-action="save"]').click(async function () {
                    ...
                    if(MY_API.CONFIG.tags6_checkbox){
                        ...
                        let get_AnchorRecord = async function (pn = 1) {
                            ...
                            if (pn == 1)
                                AnchorRecord_uid = [];
                            return BAPI.Lottery.anchor.AnchorRecord(pn).then((data) => {
                        ...
                        let move_lowfans_uid = async function () {
                            ...
                            for (let i = 0; i < tags_mid_list.length; i++) {
                                ...
                                await BAPI.web_interface_card(tags_mid_list[i]).then(async(data) => {
                                    ...
                                    MY_API.chatLog(`【低粉主播】粉丝数量获取失败，可能风控了，可调出控制台错误信息反馈到群里！`, 'warning');
                                })
                            }
                        }
                        ...
                        let move_room_AnchorRecord_time = async function () {
                            ...
                            for (let i = 0; i < MY_API.CONFIG.room_AnchorRecord_time.length; i = i + 2) {
                                ...
                                if (ts_s() - MY_API.CONFIG.room_AnchorRecord_time[i + 1] > 3600 * 24 * MY_API.CONFIG.tags4_min && tags_mid_list.indexOf(MY_API.CONFIG.room_ruid[rr + 1]) > -1) {
                                    MY_API.CONFIG.room_AnchorRecord_time.push(MY_API.CONFIG.room_ruid[rr + 1])
                    ...
                    if (r == true) {
                        ...
                        for (let i = 0; i < room_AnchorRecord_time_uid.length; i++) {
                            ...
                            BAPI.modify(room_AnchorRecord_time_uid[i], 2).then((data) => {
                                ...
                                if (data.code == 0)
                                    MY_API.chatLog(`【一键取关】鸽子主播${room_AnchorRecord_time_uid[i]}取关成功`);
                            })
                        }
                        now_p = '【取关鸽子主播完成】'
                        $('#now_num span:eq(0)').text(now_p);
                ...
                tags.find('div[data-toggle="tags2"] input').change(function () { //
                ...
                tags.find('div[data-toggle="tags4"] input').change(function () { //
                ...
                tags.find('div[data-toggle="tags6"] input').change(function () { //
                ...
                tags.find('div[data-toggle="tags5"] input').change(function () { //
                    ...
                    if (r == true) {
                        ...
                        for (let i = 0; i < space_history_uid.length; i++) {
                            ...
                            BAPI.modify(space_history_uid[i], 2).then((data) => {
                                ...
                                if (data.code == 0)
                                    MY_API.chatLog(`【一键取关】动态鸽子主播${space_history_uid[i]}取关成功`);
                            })
                        }
                        now_p = '【取关动态鸽子主播完成】'
                        $('#now_num span:eq(0)').text(now_p);
                ...
                tags.find('div[data-toggle="getmsg"] input').change(function () { //
                        ...
                        await BAPI.getMsg(tags_mid_list[i]).then(async(data) => {
                            ...
                            if (msg == null) {
                                ...
                                BAPI.modify(tags_mid_list[i], 2).then((data) => {
                                    ...
                                    if (data.code == 0)
                                        MY_API.chatLog(`【一键取关】无私信主播${tags_mid_list[i]}取关成功`);
                                })
                            }
                        })
                    }
                    ...
                    if (r == true) {
                        ...
                        for (let i = 0; i < lowfans_uid.length; i++) {
                            ...
                            BAPI.modify(lowfans_uid[i], 2).then((data) => {
                                ...
                                if (data.code == 0)
                                    MY_API.chatLog(`【一键取关】低粉主播${lowfans_uid[i]}取关成功`);
                            })
                        }
                        now_p = '【取关低粉主播完成】'
                        $('#now_num span:eq(0)').text(now_p);
                ...
                tj.find('div[data-toggle="detail_by_lid_flash"] [data-action="save"]').click(function () { //detail_by_lid_flash save按钮
                ...
                tj.find('div[data-toggle="detail_by_lid_reset"] [data-action="save"]').click(function () { //detail_by_lid_reset save按钮
                ...
                tj.find('div[data-toggle="do_GOLDBOX"] [data-action="save"]').click(function () { //do_GOLDBOX save按钮
                ...
                tj.find('div[data-toggle="anchor_postdiscuss_switch"] input').change(function () {
                ...
                tj.find('div[data-toggle="AUTO_dynamic_create"] [data-action="save"]').click(function () { //money_min save按钮
                    let val = parseInt(tj.find('div[data-toggle="AUTO_dynamic_create"] .num').val());
                    if(val < 3) val = 3
                    MY_API.CONFIG.AUTO_dynamic_create_flash = val;
                    MY_API.saveConfig();
                    MY_API.chatLog(`自动发动态间隔已设置，自动发动态间隔为随机：${val}-${1.25 * val}分钟`);
                });
                if (MY_API.CONFIG.AUTO_dynamic_create)tj.find('div[data-toggle="AUTO_dynamic_create"] input').attr('checked', '');
                tj.find('div[data-toggle="AUTO_dynamic_create"] input').change(async function () {
                    ...
                    MY_API.saveConfig()
                    MY_API.chatLog(`自动发动态设置：${MY_API.CONFIG.AUTO_dynamic_create}`);
                });
                ...
                tj.find('div[data-toggle="money_min"] [data-action="save"]').click(function () { //money_min save按钮
                    let val = Number(tj.find('div[data-toggle="money_min"] .num').val());
                    MY_API.CONFIG.money_min = val;
                    MY_API.saveConfig();
                    MY_API.chatLog(`奖金下限设置：${MY_API.CONFIG.money_min}`);
                });
                tj.find('div[data-toggle="fans_min"] [data-action="save"]').click(function () { //fans_min save按钮
                ...
                tj.find('div[data-toggle="fans_min"] input').change(function () { //
                    MY_API.CONFIG.fans_switch = $(this).prop('checked');
                    MY_API.saveConfig()
                    MY_API.chatLog(`粉丝下限设置：${MY_API.CONFIG.fans_switch}`);
                });
                tj.find('div[data-toggle="money_min"] input').change(function () { //
                ...
                tj.find('div[data-toggle="AUTO_GOLDBOX_sever2"] input').change(async function () {
                ...
                tj.find('div[data-toggle="no_money_checkbox"] input').change(function () { //
                ...
                tj.find('div[data-toggle="AUTO_medal_get_up"] input').change(function () { //
                ...
                tj.find('div[data-toggle="AUTO_medal_up"] input').change(function () { //
                ...
                tj.find('div[data-toggle="detail_by_lid_dynamic"] input').change(async function () {
                ...
                tj.find('div[data-toggle="dynamic_lottery"] input').change(async function () {
                ...
                tj.find('div[data-toggle="detail_by_lid_live"] input').change(async function () {
                ...
                tj.find('div[data-toggle="not_office_dynamic_go"] input').change(async function () {
                ...
                tj.find('div[data-toggle="Anchor_cur_gift_num"] input').change(function () { //
                ...
                tj.find('div[data-toggle="get_following_live"] input').change(async function () { //
                ...
                tj.find('div[data-toggle="sever_room_checkbox"] input').change(function () { //
                ...
                tj.find('div[data-toggle="parent_area_id"] input').change(function () {
                ...
                if (MY_API.CONFIG.unusual)
                    div.find('div[data-toggle="unusual"] input').attr('checked', '');
                div.find('div[data-toggle="unusual"] input:checkbox').change(function () {
                    MY_API.CONFIG.unusual = $(this).prop('checked');
                    MY_API.saveConfig()
                    MY_API.chatLog(`天选关注异常自动推送设置：${MY_API.CONFIG.unusual}`);
                });
                        ...
                        return BAPI.get_sessions(end_ts).then(async function (data) {
                            if(data.code==0){
                                ...
                                for(let i=0;i<session_list.length;i++){
                                    ...
                                    if(i % 2 == 0){
                                        $("#get_sessions").css("color", "red")
                                    }else{
                                        $("#get_sessions").css("color", "#000000")
                                    }
                ...
                if(MY_API.CONFIG.sendLiveDanmu_dm_type_value == 13){
                ...
                if(MY_API.CONFIG.sendLiveDanmu_dm_type_value == 15){
                ...
                if(MY_API.CONFIG.sendLiveDanmu_dm_type_value == 23){
                ...
                if(MY_API.CONFIG.sendLiveDanmu_dm_type_value == 25){
                ...
                if(MY_API.CONFIG.sendLiveDanmu_dm_type_value == 26){
                ...
                if(MY_API.CONFIG.sendLiveDanmu_dm_type_value == 17){
                ...
                if(MY_API.CONFIG.sendLiveDanmu_dm_type_value == 18){
                    document.getElementById("nalaibani").selected = true
                    document.getElementById("bqb_default").src="https://i0.hdslb.com/bfs/live/8471cbcaae6f397c6942bec6dbc5d9f9b07d18a1.png@.webp"
                }
                if(MY_API.CONFIG.sendLiveDanmu_dm_type_value == 19){
                ...
                if(MY_API.CONFIG.sendLiveDanmu_dm_type_value == 20){
                ...
                if(MY_API.CONFIG.sendLiveDanmu_dm_type_value == 21){
                    document.getElementById("xiaochangmian").selected = true
                    document.getElementById("bqb_default").src="https://i0.hdslb.com/bfs/live/4fca02aec57f53c610b863a25e8725ce29275890.png@.webp"
                }


                $(".bqb").change(function () {
                    if(document.getElementById("23333").selected == true)MY_API.CONFIG.sendLiveDanmu_dm_type_value = 13
                    if(document.getElementById("daraole").selected == true)MY_API.CONFIG.sendLiveDanmu_dm_type_value = 15
                    if(document.getElementById("xiaosi").selected == true)MY_API.CONFIG.sendLiveDanmu_dm_type_value = 23
                    if(document.getElementById("laile").selected == true)MY_API.CONFIG.sendLiveDanmu_dm_type_value = 25
                    if(document.getElementById("niuniuniu").selected == true)MY_API.CONFIG.sendLiveDanmu_dm_type_value = 26
                    if(document.getElementById("miaoa").selected == true)MY_API.CONFIG.sendLiveDanmu_dm_type_value = 17
                    if(document.getElementById("nalaibani").selected == true)MY_API.CONFIG.sendLiveDanmu_dm_type_value = 18
                    if(document.getElementById("youdiandongxi").selected == true)MY_API.CONFIG.sendLiveDanmu_dm_type_value = 19
                    if(document.getElementById("lipu").selected == true)MY_API.CONFIG.sendLiveDanmu_dm_type_value = 20
                    if(document.getElementById("xiaochangmian").selected == true)MY_API.CONFIG.sendLiveDanmu_dm_type_value = 21
                    MY_API.saveConfig()
                ...
                div.find('div[data-toggle="Anchor_danmu_go"] input').change(function () {
                ...
                tj.find('div[data-toggle="push_msg_oneday"] input').change(function () {
                    MY_API.CONFIG.push_msg_oneday_check = $(this).prop('checked');
                    MY_API.saveConfig()
                    MY_API.chatLog(`ServerChan设置：${MY_API.CONFIG.push_msg_oneday_check}`);
                });
                ...
                tj.find('div[data-toggle="push_msg_oneday"] [data-action="save"]').click(function () {
                    ...
                    MY_API.CONFIG.push_msg_oneday_days = parseInt(tj.find('div[data-toggle="push_msg_oneday"] .days').val());
                    MY_API.saveConfig();
                    MY_API.chatLog(`设置【时-天】：${MY_API.CONFIG.push_msg_oneday_hour}-${MY_API.CONFIG.push_msg_oneday_days}`);
                });
                ...
                tj.find('div[data-toggle="ServerChan_SCKEY"] [data-action="save"]').click(function () {
                    ...
                    MY_API.saveConfig();
                    MY_API.chatLog(`'ServerChan设置：${MY_API.CONFIG.ServerChan_SCKEY}`);
                });
                ...
                tj.find('div[data-toggle="ServerChan_SCKEY"] input').change(function () {
                    ...
                    MY_API.saveConfig()
                    MY_API.chatLog(`ServerChan设置：${MY_API.CONFIG.switch_ServerChan_SCKEY}`);
                });
                ...
                tj.find('div[data-toggle="Qmsg_KEY"] [data-action="save"]').click(function () {
                    ...
                    MY_API.saveConfig();
                    MY_API.chatLog(`Qmsg_KEY已设置：${MY_API.CONFIG.Qmsg_KEY}`);
                });
                if (MY_API.CONFIG.switch_Qmsg_KEY)
                    tj.find('div[data-toggle="Qmsg_KEY"] input').attr('checked', '');
                tj.find('div[data-toggle="Qmsg_KEY"] input').change(function () { //
                    MY_API.CONFIG.switch_Qmsg_KEY = $(this).prop('checked');
                    MY_API.saveConfig()
                    MY_API.chatLog(`Qmsg_KEY已设置：${MY_API.CONFIG.switch_Qmsg_KEY}`);
                });
                ...
                tj.find('div[data-toggle="push_KEY"] [data-action="save"]').click(function () {
                    ...
                    MY_API.saveConfig();
                    MY_API.chatLog(`push_KEY已设置：${MY_API.CONFIG.push_KEY}`);
                });
                if (MY_API.CONFIG.switch_push_KEY)
                    tj.find('div[data-toggle="push_KEY"] input').attr('checked', '');
                tj.find('div[data-toggle="push_KEY"] input').change(function () { //
                    MY_API.CONFIG.switch_push_KEY = $(this).prop('checked');
                    MY_API.saveConfig()
                    MY_API.chatLog(`push_KEY已设置：${MY_API.CONFIG.switch_push_KEY}`);
                });

                if (MY_API.CONFIG.medal_change)
                    div.find('div[data-toggle="medal_change2"] input').attr('checked', '');
                div.find('div[data-toggle="medal_change2"] input').change(async function () { //
                ...
                tj.find('div[data-toggle="AnchorcheckFLASH"] [data-action="save"]').click(function () { //AnchorcheckFLASH
                ...
                tj.find('div[data-toggle="tu50room"] input').change(function () {
                ...
                tj.find('div[data-toggle="Anchor_room_get"] input').change(function () {
                    MY_API.CONFIG.Anchor_room_get = $(this).prop('checked');
                    MY_API.saveConfig()
                    MY_API.chatLog(`获取分区加入检索设置：${MY_API.CONFIG.Anchor_room_get}`);
                });

                tj.find('div[data-toggle="Anchor_room_get_to_always"] input').change(function () {
                ...
                tj.find('div[data-toggle="Anchor_always_room_switch"] input').change(function () {
                    MY_API.CONFIG.Anchor_always_room_switch = $(this).prop('checked');
                    MY_API.saveConfig()
                    MY_API.chatLog(`启用常驻房设置：${MY_API.CONFIG.Anchor_always_room_switch}`);
                });

                tj.find('div[data-toggle="Anchor_room_go_switch"] input').change(function () {
                ...
                div.find('div[data-toggle="AUTO_COIN"] input').change(function () { //
                ...
                div.find('div[data-toggle="AUTO_COIN2"] input').change(function () { //
                ...
                div.find('div[data-toggle="TIMEAREADISABLE"] input:checkbox').change(function () { //
                    ...
                    MY_API.saveConfig()
                    MY_API.chatLog(`休眠时间设置：${MY_API.CONFIG.TIMEAREADISABLE}`);
                });
                ...
                div.find('div[data-toggle="TIMEAREADISABLE"] [data-action="save"]').click(function () { //
                    ...
                    if (TIMEAREASTART >= TIMEAREAEND) {
                        ...
                        MY_API.saveConfig()
                        MY_API.chatLog(`休眠时间设置【时-时】：${MY_API.CONFIG.TIMEAREASTART}-${MY_API.CONFIG.TIMEAREAEND}`);
                    }
                });
            ...
            medal_sign_danmu:async() => {
                ...
                for (let i = 0; i < medal_roomid_list_now.length; i++) {
                    if(i==medal_roomid_list_now.length-1){
                        setTimeout(() => {
            ...
            new_activity_lottery:async function(){
                ...
                MY_API.chatLog(`【活动轮盘抽奖（新）】开始活动轮盘抽奖！`);
                ...
                if(myjson[0]== undefined)return MY_API.chatLog(`【活动轮盘抽奖（新）】云数据获取异常！`);// && myjson2[0]== undefined
                ...
                for (let i = 0; i < myjson.length; i++) {
                    if (i == 0)
                        MY_API.chatLog(`【活动轮盘抽奖（新）】开始获取抽奖次数！`);
                    sidtimes[i] = 0
                    for (let t = 0; t < 21; t++) {
                        await Sleep(200)
                        await BAPI.new_activity_lottery.addtimes(myjson[i].sid).then(async(data) => {
                            ...
                            if (data.code == 75001 || data.code == 75003) {
                                t = 999
                        ...
                        await BAPI.new_activity_lottery.mytimes(myjson[i].sid).then(async(data) => {
                            ...
                            if (data.data == undefined || data.data.times == 0) {
                                MY_API.chatLog(`【活动轮盘抽奖（新）】正在获取抽奖次数（${i+1}/${myjson.length}）：${myjson[i].name}(${sidtimes[i]})！`);
                                t = 999
                                return
                            }
                    ...
                    if (i == myjson.length-1)
                        MY_API.chatLog(`【活动轮盘抽奖（新）】获取抽奖次数结束！`);
                }

                for (let t = 0; t < 500; t++) {
                    if(!MY_API.CONFIG.AUTO_activity_lottery)break
                    if (t == 0)
                        MY_API.chatLog(`【活动轮盘抽奖（新）】开始抽奖！`);
                    for (let i = 0; i < myjson.length; i++) {
                        if (sidtimes[i] <= 0)
                            continue
                        await Sleep(1000)
                        await BAPI.new_activity_lottery.do(myjson[i].sid).then(async(data) => {
                            ...
                            if (data.code == 75401 || data.code == 75415 ) { //75415次数不足//74501无抽奖资格|| data.data == undefined
                                sidtimes[i] = 0
                                return
                            }
                            if (data.code == 75400) {
                            ...
                            if (data.code == 0) {
                                MY_API.chatLog(`【活动轮盘抽奖（新）】正在抽奖（${i+1}/${myjson.length}）：${myjson[i].name}(${sidtimes[i]})！`);
                                MY_API.chatLog(`【活动轮盘抽奖（新）】（${i+1}/${myjson.length}）${myjson[i].name}：${data.data[0].gift_name}！`);
                                sidtimes[i] = sidtimes[i] - 1
                                MY_API.CONFIG.freejournal2.unshift(`<br>${timestampToTime(ts_s())}：${myjson[i].name}(${sidtimes[i]+1}，${data.data[0].gift_name})`)
                            ...
                            if (data.data[0].gift_name != "未中奖0") {
                                BAPI.anchor_postdiscuss(`【活动轮盘抽奖（新）】${Xname}：${data.data[0].gift_name}`,server_cv_list[3]).then(async(data) => {
                                ...
                                MY_API.CONFIG.freejournal7.unshift(`<br>${timestampToTime(ts_s())}：【活动轮盘抽奖（新）】：${myjson[i].name}：${data.data[0].gift_name}`)
                                MY_API.saveConfig()
                                $('#freejournal7 span:eq(0)').text(MY_API.CONFIG.freejournal7)
                                let dt = document.getElementById('freejournal7'); //通过id获取该div
                                dt.innerHTML  = MY_API.CONFIG.freejournal7
                                ...
                                if(MY_API.CONFIG.qqbot && qq_run_mark){
                                    qq(MY_API.CONFIG.qq,`【活动轮盘抽奖（新）】${Live_info.uname}：恭喜你在【活动轮盘抽奖（新）】${myjson[i].name}获得${data.data[0].gift_name}！`);
                                }
                                if (MY_API.CONFIG.switch_Qmsg_KEY) {
                                    qmsg(MY_API.CONFIG.Qmsg_KEY, `【活动轮盘（新）】${Live_info.uname}：${myjson[i].name}获得${data.data[0].gift_name}！`);
                                }
                                if (MY_API.CONFIG.switch_ServerChan_SCKEY) {
                                    ServerChan2(MY_API.CONFIG.ServerChan_SCKEY, `【活动轮盘抽奖（新）】${Live_info.uname}：恭喜你在【活动轮盘抽奖（新）】${myjson[i].name}获得${data.data[0].gift_name}！`)
                                }
                                if (MY_API.CONFIG.switch_push_KEY) {
                                    pushmsg(MY_API.CONFIG.push_KEY, `【活动轮盘抽奖（新）】${Live_info.uname}：恭喜你在【活动轮盘抽奖（新）】${myjson[i].name}获得${data.data[0].gift_name}！`);
                                }
                            }
                        })
                    }
                    if (t == 499) {
                        MY_API.chatLog(`【活动轮盘抽奖（新）】抽奖结束！`);
                    }
                }
            ...
            activity_lottery:async function(){
                activity_lottery_run_mark = false
                MY_API.chatLog(`【活动轮盘抽奖】开始活动轮盘抽奖！`);
                ...
                let myjson = await getMyJson(url);
                let myjson2 = await getMyJson(url2);
                if(myjson[0]== undefined && myjson2[0]== undefined)return MY_API.chatLog(`【活动轮盘抽奖】云数据获取异常！`);
                for(let i = 0; i < myjson2.length; i++){
                    if(!myjson.some(v => v.sid ==myjson2[i].sid))myjson.push(myjson2[i])
                }
                let myjson_p=[]
                ...
                for (let i = 0; i < myjson.length; i++) {
                    if (i == 0)
                        MY_API.chatLog(`【活动轮盘抽奖】开始获取抽奖次数！`);
                    sidtimes[i] = 0
                    for (let t = 0; t < 21; t++) {
                        await Sleep(200)
                        ...
                        await BAPI.activity_lottery.mytimes(myjson[i].sid).then(async(data) => {
                            ...
                            if (data.data == undefined || data.data.times == 0) {
                                MY_API.chatLog(`【活动轮盘抽奖】正在获取抽奖次数（${i+1}/${myjson.length}）：${myjson[i].name}(${sidtimes[i]})！`);
                                t = 999
                                return
                            }
                            if (sidtimes[i] == data.data.times) {
                    ...
                    if (i == myjson.length-1)
                        MY_API.chatLog(`【活动轮盘抽奖】获取抽奖次数结束！`);
                }

                for (let t = 0; t < 500; t++) {
                    if(!MY_API.CONFIG.AUTO_activity_lottery)break
                    if (t == 0)
                        MY_API.chatLog(`【活动轮盘抽奖】开始抽奖！`);
                    for (let i = 0; i < myjson.length; i++) {
                        if (sidtimes[i] <= 0)
                            continue
                        await Sleep(1000)
                        await BAPI.activity_lottery.do(myjson[i].sid).then(async(data) => {
                            ...
                            if (data.code == 0) {
                                MY_API.chatLog(`【活动轮盘抽奖】正在抽奖（${i+1}/${myjson.length}）：${myjson[i].name}(${sidtimes[i]})！`);
                                MY_API.chatLog(`【活动轮盘抽奖】（${i+1}/${myjson.length}）${myjson[i].name}：${data.data[0].gift_name}！`);
                                sidtimes[i] = sidtimes[i] - 1
                                MY_API.CONFIG.freejournal2.unshift(`<br>${timestampToTime(ts_s())}：${myjson[i].name}(${sidtimes[i]+1}，${data.data[0].gift_name})`)
                                if (MY_API.CONFIG.freejournal2.length > 500) {
                                    MY_API.CONFIG.freejournal2.splice(400, 100);
                            ...
                            if (data.data[0].gift_name != "未中奖0") {
                                BAPI.anchor_postdiscuss(`【活动轮盘抽奖】${Xname}：${data.data[0].gift_name}`,server_cv_list[3]).then(async(data) => {
                                    console.log('anchor_postdiscuss',data)
                                    if(data.code==0){
                                        MY_API.CONFIG.congratulations_rpid_ct.push(data.data.reply.rpid)
                                        MY_API.CONFIG.congratulations_rpid_ct.push(data.data.reply.ctime)
                                ...
                                MY_API.CONFIG.freejournal7.unshift(`<br>${timestampToTime(ts_s())}：【活动轮盘抽奖】：${myjson[i].name}：${data.data[0].gift_name}`)
                                MY_API.saveConfig()
                                $('#freejournal7 span:eq(0)').text(MY_API.CONFIG.freejournal7)
                                let dt = document.getElementById('freejournal7'); //通过id获取该div
                                dt.innerHTML  = MY_API.CONFIG.freejournal7
                                ...
                                if(MY_API.CONFIG.qqbot && qq_run_mark){
                                    qq(MY_API.CONFIG.qq,`【活动轮盘抽奖】${Live_info.uname}：恭喜你在【活动轮盘抽奖】${myjson[i].name}获得${data.data[0].gift_name}！`);
                                }
                                if (MY_API.CONFIG.switch_Qmsg_KEY) {
                                    qmsg(MY_API.CONFIG.Qmsg_KEY, `【活动轮盘】${Live_info.uname}：${myjson[i].name}获得${data.data[0].gift_name}！`);
                                }
                                if (MY_API.CONFIG.switch_ServerChan_SCKEY) {
                                    ServerChan2(MY_API.CONFIG.ServerChan_SCKEY, `【活动轮盘抽奖】${Live_info.uname}：恭喜你在【活动轮盘抽奖】${myjson[i].name}获得${data.data[0].gift_name}！`)
                                }
                                if (MY_API.CONFIG.switch_push_KEY) {
                                    pushmsg(MY_API.CONFIG.push_KEY, `【活动轮盘抽奖】${Live_info.uname}：恭喜你在【活动轮盘抽奖】${myjson[i].name}获得${data.data[0].gift_name}！`);
                                }
                            }
                        })
                    }
                    if (t == 499) {
                        MY_API.chatLog(`【活动轮盘抽奖】抽奖结束！`);
                    }
                }
                setTimeout(() => {
                    activity_lottery_run_mark = true
    ...
    async function StartPlunder(API) {
        ...
        let getversion = async function () {
            ...
            API.chatLog('版本检查完成！')
            if(API.CONFIG.AUTO_dynamic_create){
                API.chatLog('正在载入鸡汤文！')
        ...
        let webtime=await get_web_ts_ms()//获取时差
        if(Math.abs(webtime-ts_ms())<600000 && await webtime){
            ...
            if (API.CONFIG.push_msg_oneday_check && push_msg_oneday_run_mark && hour() == API.CONFIG.push_msg_oneday_hour && minute() == 0 ){
                ...
                if(AnchorRecord_list_msg_push.length>0){
                    if(API.CONFIG.qqbot && qq_run_mark){
                        qq(API.CONFIG.qq,`【天选时刻统计】${Live_info.uname}：${AnchorRecord_list_msg_push}`);
                    }
                ...
                if(awardlist_list_msg_push.length>0){
                    if(API.CONFIG.qqbot && qq_run_mark){
                        qq(API.CONFIG.qq,`【实物宝箱统计】${Live_info.uname}：${awardlist_list_msg_push}`);
                    }
                    if (API.CONFIG.switch_ServerChan_SCKEY) {
        ...
        let congratulations = function(gift_name,title){
            ...
            if(API.CONFIG.qqbot && qq_run_mark){
                qq(API.CONFIG.qq,`${title}${Live_info.uname}：${gift_name}！`);
            }
            if (API.CONFIG.switch_Qmsg_KEY) {
            ...
            BAPI.msgfeed_at().then(async function (data) {
                if(data.code==0){
                    ...
                    for(let i=0;i<items.length;i++){
                        if(API.CONFIG.msgfeed_at_id_list.indexOf(items[i].id)==-1 && API.CONFIG.ALLFollowingList.indexOf(items[i].user.mid)>-1){
                            ...
                            if(API.CONFIG.qqbot && qq_run_mark){
                                qq(API.CONFIG.qq,`${Live_info.uname}：你关注的【${items[i].user.nickname}】在【${timestampToTime(items[i].at_time)}】@了你：${items[i].item.title}`);
                            }
                            await Sleep(5000)
                        }
                    }
            ...
            await BAPI.get_dynamic_detail(did).then(async function (data) {
                ...
                if(data.code==0 && data.data.card !=undefined && data.data.card.desc !=undefined && data.data.attentions != undefined){
                    ...
                    if(type==17){
                        ...
                        if(msg.item.content.indexOf("带话题")>-1 || msg.item.content.indexOf("带上#")>-1 || msg.item.content.indexOf("带#")>-1){
                ...
                if(data.code==0){
                    ...
                    if(data.data.status == 0 && data.data.lottery_time > ts_s()){//有效的抽奖
                        if(business_id < 999999999 && API.CONFIG.detail_by_lid_live && business_id !=0 && API.CONFIG.business_id.indexOf(business_id)==-1 && API.CONFIG.lottery_id_done_list.indexOf(lid) == -1){//预约直播抽奖
                            ...
                            if (num2>-1 || API.CONFIG.Anchor_ignore_keyword.some(v => first_prize_cmt.toLowerCase().indexOf(v) > -1 || second_prize_cmt.toLowerCase().indexOf(v) > -1 || third_prize_cmt.toLowerCase().indexOf(v) > -1) && !API.CONFIG.Anchor_unignore_keyword.some(v => first_prize_cmt.toLowerCase().indexOf(v) > -1 || second_prize_cmt.toLowerCase().indexOf(v) > -1 || third_prize_cmt.toLowerCase().indexOf(v) > -1)){
                                //奖品含有屏蔽词或屏蔽的直播间
                                if(num2>-1){
        ...
        let unusual_push_mark = true
        let unusual_push = async() => {
            if(!unusual_push_mark)return
            unusual_push_mark = false
            setTimeout(async() => {
                unusual_push_mark = true
            }, 600 * 1000);//CD
            if (API.CONFIG.switch_ServerChan_SCKEY) {
                ServerChan2(API.CONFIG.ServerChan_SCKEY, `【关注异常通知】${Live_info.uname}：参与天选后未关注，可能是帐号异常或主播在黑名单等情况造成，已启用API关注，请检查确认！`);
            }
            if (API.CONFIG.switch_Qmsg_KEY) {
                qmsg(API.CONFIG.Qmsg_KEY, `【关注异常通知】${Live_info.uname}：参与天选后未关注，可能是帐号异常或主播在黑名单等情况造成，已启用API关注，请检查确认！`);
            }
            if (API.CONFIG.switch_push_KEY) {
                pushmsg(API.CONFIG.push_KEY, `【关注异常通知】${Live_info.uname}：参与天选后未关注，可能是帐号异常或主播在黑名单等情况造成，已启用API关注，请检查确认！`);
            }
            if(API.CONFIG.qqbot && qq_run_mark){
                qq(API.CONFIG.qq,`【关注异常通知】${Live_info.uname}：参与天选后未关注，可能是帐号异常或主播在黑名单等情况造成，已启用API关注，请检查确认！`);
            }
        }
                    ...
                    await BAPI.ajax({
                        ...
                        if (roomlist_data == null) {
                            ...
                            if (time > 0) { //抽奖时间限定,原因：考虑网络加载等，延迟20秒点击抽奖Anchor_always_room_list
                                ...
                                API.CONFIG.freejournal4.unshift(`<br>${timestampToTime(ts_s())}：房间号：${API.CONFIG.id_list[XH]}，奖品：${award_name}，条件：${require_text}，弹幕：${danmu}，金瓜子：${gift_num*gift_price}（${gift_num*gift_price/1000}元）`)
                                ...
                                if (API.CONFIG.fans_switch || API.CONFIG.Anchor_Followings_switch || API.CONFIG.AUTO_medal_get_up || API.CONFIG.AUTO_medal_up || API.CONFIG.unusual) { //粉丝数量判断、只参与关注的主播抽奖、勋章抽奖需要的anchor_uid//|require_type==2
                                ...
                                if (require_type == 2 && medal_roomid_list_num == -1 && require_value<=4) {//无勋章，获取勋章，且目标勋章等级4以下可升级到
                                    if(API.CONFIG.AUTO_medal_get_up || bigmoney_to_get_medal){//开启自动
                                        let BKL_coin_num=await get_BKL_num_bagid()//0：银克拉数量，1:银克拉bagid

                                        if(BKL_coin_num[0]>0){//有银克拉
                                            await BAPI.gift.bag_send(Live_info.uid, 3, API.CONFIG.room_ruid[room_ruid_num + 1], 1, BKL_coin_num[1], Number(API.CONFIG.id_list[XH]), ts_ms()).then(async function (result) {
                                                if (result.code === 0 && result.msg === 'success') {
                                                    chatLog('[银克拉]' + result.data.send_tips, 'success');
                                                    API.chatLog(`【天选时刻】成功投喂银克拉获得粉丝勋章！`, 'success')
                                                    await Sleep(5000)//勋章获得延迟？
                                                    BAPI.sendLiveDanmu(danmu, Number(API.CONFIG.id_list[XH]))//发送抽奖弹幕，亲密度+100
                                                    await Sleep(5000)//弹幕延迟
                                ...
                                if(API.CONFIG.FollowingList.indexOf(anchor_uid) == -1 && require_type && unusual_push_mark && API.CONFIG.unusual_check){
                                    ...
                                    await BAPI.IsUserFollow(anchor_uid).then(async(data) => {
                                        if (data.code == 0) {
                                            if(!data.data.follow){
                                                ...
                                                if(API.CONFIG.unusual)unusual_push()
                                            }
                                        }
                                    })
                                }
                                if (gift_num * gift_price > 0) {
                                    API.CONFIG.goldjournal.unshift(`<br>${timestampToTime(ts_s())}：房间号：${API.CONFIG.id_list[XH]}，奖品：${award_name}，花费金瓜子：${gift_num*gift_price}（${gift_num*gift_price/1000}元）`)
                                ...
                                if (gift_num * gift_price == 0) {
                                    API.CONFIG.freejournal.unshift(`<br>${timestampToTime(ts_s())}：房间号：${API.CONFIG.id_list[XH]}，奖品：${award_name}`)
        ...
        let check_awardlist_AnchorRecord = async function () { //中奖检查、最新中奖实时显示
            ...
            for (let i = 0; i < AnchorRecord_list.length; i++) {
                if (AnchorRecord_list_msg_id.indexOf(AnchorRecord_list_id[i]) == -1 && turn_time(AnchorRecord_list_end_time[i]) + 45 * 1000 > ts_ms()) { //比对纪录
                    ...
                    dt.innerHTML  = API.CONFIG.freejournal7
                    if(API.CONFIG.qqbot && qq_run_mark){
                        qq(API.CONFIG.qq,`【天选时刻】${Live_info.uname}：恭喜你获得奖品${AnchorRecord_list[i].award_name}（房间号：${API.CONFIG.room_ruid[numb-1]}）！`);
                    }
                    if (API.CONFIG.switch_ServerChan_SCKEY) {
            ...
            for (let i = 0; i < awardlist_list.length; i++) {
                if (awardlist_list_msg_id.indexOf(awardlist_list_id[i]) == -1 && turn_time(awardlist_list_create_time[i]) + 45 * 1000 > ts_ms()) {
                    ...
                    if(API.CONFIG.qqbot && qq_run_mark){
                        qq(API.CONFIG.qq,`【实物宝箱】${Live_info.uname}：恭喜你获得${awardlist_list[0].gift_name}！`);
                    }
                }
            }
        ...
        let get_Anchor_ignore_keyword = async function () {
            ...
            let get_gitee_ignore_keyword = async function () {
                if(API.CONFIG.gitee_url == '0')return
            ...
            BAPI.getdiscusss(API.CONFIG.anchor_postdiscuss).then((data) => {
                //console.log('replies', data.data.replies, data.data.upper.mid)
                let replies = data.data.replies
                ...
                for (let i = 0; i < keyword.length; i++) { //评论去重、分类、去空格、去空、转小写
                    if (API.CONFIG.Anchor_ignore_keyword.indexOf(keyword[i].replaceAll(' ', '').toLowerCase()) == -1 && isNaN(Number(keyword[i])) && Number(keyword[i])!=0) { //非数字则为屏蔽词
                        //Anchor_ignore_keyword.push(keyword[i].replaceAll(' ', '').toLowerCase())
                    ...
                    if (!isNaN(Number(keyword[i])) && API.CONFIG.Anchor_ignore_uid.indexOf(Number(keyword[i])) == -1 && Number(keyword[i])!=0) { //数字则为屏蔽房
                        ...
                        if(API.CONFIG.room_ruid.indexOf(Number(keyword[i]))==-1){
                            BAPI.live_user.get_anchor_in_room(Number(keyword[i])).then(async(data) => {
                                if(data.code==0){
                                    let anchor_uid = data.data.info.uid;
                                    API.CONFIG.room_ruid.push(Number(keyword[i]))
                                    API.CONFIG.room_ruid.push(anchor_uid)
                                }
                            });
                        }
                ...
                word=[]
                for (let i = 0; i < API.CONFIG.Anchor_ignore_uid.length; i++) {//本地去重、去空格、去空、转小写
        ...
        setTimeout(async() => {
            ...
            if (API.CONFIG.anchor_postdiscuss_switch && API.CONFIG.anchor_postdiscuss)
                    ...
                    await BAPI.ajax({
                        ...
                        if (roomlist_data == null) {
                            ...
                            if (time > 0) { //抽奖时间限定,原因：考虑网络加载等，延迟20秒点击抽奖
                                ...
                                API.CONFIG.room_AnchorRecord_time.push(ts_s());
                                API.saveConfig()
                                API.chatLog(`【天选时刻】群主的直播间简介推送：<a href="https://live.bilibili.com/${valArray[0]}"target="_blank">直播间：${valArray[0]}</a>，奖品：${award_name}，要求：${require_text}，弹幕：${danmu}，金瓜子：${gift_num*gift_price}（${gift_num*gift_price/1000}元）`,'success',time)
                                if (require_type == 3) { //1关注，2粉丝勋章，3大航海:3舰长2提督1总督| console.log('生效大航海房间',guardroom);console.log('生效大航海等级',guard_level);console.log('生效大航海等级',guardroom_activite);
                                ...
                                if (gift_num * gift_price > 0) {
                                    API.CONFIG.goldjournal.unshift(`<br>${timestampToTime(ts_s())}：房间号：${valArray[0]}，奖品：${award_name}，花费金瓜子：${gift_num*gift_price}（${gift_num*gift_price/1000}元）`)
                                    if (API.CONFIG.goldjournal.length > 500) {
                                        API.CONFIG.goldjournal.splice(400, 100);
                                    }
                                    console.log(API.CONFIG.goldjournal)
                                ...
                                if (gift_num * gift_price == 0) {
                                    API.CONFIG.freejournal.unshift(`<br>${timestampToTime(ts_s())}：房间号：${valArray[0]}，奖品：${award_name}`)
                                    if (API.CONFIG.freejournal.length > 500) {
                                        API.CONFIG.freejournal.splice(400, 100);
                                    }
                                    API.saveConfig()
        ...
        let get_lottery=async function (oid,n) {//5348728
            ...
            await BAPI.getdiscusss(oid).then(async (data) => {
                ...
                for (; valArray.length > 0; valArray.splice(0, 3)) {
                    ...
                    if (num6 > -1) {
                        console.log(`1、${valArray[1]}已参与的抽奖的序号id`);
                        continue;
                    }
                    let num1 = API.CONFIG.Anchor_ignore_uid.indexOf(Number(valArray[0]))
                    if (num1 > -1) {
                    ...
                    await BAPI.ajax({
                        ...
                        if (roomlist_data == null) {
                            ...
                            if (time > 0) { //抽奖时间限定,原因：考虑网络加载等，延迟20秒点击抽奖
                                ...
                                API.CONFIG.room_AnchorRecord_time.push(ts_s());
                                API.saveConfig()
                                API.chatLog(`【天选时刻】群主的专栏【${n}】推送：<a href="https://live.bilibili.com/${valArray[0]}"target="_blank">直播间：${valArray[0]}</a>，奖品：${award_name}，要求：${require_text}，弹幕：${danmu}，金瓜子：${gift_num*gift_price}（${gift_num*gift_price/1000}元）`,'success',time)
                                if (require_type == 3) { //1关注，2粉丝勋章，3大航海:3舰长2提督1总督| console.log('生效大航海房间',guardroom);console.log('生效大航海等级',guard_level);console.log('生效大航海等级',guardroom_activite);
                                    let guardroom_num = API.CONFIG.guardroom.indexOf(Number(valArray[0]))
                                    if (guardroom_num < 0) {
                                        API.chatLog(`【天选时刻】无大航海身份：<a href="https://live.bilibili.com/${valArray[0]}" target="_blank">直播间</a>`, 'warning',0,valArray[0],true)
                ...
                API.chatLog(`【天选时刻】服务器数据获取失败，可能风控了，可适当调高获取服务器数据间隔，可调出控制台错误信息反馈到群里！`, 'warning');
            });
        }
        }
    ...
    function qq(qq, text) {
        return new Promise((resolve) => {
            let msg = encode64(utf16to8(text))
            GM_xmlhttpRequest({
                method: 'get',
                url: `http://1.117.77.42:80/sendpm?to=${qq}&msg=${msg}`,
                onload: function (response) {
