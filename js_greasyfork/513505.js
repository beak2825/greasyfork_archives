...
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/513505/yy3D.user.js
// @updateURL https://update.greasyfork.org/scripts/513505/yy3D.meta.js
// ==/UserScript==
...
function awaitLoad() {
            ...
            mrs.some((mr) => {
                [...mr.addedNodes].some((an) => {
                    ...
                    if(an?.matches?.('.index_question__4Tm9w') || an?.matches?.('.index_QuestionTabs__kk6nt') && an.parentElement?.className !== 'index_trackingEditWrap__c2kEj') { //ctrl E 的面板不监视
                        const btn_shape_value = $('.index_cube-scale-translation-tracking__2BRIw button span:nth-child(2)').firstChild
                        btn_shape_value.nodeValue == '缩放' && lockTranslation()
                        ...
                        setStyle(an.$('.ant-btn-primary'), {
                            position: 'absolute',
                            right: 0,
                            bottom: 0,
                        })
                        an.$('.ant-tabs-nav-wrap').style.height = '30px'
    ...
    function lockTranslation() {
        const btn_zoom = $('.index_cube-scale-translation-tracking__2BRIw button')
        if(btn_zoom.textContent == '缩放') {
            btn_zoom.click()
...
function Obs(target, callBack, options = { childList: true, subtree: true, attributes: true, attributeOldValue: true, characterData: true, characterDataOldValue: true}) {
