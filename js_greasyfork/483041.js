...
// @name         115备份小工具
// @name:zh      115备份小工具
// @description  改自115转存助手ui优化版3.5，先感谢一下作者，其次我太懒了，不想写新的
// @author       115211
// @namespace    null
// @version      3.0.2
// @match        https://115.com/*
...
// @require      https://cdn.jsdelivr.net/npm/underscore@1.13.6/underscore-min.js
// @require      https://cdn.jsdelivr.net/npm/sweetalert2@11.3.0
// @require      https://cdn.jsdelivr.net/npm/node-forge@0.10.0/dist/forge.min.js
// @require      https://cdn.jsdelivr.net/npm/emoutils@2.0.0/dist/umd/emoutils.min.js
// @require      https://cdn.jsdelivr.net/npm/blueimp-md5@2.19.0/js/md5.min.js
// @require      https://cdn.jsdelivr.net/npm/big-integer@1.6.51/BigInteger.min.js
// @downloadURL https://update.greasyfork.org/scripts/483041/115%E5%A4%87%E4%BB%BD%E5%B0%8F%E5%B7%A5%E5%85%B7.user.js
// @updateURL https://update.greasyfork.org/scripts/483041/115%E5%A4%87%E4%BB%BD%E5%B0%8F%E5%B7%A5%E5%85%B7.meta.js
// ==/UserScript==
...
(function(Y) {
  function C(c, a, b) {
    ...
    g = b.encoding || 'UTF8';
    t = b.numRounds || 1;
    if (t !== parseInt(t, 10) || 1 > t) throw Error(
    ...
    if ('SHA-1' === c) m = 512, q = K, u = Z, f = 160, r = function(a) {
      ...
      if (q = function(a, b) {
        ...
        var k, f;
        if ('SHA-224' === c || 'SHA-256' === c) k = (b + 65 >>> 9 << 4) +
            15, f = 16;
    ...
    else if (0 === c.lastIndexOf('SHA3-', 0) || 0 ===
        c.lastIndexOf('SHAKE', 0)) {
      var F = 6;
      q = D;
      r = function(a) {
        var c = [],
      ...
      u = function(a, c, e, b, h) {
        ...
        var k = F,
            ...
            l = 0,
            d = c >>> 5;
        for (f = 0; f < d && c >= e; f += n) b = D(a.slice(f, f + n),
            b), c -= e;
        a = a.slice(f);
    ...
    this.setHMACKey = function(a, b, h) {
      ...
      for (a = 0; a <= h; a += 1) v[a] = b[a] ^ 909522486, w[a] = b[a] ^
          1549556828;
      l = q(v, l);
      e = m;
      I = !0;
    ...
    this.update = function(a) {
      ...
      b = c.value;
      c = a >>> 5;
      for (k = 0; k < c; k += g) f + m <= a &&
      (l = q(b.slice(k, k + g), l), f += m);
      e += f;
      h = b.slice(f >>> 5);
      n = a % m;
      y = !0;
    ...
    this.getHash = function(a, b) {
      ...
      for (g = 1; g < t; g += 1) !0 === z && 0 !== f % 32 &&
      (m[m.length - 1] &= 16777215 >>> 24 - f % 32), m = u(m, f,
          0, A(c), f);
      return k(m);
    };
  ...
  function O(c, a, b, e) {
    ...
    var n, g, d;
    d = -1 === b ? 3 : 0;
    for (n = 0; n < a; n += 1) g = c[n >>> 2] >>> 8 *
        (d + n % 4 * b), h += '0123456789abcdef'.charAt(g >>> 4 & 15) +
  ...
  function P(c, a, b, e) {
    ...
    f = -1 === b ? 3 : 0;
    for (g = 0; g < n; g += 3)
      for (d = g + 1 < n ? c[g + 1 >>> 2] : 0, p = g + 2 < n ?
          c[g + 2 >>> 2] :
          0, p = (c[g >>> 2] >>> 8 * (f + g % 4 * b) & 255) << 16 |
          (d >>> 8 * (f + (g + 1) % 4 * b) & 255) << 8 | p >>> 8 * (f +
              (g + 2) % 4 * b) & 255, d = 0; 4 > d; d += 1) 8 * g + 6 * d <= a ?
  ...
  function Q(c, a, b) {
    ...
    var h, d, g;
    g = -1 === b ? 3 : 0;
    for (h = 0; h < a; h += 1) d = c[h >>> 2] >>> 8 * (g + h % 4 * b) &
        255, e += String.fromCharCode(d);
    return e;
  }
  ...
  function N(c) {
    ...
    if ('boolean' !== typeof a.outputUpper) throw Error(
        'Invalid outputUpper formatting option');
    if ('string' !== typeof a.b64Pad) throw Error(
    ...
    switch (c) {
        ...
        c = function(c, h, d) {
          ...
          if ('UTF8' === a)
            ...
            b ? 3 : 0, f = 0; f < c.length; f += 1)
              for (g = c.charCodeAt(f), l = [], 128 > g ?
                  l.push(g) :
                  2048 > g ?
                      (l.push(192 | g >>> 6), l.push(128 | g & 63)) :
                      55296 > g || 57344 <= g ?
                          l.push(224 | g >>> 12, 128 | g >>> 6 & 63,
                              128 | g & 63) :
                          (f += 1, g = 65536 +
                              ((g & 1023) << 10 | c.charCodeAt(f) &
                                  1023), l.push(240 | g >>> 18,
                              128 | g >>> 12 & 63, 128 | g >>> 6 & 63,
                              128 | g & 63)), m = 0; m < l.length; m += 1) {
                r = p + q;
                for (u = r >>> 2; h.length <= u;) h.push(0);
                h[u] |= l[m] << 8 * (t + r % 4 * b);
                p += 1;
              } else if ('UTF16BE' === a || 'UTF16LE' === a)
            for (t = -1 === b ? 2 : 0, l = 'UTF16LE' === a && 1 !== b ||
                'UTF16LE' !== a && 1 === b, f = 0; f < c.length; f += 1) {
              g = c.charCodeAt(f);
      ...
      case 'B64':
        c = function(a, c, d) {
          var g = 0,
              l, p, f, m, q, u, r, t;
          if (-1 === a.search(/^[a-zA-Z0-9=+\/]+$/)) throw Error(
              'Invalid character in base-64 string');
          p = a.indexOf('=');
          a = a.replace(/\=/g, '');
          if (-1 !== p && p < a.length) throw Error(
          ...
          for (p = 0; p < a.length; p += 4) {
            q = a.substr(p, 4);
            for (f = m = 0; f <
            q.length; f += 1) l = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.indexOf(
                q[f]),
                m |= l << 18 - 6 * f;
      ...
      case 'BYTES':
        c = function(a, c, d) {
          ...
          p = d >>> 3;
          q = -1 === b ? 3 : 0;
          for (l = 0; l < a.length; l += 1) g = a.charCodeAt(l), m = l +
              p, f = m >>> 2, c.length <= f && c.push(0), c[f] |= g << 8 *
              (q + m % 4 * b);
          return {
            value: c,
            binLen: 8 * a.length + d,
      ...
      case 'ARRAYBUFFER':
        ...
        c =
            function(a, c, d) {
              ...
              m = -1 === b ? 3 : 0;
              q = new Uint8Array(a);
              for (g = 0; g < a.byteLength; g += 1) f = g + l, p = f >>>
                  2, c.length <= p && c.push(0), c[p] |= q[g] << 8 *
                  (m + f % 4 * b);
              return {
                value: c,
  ...
  function S(c, a) {
    return 32 < a ?
        (a -= 32, new b(c.b << a | c.a >>> 32 - a, c.a << a | c.b >>> 32 - a)) :
        0 !== a ?
            new b(c.a << a | c.b >>> 32 - a, c.b << a | c.a >>> 32 - a) :
            c;
  }
  ...
  function t(c, a) {
    var k = null,
        k = new b(c.a, c.b);
    return k = 32 >= a ?
        new b(k.a >>> a | k.b << 32 - a & 4294967295,
            k.b >>> a | k.a << 32 - a & 4294967295) :
        new b(k.b >>> a - 32 | k.a << 64 - a & 4294967295,
  ...
  function la(c, a, b, e) {
    ...
    return ((c >>> 16) + (a >>> 16) + (b >>> 16) + (e >>> 16) + (h >>> 16) &
        65535) << 16 | h & 65535;
  }
  ...
  function na(c, a, d, e) {
    ...
    h = (c.a & 65535) + (a.a & 65535) + (d.a & 65535) + (e.a & 65535) +
        (n >>> 16);
  ...
  function oa(c, a, d, e, h) {
    ...
    g = (c.b >>> 16) + (a.b >>> 16) + (d.b >>> 16) + (e.b >>> 16) +
        (h.b >>> 16) + (n >>> 16);
    l = (g & 65535) << 16 | n & 65535;
    n = (c.a & 65535) + (a.a & 65535) + (d.a & 65535) + (e.a & 65535) +
        (h.a & 65535) + (g >>> 16);
    g = (c.a >>> 16) + (a.a >>> 16) + (d.a >>> 16) + (e.a >>> 16) +
        (h.a >>> 16) + (n >>> 16);
  ...
  function A(c) {
    ...
    if ('SHA-1' === c) a = [
      ...
      271733878,
      3285377520];
    else if (0 === c.lastIndexOf('SHA-', 0)) switch (a = [
      3238371032,
      914150663,
      812702999,
      4144912697,
  ...
  function K(c, a) {
    ...
    3] ^ b[f - 8] ^ b[f - 14] ^ b[f - 16], 1), p = 20 > f ?
        H(y(e, 5), d & n ^ ~d & g, l, 1518500249, b[f]) :
        40 > f ?
            H(y(e, 5), d ^ n ^ g, l, 1859775393, b[f]) :
            60 > f ?
                H(y(e, 5), U(d, n, g), l, 2400959708, b[f]) :
                H(y(e, 5), d ^ n ^ g, l, 3395469782, b[f]), l = g, g = n, n = y(
        d, 30), d = e, e = p;
    a[0] = G(e, a[0]);
    a[1] = G(d, a[1]);
    a[2] = G(n, a[2]);
  ...
  function L(c,
    ...
    var e, h, n, g, l, p, f, m, q, u, r, t, v, w, y, A, z, x, F, B, C, D,
        E = [],
        J;
    if ('SHA-224' === k || 'SHA-256' ===
        k) u = 64, t = 1, D = Number, v = G, w = la, y = H, A = ha, z = ja, x = da, F = fa, C = U, B = aa, J = d;
    else if ('SHA-384' === k || 'SHA-512' ===
        k) u = 80, t = 2, D = b, v = ma, w = na, y = oa, A = ia, z = ka, x = ea, F = ga, C = ca, B = ba, J = V;
    else throw Error('Unexpected error in SHA-2 implementation');
    k = a[0];
    e = a[1];
    h = a[2];
    ...
    p = a[6];
    f = a[7];
    for (r = 0; r < u; r += 1) 16 > r ?
        (q = r * t, m = c.length <= q ? 0 : c[q], q = c.length <= q + 1 ?
            0 :
            c[q + 1], E[r] = new D(m, q)) :
        E[r] = w(z(E[r - 2]), E[r - 7], A(E[r - 15]), E[r -
        16]), m = y(f, F(g), B(g, l, p), J[r], E[r]), q = v(x(k),
        C(k, e, h)), f = p, p = l, l = g, g = v(n,
        m), n = h, h = e, e = k, k = v(m, q);
    a[0] = v(k, a[0]);
    a[1] = v(e, a[1]);
    a[2] = v(h, a[2]);
    a[3] = v(n, a[3]);
  ...
  function D(c, a) {
    var d, e, h, n, g = [],
        l = [];
    if (null !== c)
      for (e = 0; e < c.length; e += 2) a[(e >>> 1) % 5][(e >>> 1) / 5 | 0] = B(
          a[(e >>> 1) % 5][(e >>> 1) / 5 | 0], new b(c[e + 1], c[e]));
    for (d = 0; 24 > d; d += 1) {
      n = A('SHA3-');
      for (e = 0; 5 > e; e += 1) {
        h = a[e][0];
      ...
      for (e = 0; 5 > e; e += 1)
        for (h = 0; 5 > h; h += 1) n[h][(2 * e + 3 * h) % 5] = S(a[e][h],
            W[e][h]);
      for (e = 0; 5 > e; e += 1)
        for (h = 0; 5 > h; h += 1) a[e][h] = B(n[e][h],
            new b(~n[(e + 1) % 5][h].a & n[(e + 2) % 5][h].a,
  ...
  V = [
    new b(d[0], 3609767458),
    new b(d[1], 602891725),
    new b(d[2], 3964484399),
    new b(d[3], 2173295548),
    new b(d[4], 4081628472),
    new b(d[5], 3053834265),
    new b(d[6], 2937671579),
    new b(d[7], 3664609560),
    new b(d[8], 2734883394),
    new b(d[9], 1164996542),
    new b(d[10], 1323610764),
    new b(d[11], 3590304994),
    new b(d[12], 4068182383),
    new b(d[13], 991336113),
    new b(d[14], 633803317),
    new b(d[15], 3479774868),
    new b(d[16], 2666613458),
    new b(d[17], 944711139),
    new b(d[18], 2341262773),
    new b(d[19], 2007800933),
    new b(d[20], 1495990901),
    new b(d[21], 1856431235),
    new b(d[22], 3175218132),
    new b(d[23], 2198950837),
    new b(d[24], 3999719339),
    new b(d[25], 766784016),
    new b(d[26], 2566594879),
    new b(d[27], 3203337956),
    new b(d[28], 1034457026),
    new b(d[29], 2466948901),
    new b(d[30], 3758326383),
    new b(d[31], 168717936),
    new b(d[32], 1188179964),
    new b(d[33], 1546045734),
    new b(d[34], 1522805485),
    new b(d[35], 2643833823),
    new b(d[36], 2343527390),
    new b(d[37], 1014477480),
    new b(d[38], 1206759142),
    new b(d[39], 344077627),
    new b(d[40], 1290863460),
    new b(d[41], 3158454273),
    new b(d[42], 3505952657),
    new b(d[43], 106217008),
    new b(d[44], 3606008344),
    new b(d[45], 1432725776),
    new b(d[46], 1467031594),
    new b(d[47], 851169720),
    new b(d[48], 3100823752),
    new b(d[49], 1363258195),
    new b(d[50], 3750685593),
    new b(d[51], 3785050280),
    new b(d[52], 3318307427),
    new b(d[53], 3812723403),
    new b(d[54], 2003034995),
    new b(d[55], 3602036899),
    new b(d[56], 1575990012),
    new b(d[57], 1125592928),
    new b(d[58], 2716904306),
    new b(d[59], 442776044),
    new b(d[60], 593698344),
    new b(d[61],
        3733110249),
    new b(d[62], 2999351573),
    new b(d[63], 3815920427),
  ...
  'function' === typeof define && define.amd ?
      ...
      'undefined' !== typeof exports ?
          ...
          (module.exports = C), exports = C) :
          Y.jsSHA = C;
})(this);

var CreateDownloadTask, CreateDownloadTask_, browserInterface, bytesToString,
    cloneInto, g_key_l, g_key_s, g_kts, m115_asym_decode, m115_asym_encode,
    m115_decode, m115_encode, m115_getkey, m115_sym_decode, m115_sym_encode,
    ref, stringToBytes, xor115_enc;
...
m115_getkey = function(length, key) {
  ...
  if (key != null) {
    return (function() {
      var j, ref, results;
      results = [];
      for (i = j = 0, ref = length; (0 <= ref ? j < ref : j > ref); i = 0 <=
      ref ? ++j : --j) {
        results.push(((key[i] + g_kts[length * i]) & 0xff) ^
...
xor115_enc = function(src, srclen, key, keylen) {
  ...
  ret = [];
  if (mod4 !== 0) {
    for (i = j = 0, ref = mod4; (0 <= ref ? j < ref : j > ref); i = 0 <= ref ?
        ++j :
        --j) {
      ret.push(src[i] ^ key[i % keylen]);
    }
  }
  for (i = k = ref1 = mod4, ref2 = srclen; (ref1 <= ref2 ?
      k < ref2 :
      k > ref2); i = ref1 <= ref2 ? ++k : --k) {
...
stringToBytes = function(s) {
  var i, j, ref, ret;
  ret = [];
  for (i = j = 0, ref = s.length; (0 <= ref ? j < ref : j > ref); i = 0 <= ref ?
      ++j :
      --j) {
...
m115_asym_encode = function(src, srclen) {
  ...
  m = 128 - 11;
  ret = '';
  for (i = j = 0, ref = Math.floor((srclen + m - 1) / m); (0 <= ref ?
      j < ref :
      j > ref); i = 0 <= ref ? ++j : --j) {
    ret += new_rsa.encrypt(
...
GM_configStruct.prototype = {
  ...
  open: function() {
    ...
    if (match &&
        (match.tagName == 'IFRAME' || match.childNodes.length > 0)) return;
    ...
    function buildConfigWin(body, head) {
      ...
      for (var id in fields) {
        ...
        if (settings.section) { // the start of a new section
          ...
          if (Object.prototype.toString.call(settings.section) !==
              '[object Array]')
  ...
  center: function() {
    ...
    style.display = '';
    style.top = Math.floor((window.innerHeight / 2) - (node.offsetHeight / 2)) +
        'px';
    style.left = Math.floor((window.innerWidth / 2) - (node.offsetWidth / 2)) +
        'px';
    style.opacity = '1';
  },
  ...
  toTabs: function() { // 转为 tab 的形式
    var body = this.frame.tagName == 'IFRAME' ?
  ...
  toggleTab: function(e) {
    ...
    [].forEach.call(body.querySelectorAll('.section_header'),
        function(header, i) {
          if (i == curTab) {
            header.classList.add('active');
          } else {
    ...
    [].forEach.call(body.querySelectorAll('.section_header_holder'),
        function(holder, i) {
...
GM_configField.prototype = {
  ...
  toNode: function(configId, lastParentNode) {
    ...
    function addLabel(pos, labelEl, parentNode, beforeEl) {
      if (!beforeEl) {
        beforeEl = lastParentNode ?
            parentNode.lastChild :
            parentNode.firstChild; // oneLine 的修正
      }
...
(function() {
      ...
      const TIPS = {
        ...
        VersionTips: "秒传工具获取",
        UpdateUrl: "https://115211.xyz/forum.php?mod=viewthread&tid=1",
    ...
    <a href="https://115211.xyz/forum.php?mod=viewthread&tid=1" target="_blank" class="button btn-line btn-upload"><i class="icon-operate ifo-linktask"></i><span>获取秒传工具</span><em style="display:none;" class="num-dot"></em></a>
    </div>
  </div>
    `;
      ...
      //缩略图模式下
      waitForKeyElements(`#js_cantain_box .list-thumb li[rel="item"]`,
          AddCeateSha1ButtonInGrid);
      //文件路径旁边的”选中获取项sha1“
      waitForKeyElements('div#js_top_header_file_path_box',
          CreateSha1ButtonForSelectedItems);
      //隐藏截图中的uid
      ...
      function RenewCookie() {
        ...
        arryCookie.forEach(function(kv) {
          document.cookie = kv +
              ';expires=Thu, 01 Jan 2100 00:00:00 UTC;;domain=.115.com';
        });
      }
      ...
      function DeleteCookie(resp) {
        try {
          ...
          var filecookie = setcookie[0].slice(11) +
              '; expires=Thu, 01 Jan 1970 00:00:00 UTC;' + setcookie[3] +
              ';domain=.115.com';
    ...
    <br><span class="my115Info">无</span>115会员，<span class="my115Info">提取速度</span>受限，<span class="my115Info">转存文件大小</span>不超过5GB！！</p>`;
      ...
      if (window.top === window.self) {
        $(function() {
          ...
          $(window).on('message', function(e) {
            var dataInfo = typeof e.originalEvent.data == 'string' ?
                JSON.parse(e.originalEvent.data) :
                e.originalEvent.data;
            if (dataInfo.eventID != '115sha1' || e.originalEvent.origin !=
                'https://115.com') return;
            var message = typeof dataInfo.data == 'string' ?
                JSON.parse(dataInfo.data) :
                dataInfo.data;
            ...
            if (message.messageType == MessageType.BEGIN) {
              ...
              $errorList.append('<li><div display: flex;"><p>' + message.msg +
                  '</p><p style="font-style: italic;"><\p><\div><\li><li><hr/></li>');
              ...
              Swal.getDenyButton().addEventListener('click', e => {
                ...
                console.log(message);
                window.location.href = 'https://115.com/?cid=' +
                    message.targetID +
                    '&offset=0&tab=&mode=wangpan';
              });
              Swal.getFooter().style.display = 'none';
              Swal.hideLoading();
              ...
              document.getElementById('continuedTaskFile').
              ...
              if (size > 2 * 1024 * 1024) {
                postSha1Messgae(createMessage(MessageType.MSGERROR,
                    '暂不支持大于2MB的 text|json 文件！'));
              } else {
                Swal.fire({
                  title: '正在下载文件...',
      ...
      async function getDirectChildItemsByOffset(cid, offset) {
        var tUrl = 'https://webapi.115.com/files?aid=1&cid=' + cid +
            '&o=file_name&asc=1&offset=' + offset +
            '&show_dir=1&limit=1150&code=&scid=&snap=0&natsort=1&record_open_time=1&source=&format=json&fc_mix=&type=&star=&is_share=&suffix=&custom_order=';
      ...
      async function getDirectChildItemsByOffsetlt1200(cid, offset) {
        ...
        var tUrl = 'https://aps.115.com/natsort/files.php?aid=1&cid=' + cid +
            '&o=file_name&asc=1&offset=' + offset +
            '&show_dir=1&limit=1150&code=&scid=&snap=0&natsort=1&record_open_time=1&source=&format=json&fc_mix=0&type=&star=&is_share=&suffix=&custom_order=';
        const result = await $.ajax({
          type: 'GET',
          url: tUrl,
          dataType: 'json',
      ...
      async function getAllDirectItems(cid, folderProcessCallback) {
        ...
        while (flag) {
          ...
          if (first) {
            ...
            if (isLT1200) result = await getDirectChildItemsByOffsetlt1200(cid,
                index);
          ...
          if (totalCount >= 1) {
            result.data.forEach(function(item) {
              ...
              var itemIndex = items.findIndex(
                  q => q.name == pItem.name && q.pickCode == pItem.pickCode &&
                      q.sha1 == pItem.sha1 &&
                      (_.isEqual(q.paths, pItem.paths)));
        ...
        console.log(
            'cid: {0}, 除去完全重复count: {1}'.format(cid, noNullItems.length));
      ...
      async function getAllFiles(
          cid, fileArray, topCid, folderProcessCallback) {
        ...
        files.forEach(f => {
          var index = thisFolder.paths.findIndex(
              q => q.file_id.toString() == topCid);
          var paths = new Array();
          if (index != -1) {
        ...
        for (var folder of folders) {
          if (getTaskCancelFlag()) break;
          await getAllFiles(folder.id, fileArray, topCid,
              folderProcessCallback);
          await delay(200);
        }
      ...
      async function renameFiles(fileArray) {
        ...
        let datas = fileArray.map((value, index, array) => {
          ...
          return `${encodeURIComponent(dataKey)}=${encodeURIComponent(
              dataValue)}`;
        }).join('&');
      ...
      async function getLastUploadFiles(
          count, delayTime = 200, processCallback) {
        let files = [];
        let offset = 0;
        let limit = 115;
        let leftCount = count;
        ...
        if (leftCount > 0) {
          processCallback(
              `正在获取第${offset + 1}-${offset + leftCount + 1}个文件数据...`);
                ...
                onload: function(r) {
                  if (r.status == 200) {
                    ...
                    if (download_info.state && download_info.data) {
                      ...
                      GM_xmlhttpRequest({
                        method: 'POST',
                        url: 'https://api.115211.xyz/api/seed',
                        headers: {
                          'Content-Type': 'application/x-www-form-urlencoded',
                          'User-Agent': 'Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 6.0)',
                        },
                        ...
                        onload: function(res) {
                          ...
                          if (resInfo.status == 2) {
                            ...
                            //   outputUpper: true
                            // });
                            fileItem.preid = "0EEE99A4ED0A1844DA8B6FEF7306067F6156E4B5";
                            resolve({
                              state: true,
                              error: "",
                              fileItem: fileItem
        ...
        return function(array) {
          ...
          for (var i = 0; i < buffLen;) {
            ...
            if (byte1 <= 0x7F) {
              ...
              codePt = ((byte1 & 0x0F) << 12) | ((array[i++] & 0x3F) << 6) |
                  (array[i++] & 0x3F);
                  ...
                  try {
                    ...
                    GM_xmlhttpRequest({
                      ...
                      onprogress: function(event) {
                        let textMsg = `${event.loaded} of ${event.total} bytes, ${(event.loaded /
                            event.total).toFixed(4) * 100}%`;
                        console.log(textMsg);
      ...
      function convertToSha1Link(fileItem, isSimpleFormat) {
        ...
        if (fileItem.name && fileItem.size && fileItem.sha1 && fileItem.preid) {
          var serverId = 50660199;
          var paths = '';
          //生成最终的115SHA1链接
          // var sha1Link = "115://" + repalceValidatedName(fileItem.name) + "|" + fileItem.size + "|" + fileItem.sha1 + "|" + fileItem.preid;
          var sha1Link = '211://' + repalceValidatedName(fileItem.name) + '|' +
              fileItem.size + '|' + fileItem.sha1 + '|' + serverId;
          if (!isSimpleFormat) {
            if (fileItem.paths.length > 0) {
              ...
              paths = fileItem.paths.map(t => repalceValidatedName(t)).
                  join('/');
              // msg = sha1Link + '|' + paths;
            }
          }
          msg = sha1Link + '|' + paths + '|' + fileItem.pickCode;
          ;
          succeed = true;
        }
            ...
            onload: function(response) {
              ...
              if (response.status === 200 && response.response.status === 2) {
                ...
                if (response.status === 405) {
                  error = '频繁请求，被115限制 ([!]立即停止，尝试停止操作半小时或者重新登录)：' +
                      response.statusText;
                } else if (response.response &&
                    response.response.message) error = response.response.message;
                else if (response.response &&
                    response.response.statusmsg) error = '可能参数不正确(?)：' +
                    response.response.statusmsg;
                data.error = error;
              }
              resolve(data);
            },
      ...
      function uploadFile(targetFolder, fileItem, uploadInfo) {
        ...
        console.log();
        console.log(
            `${secret}${fileItem.sha1}${fileItem.size}${fileItem.preid}${uploadInfo.user_id}${ts}${md5(
                uploadInfo.user_id)}${appVersion}`);
        let uploadToken = md5(
            `${secret}${fileItem.sha1}${fileItem.size}${fileItem.preid}${uploadInfo.user_id}${ts}${md5(
                uploadInfo.user_id)}${appVersion}`);
        ...
        let urlData = UrlData({
          ...
          format: 'json',
          sig: GetSig(uploadInfo.user_id, fileItem.sha1, fCid,
              uploadInfo.userkey),
          t: ts,
          token: uploadToken,
        });
...
//         return x;
      ...
      async function updateParentID(
          cid, cname, thisLevel, maxLevel, items, sleepTime,
        ...
        let childFolderNames = files.map(q => q.paths[thisLevel]).
            filter(q => q).
        ...
        for (let folderName of childFolderNames) {
          ...
          let r = await createRootFolder(cid, folderName, 10, sleepTime,
              callbackMsg => {
          ...
          if (r.state) {
            await updateParentID(r.cid, folderName, thisLevel + 1, maxLevel,
                files, createFolderCallback);
          } else { //ui 目录创建失败
            //todo: ！！！尚未解决这个
        ...
        for (let line of textLines) {
          ...
          if (r.state) {
            ...
            if (nameSeparator) {
              ...
              let fileName = emojiUtils.toArray(nameStrings.join('.')).
                  map(c => c + nameSeparator).
                  join('').
                  slice(0, -1);
      ...
      async function createRootFolder(
          targetCid, folderName, retryTimes, sleepTime, processCallback) {
        let cid = targetCid;
        ...
        for (let i = 0; i < retryTimes; i++) {
          ...
          if (i != 0) {
            ...
            newFolderName = `${folderName == '' ?
                'auto_create' :
                folderName}@${getCurrentTimeString()}`;
          }
          ...
          if (tr.state) {
            ...
            processCallback && processCallback(
                `自动创建目录${newFolderName}失败！原因：${tr.error}，将自动尝试新的名字...`);
            if (tr.error.includes('云端目录不存在') ||
                tr.error.includes('文件不存在或已删除')) {
              cid = '-1'; //父目录不存在时的提示
              break;
            }
            await delay(sleepTime);
      ...
      async function processUpload(
          allFiles, workingNumber, sleepTime, resultCallback) {
        ...
        for (let file of fileArray) {
          ...
          let r = uploadFileWithTimeOut(8000, file.parentID, file, uploadInfo).
              then(t => {
      ...
      async function processRenameByUsingHistory(
          files, separator, sleepTime, resultCallback) {
        ...
        for (const file of files) {
          //console.log("file")
          //console.log(file)
          let thisFile = history.find(
              q => q.sha1 == file.sha1 && q.parentID == file.parentID &&
                  q.name ==
                  file.formatedName && !q.isMarked);
        ...
        let selectedFiles = files.filter(
            f => f.formatedName.search(separator) != -1 && f.id).map(f => {
          let fo = {
            id: f.id,
      ...
      async function processRename(
          targetFolderCid, separator, sleepTime, resultCallback) {
        let onlineFiles = new Array();
        await getAllFiles(targetFolderCid, onlineFiles, targetFolderCid,
            (fname, pIndex) => {
              if (pIndex > 1) {
                resultCallback && resultCallback({
                  state: true,
        ...
        let selectedFiles = onlineFiles.filter(
            f => f.name.search(separator) != -1).map(f => {
          let fo = {
            id: f.id,
      ...
      async function UploadFilesBySha1Links(
          config, continuedTaskSetting = null) {

        let uploadConfig = continuedTaskSetting == null ?
            config :
            continuedTaskSetting.uploadConfig;
        ...
        if (continuedTaskSetting == null) {
          ...
          postSha1Messgae(
              createMessage(MessageType.BEGIN4UPLOAD, '正在解析sha1链接...'));
          ...
          if (formatedText.startsWith('{') && formatedText.endsWith('}')) {
            ...
            if (r.state) {
              ...
              postSha1Messgae(
                  createMessage(MessageType.END4UPLOAD, 'json解析失败！是不是格式不匹配！'));
              return;
              //json 解析失败，提示，，
            }
          }
          ...
          files = parseSha1LinksToFileArray(formatedText, nameSeparator,
              errorMsg => {
          ...
          postSha1Messgae(
              createMessage(MessageType.PROCESSING, `获取到链接个数：${files.length}`));
          await delay(500);
          ...
          if (uploadConfig.folderSetting.rootFolder.needToCreate === true) {
            ...
            let root = await createRootFolder(newTargetCid, rootFolderName, 11,
                folderSleepTime * 2, msg => {
          ...
          if (newTargetCid == '-1') {
            ...
            postSha1Messgae(createMessage(MessageType.END,
                '自动创建根目录出错：                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   <br/>选择的保存处文件夹,已经被删除或者移动，请重新选择保存位置！'));
            return;
          }
          ...
          if (uploadConfig.folderSetting.notCreateAnyChildFolder === false) //可以创建目录
          {
            ...
            //cid更新
            postSha1Messgae(
                createMessage(MessageType.PROCESSING, `正在配置子目录的生成...`));
            await updateParentID(newTargetCid, '',
                level, maxLevel, files, folderSleepTime * 1.5, t => {
                  ...
                  postSha1Messgae(createMessage(MessageType.PROCESSING, msg));
                  if (!t.state) postSha1Messgae(
                      createMessage(MessageType.ERROR, msg));
                });
        ...
        //文件上传
        await processUpload(files, uploadConfig.upload.workingNumber,
            uploadConfig.upload.sleepTime, result => {
              if (result.state === true) {
                postSha1Messgae(
                    createMessage(MessageType.PROCESSING, result.msg));
              } else {
        ...
        if (uploadConfig.itemNameSeparator.needToSeparate &&
            ...
            !isTaskCanceled) {
          postSha1Messgae(
              createMessage(MessageType.PROCESSING, '开始获取文件，并自动重命名...'));
          await delay(folderSleepTime);
          console.log(files);
          await processRenameByUsingHistory(files,
              uploadConfig.itemNameSeparator.separator, folderSleepTime,
              result => {
                if (result.state === true) {
                  postSha1Messgae(
                      createMessage(MessageType.PROCESSING, result.msg));
          ...
          postSha1Messgae(
              createMessage(MessageType.PROCESSING, '文件批量去除分隔符（重命名）完成！'));
        ...
        let msg = `完成上传！成功 <b>${(files.length -
            fails.length)}</b> ，失败或者取消 <b>${fails.length}</b>\
        ...
        postSha1Messgae(
            createMessage(MessageType.END4UPLOAD, msg, newTargetCid));
      ...
      async function InnerCreateSha1Links(allFiles, txtName) {
        ...
        for (var file of files) {
          ...
          const r = getFileItemPreidWithTimeOut(20000, f).then((t) => {
            console.log(t)
            if (t.state) {
              msg = '<div align="right"><b>{0}</b> | <b>{1}</b></div><hr>获取【 <b>{2}</b> 】的sha1链接成功'.format(
                  completedIndex, allFiles.length, t.fileItem.name);
              postSha1Messgae(createMessage(MessageType.PROCESSING, msg));
            } else {
              msg = '<div align="right"><b>{0}</b> | <b>{1}</b></div><hr>获取【 <b>{2}</b> 】的sha1链接：{3}'.format(
                  completedIndex, allFiles.length, t.fileItem.name, t.error);
              postSha1Messgae(createMessage(MessageType.PROCESSING, msg));
              var filePath = t.fileItem.paths.join(' > ');
              console.log(filePath);
              if (filePath) msg = '{0},原因：{1},路径：{2}'.format(t.fileItem.name,
                  t.error, filePath);
          ...
          if (index % WORKSETTINGS.WorkingItemsNumber == 0) {
            ...
            if (index % (WORKSETTINGS.WorkingItemsNumber * 9) == 0) {
              ...
              let seconds = 2;
              for (let i = 0; i < seconds; i++) {
                postSha1Messgae(createMessage(MessageType.PROCESSING,
                    `防止115服务器限制，暂停发包中。<br><br>${seconds - i}秒后继续...`));
                await delay(1000);
              }
              promisArray = new Array();
            }
        ...
        if (succeedArray.length == 1) {
          ...
          msg = `
            ...
            总共<b>${allFiles.length}</b> ，取消或者失败 <b>${allFiles.length -
          succeedArray.length}</b>。<br>
      ...
      async function CreateSha1LinksAll(items, taskName) {
        ...
        postSha1Messgae(createMessage(MessageType.PROCESSING,
            `获取到 【<b>${taskName}</b>】 的内容 ${files.length} 项`));
        await delay(100);
        if (getTaskCancelFlag()) {
      ...
      async function CreateSha1Links(item) {
        ...
        if (!item.isFolder) {
          ...
          if (!files || files.length == 0) {
            postSha1Messgae(createMessage(MessageType.END,
                '【<b>{0}</b> 】空目录???'.format(item.name)));
            return;
          }
        }

        postSha1Messgae(createMessage(MessageType.PROCESSING,
            '获取到 【<b>{0}</b>】 的内容 {1} 项'.format(item.name, files.length)));
        await delay(100);
        if (getTaskCancelFlag()) {
          postSha1Messgae(createMessage(MessageType.END, '已经取消任务！'));
    ...
    <a href="javascript:;" style="color:#2777F8" id="neSetting2">分隔符等设置点此！</a></div>`;
      ...
      const beginUploadBySha1String = `<div class="con" id="downsha1"><a class="button" target="_blank" href="https://115211.xyz/forum.php?mod=viewthread&tid=1">开始sha1转存</a></div>`;
      ...
      function getCurrentFolderDisplayed() {
        ...
        for (let item of iframes) {
          let filePath = item.contentWindow.document.body.querySelector(
              '[rel=header_page_local]');
          if (filePath) {
      ...
      function AddDownloadSha1Btn(jNode) {
        ...
        document.querySelector(`a[btn="close"]`).
        ...
        var dialog = document.getElementsByClassName(
            'dialog-box dialog-mini offline-box window-current')[0];
        ...
        if (document.getElementById('neSelectFile') == null) {
          ...
          document.getElementById(
              'neAutoCreateRootfolder').checked = GM_config.get(
              currentConfig.createRootFolderDefaultValue);
          //是否显示不创建任何目录：
          document.getElementById(
              'neNotCreateAnyChildFolderParent').style.display = GM_config.get(
              currentConfig.createChildFolderVisible) === true ?
              'block' :
              'none';
        ...
        if (document.getElementById('downsha1') == null) {
          ...
          $btn[0].addEventListener('click', e => {
            ...
            let cid = $(`li[rel="bts_select_item"][class="selected"]`).
                attr('file_id');
            if (cid == '') {
              //目录不存在，比如把 “云下载” 目录删除
              cid = '0';
            ...
            let notCreateAnyChildFolder = document.getElementById(
                'neNotCreateAnyChildFolder').checked;
            let autoCreateRootfolder = document.getElementById(
                'neAutoCreateRootfolder').checked;
            ...
            let config = {
              ...
              itemNameSeparator: {
                ...
                needToRemoveSeparator: GM_config.get(
                    currentConfig.autoUseSeparatorToRename),
            ...
            if (onlineFile) {
              document.querySelector(`a[btn="close"]`).click();
              console.log('选择了在线文件：');
              config.folderSetting.rootFolder.folderName = onlineFile.name.split(
                  '.').slice(0, -1).join('.');
              ...
              reader.addEventListener('load', function(t) {
                //fix: v3.3 导入的文件名带"."
                config.folderSetting.rootFolder.folderName = file.name.split(
                    '.').
                    slice(0, -1).
                    join('.');
        ...
        if (save != null && document.querySelector('#saveTip') == null) {
          save.insertAdjacentHTML('afterend',
              `<div id="saveTip"><p style="margin-left:20px;margin-top:-20px;">转存也在此处选择位置。<span style="color:red;">因115页面结构以及接口调整，暂时下线【默认保存至当前位置】</span></p></div>`);
        }
        ...
        if (window.parent.document.tryUploadItem) {
          ...
          document.getElementById(
              'neOnlineFileName').innerText = window.parent.document.tryUploadItem.name;
        } else {
      ...
      function AddCeateSha1ButtonInGrid(jNode) {
        ...
        let $button = $(
            '<button class="btnInGrid" title="获取sha1链接"><i class="icon-operate-light ifol-download" style="height:14px;width:14px;position:inherit"></i></button>');
        $button.appendTo($li);
      ...
      function renameInToolTip(element, pItem) {
        ...
        $btn1[0].addEventListener('click', async e => {
          ...
          postSha1Messgae(createMessage(MessageType.PROCESSING,
              `即将开始重命名 【${pItem.name}】 下所有文件：<br><br>去除分隔符：${separator}`));
          await delay(1000);
          ...
          postSha1Messgae(
              createMessage(MessageType.END4UPLOAD, `对目录 【${pItem.name}】下的文件重命名完成！\
                <br><br>获取最新版，或者遇到问题去此反馈，感谢 !点击->\
                <a href="${TIPS.UpdateUrl}" target="_blank">${TIPS.VersionTips}</a>`,
                  pItem.id));
        });
      }
      ...
      function createSha1InToolTip(toopTip, pItem) {
        var $btn = $(
            '<a ><i></i><div style="background:white"><span>获取SHA1链接</span></div></a>');
      ...
      function usingOnlineFileToUploadInToolTip(toopTip, pItem) {
        ...
        if (extension == 'json' || extension == 'txt') {
          let $btn1 = $(
              '<a menu="offline_task" title="暂不支持大于2MB的文本文件操作"><i></i><div style="background:white"><span>尝试转存</span></div></a>');
      ...
      async function GetSearchList(isOnlySelected) {
        ...
        if (!doc) doc = document;
        var lis = doc.querySelectorAll(
            '.list-cell.lstc-search > .list-contents > ul > li');
        if (!lis) return;
        console.log(lis);
        var files = new Array();
        for (var li of lis) {
      ...
      function CreateSha1ButtonForSelectedItems(element) {
        ...
        document.getElementById('my115CreateSha1ForSelected').
            addEventListener('mousedown', async e => {
              ...
              if (items.length == 1) {
                ...
                //创建Sha1所有链接
                await CreateSha1LinksAll(items,
                    `${items[0].name}等${items.length}个`);
              }
      ...
      function ContinuedTask(taskJsonFileName) {
        ...
        reader.addEventListener('load', function(t) {
          try {
            ...
            postSha1Messgae(
                createMessage(MessageType.PROCESSING, '正在解析继续任务配置...'));
            let taskJson = JSON.parse(t.target.result);

            console.log(
                `${taskJson.taskType}, ${taskJson.fileName}, ${taskJson.data.length}`);
            ...
            if (canContinued) {
              if (taskJson.taskType == TaskType.DOWNLOAD) {
                postSha1Messgae(createMessage(MessageType.PROCESSING,
                    `正在开始对【${taskJson.fileName}】继续提取...请稍等！`));
                `
                        提取：
                            let tempFile=[
