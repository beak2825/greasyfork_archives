// ==UserScript==
// @name        lang-8 fix CJK lang-specific variants display
// @name:zh-CN      lang-8修正CJK字形变体显示问题
// @namespace   lang-8.com
// @include     /^https?:\/\/(?:www\.)?lang-8\.com\/[a-zA-Z0-9]+\/journals\//
// @version     5
// @grant       none
// @run-at      document-end
// @description fix display of language-specific glyphs of CJK ideographs
// @description:zh-CN 修正CJK字形变体显示问题
// @license     GPL
// @downloadURL https://update.greasyfork.org/scripts/25510/lang-8%20fix%20CJK%20lang-specific%20variants%20display.user.js
// @updateURL https://update.greasyfork.org/scripts/25510/lang-8%20fix%20CJK%20lang-specific%20variants%20display.meta.js
// ==/UserScript==
...
var title = $j("#subject_show"); //标题
var journalOri = $j("#body_show_ori"); //原文
var journalMotherLang = $j("#body_show_mo"); //母语原文
var refMotherLangBottom = $j("#refer_body_mo_bottom"); //底部参考母语原文

var toBeCorrectedSentencesInPostedEdits = $j(".incorrect"); //别人发布的修改中的用来参照的原文句子
var toBeCorrectedSentencesInEditArea = $j(".sentence"); //底部修改区域的待修改句子
var postedCorrections = $j("li.correct"); //修改后的句子，包含span等子元素

var comments = $j(".comment_small"); //连同修改附带的评论
var correctionComments = $j("p.correction_comment"); //每个修改的原因、备注
var correctionCommentTooltips = $j("div.correction_comment_tooltip"); // 底部气泡图标的提示文字，是修改者对一个修改的备注
var commentReplies = $j(".journal_comment_text"); //评论的回复


function addLangTag(elems, lang){
	if(lang !== "unknown"){
		$j(elems).attr("lang", lang);	
	}
}

function detectLangSingle(elem){
	var stringInElem = $j(elem).text();
	(function(elem){
		guessLanguage.detect(stringInElem, function(l){
			addLangTag(elem, l);
		});
	})(elem); 
}

function choice(array, num){
	var result, chosen;
	result = [];
	if(num > array.length){
		num = array.length;
	...
	while(num > 0){
		chosen = Math.floor(Math.random() * array.length);
		if(result.indexOf(chosen) < 0){
			result.push(chosen);
			num--;
		}
	}
	
	return result.map(_ => array[_]);
}

function detectLangItera(elems, oneDecideAll){
	function doForAll(elems){
		var thisElem, i;
		for(i = 0; i < elems.length; i++){
			thisElem = elems.eq(i);
			(function(thisElem){
				guessLanguage.detect(thisElem.text(), function(l){
					addLangTag(thisElem, l);
				});
			})(thisElem);
		}
	}
  
	if(oneDecideAll === true){
		if(elems.length >= 3){ //randomly pick 3 elements to detect their lang and apply the major result to all elements
			var chosenTexts = choice(elems, 3).map(_ => $j(_).text());
			var langList = [];
			var i, thisText;
			for(i = 0; i < chosenTexts.length; i++){
				thisText = chosenTexts[i];
				(function(thisText, elems){
					guessLanguage.detect(thisText, function(l){
						var resultLang;
						langList.push(l);
						if(langList.length === 3){
							resultLang =  langList[0] === langList[1] ? langList[0] : (langList[0] === langList[2] ? langList[0] : (langList[1] === langList[2] ? langList[1] : langList[0]));
							addLangTag(elems, resultLang);
						}
					});
				})(thisText, elems);
			}
...
detectLangSingle(title);
detectLangSingle(journalOri);
detectLangSingle(journalMotherLang);
detectLangSingle(refMotherLangBottom);

detectLangItera(toBeCorrectedSentencesInPostedEdits, true);
detectLangItera(toBeCorrectedSentencesInEditArea, true);
detectLangItera(postedCorrections, true);
detectLangItera(comments, false);
detectLangItera(correctionComments, false);
detectLangItera(correctionCommentTooltips, false);
detectLangItera(commentReplies, false);
