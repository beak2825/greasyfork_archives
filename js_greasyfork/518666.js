...
// @grant        none
// @run-at document-end
// @downloadURL https://update.greasyfork.org/scripts/518666/upm.user.js
// @updateURL https://update.greasyfork.org/scripts/518666/upm.meta.js
// ==/UserScript==sssstyle="
...
(function() {
    ...
    window.fixend = function (){
        const textarea = document.querySelector(".wysibb-texarea")
        ...
        if (len > 520) {
            ...
            const regex = /(.*)(\[\/img](\[\/url])?)((\n)*)$/
            if (regex.test(end)) {
                // textarea.value = textarea.value.replace(regex, '$1$2[/td][/tr][/table]\n')
                textarea.value = textarea.value.replace(regex, '$1$2[/td][/tr][/table]')
            }
        }
    }
