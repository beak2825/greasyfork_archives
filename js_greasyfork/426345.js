// ==UserScript==
// @homepage     https://www.xiximiao.com
// @name	 显示网站收录数和反向链接数 SEO IN CHINA
// @version      1.0.2
// @description  ,,
// @author       ppk
// @copyright    2021, ppk (https://github.com/ppk) 
// @license      MIT
...
// @exclude		https://*.sogou.com/*
// @exclude		https://*.youdao.com/*
...
// @downloadURL https://update.greasyfork.org/scripts/426345/%E6%98%BE%E7%A4%BA%E7%BD%91%E7%AB%99%E6%94%B6%E5%BD%95%E6%95%B0%E5%92%8C%E5%8F%8D%E5%90%91%E9%93%BE%E6%8E%A5%E6%95%B0%20SEO%20IN%20CHINA.user.js
// @updateURL https://update.greasyfork.org/scripts/426345/%E6%98%BE%E7%A4%BA%E7%BD%91%E7%AB%99%E6%94%B6%E5%BD%95%E6%95%B0%E5%92%8C%E5%8F%8D%E5%90%91%E9%93%BE%E6%8E%A5%E6%95%B0%20SEO%20IN%20CHINA.meta.js
// ==/UserScript==
// Author：		pythonk

// @require		http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.min.js
// 原档 https://web.archive.org/web/20111229015412/http://userscripts.org/scripts/source/56095.user.js 
console.log(GM_info); 
...
var SE={
	...
	se_config : {
		...
		"百度":{
			"show":1,
			"收录1":{"show":1, cfg:["https://www.baidu.com/s?wd=site%3A#d", '<b style="color:#333">', "</b>", "抱歉没有找到"]},
			"收录2":{"show":1, cfg:["https://www.baidu.com/s?wd=site%3A#d", ">找到相关结果数", "个", "抱歉没有找到"]},
      
			"1天内":{"show":0, cfg:["https://www.baidu.com/s?q1=site%3A#d&q2=&q3=&q4=&rn=10&lm=1&ct=0&ft=&q5=&q6=&tn=baiduadv", ">找到相关结果数", "个", "抱歉没有找到"]},
			"7天内":{"show":0, cfg:["https://www.baidu.com/s?q1=site%3A#d&q2=&q3=&q4=&rn=10&lm=7&ct=0&ft=&q5=&q6=&tn=baiduadv", ">找到相关结果数", "个", "抱歉没有找到"]},
			"1月内":{"show":0, cfg:["https://www.baidu.com/s?q1=site%3A#d&q2=&q3=&q4=&rn=10&lm=30&ct=0&ft=&q5=&q6=&tn=baiduadv", ">找到相关结果数", "个", "抱歉没有找到"]},
			"外链":{"show":0, cfg:["https://www.baidu.com/s?wd=domain%3A#d", ">找到相关结果", "个", "抱歉没有找到"]},
			"快照日期":{"show":0, cfg:["https://www.baidu.com/s?wd=#d", "<br><span class=\"g\">", "</span>", "抱歉没有找到", function(ret){return ret.split('/ ')[1];}]},			
      "首页位置":{"show":0, cfg:["https://www.baidu.com/s?wd=site%3A#d", "<div class=\"c-border\">", "相关搜索<", "抱歉没有找到",
                             function(ret){
                               var i=0, k=0; var ss=''; 
                                while((ss=SE_Tool.get_middleStr(ret, "tyle=\"text-decoration:none;position:relative;\">", "</a><div class=\"c-tools c-gap-left"))!=""){
                                  ret=ret.replace("tyle=\"text-decoration:none;position:relative;\">"+ss+"</a><div class=\"c-tools c-gap-left", "");
                                  i=i+1; 
                                  if(ss.split(' ')[0].toLowerCase()==SE.domain.toLowerCase()+"/"){k=1;break;}
                                } 
                                 return (k==1?i:0).toString();
                            }
                            ]}      
      /*, "首页位置err":{"show":0, cfg:["https://www.baidu.com/s?wd=site%3A#d", "target=\"_blank\">百度站长平台</a>。", "<br clear=all>", "抱歉没有找到",
                             function(ret){var i=0, k=0; var ss=''; 
                                while((ss=SE_Tool.get_middleStr(ret, "<br><span class=\"g\">", "</span>"))!=""){
                                  ret=ret.replace("<br><span class=\"g\">"+ss+"</span>", "");
                                  i=i+1; 
                                  if(ss.split(' ')[0].toLowerCase()==SE.domain.toLowerCase()+"/"){k=1;break;}
                                } 
                                 return (k==1?i:0).toString();
    ...
    "权重":{
			"show":1,
        "ChinaZ":  {"show":1, cfg:["http://seo.chinaz.com/#d",        "" , "" ,   "站长"  , null, 3 , "utf-8" ]},
        "爱站":    {"show":1, cfg:["https://www.aizhan.com/cha/#d",   "" , "" ,   "爱站"  , null, 3 , "utf-8" ]},
        "5118受宠":{"show":1, cfg:["https://seo.5118.com/#d",         "", ""  ,   "5118"  , null, 3 , "utf-8" ]},
			},
		"Alexa":{
			"show":1,
			"排名1":{"show":1, cfg:["https://www.alexa.com/siteinfo/#d", "<div class=\"rankmini-rank\">", "</div>",  this.domain+"该网址错误"]},
			"排名2":{"show":1, cfg:["https://www.alexa.com/siteinfo/#d", "<p class=\"big data\">\n									        	        <span class=\"hash\">#</span>", "</p>",  this.domain+"该网址错误"]},
			},
		"谷歌":{
        "show":0,
        "收录":{"show":1, cfg:["https://www.google.com.hk/search?hl=zh-CN&q=site%3A#d", "找到约", " 条结果<nobr>", "找不到和您的查询", function(ret){return ret.replace("约 ","").replace("找到", "").replace("获得", "");}]},
        "外链":{"show":0, cfg:["https://www.google.com.hk/search?hl=zh-CN&q=link%3A#d", "找到约", " 条结果<nobr>", "找不到和您的查询", function(ret){return ret.replace("约 ","").replace("找到", "").replace("获得", "");}]},
        "ＰＲ":{"show":0, cfg:["https://pr.linkhelper.cn/querypr.asp?url=#d", "</a>的PR：<img src='https://www.linkhelper.cn/pagerank", ".gif' border=0>", this.domain+"该网址错误！", null, null, "gb2312"]}
			},
		"搜狗":{
        "show":0,
        "收录":{"show":1, cfg:["https://www.sogou.com/web?query=site%3A#d", "已为您找到约", "条相关结果", "<div class=\"no-result\" id=\"noresult_part1_container\">没有找到"]},
        "外链":{"show":0, cfg:["https://www.sogou.com/web?query=link%3A#d", "已为您找到约", "条相关结果", "<div class=\"no-result\" id=\"noresult_part1_container\">没有找到"]},
        "评级":{"show":0, cfg:["https://rank.ie.sogou.com/sogourank.php?ur=http%3A%2F%2F#d%2F", "sogourank=", "", "all"]}
			},
		/*
		 * "必应":{
			"show":0,
			"收录":{"show":1, cfg:["https://cn.bing.com/search?q=site%3A#d", "条结果(共 ", " 条)</span>", "<div id=\"no_results\"><h1>未找到 <strong>site:"]},
			"外链":{"show":1, cfg:["https://cn.bing.com/search?q=link%3A#d", "条结果(共 ", " 条)</span>", "<div id=\"no_results\"><h1>未找到 <strong>site:"]}
			}
		 ,"头条":{
        "show":0,
        "收录":{"show":0, cfg:["https://www.soso.com/q?w=site%3A#d", "搜索到约", "项结果", "<font color=#DA3145>", null, null, "gb2312"]},
        "外链":{"show":0, cfg:["https://www.soso.com/q?w=link%3A#d", "搜索到约", "项结果", "<font color=#DA3145>", null, null, "gb2312"]}
			}
    ,"神马":{
        "show":0,
        "收录":{"show":0, cfg:["https://m.sm.com/search?q=site%3A#d", "<span class=\"fr\">共", "条结果</span>", "抱歉，没有找到与", function(ret){ret=ret.replace("约","");if(ret.indexOf("万")>0){var tmp=ret.split('万'); return tmp[0]*10000+(tmp[1].length>0?parseInt(tmp[1]):0);} return ret;}]},
        "外链":{"show":0, cfg:["https://www.youdao.com/search?q=link%3A#d", "<span class=\"fr\">共", "条结果</span>", "抱歉，没有找到与", function(ret){ret=ret.replace("约","");if(ret.indexOf("万")>0){var tmp=ret.split('万'); return tmp[0]*10000+(tmp[1].length>0?parseInt(tmp[1]):0);} return ret;}]}
			}
    ,"域名":{
        "show":0,
        "注册时间":{"show":0, cfg:["https://whois.linkhelper.cn/whois.asp?domain=#nd", "域名注册日期：", "</font>", "o match", null, null, "gb2312"]}
			}
    ,"雅虎":{
        "show":0,
        "收录":{"show":0, cfg:["https://sitemap.cn.yahoo.com/search?bwm=p&p=#d", "被收录的网页： 共 <strong>", "</strong>", "对不起， 你的请求不能正确处理"]},
        "外链":{"show":0, cfg:["https://sitemap.cn.yahoo.com/search?bwm=i&p=#d&bwm=i&bwmo=d", "链向该地址的网页： 共 <strong>", "</strong>", "对不起， 你的请求不能正确处理"]}
		  }*/
	},
	...
	show : function(d){
		...
		$("#se_wrapper_inner em").each(function(){
			...
			SE_Tool.get_part( 
                  cfg[0].replace('#nd', 
                  SE.domain.replace('www.','')).replace("#d", SE.domain), 
                  cfg[1], cfg[2], cfg[3], $(this), cfg[4], cfg[5]||null, cfg[6]||null );
		});
    ...
    //点击 ?
		$("#se_config_link").click(function(){
			...
			//check for update
			SE_Tool.get_part(
				"https://greasyfork.org/scripts/426345/code/426345.meta.js",
				"// @uso:timestamp",
				"// @uso:installs",
				"无此字符",
				$("#se_check_version"),
				...
				function(){
					...
					if( GM_getValue('last_version')!=undefined && new Date(GM_getValue('last_version'))<new Date(s) ){
						$("#se_check_version").html("<span style='color:red'>有新版本</span>，<a href='https://greasyfork.org/scripts/426345/code/426345.user.js' id='install_it'>点击此处安装</a>");
	...
	showConfig : function(){
		...
		se_config_html+='</div></fieldset><fieldset><legend>关于脚本</legend>脚本重置：PPK <a target="_blank" href="https://www.xiximiao.com">反馈</a><br/>'+
    '脚本地址：<a target="_blank" href="https://greasyfork.org/scripts/426345">greasyfork</a></fieldset>'+
        '<a style="position: absolute; right: 10px; top: 3px;" id="se_config_close" href="javascript:void(0);" target="_self">关闭</a></div>';
...
var SE_Tool={
	get_part : function(url, start, end, notExistStr, obj, dealFun, callback, charset){
    if (callback == 3 ) {
      obj.html("<a href=\""+url+"\" target=\"_blank\">"+notExistStr+"</a>");
      return ; 
    }
		...
		GM_xmlhttpRequest({
			...
			onload: function(responseDetails) {
				...
				if( url.indexOf("https://www.sogou.com/web?query=link%3A")>-1 && html.indexOf('<form name="authform" method="POST" action="thank.php">')>-1 ){
