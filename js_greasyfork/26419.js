...
// @version     2.6.2.1 LowSpec
// @grant       none
// @run-at      document-end
// @trigger     start
// @description KFOL助手的自定义点数分配脚本，可根据当前状态实时计算点数分配方案（仅限自动攻击相关按钮有效，如计算速度很慢，推荐使用最新版的Firefox浏览器）
// @downloadURL https://update.greasyfork.org/scripts/26419/%E5%AE%9E%E6%97%B6%E8%AE%A1%E7%AE%97%E7%82%B9%E6%95%B0%E5%88%86%E9%85%8D%E6%96%B9%E6%A1%88%E5%8A%A3%E5%8C%96%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/26419/%E5%AE%9E%E6%97%B6%E8%AE%A1%E7%AE%97%E7%82%B9%E6%95%B0%E5%88%86%E9%85%8D%E6%96%B9%E6%A1%88%E5%8A%A3%E5%8C%96%E7%89%88.meta.js
// ==/UserScript==
'use strict';
...
// 综合考虑预期外的损失及收益，返回当前方案的期望评价
    ...
    if(lowHPFlag === 1 && npcFlag <= 1){
        ...
        npcPropability = (playerPropability + 0.99) / 2; // 调整默认概率（幸运）
        ...
        npcPropability = (playerPropability + 0.01) / 2; // 调整默认概率（倒霉）
        ...
        npcPropability = (playerPropability + 0.01) / 2; // 调整默认概率（倒霉）
        strategyResultUnLucky = restLifeInNextLevelByPoints(currentLevel, currentLife, npcFlag, levelPoints);
        strategyResultUnLucky  = strategyResultUnLucky * playerPropability * (1 - npcPropability) * (npcFlag < 2 ? (1 - strongProbability) : strongProbability);
        playerPropability = tempPlayerPropability;  // 恢复默认概率
        npcPropability = tempNPCPropability;  // 恢复默认概率
        strategyResult = strategyResultNormal + strategyResultLucky + strategyResultUnLucky;
    }
            ...
            for (var CriticalHitNum = 0; CriticalHitNum <= Math.floor(attackNum / 2) + 1; CriticalHitNum++) {
                ...
                for (var SkillNum = 0; SkillNum <= Math.floor(attackNum / 2) + 1; SkillNum++) {
                   ...
                   // 计算意志
                    ...
                    var playerDamage = 0; // 计算受到伤害
                    var attackedTimes = attackedNum; // 玩家被攻击次数

                    var expectCriticalHitNum = attackedTimes - CritBinom(attackedTimes, 1 - npcParam["暴击率"], npcPropability); // 期望npc暴击次数
                    var expectSkillNum = attackedTimes - CritBinom(attackedTimes, 1 - npcParam["技能率"], npcPropability); // 期望npc技能次数
                    playerDamage += Math.ceil((attackedTimes + expectCriticalHitNum) * npcParam["攻"]); //+暴击
                    ...
                    var playerWill1 = attackedNum == 0 ? 1 : Math.floor(Math.sqrt(DFCardinal * playerDamage / (playerHPCoefficient * extraPointList.get('耐力') / 100))) - DFCardinal - basePoints["意志"];
                    var playerWill2 = attackedNum == 0 ? 1 : Math.floor(Math.sqrt(DFCardinal * playerDamage / (playerHPCoefficient * (1 + extraPointList.get('耐力') / 100)))) - DFCardinal - basePoints["意志"];
                    playerWill1 = Math.max(playerWill1,1);
                    playerWill2 = Math.max(playerWill2,1);
                    var playerWill = playerWill1;
                    //console.log(" 初始意志: " + playerWill);
                    var maxHPPlayerHealth = getPointByProperty("体质", currentLife);
                    var maxPlayerHealth = basePoints["分配点"] - levelPoints["灵活"] - 1 - levelPoints["智力"] - levelPoints["力量"] - levelPoints["敏捷"];
                    //console.log(" 最大体质: " + maxHPPlayerHealth + "  " + maxPlayerHealth);
                    var playerHealth = basePoints["分配点"] - levelPoints["灵活"] - playerWill - levelPoints["智力"] - levelPoints["力量"] - levelPoints["敏捷"]; // 计算玩家体质加点
                    if(playerHealth > maxHPPlayerHealth){
                        // 意志加点不影响生命
                        playerWill = playerWill1;
                    }
                    else if(playerHealth < maxHPPlayerHealth){
                        // 意志加点已损害最大生命值，判断是否该减小至不损害最大生命值
                        var diffPoint = Math.min(maxHPPlayerHealth - playerHealth,playerWill - 1);
                        //console.log(" 体质: " + playerHealth + " 意志 " + playerWill1 + " 力量 " + levelPoints["力量"] + " 敏捷 " + levelPoints["敏捷"] + " 伤害 " + playerDamage);
                        playerHealth += diffPoint;
                        var playerWill3 = playerWill - diffPoint;  // 加点造成损失生命的临界意志
                        playerWill = playerWill3; 
                        //console.log(" 体质: " + playerHealth + " 意志 " + playerWill1 + " 力量 " + levelPoints["力量"] + " 敏捷 " + levelPoints["敏捷"]);
                        playerWill = playerWill2 > playerWill ? playerWill2 : playerWill;
                        playerHealth = playerWill2 === playerWill ? playerHealth - (playerWill2 - playerWill3) : playerHealth;
                        //console.log(" 体质: " + playerHealth + " 意志 " + playerWill + " 力量 " + levelPoints["力量"] + " 敏捷 " + levelPoints["敏捷"]);
                    }
                    if(playerHealth < 1){
                        // 补体质点数
                        var diffPoint = Math.min(1 - playerHealth,playerWill - 1);
                        playerHealth += diffPoint;
                        playerWill -= diffPoint;
                    }
                    levelStrategy1["意志"] = playerWill;
                    levelPoints = getNextLevelPoints(currentLevel, npcFlag, levelStrategy1);
                    var restMaxLife = levelPoints["体质"] < 1 ? 0 : restLifeInNextLevelByStrategy(currentLevel, currentLife, npcFlag, levelStrategy1);
                    if(levelPoints["体质"] < 1){
                        continue;
                    }
                    var maxLife = getPropertyByPoint("体质", levelPoints["体质"]);
                    //console.log(levelStrategy1);
                    //console.log("maxLife " + maxLife + " restMaxLife " + restMaxLife + " playerWill1 " + playerWill1 +  " playerWill " + playerWill);
                    if(maxLife > restMaxLife + 100){
                        if((playerWill1 - playerWill) > ((maxLife - restMaxLife) / 20)){
                        //console.log("maxLife " + maxLife + " restMaxLife " + restMaxLife + " playerWill1 " + playerWill1 +  " playerWill " + playerWill);
                            playerWill += Math.ceil((maxLife - restMaxLife) / 20 / 3);
                        }
                    }
                    levelStrategy1["意志"] = playerWill;
                    //console.log(levelStrategy1);
                    //console.log("新预计最大生命： " + restMaxLife + " maxHPPlayerHealth " + maxHPPlayerHealth + " 体质 " + levelPoints["体质"] +  " 伤害 " + playerDamage);
                    levelStrategy1["意志"] = playerWill;
                    //console.log(levelStrategy1);
                    levelPoints = getNextLevelPoints(currentLevel, npcFlag, levelStrategy1);
                    var restNextLevelLife1 = levelPoints["体质"] < 1 ? 0 : restLifeInNextLevelByStrategy(currentLevel, currentLife, npcFlag, levelStrategy1);
                    var restNextLevelLife = levelStrategy["被攻击次数"] === -1 ? 0 : restLifeInNextLevelByStrategy(currentLevel, currentLife, npcFlag, levelStrategy);
