// ==UserScript==
// @name        Travian Task Queue for Travian 4 w/ Repeatable Tasks
// @namespace   https://greasyfork.org/en/users/288942-zhouball
// @author      Zhouball (actual Travian Task Queue is maintained by adipiciu, based on TTQ by Risi, and further edited by Nevam and then Pimp Trizkit and Serj_LV)
// @license     GPL version 3 or any later version; http://www.gnu.org/copyleft/gpl.html
// @description Schedule delayed constructions, upgrades, and attacks as tasks.
// @include     *://*.travian.*
// @include     *://*/*.travian.*
...
// @version     1.6.0.1.3
// TTQ version : 1.6.0
// Repeatable Tasks version : 1.3
// (I'll keep the version numbers clear, but I suspect adipiciu isn't incrementing his version numbers anyways)
// @downloadURL https://update.greasyfork.org/scripts/381357/Travian%20Task%20Queue%20for%20Travian%204%20w%20Repeatable%20Tasks.user.js
// @updateURL https://update.greasyfork.org/scripts/381357/Travian%20Task%20Queue%20for%20Travian%204%20w%20Repeatable%20Tasks.meta.js
// ==/UserScript==
...
function initialize() {
	...
	try {
		var uName = $gt('a',$gc('playerName',$id('sidebarBoxHero'))[0])[1].textContent;
	...
	get(fullName+'spieler.php', getuId, '');
	function getuId(httpRequest) {
		if (httpRequest.readyState == 4) {
			if (httpRequest.status == 200 && httpRequest.responseText) {
				var holder = document.createElement('div');
				holder.innerHTML = httpRequest.responseText;
				var aV = xpath('//a[(@class="active") and contains(@href,"uid")]', holder, true);
				if (aV) { 
...
function TTQ_showMenuCommand() {
	...
	tA.innerHTML = "<span id='ttqPanelSpan'><b>TTQw/RT</b> v" + sCurrentVersion + " - <span id='ttqLoad' style='font-weight: bold' >0</span> ms - <span id='ttqReloadTimer'>0</span></span>";
...
//-- определение координат, активного здания и создание SPAN с кликабельными ссылками на деревню.
	...
	if ( isNaN(iMyRace) || iMyRace < 0 ) {
		var nation = $gc("nation",$id("sidebarBoxHero"));
		if (nation.length > 0) {
			iMyRace = parseInt(nation[0].className.match(/\d+/)[0]) - 1;
			if ( isNaN(iMyRace) || iMyRace < 0 || iMyRace > 6 ) iMyRace = 0;
			else setOption("RACE", iMyRace);
		} else {
			for (i=1; i<8; i++) {
				nation = $gc("tribe"+i+"_medium",$id("sidebarBoxHero"));
				if (nation.length > 0) {
					iMyRace = i - 1;
    ...
    //added by Repeating Tasks
    var sRepeatImg = "data:image/png;base64,";
    //Styles
    var cssStyle = "";
...
function refreshTaskList(aTasks) {
	...
	for(var i = 0; i < aTasks.length; ++i) {
		...
		oTaskRow.setAttribute("repeating", aThisTask[5]);
		...
		/// *** REPEATING TASK CODE ***
		var repeating = aThisTask[5].split("_");
		if (repeating[0] != 0)
		{
			var oRepeatSpan = document.createElement('span');
			oRepeatSpan.title = "Set to repeat every " + (repeating[1] / 3600.0).toFixed(2) 
			+ " hours, plus " 
			+ (repeating[2] / 3600.0).toFixed(2) 
			+ " or minus " 
			+ (repeating[3] / 3600.0).toFixed(2) 
			+ " hours, ";
			if (parseInt(repeating[0]) > 0)
			{
				oRepeatSpan.title += + repeating[0] + " more times.";
			}
			else
			{
				oRepeatSpan.title += "forever."
			}
			var oRepeatImg = document.createElement('img');
			oRepeatImg.src = sRepeatImg;
			oRepeatImg.style.verticalAlign = "middle";
			oRepeatSpan.appendChild(oRepeatImg);
			oTaskRow.appendChild(oRepeatSpan);
		}
		/// *** END OF REPEATING TASK CODE ***
...
function editTime(ev) {
	...
	var repeating = oTaskRow.getAttribute("repeating");
	displayTimerForm(type, target, options, timestamp, taskindex, villagedid, repeating);
}
...
function addToHistory(aTask, bSuccess, tMsg) {
	...
	// *** REPEATING TASK CODE ***
	// we're plugging this into addToHistory, so if there's ever tasks added in the future that aren't added to history, they won't work
	
	var repeating = aTask[5].split("_");
	var addedTime = 0;
	if (repeating[0] != 0)
	{
		bLocked = true;
		...
		addedTime = parseInt(repeating[1]) + (Math.floor(Math.random() * (parseInt(repeating[2]) + parseInt(repeating[3]) + 1)) - parseInt(repeating[3]));
		var newTime = parseInt(aTask[1]) + addedTime;
		if (parseInt(repeating[0]) > 0)
		{
			repeating[0] = parseInt(repeating[0]) - 1;
		}
		var newValue = aTask[0] + ',' + newTime + ',' + aTask[2] + ',' + aTask[3] + ',' + aTask[4];
		newValue += ',' + repeating.join("_");
		if (data=="") {
			data = newValue;
			aTasks = data.split("|");
		} else {
...
function makeHistoryRow(aTask, index/*, iServerTimeOffset*/) {
		...
		/// *** REPEATING TASK CODE ***
		if (parseInt(aTask[7]) != 0)
		{
			var oRepeatSpan = document.createElement('span');
			oRepeatSpan.title = "Created a repeat task in " + (parseInt(aTask[7]) / 3600).toFixed(2) + " hours.";
			var oRepeatImg = document.createElement('img');
			oRepeatImg.src = sRepeatImg;
			oRepeatImg.style.verticalAlign = "middle";
			oRepeatSpan.appendChild(oRepeatImg);
			oHistoryRow.appendChild(oRepeatSpan);
		}
		/// *** END OF REPEATING TASK CODE ***
...
function upgradebuild(aTask) {
	...
	var httpRequest = new XMLHttpRequest();
		...
		httpRequest.open("GET", fullName+"build.php?" + ((oldVID != 'null')?("newdid=" + oldVID):("")) + "&id=" + aTask[2] + (eParam ? eParam: ""), true);
		httpRequest.onreadystatechange = function() {
 ...
 ***************************************************************************/
function displayTimerForm(iTask, target, options, timestamp, taskindex, villagedid, repeating = "0_0_0_0") {
	...
	var repeatingArr = repeating.split("_");
	var spanRepeating = document.createElement("span");
	spanRepeating.id = "spanRepeating";
	if (parseInt(repeatingArr[0]) == 0)
	{
		spanRepeating.style.display = "none";
	}
	spanRepeating.innerHTML = 
	'every <input name="every" type="text" id="every" style="width: 40px" value="' + (parseInt(repeatingArr[1])/60) + '" />'
	+ 'plus <input name="plus" type="text" id="plus" style="width: 40px" value="' + (parseInt(repeatingArr[2])/60) + '" />'
	+ 'or '
	+ 'minus <input name="minus" type="text" id="minus" style="width: 40px" value="' + (parseInt(repeatingArr[3])/60) + '" />'
	+ '<select name="timeUnitRepeating" id="timeUnitRepeating"><option value="1">' + aLangStrings[27]
	+ '</option><option value="60" selected="selected">' + aLangStrings[28]
	+ '</option><option value="3600">' + aLangStrings[29]
	+ '</option><option value="86400">' + aLangStrings[30]
	+ '<input name="times" type="text" id="times" style="width: 40px" value="' + repeatingArr[0] + '" title="-1 to repeat forever" /> '
	+ '<span title="-1 to repeat forever"> times </span>';

	var oRepeatingCheckbox = $e("input",[['name',"repeatingCheckbox"],['id',"repeatingCheckbox"],['type',"checkbox"]]);
	if (parseInt(repeatingArr[0]) != 0)
	{
		oRepeatingCheckbox.checked = true
	}
	ttqAddEventListener(oRepeatingCheckbox, 'click', function() {
		if (oRepeatingCheckbox.checked == true)
		{
			spanRepeating.style.display = "";
		}
		else
		{
			spanRepeating.style.display = "none";
	...
	oTimerForm.innerHTML += "Repeating: ";
	oTimerForm.appendChild(oRepeatingCheckbox);
	oTimerForm.appendChild(spanRepeating);
	oTimerForm.appendChild(document.createElement("br"));
	/// *** END OF REPEATING TASK CODE ***
...
function handleTimerForm(oForm, iAction, taskindex, villagedid) {
	...
	/// *** REPEATING TASK CODE ***
	// times _ every _ plus _ minus
	// every, plus, and minus are in seconds
	var repeating = "0_0_0_0";
	if (oForm.elements["repeatingCheckbox"].checked == true)
	{
		var spanRepeating = document.getElementById("spanRepeating");
		var timeUnitRepeating = spanRepeating.children["timeUnitRepeating"].value;
		if (parseInt(spanRepeating.children["times"].value) != 0)
		{
			//technically times = 1 is also discluded because of the below line
			repeating = parseInt(spanRepeating.children["times"].value) - 1; // subtract one here because we include the first in our count
			repeating += "_";
			repeating += spanRepeating.children["every"].value * timeUnitRepeating;
			repeating += "_";
			repeating += spanRepeating.children["plus"].value * timeUnitRepeating;
			repeating += "_";
			repeating += spanRepeating.children["minus"].value * timeUnitRepeating;
		}
	}
	...
	for (var i=0; i<iTaskTime.length; i++) {
		setTask(oForm.elements["timerTask"].value, iTaskTime[i], oForm.elements["timerTarget"].value, oForm.elements["timerOptions"].value, taskindex, villagedid, repeating);
	}
...
/**************************************************************************
  ...
  * @param repeating: data regarding repeating the task
  ***************************************************************************/
function setTask(iTask, iWhen, target, options, taskindex, villagedid, repeating) {
	var iVillageId = typeof(villagedid) != 'undefined' ? villagedid : currentActiveVillage;
	if(bLocked) {
	...
	var aTasks = data.split("|");
	var iTaskIndex = typeof(taskindex) != 'undefined' ? taskindex : aTasks.length;
	var newValue = iTask + ',' + iWhen + ',' + target.replace('|','&#124;') + ',' + options;
