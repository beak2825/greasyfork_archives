...
// @grant        none
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/462583/Slayone%20Aimbot.user.js
// @updateURL https://update.greasyfork.org/scripts/462583/Slayone%20Aimbot.meta.js
// ==/UserScript==
...
`
    ...
    (async function() {
        let script = await request(`https://slay.one/js/c6.js`);
    ...
    let observer = new MutationObserver(mutations => {
        for (const mutation of mutations) {
            for (let node of mutation.addedNodes) {
                if(node.src && node.src.match('https://slay.one/js/c6.js')){
                    node.outerHTML = ``
                    node.innerHTML = ``;
                    node.src='';
                }
