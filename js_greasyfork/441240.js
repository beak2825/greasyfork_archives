// ==UserScript==
// @name         ShowSpinnerCount
// @namespace    osu
// @version      2.1.2
// @description  Adds spinner count to beatmap pages
// @author       Magnus Cosmos
// @match        https://osu.ppy.sh/*
// @match        https://lazer.ppy.sh/*
// @require      https://greasyfork.org/scripts/473977-osu-web/code/osu-web.js
// @run-at       document-start
// @downloadURL https://update.greasyfork.org/scripts/441240/ShowSpinnerCount.user.js
// @updateURL https://update.greasyfork.org/scripts/441240/ShowSpinnerCount.meta.js
// ==/UserScript==

const countSpinnerSvg = `url(data:image/svg+xml;base64,)`;

const osu = new OsuWeb(null, function () {
    this.modifyFn(this.React, "createElement", "React.createElement", null, function (args, r) {
        if (args[1]?.className === "beatmap-basic-stats") {
            if (r.props.children.length < 5) {
                const countSpinners = r._owner.stateNode.renderEntry("count_spinners");
                countSpinners.props.children[0].props.style.backgroundImage = countSpinnerSvg;
                countSpinners.props.title = "Spinner Count";
                r.props.children.push(countSpinners);
            }
        }
    });
    this.addStyle(`
    @media (min-width: 900px) {
        .beatmapset-header {
            grid-template-columns: 1fr 320px !important;
        }
        .beatmapset-info {
            grid-template-columns: 1fr 175px 320px !important;
        }
    }`);
});