// ==UserScript==
// @name           ZenzaWatch DEV版-lei
// @namespace      https://github.com/segabito/
// @description    ZenzaWatchの開発 先行バージョン Leisskey共有用に改造済み
...
// @downloadURL https://update.greasyfork.org/scripts/405710/ZenzaWatch%20DEV%E7%89%88-lei.user.js
// @updateURL https://update.greasyfork.org/scripts/405710/ZenzaWatch%20DEV%E7%89%88-lei.meta.js
// ==/UserScript==
/* eslint-disable */
...
const nicoUtil = {
	...
	getCommonHeader: () => JSON.parse(document.querySelector('#CommonHeader').dataset.commonHeader || '{}'),
	isPremium: () => !!nicoUtil.getCommonHeader().initConfig.user.isPremium,
	isLogin: () => !!nicoUtil.getCommonHeader().initConfig.user.isLogin,
	getPageLanguage: () => {
		try {
	...
	openTweetWindow: ({watchId, duration, isChannel, title, videoId}) => {
		...
		const url =
			'https://misskey.lei202.com/share?' +
...
class CommentListView extends Emitter {
	constructor(params) {
		...
		this.scrollTop = throttle.raf(this.scrollTop.bind(this));
		this._initializeView(params, 0);
	}
	...
	_initFrame(w) {
		...
		this.frameLayer.frame.addEventListener('visibilitychange', e => {
			...
			if (this.isAutoScroll) {
				this.scrollTop(this.timeScrollTop);
			}
			this._refreshInviewElements();
		});
	...
	scrollTop(v) {
		...
		if (typeof v === 'number') {
			this._scrollTop = v;
			this.frameLayer.isVisible && (this._container.scrollTop = v);
		} else if (this.frameLayer.isVisible) {
			...
			<div class="menuItemContainer rightTop">
				<div class="scalingUI">
					<div class="menuButton zenzaTweetButton" data-command="tweet">
						<div class="tooltip">れいすきーに投稿</div>
						<div class="menuButtonInner">L</div>
					</div>
