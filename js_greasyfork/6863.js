...
1.31 - support _k.jpg and _h.jpg
...
// @version        1.30
// @downloadURL https://update.greasyfork.org/scripts/6863/Flickr%20Link%20Original%20Images%20AllMod.user.js
// @updateURL https://update.greasyfork.org/scripts/6863/Flickr%20Link%20Original%20Images%20AllMod.meta.js
// ==/UserScript==
...
var ImgLinks = {
		...
		for (var i = 0; i < list.length; i++) {
			...
			try {
				...
				if (m && m[2]) { // If href is a link to thumbnail
					...
					var imgId = m[2];
					// Input:
					//   href â€” link to supposed page with original pic
					//   Result of getLookupFunc with following inputs:
					//     imgId â€” image id
					//     img â€” content of <img> tag
					myLog(href);
  ...
  getLookupFunc: function(startUrl, img, thumbUrl) {
    return function resultParser(res) {
	  ...
	  else if (m[1].match(/_k/)) {
	    ImgLinks.add(img, m[1], "Flickr", "LAR1");
	  }
	  else if (txt.match(/\/sizes\/k\//)){
            var http1 = 'http://www.flickr.com' + txt.match(/\/photos\/[^\n]+\/[0-9]+\/sizes\/k\//);
	    myLog('http1 is: ' + http1);	  
	    ImgLinks.lookup(http1, ImgLinks.getLookupFunc(startUrl, img, thumbUrl));
          }
	  else if (m[1].match(/_h/)) {
	    ImgLinks.add(img, m[1], "Flickr", "LAR2");
	  }
	  else if(txt.match(/\/sizes\/h\//)){
            var http2 = 'http://www.flickr.com' + txt.match(/\/photos\/[^\n]+\/[0-9]+\/sizes\/h\//);
	    myLog('http2 is: ' + http2);	  		  
	    ImgLinks.lookup(http2, ImgLinks.getLookupFunc(startUrl, img, thumbUrl));
          }
	  else if(m[1].match(/_b/)){
	    ImgLinks.add(img, m[1], "Flickr", "LAR3");      
          }		  
	  else if(m[1].match(/_z/)){
	    ImgLinks.add(img, m[1], "Flickr", "MED1");      
          }		  
          else if(m[1].match(/_m/)){
	    ImgLinks.add(img, m[1], "Flickr", "MED2");      
          }  else {
			ImgLinks.add(img, m[1], "Flickr", "SML");
          }	  
        } else {
