...
// @downloadURL https://update.greasyfork.org/scripts/3828/Logical%20Gaia.user.js
// @updateURL https://update.greasyfork.org/scripts/3828/Logical%20Gaia.meta.js
// ==/UserScript==
	...
	open: function(app) {
		...
		return app.connect(
			...
			function(o) {
				...
				var cache = special.match(/data\-slot=\"(\d+)\.\d+\.[\d\w]+\" slot=\"\d+\" title=\"Project Old School(?: \((\d+)\))?\"/);

				// Do Project Old School exist?
				if (cache) {
					app.caches = cache[2];
					app.debug("Found " + cache[2] + " Project Old School.");
					return app.connect(
						...
						function(o) {
							...
							if (use) {
								app.debug("Opening Project Old School.");
								return app.connect(
									"GET", use[1], use[2],
									function(o) {
										...
										if (loot) {
											loot[1] = loot[1].replace(/^Project Old School\: /, "");
					...
					app.debug("Found NO Project Old School.");
					alert("There are no more Project Old School to open.");
				}
			}
		);
	}
