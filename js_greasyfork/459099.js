// ==UserScript==
// @name           Transfer List(Sell Bank)
// @version        1.0.10modific
// @author         Douglas_Kraft
// @description    You can use this script to see rating R4, REC, TI and Sell Bank on TL.
// @match	   https://trophymanager.com/transfer/
// @namespace  https://greasyfork.org/pt-BR/scripts/459099
// @downloadURL https://update.greasyfork.org/scripts/459099/Transfer%20List%28Sell%20Bank%29.user.js
// @updateURL https://update.greasyfork.org/scripts/459099/Transfer%20List%28Sell%20Bank%29.meta.js
// ==/UserScript==
...
const updateRow = (player, index) => {
    ...
    if ((!player.rating.find(skill => Number(skill) >= Number(minRec))) || (minTI && Number(player.ti) < Number(minTI))) {
        ...
        if (span) {
            ...
            //var age = Number(player.age) * 1 + Number(player.months) / 12;
          //var asi = Number(player.skill_index.split(',').join(''));
                //-(((asi)*500*Math.pow(300/(age*12),2.5))/1000) ;
        ...
        if (spanAsi) {
            ...
            asi = Math.round(asi / 100) / 10 ;
            asi= Math.round(((asi*500*((25/((Number(player.age) * 1 + Number(player.months) / 12)))**2.5))))/1000+ 'M';
