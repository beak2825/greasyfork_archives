// ==UserScript==
// @name           RED Populate Label Collage
// @author         The original author(s) at what.cd, minor collage pagination workaround/fix by tobber13
// @namespace      redacted.ch
// @description    Finds missing releases in label collages
// @version        1.0.7
// @match          https://*.redacted.sh/collage*.php*id=*
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/394029/RED%20Populate%20Label%20Collage.user.js
// @updateURL https://update.greasyfork.org/scripts/394029/RED%20Populate%20Label%20Collage.meta.js
// ==/UserScript==

/*
1.0.3
Additional code to deal with paginated collages is appended at the end
Existing code from 1.0.2 is essentially untouched
Issue carried over from 1.0.2: You may still need to disable your adblocker on redacted.sh
1.0.4
Updated to use less API call(s).
1.0.5
Updated for the new redacted.sh domain
The namespace was kept as redacted.ch to ensure userscript managers will update it correctly
1.0.6
Updated for the new React-based collage pages
1.0.7
Updated to use the showonlygroups parameter which should greatly improve performance for large collages (e.g. 1000+ torrents)
*/
...
if (categoryIsLabel) {
    ...
    dom.tag('form', box)[0].addEventListener('submit', function (e) {
        ...
        if (searchString) interceptSearch();
...
function startSearch() {
    ...
    function addSelectedToCollage() {
        var urls = [];
        var base = 'https://redacted.sh/torrents.php?id=';
        var elems = dom.qsa('.group input', table);
        for (var i = elems.length; i--; ) {
            ...
            return function () {
                ...
                if (groups.length) {
                    ...
                    for (var i = 0, il = groups.length; i < il; i++) {
                        ...
                        if (!foundGroups[id] && !isDupe(groups[i].groupId)) {
...
/*
December 2019; additional code to deal with paginated collages
July 2025; some changes to the below code to handle the new React-based collage pages
Some minor changes to dupe check code above to use the code below
*/
var existingGroups;

function interceptSearch() {
    // If needed, see interceptSearchOld() function below if this stops working
    // Get list of group IDs already in the collage

    // We can avoid having to make an API call for Case 1 and Case 2
    if (!document.querySelector('#pageslinksdiv') && document.querySelector('#coverart')) {
        // Case 1
        // If there is just one page of albums and the cover art is visible
        existingGroups = Array.from(
            document.querySelectorAll('#coverart li a')
        ).map(el => parseInt(el.getAttribute('href').split("id=")[1]));
    } else if (!document.querySelector('#pageslinksdiv') && !document.querySelector('#coverart')) {
        // Case 2
        // If there is just one page of albums and the cover art is not visible
        existingGroups = Array.from(
            document.querySelectorAll('.group.discog')
        ).map(el => parseInt(el.getAttribute('id').split("_")[1]));
    } else {
        // Case 3
        // Make an API call in all other scenarios to populate existingGroups
        interceptSearchOld();
        return;
    }
...
function isDupe (id) {
    // Checks if the specified group is already in the collage or not
    return existingGroups.indexOf(id) > -1;
}

// Old 1.0.3 collage API call
// Still here in 1.0.4+ in case the other logic from interceptSearch() stops working
// Can replace interceptSearch() entirely if there are issues with it in future
function interceptSearchOld () {
    var edit = document.querySelector("a[href*='action=edit&collageid=']").getAttribute("href");
    var collageid = edit.split('&collageid=')[1];
    ...
    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'https://redacted.sh/ajax.php?action=collage&id=' + collageid + '&showonlygroups');
    xhr.onload = function() {
        var jsonResponse = JSON.parse(this.responseText);
        existingGroups = jsonResponse.response.torrentGroupIDList.map(Number);
        startSearch();
    };
    xhr.onerror = function() {
        alert("There was a problem with accessing the collage API.");
    };
    xhr.send();
}
