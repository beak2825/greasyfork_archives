...
// @name         動畫瘋 - 畫質
// @version      6
// @description  提升(?)動畫瘋畫質
// @match        https://ani.gamer.com.tw/animeVideo.php?sn=*
// @grant        none
// @namespace 99pe4dkz
// @downloadURL https://update.greasyfork.org/scripts/389113/%E5%8B%95%E7%95%AB%E7%98%8B%20-%20%E7%95%AB%E8%B3%AA.user.js
// @updateURL https://update.greasyfork.org/scripts/389113/%E5%8B%95%E7%95%AB%E7%98%8B%20-%20%E7%95%AB%E8%B3%AA.meta.js
// ==/UserScript==

(function() {

// From https://github.com/NeuroWhAI/Anime4K/blob/feat-web/web/main.js
...
Scaler.prototype.resize = function(scale) {
    ...
    gl.canvas.style.width = this.inputWidth * scale + 'px';
    gl.canvas.style.height = this.inputHeight * scale + 'px';

    const devicePixelRatio = window.devicePixelRatio || 1;

    const width = Math.round(this.inputWidth * scale * devicePixelRatio);
    const height = Math.round(this.inputHeight * scale * devicePixelRatio);
...
async function main() {
  ...
  const scaler = new Scaler(gl);

  let savedScale = null;
  function fit(force=false) {
    const {width, height} = video.getBoundingClientRect();
    const scale = Math.min(
      width / scaler.inputWidth,
      height / scaler.inputHeight);
    if (isFinite(scale) && (force || scale !== savedScale)) {
      savedScale = scale;
      scaler.resize(scale);
    }
  }
