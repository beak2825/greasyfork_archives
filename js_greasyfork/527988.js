...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/527988/%E4%BA%9A%E9%A9%AC%E9%80%8A%E8%AF%84%E8%AE%BA%E6%89%B9%E9%87%8F%E5%AF%BC%E5%87%BA11111111.user.js
// @updateURL https://update.greasyfork.org/scripts/527988/%E4%BA%9A%E9%A9%AC%E9%80%8A%E8%AF%84%E8%AE%BA%E6%89%B9%E9%87%8F%E5%AF%BC%E5%87%BA11111111.meta.js
// ==/UserScript==
...
(function() {
    ...
    const CONFIG = {
        ...
        BASE_DELAY: 500,        // 基础延迟时间（毫秒），减小延迟加快速度
    ...
    function addControlPanel() {
        ...
        const btnTerminate = document.createElement('button');
        btnTerminate.textContent = '终止采集并导出';
        btnTerminate.style = 'background: #ff3333; color: white; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; margin-left: 10px;';
        btnTerminate.onclick = terminateAndExport;
        ...
        elapsedTimeEl.style = 'margin-top: 5px; font-size: 12px; color: #666;';

        panel.appendChild(btnStart);
        panel.appendChild(btnPause);
        panel.appendChild(btnResume);
        panel.appendChild(btnTerminate);
        ...
        panel.appendChild(elapsedTimeEl);
        document.body.appendChild(panel);

        state.btnPause = btnPause;
        state.btnResume = btnResume;
        state.btnTerminate = btnTerminate;
        state.elapsedTimeEl = elapsedTimeEl;
    }
    ...
    function togglePause() {
        ...
        if (state.isPaused) {
            ...
            updateStatus('采集已暂停...');
            clearInterval(state.timer);  // 暂停计时
        } else {
            state.btnPause.disabled = false;
            state.btnResume.disabled = true;
            updateStatus('继续采集...');
            state.startTime = Date.now() - state.elapsedTime * 1000; // 继续采集时恢复计时
            startTimer(); // 继续计时
            processASINs();
        }
    }
    ...
    // 终止采集并导出
    function terminateAndExport() {
        state.isTerminated = true;  // 标记为终止状态
        updateStatus('采集已终止，开始导出数据...');
        exportToExcel(state.allReviews);
        clearInterval(state.timer);  // 停止计时
        state.btnTerminate.disabled = true; // 禁用“终止采集”按钮
    }
    ...
    function updateElapsedTime() {
        ...
        if (state.elapsedTimeEl) {
            state.elapsedTimeEl.textContent = `采集用时: ${timeString}`;
        }
    }
    ...
    function startTimer() {
        state.timer = setInterval(() => {
            state.elapsedTime = Math.floor((Date.now() - state.startTime) / 1000); // 计算已过去的时间（秒）
            updateElapsedTime();
        }, 1000);
    }
    ...
    async function startExportProcess() {
        ...
        state.startTime = Date.now(); // 设置开始时间
        state.elapsedTime = 0; // 初始化计时
        startTimer(); // 启动计时器
        updateStatus('开始导出...');
        updateProgress();
        await processASINs();
        exportToExcel(state.allReviews);
        updateStatus('导出完成！');
        clearInterval(state.timer); // 导出完成后停止计时
    }

    // 处理所有 ASIN
    async function processASINs() {
        while (state.currentASINIndex < state.asins.length && !state.isPaused && !state.isTerminated) {
    ...
    async function processStarRatings(asin) {
        for (const star of CONFIG.STARS) {
            ...
            while (hasMore && page <= CONFIG.MAX_PAGES && !state.isPaused && !state.isTerminated) {
                const pageSuccess = await processPage(asin, star, page);
                if (!pageSuccess) {
                    updateStatus(`第 ${page} 页没有评论数据，跳过后续页面...`);
                    document.getElementById('export-status').textContent = `跳过 ${star} 星级的后续页面...`; // 显示简短说明
                    break; // 退出循环，不再抓取后续页
                }

                promises.push(processPage(asin, star, page));  // 并发处理请求

                if (promises.length >= CONFIG.MAX_CONCURRENT_REQUESTS) {  // 控制最大并发请求数
                    await Promise.all(promises);  // 等待并发请求完成
                    promises = [];  // 清空当前请求队列
                }
                page++;
            }
    ...
    async function processPage(asin, star, page) {
        ...
        while (retries < CONFIG.MAX_RETRIES) {
            try {
                ...
                if (reviews.length === 0) {
                    return false; // 如果当前页没有数据，返回false
                }

                state.allReviews.push(...reviews);
                updateProgress();
                return true;  // 如果成功获取评论，返回true
            } catch (error) {
                console.error(`第 ${page} 页请求失败 (尝试 ${retries + 1}/${CONFIG.MAX_RETRIES}):`, error);
                retries++;
                await randomDelay(1000);  // 减少延迟
            }
        }
        return false; // 重试多次后依然失败，返回false
    }
    ...
    function randomDelay(base = CONFIG.BASE_DELAY) {
        const delay = base + Math.random() * 500;  // 降低随机延迟
