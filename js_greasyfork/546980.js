...
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/546980/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29%20%5BMobile-Ready%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/546980/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29%20%5BMobile-Ready%5D.meta.js
// ==/UserScript==
...
- Touch-friendly floating buttons ("FLAM" and "⚙") are pinned bottom-right on mobile,
  top-right on desktop. Both are always visible (but now only on real user list pages).
...
AD / NON-LIST VISIBILITY CHANGE:
- Added @noframes so nothing runs inside ad iframes.
- Added strict page detection so the floating FLAM + Settings buttons only appear on
  canonical user list pages like /username/list/list-slug/ (and their /page/N/ pages).
...
(async function () {
  ...
  // Extra hardening: if somehow running in a frame, bail early (redundant with @noframes)
  ...
  // NEW: Strict matcher for *real* user list pages only:
  // Accepts /username/list/list-slug/ and optional pagination /page/N/
  function isUserListPageStrict() {
    const PATH = location.pathname.endsWith('/') ? location.pathname : (location.pathname + '/');
    // ^/<username>/list/<list-slug>/(optional page/N/)?$
    return /^\/[^/]+\/list\/[^/]+\/(?:page\/\d+\/)?$/.test(PATH);
  }

  // Central toggle: should we render the floating FAB on this page?
  function shouldShowFab() {
    if (window.top !== window.self) return false;  // never in iframes (ads)
    return isUserListPageStrict();
  }
  ...
  // ─── Floating buttons (FLAM, Settings) — now ONLY on real user list pages ─
  if (shouldShowFab()) {
  ...
  // Initialize state on load (FAB only appears on strict list pages now)
  // The FAB creation is gated above by shouldShowFab().
