// ==UserScript==
// @name           S-5556 calculator TI.asi.banc current & future
// @version        6.56
// @description    simulator, future. asi, banc
// @include			http://trophymanager.com/players/*
// @include			https://trophymanager.com/players/*
// @namespace https://greasyfork.org/users/192839
// @downloadURL https://update.greasyfork.org/scripts/369770/S-5556%20calculator%20TIasibanc%20current%20%20future.user.js
// @updateURL https://update.greasyfork.org/scripts/369770/S-5556%20calculator%20TIasibanc%20current%20%20future.meta.js
// ==/UserScript==
...
if (location.href.indexOf("/players/") != -1){
	...
	function CalcMaxPrice(age, si) {
		...
		if (year >= 18)
			return addCommas(Math.round(factor * si));
		else
			return "-";
	}
  
  function CalcBankPrice(TotalMonths, si) {
		...
		if (year >= 18)
    {
      ...
      return addCommas(Math.round(si * 500 * Math.pow(ageD, 2.5)));
    }
		else
			return "-";
  }
  ...
  function CalcAgeMonths(age) {
	...
	rectMaxPrcV.innerHTML = CalcMaxPrice(gettr[2].getElementsByTagName("td")[0].innerHTML, SI);
    rectBankPrc.innerHTML = "<d style=\"color: white;\">Bank Price</d>";
	rectBankPrcV.innerHTML = CalcBankPrice(AgeMonths, SI);
