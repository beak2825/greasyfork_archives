...
// @downloadURL https://update.greasyfork.org/scripts/393313/WME%20Norway%20-%20Map%20Overlay%2020.user.js
// @updateURL https://update.greasyfork.org/scripts/393313/WME%20Norway%20-%20Map%20Overlay%2020.meta.js
// ==/UserScript==
...
var providerSpreadsheetListUrl = "https://spreadsheets.google.com/feeds/list/1z3VhSIl9JXU8F98yRCvQFd1RCQTUDHi0KRy9gXIBytg/od6/public/values";
...
function NorOverlay_Bootstrap() {
    ...
    if (!_waze || !_waze.map || !_waze.loginManager.isLoggedIn() || !Nor) {
        ...
        setTimeout(NorOverlay_Bootstrap, 2000);
        return;
    }
...
function NorOverlay_Init() {
    ...
    NorOverlay.populateProviders = function () {
        this.getProviders((providers) => {
            ...
            $.each(providers, function () {
                ...
                // Add shortcut
                var shortcut;
                shortcut = new WazeWrap.Interface.Shortcut("toggleMapOverlay" + this.id, "Show overlay: " + this.provider, "WME Norway", "WME Norway", 'A', function () { NorOverlay.showOverlay(this.id)}, this)
                shortcut.add();
            });
    ...
    NorOverlay.getCoordinates = function(img) {
        ...
        var coords;
        var re = /\/\/www\.googleapis\.com\/tile\/v1\/tiles\/(\d+)\/(\d+)\/(\d+)/g;
        var match = re.exec(satelliteTileUrl);
    ...
    NorOverlay.setImages = function() {
        //log("setImages start!");
        var innerTilesContainer = $('#WazeMap #OpenLayers_Layer_Google_231');
        //log("innerTilesContainer: " + innerTilesContainer);
        innerTilesContainer.children('img.overlayGMM').each(function () {
            var default_url = $(this).attr('data-default_url');
            log("innerTilesContainer: " + default_url);
    ...
    NorOverlay.clearOverlay = function() {
        $('.overlayGMM', $('#WazeMap #OpenLayers_Layer_Google_231')).remove();
    }
...
setTimeout(NorOverlay_Bootstrap, 5000);