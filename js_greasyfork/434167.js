...
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/434167/JDY_Scroll.user.js
// @updateURL https://update.greasyfork.org/scripts/434167/JDY_Scroll.meta.js
// ==/UserScript==
...
setTimeout(function(){
    var scroll_speed = 100; //滚动速度，越小越快
    ...
    var table_height = web_height-60-12-10-5-10-2-5-47-6+54+123; //表格显示高度
...
font-size: 20px;/*表头及表格文字大小，已失效*/
padding: 9px 10px;
line-height: 28px;
}
.x-virtual-table .virtual-table-wrapper table td, .x-virtual-table .virtual-table-wrapper table th {
font-size: 20px;/*表头文字大小*/
padding: 9px 10px;
line-height: 28px;
}
.config-row-height-table .table-body td .cell-wrapper, .config-row-height-table .table-body td.table-cell, .config-row-height-table .x-table-body td .cell-wrapper, .config-row-height-table .x-table-body td.table-cell {
...
.x-divider.divider-vertical {
display:none;/*顶部竖分割线去除*/
}
.fx-entry-title {
max-width: 430px;/*标题文字显示宽度扩大*/
}
.fx-entry-title .entry-title {
font-size:24px;/*仪表盘标题文字大小*/
}
.header-title {
...
//scroll start
    ...
    var my_time;
    var stopframe = 0;
    var objDiv = document.getElementsByClassName("x-virtual-table")[0]; //滚动明细表的class，x-table-body失效
