// ==UserScript==
// @name         Tekstredactie Home van Lukas
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/556377/Tekstredactie%20Home%20van%20Lukas.user.js
// @updateURL https://update.greasyfork.org/scripts/556377/Tekstredactie%20Home%20van%20Lukas.meta.js
// ==/UserScript==
...
sendListRow.each(function() {
    var row = $(this);
    styleSendlistRow(row); // Assuming this function exists elsewhere and is needed
    ...
    var isGrafischOrange = false; // <<< NEW VARIABLE
    ...
    // Loop through all columns (<td>) except the first and last one (which are not the service columns)
    row.find('td > table > tbody > tr > td:not(:first, :last)').each(function() {
        var $col = $(this);
        $col.attr('width', '10%');

        // Check for the existing 'afgekeurd' color
        if ($col.attr("bgcolor") == "#FFC7CE") {
            afgekeurdCount++;
        }

        // <<< NEW LOGIC START: Check for the 'Grafisch' column and its orange color
        // Identify the 'Grafisch' column by the presence of a specific image/link path or text
        // The HTML suggests the link inside contains 'picture_upload.php'
        var $grafischLink = $col.find('a[href*=picture_upload]');

        if ($grafischLink.length > 0) {
            // This column is the 'Grafisch' column. Now check its background color.
            if ($col.attr("bgcolor") === "#ffa500") {
                isGrafischOrange = true;
            }
        }
        // <<< NEW LOGIC END
    });
    ...
    // Update the row attributes, including the new one
    row.attr({
        "class": "sendListRow",
        "data-dealid": dealid,
        "data-isCheckDeal": isCheckDeal,
        "data-isAfgekeurd": isAfgekeurd,
        "data-isGrafischOrange": isGrafischOrange // <<< NEW ATTRIBUTE
    });
...
$(document).ready(function() {
    ...
    // The filter array length will be 3 now (Afgekeurd, Checkdeal, GrafischOrange)
    arr.forEach(function(checked, i) {
      $('.fltrCheckbox').eq(i).prop('checked', checked);
    });

    var fltrAfgekeurd = $("#fltrAfgekeurd").is(':checked');
    var fltrCheckdeal = $("#fltrCheckdeal").is(':checked');
    var fltrGrafischOrange = $("#fltrGrafischOrange").is(':checked'); // <<< NEW LINE

    // Pass the new variable to the filter function
    filterSendList(fltrAfgekeurd, fltrCheckdeal, fltrGrafischOrange);
});

//SENDLISTFILTER: FILTERCODE (Click handler)
$(document).ready(function() {
    // Add the new checkbox ID to the click listener
    $("#fltrAfgekeurd, #fltrCheckdeal, #fltrGrafischOrange").click(function() { // <<< UPDATED LINE
...
function filterSendList(fltrAfgekeurd, fltrCheckdeal, fltrGrafischOrange) { // <<< UPDATED LINE
    $.each($('.sendListRow'), function() {
        ...
        if ($this.attr('data-isgrafischorange') == "true") { // <<< NEW ATTRIBUTE CHECK
            isGrafischOrange = true;
        }
        ...
        // Current filters: Hide if (Afgekeurd AND fltrAfgekeurd) OR (Checkdeal AND fltrCheckdeal)
        if (fltrAfgekeurd && isAfgekeurd) {
            shouldBeHidden = true;
        }
        if (fltrCheckdeal && isCheckdeal) {
            shouldBeHidden = true;
        }

        // <<< NEW FILTER LOGIC: Hide if 'Grafisch Orange' is true AND the new filter is checked
        if (fltrGrafischOrange && isGrafischOrange) {
            shouldBeHidden = true;
        }
  ...
  async function insertVlaggenTable() {
    ...
    const vlagdeal = document.querySelectorAll('#vlagTable > tbody > tr');
      ...
      vlagdeal.forEach(function(vlagdeal) {
          ...
          // SAFELY fix image
          const img = vlagdeal.querySelector('img[src*="https://media.socialdeal.nl/img/"]');
          if (img) img.style.borderRadius = "10px";

          // SAFELY insert pill row
          const firstRow = vlagdeal.querySelector('tbody > tr');
          if (firstRow) {
              firstRow.insertAdjacentHTML("afterend",
                                          '<tr class="pill-container-row" style="display: none;"><td align="left" colspan="3"></td></tr>');
          }

          // SAFELY parse dealid
          const link = vlagdeal.querySelector('a[href*=deal_aanmaken]');
          if (link) {
              const dealid = new URLSearchParams(link.search).get("id");
              if (dealid) {
