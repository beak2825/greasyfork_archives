...
// @grant        GM_addStyle
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/550357/%E5%9F%BA%E4%BA%8E%E5%BC%B9%E5%B9%95%E8%AF%86%E5%88%AB%E7%9A%84%E8%B7%B3%E8%BF%87B%E7%AB%99%E5%86%85%E7%BD%AE%E5%B9%BF%E5%91%8A%EF%BC%88%E6%94%B9%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/550357/%E5%9F%BA%E4%BA%8E%E5%BC%B9%E5%B9%95%E8%AF%86%E5%88%AB%E7%9A%84%E8%B7%B3%E8%BF%87B%E7%AB%99%E5%86%85%E7%BD%AE%E5%B9%BF%E5%91%8A%EF%BC%88%E6%94%B9%EF%BC%89.meta.js
// ==/UserScript==
...
const CONFIG = {
  ...
  rightStepSeconds: 5,
  rightMaxN: 10,
  forbiddenTokens: ['+','%','年','月','日','人','个','比','对','W','万','K','千','百','前','M','B','G','w','kb','k','a','b','d','g','T','1 1','点点','一点','1点','hz','p','os','处理器','CPU','GPU','i','最多','最少','不如'],
  measurementTokens: ['米','码','百米','速度','跑','m','km','km/h','秒','公斤','kg','斤','票','票房','分数','评分','积分','铀','分钟前','百分比','%','rpm','W','万','K','千','百','M','B','G','点点','点血','个','只','架','块','快','元'],
  maxNonTimeCharsAllowed: 3,
  nonTimeCharsPenaltyFactor: 0.2,
  acceptWeightThreshold: 0.5,
  chineseNumberMaxParseLen: 6,
  fastForwardBoost: 2.5,
  POSTAGE_WINDOW_SECONDS: 25,
  POSTAGE_TARGET_SECONDS: 60,
  TARGET_GROUP_WINDOW: 4,

  // 低权重特殊数字弹幕
  lowWeightTokens: ['9.9','111','555','233','444','333','222','911','0721','250','38','711','1024','2048','315','918','123','1.0','1.5','2.0','2.5','3.0','4.0','5.0','6.0','7.0','8.0','9.0','100','200','300','400','500','600','110','114514'],
...
/* ========== 关键词 ========== */
const TIME_KEYWORDS = ['跳伞','跳','快进','空降','跳过','跳至','快进到','不用谢','加速','向右','右','→','朗','郎','侠','绯红之王','向右下','右下'];
const WEIGHT_A_KEYWORDS = ['0帧起手','零帧起手','丝滑','0帧','变声期','触发连招','回马枪','猝不及防','警告','不好！','加速时间','500','666','要素察觉','广告','广告跳过','不想看','广告点','起招','起手帧','感谢甲方','金主','恭喜恰饭','高能预警','妙界','转转','新换的','旧的扔了','神奇小鹿','怎么办呢','贴脸开大'];
...
function createUI(){
  ...
  root.style.cssText = 'position:fixed;top:288px;right:28px;z-index:2147483647;font-family:Microsoft YaHei,Arial;';
  ...
  panel.style.cssText='display:none; width:420px; max-width:92vw; min-width:300px; background:rgba(0,10,26,0.95); color:#fff; border:2px solid #00a1d6; border-radius:10px; padding:12px; box-shadow:0 10px 40px rgba(0,0,0,0.6);';
  panel.innerHTML = `
    ...
    <div id="bili-log" style="max-height:250px;overflow:auto;font-size:13px;padding:8px;border-radius:6px;background:rgba(0,0,0,0.2);"></div>
  `;
...
function isScoreContext(text){
  ...
  const scoreKw = ['满分','评分','分数','打分','得分','多少分','评分为','分数是','给分','积分','给我分','给他分'];
...
function isPostAgeContext(text){
  ...
  const kws = ['发布一分钟','发布于','发布后','刚出炉','刚发布','刚刚发布','刚发布','分钟前','刚出','第一分钟','刚刚','发布','赛雷','塞雷','新鲜','热乎'];
...
document.addEventListener('visibilitychange', function() {
  if (document.visibilityState === 'visible' && state.waitingForVisible) {
    addLog('页面已切换到前台，尝试继续执行');
    state.waitingForVisible = false;
    if (state.pendingItems && state.pendingRunId) {
      // 添加短暂延迟确保页面完全激活
      setTimeout(() => {
        analyzeItems(state.pendingItems, state.pendingRunId);
      }, 300);
    }
  }
});// 页面可见性变化监听

function analyzeItems(items, runIdLocal, retryCount = 0) {
try{
     if (document.visibilityState !== 'visible') {
      addLog('页面在后台，等待切换到前台...');state.waitingForVisible = true;state.pendingItems = items;state.pendingRunId = runIdLocal;return;}
    ...
    // 尝试获取视频长度
    let videoDuration = state.video.duration || 0;

    // 如果视频长度为0，尝试等待并重试
    if (!videoDuration || videoDuration <= 0) {
      if (retryCount < 3) { // 最多重试3次
        const delay = Math.pow(2, retryCount) * 500; // 指数退避：500ms, 1000ms, 2000ms
        addLog(`无法获取视频长度，${delay}ms后重试 (${retryCount + 1}/3)`);
        setTimeout(() => {
          analyzeItems(items, runIdLocal, retryCount + 1);
        }, delay);
        return;
      } else {
        addLog('多次尝试后仍无法获取视频长度，终止分析');
        state.isAnalyzing = false;
        updateMiniStatus(false);
        return;
      }
    ...
    const colonRegex = /([0-9０-９零一二两三四五六七八九十百]{1,3})\s*[:：∶,点]\s*([0-9０-９零一二两三四五六七八九十百]{2,3})/g;
    ...
    const spaceSeparatedRegex = /(?<!\d)([0-9０-９零一二两三四五六七八九十百]{1,3})\s+([0-9０-９零一二两三四五六七八九十百]{1,3})(?!.*[0-9])/g;
    const dotSeparatedRegex = /([0-9０-９零一二两三四五六七八九十百]{1,3})\s*\.\s*([0-9０-９零一二两三四五六七八九十百]{2})(?!\d)/g;
    const contiguous3Regex = /(?<!\d)([0-9０-９零一二两三四五六七八九十百]{3})(?![\d？?》])/g;
    const contiguous4Regex = /(?<!\d)时间([0-9０-９零一二两三四五六七八九十百]{4})(?![\d？?》])/g;
    const arrowRightRegex = /(右.?)\s*([0-9０-９零一二两三四五六七八九十百]{1,3})\s*(下)?/i;
    const dianRegex = /时间([0-9０-９零一二两三四五六七八九十百]{1,3})\s*(?:点|点钟)\s*([0-9０-９零一二两三四五六七八九十百]{1,3})?(?:\s*(?:分|分钟))?(?:\s*([0-9０-９零一二两三四五六七八九十百]{1,3})\s*秒)?/g;
    ...
    for(const it of items){
      ...
      if(!containsTimeKw && raw.length <= 6) baseW = Math.max(0.1, baseW * 0.5);
      ...
      while((m = colonRegex.exec(raw)) !== null){
        ...
        rawCandidates.push({trigger: it.time, target, text: orig, weight: baseW + 0.7, reasons:[`冒号 ${m[0]}`]});
      }
      ...
      while((m = dianRegex.exec(raw)) !== null){
        ...
        rawCandidates.push({trigger: it.time, target, text: orig, weight: Math.max(0.6, baseW), reasons:[`点格式 ${m[0]}`]});
      }
      ...
      while((m = dotSeparatedRegex.exec(raw)) !== null){
        ...
        if(isNaN(A)||isNaN(B)) continue; if(B>=60) continue;
        const target = A*60 + B; if(target>=videoDuration||target<0) continue;
        rawCandidates.push({trigger: it.time, target, text: orig, weight: Math.max(0.7, baseW + 0.2), reasons:[`点号 ${m[0]}`]});
      }
      ...
      while((m = contiguous4Regex.exec(raw)) !== null){
        ...
        rawCandidates.push({trigger: it.time, target, text: orig, weight: baseW - 0.2, reasons:[`连续4 ${numStr} -> ${mm}:${ss}`]});
      }
    ...
    for(const c of rawCandidates){
      ...
      if (A < 30) {addLog(`排除候选（触发时间 < 30s）: "${c.text}" ;${formatTime(A)} → ${formatTime(B)} ; A=${A.toFixed(3)}s`);continue; }
    ...
    if(top.weightSum < CONFIG.acceptWeightThreshold){
      addLog(`最高候选权重 ${top.weightSum.toFixed(2)} < 阈值 ${CONFIG.acceptWeightThreshold.toFixed(2)} ，拒绝所有候选`);
