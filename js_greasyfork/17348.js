...
// @namespace DFHL
// @include http://hockey.fantasysports.yahoo.com/hockey/507*
// @description Heavily modified farm roster script originally written by Glenn Carr
// $LastChangedRevision: 0 $
// $LastChangedDate: 2016-02-23 $
// @grant GM_xmlhttpRequest
// @grant GM_getValue
// @grant GM_addStyle
// @version 1.3
// @downloadURL https://update.greasyfork.org/scripts/17348/DFHL%20Farm%20Script.user.js
// @updateURL https://update.greasyfork.org/scripts/17348/DFHL%20Farm%20Script.meta.js
// ==/UserScript==
...
//handle player data function end
    ...
    function handlePlayerData( responseText, player, playergncID ) //function that fills in stats
    {
        ...
        var numPosLoc = gSpan.getElementsByClassName('team-info');
        ...
        var hbEnd = '</td>';
        //bio fill
        var bioLoc = gSpan.getElementsByClassName('bio');
        var bioTags = bioLoc[0];
        var bioType = bioTags.getElementsByTagName('dt');
        var bioValue = bioTags.getElementsByTagName('dd');
        var bioTypeRow = '';
        var bioValueRow = '';
        for (var iBio = 0; iBio < bioType.length; iBio++)
        {
            //console.dir(bioType[iBio].innerHTML + ' ' + bioValue[iBio].innerHTML);
            var bioTypeHtml = hbStart + bioType[iBio].innerHTML + hbEnd;
            bioTypeRow += bioTypeHtml;
            var bioValueHtml = hbStart + bioValue[iBio].innerHTML + hbEnd;
            bioValueRow += bioValueHtml;   
        }
        var bioHid = 'bioH' + playergncID;
        var bioVid = 'bioV' + playergncID;
        document.getElementById(bioHid).innerHTML = bioTypeRow;
        document.getElementById(bioVid).innerHTML = bioValueRow;
        //end bio fill
        //new stat fill---------------------------------------------------------------------------------------------------------
        var changeTd = 'cH' + playergncID;
        var getTd = document.getElementById(changeTd);
        getTd.style.height = "50px";
        var statInfo = gSpan.getElementsByClassName( 'stat-info' );
        var statHtml = statInfo[0];
        if (statHtml != undefined)
        {
            var getStatType = statHtml.getElementsByTagName('dt');
            var getStatValue = statHtml.getElementsByTagName('dd');
            var statHeader = '';
            ...
            var hStart = '<td>';
            var hEnd = '</td>';
            for ( var iStat=0; iStat < getStatType.length; iStat++ )
            {
                var statCat = getStatType[iStat].innerHTML
                var statRow = hStart + statCat + hEnd;
                statHeader += statRow;
                var statValue = getStatValue[iStat].innerHTML;
            ...
            var getLog = gSpan.getElementsByClassName('yom-data yom-data-small yom-sports-stats-table phatable');
            var logTest = getLog[0];
            var logHtml;
            if (logTest == undefined)
            {
                getLog = gSpan.getElementsByClassName('yom-data yom-data-small yom-sports-stats-table');                
                logHtml = getLog[1].innerHTML;
                getLogRow.innerHTML = logHtml;
                getLogRow.removeChild(getLogRow.childNodes[1]);
                var bgCol = getLogRow.getElementsByClassName('gamelog');
        ...
        //end new stat fill---------------------------------------------------------------------------------------------------------        
        //team logo, note: no test here to see if exists, it should at this point
        var logoLoc = gSpan.getElementsByClassName( 'team-logo' );
        var logoUrl = logoLoc[0].style.backgroundImage;
        var logoId = 'logo' + playergncID;
        document.getElementById(logoId).style.backgroundImage = logoUrl;
        //player pic---------------------------------------------------------------------------------------------------------
        var picLoc = gSpan.getElementsByClassName( 'photo' );
        ...
        //end player info
        //fill career games
        var gsLoc = gSpan.getElementsByClassName('nhl-stat-type-18 stat-total');
        var gpLoc = gSpan.getElementsByClassName('nhl-stat-type-101 stat-total');
        var gamesStarted = null;
        var gamesPlayed = null;
        if ( gsLoc[0] != undefined )
        {
            gamesStarted = gsLoc[0].innerHTML;
            gamesStarted = parseInt( gamesStarted, 10 );
            player.dataCell.innerHTML = '<p style="text-align: center;">' + gamesStarted + ' career games</p>';
        }
        else if ( gpLoc != undefined )
        {
            gamesPlayed = gpLoc[0].innerHTML;
