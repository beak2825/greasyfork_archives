// ==UserScript==
// @license guap
// @name         Klavia Frenzy Clicker 0.1.3.5 Backup
...
// @downloadURL https://update.greasyfork.org/scripts/561090/Klavia%20Frenzy%20Clicker%200135%20Backup.user.js
// @updateURL https://update.greasyfork.org/scripts/561090/Klavia%20Frenzy%20Clicker%200135%20Backup.meta.js
// ==/UserScript==
...
coinsPerSecond = generators.filter(g => g.purchased).reduce((sum, g) => sum + g.cps, 0);
    ...
    let upgrades = [
        ...
        { id: 8, purchased: JSON.parse(localStorage.getItem('upgrade8Purchased')) || false, type: 'totalRaceBonus', value: 5, cost: 196, description: "Adds +5 CpR every 25 total races" },
...
if (luckyUpgrade && luckyUpgrade.purchased) {
    ...
    let myPets = JSON.parse(localStorage.getItem('ownedPets') || '[]');
    myPets.forEach(pet => {
        coinsPerRace += coinsPerRace * (pet.multiplier / 100);
    });
...
generators.forEach((gen, index) => {
    ...
    headerElement.textContent = expanded ? 'Klavia Frenzy Clicker 0.1.3.5' : 'KFC';
            syncHUDs();
});
...
function setupRacesListener() {
    ...
    database.ref('recentRaces').limitToLast(10).on('value', (snapshot) => {
        racesContent.innerHTML = '';
        const data = snapshot.val();
        if (!data) return;

        const sortedRaces = Object.values(data).sort((a, b) => b.timestamp - a.timestamp);

        sortedRaces.forEach(race => {
            const raceDiv = document.createElement('div');
            raceDiv.style.cssText = `margin-bottom: 4px; border-bottom: 1px solid #222; padding-bottom: 2px;`;
            ...
            getRoleStyles(race.user, (tag, tagColor, nameColor) => {
                ...
                if ((roleEntry && roleEntry.isRGB) || tag === '[RGB]') {
                    ...
                    if (race.user === 'guappanese') {
                        nameSpan.style.color = '#f00';
                    } else {
                        nameSpan.style.color = nameColor || '#0f0';
                    }
                }
            });

            const infoSpan = document.createElement('span');
            infoSpan.textContent = ` : ${race.time}s`;
            infoSpan.style.color = '#888';

            raceDiv.appendChild(nameSpan);
            raceDiv.appendChild(infoSpan);
...
function setupRacesListener() {
    ...
    database.ref('recentRaces').limitToLast(50).on('value', (snapshot) => {
        ...
        const uniqueEntries = Object.values(latestPerUser).sort((a, b) => b.timestamp - a.timestamp);

        uniqueEntries.forEach(race => {
            const raceDiv = document.createElement('div');
            raceDiv.style.marginBottom = '3px';

            getRoleStyles(race.user, (tag, tagColor, nameColor) => {
                const timeText = timeAgo(race.timestamp);
                raceDiv.innerHTML = `<span style="color:${nameColor}; font-weight:bold;">${race.user}</span>: <span style="color:#aaa;">${timeText}</span>`;
            });
...
function checkStoreHUD() {
        ...
        tabNames.forEach(name => {
            ...
            if (name === 'Tags') {
                ...
                const refreshTagsUI = () => {
                    ...
                    ownedList.innerHTML = '';
                    const tagPrices = { 'LEGEND': 100, 'TYPER': 10000, 'CPR GOD': 100000, 'STARBIT': 250000, 'RGB': 500000, 'GUAP': 1000000 };
                    const rainbowColors = ['#ff0000', '#ff7f00', '#ffff00', '#00ff00', '#0000ff', '#4b0082', '#8b00ff'];
                    let ownedTags = JSON.parse(localStorage.getItem('ownedTags') || '[]');
                    const currentActive = localStorage.getItem('userRole');

                    Object.keys(tagPrices).forEach(roleKey => {
                        if (ownedTags.includes(roleKey)) return;
                        const roleData = window.rolesConfig[roleKey];
                        if (!roleData) return;
                        const price = tagPrices[roleKey];

                        const tagBtn = document.createElement('div');
                        tagBtn.style.cssText = `padding: 8px; border: 1px solid #0f0; border-radius: 4px; cursor: pointer; text-align: center; font-weight: bold; font-size: 11px;`;
                        ...
                        tagBtn.onclick = () => {
                            let coins = parseInt(localStorage.getItem('klaviaCoins') || '0', 10);
                            if (coins >= price) {
                                localStorage.setItem('klaviaCoins', coins - price);
                    ...
                    ownedTags.forEach(roleKey => {
                        ...
                        miniTag.onclick = () => { localStorage.setItem('userRole', roleKey); refreshTagsUI(); };
            ...
            if (name === 'Pets') {
                ...
                const refreshPetsUI = () => {
    eggList.innerHTML = ''; inventory.innerHTML = '';
    ...
    window.eggsConfig.forEach(egg => {
        ...
        if (!isDone) {
            ...
            eggBtn.onclick = () => {
                let coins = parseInt(localStorage.getItem('klaviaCoins') || '0', 10);
                if (coins >= egg.price) {
                    localStorage.setItem('klaviaCoins', coins - egg.price);
                    ...
                    ownedPets.push({ name: chosen, multiplier: pData.multiplier, rarity: pData.rarity });
    ...
    const isRGBPet = pData && pData.rarity === 'Divine'; // Or check pData.isRGB if you add that
    ...
    if (isRGBPet) {
        ...
        p.name.split('').forEach((char, i) => {
            ...
            charSpan.style.textShadow = `0 0 3px ${color}`;
