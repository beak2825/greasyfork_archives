...
// @name VAPE Client
// @author Murka
// @description Sub To RektByMateX
// @version vVisualUpdate
// @match *://sploop.io/*
// @run-at document-start
// @grant none
// @icon         https://cdn.discordapp.com/attachments/664899301247287298/878019963900817448/743a8ebae02a90a9b147c97fb03faff4.jpg
// @namespace NoobClient
// @downloadURL https://update.greasyfork.org/scripts/468329/VAPE%20Client.user.js
// @updateURL https://update.greasyfork.org/scripts/468329/VAPE%20Client.meta.js
// ==/UserScript==
/* jshint esversion:6 */
...
Function("(" + ((GM_info) => {
    ...
    (() => {
        ...
        const defaultSettings = {
            ...
            possibleShots: true,
            autochat: true,
            autochatMessages: [ "ae69420" ],
            kill: true,
            killMessage: ["{NAME} IS FKING GAY"],
            autospawn: false,
            smootZoom: false,
            skipUpgrades: false,
            ...
            blindUsers: [ 255, 255, 255 ]
        };
        const settings = {
            ...defaultSettings,
            ...Storage.get("VAPE-settings")
        };
        ...
        let autoplacer = false

        Storage.set("VAPE-settings", settings);
        const Settings = settings;
        class Formatter {
            static object(target) {
                const layer = LayerData[target.type];
                return {
                    id: target[VAPE.props.id],
                    type: target.type,
                    x: target[VAPE.props.x],
                    y: target[VAPE.props.y],
                    x1: target[VAPE.props.x1],
                    y1: target[VAPE.props.y1],
                    x2: target[VAPE.props.x2],
                    y2: target[VAPE.props.y2],
                    angle: target[VAPE.props.angle],
                    angle1: target[VAPE.props.angle1],
                    angle2: target[VAPE.props.angle2],
                    ownerID: target[VAPE.props.ownerID],
                    radius: layer.radius,
                    layerData: layer,
                    target
                };
            ...
            static projectile(target) {
                ...
                return {
                    ...object,
                    range: target.range,
                    projectileType: target[VAPE.props.projectileType]
                };
            }
            static entity(target) {
                const object = this.object(target);
                const healthValue = target[VAPE.props.health];
                const maxHealth = object.layerData.maxHealth || 1;
                const currenthealth = healthValue;
                return {
                    ...object,
                    healthValue,
                    ...
                    entityValue: target[VAPE.props.entityValue]
                };
            }
            static player(target) {
                ...
                return {
                    ...entity,
                    hat: target[VAPE.props.hat],
                    isClown: entity.entityValue === 128,
                    currentItem: target[VAPE.props.currentItem]
                };
            }
        }
        ...
        const GM = (property, value) => {
            if (!VAPE.PRODUCTION) return true;
            try {
        ...
        const resetSkin = () => {
            const player = VAPE.myPlayer.target;
            if (!player) return;
            delete player[VAPE.props.skin];
            delete player[VAPE.props.accessory];
            delete player[VAPE.props.back];
        };
        const updateSkin = () => {
            if (!Settings.customSkins) return resetSkin();
            const player = VAPE.myPlayer.target;
            if (!player) return;
            Common_define(player, VAPE.props.skin, Settings.skin);
            Common_define(player, VAPE.props.accessory, Settings.accessory);
            Common_define(player, VAPE.props.back, Settings.back);
        };
        const Scale = {
            Default: {
                w: 1824,
        ...
        const skins = {
            skin: [ [ "VAPE Classic", 0, 0 ], [ "Yellow Classic", 1, 0 ], [ "Brown Classic", 2, 0 ], [ "Pink Classic", 3, 0 ], [ "Blue Classic", 4, 0 ], [ "Green Classic", 5, 0 ], [ "White Cat", 6, 100 ], [ "Ginger Cat", 7, 100 ], [ "Pit Bull", 8, 150 ], [ "Pig", 9, 100 ], [ "Crocodile", 10, 200 ], [ "Fox", 11, 200 ], [ "Panda", 12, 300 ], [ "Bear", 13, 300 ], [ "Penguin", 14, 300 ], [ "Cactus", 15, 400 ], [ "Strawberry", 16, 800 ], [ "Wolf", 17, 400 ], [ "Mammoth", 18, 2e3 ], [ "Golden Cow", 19, 3e3 ], [ "Shark", 20, 1e3 ], [ "Apple", 21, 200 ], [ "Stone", 22, 500 ], [ "Cave Stone", 23, 600 ], [ "Ice", 24, 700 ], [ "Gold", 25, 800 ], [ "Cow", 26, 350 ], [ "Dragon", 27, 5e3 ], [ "Black Ice", 28, 1e3 ], [ "Magma", 29, 1500 ], [ "Kawak", 30, 2500 ], [ "Snowman", 31, 400 ], [ "Elf", 32, 1e3 ], [ "Green Bauble", 33, 300 ], [ "Red Bauble", 34, 300 ], [ "Golden Bauble", 35, 800 ], [ "Duck", 36, 300 ], [ "Devil", 44, 3e3 ], [ "Golden Beetle", 38, 1500 ], [ "Mummy", 45, 300 ], [ "Pumpkin", 46, 3e3 ], [ "Monster", 50, 3e3 ], [ "Night Wolf", 49, 3e3 ], [ "Pumpkin 2", 48, 3e3 ], [ "Pumpking 3", 47, 3e3 ], [ "Eye", 43, 3e3 ], [ "Reaper", 42, 3e3 ], [ "Witch", 41, 3e3 ], [ "Ghost", 40, 3e3 ], [ "Evil Bat", 39, 3e3 ], ],
            accessory: [ [ "None", 0, 0 ], [ "Mustache", 1, 100 ], [ "Sun Glasses", 2, 500 ], [ "Yellow Cap", 3, 0 ], [ "Blue Cap", 4, 0 ], [ "Purple Cap", 5, 0 ], [ "Green Cap", 6, 0 ], [ "Pink Bow", 7, 0 ], [ "3D Glasses", 8, 300 ], [ "Scar", 9, 150 ], [ "Turban", 10, 250 ], [ "Bandage", 11, 250 ], [ "Crazy Glasses", 12, 150 ], [ "Cow's Snout", 13, 300 ], [ "Carrot", 14, 150 ], [ "Horn", 15, 1e3 ], [ "Tusk", 16, 800 ], [ "Mammoth Hair", 17, 600 ], [ "Mammoth Ears", 18, 500 ], [ "Leaf", 19, 150 ], [ "Black Mustache", 20, 500 ], [ "Snowman Hat", 21, 1e3 ], [ "Blue Beanie", 22, 200 ], [ "Green Beanie", 23, 200 ], [ "Purple Beanie", 24, 200 ], [ "Orange Beanie", 25, 200 ], [ "Yellow Scarf", 26, 250 ], [ "Red Scarf", 27, 350 ], [ "Green Scarf", 28, 300 ], [ "Red Nose", 29, 400 ], [ "Mask", 30, 1e3 ], [ "Garlands", 31, 500 ], [ "Dog Fur", 32, 150 ], [ "Nails", 33, 150 ], [ "Horns", 34, 150 ], [ "Alien Eyes", 35, 150 ], [ "Exorbited", 36, 150 ], [ "Goblin Ears", 37, 150 ], [ "Brain", 38, 150 ], [ "Axe", 39, 150 ], [ "Hannibal", 40, 150 ], ],
            back: [ [ "None", 0, 0 ], [ "Mammoth Tail", 1, 500 ], [ "Dragon Wings", 2, 5e3 ], [ "Swords", 3, 2e3 ], [ "Blue Cape", 4, 400 ], [ "Christmas Cape", 5, 400 ], [ "Speedy Cape", 6, 400 ], [ "Garland", 7, 300 ], [ "Baby Elf", 8, 1500 ], [ "Gift", 9, 1e3 ], [ "Yellow Bag", 10, 300 ], [ "Cauldron", 11, 1e3 ], [ "Pumpkin", 12, 1e3 ], [ "Trident", 13, 1e3 ], [ "Vampire Cape", 14, 1e3 ], [ "Broom Stick", 15, 1e3 ], [ "Scythe", 16, 1e3 ], [ "Bat Wings", 17, 1e3 ], [ "Candies", 18, 1e3 ], [ "Skeleton", 19, 1e3 ], ]
        };
        const Skins = skins;
        ...
        const createMenu = () => {
            ...
            IFRAME.onload = () => {
                ...
                const autochatInputss = iframeDocument.querySelectorAll(".input.autochat2");
                const autochatInputsss = iframeDocument.querySelectorAll(".input.autochat3");
                const autochatInputssss = iframeDocument.querySelectorAll(".input.autochat4");
                const autochatInputsssss = iframeDocument.querySelectorAll(".input.autochat5");
                ...
                const update = () => {
                    ...
                    for (let i = 0; i < autochatInputss.length; i++) {
                        const input = autochatInputss[i];
                        input.value = Settings.autochatMessage2[i] || "";
                        input.onchange = () => {
                            Settings.autochatMessage2[i] = input.value;
                            Storage.set("VAPE-settings", Settings);
                            input.blur();
                        };
                    }
                    for (let i = 0; i < autochatInputsss.length; i++) {
                        const input = autochatInputsss[i];
                        input.value = Settings.autochatMessage3[i] || "";
                        input.onchange = () => {
                            Settings.autochatMessage3[i] = input.value;
                            Storage.set("VAPE-settings", Settings);
                            input.blur();
                        };
                    }
                    for (let i = 0; i < autochatInputssss.length; i++) {
                        const input = autochatInputssss[i];
                        input.value = Settings.autochatMessage4[i] || "";
                        input.onchange = () => {
                            Settings.autochatMessage4[i] = input.value;
                            Storage.set("VAPE-settings", Settings);
                            input.blur();
                        };
                    }
                    for (let i = 0; i < autochatInputsssss.length; i++) {
                        const input = autochatInputsssss[i];
                        input.value = Settings.autochatMessage5[i] || "";
                        input.onchange = () => {
                            Settings.autochatMessage5[i] = input.value;
                            Storage.set("VAPE-settings", Settings);
                            input.blur();
                        };
                    }
                    headerVersion.textContent = "v" + VAPE.version;
                    ...
                    VAPE.toggleMenu = () => {
                        ...
                        if (menuContainer.classList.toggle("close") && !popupOpened) {
                            popupCount += 1;
                            if ((popupCount %= 5) === 0) {
                                createPopup();
                            }
                ...
                downloadSettings.onclick = () => {
                    download(Settings, "VAPESettings" + VAPE.version);
                };
                ...
                const applyCode = code => {
                    if (!VAPE.active) return;
                    const key = code === "Backspace" ? "..." : formatCode(code);
                    Settings[VAPE.active.id] = code === "Backspace" ? "..." : code;
                    VAPE.active.textContent = key;
                    Storage.set("VAPE-settings", Settings);
                    VAPE.active = null;
                    checkForRepeats();
                };
                menuContainer.addEventListener("keyup", (event => {
                    if (event.keyCode < 5 || !VAPE.active) return;
                    applyCode(event.code);
                }));
                menuContainer.addEventListener("mouseup", (event => {
                    const target = event.target;
                    if (VAPE.active) return applyCode(event.button);
        ...
        class EntityManagerr {
            static isPlayer(entity) {
                return entity.type === ELayer.PLAYER;
            }
            static animals() {
                const layers = VAPE.saves.entityList();
                const animals = [];
            ...
            static enemies() {
                const players = VAPE.saves.entityList()[ELayer.PLAYER];
                const enemies = [];
            ...
            static fgdo(a, b){
                return new Vector(b.y - a.y, 2).distance(new Vector(b.x - a.x, 2));
            }
            static angle(a, b) {
                return new Vector(a.x2, a.y2).angle(new Vector(b.x2, b.y2));
            }
            static sortDistance(a, b, sorted) {
                const target = sorted || VAPE.myPlayer;
            ...
            static predict(entity) {
                ...
                const distance = pos1.distance(pos2) * (entity === VAPE.myPlayer ? 1 : 2.2);
            ...
            static entityIn(entity, layer, extraRadius = 0) {
                const targets = VAPE.saves.entityList()[layer];
            ...
            static fuckme(number){
                const targets = VAPE.saved.entityList()[ELayer.PLAYER];
                return targets.some((target =>{
                    const my = VAPE.myPlayer;
                    const nEy = EntityManagerr.enemies()[0];
                    return this.distance(nEy, my) <= number
                }))
            }
            ...
            static nearestLayer(entity, layer) {
                const objects = VAPE.saves.entityList()[layer].map((target => Formatter.object(target)));
                return objects.sort(((a, b) => this.sortDistance(a, b, entity)))[0];
            }
        }
        const attackAnimation = (entity, sorted) => {
            if (!Settings.weaponReloadBar && !Settings.autosync) return;
            const b = VAPE.saves.buffer;
            const len = VAPE.saves.byteLength;
            const players = VAPE.saves.players();
                ...
                if (type === ELayer.PLAYER && target) {
                    ...
                    if (Settings.autosync && controller.canAutosync()) {
                        ...
                        if (controller.isTeammate(player) && controller.isPrimary(weapon)) {
                            const nearest = EntityManagerr.nearestPossible(weapon, player);
                            if (nearest !== null && EntityManagerr.inWeaponRange(VAPE.myPlayer, nearest, controller.itemBar[0])) {
                                const previousWeapon = controller.weapon;
                                controller.whichWeapon(false);
                                controller.attack(EntityManagerr.angle(VAPE.myPlayer, nearest));
        ...
        const createClan = () => {
            const b = VAPE.saves.buffer;
            const len = VAPE.saves.byteLength();
            teammmates = [ ...b.slice(3, len) ];
        };
        const updateClan = () => {
            const b = VAPE.saves.buffer;
            const len = VAPE.saves.byteLength();
            teammmates = [ ...b.slice(2, len) ];
        };
        const deleteClan = () => {
            teammmates = [];
        };
        ...
        class PacketManager {
            ...
            send(...args) {
                VAPE.saves.send(new Uint8Array(args));
            }
        ...
        class Controller {
            ...
            updateWeapon(type) {
                const weapon = VAPE.saves.defaultData[VAPE.props.itemBar][type];
                if (this.isWeapon(weapon) && this.itemBar[type] !== weapon) {
                    this.itemBar[type] = weapon;
                }
            }
            isMyPlayer(entity) {
                return entity.id === VAPE.saves.myPlayerID();
            }
            isTeammmate(entity) {
                return entity.id !== VAPE.saves.myPlayerID() && teammmates.includes(entity.ownerID);
            }
            isEnemy(entity) {
                return !this.isMyPlayer(entity) && !this.isTeammmate(entity);
            }
            canShoot() {
            ...
            currentCount(type) {
                return VAPE.saves.defaultData[VAPE.props.currentCount][type];
            }
            hasCount(type) {
            ...
            upgradeScythe() {
                const target = VAPE.saves.entityList()[ELayer.GOLDENCOW][0];
                if (target !== undefined) {
                    this.PacketManager.upgradeScythe(target[VAPE.props.id]);
                }
            }
            buyHat(id) {
            ...
            hatReloaded() {
                return VAPE.myPlayer.target.hatReload.current === TargetReload.HAT;
            }
            ...
            async autochat2() {
                if (this.chatToggle) return;
                this.chatToggle = true;
                const messages = Settings.autochatMessage2.filter((msg => msg.length));
                if (!messages.length) return;
                this.PacketManager.chat(messages[this.chatCount++]);
                this.chatCount %= messages.length;
                await sleep(1300);
            ...
            async autochat3() {
                if (this.chatToggle) return;
                this.chatToggle = true;
                const messages = Settings.autochatMessage3.filter((msg => msg.length));
                if (!messages.length) return;
                this.PacketManager.chat(messages[this.chatCount++]);
                this.chatCount %= messages.length;
                await sleep(1300);
            ...
            async autochat4() {
                if (this.chatToggle) return;
                this.chatToggle = true;
                const messages = Settings.autochatMessage4.filter((msg => msg.length));
                if (!messages.length) return;
                this.PacketManager.chat(messages[this.chatCount++]);
                this.chatCount %= messages.length;
                await sleep(1300);
            ...
            async autochat5() {
                if (this.chatToggle) return;
                this.chatToggle = true;
                const messages = Settings.autochatMessage5.filter((msg => msg.length));
                if (!messages.length) return;
                this.PacketManager.chat(messages[this.chatCount++]);
                this.chatCount %= messages.length;
                await sleep(1300);
            ...
            accept(which) {
                ...
                const acceptList = VAPE.saves.setData[VAPE.props.acceptList];
                acceptList.shift();
            }
            async spawn() {
                await sleep(100);
            ...
            place(type, angle, placementType) {
                ...
                if (this.wasAutoboost) {
                    const nearest = EntityManagerr.enemies()[0];
                    if (nearest !== undefined) {
                        angle = EntityManagerr.angle(VAPE.myPlayer, nearest);
            ...
            invisibleHit() {
                ...
                let angle;
                const nearest = EntityManagerr.nearestPossible(this.itemBar[+!this.weapon]);
                const shoot = this.canShoot() && !this.weapon;
                if (nearest && (Settings.meleeAim && !shoot || Settings.bowAim && shoot)) {
                    const pos1 = EntityManagerr.predict(VAPE.myPlayer);
                    const pos2 = EntityManagerr.predict(nearest);
            ...
            gopro(){
                ...
                this.mousemove(Math.floor(Math.random() * 360) * Math.PI / 180);
            }
            goku(){
                VAPE.controller.place(2, angle);
                setTimeout(function() {
                    VAPE.controller.place(2, toRad(toDegree(angle) - 72));
                    setTimeout(function() {
                        VAPE.controller.place(2, toRad(toDegree(angle) + 72));
                    }, 100);
                }, 40)
            ...
            async spikeInsta() {
                ...
                if (Settings.spikeInstaAim) {
                    const nearest = EntityManagerr.nearestPossible(this.itemBar[0]);
                    if (nearest) {
                        angle = EntityManagerr.angle(VAPE.myPlayer, nearest);
                    }
                }
                this.previousHat = VAPE.myPlayer.hat;
            ...
            handleKeydown(event, code) {
                ...
                if (code === Settings.toggleMenu && !isInput(event.target)) {
                    if (typeof VAPE.toggleMenu === "function") VAPE.toggleMenu();
                }
                if (!this.inGame) return;
                if (code === Settings.openChat) {
                    if (!isInput()) event.preventDefault();
                    VAPE.saves.toggleChat();
                }
                ...
                if (code === Settings.heal && !this.healing) {
                    ...
                    if (Settings.placementType === PlacementType.DEFAULT) {
                        ...
                        doWhile((() => this.healing), this.tripleHeal.bind(this), 0);
                    }
                }
                ...
                if (code === Settings.unequip) this.unequip();
                if (code === Settings.bush) this.bush();
                if (code === Settings.berserker) this.berserker();
                if (code === Settings.jungle) this.jungle();
                if (code === Settings.crystal) this.crystal();
                if (code === Settings.spikegear) this.spikegear();
                if (code === Settings.immunity) this.immunity();
                if (code === Settings.boost) this.boost();
                if (code === Settings.applehat) this.applehat();
                if (code === Settings.scuba) this.scuba();
                if (code === Settings.hood) this.hood();
                if (code === Settings.demolist) this.demolist();
                ...
                if (code === Settings.lockRotation) {
                    ...
                    VAPE.saves.toggleRotation(this.rotation);
                }
        ...
        const createEntity = target => {
            const id = target[VAPE.props.id];
            const type = target.type;
            const entities = VAPE.saves.entityList();
            if (type === ELayer.PLAYER) {
                if (id === VAPE.saves.myPlayerID()) {
                    VAPE.myPlayer.target = target;
                    updateSkin();
                }
                const player = Formatter.player(target);
                target.hatReload = {
        ...
        class RenderManager {
            ...
            static circle2(ctx, x, y, radius, color) {
                nearestEnemy
                ctx.strokeStyle = color;
                ctx.lineWidth = 5;
                ctx.beginPath();
            ...
            static trapActive(trap) {
                return EntityManagerr.entities().some((entity => {
                    const radius = trap.radius + entity.radius;
                    return EntityManagerr.distance(entity, trap) < radius - -25;//-25 radius
                }));
            }
            static markerColor(target, ownerID) {
                let color = null;
                const object = Formatter.object(target);
                const isMyPlayer = VAPE.myPlayer.ownerID === ownerID;
                const isTeammate = teammmates.includes(ownerID);
            ...
            static renderHP(ctx, entity, height = 0) {
                ...
                const {x, y, health, maxHealth, radius, id} = entity;
                this.renderText(ctx, `${id}`, (width => [ x - width / 2, y + radius - 70 + height ]));
                this.renderText(ctx, `${health}`, (width => [ x - width / 2, y + radius - 43 + height ]));
            }
            ...
            static windmillRotation(target) {
                ...
                if (target[VAPE.props.rotateSpeed] !== speed) {
                    target[VAPE.props.rotateSpeed] = speed;
                }
            }
            ...
            static renderTracer(ctx, entity, isTeammmate) {
                const player = Formatter.player(VAPE.myPlayer.target);
                const color = Settings.rainbow ? `hsl(${controller.hsl}, 100%, 50%)` : this.tracerColor(entity, isTeammmate);
                ...
                if (Settings.arrows) {
                    ...
                    this.circle2(ctx, entity.x, entity.y, entity.radius, color);
                }
            }
        }
        const drawEntityInfo = (target, ctx, isTeammmate) => {
            const entity = Formatter.entity(target);
            const id = VAPE.saves.myPlayerID();
            if (id === entity.id) {
                if (Settings.rainbow) {
                    VAPE.controller.hsl = (VAPE.controller.hsl + .3) % 360;
                }
                if (controller.aimTarget !== null) {
                    ...
                    VAPE.myPlayer.target[VAPE.props.angle] = dir;
                }
            }
            let height = 0;
            ...
            if (id === entity.id || !VAPE.myPlayer.target) return;
            if (Settings.possibleShots && !isTeammmate) {
                const hit = EntityManagerr.projectileCanHitEntity(entity);
                if (hit === Hit.CAN) {
                    const color = Settings.rainbow ? `hsl(${controller.hsl}, 100%, 50%)` : RenderManager.tracerColor(entity, isTeammmate);
                    RenderManager.circle(ctx, entity.x, entity.y, entity.radius, color);
                }
            }
            if (Settings.enemyTracers && entity.type === 0 && !isTeammmate || Settings.teammateTracers && entity.type === 0 && isTeammmate || Settings.animalTracers && entity.type !== 0) {
                RenderManager.renderTracer(ctx, entity, isTeammmate);
            }
        };
        const hooks_drawEntityInfo = drawEntityInfo;
        const drawItemBar = (ctx, imageData, index) => {
            if (!Settings.itemCounter) return;
            const id = VAPE.saves.defaultData[VAPE.props.itemBar][index];
            const type = Items[id].itemType;
            const currentCount = VAPE.saves.defaultData[VAPE.props.currentCount][type];
            const maxCount = controller.maxCount[type];
            if (maxCount === 0) return;
            const x = imageData[VAPE.props.x] - 10;
            const y = imageData[VAPE.props.y] + 10;
...
const getDelay = (health) => {
  if (health <= 10) return Math.random() * (10 - 0) + 0;
  else if (health <= 20) return Math.random() * (20 - 10) + 10;
  else if (health <= 30) return Math.random() * (30 - 20) + 20;
  else if (health <= 40) return Math.random() * (40 - 30) + 30;
  else if (health <= 50) return Math.random() * (50 - 40) + 40;
  else if (health <= 60) return Math.random() * (60 - 50) + 50;
  else if (health <= 70) return Math.random() * (70 - 60) + 60;
  else if (health <= 80) return Math.random() * (80 - 70) + 70;
};

const healing = async () => {
  while (true) {
    const { health, maxHealth, isClown } = VAPE.myPlayer;
    if (Settings.autoheal && health <= 80 && !isClown) {
      await sleep(getDelay(health) * (Math.random() * (1.7 - 1) + 1));
      controller.heal();
    } else if (Settings.autoheal && health <= 50 && !isClown) {
      controller.doubleHeal();
      await sleep(getDelay(health) * (Math.random() * (1.5 - 1) + 1));
    } else if (Settings.autoheal && health <= 30 && !isClown) {
      controller.tripleHeal();
      await sleep(getDelay(health) * (Math.random() * (1.5 - 1) + 1));
    } else {
      await sleep(0);
    }
...
healing();
        function animalss(){
            const nearest = EntityManagerr.nearestLayer(VAPE.myPlayer, ELayer.WOLF);
            if(nearest){
                console.log('wolf')
            }
            if (Settings.anims && controller.hasCount(ItemType.TRAP) && EntityManagerr.entityIn(VAPE.myPlayer, ELayer.WOLF, 2) && !EntityManagerr.entityIn(VAPE.myPlayer, ELayer.WOLF)) {
                controller.place(ItemType.TRAP, EntityManagerr.angle(VAPE.myPlayer, nearest), PlacementType.INVISIBLE);
            }
        }
        let trapy, trapId, inTrap = false;
        let enemyTrap, enemyIsTrapped;



        const toRad = t => t * Math.PI / 180
        /*function toRadd(angle) {
        ...
        function addAlert(str = "No Text", displayTime = 5e3) {
            let al = document.createElement("style");
            al.innerHTML = str;
            IFRAME.onload(al);
            setTimeout(()=>{
                al.style.animation="flyAway 1s linear forwards";
                setTimeout(()=>{
        ...
        window.equip = (e) => {
            if (VAPE.myPlayer.hat !== e) {
                VAPE.controller.PacketManager.equip(e)
            }
        }
        let nearestEnemy;
        let currentHat = 0;
...
function placeWalls(angle, iterations) {
  const increment = 1;
  const delay = 5;

  for (let i = 0; i < iterations; i++) {
    setTimeout(function() {
      VAPE.controller.place(ItemType.WALL, toRad(toDegree(angle + i * increment)));
    }, i * delay);
  }
}

        function autobreaker(){
            trapy = null
            if (Settings.autobreak2) {
                for (let i = 0; i < VAPE.saves.entityList()[ELayer.TRAP].length; i++) {
                    const trapx = VAPE.saves.entityList()[ELayer.TRAP][i]
                    if (trapx && trapx[VAPE.props.ownerID] !== VAPE.myPlayer.ownerID && !Allies.find(a => a.pid == trapx[VAPE.props.ownerID]) && Math.hypot(trapx[VAPE.props.x] - VAPE.myPlayer.x, trapx[VAPE.props.y] - VAPE.myPlayer.y) <= 55) {
                        trapy = trapx;
                        if (!inTrap) {
                            inTrap = true;
                            currentHat = VAPE.myPlayer.hat
                            currentWeapon = VAPE.controller.weapon
                            //VAPE.controller.PacketManager.chat(attackbot ? 'Bot: trapped' : 'trapped')
                            //controller.PacketManager.chat(`Trapped`)
                            window.equip(11)
                            let angle = Math.atan2(-(trapy[VAPE.props.y] - VAPE.myPlayer.y), -(trapy[VAPE.props.x] - VAPE.myPlayer.x));
                            VAPE.controller.place(3, angle);
                            placeWalls(angle, 360);
                            /*setTimeout(() => {
                  VAPE.controller.PacketManager.chat(attackbot ? 'Bot: antitrap done' : 'done')
                }, 500)*/
                        } else {
                            let e = Math.atan2(
                                trapy[VAPE.props.y] - VAPE.myPlayer.y,
                                trapy[VAPE.props.x] - VAPE.myPlayer.x
                            )
                            //VAPE.controller.PacketManager.chat('lol im trapped D:')
                            window.equip(11)
                            VAPE.controller.PacketManager.attack(e)
                            VAPE.controller.PacketManager.stopAttack()

                            if (VAPE.controller.itemBar[1] == 15) {
                                VAPE.controller.whichWeapon(true)
                            } else {
                                VAPE.controller.whichWeapon(false)
                            }
                        }
                        trapId = trapy[VAPE.props.id];
                        break
                    } else

                        if (trapx[VAPE.props.ownerID] == VAPE.myPlayer.ownerID && !teammmates.includes(trapx[VAPE.props.ownerID])) {
                            let d = EntityManagerr.nearestPossible(VAPE.controller.itemBar[0])
                            let nearest = EntityManagerr.enemies()[0];
                            if (d !== null && trapx && Math.hypot(trapx[VAPE.props.x] - d.x, trapx[VAPE.props.y] - d.y) <= 55) {
                                enemyIsTrapped = true;
                                enemyTrap = trapx;
                                if(Settings.azaz && enemyIsTrapped){
                                    controller.PacketManager.chat(`Enemy in trap`)
                                    if(d.health < 50){
                                        controller.PacketManager.chat(`Enemy low hp`)
                                        //log('hits')
                                        controller.attack(EntityManagerr.angle(nearest, VAPE.myPlayer))
                                        controller.PacketManager.stopAttack();
                                    }
                                }
                            }else{
                                let aspike = null;
                                for (let i = 0; i < VAPE.saves.entityList()[ELayer.SPIKE].length; i++) {
                                    const spiker = VAPE.saves.entityList()[ELayer.SPIKE][i]
                                    if (spiker && spiker[VAPE.props.ownerID] == VAPE.myPlayer.ownerID && Allies.find(a => a.pid == spiker[VAPE.props.ownerID]) && Math.hypot(spiker[VAPE.props.x] - VAPE.myPlayer.x, spiker[VAPE.props.y] - VAPE.myPlayer.y) <= 200) {
                                        aspike = spiker;
                                        let tospike = Math.atan2(-(aspike[VAPE.props.y] - VAPE.myPlayer.y), -(aspike[VAPE.props.x] - VAPE.myPlayer.x));

                                        if(Settings.hitloehp && spiker){
                                            //controller.PacketManager.chat(`Nearest spike fr?`)
                                            if(d !== null && enemyIsTrapped){
                                                this.PacketManager.moveAngle(tospike)
                                            }
                                        }
                                    }
                                }
        ...
        function autpush(){
            for (let i = 0; i < VAPE.saves.entityList()[ELayer.SPIKE].length; i++) {
                let spikex = VAPE.saves.entityList()[ELayer.SPIKE][i];
                let psuh = EntityManagerr.nearestPossible(VAPE.controller.itemBar[0])
                let topushto = Math.atan2(spikex[VAPE.props.y] - VAPE.myPlayer.y,spikex[VAPE.props.x] - VAPE.myPlayer.x)
                let spike = Math.atan2(-(spikex[VAPE.props.y] - psuh.y), -(spikex[VAPE.props.x] - psuh.x));
                if(enemyIsTrapped && psuh !== null){
                    angle = EntityManagerr.angle(VAPE.myPlayer, spike);
                    this.PacketManager.moveAngle(angle);
                    this.PakcetManager.chat('push')
                ...
                case ELayer.PLAYER:
                    {
                        ...
                        if (player.id === VAPE.saves.myPlayerID()) {
                            VAPE.myPlayer = {
                                ...VAPE.myPlayer,
                                ...player
                            };
                            const {x2, y2, health, maxHealth, isClown, hat} = VAPE.myPlayer;
                            ...
                            if (!controller.toggleJungle && Settings.jungleOnClown && isClown && hat !== Hat.JUNGLE && controller.equipHat(Hat.JUNGLE, true)) {
                            ...
                            const onPlatform = EntityManagerr.entityIn(VAPE.myPlayer, ELayer.PLATFORM);
                            if (!controller.toggleScuba && inRiver && Settings.autoScuba && !onPlatform && window.equip(9, false)) {
                                controller.toggleScuba = true;
                            }
                            if (controller.toggleScuba && (notInRiver || onPlatform) && controller.inGame && window.equip(this.previousHat, true)) {
                                controller.toggleScuba = false;
                            }
                            if (Settings.autochat) controller.autochat();
                            if (Settings.autochat2) controller.autochat2();
                            if (Settings.autochat3) controller.autochat3();
                            if (Settings.autochat4) controller.autochat4();
                            if (Settings.autochat5) controller.autochat5();
                            if (Settings.autoAccept && VAPE.saves.setData[VAPE.props.acceptList].length) {
                            ...
                            if(Set == "#play"){
                                [1,18,9,16,0,8,17,15,12].forEach(i => VAPE.controller.PacketManager.upgrade(i))
                            }
...
#KatanaSetButton:hover:active,
                            ...
                            animalss()
                            if (!trapy && inTrap) {
                                VAPE.controller.PacketManager.equip(currentHat);
                                //VAPE.controller.PacketManager.chat('untrapped' : 'detrapped')
                                if (VAPE.myPlayer.hat != currentHat) window.equip(currentHat)
                                self.activeweapon = currentWeapon
                                VAPE.controller.whichWeapon(currentWeapon)
                                currentWeapon = 0;
                                currentHat = this.previousHat;
                                trapId = null;
                                inTrap = false;
                            }
                            ...
                            if (now - updatePlayer_start > 1e4 && !isInput() && isBlind()) {
                                ...
                                // controller.PacketManager.chat(console.log('tick'));
                            }
                            const hasPlatform = controller.itemBar[ItemType.PLATFORM] === EObjects.PLATFORM;
                            if (Settings.antiFireball && hasPlatform && controller.hasCount(ItemType.PLATFORM) && EntityManagerr.entityIn(VAPE.myPlayer, ELayer.FIREBALL, 23) && !EntityManagerr.entityIn(VAPE.myPlayer, ELayer.PLATFORM)) {
                                const nearest = EntityManagerr.nearestLayer(VAPE.myPlayer, ELayer.FIREBALL);
                                controller.place(ItemType.PLATFORM, EntityManagerr.angle(nearest, VAPE.myPlayer), PlacementType.INVISIBLE);
                            }
                        }
                        const hatReload = target.hatReload;
                        if (target.prevHat !== player.hat) {
                            target.prevHat = player.hat;
                            hatReload.current = -VAPE.step;
                            hatReload.lerp = 0;
                        }
                        hatReload.current = Math.min(hatReload.current + VAPE.step, hatReload.max);
                        if (Settings.weaponReloadBar) {
                            const weaponReload = target.weaponReload;
                            weaponReload.current = Math.min(weaponReload.current + VAPE.step, weaponReload.max);
                        }
                        break;
                    }
                ...
                case ELayer.TURRET:
                    {
                        if (Settings.turretReloadBar) {
                            ...
                            turretReload.current = Math.min(turretReload.current + VAPE.step, turretReload.max);
                        }
                        break;
                    }
                ...
                case ELayer.DRAGON:
                    {
                        if (Settings.fireballReloadBar) {
                            ...
                            fireballReload.current = Math.min(fireballReload.current + VAPE.step, fireballReload.max);
                        }
                        break;
                    }
            }
        ...
        const applyHooks = code => {
            ...
            Hook.replace("buffer", /((\w+)=new \w+\(NUMBER{4096}\).+?(\w+)=.+?)function/, `$1VAPE.saves.buffer=$2;VAPE.saves.byteLength=()=>$3;function`);
            Hook.append("toggleRotation", /return (\w+)\?\w+:.+?\}/, `VAPE.saves.toggleRotation=(value)=>{$2=value};`);
            const acceptList = Hook.append("setData", /(\w+)\.(\w+)\.\w+\(\)\);.+?\}/, `VAPE.saves.setData=$2;`)[3];
            VAPE.props.acceptList = acceptList;
            Hook.append("upgradeItem", /\.001.+?for\(let \w+=0,.+?\w+\(new \w+\(\[.+?,(\w+)\]\)(,|;)?/, `,VAPE.controller.upgradeItem($2)$3`);
            Hook.replace("zoom", /(\w+):NUMBER{1824},(\w+):NUMBER{1026}/, "get $1(){return VAPE.scale.lerp.w},get $2(){return VAPE.scale.lerp.h}");
            Hook.insert("send", /=NUMBER{9999}.+?\(null\).+?{INSERT}function (\w+)\(\w+\)\{/, `VAPE.saves.send=$3;`);
            Hook.replace("toggleChat", /(return \(?(\w+&&\w+.+?)(?:,)?(?:\))?void.+?)function/, `$1VAPE.saves.toggleChat=()=>{$2};function`);
            Hook.replace("updatePlayer", /(\w+\(ARGS{16}\).+?(\w+)\.\w+=0[,;]?)\}function/, `$1;VAPE.hooks.updatePlayer($2)}function`);
            Hook.replace("createEntity", /(function \w+\((\w+),ARGS{16}\).+?\})(\}\w+\(\))/, `$1VAPE.hooks.createEntity($2)$3`);
            Hook.append("drawEntityInfo", /-NUMBER{50},.+?function \w+\((ARGS{3})\)\{/, `VAPE.hooks.drawEntityInfo($2);VAPE.hooks.ally($2);`);
            ...
            VAPE.props.y1 = y1;
            VAPE.props.y2 = y2;
            VAPE.props.angle = angle;
            VAPE.props.angle1 = angle1;
            VAPE.props.angle2 = angle2;
            const ownerID = Hook.match("ownerID", /\|\|\w+&&\w+===\w+\.(\w+)\)/)[1];
            VAPE.props.ownerID = ownerID;
            const health = Hook.match("health", /\w+\.(\w+)\/NUMBER{255}\*/)[1];
            VAPE.props.health = health;
            const entityValue = Hook.match("entityValue", /!\(\w+\.(\w+)&/)[1];
            VAPE.props.entityValue = entityValue;
            const [, currentItem, hat] = Hook.match("hat", /\(\w+\.(\w+)\|\w+\.(\w+)<<NUMBER{8}\)/);
            VAPE.props.hat = hat;
            VAPE.props.currentItem = currentItem;
            const projectileType = Hook.match("projectileType", /,\w+\[\w+\]\.(\w+),/)[1];
            VAPE.props.projectileType = projectileType;
            Hook.prepend("myPlayerID", /function \w+\(\)\{return \w+!==(\w+)/, `VAPE.saves.myPlayerID=()=>$2;`);
            const itemBar = Hook.replace("defaultData", /(\W\w+>NUMBER{1}\W.+?(\w+)\.(\w+).+?)function/, "$1VAPE.saves.defaultData=$2;function")[3];
            VAPE.props.itemBar = itemBar;
            const currentCount = Hook.match("currentCount", /(\w+):\[ARGS{11}\],/)[1];
            VAPE.props.currentCount = currentCount;
            Hook.replace("entityList", /(\(this,.+?typeof window.+?(\w+)=\[\].+?)function/, `$1VAPE.saves.entityList=()=>$2;function`);
            const rotateSpeed = Hook.match("rotateSpeed", /\w+\(ARGS{17}\)\{.+?\/NUMBER{4}.+?\/NUMBER{4}.+?\w+\.(\w+)=/)[1];
            VAPE.props.rotateSpeed = rotateSpeed;
            Hook.replace('customClan', /"\D"+\W(\w.\w{2})\W+"\D",[a-z]\(\).\w{2},"#96C949","#404040"/, `"✞ " + $1 + ' ✞', 25, "#000","#fff"`);
            Hook.append("showHoods", /\w+\.\w+!==\w+\)/, `||VAPE.settings.showHoods`);
            Hook.append("itemCounter", /AGE 0.+?\[(\w+)\][,;](\w+)\.\w+\((\w+)\)([,;])/, `VAPE.hooks.drawItemBar($4,$3,$2)$5`);
            Hook.replace("renderItems", /(\(\w+\.\w+\+\w+,\w+\.\w+\+\w+\).+?\w+\(\).+?\w+\.\w+\.\w+\)([,;]))/, `$1VAPE.hooks.renderItems(...arguments)$2`);
            Hook.replace("mousemove", /(\+NUMBER{110}.+?)(const \w+=\w+\(\).+?\w+!==\w+.+?\w+\(\w+\))/, `$1if(VAPE.controller.mousemove){$2}`);
            Hook.replace("chatMessage", /(\.NUMBER{18},.+?)(\w+\.\w+\((\w+),(\w)+\))/, `$1$2;if(${target.name}settings.autoResponse){Object.keys(${target.name}badWords).forEach(e => {let regex = $3.match(e); if(regex){${target.name}badWords[regex[0]]()}})}if(${target.name}settings.chatMirror){${target.name}controller.PacketManager.chat($3);}`);
            Hook.replace("clanMessage", /(NUMBER{1006}.+?)(\w+\.\w+\(\w+,.+?\+(\w+)\))/, `$1if(pingCount!==$3&&!VAPE.settings.hideMessages){$2}`);
            Hook.replace("players", /(\)\)\(\).+?(\w+)=new.+?)function/, `$1VAPE.saves.players=()=>$2;function`);
            const [skin, accessory, back] = Hook.matchAll("skins", /=\w+\.(\w+)\|\|NUMBER{0}/).map((a => a[1]));
            VAPE.props.skin = skin; //skin
            VAPE.props.accessory = accessory;
            VAPE.props.back = back;
        ...
        const getConstructor = () => {
            ...
            return {
                name: constructor.name + ".NABSOCOOL.",
                method: constructor
            };
        };
        let Allies = []
        ...
        target.method.NABSOCOOL = NABSOCOOL;
        window.NABSOCOOL = NABSOCOOL;
        if (!NABSOCOOL.PRODUCTION) {
            window.log = log;
        }
        window.log = log;
        window.VAPE = {
            ...
            hooks: {
                ...
                renderItems: hooks_renderItems,
                ally: (entity, p2, p3) => {
                    if(p3 && entity[VAPE.props.id] !== VAPE.controller.myPlayerID && entity.type === 0 && !Enemys.find(A=> A.id == entity[VAPE.props.id])){
                        unload({
                            x: entity[VAPE.props.x],
                            y: entity[VAPE.props.y],
                            id: entity[VAPE.props.id],
                            type: entity.type
                        });
                    }
                    if (p3 && entity[VAPE.props.id] !== VAPE.controller.myPlayerID && entity.type === 0 && !Allies.find(A => A.id == entity[VAPE.props.id])) {
                        Allies.push({
                            x: entity[VAPE.props.x],
                            y: entity[VAPE.props.y],
                            type: entity.type,
                            hp: entity[VAPE.props.health] / 255 * 100,
                            hat: entity[VAPE.props.hat],
                            pid: entity[VAPE.props.ownerID],
                            id: entity[VAPE.props.id]
                        })
                    }
                    if (!p3 && entity[VAPE.props.id] !== VAPE.controller.myPlayerID && entity.type === 0 && !Enemys.find(A => A.id == entity[VAPE.props.id])) {
        ...
        let autoreplaceQueue = 2, lastAutoreplace = 2;
        const spikes = [2, 7, 17];

        const ang = e => {
            let player = VAPE.myPlayer;
            return Math.atan2(
                -(e.y - player.y2),
                e.x - player.x2
            ) + Math.PI / 2;
        }
        function getMillType(){return this.currentItem.length>6?this.currentItem[5]:this.currentItem[4]};
        function getFoodType(){return this.currentItem[(this.currentItem.length>6)+1]};
        function getSpikeType(){return this.currentItem[(this.currentItem.length>6)+3]};
        //function getTrapType() {return VAPE.items.length>5?VAPE.items.length>6?VAPE.items[6]:VAPE.items[5]:VAPE.items[0]};
        function getTrapType() {
            let i = this.currentItem;
            let j = i.indexOf(9);
            return i[j+1?j:6];
        };
        function getPrimary(){return VAPE.items[0]};
        function getSecondary(){return this.currentItem[(this.currentItem.length>6)|0]};
        function getRoofType(){return this.currentItem.length>7?this.currentItem[7]:this.currentItem[0]};
        function unload(e) {
            let player = VAPE.myPlayer;
            if (Math.hypot(e.x - player.x2, e.y - player.y2) <= 220 && e.type != 0) {
                let s = getSpikeType();
                let t = getTrapType();
                this.place(s, ang(e));
                for(let i = 0; i < 24; i++) {
            ...
            // this.mouse.angle(VAPE.myPlayer.angle)
        }

        /* function unload(entity) {
    let player = VAPE.myPlayer;
    if (Math.hypot(entity.x - player.x2, entity.y - player.y2) <= 150
        && entity.type != 0 && !this.projectileCanHitEntity(entity.type)) {
       controller.PacketManager.chat('replace ' + entity.id + ' type:' + entity.type);
        let angle = ang(entity);
        function PLACE() {
            let st = getSpikeType();
            let tt = getTrapType();

            this.place(!spikes.includes(entity.type) || tt == getPrimary() ? st : controller.place(ItemType.TRAP), angle);
        };

        let t = performance.now() - lastAutoreplace;
        if (t >= 100) {
            controller.place(ItemType.TRAP)
        ...
        setInterval(()=>{
            const Players2 = VAPE.saves.entityList()[ELayer.PLAYER]
            const enemies2 = []
            const autoplacerrr = false;
            const nearest = EntityManagerr.enemies()[0];
            const nEy = EntityManagerr.enemies()[0];
            if(Settings.autoplacerr && controller.hasCount(ItemType.TRAP) && EntityManagerr.distance(nearest, VAPE.myPlayer) && autoplacerrr == false && EntityManagerr.fgdo(VAPE.myPlayer, nearest) <= 180){
                controller.place(ItemType.TRAP,EntityManagerr.angle(VAPE.myPlayer, nearest), PlacementType.INVISIBLE)
            }
        }, 1e3/9+45)
        let primaryWeapon = 1;
        ...
        setInterval(()=>{
            const Players2 = VAPE.saves.entityList()[ELayer.PLAYER]
            const enemies2 = []
            const autoplacerr = false;
            const nearest = EntityManagerr.enemies()[0];
            const nEy = EntityManagerr.enemies()[0];
            if(Settings.autoplacerrr && controller.hasCount(ItemType.SPIKE) && EntityManagerr.distance(nearest, VAPE.myPlayer) && autoplacerr == false && EntityManagerr.fgdo(VAPE.myPlayer, nearest) <= 170){
                controller.place(ItemType.SPIKE,EntityManagerr.angle(VAPE.myPlayer, nearest), PlacementType.INVISIBLE)
            }
        }, 1e3/9+45)
        let secondaryWeapon = 1;
        setInterval(()=>{
            const Players2 = VAPE.saves.entityList()[ELayer.PLAYER]
            const enemies2 = []
            const nearest = EntityManagerr.enemies()[0];
            const nEy = EntityManagerr.enemies()[0];
            if(Settings.autoplacerrrr && controller.hasCount(ItemType.SHIELD) && EntityManagerr.distance(nearest, VAPE.myPlayer) && EntityManagerr.fgdo(VAPE.myPlayer, nearest) <= 230){
                controller.place(ItemType.SHIELD,EntityManagerr.angle(VAPE.myPlayer, nearest.PROJECTILE), controller.place(secondaryWeapon))
            }
        }, 1e3/9+45)
        ...
        setTimeout(() => {
            setInterval(() => {
                if(Settings.autoheal2 && VAPE.myPlayer.isClown){
                    window.equip(3)
                }
            }, 0)
        }, 0)
        ...
        setTimeout(() => {
            setInterval(() => {
                if(Settings.antiinsta && VAPE.myPlayer.health < 30){
                    const nearest = EntityManagerr.nearestLayer(VAPE.myPlayer, ELayer.PLAYER);
                    controller.place(ItemType.SPIKE,window.equip(6), EntityManagerr.angle(VAPE.myPlayer, nearest), PlacementType.INVISIBLE)
                    window.equip(6)
                }
            }, 0)
        }, 0)
        setInterval(() => {
            if(Settings.antiinsta && VAPE.myPlayer.health < 70) {
                const nearest = EntityManagerr.nearestLayer(VAPE.myPlayer, ELayer.PLAYER);
                controller.place(ItemType.SPIKE,window.equip(6),EntityManagerr.nearestLayer(ItemType.SPIKE, VAPE.myPlayer, ELayer.PLAYER, ItemType.SPIKE), EntityManagerr.angle(VAPE.myPlayer, nearest), PlacementType.INVISIBLE)
                window.equip(6)
            }
        }, 0);
        /*setTimeout(() => {
            setInterval(() => {
                if(Settings.hitloehp && EntityManagerr.enemy.health < 70) {
                window.equip(2)
                const nearest = EntityManagerr.nearestLayer(VAPE.myPlayer, ELayer.PLAYER);
                controller.attack(EntityManagerr.angle(VAPE.myPlayer, nearest));
                }
        ...
        setInterval(() => {
            if(Settings.hitloehp && EntityManagerr.enemy.health < 80) {
                window.equip(2)
                const nearest = EntityManagerr.nearestLayer(VAPE.myPlayer, ELayer.PLAYER);
                controller.attack(EntityManagerr.angle(VAPE.myPlayer, nearest));
            }
        ...
        let repeater = function(e, o, t) {
            ...
            return {
                start(d) {
                    d == e && "chatbox" !== document.activeElement.id.toLowerCase() && (n = !0, void 0 === a && (a = setInterval(() => {
                        o(), o(), n || (clearInterval(a), a = void 0)
                    }, t)))
                },
                stop(o) {
                    o == e && "chatbox" !== document.activeElement.id.toLowerCase() && (n = !1)
                }
            }
        };
        setInterval(()=>{
            if (Settings.placementType === PlacementType.MACRO) {
                const trappers = repeater(86, () => {//f = pit
                    controller.place(ItemType.SPIKE)
                }, 0);
                const trapper = repeater(70, () => {//f = pit
                    controller.place(ItemType.TRAP)
                }, 0);
                document.addEventListener("keydown", (e)=>{
                    trapper.start(e.keyCode);
                })
                document.addEventListener("keyup", (e)=>{
                    trapper.stop(e.keyCode);
                })
                document.addEventListener("keydown", (e)=>{
                    trappers.start(e.keyCode);
                })
                document.addEventListener("keyup", (e)=>{
                    trappers.stop(e.keyCode);
                })
            }
        })
        const VAPE = window.VAPE;
        ...
        function mouseup(target) {
            ...
            document.querySelector('#clan-menu').style.display = "none";
        }
        let Set = "bow";
        function BowSet() {
            Set = "bow";
        ...
        window.BowSet = BowSet;
        window.PolearmSet = PolearmSet;
        window.KatanaSet = KatanaSet;
        setTimeout(() => {
            let VAPEhit = {};
            ...
            function sendKeyEvent(Down,E){
                if(Down){
                    controller.handleKeydown(E, E.code)
                } else {
                    controller.handleKeyup(E, E.code)
                }
            }
            function sendMouseEvent(Down,E){
                if(Down){
                    controller.handleKeydown(E, E.button)
                } else {
                    controller.handleKeyup(E, E.button)
                }
            }
            function sendToAllKey(Down,E){
                let I = document.querySelectorAll("#boxContainer > div.botContainer > iframe")
                for(let i = 0; i < I.length; i++){
                    //I[i].contentWindow.VAPE.controller.mouse.angle = controller.mouse.angle;
                    I[i].contentWindow.sendKeyEvent(Down,E);
                }
            }
            function sendToAllMouse(Down,E){
                let I = document.querySelectorAll("#boxContainer > div.botContainer > iframe")
                for(let i = 0; i < I.length; i++){
                    I[i].contentWindow.sendMouseEvent(Down,E);
                }
            }
            let sended = {};
            ...
            if (!inIframe()) {
                ...
                window.addEventListener("mousedown",(function(e){
                    sendToAllMouse(true,e)
                    VAPEhit.mainMouseDown = true;
                    VAPEhit.mainMouseNum = e.which;
                }));
                window.addEventListener("mouseup",(function(e){
                    sendToAllMouse(false,e)
                    VAPEhit.mainMouseDown = false;
                    VAPEhit.mainMouseNum = false;
                }));
                window.addEventListener("keydown",(function(e){
                    if(sended["key"+e.code] === undefined){
                        sended["key"+e.code] = true;
                    }
                    if(!sended["key"+e.code]){
                        sended["key"+e.code] = true;
                        sendToAllKey(true,e)
                    }
                }));
                window.addEventListener("keyup",(function(e){
                    if(sended["key"+e.code]){
                        sended["key"+e.code] = false;
                        sendToAllKey(false,e)
                    }
                }));
            }
            setInterval(() => {
                if (!inIframe()) {
                    let getpos = getPosition(cordss);
                    VAPEhit.mainX = VAPE.myPlayer.x;
                    VAPEhit.mainY = VAPE.myPlayer.y;
                    VAPEhit.mainMouseY = getpos.x / window.innerWidth;
                    VAPEhit.mainMouseX = getpos.y / window.innerHeight;
                    VAPEhit.mainMouseX1 = getpos.x - (window.innerWidth / 2);
                    VAPEhit.mainMouseY1 = getpos.y - (window.innerHeight / 2);
                    VAPEhit.mainAngle = VAPE.myPlayer.angle;
                    if(VAPE.controller.weapon == false){
                        VAPEhit.mainWeapon = 0;
                    } else {
                        VAPEhit.mainWeapon = 1;
                    }
                    VAPEhit.mainClanName = document.querySelector("#clan-title").innerText;
                    localStorage.VAPEhit = JSON.stringify(VAPEhit);
                }
                if (inIframe()) {
                    let VAPEhit = JSON.parse(localStorage.VAPEhit)
                    function mouseup(target) {
                        document.querySelector('#clan-menu').style.display = "flex";
                        target.onmouseup(new Proxy(new MouseEvent("mouseup"), {
                            get(target, prop) {
                    ...
                    if(document.querySelector("#clan-title").innerText == "Clans"){
                        for(let i = 0; i <= document.querySelector("#clan_menu_content").childElementCount; i++){
                            try{
                                if(document.querySelectorAll("#clan_menu_content > div > p")[i].innerText == VAPEhit.mainClanName){
                                    mouseup(document.querySelectorAll("#clan_menu_content > div > div > button")[i])
                                    console.log(i)
                                }
                            } catch {}
                        }
                    }
                    if(VAPEhit.mainMouseDown && VAPEhit.mainMouseNum == 1){
                        VAPE.controller.attack(Math.atan2(VAPEhit.mainY - (VAPE.myPlayer.y - VAPEhit.mainMouseY1), VAPEhit.mainX - (VAPE.myPlayer.x - VAPEhit.mainMouseX1)));
                        VAPE.controller.PacketManager.stopAttack();
                    }
                    document.querySelector("#play").click()
                    let someX = Math.pow(((VAPE.myPlayer.x - VAPEhit.mainMouseX1) - VAPEhit.mainX), 2);
                    let someY = Math.pow(((VAPE.myPlayer.y - VAPEhit.mainMouseY1) - VAPEhit.mainY), 2);
                    if (Math.sqrt(someX + someY) > 85) {
                        VAPE.controller.PacketManager.moveAngle(Math.atan2(VAPEhit.mainY - (VAPE.myPlayer.y - VAPEhit.mainMouseY1), VAPEhit.mainX - (VAPE.myPlayer.x - VAPEhit.mainMouseX1)));
                        VAPE.controller.PacketManager.changeAngle(Math.atan2(VAPEhit.mainY - (VAPE.myPlayer.y - VAPEhit.mainMouseY1), VAPEhit.mainX - (VAPE.myPlayer.x - VAPEhit.mainMouseX1)));
                        VAPE.controller.mouse.angle = Math.atan2(VAPEhit.mainY - (VAPE.myPlayer.y - VAPEhit.mainMouseY1), VAPEhit.mainX - (VAPE.myPlayer.x - VAPEhit.mainMouseX1));
                    } else {
                        VAPE.controller.PacketManager.moveByBitmask(0);
                    }
                    if(VAPEhit.mainWeapon == 0){
                        VAPE.controller.whichWeapon(false);
                    } else {
                        VAPE.controller.whichWeapon(true);
                    }
                    if(Set == "bow"){
                        [1,18,9,19,26,8,17,27,12].forEach(i => VAPE.controller.PacketManager.upgrade(i))
                    } else if(Set == "Polearm"){
                        [2,18,9,19,0,8,28,27,12].forEach(i => VAPE.controller.PacketManager.upgrade(i))
                    } else if(Set == "katana") {
                        [1,18,9,19,0,8,17,27,12].forEach(i => VAPE.controller.PacketManager.upgrade(i))
                    }
                }
            },100)
            if (!inIframe()) {
...
#katanaSetButton:hover:active,
#polearmSetButton:hover:active,
#bowSetButton:hover:active,
#reloadBotButton:hover:active,
#disconnectBotButton:hover:active,
#boxContainer > #buttonContainer >*:hover:active{
  transform: scale(.9);
}
#katanaSetButton:hover,
#polearmSetButton:hover,
#bowSetButton:hover,
#reloadBotButton:hover,
#disconnectBotButton:hover,
#boxContainer > #buttonContainer >*:hover{
  transform: scale(1.1);
}
#boxContainer > #buttonContainer >*{
  background: rgba(40, 45, 34, 1);
  transition: .2s;
...
#polearmSetButton::before{
  content: "2";
}
#katanaSetButton::before{
  content: "3";
}
#disconnectBotButton::before{
  content: "disconnect";
}
#katanaSetButton,
#polearmSetButton,
...
#boxContainer > div.botContainer > iframe{
  border-radius: 10px;
...
#boxContainer.closed > #buttonContainer>#closeButton::before{
  content: "Open";
}
#boxContainer{
  left: 10px;
...
#boxContainer.closed > .botContainer{
  position: absolute;
  left: -1000px;
}
#boxContainer.closed{
  ...
  width: 120px;
                ...
                function addbot(){
                    ...
                    let numer = document.createElement("a");
                    numer.id = "numerbot";
                    numer.innerText = botNum++;
                    let iframe = document.createElement("iframe");
                    iframe.id = botNum;
                    iframe.src = location.href;
                    let disconnectBotButton = document.createElement("button")
                    disconnectBotButton.id = "disconnectBotButton";
                    function discBot(){
                        botContainer.remove();
                    }

                    disconnectBotButton.onclick = discBot;
                    let reloadBotButton = document.createElement("button")
                    reloadBotButton.id = "reloadBotButton";
                    function reload(){
                        iframe.contentWindow.location.reload(true);
                    }

                    let bowSetButton = document.createElement("button")
                    bowSetButton.id = "bowSetButton";
                    function bowSet(){
                        iframe.contentWindow.BowSet();
                    }
                    bowSetButton.onclick = bowSet;

                    let polearmSetButton = document.createElement("button")
                    polearmSetButton.id = "polearmSetButton";
                    function polearmSet(){
                        iframe.contentWindow.PolearmSet();
                    }
                    polearmSetButton.onclick = polearmSet;

                    let katanaSetButton = document.createElement("button")
                    katanaSetButton.id = "katanaSetButton";
                    function katanaSet(){
                        iframe.contentWindow.KatanaSet();
                    }
                    katanaSetButton.onclick = katanaSet;

                    reloadBotButton.onclick = reload;
                    boxContainer.append(botContainer);
                    botContainer.append(numer);
                    botContainer.append(iframe);
                    botContainer.append(bowSetButton);
                    botContainer.append(polearmSetButton);
                    botContainer.append(katanaSetButton);
                    botContainer.append(disconnectBotButton);
                    botContainer.append(reloadBotButton);
                }
                function addbot4(){
                ...
                document.querySelector("#game-canvas").after(boxContainer);
                ...
                buttonContainer.append(addBotButton);

                let addBotButton4 = document.createElement("button")
                addBotButton4.id = "addBotButton4";
                addBotButton4.onclick = addbot4;
                buttonContainer.append(addBotButton4);
