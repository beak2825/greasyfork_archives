...
// @match        https://aidp.bytedance.com/management/task-v2/7392550115145666339/*
// @license      BSD
...
// @grant        GM_setValue
// @grant        GM_getValue
// @downloadURL https://update.greasyfork.org/scripts/518026/1688.user.js
// @updateURL https://update.greasyfork.org/scripts/518026/1688.meta.js
// ==/UserScript==
...
(function() {
    ...
    // 设置一个 MutationObserver 来监听 DOM 变化
    function observeForElement() {
        const observer = new MutationObserver(function(mutationsList, observer) {
            // 当 DOM 发生变化时，调用 checkAndAlert 函数检查元素
            if (checkAndClick()) {
                observer.disconnect(); // 一旦找到目标元素并弹窗，停止观察
            }
        });
        ...
        // 启动 MutationObserver 观察整个 document.body
    ...
    function checkAndClick() {
        ...
        const currentURL = window.location.href;
        // aidp 京东规则
        if (currentURL.indexOf("aidp.bytedance.com") > -1) {
            // 检查指定的元素是否为空
            ...
            if (elementToCheck && elementToCheck.innerHTML.trim() !== "") {
                ...
                setNativeValue(局_填入企业,value);
                局_填入企业.dispatchEvent(new Event('input', { bubbles: true }))

                const 局_填入地址 = document.querySelector("#conbination-wrap > div > div > div > div > div > div:nth-child(3) > div > div:nth-child(2) > div > div:nth-child(1) > div > div > div > div:nth-child(2) > div > div:nth-child(2) > div > div.arco-form-item-wrapper > div.arco-form-item-control-wrapper > div > div > textarea")
                var 局_地址 = GM_getValue('京东地址');
                setNativeValue(局_填入地址,局_地址);
                局_填入地址.dispatchEvent(new Event('input', { bubbles: true }))
                // 点击非个人
        ...
        if (currentURL.indexOf("jd.com/showLicence") > -1) {
            const elementToCheck = document.querySelector("#wrap > div > div.jRatingMore > div > ul > li:nth-child(1)")
            if (elementToCheck && elementToCheck.innerText == "京东商城网店经营者营业执照信息") {
                // 获取企业 元素
                const 局_企业 = document.querySelector("#wrap > div > div.jRatingMore > div > ul > li:nth-child(2) > span").innerText
                GM_setValue('京东企业',局_企业);
                // 获取企业地址
                const 局_企业地址 = document.querySelector("#wrap > div > div.jRatingMore > div > ul > li:nth-child(9) > span").innerText
                // 存储数据
                GM_setValue('京东地址',局_企业地址);
                console.log("收集完成");
                return true;
            }
        ...
        return false; // 返回 false 表示元素未找到

    }

    // 页面加载后执行检测
    window.addEventListener('load', observeForElement);
    // 以下为旧脚本
    ...
    function hotkey() {
        ...
        if (a == 38) {
            ...
            // aidp 京东规则
            if (localHost.indexOf("aidp.bytedance.com") > -1) {
                // 检查指定的元素是否为空
                const elementToCheck = document.querySelector("#conbination-wrap > div > div > div > div > div > div:nth-child(3) > div > div:nth-child(2) > div > div:nth-child(1) > div > div > div > div:nth-child(1) > div > div:nth-child(1) > div > div.arco-form-item-wrapper > div > div > div > div > div > div > div:nth-child(2) > div.arco-upload-list.arco-upload-list-type-picture-card");
                if (elementToCheck && elementToCheck.innerHTML.trim() !== "") {// 如果不为空，弹出提示
                    ...
                    局_填入地址.dispatchEvent(new Event('input', { bubbles: true }))
                    // 点击非个人
                    const 局_非个人 = document.querySelector("#conbination-wrap > div > div > div > div > div > div:nth-child(3) > div > div:nth-child(2) > div > div:nth-child(2) > div > div:nth-child(1) > div > div.arco-form-item-wrapper > div.arco-form-item-control-wrapper > div > div > div > label:nth-child(1) > span.arco-icon-hover.arco-radio-icon-hover.arco-radio-mask-wrapper > div")
                    局_非个人.click();
                } }//aidp结束
            ...
            // 1688搜索策略
            if (localHost == "s.1688.com") {
                // 获取搜索词
                var 搜索词 = document.querySelector("#alisearch-input")
                搜索词 = 搜索词.value
                for (var b = 1; b < 49; b++) {
                    var btn1688 =document.querySelector("#app > div > div > div > div.space-pc-ui2024-content > div:nth-child(5) > div.space-common-offerlist > div > div.col-feeds > div > a:nth-child("+ b +") > div.offer-shop-row > div.col-left > a")
                    if(btn1688.innerText == 搜索词){
                        btn1688.click();
                        break;
                    }
                }}// 1688搜索策略结束
            // 1688店铺策略
            var localHostsss = window.location.hostname; // 假设这是当前页面的主机名
            console.log("当前主机名:", localHostsss);
            var regex = /[\w]+\.1688\.com/; // 匹配 s.sk 后面是一串数字，然后是 .4399.com
            console.log("正则表达式:", regex);
            if (regex.test(localHostsss)) {
                // 获取 <script> 元素
                var scriptElement = document.querySelector("#tb-beacon-aplus");

                // 确认元素存在并获取 exparams 属性的值
                if (scriptElement) {
                    var exparams = scriptElement.getAttribute('exparams');

                    // 使用正则表达式提取出 atp_isdpp= 后面的值
                    var match = exparams.match(/atp_isdpp=6v([^&]+)/);

                    if (match && match[1]) {
                        var extractedValue = decodeURIComponent(match[1]); // 解码 URL 编码的值
                        console.log("提取的值是: ", extractedValue);
                        window.open("https://wp.m.1688.com/page/businessinfor.html?memberId=" + extractedValue + "&bizCode=winport");
                    } else {
                        console.log("未找到符合的值");
                    }
                } else {
            ...
            // 1688证件策略
            if (localHost.indexOf("wp.m.1688.com") > -1 ) {
                // 获取企业 元素
                var 局_企业 = document.querySelector("#bd_0_container_0 > div > div:nth-child(2) > div > div > div > div:nth-child(2) > div:nth-child(1) > div > div:nth-child(1) > span:nth-child(2)")
                局_企业 = 局_企业.innerText
                //navigator.clipboard.writeText(局_企业)
                GM_setValue('1688',局_企业地址);
                // 获取企业地址
                var 局_企业地址 = document.querySelector("#bd_0_container_0 > div > div:nth-child(2) > div > div > div > div:nth-child(2) > div:nth-child(2) > div > div:nth-child(1) > span:nth-child(2)")
                局_企业地址 = 局_企业地址.innerText
                // 使用Clipboard API将文本写入剪切板
                // navigator.clipboard.writeText(局_企业地址)
                // 存储数据
                GM_setValue('1688',局_企业地址);
            }// 1688证件策略结束

            // 京东文字策略
            if (localHost == "search.jd.com") {
                for (var c = 1; c < 61; c++) {
                    ...
                    if(btnjd.innerText == 局_京东搜索词) {
                        ...
                        if (btnjd) {
                            ...
                            // 使用正则表达式提取出 atp_isdpp= 后面的值
                            match = 局_京东店铺id.match(/index-(\w+)\.html\?/);
            ...
            // 京东证件策略
            if (jdlocalHostPathName.indexOf("mall.jd.com/showLicence") > -1) {
                // 获取企业 元素
                const 局_企业 = document.querySelector("#wrap > div > div.jRatingMore > div > ul > li:nth-child(2) > span").innerText
                //navigator.clipboard.writeText(局_企业)
                GM_setValue('京东企业',局_企业);
                //localStorage.setItem('京东企业', 局_企业);
                // 获取企业地址
                const 局_企业地址 = document.querySelector("#wrap > div > div.jRatingMore > div > ul > li:nth-child(9) > span").innerText
                // 使用Clipboard API将文本写入剪切板
                // navigator.clipboard.writeText(局_企业地址)
