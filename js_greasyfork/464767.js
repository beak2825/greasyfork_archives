...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/464767/%E5%B1%B1%E8%A5%BF%E7%9F%B3%E5%8C%96%E6%AF%94%E6%AD%A6%E8%87%AA%E5%8A%A8%E6%98%BE%E7%A4%BA%E7%AD%94%E6%A1%88-NEW.user.js
// @updateURL https://update.greasyfork.org/scripts/464767/%E5%B1%B1%E8%A5%BF%E7%9F%B3%E5%8C%96%E6%AF%94%E6%AD%A6%E8%87%AA%E5%8A%A8%E6%98%BE%E7%A4%BA%E7%AD%94%E6%A1%88-NEW.meta.js
// ==/UserScript==
...
(function() {
    ...
    document.getElementById('lxNextBtn').onclick = function(){
        console.log('success');
        let num = document.getElementById('schedule').innerHTML;//当前所在题序号
        let num1 = Number(num) + Number(4);//div序号
        let question_type = document.querySelector("#div"+num1.toString()).getAttribute('type');
        if (Number(num) >= 4){console.log('第'+(Number(num)-1)+'题');console.log('questype；'+question_type);}
        let checked = ''
        if (Number(question_type) == 4){//4:多选题，3:选择题
            for(let i=1; i<6;i++){
                ...
                if (checked1){
                    ...
                    checked = checked + checked1.innerHTML
                    checked = checked + '｜';
                }
            }
        ...
        if (check.split('，')[0] == '回答错误'){
            let answer = check.split('正确答案为：')[1]
            //console.log(question)
            //console.log(answer)
