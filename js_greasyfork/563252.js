...
// @version      1.5.15
// @description  去除1024的60秒屏蔽、菜区助手（直播、算分+批量、统计）、163右下角回家之路
...
// @downloadURL https://update.greasyfork.org/scripts/563252/1024%E7%BB%BC%E5%90%88%E5%8A%A9%E6%89%8B%20%2B%20%E6%89%B9%E9%87%8F.user.js
// @updateURL https://update.greasyfork.org/scripts/563252/1024%E7%BB%BC%E5%90%88%E5%8A%A9%E6%89%8B%20%2B%20%E6%89%B9%E9%87%8F.meta.js
// ==/UserScript==
...
(function() {
        ...
        if(G.get('zhiboEnable'))
        {
            ...
            if(G.get('pingfenEnable') && localTitle.match('\\[直播\\]') == null && localTitle.match(key) != null && localHref.match('read\.php') != null)
            {
                if($('#btn_pinfen').length == 0)
                {
                    {
                        // 定义统一样式
                        const commonStyle = 'position: fixed; right: 20px; z-index: 9999; background: #fff; padding: 5px 10px; border: 1px solid #ccc; border-radius: 4px; font-size: 13px; width: 120px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);';

                        // 1. 注入 HTML 结构
                        var uiHtml = `
        <div id="bets_tools_panel" style="position: fixed; right: 20px; top: 10px; z-index: 9999;">
            <div style="${commonStyle} position:static; margin-bottom:10px;">
                <label style="cursor:pointer; display:block;">
                    <input type="checkbox" id="bulkOutputMode_chk"> 批量快速模式
                </label>
            </div>
           <div style="${commonStyle} position:static; margin-bottom:5px;">
                <label style="cursor:pointer; display:block;">
                    <input type="checkbox" id="betsCopyOnlyBets_chk"> 只复制下注信息
                </label>
            </div>
            <div class="t_like tools_btn" id="copy_bets_btn" style="width: 140px; text-align: center; margin-bottom:5px; cursor:pointer;">复制下注信息</div>
            <div class="t_like tools_btn" id="bulk_bets_btn" style="width: 140px; text-align: center; cursor:pointer;">复制批量信息</div>
        </div>
    `;
                        $('body').append(uiHtml);

                        // 2. 定义一个同步函数，确保 UI 和配置数据一致
                        function syncUiWithConfig() {
                            if (typeof G !== 'undefined' && G.get) {
                                // 读取配置（注意：!! 将其强制转为布尔值，防止 undefined 导致未勾选）
                                let onlyBetsVal = !!G.get('betsCopyOnlyBets');
                                let bulkModeVal = !!G.get('bulkOutputMode');

                                $('#betsCopyOnlyBets_chk').prop('checked', onlyBetsVal);
                                $('#bulkOutputMode_chk').prop('checked', bulkModeVal);

                                console.log('UI配置已同步:', {onlyBetsVal, bulkModeVal});
                            }
                        }

                        // 3. 立即执行一次同步，并在 500ms 后再次补填（防止 G 对象延迟加载）
                        syncUiWithConfig();
                        setTimeout(syncUiWithConfig, 500);

                        // 4. 绑定保存事件
                        $('#betsCopyOnlyBets_chk').on('change', function() {
                            let isChecked = $(this).prop('checked') ? 1 : 0;
                            if (typeof G !== 'undefined' && G.set) {
                                G.set('betsCopyOnlyBets', isChecked);
                                // 某些脚本框架需要手动触发保存到硬盘
                                if(typeof G.save === 'function') G.save();
                                message('只复制下注信息模式：' + (isChecked ? '开启' : '关闭'), 1);
                            }
                        });

                        $('#bulkOutputMode_chk').on('change', function() {
                            let isChecked = $(this).prop('checked') ? 1 : 0;
                            if (typeof G !== 'undefined' && G.set) {
                                G.set('bulkOutputMode', isChecked);
                                if(typeof G.save === 'function') G.save();
                                message('批量快速复制模式：' + (isChecked ? '开启' : '关闭'), 1);
                            }
                        });

                        // 5. 绑定按钮点击
                        $('#copy_bets_btn').click(copy_bets);
                        $('#bulk_bets_btn').click(copy_bulk_bets);
                    }
                }
    ...
    function zhiboTools()
    {
        ...
        if (localTitle.match(key) != null && localTitle.match(/直播/) == null ){
    ...
    function copy_bets(returnText = false) {
        let onlyBets = (typeof G !== 'undefined' && G.get) ? G.get('betsCopyOnlyBets') : false;
        let showHead = (typeof G !== 'undefined' && G.get) ? G.get('betsCopyHead') : true;
        let bets_copy = '';

        if (showHead && !onlyBets) {
            let currentPage = parseInt(typeof page !== 'undefined' ? page : 1);
            ...
            if (currentPage === 1) {
                bets_copy += '楼层\tID\t下注点数\t下注场数\t得分\t队伍编号\t提示\t威望\t級別\t注册\t评分\tTips\tBy\r\n';
            }
        }

        bets_copy += extract_bets_data($(document), 1, true, onlyBets);
    ...
    // ===== 批量状态存储（全局）=====
    const BULK_KEY = '__bets_bulk_state__';

    function getBulkState(){
        try{
            return JSON.parse(sessionStorage.getItem(BULK_KEY));
        }catch(e){
            return null;
        }
    }
    ...
    function setBulkState(state){
        if(state){
            sessionStorage.setItem(BULK_KEY, JSON.stringify(state));
        }else{
    ...
    // ===== 辅助函数 (保持不变或微调) =====
    window.getBulkState = function () {
        try { return JSON.parse(sessionStorage.getItem('__bets_bulk_state__')); } catch (e) { return null; }
    };
    window.setBulkState = function (state) {
        if (state) sessionStorage.setItem('__bets_bulk_state__', JSON.stringify(state));
        else sessionStorage.removeItem('__bets_bulk_state__');
    };

    // ===== 核心提取函数（合并了单页与批量的所有逻辑） =====
    function extract_bets_data($context, pageNum, isFirstPageOfBatch, onlyBets) {
        // 移除点评/评分等多余元素干扰
        $context.find('.post_comm').remove();
        ...
        let copy_tample = '{floor}\t{author}\t{bet_points}\t{bet_count}\t{points}\t{team}\t{tips}\t{v}\t{group}\t{reg}\t{score}\t{remark}\t{by}';

        // 遍历帖子
        $context.find("div[id^='cont']:not(#conttpc)").each(function(i, bet) {
            let $bet = $(bet);
            let $table = $bet.closest('table'); // 定位当前楼层的主容器

            // 1. 基础数据初始化
            let points_item = {
                author: $table.find('.r_two b:first').text().replace('[樓主]', '').trim(),
                floor: $table.find('.tipad .s3').text().trim(),
                post_time: '',
                bet_points: 0,
                bet_count: 0,
                tips: '',
            ...
            // 2. 提取 Tips 和 评分记录 (By/Remark/Score)
            let tips = $table.find('.tips').first().text().trim();
            // 处理普通 Tips 中的 By
            if (tips && !/本帖最近評分記錄/.test(tips)) {
                tips = tips.replaceAll(/\s+|\r+|\n+| +/g, ' ');
                if (/By：/.test(tips)) {
                    let mp = tips.match(/(?<remark>.+?)By：(?<by>\S+)/);
                    if (mp && mp.groups) {
            ...
            // 处理评分记录
            let $ratingTips = $table.find('.tips').filter((_, el) => /本帖最近評分記錄/.test($(el).text())).first();
            if ($ratingTips.length) {
                let ratingText = $ratingTips.text().replace('威望', ' 威望');
                let reg_tips = /威望:(?<points>(\+|\-)\d{1,})\((?<by>\S+)\)/g;
                let match;
                while ((match = reg_tips.exec(ratingText)) !== null) {
                    points_item.score += parseInt(match.groups['points'].trim());
                    points_item.by += points_item.by ? '，' + match.groups['by'].trim() : match.groups['by'].trim();
                }
                // 提取评分留言
                let tips_tmp = ratingText.replace('本帖最近評分記錄：', '').replace(reg_tips, '').trim();
                if (tips_tmp) {
                    points_item.remark += points_item.remark ? '，' + tips_tmp : tips_tmp;
                }
            }

            // 3. 提取发帖时间与超时判断
            let tipadText = $table.find('.tipad').first().text();
            let mp = tipadText.match(reg_date_time);
            if (mp && mp.groups) points_item.post_time = mp.groups['post_time'].trim();

            if (limit_time && points_item.post_time) {
                let date1 = new Date(points_item.post_time);
                let date2 = new Date(limit_time);
                if (date1.getTime() - date2.getTime() > 5 * 60 * 1000) {
                    points_item.tips += points_item.tips ? '、超时下注' : '超时下注';
                }
            }

            // 4. 提取下注点数 (Points Tips & Bet Content)
            let $points_tips = $bet.find('.points_tips');
            if ($points_tips.length > 0) points_item.points = $points_tips.text().trim();

            let floorMatch = (points_item.floor || '').match(/(\d+)/);
            if (floorMatch) points_item.floor = parseInt(floorMatch[1].trim());

            // 清理 HTML 用于正则匹配
            let bet_html = $bet.html().replaceAll('>', '>\r\n');
            let points_bet_reg = /(下注点数|下註點數|點數|点数)(：|:)\s*(?<points>\d+)/;
            mp = bet_html.match(points_bet_reg);
            if (mp && mp.groups) points_item.bet_points = parseInt(mp.groups['points'].trim());

            // 冲刺赛模式自动算分
            try {
                let keyStr = G.get('suanfenCCScoreKey');
                if (keyStr) {
                    let key = new RegExp(keyStr);
                    if (key.test(localTitle) && points_item.bet_points === 0) {
                        let cc_points = parseInt(G.get('suanfenCCScorePoints'));
                        if (!isNaN(cc_points)) points_item.bet_points = cc_points;
                    }
                }
            } catch (e) {}

            if (/重新編輯/.test(bet_html)) {
                points_item.tips += points_item.tips ? '、重新編輯' : '重新編輯';
            }

            // 清理 HTML 用于后续内容分析
            let clean_html = $bet.html().replaceAll('\t', '').replaceAll('\r', '').replaceAll('\n', '');
            clean_html = clean_html.replace(/<font color="gray">(.*)重新編輯 ]<\/font>/g, '');
            clean_html = clean_html.replaceAll('&nbsp;', '').replaceAll(":", "：").replaceAll(' ：', '：');

            // 5. 提取作者详细信息 (用于 OnlyBets 模式或级别验证)
            let author_html = $table.find('.r_two').text().trim().replace(/\n/, '').replace(/^[\r\n\s\f\t\v\o]+/gm, "");
            let author_info = { name: points_item.author };
            if (/級別：(.*)/.test(author_html)) {
                ...
                author_info.group = ''; author_info.v = ''; author_info.r = '';
            }
            points_item.v = author_info.v ? author_info.v.replace(' 點', '') : '';
            points_item.group = author_info.group;
            points_item.reg = author_info.r;

            // 6. 提取队伍/编号
            if (/编号/.test(clean_html)) {
                if (!(/编号：/.test(clean_html))) clean_html = clean_html.replace('编号', '编号：');
                let tm = /编号：([^<]*)/.exec(clean_html);
                if (tm) points_item.team = tm[1].trim();
            } else if (/队伍名字/.test(clean_html)) {
                let tm = /队伍名字：([^<]*)/.exec(clean_html);
                if (tm) points_item.team = tm[1].trim();
            }
            if (/竞猜/.test(clean_html)) {
                clean_html = clean_html.replace('尾号', '').replace('数字', '');
                let tm = /竞猜：([^<]*)/.exec(clean_html);
                if (tm) points_item.team = tm[1].trim();
            }

            // 超级别验证
            if (points_item.bet_points > 0 && typeof check_level !== 'undefined') {
                if (!check_level(points_item.group, points_item.bet_points)) {
                    points_item.tips += points_item.tips ? '、超級別下注' : '超級別下注';
                }
            }

            // ===========================================
            // 核心分支：处理 "只复制下注信息" vs "完整模式"
            // ===========================================
            if (onlyBets) {
                // === 修复点：此处完整移植了 copy_bets 中关于 OnlyBets 的特殊逻辑 ===
                let bet_content_raw = clean_html.replaceAll('<br>', '\r\n').replaceAll('<br/>', '\r\n').replaceAll('<br />', '\r\n');
                bet_content_raw = bet_content_raw.replace(/<div class="points_tips">\S*<\/div>/g, "");

                // A. 世界杯活动特殊格式
                if (/世界杯活动正式报名帖/.test(localTitle)) {
                    let sign_info = {
                        author: points_item.author,
                        floor: points_item.floor,
                        v: author_info.v,
                        group: author_info.group,
                        reg: author_info.r,
                        c: author_info.c,
                        id: '', team: '', id2: '', p1: '', p2: '', p3: '', p4: '', p5: ''
                    };
                    if (/参赛ID：(.*)/.test(bet_content_raw)) sign_info.id = /参赛ID：(.*)/.exec(bet_content_raw)[1].trim();
                    if (/参赛队伍名：(.*)/.test(bet_content_raw)) sign_info.team = /参赛队伍名：(.*)/.exec(bet_content_raw)[1].trim();
                    if (/备用ID：(.*)/.test(bet_content_raw)) sign_info.id2 = /备用ID：(.*)/.exec(bet_content_raw)[1].trim();

                    if (/队员1：(.*)/.test(bet_content_raw)) {
                        let p1 = /队员1：(.*)/.exec(bet_content_raw)[1];
                        if (p1) {
                            sign_info.p1 = p1.trim();
                            sign_info.p2 = (/队员2：(.*)/.exec(bet_content_raw) || [])[1]?.trim() || '';
                            sign_info.p3 = (/队员3：(.*)/.exec(bet_content_raw) || [])[1]?.trim() || '';
                            sign_info.p4 = (/队员4：(.*)/.exec(bet_content_raw) || [])[1]?.trim() || '';
                        }
                    }
                    if (/队员5：(.*)/.test(bet_content_raw)) sign_info.p5 = (/队员5：(.*)/.exec(bet_content_raw) || [])[1]?.trim() || '';

                    if (sign_info.floor <= 32) {
                        bets_copy += '{floor}\t{team}\t{author}\t{id2}\t{p1}\t{p2}\t{p3}\t{p4}\t{p5}\r\n'.format(sign_info);
                    } else {
                        bets_copy += '{floor}\t{author}\t{v}\t{id}\t{id2}\t{team}\t{group}\r\n'.format(sign_info);
                    }
                }
                // B. 冲刺赛报名特殊格式
                    ...
                    bets_copy += '{floor}\t{author}\t{id2}\t{v}\t{c}\r\n'.format(sign_info);
                }
                // C. 默认 OnlyBets 格式 (包含作者信息的文本块)
                else {
                    bets_copy += '{name}\r\n級別：{group}\r\n發帖：{p}\r\n威望：{v}\r\n金錢：{u}\r\n貢獻：{g}\r\n認證：{c}\r\n'.format(author_info);
                    bets_copy += bet_content_raw.replace(/^[\r\n\s\f\t\v]+/gm, ""); // 清理头部空白
                    bets_copy += '\r\n';
                    // 追加 tipad 尾部信息
                    bets_copy += tipadText.replaceAll('\r', '').replaceAll('\n', '').replaceAll('\t', '') + '\r\n';
                }
            }
            else {
                // ===========================================
                ...
                // ===========================================
                let bets_copy_item = '';
                let re_bet = /((下注|站邊)(球|选)\S：|:)(\s*)(?<team>[^<]*)/g;
                if (/竞猜|投票/.test(localTitle)) {
                    re_bet = /(\d+)\.(\S*?)(：|:)(\s*)(?<team>[^<]*)/g;
                }

                let ms = clean_html.match(re_bet);
                if (ms) {
                    $.each(ms, function(k, m) {
                        let mc = m.match(/((\S*?)\S：|:)(\s*)(?<team>[^<]*)/);
                        if (mc && mc.groups) {
                            let bet_item = mc.groups['team'].trim().replace('<br>', '').trim();
                            bet_item = bet_item.replace(/^[ |&nbsp;]+$/, '');
                            if (bet_item) points_item.bet_count += 1;
    ...
    // ===== 批量复制入口 =====
    function copy_bulk_bets() {

        // 1. 高速模式 (不刷新页面)
        if (G.get('bulkOutputMode')) {
            (async function () {
                let onlyBets = G.get('betsCopyOnlyBets');
                let curPage = parseInt(page) || 1;
                let maxPages = parseInt(G.get('bulkMaxPages')) || 5;
                let totalResult = '';

                // 计算最大页数
                let maxPage = curPage;
                let lastHref = $('.pages a#last[href*="page="]').attr('href');
                if (lastHref) {
                    let m = lastHref.match(/page=(\d+)/);
                    if (m) maxPage = parseInt(m[1]);
                }
                if (!maxPage || maxPage <= curPage) {
                    $('.pages input[value*="/"]').each(function () {
                        let m = $(this).val().match(/\/(\d+)/);
                        if (m) maxPage = parseInt(m[1]);
                    });
                }
                if (!maxPage || maxPage <= curPage) {
                ...
                let endPage = Math.min(maxPage, curPage + maxPages - 1);
                let baseUrl = localHref.split('#')[0];
                if (!/[?&]page=\d+/.test(baseUrl)) {
                    baseUrl += (baseUrl.indexOf('?') > -1 ? '&' : '?') + 'page=1';
                }

                // 添加总表头 (只在第一页且非 OnlyBets 模式)
                if (curPage === 1 && G.get('betsCopyHead') && !onlyBets) {
                    totalResult += '楼层\tID\t下注点数\t下注场数\t得分\t队伍编号\t提示\t威望\t級別\t注册\t评分\tTips\tBy\r\n';
                }
                ...
                for (let i = curPage; i <= endPage; i++) {
                    ...
                    if (i === curPage) {
                        ...
                        let url = baseUrl.replace(/([?&])page=\d+/, '$1page=' + i);
                        let html = await new Promise(r => $.get(url, r).fail(() => r(null)));
                        if (!html) break;
                        // 对 HTML 进行预处理以防止图片加载等
                        let safeHtml = html.replace(/<img[^>]*>/gi, '');
                        $context = $('<div>' + safeHtml + '</div>');
                    }

                    // 调用核心提取
                    totalResult += extract_bets_data($context, i, i === curPage, onlyBets);

                    // 进度提示
                    message(`正在批量获取第 ${i} / ${endPage} 页...`, 1);

                    // 简单延时防止过快
                    if (i !== endPage) await new Promise(r => setTimeout(r, 200));
                }

                GM_setClipboard(totalResult, 'text');
                message(`已成功批量复制第 ${curPage} - ${endPage} 页下注信息`, 5);
            })();
            return;
        }

        // 2. 原始翻页模式
        let state = window.getBulkState();
        if (!state) {
            let maxPage = getTotalPages();
            state = { start: page, current: page, end: maxPage, text: '' };
        }

        show_bulk_progress(state.current, state.end);

        // 复用 copy_bets 的逻辑 (true 表示返回文本)
        state.text += copy_bets(true);
        state.current++;

        if (state.current <= state.end) {
            window.setBulkState(state);
            let params = new URLSearchParams(location.search);
            params.set('page', state.current);
            location.href = location.pathname + '?' + params.toString();
            return;
        }

        GM_setClipboard(state.text);
        window.setBulkState(null);
        hide_bulk_progress();
        message(`批量下注信息已完成（${state.start}-${state.end} 页）`, 5);
    }

    async function bulk_bets_fast_mode() {
        let startPage = page;
        let maxPages = parseInt(G.get('bulkMaxPages')) || 5;
        let result = '';
        ...
        for (let i = 0; i < maxPages; i++) {
            ...
            // ⚠️ 关键：复用你原来的解析函数
            result += extract_bets_from_dom(dom);
        }

        GM_setClipboard(result);
        message('极速批量下注信息已完成', 5);
    }


    window.copy_bulk_bets = function () {
        let state = window.getBulkState();
    ...
    function show_bulk_progress(current, total){
        let box = document.getElementById('bulk_progress_box');
        if(!box){
            box = document.createElement('div');
            box.id = 'bulk_progress_box';
            box.style.cssText = `position: fixed; top: 150px; right: 20px; z-index: 9999; background: #fff; border: 2px solid #ff9900; padding: 8px 12px; font-size: 14px;`;
            document.body.appendChild(box);
        }
        box.textContent = `当前批量进度：${current} / ${total} 页`;
    }

    function getTotalPages(){
        let input = document.querySelector('.pages input');
        if(input){
            let m = input.value.match(/\/(\d+)/);
            if(m) return parseInt(m[1], 10);
        }
        let max = page;
        document.querySelectorAll('.pages a[href]').forEach(a=>{
    ...
    setTimeout(function () {
        try {
            let state = window.getBulkState();
            if (state && state.current === page && !G.get('bulkOutputMode')) {
                window.copy_bulk_bets();
            }
        } catch (e) {
            console.error('BULK RESUME ERROR', e);
        }
    }, 800);

    function hide_bulk_progress(){
        let box = document.getElementById('bulk_progress_box');
        if(box) box.remove();
    }
    ...
    function settle_up(){
        ...
        $.each($("div[id^='cont']:not(#conttpc)"), function(i, bet) {
            ...
            points_list_copy += copy_tample.format(points_item);
        })
