...
(function () {
    ...
    // Функция для получения текста для аукциона/копирования
    function getAuctionText() {
        ...
        return `${videoTitle} (${totalMinutes} минут - ${buyoutAmount})`;
    }

    // Функция для отправки данных на сайт аукциона
    function sendToAuction() {
        const dataToSend = getAuctionText();
    ...
    function copyToClipboard() {
        const textToCopy = getAuctionText();
        navigator.clipboard.writeText(textToCopy).then(() => {
            GM_log(`[${currentPlatform}] Текст скопирован: ${textToCopy}`);
        }).catch(err => {
            GM_log(`[${currentPlatform}] Ошибка копирования: ${err}`);
        });
    }
    ...
    function addControlPanel() {
        ...
        const copyButton = document.createElement("button");  // Кнопка КОПИРОВАТЬ
        copyButton.className = "copy-title-button";
        copyButton.textContent = "КОПИРОВАТЬ";
        copyButton.title = "Копировать в буфер обмена";
