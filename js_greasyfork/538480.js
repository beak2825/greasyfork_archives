...
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/538480/RomaTodayit%3A%20Hide%20Annoying%20popups%20%28the%20anti-adblock%20popup%20and%20others%29.user.js
// @updateURL https://update.greasyfork.org/scripts/538480/RomaTodayit%3A%20Hide%20Annoying%20popups%20%28the%20anti-adblock%20popup%20and%20others%29.meta.js
// ==/UserScript==

//
// >>>FILENAME=RomaToday_it_HideAnnoyingPopups.js<<<
//
...
(function()
{
    ...
    blib.consoleLog(`CY==> RomaToday_it_HideAnnoyingPopups: Using library 'BasicLib' (version: ${blib.getVersion()})`);

    const haplib = window.HideAnnoyingPopupsLib;
    blib.consoleLog(`CY==> RomaToday_it_HideAnnoyingPopups: Using library 'HideAnnoyingPopupsLib' (version: ${haplib.getVersion()})`);

    //// When enabled, additional log will be shown.
    //haplib.setShowDebugLog(true);

    const myVersion = GM_info.script.version;
    blib.consoleLog(`CY==> RomaToday_it_HideAnnoyingPopups: HELLO! Loading script (version: ${myVersion})...`);

    const currUrl = window.location.href;
    blib.consoleLog(`CY==> RomaToday_it_HideAnnoyingPopups: currUrl='${currUrl}'`);
    ...
    function onMutatedAttribute(mutation)
    {
        ...
        blib.consoleLog(`CY==> RomaToday_it_HideAnnoyingPopups: onMutatedAttribute - BEGIN`);
        ...
        {
            blib.consoleLog(`CY==> RomaToday_it_HideAnnoyingPopups: onMutationList - BEFORE: mutation.target.classList='${mutation.target.classList}'`);
            mutation.target.classList.remove("iub--active");
            blib.consoleLog(`CY==> RomaToday_it_HideAnnoyingPopups: onMutationList - AFTER: mutation.target.classList='${mutation.target.classList}' ---> attribute modification REMOVED`);
            stopMutationProcessing = true;
        }

        blib.consoleLog(`CY==> RomaToday_it_HideAnnoyingPopups: onMutatedAttribute - END - stopMutationProcessing=${stopMutationProcessing}`);
    ...
    blib.consoleLog("CY==> RomaToday_it_HideAnnoyingPopups: Script loaded");
})();
