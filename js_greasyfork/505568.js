// ==UserScript==
// @name         battle_damage_tooltip (debug)
...
// @grant        GM_getValue
// @grant unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/505568/battle_damage_tooltip%20%28debug%29.user.js
// @updateURL https://update.greasyfork.org/scripts/505568/battle_damage_tooltip%20%28debug%29.meta.js
// ==/UserScript==
...
<br>`;
    ...
    function individual_calc_innerHTML(atk_obj_index, def_obj_index) {
        ...
        let calcHTML = calcPhysHTML(attacker, defender, dmg, distance_str) + calcHellFireHTML(attacker, defender, cre_collection, dmg) + (lastturn > 0 ? calcMagicHTML(attacker, defender, cre_collection, dmg) : "");
...
`
    ...
    const loadedInterval = setInterval(() => {
        mobileSetting = document.querySelector("#submit_hod_settings_whole");
        if (!mobileSetting) return;
    ...
    window.addEventListener("keyup", event => {
        /*    // Должно быть открытие настроек на Esc, но чтоб не мешало в бою надо устранять ситуацию когда открыта инфа о существе и на esc она закрывается
        if (event.key === 'Escape'){
            for (const panel of document.querySelectorAll(".window_relative")){
                if (panel.style.display !== "none" && panel.id !== "win_Settings") {console.log("smth opened already"); return}
            }
            document.querySelector("#win_Settings").style.display = document.querySelector("#win_Settings").style.display ? '' : 'none';
        } */
        if ((document.querySelector("#chattext") !== document.activeElement) && (document.querySelector("#chattext_classic") !== document.activeElement)) {
            if (["e", "E", "у", "У", ".", "ю"].includes(event.key)) manageDamageCalc();

            if (["u", "U", "г", "Г"].includes(event.key)) {
                //if (["N", "n", "т", "Т"].includes(event.key)) {
    ...
    function initMagicCalc() {
        ...
        stage.pole.calcpoison = function calcpoison(i, j, eff) {
            ...
            if (hera > 0) {
                ...
                if ((herd > 0) && (magic[herd]['bnd'])) {
                    eff = Math.round(eff / (1 + tmagic[herd]['bnd']['effect'] / 100));
                };
        ...
        defend_button_release = function defend_button_release() {
            console.log("altered defend funtcion")
            ...
            if (Object.values(heroes).includes(activeobj)) {
                alert("Скрипт battle_damage_tooltip заблокировал кнопку обороны во время хода героя!")
                return 0;
            }
...
try{
    ...
    document.body.insertAdjacentHTML("afterBegin",
            ...
            <span style="color: white; font-size: 12px;">${e.stack}</span>
        </div>`);
}