...
// @name        Starting To Bloom TM - V1.1.6
...
// @grant        all
// @downloadURL https://update.greasyfork.org/scripts/372494/Starting%20To%20Bloom%20TM%20-%20V116.user.js
// @updateURL https://update.greasyfork.org/scripts/372494/Starting%20To%20Bloom%20TM%20-%20V116.meta.js
// ==/UserScript==
	...
	$("div.player_name > a").each(function(e, player) 
	{
		...
		{ 
			if (xhr.readyState == 4)
			{
                ...
                { var player_index = transfer_info_ar[i]["id"] ;
                 ...
                 function Last_Season_Avg_Ti(TI,Season_TI,session,months,playerID,wage) {
                    ...
                    { var This_Season = months + 1;
                      ...
                      if ((playerID > 124435088) || (wage < 30001))
                          return 0;
                      else{
                ...
                return ((Math.pow(2,Math.log(weight*SI)/Math.log(Math.pow(2,7))) - Math.pow(2,Math.log(weight*wage/(wage_rate*0.8))/Math.log(Math.pow(2,7)))).toFixed(1)*10) - 1 ;
                 ...
                 function return_starting_blooming(Last_Season_Avg_Ti,Season_Avg,SI,age,rec)
                        {var Age = parseInt(age);
                         ...
                         if ((Last_Season_Avg_Ti < 10) && (Last_Season_Avg_Ti > 0))
                         ...
                         if ((Age == 22)&&(rec > 2.0)&&(rec <4.5)&&(Season_Avg > 11)) {return "$$$";}
                         if ((Age == 23)&&(rec > 2.5)&&(rec <4.5)&&(Season_Avg > 9)) {return "$$$";}
                         if ((Age == 24)&&(rec > 2.5)&&(rec <4.5)&&(Season_Avg > 8)) {return "$$$";} else {return "----";}
                ...
                $("a[player_link=" + resultado.player.player_id + "]").closest("tr").append("<td class='right align_right SB' style='padding-right:5px;'><span>" + return_starting_blooming(Last_Season_Avg_Ti(return_TI((arrary[id_index(resultado.player.player_id,arrary)][1]),resultado.player.skill_index,player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary))),Season_TI(player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary)),SI(resultado.player.skill_index),return_wage(resultado.player.wage),19.836),session,parseInt(resultado.player.months),parseInt(resultado.player.player_id),return_wage(resultado.player.wage)),return_TI((arrary[id_index(resultado.player.player_id,arrary)][1]),resultado.player.skill_index,player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary))),resultado.player.skill_index,resultado.player.age,resultado.player.rec_sort)  + "</span></td>");
