// ==UserScript==
// @name         Autotrading (Sell)
// @version      2.9
// @description  Auto buy
// @include      https://*/game.php*screen=market&mode=exchange*
// @namespace https://greasyfork.org/users/1388863
// @downloadURL https://update.greasyfork.org/scripts/525371/Autotrading%20%28Sell%29.user.js
// @updateURL https://update.greasyfork.org/scripts/525371/Autotrading%20%28Sell%29.meta.js
// ==/UserScript==
...
function checkmerchant(villageId) {
    const reloadKey = `reloadmerchant${villageId}`;
    const reloadTimeKey = `reloadTime${villageId}`;
    const justReloadedKey = `justReloaded${villageId}`;

    const availableMerchant = parseInt(document.getElementById('market_merchant_available_count').innerText);
    const currentTime = new Date();

    let reloadmerchant = parseInt(sessionStorage.getItem(reloadKey)) || 0;
    let reloadTimeRaw = sessionStorage.getItem(reloadTimeKey);
    let reloadTime = reloadTimeRaw ? new Date(reloadTimeRaw) : null;
    let justReloaded = sessionStorage.getItem(justReloadedKey) === "true";

    console.log(`Cek merchant [${villageId}] pada ${currentTime.toLocaleTimeString()} -> availableMerchant: ${availableMerchant}`);
    console.log(`Status awal [${villageId}] -> reloadmerchant: ${reloadmerchant}, reloadTime: ${reloadTime ? reloadTime.toLocaleTimeString() : 'null'}, justReloaded: ${justReloaded}`);

    if (justReloaded) {
        console.log("Halaman baru saja direload. Reset status reloadmerchant.");
        sessionStorage.removeItem(justReloadedKey);
        sessionStorage.setItem(reloadKey, "0");
        reloadmerchant = 0;
    }

    if (availableMerchant > 0) {
        console.log("✅ Merchant tersedia, jalankan autoProcessMaterialsSell()");
        sessionStorage.removeItem(reloadTimeKey);
        sessionStorage.setItem(reloadKey, "0");
        autoProcessMaterialsSell();  // pastikan fungsi ini relevan dengan villageId
        return;
    }

    if (reloadmerchant === 1) {
        console.log("🕐 reloadmerchant == 1, cek apakah waktunya reload...");
        if (reloadTime && currentTime >= reloadTime) {
            console.log("🔁 Waktu reload tercapai. Reload halaman sekarang.");
            sessionStorage.setItem(justReloadedKey, "true");
            location.reload();
        } else {
            console.log(`⏳ Belum waktunya reload. reloadTime: ${reloadTime.toLocaleTimeString()}`);
        }
        return;
    }

    if (availableMerchant < 1 && reloadmerchant === 0 && !reloadTimeRaw) {
        const randomMinutes = Math.floor(Math.random() * 3) + 8; // 8–10 menit
        reloadTime = new Date(currentTime.getTime() + randomMinutes * 60000);
        sessionStorage.setItem(reloadKey, "1");
        sessionStorage.setItem(reloadTimeKey, reloadTime.toISOString());
        console.log(`📅 Merchant tidak tersedia & belum pernah reload → Menjadwalkan reload pada ${reloadTime.toLocaleTimeString()}`);
    } else {
        console.log("📌 Merchant tidak tersedia & reload sudah dijadwalkan. Tidak menjadwalkan ulang.");
    }
}
    ...
    function createUI() {
        ...
        if (savedStatus === 'running') {
            intervalId = setInterval(checkmerchant, Math.random() * 8000 + 2000);
