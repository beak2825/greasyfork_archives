// ==UserScript==
// @name            AMQ cover image + adjust UI
// @namespace       Racoonsaki
// @author          Racoonsaki + xTsuSaKu
// @version         0.7.0
// @description     Display cover image and Adjust UI
...
// @license         MIT license
// @downloadURL https://update.greasyfork.org/scripts/516280/AMQ%20cover%20image%20%2B%20adjust%20UI.user.js
// @updateURL https://update.greasyfork.org/scripts/516280/AMQ%20cover%20image%20%2B%20adjust%20UI.meta.js
// ==/UserScript==
...
(async function () {
    ...
    if (oldCache) {
        try {
            ...
            console.log("AMQ Cover Script: Successfully migrated cache from localStorage to GM storage.");
        } catch (e) {
            console.error("AMQ Cover Script: Failed to migrate old cache. It might be corrupted.", e);
            localStorage.removeItem(LOCALSTORAGE_CACHE_KEY);
        }
    }
    ...
    try {
        ...
        console.error("AMQ Cover Script: Failed to parse cover cache from GM storage", e);
        coverCache = {};
    ...
    function saveToCoverCache(annId, url) {
        ...
        if (url.includes("/extraLarge/")) {
            prefix = "A";
            path = url.split("/extraLarge/")[1];
        } else if (url.includes("/large/")) {
            prefix = "B";
            path = url.split("/large/")[1];
        } else if (url.includes("/medium/")) {
            prefix = "C";
            path = url.split("/medium/")[1];
        } else if (url.includes("/encyc/")) {
            prefix = "N";
            path = url.split("/encyc/")[1];
        }
    ...
    async function fetchCoverImage(malId, annId, season, year, applyImageCallback) {
        ...
        if (isAprilFools) {
            ...
            if (selected && typeof applyImageCallback === "function") {
    ...
    function fetchFromAnilist(malId, season, year) {
        if (!malId || !season || !year) {
            return Promise.resolve(null);
        }

        const query = `
            query ($malId: Int, $season: MediaSeason, $seasonYear: Int) {
                Media(idMal: $malId, type: ANIME, season: $season, seasonYear: $seasonYear) {
                    coverImage { extraLarge, large, medium }
                }
            }`;
        const variables = { malId, season: season.toUpperCase(), seasonYear: year };
        ...
        return new Promise((resolve) => {
            GM_xmlhttpRequest({
                ...
                onerror: (err) => { console.warn("[AniList] Request Failed:", err); resolve(null); },
                ontimeout: () => { console.warn("[AniList] Request Timed Out"); resolve(null); }
            });
        });
    }
            ...
            GM_xmlhttpRequest({
                ...
                onload: (response) => {
                    try {
                        ...
                        console.warn("[ANN] XML Parse Error:", err);
                        resolve(null);
                    }
                },
                onerror: (err) => { console.warn("[ANN] Request Failed:", err); resolve(null); }
            });
        });
    }
    ...
    async function addCoverImage(imageUrl) {
        ...
        coverImage.onload = async () => {
            try { await coverImage.decode(); } catch (e) { console.error("Image decode failed:", e); }
            requestAnimationFrame(() => {
    ...
    onReady(() => {
        new Listener("answer results", (payload) => {
            ...
            if (!vintage) return;
            const [season, yearStr] = vintage.split(' ');
            const year = parseInt(yearStr, 10);
            if (season && !isNaN(year)) {
                fetchCoverImage(malId, annId, season, year, addCoverImage);
            }
        }).bindListener();
        ...
        if (quizPage) {
            ...
            const observer = new MutationObserver(() => {
                ...
                if (isQuizActive) {
                    ...
                    if (gcMessageContainer) {
                        setTimeout(() => { gcMessageContainer.scrollTop = gcMessageContainer.scrollHeight; }, 150);
                    }

                    if (gcMessageContainer && !gcMessageContainer._messageObserver) {
                        const messageObserver = new MutationObserver(() => {
                            if (!gcMessageContainer.matches(':hover')) {
                                gcMessageContainer.scrollTop = gcMessageContainer.scrollHeight;
                            }
                        });
                        messageObserver.observe(gcMessageContainer, { childList: true });
                        gcMessageContainer._messageObserver = messageObserver;
                    }
                    ...
                    const gcMessageContainer = document.getElementById("gcMessageContainer");
                    if (gcMessageContainer && gcMessageContainer._messageObserver) {
                        gcMessageContainer._messageObserver.disconnect();
                        delete gcMessageContainer._messageObserver;
                    }
                }
            });
            observer.observe(quizPage, { attributes: true, attributeFilter: ['class'] });
        }
    });
})();
