// ==UserScript==
// @name         wall-battlestats-and-activity
// @namespace    seintz.torn.wall-battlestats
// @version      7.1.1
// @author       seintz [2460991], finally [2060206], Shade [3129695], Kindly [1956699], Mr_Bob [479620], nao [2669774], & Hwa (TRF hee hee)
// @description  show tornstats spies on faction wall page, also activity & xanax (highlights)
// @license      GNU GPLv3
...
// @grant        unsafeWindow
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/549074/wall-battlestats-and-activity.user.js
// @updateURL https://update.greasyfork.org/scripts/549074/wall-battlestats-and-activity.meta.js
// ==/UserScript==
...
(function () {
     ...
     */
        ...
        /*! true or false */
        colorMode: false,
        // delay in milliseconds for API request (activity)
        apiDelay: 2000,
    };
    /*!
   * -------------------------------------------------------------------------
   ...
   */
    // HWA PART
    let activityCache = JSON.parse(localStorage.getItem("activityCache") || "{}");
    console.log("activity cache?");
    // HWA PART
    ...
    async function getActivity(userId) {
        const now = Date.now();
        const cacheEntry = activityCache[userId];

        // cache for 1 week
        if (cacheEntry && (now - cacheEntry.fetchedAt) < 7 * 24 * 60 * 60 * 1000) {
            return cacheEntry.data;
        }

        const since = Math.floor(Date.now() / 1000 - 30 * 24 * 60 * 60);
        const { tornApiKey, apiDelay } = window.nstWallBsSettings;

        const pastUrl = `https://api.torn.com/v2/user/${userId}/personalstats?stat=timeplayed,xantaken&timestamp=${since}&key=${tornApiKey}`;
        const currentUrl = `https://api.torn.com/v2/user/${userId}/personalstats?stat=timeplayed,xantaken&key=${tornApiKey}`;
        const profileUrl = `https://api.torn.com/user/${userId}?selections=profile&key=${tornApiKey}`;
        ...
        try {
            ...
            const [pastJson, currentJson] = await Promise.all([
                gmRequest(pastUrl),
                gmRequest(currentUrl)
            ]);

            // default divisor
            let days = 30;

            const pastXan = Array.isArray(pastJson.personalstats)
            ? pastJson.personalstats.find(stat => stat.name === "xantaken")?.value ?? 0
            : 0;
            const pastActivity = Array.isArray(pastJson.personalstats)
            ? pastJson.personalstats.find(stat => stat.name === "timeplayed")?.value ?? 0
            : 0;

            // if no past stats -> fallback to age
            if (pastXan === 0 && pastActivity === 0) {
                const profileJson = await gmRequest(profileUrl);
                if (profileJson?.age) {
                    days = profileJson.age; // use account age in days
                }
            }

            // current values (nested object)
            const currentXan = Array.isArray(currentJson.personalstats)
            ? currentJson.personalstats.find(stat => stat.name === "xantaken")?.value ?? 0
            : 0;
            const currentActivity = Array.isArray(currentJson.personalstats)
            ? currentJson.personalstats.find(stat => stat.name === "timeplayed")?.value ?? 0
            : 0;

            if (currentXan == undefined || currentActivity == undefined) {
                console.error("Failed to fetch activity:", err);
                return { xanpd: -1, activitypd: -1 };
            }

            const xanPerDay = (currentXan - pastXan) / days;
            const activityDiffSeconds = (currentActivity - pastActivity);
            const activityPerDayHours = (activityDiffSeconds / days / 3600); // 3600 sec = 1 hour

            const data = { xanpd: xanPerDay, activitypd: activityPerDayHours };

            activityCache[userId] = { data, fetchedAt: now };
            localStorage.setItem("activityCache", JSON.stringify(activityCache));
    ...
    async function getFactionData() {
        ...
        // Step 1: Collect all member IDs from all factions
        const factionPromises = factions.map(factionId => {
            return new Promise(resolve => {
                const url = `https://api.torn.com/faction/${factionId}?selections=basic&key=${tornApiKey}&comment=wallbs`;
                webReq(url, async (xhr) => {
                    try {
                        const factionJson = JSON.parse(xhr.responseText);
                        const members = factionJson.members || {};
                        allMemberIds.push(...Object.keys(members));
                        resolve();
                    } catch (err) {
                        console.error("Failed to parse faction JSON:", err);
                        resolve();
                    }
                });
            });
        ...
        // Step 2: Fetch activity SEQUENTIALLY with a delay
        for (const memberId of allMemberIds) {
            try {
                const activity = await getActivity(memberId);

                // Step 3: Update all DOM nodes for this member
                const nodes = document.querySelectorAll(`[data-userid="${memberId}"]`);
                nodes.forEach(node => {
                    updateStats(memberId, node, node.parentNode);
                });
            } catch (err) {
                console.error(`Failed to fetch activity for member ${memberId}:`, err);
            }
        }
    ...
    function updateStats(id, node, parentNode) {
        if (!node) return;
        node.dataset.userid = id; // hwa part
        ...
        try {
            const activityData = activityCache[id]?.data; // <-- access .data
            if (activityData) {
                const colorsLight = {
                    xan: "#FFD966",
                    activity: "#4DA6FF",
                ...
                // Detect mode based on text color
                const nodeColor = window.getComputedStyle(node).color;
                const rgb = nodeColor.match(/\d+/g).map(Number);
                const isDarkMode = rgb[0] === 221 && rgb[1] === 221 && rgb[2] === 221;

                const colors = isDarkMode ? colorsDark : colorsLight;

                if (activityData.xanpd >= 2 && activityData.activitypd >= 1) bgColor = colors.both;
                else if (activityData.xanpd >= 2) bgColor = colors.xan;
                else if (activityData.activitypd >= 1) bgColor = colors.activity;
                else bgColor = colors.neither;

                // add activity data
                activityHtml = `
                    </br>
                    <br/><b>X/d:</b> ${activityData.xanpd.toFixed(2)}
                    <br/><b>A/d:</b> ${activityData.activitypd.toFixed(2)} hrs
                `;
            }
        } catch (err) {
        }

        // Apply background color to the div
    ...
    function showStats(node) {
        ...
        if (bsNode) {
            bsNode.dataset.userid = id; // Hwa Part
        }
