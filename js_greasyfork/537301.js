// ==UserScript==
// @name         HYS Test Helper
// @namespace    https://yinr.cc/
// @version      0.1.2
// @description  北京市全员培训考试助手
// @author       Yinr
// @license      MIT
// @match        https://bjsqypx.haoyisheng.com/qypx/bj/exam.jsp*
// @match        https://bjsqypx.haoyisheng.com/qypx/bj/examQuizPass.jsp*
// @match        https://bjsqypx.haoyisheng.com/qypx/bj/examQuizFail.jsp*
...
// @grant        GM_listValues
// @grant        unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/537301/HYS%20Test%20Helper.user.js
// @updateURL https://update.greasyfork.org/scripts/537301/HYS%20Test%20Helper.meta.js
// ==/UserScript==
...
(function() {
     ...
     * @param {QuesRecord} ques 
     */
    const setAnswerLastWrong = (ques) => {
        const last = ques.answer.find(ans => ans.selected)
        last.right = false
        last.selected = false
    }
    ...
    if (path.includes('examQuizFail.jsp')) {
        ...
        for (let i = 0; i < error_ques.length; i++) {
            // const order = error_order[i]
            const ques_id = error_ques[i]
            // const answer = ansList[order]
            const ques = getQues(testRec, ques_id)
            setAnswerLastWrong(ques)
            // const answer = ques.answer.find(ans => ans.selected)
            // if (answer) {
            //     setAnswer(ques, answer.answer, false)
            // }
        }
        ...
        console.log(testRec)
    } else if (path.includes('examQuizPass.jsp')) {
        // 考试通过页
        const search = new URLSearchParams(document.location.search)
        const rightRate = search.get('rightRate')
        const course_id = search.get('course_id')
        const paper_id = search.get('paper_id')
        const testRec = {
            course_id, paper_id, ques: [],
            ...getTest(course_id, paper_id),
        }
        testRec.ques.forEach(q => {
            q.answer.forEach(a => {
        ...
        // 考试页
        GM_addStyle(`
            .kaoshi dl dd p label { cursor: pointer; }
            .kaoshi dl dd p.ansWrong { color: red; }
            .kaoshi dl dd p.ansRight { color: green; }
        `)
        const search = new URLSearchParams(document.location.search)
        /** @type {HTMLDivElement} */
        const box = document.querySelector('form[action]')
        /** @type {string} */
        ...
        /** @type {string} */
        const test_title = document.querySelector('div.block h2 > em').innerText
        /** @type {string} */
        ...
        const ques_els = Array.from(box.querySelectorAll('div.kaoshi>dl'))
        /** @type {string[]} */
        const ques_texts = ques_els.map(el => el.querySelector('dt').innerText)
        ...
        for (let ques_idx = 0; ques_idx < ques_els.length; ques_idx++) {
            ...
            const ans_els = Array.from(ques_el.querySelectorAll('dd>p'))
            ans_els.forEach(el => {
        ...
        box.querySelector('.kaoshi').addEventListener('click', e => {
        ...
        const title = document.querySelector('div.block h2')
        ...
        btn.addEventListener('click', () => {
            box.querySelector('input.btn1').click()
