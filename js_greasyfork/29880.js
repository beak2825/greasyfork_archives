// ==UserScript==
// @name         PeopleSoft Timecard Enhance
...
// @downloadURL https://update.greasyfork.org/scripts/29880/PeopleSoft%20Timecard%20Enhance.user.js
// @updateURL https://update.greasyfork.org/scripts/29880/PeopleSoft%20Timecard%20Enhance.meta.js
// ==/UserScript==
...
(function (){
    ...
    if( $( 'span.PAPAGETITLE[text="Timesheet"]' ) ) {
		...
		// Adding jquery ui for modal dialog boxes
        ...
        var targetFrame = $( '#ptifrmtgtframe' );
    ...
    function iframeLoaded() {
      ...
      $( 'button#autoFill' ).on( 'click', function() {
          ...
          // and prompt the user how to continue if they do.
          ...
          function fillTimes(){
              ...
              var whichView = $( 'span[id^="TR_WEEKLY_GRID$"]', ourFrame ),
...
///////////////////////////////////////////////////////////////////////////////
        ...
        // Sick days.
            ...
            // Set the Global Variable so we will not duplicate this function when we insert our elements.
            ptoViewable = true;

            // Gather all the PTO data points required to calculate future PTO
            var sick = Number( $( 'span[id="DERIVED_TL_COMP_END_BAL$0"]', ourFrame  ).text() ),
                vacation = Number( $( 'span[id="DERIVED_TL_COMP_END_BAL$1"]', ourFrame  ).text() ),
                personal = Number( $( 'span[id="DERIVED_TL_COMP_END_BAL$2"]', ourFrame  ).text() ),
                leaveText = $( '#win0divTL_COMPLEAV_TBLGP\\$0', ourFrame).text(),
            ...
            var ptoTable = $( 'table[id^="TL_COMPLEAV_TBL"] > tbody', ourFrame );

            ptoTable.append( '<tr id="ptoCalc"><td><table id="ptoCalcTable"><caption>Future Balances</caption><thead></thead><tbody></tbody><tfoot></tfoot></table></td></tr>' );
            var calcTable = $( '#ptoCalcTable', ourFrame );


            $( 'thead', calcTable ).append( '<tr><th style="text-align: left !important">Accrual Dates</th><th>Sick</th><th>Vacation</th><th>Personal</th></tr>' )
            ...
            function addPTOrows( lastUpdate, bonus ) {
                ...
                for ( let x = 0; x < 36; x++ ) {
                    ...
                    let ourTR = $( 'tr', tableBody ).last();
                    // Stripe odd rows
                    if ( x % 2 === 0 ) {
                        ourTR.css( 'background-color', '#e6e6e6' );
                    }
                    
                    // Add a new <td> with the bonus date for the month
                    ourTR.append( '<td>' + firstMonth.toDateString() + '</td>' );
                    
                    // Check if this is a sick time bonus month from 
                    // the sickAccrualMonths array.
                    $.each( sickAccrualMonths, function( index, value ) {
                    ...
                    ourTR.append( '<td style="text-align: right">' + sick + '</td' );
                    ...
                    ourTR.append( '<td style="text-align: right">' + vacation + '</td' );
                    ...
                    ourTR.append( '<td style="text-align: right">' + personal + '</td' );
            ...
            calcTable.add( calcTable.find('th') ).add( calcTable.find( 'td' ) ).css( {
                'border-bottom': '1px solid black',
                'border-collapse': 'collapse'
            } );
            
            calcTable.find( 'th' ).css( {
               'color': '#426a92',
               'font-size': '13px',
               'text-align': 'right'
            } );
            
            calcTable.find( 'td' ).css( { 
                'width': '100px'
            } );
        ...
        var ptoElement = $( 'span[id="DERIVED_TL_COMP_DESCR$0"]', ourFrame ).length;
        if ( ptoViewable === false && ptoElement ) {
            ptoExpand()
        }
...
///////////////////////////////////////////////////////////////////////////////
        ...
        // Adjust the length of the two info divs at bottom of page
        var divLeave = $( 'div[id^="win0divTL_COMPLEAV_TBLGP"]', ourFrame ),
            divTimeSummary = $( 'div[id^="win0divTL_INSTALL_VW3_GP"]', ourFrame ),
            divReportedTime = $( 'div[id^="win0divTL_RPT_TIME_GRIDGP"]', ourFrame );

        divLeave.add(divTimeSummary).add(divReportedTime).css( 'width', '888px' );
      
        // Adjust the space between the PTO divs
        $( '#ACE_width > tbody > tr:nth-child(24) > td', ourFrame ).css( 'height', '4px' );
    ...
    function showUserSettings() {
        ...
        if ( $('#prefix-modal').length < 1 ) {
            ...
            $( 'body' ).prepend( '<div id="prefix-modal" title="Product Code Prefix">Please enter the two digits prefix code used with the <b>Product Code</b> text box. (i.e. <b>99</b> is the prefix in <b>99</b>136, the Product Code for Holiday):</div>' );
                ...
                [
                    {
                        ...
                        click : function() {
                            ...
                            if ( $( '#prefix-code' ).val() !== "" ) {
								...
								GM_setValue( "userSettings", $( '#prefix-code' ).val() );
                            }
                            $( this ).dialog( 'close' );
    ...
    console.info( 'All synchronous functions complete.  Waiting for User initiated events.' );
