// ==UserScript==
// @name         [ARKHANGELSK] | Скрипт для Кураторов Форума by J.Megakrutoy
// @namespace    https://forum.blackrussia.online
// @version      1.0
// @description  По вопросам(ВК): https://vk.com/bibizyantag
// @author       James Megakrutoy
...
// @updateURL
// @downloadURL https://update.greasyfork.org/scripts/525654/%5BARKHANGELSK%5D%20%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%20%D0%A4%D0%BE%D1%80%D1%83%D0%BC%D0%B0%20by%20JMegakrutoy.user.js
// @updateURL https://update.greasyfork.org/scripts/525654/%5BARKHANGELSK%5D%20%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%20%D0%A4%D0%BE%D1%80%D1%83%D0%BC%D0%B0%20by%20JMegakrutoy.meta.js
// ==/UserScript==
  (function () {
	'use strict';
	...
	const buttons = [
      ...
      },
     {
	  ...
	  content:
		'[CENTER][IMG width="162px"]https://i.postimg.cc/JzfRn92C/photo-2025-01-30-18-06-54-Photoroom-1.png[/IMG][/CENTER]<br><br>' +
	...
	},
     {
	  title: '| Не по форме |',
	  content:
		'[CENTER][IMG width="162px"]https://i.postimg.cc/fRCqz6q8/photo-2025-01-30-18-06-54-no-bg-preview-carve-photos-1.png[/IMG][/CENTER]<br><br>' +
		"[B][CENTER][COLOR=#00FFFF][ICODE]{{ greeting }}, уважаемый {{ user.name }} [/ICODE][/COLOR][/CENTER][/B]<br><br>"+
	...
	},
{
 ...
 "[B][CENTER][COLOR=lavender]Приятной игры на [COLOR=#000000]BLACK[/COLOR] [COLOR=#FF0000]RUSSIA[/COLOR] .<br><br>"+
  ...
  addButton('Ожидание', 'wait', 'border-radius: 13px; margin-right: 5px; border: 4px solid; border-color: rgb(173, 216, 230, 10);');
  addButton('На расмотрение', 'pin', 'border-radius: 13px; margin-right: 5px; border: 4px solid; border-color: rgb(255, 140, 0, 10);');
  addButton('Важно', 'vajno', 'border-radius: 13px; margin-right: 5px; border: 4px solid; border-color: rgb(128, 0, 0, 10);');
	addButton('Одобрено', 'accept', 'border-radius: 13px; margin-right: 5px; border: 4px solid; border-color: rgb(110, 192, 113, 10);');
	addButton('Отказано', 'unaccept', 'border-radius: 13px; margin-right: 5px; border: 4px solid; border-color: rgb(220, 89, 89, 10);');
  addButton('Главному Админу', 'ga', 'border-radius: 13px; margin-right: 5px; border: 4px solid; border-color: rgb(178, 34, 34, 10);');
  addButton('Тех админу', 'techspec', 'border-radius: 13px; margin-right: 5px; border: 4px solid; border-color: rgb(0, 0, 139, 10);');
  addButton('Команде проекта', 'teamProject', 'border-radius: 13px; margin-right: 5px; border: 4px solid; border-color: rgb(255, 255, 0, 10);');
  addButton('Спец админу', 'Spec', 'border-radius: 13px; margin-right: 5px; border: 4px solid; border-color: rgb(255, 0, 0, 10);');
	addAnswers();
		...
		return `<div class="select_answer">${buttons
		  .map(
			(btn, i) =>
			  ...
			  `rippleButton"style="margin-left: 5px; margin-top: 10px; border-radius: 15px; style="margin:5px; background-color: ${btn.color || "grey"}"><span class="button-text">${btn.title}</span></button>`,
		  )
	.join('')}</div>`;
	}
...
const buttonConfig = (text, href) => {
  ...
  return button;
    ...
    const Button1 = buttonConfig("56", "https://forum.blackrussia.online/forums/%D0%A1%D0%B5%D1%80%D0%B2%D0%B5%D1%80-%E2%84%9656-arkhangelsk.2473/");

    bgButtons.append(Button1);


    const Button2 = buttonConfig("Жалобы на игроков", "https://forum.blackrussia.online/forums/%D0%96%D0%B0%D0%BB%D0%BE%D0%B1%D1%8B-%D0%BD%D0%B0-%D0%B8%D0%B3%D1%80%D0%BE%D0%BA%D0%BE%D0%B2.2500/");
