// ==UserScript==
// @name         专业化团队-项目维护工具
// @namespace    epoint.dzzw.dev
// @version      2.5
// @description  政务研发群内部使用
// @author       wl
...
// @license      MIT
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/454600/%E4%B8%93%E4%B8%9A%E5%8C%96%E5%9B%A2%E9%98%9F-%E9%A1%B9%E7%9B%AE%E7%BB%B4%E6%8A%A4%E5%B7%A5%E5%85%B7.user.js
// @updateURL https://update.greasyfork.org/scripts/454600/%E4%B8%93%E4%B8%9A%E5%8C%96%E5%9B%A2%E9%98%9F-%E9%A1%B9%E7%9B%AE%E7%BB%B4%E6%8A%A4%E5%B7%A5%E5%85%B7.meta.js
// ==/UserScript==
...
(function () {
    ...
    //#region 需求流程处理页面
    if (window_url.indexOf("https://oa.epoint.com.cn/productrelease/cpzt/demandmanage/demandbasicinfo_detail?") != -1 || window_url.indexOf("https://oa.epoint.com.cn/productrelease/cpzt/demandmanagezw/demandbasicinfo_detail?") != -1 ) {
        // 需求信息
        ...
        function loadBtn() {
            request.post(
                ...
                function (data) {
                    if (data.success) {
                        if (signBtn) {
                            console.log(signBtn.getEl());
                            // signBtn.getEl().remove();
                            signBtn.destroy();
                        }
                        if (planBtn) {
                            console.log(planBtn.getEl());
                            // planBtn.getEl().remove();
                            planBtn.destroy();
                        }
                        if (data.custom.hasSigned == "0") {
                            signBtn = createBtn("签收", function () {
                                loadDemandInfo();
                                var datagrid_projectgroup = mini.get('datagrid_projectgroup');
                                var itemdate = datagrid_projectgroup.getSelecteds();
                                if (itemdate.length == 0) {
				                  epoint.alert('请选择开发小组！');
				                  return;
			                    }
                                $.each(itemdate, function(index, node) {
				                var projectname = node.projectname;
				                var username = node.username;
				                var ouname = node.ouname;
				                var json = {};
				                json.projectname = projectname;
				                json.username = username;
				                json.ouname = ouname;
                                json.demandname = mini.get('demandname').getValue();
                                json.hopefinishdate = mini.get('hopefinishdate').getValue();
                                      window.open("http://218.4.136.126:8881/epoint-jxgl-web/jxgl/performancerequire/performancerequireadd?data="+epoint.encodeUtf8(JSON.stringify(json))+
                        ...
                        if (data.custom.hasPlaned == "1") {
                            planBtn = createFeedbackBtn('计划调整', 'sjap');
                            var balanceBtn = createFeedbackBtn('工作结算', 'gzjs');
                            planBtn.render(domToolBar);
                            balanceBtn.render(domToolBar);
                        }
