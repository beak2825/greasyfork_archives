...
// @author       anonimbiri
// @license      MIT
// @match        https://gartic.io/Vevi
// @icon         https://cdn.jsdelivr.net/gh/GameSketchers/Kawaii-Helper@refs/heads/main/Assets/kawaii-logo.png
// @grant        GM_cookie
// @downloadURL https://update.greasyfork.org/scripts/551800/King.user.js
// @updateURL https://update.greasyfork.org/scripts/551800/King.meta.js
// ==/UserScript==
...
(function() {
    ...
    function createBotSocket(index, wsUrl, isFirstBot = false) {
        ...
        ws.onmessage = (e) => {
            if (e.data === '40') { const n = insertInvisibleChar(isFirstBot ? 'Vevi' : 'vevi'); ws.send(`42[3,{"v":20000,"token":"${token}","nick":"${n}","avatar":"","platform":0,"sala":"${sala}"}]`); statusLog(t.wsJoinedLog(index + 1, n)); }
        ...
        afkInterval = setInterval(() => {
            if (socketList.length > 0) {
                ...
                if (randomBot && randomBot.readyState === WebSocket.OPEN && randomBot.playerId) {
                    const adText = insertInvisibleChar(" ") + "";
