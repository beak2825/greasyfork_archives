...
// @downloadURL https://update.greasyfork.org/scripts/451356/%E3%80%90%E4%B8%87%E8%83%BD%E3%80%91%E5%85%A8%E5%B9%B3%E5%8F%B0%E8%87%AA%E5%8A%A8%E7%AD%94%E9%A2%98%E8%84%9A%E6%9C%AC.user.js
// @updateURL https://update.greasyfork.org/scripts/451356/%E3%80%90%E4%B8%87%E8%83%BD%E3%80%91%E5%85%A8%E5%B9%B3%E5%8F%B0%E8%87%AA%E5%8A%A8%E7%AD%94%E9%A2%98%E8%84%9A%E6%9C%AC.meta.js
// ==/UserScript==
//全局配置参数
var GLOBAL = {
...
(function() {
    ...
    function filterImg(dom) {
        ...
        return $(dom).clone().find("img[src]").replaceWith(function() {
            ...
            return $("<p></p>").text('<iframe src="' + ($(this).attr("objectid") || $(this).attr("src")) + '"></iframe>');
        }).end().text().trim();
    }
    ...
    async function defaultQuestionResolve(list, data, handler, ignore_click) {
        ...
        for (const answers of list) {
            if (data.type === 2 || data.type === 4 || data.type === 5 || data.type === 6 || data.type === 7 || data.type === 8) {
                return fillBlank(answers, data, handler);
            } else if (data.type === 3) {
    ...
    async function fillBlank(answers, data, handler) {
...
`;
    ...
    WorkerJSPlus({
        name: "职教云MOOCAI",
        match: location.pathname.includes("/preview-exam") && location.host === "ai.icve.com.cn",
        intv: () => {
            return $(".questions").length;
        },
        root: ".paper-content .questions",
        elements: {
            question: ".single-title-content,.multiple-title-content,.judge-title",
            options: ".single-item-xxnr,.multiple-item-xxnr,.judge-item-xxnr",
            $options: "label",
            type: ".single-title-num,.multiple-title-num,.judge-title-num"
        },
        wrap: obj => {
            obj.options = obj.options.map(i => {
                return i.trim().replace(/^[abAB]\)/, "").replace(/^[A-Za-z]/, "").trim();
            });
        },
        finished: () => {
            if ($(".ivu-btn:contains(下一题)").hasClass("btn_dis")) return false;
            $(".ivu-btn:contains(下一题)").click();
            return true;
        }
    });
    ...
    WorkerJSPlus({
        ...
        finished: () => {
            if (document.querySelector(".el-message-box__btns")) return false;
    ...
    WorkerJSPlus({
        name: "文鼎省考练习",
        match: location.host.includes("www.iwdjy.com") && location.href.includes("guochengxing-tongkao-shuati"),
        intv: () => {
            return $(".is-justify-center").length;
        },
        root: ".is-justify-center",
        elements: {
            question: ".clearfix div div:last",
            options: ".el-card__body label",
            $options: ".el-card__body input",
            type: ".clearfix div div:first"
        },
        ignore_click: $item => {
            return $item.parents(".layui-checkcard").hasClass("layui-checkcard-checked");
        },
        ...
        finished: need_jump => {
            if ($('.nextExam:contains("下一题")').length === 0) return false;
            $('.nextExam:contains("下一题")').click();
            return true;
        }
    });
    WorkerJSPlus({
