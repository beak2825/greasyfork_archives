...
(function () {
  ...
  panel.innerHTML = `
    <div style="font-weight:700;margin-bottom:6px;">🎰 抽奖助手 v0.6</div>
    <div id="wof-last">上次：${lastResult}</div>
    <div id="wof-remaining">剩余：-</div>
    <div id="wof-total">总抽：${counts.lottery}</div>
    <div id="wof-prizes">一/二/三/四/五/六：${counts.w1}/${counts.w2}/${counts.w3}/${counts.w4}/${counts.w5}/${counts.w6}</div>
  ...
  const syncPanel = () => {
    ...
    el.total.textContent = `总抽：${counts.lottery}`;
    el.prizes.textContent = `一/二/三/四/五/六：${counts.w1}/${counts.w2}/${counts.w3}/${counts.w4}/${counts.w5}/${counts.w6}`;
    updateRemaining();
  ...
  window.alert = function (message) {
    counts.lottery += 1;
    if (message.startsWith('一等奖')) counts.w1 += 1;
    else if (message.startsWith('二等奖')) counts.w2 += 1;
    else if (message.startsWith('三等奖')) counts.w3 += 1;
    else if (message.startsWith('四等奖')) counts.w4 += 1;
    else if (message.startsWith('五等奖')) counts.w5 += 1;
    else if (message.startsWith('六等奖')) counts.w6 += 1;
