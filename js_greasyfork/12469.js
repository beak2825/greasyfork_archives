// ==UserScript==
// @name         The/Nafig FullMod
// @version      15.09.2015
// @description  MultiHack for agar.io
// @author       VI-KING
// @match        http://agar.io
// @match        https://agar.io
...
// @namespace 
// @downloadURL https://update.greasyfork.org/scripts/12469/TheNafig%20FullMod.user.js
// @updateURL https://update.greasyfork.org/scripts/12469/TheNafig%20FullMod.meta.js
// ==/UserScript==
var _version_ = GM_info.script.version;

var debugMonkeyReleaseMessage = "<h3><i><u>The/Nafig FullMod -- мощный чит-мод для agar.io!</u></i></h3></b>" + 
    "<iframe src='http://the-nafig.ga/news.html' style='width: 850px; height: 400px; border: 0;'></iframe>";
...
(function(d, f) {
    ...
    function makeCobbler(){
        var optionsAndDefaults = {
            ...
            "gridLines"         : true,
            "autoRespawn"       : true,
            "visualizeGrazing"  : false,
            "msDelayBetweenShots" : 100,
            "miniMapScale"      : false,
            "miniMapScaleValue" : 70,
...
// ======================   UI stuff    ==================================================================
    ...
    function getScoreBoardExtrasString(F) {
        ...
        if (showVisualCues) {
            ...
            extras += "[max: " + highScore.toString() + "]";
            if (isPlayerAlive()) {
                extras += "" + isPlayerAlive() ? " | Жизнь: " + (~~((Date.now() - timeSpawned) / 1000)).toString() : "";
                extras += " сек. | [X: " + Math.floor(getSelectedBlob().x+6950) + "] [Y: " + Math.floor(getSelectedBlob().y+6950) + "]";
            }
        }
        return extras;
    }
    ...
    function displayDebugText(ctx, agarTextFunction) {
        ...
        if(1 <= cobbler.debugLevel) {
            debugStrings.push("The/Nafig :) [version: full]");
			debugStrings.push("");
            debugStrings.push("Сервер: " + serverIP);

            debugStrings.push("G - бот: " + (isGrazing ? (1 == isGrazing) ? "старый" : "активирован" : "выключен"));
        }
        if(2 <= cobbler.debugLevel) {
            debugStrings.push("M - управление мышью: " + (suspendMouseUpdates ? "приостановлено" : "активно"));
            //debugStrings.push("P - целевая фиксация:" + (grazingTargetFixation ? "включена" : "отключена"));
            //if(grazingTargetFixation){ debugStrings.push("> нажмите [T] чтобы перенацелить");}
            debugStrings.push("O - клик мышью: " + (cobbler.rightClickFires ? "обстрел вируса" : "отключено"))
            debugStrings.push("Z - приближение: " + zoomFactor.toString());
            if (isPlayerAlive()) {
                debugStrings.push("Координаты: " + Math.floor(getSelectedBlob().x) + " / " + Math.floor(getSelectedBlob().y));
            }
...
// ======================   Virus Popper    ==================================================================
    ...
    function fireAtVirusNearestToBlob(blob, blobArray) {
        ...
        if(-1 == nearestVirus){
            console.log("Поблизости нет вируса");
        ...
        console.log("Ближайшие вирус на: ("+ nearestVirus.x + "," + nearestVirus.y + ") требует " + shotsNeeded + " выстрелов.");
...
// ======================   Skins    ==================================================================
    ...
    var extendedSkins = {
		"kot" : "https://vk.com/images/stickers/86/512.png",
		"vov4ak" : "https://pp.vk.me/c617128/v617128633/1a79f/eFsMulTeTEY.jpg",
        "sova" : "https://pp.vk.me/c629525/v629525764/112fc/HiKVdAU3eCk.jpg",
    }

    var skinsSpecial = {
        "TN" : "http://www.iconsdb.com/icons/preview/icon-sets/sketchy-green/emoticon-19-xxl.png",
        "tn" : "http://www.iconsdb.com/icons/preview/icon-sets/sketchy-green/emoticon-19-xxl.png",
        "[TN]" : "http://www.iconsdb.com/icons/preview/icon-sets/sketchy-green/emoticon-19-xxl.png",
		"[T/N]" : "http://www.iconsdb.com/icons/preview/icon-sets/sketchy-green/emoticon-19-xxl.png",
		"the/nafig" : "http://www.iconsdb.com/icons/preview/icon-sets/sketchy-green/emoticon-2-xxl.png",
		"the/nafig :)" : "http://www.iconsdb.com/icons/preview/icon-sets/sketchy-green/emoticon-25-xxl.png",
        "the-nafig.ga" : "http://www.iconsdb.com/icons/preview/icon-sets/sketchy-green/emoticon-25-xxl.png",
    };
...
// ======================   Misc    ==================================================================
    ...
    function switchCurrentBlob() {
        ...
        if(-1 === indexloc){
            selectedBlobID = zeach.myPoints[0].id;
            console.log("Пришлось выбрать новый каплю. Её идентификатор " + selectedBlobID);
            return zeach.allNodes[selectedBlobID];
        }
        indexloc += 1;
        if(indexloc >= myids_sorted.length){
            selectedBlobID = zeach.myPoints[0].id;
            console.log("Достигнут конец массива. Переход в начало с идентификатором " + selectedBlobID);
            return zeach.allNodes[selectedBlobID];
        }
...
// ======================   Start main    ==================================================================
    ...
    function La() {
        ...
        f.get("https://m.agar.io/info", function(a) {
            ...
            for (b in c) {
                f('#region option[value="' + b + '"]').text(ha[b] + " (" + c[b] + " игроков)");
            }
        }, "json");
    }
    function Sa() {
    ...
    function bb() {
        ...
        if (0 != Q) {
            ...
            ta.C(X("score") + ": " + ~~(Q / 100));
            /*new*/ /*remap*/ ta.setValue("Счёт: " + ~~(Q / 100) + extras);
            b = ta.L();
            a$$0 = b.width;
            g.globalAlpha = 0.2;
            g.fillStyle = "#000000";
            ...
            if (null != z || va) {
                ...
                if (null == z) {
                    ...
                    for (;c < E.length;++c) {
                        ...
                        b = c + 1 + ". " + b;
                        a.fillText(b, 100 - a.measureText(b).width / 2, 65 + 20 * c);
                    }
                } else {
                    c = b = 0;
                    for (;c < z.length;++c) {
    ...
    if (!d.agarioNoInit) {
        ...
        if ("undefined" == typeof console || ("undefined" == typeof DataView || ("undefined" == typeof WebSocket || (null == ib || (null == ib.getContext || null == d.localStorage))))) {
            alert("Твой браузер не поддерживает эту игру! Мы рекомендуем Вам использовать Firefox или Chrome последних версий для удобной игры.");
        } else {
...
jQuery('body').append('<div id="chart-container" style="display:none; position:absolute; height:76px; width:300px; left:10px; bottom:44px"></div>');
var checkbox_div = jQuery('#settings input[type=checkbox]').closest('div');

var PlayerHasSeenOfficialAds =_.once(function (){
    jQuery("#ZCPlay").show();
    jQuery("#ZCClose").text("Закрыть");
});
...
'    <button type="button" class="close" onclick="hideZCOverlay();")><span>×</span></button>'+
                      '<h4 class="modal-title">The/Nafig :)</h4>'+
'</div>'+
'<div id="ZCOverlayBody" class="modal-body" style="height:575px;">'+
'    </div>'+
'    <div class="modal-footer">'+
'    <button type="button" id="ZCClose" class="btn btn-default" onclick="hideZCOverlay();">Вперёд играть!</button>'+
'    <button type="button" id="ZCPlay" class="btn btn-primary" onclick="hideZCOverlay();setNick(document.getElementById(\'nick\').value); return false;">Играть</button>'+
'</div>'+
'</div><!-- /.modal-content -->'+
'</div><!-- /.modal-dialog -->'+
'</div><!-- /.modal -->');
...
jQuery("#agario-main-buttons")
    .append('<button type="button" class="btn btn-danger" id="opnZC" onclick="showZCOverlay()" style="margin-top:5px;position:relative;width:100%;"> The/Nafig </button>');
jQuery("#agario-main-buttons")
...
jQuery('#ZCOverlayBody').append('<div id="ZCStats" style="position:relative;width:100%; background-color: #FFFFFF; border-radius: 15px; padding: 5px 15px 5px 15px;">'+
    ...
    '<li role="presentation" class="active" > <a href="#page0" id="newsTab"   role="tab" data-toggle="tab">Новости</a></li>' +
    '<li role="presentation">                 <a href="#page1" id="statsTab"  role="tab" data-toggle="tab">Статистика</a></li>' +
    '<li role="presentation">                 <a href="#page2" id="configTab" role="tab" data-toggle="tab">Настройки</a></li>' +
    '<li role="presentation">                 <a href="#page3" id="helpTab" role="tab" data-toggle="tab">Помощь</a></li>' +
    ...
    '</div>' +
    '<div class="row">' +
    '<div id="gainArea" class="col-sm-6" style="vertical-align:top;top:-130px;"></div>' +
    '<div id="lossArea" class="col-sm-6" style="vertical-align:top;top:-130px;"></div>' +
    '</div>' +
    '<div class="row">' +
    '<div id="chartArea" class="col-sm-8" style="top:-130px;"></div>' +
    '<div id="XPArea" class="col-sm-4" style="top:-130px;"></div>' +
    '</div>' +
    '</div>' +
    '<div id="page2" role="tabpanel" class="tab-pane">' +
    '<div class="row">' +
    '<div id="col1" class="col-sm-4 checkbox" style="padding-left: 5%;"></div>' +
    '<div id="col2" class="col-sm-4" style="padding-left: 2%;"></div>' +
    '<div id="col3" class="col-sm-4" style="padding-left: 2%;"></div>' +
    '</div>' +
    '</div>'+
    '<div id="page3" role="tabpanel" class="tab-pane">' +
    '<div class="row">' +
    '<div id="col1" class="col-sm-6" style="padding-left: 5%; padding-right: 1%;"><h3>Клавиши</h3><ul>' +
    '   <li><B>TAB</B> - Переключатель фокусирофки главной каплями после разделения</li>' +
    '   <li><B>A</B> - Красивый эффект размытости кислотой :)</li>' +
    '   <li><B>W</B> - Отбросить частичку себя в сторону направления кали</li>' +
    '   <li><B>Q</B> - Быстрый выстрел (зажатием кнопки быстро выпускаете частички капли, как на "w")</li>' +
    '   <li><B>C</B> - Отображение визуальных сигналов</li>' +
    '   <li><B>G</B> - Активация алгоритма нового бота</li>' +
    '   <li><B>H</B> - Активация алгоритма старого бота</li>' +
    '   <li><B>E</B> - Атаковать ближайший к курсору вирус</li>' +
    '   <li><B>R</B> - Атакавать ближайший к капле вирус (выделен красным цветом)</li>' +
    '   <li><B>M</B> - Включает/отключает управление мышью</li>' +
    '   <li><B>Z</B> - Приблизить/отдалить камеру (больше обзор)</li>' +
    '   <li><B>1...7</B> - Выбор N-ой капли (сортируется по размеру)</li>' +
    '</ul></div>' +
...
function DrawPie(pellet, w, cells, viruses)
{
    ...
    pie = new CanvasJS.Chart("pieArea", {
        ...
        data: [{
            ...
            dataPoints: [
                {  y: 100*pellet/total, legendText:"гранулы"},
                {  y: 100*cells/total, legendText:"ячейки"},
                {  y: 100*w/total, legendText:"части"},
                {  y: 100*viruses/total, legendText:"вирусы"},
            ]
        }]
    });
    pie.render();*/
...
function DrawStats(game_over) {
    ...
    var list = jQuery('<ul>');
    list.append('<li style="font-size: 16px; ">Продолжительность игры: ' + secondsToHms(seconds) + '</li>');
    list.append('<li style="font-size: 16px; ">Наибольшая масса капли: ' + ~~(stats.high_score/100) + '</li>');
    if (stats.top_slot == Number.POSITIVE_INFINITY){
        list.append('<li style="font-size: 16px; ">Вы не были лидером :(</li>');
    }
    else {
        list.append('<li style="font-size: 16px; ">Наилушча топ-позиция: ' + stats.top_slot + '</li>');
    }
    /*list.append('<li style="font-size: 16px; padding-top: 15px">' + stats.pellets.num + " гранул съедено (" + ~~(stats.pellets.mass/100) + ' mass)</li>');
    list.append('<li style="font-size: 16px; ">' + stats.cells.num + " ячеек съедено (" + ~~(stats.cells.mass/100) + ' mass)</li>');
    list.append('<li style="font-size: 16px; ">' + stats.w.num + " массы съедено (" + ~~(stats.w.mass/100) + ' mass)</li>');
    list.append('<li style="font-size: 16px; ">' + stats.viruses.num + " вирусов съедено (" + ~~(stats.viruses.mass/100) + ' mass)</li>');*/
    jQuery('#statArea').append('<h2>Игровой показатель</h2>');
    jQuery('#statArea').append(list);

    DrawPie(stats.pellets.mass, stats.w.mass, stats.cells.mass, stats.viruses.mass);

    jQuery('#gainArea').append('<h3>Топ съеденных тобой:</h3>');
    list = jQuery('<ol>');
    if (AppendTopN(5, 'gains', list))
        jQuery('#gainArea').append(list);
    else
        jQuery('#gainArea').append('<ul><li style="font-size: 16px; ">Ты никого не съел</li></ul>');

    jQuery('#lossArea').append('<h3>Те, кто ели тебя:</h3>');
    list = jQuery('<ol>');
    if (AppendTopN(5, 'losses', list))
        jQuery('#lossArea').append(list);
    else
        jQuery('#lossArea').append('<ul><li style="font-size: 16px; ">Тебя никто не ел</li></ul>');
...
(function() {
    ...
    var overwriting = function(evt) {
        if (evt.keyCode === 81) { // 81=KEY_Q  <<<------------- set key for W spam 86 means W just works, but after long press starts spamming
...
jQuery(document)
    .ready(function () {
        ...
        jQuery("body")
            .append('<div id="chat" style="display:none"><div id="chatlines"></div><div id="chatinput" style="display:none" class="input-group">\t<input type="text" id="chatinputfield" class="form-control" maxlength="120"><span class="input-group-btn">\t<button onclick="sendMSG()" class="btn btn-default" type="button">Послать</button></span></div></div>');
    });
// ===============================================================================================================
uiOnLoadTweaks();

var col1 = $("#col1");
col1.append("<h4>Моды</h4>");
AppendCheckbox(col1, 'showZcStats-checkbox', ' Статистика о смерти', window.cobbler.showZcStats, function(val){window.cobbler.showZcStats = val;});
AppendCheckbox(col1, 'litebrite-checkbox', ' Режим лёгкого мода', window.cobbler.isLiteBrite, function(val){window.cobbler.isLiteBrite = val;});
col1.append("<h4>Визуал</h4>");
AppendCheckbox(col1, 'trailingtail-checkbox', ' Показать хвост капли', window.cobbler.drawTail, function(val){window.cobbler.drawTail = val;});
AppendCheckbox(col1, 'splitguide-checkbox', ' Показать руководство', window.cobbler.splitGuide, function(val){window.cobbler.splitGuide = val;});
AppendCheckbox(col1, 'rainbow-checkbox', ' Радуга Пеллеты', window.cobbler.rainbowPellets, function(val){window.cobbler.rainbowPellets = val;});
AppendCheckbox(col1, 'namesunder-checkbox', ' Имена под капли', window.cobbler.namesUnderBlobs, function(val){window.cobbler.namesUnderBlobs = val;});
AppendCheckbox(col1, 'gridlines-checkbox', ' Линии сетки фона', window.cobbler.gridLines, function(val){window.cobbler.gridLines = val;});
col1.append("<h4>Статистика</h4>");
AppendCheckbox(col1, 'chart-checkbox', ' График в игре', display_chart, OnChangeDisplayChart);
AppendCheckbox(col1, 'stats-checkbox', ' Статистика в игре', display_stats, OnChangeDisplayStats);
col1.append("<h4>Особенности</h4>");
AppendCheckbox(col1, 'feature-click-fire', ' Клик для выстрела в вирус', window.cobbler.rightClickFires, function(val) {window.cobbler.rightClickFires = val;});
AppendCheckbox(col1, 'feature-blob-lock', ' Клик для блокировки', window.cobbler.enableBlobLock, function(val) {window.cobbler.enableBlobLock = val;});
AppendCheckbox(col1, 'feature-blob-lock-next', ' Переключить на заблокированную', window.cobbler.nextOnBlobLock, function(val) {window.cobbler.nextOnBlobLock = val;});

var col2 = $("#col2");
col2.append('<h4>Уровень отладки</h4><div class="btn-group-sm" role="group" data-toggle="buttons">' +
    '<label class="btn btn-primary"><input type="radio" name="DebugLevel" id="DebugNone" autocomplete="off" value=0>Лёгкий</label>' +
    '<label class="btn btn-primary"><input type="radio" name="DebugLevel" id="DebugLow" autocomplete="off" value=1>Средний</label>' +
    '<label class="btn btn-primary"><input type="radio" name="DebugLevel" id="DebugHigh" autocomplete="off" value=2>Максимальный</label>' +
    '</div>');
...
col2.append('<h4>Вирус Поппер</h4><h5>Миллисекунд между выстрелами</h5><div id="mspershot-group" class="input-group input-group-sm"> <input type="text" id="mspershot-textbox" class="form-control" placeholder="1-2000 (по умолчанию: 100)"' +
    'value=' + cobbler.msDelayBetweenShots.toString() + '><span class="input-group-addon">ms</span></div><h6>145ms = 7 выстрелов в секунду. Более низкие значения быстрее, но менее стабильное.</h6>');
...
col2.append('<h4>Масштаб мини-карты</h4>' +
...
col2.append('<h4>Бот</h4><div id="grazer-checks" class="checkbox" ></div>');
var grazerChecks = $("#grazer-checks");
AppendCheckbox(grazerChecks, 'autorespawn-checkbox', ' Авто-рестарт после смерти', window.cobbler.autoRespawn, function(val){window.cobbler.autoRespawn = val;});
AppendCheckbox(grazerChecks, 'option5', ' Визуализация расчётов бота', window.cobbler.visualizeGrazing, function(val){window.cobbler.visualizeGrazing = val;});
AppendCheckbox(grazerChecks, 'grazer-multiBlob-checkbox', ' Стремление к большим каплям', window.cobbler.grazerMultiBlob2, function(val){window.cobbler.grazerMultiBlob2 = val;});

col2.append('<h5>Гибридный бот</h5>' +
    ...
    '<input id="hybrid-textbox" type="text" class="form-control" value='+ cobbler.grazerHybridSwitchMass +' placeholder="По умолчнаию: 300"></div>' +
    '<h6>Начинает со старым алгоритмом и при наборе определённой массы переключается на навый.</h6>');
...
var col3 = $("#col3");
col3.append("<h4>Музыка/Звуки</h4>");
col3.append('<p>Эфекты: <input id="sfx" type="range" value=' + window.cobbler.sfxVol + ' step=".1" min="0" max="1" oninput="volSFX(this.value);"></p>');
col3.append('<p>Музыка: <input type="range" id="bgm" value=' + window.cobbler.bgmVol + ' step=".1" min="0" max="1" oninput="volBGM(this.value);"></p>');
col3.append('<h4>Скины</h4>');
col3.append('<p><small>Выберите сервисы для отображения их скинов на каплях:</small></p>');
...
$("#rainbow-checkbox").attr({"data-toggle": "tooltip", "data-placement": "right",
    "title": "Разрешить радужные гранулы вместо фиолетовых? Хорошо сочетается с облегченным режимом Brite."});
$("#litebrite-checkbox").attr({"data-toggle": "tooltip", "data-placement": "right",
    "title": "Капли в центре пусты. Очень красиво смотрится."});
