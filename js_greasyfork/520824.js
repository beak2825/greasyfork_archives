// ==UserScript==
// @name Download YouTube Videos as MP4 (use yt-dlp)
...
// @downloadURL https://update.greasyfork.org/scripts/520824/Download%20YouTube%20Videos%20as%20MP4%20%28use%20yt-dlp%29.user.js
// @updateURL https://update.greasyfork.org/scripts/520824/Download%20YouTube%20Videos%20as%20MP4%20%28use%20yt-dlp%29.meta.js
// ==/UserScript==
...
(function () {
  ...
  async function run(isMaterialDesign) {
    ...
    let isLive = false;
    var videoID=null, videoFormats=null, videoAdaptFormats=null, videoManifestURL=null, paygatedFormats=null;
    ...
    if (ytplayer && ytplayer.config && ytplayer.config.args) {
      ...
      if(args.raw_player_response && args.raw_player_response.microformat && args.raw_player_response.microformat.playerMicroformatRenderer) {
        ...
        if(pmr.liveBroadcastDetails && pmr.liveBroadcastDetails.startTimestamp) {
          updatetime = pmr.liveBroadcastDetails.startTimestamp;
        } else if(pmr.publishDate) {
          updatetime = pmr.publishDate;
        } else if(pmr.uploadDate) {
          updatetime = pmr.uploadDate;
        }
        const dateObject = new Date(updatetime);
        updatetime = dateObject.toLocaleDateString('ko-KR', {year: 'numeric', month: '2-digit', day: '2-digit'}).replace(/\D/g, '');
  ...
  if (videoFormats.indexOf(',')>-1||videoFormats.indexOf('&')>-1||videoFormats.indexOf('\\u0026')>-1) {
    ...
    for(let format of videoFormats) {
      if(format.mimeType.match(/^([^;]+);\s*codecs="([^\."]+)/)) {
        switch (RegExp.$1) {
          case "video/mp4":
            format.qualityLabel = format.qualityLabel.replace(/\d+([a-z]+)(?:\d+)?/, `${format.itag}: ${format.height}$1${format.fps}`);
            if (format.audioQuality !== undefined) format.ext = "mp4";
            else format.ext = "mp4v", format.qualityLabel += ` (${fmtBitrate(format.bitrate)})`;
            //format.qualityLabel += ` (${fmtFileSize(format.contentLength)})`;
            if (-1 != format.qualityLabel.toLowerCase().indexOf('premium')) {
              format.premium = true;
              format.qualityLabel += ` (${fmtFileSize(format.contentLength)})`;
            }
            format.cmd = `av@_@${format.itag}@_@m4a@_@`;
            break;
          case "video/webm":
            format.qualityLabel = format.qualityLabel.replace(/\d+([a-z]+)(?:\d+)?/, `${format.itag}: ${format.height}$1${format.fps}`);
            if (format.audioQuality !== undefined) format.ext = "webm";
            else format.ext = "webmv", format.qualityLabel += ` (${fmtBitrate(format.bitrate)})`;
            //format.qualityLabel += ` (${fmtFileSize(format.contentLength)})`;
            if (-1 != format.qualityLabel.toLowerCase().indexOf('premium')) {
              format.premium = true;
              format.qualityLabel += ` (${fmtFileSize(format.contentLength)})`;
            }
            format.cmd = `av@_@${format.itag}@_@${isLive ? 'm4a' : 'webm'}@_@`;
            break;
          case "audio/mp4":
            format.ext = "m4a";
            format.qualityLabel = `${format.itag}: ${fmtBitrate(format.bitrate)} - \u97f3\u58f0\u306e\u307f`;
            format.cmd = `oa@_@0@_@m4a@_@`;
            break;
          case "audio/webm":
            format.ext = RegExp.$2;
            format.qualityLabel = `${format.itag}: ${fmtBitrate(format.bitrate)} - \u97f3\u58f0\u306e\u307f`;
            format.cmd = `oa@_@0@_@webm@_@`;
            break;
          default:
            format.ext = RegExp.$2;
    ...
    for (var i=0;i<videoFormatsGroup.length;i++) {
      ...
      // 더이상 직접 추출이 불가능하므로 URL 체크 제거(프리미엄 영상인지만 체크)
      let itag = videoFormatsPair['itag'];
      if (videoFormatsPair.premium) {
        videoURL[itag] = '';
      } else {
        videoURL[itag] = `?clen=${videoFormatsPair.contentLength}&`;
      }
      if(videoFormatsPair.height === undefined || videoFormatsPair.height >= 300) {
        FORMAT_ORDER.push(String(itag));
        FORMAT_LABEL[itag] = `${videoFormatsPair['ext'].toUpperCase()} ${videoFormatsPair['qualityLabel']}${isLive ? " - Live" : ""}`;
        FORMAT_TYPE[itag] = videoFormatsPair['ext'];
        FORMAT_CMD[itag] = videoFormatsPair['cmd'];
      }
    }
    ...
    for (let i=0;i<downloadCodeList.length;i++) {
      ...
      listLink.setAttribute('href', `ytdlp:${FORMAT_CMD[downloadCodeList[i].format]}${window.location.href}`);
