// ==UserScript==
// @name         Eka Tag Hover
// @namespace    com.aryion.underside.hungry-vixen-userscripts
// @description  Hover over items in your message page watch lists on Eka's Portal to see tags.
// @usage        Hover over items in your watch lists to see tags.
// @author       Underside
// @include     *aryion.com/g4/*
// @match        *aryion.com/g4/messagepage.php
// @version     1.0
// @icon        data:image/png;base64,

// @downloadURL https://update.greasyfork.org/scripts/402297/Eka%20Tag%20Hover.user.js
// @updateURL https://update.greasyfork.org/scripts/402297/Eka%20Tag%20Hover.meta.js
// ==/UserScript==
var EkaTagAltDispTool={
    //Keeping a small footprint with encapsulation.
    _listGalleryItems:function(){
        var _list=[];
        var _pglist=document.getElementsByTagName('li');
        //Get all the <li> tags
        for(var x=0,ln=_pglist.length;x<ln;x++){
            if(_pglist[x].className=="gallery-item ui-selectee"){
                //but just the ones we want get added to the list.
                _list.push(_pglist[x]);
            }
        }
        return _list;
    }
    ,
    _updateAltText:function(gallery){
        for(var x=0,ln=gallery.length;x<ln;x++){
            //Make a string of the data-tagnames pseudo-element.
            var tag_str = gallery[x].outerHTML.split("data-tagnames=")[1].split("\"")[1];
            //Find the thumbnail image
            var gal_item = gallery[x].childNodes
            var x_thumb = gal_item[3].childNodes[1].childNodes[0];
            //Fill the title and alt text of the thumbnail with the tag string.
            x_thumb.title = tag_str;
            x_thumb.alt = tag_str;
        }
    }
    ,
    tag_display:function(){
        //A facade function to facilitate adding in alt text for mouseover.
        EkaTagAltDispTool._updateAltText(EkaTagAltDispTool._listGalleryItems());
    }
};
EkaTagAltDispTool.tag_display();
