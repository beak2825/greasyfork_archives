// ==UserScript==
// @name         新疆医学教育-课程助手
// @namespace    http://tampermonkey.net/zzzzzzys_新疆医学教育
// @version      1.0.0
// @copyright    zzzzzzys.All Rights Reserved.
// @description  新疆医学教育-可快速学习视频，也可自动考试！
// @author       zzzzzzys
// @match        https://www.xjyxjyw.com/member/pt_play4.do*
// @match        https://www.xjyxjyw.com/member/leyiexam_quelist.do*
// @match        https://www.xjyxjyw.com/member/hsp_info.do*
// @match        https://www.xjyxjyw.com/member/courseWare_play.do*
// @match        https://www.xjyxjyw.com/member/exam_quelist.do*
...
// @license
// @downloadURL https://update.greasyfork.org/scripts/554716/%E6%96%B0%E7%96%86%E5%8C%BB%E5%AD%A6%E6%95%99%E8%82%B2-%E8%AF%BE%E7%A8%8B%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/554716/%E6%96%B0%E7%96%86%E5%8C%BB%E5%AD%A6%E6%95%99%E8%82%B2-%E8%AF%BE%E7%A8%8B%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
class Runner {
    constructor() {
        this.runner = null
    ...
    run() {
        ...
        if (url.includes("pt_play4") || url.includes("hsp_info") || url.includes("courseWare_play")) {
            this.runner = new Course("channel-xjyxjyw")
            // this.runner.run()
        } else if (url.includes("exam_quelist")) {
            this.runner = new Exam("channel-xjyxjyw")
        }
    }
}
...
class Exam {
    ...
    async runVIP() {
        try {
            ...
            // const courseId=new URLSearchParams(location.search).get('leyiQuestion.courseId')
            const courseId=new URLSearchParams(location.search).get('ncmeQuestion.courseId')
            const coursewareId=new URLSearchParams(location.search).get('ncmeQuestion.coursewareId')
            ...
            if(!ans){
                ...
                // let questionList =this.getDomQuesList()
                let questionList =this.getDomQuesListFromInput()
                const rightList=await this.getRightOption(questionList)
                this.rightList = rightList
                // GM_setValue(db_key,rightList)
                // return
            }
            // const ansList=GM_getValue(db_key,[])
            const ansList=this.rightList
            Swal.fire({
                title: "自动答题中，请等待...",
                text: "已开始自动答题...",
                icon: 'info',
            ...
            for (let i = 0; i < ansList.length; i++) {
                const id=ansList[i].id
                const ans=ansList[i].rightAnswer
                const input=document.querySelector(`#${id}[value="${ans}"]`)
                if(input){
                    input.click()
                }
                await sleep(200)
    ...
    getDomQuesListFromInput(){
       const all_input = document.querySelectorAll('.wen input');
       const QuesList = []
       for(let i=0;i<all_input.length;i++){
           const input = all_input[i];
           const id = input.id;
           const value=input.value;
           const hasId = QuesList.find(x=>x.id===id);
           if(!hasId){
               QuesList.push({
                   id: id,
                   question:input.parentElement.parentElement.previousElementSibling.previousElementSibling.textContent.replace(/\s+/g, '').replace(' ','').trim(),
                   options:[value]
               })
               continue
           }
           const option= hasId.options || []
    ...
    getDomQuesList(){
        // 获取题目容器
        const wenContainer = document.querySelector('.wen');

        // 遍历所有子元素进行结构化处理
        const children = Array.from(wenContainer.children);
        const questions = [];
        let currentQuestion = null;

        children.forEach(child => {
            if (child.classList.contains('h3')) { // 题目标题
                currentQuestion = {
                    id: null,
                    question: child.textContent.replace(/\s+/g, '').replace(' ','').trim(),
                    options: {}
                };
                questions.push(currentQuestion);
            } else if (child.tagName === "p" && child.querySelector('input') && currentQuestion) { // 选项
                const input = child.querySelector('input[type="radio"]');
                if (!input) return;
                console.log(input)
                // 获取题目ID（每个选项共用相同ID）
                if (!currentQuestion.id) {
                    currentQuestion.id = input.id;
                }
                ...
                currentQuestion.options[option.value] = option.text;
            }
        });
        console.log(questions)
        return questions
    }
    async getRightOption(questions){
        const optionList = ['A', 'B', 'C', 'D', 'E']; // 固定遍历所有可能选项

        for (let i = 0; i < optionList.length; i++) { // 遍历全部选项(A-E)
            const currentOption = optionList[i];

            // 仅筛选包含当前选项的题目生成请求列表
            const dataList = questions
                // .filter(q => q.options && currentOption in q.options)
                .map(q => {
                    if(q.options && q.options.includes(currentOption)){
                        return{
                            name: q.id,
                            value: currentOption // 直接使用当前遍历的选项值
                        }
                    }else{
                        return {
                            name: q.id,
                            value: "D" // 直接使用当前遍历的选项值
                        }
                    }
                });

            if (dataList.length === 0) {
                console.log(`选项${currentOption}无对应题目，跳过`);
                continue; // 无题目需要此选项则跳过
            }

            console.log("正在测试选项:", currentOption, "dataList:", dataList);
            const results = await this.resolveExam(this.courseId,this.coursewareId, dataList);

            // 更新答案时校验选项有效性
            const updatedQuestions = questions.map(original => {
                if (!original || !original.options || !(original.options.includes(currentOption)))
                    return original;

                const result = results.find(r =>
                    r.question?.trim() === original.question?.trim()
                );

                if (result?.answer === '正确' && !original.rightAnswer) {
                    console.log(`发现答案：${original.question} -> ${currentOption}`);
                    return {
                        ...original,
                        rightAnswer: currentOption,
                        rightAnswerStr: original.options[currentOption]
                    };
                }
                return original;
            });
            ...
            // 全部找到则提前退出
            if (questions.every(q => q.rightAnswer)) break;
            await new Promise(resolve => setTimeout(resolve, 800));
        }
        // 校验结果
        const unsolved = questions.filter(q => !q.rightAnswer);
        if (unsolved.length > 0) {
            console.warn('未解决的题目:', unsolved.map(q => q.question));
        } else {
            console.log('成功解析所有答案:', questions);
        }

        return questions;

    }
    async resolveExam(courseId,coursewareId,ans_data){
        // const data={
        //     backurl:"",
        //     "leyiQuestion.data":JSON.stringify(ans_data),
        //     "leyiQuestion.courseId":courseId
        // }
        const data={
            flag:"",
            "ncmeQuestion.data":JSON.stringify(ans_data),
            "ncmeQuestion.courseId":courseId,
            "ncmeQuestion.coursewareId":coursewareId
        }
        let res=await fetch("https://www.xjyxjyw.com/member/exam_result.do", {
            "headers": {
            ...
            "referrer": "https://www.xjyxjyw.com/member/leyiexam_quelist.do?leyiQuestion.courseId="+courseId,
        ...
        if(res.ok){
            ...
            const questionElements = doc.querySelectorAll('.wen p.h4');
            ...
            questionElements.forEach(element => {
                ...
                // 分割题目和答案
                const parts = text.split('您的回答：');

                if (parts.length === 2) {
                    const question = parts[0].trim().replace(/^(\d+)\s*、/, '$1、'); // 标准化题号格式
                    const answer = parts[1].trim();

                    result.push({
                        question: question,
                        answer: answer === '正确' ? '正确' : '错误' // 标准化答案
                    });
                }
            });
            console.log(result);
...
class Utils {
    ...
    static flag = 'xjyxjyw_VIP'
    static js_Flag = 'xjyxjyw_jsCode'
    static vipText = '高级功能已启用！(新疆医学教育网)'
    static baseText = '您正在使用基础版本，功能可能存在限制'
    static vipSign = 'xjyxjyw_vipSign'
    ...
    static async validateCode(data) {
        try {
            console.log(data);
            let value = document.querySelector('#login111 li').innerText
            if(!value){
                throw Error("无效的账号信息！")
            }
            data.bindInfo = value
            data.website = "681dc71c9755e3437a1d7ca4"
            console.log(data);
            // return
