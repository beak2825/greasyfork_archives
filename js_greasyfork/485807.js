...
// @version      1.1.18
// @author       Zone
// @description  为 JavDB、JavBus、JavLibrary、avhd101、cili 这三个站点添加跳转在线观看的链接
// @license      MIT
// @icon         https://javdb.com/favicon-32x32.png
// @include      /^https?:\/\/(\w*\.)?javdb(\d)*\.com.*$/
// @include      /^https?:\/\/(\w*\.)?(javbus|seejav|javsee)*\.(com|cc|me|life|bid).*$/
// @include      /^http.*\/cn\/\?v=jav.*$/
// @include      https://avhd101.com/watch?v=*
// @include      /.*cili\.[a-z]+\/search\?q=.*$/
// @include      /https://javgg.net/jav/.*$/
...
// @downloadURL https://update.greasyfork.org/scripts/485807/%E8%87%AA%E7%94%A8%E6%96%87%E6%A1%A3.user.js
// @updateURL https://update.greasyfork.org/scripts/485807/%E8%87%AA%E7%94%A8%E6%96%87%E6%A1%A3.meta.js
// ==/UserScript==
...
(function (preact) {
  ...
  const libSites = [
    ...
    },
    {
      ...
      name: "avhd101",
      enable: true,
      href: /^https?:\/\/(\w*\.)?avhd101(\d)*\.com.*$/,
      querys: {
        panelQueryStr: ".play-thumb",
        codeQueryStr: `#flike > div:nth-child(3) > ul > li:nth-child(4) > h3 > a`,
      },
      method() {
          const panel = document.querySelector(".video-plan");
          panel.classList.add("lib-panel");
      ...
      name: "clili",
      enable: true,
      href: /.*cili\.[a-z]+\/search\?q=.*$/,
      querys: {
        panelQueryStr: ".header",
        codeQueryStr: `.form-control`,
      },
      method() {
      ...
      href: /.*javgg\.[a-z]+\/jav\/.*$/,
      querys: {
        panelQueryStr: ".dooplay_player",
        codeQueryStr: `meta[property="og:image:alt"]`,
      },
      method() {
          const panel = document.querySelector(".dooplay_player");
          panel.classList.add("lib-panel");
  ...
  const siteList = [
    ...
    {
      name: "1cili",
      disableLibItemName: "1cili",
      hostname: "1cili.com",
      url: "https://1cili.com/search?q={{code}}",
      fetchType: "get",
      domQuery: {
        linkQuery: "body > div > table",
        titleQuery: "body > div > table",
      },
    },
  ];
  const SP_PREFIX = "300";
...
console.info('codeNode'+codeNode);
    if (!codeNode) return "";
    const codeText =
      libItem.name === "javdb"
    ...
    const cliliTexts =libItem.name==='clili'?codeNode.value: codeText;
      if(cliliTexts.startsWith("FC2")) return cliliTexts.split("-")[0]+'-PPV-'+cliliTexts.split("-").pop();
    const javggTexts =libItem.name==='javgg'?codeNode.content: cliliTexts;
    return javggTexts;
  };
  const gmGet = ({ url }) => {
