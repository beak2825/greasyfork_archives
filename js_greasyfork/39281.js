// ==UserScript==
// @name        iks: virtonomica оптовое строительство V2
// @namespace   virtonomica
// @description Автоматический запуск постройки нескольких подразделений одного типа
// @include     http*://*virtonomic*.*/*/main/unit/create/*
// @include     http*://*virtonomic*.*/*/main/unit/view/*
// @include     http*://*virtonomic*.*/*/main/company/view/*/*
// @version     10.49
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/39281/iks%3A%20virtonomica%20%D0%BE%D0%BF%D1%82%D0%BE%D0%B2%D0%BE%D0%B5%20%D1%81%D1%82%D1%80%D0%BE%D0%B8%D1%82%D0%B5%D0%BB%D1%8C%D1%81%D1%82%D0%B2%D0%BE%20V2.user.js
// @updateURL https://update.greasyfork.org/scripts/39281/iks%3A%20virtonomica%20%D0%BE%D0%BF%D1%82%D0%BE%D0%B2%D0%BE%D0%B5%20%D1%81%D1%82%D1%80%D0%BE%D0%B8%D1%82%D0%B5%D0%BB%D1%8C%D1%81%D1%82%D0%B2%D0%BE%20V2.meta.js
// ==/UserScript==
function test() {
  var data = [
  ];
  data.push({
    'unitCreateData[unit_class]': '2043',
  ...
  if (cooki) {
    ...
    if (n > 0) {
        ...
        if (o[$(this).attr('name')]) {
          if ($(this).val() == o[$(this).attr('name')]) {
            ...
            $.when(this).then(function (id) {
              ...
              if (timeUpClik > 0) setTimeout(function () {
                $('input.button250[name="next"]').click();
              }, Math.floor(Math.random() * (timeUpClik - (timeUpClik / 10) + 1)) + (timeUpClik / 10));
      ...
      $('input.button250[name!="next"]').each(function () {
        ...
        $('[name="unitCreateData[custom_name]"]').val(o['unitCreateData[custom_name]']);
    ...
    var nameOfUnitsTitle = (getLocale() == 'en') ? 'Unit\'s name' : 'Название подразделений';
    ...
    $('input.button250[name!="next"]').each(function () {
      if (o['unitCreateData[unit_class]'] != '1815') {
        ...
        $('#add_addition').bind('change keyup input click', function () {
          ...
          + nameOfUnitsTitle + '<input type="text" value="" id="customname_' + addition_num + '" style="width: 100%"></input><br>'
    ...
    $('input.button250').click(function () {
      if ($(this).attr('name') == 'next') {
        ...
        //        var custom_name = $('[name="unitCreateData[custom_name]"]').val();
        $('select[id^="choose_town"]').each(function () {
          var sel = $(this);
          var suf = sel.attr('id').substr('choose_town'.length);
          var impNumUnitAdd = parseInt($('#impNumUnit' + suf).val());
          var custom_name = $('#customname' + suf).val().split(';');
          var zi = 0;
          for (zi = 0; zi < custom_name.length; zi++) {
            if (sel.val() == '') {
              // выбран весь регион/страна
              $('> option', sel).each(function () {
                ...
                if (option.attr('value') != '') {
                  var tmp = {
                    ...
                    'unitCreateData[custom_name]': custom_name[zi]
                  };
