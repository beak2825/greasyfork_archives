...
// @run-at       document-end
// @license      MIT License
// @downloadURL https://update.greasyfork.org/scripts/39579/%E5%8F%BD%E5%8F%BD%E5%8F%BD.user.js
// @updateURL https://update.greasyfork.org/scripts/39579/%E5%8F%BD%E5%8F%BD%E5%8F%BD.meta.js
// ==/UserScript==
                    ...
                    if (v.status === 1) { // 可以参加
                        API.SmallTV.join(room_id, v.raffleId).done(function(response) {
                            ...
                            if (response.code === 0) {
                                ...
                                toast('正在吸【' + room_id + '】的小电视', 'success');
                            }
                        });
                    ...
                    if (v.status === 1) { // 可以参加
                        API.Raffle.join(room_id, v.raffleId).done(function(response) {
                            ...
                            if (response.code === 0) {
                                ...
                                toast('正在吸【' + room_id + '】的辣鸡活动', 'success');
            ...
            if (response.code === 0) {
                if (response.data.status === 1) {
                    ...
                    if (response.data.gift_id === '-1' && !response.data.gift_name) {
                        toast('【' + room_id + '】的小电视吸到了：' + response.msg, 'info');
                    } else {
                        toast('【' + room_id + '】的小电视吸到了：' + response.data.gift_name + '*' + response.data.gift_num, 'info');
                    }
                    ...
                    toast('【' + room_id + '】的小电视吸到了：' + response.msg, 'error');
                }
            } else {
                // 其他情况
                setTimeout(function() {
        ...
        API.Raffle.notice(room_id, raffleId).done(function(response) {
            ...
            if (response.code === 0) {
                if (response.data.gift_id === '-1') {
                    toast('【' + room_id + '】的辣鸡活动吸到了：' + response.msg, 'info');
                } else {
                    toast('【' + room_id + '】的辣鸡活动吸到了：' + response.data.gift_name + '*' + response.data.gift_num, 'info');
                }
            ...
            if (response.code === 0) {
                DOM.storm.image[0].onload = function() {
                    ...
                    API.lottery.join(id, response.data.token, phrase, Info.csrf_token).done(function(response) {
                        ...
                        if (response.code === 0) {
                            toast('节奏风暴吸到了：' + response.data.gift_name + '*' + response.data.gift_num, 'info');
                        } else {
                            setTimeout(function() {
                                StormJoin(id, cnt + 1);
                            }, 1e3);
            ...
            DOM.treasure.image[0].onload = function() {
                ...
                if (captcha) {
                    ...
                    API.SilverBox.getAward(ts_s(), ts_s(), captcha).done(function(response) {
                        ...
                        if (response.code === 0) {
                            // 领取瓜子成功
                            toast('吃瓜成果： ' + response.data.awardSilver + ' 银瓜子', 'success');
                            callback();
                        } else if (response.code === -903) {
                            // -903: 已经领取过这个宝箱
                            toast('吃瓜成果：这个瓜已经吃完了', 'caution');
                            callback();
        ...
        try {
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    toast('每日扭蛋币：<span style="padding-left: 10px;">' + response.msg, 'info');
                }
            });
            API.sign.GetSignInfo().done(function(response) {
                ...
                if (response.code === 0) {
                    if (response.data.status === 0) {
                        ...
                        // 已签到
                        toast('今日已签到：' + response.data.text, 'info');
                    }
                }
            });
