...
// @downloadURL https://update.greasyfork.org/scripts/375046/Test%20DDJ.user.js
// @updateURL https://update.greasyfork.org/scripts/375046/Test%20DDJ.meta.js
// ==/UserScript==
if(!jQuery._FN){jQuery._FN={};}
...
  ordNb:(Math.floor(Math.random()*1000+1)),
  hideField:['div.PlayerToolbar-Task','div.LuckGiftEnter','div.TurntableLottery'],
  qScore:-1
};
...
    '<input type="checkbox" id="tm_check_rf" checked>刷新</div>'+
    '<div style="margin:1px;">&nbsp;'+
...
        if(!!s&&s>=2&&_VR.qScore<t){// 抢分查询
          _VR.reNum=0;_VR.qLock=false;_VR.qScore=t;
        }
        if(t!=_VR.score){
...
  },
  refresh:function(d){
  if($('#tm_check_rf').is(':checked')){
     d=new Date();
    var h=d.getHours(),m=d.getMinutes();
    if(!_VR.isOnAir&&45==m&&(6==h||11==h||18==h)){location.reload();return;}
    var $seWin=$('.account-security');
    if($seWin.length>0&&!$seWin.is(':hidden')){location.reload();return;}
  }
  },
  hideIcon:function(){
    if(_VR.hideField&&_VR.hideField.length>0){
      _VR.hideField.forEach(function(str){
        try{
         var $tmp=$(str);
...
    if(_VR.isOnAir&&$('#tm_check_rf').is(':checked')){// h5挂机
    var $nFilm=$("div[class^='rate'] div[class^='tip'] li:contains('流畅')");
    var $curFilm=$("div[class^='text'] label[class^='textLabel']");
    if($nFilm.length>0&&'流畅'!=$curFilm.text().trim()){
    $nFilm.click();
    }
...
        }
      }else{
        if((_VR.isOnAir&&_VR.reNum<=4)||(!_VR.isOnAir&&_VR.reNum<=8)){// x次 抢分
          if('发送'==$('div.ChatSend-button').text()){_VR.reNum++;$._FN.thxB();}
        }gNum=t;
        if((!_VR.isOnAir)&&!_VR.qLock&&(_VR.reNum>=8)&& (t>0)){_VR.qLock=true;$._FN.qMk(2);}
        if((_VR.isOnAir)&&!_VR.qLock&&(_VR.reNum>=4)&& (t>0)){_VR.qLock=true;$._FN.qMk(2);}
      }
