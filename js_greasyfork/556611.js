// ==UserScript==
// @name               ChatGPT Exporter [Enhanced HTML - Searchable Chats]
// @name:zh-CN         ChatGPT Exporter
// @name:zh-TW         ChatGPT Exporter
// @namespace          pionxzh - enhanced by Integrace on GreasyFork
// @version            2.29.3
// @author             pionxzh
...
// @grant              unsafeWindow
// @run-at             document-end
// @downloadURL https://update.greasyfork.org/scripts/556611/ChatGPT%20Exporter%20%5BEnhanced%20HTML%20-%20Searchable%20Chats%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/556611/ChatGPT%20Exporter%20%5BEnhanced%20HTML%20-%20Searchable%20Chats%5D.meta.js
// ==/UserScript==

(e=>{const n=document.createElement("style");n.textContent=e,document.head.append(n),setInterval(()=>{n.isConnected||document.head.append(n)},300)})(`
...
.CheckBoxLabel input:checked ~ .IconWrapper {
    animation: checkBounce 0.5s ease;
}

.CheckBoxLabel input:checked ~ svg {
    color: rgb(28 100 242);
    filter: drop-shadow(0 0 8px rgba(28, 100, 242, 0.5));
}
...
.bg-menu {
    background: linear-gradient(135deg, var(--ce-menu-primary) 0%, rgba(255, 255, 255, 0.02) 100%);
...
.menu-item.mt-1:hover::before {
    opacity: 1;
}

.menu-item.mt-1:hover {
...
/* Row full span for export button */
.row-full .menu-item {
...
.row-full .menu-item::before {
    background: linear-gradient(135deg, rgba(255, 255, 255, 0.2) 0%, transparent 100%);
    left: 0;
}

.row-full .menu-item:hover {
...
.row-full .menu-item:hover::before {
    opacity: 1;
}

.row-full .menu-item:active {
...
.DialogTitle::after {
    ...
    background: var(--primary-gradient);
    border-radius: 2px;
    animation: titleUnderline 0.6s cubic-bezier(0.16, 1, 0.3, 1);
...
.dark .DialogTitle::after {
    background: linear-gradient(135deg, #90caf9 0%, #ce93d8 100%);
    box-shadow: 0 0 20px rgba(144, 202, 249, 0.3);
}

/* Scrollable content area */
.DialogContent > div:not(.DialogTitle):not(:last-child) {
    overflow-y: auto;
    flex: 1;
    padding-bottom: 16px;
}

/* Sticky footer for buttons and progress */
.DialogContent > div:last-child {
    position: sticky;
    bottom: 0;
    z-index: 10;
    background: inherit;
...
.dark .DialogContent > div:last-child {
...
.Input:focus {
    ...
    transform: translateY(-2px);
    background: linear-gradient(white, #fafafa) padding-box,
                linear-gradient(135deg, #667eea 0%, #764ba2 100%) border-box;
}
...
.dark .Input:focus {
    background: linear-gradient(#2f2f2f, #252525) padding-box,
                linear-gradient(135deg, #667eea 0%, #764ba2 100%) border-box;
}
...
.DialogContent .w-full.bg-gray-200 {
...
.dark .DialogContent .w-full.bg-gray-200 {
...
/* Progress text */
.DialogContent .mt-2.mb-1 {
    font-weight: 600;
    font-size: 0.9375rem;
    color: var(--page-text);
}

.DialogContent .mt-2.mb-1 span:first-child {
    opacity: 0.9;
}

.DialogContent .mt-2.mb-1 span:last-child {
...
(function (JSZip, html2canvas) {
  ...
  async function getUserAvatar() {
    try {
      ...
      // User profile not available, trying alternative method
    }
    try {
      ...
      // Avatar not found in DOM, using default
    }
...
`;
  const templateSearchableHtml = `<!DOCTYPE html>
<html lang="{{lang}}" data-theme="{{theme}}">
<head>
    <meta charset="UTF-8" />
    <link rel="icon" href="https://chat.openai.com/favicon.ico" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>ChatGPT Conversations - Searchable Export</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.7.0/styles/github-dark.min.css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.7.0/highlight.min.js"><\/script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/KaTeX/0.16.3/katex.min.css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/KaTeX/0.16.3/katex.min.js"><\/script>
    ...
    <style>
        ...
        [data-theme="dark"] {
            ...
            --sidebar-hover: rgba(144, 202, 249, 0.08);
            --sidebar-active: linear-gradient(135deg, rgba(144, 202, 249, 0.15) 0%, rgba(206, 147, 216, 0.15) 100%);
            --border-color: rgba(255, 255, 255, 0.08);
            --search-border: #4b5563;
            --search-focus: #90caf9;
            --badge-bg: linear-gradient(135deg, #90caf9 0%, #ce93d8 100%);
            --badge-text: #000;
            --highlight-bg: #854d0e;
        ...
        * {
        ...
        [data-theme="dark"] body::before {
            background:
                radial-gradient(circle at 20% 20%, rgba(144, 202, 249, 0.03) 0%, transparent 50%),
                radial-gradient(circle at 80% 80%, rgba(206, 147, 216, 0.03) 0%, transparent 50%);
        }
        ...
        [data-theme="dark"] .search-box:focus {
            box-shadow: var(--shadow-lg), 0 0 0 4px rgba(144, 202, 249, 0.1);
        }
        ...
        .search-highlight {
            background: linear-gradient(135deg, var(--highlight-bg) 0%, #fde047 100%);
        ...
        @keyframes highlightPulse {
            ...
            50% {
                box-shadow: 0 1px 6px rgba(254, 240, 138, 0.4);
            }
        }

        [data-theme="dark"] .search-highlight {
            background: linear-gradient(135deg, var(--highlight-bg) 0%, #a16207 100%);
        }
...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll('.conversation-section').forEach(section => {
                allConversations.push({
                    id: section.dataset.conversationId,
                    title: section.dataset.conversationTitle,
                    element: section,
                    messages: Array.from(section.querySelectorAll('.message'))
                });
            });

            // Show first conversation by default if no search
            if (allConversations.length > 0) {
                showConversation(allConversations[0].id);
            }

            // Set up theme from URL
            const urlParams = new URLSearchParams(window.location.search);
            const theme = urlParams.get('theme');
            if (theme) toggleDarkMode(theme);
        });
        ...
        function showConversation(conversationId) {
            ...
            // Show/hide conversation sections
            document.querySelectorAll('.conversation-section').forEach(section => {
                if (section.dataset.conversationId === conversationId) {
        ...
        function performSearch(query) {
            ...
            let totalMatches = 0;
            let conversationsWithMatches = 0;

            allConversations.forEach(conversation => {
                let conversationMatches = 0;
                const sidebarItem = document.querySelector(\`.conversation-item[data-conversation-id="\${conversation.id}"]\`);
                ...
                conversation.messages.forEach(message => {
                    ...
                    // Get original text (remove any previous highlights)
                    ...
                    if (originalText.toLowerCase().includes(lowerQuery)) {
                        ...
                        if (matches) {
                            conversationMatches += matches.length;
                        }

                        // Highlight matches
                        messageContent.innerHTML = highlightText(messageContent.innerHTML, query);
                    } else {
                ...
                if (conversationMatches > 0) {
                    totalMatches += conversationMatches;
                    conversationsWithMatches++;
                    sidebarItem.classList.remove('hidden');

                    // Update match badge
                    let badge = sidebarItem.querySelector('.match-badge');
                    if (!badge) {
                        badge = document.createElement('span');
                        badge.className = 'match-badge';
                        sidebarItem.appendChild(badge);
                    }
                    badge.textContent = conversationMatches;
                } else {
                    sidebarItem.classList.add('hidden');
                    const badge = sidebarItem.querySelector('.match-badge');
            ...
            // Update search info
            if (totalMatches > 0) {
                searchInfo.textContent = \`Found \${totalMatches} match\${totalMatches !== 1 ? 'es' : ''} in \${conversationsWithMatches} conversation\${conversationsWithMatches !== 1 ? 's' : ''}\`;

                // Auto-show first conversation with matches
                const firstMatch = allConversations.find(c =>
                    !document.querySelector(\`.conversation-item[data-conversation-id="\${c.id}"]\`).classList.contains('hidden')
                );
                if (firstMatch) {
                    showConversation(firstMatch.id);
                }
            } else {
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .conversation-header-main h1 {
            font-size: 2rem;
            font-weight: 600;
            margin-bottom: 0.5rem;
        }

        .conversation-header-main .meta {
            font-size: 0.875rem;
            color: var(--meta-title);
        }
...
<body>
    <div class="container">
        <div class="sidebar">
            <div class="sidebar-header">
                <h2>Conversations (<span id="conversationCount">{{conversationCount}}</span>)</h2>
                <button class="theme-toggle" onclick="toggleDarkMode()">
                    <svg class="sun" stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="5"></circle><line x1="12" y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line><line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line></svg>
                    <svg class="moon" stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg>
                </button>
    ...
    <script>
        ...
        async function loadConversations() {
            try {
                ...
                document.getElementById('contentArea').innerHTML =
                    '<div class="empty-state"><div><h3>Connection Error</h3><p>Make sure the Python server is running: <code>python server.py</code></p></div></div>';
            }
        }
        ...
        function renderConversationList() {
            ...
            listElement.innerHTML = allConversations.map(conv => {
                const updateDate = conv.updateTime
                    ? new Date(conv.updateTime * 1000).toLocaleDateString()
                    : 'Unknown';

                return "<div class=\\"conversation-item\\" data-conversation-id=\\"" + conv.id + "\\" onclick=\\"showConversation('" + conv.id + "')\\\">" +
                    "<div class=\\"conversation-title\\">" + escapeHtml(conv.title) + "</div>" +
                    "<div class=\\"conversation-meta\\">" + updateDate + " · " + conv.messageCount + " messages</div>" +
                    "</div>";
            }).join('');
        }
        ...
        async function showConversation(conversationId) {
            ...
            contentArea.innerHTML = '<div class="empty-state"><div><h3>Loading...</h3></div></div>';
            ...
            try {
                ...
                contentArea.innerHTML = '<div class="empty-state"><div><h3>Error</h3><p>Could not load this conversation</p></div></div>';
            }
        }
        ...
        function renderConversation(data) {
            ...
            const createDate = data.createTime ? new Date(data.createTime * 1000).toLocaleDateString() : 'Unknown';
            const updateDate = data.updateTime ? new Date(data.updateTime * 1000).toLocaleDateString() : 'Unknown';

            const messagesHtml = data.messages.map(msg => renderMessage(msg)).join('');

            contentArea.innerHTML = '<div class="conversation-content">' +
                '<div class="conversation-header-main">' +
                '<h1>' + escapeHtml(data.title) + '</h1>' +
                '<div class="meta">Created: ' + createDate + ' · Updated: ' + updateDate + '</div>' +
                '</div>' +
                messagesHtml +
                '</div>';
        ...
        function renderMessage(msg) {
            ...
            if (author === 'user') {
                avatarHtml = '<div class="author user"><img src="' + USER_AVATAR + '" /></div>';
            } else if (author === 'assistant') {
                avatarHtml = '<div class="author assistant"><svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><use href="#chatgpt"></use></svg></div>';
            } else {
                avatarHtml = '<div class="author system"><svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg></div>';
            }
            ...
            if (typeof msg.content === 'string') {
                ...
                textContent = msg.content.parts
                    ...
                    .join('\\n\\n');
            } else if (msg.content.content_type === 'text' && msg.content.parts) {
                textContent = msg.content.parts
                    .filter(p => typeof p === 'string')
                    .join('\\n\\n');
            }

            // Process markdown (marked.js should be loaded)
            let contentHtml = '';
            if (typeof marked !== 'undefined') {
                try {
                    // Protect LaTeX expressions from markdown parser
                    const latexPlaceholders = [];
                    let protectedText = textContent;

                    // Protect display math \\[ ... \\]
                    protectedText = protectedText.replace(/\\\\\\[(.*?)\\\\\\]/gs, function(match) {
                    ...
                    // Parse markdown
                    contentHtml = marked.parse(protectedText);

                    // Restore LaTeX expressions
                    latexPlaceholders.forEach(function(latex, index) {
                        contentHtml = contentHtml.replace('<<<LATEX_DISPLAY_' + index + '>>>', latex);
                        contentHtml = contentHtml.replace('<<<LATEX_INLINE_' + index + '>>>', latex);
                    });
                } catch (e) {
        ...
        async function performSearch(query) {
            ...
            try {
                ...
                let totalMatches = 0;
                let conversationsWithMatches = results.length;

                // Update sidebar to show/hide conversations
                allConversations.forEach(conversation => {
                    const sidebarItem = document.querySelector('[data-conversation-id="' + conversation.id + '"]');
                    const result = results.find(r => r.id === conversation.id);

                    if (result) {
                        sidebarItem.classList.remove('hidden');
                        totalMatches += result.matchCount || 1;
                        ...
                        badge.textContent = result.matchCount || 1;
                    } else {
                        sidebarItem.classList.add('hidden');
                        const badge = sidebarItem.querySelector('.match-badge');
                        if (badge) badge.remove();
                ...
                if (results.length > 0) {
                    searchInfo.textContent = 'Found matches in ' + conversationsWithMatches + ' conversation' + (conversationsWithMatches !== 1 ? 's' : '');

                    // Auto-show first conversation with matches
                    showConversation(results[0].id);
                } else {
                    searchInfo.textContent = 'No matches found';
                }
            } catch (error) {
...
\f\r "&'/<=>`.split(""), "\0	\n\f\r \"&'/<=>`".split("")]
  ...
  async function exportAllToSearchableHtml(fileNameFormat, apiConversations, metaList) {
    const userAvatar = await getUserAvatar();
    const conversations = apiConversations.map((x2) => processConversation(x2));

    // Sort conversations by updateTime (newest first)
    conversations.sort((a, b) => (b.updateTime || 0) - (a.updateTime || 0));

    // Create ZIP file
    const zip = new JSZip();

    // Build database-ready data
    const conversationsData = [];
    const messagesData = [];

    conversations.forEach((conversation) => {
      const { id, title: title2, createTime, updateTime, conversationNodes } = conversation;

      // Extract all text content for full-text search
      const searchableText = conversationNodes.map(({ message }) => {
        if (!message || !message.content) return '';
        if (typeof message.content === 'string') {
          return message.content;
        } else if (message.content.parts) {
          return message.content.parts.filter(p => typeof p === 'string').join(' ');
        }
        return '';
      }).filter(Boolean).join(' ');
      ...
      conversationsData.push({
        ...
        messageCount: conversationNodes.length,
        searchableText
      });

      // Extract messages for this conversation
      conversationNodes.forEach(({ message }) => {
        if (!message || !message.content) return;
        if (message.recipient !== "all") return;
...
'"""\n' +
'ChatGPT Conversations Viewer - Local Server\n' +
'Automatically creates database and starts server\n' +
'"""\n' +
'\n' +
'import sqlite3\n' +
'import json\n' +
...
'\n' +
'def create_database():\n' +
'    """Create database from JSON files if it doesn\'t exist"""\n' +
...
'\n' +
'    # Create indexes\n' +
'    cur.execute("CREATE INDEX IF NOT EXISTS idx_search ON conversations(searchableText)")\n' +
'    cur.execute("CREATE INDEX IF NOT EXISTS idx_conv_time ON conversations(updateTime DESC)")\n' +
'    cur.execute("CREATE INDEX IF NOT EXISTS idx_msg_conv ON messages(conversationId)")\n' +
'\n' +
...
'        print("UTF-8 failed for conversations.json — retrying with Latin-1...")\n' +
'        with open(\'conversations.json\', \'r\', encoding=\'latin-1\') as f:\n' +
'            text = f.read()\n' +
'            conversations = json.loads(text)\n' +
'\n' +
'    for conv in conversations:\n' +
'        cur.execute("""\n' +
'            INSERT INTO conversations (id, title, createTime, updateTime, messageCount, searchableText)\n' +
'            VALUES (?, ?, ?, ?, ?, ?)\n' +
'        """, (conv[\'id\'], conv[\'title\'], conv[\'createTime\'], conv[\'updateTime\'],\n' +
'              conv[\'messageCount\'], conv[\'searchableText\']))\n' +
'\n' +
'    try:\n' +
'        with open(\'messages.json\', \'r\', encoding=\'utf-8\') as f:\n' +
'            messages = json.load(f)\n' +
'    except UnicodeDecodeError:\n' +
'        print("UTF-8 failed for messages.json — retrying with Latin-1...")\n' +
'        with open(\'messages.json\', \'r\', encoding=\'latin-1\') as f:\n' +
'            text = f.read()\n' +
'            messages = json.loads(text)\n' +
'\n' +
'    for msg in messages:\n' +
'        cur.execute("""\n' +
'            INSERT OR IGNORE INTO messages (id, conversationId, author, content, createTime, metadata)\n' +
'            VALUES (?, ?, ?, ?, ?, ?)\n' +
'        """, (msg[\'id\'], msg[\'conversationId\'], msg[\'author\'], msg[\'content\'],\n' +
'              msg[\'createTime\'], msg[\'metadata\']))\n' +
'\n' +
'    conn.commit()\n' +
'    conn.close()\n' +
'\n' +
...
'            return\n' +
'\n' +
'        # API: Get all conversations metadata\n' +
...
'\n' +
'            cur.execute("""\n' +
'                SELECT id, title, createTime, updateTime, messageCount\n' +
'                FROM conversations\n' +
...
'            return\n' +
'\n' +
'        # API: Get specific conversation with messages\n' +
...
'            # Get messages\n' +
'            cur.execute("""\n' +
'                SELECT id, author, content, createTime, metadata\n' +
'                FROM messages\n' +
...
'            return\n' +
'\n' +
'        # API: Search across conversations\n' +
'        if path == \'/api/search\':\n' +
...
'            cur.execute("""\n' +
'                SELECT id, title, createTime, updateTime, messageCount,\n' +
'                       LENGTH(searchableText) - LENGTH(REPLACE(LOWER(searchableText), ?, \'\'))\n' +
'                       AS matchCount\n' +
'                FROM conversations\n' +
'                WHERE searchableText LIKE ?\n' +
'                ORDER BY matchCount DESC, updateTime DESC\n' +
'            """, (query, \'%\' + query + \'%\'))\n' +
'\n' +
...
'        print("Make sure you extracted all files from the ZIP.")\n' +
'        return\n' +
'\n' +
'    if not os.path.exists(\'conversations.json\') or not os.path.exists(\'messages.json\'):\n' +
'        print("Error: conversations.json or messages.json not found!")\n' +
'        print("Make sure you extracted all files from the ZIP.")\n' +
'        return\n' +
'\n' +
...
'    print(f"==============================================")\n' +
'    print(f"  ChatGPT Conversations Viewer")\n' +
'    print(f"==============================================")\n' +
...
'    main()\n';
    ...
    zip.file('server.py', pythonServer);

    // Save conversations and messages as JSON
    ...
    // Generate index HTML
    const theme = ScriptStorage.get("exporter:theme") ?? "light";
    const lang = ScriptStorage.get("exporter:language") ?? "en";

    const indexHtml = templateIndexHtml
      .replaceAll("{{lang}}", lang)
      .replaceAll("{{theme}}", theme)
      .replaceAll("{{conversationCount}}", conversations.length.toString())
      .replaceAll("{{avatar}}", userAvatar);

    zip.file('index.html', standardizeLineBreaks(indexHtml));

    // Generate README
    const readme = '# ChatGPT Conversations Viewer\n' +
'\n' +
'## Quick Start\n' +
'\n' +
...
'- Skips database creation if it already exists\n' +
'- Starts the web server\n' +
'\n' +
'## Features\n' +
'\n' +
'✅ **One-command startup** - No separate setup steps\n' +
'✅ **Search across all conversations** - Fast full-text search\n' +
'✅ **Instant loading** - Efficient database queries\n' +
'✅ **Markdown & LaTeX rendering** - Properly formatted messages\n' +
'✅ **Syntax highlighting** - Beautiful code blocks\n' +
'✅ **No file size limits** - Handle thousands of conversations\n' +
'✅ **Works offline** - All data stays local\n' +
'✅ **Modern UI** - Beautiful, responsive interface\n' +
'\n' +
'## Requirements\n' +
'\n' +
'- Python 3.6+ (comes with macOS/Linux, download for Windows)\n' +
...
'## Files Included\n' +
'\n' +
'- `server.py` - All-in-one server (creates DB + serves content)\n' +
'- `index.html` - Web interface\n' +
'- `conversations.json` - Conversation metadata\n' +
'- `messages.json` - All messages data\n' +
'- `conversations.db` - Created automatically on first run\n';
    ...
    downloadFile('chatgpt-conversations-' + timestamp + '.zip', "application/zip", blob);
    return true;
  }

  function conversationToSearchableMessages(conversation, avatar) {
    const { conversationNodes } = conversation;
    const LatexRegex2 = /(\s\$\$.+?\$\$\s|\s\$.+?\$\s|\\\[.+?\\\]|\\\(.+?\\\))|(^\$$[\S\s]+?^\$$)|(^\$\$[\S\s]+?^\$\$\$)/gm;

    const messagesHtml = conversationNodes.map(({ message }) => {
      var _a, _b, _c, _d;
      ...
      if (message.author.role === "tool") {
        if (
          ...
          !(message.content.content_type === "execution_output" &&
            ((_c = (_b = (_a = message.metadata) == null ? void 0 : _a.aggregate_result) == null ? void 0 : _b.messages) == null ? void 0 : _c.some((msg) => msg.message_type === "image")))
        ) {
          return null;
        }
      }
      ...
      return `<div class="message">
    <div class="author ${authorType}">${avatarEl}</div>
...
${content2}`;
  ...
  const DialogContent = ({ format }) => {
    ...
    const exportAllOptions = F$1(() => [
      { label: "Searchable HTML", callback: exportAllToSearchableHtml }
    ], []);
