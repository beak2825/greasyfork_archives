...
// @license      MIT
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/535250/Show%20Weapon%20Bonuses%20on%20Market%20%28No%20API%20needed%29.user.js
// @updateURL https://update.greasyfork.org/scripts/535250/Show%20Weapon%20Bonuses%20on%20Market%20%28No%20API%20needed%29.meta.js
// ==/UserScript==
...
const stylesheet = `
  <style>
    ...
    .bonuses___a8gmz i:nth-of-type(1){
      transform: scale(2) translate(3px, 2px);
    }
    .bonuses___a8gmz i:nth-of-type(2){
      transform: scale(2) translate(-14px, 2px);
    }
  </style>
`;
...
async function processItem(itemEl) {
  ...
  for (const bonus of bonusElements) {
    ...
    let className = 'bonus-item';
    const percentAmount = Number(percentage.substr(0,percentage.length-1));
    if ((bonusName == "Shock" && percentAmount >= 80) ||
       (bonusName == "Poison" && percentAmount >= 95)) {
        className = 'bonus-item great-bonus';
        titleContainer.parentElement.parentElement.className += " enhanced-item enhanced-item--blue";
    }
    bonusSpan.className = className;
