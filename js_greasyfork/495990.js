...
// @description  Ce script créer des boutons pour accéder à des liens externes.
...
// @grant           none
// @author          TXS
// @namespace       https://greasyfork.org/fr/scripts/495990-wme-mapsfr
// @downloadURL https://update.greasyfork.org/scripts/495990/WME%20MapsFr.user.js
// @updateURL https://update.greasyfork.org/scripts/495990/WME%20MapsFr.meta.js
// ==/UserScript==
/* global $ */
...
let gps,mapsUrl,gz,coord,href,addon,userTabs,navTabs,tabContent,newtab,token,setLayer,citycode,street,locality,oldcitycode,oldcity;
...
const detect_GouvRue =()=> {
    ...
    fetch(`https://api-adresse.data.gouv.fr/reverse/?lon=${c[0]}&lat=${c[1]}`)
        ...
        if((s!='')&&(s!=null)){window.open(`https://adresse.data.gouv.fr/base-adresse-nationale/${s}`,'_self');}
        if((s=='')&&(s!=null)) {window.open(`https://www.geoportail.gouv.fr/carte?c=${c[0]},${c[1]}&z=${c[2]}&l0=ORTHOIMAGERY.ORTHOPHOTOS::GEOPORTAIL:OGC:WMTS(1)&l1=LIMITES_ADMINISTRATIVES_EXPRESS.LATEST::GEOPORTAIL:OGC:WMTS(1)&l2=STATISTICALUNITS.IRIS::GEOPORTAIL:OGC:WMS(1)&permalink=yes`,'_self');}
        if (s==null){window.close();}}
        citycode = (geocodingResult.features[0].properties.citycode);
        mapsUrl = `https://adresse.data.gouv.fr/base-adresse-nationale/${citycode}`;
        window.open(mapsUrl,'_self');
    });};
...
const detect_VilleRue =()=> {
    ...
    fetch(`https://api-adresse.data.gouv.fr/reverse/?lon=${c[0]}&lat=${c[1]}`)
        .then(resp => resp.json())
        .then((geocodingResult) => {
        if (city||oldcity === undefined) { var s=''; if (window.getSelection){ s = window.getSelection();} else if (document.getSelection){ s= document.getSelection();} else if (document.selection){ s = document.selection.createRange().text;}
        ...
        mapsUrl = `https://rues.openalfa.fr/${oldcity}`;
        if (geocodingResult.features[0].properties.oldcity === undefined) { mapsUrl = `https://rues.openalfa.fr/${city}_${city}`;}
        window.open(mapsUrl,'_self');
    });};
...
const detect_InseCog =()=> {
    ...
    fetch(`https://api-adresse.data.gouv.fr/reverse/?lon=${c[0]}&lat=${c[1]}`)
        ...
        if((s!='')&&(s!=null)){window.open(`https://www.insee.fr/fr/metadonnees/cog/commune/COM${s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[\u0020|\u0027]/g, '-')}`,'_self');}
        if((s=='')&&(s!=null)) {window.open(`https://www.geoportail.gouv.fr/carte?c=${c[0]},${c[1]}&z=${c[2]}&l0=ORTHOIMAGERY.ORTHOPHOTOS::GEOPORTAIL:OGC:WMTS(1)&l1=LIMITES_ADMINISTRATIVES_EXPRESS.LATEST::GEOPORTAIL:OGC:WMTS(1)&l2=STATISTICALUNITS.IRIS::GEOPORTAIL:OGC:WMS(1)&permalink=yes`,'_self');}
        if (s==null){window.close();}}
        city = (geocodingResult.features[0].properties.city.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[\u0020|\u0027]/g, '-'));
        citycode = (geocodingResult.features[0].properties.citycode);
        if ((geocodingResult.features[0].properties.oldcitycode === undefined)||(geocodingResult.features[0].properties.oldcity === undefined)) {window.open(`https://www.insee.fr/fr/metadonnees/cog/commune/COM${citycode}-${city}`,'_self');}
        oldcity = (geocodingResult.features[0].properties.oldcity.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[\u0020|\u0027]/g, '-'));
        oldcitycode = (geocodingResult.features[0].properties.oldcitycode);
        window.open(`https://www.insee.fr/fr/metadonnees/cog/commune/COM${oldcitycode}-${oldcity}`,'_self');
    });};
...
function add_buttons() {
    ...
    let btn3 = $('<button class="btnMaps" title="Base Adresse Nationale" style="background-color:#E6E4FF">B A N</button>');
    btn3.click(()=>{
    ...
    let btn5 = $('<button class="btnMaps" title="Open Street Map">O S M</button>');
    btn5.click(()=>{
        let {lon, lat, zoom} = getGps(0);
        mapsUrl = `http://www.openstreetmap.org/#map=${zoom}/${lat}/${lon}`;
        window.open(mapsUrl,'_blank');
    });
    let btn6 = $('<button class="btnMaps" title="clic => Carte Via Michelin Trafic,\rCTRL+clic => même carte plus épurée.">Michelin</button>');
    btn6.click(()=>{
        let {lon, lat, zoom} = getGps(0);
        mapsUrl = `https://www.viamichelin.fr/web/Trafic?position=${lat};${lon};${zoom} `;
        var wazeExt = W.map.getExtent();
        wazeExt.transform(new OpenLayers.Projection('EPSG:900913'),new OpenLayers.Projection('EPSG:4326'));
        if (event.ctrlKey) {mapsUrl = `https://www.viamichelin.fr/web/Trafic?geoboundaries=${wazeExt.bottom},${wazeExt.left}:${wazeExt.top},${wazeExt.right}`;}
    ...
    btn11.click(()=>{
        let {lon, lat, zoom} = getGps(0);
        mapsUrl = `https://www.bing.com/maps?osid=ef3f0715-2277-4a9d-8706-500556506052&cp=${lat}~${lon}&lvl=${zoom}&style=h`;
        window.open(mapsUrl,'_blank');
    });
    ...
    let btn13 = $('<button class="btnMaps" title="clic => Mapillary\rCTRL+clic OpenStreetCam"style="background-color:#27cc35">Mapillary +</button>');
    btn13.click(()=>{
        let {lon, lat, zoom} = getGps(-1);
        mapsUrl = `https://www.mapillary.com/app/?lat=${lat}&lng=${lon}&z=${zoom}`;
        if (event.ctrlKey) { mapsUrl = `https://kartaview.org/map/@${lat},${lon},${zoom}z`;}
        if (event.ctrlKey) {setTimeout(() => window.open(mapsUrl, '_blank')); } else { window.open(mapsUrl, '_blank'); }
    });
    ...
    let btn30 = $('<input class="btnMapsIc" type="image" alt="La Poste" title="clic => carte La Poste \rCTRL+clic carte La Poste OpenData" src="https://www.laposte.fr/_ui/eboutique/images/fusion/logolp_part.svg">');
    btn30.click(()=>{
        let {lon, lat, zoom} = getGps(0);
        mapsUrl = `https://www.google.fr/?MFRlaposte[${lon},${lat},${zoom}]`;
        if (event.ctrlKey) { mapsUrl = `https://datanova.laposte.fr/applications/carte-des-bureaux-de-poste-agences-postales-et-relais-poste?lat=${lat}&lng=${lon}&zoom=${zoom-1}`;}
        if (event.ctrlKey) {setTimeout(() => window.open(mapsUrl, '_blank')); } else { window.open(mapsUrl, '_blank'); }
    });
    ...
    let btn31 = $('<input class="btnMapsIc" type="image" alt="Chargemap"  title="clic => Carte des bornes de recharge,\rCTRL+clic pour recherche par ville." src="https://fr.chargemap.com/client/chargemap/images/logo-inline.svg ">');
    btn31.click(()=>{
        let {lon, lat, zoom} = getGps(0);
    ...
    let btn40 = $('<input class="btnMapsIc" type="image" alt="radars-auto\.com"  title="Emplacement des radars routiers sur radars-auto.com,\rou CTRL+clic pour accéder à la carte Lufop." src="https://www.radars-auto.com/images/pictos/blog/.cat-general_sq.jpg" >');
    btn40.click(()=>{
        let {lon, lat, zoom} = getGps(0);
        mapsUrl = `https://www.google.fr/?MFRradauto[${lon},${lat},${zoom}]`;
        if (event.ctrlKey) { mapsUrl = `https://lufop.net/MAP/?point=off&q=${lat},${lon}&pays=FR&lang=fr#`;}
        if (event.ctrlKey) {setTimeout(() => window.open(mapsUrl, '_blank')); } else { window.open(mapsUrl, '_blank'); }
    });
    ...
    btn42.click(()=>{
        ...
        if (event.ctrlKey) { mapsUrl = `https://anct-carto.github.io/france_services/?lat=${cproj.lat}&lng=${cproj.lon}&z=}`;}
        if (event.ctrlKey) {setTimeout(() => window.open(mapsUrl, '_blank')); } else { window.open(mapsUrl, '_blank'); }
    });
 
 
    let btn43 = $('<input class="btnMapsIc" type="image" alt="Freshmile"  title="Bornes Freshmile" src="https://www.freshmile.com/_nuxt/freshmile-logo.825cc018.svg" >');
    btn43.click(()=>{
        let {lon, lat, zoom} = getGps(+1);
...
// https://wxs.ign.fr/an7nvfzojv5wa96dsga5nk8w/geoportail/v/wms?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetFeatureInfo&FORMAT=image/png&TRANSPARENT=TRUE&QUERY_LAYERS=STATISTICALUNITS.IRISGE&LAYERS=STATISTICALUNITS.IRISGE&INFO_FORMAT=text/html&I=50&J=50&CRS=EPSG:3857&STYLES=&WIDTH=101&HEIGHT=101&BBOX=-321156.27105565934,6111197.888493066,-319841.4690007886,6112512.690547937
...
//    });
    ...
    $("#sidepanel-MapsFr").append('<br>');
    $("#sidepanel-MapsFr").append(btn38); $("#sidepanel-MapsFr").append(btn42);
    $("#sidepanel-MapsFr").append('<br>');
    $("#sidepanel-MapsFr").append('<br>');
    ...
    $("#sidepanel-MapsFr").append('<br>');
    $("#sidepanel-MapsFr").append(btn34); $("#sidepanel-MapsFr").append(btn43); //$("#sidepanel-MapsFr").append(btn45);
}
