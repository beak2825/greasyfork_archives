// ==UserScript==
// @name              M3U8 ad heimuer
// @namespace         http://tampermonkey.net/
// @version           2.8.0
// @description       结合M3U8广告过滤和特定视频链接解析功能。拦截和过滤M3U8广告切片，同时为特定网站含$符号的链接提供解析播放按钮，并通过iframe自定义DPlayer标题，支持按钮变色记忆及页面内过滤模式选择。
// @author            ltxlong & adapted
// @match             *://*/*
// @run-at            document-start
// @grant             unsafeWindow
// @grant             GM_getResourceText
// @grant             GM_registerMenuCommand
// @grant             GM_unregisterMenuCommand
// @grant             GM_setValue
// @grant             GM_getValue
// @require           https://unpkg.com/sweetalert2@11/dist/sweetalert2.min.js
// @resource Swal     https://unpkg.com/sweetalert2@11/dist/sweetalert2.min.css
// @resource SwalDark https://unpkg.com/@sweetalert2/theme-dark@5/dark.min.css
// @license           MIT
// @downloadURL https://update.greasyfork.org/scripts/536224/M3U8%20ad%20heimuer.user.js
// @updateURL https://update.greasyfork.org/scripts/536224/M3U8%20ad%20heimuer.meta.js
// ==/UserScript==

(function() {
    'use strict';

    // -----------------------------
    // Part 1: M3U8 Filter Ad Script
    // -----------------------------

    let ts_name_len = 0; // ts前缀长度
    let ts_name_len_extend = 1; // 容错
    let first_extinf_row = '';
    let the_extinf_judge_row_n = 0;
    let the_same_extinf_name_n = 0;
    let the_extinf_benchmark_n = 5; // 基准
    let prev_ts_name_index = -1; // 上个ts序列号
    let first_ts_name_index = -1; // 首个ts序列号
    let ts_type = parseInt(localStorage.getItem('filter_ts_type')) || 0; // 0：xxxx000数字递增.ts模式0；1：xxxxxxxxxx.ts模式1；2：***.ts模式2-暴力拆解，从localStorage初始化
    let the_ext_x_mode = parseInt(localStorage.getItem('filter_ext_x_mode')) || 0; // 0：ext_x_discontinuity判断模式0；1：ext_x_discontinuity判断模式1，从localStorage初始化
    let the_current_host = unsafeWindow.location.hostname;
    let script_whitelist_mode_flag = GM_getValue('script_whitelist_mode_flag', false); // 是否启用白名单模式，默认否
    let the_current_host_in_whitelist_flag = GM_getValue(the_current_host, false); // 当前域名是否在白名单，默认否
    let show_toast_tip_flag = GM_getValue('show_toast_tip_flag', false); // 是否启用弹窗提示，默认否
    let violent_filter_mode_flag = localStorage.getItem('filter_violent_mode') === 'true'; // 是否启用暴力拆解模式，默认否-自动判断模式，从localStorage初始化
    let filter_log_html = '';
    let filter_done_flag = false;

    function filter_log(...msg) {
        const log_content = msg.join('</p><p>');
        console.log('%c[m3u8_filter_ad]', 'font-weight: bold; color: white; background-color: #70b566b0; padding: 2px; border-radius: 2px;', ...msg);
        filter_log_html += `
            <p>
                <span style="font-weight: bold; color: white; background-color: #70b566b0; padding: 2px; border-radius: 2px;">[m3u8_filter_ad]</span>
                ${log_content}
            </p>
        `;
    }

    let the_swalcss_color = "#ff679a";
    let swalcss = `
        .swal2-styled{transition: all 0.2s ease;}
        .swal2-loader{display:none;align-items:center;justify-content:center;width:2.2em;height:2.2em;margin:0 1.875em;-webkit-animation:swal2-rotate-loading 1.5s linear 0s infinite normal;animation:swal2-rotate-loading 1.5s linear 0s infinite normal;border-width:.25em;border-style:solid;border-radius:100%;border-color:${the_swalcss_color} transparent }
        .swal2-styled.swal2-confirm{border:0;border-radius:.25em;background:initial;background-color:${the_swalcss_color};color:#fff;font-size:1em}
        .swal2-styled.swal2-confirm:hover,.swal2-styled.swal2-deny:hover{opacity:0.8;background-image:none!important}
        .swal2-styled.swal2-confirm:focus{box-shadow:0 0 0 3px ${the_swalcss_color}80}
        .swal2-styled.swal2-deny:focus{box-shadow:0 0 0 3px #dc374180}
        .swal2-timer-progress-bar-container{position:absolute;right:0;bottom:0;left:0;grid-column:auto;overflow:hidden;border-bottom-right-radius:5px;border-bottom-left-radius:5px}
        .swal2-timer-progress-bar{width:100%;height:.25em;background:${the_swalcss_color}33 }
        .swal2-progress-steps .swal2-progress-step{z-index:20;flex-shrink:0;width:2em;height:2em;border-radius:2em;background:${the_swalcss_color};color:#fff;line-height:2em;text-align:center}
        .swal2-progress-steps .swal2-progress-step.swal2-active-progress-step{background:${the_swalcss_color} }
        .swal2-progress-steps .swal2-progress-step-line{z-index:10;flex-shrink:0;width:2.5em;height:.4em;margin:0 -1px;background:${the_swalcss_color}}
        .swal2-popup {padding:1.25em 0 1.25em;flex-direction:column}
        .swal2-close {position:absolute;top:1px;right:1px;transition: all 0.2s ease;}
        div:where(.swal2-container) .swal2-html-container{padding: 1.3em 1.3em 0.3em;}
        div:where(.swal2-container) button:where(.swal2-close):hover {color:${the_swalcss_color}!important;font-size:60px!important}
        div:where(.swal2-icon) .swal2-icon-content {font-family: sans-serif;}
        .swal2-container {z-index: 1145141919810;}
    `;

    function add_style(id, css) {
        let try_add_style_n = 0;
        let try_to_add_style = function() {
            let the_style_dom = unsafeWindow.document.getElementById(id);
            if (the_style_dom) the_style_dom.remove();
            let the_style = unsafeWindow.document.createElement('style');
            the_style.rel = 'stylesheet';
            the_style.id = id;
            the_style.innerHTML = css;
            let the_target_element = unsafeWindow.document.body;
            if (the_target_element) {
                the_target_element.insertBefore(the_style, the_target_element.firstChild);
            } else {
                try_add_style_n++;
                if (try_add_style_n < 50) {
                    setTimeout(try_to_add_style, 100);
                }
            }
        };
        try_to_add_style();
    }

    // 监听颜色方案变化并设置样式
    unsafeWindow.matchMedia('(prefers-color-scheme: dark)').addListener(function (e) {
        if (e.matches) {
            add_style('swal-pub-style', GM_getResourceText('SwalDark'));
        } else {
            add_style('swal-pub-style', GM_getResourceText('Swal'));
        }
        add_style('Panlinker-SweetAlert2-User', swalcss);
    });

    if (unsafeWindow.matchMedia && unsafeWindow.matchMedia('(prefers-color-scheme: dark)').matches) {
        add_style('swal-pub-style', GM_getResourceText('SwalDark'));
    } else {
        add_style('swal-pub-style', GM_getResourceText('Swal'));
    }
    add_style('Panlinker-SweetAlert2-User', swalcss);

    // Toast 提示配置
    let toast = Swal.mixin({
        toast: true,
        position: 'top-end',
        showConfirmButton: false,
        timer: 3000,
        timerProgressBar: true,
        showCloseButton: true,
        didOpen: function (toast) {
            toast.addEventListener('mouseenter', Swal.stopTimer);
            toast.addEventListener('mouseleave', Swal.resumeTimer);
        }
    });

    let message = {
        success: function (text) { toast.fire({ title: text, icon: 'success' }); },
        error: function (text) { toast.fire({ title: text, icon: 'error' }); },
        warning: function (text) { toast.fire({ title: text, icon: 'warning' }); },
        info: function (text) { toast.fire({ title: text, icon: 'info' }); },
        question: function (text) { toast.fire({ title: text, icon: 'question' }); }
    };

    function is_m3u8_file(url) {
        return /\.m3u8($|\?)/.test(url);
    }

    function extract_number_before_ts(str) {
        const match = str.match(/(\d+)\.ts/);
        if (match) {
            return parseInt(match[1], 10);
        }
        return null;
    }

    function filter_lines(lines) {
        let result = [];
        if (violent_filter_mode_flag) {
            filter_log('----------------------------暴力拆解模式--------------------------');
            ts_type = 2;
        } else {
            filter_log('----------------------------自动判断模式--------------------------');
            let the_normal_int_ts_n = 0;
            let the_diff_int_ts_n = 0;
            let last_ts_name_len = 0;
            for (let i = 0; i < lines.length; i++) {
                const line = lines[i];
                if (the_extinf_judge_row_n === 0 && line.startsWith('#EXTINF')) {
                    first_extinf_row = line;
                    the_extinf_judge_row_n++;
                } else if (the_extinf_judge_row_n === 1 && line.startsWith('#EXTINF')) {
                    if (line !== first_extinf_row) {
                        first_extinf_row = '';
                    }
                    the_extinf_judge_row_n++;
                }
                let the_ts_name_len = line.indexOf('.ts');
                if (the_ts_name_len > 0) {
                    if (the_extinf_judge_row_n === 1) {
                        ts_name_len = the_ts_name_len;
                    }
                    last_ts_name_len = the_ts_name_len;
                    let ts_name_index = extract_number_before_ts(line);
                    if (ts_name_index === null) {
                        if (the_extinf_judge_row_n === 1) {
                            ts_type = 1;
                        } else if (the_extinf_judge_row_n === 2 && (ts_type === 1 || the_ts_name_len === ts_name_len)) {
                            ts_type = 1;
                            filter_log('----------------------------识别ts模式1---------------------------');
                            break;
                        } else {
                            the_diff_int_ts_n++;
                        }
                    } else {
                        if (the_normal_int_ts_n === 0) {
                            prev_ts_name_index = ts_name_index;
                            first_ts_name_index = ts_name_index;
                            prev_ts_name_index = first_ts_name_index - 1;
                        }
                        if (the_ts_name_len !== ts_name_len) {
                            if (the_ts_name_len === last_ts_name_len + 1 && ts_name_index === prev_ts_name_index + 1) {
                                if (the_diff_int_ts_n) {
                                    if (ts_name_index === prev_ts_name_index + 1) {
                                        ts_type = 0;
                                        prev_ts_name_index = first_ts_name_index - 1;
                                        filter_log('----------------------------识别ts模式0---------------------------');
                                        break;
                                    } else {
                                        ts_type = 2;
                                        filter_log('----------------------------识别ts模式2---------------------------');
                                        break;
                                    }
                                }
                                the_normal_int_ts_n++;
                                prev_ts_name_index = ts_name_index;
                            } else {
                                the_diff_int_ts_n++;
                            }
                        } else {
                            if (the_diff_int_ts_n) {
                                if (ts_name_index === prev_ts_name_index + 1) {
                                    ts_type = 0;
                                    prev_ts_name_index = first_ts_name_index - 1;
                                    filter_log('----------------------------识别ts模式0---------------------------');
                                    break;
                                } else {
                                    ts_type = 2;
                                    filter_log('----------------------------识别ts模式2---------------------------');
                                    break;
                                }
                            }
                            the_normal_int_ts_n++;
                            prev_ts_name_index = ts_name_index;
                        }
                    }
                }
                if (i === lines.length - 1) {
                    ts_type = 2;
                    filter_log('----------------------------进入暴力拆解模式---------------------------');
                }
            }
        }

        for (let i = 0; i < lines.length; i++) {
            let ts_index_check = false;
            const line = lines[i];
            if (ts_type === 0) {
                if (line.startsWith('#EXT-X-DISCONTINUITY') && lines[i + 1] && lines[i + 2]) {
                    if (i > 0 && lines[i - 1].startsWith('#EXT-X-')) {
                        result.push(line);
                        continue;
                    } else {
                        let the_ts_name_len = lines[i + 2].indexOf('.ts');
                        if (the_ts_name_len > 0) {
                            if (the_ts_name_len - ts_name_len > ts_name_len_extend) {
                                if (lines[i + 3] && lines[i + 3].startsWith('#EXT-X-DISCONTINUITY')) {
                                    filter_log('过滤规则: #EXT-X-DISCONTINUITY-ts文件名长度-');
                                    filter_log('过滤的行:', "\n", line, "\n", lines[i + 1], "\n", lines[i + 2], "\n", lines[i + 3]);
                                    filter_log('------------------------------------------------------------------');
                                    i += 3;
                                } else {
                                    filter_log('过滤规则: #EXT-X-DISCONTINUITY-ts文件名长度');
                                    filter_log('过滤的行:', "\n", line, "\n", lines[i + 1], "\n", lines[i + 2]);
                                    filter_log('------------------------------------------------------------------');
                                    i += 2;
                                }
                                continue;
                            } else {
                                ts_name_len = the_ts_name_len;
                            }
                            let the_ts_name_index = extract_number_before_ts(lines[i + 2]);
                            if (the_ts_name_index !== prev_ts_name_index + 1) {
                                if (lines[i + 3] && lines[i + 3].startsWith('#EXT-X-DISCONTINUITY')) {
                                    filter_log('过滤规则: #EXT-X-DISCONTINUITY-ts序列号-');
                                    filter_log('过滤的行:', "\n", line, "\n", lines[i + 1], "\n", lines[i + 2], "\n", lines[i + 3]);
                                    filter_log('------------------------------------------------------------------');
                                    i += 3;
                                } else {
                                    filter_log('过滤规则: #EXT-X-DISCONTINUITY-ts序列号');
                                    filter_log('过滤的行:', "\n", line, "\n", lines[i + 1], "\n", lines[i + 2]);
                                    filter_log('------------------------------------------------------------------');
                                    i += 2;
                                }
                                continue;
                            }
                        }
                    }
                }
                if (line.startsWith('#EXTINF') && lines[i + 1]) {
                    let the_ts_name_len = lines[i + 1].indexOf('.ts');
                    if (the_ts_name_len > 0) {
                        if (the_ts_name_len - ts_name_len > ts_name_len_extend) {
                            if (lines[i + 2] && lines[i + 2].startsWith('#EXT-X-DISCONTINUITY')) {
                                filter_log('过滤规则: #EXTINF-ts文件名长度-');
                                filter_log('过滤的行:', "\n", line, "\n", lines[i + 1], "\n", lines[i + 2]);
                                filter_log('------------------------------------------------------------------');
                                i += 2;
                            } else {
                                filter_log('过滤规则: #EXTINF-ts文件名长度');
                                filter_log('过滤的行:', "\n", line, "\n", lines[i + 1]);
                                filter_log('------------------------------------------------------------------');
                                i += 1;
                            }
                            continue;
                        } else {
                            ts_name_len = the_ts_name_len;
                        }
                        let the_ts_name_index = extract_number_before_ts(lines[i + 1]);
                        if (the_ts_name_index === prev_ts_name_index + 1) {
                            prev_ts_name_index++;
                        } else {
                            if (lines[i + 2].startsWith('#EXT-X-DISCONTINUITY')) {
                                filter_log('过滤规则: #EXTINF-ts序列号-');
                                filter_log('过滤的行:', "\n", line, "\n", lines[i + 1], "\n", lines[i + 2]);
                                filter_log('------------------------------------------------------------------');
                                i += 2;
                            } else {
                                filter_log('过滤规则: #EXTINF-ts序列号');
                                filter_log('过滤的行:', "\n", line, "\n", lines[i + 1]);
                                filter_log('------------------------------------------------------------------');
                                i += 1;
                            }
                            continue;
                        }
                    }
                }
            } else if (ts_type === 1) {
                if (line.startsWith('#EXTINF')) {
                    if (line === first_extinf_row && the_same_extinf_name_n <= the_extinf_benchmark_n && the_ext_x_mode === 0) {
                        the_same_extinf_name_n++;
                    } else {
                        the_ext_x_mode = 1;
                    }
                    if (the_same_extinf_name_n > the_extinf_benchmark_n) {
                        the_ext_x_mode = 1;
                    }
                }
                if (line.startsWith('#EXT-X-DISCONTINUITY')) {
                    if (i > 0 && lines[i - 1].startsWith('#EXT-X-PLAYLIST-TYPE')) {
                        result.push(line);
                        continue;
                    } else {
                        if (lines[i + 1] && lines[i + 1].startsWith('#EXTINF') && lines[i + 2] && lines[i + 2].indexOf('.ts') > 0) {
                            let the_ext_x_discontinuity_condition_flag = false;
                            if (the_ext_x_mode === 1) {
                                the_ext_x_discontinuity_condition_flag = lines[i + 1] !== first_extinf_row && the_same_extinf_name_n > the_extinf_benchmark_n;
                            }
                            if (lines[i + 3] && lines[i + 3].startsWith('#EXT-X-DISCONTINUITY') && the_ext_x_discontinuity_condition_flag) {
                                filter_log('过滤规则: #EXT-X-DISCONTINUITY-广告-#EXT-X-DISCONTINUITY过滤');
                                filter_log('过滤的行:', "\n", line, "\n", lines[i + 1], "\n", lines[i + 2], "\n", lines[i + 3]);
                                filter_log('------------------------------------------------------------------');
                                i += 3;
                            } else {
                                filter_log('过滤规则: #EXT-X-DISCONTINUITY-单个标识过滤');
                                filter_log('过滤的行:', "\n", line);
                                filter_log('------------------------------------------------------------------');
                            }
                            continue;
                        }
                    }
                }
            } else {
                if (line.startsWith('#EXT-X-DISCONTINUITY')) {
                    if (i > 0 && lines[i - 1].startsWith('#EXT-X-PLAYLIST-TYPE')) {
                        result.push(line);
                        continue;
                    } else {
                        filter_log('过滤规则: #EXT-X-DISCONTINUITY-单个标识过滤');
                        filter_log('过滤的行:', "\n", line);
                        filter_log('------------------------------------------------------------------');
                        continue;
                    }
                }
            }
            result.push(line);
        }
        return result;
    }

    async function safely_process_m3u8(url, content) {
        try {
            const lines = content.split('\n');
            const new_lines = filter_lines(lines);
            return new_lines.join('\n');
        } catch (e) {
            filter_log(`处理 m3u8 文件时出错: ${url}`, e);
            return content;
        }
    }

    // 脚本菜单变量
    let menu_item_violent = null;
    let menu_item_mode = null;
    let menu_item_host_join = null;
    let menu_item_toast = null;
    let menu_item_filter_tip = null;

    function hookXHR() {
        const OriginalXHR = unsafeWindow.XMLHttpRequest;
        unsafeWindow.XMLHttpRequest = class extends OriginalXHR {
            constructor() {
                super();
                this.addEventListener('readystatechange', async function () {
                    if (this.readyState === 4 && this.status === 200 && is_m3u8_file(this.responseURL)) {
                        filter_log('----------------------------hookXHR成功---------------------------');
                        const modifiedResponse = await safely_process_m3u8(this.responseURL, this.responseText);
                        Object.defineProperty(this, 'responseText', { value: modifiedResponse });
                        Object.defineProperty(this, 'response', { value: modifiedResponse });
                        if (show_toast_tip_flag) {
                            message.success('已成功过滤切片广告');
                        }
                        filter_done_flag = true;
                        GM_unregisterMenuCommand(menu_item_filter_tip);
                        check_menu_item_filter_tip();
                    }
                }, false);
            }
        };
    }

    function initHook() {
        hookXHR();
    }

    // 初始化菜单判断变量
    violent_filter_mode_flag = localStorage.getItem('filter_violent_mode') === 'true';
    script_whitelist_mode_flag = GM_getValue('script_whitelist_mode_flag', false);
    the_current_host_in_whitelist_flag = GM_getValue(the_current_host, false);
    show_toast_tip_flag = GM_getValue('show_toast_tip_flag', false);

    function check_menu_item_violent() {
        if (violent_filter_mode_flag) {
            menu_item_violent = GM_registerMenuCommand('暴力拆解模式（可点击切换到自动判断过滤模式）', function() {
                GM_setValue('violent_filter_mode_flag', false);
                violent_filter_mode_flag = false;
                localStorage.setItem('filter_violent_mode', 'false');
                message.success('已设置：<br><br>自动判断过滤模式！');
                unsafeWindow.location.reload();
            });
        } else {
            menu_item_violent = GM_registerMenuCommand('自动判断过滤模式（可点击切换到暴力拆解模式）', function() {
                GM_setValue('violent_filter_mode_flag', true);
                violent_filter_mode_flag = true;
                localStorage.setItem('filter_violent_mode', 'true');
                message.success('已设置：<br><br>暴力拆解模式！');
                unsafeWindow.location.reload();
            });
        }
    }

    function check_menu_item_mode() {
        if (script_whitelist_mode_flag) {
            menu_item_mode = GM_registerMenuCommand('现在是白名单模式（可点击切换到全匹配模式）', function() {
                GM_setValue('script_whitelist_mode_flag', false);
                script_whitelist_mode_flag = false;
                message.success('已设置：<br><br>全匹配模式，即匹配所有网站！');
                unsafeWindow.location.reload();
            });
        } else {
            menu_item_mode = GM_registerMenuCommand('现在是全匹配模式（可点击切换到白名单模式）', function() {
                GM_setValue('script_whitelist_mode_flag', true);
                script_whitelist_mode_flag = true;
                message.success('已设置：<br><br>白名单模式，即需要单个网站设置加入过滤名单！');
                unsafeWindow.location.reload();
            });
        }
    }

    function check_menu_item_host_join() {
        if (script_whitelist_mode_flag) {
            if (the_current_host_in_whitelist_flag) {
                initHook();
                if (unsafeWindow.self === unsafeWindow.top) {
                    if (menu_item_host_join === null) {
                        filter_log('----------------------------脚本加载完成---------------------------');
                        filter_log('----------------------------还没 hookXHR---------------------------');
                    }
                    menu_item_host_join = GM_registerMenuCommand('本网站已开启过滤（可点击关闭广告过滤）', function() {
                        GM_setValue(the_current_host, false);
                        the_current_host_in_whitelist_flag = false;
                        message.success('已设置：<br><br>关闭本网站的广告过滤！');
                        unsafeWindow.location.reload();
                    });
                }
            } else {
                if (unsafeWindow.self === unsafeWindow.top) {
                    if (menu_item_host_join === null) {
                        filter_log('----------------------------还没开启过滤---------------------------');
                    }
                    menu_item_host_join = GM_registerMenuCommand('本网站已关闭过滤（可点击开启广告过滤）', function() {
                        GM_setValue(the_current_host, true);
                        the_current_host_in_whitelist_flag = true;
                        message.success('已设置：<br><br>开启本网站的广告过滤！');
                        unsafeWindow.location.reload();
                    });
                }
            }
        } else {
            if (menu_item_host_join === null) {
                filter_log('----------------------------脚本加载完成---------------------------');
                filter_log('----------------------------还没 hookXHR---------------------------');
                initHook();
            }
        }
    }

    function check_menu_item_toast() {
        if (show_toast_tip_flag) {
            if (unsafeWindow.self === unsafeWindow.top) {
                menu_item_toast = GM_registerMenuCommand('已开启弹窗提示（可点击设置关闭）', function() {
                    GM_setValue('show_toast_tip_flag', false);
                    show_toast_tip_flag = false;
                    message.success('已设置：<br><br>关闭弹窗提示！');
                    add_menu_item_all();
                });
            }
        } else {
            if (unsafeWindow.self === unsafeWindow.top) {
                menu_item_toast = GM_registerMenuCommand('已关闭弹窗提示（可点击设置开启）', function() {
                    GM_setValue('show_toast_tip_flag', true);
                    show_toast_tip_flag = true;
                    message.success('已设置：<br><br>开启弹窗提示！');
                    add_menu_item_all();
                });
            }
        }
    }

    function register_menu_filter_done_tip() {
        menu_item_filter_tip = GM_registerMenuCommand('提示：已成功过滤视频切片广告', function() {
            Swal.fire({
                title: '过滤日志',
                type: 'info',
                icon: 'info',
                html: filter_log_html,
                width: '50%',
                showClass: { popup: '' },
                hideClass: { popup: '' },
                confirmButtonText: 'OK',
                showCloseButton: true
            });
        });
    }

    function register_menu_filter_undone_tip() {
        menu_item_filter_tip = GM_registerMenuCommand('提示：还没有过滤视频切片广告', function() {
            Swal.fire({
                type: 'info',
                icon: 'info',
                html: '过滤视频切片广告失败！ <br><br> ctrl + F5 刷新试试 <br><br> 如果还不行，那说明：<br><br> 视频格式不是m3u8，无法过滤！',
                showClass: { popup: '' },
                hideClass: { popup: '' },
                confirmButtonText: 'OK',
                showCloseButton: true
            });
        });
    }

    function check_menu_item_filter_tip() {
        if (filter_done_flag) {
            register_menu_filter_done_tip();
        } else {
            if ((script_whitelist_mode_flag && the_current_host_in_whitelist_flag) || !script_whitelist_mode_flag) {
                register_menu_filter_undone_tip();
            }
        }
    }

    function remove_menu_item_all() {
        GM_unregisterMenuCommand(menu_item_violent);
        GM_unregisterMenuCommand(menu_item_mode);
        GM_unregisterMenuCommand(menu_item_host_join);
        GM_unregisterMenuCommand(menu_item_toast);
    }

    function add_menu_item_all() {
        remove_menu_item_all();
        check_menu_item_violent();
        check_menu_item_mode();
        check_menu_item_host_join();
        check_menu_item_toast();
    }

    add_menu_item_all();

    function listen_video_load_meta() {
        if ((script_whitelist_mode_flag && the_current_host_in_whitelist_flag) || !script_whitelist_mode_flag) {
            let try_listen_video_meta_n = 0;
            let try_to_add_listen_video_meta = function () {
                let the_video_dom = unsafeWindow.document.querySelector('video');
                if (the_video_dom) {
                    the_video_dom.addEventListener('loadedmetadata', () => {
                        setTimeout(function() {
                            if (!filter_done_flag) {
                                if (show_toast_tip_flag) {
                                    if (the_video_dom.src.indexOf('.mp4') > 0) {
                                        message.error('过滤失败！<br><br>播放视频格式是mp4 <br><br>不是m3u8，无法过滤！');
                                    } else {
                                        message.warning('过滤失败！ctrl+F5 刷新试试，<br><br> 若还不行，说明播放视频格式<br>不是m3u8，无法过滤！');
                                    }
                                }
                                GM_unregisterMenuCommand(menu_item_filter_tip);
                                check_menu_item_filter_tip();
                            }
                        }, 1000);
                    });
                } else {
                    try_listen_video_meta_n++;
                    if (try_listen_video_meta_n < 50) {
                        setTimeout(try_to_add_listen_video_meta, 100);
                    }
                }
            }
            try_to_add_listen_video_meta();
        }
    }

    listen_video_load_meta();

    // -----------------------------
    // Part 2: Video Parse Assistant
    // -----------------------------

    // 特定域名列表
    const supportedDomains = ['bfzy.tv', 'suonizy.net', 'heimuer.tv'];

    // 检查当前域名是否在支持列表中
    function isSupportedDomain() {
        const hostname = window.location.hostname;
        return supportedDomains.some(domain => hostname.includes(domain));
    }

    // 只有在支持的域名下才执行视频解析功能
    if (isSupportedDomain()) {
        console.log('%c[Video Parse Assistant]', 'font-weight: bold; color: white; background-color: #2196F3; padding: 2px; border-radius: 2px;', 'Script loaded for supported domain:', window.location.hostname);
        
        // 添加样式
        const styles = `
            .parse-btn {
                display: inline-block;
                margin-left: 8px;
                padding: 2px 8px;
                color: white;
                background: #2196F3;
                border-radius: 4px;
                cursor: pointer;
                font-size: 12px;
                vertical-align: middle;
                transition: background 0.3s;
            }
            .parse-btn:hover {
                background: #1976D2;
            }
            .parse-btn.active {
                background: #FF5252;
            }
            .parse-btn.active:hover {
                background: #D32F2F;
            }
            .video-iframe-container {
                position: fixed;
                top: 0;
                left: 0;
                width: 100%;
                height: 100%;
                background: rgba(0, 0, 0, 0.9);
                z-index: 9999;
                display: flex;
                justify-content: center;
                align-items: center;
            }
            .video-iframe {
                width: 100%;
                height: 100%;
                border: none;
            }
            .close-iframe-btn {
                position: absolute;
                top: 10px;
                right: 10px;
                background: #ff5252;
                color: white;
                border: none;
                padding: 5px 10px;
                cursor: pointer;
                font-size: 16px;
                border-radius: 4px;
                z-index: 10000;
            }
            .filter-control-panel {
                position: fixed;
                top: 10px;
                right: 10px;
                background: rgba(0, 0, 0, 0.7);
                color: white;
                padding: 8px;
                border-radius: 5px;
                z-index: 1000;
                display: flex;
                flex-direction: column;
                gap: 4px;
                font-size: 12px;
            }
            .filter-btn {
                background: #2196F3;
                color: white;
                border: none;
                padding: 3px 6px;
                cursor: pointer;
                border-radius: 3px;
                font-size: 11px;
                min-width: 60px;
            }
            .filter-btn:hover {
                background: #1976D2;
            }
            .filter-btn.active {
                background: #FF5252;
            }
            .filter-btn.active:hover {
                background: #D32F2F;
            }
            .filter-group {
                display: flex;
                gap: 3px;
                align-items: center;
                flex-wrap: wrap;
            }
            .filter-label {
                font-size: 12px;
                font-weight: bold;
                min-width: 80px;
            }
        `;
        const styleSheet = document.createElement("style");
        styleSheet.textContent = styles;
        if (document.head) {
            document.head.appendChild(styleSheet);
        } else {
            window.addEventListener('load', () => {
                document.head.appendChild(styleSheet);
            });
        }

        // 解析播放基础URL
        const PLAYER_BASE_URL = 'https://vip.vipuuvip.com/?url=';

        // 获取当前页面的唯一标识（如URL）以区分不同页面或剧集列表
        const pageIdentifier = window.location.pathname;

        // 创建过滤模式控制面板（在页面上）
        function createFilterControlPanel() {
            // 移除已存在的控制面板（如果有）
            const existingPanel = document.querySelector('.filter-control-panel');
            if (existingPanel) {
                existingPanel.remove();
            }

            const panel = document.createElement('div');
            panel.className = 'filter-control-panel';

            // TS Type 过滤模式选择
            const tsTypeGroup = document.createElement('div');
            tsTypeGroup.className = 'filter-group';
            const tsTypeLabel = document.createElement('span');
            tsTypeLabel.className = 'filter-label';
            tsTypeLabel.textContent = 'TS 过滤模式:';
            tsTypeGroup.appendChild(tsTypeLabel);

            const tsTypeOptions = [
                { value: 0, text: '递增模式'},
                { value: 1, text: '固定模式' },
                { value: 2, text: '暴力模式' }
            ];

            tsTypeOptions.forEach(opt => {
                const btn = document.createElement('button');
                btn.className = 'filter-btn';
                btn.textContent = opt.text;
                if (ts_type === opt.value) {
                    btn.classList.add('active');
                }
                btn.onclick = function() {
                    ts_type = opt.value;
                    localStorage.setItem('filter_ts_type', opt.value);
                    updateFilterButtons(tsTypeGroup, opt.value);
                    console.log('%c[Video Parse Assistant]', 'font-weight: bold; color: white; background-color: #2196F3; padding: 2px; border-radius: 2px;', 'Updated TS Type to:', opt.value);
                    message.success(`已设置 TS 过滤模式为: ${opt.text}`);
                };
                tsTypeGroup.appendChild(btn);
            });

            // EXT-X Mode 过滤模式选择
            const extXModeGroup = document.createElement('div');
            extXModeGroup.className = 'filter-group';
            const extXModeLabel = document.createElement('span');
            extXModeLabel.className = 'filter-label';
            extXModeLabel.textContent = 'EXT-X 模式:';
            extXModeGroup.appendChild(extXModeLabel);

            const extXModeOptions = [
                { value: 0, text: '保守模式'},
                { value: 1, text: '激进模式'}
            ];

            extXModeOptions.forEach(opt => {
                const btn = document.createElement('button');
                btn.className = 'filter-btn';
                btn.textContent = opt.text;
                if (the_ext_x_mode === opt.value) {
                    btn.classList.add('active');
                }
                btn.onclick = function() {
                    the_ext_x_mode = opt.value;
                    localStorage.setItem('filter_ext_x_mode', opt.value);
                    updateFilterButtons(extXModeGroup, opt.value);
                    console.log('%c[Video Parse Assistant]', 'font-weight: bold; color: white; background-color: #2196F3; padding: 2px; border-radius: 2px;', 'Updated EXT-X Mode to:', opt.value);
                    message.success(`已设置 EXT-X 模式为: ${opt.text}`);
                };
                extXModeGroup.appendChild(btn);
            });

            // Violent Mode 切换
            const violentModeGroup = document.createElement('div');
            violentModeGroup.className = 'filter-group';
            const violentModeLabel = document.createElement('span');
            violentModeLabel.className = 'filter-label';
            violentModeLabel.textContent = '暴力拆解:';
            violentModeGroup.appendChild(violentModeLabel);

            const violentBtn = document.createElement('button');
            violentBtn.className = 'filter-btn';
            violentBtn.textContent = violent_filter_mode_flag ? '开启' : '关闭';
            if (violent_filter_mode_flag) {
                violentBtn.classList.add('active');
            }
            violentBtn.onclick = function() {
                violent_filter_mode_flag = !violent_filter_mode_flag;
                localStorage.setItem('filter_violent_mode', violent_filter_mode_flag);
                violentBtn.textContent = violent_filter_mode_flag ? '开启' : '关闭';
                if (violent_filter_mode_flag) {
                    violentBtn.classList.add('active');
                } else {
                    violentBtn.classList.remove('active');
                }
                console.log('%c[Video Parse Assistant]', 'font-weight: bold; color: white; background-color: #2196F3; padding: 2px; border-radius: 2px;', 'Updated Violent Mode to:', violent_filter_mode_flag);
                message.success(`已设置暴力拆解模式为: ${violent_filter_mode_flag ? '开启' : '关闭'}`);
            };
            violentModeGroup.appendChild(violentBtn);

            panel.appendChild(tsTypeGroup);
            panel.appendChild(extXModeGroup);
            panel.appendChild(violentModeGroup);
            document.body.appendChild(panel);
        }

        // 更新按钮激活状态
        function updateFilterButtons(group, activeValue) {
            const buttons = group.querySelectorAll('.filter-btn');
            buttons.forEach(btn => {
                const btnValue = btn.textContent.includes('模式') ? parseInt(btn.textContent.match(/\d+/)) : (btn.textContent === '开启' ? true : false);
                if (btnValue === activeValue || (typeof activeValue === 'boolean' && btn.textContent === (activeValue ? '开启' : '关闭'))) {
                    btn.classList.add('active');
                } else {
                    btn.classList.remove('active');
                }
            });
        }

        // 在页面加载完成后创建控制面板
        if (document.readyState === 'loading') {
            window.addEventListener('load', createFilterControlPanel);
        } else {
            createFilterControlPanel();
        }

        // 创建播放按钮
        function createPlayButton(link) {
            const btn = document.createElement('span');
            btn.className = 'parse-btn';
            btn.textContent = '解析播放';
            btn.title = '点击解析播放视频';
            // 设置唯一标识，用于区分不同集的按钮
            const linkText = link.textContent;
            const linkId = `${pageIdentifier}_${linkText}`;
            btn.dataset.linkId = linkId;
            
            // 检查是否是之前点击过的集数，如果是则应用激活样式
            const activeLinkId = localStorage.getItem('activeEpisodeLinkId');
            if (activeLinkId === linkId) {
                btn.classList.add('active');
            }

            btn.onclick = function(e) {
                e.preventDefault();
                e.stopPropagation();
                // 更新激活的集数并保存到localStorage
                localStorage.setItem('activeEpisodeLinkId', linkId);
                // 移除所有按钮的激活样式
                const allBtns = document.querySelectorAll('.parse-btn');
                allBtns.forEach(b => b.classList.remove('active'));
                // 为当前按钮添加激活样式
                btn.classList.add('active');
                console.log('%c[Video Parse Assistant]', 'font-weight: bold; color: white; background-color: #2196F3; padding: 2px; border-radius: 2px;', 'Set active episode:', linkId);
                
                // 获取链接文本内容
                const text = link.textContent;
                const dollarIndex = text.indexOf('$');
                if (dollarIndex !== -1) {
                    // 提取视频URL（$后面的部分）
                    let videoUrl = text.substring(dollarIndex + 1).trim();
                    videoUrl = videoUrl.replace(/\s+/g, '');
                    // 提取集数标题（$前面的部分，通常是数字或其他标识）
                    let episodeTitle = text.substring(0, dollarIndex).trim();
                    if (!episodeTitle) {
                        episodeTitle = '当前集数'; // 如果没有标题，默认值
                    }
                    console.log('%c[Video Parse Assistant]', 'font-weight: bold; color: white; background-color: #2196F3; padding: 2px; border-radius: 2px;', 'Opening video:', videoUrl, 'with title:', episodeTitle);
                    if (videoUrl) {
                        // 创建全屏iframe而不是新窗口
                        const iframeContainer = document.createElement('div');
                        iframeContainer.className = 'video-iframe-container';
                        const iframe = document.createElement('iframe');
                        iframe.className = 'video-iframe';
                        iframe.src = PLAYER_BASE_URL + videoUrl;
                        iframe.allowFullscreen = true;
                        const closeBtn = document.createElement('button');
                        closeBtn.className = 'close-iframe-btn';
                        closeBtn.textContent = '关闭';
                        closeBtn.onclick = function() {
                            iframeContainer.remove();
                        };
                        iframeContainer.appendChild(iframe);
                        iframeContainer.appendChild(closeBtn);
                        
                        document.body.appendChild(iframeContainer);
                        
                        // 尝试在iframe加载后注入脚本修改标题
                        iframe.onload = function() {
                            try {
                                const iframeDocument = iframe.contentDocument || iframe.contentWindow.document;
                                if (iframeDocument) {
                                    const script = iframeDocument.createElement('script');
                                    script.textContent = `
                                        window.addEventListener('load', function() {
                                            try {
                                                document.title = "${episodeTitle}";
                                                if (typeof dplayer !== 'undefined' && dplayer) {
                                                    dplayer.notice("标题已更新为：${episodeTitle}", 3000);
                                                    console.log('DPlayer title updated to: ${episodeTitle}');
                                                }
                                            } catch (err) {
                                                console.error('Failed to update title in iframe:', err);
                                            }
                                        });
                                    `;
                                    iframeDocument.head.appendChild(script);
                                    console.log('%c[Video Parse Assistant]', 'font-weight: bold; color: white; background-color: #2196F3; padding: 2px; border-radius: 2px;', 'Injected script into iframe for title:', episodeTitle);
                                }
                            } catch (error) {
                                console.error('%c[Video Parse Assistant]', 'font-weight: bold; color: white; background-color: #FF5252; padding: 2px; border-radius: 2px;', 'Failed to access iframe content for script injection:', error);
                            }
                        };
                    }
                }
            };
            return btn;
        }

        // 查找所有链接
        function processLinks() {
            const links = document.getElementsByTagName('a');
            for (let link of links) {
                if (link.textContent.includes('$') && !link.getAttribute('data-processed')) {
                    link.setAttribute('data-processed', 'true');
                    const playButton = createPlayButton(link);
                    link.parentNode.insertBefore(playButton, link.nextSibling);
                    console.log('%c[Video Parse Assistant]', 'font-weight: bold; color: white; background-color: #2196F3; padding: 2px; border-radius: 2px;', 'Processed link:', link.textContent);
                }
            }
        }

        // 页面加载完成后执行
        if (document.readyState === 'loading') {
            window.addEventListener('load', processLinks);
        } else {
            processLinks();
        }

        // 处理动态加载的内容
        try {
            const observer = new MutationObserver(function(mutations) {
                processLinks();
            });
            if (document.body) {
                observer.observe(document.body, {
                    childList: true,
                    subtree: true
                });
            } else {
                window.addEventListener('load', () => {
                    observer.observe(document.body, {
                        childList: true,
                        subtree: true
                    });
                });
            }
        } catch (error) {
            console.error('%c[Video Parse Assistant]', 'font-weight: bold; color: white; background-color: #FF5252; padding: 2px; border-radius: 2px;', 'Failed to initialize MutationObserver:', error);
        }
    }
})();
