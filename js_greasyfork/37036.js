// ==UserScript==
// @name         火
// @namespace    http://swordman-s1.yytou.com/?id=6108593&time=1512918184949&key=1a847587d4e1580694854d70b566027f&s_line=1&game_skin=2
// @version      1.7
// @description  try to take over the world!
// @author       MARBO&jj
// @match        http://swordman-s1.yytou.com/?id=6108593&time=1512918184949&key=1a847587d4e1580694854d70b566027f&s_line=1&game_skin=2
// @grant        http://swordman-s1.yytou.com/?id=6108593&time=1512918184949&key=1a847587d4e1580694854d70b566027f&s_line=1&game_skin=2
// @downloadURL https://update.greasyfork.org/scripts/37036/%E7%81%AB.user.js
// @updateURL https://update.greasyfork.org/scripts/37036/%E7%81%AB.meta.js
// ==/UserScript==
...
function hideButton(){
	   ...
	   lotusButton.style.visibility="hidden";
...
function showButton(){
	   ...
	   lotusButton.style.visibility="visible";
...
function CheckIn(){ // 進入揚州
    ...
    clickButton('event_1_6640312', 1);  //領每周禮包;  //領每周禮包
...
function QiXiaTalkFunc(){
	...
	delta_Time = 300;
	var QiXiaList_Input= "";
	if(!(QiXiaList_Input = prompt("請輸入奇俠順序（奇俠間用中文分號'；'隔開）","火雲邪神；郭濟；浪喚雨；吳縝；步驚鴻；王蓉；李宇飛；龐統；風行騅；風南；逆風舞；狐蒼雁；護竺"))){
		return;
	}
...
// 刻刀 ------------------------------------------------------------------------------------------------------
//document.body.removeChild(knifeButton);
var knifeButton = document.createElement('button');
knifeButton.innerText = '刻刀';
knifeButton.style.position = 'absolute';
knifeButton.style.right = '0px';
knifeButton.style.top = currentPos + 'px';
currentPos = currentPos + delta;
knifeButton.style.width = buttonWidth;
knifeButton.style.height = buttonHeight;
document.body.appendChild(knifeButton);
knifeButton.addEventListener('click', knife)
function knife(){ //
	console.log('做刻刀！');
	clickButton('jh 1');
	clickButton('go east');
	...
	clickButton('go north');
	clickButton('go west');
	clickButton('event_1_45330709', 1);
	clickButton('home');
}
// 禦寒衣 ------------------------------------------------------------------------------------------------------
//document.body.removeChild(clothesButton);
var clothesButton = document.createElement('button');
clothesButton.innerText = '禦寒衣';
clothesButton.style.position = 'absolute';
clothesButton.style.right = '0px';
clothesButton.style.top = currentPos + 'px';
currentPos = currentPos + delta;
clothesButton.style.width = buttonWidth;
clothesButton.style.height = buttonHeight;
...
function clothes(){ //
	...
	clickButton('go north');
	clickButton('go west');
	clickButton('event_1_58666869', 1);
    clickButton('look_room');
	clickButton('home');
}
// 答題 ------------------------------------------------------------------------------------------------------
...
// 蓮花 ------------------------------------------------------------------------------------------------------
//document.body.removeChild(lotusButton);
var lotusButton = document.createElement('button');
lotusButton.innerText = '蓮花';
lotusButton.style.position = 'absolute';
lotusButton.style.right = '0px';
lotusButton.style.top = currentPos + 'px';
currentPos = currentPos + delta;
lotusButton.style.width = buttonWidth;
lotusButton.style.height = buttonHeight;
document.body.appendChild(lotusButton);
lotusButton.addEventListener('click', lotus)
function lotus(){ //
        clickButton('jh 2');
        clickButton('go north');
		clickButton('go north');
...
function qmj_addbtns() {
    var a = ['山坳', '石街','桃花泉', '潭畔草地', '長空棧道','臨淵石台', '千尺幢', '玉女峰', '猢猻愁', '沙丘小洞', '九老洞', '懸根松', '夕陽嶺', '青雲坪', '湖邊','玉壁瀑布','碧水寒潭','懸崖','寒水潭','戈壁','盧崖瀑布','啟母石','無極老姆洞','山溪畔','奇槐坡','小洞天','天梯','雲步橋','觀景台','危崖前','無名山峽谷','天策','賭場','破陣','鱷魚','挖鐵','杭界山','朱果'];
    var b = [];
...
function wooo(){//朱果
    alert("亂石山2354龍淵刀樓1539桃花渡1789綠水山1259戈壁綠洲2039帝龍陵2389佛門石窟2429大福船3094天龍山3104八卦門3649濱海古城3399");
}
//彈出視窗_1------------------------------------------------------------------------
		var popList1 = {};
...
// 外傳1 ------------------------------------------------------------------------------------------------------
		...
		function d41(){// 歐陽敏
		    go("jh 14;w;n;n;n;e;e;n;n;look_npc tangmen_tangmei;look_npc tangmen_tangmei;look_npc tangmen_tangmei;e;'event_1_8413183;event_1_39383240;e;s;e;n;w;n;n");
		}
		function d42(){// 衛青
