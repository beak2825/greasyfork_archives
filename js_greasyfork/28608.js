// ==UserScript==
// @name           Spacom: пиратская карта
// @version        1.06
// @namespace      Spacom
// @author         Agor71
// @description    Меняем цвет пиратов в зависимости от времени прилёта
// @include        http*://spacom.ru/?act=map*
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/28608/Spacom%3A%20%D0%BF%D0%B8%D1%80%D0%B0%D1%82%D1%81%D0%BA%D0%B0%D1%8F%20%D0%BA%D0%B0%D1%80%D1%82%D0%B0.user.js
// @updateURL https://update.greasyfork.org/scripts/28608/Spacom%3A%20%D0%BF%D0%B8%D1%80%D0%B0%D1%82%D1%81%D0%BA%D0%B0%D1%8F%20%D0%BA%D0%B0%D1%80%D1%82%D0%B0.meta.js
// ==/UserScript==
...
function Star(params) {
    ...
    Star.prototype.drawOn = function (canvas, target) {
        ...
        ;
        if (((target == 'all') || (target == 'text')) && (this.visited == '1')) {
            var color = "#c1c9d0";
            if (this.explored == '0') {
                color = "#6b6d73";
            }
            ...
            if (this.text_state == false) {
                ...
                this.textCanvas = new fabric.Text(this.name, {
                    left: this.x * box_size - 16 - this.name.length * 4,
                    top: this.y * box_size - 48,
                    fill: color,
                    fontSize: "14",
                    selection: false,
                    hasRotatingPoint: false,
...
function Fleet(params, owner) {
    ...
    if (this.turn > "0") {
        ...
        if (typeof this.star_id == "undefined") {
            console.log("!! Action");
            console.log(this);
        }
	...
	//Моды от криворукого Агора
	if (this.owner == "pirate" && this.turn == "1") {
     this.image_color = "rgb(0,146,193)";
}
	if (this.owner == "pirate" && this.turn == "2") {
     this.image_color = "rgb(0,402,255)";    
}
    ...
    Fleet.prototype.drawOn = function (canvas, target) {
        ...
        if (target === undefined)
            target = 'all';
        if (((target == 'all') || (target == 'image')) && ((this.garrison == "0") || (this.weight > "0") )) {
            if (this.image_state == false) {
                this.image_state = true;
                this.image_rendered = false;
                map.queueToRender++;
    ...
    Fleet.prototype.clickTransfer = function () {
            ...
            $.getJSON(APIUrl() + '&act=map&task=fleets&order=transfer_start&fleet_id=' + this.fleet_id + '&format=json', {}, function (json) {
                //console.log(json);
                //console.log(json["explore"]["status"]);
                if (json["transfer"]["status"] == "1") {
                    showSmallMessage("Предложение принять ваш флот на управление передано союзнику");
                    parseAnswer(json, '');
                } else {
...
//$("#ships div.ships").customScrollbar("remove");
        ...
        if (json["fleets"]["fleet"]["ships"]) {
            ...
            $.each(json["fleets"]["fleet"]["ships"], function (index) {
                ...
                //fleet.owner = 'own';
                this.bonus = "<b>" + this.name + "</b><br/><br/>";
                if (parseInt(this.hp_max) > parseInt(this.hp)) {
                    this.bonus = this.bonus + "Прочность:  " + parseInt(this.hp) + " из " + this.hp_max + "<br />";
                } else {
                    this.bonus = this.bonus + "Прочность:  " + this.hp_max + "<br />";
                }
                this.bonus = this.bonus + "Скорость:  " + this.speed + "<br />";
                this.bonus = this.bonus + "Дальность:  " + this.distance + "<br />";
                if (this.lazer_defence > 0) {
                    this.bonus = this.bonus + "Защита от лазеров:  " + this.lazer_defence + "%<br />";
                }
                if (this.rocket_defence > 0) {
                    this.bonus = this.bonus + "Защита от ракет:  " + this.rocket_defence + "<br />";
                }
                if (this.cannon_defence > 0) {
                    this.bonus = this.bonus + "Защита от пушек:  " + this.cannon_defence + "<br />";
                }
                //this.bonus = this.bonus + "<br/>";
                if (this.lazer_power > 0) {
                    this.bonus = this.bonus + "Мощь лазера:  " + this.lazer_power + "<br />" + "Выстрелов:  " + this.lazer_shots + "<br />";
                }
                if (this.rocket_power > 0) {
                    this.bonus = this.bonus + "Мощь ракеты:  " + this.rocket_power + "<br />" + "Точность:  " + this.rocket_accuracy + "<br />";
                }
                if (this.cannon_power > 0) {
                    this.bonus = this.bonus + "Мощь пушек:  " + this.cannon_power + "<br />" + "Целей:  " + this.cannon_targets + "<br />";
                }
            ...
            $("#ships_info").after("<div class='player_fleet_title'>В составе этого флота нет кораблей</div>");
...
var setScale = function (scaleToSet, anchorX, anchorY) {
    ...
    $("#map_container").css("background-size", (99.25 + 0.75 * current_scale) + "% auto");
    map.renewMap();
