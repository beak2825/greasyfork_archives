// ==UserScript==
// @name         TORN: Ghost Bank - LemonWedge
// @namespace    dekleinekobini.private.ghostbank
// @version      1.3.0
// @author       DeKleineKobini [2114440]
// @description  Make trades easier to use as a bank. Modified for LemonWedge [2884245].
// @icon         https://www.google.com/s2/favicons?sz=64&domain=torn.com
// @match        https://www.torn.com/trade.php*
// @grant        GM_addStyle
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/503553/TORN%3A%20Ghost%20Bank%20-%20LemonWedge.user.js
// @updateURL https://update.greasyfork.org/scripts/503553/TORN%3A%20Ghost%20Bank%20-%20LemonWedge.meta.js
// ==/UserScript==
...
(function () {
  ...
  function displayButtons() {
    ...
    gbAddButton.addEventListener("click", (event) => {
      event.preventDefault();
      onButtonClickModifyMoney(gbAddButton, gbAddInput, (currentAmount, input) => currentAmount + input);
    });
    gbAddButton.textContent = "Add";
    gbButtons2.appendChild(gbAddButton);
    const gbRemoveButton = document.createElement("button");
    gbRemoveButton.classList.add(styles.gbButton);
    gbRemoveButton.addEventListener("click", (event) => {
      event.preventDefault();
      onButtonClickModifyMoney(gbRemoveButton, gbAddInput, (currentAmount, input) => currentAmount - input);
    });
    gbRemoveButton.textContent = "Remove";
    gbButtons2.appendChild(gbRemoveButton);
    tradeContentElement.appendChild(gbButtons2);
  }
  function onButtonClickModifyMoney(button, input, modifier) {
    const currentAmount = getTradeAmount();
    const inputAmount = moneyTextToNumber(input.value);
    if (inputAmount <= 0) return;
    const newAmount = modifier(currentAmount, inputAmount);
    button.setAttribute("disabled", "");
    setMoneyValue(newAmount).then((response) => {
      button.removeAttribute("disabled");
      updateTradeUI(response);
    }).catch((cause) => console.error("[GB] Some error occurred when updating the trade amount.", cause));
    setTimeout(() => {
      if (button.checkVisibility() && button.hasAttribute("disabled")) {
        button.removeAttribute("disabled");
      }
    }, 1e3);
  }
