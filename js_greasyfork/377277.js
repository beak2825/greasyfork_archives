// ==UserScript==
// @name        EvadesBox
// @description Evades.io Multibox Extension
// @version     1.2
// @author      Rocket (RocketComet)
// @include     http://evades.io/*
// @include     https://evades.io/*
// @connect     evades.io
// @grant       GM_getValue
// @grant       GM_setValue
// @run-at      document-start
// @namespace https://greasyfork.org/users/242165
// @downloadURL https://update.greasyfork.org/scripts/377277/EvadesBox.user.js
// @updateURL https://update.greasyfork.org/scripts/377277/EvadesBox.meta.js
// ==/UserScript==
...
var const_SC = getScreenConstant();
    ...
    function SyncMouse()
    {
        ...
        {
            holdingMouse[0] = button0;simulateMousePress(0, cX, cY, button0);
        }

        if(holdingMouse[1] != button1)
        {
            holdingMouse[1] = button1; simulateMousePress(1, cX, cY, button1);
        }

        if(holdingMouse[2] != button2)
        {
            holdingMouse[2] = button2; simulateMousePress(2, cX, cY, button2);
        }
	}
    ...
    function SyncKeys()
    {
        ...
        {
            holdingKey[37] = left;simulateKeyPress(37, left);
        }
        if(holdingKey[38] != up)
        {
            holdingKey[38] = up;simulateKeyPress(38, up);
        }
        if(holdingKey[39] != right)
        {
            holdingKey[39] = right; simulateKeyPress(39, right);
        }
        if(holdingKey[40] != down)
        {
            holdingKey[40] = down;simulateKeyPress(40, down);
        }

        if(holdingKey[49] != num_1)
        {
            holdingKey[49] = num_1;simulateKeyPress(49, num_1);
        }
        if(holdingKey[50] != num_2)
        {
            holdingKey[50] = num_2;simulateKeyPress(50, num_2);
        }
        if(holdingKey[51] != num_3)
        {
            holdingKey[51] = num_3;simulateKeyPress(51, num_3);
        }
        if(holdingKey[52] != num_4)
        {
            holdingKey[52] = num_4;simulateKeyPress(52, num_4);
        }
        if(holdingKey[53] != num_5)
        {
            holdingKey[53] = num_5;simulateKeyPress(53, num_5);
        }
        if(holdingKey[54] != num_6)
        {
            holdingKey[54] = num_6; simulateKeyPress(54, num_6);
        }
        if(holdingKey[55] != num_7)
        {
            holdingKey[55] = num_7;  simulateKeyPress(55, num_7);
        }
        if(holdingKey[56] != num_8)
        {
            holdingKey[56] = num_8; simulateKeyPress(56, num_8);
        }

        if(holdingKey[16] != shift)
        {
            holdingKey[16] = shift; simulateKeyPress(16, shift);
        }
        if(holdingKey[13] != enter)
        {
            holdingKey[13] = enter;simulateKeyPress(13, enter);
        }
	}
