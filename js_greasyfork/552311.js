...
// @downloadURL https://update.greasyfork.org/scripts/552311/Walla%20Walla.user.js
// @updateURL https://update.greasyfork.org/scripts/552311/Walla%20Walla.meta.js
// ==/UserScript==
var tertWars = {};
...
function updateUserList() {
    ...
    membersDOMs.each(function() {
        ...
        const player = el.find('[class*="honorWrap_"] [class*="linkWrap__"]');
        ...
        if (player.find('[class*="honorName__"]').length) {
            name = player.find('[class*="honorName__"]').text();
        }
        if (player.find('.honor-text').not('.honor-text-svg').length) {
            name = player.find('.honor-text').not('.honor-text-svg').text();
        }
