...
// @grant       GM_getValue
// @grant       GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/12145/mR2hao.user.js
// @updateURL https://update.greasyfork.org/scripts/12145/mR2hao.meta.js
// ==/UserScript==
...
var $b = function(selector, target, func) {
  var result = document.querySelectorAll(selector);
  for (var i = 0; i < result.length; i++) {
...
var create_wrapper = function(data) {
  ...
  var from_info = $c({
    tag: 'h4',
    prop: {
      innerHTML: '来自<a id= \'magnet-href\' href=\'' + data.src + '\' target=\'_blank\' style=\'color: #FF10FF;\'>' + current_search.get().name + '</a>搜索结果:',
    }
  });
  if (data.info.length) {
...
var run = function() {
  for (var key in main) {
    if (location.href.match(main[key].re)) {
      if (main[key].vid) {
        add_style();
