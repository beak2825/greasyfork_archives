...
// @include  *oneplus.net*
// @grant  none
// @downloadURL https://update.greasyfork.org/scripts/5914/Text%20Rainbowifier.user.js
// @updateURL https://update.greasyfork.org/scripts/5914/Text%20Rainbowifier.meta.js
// ==/UserScript==
...
function rainbow(event) {
    if (document.getElementsByClassName('redactor_textCtrl redactor_MessageEditor redactor_BbCodeWysiwygEditor redactor_NoAutoComplete')[0]) {
        ...
        var message = "<span>"+iframe.contentWindow.document.getElementsByTagName('body')[0].innerHTML+"</span>";
    if (message.indexOf("http") == -1 && message.indexOf("www") == -1 && message.indexOf("@") == -1 && message.indexOf("QUOTE") == -1 && message.indexOf("<font color") == -1) {
        iframe.contentWindow.document.getElementsByTagName('body')[0].innerHTML = MakeSFX(message,true);
    }
}
