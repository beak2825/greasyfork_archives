// ==UserScript==
// @name         Geo-Bundesliga-Hintergrund (Game Master) - auch für Better Moving Duels
// @namespace    http://tampermonkey.net/
// @version      0.0.5
// @description  Game Master-Skript mit Geo-Bundesliga-Hintergrund. ACHTUNG: Für die Geo-Bundesliga-Playoffs bitte in den Einstellungen vom Skript "First to 5" einstellen, und Tie Range "Disabled". Beim Wechseln zwischen Better Moving Duels und normalen Duels kann es passieren, dass die Scores nicht richtig angezeigt werden. In diesem Fall muss die Seite einmal am Anfang des Duels kurz neugeladen werden. Credits to Blinky for the original version of this adapted Pinpointing Duels script.

// @match        https://www.geoguessr.com/*
// @icon         https://www.geobundesliga.com/icon.png
// @grant        GM_registerMenuCommand
// @grant        GM_setValue
// @grant        GM_getValue
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/540750/Geo-Bundesliga-Hintergrund%20%28Game%20Master%29%20-%20auch%20f%C3%BCr%20Better%20Moving%20Duels.user.js
// @updateURL https://update.greasyfork.org/scripts/540750/Geo-Bundesliga-Hintergrund%20%28Game%20Master%29%20-%20auch%20f%C3%BCr%20Better%20Moving%20Duels.meta.js
// ==/UserScript==
...
function showBonusDisplay(team) {
  ...
  const customStyles = `
        ...
        .views_activeRoundWrapper__1_J5M {
            background-image: url('https://i.imgur.com/mYbD4GN.jpeg') !important;
  ...
  const modifyHealthBars = () => {
    ...
    document
      .querySelectorAll('[class*="wc-health-bar_container__zK0hz"]')
      .forEach((bar) => (bar.style.visibility = "hidden"));
    document
      .querySelectorAll(".views_roundMultiplier__iQZZW")
  ...
  const createScoreDisplays = () => {
    ...
    updateScoreBoxPosition(
      GM_getValue("scoreBoxOffset", 30),
      GM_getValue("scoreBoxTop", 86)
    );
  };
  ...
  const showEndScreen = () => {
    ...
    overlay.style.cssText = `
        ...
        `;
    const winnerText =
      leftScore >= winThreshold
        ? "BLAU gewinnt das Duel!"
        : "ROT gewinnt das Duel!";
    ...
    messageElem.innerText =
      "Die Spieler klicken die restlichen Runden einmal kurz durch.";
  ...
  const flashScore = (scoreElement) => {
    ...
    const flashBackground =
      "linear-gradient(180deg,rgba(90,219,149,1),rgba(60,200,110,1) 95%),#5adb95";
    let flashCount = 0;
    const interval = setInterval(() => {
      // Toggle the background on odd/even counts.
      scoreElement.style.background =
        flashCount % 2 === 0 ? flashBackground : originalBackground;
      flashCount++;
      if (flashCount >= 10) {
        // 10 toggles * 0.5s = 5 seconds total.
  ...
  const updateScores = (response) => {
    console.log("innital health is" + response.options.initialHealth);
    if (
      response.options.initialHealth == 1000000 &&
      response.options.maxNumberOfRounds % 2 == 1 &&
      response.options.disableHealing == true &&
      response.options.multiplierIncrement == 0 &&
      !response.movementOptions.forbidMoving &&
      !response.movementOptions.forbidRotating &&
      !response.movementOptions.forbidZooming
    ) {
      isBMD = true;
      console.log("BMD is set to true");
    } else {
      isBMD = false;
      console.log("BMD is set to false");
    }
    ...
    for (
      let i = 0;
      i < response.currentRoundNumber &&
      newTeam0Score < winThreshold &&
      newTeam1Score < winThreshold;
      i++
    ) {
      ...
      let roundStartTime =
        new Date(response.rounds[i]?.startTime) / 1000 || Infinity;
      let roundEndTime =
        new Date(response.rounds[i]?.endTime) / 1000 || Infinity;
      let team0RoundScore = 0,
        team1RoundScore = 0;
      let team0Time = Infinity,
        team1Time = Infinity;
      let team0BestScore = 0,
        team1BestScore = 0;

      for (let j = 0; j < response.teams[0]?.players.length; j++) {
        let currentGuess;
        response.teams[0].players[j].guesses.forEach((guess) => {
          if (guess.roundNumber == i + 1) currentGuess = guess;
        });
      ...
      for (let j = 0; j < response.teams[1]?.players.length; j++) {
        let currentGuess;
        response.teams[1].players[j].guesses.forEach((guess) => {
          if (guess.roundNumber == i + 1) currentGuess = guess;
        });
        if (currentGuess === undefined) continue;
        let guessTime = new Date(currentGuess.created) / 1000 || Infinity;
      ...
      let message = "",
        tieMessage = "",
        winMessage = "";
      team0Score = newTeam0Score;
      team1Score = newTeam1Score;
      const highestScore =
        team0RoundScore > team1RoundScore ? team0RoundScore : team1RoundScore;
      if (tieRange > 0)
        tieDistance = Math.floor((5000 - highestScore) / tieRange);
      ...
      if (team0RoundScore === 5000 && team1RoundScore === 5000) {
        ...
        message = `Punkt für schnellsten 5k`;
      } else if (
        team0RoundScore > team1RoundScore + tieDistance ||
        team1RoundScore > team0RoundScore + tieDistance
      ) {
        if (team0RoundScore > team1RoundScore + tieDistance) newTeam0Score++;
        else newTeam1Score++;
        message = `Punkt für näheren Guess`;
        if (tieRange > 0)
          tieMessage = `<br><span style="font-size: 14px;">TIE RANGE: ${tieDistance} POINTS</span>`;
      } else {
        message = `UNENTSCHIEDEN!`;
        if (tieRange > 0)
          tieMessage = `<br><span style="font-size: 14px;">TIE RANGE: ${tieDistance} POINTS</span>`;
      }
      if (message !== `UNENTSCHIEDEN!`)
        winMessage =
          newTeam0Score > team0Score
            ? "BLAU gewinnt die Runde! <br>"
            : "ROT gewinnt die Runde! <br>";
      const isMatchPoint =
        newTeam0Score > winThreshold - 2 || newTeam1Score > winThreshold - 2;
      showResultDisplay(
        `${winMessage} ${message} ${
          isMatchPoint ? "<br>MATCHPUNKT!" : ""
        } ${tieMessage}`
      );
    }
    ...
    const isMatchPoint =
      leftScore > winThreshold - 2 || rightScore > winThreshold - 2;
    ...
    if (
      !gameOver &&
      (leftScore >= winThreshold || rightScore >= winThreshold)
    ) {
      gameOver = true;
      showEndScreen();
    }
  ...
  const updateActiveRoundBackground = () => {
    let bgUrl = "https://i.imgur.com/mYbD4GN.jpeg"; // default background
    // If game is over, use default background.
    if (leftScore >= winThreshold || rightScore >= winThreshold) {
      bgUrl = "https://i.imgur.com/mYbD4GN.jpeg";
    }
    // Both players at match point.
    else if (
      leftScore === winThreshold - 1 &&
      rightScore === winThreshold - 1
    ) {
    ...
    const activeRoundElem = document.querySelector(
      ".views_activeRoundWrapper__1_J5M"
    );
    if (activeRoundElem) {
      activeRoundElem.style.setProperty(
        "background-image",
        `url('${bgUrl}')`,
        "important"
      );
  ...
  const fetchDuelData = () => {
    ...
    currentDuel = duelId;
    fetch(`https://game-server.geoguessr.com/api/duels/${duelId}/spectate`, {
      method: "GET",
      credentials: "include",
    })
      .then((res) => res.json())
