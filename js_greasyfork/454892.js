...
// @grant        none
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/454892/ich-demo.user.js
// @updateURL https://update.greasyfork.org/scripts/454892/ich-demo.meta.js
// ==/UserScript==
(function() {
    'use strict';

    // Stage 1 Объявили переменные
    var name_courier = '';
    // Stage 2 Ищем ключевой блок
    let i = setInterval(function() {
        if (document.querySelector(".popover__opener")){
            ...
            if (name_courier == user_name.innerHTML) {
                console.log("кажеца одно и тоже пока")
                var dv_active_has = document.getElementById('dv-active-order');
                if (dv_active_has.innerHTML == "") {
                    write_blocks(); console.log("Не вижу, чтобы блок с активными заказами был полон")
                } else {
                    return;
                }
            } else {
                console.log("Не похоже короч пытаемся запустить функцию")
                write_blocks();
            }
