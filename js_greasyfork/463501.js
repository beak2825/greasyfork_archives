// ==UserScript==
// @name         Random Player Profile Loader
// @namespace    WeekendTechies.randomplayerprofile
// @version      1.0.0
// @description  Creates a link in sidebar to a random player on Torn.
// @author       WeekendTechie
// @match        https://www.torn.com/*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/463501/Random%20Player%20Profile%20Loader.user.js
// @updateURL https://update.greasyfork.org/scripts/463501/Random%20Player%20Profile%20Loader.meta.js
// ==/UserScript==
'use strict';

const linksarray = ["profiles.php?XID="];

const travellinksarray = ["profiles.php?XID="];
        ...
        if (document.contains(document.getElementById("top-page-links-list")) && !url.includes("laptop.php") && !url.includes("forums.php")) {
          ...
          if (url.includes("profiles.php") || url.includes("credits.php") || url.includes("rules.php")) {
              if (document.contains(document.getElementById("skip-to-content"))) {
                  console.log("Random Player Script: Found Other Header!");
                  observer.disconnect();
...
if (url.includes("laptop.php") || url.includes("forums.php")) {
    if ($('#body').data('traveling') == true) {
        $( document ).ajaxComplete(function() {
            if (document.getElementById("playerrandom")) {}else{
                if (document.getElementById("top-page-links-list")) {
                    console.log("Random Player Script Inserted");
                    insertTravel(document.getElementById("top-page-links-list"));
                }
            }
        });
...
function insertTravel(travelHeader) {
...
function insertNormal() {
    ...
    let playerspans = `
     ...
     <span class="linkName___1Q0vN">Random Player</span>
    `;
