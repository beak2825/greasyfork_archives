// ==UserScript==
// @name         Bonk Commands Helloim
// @namespace    sla_bonk_command_helloim
// @version      1.1
// @description  Adds lots of commands to bonk.io. Type /? or /help in bonk chat to get started. helloim kkkkkj
// @author       LEGENDBOSS123 + left paren + mastery3 + Helloim0_0
// @match        https://bonk.io/*
...
// @grant        none
// @unwrap
// @downloadURL https://update.greasyfork.org/scripts/521032/Bonk%20Commands%20Helloim.user.js
// @updateURL https://update.greasyfork.org/scripts/521032/Bonk%20Commands%20Helloim.meta.js
// ==/UserScript==
...
scope.commandhandle = function(chat_val){
    ...
    else if (chat_val.substring(1,3)=="xp"){
        xp = !xp;
        displayInChat("O comando do xp foi " + (xp ? "" : "des") + "ativado.");
        return "";
    }
    else if (chat_val.substring(1,7)=="ready "){
        var args = chat_val.substring(7).split(" ");
        if(args.length < 2) return displayInChat("Insufficient number of arguments.","#DA0808","#1EBCC1");
        if(typeof Number(args[0]) != "number" && Number(args[0]) % 1 != 0) return displayInChat("The number of clicks is not an integer.","#DA0808","#1EBCC1");
        if(typeof Number(args[0]) != "number") return displayInChat("The duration is not a number.","#DA0808","#1EBCC1");
        displayInChat("Clicking the ready button " + args[0] + " times for " + args[1] + " seconds.","#DA0808","#1EBCC1");
        for (let i = 0; i < args[0]; i++) {
            setTimeout(function() {
                SEND('42[16,{"ready":true}]');
            }, (args[1] * 1000) / args[0] * i);
        }
        setTimeout(function() {
            SEND('42[16,{"ready":false}]');
        }, args[1] * 1000 + 500);
        return "";
    }
    else if(ishost){
