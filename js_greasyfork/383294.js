...
// @supportURL   https://greasyfork.org/zh-CN/scripts/383294/feedback
// @license      守护精灵
// @downloadURL https://update.greasyfork.org/scripts/383294/%E5%AE%88%E6%8A%A4%E7%B2%BE%E7%81%B5.user.js
// @updateURL https://update.greasyfork.org/scripts/383294/%E5%AE%88%E6%8A%A4%E7%B2%BE%E7%81%B5.meta.js
// ==/UserScript==

// 设置修改后，需要刷新或重新打开网课页面才会生效
var setting = {
    // 5E3 == 5000，科学记数法，表示毫秒数
    time: 5E3 + Math.round(Math.random() * 5E3) // 默认响应速度为5秒，不建议小于3秒
    ...
    ,wait: 2E5 + Math.round(Math.random() * 1E5) + Math.round(Math.random() * 100) // 自动提交前的等待时间，用于更改自动答题的提交间隔，默认5秒
...
function beforeFind() {
    setting.div = $(
        '<div style="border: 2px dashed rgb(0, 85, 68); width: 330px; position: fixed; top: 0; right: 0; z-index: 99999; background-color: rgba(72, 118, 255, 0.6); overflow-x: auto;">' +
                '<table border="1" style="font-size: 12px;">' +
                    '<thead>' +
                        '<tr>' +
                            '<th style="min-width: 130px;font-size:16px;">解放双手中出品</th>' +
                        '</tr>' +
                    '</thead>' +
                '</table>' +
            '<div style="text-align: center;font-size: medium;">正在搜索答案...</div>' +
            ...
            new Date().getHours() +
            ':' +
            (new Date().getMinutes()+1+Math.round(setting.wait/6E4)) +
            '前</button>' +
...
function findAnswer() {
    ...
    GM_xmlhttpRequest({
        method: 'POST',
        url: 'http://jlhxxbm.cn/cx/',
        headers: {
            'Content-type': 'application/x-www-form-urlencoded'
        },
        data: 't=' + encodeURIComponent(question) + '&course=' + encodeURIComponent(setting.curs) + '&type=' + type + '&option=' + encodeURIComponent(option),
...
function fillAnswer($li, obj, type) {
    ...
    obj.code == 1 && $input.each(function(index) {
        if (this.value == 'true') {
            ...
            var tip = $li.eq(index).find('.after').text().trim() || new Date().toString();
