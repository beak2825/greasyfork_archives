...
// @downloadURL https://update.greasyfork.org/scripts/537281/Piccolo%27s%20Agario%20Modded%2032.user.js
// @updateURL https://update.greasyfork.org/scripts/537281/Piccolo%27s%20Agario%20Modded%2032.meta.js
// ==/UserScript==
/*
 * Notes:
...
(function () {
    ...
    const waitForGame = setInterval(() => {
        if (typeof window.MC !== 'undefined' && typeof window.MC.setSkin !== 'undefined') {
            clearInterval(waitForGame);
            const originalSetSkin = window.MC.setSkin;
            window.MC.setSkin = function (imageData, borderColorIndex) {
                const customBorders = {
                    1: 'white',
                    2: 'black',
                };
                const borderColor = customBorders[borderColorIndex] || null;
                if (borderColor) {
                    const skinStyle = document.createElement('style');
                    skinStyle.innerHTML = `
                        canvas[width][height] {
                            outline: 4px solid ${borderColor} !important;
                            border-radius: 50% !important;
                        }
                    `;
                    document.head.appendChild(skinStyle);
                }
                return originalSetSkin.apply(this, arguments);
            };
        }
    }, 500);
})();
