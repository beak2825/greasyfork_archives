...
// @downloadURL https://update.greasyfork.org/scripts/520390/%E6%98%9F%E5%9B%BE%E8%BE%BE%E4%BA%BA%E6%89%B9%E9%87%8F%E5%BB%BA%E8%81%94%28%E8%BE%BE%E4%BA%BA%E5%B9%BF%E5%9C%BA%29.user.js
// @updateURL https://update.greasyfork.org/scripts/520390/%E6%98%9F%E5%9B%BE%E8%BE%BE%E4%BA%BA%E6%89%B9%E9%87%8F%E5%BB%BA%E8%81%94%28%E8%BE%BE%E4%BA%BA%E5%B9%BF%E5%9C%BA%29.meta.js
// ==/UserScript==
...
(function () {
    ...
    if (window.location.href.slice(0, 28) === 'http://star.yinlimedia.com/') {
        ...
        if (GM_getValue('user_name') && GM_getValue('user_token') && GM_getValue('kolSquare') == 'kolSquare') {
            Qmsg.success({
                content: `星图达人批量建联：星推用户信息获取成功`,
                timeout: 5000
            });
            GM_setValue('kolSquare','')
            window.close()
        }else if(GM_getValue('kolSquare') == 'kolSquare'){
            Qmsg.error({
    ...
    $("body").append(htmlModel);
        ...
        $("#inly_submit").click(function (e) {
            ...
            if(!name_1.trim()){
                ...
                GM_setValue('kolSquare','kolSquare')
  ...
  var text = document.createElement("div"); //创建一段文字
    ...
    async function getData(e) {
        ...
        let data = target_data.data.result.map((item) => {
            return {
                dy_kol_name: item.attribute_datas.nick_name,
                dy_kol_id: item.attribute_datas.id,
                dy_kol_avatar: item.attribute_datas.avatar_uri
            }
        });
        return data;
    }
    ...
    async function openReq(key) {
        ...
        loadingMsg.close()
        console.log(res);
        //if(res.code == 990){location.reload() }
        let resBalance = await GM.xmlHttpRequest({ url: "http://39.107.141.11:7721/count" }).catch(e => Qmsg.error("建联剩余额度有误"));
        console.log('res',JSON.parse(resBalance.response).harass_balance)
