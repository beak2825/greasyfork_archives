// ==UserScript==
// @name         FreeNem.com (Pescador de Cripto)
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/410979/FreeNemcom%20%28Pescador%20de%20Cripto%29.user.js
// @updateURL https://update.greasyfork.org/scripts/410979/FreeNemcom%20%28Pescador%20de%20Cripto%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    function freenem_com(){
        setInterval(function(){
            if ($('.roll-wrapper').find('.roll-button').is(':visible')) {
                ...
                if($('.free').find('.result').text().indexOf('0') > -1){
                    ...
                    $.ajax({
                        url:'https://freenem.com/settings',
                        type:'GET',
        ...
        if (window.location.href.indexOf("freenem.com/free") > -1) {
            setInterval(function(){
                window.history.go(0);
            },300000);
        }
    ...
    function open(){
        if (window.location.href.indexOf("pescadordecripto.com/dashboard") > -1) {
            window.open("https://freenem.com/free", "_blank","width=10,height=10,left=-3000,top=-3000");
        }
    ...
    if (window.location.href.indexOf("pescadordecripto.com/install") > -1) {
        document.getElementById('freenem-com').classList.add("faucet-active");
    }
})();