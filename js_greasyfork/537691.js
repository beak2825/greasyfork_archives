// ==UserScript==
// @name        Showdown - Traducción al español(ian y nacho)
// @match       http://play.pokemonshowdown.com/*
// @match       http://replay.pokemonshowdown.com/*
// @match       https://play.pokemonshowdown.com/*
// @match       https://replay.pokemonshowdown.com/*
// @version     1.0
// @description Traducción completa y corregida de Pokémon Showdown al castellano
// @author      ian 55
// @require     http://code.jquery.com/jquery-latest.min.js
// @run-at      document-end
// @namespace   https://greasyfork.org/users/54
// @downloadURL https://update.greasyfork.org/scripts/537691/Showdown%20-%20Traducci%C3%B3n%20al%20espa%C3%B1ol%28ian%20y%20nacho%29.user.js
// @updateURL https://update.greasyfork.org/scripts/537691/Showdown%20-%20Traducci%C3%B3n%20al%20espa%C3%B1ol%28ian%20y%20nacho%29.meta.js
// ==/UserScript==
...
var regex_ability = /Ability: ([A-Za-z\- ]+)$/;
var regex_abi_and_item = /Ability: ([A-Za-z\- ]+) \/ Item: ([A-Za-z\- ()]+)/;
var regex_possible_ability = /Possible abilities: ([A-Za-z\- ]+)$/;
var regex_possible_ability2 = /Possible abilities: ([A-Za-z\- ]+), ([A-Za-z\- ]+)$/;
var regex_possible_ability3 = /Possible abilities: ([A-Za-z\- ]+), ([A-Za-z\- ]+), ([A-Za-z\- ]+)$/;
var regex_Item = /Item: ([A-Za-z\- ()]+)/;
var regex_attack = /([A-Za-z ]+) will use ([A-Za-z\- ]+)/;

// === FUNCIONES DE TRADUCCIÓN ===

function translateText(txt) {
    if (translations[txt] !== undefined) return translations[txt];
    if (AbilitiesDico[txt] !== undefined) return AbilitiesDico[txt];
    return txt;
}

function translateTextByMatch(originalStr) {
    let match;
    if ((match = originalStr.match(regex_ability))) return "Habilidad: " + translateText(match[1]);
    if ((match = originalStr.match(regex_possible_ability))) return "Posibles habilidades: " + translateText(match[1]);
    if ((match = originalStr.match(regex_possible_ability2))) return "Posibles habilidades: " + translateText(match[1]) + ", " + translateText(match[2]);
    if ((match = originalStr.match(regex_possible_ability3))) return "Posibles habilidades: " + translateText(match[1]) + ", " + translateText(match[2]) + ", " + translateText(match[3]);
    if ((match = originalStr.match(regex_Item))) return "Objeto: " + translateText(match[1]);
    if ((match = originalStr.match(regex_attack))) return match[1] + " usará " + translateText(match[2]);

    return translateText(originalStr).replace(/ lost ([0-9]+)% of its health!/, " perdió $1% de salud.");
}

// === FUNCIÓN DE TRADUCCIÓN DEL DOM ===

function translateElement(element) {
    var elTW = document.createTreeWalker(element, NodeFilter.SHOW_TEXT, null, false);
    var node;
    while ((node = elTW.nextNode())) {
        if (node.nodeType === Node.TEXT_NODE && node.parentNode && node.parentNode.getAttribute("class") !== "textbox") {
            node.nodeValue = translateTextByMatch(node.nodeValue);
        }
    }
}
