...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/552368/GMGN%20%E5%89%8D%E6%8E%92%E7%BB%9F%E8%AE%A1%E6%B5%8B%E8%AF%95%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/552368/GMGN%20%E5%89%8D%E6%8E%92%E7%BB%9F%E8%AE%A1%E6%B5%8B%E8%AF%95%E7%89%88.meta.js
// ==/UserScript==
...
`;
    ...
    const ApiInterceptor = {
            ...
            try {
                ...
                if (isHoldersTarget) {
                    ...
                    if (match && match[2]) {
                        ...
                        // 新增：提取到API参数后，主动获取K线数据
                        console.log('[加仓功能] 准备主动获取K线数据...');
                        setTimeout(() => {
                            this.fetchKlineData();
                        }, 500); // 延迟500ms确保参数已设置
                    }
                ...
                // 注意：移除了K线API的拦截逻辑，改为主动获取
                return isHoldersTarget;
            } catch (err) {
        ...
        async fetchKlineData() {
            try {
                const { currentCAAddress, currentChain, apiParams, klineDataFetched } = GlobalState;

                // 检查是否已获取过K线数据
                if (klineDataFetched) {
                    console.log('[K线获取] K线数据已获取，跳过重复请求');
                    return;
                }

                // 检查必要参数
                if (!currentCAAddress) {
                    console.warn('[K线获取] 缺少代币地址');
                    return;
                }

                if (!currentChain) {
                    console.warn('[K线获取] 缺少链网络信息');
                    return;
                }

                if (!apiParams) {
                    console.warn('[K线获取] 缺少API参数，等待参数提取...');
                    return;
                }

                console.log('[K线获取] ========== 开始获取K线数据 ==========');
                console.log('[K线获取] 代币地址:', currentCAAddress);
                console.log('[K线获取] 链网络:', currentChain);
                console.log('[K线获取] API参数:', apiParams);
                ...
                // 构建完整URL
                const url = `https://gmgn.ai/api/v1/token_mcap_candles/${currentChain}/${currentCAAddress}?${params.toString()}`;

                console.log('[K线获取] 请求URL:', url);
                ...
                console.log('[K线获取] 响应状态:', response.status, response.statusText);

                if (!response.ok) {
                    console.error('[K线获取] 请求失败，HTTP状态:', response.status);
                    return;
                }

                const data = await response.json();
                console.log('[K线获取] ✅ 成功获取K线数据');
                console.log('[K线获取] 数据结构:', {
                    hasData: !!data.data,
                ...
                if (data && data.data && data.data.list) {
                    console.log('[K线获取] 开始处理K线数据...');
                    this.processKlineData(data);

                    // 标记K线数据已获取，避免重复请求
                    GlobalState.klineDataFetched = true;
                    console.log('[K线获取] 已标记K线数据为已获取状态');
                } else {
                    console.warn('[K线获取] K线数据格式异常');
                }

                console.log('[K线获取] ========== K线数据获取完成 ==========');

            } catch (err) {
                console.error('[K线获取] ❌ 获取K线数据失败:', err);
                console.error('[K线获取] 错误堆栈:', err.stack);
            }
        },
        ...
        processKlineData(data) {
            try {
                console.log('[K线处理] ========== 开始处理K线数据 ==========');

                if (!data || !data.data || !data.data.list || data.data.list.length === 0) {
                    console.warn('[K线处理] K线数据为空');
                    return;
                }

                const klineList = data.data.list;
                console.log(`[K线处理] K线数据条数: ${klineList.length}`);
                console.log('[K线处理] 第一条K线:', klineList[0]);
                console.log('[K线处理] 最后一条K线:', klineList[klineList.length - 1]);
                ...
                console.log(`[K线处理] ATH价格: ${athPrice}, 时间: ${new Date(athTimestamp).toLocaleString()}`);

                // 检测回调60%
                this.detectRebound60(klineList, athPrice, athTimestamp);

                console.log('[K线处理] ========== K线数据处理完成 ==========');

            } catch (err) {
                console.error('[K线处理] ❌ 处理K线数据失败:', err);
                console.error('[K线处理] 错误堆栈:', err.stack);
            }
        },
    ...
    function calculateStats() {
        ...
        if (GlobalState.cache.lastDataHash === currentHash &&
            ...
            // 缓存命中，但需要更新addedPosition（因为它独立于持有者数据）
            const cachedStats = GlobalState.cache.calculatedStats;
            cachedStats.addedPosition = GlobalState.addedPositionAddresses.length;
            return cachedStats;
        }
        const currentTime = Math.floor(Date.now() / 1000);
