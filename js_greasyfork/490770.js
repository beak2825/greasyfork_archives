...
// @downloadURL https://update.greasyfork.org/scripts/490770/ICEbot%20v4%20websocket.user.js
// @updateURL https://update.greasyfork.org/scripts/490770/ICEbot%20v4%20websocket.meta.js
// ==/UserScript==
  ...
  deleteElement(element);
    ...
    function updatespeckicks(){
        ...
        roomusers.forEach(user=>{
            user.nick.split("‏").join("")!="ysuf"?f(".userkickmenu").innerHTML+=`<input type="submit" class="kickmenubtn" value="`+user.nick+`" onclick="window.postMessage('kickuser.`+user.id+`','*')">`:0
        })
    }
