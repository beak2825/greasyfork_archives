...
// @downloadURL https://update.greasyfork.org/scripts/448465/%E8%A1%A8%E6%83%85%E8%B4%B4%E7%BA%B8%E5%A2%9E%E5%BC%BA%E6%8F%92%E4%BB%B6.user.js
// @updateURL https://update.greasyfork.org/scripts/448465/%E8%A1%A8%E6%83%85%E8%B4%B4%E7%BA%B8%E5%A2%9E%E5%BC%BA%E6%8F%92%E4%BB%B6.meta.js
// ==/UserScript==
...
], ww = window.innerWidth || document.body.clientWidth, wh = window.innerHeight || document.body.clientHeight, customize = defaultSConf,
...
if (isKF) {
    ...
    for (i = 1; i < 49; i++) { KfSmileList.push(`https://sticker.inari.site/pesoguin/em${i}.gif`); }
    for (i = 1; i < 204; i++) { KfSmileList.push(`https://sticker.inari.site/pesoguin/${i}.gif`); }
}
// 随机
RandomSmileList.push(`https://sticker.inari.site/yukika/${Math.ceil(Math.random() * 6)}.jpg`);
for (i = 0; i < 29; i++) { RandomSmileList.push(`https://sticker.inari.site/rwebp/${Math.ceil(Math.random() * 6930)}.webp`); }
for (i = 1; i < 10; i++) { RandomSmileList.push(`https://sticker.inari.site/rgif/${Math.ceil(Math.random() * 2555)}.gif`); }
// 自定义
...
const addCode = function (textArea, code, selText = '') {
    ...
    if (typeof textArea.selectionStart !== 'undefined') {
        ...
        textArea.selectionStart = prePos + startPos; textArea.selectionEnd = prePos + startPos + selText.length;
        if ($('#spp-reply-textarea').length > 0) {
            ...
            textArea2.selectionStart = prePos + startPos; textArea2.selectionEnd = prePos + startPos + selText.length;
        }
    }
    else { textArea.value += code; if ($('#spp-reply-textarea').length > 0) { $('#spp-reply-textarea')[0].value += code; } }
};
...
const addHCode = function (code) {
    temp = document.createTextNode("\u0001"); $realtimeView.getSelection().getRangeAt(0).insertNode(temp);
    let position = $realtimeView.body.innerHTML.indexOf("\u0001"); temp.parentNode.removeChild(temp);
    if (position > 0) { temp = $realtimeView.createElement("nobr"); temp.innerHTML = code; $realtimeView.getSelection().getRangeAt(0).insertNode(temp); }
...
const showZoomInImage = function ($img) {
    ...
    let offset = $img.offset(), $zoomIn = $(`<img class="stickerpp-zoom-in" src="${$img.attr('src')}" alt="[预览图片]">`).appendTo('body'),
...
const getSmilePanelHtml = function (key) {
    ...
    let html = '', ref; for (i = 0; i < data.addr.length; i++) {
        if (data.datatype === 'image') { html += `<img class="stickerpp-smile" src="${data.addr[i]}" alt="[表情]">`; }
...
const getSubMenuHtml = function () {
    let html = '';
    $.each(MenuList, function (key, data) { html += `<a class="stickerpp-sub-menu" data-key="${key}" href="#" title="${data.desc}">${data.title}</a>`; });
    return html;
};
...
// 表情商店相关
const
    StickerPPDialogHtml = `<form><div class="stickerpp-shop_box" id="StickerPP-shop-dialog" style="display: block; top: 8px; left: 336px;">
     <sheader><logo>&nbsp;&nbsp;&nbsp;表情贴纸商店 | Sticker Shop</logo><span class="stickerpp-close-shop">×&nbsp;&nbsp;</span></sheader>
     <div class="stickerpp-shop_main" ><br><div class="StickerPP-list-content"></div></div>
    ...
    prevNextPageHtml = `<div class="StickerPP-pagination-prev-next"></div>`
    ;
$(document).on("click", "#StickerPP-shop-dialog .sticker-item", function (e) {
    ...
    if (olhaved == false) {
        if (confirm("是否选择启用ID为" + $(e.target).parents(".sticker-item").data("id") + "的在线表情贴纸" + $(e.target).parents(".sticker-item").text())) {
            customize.olimglists.push(selctedid); OnlineRawslists.push(selctedtext); localStorage.setItem('onlineraws', JSON.stringify(OnlineRawslists));
            localStorage.setItem('StickerConf', JSON.stringify(customize)); sessionStorage.removeItem('OnlineSmile');
        } else { alert("启用操作取消") }
    }
        ...
        if (confirm("是否选择停用ID为" + $(e.target).parents(".sticker-item").data("id") + "的在线表情贴纸" + $(e.target).parents(".sticker-item").text())) {
            ...
            customize.olimglists.splice(customize.olimglists.indexOf(selctedid), 1); localStorage.setItem('onlineraws', JSON.stringify(OnlineRawslists));
            localStorage.setItem('StickerConf', JSON.stringify(customize)); sessionStorage.removeItem('OnlineSmile');
        } else { alert("停用操作取消") }
    }
}).on("click", "#StickerPP-shop-dialog .StickerPP-pagination-item-button", function (e) {
    $(document).find('.stickerpp-shop_box').hide(); $dialog = $("#StickerPP-shop-dialog")[0];
    $("body").append(StickerPPDialogHtml); StickerPPLoadSticker($(e.target).data("id"));
...
const createContainer = function (textArea, qufen) {
    let $container = $(`<div class="stickerpp-container">
     ...
     <span class="stickerpp-close-panel" title="表情增强插件，${updatelog}" style="cursor: pointer;"><b>⑨</b></span>
     ...
     <input type="checkbox" class="confbt" id="markdown${qufen}" value="markdown">用M↓取代BBcode
     <input type="checkbox" class="confbt" id="notauthed${qufen}" value="auth">显示未验证贴纸组</li>
     <li><input type="button" class="stickerpp-res-kanbanloc" value="重置看板位置">&nbsp;&nbsp;<input type="button" class="stickerpp-res-hidekanban" value="隐藏看板娘">&nbsp;&nbsp;<input type="button" class="stickerpp-res-all" value="全部初始化">&nbsp;&nbsp;<input type="button" class="stickerpp-conf-close" value="关闭列表"></li>
     </td></tr></table></div></div>
     ...
     <input type="button" class="stickerpp-user-out" value="登出"></div>
    ...
    if (realedit == true && rhview == false && isMQ == false) {
        ...
        if ($('#spp-reply-textarea').length > 0) { $realPHE[0].innerHTML = bb2html($('#spp-reply-textarea')[0].value); }
    ...
    $container.on('click', '.stickerpp-sub-menu', function (e) {
        e.preventDefault(); $container.find('.stickerpp-acc-panel').hide(); $container.find('.stickerpp-bqz-panel').hide(); $this = $(this); let key = $this.data('key'); if (!key) return;
        $container.find('.stickerpp-sub-menu').removeClass('stickerpp-sub-menu-active'); $this.addClass('stickerpp-sub-menu-active'); $container.find('.stickerpp-smile-panel').hide();
        $container.find('.stickerpp-conf-panel').hide(); $panel = $container.find(`.stickerpp-smile-panel[data-key="${key}"]`);
        if ($panel.length > 0) { $panel.show(); }
        else { $('#smilepndw' + qufen).append($(getSmilePanelHtml(key))).show(); let $panels = $container.find(`.stickerpp-smile-panel[data-key="${key}"]`); $panels.show(); }
        ...
        } else if (rhview == true) { if (!code) code = `<img src="${$this.attr('src')}" border="0" alt="" style="max-width:400px"></img>`; addHCode(code); }
        if (/(Mobile|MIDP)/i.test(navigator.userAgent)) textArea.blur(); else textArea.focus();
    }).on('mouseenter', '.stickerpp-smile', function () {
        $('.stickerpp-zoom-in').remove(); showZoomInImage($(this));
        ...
        e.preventDefault(); $('#stickerpp-user-p' + qufen).click();
    }).on('click', '.stickerpp-user-g', function (e) {
        e.preventDefault(); $container.find('.stickerpp-smile-panel').hide(); $container.find('.stickerpp-diy-panel').hide(); $container.find('.stickerpp-acc-panel').hide();
        $this = $(this); $container.find('.stickerpp-user-g').removeClass('stickerpp-user-g-active'); $this.addClass('stickerpp-user-g-active');
        $container.find('.stickerpp-diy-panel').hide(); $panel = $container.find(`.stickerpp-bqz-panel`); $panel.show();
    }).on('click', '.stickerpp-user-i', function (e) {
        e.preventDefault(); $container.find('.stickerpp-acc-panel').hide(); $container.find('.stickerpp-bqz-panel').hide(); $this = $(this);
        $container.find('.stickerpp-user-i').removeClass('stickerpp-user-i-active'); $this.addClass('stickerpp-user-i-active');
        $container.find('.stickerpp-diy-panel').hide(); $panel = $container.find(`.stickerpp-diy-panel`); $panel.show();
    }).on('click', '.stickerpp-user-y', function (e) {
        e.preventDefault(); $container.find('.stickerpp-smile-panel').hide(); $container.find('.stickerpp-diy-panel').hide(); $container.find('.stickerpp-bqz-panel').hide();
        $this = $(this); $container.find('.stickerpp-user-y').removeClass('stickerpp-user-y-active'); $this.addClass('stickerpp-user-y-active');
        $container.find('.stickerpp-acc-panel').hide(); $panel = $container.find(`.stickerpp-acc-panel`); $panel.show();
        ...
        else if (temp != 0) { t = customize.lcimglists; for (i = 0; i < t.length; i++) { $("#ng" + qufen + t[i]).attr("checked", true); } }
        else { alert(errT); console.log('当前已选本地贴纸组数据为：' + customize.lcimglists) };
        $container.find('.stickerpp-diy-panel').hide(); $this = $(this); $container.find('.stickerpp-user-loc').removeClass('stickerpp-user-loc-active');
        $this.addClass('stickerpp-user-loc-active'); $container.find('.stickerpp-loc-panel').hide(); $panel = $container.find(`.stickerpp-loc-panel`); $panel.show();
        ...
        e.preventDefault(); $container.find('.stickerpp-smile-panel').hide();
        // 载入个性化
        ...
        $panel = $container.find(`.stickerpp-conf-panel`); $panel.show();
    ...
    }).on('click', '.stickerpp-res-hidekanban', function () {
        ...
        $panel = $(`#Htmlediterpannel${qufen}`);
        if (rhview == false) {
            temp = $container.find(`#Htmleditarea${qufen}`);
            if (e.target.checked) {
                ...
                if (realedits == false) {
                    ...
                    HeContent = html2bb(temp[0].innerHTML); textArea.innerHTML = HeContent; textArea.innerText = HeContent; textArea.value = HeContent;
                    if ($('#spp-reply-textarea').length > 0) {
...
const StickerPPShowUpload = function () {
    $dialog = $("#StickerPP-shop-dialog")[0]; $("body").append(StickerPPUploadHtml); $root = $("#StickerPP-shop-dialog .StickerPP-list-content");
        ...
        $.ajax({ url: cApi + 'CheckSession&user_id=' + aId + '&token=' + aToken, type: 'POST', dataType: 'json' })
            .done(data => {
                if (data.ret == 200) {
                    ...
                    if (temp == true) { $root.append($(StickerPPlogedUp)); }
                    else { $root.append($(StickerPPunlogUp)); }
                }
                else { $root.append($(StickerPPunlogUp)); }
            })
            .fail(data => { $root.append($(StickerPPunlogUp)); console.log(data) });
    }
    else { $root.append($(StickerPPunlogUp)); }; $root.append($(StickerPPtextUp));
}
const StickerPPShowDialog = function () { $dialog = $("#StickerPP-shop-dialog")[0]; $("body").append(StickerPPDialogHtml); StickerPPLoadSticker(1); }
...
const loadStickerList = function (items) {
    $root = $("#StickerPP-shop-dialog .StickerPP-list-content"); $root.empty();
    $.each(items, function (_, o) {
        temp = JSON.parse(o.content); $node = $(StickerPPItemHtml).prop("title", temp.desc).data("id", o.id).data("content", o.content)
...
const loadStickerListPagination = function (data) {
    let total = Math.ceil(data.total / 20), page = data.page, id; $root = $("#StickerPP-shop-dialog .StickerPP-list-pagination");
    if (page != 1) { $root.append($(StickerPPPaginationItemHtml).data("id", 1).text("回首页")); $root.append($(prevNextPageHtml).data("id", page - 1).text("上一页")); }
    if (total < 12 || page < 7) {
        for (i = 1; i < page; ++i) { id = i; $node = $(StickerPPPaginationItemHtml).data("id", id).text(id); $root.append($node); }
        $node1 = $(StickerPPNowPageHtml).data("id", page).text(page); $root.append($node1);
        for (i = page; i < total; ++i) { id = i + 1; $node = $(StickerPPPaginationItemHtml).data("id", id).text(id); $root.append($node); }
    }
    else if (total > 11 && page + 5 < total) {
        for (i = page - 5; i < page; ++i) { id = i; $node = $(StickerPPPaginationItemHtml).data("id", id).text(id); $root.append($node); }
        $node1 = $(StickerPPNowPageHtml).data("id", page).text(page); $root.append($node1);
        for (i = page; i < page + 5; ++i) { id = i + 1; $node = $(StickerPPPaginationItemHtml).data("id", id).text(id); $root.append($node); }
    }
    else if (total > 11 && page + 6 > total) {
        for (i = total - 10; i < page; ++i) { id = i; $node = $(StickerPPPaginationItemHtml).data("id", id).text(id); $root.append($node); }
        $node1 = $(StickerPPNowPageHtml).data("id", page).text(page); $root.append($node1);
        for (i = page; i < total; ++i) { id = i + 1; $node = $(StickerPPPaginationItemHtml).data("id", id).text(id); $root.append($node); }
    }
    if (page != total) { $root.append($(prevNextPageHtml).data("id", page + 1).text("下一页")); $root.append($(StickerPPPaginationItemHtml).data("id", total).text("去末页")); }
}
...
if (isKF == true) {
    ...
    document.body.querySelectorAll('.readtext a').forEach(i => {
        if (i.innerHTML === '<span class=\"k_f18\">请手动点击打开本图片</span>') {
            ...
            if (t.src.match(/https:\/\/sticker.inari.site/)) { i.parentElement.replaceChild(t, i); }
...
function attspan(num) { $('.StickerPPHtmlEditer')[num].innerHTML += `<img src="${uupath[num]}" type="upload_1" width="240">`; }
// html2bb&bb2html
...
// 注册&登录方法
function loginfunc() {
    if (GM_getValue('sppuid', false) != false && GM_getValue('spptoken', false) != false) {
        if (confirm('检测到已在其他网站登录云同步账号，是否使用快速登录？')) {
            localStorage.setItem('logindata', JSON.stringify([GM_getValue('sppuid', false), GM_getValue('spptoken', false)])); imgbindcheckfunc(); return;
        }
    }
        ...
        $.ajax({ url: iApi + 'upload', type: 'POST', dataType: 'json', data: formData, contentType: false, processData: false, })
            .done(data => {
                if (data.status == true) {
                    ...
                    if (rhview == false && realedit == true) { document.execCommand('insertImage', false, temp.url); }
                    else if (rhview == false && realedit == false) {
                        if (customize.markdown == false) { addCode(textArea, temp.bbcode); }
                        else if (customize.markdown == true) { addCode(textArea, `![](${temp.url})`); }
                    } else if (rhview == false && realedit == true) { addHCode(`<img src="${temp.url}" border="0" alt="" style="max-width:400px"></img>`); }
...
window.onresize = function () {
    temp = $('#stickerppkanban'); ww = window.innerWidth || document.body.clientWidth; wh = window.innerHeight || document.body.clientHeight;
    if (isKfMobile || isMobile) {
...
const appendCss = function () {
   ...
   .StickerPPHtmlEditer video {max-width:500px}
   </style>`);
 ...
 * 初始化
 */
const init = function () { $textAreas = $("textarea"); if (!$textAreas.length) return; appendCss(); $textAreas.each(function (i) { createContainer(this, i); }); };
if (loadcustom == false) {
...
if ($("iframe[id='e_iframe'],iframe[name='iframe']").length > 0) { $realtimeView = $("iframe[id*='iframe'],iframe[name='iframe']")[0].contentWindow.document, rhview = true; }
init();
