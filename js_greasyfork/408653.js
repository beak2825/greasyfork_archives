...
// @namespace    tehapollo
// @downloadURL https://update.greasyfork.org/scripts/408653/Hybrid%20ScraperBlock%20All%20in%20One.user.js
// @updateURL https://update.greasyfork.org/scripts/408653/Hybrid%20ScraperBlock%20All%20in%20One.meta.js
// ==/UserScript==
...
///SCRAPER STUFF/////////////////////////////////////////////////////////////////////////////////
//Styling
$('<input type="checkbox" id="Pushing" name="Pushing"/><label>PushBullet <label/>').insertAfter("h1:eq(0)");
$('<input type="checkbox" id="Desktop" name="Desktop"/><label>Desktop <label/>').insertAfter("h1:eq(0)");
$('<input type="checkbox" id="ConstantSound" name="ConstantSound"/><label>Constant Sound <label/>').insertAfter("h1:eq(0)");
$('<input type="checkbox" id="Today" name="Today"/><label>Today Only <label/>').insertAfter("h1:eq(0)");
$('<input type="checkbox" id="Auto" name="Auto"/><label>Auto-Start Scraper <label/>').insertAfter("h1:eq(0)");
$('<h5><h5/><label>Min Pay:$<label/><input type="text" name="minPay" id="minPay" value=""style="width: 50px; height: 18px;">').insertBefore('table.table-striped.data-table.dt-reponsive:eq(0)');
...
$('<h6><h6/><input type="button" value="Save Settings" id="SaveSettings"/>').insertBefore('table.table-striped.data-table.dt-reponsive:eq(0)');
...
$('h1:eq(0)').append('<p id="WarnText" style=" margin: 0px 50px 0px; font-size: 50pt;"form="usrform">WARNING Scraper Not On!!!!!!!</p>');
(function() {
    'use strict';
    ...
    $.get(url, function (data) {
        ...
        work.forEach( function(el) {
            ...
            if (el.pay >= min_pay && today === el.created && constant_sound.checked == true && !blocked_projects.includes(el.description) && Today_Only.checked == true && !el.description.includes("Draw a box") && !el.description.includes("_UTC") && el.tasks >= min_task || el.pay >= min_pay && constant_sound.checked == true && !blocked_projects.includes(el.description) && Today_Only.checked == false && !el.description.includes("Draw a box") && !el.description.includes("_UTC") && el.tasks >= min_task) {
                audio.play();
                  if (pushbullet_key.length && !pushed.includes(el.description) && bullet_checked.checked == true) { pushItRealGood(el.description + ' - $' + el.pay + ' - ' + el.tasks + ' hits' + '\n'+ el.link)
