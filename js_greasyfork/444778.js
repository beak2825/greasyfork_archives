...
// @downloadURL https://update.greasyfork.org/scripts/444778/%E9%A1%B9%E7%9B%AE%E7%BB%84%E6%8F%90%E9%97%AE%E5%B0%8F%E5%8A%A9%E6%89%8Bv10.user.js
// @updateURL https://update.greasyfork.org/scripts/444778/%E9%A1%B9%E7%9B%AE%E7%BB%84%E6%8F%90%E9%97%AE%E5%B0%8F%E5%8A%A9%E6%89%8Bv10.meta.js
// ==/UserScript==
			...
			switch ($('input[name="module"]:checked').val()) {
				case "报表":
					tags = ['finereport-11.0', 'finereport-10.0', 'finereport-9.0', 'finereport-8.0',
						...
						'决策报表-展现/布局', '决策报表-离屏控制', '决策报表-取数/计算','决策报表-新自适应', '参数-参数计算', '参数-参数面板控件', '填报-导入excel',
				...
				case "图表":
					tags = ['finereport-11.0', 'finereport-10.0', 'finereport-9.0', 'finereport-8.0', 'BI-5.0', 'BI-6.0', 
						'图表-报错', '图表-接口', '图表-难还原', '图表-需求&方案咨询','图表-扩展图表', '第三方插件-图表类'
					];
					break;
				case "移动端":
					tags = ['finereport-11.0', 'finereport-10.0', 'finereport-9.0', 'finereport-8.0', 'BI-5.0', 'BI-6.0', 
				...
				default:
					tags = ['finereport-10.0', 'finereport-9.0', 'finereport-8.0', 'BI-5.0', 'BI-6.0', 
						...
						"移动端-控件&oem平台", "移动端-集成&bi", '其他-找杨洋', '第三方插件功能确认'
					]
			}
		} else {
			tags = ['finereport-11.0', 'finereport-10.0', 'finereport-9.0', 'finereport-8.0', 'BI-5.0', 'BI-6.0', 
...
.radio input[type="radio"]:checked + .radio-label:before {
  ...
  box-shadow: inset 0 0 0 4px #f4f4f4;
		...
		document.getElementById('question-body_ifr').contentDocument.getElementById('tinymce').innerHTML =
			"<p>【问题描述】：</p><p>【已查文档】：</p><p>【JAR包版本】：</p><p>【相关插件】：</p><p>【环境信息】：</p><p>【相关日志】：</p><p>【报错截图】：</p>";
		document.getElementById('question-body_ifr').contentDocument.getElementById('tinymce').title =
			`【已查文档】：若查阅了相关文档但仍未解决，可以反馈给三线优化
【问题描述】：写上执行过的排查步骤，需求类问题需补充上客户的完整需求场景
【环境信息】：若对环境有特殊需求则备注上方便定问
【JAR包版本/相关插件】：如果不是最新版本，补充不能升级的原因；以及插件的影响
【截图/日志附件】：尽量获取日志和内置数据集模版，使用insert link来上传文件
        `
		//内容和标题填充
		var notice =
			'<label  style="color:green;">标签不少于3项（团队+版本+模块），点击下方表格可自动添加标签；</br>问答推送时间：上午11点30&下午17点，加急请找young.yang！</label>'
		$('input#tags').parent('div.form-item').children('label').click(function() {
			window.open("https://kms.fineres.com/pages/viewpage.action?pageId=399969055")
		})
		...
		var module =
			'<div style="display:inline"><label style="font-size:20px;color:turquoise;font-weight:bold">问题模块</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="module" id="report" checked value="报表"</input><label class="radio-label"  for="report" >报表</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="module" id="chart" value="图表"</input><label  for="chart" class="radio-label">图表</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="module" id="mobile" value="移动端"</input><label class="radio-label"  for="mobile" >移动端</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="module" id="dec" value="平台"</input><label  for="dec" class="radio-label">平台</label></div><br/>';
		var type =
		...
		$('input[type="radio"][name="target"]').click(function() {
			...
			if ($(this).val() == "普通版") {
				...
				$('#question-form > div.page-title > h1').append(
					'<a name="my-link" target="_blank" href="https://kms.fineres.com/pages/viewpage.action?pageId=399969739">&nbsp;&nbsp;&nbsp;&nbsp;请熟读问答规范</a>'
					)
