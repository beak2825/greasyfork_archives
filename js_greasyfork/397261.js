...
var run = function() {
    ...
    window.localStorage.setItem(stockStorageName,JSON.stringify(stocks));
      // а если это бы чужой склад
    var list = $('table[class="grid"] > tbody > tr > td:nth-child(3) > a:has(img[src="/img/supplier_add.gif"])');
    if ( list.length > 0 ) {
    	// Стили  
  		var st = $("style");
  		...
  		st.append(".my_btn:hover{opacity:1.0}");
			...
			var wc_save = $("<li class='my_btn fl_r'><img src='https://cdn0.iconfinder.com/data/icons/pinterest-ui-flat/48/Pinterest_UI-06-512.png' title='Запомнить этот склад как склад для массового заказа'>");
  		var wc_out = $("<div class='fl_r' id=save_out></div>");
      
  		if ( $("ul.tabu").length > 0 ) {
    		$("ul.tabu").append( wc_save );
  			$("#childMenu").before( wc_out );
  		}
  		wc_save.click( function(){
				$('table[class="grid"] > tbody > tr > td:nth-child(3) > a:has(img[src="/img/supplier_add.gif"])').each(function() {
				matches = $(this).attr('href').match(/\/product:(\d+)\//);
				idvProduct = matches[1];
				console.log("(s)idvProduct = " + idvProduct);
