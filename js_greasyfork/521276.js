// ==UserScript==
// @name        SPXXNeko
// @description Minecraft.net & X.com blog article to BBCode converter, adapted to MCNeko
// @namespace   npmjs.com/package/@SPXXNeko/userscript
// @author      MeowcoQAQ & Cinder & SPGoding & SPX Fellow
// @connect     *
// @connect     feedback.minecraft.com
// @connect     help.minecraft.net
// @connect     raw.githubusercontent.com
// @homepage    https://github.com/azdyqwo/SPXXNeko
...
(function () {
  ...
  if (src == "Github") {
    console.log("[SPXXNeko] 正在使用 Github 漏洞中心");
    tr =
      "https://raw.githubusercontent.com/SPXFellow/spxx-translation-database/crowdin/zh-CN/zh_CN.json";
    tor =
      "https://raw.githubusercontent.com/SPXFellow/spxx-translation-database/master/translator.json";
    ...
    console.log("[SPXXNeko] 正在使用自定义漏洞中心");
...
[color=#388e3c][size=5]|[/size][/color][size=4]本文内容按照 [/size][url=https://creativecommons.org/licenses/by-nc-sa/4.0/deed.zh-hans][size=4][color=#2e8b57][u]CC BY-NC-SA 4.0[/u][/color][/size][/url][size=4] 协议进行授权，[b]转载本帖时须注明[color=#ff0000]原作者[/color]以及[color=#ff0000]本帖地址[/color][/b]。[/size][hr]\n`;
  ...
  function getFooter(articleType, type) {
    ...
    const poweredBy = `\n[align=center][size=1][color=Silver]Powered by SPXXNeko ${SPXXNekoVersion} with love
Converted at ${time.getFullYear()}-${
    ...
    switch (type) {
      case VersionType.Snapshot:
        return `\n${poweredBy}\n[hr][color=#388e3c][size=5]|[/size][/color][size=4][b]想了解更多游戏资讯？[/b][/size][list][*][size=3][url=https://www.mcneko.com/forum-8-1.html][color=#388e3c][u]MCNeko - 游戏资讯版块[/u][/color][/url][/size][/list]`;
  ...
  const converters = {
    ...
    convert: async (node, ctx) => {
      if (node.classList?.contains("SPXXNeko-userscript-ignored")) {
        return "";
  ...
  function getZendesk(controlDOM, titleSlice, contentClass, versionType) {
    ...
    button.classList.add("SPXXNeko-userscript-ignored", "navLink");
...
【本文排版借助了：[url=https://github.com/MCNeko-Forum/SPXXNeko][color=#388d40][u]SPXXNeko[/u][/color][/url] 用户脚本 v${SPXXNekoVersion}】\n\n${footer}`;
  ...
  function twitter() {
    ...
    function getTweetBbcode(tweet, mode) {
      ...
      const dateString = `${tweet.date} · ${tweet.source} · SPXXNeko v${SPXXNekoVersion} · 转载请注明原作者及本帖地址`;
...
[color=${foregroundColor}][size=23px]${content}[/size]
  ...
  [/table][/align]`;
    ...
    function x() {
      console.info("[SPXXNeko] Activated");
      ...
      const checkLoaded = setInterval(() => {
        ...
        if (targetDiv && !document.querySelector("#SPXXNeko-buttons")) {
          const container = document.createElement("div");
          container.id = "SPXXNeko-buttons";
          container.style.display = "flex";
          container.style.flexDirection = "column";
