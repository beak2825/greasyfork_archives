...
// @run-at         document-end
// @downloadURL https://update.greasyfork.org/scripts/434344/Chaturbate%20%7C%20JSON-Z.user.js
// @updateURL https://update.greasyfork.org/scripts/434344/Chaturbate%20%7C%20JSON-Z.meta.js
// ==/UserScript==


var script = document.createElement('script');
script.src = 'https://cdnjs.cloudflare.com/ajax/libs/handlebars.js/4.7.6/handlebars.min.js';
script.setAttribute("id", "customHANDLES");
...
$(function(){
    ...
    addGlobalStyle('*,body{box-sizing:border-box!important}body{padding:0;margin:0}.margin-top40{margin-top:40px!important}.profile:first-child,.profile:nth-child(2){margin-top:40px}@media(max-width:900px) and (min-width:414px) and (orientation:landscape){.profile{width:32.5%!important}}.playing .vid{visibility:visible!important;opacity:1!important}.mobile-hashtag-spacer{display:none!important;}header{background:#308c91!important;}');



    $('meta[name*="viewport"]').remove();
    $('head').prepend('<meta name="viewport" content="width=device-width,user-scalable=no,initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0">');
    ...
    function windowReszr() {
        ...
        $('.profile:first-child').addClass('margin-top40');
        $('.profile:nth-child(2)').addClass('margin-top40');
    ...
    function hovv() {
        ...
        $(".profile").each(function() {
            ...
            //console.log({m3u8URL});
            //var regExrm3u8URL = m3u8URL.replace(/%22/g, '').replace(/"/g, '');
            //console.log({regExrm3u8URL});
            ...
            if( m3u8URL.length > -1 ) {
                ...
                $(this).prepend('<div class="copyVidURL" style="z-index: 10;height: 25px;position: absolute;right: 8px;top: 8px;opacity: .7;cursor: pointer;"> <a href="https://bounsr.com/cx4/m/watchatur.html?directVid='+m3u8URL+'" target="_blank"> <svg class="svg" viewbox="0 0 488 512" height="25"> <path class="path" fill="#fff" stroke="#454545" stroke-width="8" d="M308.4,424.8v35.2c0,11.7-9.4,21.1-21.1,21.1H48.2c-11.7,0-21.1-9.4-21.1-21.1V136.5c0-11.7,9.4-21.1,21.1-21.1 h63.3v260.2c0,27.2,22.1,49.2,49.2,49.2H308.4z M308.4,122.4V31H160.7c-11.7,0-21.1,9.4-21.1,21.1v323.4c0,11.7,9.4,21.1,21.1,21.1 h239.1c11.7,0,21.1-9.4,21.1-21.1v-232h-91.4C317.9,143.5,308.4,134,308.4,122.4z M414.7,95.2l-58-58c-4-4-9.3-6.2-14.9-6.2h-5.3 v84.4h84.4V110C420.9,104.5,418.7,99.1,414.7,95.2z" style="stroke-dasharray: 1300;stroke-dashoffset: 1300;"> </path> </svg></a></div>');
            }
...
*/
    ...
    function gayFilter() {
        ...
        $(".profile").each(function() {
            ...
            if(profileCount == 1 || profileCount == 2) {
              console.log("profile is 1 or 2");
              $(this).css('margin-top', '40px');
              $(this).addClass('margin-top40');
            }
    ...
    function appendStructure() {
        ...
        $('body').attr('style', 'width: 100vw !important;max-width: 100vw !important;box-sizing: border-box;');
    ...
    function buildProfileArrays() {
        console.log("BUILDING PROFILE ARRAYS");
            ...
            $('ul.list li').each(function() {
                ...
                var thumb = $(this).find('.room_thumbnail').attr("src");
                thumb = thumb.replace('\/ribw\/g', '/riw/');

                var user = $(this).find('.cardTitle a').text();
                var viewStats = $(this).find('.sub-info .viewers').text();
                var subject = $(this).find('.subject').text();

                item["originalProfURL"] = originalProfURL;
                item["m3u8"] = encodeURI(m3u8);
                item["thumb"] = thumb;
                item["user"] = user;
                ...
                if(originalProfURL != "https://chaturbate.comundefined") {
                    myArr.push(item);
                }
            });
            ...
            $('<div class="newMain" style="visibility:hidden;"> <script id="entry-template" type="text/x-handlebars-template"> <style>@media(max-width:900px) and (min-width:414px) and (orientation:landscape){.profile{width: 32.5% !important;}}</style>{{#each users}}<div class="profile" style="width:48.5%;display:inline-block;float:left;position:relative;transition:transform .2s ease-out;background:#fff;min-height:225px;overflow:hidden;margin:2px;border:2px solid #ccc;max-height:225px!important;" data-origURL="{{originalProfURL}}" data-m3u8="{{m3u8}}" data-subject="{{subject}}"><div class="thumb" style="background:url({{thumb}});width:100%;height:205px;max-height:205px;position:relative;display:inline-block;background-repeat:no-repeat!important;background-position:center center!important;" data-resolution=""><video class="vid" data-src="{{m3u8}}" muted="" defaultmuted="" playsinline="playinline" webkit-playsinline="" style="width:100%;height:100%;max-height:205px;opacity:0;transition:.8s ease all;background:rgba(0,0,0,.5);border:2px solid transparent;visibility:hidden;"></video></div><div class="userInfo" style="width:100%;position:relative;display:inline-block;padding:0 6px 0;box-sizing:border-box;top:-4px;"><a class="profileURL" target="_blank" data-href="{{originalProfURL}}" style="position:relative;display: block;overflow:hidden;"><span class="user" style="float:left;font-size:10px;">{{user}}</span><span class="viewers" style="float:right;font-size:10px;">{{viewStats}}</span></a></div></div>{{/each}}</script></div>').prependTo('body');
    ...
    function updateDirectURL() {
        ...
        $('#main ul.list li').each(function() {
            var $this = $(this);

            var profHref = $(this).find(".room_thumbnail_container").attr('href'); console.log({profHref});
            var newProfLink = 'https://chaturbate.com'+profHref; console.log({newProfLink});
            var origProfLink = 'https://chaturbate.com'+profHref; console.log({origProfLink});
            $(this).attr('href', newProfLink).attr('data-origProfURL', newProfLink);

            // console.log({newProfLink}, {origProfLink});
            ...
            $.get(newProfLink, function(response) {
                ...
                // hlsURL = hlsURL.match(/(^.*)(?=\?)/g, /$&/);
                //hlsURL = hlsURL[0];
                console.log("[[445] "+hlsURL);

                $this.attr('data-m3u8', hlsURL);
                $this.find(".room_thumbnail_container").attr('href', hlsURL);
