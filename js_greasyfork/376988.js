...
// @author         2015, suchar, 2019, woland
// @homepage       https://greasyfork.org/pl/scripts/376988-multikoszyk-fotka-pl
// @include        http://fotka.com/*
...
// @grant          GM_getValue
// @grant          GM_setValue
// @downloadURL https://update.greasyfork.org/scripts/376988/MultiKoszyk%20fotkapl.user.js
// @updateURL https://update.greasyfork.org/scripts/376988/MultiKoszyk%20fotkapl.meta.js
// ==/UserScript==
...
if (document.getElementById("content")){ // czy to nie popup?
    dodajMenuGorne();
    if (document.getElementById("profile-box-data-container") != null){ // czy jesteśmy na profilu?
        dodajPrzyciskProfil();
        dodajMenuPowod();
    }
}
...
function dodajMenuGorne(){
    ...
    noweMenu.className = "header-menu";
    noweMenu.style.cssText = "margin-bottom: 30px;";
    ...
    a.innerHTML = "Koszyk";
    a.className = "grey-link margin-left" ;
    noweMenu.appendChild(a);
    $("#header-logo").after(noweMenu);
...
<table style="width: 100%;">\n    <tr>\n\
        ...
        <td><a href="javascript:void(0)" id="Koszyk_HTML_'+x+'">Na grupę</a></td>\n\
        <td><a href="javascript:void(0)" id="Koszyk_BB_'+x+'">Na forum</a></td>\n\
    ...
    //obsługa zdarzeń
    document.getElementById('button_export').addEventListener("click", exportuj, false);
    document.getElementById('button_import').addEventListener("click", importuj_pytanie, false);
...
function pokazHTML(e){
    ...
    for (w=0; w<wpisy(nr_koszyka).length; w++){
        if (wpisy(nr_koszyka)[w]["info"] == ""){
            zawartosc += [w+1] + ' ' + wpisy(nr_koszyka)[w]["profil"] + ' <br />\n';
        }else{
            zawartosc += [w+1] + ' ' + wpisy(nr_koszyka)[w]["profil"] + ' - ' + wpisy(nr_koszyka)[w]["info"] + ' <br />\n';
        }
    }
    zawartosc += '<br>\n<br>\n</div>\n';
    var tmp = newElem('div');
...
function importuj_pytanie(){
    ...
    if(pelny) {
        if (confirm("W koszyku są profile. Czy napewno zrobić import?")){
             importuj(kosz);
        }
    } else {
        importuj();
...
function dodajPrzyciskProfil(){
    ...
    var koszyk;
    var powod;
    var url = "http://www.fotka.pl/profil/" + unsafeWindow._currentProfile.login + "/";
    for(var x=1; x<=ile_koszykow; x++){
...
function dodajProfil(){
    var login = unsafeWindow._currentProfile.login;
    var powod = $("#f_powod").val();
    var av = $('#profile-avatar a[class^="av-64"] img')[0].src;
    var koszyk_nr;
...
function usunWpis(e){
    var nr_koszyka =  unsafeWindow.document.getElementById('Koszyk_nr').value;
...
function zbudujOknoMenu(text, w, h){
    ...
    //<div id="fotkaLightBoxClose" class="sprites ico-close-cross popup-close"></div>
    var div4 = newElem("div");
    div4.id = "fotkaLightBoxClose";
    div4.className = "sprites ico-close-cross popup-close";
...
function oproznijKoszyk(e){
    ...
    if (n>0){
        if (confirm("Czy napewno chcesz usuńąć " + n + " profili z koszyka?")){
