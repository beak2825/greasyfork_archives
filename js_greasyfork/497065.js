...
// @downloadURL https://update.greasyfork.org/scripts/497065/%E4%BF%9D%E5%9B%BD%E9%80%9B%EF%BC%88%E4%BB%85%E4%BB%85%E6%88%91%E6%9C%AC%E4%BA%BA%E8%87%AA%E7%94%A8%E8%80%8C%E5%B7%B2%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/497065/%E4%BF%9D%E5%9B%BD%E9%80%9B%EF%BC%88%E4%BB%85%E4%BB%85%E6%88%91%E6%9C%AC%E4%BA%BA%E8%87%AA%E7%94%A8%E8%80%8C%E5%B7%B2%EF%BC%89.meta.js
// ==/UserScript==
 ...
 // 自动向下滚动功能
    ...
    document.addEventListener('visibilitychange', function() {
        if (document.visibilityState === 'visible' && !hasScrolled) {
            scrollDown();
            hasScrolled = true; // 设置标记，确保只滚动一次
        }
    });

    // 下面是添加按钮功能的代码
     // 创建按钮1：变为sehuatang.net
    var buttonNet = document.createElement('button');
    buttonNet.innerHTML = '变为sehuatang.net';
    buttonNet.style.position = 'fixed';
    buttonNet.style.top = '10px';
    buttonNet.style.right = '10px'; // 右上角
    buttonNet.style.zIndex = 1000;
    buttonNet.style.backgroundColor = '#f90';
    buttonNet.style.color = '#fff';
    buttonNet.style.border = 'none';
    buttonNet.style.padding = '10px';
    buttonNet.style.cursor = 'pointer';
    ...
    buttonNet.onclick = function() {
        ...
        // 替换域名为sehuatang.net
        var newUrl = currentUrl.replace(window.location.hostname, 'sehuatang.net');
        // 在新窗口中打开新网址
        window.open(newUrl, '_blank');
    };

    // 将按钮1添加到页面
    document.body.appendChild(buttonNet);

    // 创建按钮2：变为sehuatang.org
    var buttonOrg = document.createElement('button');
    buttonOrg.innerHTML = '变为sehuatang.org';
    buttonOrg.style.position = 'fixed';
    buttonOrg.style.top = '70px'; // 按钮2相对于按钮1的位置
    buttonOrg.style.right = '10px'; // 右上角
    buttonOrg.style.zIndex = 1000;
    buttonOrg.style.backgroundColor = '#0c0';
    buttonOrg.style.color = '#fff';
    buttonOrg.style.border = 'none';
    buttonOrg.style.padding = '10px';
    buttonOrg.style.cursor = 'pointer';

    // 按钮2点击事件
    buttonOrg.onclick = function() {
        // 获取当前网址
        var currentUrl = window.location.href;
        // 替换域名为sehuatang.org
        var newUrl = currentUrl.replace(window.location.hostname, 'sehuatang.org');
        // 在新窗口中打开新网址
        window.open(newUrl, '_blank');
    };

    // 将按钮2添加到页面  lc代码截止
    document.body.appendChild(buttonOrg);
  /**
  ...
  function getSettings() {
    ...
    return {
      ...
      imageUrl: GM_getValue(
        "imageUrl",
        "/uc_server/data/avatar/000/46/26/94_avatar_big.jpg"
      ), // 图片的URL
