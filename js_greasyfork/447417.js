// ==UserScript==
// @name         Gazelle Music Tracker Snatched (patched)
// @name:zh      GMTS：Gazelle框架音乐PT--种子下载状态统计标记助手
// @name:zh-CN   GMTS：Gazelle框架音乐PT--种子下载状态统计标记助手
// @name:zh-TW   GMTS：Gazelle框架音樂PT--種子下載狀態統計標記助手
// @namespace       Gazelle Snatched
// @description     Mark snatched torrents across your favorite gazelle music trackers.
// @description:zh      将你在Gazelle框架音乐PT上下载中的，下载完成的等等种子用不同的颜色标记出来.
// @description:zh-CN   将你在Gazelle框架音乐PT上下载中的，下载完成的等等种子用不同的颜色标记出来.
// @description:zh-TW   將你在Gazelle框架音樂PT上下載中的，下載完成的等等種子用不同的顏色標記出來.
// @author          Mordred
...
// @version         3.0.1
// @date            2022-07-04
// @downloadURL https://update.greasyfork.org/scripts/447417/Gazelle%20Music%20Tracker%20Snatched%20%28patched%29.user.js
// @updateURL https://update.greasyfork.org/scripts/447417/Gazelle%20Music%20Tracker%20Snatched%20%28patched%29.meta.js
// ==/UserScript==
...
(function () {
  ...
  function GM_setLSValue(key, value) {
    try {
      ...
      console.error(
        "Gazelle Snatched Error: Could not update torrent list. You will most likely need to update your maximum localStorage size. \
  ...
  function doOptionsMenu() {
    ...
    if (options_menu.length == 0) {
      ...
      var refreshInput = $(
        '<input class="gazelle_subItem gazelle_numeric" type="text" name="interval" maxlength="3">Interval between updates in minutes (minimum of 10)<br>'
      ...
      var check_box_hide = $(
        '<input class="gazelle_subItem" type="checkbox" name="visibility">Show the status box on all pages<br>'
      );
      var explanation_div = $(
        "<div class=\"gazelle_small_text gazelle_subItem\">The status box will always appear on '/torrents.php?type=...' and whenever a script update is available.</div>"
      );
      ...
      var positionHeader = $(
        '<h3 class="gazelle_header">Status Box Position</h3>'
      );
      var radio_button_tl = $(
        '<input class="gazelle_subItem" type="radio" name="location" id="top_left"/>Top Left<br>'
      );
      var radio_button_tr = $(
        '<input class="gazelle_subItem" type="radio" name="location" id="top_right"/>Top Right<br>'
      );
      var radio_button_bl = $(
        '<input class="gazelle_subItem" type="radio" name="location" id="bottom_left"/>Bottom Left<br>'
      );
      var radio_button_br = $(
        '<input class="gazelle_subItem" type="radio" name="location" id="bottom_right"/>Bottom Right<br>'
      );
      ...
      var offsetHeader = $(
        '<h3 class="gazelle_header">Status Box Y-Offset</h3>'
      );
      var offsetInput = $(
        '<input class="gazelle_subItem gazelle_numeric" type="text" name="yOffset" maxlength="3">The offset in pixels from the top or bottom of the window<br>'
      ).css({ "text-align": "right", width: "20px" });
      ...
      var styleHeader = $(
        '<h3 class="gazelle_header">Link Style Settings</h3>'
      );
      full_div.append(styleHeader);
      var sampleText = $(
        '<span class="gazelle_class"></span><a href="#" id="sample_gsnatched">Sample Group Snatched Torrent Link</a><br>'
      );
      ...
      var defaultLink = $(
        '<span class="gazelle_btn gazelle_sm_btn">Default</span>'
      );
      ...
      var sampleText = $(
        '<span class="gazelle_class"></span><a href="#" id="sample_tsnatched">Sample Snatched Torrent Link</a><br>'
      );
      sampleText.click(function () {
        return false;
      });
      ...
      sampleText = $(
        '<span class="gazelle_class"></span><a href="#" id="sample_uploaded">Sample Uploaded Torrent Link</a><br>'
      );
      sampleText.click(function () {
        return false;
      });
      ...
      sampleText = $(
        '<span class="gazelle_class"></span>Seeding links will <i>always</i> have either the .gazelle_snatched style or the .gazelle_uploaded style applied<br><span class="gazelle_class"></span>to them, so .gazelle_seeding is commonly used to override those base styles.</br>'
      );
      var sampleTxt2 = $(
        '<span class="gazelle_class"></span><a href="#" id="sample_seeding">Sample Seeding Snatched Torrent Link</a>&nbsp;&nbsp;<a href="#" id="sample_ul_seed">Sample Seeding Uploaded Torrent Link</a><br>'
      );
      ...
      sampleText = $(
        '<span class="gazelle_class"></span><a href="#" id="sample_leeching">Sample Leeching Torrent Link</a><br>'
      );
      sampleText.click(function () {
        return false;
      });
      ...
      sampleText = $(
        '<span class="gazelle_class"></span><a href="#" id="sample_bookmarked">Sample Bookmarked Torrent Link</a><br>'
      );
      sampleText.click(function () {
        return false;
      });
      ...
      var okay_button = $(
        '<span id="js_ok_button" class="gazelle_btn pull-right">Submit</span>'
      );
      ...
      var cancel_button = $(
        '<span id="js_close_button" class="gazelle_btn pull-right">Cancel</span>'
      );
  ...
  registerMenuCommand("Gazelle Snatched: Options", function () {
    ...
    torrent_table.find("div.group_info").each(function (i) {
      ...
      if (m) {
        ...
        if (!m) {
          status
            .contents()
            .append(
              '<div><span style="color: red;">Failed:</span> ' +
      ...
      if (group_id && torrent_id) {
        ...
        if (
          ...
          if (!reg) {
            console.error(
              "looks like we couldn't find the name of the torrent group. Please let Mordred know."
            );
            console.error(
              "Attempting to parse group name from:",
              $(this).text()
            );
          }
          var nm = reg[1].trim();
  ...
  function scan_all_torrent_pages(type, page_cb, finish_cb, forced_full) {
    ...
    function error_handler(response, reason) {
      status
        .contents()
        .append(
          '<div><span style="color: red;">Error:</span> Unable to fetch ' +
            type +
            " page " +
            page +
            " (" +
        ...
        if (torrent_id && d.torrents[torrent_id]) {
          ...
          if (d.torrents[torrent_id].sd) {
            if (d.torrents[torrent_id].ty != "uploaded")
              $(this).addClass(
                "gazelle_seeding gazelle_snatched"
    ...
    if (/\/torrents\.php\?id/.test(document.URL)) {
      ...
      if (
        ...
        if (g.groups[group_id])
          mark_snatched = $(
            '<a href="#" id="mark_snatched" class="brackets">Unmark Snatched</a>'
          );
        else
          mark_snatched = $(
            '<a href="#" id="mark_snatched" class="brackets">Mark Snatched</a>'
          );
  ...
  if (/\/torrents\.php/.test(document.URL)) {
    ...
    if (
      ...
      scan_status
        ...
        .text(
          "Done (" +
            (found > 0 ? found + " updates found" : "no updates found") +
            ")"
        );
      status.show(5000);
    }
  ...
  if (/\/bookmarks\.php(?!.action=edit)/i.test(document.URL)) {
    ...
    scan_status
      .children("span")
      .text(found > 0 ? found + " bookmarks found" : "no bookmarks found");
    status.show(5000);
  }
  ...
  if (next_update < now.getTime() && just_updated != 1) {
    ...
    var update_status = {
      ...
      bookmark: $(
        "<div>Updating bookmarks: <span>Initializing...</span></div>"
      ),
    };
