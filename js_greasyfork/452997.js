// ==UserScript==
// @name         BlackRussia CHOCO by K.Mironova
// @namespace    https://forum.blackrussia.online
// @version      0.228
// @description  Mironova is higher than you thought
// @author       Ksenia
// @match        https://forum.blackrussia.online/index.php?threads/*
// @grant        none
// @license      MIT
// @icon https://img.icons8.com/nolan/452/fresco.png
// @downloadURL https://update.greasyfork.org/scripts/452997/BlackRussia%20CHOCO%20by%20KMironova.user.js
// @updateURL https://update.greasyfork.org/scripts/452997/BlackRussia%20CHOCO%20by%20KMironova.meta.js
// ==/UserScript==
(function () {
  'use strict';
...
const buttons = [
    ...
    },
    {
      ...
      content:
      '[CENTER][COLOR=rgb(30, 144, 255)][FONT=arial][SIZE=3][B]Доброго времени суток, уважаемый(-ая) {{ user.mention }}. [/SIZE][/FONT][/COLOR]<br><br>' +
      "[FONT=arial][SIZE=3]Ваша жалоба взята на [COLOR=#ff4500]рассмотрение[/COLOR][COLOR=rgb(255, 255, 255)].[/COLOR] [/FONT][/SIZE]<br><br>" +
      '[SIZE=3][COLOR=rgb(255, 255, 0)][FONT=arial][B]Ожидайте ответа.[/B][/FONT][/COLOR][/SIZE][/CENTER]',
      prefix: PIN_PREFIX,
      status: true,
    },
    {
      title: 'DM',
      content:
        '[CENTER][B][COLOR=rgb(30, 144, 255)][SIZE=3][FONT=arial]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/FONT][/SIZE][/COLOR][/B]<br><br>' +
        "[SIZE=3][FONT=arial][B]Игрок будет наказан по следующему пункту общих правил серверов:<br><br>" +
        ...
        "[COLOR=rgb(255, 0, 0)]Примечание:[/COLOR] разрешен ответный DM в целях защиты, обязательно иметь видео доказательство в случае наказания администрации, нанесение урона по транспорту также является нарушением данного пункта правил.<br>[COLOR=rgb(255, 0, 0)]Примечание:[/COLOR] нанесение урона с целью защиты особняка или его территории, а также нанесение урона после ДТП не является веской IC причиной, для войны семей предусмотрено отдельное системное мероприятие.[/B][/FONT][/SIZE]<br><br>" +
        '[B][COLOR=rgb(0, 255, 0)][SIZE=3][FONT=arial]Одобрено.[/FONT][/SIZE][/COLOR][/B][/CENTER]',
    ...
    },
    {
      title: 'MG',
      content:
        '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE]<br><br>' +
        "[B][SIZE=3][FONT=arial]Нарушитель буден наказан по следующему пункту общих правил серверов:<br><br>" +
        "[COLOR=rgb(255, 0, 0)]2.18. [/COLOR]Запрещен MG (MetaGaming) — использование ООС информации, которую Ваш персонаж никак не мог получить в IC процессе [COLOR=rgb(255, 0, 0)]| Mute 30 минут<br><br>" +
        "Примечание: [/COLOR]использование смайлов в виде символов «))», «=D» запрещено в IC чате.<br>[COLOR=rgb(255, 0, 0)]Примечание:[/COLOR] телефонное общение также является IC чатом.<br>[COLOR=rgb(255, 0, 0)]Исключение: [/COLOR]за написанный однократно вопросительный «?» или восклицательный «!» знак в IC чате, наказание не выдается.[/FONT][/SIZE][/B]<br><br>" +
        '[SIZE=3][FONT=arial][B][COLOR=rgb(0, 255, 0)]Одобрено.[/COLOR][/B][/FONT][/SIZE][/CENTER]',
      prefix: ACCEPT_PREFIX,
      status: false,
    },
    {
    ...
    },
    {
      ...
      content:
        '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE]<br><br>' +
        "[B][SIZE=3][FONT=arial]Игрок будет наказан по следующему пункту общих правил серверов:<br>[COLOR=rgb(255, 0, 0)]3.02.[/COLOR] Запрещено использование верхнего регистра (CapsLock) при написании любого текста в любом чате [COLOR=rgb(255, 0, 0)]| Mute 30 минут[/COLOR].[/FONT][/SIZE][/B]<br><br>" +
        '[SIZE=3][FONT=arial][B][COLOR=rgb(0, 255, 0)]Одобрено.[/COLOR][/B][/FONT][/SIZE][/CENTER]',
      prefix: ACCEPT_PREFIX,
      status: false,
    },
    {
      ...
      content:
        '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE]<br><br>' +
        "[B][SIZE=3][FONT=arial]Нарушитель буден наказан по следующему пункту общих правил серверов:<br><br>" +
        "[COLOR=rgb(255, 0, 0)]3.05.[/COLOR] Запрещен флуд — 3 и более повторяющихся сообщений от одного и того же игрока [COLOR=rgb(255, 0, 0)]| Mute 30 минут[/COLOR][/FONT][/SIZE][/B]<br><br>" +
        '[SIZE=3][FONT=arial][B][COLOR=rgb(0, 255, 0)]Одобрено.[/COLOR][/B][/FONT][/SIZE][/CENTER]',
         prefix: ACCEPT_PREFIX,
      status: false,
    },
    ...
    },
    {
      ...
      content:
        '[CENTER][COLOR=rgb(30, 144, 255)][FONT=arial][SIZE=3][B]Доброго времени суток, уважаемый(ая) {{ user.mention }}. [/SIZE][/FONT][/COLOR]<br><br>' +
        "[B][SIZE=3][FONT=arial][COLOR=rgb(255, 255, 255)]Нарушитель буден наказан по следующему пункту общих правил серверов:[/COLOR]<br><br>" +
        "[COLOR=rgb(255, 0, 0)]2.05.[/COLOR][COLOR=rgb(209, 213, 216)] [/COLOR][COLOR=rgb(255, 255, 255)]Запрещены любые OOC обманы и их попытки, а также любые IC обманы с нарушением Role Play правил и логики [/COLOR][COLOR=rgb(255, 0, 0)]| PermBan<br><br>" +
        "Примечание: [/COLOR][COLOR=rgb(255, 255, 255)]после IC договоренности получить денежные средства и сразу же выйти из игры с целью обмана игрока, или же, договорившись через OOC чат (/n), точно также получить денежные средства и сразу же выйти из игры и тому подобные ситуации.[/COLOR]<br>[COLOR=rgb(255, 0, 0)]Примечание: [/COLOR][COLOR=rgb(255, 255, 255)]разблокировка игрового аккаунта нарушителя будет возможна только в случае возврата полной суммы причиненного ущерба, либо непосредственно самого имущества, которое было украдено (по решению обманутой стороны).[/COLOR][/FONT][/SIZE][/B]<br><br>" +
        '[SIZE=3][FONT=arial][COLOR=rgb(0, 255, 0)][B]Одобрено.[/B][/COLOR][/FONT][/SIZE][/CENTER]',
      prefix: ACCEPT_PREFIX,
      status: false,
    },
     {
      title: 'Донат/не донат авто',
      content:
        '[CENTER][COLOR=rgb(255, 0, 0)][FONT=times new roman][SIZE=4][I]{{ greeting }}, уважаемый {{ user.mention }}.[/I][/SIZE][/FONT][/COLOR]<br><br>' +
        "[I][COLOR=rgb(209, 213, 216)][FONT=times new roman][SIZE=4]Цены авто для продажи государству, с 02.06.22 донат и не донат авто одинаковы (приравнены к старым ценам донат авто) [/SIZE][/FONT][/COLOR][/I]<br><br>" +
        "[FONT=book antiqua][SIZE=4][COLOR=rgb(255, 0, 0)][/COLOR][COLOR=rgb(209, 213, 216)] Примечание: если авто куплено до 2.06.22 человек видит старую цену продажи в госс, а у Вас она будет новая [/COLOR][COLOR=rgb(255, 0, 0)] | PermBan[/COLOR][/SIZE][/FONT]<br><br>" +
        "[I][COLOR=rgb(209, 213, 216)][FONT=times new roman][SIZE=4]Приятной игры на BLACK RUSSIA [/SIZE][/FONT][/COLOR][COLOR=rgb(139, 69, 19)][FONT=times new roman][SIZE=4]CHOCO[/SIZE][/FONT][/COLOR][COLOR=rgb(209, 213, 216)][FONT=times new roman][SIZE=4].[/SIZE][/FONT][/COLOR][/I]<br><br>" +
    ...
    },
    {
      title: 'Упом род',
      content:
      '[CENTER][COLOR=rgb(30, 144, 255)][FONT=arial][SIZE=3][B]Доброго врмени суток, уважаемый(-ая) {{ user.mention }}.[/SIZE][/FONT][/COLOR]<br><br>' +
      "[B][B][SIZE=3][FONT=arial][COLOR=rgb(255, 255, 255)]Нарушитель буден наказан по следующему пункту общих правил серверов:[/COLOR]<br><br>" +
      "[COLOR=rgb(255, 0, 0)]3.04. [/COLOR][COLOR=rgb(255, 255, 255)]Запрещено оскорбление или косвенное упоминание родных вне зависимости от чата (IC или OOC)[/COLOR] [COLOR=rgb(255, 0, 0)]| Mute 120 минут / Ban 7 - 15 дней.<br>Примечание:[/COLOR][COLOR=rgb(255, 255, 255)] термин «MQ» расценивается, как упоминание родных.[/COLOR]<br><br>" +
      "[COLOR=rgb(255, 0, 0)]Исключение: [/COLOR][COLOR=rgb(255, 255, 255)]если упоминание родных было совершено в ходе Role Play процесса и не содержало в себе прямого или завуалированного оскорбления.[/COLOR] [/FONT][/SIZE][/B]<br><br>" +
      '[COLOR=rgb(0, 255, 0)][FONT=arial][SIZE=3][B]Одобрено.[/B][/SIZE][/FONT][/COLOR][/B][/CENTER]',
      prefix: ACCEPT_PREFIX,
      status: false,
    },
    {
title: 'оск род войс',
      content:
        '[CENTER][B][COLOR=rgb(30, 144, 255)][SIZE=3][FONT=arial]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/FONT][/SIZE][/COLOR]<br><br>' +
        "[COLOR=rgb(255, 255, 255)][SIZE=3][FONT=arial]Нарушитель будет наказан по следующему пункту общих правил серверов:[/FONT][/SIZE][/COLOR][/B]<br>[SIZE=3][FONT=arial][B][COLOR=rgb(255, 0, 0)]3.15. [/COLOR]Запрещено оскорблять игроков или родных в Voice Chat [COLOR=rgb(255, 0, 0)]| Mute 120 минут / Ban 7 - 15 дней[/COLOR].[/B][/FONT][/SIZE]<br><br>" +
        '[B][COLOR=rgb(0, 255, 0)][SIZE=3][FONT=arial]Одобрено.[/FONT][/SIZE][/COLOR][/B][/CENTER]',
      prefix: ACCEPT_PREFIX,
      status: false,
    },
...
title: 'реклама войс',
    ...
    },
    {
      ...
      content:
        '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE]<br><br>' +
        "[B][SIZE=3][COLOR=rgb(255, 255, 255)][FONT=arial][/FONT][/COLOR][FONT=arial][COLOR=rgb(255, 255, 255)]Игрок будет наказан по следующему пункту общих правил серверов:[/COLOR]<br><br>" +
        "[COLOR=rgb(255, 0, 0)]3.07.[/COLOR][COLOR=rgb(209, 213, 216)] [/COLOR][COLOR=rgb(255, 255, 255)]Запрещены совершенно любые оскорбления или действия, порочащие честь и достоинства, несущие в себе подтекст сексуального характера вне зависимости от чата[/COLOR][COLOR=rgb(255, 0, 0)] | Mute 30 минут<br>Примечание: [/COLOR][COLOR=rgb(255, 255, 255)]«дырка», «шмара», «ведро», «мадагаскарский присосконог», «свиноногий бандикут», «скорострел» и так далее.[/COLOR][/FONT][/SIZE][/B]<br><br>" +
        '[COLOR=rgb(0, 255, 0)][FONT=arial][SIZE=3][B]Одобрено.[/B][/SIZE][/FONT][/COLOR][/CENTER]',
      prefix: ACCEPT_PREFIX,
      status: false,
    },
    {
    ...
    },
    {
         title: 'Слив СМИ',
      content:
       '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE][COLOR=rgb(30, 144, 255)] [/COLOR]<br><br>' +
       "[COLOR=rgb(255, 255, 255)][B][SIZE=3][FONT=arial]Игрок будет наказан по следующему пункту общих правил серверов:[/FONT][/SIZE][/B][/COLOR]<br>[COLOR=rgb(30, 144, 255)][B][SIZE=3][FONT=arial][COLOR=rgb(255, 0, 0)]3.08. [/COLOR][/FONT][/SIZE][/B][/COLOR][COLOR=rgb(255, 255, 255)][B][SIZE=3][FONT=arial]Запрещены любые формы «слива» посредством использования глобальных чатов[/FONT][/SIZE][/B][/COLOR][COLOR=rgb(30, 144, 255)][B][SIZE=3][FONT=arial] [COLOR=rgb(255, 0, 0)]| PermBan [/COLOR][/FONT][/SIZE][/B][/COLOR]<br><br>" +
       '[COLOR=rgb(30, 144, 255)][SIZE=3][FONT=arial][B][COLOR=rgb(0, 255, 0)]Одобрено.[/COLOR][/B][/FONT][/SIZE][/COLOR][/CENTER]',
        prefix: ACCEPT_PREFIX,
      status: false,
    },
    {
...
title: 'не на русском в чате',
    ...
    },
    {
      title : 'Оск проекта' , 
     content:
      '[CENTER][B][COLOR=rgb(30, 144, 255)][FONT=arial][SIZE=3]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/SIZE][/FONT][/COLOR][/B]<br><br>' +
      "[FONT=arial][SIZE=3][B]Игрок будет наказан по следующему пункту общих правил серверов:[/B]<br><br>" +
      "[COLOR=rgb(255, 0, 0)]2.40.[/COLOR][B] Запрещены совершенно любые деструктивные действия по отношению к проекту: неконструктивная критика, призывы покинуть проект, попытки нарушить развитие проекта или любые другие действия, способные привести к помехам в игровом процессе[COLOR=rgb(255, 0, 0)] | Mute 300 минут / Ban 30 дней (Ban выдается по согласованию с главным администратором)[/B][/COLOR].<br><br>" +
      '[COLOR=rgb(0, 255, 0)]Одобрено.[/COLOR] [/SIZE][/FONT][/CENTER]',
      prefix: ACCEPT_PREFIX,
      status: false,
    },
    {
    ...
    },
    {
      ...
      content:
        '[CENTER][B][COLOR=rgb(30, 144, 255)][FONT=arial][SIZE=3]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/SIZE][/FONT][/COLOR][/B]<br><br>' +
        "[FONT=arial][SIZE=3][B]Нарушитель буден наказан по следующему пункту правил государственных структур:<br>[COLOR=rgb(255, 0, 0)]6.03.[/COLOR] Запрещено оказывать задержание без Role Play отыгровки [COLOR=rgb(255, 0, 0)]| Warn[/COLOR][/B][/SIZE][/FONT]<br><br>" +
        '[B][FONT=arial][SIZE=3][COLOR=rgb(0, 255, 0)]Одобрено.[/COLOR] [/SIZE][/FONT][/B][/CENTER]',
      prefix: ACCEPT_PREFIX,
      status: false,
    },
    {
         title: 'DM госс',
      content:
        '[CENTER][FONT=arial][SIZE=3][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/SIZE][/FONT][COLOR=rgb(30, 144, 255)][B][FONT=arial][SIZE=3] [/SIZE][/FONT][/B][/COLOR]<br><br>' +
        "[B][FONT=arial][SIZE=3]Игрок будет наказан по следующему пункту правил государственных организаций:<br><br>" +
        "[COLOR=rgb(255, 0, 0)]6.03. [/COLOR]Запрещено nRP поведение [COLOR=rgb(255, 0, 0)]| Warn<br>Примечание:[/COLOR] поведение, не соответствующее сотруднику УМВД.<br>[COLOR=rgb(255, 0, 0)]Пример[/COLOR]:<br>- открытие огня по игрокам без причины,<br>- расстрел машин без причины,<br>- нарушение ПДД без причины,<br>- сотрудник на служебном транспорте кричит о наборе в свою семью на спавне. [/SIZE][COLOR=rgb(0, 255, 0)][SIZE=3][/SIZE][/COLOR][/FONT][/B]<br><br>" +
        '[FONT=arial][SIZE=3][B][COLOR=rgb(0, 255, 0)]Одобрено.[/COLOR][/B][/SIZE][/FONT][/CENTER]',
         prefix: ACCEPT_PREFIX,
      status: false,
    },
    {
    ...
    },
    {
      ...
      content:
       '[CENTER][B][COLOR=rgb(30, 144, 255)][FONT=arial][SIZE=3]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/SIZE][/FONT][/COLOR][/B]<br><br>' +
       "[FONT=arial][SIZE=3][B]Ваша жалоба переадресована [COLOR=rgb(255, 140, 0)]Техническому Специалисту[/COLOR].[/B][/SIZE][/FONT]<br><br>" +
       '[B][COLOR=rgb(255, 255, 0)][FONT=arial][SIZE=3]Ожидайте ответа в данной теме.[/SIZE][/FONT][/COLOR][/B][/CENTER]',
      prefix: TEX_PREFIX,
      status: true,
    },
    {
      ...
      content:
       '[CENTER][COLOR=rgb(30, 144, 255)][FONT=arial][SIZE=3][B]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/SIZE][/FONT][/COLOR]<br><br>' +
       "[B][SIZE=3][FONT=arial][COLOR=rgb(255, 255, 255)]Недостаточно доказательств для корректного рассмотрения вашей жалобы.[/COLOR][/FONT][/SIZE][/B]<br><br>" +
       '[COLOR=rgb(255, 0, 0)][SIZE=3][FONT=arial][B]Отказано.[/B][/FONT][/SIZE][/COLOR][/CENTER]',
      prefix: UNACCEPT_PREFIX,
      status: false,
    },
    {
      ...
      content:
       '[CENTER][B][COLOR=rgb(30, 144, 255)][FONT=arial][SIZE=3]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/SIZE][/FONT][/COLOR][/B]<br><br>' +
       "[FONT=arial][SIZE=3][B]Отсутствуют доказательства - следовательно, рассмотрению не подлежит. Загрузите доказательства на фото-видео хостинги [COLOR=rgb(255, 69, 0)]YouTube[/COLOR],[COLOR=rgb(50, 205, 50)] Imgur[/COLOR], [COLOR=rgb(153, 50, 204)]Yapix[/COLOR] и так далее. [/B][/SIZE][/FONT]<br><br>" +
       '[B][COLOR=rgb(255, 0, 0)][FONT=arial][SIZE=3]Отказано.[/SIZE][/FONT][/COLOR][/B][/CENTER]',
      prefix: UNACCEPT_PREFIX,
      status: false,
    },
    {
      ...
      content:
        '[CENTER][FONT=arial][SIZE=3][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/SIZE][/FONT]<br><br>' +
        "[B][FONT=arial][SIZE=3]Доказательства в социальных сетях и т.д. не принимаются. Загрузите доказательства на фото-видео хостинги [COLOR=rgb(255, 69, 0)]YouTube[/COLOR],[COLOR=rgb(50, 205, 50)] Imgur[/COLOR], [COLOR=rgb(153, 50, 204)]Yapix[/COLOR] и так далее.[/SIZE][/FONT][/B]<br><br>" +
        '[FONT=arial][SIZE=3][B][COLOR=rgb(255, 0, 0)]Закрыто.[/COLOR][/B][/SIZE][/FONT][/CENTER]',
      prefix: UNACCEPT_PREFIX,
      status: false,
    },
    {
      title: 'Нарушений нет',
      content:
       '[CENTER][COLOR=rgb(30, 144, 255)][FONT=arial][SIZE=3][B]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/B][/SIZE][/FONT][/COLOR][B][FONT=arial][SIZE=3]<br><br>' +
       "[COLOR=rgb(255, 255, 255)]Нарушений со стороны игрока не было замечено.[/COLOR][/SIZE][/FONT][/B]<br>[COLOR=rgb(255, 255, 255)][B][FONT=arial][SIZE=3]Внимательно изучите общие правила серверов - [URL='https://forum.blackrussia.online/index.php?threads/%D0%9E%D0%B1%D1%89%D0%B8%D0%B5-%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0-%D1%81%D0%B5%D1%80%D0%B2%D0%B5%D1%80%D0%BE%D0%B2.312571/']*Нажмите сюда*[/URL][/SIZE][/FONT][/B][/COLOR][B][FONT=arial][SIZE=3][/SIZE][/FONT][/B]<br><br>" +
       '[COLOR=rgb(255, 0, 0)][SIZE=3][FONT=arial][B]Отказано.[/B][/FONT][/SIZE][/COLOR][/CENTER]',
      prefix: UNACCEPT_PREFIX,
      status: false,
    },
    {
      ...
      content:
       '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE]<br><br>' +
       "[B][SIZE=3][FONT=arial]На доказательствах отсутствуют дата и время - следовательно, рассмотрению не подлежит.[/FONT][/SIZE][/B]<br><br>" +
       '[SIZE=3][FONT=arial][B][COLOR=rgb(255, 0, 0)] Отказано.[/COLOR][/B][/FONT][/SIZE][/CENTER]',
      prefix: UNACCEPT_PREFIX,
      status: false,
    },
    {
      ...
      content:
        '[CENTER][B][COLOR=rgb(30, 144, 255)][SIZE=3][FONT=arial]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/FONT][/SIZE][/COLOR][/B]<br><br>' +
        "[COLOR=rgb(255, 255, 255)][SIZE=3][FONT=arial][B]Доказательства были подвергнуты редактированию - следовательно, рассмотрению не подлежит.<br>Загрузите оригиналы видеозаписи / скриншотов, создав новую тему в данном разделе.[/B][/FONT][/SIZE][/COLOR]<br><br>" +
        '[B][COLOR=rgb(255, 0, 0)][SIZE=3][FONT=arial]Отказано.[/FONT][/SIZE][/COLOR][COLOR=rgb(255, 255, 255)][SIZE=3][FONT=arial] [/FONT][/SIZE][/COLOR][/B][/CENTER]',
      prefix: UNACCEPT_PREFIX,
      status: false,
    },
    {
     ...
     content:
        '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE][B][SIZE=3][FONT=arial]<br><br>' +
        "[COLOR=rgb(255, 255, 255)]Ваши доказательства не работают или же битая ссылка. [/COLOR][/FONT][/SIZE][/B]<br><br>" +
        '[COLOR=rgb(255, 0, 0)][SIZE=3][FONT=arial][B]Отказано.[/B][/FONT][/SIZE][/COLOR][/CENTER]',
      prefix: UNACCEPT_PREFIX,
      status: false,
    },
    {
     ...
     content:
        '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE][B][SIZE=3][COLOR=rgb(30, 144, 255)][FONT=arial] [/FONT][/COLOR]<br><br>' +
        "[FONT=arial][/FONT][/SIZE][/B][SIZE=3][FONT=arial][B]В предоставленных доказательствах отсутствуют условия сделки.<br><br>" +
        '[COLOR=rgb(255, 0, 0)]Закрыто.[/COLOR][/B][/FONT][/SIZE][/CENTER]',
        prefix: UNACCEPT_PREFIX,
      status: false,
     },
    {
      ...
      content:
       '[CENTER][FONT=arial][SIZE=3][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(ая) {{ user.mention }}.[/COLOR][/B][/SIZE][/FONT]<br><br>' +
       "[B][FONT=arial][SIZE=3]Ваша видеозапись обрывается. Загрузите полную видеозапись на видео-хостинг [COLOR=rgb(255, 69, 0)]YouTube[/COLOR].[/SIZE][/FONT][/B]<br><br>" +
       '[FONT=arial][SIZE=3][B][COLOR=rgb(255, 0, 0)]Закрыто.[/COLOR][/B][/SIZE][/FONT][/CENTER]',
      prefix: UNACCEPT_PREFIX,
      status: false,
    },
    ...
    },
    {
      title : 'Нет таймкодов'  ,
     content:
       '[CENTER][COLOR=rgb(30, 144, 255)][FONT=arial][SIZE=3][B]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/B][/SIZE][/FONT][/COLOR] [FONT=arial][SIZE=3][B]<br><br>' +
       "[COLOR=rgb(255, 255, 255)]Ваша жалоба отказана, т.к в ней отсутствуют таймкоды.<br>Если видео длится больше 3-ех минут - Вы должны указать таймкоды нарушений.[/COLOR][/B][/SIZE][/FONT]<br><br>" +
       '[SIZE=4][COLOR=rgb(255, 0, 0)][SIZE=3][FONT=arial][B]Отказано.[/B][/FONT][/SIZE][/COLOR][/SIZE][/CENTER]',
     prefix: UNACCEPT_PREFIX,
     status: false,
    },
    {
      ...
      content:
        '[CENTER][B][COLOR=rgb(30, 144, 255)][SIZE=3][FONT=arial]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/FONT][/SIZE][/COLOR][/B]<br><br>' +
        "[SIZE=3][FONT=arial][B]Ваша жалоба отказана, так как с момента нарушения прошло более 72 часов (3 дня).[/B][/FONT][/SIZE]<br><br>" +
        '[B][COLOR=rgb(255, 0, 0)][SIZE=3][FONT=arial]Закрыто.[/FONT][/SIZE][/COLOR][/B][/CENTER]',
     prefix: UNACCEPT_PREFIX,
      status: false, 
    },
    {
     ...
     content:
        '[CENTER][FONT=arial][SIZE=3][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/SIZE][/FONT]<br><br>'+
        "[B][FONT=arial][SIZE=3]Ваша жалоба отказана, т.к она написана от 3-его лица.[/SIZE][/FONT][/B]<br><br>"+
        '[FONT=arial][SIZE=3][B][COLOR=rgb(255, 0, 0)]Закрыто.[/COLOR][/B][/SIZE][/FONT][/CENTER]',
     prefix: UNACCEPT_PREFIX,
      status: false, 
    },
    ...
    },
    {
      title: 'Не по форме',
      content:
       '[CENTER][B][FONT=arial][SIZE=3][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/SIZE][/FONT][/B]<br><br>' +
       "[FONT=arial][SIZE=3][B][COLOR=rgb(255, 255, 255)]Ваша жалоба составлена не по форме. Внимательно прочитайте правила подачи жалоб на игроков, закрепленные в этом разделе.[/COLOR][/B][/SIZE][/FONT]<br><br>" +
       '[COLOR=rgb(255, 0, 0)][SIZE=3][FONT=arial][B]Отказано.[/B][/FONT][/SIZE][/COLOR][/CENTER]',
      prefix: UNACCEPT_PREFIX,
      status: false,
    },
...
title: 'нонрп обман донат/не донат',
     ...
     },
    {
      title: 'Нет долгов',
      content:
        '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE][COLOR=rgb(30, 144, 255)][B][SIZE=3][FONT=arial] [/FONT][/SIZE][/B][/COLOR]<br><br>' +
        "[B][SIZE=3][FONT=arial]Правил долгов / кредитов нет.<br><br>" +
        '[/FONT][/SIZE][/B][SIZE=3][FONT=arial][B][COLOR=rgb(255, 0, 0)]Отказано.[/COLOR] [/B][/FONT][/SIZE][/CENTER]',
      prefix: UNACCEPT_PREFIX,
      status: false,
    },
    {
      ...
      content:
     '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE][B][SIZE=3][COLOR=rgb(30, 144, 255)][FONT=arial] [/FONT][/COLOR]<br><br>' +
     "[FONT=arial]Игрок будет наказан по следующему пункту общих правил серверов:r><br>" +
     "[COLOR=rgb(255, 0, 0)]7.02.[/COLOR] Запрещено выдавать розыск, штраф без Role Play причины[COLOR=rgb(255, 0, 0)] | Jail 30 минут[/COLOR][/FONT][/SIZE][/B]<br><br>" +
     '[SIZE=3][FONT=arial][B][COLOR=rgb(0, 255, 0)]Одобрено.[/COLOR][/B][/FONT][/SIZE][/CENTER]',
      prefix: ACCEPT_PREFIX,
      status: false,
    },
    {
      ...
      content:
    '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE]<br><br>' +
    "[B][SIZE=3][FONT=arial][COLOR=rgb(255, 255, 255)]Ваша жалоба отказана по причине: нельзя писать одну жалобу на двух и белее игроков (на каждого игрока отдельная жалоба).[/COLOR][/FONT][/SIZE][/B]<br><br>" +
    '[COLOR=rgb(255, 0, 0)][SIZE=3][FONT=arial][B]Отказано.[/B][/FONT][/SIZE][/COLOR][/CENTER]',
          prefix: UNACCEPT_PREFIX,
      status: false,
    },
    {
      ...
      content:
     '[CENTER][FONT=arial][SIZE=3][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR] [/B][/SIZE][/FONT]<br><br>' +
     "[B][FONT=arial][SIZE=3][COLOR=rgb(255, 255, 255)]Ваша жалоба отказана, т.к ранее уже был дан корректный ответ.[/COLOR][/SIZE][/FONT][/B]<br><br>" +
     '[SIZE=4][COLOR=rgb(255, 0, 0)][SIZE=3][FONT=arial][B]Отказано.[/B][/FONT][/SIZE][/COLOR][/SIZE][/CENTER]',
          prefix: UNACCEPT_PREFIX,
      status: false,
    },
    {
      ...
      content:
      '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE]<br><br>' +
      "[B][SIZE=3][FONT=arial]Обратитесь в раздел жалобы на сотрудников данной организации.[/FONT][/SIZE][/B]<br><br>" +
      '[SIZE=3][FONT=arial][B][COLOR=rgb(255, 0, 0)]Закрыто.[/COLOR][/B][/FONT][/SIZE][/CENTER]',
          prefix: UNACCEPT_PREFIX,
      status: false,
    },
    {
        title: 'NonRP редактирование',
      content:
      '[CENTER][SIZE=3][FONT=arial][B][COLOR=rgb(30, 144, 255)]Доброго времени суток, уважаемый(-ая) {{ user.mention }}.[/COLOR][/B][/FONT][/SIZE]<br><br>' +
      "[B][SIZE=3][FONT=arial]Игрок будет наказан по следующему пункту правил государственных организаций:<br>[COLOR=rgb(255, 0, 0)]4.04.[/COLOR] Запрещено редактировать поданные объявления в личных целях заменяя текст объявления на несоответствующий отправленному игроком [COLOR=rgb(255, 0, 0)]| Ban 7 дней + ЧС организации[/COLOR].[/FONT][/SIZE][/B]<br><br>" +
      '[SIZE=3][FONT=arial][B][COLOR=rgb(0, 255, 0)]Одобрено.[/COLOR][/B][/FONT][/SIZE][/CENTER]',
        prefix: ACCEPT_PREFIX,
      status: false,
    },
