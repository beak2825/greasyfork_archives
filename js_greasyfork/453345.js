...
// @downloadURL https://update.greasyfork.org/scripts/453345/POI%E8%AF%84%E8%AE%BA%E4%BF%A1%E6%81%AF%E9%87%87%E9%9B%86%E5%B7%A5%E5%85%B7forDianping.user.js
// @updateURL https://update.greasyfork.org/scripts/453345/POI%E8%AF%84%E8%AE%BA%E4%BF%A1%E6%81%AF%E9%87%87%E9%9B%86%E5%B7%A5%E5%85%B7forDianping.meta.js
// ==/UserScript==
...
(function () {
  ...
  async function main(totalpages, urlList) {
    const min = 3;
    const max = 15;
    for (let i = 0; i < totalpages; i++) {
      console.log(urlList[i]);
      let popup = window.open(',');
      popup.location.href = urlList[i];
      const moreBtns = $(moreBtnClass);
      moreBtns.forEach(b => {
        b.click();
        b.style.opacity = 0;
      ...
      // 生成随机数，暂停X秒
      var rtime = Math.random() * (max - min) + min;
      await sleep(rtime);
      //点击下一页按钮
      // next.click(); 
    }
    console.log('全部页面信息采集完成');
  };
  autoExe.onclick = () => {
    ...
    var urlList = [];
    var url_c = "https://www.dianping.com/shop/H8sOT7dhpFprpFAZ/review_all";
    urlList.push(url_c);
    for (let j = 2; j <= pages; j++) {
      url_c = "https://www.dianping.com/shop/H8sOT7dhpFprpFAZ/review_all/p" + j.toString();
      urlList.push(url_c);
    }
    console.log(urlList);
    main(2, urlList);  // 测试执行                  
