...
// @downloadURL https://update.greasyfork.org/scripts/530847/Monitor%20Popmundo%2050%20%F0%9F%A4%93.user.js
// @updateURL https://update.greasyfork.org/scripts/530847/Monitor%20Popmundo%2050%20%F0%9F%A4%93.meta.js
// ==/UserScript==

(function() {
    'use strict';

    // Configuração da validação de API - URL ATUALIZADA
    const API_URL = 'https://script.google.com/macros/s/AKfycby4KR4vMj3DzcmpXzE_H9lusQUNQS5XMg37PCf7ML_6sb6Uz-anrxc1JvZ195oW0BdJ/exec';
    let apiKey = GM_getValue('apiKey', '');
    let isApiKeyValidated = GM_getValue('isApiKeyValidated', false);
    let apiKeyValidationInProgress = false;
    ...
    const ITEMS_PAGE_URL = '/World/Popmundo.aspx/Character/Items/';
    const IMPROVE_PAGE_URL = '/World/Popmundo.aspx/Character/ImproveCharacter/';
    const ACTION_DELAY = 14000; // 14 segundos

    // Função para gerar/obter ID do dispositivo
    function getDeviceId() {
        let deviceId = localStorage.getItem('myScript_deviceId');

        if (!deviceId) {
            // Gera um ID único combinando informações do navegador
            ...
            // Criar uma string única combinando múltiplos fatores
            const uniqueString = navegador + resolucao + timestamp + timezone + idioma;
            deviceId = btoa(uniqueString).replace(/=/g, '').slice(0, 24);
            localStorage.setItem('myScript_deviceId', deviceId);
        }

        return deviceId;
    }

    // Função para validar a chave de API (com cache para evitar validações repetidas)
    function validateApiKey(callback, forceValidation = false) {
        // Se a chave já foi validada e não estamos forçando uma nova validação, retornar imediatamente
        if (isApiKeyValidated && !forceValidation) {
            if (callback) callback(true, 'Chave já validada');
            return;
        }

        if (apiKeyValidationInProgress) return;

        apiKeyValidationInProgress = true;
        const deviceId = getDeviceId();

        addToStatusLog('🔑 Validando chave API...');
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                try {
                    ...
                    isApiKeyValidated = result.success;
                    GM_setValue('isApiKeyValidated', isApiKeyValidated);

                    if (isApiKeyValidated) {
                        addToStatusLog('✅ Chave API validada com sucesso!');
                    } else {
                        addToStatusLog(`❌ Validação da chave API falhou: ${result.message}`);
                    }

                    apiKeyValidationInProgress = false;
                    if (callback) callback(isApiKeyValidated, result.message);
                    updateStatusDisplay();
                } catch(e) {
                    addToStatusLog('❌ Erro ao processar resposta de validação: ' + e.toString());
                    apiKeyValidationInProgress = false;
                    isApiKeyValidated = false;
                    GM_setValue('isApiKeyValidated', false);
                    if (callback) callback(false, 'Erro ao processar resposta');
                    updateStatusDisplay();
                }
            },
            onerror: function(error) {
                addToStatusLog('❌ Erro na requisição de validação');
                apiKeyValidationInProgress = false;
                isApiKeyValidated = false;
                GM_setValue('isApiKeyValidated', false);
                if (callback) callback(false, 'Erro na conexão');
                updateStatusDisplay();
            }
        });
    }

    // Função para solicitar nova chave API
    function requestApiKey() {
        const newApiKey = prompt("Digite sua chave de API para ativar o script:");
        if (newApiKey) {
            apiKey = newApiKey.trim();
            GM_setValue('apiKey', apiKey);

            validateApiKey(function(valid, message) {
                if (valid) {
                    addToStatusLog('✅ Nova chave API validada com sucesso!');
                    runInitialization(); // Iniciar o script após validação bem-sucedida
                } else {
                    alert(`Validação falhou: ${message}\nTente novamente com uma chave válida.`);
                    requestApiKey(); // Pedir novamente se a validação falhou
                }
            }, true); // Forçar validação
        } else {
            alert("Uma chave API válida é necessária para usar este script.");
        }
    }
    ...
    function sendTelegramMessage(message, maxRetries = 3, currentRetry = 0) {
        if (!isApiKeyValidated) {
            addToStatusLog('⚠️ Mensagem não enviada: Chave API não validada');
            return;
        }
    ...
    function navigateToImprovePage() {
        if (!isApiKeyValidated) {
            addToStatusLog('⚠️ Navegação bloqueada: Chave API não validada');
            return;
        }
    ...
    function performAutoLogin() {
        if (!isApiKeyValidated) {
            addToStatusLog('⚠️ Login bloqueado: Chave API não validada');
            return;
        }
    ...
    function selectCharacter() {
        if (!isApiKeyValidated) {
            addToStatusLog('⚠️ Seleção de personagem bloqueada: Chave API não validada');
            return;
        }
    ...
    function useNoobTubeItem() {
        if (!isApiKeyValidated) {
            addToStatusLog('⚠️ Uso de item bloqueado: Chave API não validada');
            return;
        }
    ...
    function checkCurrentPage() {
        if (!isApiKeyValidated) {
            addToStatusLog('⚠️ Verificação de página bloqueada: Chave API não validada');
            return;
        }
    ...
    function clickHealthBoostButton() {
        if (!isApiKeyValidated) {
            addToStatusLog('⚠️ Aprimoramento de saúde bloqueado: Chave API não validada');
            return;
        }
    ...
    function clickMoodBoostButton() {
        if (!isApiKeyValidated) {
            addToStatusLog('⚠️ Aprimoramento de humor bloqueado: Chave API não validada');
            return;
        }
    ...
    function checkUrlChange() {
        if (!isApiKeyValidated || !isMonitoringActive) return;
    ...
    function checkAttributes() {
        if (!isApiKeyValidated || !isMonitoringActive) {
            return;
        }
    ...
    function checkMinimumHealth(healthValue) {
        if (!isApiKeyValidated || !isMonitoringActive || healthValue === null) return;
    ...
    function checkMinimumMood(moodValue) {
        if (!isApiKeyValidated || !isMonitoringActive || moodValue === null) return;
    ...
    function updateStatusDisplay() {
        ...
        if (statusIndicator) {
            statusIndicator.style.backgroundColor = isMonitoringActive && isApiKeyValidated ? '#4CAF50' : '#FF5252';
        }
        ...
        // Atualizar status da chave API
        const apiKeyStatusDisplay = document.getElementById('apiKeyStatus');
        if (apiKeyStatusDisplay) {
            apiKeyStatusDisplay.textContent = isApiKeyValidated ? '✅ Chave válida' : '❌ Chave não validada';
            apiKeyStatusDisplay.style.color = isApiKeyValidated ? '#4CAF50' : '#FF5252';
        }
    }
    ...
    function setRefreshInterval(interval) {
        if (!isApiKeyValidated) {
            addToStatusLog('⚠️ Atualização automática bloqueada: Chave API não validada');
            return;
        }
        ...
        if (isMonitoringActive && interval > 0) {
            refreshIntervalId = setInterval(function() {
                if (!isApiKeyValidated) {
                    clearInterval(refreshIntervalId);
                    refreshIntervalId = null;
                    addToStatusLog('⚠️ Atualização automática desativada: Chave API não validada');
                    return;
                }
    ...
    function createConfigPanel() {
        ...
        statusIndicator.style.cssText = `
            ...
            height: 12px;
            border-radius: 50%;
            background-color: ${isMonitoringActive && isApiKeyValidated ? '#4CAF50' : '#FF5252'};
            box-shadow: 0 0 5px ${isMonitoringActive && isApiKeyValidated ? '#4CAF50' : '#FF5252'};
        `;
        ...
        // Status da chave API
        const apiKeyStatusContainer = document.createElement('div');
        apiKeyStatusContainer.style.cssText = `display: flex; justify-content: space-between; margin-bottom: 5px;`;
        const apiKeyStatusLabel = document.createElement('span');
        apiKeyStatusLabel.textContent = 'Chave API:';
        const apiKeyStatusValue = document.createElement('span');
        apiKeyStatusValue.id = 'apiKeyStatus';
        apiKeyStatusValue.textContent = isApiKeyValidated ? '✅ Chave válida' : '❌ Chave não validada';
        apiKeyStatusValue.style.fontWeight = 'bold';
        apiKeyStatusValue.style.color = isApiKeyValidated ? '#4CAF50' : '#FF5252';
        apiKeyStatusContainer.appendChild(apiKeyStatusLabel);
        apiKeyStatusContainer.appendChild(apiKeyStatusValue);
        currentStateSection.appendChild(apiKeyStatusContainer);
        ...
        testLoginButton.addEventListener('click', function() {
            if (!isApiKeyValidated) {
                alert('Você precisa ter uma chave API válida para usar esta função.');
                return;
            }
        ...
        // Seção de configuração da chave API
        ...
        apiKeyTitle.textContent = 'Configuração da Chave API:';
        apiKeyTitle.style.cssText = `font-weight: bold; margin-bottom: 10px;`;
        apiKeySection.appendChild(apiKeyTitle);

        const apiKeyInputLabel = document.createElement('div');
        apiKeyInputLabel.textContent = 'Sua chave API:';
        apiKeyInputLabel.style.marginBottom = '2px';
        apiKeySection.appendChild(apiKeyInputLabel);

        const apiKeyInput = document.createElement('input');
        apiKeyInput.type = 'text';
        apiKeyInput.value = apiKey;
        apiKeyInput.placeholder = 'Digite sua chave API fornecida';
        apiKeyInput.style.cssText = `width: 100%; margin-bottom: 8px; padding: 5px; box-sizing: border-box;`;
        apiKeyInput.addEventListener('change', function() {
            apiKey = this.value.trim();
            GM_setValue('apiKey', apiKey);
            addToStatusLog(`🔧 Chave API configurada`);
        });
        apiKeySection.appendChild(apiKeyInput);

        const validateKeyButton = document.createElement('button');
        validateKeyButton.textContent = 'Validar Chave API';
        validateKeyButton.style.cssText = `
            width: 100%;
            padding: 8px;
            background-color: #2196F3;
            border: none;
        ...
        validateKeyButton.addEventListener('click', function() {
            if (!apiKey) {
                alert('Por favor, insira uma chave API primeiro.');
                return;
            }
            // Forçar uma nova validação
            validateApiKey(function(valid, message) {
                if (valid) {
                    alert('Chave API validada com sucesso!');
                } else {
                    alert('Falha na validação: ' + message);
                }
            }, true); // O parâmetro "true" força a revalidação
        });
        apiKeySection.appendChild(validateKeyButton);

        configSection.appendChild(apiKeySection);
        ...
        toggleMonitoringButton.addEventListener('click', function() {
            if (!isApiKeyValidated && !isMonitoringActive) {
                alert('Você precisa ter uma chave API válida para ativar o monitoramento.');
                return;
            }
            ...
            if (isMonitoringActive && refreshInterval > 0 && isApiKeyValidated) {
                setRefreshInterval(refreshInterval);
    ...
    // Função para inicialização após validação de API
    function runInitialization() {
        ...
        setInterval(function() {
            ...
            if (isMonitoringActive && isApiKeyValidated) {
                checkAttributes();
                checkUrlChange();
    ...
    function init() {
        ...
        // Verificar a chave API primeiro
        if (!apiKey) {
            addToStatusLog('⚠️ Nenhuma chave API configurada. Por favor, configure uma chave API nas configurações.');
            return;
        }

        // Verificar se a chave já foi validada anteriormente (armazenada)
        if (isApiKeyValidated) {
            addToStatusLog('✅ Usando chave API previamente validada');
            runInitialization();
            return;
        }

        // Validar a chave API apenas se ainda não foi validada
        validateApiKey(function(valid, message) {
            if (valid) {
                addToStatusLog('✅ Chave API validada com sucesso!');
                runInitialization();
            } else {
                addToStatusLog(`❌ Chave API inválida: ${message}`);
                isApiKeyValidated = false;
                GM_setValue('isApiKeyValidated', false);
            }
        });
