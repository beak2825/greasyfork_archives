...
// @grant        none
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/466240/TriviaBot.user.js
// @updateURL https://update.greasyfork.org/scripts/466240/TriviaBot.meta.js
// ==/UserScript==
...
const processMessage = (username, uuid, message) => {
    const cleanedUsername = username.replace(/[\s\W]+/g, '');
    if (message.startsWith(prefix)) {
        const command = message.slice(1);
        if(enableCommands) handleEightBallQuestion({ message });
  ...
  mutations.forEach(({ addedNodes }) => {
    addedNodes.forEach((node) => {
      if (node.tagName === 'P') {
        ...
        if (username && message) {
          handleGreeting({ username, message });
        }
      }
    });
  });
...
const PENALTY_TIME = 2 * 60 * 1000; // 2 minutes penalty for wrong answers
...
const handleTriviaQuestion = () => {
  ...
  // Check if it's a bonus round
  const isBonus = Math.random() < 0.1; // 10% chance for a bonus round
  if (isBonus) {
    isBonusRound = true;
...
const handleAnswer = ({ username, message }) => {
  ...
  if (Date.now() - lastAttemptTime[username] < 30 * 1000) { //30 seconds delay between answers
    sendChatMessage(`Please wait a while before submitting another answer, ${username}.`);
    return;
  }

  if (userAnswers[username].includes(message.toLowerCase())) { //Disallow repeated answers
    sendChatMessage(`You've already submitted that answer, ${username}. Try something else.`);
    return;
  }

  userAnswers[username].push(message.toLowerCase());
  lastAttemptTime[username] = Date.now();

  if (attempts[username] >= 3) { //Maximum 3 attempts per question
  ...
  if (currentAnswer && message.toLowerCase() === currentAnswer.toLowerCase()) {
    const pointsMultiplier = isBonusRound ? 2 : 1;
    points[username] = (points[username] || 0) + 1 * pointsMultiplier;

    //Retrieve the stored points, if they exist; if not, default to 0
    ...
    currentAnswer = null;
    attempts[username] = 0; //Reset attempts after correct answer
    userAnswers[username] = []; //Reset answers after correct answer
  } else if (attempts[username] === 3) { // Penalty for wrong answer after 3 attempts
  ...
  updateLeaderboard(); //Update the leaderboard after each answer
};
