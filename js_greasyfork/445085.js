...
// @license      MIT License
// @downloadURL https://update.greasyfork.org/scripts/445085/%E5%92%95%E5%92%95%E9%95%87%E6%95%B0%E6%8D%AE%E9%87%87%E9%9B%86.user.js
// @updateURL https://update.greasyfork.org/scripts/445085/%E5%92%95%E5%92%95%E9%95%87%E6%95%B0%E6%8D%AE%E9%87%87%E9%9B%86.meta.js
// ==/UserScript==
/*function getEquipmentInfo(nodes) {}对正则和数据做了变更*/
(function() {
    'use strict'
    ...
    function getEquipmentInfo(nodes) {
        ...
        if (nodes.length > 0) {
            for (let i = 0; i < nodes.length; i++) {
                ...
                /*数据处理*/
                let lvs=lv.replace('<', '').replace('>', '').trim()
                let atrr0=atrr[0].replace('%', '').replace('<', '').replace('>', '').trim()
                let atrr1=atrr[1].replace('%', '').replace('<', '').replace('>', '').trim()
                let atrr2=atrr[2].replace('%', '').replace('<', '').replace('>', '').trim()
                let atrr3=atrr[3].replace('%', '').replace('<', '').replace('>', '').trim()

                data.push(new Array(name, lvs,atrr0 ,atrr1 , atrr2, atrr3, mys, id));
            }
        }
        return data;
    }
