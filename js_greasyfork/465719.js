...
// @namespace       kba
// @description     Throne Room Monitor for Kingdoms of Camelot - For Standalone use as "Monitor Lite", or in conjunction with PowerBot+ as an Additional Monitor.
...
// @releasenotes 	<p>New URL</p>
// @downloadURL https://update.greasyfork.org/scripts/465719/KoC%20Additional%20Throne%20Monitor.user.js
// @updateURL https://update.greasyfork.org/scripts/465719/KoC%20Additional%20Throne%20Monitor.meta.js
// ==/UserScript==
...
var KOCBYTE_LOGO = 'https://graph.facebook.com/kocbyte/picture';
var KOCBYTE_ON = true;
...
function btStartup (){
	...
	if (Options.StandAlone) {
		var MapFunc = function(e) {if (e.user.id != "0") { initMonitor (e.user.id, false); }}
		uWExportFunction ('btMapMonitorTR', MapFunc);

		// allow access from external tools

		var ExtFunc = function(UID) { if (UID !="") { initMonitor (UID, false); }}
		uWExportFunction ('btMonitorExternalCallUID', ExtFunc);
		TitleSuffix = 'Lite';
	}
	...
	var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
				...
				.divHeader		 {border:0px solid; border-color:#000000; background: -moz-linear-gradient(top, #E9D9AE, #8C7D5D); background: -webkit-linear-gradient(top, #E9D9AE, #8C7D5D); -moz-border-radius:5px; height: 16px;border-bottom:0px solid #000000;font-weight:bold;font-size:11px;opacity:0.75;margin-left:0px;margin-right:0px;margin-top:1px;margin-bottom:0px;padding-top:4px;padding-right:10px;padding-left:4px;vertical-align:text-top;align:left; color:#000000;}\
	...
	mainPop = new CPopup ('btMain_'+NameSpace, Options.WinPos.x, Options.WinPos.y, Options.WinSize.x, Options.WinSize.y, true, saveOptions);

	mainPop.getTopDiv().innerHTML = '<DIV align=center><B>Battle Monitor ('+TitleSuffix+')</B></DIV>';

	m = '';
	if (Options.StandAlone) { m += '<STYLE>'+ styles +'</style>'; }
	...
	m += '<a id=btOptionLink_'+NameSpace+' class=divLink ><div class="divHeader" align="left"><img id=btOption_'+NameSpace+'Arrow height="10" src="'+RightArrow+'">&nbsp;OPTIONS</div></a>';
	...
	m += '<TR><TD class=xtab>&nbsp;</td><td class=xtab><INPUT true id=AutoUpdateChk_'+NameSpace+' type=checkbox /></td><td class=xtab>Automatically check for script updates&nbsp;&nbsp;<a id=btUpdateCheck_'+NameSpace+' class="inlineButton btButton brown11"><span>Check Now</span></a></td></tr>';
	m += '<TR><TD class=xtab>&nbsp;</td><TD class=xtab>&nbsp;</td><td class=xtab><input id=btGC_'+NameSpace+' type=radio name=btuploc_'+NameSpace+' '+((Options.UpdateLocation==1)?'CHECKED':'')+'>SourceForge&nbsp;&nbsp;<input id=btGF_'+NameSpace+' type=radio name=btuploc_'+NameSpace+' '+((Options.UpdateLocation==2)?'CHECKED':'')+'>Greasyfork</td></tr>';
	m += '</table></div>';
...
//	ToggleOption ('AutoUpdateChk_'+NameSpace, 'AutoUpdates');
	...
	if (Options.StandAlone) {
		setTimeout( function() { AddPowerBarLink('BATTLE', eventHideShow, mouseMainTab); },1000);
	}
	else {
		setTimeout( function() { AddPowerBarLink('BATTLE ('+TitleSuffix+')', eventHideShow, mouseMainTab); },1000);
	}
	...
	if (Options.StandAlone) {
		...
		var TRFunc = function() { if (ThroneUID != 0) { initMonitor (ThroneUID, false); }}
		uWExportFunction ('btThroneMonitorTR', TRFunc);
		...
		if (typeof exportFunction == 'function') { exportFunction(newTRViewFunc,uW.cm.ThroneView, {defineAs:"openThrone"}); }
		else { uW.cm.ThroneView.openThrone = newTRViewFunc; };
...
function AddPowerBarLink (text, eventListener, mouseListener) {
	var PBX = ById('btPowerBarExtra');
	if (!PBX) {
		AddMainTabLink (text.toUpperCase(), eventListener, mouseListener);
		return;
	}
...
var AutoUpdater = {
    ...
    call: function(secure,response) {logit("Checking for "+this.name+" Update!"+(secure ? ' (SSL)' : ' (plain)'));
		...
		if (Options.UpdateLocation == 1) {CheckURL = SourceForgeURL;}
...
function doBOTUpdate(){
	...
	if (Options.UpdateLocation == 1) {DownloadURL = SourceForgeURL;}
...
function eventPaintPlayerInfo (){
	...
	m += '<tr><TD class=xtab align="center" colspan="3">UID: <B>' + parseInt(userInfo.userId) + '</b>&nbsp;<a id=btProfile_'+NameSpace+'>(View Profile)</a>';
	if (KOCBYTE_ON) m += '&nbsp;<a target="_tab" href="http://www.kocbyte.com/player/' + getServerId() + '/' + userInfo.userId + '"><img title="View player on KoCByte" width="'+imgwidth+'" style="vertical-align:bottom;opacity:0.75;" src="'+KOCBYTE_LOGO+'"></a>';
	m += '</td></tr>';
	...
	m += '<tr><TD class=xtab align="center" colspan="3">Glory Might: <B>' + addCommas(Math.round(userInfo.mightGlory)) + '</b></td></tr>';
...
function fetchCourtInfo (notify) {
    ...
    new MyAjaxRequest(uW.g_ajaxpath + "ajax/viewCourt.php" + uW.g_ajaxsuffix, {
      ...
      onSuccess: function (rslt) {
		...
		userInfo.mightGlory = rslt.playerInfo.mightGlory;
							...
							if (i<=parseInt(y["quality"])) {
								if (CompositeEffects.hasOwnProperty(id)) {
									...
									for (var e=0;e<Composite.length;e++) {
										HisStatEffects[Composite[e]] += Current;
									}
								}
								else {
									HisStatEffects[id] += Current;
								}
							}
						}
					}
