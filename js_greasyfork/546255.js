// ==UserScript==
// @name         RR Tracker v15(final release)
...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/546255/RR%20Tracker%20v15%28final%20release%29.user.js
// @updateURL https://update.greasyfork.org/scripts/546255/RR%20Tracker%20v15%28final%20release%29.meta.js
// ==/UserScript==
...
const DEFAULT_DYNAMIC_SETTINGS = {
    ...
    martingaleValue: 2,
    reinforce: false,
    boostedBets: false, // <-- ADD THIS LINE
    lockedField: 'capital',
    startingBet: 0
};
...
// ===================== INSERT THIS FIX =====================
// This explicitly checks if the manual bets key exists.
// If it doesn't, this is a first-time run for a new user.
if (localStorage.getItem(MANUAL_BETS_STORAGE_KEY) === null) {
    console.log("RR Tracker: First run detected. Initializing and saving default manual bets.");
    // The 'manualBets' variable already holds the default values from the loadManualBets() call.
    // We just need to force a save to localStorage to ensure the key is created.
    saveManualBets();
}
// =================== END OF FIX ============================
...
// ADD THE FOLLOWING CODE BLOCK RIGHT AFTER IT
// FIND THE "boostedBetsContainer" BLOCK YOU ADDED EARLIER AND REPLACE IT WITH THIS
...
boostedBetsLabel.textContent = 'Boosted Bets (x2 every 5th L)';
boostedBetsLabel.htmlFor = 'dyn-boosted';
boostedBetsLabel.style.cursor = 'pointer';
boostedBetsContainer.title = "If checked, doubles the calculated bet on every 5th loss in a streak (5th, 10th, 15th, etc.).";

// This onchange event is critical - it recalculates and redraws the buttons
boostedBetsCheck.onchange = () => {
    dynamicBetsSettings.boostedBets = boostedBetsCheck.checked;
    saveDynamicBetsSettings();
    calculateBetsAndRefresh(); // This function updates the button values
};

boostedBetsContainer.append(boostedBetsCheck, boostedBetsLabel);
...
// --- ADD THIS NEW CONTAINER ---
    ...
    statsDisplayPanel.appendChild(lifetimeStreaksDiv);
...
// --- ADD THIS NEW LOGIC ---
    ...
    async function fetchAllLogsPaginated(logType) {
        ...
        const stopTimestamp = Math.floor(Date.now() / 1000) - (365 * 24 * 3600); // 100 years ago
...
// ...
        ...
        if (allRawLogs.length > 0) {
            ...
            let newlyProcessedGames;

            if (isInitialSync) {
                // For deep scans, use the non-blocking chunk processor
                const forceSyncButton = document.getElementById('rr-force-sync-btn');
                console.log("RR Tracker: Processing large dataset in chunks to prevent freezing...");

                newlyProcessedGames = await processLogsInChunks(allRawLogs, (progress) => {
                    // This provides a progress update to the UI
                    if(forceSyncButton) forceSyncButton.textContent = `Syncing... ${progress}%`;
                });

            } else {
                // For quick scans, the original synchronous method is fine
                const startGameIndices = [];
...
// REPLACE THE ENTIRE getSumOfRatios FUNCTION WITH THIS
function getSumOfRatios(L, M, reinforce, numCoeffs, boosted) {
    if (numCoeffs <= 0 || M <= 1) return { sum: 0, coeffs: [] };
    const maxCoeffs = Math.max(L, numCoeffs);
    const allCoeffs = [];

    if (!reinforce) {
        // This is the standard (non-reinforced) Martingale logic, which is unchanged.
        let lastCoeff = 1;
        allCoeffs.push(lastCoeff);

        for (let i = 1; i < maxCoeffs; i++) {
            let currentCoeff = lastCoeff * M;
            // Apply boost if it's the 5th, 10th, etc., bet
            if (boosted && (i + 1) % 5 === 0) {
                currentCoeff *= 2;
            }
        ...
        // --- THIS IS THE UPDATED REINFORCE LOGIC ---
        // It calculates each bet based on the new formula (e.g., B5 = 2*B4 + B3)
        for (let i = 0; i < maxCoeffs; i++) {
            ...
            if (i === 0) { // Bet 1 (B1 = x)
                currentCoeff = 1;
            } else if (betNumber % 2 === 0) { // Even bets (B2=2*B1, B4=2*B3, etc.)
                currentCoeff = M * allCoeffs[i - 1];
            } else { // Odd bets (B3=2*B2+B1, B5=2*B4+B3, etc.)
                // We add the coefficient from the previous odd bet, which is 2 positions back in the array.
                const reinforcementCoeff = allCoeffs[i - 2] || 0;
                currentCoeff = (M * allCoeffs[i - 1]) + reinforcementCoeff;
            }

            // Apply boost if it's the 5th, 10th, etc., bet
            if (boosted && betNumber % 5 === 0) {
                currentCoeff *= 2;
            }
            allCoeffs.push(currentCoeff);
        }
...
const { sum, coeffs } = getSumOfRatios(maxLStreak, martingaleValue, reinforce, numCoeffsToGenerate, dynamicBetsSettings.boostedBets);
    ...
    function buildBetButtons() {
        ...
        // --- ADD THIS STOP-LOSS LOGIC ---
        if (isStopLossActive) {
            // If stop loss is active, show a message and do not build buttons
            mainBetGridContainer.innerHTML = `<div style="text-align:center; padding: 10px; color: #ff7a7a;">Daily loss limit reached. Bets are disabled.</div>`;
            miniBetGridContainer.innerHTML = `<div style="text-align:center; font-size: 10px; padding: 5px; color: #ff7a7a;">Loss Limit</div>`;
            return; // Exit the function early
        }
        // --- END OF STOP-LOSS LOGIC ---
...
function refreshAll() {
    ...
    // --- CORRECTED: Calculate dailyProfit FIRST ---
    ...
    // Logic now checks if the stop-loss feature is enabled by the user
    if (stopLossEnabled && lossLimit > 0 && dailyProfit <= -lossLimit && !alertShownLoss) {
        showAlert = true;
    ...
    else if (profitTarget > 0 && dailyProfit >= profitTarget && !alertShownProfit) {
        showAlert = true;
