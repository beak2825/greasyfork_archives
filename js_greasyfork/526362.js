...
// @version      2.0.9
// @description  在比赛中预测 AtCoder 的表现
// @author       Sky390
// @license      MIT
// @match        https://atcoder.jp/*
// @exclude      /^https://atcoder\.jp/[^#?]*/json/
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/526362/ac-predictor-cn.user.js
// @updateURL https://update.greasyfork.org/scripts/526362/ac-predictor-cn.meta.js
// ==/UserScript==
var config_header_text$1 = "ac-predictor 设置";
var config_hideDuringContest_label$1 = "在比赛期间隐藏预测";
var config_hideUntilFixed_label$1 = "在表现确定之前隐藏预测";
var config_useFinalResultOnVirtual_label$1 = "在虚拟参与时使用最终结果进行表现计算";
var config_useFinalResultOnVirtual_description$1 = "勾选后，表现将按当时参与者已完成比赛计算。";
var config_dropdown$1 = "ac-predictor 设置";
var standings_performance_column_label$1 = "表现";
var standings_rate_change_column_label$1 = "评分变化";
var standings_click_to_compute_label$1 = "点击计算";
var standings_not_provided_label$1 = "不可提供";
var jaJson = {
...
var config_header_text = "ac-predictor 设置";
var config_hideDuringContest_label = "在比赛期间隐藏预测";
var config_hideUntilFixed_label = "在表现确定之前隐藏预测";
var config_useFinalResultOnVirtual_label = "在虚拟参与时使用最终结果作为表现参考";
var config_useFinalResultOnVirtual_description = "如果启用，表现将按原参与者已完成比赛计算。";
var config_dropdown = "ac-predictor";
var standings_performance_column_label = "表现";
var standings_rate_change_column_label = "评分变化";
var standings_click_to_compute_label = "点击计算";
var standings_not_provided_label = "不可提供";
var enJson = {
...
class ConfigController {
    register() {
        ...
        if (isDebugMode()) {
            configView.addCheckbox("[DEBUG] 启用调试模式", getConfig("isDebug"), null, val => setConfig("isDebug", val));
            configView.addCheckbox("[DEBUG] 使用结果", getConfig("useResults"), null, val => setConfig("useResults", val));
        }
    }
}
...
async function getAPerfs(contestScreenName) {
    ...
    if (!result.ok) {
        throw new Error(`获取 aperfs 失败: ${result.status}`);
    }
...
class ContestDetails {
    ...
    get defaultAPerf() {
        ...
        if (this.ratedrange.end == 2399)
            return 800; // 该值不相关，因为从未使用
        const DEFAULT_CHANGED_AT = new Date("2019-05-25"); // 可能错误
...
async function getContestDetails() {
    ...
    if (!result.ok) {
        throw new Error(`获取比赛详情失败: ${result.status}`);
    }
    const parsed = await result.json();
    const res = [];
    for (const elem of parsed) {
        if (typeof elem !== "object")
            throw new Error("返回的对象无效");
...
async function getExtendedStandings(contestScreenName) {
    if (!cache$4.has(contestScreenName)) {
        ...
        if (!result.ok) {
            throw new Error(`获取扩展排名失败: ${result.status}`);
        }
...
//版权所有 © 2017 koba-e964。
//来自: https://github.com/koba-e964/atcoder-rating-estimator
const finf = bigf(400);
function bigf(n) {
    let pow1 = 1;
    let pow2 = 1;
...
/**
 * 从表现历史计算未正值评级
 * @param {Number[]} [history] 按升序排列的表现历史
 * @returns {Number} 未正值评级
 */
...
/**
 * 从最后状态计算未正值评级
 * @param {Number} [last] 最后未正值评级
 * @param {Number} [perf] 表现
 * @param {Number} [ratedMatches] 参与的评级比赛数量
 * @returns {number} 估算的未正值评级
 */
...
/**
 * 计算达到目标评级所需的表现
 * @param {Number} [targetRating] 目标未正值评级
 * @param {Number[]} [history] 按升序排列的表现历史
 * @returns {number} 表现
 */
...
/**
 * 获取基于比赛开始和结束日期的启发式评级计算中的权重
 * @param {Date} startAt - 比赛开始日期。
 * @param {Date} endAt - 比赛结束日期。
 * @returns {number} 比赛的权重。
 */
...
/**
 * 从表现历史计算未正值评级
 * @param {RatingMaterial[]} [history] 表现历史
 * @returns {Number} 未正值评级
 */
...
const colorNames = ["unrated", "grey", "brown", "green", "cyan", "blue", "yellow", "orange", "red"];
...
class StandingsLoadingView {
    ...
    static Get() {
        ...
        if (loadingElem === null) {
            throw new Error("未找到 loadingElem");
        }
...
function getDefferedRatingElem(result) {
    ...
    async function listener() {
        ...
        try {
            ...
            elem.append(getSpan(["加载错误"], []), document.createElement("br"), getSpan(["(悬停以查看详细信息)"], ["grey", "小"]), getSpan([e.toString()], ["my-tooltiptext"]));
...
function getErrorRatingElem(result) {
    const elem = document.createElement("div");
    elem.append(getSpan(["加载错误"], []), document.createElement("br"), getSpan(["(悬停以查看详细信息)"], ["grey", "小"]), getSpan([result.message], ["my-tooltiptext"]));
    elem.classList.add("my-tooltip");
    return elem;
}
...
class StandingsTableView {
    ...
    update() {
        ...
        if (!header)
            console.warn("未找到头元素", this.element);
        else
            modifyHeader(header);
...
class ExtendedStandingsPageController {
    ...
    async initialize() {
        ...
        this.standingsTableView = StandingsTableView.Get(async (userScreenName) => {
            if (!this.performanceProvider)
                return { "type": "error", "message": "performanceProvider 缺失" };
            if (!this.performanceProvider.availableFor(userScreenName))
                return { "type": "error", "message": `用户 ${userScreenName} 的表现不可用` };
    ...
    async updateData() {
        if (!this.contestDetails)
            throw new Error("contestDetails 缺失");
...
async function getHistory(userScreenName, contestType = "algorithm") {
    ...
    if (!cache$3.has(key)) {
        ...
        if (!result.ok) {
            throw new Error(`获取历史记录失败: ${result.status}`);
        }
...
async function getResults(contestScreenName) {
    if (!cache$2.has(contestScreenName)) {
        ...
        if (!result.ok) {
            throw new Error(`获取结果失败: ${result.status}`);
        }
...
async function getStandings(contestScreenName) {
    if (!cache$1.has(contestScreenName)) {
        ...
        if (!result.ok) {
            throw new Error(`获取排名失败: ${result.status}`);
        }
...
class IncrementalAlgRatingProvider {
    ...
    async getRating(userScreenName, newPerformance) {
        if (!this.availableFor(userScreenName)) {
            throw new Error(`评分对 ${userScreenName} 不可用`);
        }
    ...
    async initialize() {
        ...
        this.standingsTableView = StandingsTableView.Get(async (userScreenName) => {
            if (!this.ratingProvider)
                return { "type": "error", "message": "ratingProvider 缺失" };
            if (!this.performanceProvider)
                return { "type": "error", "message": "performanceProvider 缺失" };
            if (!this.isRatedMaps)
                return { "type": "error", "message": "isRatedMapping 缺失" };
            if (!this.oldRatings)
                return { "type": "error", "message": "oldRatings 缺失" };
            if (!this.oldRatings.has(userScreenName))
                return { "type": "error", "message": `未找到 ${userScreenName} 的 oldRating` };
            ...
            if (this.isRatedMaps.get(userScreenName)) {
                if (!this.ratingProvider.provider.availableFor(userScreenName))
                    return { "type": "error", "message": `评分对 ${userScreenName} 不可用` };
...
function duringVirtualParticipation() {
    if (!isVirtualStandingsPage()) {
        throw new Error("不在此页面不可用");
    }
    const timerText = document.getElementById("virtual-timer")?.textContent ?? "";
    if (timerText && !timerText.includes("结束") && !timerText.includes("over"))
        return true;
    else
        return false;
}
