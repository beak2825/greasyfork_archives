...
// @downloadURL https://update.greasyfork.org/scripts/548401/Faction%20Target%20Finder%20%2B%20War%20Mode%20%28FF%20Ranked%29.user.js
// @updateURL https://update.greasyfork.org/scripts/548401/Faction%20Target%20Finder%20%2B%20War%20Mode%20%28FF%20Ranked%29.meta.js
// ==/UserScript==
...
(function () {
    ...
    GM_xmlhttpRequest({
        ...
        onload(res) {
            ...
            if (data?.error) {
                alert("Error fetching your stats: " + data.error.error);
                callback(null);
                return;
            }
            // Torn returns stats directly (not nested in `battle_stats`)
            callback(data.battle_stats || data);
        },
        onerror() {
            callback(null);
        }
    ...
    if (!data) return null;
    const stats = data.battle_stats || data; // handle both API shapes
    if (!stats.strength || !stats.speed || !stats.dexterity || !stats.defense) {
        return null; // missing fields, skip
    }
    return {
        ...
        total: Number(stats.total) || (
            ...
            Number(stats.speed) +
            Number(stats.dexterity) +
            Number(stats.defense)
        )
    };
}
    getMyStats(raw => {
    myStats = normalizeStats(raw);
    if (!myStats) {
        alert("Could not fetch your battlestats.");
        return;
    }
    processWarTargets();
});

getEnemyStats(enemyId, raw => {
    let stats = normalizeStats(raw);
    if (stats) enemyList.push({ id: enemyId, stats });
    progressBar.update();
    nextEnemy();
});
