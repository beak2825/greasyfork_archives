...
// @downloadURL https://update.greasyfork.org/scripts/537359/Piccolo%27s%20Agario%20Modded%2032.user.js
// @updateURL https://update.greasyfork.org/scripts/537359/Piccolo%27s%20Agario%20Modded%2032.meta.js
// ==/UserScript==
/*
 * Notes:
...
// Agregar configuración por defecto para skinBorderColor y skinBorderWidth
const defaultConfig = {
  // ... (otras configuraciones)
  skinBorderColor: "#000000",
  skinBorderWidth: 2,
  // ... (otras configuraciones)
};

// Función para aplicar el borde a las skins
function applySkinBorder() {
  const skinElements = document.querySelectorAll(".skinWrapper img");
  skinElements.forEach((element) => {
    element.style.border = `${CONFIG.skinBorderWidth}px solid ${CONFIG.skinBorderColor}`;
  });
}

// Función para crear la interfaz de usuario para cambiar el borde
function createBorderUI() {
  const borderUI = document.createElement("div");
  borderUI.innerHTML = `
    <span>Color del borde de la skin:</span>
    <input type="color" id="border-color" value="${CONFIG.skinBorderColor}">
    <span>Ancho del borde de la skin:</span>
    <input type="number" id="border-width" value="${CONFIG.skinBorderWidth}">
  `;
  document.body.appendChild(borderUI);
  const borderColorInput = document.getElementById("border-color");
  borderColorInput.addEventListener("input", function () {
    CONFIG.skinBorderColor = borderColorInput.value;
    saveConfig(CONFIG);
    applySkinBorder();
  });
  const borderWidthInput = document.getElementById("border-width");
  borderWidthInput.addEventListener("input", function () {
    CONFIG.skinBorderWidth = parseInt(borderWidthInput.value);
    saveConfig(CONFIG);
    applySkinBorder();
  });
}

// Función para aplicar estilos a la interfaz de usuario
