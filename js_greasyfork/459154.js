...
// @version      6.2.7.9
// @description        任意轉換網頁中的簡體中文與正體中文（預設簡體→正體），顯示漢字對應漢語拼音，自訂任意替換文字
...
// @downloadURL https://update.greasyfork.org/scripts/459154/%E7%B9%81%E7%B0%A1%E8%87%AA%E7%94%B1%E5%88%87%E6%8F%9B%EF%BC%88%E8%87%AA%E7%94%A8%E7%89%88%E6%9C%AC%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/459154/%E7%B9%81%E7%B0%A1%E8%87%AA%E7%94%B1%E5%88%87%E6%8F%9B%EF%BC%88%E8%87%AA%E7%94%A8%E7%89%88%E6%9C%AC%EF%BC%89.meta.js
// ==/UserScript==
//因一簡對多繁，所以簡轉繁需要優先排除異體字，並根據詞彙轉換。其他需要語義分析的，暫時無解。整理繁簡對照表很費時，因此不打臉的話不再更新，如有需求，刪減自用。更精細的需求可自行申請相應API或自行訓練語義AI並搭建對照數據庫。在油猴脚本裡面如此這般折騰，我是覺得沒有意義啦。。。
//2025/05/09 更新無BUG版本與詞庫
(function() {
    'use strict';
    ...
    var sc2tc = {
        ...
        '儿':[
            '兒',
            ['裡','这儿'],
            ['','玩儿梗','会儿','玩意儿','女孩儿','事儿','那会儿','点儿','门儿','一溜烟儿','小孩儿','老头儿','信儿','状况儿','份儿','边儿']
        ],
        '声':[
            '聲',
            ['声','声の','の声','声掛け','声も','声が']
        ...
        '冲':[
            '沖',
            ['用','不好冲'],
            ['衝','冲奖','冲高','冲决','冲浪','冲子','冲力','冲要','冲破','冲口','冲顶','冲床','冲突','冲刺','冲金','冲模','冲撞','冲腾','冲锋','冲量','冲动','冲程','冲压','冲杀','冲激','冲击','俯冲','反冲','折冲','缓冲','脉冲','要冲','冲锋枪','冲孔机','冲劲','冲金点','冲压机','冲击波','反冲力','冲锋','横冲','冲冠','首当其冲','冲了','冲着','前冲','猛冲']
        ],
        '恶':[
            '惡',
            ['噁','恶心']
        ...
        '发':[
            ...
            ['傳','宣发'],
            ['發','发回','发声','发现','发布'],
            ['髮','发网','发际','发箍','发丝','发式','发带','发型','发卡','发妻','发指','发廊','发饰','发乳','发夹','发菜','发屋','发姐','发油','发套','发蜡','发鬓','发髻','发雕','发辫','发胶','发浆','假发','健发','削发','卷发','握发','束发','染发','植发','栉发','毛发','毫发','烫发','理发','白发','短发','秀发','秃发','结发','美发','胎发','脱发','华发','落发','蓄发','护发','金发','银发','头发','驳发','鬓发','须发','发小','剃发令','洗发','发短心长','怒发冲冠','断发文身','被发','鹤发','黄发垂髫','擢发难数','庞眉皓发','披头散发','间不容发','发色','红发','橙发','棕发','长发','黑发']
        ],
        '复':[
            '復',
            ...
            ['覆','复電','批复','核复','禀复','答复','被复','赐复','颠复','倾复','函复','反复','复亡','复函','复命','复审','复复','复败','复书','复核','复没','复灭','复舟']
        ],
        '鉴':[
            '鑒',
        ...
        '赞':[
            '讚',
            ['贊','赞助','赞同'],
			      ['讚','赞赏','赞佩','赞美','赞誉','赞歌','赞叹','赞许','赞扬','赞颂','赞语','按赞','盛赞','礼赞','称赞','夸赞','颂赞','点赞','赞不绝口','相当赞','很赞','超赞']
        ],
        '钟':[
            '鍾',
        ...
        '只':[
            '只',
            ['只','只有','只会','只管','只消','只当','只好','只要','只能','只会','只是','只怕','只得','只见','只顾','只许','只因','不只','仅只','只不过','只此一家','只欠东风','只争朝夕','只是','只看'],
            ['隻','几只','一只','二只','两只','三只','四只','五只','六只','七只','八只','九只','十只','0只','1只','2只','3只','4只','5只','6只','7只','8只','9只','10只','百只','千只','万只','形单影只','只言片语','只字不提']
        ],
        '捂':[
            '捂',
            ['摀','紧捂']
        ...
        '云':[
            '雲',
            ['云','人云亦云','云云','不知所云','但云','古云','胡云','诗云子曰','云為',',云尔','云乎']
        ],
        '游':[
            '遊',
            ['游','游泳','游水','花游']
        ...
        '松':[
            '松',
            ['鬆','松口','松手','松弛','松泛','松气','松脆','松动','松爽','松软','松閒','松绑','松紧','松缓','松嘴','松懈','轻松','放松','疏松','宽松','弄松','松松','稀松','肉松','鱼松','松快','蓬松','松开','松了','松一口气']
        ],
        '愈':[
            '愈',
        ...
        '斗':[
            '斗',
            ['鬥','不斗','久斗','互斗','仍斗','共斗','再斗','初斗','勇斗','博斗','又斗','合斗','吵斗','善斗','大斗','好斗','想斗','打斗','批斗','抓斗','抗斗','拆斗','拼斗','挑斗','接斗',
             '搏斗','敢斗','文斗','斗一','斗上','斗不','斗久','斗了','斗他','斗你','斗倒','斗出','斗到','斗力','斗勇','斗去','斗口','斗命','斗嘴','斗在','斗垮','斗士','斗奇','斗她','斗妍',
             '斗完','斗弄','斗得','斗心','斗忍','斗志','斗快','斗意','斗成','斗我','斗批','斗技','斗招','斗拳','斗掌','斗斗','斗智','斗棋','斗法','斗牛','斗狗','斗狠','斗眼','斗神','斗私',
             '斗草','斗角','斗起','斗趣','斗酒','斗魂','智斗','暗斗','未斗','格斗','械斗','武斗','死斗','比斗','游斗','激斗','狠斗','猛斗','相斗','私斗','群斗','苦斗','虎斗','血斗','要斗',
             '角斗','越斗','跟斗','迎斗','邀斗','酣斗','乱斗','内斗','别斗','剧斗','劲斗','夺斗','奋斗','厮斗','恶斗','战斗','斗来','斗个','斗传','斗剑','斗劲','斗胜','斗场','斗将','斗恶',
             '斗战','斗擞','斗败','斗敌','斗杀','斗殴','斗气','斗争','斗兽','斗毕','斗舰','斗艺','斗艳','斗赢','斗输','斗过','斗鸡','斗饮','斗闹','斗鱼','斗丽','会斗','权斗','殴斗','决斗',
             '争斗','独斗','竞斗','约斗','缠斗','罢斗','观斗','赌斗','较斗','连斗','门斗','双斗','颤斗','凤斗','斗！']
        ],
        '系':[
            '系',
            ['係','关系','系数'],
        ...
        '干':[
            '幹',
            ['乾','口干','吃干','吐干','吮干','吸干','吹干','呷干','喉干','喝干','嘴干','太干','干井','干似','干冰','干冷','干化','干咳','干咽','干品','干哥','干嚎','干土','干坤','干妹',
             '干姊','干姐','干姜','干娘','干爹','干爸','干妈','干季','干巴','干布','干干','干式','干弟','干性','干料','干旱','干杯','干果','干枝','干枯','干柴','干梅','干沙','干泥','干洗',
             '干涸','干渴','干焦','干熬','干燥','干爽','干球','干疤','干瘦','干眼','干瞪','干硬','干窘','干笑','干等','干粉','干耗','干肉','干股','干脆','干花','干草','干菜','干薪','干衣',
             '干裂','干透','干酪','干醋','干隆','干面','弄干','很干','抹干','抽干','揩干','擦干','晾干','朝干','未干','杯干','果干','桑干','榨干','水干','流干','海干','滴干','炒干','烘干',
             '烤干','焙干','焦干','煨干','熨干','略干','碗干','粉干','耗干','肉干','舔干','菜干','蒸干','速干','干儿','干哑','干呕','干坛','干孙','干尸','干搁','干晒','干净','干涩','干涧',
             '干湿','干热','干烧','干瘪','干瘾','干发','干粮','干结','干丝','干声','干叶','干号','干货','干阳','干饭','拧干','晒干','极干','泪干','沥干','烧干','烩干','发干','笋干','绞干',
             '阴干','难干','风干','饮干','饼干','鱼干','唇干'],
            ['干','干系','天干','干涉','干扰','干戈','相干','若干','干擾']
        ],
        '了':[
            '了',
        ...
        '谷':[
            '谷',
            ['谷','低谷','山谷','谷峰','谷底','谷口'],
            ['穀','五谷杂粮','五谷','布谷','辟谷','米谷','打谷','稻谷','田谷','年谷','谷类','谷米','谷贱','谷贵','谷食','谷物','谷仓','钱谷','息谷','殖谷','积谷','九谷','臧谷亡羊']
        ],
        '仿':[
            '仿',
            ['倣','仿效'],
        ...
        '吊':[
            '吊',
            ['弔','吊信','吊古','吊唁','吊奠','吊孝','吊客','吊影','吊念','吊慰','吊文','吊民','吊祭','哀吊','唁吊','盆吊','祭吊','陪吊','吊问','吊丧','吊场','吊书','吊词','吊诡','吊贺',
             '吊钱','凭吊']
        ],
        '台':[
            '台',
        ...
        '回':[
            '回',
            ['迴','北回','南回','回圈','回廊','回旋','回游','回翔','回避','峰回','巡回','迂回','回环','回纹','回绕','回肠','回荡','回銮','回响','回风','梦回','盘回','纡回','萦回','轮回',
             '递回','递回']
        ],
        '后':[
            '後',
        ...
        '征':[
            '征',
            ['徵','代征','停征','像征','免征','咎征','征了','征人','征信','征候','征兆','征入','征兵','征募','征去','征友','征取','征召','征地','征婚','征引','征得','征收','征文','征求',
             '征片','征用','征稿','征管','征聘','征象','征逐','征集','急征','性征','新征','特征','狂征','病征','稽征','考征','苛征','表征','象征','超征','魏征','带征','广征','强征','征个',
             '征启','征敛','征状','征税','征粮','征纳','征缴','征诏','征询','征调','征财','征费','征赋','征购','征选','应征','横征','减征','滥征','纳征','缓征','联征','详征','诚征','课征',
             '变征','开征','体征']
        ],
        '注':[
            '注',
        ...
        '并':[
            '並',
            ['併','一并','不并','并入','并力','并合','并吞','并图','并拢','并案','并叠','并砌','并科','并负','并购','并赃','并除','并陇','侵并','兼并','合并','吞并','整并','归并','相并',
             '砌并','被并','裁并','购并','双并']
        ],
        '念':[
            '念',
        ...
        '志':[
            '志',
            ['誌','日志','网志','墓志铭','聊斋志异','三国志','杂志','同人志','标志']
        ],
        '么':[
            '麼',
            ['嗎','的么'],
        ...
        '里':[
            '裡',
            ['里','○里','一里','七里','三里','下里','九里','二里','五里','亚里','佳里','克里','全里','两里','八里','公里','六里','凯里','劈里','加里','北里','十里','千里','南里','卡里','吉里','哈里','哥里','啰里','四里','埔里','多里','少里','尤里','居里','峇里','布里','几里','底里','德里','拉里','故里','数里','斯里','普里','东里','格里','归里','波里','乌里','百里','稀里','罗里','英里','莫里','万里','苏里','里亚','里仁','里夫','里奥','里尼','里布','里拉','里昂','里根','里民','里尔','里科','里程','里约','里纳','里美','里肌','里兰','里路','里里','里长','西里','贝里','路里','道里','乡里','邻里','阿里','马里','０里','0里','1里','2里','3里','4里','5里','6里','7里','8里','9里','数里'],
            ['裏','里番','表里']
        ],
        '面':[
            '面',
        ...
        '制':[
            '制',
            ['製','复制','制造','制程','制片','制版','制品','制图','制片','制作','制药','制作人','制成','烧制','制砖','制卡'],
            ['制','制服','机制','機制','制度','强制','強制','限制','控制','回合制','制裁','即时制','压制','制伏','制约']
        ],
        '账':[
            '帳',
            ['帳','帳号']
    ...
    //此處為用語轉換，優先級最高，也可以用來給以上兩種轉換打補丁，此處簡繁共用
    var sc2tcComb = {
        '香烟袅袅':'香煙裊裊',
        '袅袅香烟':'裊裊香煙',
        '补丁':'補丁',
        ...
        if (pinyinTree) {
            ...
            storage.getItem("pinyinTree", async value => {
                if (value) {
                    ...
                    if (!pinyinTree) {
                        pinyinTree = {};
                        alert("拼音字典讀取失敗，請檢查網路服務或者禁用拼音顯示");
                    } else {
    ...
    function run() {
        ...
        if (inConfigPage) {
            ...
            defaultSimpleTitle.innerText = '預設語言：';
            ...
            customTermTitle.innerText = '自訂簡繁用語，建議兩字以上為簡體與表格衝突時使用（可透過萬用字元設定生效網址範圍）：';

            let addNewGlob1 = document.createElement('button');
            addNewGlob1.innerText = '新增生效網站';
            addNewGlob1.addEventListener("click", function(e) {
                let glob = prompt("生效網站萬用字元", "https://google.com/*");
                if (!glob) return;
            ...
            customTermInput.style.minHeight = "150px";
            customTermInput.value = JSON.stringify(sc2tcCombConfig, null, 4);
            ...
            customIlliteracyTitle.innerText = '通用字詞轉換，繁體或其他語言轉換時使用（可透過萬用字元設定生效網址範圍）：';
            let addNewGlob2 = document.createElement('button');
            addNewGlob2.innerText = '新增生效網站';
            ...
            customIlliteracyInput.style.minHeight = "150px";
            customIlliteracyInput.value = JSON.stringify(illiteracyConfig, null, 4);
    ...
    getMulValue(["version", "auto", "shortcutKey", "ctrlKey", "altKey", "shiftKey", "metaKey", "disablePinyin", "keyCallNotSave", "pinyinShortcutKey", "pinyinCtrlKey", "pinyinAltKey", "pinyinShiftKey", "pinyinMetaKey", "sc2tcCombConfig", "illiteracyConfig", "notification", "isSimple", "sc2tcCombTree", "tc2scCombTree", "fuckIlliteracyTree", "stDict", "tsDict", currentAction], async values => {
       ...
       if (!disablePinyin && false) {  // 透過 false 直接禁用"顯示拼音"
        ...
        if (!isSimple && false) {  // 透過 false 直接禁用"提交詞彙"
