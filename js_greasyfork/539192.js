...
(function () {
    ...
    // --- 1. 如果是课程列表页 ---
    if (currentUrl.includes("/user/lesson") || currentUrl.includes("/fzdx/lesson")) {
        setTimeout(() => {
            const courseUnits = $('.study_plan2');
            let found = false;
            courseUnits.each(function() {
                const unit = $(this);
                if (unit.text().includes('未完成')) {
                    const studyButton = unit.find('a.study_a');
                    if (studyButton.length > 0) {
                        GM_notification({ text: `找到未完成课程：${unit.find('a.plan_a').text().trim()}，即将跳转学习...`, timeout: 3000 });
                        found = true;
                        studyButton[0].click();
                        return false;
                    }
            ...
            if (!found) {
                 GM_notification({ text: '本页所有课程均已完成！', timeout: 5000 });
            }
        }, 2000); // 延迟2秒执行，给页面加载时间

        return; // 列表页逻辑执行完毕，退出脚本
    }


    if (currentUrl.includes("/fzdx/play")) {
            ...
            window.start = setInterval(() => {
                ...
                if (ended) {
                    ...
                    if (next && next.querySelector("a")) {
                        ...
                        clearInterval(window.start);
                        const baseUrl = location.href.split('/fzdx/play')[0];
                        location.href = baseUrl + '/user/lesson';
                    }
                    return;
                }
