...
// @downloadURL https://update.greasyfork.org/scripts/397614/Youtube%20Speed%20By%20Channel.user.js
// @updateURL https://update.greasyfork.org/scripts/397614/Youtube%20Speed%20By%20Channel.meta.js
// ==/UserScript==
...
(async () => {
    ...
    $.each([
        ...
        ["Save resume time to url", async function(){
            ...
            if (temp === "true" || temp === "false" || temp === "onpause") toggleconfig('SAVE_RESUME_TIME', (temp === "false" ? false : temp));
        }],
    ...
    async function getspeed(params = {}){
        let speed, reason;
        let channelspeed = [await GM.getValue(params.channel[0]), await GM.getValue(params.channel[1])];
        params['channelspeed'] = channelspeed;
        if (params.hasOwnProperty('force1x') && params.force1x){
            speed = 1;
            reason = "forcing 1x (live?)";
        } else if (channelspeed && typeof (channelspeed[0] ?? channelspeed[1]) === "number"){
            speed = channelspeed[0] ?? channelspeed[1];
            reason = "channelspeed";
    ...
    function getchannelname (id, div = null, returnboth = false){
        try {
            if (div === null) div = $('#channel-name[vsb-channel=' + id + ']');
            let channel = div.find('#container #text:visible:first').text().trim(),
                channelID = div.find('a[href^="\/@"]')?.attr('href')?.match(/^\/(@.+)$/)?.[1];
            if (!channel){
                channel = div.find('.ytd-channel-name').find('a').text().trim();
            }
            if (!channel){
                // videos with collabs
                channel = $('#primary .watch-active-metadata #owner #attributed-channel-name:visible').find('a').contents().filter(function() { return this.nodeType === 3; /* TEXT_NODE */}).first().text().trim();
            }
            if (!channel){
    ...
    function setchanneldefault(el){
        ...
        let channel = getchannelname(id, null, true);
        changebuttontitle(id, channel[0] ?? channel[1]);
        ...
        if (channel[0]){
            GM.setValue(channel[0], currentspeed);
            console.log('changing default for (' + channel[0] + ') to (' + currentspeed + ')');
        }
        if (channel[1]){
            GM.setValue(channel[1], currentspeed);
            console.log('changing default for (' + channel[1] + ') to (' + currentspeed + ')');
        }
    }
        ...
        for(let mutation of mutationsList) {
            if (mutation.attributeName === 'video-id'){
                ...
                setTimeout(async function(){
                    let channelname = getchannelname(id, channeldiv);
                    let tries = 1;
                    ...
                    let speed = await getspeed({
                        channel: getchannelname(id, channeldiv, true),
                    ...
                    if($('span[speed="' + speed[0] + '"]', el).click().length === 0) $('video[vsb-video="' + id + '"]')[0].playbackRate = speed[0];
                    if (speed[1] === 'channelspeed') $('span[speed="' + speed[0] + '"]', el).css("text-decoration", "underline");
                },500);
            }
        }
    }
    ...
    function ob_youtube_c4player (mutationsList, observer){
        for(let mutation of mutationsList) {
            if (mutation.attributeName === 'src' && mutation.target.src !== ''){
                ...
                setTimeout(async function(){
                    $('video[vsb-video="' + id + '"]')[0].playbackRate = await getspeed({
                        channel: getchannelname(id, channeldiv, true),
            ...
            function(){
                ...
                if (video.attr('vsb-video') === undefined){
                    ...
                    setTimeout(function(){
                        ...
                        if (SHOW_RELATIVE_TIME || SAVE_RESUME_TIME) (!SHOW_RELATIVE_TIME && SAVE_RESUME_TIME === 'onpause' ? ['pause'] : ['timeupdate','seeked', 'pause']).forEach( function(evt) { video[0].addEventListener(evt, changetime,false) });
                    },750);
                } else {
    ...
    function changetime (event){
        ...
        if (SHOW_RELATIVE_TIME){
            ...
            let timediv = $('#movie_player[monitored="' + id + '"]:visible .ytp-time-display:visible .ytp-time-contents');
        ...
        if (SAVE_RESUME_TIME && (SAVE_RESUME_TIME !== 'onpause' || event.type === 'pause')){
        ...
        $('#movie_player:visible:not([monitored]), #c4-player:visible:not([monitored])').each(async function( index ) {
            ...
            if (this.id === "movie_player" && !this.classList.contains('ytp-player-minimized')){
                ...
                speed = await getspeed({
                    channel: getchannelname(-1, channeldiv, true),
                    defspeed: (channeldiv.find('.badge-style-type-verified-artist').length > 0 || channelname.match(/VEVO$/) ? 1 : null),
                    force1x: (el.find('.ytp-live').length === 1)
                });
                let div = createcontainer(speed[0], id);
                if (speed[1] === 'channelspeed') $('span[speed="' + speed[0] + '"]', div).css("text-decoration", "underline");
                if (SHOW_ON_PLAYER){
                ...
                changebuttontitle(id, channelname);
                videodiv[0].playbackRate = speed[0];
                if (SHOW_RELATIVE_TIME || SAVE_RESUME_TIME) (!SHOW_RELATIVE_TIME && SAVE_RESUME_TIME === 'onpause' ? ['pause'] : ['timeupdate','seeked', 'pause']).forEach( function(evt) { videodiv[0].addEventListener(evt, changetime,false) });
                ...
                videodiv[0].playbackRate = await getspeed({
                    channel: getchannelname(-1, channeldiv, true),
                    defspeed: (channeldiv.find('.badge-style-type-verified-artist').length === 1 ? 1 : null)
                })[0];
            }
