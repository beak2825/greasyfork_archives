// ==UserScript==
// @name         改编-资源网助手
// @namespace    https://greasyfork.org/zh-CN/users/247258
// @version      2.1.1
...
// @downloadURL https://update.greasyfork.org/scripts/381362/%E6%94%B9%E7%BC%96-%E8%B5%84%E6%BA%90%E7%BD%91%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/381362/%E6%94%B9%E7%BC%96-%E8%B5%84%E6%BA%90%E7%BD%91%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
/* jshint esversion: 6 */
;
GM_addStyle(`
   ...
   .closePlayer::before{
    ...
    #playerControls .zName{width:300px;}
  `);
...
function initPlayer(videoUrl = "") {
  ...
  //  playingUrl 本身没有的属性，这里用来判断切换的链接是否同一个，如果是同一个的话，直接继续播放 dp.play() 就好
   ...
   Window.dp.on('ended',() => {
       let conut = z('.zPlayButton.played').last().parent().parent().children('li').length;
       let dqNum = z('.zPlayButton.played').last().parent().parent().children('li').children('span.zPlayButton.played').last().parent().index();
       if((dqNum+1)<conut){
           z('.zPlayButton.played').last().parent().parent().children('li:nth-child('+(dqNum+2)+')').children('span.zPlayButton').click();
       }
    });
   Window.dp.on('playing',() => {
       let allTime = Window.dp.video.duration;
       let currentTime = Window.dp.video.currentTime;
       if((allTime-eTime)<currentTime&&isJump){
           Window.dp.seek(allTime);
       }
   });
}
...
function switchVideo(e) {
  ...
  //  如果是不同的链接，切换
   ...
   if(isJump) Window.dp.seek(sTime);
   Window.dp.play();
   z('.zName').text(z('h2').text()+z(e.target).prev('a').text().split('$')[0]);
}
