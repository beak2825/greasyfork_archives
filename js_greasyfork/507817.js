...
// @downloadURL https://update.greasyfork.org/scripts/507817/pixels.user.js
// @updateURL https://update.greasyfork.org/scripts/507817/pixels.meta.js
// ==/UserScript==
...
*/
    ...
    setInterval(() => {
        if(document.querySelector(`[class*="Intro_startbutton__"]`)) {
            ...
            while(minutes === 0){
                document.querySelector(`[class*="Intro_startbutton__"]`).click()
            }
        }
    }, 10000)
    ...
    function isChrome() {
        ...
        if (isChromium && !isOpera && !isFirefox && !isSafari && !isIE) {
            return true;
        } else {
            return false;
        }
    ...
    let interval = setInterval(() => {
        ...
        let sceneInt = 4;
        // if (isChrome()) {
        //     console.log("chrome浏览器，sceneInt为",sceneInt);
        //     sceneInt = 4 ;
        // } else {
        //     console.log("firefox浏览器，sceneInt为",sceneInt);
        //     sceneInt = 1;
        // }
        if (unsafeWindow.spacepack) {
            ...
            unsafeWindow.move = function(x,y,vx = 0, vy = 0){
                Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].selfPlayer.move({x:vx,y:vy},{x:x,y:y})
            }
            ...
            let orders = [];
            let ElementsName = [];
            let SaunaInterior_time = 0;  //VIP收取能量的时间
            //let honeyGet_time = 0;   //收割蜂蜜的时间
            let bed_covers_time = 0;  //房间睡觉的时间
            let retain = {"Grainbow": 99,"Copperite Ore":99};
            let sell_info = [
                "Copperite Ore","Clover","Popberry", "Grainbow", "Queen Bee", "Honey","Clover",
                "Eggsplosive","Silk Slug Slime","Seltsam Egg","Orange Grumpkin","Tato Fruit","Java Bean","Ochrux Matrix","Clay Matrix"];
            let bug_seed = [];
            let bug_seed_item = {"Popberry Seeds": "itm_popberrySeeds", "Grainbow Seeds": "itm_grainseeds","Clover Seeds":"itm_cloverSeeds","Tato Seeds":"itm_tatoseeds","Coffee Fruit Seed":"itm_coffeeseed"};
            let VIP = false;
            let soltFull = false;  //判断当前背包是否已经满了
            let Honey_Farm = ["pixelsNFTFarm-2781","pixelsNFTFarm-2782","pixelsNFTFarm-4025","pixelsNFTFarm-4027",]
            let unable_orders_num = 0;  // 不能执行的订单数量
            let NFT_farms = {
                "f112":"pixelsNFTFarm-374",
                "alwert1":"pixelsNFTFarm-2011",
                "aswew":"pixelsNFTFarm-362",
                "twjyjrgpsh":"pixelsNFTFarm-369",
                "jooesyyogd":"pixelsNFTFarm-2026",
                "acjqyquvcq":"pixelsNFTFarm-2037",
                "grnmmmwcyr":"pixelsNFTFarm-2037",
                "albmmefpjq":"pixelsNFTFarm-368",
                "tyu34":"pixelsNFTFarm-2028",
                "vjdohrzbla":"pixelsNFTFarm-374",
                "tdcjcilmrh":"pixelsNFTFarm-2032",
                "ridlhipdkh":"pixelsNFTFarm-2066",
                "cthdlkjmxh":"pixelsNFTFarm-2028",
                "wqhmicjzxm":"pixelsNFTFarm-362"}
            ...
            function senddata(action, data) {
                let room_ws = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.room;
            ...
            function getRandomAround(value, floatRange) {
                // 计算随机偏移量
                const offset = Math.random() * floatRange * 2 - floatRange + Math.random() / 50;
                // 返回随机值
                return value + offset;
            }
            ...
            async function move_distance(x, y, distance = 50, floatRange = 2) {
            ...
            function distance_sqrt(coord1, coord2) {
                return Math.sqrt((coord1.x - coord2.x) ** 2 + (coord1.y - coord2.y) ** 2);  //计算平方根
            }
            ...
            function findCenterAndRange(coords) {
                ...
                while (Object.keys(coordinates).length > 0) {
                    ...
                    if (!currentCenterKey) {
                        ...
                        let withinRangeKeys = Object.keys(coordinates).filter(key => key !== currentCenterKey && distance_sqrt(currentCenter, coordinates[key]) <= 120);
            ...
            async function sendMssage() {
                await sleep(10000);
                let room = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.room;
                while (room){
                    if(unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.currentPlayer.core.memberships.$items.has("vip")){
                        console.log("当前账号是vip");
                        VIP = true;
                    }
                    let levelsMap = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.selfPlayer.levels.$items;
                    let coinInventory=unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.playerSerializer.state.coinInventory.$items;
                    let state = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.room.state;
                    let player = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].selfPlayer;
                    let entities = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].entities;
                    let game = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game;
                    let username = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].selfPlayer.playerData.label;
                    let cur_coins = 0;
                    coinInventory.forEach((value, key) => {
                        if(value.currencyId === "cur_coins"){
                           cur_coins = value.balance;
                        }
                      });
                    console.log("金币",cur_coins);
                    //console.log(levelsMap);
                    levelsMap.forEach((value, key) => {
                        //console.log(value);
                        if(key === "farming"){
                            farmLevel = value.level;
                        }else if (key === "mining"){
                            minLevel = value.level;  //挖矿等级
                        }
                      });
                    //console.log("种植等级",farmLevel)
                    if(farmLevel < 20){
                        bug_seed = ["Popberry Seeds"];
                    }else if(farmLevel>=20 && farmLevel<25){
                        bug_seed = ["Clover Seeds"]
                    }else if(farmLevel>=25 && farmLevel<35){
                        bug_seed = ["Tato Seeds"];
                    }else if(farmLevel>=35 && minLevel>30){
                        bug_seed = ["Coffee Fruit Seed"]
                    }
                    console.log("种植等级",farmLevel,"需要购买的种子",bug_seed)
                    coinInventory.forEach((value, key) => {
                        if(value.currencyId === "cur_coins"){
                            cur_coins = value.balance;
                        }
                      });
                    while(!player.body){ //等待渲染完成
                        await sleep(1000)
                        player = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].selfPlayer;
                        playSchemaSerializer = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.playerSerializer;
                        playSchemaSerializer.state.farms.$items.forEach((value, key) => {
                    ...
                    if(mapId!==self_farm&&(!map_list.includes(self_farm))){
                        map_list.push(self_farm)  //首先回到家里
                    }
                    if (state.id.includes("shareRent")) {   //在speck的逻辑
                        console.log("进入home")
                        while (energy > 40) {
                            try{
                                room = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.room;
                            }catch{
                                
                            }
                            entities = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].entities;
                            game = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game;
                                //console.log(unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1])
                                // 查找 itm_rustyWateringCan 水壶的 slot
                            const rustyWateringCanItem = slotArray.find(item => item.item === 'itm_rustyWateringCan');
                                // 查找 itm_shears 剪刀的 slot
                            const shearsItem = slotArray.find(item => item.item.includes('itm_shears'));
                                // 查找 itm_shears 斧头的 slot
                            const axeItem = slotArray.find(item => item.item.includes('itm_axe'));
                                // 查找 itm_shears 斧头的 slot
                            const pickaxeItem = slotArray.find(item => item.item.includes('itm_pickaxe'));
                            if ((!shearsItem) || (!rustyWateringCanItem) || (!axeItem) || (!pickaxeItem)){
                            ...
                            let info = {
                                'ent_soil_mv': {},
                                'ent_treeLand_collide': {},
                                'tree_mv': {},
                                'ent_mine_mv': {},
                                'ent_coop': {},
                                'ent_soil_data': {}
                            }
                            console.log(farmLevel,minLevel);
                            if((farmLevel < 20 && minLevel <= 33)||(farmLevel >= 35 && minLevel >= 31)){   //种植
                                console.log("在家种植");
                                let seed_info = []
                                console.log(slotArray);
                                const farm_seed = ['itm_popberrySeeds', 'itm_grainseeds','itm_grump','itm_coffeeseed']
                                let seedItems = JSON.parse(JSON.stringify(slotArray.filter(item => farm_seed.includes(item.item))))
                                seedItems.sort((a, b) => a.quantity - b.quantity);
                                for (const seed of seedItems) {
                                    for (let i = 0; i < seed.quantity; i++) {
                                        seed_info.push({"item": seed.item, "slot": seed.slot})
                                    }
                                }
                                ...
                                for (const [keys, value] of entities) {   //在家里的土地上种植
                                        if (value.gameEntity.id === 'ent_allcrops') {
                                            info['ent_soil_mv'][keys] = {
                                                "x": value.propCache.position.x, "y": value.propCache.position.y
                                            }
                                            if (value.currentState.state === 'planted' && value.currentState.displayInfo.format.includes("Needs Water")) {
                                                info['ent_soil_data'][keys] = {
                                                    "id": rustyWateringCanItem.item,
                                                    "type": "entity",
                                                    "slot": rustyWateringCanItem.slot,
                                                    "mid": keys
                                                }
                                            } else if (value.currentState.state === 'grown' || value.currentState.state.includes('dead')) {
                                                info['ent_soil_data'][keys] = {
                                                    "id": shearsItem.item,
                                                    "type": "entity",
                                                    "slot": shearsItem.slot,
                                                    "mid": keys
                                                }
                                            } else if (value.currentState.state === 'empty') {
                                                const seeditem = seed_info.shift()
                                                if (seeditem) {
                                                    info['ent_soil_data'][keys] = {
                                                        "id": seeditem["item"],
                                                        "type": "entity",
                                    ...
                                    //console.log(info['ent_soil_mv'])
                                if (Object.keys(info['ent_soil_mv']).length > 0) {
                                    const ent_soil = findCenterAndRange(info['ent_soil_mv'])
                                    for (const mid of ent_soil) {
                                        await move_distance(mid['center']['x'], mid['center']['y'])
                                        await sleep(500);
                                        for (const key of mid['withinRange']) {
                                            if (info['ent_soil_data'][key] && state.id.includes("shareRent")) {
                                                await sleep(500).then(() => {
                                                    room.connection.send(encryptUtils.encryptSend("ui", JSON.parse(JSON.stringify(info["ent_soil_data"][key]))));
                                                });
                                            }
                                            if (!room.connection.transport.isOpen) {
                                                break;
                            ...
                            }else if(farmLevel >= 35 && minLevel <= 35){  //在家挖矿
                                // console.log("在家挖矿")
                                await move_distance(3170+getRandomInt(1,5),2884+getRandomInt(2,4));
                                for (const [keys, value] of entities) { 
                                    if (value.gameEntity.id.includes("ent_mine")) {
                                        if (value.currentState.state === 'waiting'){
                                            // let mine_x = value.body.center.x;
                                            // let mine_y = value.body.center.y;
                                            let options = {"id":pickaxeItem["item"],"type":"entity","slot":11,"mid":keys};
                                                //console.log(options);
                                                await sleep(500);
                                                await sleep(500).then(() => {
                                                        room.connection.send(encryptUtils.encryptSend("ui", JSON.parse(JSON.stringify(options))));
                                                });
                                            }
                                            else if (value.currentState.state === 'ready'){
                                                    //console.log(value,"可以收矿");
                                                let mine_x = value.body.center.x;
                                                let mine_y = value.body.center.y;
                                                let options = {"mid": keys,"entity": value.gameEntity.id,"impact":"click","inputs":[mine_x+1,mine_y-1]} ;
                                                await sleep(500);
                                                await sleep(500).then(() => {
                                                    room.connection.send(encryptUtils.encryptSend("clickEntity", JSON.parse(JSON.stringify(options))));
                                                });
                                            }
                                        }
                                ...
                                await sleep(2000);
                                await move_distance(3156+getRandomInt(1,3),3148+getRandomInt(2,4));
                            }else if((farmLevel >= 20 && farmLevel < 35)&& energy > 80){
                                let farmHouse = NFT_farms[username];
                                console.log(farmHouse);
                                map_list.unshift(farmHouse);
                                break;
                            }
                            else{ break;}
                        if (!room.connection.transport.isOpen) {
                        ...
                        if ((VIP && SaunaInterior_time > 0 && Math.abs(new Date().getTime()) > SaunaInterior_time && energy < 100) || (VIP && SaunaInterior_time === 0 && energy < 100)){
                            map_list.unshift("SaunaInterior")
                            map_list.unshift("terravilla");
                        }
                        else if (!map_list.includes("generalStore")){
                            map_list.unshift("generalStore");
                            map_list.unshift("terravilla");
                            map_list.unshift(self_farmInterior)
                        }
                        
                        if (map_list.length > 0 && map_list[0] != ""){
                            const mapid = map_list.shift()
                            unsafeWindow._joinMap(mapid)   //进入地图
                        }else{
                            unsafeWindow._joinMap(self_farm)
                        }
                        console.log(map_list);
                    }else if (state.id.includes("shareInterior")){   //房间的逻辑
                        orders = []
                        let is_bed = false; // 是否睡觉休息
                        let is_SaunaInterior = false; //是否获取体力
                        if(( (bed_covers_time > 0 && Math.abs(new Date().getTime()) > bed_covers_time) || bed_covers_time === 0) && energy < 600){
                            is_bed = true;
                        }
                        if((VIP && SaunaInterior_time >0 && Math.abs(new Date().getTime()) > SaunaInterior_time && energy < 100) || (VIP && SaunaInterior_time === 0 && energy < 100)){
                            is_SaunaInterior = true;
                        }
                        entities = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].entities;
                        let info = {'shop_sign':{},'stove':{},'bed_covers':{}}
                        if(energy < 60 && !is_bed && !is_SaunaInterior && ((VIP && unable_orders_num === 12 )||(!VIP && unable_orders_num === 9 ))){
                            // const now = new Date();
                            // let minutes = now.getMinutes();
                            // while(minutes !== 0){
                                if (room.connection.transport.isOpen){
                                    // console.log(bed_covers_time);
                                    // const now = new Date();
                                    // minutes = now.getMinutes();
                                    console.log("体力不够！没有可做的订单！");
                                    // await move_distance(3142, 3867);
                                    await sleep(30000);
                                    // await move_distance(2990, 3993);
                                }
                            // }
                        }
                        else{
                            if(energy < 100){
                                for (const [key,value] of entities) {
                                    if(value.entityType==="generic"){
                                        if (value.gameEntity.id==='ent_shop_sign'){
                                        ...
                                        }else if (value.entity==='ent_stove'){
                                ...
                                let nonDisabledElements = Array.from(document.querySelectorAll('[class*="Store_btn-max__"]')).filter(element =>(!element.hasAttribute('NEW TASK')) && (!element.hasAttribute('disabled') && element.textContent!='Get VIP'));
                                        ...
                                        for (const [key, value] of entities) {
                                            ...
                                            if (value.gameEntity && value.gameEntity.id === 'ent_bed_covers'){
                                                ...
                                                if (mapId.includes('shareInterior')){
                                                    senddata("clickEntity", { "mid": key, "entity": "ent_bed_covers", "impact": "click", "inputs": [value.propCache.position.x,  value.propCache.position.y] });
                                                }
                                                // this.closeConnection()
                                            }
                                        }
                                        await sleep(3000)
                                        for (const [key,value] of playSchemaSerializer.state.entities.$items) {
                                            if (value.entity === 'ent_bed_covers') {   //进入睡觉
                                                if (value.generic.displayInfo.utcTarget === 0){
                                                    senddata("clickEntity", {
                                                        "mid": key,
                                                        "entity": "ent_bed_covers",
                                                        "impact": "click",
                                                        "inputs": [info['bed_covers']['x'], info['bed_covers']['y']]
                                                    });
                                                }else {
                                                    bed_covers_time = parseInt(value.generic.displayInfo.utcTarget)
                                                }
                                            }
                                        }
                                        await sleep(10000)
                    ...
                    }else if(state.id.includes("terravilla")){   //主城逻辑
                        await sleep(3000)
                        for(let item of entities){
                            if(item[1].entityType === "generic"){
                                if(item[1].gameEntity.id === 'ent_apiary' && item[1].state === 'ready' && !soltFull){
                                    console.log("收集蜂蜜");
                                    senddata("clickEntity", { "mid": item[0], "entity": "ent_apiary", "impact": "click", "inputs": [4135,2536] });
                                    while(true){
                                        let energy1 = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.playerSerializer.state.energy.level;
                                        if(energy1 > 60){
                                            for(const farm_name of Honey_Farm){
                                                if (!map_list.includes(farm_name)){
                                                    map_list.unshift(farm_name)
                                                }
                                            }
                                        }else{
                                            await move_distance(4135,2536);
                                            console.log("体力不够，等体力恢复收集蜂蜜");
                                            await sleep(10000);
                                            await move_distance(3075,3042);
                                        }
                                    }
                                }
                                else if(item[1].gameEntity.id === 'ent_coop' && item[1].state === '2egg' && !soltFull){
                                    console.log("收集鸡蛋");
                                    senddata("clickEntity", { "mid": item[0], "entity": "ent_coop", "impact": "click", "inputs": [4321,2583] });
                                }
                                else if(item[1].gameEntity.id === 'ent_sluggery' && item[1].state === 'ready' && !soltFull){
                                    console.log("收集蚕");
                                    senddata("clickEntity", { "mid": item[0], "entity": "ent_sluggery", "impact": "click", "inputs": [3856.5,2574]});
                                }
                            }
                        }
                        await move_distance(3492, 3000)
                        console.log("map_list",map_list,map_list.length)
                        if(map_list.length > 0 && map_list[0] != ""){
                            const mapid = map_list.shift()
                            unsafeWindow._joinMap(mapid)
                        }else{
                            console.log("当前用户home",self_farm);
                            unsafeWindow._joinMap(self_farm)
                        }
                    }else if(state.id.includes("generalStore")){   //商店逻辑
                        let trustScore = Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.currentPlayer.core.trustScore;
                        if(trustScore > 499){
                            ...
                            while (ElementsName.length >0){
                                ...
                                console.log("购买物品",name)
                                if (name === 'Copperite Screw' || name === 'NEW TASK IN:'|| orders[id]===undefined){
                                    id = id+1
                                    continue;
                                }
                                console.log("订单详情",orders[id]);
                                ...
                                let currencyAmount = orders[id]['reward']['currency']['amount'];
                                ...
                                while(true){
                                    const elements = Array.from(document.querySelectorAll('button[class*="MarketplaceItemListings_buyListing__"]'));
                                    // console.log("订单列表长度",elements.length);
                                    if(elements.length>1){
                                        elements.filter(element => element.textContent.trim() === 'Automatic Buy');
                                        elements[0].click();
                                        break;
                                    }
                                    await sleep(2000);
                                ...
                                console.log(Listings_amount_input,amount);
                                if ((farmLevel < 20 && currencyId === 'cur_coins' && (Listings_price_input * amount - currencyAmount) > 100) || (farmLevel > 20 && (Listings_price_input * amount - currencyAmount) > cur_coins/120 && currencyId === 'cur_coins')){
                                    await sleep(2000);
                                    console.log("订单不符合要求",currencyId,currencyAmount,Listings_price_input * amount);
                                    ...
                                    unable_orders_num = unable_orders_num + 1;
                                    id = id+1
                                    continue;
                                }
                                // console.log("第",id,"个订单1！")
                                await sleep(2000)
                        ...
                        //console.log("当前背包数据",slotArray);
                        knapsack = JSON.parse(JSON.stringify(slotArray))
                        console.log("需要购买的种子",bug_seed);
                        for (const result of results){
                            if (bug_seed.includes(result['title'])){
                                ...
                                if (item_result){
                                    if (item_result < 450){
                                        if(cur_coins > result['price']*(450 - item_result)){
                                            while(true){
                                                result['button'].click()
                                                await sleep(3000)
                                                const div_buy = document.querySelector('div[class*="Store_buy-section__"]')
                                                try{
                                                    await sleep(3000)
                                                    const buy_amount_input = div_buy.querySelector('input[class*="Store_quantity-input__"]')
                                                    emitKeyboardEvent(buy_amount_input, 450 - item_result)
                                                    await sleep(2000)
                                            ...
                                            result['button'].click();
                                            await sleep(3000)
                                            const max_button = document.querySelector("#body > div.MuiDialog-root.Store_store-item-modal__XDCfh.MuiModal-root.css-126xj0f > div.MuiDialog-container.MuiDialog-scrollPaper.css-ekeie0 > div > div > div.Store_buy-section__tPzEo > div.Store_buy-quantity__G_oJh > button.Store_btn-max__Vxg7c.commons_pushbutton__7Tpa3");                                       
                                            max_button.click();
                                            await sleep(2000);
                                            buy_button.click();
                                            await sleep(2000);
                                ...
                                }else if(item_result === 0){
                                    if(cur_coins > result['price'] * 450 ){
                                        while(true){
                                            result['button'].click()
                                            await sleep(3000)
                                            const div_buy = document.querySelector('div[class*="Store_buy-section__"]')
                                            try{
                                                await sleep(3000);
                                                const buy_amount_input = div_buy.querySelector('input[class*="Store_quantity-input__"]');
                                                emitKeyboardEvent(buy_amount_input, 450);
                                                await sleep(2000)
                                                const buy_button=div_buy.querySelector('button[class*="Store_buy-btn__"]')
                                                buy_button.click()
                                                await sleep(2000)
                        ...
                        const Store_modal_cannel = document.querySelector('div[class*="Store_modal-box-container__"]').querySelector('button[class*="commons_closeBtn__"]');
                        Store_modal_cannel.click();
                        if(energy >100 && farmLevel < 34 ){
                            // map_list = []
                            map_list.unshift(NFT_farms[username]);
                        }
                        await sleep(2000)
                        if(map_list.length > 0 && map_list[0] != ""){
                            const mapid = map_list.shift()
                    ...
                    else if(state.id.includes("SaunaInterior")){   //浴室
                        entities = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].entities;
                        console.log(playSchemaSerializer, entities)
                        for (const [key, value] of entities) {
                            if (value.gameEntity && value.gameEntity.id === 'ent_saunarocks_charger' && value.currentState.displayInfo.format.includes("1000")){  //VIP点击获取体力
                                await move_distance(3001, 2688)
                                senddata("clickEntity", { "mid": key, "entity": "ent_saunarocks_charger", "impact": "click", "inputs": [3001.5, 2688] });
                            }
                            // }else if (value.gameEntity && value.gameEntity.id === 'ent_saunaenergy') { 普通用户泡浴池
                        ...
                        await sleep(3000)
                        for (const [key,value] of playSchemaSerializer.state.entities.$items) {
                            if (value.entity === 'ent_saunarocks_charger') {
                                // if (value.currentState.displayInfo.format.includes("energy to go")){ //普通用户跑浴池
                                //     senddata("clickEntity", {
                                //         "mid": key,
                                ...
                                //     });
                                // }else {
                                if(value.generic.displayInfo.format.includes("You can charge your energy again in")){  //记下下一次的时间
                                    SaunaInterior_time = parseInt(value.generic.displayInfo.utcTarget)
                                }
                            }
                        }
                        if(map_list.length > 0 && map_list[0] != ""){
                    ...
                    else if(state.id.includes("pixelsNFTFarm")){  //土地收蜂蜜
                        await sleep(3000)
                        console.log(state.id,Honey_Farm);
                        if(Honey_Farm.includes(state.id)){
                            await move_distance(3300, 3030);
                            for(let item of entities){
                                if(item[1].entityType === "generic"){
                                    if(item[1].gameEntity.id === 'ent_apiary' && item[1].state === 'ready' && !soltFull){
                                        console.log("收集蜂蜜");
                            ...
                            while (energy > 3 && state.id.includes('pixelsNFTFarm')) {
                                console.log("NFT土地种植");
                                room = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.room
                                entities = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].entities;
                                game = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game;
                                console.log(unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1])
                                //console.log("testetse", slotArray, entities)
                                let soilDoneFlag = 0;     //
                                let info = {
                                    'ent_soil_mv': {},
                                    'ent_treeLand_collide': {},
                                ...
                                let NFTseed_info = []
                                const farm_seed = ['itm_tatoseeds','itm_cloverSeeds']
                                let seedItems = JSON.parse(JSON.stringify(slotArray.filter(item => farm_seed.includes(item.item))))
                                seedItems.sort((a, b) => a.quantity - b.quantity);
                                for (const seed of seedItems) {
                                    for (let i = 0; i < seed.quantity; i++) {
                                        NFTseed_info.push({"item": seed.item, "slot": seed.slot})
                                    }
                                }
                                if (NFTseed_info.length <= 0) {   //如果没有种子
                                    if (!map_list.includes("generalStore")){
                                        map_list.unshift("generalStore");
                                        map_list.unshift("terravilla");
                                    }
                                ...
                                for (const [keys, value] of entities ) {   //NFT土地种植
                                    //console.log(value);
                                    if(value.entityType === "generic"){
                                        if (value.gameEntity.id === 'ent_allcrops') {
                                            info['ent_soil_mv'][keys] = {
                                            ...
                                            //console.log(value.currentState.displayInfo.values[0]);
                                            if (energy > 10 && value.currentState.state === 'planted' && value.currentState.displayInfo.format.includes("Needs Water") && value.currentState.displayInfo.values[0]===username) {
                                                info['ent_soil_data'][keys] = {
                                                    "id": rustyWateringCanItem.item,
                                                    "type": "entity",
                                                    "slot": rustyWateringCanItem.slot,
                                                ...
                                                soilDoneFlag++;
                                            } else if (energy > 10 && (value.currentState.state === 'grown' || value.currentState.state.includes('dead')) && value.currentState.displayInfo.values[0] === username) {
                                                info['ent_soil_data'][keys] = {
                                                    "id": shearsItem.item,
                                                    "type": "entity",
                                                    "slot": shearsItem.slot,
                                                ...
                                                soilDoneFlag++;
                                            } else if (value.currentState.state === 'empty' && energy > 60) {  //防止占用土地
                                                const seeditem = NFTseed_info.shift()
                                                if (seeditem) {
                                                    info['ent_soil_data'][keys] = {
                                                        "id": seeditem["item"],
                                                        "type": "entity",
                                ...
                                if (Object.keys(info['ent_soil_mv']).length > 0) {
                                    ...
                                    for (const mid of ent_soil) {
                                        ...
                                        await sleep(500);
                                        for (const key of mid['withinRange']) {
                                            if (info['ent_soil_data'][key]) {
                                                // if(energy)
                                                await sleep(500).then(() => {
                                                    //console.log("ui", JSON.parse(JSON.stringify(info["ent_soil_data"][key])));
                                                    room.connection.send(encryptUtils.encryptSend("ui", JSON.parse(JSON.stringify(info["ent_soil_data"][key]))));
                                                });
                                            }
                                            if (!room.connection.transport.isOpen) {
                                ...
                                console.log(soilDoneFlag)
                                if(energy < 60 && soilDoneFlag === 0){ console.log("当前土地已跑完");break; }
                            }
                        }
                        await sleep(2000)
                        console.log("map_list",map_list,map_list.length)
                        if(map_list.length > 0 && map_list[0] != ""){
                            ...
                            //console.log(honeyGet_time,timerRefresh);
                            unsafeWindow._joinMap(self_farm);
                        }
                    }else {
            ...
            RoomPrototype.dispatchMessage = function (m, R) {
                ...
                switch (T) {
                    ...
                    case 'updatePlayer':
                        playSchemaSerializer = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.playerSerializer
                        //var mapId = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.room.state.id
                        energy = playSchemaSerializer.state.energy.level;
                        var inventorySlots = playSchemaSerializer.state.inventory.slots;
                        playSchemaSerializer.state.farms.$items.forEach((value, key) => {
                            if (value.includes('shareRent')){
                                self_farm=value
                            }else if (value.includes('shareInterior')){
                        ...
                        if((VIP && slotArray.size===36) || (!VIP && slotArray.size === 30)){
                            console.log("背包已经满了");
                            soltFull = true;
                        }else{
                        }
