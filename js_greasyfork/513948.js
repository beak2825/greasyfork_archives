// ==UserScript==
// @name        Bangumi-Rename-Related-Subjects-Fork
// @namespace   BRRS
...
// @version     1.1.4-alpha
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/513948/Bangumi-Rename-Related-Subjects-Fork.user.js
// @updateURL https://update.greasyfork.org/scripts/513948/Bangumi-Rename-Related-Subjects-Fork.meta.js
// ==/UserScript==
...
templates.platforms = (function () {
  ...
  platformContent = '';
  tpl =
    '<label for="platform-{{bgmid}}-{{id}}"><input id="platform-{{bgmid}}-{{id}}" type="radio" value="{{id}}" name="platform-{{bgmid}}"> {{name}}</label>';
  ref = platforms[platform];
  for (j = 0, len = ref.length; j < len; j++) {
    i = ref[j];
    platformContent += tpl
      .replace(/{{meta}}/g, i.meta)
      .replace(/{{id}}/g, i.id)
  ...
  toolkit: {
    restore: function () {
      ...
      return $('.brrs-editor-item .brrs-col-platform').each(function () {
        return $(this)
          .find('[value="' + $(this).attr('data-origin') + '"]')
          .attr('checked', true);
      });
    },
    removechs: function () {
...
Bootstrap = function (subjects) {
  ...
  for (key = j = 0, len = subjects.length; j < len; key = ++j) {
    ...
    results.push(
      $.get('/subject/' + value.bgmid + '/edit_detail', function (data) {
        var bgmid;
        ...
        Core.editor.setPlatform(
          bgmid,
          (data.match(/value="(\d+)"\s+onclick="WikiTpl\('[^']+'\)"\s+checked>/) || ['', ''])[1]
        );
...
EventBinding = function () {
  ...
  $('#brrs-save').click(function () {
    ...
    for (j = 0, len = submitCue.length; j < len; j++) {
      ...
      results.push(
        $.post('/subject/' + i.bgmid + '/new_revision', postData, function () {
          Core.loading.setProgress(Core.loading.getLoaded() + 1);
          if (Core.loading.getTotal() === Core.loading.getLoaded()) {
