// ==UserScript==
// @name         Localhost Bet List Cleaner & DOM Replacement
...
// @run-at       document-end
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/554599/Localhost%20Bet%20List%20Cleaner%20%20DOM%20Replacement.user.js
// @updateURL https://update.greasyfork.org/scripts/554599/Localhost%20Bet%20List%20Cleaner%20%20DOM%20Replacement.meta.js
// ==/UserScript==
...
(function () {
    ...
    const FIXED_ID_SUFFIX = '8456732224'; // остава наличен за други нужди
    ...
    function applyReplacements(root = document) {
        try {
            // --- 0) Нова цел: div.text-truncate.bet-item__text--secondary "Селтик, Алавес" -> "Гуастатоя, 2:0" (и title)
            ...
            // --- 3) span.WXm9U: замяна на отборни имена в innerHTML ---
            root.querySelectorAll('span.WXm9U').forEach(sp => {
                ...
                if (/Глазгоу/.test(html)) { html = html.replace(/Глазгоу/g, 'Гуастатоя'); changed = true; }
            ...
            // --- 6) span.bet-item__text--date: 17:59 -> 12:08 ---
            root.querySelectorAll('span.bet-item__text--date').forEach(sp => {
                replaceTextIfExact(sp, '17:59 ч. 02.11.2025 г.', '12:08 ч. 02.11.2025 г.');
            });

            // --- 7) div.tlLw1: Краен Резултат -> Точен Резултат ---
            root.querySelectorAll('div.tlLw1').forEach(d => {
                replaceTextIfExact(d, 'Краен Резултат', 'Точен Резултат');
            });
    ...
    console.log('✅ Localhost Bet List Cleaner & DOM Replacement (v1.4) active.');
})();