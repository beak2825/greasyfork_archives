...
// @grant       GM_setValue
// @namespace   no u
// @downloadURL https://update.greasyfork.org/scripts/407920/sbb%20afk.user.js
// @updateURL https://update.greasyfork.org/scripts/407920/sbb%20afk.meta.js
// ==/UserScript==
(function(){//info
...
var mdx = (0.5 * window.innerWidth), mdy = (0.5 * window.innerHeight);
var scaleFactor=3.1;
var xOffset, yOffset;
var scriptBotFlag=false;
var MasterMouseX, MasterMouseY;
var slaveMouseX, slaveMouseY;
var legitFactoryFlag=false;
var cx = (0.5 * window.innerWidth), cy = (0.5 * window.innerHeight), r = 5;
var a = 0, u = 0;
var xMouseOffset=0;
var yMouseOffset=0;
var faceMouseXOffset, faceMouseYOffset;
var tx = 0, ty = 0;
var repelFlag=false;
var faceMouseFlag=false;
var afkFlag=false;
var afkX=0, afkY=0;
var scriptInputKey_Game = 0;
window.updateInfo && window.updateInfo("h", " ");
window.updateInfo && window.updateInfo("hh", " ");
window.updateInfo && window.updateInfo("hhh", " ");
window.updateInfo && window.updateInfo("hhhhhhhh", "AFK Mode Enabled: "+afkFlag);
...
function _classCallCheck(t, e) {
    ...
    2: [function(L, t, R) {
        (function(c) {
            ...
            R.kMaxLength = r, (c.TYPED_ARRAY_SUPPORT = function() {
                ...
                if (x(t, Uint8Array) && (t = c.from(t, t.offset, t.byteLength)), !c.isBuffer(t)) throw new TypeError('The "target" argument must be one of type Buffer or Uint8Array. Received type ' + (void 0 === t ? "undefined" : _typeof(t)));
        ...
        e.exports = {
            inject: function(n, i) {
                ...
                return window.WebSocket.prototype.send = function(t) {
                    var e = new Uint8Array(t);
                    if (!o.has(this)) {
                        console.log("Hello: New WebSocket is being used."), o.add(this);
    ...
    8: [function(t, e, r) {
        ...
        // *** ƒ^ƒ“ƒNî•ñ‚Ì•Ï”éŒ¾ *** //
        const TANK_SWITCH_CLASS = -1, TANK_LOOP_BREAK = -2;
        window.switchPeriod = 1, window.isOn = 0;
        // *** ƒQ[ƒ€‚Ìˆ— *** //
        var h = n(null, function(t) {
            var e = this;
            // *** ƒT[ƒo[‚ÉƒQ[ƒ€ƒf[ƒ^‚ð‘—M *** //
            window.sendPackets = function(t) {
                return s(e, t)
            };
            // *** ƒNƒ‰ƒCƒAƒ“ƒg‚ÌƒQ[ƒ€ƒf[ƒ^‚ðŽóM*** //
            var dX = GM_getValue("MasterMouseX");
            var dY = GM_getValue("MasterMouseY");
            var r = new o(t).parseOutbound();
            
            // *** ƒƒjƒ…[‚Ì•`‰æ *** //
            // *** ƒƒjƒ…[‚Ì•`‰æ *** //

            if (r.kind !== i.outPacketKinds.INPUT) {
                return t;
            }
            if (r.kind == 1) {
                tx = r.x;
            ...
            // *** Bot‚ searchforhere*** //
            s(this, [r]);
            if (scriptBotFlag == false) {
                GM_setValue("GM_diepPosX", tx);
                GM_setValue("GM_diepPosY", ty);
            }
            var button0 = GM_getValue("GM_Diep_Mouse0");
            var button1 = GM_getValue("GM_Diep_Mouse1");
            var button2 = GM_getValue("GM_Diep_Mouse2");
            var tankposxy = [GM_getValue("GM_diepPosX"), GM_getValue("GM_diepPosY")];
            var scriptInputKey = i.keyInput.LEFT_MOUSE;
            var tankposX=tankposxy[0];
            var tankposY=tankposxy[1];
            var trueTankX=tankposX-dX*scaleFactor;
            var trueTankY=tankposY-dY*scaleFactor;
            if (scriptBotFlag) {
                input.keyUp(68);
                input.keyUp(65);
                input.keyUp(83);
                input.keyUp(87);
                var slaveTankX=tx-dX*scaleFactor;
                var slaveTankY=ty-dY*scaleFactor;
                xOffset=trueTankX-slaveTankX;
                yOffset=slaveTankY-trueTankY;
                if (afkFlag) {
                    xOffset=afkX-slaveTankX;
                    yOffset=slaveTankY-afkY;
                }
                var angleOffset=Math.atan2(yOffset, xOffset)*180/Math.PI; //Degree Offset
                if (angleOffset<=22.5 && angleOffset>=-22.5) {
                    //D
                    input.keyDown(68);
                    scriptInputKey = scriptInputKey + i.keyInput.RIGHT;
                }
                if (angleOffset<=67.5 && angleOffset>=22.5) {
                    //D+W
                    input.keyDown(68);
                    scriptInputKey = scriptInputKey + i.keyInput.RIGHT;
                    input.keyDown(87);
                    scriptInputKey = scriptInputKey + i.keyInput.UP;
                }
                if (angleOffset<=112.5 && angleOffset>=67.5) {
                    //W
                    input.keyDown(87);
                    scriptInputKey = scriptInputKey + i.keyInput.UP;
                }
                if (angleOffset<=157.5 && angleOffset>=112.5) {
                    //W+A
                    input.keyDown(65);
                    scriptInputKey = scriptInputKey + i.keyInput.LEFT;
                    input.keyDown(87);
                    scriptInputKey = scriptInputKey + i.keyInput.UP;
                }
                if (angleOffset<=-157.5 || angleOffset>=157.5) {
                    //A
                    input.keyDown(65);
                    scriptInputKey = scriptInputKey + i.keyInput.LEFT;
                }
                if (angleOffset<=-22.5 && angleOffset>=-67.5) {
                    //D+S
                    input.keyDown(68);
                    scriptInputKey = scriptInputKey + i.keyInput.RIGHT;
                    input.keyDown(83);
                    scriptInputKey = scriptInputKey + i.keyInput.DOWN;
                }
                if (angleOffset<=-67.5 && angleOffset>=-112.5) {
                    //S
                    input.keyDown(83);
                    scriptInputKey = scriptInputKey + i.keyInput.DOWN;
                }
                if (angleOffset<=-112.5 && angleOffset>=-157.5) {
                    //S+A
                    input.keyDown(65);
                    scriptInputKey = scriptInputKey + i.keyInput.LEFT;
                    input.keyDown(83);
...
var const_SC = getScreenConstant();
    ...
    document.addEventListener('mousemove', function(e)
	{
        if (!scriptBotFlag) {
            ...
            GM_setValue("MasterMouseX", e.clientX - (window.innerWidth / 2));
            GM_setValue("MasterMouseY", e.clientY - (window.innerHeight / 2));
        }
	});
    ...
    function SyncMouse()
    {
        ...
        var clamped;
        if (faceMouseFlag && legitFactoryFlag) {
            clamped = GetCoordClamp(xOffset/7 + innerWidth / 2, -yOffset/7 + innerHeight / 2);
        } else {
            clamped = GetCoordClamp(dX + innerWidth / 2, dY + innerHeight / 2);
        }
