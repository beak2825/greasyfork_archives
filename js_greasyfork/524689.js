...
// @namespace    ShukerLee
// @version      0.0.1
// @description  sciencedirect+ieeexplore+acm跳转 SYSU
// @author       ShukerLee
...
// @downloadURL https://update.greasyfork.org/scripts/524689/%E5%87%BA%E7%89%88%E7%A4%BE2SYSU.user.js
// @updateURL https://update.greasyfork.org/scripts/524689/%E5%87%BA%E7%89%88%E7%A4%BE2SYSU.meta.js
// ==/UserScript==
    ...
    function addButton() {
		...
		else if(link==html_title[1])
		{
			...
			if (targetElement && !document.getElementById("another-button")) {
				...
				newButton.type = "button";
				newButton.innerHTML = "跳转SYSU";  // 按钮的文本内容
                // 设置按钮的颜色样式
                newButton.style.backgroundColor = "#007BFF";  // 设置背景颜色为蓝色
                newButton.style.color = "white";  // 设置文字颜色为白色
                newButton.style.border = "none";  // 移除按钮边框
				...
				newButton.addEventListener("click", function() {
					link = location.href;
					link = link.replace('ieeexplore.ieee.org','cas.sysu.edu.cn/cas/login?service=https%3A%2F%2Fidp.sysu.edu.cn%2Fidp%2FAuthn%2FExtCas%3Fconversation%3De1s1&entityId=https%3A%2F%2Fieeexplore.ieee.org%2Fshibboleth-sp');
                    GM_openInTab(link, { active: true });
				});
        ...
        else if(link==html_title[2])
        {
            ...
            if (doiElement && !document.getElementById("another-button")) {
                ...
                newButton.innerHTML = `
                <span class="button-link-text-container">
                    <span class="button-link-text">跳转SYSU</span>
                </span>
            `;
                // 设置按钮的颜色样式
                newButton.style.backgroundColor = "#007BFF";  // 设置背景颜色为蓝色
        ...
        {
            // 找到目标元素
            let targetElement = document.querySelector(".info-panel__right-content");
            ...
            if (targetElement && !document.getElementById("another-button")) {
                ...
                newButton.addEventListener("click", function() {
                    link = location.href;
                    link = link.replace('dl.acm.org','cas.sysu.edu.cn/cas/login?service=https%3A%2F%2Fidp.sysu.edu.cn%2Fidp%2FAuthn%2FExtCas%3Fconversation%3De1s1&entityId=https%3A%2F%2Fdl.acm.org%2Fshibboleth');
                    GM_openInTab(link, { active: true });
                });
