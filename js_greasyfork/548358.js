...
// @downloadURL https://update.greasyfork.org/scripts/548358/Bilibili%20%E5%B1%8F%E8%94%BD.user.js
// @updateURL https://update.greasyfork.org/scripts/548358/Bilibili%20%E5%B1%8F%E8%94%BD.meta.js
// ==/UserScript==
...
<div id="blockedMenuUi">
    <div id="menuTitle">Bilibili按标签、标题、时长、UP主屏蔽视频</div>
...
<div class="menuOptions">
            <div class="titleLabelLeft">
                <label title="正则以/?/表示"><input type="checkbox" v-model="menuUiSettings.blockedNameOrUid_Switch" />按UP名称或Uid屏蔽视频(?)</label>
            </div>

            <input type="text" placeholder="普通文本不区分大小写, 正则请用/?/" spellcheck="false"
    ...
    createApp({
        setup() {
            ...
            const addArrayButton = (tempInputValue, menuUiSettings, keyName) => {
                ...
                // 将目标数组的所有项转换为小写，用于不区分大小写的重复检查
                const existingItemsLower = menuUiSettings[keyName].map(item => item.toLowerCase());
                let addedCount = 0;
                ...
                if (keyName === "doubleBlockedTag_Array" && tempInputValue[keyName].trim()) {
                    ...
                    items.forEach((secondSplitItem) => {
                        ...
                        // 不区分大小写检查重复
                        if (!existingItemsLower.includes(formattedItem.toLowerCase())) {
                ...
                // 其他所有普通数组的处理
                else if (tempInputValue[keyName].trim()) {
                    const items = tempInputValue[keyName].split(",").map((item) => item.trim()).filter(item => item !== "");

                    items.forEach(item => {
                        // 不区分大小写检查重复
                        if (!existingItemsLower.includes(item.toLowerCase())) {
                            menuUiSettings[keyName].push(item);
                            addedCount++;
                        } else {
                            duplicateCount++;
                        }
                ...
                if (addedCount > 0 && duplicateCount > 0) {
                    showPromptText(`添加 ${addedCount} 项, ${duplicateCount} 项重复`);
                } else if (addedCount > 0) {
                    showPromptText(`成功添加 ${addedCount} 项`);
                } else if (duplicateCount > 0) {
                    showPromptText(`${duplicateCount} 项已存在, 未添加`);
                }
...
function handleBlockedTitle(videoBv) {
    ...
    if (blockedParameter.blockedTitle_UseRegular) {
        ...
        const blockedTitleHitItem = blockedParameter.blockedTitle_Array.find((blockedTitleItem) => {
            // 正则化屏蔽标题
            const blockedTitleRegEx = new RegExp(blockedTitleItem, 'i');
...
function handleBlockedNameOrUid(videoBv) {
    ...
    // .find() 会返回第一个匹配的规则项
    const blockedNameOrUidHitItem = blockedParameter.blockedNameOrUid_Array.find((blockedItem) => {
        // 1. UID 匹配 (精确匹配)
        // 确保视频有UID且规则项与之一致
        if (upUid && blockedItem == upUid) {
            return true;
        }
        ...
        if (upName) {
            // 检查规则项是否为 /.../ 格式的正则表达式
            if (blockedItem.startsWith('/') && blockedItem.endsWith('/') && blockedItem.length > 2) {
                const pattern = blockedItem.slice(1, -1);
                try {
                    // 创建不区分大小写的正则表达式
                    const regex = new RegExp(pattern, 'i');
                    // 直接返回测试结果 (true/false)
                    return regex.test(upName);
                } catch (e) {
                    consoleLogOutput(`无效的UP主屏蔽正则: ${blockedItem}`, e);
                    return false;
                }
            } else {
                // 普通文本匹配，不区分大小写
                // 直接返回比较结果 (true/false)
                return blockedItem.toLowerCase() === upName.toLowerCase();
            }
        }

        // 如果视频没有UP主名称，则无法进行名称匹配
        return false;
    });
    ...
    if (blockedNameOrUidHitItem) {
        // 标记为屏蔽目标并记录触发的规则
        markAsBlockedTarget(videoBv, "按UP主屏蔽", blockedNameOrUidHitItem);
    }
}
    ...
    if (blockedParameter.blockedVideoPartitions_UseRegular) {
        ...
        const blockedVideoPartitionsHitItem = blockedParameter.blockedVideoPartitions_Array.find(
            (blockedVideoPartitionsItem) => {
                // 正则化屏蔽视频分区
                const blockedVideoPartitionsRegEx = new RegExp(blockedVideoPartitionsItem, 'i');
...
function handleBlockedUpSigns(videoBv) {
    ...
    if (blockedParameter.blockedUpSigns_UseRegular) {
        ...
        const blockedUpSignsHitItem = blockedParameter.blockedUpSigns_Array.find((blockedUpSignsItem) => {
            // 正则化屏蔽UP主简介
            const blockedUpSignsRegEx = new RegExp(blockedUpSignsItem, 'i');
...
function handleBlockedTag(videoBv) {
    ...
    if (blockedParameter.blockedTag_UseRegular) {
        ...
        const blockedTagHitItem = blockedParameter.blockedTag_Array.find((blockedTagItem) => {
            // 正则化屏蔽视频标签
            const blockedTagRegEx = new RegExp(blockedTagItem, 'i');
...
function handleDoubleBlockedTag(videoBv) {
    ...
    if (blockedParameter.doubleBlockedTag_UseRegular) {
        ...
        const doubleBlockedTagHitItem = blockedParameter.doubleBlockedTag_Array.find((doubleBlockedTag) => {
            ...
            const doubleBlockedTagRegEx0 = new RegExp(doubleBlockedTagSplitArray[0], 'i');
            const doubleBlockedTagRegEx1 = new RegExp(doubleBlockedTagSplitArray[1], 'i');
...
function handleBlockedTopComment(videoBv) {
    ...
    if (blockedParameter.blockedTopComment_UseRegular) {
        ...
        const blockedTopCommentHitItem = blockedParameter.blockedTopComment_Array.find((blockedTopComment) => {
            // 正则化屏蔽置顶评论
            const blockedTitleRegEx = new RegExp(blockedTopComment, 'i');
...
function handleBlockedTrendingItemElements(trendingItem, blockedTrendingItem_Array, useRegex) {
    ...
    if (useRegex) {
        ...
        const blockedTrendingHitItem = blockedTrendingItem_Array.find((blockedTrendingItem) => {
            // 正则化屏蔽热搜项数组
            const blockedTrendingItemRegEx = new RegExp(blockedTrendingItem, 'i');
...
function blockedOrUnblocked(videoElement, videoBv, setTimeoutStatu = false) {
    ...
    function addHiddenOrOverlay(videoElement, videoBv, setTimeoutStatu) {
        // ==================== 代码修改开始 ====================
        // 你的需求：在 popular 页面，直接删除被屏蔽的元素
        if (window.location.href.startsWith("https://www.bilibili.com/v/popular/")) {
            videoElement.remove(); // 直接删除该视频卡片元素
            return; // 提前结束函数，不再执行后续的隐藏或叠加层逻辑
        }
        // ==================== 代码修改结束 ====================
...
document.addEventListener('keydown', function(event) {
    ...
    if (event.key === '~' || event.key === '`') {
        const menuUiElement = document.getElementById("blockedMenuUi");

        if (menuUiElement) {
            // 如果面板已存在，则关闭它
            menuUiElement.parentNode.removeChild(menuUiElement);
        } else {
            // 如果面板不存在，则创建并显示它
            blockedMenuUi();
        }
