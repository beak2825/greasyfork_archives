// ==UserScript==
// @name         知乎🔥2024 最新增强插件
// @name:zh-CN   知乎🔥2024 最新增强插件
// @name:zh-TW   知乎🔥2024 最新增強插件
// @version      2.2.15
// @author       cassss
// @description  2024 年最新知乎增强插件，基于老牌插件进一步修改，可以移除登录弹窗、屏蔽首页视频、默认收起回答、快捷收起回答/评论（左键两侧）、快捷回到顶部（右键两侧）、屏蔽用户、屏蔽关键词、移除高亮链接、屏蔽盐选内容/热榜杂项、净化搜索热门、净化标题消息、展开问题描述、显示问题作者、置顶显示时间、完整问题时间、区分问题文章、直达问题按钮、默认高清原图、默认站外直链
// @description:zh-TW  2024 年最新知乎增強插件，基於老牌插件進一步修改，可以移除登錄彈窗、屏蔽首頁視頻、默認收起回答、快捷收起回答/評論、快捷回到頂部、屏蔽用戶、屏蔽關鍵詞、移除高亮鏈接、屏蔽鹽選內容、淨化搜索熱門、淨化標題消息、置頂顯示時間、完整問題時間、區分問題文章、默認高清原圖、默認站外直鏈...
// @match        *://www.zhihu.com/*
...
// @run-at       document-end
// @namespace https://greasyfork.org/users/33514
// @downloadURL https://update.greasyfork.org/scripts/485653/%E7%9F%A5%E4%B9%8E%F0%9F%94%A52024%20%E6%9C%80%E6%96%B0%E5%A2%9E%E5%BC%BA%E6%8F%92%E4%BB%B6.user.js
// @updateURL https://update.greasyfork.org/scripts/485653/%E7%9F%A5%E4%B9%8E%F0%9F%94%A52024%20%E6%9C%80%E6%96%B0%E5%A2%9E%E5%BC%BA%E6%8F%92%E4%BB%B6.meta.js
// ==/UserScript==
// 感谢 X.I.U 大佬的插件，陪伴我走过很多个月，既然大佬没那么有空来更新，我就看看能不能站在巨人的肩膀上维护并开发更多功能
"use strict";
var menu_ALL = [
...
function registerMenuCommand() {
  if (menu_ID.length > menu_ALL.length) {
    // 如果菜单 ID 数组多于菜单数组，说明不是首次添加菜单，需要卸载所有脚本菜单
...
[data-theme="dark"] .zhihuE_SettingMain hr {border: 0.5px solid #2d333b;}</style>
  ...
  setTimeout(function () {
    ...
    // 点击周围空白处 = 点击关闭按钮
    document.querySelector(".zhihuE_SettingBackdrop_2").onclick = function (
      event
    ) {
    ...
    if (event.target == this) {
      ...
      if (rightButton) {
        ...
        if (
          ...
          commentCollapseButton.textContent.indexOf("收起评论") > -1
        )
        ...
        if (commentCollapseButton_ == false) {
          ...
          if (commentCollapseButton_1.length > 0) {
            for (let el of commentCollapseButton_1) {
              if (isElementInViewport(el)) {
                let parentElement =
                    findParentElement(el, "List-item") ||
                    findParentElement(el, "Card "),
                  commentCollapseButton = parentElement.querySelector(
...
function customBlockUsers() {
  ...
  let newBlockUsers = prompt(
    '编辑 [自定义屏蔽用户]\n（不同用户名之间使用 "|" 分隔，例如：用户 A|用户 B|用户 C）',
      ...
      document.querySelectorAll(className1).forEach(function (item1) {
        ...
        if (item) {
          for (const keyword of menu_value("menu_customBlockUsers")) {
            ...
            if (
              keyword != "" &&
              item.dataset.zop.indexOf('authorName":"' + keyword + '",') > -1
            ) {
              // 找到就删除该信息流
          ...
          if (target.className === className2) {
            ...
            if (item) {
              for (const keyword of menu_value("menu_customBlockUsers")) {
                ...
                if (
                  keyword != "" &&
                  item.dataset.zop.indexOf('authorName":"' + keyword + '",') >
                    -1
                ) {
                  // 找到就删除该信息流
                  console.log("已屏蔽：" + item.dataset.zop);
          ...
          if (
            ...
            if (item1) {
              menu_value("menu_customBlockUsers").forEach(function (item2) {
                // 遍历用户黑名单
                if (
                  item1.dataset.zop.indexOf('authorName":"' + item2 + '",') > -1
                ) {
                  // 找到就删除该回答
            ...
            .forEach(function (item) {
              ...
              if (item1) {
                menu_value("menu_customBlockUsers").forEach(function (item2) {
                  // 遍历用户黑名单
                  if (
                    item1.dataset.zop.indexOf('authorName":"' + item2 + '",') >
                    -1
                  ) {
                    // 找到就删除该回答
                    console.log("已屏蔽：" + item1.dataset.zop);
  ...
  (function ($2) {
    ...
    document.body.addEventListener(
      ...
      async (e) => {
        ...
        if (target.classList.contains("CPSCommonCard")) {
          e.stopPropagation();
          e.preventDefault();
          const goodsRecommendCard = findParentByClassName(
            target,
            "GoodsRecommendCard"
          );
          if (!goodsRecommendCard) return;
          const skuId = goodsRecommendCard.dataset.mcnSkuid;
          if (!skuId) return;
          const href = goodsRecommendCard.getAttribute("href");
          if (!href) return;
          window.open(href);
        }
      },
    ...
    async function trans(skuId) {
      return new Promise((resolve, reject) => {
        const jdURL = `https://28bakqq4zu.us.aircode.run/test?id=${skuId}`;
        $2.get(jdURL, (data) => {
          if (data.url) {
            resolve(data.url);
          } else {
    ...
    function handleGoodsRecommendCard(node) {
      var _a, _b;
      if (node instanceof HTMLElement) {
        const skuId = (_a = node.dataset) == null ? void 0 : _a.mcnSkuid;
        if (
          !skuId ||
          ((_b = node.dataset) == null ? void 0 : _b.mcnSource) !== "0"
        )
          return;
        if (/^\d+$/.test(skuId) === false) return;
        if (node.dataset["mcnResult"]) return;
        trans(skuId)
          .then((transformedHref) => {
            node.setAttribute("href", transformedHref);
            node.dataset["mcnResult"] = "1";
          })
          .catch(() => {
            node.dataset["mcnResult"] = "2";
          });
      }
    }
        ...
        mutationsList.forEach((mutation) => {
          if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
            ...
            if (
              ...
              addedNode.tagName === "A" &&
              addedNode.classList.contains("GoodsRecommendCard")
            ) {
              handleGoodsRecommendCard(addedNode);
            }
          }
        });
      }
      document
        .querySelectorAll("a.GoodsRecommendCard")
        .forEach((ele) => handleGoodsRecommendCard(ele));
  ...
  function blockKeywords_comment() {
    function filterComment(comment) {
      ...
      for (let i = 0; i < content.children.length; i++) {
        // 该条针对的是评论中的表情
        let emoticonValue = content.children[i].getAttribute(
          "data-zhihu-emoticon"
        ); // 确定是表情就将其添加到稍后遍历的数组中
        if (emoticonValue) {
      ...
      for (const mutation of mutationsList) {
        for (const target of mutation.addedNodes) {
          ...
          for (const node of target.querySelectorAll("*")) {
            if (node.className === "CommentItemV2-metaSibling")
              filterComment(node);
          }
        }
      }
...
function blockType(type) {
  ...
  if (type === "search") {
    ...
    ).textContent = `.VideoAnswerPlayer, .VideoAnswerPlayer video, .VideoAnswerPlayer-video, .VideoAnswerPlayer-iframe {display: none !important;}`;
    ...
    if (menu_value("menu_blockTypeVideo"))
      document.lastChild.appendChild(
        document.createElement("style")
      ).textContent = `.Card .ZVideoItem-video, .VideoAnswerPlayer video, nav.TopstoryTabs > a[aria-controls="Topstory-zvideo"] {display: none !important;}`;
      ...
      for (const target of mutation.addedNodes) {
        ...
        if (
          ...
          if (
            ...
            location.search.indexOf("type=content") > -1
          )
            target.hidden = true;
        } else {
  ...
  function blockType_(titleA) {
    ...
    if (location.pathname === "/search") {
      ...
      if (
        titleA.href.indexOf("/zvideo/") > -1 ||
        titleA.href.indexOf("video.zhihu.com") > -1
      ) {
        // 如果是视频
        if (menu_value("menu_blockTypeVideo"))
          findParentElement(titleA, "Card").remove();
      } else if (titleA.href.indexOf("zhuanlan.zhihu.com") > -1) {
        // 如果是文章
        if (menu_value("menu_blockTypeArticle"))
          findParentElement(titleA, "Card SearchResult-Card").hidden = true;
      ...
      // 问题页
      if (menu_value("menu_blockTypeVideo"))
        findParentElement(titleA, "List-item").hidden = true;
    } else {
      // 首页
      if (titleA.className == "ContentItem PinItem") {
        ...
        //           如果是问题（视频回答）
        if (
          findParentElement(titleA, "ContentItem AnswerItem").querySelector(
            ".VideoAnswerPlayer"
          )
        ) {
          if (menu_value("menu_blockTypeVideo")) {
            ...
            findParentElement(titleA, "ContentItem AnswerItem").remove();
          }
        }
...
function removeHighlightLink() {
    ...
    for (const mutation of mutationsList) {
      for (const target of mutation.addedNodes) {
        ...
        if (
          ...
          target.href.indexOf("https://www.zhihu.com/search?q=") > -1
        ) {
  ...
  document
    .querySelectorAll(
      'span > a[data-za-not-track-link][href^="https://www.zhihu.com/search?q="]'
    )
...
function addTypeTips() {
  ...
  let style = `font-weight: bold;font-size: 13px;padding: 1px 4px 0;border-radius: 2px;display: inline-block;vertical-align: top;margin: ${
    location.pathname === "/search" ? "2" : "4"
  }px 4px 0 0;`;
...
function addToQuestion() {
  ...
  if (location.pathname === "/search") {
    document.lastChild.appendChild(
      document.createElement("style")
    ).textContent = `a.zhihu_e_toQuestion {font-size: 13px !important;font-weight: normal !important;padding: 1px 6px 0 !important;border-radius: 2px !important;display: inline-block !important;vertical-align: top !important;height: 20.67px !important;line-height: 20.67px !important;margin-top: 2px !important;}`;
    addSetInterval_("h2.ContentItem-title a:not(.zhihu_e_tips)");
  } else {
    document.lastChild.appendChild(
      document.createElement("style")
    ).textContent = `a.zhihu_e_toQuestion {font-size: 13px !important;font-weight: normal !important;padding: 1px 6px 0 !important;border-radius: 2px !important;display: inline-block !important;vertical-align: top !important;margin-top: 4px !important;}`;
    document
  ...
  function addTypeTips_(titleA) {
    ...
    if (/answer\/\d+/.test(titleA.href)) {
      ...
      titleA.insertAdjacentHTML(
        "afterend",
        `<a class="zhihu_e_toQuestion VoteButton" href="${
          titleA.parentElement.querySelector('meta[itemprop="url"]').content
        }" target="_blank">直达问题</a>`
      );
    }
  }
...
function question_author() {
  ...
  let qJson = JSON.parse(document.querySelector("#js-initialData").textContent)
      .initialState.entities.questions[/\d+/.exec(location.pathname)[0]].author,
...
function topTime_(css, classs) {
  document.querySelectorAll(css).forEach(function (_this) {
    ...
    if (
      ...
      t.querySelector("span").textContent != null
    ) {
      // 完整显示时间
      topTime_allTime(t);
      // 发布时间置顶
...
function topTime_allTime(t) {
  if (
    t.textContent.indexOf("发布于") > -1 &&
    t.textContent.indexOf("编辑于") == -1
  ) {
    t.querySelector("span").textContent =
      t.querySelector("span").dataset.tooltip;
  ...
  document
    ...
    .forEach(function (_this) {
      _this.href = decodeURIComponent(
        _this.href.substring(_this.href.indexOf("link.zhihu.com/?target=") + 23)
      );
    });
}
...
function originalPic() {
  document
    .querySelectorAll("img[data-original]:not(.comment_sticker):not(.Avatar)")
    .forEach(function (one) {
      if (one.src != one.dataset.original) {
        one.src = one.dataset.original;
      }
    });
}
...
(function () {
  ...
  window.addEventListener("urlchange", function () {
    ...
    if (
      ...
      location.pathname.indexOf("waiting") === -1 &&
  ...
  function start() {
    ...
    if (
      ...
      if (
        ...
        location.href.indexOf("/top-answers") > -1
      ) {
        // 仅限 [讨论] [精华]
      ...
      location.href.indexOf("org") > -1
    ) {
      // 用户主页 //
      if (location.pathname.split("/").length === 3) addTypeTips();
      addToQuestion(); // 区分问题文章、直达问题按钮
      collapsedNowAnswer("main div"); //                                 收起当前回答 + 快捷返回顶部
      ...
      if (menu_value("menu_blockTypeVideo"))
        document.lastChild.appendChild(
          document.createElement("style")
        ).textContent = `.Card .ZVideoItem-video, nav.TopstoryTabs > a[aria-controls="Topstory-zvideo"] {display: none !important;}`;
