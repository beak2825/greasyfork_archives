// ==UserScript==
// @name        Travian Web Automator 1.0
// @namespace   http://greasyfork.org/
// @author      Komançero (by CrimsonKinq)
// @license     GPL version 3 or any later version; http://www.gnu.org/copyleft/gpl.html
// @description İşlemleri otomatiğe alın Zekeriya hocamı kızdırmayın.
// @contributionURL No
// @include     *://*.travian.*
// @include     *://*/*.travian.*
...
// @version     1.0.0
// @downloadURL https://update.greasyfork.org/scripts/388131/Travian%20Web%20Automator%2010.user.js
// @updateURL https://update.greasyfork.org/scripts/388131/Travian%20Web%20Automator%2010.meta.js
// ==/UserScript==
...
if (init) {
	...
	var nLangBuildings = ["", "Oduncu", "Tuğlacı", "Demir Madeni", "Tarla", "Kereste Fabrikası", "Brickyard", "Demir Dökümhanesi", "Tahıl Değirmeni", "Fırın", "Depo", "Tahıl ambarı", "<Hayır Bina> "," Smithy "," Turnuva Meydanı "," Ana Bina "," Rally Point "," Marketplace "," Elçilik "," Kışla "," Ahır "," Atölye "," Akademi "," Cranny " , "Belediye Binası", "Konut", "Saray", "Hazine", "Ticaret Bürosu", "Büyük Kışla", "Büyük Ahır", "Şehir Duvarı", "Dünya Duvarı", "Palisade", "Stonemason's Lodge "," Bira Fabrikası "," Tuzakçı "," Kahraman Konağı "," Büyük Depo "," Büyük Tahıl Ambarı "," Dünyanın Harikaları "," At İçme Teknesi "," Taş Duvar "," Makeshift Duvarı "," Komutanlığı Merkez "," Su İşleri "];
	var nLangTasks = ["Build", "Upgrade", "Attack", "Research", "Train", "Festival", "Bina Yıkımı", "Send Merchants", "Send Back/Withdraw"];
	var nLangStrings = ["Sonra inşa et", "Sonra yükselt", "Bina bulunamadı", "Sonra araştır", "İşlem daha sonraya ayarlandı", "İnşa işlemi başladı ", "<center>İŞLEM DURUMU!</center><br>Lütfen bekleyin, TWA İşlemi başlatıyor!<br>Aşama", "Tuzaklar", " yapı isteği gönderildi. Ancak, binanın bina olmadığı anlaşılıyor.", "denendi, ancak sunucu bizi yönlendirdi.", "İşlem sıraya eklendi.", "Yönlendiriliyor", "Şu anda bu görevi zamanlayamıyoruz.", "Hata", "İşlem Listesi", "Kaldır", "Sonra gönder", "Hiçbir asker seçilmedi.", "Your troops were sent to", "Birlikleriniz gönderildi", "Destek", "Saldırı", "Yağmalama", "Mancınık hedefleyecek", "rastgele", "at", "or after", "seconds", "minutes", "hours", "days", "Kaynaklar ve birlikler için casus", "Birlikler ve savunma için casus", "uzakta", "Hedef belirtilmediği için saldırı programlanamıyor.", "at site no.", "Sort by:", "type ", "time ", "target ", "options ", "village ", "İşlem Geçmişi", "Flush History", "Araştırma başlatılıyor ", " cannot be researched.", "Sayfa yüklenemedi", "Spy", "train later", "troops.", "... May have not happened!", "We started training ", " cannot be trained.", "Festival Düzenle", " but not today.", "We started to ", "Close", "Görev Ekleyici", "Düzenle ve Kapat", "Ekle ve Kapat", "Ekle", "Are you sure you want to [s1] [s2]?", "Daha Sonra Yık", "Bina Yıkılıyor", "Cannot demolish", "Geçersiz koordinatlar veya kaynak seçilmedi.", "Yerel Saat Kullanılıyor", "Server Saati Kullanılıyor", " denendi ancak bağlantıyı bulamadık.", " denendi ancak başarısız oldu. neden: ", "No Link", " teşebbüs edildi ancak bina bulunamadı.", "No Building", " was attempted but the server returned an error.", "Server:", "Confirmation Failed", "Sorry, I <b>may</b> have built the building in the wrong town.", "Misbuild:", "Sent Back/Withdrew troops.<br>Troops are going home to:", "Sent Back/Withdrew troops Failed (I think).<br>Troops were supposed to go home to: ", "Click to make this your Active Village." , "Click to see this Village Details screen.", "Zamanaşımı TWA Hata Halinde"];
	var nLangMenuOptions = ["TWA: ", "Use server time", "Use local time", "Set your tribe", "Task History", "Sıfırla", "\nHow many past tasks do we keep in history?\n(Type 0 to disable task history.) \nCurrently: ", " What is your tribe on this server?\nType 0 for Romans, 1 for Teutons, 2 for Gauls, 5 for Egyptians, 6 for Huns. Or a negative number to enable autodetect (ie: -1)\nCurrently: ", "Tüm işlemleri sıfırlamak istediğine emin misin?"];
...
function TTQ_showMenuCommand() {
	...
	tA.innerHTML = "<span id='ttqPanelSpan'><b>TWA</b>-T4.4 v" + sCurrentVersion + " - <span id='ttqLoad' style='font-weight: bold' >0</span> ms - <span id='ttqReloadTimer'>0</span>by Komançero</span>";
...
//-- определение координат, активного здания и создание SPAN с кликабельными ссылками на деревню.
    ...
    var sTitleBarLogo = "";
    var sTimerFormBackground = "https://image.yenisafak.com/resim/imagecrop/2015/10/13/resized_f55c2-b4f2e5b9putperestler_1328604510111.jpg";
...
function scheduleSendClub () {
	...
	var listName = $gc("listTitleText",list)[0].textContent.replace('|','&#124;').replace(',','&#44;').trim();
...
function getActiveVillage (el) {
	var reqVID = xpath('.//div[@id="sidebarBoxVillagelist"]//ul/li/a[@class="active"]',el,true);
	if ( reqVID ) {
...
function handleTimerForm(oForm, iAction, taskindex, villagedid) {
	...
	for (var i=0; i<iTaskTime.length; i++) {
		if ( iTaskTime[i] ) setTask(oForm.elements["timerTask"].value, iTaskTime[i], oForm.elements["timerTarget"].value, oForm.elements["timerOptions"].value, taskindex, villagedid, i==iTaskTime.length-1 ? true : false );
	}
...
/**************************************************************************
  ...
  ***************************************************************************/
function setTask(iTask, iWhen, target, options, taskindex, villagedid, refreshTask) {
	var iVillageId = typeof(villagedid) != 'undefined' ? villagedid : currentActiveVillage;
	if(bLocked) {
	...
	var newValue = iTask + ',' + iWhen + ',' + target.replace('|','&#124;').replace(',','&#44;') + ',' + options;
...
function promptLang() {
// writed by Serj_LV
	var aLangPrompt = "0 - otomatik algılamaya çalışır: ";
	var t=1;
	allLangs.sort();
...
function onLoad() {
	...
	TTQ_registerMenuCommand("Dil Seçimi", promptLang);
