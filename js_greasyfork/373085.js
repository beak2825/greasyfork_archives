...
// @date         01/07/2018
// @modified     08/10/2018
// @homepage     https://greasyfork.org/zh-CN/scripts/373085
// @description  MUD
// @author       fjcqv
// @match        http://game.wsmud.com/*
// @require      https://cdn.staticfile.org/jquery/3.3.1/jquery.min.js
// @require      https://cdn.staticfile.org/jqueryui/1.12.1/jquery-ui.js
// @require      https://cdn.staticfile.org/jquery-contextmenu/3.0.0-beta.2/jquery.contextMenu.min.js
// @grant        unsafeWindow
// @grant        GM_addStyle
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/373085/wsmud_plugins.user.js
// @updateURL https://update.greasyfork.org/scripts/373085/wsmud_plugins.meta.js
// ==/UserScript==
(function () {
    "use strict";
    //快捷键功能
...
//     var remove_listener = unsafeWindow.remove_listener;
       ...
       var binow = false;
//        var aliases = new Map();
//        aliases.set('biguan', 'jh fam 0 start;go west;go west;go north;go enter;go west;xiulian');
    var blackpfm = ['sword.wu','sword.poqi','parry.pojian','parry.wu','dodge.power','force.power','force.wang','unarmed.duo','sword.duo',"force.duo"];
    var wuxing = "enable force shenzhaojing;enable parry yihuajiemu;enable unarmed canhezhi;enable dodge tiannanbu;enable sword quanzhenjianfa;";
    var zhandou = "enable force jiuyinshengong;enable parry shedaoqigong;enable unarmed sanyinzhua;enable dodge taxuexunmei2;enable sword xuantiejianfa;";
    //前置检查
    if (send_cmd == undefined) {

        document.getElementsByClassName("signinfo")[0].innerHTML += "<br><HIY>请安装前置</HIY><a target='_blank'  href='https://greasyfork.org/zh-CN/scripts/373086'><HIY>https://greasyfork.org/zh-CN/scripts/371517</HIY></a>"
        console.log("请安装前置https://greasyfork.org/zh-CN/scripts/373086");
        return;
    }
    var obj = document.getElementsByClassName("room_items");
    obj[0].style = "max-height: 40rem; overflow-y: auto;" //zyb
...
//             this.add(89, function () { WG.zdwk(); });
            ...
            this.add(58, function () { KEY.combat_commands(9); });
...
//             this.add(54 + 1024, function () { KEY.room_commands(5); });
    ...
    const C = {
        ...
        path: {
            ...
            "扬州城-帮派": "jh fam 0 start;go south;go south;go east;go east;go east",
            ...
            "峨眉派-走廊2": "jh fam 4 start;go west;go south;go west",
            ...
            "逍遥派-地下石室2": "jh fam 5 start;go down;go down",
            "丐帮-树洞内部": "jh fam 6 start",
            "丐帮-树洞下": "jh fam 6 start;go down",
            "丐帮-暗道": "jh fam 6 start;go down;go east",
            "丐帮-暗道2": "jh fam 6 start;go down;go east;go east;go east;go east",
            ...
            add_listener(["login", "room", "items", "itemadd", "itemremove", "sc", "text", "state", "msg", "perform", "dispfm", "combat"], function (data) {
                if (data.type == "login") {
                    ...
                    if (G.in_fight) {
                        ...
                        //修正自动战斗
                        if (data.msg[0] == '【' && data.msg.indexOf("只能在战斗中使用") > -1) { test({ data: '{type:"combat",end:1}' }); }
                    }
                }
                    ...
                    //抢首席
                    if (S.auto_shouxi && data.ch == 'sys'
                        && /^排行榜奖励发放完毕，进入社交系统消息中收取。$/.test(data.content)) {
                        send_cmd('task sm mshw4463c70;task sm mshw4463c70 give f4zm2c34dd6');
                        messageAppend('抢首席啦！');
                        //send_cmd('stopstate');
                        //WG.sm();
                        //WG.xiyan();
                    ...
                    if (S.auto_boss && G.can_auto && data.ch == 'rumor') {
                        ...
                        if ( (r[1].indexOf("枯荣") > -1)||(r[1].indexOf("童姥") > -1) ) {
                              messageAppend(r[1] + ' at ' + r[2] + '！')
                              if (r) {
                                G.stat_boss_find++;
                                WG.boss(r[1], r[2]);
                            }
                        } else {
                            messageAppend(r[1] + ' at ' + r[2] + '，不是枯荣我不打！')
...
.progress {width: 5em;}`;
                ...
                $(".eq1").on("click", function () { if (G.eq1) { send_cmd(zhandou+G.eq1); } else { messageAppend("未设置"); } });
                $(".eq2").on("click", function () { if (G.eq2) { send_cmd(wuxing+G.eq2); } else { messageAppend("未设置"); } });
            ...
            if (S.item_hp) {
                ...
                if (id) {
                    ...
                    if (v) {
                        ...
                        let html = "";
                        if (v.hp != undefined) html += "<hij>" + v.hp + "(" + Math.floor(v.hp / v.max_hp * 100) + "%)</hij>";
                        s.html(html);
                    }
                    else {
                ...
                if (sm_npc_id) {
                    ...
                    WG.todo(function () {
                        ...
                        if (id) {
                            ...
                            messageAppend("<hio>师门任务</hio>未发现师门NPC" + sm_npc); WG.sm("stop", callback);
                        }
                    });
                }
            };
                ...
                switch (state) {
                    case 0://任务判断
                        ...
                        if (data.type == 'dialog' && data.dialog == 'pack') {
                            //自动服用培元，养精
                            if ((/培元丹1|养精丹/).test(data.name)) send_cmd("use " + data.id);
                        }
                        break;
                    case 1: //背包搜索
                        if (data.type == 'dialog' && data.dialog == 'pack') {
        ...
        xiyan: function () {
            ...
            h = add_listener(['items', 'cmds', 'text'], function (data) {
                if (data.type == 'items') {
                    ...
                    let tmp = /^店小二拦住你说道：怎么又是你，每次都跑这么快，等下再进去。$/.test(data.msg);
                    if (tmp || /^店小二拦住你说道：这位(.+)，不好意思，婚宴宾客已经太多了。$/.test(data.msg)) {
                        messageAppend("<hio>自动喜宴</hio><MAG>失败</MAG>" + tmp ? "今天已经领过了吧！" : "脚本党太多！");
                        remove_listener(h);
                        if (t) clearTimeout(t);
                        WG.guaji();
                    }
        ...
        mpz: function (mpz_party, bpz) {
            if (mpz_party == "stop") {
                if (G.mpz_listener) {
                    remove_listener(G.mpz_listener);
                    messageAppend("<hio>门派战</hio><MAG>结束</MAG>");
                }
                setTimeout(() => {
                    if (bpz) send_cmd("pty 风紧跑路，我去挂机了");
                    WG.guaji();
                }, 3000);
                return;
            }
            ...
            let state = 0;
            let path = C.mpz_path[mpz_party].split(";");
            let path_index = 0;
            let kill_target = [];
            let kill_now = [];
            let mpz_wait_timer = undefined;
            G.mpz_listener = add_listener(['items', 'sc', 'itemremove', 'die', "dispfm", "text", "status"], function (data) {
                if (state == 0 && data.type == 'items') {
                    kill_target = [];
                    kill_now = [];
                    for (var i = 0; i < data.items.length; i++) {
                        if (data.items[i].max_hp == 2000000 && data.items[i].p == undefined) {
                            //发现紫
                            kill_target.push(data.items[i].id);
                        }
                    }
                    if (kill_target.length > 0) {
                        messageAppend("<hio>门派战</hio>发现紫色目标");
                        if (bpz) send_cmd("pty 我在" + G.room_name + "等你们5分钟，你们先手，我就上。");
                        $(".eq1").click();
                        send_cmd("liaoshang");
                        state = 1;
                        mpz_wait_timer = setTimeout(() => {
                            messageAppend("<hio>门派战</hio>5分钟没人打，超时返回挂机");
                            WG.mpz("stop", bpz);
                        }, 5 * 60 * 1000);
                    } else {
                        if (path_index < path.length) {
                            send_cmd(path[path_index++]);
                        }
                        else {
                            WG.mpz("stop", bpz);
                        }
                ...
                else if (state == 1 && kill_target.length && data.type == 'sc') {
                    for (let i = 0; i < kill_target.length; i++) {
                        if (kill_target[i] == data.id) {
                            send_cmd("stopstate;kill " + data.id);
                            kill_target.splice(i, 1);
                            kill_now.push(data.id);
                            messageAppend("<hio>门派战</hio>叫杀目标，剩余目标" + kill_target.length);
                        }
                        if (mpz_wait_timer) {
                            clearTimeout(mpz_wait_timer);
                            mpz_wait_timer = undefined;
                            if (kill_target.length) {
                                mpz_wait_timer = setTimeout(() => {
                                    messageAppend("<hio>门派战</hio>未全部击杀超时");
                                    WG.mpz("stop", bpz);
                                }, 5 * 60 * 1000);
                            }
                        }
                ...
                else if (state == 1 && kill_now.length && data.type == "itemremove") {
                    for (let i = 0; i < kill_now.length; i++) {
                        if (kill_now[i] == data.id) {
                            kill_now.splice(i, 1);
                        }
                    }
                    //击杀完成
                    if (kill_now.length == 0) {
                        if (G.in_fight) {
                            //修正战斗不结束
                        ...
                        if (path_index < path.length) {
                            ...
                            messageAppend("<hio>门派战</hio>叫杀完成，前往下一地点");
                        }
                        else {
                            WG.mpz("stop");
                        }
                ...
                else if (state == 1 && kill_now.length > 1 && data.type == "dispfm") {
                    //输出平均
                    let min, max, min_id;
                    min_id = kill_now[0];
                    min = max = G.items.get(kill_now[0]).damage;
                    for (let i = 1; i < kill_now.length; i++) {
                        let tmp = G.items.get(kill_now[i]).damage;
                        if (tmp < min) {
                            min = tmp;
                            min_id = kill_now[i];
                        }
                    ...
                    //dps差距过大，转换目标
                    if (max > (min * 2)) {
                        send_cmd("kill " + min_id);
                    }

                }
                else if (state < 2 && data.type == 'die') {
                    messageAppend("<hio>门派战</hio>击杀失败，复活挖矿");
                    send_cmd('relive');
                    setTimeout(function () {
                        send_cmd('liaoshang');
                    }, 500);
                    state = 2;
                } else if (state == 2 && data.type == 'text' && data.msg == '你内力不够，无法治疗自身伤势。') {
                    setTimeout(function () {
                        send_cmd('liaoshang');
                    }, 3000);
                } else if (state == 2 && data.type == 'status' && data.id == G.id && data.action == 'remove' && data.sid == 'xuruo') {
                    WG.mpz("stop", bpz);
                    state = 0;
                }
            });
            ...
            $(".eq1").click();
            send_cmd(path[path_index++]);
            messageAppend("<hio>门派战</hio><MAG>开始</MAG>");
            if (bpz) send_cmd("pty 大家走起");
        },
...
//                         send_cmd('eq f8063321281');
                    ...
                    var r = data.msg.match(/(.+)对著(.+)喝道：「.+！今日不是你死就是我活！」/);//ztb
                    if (r && r[2] == boss_name) {
                        setTimeout(function () {
                    ...
                    messageAppend("<hio>世界BOSS</hio>击杀失败，复活挖矿");
                    send_cmd('relive');
                    setTimeout(function () {
                        send_cmd('liaoshang');
                    }, 500);
...
//                messageAppend(v.name+" "+v.title);
                 if ((v.title.indexOf("首席") > -1) || (v.title.indexOf("大师兄") > -1) || (v.title.indexOf("大师姐") > -1)|| (v.title.indexOf("长老") > -1)|| (v.title.indexOf("三十七") > -1)|| (v.title.indexOf("六袋弟子") > -1)) continue;
                if (v.p) {
                    if (S.kill_player)
                        send_cmd("kill " + k);
                } else {
                    send_cmd("kill " + k);
//                     messageAppend(v.title+" "+v.name);
                }
            ...
            G.xl_listener = add_listener(['msg', 'text'], function (data) {
                if (data.type == 'text') {
                    if (data.msg.match(/^也许是缺乏实战经验，你觉得你的.+已经到了瓶颈了。$/)
                        ...
                        || data.msg.match(/^你的.+练习完毕。$/)
                        || data.msg.match(/^你的.+已经练习完成了。$/)
                ...
                } else if (data.type == 'msg' && data.ch == 'sys' && G.map == "home" && !G.xiulian_now && !binow) {
        ...
        auto_fight: function () {
            ...
            if (G.fight_listener) {
                ...
                remove_listener(G.fight_listener);
                G.fight_listener = undefined;
                WG.recover(0);
            }
            G.fight_listener = add_listener(["sc", "combat"], function (data) {
                if (data.type == "combat" && data.end) {
                    WG.recover(1, 1, 0, function () { });
                }
                else if (data.type == "sc" && data.id == id) {
                    let item = G.items.get(id);
                    if (item.hp >= item.max_hp) {
                        WG.recover(0, 1, 0, function () { });
                        send_cmd("stopstate;fight " + id);
                    }
                }
        ...
        auto_xy: function () {
            if (G.xy_listener) {
                messageAppend("<hio>自动襄阳</hio>结束");
                remove_listener(G.xy_listener);
                G.xy_listener = undefined;
                return;
            }
            let kill_targets = ["万夫长","千夫长", "百夫长", "十夫长", "蒙古兵"];
            let kill_targets1 = ["洪七公", "灭绝", "张三丰", "逍遥子", "玄难"];
            let has_stopstate = false;
            G.xy_listener = add_listener(["items", "itemadd", "combat", "die"], function (data) {
                if (data.type == "items") {
                    for (let [k, v] of G.items) {
                        if (v.p) continue;
                        if (WG.inArray(v.title, kill_targets) || WG.inArray(v.name, kill_targets)) {
                            send_cmd("kill " + k);
                        }
                    }
                }
                else if (data.type == "itemadd") {
                    let v = G.items.get(data.id);
                    if (WG.inArray(v.title, kill_targets) || WG.inArray(v.name, kill_targets)) {
                        if (has_stopstate == false) {
                            has_stopstate = true;
                            send_cmd("stopstate");
                            setTimeout(() => {
                                has_stopstate = false;
                    ...
                    if (WG.inArray(v.title, kill_targets1) || WG.inArray(v.name, kill_targets1)) {
//                         if (has_stopstate == false) {
//                             has_stopstate = true;
//                             send_cmd("stopstate");
//                             setTimeout(() => {
...
//                         send_cmd("kill " + data.id);
                    ...
                    //WG.recover(1, 0.7, 0, function () { });
                }
                else if (data.type == "die") {
                    messageAppend("<hio>自动襄阳</hio>太水了，结束");
                    remove_listener(G.xy_listener);
                    G.xy_listener = undefined;
                }
            });
            messageAppend("<hio>自动襄阳</hio>开始");
        },
        zdwk: function (v) {
            if (v == "remove") {
                if (G.wk_listener) {
...
//         },
            send_cmd("stopstate");
            if (biguan) WG.go("住房-练功房");
            send_cmd(biguan ? 'xiulian' : 'wakuang');
        },
        auto_check: function () {
            ...
            G.wd_linstener = add_listener(["items", "text",], (data) => {
                if (data.type == "items") {
                    ...
                    if (lv > lv_normal) {
                        ...
                        WG.recover(1, 0.7, lv > lv_try, () => { $(".eq10").click(); send_cmd('go up'); });
                    }
                    else {
                        setTimeout(function () {
                            send_cmd('go up');
                    ...
                    WG.go("扬州城-武庙");//zyb
                    WG.wudao("stop");
                    WG.recover(1, 0.7, 1, () => {WG.wudao();});//zyb
                }
            });
            if (G.room == "ta") {
...
//         },
        cmd_echo_button: function () {
            if (G.tangshi) {
                G.tangshi = false;
                send_cmd('enable sword xuantiejianfa');
            }
            else {
                G.tangshi = true;
                send_cmd('enable sword tangshijianfa');
            }
//             cmd_echo(G.cmd_echo);
        },
...
`;
        ...
        html: function () {
            ...
            var setting_html = "<h3 class='WG_setting'>插件设置</h3>" +
                ...
                this.html_input("eq3", "自定义装备3（练习）", `placeholder="标签:代码1,代码2……"`) +
            ...
            items: {
                ...
                "自动": {
                    ...
                    "items": {
                        ...
                        "自动比试": { name: "自动比试", visible: function (key, opt) { return G.fight_listener == undefined; }, callback: function (key, opt) { WG.auto_fight(); }, },
                        "关闭比试": { name: "关闭比试", visible: function (key, opt) { return G.fight_listener != undefined; }, callback: function (key, opt) { WG.auto_fight(); }, },
                        "自动襄阳": { name: "自动襄阳", visible: function (key, opt) { return G.xy_listener == undefined; }, callback: function (key, opt) { WG.auto_xy(); }, },
                        "关闭襄阳": { name: "关闭襄阳", visible: function (key, opt) { return G.xy_listener != undefined; }, callback: function (key, opt) { WG.auto_xy(); }, },
                    },
                },
                "快捷传送": {
                    name: "常用地点",
                    "items": {
                        ...
                        "kj1": { name: "当铺", callback: function (key, opt) { WG.go("扬州城-当铺"); }, },
                        "kj2": { name: "仓库", callback: function (key, opt) { WG.go("扬州城-钱庄"); send_cmd("store") }, },
                        "kj3": { name: "武庙", callback: function (key, opt) { WG.go("扬州城-武庙"); send_cmd("liaoshang") }, },
                        "kj4": { name: "镖局正厅", callback: function (key, opt) { WG.go("扬州城-镖局正厅");var id = "k7p7425321f";send_cmd("biao "+id+";task yunbiao "+id+" begin") }, },
                        "kj5": { name: "帮派", callback: function (key, opt) { WG.go("扬州城-帮派"); }, },
                        "kj6": { name: "衙门", callback: function (key, opt) { WG.go("扬州城-衙门正厅"); }, },
                        "kj7": { name: "擂台", callback: function (key, opt) { WG.go("扬州城-擂台"); }, },
                        "kj8": { name: "杂货铺", callback: function (key, opt) { WG.go("扬州城-杂货铺"); }, },
                        "kj9": { name: "打铁铺", callback: function (key, opt) { WG.go("扬州城-打铁铺"); }, },
                        "kj10": { name: "药铺", callback: function (key, opt) { WG.go("扬州城-药铺"); }, },
                    },
                },
                "门派传送": {
                    name: "门派传送",
                    "items": {
                        ...
                        "mp7": { name: "杀手楼", callback: function (key, opt) { WG.go("杀手楼-大门"); }, },
                        "mp8": { name: "襄阳", callback: function (key, opt) { WG.go("襄阳城-广场"); }, },
                        "mp9": { name: "武道", callback: function (key, opt) { WG.go("武道塔"); }, },
                    },
                },
                "门派野战": {
                    name: "门派野战",
                    "items": {
                        "yz1": { name: "武当boss", callback: function (key, opt) { WG.go("武当派-后山小院"); }, },
                        "yz2": { name: "少林boss", callback: function (key, opt) { WG.go("少林派-方丈楼"); }, },
                        "yz3": { name: "华山boss", callback: function (key, opt) { WG.go("华山派-客厅"); }, },
                        "yz4": { name: "峨眉boss", callback: function (key, opt) { WG.go("峨眉派-清修洞"); }, },
                        "yz5": { name: "逍遥boss", callback: function (key, opt) { WG.go("逍遥派-地下石室2"); }, },
                        "yz6": { name: "丐帮boss", callback: function (key, opt) { WG.go("丐帮-林间小屋"); }, },
                        "yz7": { name: "武当紫", callback: function (key, opt) { WG.go("武当派-林间小径"); }, },
                        "yz8": { name: "少林紫", callback: function (key, opt) { WG.go("少林派-后殿"); }, },
                        "yz9": { name: "华山紫", callback: function (key, opt) { WG.go("华山派-玉女峰"); }, },
                        "yz10": { name: "峨眉紫", callback: function (key, opt) { WG.go("峨眉派-走廊2"); }, },
                        "yz11": { name: "逍遥紫", callback: function (key, opt) { WG.go("逍遥派-地下石室"); }, },
                        "yz12": { name: "丐帮紫", callback: function (key, opt) { WG.go("丐帮-暗道2"); }, },


                    },
                },
                "设置": { name: "设置", callback: function (key, opt) { $("span[command='setting']").click(); $(".footer-item[for='custom']").click(); }, },
                "挖矿指南": { name: "挖矿指南", callback: function (key, opt) { send_cmd("chat wkzn") }, },
                "门派战": { name: "门派战", callback: function (key, opt) { send_cmd("chat mpz") }, },
