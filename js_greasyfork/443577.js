...
// @namespace    wwhm
// @version      1.0.1
// @description  支持超星视频倍速、文档、答题(beta)、掉线自动登录
// @author       wwhm
// @run-at       document-end
// @match        *://*.chaoxing.com/*
// @match        *://*.edu.cn/*
...
//安全网址请填写在上方空白区域
// @downloadURL https://update.greasyfork.org/scripts/443577/%E5%AD%A6%E4%B9%A0%E9%80%9A%E7%9C%8B%E8%AF%BE%E5%8A%A9%E6%89%8B%20By%E6%9C%AA%E9%97%BB%E8%8A%B1%E5%90%8D.user.js
// @updateURL https://update.greasyfork.org/scripts/443577/%E5%AD%A6%E4%B9%A0%E9%80%9A%E7%9C%8B%E8%AF%BE%E5%8A%A9%E6%89%8B%20By%E6%9C%AA%E9%97%BB%E8%8A%B1%E5%90%8D.meta.js
// ==/UserScript==
(()=>{
var maxRate = 16 , //限制视频最高倍速为1.5倍，防止倍速过高导致的出现异常记录/清除进度，建议根据自己胆量修改。
    token = '' ,
...
`;
    var logs = {
        ...
        if(_d.getElementById('unrivalRate')&&_d.getElementById('updateRateButton')&&_d.getElementById('reviewModeButton')&&_d.getElementById('autoDoWorkButton')&&_d.getElementById('autoSubmitButton')&&_d.getElementById('autoSaveButton')){
            ...
            _d.addEventListener('visibilitychange', function () {
                let isH = _d.hidden;
                if (!isH) {
                    logs.addLog('小陈say:请保持此页面在最前端，不要切屏或最小化，否则脚本无法正常运行','red');
                }
            });
            ...
            _d.getElementById('updateRateButton').onclick = function(){
                ...
                if(urate>maxRate){
                    _d.getElementById('unrivalRate').value = rate;
                    logs.addLog('小陈say:已超过脚本限制最高倍速，修改失败，<b>倍速大于1可能会面临清除进度/全校通报风险</b>，如有特殊需求请修改脚本代码内限制参数','red');
                    return;
                }
                if(urate>0){
                    GM_setValue('unrivalrate',urate);
                    logs.addLog('小陈say:视频倍速已更新为'+urate+'倍，将在3秒内生效','green');
                    rate = urate;
                }
            }
            _d.getElementById('reviewModeButton').onclick=function(){
                ...
                if(reviewButton.getAttribute('class')=='btn btn-default'){
                    ...
                    logs.addLog('小陈say:复习模式已开启，遇到已完成的视频任务不会跳过','green');
                    ...
                    logs.addLog('小陈say:复习模式已关闭，遇到已完成的视频任务会自动跳过','green');
            ...
            _d.getElementById('autoSubmitButton').onclick=function(){
                ...
                if(autoSubmitButton.getAttribute('class')=='btn btn-default'){
                    ...
                    logs.addLog('小陈say:符合提交标准的章节测试将会自动提交','green');
                    ...
                    logs.addLog('小陈say:章节测试将不会自动提交，等待用户操作','green');
    ...
    try{
        ...
        if(jumpType!=2){
            _w.top.jump = true;
            logs.addLog('小陈say:此页无任务，5秒后自动下一章','green');
        }else{
            logs.addLog('小陈say:此页无任务，用户设置为不跳转，脚本已结束运行，如需自动跳转，请编辑脚本代码参数','green');
        }
        return;
    }
    ...
    var video_getReady=(item)=>{
            ...
            GM_xmlhttpRequest({
                ...
                onerror:function(err){
                    ...
                    if(err.error.indexOf('@connect list')>=0){
                        logs.addLog('小陈say:请添加安全网址，将 【 //@connect      '+_h+' 】方括号里的内容(不包括方括号)添加到脚本代码内指定位置，否则脚本无法正常运行，如图所示：','red');
                        logs.addLog('小陈say:<img src="https://pan-yz.chaoxing.com/thumbnail/0,0,0/609a8b79cbd6a91d10c207cf2b5f368d">');
                        stop = true;
                    }else{
                        logs.addLog('小陈say:获取任务详情失败','red');
                        logs.addLog('小陈say:错误原因：'+err.error,'red');
                    }
                }
            });
        },
        doVideo = (item)=>{
            ...
            logs.addLog('小陈say:开始刷视频：'+item['name']+'，倍速：'+String(rate)+'倍');
            logs.addLog('小陈say:视频观看信息每60秒上报一次，请耐心等待，脚本在正常运行，请不要在60秒内卸载脚本然后去评论脚本不能用，奶奶滴！','green');
            if(item['doublespeed']==0&&rate!=1&&_w.top.unrivalReviewMode=='0'){
                logs.addLog('小陈say:倍速播放此视频有99%几率导致“老师发现”、“清除进度”！！！','red');
                logs.addLog('小陈say:倍速播放此视频有99%几率导致“老师发现”、“清除进度”！！！','red');
                logs.addLog('小陈say:倍速播放此视频有99%几率导致“老师发现”、“清除进度”！！！','red');
            }
            let playTime = 0,
                ...
                loop = setInterval(function(){
                    ...
                    if(times==0||times%60==0||playTime>=item['duration']){
                        ...
                        encUrl = hostList[host]+'chaoXing/v1/getEnc-as.php?classid='+classId+'&playtime='+playTime+'&duration='+item['duration']+'&objectid='+item['objectId']+'&jobid='+item['jobid']+'&uid='+UID;
                        busyThread +=1;
                        ...
                        GM_xmlhttpRequest({
                            ...
                            onload: function(res) {
                                ...
                                if(enc.length!=32){
                                    logs.addLog('小陈say:获取视频enc错误，五秒后重试','red');
                                    logs.addLog('小陈say:如果反复失败，请尝试在脚本代码内更改服务器线路','red');
                                    logs.addLog(enc,'red');
                                    times=-5;
                                    return;
                                }
                                    ...
                                    onload: function(res) {
                                        try{
                                            ...
                                            if(res.responseText.indexOf('验证码')>=0){
                                                logs.addLog('小陈say:已被超星风控，请<a href="'+reportsUrl+'" target="_blank">点我处理</a>，60秒后自动刷新页面','red');
                                                missionList['m'+item['jobid']]['running']=false;
                                                clearInterval(loop);
                                                stop = true;
                                                setTimeout(function(){
                                            ...
                                            if(rt=='0.9'){
                                                if(first){
                                                    logs.addLog('小陈say:超星返回错误信息，尝试更换参数','orange');
                                                    rt='1';
                                                    times = -3;
                                                }else{
                                                    logs.addLog('小陈say:超星返回错误信息，十秒后重试(1)','red');
                                                    times = -10;
                                                }
                                                return;
                                            }else{
                                                ...
                                                logs.addLog('小陈say:超星返回错误信息，十秒后重试(2)','red');
                                                times = -10;
                            ...
                            onerror:function(err){
                                console.log(err);
                                logs.addLog('小陈say:获取视频enc失败，请检查脚本插件是否有完整的访问权限，具体请见脚本下载页','red');
                                missionList['m'+item['jobid']]['running']=false;
                                clearInterval(loop);
                            }
                        });
        ...
        doDocument=(item)=>{
            missionList['m'+item['jobid']]['running']=true;
            logs.addLog('小陈say:开始刷文档：'+item['name']);
            setTimeout(function(){
                busyThread += 1;
                GM_xmlhttpRequest({
                    method: "get",
                    ...
                    onload: function(res) {
                        try{
                            ...
                            let ispass = JSON.parse(res.responseText);
                            if(ispass['status']){
                                logs.addLog('小陈say:文档任务已完成','green');
                            }else{
                                logs.addLog('小陈say:文档已阅读完成，但任务点未完成','red');
                            }

                        }catch(err){
                            console.log(err);
                            console.log(res.responseText);
                            logs.addLog('小陈say:解析文档内容失败','red');
                        }
                        missionList['m'+item['jobid']]['running']=false;
                        missionList['m'+item['jobid']]['done']=true;
                    },
        ...
        doWork = (item)=>{
            missionList['m'+item['jobid']]['running']=true;
            logs.addLog('小陈say:开始刷作业：'+item['name']);
    ...
    var loopjob= ()=>{
        if(_w.top.unrivalScriptList.length>1){
            logs.addLog('小陈say:您同时开启了多个刷课脚本，会挂科，会挂科，会挂科，会挂科，会挂科，会挂科，会挂科，会挂科','red');
        }
        if(stop){
            return;
        }
        ...
        if(busyThread <=0){
            if(jumpType!=2){
                _w.top.jump = true;
                logs.addLog('小陈say:所有任务已完成，5秒后自动下一章','green');
            }else{
                logs.addLog('小陈say:所有任务已完成，用户设置为不跳转，脚本已结束运行，如需自动跳转，请编辑脚本代码参数','green');
            }
    ...
    var readyCheck = ()=>{
        setTimeout(function(){
            try{
                logs.addLog('小陈say:将页面最小化或切屏可能会导致脚本异常终止，请保持页面始终在最前端显示','orange');
                if(_w.top.unrivalReviewMode=='1'){
                    logs.addLog('小陈say:复习模式已开启，遇到已完成的视频任务不会跳过','green');
