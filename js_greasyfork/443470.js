...
// @version      1.0.2
// @description  通用视频同步工具
// @license      MIT
// @author       lisnote
// @match        *://*/*
// @icon         https://avatars.githubusercontent.com/u/53322810?v=4
// @downloadURL https://update.greasyfork.org/scripts/443472/Sync%20video.user.js
// @updateURL https://update.greasyfork.org/scripts/443472/Sync%20video.meta.js
// ==/UserScript==
/* global io */
/*!
 * Socket.IO v4.4.1
...
let observer = 0;
let video = null;
let socket = io('wss://overseas.lisnote.com:10000', {
...
observer = setInterval(function () {
  ...
  if (video != null) {
    videoEvent();
    socket.emit('video init', location.href);
    // Time Sync
    function videoSync() {
      socket.emit('video sync', location.href, currentTime);
    }
...
function socketStart() {
  // socket events
  socket.on('video update', function (remoteHref, time) {
    if (location.href == remoteHref && video) {
      video.currentTime = time;
    }
  });
  socket.on('video pause', function (remoteHref, time) {
    if (location.href == remoteHref) {
      video.pause();
    }
  });
  socket.on('video play', function (remoteHref, time) {
    if (location.href == remoteHref) {
      video.play();
    }
  });
...
function videoEvent() {
  video.addEventListener('timeupdate', (e) => {
    if (Math.abs(video.currentTime - currentTime) > 3) {
      socket.emit('video update', location.href, video.currentTime);
    }
    currentTime = video.currentTime;
  });
  video.addEventListener('pause', () => {
    socket.emit('video pause', location.href, video.currentTime);
  });
  video.addEventListener('play', (e) => {
    socket.emit('video play', location.href, video.currentTime);
  });
}
