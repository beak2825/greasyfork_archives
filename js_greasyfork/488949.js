...
// @namespace      nexterot
// @version        2.1.0
// @homepage       https://greasyfork.org/ru/scripts/488949-transfersearch
// @author         Alex_2oo8, Рианти, CheckT, emptimd, Something begins, nexterot
...
// @license        none
// @downloadURL https://update.greasyfork.org/scripts/488949/TransferSearch.user.js
// @updateURL https://update.greasyfork.org/scripts/488949/TransferSearch.meta.js
// ==/UserScript==

(function(){

    const last_page = 999999;
    var link = '';
    var title = '';
    ...
    if (window.location.href.includes('sklad_log'))
    {
        link = 'sklad_log';
        title = 'склада';
        protocolType = ProtocolType.AMBAR;
    }
    else if (window.location.href.includes('pl_transfers'))
    {
        link = 'pl_transfers';
        title = 'игрока';
        protocolType = ProtocolType.PLAYER;
    }
    else if (window.location.href.includes('clan_log'))
    {
        link = 'clan_log';
        title = 'клана';
        protocolType = ProtocolType.CLAN;
    }
    //console.log(link);
  ...
  function search( id, elem, type ) {
    ...
    request(url+link+'.php?page=' + last_page + '&id=' + id, search_1_last_page, type, first_page);
  }
  ...
  function search_1_last_page(objXMLHttpReq, type, first_page){
    if (objXMLHttpReq.readyState == 4 && objXMLHttpReq.status == 200) {
      ...
      if(!element) return;
      var lastPg = element.getElementsByTagName('center')[0].querySelector('a.active').innerHTML;
  ...
  function search2( id, type, elem, pgCount, first_page){
    ...
    switch(type){
      case 'Nick':
        ...
        reg = new RegExp( '[0-9]{2}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}: .*<b>' + rep( search_str ) + '<\/b><\/a>', 'i');
        break;
      case 'Fine':
...
/*!*/	  reg = new RegExp( '[0-9]{2}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}: .* ["\'].*' + rep( search_str ) + '.*["\']', 'i');
        break;
      case 'Any':
    ...
    center.innerHTML = 'Поиск по протоколу передач ' + title + ' ';
  ...
  function startSearch( pg, id, reg, lastPg, elem, type, search_str, first_page, curr_page_date ) {
    if ( $('stop').value != '1' && pg <= lastPg ) {
      ...
      request(url+link+'.php?id=' + id + '&page=' + (pg-1), search_2_pages, args);
    } else {
  ...
  function create_header(elem){
    ...
    const byNickName = protocolType == ProtocolType.PLAYER ?
          ...
          '</form></td></tr>' : '';
    const byFines = protocolType == ProtocolType.PLAYER ?
    ...
    const byArt = protocolType == ProtocolType.PLAYER ?
    ...
    div.innerHTML = `<table>${byNickName}${byFines}${byArt}${common}${byRegular}${byId}${fromPage}</table>`;
