...
;(function () {
	...
	const GREASYFORK_URL =
		'https://greasyfork.org/en/scripts/540058-mwi-market-addon'
	...
	`
		banner
			.querySelector('.mwm-update-dismiss')
	...
	async function fetchPriceChart(itemHrid, enhancement = 0) {
		...
		const url =
			enhancement > 0
				? `${MARKET_API}/addon/prices/${itemHrid}?enhancement=${enhancement}`
				: `${MARKET_API}/addon/prices/${itemHrid}`
				...
				onload: (response) => {
					if (response.status === 426) {
						try {
							...
							const match =
								errorData.error?.message?.match(/Expected ([^\s,]+)/)
