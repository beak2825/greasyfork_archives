// ==UserScript==
// @name         Reply-command (hotkey) Library
// @description  Library for Ventero's Reply-command (hotkey) script
// @version
// @license      Public Domain
// ==/UserScript==
// The following code is released under public domain.

var AutoUpdater_47983 = {
    id: 47983,
    days: 2,
    ...
    call: function(response, secure) {
	    ...
	    onload: function(xpr) {AutoUpdater_47983.compare(xpr, response);},
            onerror: function(xpr) {if (secure) AutoUpdater_47983.call(response, false);}
        });
    },
    enable: function() {
        GM_registerMenuCommand("Enable "+this.name+" updates", function() {
            GM_setValue('updated_47983', new Date().getTime()+'');
            AutoUpdater_47983.call(true, true)
        });
    },
    ...
    check: function() {
        if (GM_getValue('updated_47983', 0) == "off")
            this.enable();
        else {
            if (+this.time > (+GM_getValue('updated_47983', 0) + 1000*60*60*24*this.days)) {
                GM_setValue('updated_47983', this.time+'');
