// ==UserScript==
// @name         r* operator 0.2.4
// @namespace    http://tampermonkey.net/
// @version      0.2.4
// @description  support new url & solve event list img error
// @author       You
...
// @include        /https://admin.r\w+.com/store-operation\/main\/event_detail/
// @include        /https://admin.r\w+.com/store-operation\/main\/top_banner_detail/
...
// @include        /https://cms.r\w+.zone/store-operation\/banner\/list/
// @include        /https://cms.r\w+.zone/store-operation\/main\/event_detail/
// @include        /https://cms.r\w+.zone/store-operation\/main\/top_banner_detail/
// @include        /https://cms.r\w+.zone/store-operation\/event\/add_event/
// @include        /https://cms.r\w+.zone/store-operation\/coupon\/coupon_list/
// @include        /https://cms.r\w+.zone/store-operation\/event\/eventGroupImage/
// @run-at document-start
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/368857/r%2A%20operator%20024.user.js
// @updateURL https://update.greasyfork.org/scripts/368857/r%2A%20operator%20024.meta.js
// ==/UserScript==
...
$(function() {
    ...
    if(/store-operation\/main\/top_banner_detail/.test(document.location.href)) {
        $('#background_color').on('change', function(){
            var color = $(this).val();
            $('#detail_form img.attached').css('border', '40px solid ' + color);
        }).trigger('change');
    }
    ...
    if(
        ...
        || /store-operation\/main\/event_detail/.test(document.location.href)
        || /store-operation\/main\/top_banner_detail/.test(document.location.href)
        ...
        var checkImage = function(image_size, name){
            ...
            if(/store-operation\/main\/event_detail/.test(document.location.href) && (name == 'image' || name == 'extra_images[]')){
                if($('input[name=target_device]').val() == 'M')
                    assertImageSize('474x180', image_size);
                if($('input[name=target_device]').val() == 'PC')
                    assertImageSize('474x210', image_size);
            }
            if(/store-operation\/main\/top_banner_detail/.test(document.location.href)){
                if($('input[name=target_device]').val() == 'PC' && name == 'main_image')
                    assertImageSize('2000x640', image_size);
                if($('input[name=target_device]').val() == 'PC' && name == 'rolling_banner_image')
                    assertImageSize('600x92', image_size);
                if($('input[name=target_device]').val() == 'M' && name == 'main_image')
                    assertImageSize('720x360', image_size);
            }
            if(/store-operation\/event\/add_event/.test(document.location.href) && (name == 'banner_image')){
                if($('#genre').val() == 'general')
                    assertImageSize('824x480', image_size);
