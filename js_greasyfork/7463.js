...
// FYI THIS IS NOT IN THE ORIGINAL SCRIPT UPLOADED BY APPARITION.
// @include        http://*.pardus.at/squad_main.php*
// END MODIFICATIONS
// @grant          GM_getValue
// @grant          GM_setValue
// @grant          GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/7463/Pardus%20QI%20Augmenter.user.js
// @updateURL https://update.greasyfork.org/scripts/7463/Pardus%20QI%20Augmenter.meta.js
// ==/UserScript==////
(function () {
...
var UNIVERSE = location.host.split('.')[0];
if (UNIVERSE === undefined || UNIVERSE === '')
{
    UNIVERSE = 'orion';
    // console.log('UNABLE TO FIND THE UNIVERSE STOPPING');
    // return;
}
...
if ((location.pathname.indexOf('main.php') >= 0))
{
    ...
    if (options.useNavGrid)
    {
		...
		//MODIFIED FROM ORIGINAL SCRIPT TO WORK WITH NON-SQUARE WINDOWS. NOT INSIDE ORIGINAL SCRIPT BY APPARITION
        var navtable = document.getElementById('navarea');
        navtable.className += 'qi-navgrid';
        if (options.useHighlightTile)
        {
               var centerCells = Math.floor(navtable.rows[0].cells.length/2);
               var middle = Math.floor((navtable.rows.length) / 2  );
            navtable.rows[middle].cells[centerCells].className += ' qi-mytile';
        }
        ...
        for (var i = 0, len = pilots.length; i < len; i++)
        {
            ...
            if (pilotInfo.pilotId !== null)
            {
                    ...
                    options.priorityTrade.indexOf(pilotInfo.pilotName) !== -1)
                {
					...
					//ADDED DRUG TRACKER FOR INDIVIDUAL PILOTS
                    var li = document.createElement('LI');
					
                    li.innerHTML = [
                        '<a href="ship2ship_transfer.php?playerid=' ,
                         pilotInfo.pilotId ,
                         '">' ,'D: ', ' ',pilotInfo.pilotName, '</a>'
                    ].join('');
    ...
    if (options.useMapLink)
    {
        var sectorNode     = document.getElementById('sector'),
            ...
            mapLinkBaseUrl = 'http://pardusmap.mhwva.net',
...
if (location.pathname.indexOf('building.php') >= 0)
{
    ...
    {
        if (tbl[it].previousSibling && tbl[it].previousSibling.textContent == 'Condition')
        {
            tbl[it].previousSibling.textContent = 'Condition : ' + tbl[it].getAttribute('width') +'+';
            break;
        }
    }
...
if (location.pathname.indexOf('msgframe.php') >= 0)
{
    ...
    {
        var table = getFirstChild(document.getElementsByTagName('body')[0]);
