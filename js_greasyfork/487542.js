// ==UserScript==
// @name         FriendovTask100
// @namespace    http://tampermonkey.net/
// @version      1.86
// @description  FriendovVajda
// @author       xxxx
// @include https://*.the-west.*/game.php*
// @icon         data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/487542/FriendovTask100.user.js
// @updateURL https://update.greasyfork.org/scripts/487542/FriendovTask100.meta.js
// ==/UserScript==
...
(function() {
    ...
    ConsumablePrototype.prototype = {
        ...
        setSpecial:function(special) {
            this.hasCharacterBonus = special;
        }
    };
     ...
     Dobby = {
         ...
         forts:[],
         homeTown:null,
         jobFilter:{filterOnlySilver:true,filterNoSilver:false,filterCenterJobs:false,filterJob:""},
         sortJobTableXp:0,
         ...
         searchKeys:{
             ...
             "sk_SK":{
                 ...
                 productText:" Vylepšenie šance na získanie produktu",
                 expText:" Skúseností z práce, duelov a boja o pevnosť",
                 moneyText:" Peňazí z práce a duelov",
                 strengthText:" Sila",
                 mobilityText:" Pohyblivosť",
                 dexterityText:" Zručnosť",
                 charismaText:" Charizma"
             },
             "cs_CZ":{
                 energy:"Energie",
    ...
    Dobby.addConsumable = function(item) {
        ...
        if(bonuses[3] !== 0 || bonuses[4] !== 0 || bonuses[5] !== 0 || bonuses[6] !== 0 || bonuses[7] !== 0){
    ...
    Dobby.removeConsumable = function(item) {
        ...
        if(index != undefined) {
            if(Dobby.allConsumables[index].count > 1) {
                ...
                Dobby.allConsumables.splice(index,1);
            }
        }
    };
    Dobby.parseConsumableBonuses = function(bonuses) {
        var getBonus = function(text,type) {
            switch(type) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    text = text.replace(Dobby.searchKeys[Dobby.language].expText,"");
                    break;
                case 4:
                    text = text.replace(Dobby.searchKeys[Dobby.language].productText,"");
                    break;
                case 5:
                    text = text.replace(Dobby.searchKeys[Dobby.language].luckText,"");
                    break;
                case 6:
                    text = text.replace(Dobby.searchKeys[Dobby.language].moneyText,"");
                    break;
                case 7:
                    text = text.replace(Dobby.searchKeys[Dobby.language].strengthText,"");
                    text = text.replace(Dobby.searchKeys[Dobby.language].mobilityText,"");
                    text = text.replace(Dobby.searchKeys[Dobby.language].dexterityText,"");
                    text = text.replace(Dobby.searchKeys[Dobby.language].charismaText,"");
                    break;
            }
            text = text.slice(1);
            text = text.replace("%","");
        ...
        for(var i = 0; i < bonuses.length;i++) {
            ...
            if(bonuses[i].includes(Dobby.searchKeys[Dobby.language].energyText)) {
                ...
                type = 2;
            }else if(bonuses[i].includes(Dobby.searchKeys[Dobby.language].expText)) {
                type = 3;
            }else if(bonuses[i].includes(Dobby.searchKeys[Dobby.language].productText)) {
                type = 4;
            }else if(bonuses[i].includes(Dobby.searchKeys[Dobby.language].luckText)) {
                type = 5;
            }else if(bonuses[i].includes(Dobby.searchKeys[Dobby.language].moneyText)) {
                type = 6;
            }else if(bonuses[i].includes(Dobby.searchKeys[Dobby.language].strengthText)
                || bonuses[i].includes(Dobby.searchKeys[Dobby.language].mobilityText)
                || bonuses[i].includes(Dobby.searchKeys[Dobby.language].dexterityText)
                || bonuses[i].includes(Dobby.searchKeys[Dobby.language].charismaText)) {
                type = 7;
            }
            if(type != -1)
    ...
    Dobby.findProperConsumable = function(motivationMissing,energyMissing,healthMissing,averageMotivationMissing,consumables) {
        ...
        var findMotivationConsume = function(consumes) {
            ...
            for(var i = 0; i < consumes.length; i++) {
                ...
                if(consumeToChoose != null && consumes[i].motivation != 0) {
                    consumeToChoose = betterMotivation(consumeToChoose,consumes[i]); // ??? bolo tu consumables[i]
                    if(consumeToChoose.energy <= energyMissing+2)
                        consumeLessEnergy = consumeToChoose;
                }
            }
            if(consumeLessEnergy != null) return consumeLessEnergy;
            else return consumeToChoose;
        };
        var findHealthConsume = function(consumes) {
            ...
            for(var i = 0; i < consumes.length; i++) {
                if(consumes[i].health != 0) {
                    ...
                    if(consumes[i].energy <= energyMissing+2) {
                        if(consumes[i].energy == 0){
                            if(consumes[i].health < consumeToChoose.health){
                                consumeToChoose = consumes[i];
                            }
                        } else {
                            return consumes[i];
        ...
        if(energyMissing >= 99 && consums[0].energy != 0) {
            // Find the index of the first item with a different amount of energy
            const index = consums.findIndex((item, i, arr) => i > 0 && item.energy !== arr[0].energy);
            // Slice the array to keep only the items with the same amount of energy
            const slicedConsums = index !== -1 ? consums.slice(0, index) : consums;
            if(slicedConsums[0].motivation != 0) {
                 return findMotivationConsume(slicedConsums);
            } else {
                return slicedConsums[0];
            }
        }
        return null;
    };
    Dobby.tryUseConsumable = function(result) {
        ...
        //Dobby.findAllConsumables(); //check for newly added items in inventory before using consumable
        ...
        var isKO = false;
        if(Dobby.isHealthBelowLimit() && energyMissing >= 99 && TaskQueue.queue.length != 0) isKO = true;
        if(itemToUse == null && !isKO) return false;
        else if(itemToUse == null && isKO) {
            if(Dobby.regenerationSet != -1) Dobby.equipSet(Dobby.regenerationSet);
            else if(Dobby.healthSet != -1) Dobby.equipSet(Dobby.healthSet);
            return false;
        }
        Dobby.useConsumable(itemToUse);
        if(isKO) Dobby.cancelJobs();
        return true;
    };
    ...
    Dobby.isHealthBelowLimit = function() {
        if(Dobby.settings.healthStop >= ((Character.health/Character.maxHealth) * 100) && Dobby.settings.healthStopNumber >= Character.health) {
            return true;
        }
        return false;
    };
    ...
    Dobby.walkToJob = async function(index) {
        var jobToWalkToID = Dobby.addedJobs[index].id;
        for(var i = 0 ; i < Dobby.allJobs.length;i++) {
          if(Dobby.allJobs[i].x == Dobby.addedJobs[index].x && Dobby.allJobs[i].y == Dobby.addedJobs[index].y && Dobby.allJobs[i].id != Dobby.addedJobs[index].id){
                jobToWalkToID = Dobby.allJobs[i].id;
                break;
          }
        }
        JobWindow.startJob(jobToWalkToID, Dobby.addedJobs[index].x, Dobby.addedJobs[index].y, 15);
        while(true) {
            if(Map.calcWayTime(Character.position,{x:Dobby.addedJobs[index].x,y:Dobby.addedJobs[index].y}) == 0) {
                break;
            }
    ...
    Dobby.setCookies = function() {
        ...
        var temporaryObject = {
            jobsInSession:Dobby.statistics.jobsInSession,
            xpInSession:Dobby.statistics.xpInSession,
            addedJobs:Dobby.addedJobs,
            travelSet:Dobby.travelSet,
            jobSet:Dobby.jobSet,
            healthSet:Dobby.healthSet,
            regenerationSet:Dobby.regenerationSet,
        ...
        var pernamentObject = {
            //selectedSleepPlace:Dobby.selectedSleepPlace,
    ...
    Dobby.getCookies = function() {
        ...
        for(var i = 0; i < cookie.length;i++) {
            if(cookie[i].includes("dobby2temporary")) {
                ...
                Dobby.statistics.jobsInSession = tempObject.jobsInSession || 0;
                Dobby.statistics.xpInSession = tempObject.xpInSession || 0;
                Dobby.travelSet = tempObject.travelSet;
                Dobby.jobSet = tempObject.jobSet;
                Dobby.healthSet = tempObject.healthSet;
                Dobby.regenerationSet = tempObject.regenerationSet;
            ...
            if(cookie[i].includes("dobby2pernament")) {
                ...
                //Dobby.selectedSleepPlace = pernamentObject.selectedSleepPlace;
    ...
    Dobby.createConsumablesTable = function() {
        ...
        if(Dobby.consumableUsed.length === 1) {
            lastConsumUsed = Dobby.consumableUsed.at(-1).name;
        }
        if(Dobby.consumableUsed.length === 2) {
            lastConsumUsed = Dobby.consumableUsed.at(-1).name + ", " + Dobby.consumableUsed.at(-2).name;
        }
        if(Dobby.consumableUsed.length > 2) {
            lastConsumUsed = Dobby.consumableUsed.at(-1).name + ", " + Dobby.consumableUsed.at(-2).name + "...";
        }
        var lastConsumUsedText = $("<div>Last used: " + lastConsumUsed + "</div>");
        lastConsumUsedText.css('margin-top', '38px');
        lastConsumUsedText.css('margin-left', '15px');
    ...
    Dobby.createSettingsGui = function() {
		...
		var htmlHealthStopNumber = $("<div></div>");
        htmlHealthStopNumber.append("<span> Stoppage minimum real health value </span>");
        var healthStopNumberTextfiled = new west.gui.Textfield("healthStopNumber");
        healthStopNumberTextfiled.setValue(Dobby.settings.healthStopNumber);
        healthStopNumberTextfiled.setWidth(100);
        htmlHealthStopNumber.append(healthStopNumberTextfiled.getMainDiv());
        ...
        var buttonApply = new west.gui.Button("Apply",function() {
            ...
            if(Dobby.isNumber(healthStopNumberTextfiled.getValue())) {
                var healthStopNumber = parseInt(healthStopNumberTextfiled.getValue());
                healthStopNumber = Math.round(healthStopNumber);
                Dobby.settings.healthStopNumber = healthStopNumber;
            }
        ...
        htmlSkel.append("<br>");
        htmlSkel.append(htmlHealthStopNumber);
        htmlSkel.append("<br>");
    ...
    Dobby.createStatisticsGui = function() {
        ...
        var buttonResetStats = new west.gui.Button("Reset session",function() {
            Dobby.statistics.jobsInSession = 0;
            Dobby.statistics.xpInSession = 0;
            Dobby.setCookies();
            Dobby.selectTab("stats");
        });
        htmlSkel.append("<br>");
        htmlSkel.append(buttonResetStats.getMainDiv());
        return htmlSkel;
    };
