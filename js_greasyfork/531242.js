...
// @downloadURL https://update.greasyfork.org/scripts/531242/oc%20timer%2020%20-%20Heart.user.js
// @updateURL https://update.greasyfork.org/scripts/531242/oc%20timer%2020%20-%20Heart.meta.js
// ==/UserScript==

(async function () {
    'use strict';

    const API_KEY = "9MnYCKePrfSiXade";
    ...
    function createTimerDisplay() {
        ...
        timerContainer.style = 'position: absolute;bottom: -26px;left: 12px; display: flex; gap: 8px;';
    ...
    // Set the text label (e.g., O: for Organized Crime)
    timerText.innerText = text;

    // Assign IDs to the dynamic timer and end time elements
    dynamicTimer.id = id;
    endTime.id = id + 'End';

    // Style the end time (small font)
    endTime.style.fontSize = '10px'; // Change this value to adjust end time size
    endTime.style.color = 'skyblue';
    endTime.style.display = 'block';
      endTime.style.fontweight = 'bold';


    // Set the styles for the dynamic timer (increased font size)
    dynamicTimer.style.fontSize = '12.5px';  // Change this value to adjust the timer text size
    dynamicTimer.style.fontWeight = 'bold'; // Optional: makes the timer text bold

    // Set the styles for the timer text (increased font size)
    timerText.style.fontSize = '14px';  // Change this value to adjust the label size
