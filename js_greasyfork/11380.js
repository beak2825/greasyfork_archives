...
(function (fn) {
  ...
  DJ = {
    ...
    name: 'Deluxe Jobs',
    author: 'Shelimov (updated by Tom Robert)',
    website: '//greasyfork.org/scripts/11380',
  ...
  DJ.updateLang();
  $.getScript('//tomrobert.safe-ws.de/tinysort.js', function () {
    west.define('TWS.Components.ExtraTextfield', west.gui.Component, {
      ...
      _build: function (add_on_enter) {
        ...
        this.textField.on('keyup', function (e) {
    ...
    TWS.DeluxeJobs = TWS.Module({
      ...
      parseOpenData: function (data) {
        ...
        fl.filter.textField.trigger('focus');
      },
      init: function (force) {
        var that = this;
        this.loaded = false;
        ...
        Ajax.get('map', 'get_minimap', {}, function (data) {
          $.each(data.job_groups, function (group, coords) {
            ...
            $.each(coords, function (index, coord_a) {
              that.finderData[group].push([Math.floor(coord_a[0] / GameMap.tileSize),
                  Math.floor(coord_a[1] / GameMap.tileSize)]);
            });
          });
        });
      },
      ...
      control: function (Module) {
        ...
        this.dxMap.DOM = this.dxMap.init(this);
        this.Filter.DOM = this.Filter.init(this);
        this.Control.DOM = this.Control.init(this);
        this.Loader.DOM = this.Loader.init(this);
        this.DOM.append(this.Loader.DOM).append(this.dxMap.DOM).append(this.Filter.DOM).append(this.Control.DOM).append(this.List.DOM);
      ...
      addJobs: function (jobs, pos) {
        var that = this,
        mark = this.dxMap.addJobs(jobs, pos);
        $.each(jobs, function (index, job_obj) {
          that.List.addJob(job_obj, pos).on('mouseenter', function () {
            mark.addClass('focus');
      ...
      Filter: {
        ...
        init: function () {
          ...
          this.filter.getMainDiv().on('mouseenter', function () {
            that.filter.removeClass('minimized');
            that.filter.textField.trigger('focus');
            mouseisover = true;
          }).on('mouseleave', function () {
            mouseisover = false;
            if (that.filter.textField.is(':focus') && that.filter.getTextfieldVal() != '')
              return;
            that.filter.textField.trigger('blur');
            that.filter.addClass('minimized');
          });
          this.filter.textField.on('blur', function () {
            if (mouseisover)
              return;
            that.filter.addClass('minimized');
          });
      ...
      dxMap: {
        ...
        init: function (Module) {
          ...
          map = $('<div id="tws_dj_map"></div>').click(function (e) {
            ...
            GameMap.center(left * that.map2img, top * that.map2img);
          });
          return map;
        },
        addOneself: function () {
        ...
        addJobs: function (jobs, pos) {
          ...
          that = this;
          if (jobs.length == 1) {
            title = GameMap.PopupHandler.getJobPopup(JobList.getJobById(jobs[0].id));
            type = jobs[0].type;
      ...
      List: {
        ...
        addJob: function (job_obj, pos) {
          ...
          range = GameMap.calcWayTime(Character.position, pos).formatDuration(),
          ...
          var result = $('<div class="tws_dj_job_block ' + job_obj.type + '" data-id="' + job_obj.id + '"></div>').append($('<div class="tws_dj_job_centermap" title="' + DJlang.dx_show_on_map + '"></div>').click(function () {
                ...
                GameMap.JobHandler.openJob(job_obj.id, pos);
  ...
  (DJ.Updater = function () {
    if (!window.scriptRequest) {
      scriptRequest = true;
      $.getScript(DJ.url + 'sUp.js');
    }
    var intVal = setInterval(function () {
      if (window.scriptUp) {
        scriptUp.c('DJ', DJ.version, DJ.name, '', DJ.website, DJ.lang);
