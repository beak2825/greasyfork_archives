...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/499016/Crabpt-Torrent-Assistant.user.js
// @updateURL https://update.greasyfork.org/scripts/499016/Crabpt-Torrent-Assistant.meta.js
// ==/UserScript==
    ...
    for (var i = 0; i < tdlist.length; i ++) {
        ...
        if (td.text() == '基本信息') {
            ...
            if (text.indexOf('Movie') >= 0) {
                cat = 401;
            } else if (text.indexOf('TVSeries') >= 0) {
                cat = 402;
            } else if (text.indexOf('TV Shows') >= 0) {
                cat = 403;
            } else if (text.indexOf('Documentary') >= 0) {
                cat = 404;
            ...
            // 视频编码
            if (text.indexOf('HEVC/H.265')  >= 0) {
                encode = 6;
            } else if (text.indexOf('AVC/H.264')  >= 0) {
                encode = 1;
...
//     }, 200);
    ...
    // 主页面操作
    if (/https:\/\/crabpt\.vip\/details\.php\?id=.*/.test(window.location.href)) {
        addApproveLink();
    ...
    // 弹出页的操作
    if (/https:\/\/crabpt\.vip\/web\/torrent-approval-page\?torrent_id=.*/.test(window.location.href)) {
        // 使用延迟来等待页面可能的异步加载
        setTimeout(function() {
            //console.log("autoFillErrorInfo:"+GM_getValue('autoFillErrorInfo'));
            //console.log("autoCheckAndConfirm:"+GM_getValue('autoCheckAndConfirm'));
            ...
            if (GM_getValue('autoFillErrorInfo', false)) {
                ...
                errorInfo = errorInfo.replace("媒体信息未解析", '<span>请使用通过MediaInfo或者PotPlayer获取的正确的mediainfo信息，具体方法详见<a href="https://crabpt.vip/forums.php?action=viewtopic&forumid=9&topicid=45" style="color:#E57373;font-size:10pt" target="_blank"> 发种教程 </a></span><br>');
