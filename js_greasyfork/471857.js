function 模板生产(src){
	...
	function 添加toolbar按钮(html){
		if (toolbar.find('.weiyiyici').length==0){
			...
			if (local_菌引物板号=='true'){
				toolbar.append('<button id="button_jun_banhao" onclick="return false">菌引物板号</button>')
				//点击 菌引物板号  按钮
				toolbar.find('#button_jun_banhao').click(function(){
					获取菌引物板号()
				})
			}
			if (local_重抽引物板号=='true'){
				toolbar.append('<button id="button_chongchou_banhao" onclick="return false">重抽引物板号</button>')
				//点击 重抽引物板号  按钮
	...
	//获取质粒引物板号   搜索  引物浓度包含对应  且  模板板号包含ZL 
	function 获取质粒引物板号(){
		url='/seq/ashx/SeqProcessHandler.ashx?_search=true&nd=1708850474842&rows=200&page='+'第几页'+'&sidx=seqs_tempplate&sord=desc&filters={"groupOp":"AND","rules":[{"field":"seqs_observe","op":"cn","data":"对应"},{"field":"seqs_sam_type","op":"cn","data":"质粒"}]}'
		公共_引物(url)
	}
	
	//获取直提引物板号  搜索 引物浓度包含对应  模板板号包含T
	function 获取直提引物板号(){
		url='/seq/ashx/SeqProcessHandler.ashx?_search=true&nd=1708850474842&rows=200&page='+'第几页'+'&sidx=seqs_tempplate&sord=desc&filters={"groupOp":"AND","rules":[{"field":"seqs_observe","op":"cn","data":"对应"},{"field":"seqs_tempplate","op":"cn","data":"T"}]}'
		公共_引物(url)
	}
	//获取切胶引物板号  搜索 引物浓度包含对应 模板板号包含Q
	function 获取切胶引物板号(){
		url='/seq/ashx/SeqProcessHandler.ashx?_search=true&nd=1708850474842&rows=200&page='+'第几页'+'&sidx=seqs_tempplate&sord=desc&filters={"groupOp":"AND","rules":[{"field":"seqs_observe","op":"cn","data":"对应"},{"field":"seqs_tempplate","op":"cn","data":"Q"}]}'
		公共_引物(url)
	}
	
	//获取菌引物板号  搜索 引物浓度包含对应  且  样品类型包含菌  在 记录需要删除的索引 搜索重抽并去掉
	function 获取菌引物板号(){
		url='/seq/ashx/SeqProcessHandler.ashx?_search=true&nd=1708850474842&rows=200&page='+'第几页'+'&sidx=seqs_tempplate&sord=desc&filters={"groupOp":"AND","rules":[{"field":"seqs_observe","op":"cn","data":"对应"},{"field":"seqs_sam_type","op":"cn","data":"菌"}]}'
		公共_引物(url)
	}
	//获取重抽引物板号  搜索 引物浓度包含对应  且  返回状态包含重抽
	function 获取重抽引物板号(){
		url='/seq/ashx/SeqProcessHandler.ashx?_search=true&nd=1708850474842&rows=200&page='+'第几页'+'&sidx=seqs_tempplate&sord=desc&filters={"groupOp":"AND","rules":[{"field":"seqs_observe","op":"cn","data":"对应"},{"field":"seqs_back","op":"cn","data":"重抽"}]}'
		公共_引物(url,'重抽')
	}
	
	//公共函数
	function 公共_引物(url_原始,是否是查重抽){
		//先改成1，获取总共页数
		...
		for(var i=0;i<备注_arr.length;i++){
			...
			//如果返回状态包含重抽 并且不是查重抽 则去除
			if(返回状态_arr[i].indexOf('重抽')!=-1 && 是否是查重抽!='重抽'){
				记录需要删除的索引.push(i)
				continue
			}
		}
