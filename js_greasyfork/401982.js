...
// @version      4.0.0
// @namespace    coder_tq
// @description  自动挂机看尔雅MOOC，支持视频、音频、文档、图书自动完成，章节测验自动答题提交，支持自动切换任务点、挂机阅读时长、自动登录等，解除各类功能限制，开放自定义参数，感谢前辈开源！
// @author       coder_tq
// @match        *://*.chaoxing.com/*
// @match        *://*.edu.cn/*
// @match        *://*.nbdlib.cn/*
...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/401982/%E8%B6%85%E6%98%9F%E7%BD%91%E8%AF%BE%E5%8A%A9%E6%89%8B%285-5%E5%80%9F%E5%B0%B8%E8%BF%98%E9%AD%82%29.user.js
// @updateURL https://update.greasyfork.org/scripts/401982/%E8%B6%85%E6%98%9F%E7%BD%91%E8%AF%BE%E5%8A%A9%E6%89%8B%285-5%E5%80%9F%E5%B0%B8%E8%BF%98%E9%AD%82%29.meta.js
// ==/UserScript==
...
if (url == '/mycourse/studentstudy') {
    ...
    _self.layui.use('layer', function() {
        this.layer.open({content: '拖动进度条、倍速播放、秒过会导致不良记录！千万别拖动进度条！题库在慢慢补充，搜不到的题目系统会在以后随缘补充', title: '超星网课助手提示', btn: '我已知悉', offset: 't', closeBtn: 0});
    });
...
function findAnswer() {
    if (setting.num >= $('.TiMu').length) {
        var arr = setting.lose ? ['共有 <font color="red">' + setting.lose + '</font> 这题我不会瞎写的你猜对不对', saveThis] : ['答题已完成', submitThis];
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if (obj.code == 1) {
                    ...
                    setting.div.children('div:eq(0)').html(setting.over + obj.answer || setting.over + '不太会，正在重试...');
                }
                setting.div.children('span').html(obj.msg || '');
            } else if (xhr.status == 403) {
                var html = xhr.responseText.indexOf('{') ? '你太快了，建议稍后再试' : $.parseJSON(xhr.responseText).data;
                setting.div.children('div:eq(0)').data('html', html).siblings('button:eq(0)').click();
            } else {
                setting.div.children('div:eq(0)').html('慢点慢点，查询太频繁，怀疑你动机不正！明天再来吧...');
            }
        },
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').html(setting.over + '脑子有点转不动，正在重试...');
        }
    });
}
