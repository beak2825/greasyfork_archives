...
// @author       Elnaeth
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/544619/Stats%2C%20Shards%2C%20XP%2C%20Dust%2C%20Quest%2C%20Res%2C%20Loot%20and%20Level%20tracker.user.js
// @updateURL https://update.greasyfork.org/scripts/544619/Stats%2C%20Shards%2C%20XP%2C%20Dust%2C%20Quest%2C%20Res%2C%20Loot%20and%20Level%20tracker.meta.js
// ==/UserScript==
...
const ItemTypes = Object.freeze({
  ...
  DUNGEON_POINTS: { id: 48, name: "Dungeon Point", rarity: "common" },
  EGG: { id: 49, name: "Egg", rarity: "rare" },
...
const shardTracker = {
  ...
  parse: () => {
    ...
    for (const [i, lootDrop] of manarion.lootTracker.entries.entries()) {
	  ...
	  // Ignore dungeon shard drops
	  let n = 1;
	  while (i+n<manarion.lootTracker.entries.length && manarion.lootTracker.entries[i].Timestamp === manarion.lootTracker.entries[i+n].Timestamp) {
		  if(manarion.lootTracker.entries[i+n].LootID === ItemTypes.DUNGEON_POINTS.id) {
			  n = 0;
			  break;
		  }
		  n++;
  ...
  createTrackerContainers: () => {
    ...
    if (span) {
      span.parentElement.insertAdjacentHTML(
        "afterend",
        `
        <div class="col-span-2 flex justify-between"><span>Fish:</span><span id="elnaeth-currency-fish">0</span></div>
        <div class="col-span-2 flex justify-between"><span>Wood:</span><span id="elnaeth-currency-wood">0</span></div>
        <div class="col-span-2 flex justify-between"><span>Iron:</span><span id="elnaeth-currency-iron">0</span></div>        
        `
      );
    }
