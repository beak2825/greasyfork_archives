...
// @namespace https://greasyfork.org/users/765861
// @downloadURL https://update.greasyfork.org/scripts/441068/%E8%A7%86%E9%A2%91%E8%A7%A3%E6%9E%90.user.js
// @updateURL https://update.greasyfork.org/scripts/441068/%E8%A7%86%E9%A2%91%E8%A7%A3%E6%9E%90.meta.js
// ==/UserScript==
(function() {
 /*
  * 自定义区域
 ...
 // 解析接口菜单框展开的高度
 var developMenuHeight = 100;
 // 解析接口菜单框展开的速度（如果展开动画卡顿请设置0,单位是秒）
 var developMenuSecond = 0.2;
 // 解析接口（可多个）
 const parseInterfaces =["https://www.ckplayer.vip/jiexi/?url=","https://vip.parwix.com:4433/player/?url=","https://jsap.attakids.com/?url="];
 /*
  * 非自定义区域
  * 以下代码勿动
  * 以下代码勿动
        ...
        try{
            ...
            for(var i in parseInterfaces){
                if(i==parseInterfaces.length-1){
                    html += "<span class='spanStyle' style='"+MenuItemsStyle+"' url='"+parseInterfaces[i]+"'>解析接口"+(parseInt(i)+1)+"</span>";
                }else{
                    html += "<span class='spanStyle' style='"+MenuItemsStyle+"border-bottom-style:solid;' url='"+parseInterfaces[i]+"'>解析接口"+(parseInt(i)+1)+"</span>";
                }
            }
            html += "</div></div>";
            div.innerHTML = html;
