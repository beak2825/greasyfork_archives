...
// @namespace    https://greasyfork.org/users/807481
// @license      GPL
// @homepageURL  https://greasyfork.org/en/scripts/431197
// @copyright    2021-2021, 29maxsh
// @run-at       document-begin

// @downloadURL https://update.greasyfork.org/scripts/431197/Freebitcoin.user.js
// @updateURL https://update.greasyfork.org/scripts/431197/Freebitcoin.meta.js
// ==/UserScript==
...
var HOURS_BETWEEN_BUY_BONUS = 24; // How many hours to wait before to buy Bonuses Rewards Promo again
...
var ROLL_P = 50;                // How many time to play multiply, AFTER roll AND AFTER HOURS passed; Values: 0-100
...
var INCR = 133;    // Porcentage of increment in case of lost.
var HIGH_LOW = 4; //You can decide how to play; 0 random, 1 H, 2 L, 3 OLAER; , 4 - change on win; check guides

var MIN_LOSSES_BEFORE_PLAY = 1; // How many time it will play MIN_BET and lose before begin play bas_bet and double;  strategy >= 1
...
function panelInit () {
    ...
    var forced_start_roll = '<a id="forced_start_roll">start</a>';
    var forced_stop_roll = '<a id="forced_stop_roll">stop</a>';
    var multiply_full_container_content = '<div id="multiply_message"></div>';
    multiply_full_container_content += '<div id="multiply_cons_wins">wins: <b>0</b></div>';
    multiply_full_container_content += '<div id="multiply_cons_losts">losts: <b>0</b></div>';
    multiply_full_container_content += '<div id="multiply_bet_count">bet_count: <b>0</b></div>';
    multiply_full_container_content += '<div id="multiply_play_count">play_count: <b>0</b></div>';
    multiply_full_container_content += '<div id="multiply_max_cons_losts">max_cons_losts: <b>0</b></div>';
    multiply_full_container_content += '<div id="multiply_max_bet">max_bet: <b>0</b></div>';
    multiply_full_container_content += '<div id="balance_before">balance_before: <b>0</b></div>';
    multiply_full_container_content += '<div id="balance_now">balance_now: <b>0</b></div>';
    multiply_full_container_content += '<div id="multiply_wagered">multiply_wagered: <b>0</b></div>';
    multiply_full_container_content += '<div id="multiply_profit">multiply_profit: <b>0</b></div>';
    var multiply_full_container = '<div id="multiply_full_container">'+multiply_full_container_content+'</div>';
    var multiply_full_container_row = '<div class="row center" style="margin:0; margin-top:20px; padding-top:10px; position:fixed; left: 0;width:12.5%;"><div class="center" style="display:inline-block;">'+multiply_full_container+'</div><div class="center">'+forced_start_roll+'</div><div class="center">'+forced_stop_roll+'</div></div>';
    $('#main_content').prepend(multiply_full_container_row);
...
function multiply_manual() {
    ...
    const callback = function(mutationsList, observer) {
        ...
        if (won != 0) {
            ...
            bet_hilo = get_bet_hilo(consecutive_lost,bet_hilo);
    ...
    setTimeout(function(){
        bet_hilo = get_bet_hilo(0,bet_hilo);
...
function get_bet_hilo (index, bet_hilo) {
    console.log(index,bet_hilo);
    if (HIGH_LOW == 4 ) {
        if(index == 0){
            return bet_hilo == 'H' ? 'L' : 'H';
        } else {
            return bet_hilo;
        }
    } else if (HIGH_LOW == 3 ) {
