// ==UserScript==
// @name        WaniKani Context Sentences in Reviews (kustom)
// @namespace   nelemnaru
// @description Shows the context sentence in the question during vocab reviews.
// @include     *://www.wanikani.com/review/session*
// @version     2.3 code cleanup
// @license     Please improve and repost!
// @grant       none

// @downloadURL https://update.greasyfork.org/scripts/27353/WaniKani%20Context%20Sentences%20in%20Reviews%20%28kustom%29.user.js
// @updateURL https://update.greasyfork.org/scripts/27353/WaniKani%20Context%20Sentences%20in%20Reviews%20%28kustom%29.meta.js
// ==/UserScript==

// @require     http://code.responsivevoice.org/responsivevoice.js

//(2.2 added TTS, keyboard shortcut for showing sentE)
//(2.1 blank sente while fetching)

// Hide upper-right corner stats
//$("#stats").hide();

// Link to Jisho
$("#character span").click(function(){
	window.open("http://jisho.org/search/" + $(this).text());
});
...
var currentlevel = 30;
var otherknownkanji = "彼誰俺僕";//Here you can add other kanji you already know so that they won't be marked

var autoplayaudio = false;

//TODO make sentence TTS audio optional, and add for lessons
...
//Global variables
var currentItem, questionType, previousItemVoc, japanese, english, jvoc, gettingjson, listknown, listcurrent, prevtext;

var playSentenceAudio = function(realtext) {
	responsiveVoice.cancel();
    responsiveVoice.speak(realtext, "Japanese Female");
    prevtext = realtext;
	...
	setTimeout(function(){
		...
		if (currentItem.voc) msg.text = japanese;
        else if (currentItem.kan != undefined) msg.text = currentItem.kan;
...
var highlightVocab = function() {
    if (questionType == "reading") {$(".highlighted").css({"color":"#2e2e2e","background-color":"#fff"});}
    if (questionType == "meaning") {$(".highlighted").css({"color":"#e9e9e9","background-color":"#555"});}
}

//Formatting for marking kanji
var highlightKanji = function() {
    if (questionType == "reading") {$(".kanjicurrent").css({"color":"#80f0f0"});$(".highlighted > .kanjicurrent").css({"color":"#2e80f0"});}
    if (questionType == "meaning") {$(".kanjicurrent").css({"color":"#5080e9"});$(".highlighted > .kanjicurrent").css({"color":"#80e9e9"});}
    
    if (questionType == "reading") {$(".kanjiunknown").css({"color":"orange"});}
    if (questionType == "meaning") {$(".kanjiunknown").css({"color":"red"});}
}
...
$("#sentwrap").click(function() {//set to #question-type to make entire (black/white) bar clickable
  if (questionType === "meaning") {
    $("#sente").toggle();
  } 
});

$("#question-type > h1").before("<div id='sentplay' style='position:absolute;z-index:20;right:150px;padding-top:1.3em;font-size:1.4em;-moz-user-select: none;'>?</div>");

$("#sentplay").click(function(){playSentenceAudio(prevtext)});
    
	//Make list of known and current kanji based on level
	if (currentlevel != 0) {
		var listkanji = getWKKanjiLevels();
		listcurrent = listkanji[currentlevel - 1];
		listknown = "";
	...
	if (showaftercorrect) {
		$("#answer-form form button").on("click",function() {
			setTimeout(function(){
			　if (currentItem.voc !== undefined && $("#answer-form form fieldset").hasClass("correct")){$("#sentwrap").show();}
			}, 100);
		});
	}
	
	/* Listen for questionType */
	$.jStorage.listenKeyChange('questionType', function() {
		questionType = $.jStorage.get('questionType');
	
		// Cancel sentence audio
		//if (speechSynthesis.speaking) speechSynthesis.cancel();
		//responsiveVoice.cancel();
	});
	
	/* Listen for currentItem */
	$.jStorage.listenKeyChange('currentItem', function() {
		...
		if (previousItemVoc !== currentItem.voc) {
			$("#sentj").html("　");$("#sente").html("");//blank sentence area while fetching json
		...
		previousItemVoc = currentItem.voc;
		
		if (gettingjson) {gettingjson.abort();} //Abort previous getJSON

		// Only vocab has context sentences.
		if (currentItem.voc !== undefined) {
			fetchSentences();
            gettingjson.done(function(){
	            $("#sentj").html(japanese);
	            $("#sente").html(english);
	
	            highlightVocabInSentence();
	            highlightCurrentAndUnknownKanjiInSentence();

			   //Play TTS audio of sentence
			   if (!$("#answer-form form fieldset").hasClass("correct") &&
				   !$("#answer-form form fieldset").hasClass("incorrect") &&
			       questionType === "meaning") {
					   if (autoplayaudio) playSentenceAudio(japanese);
					   else prevtext = japanese;
			   }
			
			});
		} else {// if (currentItem.voc == undefined)
			$("#sentj").html("");
			$("#sente").html("");
		}
...
var fetchSentences = function() {
	// Grab the item info from WK server.  Process result when it arrives.
	gettingjson = $.getJSON('/json/vocabulary/' + currentItem.id, function(json) {
	japanese = json.sentences[0][0];
	english = json.sentences[0][1];
...
var highlightVocabInSentence = function() {
    if (highlightword) {
		jvoc = currentItem.voc;
...
var highlightCurrentAndUnknownKanjiInSentence = function() {
    if (currentlevel != 0) {
...
var playKanji = function() {
    setTimeout(function(){
       if (!$("#answer-form form fieldset").hasClass("correct") &&
		   !$("#answer-form form fieldset").hasClass("incorrect") &&
	      	questionType === "reading" &&
	      	currentItem.kan !== undefined &&
           $("#user-response").val() != "") {
       
           playSentenceAudio($("#user-response").val())
       }
