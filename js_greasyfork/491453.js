...
// @run-at       document-end
// @license      Apache 2.0 
// @downloadURL https://update.greasyfork.org/scripts/491453/Easy%20Easter%20%28PDA%29%20Bar.user.js
// @updateURL https://update.greasyfork.org/scripts/491453/Easy%20Easter%20%28PDA%29%20Bar.meta.js
// ==/UserScript==
...
(async () => {
    ...
    const button = $("<button>", { id: "kw--egg-navigator" });
    const image = $("<img>", { src: "https://gyazo.com/0d7e5bd899656fc588b4ccd5ebb3b5b6.png", alt: "Egg Image", height: "35px", width: "45px" });
    const indexSpan = $("<span>").text(`(${++index})`);
    
    // Clone the image element twice
    const clonedImage1 = image.clone();
    const clonedImage2 = image.clone();
    const clonedImage3 = image.clone(); // Clone the image element twice more
    
    // Append all elements in the desired order
    button.append(clonedImage1).append(clonedImage2).append(image).append(indexSpan).append(image.clone()).append(clonedImage2.clone()).append(clonedImage3);
