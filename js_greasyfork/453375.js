// ==UserScript==
// @name         Piracy links in stores
// @author       framerunner
// @license      CC BY-NC-SA 4.0
// @version      1.6.2
// @description  Advanced version of "Piracy (+GOG) links on Steam" that adds links at the bottom of the review area for piracy
// @require      https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js
// @match        https://store.steampowered.com/app/*
// @match        https://www.gog.com/*/game/*
// @icon         data:image/png;base64,
// @grant        none
// @namespace framerunner
// @downloadURL https://update.greasyfork.org/scripts/453375/Piracy%20links%20in%20stores.user.js
// @updateURL https://update.greasyfork.org/scripts/453375/Piracy%20links%20in%20stores.meta.js
// ==/UserScript==

// Pirate Sites
var pirateLinks = [
  {
    url: "https://gload.to/?s=",
    urlSpecial: "",
    title: "GLOAD"
  },
  {
    url: "http://igg-games.com/?s=",
    urlSpecial: "",
    title: "IGG"
  },
  {
    url: "https://www.skidrowreloaded.com/?s=",
    urlSpecial: "",
    title: "Skidrow & Reloaded"
  },
  {
    url: "https://fitgirl-repacks.site/?s=",
    urlSpecial: "",
    title: "FitGirl"
  },
  {
    url: "https://ddl-warez.cc/?s=",
    urlSpecial: "&gamecategory=pc",
    title: "DDL-Warez"
  },
  {
    url: "https://thepiratebay.org/search.php?q=",
    urlSpecial: "&cat=401",
    title: "Pirate Bay"
  }
];

// Where it works
var storePages = [
  {
    url: "https://store.steampowered.com/*",
    title: "STEAM"
  },
  {
    url: "https://www.gog.com/*",
    title: "GOG"
  }
];

var storePageResult = "";

storePages.forEach(function(e) {
  if (!!document.URL.match(e.url)) storePageResult = e.title;
});

console.log("Domain Match: ", storePageResult);

switch (storePageResult) {
  case "GOG":
        // Clean app names
        var appNameRaw = document.getElementsByClassName("apphub_AppName")[0].textContent;
        appNameRaw = appNameRaw
            .replace(/(™|®|©|&trade;|&reg;|&copy;|&#8482;|&#174;|&#169;)/g, "")  // remove trademark symbols
            .trim();

        // For search: remove most special characters except letters, numbers, spaces, and dots
        var appNameCleaned = appNameRaw.replace(/[^a-zA-Z0-9 .]/g, "");

        // URL encode for use in links
        var appNameEncoded = encodeURIComponent(appNameCleaned);

    pirateLinks.forEach(function(e) {
      $("button.cart-button")[0].parentElement.parentElement.append(rigGOG(e.url + appNameEncoded + e.urlSpecial, e.title));
    });
    break;

  case "STEAM":
        // Clean app names
        var appNameRaw = document.getElementsByClassName("apphub_AppName")[0].textContent;
        appNameRaw = appNameRaw
            .replace(/(™|®|©|&trade;|&reg;|&copy;|&#8482;|&#174;|&#169;)/g, "")  // remove trademark symbols
            .trim();

        // For search: remove most special characters except letters, numbers, spaces, and dots
        var appNameCleaned = appNameRaw.replace(/[^a-zA-Z0-9 .]/g, "");

        // URL encode for use in links
        var appNameEncoded = encodeURIComponent(appNameCleaned);

    $(".game_area_purchase:first").prepend('<div class="game_area_purchase_game demo_above_purchase"><h1>Download ' + appNameRaw + '</h1><p class="game_purchase_discount_countdown">These links lead to the search feature on specified websites</br></br></p><div class="game_purchase_action"><div class="game_purchase_action_bg"></div></div></div></div>');
    pirateLinks.forEach(function(e) {
      $(".game_purchase_action_bg:first").append(rigSteam(e.url + appNameEncoded + e.urlSpecial, e.title));
    });
    Array.prototype.forEach.call(document.getElementsByClassName("game_purchase_action"), function(e) {
      e.style.overflow = "auto";
    });
    // Sidebar for Steam
    $(".glance_ctn_responsive_left:first").append(' <div class="dev_row"><div class="subtitle column"><br></div></div><hr><br>');
    var gogSearchUrl = "https://www.gog.com/en/games?query=";
    $(".glance_ctn_responsive_left:first").append(' <div class="dev_row"><div class="subtitle column">GOG:</div> <div class="summary column"><a href="' + gogSearchUrl + appNameEncoded + '"target="_blank">Search GOG for ' + appNameRaw + '</a></div></div>');
    var f95SearchUrl = "https://f95zone.to/search/276536854/?q=";
    $(".glance_ctn_responsive_left:first").append(' <div class="dev_row"><div class="subtitle column">F95:</div> <div class="summary column"><a href="' + f95SearchUrl + appNameEncoded + '" target="_blank">Search F95 for' + appNameRaw + '</a></div></div>');
    var ddgReleasesSearchUrl = "https://duckduckgo.com/?q=site%3Areleases.com+";
    $(".glance_ctn_responsive_left:first").append(
    ' <div class="dev_row"><div class="subtitle column">Releases:</div> ' +
    '<div class="summary column"><a href="' + ddgReleasesSearchUrl + appNameEncoded + '" target="_blank">Search Releases.com for ' + appNameRaw + '</a></div></div>');
    var akSearchUrl = "https://www.allkeyshop.com/blog/products/?search_name=";
    var akSearchUrlEnd = "&operating_systems=pc&type=game&pagenum=1";
    $(".glance_ctn_responsive_left:first").append(' <div class="dev_row"><div class="subtitle column">Keyseller:</div> <div class="summary column"><a href="' + akSearchUrl + appNameEncoded + akSearchUrlEnd + '" target="_blank">Search Key for ' + appNameRaw + '</a></div></div><br><hr>');
    break;
}

// Buttons
function rigGOG(href, innerHTML) {
  var element = document.createElement("a");
  element.target = "_blank";
  element.style = "margin: 5px 0 5px 0 !important; padding: 5px 10px 5px 10px;";
  element.classList.add("button");
  element.classList.add("button--big");
  element.classList.add("cart-button");
  element.classList.add("ng-scope");
  element.href = href;
  element.innerHTML = innerHTML;
  return element;
}

function rigSteam(href, innerHTML) {
  var div = document.createElement("div");
  var element = document.createElement("a");
  var span = document.createElement("span");
  element.target = "_blank";
  div.id = "demoGameBtn";
  div.append(element);
  div.className = "btn_addtocart";
  element.classList.add("btn_green_steamui");
  element.classList.add("btn_medium");
  element.href = href;
  element.append(span);
  span.innerHTML = innerHTML;
  return div;
}

// CC BY-NC-SA 4.0 License in short:
// You can copy and adapt this code to your liking, as long as you credit me, keep it non-commercial and share it under these exact conditions.
// More details: https://creativecommons.org/licenses/by-nc-sa/4.0/