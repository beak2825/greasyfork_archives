// ==UserScript==
// @name         AutoDL 自动刷新实例 - 防止回收 (增强版)
// @namespace    http://tampermonkey.net/
// @version      1.1.0
// @description  自动刷新AutoDL平台长时间关机的实例，防止被平台回收。支持自定义天数阈值和批量操作。新增：运行实例监控和一键关机功能。
// @author       AutoDL助手
...
// @run-at       document-end
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/545451/AutoDL%20%E8%87%AA%E5%8A%A8%E5%88%B7%E6%96%B0%E5%AE%9E%E4%BE%8B%20-%20%E9%98%B2%E6%AD%A2%E5%9B%9E%E6%94%B6%20%28%E5%A2%9E%E5%BC%BA%E7%89%88%29.user.js
// @updateURL https://update.greasyfork.org/scripts/545451/AutoDL%20%E8%87%AA%E5%8A%A8%E5%88%B7%E6%96%B0%E5%AE%9E%E4%BE%8B%20-%20%E9%98%B2%E6%AD%A2%E5%9B%9E%E6%94%B6%20%28%E5%A2%9E%E5%BC%BA%E7%89%88%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    GM_addStyle(`
        .autodl-refresh-panel {
            ...
            right: 20px;
            width: 280px;
            background: rgba(255, 255, 255, 0.15);  /* 白色背景，95%不透明度（5%透明） */
        ...
        /* 新增：运行实例版本的展开宽度 */
        .autodl-refresh-panel.with-instances {
            width: 480px;
        }
        ...
        /* 收缩模式下隐藏右侧面板 */
        .autodl-refresh-panel:not(.with-instances) .autodl-right-panel {
            display: none;
        }
        ...
        .autodl-small-btn:hover {
            opacity: 0.8;
        }

        .autodl-shutdown-all-btn {
            width: 100%;
            padding: 10px;
        ...
        .autodl-shutdown-all-btn:hover {
            transform: translateY(-2px);
            box-shadow: 0 4px 8px rgba(220, 53, 69, 0.3);
        }

        .autodl-shutdown-all-btn:disabled {
            background: #6c757d;
            cursor: not-allowed;
            transform: none;
        }
        ...
        /* 展开模式下恢复单列布局 */
        .autodl-refresh-panel.expanded .autodl-main-layout {
            display: block;
        }

        .autodl-refresh-panel.expanded .autodl-right-panel {
            display: block;
            margin-top: 10px;
        }
    ...
    // 静默登录（不显示日志）
    async function loginSilently(phone, plaintextPassword) {
        try {
            debugLog(`开始静默登录流程，手机号: ${phone}`);
            ...
            const token = passportData.data.token;
            debugLog(`静默登录成功，获取到Token: ${token.substring(0, 20)}...`);
            return token;

        } catch (error) {
            debugLog(`静默登录失败: ${error.message}`, error);
            throw error;
        }
    }
    ...
    // 静默获取实例列表（不显示日志）
    async function getInstancesSilently(headers) {
        try {
            debugLog(`开始静默获取实例列表...`);
            ...
            const data = await response.json();
            debugLog('静默获取实例响应数据:', data);

            if (data.code === "Success") {
                const instances = data.data.list;
                debugLog(`静默获取成功，共 ${instances.length} 个实例`);
                return instances;
            } else {
                throw new Error(`获取实例列表失败: ${JSON.stringify(data)}`);
            }
        } catch (error) {
            debugLog(`静默获取实例列表失败: ${error.message}`, error);
            throw error;
        }
    }
    ...
    async function getRunningInstances() {
        try {
            ...
            return instances.filter(inst => inst.status === "running");
        } catch (error) {
            debugLog(`获取运行实例失败: ${error.message}`, error);
            return [];
        }
    }

    // 更新运行实例显示
    async function updateRunningInstancesDisplay() {
        const listContainer = document.getElementById('autodl-running-list');
        if (!listContainer) return;
        
        listContainer.innerHTML = '<div style="color: #6c757d;">检查中...</div>';
        
        try {
            const runningInstances = await getRunningInstances();
            
            if (runningInstances.length === 0) {
                listContainer.innerHTML = '<div style="color: #28a745; font-size: 11px;">✅ 无运行实例</div>';
            } else {
                const instancesHtml = runningInstances.map(inst => {
                    const displayName = inst.name || inst.uuid.substring(0,8)+'...';
                    return `<div class="autodl-instance-item">• ${displayName}</div>`;
                }).join('');
                
                listContainer.innerHTML = `
                    <div style="color: #dc3545; font-weight: bold; font-size: 11px; margin-bottom: 3px;">
                        共 ${runningInstances.length} 台运行中
                    </div>
                    ${instancesHtml}
                `;
            }
            ...
            // 更新一键关机按钮状态
            const shutdownBtn = document.getElementById('autodl-shutdown-all-btn');
            if (shutdownBtn) {
                shutdownBtn.disabled = runningInstances.length === 0;
                shutdownBtn.textContent = runningInstances.length > 0 
                    ? `🔴 一键关机所有运行实例 (${runningInstances.length}台)`
                    : '🔴 一键关机所有运行实例';
            }
            
        } catch (error) {
            listContainer.innerHTML = '<div style="color: #dc3545; font-size: 11px;">❌ 检查失败</div>';
            debugLog(`更新运行实例显示失败: ${error.message}`, error);
        }
    }
    ...
    async function shutdownAllRunningInstances() {
        try {
            ...
            if (runningInstances.length === 0) {
                addLog("ℹ️ 没有运行中的实例需要关机", "info");
                return;
            }
            
            const instanceNames = runningInstances.map(inst => 
                inst.name || inst.uuid.substring(0,8)+'...'
            ).join('\n');
            
            if (!confirm(`确定要关闭以下 ${runningInstances.length} 个运行中的实例吗？\n\n${instanceNames}`)) {
                return;
            }
            ...
            if (panel && expandedContent && !expandedContent.classList.contains('visible')) {
                ...
                document.getElementById('autodl-expand-hint').style.display = 'none';
            ...
            addLog(`🔴 开始批量关机 ${runningInstances.length} 个运行实例...`, "info");
            ...
            shutdownBtn.textContent = '关机中...';
            
            for (const inst of runningInstances) {
                try {
                    const displayName = inst.name || inst.uuid.substring(0,8)+'...';
                    addLog(`正在关机: ${displayName}`, "info");
                    await powerOffInstance(authHeaders, inst.uuid);
                    addLog(`✅ ${displayName} 关机成功`, "success");
                    successCount++;
                    await sleep(2000); // 间隔2秒避免请求过快
                } catch (error) {
                    const displayName = inst.name || inst.uuid.substring(0,8)+'...';
                    addLog(`❌ ${displayName} 关机失败: ${error.message}`, "error");
                    errorCount++;
                }
            }
            
            addLog(`🎉 批量关机完成！成功: ${successCount}, 失败: ${errorCount}`, "success");
            
            // 更新显示
            setTimeout(updateRunningInstancesDisplay, 3000);
            
        } catch (error) {
            addLog(`❌ 批量关机失败: ${error.message}`, "error");
        } finally {
            const shutdownBtn = document.getElementById('autodl-shutdown-all-btn');
            if (shutdownBtn) {
                shutdownBtn.disabled = false;
                shutdownBtn.textContent = '🔴 一键关机所有运行实例';
            }
        }
    }
    ...
    function startAutoRefresh() {
        ...
        // 立即执行一次完整检查
        updateRunningInstancesDisplay();
        checkInstanceStatus();
        
        // 设置5分钟定时器，同时检查运行实例和即将释放的实例
        refreshInterval = setInterval(() => {
            updateRunningInstancesDisplay(); // 检查运行实例
            checkInstanceStatus(true); // 静默检查即将释放的实例（不显示过多日志）
        }, 5 * 60 * 1000); // 5分钟
        
        debugLog('✅ 已启动5分钟自动刷新：运行实例状态 + 即将释放检查');
        addLog('🔄 已启动5分钟自动刷新：运行实例监控 + 即将释放预警', 'success');
    }
    ...
    // 独立检查实例状态（不执行刷新任务）
    async function checkInstanceStatus(silent = false) {
        const phone = document.getElementById('autodl-phone').value.trim();
        const password = document.getElementById('autodl-password').value.trim();
        ...
        try {
            ...
            // 登录并获取实例列表（静默模式不显示登录日志）
            const token = silent ? await loginSilently(phone, password) : await loginAndGetToken(phone, password);
            const authHeaders = { ...HEADERS, "Authorization": token };
            const instances = silent ? await getInstancesSilently(authHeaders) : await getInstances(authHeaders);
    ...
    function initializeExpandedContent() {
        ...
        addLog("🚀 AutoDL自动刷新插件已加载 (增强版)", "success");
        addLog("💡 功能说明：自动刷新关机超过指定天数的实例，防止被平台回收", "info");
        addLog("🆕 新增功能：运行实例监控和一键关机", "info");
    ...
    function createControlPanel() {
            ...
            <div class="autodl-refresh-content">
                ...
                <div class="autodl-main-layout">
                    ...
                    <div class="autodl-left-panel">
                        ...
                        <div style="font-size: 10px; color: #6c757d; margin-top: -4px; margin-bottom: 6px;">
                            输入还有几天会被释放
                        </div>
                        ...
                        <div class="autodl-warning-counter safe" id="autodl-warning-counter">
                            5天后被释放: <span id="autodl-warning-count">检查中...</span>
                        </div>
                    </div>
                    
                    <!-- 右侧：运行实例显示 + 一键关机 -->
                    <div class="autodl-right-panel">
                        <div class="autodl-running-instances" id="autodl-running-instances">
                            <div class="autodl-running-header">
                                <span>🟢 运行中的实例:</span>
                                <button id="autodl-refresh-instances-btn" class="autodl-small-btn" title="刷新实例状态">🔄</button>
                            </div>
                            <div id="autodl-running-list" class="autodl-instance-list">
                                检查中...
                            </div>
                        </div>
                        <!-- 一键关机按钮 -->
                        <button id="autodl-shutdown-all-btn" class="autodl-shutdown-all-btn" style="margin-top: 8px;">
                            🔴 一键关机所有运行实例
                        </button>
                    </div>
                </div>
        ...
        // 绑定新增的按钮事件
        document.getElementById('autodl-refresh-instances-btn').addEventListener('click', updateRunningInstancesDisplay);
        document.getElementById('autodl-shutdown-all-btn').addEventListener('click', shutdownAllRunningInstances);

        // 展开/收起切换功能 (修改版 - 支持运行实例显示)
        document.getElementById('autodl-expand-btn').addEventListener('click', () => {
            ...
            if (expandedContent.classList.contains('visible')) {
                ...
                panel.classList.add('with-instances'); // 显示运行实例面板
                ...
                // 当前是收起状态，执行展开
                panel.classList.add('expanded');
                panel.classList.remove('with-instances');
                expandedContent.classList.add('visible');
                expandBtn.textContent = '⬆️';
                document.getElementById('autodl-expand-hint').style.display = 'none'; // 隐藏提示
                // 初始化展开内容
        ...
        const saveSettings = () => {
            ...
            // 如果手机号和密码都有值，启动自动刷新（包含所有检查）
            if (phone && password) {
                // 启动运行实例管理功能
                const panel = document.querySelector('.autodl-refresh-panel');
                if (panel && !panel.classList.contains('with-instances')) {
                    panel.classList.add('with-instances'); // 显示运行实例面板
                }
                
                // 如果还没有启动自动刷新，则启动
                if (!refreshInterval) {
                    startAutoRefresh(); // 启动5分钟自动刷新（包含运行实例检查和即将释放检查）
                }
            }
        };
    ...
    // 等待页面加载完成后创建控制面板 (保持原有逻辑不变)
    function init() {
        console.log('🚀 AutoDL 自动刷新插件正在初始化... (增强版)');
        ...
        if (window.location.hostname.includes('autodl.com')) {
            ...
            setTimeout(() => {
                createControlPanel();
                console.log('✅ AutoDL 自动刷新插件初始化完成 (增强版)');
            }, 2000);
        } else {
