// ==UserScript==
// @name         Wanikani Vertical Review Progress Bar
// @namespace    Wanikani Vertical Review Progress Bar
// @version      1.1
// @description  bar
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/465784/Wanikani%20Vertical%20Review%20Progress%20Bar.user.js
// @updateURL https://update.greasyfork.org/scripts/465784/Wanikani%20Vertical%20Review%20Progress%20Bar.meta.js
// ==/UserScript==
...
(async function() {
    ...
    const apiToken = 'your api token here please uwu';
    ...
    if(apiToken == 'your api token here please uwu'){
...
async function init_fetchReviews(url = 'https://api.wanikani.com/v2/assignments?immediately_available_for_review=true', allReviews = []) {
  const response = await fetch(url, { method: 'GET', headers: requestHeaders });
  const responseBody = await response.json();

  const currentPageReviews = responseBody.data.map(assignment => assignment.data.subject_type);
  ...
  if (responseBody.pages.next_url) {
    ...
    //return allReviews;
    init_radicalCount = allReviews.filter(type => type === 'radical').length;
    init_kanjiCount = allReviews.filter(type => type === 'kanji').length;
    init_vocabCount = allReviews.filter(type => type === 'vocabulary').length;

    new_radicalCount = allReviews.filter(type => type === 'radical').length;
    new_kanjiCount = allReviews.filter(type => type === 'kanji').length;
    new_vocabCount = allReviews.filter(type => type === 'vocabulary').length;

    all = init_radicalCount + init_kanjiCount + init_vocabCount;

    console.log(`Radicals: ${init_radicalCount}, Kanji: ${init_kanjiCount}, Vocab: ${init_vocabCount}`);
  }
}

  async function new_fetchReviews(url = 'https://api.wanikani.com/v2/assignments?immediately_available_for_review=true', allReviews = []) {
  const response = await fetch(url, { method: 'GET', headers: requestHeaders });
  const responseBody = await response.json();
  ...
  if (responseBody.pages.next_url) {
    return new_fetchReviews(responseBody.pages.next_url, allReviews);
  } else {
    //return allReviews;
    new_radicalCount = allReviews.filter(type => type === 'radical').length;
    new_kanjiCount = allReviews.filter(type => type === 'kanji').length;
    ...
    console.log(`new Radicals: ${new_radicalCount}, new Kanji: ${new_kanjiCount}, new Vocab: ${new_vocabCount}`);
  }
}
  ...
  async function update(){
    ...
    let colour_setup = `#guesses_ > a.show > *, #guesses_ > a.show::before, #guesses_ > button_.show { font-size: 1.35em; text-shadow: 2px 2px 3px rgba(0,0,0,0.4); background-image: linear-gradient(to bottom, ${rest_colour} 0%, ${rest_colour} ${restCount_percent}, ${radical_colour} ${restCount_percent}, ${radical_colour} ${radicalCount_percent}, ${kanji_colour} ${radicalCount_percent}, ${kanji_colour} ${kanjiCount_percent}, ${vocab_colour} ${kanjiCount_percent}, ${vocab_colour} 100%); box-shadow: 0.5rem 0.2rem 0.2rem #0000001b; }`
 ...
 "#confusionGuesserOverlay_ { position: absolute; top: 1.5rem; left: 1.5rem; padding-top: 1.0rem; z-index: 100; overflow-x: hidden; pointer-events: none; min-height: 15rem; max-height: 15rem; min-width: 1.5rem; max-width: 2.5rem;}" +
			...
			colour_setup +
			"#guesses_ > button_ { grid-column: 1 / -1; padding: 0.5rem; color: rgba(255, 255, 255, 0.3); background: none; border: thin dashed rgba(255, 255, 255, 0.3); border-radius: 0.5rem; --gc: var(--vissim); }" +
