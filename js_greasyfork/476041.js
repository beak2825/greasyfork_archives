// ==UserScript==
// @name           WFES - AutoReleaseHold
// @version        1.1.0
// @description    Release nomination from HOLD when this plugin is active
// @author         Skinny
...
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/476041/WFES%20-%20AutoReleaseHold.user.js
// @updateURL https://update.greasyfork.org/scripts/476041/WFES%20-%20AutoReleaseHold.meta.js
// ==/UserScript==
...
(function() {
  ...
  function _setQueue(id) {
    // XHR -> releasehold
    const theUrl = "/api/v1/vault/manage/releasehold";
    ...
    request.addEventListener("load", function() {
      if (request.status >= 200 && request.status < 300) {
        ...
        window.wfes.f.createNotification("AutoQueue failed, see console for details", "red");
  ...
  function _prozessNext() {
    if (idlist.length > 0) {
      // have more?
      const o = idlist.pop();
      window.wfes.f.createNotification(`AutoQueue: ${o.title}`, "orange");
      _setQueue(o.id);
    } else {
      window.wfes.f.createNotification("AutoQueue: all nominations processed, reloading page", "green");
  ...
  function AutoQueue() {
    ...
    for (let i = 0; i < nomList.length; i++) {
      ...
      if (("NOMINATION" === nom.type) && ("HELD" === nom.status)) {
        // search for '#hold'
  ...
  if (window.wfes.f.hasMinVersion("1.4.0")) {
    window.addEventListener("WFESNominationListLoaded", AutoQueue);
  } else {
