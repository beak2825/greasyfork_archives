...
// @downloadURL https://update.greasyfork.org/scripts/393743/Toolkit%20for%20HW.user.js
// @updateURL https://update.greasyfork.org/scripts/393743/Toolkit%20for%20HW.meta.js
// ==/UserScript==
...
function loadIpLogs(dbName) {
    ...
    var getBtns = function (i) {
        ...
        return '<a href="#" id="deleteip" name="' + i + '">[delete]</a> ' + savedLink + ' ' + ignoreLink + ' <a href="http://hackerwars.io/internet?ip=' + i +'&action=hack&method=bf" id="bruteip" name="' +
        i + '" target="_blank">[brute]</a>';
    };
    for (var i in db) {
		if (db.hasOwnProperty(i)) {
			$('#logdblist').append('<div id="' + i + '"><a href="http://hackerwars.io/internet?ip=' + i + '" id="loadlocal" name="' + i + '">' + i + '</a>&nbsp;&nbsp;&nbsp;' +
			getBtns(i) + '</br></div>');
		}
    }
...
if (window.location.href.indexOf("internet") != -1 && successAlert(alertText())) {
    window.location.replace("http://hackerwars.io/internet?view=logs");
}
...
if (window.location.href.indexOf("hackerwars.io/list") != -1 ) {
...
if (window.location.href.indexOf("hackerwars.io/log") != -1) {
...
if (window.location.href.indexOf("hackerwars.io/internet?view=logs") != -1) {
    var log = $('form.log').find('.logarea');
    if (log && log.length > 0) {
        text = log.val();
