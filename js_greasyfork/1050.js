...
// @version        1.26.15 neuer Bilderhoster
// @downloadURL https://update.greasyfork.org/scripts/1050/Infozentrale.user.js
// @updateURL https://update.greasyfork.org/scripts/1050/Infozentrale.meta.js
// ==/UserScript==
...
//
// @version        1.26.14 Probleme mit Bilderhoster behoben; diverse kleine Fehler korrigiert
// @version        1.26.13 Fehler bei Kampfstärkewarnung behoben
// @version        1.26.12 Totalumbau der gehosteten Bilder
// @version        1.26.11 neuer Bilderhoster
// @version        1.26.10 Bilder erneuert und kleine Korrekturen
// @version        1.26.9 Umstellung auf https und kleine Korrekturen
// @version        1.26.8 Sortiermöglichkeiten bei Haustier erweitert
// @version        1.26.7 Performance verbessert; Tagesaufgaben für Atlantis korrigiert
// @version        1.26.6 Fehler bei Bastelfunktion behoben; Anzeige von Boostern/Verletzungen geändert
// @version        1.26.5 Bastelfunktion war defekt
// @version        1.26.4 Korrekturen Atlantis; Anzeige Missionsplunder in Stadtteilen/Gebieten
// @version        1.26.3 DEX-Stufe beim Basteln wurde nicht berücksichtigt
// @version        1.26.2 Plunder basteln mit einem Klick
// @version        1.26.1 Anpassungen Atlantis
// @version        1.25.13 Anzeige des fehlenden Plunders beim Basteln
// @version        1.25.12 Anzeige des im Inventar befindlichen Plunders bei Eigentumupgrades
// @version        1.25.11 ausgewählte Nachrichten als gelesen markieren
// @version        1.25.10 Promillesteuerung optimiert; Fehler bei Minijobs (VT) behoben
...
// @version        $Id: infozentrale.user.js 212 2025-03-24 09:39:56Z mkl $

if (!String.prototype.endsWith)
    String.prototype.endsWith = function(str) {
        return (this.substr(this, -str.length) == str);
    };
...
if (myprof.getElementsByTagName("form").length > 0) {
    ...
    myprof.getElementsByTagName("form")[0].onsubmit = function(event) {
            ...
            var logout = true;
            if (warn != "")
                logout = confirm(warn + " " + getLangTxt(TxtLogout));
            if (logout)
               {
...
// Holen einer Variablen
function PG_getValue(varname, deflt) {
    return GM_getValue(TOWNEXTENSION + varname, deflt);
}
...
var PLUNDERIMAGE_URL = new Array();
    PLUNDERIMAGE_URL[0] = "//static.pennergame.de/img/pv4/plunder_new/old/";
    PLUNDERIMAGE_URL[1] = "//static.pennergame.de/img/pv4/plunder_new/";
    PLUNDERIMAGE_URL[2] = "//static.pennergame.de/img/pv4/plunder/";
    PLUNDERIMAGE_URL[3] = "//static2.pennergame.de/img/pv4/plunder_new/";
...
var TxtLifepts3 = new Array();
    TxtLifepts3[0] = 'Lebenspunkte können nicht %saufgefüllt werden. Bitte warten!';
var TxtLifepts4 = new Array();
    TxtLifepts4[0] = 'Ahh! Das tat gut!';
...
var MAXNROFSLOTS = 9;
var anzPlnd = 3;             // Anzahl der Plunderstücke im Plundershop
var bgrndInd = 1;
var DEXSkill = -1;             // Skilllevel DEX
var aktMoney = 0;
var misscontent = "";
...
// Links für die täglichen Aufgaben
var dailyTaskLinks = new Array();
    ...
    dailyTaskLinks[22] = 'city/plundershop/';
...
// Wenn im Vatikan gespielt wird
    ...
    TOWNEXTENSION = 'VT';
    DRINK_STACK += 'foodstuffs/';
    dailyTaskLinks[13] += 'foodstuffs/';
    FoodNames[0] = 'Halloween S&uuml;&szlig;igkeiten';
    FoodNames[1] = 'harte Brotkanten';
    FoodNames[2] = 'Oblate';
    FoodNames[3] = 'Stollen';
    FoodNames[4] = 'Götterspeise';
    alcoplunder[0] = "Nikolaus-Plunder";
    alcoplunder[1] = "Goldenes Bier";
    TxtBeer2[0] = "Weihwasser";
    TxtBought[0] = 'Benutzen';
    var TxtBoughtMulti = "Du besitzt diese Waffe";
    noWash = true;
    anzPlnd = 6;
// Wenn in Atlantis gespielt wird
} else if (language == "at_DE") {
    TOWNEXTENSION = 'AT';
    GM_deleteValue("ATskriptmustupdate");
    DRINK_STACK += 'foodstuffs/';
    dailyTaskLinks[13] += 'foodstuffs/';
    FoodNames[0] = 'Halloween S&uuml;&szlig;igkeiten';
    FoodNames[1] = 'harte Brotkanten';
    FoodNames[2] = 'Zwieback';
    FoodNames[3] = 'Stollen';
    FoodNames[4] = 'Pökelfleisch';
    alcoplunder[0] = "Nikolaus-Plunder";
    alcoplunder[1] = "Goldenes Bier";
    TxtBeer2[0] = "Wasser";
    TxtBought[0] = 'Benutzen';
...
// Wenn Operation Pennersturm gespielt wird
    ...
    currency = '$';
// Wenn Speedwelt Athen gespielt wird
} else if (language == 's7_DE') {
    TOWNEXTENSION = 'ATH';
    FoodNames[0] = 'Halloween S&uuml;&szlig;igkeiten';
    FoodNames[1] = 'Tsatsiki';
    FoodNames[2] = 'Souflaki';
    FoodNames[3] = 'Stollen';
    FoodNames[4] = 'Gyros';
    alcoplunder[0] = "Nikolaus-Plunder";
    alcoplunder[1] = "Goldenes Bier";
    TxtBeer1[0] = "Zitronenwasser";
    noWash = true;
    anzPlnd = 6;
...
for (i = 0; i < keys.length; i++) {
    var val = keys[i];
    if (val.substr(0,9) == "undefined" || val.endsWith("undefined") || val.endsWith("Userdata") || val.endsWith("PlunderDirectAccess") || val == "ScriptVersion" || val == "GetInfo" || val.indexOf('XmasCalendar') != -1)
        GM_deleteValue(val);
...
function convertUni(text) {
    return text.replace(/&auml;/g, "ä").replace(/&Auml;/g, "Ä").replace(/&ouml;/g, "ö").replace(/&Ouml;/g, "Ö").replace(/&uuml;/g, "ü").replace(/&Uuml;/g, "Ü").replace(/&szlig;/g, "ß");
}
...
function initDaily(lang) {
    ...
    switch (lang) {
        case 0:
                ...
                if (TOWNEXTENSION == 'AT')
                    taskTexts[1] = 'Eine Sabotage erfolgreich begehen';
                else if (TOWNEXTENSION == 'VT')
                    taskTexts[1] = 'Eine Sünde erfolgreich begehen';
                ...
                taskTexts[14] = 'Einmal ' + (TOWNEXTENSION == "AT"?"Perlen":"Flaschen") + 'sammeln starten';
                taskTexts[15] = 'Jetzt ' + (TOWNEXTENSION == "AT"?"Perlen":"Flaschen") + ' verkaufen';
                taskTexts[16] = 'Jetzt Plunder verkaufen';
                if (new Array('SY', 'PM', 'VT', 'AT').indexOf(TOWNEXTENSION) != -1)
...
function dblclick(e){
    ...
    if (lockmove)
        if (img)
            ...
            ShowImg ('PDBACK', '', get_icon("lock"), getLangTxt(TOOLTIP_LOCK), 10, 10, 240, 1, 1, 'lockmove');
    else
...
function getIconAddr(img){
    ...
    imgs = img.split('#');
    indx = imagehoster.substr(NrOfCalls % imagehoster.length, 1);

    return imgPrefix[indx] + imgs[indx];
}

function get_icon(s){
    var i = iconNames.indexOf(s);
    if (i < 0)
        return "";
    return allIcons[i];
}
...
function ShowImg(bgrnd, imglink, imgsource, imgtitle, imgwidth, imgheight, imgleft, imgtop, imgzindex, imgid) {
    ...
    if (bgrnd == '') {
        ...
        var newspan = document.getElementById(imgid);
        if (newspan) {
            var newdiv = document.getElementById(imgid + "Div");
            var newdiv2 = document.getElementById(imgid + "Div2");
        }
        else {
        ...
        if (!document.getElementById(bgrnd))
            return null;
        var newdiv = document.getElementById(bgrnd+"Div");
        for (var i = 0; i < koord.length; i++)
            if (bgrnd == koord[i][0]) {
                imgleft += koord[i][1];
                imgtop  += koord[i][2];
    ...
    if (typeof imglink == "function") {
        var newElem = document.getElementById(imgid + "lnk");
        if (!newElem)
            newElem = newdiv.appendChild(document.createElement('a'));
...
function CheckForUpdate(checkminutes) {
    ...
    if (IsTimeToCheck("LastUpdateCheck", (checkminutes==0?120:checkminutes), "x") || iconVers == "") {
        allIcons = "";
        ...
        GM_xmlhttpRequest({method: 'GET',
            ...
            onload:function(responseDetails) {
                ...
                if (responseDetails.status == 200) {
                    ...
                    var iv = getnewvar(content, "iconVers");
                    if (iv != iconVers) {
                        imagehoster = getnewvar(content, "imagehoster")+"";
                        GM_setValue("imagehoster", imagehoster);
                        imgPrefix = getnewvar(content, "imgPrefix");
                        GM_setValue("imgPrefix", imgPrefix);
                        iconNames = getnewvar(content, "iconNames");
                        GM_setValue("iconNames", iconNames);
                        allIcons = getnewvar(content, "allIcons");
                        GM_setValue("allIcons", allIcons);
                        iconVers = iv;
                        GM_setValue("iconVers", iconVers);
                    }
                    ...
                    if (thisver < thatver) {
                        ...
                        alert(printf(getLangTxt(TxtNewVersion), getLangTxt(THISSCRIPTNAME), scriptfullversion, THISSCRIPTINSTALL_URLGF));
                        // Seite mit dem neuen Skript laden, um eine Installation zu ermöglichen
                        window.location.href = THISSCRIPTINSTALL_URLGF+'/code/Infozentrale.user.js';
                    }
                }
            }
        });
...
function getnewvar (string, v, alrt) {
    var pos = string.indexOf("*" + v + "* = \'");
    if (pos < 0) {
		if (alrt == undefined || alrt)	
			alert("Fehler: Variable " + v + " nicht gefunden.");
        return "";
    }
    return string.substr(pos).split("'")[1];
}
...
// **********************************************************************************
// **********************************************************************************
function GetIncomingFights(content) {
    ...
    try {
        ...
        for (i = 0; i < splitted.length - 1; i++) {
            ...
            fights.push(new Array(time, attacker, ausweich));
        }
 // var fighttext = "14:59:00 riffi#N/15:00:00 Hetzjagd#N/15:00:00 U-Bahnkellner86#N/15:00:01 Max_von_ne_Weide#N/15:00:02 Gammelsuse2#N/";
 ...
 // fighttext = fighttext.split("/").slice(0, -1);
 // for (var i = 0; i < fighttext.length; i++) {
 //     var split1 = fighttext[i].split('#');
 //     var split2 = split1[0].split(' ');
 //     fights.push(new Array(split2.splice(0, 1)[0], split2.join(" "), split1[1]));
 //     }
        return fights;
    } catch(err) {
...
function IsPlunderPage() {
    ...
    if (currentURL.endsWith("/stock/plunder/?success") || currentURL.endsWith("/stock/plunder/")) {
        // True zurückgeben
        return true;
    } else {
        // False zurückgeben
...
function IsCraftPage() {
    ...
    // Wenn es sich bei der aktuellen Seite um die Plunderseite handelt
    if (currentURL.indexOf("/craft/?status=") != -1  || currentURL.endsWith("/craft/")) {
        // True zurückgeben
        return true;
    } else {
        // False zurückgeben
...
function IsMessagesPage() {
    ...
    if (currentURL.endsWith("/messages/") || currentURL.match(/\/messages\/\d+\//) != null) {
        // True zurückgeben
        return true;
    } else {
        // False zurückgeben
...
function IsFightPage() {
    ...
    return (!currentURL.endsWith("/gang/fight/") && currentURL.endsWith("/fight/"));
}
...
function IsOverviewPage() {
    ...
    return (currentURL.endsWith("/overview/") && !currentURL.endsWith("/fight/overview/"));
}
...
function IsFightOverviewPage() {
    ...
    return (currentURL.endsWith("/gang/fight/"));
}
...
function DispZeit(jetzt, cnt, url, Txt, back, id, x, y, z, maxTime, maxwidth) {
    ...
    ShowImg (back, url, get_icon("plunderb"), text, maxwidth, 5, x, y, z, id+'bg');
    if (width > 0)
        ShowImg (back, url, get_icon("clean"), text, width, 5, x, y, z+1, id+'fg');
...
function ShowMiniJob(contover) {
    ...
    if (timetext.indexOf('"rot"') == -1) {
        ...
        if (JobEndAlert == 1) {
            var fass = contover.split("Fassungsverm")[1].match(/[\d.,]+/);
            var maxmoney = Number(fass[0].replace(/[.,]/g,''));
            var mon = contover.split('class="icon money"')[1].split("</a>")[0];
            ...
            if (salary > maxmoney)
                if (IsTimeToCheck(TOWNEXTENSION + "LastJobWarn" + m_ownuserid, 10, "x"))
                    alert("Dein Verdienst ist höher als Dein Behälter aufnehmen kann. Bitte Verdienst abholen!!");
            else if (salary > maxmoney - 100000)
                if (IsTimeToCheck(TOWNEXTENSION + "LastJobWarn" + m_ownuserid, 10, "x"))
                    alert("Dein Verdienst erreicht demnächst das Fassungsvermögen Deines Behälters !!");
        }
    }
    else {
    ...
    GM_xmlhttpRequest({method:"GET", url: MINIJOBS_URL, onload:function(responseDetails) {
            ...
            for (var i = 0; i < tables.length; i++) {
                ...
                if (tds.length < 2)
                    continue;
                var img = tds[0].getElementsByTagName("img")[0].src;
                ShowImg('ADBACK', MINIJOBS_URL, img, titletext, 39, 39, 85, 11, 1, 'defweapon');
...
function GetAllValues(attvalue, defvalue) {
    ...
    GM_xmlhttpRequest({method:"GET", url: TOWNBASE_URL+'activities/', onload:function(responseDetails) {
                    ...
                    onload: function(responseDetails) {
                        ...
                        if (content.indexOf('name="xycoords"') != -1) {
                            if (content.indexOf('name="bottlecollect_pending"') == -1) {
                                ...
                                if (location.toString.endsWith("activities/bottle/"))
            ...
            if (content.indexOf('bottlecollect') != -1) {
                ShowImg ('SCBACK', '', get_icon("redbg"), '', '', '', 84, 13, 4, 'cartbg');
                var cart = ShowImg ('SCBACK', '', get_icon("cart"), getLangTxt(TxtCart), 25, 25, 90, 20, 5, 'cart');
                if (cart == null)
                    return;
                ...
                // eventuell gibt es noch einen weiteren Counter, der alle
                // anderen um 1 verschiebt (enemy_counter). Dieser wird, falls
                // vorhanden, aus der Liste entfernt, damit wieder alles stimmt
                var counters = content.split("counter(");
                var posci = content.indexOf('"enemy_counter_info"');
                if (posci != -1) {
                    var posci2 = posci;
                    posci = content.substr(posci+20,200).indexOf("counter");
                    if (posci != -1)
                        posci += posci2 + 20;
                }
                if (posci != -1) {
                    for (k=0; k< counters.length; k++)
                        if (counters[k].substr(0,2)==content.substr(posci+8,2)) {
                            counters.splice(k, 1);
                            break;
                        }
                }
                ...
                if (counter != 0) {        // Flaschen werden gesammelt
                    ...
                    ShowImg ('SCBACK', TOWNBASE_URL+'activities/', get_icon("nodefweapon"), '', '', '', 84, 13, 4, 'cartbg');
                    ShowImg ('SCBACK', TOWNBASE_URL+'activities/', get_icon("cart"), getLangTxt(TxtCart3) + " (" + FmtInfo(jetzt, end_time) + ")", 25, 25, 90, 20, 5, 'cart');
                } else {
                    ...
                    if (vcounter != 0) {   // Verbrechen wird geplant
                        ...
                        if (promille > 0) {
                            var href = EAT_STACK;
                            ShowImg ('SCBACK', href, get_icon("redbg"), '', (width==19?19:39), 39, 84 + (fcounter>0?20:0), 13, 4, 'cuffbg');
                            logoutWarn = 1;
                        } else {
                            var href = TOWNBASE_URL+'activities/';
                            ShowImg ('SCBACK', href, get_icon("nodefweapon"), '', (width==19?19:39), 39, 84 + (fcounter>0?20:0), 13, 4, 'cuffbg');
                        }
                        ShowImg ('SCBACK', href, get_icon("cuff"), getLangTxt(TxtCrime) + " (" + FmtInfo(jetzt, vend_time) + ")" + (promille?getLangTxt(TxtCrimePro):""), width, 25, 84 + (fcounter>0?20:6), 20, 5, 'cuff');
                    }
                    if (fcounter > 0) {    // Kampf läuft
                        var fend_time = jetzt + Number(fcounter);
                        ShowImg ('SCBACK', FIGHT_URL, get_icon("nodefweapon"), '', (width==19?19:39), 39, 84, 13, 4, 'fghtbg');
                        ...
                        if (attwarn > 0)
                            ShowImg ('SCBACK', FIGHT_URL, get_icon("redbg"), alerttext, (width==19?19:39), 39, 84, 13, 4, 'fghtbg');
                        else
                            ShowImg ('SCBACK', FIGHT_URL, get_icon("nodefweapon"), '', (width==19?19:39), 39, 84, 13, 4, 'fghtbg');

                        ShowImg ('SCBACK', FIGHT_URL, get_icon("attack"), text + alerttext, width, 25, 84 + (vcounter>0?0:6), 20, 5, 'fght');
                ...
                if (counter != 0 || vcounter != 0 || fcounter > 0) {
                    ...
                    var CntDwn = function() {
                        ...
                        if (counter != 0) {    // Flaschen werden gesammelt
                            if (jetzt <= end_time)
                                ...
                                again = false;
                                document.getElementById('cartbg').src = getIconAddr(get_icon("redbg"));
                    ...
                    ShowImg ('SCBACK', '', get_icon("grnbg"), '', '', '', 84, 13, 4, 'cartbg');
                    var cart = ShowImg ('SCBACK', '', get_icon("cart"), getLangTxt(TxtCart2), 25, 25, 90, 20, 5, 'cart');
                    if (cart == null)
                        return;
                    cart.style.cursor = 'pointer';
            ...
            if (location.toString().endsWith("activities/") || location.toString().endsWith("activities/bottle/"))
    ...
    GM_xmlhttpRequest({method:"GET", url: WB_URL, onload:function(responseDetails) {
            ...
            if (sk && skAnz == 3) {
                document.getElementById("ADBACKimg").src = getIconAddr(get_icon("attdefjobback")[nrOfTabs >= 6?0:1]);
                if (document.getElementById("defweaponlnk"))
                    document.getElementById("defweaponlnk").href = MINIJOBS_URL;
                else
                    ShowImg('ADBACK', MINIJOBS_URL, get_icon("nodefweapon"), '', '', '', 84, 13, 1, 'defweapon');
            }
            for (var i = 0; i < tables.length; i++) {
                var table = tables[i];
            ...
            if (actSkills == 0) {
                ShowImg ('SCBACK', WB_URL, get_icon("plunderb"), getLangTxt(TxtNoSkill), '', '', 6, (skAnz==4)?18:21, 4, 'skill1');
                ShowImg ('SCBACK', WB_URL, get_icon("plunderb"), getLangTxt(TxtNoSkill), '', '', 6, (skAnz==4)?26:31, 4, 'skill2');
            } else {
                var start_time = content.split("fortschritt()")[1].split('var start = ')[1].split(';')[0];
                var end_time = content.split("fortschritt()")[1].split('var end = ')[1].split(';')[0];
                ...
                ShowImg ('SCBACK', WB_URL, get_icon("plundera"), text, '', '', 6, (skAnz==4)?18:21, 3, 'skill1bg');
                if (width > 0)
                    ShowImg ('SCBACK', WB_URL, get_icon("clean"), text, width, 5, 6, (skAnz==4)?18:21, 4, 'skill1fg');
                if (actSkills > 1) {
                    ...
                    ShowImg ('SCBACK', WB_URL, get_icon("plundera"), text2, '', '', 6, (skAnz==4)?26:31, 3, 'skill2bg');
                } else {
                    ShowImg ('SCBACK', WB_URL, get_icon("plunderb"), getLangTxt(TxtNoSkill), '', '', 6, (skAnz==4)?26:31, 4, 'skill2');
                }
                var skillTim = function() {
                    ...
                    if (nwidth > 0)
                        if (width == 0)
                            ShowImg ('SCBACK', WB_URL, get_icon("clean"), text, nwidth, 5, 6, (skAnz==4)?18:21, 4, 'skill1fg');
                        else {
                    ...
                    if (end_time - jetzt < 3600 && actSkills == 1)
                        if (jetzt-Math.floor(jetzt/2)*2 != 0)
                            document.getElementById('skill1fg').src = getIconAddr(get_icon("plunderb"));
                        else
                            document.getElementById('skill1fg').src = getIconAddr(get_icon("clean"));
                }
                window.setTimeout(skillTim, 1000);
            }
            // **********************************************************************************
            ...
                            var newli = li[i].cloneNode(true);
                ...
                GM_xmlhttpRequest({method:"GET", url: WBPET_URL, onload:function(responseDetails) {
                        ...
                        if (content.indexOf(KEYWORD_MYBUM[lang]) != -1) {
                            ...
                            var actSkills = doc.getElementsByClassName('box_main_small').length;
                            if (actSkills == 0) {
                                ShowImg ('SCBACK', WBPET_URL, get_icon("plunderb"), getLangTxt(TxtNoSkill), '', '', 6, (skAnz==4)?34:41, 4, 'skillpet');
                            } else {
                                var start_time = content.split("fortschritt()")[1].match(/var start *= *\d+/)[0].match(/\d+/)[0];
                                var end_time = content.split("fortschritt()")[1].match(/var end *= *\d+/)[0].match(/\d+/)[0];
                                var jetzt = content.split('<input id="now_timestamp" type="hidden" value="')[1].split('">')[0];
                                var skilllvl = content.split('box_text_small">')[1];
                                var skill = skilllvl.split('">')[1].split('<')[0];
                                var level = skilllvl.split('[')[1].split(']')[0].split(' ')[1];
                                ...
                                var text = skill + ' ' + level + ', ' + percent + '%, ' + FmtInfo(jetzt, end_time);
                                var width = Math.ceil(25/100*percent);
                                ShowImg ('SCBACK', WBPET_URL, get_icon("plundera"), text, '', '', 6, (skAnz==4)?34:41, 3, 'skillpetbg');
                                if (width > 0)
                                    ShowImg ('SCBACK', WBPET_URL, get_icon("clean"), text, width, 5, 6, (skAnz==4)?34:41, 4, 'skillpetfg');
                                var PetSkillTim = function() {
                                    ...
                                    if (nwidth > 0)
                                        if (width == 0)
                                            ShowImg ('SCBACK', WB_URL, get_icon("clean"), text, nwidth, 5, 6, (skAnz==4)?34:41, 4, 'skillpetfg');
                                        else {
                                    ...
                                    if (end_time - jetzt < 3600)
                                        if (jetzt-Math.floor(jetzt/2)*2 != 0)
                                            document.getElementById('skillpetfg').src = getIconAddr(get_icon("plunderb"));
                                        else
                                            document.getElementById('skillpetfg').src = getIconAddr(get_icon("clean"));
                                }
                            window.setTimeout(PetSkillTim, 1000);
                            }
                        }
                ...
                GM_xmlhttpRequest({method:"GET", url: PET_URL, onload:function(responseDetails) {
                        ...
                        if (content.indexOf(KEYWORD_MYBUM[lang]) != -1) {
                            ...
                            if (content.indexOf("pet_kader") == -1) {
                                ShowImg ('SCBACK', PET_URL, get_icon("plundera"), "Du hast noch kein Tier", '', '', 6, (skAnz==4)?42:41, 4, 'streunpet');
                            } else {
                               ...
                               if (backflg != 2) {
                                   ...
                                   ShowImg ('SCBACK', PET_URL, get_icon("plunderb"), text, '', '', 6, (skAnz==4)?42:41, 4, 'streunpet');
                               }
                            }
                        }
                    }
            ...
            if (!oldVersion)
                GM_xmlhttpRequest({method: 'GET', url: PLUNDERSHOP_URL, onload: function(responseDetails) {
                        function showPlunderShop(tdiv, i) {
                            ...
                            if (i < tdiv.length) {
                                ...
                                GM_xmlhttpRequest({method: 'GET', url: tda[ii].href, onload: function(responseDetails) {
                                        ...
                                        var curElem = ShowImg ('PSBACK', '', src, kauf+": "+plunder+" ("+kost+(padd==""?"":"; "+padd.substr(2,padd.length-3))+")", 24, 24/fkt, 1 + (i % 3)*28, 20 + Math.floor(i / 3)*12, 1, 'buy'+i);
                                        if (curElem != null) {
                                ...
                                var src = getIconAddr(get_icon("noplunderequip"));
                                ShowImg ('PSBACK', PLUNDERSHOP_URL, src, 'verkauft', 24, 24/fkt, 1 + (i % 3)*28, 20 + Math.floor(i / 3)*12, 1, 'pshop'+i);
                            }
                            return 0;
                        }
                    var content = responseDetails.responseText;
                    ...
                    if (content.indexOf(KEYWORD_MYBUM[lang]) != -1) {
                        ...
                        aP = Math.ceil((content.split("trhover").length-2)/3)*3;
                        if (anzPlnd < aP)
                            anzPlnd = aP;
                        ...
                        var pCnt = 0;
                        for (j = 0; j < anzPlnd; j++)
                            pCnt += showPlunderShop(tdiv, j);
                        var text = (pCnt == 0?"Plundershop":getLangTxt(TxtBuyAll));
                        src = getIconAddr(get_icon("plunderreset"));
                        //tdiv = doc.getElementById("refresh_plundershop").getElementsByClassName("refreshbutton")[0];
                        //var text = tdiv.value;
                        var curElem = ShowImg ('PSBACK', '', src, text, 24, 24, 85, 20, 1, 'pshop' + anzPlnd);
                        if (curElem == null)
                            return;
                        curElem.style.cursor = 'pointer';
                        // **********************************************************************************
                        ...
                        if (tdiv.length > 1) {
                            ...
                            DispZeit(jetzt, tdiv[tdiv.length-1], PLUNDERSHOP_URL, TxtWaitSort, "PSBACK", "pshop", 0, 47, 1, PSWaitTime, PSwidth);
                        }
                    }
                }
                });
                ...
                onload: function(responseDetails) {
                    ...
                    if (content.indexOf(KEYWORD_MYBUM[lang]) != -1) {
                        ...
                        if (table.length > 0) {
                            ...
                            var newlink = document.getElementById("SCBACK").parentNode.appendChild(document.createElement('a'));
                    ...
                    GM_xmlhttpRequest({method: 'GET', url: OVERVIEW_URL, onload: function(responseDetails) {
                        ...
                        if (content.indexOf(KEYWORD_MYBUM[lang]) != -1) {
                            ...
                            var today = new Date();
                            var taskDone = (content.indexOf('korkenhaken.png') != -1 || (content.indexOf('tagesaufgabe') != -1 && content.indexOf('Tagesaufgabe offen') == -1));
                            if ((content.indexOf('/daily/') == -1) && !taskDone)
                                ShowImg ('SCBACK', '', get_icon("wutinaktiv"), '', '', '', 49, 23, 4, 'taskdone');
                            else {
                                if (taskDone) {
                                    var href = REWARDS_URL;
                                    ShowImg ('SCBACK', href, get_icon("grnbg"), '', '', '', 42, 13, 4, 'taskdone');
                                    newlink.setAttribute('href', href);
                                } else {
                                    var href = DAILY_URL;
                                    ShowImg ('SCBACK', href, get_icon("redbg"), '', '', '', 42, 13, 4, 'taskdone');
                                    GM_xmlhttpRequest({
                                        method: 'GET',
                                        url: DAILY_URL,
                                        onload: function(responseDetails) {
                                                ...
                                                if (content.indexOf(KEYWORD_MYBUM[lang]) != -1) {
                                                    ...
                                                    if (lockDA != FormatDate(today)) {
                                                        document.getElementById('provocation_area').innerHTML += '<form name="prov_form"><div id="provocation_note"><a class="close" href="#" onclick="CloseProvocation();"><img id="prov_close" src="' + window.location.protocol + '//static.pennergame.de/img/pv4/premiummedia/img/premium/provocation/provocation_note_close.png" border="0"></a><div id="provocation_text" class="provocation_text"><div class="prov_links"></div></div></div></form>';
                                                        document.getElementById('provocation_text').innerHTML ='<img src="' + window.location.protocol + '//static.pennergame.de/img/pv4/daily_jobs/korken.png"><font style=\"color:yellow; font-size:120%;\"><b>' + getLangTxt(TxtDaily) + '</b></font><br><a href="' + href + '">'+ task +'</a><br><font style=\"color:green; font-size:120%;\"><b>' + getLangTxt(TxtCards) + cards + '</b></font>';
                            ...
                            if (!isNaN(clean)) {
                                ...
                                if (noWash) {
                                    ...
                                    if (clean < 80) {
                                        ...
                                        ShowImg ('PWBACK', '', get_icon("redbg"), '', 30, 30, 78, 13, 2, 'lifebg');
                                    }
                                ...
                                if (clean != 0)
                                    ShowImg ('PWBACK', url, get_icon("clean"), proz, width, 5, 78, 46, 4, 'cleanliness');
                                if (clean != 100)
                                    ShowImg ('PWBACK', url, get_icon("dirty"), proz, 30, 5, 78, 46, 3, 'dirtyness');
                            }
                            else {
...
function CheckFightValues(content) {
    ...
    if (ATTBoost > 0) {
        ...
        ShowImg ('ADBACK', FIGHT_URL, get_icon("grnbg"), titletext, 39, 40, 0, 12, 1, 'attboost');
        var newlink = document.getElementById("attboost").parentNode.appendChild(document.createElement('div'));
        newlink.id = "attboosth";
        newlink.align = "center";
        newlink.style = 'position:absolute; left:' + (AD_X+1) + 'px; top:' + (AD_Y+20) + 'px; z-index:'+(getZ("ADBACK")+5)+'; color:black; width:39px;font-size:20px;">';
        newlink.innerHTML = '<b>+' + ATTBoost + '</b>';
        newlink.title = titletext;
        cacheBgnds("ADBACK", 'attboosth');
    }
    ...
    if (DEFBoost > 0) {
        ...
        ShowImg ('ADBACK', FIGHT_URL, get_icon("grnbg"), titletext, 39, 40, 42, 12, 1, 'defboost');
        var newlink = document.getElementById("defboost").parentNode.appendChild(document.createElement('div'));
        newlink.id = "defboosth";
        newlink.align = "center";
        newlink.style = 'position:absolute; left:' + (AD_X+44) + 'px; top:' + (AD_Y+20) + 'px; z-index:'+(getZ("ADBACK")+5)+'; color:black; width:39px;font-size:20px;">';
        newlink.innerHTML = '<b>+' + DEFBoost + '</b>';
        newlink.title = titletext;
        cacheBgnds("ADBACK", 'defboosth');
    }
    ...
    if (injury < 0) {
        ...
        if (ATTBoost > 0) {
            document.getElementById('attboost').setAttribute('height', '19px');
            document.getElementById('attboosth').style.top = (AD_Y+11)+ 'px';
            var height = 20;
        }
        else
            var height = 40;
        ShowImg ('ADBACK', FIGHT_URL, get_icon("redbg"), titletext, 39, height, 0, (height==40?12:32), 1, 'injboost');
        var newlink = document.getElementById("injboost").parentNode.appendChild(document.createElement('div'));
        newlink.id = "injboosth";
        newlink.align = "center";
        newlink.style = 'position:absolute; left:' + (AD_X+1) + 'px; top:' + (AD_Y+(height==40?20:31)) + 'px; z-index:'+(getZ("ADBACK")+5)+'; color:black; width:39px;font-size:20px;">';
        newlink.innerHTML = '<b>' + injury + '</b>';
        newlink.title = titletext;
        cacheBgnds("ADBACK", 'injboosth');
    }
    ...
    if (injury > 0) {
        ...
        if (DEFBoost > 0) {
            document.getElementById('defboost').setAttribute('height', '19px');
            document.getElementById('defboosth').style.top = (AD_Y+11)+ 'px';
            var height = 20;
        }
        else
            var height = 40;
        ShowImg ('ADBACK', FIGHT_URL, get_icon("redbg"), titletext, 39, height, 42, (height==40?12:32), 1, 'injdboost');
        var newlink = document.getElementById("injdboost").parentNode.appendChild(document.createElement('div'));
        newlink.id = "injdboosth";
        newlink.align = "center";
        newlink.style = 'position:absolute; left:' + (AD_X+44) + 'px; top:' + (AD_Y+(height==40?20:31)) + 'px; z-index:'+(getZ("ADBACK")+5)+'; color:black; width:39px;font-size:20px;">';
        newlink.innerHTML = '<b>-' + injury + '</b>';
        newlink.title = titletext;
        cacheBgnds("ADBACK", 'injdboosth');
    }
            ...
            if (gangcontent.indexOf(KEYWORD_INGANG[lang]) == -1) {
                ...
                if (PGu_getValue("ShowMissionPanel", 1) == 0) {
                    ...
                    if (trs[2].getElementsByTagName("span").length > 0) {
                        ...
                        ShowImg ('MIBACK', goMission, get_icon("plunderb"), text, 40, 5, 1, 45, 2, 'missbg4');
                        if (width > 0)
                            ShowImg ('MIBACK', goMission, get_icon("clean"), text, width, 5, 1, 45, 4, 'missfg4');
                        if (counter >= 0) {
                    ...
                    GM_xmlhttpRequest({method: 'GET', url: MISSION_URL, onload: function(missresponseDetails) {
                        ...
                        if (missresponseDetails.status == 200 &&
                            ...
                            misscontent = missresponseDetails.responseText;
                            ...
                            for (i = 0; i < anzMiss; i++) {
                                ...
                                if (zeit > 0 && waitTime == 0) {
                                    ...
                                    var fehl = text0.split("mission_item_notready");
                                    if (fehl.length > 1) {
                                        if (fehl[1].indexOf("sprite_icons") != -1)
                                            text = "Geld" + (fehl.length > 2?" und Plunder":"")
                                        else
                                            text = "Plunder";
                                        text += " fehlt";
                                    }
                                ...
                                ShowImg ('MIBACK', goMission, get_icon("plunderb"), balkText, 40, 5, 44, y, 2, 'missbg'+(i+5));
                                if (width > 0) {
                ...
                if (oldVersion) {
                    ...
                    var attWut = parseInt(ATTtxt.substr(ATTpos));
...
function refreshPage() {
    ...
    if (page.endsWith("activities/bottle/"))
        page = page.substr(0, page.length-7);
    else if (page.endsWith("/buy/"))
        page = page.substr(0, page.length-4);
    else if (page.endsWith("/speed/"))
        page = page.substr(0, page.length-6);
    else if (page.endsWith("/success/"))
...
function ShowGangFightIcon(GangFightIcon, GangfightTitle) {
    ShowImg('GFBACK', '/gang/fight/', get_icon("gangfight")[GangFightIcon], GangfightTitle, ICON_WIDTH, ICON_WIDTH, 10, 15, 6, 'gangfight');
}
...
function ShowNewIcon() {
    ShowImg('GFBACK', '/gang/fight/', get_icon("new"), '', '', '', 30, 12, 7, 'gangfightnew');
}
...
function ShowFightIcon(FightIcon, FightTitle) {
    ...
    ShowImg('FBBACK', '/fight/overview/#form1', get_icon("fight")[FightIcon], tooltip, ICON_WIDTH, ICON_WIDTH, 10, 15, 2, 'fight');
    if (FightTitle != "") {
...
function ShowFightStateIcon(FightState, currentpower, maxpower) {
    switch (FightState) {
        case FIGHTSTATE_OK:
            ShowFightImg(get_icon("fight_ok"), printf(getLangTxt(TOOLTIP_FIGHTOK), currentpower), '', '', 90, 22, 2);
            break;
        case FIGHTSTATE_WEAK:
            ShowFightImg(get_icon("fight_weak"), printf(getLangTxt(TOOLTIP_FIGHTWEAK), currentpower, maxpower), '', '', 90, 22, 2);
            break;
        case FIGHTSTATE_DANG:
            ShowFightImg(get_icon("fight_dang"), printf(getLangTxt(TOOLTIP_FIGHTDANG), currentpower, maxpower), 25, 25, 90, 22, 2);
            break;
        case FIGHTSTATE_ERROR:
            ShowImg('PBBACK', '', get_icon("error"), getLangTxt(TOOLTIP_LOADERROR), '', '', 90, 22, 2, 'fightstateerror');
            break;
        default:
            ShowImg('PBBACK', '/gang/upgrades/', get_icon("error"), 'Unbekannter Zustand!', '', '', 90, 22, 2, 'resetfight');
            break;
    }
}
...
function ShowBackgrounds(version, gangid) {
    if (gangid > 0) {
        ShowImg('', '', get_icon("gangfightback")[version], '', '', '', GF_X, GF_Y, '', 'GFBACK');
        if (lang == 0)
            ShowImg('', '', get_icon("missionback")[version], '', '', '', MI_X, MI_Y, '', 'MIBACK');
    }
    else if (gangid < 0) {
        ShowImg('', '', get_icon("fightback")[version], '', '', '', FB_X, FB_Y, '', 'FBBACK');

        ShowImg('', '', get_icon("plunderdirectback")[version], '', '', '', PD_X, PD_Y, '', 'PDBACK');
        if (!oldVersion)
            ShowImg('', '', get_icon("plundershopback")[version], '', '', '', PS_X, PS_Y, '', 'PSBACK');
        if (noWash)
            ShowImg('', '', get_icon("promilleback2")[version], '', '', '', PW_X, PW_Y, '', 'PWBACK');
        else
            ShowImg('', '', get_icon("promilleback")[version], '', '', '', PW_X, PW_Y, '', 'PWBACK');
        ShowImg('', '', get_icon("attdefwpnback")[version], '', '', '', AD_X, AD_Y, '', 'ADBACK');
        ShowImg('', '', get_icon("sklcrdcrtback")[version], '', '', '', SC_X, SC_Y, '', 'SCBACK');
        if (lockmove)
            ShowImg ('PDBACK', '', get_icon("lock"), getLangTxt(TOOLTIP_LOCK), 10, 10, 240, 1, 1, 'lockmove');
        var langSel = ShowImg('PDBACK', '', getLangTxt(get_icon("flags")), getLangTxt(TxtLang), 16, 11, 2, 1, 2, 'langSelect');
        langSel.addEventListener("click", function(event) {
            myLang++;
            if (myLang == get_icon("flags").length)
               myLang = 0;
            this.src = getIconAddr(getLangTxt(get_icon("flags")));
...
function ShowCurrentPlunder(plunderimg, plundername, big) {
    ...
    if (plunderimg == "none") {
        plunderimg = get_icon("noplunderequip");
...
function ShowWutIcon(WutState) {
    ...
    switch (WutState) {
        case WUTSTATE_ACTIVE:
            ...
            if (wuttd == null)
                wuttd = ShowImg('PBBACK', '/gang/upgrades/', get_icon("wutaktiv"), '', '', '', 49, 23, 11, 'wutstate');
            break;
        case WUTSTATE_INACTIVE:
            if (WutDatum != "") {
                wuttext = getLangTxt(TxtWiWut[WiWut-1]) + getLangTxt(TOOLTIP_WIWUTAKTIV) + WutDatum;
                if (wuttd == null) {
                    wuttd = ShowImg('PBBACK', '', get_icon("wutaktiv"), "", '', '', 49, 23, 11, 'wutstate2');
                    ShowImg('PBBACK', '/gang/upgrades/', get_icon("wutinaktiv"), '', '', '', 49, 23, 11, 'wutstate');
                }
            }
                ...
                if (WiWut != 0) {
                    ...
                    if (wuttd == null)
                        wuttd = ShowImg('PBBACK', '/gang/upgrades/', get_icon("wutinaktiv"), '', '', '', 49, 23, 11, 'wutstate');
        }
        else {
                    if (wuttd == null)
                        wuttd = ShowImg('PBBACK', '/gang/', get_icon("wutaktiv"), '', '', '', 49, 23, 11, 'wutstate');
        }
            }
            else {
                if (wuttd == null)
                    wuttd = ShowImg('PBBACK', '/gang/', get_icon("wutinaktiv"), '', '', '', 49, 23, 11, 'wutstate');
                wuttext = getLangTxt(KEYWORD_INGANG) + "!!";
            }
            break;
        case WUTSTATE_ERROR:
            ShowImg('PBBACK', '/gang/upgrades/', get_icon("error"), getLangTxt(TOOLTIP_LOADERROR), '', '', 49, 23, 11, 'wutstate');
            break;
    }
...
function GetCurrentPlunder(content) {
    ...
    if (oldVersion)
        ...
        var korr = (plunderimg == get_icon("noplunderequip"))?1:0;
        var nrOfPlnd = plunderbox.innerHTML.split('id="favorite"')[0].split('<img').length - 1;
        for (var i = 0; i < nrOfPlnd; i++) {
            if (i != 0) {
            ...
            var tab = plunderbox.innerHTML.split("<strong>")[i+1].split("<")[0];
...
function getOwnUserID() {
    try {
        ...
        if (!myprof)
            myprof = document.getElementById("my-profile");
        var ownuserid = myprof.innerHTML.split('href="/profil/id:')[1];
    ...
    try {
        ...
        if (psplit.length == 1) {
            ...
            for (i = 1; i < psplit.length; i++)
                if (psplit[i].search('> *' + plundername.replace("(", "\\(").replace(")", "\\)").replace("+", "\\+") + ' *<') != -1) {
...
function isMultiUsablePlunder(plunder) {
    var MultiUsablePlunder = new Array ('flaschenlaster', 'geldbeutel', 'buch', 'seife', 'tageskarte', 'haustiertrick',  'gelbbeutel', 'boost_3000_pfandflaschen', 'hexenb', 'skiequ', 'schlitten2', 'plunder_zahn', 'schippchen', 'boost_zehn_euro', 'prov_kugel', 'prov_osterei', 'prov_wollhandschuhe');
...
function useMorePlunder(imgtitle, action, phref, mtimes, times, player, count, consume) {
    if (mtimes == 1) {
        GM_xmlhttpRequest({method:"GET", url: PLUNDER_URL + (oldVersion ? "" : "ajax/?"+ajaxcode+"=" + (nrOfTabs >= 6?4:(nrOfTabs==2?1:3))), onload:function(responseDetails) {
            var content = responseDetails.responseText;
...
function usePlunder(imgtitle, action, phref, mtimes, times, player, count, consume) {
    ...
    if (oldVersion)
        ...
        params = "plunder="+action+"&plunder_count="+(Number(count)-(Number(times)-Number(mtimes)));
    }
    else {
    ...
    // **********************************************************************************
    GM_log("Plunder wird benutzt: " + PLUNDERUSE_URL + params);
                ...
                onload: function(responseDetails) {
                    ...
                    if (!consume && responseDetails.responseText.indexOf(KEYWORD_MYBUM[lang]) != -1) {
                        ...
                        if (responseDetails.responseText.indexOf(href) != -1) {
                            //PGu_setValue("plunderchange"+imgtitle, "52:Jetpack");
                            //var pch = PGu_getValue("plunderchange"+imgtitle, "");
                            //if (pch != "") {
                            //    var choice = pch.split(":")[0];
                                PLUNDERUSE_URL = TOWNBASE_URL + href.substr(1);
                                GM_xmlhttpRequest({method: 'POST', url: PLUNDERUSE_URL, headers: {'Content-type': 'application/x-www-form-urlencoded'},
                                data: encodeURI("pid="+action),
                            //    data: encodeURI("pid="+action+"&choice="+choice),
...
function AddPlunderIcon(content, plunder, imgtitle, usemode, slotnr) {
    ...
    if (slotnr > 1 && slotnr % MAXNROFSLOTS == 0 && !document.getElementById('PDBACK'+row)) {
        ShowImg('PDBACK', '', get_icon("plunderdirectback")[bgrndInd], '', '', '', 0, row*41, -2*row, 'PDBACK'+row);
    }
    row--;
    ...
    if (slotnr  > 0) {
        ...
        if (usemode == "A") {
            ...
            var usemodeimg = get_icon("plundera");
        ...
        // sonst: Der Plunder soll benutzt werden
            ...
            var usemodeimg = get_icon("plunderb");
...
function AddResetIcon(plundertable) {
    ...
    newimg.setAttribute('src', getIconAddr(get_icon("plunderreset")));
    newimg.setAttribute('border', '0');
...
function getPicAddr(image) {
    ...
    return window.location.protocol + PLUNDERIMAGE_URL[indx] + imgSplit[0];
}
...
function ShowAllPlunder(content, NrOfPlunder, step, marker) {
    if (m_ownuserid != getOwnUserID())
        return;
    var ajax = content.match(/ajax\/\?[^=]*=/g).pop();
    // Wenn die Seite abgerufen werden konnte (kein Seitenladefehler)
    if (content.indexOf(KEYWORD_MYBUM[lang]) != -1) {
        if (step > 0 && collAllPlunder != marker)
            return;
        if (step > 0 && step <= nrOfTabs) {
            GM_xmlhttpRequest({method:"GET", url: PLUNDER_URL+ajax+step, onload:function(responseDetails) {
                if (PGu_getValue("callCount") != callCount)
                    return;
            ...
            if (step == 0 && !oldVersion) {
                collAllPlunder++;
                ShowAllPlunder (ajax+":"+KEYWORD_MYBUM[lang] + KEYWORD_JUNK[lang], NrOfPlunder, step + 1, collAllPlunder);
                return;
            }
        }
        ...
        // Wenn weniger als 9 Plunder eingetragen sind
            // +-Icon anzeigen
            ShowImg('PDBACK', PLUNDER_URL, get_icon("plunderfree"), getLangTxt(TOOLTIP_PLUNDERAUSWAHL), '', '', 1 + (NrOfPlunder % MAXNROFSLOTS) * 28, Math.floor(NrOfPlunder/MAXNROFSLOTS)*41 + 18, 1, 'AddPlunderIcon');
    }
    else
        ShowAllPlunder (content, NrOfPlunder, step, marker);
}
...
function GetPromille(doc) {
    ...
    promille = promille.innerHTML.match(/\d+/g);

    return Number(promille[0]) + Number(promille[1]) / 100;
}
...
function money_format(zahl, tr) {
    ...
    if (currency1 == "")
        return currency + tr + new_string.reverse().join(getLangTxt(TZ)) + getLangTxt(DZ) + nachkomma;
    else
    ...
    currentelem.addEventListener('click', function(event) {
        ...
        function DrinkBeer(content, nrofbeers) {
            ...
            // ***********************************************************************************************
                ...
                if (confirm(printf(getLangTxt(TxtNotMuchToDrink), drink)))
                    window.setTimeout("window.location.href = window.location.protocol + '//' + window.location.host + '/city/supermarket/'", 2000);
            }
        }
        ...
        function alcoPlunderAnlegen(content1, promille, restore) {
            function collectAlcoPlunder(tabNr) {
                // **********************************************************************************
                // *** GM_XMLHTTPREQUEST *** Abrufen der Plunder-Seite
                // **********************************************************************************
                GM_xmlhttpRequest({method:"GET", url: PLUNDER_URL + (oldVersion ? "" : "ajax/?"+ajaxcode+"=" + tabNr), onload:function(responseDetails) {
                    var content = responseDetails.responseText;
                    if (tabNr != 4 && restore && restPlunder == "") {
                        // Aktuellen Plunder ermitteln und anzeigen
                        if (oldVersion)
                    ...
                    if (doc.getElementsByTagName("table").length > 0) {
                        ...
                        if (tabNr == 3 && alcocnt < alcoplunder.length) {
                            collectAlcoPlunder(1);
                            return;
                        }
                    ...
                    if (tabNr == 4 && alcpl >= 0) {
                        ...
                        if (href.substr(0,4) == "java") {
                            href = content.substr(tdpm).split('<form')[1].split('action="')[1].split('"')[0];
                            var inputs = content.substr(tdpm).split('/form')[0].split('<input');
                            var data = "";
                            for (ii = 1; ii < inputs.length; ii++) {
                                var name = inputs[ii].split('name="')[1].split('"')[0];
                                var val  = inputs[ii].split('value="')[1].split('"')[0];
                                data += "&" + name + "=" + val;
                            }
                            var data = data.substr(1);
                        }
                        else
                            var data = "plunder="+ActionNr;
                            ...
                            onload: function(responseDetails) {
                                if (restPlunder != "none" && restPlunder != "" && restPlunder != GetNameFromPlunderInfo(plunderinfo))
                                {
                                    ...
                                    if (restActionNr == "")     // Plunder kann nicht wiederangelegt werden !!
                                        ...
                                        // **********************************************************************************
                                        // *** GM_XMLHTTPREQUEST *** POSTEN des Plunderwechsels für den ursprünglichen Plunder
                                        // **********************************************************************************
                                        GM_xmlhttpRequest({method: 'POST', url: PLUNDERCHANGE_URL, headers: {'Content-type': 'application/x-www-form-urlencoded'},
    ...
    currentelem.addEventListener('click', function(event) {
        ...
        GM_xmlhttpRequest({method: 'GET', url: EAT_STACK, onload: function(responseDetails) {
            ...
            // ***********************************************************************************************
                ...
                CursorWait(currentelem, false);
                // Benutzer benachrichtigen
                if (confirm(printf(getLangTxt(TxtNotMuchToEat), convertUni(FoodNames[1]), convertUni(FoodNames[4]))))
                    window.setTimeout("window.location.href = window.location.protocol + '//' + window.location.host + '/city/supermarket/food/'", 2000);
            }
        } });
    },false);
}
...
function GetMoney(doc) {
    var money = doc.getElementsByClassName("icon money")[0].getElementsByTagName("a")[0].innerHTML.trim();
    if (currency1 != "") {
        money = money.replace(currency1, '.');
        if (money.substr(money.length-2,1) == ".")
            money = money.insert("0", -1);
    }
    else if (money.substr(money.length-3, 1) == '.') {
        DZ[lang] = '.';
        TZ[lang] = ',';
    }
    money = money.match(/[\d,.]+/)[0].replace(/[,.]/g, "") / 100;
    return money;
}
...
// **********************************************************************************
// **********************************************************************************
function CureMe(currentelem, cnt, pts, first) {
    ...
    GM_xmlhttpRequest({method: 'POST', url: CURE_URL, headers: {'Content-type': 'application/x-www-form-urlencoded'},
        ...
        onload: function(respDetails) {
            GM_xmlhttpRequest({method:"GET", url: OVERVIEW_URL, onload:function(responseDetails) {
                var lifepts = getCleanliness(responseDetails.responseText);
                if (pts == lifepts)
                    alert(printf(getLangTxt(TxtLifepts3), (first?"":"weiter ")));
                if (cnt > 1 && pts != lifepts)
                    CureMe(currentelem, cnt-1, lifepts, false);
                else {
                    // Keinen Wartecursor anzeigen
                    CursorWait(currentelem, false);
            ...
            GM_xmlhttpRequest({method:"GET", url: OVERVIEW_URL, onload:function(responseDetails) {
                ...
                if (lifepts == 100) {
                    ...
                    if (currentmoney >= cost) {
                        CureMe(currentelem, cnt, lifepts, true);
                    // ***********************************************************************************************
...
function FeedbackHandler(bgrnd) {
    ...
    if (bgrnd == 'PW' && EatDrinkFeedBack == 0 || bgrnd == 'SC' && CollEndAlert == 0 || bgrnd == 'PB' && FightWeakAlert == 0 || bgrnd == "MI" && MissionAlert == 0 || bgrnd == "AD" && JobEndAlert == 0 || bgrnd == "PD" && NewPlunderAlert == 0)
        var curElem = ShowImg(bgrnd + 'BACK', '', get_icon("noinfo"), getLangTxt(TxtNoInfo), 10, 10, (bgrnd == "MI"?31:(bgrnd == 'PD'?241:(bgrnd == 'PW'?102:112))), 13, 8, bgrnd + 'Info');
    else
        var curElem = ShowImg(bgrnd + 'BACK', '', get_icon("info"), getLangTxt(TxtInfo), 10, 10, (bgrnd == "MI"?31:(bgrnd == 'PW'?102:112)), 13, 8, bgrnd + 'Info');
    ...
    curElem.addEventListener('click', function(event) {
        if (bgrnd == 'PW') {
            ...
            var val = JobEndAlert = 1 - JobEndAlert;
            PGu_setValue("JobEndAlert", JobEndAlert);
        }
        else if (bgrnd == 'PD') {
            var val = NewPlunderAlert = 1 - NewPlunderAlert;
            PGu_setValue("NewPlunderAlert", NewPlunderAlert);
        }
        else {
        ...
        if (val == 0) {
            this.src = getIconAddr(get_icon("noinfo"));
            this.title = getLangTxt(TxtNoInfo);
        } else {
            this.src = getIconAddr(get_icon("info"));
    ...
    function checkOneEquip(code) {
        ...
        GM_xmlhttpRequest({method:"GET", url: urls[code], onload:function(responseDetails) {
                ...
                for (var i = 0; i < tables.length; i++) {
                    ...
                    if (trs.length < 2)
                        continue;
                    if (table.innerHTML.indexOf("enötigt") != -1)
                        continue;
                    ...
                    var trn = code == 1 && (TOWNEXTENSION == "VT" || TOWNEXTENSION == "AT") ? 2 : 3;
                    var gekauftw = (trs[trn-1].innerHTML.indexOf(code==0&&(TOWNEXTENSION=="VT"||TOWNEXTENSION=="AT")?TxtBoughtMulti:TxtBought[lang]) != -1);
                    var benutzt = (trs[trn].innerHTML.indexOf(TxtSelected[lang]) != -1);
                    ...
                    if (code == 1 && benutzt && weapon[0] == 0) {
                        weapondef = trs[trn].getElementsByTagName("td")[0].innerHTML.split('value="')[1].split('"')[0];
                    ...
                    else if (attdef > maxval && attdef > betterval) {
                        betterequip = equip;
                        betterval = attdef;
                        owned = gekauft;
                    }
...
function AutoDefWeapon() {
    ...
    GM_xmlhttpRequest({method:"GET", url: WEAPONSTOREDEF_URL, onload:function(responseDetails) {
            ...
            if (!gekauft) {
                ShowImg('ADBACK', WEAPONSTOREDEF_URL, get_icon("nodefweapon"), '', '', '', 84, 13, 1, 'defweapon');
...
var JobEndAlert = PGu_getValue("JobEndAlert", 0);
var NewPlunderAlert = PGu_getValue("NewPlunderAlert", 0);
var attwarn = 0;
...
function doCraft(event) {
    var aktPlunder = "";
    var unarm;
    function CraftPlunder(craftForm, neededPlunder, i, pAnz) {
        for ( ; i < neededPlunder.length; i++) {
            var pName = neededPlunder[i].split("</strong>")[0].split(">").pop().trim();
            if (pName == aktPlunder) {
                aktPlunder = "";
                var url = unarm.split("?");
                GM_xmlhttpRequest({method: 'POST', url: url[0],
                    headers: {'Content-type': 'application/x-www-form-urlencoded'},
                    data: encodeURI(url[1]),
                    onload: function(responseDetails) {
                        CraftPlunder(craftForm, neededPlunder, i, pAnz);
                    }
                });
                return;
            }
            var menge = neededPlunder[i].split("</strong>")[1].split("<br")[1];
            var pSoll = Number(menge.split("<")[0].split("x")[1].trim()) * pAnz;
            var pIst = Number(menge.split("[")[1].split("]")[0]);
            if (pIst >= pSoll || neededPlunder[i].indexOf("Bastelbar") == -1)
                continue;
            stack.push(pAnz);
            stack.push(i+1);
            stack.push(neededPlunder);
            stack.push(craftForm);
            var href = window.location.protocol + '//' + window.location.host + neededPlunder[i].split('href="')[1].split('"')[0];
            GM_xmlhttpRequest({method:"GET", url: href, onload:function(responseDetails) {
                var table = responseDetails.responseText.split('id="content"')[1].split("<table ")[1];
                var inputs = responseDetails.responseText.split('id="craftform"')[1].split("<input ");
                var data = new Array();
                for (var k = 1; k < inputs.length; k++) {
                    var name = inputs[k].split('name="')[1].split('"')[0];
                    var value = inputs[k].split('value="')[1].split('"')[0];
                    data.push(name + "=" + value);
                }
                var craftForm = window.location.protocol + '//' + window.location.host + responseDetails.responseText.split('id="craftform"')[1].split('action="')[1].split('"')[0] + "?" + data.join("&");
                neededPlunder = table.split("<tr")[2].split("<td ")[1].split('<a ');
                CraftPlunder(craftForm, neededPlunder, 1, pSoll - pIst);
            }});
            return;
        }
        var url = craftForm.split("?");
        GM_xmlhttpRequest({method: 'POST', url: url[0],
            headers: {'Content-type': 'application/x-www-form-urlencoded'},
            data: encodeURI(url[1]),
            onload: function(responseDetails) {
                if (pAnz > 1) {
                    CraftPlunder(craftForm, neededPlunder, i, pAnz - 1);
                    return;
                }
                if (stack.length > 0) {
                    CraftPlunder(stack.pop(), stack.pop(), stack.pop(), stack.pop());
                    return;
                }
    ...
    document.getElementById("craft_image").removeEventListener('click', doCraft);
    document.getElementById("craft_image").style.visibility = "hidden";
    document.getElementById("craft_image").style.height = "1px";
    var div = document.createElement("div");
    div.innerHTML = '<img src="' + document.getElementById("craft_image").src.replace(/craft_do.*/, "craft_progress.gif") + '"><br><span style="font-family:\'CrackhousefranceRegular\'; font-size: 16px;">Bauen...</span>"';
    document.getElementById("craft_image").parentNode.parentNode.appendChild(div);
    var table = document.getElementsByTagName("table")[0].innerHTML;
    var inputs = document.getElementById("craftform").getElementsByTagName("input");
    var data = new Array();
    for (var k = 0; k < inputs.length; k++)
        data.push(inputs[k].name + "=" + inputs[k].value);
    var craftForm = document.getElementById("craftform").action + "?" + data.join("&");
    var neededPlunder = table.split("<tr")[2].split("<td ")[1].split('<a ');
    var stack = new Array();
    GM_xmlhttpRequest({method:"GET", url: window.location.protocol + '//' + window.location.hostname + '/overview/', onload:function(responseDetails) {
        var content = responseDetails.responseText;
        var pos = content.search("Angelegte Plunder");
        if (pos <= 0)
            return;
        var table = content.substr(pos).split("<table")[1].split("</table>")[0];
        var inputs = table.split("<form")[1].split("<input ");
        var data = new Array();
        for (var k = 1; k < inputs.length; k++) {
            var name = inputs[k].split('name="')[1].split('"')[0];
            if (inputs[k].indexOf('value="') != -1)
                var value = inputs[k].split('value="')[1].split('"')[0];
            else
                var value = 0;
            data.push(name + "=" + value);
        }
        unarm = window.location.protocol + '//' + window.location.host + inputs[0].split('action="')[1].split('"')[0] + "?" + data.join("&");
        var tabs = table.split("<tr")[2].split("<td");
        if (tabs[1].indexOf('<strong ') != -1)
            aktPlunder = tabs[1].split('<strong ')[1].split('</strong>')[0].split(">")[1].trim();
        CraftPlunder(craftForm, neededPlunder, 1, 1);
    }});
}

function changeCraft(reqSkill) {
    if (DEXSkill < 0) {
        window.setTimeout(changeCraft, 500, reqSkill);
        return;
    }
    if (reqSkill <= DEXSkill) {
        document.getElementById("craft_image").src = document.getElementById("craft_image").src.replace("unavailable", "do");
        document.getElementById("craft_image").addEventListener('click', doCraft, false);
    }
}

function CheckCraftDetails() {
    function CheckPlunderToCraft(neededPlunder, i, pAnz) {
        if (i >= neededPlunder.length) {
            if (location.toString() != aktcraft)
                return;
            if (stack.length > 0) {
                CheckPlunderToCraft(stack.pop(), stack.pop(), stack.pop());
                return;
            }
            var plunderdiv = document.getElementsByTagName("table")[0].getElementsByTagName("tr")[1].getElementsByTagName("td")[1].getElementsByTagName("strong")[0].parentNode;
            var plunder = plunderdiv.innerHTML.split("<br>");
            if (missingPlunder.length == 0) {
                pMax++;
                CheckPlunderToCraft(neededPlunder, 1, 1);
                return;
            }
            else if (pMax == 0){
                plunder.push("<br><span>Dir fehlt noch folgender Plunder:</span>");
                for (var k = 0; k < missingPlunder.length; k++)
                    plunder.push("<span>" + missingPlunder[k] + " x " + missingPlunderAnz[k] + "</span>");
            }
            else {
                if (pMax == 1)
                    plunder.push("<br><span>Du hast alle Plunderstücke</span>");
                else
                    plunder.push("<br><span>Du kannst den Plunder " + pMax + "x basteln</span>");
                window.setTimeout(changeCraft, 500, reqSkill);
            }
            plunderdiv.innerHTML = plunder.join("<br>");
            plunderdiv.parentNode.style.height = parseInt(plunderdiv.parentNode.style.height) + (plunder.length-2)*16 + "px";
            plunderdiv.parentNode.parentNode.getElementsByTagName("div")[0].style.marginTop = (plunder.length-2)*8 + "px";
            return;
        }
        var pName = neededPlunder[i].split("</strong>")[0].split(">").pop().trim();
        var menge = neededPlunder[i].split("</strong>")[1].split("<br")[1];
        var pSoll = Number(menge.split("<")[0].split("x")[1].trim()) * pAnz;
        var pIst = Number(menge.split("[")[1].split("]")[0]);
        var index = existingPlunder.indexOf(pName)
        if (index == -1 && pIst > 0) {
            existingPlunder.push(pName);
            existingPlunderAnz.push(pIst);
            index = existingPlunder.length - 1;
        }
        else if (index >= 0)
            pIst = existingPlunderAnz[index];
        if (pIst >= pSoll) {
            existingPlunderAnz[index] -= pSoll;
            CheckPlunderToCraft(neededPlunder, i + 1, pAnz);
            return;
        }
        if (neededPlunder[i].indexOf("Bastelbar") == -1) {
            var index = missingPlunder.indexOf(pName)
            if (index == -1) {
                missingPlunder.push(pName);
                missingPlunderAnz.push(pSoll - pIst);
            }
            else
                missingPlunderAnz[index] += pSoll - pIst;
            CheckPlunderToCraft(neededPlunder, i + 1, pAnz);
            return;
        }
        ...
        GM_xmlhttpRequest({method:"GET", url: href, onload:function(responseDetails) {
            var table = responseDetails.responseText.split('id="content"')[1].split("<table ")[1];
            neededPlunder = table.split("<tr")[2].split("<td ")[1].split('<a ');
            existingPlunderAnz[index] = 0;
            CheckPlunderToCraft(neededPlunder, 1, pSoll - pIst);
        }});
    }

    var aktcraft = location.toString();
    if (aktcraft.indexOf("/craft/details/") == -1)
        return;

    var plunderdiv = document.getElementsByTagName("table")[0].getElementsByTagName("tr")[1].getElementsByTagName("td")[1].getElementsByTagName("strong")[0].parentNode;
    if (plunderdiv.innerHTML.indexOf("Bastelbar") == -1 || plunderdiv.parentNode.innerHTML.indexOf("craft_do") != -1)
        return;

    var reqSkill = Number(plunderdiv.innerHTML.split("Bastelbar")[1].split("<")[0].trim().split(" ").pop());
    var missingPlunder = new Array();
    var missingPlunderAnz = new Array();
    var existingPlunder = new Array();
    var existingPlunderAnz = new Array();
    var stack = new Array();
    var table = document.getElementsByTagName("table")[0].innerHTML;
    var neededPlunder = table.split("<tr")[2].split("<td ")[1].split('<a ');
    var pMax = 0;
...
function insertPlunderIntoDistrict() {
    if (misscontent == "") {
        window.setTimeout(insertPlunderIntoDistrict, 1000);
        return;
    }
    var mplunder = new Array();
    var mpdist = new Array();
    var missions = misscontent.split('class="gang_mission"');
            ...
            if (beg.length > 1) {
                ...
                if (list != null) {
                    if (list[0].indexOf("deactivate") != -1) {
                        ...
                        for (var k = 1; k < dlist.length; k++) {
                            var dist = dlist[k].split("<")[0].trim();
                            if (mpdist.indexOf(dist) == -1) {
                                mpdist.push(dist);
                                mplunder.push(new Array(plnd));
                            }
                            else
                                mplunder[mpdist.indexOf(dist)].push(plnd);
                        }
                    }
                }
            }
    ...
    var trs = table.getElementsByTagName("tr");
    var td = trs[0].getElementsByTagName("td")[4].cloneNode(true);
    td.getElementsByTagName("span")[0].innerHTML = "Plunder";
    td.style.width = "150px";
    trs[0].appendChild(td);
    for (var i = 1; i < trs.length; i++) {
        var td = trs[i].getElementsByTagName("td")[0].cloneNode(true);
        var dist = td.getElementsByTagName("span")[0].innerHTML.trim();
        td.getElementsByTagName("span")[0].style.paddingLeft = "";
        if (mpdist.indexOf(dist) != -1)
            td.getElementsByTagName("span")[0].innerHTML = mplunder[mpdist.indexOf(dist)].join("<br>");
        else
...
function getPlunderPage(callfkt) {
    if (callfkt != undefined) {
        if (plundercontent == "")
            setTimeout(getPlunderPage, 500, callfkt);
        else
            callfkt();
        return;
    }
    ...
    GM_xmlhttpRequest({method:"GET", url: PLUNDER_URL, onload:function(responseDetails) {
        ...
        if (responseDetails.responseText.indexOf('id="plunderlist"') != -1) {
            plundercontent = responseDetails.responseText;
            return;
        }
        setTimeout(getPlunderPage, 500);
    }});
}

function getGangUpgr(callfkt) {
    if (callfkt != undefined) {
        if (gangupgcontent == "")
            setTimeout(getGangUpgr, 500, callfkt);
        else
            callfkt();
        return;
    }
    ...
    GM_xmlhttpRequest({method:"GET", url: GANGUPGRADE_URL, onload:function(responseDetails) {
        ...
        if (responseDetails.responseText.indexOf(KEYWORD_MYBUM[lang]) != -1) {
            // Content der Bandenupgradeseite speichern
            gangupgcontent = responseDetails.responseText;
            return;
        }
        setTimeout(getGangUpgr, 500);
    }});
}

function checksum(s, l)
{
  var chk = 0;
  for (var i = 0; i < l; i++) {
...
var doTheAction = function () {
    ...
    GM_registerMenuCommand(THISSCRIPTNAME[myLang] + ": auf Update prüfen", function (event) {
		GM_setValue("iconVers", "");
		location.href = OVERVIEW_URL;
    });
    GM_registerMenuCommand(THISSCRIPTNAME[myLang] + ": alle Einstellungen exportieren", function (event) {
        ...
        for (var i = 0; i < gm_vars.length; i++) {
            if (gm_vars.indexOf(gm_vars[i]) < i)
                continue;
            if (gm_vars[i].indexOf("winnings") != -1 || gm_vars[i] == "") {
                GM_deleteValue(gm_vars[i]);
                deleted++;
                continue;
            }
            var val = GM_getValue(gm_vars[i], "xyzundefinedzyx");
            if (val == "xyzundefinedzyx")
                continue;
            if (gm_vars[i].length > 50)
                if (confirm("Variable löschen: " + gm_vars[i])) {
                    GM_deleteValue(gm_vars[i]);
                    deleted++;
                    continue;
                }
        ...
        if (i < gm_vars.length)
            ...
            vars += "$" + checksum(vars, vars.length);
    ...
    GM_registerMenuCommand(THISSCRIPTNAME[myLang] + ": alle Einstellungen importieren", function (event) {
        var vars = prompt("Eingabe: ");
        var i = vars.lastIndexOf("$");
        var gmvars = vars.split("$");
        var chksum = gmvars.pop();
        if (chksum != checksum(vars, i)) {
    ...
    for (var i = ads.length - 1; i >= 0; i--) {
        if (ads[i].src.indexOf('XmasAdventCalendar') != -1 || ads[i].src.indexOf('adventskalender') != -1) {
            ads[i].addEventListener('click', function(event) {
                if (event.shiftKey != 0) {
                    if (PGu_getValue("noXmasInfo", 0) != 0) {
                        PGu_setValue("noXmasInfo", 0);
                    }
                    else {
                        var now = new Date();
                        PGu_setValue("noXmasInfo", (Number(now)+24*86400000).toString());
                    }
                }
            }, false);
            xmaskal = true;
    ...
    if (!xmaskal)
        xmaskal = (now.getMonth() == 11 && now.getDate() <= 24);

    if (xmaskal && PGu_getValue("noXmasInfo", 0) <= Number(now)) {
        // **********************************************************************************
            ...
            if (content.indexOf('/event/xmas/get/') != -1) {
                ...
                for (var j = 0; j < xmasget.length; j++)
                    if (xmasget[j].split('img src')[1].indexOf('adventskalender_on') != -1) {
                        if (true /*confirm('Adventskalendertürchen jetzt öffnen ?')*/ ) {
                            window.setTimeout("window.location.href = window.location.protocol + '//' + window.location.host + '/event/xmas/get/'", 500);
                        }
                        else if (confirm('Ans Adventskalendertürchen öffnen dieses Jahr nicht mehr erinnern ?')) {
                            var now = new Date();
                            PGu_setValue("noXmasInfo", (Number(now)+24*86400000).toString());
                        }
                        else {
                            var now = new Date();
                            PGu_setValue("noXmasInfo", (Math.floor((Number(now)/86400000)+1)*86400000).toString());
                            alert ("Vergiß nicht, das Türchen heute selbst zu öffnen!!");
                        }
                        found = true;
                        break;
                    }
    ...
    if (location.toString().endsWith('daily/'))
    ...
    getPlunderPage(function() {
        ...
        if (nrOfTabs <= 1 || nrOfTabs >= 8 || nrOfTabs == 5) {
            alert("Aufgrund einer Änderung im Design des Spiels bleibt die Infozentrale bis zum nächsten Update abgeschaltet");
            return;
        }

        bgrndInd = nrOfTabs >= 6?0:1;
        if (nrOfTabs < 6)
            ajaxcode = "c";
        else if (ajaxcode == "rc")
            bgrndInd = 1;
        ...
        getGangUpgr(function() {
            // Wenn eine Bande vorhanden ist
            if (gangupgcontent.indexOf(KEYWORD_GANGUPGR[lang]) != -1) {
                var doc = HTML2DOM(gangupgcontent);
                ...
                m_owngangid = gangupgcontent.split('/profil/bande:')[1].split('/')[0];
            ...
            if (WiWut == 1)
                ShowImg('', '', get_icon("plndwutpowerback")[bgrndInd], '', 123, 52, PB_X, PB_Y, '', 'PBBACK');
            else if (WiWut == 2)
                ShowImg('', '', get_icon("plndwiwupowerback")[bgrndInd], '', 123, 52, PB_X, PB_Y, '', 'PBBACK');
            else
                ShowImg('', '', get_icon("plndgangpowerback")[bgrndInd], '', 123, 52, PB_X, PB_Y, '', 'PBBACK');
            ...
            if (!myprof)
                myprof = document.getElementById("my-profile");
            if (myprof.getElementsByTagName("div")[2].innerHTML.indexOf("new_msg_infoscreen") != -1)
                myprof.getElementsByTagName("div")[2].setAttribute('style', 'left: '+(nrOfTabs<6?12:40)+'px; bottom: 45px');

            // ***********************************************************************************************
            // Promille- und Waschleiste
            // ***********************************************************************************************
            // Wasch-Icon einfügen
            var curElem = ShowImg('PWBACK', '', noWash?get_icon("redc"):get_icon("wash"), getLangTxt(noWash?TxtLife:TxtWash), '', '', 80, 20, 3, 'infozentrale_wash');
            if (curElem == null)
                return;
            WashHandler(curElem);

            curElem = ShowImg('PWBACK', '', get_icon("promilleup"), getLangTxt(TxtPromUp), '', '', 9, 20, 3, 'infozentrale_promilleup');
            if (curElem == null)
                return;
            PromilleUpHandler(curElem);
            curElem = ShowImg('PWBACK', '', get_icon("promilledown"), getLangTxt(TxtPromDown), '', '', 38, 20, 3, 'infozentrale_promilledown');
            if (curElem == null)
                return;
        ...
        // sonst: Es wurde noch kein Plunderstück ausgewählt
            ...
            ShowImg('PDBACK', PLUNDER_URL, get_icon("plunderfree"), getLangTxt(TOOLTIP_PLUNDERAUSWAHL), '', '', 2, 20, 1, 'AddPlunderIcon');
        }
        ...
        if (location.toString().endsWith("/city/district/") && inGangStatus == 1)
            window.setTimeout(insertPlunderIntoDistrict, 1000);

        if (location.toString().endsWith("/gang/stuff/upgrades/")) {
            function showExistPlunder(allplunder) {
                var trs = allplunder.split("<tr ");
                var plunderNames = new Array();
                var plunderAnz = new Array();
                for (var i = 1; i < trs.length; i++) {
                    var plunder = trs[i].split("<a ")[1].split("</a>")[0];
                    plunderNames.push(plunder.split("<strong")[1].split(">")[1].split("<")[0].trim());
                    plunderAnz.push(plunder.split("<span")[1].split(">")[1].match(/\d+/))[0];
                }
                var skb = document.getElementsByClassName("skill_block");
                for (var i = 0; i < skb.length; i++) {
                    var req = skb[i].getElementsByClassName("required_skills")[0].innerHTML;
                    var plunder = req.split("<br>");
                    for (var j = 0; j < plunder.length - 1; j++) {
                        var pl = plunder[j].split("<span ")[0];
                        var plAnz = pl.match(/\d+/);
                        var plName = pl.replace(plAnz, "").trim();
                        var index = plunderNames.indexOf(plName);
                        if (index == -1)
                            plAnz = 0;
                        else
                            plAnz = plunderAnz[index];
                        plunder[j] += '<span style="color: green;">(' + plAnz + ')</span>';
                    }
                    skb[i].getElementsByClassName("required_skills")[0].innerHTML = plunder.join("<br>");
                }
            }
            GM_xmlhttpRequest({method:"GET", url: PLUNDER_URL + (oldVersion ? "" : "ajax/?"+ajaxcode+"=1"), onload:function(responseDetails) {
                var allplunder = responseDetails.responseText;
                if (nrOfTabs > 5)
                    GM_xmlhttpRequest({method:"GET", url: PLUNDER_URL + (oldVersion ? "" : "ajax/?"+ajaxcode+"=6"), onload:function(responseDetails) {
                        allplunder += responseDetails.responseText;
                        showExistPlunder(allplunder);
                    }});
                else
                    showExistPlunder(allplunder);
            }});
            ...
            if (divs[0].className == "new_msg") {
                ...
                for (i = 0; i < aElem.length; i++) {
                    if (aElem[i].innerHTML.indexOf("fightinfo.gif") != -1) {
                        ...
                        newimg.setAttribute('src', getIconAddr(get_icon("delfightlist")));
            ...
            var makePinsTO = function() {
                function makePins(table) {
                            ...
                            while (pos != -1) {
                                ...
                                if (!pinThis && datetime < pinnedSB.split("_")[1] && lastSB.split("_")[1] >= pinnedSB.split("_")[1]) {
                                    ...
                                    if (document.getElementsByName(lastSB).length > 0)
                                        document.getElementsByName(lastSB)[0].src = getIconAddr(get_icon("pin2"));
                                    pinThis = true;
                                }
                    ...
                    if (pos > 0)
                        ...
                        '<img id="gotoSB" width="0" height="15" style="cursor:pointer" src="' + getIconAddr(get_icon("pin2")) + '"></img>' +
                    ...
                    if (obj.innerHTML.indexOf('<span style="color: rgb(136') != -1 || obj.innerHTML.indexOf('<span style="color:#88') != -1) {
                        ...
                        while (pos != -1) {
                            ...
                            if (pinThis)
                                ...
                                pinnedOnPage = true;
                                text = text.substr(0,oldPos) + text.substr(oldPos).replace(getIconAddr(get_icon("pin")), getIconAddr(get_icon("pin2")));
                            }
                            lastSB = sb;
                            oldPos = text.indexOf("<", pos+1);
                            text = text.substr(0,oldPos) + '<input type="hidden" id="' + sb + '" value=""><img id="pin' + sbnr + '" width="10" height="10" src="' + getIconAddr(pinThis&&first?get_icon("pin2"):get_icon("pin")) + '" style="cursor:pointer" name="' + sb +'"></img>&nbsp;&nbsp;' + text.substr(oldPos);
                            if (pinThis)
                                first = false;
                        ...
                        for (var j=1; true; j++) {
                            ...
                            img.addEventListener("click", function(event)  {
                                if (this.src == getIconAddr(get_icon("pin"))) {
                                    var pinnedSB = PGu_getValue("pinnedSB", "");
                                    PGu_setValue("pinnedSB", this.name);
                                    if (pinnedSB != "") {
                                        if (document.getElementsByName(pinnedSB).length > 0)
                                            document.getElementsByName(pinnedSB)[0].src = getIconAddr(get_icon("pin"));
                                    ...
                                    this.src = getIconAddr(get_icon("pin2"));
                                } else {
                                    this.src = getIconAddr(get_icon("pin"));
                ...
                if (selectd)
                    if ((table.innerHTML.indexOf('<span style="color:#88') != -1 || table.innerHTML.indexOf('<span style="color: rgb(136') != -1) && table.innerHTML.indexOf(getIconAddr(get_icon("pin"))) == -1 && table.innerHTML.indexOf(getIconAddr(get_icon("pin2"))) == -1) {
                        makePins(table);
                    }
            }
        ...
        GM_xmlhttpRequest({method: 'GET', url: FIGHT_URL, onload: function(responseDetails) {
                ...
                function sortByTime(a, b) {
                    ...
                    if (a.length == 0 || b.length == 0)
                        return (a.length != 0)?-1:(b.length != 0)?1:0;

                    var jetzt = new Date();
                    var nowtime = Right$(jetzt.toLocaleString(), 8);

                    var x = ReformatHours(nowtime, a[0]);
                    var y = ReformatHours(nowtime, b[0]);
                ...
                getGangUpgr(function() {
                    ...
                    if (content.indexOf(KEYWORD_MYBUM[lang]) != -1) {
                        ...
                        var IncomingFights = GetIncomingFights(content);

                        // Wenn es eingehende Kämpfe gibt
                        var NrOfFights = IncomingFights.length;
                        if (NrOfFights != 0) {
                            IncomingFights.sort(sortByTime);
                            var FightIcon = Math.min(NrOfFights, 6);
                            // Wenn mehr als ein Kampf eingeht
                            if (NrOfFights > 1) {
                                var FightTitle = printf(getLangTxt(TxtIncomingFights2), NrOfFights);
                                showMsg('Information', "Es laufen " + NrOfFights + " Angriffe auf Dich !! Der erste Angriff erfolgt von " + IncomingFights[0][1] + " um " + IncomingFights[0][0], 'fight_incoming');
                            // sonst: Es gibt nur einen eingehenden Kampf
                            } else {
                                var FightTitle = getLangTxt(TxtIncomingFights1);
                                showMsg('Information', "Du wirst um " + IncomingFights[0][0] + " von " + IncomingFights[0][1] + " angegriffen !!", 'fight_incoming');
                            }

                            FightTitle = '<tr><td style="font-size:12px"><b>' + FightTitle + "</b></td></tr>";
                            for (var i = 0; i < NrOfFights; i++) {
                                var splitted = IncomingFights[i];
                                var ausweich = printf(getLangTxt(TxtAusweichen), (splitted[2] == 'N'?getLangTxt(TxtAusweichN):splitted[2] == 'V'?getLangTxt(TxtAusweichV):''));
                                if (splitted[2][0] == "A")
                                    ausweich += " (-" + splitted[2].split(':')[1] + ")";
                                FightTitle += '<tr><td>' + splitted[0] + " " + splitted[1] + '</td><td>&nbsp;</td><td>' + ausweich + '</td></tr>';
                                }
                            ...
                            ShowImg('FBBACK', '/fight/', get_icon("fight")[0], '', '', '', 0, 14, 1, 'fight');
        ...
        GM_xmlhttpRequest({method:"GET", url: GANGFIGHT_URL, onload:function(responseDetails) {
                ...
                if(NrOfGangFights > 0) {
                    ...
                    ShowImg('GFBACK', '/gang/fight/', get_icon("gangfight")[0], '', '', '', 0, 14, 1, 'gangfight');
            }
        });
    });
    ...
    if (location.toString().substr(-5) == "/pet/" || location.toString().indexOf("/pet/?") != -1) {
        ...
        function calcTime(level, ist, istB, boost, boost2) {
            var zeit = calcMin(level, ist, 0, boost2);
            if (boost == 0)
                return zeit;
            return zeit + " (mit Boost " + calcMin(level, istB, boost, boost2) + ")";
        }
        function calcMin2(level, ist, boost, boost2, maxdauer) {
            var rest = (level-1)*(300+(level-1)*20) - ist;
            var zeit = calcMin(level, (rest>0?0:-rest), boost, boost2);
            if (rest < 0)
                return zeit;
            var routen = document.getElementsByName("route_length");
            if (routen.length > 0) {
                routen = routen[0].innerHTML.split('value="');
                var zeiten = new Array();
                for (var k = 1; k < routen.length; k++)
                    zeiten[k-1] = Number(routen[k].split('"')[0]);
            }
            else
                var zeiten = new Array(10, 30, 60, 90, 120, 180);
            for (var k = 0; k < zeiten.length; k++) {
                if (maxdauer < zeiten[k])
                    break;
        ...
        function calcTime2(level, ist, istB, boost, boost2, maxdauer) {
            var zeit = calcMin2(level, ist, 0, boost2, maxdauer);
            if (boost == 0)
                return zeit;
            return zeit + " (mit Boost " + calcMin2(level, istB, boost, boost2, maxdauer) + ")";
        }
        function timeTitles(doc) {
            var petafs = doc.getElementsByClassName("pet_action_finished_shell");
            if (petafs.length > 0) {
                var hs = petafs[0].getElementsByClassName("hs_shell");
                var trs = hs[0].getElementsByTagName("tr");
                var xp1 = Number(trs[1].getElementsByTagName("td")[0].innerHTML.split(" ").pop());
                var xp2 = Number(trs[3].getElementsByTagName("td")[0].innerHTML.split(" ").pop());
                var tds = hs[0].getElementsByTagName("td");
                for (var k = 0; k < tds.length; k++)
                    if (tds[k].id && tds[k].id == "roam_value_xp") {
                        var xpg = Number(tds[k].innerHTML.split(" ").pop());
                        break;
                    }
            }
            ...
            for (j = 0; j < petas.length; j++) {
                ...
                var expist1 = Number(expTxt.split(" ")[0]);
                var expist2 = expist1;
                var fertig = false;
                if (div[3].innerHTML.indexOf(">Fertig<") != -1) {
                    expist1 += xp1 + xp2;
                    expist2 += xpg;
                    fertig = true;
                }
                bar =  div[1].getElementsByTagName("div")[0].innerHTML;
                var maxdauer = Number(bar.split("/")[1]);
                if (level < 49)
                    var zeit2txt = "; nächstes Level: " + calcTime2(level + 1, expist1, expist2, erfboost, erfboost2, maxdauer);
                else
                    var zeit2txt = "";
                div[0].innerHTML = div[0].innerHTML.replace(expTxt, expTxt + "'; this.title='" + (fertig?"Dein Haustier ist zurück: ":"")+"dieses Level noch " + calcTime(level, expist1, expist2, erfboost, erfboost2) + zeit2txt);
            }
        }
        function sortValues (elements, q) {
            var last = elements.length - 1;
            if (elements[last].className.indexOf("slot_disabled") != -1)
                last--;
            for (var i = 0; i < last; i++) {
                var k = (q == -1 ? last : i);
                if (k < last)
                    var stamina = Number(elements[i].getElementsByClassName("statcontainer")[q].innerText);
                for (var j = k+1; j <= last; j++) {
                    var stam = Number(elements[j].getElementsByClassName("statcontainer")[q].innerText);
                    if (stam > stamina) {
                        k = j;
                        stamina = stam;
                    }
                ...
                if (k != i) {
                    var nextSibling = elements[i].parentNode.removeChild(elements[k]);
                    elements[i].parentNode.insertBefore(nextSibling, elements[i]);
                }
            }
        }
        var sortind = [];
        function ModifySortBox(box) {
            var sortBox = document.getElementsByName("sorting");
            if (sortBox.length == 0) {
                window.setTimeout(ModifySortBox, 1000, 0);
                return;
            }
            if (sortBox.length == 1)
                document.getElementById("meadow_click").addEventListener('click', function(event) {
                    window.setTimeout(ModifySortBox, 500, 1);
                    return;
                }, false);
            var quals = ["Ausdauer", "Kampfkraft", "Spürnase", "Wachsamkeit", "umgekehrt"];
            if (sortBox[box].innerHTML.indexOf(quals[quals.length-1]) != -1)
                return;
            var option = "<option" + sortBox[box].innerHTML.split("<option").pop();
            for (var i = 0; i < quals.length; i++)
                sortBox[box].innerHTML = sortBox[box].innerHTML + option.replace(/>.*</, ">" + quals[i] +"<");
            sortind[box] = sortBox[box].selectedIndex;
            sortBox[box].addEventListener("change", function(event) {
                var i = quals.indexOf(this.value);
                if (i >= 0) {
                    sortValues(document.getElementsByClassName("inactive_shell")[box].getElementsByClassName('petshell'), (i == quals.length-1?-1:i));
                }
                if (i == quals.length - 1)
                    this.selectedIndex = sortind[box];
                else
                    sortind[box] = this.selectedIndex;
            }, false);
        }

        var kader = document.getElementById("pet_kader");
        if (kader) {
            if (document.getElementsByClassName("pet_action_finished_shell").length > 0)
                timeTitles(document);
            else {
                GM_xmlhttpRequest({method:"GET", url: PET_URL + "tab/action/", onload:function(responseDetails) {
                    var content = responseDetails.responseText;
                    var doc = HTML2DOM(content);
                    timeTitles(doc);
                }});
        ...
        document.getElementById("plundertab").getElementsByTagName("a")[1].addEventListener('click', function(event) {
            window.setTimeout(ModifySortBox, 500, 0);
            return;
        }, false);
    }
    ...
    if (IsPlunderPage() && document.getElementsByClassName("grunge zleft").length > 0) {
        var fnPlunderChB = function () {
            ...
            // Referenz auf Plundertabelle speichern
            var plundertable = document.getElementById("plunderlist").getElementsByTagName("table");
    ...
    if (IsGamePage()) {
        function Losekaufen(menge, maxMenge, gewinne)
        {
            if (menge <= 0) {
                ...
                var gesamt = 0;
                var einsatz = 0;
                var price = Number(document.getElementById("preis_cent1").value)/100;
                ...
                for (i = 0; i < gewinne.length; i++) {
                    ...
                    einsatz += gewinne[i][1] * price;
                }
                alert(Stat + printf(getLangTxt(TxtTotalWon), gesamt, einsatz, gesamt - einsatz));
                //alert(PGu_getValue("winnings", ""));
                refreshPage();
                return;
            }
            ...
            GM_xmlhttpRequest({method: 'POST',
                               url: window.location.protocol + '//' + window.location.hostname + '/city/games/buy/',
                               headers: {'Content-type': 'application/x-www-form-urlencoded'},
                               data: encodeURI(data.substr(0,data.length-1)),
                               onload: function(responseDetails) {
                                           var content = responseDetails.responseText;
                                           var money = document.getElementsByClassName("icon money");
                                           if (content.indexOf("new_cash") != -1) {
                                               var money2 = money_format(Number(content.split('"new_cash":')[1].split(",")[0].trim())/100, "");
                                               var spl = money[0].innerHTML.split("</a>")[0].split(">");
                                               money[0].innerHTML = spl[0]+">"+money2+"</a>";
                                               menge = menge - bmenge;
                                               var gew = content.split('"winnings":')[1].split(",")[0].trim();
                                               //PGu_setValue("winnings", PGu_getValue("winnings", "") + content);
                                               if (!isNaN(gew) && gew != "0")
                                                   gew = gew.substr(0, gew.length-2);
        ...
        function getAnzLose() {
            ...
            var pos = tds[0].innerHTML.indexOf('id="lose_remaining"');
            if (pos >= 0)
                return Number(tds[0].innerHTML.substr(pos).split("<")[0].split(">")[1].trim());
    ...
    if (IsMessagesPage() && lang == 0) {
        ...
        input2.type = 'button';
        input2.id = 'setmsgread';
        input2.name = 'setmsgread';
        input2.value = "Ausgewählte Nachrichten als gelesen markieren";
        var form = document.getElementById("delete_all");
        form.appendChild(input2);
        document.getElementById('setmsgread').addEventListener('click', function onClick() {
            var table = document.getElementById("messageslist");
            var delmsg = document.getElementsByName("del_msg");
            var trs = table.getElementsByClassName("msglist");
            for (var i = 0; i < trs.length; i++)
                if (delmsg[i].checked) {
                    GM_xmlhttpRequest({method: 'GET', url: trs[i].getElementsByTagName("a")[0].href, onload: function(responseDetails) { }});
                    delmsg[i].checked = false ;
                }
            window.setTimeout("document.location.reload()", 2000);
        },false);
...
var waitAndStart = function(first) {
    if (first) {
        if (allIcons == "") {
            setTimeout(waitAndStart, 500, true);
            return;
        }
        imgPrefix = imgPrefix.split("§");
        var imgPrefixSub = imagehoster.split("§");
		imagehoster = imagehoster[0];
		for (var k = 1; k < imgPrefixSub.length; k++)
			if (imgPrefixSub[k].indexOf(TOWNEXTENSION + ":") != -1) {
				var prefs = imgPrefixSub[k].split(":").pop();
				for (var j = 0; j < prefs.length; j++)
					imgPrefix[prefs[j]] += TOWNEXTENSION + "/";
			}
        iconNames = iconNames.split("§");
        allIcons = allIcons.split("§");
        for (var i = 0; i < allIcons.length; i++)
            if (allIcons[i].indexOf("/") != -1)
                allIcons[i] = allIcons[i].split("/");
    ...
    if (wrap == null) {
        ...
        if (wrap == null) {
            setTimeout(waitAndStart, 500, false);
            return;
        }
    }
...
imagehoster = GM_getValue("imagehoster", "")+"";
imgPrefix = GM_getValue("imgPrefix", "");
iconNames = GM_getValue("iconNames", "");
allIcons = GM_getValue("allIcons", "");
if (imagehoster == "" || imgPrefix == "" || iconNames == "" || allIcons == "")
    iconVers = "";
else
    iconVers = GM_getValue("iconVers", "");
