...
// @grant none
// @downloadURL https://update.greasyfork.org/scripts/491925/TW%20Kick-o-Matic.user.js
// @updateURL https://update.greasyfork.org/scripts/491925/TW%20Kick-o-Matic.meta.js
// ==/UserScript==
...
(function (fn) {
    ...
    KoM = {
      ...
      loading: false,
      inProgress: {},
      fortsCapacity: [[46, 42], [92, 84], [130, 120]],
      ...
      langs: {
        ru: {
          ...
          showFort: 'Показать обзор форта',
          showBattle: 'Показать поле боя',
          atFort: 'В форте',
          nearFort: 'Рядом с фортом',
          notFort: 'Не в форте',
          evaluated: 'Уже назначен',
          started: 'Битва уже началась',
          remindAlly: 'Позвать незарегистрированных членов альянса',
          where: 'Где',
          when: 'Когда',
          att: 'Атакующий',
          def: 'Защитник',
          ...
          asmajor_general: 'Провозгласить генерал-майором',
          asprivate: 'Назначить рядовым',
          asrecruit: 'Взять в рекруты',
          asreservist: 'Демобилизовать',
    ...
    KoM.makeDistanceImage = function (fortX, fortY, playerX, playerY) {
      ...
      } else if (Math.abs(diffX) <= 1000 && Math.abs(diffY) <= 1000) {
    ...
    KoM.bindClickFunctions = function () {
      ...
      var classes = [
        ...
        'chat_servicegrade_major_general',
