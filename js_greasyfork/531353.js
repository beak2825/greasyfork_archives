...
// @version      202506311
// @description  IDBI_CORP_MINI_2025
// @author       You
// @match        https://corp.idbibank.co.in/corp/Finacle;jsessionid*
...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/531353/IDBI_CORP_MINI.user.js
// @updateURL https://update.greasyfork.org/scripts/531353/IDBI_CORP_MINI.meta.js
// ==/UserScript==
...
(function() {
	...
	setInterval(function(){
		...
		if(_startFlag && null!=channelName){
			let mini = document.getElementById("PageConfigurationMaster_CACUX5W__1:VIEW_MINI_STATEMENT[0]3");
			let blockLength = document.querySelectorAll("div[class='blockUI']").length;
			if(null ==closeIcon && mini && blockLength == 0 && !mini_load){
				mini.click();
	...
	function addElement(){
		GM_addElement(document.getElementById("wrapper1"), 'input', {
    ...
    function commitData(dataArray){
		...
		for (var i = 0; i < dataArray.length; i++) {
		  ...
		  if(record.length>2){
			  let tradeType = record[1];
			  let amount = record[2];
			  let desc = dataArray[i+1][0];
			  if(tradeType && amount && desc){
				  let item = {};
				  record[3] = desc;
				  amount = amount.replaceAll(",","");
				  if('CR' === tradeType && desc.startsWith('UPI')){
					item['amount'] = amount;
