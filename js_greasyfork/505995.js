// ==UserScript==
// @name   🌱【免费版】新版辽宁省干部在线学习网|云南青海河北新疆干部网络学院|*收费版本见使用说明：http://doc.zhanyc.cn/pages/hbgb/
// @namespace    http://jb.zhanyc.cn/
// @icon    https://js.zhanyc.cn/img/js-logo.svg
// @version      1.6
// @description  当前是免费版本，只包含了简单基本功能。广告:付费版本可解锁全自动无人值守，极速秒过，欢迎选购，请我喝一杯咖啡，保你无忧学习|接各类脚本开发工作，微信：zhanyc_cn 备用微信:zhanfengkuo 个人网站：http://doc.zhanyc.cn
// @author       zfk
...
// @antifeature 
// @license GPL
// @downloadURL https://update.greasyfork.org/scripts/505995/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E6%96%B0%E7%89%88%E8%BE%BD%E5%AE%81%E7%9C%81%E5%B9%B2%E9%83%A8%E5%9C%A8%E7%BA%BF%E5%AD%A6%E4%B9%A0%E7%BD%91%7C%E4%BA%91%E5%8D%97%E9%9D%92%E6%B5%B7%E6%B2%B3%E5%8C%97%E6%96%B0%E7%96%86%E5%B9%B2%E9%83%A8%E7%BD%91%E7%BB%9C%E5%AD%A6%E9%99%A2%7C%2A%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%E8%A7%81%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E%EF%BC%9Ahttp%3Adoczhanyccnpageshbgb.user.js
// @updateURL https://update.greasyfork.org/scripts/505995/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E6%96%B0%E7%89%88%E8%BE%BD%E5%AE%81%E7%9C%81%E5%B9%B2%E9%83%A8%E5%9C%A8%E7%BA%BF%E5%AD%A6%E4%B9%A0%E7%BD%91%7C%E4%BA%91%E5%8D%97%E9%9D%92%E6%B5%B7%E6%B2%B3%E5%8C%97%E6%96%B0%E7%96%86%E5%B9%B2%E9%83%A8%E7%BD%91%E7%BB%9C%E5%AD%A6%E9%99%A2%7C%2A%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%E8%A7%81%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E%EF%BC%9Ahttp%3Adoczhanyccnpageshbgb.meta.js
// ==/UserScript==
(function () {
    ...
    let docUrl = "http://doc.zhanyc.cn/pages/hbgb/";
    let plugMain = Object.assign(baseConfig, {
        ...
        async runByUrl(url) {
            ...
            plugMain.pageData.tipsAndCloseMark = plugMain.now()
            if (url.includes("/study_play.do?")) {
                plugMain.page_video();
            } else if (
                url.includes("/study_center/sent_detail/tool_course")
            ) {
                plugMain.showPaidContent(`免费版本不包含自动换课功能，收费版本含全自动秒过换课功能，如需使用请安装收费版本`)
            } else if (
                url.includes("course_myselect.do?") ||
                url.includes("/course_myrequired.do?")
            ) {

                plugMain.showPaidContent(`免费版本不包含自动换课功能，收费版本含全自动秒过换课功能，如需使用请安装收费版本`)
            } else if (url.includes("/portal/course_detail.do?")) {
                plugMain.page_course_detail();
            } else if (
                url.includes('/study_center/my_course')
            ) {

                plugMain.showPaidContent(`免费版本不包含自动换课功能，收费版本含全自动秒过换课功能，如需使用请安装收费版本`)
            } else if (url.includes("/course_detail?")) {
                plugMain.page_course_detail2();
            } else if (url.includes("/video_detail?")) {
                plugMain.page_video2();
            } else if (url.includes("/study_center/curricula_variable")) {
                plugMain.page_curricula_variable();
            } else if (url.includes("/pc/index.html")) {
                plugMain.page_pcIndex();
            }
        },
        async page_pcIndex() {
            await plugMain.waitOf(a => $(`[aria-label="关于“专业技术人才在线学习专区”学习有关问题的通知"]`).length > 0)
            plugMain.confirmMsg("检测到倒计时弹窗，是否跳过", {
                title: "脚本提示（3秒后自动关闭本提示）", btn: ["跳过倒计时", "取消"], time: 3000,
                fun1(index) {
                    $(`[aria-label="关于“专业技术人才在线学习专区”学习有关问题的通知"]`).next().remove()
                    $(`[aria-label="关于“专业技术人才在线学习专区”学习有关问题的通知"]`).remove()
                    layer.close(index)
                },
                fun2() { },
                fun3() { }
        ...
        async page_curricula_variable() {
            console.log("%c page_curricula_variable", "background:rgb(0,0,0);color:#fff");
            await plugMain.waitOf(a => $(".content_box_header").length > 0)
            $(".content_box_header").before(`<div text="text-center"><button class="zfk-btn" type="button" onclick="plugMain.xk()">脚本一键选课：本页可见课程</button></div>`)

        },
        async page_course_detail() {
            console.log("%c page_course_detail", "background:rgb(0,0,0);color:#fff");
            ...
            await plugMain.waitOf(
                (a) =>
                    $("#cse_dtl_btn input").length > 0 || $("#cse_dtl_btn a").length > 0
            );
            plugMain.confirmRun().then((a) => {
                if ($("#cse_dtl_btn a").length > 0) {
                    $("#cse_dtl_btn a")[0].click();
                }
                $("#cse_dtl_btn input").click();
                plugMain.openLjTips(true, true);
            });
        },

        async page_course_detail2() {
            console.log("%c page_course_detail2", "background:rgb(0,0,0);color:#fff");
            plugMain.closeWaitConfrimWin();
            if (plugMain.pageData.courseDetailIndex == null) {
                plugMain.pageData.courseDetailIndex = setTimeout(() => {
                    plugMain.confirmRun("网页卡住了，3秒后刷新页面").then(a => {
                        location.reload()
                    })
                }, 60 * 1000);
            }
            await plugMain.waitOf(
                (a) =>
                    $(".select_course").length > 0
            );
            await plugMain.waitTimeout(2000)
            if ($(".select_course").text() != "开始学习") {
                plugMain.tipsMsg("卡住了，5秒后重试", 5000)
            ...
            plugMain.confirmRun().then((a) => {
                $(".select_course").click()
                plugMain.openLjTips(true, true)
                // plugMain.checkUrlChangeAndRun()
            });
        },

        async page_video2() {
            plugMain.closeWaitConfrimWin();
            console.log("%c page_video2", "background:rgb(0,0,0);color:#fff");
            let timeout = 2;
        ...
        async page_video() {
            ...
            plugMain.pageData.video.index = setInterval(async () => {
                try {
                    ...
                    if (plugMain.getElByText($("div.user_choise"), "继续学习") != null) {
                        plugMain.getElByText($("div.user_choise"), "继续学习").click();
                        return;
                    }
                    if (!plugMain.getVideo()) {
                        console.log("%c zfk no video", "background:rgb(0,0,0);color:#fff");
        ...
        async showPaidContent(msg = "此页面未付费内容，免费脚本不包含", withPostfix = true) {
            if (withPostfix) {
