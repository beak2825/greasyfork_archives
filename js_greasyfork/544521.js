...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/544521/CS-Torrent-Assistant-New.user.js
// @updateURL https://update.greasyfork.org/scripts/544521/CS-Torrent-Assistant-New.meta.js
// ==/UserScript==
...
(function () {
    ...
    if (title_lowercase.includes("web-dl") || title_lowercase.includes("webdl") || title_lowercase.includes("webrip") || title_lowercase.includes("web-rip")) {
        ...
        title_type = 12;
    } else if (title_lowercase.includes("AV1")  ) {
        title_type = 13;
    ...
    // 分辨率
    if (title_lowercase.includes("1440p") || title_lowercase.includes("1440i")) {
        title_resolution = 11;
    } else if (title_lowercase.includes("1080p") || title_lowercase.includes("1080i")) {
        title_resolution = 7;
    ...
    $('#kdescr img').each(function (index, element) {
        ...
        if (src != undefined) {
            ...
            if (banPictureBed.some(domain => src.includes(domain))) {
                // 如果还没有添加过这个错误信息，则添加
                isBanPictureBed = true;
            }
        }
    ...
    if (isBanPictureBed) {
        $('#assistant-tooltips').append('请检查海报或者截图是否加载正常<br>');
        error = true;
    }
