// ==UserScript==
// @name        HIT Scraper WITH EXPORT (MTF Discord mod)
// @author      DaNipper (feihtality)
...
// @version     1
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/23053/HIT%20Scraper%20WITH%20EXPORT%20%28MTF%20Discord%20mod%29.user.js
// @updateURL https://update.greasyfork.org/scripts/23053/HIT%20Scraper%20WITH%20EXPORT%20%28MTF%20Discord%20mod%29.meta.js
// ==/UserScript==
      ...
      var _vb = () => {//{{{
        ...
        _irc = () => {//{{{
          ...
          Core.fetch(api+payload, null, 'text', false).then( r => {
            ...
            this.node.innerHTML = '<p>Discord Export</p>' +
              '<textarea style="display:block;padding:2px;margin:auto;height:130px;width:500px" tabindex="1">' + '@hits ' +
              (/masters/i.test(this.record.quals.join()) ? `\n • MASTERS ` : '') +
              `\n • Requester: ${this.record.requester.name} ${urlArr[0]} \n • HIT: ${this.record.title} ` +
              `${urlArr[1]} \n • Pay: ${this.record.pay} ${sym} Avail: ${this.record.numHits} ${sym} ` +
              `Limit: ${this.record.time} \n • TO: ${getTO()} ${urlArr[2]} \n • PandA: ${urlArr[3]}</textarea>` +
