...
// @version     1.7
// @author      jawz
// @match       https://www.mturk.com/mturk/findhits?match=true?GOATScraper*      
...
// @downloadURL https://update.greasyfork.org/scripts/7813/GOAT%20Scraper.user.js
// @updateURL https://update.greasyfork.org/scripts/7813/GOAT%20Scraper.meta.js
// ==/UserScript==
...
////Links////
var preview_Link = "https://www.mturk.com/mturk/preview?rank=24&sortType=NumHITs%3A1&groupId="
var previewAccept_Link = "https://www.mturk.com/mturk/previewandaccept?groupId="
var $batchSrc;
var $singleSrc;
var $queueSrc;
var $next_URL;
...
////Qualified State////
var batchQualifiedState = "on";
var singleQualifiedState = "on";
...
tbl3.rows[0].cells[0].onclick =  function() {    
    if (startStop == false) {
        $batchSrc = "https://www.mturk.com/mturk/searchbar?selectedSearchType=hitgroups&searchWords=&minReward=" + reward_input.value + "&qualifiedFor=" + batchQualifiedState + "&sortType=NumHITs%3A1";
    	tbl3.rows[0].cells[0].innerHTML =  "Stop";
        linksBlack();
        tbl3.rows[0].cells[3].innerHTML = "Grazing...";
    	startStop = true;
...
tbl4.rows[0].cells[0].onclick =  function() {
    if (qual_check == 0){
    	...
    	qual_check = 1;
        batchQualifiedState = "on";
    } else {
        tbl4.rows[0].cells[0].style.backgroundColor =  UNCHECKED;
        qual_check = 0;
        batchQualifiedState = "off";
    }
}

////Batch Sound////
tbl3.rows[1].cells[3].innerHTML =  "Alert Sound: ";
tbl3.rows[1].cells[3].style.cursor = 'default';

bsoundbtn  = document.createElement('table');
tbl3.rows[1].cells[3].appendChild(bsoundbtn);
...
tbl9.rows[0].cells[3].style.cursor = "default";
...
tbl9.rows[0].cells[0].onclick =  function() {
    if (startStop3 == false) {
        $singleSrc = "https://www.mturk.com/mturk/searchbar?selectedSearchType=hitgroups&searchWords=&minReward=" + reward_input3.value + "&qualifiedFor=" + singleQualifiedState + "&sortType=LastUpdatedTime%3A1";
    	tbl9.rows[0].cells[0].innerHTML =  "Stop";
        linksBlack();
        tbl9.rows[0].cells[3].innerHTML = "Grazing...";
    	startStop3 = true;
...
////Newest Sound////
tbl9.rows[1].cells[3].innerHTML =  "Alert Sound: ";
tbl9.rows[1].cells[3].style.cursor = 'default';

nsoundbtn  = document.createElement('table');
tbl9.rows[1].cells[3].appendChild(nsoundbtn);
...
var mturkTime;
var monthDay;
var $SubmittedSrc = "https://www.mturk.com/mturk/status";
var $earningsSrc = "https://www.mturk.com/mturk/transferearnings";
var month;
var day;
var year;
...
function scrapeHybrid() {
    GM_xmlhttpRequest ({
        method: "GET",
        url: 'http://www.gethybrid.io/workers/projects',
        onload: function (results) {
            if (results.finalUrl == 'http://www.gethybrid.io/users/sign_in') { //AUTO
                var temp = window.open('http://www.gethybrid.io/users/sign_in');
                setTimeout(function(){ 
                    temp.close(); 
                    scrapeHybrid();
                    return;
            ...
            var links = data.find('a[href*="projects/"]');
            var task = [];
            var pay = [];
            var poster = [];
            var created = [];
            ...
            for (var i=0, length=links.length; i<length; i++) {
                ...
                link[i] = 'http://www.gethybrid.io' + links.eq(i).attr("href");
...
function batchGrabData() {
    ...
    .done(function(data) {
    	...
    	{
            $next_URL = $batchData.find('a[href^="/mturk/viewsearchbar"]:contains("Next")');
            $batchSrc = ($next_URL.length != 0) ? $next_URL.attr("href") : "";
            batchNextPage++;
    ...
    .fail(function() {
        ...
        //$(captchaDiv).show();    
        setTimeout(function(){batchGrabData();}, 10000);
...
function singleGrabData() {
	$.get($singleSrc, function(data) {
    	...
    	if (maxpagerate.length == 0) {
            $next_URL = $singleData.find('a[href^="/mturk/viewsearchbar"]:contains("Next")');
            $singleSrc = ($next_URL.length != 0) ? $next_URL.attr("href") : "";
            singleNextPage++;
    ...
    //captchaTbl.rows[0].cells[0].innerHTML = "You've been logged out";
    //$(captchaDiv).show();    
    setTimeout(function(){singleGrabData();}, 10000);
    logger = window.open('https://www.mturk.com/mturk/dashboard','remote1');    
  });
}
...
////Get Requester////
function getRequesters(data) {
	var requester = data.find('a[href^="/mturk/searchbar?selectedSearchType=hitgroups&requester"]');
	for (var i = 0; i < requester.length; i++) { requester[i] = requester[i].textContent; } 
    return requester;
}

////Get Requester Links////
function getRequesterLinks(data) {
	var requesterlink = data.find('a[href^="/mturk/searchbar?selectedSearchType=hitgroups&requester"]');
	for (var i = 0; i < requesterlink.length; i++) { requesterlink[i] = requesterlink[i].href; }
    return requesterlink;
}

////Get Reward////
function getRewards(data) {
    var reward = data.find('span[class="reward"]');
    for (var i = 0; i < reward.length; i++) { reward[i] = ( reward[i].textContent ); } 
	for (var i = 0; i < reward.length; i++) { reward[i] = ( reward[i].replace("$", "") ); } 
    return reward;
}
...
function queueFilterData($queueData) {
    ...
    ////Get Titles////
	var $title = getTitles($queueData);
    var $reward = getRewards($queueData);
	...
	for (var i = 0; i < $preview.length; i++) { $preview[i] = ( $preview[i].href ); }
	
    var $contact = $queueData.find('a[href^="/mturk/contact?"]'); // GETS CONTACT LINK
	for (var i = 0; i < $contact.length; i++) { $contact[i] = ( $contact[i].href ); }
    
    var $return = $queueData.find('a[href^="/mturk/return?"]'); // GETS RETURN LINK
	for (var i = 0; i < $return.length; i++) { $return[i] = ( $return[i].href ); }
...
function stopScrape() {
    ...
    if (batchTimerState == true) {
        tbl3.rows[0].cells[0].innerHTML =  "Start";
        linksBlack();
        tbl3.rows[0].cells[3].innerHTML = ""
        tbl3.rows[0].cells[3].innerHTML = "Stopped"
        clearInterval(timerSwitch);
        timerVar = 0;
        batchTimerState = false;
        }
    else {
        tbl3.rows[0].cells[3].innerHTML = "Stopping"
...
function stopScrape3() {
    ...
    if (singleTimerState == true) {
        tbl9.rows[0].cells[0].innerHTML =  "Start";
        linksBlack();
        tbl9.rows[0].cells[3].innerHTML = ""
        tbl9.rows[0].cells[3].innerHTML = "Stopped"
        clearInterval(timerSwitch3);
        timerVar3 = 0;
        singleTimerState = false;
    } else {
        tbl9.rows[0].cells[3].innerHTML = "Stopping"
...
var scrapeTimer = function () {
    tbl3.rows[0].cells[3].innerHTML = "Next graze in: " + timerVar + " seconds";
    timerVar = timerVar - 1;
    if (timerVar < 0) {
        batchTimerState = false;
        clearInterval(timerSwitch)
...
var scrapeTimer3 = function () {
    tbl9.rows[0].cells[3].innerHTML = "Next graze in: " + timerVar3 + " seconds";
...
function batchScrape() {
    ...
    for (var i=0; i < $batchRequesterLink.length; i++) { 
		...
		if (i < ($batchRequesterLink.length - 1))	
   		    idList += requesterId[i] + ",";
    	else
    	    idList += requesterId[i];
	}
    ...
    for (var i = 0; i < $batchRequester.length; i++) {
        ...
        if ($batchNotQualifiedGroupIds.indexOf($batchListy[i]) < 0) {
        	tbl.rows[i+1].cells[1].innerHTML = "<a href='"+ preview_Link + $batchListy[i] +"' target='_blank' title='"+ $batchTitle[i] +"'>" + $batchTitle[i] + "</a>";
            ...
            tbl.rows[i+1].cells[6].onclick = function() { 
                var $item = $(this).closest("tr").find("td:nth-child(2)")[0].innerHTML.replace('<a href="', "").split('" target', 1)[0].replace("preview?", "previewandaccept?");
                var $requester = $(this).closest("tr").find("td:nth-child(1)")[0].textContent;
                var $title = $(this).closest("tr").find("td:nth-child(2)")[0].textContent;
    ...
    for (i = 0; i < requesterId.length; i++) { batchtoRequesterId[i] = requesterId[i]; }
        ...
        onload: function (results)
        	{
                ...
                for (i = 0; i < batchtoRequesterId.length; i++) {
                    ...
                    if (rdata[batchtoRequesterId[i]]) {
                    	...
                    	var fair = rdata[batchtoRequesterId[i]].attrs.pay;
                    	var fast = rdata[batchtoRequesterId[i]].attrs.pay;
                        
                        sum = ((pay*10) + (fair*5))/15;
                        sum = sum.toFixed(2);
                        batchtoAverage = sum;
                        var tit = 'Comm: ' + comm + '\n' + 'Pay: ' + pay + '\n' + 'Fair: ' + fair + '\n' + 'Fast: ' + fast;
                        tbl.rows[i+1].cells[5].innerHTML = "<a href='"+ "http://turkopticon.ucsd.edu/reports?id=" + batchtoRequesterId[i] +"' target='_blank' title='" + tit + "'>" + batchtoAverage + "</a>";
                        } else {
                            batchtoAverage = -1;
...
function singleScrape() {
    ...
    for (var i=0; i < $singleRequesterLink.length; i++) { 
		...
		if (i < ($singleRequesterLink.length - 1)) {	
   		    idList3 += requesterId3[i] + ",";
    	} else
    	    idList3 += requesterId3[i];
	}
    ...
    for (var i = 0; i < $singleRequester.length; i++) {
        ...
        if ($singleNotQualifiedGroupIds.indexOf($singleListy[i]) < 0) {
        	...
        	tbl12.rows[i+1].cells[1].innerHTML = "<a href='"+ preview_Link + $singleListy[i] +"' target='_blank' title='"+ $singleTitle[i] +"'>" + $singleTitle[i] + "</a>";  //
    ...
    newestRequest(requesterId3, $singleNotQualifiedGroupIds, $singleListy, $singleReward, $singleAllotedSeconds, $singleHits, $singleRequester, $singleTitle);
        ...
        onload: function (results) {
            ...
            for (i = 0; i < singletoRequesterId.length; i++){
                ...
                if (rdata[singletoRequesterId[i]]){
                    ...
                    var fair = rdata[singletoRequesterId[i]].attrs.pay;
                    var fast = rdata[singletoRequesterId[i]].attrs.pay;
                    sum = ((pay*10) + (fair*5))/15;
                    sum = sum.toFixed(2);
                    singletoAverage = sum;
                    var tit = 'Comm: ' + comm + '\n' + 'Pay: ' + pay + '\n' + 'Fair: ' + fair + '\n' + 'Fast: ' + fast;
...
function queueScrape() {
    ...
    for (var i = 0; i < $finalqueuerequester.length; i++) {
        ...
        tbl7.rows[i+1].cells[4].onclick = function() { 
                var $item = returnLink + $(this).closest("tr").find("td:nth-child(2)")[0].innerHTML.replace('<a href="', "").split('hitId=', 2)[1].split('"', 1)[0]
...
function grabEarnings() {
	$.get($earningsSrc, function(data) {
    	...
    	if (maxpagerate.length == 0) {
            var $reward = $earningsData.find('span[class="reward"]');
