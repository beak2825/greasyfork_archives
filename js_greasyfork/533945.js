// ==UserScript==
// @name         Lolzteam Multiaccount Finder by xellan
// @version      2.7.4
// @license MIT
...
// @downloadURL https://update.greasyfork.org/scripts/533945/Lolzteam%20Multiaccount%20Finder%20by%20xellan.user.js
// @updateURL https://update.greasyfork.org/scripts/533945/Lolzteam%20Multiaccount%20Finder%20by%20xellan.meta.js
// ==/UserScript==
...
// Проверка наличия элементов с помощью цикла while
while (!accountMenu || !linksList) {
    try {
        ...
        console.error("Произошла ошибка при проверке элементов: AccountMenu");
    }
}
...
async function openSettings(page = 1, lastId = generateRandomString(10)) {
    ...
    XenForo.alert(content, 'Lolzteam Multiaccount Finder by xellan');
...
function logWithPrefix(text) {
    console.log(`[Lolzteam Multiaccount Finder by xellan] ${text}`)
}
...
async function template(name, description, link, open = true, source = undefined, bannedPercent) {
    ...
    if (await checkKeyValue("AutomaticFraudComplaint") === "true") {
        if (await checkKeyValue("ForumComplaintHide") === "true") {
            message = `[CLUB]1. Никнейм нарушителя и ссылка на профиль: ${link.replace('/shared-ips', '')}\n2. Краткое описание жалобы: ${description}\n3. Доказательства: ${link}[/CLUB]\nЖалоба была сформирована с использованием Lolzteam Multiaccount Finder by xellan ([URL]https://${domain}/threads/8621814/[/URL])[CLUB]Создано автоматически\nДата и время: ${moscowTime}[/CLUB]`
        }
        else {
...
async function checkUser(link, source, gifId, style = null) {
    ...
    if (notToCheckPreviouslyChecked && checkedItem) {
        ...
        if (gifElement) {
            ...
            xenforoLogAndAlert(`${output}`, `Lolzteam Multiaccount Finder by xellan`);
        }
        return;
    }
