...
// @grant        GM_getValue
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/474703/Shoplifting%20alert.user.js
// @updateURL https://update.greasyfork.org/scripts/474703/Shoplifting%20alert.meta.js
// ==/UserScript==
...
(function() {
    ...
    function displayRedSquare(camera, guard) {
        ...
        if (camera) { cameraTextNode = document.createTextNode(`Camera: Offline`); }
        if (guard) { guardTextNode = document.createTextNode(`Guards: Gone`); }
        const lineBreak = document.createElement('br');
        ...
        redSquare.addEventListener('click', () => {
            ...
            GM_setValue('timeOut', Date.now() + timeOut);
        ...
        if (currentTime - lastCallTime >= 60000) {
            ...
            fetch(`https://api.torn.com/torn/?selections=shoplifting&key=${apiKey}`)
                ...
                if (data.error && data.error.error === "Incorrect key") {
                    GM_setValue('shoplifting_api_key', "");
                    displayYellowSquare();
                } else {
