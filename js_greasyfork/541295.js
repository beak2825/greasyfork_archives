...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/541295/Notion%20AI.user.js
// @updateURL https://update.greasyfork.org/scripts/541295/Notion%20AI.meta.js
// ==/UserScript==
...
(function() {
    ...
    async function processPrompt(data) {
        ...
        log('开始处理提示词:',modelName, data.transcript.length);
        ...
        window.fetch = async function(...args) {
            ...
            if (url.includes('/runInferenceTranscript')) {
                ...
                try {
                    ...
                    const userId = body.transcript[1].value.userId
                    // 删除body.transcript[2]
                    body.transcript.splice(2, 1);
                    // 添加requestBody.transcript,设置每项的userId
                    requestBody.transcript.forEach(item => {
                        item.userId = userId;
                    });
                    body.transcript.push(...requestBody.transcript);
