...
// @downloadURL https://update.greasyfork.org/scripts/547046/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29.user.js
// @updateURL https://update.greasyfork.org/scripts/547046/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29.meta.js
// ==/UserScript==
...
(async function () {
  ...
  // ────────────────────────────────────────────────────────────────────────
  // UI helpers (side info bubble, processing bubble) — bottom-right aware
  // ────────────────────────────────────────────────────────────────────────
  function getSafeInsetRight() {
    // Fallback if env() is unsupported; we can't read CSS env directly, add a small cushion.
    return 8; // px
  }
  ...
  function showSideInfoNearEl(el, lines, ms = 5000) {
    ...
    // Position ABOVE the icon near bottom-right
    let rightPx = getSafeInsetRight() + Math.max(0, Math.round(window.innerWidth - (rect?.right ?? window.innerWidth) + 0));
    let bottomPx = getSafeInsetBottom() + Math.max(0, Math.round((rect ? (window.innerHeight - rect.top) : 80) + gap));
    // If that would go too high (tiny screens), place to the left instead
    if (bottomPx > window.innerHeight - 120) {
      bottomPx = getSafeInsetBottom() + 70;
      rightPx = getSafeInsetRight() + Math.max(0, Math.round(window.innerWidth - (rect?.right ?? window.innerWidth) + 0));
    }
  ...
  function showOrUpdateProcessingBubble(targetEl, lines, persist = false) {
    ...
    const preferLeft = leftSpace >= 260; // if enough room, keep to the left (desktop-like)
    if (preferLeft) {
      // LEFT of icon (desktop parity)
      PROCESS_BUBBLE_EL.style.top = `${Math.max(0, Math.round(rect.top))}px`;
      PROCESS_BUBBLE_EL.style.right = `${Math.max(0, Math.round(window.innerWidth - rect.left + GAP))}px`;
      PROCESS_BUBBLE_EL.style.bottom = '';
    } else {
      // ABOVE icon (phones)
      PROCESS_BUBBLE_EL.style.bottom = `${getSafeInsetBottom() + Math.max(0, Math.round(window.innerHeight - rect.top + GAP))}px`;
      PROCESS_BUBBLE_EL.style.right = `${getSafeInsetRight() + Math.max(0, Math.round(window.innerWidth - rect.right))}px`;
      PROCESS_BUBBLE_EL.style.top = '';
    }
  ...
  async function handleShowUrl(btnEl) {
    ...
    panel.innerHTML = `
      ...
      <textarea id="kodishowurl_text" style="width:600px; height:200px; max-width:90vw; font-size:14px; box-sizing:border-box; white-space:pre-wrap;">${info.url}</textarea>
  ...
  // ────────────────────────────────────────────────────────────────────────
  // Floating UI: ICON (launcher) + Settings button — bottom-right placement
  // ────────────────────────────────────────────────────────────────────────
  ...
  GM_addStyle(`
    ...
    /* Prevent bottom toolbars covering it on very small screens */
    @media (max-height: 540px) {
      #lb-tmdb-icon-wrap { bottom: max(14px, env(safe-area-inset-bottom)); }
    }
  `);
  ...
  function editDescriptionOverlay(defaultDesc) {
    return new Promise(resolve => {
      ...
      panel.innerHTML = `
        ...
        <textarea id="kodieditdesc_text" style="width:600px; height:300px; max-width:90vw; font-size:14px; box-sizing:border-box; white-space:pre-wrap;">${defaultDesc}</textarea>
        <div style="margin-top:12px; text-align:right;">
  ...
  mainIcon.addEventListener('click', async function () {
    ...
    // Hide our processing bubble immediately — do NOT show any extra "Sent!" message.
    hideProcessingBubble(true);

    // Show ONLY the original end-of-run info bubble
    const limitBytes = 65536;
  ...
  // ────────────────────────────────────────────────────────────────────────
  // Settings UI (Tabbed + Scrollable) — unchanged visuals (fits mobile)
  // ────────────────────────────────────────────────────────────────────────
