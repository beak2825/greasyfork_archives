/*
 * ============================================================================== *
 ****************************** CodemaoProMax v4.1.3 ******************************
 ************* Copyright (C) 2023 幻想一笑而过、萌新大佬、xiaohong2022 ************
 * ============================================================================== *
*/
...
// @downloadURL https://update.greasyfork.org/scripts/455918/CodemaoProMax.user.js
// @updateURL https://update.greasyfork.org/scripts/455918/CodemaoProMax.meta.js
// ==/UserScript==
var banben = "v4.1.3";
...
(async function () {
    async function run() {
        ...
        function logs(...l){console.log("%cCodemaoProMax","background: #e7520d; border-radius: 3px; padding: 0 4px; color: #fff",...l)};//输出数据
        ...
        try{
            ...
            var CodemaoProMax = {toolfunc,toolfunc2,toolfunc3,toolfunc4,nowmode,beiyongnowmode,isoldmode,isstagingmode};
            updatastate()
            ...
            var codemaopromaxsettings = {
                ...
                save:()=>{
                    localStorage.setItem("cpmset",`${JSON.stringify({
                        ...
                        blur:codemaopromaxsettings.blur,
                        primaryColor:codemaopromaxsettings.primaryColor,
                        borderRound:codemaopromaxsettings.borderRound,
                        windowPos:codemaopromaxsettings.windowPos,
                        left:codemaopromaxsettings.left,
                        top:codemaopromaxsettings.top,
                    })}`)
                ...
                reset:()=>{
                    ...
                    codemaopromaxsettings.autoSave=true;
                    codemaopromaxsettings.primaryColor= "#ff5f42"
                    codemaopromaxsettings.blur= 5
                    codemaopromaxsettings.borderRound= true
                    codemaopromaxsettings.windowPos= [0, 0]
                    codemaopromaxsettings.top = 0
                    codemaopromaxsettings.left = 100
            ...
            function updatamode(m){
                ...
                var init = async(md)=>{
                    ...
                    var datas = {
                        ...
                        "服务条款":()=>{
                            window.open("https://paste.liumingye.cn/9657")
                        },
                        "备用服务条款":()=>{
                            window.open("https://static.box3.codemao.cn/block/QmZbxNc4HKP6TUbWTcRCH4y3VQd3VbKa4bwf1hRexTQGF1_cover_0_0.png")
                        },
                        "更新":()=>{
                            window.open("https://greasyfork.org/scripts/455918-codemaopromax/code/CodemaoProMax.user.js")
                        },
                        "建造256256704地图":async()=>{
                    ...
                    Tooltitle.onmousedown = (en) => {
                        ...
                        document.onmousemove = (e) => {
                            ...
                            codemaopromaxsettings["left"]= (e.clientX - en.offsetX);
                            codemaopromaxsettings["top"]= (e.clientY - en.offsetY);
                            codemaopromaxsettings.save();
                        }
                    }
                    Tooltitle.onmouseup = () => { document.onmousemove = null; }
                    tool.domElement.style.userSelect = 'none';
                    tool.domElement.style.left = Number(codemaopromaxsettings["left"]).toString()+'px';
                    tool.domElement.style.top=Number(codemaopromaxsettings["top"]).toString()+"px";
                    element.style.setProperty("--primary-color", codemaopromaxsettings["primaryColor"]);
                    element.style.setProperty(
                        "--border-radius",
                        codemaopromaxsettings["borderRound"] ? "5px" : "0"
                    );
                    element.style.setProperty("--blur", codemaopromaxsettings["blur"].toString() + "px");
                    element.style.setProperty("--primary-color", codemaopromaxsettings["primaryColor"]);
                    element.style.setProperty("--primary-color", codemaopromaxsettings["primaryColor"]);
                    window["codemaopromaxDiv"]=tool;
                    var p25 = tool.addFolder('⚙️ CodemaoProMax设置（Settings）').close();
                    ...
                    p25.add(codemaopromaxsettings, 'blur', 0, 10, 1).name('背景模糊度').onFinishChange((e)=>{
                        element.style.setProperty("--blur", codemaopromaxsettings["blur"].toString() + "px");
                        if(codemaopromaxsettings.autoSave)codemaopromaxsettings.save();
                    })
                    p25.addColor(codemaopromaxsettings, "primaryColor").name("主题颜色").onChange((e)=>{
                        if(codemaopromaxsettings.autoSave)codemaopromaxsettings.save();
                    })
                    var jianshiqi = p25.addFolder('⚙️ 监视器设置').close();
                    jianshiqi.add(codemaopromaxsettings, 'showmonitor').name('👁️‍🗨️ 显示监视器').onChange((e)=>{
                        statsDiv.domElement.style.display = e?"block":"none";
                        if(codemaopromaxsettings.autoSave)codemaopromaxsettings.save();
                    });
                    jianshiqi.add(codemaopromaxsettings, 'monitorposition',{"左上角":0,"右上角":1,"右下角":2,"左下角":3}).name('监视器位置').onChange((e)=>{
                        var data =[
                            [0,null,0,null],
                            [0,null,null,0],
                            [null,0,null,0],
                    ...
                    jianshiqi.add(codemaopromaxsettings, 'monitortm',10,100,1).name('👁️‍🗨️ 监视器不透明度').onChange((e)=>{
                    ...
                    p25.add(codemaopromaxsettings, 'manualSave').name('💾 保存并刷新');
                    p25.add(codemaopromaxsettings, 'del').name('❌ 删除保存的设置并刷新');
                    p25.add(codemaopromaxsettings, 'reset').name('❌ 重置并刷新');
                    codemaopromaxsettings.updateSettings();
                    ...
                    p2.add(datas, '服务条款').name('❗️ 服务条款');
                    p2.add(datas, '备用服务条款').name('❗️ 服务条款｜备用');
                    p2.add(datas, '作者').name('✨ 作者主页');
                    p2.add(datas, '更新').name('🆕 检查更新');
                    p2.add(datas, 'Q群').name('✉️ 联系我们');
                    ...
                    if(md == 'Play'){
                        ...
                        Object.assign({hashcreatemapdata,zidingyiditujianzaodata},CodemaoProMax)
