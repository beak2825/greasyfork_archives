...
// @downloadURL https://update.greasyfork.org/scripts/530612/MWITools%E5%B0%8Fr%E7%A7%98%E5%88%B6%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/530612/MWITools%E5%B0%8Fr%E7%A7%98%E5%88%B6%E7%89%88.meta.js
// ==/UserScript==
        ...
        if (
            ...
            appendHTMLStr += `<div style="color: ${SCRIPT_COLOR_TOOLTIP};">${isZH ? "利润: " : "Profit: "}${numberFormatter(profitPerHour)}${
                isZH ? "/小时" : "/hour"
                            ...
                            // 小r添加数据
                appendHTMLStr += `<div style="color: ${SCRIPT_COLOR_TOOLTIP}; font-size: 10px; margin-top:5px;">
                    <div style="display: inline-block; width: 80px;">${isZH ? "售价" : "Price"}</div>
                    <div style="display: inline-block; width: 80px;text-align: right;">${isZH ? "小时利润" : "Hourly"}</div>
                    <div style="display: inline-block; width: 80px;text-align: right;">${isZH ? "天利润" : "Daily"}</div>
                </div>`;

                for(var i=0; i<=6;i++){
                    var k = (bid + (ask - bid)*i/5).toFixed(0);
                    var k2 = k*0.98;
                    const hourlyProfit = itemPerHour * (k2 - totalResourcesAskPrice) + extraFreeItemPerHour * k2 - drinksConsumedPerHourAskPrice;
                    const dailyProfit = 24 * hourlyProfit;
                    const colorStyle = hourlyProfit > 1000000 ? "#228b22" : SCRIPT_COLOR_TOOLTIP;

                    appendHTMLStr += `<div style="color: ${colorStyle}; font-size: 10px;">
                        <div style="display: inline-block; width: 80px;">${k}</div>
                        <div style="display: inline-block; width: 80px; text-align: right;">${numberFormatter(hourlyProfit)}</div>
                        <div style="display: inline-block; width: 80px; text-align: right;">${numberFormatter(dailyProfit)}</div>
                    </div>`;
                }
             appendHTMLStr += `<div style="color: #5555ff;">${itemName}</div>`;
        }
