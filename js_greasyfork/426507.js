...
// @downloadURL https://update.greasyfork.org/scripts/426507/Sci-hub%20button.user.js
// @updateURL https://update.greasyfork.org/scripts/426507/Sci-hub%20button.meta.js
// ==/UserScript==

var sciHubBaseUrl = 'https://sci-hub.shop/'; // Set sci-hub url
...
// The latest Sci-Hub working domain(Last check time:Sat, 02 May 2020 17:13:01 GMT)
// 推荐使用以下网址：
...
var floatBox = function () {
    ...
    if (!site.startsWith(sciHubBaseUrl)) {
        ...
        else if (/http:\/\/xueshu\.baidu\.com\/usercenter\/paper/i.test(site) ) {
        ...
        if (site != orig_link && !orig_link.startsWith(sciHubBaseUrl)) {
            ...
            if (/^https?:\/\/[\w\.]+\/doi\/pdf\/10\./i.test(orig_link) ||
                ...
                if (/^https?:\/\/(dx\.)?doi\.org\/10\.1371/i.test(orig_link)) {
                    ...
                    addScihubIco("https://www.biomedcentral.com/track/epub/" + doi, link[i], epub_ico);
                    addScihubIco("https://www.biomedcentral.com/track/pdf/" + doi, link[i], pdf_ico);
            ...
            // 开放获取期刊提供直接下载链接
                ...
                addScihubIco("https://www.biomedcentral.com/track/epub/10.1186/" + paper_id, link[i], epub_ico);

                addScihubIco("https://www.biomedcentral.com/track/pdf/10.1186/" + paper_id, link[i], pdf_ico);
            }
                ...
                addScihubIco("https://www.biomedcentral.com/track/epub/" + paper_id, link[i], epub_ico);

                addScihubIco("https://www.biomedcentral.com/track/pdf/" + paper_id, link[i], pdf_ico);
            }
            ...
            else if (/^https?:\/\/(www\.)?aasv\.org\/shap\/issues\/\w+\/\w+\.html/i.test(orig_link)) {
                // AASV
            ...
            else if (/\.wanfangdata\.com\.cn\//i.test(orig_link)) {
                addScihubIco(getLuooqiLink(link[i].text), link[i], fish_ico);
...
var getWanfangLink = function (orig_link) {
    if (/\.wanfangdata\.com\.cn\/Paper\/Detail\?/i.test(orig_link)) {
    ...
    else if (/d\.(g\.)?wanfangdata\.com\.cn\/(Periodical|Thesis|Conference|Patent)_[\w\d-\.]+\.aspx/i.test(orig_link)) {
        // 如果设置了万方镜像地址，增加跳转到镜像按钮
    ...
    else if (/\.wanfangdata\.com\.cn\/(Periodical|Thesis|Conference|Patent)\//i.test(orig_link)) {
        // 如果设置了万方镜像地址，增加跳转到镜像按钮
        ...
        paper_id = /(Periodical|Thesis|Conference|Patent)\/([^&]+)/i.exec(orig_link)[2];
...
var getSciHubLink = function (orig_link) {
    ...
    // 网址包含PMCid
        ...
        return sciHubUrl;
               ...
               /^https?:\/\/(\w+\.)?pnas\.org\/content\/early\/\d+\/\d+\/\d+\/\d+?$/i.test(orig_link) ||
