// ==UserScript==
// @name         空城里链接助手
// @namespace    https://www.ilzya.com/archives/4/
// @version      1.0.3
// @description  免等待下载文件，超酷的哎~
// @author       空城里
// @icon         data:image/jpeg;base64,
// @license AGPL
 
 
// @include        *://www.feimaoyun.com/*
// @include        *://*.lanzou*.com/*
// @include        *://*.77file.com/*
// @include        *://*.xunniupan.com/*
// @include        *://*.567yunpan.com/*
// @include        *://dufile.com/*
// @include        *://*.expfile.com/*
// @include        *://*.xueqiupan.com/*
// @include        *://*.xfpan.cc/*
// @include        *://ownfile.net/*
// @include        *://*.dudujb.com/*
// @include        *://*.yifile.com/*
// @include        *://*.1988wp.com/*
// @include        *://rosefile.net/*
// @include        *://*.xingyaopan.com/*
// @include        *://*.kufile.net/*
 
 
 
 
 
// @grant        GM_xmlhttpRequest
// @downloadURL https://update.greasyfork.org/scripts/449206/%E7%A9%BA%E5%9F%8E%E9%87%8C%E9%93%BE%E6%8E%A5%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/449206/%E7%A9%BA%E5%9F%8E%E9%87%8C%E9%93%BE%E6%8E%A5%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
 
(function () {
    'use strict';
 
    let durl = null;
 
 
    let div_main = document.createElement('div');
    div_main.style.width = '150px';
    div_main.style.height = '345px';
    div_main.style.position = 'fixed';
    div_main.style.right = 0;
    div_main.style.top = '150px';
    div_main.style.border = 'medium solid #ff3300';
 
 
 
    let title = document.createElement("h3");
    title.innerHTML = '网盘VIP下载器'
    title.style.textAlign = 'center';
 
 
 
 
 
 
    let styleMap = {
        display: "inline-block",
        "background-color": "red",
        cursor: "pointer",
        "user-select": "none",
        "width": "100px",
        height: "28px",
        "border-radius": "16px",
        color: "#fff",
        "font-size": "14px",
        "line-height": "28px",
        "text-align": "center",
        padding: "0px 10px",
        "margin-left": "16px",
        "margin-top": "10px",
    };
    //解析按钮
    let btn_resolve = document.createElement("div");
    btn_resolve.innerHTML = "获取下载链接";
    btn_resolve.onclick = function () {
        nodejs.resolve(document.location)
    }
 
    //下载按钮
    let btn_down = document.createElement('div');
    btn_down.innerHTML = '浏览器下载';
    btn_down.onclick = function () {
        nodejs.down(durl)
    }
 
    //博客
    let btn_go_blog = document.createElement("div");
    btn_go_blog.innerHTML = "作者博客";
    btn_go_blog.onclick = function () {
        window.open('https://www.ilzya.com/');
    }
 
 
    //交流群
    let btn_join_qq_group = document.createElement("div");
    btn_join_qq_group.innerHTML = "QQ交流群";
    btn_join_qq_group.onclick = function () {
        window.open('https://jq.qq.com/?_wv=1027&k=ulFSrgM9');
    }
 
 
    //交流群
    let btn_join_tg_group = document.createElement("div");
    btn_join_tg_group.innerHTML = "TG群";
    btn_join_tg_group.onclick = function () {
        window.open('https://t.me/ilzyaOwO');
    }
 
 
    //交流群
    let btn_donate = document.createElement("div");
    btn_donate.innerHTML = "赞助我";
    btn_donate.onclick = function () {
        window.open('https://www.ilzya.com/donate.html');
    }
 
 
 
 
    // 添加样式
    for (let i in styleMap) {
        btn_resolve.style[i] = styleMap[i];
        btn_down.style[i] = styleMap[i];
        btn_go_blog.style[i] = styleMap[i];
        btn_join_qq_group.style[i] = styleMap[i];
        btn_join_tg_group.style[i] = styleMap[i];
        btn_donate.style[i] = styleMap[i];
 
    }
 
 
    //分割线
    let divider = document.createElement("div");
    divider.style.width = '100%';
    divider.style.height = '1px'
    divider.style.backgroundColor = 'black';
    divider.style.marginTop = '20px';
 
    //插件信息
    let info = document.createElement("div");
    info.innerHTML = '插件信息';
    info.style.height = '1px'
    info.style.marginTop = '10px';
    info.style.textAlign = 'center';
 
    //解析结果
    let resultMsg = document.createElement("div");
    resultMsg.innerHTML = '解析结果:null';
    resultMsg.style.height = '1px'
    resultMsg.style.marginTop = '20px';
    resultMsg.style.textAlign = 'center';
    resultMsg.style.color = 'red';
 
 
    //分割线
    let divider1 = document.createElement("div");
    divider1.style.width = '100%';
    divider1.style.height = '1px'
    divider1.style.backgroundColor = 'black';
    divider1.style.marginTop = '30px';
 
 
 
 
    document.body.append(div_main);
    div_main.appendChild(title);
    div_main.appendChild(btn_resolve);
    div_main.appendChild(btn_down);
    div_main.appendChild(divider);
    div_main.appendChild(info);
    div_main.appendChild(resultMsg);
    div_main.appendChild(divider1);
    div_main.appendChild(btn_go_blog);
    div_main.appendChild(btn_join_qq_group);
    div_main.appendChild(btn_join_tg_group);
    div_main.appendChild(btn_donate);
 
 
 
    let nodejs = {};
 
    nodejs.resolve = function (val) {
        GM_xmlhttpRequest({
            method: "post",
            url: 'http://api.ilzya.com:1306/resolve',
            data: 'url=' + val,
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            onload: function (res) {
                let result = JSON.parse(res.response);
                resultMsg.innerHTML = "解析结果:" + result.message
                if (result.code === 0) {
                    durl = result.durl;
                }
 
 
            }
        });
    }
 
    nodejs.down = function (val) {
        if (val != null && val != undefined && val != '') {
            window.open(val)
        } else {
            alert('请先获取下载链接,在进行下载！')
        }
    }
 
 
})();