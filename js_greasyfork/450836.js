...
// @author KS-MLC
// @version 3.7
// @description 视频解析:爱奇艺、腾讯视频、优酷、乐视、芒果TV、搜狐、1905、PPTV、B站。
// @namespace https://greasyfork.org/users/954101
// @include *
// @match *
// @license MIT *
// @downloadURL https://update.greasyfork.org/scripts/450836/%E8%A7%86%E9%A2%91%E8%A7%A3%E6%9E%90PC%E7%AB%AF.user.js
// @updateURL https://update.greasyfork.org/scripts/450836/%E8%A7%86%E9%A2%91%E8%A7%A3%E6%9E%90PC%E7%AB%AF.meta.js
// ==/UserScript==
(function() {
	/*
	 * 自定义区域
	 ...
	 */
	// 主颜色(图标整体颜色,如方框颜色)
	const mianColor = "linear-gradient(to bottom right, red, yellow)";
	// 副颜色(图标层次颜色,如字体颜色)
	...
	const iconFilletPercent = 0.2;
	// 解析接口菜单框展开的高度
	var developMenuHeight = 315;
	// 解析接口菜单框展开的速度（如果展开动画卡顿请设置0,单位是秒）
	var developMenuSecond = 0;
	// 解析接口（可多个）
	const parseInterfaces =["https://api.jiexi.la/?url=","https://jx.m3u8.tv/jiexi/?url=","https://dmjx.m3u8.tv/?url=","https://www.ckplayer.vip/jiexi/?url=","https://jx.xmflv.com/?url=","https://okjx.cc/?url=","https://api.okjx.cc:3389/jx.php?url="];
	 ...
	 */
	// 视频网站(规则已定，不可随意更改)
	const videoSites = ["v.qq.com","tv.sohu.com","iqiyi.com","youku.com","mgtv.com","m.le.com","www.le.com","1905.com","pptv.com","bilibili.com","m.iqiyi.com"];
    ...
    // 视频播放框类ID
	var classAndIDMap	= {"pc":{"v.qq.com":"player-container","iqiyi.com":"flashbox","youku.com":"ykPlayer","mgtv.com":"mgtv-player-wrap","sohu.com":"x-player","le.com":"fla_box","1905.com":"player","pptv.com":"pplive-player","bilibili.com":"bilibiliPlayer|player_module|bilibili-player|bilibili-player-video-wrap|player-limit-mask"},"mobile":{"v.qq.com":"mod_player","iqiyi.com":"m-box","youku.com":"h5-detail-player","mgtv.com":"video-area","sohu.com":"player-view","le.com":"playB","1905.com":"player","pptv.com":"pp-details-video","bilibili.com":"bilibiliPlayer|player-wrapper"}};
    // 创建图标
    createIcon();
	// 判断页面加载完成以后图标是否存在
    ...
    function createIcon(){
        try{
            ...
            //显示接口文字
            var jiekName = [":1080P",":1080P",":1080P",":1080P",":1080P",":720P",":720P"];
            for(var i in parseInterfaces){
                if(i==parseInterfaces.length-1){
                    html += "<span class='spanStyle' style='"+MenuItemsStyle+"' url='"+parseInterfaces[i]+"'>"+(parseInt(i)+1)+jiekName[i]+"</span>";
                }else{
                    html += "<span class='spanStyle' style='"+MenuItemsStyle+"border-bottom-style:solid;' url='"+parseInterfaces[i]+"'>"+(parseInt(i)+1)+jiekName[i]+"</span>";
                }
            }
            html += "</div></div>";
            div.innerHTML = html;
