...
// @name              百度网盘下载助手
// @version           1.2.2
// @author            高乐喆（Greasy Fork用户名：lezhe）、油小猴
...
// @description       可以获取网盘文件真实下载地址。现已支持百度网盘、阿里网盘、天翼网、盘迅雷网盘、夸克网盘、移动网盘（139网盘）六大网盘，基于【网盘直链下载助手】修改自1.0.8.2版本,自用,去推广,修原有BUG,修改界面,甚至比原版还好用！
...
// @namespace         https://github.com/Gao-Lezhe/Online-disk-direct-link-download-assistant
// @downloadURL https://update.greasyfork.org/scripts/484953/%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E4%B8%8B%E8%BD%BD%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/484953/%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E4%B8%8B%E8%BD%BD%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
			...
			return new Promise((resolve, reject) => {
				const sendRequest = () => {
					let requestObj = GM_xmlhttpRequest({
						...
						onloadstart() {
							console.log('【网盘下载助手】Get OnloadStart\n请求地址：' + url + '\n请求头部：' + JSON.stringify(headers));
	...
	*/
	...
	let main = {
		init() {
			...
			if (/www.(aliyundrive|alipan).com/.test(location.host)) {//阿里网盘
				ali.initPanLinker();
			}
			if (/cloud.189.cn/.test(location.host)) {//天翼网盘
				tianyi.initPanLinker();
			}
			if (/pan.xunlei.com/.test(location.host)) {//迅雷网盘
				xunlei.initPanLinker();
			}
			if (/pan.quark.cn/.test(location.host)) {//夸克网盘
				quark.initPanLinker();
			}
			if (/(yun|caiyun).139.com/.test(location.host)) {//139网盘
