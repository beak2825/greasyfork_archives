...
// @version      8
// @author       You
// @match        https://gamdom.com/chat
// @require http://code.jquery.com/jquery-1.12.4.min.js
// @grant        none
// @description  tipscyka
// @downloadURL https://update.greasyfork.org/scripts/374259/tipbot.user.js
// @updateURL https://update.greasyfork.org/scripts/374259/tipbot.meta.js
// ==/UserScript==
...
setTimeout(()=>{
	...
	$(document).ready(function(){
	    setInterval(function(){
			...
			if(tiptoalt===true && sleepmoscow==false){
				let reward = 625;
				console.log("tiptoalt = true into if");
				let ul = document.getElementsByClassName("msg-chat-message");
				inputsArray = Array.prototype.slice.call(ul, 0).filter(function (el)
				{
					return !(el.classList.contains('rain-message'));
    			})
    			rinputsArray = Array.prototype.slice.call(ul, 0).filter(function (el)
				{
					return !(el.classList.contains('msg-info-message'));
    			});
				for (let i = 0,len = ul.length; i < len; i++){
					...
					//for (let j = 0; j < li.childNodes.length; j++){
						...
						console.log("3");
							if(child.childNodes[2].childNodes[0]!==undefined)
							{
								console.log("3.5");
					...
					{
						console.log("broke from undermined");
						break;
					}
		...
		$(document).arrive(".success", function() {
			...
			if(text.includes("передали пользователю")){
				//if((!text.includes("muted") && !text.includes("You have given") && !text.includes("Вы передали") && !text.includes("Success!")))
				//{
					setTimeout(()=>{
						$(".success").click();
