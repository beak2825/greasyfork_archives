...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/541787/Lmarena%20AI.user.js
// @updateURL https://update.greasyfork.org/scripts/541787/Lmarena%20AI.meta.js
// ==/UserScript==
...
(function() {
    ...
    let refresh_page_url='https://lmarena.ai/zh/c/new?mode=direct'
    // 日志函数
    ...
    async function processPrompt(data) {
        try {
            ...
            // 直接发送请求
            const response = await window.fetch(   `https://lmarena.ai/${data.backend_url || 'nextjs-api/stream/create-evaluation'}`, {
                method: 'POST',
                headers: {
    ...
    async function retryPrompt(data) {
        try {
            ...
            const baseurl = `https://lmarena.ai/${data.backend_retry_url || 'nextjs-api/stream/retry-evaluation-session-message'}/${holdChatConvertion.id}/messages/${holdChatConvertion.modelAMessageId}`
            // 直接发送请求
    ...
    async function post_to_evaluation(data) {
        try {
            ...
            const baseurl = `https://lmarena.ai/${data.backend_retry_url || 'nextjs-api/stream/post-to-evaluation'}/${holdChatConvertion.id}`
            // 直接发送请求
            const response = await window.fetch(baseurl, {
                method: 'POST',
                headers: {
