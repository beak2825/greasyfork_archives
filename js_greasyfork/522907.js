...
// @version           1.0.15
// @updateDesc        观看全站vip金币视频
// @description       🔥观看全站vip金币视频，复制播放链接，下载视频(日限)，去广告🔥
// @icon              https://vip.yotade.cc/static/txh.png
// @namespace         https://vip.yotade.cc/detail?app_id=0003
// @author            yota
// @include           *://*.txh05*.com/*
// @include           *://txh05*.com/*
// @include           *://txh*.com/*
...
// @downloadURL https://update.greasyfork.org/scripts/522907/%E7%B3%96%E5%BF%83%7C%E8%A7%82%E7%9C%8Bvip%E9%87%91%E5%B8%81%E8%A7%86%E9%A2%91.user.js
// @updateURL https://update.greasyfork.org/scripts/522907/%E7%B3%96%E5%BF%83%7C%E8%A7%82%E7%9C%8Bvip%E9%87%91%E5%B8%81%E8%A7%86%E9%A2%91.meta.js
// ==/UserScript==
                ...
                methods: {
                    ...
                    interceptXHR() {
                        ...
                        XMLHttpRequest.prototype.open = function (method, url) {
                            if (url.endsWith("/movie/detail") || url.endsWith('album/search') || url.endsWith('album/photoList') || url.endsWith('post/search') || url.endsWith('.m3u8')) {
                                this.requestUrl = url;
                            }
                            if (url.endsWith('movie/block')) {
                                that.findElement('.van-dialog', 20).then(r => {
                                    if (r.__vue__) {
                                        if (r.__vue__.touchMove) {
                                            r.__vue__.touchMove = null
                                        } else {
                                            r.__vue__.$parent.touchMove = null
                                        }
                                    }
                                })
                            }
                        ...
                        XMLHttpRequest.prototype.send = async function (...args) {
                            if (this.requestUrl) {
                                await that.interceptXHRSend3(this, this.requestUrl, args);
                                // await that.interceptXHRSend2(this, this.requestUrl, args);
                            }
                    ...
                    async interceptXHRSend3(that, url, params) {
                        if (url.endsWith("/movie/detail") || url.endsWith('album/search') || url.endsWith('album/photoList') || url.endsWith('post/search')) {
                            ...
                            try {
                                ...
                                let res = await this.reqByCROS(`${this.server}/${this.entrance}/parsebyreq`, 'POST', headers, JSON.stringify(data))
                                if (res.code == 200) {
                                    let res_data = JSON.parse(res.data);
                                    if (res_data.code == 200) {
                                        let resp = res_data.data.resp;
                                        this.replaceResp(that, resp);
                                        this.showToast("解析成功")
                                        this.play_state = 'success';
                                        // this.openTabAndPlay()
                                    } else {
                    ...
                    async interceptXHRSend2(that, url, params) {
                        if (url.endsWith("/movie/detail")) {
                            ...
                            try {
                                ...
                                let res = await this.reqByCROS(`${this.server}/${this.entrance}/parse`, 'POST', headers, JSON.stringify(data))
                                if (res.code == 200) {
                                    let res_data = JSON.parse(res.data);
                                    if (res_data.code == 200) {
                                        // this.video_code = res_data.data.video_code;
                                        let res_data = res_data.data;
                                        // this.showToast("解析成功")
                                        this.play_state = 'success';
