// ==UserScript==
// @name         diamond-community-jg
// @namespace    http://tampermonkey.net/
// @version      1.2.2
// @description  钻石社区薅羊毛(分享版)
// @author       You
...
// @license     MIT
// @downloadURL https://update.greasyfork.org/scripts/475335/diamond-community-jg.user.js
// @updateURL https://update.greasyfork.org/scripts/475335/diamond-community-jg.meta.js
// ==/UserScript==
...
function addGetLikeBtn() {
     ...
     $('a.topControl.trans').before(`<a class="quickReply2 trans" href="javascript:;"></a>`);
...
async function reply(specialType) {
     ...
     for (let j = 0; j < 5; j++) {
          ...
          const path = pathname.endsWith('/') ? pathname.substring(0, pathname.lastIndexOf('/')) : pathname;
          const postId = path.substring(path.lastIndexOf('/') + 1);
          const textHtml = '<p><img class="emjoy-ng" src="/iflyteksns/template/1/default/files/js/ueditor/dialogs/emotion/images/diamond3/di_0014.gif" _src="/iflyteksns/template/1/default/files/js/ueditor/dialogs/emotion/images/diamond3/di_0014.gif">像优秀的同事们学习</p>';
...
async function allReply() {
     ...
     let nomalPostIds = await getNormalPostIds();
     let postIds = specialPostIds.concat(nomalPostIds);
     //let postIds = nomalPostIds;
