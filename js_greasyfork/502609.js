// ==UserScript==
// @name   🌱【免费版】德诚在线|西北师范大学-专业技术人员继续教育培训专业选修课|使用说明：http://doc.zhanyc.cn/pages/deczx/
// @namespace    http://jb.zhanyc.cn/
// @icon    https://js.zhanyc.cn/img/js-logo.svg
// @version      2.2
// @description  当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制、自动下一集功能。如需自动换课程、全自动无人值守等高级功能可升级付费版本，一杯咖啡钱，保你无忧学习，且永久使用|接各类脚本开发工作，微信：zhanyc_cn 备用微信:zhanfengkuo 个人网站：http://doc.zhanyc.cn
// @author       zfk
// @include    *://*.wkw.net.cn/*
// @include    *://*.nweduline.com/*
// @include    *://*.cc-edu.cn/*
...
// @antifeature 
// @license GPL
// @downloadURL https://update.greasyfork.org/scripts/502609/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E5%BE%B7%E8%AF%9A%E5%9C%A8%E7%BA%BF%7C%E8%A5%BF%E5%8C%97%E5%B8%88%E8%8C%83%E5%A4%A7%E5%AD%A6-%E4%B8%93%E4%B8%9A%E6%8A%80%E6%9C%AF%E4%BA%BA%E5%91%98%E7%BB%A7%E7%BB%AD%E6%95%99%E8%82%B2%E5%9F%B9%E8%AE%AD%E4%B8%93%E4%B8%9A%E9%80%89%E4%BF%AE%E8%AF%BE%7C%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E%EF%BC%9Ahttp%3Adoczhanyccnpagesdeczx.user.js
// @updateURL https://update.greasyfork.org/scripts/502609/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E5%BE%B7%E8%AF%9A%E5%9C%A8%E7%BA%BF%7C%E8%A5%BF%E5%8C%97%E5%B8%88%E8%8C%83%E5%A4%A7%E5%AD%A6-%E4%B8%93%E4%B8%9A%E6%8A%80%E6%9C%AF%E4%BA%BA%E5%91%98%E7%BB%A7%E7%BB%AD%E6%95%99%E8%82%B2%E5%9F%B9%E8%AE%AD%E4%B8%93%E4%B8%9A%E9%80%89%E4%BF%AE%E8%AF%BE%7C%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E%EF%BC%9Ahttp%3Adoczhanyccnpagesdeczx.meta.js
// ==/UserScript==
(function () {
    ...
    let freeTips = " 当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制、自动下一集功能。如需自动换课程、全自动无人值守等高级功能，可点击下方按钮查看付费版本"
    let docUrl = "http://doc.zhanyc.cn/pages/deczx/";
    let plugMain = Object.assign(baseConfig, {
        ...
        async runByUrl(url) {
            if (url.includes("/index/classStudy?")) {
                plugMain.page_videoCtrl();
            } else if (url.includes("/project/index?")) {
                plugMain.showPaidContent(`免费版本不包含自动换课功能，如需使用请安装收费版本`);
            } else if (url.includes("preview.dccloud.com.cn/")) {
                plugMain.tipsAndClose();
            }

        },

        async page_videoCtrl() {
            console.log("%c page_videoCtrl", "background:rgb(0,0,0);color:#fff");
            plugMain.closeWaitConfrimWin();
            await plugMain.waitOf((a) => $(".video_round").length > 0);
            await plugMain.waitTimeout(1000);
            if ($(".video_round").not(".study").length == 0) {
                plugMain.confirmRun("当前课程已经学习完毕")
                plugMain.showPaidContent(`免费版本不包含自动换课功能，如需使用请安装收费版本`);
                return;
            }
            $(".video_round").not(".study").eq(0).next().click();
            zfk
                .waitOf(
                    (a) => plugMain.getElByText($(".el-message-box button"), "确定") != null
                )
                .then((a) => {
                    plugMain.getElByText($(".el-message-box button"), "确定").click();
                });
            plugMain.page_video();
        },
        async page_video_fileFun() {
            console.log("%c page_video_fileFun", "background:rgb(0,0,0);color:#fff");
            if ($(".file-box").length == 0) {
                return;
            }
            plugMain.showPaidContent(`免费版本不包含自动学习文档功能，如需使用请安装收费版本`);
        },
        async page_video_file() {
            console.log("%c page_video_file", "background:rgb(0,0,0);color:#fff");
            if (plugMain.pageData.videoFileIndex != null) {
                return;
            }
            plugMain.page_video_fileFun();
        },
        async page_video() {
            console.log("%c page_video", "background:rgb(0,0,0);color:#fff");
            let timeout = 2;
            zfk
                .waitOf((a) => $(".file-box").length > 0)
                .then((a) => {
                    plugMain.page_video_file();
                });
            await plugMain.waitOf(
                (a) =>
                    $(".video-swiper .swiper-slide").length > 0 || $("video").length > 0
            );
            await plugMain.waitTimeout(1000);
            if ($(".video-swiper .swiper-slide").length > 0) {
                if (
                    $(".video-swiper .swiper-slide.swiper-slide-active .video-status").is(
                        ".success"
                    )
                ) {
                    $(".video-swiper .swiper-slide .video-status")
                        .not(".success")
                        .eq(0)
                        .click();
                }
