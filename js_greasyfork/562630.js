// ==UserScript==
// @name         国家中小学智慧教育平台教材PDF下载 修改版
// @namespace    https://greasyfork.org/zh-CN/scripts/562630
// @version      4.0.0
// @description  教材列表页与预览页添加了PDF按钮，免登录查看或下载电子课本与课外书籍，可批量下载，支持新课标教材
// @description  原作者失效 更新修改  https://greasyfork.org/zh-CN/scripts/466598
...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/562630/%E5%9B%BD%E5%AE%B6%E4%B8%AD%E5%B0%8F%E5%AD%A6%E6%99%BA%E6%85%A7%E6%95%99%E8%82%B2%E5%B9%B3%E5%8F%B0%E6%95%99%E6%9D%90PDF%E4%B8%8B%E8%BD%BD%20%E4%BF%AE%E6%94%B9%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/562630/%E5%9B%BD%E5%AE%B6%E4%B8%AD%E5%B0%8F%E5%AD%A6%E6%99%BA%E6%85%A7%E6%95%99%E8%82%B2%E5%B9%B3%E5%8F%B0%E6%95%99%E6%9D%90PDF%E4%B8%8B%E8%BD%BD%20%E4%BF%AE%E6%94%B9%E7%89%88.meta.js
// ==/UserScript==

(function() {
    'use strict';

    // �?Cookie 中提�?access_token
    function getAccessTokenFromCookie() {
        const cookies = document.cookie.split(';');
        for (const cookie of cookies) {
            const [name, value] = cookie.trim().split('=');
            if (name && name.includes('UC_TOKEN') && name.includes('ncet-xedu')) {
                try {
    ...
    // 优先�?Cookie 获取，否则使用存储的�?
    const cookieToken = getAccessTokenFromCookie();
    const macId = cookieToken || GM_getValue("macId", "0");
    ...
    function fullscreenSwitch() {
        ...
        if (courseDocument.classList.contains("full-screen")) { // 当前处于"全屏状�?，切换为"非全屏状�?
            courseDocument.classList.remove("full-screen");
            html.style.overflow = "";
            document.querySelector("iframe")?.scrollIntoView({
                block: "nearest"
            });
        } else { // 当前处于"非全屏状�?，切换为"全屏状�?
    ...
    if (window.self !== window.top) {
        ...
        if (verticalToolbarSeparator) {
            ...
            fullscreenBtn.addEventListener("click", () => {
                window.parent.postMessage("full-screen mode switches", "*"); // iframe 向主页面发送消息，"*" 表示允许发送到任意源的主页�?
            });
        }
    }
    ...
    function downloadFile(fileUrl, element2, progressText, fileName, completeText) {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                if (response.status === 200) {
                    ...
                    element2.innerText = "下载失败 (" + response.status + ")";
                }
            },
            onerror: function(error) {
                console.error("下载出错:", error);
                element2.innerText = "下载出错";
            }
        });
    }
    ...
    function AddBtnsToListPg(i, title, pdfUrl) {
        // 兼容新旧版本类名
        let allItems = document.querySelectorAll("li[class*='index-module_items_']");
        if (allItems.length === 0) {
            allItems = document.querySelectorAll("li[class*='index-module_item_']");
        }
        const container = allItems[i];
        container?.querySelector(".PDF-btns")?.remove(); // 去除旧按�?
        // 设置父容器为相对定位
        container.style.position = "relative";
        const PDF_btns = document.createElement("div"); // 创建 PDF_btns
        PDF_btns.setAttribute("class", "PDF-btns");
        PDF_btns.setAttribute("style", "position: absolute; right: 24px; top: 50%; transform: translateY(-50%); display: flex; align-items: center;");
        PDF_btns.innerHTML = `
        ...
        `;
        container.appendChild(PDF_btns); // �?PDF_btns 添加到父元素�?
        // 兼容新旧版本类名
        const contentEl = document.querySelectorAll("[class*='index-module_content_']")[i] || document.querySelectorAll("[class*='index-module_cover_']")[i]?.parentElement;
        if (contentEl) contentEl.style.width = "550px";
        PDF_btns.addEventListener("click", function(event) {
            event.stopPropagation();
        });
        const element2 = PDF_btns.querySelector("a.fish-btn.fish-btn-primary");
        const progressText = "下载中";
        const fileName = (title || document.querySelectorAll("div[class*='index-module_line_']")[i]?.querySelector("span")?.getAttribute("title") || "PDF") + ".pdf";
    ...
    XMLHttpRequest.prototype.open = function(method, url) {
        if (window.location.pathname === "/tchMaterial" && url.includes("query?res_ids=")) { // 获取列表页各 id
            const ids = url.match(/res_ids=([^&]+)/)[1].split(","); // 多个 id 构成的数�?// 匹配�?url 中“query?res_ids=”之后的内容，并将匹配出来的内容以其中的�?”隔开，形成多个“id�?
                        const jsonUrls = ids.map(id => `https://s-file-1.ykt.cbern.com.cn/zxx/ndrv2/resources/tch_material/details/${id}.json`); // 多个 jsonURL 构成的数�?
                        const jsonUrlsThe2nd = ids.map(id => `https://s-file-1.ykt.cbern.com.cn/zxx/ndrs/special_edu/thematic_course/${id}/resources/list.json`); // 多个 jsonURL 构成的数�?
            ...
            for (let i = 0; i < jsonUrls.length; i++) {
                fetch(jsonUrls[i])
                    ...
                    .then(text => {
                        ...
                        let pdfUrl = text.match(/\bhttps?:\/\/[^"]*pkg[^"]*\.pdf\b/g).slice(-1)[0];
                ...
                if (this.readyState === 4 && this.status === 200) {
                    try {
                        ...
                        if (url.includes("resources/tch_material/details")) {
                            ...
                            pdfUrl = text.match(/\bhttps?:\/\/[^"]*pkg[^"]*\.pdf\b/g).slice(-1)[0];
                        } else if (url.includes("special_edu/thematic_course") && url.endsWith("list.json")) {
                            const urlList = text.match(/\bhttps?:\/\/(?!r1|r2)[^"]*pkg[^"]*\.(pdf|pptx?)\b/g);
            ...
            const suggestBtn = document.querySelector("[class*='index-module_suggestion-wrap']");
            if (suggestBtn && !document.querySelector(".mp3")) {
                ...
                if (!document.querySelector("[class*='index-module_wrapper_'] > [class*='imageList-module_special-edu-image_']")) { // 非常规界面空间不�?
                ...
                element1.addEventListener("click", function(event) { // 点击下载
                    ...
                    xhr.onloadstart = function() { // 初始化进�?
                        element1.innerText = "📼 下载�?...";
                    };
                    xhr.onprogress = function(event) {
                        if (event.lengthComputable) {
                            var progress = Math.round((event.loaded / event.total) * 100);
                            element1.innerText = "📼 下载�?(" + progress + "%)";
                        }
                    };
                    xhr.onload = function() {
                        if (xhr.status === 200) {
                            ...
                            downloadLink.href = URL.createObjectURL(blob);
                            downloadLink.download = document.querySelector(".audioList-module_audio-item_GGkA9.audioList-module_audio-item-active_Xx7f- .audioList-module_center_MjbID").innerText || "MP3"; // 文件�?
                            downloadLink.dispatchEvent(new MouseEvent("click"));
                            element1.innerText = "📼 下载MP3"; // 下载完成后恢复按钮文�?
                        } else {
    ...
    const maxTimeToCheck = 15000; // 最多检�?15000 毫秒，即 15 �?
    let elapsedTime = 0;
    ...
    function checkLoginModal() {
        ...
        if (loginModal) {
            clearInterval(intervalId1); // 清除定时�?
            // 去除阻碍
            loginModal.remove(); // 去除遮罩
            const body = document.querySelector("html > body"); // 去除 body 属�?
  ...
  <div class="index-module_header_tG-zz">
    ...
    <div class="index-module_info_evO1d">
      <span class="index-module_origin_nuihE">
        ...
        <span class="index-module_department_ewVZW">智慧中小�?/span>
      </span>
      <div class="index-module_assesment-detail_jhGLz">
        <div class="index-module_assessment-content_06v6Z">
          <div class="fish-dropdown-trigger index-module_assessment-rate_euPTQ">
            ...
            <span class="index-module_text_rukZW">5.0�?/span>
          </div>
        </div>
      ...
      <div class="index-module_extra_tUQog">
        <div class="index-module_like-wrap_NbyLe  ">
          ...
          <div class="index-module_like-count_GXOGd">100�?</div>
        </div>
      ...
      <div class="course-document">
        <div class="document-context" style="overflow: hidden; height: 100%;">
          <iframe id="pdfPlayerFirefox" src='/pdfjs/2.13/web/viewer.html?file=${pdfUrl}&headers=%7B"X-ND-AUTH":"MAC%20id=%5C"${macId}%5C",nonce=%5C"0%5C",mac=%5C"0%5C""%7D#disablestream=true&disableAutoFetch=true&page=1' allowfullscreen="" frameborder="0" height="100%" width="100%"></iframe>
            ...
            // 重定向至登录�?
            const toLogin = [
                document.querySelector("[class*='fish-dropdown-trigger'][class*='index-module_assessment-btn_']"),
                document.querySelector("[class*='index-module_like-wrap_']"),
                document.querySelector("[class*='index-module_suggestion-wrap']")
            ];
            ...
            if (elapsedTime >= maxTimeToCheck) {
                clearInterval(intervalId1); // 清除定时器，停止检�?
            }
        }
    }
    const intervalId1 = setInterval(checkLoginModal, 100); // 每隔 100 毫秒检查一�?.fish-modal-root 元素是否存在

    // 内页
    function checkIndexModule() {
        // 兼容新旧版本类名
        const container = document.querySelector("[class*='index-module_extra-area_']") 
            || document.querySelector("[class*='index-module_extra_']")
            || document.querySelector("[class*='index-module_info_']"); // 找到要添加按钮的容器元素
        if (container && !document.querySelector(".Btns")) {
            clearInterval(intervalId2); // 清除定时�?
            const div = document.createElement("div");
            div.className = "Btns";
            ...
            container.appendChild(div); // 将按钮添加到网页�?
            ...
            if (element2) {
                const progressText = "📓 下载中";
                const fileName = (title || document.querySelector("[class*='index-module_title_']")?.innerText || "PDF") + ".pdf";
                const completeText = "📓 下载完成";
                element2.addEventListener("click", function() {
                    downloadFile(pdfUrl, element2, progressText, fileName, completeText);
                });
            ...
            setTimeout(function() {
                ...
                if (iframe) {
                    ...
                    if (element1) {
                        element1.href = `${iframePDF}?accessToken=${macId}`; // 更新 element1 �?URL
                    }
                    pdfUrl = iframePDF; // 更新 element2 �?URL
                }
            }, 3000);
        } else {
            elapsedTime += 100;
            if (elapsedTime >= maxTimeToCheck) {
                clearInterval(intervalId2); // 清除定时器，停止检�?
            }
        }
    }
    const intervalId2 = setInterval(checkIndexModule, 100); // 每隔 100 毫秒检查一�?index-module_extra_tUQog 元素是否存在
        ...
        return originalFetch(url, options) // 调用原始 fetch 函数
            .then(response => {
                if (response.status === 200 && url.endsWith(".pdf")) {
                    const headers = Object.fromEntries(response.headers.entries()); // 响应�?
                        ...
                        if (element1) { // iframe 内部不存在此元素；可在主文档中通过 iframe 获取文件 URL
                            element1.href = url; // 修改 element1 �?URL
                        }
                        pdfUrl = url; // 修改 element2 �?URL
                    } else { // 课程教学
                        const syncClassSuggestion = document.querySelector("[class*='index-module_suggestion-wrap']");
