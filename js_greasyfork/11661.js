// ==UserScript==
// @name           BrokenStones Advanced Buffer Calculator
// @namespace      dahlia@brokenstones
// @description    Show buffer in page header and on user pages.
// @description    Original Buffer Calculator script by pinguen for What.CD.
// @description    Updated to Advanced Buffer Calculator by jonls.
// @description    Modified for BrokenStones by dahlia.
...
// @downloadURL https://update.greasyfork.org/scripts/11661/BrokenStones%20Advanced%20Buffer%20Calculator.user.js
// @updateURL https://update.greasyfork.org/scripts/11661/BrokenStones%20Advanced%20Buffer%20Calculator.meta.js
// ==/UserScript==
...
(function() {
  ...
  function ratio_class(amount) {
    if (amount > (100*GiB)) return 'r99'; // 100GB+ is blue
    else if (amount > (50*GiB)) return 'r20'; // 50GB+ is green
    else if (amount > (10*GiB)) return 'r10'; // 10GB+ is green
    else if (amount > 0) return 'r05'; // 0GB+ is orange
    else if (amount > (-10*GiB)) return 'r01'; // 0GB to -10GB is red
    return 'r00'; // less than -10GB is flashing red
  }
  ...
  GM_registerMenuCommand('BrokenStones Advanced Buffer Calculator: '+(display_in_header ? 'Hide buffer in header' : 'Show buffer in header'), function() {
  ...
  GM_registerMenuCommand('BrokenStones Advanced Buffer Calculator: '+(use_ratio_coloring ? 'Use simple ratio coloring' : 'Use advanced ratio coloring'), function() {
  ...
  function insert_buffer(parent, up, down, save_name) {
    ...
    /* Append buffer stats */
    var buffer_item = $('<li id="stats_required" class="stats_buffer" style="-webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; -o-user-select: none; user-select: none;">Buffer: </li>');
    ...
    function update_target() {
      ...
      if (use_ratio_coloring) buffer_amount.css('color', ratio_color(up/down));
      else buffer_amount.removeClass().addClass(ratio_class(calculate_buffer(up, down, target.toFixed(2))));
  ...
  if (/brokenstones\.me\/user\.php\?id=/.test(document.URL)) {
    /* Find stats box */
