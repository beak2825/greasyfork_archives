...
// @namespace    jiba.gay
// @version      0.2
// @description  解决discuz论坛只能单个而无法批量上传附件(注意：这个代码仅限特定网站使用，其他网站不可用...)
// @author       HuGu5278
// @match        *://*/*forum.php?mod=post*
// @require      https://cdn.jsdelivr.net/npm/jquery@3.4.0/dist/jquery.min.js
// @require      http://cdn.staticfile.org/webuploader/0.1.5/webuploader.js
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/418670/Discuz%20%E9%99%84%E4%BB%B6%E6%89%B9%E9%87%8F%E8%BE%85%E5%8A%A9.user.js
// @updateURL https://update.greasyfork.org/scripts/418670/Discuz%20%E9%99%84%E4%BB%B6%E6%89%B9%E9%87%8F%E8%BE%85%E5%8A%A9.meta.js
// ==/UserScript==
...
`;
        ...
        var notices = [ '<span style="color:red;">上传文件的大小超过网站设置的将会失败!!</span>', '<span style="color:red;">上传不支持的格式同样会失败!!</span>','<span style="color:green;">请检查好后再批量上传，否则会出现漏传的情况!!!</span>', '<span style="color:red;">胡乱的上传、过多的漏传将会被封号!!</span>']
        Array.from(document.getElementsByClassName('notice upnf')).forEach(function(item) {
            var temp_div = document.createElement("div");
            temp_div.style.border="1px dotted red";
            temp_div.style.padding="6px 2px";
            temp_div.style.marginTop="20px";
            temp_div.style.textAlign="center";
            notices.forEach(function(x){
                var temp_div2 = document.createElement("div");
                temp_div2.innerHTML = x;
                temp_div2.style.margin="4px";
                temp_div.appendChild(temp_div2);
            });
            item.appendChild(temp_div);
        });
    }
    document.getElementById('e_upload').style.display='none';
    document.getElementById('e_attachlist').style.display='';
    addStyle();
    jQuery.noConflict();
    ...
    SWFUpload.prototype.initSettings = function (userSettings) {
        ...
        this.ensureDefault("file_types", "*.jpg");
        this.ensureDefault("file_types_description", "All Files");
        this.ensureDefault("file_size_limit", 200);	// Default zero means "unlimited"
        ...
        try {
            ...
            if(this.customSettings.uploadSource == 'forum') {
                if(this.customSettings.uploadType == 'poll') {
                    ...
                    serverData = serverData._raw.split('|')[2];
                    var aid = parseInt(serverData);
                    if(aid > 0) {
                        if(this.customSettings.uploadType == 'attach') {
                            ajaxget('forum.php?mod=ajax&action=attachlist&ajaxtarget=attachlist&aids=' + aid+'&posttime=' + document.getElementById('posttime').value + (!fid ? '' : '&fid=' + fid), file.id);
                        } else if(this.customSettings.uploadType == 'image') {
...
//                             $(file.id).style.display = 'none';
    ...
    var imgUpload = new SWFUpload({
        upload_url: "/misc.php?mod=swfupload&operation=upload&simple=1",
        post_params: {"uid" : discuz_uid, "hash": document.querySelector("input[name='hash']").value , "type":"image"},
    ...
    var upload = new SWFUpload({
        upload_url: "/misc.php?mod=swfupload&operation=upload&simple=1",
        post_params: {"uid":discuz_uid, "hash":document.querySelector("input[name='hash']").value},
        file_size_limit : "2000",
        file_types : "*.jpg",
