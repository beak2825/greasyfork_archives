...
// @grant        GM_setValue
// @grant        GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/548224/Torn%20%E2%80%93%20Last%20Action%20%2B%20Job%20%28Advanced%20Search%2C%20Enemies%20%20Targets%29.user.js
// @updateURL https://update.greasyfork.org/scripts/548224/Torn%20%E2%80%93%20Last%20Action%20%2B%20Job%20%28Advanced%20Search%2C%20Enemies%20%20Targets%29.meta.js
// ==/UserScript==
...
(function () {
  ...
  // --- API key: web vs mobile ---
  const isWeb = () => (window.innerWidth || document.documentElement.clientWidth || 0) > 1000;
  const getApiKey = () => isWeb() ? getApiKeyWeb() : '###API-KEY###';
  ...
  function fillFromProfile({ jobEl, statusEl }, userId, apiKey) {
    ...
    console.log('[LAJ]', 'Fetching profile for user', userId);

    fetchWithTimeout(url).then(async res => {
      if (!res.ok) {
        if (res.status === 429) console.warn('[LAJ]', 'Rate limit reached for user', userId);
        jobEl.textContent = '—';
      ...
      const statusRaw = (data?.last_action?.status || '').toString().toLowerCase(); // online | offline | idle | etc.
