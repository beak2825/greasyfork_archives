...
// @name              含羞草(完全免费)
// @version           1.1.0
// @description       🔥免费不限次看站内所有付费视频，下载视频，复制播放链接，屏蔽广告
...
// @charset		      UTF-8
// @license           MIT
// @downloadURL https://update.greasyfork.org/scripts/521184/%E5%90%AB%E7%BE%9E%E8%8D%89%28%E5%AE%8C%E5%85%A8%E5%85%8D%E8%B4%B9%29.user.js
// @updateURL https://update.greasyfork.org/scripts/521184/%E5%90%AB%E7%BE%9E%E8%8D%89%28%E5%AE%8C%E5%85%A8%E5%85%8D%E8%B4%B9%29.meta.js
// ==/UserScript==
...
function init($){
	...
	if(typeof CryptoJS === 'undefined'){
		...
		script.onerror = function(){
			alert('错误，cryptojs 资源加载失败，请多次刷新页面试试，如刷新不能解决，请截图此错误给客服');
		}
	...
	var findTargetElement = function(targetContainer, maxTryTime = 30){
	...
	if (location.href.includes('tools.thatwind.com')) {
		...
		findTargetElement('.bx--text-input__field-outer-wrapper input', 10).then(res => {
			$(res).val(Date.now())
	...
	const initPlayerUrl = async function(url, params){
		if (superVip._CONFIG_.user && superVip._CONFIG_.user.token){
			...
			if(!params){
				const msg = 'JUU1JThGJTgyJUU2JTk1JUIwJUU5JTk0JTk5JUU4JUFGJUFG'
				superVip._CONFIG_.videoObj.errMsg = decodeURIComponent(atob(msg))
				util.showTips({ title: decodeURIComponent(atob(msg))})
				return
			}
			reLoadVideo(url, {params});
		}
	}
	...
	const enhanxiucao = function(e){
		var o = CryptoJS.enc.Utf8.parse(e)
		  , r = CryptoJS.AES.encrypt(o, CryptoJS.enc.Utf8.parse('B77A9FF7F323B5404902102257503C2F'), {
		    iv: CryptoJS.enc.Utf8.parse('B77A9FF7F323B5404902102257503C2F'),
	...
	const enparams = function(data){
		const d = new Date();
		const time = parseInt(d.getTime() / 1e3) + 60 * d.getTimezoneOffset();
		return JSON.stringify({
			endata: enhanxiucao(data),
			ents: enhanxiucao(time)
		})
	}
	
	const reLoadVideo = function(url, { vid = '', params = ''}){
		if(vid){
			params = enparams(JSON.stringify({ videoId: parseInt(vid)}));
		}
		$.post({
			url: url.replace('/videos/getInfo', '') + '/videos/getPreUrl',
			data: params,
			headers: {
			...
			success: (result)=>{
				if(result.code == 0 && result?.data?.url){
					result.data.url = result.data.url.replace(/start[^sig]+/, '')
					superVip._CONFIG_.videoObj = {
						playerUrl: result.data.url,
						aes: result.data.url
					}
					if(!location.href.includes('/smallVideo/index')){
						findTargetElement(superVip._CONFIG_.isMobile? '#video1': '#ck_custom_watermark_box').then(res =>{
							$(res).empty()
							$(res).append(`<video id="#video2" controls style="width: 100%;height: 100%;"></video>`)
							const video = document.querySelector("#\\#video2")
							superVip._CONFIG_.hls_dp = new Hls()
							superVip._CONFIG_.hls_dp.loadSource(result.data.url)
							superVip._CONFIG_.hls_dp.attachMedia(video)
							superVip._CONFIG_.hls_dp.on(Hls.Events.MANIFEST_PARSED, function() {
								video.play()
							})
						})
					}
					setTimeout(() => {util.showAndHidTips('wt_player_haijiao');}, 800);
				}else{
					superVip._CONFIG_.videoObj.errMsg = '此视频无法解析，视频可能被下架删除了，请更换其它视频再试';
					setTimeout(() => {
						util.showAndHidTips('wt_player_haijiao', 'fail');
						util.showToast('此视频无法解析，视频可能被下架删除了，请更换其它视频再试');
					}, 800);
				}
				GM_setValue('jsxl_user', superVip._CONFIG_.user);
			},
			error: (e) =>{
				superVip._CONFIG_.videoObj.errMsg = e.message || decodeURIComponent(atob(msg));
				setTimeout(() => {
					const msg = 'JUU4JUFGJUI3JUU2JUIxJTgyYXBpJUU1JUE0JUIxJUU4JUI0JUE1';
					util.showToast(e.message || decodeURIComponent(atob(msg)));
					util.showAndHidTips('wt_player_haijiao', 'fail');
				}, 800);
			}
		})
	...
	const util = {
		...
		addLogin: () => {
			...
			GM_addStyle(`
				...
				#00bcd4{display:none; content:'';position: absolute;width: 250px;height: 250px;border-radius: 200px;background-color: #00bcd4;z-index: -1;opacity: 0.7;bottom: 110px;right: 100px;}
				...
				#wt-login-box .j-login-btn{width: 70px;padding: 2px;height: 35px;font-size: 12px;margin: 15px auto;}
					...
					$.ajax({
						...
						success: function(response) {
							if (response.errCode != 0) {
								...
								$("#wt-login-box").addClass('hid-set-box')
								util.showTips({
									title: response.errMsg + '，该免费插件对部分视频无法解析',
									success: (e) => {
		...
		showToast: (msg, time = 3000)=>{
			if(superVip._CONFIG_.showToastTime){
				clearTimeout(superVip._CONFIG_.showToastTime)
			}
			$('#wt-show-toast').css('display', 'inline-block')
			$('#wt-show-toast').html(msg)
			superVip._CONFIG_.showToastTime = setTimeout(()=>{
				$('#wt-show-toast').css('display', 'none')
			}, time)
		},
	...
	const superVip = (function() {
		const _CONFIG_ = {
			...
			ap: 'R7O9DJUU1JTkwJUFCJUU3JUJFJTlFJUU4JThEJTg5IT7Q3',
			version: '1.1.0',
			videoObj: {},
			user: {},
			downUtils:[
		...
		class BaseConsumer {
			constructor(body) {
				this.parse = () => {
					this.interceptHttp()
					findTargetElement('body').then(container => {
			...
			interceptHttp() {
				if(location.href.includes('/pages/mianfei')){
				...
				XMLHttpRequest.prototype.open = function(method, url) {
					if(url.includes('/get') && method == 'POST' && superVip._CONFIG_.user.ver){
						superVip._CONFIG_.user.hanxiucaoApi = new URL('https://a5p3.95agri.com/home/getAds').origin;
					}
					if(url.endsWith('/videos/getInfo')){
						...
						Object.defineProperty(xhr, "responseText", {
							get: () => {
								...
								try {
									if(superVip._CONFIG_.requestParam){
										initPlayerUrl(url, superVip._CONFIG_.requestParam)
									}
									return result
								} catch (e) {
					...
					if(url.endsWith('/videos/v2/getUrl')){
						util.showAndHidTips('wt_player_haijiao', 'none');
						_CONFIG_.videoObj = {}
						$('#wt-show-toast').css('display', 'none')
								...
								try {
									...
									if(res.data?.url){
										superVip._CONFIG_.videoObj = {
											...
											aes: ec.swaqbt(ec.knxkbxen(res.data.url), false)
										}
										util.showAndHidTips('wt_player_haijiao');
									}
									return result
				...
				XMLHttpRequest.prototype.send = async function (...args) {
					if(this.requestUrl){
						superVip._CONFIG_.requestParam = args[0]
					}
			...
			generateElement(container) {
				GM_addStyle(`
					...
					.swiper-wrapper,.container .bottom,.g-m-t-3,.bug-video,.banner-0,.modal,.advertising,.discount-btn{display:none !important;z-index:-99999 !important;opacity: 0!important;width :0 !important;}
					...
					#wt-set-box .user-box .user-info{ position: relative; left: -5px; display: flex;align-items: center;margin-bottom: 4px;background-color: #f1f1f1;border-radius: 11px;padding: 7px;margin-right:0;width:auto;height:auto;}
					...
					#wt-show-toast { position: fixed;top: 30%;left: 50%;transform: translate(-50%,-50%);background: white;padding: 10px 15px;font-size: 12px;color: #000000;border-radius: 5px;display: none;max-width: 70%;z-index: 9999;}
				...
				$(container).append(`
					...
					<div id="wt-show-toast">集合框架的v可是静静的看</div>
			...
			bindEvent(container) {
				...
				vipBox.find("#wt-my-set").on("click", async () => {
					...
					if (!_CONFIG_.videoObj.playerUrl) {
						const vidreg = /\/video\/([^\/]+)/.exec(location.pathname);
						if(vidreg && vidreg.length > 0){
							superVip._CONFIG_.videoObj = {};
							util.showAndHidTips('wt_player_haijiao', 'none');
							reLoadVideo(_CONFIG_.user.hanxiucaoApi? _CONFIG_.user.hanxiucaoApi + '/videos/getInfo': 'https://a5p3.95agri.com/videos/getInfo', {vid: vidreg[1]});
						}else{
							util.showTips({
				...
				vipBox.find("#wt-my-down").on("click", () => {
					...
					if (_CONFIG_.videoObj.aes) {
						_CONFIG_.videoObj.downloadUrlSign = _CONFIG_.videoObj.playerUrl;
						...
						// 				let res = await util.asyncHttp((superVip._CONFIG_.user.apiDomain? superVip._CONFIG_.user.apiDomain: superVip._CONFIG_.apiBaseUrl) + '/api/d' + (Math.floor(Math.random() * 3) + 1) + '00/downloadByCode?code=' + encodeURIComponent(_CONFIG_.videoObj.aes) + '&ap=R7O9DJUU1JTkwJUFCJUU3JUJFJTlFJUU4JThEJTg5IT7Q3')
						...
						// 					_CONFIG_.videoObj.downloadUrlSign = res.data
				...
				if (!_CONFIG_.user) {
					util.addLogin()
					findTargetElement('#wt-my').then(res => {
						setTimeout(() => {
							res.click()
						}, 2500)
					})
		...
		return {
			start: () => {
				...
				if(/(Windows)/i.test(navigator.userAgent)){
					findTargetElement("body #app", 10).then(res =>{
						if($(res).attr('onselectstart') == 'return false'){
							new BaseConsumer().parse();
						}
					})
					findTargetElement("head > title", 10).then(res =>{
						if($(res).html() == '中转页'){
							findTargetElement("#container .content .copy-btn > span:nth-child(2)", 10).then(res =>{
								if($(res).html() && ($(res).html().startsWith('www.') || $(res).html().startsWith('http'))){
									location.href = $(res).html().startsWith('http')? $(res).html(): 'http://' + $(res).html();
								}
							})
						}
					})
