// ==UserScript==
// @name          구글 검색 무한 스크롤
// @namespace     구글 검색 무한 스크롤
// @match       https://www.google.com/*
// @version       0.1
// @description   구글 검색 결과 창에서 스크롤을 무한으로 내릴 수 있고, 다음 페이지의 검색 결과가 무한으로 아래 추가됩니다.
// @icon          data:image/jpeg;base64,
// @author        mickey90427 <mickey90427@naver.com>
// @downloadURL https://update.greasyfork.org/scripts/460914/%EA%B5%AC%EA%B8%80%20%EA%B2%80%EC%83%89%20%EB%AC%B4%ED%95%9C%20%EC%8A%A4%ED%81%AC%EB%A1%A4.user.js
// @updateURL https://update.greasyfork.org/scripts/460914/%EA%B5%AC%EA%B8%80%20%EA%B2%80%EC%83%89%20%EB%AC%B4%ED%95%9C%20%EC%8A%A4%ED%81%AC%EB%A1%A4.meta.js
// ==/UserScript==

(function() {
    'use strict';

    // 최대 스크롤 높이 (픽셀)
    const MAX_SCROLL_HEIGHT = 20000;

    // 페이지 번호
    let pageNum = 1;

    // 스크롤 위치 감지 함수
    function handleScroll() {
        const scrollHeight = document.documentElement.scrollHeight;
        const scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
        const windowHeight = window.innerHeight;

        // 스크롤이 끝까지 내려간 경우
        if (scrollHeight - scrollTop - windowHeight < 50) {
            // 최대 스크롤 높이에 도달하지 않은 경우
            if (scrollTop < MAX_SCROLL_HEIGHT) {
                pageNum++;
                const nextUrl = `https://www.google.com/search?q=${encodeURIComponent(window.location.search.substring(3))}&start=${(pageNum - 1) * 10}`;
                const xhr = new XMLHttpRequest();
                xhr.open('GET', nextUrl, true);
                xhr.onload = function() {
                    const parser = new DOMParser();
                    const doc = parser.parseFromString(xhr.responseText, 'text/html');
                    const searchResults = doc.getElementById('search');
                    const container = document.getElementById('search').parentNode;
                    container.insertBefore(searchResults, container.lastElementChild.nextSibling);
                };
                xhr.send();
            }
            // 최대 스크롤 높이에 도달한 경우
            else {
                // 스크롤 위치 감지 함수 제거
                window.removeEventListener('scroll', handleScroll);
            }
        }
    }

    // 스크롤 위치 감지 함수 등록
    window.addEventListener('scroll', handleScroll);
})();