// ==UserScript==
// @name         Black Theme for Roblox
// @version      10.1
// @description  black theme for ur roblox website, enable roblox dark theme to make it work: Settings > Theme > Dark.
// @author       weyble
// @match       *://www.roblox.com/*
// @namespace    https://greasyfork.org/users/754911
// @downloadURL https://update.greasyfork.org/scripts/473225/Black%20Theme%20for%20Roblox.user.js
// @updateURL https://update.greasyfork.org/scripts/473225/Black%20Theme%20for%20Roblox.meta.js
// ==/UserScript==
var style= document.createElement("style");
style.innerHTML = `.home-header {display: flex;flex-direction: row;margin-bottom: 25px;} .user-info-container {display: flex;align-items: center;margin-left: 25px;} .user-avatar-container {min-width: 150px;}`;
function header(link,pic,name) {
return `<div id="home-header" class="col-xs-12 home-header-container"><div class="home-header">
<a href="${link ??"/users/1/profile" }" class="user-avatar-container avatar avatar-headshot-lg"><span class="thumbnail-2d-container avatar-card-image">
<img src="${pic ??"data:image/gif;base64,R0lGODlhAQABAIAAAAAAAAAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==" }" alt="" class="avatar-card-image"></span></a><div class="user-info-container"><h1 class="user-name-container">
<a href="${link ??"/users/1/profile" }">${name ??"Roblox" }</a></h1></div></div></div>`}
function rahh () {
document.querySelector('link[rel="icon"]').href= 'data:image/x-icon;base64,'
var css=`.dark-theme .rbx-divider{
 background-color: gold;
}
.dark-theme{
 background-color: black;
}
.dark-theme .content{
 background-color: black;
}
.dark-theme .container-fluid{
 background-color: black;
}
.dark-theme .rbx-upgrade-now .btn-secondary-md:visited{
 background-color: black;
}
.dark-theme .chat-container .chat-main .chat-header{
 background-color: black;
}
.dark-theme .rbx-upgrade-now .btn-secondary-md:visited{
 color: white;
}
.dark-theme .gotham-font .font-title, .gotham-font .h1, .gotham-font h1{
 color: gold;
}
.dark-theme .container-footer{
 background: black;
}
.dark-theme .rbx-upgrade-now .btn-secondary-md{
 display: none
}
.dark-theme .profile-header .profile-header-content{
 background: #0f0f0f;
}
.dark-theme .slide-switcher .slide-item-container .slide-item-container-right{
 background: #0f0f0f;
}
.dark-theme .slide-switcher .slide-item-container .slide-item-container-left{
 background: #0f0f0f;
}
.dark-theme .rbx-tabs-horizontal .rbx-tab.active .rbx-tab-heading{
 background-color: #0f0f0f;
}
.dark-theme .rbx-tab .rbx-tab-heading{
 background-color: #0f0f0f;
}
.dark-theme .profile-avatar-right .profile-avatar-mask{
 background-color: #0f0f0f;
}
.dark-theme .section-content{
 background-color: #0f0f0f;
}
.dark-theme .layer{
 background-color: #0f0f0f;
}
.dark-theme .profile-accoutrements-container .profile-accoutrements-slider .accoutrement-item{
 padding: 2px;
}
.dark-theme .rbx-tabs-horizontal .rbx-tab .rbx-tab-heading.active{
 background-color: #0f0f0f;
}
.dark-theme .friends-content .avatar-card-content{
 background-color: #0f0f0f;
}
.dark-theme .avatar-card-btns{
 background-color: #0f0f0f;
}
.dark-theme .rbx-tabs-horizontal #horizontal-tabs .rbx-tab:not(.active) a:not(:hover){
 background-color: #0f0f0f;
}
.dark-theme .menu-vertical .menu-option-content{
 background-color: #0f0f0f;
}
.dark-theme .rbx-header .rbx-navbar-icon-group>li .dropdown-menu li, .rbx-header .rbx-navbar-right-nav>li .dropdown-menu li{
 background-color: #0f0f0f;
}
.dark-theme .chat-friend-list .chat-friend-container{
 background-color: #0f0f0f;
}
.dark-theme .dialog-container .dialog-body .dialog-messages{
 background-color: #0f0f0f;
}
.dark-theme .dialogs .dialog-container .dialog-body{
 background-color: #0f0f0f;
}
.dark-theme .table{
 background-color: #0f0f0f;
}
.dark-theme .premium-container .page-refined{
 background-image: none;
}
.dark-theme .rbx-header .navbar-search .navbar-search-option a{
 background-color: #0f0f0f;
}
.dark-theme .input-group-btn .dropdown-menu li a, .input-group-btn .dropdown-menu li button{
 background-color: #0f0f0f;
}
.dark-theme .stack .stack-list .stack-row{
 background-color: #0f0f0f;
}
.dark-theme .thumbnail-holder .thumbnail-span{
 background-color: #0f0f0f;
}
.dark-theme .stack .card-list .card-item{
 background-color: #0f0f0f;
}
.dark-theme .container-empty, .dark-theme .section-content-off{
 background-color: #0f0f0f;
}
.dark-theme .asset-thumb-container{
 background-color: #1a1919;
}
.dark-theme .dropdown-menu li{
 background-color: #0f0f0f;
}
.dark-theme .details-container{
 background-color: #0f0f0f;
}
.dark-theme .details-members-container .add-friends-option .add-friends-label{
 background-color: #0f0f0f;
}
.dark-theme .details-members-container .add-friends-option .add-friends-icon{
 background-color: #0f0f0f;
}
.dark-theme .friends-list-container .friend-container{
 background-color: #0f0f0f;
}
.dark-theme .people-info-card-container .place-container {
 background-color: #0f0f0f;
}
.dark-theme .chat-main-empty .chat-disconnect{
 background-color: black;
}
.dark-theme .not-approved-page-content .not-approved-page{
 background-color: #0f0f0f;
}
.dark-theme .mCustomScrollBox{
 background-color: black;
}
.dark-theme .notification-stream-header{
 background-color: black;
}
.dark-theme .modal-dialog .modal-content{
 background-color: #0f0f0f;
}
.dark-theme .request-error-page-content .default-error-page{
 background-color: #0f0f0f;
}
.dark-theme .new-notification-stream-2022 .notification-stream-container .notification-content-view .notification-stream-body .notification-stream-scrollbar .notification-stream-list{
 background-color: black;
}
.player-search-container .player-search-page .search-result .player-item .avatar-card-container .avatar-card-content, .player-search-container .player-search-page .search-result .responsive-card .avatar-card-container .avatar-card-content{
 background-color: #0f0f0f;
}
.dark-theme .table-striped>tbody>tr:nth-child(2n)>td, .dark-theme .table-striped>tbody>tr:nth-child(2n)>th{
 background-color: #0f0f0f;
}
.dark-theme .catalog-container .sticky .search-bars{
 background-color: black;
}
.dark-theme .catalog-container .sticky .topic-container{
 background-color: black;
}
.dark-theme .messages .messageDivider.unread{
 background-color: #303031;
}
.dark-theme .messages .messageDivider.read{
 background-color: #0f0f0f;
}
.dark-theme .rbx-left-col{
 background-color: black;
}`;
document.title="-Roblox";
var e=document.createElement("style");
e.appendChild(document.createTextNode(css));
(document.querySelector("head")||
document.documentElement).appendChild(e)};
document.getElementsByTagName("head")[0].appendChild(style);
async function element(q) {
while (document.querySelector(q)== null) {
await new Promise(r => requestAnimationFrame(r));};
return document.querySelector(q);};
element("#HomeContainer .section:first-child").then(async (hdrSec)=>{
var user= await element("#navigation.rbx-left-col > ul > li > a")?? null;
var link= user.getAttribute("href") ?? null;
var pic= (await element("#navigation.rbx-left-col > ul > li > a img")).getAttribute("src")?? null;
var name= (await element("#navigation.rbx-left-col > ul > li > a .font-header-2")).innerText?? null;
name = "superior/above you⠀-⠀" + name +" ✓";
var html=header(link,pic,name);
hdrSec.outerHTML=html;});
setTimeout(()=>{rahh()},200);
setTimeout(()=>{rahh()},999);//double check\\