...
// @run-at       document-end
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/543734/Torn%20War%20Helper%20%28highlights%20and%20chain%20timers%29.user.js
// @updateURL https://update.greasyfork.org/scripts/543734/Torn%20War%20Helper%20%28highlights%20and%20chain%20timers%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    const SETTINGS = {
        ...
        enhanceJoinFightButton: true,
        apiKey: '',
        apiKeyConfigured: false,
        enableHospitalCountdown: true
    };
    ...
    const playerHospitalTimes = new Map();
    ...
    function addStyles() {
        ...
        styleElement.textContent = `
            ...
            /* Hospital countdown styles */
            .wh-hospital-countdown {
    ...
    function processHospitalStatus(row, playerId) {
        ...
        statusElements.forEach(el => {
            ...
            if ((typeof classNames === 'string' && 
                (classNames.includes('status___') || classNames.includes('prevColumn___'))) ||
                (el.classList && 
                (el.classList.contains('status') || el.classList.contains('userStatus')))) {
                
                statusElement = el;
                
                // Check if hospital by text content or class
                const textContent = el.textContent.toLowerCase();
                isHospital = textContent.includes('hospital') || 
                            (typeof classNames === 'string' && classNames.includes('not-ok'));
                
                // If in hospital, add or update countdown element
                if (isHospital && SETTINGS.enableHospitalCountdown && SETTINGS.apiKeyConfigured && SETTINGS.apiKey) {
                    // Convert text playerId to number if needed
                    const playerIdNum = isNaN(parseInt(playerId)) ? null : parseInt(playerId);
                    
                    if (playerIdNum) {
                        // Fetch hospital time for this player
                        fetchPlayerHospitalTime(playerIdNum);
                        
                        // Add countdown element if not already there
                        let countdownEl = el.querySelector('.wh-hospital-countdown');
                        if (!countdownEl) {
                            countdownEl = document.createElement('span');
                            countdownEl.className = 'wh-hospital-countdown';
                            countdownEl.setAttribute('data-player-id', playerIdNum);
                            countdownEl.style.marginLeft = '5px';
                            countdownEl.style.fontSize = '12px';
                            countdownEl.style.fontWeight = 'bold';
                            el.appendChild(countdownEl);
                        }
                    }
                } else if (!SETTINGS.enableHospitalCountdown) {
                    // Remove countdown if setting is disabled
                    const countdownEl = el.querySelector('.wh-hospital-countdown');
                    if (countdownEl) {
                        countdownEl.remove();
                    }
                }
        ...
        if (SETTINGS.flashHospitalChange && statusElement) {
            const previousStatus = playerStatusHistory.get(playerId);
            const currentStatus = isHospital ? 'hospital' : 'ok';
    ...
    function initialize() {
        ...
        // Start hospital countdown timer if API key is configured
        if (SETTINGS.apiKeyConfigured && SETTINGS.apiKey) {
            startHospitalCountdownTimer();
        }
                ...
                if (mutation.type === 'childList' && mutation.target) {
                    if (mutation.target.classList && 
                        (mutation.target.classList.contains('members-list') || 
                         mutation.target.classList.contains('membersWrap___') || 
                         mutation.target.classList.contains('member'))) {
                        highlightFactionMembers();
                        break;
                    }
                    
                    if (typeof mutation.target.className === 'string') {
                        if (mutation.target.className.indexOf('membersWrap___') !== -1 || 
                            mutation.target.className.indexOf('member') !== -1) {
                            highlightFactionMembers();
                            break;
                        }
                    }
                    ...
                    if (mutation.target.className && 
                        typeof mutation.target.className === 'object' && 
                        mutation.target.className.baseVal) {
                        if (mutation.target.className.baseVal.indexOf('membersWrap___') !== -1 || 
                            mutation.target.className.baseVal.indexOf('member') !== -1) {
                            highlightFactionMembers();
                            break;
                        }
                    }
    ...
    // Function of the "JOIN FIGHT" button
    run();
    ...
    async function run() {
        ...
        if (x && x.innerText === "JOIN FIGHT" && SETTINGS.enhanceJoinFightButton) {
    ...
    // Function to fetch hospital time for a player
    function fetchPlayerHospitalTime(playerId) {
        // Skip if no API key is configured or hospital countdown is disabled
        if (!SETTINGS.apiKey || !SETTINGS.apiKeyConfigured || !SETTINGS.enableHospitalCountdown) return;
        
        // Only fetch if we haven't recently fetched this player's data
        if (playerHospitalTimes.has(playerId) && 
            Date.now() - playerHospitalTimes.get(playerId).lastFetched < 60000) {
            return;
        }
        
        try {
            const apiUrl = `https://api.torn.com/user/${playerId}?selections=profile&key=${SETTINGS.apiKey}`;
            ...
            fetch(apiUrl)
                ...
                .then(data => {
                    if (data.error) {
                        console.warn(`Torn API Error: ${data.error.code} - ${data.error.error}`);
                        return;
                    }
                    
                    if (data.status && data.status.state === 'Hospital') {
                        try {
                            const hospitalOut = new Date(data.status.until * 1000);
                            playerHospitalTimes.set(playerId, {
                                until: hospitalOut,
                                lastFetched: Date.now()
                            });
                            
                            // Update the display immediately
                            setTimeout(updateHospitalStatusDisplay, 10);
                        } catch (e) {
                            console.warn('Error processing hospital data:', e);
                        }
                    } else {
                        // Player is not in hospital
                        playerHospitalTimes.delete(playerId);
                    }
                })
                .catch(error => {
                    console.warn(`Failed to fetch hospital data: ${error}`);
                });
        } catch (e) {
            console.warn('Error in fetchPlayerHospitalTime:', e);
        }
    }

    // Update hospital status displays
    function updateHospitalStatusDisplay() {
        // Skip if hospital countdown is disabled
        if (!SETTINGS.enableHospitalCountdown) return;
        
        const now = new Date();
        
        document.querySelectorAll('.wh-hospital-countdown').forEach(countdownEl => {
            try {
                const playerId = countdownEl.getAttribute('data-player-id');
                if (!playerId || !playerHospitalTimes.has(playerId)) {
                    countdownEl.textContent = '';
                    countdownEl.style.display = 'none';
                    return;
                }
                
                const hospitalData = playerHospitalTimes.get(playerId);
                if (!hospitalData || !hospitalData.until) {
                    countdownEl.textContent = '';
                    countdownEl.style.display = 'none';
                    return;
                }
                
                const timeLeft = Math.max(0, hospitalData.until - now);
                
                if (timeLeft <= 0) {
                    countdownEl.textContent = 'Out now!';
                    countdownEl.style.color = '#5cb85c';
                    countdownEl.classList.add('out-now');
                    
                    // Remove from map after displaying "Out now!" for a while
                    setTimeout(() => {
                        playerHospitalTimes.delete(playerId);
                        updateHospitalStatusDisplay();
                    }, 5000);
                } else {
                    ...
                    countdownEl.style.color = '#f0ad4e';
                    countdownEl.classList.remove('out-now');
                }
                
                countdownEl.style.display = 'inline';
            } catch (e) {
                console.warn('Error updating hospital countdown:', e);
            }
        });
    }

    // Start a timer to update hospital countdowns
    function startHospitalCountdownTimer() {
        setInterval(() => {
            if (SETTINGS.enableHospitalCountdown) {
                updateHospitalStatusDisplay();
            }
        }, 1000);
    }
