...
(function() {
    ...
    // --- Watermark Removal Core (Ported from watermark_remove.js) ---
            ...
            try {
                ...
                await Promise.all([
                    new Promise((resolve, reject) => {
                        bg48.onload = resolve;
                        bg48.onerror = () => reject(new Error("Failed to load bg48"));
                        bg48.src = bg_48_default;
                    }),
                    new Promise((resolve, reject) => {
                        bg96.onload = resolve;
                        bg96.onerror = () => reject(new Error("Failed to load bg96"));
                        bg96.src = bg_96_default;
                    })
                ]);
                return new WatermarkEngine({ bg48, bg96 });
            } catch (err) {
                console.error("Gemini Batch Downloader: Error creating engine assets", err);
                return null;
            }
        }
    ...
    (async () => {
        try {
            ...
            console.log("Gemini Batch Downloader: Watermark Engine initialized.");
            setupObserver();
        } catch (e) {
            console.error("Gemini Batch Downloader: Failed to initialize engine", e);
        }
    })();

    function findGeminiImages() {
        // Selector logic based on PRD and observation
        // Images usually have googleusercontent.com in src and are within generated-image containers
        // Updated based on user debug trace: img -> button -> div -> single-image -> generated-image
        const images = [...document.querySelectorAll('img[src*="googleusercontent.com"]')]
            .filter(img => img.closest('generated-image, single-image, .generated-image-container'));
        return images;
    }
    ...
    const STYLES = `
        ...
        .gbd-progress-toast.show {
            opacity: 1;
            transform: translateY(0);
        }
    `;

    // Add styles using GM_addStyle to bypass CSP
    ...
    function createUI() {
        ...
        downloadBtn.className = 'gbd-btn';
        downloadBtn.textContent = '📥 Batch Download';
        downloadBtn.style.display = 'none';
        
        // Inline fallback styles just in case class is blocked
        ...
        downloadBtn.onclick = handleBatchDownload;
        document.body.appendChild(downloadBtn);

        progressToast = document.createElement('div');
        progressToast.className = 'gbd-progress-toast';
    ...
    function updateUI() {
        ...
        if (images.length > 0) {
            if (!downloadBtn) createUI();
            downloadBtn.style.display = 'flex';
            downloadBtn.textContent = `📥 Batch Download (${images.length})`;
        } else if (downloadBtn) {
            downloadBtn.style.display = 'none';
        }
    }
    ...
    function setupObserver() {
        // Debounced observer to update UI when new images appear
        let timeout;
    ...
    async function handleBatchDownload() {
        ...
        showToast(`Starting download of ${total} images...`, 0);

        const tasks = images.map((img, index) => async () => {
            try {
                // 1. Get original URL
                const originalUrl = replaceWithNormalSize(img.src);
                
                // 2. Fetch Blob
                const blob = await fetchBlob(originalUrl);

                // 3. Convert to Image for processing
                let finalBlob = blob;
                
                if (engine) {
                    try {
                        const blobUrl = URL.createObjectURL(blob);
                        const imageObj = await loadImage(blobUrl);

                        // 4. Remove Watermark
                        const processedCanvas = await engine.removeWatermarkFromImage(imageObj);
                        
                        // 5. Convert back to Blob
                        finalBlob = await canvasToBlob(processedCanvas);
                        URL.revokeObjectURL(blobUrl);
                    } catch (e) {
                        console.warn("Watermark removal failed, falling back to original", e);
                    }
                }
        ...
        // Concurrency Control
        await downloadWithConcurrency(tasks, 3);

        showToast(`All downloads complete!`, 3000);
