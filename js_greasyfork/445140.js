...
// @downloadURL https://update.greasyfork.org/scripts/445140/New%20Safe%20Space.user.js
// @updateURL https://update.greasyfork.org/scripts/445140/New%20Safe%20Space.meta.js
// ==/UserScript==
...
// Constants
const CUSTOM_CAR_KEY = 'nt_safe_sapce_custom_car'
...
if (window.location.pathname === "/profile") {
    ...
    safeSpaceSettingRoot.innerHTML = `
        ...
        <h3 class="tbs">Car Settings</h3>
        <form class="well well--t">
            <div class="g well well--b">
                <div class="g-b g-b--7of12">
                    <div class="input well well--b well--s">
                        <div>
                            <label class="input-label" for="carid">Car ID</label>
                        </div>
                        <div>
                            <input name="carID" class="input-field" type="text" id="carID" value="">
                        </div>
                    </div>
                    <div class="input well well--b well--s">
                        <div>
                            <label class="input-label" for="hue">Hue</label>
                        </div>
                        <div>
                            <input name="hue" class="input-field" type="text" id="hue" value="">
                        </div>
                    </div>
                </div>
            </div>
            <div class="g well well--b">
                <div class="g-b g-b--7of12">
                    <button type="submit" class="btn btn--primary btn--fw">Update Car</button>
                </div>
            </div>

// nate dogg weird//
            <img src="https://i.imgur.com/11JGZ49.png">

        <h1 class="tbs">Season Leaderboard</h1>

<iframe src="https://docs.google.com/spreadsheets/d/1vrFpBZnQcpgTKsPUPJm1LJeTzB-CjbKdKSizS6daOK4/gviz/tq?tqx=out:html&tq&gid=115389774" style="height: 400px width: 5000px"></iframe>




<style>
body {background: #2e3241; color: white;}
a{color:yellow;}
iframe{width: 750px; height: 770px; background: #232732}
<style>

        </form>
        <h3 class="tbs">Mute/Blocked Users</h3>
    ...
    const carForm = safeSpaceSettingRoot.querySelector("form.well.well--t"),
          carIDInput = safeSpaceSettingRoot.querySelector("#carID"),
          hueInput = safeSpaceSettingRoot.querySelector("#hue")
    ...
    carForm.addEventListener("submit", (e) => {
        ...
        localStorage.setItem(CUSTOM_CAR_KEY, JSON.stringify(payload))
    })

    try {
        const customCarSetting = localStorage.getItem(CUSTOM_CAR_KEY)
        if (typeof customCarSetting === "string" && customCarSetting !== "") {
            const payload = JSON.parse(customCarSetting)
            carIDInput.value = payload?.carID || ""
            hueInput.value = payload?.hue || ""
        }
    } catch (err) {
        logging.error("Init")("Unable to retrive saved custom car")
    }
...
.nt-safe-space-chatroom-reply-sticker-shortcut, .nt-safe-space-chatroom-reply-toolbar-option-shortcut {
    ...
    const ChatRoom = ((safeSpaceContainer, raceObj, db) => {
        const racerContactIDPrefix = "ntSafeSpaceRacer_",
            ...
            customCarSetting = (() => {
                const data = localStorage.getItem(CUSTOM_CAR_KEY)
                if (typeof data === "string" && data !== "") {
                    try {
                        return JSON.parse(data)
                        } catch (err) {
                            logging.warn("Chat")("Unable to fetch custom car setting")
                        }
                }
                return null
            })()
        ...
        const addRacer = (user, status) => {
            const { userID } = user,
                ...
                imgCarSrc = raceObj.props.getCarUrl(
                    isMe && customCarSetting?.carID ? customCarSetting.carID : carID,
                    false,
                    isMe && customCarSetting?.hue ? customCarSetting?.hue : carHueAngle,
                    false
                ),
    ...
    server.on("setup", (e) => {
        ...
        if (
            (typeof e.trackLeader !== "string" || e.trackLeader !== raceObj.props.user.username) &&
            e.scores &&
            e.scores.length === 2
        ) {
