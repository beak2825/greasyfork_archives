...
// ==UserScript==
// @license MIT
// @description      Nitro typer autlo typer for chromebooks (works for windows and mac as well)
// @name             Chromebot - nitmro type bot
...
// @downloadURL https://update.greasyfork.org/scripts/468623/Chromebot%20-%20nitmro%20type%20bot.user.js
// @updateURL https://update.greasyfork.org/scripts/468623/Chromebot%20-%20nitmro%20type%20bot.meta.js
// ==/UserScript==
...
async function main(ws) {
    ...
    async function type(msg, speed, accuracy, nitros='true') {
        ...
        await sleep(4.3);
        ws.send('5{"stream":"race","msg":"update","payload":{"t":1,"f":0}}') //First character has to have "f":0
        var t = 2
        var e = 1
        console.log(words);
        for (w in words) {
            if(int(len(w)) >= int(biggestWord) && usedNitro == false) {
                t += len(w)
                var payload = '5{"stream":"race","msg":"update","payload":{"n":1,"t":' + str(t) + ',"s":' + str(len(w)) + '}' //Using nitro
                ws.send(payload)
                await sleep(0.2)
                payload = '5{"stream":"race","msg":"update","payload":{"t":' + str(t) + '}}' //sending another character
                ws.send(payload)
                t += 1
                usedNitro = true
            } else {
                for (var c in w) {
                    ...
                    if(accuracyWrongPercentage >= errorProbability) {
                        payload = '5{"stream":"race","msg":"update","payload":{"e":' + str(e) + '}}' //sending error
                        ws.send(payload)
                        e += 1
                    }
                    if ((t % 4 == 0) || (t >= (numOfChars - 4))) {
                        payload = '5{"stream":"race","msg":"update","payload":{"t":' + str(t) + '}}' //sending character
                        ws.send(payload)
                    }
                    t += 1
