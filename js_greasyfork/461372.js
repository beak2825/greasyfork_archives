...
// @downloadURL https://update.greasyfork.org/scripts/461372/%E4%B8%87%E7%BE%A4%E5%BC%95%E6%93%8E%E6%96%87%E6%A1%88%E8%BD%AC%E9%93%BE.user.js
// @updateURL https://update.greasyfork.org/scripts/461372/%E4%B8%87%E7%BE%A4%E5%BC%95%E6%93%8E%E6%96%87%E6%A1%88%E8%BD%AC%E9%93%BE.meta.js
// ==/UserScript==
       ...
       checkUserInfo().then(__user_id => {
        ...
        Promise.all(pa).then(() => {
         parents.addClass('cov-ok')
            let wenan = [];
            parents.find('span.block').each(function(index,item) {
         // console.log('index',index,'item',item)
            const $item = $(item)
            wenan.push($item.hasClass('hover-underline')?$item.html()+'\n':$item.html()) ;
        })
            wenan= wenan.join('')
        btn.attr('data-clipboard-text',wenan);
        tip.text('转链完毕,点击复制').attr('data-clipboard-text',wenan).mouseup(function() {
             const clipboard = new ClipboardJS(this);
