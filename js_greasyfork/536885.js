...
// @version      1
// @description  Calculates ranked war targets, API fetching, draggable & minimizable UI (toggle in top-right).
// @author       TR0LL
...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/536885/Torn%20Ranked%20War%20Calculator.user.js
// @updateURL https://update.greasyfork.org/scripts/536885/Torn%20Ranked%20War%20Calculator.meta.js
// ==/UserScript==
...
(function() {
    ...
    GM_addStyle(`
        #warCalcContainer {
            ...
            flex-direction: column; top: 80px; right: 10px; /* Default panel position */
        }
        #warCalcHeader {
        ...
        #fetchApiDataButton, #warCalcButton, #resetWarCalcButton { padding: 8px 12px; margin-top: 8px; font-size: 12px; letter-spacing: 0.3px; color: white; border: none; border-radius: 4px; font-weight: bold; cursor: pointer; width: 100%; transition: all 0.15s ease-in-out; text-transform: uppercase; }
        ...
        #warCalcButton:hover { transform: translateY(-1px) scale(1.01); box-shadow: 0 3px 10px rgba(0,0,0,0.2); background: linear-gradient(45deg, #2ebf4e, #259d40); }
        #resetWarCalcButton { background: linear-gradient(45deg, #dc3545, #c82333); }
        #resetWarCalcButton:hover { transform: translateY(-1px) scale(1.01); box-shadow: 0 3px 10px rgba(0,0,0,0.2); background: linear-gradient(45deg, #e04b59, #d32f3f); }
        ...
        #warCalcToggleButton {
            ...
            width: 45px;
            height: 45px;
            font-size: 18px; /* For emoji icon */
            font-weight: bold;
            line-height: 45px; /* Vertically center emoji */
    ...
    container.innerHTML = `
        ...
        <div id="warCalcContent">
            <div class="war-calc-input-section">
                ...
                <input type="password" id="apiKeyInput" placeholder="Enter API Key (optional)">
    ...
    toggleButton.innerHTML = '🎯'; // Default: show panel icon
    ...
    function applyPanelVisibility() {
        if (isPanelVisible) {
            ...
            toggleButton.innerHTML = '🔽'; // Panel is visible, button shows "hide" icon
        } else {
            container.style.display = 'none';
            toggleButton.innerHTML = '🎯'; // Panel is hidden, button shows "show" icon
        }
    }
    ...
    applyPanelVisibility(); // Apply initial state
    ...
    else { /* Default CSS position (top: 80px; right: 10px;) is used if nothing saved */ }
    ...
    // --- GET ELEMENTS & 나머지 스크립트 로직 ---
    // (The rest of the JavaScript: element selections, state variables, helper functions, event listeners, etc.
    // remains the same as in version 1.3.2 / API version. Ensure all element IDs match.)
    ...
    function updateDisplay() { /* ... (same as v1.3.2) ... */
    ...
    function clearAndReset() { /* ... (same as v1.3.2) ... */
        if (intervalId) clearInterval(intervalId); intervalId = null; if(apiKeyInputEl) apiKeyInputEl.value = GM_getValue('apiKey', ''); if(warInfoPasteEl) warInfoPasteEl.value = ''; if(initialTargetScoreEl) initialTargetScoreEl.value = ''; if(desiredFinalTargetScoreEl) desiredFinalTargetScoreEl.value = '0'; if(factionANameEl) factionANameEl.value = ''; if(factionAScoreEl) factionAScoreEl.value = '0'; if(factionBNameEl) factionBNameEl.value = ''; if(factionBScoreEl) factionBScoreEl.value = '0'; warStartDate = null; originalTargetScore = 0;
        const keysToClear = ['warInfoPaste', 'initialTargetScore', 'factionAName', 'factionAScore', 'factionBName', 'factionBScore']; keysToClear.forEach(key => GM_setValue(key, (key.includes('Score') && !key.includes('initial')) ? '0' : ''));
        if(displayDiv) displayDiv.innerHTML = '<div class="status-message">Enter data or use API fetch, then Process.</div>'; if(apiStatusMessageEl) apiStatusMessageEl.textContent = 'Enter API Key and Fetch for live data.'; console.log("War Calculator Reset.");
    }
    if(resetButton) resetButton.addEventListener('click', clearAndReset);
    function processAndCalculate() { /* ... (same as v1.3.2) ... */
    ...
    if(calculateButton) calculateButton.addEventListener('click', processAndCalculate);
    if(fetchApiDataButtonEl) fetchApiDataButtonEl.addEventListener('click', () => { /* ... (API fetch logic same as v1.3.2) ... */
        const apiKey = apiKeyInputEl ? apiKeyInputEl.value.trim() : ''; if (!apiKey) { if(apiStatusMessageEl) { apiStatusMessageEl.textContent = 'API Key is required.'; apiStatusMessageEl.style.color = '#F04747';} return; } if(apiStatusMessageEl) { apiStatusMessageEl.textContent = 'Fetching data...'; apiStatusMessageEl.style.color = '#ffae42';}
        GM_xmlhttpRequest({ method: "GET", url: `https://api.torn.com/faction/?selections=rankedwars&key=${apiKey}&comment=RankedWarCalculatorV1.3.4`,
            onload: function(response) { try { const data = JSON.parse(response.responseText); if (data.error) { if(apiStatusMessageEl) { apiStatusMessageEl.textContent = `API Error: ${data.error.error} (Code: ${data.error.code})`; apiStatusMessageEl.style.color = '#F04747';} console.error("Torn API Error:", data.error); return; }
                if (data.rankedwars && Object.keys(data.rankedwars).length > 0) { let activeWarFound = false;
                    for (const warId in data.rankedwars) { const warData = data.rankedwars[warId]; if (warData.war && warData.war.end === 0) { activeWarFound = true; warStartDate = new Date(warData.war.start * 1000); originalTargetScore = warData.war.target; if(initialTargetScoreEl) initialTargetScoreEl.value = originalTargetScore; const factionKeys = Object.keys(warData.factions);
                        ...
                        if(apiStatusMessageEl) { apiStatusMessageEl.textContent = 'Active war data loaded! Processed.'; apiStatusMessageEl.style.color = '#28a745';}
    ...
    if(apiKeyInputEl) apiKeyInputEl.addEventListener('change', () => GM_setValue('apiKey', apiKeyInputEl.value.trim()));
    ...
    console.log("Torn Ranked War Calculator (Inspired Design v1.3.4 Minimizable Top-Right) Loaded.");
})();