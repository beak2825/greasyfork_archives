...
// @version      1.0.4
// @author       Blu
// @description  A userscript to add Sexy back to bonk.io
// @match        https://bonk.io/gameframe-release.html
// @run-at       document-start
// @grant        none
// @namespace https://greasyfork.org/users/826975
// @downloadURL https://update.greasyfork.org/scripts/459931/bonk-sexy.user.js
// @updateURL https://update.greasyfork.org/scripts/459931/bonk-sexy.meta.js
// ==/UserScript==
...
function injector(src){
  ...
  // Adds Sexy to mode selection button
  newSrc = newSrc.replace(modearrayMatch[0], `=[${modearrayMatch[1]},"${modeName}",${modearrayMatch[2]}]`);

  // locate Sexy mode data initialisation
  const SEXYDATA_REGEX = new RegExp(`(${modeIndices[modeName]}\\)]={lobbyName:${localObject}\\.[\\w$]{1,3}\\(.*?),editorCanTarget:false}`);
  let sexydataMatch = newSrc.match(SEXYDATA_REGEX);
  // Adds Sexy to map editor "For mode:" selector
  newSrc = newSrc.replace(sexydataMatch[0], `${sexydataMatch[1]},editorCanTarget:true}`);
