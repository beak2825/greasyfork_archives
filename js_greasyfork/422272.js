...
(function () {
    ...
    const APPLICATION_PARAM = {
        ...
        SEASON_COUNT_LOCAL_STORAGE_KEY: "TMVN_CLUB_H2H_SEASON_COUNT"
    }

    const CONTROL_ID = {
        INPUT_SEASON_COUNT: 'tmvn_club_h2h_script_input_season_count',
        BUTTON_SEASON_COUNT: 'tmvn_club_h2h_script_button_season_count_set'
    }
    ...
    if (seasonCount == null || seasonCount == "") {
        ...
        seasonCount = parseInt(seasonCount);
    }
    ...
    function checkHeadToHead() {
        ...
        try {
            currentSeason = parseInt($('#top_menu a[class="none white small"]')[0].innerText.split(/(\s+)/)[2]);
        } catch (e) {
            currentSeason = 0;
        }
        ...
        $.ajax(headToHeadUrl, {
            ...
            success: function (response) {
                ...
                if (matches.length == 0) {
                    ...
                    // Lấy danh sách các key (seasons)
                    let seasonKeys = Object.keys(matches);

                    // Nếu seasonCount > 0, chỉ lấy những mùa >= (currentSeason - seasonCount + 1)
                    let filteredSeasonKeys = seasonKeys;
                    if (seasonCount > 0 && currentSeason > 0) {
                        let fromSeason = currentSeason - seasonCount + 1;
                        filteredSeasonKeys = seasonKeys.filter(key => parseInt(key) >= fromSeason);
                    }

                    // Duyệt qua các mùa đã được filter
                    filteredSeasonKeys.forEach(function (key) {
        ...
        let headToHead =
            ...
            "<tr>" +
            "<td>" +
            "<span style='display: inline-block;'><input id='" + CONTROL_ID.INPUT_SEASON_COUNT + "' type='text' class='embossed' style='width: 150px; line-height: 100%; padding: 3px 3px 4px 3px;' placeholder='Count'></span>" +
            "</td>" +
            "<td>" +
        ...
        // Set giá trị cho textbox
        if (seasonCount == APPLICATION_PARAM.DEFAULT_SEASON_COUNT) {
            $('#' + CONTROL_ID.INPUT_SEASON_COUNT).val('');
        } else {
    ...
    function setSeasonCount() {
        ...
        if (seasonCount == '') {
            ...
            alert('Season count must be positive integer. Leave blank to show all seasons.');
        } else {
