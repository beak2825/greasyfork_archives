...
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/549294/4ndr0tools-Pixverse%2B%2B.user.js
// @updateURL https://update.greasyfork.org/scripts/549294/4ndr0tools-Pixverse%2B%2B.meta.js
// ==/UserScript==
...
(() => {
  ...
  function modifyPlanDetails(data) {
     log('Bypassing Plan Details for available qualities, originally:', data?.Resp?.qualities);
     return {
        ...data,
        Resp: {
            ...data.Resp,
  ...
  function modifyResponse(data, url) {
    ...
    else if (url.includes("/creative_platform/members/plan_details")) return modifyPlanDetails(data);
      ...
      XMLHttpRequest.prototype.send = function (body) {
        ...
        if ((url.includes('/media/batch_upload_media') || url.includes('/media/upload')) && currentBody) {
          try {
            ...
            if (url.includes('/media/batch_upload_media')) {
                // Make secondary POST request
                fetch(url.replace("batch_upload_media","upload"), {
                    method: 'POST',
                    headers: {
                    ...this._headers,
                    'Cookie': document.cookie
                    },
                    body: JSON.stringify(data?.images?.[0]) // or JSON.stringify(parsedBody) if it's JSON
                })
            }
  ...
  function setupWatermarkButton() {
    ...
    function tryAttachButton() {
      let watermarkDiv = Array.from(document.querySelectorAll('div[role="menuitem"] span')).find(
