...
(function (vue) {
  ...
  const getContent = (ID, ResCode) => {
    return new Promise(function(resolve, reject) {
      _GM_xmlhttpRequest({
        ...
        // url: `http://localhost:8000/api/fanqie?id=${ID}&code=${ResCode}`,
        headers: {
          ...
          if (res.status == "403") {
            ...
            if (res.response !== void 0) {
              if (res.response == "503") {
                resolve({
                  code: 503,
                  info: "因接口压力，每日限制免费解析30次，如需增加解析次数，请到https://www.softrr.cn/上的公众号：软件爬取者，后台加微信！"
                });
              } else {
  ...
  const _withScopeId = (n) => (vue.pushScopeId("data-v-1bf44c6b"), n = n(), vue.popScopeId(), n);
      ...
      const onPrase = async () => {
        ...
        if (locaCode == code.value) {
          ...
          if (await getContent(extractedId.value) !== "无法解析") {
            ...
            if (contentList.code != 200) {
              content = "<h1>" + contentList.info + "</h1>";
            } else {
              content = "<p>" + contentList.data + "<p/>";
  ...
  const App = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-1bf44c6b"]]);
