...
// @version       4.2
// @description   Fires custom Compass routes with infinite loops. Optimized for speed using fast retry-logic.
// @author        anon
...
// @grant         GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/559228/%F0%9F%A7%AD%20Compass%20Route%20Runner.user.js
// @updateURL https://update.greasyfork.org/scripts/559228/%F0%9F%A7%AD%20Compass%20Route%20Runner.meta.js
// ==/UserScript==
...
(function () {
    ...
    /**
     * Finds an element using a finder function, retrying quickly until timeout.
     * @param {function(): HTMLElement | null} finderFn - Function to find the element.
     * @param {number} timeoutMs - Maximum time to wait (e.g., 1000ms).
    ...
    function getCurrentState() {
        const stateRow = [...document.querySelectorAll('.box table tr')]
            .find(tr => {
                ...
                // Added "Durum" for Turkish support
    ...
    function checkStateAndResume() {
        ...
        // Check if the CURRENT step (state.index) is a 'WAIT' command
        ...
        // If state is Normal after a reload, advance and continue
        if (current === "Normal") {
        ...
        // Clean up previous (failed) timer log from storage before starting a new one.
    ...
    /**
     * UPDATED: Now looks for any .notification-error div without checking text.
     */
    function checkMovementError() {
        const notif = document.querySelector("#notifications .notification-error");
        if (notif) {
            // Stop immediately if any error notification exists
    ...
    async function initialize() {
        console.log("Compass Runner (Infinite) initializing...");
        ...
        const state = loadState();
        if (state?.active) {
            // CRITICAL FIX: Wait 500ms for the error notification to be fully rendered in the DOM
            await sleep(500);
            ...
            // Original delay for performNext, executed after the successful check
            setTimeout(() => {
                // FORCE performNext to handle the WAIT logic checks.
                performNext();
            }, 50);
        }
    }
...
const reverseDirection = d => {
    ...
    const findDirEl = d => {
        ...
        // Using querySelectorAll instead of querySelector to ensure we catch all potential elements
        const elements = document.querySelectorAll(`g[data-dir="${nd}"], a[href*="/MoveTo/"][title*="${nd}"]`);
        // Return the first one found
        return elements.length > 0 ? elements[0] : null;
    };

    // Parse route to handle Infinite loops
    function parseRoute(routeStr) {
        ...
        while (i < segments.length) {
            ...
            // Matches "Infinite:" followed by the direction string
            const infiniteMatch = seg.match(/^Infinite:(.+)$/i);

            if (infiniteMatch) {
                // Split the directions by comma, space, slash, OR DASH (-)
                const dirs = infiniteMatch[1].split(/[\s,/-]+/).filter(Boolean);
                i++;

                // Look for the mandatory "Item:" segment following the Infinite loop
                ...
                // Error if "Item:" part is missing or incorrectly formatted
                parsed.push({ type: 'error', step: seg });
                continue;
            }

            // Normal movement steps
    ...
    async function performNext() {
        ...
        try {
            ...
            // ✅ GLOBAL ERROR CHECK (runs after every reload & before any action)
    ...
    function setupEventListeners(ui) {
        ...
        ui.querySelector("#btn-resume").addEventListener("click", () => {
            ...
            if (s && !s.active && s.index < s.parsedPath?.length) {
                s.active = true;
                saveState(s);
                // ALWAYS call performNext to allow logic checks to run
                performNext();
            } else {
    ...
    // 2. Explicitly clear the text from the input field
    const input = ui.querySelector("#compass-input");
    if (input) {
        input.value = "";
    }

    // 3. Refresh the UI to reset the log and button states
    updateUI();
});
