...
// @downloadURL https://update.greasyfork.org/scripts/493027/Sploopio-Modifications%20%5B%20spikeMod%20-%20LegitMod%20%5D%20-.user.js
// @updateURL https://update.greasyfork.org/scripts/493027/Sploopio-Modifications%20%5B%20spikeMod%20-%20LegitMod%20%5D%20-.meta.js
// ==/UserScript==
...
document.getElementById("modBind").addEventListener("mousedown", e => {
    ...
    jQuery(function() {
        if (localStorage.spikeInput) {
            ...
            jQuery('#hatStore, #clanMenu, #homeDisplay, #ThinStore, #betterStore, #antiName, #BiggerR, #classicA, #sandboxA, #opacityDisplay, #cpsDisplay, #fpsDisplay, #serverDisplay, #gameDisplay, #showspike').prop('checked', function(i) {
                return checks[i];
            });
        }
    });
    jQuery('#hatStore, #clanMenu, #homeDisplay, #ThinStore, #betterStore, #antiName, #BiggerR, #classicA, #sandboxA, #opacityDisplay, #cpsDisplay, #fpsDisplay, #serverDisplay, #gameDisplay, #showspike').on('change', function() {
        localStorage.spikeInput = JSON.stringify(jQuery('#hatStore, #clanMenu, #homeDisplay, #ThinStore, #betterStore, #antiName, #BiggerR, #classicA, #sandboxA, #opacityDisplay, #cpsDisplay, #fpsDisplay, #serverDisplay, #gameDisplay, #showspike').map(function() {
            return this.checked;
        }).get());
    });
</script>
