...
// @downloadURL https://update.greasyfork.org/scripts/499472/Anime4K.user.js
// @updateURL https://update.greasyfork.org/scripts/499472/Anime4K.meta.js
// ==/UserScript==
...
const isBilibili = () => window.location.href.toLowerCase().includes("bilibili.com")
const isAcfun = () => window.location.href.toLowerCase().includes("acfun.cn")
...
Scaler.prototype.resizeBoard = function (originRatio, newRatio) {
  ...
  console.log('globalBoard.style.width: ', globalBoard.style.width, 'globalBoard.style.height', globalBoard.style.height)
}
...
Scaler.prototype.render = async function () {
  ...
  if (document.fullscreenElement != null) {  // To prevent float precision caused problem.
    ...
    if (this.isFullscreen) {
      ...
      globalBoard.style.height = isAcfun() ? "calc(100% - 40px)" : "100%"
...
async function injectCanvas() {
  ...
  // console.log('globalMovOrig: ', globalMovOrig)
  ...
  if (isBilibili()) {
    ...
    while (div.className != "bpx-player-video-wrap") {
  ...
  if (!globalBoard) {
    ...
    // acfun 去掉弹幕控制层高度
    console.log("isAcfun: ", isAcfun)
    globalBoard.style.height = isAcfun() ? "calc(100% - 40px)" : '100%';
    console.log(globalBoard.style.height)
...
async function doFilter() {
  ...
  getScreenRefreshRate(function (screenRefreshRate) {
    console.log("real fps " + screenRefreshRate + " FPS.");
