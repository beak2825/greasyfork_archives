// ==UserScript==
// @name         Blooket Cheat Script (SPIDEYBYTE UPDATE)
// @description  A script for Blooket cheats, with various functionalities and checks. OG MOD FROM 005KONZ NOT ME I JUST ADDED STUFF
// @namespace    https://github.com/005Konz/Blooket-Cheats
// @version      1.0.0
// @license      StewartPrivateLicense-2.0.1
// @match        *://*.blooket.com/*
// @downloadURL https://update.greasyfork.org/scripts/531556/Blooket%20Cheat%20Script%20%28SPIDEYBYTE%20UPDATE%29.user.js
// @updateURL https://update.greasyfork.org/scripts/531556/Blooket%20Cheat%20Script%20%28SPIDEYBYTE%20UPDATE%29.meta.js
// ==/UserScript==
...
(() => {
    ...
    const cheat = (async () => {
        ...
        let settings, settingsKey = "SpideyByte";
        const Settings = {
            data: null,
            setItem(k, v) {
                    ...
                    inputs: [
                        {
                            name: "Gamemode",
                            type: "options",
                            options: ["Racing", "Classic", "Factory", "Cafe", "Defense2", "Defense", "Royale", "Gold", "Candy", "Brawl", "Hack", "Pirate", "Fish", "Dino", "Toy", "Rush", "Tag"]
                        }
                    ],
                    run: function (type) {
            ...
            crypto: [
                {
                    ...
                    run: function () {
                        if (!this.enabled) {
                            ...
                            {
                    name: "Auto Choose",
                    description: "Automatically picks the option that would give you the most crypto",
                    type: "toggle",
                    enabled: false,
                    data: null,
                    run: function () {
                                    ...
                                    stateNode.props.liveGameController.getDatabaseVal("c", (players) => {
                                        try {
                                            ...
                                            for (let i = 0; i < stateNode.state.choices.length; i++) {
                                                ...
                                                let value = stateNode.state.crypto;
                                                if (choice.type == "crypto")
                                                    value = stateNode.state.crypto + choice.val || stateNode.state.crypto;
                                                else if (choice.type == "multiply" || choice.type == "divide")
                                                    value = Math.round(stateNode.state.crypto * choice.val) || stateNode.state.crypto;
                                                else if (choice.type == "swap")
                                                    value = most || stateNode.state.crypto;
                                                else if (choice.type == "take")
                                                    value = stateNode.state.crypto + most * choice.val || stateNode.state.crypto;
        ...
        addMode("Extra Things", null, Cheats.extrathings, true);
    ...
    img.onerror = img.onabort = () => {
        ...
        cheat();
        let iframe = document.querySelector("iframe");
        iframe.contentWindow.alert("IF I GET RID OF THIS EVERYTHING BREAKS SORRY")
    }
})();