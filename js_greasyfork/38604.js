...
// @downloadURL https://update.greasyfork.org/scripts/38604/Pixiv%20-%20Manga%20Viewer.user.js
// @updateURL https://update.greasyfork.org/scripts/38604/Pixiv%20-%20Manga%20Viewer.meta.js
// ==/UserScript==

(function() {

    var original_solo_images = false;   // Replace individual illusts with original image
    var solo_images_in_new_tab = true;  // Open individual illusts in new tab
    var original_manga_images = false;  // Replace default manga illusts with original images
    var load_images_one_by_one = false; // Load manga illusts one by one instead of load all at the same time
    var manga_images_counting = true;   // Display manga image counting number

    var external_compressor = true;     // Not recommended, very powerful external image compressor (250KB = 60KB)
    ...
    function request() {
        ...
        xhr.onerror = function(error) {
            if (xhr.readyState == 0) {
                ...
                const retry = retryCount < retryLimit ? retryCount++ & setTimeout(request, 3000) : null;
            }
            console.error('Pixiv - Manga Viewer Error : ' + url + count);
        };
    }
    ...
    function response() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            const retry = retryCount > 0 ? retryCount = 0 : null;
        }
    }
