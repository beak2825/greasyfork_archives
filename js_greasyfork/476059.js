...
// @description       原作者https://github.com/zhengbangbo/chat-gpt-userscript,因為和 DuckDuckGo 不適配，故做了些改動
// @homepageURL       https://github.com/zhengbangbo/chat-gpt-userscript/
// @supportURL        https://greasyfork.org/zh-TW/scripts/456077

// @license            MIT
// @icon               https://github.com/mr-smilin/oss/raw/main/logo/chat-gpt-userscript.png
...
// @downloadURL https://update.greasyfork.org/scripts/476059/ChatGPT%20%E6%90%9C%E7%B4%A2%2020.user.js
// @updateURL https://update.greasyfork.org/scripts/476059/ChatGPT%20%E6%90%9C%E7%B4%A2%2020.meta.js
// ==/UserScript==

((e) => {
	const t = document.createElement("style");
	(t.dataset.source = "vite-plugin-monkey"),
		(t.innerText = e),
...
(function () {
	...
	var GM_registerMenuCommand = /* @__PURE__ */ (() =>
		monkeyWindow.GM_registerMenuCommand)();
	var GM_unregisterMenuCommand = /* @__PURE__ */ (() =>
		monkeyWindow.GM_unregisterMenuCommand)();
	var GM_xmlhttpRequest = /* @__PURE__ */ (() =>
	...
	function containerShow(answer) {
		...
		container2.innerHTML =
			'<p><span class="prefix">ChatGPT</span><pre></pre></p>';
	...
	function alertLogin() {
		containerAlert(
			`<p>${i18n(
				"login"
			)}<a href="https://chat.openai.com" target="_blank" rel="noreferrer">chat.openai.com</a></p>`
		);
	}
				...
				return function (stream) {
					...
					reader.read().then(function processText({ done, value }) {
						...
						if (responseItem.startsWith("data: {")) {
							const responseItemJson = JSON.parse(responseItem.slice(6));
							if (responseItemJson.is_completion) return;
							const answer =
								responseItemJson.message === undefined
									? ""
									: responseItemJson.message.content.parts[0];
							containerShow(answer);
						} else if (responseItem.startsWith("data: [DONE]")) {
							console.log("使用到 DONE");
							return;
						}
	...
	function initUI() {
		...
		function duckduckgoInjectContainer() {
			...
			const siderbarContainer =
				document.getElementsByClassName("js-react-sidebar")[0];
			GM_addStyle(
				".chat-gpt-container{background-image: linear-gradient(to bottom, #008b8b, #ffff00)}"
			);
			!!siderbarContainer && siderbarContainer.prepend(container2);
		}
		function deeplInjectContainer() {
			...
			document
				.getElementsByClassName("lmt__textarea_container")[0]
				.appendChild(button);
			button.addEventListener("click", function () {
				...
				try {
					document
						.getElementsByClassName("lmt__raise_alternatives_placement")[0]
						.insertBefore(
							container2,
					...
					document
						.getElementsByClassName("lmt__textarea_container")[1]
						.insertBefore(
							container2,
							document.getElementsByClassName("lmt__translations_as_text")[0]
						);
				}
				let outlang = document.querySelectorAll(
					"strong[data-testid='deepl-ui-tooltip-target']"
				)[0].innerHTML;
				let question =
	...
	function initMenu() {
		let position_id = GM_registerMenuCommand(
			i18n("containerPosition") + getPosition(),
			position_switch,
			"M"
		);
		function position_switch() {
			...
			position_id = GM_registerMenuCommand(
				i18n("containerPosition") + getPosition(),
				position_switch,
				"M"
			);
