...
// @downloadURL https://update.greasyfork.org/scripts/518224/Ex%E6%B5%AE%E5%8B%95%E9%A1%AF%E7%A4%BATag.user.js
// @updateURL https://update.greasyfork.org/scripts/518224/Ex%E6%B5%AE%E5%8B%95%E9%A1%AF%E7%A4%BATag.meta.js
// ==/UserScript==
...
(function() {
    ...
    if ((spl[3].substr(0, 1).match(/[?#fptw]/i) && !spl[3].startsWith('toplist')) || !spl[3]) {
        ...
        $('.itg').on('mouseover', element, function(e) {
            if ($('#tagPreview').children().length > 2) { $tagP.empty(); }

            title = this.children[0].title; // Save the title so we can put it back later
            this.children[0].title = ""; // Clear the title so we don't have it over our new window

            const str = this.href.split('/');
            generateRequest(str[4], str[5]);

            const posX = (e.pageX + 432 < screen.width) ? e.pageX + 10 : e.pageX - 412;
            let posY;
            ...
            if ((scrollHeight - scrollPosition) < (scrollHeight / 10)) {
                posY = (e.pageY + $('#tagPreview').height() < scrollHeight) ? e.pageY + 10 : e.pageY - 10 - $('#tagPreview').height();
            } else {
                posY = e.pageY + 10;
            }
    ...
    function generateListing(apirsp) {
        generateTags(apirsp); // Generate the tag list from the raw JSON file
        ...
        for (const namespace in tags) {
            ...
            for (let i = 0; i < tags[namespace].length; i++) {
    ...
    function generateRequest(gid, token) {
        const reqList = [[gid, token]]; // We use an array for our gidlist, since the API can handle up to 25 galleries per request
        const request = {"method": "gdata", "gidlist": reqList, "namespace": 1};

        const req = new XMLHttpRequest();
        req.onreadystatechange = function() {
            if (this.readyState === 4 && this.status === 200) {
                const apirsp = JSON.parse(this.responseText);
                for (let i = 0; i < apirsp.gmetadata.length; i++) {
                    generateListing(apirsp.gmetadata[i]);
                }
            }
        };
