...
function encode_key(dfsId){
	var dfsId = dfsId.toString();
	var table = '3go8&$8*3*3h0k(2)2';
	var key = '';

	for(var i = 0; i < dfsId.length; i++){
		key += String.fromCharCode(dfsId.charCodeAt(i) ^ table.charCodeAt(i % table.length));
	}
	
	results = unsafeWindow.CryptoJS.MD5(key).toString(unsafeWindow.CryptoJS.enc.Base64);
	results = results.replace(/\//g, '_').replace(/\+/g, '-');

	return results;
}

function encrypt(reqObj){
	var encryptedData = unsafeWindow.asrsea(JSON.stringify(reqObj), '010001', '', '0CoJUm6Qyw8W8jud');
	var postData = 'params=' + encodeURIComponent(encryptedData.encText)+ '&encSecKey=' + encodeURIComponent(encryptedData.encSecKey);
	
	return postData;
}

function searchFM(){
	window.open('http://itwusun.com/search/wy/song_' + this.getAttribute('data-res-id') + '.html');
}

function detail(id, ver, cb){
	var mp3 = {
		'br': 0,
		'mp3Url': ''
	};
	...
	switch(ver){
		case 'v1':
			var url = 'http://music.163.com/weapi/v1/song/detail?csrf_token=';
			var data = {
				'ids': [id],
				'csrf_token': ''
			};
	...
	GM_xmlhttpRequest({
		...
		'data': encrypt(data),
		'onload': function(res){
			var song = JSON.parse(res.responseText.replace(/"fid":(\d+)/g, '"fid":"$1"')).songs[0];
			song = song.h || song.m || song.l || {};
			
			if(song.fid && song.fid != '0'){
				var xhr = GM_xmlhttpRequest({
					'method': 'GET',
					'url': 'http://music.163.com/api/img/blur/' + song.fid,
					'timeout': 5000,
...
function cloud(id, cb){
	...
	GM_xmlhttpRequest({
		'method': 'GET',
		'url': 'http://music.163.com/api/cloud/media/url?uid=32645756&id=' + id,
		'headers': {
			'Cookie': 'os=pc',
			'Referer': 'http://music.163.com/'
		},
...
function itwusun(id, ver, cb){
	...
	switch(ver){
		case 'json':
			var url = 'http://api.itwusun.com/music/song/wy/' + id + '?format=json&sign=a5cc0a8797539d3a1a4f7aeca5b695b9';
			var headers = {'User-Agent': 'Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident/6.0)'};
			var resType = 'json';
			break;
			
		case 'html':
			var url = 'http://itwusun.com/search/wy/song_' + id + '.html';
			var headers = {'Referer': 'http://itwusun.com/'};
			var resType = 'document';
			break;
	}
	...
	GM_xmlhttpRequest({
		...
		'onload': function(res){
			switch(ver){
				case 'json':
					var url = res.response[0].CopyUrl;
					break;
					
				case 'html':
					var url = res.response.querySelector('.song-link').value;
					break;
			}
			...
			var xhr = GM_xmlhttpRequest({
				...
				'onreadystatechange': function(res){
					switch(res.readyState){
						...
						case 4:
							if(res.finalUrl.search(/m2\.music\.126\.net/) == -1){
								mp3.br = parseInt(url.match(/wy_(\d+)_/)[1]+'000', 10);
								mp3.mp3Url = res.finalUrl;
							}
							cb(mp3);
							break;
...
function download(){
	...
	if(location.pathname=='/song'){
		var state = DLB.firstElementChild;
		var artist = document.querySelector('.des.s-fc4').firstElementChild.textContent.replace(/ \//g, ',');
		var name = document.querySelector('.f-ff2').textContent;
	}else{
		var state = DLB.parentElement.parentElement.firstElementChild;
		var par = DLB.parentElement.parentElement.parentElement;
		var artist = ((location.pathname=='/artist') ? document.querySelector('#artist-name').textContent : par.querySelector('.text').title.replace(/\//g, ', '));
		var name = par.querySelector('b').title;
	}
	// 網頁原始碼的空格代碼 &nbsp; ，使用textContent會被轉換成像空格的特殊字元，需重新替換成真正的空格
	// 簡轉繁，不能成為檔名的半形字替換為全行字
	name = toTrad(name.replace(/\u00a0/g, ' ')).replace(/\\|\/|:|\*|\?|"|<|>|\|/g, halfToFull);
	var fileName = artist + ' - ' + name + '.mp3';
	state.style.color = 'blue';
	state.style.fontWeight = 'bold';
	state.innerHTML = '解析中';
	
	var id = DLB.getAttribute('data-res-id');
	var mp3 = {
		'br': 0,
		'mp3Url': ''
	};
	...
	function capture(){
		if(mp3.mp3Url){
			if(mp3.br==320000 || confirm('音質最高只有' + mp3.br/1000 + 'K，是否仍要下載?')){
				GM_xmlhttpRequest({
					'method': 'GET',
					'url': mp3.mp3Url,
					'Referer': 'http://music.163.com/',
					...
					'onprogress': function(res){
						state.innerHTML = Math.floor(res.loaded*100/res.total) + ' %';
					},
					'onload': function(res){
			...
			alert('歌曲可能已經下架');
			state.innerHTML = '無法下載';
		}
	}
	
	detail(id, 'v1', function(song){
		mp3 = song;
		
		if(mp3.br >= 320000){
			capture();
		}else{
			cloud(id, function(song){
				if(song.br > mp3.br){
					mp3 = song;
				}
...
function domLoaded(){
	// 解除鎖區和版權攔截
	// unsafeWindow.NEJ.P('nm.x').oP = function(co){
		// return 0;
	// };
	
	// 解除灰色歌曲下載按鈕顯示
	var dis = document.querySelectorAll('.js-dis');
	for(var i=0; i<dis.length; i++){
		dis[i].querySelector('td:nth-child(2)').classList.add('js-dis');
		dis[i].classList.remove('js-dis')
	}

	// 修改排行榜列表項目的下載按鈕
	var icndl = document.querySelectorAll('.icn-dl');
	for(var i=0; i<icndl.length; i++){
		icndl[i].removeAttribute('data-res-action');
		icndl[i].onclick = download;
	}

	// 修改排行榜列表項目的分享按鈕
	// var icnsh = document.querySelectorAll('.icn-share');
	// for(var i=0; i<icnsh.length; i++){
		// icnsh[i].title = '雅音';
		// icnsh[i].removeAttribute('data-res-action');
		// icnsh[i].onclick = searchFM;
	// }
	
	// 單曲頁面按鈕修改
	if(location.pathname=='/song'){
		var DLB = document.querySelector('.u-btni-dl');
		// var FM = document.querySelector('.u-btni-share');
		
		DLB.removeAttribute('data-res-action');
		DLB.onclick = download;
		
		// FM.firstElementChild.innerHTML = '雅音';
		// FM.removeAttribute('data-res-action');
		// FM.onclick = searchFM;
	}
}

function searchCG(){
	if(location.pathname=='/search/' && location.hash.search(/\+/) > -1){
