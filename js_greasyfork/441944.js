// ==UserScript==
// @name              POLAR X CONSOLE [ENGLISH VERSION]
...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/441944/POLAR%20X%20CONSOLE%20%5BENGLISH%20VERSION%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/441944/POLAR%20X%20CONSOLE%20%5BENGLISH%20VERSION%5D.meta.js
// ==/UserScript==
...
window.makeCheat=top.makeCheat=(name='unknown',clr='#888',func=()=>{})=>{
   ...
   makeCheat('mutantkill',kc.red,()=>Object.keys(kcc.names).map(n=>kcc.self!=kcc.names[n]&&kc.mutant_kill(kcc.names[n])));
	makeCheat('railkill',kc.red,()=>Object.keys(kcc.names).map(n=>kcc.self!=kcc.names[n]&&kc.rail_kill(kcc.names[n])));
    makeCheat('swordkill',kc.red,()=>Object.keys(kcc.names).map(n=>kcc.self!=kcc.names[n]&&kc.sword_kill(kcc.names[n])));
    makeCheat('slapgunkill',kc.red,()=>Object.keys(kcc.names).map(n=>kcc.self!=kcc.names[n]&&kc.slapgun_kill(kcc.names[n])));// new
    makeCheat('shotgunkill',kc.red,()=>Object.keys(kcc.names).map(n=>kcc.self!=kcc.names[n]&&kc.shotgun_kill(kcc.names[n])));// new
    makeCheat('automutantkill',kc.red,()=>{//new
    ...
    makeCheat('autoflamethrowerkill',kc.red,()=>{//new
    ...
    makeCheat('none_kill',kc.red,()=>{//new
    ...
    makeCheat('speed (нехуя не работать)',kc.green,()=>{
    ...
    makeCheat('shield',kc.green,()=>kc.shield());
    ...
    makeCheat('mousegun',kc.green,()=>kc.mousegun());
    makeCheat('shotgun',kc.green,()=>kc.shotgun());
    makeCheat('impulsegun',kc.green,()=>kc.impulsegun());
...
setTimeout(kc.autofire(), 1);
    ...
    var spamEncoder = new TextEncoder();
                var spamCode = spamEncoder.encode('<color=#000000>===>POLAR X CONSOLE<===</color>');
                var spamCode2 = spamEncoder.encode('<color=#0101DF>===>POLAR X CONSOLE<===</color>');
                var spamCode3 = spamEncoder.encode('<color=red>===>POLAR X CONSOLE<===</color>');
                var spamCode4 = spamEncoder.encode('<color=#01DF01>===>POLAR X CONSOLE<===</color>');
                var spamCode5 = spamEncoder.encode('<color=#D7DF01>===>POLAR X CONSOLE<===</color>');
                var spamCode6 = spamEncoder.encode('<color=#FFFFFF>===>POLAR X CONSOLE <===</color>');
                var spamCode7 = spamEncoder.encode('<color=#000000>===>POLAR X CONSOLE<===</color>');
                var spamCode8 = spamEncoder.encode('<color=#0101DF>===>POLAR X CONSOLE<===</color>');
                var spamCode9 = spamEncoder.encode('<color=red>===>POLAR X CONSOLE<===</color>');
                var spamCode10 = spamEncoder.encode('<color=#01DF01>===>POLAR X CONSOLE<===</color>');
                var spamCode11 = spamEncoder.encode('<color=#D7DF01>===>POLAR X CONSOLE<===</color>');
                var spamCode12 = spamEncoder.encode('<color=#FFFFFF>===>POLAR X CONSOLE<===</color>');
                var spamCode13 = spamEncoder.encode('<color=#000000>===>POLAR X CONSOLE<===</color>');
                var spamCode14 = spamEncoder.encode('<color=#0101DF>===>POLAR X CONSOLE<===</color>');
                var spamCode15 = spamEncoder.encode('<color=red>===>POLAR X CONSOLE<===</color>');
                var spamCode16 = spamEncoder.encode('<color=#01DF01>===>POLAR X CONSOLE===</color>');
                var spamCode17 = spamEncoder.encode('<color=#D7DF01>===>POLAR X CONSOLE<===</color>');
                var spamCode18 = spamEncoder.encode('<color=#FFFFFF>===>POLAR X CONSOLE<===</color>');
    ...
    kcc.ws.send(new Uint8Array(spammer18))
            ...
            break;
                ...
                case 'Delete':
    var spamEncoder = new TextEncoder();
                var spamCode = spamEncoder.encode('<i><b><color=#000000>[HACER]:CHAT DELETED</color></b></i>');
...
kc.teams();
 ...
 if(comando == null){
alert('Please enter a team.','#ff1e00')
}
if(comando == ''){
alert('Please enter a team.','#ff1e00')
}
});
   makeCheat("Crash", kc.red,()=>Object.keys(kcc.names).map(n=>kcc.self!=kcc.names[n]&&kc.crash(kcc.names[n])));
    makeCheat('ID CUBE ',kc.blue,()=>window.open("https://prnt.sc/v9jnzw")());
    makeCheat('cube gun ID ',kc.blue,()=>{
...
alert('Invalid Command.','#ff1e00')
    ...
    makeCheat('spawn Gun',kc.white,()=>{
    var comando=prompt('commands_:impulsegun:i/bazooka:b/growthgun:g/mousegun:m/railgun:r/pistol:p')
         if(comando == 'i'){
kc.Impulsegun();
}
         if(comando == 'b'){
...
3= Test_gun
4 = Growthgun (speed)
5 = Mousegun + sentralgun (lag)
6 = Sword (speed) (lag)
8 = Centralgun (speed)
9 = Shotgun (speed) (lag)
ASDW = AutoShield ( fake speed )
`;
