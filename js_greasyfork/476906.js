// ==UserScript==
// @name         Source Code with Syntax Highlighting - uso.kkx.one
// @namespace    https://greasyfork.org/users/124677-pabli
// @version      0.2
// @description  Show Source Code. Additional features: syntax highlighting, theme switch, wrap code, copy all code button
// @author       Pabli
// @license      MIT
// @match        https://uso.kkx.one/*
// @icon         https://icons.duckduckgo.com/ip3/uso.kkx.one.ico
...
// @grant        GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/476906/Source%20Code%20with%20Syntax%20Highlighting%20-%20usokkxone.user.js
// @updateURL https://update.greasyfork.org/scripts/476906/Source%20Code%20with%20Syntax%20Highlighting%20-%20usokkxone.meta.js
// ==/UserScript==
...
<div style="order: 99">
  <span style="display: flex; justify-content: space-between; align-items: center">
    <h2>Source code</h2>
	<span>
	  <input type="checkbox" id="code-wrap"><label for="code-wrap" style="margin-left: 0.25rem">code wrap</label>
	  ...
	  <button id="code-copy" title="Copy code" style="width: 24px; height: 24px; background: none; padding: 0; margin: 0; border: 0">
	    <svg style="vertical-align: top" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
  ...
  <div id="code-source">
    <pre class="border rounded hljs" style="min-height: 20rem; height: 20rem; resize: vertical"><code class="language-css"></code></pre>
  </div>
</div>
`;

const observer = new MutationObserver(mutations => {
	if (document.querySelector(".container > div.mt-4.row")) {
		if (!document.querySelector("#code-source")) {
			document.querySelector(".container > div.mt-4.row").innerHTML += div;
			getCss();
			codeWrap();
			codeTheme();
			codeCopy();
...
async function getCss() {
	const installBtn = document.querySelector("a.btn[href$='.user.css']");
	const code = document.querySelector("#code-source code");
	try {
...
function codeWrap() {
	const checkbox = document.querySelector("#code-wrap");
	const pre = document.querySelector("#code-source pre");
	...
	checkbox.addEventListener("click", () => {
		...
		localStorage.setItem("codeWrap", checkbox.checked);
	});
}

function codeTheme() {
	const themeSelect = document.querySelector("#theme-select");
	const codeDiv = document.querySelector("#code-source");

	const themeLs = localStorage.getItem("codeTheme");
	if (themeLs) {
		themeSelect.value = themeLs;
		codeDiv.className = "theme-" + themeLs;
	} else {
		codeDiv.className = "theme-a11y-dark";
	}

	themeSelect.addEventListener("change", () => {
		const selectedTheme = themeSelect.value;
		codeDiv.className = "theme-" + selectedTheme;
...
function codeCopy() {
	...
	copy.addEventListener("click", () => {
		navigator.clipboard.writeText(document.querySelector("#code-source code").textContent);
	});
}

GM_addStyle(`
html.dark {color-scheme: dark}
#code-copy {fill: var(--bs-body-color, #212529)}
html.dark #code-copy {fill: var(--bs-body-color-alt, #e1e1e1)}
