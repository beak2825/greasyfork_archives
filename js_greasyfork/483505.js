// ==UserScript==
// @name         Moomoo.io Nighttime Chillin Script
// @namespace    http://tampermonkey.net/
// @version      1.1
// @description  Look below for function.
// @author       ItzBetalol
// @match        *://moomoo.io/*
// @match        *://*.moomoo.io/*
// @icon         https://pic.onlinewebfonts.com/thumbnails/icons_443460.svg
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/483505/Moomooio%20Nighttime%20Chillin%20Script.user.js
// @updateURL https://update.greasyfork.org/scripts/483505/Moomooio%20Nighttime%20Chillin%20Script.meta.js
// ==/UserScript==

(function() {
    'use strict';

    var link = document.querySelector("link[rel~='icon']");
if (!link) {
    link = document.createElement('link');
    link.rel = 'icon';
    document.head.appendChild(link);
}
link.href = 'https://pic.onlinewebfonts.com/thumbnails/icons_573711.svg';
alert("The menu being centered and the game being in darkmode is thanks to NotYou. His script is at the bottom-right.");
    // Changing names and colors
    document.getElementById('gameName').innerHTML = 'MOOMOO.IO';
    document.getElementById('gameName').style.color = "black";
    document.getElementById('gameName').style.textShadow = 'none';
    document.getElementById('gameName').style.fontSize = "80px";

    document.getElementById('linksContainer2').style.background = "black";

    document.getElementById('loadingText').innerHTML = 'Starting...';
    document.getElementById('loadingText').style.color = "black";

    document.getElementById('mapDisplay').height = '450';
    document.getElementById('mapDisplay').width = '450';

    document.title = 'Beta`s Mod';

    document.getElementById('allianceButton').style.color = "cyan";
    document.getElementById('chatButton').style.color = "lime";
    document.getElementById('storeButton').style.color = "gold";

    document.getElementById('enterGame').innerHTML = 'Begin';
    document.getElementById('enterGame').style.background = "grey";
    document.getElementById('enterGame').style.color = "black";

    document.getElementById('nameInput').placeholder = '15 Character Limit';

    document.getElementById('setupCard').style.background = "black";
    document.getElementById('guideCard').style.background = "black";

    document.getElementById('diedText').innerHTML = 'Want a break from the ads?';
    document.getElementById('diedText').style.color = "#219B00";

    // Removing ads
    $("#wideAdCard").remove();
    $("#adCard").remove();
    $("#promoImgHolder").remove();
   let style = document.createElement('style')
   style.appendChild(document.createTextNode(`
:root {
  --w: rgb(255, 255, 255);
  --b: rgb(19, 19, 19);
  --f: rgb(9, 9, 9);
  --b-t: rgba(0, 0, 0, 0.6);
  --f-t: rgba(0, 0, 0, 0.8);

  #mapDisplay {
  background-image: url(data:image/png;base64,) !important;
  background-size: contain !important;
}
#menuCardHolder[style*="block"] {
  display: grid !important;
}
.menuCard {
  box-shadow: var(--f) 0 7px;
}
#ageBody, #actionBar *, #resDisplay *, #upgradeHolder *, #mapDisplay, #topInfoHolder, #killCounter, .uiElement, #ageBar, #storeHolder, .storeTab, #allianceHolder, #allianceManager *, #chatBox, .notifButton {
  background-color: var(--f-t) !important;
}
#touch-controls-fullscreen {
background-color: rgba(0, 0, 0, 0.5);
}
`))
    document.head.appendChild(style)
    let discord = document.querySelector('#linksContainer2 [href*="discord"]')
    discord.href = '//greasyfork.org/scripts/449133'
    discord.innerHTML = 'NotYou'

})();