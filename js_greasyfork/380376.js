...
// @run-at document-end
// @downloadURL https://update.greasyfork.org/scripts/380376/WaniKani%20hide%20mnemonics.user.js
// @updateURL https://update.greasyfork.org/scripts/380376/WaniKani%20hide%20mnemonics.meta.js
// ==/UserScript==
 ...
 function setCorrectStorage(action, which)
{
    ...
    else if (action == "More" && note)
    {
        // Force section to be visible
        setStorage(which, 1);
    }
    else if (action == "Less" && !note)
    {
 ...
 * @param action Specifies what happens when the header is pressed, either "More" or "Less".
 ...
 * @param action Specifies if the link is "Less" or "More".
 */
...
function setCorrectTextFor(which)
{
    if (isHidden(which))
    {
        // Display the "More" link in the note.
        textForHeader(which, "More", getNoteHeader(which));
    }
    else
    {
        // Display the "Less" link in the header.
        textForHeader(which, "Less", getMnemonicHeader(which));
