...
// @version     4.0.0703
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/26042/HIT%20Scraper%20WITH%20EXPORT%20-%20With%20next%20accept.user.js
// @updateURL https://update.greasyfork.org/scripts/26042/HIT%20Scraper%20WITH%20EXPORT%20-%20With%20next%20accept.meta.js
// ==/UserScript==
                ...
                for (var r of (Settings.user.bubbleNew ? bubbleNewHits(results) : results)) { 
                    ...
                    if (Settings.user.acceptNext) {
                        if(r.isNew && !shouldHide){
                            ...
                            if(aaTing.showing) aaTing.box.prepend(`<tr><td class="aaHit">${getTime()}</td><td class="aaHit">${r.pay}</td><td class="aaHit">${r.requester.name}</td><td class="aaHit"><a href="${r.hit.panda}">${r.title.replace(/"/g,'&quot;')}</a></td></tr>`);
