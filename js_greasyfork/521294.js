...
// @timestamp   2024-12-20T12:16:53.270Z
// @require     https://cdn.jsdelivr.net/npm/@violentmonkey/dom@2
// @require     https://cdn.jsdelivr.net/npm/@violentmonkey/ui@0.7
// @require     https://cdn.jsdelivr.net/npm/@violentmonkey/dom@2/dist/solid.min.js
// @grant       GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/521294/Spotimizer.user.js
// @updateURL https://update.greasyfork.org/scripts/521294/Spotimizer.meta.js
// ==/UserScript==
...
var _tmpl$2$1 = /*#__PURE__*/web.template(`<div class=snFK6_ei0caqvFI6As9Q><div data-testid=alternative-playing-widget class=deomraqfhIAoSB3SgXpu role=contentinfo style=flex-direction:row-reverse;><div data-testid=CoverSlotCollapsed__container class="GQ5_gIWzIqAfBdmQm8yJ IcyWfMS5VkeOhaI7OWIx"><div class="GlueDropTarget GlueDropTarget--albums GlueDropTarget--tracks GlueDropTarget--episodes GlueDropTarget--local-tracks"data-testid=cover-drop-target><div data-testid=cover-art-button class=HD9s7U5E1RLSWKpXmrqx><div class=BFR9Zt3zpL8BATBMiwQB><div class=H0HbpIM3UrcupWIAjLWu aria-hidden=true style=width:56px;height:56px;><div class=zmOtW0vqqn1qpZrtQ_w9><svg data-encore-id=icon role=img aria-hidden=true data-testid=track viewBox="0 0 24 24"class="Svg-sc-ytk21e-0 bneLcE"><path d="M6 3h15v15.167a3.5 3.5 0 1 1-3.5-3.5H19V5H8v13.167a3.5 3.5 0 1 1-3.5-3.5H6V3zm0 13.667H4.5a1.5 1.5 0 1 0 1.5 1.5v-1.5zm13 0h-1.5a1.5 1.5 0 1 0 1.5 1.5v-1.5z"></path></svg></div><img aria-hidden=false loading=eager data-testid=cover-art-image alt=""class="mMx2LUixlnN_Fu45JpFB FqmFsMhuF4D0s35Z62Js"></div></div></div></div></div><div class="j96cpCtZAIdqxcDrYHPI iZrIHsls0lCEhoMDA9kc"><div class=PcH6VnzkkDqD36P93i9Q><div class=eSMjmiD29Ox35O95waw6><div class=W5cB_o0XkkU7Q8tlTGxq><div class=PGSe59fD1Hwc9yUM2d3U style=--trans-x:0px;justify-self:flex-end;><div class="encore-text encore-text-body-small K9Nj3oI7bTNFh5AGp5GA"data-encore-id=text data-testid=context-item-info-title dir=auto><span draggable=true><a draggable=false data-testid=context-item-link href=/album/0PjSvHusBgifniYJcyBsBp></a></span></div></div></div></div></div><div class="p2ya1fQ3o9pY4alcW0o4 MrkH0O1OzmNv_oCQdvI8"></div><div class="w_TTPh4y9H1YD6UrTMHa r6Psl2_K_0vpdX6vFLQd"><div class="eSMjmiD29Ox35O95waw6 EOLjmX4qyDsRvnxBK0JO"><div class=W5cB_o0XkkU7Q8tlTGxq><div class=PGSe59fD1Hwc9yUM2d3U style=--trans-x:0px;><div class="encore-text encore-text-marginal encore-internal-color-text-subdued w_TTPh4y9H1YD6UrTMHa"data-encore-id=text data-testid=context-item-info-subtitles>`),
  _tmpl$3 = /*#__PURE__*/web.template(`<span><a draggable=true data-testid=context-item-info-artist dir=auto>`);
function CustomBar(props) {
  ...
  const gettrackPositionInPLaylistIndex = track => {
    const items = JSON.parse(localStorage.getItem(`playlist_${getCurrentPlaylistId()}_items`));
    return items.findIndex(item => item.track.id === track.id);
  };
  ...
  async function playItem(track) {
    var _await$api$player$get;
    //console.log('playItem', track);
    const api = getSpotifyApi();
    const currentlyPlaying = await api.player.getCurrentlyPlayingTrack();
    if ((currentlyPlaying == null ? void 0 : currentlyPlaying.item.id) === track.id) {
      return;
    }
    const availableDevices = (_await$api$player$get = await api.player.getAvailableDevices()) == null ? void 0 : _await$api$player$get.devices;
    const device = (currentlyPlaying == null ? void 0 : currentlyPlaying.device) || availableDevices.reverse().findLast(device => device.is_active) || availableDevices[0];
    const currenttrackPositionInPLaylistIndex = gettrackPositionInPLaylistIndex(getCurrentlyPlayingItem());
    const trackPositionInPLaylistIndex = gettrackPositionInPLaylistIndex(track);
    const playlistId = getCurrentPlaylistId();
    if (currenttrackPositionInPLaylistIndex < trackPositionInPLaylistIndex) {
      //console.log('move current track', currenttrackPositionInPLaylistIndex, currenttrackPositionInPLaylistIndex + 1);
      api.playlists.movePlaylistItems(playlistId, currenttrackPositionInPLaylistIndex, 1, currenttrackPositionInPLaylistIndex + 2);
      if (trackPositionInPLaylistIndex !== 0) {
        //console.log('move track', trackPositionInPLaylistIndex, trackPositionInPLaylistIndex - 1);
        api.playlists.movePlaylistItems(playlistId, trackPositionInPLaylistIndex, 1, trackPositionInPLaylistIndex - 1);
      }
    }
    cachePlaylist(playlistId);

    // TODO: keep current playlist, loosing context when not playing music before switching to alternative song.
    api.player.startResumePlayback(device.id, currentlyPlaying == null ? void 0 : currentlyPlaying.context.uri, currentlyPlaying != null && currentlyPlaying.context.uri ? undefined : [track.uri]);
  }
  return [web.memo(() => web.memo(() => !!props.isVisible)() && alternativeWidgetContainer()), web.memo(() => false)];
  function alternativeWidgetContainer() {
    // <div data-testid="now-playing-widget" class="deomraqfhIAoSB3SgXpu" role="contentinfo" aria-label="Now playing: Cyanide by What So Not, Tommy Trash, Lucille Croft, Lord Gasp"><div data-testid="CoverSlotCollapsed__container" class="GQ5_gIWzIqAfBdmQm8yJ IcyWfMS5VkeOhaI7OWIx"><div draggable="true"><div class="GlueDropTarget GlueDropTarget--albums GlueDropTarget--tracks GlueDropTarget--episodes GlueDropTarget--local-tracks" data-testid="cover-drop-target"><button type="button" data-testid="cover-art-button" class="HD9s7U5E1RLSWKpXmrqx" aria-label="Now playing view"><div class="BFR9Zt3zpL8BATBMiwQB"><div class="H0HbpIM3UrcupWIAjLWu" aria-hidden="true" style="width: 56px; height: 56px;"><div class="zmOtW0vqqn1qpZrtQ_w9"><svg data-encore-id="icon" role="img" aria-hidden="true" data-testid="track" viewBox="0 0 24 24" class="Svg-sc-ytk21e-0 bneLcE"><path d="M6 3h15v15.167a3.5 3.5 0 1 1-3.5-3.5H19V5H8v13.167a3.5 3.5 0 1 1-3.5-3.5H6V3zm0 13.667H4.5a1.5 1.5 0 1 0 1.5 1.5v-1.5zm13 0h-1.5a1.5 1.5 0 1 0 1.5 1.5v-1.5z"></path></svg></div><img aria-hidden="false" draggable="false" loading="eager" src="https://i.scdn.co/image/ab67616d000048519167e697d66d2cc678927826" data-testid="cover-art-image" alt="" class="mMx2LUixlnN_Fu45JpFB FqmFsMhuF4D0s35Z62Js"></div></div></button></div></div><div id="VideoPlayerNpb_ReactPortal"></div><button class="_9sCL61nGvQFXv2u02jXw" aria-label="Now playing view" aria-pressed="true"><svg data-encore-id="icon" role="img" aria-hidden="true" viewBox="0 0 16 16" class="Svg-sc-ytk21e-0 dYnaPI"><path d="M.47 4.97a.75.75 0 0 1 1.06 0L8 11.44l6.47-6.47a.75.75 0 1 1 1.06 1.06L8 13.56.47 6.03a.75.75 0 0 1 0-1.06z"></path></svg></button></div><div class="j96cpCtZAIdqxcDrYHPI iZrIHsls0lCEhoMDA9kc"><div class="PcH6VnzkkDqD36P93i9Q"><div class="eSMjmiD29Ox35O95waw6"><div class="W5cB_o0XkkU7Q8tlTGxq"><div class="PGSe59fD1Hwc9yUM2d3U" style="--trans-x: 0px;"><div class="encore-text encore-text-body-small K9Nj3oI7bTNFh5AGp5GA" data-encore-id="text" data-testid="context-item-info-title" dir="auto"><span draggable="true"><a draggable="false" data-testid="context-item-link" href="/album/0PjSvHusBgifniYJcyBsBp">Cyanide</a></span></div></div></div></div></div><div class="p2ya1fQ3o9pY4alcW0o4 MrkH0O1OzmNv_oCQdvI8"></div><div class="w_TTPh4y9H1YD6UrTMHa r6Psl2_K_0vpdX6vFLQd"><div class="eSMjmiD29Ox35O95waw6 EOLjmX4qyDsRvnxBK0JO"><div class="W5cB_o0XkkU7Q8tlTGxq"><div class="PGSe59fD1Hwc9yUM2d3U" style="--trans-x: 0px;"><div class="encore-text encore-text-marginal encore-internal-color-text-subdued w_TTPh4y9H1YD6UrTMHa" data-encore-id="text" data-testid="context-item-info-subtitles"><span><a draggable="true" data-testid="context-item-info-artist" dir="auto" href="/artist/4AA8eXtzqh5ykxtafLaPOi">What So Not</a></span><span>, <a draggable="true" data-testid="context-item-info-artist" dir="auto" href="/artist/1tBU8jUEdVR3mqSsAqEGfD">Tommy Trash</a></span><span>, <a draggable="true" data-testid="context-item-info-artist" dir="auto" href="/artist/270fGluVXrZVyTNECXbrsy">Lucille Croft</a></span><span>, <a draggable="true" data-testid="context-item-info-artist" dir="auto" href="/artist/26fmgj9vUob3WnTDojZxBw">Lord Gasp</a></span></div></div></div></div></div></div><button class="Button-sc-1dqy6lx-0 dmdXQN" aria-checked="false" aria-label="Add to Liked Songs" data-encore-id="buttonTertiary"><span aria-hidden="true" class="IconWrapper__Wrapper-sc-16usrgb-0 hYdsxw"><svg data-encore-id="icon" role="img" aria-hidden="true" viewBox="0 0 16 16" class="Svg-sc-ytk21e-0 dYnaPI"><path d="M8 1.5a6.5 6.5 0 1 0 0 13 6.5 6.5 0 0 0 0-13zM0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8z"></path><path d="M11.75 8a.75.75 0 0 1-.75.75H8.75V11a.75.75 0 0 1-1.5 0V8.75H5a.75.75 0 0 1 0-1.5h2.25V5a.75.75 0 0 1 1.5 0v2.25H11a.75.75 0 0 1 .75.75z"></path></svg></span></button></div>
    return (() => {
      ...
      _el$20.$$click = () => playItem(getAlternativeItem());
      web.insert(_el$28, () => {
        var _getAlternativeItem2;
        return (_getAlternativeItem2 = getAlternativeItem()) == null ? void 0 : _getAlternativeItem2.name;
      });
      web.insert(_el$34, () => {
        var _getAlternativeItem3;
        return (_getAlternativeItem3 = getAlternativeItem()) == null ? void 0 : _getAlternativeItem3.artists.map((artist, index) => (() => {
          var _el$35 = _tmpl$3(),
            _el$36 = _el$35.firstChild;
          web.insert(_el$35, index > 0 && ', ', _el$36);
          web.insert(_el$36, () => artist.name);
          web.effect(() => web.setAttribute(_el$36, "href", `/artist/${artist.id}`));
          return _el$35;
        })());
      });
      web.effect(() => {
        var _getAlternativeItem4;
        return web.setAttribute(_el$20, "src", (_getAlternativeItem4 = getAlternativeItem()) == null ? void 0 : _getAlternativeItem4.album.images.find(image => image.height == image.width && image.height == 64).url);
      });
      return _el$12;
    })();
  }
...
function ExpandCollapseButton(props) {
  const [getIsExpanded, setIsExpanded] = solidJs.createSignal(true);
  solidJs.createEffect(() => {
...
function SpotifyPageInjector() {
  ...
  const [getIsCustomBarOpened, setIsCustomBarOpened] = solidJs.createSignal(true);
