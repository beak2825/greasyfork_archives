...
// @name        FFixerUpdated
// @namespace   
// @description FFixerUpdated版 對Facebook更大的個人資料圖片和照片之增強，便於觀看的相冊.鏈接.下載視頻.書籤，鍵盤快捷鍵及更多完全可訂製！。
...
// @exclude     https://fastzone.org
// @exclude     *://fastzone.org/forum.php?*
// @author      Vaughan Chandler
// @timestamp   1359222040
// @version     2.3.3
// @downloadURL https://update.greasyfork.org/scripts/3760/FFixerUpdated.user.js
// @updateURL https://update.greasyfork.org/scripts/3760/FFixerUpdated.meta.js
// ==/UserScript==

// Edited 2013-06-17 by Andy Dremeaux
// Edited 2013-01-01 by morph4u
// Edited 2013-01-26 2.3.1.12 by toyotabedzrock Fix update detection
// Edited 2013-01-16 2.3.1.11 by toyotabedzrock Reintegrate translations
// Edited 2012-11-01 by Andy Dremeaux
// See http://userscripts.org/topics/121200 for changes
...
var version = '2.3.1.12';
var version_timestamp = 1359222040; // javascript:window.alert(new Date().getTime());
var release_date = 20130126;
...
var lang = {
	...
	fr : {
		...
		'ConfFBFTimestamps' : 'Ajout de la datation FFixer après la datation Facebook (ex. "11:45").',
		...
		'ConfUpdates' : 'Vérifie quotidiennement si une mise à jour de FFixer est disponible sur Userscripts.org. Ou <a href="#" id="fbfUpdateLink" onclick="return false;">vérifier maintenant</a>.',
		...
		'FacebookFixerConflict' : 'FFixer est maintenant devenu FFixer. A cause du changement de nom, vous aurez besoin de désinstaller manuellement FFixer de votre explorateur, ou les deux scripts rentreront en conflit. Si vous n\'êtes pas sûr de comment faire pour désinstaller un userscript, <a %s>cliquez ici pour la marche à suivre</a>.',
		...
		'UpdateAvailable1' : 'Une mise à jour de FFixer est disponible',
	...
	el : {
		...
		'Close' : 'Κλείσιμο',
		'ConfigureFacebookFixer' : 'Ρυθμίσεις FFixer',
		...
		'ConfFacebookFixerLanguage' : 'Επιλογή γλώσσας για το FFixer',
	...
	zh_tw : {
		...
		'ConfDisableTheater' : '相片禁用幻燈功能改為舊式.',
...
function parseMonth(str) {
	...
	var dict = {
		...
		'el'		: Array('iανουάριος', 'φεβρουάριος', 'μάρτιος', 'απρίλιος', 'μάιος', 'iούνιος', 'iούλιος', 'αύγουστος', 'σεπτέμβριος', 'oκτώβριος', 'νοέμβριος', 'δεκέμβριος'),
		...
		'mk'		: Array('јануари', 'февруари', 'март', 'април', 'мај', 'јуни', 'јули', 'август', 'септември', 'октомври', 'ноември', 'декември'),
		...
		'sr'		: Array('јануар', 'фебруар', 'март', 'април', 'мај', 'јуни', 'јули', 'август', 'септембар', 'октобар', 'новембар', 'децембар'),
...
if (menu = $('//div[@id="userNavigation"]/div',null,true)) {
...
function FBFUpdateCheck(forced) {
	if((forced)||(parseInt(getValue("LastUpdate", "0")) + 86400000 <= (new Date().getTime()))) {
		updateForced = forced;
		// new: http://userscripts.org/scripts/source/156745.meta.js old: http://userscripts.org/scripts/review/8861
		try { xmlhttpRequest({method: "GET",url: "http://userscripts.org/scripts/source/156745.meta.js?" + new Date().getTime(),headers: {'Cache-Control': 'no-cache'}}, handleUpdateResponse); }
...
function showUpdatePopup() {
	...
	onClick('fbfUpdateInstall', function() { openInTab('http://userscripts.org/scripts/source/156745.user.js'); hidePopup(); });
	onClick('fbfUpdateHomepage', function() { window.open('http://userscripts.org/scripts/show/156745'); hidePopup(); });
...
style = style +
	'.fbfPopup { padding:10px; background:#f6f6f6; border:3px double #666666; border-radius:5px; }'+
	'.fbfPopupContainer { display:none; top:0; right:0; bottom:0; left:0; }'+
	'#ff-popup-pic-div { display:none; background:white; border:1px solid #333; position:fixed !important; top:3px !important; padding:4px; min-width:130px; z-index:99999 !important; border-radius:3px; }'+
	'.ff-popup-pic-div-left { left:3px !important; right:auto !important; box-shadow:5px 5px 5px rgba(0,0,0,0.6); }'+
	'.ff-popup-pic-div-right { right:3px !important; left:auto !important; box-shadow:-5px 5px 5px rgba(0,0,0,0.6); }'+
...
if (prefs['HomeLeftColumnFixed'])		{ style = style + ' .home #leftCol { position:fixed !important; }'; }
...
if (!prefs['FindFriendsLink'])			{ style = style + ' #pageNav > li:nth-child(4) { display:none; }'; }
...
if (!prefs['HomeRecommendations'])		{ style = style + ' #pagelet_netego, #pagelet_netego_lower, #pagelet_ego_pane_w { display:none; }'; }
...
if (prefs['Bookmarks'] && !$('#fbf-bookmarks')) {
	try {
		...
		if (pageNav) {
		  ...
		  bookmarks.innerHTML = '<a href="#" onclick="return false;" id="fbf-bookmark-link" role="button" rel="toggle" class="navLink">' + $l('Bookmarks') + '</a>'+'<ul id="fbf-bookmark-list"></ul>';
...
if (prefs['LogoutLink'] && !$('#ff-logout')) {
	try {
		...
		newLogout.className = 'navItem middleItem';
			newLogout.innerHTML = '<a class="navLink">' + $('//form[@id="logout_form"]//input[@type="submit"]', null, true).value + '</a>';
...
if (prefs['ProfilePicPopup'] || prefs['PhotoPopup']) {
	...
	function showPopupPic(e) {
		try {
			...
			//new check, new style images
			var newestSrc;
			var divRegex = /(https?):\/\/([^\/]+fbcdn\.net|fbcdn[^\/]+.akamaihd\.net)\/(.*?)\/(.*?)[0-9]*x[0-9]*\/(.*?)\.jpg/;
		
			if (t.tagName == 'IMG' || t.tagName == 'I' || t.tagName == 'img') {
				if (divRegex.exec(t.src)) {
					newestSrc = t.src;
				}
			} else if (t.tagName == 'DIV' && t.firstChild && t.firstChild.tagName == 'IMG') {
				if (divRegex.exec(t.firstChild.src)) {
					newestSrc = t.firstChild.src;
				}
			} else if (t.tagName == 'A' && t.firstChild && t.firstChild.tagName == 'DIV' &&
							t.firstChild.firstChild && t.firstChild.firstChild.tagName == 'IMG') {
				if (divRegex.exec(t.firstChild.firstChild.src)) {
					newestSrc = t.firstChild.firstChild.src;
				}
			}
			//end new check
			...
			if (!newestSrc) {
				...
				if ((oldSrc && picRegex_n.exec(oldSrc))&&(!newStylePic)) {
					...
					} else if (t.className == "uiMediaThumbImg") {
						// new photo view
			...
			if (oldSrc || newSrc || newestSrc) {
				
				if (newestSrc) {
					m = newestSrc.match(divRegex);
					
					newSrc = m[1] + "://" + m[2] + "/" + m[3] + "/" + m[4] + bigPicSize + "x" + bigPicSize + "/" + m[5] + ".jpg";
					console.log(newSrc);
				}
				else {
					if (!newSrc) {
					...
					newSrc = newSrc.replace("/p206x206", "");
				}	
...
function processPage() {
	...
	var sponsoredLinks = $('//a[text()="Sponsored"]');
	for (var i = 0; i < sponsoredLinks.snapshotLength; i++) {
		var elem = sponsoredLinks.snapshotItem(i).parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode;
		var parent = elem.parentNode;
		parent.removeChild(elem);
		parent.removeChild($('//div[contains(@class,"uiStreamEdgeStoryLineWithLabel")]', parent, true)); 
	}
	...
	if (prefs['FBFTimestamps']) {
		try {
			...
			var timestamps = $('//abbr[@data-utime][not(contains(@class,"ffixer-handled"))][not(contains(@class,"_35"))]');
