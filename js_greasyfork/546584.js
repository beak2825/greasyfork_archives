// ==UserScript==
// @name         Letterboxd → FLAM (Fenlight) [MOBILE • Optimized]
// @namespace    http://tampermonkey.net/
// @version      3.6.0-mobile-opt
// @description  Mobile-optimized & cache-fast: sends Letterboxd lists/nanogenres to Kodi Fenlight via TMDB. Touch-first UI + zero-network FLAM when cached. No page refetch on single-page lists. IndexedDB writes only when dirty. Background base-cache revalidate at most once/day.
...
// @downloadURL https://update.greasyfork.org/scripts/546584/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29%20%5BMOBILE%20%E2%80%A2%20Optimized%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/546584/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29%20%5BMOBILE%20%E2%80%A2%20Optimized%5D.meta.js
// ==/UserScript==
...
(async function () {
  ...
  const BASE_REVAL_INTERVAL_MS = 24 * 60 * 60 * 1000; // once/day
  ...
  async function loadBaseBinOnce({ allowRevalidate = false } = {}) {
    ...
    // Non-blocking: we won't await callers on this path, they should pass allowRevalidate:false
    GM_xmlhttpRequest({
      method: 'GET',
      url: baseBinUrl(),
      ...
      onload: async (res) => {
        if (res.status === 304 && BASE_PAIRS_U32) return;
          ...
          try {
            if (res.status >= 200 && res.status < 300 && res.response) {
              ...
              if (u32.length % 2 === 0) {
                ...
                const newEtag = m ? m[1].trim() : '';
                try { await putBaseToIDB(newBuf, newEtag); } catch {}
                GM_setValue('baseLastReval', Date.now());
  ...
  // Background revalidate at most once/day (non-blocking)
  async function scheduleBaseRevalidateIfStale() {
    const last = GM_getValue('baseLastReval', 0);
    if (Date.now() - last < BASE_REVAL_INTERVAL_MS) return;
  ...
  let LOCAL_OVERRIDES = null; // { filmId: packed }
  ...
  function overrideSetPacked(filmIdStr, tmdbId, isTv) {
    ...
    const newVal = (tmdbId << 1) | (isTv ? 1 : 0);
    const prev = LOCAL_OVERRIDES[filmIdStr];
    if (prev !== newVal) { LOCAL_OVERRIDES[filmIdStr] = newVal; return true; }
    return false;
  }
  ...
  // ─── Mobile UI helpers ──────────────────────────────────────────────────
  function mkEl(tag, props = {}, styles = {}) { const el = document.createElement(tag); Object.assign(el, props); Object.assign(el.style, styles); return el; }
  ...
  function overlayPanel({ title = '', content, footerButtons = [] }) {
    ...
    const h = mkEl('h2', { className: 'kb-panel__title' }); h.textContent = title;
    const body = mkEl('div', { className: 'kb-panel__body' });
    const footer = mkEl('div', { className: 'kb-sticky-bottom' });
    header.append(h); if (content) body.append(content);
  ...
  function editDescriptionOverlay(defaultDesc) {
    return new Promise(resolve => {
      ...
      const { overlay } = overlayPanel({ title: 'Edit Description', content: ta, footerButtons: [saveBtn, cancelBtn] });
  ...
  function openSmartLogOverlay() {
    ...
    const lines = [header];
    SMART_LOG.slice().sort((a,b)=>(a.listIndex||999999)-(b.listIndex||999999)).forEach(e=>{
  ...
  async function scrapeAllItems_ListPages() {
    // Determine page count using the DOM. If only one page, do NOT refetch current page (mobile speed-up)
    ...
    // Multi-page: fetch additional pages beyond the current one.
    const urls = [];
  ...
  async function processItems(items, providedOverrides = null) {
    // Caller ensures base cache and overrides are loaded. We keep this hot path zero-network.
    if (providedOverrides) LOCAL_OVERRIDES = providedOverrides;

    // QUICK EXIT: if everything is already in cache/base, skip work entirely.
    const toFetch = [];
    ...
    if (toFetch.length === 0) {
      // Nothing to look up and no mutations → zero disk writes.
      return LOCAL_OVERRIDES || {};
    }

    // We'll mark if we changed anything; only then persist + create backup snapshot.
    let dirty = false;
    const preSnapshot = JSON.parse(JSON.stringify(LOCAL_OVERRIDES || {}));
    ...
    for (let i = 0; i < toFetch.length; i += batchSize) {
      ...
      batches.push((async () => {
        ...
        await Promise.all(batch.map(async it => {
          ...
          if (it.tmdbId) {
            it.isTv = false;
            if (overrideSetPacked(it.filmId, it.tmdbId, it.isTv)) dirty = true;
          }
        }));
      })());
    ...
    // Final retry (only if we still have unresolved & list meets threshold)
    ...
    // Persist ONLY if dirty; also store the pre-change snapshot as backup for the editor.
    if (dirty) {
      try { await setCache(preSnapshot, CACHE_BACKUP_KEY); } catch {}
      try { await setCache(LOCAL_OVERRIDES, CACHE_KEY); } catch {}
    }
    return LOCAL_OVERRIDES || {};
  }
  ...
  // ─── Show URL (overlay; zero-network on cached) ─────────────────────────
  async function handleShowUrl(btnEl) {
    if (btnEl) btnEl.disabled = true;

    // Hot path: load base from IDB only (no network), and schedule background revalidate if stale.
    await loadBaseBinOnce({ allowRevalidate: false });
    await loadLocalOverridesOnce();
    scheduleBaseRevalidateIfStale();
    ...
    const ta = mkEl('textarea', { className:'kb-textarea', spellcheck:false }); ta.value = url;
    const copyBtn = mkEl('button', { className:'kb-btn', textContent:'Copy URL' });
    const closeBtn = mkEl('button', { className:'kb-btn kb-btn--ghost', textContent:'Close' });
    const info = mkEl('div', {}, { color:'#bbb', fontSize:'12px', marginTop:'8px' }); info.textContent = `URL size: ${urlBytes} / ${limitBytes} bytes (${pct}%)`;
  ...
  async function openReviewPanelPromise() {
    ...
    return new Promise(async (resolve) => {
      ...
      const listWrap = mkEl('div'); wrap.append(listWrap);
      ...
      const { overlay } = overlayPanel({ title: 'Review Changes', content: wrap, footerButtons: [saveSend, saveClose, saveOnly, close] });
      ...
      } catch (err) { summary.textContent = `Error loading candidates: ${String(err)}`; }

      function posterUrl(p) { return p ? `${TMDB_IMG_BASE}${TMDB_POSTER_SIZE}${p}` : ''; }

      changed.slice().sort((a,b)=>(a.listIndex||999999)-(b.listIndex||999999)).forEach(e=>{
        ...
        titleRow.append(mkEl('span', {}, { fontSize:'18px', color:'#ff3b3b', fontWeight:'800' , textContent:`#${e.listIndex||'?'}` }), mkEl('span', { textContent:` ${e.lbTitle} (${e.lbYear})` }));
        const meta1 = mkEl('div', {}, { color:'#bbb', fontSize:'12px' , textContent:`film:${e.filmId}`});
        const meta2 = mkEl('div', {}, { color:'#bbb', fontSize:'12px' , textContent:`Directors: ${(e.lbDirectors||[]).join(', ')||'—'}`});
        const meta3 = mkEl('div', {}, { color:'#bbb', fontSize:'12px' , textContent:`TMDb top: ${e.top ? `${e.top.title} (${e.top.release_date||'n/a'}) [${e.top.id}]` : 'n/a'}`});
        const meta4 = mkEl('div', {}, { color:'#bbb', fontSize:'12px' , textContent:`Our choice: ${e.chosen ? `${e.chosen.title} (${e.chosen.release_date||'n/a'}) [${e.chosen.id}]` : 'n/a'}`});
        const reason = mkEl('div', {}, { fontSize:'12px', marginTop:'6px' , textContent: reasonsToText(e) });
        ...
        if (candidates.length) {
          candidates.forEach(c => {
            ...
            if (c.id === e.top?.id) { const b = mkEl('div', { className:'kb-badge kb-badge--alt' }, { position:'absolute', top:'6px', left:'6px' }); b.textContent='TMDb top'; card.append(b); }
            if (c.id === currentChosenId) { const b = mkEl('div', { className:'kb-badge' }, { position:'absolute', top:'6px', right:'6px' }); b.textContent='Chosen'; card.append(b); }
            card.append(img, cap); grid.append(card);
          });
        } else {
      ...
      async function applySavesToCache(doc) {
        ...
        for (const e of changed) {
          ...
          if (beforeId !== selectedId) {
            if (overrideSetPacked(e.filmId, selectedId, /*isTv=*/false)) changesApplied++;
            const top = e.top ? { ...e.top } : null;
            const newChosen = { id: selectedId, title: '(manual)', release_date: '' };
            SMART_LOG.push({
              filmId: e.filmId, listIndex: e.listIndex || 0, lbTitle: e.lbTitle, lbYear: e.lbYear, lbDirectors: e.lbDirectors, lbRuntime: e.lbRuntime,
              top, chosen: newChosen, reasons: ['manual_override'], candidates: null, matchedDirectors: [], runtimeUsed: null, timestamp: Date.now()
            });
          }
        }
        if (changesApplied > 0) {
      ...
      const finish = (payload) => { overlay.remove(); safeResolve(payload); };
      close.onclick = () => finish({ action: 'close' });
      saveOnly.onclick = async () => { const n = await applySavesToCache(overlay); summary.textContent = n ? `Saved ${n} override(s).` : 'No changes to save.'; updateSmartLogButtonState(); };
      saveClose.onclick = async () => { const n = await applySavesToCache(overlay); summary.textContent = n ? `Saved ${n} override(s).` : 'No changes to save.'; finish({ action: 'save-close' }); updateSmartLogButtonState(); };
      saveSend.onclick = async () => { const n = await applySavesToCache(overlay); summary.textContent = (n ? `Saved ${n} override(s). ` : '') + 'Sending…'; finish({ action: 'save-send' }); updateSmartLogButtonState(); };
    });
  }

  // ─── URL action (Show URL) is above; now FLAM action ────────────────────
  ...
  flamBtn.addEventListener('click', async function () {
    ...
    // Hot path: load base from IDB only; schedule non-blocking daily revalidate.
    await loadBaseBinOnce({ allowRevalidate: false });
    await loadLocalOverridesOnce();
    scheduleBaseRevalidateIfStale();
    ...
    // Pre-FLAM cache stats (IDB read + base lookup only)
    ...
    setTimeout(() => { btn.textContent = 'FLAM'; }, 1000);
    btn.disabled = false;
  ...
  function showSettings() {
    ...
    toolsWrap.innerHTML = `
      <div class="kb-actions" style="margin-bottom:10px">
        ...
        <button id="btnClearCache" class="kb-btn" title="Clear overrides">Clear Cache</button>
  ...
  async function openCacheEditor() {
    ...
    const fromFriendly = (friendly) => {
      ...
      for (const [k, v] of Object.entries(friendly || {})) {
        ...
        const tmdbId = Number(v?.tmdbId); if (!Number.isFinite(tmdbId)) continue;
        const type = String(v?.type || 'm').toLowerCase(); const isTv = (type === 'tv');
  ...
  // ─── Initial warm load: base from IDB; optional background revalidate ───
  await loadBaseBinOnce({ allowRevalidate: false });
  scheduleBaseRevalidateIfStale();
  await loadLocalOverridesOnce();
})();
