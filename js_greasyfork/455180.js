...
// @version      1.8.2
// @description  内部版本，仅用于EasyWJX在线版，请前往安装EasyWJX
// @author       MelonFish
...
// @license      GNU GPLv3
// @downloadURL https://update.greasyfork.org/scripts/455180/EasyWJX-iFrame.user.js
// @updateURL https://update.greasyfork.org/scripts/455180/EasyWJX-iFrame.meta.js
// ==/UserScript==
...
(function() {
    ...
    var iframeDocument = document.getElementById("i1").contentWindow.document;
    var iframeWin = document.getElementById("i1").contentWindow
    $('#i1').contents().find(".textCont,input,textarea").off()
    if(iframeWin.location.protocol == 'https:') {
        iframeWin.location.href = iframeWin.location.href.replace('https', 'http')
    }
    if ($('#i1').contents().find("#divTip").text().indexOf("最大填写次数")>=0) {
    ...
    $('#i1').contents().find('#spanPower').html('<a href="https://xq.kzw.ink" title="线圈_专为学生谋福利">线圈脚本</a><span>提供破解</span>')
    // 创建链接聊天室按钮
    var chat_btn = iframeDocument.createElement("div");
    ...
    $('#i1').contents().find('body').append(chat_btn)

    // 破解复制限制
    iframeDocument.oncontextmenu = function () {return true;};
    iframeDocument.onselectstart = function () {return true;};
    setTimeout(async function () {
        $('#i1').contents().find(".textCont,input,textarea").off()

        while (true) {
            var all_textCont = iframeDocument.querySelectorAll('.textCont')
    ...
    /*
    $('#i1').contents().find(".textCont,input,textarea").on({
        paste: function(e) {
            console.log(e.originalEvent.clipboardData)
            $('#i1').contents().find(e.currentTarget).val(e.originalEvent.clipboardData.getData())
        }
    })
    */
    $('#i1').contents().find("body").css("user-select", "text");
    // 创建按钮
    var addtrueans_btn = iframeDocument.createElement("button"); //创建一个input对象（提示框按钮）
    ...
    addtrueans_btn.onclick = function (e){
        ...
        if (addtrueans_ls!=null && addtrueans_ls!="") {
            if (addtrueans_ls!='all') {
                ...
                var ans_list = iframeDocument.querySelectorAll('.data__items');
    ...
    var clear_btn = iframeDocument.createElement("button"); //创建一个input对象（提示框按钮）
    ...
    clear_btn.onclick = function (e){
        ...
        $('#i1').contents().find('#spanPower').html('<a href="https://xq.kzw.ink" title="线圈_专为学生谋福利">问卷星</a><span>提供技术支持</span>')
        clear_btn.style.display = 'none';
        addtrueans_btn.style.display = 'none';
        var ques_titles = $('#i1').contents().find('.data__tit_cjd');
        for (var i=0; i<ques_titles.length; i++) {
            ques_titles.eq(i).text(ques_titles.eq(i).text().split('  题目ID：')[0])
        }
    };
    // 以上为创建按钮
    $('#i1').contents().find("#stop_jiexiTxt").append(addtrueans_btn).append(clear_btn)
    // 判断是否直接修改成绩
    if ($('#i1').contents().find(".score-font-style").length>0) {
        if(iframeWin.location.protocol == 'https:') {
            iframeWin.location.href = iframeWin.location.href.replace('https', 'http')
        } else {
            setTimeout(function () {
            ...
            if (r==true)
            {
                ...
                if (score!=null && score!="")
                {
                    $('#i1').contents().find(".score-font-style").eq(0).text(score)
                }
                var correct_num=prompt("修改后的正确题数（注意不要大于总题数）")
                if (correct_num!=null && correct_num!="")
                {
                    //$('#i1').contents().find(".tbottom-title").eq(3).html('<span style="font-size:20px;">'+correct_num+'</span>')
                    //$('#i1').contents().find("span[style$('#i1').contents().find='font-size:20px;']").text(correct_num)
                    $('#i1').contents().find(".score-form__list.clearfix .tht-content span").text(correct_num)
                }
                if ($('#i1').contents().find(".score-form__list.clearfix .tht-content").text().indexOf('名')>=0) {
                    var rank=prompt("修改后的名次（如果没有排名或者排名修改后出错请点击取消或留空）")
                    if (rank!=null && rank!="")
                    {
                        //$('#i1').contents().find(".tbottom-title").eq(3).html('<span style="font-size:20px;">'+correct_num+'</span>')
                        //$('#i1').contents().find("span[style$('#i1').contents().find='font-size:20px;']").text(correct_num)
                        $('#i1').contents().find(".score-form__list.clearfix .tht-content").eq(1).text("第"+rank+"名")
                    }
                }
            }
        }
        ...
        */
        var writeAnswer_btn = iframeDocument.createElement("button"); //创建一个input对象（提示框按钮）
        ...
        var clearCookie_btn = iframeDocument.createElement("button"); //创建一个input对象（提示框按钮）
        ...
        //writeLocalAnswer()
        /*
        var quickPass_btn = iframeDocument.createElement("button"); //创建一个input对象（提示框按钮）
        ...
        */
        $('#i1').contents().find('#toptitle h1').eq(0).append(writeAnswer_btn)
        $('#i1').contents().find('#toptitle h1').eq(0).append(clearCookie_btn)
        //$('#i1').contents().find('#toptitle h1').eq(0).append(quickPass_btn)
    }

    if ($('#i1').contents().find(".wxtxt").length >0) {
        var r2=confirm("监测到微信限制。是否需要移除限制并查看题目（可以查看题目但无法提交）")
        if (r2==true) {
            $('#i1').contents().find("#zhezhao2").remove();
            $('#i1').contents().find("#divContent").removeClass('disabled').removeClass('isblur');
            $('#i1').contents().find("#ctlNext").text('破解后可能无法提交')
            setTimeout(function () {
    ...
    // 使用搜索的方式
    var div_list = $('#i1').contents().find(".field-label");
    var btn_list = [];
    // var btn = $('#i1').contents().find('<button type="button" style="height: 1.5rem; width: 2.3rem; margin-left: 1rem;" onclick="alert(102210);">搜索</button>')
    for (i=0; i<div_list.length; i++) {
        var btn = iframeDocument.createElement("button"); //创建一个input对象（提示框按钮）
        ...
        btn.onclick = function (e){
            ...
            */
            ...
            //iframeWin.open('https://www.bing.com/search?q='+search_content)
            if (e.srcElement.textContent=="搜索") {
                var iframe = iframeDocument.createElement("iframe");
                ...
                iframeDocument.getElementById("search_iframe"+btn_num).remove();
            }
        };
        div_list.eq(i).append(btn)
    }
    for (var j=0; j<div_list.length; j++) {
        var copy_btn = iframeDocument.createElement("button"); //创建一个input对象（提示框按钮）
    ...
    function copy_to_clipboard(txt_str){
        ...
        iframeDocument.body.removeChild(input);
    }
    // -----------------------------------------以下这一段都是用来清理cookie的-------------------------------------------
    // 根据实测，foreach毫无效果，deleteAllCookies有少量效果（只能破解一半），clearCookie完全可以
    function deleteAllCookies() { // 方法1
        var cookies = iframeDocument.cookie.split(";");
        console.log(cookies)
        for (var i = 0; i < cookies.length; i++) {
            var cookie = cookies[i];
            var eqPos = cookie.indexOf("=");
            var name = eqPos > -1 ? cookie.substr(0, eqPos) : cookie;
            iframeDocument.cookie = name +"=;";
            //iframeDocument.cookie = null
        }
        var cookies2 = iframeDocument.cookie.split(";");
    ...
    function DelCookie(name) { // 方法2，删除一个的
        ...
        iframeDocument.cookie = name + "=" + cval + "; expires="+ exp.toGMTString();
    }
    function foreach() {// 方法2，删除全部的
        var strCookie=iframeDocument.cookie;
    ...
    function GetCookie(name) {// 方法2，这是获取的
        var arg = name + "=";
        var alen = arg.length;
        var clen = iframeDocument.cookie.length;
        var i = 0;
        while (i < clen)
        {
            var j = i + alen;
            if (iframeDocument.cookie.substring(i, j) == arg)
                return GetCooki (j);
            i = iframeDocument.cookie.indexOf(" ", i) + 1;
            if (i == 0) break;
        }
        return null;
    }
    function GetCooki(offset) {// 方法2，这是获取的
        var endstr = iframeDocument.cookie.indexOf (";", offset);
        if (endstr == -1)
            endstr = iframeDocument.cookie.length;
        return decodeURIComponent(iframeDocument.cookie.substring(offset, endstr));
    }
    */
    function clearCookie(){ //方法3
        // 这段代码来自其它脚本，为MIT协议，
        var keys = iframeDocument.cookie.match(/[^ =;]+(?==)/g);
        if (keys) {
            for (var i = keys.length; i--;) {
                iframeDocument.cookie = keys[i] + '=0;path=/;expires=' + new Date(0).toUTCString();
                iframeDocument.cookie = keys[i] + '=0;path=/;domain=' + iframeDocument.domain + ';expires=' + new Date(0).toUTCString();
                iframeDocument.cookie = keys[i] + '=0;path=/;domain=ratingdog.cn;expires=' + new Date(0).toUTCString();
            }
        }
    ...
    function getAnswer_radio(str_id) {
        ...
        console.log('开始检查',id)
        var ans_list_html = iframeDocument.querySelector('.query__data-result.new__data-result');
    ...
    function getAllAnswer_radio_input() {
        ...
        for (var i=0; i<ans_title_list.length; i++) {
            ...
            if (ans != 'NOTRADIO') {
                send_ls.push({ques_id: id, answer: ans, kind: kind})
    ...
    function getQueryString(name) {
        ...
        var r = iframeWin.location.search.substr(1).match(reg);
        if (r != null) return unescape(r[2]); return null;
    }
    function writeAnswer_radio_input() {
        var wj_id = iframeWin.location.pathname.replace('/vm/', '').replace('.aspx', '')
        console.log(wj_id)
        var answer_ls;
        $.post('//'+server_ip, {wj_id: wj_id}, function (result) {
            if (result.length==0) {
                console.log('NONE_ANSWER')
                iframeDocument.querySelector('#writeAnswer_btn').innerText = '暂无答案';
                setTimeout(function(){
                    iframeDocument.querySelector('#writeAnswer_btn').innerText = '自动填写';
                }, 2000)
                return 'NONE_ANSWER';
            }
            ...
            var ans_ls_html = iframeDocument.querySelectorAll('.field.ui-field-contain');
            ...
            for (i=0; i<ans_ls_html.length; i++) {
                ...
                if (input) {
                    for (var j=0; j<answer_ls.length; j++) {
                        if (answer_ls[j].ques_id==i && answer_ls[j].kind=='input') {
            ...
            for (i=0; i<ans_ls_html.length; i++) {
                ...
                if (checkbox.length!=0){
                    for (var j=0; j<answer_ls.length; j++) {
                        if (answer_ls[j].ques_id==i && answer_ls[j].kind=='checkbox') {
            ...
            for (i=0; i<ans_ls_html.length; i++) {
                ...
                var input_group = iframeDocument.querySelector('body').appendChild(copy_input_group_dom)
                //var input_group = iframeDocument.querySelectorAll('div')[iframeDocument.querySelectorAll('div').length-1]
                // 添加换行符方便分割
                var all_span = input_group.querySelectorAll('.textEdit');
                console.log('all_span',all_span)
                for (var v=0; v<all_span.length; v++){
                    var br = iframeDocument.createElement("br");
    ...
    function saveAnswerToLocal(){
        ...
        var answer_html = iframeDocument.querySelector('#fieldset1').innerHTML
    ...
    function writeLocalAnswer(){
        ...
        iframeDocument.querySelector('#fieldset1').innerHTML = answer_html
    }
    function sleep(time) {
    ...
    */
    ...
    function parseDom(arg) {
        var objE = iframeDocument.createElement("div");
        objE.innerHTML = arg;
        return objE;
    };
