// ==UserScript==
// @name         2016 Signup Theme
// @namespace    http://tampermonkey.net/
// @version      1.0.7
// @description  Brings Back The Funcationality Of The 2016 Signup Page
// @author       Vue2016
// @match        http://www.roblox.com
// @match        https://www.roblox.com
// @match        http://www.roblox.com/?returnUrl*
// @match        http://www.roblox.com/?ReturnUrl*
// @match        https://www.roblox.com/?returnUrl*
// @match        https://www.roblox.com/?ReturnUrl*
// @match        http://www.roblox.com/
// @match        http://www.roblox.com/
// @match        *https://www.roblox.com/NewLogin*
// @match        *https://www.roblox.com/Login*
// @match        *https://www.roblox.com/CreateAccount*
// @match        *https://www.roblox.com/CreateAccount?returnUrl=*
// @icon         data:image/png;base64,
// @grant        none
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/533474/2016%20Signup%20Theme.user.js
// @updateURL https://update.greasyfork.org/scripts/533474/2016%20Signup%20Theme.meta.js
// ==/UserScript==

(function() {
    'use strict';

document.querySelector(".rbx-login-form.theme-bg.signup-container").innerHTML += `
    <div class="legal-text-container">
                By signing up you agree to our <a href="https://www.roblox.com/info/terms" target="_blank">Terms</a> and <a href="https://www.roblox.com/info/privacy" target="_blank">Privacy Policy</a>
            </div> `;

    document.querySelector("#action-bar").innerHTML += `
    <ul id="TopLeftNavLinks" class="nav navbar-nav">
                    <li id="PlayLink" class="pull-left"><a href="#RollerContainer" onclick="return scrollTo(1, '#RollerContainer');">Play</a></li>
                    <li id="AboutLink" class="pull-left"><a href="#WhatsRobloxContainer" onclick="return scrollTo(2, '#WhatsRobloxContainer');">About</a></li>
                    <li id="PlatformLink" class="pull-left"><a href="#RobloxDeviceText" onclick="return scrollTo(3, '#RobloxDeviceText');">Platforms</a></li>
                    <li id="magic-line" style="left: 41px; width: 61px;"></li>
                </ul>`;

    document.querySelector(".header-icon-logo.icon-default-logo").innerHTML += `
    <div id="MainLogo" class="text-right">
                       <div id="LogoAndSlogan" class="text-center">
                            <img id="MainLogoImage" title="ROBLOX" class="center-block img-responsive" src="http://images.rbxcdn.com/f76c76889f4b167ca1f27edc27eb8146.png">
                            <div class="clearfix"></div>
                            <h1>You Make the Game<span> ™ </span></h1>
                        </div>
                    </div>
    `;

    document.querySelector("#action-bar").innerHTML += `
    <a id="HeaderForgotPassword" class="navbar-link" href="https://www.roblox.com/Login/ResetPasswordRequest.aspx">Forgot Username/Password?</a>`;
    document.querySelector("#action-bar").innerHTML += `
    <div class="form-group" id="LoginUsernameParent">
            <input id="LoginUsername" type="text" placeholder="Username" class="form-control" name="username" rbx-form-interaction="">
        </div> `;
    document.querySelector("#action-bar").innerHTML += `
    <div class="form-group" id="LoginPasswordParent">
            <input id="LoginPassword" type="password" placeholder="Password" class="form-control" name="password" rbx-form-interaction="">
        </div> `;
    document.querySelector(".rbx-login-form.theme-bg.signup-container").innerHTML += `
    <div class="form-group" ng-class="{'has-error' : (badSubmit || signupForm.passwordConfirm.$dirty) &amp;&amp; signupForm.passwordConfirm.$invalid, 'has-success': (signupForm.passwordConfirm.$dirty &amp;&amp; signupForm.passwordConfirm.$valid) }">
                <input id="signup-password-confirm" ng-trim="false" name="passwordConfirm" class="form-control input-field ng-isolate-scope ng-pristine ng-invalid ng-invalid-match" match="signup.password" rbx-valid-password-confirm="" rbx-form-interaction="" rbx-form-validation="" rbx-form-validation-redact-input="" type="password" tabindex="3" placeholder="Confirm Password" ng-model="signup.passwordConfirm">
                <p id="signup-passwordConfirmInputValidation" class="form-control-label input-validation text-error ng-binding" ng-bind="(badSubmit || signupForm.passwordConfirm.$dirty) ? signupForm.passwordConfirm.$validationMessage : ''"></p>
            </div>`;
    document.querySelector(".rbx-login-form.theme-bg.signup-container").innerHTML += `
    <div class="attribution hidden-xs">
            <span class="notranslate">Game: ROBLOX Point</span><br>
            Developer: <span class="notranslate">StarMarine614</span>
        </div> `;

        document.querySelector(".container-footer").innerHTML += `
<footer class="row">
        <div class="col-xs-12">
            <div id="FooterBigLinks" class="row">
                <div class="col-md-12 text-center">
                    <a href="//web.archive.org/web/20150601150558/http://corp.roblox.com/" target="_blank">About Us</a>
                    <a href="//web.archive.org/web/20150601150558/http://corp.roblox.com/jobs" target="_blank">Jobs</a>
                    <a href="//web.archive.org/web/20150601150558/http://blog.roblox.com/" target="_blank">Blog</a>
                    <a href="/web/20150601150558/http://www.roblox.com/Info/Privacy.aspx" target="_blank">Privacy</a>
                    <a href="//web.archive.org/web/20150601150558/http://corp.roblox.com/parents" target="_blank">Parents</a>
                    <a href="//web.archive.org/web/20150601150558/http://en.help.roblox.com/" target="_blank">Help</a>
                </div>
            </div>
            <div class="row">
                <div id="FooterLegalText" class="col-xs-11 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3 text-justify">
                    ROBLOX, "Online Building Toy", characters, logos, names, and all related indicia are trademarks of <a target="_blank" href="//web.archive.org/web/20150601150558/http://corp.roblox.com/">ROBLOX Corporation</a>, ©2015. Patents pending. ROBLOX is not sponsored, authorized or endorsed by any producer of plastic building bricks, including The LEGO Group, MEGA Brands, and K'Nex, and no resemblance to the products of these companies is intended. Use of this site signifies your acceptance of the <a href="/web/20150601150558/http://www.roblox.com/info/terms-of-service" target="_blank">Terms and Conditions</a>.
                </div>
            </div>
        </div>
    </footer>`;

document.title = "ROBLOX.com"

})();

    