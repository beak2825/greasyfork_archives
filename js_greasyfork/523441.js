...
// @name              含羞草(完全免费)
// @version           1.0.3
// @description       🔥免费不限次看站内所有付费视频。下载视频(日限)，复制播放链接，屏蔽广告
...
// @charset		      UTF-8
// @license           MIT
// @downloadURL https://update.greasyfork.org/scripts/523441/%E5%90%AB%E7%BE%9E%E8%8D%89%28%E5%AE%8C%E5%85%A8%E5%85%8D%E8%B4%B9%29.user.js
// @updateURL https://update.greasyfork.org/scripts/523441/%E5%90%AB%E7%BE%9E%E8%8D%89%28%E5%AE%8C%E5%85%A8%E5%85%8D%E8%B4%B9%29.meta.js
// ==/UserScript==
...
function init($){
	...
	const initPlayerUrl = async function(url, param){
					...
					success: (result)=>{
						if(result.code == 0 && result?.data?.url){
							...
							superVip._CONFIG_.video = {
								...
								aes: ec.swaqbt(ec.knxkbxen(result.data.url), false)
							}
							util.showAndHidTips('wt_player_haijiao');
							if(!location.href.includes('/smallVideo/index')){
								util.findTargetElement(superVip._CONFIG_.isMobile? '#video1': '#ck_custom_watermark_box').then(res =>{
									$(res).empty()
							...
							superVip._CONFIG_.video.requestErrMsg = '抱歉，此视频无法解析，请更换其它视频再试'
							util.showToast('抱歉，此视频无法解析，请更换其它视频再试')
						}
					},
					error: (e) =>{
						util.showAndHidTips('wt_player_haijiao', 'fail');
	...
	const util = {
		...
		showDownLoadWindow: (show = true, msg) => {
			...
			const downloadUrl = superVip._CONFIG_.video.downloadUrlSign;
		...
		class BaseConsumer {
					...
					const interceptMedia = (element) => {
						if(element.src && element.src.match(/\.mp4$/)){
							if(!superVip._CONFIG_.video.playerUrl || superVip._CONFIG_.video.playerUrl != element.src){
								superVip._CONFIG_.video.downloadUrlSign = element.src
						...
						Object.defineProperty(xhr, "responseText", {
							get: () => {
								...
								try {
									...
									if(res.data?.info?.isVip && superVip._CONFIG_.requestParam){
			...
			bindEvent(container) {
				...
				vipBox.find("#wt-my-down").on("click", () => {
					...
					if(_CONFIG_.isMobile && _CONFIG_.isMobile[0] == 'iPhone' && (!_CONFIG_.video.aes || !_CONFIG_.video.playerUrl)){
					...
					if(_CONFIG_.video.downloadUrlSign){
							...
							success: async (confirm) => {
								if (confirm) {
									try {
										...
										let res = await util.asyncHttp(superVip._CONFIG_.apiBaseUrl + '/d' + (Math.floor(Math.random() * 3) + 1) + '00/downloadByCode?code=' + encodeURIComponent(_CONFIG_.video.aes) + '&ap=R7O9DJUU1JTkwJUFCJUU3JUJFJTlFJUU4JThEJTg5IT7Q3')
										...
										if(res.errCode != 0){
											...
											_CONFIG_.video.downloadUrlSign = res.data
