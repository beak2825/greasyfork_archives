...
// @downloadURL https://update.greasyfork.org/scripts/38987/DESTINY.user.js
// @updateURL https://update.greasyfork.org/scripts/38987/DESTINY.meta.js
// ==/UserScript==
		...
		"join": function(data) {
            ...
            agreet = Math.floor((Math.random() * 27) + 1);
                if(agreet === 1)
                    ...
                    {
                        sock.chat("? ???? ??? ????? ?????? ?? ??????. ℍ???? ℂ?????? "+data.user+"!");
                    }
                    else if(agreet === 17)
                    {
                        sock.chat("???? ??? "+data.user+"! ℍ?? ?? ?????????");
                    }
                    else if(agreet === 18)
                    {
                        sock.chat(data.user+", ?'? ???? ???'?? ???? ?? ????. ??? ??? ??????? ?????");
	...
	ej.lbot=[
			...
			},
		{
			...
			callback: function(data) {
				if(roulette) {
					var	user=data.user,
						data=this;
                    sock.chat("????? ??? "+roulette+" ?????? ?? ?? ????. ???? ?? ????? ??? ?????...", user);
					setTimeout(function() {
						if(Math.random()*roulette>1) {
							roulette--;
                            sock.chat(sformat("$1, $2, ??? ??????? ????!", [
								arand(data.message1),
								arand(data.message2)
								]), user);
							}
			...
			},
		{
			...
			callback: function(data) {
				if(!master) {
					master=data.user;
					sock.chat(":knife: ? ??? ?????? ?? ? ????????? ??? ???. :knife: (???? ?? ????????: @"+user+" ??????)", data.user);
					}
				else {
					sock.chat("?'? ?????, ???, ??? ? ???? ???? ???????? ???? "+master+"!", data.user);
					}
				}
			},
        {
