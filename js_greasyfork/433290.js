...
// @run-at      document-end
// @namespace https://greasyfork.org/users/713845
// @downloadURL https://update.greasyfork.org/scripts/433290/%E7%8E%B0%E5%9C%BA%E7%9B%B4%E6%92%AD.user.js
// @updateURL https://update.greasyfork.org/scripts/433290/%E7%8E%B0%E5%9C%BA%E7%9B%B4%E6%92%AD.meta.js
// ==/UserScript==
(function() {
    'use strict';
    var qy_settingData = {
        ...
        list = list.filter(function(item,index){
            if(index<=list.length-2){
                if(item.includes("车")){
                    ...
                    let cc = bb.split(".");
                    qy_xzcda[Number(aa)-1]=true;
                    //console.log("qy_xzcda：",aa );
                    for( j=0;j<cc.length;j++){
                        qy_xzcdb[Number(aa)-1][Number(cc[j])-1]=true;
                    };
                    qy_xhjs++;
                    qy_s11=qy_s11+aa;
    ...
    function qy_huixianVar(hx_kg,hx_xhjs){
        if(hx_kg){
            ...
            //if(!hx_xhjs){win.querySelector("#qy_iput11").style.display="block";win.querySelector("#qy_iput12").style.display="block";};
            win.querySelector("#qyr-setMenu #qy_iput11").value=dui_bs[hx_xhjs];
            win.querySelector("#qyr-setMenu #qy_iput12").value=cuo_bs[hx_xhjs];
            for(var i=0;i<10;i++){
                ...
                if(i==hx_xhjs){
                    qy_xzcda[hx_xhjs] ? win.querySelector("#qyr-setMenu #btnchecked"+String(hx_xhjs+1)).checked=true:"";
                };
                console.log(hx_xhjs,"hx_xhjs：",qy_xzcdb[hx_xhjs][i] ,"i==", i);
    ...
    function get_xintiao(){
        ...
        if(qy_shortcut==0){
            ...
            if(qy_xintiaokg>11){win=window.document;win.querySelectorAll("#sub_PK10JSC > a")[1].click();qy_xintiaokg=0};
        ...
        if(qy_shortcut2){
            ...
            qy_kskg = 0;qy_xtkg = 3;qy_xintiaokg=0;
        };
    };
    ...
    function qy_xuaz_u(xz_cd,xz_hm){
        if(qy_shortcut==1 || qy_shortcut==3){var win=window.frames[0].document;};
        if(qy_shortcut2 && qy_bnode==true){
            qy_jnr=qy_jiner*qy_beilv;
            var sg_cd=2,sg1_cd=0,sg2_cd=xz_hm+1,sg3_cd=xz_cd+1;
            if(xz_cd==0 || xz_cd==5){
                sg1_cd=1;
            ...
            if(qy_shortcut==5){
                win.querySelector("#a_B"+sg3_cd+"_"+xz_hm+">input").value=qy_jnr;
            }else if(qy_shortcut==1 || qy_shortcut==3){
                win=window.frames[0].document;
                //if(xz_cd>4){sg_cd=3};
                win.querySelector('[name=B'+sg3_cd+'_'+xz_hm+']').value=qy_jnr;
            }else if(qy_shortcut==2){
    ...
    function setMenu(){
        ...
        for(var i=0;i<10;i++){
            if(qy_xzcda[i] && j==0){
                j=1;k=i;
                CheckVar1[i] = qy_xzcda[i]?'checked':'';
            };
            if(j){
        ...
        var innerH = "" +
            ...
            ">" +
            "高停:<input id='qy_iput6' type='text' style='width:40px;height:20px' value="+qy_zhisun+">" +
