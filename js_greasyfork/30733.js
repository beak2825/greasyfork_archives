...
// @namespace   HHeroes
// @description Bot
// @include     https://*.hentaiheroes.*/*
// @include     http://*.hentaiheroes.*/*
// @version     1.2
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/30733/HentHeroes.user.js
// @updateURL https://update.greasyfork.org/scripts/30733/HentHeroes.meta.js
// ==/UserScript==

var uri = document.documentURI;
var x;
var script = document.createElement("script");
var txt = sleep.toString() + cleanArmory.toString() + cleanBooster.toString();
...
if (uri.includes("harem")) {
    ...
    window.onload = function(){
      collectHarem();
      setInterval(collectHarem, 10 * 1000);
    }
}
...
function loadHud() {
  ...
  btn.appendChild(document.createTextNode("Clean Armory"));
  btn.setAttribute("style", "float: left; position: fixed; z-index: 999; margin-top: 7%;");
  btn.setAttribute("onclick", "cleanArmory()");
  document.body.getElementsByTagName("header")[0].appendChild(btn);
  
  btn = document.createElement("BUTTON");
  btn.appendChild(document.createTextNode("Clean Boosters"));
  btn.setAttribute("style", "float: left; position: fixed; z-index: 999; margin-top: 7%; margin-left: 14%");
  btn.setAttribute("onclick", "cleanBooster()");
  document.body.getElementsByTagName("header")[0].appendChild(btn);
}
...
async function cleanArmory() {
  ...
  if (document.getElementById("type_item").getElementsByClassName("selected")[0].getAttribute("type") != "armor"){
    document.getElementById("type_item").children[0].click();
  }
  var beforeMoney = parseInt(document.getElementsByClassName("currency")[0].firstChild.innerHTML.replace(/[^0-9\.]/g, ""));
  if (document.getElementsByClassName("currency")[0].firstChild.innerHTML.includes("M")){
    beforeMoney *= 1000000;
  }
  var soldItems = 0;
  var stackSize;
...
async function cleanBooster(){
  ...
  if (document.getElementById("type_item").getElementsByClassName("selected")[0].getAttribute("type") != "booster"){
    document.getElementById("type_item").children[1].click();
  }
  var x, selected;
  var stackSize = 0;
...
async function checkBattleOver(){
  if (document.getElementById("battle_end").style.display == "block"){
    if (uri.includes("id_troll")) {
      ...
      var n = str.lastIndexOf('=');
      var result = parseInt(str.substring(n+1)) + 1;
      location.href = "https://www.hentaiheroes.com/world/" + result;
    }else {
      location.href = "https://www.hentaiheroes.com/arena.html";
    }
  }
}
