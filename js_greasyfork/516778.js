...
// @version      1.0
// @description  Customize gradients and colors on cracked.sh, add particle effects, and integrate a YouTube music player with loop functionality.
// @author       SpEc
// @match        https://*.cracked.sh/*
// @icon         https://static.cracked.sh/uploads/avatars/avatar_3116424.gif?dateline=1719456680
// @grant        none
// @supportURL   https://cracked.sh/spec
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/516778/Spectify.user.js
// @updateURL https://update.greasyfork.org/scripts/516778/Spectify.meta.js
// ==/UserScript==
...
(function() {
    ...
    const baseThemes = {
        ...
        'Space Theme': {
            ...
            '--background-floating': '#1c1c1c',
            '--background-floating-alt': '#262626',
            '--background-floating-light': '#2e2e2e',
            '--border-color': '#444444',
            '--border': '1px solid var(--border-color)',
            '--radius': '10px',
            '--banner-padding': '20px',
            '--btn': '#3d3d3d7a',
            '--btn-hover': 'linear-gradient(45deg, #666666, #333333)',
            '--dropdown-hover': 'linear-gradient(45deg, #333333, #666666)',
            '--text-dark': '#ffffff',
            '--forumtabs-icon-color': '#888888',
            '--panel-upper-icon-color': '#aaaaaa',
            '--trow-border-top-color': '#4a4a4a',
            '--trow-border-bottom-color': '#1a1a1a',
            '--trow-text-color': '#ffffff',
            '--reputation-positive-color': '#4a4a4a',
            '--helpdocs-number-color': '#b3b3b3',
            '--footer-color': '#000000',
            '--particle-color': '#ffffff',
            '--particle-sway-amplitude': '50px',
            '--snow-enabled': 'true'
    ...
    function setCookie(name, value, days) {
        ...
        document.cookie = name + "=" + (value || "")  + expires + "; path=/; domain=.cracked.sh";
    }
    ...
    function eraseCookie(name) {
        document.cookie = name+'=; Max-Age=-99999999; path=/; domain=.cracked.sh';
    }
    ...
    window.addEventListener('message', (event) => {
        ...
        if (!event.origin.includes('cracked.sh')) return;
    ...
    function openMusicPlayer() {
        ...
        if (!musicPlayerWindow) {
            showNotification('❌ Unable to open music player window. Please allow popups for cracked.sh.');
            return;
        }
    ...
    function removeShoutbox() {
        ...
        if (shoutbox) {
            shoutbox.classList.add('spectify-transition', 'spectify-hidden');
            setTimeout(() => {
    ...
    function showShoutbox() {
        ...
        if (shoutbox) {
            ...
            shoutbox.classList.add('spectify-transition');
            void shoutbox.offsetWidth; // Reflow to apply the transition
    ...
    function removePinnedTools() {
        const pinnedToolsContainers = document.querySelectorAll('div[align="center"]:not(.inner_stuff)');
        pinnedToolsContainers.forEach(container => {
            container.classList.add('spectify-transition', 'spectify-hidden');
            setTimeout(() => {
    ...
    function showPinnedTools() {
        ...
        pinnedToolsContainers.forEach(container => {
            ...
            container.classList.add('spectify-transition');
    ...
    // Initialize Element Removal on script load
