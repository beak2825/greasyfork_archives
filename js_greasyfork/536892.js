// ==UserScript==
// @name         LSS - Fehlende Fahrzeuge Nachalarmieren
...
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/536892/LSS%20-%20Fehlende%20Fahrzeuge%20Nachalarmieren.user.js
// @updateURL https://update.greasyfork.org/scripts/536892/LSS%20-%20Fehlende%20Fahrzeuge%20Nachalarmieren.meta.js
// ==/UserScript==

(function() {
    'use strict';

    const INITIAL_DELAY_MS = 200; // Initial delay before checking for the alarm window and adding the button
    const CHECKBOX_DELAY_MS = 2; // Verzögerung zwischen den einzelnen Checkbox-Klicks (reduziert für schnellere Aktivierung)
    const RELOAD_WAIT_MS = 2000;
    const MAX_RELOAD_ATTEMPTS = 15; // Max. Nachladeversuche auf 8 gesetzt
    ...
    const CARE_VEHICLE_IDS = [130, 131].map(String);
    ...
    const NEF_PER_10_SILENT_PATIENTS = 3; // Number of NEF-equivalents (IDs 29, 31, 149, 157)
    const RTH_PER_10_SILENT_PATIENTS = 0; // Number of RTHs (IDs 31, 157)
    ...
    "lüfter": [114, 115, 116], "lüfter-erweiterungen": [114, 115, 116], "mannschaftstransport-wagen": [36, 40, 80, 82, 93, 106, 124, 125, 126, 140, 148, 173],
    ...
    "mzgw sb": [109], "mzkw": [41, 109], "nea200": [112, 113], "nea200-erweiterungen": [112, 113], "nea50": [110, 111, 172, 175],
    "nea50-erweiterungen": [110, 111, 172, 175], "nef": [29, 31, 149, 157], "nef, lna, orgl": [55, 56, 29],
    ...
    "polizeihubschrauber": [61, 156], "polizeimotorrad": [95], "polizeimotorräder": [95], "polizisten": [32, 98, 50, 51, 52, 95, 103],
    ...
    "rettungstreppe": [76],
    "rettungstreppen": [76],
    "rettungswache": [38, 58, 28, 73, 74, 97, 171, 172, 173, 175],
    "rth": [31, 157],
    "rtw": [28, 38, 58, 73],
    ...
    "gw-bt": [130], "gw bt": [130], "bt-kombi": [131], "fkh": [132, 141], "bt lkw": [133],
    ...
    "rettungsdienstler": [28, 29, 31, 38, 55, 56, 58, 59, 60, 73, 74, 91, 97, 149, 171, 172, 173, 175],
    "fükomkw": [146], // Specific, no alternatives
    ...
    const EQUIPMENT_TO_VEHICLE_MAPPING = {
        'Betreuungs- und Verpflegungsausstattung': ['GW-Bt', 'Bt-Kombi'],
    };
    ...
    const EMS_PERSONNEL_TYPES = ['LNA', 'OrgL'];
    ...
    function trySelectVehicleDryRun(vehicleData, tempReqs, tempAltReqs, tempGenLFReq, tempSelCheckboxes, availableVehiclesForDryRun) {
        ...
        if (selectedVehicleCapabilities) {
            ...
            if (vehicleData.vehicleTypeId === ktwTypB_Id && gwSan_Id) {
                ...
                const neededGwSansForKtws = Math.ceil(totalKtwBTypCount / 5);
    ...
    async function processAlarmRequirements(filterCategory = 'all', bestEffortMode = false) {
            ...
            if (sections.vehicles) {
                sections.vehicles.split(',').map(s => s.trim()).filter(Boolean).forEach(part => {
                    ...
                    if (vehicleMatch) {
                        ...
                        // START - Anforderung ignorieren
                        if (fullRequestText.toLowerCase().includes('lkw k 9')) {
                            console.log(`[LSS-Nachalarmierung] IGNORIERT: Die Anforderung für "${fullRequestText}" wird übersprungen.`);
                            return;
                        }
                        // ENDE - Anforderung ignorieren
        ...
        let bestDryRunResult = { selectedCheckboxes: new Set(), metAllReqs: false, remainingReqs: Infinity };
        ...
        while (reloadAttempts <= MAX_RELOAD_ATTEMPTS && !allRequirementsMetInLoop) {
            ...
            if (dryRunRequirementsMet) {
                ...
                allRequirementsMetInLoop = true;
                 ...
                 console.log(`[LSS-Nachalarmierung] Trockenlauf nicht erfolgreich. Verbleibende Anforderungen: ${remainingReqsCount}. Versuche Nachladen.`);
            }

            reloadAttempts++;
        } // End while loop
// ===== START: Hinzugefügter und überarbeiteter Code für ELW 1 SEG Regel mit Nachladefunktion =====
...
if (elwSegTypeId) {
    ...
    if (rdVehicleSelected) {
        const elwSegIsAlreadyDispatched = (dispatchedVehicles.onScene.get(elwSegTypeId) || 0) > 0 || (dispatchedVehicles.ownOnWay.get(elwSegTypeId) || 0) > 0;
        ...
        for (const checkboxId of bestDryRunResult.selectedCheckboxes) {
             const vehicleData = finalAvailableVehicleData.find(v => v.checkbox.id === checkboxId) || globalCheckboxesToProcess.find(v => v.checkbox.id === checkboxId);
        ...
        if (!elwSegIsAlreadyDispatched && !elwSegIsAlreadyExplicitlyRequired && !elwSegIsAlreadySelected) {
            console.log("[LSS-Nachalarmierung] REGEL: Suche nach ELW 1 (SEG) gestartet, da ein RD-Fahrzeug alarmiert wird.");
            let elwSegFoundAndSelected = false;
            let elwSegReloadAttempts = 0;

            while (!elwSegFoundAndSelected && elwSegReloadAttempts <= MAX_RELOAD_ATTEMPTS) {
                let currentVehicleListForElw = Array.from(document.querySelectorAll('tr.vehicle_select_table_tr')).map(row => {
                     const checkbox = row.querySelector('input[type="checkbox"].vehicle_checkbox');
                     if (!checkbox || checkbox.checked) return null;
                     const vehicleTypeTd = row.querySelector('td[vehicle_type_id]');
                     const vehicleTypeId = vehicleTypeTd ? vehicleTypeTd.getAttribute('vehicle_type_id') : null;
                     if (vehicleTypeId === elwSegTypeId) {
                         return { checkbox: checkbox, vehicleLabel: (vehicleTypeTd.querySelector('label.mission_vehicle_label')?.textContent.trim().replace(/\s+/g, ' ') || 'Unbekannt') };
                     }
                     return null;
                }).filter(Boolean);

                if (currentVehicleListForElw.length > 0) {
                    const availableElwSeg = currentVehicleListForElw.sort((a,b) => parseDistance(a.checkbox.closest('tr').querySelector('td[id^="vehicle_sort_"]').textContent) - parseDistance(b.checkbox.closest('tr').querySelector('td[id^="vehicle_sort_"]').textContent))[0];
                    bestDryRunResult.selectedCheckboxes.add(availableElwSeg.checkbox.id);
                    console.log(`[LSS-Nachalarmierung] REGEL: ELW 1 (SEG) "${availableElwSeg.vehicleLabel}" gefunden und zur Auswahl hinzugefügt.`);
                    elwSegFoundAndSelected = true;
                } else if (elwSegReloadAttempts < MAX_RELOAD_ATTEMPTS) {
                    console.log(`[LSS-Nachalarmierung] Kein ELW 1 (SEG) in der Liste gefunden. Lade weitere Fahrzeuge nach (Versuch: ${elwSegReloadAttempts + 1}).`);
                    const reloadButton = document.querySelector('a.missing_vehicles_load');
                    if (reloadButton) {
                        try {
                            reloadButton.click();
                            ...
                            console.error("[LSS-Nachalarmierung] Fehler bei Klick auf Reload-Button:", e);
                            break;
                        }
                    } else {
                        console.warn("[LSS-Nachalarmierung] Reload-Button für ELW 1 (SEG) Suche nicht gefunden. Breche Suche ab.");
                        break;
                    }
                    await new Promise(resolve => setTimeout(resolve, RELOAD_WAIT_MS));
                }
                elwSegReloadAttempts++;
            }

            if (!elwSegFoundAndSelected) {
                 console.warn("[LSS-Nachalarmierung] Konnte nach allen Versuchen keinen verfügbaren ELW 1 (SEG) finden.");
            }
        }
    }
}
// ===== ENDE: Hinzugefügter und überarbeiteter Code für ELW 1 SEG Regel mit Nachladefunktion =====
...
async function processCareVehicleRequirements() {
    ...
    if (requiredKitchens <= 0 && requiredCareHelpers <= 0) {
        console.log("[LSS-Nachalarmierung] Kein Care Vehicle Bedarf. Prozess wird übersprungen.");
        return;
    }

    console.log(`[LSS-Nachalarmierung] Anfänglicher Care-Bedarf: ${requiredKitchens} Küchen, ${requiredCareHelpers} Betreuer.`);

    let reloadAttempts = 0;

    while ((requiredKitchens > 0 || requiredCareHelpers > 0) && reloadAttempts <= MAX_RELOAD_ATTEMPTS) {
        console.log(`[LSS-Nachalarmierung] Care-Schleife ${reloadAttempts + 1}/${MAX_RELOAD_ATTEMPTS + 1}. Bedarf: ${requiredKitchens} Küchen, ${requiredCareHelpers} Betreuer.`);
        ...
        if ((requiredKitchens > 0 || requiredCareHelpers > 0) && reloadAttempts < MAX_RELOAD_ATTEMPTS) {
            console.log(`[LSS-Nachalarmierung] Care-Bedarf nicht gedeckt. Lade weitere Fahrzeuge nach (Versuch: ${reloadAttempts + 1}).`);
            const reloadButton = document.querySelector('a.missing_vehicles_load');
            if (reloadButton) {
                try {
                    reloadButton.click();
                ...
                console.error("[LSS-Nachalarmierung] Fehler: Reload-Button nicht gefunden. Breche Care-Schleife ab.");
                break;
            }
            await new Promise(resolve => setTimeout(resolve, RELOAD_WAIT_MS));
        }
    ...
    if (requiredKitchens > 0 || requiredCareHelpers > 0) {
        console.warn(`[LSS-Nachalarmierung] Care Vehicle Prozess abgeschlossen, aber es verbleibt ein Bedarf von ${requiredKitchens} Küchen und ${requiredCareHelpers} Betreuern.`);
    } else {
        console.log("[LSS-Nachalarmierung] Care Vehicle Prozess erfolgreich abgeschlossen.");
    }
}
...
async function alarmMissingEMSVehiclesManually() {
    ...
    try {
        ...
        // Der Block für die Care-Fahrzeuge wurde hier entfernt.
