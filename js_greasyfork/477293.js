...
// @contributor    CCO4308 - Mod MCV rewok/redesign
// @namespace      https://*.alliances.commandandconquer.com/*/index.aspx*
// @match          https://*.alliances.commandandconquer.com/*/index.aspx*
// @grant none
// @downloadURL https://update.greasyfork.org/scripts/477293/MaelstromTools%20Dev%20%28Modv17%20for%20MCV%29.user.js
// @updateURL https://update.greasyfork.org/scripts/477293/MaelstromTools%20Dev%20%28Modv17%20for%20MCV%29.meta.js
// ==/UserScript==
                        ...
                        members: {
                            ...
                            calculateCostsForNextMCV: function () {
                                try {
                                    ...
                                    //  return;
                                    //}
                                    this.mcvPopup.setCaption(Lang.gt("ADDITIONAL BASE"));
                                    if (creditTimeLeftInHours > 0 && doGrow) {
                                        this.mcvTimerLabel.setValue("<span style='color: #ffffff;'>MCV in: </span> <span style='color: #ff8a7f;'>" + creditTimeLeftInDays + "</span>");
                                    }
									if (creditTimeLeftInHours < 24 && doGrow){
											this.mcvTimerLabel.setValue("<span style='color: #ffffff;'>MCV in: </span> <span style='color: #7fffb8;'>" + creditTimeLeftInDays + "</span>");
									}
									if(PercentageOfCredits >= 100 && PercentageOfResearchPoints >= 100){
										this.mcvTimerLabel.setValue("<span style='color: #7fffb8;'><span style='margin-right:8px;'>&check;</span>MCV</span>");
									}
									if (doGrow == false){
                                       this.mcvTimerLabel.setValue("<span style='color: #ff8a7f;'>MCV: Timer Stopped</span>");
                                    }
                                    if (PercentageOfCredits >= 100) {
                                        this.mcvCreditProcentageLabel.setValue("<span style='color: #7fffb8;text-align:left;'><span style='margin-right:8px;'>&check;</span>Credits</span>");
                                        this.mcvplace.setValue("<span style='color: #7fffb8;'>Credits: +" + MaelstromTools.Wrapper.FormatNumbersCompact(Math.abs(creditsNeeded - ZXZ)) + "</span>");
                                    }
                                    if (PercentageOfCredits < 100) {
                                        var nogrowing = "when hell is cold..";
                                        this.mcvCreditProcentageLabel.setValue("<span style='color: #ffffff;'>Credits: </span><span style='color: #ff8a7f;'> -" + MaelstromTools.Wrapper.FormatNumbersCompact(creditsNeeded - ZXZ) + "</span> / <span>" + MaelstromTools.Wrapper.FormatNumbersCompact(creditsNeeded) + "</span><span style='color: #7fffb8;'> (" + (PercentageOfCredits).toFixed(1) + "%)</span>");
                                        this.mcvplace.setValue("<span style='color: #ffffff;'>Credits in: </span><span style='color: #ff8a7f;'>" + (doGrow ? creditTimeLeftInDays : nogrowing) + "</span>");
                                    }
                                    if (PercentageOfResearchPoints >= 100) {
                                        this.mcvResearchTimerLabel.setValue("<span style='color: #7fffb8;'><span style='margin-right:8px;'>&check;</span>Research Points</span>");
                                        this.mcvplace.setValue(this.mcvplace.$$user_value + "<br><span style='color: #7fffb8;'>Research Points: +" + MaelstromTools.Wrapper.FormatNumbersCompact(Math.abs(researchNeeded - currentResearchPoints)) + "</span>");
                                    }
                                    if (PercentageOfResearchPoints < 100) {
                                        this.mcvResearchTimerLabel.setValue("<span style='color: #ffffff;'>RP: </span><span style='color: #ff8a7f;'> -" + MaelstromTools.Wrapper.FormatNumbersCompact(researchNeeded - currentResearchPoints) + "</span> /<span> " + MaelstromTools.Wrapper.FormatNumbersCompact(researchNeeded) + "</span><span style='color: #7fffb8;'> (" + (PercentageOfResearchPoints).toFixed(1) + "%)</span>");
                                        this.mcvplace.setValue(this.mcvplace.$$user_value + "<br><span style='color: #ffffff;'>Research Points: </span><span style='color: #ff8a7f;'>" + (PercentageOfResearchPoints).toFixed(1) + "%</span>");
                                    }
                                    this.extItems = [];
