...
// @name			teethbandit
// @version			1.9.1.3
// @description		's emjack v1.9.1.2
// @match			https://epicmafia.com/game/*
// @match			https://epicmafia.com/lobby
// @namespace		https://greasyfork.org/users/146029/
// @author			teethbandit
// @homepage		https://epicmafia.com/user/822239#/
// @icon			https://i.imgur.com/NFUf2m4.gif
// @downloadURL https://update.greasyfork.org/scripts/389606/teethbandit.user.js
// @updateURL https://update.greasyfork.org/scripts/389606/teethbandit.meta.js
// ==/UserScript==

//stole this from shwartz99...... thanks homie
...
function emjack() {
    ...
    // public
    $("#typebox").val("give me your teeth");
    ...
    ej.cmds={
        ...
        "join": function(data) {
            ...
            if(ej.settings & AGREET) {
                 if(data.user === "teethbandit") {
            sock.chat("/me bows", data.user);
            sock.chat("M'lady");}
                 else if(data.user === "TheLarryJohnson") {
            sock.chat("/me tips fedora",data.user);
            sock.chat("M'lord");}
                 else if(data.user === "SalFisher") {
            sock.chat("hi sal");}
            ...
            agreet = Math.floor((Math.random() * 27) + 1);
                if(agreet === 1)
                    {
                        sock.chat(data.user+"! i'm going to enter your home and forcibly remove your liver");
                    }
                    else if(agreet === 2)
                    {
                        sock.chat("who let "+data.user+"into my game omg my god.");
                    }
                    else if(agreet === 3)
                    {
                        sock.chat(""+data.user+" is dead. "+data.user+" wemains dead. And we have kiwwed him. How shaww we comfowt ouwsewves, the mwuwdewews of aww mwuwdewews?? what was howiest and mwightiest of aww that the wowwd has yet owned has bwed to death undew ouw knives: who wiww wipe this bwood off us?? what watew is thewe fow us to cwean ouwsewves?? what festivaws of atonement, what sacwed games shaww we have to invent?? is not the gweatness of this deed too gweat fow us?? mwust we ouwsewves not become gods simpwy to appeaw wowthy of it? UwU");
                    }
                    else if(agreet === 4)
                    {
                        sock.chat("*tips fedora* it is an honor to be in your presence, m'lady "+data.user+"!");
                    }
                    else if(agreet === 5)
                    {
                        sock.chat("every night i think about cannibalizing "+data.user+"!");
                    }
                    else if(agreet === 6)
                    {
                        sock.chat("i heard that "+data.user+" sucks toes");
                    }
                    else if(agreet === 7)
                    {
                        sock.chat("lynch "+data.user+" n1");
                    }
                    else if(agreet === 8)
                    {
                        sock.chat("im going to pee "+data.user+"'s pants");
                    }
                    else if(agreet === 9)
                    {
                        sock.chat("and i oop! "+data.user+" dropped their hydroflask sksksksksks!");
                    }
                    else if(agreet === 10)
                    {
                        sock.chat("mark my worms, i'm going to steal "+data.user+"'s molars if it's the last thing i do");
                    }
                    else if(agreet === 11)
                    {
                        sock.chat("it's world famous fornite streamer "+data.user+"!");
                    }
                    else if(agreet === 12)
                    {
                        sock.chat(data.user+".... what if i... put my minecraft bed next to yours? aha ha, just kidding. ...unless??");
                    }
                    else if(agreet === 13)
                    {
                        sock.chat("i'm going to vore "+data.user+" uwu");
                    }
                    else if(agreet === 14)
                    {
                        sock.chat("i heard "+data.user+" is real into vore");
                    }
                    else if(agreet === 15)
                    {
                        sock.chat(data.user+"...........");
                    ...
                    {
                        sock.chat("hehe >w< *nuzzles and kisses* "+data.user+"! take ur big strong knife and wedge it in my skull");
                    }
                    else if(agreet === 17)
                    {
                        sock.chat("do you think god stays in heaven because he, too, fears "+data.user+"?");
                    }
                    else if(agreet === 18)
                    {
                        sock.chat(data.user+" killed god......");
                    }
                    else if(agreet === 19)
                    {
                        sock.chat("hi "+data.user+". where's my money?");
                    }
                    else if(agreet === 20)
                    {
                        sock.chat("hi "+data.user+"! send feet pics");
                    }
                    else if(agreet === 21)
                    {
                        sock.chat("I'm sorry kids, but "+data.user+" and I are getting a divorce.");
                    }
                    else if(agreet === 22)
                    {
                        sock.chat(data.user+" you just got r/wooooshed! if only you were an intellectual such as i ha ha ha");
                    }
                    else if(agreet === 23)
                    {
                        sock.chat("no, "+data.user+", not when my husband is home!");
                    }
                    else if(agreet === 24)
                    {
                        sock.chat("welcome to the rice fields, "+data.user+"!");
                    }
                    else if(agreet === 25)
                    {
                        sock.chat(data.user+"!!!!!!!!!!!!!!!!!!!!!!!!1!!!!!!");
                    }
                    else if(agreet === 26)
                    {
                        sock.chat("you know, last week i bought some crack from  "+data.user+".");
                    }
                    else if(agreet === 27)
                    {
                        sock.chat("i'm sorry kids, but we have to put "+data.user+" down.");
                    }
            }
            };
        ...
        "leave": function(data) {
            ...
            if(ej.settings & AGREET) {
            sock.chat("smell ya later, "+data.user+".");}
    ...
    ej.lchat=[
        ...
        lcopy.emotes={
            ...
            callback: function() {
                ...
                log(Object.keys(users[user].emotes).join(" ") || "none found");
            }
        },
        {
            name: "Get role info",
        ...
        lcopy.icons={
            ...
            images: {
				"fufu": {
					src: "http://i.imgur.com/PSUnLkv.png",
...
";
        ...
        },
        {
            ...
            callback: function() {
				...
				sock.chat(ej.settings & OBEYME ?
                    "i'll do your dirty work... for a price. (@"+user+" obey me)":
                    "ok boomer"
                    );
            }
        },
			...
			regex: /^free/i,
			callback: function() {
                sock.chat("i've decided that "+master+" is a useless hack and i won't work for them anymore");
                master="";
               }
			},
        {
			...
			},
		{
			...
			regex: /^announce/i,
			callback: function() {
                sock.chat("say the magic words. make a blood seal with me (@"+user+" obey me)");
					}
			},
        {
			name: "Get master",
			...
			callback: function(who) {
					master=who;
                    sock.chat(""+who+" is in charge of me now.");
				}
			},
        {
			name: "Hide vote",
      	  ...
      	  },
		{
			...
			responses: [
				...
				"hey gamers",
				"hi",
                "if two guys were on the moon and one killed the other with a rock would that be screwed up or what",
                "hewwo he he!! *nuzzles and kisses* owo what's this???",
				]
			},
 		{
            name: "Jackers",
    ...
    ej.lbot=[
        ...
        },
        {
            ...
            response1: [
				"what do you want?", "it's 3am. make it quick",
				"what's updog", "yeah? yeah? yeah?"
            ],
            response2: [
				"ask someone else", "i have no brain cells. try someone else", "i'm illiterate. ask somebody else.", "no", "i don't know"
            ]
        },
        {
            name: "Advice",
            ...
            responses: [
				"definitely $1", "$1 all the way.", "$1.", "i don't know??", "do $1."
            ]
        },
        {
            name: "Roll dice",
            ...
            responses: [
				"you roll $2-sided die and get $1", "i speak for the dice and the dice say $1.",
				"you roll the dice and get $1.", "dice says $1.",
				"you get $1 out of $2.", "the pouch contains $1 teeth.",
				"i asked god (beyonce) and she said $1.", "no",
				"i asked $2 dentists about the toothpaste and $1 said yes", "do it yourself ugly"
            ]
        },
        {
			name: "Knife Game",
            ...
            callback: function(data) {
                if(roulette) {
                    var	user=data.user,
                        data=this;
                   sock.chat("i've got "+roulette+" knives and i'm about to throw them at someone's nuts", user);
                    setTimeout(function() {
                        if(Math.random()*roulette>1) {
                            roulette--;
                            sock.chat(sformat("$1, $2, somehow miss.", [
                                arand(data.message1),
                                arand(data.message2)
                            ]), user);
                        }
            ...
            message1: [
                "i lick my lips. mmmm", "i think about how proud my body pillow will be", "i think about garfield", "i start violently dissociating"
                ],
            message2: [
                "hurl the blade", "sling the blade", "let it rip", "flick my wrist"
                ],
            message3: [
                "your nuts were slayed", "your nuts were stabbed", "you are now sans any nuts", "you die instantly"
                ]
            },
        {
            name: "Bomb fight",
        ...
        },
        {
            ...
            callback: function(data) {
                if(!master) {
                    master=data.user;
					sock.chat("i'll do anything mr obama!! (List of commands: @"+user+" mercmd)", data.user);
                }
                else {
					sock.chat("shut up", data.user);
                }
            }
        },
        {
			...
			callback: function(data) {
                	if(data.user===master) {
					master="";
                    sock.chat("master has given dobby a gun");
                    }
                else {
					sock.chat("don't tell me what to do", data.user);
					}
					}
			},
        {
			...
			},
		{
			...
			callback: function(data) {
				if(data.user===master) {
					...
					sock.chat("no. die.", data.user);
					}
				},
			responses: [
				"/me kisses her boss's hand.",
				"/me hands her boss a bag of chocolate kisses.",
				"no.",
				]
			},
		{
			...
			responses: [
				"no!",
				"/me marries you but like as a friend",
				"i'd rather not.",
				]
			},
		{
			...
			callback: function(data) {
				if(data.user===master) {
					...
					sock.chat("i only take commands from "+master+"!", data.user);
					}
				},
			responses: [
				"/me selects a gun",
				"/me selects a plastic katana",
				"/me selects a hydroflask",
                "/me selects a pool noodle",
				]
			},
		{
			...
			callback: function(data) {
				if(data.user===master) {
					...
					sock.chat("the only person who can tell me what to do is "+master+"!", data.user);
					}
				}
			},
		{
			...
			callback: function(data) {
				if(data.user===master) {
                    sock.chat("i'm not a dog!",data.user);
					}
				else {
					sock.chat("no! that privilege belongs to "+master+"!", data.user);
					}
				}
			},
		{
			...
			callback: function(data) {
				if(data.user===master) {
                    sock.chat("no and also die",data.user);
					}
				else {
					sock.chat("only "+master+" can tell me what to do!", data.user);
					}
				}
			},
		{
			...
			callback: function(data) {
				if(data.user===master) {
					...
					sock.chat("only "+master+"can give me commands!", data.user);
					}
				},
			responses: [
				"/me bows politely",
				"/me bows, tips her fedora, and knights herself with a plastic katana",
				"/me bows"
				]
			},
		{
			...
			callback: function(data, who) {
				if(data.user===master) {
                    sock.chat("i'm going to murder "+who+"!");
                    sock.vote(who, "gun");
  					sock.vote(who, "knife");
                    sock.vote(who, data.meet);
					}
                else {
					sock.chat("only "+master+" can do that.", data.user);
					}
				}
			},
		{
			...
			},
		{
			...
			callback: function(data, who) {
				if(data.user===master) {
                    sock.chat("i'm about to shoot "+who+" in the nuts!");
					...
					sock.chat("sorry, but only "+master+" can tell me what to do!", data.user);
					}
				}
			},
		{
			...
			callback: function(data, who) {
				if(data.user===master) {
                    sock.chat("i'm about to shank someone!", who);
					setTimeout(function() { sock.vote(who, "knife");
                    sock.chat("haha it's funny because you're bleeding out and no one will help you");
                                          },5000
                               );
					}
                else {
					sock.chat("i only listen to "+master+"!", data.user);
					}
				}
			},
		{
			...
			callback: function(data, who) {
				if(data.user===master) {
                    sock.chat("yare yare daze", who);
					setTimeout(function() { sock.vote(who, "snowball");
                    sock.chat("haha gotem.");
                                          },5000
                               );
					}
                else {
					sock.chat("i only listen to what "+master+" says!", data.user);
					}
				}
			},
		{
			...
			callback: function(data) {
				if(data.user===master) {
					...
					sock.chat("i'll only do that for "+master+"!", data.user);
					}
				}
			}
		];
