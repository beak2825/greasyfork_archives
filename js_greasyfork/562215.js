// ==UserScript==
// @name         FF Scouter V2 SG - column fix
// @namespace    Violentmonkey Scripts
// @match        https://www.torn.com/*
// @version      2.71.5-BS-safe
// @author       rDacted, Weav3r, xentac - Custom by SuperGogu
...
// @license      GPL-3.0
// @downloadURL https://update.greasyfork.org/scripts/562215/FF%20Scouter%20V2%20SG%20-%20column%20fix.user.js
// @updateURL https://update.greasyfork.org/scripts/562215/FF%20Scouter%20V2%20SG%20-%20column%20fix.meta.js
// ==/UserScript==

const FF_VERSION = "2.71.2";
...
// ADD-ON only (păstrezi blocul tău “bază” exact cum e, și pui asta DUPĂ el)
GM_addStyle(`
/* ===== FF Scouter add-ons (Faction War members list) ===== */

.d .f-war-list.war-new .faction-war .tab-menu-cont .members-list .ffsc-bs-col,
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-cont .ffsc-bs-col
{
  padding: 0 !important;
}

.d .f-war-list.war-new .faction-war .tab-menu-cont .members-list .level,
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-cont .level
{
...
GM_addStyle(`
/* B.S. on 2 lines (ONLY the injected BS cell, not the normal Level column) */
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-list .ffsc-bs-cell.ffsc-bs-col,
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-cont .ffsc-bs-cell.ffsc-bs-col {
  display: flex !important;
...
/* top line: Est. Stats */
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-list .ffsc-bs-cell.ffsc-bs-col .ffsc-bs-top,
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-cont .ffsc-bs-cell.ffsc-bs-col .ffsc-bs-top {
...
/* bottom line: FairFight multiplier (smaller) */
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-list .ffsc-bs-cell.ffsc-bs-col .ffsc-bs-ff,
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-cont .ffsc-bs-cell.ffsc-bs-col .ffsc-bs-ff {
  display: block !important;
  font-size: 10px !important;
...
`);
    ...
    GM_addStyle(`
/* Center the "B.S." header text only */
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-list .ffsc-bs-header,
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-cont .ffsc-bs-header {
  text-align: center !important;
  justify-content: center !important; /* dacă headerul e flex */
}

/* dacă în header ai un span/text separat, îl centrează și pe acela */
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-list .ffsc-bs-header *,
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-cont .ffsc-bs-header * {
...
GM_addStyle(`
/* Status: force 50px only on RW members list */
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-list .status,
.d .f-war-list.war-new .faction-war .tab-menu-cont .members-cont .status {
...
`);
  ...
  async function process_queue() {
    ...
    // Hard minimum delay between requests to avoid empty responses / rate-limit issues
    const ffsc_min_delay = 1250;
    if (next_check < ffsc_min_delay) next_check = ffsc_min_delay;
    ...
    if (unknown_player_ids.length > 0) {
      ...
      rD_xmlhttpRequest({
        ...
        onload: function (response) {
          if (!response) {
            // Torn PDA may return nothing if requests are too close together; re-queue and try later.
            try {
              queued_player_ids.push(...unknown_player_ids);
              queued_player_ids = [...new Set(queued_player_ids)];
              setTimeout(process_queue, 1500);
            } catch (_) {
              // ignore
            }
            return;
  ...
  // --- Faction member list (ranked war) battle stats column ---
  let ffscFactionBsTimer = null;
  let ffscFactionBsLastRun = 0;

  function ffsc_parse_human_number(value) {
  ...
  function schedule_faction_bs_column() {
    if (!window.location.href.startsWith("https://www.torn.com/factions.php")) {
      return;
    }
    const now = Date.now();
    if (now - ffscFactionBsLastRun < 3000) return;

    if (ffscFactionBsTimer) return;
    ffscFactionBsTimer = setTimeout(async () => {
      ffscFactionBsTimer = null;
      ffscFactionBsLastRun = Date.now();
      try {
        await apply_faction_bs_column();
      } catch (e) {
        ffdebug("[FF Scouter V2] apply_faction_bs_column error:", e);
      }
    }, 250);
  }

  function ffsc_get_member_li_player_id(li) {
    try {
      const id = get_player_id_in_element(li);
      if (id) return id;
    } catch (_) {
      // Ignore
    }
    const a = li.querySelector('a[href*="/profiles.php?XID="],a[href*="profiles.php?XID="]');
    if (!a) return null;
    const m = a.href.match(/XID=(\d+)/);
    return m ? parseInt(m[1], 10) : null;
  }

  function ffsc_ensure_header_bs(header) {
    if (header.querySelector(".ffsc-bs-header")) return;

    const levelHeader = header.querySelector("div.level");
    const memberHeader = header.querySelector("div.member");
    if (!levelHeader || !memberHeader) return;

    // Clone the Level header to inherit Torn styling, then change its label.
    const bsHeader = levelHeader.cloneNode(true);
    bsHeader.classList.add("ffsc-bs-header", "ffsc-bs-col");

    // Preserve sort icon (if any), but reset state.
    let icon = bsHeader.querySelector("div");
    if (icon) {
      icon = icon.cloneNode(true);
      // Remove any "active" indicator classes (they're hashed, so we remove by prefix).
      icon.className = icon.className
        .split(/\s+/)
        .filter((c) => !c.startsWith("activeIcon___"))
        .join(" ");
      icon.style.transform = "";
    }

    // Rebuild content: label + existing icon.
    bsHeader.textContent = "B.S.";
    if (icon) bsHeader.appendChild(icon);

    // Insert before Level
    header.insertBefore(bsHeader, levelHeader);

    // Click-to-sort (client-side)
    bsHeader.addEventListener("click", (ev) => {
      ev.preventDefault();
      ev.stopPropagation();

      const membersCont = header.closest(".members-cont");
      const ul = membersCont ? membersCont.querySelector("ul.members-list") : null;
      if (!ul) return;

      const current = bsHeader.dataset.ffscSortDir || "none";
      const next = current === "asc" ? "desc" : "asc";
      bsHeader.dataset.ffscSortDir = next;

      // Sort items
      const items = Array.from(ul.children).filter((n) => n && n.tagName === "LI");

      items.sort((a, b) => {
        const av = parseFloat(a.dataset.ffscBsNum || "");
        const bv = parseFloat(b.dataset.ffscBsNum || "");
  ...
  function ffsc_ensure_row_bs(li) {
    if (li.querySelector(":scope > div.ffsc-bs-cell")) return;

    const levelDiv = Array.from(li.children).find((el) =>
      el.classList?.contains("level"),
    );
    if (!levelDiv) return;

    // Clone level div for styling, but override width via CSS.
    const bsDiv = levelDiv.cloneNode(false);
    bsDiv.classList.add("ffsc-bs-cell", "ffsc-bs-col");
    bsDiv.innerHTML = '<span class="ffsc-bs-top"></span><span class="ffsc-bs-ff"></span>';
    bsDiv.title = "";
    li.insertBefore(bsDiv, levelDiv);
  }

  async function ffsc_render_bs_for_container(membersCont) {
    const ul = membersCont.querySelector(":scope > ul.members-list");
    if (!ul) return;

    const lis = Array.from(ul.querySelectorAll(":scope > li"));
    const ids = [];
    for (const li of lis) {
      ffsc_ensure_row_bs(li);
      const id = ffsc_get_member_li_player_id(li);
      if (id) {
        li.dataset.ffscPlayerId = String(id);
        ids.push(id);
      }
    }
    ...
    const cached = await ffcache.get(ids);
    const unknown = [];

    for (const li of lis) {
      const id = parseInt(li.dataset.ffscPlayerId || "", 10);
      if (!id) continue;

      const cell = li.querySelector(":scope > div.ffsc-bs-cell");
      if (!cell) continue;

      const c = cached[id];
      if (!c) {
        unknown.push(id);
        const top = cell.querySelector(".ffsc-bs-top");
        const ff = cell.querySelector(".ffsc-bs-ff");
        if (top) top.textContent = "";
        if (ff) ff.textContent = "";
        cell.title = "";
        li.dataset.ffscBsNum = "";
        continue;
      }
      ...
      const human = c.bs_estimate_human ? String(c.bs_estimate_human) : "";
      const ffRaw = c.value != null ? String(c.value) : "";
      const mff = ffRaw.match(/(\d+(?:\.\d+)?)/);
      const ffNum = mff ? mff[1] : "";

      if (top) top.textContent = human;
      if (ff) ff.textContent = ffNum ? `${ffNum}×` : "";

      if (human || ffNum) {
        const t1 = human ? `Est. Stats: ${human}` : "Est. Stats: —";
        const t2 = ffNum ? `FairFight: ${ffNum}` : "FairFight: —";
        cell.title = `${t1}\n${t2}`;
      } else {
        cell.title = "";
      }

      const n = ffsc_parse_human_number(human);
      li.dataset.ffscBsNum = n != null ? String(n) : "";
    }

    // Fetch missing values (avoid immediate-callback loops by only requesting unknown ids)
    if (unknown.length > 0 && key) {
      const uniqueUnknown = [...new Set(unknown)];
      await update_ff_cache(uniqueUnknown, () => {
        ffsc_render_bs_for_container(membersCont);
      });
    }
  }

  async function apply_faction_bs_column() {
    // Ranked war member lists use .members-cont with a header (.white-grad) and list (ul.members-list)
    const membersConts = Array.from(document.querySelectorAll(".members-cont"));
    if (membersConts.length === 0) return;

    for (const cont of membersConts) {
      const header = cont.querySelector(":scope > .white-grad");
      const ul = cont.querySelector(":scope > ul.members-list");
      if (!header || !ul) continue;

      ffsc_ensure_header_bs(header);
      await ffsc_render_bs_for_container(cont);
    }
  }
  // --- /Faction member list (ranked war) battle stats column ---
