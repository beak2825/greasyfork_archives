// ==UserScript==
// @name         Dead Frontier Auto Bank Standalone with longer timeouts
// @namespace    http://tampermonkey.net/
// @version      4.8
// @author       Zega
// @description  Self-contained Auto Bank buttons with collapse/remember state (no QuickBuy dependency). Anchors under QuickBuy if present, else in the right column; graceful fallback to bottom-left if layout not found.
// @match        https://fairview.deadfrontier.com/onlinezombiemmo/index.php*
// @grant        none
// @run-at       document-idle
// @downloadURL https://update.greasyfork.org/scripts/550965/Dead%20Frontier%20Auto%20Bank%20Standalone%20with%20longer%20timeouts.user.js
// @updateURL https://update.greasyfork.org/scripts/550965/Dead%20Frontier%20Auto%20Bank%20Standalone%20with%20longer%20timeouts.meta.js
// ==/UserScript==
...
(function() {
  ...
  function buildButtons(panel) {
    ...
    if (currentPage === '35' && sessionStorage.getItem('df_auto_restore')) {
      ...
      if (inp && last) {
        ...
        setTimeout(()=> document.getElementById('makeSearch')?.click(), 200);
      }
    }
    ...
    defs.forEach(([id,label,act,amt])=>{
      ...
      btn.addEventListener('click',()=>{
        if (currentPage === '35') {
          const si = document.getElementById('searchField'); if (si) localStorage.setItem('lastDFsearch', si.value);
        }
  ...
  function injectPanel(attempt=0){
    ...
    if (!anchored) {
      ...
      return setTimeout(()=>injectPanel(attempt+1), 800);
    }
