// ==UserScript==
// @name         comfyui 生图 ios 适配
// @namespace    http://tampermonkey.net/
// @version      19.9
// @license      GPL
// @description  魔改，用于保存用户设置 (Save user settings)
// @author       99
// @match        *://*/*
// @connect      *
// @connect      127.0.0.1
...
// @grant        GM_listValues
// @downloadURL https://update.greasyfork.org/scripts/543393/comfyui%20%E7%94%9F%E5%9B%BE%20ios%20%E9%80%82%E9%85%8D.user.js
// @updateURL https://update.greasyfork.org/scripts/543393/comfyui%20%E7%94%9F%E5%9B%BE%20ios%20%E9%80%82%E9%85%8D.meta.js
// ==/UserScript==
    ...
    try {
        ...
        const response = await new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                onerror: function(error, response) {
                    button1.textContent = "生成图片";
                    xiancheng = true;
                    alert("请求错误。请检测地址" + urlObj + "是否正确，或ComfyUI已正确开启。启动器需要启用API功能（如，绘世启动器->高级选项->启用API选择开启），并且需要重启启动器才能生效。错误详情: " + error.status + " " + error.statusText);
                    reject(error);
                }
            });
        });
        ...
        while (true) {
            try {
                ...
                if (!response2 || !response2.responseText) {
                    await sleep(1000); continue;
                }
                let re;
                try {
                    re = JSON.parse(response2.responseText);
                } catch (e) {
                    await sleep(1000); continue;
                }
                console.log("response2 (parsed):", re);
                ...
                if (re.hasOwnProperty(id)) {
                    ...
                    // === 【运维工程师修复方案 V7.6 - 采用 fetch API 增强兼容性】 ===
                    let response_img;
                    try {
                        response_img = await fetch(fileurl);
                        if (!response_img.ok) { // 检查响应是否成功 (状态码 200-299)
                            throw new Error(`HTTP error! status: ${response_img.status}`);
                        }
                    } catch (error) {
                        console.error("使用 fetch 下载图片失败:", error);
                        // 抛出错误，让外层catch捕获并终止流程
                        throw new Error(`图片下载请求失败。这可能是移动端网络限制或CORS问题。错误: ${error.message}`);
                    }
                    // =======================================================
                    
                    const arrayBuffer = await response_img.arrayBuffer();
                ...
                if (ii > 200) {
                    xiancheng = true;
                    button1.textContent = "生成图片";
                    console.error('Error generating image: timeout');
                    alert("ComfyUI任务超时：未能及时获取到生成的图片信息，请检查后台是否报错。");
                    break;
                }
            } catch (error) {
                xiancheng = true;
                button1.textContent = "生成图片";
                console.error('Error in polling/history loop:', error);
                alert("轮询或下载图片时发生错误: " + error.message);
                break;
            }
        }
    } catch (error) {
        console.error('Error in main comfyui function:', error);
        alert("ComfyUI主功能发生未知错误，请检查设置。将自动解锁线程，请重试。");
        xiancheng = true;
        button1.textContent = "生成图片";
    }
}
