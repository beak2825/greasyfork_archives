...
// @namespace https://greasyfork.org/users/1020202
// @downloadURL https://update.greasyfork.org/scripts/490749/%5Bhwm%5D%5BEN%5D_change_fraction.user.js
// @updateURL https://update.greasyfork.org/scripts/490749/%5Bhwm%5D%5BEN%5D_change_fraction.meta.js
// ==/UserScript==
const redirectTo = "/inventory.php";
const depoLink = "https://www.heroeswm.ru/sklad_info.php?id=19&cat=2";
const mapWarsLink = "https://www.heroeswm.ru/map.php";
const selfLink = "https://www.heroeswm.ru/pl_info.php?id=7320599";
...
let talents = [];
let arts = ['<i>Take everything off</i>'];
let builds = [];
let spanFractions;
...
function afterSend() {
    ...
    if (isResponseSuccess()) {
        if (talentUrl) {
           ...
           // location.href = httpType + "://" + host + redirectTo;
            location.href = mapWarsLink;
        }
    }
}
...
function findSpanFractionsPlace() {
    ...
    if (!spanFractions || spanFractions == null) {
        ...
        for (let id_center in centers) {
            let center = centers[id_center].innerHTML;
    ...
    spanFractions = elementById('spanFractions');
    spanFractions.innerHTML = "<a class=pi href='castle.php'>Factions <font style='font-size:8px;'>▼</font></a>";
...
function initListFractions() {
    ...
    innerInfo += "<tr><td colspan=2><table class=wb width=100%>" +
        "<tr><td class=wblight rowspan=3 width=50% align=center>Use:</td>" +
        "<td class=" + classes[0] + "><input name=radio type=radio id=radio_0 " + checked[0] + "><label for=radio_0>Factions</label></td></tr>" +
        "<tr><td class=" + classes[1] + "><input name=radio type=radio id=radio_1 " + checked[1] + "><label for=radio_1>Talents</label></td></tr>" +
        "<tr><td class=" + classes[2] + "><input name=radio type=radio id=radio_2 " + checked[2] + "><label for=radio_2>Builds</label></td></tr>" +
        "<tr><td class=wbwhite colspan=2 align=center><input id=buildManager type=submit value='Build manager' " + (useType !== TYPE_BUILD ? "disabled" : "") + "></td></tr>" +
        "</table></td></tr>";
    innerInfo += "</table>";
    ...
    elementById('buildManager').onclick = function () {
        ...
        if (currentFractionId > 0) {
            ...
            let pattern = /\/i\/r(\d+)\.gif/;
            if (location.href === httpType + "://" + host + "/home.php") {
                ...
                send("GET", httpType + "://" + host + "/home.php", null,
                    function () {
                        if (isResponseSuccess()) {
...
function generateBuildContent(div, divPopup, fractionId) {
    let contentAll = "<table class=wb width=670 cellpadding=0 cellspacing=0 style='height:100%;background:#FFF'>" +
        ...
        "<td><b>Current faction:</b> <img width=15 height=15 border=0 align=absmiddle src='" + getFractionImg(fractionId) + "'></td>" +
    ...
    contentAll += "<tr><td style='padding:4px;' rowspan=2 width=250 valign=top>" +
        ...
        "<tr><td class=wblight style='padding:4;height:5;' colspan=4><b>Saved builds:</b></td></tr>" +
        "<tr><td class=wbwhite style='padding:4;' valign=top>" +
        "<span style='color:red'><b>No builds!</b></span>" +
        "</td></tr>" +
        "</table>" +
        "</td>";
    ...
    contentAll += "" +
        ...
        "<tr><td class=wblight style='padding:4;' colspan=7><b>Enter stack amounts:</b></td></tr>" +
        "<tr>";
    ...
    for (let mobId in mobNames[fractionId]) {
        ...
        contentValues += "<td class=wbwhite style='padding:4px;'><input type=text size=1 maxlength=3 style='border:1px solid #777' id=countArmy_" + mobId + "></td>";
    }
    ...
    contentAll += "<tr>" +
        "<td style='padding:4;'><table class=wb width=100% cellpadding=0 cellspacing=0>" +
        "<tr>" +
        "<td class=wblight style='padding:4;'><b>Choose talents:</b></td>" +
        "<td class=wblight style='padding:4;'><b>Choose art set:</b></td>" +
        "</tr>" +
    ...
    contentAll += "</td>" +
        ...
        "<br><input type=submit id=updateArts value='Refresh art sets'>" +
        "</td></tr>";
    contentAll += "</table></td></tr>";
    ...
    contentAll +=
        ...
        "<tr><td class=wblight style='padding:4;height:5;' colspan=4><b>Build export/import:</b></td></tr>" +
        "<tr align=center><td class=wbwhite><input type=submit id=exportBuilds value='Export to notepad (LWM) ->'>" +
        "<br><br><input type=submit id=importBuilds value='<- Import from notepad (LWM)'></td></tr>" +
        "</table>" +
        "</td>";
    // build
    contentAll +=
        ...
        "<tr class=wblight><td style='padding:4;' colspan=2><span id=buildCap style='color:red'><b>Build:</b></span></td></tr>" +
        "<tr class=wbwhite><td style='padding:4;' colspan=2 align=center><table width=80%>" +
        "<tr><td style='padding:2;' width=25%>Name:</td><td><input type=text maxlength=32 size=16 style='border:1px solid #777' id=buildName></td></tr>" +
        "<tr><td style='padding:2;'>Army:</td><td id=buildArmy><i>no army</i></td></tr>" +
        "<tr><td style='padding:2;'>Talents:</td><td id=buildTalent><i>no talents</i></td></tr>" +
        "<tr><td style='padding:2;'>Arts:</td><td id=buildArts><i>no arts</i></td></tr>" +
        "</table></td></tr>" +
        "<tr class=wbwhite align=center>" +
        "<td style='padding:4;' width=50%><input type=submit id=saveBuild disabled value='Save build'></td>" +
        "<td style='padding:4;' width=50%><input type=submit id=cancelBuild value='Clear form'></td>" +
        "</table></td></tr>";
    // end
    contentAll += "</table>";
...
function fillBuilds(fractionId) {
    let innerText = "<tr><td class=wblight style='padding:4px;height:5px;' colspan=4><b>Saved builds:</b></td></tr>";
