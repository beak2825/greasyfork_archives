...
// @downloadURL https://update.greasyfork.org/scripts/444734/%E9%A1%B9%E7%9B%AE%E7%BB%84%E6%8F%90%E9%97%AE%E5%B0%8F%E5%8A%A9%E6%89%8Bv10.user.js
// @updateURL https://update.greasyfork.org/scripts/444734/%E9%A1%B9%E7%9B%AE%E7%BB%84%E6%8F%90%E9%97%AE%E5%B0%8F%E5%8A%A9%E6%89%8Bv10.meta.js
// ==/UserScript==
	...
	window.MakeTags = function(type) {
		...
		if (type == "common") {
			switch ($('input[name="module"]:checked').val()) {
				case "报表":
					tags = ['finereport-11.0', 'finereport-10.0', 'finereport-7.0', 'finereport-8.0',
						'finereport-9.0', '数据源-原生方法（非插件）', '输出-导出pdf', '输出-打印&导出（除了pdf）', '决策报表-设计/制作',
				...
				case "平台":
					tags = ['finereport-11.0', 'finereport-10.0', 'finereport-7.0', 'finereport-8.0',
						'finereport-9.0', '独立模块-注册机制&注册异常', '独立模块-10.0升级工具&问题', '独立模块-公有私有云', '独立模块-web集群',
				...
				case "图表":
					tags = ['finereport-11.0', 'finereport-10.0', 'finereport-7.0', 'finereport-8.0',
						'finereport-9.0', '图表-报错', '图表-接口', '图表-难还原', '图表-需求&方案咨询', '第三方插件-图表类'
					];
					break;
				case "移动端":
					tags = ['finereport-11.0', 'finereport-10.0', 'finereport-7.0', 'finereport-8.0',
						'finereport-9.0', "移动端-fr图表", "移动端-后端&性能", "移动端-报表展现", "移动端-控件&oem平台", "移动端-集成&bi"
					];
					break;
				default:
					tags = ['finereport-10.0', 'finereport-7.0', 'finereport-8.0', 'finereport-9.0',
			...
			tags = ['finereport-11.0', 'finereport-10.0', 'finereport-7.0', 'finereport-8.0', 'finereport-9.0',
...
【附件】：尽量获取日志和内置数据集模版，使用insert link来上传文件
		...
		//这部分无论选什么都不会变
		var target =
			'<div style="display:inline"><label style="font-size:20px;color:royalblue;font-weight:bold;">请选择向谁提问</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="target" id="putong" value="普通版"</input><label for="putong" class="radio-label">普通版</label></div>';
		var module =
			'<div style="display:inline"><label style="font-size:20px;color:turquoise;font-weight:bold">请选择提问模块</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="module" id="report" checked value="报表"</input><label class="radio-label"  for="report" >报表</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="module" id="bi" value="BI"</input><label for="bi" class="radio-label">BI</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="module" id="chart" value="图表"</input><label  for="chart" class="radio-label">图表</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="module" id="mobile" value="移动端"</input><label class="radio-label"  for="mobile" >移动端</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="module" id="dec" value="平台"</input><label  for="dec" class="radio-label">平台</label></div>';
		var type =
			'<div style="display:inline"><label style="font-size:20px;color:SkyBlue;font-weight:bold">请选择问题类型</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="leixing" id="wenti"  checked value="问题"</input><label for="wenti" class="radio-label">问题</label></div><div class="radio" style="display:inline"><input type="radio" class="radio-label" name="leixing" id="xuqiu" value="需求"</input><label for="xuqiu" class="radio-label">需求(确认逻辑/不知道是需求还是BUG)</label></div>'
		...
		$('input[type="radio"][name="target"]').click(function() {
			...
			if ($(this).val() == "普通版") {
				...
				$('#question-form > div.page-title > h1').append(
					'<a  name="my-link" target="_blank" href="https://kms.fineres.com/pages/viewpage.action?pageId=89685745">&nbsp;&nbsp;&nbsp;&nbsp;高效提问,请看我</a>'
					)
				$('#question-form > div.page-title > h1').append(
					"<label name='xswl' style='color:red'>  ←这是两个超链</label>")
				//加两个超链
				MakeTags("common");
				addClick()
			} else {
		...
		if (window.location.href.indexOf("qa/questions/?page=") != -1 || window.location.href ==
			"https://kms.fineres.com/display/support/qa/questions" || window.location.href ==
			"https://kms.fineres.com/qa/questions") {
			console.log("首页")
			setTimeout(function() {
				$('#content').css("max-width", "none")
				$('#question-search').after('<button id="myButton">点我可以全文检索</button>')
				$('a.question-hyperlink[href^="/display/support/qa/questions/"]').css("font-size", "18px")
