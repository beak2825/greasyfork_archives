...
// @downloadURL https://update.greasyfork.org/scripts/553091/JavScript-iphone.user.js
// @updateURL https://update.greasyfork.org/scripts/553091/JavScript-iphone.meta.js
// ==/UserScript==

// prettier-ignore
(function () {
  const CODE_SELECTOR = "span[style='color: rgb(204, 0, 0); --darkreader-inline-color: #ff3d3d;']";
  ...
  const fadeInImg = node => {
    ...
    const addCopy = (selectors, attrs = {}) => {
      const node =
        typeof selectors === 'string' ? DOC.querySelector(selectors) : selectors
      const _attrs = {
        'data-copy': node.textContent,
        class: 'x-ml',
        href: VOID,
      }
    ...
    const handleCopy = (e, tip = '复制成功', copy) => {
      ...
      // Use navigator.clipboard for better iPhone compatibility
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard
          .writeText(copy)
          .then(() => {
            // Success callback - copy successful
          })
          .catch(err => {
            console.error('复制失败，请手动复制', err)
            // Fallback to GM_setClipboard if clipboard API fails
            if (typeof GM_setClipboard !== 'undefined') {
              GM_setClipboard(copy)
            }
          })
      } else {
        // Fallback for older browsers or when GM_setClipboard is available
        if (typeof GM_setClipboard !== 'undefined') {
          GM_setClipboard(copy)
        }
      }
  ...
  class JavDB extends Common {
      ...
      movie = {
          ...
          async contentLoaded() {
              ...
              // Copy button handled in another section to avoid duplication
...
.x-table tr > *:nth-child(3) {
    width: 100px;
}
.x-table tr > *:nth-child(4) {
    width: 80px;
}
.x-table tr > *:last-child {
    width: 90px;
}
/* Add this to ensure tfoot also displays as table */
.x-table tfoot {
    display: table;
    width: 100%;
    table-layout: fixed;
...
.x-table td a:not(:last-child) {
    margin-right: 10px;
          ...
          contentLoaded() {
              ...
              const codeNode = infoNode.querySelector(CODE_SELECTOR);
              ...
              if (this.M_COPY) {
                  // Check if copy button already exists to prevent duplication
                  if (!codeNode.nextElementSibling?.querySelector("a[data-copy]")) {
                      // Create a single copy button that copies both title and code
                      const copyText = `${code} ${title}`;
                      const container = DOC.create("span", { style: "margin-left: 10px;" });
                      const copyButton = DOC.create("a", {
                          "data-copy": copyText,
                          class: "x-ml",
                          href: VOID,
                          title: "复制番号和标题"
                      }, "复制");
                      copyButton.addEventListener("click", e => handleCopy(e, "成功"));
                      container.append(copyButton);

                      // Add the copy button after the code node
                      codeNode.after(container);
                  }
          ...
          movieTitle() {
              this.upMovieResource(
                  ...
                  () => {
                      DOC.querySelector(CODE_SELECTOR).parentElement.insertAdjacentHTML(
                          "beforebegin",
              ...
              table.parentElement.innerHTML = `
                <table class="table table-striped table-hover table-bordered x-table">
                    ...
                    <thead>
                        <tr>
                            <th scope="col">名称</th>
                            <th scope="col" class="text-center">大小</th>
