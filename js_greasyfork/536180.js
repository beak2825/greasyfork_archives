...
// @match        *://grok.com/*
// @version 0.0.1.20251215185344
// @namespace https://greasyfork.org/users/1435046
// @downloadURL https://update.greasyfork.org/scripts/536180/grok%20Paste.user.js
// @updateURL https://update.greasyfork.org/scripts/536180/grok%20Paste.meta.js
// ==/UserScript==
...
(function () {
  ...
  window.addEventListener("message", event => {
    ...
    let chatMessageInput = document.querySelector('div:has(> div > div > div > form > div > div > div > div > div > div[contenteditable].tiptap.ProseMirror)');
    let chatMessageInputRule = 'div:has(> div > div > div > form > div > div > div > div > div > div[contenteditable].tiptap.ProseMirror) {display: none !important;}';
    ...
    if (event.data?.type === 'defaultChatMessageInputDisplay') {
      ...
      if (chatMessageInput) {
        ...
        grokCssStyleId.innerHTML = grokCssStyleId.innerHTML.replace(`${headerCssRule}`, '');
    ...
    if (event.data?.type === 'customizeChatMessageInputDisplay') {
      ...
      if (chatMessageInput) {
        ...
        grokCssStyleId.innerHTML += `${headerCssRule}`
