...
// @downloadURL https://update.greasyfork.org/scripts/533067/Fullchan%20X.user.js
// @updateURL https://update.greasyfork.org/scripts/533067/Fullchan%20X.meta.js
// ==/UserScript==
...
class fullChanX extends HTMLElement {
  ...
  async init() {
    ...
    this.headerMenuEl = document.querySelector('fullchan-x-header-menu');
...
class FullChanXHeaderMenu extends HTMLElement {
  constructor() {
    super();
  }
  ...
  init() {
    ...
    this.menuContainer = this.querySelector('.fcxm__menu-container');
    this.menuButtonsContainer = this.querySelector('.fcxm__menu-tabs');
  ...
  listeners() {
    ...
    if (this.menuTrigger) {
      let hideTimeout; // Declare it here

      const activate = () => {
        clearTimeout(hideTimeout);
        this.classList.add('fcx-active');
        this.menuTrigger.classList.add('fcx-active');
      };

      const scheduleDeactivate = () => {
        hideTimeout = setTimeout(() => {
          this.classList.remove('fcx-active');
          this.menuTrigger.classList.remove('fcx-active');
        }, 300);
      };

      [this.menuTrigger, this].forEach(el => {
        console.log('el', el)
...
window.customElements.define('fullchan-x-header-menu', FullChanXHeaderMenu);
