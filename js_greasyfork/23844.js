...
// @downloadURL https://update.greasyfork.org/scripts/23844/%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/23844/%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
...
(function (){
    ...
    var supermanMenuBtns=(function(){
        ...
        html+='<div id="supermanMenu" style="display:none;position:fixed;z-index:999999;">';
        ...
        for(var i=0;i<menuTitleArr.length;i++){
            html+='<li><a href="javascript:;" class="supermanMenuBtn" type="'+i+'"><b>'+menuTitleArr[i]+'</b></a></li>';
        }
        ...
        for (var i = 0; i < panBtnsArr.length; i++) {
            ...
            createsupermanBtn(item);
        }
        function createsupermanBtn(btn) {
            var newnode=btn.cloneNode(true),html=newnode.innerHTML;
            $(newnode).attr('id','').attr('href','javascript:void(0)').attr('data-key','downloadsuperman').attr('node-type','btn-superman').attr('onclick','').css({width:63}).html(html.replace(/[\u4E00-\u9FA5]{2,4}(\(.*\)|\uff08.*\uff09)?/,'\u7f51\u76d8\u52a9\u624b')).unbind();
            var o=$('<div class="supermanBtn" style="display:inline-block;">').append(newnode)[0];
            btn.parentNode.insertBefore(o, btn.nextSibling);
            return o;
        }
        var supermanBtn = $('.supermanBtn'),supermanMenu = $('#supermanMenu'),
        menuFun = function() {
            supermanDownload($(this).attr('type') || 0);
            supermanMenu.hide();
        };
        supermanBtn.click(menuFun).mouseenter(function() {
            $(this).addClass('b-img-over');
            var o=$(this).children('a'),offset=o.offset(),w=o.outerWidth()-parseInt(o.css('paddingRight'));
            supermanMenu.children('ul').css('width', w-2);
            supermanMenu.css('top', offset.top + o.height() + parseInt(o.css('paddingTop')) - $(document).scrollTop());
            supermanMenu.css('left', offset.left).show();
        }).mouseleave(function() {
        ...
        supermanMenu.mouseenter(function() {
            $(this).show();
        }).mouseleave(function() {
            $(this).hide();
        });
        supermanMenu.find('a').css('text-align', 'center');
        return supermanMenu.find('a.supermanMenuBtn').click(menuFun).toArray();
    })();
    if(!supermanMenuBtns.length){return;}
    function supermanDownload(type){
        ...
        if(1 == len) {
            ...
            if(isUrl(url)) {
                if(2 == type) {
                    showsupermanDialog(type, items, {"errno": 0,"dlink": url});
                }else if(1 == type){
                    copyText(url);
                }else{
                    myToast(msg[3],1);
    ...
    function getDownloadInfo(type, items, vcode) {
        if(!vcode) {
            showsupermanDialog(supermanMenuBtns.length+1, items);
            vcode = {};
        }
        ...
        httpHwnd = $.post(url, data,
            function(o) {
                ...
                if(-20 === o.errno){
                    ...
                    showsupermanDialog(type, items, o, vcode);
                }else{
                    showsupermanDialog(type, items, o, vcode);
                }
            });
    }
    function showsupermanDialog(type, items, opt, vcode) {
        var canvas =document.canvas ? document.canvas : Canvas ? new Canvas() : new disk.ui.Canvas(),
        _ = document.supermandialog || createsupermanDialog(),isVisible = _.isVisible(),status=0;
        document.canvas = canvas;
        _.canvas = canvas;
        _.type = type;
        _.items = items;
        if (type < supermanMenuBtns.length) {
            if (0 === opt.errno) {
                status=1;
                if(type < 2) {
    ...
    function createsupermanDialog() {
        ...
        _._mUI.pane = o[0];
        _.loading = o.find('#supermanloading')[0];
        ...
        o.find('#supermandialogclose').click(dialogClose);
        o.find('.dbtn').click(dialogClose);
        _.setVisible(false);
        document.supermandialog = _;
        return _;
    }
    ...
    function createDownloadIframe(){
        ...
        var o = $('#supermandownloadiframe');
        iframe=o.length ? o[0] : '';
        if(!iframe) {
            iframe = $('<div style="display:none;">').html('<iframe src="" id="supermandownloadiframe" name="supermandownloadiframe"></iframe>').appendTo(document.body).find('#supermandownloadiframe')[0];
        }
