// ==UserScript==
// @name         🔥🔥起点中文网VIP内容免费解析，支持章节下载🔥🔥
// @namespace    https://www.softrr.cn/
// @version      2.0.2
// @author       hackhase
// @description  起点中文网VIP内容免费解析，支持VIP章节，支持本章节内容直接下载成txt文件，另外不是所有书籍都能解析，介意的绕过，有问题可以反馈！
// @license      MIT
...
// @grant        unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/486071/%F0%9F%94%A5%F0%9F%94%A5%E8%B5%B7%E7%82%B9%E4%B8%AD%E6%96%87%E7%BD%91VIP%E5%86%85%E5%AE%B9%E5%85%8D%E8%B4%B9%E8%A7%A3%E6%9E%90%EF%BC%8C%E6%94%AF%E6%8C%81%E7%AB%A0%E8%8A%82%E4%B8%8B%E8%BD%BD%F0%9F%94%A5%F0%9F%94%A5.user.js
// @updateURL https://update.greasyfork.org/scripts/486071/%F0%9F%94%A5%F0%9F%94%A5%E8%B5%B7%E7%82%B9%E4%B8%AD%E6%96%87%E7%BD%91VIP%E5%86%85%E5%AE%B9%E5%85%8D%E8%B4%B9%E8%A7%A3%E6%9E%90%EF%BC%8C%E6%94%AF%E6%8C%81%E7%AB%A0%E8%8A%82%E4%B8%8B%E8%BD%BD%F0%9F%94%A5%F0%9F%94%A5.meta.js
// ==/UserScript==

(e=>{if(typeof GM_addStyle=="function"){GM_addStyle(e);return}const o=document.createElement("style");o.textContent=e,document.head.append(o)})(" :root{font-family:Inter,Avenir,Helvetica,Arial,sans-serif;font-size:16px;line-height:24px;font-weight:400;color-scheme:light dark;color:#ffffffde;background-color:#242424;font-synthesis:none;text-rendering:optimizeLegibility;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;-webkit-text-size-adjust:100%}a{font-weight:500;color:#646cff;text-decoration:inherit}a:hover{color:#535bf2}body{margin:0;place-items:center;min-width:320px;min-height:100vh}h1{font-size:3.2em;line-height:1.1}button{border-radius:8px;border:1px solid transparent;padding:.6em 1.2em;font-size:1em;font-weight:500;font-family:inherit;background-color:#1a1a1a;cursor:pointer;transition:border-color .25s}button:hover{border-color:#646cff}button:focus,button:focus-visible{outline:4px auto -webkit-focus-ring-color}.card{padding:2em}@media (prefers-color-scheme: light){:root{color:#213547;background-color:#fff}a:hover{color:#747bff}button{background-color:#f9f9f9}}.modal-wrapper[data-v-033b84f8]{position:fixed;top:0;left:0;width:100%;height:100%;background-color:#00000080;display:flex;justify-content:center;align-items:center;z-index:9999}.modal[data-v-033b84f8]{background-color:#fff;padding:20px;border-radius:5px}.header[data-v-033b84f8]{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px}.header h2[data-v-033b84f8]{margin:0;font-size:20px;font-weight:700}.header button[data-v-033b84f8]{border:none;background-color:transparent;font-size:20px;cursor:pointer}.content[data-v-033b84f8]{max-height:400px;overflow:auto;font-size:16px;display:flex;justify-content:space-between}.content .produce p[data-v-033b84f8]{margin-top:15px}.content .produce .ipt[data-v-033b84f8]{border:1px solid #ccc;margin-top:15px;width:200px;height:30px;border-radius:5px;padding-left:10px}.content .img[data-v-033b84f8]{display:flex;align-items:center;justify-content:center}.content .img img[data-v-033b84f8]{width:180px}input[data-v-033b84f8]::-webkit-input-placeholder{color:#aab2bd;font-size:14px;padding-left:5px}.copy[data-v-ea6defee]{width:160px;position:fixed;right:10px;top:80px;color:#111;z-index:999;display:flex;flex-direction:column}.copy .prase[data-v-ea6defee],.copy .download[data-v-ea6defee]{width:80px;height:80px;background-color:red;color:#fff;border-radius:50%}.copy .prase[data-v-ea6defee]:hover{background-color:#87ceeb;color:#fff}.copy .download[data-v-ea6defee]{background-color:#171602}.copy .download[data-v-ea6defee]:hover{background-color:#a087eb;color:#fff} ");
...
(function (vue) {
  ...
  const _withScopeId = (n) => (vue.pushScopeId("data-v-033b84f8"), n = n(), vue.popScopeId(), n);
  ...
  const Model = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-033b84f8"]]);
  ...
  let baseUrl = "https://api.softrr.cn";
  const getBiQuGe = (url) => {
        ...
        onload: function(res) {
          if (res.status == "403") {
            resolve(JSON.stringify({
              code: 403,
              info: "超出接口限制，有需要的请到公众号：软件珍藏室，后台加微信"
            }));
          } else {
  ...
  const getBookName = () => {
    ...
    if (baseUrl2.includes("book")) {
      var bookName = document.querySelector("#bookName").innerText.trim();
    } else if (baseUrl2.includes("chapter")) {
      var bookName = document.querySelector("#r-breadcrumbs").children[6].innerText.trim();
    }
    var chartName = document.querySelector(".title").innerHTML.split("<")[0].trim();
    var authortName = document.querySelector(".mt-4px").innerText.split("\n")[1].trim();
    return [bookName, chartName, authortName];
  };
  const getBiGuGeContent = async (QdCode) => {
    let bookInfo = getBookName();
    let contentdata = "";
    let bookUrl = `${baseUrl}/api/biquge5200?book_name=${bookInfo[0]}&book_author=${bookInfo[2]}`;
    let response = await getBiQuGe(bookUrl);
    if (JSON.parse(response).code == 503) {
      contentdata = 503;
    } else {
      let result = JSON.parse(response).data;
      const chartRes = [];
      for (var i = 0; i < result.length; i++) {
        if (result[i][0] == bookInfo[1]) {
          chartRes.push(result[i]);
        }
      }
      let chartUrl = chartRes[0][1];
      let contentUrl = `${baseUrl}/api/biquge5200/chapter?content_url=${chartUrl}&code=${QdCode}`;
      let responseContent = await getBiQuGe(contentUrl);
      if (JSON.parse(responseContent).code == 403) {
        contentdata = "403";
      } else if (JSON.parse(responseContent).code == 503) {
        contentdata = "503";
      } else {
        contentdata = JSON.parse(responseContent).data;
      }
    }
    return contentdata;
  };
  const getXbotaodzContent = async (QdCode) => {
    let bookInfo = getBookName();
    alert(QdCode);
    let contentdata = "";
    let bookUrl = `${baseUrl}/api/xbotaodz?book_name=${bookInfo[0]}&book_author=${bookInfo[2]}&code=${QdCode}`;
    let response = await getBiQuGe(bookUrl);
    if (JSON.parse(response).code == 503) {
      contentdata = "403";
    } else {
      ...
      let chartUrl = chartRes[0][1];
      let contentUrl = `${baseUrl}/api/xbotaodz/chapter?content_url=${chartUrl}&code=${QdCode}`;
      let responseContent = await getBiQuGe(contentUrl);
      if (JSON.parse(responseContent).code == 403) {
        contentdata = "403";
      } else if (JSON.parse(responseContent).code == 503) {
    ...
    setup(__props) {
      ...
      const onPrase = async () => {
        ...
        if (locaCode == code.value) {
          let QdCode = localStorage.getItem("QdCode") ? localStorage.getItem("QdCode") : "1234";
          let content = await getBiGuGeContent(QdCode);
          if (content == "403") {
            content = "超出每日解析接口限制，有需要的请到公众号：软件珍藏室，后台加微信";
          } else if (content == "503") {
            content = await getXbotaodzContent(QdCode);
            if (content == "403") {
              content = "超出每日解析接口限制，有需要的请到公众号：软件珍藏室，后台加微信";
            } else if (content == "503") {
              content = "无法解析！";
            } else {
              content = content + '</br><a href="https://www.softrr.cn/" target="_blank">有问题请到软件珍藏室后台反映</a>';
            }
          } else {
            content = content + '</br><a href="https://www.softrr.cn/" target="_blank">有问题请到软件珍藏室后台反映</a>';
          }
      ...
      vue.onMounted(() => {
        if (document.querySelector(".mt-16px")) {
          onPrase();
        }
      });
      return (_ctx, _cache) => {
  ...
  const App = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-ea6defee"]]);
