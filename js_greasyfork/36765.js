// ==UserScript==
// @name         Bilibili-优化ASMR直播体验
// @namespace    mscststs
// @version      1.60
// @description  听asmr自动领瓜子
// @author       mscststs
// @include        /^https?:\/\/live\.bilibili\.com\/\d/
// @include        /^https?:\/\/api\.live\.bilibili\.com\/freeSilver\/getCaptcha.*?/
// @grant        none
// @license            MIT License
// @downloadURL https://update.greasyfork.org/scripts/36765/Bilibili-%E4%BC%98%E5%8C%96ASMR%E7%9B%B4%E6%92%AD%E4%BD%93%E9%AA%8C.user.js
// @updateURL https://update.greasyfork.org/scripts/36765/Bilibili-%E4%BC%98%E5%8C%96ASMR%E7%9B%B4%E6%92%AD%E4%BD%93%E9%AA%8C.meta.js
// ==/UserScript==
 ...
 if((window.location.href+"").indexOf("getCaptcha")>10){
    ...
    $().ready(function(){
        ...
        function init(){
            ...
            if(window.localStorage["helper_help"]!="1.54"){
                $("body").append('<style>#helper_help{width:100%;height:100%;top:0;left:0;position:fixed;z-index:999999;background-color:rgba(0,0,0,.4);overflow:hidden;word-break:break-all}.helper_lisences{background-color:#555;box-shadow:0 0 15px #111;margin-left:20%;margin-top:10px;width:60%;font-size:18px;color:#fff;border-radius:5px;border:1px solid #fff;padding:5px 10px}</style><div id="helper_help"><div class="helper_lisences"><p><em>当你看到该提示，说明你的脚本是第一次启动或在最近发生了更新，请阅读以下说明 (5秒后可点击空白处关闭)</em><a  href="https://greasyfork.org/zh-CN/scripts/31994" style="color:#7f7">点此查看详细介绍</a></p><h4>使用须知：</h4><ul><li>请勿宣传，闷声发财</li><li>使用本脚本而导致的一切后果由你本人承担</li><li>MIT协议，在任何情况下对本脚本进行二次开发均需在UI中注明版权</li><li>作者有权决定何时何种情况对脚本进行处理，包括但不限于：<em>官方与作者进行了交涉</em> 等</li></ul><h4>功能简介：</h4><ul><li>单击进度条白色部分：查看宝箱领取进度</li><li>双击进度条灰色部分：查看低保领取情况</li><li>右键单击进度条，拖动绿色提示：更改脚本提示信息的位置</li></ul><h4>注意事项：（重要）</h4><ul><li>兼容：FireFox/Chrome + Tampermonkey 。其他兼容性问题概不负责</li><li>已知与助手不兼容</li><li>仅保留最后一个直播间的脚本生效，回到原直播间的话，请刷新</li><li>进度条显示在视频下方，自动领瓜子开始时将自动隐藏宝箱</li><li>领低保功能存在延迟（20秒），有失败的可能性，非脚本问题</li><li>长时间挂机有可能导致网站检测离线，脚本工作不正常，非脚本问题</li><li><a href="https://greasyfork.org/zh-CN/scripts/31994" style="color:#7f7">如果你想支持我，请点击此处捐赠（支付鸨）</a></li></ul><h4>      鸣谢：</h4><ul><li>Greasy Fork 网站提供的脚本托管服务</li></ul></div></div>');
                setTimeout(function(){
                    window.localStorage["helper_help"] = "1.54";
