...
// @match        https://lemonhd.club/userdetails.php?id=*
// @icon         https://ourbits.club//favicon.ico
// @grant        GM_addElement
// @grant        GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/515795/%E5%AE%98%E7%A7%8D%E4%BF%9D%E7%A7%8D%E7%BB%9F%E8%AE%A1.user.js
// @updateURL https://update.greasyfork.org/scripts/515795/%E5%AE%98%E7%A7%8D%E4%BF%9D%E7%A7%8D%E7%BB%9F%E8%AE%A1.meta.js
// ==/UserScript==
...
var config = [
    ...
    },
    {
        ...
        abbrev: "SSD",
        seedList: "#ka1 >  table > tbody > tr > td:nth-child(2) > div > a",
        seedListSize: "#ka1 >  table > tbody > tr > td:nth-child(3)",
        seedListSeederCount: "#ka1 > table > tbody > tr > td:nth-child(4)",
        seedingSummary: "#ka1 > b",
        siteRegex: /[@-]\s?(CMCT|Oldboys|XY|iFree|RO|CatEDU|7³ACG|Anonymous|Telesto|GTR|Lislander|Crazyleikm|NOVOLAND)/i,
        seederLevels: [
            { seederNum: 3, seederLevelCount: 0, seederLevelSize: 0 },
            { seederNum: 5, seederLevelCount: 0, seederLevelSize: 0 },
            { seederNum: 7, seederLevelCount: 0, seederLevelSize: 0 },
    ...
    },
    {
        ...
        groups: [
            ...
            {
                groupName: 'RLWeb',
                groupRegex: /[@-]\s?(RLWeb)\b/i,
                groupCount: 0,
                groupSize: 0,
            },
            {
                groupName: 'RLeaves',
                groupRegex: /[@-]\s?(RLeaves)\b/i,
                groupCount: 0,
                groupSize: 0,
            },
        ],
    ...
    },
    {
        ...
        groups: [
            {
                groupName: 'LHD',
                groupRegex: /[@-]\s?(LHD)\b/i,
                groupCount: 0,
                groupSize: 0,
            },
            {
                groupName: 'LeagueHD',
                groupRegex: /[@-]\s?(LeagueHD)\b/i,
                groupCount: 0,
                groupSize: 0,
            },
            {
                groupName: 'LeagueNF',
                groupRegex: /[@-]\s?(LeagueNF)\b/i,
                groupCount: 0,
                groupSize: 0,
            },
            {
                groupName: 'LeagueTV',
                groupRegex: /[@-]\s?(LeagueTV)\b/i,
                groupCount: 0,
                groupSize: 0,
            },
            {
                groupName: 'LeagueCD',
                groupRegex: /[@-]\s?(LeagueCD)\b/i,
                groupCount: 0,
                groupSize: 0,
            },
            {
                groupName: 'LeagueWEB',
                groupRegex: /[@-]\s?(LeagueWEB)\b/i,
                groupCount: 0,
                groupSize: 0,
            }],
        seederLevels: [
...
var parseSeedPage = (html, theConfig, seedColor, eleSeedList, eleTorItem, eleTorSize, eleTorSeedNum) => {
    ...
    for (var i = 0; i < torlist.length; i++) {
        ...
        if (theConfig.useTitle) {
            torName = item.attr('title');
            if (!torName) torName = $(element).find("div.torrent-title > span > span > a").text();
        }
...
var rlGetNextPage = async (nextLink) => {
    ...
    nextPageLink = nextPageLink + "&userid=" + userId + "&type=seeding";
...
var rlHasNextPageLink = (html, RL_PAGE) => {
    ...
    console.log($(currentPageEle).text().trim());
    if ($(currentPageEle).text().trim() === '下一页 >>') {
        var link = $(currentPageEle).prop("href");
        return link ? link : '';
    } else {
        return '';
    }
...
var fetchRLSeedPages = async (seedHtml, theConfig) => {
    var RL_PAGE = '#ka1 > p > a:nth-child(2)';
    ...
    if (fullLinkEle.length > 0) {
        ...
        while (bNext) {
            ...
            if (nextlink) {
                if (count % 8 == 0) await sleep(3000);
                let nextpage = await rlGetNextPage(nextlink);
                if (nextpage) {
                    nextpage = '<div id="ka1">' + nextpage + '</div>';
                    page = new DOMParser().parseFromString(nextpage, 'text/html');
                }
                intpage += 1;
            } else {
                bNext = false;
...
var lhdGetNextPage = async (nextLink) => {
    // var currentPageEle = $(html).find(SSD_SEED_PAGE).last();
    var nextPageLink = nextLink;
    console.log(nextPageLink);
    // $.get(nextPageLink).done(e => getPageList(e))
...
var lhdHasNextPageLink = (html, LHD_PAGE) => {
    var currentPageEles = $(html).find(LHD_PAGE);
    if (!currentPageEles) return '';
    var currentPageEle = currentPageEles[0];
    ...
    if ($(currentPageEle).text().trim() === '[下一页]') {
        var link = $(currentPageEle).prop("href");
        if(!link) return '';
        var match = link.match(/getusertorrentlistajax\('(\d+)', '([^']+)', '([^']+)', '(\d+)'\)/);
        ...
        if (match) {
            ...
            // 构造新的 URL
            var newUrl = `https://lemonhd.club/getusertorrentlistajax.php?userid=${userId}&type=${type}&blockid=${blockId}&page=${page}`;
...
var fetchLHDSeedPages = async (seedHtml, theConfig) => {
    var LHD_PAGE = '#ka1 > div.pages > :nth-child(2)';
    var fullLinkEle = $(seedHtml).find(LHD_PAGE);
    if (fullLinkEle.length > 0) {
        ...
        while (bNext) {
            //debugger;
            parseOneSeedPage(page, theConfig);
            var nextlink = lhdHasNextPageLink(page, LHD_PAGE);
            count++;
            if (nextlink) {
                if (count % 8 == 0) await sleep(3000);
                let nextpage = await lhdGetNextPage(nextlink);
                if (nextpage) {
                    nextpage = '<div id="ka1">' + nextpage + '</div>';
                    page = new DOMParser().parseFromString(nextpage, 'text/html');
                }
        ...
        console.log(curTorrentIds);
        console.log(realTorrentIds);
        return true;
    }
    else return false;
}
...
function parseOneSeedPage(seedHtml, theConfig) {
    ...
    for (var i = 0; i < seedList.length; i++) {
        ...
        var href = seedList[i].href;
        const match = href.match(/id=(\d+)/);
        const id = match ? match[1] : null;
        if(!id)
            continue;

        curTorrentIds.push(id);

        if(realTorrentIds.some(a=> a === id)){
            continue;
        }
        realTorrentIds.push(id);
...
async function getSeedList(seedHtml, theConfig) {
    ...
    else if (theConfig.host == "lemonhd.club") {
        sumaryShown = await fetchLHDSeedPages(seedHtml, theConfig)
        if (sumaryShown) {
            colorSeed(seedHtml, theConfig)
            return;
        }
