// ==UserScript==
// @name         MWITools(modified)
// @namespace    http://tampermonkey.net/
// @version      22.9.0
// @description  Modified version of MWITools with additional features/fixes.
// @author       Original: bot7420 | Modified feilong101
...
// @downloadURL https://update.greasyfork.org/scripts/536352/MWITools%28modified%29.user.js
// @updateURL https://update.greasyfork.org/scripts/536352/MWITools%28modified%29.meta.js
// ==/UserScript==
...
(() => {
    ...
    const MARKET_API_URL_PROXY = "https://ghproxy.net/https://raw.githubusercontent.com/holychikenz/MWIApi/main/milkyapi.json"; // For players with difficulties to accesss Github
            ...
            invElem.insertAdjacentHTML(
                ...
                `<div style="text-align: left; color: ${SCRIPT_COLOR_MAIN}; font-size: 14px;">
                    ...
                    <div style="font-weight: bold;">
                        ${isZH ? "+ 库存价值：" : "+ Inventory value: "}${numberFormatter(inventoryNetworthAsk)}
                    </div>
                    <!-- 总NetWorth -->
    ...
    async function handleTooltipItem(tooltip) {
        ...
        if (settingsMap.itemTooltip_prices.isTrue) {
            ...
            vendor = marketJson?.market[itemName]?.vendor;
            appendHTMLStr += `
        ...
        if (
            ...
            const askAfterTax = ask * 0.98;

            // 每小时利润
            const profitPerHour_ask_in_bid_out =
            ...
            const profitPerHour_bid_in_ask_out =
                  itemPerHour * (askAfterTax - totalResourcesBidPricePerAction / droprate) +
                  extraFreeItemPerHour * bidAfterTax -
                  drinksConsumedPerHourAskPrice;

            const profitPerHour_bid_in_bid_out =
                  itemPerHour * (bidAfterTax - totalResourcesBidPricePerAction / droprate) +
                  extraFreeItemPerHour * bidAfterTax -
                  drinksConsumedPerHourAskPrice;

            const profitPerHour_ask_in_ask_out =
                  itemPerHour * (askAfterTax - totalResourcesAskPricePerAction / droprate) +
                  extraFreeItemPerHour * askAfterTax -
                  drinksConsumedPerHourAskPrice;


            const totalResourceAsk_for_positive_profit = itemPerHour*bidAfterTax + extraFreeItemPerHour * bidAfterTax - drinksConsumedPerHourAskPrice;

            let positive_resource_ask_per_action = totalResourceAsk_for_positive_profit/actionPerHour

            let discount_required = (positive_resource_ask_per_action-upgradedFromItemAsk) / ((totalResourcesAskPricePerAction-upgradedFromItemAsk) / droprate)
            if (discount_required > 1.0){
                discount_required = 1.0
                positive_resource_ask_per_action = totalResourcesAskPricePerAction
            }
            if (positive_resource_ask_per_action<0){
                discount_required = Infinity
                positive_resource_ask_per_action=Infinity
            }
            ...
            if (isProduction) {
                                ...
                                <div style="color: ${SCRIPT_COLOR_TOOLTIP}; font-size: 10px;">
                                    <table style="width:100%; border-collapse: collapse;">
                                        <tr style="border-bottom: 1px solid ${SCRIPT_COLOR_TOOLTIP};">
                                            ...
                                            <th style="text-align: right;">${isZH ? "保本价" : "discount"}</th>
                                        </tr>
                                        <tr style="border-bottom: 1px solid ${SCRIPT_COLOR_TOOLTIP};">
                                            ...
                                            <td style="text-align: right;"><b>${numberFormatter(positive_resource_ask_per_action)}</b></td>
                                        </tr>`;
                ...
                for (const item of inputItems) {
                    appendHTMLStr += `
                                        <tr>
                                            ...
                                            <td style="text-align: right;">${numberFormatter(item.perAskPrice*discount_required)}</td>
                                        </tr>`;
                }
                ...
                appendHTMLStr += `<div style="color: ${SCRIPT_COLOR_TOOLTIP};">${isZH ? "正利润需求: " : "Profit: "}${numberFormatter(
                positive_resource_ask_per_action
                )}, ${isZH ? "需求数量: " : "vendor: "}${numberFormatter(vendor)}</div>`;
                appendHTMLStr += `<div style="color: ${SCRIPT_COLOR_TOOLTIP};">${isZH ? "基础物品 ask: " : "Base item ask: "}${numberFormatter(
                upgradedFromItemAsk
                )}, ${isZH ? "基础物品 bid: " : "Base item bid: "}${numberFormatter(upgradedFromItemBid)}</div>`;
            ...
            appendHTMLStr += `<div style="color: ${SCRIPT_COLOR_TOOLTIP};">${isZH ? "左进右出利润: " : "Profit: "}${numberFormatter(
                profitPerHour_ask_in_bid_out / actionPerHour
            )}${isZH ? "/动作" : "/action"}, ${numberFormatter(profitPerHour_ask_in_bid_out)}${isZH ? "/小时" : "/hour"}, ${numberFormatter(24 * profitPerHour_ask_in_bid_out)}${
                isZH ? "/天" : "/day"
            }</div>`;

            appendHTMLStr += `<div style="color: ${SCRIPT_COLOR_TOOLTIP};">${isZH ? "右进左出利润: " : "Profit: "}${numberFormatter(
                profitPerHour_bid_in_ask_out / actionPerHour
            )}${isZH ? "/动作" : "/action"}, ${numberFormatter(profitPerHour_bid_in_ask_out)}${isZH ? "/小时" : "/hour"}, ${numberFormatter(24 * profitPerHour_bid_in_ask_out)}${
                isZH ? "/天" : "/day"
            }</div>`;

            appendHTMLStr += `<div style="color: ${SCRIPT_COLOR_TOOLTIP};">${isZH ? "右进右出利润: " : "Profit: "}${numberFormatter(
                profitPerHour_bid_in_bid_out / actionPerHour
            )}${isZH ? "/动作" : "/action"}, ${numberFormatter(profitPerHour_bid_in_bid_out)}${isZH ? "/小时" : "/hour"}, ${numberFormatter(24 * profitPerHour_bid_in_bid_out)}${
                isZH ? "/天" : "/day"
            }</div>`;

            appendHTMLStr += `<div style="color: ${SCRIPT_COLOR_TOOLTIP};">${isZH ? "左进左出利润: " : "Profit: "}${numberFormatter(
                profitPerHour_ask_in_ask_out / actionPerHour
            )}${isZH ? "/动作" : "/action"}, ${numberFormatter(profitPerHour_ask_in_ask_out)}${isZH ? "/小时" : "/hour"}, ${numberFormatter(24 * profitPerHour_ask_in_ask_out)}${
                isZH ? "/天" : "/day"
            }</div>`;
