...
// @downloadURL https://update.greasyfork.org/scripts/39485/Harem%20Heroes%2B%2B%20modified%20by%201121.user.js
// @updateURL https://update.greasyfork.org/scripts/39485/Harem%20Heroes%2B%2B%20modified%20by%201121.meta.js
// ==/UserScript==
...
texts.fr = {
    ...
    evolution_costs:"Ses coūts d'évolution sont",
    world:"Monde ",
    villain:" troll",
    ...
    remainingBattles: " Combats restants à faire",
    remainingAttempts: " Points de combats gratuits"
};
texts.es = {
    navigate:"Navegar",
    current:"actual",
    ...
    remainingBattles: " Remaining fights to be done",
    remainingAttempts: " Remaining free fighting points"
};
...
$('#contains_all > header').children('[type=energy_fight]').append('<div class="energy_counter"><span class="hudScript_mix_icn"></span><div class="bar-wrapper">	<div id="scriptLeagueBar" class="bar red"></div><div class="over"><span id="scriptLeagueAttempts">0</span>/<span rel="max">15</span><span id="scriptLeagueTimer" rel="count_txt" timeforsinglepoint="2100" timeonload="24"></span></div></div></div>');

var leaguetime = Hero.infos.recharge_timers.calculation_timestamp + Hero.infos.recharge_timers.challenge_recharge_time*1000;
// verify localstorage
...
else if (CurrentPage.indexOf('arena') != -1) arena();				// Current page: arena
else if (CurrentPage.indexOf('home') != -1) home();					// Current page: Homepage
else if (CurrentPage.indexOf('log_in') != -1) home();					// Current page: Homepage
else if (CurrentPage.indexOf('activities') != -1) activities();		// Current page: missions
else if (CurrentPage.indexOf('tower-of-fame') != -1) leagues();		// Current page: tower of fame
else if (CurrentPage.indexOf('pachinko') != -1) pachinko();		// Current page: pachinko
eventTrolls = JSON.parse(localStorage.getItem("eventTrolls"));
missionTime = localStorage.getItem("missionTime");
arenaTime = localStorage.getItem("arenaTime");
...
scriptTimers();
$('#contains_all > header').children('[type=energy_quest]').append('<div class="scriptInfo" id="FightArena"><a href="/arena.html">Arena: <span id="atime">0m 0s</span></a></div>');
$('#contains_all > header').children('[type=xp]').append('<div class="scriptInfo" id="FightQuest"><a href="/activities.html">Mission: <span id="mtime">0m 0s</span></a></div>');

$('#contains_all > header').children('.currency:first').append('<div><a class="scriptquest round_blue_button continue_quest_home" href="' + Hero.infos.questing.current_url + '" hh_title="Current quest" tooltip-id="tooltip_2"><span class="townForward_flat_icn"></span></a></div>');



function scriptTimers(){
    $('#atime').text(calculateTime(arenaTime));
    $('#mtime').text(calculateTime(missionTime));
    setInterval(function(){$('#atime').text(calculateTime(arenaTime));}, 1000);
    setInterval(function(){$('#mtime').text(calculateTime(missionTime));}, 1000);
    setInterval(function(){leagueBattles();}, 1000);
}

function leagueBattles(){
//console.log(leaguetime);
var time = new Date().getTime();
    var attempts = 15 - Math.ceil((leaguetime - time)/2100000);
    var remainingTime = convertToTimeFormat(parseInt((leaguetime - time)/1000 - Math.floor((leaguetime - time)/2100000)*2100));
    var percentage = parseInt(attempts/15*100);
    $('#scriptLeagueAttempts').text(attempts);
    if(attempts < 15)
        $('#scriptLeagueTimer').html("+1 in <span rel=\"count\">" + remainingTime + "</span>");
    $('#scriptLeagueBar').css("width", percentage + "%");
}

function leagues(){
var time = new Date().getTime();
    var attemptsTillSeasonEnd = parseInt(season_end_at/2100);
    var remainingAttempts = 15 - Math.ceil((leaguetime - time)/2100000) + parseInt(season_end_at/2100);
    var nextAttemptTime = parseInt((leaguetime - time)/1000 - Math.floor((leaguetime - time)/2100000)*2100);
    var remainingTimeOfSeason = season_end_at - parseInt(season_end_at/2100)*2100;
    if( nextAttemptTime < remainingTimeOfSeason)
        remainingAttempts++;
    var possibleBattles = -3;
    leagues_list.forEach(function(opponent) {
        possibleBattles += 3 - opponent.nb_challenges_played;
    });
    $('.league_end_in').append("<span>&nbsp;&nbsp;&nbsp;" + texts[lang].remainingBattles + " </span><span><b> " + possibleBattles + "</b></span><span>&nbsp;&nbsp;&nbsp;" + texts[lang].remainingAttempts + "</span><span><b> " +remainingAttempts + "</b></span>");
 //   var remainingAttempts =
}

function pachinko(){
    var pachinkoTime = Hero.infos.recharge_timers.calculation_timestamp + pachinkoVar.next_game*1000;
    localStorage.setItem("pachinkoTime", pachinkoTime);
}

function home() {
    var marketTime = localStorage.getItem("marketTime");
    $('a[href$="shop.html"]>.position').append('<span id="scriptMarketTimer">' + calculateTime(marketTime) + '</span>');
    setInterval(function(){$('#scriptMarketTimer').text(calculateTime(marketTime));}, 1000);
    var pachinkoTime = localStorage.getItem("pachinkoTime");
    $('a[href$="pachinko.html"]>.position').append('<span id="scriptPachinkoTimer">' + calculateTime(pachinkoTime) + '</span>');
    setInterval(function(){$('#scriptPachinkoTimer').text(calculateTime(pachinkoTime));}, 1000);

    if (typeof arena_data !== 'undefined') {
        serverTime = Hero.infos.recharge_timers.calculation_timestamp;
        arenaTime = serverTime + arena_data.countdown*1000;

        if (typeof missions_datas.remaining_time !== 'undefined') {
            missionTime = serverTime + missions_datas.remaining_time*1000;
        }
        //    missionTime = 0;
        localStorage.setItem("arenaTime", arenaTime);
        localStorage.setItem("missionTime", missionTime);
    } else{
        arenaTime = localStorage.getItem("arenaTime");
        missionTime = localStorage.getItem("missionTime");
        //eventTrolls = localStorage.getItem("eventTrolls");
    }
...
function activities() {
    ...
    if (typeof HHTimers.timers !== 'undefined') {
        serverTime = Hero.infos.recharge_timers.calculation_timestamp;
        var timers = HHTimers.timers;
        for (var i = 0; i < Object.keys(timers).length; i++) {
//            console.log(timers[i]);
            if(timers[i].hasOwnProperty('$barElm')){

 //               console.log(timers[i].$barElm.selector);
                if(timers[i].$barElm.selector == ".finish_in_bar .frontbar" ){
                    missionTime = serverTime + timers[i].remainingTime*1000;
                }
            }
        }
        localStorage.setItem("missionTime", missionTime);
...
function arena() {
    ...
    if (typeof HHTimers.timers !== 'undefined') {
        ...
        var timers = HHTimers.timers;
        for (var i = 0; i < Object.keys(timers).length; i++) {
            if(timers[i].hasOwnProperty('$elm')){
                if(timers[i].$elm.selector == '.arena_refresh_counter [rel="count"]' ){
                    arenaTime = serverTime + timers[i].remainingTime*1000;
                }
            }
        }
        localStorage.setItem("arenaTime", arenaTime);
...
function FightATroll() {
    ...
    // Trolls' database
    var Trolls = ['Dark Lord', 'Ninja Spy', 'Gruntt', 'Edwarda', 'Donatien', 'Silvanus', 'Bremen', 'Finalmecia', 'Roko Senseï', 'Karole', 'Jackson\'s Crew', 'Pandora witch'];
...
function ModifyMarket() {
    var shop = $( "#shop.shop_count" );
    var marketTime = Hero.infos.recharge_timers.calculation_timestamp + $( "#shop>.shop_count> span").attr( "time" )*1000;
...
function ModifyHarem() {
    ...
    try {
        ...
        if (new Date() > lsMarket.restock.time || Hero.infos.level > lsMarket.restock.herolvl) {
            ...
            var	marketBookTxt = lsMarket.buyable.potion.Nb + ' ' + texts[lang].books + ' (' + NbCommas(lsMarket.buyable.potion.Xp) + ' ' + texts[lang].Xp + ')',
                marketGiftTxt = lsMarket.buyable.gift.Nb + ' ' + texts[lang].gifts + ' (' + NbCommas(lsMarket.buyable.gift.Xp) + ' ' + texts[lang].affection + ')';
            RestockInfo = '- ' + marketBookTxt + ' = ' + NbCommas(lsMarket.buyable.potion.Value) + ' <span class="imgMoney"></span>'
                + '<br />- ' + marketGiftTxt + ' = ' + NbCommas(lsMarket.buyable.gift.Value) + ' <span class="imgMoney"></span>'
        ...
        var myArmorTxt = NbCommas(lsMarket.stocks.armor.Nb) + (lsMarket.stocks.armor.Nb > 99 ? '+ ' : ' ') + ' ' + texts[lang].equipments,
            ...
            myBookTxt = NbCommas(lsMarket.stocks.potion.Nb) + ' ' + texts[lang].books + '(' + NbCommas(lsMarket.stocks.potion.Xp) + ' ' + texts[lang].Xp + ')',
            myGiftTxt = NbCommas(lsMarket.stocks.gift.Nb) + ' ' + texts[lang].gifts + ' (' + NbCommas(lsMarket.stocks.gift.Xp) + ' ' + texts[lang].affection + ')',
    ...
    + stats.rarities.starting + ' ' + texts[lang].starter + ', ' + stats.rarities.common + ' ' + texts[lang].common + ', ' + stats.rarities.rare + ' ' + texts[lang].rare + ', ' + stats.rarities.epic + ' ' + texts[lang].epic + ', ' + stats.rarities.legendary + ' ' + texts[lang].legendary + ' ' + '<br />- '
    + stats.unlockedScenes + '/' + stats.allScenes + ' ' + texts[lang].unlocked_scenes + ''
    + '<span class="subTitle">' + texts[lang].money_incomes + ':</span>'
    + '~' + NbCommas(stats.hourlyMoney) + ' <span class="imgMoney"></span> ' + texts[lang].per_hour
    + '<br />' + NbCommas(stats.allCollect) + ' <span class="imgMoney"></span> ' + texts[lang].when_all_collectable
    ...
    + NbCommas(stats.xp) + ' ' + texts[lang].Xp + ' (' + NbCommas(stats.xp*200) + ' <span class="imgMoney"></span>), '+ ' <br />'
    ...
    $('#harem_left').append('<div id="CustomBar">'
                            + '<img f="stats" src="https://i.imgur.com/DwsBavN.png">'
                            + '</div>'
    ...
    function addPriceRow(rowName, affection, money, kobans){
        return '<b>' + rowName + '</b> ' +
            NbCommas(affection) + ' ' + texts[lang].affection + '(' + NbCommas(affection*417) + ' <span class="imgMoney"></span>) and '+
...
*/    sheet.insertRule('.eventTroll {'
    ...
    sheet.insertRule('.hudScript_mix_icn{'
                     + 'display:block;'
                     + 'width:29px;'
                     + 'height:31px;'
                     + 'background-image:url(https://hh.hh-content.com/league_points.png);'
...
function marketCss(){
    ...
    sheet.insertRule('#inventory .imgMoney {'
                     ...
                     + 'background-image: url("https://i.imgur.com/hnnHBux.png");'
    ...
    sheet.insertRule('.hero_stats .CustomStats {'
                     + 'float: right;'
                     + 'margin-left: -25px;'
                     + 'background-image: url("https://i.imgur.com/7nhmtbM.png");'
                     + 'background-size: 18px 18px;'
                     + 'background-position: center;'
                     + 'background-repeat: no-repeat;'
                     + 'width: 18px;'
...
function haremCss(){
    ...
    //     CSS RULES
    // -----------------
    sheet.insertRule('#harem_left #collect_all_container {'
                     + 'margin: 5px auto; }');
