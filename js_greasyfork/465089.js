// ==UserScript==
// @name         LibReddit Redirector
// @namespace    https://violentmonkey.github.io/
// @version      1.4
// @description  Automatically redirects all links from Reddit to Libreddit or Redlib
// @author       Streampunk
// @icon         data:image/png;base64,
// @match        https://www.reddit.com/*
// @match        https://new.reddit.com/*
// @match        https://old.reddit.com/*
// @match        https://l.opnxng.com/*
// @match        https://reddit.com/*
// @grant        none
// @run-at       document-start
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/465089/LibReddit%20Redirector.user.js
// @updateURL https://update.greasyfork.org/scripts/465089/LibReddit%20Redirector.meta.js
// ==/UserScript==

(function () {
	 'use strict';
// You can set alternative private front-end that works better for you such as redlib.catsarch.com, redlib.freedit.eu or redlib.ducks.party.
//	 top.location.hostname = "safereddit.com";
// By default userscript uses Farside provides links that automatically redirect to working instances of Libreddit
	 window.location.assign((window.location.href.split("?")[0]).replace("www.reddit.com", "farside.link/libreddit").replace("new.reddit.com", "farside.link/libreddit").replace("old.reddit.com", "farside.link/libreddit").replace("l.opnxng.com", "farside.link/libreddit").replace("reddit.com", "farside.link/libreddit"))
})();