// ==UserScript==
// @name         Threads 嘉版本 5.3 (Auto-Match .net / .com)
// @namespace    http://tampermonkey.net/
// @version      5.3
// @description  支援多目標、自訂首頁、429 限流重試與滾動模擬的 Threads 自動瀏覽腳本
// @author       ChatGPT
// @match        *://threads.net/*
...
// @license      MIT
// @language     zh-TW
// @downloadURL https://update.greasyfork.org/scripts/534772/Threads%20%E5%98%89%E7%89%88%E6%9C%AC%2053%20%28Auto-Match%20net%20%20com%29.user.js
// @updateURL https://update.greasyfork.org/scripts/534772/Threads%20%E5%98%89%E7%89%88%E6%9C%AC%2053%20%28Auto-Match%20net%20%20com%29.meta.js
// ==/UserScript==

(function() {
    'use strict';
    console.log('[ThreadsV52] script loaded');
    ...
    // 限流處理：攔截 fetch 429/401 並靜默，不在 Console 泄錯
    ...
    window.fetch = async function(resource, init) {
        ...
        // 靜默 429 與 401，不印出錯誤或警告
        if (resp.status === 429 || resp.status === 401) {
            return resp;
        }
        // 偵測 GraphQL 查詢請求並記錄
        if ((url.includes('/graphql/query') || url.includes('/api/graphql')) && init && init.method === 'POST') {
            console.log('[ThreadsV52] GraphQL 200 OK:', url, 'Payload:', init.body);
        }
        return resp;
    };
    ...
    inpTar.value  = load(STORAGE.TARGETS, JSON.stringify([location.origin])).replace(/[\[\]"]+/g,'').split(',').join(', ');

    function getTargets() { try { return JSON.parse(load(STORAGE.TARGETS, '[]')); } catch { return []; } }
    function getNext() {
    ...
    function stop() { save(STORAGE.RUN, false); updateStatus('已停止'); notify('停止', '已停止'); btnStart.disabled = false; }
    ...
    ['pushState','replaceState'].forEach(fn => {
        ...
        history[fn] = function() { orig.apply(this, arguments); window.dispatchEvent(new Event('urlchange')); };
    });
