...
// @version      20230104
// @description  wwd做的脚本出了点小问题，简单修改之后自己使用
// @author       wwd。
...
// @grant        GM_openInTab
// @license       wwd all
// @downloadURL https://update.greasyfork.org/scripts/457592/%E5%AD%A6%E4%B9%A0%E5%BC%BA%E5%9B%BD%E4%BB%85%E4%BE%9B%E4%B8%AA%E4%BA%BA%E4%BD%BF%E7%94%A8%E3%80%82.user.js
// @updateURL https://update.greasyfork.org/scripts/457592/%E5%AD%A6%E4%B9%A0%E5%BC%BA%E5%9B%BD%E4%BB%85%E4%BE%9B%E4%B8%AA%E4%BA%BA%E4%BD%BF%E7%94%A8%E3%80%82.meta.js
// ==/UserScript==
...
function setTaskProgressIndex(taskProgress){
    const taskIndex = {};
    for (let i = 0;i<taskProgress.length ; i++){
        if(taskProgress[i].title === "我要选读文章")taskIndex.KanXinWen = i;
        if(taskProgress[i].title === "视听学习")taskIndex.KanShiPing = i;
        if(taskProgress[i].title === "视听学习时长")taskIndex.KanShiPingShiChang = i;
        if(taskProgress[i].title === "登录")taskIndex.DengLu = i;
        if(taskProgress[i].title === "专项答题")taskIndex.ZhuanXiangDaTi = i;
        if(taskProgress[i].title === "每日答题")taskIndex.MeiRiDaTi = i;
        if(taskProgress[i].title === "每周答题")taskIndex.MeiZhouDaTi = i;
    }
    return taskIndex;
}
//开始
async function start() {
    ...
    if (loggedBox && loggedBox.innerText) {
        ...
        while (continueToDo) {
            ...
            const taskIndex = setTaskProgressIndex(taskProgress);

            if (taskProgress != null) {
                console.log("开始学习")
 
                //检查新闻
                if (settings[0] && taskProgress[taskIndex.KanXinWen].currentScore != taskProgress[taskIndex.KanXinWen].dayMaxScore) {
                    tasks[0] = false;//只要还有要做的，就当做没完成
                    newsNum = taskProgress[taskIndex.KanXinWen].dayMaxScore - taskProgress[taskIndex.KanXinWen].currentScore;//还需要看多少个新闻
                    console.log("1.看新闻");
                    await readNews();
                } else {
                    tasks[0] = true;
                ...
                //检查视频
                let temp = parseInt(taskProgress[taskIndex.KanShiPing].dayMaxScore - taskProgress[taskIndex.KanShiPing].currentScore);
                let temp2 = parseInt(taskProgress[taskIndex.KanShiPingShiChang].dayMaxScore - taskProgress[taskIndex.KanShiPingShiChang].currentScore);
                ...
                //检查每日答题
                if (settings[6] && taskProgress[taskIndex.MeiRiDaTi].currentScore != taskProgress[taskIndex.MeiRiDaTi].dayMaxScore) {
                ...
                //检查每周答题
                if (taskIndex.MeiZhouDaTi && settings[2] && taskProgress[taskIndex.MeiZhouDaTi].currentScore == 0) {
                ...
                //检查专项练习
                if (settings[5] && taskProgress[taskIndex.ZhuanXiangDaTi].currentScore == 0) {
