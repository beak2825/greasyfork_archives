...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/369808/yt_clipper.user.js
// @updateURL https://update.greasyfork.org/scripts/369808/yt_clipper.meta.js
// ==/UserScript==
...
#speed-input:invalid, #crop-input:invalid, #res-input:invalid {
  ...
  player.dispatchEvent(new MouseEvent('mouseclick', { button: 2 }));
  const url = player.getVideoUrl();
  const playerData = player.getVideoData();
  const duration = player.getDuration();
  const video = document.getElementsByTagName('video')[0];
  const isVerticalVideo = player.getVideoAspectRatio() <= 1;
  const progress_bar = document.getElementsByClassName('ytp-progress-bar')[0];

  try {
    let playerApiScript = document.querySelectorAll(
      '#player > script:nth-child(3)'
    )[0].textContent;
    var fps = parseInt(playerApiScript.match(/fps=(\d+)/)[1]);
  } catch {
    var fps = null;
  }

  const CLIENT_ID = '2_KgMc9b';
  ...
  let settings = {
    ...
    shortTitle: `${playerData.video_id}`,
    videoRes: isVerticalVideo ? '1080x1920' : '1920x1080',
    videoWidth: isVerticalVideo ? 1080 : 1920,
    videoHeight: isVerticalVideo ? 1920 : 1080
  };
  ...
  progress_bar.appendChild(markers_div);

  const markers_svg = markers_div.firstChild;
  ...
  function hotkeys(e) {
    ...
    if (toggleKeys) {
      switch (e.which) {
        ...
        case keys.C:
          if (!e.shiftKey && !e.ctrlKey) {
            sendGfyRequests(markers, url);
          } else if (e.shiftKey && !e.ctrlKey) {
            requestGfycatAuth();
          } else if (!e.shiftKey && e.ctrlKey) break;
      }
    }
    if (e.shiftKey && e.ctrlKey && e.which === keys.A) {
  ...
  function toggleSpeedAutoDucking() {
    if (video.ontimeupdate === autoducking) {
      video.removeEventListener('timeupdate', autoducking);
    } else {
      video.addEventListener('timeupdate', autoducking, false);
    }
  }
  ...
  function saveMarkers() {
    const markersJson = JSON.stringify({ [playerData.video_id]: markers });
  ...
  function receivedJson(e) {
    ...
    if (markersJson[playerData.video_id]) {
      markers.length = 0;
      undoMarkerOffset = 0;
      markersJson[playerData.video_id].forEach(markerPair => {
  ...
  function addMarker(markerConfig = [null, null, null]) {
    ...
    if (fps) {
      var currentTime = Math.floor(roughCurrentTime * fps) / fps;
    } else {
      var currentTime = roughCurrentTime;
    }
    const progress_pos = (currentTime / duration) * 100;
    const marker_attrs = {
      width: '2px',
      height: '12px',
      style: 'pointer-events:fill',
      slowdown: markerConfig[1] || settings.defaultSlowdown.toString(),
  ...
  function drawCropOverlay() {
    if (document.getElementById('crop-input')) {
      const videoRect = player.getVideoContentRect();
      video.addEventListener('mousedown', e => beginDraw(e, videoRect), {
        once: true,
        capture: true
      });
...
def clipper(markers, title, videoUrl, ytdlFormat, cropMultiple, overlayPath=''):
    ...
    def trim_video(startTime, endTime, slowdown, cropString,  outPath):
        ...
        ffmpegCommand = ' '.join((
            ...
            f'''-qmin 14 -crf 22 -qmax 40 -qcomp 1 -b:v 0 -movflags +faststart -f webm''',
    ...
    return new Promise((resolve, reject) => {
      fetch(REDIRECT_URI, { mode: 'cors' })
        ...
        .then(json => {
          ...
          sendGfyRequests(markers, url, accessToken);
        })
