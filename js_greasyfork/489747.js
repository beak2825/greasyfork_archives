// ==UserScript==
// @name        bnbfree and freeth [2024] - Auto Roll - Multiply - Status Console
...
// @description 1/31/2024, 11:21:57 AM
// @license      GPL
// @compatible   Firefox Tested with Violentmonkey
// @downloadURL https://update.greasyfork.org/scripts/489747/bnbfree%20and%20freeth%20%5B2024%5D%20-%20Auto%20Roll%20-%20Multiply%20-%20Status%20Console.user.js
// @updateURL https://update.greasyfork.org/scripts/489747/bnbfree%20and%20freeth%20%5B2024%5D%20-%20Auto%20Roll%20-%20Multiply%20-%20Status%20Console.meta.js
// ==/UserScript==
...
var MULTIPLY_WAIT_HOURS = 60; // How many hours to wait before to play multiply
...
var MAX_WIN = 0.00000900;          // Target Win, Stop to play multiply when those sats won.
...
var BAS_BET = 0.00000005; // BASE BET when begin the PLAY
...
var MIN_LOSSES_BEFORE_PLAY = 3; // How many time it will play MIN_BET and lose before begin play bas_bet and double;  strategy >= 1
var WAIT_PLAY_AFTER_LOSSES = 1; // Just with STRATEGY=2; Check guides.

var SPEED = 2;      //Speed of multiply betting in manual mode, 3 fast, 2 medium, 1 human, 0 slowest
...
function panel_referral_init(){
  ...
  console.log('last_multiply', last_multiply)
    console.log("last multiply_diff"+(last_multiply_diff/1000/60)+" minutes ago")
...
function rewards(after_refresh) {
    ...
    if ($("#bonus_container_fp_bonus").length != 0) {
        ...
        bonus_missing_time.text = $('#bonus_span_fp_bonus').text();
...
function Roll() {
    ...
    if ($('#free_play_form_button').is(':visible') && ! $('#play_without_captchas_button').is(':visible')) {
        ...
        if ( isNight() && Boolean(NIGHT_MODE) ) {
            ...
            if (r <= 10)        rand=random(9000,240000); // xx%, long wait, more than hour
            else if (r <= 30)   rand=random(6000,180000); // long but not longest
            else if (r <= 71)   rand=random(3000,120000); // xx% cases medium
...
function rollAndRetrieve (rand) {
        ...
        if ( $('#free_play_form_button').is(':visible') ){
            ...
            setTimeout(function(){
                ...
                if ( $('#free_play_error').html().length > 0 ) {
                    ...
                    var win_btc = $('#winnings').text() ?? 0;
                    var win_lottery = $('#fp_lottery_tickets_won').text() ?? 0;
                    var win_rp = $('#fp_reward_points_won').text() ?? 0;
                    executions++;
                }
                ...
                if (win_lottery > 0) {
                    ...
                    if (LOGGING > 1) console.log('%c[Roll] Some error retrieving Roll Winnings. Winning lottery was: '+win_lottery, 'color: red');
                }
                if (win_rp > 0) {
                    ...
                    if (LOGGING > 1) console.log('%c[Roll] Error retrieving Roll Winnings. Winning RP was: '+win_rp, 'color: red');
                }
            }, 6999);
        } else {
  ...
  console.log('[rollAndRetrieve][Action After Roll] rand: ', rand, ' ROLL_P: ', ROLL_P)
    if (rand > ROLL_P) {
...
function multiply(after_roll, forced) {
    ...
    if (LOGGING > 4) console.log("%c[Multiply] [Debug] lastMultiply diff: "+last_multiply_diff+" -- milli_between_multiplies: "+milli_between_multiplies+" -- last_multiply: "+last_multiply+" -- now: "+d.getTime(), 'color:grey');
    ...
    if (Boolean(after_roll)) {
      ...
      console.log('setting last_multiply to ', d.getTime());
        setCookie("last_multiply", d.getTime(), 30);
      console.log('confirmar cookie last_multiply', getCookie('last_multiply'));
