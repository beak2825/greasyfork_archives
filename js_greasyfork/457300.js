// ==UserScript==
// @name         facebook_spider_v2
// @namespace    http://tampermonkey.net/
// @version      0.30.18
// @description  facebook collection Tool
// @author       You
// @match        mobile.facebook.com/*
// @grant        GM_xmlhttpRequest
// @require      https://code.jquery.com/jquery-2.1.4.min.js
// @icon         https://www.google.com/s2/favicons?sz=64&domain=tampermonkey.net
// @downloadURL https://update.greasyfork.org/scripts/457300/facebook_spider_v2.user.js
// @updateURL https://update.greasyfork.org/scripts/457300/facebook_spider_v2.meta.js
// ==/UserScript==
...
$(function () {
    ...
    let targetUrl = 'mobile.facebook.com';
    ...
    console.log('facebook_spider', userId);
    
    if (windowHref.indexOf(targetUrl+'/groups/?seemore') == -1 && windowHref.indexOf(targetUrl+'/groups') == -1) {
        window.location.href = 'https://'+targetUrl+'/groups/?seemore';
        return
    }
    
    // 群组列表
    if (windowHref.indexOf(targetUrl+'/groups/?seemore') !== -1) {
        setTimeout(() => {
    ...
    function initGroupList() {
        ...
        function getGourpsList (cb) {
            let groupData = [];
            // $('#rootcontainer')
            $('#objects_container #root').find('a').each(function(index, item) {
                // if ($(item).attr('href').indexOf('/groups/') !== -1 && $(item).attr('href').indexOf('ref=group_browse')
    ...
    function initGroup() {
        ...
        function getGroupsItems() {
            ...
            */
            ...
            let groupsItems = groupsBox.find("[role='article']");
            let listData = []
                    ...
                    if ($(e).attr('data-ft')) {
                        ...
                        if (ft) {
                            ...
                            $.each(ft.page_insights, function(i,item){
                                // 时间戳格式
                                updatetime = item.post_context.publish_time
                            })
                            */
                            let eg = $(e).find('div.eg').children()
                            updatetime = $($(eg).find('abbr')[0]).text()
                            let content = $(e).find('div.do').children()
                            let imglist = [];
        ...
        function sendXml(list, cb) {
            ...
            window.xnmload = setInterval(() => {
                // 所有数据调用接口成功
                console.log('调用成功当前分页执行完毕---'+num+'----总队列',maxLengt)
                if (num == maxLengt) {
                    // 记录总条数
                    ...
                    clearInterval(window.xnmload)
                    console.log('调用成功当前队列执行完毕-------',localStorage.getItem('TASK_NUM'))
                    // 执行100条以上 结束任务
