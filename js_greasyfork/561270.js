// ==UserScript==
// @name              KeepChatGPT (Mobile Optimized)
...
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/561270/KeepChatGPT%20%28Mobile%20Optimized%29.user.js
// @updateURL https://update.greasyfork.org/scripts/561270/KeepChatGPT%20%28Mobile%20Optimized%29.meta.js
// ==/UserScript==
...
`;
    ...
    const loadKCG = function() {
        ...
        // Tworzenie tła dla menu mobilnego (backdrop)
        if (!$(".kmenu-backdrop")) {
        ...
        // Logika wstrzykiwania przycisku
        const isMobile = window.innerWidth <= 768;

        if ($(symbol1_selector) && !isMobile) {
            // Wersja Desktop (z paskiem bocznym)
            ...
            // Wersja Mobile (lub brak paska bocznego) - wstrzykuj do BODY
            ndivkcg.innerHTML = ndivkcg._symbol2_innerHTML; // Tylko ikona
            ndivkcg.classList.remove('kcg-pc');
            // Nie używamy klasy kcg-mb, pozycjonujemy przez CSS fixed
        ...
        ndivkcg.addEventListener('click', (e) => {
            ...
            if (ndivmenu.classList.contains('kshow')) {
                ...
                // Pozycjonowanie menu na desktopie
                if (!isMobile && $("#kcg")) {
                     ndivmenu.style.left = `${$("#kcg").getBoundingClientRect().right + 20}px`;
                     ndivmenu.style.top = `${$("#kcg").getBoundingClientRect().top}px`;
                }
...
@media screen and (max-width: 768px) {
    ...
    /* Ukrywamy tekst "KeepChatGPT", zostawiamy tylko ikonę */
    #kcg div {
    ...
    /* Menu na środku ekranu dla mobile */
    .kmenu {
        position: fixed !important;
        top: 50% !important;
        left: 50% !important;
    ...
    /* Większe przyciski w menu dla dotyku */
    .kmenu li {
    ...
    .kmenu-backdrop.show { display: block; }
}
/* --- KONIEC STYLI MOBILNYCH --- */
