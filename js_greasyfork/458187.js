...
// @downloadURL https://update.greasyfork.org/scripts/458187/SilverScripts%20MOD.user.js
// @updateURL https://update.greasyfork.org/scripts/458187/SilverScripts%20MOD.meta.js
// ==/UserScript==
...
(function() {
    ...
    var locations = {
        ...
        "forum" : ["fairview.deadfrontier.com/onlinezombiemmo/index.php?board=","fairview.deadfrontier.com/onlinezombiemmo/index.php?action=forum"]
    };
    ...
    async function checkForScriptUpdate(){
        ...
        if(needsUpdate && isVersionAfterSkip && daysElapsedSinceLastCheck > 1){
    ...
    function initInventoryArray(){
        ...
        for(var i=1;i<=userData.maxInvSlots;i++){
            ...
            item.extraInfo = "";
            item.type = "";
            item.flashType = userVars["DFSTATS_df_inv" + i + "_type"];
            ...
            if(item.id != "" && item.id != undefined){
                ...
                if(item.type == "Item"){
                    ...
                    if(itemGlobData.needcook == "1" && item.extraInfo != "Cooked"){
                        ...
                        item.serviceTooltip = "Administrar";
                    }
                }
    ...
    function autoServiceHelper(targetInventoryItem,action){
        ...
        if(targetInventoryItem.id != "" && unsafeWindow.lockedSlots.indexOf(lastSlotHovered) == -1){
            ...
            if( (
                ...
                )&&(servicesDataBank[targetInventoryItem.profession][targetInventoryItem.professionLevel] != undefined)
            ){
...
//     }
    ...
    function fillHoverBox(){
        ...
        bpu.innerHTML = "Precio unitario: $" + (Math.round((targetInventoryItem.bestPricePerUnit.toFixed(2)) * 10) / 10).toLocaleString('en-US') + " (" + Math.floor(targetInventoryItem.averagePricePerUnit.toFixed(2)).toLocaleString('en-US') + ")";
		bpu.style.color = "#12FF00";
        infoBox.appendChild(bpu);
		//Green = 12FF00 ; Gold = ABA000 ; Darker Gold = 897129 ; Orange = FFA500 ; Purple = 9947AD ; Red = D20303
        ...
        if(targetInventoryItem.quantity != 1 && targetInventoryItem.type != "Armour"){
            ...
            bps.innerHTML = "Precio lote: $" + Math.floor((targetInventoryItem.bestPricePerUnit * targetInventoryItem.quantity).toFixed(2)).toLocaleString('en-US') + " (" + Math.floor((targetInventoryItem.averagePricePerUnit * targetInventoryItem.quantity).toFixed(2)).toLocaleString('en-US') + ")";
...
//         }
        ...
        if(targetInventoryItem.type == "Cookable"){
            if(servicesDataBank[targetInventoryItem.profession][targetInventoryItem.professionLevel] != undefined){
                serv.innerHTML = "Precio para cocinar: $" + Math.floor((servicesDataBank[targetInventoryItem.profession][targetInventoryItem.professionLevel]["price"])).toLocaleString('en-US');
            }else{
            ...
            if(cookedItem != null){
                ...
                cookedBestPrice.innerHTML = "Precio cocinado: $" + (Math.round((cookedItem.bestPricePerUnit.toFixed(2)))).toLocaleString('en-US') + " (" + (Math.floor((cookedItem.averagePricePerUnit.toFixed(2)))).toLocaleString('en-US') + ")";
                ...
                lowestCookingEarnings.innerHTML = "Ganancia por cocinar: $" + Math.round((cookedItem.bestPricePerUnit - targetInventoryItem.bestPricePerUnit - servicesDataBank[targetInventoryItem.profession][targetInventoryItem.professionLevel]["price"]).toFixed(2)).toLocaleString('en-US');
                infoBox.appendChild(lowestCookingEarnings);
            }
        }else if(targetInventoryItem.type == "Armour"){
            if(servicesDataBank[targetInventoryItem.profession][targetInventoryItem.professionLevel] != undefined){
                serv.innerHTML = "Precio para reparar: $" + (servicesDataBank[targetInventoryItem.profession][targetInventoryItem.professionLevel]["price"]).toLocaleString('en-US');
            }else{
                serv.innerHTML = "No services available at this outpost";
            }
            infoBox.appendChild(serv);
            ...
            if(servicesDataBank[targetInventoryItem.profession][targetInventoryItem.professionLevel] != undefined){
                serv.innerHTML = "Precio para administrar: $" + (servicesDataBank[targetInventoryItem.profession][targetInventoryItem.professionLevel]["price"]).toLocaleString('en-US');
            }else{
                serv.innerHTML = "No services available at this outpost";
            }
            infoBox.appendChild(serv);
        ...
        if(targetInventoryItem.scrapValue != undefined){
            ...
            scrapValue.innerHTML = "Scrap value: $" + targetInventoryItem.scrapValue.toLocaleString('en-US');
            scrapValue.style.color = "#E6CC4D";
