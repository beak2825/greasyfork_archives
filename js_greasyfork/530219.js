...
// @originalAuthor  drlivog
// @author          Nyannerz
// @match           https://gazellegames.net/forums.php?*action=viewthread*
// @icon            https://gazellegames.net/favicon.ico
// @grant           none
// @originalDownloadURL https://update.greasyfork.org/scripts/462932/GGn%2020%20Questions%20Host.user.js
// @originalUpdateURL https://update.greasyfork.org/scripts/462932/GGn%2020%20Questions%20Host.meta.js
// @downloadURL https://update.greasyfork.org/scripts/530219/GGn%2020%20Questions%20Host.user.js
// @updateURL https://update.greasyfork.org/scripts/530219/GGn%2020%20Questions%20Host.meta.js
// ==/UserScript==
/* globals $ */
...
//Personalized by Nyannerz for my own use.

const activategames = [960, 20829, 28282]; //threadids to work on
const default_numbered = false;
const use_floating_window = false;
const extraNewlineAfterCopy = true; //Adds an extra empty line after copying your previous list of answers to separate the new questions & answers.
const deleteOldNewline = true; //Deletes the new line added by the above option when copying the old post.
const copyQuoteRemoveDelay = 500; //value in ms to delay removing the quote. Increase if it's not getting removed.
const addInvalidButton = false; //Add an additional button to mark invalid questions.
const strikeThroughInvalidQuestions = true;

const defaultYesResponse = "[color=lightgreen]Yes[/color]"; //Allows you to easily change how you want your default response to look.
const defaultNoResponse = "[color=red]No[/color]";
const defaultYellowResponse = "[color=yellow]Kinda[/color]";
const defaultInvalidResponse = "[color=grey]Invalid Question[/color]" //Changing this mid game will potentially mess things up
...
(function() {
    ...
    $('a:contains("[Quote]")')
        .after(function() { if(addInvalidButton) {return ` <a id="${this.id.replace('quote','invalid')}" class="answerinvalid" data-id="${this.id.replace('quote_','')}" href="#0" style="font-size: 1em;">[Invalid]</a>`;}}) //Only add invalid button if enables
        .after(function() { return ` <a id="${this.id.replace('quote','qmark')}" class="answerkinda" data-id="${this.id.replace('quote_','')}" href="#0" style="font-size: 1em;">[?]</a>`;})
        .after(function() { return ` <a id="${this.id.replace('quote','no')}" class="answerno" data-id="${this.id.replace('quote_','')}" href="#0" style="font-size: 1em;">[No]</a>`;})
        .after(function() { return ` <a id="${this.id.replace('quote','yes')}" class="answeryes" data-id="${this.id.replace('quote_','')}" href="#0" style="font-size: 1em;">[Yes]</a>`;})
        .after(function() { return ` <a id="${this.id.replace('quote','copy')}" class="copyblock" data-id="${this.id.replace('quote_','')}" href="#0" style="font-size: 1em;">[Copy]</a>`;});
    $('.answeryes').click( (event) => answer(event.target.dataset.id, defaultYesResponse));
    $('.answerno').click( (event) => answer(event.target.dataset.id, defaultNoResponse));
    $('.answerkinda').click( (event) => answer(event.target.dataset.id, defaultYellowResponse));
    $('.copyblock').click( (event) => CopyPost(event.target.dataset.id));
    if(addInvalidButton) $('.answerinvalid').click( (event) => answer(event.target.dataset.id, defaultInvalidResponse)); //Only add invalid button functionality if enabled.
    ...
    if (use_floating_window) {
        ...
        if (!default_numbered) {
            ...
            $("#quicknumber").click(function() {
                ...
                let count=parseInt($("#quicknumberstart").val())-1 || 0;
                for(let i=0; i<text.length; i++) {
                    if (text[i] == "") continue; //skip empty lines
                    if (text[i].startsWith("[spoiler" || text[i].startsWith("[hide"))) //Skip lines that start the spoiler/hide block, but start the count from the next number.
                    {
                      count++;
                      continue;
                    }
                    if (text[i].match(/^(\d*?\.\s?).*/)) {
                        if (addInvalidButton && text[i].includes(defaultInvalidResponse)) text[i] = text[i].replace(/^(\d*?\.\s?)/, (count)+". ");//If invalid question, don't increment question counter.
                        else text[i] = text[i].replace(/^(\d*?\.\s?)/, (++count)+". ");
                        continue;
                    }
                    if (text[i].match(/^(\[\*\]\s?)/)) {
                        if (addInvalidButton && text[i].includes(defaultInvalidResponse)) text[i] = text[i].replace(/^(\[\*#\]\s?)/, (count)+". ");//If invalid question, don't increment question counter.
                        else text[i] = text[i].replace(/^(\[\*#\]\s?)/, (++count)+". ");
                        continue;
                    }
                    if (text[i].indexOf("?") > -1) { //only number lines that have a question mark
                        if (addInvalidButton && text[i].includes(defaultInvalidResponse)) //If invalid question, don't increment question counter.
                        {
                          if(strikeThroughInvalidQuestions) text[i] = (count)+". [s]"+text[i].substring(0, text[i].indexOf("?")+1)+"[/s] " + defaultInvalidResponse;
                          else text[i] = (count)+". "+text[i];
                        }
                        else text[i] = (++count)+". "+text[i];
                    }
                }
...
{
  Quote(postId,'', true);
  var quickReplyElement = document.getElementById("quickpost");
  console.log(`[quote=|${postId}]`);
  setTimeout(function(){
        if(!quickReplyElement.value)
          {
            setTimeout(function(){
              console.log(quickReplyElement.value);
              quickReplyElement.value = quickReplyElement.value.replace(`[quote=|${postId}]`,"").replace("[/quote]","");
              if(deleteOldNewline) quickReplyElement.value = quickReplyElement.value.replace(/\n\s*\n/g, '\n');
              if(extraNewlineAfterCopy) quickReplyElement.value += "\n";
            },copyQuoteRemoveDelay);
          }
        else
        {
          console.log(quickReplyElement.value);
   ...
   },copyQuoteRemoveDelay); //delay is in milliseconds
...
function answer(postid, response) {
    ...
    if (getSelection().toString() && inPost(getSelection().anchorNode) && inPost(getSelection().focusNode)) {
        ...
        if (post.indexOf("?") <0) post+="?"; //Add a question mark at the end if it's missing.
        if (post.charAt(post.length-2) == " ") post = post.substr(0,post.length-2) + "?";
        if (post.slice(0,7).toLowerCase() == "is it's") post = post.trim().replace(/^is it's/i,"is its").trim();
        if (post.slice(0,6).toLowerCase() != "is its" && post.slice(0,12).toLowerCase() != "is it or was")post = post.trim().replace(/^is it/i,"").trim(); //remove is it from the begining
        ...
        if (first_letter_case.toLowerCase() == "upper") {
            ...
            if(post.slice(0,3).toLowerCase() != "ggn") post = post.slice(0,1).toLowerCase() + post.substring(1);
            else post = "GGn" + post.substring(3);
        }
