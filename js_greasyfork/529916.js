...
// @version      18.2
// @license GPL
// @description  使用sd、novelai3、comfyui 配合ai在酒馆生成插图 以及酒馆正则管理，咸鱼二改支持外部sdwebui的账号鉴权
// @author       咸鱼王
// @grant        unsafeWindow
// @match        *://*/*
...
// @connect      sdapi.xianyuw.cn
// @grant        unsafeWindow
// @grant        GM_xmlhttpRequest
// @connect       *
// @connect      127.0.0.1
// @connect      novelai.net
// @connect      1316649238-4k03zsyk0n.ap-singapore.tencentscf.com
// @match        *://*/*
...
// @downloadURL https://update.greasyfork.org/scripts/529916/%E5%92%B8%E9%B1%BC%E4%BA%8C%E6%94%B9ai%E6%8F%92%E5%9B%BE%E8%84%9A%E6%9C%AC.user.js
// @updateURL https://update.greasyfork.org/scripts/529916/%E5%92%B8%E9%B1%BC%E4%BA%8C%E6%94%B9ai%E6%8F%92%E5%9B%BE%E8%84%9A%E6%9C%AC.meta.js
// ==/UserScript==
...
(function () {
    ...
    const defaultSettings = {
        ...
        sdUsername: '', // 新增：SD API用户名
        sdPassword: '', // 新增：SD API密码
        sdModel: '', // 新增：当前选择的SD模型
        sdModelList: {}, // 新增：存储可用的SD模型列表
        novelaiToken: '000000', // NovelAI Token
        novelaiUrl: 'https://1316649238-4k03zsyk0n.ap-singapore.tencentscf.com/ai/generate-image', // NovelAI 自定义URL
        startTag: 'image###',
        endTag: '###',
        fixedPrompt: '',
        nai3Scale: '10',
    ...
    for (const [key, defaultValue] of Object.entries(defaultSettings)) {
        ...
        if (settings[key] === defaultValue) {
            if (key == "yushe" || key == "workers" || key == "worker" || key == "sdModelList") {
                GM_setValue(key, JSON.stringify(defaultValue));//使用字符串保存
            } else {
                GM_setValue(key, defaultValue);
            }
        } else if (key == "yushe" || key == "workers" || key == "sdModelList") {
            ...
            try {
                ...
                } else if (key == "sdModelList") {
                    settings["sdModelList"] = {};
                }
            }
        }
    }
    ...
    function createSettingsPanel() {
        ...
        panel.style.backgroundColor = 'white';  // 设置背景为白色
        panel.style.color = 'black';// 设置字体为黑色
        panel.style.padding = '15px';
        panel.style.border = '1px solid #ccc';// 设置边框为灰色
        panel.style.boxShadow = '0 4px 8px rgba(0, 0, 0, 0.1)';// 添加阴影效果
      ...
      `;
        ...
        <div class="settings-tabs">
          <div class="settings-tab active" data-tab="general">基本设置</div>
          <div class="settings-tab" data-tab="prompts">提示词设置</div>
          <div class="settings-tab" data-tab="api">API设置</div>
          <div class="settings-tab" data-tab="advanced">高级选项</div>
        </div>
  
        <!-- 基本设置选项卡 -->
        <div id="general-tab" class="settings-content active">
          <div class="settings-form-group">
            <label class="settings-label">生成模式</label>
          <select id="mode">
          ...
          <div class="settings-form-group">
            <label class="settings-label">双击图片隐藏按钮 (双击震动反馈一次即可)</label>
          <select id="dbclike">
        ...
        <!-- 提示词设置选项卡 -->
        <div id="prompts-tab" class="settings-content">
          <div class="settings-form-group">
            <div class="inline-elements">
              <label class="settings-label">固定提示词设置</label>
              <select id="yusheid" style="max-width: 250px; overflow: hidden; text-overflow: ellipsis;">`
        ...
        html +=
            `  </select>
                ...
                <div class="menu_button interactable" title="保存风格" data-i18n="[title]Save style" id="tishici_save_style" tabindex="0" style="margin-left: 5px;">
                        <i class="fa-solid fa-save"></i>
                    </div>
  
                <div class="menu_button interactable" title="删除风格" data-i18n="[title]Delete style" id="tishici_delete_style" tabindex="0" style="margin-left: 5px;">
            ...
            <div class="settings-form-group">
              <label class="settings-label">固定正面提示词</label>
              <textarea class="text_pole textarea_compact" id="fixedPrompt" style="height: 55px; width: 100%;">${settings.fixedPrompt}</textarea>
            </div>
  
            <div class="settings-form-group">
              <label class="settings-label">固定负面提示词</label>
              <textarea class="text_pole textarea_compact" id="negativePrompt" style="height: 55px; width: 100%;">${settings.negativePrompt}</textarea>
            </div>`

        // Comfyui相关设置，放在提示词选项卡内
        if (settings.mode === 'comfyui') {
            html += `
            <div class="settings-form-group">
              <label class="settings-label">LORA添加</label>
              <div class="settings-row">
                <select id="ComfyuiLORA" style="width: calc(100% - 70px);">
        ...
        if (settings.mode === 'comfyui') {
            ...
            <div class="settings-form-group">
              <div class="inline-elements">
                <label class="settings-label">工作流设置</label>
                <select id="workerid" style="max-width: 250px; overflow: hidden; text-overflow: ellipsis;">`
           ...
           `
            ...
            html +=
                `  </select>
                <div class="menu_button interactable" title="保存风格" data-i18n="[title]Save style" id="worker_save_style" tabindex="0" style="margin-left: 5px;">
                        <i class="fa-solid fa-save"></i>
                    </div>
  
                <div class="menu_button interactable" title="删除风格" data-i18n="[title]Delete style" id="worker_delete_style" tabindex="0" style="margin-left: 5px;">
                        <i class="fa-solid fa-trash-can"></i>
                    </div>
        </div>
            </div>
            ...
            <div class="settings-form-group">
              <label class="settings-label">工作流</label>
              <textarea class="text_pole textarea_compact" id="worker" style="height: 55px; width: 100%;">${settings["worker"]}</textarea>
            </div>
  
            <div class="settings-form-group">
              <label class="settings-label">自定义工作</label>
      ...
      `
        ...
        html += `
          ...
          <div id="advanced-tab" class="settings-content">
            <div class="settings-form-group">
              <label class="settings-label">正面预设</label>
          <select id="AQT">
                <option value="best quality, amazing quality, very aesthetic, absurdres" ${settings.AQT === 'best quality, amazing quality, very aesthetic, absurdres' ? 'selected' : ''}>开启</option>
                <option value="" ${settings.AQT === '' ? 'selected' : ''}>关闭</option>
          </select>
            </div>
  
            <div class="settings-form-group">
              <label class="settings-label">负面预设</label>
          <select id="UCP">
           <option value="" ${settings.UCP === '' ? 'selected' : ''}>无</option>
            ...
            <div class="settings-form-group">
              <label class="settings-label">缓存设置</label>
          <select id="cache">
          ...
          <!-- API设置选项卡 -->
          <div id="api-tab" class="settings-content">
            <!-- 通用API设置容器 -->
            <div id="common-api-settings" class="settings-container">
              <h3 style="margin-top: 0;">通用参数设置</h3>
              <div class="settings-form-group">
                <label class="settings-label">开始标记</label>
                <input type="text" id="startTag" value="${settings.startTag}" style="width: 100%;">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">结束标记</label>
                <input type="text" id="endTag" value="${settings.endTag}" style="width: 100%;">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">生成步数</label>
                <input type="number" id="steps" value="${settings.steps}">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">宽度</label>
                <input type="number" id="width" value="${settings.width}">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">高度</label>
                <input type="number" id="height" value="${settings.height}">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">生成种子</label>
                <input type="number" id="seed" value="${settings.seed}">
              </div>
            </div>
  
            <!-- SD API设置容器 -->
            <div id="sd-api-settings" class="api-settings-container" style="display: ${settings.mode === 'sd' ? 'block' : 'none'};">
              <h3>SD 模式设置</h3>
              <div class="settings-form-group">
                <label class="settings-label">SD/COMFYUI URL</label>
                <input type="text" id="sdUrl" value="${settings.sdUrl}" style="width: 100%;">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">SD 用户名 (API认证)</label>
                <input type="text" id="sdUsername" value="${settings.sdUsername}" style="width: 100%;">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">SD 密码 (API认证)</label>
                <input type="password" id="sdPassword" value="${settings.sdPassword}" style="width: 100%;">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">当前SD模型</label>
                <select id="sdModel" style="width: 100%;">
              <option value="" ${settings.sdModel === '' ? 'selected' : ''}>自动加载模型列表...</option>
              ${Object.entries(settings.sdModelList).map(([modelId, modelName]) =>
            `<option value="${modelId}" ${settings.sdModel === modelId ? 'selected' : ''}>${modelName}</option>`
        ).join('')}
            </select>
                <button id="loadSDModels" type="button" style="margin-top: 5px;">加载模型列表</button>
        </div>
  
              <div class="settings-form-group">
                <label class="settings-label">关键词关联性cfg_scale</label>
                <input type="number" id="sdCfgScale" value="${settings.sdCfgScale}">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">SD面部修复</label>
          <select id="restoreFaces">
              ...
              <div class="settings-form-group">
                <label class="settings-label">SD采样方式</label>
                <select id="samplerName" style="width: 100%;">
            ...
            <!-- NovelAI API设置容器 -->
            <div id="novelai-api-settings" class="api-settings-container" style="display: ${settings.mode === 'novelai' ? 'block' : 'none'};">
              <h3>NovelAI 模式设置</h3>
              <div class="settings-form-group">
                <label class="settings-label">NovelAI Token</label>
                <input type="text" id="novelaiToken" value="${settings.novelaiToken}" style="width: 100%;">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">NovelAI URL</label>
                <input type="text" id="novelaiUrl" value="${settings.novelaiUrl}" style="width: 100%;">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">NovelAI模型</label>
                <select id="novelaimode" style="width: 100%;">
              ...
              <div class="settings-form-group">
                <label class="settings-label">NovelAI关键词关联性Prompt Guidance</label>
                <input type="number" id="nai3Scale" value="${settings.nai3Scale}">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">NovelAI关键词重调Prompt Guidance Rescale</label>
                <input type="number" id="cfg_rescale" value="${settings.cfg_rescale}">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">NovelAI采样方式</label>
                <select id="sampler" style="width: 100%;">
            ...
            <option value="k_euler_ancestral" ${settings.sampler === 'k_euler_ancestral' ? 'selected' : ''}>k_euler_ancestral</option>
                      ...
                      <div class="settings-form-group">
                <label class="settings-label">NovelAI噪点</label>
                <select id="Schedule" style="width: 100%;">
              <option value="native" ${settings.Schedule === 'native' ? 'selected' : ''} data-model="nai-diffusion-3">native (仅适用于nai-diffusion-3)</option>
              ...
              <option value="karras" ${settings.Schedule === 'karras' ? 'selected' : ''}>karras (推荐选择)</option>
             </select>
             <div id="schedule-tip" style="display: none; margin-top: 5px; color: #ff6b6b; font-size: 12px;">注意：当前模型不支持native噪点，推荐使用karras</div>
              </div>
  
              <!-- NAI3特有功能设置 - 仅在选择nai3模型时显示 -->
              <div id="nai3-features-container" class="settings-form-group" style="display: ${settings.novelaimode === 'nai-diffusion-3' ? 'block' : 'none'};">
                <label class="settings-label">NAI3功能设置 (仅适用于nai-diffusion-3)</label>
                <div style="margin-top: 5px;">
                  <label><input type="checkbox" id="sm" ${settings.sm === 'true' ? 'checked' : ''}> NAI3sm</label>
                </div>
                <div style="margin-top: 5px;">
                  <label><input type="checkbox" id="dyn" ${settings.dyn === 'true' ? 'checked' : ''}> NAI3dyn</label>
                </div>
                <div style="margin-top: 5px;">
                  <label><input type="checkbox" id="nai3Variety" ${settings.nai3Variety === 'true' ? 'checked' : ''}> NAI3Variety多样性</label>
                </div>
                <div style="margin-top: 5px;">
                  <label><input type="checkbox" id="nai3Deceisp" ${settings.nai3Deceisp === 'true' ? 'checked' : ''}> NAI3Deceisp减少伪影</label>
                </div>
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">NovelAI氛围参考图片</label>
                <div style="display: flex; align-items: center; margin-top: 5px;">
                  <input type="file" id="imageInput" accept="image/*" style="display: none;">
                  <button type="button" onclick="document.getElementById('imageInput').click();" style="margin-right: 10px;">选择图片</button>
                  <img id="previewImage" src="" alt="预览图片" style="max-width: 100px; max-height: 100px;">
                </div>
              </div>
  
              <div id="nai3-vibe-container" class="settings-form-group" style="display: ${settings.novelaimode === 'nai-diffusion-3' ? 'block' : 'none'};">
                <label><input type="checkbox" id="nai3VibeTransfer" ${settings.nai3VibeTransfer === 'true' ? 'checked' : ''}> NAI3图片氛围转移VibeTransfer</label>
              </div>
  
              <div id="nai3-vibe-settings-container" class="settings-form-group" style="display: ${settings.novelaimode === 'nai-diffusion-3' ? 'block' : 'none'};">
                <label class="settings-label">氛围提取信息InformationExtracted(0-1)</label>
                <input type="number" id="InformationExtracted" value="${settings.InformationExtracted}" step="0.01" min="0" max="1">
              </div>
  
              <div id="nai3-vibe-strength-container" class="settings-form-group" style="display: ${settings.novelaimode === 'nai-diffusion-3' ? 'block' : 'none'};">
                <label class="settings-label">氛围强度ReferenceStrength(0-1)</label>
                <input type="number" id="ReferenceStrength" value="${settings.ReferenceStrength}" step="0.01" min="0" max="1">
              </div>
            </div>
  
            <!-- Comfyui API设置容器 -->
            <div id="comfyui-api-settings" class="api-settings-container" style="display: ${settings.mode === 'comfyui' ? 'block' : 'none'};">
              <h3>Comfyui 模式设置</h3>
              <div class="settings-form-group">
                <label class="settings-label">SD/COMFYUI URL</label>
                <input type="text" id="sdUrl" value="${settings.sdUrl}" style="width: 100%;">
              ...
              <div class="settings-form-group">
                <label class="settings-label">连接测试</label>
                <button id="testComfyui">点击连接</button>
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">Comfyui模型文件名称</label>
              ...
              <div class="settings-form-group">
                <label class="settings-label">Comfyui采样方式</label>
                <select id="comfyuisamplerName" style="width: 100%;">
              ...
              <div class="settings-form-group">
                <label class="settings-label">Comfyui氛围参考图片</label>
                <div style="display: flex; align-items: center; margin-top: 5px;">
                  <input type="file" id="imageInput2" accept="image/*" style="display: none;">
                  <button type="button" onclick="document.getElementById('imageInput2').click();" style="margin-right: 10px;">选择图片</button>
                  <img id="previewImage2" src="" alt="预览图片" style="max-width: 100px; max-height: 100px;">
                </div>
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">IPA设置</label>
                <select id="ipa" style="width: 100%;">
            ...
            <option value="FULL FACE - SD1.5 only (portraits stronger)" ${settings.ipa === 'FULL FACE - SD1.5 only (portraits stronger)' ? 'selected' : ''}>FULL FACE - SD1.5 only (portraits stronger)</option>
              ...
              <div class="settings-form-group">
                <label class="settings-label">氛围强度</label>
                <input type="number" id="c_fenwei" value="${settings.c_fenwei}">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">细节强度</label>
                <input type="number" id="c_xijie" value="${settings.c_xijie}">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">权重</label>
                <input type="number" id="c_quanzhong" value="${settings.c_quanzhong}">
              </div>
  
              <div class="settings-form-group">
                <label class="settings-label">faceid权重</label>
                <input type="number" id="c_idquanzhong" value="${settings.c_idquanzhong}">
              </div>
            </div>
        ...
        html += `
          ...
          <div id="advanced-tab" class="settings-content">
            <div class="settings-form-group">
              <label class="settings-label">自动点击生成</label>
        ...
        // 添加底部按钮区域（位于所有选项卡外部）
        html += `
          <div class="settings-footer" style="display: flex; justify-content: space-between; margin-top: 20px; padding-top: 15px; border-top: 1px solid #444;">
            <div>
              <button id="Clear-Cache" class="settings-btn">清除图片缓存</button>
            </div>
            <div>
              <button id="save-settings" class="settings-btn">保存设置</button>
      ...
      `;
        ...
        // 添加模型列表加载按钮事件监听
        const loadSDModelsBtn = document.getElementById('loadSDModels');
        if (loadSDModelsBtn) {
            loadSDModelsBtn.addEventListener('click', loadSDModelList);
        }

        // 添加模式变更监听，以显示/隐藏对应的API设置
        const modeSelect = document.getElementById('mode');
        if (modeSelect) {
            modeSelect.addEventListener('change', function () {
                // 获取所有模式的设置容器
                const sdContainer = document.getElementById('sd-api-settings');
                const novelaiContainer = document.getElementById('novelai-api-settings');
                const comfyuiContainer = document.getElementById('comfyui-api-settings');

                // 隐藏所有容器
                if (sdContainer) sdContainer.style.display = 'none';
                if (novelaiContainer) novelaiContainer.style.display = 'none';
                if (comfyuiContainer) comfyuiContainer.style.display = 'none';

                // 显示当前选择的模式容器
                if (this.value === 'sd' && sdContainer) {
                    sdContainer.style.display = 'block';
                } else if (this.value === 'novelai' && novelaiContainer) {
                    novelaiContainer.style.display = 'block';
                } else if (this.value === 'comfyui' && comfyuiContainer) {
        ...
        // 添加NovelAI模型切换事件监听
        const novelaiModelSelect = document.getElementById('novelaimode');
        if (novelaiModelSelect) {
            novelaiModelSelect.addEventListener('change', function () {
                ...
                // 获取NAI3特有功能设置容器
                const nai3FeaturesContainer = document.getElementById('nai3-features-container');
                const nai3VibeContainer = document.getElementById('nai3-vibe-container');
                const nai3VibeSettingsContainer = document.getElementById('nai3-vibe-settings-container');
                const nai3VibeStrengthContainer = document.getElementById('nai3-vibe-strength-container');
                const scheduleTip = document.getElementById('schedule-tip');
                const scheduleSelect = document.getElementById('Schedule');

                // 根据模型控制NAI3功能显示
                if (nai3FeaturesContainer) {
                    nai3FeaturesContainer.style.display = currentModel === 'nai-diffusion-3' ? 'block' : 'none';
                }

                if (nai3VibeContainer) {
                    nai3VibeContainer.style.display = currentModel === 'nai-diffusion-3' ? 'block' : 'none';
                }

                if (nai3VibeSettingsContainer) {
                    nai3VibeSettingsContainer.style.display = currentModel === 'nai-diffusion-3' ? 'block' : 'none';
                }

                if (nai3VibeStrengthContainer) {
                    nai3VibeStrengthContainer.style.display = currentModel === 'nai-diffusion-3' ? 'block' : 'none';
                }

                // 控制native噪点选项和提示显示
                if (scheduleSelect && scheduleTip) {
                    const nativeOption = scheduleSelect.querySelector('option[value="native"]');

                    if (currentModel !== 'nai-diffusion-3') {
                        // 如果当前选择的是native且模型不是nai3，自动切换为karras
                        ...
                        // 启用native选项
                        if (nativeOption) nativeOption.disabled = false;
            ...
            // 初始化时触发一次，确保UI状态正确
            novelaiModelSelect.dispatchEvent(new Event('change'));
        }
        ...
        tabButtons.forEach(tab => {
            tab.addEventListener('click', function () {
                ...
                // 显示对应的内容区域
                const contentId = this.getAttribute('data-tab') + '-tab';
        ...
        if (modeSelectElement) {
            modeSelectElement.addEventListener('change', function () {
                document.getElementById('sd-api-settings').style.display = this.value === 'sd' ? 'block' : 'none';
                document.getElementById('novelai-api-settings').style.display = this.value === 'novelai' ? 'block' : 'none';
                document.getElementById('comfyui-api-settings').style.display = this.value === 'comfyui' ? 'block' : 'none';
            });
        }

        // 监听NovelAI模型选择变化
        const novelaiModelSelectElement = document.getElementById('novelaimode');
        if (novelaiModelSelectElement) {
            novelaiModelSelectElement.addEventListener('change', updateNovelAISettings);
            // 初始化时调用一次
     ...
     * 若用户未填写或填写为空，则使用默认地址
     */
    function getNovelAiEndpoint() {
        const custom = settings.novelaiUrl && settings.novelaiUrl.trim();
        if (custom) {
            return removeTrailingSlash(custom);
        }
        return 'https://1316649238-4k03zsyk0n.ap-singapore.tencentscf.com/ai/generate-image';
    }
    ...
    function saveSettings() {
        for (const key of Object.keys(defaultSettings)) {
            ...
            if (element) {
                ...
                if (key == "sdUrl" || key == "novelaiUrl") {

                    if (element.value !== settings[key]) {

                        element.value = removeTrailingSlash(element.value);
                        if (!confirm("你修改了URL，新的URL是：" + element.value + "。确认使用此URL吗？如果不正确，请取消并修改。")) {
                            return;
                        }
                        // return false;
                ...
                if (key == "height" || key == "width") {
                    // 如果正在修改宽度或高度，计算总像素
                    let currentWidth = key == "width" ? Number(element.value) : Number(document.getElementById("width").value);
                    let currentHeight = key == "height" ? Number(element.value) : Number(document.getElementById("height").value);

                    // 计算总像素
                    let totalPixels = currentWidth * currentHeight;

                    // 判断是否超过1024×1024=1048576像素
                    if (element.value !== settings[key] && totalPixels > 1048576 && settings["mode"] == "novelai") {
                        if (!confirm(`你修改了nai3的宽高，总像素(${currentWidth}×${currentHeight}=${totalPixels})超过1024×1024=1048576将会收费，确认继续？如果不清楚请点取消刷新网页恢复默认！`)) {
                            return;
                        }
                    }
                }
    ...
    async function nai3(button1, Xwidth = null, Xheight = null) {
        ...
        if (button1.dataset.link.includes("Scene Composition") && (settings.novelaimode == "nai-diffusion-4-curated-preview" || settings.novelaimode == "nai-diffusion-4-full" || settings.novelaimode == "nai-diffusion-4-5-curated")) {
            Divide_roles = true;
        }
        console.log("Divide_roles", Divide_roles);

        let access_token = settings.novelaiToken//填写你的novelai的token，在官方网站的设置  Account 里 Get Persistent API Token
        ...
        let aqt = "";
        if (settings.AQT != '' && (settings.novelaimode == "nai-diffusion-4-curated-preview" || settings.novelaimode == "nai-diffusion-4-5-curated")) {
        ...
        // 检查是否是NAI4/NAI4.5系列模型
        const isNai4Model = settings.novelaimode.startsWith("nai-diffusion-4");
        const isNai45Model = settings.novelaimode === "nai-diffusion-4-5-curated";

        // 根据模型类型设置适当的噪点排程
        let noiseSchedule = settings.Schedule;
        if (isNai4Model || isNai45Model) {
            // 对于NAI4和NAI4.5模型，建议使用karras
        ...
        let preset_data = {
            ...
            "n_samples": 1,
            "ucPreset": 3,//预设
            "qualityToggle": isNai45Model ? true : false,
            "dynamic_thresholding": settings.nai3Deceisp === "false" ? false : true,
            "controlnet_strength": 1,
            "legacy": false,
            "add_original_image": isNai4Model || isNai45Model ? true : false,
        ...
        // 仅为NAI3模型添加特定参数
        if (!isNai4Model && !isNai45Model) {
            preset_data.sm = settings.sm === "false" ? false : true;
            preset_data.sm_dyn = settings.dyn === "false" || settings.sm === "false" ? false : true;
            preset_data.skip_cfg_above_sigma = settings.nai3Variety === "false" ? null : 19.343056794463642;
            preset_data.legacy_v3_extend = false;
        }

        // 为NAI4和NAI4.5模型添加特定参数
        if (isNai4Model || isNai45Model) {
            if (Divide_roles) {
                let characterPrompts = [];
                for (let i = 1; i <= 4; i++) {
                    if (prompt_data[`Character ${i} Prompt`]) {
                        characterPrompts[i - 1] = {
                            prompt: prompt_data[`Character ${i} Prompt`],
                            center: prompt_data[`Character ${i} coordinates`],
                            uc: prompt_data[`Character ${i} UC`] ? prompt_data[`Character ${i} UC`] : 'one arms,lowres, aliasing, jaggy lines,bad hands,one legs'
                        };
                    }
                }
                ...
                v4_negative_prompt["legacy_uc"] = false;

                for (let i = 1; i <= 4; i++) {
                    if (prompt_data[`Character ${i} Prompt`]) {
                        v4_negative_prompt["caption"]["char_captions"].push({
                            char_caption: prompt_data[`Character ${i} UC`] ? prompt_data[`Character ${i} UC`] : 'one arms,lowres, aliasing, jaggy lines,bad hands,one legs',
                ...
                for (let i = 1; i <= 4; i++) {
                    if (prompt_data[`Character ${i} Prompt`]) {
                        v4_prompt["caption"]["char_captions"].push({
                ...
                preset_data["use_coords"] = true;
                preset_data["characterPrompts"] = characterPrompts;
                preset_data["v4_prompt"] = v4_prompt;
                preset_data["v4_negative_prompt"] = v4_negative_prompt;
            } else {
                preset_data["use_coords"] = false;
                preset_data["characterPrompts"] = [];
                preset_data["v4_prompt"] = {
                    "caption": {
                        "base_caption": prompt,
                        "char_captions": []
                    },
                ...
                preset_data["v4_negative_prompt"] = {
                    "caption": {
        ...
        const endpoint = getNovelAiEndpoint();
    ...
    async function sd(button1, Xwidth = null, Xheight = null) {
        ...
        // 构建payload，如果选择了模型则添加到请求中
        const payload = {
            "prompt": prompt,
            "negative_prompt": negative_prompt,
            "steps": parseInt(unsafeWindow.steps),
            "sampler_name": unsafeWindow.sampler_name,
            "width": Xwidth ? parseInt(Xwidth) : parseInt(unsafeWindow.width),
            "height": Xheight ? parseInt(Xheight) : parseInt(unsafeWindow.height),
            "restore_faces": unsafeWindow.restore_faces === "true",
            "cfg_scale": parseFloat(unsafeWindow.cfg_scale),
            "seed": settings.seed === 0 || settings.seed === "0" || settings.seed === "" ? -1 : parseInt(settings.seed)
        };

        // 如果选择了模型，添加到请求
        if (settings.sdModel) {
            payload.override_settings = {
                "sd_model_checkpoint": settings.sdModel
            };
        }
        ...
        try {
            ...
            const urlObj = new URL(apiUrl + "/sdapi/v1/txt2img");
            xiancheng = false;

            // 准备headers，添加Basic认证（如果设置了用户名和密码）
            let headers = {
                "Content-Type": "application/json",
                "Access-Control-Allow-Origin": "*"
            };

            if (settings.sdUsername && settings.sdPassword) {
                const authString = `${settings.sdUsername}:${settings.sdPassword}`;
            ...
            for (let i of r['images']) {
                ...
                if (settings.sdUsername && settings.sdPassword) {
                    const authString = `${settings.sdUsername}:${settings.sdPassword}`;
                    const base64Auth = btoa(authString);
                    pngInfoHeaders["Authorization"] = `Basic ${base64Auth}`;
                }

                const response2 = await new Promise((resolve, reject) => {
                    GM_xmlhttpRequest({
                        method: "POST",
                        url: `${apiUrl}/sdapi/v1/png-info`,
                        data: JSON.stringify(png_payload),
                        headers: pngInfoHeaders,
                        onload: resolve,
                        onerror: reject
                    });
                });
    ...
    let kunlun = `1.修补</输出>|0.最大截取上下文|2.输出提取|3.昆仑去除其他|4.昆仑去掉\\n|5.用户发言去掉\\n|6.用户发言|7.最后用户发言|8.角色全部总结|9.替换/n|10.提取角色最后总结|11.换回/n|12.资料|13.隐藏自我总结显示|14.隐藏输出|15.选择去除`
    let karukaru = `[1]去除</结束无效提示>|[2]去除思维链|[3]过滤AI中人类输出|[4]过滤AI中人类输出 外包围|[5]隐藏动态记忆输出|[5-1]减少动态记忆保留|[6]隐藏<场景图>|[7]小说任务隐藏|[8]仅保留一个小说任务|[9]六个选项过滤优化|[10]<双语模式过滤>|[11]<双语模式过滤中文左右标签>|[12]段落数标记过滤|[13][过滤user指向角色内容]|[14]保留最后1回合正文(务必开启\`创作响应结构\`中"即时总结")|[15]保留最新状态栏|[16]用户消息转移[置空所有]|[17]独立认知滤网过滤`
    let oldXHR = unsafeWindow.XMLHttpRequest;
    // 定义默认设置
    const defaultSettings2 = {
        scriptEnabled2: false,
        res: { "昆仑": kunlun, "karukaru": karukaru },
        reid: "昆仑",
        re: ""
    };
    let settings2 = {};
    ...
    for (const [key, defaultValue] of Object.entries(defaultSettings2)) {

        settings2[key] = GM_getValue(key, defaultValue);


        // 如果没有读取到值，就使用默认值并保存
        if (settings2[key] === defaultValue) {
            ...
            settings2[key] = JSON.parse(GM_getValue(key));
    ...
    settings2["re"] = settings2.res[settings2.reid];





    function createSettingsPanel2() {

        console.log(settings2.res)
        console.log(settings2.re)
        const panel = document.createElement('div');
        panel.id = 'settings-panel2';
        panel.style.position = 'absolute';
        panel.style.top = '50%';
        panel.style.left = '50%';
        panel.style.transform = 'translate(-50%, -50%)';
  ...
  <h2>设置面板</h2>
    <label class="switch">
  <input type="checkbox" id="scriptToggle2" ${settings2.scriptEnabled2 ? 'checked' : ''}>
  <span class="slider"></span>
  </label>
  <label for="scriptToggle" style="display: inline-block; margin-left: 10px;">启用脚本</label>
  <br><br>
  ...
  <div class="inline-elements">正则设置:
      <select id="reid">`
        for (const [key, defaultValue] of Object.entries(settings2.res)) {
            html += `
        <option value="${key}"  ${settings2.reid === key ? 'selected' : ''}>${key}</option>
       `
        }
        html +=
            `  </select>
  
    <div class="menu_button interactable" title="保存风格" data-i18n="[title]Save style" id="res_save_style" tabindex="0">
                    <i class="fa-solid fa-save"></i>
                </div>
  
    <div class="menu_button interactable" title="删除风格" data-i18n="[title]Delete style" id="res_delete_style" tabindex="0">
                    <i class="fa-solid fa-trash-can"></i>
                </div>
    </div>
  <br>
  
    <label>开启的正则组合：</label><br>
    <textarea class="text_pole textarea_compact" id="re" style="height: 55px;">${settings2.re}</textarea>
  
  <br><br>
  <button id="onle-re">仅开启此正则(关闭其他)</button>
  <br><br>
  <button id="open-re">开启此正则</button>
  <br><br>
  <button id="close-nowre">关闭此正则</button>
  <br><br>
  <button id="del-re">删除此正则</button>
  <br><br>
  
  <button id="open-allre">开启所有正则</button>
  <br><br>
  <button id="close-re">关闭所有正则</button>
  <br><br>
  <button id="del-allre">删除所有正则</button>
  <br><br>
  
  <label>导入正则:
      <input type="file" id="zipInput" accept="zip/*">
      <button type="button" onclick="document.getElementById('zipInput').click();">选择zip导入</button>
      </label>
  <br><br>
  
  <button id="save-settings2">保存设置</button>
  <button id="close-settings2">关闭</button>
  
  <a id="visit-website-link" href="https://gxcgf4l6b2y.feishu.cn/docx/Hha8dD4M6ofTCsx4LoxcAhIInEd?from=from_copylink" target="_blank">帮助</a>
  <a id="visit-website-link" href="https://discord.com/channels/1134557553011998840/1331642875393474611/1331642875393474611" target="_blank">dc讨论</a>
  ...
  `;
        ...
        document.body.appendChild(panel);
        document.head.appendChild(style);
        document.getElementById('save-settings2').addEventListener('click', function saveSettings2() {
            settings2.scriptEnabled2 = document.getElementById('scriptToggle2').checked;
            settings2.reid = document.getElementById('reid').value;
            settings2.re = document.getElementById('re').value;
            settings2.novelaiUrl = document.getElementById('novelaiUrl').value;
            settings2.novelaiToken = document.getElementById('novelaiToken').value;
            GM_setValue('scriptEnabled2', settings2.scriptEnabled2);
            GM_setValue('reid', settings2.reid);
            GM_setValue('re', settings2.re);
            GM_setValue('novelaiUrl', settings2.novelaiUrl);
            GM_setValue('novelaiToken', settings2.novelaiToken);
        });
        document.getElementById('close-settings2').addEventListener('click', closeSettings2);
        document.getElementById('del-allre').addEventListener('click', delallre);
        document.getElementById('open-re').addEventListener('click', openre);
        document.getElementById('onle-re').addEventListener('click', onlere);
        document.getElementById('close-re').addEventListener('click', closere);
        document.getElementById('del-re').addEventListener('click', delre);
        document.getElementById('open-allre').addEventListener('click', openallre);
        document.getElementById('close-nowre').addEventListener('click', closenowre);

        document.getElementById('zipInput').addEventListener('change', handleZipUpload);

        // 添加滑块切换事件监听器
        document.getElementById('scriptToggle2').addEventListener('change', function () {
            settings2.scriptEnabled2 = this.checked;
            GM_setValue('scriptEnabled2', this.checked);
            console.log('Script ' + (this.checked ? 'enabled' : 'disabled'));

            if (settings2.scriptEnabled2) {
                // 启用脚本
                console.log('启用脚本');
            } else {
                // 禁用脚本
                console.log('关闭脚本');
                unsafeWindow.XMLHttpRequest = oldXHR;  //开启监控
                // 在这里执行禁用脚本的逻辑
            }
        ...
        document.getElementById('reid').addEventListener('change', res_change);

        document.getElementById('res_save_style').addEventListener('click', res_save);
        document.getElementById('res_delete_style').addEventListener('click', res_delete);
        return panel;
    }
    ...
    async function handleZipUpload(event) {
        ...
        if (file.name.includes('zip')) {
            try {
                ...
                for (const [filename, fileEntry] of Object.entries(zipFile.files)) {
                    // 跳过目录（ZIP 中的目录通常以斜杠结尾）
                    if (fileEntry.dir || filename.endsWith('/')) {
                        continue;
                    }
                    ...
                    if (filename.toLowerCase().endsWith('.json')) {
                        try {
                            ...
                            pureName = pureName.split('.')[0] || '0'; // 防御空路径
                            ...
                            console.error(`解析 ${filename} 时出错:`, parseError);
                        }
                    }
                }
                rejsons = textrejsons.sort((a, b) => {
                    const getNumber = (filename) => {
                        const match = filename.match(/\d+/);
                        return match ? parseInt(match[0], 10) : 0;
                    }
                    return getNumber(a.filename) - getNumber(b.filename)
                }).map(item => item.content) // 提取纯内容

                unsafeWindow.XMLHttpRequest = fuckXHR;
                if (document.getElementById('update_oai_preset')) {
                    isrejsons = true;
                    const element = document.getElementById('update_oai_preset');
                ...
                console.error('读取 ZIP 文件时出错:', error);
    ...
    function res_save() {
        ...
        stylInput("请输入配置名称").then((result) => {
            if (result) {
                ...
                const selectElement = document.getElementById("reid");
                // 创建新的option元素
                let newOption = new Option(result, result);

                // 将新的option添加到select中
                if (!settings2.res.hasOwnProperty(result)) {
                    selectElement.add(newOption);
                    selectElement.value = result;
                }
                settings2.reid = result;
                settings2.res[result] = re.value;
                settings2.re = re.value;
                GM_setValue("res", JSON.stringify(settings2.res));
                GM_setValue("reid", settings2.reid);
                GM_setValue("re", JSON.stringify(settings2.re));
                return true;
            } else {
                return false;
            }
    ...
    function res_delete() {
        ...
        stylishConfirm("是否确定删除").then((result) => {
            if (result) {
                ...
                // negativePrompt.value="";
                //  fixedPrompt.value="";
                Reflect.deleteProperty(settings2["res"], selectElement.value);
                selectElement.remove(selectElement.selectedIndex);
                selectElement.value = "昆仑";
                settings2.reid = "昆仑";
                settings2.re = settings2["res"][settings2["reid"]];//设置提示词
                re.value = settings2["res"][settings2["reid"]];//设置提示词
                GM_setValue("reid", settings2["reid"]);
                GM_setValue("res", JSON.stringify(settings2["res"]));
                GM_setValue("re", JSON.stringify(settings2["re"]));
                // GM_setValue("negativePrompt","");
    ...
    function delre() {
        ...
        unsafeWindow.XMLHttpRequest = fuckXHR;
        stylishConfirm("是否确定删除正则").then((result) => {
            if (result) {
    ...
    function delallre() {
        ...
        unsafeWindow.XMLHttpRequest = fuckXHR;
        stylishConfirm("是否确定删除所有正则").then((result) => {
            if (result) {
                if (document.getElementById('update_oai_preset')) {
                    isdelallre = true;
                    const element = document.getElementById('update_oai_preset');
    ...
    function openallre() {
        ...
        unsafeWindow.XMLHttpRequest = fuckXHR;
        stylishConfirm("是否确定开启所有正则").then((result) => {
            if (result) {
                if (document.getElementById('update_oai_preset')) {
                    isopenallre = true;
                    const element = document.getElementById('update_oai_preset');
    ...
    function res_change() {
        ...
        settings2["reid"] = selectElement.value;
    ...
    function showSettingsPanel2() {

        const panel = document.getElementById('settings-panel2');
        if (!panel) {
            createSettingsPanel2();
        }
        document.getElementById('settings-panel2').style.display = 'block';
    }

    function hideSettingsPanel2() {
        document.getElementById('settings-panel2').style.display = 'none';
    }

    function saveSettings2() {
        for (const key of Object.keys(defaultSettings2)) {
    ...
    function fuckXHR() {
        ...
        xhr.open = function (method, url, async, user, password) {
            ...
            if (url == "/api/settings/save") {
                sendmodel = "/api/settings/save"
            }
            else if (url == "/api/characters/get") {

                sendmodel = "/api/characters/get"
            } else {
                sendmodel = ""
        ...
        xhr.send = function (data) {
            if (data != null) {
                try {
                    if (sendmodel == "/api/settings/save") {
                        unsafeWindow.XMLHttpRequest = oldXHR;  //关闭监控
                        const jsonData = JSON.parse(data);
                        if (iscloseAllre) {
                            iscloseAllre = false;
                            if (jsonData.extension_settings.hasOwnProperty("regex")) {
                                let allregex = jsonData.extension_settings.regex
                                console.log("allregex:", allregex);
                                //   active_character[0].disabled=false;

                                for (let i = 0; i < allregex.length; i++) {
                                    allregex[i].disabled = true;
                                }

                                jsonData.extension_settings.regex = allregex;
                        ...
                        if (isopenres) {
                            ...
                            if (jsonData.extension_settings.hasOwnProperty("regex")) {
                                let allregex = jsonData.extension_settings.regex
                                console.log("allregex:", allregex);
                                let ress = settings2.re.split("|");

                                for (let i = 0; i < allregex.length; i++) {
                                    if (ress.includes(allregex[i].scriptName)) {
                                        allregex[i].disabled = false;
                                    }
                        ...
                        if (isdelre) {
                            ...
                            if (jsonData.extension_settings.hasOwnProperty("regex")) {
                                ...
                                for (let i = 0; i < allregex.length; i++) {
                                    if (!ress.includes(allregex[i].scriptName)) {
                                        exre.push(allregex[i])
                                    }
                                }

                                jsonData.extension_settings.regex = exre;
                                data = JSON.stringify(jsonData);
                                return oldSend.call(this, data);
                            }
                        ...
                        if (isrejsons) {
                            ...
                            if (jsonData.extension_settings.hasOwnProperty("regex")) {
                                ...
                                // 找出 rejsons 中名称不在 allregex 中的元素
                                const uniqueRejsons = rejsons.filter(
                                    rejson => !allregex.find(regex => regex.scriptName === rejson.scriptName)
                                );

                                // 合并数组
                                let exre = [...allregex, ...uniqueRejsons];
                        ...
                        if (isdelallre) {
                            ...
                            if (jsonData.extension_settings.hasOwnProperty("regex")) {
                                let allregex = jsonData.extension_settings.regex
                                console.log("allregex:", allregex);
                                jsonData.extension_settings.regex = [];
                                data = JSON.stringify(jsonData);
                                return oldSend.call(this, data);
                            }
                    ...
                    if (sendmodel == "/api/characters/edit") {
                        for (let [name, value] of data) {
                            if (name == "description") {
                                console.log(`232444444444444444444444+${name}: ${value}`);
                            }
                        }
                        console.log("active_jsonData:", data);
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                    ...
                    if (sendmodel == "/api/characters/get") {
            ...
            if (sendmodel == "/api/characters/edit") {
                //  arguments[0].description="策划测试"+arguments[0].description;
                // console.log("XHR data:", data);
                // console.log("XHR arguments[0]:", arguments[0]);
    ...
    // 添加加载SD模型列表的函数
    async function loadSDModelList() {
        try {
            const url = settings.sdUrl;
            if (!url) {
                alert("请先设置SD URL");
                return;
            }
            ...
            apiUrl = removeTrailingSlash(apiUrl);
            apiUrl = apiUrl + "/sdapi/v1/sd-models";
            ...
            const response = await new Promise((resolve, reject) => {
                GM_xmlhttpRequest({
                    ...
                    onerror: function (error) {
                        alert("请求错误，请检查SD URL是否正确以及SD是否已启动");
                        reject(error);
                    }
                });
            });
            ...
            if (Array.isArray(models) && models.length > 0) {
                ...
                models.forEach(model => {
                    settings.sdModelList[model.title] = model.model_name;
                });

                // 更新存储
                GM_setValue('sdModelList', JSON.stringify(settings.sdModelList));

                // 更新模型选择下拉框
                updateSDModelSelect();

                alert(`成功加载了 ${models.length} 个模型`);
            } else {
                alert("未找到可用模型");
            }
        } catch (error) {
            console.error("加载模型列表出错:", error);
            alert("加载模型列表出错: " + error.message);
        }
    }

    // 更新SD模型选择下拉框
    function updateSDModelSelect() {
        const selectElement = document.getElementById('sdModel');
        ...
        // 添加模型选项
        Object.entries(settings.sdModelList).forEach(([modelId, modelName]) => {
            const option = document.createElement('option');
            option.value = modelId;
            option.textContent = modelName;
            option.selected = (settings.sdModel === modelId);
            selectElement.appendChild(option);
        });
    }

    // 添加更新NovelAI设置的函数，根据模型类型动态显示/隐藏选项
    function updateNovelAISettings() {
        const modelSelect = document.getElementById('novelaimode');
        if (!modelSelect) return;

        const selectedModel = modelSelect.value;
        const isNai4Model = selectedModel.startsWith('nai-diffusion-4');
        const isNai45Model = selectedModel === 'nai-diffusion-4-5-curated';

        // SM设置选项（只在NAI3模型中显示）
        const smOption = document.getElementById('sm-option');
        const dynOption = document.getElementById('dyn-option');
        const varietyOption = document.getElementById('nai3Variety-option');

        if (smOption) smOption.style.display = isNai4Model || isNai45Model ? 'none' : 'block';
        if (dynOption) dynOption.style.display = isNai4Model || isNai45Model ? 'none' : 'block';
        if (varietyOption) varietyOption.style.display = isNai4Model || isNai45Model ? 'none' : 'block';

        // 噪点设置（noise_schedule）
        const scheduleSelect = document.getElementById('Schedule');
        if (scheduleSelect) {
            if (isNai4Model || isNai45Model) {
                // 如果是NAI4或NAI4.5模型，推荐使用karras
                const nativeOption = scheduleSelect.querySelector('option[value="native"]');
                if (nativeOption) {
                ...
                // 添加提示信息
                const scheduleLabel = document.querySelector('label[for="Schedule"]');
                if (scheduleLabel && !document.getElementById('schedule-hint')) {
                    const hint = document.createElement('small');
                    hint.id = 'schedule-hint';
                    hint.style.color = '#666';
                    hint.style.display = 'block';
                    hint.textContent = 'NAI4/4.5模型推荐使用karras';
                    scheduleLabel.parentNode.insertBefore(hint, scheduleLabel.nextSibling);
                }
            } else {
                // NAI3模型不需要限制
                const nativeOption = scheduleSelect.querySelector('option[value="native"]');
