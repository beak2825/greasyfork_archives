// ==UserScript==
// @name         Kosgeb Ders Atlayıcı
// @namespace    http://tampermonkey.net/
// @version      0.1
// @description  Kosgeb Girişimcilik Eğitimlerinde videoları izlemek zorunda kalmadan atlayabilirsiniz video altında geç butonu belirecektir...
// @author       NewSubject
// @match        https://lms.kosgeb.gov.tr/mod/hvp/view.php?id=*
// @icon         https://www.kosgeb.gov.tr/Web/Content/assets/img/logo.png
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/432417/Kosgeb%20Ders%20Atlay%C4%B1c%C4%B1.user.js
// @updateURL https://update.greasyfork.org/scripts/432417/Kosgeb%20Ders%20Atlay%C4%B1c%C4%B1.meta.js
// ==/UserScript==
 /**
...
function test(){
    console.log("Script Çalıştı")
     $("#page-content").append("<button id='myGecBtn'>Tamam La Geç İşte</button>");
    url = H5PIntegration.ajax.setFinished;
    url = url + '&score=1&maxScore=1';
    console.log(url);
    $( "#myGecBtn" ).on( "click", gec );


    // $('<input type="button" value="Tamam La Geç" id="gec"/>').appendTo('#page-content');
    // console.log($("#maincontent"));
    // var cid = Object.keys(H5PIntegration.contents)[0];
    // cid = cid.split('-')[1];
    // console.log(cid);
    // var instance = getH5PInstance(parseInt(cid));
    // console.log(instance);
    // var xapidata = instance.getXAPIData();
    // console.log(xapidata);
    // var xapistr = JSON.stringify(xapidata);
    // console.log(xapistr);
};
...
function gec(){
       $.ajax({
            ...
            error: function (data) {
                alert("Hata Oldu Babuş Geçemedik Bu Dersi\n" + data);
            }
        });
};
