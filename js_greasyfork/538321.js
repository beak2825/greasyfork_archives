// ==UserScript==
// @name         LBD-JiraHelper-Copier
// @namespace    lbd.jira.copier
// @version      2.2.2
...
// @match        https://msdevopsjira.fsc.atos-services.net/browse/*
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/538321/LBD-JiraHelper-Copier.user.js
// @updateURL https://update.greasyfork.org/scripts/538321/LBD-JiraHelper-Copier.meta.js
// ==/UserScript==
...
(function() {
    ...
    async function fetchGitLabMerges(currentURL) {
        ...
        if (match) {
            const gitHubToken = "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"; // ← PUT YOUR TOKEN HERE (safely!)
            let gitLabApiUrl = 'https://github.gsissc.myatos.net/api/v3/search/issues?q=repo:NL-EHV-NWC/LBDash4+is:pr+' + match[0];
            //console.info(gitLabApiUrl);
            fetch(gitLabApiUrl, {
                method: "GET",
                mode: "cors",
                cache: "no-cache",
                ...
                let mrm = 0;
                let mrc = 0;
                data?.items?.forEach(mr => {
                    if(mr.closed_at === null && mr.pull_request.merged_at === null) mrd++;
                    if(mr.closed_at !== null && mr.pull_request.merged_at === null) mrc++;
                    if(mr.pull_request.merged_at !== null) mrm++;
                });
    ...
    function hookTheJiraMonkey() {
        ...
        function copyToClipboard(el, option) {
            ...
            if(option !== 'id') {
                let title = document?.querySelector("div[id='summary-val']")?.childNodes[0]?.innerHTML?.trim() ?? 'unknown';
                if(title === 'unknown'){
                    title = el?.parentNode?.parentNode?.parentNode?.querySelector("dd[id='summary-val']")?.innerText ?? 'unknown';
                }
    ...
    function goToUrl(el){
    	let baseUrl = 'https://github.gsissc.myatos.net/NL-EHV-NWC/LBDash4/pulls?q=is%3Apr+';
        let id = document?.querySelector("a[id='key-val']")?.innerHTML?.trim() ?? 'unknown';
        if(id === 'unknown'){
        ...
        let announcements = document?.querySelector("#announcement-banner");
