...
// @description Agencement différent de l'accueil
// @downloadURL https://update.greasyfork.org/scripts/601/AutoNovAccueilMatrice.user.js
// @updateURL https://update.greasyfork.org/scripts/601/AutoNovAccueilMatrice.meta.js
// ==/UserScript==

(function() {
    
    var auth = localStorage.getItem('SIF_NovA-ccueilMa-trice');
    if(auth!=null && auth=="on") 
    {   
       $(document.getElementById('liste_forums').children[21].children[2]).css('display','none');

         myRequest = new XMLHttpRequest();
                var url = "https://www.dreadcast.eu/Forum/1-22-forum-prive?1";
                ...
                $(forums).find('.peut_voir').each(function () {
                    if($(this).attr('href').indexOf('-cercle-')!=-1 && !this.classList.contains("vu"))
                    {
                       var tmpNode = this;
                ...
                headerForum.innerHTML ='<font size="2" color="gray">Accueil amélioré par SIF.</font><br><br>';
    ...
    else
    {
        ...
        $('#supprMess').text("Suppression définitive de vos EMs en cours.");
...
function loadBan()
{
    $.ajax({
        type: 'GET',
        url: "https://docs.google.com/uc?export=download&id=0B4Igp0h82K3yd0tHYV8yN1FqMTg",
        async: true,
    ...
    if(myRequest.readyState == 4) {
       if(myRequest.status == 200) {
          ...
          if ($(result).find('#liste_sujets').length) {
              ...
              $(document.getElementById('liste_forums').children[6].children[2]).css('display','block');
           } else {
