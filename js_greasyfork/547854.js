// ==UserScript==
// @name         Letterboxd → FLAM (Fenlight)Accurate Cache Stats
// @namespace    http://tampermonkey.net/
// @version      10.0.0
// @description  Send Letterboxd lists, watchlists
...
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/547854/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29Accurate%20Cache%20Stats.user.js
// @updateURL https://update.greasyfork.org/scripts/547854/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29Accurate%20Cache%20Stats.meta.js
// ==/UserScript==
...
(async function () {
  ...
  function isTargetAllowedPage() {
    ...
    const isList = /^\/[^/]+\/list\/[^/]+\/(?:page\/\d+\/)?$/.test(p);
    const isWatch = /^\/[^/]+\/watchlist\/(?:page\/\d+\/)?$/.test(p);
  ...
  // NEW: cap concurrent pagination fetches (requested: 3)
  ...
  // Robust watchlist author capture: prefer <a href="/{user}/">Display Name</a> on THIS page.
  function getWatchlistAuthorFromPage(username) {
    try {
      username = username || getUsernameFromPath();
      if (!username) return '';

      // 1) Exact anchor match first
      let el = document.querySelector(`a[href="/${username}/"]`);
      let text = normalizeText(el?.textContent);

      // 2) If empty, scan all anchors with exact href and pick best non-slug text
      if (!text) {
        ...
        for (const a of candidates) {
          ...
          } else if (!best) best = t; // keep slug as weakest fallback
        }
        text = best;
      }

      // 3) OG title fallback: "Watchlist of Dave Vis • Letterboxd"
      if (!text) {
  ...
  // Cached user-profile meta (fanart + bio)
  ...
  async function fetchUserProfileMeta(username) {
    ...
    try {
      ...
      // Fanart: prefer #backdrop[data-backdrop], then .backdropimage style
      let fanartUrl = '';
  ...
  function askForActionOverlay() {
    return new Promise((resolve) => {
      ...
      panel.innerHTML = `
        ...
        <div style="font-size:13px; color:#bbb; margin:0 0 14px;">Which Kodi action do you want to use for this run?</div>
  ...
  async function buildUrlFromCache(items, descriptionText, actionOverride = null) {
    ...
    // Author: prefer watchlist-page anchor display name (emoji preserved)
