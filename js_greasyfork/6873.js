...
// @grant          none
// @description    en 
// @downloadURL https://update.greasyfork.org/scripts/6873/My%20Match%20Analyzer.user.js
// @updateURL https://update.greasyfork.org/scripts/6873/My%20Match%20Analyzer.meta.js
// ==/UserScript==
...
function clickAnalyze()
{
	...
	else
	{
		...
		myform.action = "http://trophyhelptool.herobo.com/mma/report.php";
		myform.submit();
	}
}
...
function clickSave()
{
	...
	{
		myform = document.getElementById("mma_form");
		myform.action = "http://trophyhelptool.herobo.com/mma/savereport.php";
		myform.submit();
	}
}
...
window.addEventListener('load', function (e)
{
	...
	var fileref=document.createElement('script');
	fileref.setAttribute("type","text/javascript");
	fileref.setAttribute("src", "http://trophyhelptool.herobo.com/mma/json2.js");
	
	document.getElementsByTagName("head")[0].appendChild(fileref);
	
	loadJS("http://trophyhelptool.herobo.com/mma/json2.js");
	
	addCss(".mma_big { height: 30px; width: 400px; background: url(http://trophyhelptool.herobo.com/mma/scr_back.png); position: fixed; bottom: 0px; left: 10px; box-shadow: 1px -1px 10px black; border-radius: 10px 10px 0px 0px; z-index: 10;}");
	...
	addCss(".mma_analyze:hover { background: url(http://trophyhelptool.herobo.com/mma/analyze_hover.png); cursor: pointer; }");
