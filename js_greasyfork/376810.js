...
// @grant          GM_getValue
// @grant          GM_setValue
// @downloadURL https://update.greasyfork.org/scripts/376810/PardusMemberInfo.user.js
// @updateURL https://update.greasyfork.org/scripts/376810/PardusMemberInfo.meta.js
// ==/UserScript==
...
function displayStats() {
	...
	if (flag_getData) {
		...
		var regexp1 = /alt=\S(\w*\s?\w*\s?\w*\s?\w+)\S\stitle=\SView/gi;	//JQ -- capture only the ones that are followed by 'View( profile')

		var ship;

		while (result == regexp1.exec(html)) {
		...
		var regexp10 = /<td>(<span.*<\/span>)<\/td>/gi;	//get activeness but with format
		var active;

		while (result == regexp10.exec(html)) {
			active = result[1];
