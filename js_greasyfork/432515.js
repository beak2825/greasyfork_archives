...
// @connect lordswm.com
// @namespace 
// @downloadURL https://update.greasyfork.org/scripts/432515/GN_Market.user.js
// @updateURL https://update.greasyfork.org/scripts/432515/GN_Market.meta.js
// ==/UserScript==
...
//----------------------------------------------------------------------------//
    ...
    let isCustomArtProcessing = false
...
//----------------------------------------------------------------------------//
        ...
        this.get_artefact = function(id){
            ...
            if (!isCustomArtProcessing) {
                isCustomArtProcessing = true
                doGet(`https://www.lordswm.com/art_info.php?id=${id}`, (doc) => {
                    let repairPrice = Array.from(doc
                        ...
                        .replaceAll(",", "") - 0
                    customArts.push({
                        "ap": doc.body.innerHTML.match(/Ammunition points:<\/b> (\d{1,2})</)[1]-0,
                        "ex_states": {},
                        "id": id,
                        "kind": 1,
                        "lvl": 7,
                        "market_cat": enum_amc[document.body.innerHTML.match(new RegExp(`\\/auction\\.php\\?cat=([a-zA-Z_-]+)&sort=0&art_type=${id}`))[1]],
                        "name": "custom",
                        "price": 0,
                        "repair_cost": repairPrice,
                        "set": 0,
                        "slot": 0,
                        "states": {},
                        "type": 3
                    })
                    set("customArts", customArts)
                    location.reload()
                })
            }
...
//----------------------------------------------------------------------------//
    ...
    var ex_states = [
        ...
        { label: 'Допурон в ближнем, %:',  id: 'IncreaseRangeDamageInput' },
        { label: 'Допурон стрелкам, %:',   id: 'IncreaseCloseDamageInput' },
...
//----------------------------------------------------------------------------//
    ...
    function get_artefacts(parent){
        ...
        for(var i = 0; i < rows.length; ++i){
            ...
            var dur_el = id_el.querySelector('img');
            re      = /Прочность: (\d{1,3})\/(\d{1,3})/;
            matches = re.exec(dur_el.getAttribute('title'));

            var cur_dur   = +matches[1];
            var total_dur = +matches[2];
            ...
            var modificators = matches ? matches[1] : '';
