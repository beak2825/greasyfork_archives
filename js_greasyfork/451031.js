...
// @description:ja  サポートされている検索エンジンは、1。Baidu、2。Sogou、3。Google、4。Bingです。 「AC-baidu」スクリプトと互换性があります[カスタム背景のサポート] [ランダムな背景画像のサポート] [背景画像の分类のサポート] [シールド広告のサポート] [透明度の调整のサポート]
// @icon         https://upyun.calendarli.com/logo.png
// @author       只为你收集世间云朵（原作者，修改wwdboy）
// @license      MIT License
// @version      4.1.3
...
// @copyright:ja   このスクリプトは你收集世间云朵@ greasyforkによって完全にオリジナルです。コードの一部または全部をコピーしないでください。 参照コードがある场合は、スクリプトリンクを宣言してマークを付けてください。
// @downloadURL https://update.greasyfork.org/scripts/451031/%E6%90%9C%E7%B4%A2%E9%A1%B5%E9%9D%A2%E7%BE%8E%E5%8C%96-%E8%87%AA%E7%94%A8.user.js
// @updateURL https://update.greasyfork.org/scripts/451031/%E6%90%9C%E7%B4%A2%E9%A1%B5%E9%9D%A2%E7%BE%8E%E5%8C%96-%E8%87%AA%E7%94%A8.meta.js
// ==/UserScript==
...
(function() {
    ...
    var getVariable = (function(name) {
        /* 获取localStorage数据*/
        window.BackgroundTransparencyNumberValue = CL_LocalStorage.getItem('BackgroundTransparencyNumberValue', '0') //背景透明
        window.ListTransparencyNumberValue = CL_LocalStorage.getItem('ListTransparencyNumberValue', '0') //列表透明
        ...
        window.CustomizeURLChecked = CL_LocalStorage.getItem('CustomizeURLChecked', true) //自定义URL
    ...
    $('body').append(
        $('<div id="CL_Console">').append(
            ...
            $('<div id="CL_OperationPanel" class="none">').append(
                ...
                $('<div class="CustomizeURL">').append('<div><p class="Customize checkboxWrap" title="可填写图床URL"><input type="checkbox" name="CustomizeURL" value="自定义URL" default="true">自定义URL</p><input type="text" name="CustomizeUrlValue"></div>')
            ).append(
                $('<div class="transparency">').append(
                    $('<span>').append('背景透明度：')
                ).append('<select class="BackgroundSelector" name="BackgroundTransparency" cl-select-number defaultValue="0" maxValue="1" minValue="0" stepValue="0.1"></select>').append(
                    $('<span>').append('列表透明度：')
                ).append('<select class="ListSelector" name="ListTransparency" cl-select-number defaultValue="0" maxValue="1" minValue="0" stepValue="0.1"></select>')
            ).append(
     ...
     * 默认调整样式
     */
    $('div#rs,span.fk,#botstuff,#b_results>.b_ans,div#promotion_adv_container,.hintBox,#leftbottomleadContainer').hide()
