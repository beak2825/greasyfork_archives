// ==UserScript==
// @name               AI Question Navigation sidebar
// @name:zh-CN         AI 问题导航侧边栏
// @namespace          npm/ai-question-navigator
// @version            3.0.0
// @author             okokdi
// @description        It provides a convenient question sidebar directory for AI chat websites like ChatGPT and Grok. It automatically collects the questions asked by users on the current session page and displays them on the sidebar, enabling quick navigation to the location of historical questions.
// @description:zh-CN  为ChatGPT和Grok等AI聊天网站提供了一个便捷的问题侧边栏目录。它能够自动搜集当前会话页面的用户提的问题，并展示在侧边栏上，提供快速导航到历史问题的位置的能力。
// @license            MIT
...
// @downloadURL https://update.greasyfork.org/scripts/535968/AI%20Question%20Navigation%20sidebar.user.js
// @updateURL https://update.greasyfork.org/scripts/535968/AI%20Question%20Navigation%20sidebar.meta.js
// ==/UserScript==
...
(function () {
  ...
  class GrokAdapter {
    ...
    getQuestionElements() {
      const elements = Array.from(
        ...
        return el2.classList.contains("bg-surface-l2");
      });
      return elements;
    }
    /**
     * 获取当前活跃问题的索引
     */
    getCurrentQuestionIndex(elements) {
      if (elements.length === 0)
        return -1;
      const viewportHeight = window.innerHeight;
      const viewportCenter = viewportHeight / 2;
      const distances = elements.map((el2) => {
      ...
      distances.forEach((distance, index) => {
        var _a;
        const rect = (_a = elements[index]) == null ? void 0 : _a.getBoundingClientRect();
      ...
      if (closestIndex === -1) {
        const topIndex = elements.findIndex((el2) => {
          const rect = el2.getBoundingClientRect();
          return rect.top >= 0;
        });
        if (topIndex > -1) {
  ...
  class ChatGPTAdapter {
     ...
     */
    getQuestionText(element) {
      const authorElement = element.querySelector("[data-message-author-role]");
      return (authorElement == null ? void 0 : authorElement.innerText) || "";
    }
    /**
     * 判断页面是否已加载完成
     */
  ...
  class GeminiAdapter {
    ...
    getConversationContainer() {
      ...
      return document.querySelector(chatContainerSelector);
    }
    /**
     * 获取问题元素列表
     */
    ...
    getCurrentQuestionIndex(elements) {
      ...
      if (closestIndex === -1) {
        const topThreshold2 = 75;
        const topIndex = elements.findIndex((el2) => {
          const rect = el2.getBoundingClientRect();
          return rect.top >= topThreshold2;
        });
        if (topIndex > -1) {
          return topIndex === 0 ? 0 : topIndex - 1;
        } else if (elements.length > 0) {
    ...
    getQuestionText(element) {
      ...
      const text = (textEl == null ? void 0 : textEl.textContent) || "";
  ...
  function useQuestionEls() {
    const [questionEls, setQuestionEls] = reactExports.useState([]);
    ...
    reactExports.useEffect(() => {
      const updateQuestions = () => {
        ...
        if (adapter && adapter.isPageReady()) {
          ...
          setQuestions(elements.map((el2) => adapter.getQuestionText(el2)));
        } else {
      ...
      updateQuestions();
      const interval = setInterval(updateQuestions, 1e3);
  ...
  function useActiveQuestionIndex() {
    ...
    const scrollContainer = useMountedCallbackValue(() => {
      ...
      const adapter = adapterManager.getCurrentAdapter();
      if (adapter) {
        return ((_a = adapter.getConversationContainer()) == null ? void 0 : _a.parentElement) || null;
      }
      return (_b = queryChatContainer()) == null ? void 0 : _b.parentElement;
    });
  ...
  function App() {
    ...
    const currentSiteId = reactExports.useMemo(() => {
      const adapter = adapterManager.getCurrentAdapter();
      return (adapter == null ? void 0 : adapter.getSiteId()) || "";
    }, []);
    const useCustomListStyle = reactExports.useMemo(() => {
      return true;
    }, []);
...
.after\\:right-4px::after{right:4px;}
.box-border{box-sizing:border-box;}
.after\\:h-\\.7em::after{height:0.7em;}
.after\\:w-full::after,
.w-full{width:100%;}
.max-h-\\[var\\(--app-max-list-height\\)\\]{max-height:var(--app-max-list-height);}
.w-\\[var\\(--app-width\\)\\]{width:var(--app-width);}
.w-0{width:0;}
.w-22px{width:22px;}
.w-4px{width:4px;}
.flex{display:flex;}
...
.after\\:from-white::after{--un-gradient-from-position:0%;--un-gradient-from:rgba(255,255,255,var(--un-from-opacity, 1)) var(--un-gradient-from-position);--un-gradient-to-position:100%;--un-gradient-to:rgba(255,255,255,0) var(--un-gradient-to-position);--un-gradient-stops:var(--un-gradient-from), var(--un-gradient-to);}
.after\\:to-white::after{--un-gradient-to-position:100%;--un-gradient-to:rgba(255,255,255,var(--un-to-opacity, 1)) var(--un-gradient-to-position);}
.after\\:from-opacity-90::after{--un-from-opacity:0.9;}
.after\\:to-opacity-20::after{--un-to-opacity:0.2;}
.after\\:bg-gradient-to-t::after{--un-gradient-shape:to top;--un-gradient:var(--un-gradient-shape), var(--un-gradient-stops);background-image:linear-gradient(var(--un-gradient));}
.py-1{padding-top:0.25rem;padding-bottom:0.25rem;}
.last-of-type\\:pb-\\.7em:last-of-type{padding-bottom:0.7em;}
.pb-2{padding-bottom:0.5rem;}
.pl-8px{padding-left:8px;}
.pr-1{padding-right:0.25rem;}
[text~="\\31 2px"]{font-size:12px;}
.leading-snug{line-height:1.375;}
.hover\\:text-brand-primary:hover,
[text~="brand-primary"]{--un-text-opacity:1;color:rgba(25,195,125,var(--un-text-opacity));}
[text~="\\$\\{activeQuestionIndex"]{color:var(--\\{activeQuestionIndex);}
[text~="gray-primary"]{--un-text-opacity:1;color:rgba(110,110,128,var(--un-text-opacity));}
...
[scrollbar~="hover\\:thumb-color-gray-300"]:hover{--un-scrollbar-thumb-opacity:1;--scrollbar-thumb:rgba(209,213,219,var(--un-scrollbar-thumb-opacity));}
[scrollbar~="thumb-color-transparent"]{--scrollbar-thumb:transparent;}
[scrollbar~="track-color-transparent"]{--scrollbar-track:transparent;}
[scrollbar~="w-0"]{--scrollbar-width:0rem;}
[scrollbar~="w-4px"]{--scrollbar-width:4px;}` }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `transition-width ${show ? "w-[var(--app-width)]" : "w-22px"} `, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex select-none justify-between pb-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-hidden whitespace-nowrap", children: "📄 Questions" }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "relative after:content-[''] after:absolute after:bottom-0 after:right-4px after:w-full after:h-.7em after:bg-gradient-to-t after:from-white after:from-opacity-90 after:to-white after:to-opacity-20", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "ul",
          {
            ref: elRef,
            className: `max-h-[var(--app-max-list-height)] list-inside ${useCustomListStyle ? "custom-list-style" : "list-disc"} overflow-y-auto`,
            ...
            children: questions.map((q2, index) => /* @__PURE__ */ jsxRuntimeExports.jsx(
              "li",
              {
  ...
  const baseStyle = "*{margin:0;padding:0}:host{padding:8px 8px 0;border-radius:6px}\n";
  const chatgptStyle = ":host{--app-width: 120px;--app-max-list-height: 1300px;position:fixed;top:66px;right:20px;z-index:2000;padding:8px 8px 0;border-radius:6px;background:transparent}\n";
  const chatgptSidebarEnhancedStyle = ':root{--chatgpt-brand-color: #10a37f;--chatgpt-bg-hover: transparent;--chatgpt-bg-active: transparent}:host li{transition:all .3s ease;position:relative;padding-left:10px;border-radius:4px}:host li[data-active=true]{background-color:var(--chatgpt-bg-active);color:var(--chatgpt-brand-color)!important;font-weight:500;padding-left:14px;border-left:2px solid var(--chatgpt-brand-color);transform:translate(2px)}:host li:hover{background-color:var(--chatgpt-bg-hover);transform:translate(2px)}:host ul::-webkit-scrollbar{width:4px}:host ul::-webkit-scrollbar-thumb{background-color:transparent;border-radius:4px}:host ul::-webkit-scrollbar-track{background-color:transparent}:host li[data-active=true]:before{content:"➤";position:absolute;left:0;opacity:.8;font-size:10px;transform:translateY(1px)}:host ul.custom-list-style{list-style-type:none}:host li:not(:last-child){margin-bottom:4px}\n';
  const grokStyle = ":host{--app-width: 170px;--app-max-list-height: 1400px;position:fixed;top:80px;right:20px;z-index:2000;padding:8px 8px 0;border-radius:6px;background:transparent}\n";
  const grokSidebarEnhancedStyle = ':root{--grok-brand-color: #10a37f;--grok-bg-hover: transparent;--grok-bg-active: transparent}:host li{transition:all .3s ease;position:relative;padding-left:10px;border-radius:4px}:host li[data-active=true]{background-color:var(--grok-bg-active);color:var(--grok-brand-color)!important;font-weight:500;padding-left:14px;border-left:none!important;transform:translate(2px)}:host li:hover{background-color:var(--grok-bg-hover);transform:translate(2px)}:host ul::-webkit-scrollbar{width:4px}:host ul::-webkit-scrollbar-thumb{background-color:transparent;border-radius:4px}:host ul::-webkit-scrollbar-track{background-color:transparent}:host li[data-active=true]:before{content:"➤";position:absolute;left:0;opacity:.8;font-size:10px;transform:translateY(1px)}:host ul.custom-list-style{list-style-type:none}:host li:not(:last-child){margin-bottom:4px}:host,:host ul,:host footer,:host .sidebar-footer,:host>div{background:transparent!important;box-shadow:none!important;border:none!important}:host li{background:transparent!important;border:none!important;box-shadow:none!important}:host li:last-of-type{padding-bottom:0!important}div.relative:after{background:transparent!important;background-image:none!important;box-shadow:none!important;opacity:0!important;content:""!important}\n';
  const geminiStyle = ":host{--app-width: 120px;--app-max-list-height: 1400px;position:fixed;top:80px;right:20px;z-index:2000;padding:8px 8px 0;border-radius:6px;background:transparent}\n";
  const geminiSidebarEnhancedStyle = ':root{--gemini-brand-color: #10a37f;--gemini-bg-hover: transparent;--gemini-bg-active: transparent}:host li{transition:all .3s ease;position:relative;padding-left:10px;border-radius:4px}:host li[data-active=true]{background-color:var(--gemini-bg-active);color:var(--gemini-brand-color)!important;font-weight:500;padding-left:14px;border-left:2px solid var(--gemini-brand-color);transform:translate(2px)}:host li:hover{background-color:var(--gemini-bg-hover);transform:translate(2px)}:host ul::-webkit-scrollbar{width:4px}:host ul::-webkit-scrollbar-thumb{background-color:transparent;border-radius:4px}:host ul::-webkit-scrollbar-track{background-color:transparent}:host li[data-active=true]:before{content:"➤";position:absolute;left:0;opacity:.8;font-size:10px;transform:translateY(1px)}:host ul.custom-list-style{list-style-type:none}:host li:not(:last-child){margin-bottom:4px}\n';
  const DOM_MARK = "data-chatgpt-question-directory";
  function mount(el2, siteId) {
    el2.attachShadow({ mode: "open" });
    const shadowRoot = el2.shadowRoot;
    const baseSheet = new CSSStyleSheet();
    baseSheet.replaceSync(baseStyle);
    let siteStyleSheet = null;
    let enhancedStyleSheet = null;
    if (siteId === "gemini") {
    ...
    client.createRoot(shadowRoot).render(
  ...
  function load() {
    ...
    const adapter = adapterManager.getCurrentAdapter();
    const siteId = (adapter == null ? void 0 : adapter.getSiteId()) || null;
    mount(dom, siteId);
  ...
  setInterval(() => {
    ...
    if (!loaded && hasQuestion) {
      ...
      if (isSharePage) {
        questionEls.forEach((q2) => {
          q2.style.scrollMarginTop = scrollMarginTop + "px";
        });
      }
    }
  }, 600);
  function getConversationIdByUrl() {
    ...
    return (res == null ? void 0 : res[1]) ?? null;
  }
