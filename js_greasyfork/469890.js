// ==UserScript==
// @name         Sword Gale Online 介面優化plus
...
// @downloadURL https://update.greasyfork.org/scripts/469890/Sword%20Gale%20Online%20%E4%BB%8B%E9%9D%A2%E5%84%AA%E5%8C%96plus.user.js
// @updateURL https://update.greasyfork.org/scripts/469890/Sword%20Gale%20Online%20%E4%BB%8B%E9%9D%A2%E5%84%AA%E5%8C%96plus.meta.js
// ==/UserScript==
...
function registerHuntLogOberserverAndHideRestButtons() {
  ...
  if (settingStorage.get('GENERAL.CAMP_MODE')) hideRushAndBattleButtons();
  commonUtil.clearObservers();
...
function hideRushAndBattleButtons() {
  document.querySelectorAll("[tabindex='0'] > .chakra-container > div > button").forEach(button => {
    if (button.textContent === "趕路" || button.textContent === "戰鬥") button.style.display = "none"; //button.style.marginLeft = "auto";
  });
}
...
},{"../utils/common":10, "../utils/ui":12}],7:[function(require,module,exports){
"use strict";
...
exports.createAccountListLink = createAccountListLink;
const commonUtil = require("../utils/common");
const settingStorage = require("../storage/setting");
const globalVarsStorage = require("../storage/globalVars");
...
function createAccountListLink(){
  const characterContainer = document.querySelectorAll(".chakra-heading")[0];
...
function createAccountListUI(){
  let tokenList = settingStorage.get("ACCOUNT_LIST");

  //debug
  if(!tokenList || tokenList.length === 0){
    tokenList = [
    ];
    settingStorage.set("ACCOUNT_LIST", tokenList);
    settingStorage.save();
  }
  const wrapper = document.createElement("div");
  wrapper.className = "wrapper";
  ...
  wrapper.innerHTML = ` 
    <div class="dialog">
        ...
        <div class="header">
            <h1>切換帳號</h1>
            <div>
                <button id="reset-settings-btn" style="display:none">RESET</button>
                <button id="close-dialog-btn">X</button>
            </div>
        </div>
        <div class="content-container">
  ...
  const panel = [
    {
      category: "列表",
      description: "點擊切換帳號<br>也可以像連結一樣開啟新分頁",
      rows:[
        {
          id: "account-table",
          type: "account_table",
        ...
        },
        {
          ...
          event: e => {
            ...
            const token = document.getElementById('add-account-token').value;
            if(name !== "" && token !== ""){
              let tokenList = settingStorage.get("ACCOUNT_LIST");
              tokenList.push([
                name,
              ...
              const div = document.createElement("div");
              div.innerHTML = `
                    <a href="https://swordgale.online/#${token}" style="user-select: none;">${name}</a>
                    <button class="account-del-btn">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-x" width="19" height="19" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                            <line x1="18" y1="6" x2="6" y2="18" />
                            <line x1="6" y1="6" x2="18" y2="18" />
  ...
  function createRow(rowDiv, rowData){
    const type = {
      account_table: () => {
        const tableData = settingStorage.get("ACCOUNT_LIST");
        let gridRowHTML = "";
        if(!tableData.length){
          return;
        }else{
          tableData.forEach((arr, index) => {
            ...
            const div = document.createElement("div");
            div.setAttribute("data-name", arr[0]);
            div.setAttribute("data-token", arr[1]);
            div.innerHTML = `
              <a href="https://swordgale.online/#${arr[1]}" style="user-select: none;">${arr[0]}</a>
              <button class="account-del-btn">
                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-x" width="19" height="19" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                  <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                  <line x1="18" y1="6" x2="6" y2="18" />
        ...
        <div class="grid" id="${rowData.id}" bind-setting="ACCOUNT_LIST">
        ...
        rowDiv.innerHTML = `
          <div class="add-new-container">
            ...
            <div style="display:flex; flex-direction: row; align-items: center">
              <label for="${rowData.id}-name">名稱</label>
              <input type="text" id="${rowData.id}-name">
            </div>
            <div style="display:flex; flex-direction: row; align-items: center">
              <label for="${rowData.id}-token">token</label>
              <input type="text" id="${rowData.id}-token">
            </div>
            <div>
              <button class="warning">新增帳號</button>   
            </div>
          </div>
          `;
        rowDiv.querySelector("button").onclick = rowData.event;
...
function registerSettingUIEvent() {
  ...
  document.querySelectorAll(".grid-row > .account-del-btn").forEach(btn => {
    btn.onclick = e => {
      const grid = e.currentTarget.parentElement.parentElement;
      const name = e.currentTarget.parentElement.getAttribute("data-name");
      const token = e.currentTarget.parentElement.getAttribute("data-token");
      const tableList = settingStorage.get("ACCOUNT_LIST");
      settingStorage.set("ACCOUNT_LIST", tableList.filter(row => {
        return !(row[0] === name && row[1] === token);
      }));
      settingStorage.save();
  ...
  document.querySelectorAll(".grid-row > button:not(.account-del-btn)").forEach(btn => {
    btn.onclick = e => {
      const grid = e.currentTarget.parentElement.parentElement;
