// ==UserScript==
// @name           Duty™ Engine
// @namespace      com.turbo.engine
// @version        5.1.4
// @description    A Different Engine For Kaskus
// @include        http://www.kaskus.co.id/thread/*
// @include        http://www.kaskus.co.id/post/*
// @include        http://www.kaskus.co.id/lastpost/*
// @include        http://beta.kaskus.co.id/thread/*
// @include        http://beta.kaskus.co.id/post/*
// @include        http://beta.kaskus.co.id/lastpost/*
// @include        http://kaskus.co.id/thread/*
// @include        http://kaskus.co.id/post/*
// @include        http://kaskus.co.id/lastpost/*    
// @require        http://ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js
// @require        http://cdn.kaskus.com/themes_3.0/js/editor/jquery.markitup.js
// @require        http://cdn.kaskus.com/themes_3.0/js/editor/kaskus_bbcode.js
// @author         Duty™ Corporation
// @co-author      Azusa™ Engine
// @downloadURL https://update.greasyfork.org/scripts/1131/Duty%E2%84%A2%20Engine.user.js
// @updateURL https://update.greasyfork.org/scripts/1131/Duty%E2%84%A2%20Engine.meta.js
// ==/UserScript==
//
// Changelog:
// 5.1.4
// Kaskus Beta Supported
// 
// More Updates : Visit http://userscripts.org:8080/scripts/show/151402
...
$(document).ready(function(){
	...
	var loremIpsum = 'lorem ipsum makan capcay. lalu di capcaynya si lorem ada usus hamster, sedangkan di capcaynya si ipsum ada otak rusa. <br> tanpa mereka sadari, mereka muntah dan isi muntahannya adalah seluruh isi tubuhnya. mereka kaget dan menuntut ke cabai, pemilik warung nasi capcay. si cabai lapor ke polisi mengenai masalah ini. dan si lorem sama ipsum ditangkap karena memalsukan isi muntahannya itu. THE END';';+mereka kaget dan menuntut ke cabai, pemilik warung nasi capcay. si cabai lapor ke polisi mengenai masalah ini. dan si lorem sama ipsum ditangkap karena memalsukan isi muntahannya itu. THE END';
	var styleTemplate = "{post}";
	var AJAXLOAD_IMAGE_URL = "http://preloaders.net/generator.php?image=301&speed=16&fore_color=000000&back_color=FFFFFF&size=30x30&transparency=0&reverse=0&orig_colors=0&gray_transp=0&image_type=1&inverse=0&flip=0&frames_amount=75&word=(lc)oading(dot)(dot)(dot)&download&uncacher=46.12992450129241";
			...
			'<tr>'+
				'<td>'+
						'<div class="reply-message" id="pesan">'+
							...
							//############### reply text area ###################
							'<div id="div_fitur"><div class="fitur-separator">----</div><img id="pps" class="cursor" src="'+IMG_PPS+'" title="Paste Previous Post"></div>'+
							'<textarea id="reply-message" rows="250" name="message" class="markItUpEditor"></textarea>'+
							//############## save siggy button #################
							'<a id="btnSaveSiggy" href="javascript:void(0);" class="button small black" >Save Siggy</a>'+
							...
							'<fieldset class="fieldset">'+
								...
								//##### gbr jengkol ###############
									'<div id="div_gbrCapcay">'+
										'<img id="gbrCapcay" src="http://sadpanda.us/images/1636849-DL6Q9I2.jpg"><div class="clear_both"></div>'+
									'</div>'+
								...
								'</div>'+
							  //########## tombol submit ############
							  '<input type="submit" accesskey="r" value="Post It!" name="sbutton" class="button large green" style="float:right">'+
							  '<input type="submit" accesskey="s" value="Check Your Post" name="preview" id="preview" class="button large red" style="float:right;margin:0px 5px;">'+							  
							  ...
							  '<li>Klik Lamborghini Veneno jika capcay tidak muncul</li><li>Save siggy dahulu sebelum melakukan preview</li><li>Buka semua spoiler hotkey CTRL+G/SHIFT+X</li>'+
							  '<li>Biasakan ganti IP sehari sekali</li><li>Jangan lupa selalu mematikan komputer (Agar tidak meledug :hammer)</li></ul></div>'+
							'</fieldset>'+
						'</div>'+
					//################ end capcay ##########################
					'</td></tr>'+
...
'</div>'
    ...
    var uritambahan = "http://sadpanda.us/images/";
	emotContainer.innerHTML=''+
	'<div id="div_emotWrapper">'+
	'<div class="emotButtonWrapper">'+
		'<a class="emotTab" id="kecil" href="javascript:void(0);"><img src="http://kkcdn-static.kaskus.co.id/images/smilies/paw.gif">Mini</a>'+
 		'<a class="emotTab" id="besar" href="javascript:void(0);"><img src="http://kkcdn-static.kaskus.co.id/images/smilies/hammers.gif">Jumbo</a>'+
		'<a class="emotTab" id="unggah" href="javascript:void(0);">Upload</a>'+
		'<a class="emotTab" id="siggy" href="javascript:void(0);">Siggy</a>'+
		'<a class="emotTab" id="gaya" href="javascript:void(0);">Gentle Style</a>'+
    ...
    '<img class="emots emots_big" title="Gangnam Style" alt="[img]http://sadpanda.us/images/1244002-3GE9L49.gif[/img]" src="'+uritambahan+'1244002-3GE9L49.gif">'+
    '<img class="emots emots_big" title="Sleep" alt="[img]http://sadpanda.us/images/1240354-URJVTL7.gif[/img]" src="'+uritambahan+'1240354-URJVTL7.gif">'+
	''+
	...
	'<input id="browse" type="file" name="forumimg" ><br><input id="btnUnggah" type="button" value="Upload" class="button large purple"></div></div>'+	
	'<div id="loading" style="display:none;margin:10px 5px"> '+
	'Uploading... <img src="http://preloaders.net/generator.php?image=294&speed=14&fore_color=000000&back_color=FFFFFF&size=55x5&transparency=0&reverse=0&orig_colors=0&gray_transp=0&image_type=1&inverse=0&flip=0&frames_amount=37&word=(lc)oading(dot)(dot)(dot)&download&uncacher=22.00736242812127" alt="loading"/></div>'+
	'</div>'+ 
	//####################### siggy ##############################
	'<div id="siggyWrapper" class="emotContainer options">'+
	'<a id="btnEditSiggy" href="javascript:void(0);" class="button small blue">Edit Siggy</a>'+
	...
	'<b>Gentle Style:</b>'+
	'<input type="radio" name="GentleStyle" id="rbGentleStyleOn"  class="rbSiggy"/><label for="rbGentleStyleOn" class="clear_left jarak5">On</label>'+
	'<input type="radio" name="GentleStyle" id="rbGentleStyleOff"  class="rbSiggy"/><label for="rbGentleStyleOff" class="clear_left jarak5">Off</label>'+
	'<br><br>'+
	...
	function addQQButton(){
		...
		var linkText = document.createTextNode("Turbo Quote");
 		var linkText2 = document.createTextNode("Turbo Multi Quote");
		btnQQ.setAttribute('class','button medium red jarak5');
		btnQQ.setAttribute('id','btnQQ');
		btnQQ.appendChild(linkText);
		btnMQQ.setAttribute('class','button medium red jarak5');
	...
	function assignCSS(){
		...
		"#recaptcha_table,#recaptcha_area{width:510px!important;margin-left:211px;margin-bottom:10px}"+
		...
		"#gbrCapcay{cursor:pointer; border:0px solid rgb(135,5,0);float:right;margin-bottom:5px}"+
 		"#QR-wrap{background: none repeat scroll 0 0 rgba(255, 255, 255, 0.624); "+
 			"border-radius: 5px 5px 5px 5px; bottom: -5px; width:1310px; "+
		...
		"#reply-message{width:712px!important; height:350px !important;}"+
		"#btnSaveSiggy{float:right;margin-right:15px;margin-top:10px;display:none}"+
		...
		".emots_big{height:50px}"+
		".emotContainer{padding:10px 50px; display:none}"+
 		".emotButtonWrapper{padding:10px 30px;}"+
		" #smiliesText{color:rgb(136,136,136); font-size:12px}"+
		".emotTab{font-size:16px;background:#F49011;padding:10px 20px;border-radius:5px 5px 0px 0px;"+
		...
		".orangeBorder{border:12px solid rgb(244, 144, 17)}"+
		...
		".fitur-separator{background-color:#CCCCCC; height:32px; margin:3px 10px 0px 0px; overflow:hidden; width: 5px; float:left;}"+
		".cursor{cursor:pointer}"+
		".kanan{float:right;}"+
		".transparan{opacity:0.6} .transparan:hover{opacity:1}"+	
		".jarak5{margin-left:5px}"+	
		".markItUpHeader{width:710px !important;}"+
		...
		"#div_wrapperStylePreview{ width:900px; height:225px;margin-left:275px}"+
		...
		"#txtChosenColor{font-weight:bold; width:230px !important; background:#b8c5cc}"
		);
	}
	...
	function autoUpdate(){
		...
		//min ver
		/*
		 var SUC_script_num = 151402; // Change this to the number given to the script by userscripts.org (check the address bar)
try{function updateCheck(forced){if ((forced) || (parseInt(GM_getValue('SUC_last_update', '0')) + 86400000 <= (new Date().getTime()))){try{GM_xmlhttpRequest({method: 'GET',url: 'http://userscripts.org/scripts/source/'+SUC_script_num+'.meta.js?'+new Date().getTime(),headers: {'Cache-Control': 'no-cache'},onload: function(resp){var local_version, remote_version, rt, script_name;rt=resp.responseText;GM_setValue('SUC_last_update', new Date().getTime()+'');remote_version=parseInt(/@uso:version\s*(.*?)\s*$/m.exec(rt)[1]);local_version=parseInt(GM_getValue('SUC_current_version', '-1'));if(local_version!=-1){script_name = (/@name\s*(.*?)\s*$/m.exec(rt))[1];GM_setValue('SUC_target_script_name', script_name);if (remote_version > local_version){if(confirm('There is an update available for the Greasemonkey script "'+script_name+'."\nWould you like to go to the install page now?')){GM_openInTab('http://userscripts.org/scripts/show/'+SUC_script_num);GM_setValue('SUC_current_version', remote_version);}}else if (forced)alert('No update is available for "'+script_name+'."');}else GM_setValue('SUC_current_version', remote_version+'');}});}catch (err){if (forced)alert('An error occurred while checking for updates:\n'+err);}}}GM_registerMenuCommand(GM_getValue('SUC_target_script_name', '???') + ' - Manual Update Check', function(){updateCheck(true);});updateCheck(false);}catch(err){}
						...
						GM_xmlhttpRequest(
						{
							...
							onload: function(resp)
							{
								...
								if(local_version!=-1)
								{
									...
									{
										if(confirm('The New Version of Duty™ Engine Has Been Released, Do You Want To Update?'))
										{
										...
										alert('There is No Update For "'+script_name+'."');
								}
								else
									GM_setValue('SUC_current_version', remote_version+'');
							}
	...
	var scriptUploadGambar = function uploadGambar()
	{
		...
		$.ajaxFileUpload(
		{
		url:'http://beta.kaskus.co.id/misc/upload_image',
		secureuri:false,
		fileElementId:'browse',
		dataType: 'json',
		success: function (data, status){
		if(data.status == 'ok'){
			$('#preview-image').append('<img class="unggahan" src="'+data.url+'" width="92" height="92" alt="[img]'+data.url+'[/img]" style="cursor: pointer; " />');
			...
			alert("Kaskus server maybe has a problem, please try again later");
			}		
		},
	...
	function updateGentleStyleSetting(arrNum,value){
		var oldSetting = getGentleStyleSetting().split(",");
		...
		setGentleStyleSetting(newSetting);
		updateGentleStylePreview();
	}
	function updateGentleStylePreview(){
		var setting = getGentleStyleSetting().split(",");
		var temp = loremIpsum;
	...
	function Mainkan(){
		//############### variable setup #######################
		var DEFAULT_SIGGY = '[FONT="Comic Sans MS"][RIGHT][size="1"]Powered By Duty™ Engine 50.4.2291 (HD Version)[/size][/RIGHT][/FONT]';
		var temp_siggy = "\n";
		var use_siggy = "1";
		...
		if((getGentleStyleStatus()==null) || (getGentleStyleStatus()=="")){
			setGentleStyleStatus("0");
		}
		if((getGentleStyleSetting()==null) || (getGentleStyleSetting()=="")){
			setGentleStyleSetting(styleSetting);
		}
		if((getGentleStyleTemplate()==null) || (getGentleStyleTemplate()=="")){
			getGentleStyleTemplate(styleTemplate);
		}
		...
		$("#gaya").toggle(function(){
			...
			$("#siggyWrapper").hide();	
			$("#styleWrapper").show();
			if(getGentleStyleStatus()=="0"){
				$("#rbGentleStyleOff").prop("checked",true);
				$("#div_styleContainer").hide();
			}
			else{
				$("#rbGentleStyleOn").prop("checked",true);
				setGentleStyleSettings();
				updateGentleStylePreview();
	    ...
	    //############ Style related code ###############
	    $("#rbGentleStyleOn").click(function(){
			setGentleStyleStatus("1");
			setGentleStyleSettings();
			$("#div_styleContainer").show();
			scrollToStyle();
		})
		
		$("#rbGentleStyleOff").click(function(){
			setGentleStyleStatus("0");
			$("#div_styleContainer").hide();
		})
	    ...
	    $("#txtChosenColor").blur(function(){
	    	...
	    	updateGentleStyleSetting(5,$("#txtChosenColor").val());
	    })
	    ...
	    $("li.fontList").click(function(){
	    	...
	    	updateGentleStyleSetting(0,$(this).text());
	    	$("#fontTypeList").hide();
	    })
	    $("#cbBold").click(function(){
	    	var cek = $(this).attr("checked");
	    	if(cek=="checked"){
	    		updateGentleStyleSetting(1,"1");
	    	}
	    	else{
	    		updateGentleStyleSetting(1,"0");
	    	}
	    })
	    $("#cbItalic").click(function(){
	    	var cek = $(this).attr("checked");
	    	if(cek=="checked"){
	    		updateGentleStyleSetting(2,"1");
	    	}
	    	else{
	    		updateGentleStyleSetting(2,"0");
	    	}
	    })
	    $("#cbUnderline").click(function(){
	    	var cek = $(this).attr("checked");
	    	if(cek=="checked"){
	    		updateGentleStyleSetting(3,"1");
	    	}
	    	else{
	    		updateGentleStyleSetting(3,"0");
	    	}
	    })
	    $("#rbAlignLeft").click(function(){
	    	updateGentleStyleSetting(4,"0");
	    })
	    $("#rbAlignCenter").click(function(){
	    	updateGentleStyleSetting(4,"1");
	    })
	    $("#rbAlignRight").click(function(){
	    	updateGentleStyleSetting(4,"2");
	    })
	    //######### Preview click #########
		...
		$("#postreply").submit(function(){
			...
			if((getGentleStyleStatus()=="1") && (postAttemp == 0)){
				var temp = getGentleStyleTemplate();
