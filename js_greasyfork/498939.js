...
// @downloadURL https://update.greasyfork.org/scripts/498939/%5B%E8%87%AA%E5%88%B6%5D%E6%B9%96%E5%8D%97%E5%BC%80%E6%94%BE%E5%A4%A7%E5%AD%A6.user.js
// @updateURL https://update.greasyfork.org/scripts/498939/%5B%E8%87%AA%E5%88%B6%5D%E6%B9%96%E5%8D%97%E5%BC%80%E6%94%BE%E5%A4%A7%E5%AD%A6.meta.js
// ==/UserScript==
(async function () {
    'use strict';

    const coursesUrl = 'https://lms.ouchn.cn/api/my-courses?conditions=%7B%22status%22:%5B%22ongoing%22,%22closed%22%5D,%22keyword%22:%22%22%7D&fields=id,display_name&page=1&page_size=100';
    ...
    function add_log(text, type = "succ") {
        ...
        let logMessage = `<span style="color:#aaa; float:left; margin-top: 1px; font-size: 16px;">${formattedDate}</span> ${text}`;
        if (type === "succ") {
    ...
    async function initUI() {
        $('head').append('<link href="https://unpkg.com/layui@2.9.13/dist/css/layui.css" rel="stylesheet" type="text/css" />');
        $('head').append(`
        <style>
            #startBtn {background-color: deepskyblue; }
            #viewPage {background-color: red; }
            .fcBtn {
                border: none; /* 去掉边框 */
                color: white; /* 白色文字 */
        ...
        $('body').append(`
        <div id="cantrolPanel" class="layui-form" style="position: fixed; top: 20px; z-index: 9999; background-color: #fff;padding: 10px; box-shadow: 0px 0px 10px rgba(0,0,0,0.1); width: 260px;">
            <div class="layui-form-item" style="margin-bottom:0;cursor:pointer;margin-bottom: 1px;">
                <select id="studyCourse" lay-filter="studyCourse">
                    <option value="0" selected>全部选中</option>
                </select>
            </div>
            <button id="startBtn" class="fcBtn">视频</button>
            <button id="viewPage" class="fcBtn">文档</button>
            ...
            <div id="video-count">
                <!-- Current video number and total videos will be displayed here -->
            </div>
        </div>
        `);
        rc = await requests(coursesUrl);
        rc.courses.map(item => {
            $('#studyCourse').append(`<option value="${item.id}">${item.display_name} ${item.completeness}%</option>`);
        });
    ...
    async function checkNextActivity(activities, currentActivityIndex, retryCount = 0) {
        ...
        document.getElementById('video-count').innerText = `${currentActivityIndex} / ${activities.length}`;
        ...
        try {
            ...
            if (response.completeness === 'full') {
                ...
                const seconds = duration * completeness / 100;
                const hours = Math.floor(seconds / 3600);
                const minutes = Math.floor((seconds % 3600) / 60);
                const secs = Math.floor(seconds % 60);
                const formatNumber = num => num.toString().padStart(2, '0');
                add_log(` ${activity.title} ${formatNumber(hours)}:${formatNumber(minutes)}:${formatNumber(secs)}`);
                layui.element.progress('demo-filter-progress', `${completeness}%`);
                await sleep(2000);
    ...
    $.getScript('https://lib.baomitu.com/layui/2.6.8/layui.js', function () {
        layui.use('form', function () {
            ...
            add_log(`${$('#studyCourse option:selected').text()}`)
            form.on('select(studyCourse)', function (data) {
                name = data.value;
                add_log(`${data.elem[data.elem.selectedIndex].text}`);
            });
