...
// @grant        none
// @license      LedFox
// @downloadURL https://update.greasyfork.org/scripts/550737/AnimeStars%20Clan%20Highlighter.user.js
// @updateURL https://update.greasyfork.org/scripts/550737/AnimeStars%20Clan%20Highlighter.meta.js
// ==/UserScript==
...
;(async function() {
    ...
    async function getClanData(key, id) {
        ...
        const clubA = `https://${window.location.host}/clubs/${id}/`;
        if (!clubA) return { members: [], icon: null, fetched: Date.now() };
        const res = await fetch(clubA, { credentials: 'include' });
        ...
        localStorage.setItem(key, JSON.stringify(clanData));
        return clanData;
    }
    ...
    function scanAndAnnotate(members, icon) {
        ...
        document.querySelectorAll('a.card-show__owner').forEach(a => {
            ...
            if (name && members.includes(decoded)) annotateEl(a, icon);
        });
        ...
        document.querySelectorAll('a.ncard__meta-item.ncard__user-name').forEach(a => {
            ...
            const decoded = decodeURIComponent(name.replace(/\+/g, ' '));
            if (name && members.includes(decoded)) annotateEl(a, icon, 10, 12);
        });
        ...
        document.querySelectorAll('div.dropdown-item').forEach(div => {
            ...
            const decoded = decodeURIComponent(name.replace(/\+/g, ' '));
            if (name && members.includes(decoded)) annotateEl(div, icon, 26, 8);
        });
        ...
        document.querySelectorAll('a.trade__list-item').forEach(el => {
            ...
            const decoded = decodeURIComponent(name.replace(/\+/g, ' '));
            if (name && members.includes(decoded)) annotateEl(avatar, icon);
        });
        ...
        document.querySelectorAll('div.notification').forEach(el => {
            ...
            const name = link?.textContent.trim();
            const decoded = decodeURIComponent(name.replace(/\+/g, ' '));
            if (name && members.includes(decoded)) annotateEl(el, icon, 8, 8);
        });
        ...
        document.querySelectorAll('div.trade__header').forEach(div => {
            ...
            const decoded = decodeURIComponent(name.replace(/\+/g, ' '));
            if (name && members.includes(decoded)) annotateEl(avatar, icon, -8, -8);
        });
        ...
        document.querySelectorAll('div.history__name a[href^="/user/"]')
            ...
            const decoded = decodeURIComponent(name.replace(/\+/g, ' '));
            if (name && members.includes(decoded)) annotateEl(a.parentElement.parentElement, icon, 0, 0);
        });
        ...
        document.querySelectorAll('a.dpm-header-profile').forEach(a => {
            ...
            const decoded = decodeURIComponent(name.replace(/\+/g, ' '));
            if (name && members.includes(decoded)) annotateEl(a.parentElement, icon, 16, 16);
        });
        ...
        document.querySelectorAll('div.noffer__tech-row b').forEach(b => {
            ...
            const decoded = decodeURIComponent(name.replace(/\+/g, ' '));
            if (name && members.includes(decoded)) annotateEl(b, icon, 2, -20);
        });
        ...
        document.querySelectorAll('h1.ncard__main-title a[href^="/user/"]').forEach(a => {
            const name = a.textContent.trim();
            const decoded = decodeURIComponent(name.replace(/\+/g, ' '));
            if (name && members.includes(decoded)) annotateEl(a, icon, -8, 0);
        });
        ...
        document.querySelectorAll('div.ncard__users-toplist a.card-inline').forEach(a => {
            ...
            const decoded = decodeURIComponent(name.replace(/\+/g, ' '));
            if (name && members.includes(decoded)) annotateEl(a, icon, 0, 0);
        });
        ...
        document.querySelectorAll('div.usn-fr-list div.card-inline').forEach(el => {
            ...
            const name = a?.textContent.trim();
            const decoded = decodeURIComponent(name.replace(/\+/g, ' '));
            if (name && members.includes(decoded)) annotateEl(el, icon, 0, 0);
        });

    }

    const clanIds = [36,76,189,73,212,345]; // список ID кланов - (много не задавать что бы не отлететь в бан)

for (const id of clanIds) {
    const { members, icon } = await getClanData(`clan_${id}`, id);
    if (!Array.isArray(members)) {
        console.error(`Клан ${id} не содержит участников`);
        continue;
    }
