...
// @downloadURL https://update.greasyfork.org/scripts/555580/Autofill%20Tiktok%20Pros.user.js
// @updateURL https://update.greasyfork.org/scripts/555580/Autofill%20Tiktok%20Pros.meta.js
// ==/UserScript==
...
async function runFilling({
  ...
  targetjam = "21",
  targetmenit = "00",
  autoUpload = false // NEW: jika true, klik tombol upload + Post now di akhir
} = {}) {

  console.log('🚀 runFilling started dengan parameter:', {
    searchText, radioValue, deskripsi, currentdate, targetDate, targetjam, targetmenit, autoUpload
  });
  ...
  try {
    ...
    // DIPINDAHKAN KE SINI - SETELAH SCHEDULING SELESAI
    // -------------------------
    if (autoUpload) {
      try {
...
const defaultSettings = {
  ...
  increaseHours: false,
  autoUpload: false,
  counterStart: 1,  // NEW: nilai awal counter
  currentCounter: 1, // NEW: counter saat ini
  selectedPreset: '', // NEW: preset yang dipilih
  csvUrl: '' // NEW: URL file CSV GitHub
};
...
function toggleRunSettingsPopup() {
  ...
  settingsPopup.innerHTML = `
    ...
    <label><input type="checkbox" id="rf_increaseHours" ${s.increaseHours?'checked':''}> Increase Hours (+2 hours)</label><br>
    <label><input type="checkbox" id="rf_autoUpload" ${s.autoUpload?'checked':''}> AutoUpload</label>
...
function createRunUI() {
  ...
  startBtn.onclick = async () => {
    ...
    try {
      // Panggil runFilling dengan parameter autoUpload
      ...
      if (s.increaseHours) {
        const j = parseInt(s.targetjam, 10);
        if (!isNaN(j)) {
          const added = j + 2;
          if (added > 21) {
            newS.targetjam = '08';
          } else {
            newS.targetjam = String(added).padStart(2, '0');
          }
          changed = true;
        }
      }
      ...
      if (changed) {
        saveRunSettings(newS);
        updateCounterDisplay(); // update tampilan counter
        showTempNotice('Settings updated - Next: #' + newS.currentCounter, 2000); // Hilang setelah 2 detik
      } else {
        showTempNotice('Done!', 1400);
      }
    } catch (err) {
      console.error('Error during runFilling:', err);
      showTempNotice('Error occurred', 1400);
    }
  };
