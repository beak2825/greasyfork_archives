...
// @downloadURL https://update.greasyfork.org/scripts/535160/QQ%E9%82%AE%E7%AE%B1%E9%99%84%E4%BB%B6%E7%AE%A1%E7%90%86%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/535160/QQ%E9%82%AE%E7%AE%B1%E9%99%84%E4%BB%B6%E7%AE%A1%E7%90%86%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
...
(function () {
    ...
    class AttachmentManager {
        constructor(downloader) {
            ...
            this.retryCount = 3;
            this.attachments = [];
            this.allMailSubjects = [];  // 存储所有邮件的主题信息（包括无附件的邮件）
            this.markedAsExisting = this.loadMarkedItems();  // 加载已标记为存在的项目
            this.filters = {
                date: 'all',
                dateRange: {
                    start: null,
            ...
            this.createButtonTimeout = null;
            this.lastButtonCreateTime = 0;
            this.buttonCreateDebounceDelay = 300; // 防抖延迟（毫秒）
            this.listenersSetup = false; // 标记监听器是否已设置
            this.createButtonRetryCount = 0; // 重试计数器
            this.maxRetryCount = 5; // 最大重试次数
            this.isFallbackMode = false; // 标记是否已降级使用浮动按钮
        ...
        init() {
            ...
            // 智能等待工具栏加载
            this.waitForToolbarAndCreateButton();
        }

        waitForToolbarAndCreateButton() {
            ...
            // 如果工具栏已存在，立即创建
            if (checkToolbar()) {
                setTimeout(() => this.createAndInjectButton(), 100);
                return;
            }

            // 如果页面已经加载完成，缩短等待时间
            const isPageLoaded = document.readyState === 'complete';
            const maxChecks = isPageLoaded ? 3 : 6; // 页面已加载：1.5秒，否则 3 秒
            const checkInterval = 500;

            // 否则，使用 MutationObserver 监听工具栏出现
            let observer = null;
            let timeout = null;
            let checkCount = 0;
            ...
            const periodicCheck = () => {
                ...
                timeout = setTimeout(periodicCheck, checkInterval);
            };

            // 使用 MutationObserver 监听 DOM 变化（只监听关键区域）
            ...
            // 优先观察主应用区域，减少性能开销
            const targetNode = document.querySelector('#mailMainApp') || document.body;
                        ...
                        <div id="settings-panel-download" class="settings-panel" style="display: none;">
                            ...
                            <div style="${StyleManager.getStyles().dialogs.settingsSection}">
                                ...
                                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-top: 16px;">
                                    ...
                                    <div style="${StyleManager.getStyles().dialogs.settingsCheckboxItem}">
                                        <input type="checkbox" id="folder-structure-compare" style="${StyleManager.getStyles().dialogs.settingsCheckbox}" checked>
                                        <label for="folder-structure-compare" style="${StyleManager.getStyles().dialogs.settingsLabel}; margin-bottom: 0;">对比时根据文件夹结构匹配主题</label>
                                    </div>
                                    <div style="margin-left: 28px; font-size: 12px; color: #666; margin-top: 4px; margin-bottom: 12px;">
                                        启用后，对比时会优先匹配文件夹名称和邮件主题，提高按主题分组文件的匹配准确度
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    loadSettings(dialog) {
        ...
        dialog.querySelector('#folder-structure-compare').checked = settings.downloadBehavior?.folderStructureCompare !== false;
    ...
    saveSettings(dialog) {
        ...
        this.downloadSettings.downloadBehavior.autoCompareAfterDownload = dialog.querySelector('#auto-compare').checked;
            this.downloadSettings.downloadBehavior.folderStructureCompare = dialog.querySelector('#folder-structure-compare').checked;
    ...
    async showComparisonResults(dirHandle) {
        ...
        dialogContent.innerHTML = `
            ...
            <div id="compare-results" style="display: none;">
                ...
                <div id="missing-folders" style="margin-bottom: 16px;"></div>
    ...
    async performComparison(dirHandle, dialogContent) {
        ...
        const missingFoldersDiv = dialogContent.querySelector('#missing-folders');
        ...
        try {
            ...
            this.displayComparisonResults(comparisonResult, summaryDiv, missingFoldersDiv, missingDiv, duplicateDiv, matchedDiv);

        // 事件监听器已在 displayComparisonResults 内部设置，无需重复调用
    ...
    // 生成附件下载时可能的文件名（考虑验证规则和重命名）
    getPossibleFileNames(attachment) {
        const possibleNames = new Set();

        // 1. 原始文件名
        possibleNames.add(attachment.name);

        // 2. 如果启用了验证规则，生成可能的重命名文件名
        ...
        if (validation?.enabled) {
            try {
                ...
                if (validation.pattern) {
                    ...
                    const nameWithoutExt = this.processFileName(attachment.name, 'removeExtension');
                    needsFallback = !regex.test(nameWithoutExt);
                }

                // 如果需要fallback，生成重命名后的文件名
                if (needsFallback && validation.fallbackPattern) {
                    let renamedFileName = null;

                    if (validation.fallbackPattern === 'auto') {
                        // 使用auto模式（不带namingStrategy的简化版本）
                        renamedFileName = this.generateAutoFileName(attachment, fileNamingConfig, null, null);
                    } else if (validation.fallbackPattern === 'customTemplate' && validation.fallbackTemplate) {
                        // 使用自定义模板
                        renamedFileName = this.parseNamingPattern(validation.fallbackTemplate, attachment);
                    } else if (validation.fallbackPattern === 'mailSubject') {
                        const ext = this.processFileName(attachment.name, 'getExtension');
                        const subject = this.sanitizeFileName(attachment.mailSubject || '', attachment);
                        renamedFileName = subject + ext;
                    } else if (validation.fallbackPattern === 'sender') {
                        const ext = this.processFileName(attachment.name, 'getExtension');
                        const sender = this.sanitizeFileName(attachment.senderName || attachment.sender || '', attachment);
                        renamedFileName = sender + ext;
                    }
                    // 注意：toTime模式因为时间戳不可预测，无法精确匹配，会通过模糊匹配来识别

                    if (renamedFileName) {
                        possibleNames.add(renamedFileName);
                    }
                }
            } catch (error) {
                // 如果生成失败，只使用原始文件名
    ...
    async compareFilesWithProgress(localFiles, emailAttachments, updateProgress) {
        try {
            ...
            // 检查是否启用文件夹结构对比模式
            const folderStructureMode = this.downloadSettings?.downloadBehavior?.folderStructureCompare !== false;

            for (let i = 0; i < emailAttachments.length; i++) {
                const attachment = emailAttachments[i];
                const sizeTypeKey = `${attachment.size}_${attachment.type}`;
                let bestMatch = null;
                let folderMatchInfo = null;

                // 获取所有可能的文件名（包括重命名后的）
                const possibleNames = this.getPossibleFileNames(attachment);

                // 尝试用所有可能的文件名进行精确匹配
                for (const possibleName of possibleNames) {
                    const normalizedName = this.normalizeFileName(possibleName);
                    const exactCandidates = localFileMap.get(normalizedName) ?? [];

                    for (const { file } of exactCandidates.filter(c => c.type === 'exact')) {
                        if (!usedLocalFiles.has(file) && file.size === attachment.size && file.type === attachment.type) {
                            // 如果启用文件夹结构对比，检查文件夹匹配
                            if (folderStructureMode && attachment.mailSubject) {
                                const folderMatch = this.matchFolderWithSubject(file.path, attachment.mailSubject);

                                // 如果还没有找到匹配，或者当前文件的文件夹匹配度更高，则更新
                                if (!bestMatch || (folderMatch.matched && (!folderMatchInfo || folderMatch.similarity > folderMatchInfo.similarity))) {
                                    bestMatch = { file, type: 'exact', similarity: 1.0 };
                                    folderMatchInfo = folderMatch;
                                    // 如果文件夹完全匹配，立即使用这个结果
                                    if (folderMatch.similarity >= 0.9) {
                                        break;
                                    }
                                }
                            } else {
                                // 不启用文件夹对比或没有主题，使用第一个匹配的文件
                                bestMatch = { file, type: 'exact', similarity: 1.0 };
                                break;
                            }
                        }
                    ...
                    if (bestMatch && (!folderStructureMode || !folderMatchInfo || folderMatchInfo.similarity >= 0.9)) break;
                }
                ...
                if (!bestMatch) {
                    ...
                    for (const { file } of fuzzyCandidates.filter(c => c.type === 'fuzzy')) {
                        ...
                        const nameSimilarity = this.calculateNameSimilarity(attachment.name, file.name);

                        // 如果启用文件夹结构对比且有主题，考虑文件夹匹配
                        let finalSimilarity = nameSimilarity;
                        let currentFolderMatch = null;

                        if (folderStructureMode && attachment.mailSubject) {
                            currentFolderMatch = this.matchFolderWithSubject(file.path, attachment.mailSubject);
                            // 如果文件夹匹配，提高总体相似度
                            if (currentFolderMatch.matched) {
                                finalSimilarity = nameSimilarity * 0.7 + currentFolderMatch.similarity * 0.3;
                            }
                        }

                        if (finalSimilarity > 0.6 && finalSimilarity > bestSimilarity) {
                            bestMatch = { file, type: 'renamed', similarity: nameSimilarity };
                            bestSimilarity = finalSimilarity;
                            folderMatchInfo = currentFolderMatch;
                        }
                    }
                }
                ...
                if (bestMatch) {
                    ...
                    // 如果有文件夹匹配信息，添加到结果中
                    if (folderMatchInfo && folderMatchInfo.matched) {
                        matchResult.folderMatch = folderMatchInfo;
                    }
                    ...
                    // 在标记为缺失之前，进行更深入的检查
                    // 检查对应邮件主题的文件夹是否存在，以及是否可能包含此文件
                    let possibleMatchInFolder = null;

                    if (folderStructureMode && attachment.mailSubject) {
                        // 查找所有未使用且在对应主题文件夹中的文件
                        const filesInSubjectFolder = localFiles.filter(file => {
                            if (usedLocalFiles.has(file)) return false;

                            // 检查文件是否在与邮件主题匹配的文件夹中
                            const folderMatch = this.matchFolderWithSubject(file.path, attachment.mailSubject);
                            return folderMatch.matched && folderMatch.similarity >= 0.7;
                        });

                        if (filesInSubjectFolder.length > 0) {
                            // 在这些文件中找最相似的
                            let bestFolderMatch = null;
                            let bestFolderSimilarity = 0;

                            for (const file of filesInSubjectFolder) {
                                // 检查大小和类型是否接近
                                const sizeDiff = Math.abs(file.size - attachment.size);
                                const sizeMatch = sizeDiff / attachment.size < 0.1; // 10%容差

                                if (file.type === attachment.type && sizeMatch) {
                                    const nameSimilarity = this.calculateNameSimilarity(attachment.name, file.name);

                                    if (nameSimilarity > bestFolderSimilarity && nameSimilarity >= 0.5) {
                                        bestFolderSimilarity = nameSimilarity;
                                        bestFolderMatch = file;
                                    }
                                }
                            }

                            if (bestFolderMatch) {
                                possibleMatchInFolder = {
                                    file: bestFolderMatch,
                                    similarity: bestFolderSimilarity,
                                    reason: '在对应主题文件夹中找到可能匹配'
                                };
                            }
                        }
                    }

                    // 如果找到可能的匹配，标记为可能匹配而不是缺失
                    if (possibleMatchInFolder) {
                        result.matched.push({
                            email: attachment,
                            local: possibleMatchInFolder.file,
                            matchType: 'folder-search',
                            similarity: possibleMatchInFolder.similarity,
                            note: possibleMatchInFolder.reason
                        });
                        usedLocalFiles.add(possibleMatchInFolder.file);
                    } else {
                        result.missing.push(attachment);
                    }
                }

                // 每处理20个文件更新一次进度并让出主线程
                if ((i + 1) % 20 === 0 || i === emailAttachments.length - 1) {
                    updateProgress('比对文件匹配', `已处理 ${i + 1}/${emailAttachments.length} 个附件`, 3, 4);
                    await new Promise(resolve => setTimeout(resolve, 0));  // 让出主线程，避免阻塞
                }
            }
            ...
            // 如果启用了文件夹结构对比，分析缺失的主题/文件夹
            result.missingFolders = [];
            if (folderStructureMode) {
                // 收集所有邮件主题（包括无附件的邮件）
                const emailSubjects = new Map(); // subject -> {mailInfo, attachments[]}

                // 先从 allMailSubjects 收集所有邮件主题
                this.allMailSubjects.forEach(mailInfo => {
                    const subject = mailInfo.subject || '未知主题';
                    if (!emailSubjects.has(subject)) {
                        emailSubjects.set(subject, {
                            mailInfo: mailInfo,
                            attachments: [],
                            mailCount: 1,  // 记录同主题的邮件数量
                            totalAttachmentCount: mailInfo.attachmentCount || 0  // 累计附件数量
                        });
                    } else {
                        // 如果主题重复，累加邮件数量和附件数量
                        const existing = emailSubjects.get(subject);
                        existing.mailCount++;
                        existing.totalAttachmentCount += (mailInfo.attachmentCount || 0);
                        // 更新 hasAttachments 标记：只要有一封邮件有附件就标记为 true
                        if (mailInfo.hasAttachments) {
                            existing.mailInfo.hasAttachments = true;
                        }
                    }
                });
                ...
                emailAttachments.forEach(att => {
                    const subject = att.mailSubject || '未知主题';
                    if (emailSubjects.has(subject)) {
                        emailSubjects.get(subject).attachments.push(att);
                    }
                });

                // 收集文件的直接父文件夹（最后一级文件夹）
                // 用 Map 存储：文件夹名 -> 该文件夹下的文件数量
                const localFolderStats = new Map();
                for (let i = 0; i < localFiles.length; i++) {
                    ...
                    if (file.path && file.path.indexOf('/') !== -1) {
                        ...
                        if (parentFolder) {
                            const count = localFolderStats.get(parentFolder) || 0;
                            localFolderStats.set(parentFolder, count + 1);
                        }
                    }

                    // 每处理 100 个文件，让出主线程，避免阻塞
                    if (i % 100 === 0 && i > 0) {
                ...
                // 预先计算本地文件夹的统计信息（文件数量和总大小）
                const localFolderFullStats = new Map();
                for (let i = 0; i < localFiles.length; i++) {
                    const file = localFiles[i];
                    if (file.path && file.path.indexOf('/') !== -1) {
                        const pathParts = file.path.split('/');
                        const parentFolder = pathParts[pathParts.length - 2];
                        if (parentFolder) {
                            if (!localFolderFullStats.has(parentFolder)) {
                                localFolderFullStats.set(parentFolder, { count: 0, totalSize: 0 });
                            }
                            const stats = localFolderFullStats.get(parentFolder);
                            stats.count++;
                ...
                // 对比每个主题是否有对应的文件夹（使用 for 循环避免阻塞）
                let subjectIndex = 0;
                for (const [subject, data] of emailSubjects.entries()) {
                    const { mailInfo, attachments } = data;
                    let matchedFolder = null;
                    let matchType = null;
                    let matchScore = 0;
                    const normalizedSubject = this.normalizeFileName(subject);
                    const emailTotalSize = attachments.reduce((sum, att) => sum + att.size, 0);
                    const emailFileCount = attachments.length;

                    // 检查是否有文件夹与该主题匹配
                    for (const [folder, fileCount] of localFolderStats.entries()) {
                        const normalizedFolder = this.normalizeFileName(folder);

                        // 1. 完全匹配（规范化后相同）- 最高优先级
                        if (normalizedFolder === normalizedSubject) {
                        ...
                        // 2. 高相似度匹配（使用严格但合理的阈值）
                        const similarity = this.calculateNameSimilarity(subject, folder);
                        if (similarity >= 0.70) {
                            if (!matchedFolder || similarity > matchScore) {
                                matchedFolder = folder;
                                matchType = '高相似度';
                    ...
                    // 3. 如果文本匹配失败，尝试基于文件数量和大小的补充匹配
                    if (!matchedFolder && emailFileCount > 0) {
                        for (const [folder, stats] of localFolderFullStats.entries()) {
                            // 检查文件数量是否完全相同
                            if (stats.count !== emailFileCount) continue;

                            // 检查总大小是否完全相同（或差异在1%以内）
                            const sizeDiff = Math.abs(stats.totalSize - emailTotalSize);
                            const sizeThreshold = emailTotalSize * 0.01; // 1% 容差
                            if (sizeDiff > sizeThreshold) continue;

                            // 检查是否包含部分关键词（去除通用前缀后的匹配）
                            const normalizedFolder = this.normalizeFileName(folder);
                            // 提取主题中的关键词（去除常见前缀）
                            const subjectKeywords = normalizedSubject
                                .replace(/^华清宫.*?大赛[\s\+\-]*/, '')
                                .replace(/^第[一二三四五六七八九十\d]+届/, '')
                                .trim();

                            // 检查文件夹名是否包含这些关键词
                            if (subjectKeywords.length > 3 && normalizedFolder.includes(subjectKeywords)) {
                                const similarity = subjectKeywords.length / normalizedFolder.length;
                                if (!matchedFolder || similarity > matchScore) {
                                    matchedFolder = folder;
                                    matchType = '文件特征匹配';
                                    matchScore = similarity;
                    ...
                    if (matchedFolder) {
                        ...
                        // 如果没有匹配的文件夹，记录为缺失
                        // 计算与本地文件夹的相似度（综合考虑名称、附件数、文件大小）
                        const similarities = Array.from(localFolderStats.entries()).map(([folder, fileCount]) => {
                            const nameSimilarity = this.calculateNameSimilarity(subject, folder);
                            const folderStats = localFolderFullStats.get(folder);

                            // 计算综合相似度分数
                            let totalScore = nameSimilarity * 0.6; // 名称相似度权重60%
                            let matchDetails = [];

                            // 附件数量匹配检查（权重20%）
                            if (emailFileCount > 0 && fileCount === emailFileCount) {
                                totalScore += 0.2;
                                matchDetails.push('数量匹配');
                            } else if (emailFileCount > 0) {
                                const countDiff = Math.abs(fileCount - emailFileCount);
                                const countSimilarity = 1 - (countDiff / Math.max(fileCount, emailFileCount));
                                totalScore += countSimilarity * 0.2;
                            }

                            // 文件总大小匹配检查（权重20%）
                            if (folderStats && emailTotalSize > 0) {
                                const sizeDiff = Math.abs(folderStats.totalSize - emailTotalSize);
                                const sizeThreshold = emailTotalSize * 0.05; // 5% 容差

                                if (sizeDiff <= sizeThreshold) {
                                    totalScore += 0.2;
                                    matchDetails.push('大小匹配');
                                } else {
                                    // 计算大小相似度
                                    const sizeSimilarity = 1 - Math.min(sizeDiff / emailTotalSize, 1);
                                    totalScore += sizeSimilarity * 0.2;
                                }
                            }
                            ...
                            return {
                                ...
                                fileCount: fileCount,
                                folderSize: folderStats?.totalSize || 0,
                                matchDetails: matchDetails.join(' + '),
                                // 判断是否为强匹配（可能是手动处理过的）
                                isPossibleMatch: totalScore >= 0.7 && matchDetails.length >= 2
                            };
                        }).sort((a, b) => b.totalScore - a.totalScore).slice(0, 5); // 保留前5个最相似的

                        result.missingFolders.push({
                            subject: subject,
                            attachmentCount: attachments.length,
                            attachments: attachments,
                            totalSize: emailTotalSize,
                            hasAttachments: mailInfo.hasAttachments,
                            // 提取邮件特征信息
                            sender: mailInfo.sender || '未知',
                            senderName: mailInfo.senderName || '未知',
                            mailId: mailInfo.mailId,
                            // 相似的本地文件夹（可能手动处理过）
                            similarFolders: similarities,
                            // 判断缺失原因
                            missingReason: !mailInfo.hasAttachments ? '无附件邮件' :
                                          similarities[0]?.isPossibleMatch ? '可能已手动处理' : '未找到匹配'
                        });
                    }

                    // 每处理 10 个主题，让出主线程，避免阻塞（因为每个主题都要遍历所有文件夹，计算量大）
                    subjectIndex++;
                    if (subjectIndex % 10 === 0) {
                        await new Promise(resolve => setTimeout(resolve, 0));
                    }
                }
                ...
                // 按照优先级排序：
                // 1. 有附件的优先（更重要）
                // 2. 在有附件/无附件分组内，按附件数量降序
                result.missingFolders.sort((a, b) => {
                    // 有附件的排在前面
                    if (a.hasAttachments !== b.hasAttachments) {
                        return b.hasAttachments ? 1 : -1;
                    }
                    // 同类型内，按附件数量降序
                ...
                // 保存匹配详情到结果中，供界面显示
                result.folderMatchDetails = matchDetails;

                // 保存调试数据
                result.debugData = {
                    emailSubjects: Array.from(emailSubjects.entries()).map(([subject, data]) => ({
                        subject: subject,
                        normalizedSubject: this.normalizeFileName(subject),
                        attachmentCount: data.attachments.length,  // 使用 data.attachments 数组
                        totalSize: data.attachments.reduce((sum, att) => sum + att.size, 0),
                        totalSizeFormatted: this.formatData(data.attachments.reduce((sum, att) => sum + att.size, 0), 'size'),
                        hasAttachments: data.mailInfo.hasAttachments,  // 是否有附件
                        // 添加邮件特征信息用于匹配
                        mailId: data.mailInfo.mailId,
                        sender: data.mailInfo.sender || '',  // 发件人邮箱
                        senderName: data.mailInfo.senderName || '',  // 发件人姓名
                        date: data.mailInfo.totime ? new Date(data.mailInfo.totime * 1000).toISOString() : '',
                        mailCount: data.mailCount || 1  // 该主题的邮件数量
                    })),
                    localFolders: Array.from(localFolderStats.entries()).map(([folder, count]) => ({
                        folder: folder,
                        normalizedFolder: this.normalizeFileName(folder),
                        fileCount: count
                    })),
                ...
                // 详细调试信息
                const duplicateSubjects = Array.from(emailSubjects.values()).filter(data => data.mailCount > 1);
                const totalMails = this.allMailSubjects.length;

                console.log('📁 文件夹结构对比统计:');
                console.log(`   📧 总邮件数: ${totalMails}`);
                console.log(`   ✉️  独立主题数: ${emailSubjects.size}`);
                console.log(`   🔁 重复主题数: ${duplicateSubjects.length} (${totalMails - emailSubjects.size} 封邮件)`);
                console.log(`   📂 本地文件夹数: ${localFolderStats.size}`);
                console.log(`   ✅ 成功匹配数: ${matchDetails.length}`);
                console.log(`   ❌ 缺失主题数: ${result.missingFolders.length}`);
                console.log(`   📊 理论缺失: ${emailSubjects.size - localFolderStats.size}`);

                // 显示重复主题列表
                if (duplicateSubjects.length > 0) {
                    console.log(`\n   🔁 重复的主题 (${duplicateSubjects.length} 个):`);

                    // 按重复次数降序排列
                    const sortedDuplicates = duplicateSubjects.sort((a, b) => b.mailCount - a.mailCount);

                    sortedDuplicates.forEach((data, index) => {
                        console.log(`     ${index + 1}. "${data.mailInfo.subject}" (${data.mailCount} 封邮件)`);
                    });

                    console.log(`\n   💡 共有 ${duplicateSubjects.length} 个主题重复，涉及 ${totalMails - emailSubjects.size} 封重复邮件`);
                }
                ...
                if (matchDetails.length > 0) {
                    ...
                    matchDetails.slice(0, 10).forEach((detail, index) => {
                        const scoreStr = detail.score === 1.0 ? '100%' : `${Math.round(detail.score * 100)}%`;
                        console.log(`     ${index + 1}. [${detail.type}] "${detail.subject}" → "${detail.folder}" (${scoreStr})`);
                    });
                    if (matchDetails.length > 10) {
                        console.log(`     ... 还有 ${matchDetails.length - 10} 个匹配`);
                    }
                }
                ...
                if (result.missingFolders.length > 0) {
                    console.log('\n   ❌ 缺失的主题列表:');
                    result.missingFolders.forEach((folder, index) => {
                        console.log(`     ${index + 1}. "${folder.subject}" (${folder.attachmentCount}个附件, ${this.formatData(folder.totalSize, 'size')})`);
                    });

                    // 显示"未找到匹配"的主题详细信息
                    const notFoundFolders = result.missingFolders.filter(f => f.missingReason === '未找到匹配');
                    if (notFoundFolders.length > 0) {
                        console.log('\n   🔍 详细匹配分析 - "未找到匹配" 的主题:');
                        console.log(`      共 ${notFoundFolders.length} 个主题，以下是前 10 个的详细信息:\n`);

                        notFoundFolders.slice(0, 10).forEach((folder, idx) => {
                            console.log(`      ${idx + 1}. 邮件主题: "${folder.subject}"`);
                            console.log(`         规范化后: "${this.normalizeFileName(folder.subject)}"`);
                            console.log(`         附件数量: ${folder.attachmentCount} 个`);
                            console.log(`         总大小: ${this.formatData(folder.totalSize, 'size')}`);

                            if (folder.similarFolders && folder.similarFolders.length > 0) {
                                console.log(`         最相似的本地文件夹 (前3个):`);
                                folder.similarFolders.slice(0, 3).forEach((sim, simIdx) => {
                                    console.log(`           ${simIdx + 1}) "${sim.folder}"`);
                                    console.log(`              规范化后: "${this.normalizeFileName(sim.folder)}"`);
                                    console.log(`              综合得分: ${Math.round(sim.totalScore * 100)}% (名称: ${Math.round(sim.nameSimilarity * 100)}%)`);
                                    console.log(`              文件数量: ${sim.fileCount} vs ${folder.attachmentCount} (邮件)`);
                                    console.log(`              文件大小: ${this.formatData(sim.folderSize, 'size')} vs ${this.formatData(folder.totalSize, 'size')} (邮件)`);
                                    if (sim.matchDetails) {
                                        console.log(`              匹配详情: ${sim.matchDetails}`);
                                    }
                                });
                            } else {
                                console.log(`         ⚠️  没有找到任何相似的文件夹`);
                            }
                            console.log('');
                        });

                        if (notFoundFolders.length > 10) {
                            console.log(`      ... 还有 ${notFoundFolders.length - 10} 个"未找到匹配"的主题\n`);
                        }
                    }
                }

                // 显示本地文件夹示例
                console.log(`\n   📂 本地文件夹示例: [${Array.from(localFolderStats.keys()).slice(0, 10).join('", "')}${localFolderStats.size > 10 ? '", ...' : ''}]`);
            }
        ...
        result.summary.missingTotalSize = result.missing.reduce((sum, att) => sum + att.size, 0);
            ...
            Object.assign(result.summary, {
                ...
                missingFoldersCount: result.missingFolders?.length || 0
            });
    ...
    normalizeFileName(fileName) {
        // 重要：先移除邮箱域名，避免 @xx.com 中的 .com 被当作文件扩展名
        let cleaned = fileName
            // 移除邮箱域名（先移除完整邮箱格式，再清理残留的@符号）
            .replace(/@[\w\.-]+\.\w+/gi, '')  // 移除完整邮箱格式（如 @qq.com, @163.com 等）
            .replace(/@[\w\.-]+/gi, '');      // 移除残留的 @username 部分

        // 然后移除扩展名
        const nameWithoutExt = this.processFileName(cleaned, 'removeExtension');
        ...
        const normalized = nameWithoutExt
            ...
            .replace(/名叫：?/g, '')
            .replace(/名叫/g, '')
            .replace(/^名字：?/g, '')     // 仅移除开头的"名字"
            .replace(/主题的IP设计/g, 'IP设计')
            // 统一中文标点符号为空格
            .replace(/，/g, ' ')
            .replace(/：/g, ' ')
            .replace(/、/g, ' ')
    ...
    // 匹配文件夹名称和邮件主题
    // 返回匹配结果对象: { matched: boolean, similarity: number, folderName: string }
    matchFolderWithSubject(filePath, subject) {
        // 如果没有主题或文件路径不包含文件夹，返回不匹配
        if (!subject || !filePath || filePath.indexOf('/') === -1) {
            return { matched: false, similarity: 0, folderName: '' };
        }

        // 提取文件所在的最后一级文件夹名称
        const pathParts = filePath.split('/');
        const folderName = pathParts[pathParts.length - 2]; // 倒数第二个部分是文件夹名
        ...
        // 规范化文件夹名称和主题
        const normalizedFolder = this.normalizeFileName(folderName);
        const normalizedSubject = this.normalizeFileName(subject);

        // 如果规范化后完全相同，返回高匹配度
        if (normalizedFolder === normalizedSubject) {
            return { matched: true, similarity: 1.0, folderName };
        }

        // 检查是否包含关系（文件夹名包含主题或主题包含文件夹名）
        if (normalizedFolder.includes(normalizedSubject) || normalizedSubject.includes(normalizedFolder)) {
            const similarity = Math.min(normalizedFolder.length, normalizedSubject.length) /
                             Math.max(normalizedFolder.length, normalizedSubject.length);
            if (similarity > 0.6) {
                return { matched: true, similarity, folderName };
            }
        }

        // 使用编辑距离计算相似度
        const distance = this.levenshteinDistance(normalizedFolder, normalizedSubject);
        const maxLength = Math.max(normalizedFolder.length, normalizedSubject.length);
        ...
        return { matched: false, similarity, folderName };
    }

    // 显示比对结果
    displayComparisonResults(result, summaryDiv, missingFoldersDiv, missingDiv, duplicateDiv, matchedDiv) {
        // 保存当前比对结果，供"保留最新"功能使用
        this.currentComparisonResult = result;

        // 为显示区域添加数据属性，便于后续更新
        summaryDiv.setAttribute('data-comparison-summary', '');
        missingFoldersDiv.setAttribute('data-comparison-missing-folders', '');
        ...
        // 优化摘要区域 - 增加色彩层次和视觉引导
        // 检查是否启用了文件夹结构对比模式
        const folderStructureModeEnabled = this.downloadSettings?.downloadBehavior?.folderStructureCompare !== false;
        const folderMatchCount = result.matched.filter(m => m.folderMatch?.matched).length;
        ...
        summaryDiv.innerHTML = `
            <div style="margin-bottom: 24px;">
                ...
                ${folderStructureModeEnabled ? `
                    <div style="background: linear-gradient(135deg, #e3f2fd, #ffffff); border-left: 3px solid #0F7AF5; padding: 12px 16px; border-radius: 6px; margin-bottom: 16px;">
                        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#0F7AF5" stroke-width="2">
                                <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/>
                                <path d="M12 11v6m0 0l-2-2m2 2l2-2"/>
                            </svg>
                            <div style="flex: 1;">
                                <div style="font-size: 13px; font-weight: 600; color: #0F7AF5; margin-bottom: 4px;">文件夹结构对比模式已启用</div>
                                <div style="font-size: 11px; color: #666; line-height: 1.5;">
                                    📧 总邮件数: <strong>${this.allMailSubjects.length}</strong> 封
                                    ${this.allMailSubjects.length > (result.debugData?.emailSubjects?.length || 0) ? `
                                        （含 <strong style="color: #ff9800;">${this.allMailSubjects.length - (result.debugData?.emailSubjects?.length || 0)}</strong> 封重复主题邮件）
                                    ` : ''}
                                    <br>
                                    ✉️ 独立主题数: <strong>${result.debugData?.emailSubjects?.length || 0}</strong> 个
                                    | 📂 本地文件夹: <strong>${result.debugData?.localFolders?.length || 0}</strong> 个
                                    <br>
                                    ✅ 已匹配: <strong style="color: #4caf50;">${result.folderMatchDetails?.length || 0}</strong> 个主题
                                    ${result.summary.missingFoldersCount > 0 ? `
                                        | ❌ 缺失: <strong style="color: #f44336;">${result.summary.missingFoldersCount}</strong> 个主题
                                    ` : ''}
                                    <br>
                                    <span style="color: #999; font-size: 10px;">
                                        💡 理论缺失: ${(result.debugData?.emailSubjects?.length || 0) - (result.debugData?.localFolders?.length || 0)} 个
                                        （独立主题数 - 本地文件夹数）
                                    </span>
                                </div>
                            </div>
                            ${result.folderMatchDetails && result.folderMatchDetails.length > 0 ? `
                                <button class="toggle-match-details-btn" style="padding: 4px 12px; background: #0F7AF5; color: white; border: none; border-radius: 4px; font-size: 11px; cursor: pointer; white-space: nowrap;">
                                    查看匹配详情
                                </button>
                            ` : ''}
                        </div>
                        ...
                        <!-- 调试工具按钮 -->
                        <div style="display: flex; gap: 8px; padding-top: 8px; border-top: 1px solid #90caf9; align-items: center;">
                            <button class="export-comparison-log-btn" style="padding: 6px 14px; background: #9c27b0; color: white; border: none; border-radius: 4px; font-size: 11px; font-weight: 600; cursor: pointer; box-shadow: 0 2px 4px rgba(156, 39, 176, 0.2); flex-shrink: 0;">
                                📋 导出对比日志
                            </button>
                            <div style="display: flex; gap: 4px; margin-left: auto;">
                                <button class="export-email-subjects-btn" style="padding: 3px 8px; background: transparent; color: #999; border: 1px solid #e0e0e0; border-radius: 3px; font-size: 9px; cursor: pointer; white-space: nowrap;">
                                    📤 邮件
                                </button>
                                <button class="export-local-folders-btn" style="padding: 3px 8px; background: transparent; color: #999; border: 1px solid #e0e0e0; border-radius: 3px; font-size: 9px; cursor: pointer; white-space: nowrap;">
                                    📁 文件夹
                                </button>
                            </div>
                        </div>
                        ${result.folderMatchDetails && result.folderMatchDetails.length > 0 ? `
                            <div class="match-details-panel" style="display: none; margin-top: 12px; padding-top: 12px; border-top: 1px solid #90caf9;">
                                <div style="font-size: 12px; font-weight: 600; color: #0F7AF5; margin-bottom: 8px;">
                                    主题与文件夹匹配详情 (共 ${result.folderMatchDetails.length} 个)
                                </div>
                                <div style="max-height: 300px; overflow-y: auto; background: white; border-radius: 4px; padding: 8px;">
                                    ${result.folderMatchDetails.map((detail, index) => {
                                        const scoreStr = detail.score === 1.0 ? '100%' : Math.round(detail.score * 100) + '%';
                                        const typeColor = detail.type === '完全匹配' ? '#4caf50' : '#2196f3';
                                        return `
                                            <div style="padding: 6px 8px; border-bottom: ${index === result.folderMatchDetails.length - 1 ? 'none' : '1px solid #f0f0f0'}; font-size: 11px;">
                                                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 2px;">
                                                    <span style="background: ${typeColor}; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">${detail.type}</span>
                                                    <span style="color: #999;">${scoreStr}</span>
                                                </div>
                                                <div style="color: #333; margin-left: 8px;">
                                                    "${this.processData(detail.subject, 'escapeHtml')}" → "${this.processData(detail.folder, 'escapeHtml')}"
                                                </div>
                                            </div>
                                        `;
                                    }).join('')}
        ...
        if (result.missingFolders && result.missingFolders.length > 0) {
            const totalMissingFolderFiles = result.missingFolders.reduce((sum, folder) => sum + folder.attachmentCount, 0);
            const totalMissingFolderSize = result.missingFolders.reduce((sum, folder) => sum + folder.totalSize, 0);

            // 统计不同缺失原因的数量
            const noAttachmentCount = result.missingFolders.filter(f => f.missingReason === '无附件邮件').length;
            const possibleMatchCount = result.missingFolders.filter(f => f.missingReason === '可能已手动处理').length;
            const notFoundCount = result.missingFolders.filter(f => f.missingReason === '未找到匹配').length;

            missingFoldersDiv.innerHTML = `
                <div style="background: linear-gradient(135deg, #fff8e1, #ffffff); border: 2px solid #ffca28; border-radius: 8px; margin-bottom: 20px; box-shadow: 0 2px 6px rgba(255,193,7,0.15);">
                    <!-- 头部概览 -->
                    <div style="padding: 16px 20px; border-bottom: 1px solid #ffe082;">
                        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#f57c00" stroke-width="2">
                                <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/>
                                <path d="M12 9v6m0 0l-2-2m2 2l2-2"/>
                            </svg>
                            <div style="flex: 1;">
                                <h4 style="margin: 0 0 4px 0; font-size: 16px; font-weight: 600; color: #e65100;">缺失的主题文件夹</h4>
                                <div style="font-size: 13px; color: #ef6c00;">
                                    发现 ${result.missingFolders.length} 个主题在本地没有对应的文件夹，包含 ${totalMissingFolderFiles} 个附件 (${this.formatData(totalMissingFolderSize, 'size')})
                                </div>
                            </div>
                        </div>
                        <!-- 缺失原因分类统计 -->
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 8px; margin-top: 12px;">
                            ${notFoundCount > 0 ? `
                                <div style="background: #fff; padding: 8px 12px; border-radius: 4px; border: 1px solid #e65100;">
                                    <div style="font-size: 11px; color: #666;">未找到匹配</div>
                                    <div style="font-size: 16px; font-weight: 600; color: #e65100;">${notFoundCount}</div>
                                </div>
                            ` : ''}
                            ${possibleMatchCount > 0 ? `
                                <div style="background: #fff; padding: 8px 12px; border-radius: 4px; border: 1px solid #ff9800;">
                                    <div style="font-size: 11px; color: #666;">可能已手动处理</div>
                                    <div style="font-size: 16px; font-weight: 600; color: #ff9800;">${possibleMatchCount}</div>
                                </div>
                            ` : ''}
                            ${noAttachmentCount > 0 ? `
                                <div style="background: #fff; padding: 8px 12px; border-radius: 4px; border: 1px solid #999;">
                                    <div style="font-size: 11px; color: #666;">无附件邮件</div>
                                    <div style="font-size: 16px; font-weight: 600; color: #999;">${noAttachmentCount}</div>
                                </div>
                            ` : ''}
                        </div>
                    </div>
                    ...
                    <div style="max-height: 500px; overflow-y: auto;">
                        ${result.missingFolders.map((folder, index) => {
                            // 根据缺失原因确定样式
                            const isNoAttachment = folder.missingReason === '无附件邮件';
                            const isPossibleMatch = folder.missingReason === '可能已手动处理';
                            const reasonColor = isNoAttachment ? '#999' : isPossibleMatch ? '#ff9800' : '#e65100';
                            const reasonBg = isNoAttachment ? '#f5f5f5' : isPossibleMatch ? '#fff3cd' : '#fff';

                            return `
                            <div style="padding: 16px 20px; border-bottom: ${index === result.missingFolders.length - 1 ? 'none' : '1px solid #fff8e1'}; transition: background 0.2s;"
                                 data-missing-folder-item
                                 data-subject="${this.processData(folder.subject, 'escapeHtml')}"
                                 onmouseover="this.style.background='#fffbf0'"
                                 onmouseout="this.style.background='transparent'">
                                <div style="display: flex; align-items: start; gap: 16px;">
                                    <div style="width: 40px; height: 40px; background: linear-gradient(135deg, ${isNoAttachment ? '#e0e0e0' : isPossibleMatch ? '#ffe082' : '#ffeb3b'}, ${isNoAttachment ? '#bdbdbd' : isPossibleMatch ? '#ffca28' : '#ffc107'}); border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; box-shadow: 0 2px 4px rgba(255,193,7,0.2);">
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="${reasonColor}" stroke-width="2">
                                            <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/>
                                        </svg>
                                    </div>
                                    <div style="flex: 1; min-width: 0;">
                                        <!-- 主题名称 -->
                                        <div style="font-weight: 600; color: ${reasonColor}; margin-bottom: 6px; word-break: break-all; font-size: 14px;">
                                            📁 ${this.processData(folder.subject, 'escapeHtml')}
                                        </div>

                                        <!-- 邮件信息和操作按钮 -->
                                        <div style="display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; font-size: 12px; color: ${reasonColor}; margin-bottom: 8px;">
                                            <div style="display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
                                                <span style="display: inline-flex; align-items: center; gap: 4px;">
                                                    ...
                                                    ${folder.attachmentCount} 个附件
                                                </span>
                                                <span style="display: inline-flex; align-items: center; gap: 4px;">
                                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                        <path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/>
                                                    </svg>
                                                    ${this.formatData(folder.totalSize, 'size')}
                                                </span>
                                                <span style="display: inline-flex; align-items: center; gap: 4px;">
                                                    👤 ${this.processData(folder.senderName, 'escapeHtml')}
                                                </span>
                                                ${folder.sender ? `
                                                    <span style="display: inline-flex; align-items: center; gap: 4px;">
                                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                        ...
                                                        ${this.processData(folder.sender, 'escapeHtml')}
                                                    </span>
                                                ` : ''}
                                            </div>
                                            <div style="display: flex; gap: 8px;">
                                                <button class="mark-folder-ignored-btn" data-subject="${this.processData(folder.subject, 'escapeHtml')}"
                                                        title="标记为已存在"
                                                        style="padding: 6px 12px; background: #6c757d; color: white; border: none; border-radius: 4px; font-size: 11px; font-weight: 500; cursor: pointer; transition: background 0.2s; white-space: nowrap;"
                                                        onmouseover="this.style.background='#5a6268'" onmouseout="this.style.background='#6c757d'">
                                                    标记
                                                </button>
                                                ${folder.mailId ? `
                                                    <button data-mailid="${folder.mailId}" class="view-missing-folder-mail-btn"
                                                            style="padding: 6px 12px; background: linear-gradient(135deg, #0F7AF5, #0d6edb); color: white; border: none; border-radius: 4px; font-size: 11px; font-weight: 500; cursor: pointer; transition: all 0.2s; box-shadow: 0 2px 4px rgba(15,122,245,0.2); white-space: nowrap;"
                                                            onmouseover="this.style.background='linear-gradient(135deg, #0d6edb, #0c5fc7)'; this.style.boxShadow='0 3px 6px rgba(15,122,245,0.3)'"
                                                            onmouseout="this.style.background='linear-gradient(135deg, #0F7AF5, #0d6edb)'; this.style.boxShadow='0 2px 4px rgba(15,122,245,0.2)'">
                                                        📧 查看邮件
                                                    </button>
                                                ` : ''}
                                            </div>
                                        ...
                                        <div style="margin-bottom: 8px;">
                                            <span style="display: inline-block; padding: 4px 8px; background: ${reasonBg}; color: ${reasonColor}; border-radius: 4px; font-size: 11px; font-weight: 600; border: 1px solid ${reasonColor};">
                                                ${folder.missingReason === '无附件邮件' ? '⚠️ 无附件邮件' :
                                                  folder.missingReason === '可能已手动处理' ? '🔍 可能已手动处理' :
                                                  '❌ 未找到匹配'}
                                            </span>
                                        </div>

                                        <!-- 相似文件夹信息 -->
                                        ${folder.similarFolders && folder.similarFolders.length > 0 && folder.similarFolders[0].totalScore > 0.3 ? `
                                            <div style="font-size: 11px; color: #666; background: #fff; padding: 10px 12px; border-radius: 4px; border-left: 3px solid ${isPossibleMatch ? '#ff9800' : '#2196f3'};">
                                                <div style="font-weight: 600; margin-bottom: 6px; color: #333;">
                                                    ${isPossibleMatch ? '🎯 可能的匹配文件夹：' : '💡 相似的本地文件夹：'}
                                                </div>
                                                ${folder.similarFolders.slice(0, 3).map((sim, idx) => `
                                                    <div style="margin-bottom: ${idx < 2 ? '6px' : '0'}; padding: 6px; background: ${sim.isPossibleMatch ? '#fff8e1' : '#f8f9fa'}; border-radius: 3px;">
                                                        <div style="display: flex; justify-content: space-between; align-items: start; gap: 8px;">
                                                            <div style="flex: 1; min-width: 0;">
                                                                <div style="font-weight: 500; color: ${sim.isPossibleMatch ? '#e65100' : '#333'}; word-break: break-all;">
                                                                    ${idx + 1}. ${this.processData(sim.folder, 'escapeHtml')}
                                                                </div>
                                                                <div style="font-size: 10px; color: #666; margin-top: 2px;">
                                                                    ${sim.fileCount} 个文件 · ${this.formatData(sim.folderSize, 'size')}
                                                                    ${sim.matchDetails ? ' · ' + sim.matchDetails : ''}
                                                                </div>
                                                            </div>
                                                            <div style="flex-shrink: 0;">
                                                                <span style="display: inline-block; padding: 2px 6px; background: ${sim.totalScore >= 0.7 ? '#4caf50' : sim.totalScore >= 0.5 ? '#ff9800' : '#9e9e9e'}; color: white; border-radius: 3px; font-size: 10px; font-weight: 600;">
                                                                    ${Math.round(sim.totalScore * 100)}%
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
                                        ...
                                        ` : !isNoAttachment ? `
                                            <div style="font-size: 11px; color: #666; background: #fff; padding: 8px 12px; border-radius: 4px; border-left: 3px solid #e65100;">
                                                💡 建议：创建名为 "<strong>${this.processData(folder.subject, 'escapeHtml')}</strong>" 的文件夹来组织这些文件
                                            </div>
                                        ` : ''}
                                    </div>
                                </div>
        ...
        // 重新设计缺失文件区域 - 按主题分组，支持文件类型过滤
        if (result.missing.length > 0) {
            // 按邮件主题分组缺失文件
            const missingBySubject = new Map();
            result.missing.forEach(attachment => {
                const subject = attachment.mailSubject || '未知主题';
                if (!missingBySubject.has(subject)) {
                    missingBySubject.set(subject, {
                        subject: subject,
                        sender: attachment.mailSender,
                        date: attachment.mailDate,
                        mailId: attachment.mailId,
                        files: []
                    });
                }
                missingBySubject.get(subject).files.push(attachment);
            });
            ...
            missingDiv.innerHTML = `
                <div style="background: #ffffff; border: 1px solid #dee2e6; border-radius: 6px; margin-bottom: 20px;" data-missing-container>
                    <!-- 头部概览 -->
                    <div style="padding: 16px 20px; border-bottom: 1px solid #e9ecef;">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px;">
                            <div>
                                ...
                                <div style="font-size: 13px; color: #6c757d;">
                                    <span data-total-count>${result.missing.length}</span> 个文件，总大小 <span data-total-size>${this.formatData(totalSize, 'size')}</span>
                                </div>
                            </div>
                            <div style="display: flex; gap: 8px;">
                        ...
                        <!-- 文件类型过滤标签 -->
                        <div style="display: flex; gap: 8px; flex-wrap: wrap;" data-type-filters>
                            ${Object.entries(typeStats).map(([type, count]) => `
                                <div data-file-type="${type}"
                                     class="type-filter-tag active"
                                     style="background: #007bff; color: white; border: 1px solid #0056b3; border-radius: 4px; padding: 4px 10px; font-size: 11px; cursor: pointer; user-select: none; transition: all 0.2s; font-weight: 500;">
                                    ${type} (${count})
                                </div>
                            `).join('')}
                        </div>
                    ...
                    <!-- 按主题分组的缺失文件列表 -->
                    <div style="max-height: 500px; overflow-y: auto;" data-missing-list>
                        ${Array.from(missingBySubject.values()).map((group, groupIndex) => {
                            const groupTotalSize = group.files.reduce((sum, f) => sum + f.size, 0);
                            return `
                            <div style="border-bottom: ${groupIndex === missingBySubject.size - 1 ? 'none' : '1px solid #e9ecef'}; padding: 16px 20px;" data-subject-group>
                                <!-- 主题头部 -->
                                <div style="display: flex; align-items: start; gap: 12px; margin-bottom: 12px;">
                                    <div style="width: 6px; height: 20px; background: linear-gradient(135deg, #ffc107, #ff9800); border-radius: 3px; flex-shrink: 0; margin-top: 2px;"></div>
                                    <div style="flex: 1; min-width: 0;">
                                        <div style="font-weight: 600; color: #13181D; margin-bottom: 4px; word-break: break-all; font-size: 14px;">
                                            📧 ${this.processData(group.subject, 'escapeHtml')}
                                        </div>
                                        <div style="display: flex; align-items: center; gap: 16px; flex-wrap: wrap; font-size: 11px; color: #6c757d;">
                                            <span>${group.files.length} 个缺失文件</span>
                                            <span>${this.formatData(groupTotalSize, 'size')}</span>
                                            ${group.sender ? `<span>发件人: ${this.processData(group.sender, 'escapeHtml')}</span>` : ''}
                                            ${group.date ? `<span>${this.formatDate(group.date, 'YYYY-MM-DD HH:mm')}</span>` : ''}
                                        </div>
                                    </div>
                                    ${group.mailId ? `
                                        <button data-mailid="${group.mailId}" class="view-mail-btn"
                                                style="padding: 4px 10px; background: transparent; color: #0F7AF5; border: 1px solid #0F7AF5; border-radius: 4px; font-size: 11px; cursor: pointer; transition: all 0.2s; white-space: nowrap; flex-shrink: 0;"
                                                onmouseover="this.style.background='#0F7AF5'; this.style.color='white'"
                                                onmouseout="this.style.background='transparent'; this.style.color='#0F7AF5'">
                                            查看邮件
                                        </button>
                                    ` : ''}
                                </div>
                                ...
                                <div style="display: flex; flex-direction: column; gap: 8px; margin-left: 18px;">
                                    ${group.files.map((attachment, fileIndex) => `
                                        <div data-file-item data-file-type="${attachment.type || '未知'}" data-fileid="${attachment.fileid}" data-size="${attachment.size || 0}"
                                             style="display: flex; align-items: center; gap: 12px; padding: 10px 12px; background: #f8f9fa; border-radius: 4px; transition: all 0.2s;"
                                             onmouseover="this.style.background='#e9ecef'"
                                             onmouseout="this.style.background='#f8f9fa'">
                                            <div style="flex: 1; min-width: 0;">
                                                <div style="font-weight: 500; color: #13181D; margin-bottom: 2px; word-break: break-all; font-size: 13px;">
                                                    ${this.processData(attachment.name, 'escapeHtml')}
                                                </div>
                                                <div style="display: flex; align-items: center; gap: 12px; font-size: 11px; color: #6c757d;">
                                                    <span>${this.formatData(attachment.size, 'size')}</span>
                                                    <span style="background: #fff; padding: 2px 6px; border-radius: 3px; border: 1px solid #dee2e6;">${attachment.type || '未知类型'}</span>
                                                </div>
                                            </div>
                                            <div style="display: flex; gap: 6px; flex-shrink: 0;">
                                                <button data-fileid="${attachment.fileid}" class="mark-file-ignored-btn"
                                                        title="标记为已存在"
                                                        style="padding: 6px 12px; background: #6c757d; color: white; border: none; border-radius: 4px; font-size: 11px; font-weight: 500; cursor: pointer; transition: background 0.2s; white-space: nowrap;"
                                                        onmouseover="this.style.background='#5a6268'" onmouseout="this.style.background='#6c757d'">
                                                    标记
        ...
        if (result.matched.length > 0) {
            ...
            const folderSearchMatches = result.matched.filter(match => match.matchType === 'folder-search');
                    ...
                    <div style="max-height: 300px; overflow-y: auto;">
                        ${exactMatches.length > 0 ? `
                            ...
                            ${exactMatches.map((match, index) => `
                                <div style="padding: 16px 20px; border-bottom: ${index === exactMatches.length - 1 && renamedMatches.length === 0 && folderSearchMatches.length === 0 ? 'none' : '1px solid #f8f9fa'}; display: flex; align-items: center; gap: 16px; transition: background 0.2s;" onmouseover="this.style.background='#f8f9fa'" onmouseout="this.style.background='transparent'">
                                    ...
                                    <div style="flex: 1; min-width: 0;">
                                        ...
                                        ${match.folderMatch?.matched ? `
                                            <div style="font-size: 11px; color: #0F7AF5; margin-bottom: 4px; display: inline-flex; align-items: center; gap: 4px;">
                                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                    <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/>
                                                </svg>
                                                文件夹匹配: ${this.processData(match.folderMatch.folderName, 'escapeHtml')} (${Math.round(match.folderMatch.similarity * 100)}%)
                                            </div>
                                        ` : ''}
                        ...
                        ${renamedMatches.length > 0 ? `
                            ...
                            ${renamedMatches.map((match, index) => `
                                <div style="padding: 16px 20px; border-bottom: ${index === renamedMatches.length - 1 && folderSearchMatches.length === 0 ? 'none' : '1px solid #f8f9fa'}; display: flex; align-items: center; gap: 16px; transition: background 0.2s;" onmouseover="this.style.background='#f8f9fa'" onmouseout="this.style.background='transparent'">
                        ...
                        ${folderSearchMatches.length > 0 ? `
                            <div style="padding: 12px 20px; background: #fff9e6; color: #13181D; font-weight: 600; font-size: 14px; border-bottom: 1px solid #ffe5a3;">
                                文件夹搜索匹配 (${folderSearchMatches.length})
                            </div>
                            ${folderSearchMatches.map((match, index) => `
                                <div style="padding: 16px 20px; border-bottom: ${index === folderSearchMatches.length - 1 ? 'none' : '1px solid #f8f9fa'}; display: flex; align-items: center; gap: 16px; transition: background 0.2s;" onmouseover="this.style.background='#fffbf0'" onmouseout="this.style.background='transparent'">
                                    <div style="width: 6px; height: 6px; background: #ff9800; border-radius: 50%; flex-shrink: 0;"></div>
                                    <div style="flex: 1; min-width: 0;">
                                        <div style="font-weight: 600; color: #13181D; margin-bottom: 4px; word-break: break-all;">${this.processData(match.email.name, 'escapeHtml')}</div>
                                        <div style="font-size: 12px; color: #6c757d; margin-bottom: 4px;">→ ${this.processData(match.local.name, 'escapeHtml')}</div>
                                        <div style="display: inline-flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                                            <div style="font-size: 11px; color: #ff9800; background: #fff3e0; padding: 3px 8px; border-radius: 3px; display: inline-flex; align-items: center; gap: 4px;">
                                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                    <circle cx="11" cy="11" r="8"/>
                                                    <path d="M21 21l-4.35-4.35"/>
                                                </svg>
                                                文件夹搜索 · 相似度 ${Math.round(match.similarity * 100)}%
                                            </div>
                                        </div>
                                        ${match.note ? `
                                            <div style="font-size: 11px; color: #6c757d; background: #f8f9fa; padding: 4px 8px; border-radius: 3px; margin-bottom: 4px;">
                                                ${this.processData(match.note, 'escapeHtml')}
                                            </div>
                                        ` : ''}
                                        <div style="display: flex; align-items: center; gap: 12px; font-size: 12px; color: #6c757d;">
                                            <span>${this.formatData(match.email.size, 'size')}</span>
    ...
    setupComparisonDialogEvents(dialogContent) {
        ...
        // 查看匹配详情按钮
        const toggleMatchDetailsBtns = dialogContent.querySelectorAll('.toggle-match-details-btn');
        toggleMatchDetailsBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                const panel = dialogContent.querySelector('.match-details-panel');
                if (panel) {
                    const isHidden = panel.style.display === 'none';
                    panel.style.display = isHidden ? 'block' : 'none';
                    btn.textContent = isHidden ? '隐藏匹配详情' : '查看匹配详情';
                }
            });
            btn.addEventListener('mouseenter', (e) => e.target.style.background = '#0d6efd');
            btn.addEventListener('mouseleave', (e) => e.target.style.background = '#0F7AF5');
        });

        // 导出邮件主题按钮（次要按钮）
        const exportEmailSubjectsBtns = dialogContent.querySelectorAll('.export-email-subjects-btn');
        exportEmailSubjectsBtns.forEach(btn => {
            btn.addEventListener('click', () => this.exportEmailSubjects());
        ...
        // 导出本地文件夹按钮（次要按钮）
        const exportLocalFoldersBtns = dialogContent.querySelectorAll('.export-local-folders-btn');
        exportLocalFoldersBtns.forEach(btn => {
            btn.addEventListener('click', () => this.exportLocalFolders());
            btn.addEventListener('mouseenter', (e) => {
                e.target.style.background = '#f0f0f0';
                e.target.style.borderColor = '#999';
            });
        ...
        // 导出对比日志按钮
        const exportComparisonLogBtns = dialogContent.querySelectorAll('.export-comparison-log-btn');
        exportComparisonLogBtns.forEach(btn => {
            btn.addEventListener('click', () => this.exportComparisonLog());
            btn.addEventListener('mouseenter', (e) => e.target.style.background = '#8e24aa');
            btn.addEventListener('mouseleave', (e) => e.target.style.background = '#9c27b0');
        });

        // 查看邮件按钮（缺失文件列表中的）
        const viewMailBtns = dialogContent.querySelectorAll('.view-mail-btn');
        viewMailBtns.forEach(btn => {
            btn.addEventListener('click', () => {
        ...
        // 查看邮件按钮（缺失主题文件夹列表中的）
        const viewMissingFolderMailBtns = dialogContent.querySelectorAll('.view-missing-folder-mail-btn');
        viewMissingFolderMailBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                const mailId = btn.getAttribute('data-mailid');
                this.openMail(mailId);
            });
        ...
        // 文件类型过滤标签点击事件
        const typeFilterTags = dialogContent.querySelectorAll('.type-filter-tag');
        if (typeFilterTags.length > 0) {
            typeFilterTags.forEach(tag => {
                tag.addEventListener('click', () => {
                    ...
                    // 执行过滤
                    this.filterMissingFilesByType(dialogContent);
                });
            });
        }

        // 标记文件按钮事件
        const markFileBtns = dialogContent.querySelectorAll('.mark-file-ignored-btn');
        markFileBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                const fileId = btn.getAttribute('data-fileid');
                const fileItem = btn.closest('[data-file-item]');
                this.markFileAsIgnored(fileId, fileItem);
            });
        });

        // 标记主题按钮事件
        const markFolderBtns = dialogContent.querySelectorAll('.mark-folder-ignored-btn');
        markFolderBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                const subject = btn.getAttribute('data-subject');
                const folderItem = btn.closest('[data-missing-folder-item]');
                this.markFolderAsIgnored(subject, folderItem);
            });
        });
    }

    // 根据选中的文件类型过滤缺失文件列表
    filterMissingFilesByType(dialogContent) {
        const container = dialogContent.querySelector('[data-missing-container]');
        if (!container) return;

        // 获取所有激活的文件类型
        const activeTypes = new Set();
        const typeFilterTags = container.querySelectorAll('.type-filter-tag.active');
        typeFilterTags.forEach(tag => {
            activeTypes.add(tag.getAttribute('data-file-type'));
        });
        ...
        // 过滤文件项
        const fileItems = container.querySelectorAll('[data-file-item]');
        let visibleCount = 0;
        let visibleSize = 0;

        fileItems.forEach(item => {
            const fileType = item.getAttribute('data-file-type');
            const shouldShow = showAll || activeTypes.has(fileType);
            ...
            if (shouldShow) {
                ...
                // 计算可见文件的大小
                const sizeText = item.querySelector('[data-file-item] > div > div:last-child > span:first-child');
                if (sizeText) {
                    // 从格式化的文本中提取大小（这里简化处理）
                    // 实际应该从原始数据中获取
                }
        ...
        // 隐藏没有可见文件的主题组
        const subjectGroups = container.querySelectorAll('[data-subject-group]');
        let visibleGroups = 0;

        subjectGroups.forEach(group => {
            const visibleFiles = group.querySelectorAll('[data-file-item][style*="display: flex"]');
        ...
        // 更新统计信息
        const totalCountSpan = container.querySelector('[data-total-count]');
        if (totalCountSpan) {
            totalCountSpan.textContent = visibleCount;
        }

        // 如果所有主题组都被隐藏，显示提示信息
        const missingList = container.querySelector('[data-missing-list]');
        if (missingList && visibleGroups === 0) {
            const existingEmptyMsg = missingList.querySelector('[data-empty-filter-message]');
            if (!existingEmptyMsg) {
                missingList.innerHTML = `
                    <div data-empty-filter-message style="padding: 40px 20px; text-align: center; color: #6c757d;">
                        <div style="font-size: 48px; margin-bottom: 16px;">🔍</div>
                        <div style="font-size: 16px; font-weight: 600; margin-bottom: 8px;">没有匹配的文件</div>
                        <div style="font-size: 13px;">请选择至少一个文件类型过滤器</div>
                    </div>
                `;
            }
        } else if (missingList && visibleGroups > 0) {
            // 移除空状态提示
            const existingEmptyMsg = missingList.querySelector('[data-empty-filter-message]');
            if (existingEmptyMsg) {
    ...
    loadMarkedItems() {
        try {
            const saved = localStorage.getItem('attachmentManager_markedAsExisting');
            if (saved) {
    ...
    saveMarkedItems() {
        try {
            const data = {
                files: Array.from(this.markedAsExisting.files),
                folders: Array.from(this.markedAsExisting.folders),
                lastUpdated: new Date().toISOString()
            };
            localStorage.setItem('attachmentManager_markedAsExisting', JSON.stringify(data));
        } catch (e) {
            console.error('保存标记数据失败:', e);
        }
    }

    // 标记文件为已忽略（从缺失列表移除）
    markFileAsIgnored(fileId, fileItem) {
        this.markedAsExisting.files.add(fileId);
        ...
        if (fileItem) {
            ...
            setTimeout(() => {
                const subjectGroup = fileItem.closest('[data-subject-group]');
                fileItem.remove();

                // 检查该主题组是否还有文件
                if (subjectGroup) {
                    const remainingFiles = subjectGroup.querySelectorAll('[data-file-item]');
                    if (remainingFiles.length === 0) {
                        subjectGroup.remove();
                    }
                }

                this.updateIgnoredCounts();
            }, 300);
        }

        this.showToast('已标记为已存在', 'success');
    }

    // 标记主题为已忽略（从缺失列表移除）
    markFolderAsIgnored(subject, folderItem) {
        // 标记主题
        this.markedAsExisting.folders.add(subject);

        // 查找并标记该主题下的所有文件
        let markedFilesCount = 0;
        if (folderItem) {
            // 从对应主题的文件列表中找到所有文件
            const subjectGroup = folderItem.closest('[data-subject-group]');
            if (subjectGroup) {
                const fileItems = subjectGroup.querySelectorAll('[data-file-item]');
                fileItems.forEach(fileItem => {
                    const fileId = fileItem.getAttribute('data-fileid');
                    if (fileId) {
                        this.markedAsExisting.files.add(fileId);
                        markedFilesCount++;
                    }
        ...
        // 如果是在缺失主题列表中标记，也要在缺失文件列表中找到对应文件并标记及移除
        const dialog = document.querySelector('.comparison-dialog');
        if (dialog) {
            const missingContainer = dialog.querySelector('[data-missing-container]');
            if (missingContainer) {
                const allSubjectGroups = missingContainer.querySelectorAll('[data-subject-group]');
                const groupsToRemove = [];

                allSubjectGroups.forEach(group => {
                    // 检查主题是否匹配
                    const groupSubject = group.querySelector('div[style*="font-weight: 600"]');
                    if (groupSubject && groupSubject.textContent.includes(subject)) {
                        const fileItems = group.querySelectorAll('[data-file-item]');
                        fileItems.forEach(fileItem => {
                            const fileId = fileItem.getAttribute('data-fileid');
                            if (fileId) {
                                this.markedAsExisting.files.add(fileId);
        ...
        if (folderItem) {
            ...
            folderItem.style.transform = 'translateX(-20px)';
        ...
        const message = markedFilesCount > 0
            ? `已标记主题及其下 ${markedFilesCount} 个文件为已存在`
            : '已标记主题为已存在';
    ...
    updateIgnoredCounts() {
        ...
        const ignoredFilesCount = this.markedAsExisting.files.size;
        const ignoredFoldersCount = this.markedAsExisting.folders.size;

        // 更新缺失文件的统计
        const missingContainer = dialog.querySelector('[data-missing-container]');
        if (missingContainer) {
            const totalCountSpan = missingContainer.querySelector('[data-total-count]');
            const totalSizeSpan = missingContainer.querySelector('[data-total-size]');
            const fileItems = missingContainer.querySelectorAll('[data-file-item]');

            // 计算当前显示的文件数量和总大小
            let currentCount = fileItems.length;
            let currentSize = 0;
            ...
            if (totalCountSpan) {
                totalCountSpan.textContent = currentCount;
            }

            if (totalSizeSpan) {
                totalSizeSpan.textContent = this.formatData(currentSize, 'size');
            }

            // 更新或创建已忽略提示
            this.updateIgnoredHint(missingContainer, ignoredFilesCount, 'file');
        }

        // 更新缺失文件夹的统计
        const missingFoldersDiv = dialog.querySelector('[data-comparison-missing-folders]');
        if (missingFoldersDiv) {
            const folderItems = missingFoldersDiv.querySelectorAll('[data-missing-folder-item]');

            // 更新缺失主题的标题统计
            const missingFoldersTitle = missingFoldersDiv.querySelector('h4');
            if (missingFoldersTitle && folderItems.length >= 0) {
                const currentText = missingFoldersTitle.textContent;
                const newText = currentText.replace(/\d+/, folderItems.length);
                if (newText !== currentText) {
                    missingFoldersTitle.textContent = newText;
                }
            }

            this.updateIgnoredHint(missingFoldersDiv, ignoredFoldersCount, 'folder');
        }
    }

    // 更新或创建已忽略提示
    updateIgnoredHint(container, count, type) {
        if (count === 0) {
            // 移除提示
            const existingHint = container.querySelector('[data-ignored-hint]');
            if (existingHint) {
                existingHint.remove();
            }
            return;
        ...
        let hint = container.querySelector('[data-ignored-hint]');

        if (!hint) {
            // 创建提示元素
            hint = document.createElement('div');
            hint.setAttribute('data-ignored-hint', '');
            hint.style.cssText = `
                padding: 8px 16px;
                background: #f8f9fa;
            ...
            // 插入到容器底部
            const contentDiv = container.querySelector('[data-missing-list]') || container.children[container.children.length - 1];
            if (contentDiv && contentDiv.parentNode === container) {
                container.insertBefore(hint, contentDiv.nextSibling);
            } else {
                container.appendChild(hint);
            }
        }

        const typeName = type === 'file' ? '文件' : '主题';
        hint.innerHTML = `
            <span>
                ...
                已忽略 ${count} 个${typeName}
            </span>
            <button class="show-ignored-btn" data-type="${type}" style="
                padding: 4px 8px;
                background: transparent;
                color: #0F7AF5;
                border: 1px solid #0F7AF5;
                border-radius: 3px;
                font-size: 11px;
                cursor: pointer;
                transition: all 0.2s;
            " onmouseover="this.style.background='#0F7AF5'; this.style.color='white'" onmouseout="this.style.background='transparent'; this.style.color='#0F7AF5'">
                查看已忽略
            </button>
        `;

        // 添加查看已忽略的按钮事件
        const showBtn = hint.querySelector('.show-ignored-btn');
        if (showBtn && !showBtn.hasAttribute('data-listener-added')) {
            showBtn.setAttribute('data-listener-added', 'true');
            showBtn.addEventListener('click', () => {
                this.showIgnoredItems(type);
            });
        }
    }

    // 显示已忽略的项目
    showIgnoredItems(type) {
        const items = type === 'file'
            ? Array.from(this.markedAsExisting.files)
            : Array.from(this.markedAsExisting.folders);

        if (items.length === 0) {
            this.showToast('没有已忽略的项目', 'info');
            return;
        }
        ...
        const content = `
            <div style="max-height: 500px; overflow-y: auto;">
                <div style="padding: 16px 20px; border-bottom: 1px solid #e9ecef; background: #f8f9fa;">
                    <h4 style="margin: 0 0 8px 0; font-size: 16px; font-weight: 600; color: #13181D;">
                        已忽略的${typeName}
                    </h4>
                    <div style="font-size: 13px; color: #6c757d;">
                        共 ${items.length} 个${typeName}被标记为已存在
                    </div>
                </div>
                <div style="padding: 16px 20px;">
                    ${items.map((item, index) => `
                        <div style="display: flex; align-items: center; justify-content: space-between; padding: 12px; background: #f8f9fa; border-radius: 4px; margin-bottom: 8px;">
                            <div style="flex: 1; min-width: 0;">
                                <div style="font-size: 13px; color: #13181D; word-break: break-all;">
                                    ${index + 1}. ${this.processData(item, 'escapeHtml')}
                                </div>
                            </div>
                            <button class="unmark-btn" data-type="${type}" data-id="${item}"
                                    style="padding: 4px 12px; background: #dc3545; color: white; border: none; border-radius: 4px; font-size: 11px; cursor: pointer; transition: background 0.2s; white-space: nowrap; flex-shrink: 0; margin-left: 12px;"
        ...
        // 创建对话框
        const overlay = document.createElement('div');
        overlay.className = 'ignored-items-overlay';
        overlay.style.cssText = `
            position: fixed;
            top: 0;
            left: 0;
        ...
        // 取消标记按钮事件
        dialog.querySelectorAll('.unmark-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const itemType = btn.getAttribute('data-type');
                const itemId = btn.getAttribute('data-id');
                this.unmarkItem(itemType, itemId);
                btn.closest('div[style*="padding: 12px"]').remove();

                // 更新计数
                const remaining = dialog.querySelectorAll('.unmark-btn').length;
                if (remaining === 0) {
                    overlay.remove();
                    this.showToast('所有标记已取消', 'success');
                } else {
                    dialog.querySelector('div[style*="共"]').textContent = `共 ${remaining} 个${typeName}被标记为已存在`;
                }
            });
        });
    }
    ...
    unmarkItem(type, id) {
        if (type === 'file') {
            this.markedAsExisting.files.delete(id);
        } else {
            this.markedAsExisting.folders.delete(id);
        }
        this.saveMarkedItems();
        this.updateIgnoredCounts();
        this.showToast('已取消标记', 'success');
    }

    // 导出邮件主题数据
    exportEmailSubjects() {
        if (!this.currentComparisonResult?.debugData) {
            this.showToast('没有可导出的数据，请先执行对比', 'warning');
            return;
        }

        const data = this.currentComparisonResult.debugData.emailSubjects;
        const totalMails = this.allMailSubjects.length;
        const uniqueSubjects = data.length;
        const duplicateMails = totalMails - uniqueSubjects;

        const output = {
            exportTime: new Date().toISOString(),
            statistics: {
                totalMailCount: totalMails,
                uniqueSubjectCount: uniqueSubjects,
                duplicateMailCount: duplicateMails,
                duplicateSubjectCount: data.filter(item => item.hasAttachments === false).length
            },
            subjects: data.map((item, index) => ({
                ...
                totalSizeFormatted: this.formatData(item.totalSize, 'size'),
        ...
        a.href = url;
        a.download = `邮件主题数据_${new Date().toISOString().slice(0, 10)}.json`;
        a.click();
        URL.revokeObjectURL(url);

        this.showToast(`已导出 ${data.length} 个邮件主题`, 'success');
        console.log('📤 邮件主题数据已导出:', output);
    }
    ...
    exportLocalFolders() {
        ...
        const data = this.currentComparisonResult.debugData.localFolders;
        const output = {
            exportTime: new Date().toISOString(),
            ...
            folders: data.map((item, index) => ({
                index: index + 1,
                folder: item.folder,
                normalizedFolder: item.normalizedFolder,
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `本地文件夹数据_${new Date().toISOString().slice(0, 10)}.json`;
        a.click();
        URL.revokeObjectURL(url);

        this.showToast(`已导出 ${data.length} 个本地文件夹`, 'success');
        console.log('📁 本地文件夹数据已导出:', output);
    }
    ...
    exportComparisonLog() {
        ...
        const debugData = this.currentComparisonResult.debugData;
        const totalMails = this.allMailSubjects.length;
        const uniqueSubjects = debugData.emailSubjects.length;
        const duplicateMails = totalMails - uniqueSubjects;
        ...
        const output = {
            ...
            summary: {
                ...
                uniqueSubjectCount: uniqueSubjects,
                duplicateMailCount: duplicateMails,
                localFoldersCount: debugData.localFolders.length,
                matchedCount: debugData.matchDetails.length,
                missingCount: debugData.missingFolders.length,
                theoreticalMissing: uniqueSubjects - debugData.localFolders.length
            },
            emailSubjects: debugData.emailSubjects,
            localFolders: debugData.localFolders,
            matchDetails: debugData.matchDetails.map((detail, index) => ({
                index: index + 1,
                subject: detail.subject,
                matchedFolder: detail.folder,
                matchType: detail.type,
                similarity: detail.score,
                similarityPercent: Math.round(detail.score * 100) + '%'
            })),
            missingFolders: debugData.missingFolders.map((item, index) => ({
                index: index + 1,
                subject: item.subject,
                attachmentCount: item.attachmentCount,
                totalSize: item.totalSize,
                ...
                // 添加相似文件夹信息
                topSimilarFolders: item.similarFolders ? item.similarFolders.slice(0, 3).map(sim => ({
                    folder: sim.folder,
                    totalScore: sim.totalScore,
                    totalScorePercent: Math.round(sim.totalScore * 100) + '%',
                    nameSimilarity: Math.round(sim.nameSimilarity * 100) + '%',
                    fileCount: sim.fileCount,
                    folderSize: sim.folderSize,
                    folderSizeFormatted: this.formatData(sim.folderSize, 'size'),
                    matchDetails: sim.matchDetails,
                    isPossibleMatch: sim.isPossibleMatch
                })) : []
            })),
            detailedComparison: this.generateDetailedComparison(debugData)
        };
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `对比日志_${new Date().toISOString().slice(0, 10)}.json`;
        a.click();
        URL.revokeObjectURL(url);

        this.showToast('对比日志已导出', 'success');
        console.log('📋 对比日志已导出:', output);
    }

    // 生成详细对比信息
    generateDetailedComparison(debugData) {
        const comparison = [];

        // 对每个邮件主题进行详细分析
        debugData.emailSubjects.forEach(emailSubject => {
            const matched = debugData.matchDetails.find(m => m.subject === emailSubject.subject);
            ...
            if (matched) {
                comparison.push({
                    ...
                    attachmentCount: emailSubject.attachmentCount,
                    totalSize: emailSubject.totalSize,
                    totalSizeFormatted: emailSubject.totalSizeFormatted,
                    // 添加邮件特征信息
                    mailId: emailSubject.mailId,
                    sender: emailSubject.sender,
                    senderName: emailSubject.senderName,
                    date: emailSubject.date,
                    mailCount: emailSubject.mailCount
                });
            } else {
                // 找出相似但未匹配的文件夹（用于分析为什么没匹配上）
                const similarities = debugData.localFolders.map(folder => ({
                    folder: folder.folder,
                    similarity: this.calculateNameSimilarity(emailSubject.subject, folder.folder),
                    normalizedMatch: emailSubject.normalizedSubject === folder.normalizedFolder
                ...
                comparison.push({
                    ...
                    date: emailSubject.date,
                    mailCount: emailSubject.mailCount,
                    topSimilarFolders: similarities.map(s => ({
                        folder: s.folder,
                        similarity: Math.round(s.similarity * 100) + '%',
                        normalizedMatch: s.normalizedMatch
                    })),
                    reason: similarities[0].similarity >= 0.70 ? '相似度不够（需要≥70%）' :
                            similarities[0].similarity >= 0.5 ? `相似度 ${Math.round(similarities[0].similarity * 100)}% 不够高（需要≥70%）` :
                            '没有找到相似的文件夹'
                });
            }
        });
    ...
    refreshComparisonDisplay() {
        ...
        const missingFoldersDiv = document.querySelector('[data-comparison-missing-folders]');
        ...
        if (summaryDiv && missingFoldersDiv && missingDiv && duplicateDiv && matchedDiv) {
    ...
    async processMailsInBatches(mails, folderId) {
        ...
        for (let i = 0; i < totalBatches; i++) {
            ...
            for (const mail of batch) {
                ...
                // 使用 API 提供的 attach 字段判断是否有附件
                // 注意：某些邮件可能没有 attach 字段，此时默认为无附件
                const hasAttachmentByAPI = mail.attach == 1;

                // 处理有附件的邮件（根据 API 标记和实际数据）
                if (hasAttachmentByAPI && mail?.normal_attach?.length > 0) {
                    const mailAttachments = this.processMailAttachments(mail);
                    actualAttachmentCount = mailAttachments.length;
                ...
                // 收集所有邮件的主题信息（包括无附件的邮件）
                // 重要：使用 API 的 attach 字段判断是否有附件，这是最准确的
                if (mail && mail.subject) {
                    this.allMailSubjects.push({
                        subject: mail.subject,
                        mailId: mail.emailid,
                        totime: mail.totime,
                        sender: mail.senders?.item?.[0]?.email,
                        senderName: mail.senders?.item?.[0]?.nick,
                        hasAttachments: hasAttachmentByAPI,  // 使用 API 的标记
                        attachmentCount: actualAttachmentCount,  // 实际能处理的附件数量
                        apiAttachFlag: mail.attach  // 保存原始 API 标记用于调试
                    });
                }
            }
        ...
        // 调试信息：显示收集到的邮件主题数量
        const mailsMarkedWithAttachments = this.allMailSubjects.filter(m => m.hasAttachments);
        const mailsMarkedWithoutAttachments = this.allMailSubjects.filter(m => !m.hasAttachments);
        const mailsMissingApiFlag = this.allMailSubjects.filter(m => m.apiAttachFlag === undefined);

        console.log(`📊 邮件处理统计:`, {
            totalMailsProcessed: mails.length,
            mailSubjectsCollected: this.allMailSubjects.length,
            attachmentsCollected: this.attachments.length,
            mailsWithAttachments: mailsMarkedWithAttachments.length,
            mailsWithoutAttachments: mailsMarkedWithoutAttachments.length,
            mailsMissingApiAttachFlag: mailsMissingApiFlag.length
        });

        if (mailsMissingApiFlag.length > 0) {
            console.log(`ℹ️ ${mailsMissingApiFlag.length} 封邮件缺少 API attach 字段（可能是旧邮件或特殊类型邮件）`);
        }

        // 检查：API 标记有附件但实际没有处理到附件的邮件
        const hasAttachButNoFiles = this.allMailSubjects.filter(m => m.hasAttachments && m.attachmentCount === 0);

        if (hasAttachButNoFiles.length > 0) {
            console.error('❌ API标记有附件但无法处理的邮件:', {
                count: hasAttachButNoFiles.length,
                samples: hasAttachButNoFiles.slice(0, 10).map(m => ({
                    subject: m.subject,
                    mailId: m.mailId,
                    hasAttachments: m.hasAttachments,
                    apiAttachFlag: m.apiAttachFlag !== undefined ? m.apiAttachFlag : '未提供',
                    attachmentCount: m.attachmentCount
                }))
            });
            console.warn('💡 这些邮件的附件可能：1) 被过滤了（缺少name或download_url）2) 是超大附件 3) 是其他类型的附件');
        }

        // 额外检查：显示那些标记为无附件但可能有问题的邮件
        // 使用更严格的规则来减少误报
        const suspiciousNoAttachMails = mailsMarkedWithoutAttachments.filter(m => {
            const subject = m.subject;
            // 只匹配明确暗示有附件的表达，而不是简单包含关键词
        ...
        if (suspiciousNoAttachMails.length > 0) {
            console.warn('⚠️ 可疑的无附件邮件（主题明确暗示应该有附件）:', {
                count: suspiciousNoAttachMails.length,
                samples: suspiciousNoAttachMails.slice(0, 5).map(m => ({
                    subject: m.subject,
                    mailId: m.mailId,
                    hasAttachments: m.hasAttachments,
                    apiAttachFlag: m.apiAttachFlag !== undefined ? m.apiAttachFlag : '未提供',
            ...
            console.log('💡 这些邮件可能：1) 附件在正文中以链接形式提供 2) 附件已被删除 3) 使用了超大附件/网盘链接');
        }
    }
    ...
    processMailAttachments(mail) {
        ...
        // 调试日志：检查原始附件数量
        const originalAttachCount = mail.normal_attach?.length || 0;

        // 过滤有效附件
        const validAttachments = mail.normal_attach
            .filter(attach => {
                const isValid = attach?.name && attach?.download_url;
                // 记录被过滤掉的附件
                if (!isValid && originalAttachCount > 0) {
                    console.warn('⚠️ 附件被过滤:', {
                        subject: mail.subject,
                        originalCount: originalAttachCount,
                        attachInfo: attach,
                        reason: !attach?.name ? '缺少name' : '缺少download_url'
                    });
                }
                return isValid;
            });

        // 如果有附件但全部被过滤，记录详细警告和完整邮件对象
        if (originalAttachCount > 0 && validAttachments.length === 0) {
            console.error('❌ 邮件有附件但全部被过滤:', {
                subject: mail.subject,
                mailId: mail.emailid,
                apiAttachFlag: mail.attach,
                originalNormalAttachCount: originalAttachCount,
                normalAttachArray: mail.normal_attach
            });

            // 输出完整的邮件对象，帮助诊断问题
            console.group('🔍 完整邮件对象详情:');
            console.log('邮件对象:', mail);
            console.log('mail.attach (API标记):', mail.attach, mail.attach == 1 ? '✅有附件' : '❌无附件');
            console.log('mail.normal_attach (数组):', mail.normal_attach);

            // 详细检查每个附件为什么被过滤
            if (mail.normal_attach && mail.normal_attach.length > 0) {
                console.log('每个附件的详细信息:');
                mail.normal_attach.forEach((attach, idx) => {
                    console.log(`  附件 ${idx + 1}:`, {
                        name: attach?.name || '❌缺失',
                        download_url: attach?.download_url || '❌缺失',
                        size: attach?.size,
                        完整对象: attach
                    });
                });
            ...
            // 检查其他可能的附件字段
            const otherFields = Object.keys(mail).filter(key =>
                key.toLowerCase().includes('attach') ||
                key.toLowerCase().includes('file') ||
                key.toLowerCase().includes('big')
            );
            if (otherFields.length > 0) {
                console.log('其他附件相关字段:', otherFields.map(key => ({
                    key: key,
                    value: mail[key]
                })));
            }
        ...
        return validAttachments.map(attach => {
    ...
    async prepareForInitialization() {
        ...
        this.isLoading = true;
        this.attachments = [];
        this.allMailSubjects = [];  // 清空所有邮件主题数据
        this.selectedAttachments.clear();
    }
    ...
    async loadAttachmentData() {
        ...
        console.log(`📧 邮件获取统计:`, {
            ...
            actualMailsReceived: allMails.length,
            difference: totalMailCount - allMails.length
        });
    ...
    createAndInjectButton() {
        // 防抖：清除之前的定时器
        if (this.createButtonTimeout) {
            clearTimeout(this.createButtonTimeout);
            this.createButtonTimeout = null;
        }

        // 防止过于频繁的调用（300ms内只执行一次）
        const now = Date.now();
        const timeSinceLastCreate = now - this.lastButtonCreateTime;

        if (timeSinceLastCreate < this.buttonCreateDebounceDelay) {
            return;
        }
        ...
        // 使用延迟执行，避免在DOM频繁变化时重复创建
        this.createButtonTimeout = setTimeout(() => {
            this._doCreateButton();
        }, 100);
    }
    ...
    _doCreateButton() {
        ...
        this.lastButtonCreateTime = Date.now();
        ...
        try {
            ...
            if (this.isFallbackMode) {
                buttonCreated = this.createFloatingButton();
            } else {
                // 检查屏幕宽度决定显示方式
                ...
                if (screenWidth < 1600) {
                    ...
                    buttonCreated = this.createToolbarButton();
                }
            }

            // 如果按钮创建失败（找不到工具栏），不继续执行
            if (!buttonCreated) {
                return;
            }
            ...
            // 成功创建，重置重试计数器
            this.createButtonRetryCount = 0;
        } catch (error) {
            console.error('[按钮创建] 创建按钮时出现错误:', error);

            // 检查重试次数
            if (this.createButtonRetryCount >= this.maxRetryCount) {
                console.warn('[按钮创建] 已达到最大重试次数，放弃创建按钮');
                this.createButtonRetryCount = 0; // 重置计数器
                this.isCreatingButton = false;
                return;
            }

            // 延迟重试
            this.createButtonRetryCount++;
            setTimeout(() => {
                this.isCreatingButton = false; // 重置状态以允许重试
                this.createAndInjectButton();
            }, 2000);
        } finally {
            // 延迟重置状态，避免立即再次触发
    ...
    createToolbarButton() {
            ...
            if (!toolbar) {
                ...
                if (this.createButtonRetryCount >= this.maxRetryCount) {
                    this.createButtonRetryCount = 0; // 重置计数器
                    this.isFallbackMode = true; // 标记已降级
                    // 降级：使用浮动按钮
                ...
                // 如果找不到工具栏，延迟重试
                this.createButtonRetryCount++;
                const retryDelay = 500 * this.createButtonRetryCount; // 递增延迟：500ms, 1000ms, 1500ms...
                setTimeout(() => {
                    this.isCreatingButton = false; // 重置状态以允许重试
                    this.createAndInjectButton();
                }, retryDelay);
    ...
    setupResponsiveListener() {
            ...
            const handleResize = () => {
                ...
                resizeTimeout = setTimeout(() => {
                    ...
                    if (screenWidth < 1600 && hasToolbarBtn && !hasFloatingBtn) {
                        // 需要切换到浮动按钮
                        this.createAndInjectButton();
                    } else if (screenWidth >= 1600 && !hasToolbarBtn && hasFloatingBtn) {
                        // 需要切换到工具栏按钮
                        this.createAndInjectButton();
                    }
                }, 200);
    ...
    setupButtonDuplicationCheck() {
        ...
        this.buttonObserver = new MutationObserver((mutations) => {
            // 避免在面板初始化过程中或按钮创建过程中进行检查
            if (this.isLoading || this.isCreatingButton) {
                return;
            }
            ...
            const hasRelevantChanges = mutations.some(mutation => {
                return Array.from(mutation.addedNodes).some(node => {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        return node.id === 'attachment-downloader-btn' ||
                               node.hasAttribute?.('data-attachment-manager-btn') ||
                               node.classList?.contains('attachment-floating-btn');
                    }
                    return false;
                });
            });
