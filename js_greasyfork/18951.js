// ==UserScript==
// @name                CotG Raid Warning
// @name:en             CotG Raid Warning
// @description 		Script verschickt Benachrichtigungen via Ingame-Mail bei Raids mit zu wenig Truppen
// @description:en      Script sends you a Ingame-Mail if you have Raids running with to many troops 
// @author				Darius83
...
// @icon				https://sc-cdn.scaleengine.net/i/dcefd67a27049455677e68f2af4d8612.png
// @downloadURL https://update.greasyfork.org/scripts/18951/CotG%20Raid%20Warning.user.js
// @updateURL https://update.greasyfork.org/scripts/18951/CotG%20Raid%20Warning.meta.js
// ==/UserScript==
/* jshint -W097 */
'use strict';

// Special thanks to the creators of GM_config ^^
...
GM_configStruct.prototype = {
  ...
  open: function () {
    ...
    var defaultStyle = ' border: 0px; '
	  + ' display: none; height: 50%;'
      + ' margin: 0; max-height: 65%; max-width: 65%; opacity: 0;'
      + ' overflow: auto; padding: 0; position: absolute; '
...
//https://w1.crownofthegods.com/World00.php

GM_config.init(
{
	'id': 'CotGRaid', // The id used for this instance of GM_config
	'title': 'CotG Raid Warning Config',
	'fields': // Fields object
	{
		'WarningDropdown':
		{
			'label': 'Warning at', // Appears next to field
			'type': 'select', // Makes this setting a dropdown
			'title': 'Warns you if your troops total carry drops below chosen % while raiding.',
			'options': ['50%', '75%', '80%', '90%', '95%', '100%', '105%', '110%', '115%'], // Possible choices
			'default': '100%' // Default value if user doesn't change it
		}
	},
	'css': '#CotGRaid_header { }' 
	+ '#CotGRaid { background-color: rgba(220,160,140,0.0); font-family: Arial, Sans-Serif; }'
	+ '#CotGRaid .config_header { background-image: url("https://w1.crownofthegods.com/images/background/bmarb.png"); background-repeat: repeat; margin-left: 2%; color: #E2CBAC; font-size: 13px; line-height: 32px; font-style: italic;  margin-left: -6px; margin-top: -4px; height: 32px; line-height: 32px; vertical-align: middle; border-radius: 10px; border: 4px ridge #DAA520; font-size: 16pt; zoom: 1; overflow: visible; width: calc(100% + 4px);}'
	+ '#CotGRaid_wrapper { background-color: #E2CBAC; border-radius: 10px; border: 4px ridge #DAA520; border-top-style: solid; border-left-style: solid; }'
	+ '#CotGRaid_buttons_holder { margin: 2%; }'
	+ '#CotGRaid .config_var { margin: 2%; }'
	+ '#CotGRaid_saveBtn, #CotGRaid_closeBtn { background: url(https://w1.crownofthegods.com/images/but1.png); background-size: 100% 100%; border: none !important; color: #fff !important; }'
	+ '#CotGRaid_saveBtn:hover, #CotGRaid_closeBtn:hover { background: url(https://w1.crownofthegods.com/images/but2.png); background-size: 100% 100%; border: none !important; color: #fff !important; }'
});
...
GM_registerMenuCommand('CotG Raid Config', openconfig);
...
var notfallArray = new Array(); // 0 => City, 1=> Höhle, 2=> Anzahl Warnungen, 3=> Niedrigste Warnung
var notfallZaehler = 0;
var searchCity = false;
var ajax = null;
ajax = new XMLHttpRequest();
ajax.open("POST","https://w1.crownofthegods.com/overview/rreps.php",true);
ajax.send(null);
ajax.onreadystatechange = function() {
	if(this.readyState == 4) {
		if(this.status == 200) {
			//if (DEBUG) { console.debug(this.responseText); }
			var jsonObj = JSON.parse(this.responseText);
			//if (DEBUG) { console.debug(jsonObj['b']); }
			var myWarning = GM_config.get('WarningDropdown');
			myWarning = myWarning.substr( 0, myWarning.length - 1 );
			for (var i = 0; i < jsonObj['b'].length; i++) {
				if (jsonObj['b'][i][7] === 0) { // Wenn der Bericht noch ungelesen ist...
					if (jsonObj['b'][i][3] < myWarning) { // Wenn der Bericht ein Carry von unter 100 hat...
						//if (DEBUG) { console.debug(jsonObj['b'][i]); }
						if (notfallArray.length === 0) {
							notfallArray[notfallZaehler] = [jsonObj['b'][i][1], jsonObj['b'][i][5], 1, jsonObj['b'][i][3]];
							notfallZaehler++;
						} else {
							for (var k = 0; k < notfallZaehler; k++) {
								if (notfallArray[k][0] === jsonObj['b'][i][1] && notfallArray[k][1] === jsonObj['b'][i][5]) {
									notfallArray[k][2]++;
									if (jsonObj['b'][i][3] < notfallArray[k][3]) { 
										notfallArray[k][3] = jsonObj['b'][i][3]; 
									}
									searchCity = true;
									break;
								}
					...
					// Hier den Bericht als gelesen markieren
					ajax = null;
					ajax = new XMLHttpRequest();
					ajax.open("POST","https://w1.crownofthegods.com/overview/gFrep.php",true);
					ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
					ajax.send('a='+jsonObj['b'][i][6]);
				}
			}
			if (DEBUG) { console.debug(notfallArray); }
			// Hier Daten als Mail verschicken an mich selbst
			if (notfallZaehler === 0) {
				
			} else {
				var playerName = document.getElementById('playernameandally').firstElementChild.innerHTML
				if (DEBUG) { console.debug(playerName); }
				ajax = null;
				var dataVar = 'a='+playerName+';' // Hier muss noch der Empfaenger automatisch erkannt werden
					+'&c=Raid-Warning'
					+'&d=<p>ATTENTION! Please check these Raidtroops:<br /><br /></p><ul>'; 
				//console.debug(notfallZaehler);
				for (var i = 0; i < notfallZaehler; i++) {
					//console.debug(i);
					dataVar += '<li>';
					dataVar += 'City: ' + notfallArray[i][0];
					dataVar += ' Dungeon: <coords>' + notfallArray[i][1] +'</coords>';
					//dataVar += ' Anzahl Raids mit zu wenig Truppen: ' + notfallArray[i][2];
					dataVar += ' Carry of the last Raid: ' + notfallArray[i][3];
					dataVar += '</li>';
				}
				dataVar += '</ul>&f=0';
				ajax = new XMLHttpRequest();
				ajax.open("POST","https://w1.crownofthegods.com/includes/sM.php",true);
				ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
				ajax.send(dataVar);
				ajax.onreadystatechange = function() {
					if(this.readyState == 4) {
