...
// @downloadURL https://update.greasyfork.org/scripts/495985/ICEbot%20v3.user.js
// @updateURL https://update.greasyfork.org/scripts/495985/ICEbot%20v3.meta.js
// ==/UserScript==
...
if (customkick) {
    ...
    function updatespeckicks(){
        ...
        roomusers.forEach(user=>{
            ...
            user.nick.split("‏").join("")!="yosef_elbot"?f(".userkickmenu").innerHTML+=`<input type="submit" class="kickmenubtn" value="`+user.nick+`" onclick="window.postMessage('kickuser.`+user.id+`','*')">`:0
        })
    }
...
if(antiafkcheckbox.checked){
    ...
    <input type="submit" style="width:90px; background:red" onclick="window.postMessage('kickall','*')" value="KICK ALL">
        <input type="checkbox" class="kickonjoin">&nbsp;Kick when join<br>
    </div>
    <div class="option">
...
<input class="broadcastspam" type="range" oninput="postMessage('broadcastspam')" min="1000" max="30000">
