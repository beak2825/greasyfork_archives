...
// @downloadURL https://update.greasyfork.org/scripts/518416/%E7%81%BF%E8%80%80%E5%95%86%E5%93%81%E8%A7%86%E9%A2%91%E6%90%9C%E7%B4%A2.user.js
// @updateURL https://update.greasyfork.org/scripts/518416/%E7%81%BF%E8%80%80%E5%95%86%E5%93%81%E8%A7%86%E9%A2%91%E6%90%9C%E7%B4%A2.meta.js
// ==/UserScript==
...
(function() {
    ...
    let div = `
        ...
        <div class="phone_item"><span>当前登录手机号：</span><input class="cyInp" type="text" id="phone" /></div>
    ...
    const tagsArr = [{name:'逛逛',value:11},{name:'京东',value:12},{name:'拼多多',value:13}];//模式列表
    ...
    let mode_type = Number(localStorage.getItem('mode_type')||11);//模式
    ...
    //修改模式默认值
    if(tagsArr.filter(e=>e.value == Number(mode_type)).length==0){
        mode_type =11
        localStorage.setItem('mode_type',11)
    }
    //收起/展开
...
//获取时间
    ...
    if(rex.match(/https:\/\/www.douyin.com\/user\/*/) != null){
        ...
        const onScroll=()=> {
            console.log("开始滚动")
            var div = document.querySelector(".lNYhMAbF");
...
//搜索页
    if(rex.match(/https:\/\/www.douyin.com\/search\/*/) != null){
        ...
        /*    function searchClick() {
                ...
                // 找到要点击的页面元素
                var elementToClick = document.querySelector('.dropdown-items li');
                // 创建一个点击事件
                var clickEvent = new MouseEvent('click', {
                    bubbles: true,
                    cancelable: false,
        ...
        const sendList = (keyword,id,num=0)=>{
            let pattern = /^(http|https):\/\/[\w\-]+(\.[\w\-]+)+([\w\-\.,@?^=%&:/~\+#]*[\w\-\@?^=%&/~\+#])?$/;
            if(pattern.test(keyword)){//是否链接
                localStorage.setItem('xhsIsHandle',1)//是否需要数据处理
                const regex = /explore\/(\w+)\?xsec_token=(\w+)/
                const match = keyword.match(regex)
                if(!match[1]||!match[2]) return alert("链接读取失败")
                window.open("https://www.xiaohongshu.com/search_result/"+match[1]+`?xsec_token=${match[2]}&xsec_source=pc_search&type=keyword`)
        ...
        $(".cyBtn button").click(function(){
            ...
            // 读取localStorage中的数据
            const device_id = localStorage.getItem('device_id');
            const phone = document.getElementById('phone');//手机
            const operator = document.getElementById('operator');//操作人
