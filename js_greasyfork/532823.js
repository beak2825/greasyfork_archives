...
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/532823/%D0%96%D0%B0%D0%BB%D0%BE%D0%B1%D1%8B%20%7C%7C%20Blue.user.js
// @updateURL https://update.greasyfork.org/scripts/532823/%D0%96%D0%B0%D0%BB%D0%BE%D0%B1%D1%8B%20%7C%7C%20Blue.meta.js
// ==/UserScript==
...
content:
`[SIZE=4][FONT=georgia][CENTER]${greeting}, уважаемый ${user.mention}[/CENTER]<br><br>` +
"[CENTER]Перешлите мне эти сообщения в Telegram - @Miny_Leted[/CENTER]<br><br>" +
`[CENTER]На рассмотрении.[/CENTER][/FONT][/SIZE]<br><br>`+
`[SIZE=4][FONT=georgia][CENTER][COLOR=lightgrey] Приятной игры и времяпровождение на сервере «BLUE» [/COLOR][/FONT][/SIZE][/CENTER]`,
prefix: PIN_PREFIX,
status: true,
...
content:
`[SIZE=4][FONT=georgia][CENTER]${greeting}, уважаемый ${user.mention}[/CENTER]<br><br>` +
"[CENTER]Перешлите мне эти сообщения в VKontakte - @miny_leted[/CENTER]<br><br>" +
`[CENTER]На рассмотрении.[/CENTER][/FONT][/SIZE]<br><br>`+
`[SIZE=4][FONT=georgia][CENTER][COLOR=lightgrey] Приятной игры и времяпровождение на сервере «BLUE» [/COLOR][/FONT][/SIZE][/CENTER]`,
prefix: PIN_PREFIX,
status: true,
},
{
            title: `Читы`,
            content:`[SIZE=4][FONT=georgia][CENTER]${greeting}, уважаемый ${user.mention}.[/CENTER]<br>` +
            `[CENTER]По таким доказательствам мы не можем выдать игроку блокировку за стороннее ПО, однако в таких случаях в наших силах организовать усиленную слежку за игроком, так как имеются достаточно серьезные подозрения. Подобная слежка за данным игроком будет организована, при достаточной доказательной базе он будет заблокирован администрацией (как это было в одном из прошлых случаев, когда Вы подавали жалобу на игрока). Благодарим за обращение.[/CENTER]<br>` +
		    `[CENTER]Закрыто.[/CENTER][/FONT][/SIZE]`,
            prefix: CLOSE_PREFIX,
            status:false,
        },
...
title: `Оск род`,
content: `[SIZE=4][FONT=georgia][CENTER]${greeting}, уважаемый ${user.mention}[/CENTER]<br><br>` +
`[CENTER] Ваш форумный аккаунт будет заблокирован за ввод в оскорбление родных.<br>`+
`[CENTER] Закрыто.<br>`,
prefix: UNACCEPT_PREFIX,
status:false,
},
