...
// @version      2024.4.23.2 Dev
// @description  try to take over the world!
// @author       You
// @match        https://ap.inceptio.cn/*
// @icon         https://ap.inceptio.cn/favicon.ico
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/492884/%E7%99%BE%E7%89%B9YC%EF%BC%88Dev%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/492884/%E7%99%BE%E7%89%B9YC%EF%BC%88Dev%EF%BC%89.meta.js
// ==/UserScript==
...
(async function() {
				...
				mrs.forEach(mr => {
					...
					Array.from(mr.addedNodes).forEach(an => {
						...
						if(an?.children?.[0]?.matches('.inceptio-popover') && an.textContent.includes('工具箱') && (_ds.initKeyW || _ds.initKeyR)) {
							...
							if(_ds.initKeyW) (document.body.dispatchEvent(new KeyboardEvent('keydown', {key: 'w', keyCode: 87, bubbles: true})), (_ds.initKeyW = false))
					...
					if(mr.type === 'characterData' && /第.*帧/.test(mr.target.nodeValue)) {
                        ...
                        if(oldFrameNum !== void 0 && !(oldFrameNum+1 == _ds.curFrameNum || oldFrameNum-1 == _ds.curFrameNum)) showMessage('注意：跨帧', {type: 'warning', direction: 'bottom5'})
	...
	function showMessage(message, config) { //type = 'default', showTime = 3000, direction
		...
		switch(config?.direction) {
			...
			case 'bottom5': setElStyle(MessageWrap, {bottom: '5%', left: '50%', transform: 'translate(-50%, -50%)'}); break;
