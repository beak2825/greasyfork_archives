...
// @grant          unsafeWindow
// @run-at         document-end
// @downloadURL https://update.greasyfork.org/scripts/533796/HV%20Utils.user.js
// @updateURL https://update.greasyfork.org/scripts/533796/HV%20Utils.meta.js
// ==/UserScript==
...
var settings = {
  ...
  topMenuAlign: '', // '' (blank, default), 'left', 'center', 'right', or 'space-between', 'space-around', space-evenly'
...
var $price = {
  ...
  selector: function (size) {
    ...
    $element('option', selector, { text: 'Edit Prices...', value: '' });
...
if ($id('stats_pane')) {
  ...
  toggle_button($input('button', $id('stats_pane'), { style: 'position: absolute; top: 12px; right: 20px; width: 70px;' }), 'Collapse', 'Expand', $id('eqch_outer'), 'hvut-ch-expand', settings.characterExpandStatsByDefault);
}
...
_top.create = function () {
  ...
  if (!_isekai) {
    ...
    $input('submit', _top.node.stamina_form, { value: 'USE RESTORATIVE', disabled: _player.stamina > settings.disableStaminaRestorative, style: 'width: 200px;' });
  ...
  if (_player.level !== 500) {
    ...
    $element('p', sub, `Total: ${exp.toLocaleString()} / ${up.toLocaleString()}`);
    $element('p', sub, `Next: ${next.toLocaleString()}`);
    $element('p', sub, `Level: ${level_exp.toLocaleString()} / ${level_up.toLocaleString()} (${pct}%)`);
    $element('div', sub, ['.hvut-top-exp', `/<div style="width: ${pct}%;"></div>`]);
  }
  if (_top.node.server) {
    const sub = $element('div', _top.node.server, ['.hvut-top-sub']);
    if (_isekai) {
      $element('a', sub, { href: '/', innerHTML: `<p>Currently playing on Isekai</p><p>${_isekai}</p><p>Click to switch to Persistent</p>` });
    } else {
      $element('a', sub, { href: '/isekai/', innerHTML: '<p>Currently playing on Persistent</p><p>Click to switch to Isekai</p>' });
    }
  }
};
...
_top.stamina_create = async function () {
  ...
  if (items.length) {
    ...
    p.textContent = 'No item available';
  }
};

_top.stamina_submit = function (e) {
  if (settings.confirmStaminaRestorative && !confirm('Are you sure you want to use a stamina restorative?')) {
    e.preventDefault();
  }
};
...
_top.init = function () {
  ...
  if (!settings.topMenuIntegrate * 9 + settings.topMenuLinks.length <= 19) {
    const current = _isekai ? 'Isekai' : 'Persistent';
...
if (settings.showLottery) {
  _bottom.show_lottery = function (ss) {
    ...
    _bottom.node[ss].equip = $element('a', _bottom.node[ss].div, { textContent: 'Loading...', href: '/?s=Bazaar&ss=' + ss, target: !_isekai ? '_self' : '_blank' });
...
if (settings.character && (_query.s === 'Character' && _query.ss === 'ch' || $id('persona_outer'))) {
  ...
  _ch.init_simulate = function () {
    ...
    _ch.node.ass = $input(['number', 'Assimilator', 'before'], _ch.node.div, { value: _ch.ass, min: 0, max: 25, style: 'width: 30px;' });
    _ch.calc_prof();
  };
  ...
  $input(['button', 'EXP Simulator'], _ch.node.div, null, () => { _ch.init_simulate(); });
...
if (settings.equipment && _query.s === 'Character' && _query.ss === 'eq') {
  ...
  _eq.prof = {
    ...
    init: function () {
      ...
      $input(['button', 'New'], node.side, { dataset: { action: 'add' } });

      const h4 = $element('h4', node.div);
      node.name = $element('span', h4);
      $input(['button', 'Save'], h4, { dataset: { action: 'save' } });
      $input(['button', 'Delete'], h4, { dataset: { action: 'delete' } });
      ...
      node.equips = ['Willow Staff', 'Cotton Cap', 'Cotton Robe', 'Cotton Gloves', 'Cotton Pants', 'Cotton Shoes'].map((e, i) => {
        ...
        if (i === 0) {
          eqnode.type = $element('select', tr.children[1], [{ dataset: { action: 'staff' } }, '/' + ['Oak Staff', 'Willow Staff', 'Redwood Staff', 'Redwood Staff of the Elementalist', 'Katalox Staff', 'Katalox Staff of the Heaven-sent/Demon-fiend'].map((e) => `<option>${e}</option>`).join('')]);
        } else {
    ...
    save: function (name = _eq.prof.current) {
      if (_eq.prof.data[name].isnew) {
        const newname = prompt('Enter the name')?.trim();
        if (!newname) {
          return;
  ...
  if (_query.equip_slot) {
    ...
    $input(['button', 'Equip Popups'], _eq.node.buttons, null, () => { _eq.equip_popups(); });
    $input(['button', 'Proficiency Calculator'], _eq.node.buttons, null, () => { _eq.prof.toggle(); });
    _eq.node.equipset_name = $input('text', _eq.node.buttons, { value: $persona.json.ename || 'Set ' + $persona.json.eidx, style: 'width: 100px; margin-left: auto; text-align: center;' });
...
if (settings.abilities && _query.s === 'Character' && _query.ss === 'ab') {
  ...
  _ab.calc = {
    ...
    init: function () {
      ...
      $input(['button', 'Close'], node.side, { dataset: { action: 'toggle' } });
    ...
    table: function () {
      ...
      _ab.calc.node.table.innerHTML = `<thead><tr><td data-action="reverse">${arrow} Level</td><td>Ability Points</td><td>Ability Boost</td><td>Abilities</td></tr></thead><tbody></tbody>`;
  ...
  $input(['button', 'Ability Calculator'], $id('ability_outer'), { style: 'position: absolute; top: 20px; left: -80px; width: 90px; white-space: normal;' }, () => { _ab.calc.toggle(); });
} else
...
if (settings.training && _query.s === 'Character' && _query.ss === 'tr') {
  ...
  _tr.node.select = $element('select', _tr.node.div, ['/<option value="">Reserve Training...</option>'], { change: () => { _tr.change(_tr.node.select.value); } });
  ...
  $input(['button', 'Cancel Reservation'], _tr.node.div, null, () => { _tr.cancel(true); });
  ...
  if ($id('train_progress')) {
    confirm_event($qs('img[src$="/canceltrain.png"]'), 'click', 'Are you sure you want to cancel the current training?', null, _tr.cancel);
  }
...
if (settings.itemInventory && _query.s === 'Character' && _query.ss === 'it') {
  ...
  $input(['button', 'WTS Code'], _it.node.side, null, () => { _it.wts(); });
  $input(['button', 'WTS Prices'], _it.node.side, null, () => { $price.edit('WTS'); });
...
if (settings.equipInventory && _query.s === 'Character' && _query.ss === 'in') {
  ...
  _in.wts = function () {
    ...
    _in.node.wts = popup_text(code.trim() || 'No equipment selected.', 'width: 900px; height: 500px; white-space: pre;', [
      { value: 'Equip Code Format', click: () => {
        _in.edit_equipcode();
      } },
      { value: 'Name Code Rule', click: () => {
        _in.edit_namecode();
      } },
    ]);
  };
  ...
  if (_query.filter || !settings.equipInventoryIntegration) {
    ...
    $input(['button', 'sort: eid'], _in.node.side, null, () => { _in.sort_list('eid'); });
    ...
    $input(['checkbox', 'Reverse'], _in.node.side, null, () => { _in.check_list('reverse'); });
...
if (settings.settings && _query.s === 'Character' && _query.ss === 'se') {
  ...
  $input(['button', 'Store Current Settings'], _se.div, { dataset: { action: 'save' }, style: 'margin-bottom: 15px;' });
  $element('br', _se.div);
...
if (settings.equipmentShop && _query.s === 'Bazaar' && _query.ss === 'es') {
  ...
  _es.salvage = async function (eq, ask = true) {
    ...
    if (ask && (settings.equipmentShopConfirm === 2 || settings.equipmentShopConfirm === 1 && !eq.data.salvage_recommended || settings.equipmentShopConfirm > 0 && eq.data.valuable) && !confirm(`[${eq.info.name}]\nAre you sure you want to SALVAGE this?` + (!eq.data.salvage_recommended ? '\nSELL would give you more credit value.' : ''))) {
      return;
    }
    eq.data.sold = true;
    eq.node.wrapper.classList.add('hvut-es-disabled');
  ...
  _es.shop_buy = async function (eq, ask = true) {
    ...
    if (ask && !confirm(`[${eq.info.name}]\nAre you sure you wish to buy this for ${eq.data.value.toLocaleString()} credits?`)) {
      return;
    }
    eq.data.sold = true;
    eq.node.wrapper.classList.add('hvut-es-disabled');
  ...
  _es.shop_salvage = async function (eq, ask = true) {
    if (eq.data.sold || ask && !confirm(`[${eq.info.name}]\nAre you sure you wish to buy this and then salvage it?`)) {
      return;
    }
    eq.data.sold = true;
    eq.node.wrapper.classList.add('hvut-es-disabled');
  ...
  _es.sell_all = async function () {
    ...
    if (!selected.length || (settings.equipmentShopConfirm === 2 || settings.equipmentShopConfirm === 1 && salvage.length || valuable.length) && !confirm(`Are you sure you wish to sell ${selected.length} equipment piece(s) for ${sum.toLocaleString()} credits?` + (valuable.length ? '\n\n[Protected Equipment]\n' + valuable.join('\n') : '') + (salvage.length ? '\n\n[Better to Salvage]\n' + salvage.join('\n') : ''))) {
      return;
    }
    selected.forEach((eq) => {
      eq.data.sold = true;
  ...
  _es.salvage_all = function () {
    ...
    if (!selected.length || (settings.equipmentShopConfirm === 2 || settings.equipmentShopConfirm === 1 && sell.length || valuable.length) && !confirm(`Are you sure you wish to salvage ${selected.length} equipment piece(s)?` + (valuable.length ? '\n\n[Protected Equipment]\n' + valuable.join('\n') : '') + (sell.length ? '\n\n[Better to Sell]\n' + sell.join('\n') : ''))) {
      return;
    }
  ...
  _es.edit_filter = function (n) {
    const filter = {
      protect: { key: 'es_protect', default: settings.equipmentShopProtectFilter, comment: '// prevent YOUR valuable equipment from being selected by "Select All" button' },
      bazaar: { key: 'es_bazaar', default: settings.equipmentShopBazaarFilter, comment: '// keep valuable equipment in BAZZAR, then hide all other trash' },
    }[n];
    filter.current = getValue(filter.key, filter.default);
    popup_text(filter.comment + '\n\n' + filter.current.join('\n'), 'width: 600px; height: 500px; white-space: pre;', [
      { value: 'Save Filter', click: (w, t) => {
  ...
  _es.get_item_pane = function (filter, outer) {
    ...
    equiplist.forEach((eq) => {
      ...
      eq.node.salvage = $element('span', eq.node.sub, ['Salvage ' + eq.data.salvage, { dataset: { action: 'salvage', eid: eq.info.eid, pane: 'item' } }]);
      if (eq.info.tier) {
  ...
  _es.get_shop_pane = function (filter, outer) {
    ...
    equiplist.forEach((eq) => {
      ...
      $element('span', eq.node.sub, ['Buy ' + eq.data.value, { dataset: { action: 'buy', eid: eq.info.eid, pane: 'shop' } }]);
      if (eq.data.salvage > eq.data.value) {
        eq.data.visible = true;
        $element('span', eq.node.sub, ['Salvage ' + eq.data.salvage, '.hvut-es-bold', { dataset: { action: 'salvage', eid: eq.info.eid, pane: 'shop' } }]);
      }
  ...
  toggle_button($input('button', _es.node.side), 'Show All Equipment', 'Show Only Filtered', $id('shop_pane'), 'hvut-none-cont', true);
  $input(['button', 'Select:'], _es.node.side, null, () => { _es.select_all('sell'); });
  $input(['button', 'Sell'], _es.node.side, null, () => { _es.sell_all(); });
  $input(['button', 'Select:'], _es.node.side, null, () => { _es.select_all('salvage'); });
  $input(['button', 'Salvage'], _es.node.side, null, () => { _es.salvage_all(); });
  $input(['button', 'Materials Prices'], _es.node.side, null, () => { $price.edit('Materials', () => { if (confirm('You need to refresh the page.')) { location.href = location.href; } }); });
  $input(['button', 'Protection Filter'], _es.node.side, null, () => { _es.edit_filter('protect'); });
  $input(['button', 'Bazaar Filter'], _es.node.side, null, () => { _es.edit_filter('bazaar'); });
...
if (settings.shrine && _query.s === 'Bazaar' && _query.ss === 'ss') {
  ...
  _ss.offer = function (iid, count) {
    ...
    if (item.type === 'Trophy') {
      if (_ss.node.selected && !_ss.node.selected.disabled) {
        ...
        alert('Select the major class of the item.');
        return;
      }
  ...
  _ss.view_log = function () {
    ...
    $input(['button', 'Reset Log'], buttons, null, () => { _ss.reset_log(); });
    $input(['button', 'Close'], buttons, null, () => { _ss.toggle_log(); });
  };
  ...
  _ss.node.results_equip = $input(['button', 'Equip Slots'], _ss.node.results_buttons, { style: 'width: 380px;' });
  $input(['button', 'Close'], _ss.node.results_buttons, null, () => { _ss.toggle_results(); });
...
if (settings.market && _query.s === 'Bazaar' && _query.ss === 'mk') {
  ...
  _mk.init = function (p) {
    ...
    _mk.select = $element('select', div, ['/<option value="">Set Prices...</option><option value="bid">Set Market Bid as the price</option><option value="ask">Set Market Ask as the price</option><option value="day">Set Daily Average as the price</option><option value="week">Set Weekly Average as the price</option><option value="update">Update Transaction Data [Last Update: ' + new Date(_mk.prices.lastupdate).toLocaleString() + ']</option>', '!width: 135px;'], { change: () => { _mk.set_prices(_mk.select.value); } });
    $input(['button', 'Save'], div, null, () => { _mk.save_prices(); });
    $input(['button', 'Revert'], div, null, () => { _mk.load_prices(); });
    $input(['button', 'Edit'], div, null, () => { $price.edit(_mk.tag, _mk.load_prices); });
  };
  ...
  _mk.get_prices = async function () {
    if (_query.screen !== 'browseitems') {
      alert('Try it in the [Browse Items]');
      return;
    }
  ...
  if (_query.create) {
    ...
    _ml.main = {
      ...
      make_summary: function () {
        ...
        $element('li', _ml.main.node.summary, `${mobs.length} monster(s) brought you ${gains.length} gift(s), ${_ml.price2str(profit)} credits`);
      ...
      make_log: function (index) {
        ...
        if (index == -1) { // summary
          $element('li', mob.node.log, ['.hvut-ml-reset']).appendChild(
            $input(['button', 'Reset Log'], null, null, () => { _ml.main.reset_log(); })
          );
        }
      },
      reset_log: function () {
        if (confirm('Monster gift log will be reset.\nContinue?')) {
          deleteValue('ml_log');
          location.href = location.href;
        }
      },
    ...
    $input(['button', 'Materials Prices'], side_div, null, () => { $price.edit('Materials', _ml.main.change_price); });
    ...
    _ml.upgrade = {
      ...
      sum: function (calc) {
        ...
        _ml.upgrade.ct.ul.innerHTML = `
          ...
          <li><span>Used to Unlock</span><span>${_ml.upgrade.ct.unlock.toLocaleString()}</span></li>
          <li><span>Used to Upgrade</span><span>${_ml.upgrade.ct.used.toLocaleString()}</span></li>
          <li><span>Total</span><span>${(_ml.upgrade.ct.unlock + _ml.upgrade.ct.used).toLocaleString()}</span></li>
  ...
  if (_query.filter === 'new' && _query.hvut !== 'disabled') {
    ...
    _mm.write_pack = function (e) {
      ...
      if (selected.some((e) => e.data.count > e.data.stock)) {
        alert('Insufficient items'); // Insufficient items, kupo!
        return;
      }
    ...
    _mm.node.item_search = $input('text', _mm.node.item_menu, { placeholder: 'heal dra, man pot, elix', style: 'width: 170px;' }, { input: (e) => { _mm.item_search(e.target.value); }, keyup: (e) => { if (e.which === 27) { _mm.item_search('', true); } } });
    ...
    _mm.node.equip_search = $input('text', _mm.node.equip_menu, { placeholder: 'Equipment name or eid', style: 'width: 310px;' }, { input: (e) => { _mm.equip_search(e.target.value); }, keyup: (e) => { if (e.which === 27) { _mm.equip_search('', true); } } });
    ...
    _mm.credits_multi = function () {
      ...
      _mm.node.credits_multi.value.split('\n').forEach((t) => {
        ...
        if (!ctext) {
          ...
          error.push('Invalid attachment: ' + t);
          return;
        }
    ...
    _mm.db = {
      ...
      import: function () {
        ...
        const run = $input(['button', 'Select'], _mm.node.db_div, null, () => {
    ...
    _mm.page_create = function (list, p) {
      ...
      $element('tr', tbody, [`/<td>${type}</td><td>${p} Page</td><td>Attached</td><td>CoD</td><td>Sent</td><td>Read</td>`]);
    ...
    _mm.mail_view = function (mail) {
      ...
      if (db.attach) {
        ...
        $input(['button', `Edit ${wtx} Prices`], buttons, { className: 'hvut-mm-edit', dataset: { action: 'edit_price', value: wtx } });
    ...
    _mm.mail_click = function (e) {
      ...
      if (action === 'close') {
        ...
        if (value && !confirm(`Removing the attachments will deduct ${parseInt(value).toLocaleString()} Credits from your account, kupo! Are you sure?`)) {
          return;
        }
    ...
    _mm.search = function (query) {
      ...
      _mm.db.search(query).then((results) => {
        ...
        $element('tr', tbody, [`/<td>Search</td><td>${results.length} mail(s)</td><td>Attached</td><td>CoD</td><td>Sent</td><td>Read</td>`]);
    ...
    _mm.search_toggle = function () {
      ...
      _mm.node.search_filter = $element('select', _mm.node.search_form, ['/<option value="">All</option><option value="inbox">Inbox</option><option value="read">Read</option><option value="sent">Sent</option>']);
      ...
      $input(['button', 'Clear'], _mm.node.search_form, null, () => { _mm.search_close(); });
    };
    ...
    _mm.itemshop_confirm = function (mid) {
      ...
      let msg;
      if (cost !== credits) {
        msg = 'The total price of requested materials is ' + cost.toLocaleString() + ' credits, but the amount of attached credits is ' + credits.toLocaleString() + '.\nContinue?';
      }
      if (msg && !confirm(msg)) {
        return;
      }
    ...
    _mm.itemshop_parse = function (text = _mm.node.mail_body.value) {
      ...
      _mm.mail_log(`# Total Cost: ${cost.toLocaleString()}c`);
      return [items, cost];
    };
    ...
    _mm.itemshop = async function (mid, items) {
      ...
      async function buy(id, count) {
        ...
        _mm.mail_log(`Bought (${done}/${total})`);
        return true;
      }
    ...
    _mm.reforge_confirm = function (mid) {
      ...
      let msg;
      if (!amnesia) {
        msg = `This costs ${cost} Amnesia Shard(s), but nothing attached.\nContinue?`;
      } else if (amnesia !== cost) {
        msg = `This costs ${cost} Amnesia Shard(s), but the number of attached items is ${amnesia}.\nContinue?`;
      }
      if (msg && !confirm(msg)) {
        return;
      }
...
if (settings.lottery && _query.s === 'Bazaar' && (_query.ss === 'lt' || _query.ss === 'la')) {
  if (settings.showLottery && $qs('img[src$="lottery_next_d.png"]')) {
    ...
    $input(['checkbox', 'Show this lottery on every page'], $element('div', $id('rightpane'), ['!margin-top: 10px; color: #c00;']), { checked: !_lt.json.hide }, { change: (e) => { _lt.toggle(e.target.checked); } });
  }
  ...
  if (settings.equipEnchant) {
    var $battle = {
      ...
      display_inventory: function () {
        ...
        Object.entries(settings.equipEnchantInventory).forEach(([n, m]) => {
          const c = $item.count(n);
        ...
        } else if ($battle.repair.repairall === 0) {
...
if (settings.upgrade && _query.s === 'Forge' && _query.ss === 'up') {
  ...
  _up.salvage_calc = function (q) {
    ...
    const pxp_text = pxp ? `${quality} (${pxp})`
        ...
        : `${quality} ?? (Unable to calculate the pxp of this equipment)`;
  ...
  _up.salvage_init = function () {
    ...
    $input(['button', 'Materials Prices'], left, null, () => { $price.edit('Materials', _up.salvage_calc); });
  ...
  if ($id('equip_extended')) {
    ...
    _up.upgrade_sum = function () {
      ...
      _up.node.costs.innerHTML = '<tr><td>Materials</td><td>Requires</td><td>Stock</td></tr>';
    ...
    _up.upgrade_run = async function () {
      if (_up.upgrade_run.done || !_up.upgrade_sum() || !confirm('Are you sure you wish to upgrade this equipment?')) {
        return;
      }
    ...
    _up.buy_catalysts = async function () {
      ...
      await Promise.all(requests);
      _up.node.catalysts.value = 'Buy Catalysts';
      _up.load_inventory();
    };
    ...
    $element('div', _up.node.div, ['.hvut-up-buttons']).append(
      ...
      $input(['button', 'Prices'], null, { tabIndex: -1, style: 'width: 80px;' }, () => { $price.edit('Materials', _up.upgrade_sum); })
    );
...
if (settings.salvage && _query.s === 'Forge' && _query.ss === 'sa') {
  ...
  if ($id('salvage_button')) {
    confirm_event($id('salvage_button').parentNode, 'click', 'Are you sure you want to salvage this item?', () => $id('salvage_button').src.includes('salvage.png'));
  }
} else
