...
// @downloadURL https://update.greasyfork.org/scripts/22424/Loan%20Status.user.js
// @updateURL https://update.greasyfork.org/scripts/22424/Loan%20Status.meta.js
// ==/UserScript==
/**************************************
...
var HEADER_URL = 'https://raw.githubusercontent.com/CheetahJsp/PokerStatus/master/master.json';
...
function showPreferencesMenu(){
    ...
    $('BODY').append('<div id="Loan_Status_bkgrnd" style="position:absolute;top:0%;left:0%;width:100%;height:100%;background-color:black;z-index:1001;-moz-opacity:0.6;opacity:.60;filter:alpha(opacity=60)">' +
        '</div><div id="Loan_Status_forgrnd" style="position:absolute;top:25%;left:25%;width:50%;height:50%;padding:16px;border:5px solid #8FBEFF;background-color:#E4EBFF;z-index:1002;overflow:auto;color:black;"></div>');
    var holder = $('#Loan_Status_forgrnd');
    $(holder).html('<div style="font-size:12pt;"><span style="font-weight:bold;text-decoration:underline;">Loan Status Settings:</span><span id="Loan_Status_close" style="float:right;cursor:pointer;color:blue;text-decoration:underline;">Close</span></div>');
    $('#Loan_Status_close').click(function(){
        $('#Loan_Status_bkgrnd').remove();
        $('#Loan_Status_forgrnd').remove();
    ...
    if(GM_getValue('BSExpireTime') == '0'){
        $(holder).append('<br /><br /><br /><span id="Loan_Status_refreshSpan">Names List will reload when the page is refreshed (F5)</span>');
    }else{
        $(holder).append('<br /><br /><br /><span id="Loan_Status_refreshSpan"><span style="font-style:italic;">Names List Last Updated ' + timeAgo + ' minutes ago.</span> <a href="#" id="Loan_Status_refresh">Reload Now</a></span>');
        $('#Loan_Status_refresh').click(function(){
            GM_setValue('BSExpireTime','0');
            $('#Loan_Status_refreshSpan').html('Will now reload when the page is refreshed (F5)');
        });
    }
}

function showPreferencesLink(){
    //$('BODY DIV.bar DIV.barR').append('<a href="#" id="Loan_Status_pref">Loan Status Settings</a>');
    $('BODY TABLE DL DD UL.mL').append('<li><a href="#" id="Loan_Status_pref">Loan Status Settings</a></li>');
    $('#Loan_Status_pref').click(function(){
        showPreferencesMenu();
    });
}
    ...
    for(var i=0;i<nameList.length;i++){
        ...
        if(results.rank > -1){
            if(GM_getValue('BSDispType','rolled') == 'rolled'){
                function createLink(){
                    $(nameHolders[i]).append('<div class="sportStatusHolder"><a href="javascript:void(0);">Loan Status</a>: ' + colorTheStatus(results.status) + '</div>');
...
function showMultiSportsInfo(resultsArray,link){
    var offset = $(link).offset();
    $('#bsFieldset').css({top:offset.top,left:offset.left}).show();
    $('#bsFieldset LEGEND SPAN').html('Loan Status Info');
...
function showMedList(){
    ...
    $('BODY DIV.tbb DIV.links B').append('<a id="BSMedListLink" href="javascript:void(0);">Loan Med List</a>');
...
function showMediators(sportObj,link){
    var offset = $(link).offset();
    $('#bsFieldset').css({top:offset.top,left:offset.left}).show();
    $('#bsFieldset LEGEND SPAN').html('Loan Med List');
    $('#bsFieldset DIV.main').html('');
...
function showFlushLink(){
    var timeAgo = Math.round(((new Date().getTime()) - (GM_getValue('BSExpireTime') - CACHE_TIME)) / 60000);
    $('BODY DIV.crt.links').append(' | Loan Status version: ' + VERSION.toFixed(2) + ', <span id="BSBottomSpan">cached for ' + timeAgo + ' minutes <a href="javascript:void(0);" id="BSBottomFlush">flush</a></span>');
    ...
    GM_xmlhttpRequest({
        ...
        onload:function(response){
            ...
            if(LIST.version != VERSION){
                showErrorMsg('Your Loan status script is out of date, <a href="http://userscripts.org/scripts/show/50720" target="_blank">Click Here</a> to upgrade!');
            }
            gatherSport(0);
        },
        onerror:function(err){
