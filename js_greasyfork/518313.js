...
// @downloadURL https://update.greasyfork.org/scripts/518313/ChatGPT%3A%20%E8%AA%9E%E9%9F%B3%E8%BC%B8%E5%85%A5%E8%88%87%E8%AA%9E%E9%9F%B3%E5%90%88%E6%88%90%E5%8A%9F%E8%83%BD%20%28%E6%94%AF%E6%8F%B4%E4%B8%AD%E8%8B%B1%E6%97%A5%E9%9F%93%E8%AA%9E%E8%A8%80%29.user.js
// @updateURL https://update.greasyfork.org/scripts/518313/ChatGPT%3A%20%E8%AA%9E%E9%9F%B3%E8%BC%B8%E5%85%A5%E8%88%87%E8%AA%9E%E9%9F%B3%E5%90%88%E6%88%90%E5%8A%9F%E8%83%BD%20%28%E6%94%AF%E6%8F%B4%E4%B8%AD%E8%8B%B1%E6%97%A5%E9%9F%93%E8%AA%9E%E8%A8%80%29.meta.js
// ==/UserScript==
(() => {
  ...
  (async function () {
    ...
    window.speechSynthesis.onvoiceschanged = function () {
      currentVoice = speechSynthesis
        .getVoices()
        .filter((x) => x.lang === "zh-TW")
        .pop();
    };
    const isMac = () =>
    ...
    microphoneButtonElement.classList =
      "absolute p-1 rounded-md text-gray-500 bottom-1.5 right-1 md:bottom-3.5 md:right-2 hover:bg-gray-100 dark:hover:text-gray-400 dark:hover:bg-gray-900 disabled:hover:bg-transparent dark:disabled:hover:bg-transparent";
    microphoneButtonElement.style.right = "3rem";
    microphoneButtonElement.title = `\u958B\u555F\u8A9E\u97F3\u8FA8\u8B58\u529F\u80FD (${
      isMac() ? "command+option+s" : "alt+s"
    })`;
    ...
    speakerButtonElement.classList =
      "absolute p-1 rounded-md text-gray-500 bottom-1.5 right-1 md:bottom-3.5 md:right-2 hover:bg-gray-100 dark:hover:text-gray-400 dark:hover:bg-gray-900 disabled:hover:bg-transparent dark:disabled:hover:bg-transparent";
    speakerButtonElement.style.right = "5rem";
    speakerButtonElement.innerHTML = svgSpeakerOff;
    speakerButtonElement.title = `\u958B\u555F\u8A9E\u97F3\u5408\u6210\u529F\u80FD (${
      isMac() ? "command+option+m" : "alt+m"
    })`;
    ...
    speakerButtonElement.addEventListener("contextmenu", function (event) {
      ...
      selectElement.addEventListener("change", function (event2) {
        currentVoice = speechSynthesis
          .getVoices()
          .filter((x) => x.voiceURI === this.value)
          .pop();
        console.log(
          "\u4F60\u76EE\u524D\u9078\u4E2D\u7684\u8A9E\u97F3\u5408\u6210\u8072\u97F3\u662F: ",
          currentVoice
    ...
    speechRecognition.onstart = (event) => {
      logLevel >= 1 &&
        console.log(
          "\u958B\u59CB\u9032\u884C SpeechRecognition \u8A9E\u97F3\u8FA8\u8B58"
        );
    };
    speechRecognition.onerror = (event) => {
      logLevel >= 1 &&
        console.log(
          "SpeechRecognition \u8A9E\u97F3\u8FA8\u8B58\u932F\u8AA4(error)\u6216\u4E2D\u65B7(abort)!",
          event
        );
    };
    ...
    speechRecognitionStart$.subscribe(() => {
      ...
      microphoneButtonElement.title = `\u95DC\u9589\u8A9E\u97F3\u8FA8\u8B58\u529F\u80FD (${
        isMac() ? "command+option+s" : "alt+s"
      })`;
      ...
      logLevel >= 1 &&
        console.log(
          "speechRecognitionStart$ Started",
          Parts,
    ...
    speechSynthesisStart$.subscribe(() => {
      ...
      speakerButtonElement.title = `\u95DC\u9589\u8A9E\u97F3\u5408\u6210\u529F\u80FD (${
        isMac() ? "command+option+m" : "alt+m"
      })`;
    });
    ...
    const createUtteranceTextListener = () => {
      return new Observable((subscriber) => {
        ...
        var observer = new MutationObserver((mutations) => {
          logLevel >= 2 &&
            console.log(
              `\u76E3\u6E2C\u5230 ${mutations.length} \u500B\u8B8A\u66F4`,
              mutations
            );
          mutations.forEach((mutation) => {
            logLevel >= 2 &&
              console.log(
                `TYPE: ${mutation.type}, \u65B0\u589E ${mutation.addedNodes.length} \u500B\u7BC0\u9EDE\uFF0C\u522A\u9664 ${mutation.removedNodes.length} \u500B\u7BC0\u9EDE`
              );
            if (
              mutation.type === "characterData" &&
              (mutation.target.parentNode.tagName === "P" ||
                mutation.target.parentNode.tagName === "LI")
            ) {
              ...
              if (
                ...
                logLevel >= 2 &&
                  console.log("lastParagraphElement = ", lastParagraphElement);
            ...
            if (
              ...
              mutation.target.type !== "button" &&
              mutation.addedNodes.length === 1 &&
              mutation.addedNodes[0].nodeName === "svg" &&
    ...
    const listenUtteranceTextAndSpeak = () => {
      defer(() => createUtteranceTextListener())
        .pipe(
          switchMap((lastParagraphTextFromChatGPT) =>
            SpeakText(lastParagraphTextFromChatGPT)
          ),
          retry()
        )
        .subscribe({
    ...
    const selectTextToSpeak = () => {
      const toDocumentSelectedText = () => (observable2) =>
        observable2.pipe(
      ...
      fromEvent(document, "selectionchange")
        .pipe(
          ...
          switchMap((selectedText) =>
            timer(1e3).pipe(switchMap(() => SpeakText(selectedText)))
          ),
    ...
    const registerHotKeys = () => {
      ...
      const keydownEscape$ = keydown$.pipe(
        filter(
          (ev) =>
            ev.key === "Escape" &&
      ...
      const keydownEnter$ = keydown$.pipe(
        filter(
          (ev) =>
            ev.key === "Enter" &&
      ...
      const keydownAltS$ = keydown$.pipe(
        filter((ev) => altOrCommandOption(ev) && ev.code === "KeyS")
      );
      const keydownAltT$ = keydown$.pipe(
        filter((ev) => altOrCommandOption(ev) && ev.code === "KeyT")
      );
      const keydownAltR$ = keydown$.pipe(
        filter((ev) => altOrCommandOption(ev) && ev.code === "KeyR")
      );
      const keydownAltM$ = keydown$.pipe(
        filter((ev) => altOrCommandOption(ev) && ev.code === "KeyM")
      );
      ...
      keydownAltS$.subscribe((ev) => {
        microphoneButtonElement.dispatchEvent(
          new Event("click", { bubbles: true })
        );
      });
      keydownAltM$.subscribe((ev) => {
        speakerButtonElement.dispatchEvent(
          new Event("click", { bubbles: true })
        );
      });
    ...
    function addButtons() {
      ...
      textAreaElement.parentElement.insertBefore(
      ...
      .pipe(
        ...
        filter(
          (element) =>
            ...
            element.nextSibling.tagName === "BUTTON"
        ),
        take(1)
      )
      .subscribe((textarea) => {
