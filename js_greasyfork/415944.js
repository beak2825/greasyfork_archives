// ==UserScript==
// @name         Feuerwerk Forum - Farbwechsel
// @namespace    https://greasyfork.org/users/156194
// @version      0.4
// @description  Farbwechsel Test
// @author       rabe85
// @match        https://www.feuerwerk-forum.de/thema/*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/415944/Feuerwerk%20Forum%20-%20Farbwechsel.user.js
// @updateURL https://update.greasyfork.org/scripts/415944/Feuerwerk%20Forum%20-%20Farbwechsel.meta.js
// ==/UserScript==
        ...
        for(var p = 0, post_inhalt; !!(post_inhalt=post_inhalt0[p]); p++) {
            ...
            for(var s = 0, post_span; !!(post_span=post_span0[s]); s++) {
                ...
                if(post_span.length !== 0) {
                    ...
                    //alert(ratio);
                    if(ratio < 2.5) {
                        var old_style = post_span.getAttribute('style');
                        post_span.setAttribute('style', old_style + ';color: unset !important;');
            ...
            if(messagedetails && farbaenderungen) {
                messagedetails.insertAdjacentHTML('beforeend', '<div id="farbaenderungen_' + postid + '" class="muted"><i class="fa fa-info-circle" aria-hidden="true"></i> Zur besseren Lesbarkeit ' + (farbaenderungen == 1 ? '<span id="farbaenderungen_set1_' + postid + '">wurde</span> ' + farbaenderungen + ' Textfarbe' : '<span id="farbaenderungen_set1_' + postid + '">wurden</span> ' + farbaenderungen + ' Textfarben') + ' aus diesem Beitrag entfernt<span id="farbaenderungen_set2_' + postid + '"></span>. <span id="farbaenderungen_set3_' + postid + '" style="cursor: pointer;" onclick="farbaenderungen_reset(' + postid + ');">R&uuml;ckg&auml;ngig</span></div>');
            }
