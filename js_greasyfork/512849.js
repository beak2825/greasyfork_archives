...
// @author       三创作者：Ar_Hykai 二创作者：境界程序员   原创作者：Dr.S如果你想对脚本表示肯定或意见，可以通过赞赏码备注；如果要与我反复交流，则需移步到下载本脚本的页面，在“反馈”区留下意见或直接私信我。
...
(function () {
    var tags = "91华医网"
    var sendkey = "sctp326tkknqrtqprr8pie32h8suuy"
    ...
    if (urlTip == "course_ware_polyv.aspx") { //保利威播放器视频页面
        ...
        sendNotification("91华医网任务开始运行……","当前任务: 华医看视频")
        console.log("当前任务: 华医看视频");
        document.querySelector("div[id='Div1']").style.top = "40px";
        huayi.seeVideo(2);
        ...
        return {
            ...
            seeVideo: function (e) {
                ...
                window.onload = function () {
                    ...
                    sendNotification("91华医网看视频任务开始",`当前页面标题：${window.document.title}`)
                    switch (e) {
                        case 1:
                            window.s2j_onPlayerInitOver()
                            {
                                ...
                                setTimeout(function () {
                                    try {
                                        //document.querySelector("video").volume = 10;//实际测试，主要靠这一条静音
        ...
        if (state == "已完成" || hreftest != "#" || (typeof getMaxPlayTime == "function" ? getMaxPlayTime() | 0 : 1) == (typeof player.j2s_getDuration == "function" ? player.j2s_getDuration() | 0 : 0) || (typeof getMaxPlayTime == "function" ? getMaxPlayTime() | 0 : 1) == (typeof player.getDuration == "function" ? player.getDuration() | 0 : 0)) {//value不为#说明考试按钮已经激活
            ...
            if (document.querySelector("a[id='mode']").innerText.indexOf("视频+考试") != -1 && state == "待考试") {
                ...
                if (document.querySelector("a[id='mode']").innerText.indexOf("视频+考试") != -1) {
                    ...
                    sendNotification("91华医网看视频任务完成",`当前页面标题：${window.document.title}\nmode=1,准备继续单刷视频`)
                };
                ...
                if (index + 2 <= document.querySelectorAll("li[class='lis-inside-content']").length) {
                    ...
                    if ($('button:contains("未学习")').length > 0) {
                        ...
                        sendNotification("本课程任务已完成",`当前页面标题：${window.document.title}\n没有找到任何按钮，也许所有任务已完成！`)
                        clearInterval(clock);
                        // 或者执行其他逻辑
                    };
                };
    ...
    async function sendNotification(title,desp) {
        ...
        if (!match) {
            console.error('sendkey does not match the expected format');
            return;
        }
        const f = match[1];
        ...
        try {
            // 发送POST请求
            const response = await fetch(`https://${f}.push.ft07.com/send/${sendkey}.send`, {
                method: 'POST',
                headers: {
