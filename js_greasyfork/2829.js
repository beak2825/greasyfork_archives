// ==UserScript==
// @name        Custom UI
// @description Custom Salesforce UI 
// @namespace   http://userscripts.org/users/126885
// @include     https://*.salesforce.com/*
// @version     1.2
// @downloadURL https://update.greasyfork.org/scripts/2829/Custom%20UI.user.js
// @updateURL https://update.greasyfork.org/scripts/2829/Custom%20UI.meta.js
// ==/UserScript==
...
function show_settFloat(e) {
	if (e.which==81 && e.ctrlKey){
		...
		var elements = document.getElementById("SFDCSettings").elements;//forms['SFDCSettings'].elements;
...
(function(){
	...
	if(document.getElementById("ep") != null){
		...
		dim.id="dim";
		css +="#dim {height:100%; width:100%; position:fixed; left:0; top:0; background-color:white; opacity:0.7; visibility: hidden;}";
		...
		var sf = '<b style="margin-left:130px ;font-size: 1.1em;">SFDC Changes Settings</b><br><br><form name="SFDCSettings" id="SFDCSettings" onsubmit="settSave();">';
	...
	if(GM_getValue("sidebarHide","false")=="true" || GM_getValue("sidebarFloat","false")=="true"){
		if(document.getElementById("sidebarCell") != null){
			...
			document.getElementById("sidebarCell").addEventListener("mouseover",function(a) {
						mo_timeout = setTimeout(function(){document.getElementById("sidebarCell").setAttribute("class", "sidebarCell sidebarCollapsible");},500)
					},false);
			document.getElementById("sidebarCell").addEventListener("mouseout",function(a) {
						if (!a) var a = window.event;
						var tg = (window.event) ? a.srcElement : a.target;
						//alert(tg.id);
						if (tg.id != 'sidebarDiv') return;
						var reltg = (a.relatedTarget) ? a.relatedTarget : a.toElement;
	...
	css+="#CDI {width:100%; overflow:auto;}";
	css+="#CD {width:50%; float:left; border-right:1px solid #c0be72;margin-right:-1px;}	";
	css+="#CI {width:50%; float:left; border-left:1px solid #c0be72; margin-left:-1px;}";	
