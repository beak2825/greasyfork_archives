// ==UserScript==
// @name         Overlay FL Miami-Dade Parcel #1
// @namespace    https://greasyfork.org/users/45389
// @version      0.4.4.001
// @description  Adds a layer to display Parcel info for Miami-Dade Co FL
// @author       MapOMatic
...
// @downloadURL https://update.greasyfork.org/scripts/37000/Overlay%20FL%20Miami-Dade%20Parcel%201.user.js
// @updateURL https://update.greasyfork.org/scripts/37000/Overlay%20FL%20Miami-Dade%20Parcel%201.meta.js
// ==/UserScript==
...
(function() {
    ...
    var _settingsStoreName = 'wme_FL_Miami-Dade_Co-Parcel1';
    ...
    function log(message, level) {
        if (message && (!level || (level <= _debugLevel))) {
            console.log('FL Miami-Dade Co parcel1: ', message);
        }
    }
    ...
    function getUrl(extent, zoom) {
        ...
        var url = 'https://gisweb.miamidade.gov/arcgis/rest/services/MD_Emaps/MapServer/72/query?geometry=' + encodeURIComponent(geometryStr);
        url += '&returnGeometry=true';
        //url += '&maxAllowableOffset=' + offsets[zoom];
        url += '&outFields=' + encodeURIComponent('TRUE_SITE_ADDR');
        ...
        if (_mapLayer !== null) {
            ...
            for (var i=0;i<_mapLayer.features.length;i++){
                ...
                if(feature.geometry.containsPoint(mapCenter)) {
                    ...
                    url = 'http://gisweb.miamidade.gov/PublicNotification/?address=' + num + '';
                    $span.append($('<a>', {href:url, target:'__blank', title:'Search Address on County GIS'}).text(text).css({color:'white',display:'inline-block'}));
                    GM_xmlhttpRequest({
    ...
    function processBoundaries(states, token) {
        ...
        states.forEach(function(state) {
            var attributes = {
                name: state.attributes.TRUE_SITE_ADDR
            };
    ...
    function initLayer(){
        ...
        _mapLayer = new OpenLayers.Layer.Vector("Overlay FL Miami-Dade Co Parcels1", {
            uniqueName: "__WMEFLmiamidadecopar",
        ...
        WazeWrap.Interface.AddLayerCheckbox("overlay", "FL Miami-Dade Parcel 1", _settings.layerVisible, onLayerToggleChanged);
    }
