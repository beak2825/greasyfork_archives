...
(function () {
    ...
    const RAW_REGEX_RULES = [
        ...
        [/^Multiplies by (\d+\.?\d*)x the base damage of moves with equal or less than (\d+) power/i, '威力小于等于$2的招式威力乘$1倍'],
        [/^Multiplies the damage by x(\d+\.?\d*) when the opposite Pokemon shares a type/i, '目标与自身属性相同时，伤害乘$1倍'],
        [/^"([^"]+)"-related moves have their base damage multiplied by x([\d.]+)/i, '"$1"类招式基础威力乘$2倍'],
        [/^"([^"]+)"-related moves are executed twice as fast/i, '"$1"类招式速度变快为原来的2倍'],
        [/^Multiplies Same-Type-Attack-Bonus by x(\d+\.?\d*)/i, '属性一致加成增加0.2'],//原文属性一致加成乘$1倍
        [/^Positive secondary effect of damaging moves are removed, and their damage is multiplied by x([\d\.]+)/i, '移除进攻招式的正面追加效果，伤害乘$1倍(仅包含 进攻速度修正、招式伤害修正 的招式除外)'],
        [/^Multiplies by x(\d+\.?\d*) the power of (\w+)-type moves of all team members/i, '我方全队$2属性招式威力乘$1'],
        [/^Increases the Damage dealt by x(\d+\.?\d*) for every team member defeated/i, '每有一名队友倒下，造成伤害乘$1倍'],
        [/^Increases the Damage dealt by x(\d+\.?\d*) when afflicted with (\w+), and nullifies the damage overtime caused by it/i, '处于$2状态时，造成伤害乘$1倍，并免疫该状态的持续伤害'],
        ...
        [/^All moves become (\w+)-type moves, and their power is multiplied by x(\d+\.?\d*)/i, '所有招式变为$1属性，威力乘$2倍'],
        [/^Normal-type moves become (\w+)-type moves, and their power is multiplied by x(\d+\.?\d*)/i, '一般属性招式变为$1属性，威力乘$2倍'],
        ...
        [/^Moves that execute (\w+) than usual have their move power multiplied by x(\d+\.?\d*)/i, '进攻速度变$1的招式，招式威力乘$2倍'],
