...
// @name 		[1180] clan menu
// @version 	1.6
// @description 	clan menu - Replaces the link on the roulette clan Menu
...
// @downloadURL https://update.greasyfork.org/scripts/8477/%5B1180%5D%20clan%20menu.user.js
// @updateURL https://update.greasyfork.org/scripts/8477/%5B1180%5D%20clan%20menu.meta.js
// ==/UserScript==
...
var hard_site_link = 		"http://web-spider.clan.su/"; 	// адрес клан-сайта  по умолчанию
var hard_gift_box_log = 		"gift_box_log.php"; 	    
var hard_clan_chat = 		"/forum";		// клан-чат на сайте 	         //-//
var hard_main_server = 	"http://web-spider.clan.su";	// адрес сервера ДМ          //-//
var hard_alt_server = 		"http://web-spider.clan.su";		// альтернативный сервер  //-//
var hard_def_servise = 	"http://1180.bl.ee";	// Таблица защит       //-//
var hard_name_plink_1 = 	"Ссылка #1";
var hard_name_plink_2 = 	"Ссылка #2";
...
var kks_stat = 	"http://hwm.kekus.de/"; // Статистика от Кекуса:
...
var str_url = 	"https://greasyfork.org/ru/users/9016-mefistophel-gr";

//====== Переменные, заменяемые в настройках =======	// 
var clan_name = 	GM_getValue("new_clan_name", "Паутина"); 	// название клана
var clan_id = 	GM_getValue("new_clan_id", 1180); 		// ID клана
var sklad_id = 	GM_getValue("new_sklad_id", 12); 		// ID склада клана
// var clan_icon = 	GM_getValue("new_clan_icon", false); 		// true - отображать значок клана, false - не отображать

var akdm1_name = 	GM_getValue("new_akdm1_name", "первый"); 	// название клан 1
var akdm1_id = 	GM_getValue("new_akdm1_id", ""); 		// ID  клан 1
var akdm2_name = 	GM_getValue("new_akdm2_name", "клан 2"); 	// название клан 2
var akdm2_id = 	GM_getValue("new_akdm2_id", ""); 		// ID  клан 2
...
var str_txt_akdm1_name = 	ustring("Тех. клан №1: ");
var str_txt_akadem_id_1 = 	ustring("Номер тех.клана #1: ");
var str_txt_akdm2_name = 	ustring("Тех. клан #2: ");
var str_txt_akadem_id_2 = 	ustring("Номер тех.клана #2: ");
...
var str_txt_clan_chat = 	ustring("Форум на сайте: ");
var str_txt_main_server = 	ustring("Адрес сервера: ");
...
var str_txt_def_servise = 	ustring("Таблица защит: ");
...
var str_txt_add_serv_DM = 	ustring(" сервер клана");
var str_txt_add_serv_alt = 	ustring(" альтернативн. сервер");

var str_user_url = 		ustring("для Бойцов клана: ");
var str_gift_box_log = 		ustring("&nbsp;Редкие ларцы");
var str_gift_box_log_title = 	ustring("Доступно для всех");    
...
var gift_box_log = 	GM_getValue("hwm_gift_box_log", true);    
...
var gift_box_log = 	GM_getValue("hwm_gift_box_log", false);    
...
function Open_Settings() {
    ...
    d.setAttribute('style', 'position: absolute; margin: -110px 0px 0px 735px; text-align: center;'); 	//Положение кнопки вызова настроек
    d.innerHTML = '<style> .hwm_cmenu * {font-size: 12px; color: #F5C137;} .cell_cmenu {border: 1px; border-color: #F5C137; border-style: solid; white-space: nowrap; height: 18px; background: url(http://dcdn1.heroeswm.ru/i/top/line/t_com_bkg.jpg); font-weight: bold;} </style>' +
    '<table class="hwm_cmenu" width=100%px> <tr height=20>'+ 	// Размер и фон, для кнопки вызова настроек
    '<td class="cell_cmenu" style="cursor:pointer" id="set_Clan_Menu" title="'+str_button_title+'">'+ str_button +'</td>' +
    '</tr> </table>';
    point.parentNode.parentNode.parentNode.appendChild(d);

    addEvent( $("set_Clan_Menu"), "click", settings_Clan_Menu );
...
function settings_Clan_Menu () {
    ...
    bgc.style.left = ( (ClientWidth() - 950) / 2 )+'px';
    bgc.style.width = '950px';
	...
	'<tr> <td width="21%" align="right"><b>'+str_txt_clan_name+'</b></td> 	<td align="left" width="26%" style="border-right: solid 2px gray;">&nbsp;<input id="set_clan_name" title="'+str_clan_name_title_1+'" value="'+clan_name+'" size="13" maxlength="10"> <input type="submit" id="set_clan_name_ok" value="OK" title="'+str_clan_name_title_2+'"></td> 	<td colspan=2 align="center"><font style="color:#6A5ACD; font-family: Tahoma; font-size:15px; line-height: 1.5">'+str_show_url+'</font></td> </tr>'+
	'<tr> <td colspan=2 align="center" valign="top" style="border-right: solid 2px gray;"><label style="cursor:pointer;" title=""> <b></b></label></td> 	<td align="center" style="border-bottom: solid 2px gray;"><font style="line-height: 2"><i><b>'+str_user_url+'</b></i></font></td> 	<td align="center" style="border-bottom: solid 2px gray;"><font style="line-height: 2"><i><b>'+str_admin_url+'</b></i></font></td> </tr>'+
	'<tr> <td align="right"><b>'+str_txt_clan_id+'</b></td> 	<td align="left" style="border-right: solid 2px gray;">&nbsp;<i><b>'+str_txt_id+'</b></i><input id="set_clan_id" value="'+clan_id+'" size="4" maxlength="4"> <input type="submit" id="set_clan_id_ok" value="OK" title="'+str_txt_id_title+'"> </td> 	<td><label style="cursor:pointer;" title="'+str_gift_box_log_title+'"><input type=checkbox '+(gift_box_log =="1"?"checked":"")+'  id=set_gift_box_log>'+str_gift_box_log+'</label></td> 	<td><label style="cursor:pointer;"><input type=checkbox '+(clan_log =="1"?"checked":"")+'  id=set_clan_log>'+str_clan_log+'</label></td> </tr>'+
	'<tr> <td align="right"><b>'+str_txt_sklad_id+'</b></td> 	<td align="left" style="border-right: solid 2px gray;">&nbsp;<i><b>'+str_txt_id+'</b></i><input id="set_sklad_id" title="'+str_absent_title+'";" value="'+sklad_id+'" size="4" maxlength="3"> <input type="submit" id="set_sklad_id_ok" value="OK" title="'+str_txt_id_title+'"> </td> 	<td><label style="cursor:pointer;"><input type=checkbox '+(clan_site =="1"?"checked":"")+'  id=set_clan_site>'+str_clan_site+'</label></td>  	<td><label style="cursor:pointer;"><input type=checkbox '+(store_log =="1"?"checked":"")+'  id=set_store_log>'+str_store_log+'</label></td> </tr>'+
	'<tr> <td colspan=2 style="border-right: solid 2px gray;"> <hr width="99%" align="left" color="gray"/> </td>  	<td><label style="cursor:pointer;"><input type=checkbox '+(clan_store =="1"?"checked":"")+'  id=set_clan_store>'+str_clan_store+'</label></td>	<td><label style="cursor:pointer;" title="'+str_clan_contr_title+'"><input type=checkbox '+(clan_contr =="1"?"checked":"")+'  id=set_clan_contr>'+str_clan_contr+'</label></td></tr>'+
	'<tr> <td align="right"><b>'+str_txt_akdm1_name+'</b></td> 	<td align="left" style="border-right: solid 2px gray;">&nbsp;<input id="set_akdm1_name" title="'+str_akdm_name_title+'";" value="'+akdm1_name+'" size="13" maxlength="11"> <input type="submit" id="set_akdm1_name_ok" value="OK" title="'+str_clan_name_title_2+'"></td>  	<td><label style="cursor:pointer;" title="'+str_clan_def_title+'"><input type=checkbox '+(clan_def =="1"?"checked":"")+'  id=set_clan_def>'+str_clan_def+'</label></td> 	<td><label style="cursor:pointer;" title="'+str_clan_memb_title+'"><input type=checkbox '+(clan_memb =="1"?"checked":"")+'  id=set_clan_memb>'+str_clan_memb+'</label></td></tr>'+
...
//    addEvent($("set_clan_icon"), "click", change_clan_icon); 	//чек-бокс
    ...
    addEvent($("set_akdm1_name_ok"), "click", change_akdm1_name); 	//строка ввода - имя клан 1
    addEvent($("set_akdm1_id_ok"), "click", change_akdm1_id);		//строка ввода - ID клан #1
    addEvent($("set_akdm2_name_ok"), "click", change_akdm2_name); 	//строка ввода - имя клан 2
    addEvent($("set_akdm2_id_ok"), "click", change_akdm2_id);		//строка ввода - ID клан #2
    ...
    addEvent($("set_gift_box_log"), "click", change_gift_box_log); 	//чек-бокс    
...
function change_gift_box_log()	{GM_setValue("hwm_gift_box_log", gift_box_log = !gift_box_log);}    
...
function change_akdm1_name() {	// Название клан 1
    ...
    if (akdm1_name.length == 0) {
        GM_setValue("new_akdm1_name", "клан 1");
...
function change_akdm2_name() {	// Название клан 2
    ...
    if (akdm2_name.length == 0) {
        GM_setValue("new_akdm2_name", "клан 2");
...
function change_akdm1_id() {		// ID клан 1
    ...
    if ( akdm1_id.length == 0 ) {
        GM_setValue("new_akdm1_name", "клан 1");
        document.getElementById('set_akdm1_name').value = "клан 1";
...
function change_akdm2_id() {		// ID клан 2
    ...
    if ( akdm2_id.length == 0 ) {
        GM_setValue("new_akdm2_name", "клан 2");
        document.getElementById('set_akdm2_name').value = "клан 2";
...
function change_clan_chat() {		// Форум на сайте
...
function change_def_servise() {		// Cервис координации
    def_servise = ustring($ustring($("set_def_servise").value).replace(/[^ 0-9A-Za-z:\-/.]/g, "")); //удаление лишних символов
...
function change_link_plink_1() {		// Изменение Личной ссылки #1
    ...
    link_plink_1 = ustring($ustring($("set_link_plink_1").value).replace(/[^ 0-9A-Za-z:\-/.]/g, "")); //удаление лишних символов
...
function change_link_plink_2() {		// Изменение Личной ссылки #2
    ...
    link_plink_2 = ustring($ustring($("set_link_plink_2").value).replace(/[^ 0-9A-Za-z:\-/.]/g, "")); //удаление лишних символов
...
function open_transfer() {
   if ( location.href.match('lordswm') ) {
	window.location = "transfer.php?nick=berkut009&shortcomment=Transferred 10000 Gold 5 Diamonds";
   } else {
	window.location = "transfer.php?nick=berkut009&shortcomment=%CF%E5%F0%E5%E4%E0%ED%EE%2010000%20%C7%EE%EB%EE%F2%EE%205%20%C1%F0%E8%EB%EB%E8%E0%ED%F2%FB";
   }
}
...
function ScanClanID(clan_id) {
    ...
    method: "GET",
    url: "http://"+location.hostname+"/clan_info.php?id=" + clan_id,
    synchronous: true,
    });
    if(ans.responseText.indexOf('#'+clan_id) > -1) return true;
    else return false;
}
...
function ScanSkladID(sklad_id) {
    var ans = GM_xmlhttpRequest({
    method: "GET",
    url: "http://"+location.hostname+"/sklad_info.php?id=" + sklad_id,
    synchronous: true,
    });
    if(ans.responseText.indexOf('sklad_log.php?id='+ sklad_id) > -1) return true;
    else return false;
}
...
var h = document.getElementsByTagName('a');
    for(var i = 0; i < h.length; i++) {
        if(h[i].href.indexOf("roulette.php") > -1) {
           ...
           // if(clan_icon) { menu += "<img width='20' height='14' border='0' align='right' alt='#"+clan_id+"' src='i_clans/l_"+clan_id+".gif'></img>"; }
                menu += "<font style='bold; '><b><a style='text-decoration: none;color: #ffd875;' href='clan_info.php?id="+clan_id+"'>&nbsp;"+clan_name+"</a></b></font> </nobr> <ul>";
            if(gift_box_log) { menu += "<li><a href='gift_box_log.php'  title='"+str_gift_box_log_title+"'>&nbsp; Редкие ларцы</a></li>"; }
            if(clan_site) { menu += "<li><a href="+site_link+">&#187; Cайт клана</a></li>"; }
                menu += "<li><a href='clan_info.php?id="+clan_id+"'>&#187; Информация о клане</a></li>";
            if(clan_log) { menu += "<li><a href='clan_log.php?id="+clan_id+"'>&#187; Протокол клана</a></li>"; }
            if(clan_store) { menu += "<li><a href='sklad_info.php?id="+sklad_id+"&cat=0'>&#187; Клановый склад</a></li>"; }
            if(store_log) { menu += "<li><a href='sklad_log.php?id="+sklad_id+"'>&#187; Протокол склада</a></li>"; }
            if(clan_def) { menu += "<hr/> <li><a href="+def_servise+" title='"+str_clan_def_title+clan_name+"'>Сервис записи на Защиту</a></li>"; } 
            if(clan_irc) { menu += "<li><a href="+site_link+clan_chat+" title='"+str_clan_irc_title+"'>Форум на сайте клана</a></li>"; }
            if(clan_contr) { menu += "<hr/> <li><a href='clan_control.php?id="+clan_id+"' title='"+str_clan_contr_title+"'>Управление Кланом</a></li>"; }
            if(clan_memb) { menu += "<li><a href='clan_members.php?id="+clan_id+"' title='"+str_clan_memb_title+"'>Управление Составом</a></li>"; }
            if(clan_inv) { menu += "<li><a href='clan_invites.php?id="+clan_id+"' title='"+str_clan_inv_title+"'>Приглашение в Клан</a></li>"; }
            if(clan_acc) { menu += "<li><a href='clan_balance.php?id="+clan_id+"' title='"+str_clan_acc_title+"'>Управление Казной клана</a></li>"; }
          
            if(glory_points) { menu += "<li><a href='clan_glory.php?id="+clan_id+"' title='"+str_glory_points_title+"'>Управление Очками БС</a></li>"; }
            if(mil_policy) { menu += "<li><a href='clan_bplan.php?id="+clan_id+"' title='"+str_mil_policy_title+"'>Военная политика клана</a></li>"; }
            if(cast_main) { menu += "<li><a href='clan_broadcast.php?id="+clan_id+"' title='"+str_cast_all_title+"'>Рассылка по "+clan_name+"</a></li>"; }
            if(cast_add_1 && akdm1_id != "") { menu += "<li><a href='clan_broadcast.php?id="+akdm1_id+"' title='"+str_cast_all_title+"'>Рассылка по "+akdm1_name+"</a></li>"; } else { GM_setValue("hwm_cast_add_1", false); }
            if(cast_add_2 && akdm2_id != "") { menu += "<li><a href='clan_broadcast.php?id="+akdm2_id+"' title='"+str_cast_all_title+"'>Рассылка по "+akdm2_name+"</a></li>"; } else { GM_setValue("hwm_cast_add_2", false); }
            if(stat_auction) { menu += "<hr/> <li><a href="+main_server+mark_stat+" title='"+str_stat_auction_title+"'>Статистика рынка</a></li>"; }
            if(stat_buy) { menu += "<li><a href="+main_server+pers_stat + getCookie('pl_id')+" title='"+str_stat_buy_title+"'>Статистика моих покупок</a></li>"; }
            if(stat_store) { menu += "<hr/> <li><a href="+kks_stat+kks_st_sklad+" title='"+str_stat_store_title+"'>Статистика по Складу</a></li>"; }
            if(stat_treas) { menu += "<li><a href="+kks_stat+kks_st_finance+" title='"+str_stat_treas_title+"'>Статистика по Финансам</a></li>"; }
            if(stat_def) { menu += "<hr/> <li><a href="+kks_stat+kks_st_defers+" title='"+str_stat_def_title+"'>Статистика по Защитам</a></li>"; }
            if(stat_elem) { menu += "<li><a href="+kks_stat+kks_st_element+" title='"+str_stat_elem_title+"'>Статистика по Элементам</a></li>"; }
            if(stat_battle) { menu += "<li><a href="+kks_stat+kks_st_bat_char+" title='"+str_stat_battle_title+"'>Статистика боеа</a></li>"; }
            if(stat_lgnd) { menu += "<hr/> <li><a href="+lgnd_stat + getCookie('pl_id')+" title='"+str_stat_lgnd_title+"'>Статистика на <b>legend.ru</b></a></li>"; }
            if(pers_link_1 && link_plink_1 != "") { menu += "<hr/> <li><a href="+link_plink_1+">"+name_plink_1+"</a></li>"; } else { GM_setValue("hwm_pers_link_1", false); } 
            if(pers_link_2 && link_plink_2 != "") { menu += "<li><a href="+link_plink_2+">"+name_plink_2+"</a></li>"; } else { GM_setValue("hwm_pers_link_2", false); } 
            menu += "</ul>";
