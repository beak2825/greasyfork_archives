...
// @name         AcWing Better++
// @version      3.29.1
// @description  在AcWing Better!的基础上支持多设备同时登录功能(改自北极小狐的AcWing Better!)
// @author       CN059
...
// @license      MIT
// @namespace    https://greasyfork.org/zh-CN/users/1426478
// @downloadURL https://update.greasyfork.org/scripts/530971/AcWing%20Better%2B%2B.user.js
// @updateURL https://update.greasyfork.org/scripts/530971/AcWing%20Better%2B%2B.meta.js
// ==/UserScript==
...
const sessionId = getGMValue("sessionId", "");

// 检查用户是否登录
function checkLoginStatus() {
    // 检查是否有登录/注册按钮
    const hasLoginButton = document.querySelector('a[href="#"][data-toggle="modal"][data-target="#login-modal"]') !== null;
    return !hasLoginButton; // 如果有登录按钮，表示未登录
}

// 检查sessionId是否存在和登录状态
function checkSessionAndLogin() {
    const isLoggedIn = checkLoginStatus();
    const hasSessionId = document.cookie.includes('sessionid=');

    if (!isLoggedIn || !hasSessionId) {
        showSessionNotification("您的账号信息已过期，请在设置中更新sessionId", "error");
    }
}
...
// 显示通知
function showSessionNotification(message, type) {
    // 防止重复显示通知
    if (document.getElementById('acwing-session-notification')) {
        return;
    }

    const notification = document.createElement('div');
    notification.id = 'acwing-session-notification';
    notification.style.position = 'fixed';
    notification.style.top = '20px';
    notification.style.right = '20px';
    notification.style.padding = '15px 20px';
    notification.style.backgroundColor = type === 'error' ? '#ffebee' : '#e8f5e9';
    notification.style.color = type === 'error' ? '#c62828' : '#2e7d32';
    notification.style.border = type === 'error' ? '1px solid #ef9a9a' : '1px solid #a5d6a7';
    ...
    notification.innerHTML = `
        <span>${message}</span>
        <svg style="margin-left: 10px; cursor: pointer;" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <line x1="18" y1="6" x2="6" y2="18"></line>
            <line x1="6" y1="6" x2="18" y2="18"></line>
        </svg>
    `;

    const closeBtn = notification.querySelector('svg');
...
const ACwingBetterSettingMenuHTML = `
    ...
    <div class="session-id-container">
        <label for="sessionIdInput" class="session-id-label">Session ID</label>
        <input type="text" id="sessionIdInput" class="session-id-input" placeholder="输入您的sessionId" value="${getCookie('sessionid') || sessionId}">
        <p style="font-size: 12px; color: #666; margin-top: 5px;">用于保持登录状态，可在浏览器开发者工具中查看</p>
    </div>
    <br>
...
$(document).ready(function () {
    $(".ACBetter_setting").click(function () {
        ...
        $("#save").click(function () {
            ...
            // 保存sessionId到GM值和cookie
            const newSessionId = $("#sessionIdInput").val().trim();
            GM_setValue("sessionId", newSessionId);
            setCookie('sessionid', newSessionId, 30); // 始终更新cookie
