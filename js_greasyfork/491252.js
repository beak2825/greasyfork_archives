// ==UserScript==
// @name            赛尔号启航Pc-tztest
// @name:en         赛尔号启航Pc-tztest
// @namespace       赛尔号启航Pc-tztest
// @version         1.0.6
// @description     赛尔号启航Pc.
...
// @downloadURL https://update.greasyfork.org/scripts/491252/%E8%B5%9B%E5%B0%94%E5%8F%B7%E5%90%AF%E8%88%AAPc-tztest.user.js
// @updateURL https://update.greasyfork.org/scripts/491252/%E8%B5%9B%E5%B0%94%E5%8F%B7%E5%90%AF%E8%88%AAPc-tztest.meta.js
// ==/UserScript==
(function () {
    ...
    var querybattle = false
    var getEffortValue = false
    ...
    if (topWindow == window) {
        ...
        tosmall.style.width = "25px";
        tosmall.style.height = "20px";
        // 在点击按钮时输出日志到文本框中
        tosmall.onclick = function () {
            if (tosmall.innerHTML == "缩") {
                tosmall.innerHTML = "展"
        ...
        button0.innerHTML = "⚡启航护肝小助手3.1.2⚡\n  ";
        ...
        supportTxt.innerHTML = "❤群744308936【永久免费】❤";
        ...
        var ziyuan2 = document.createElement("button");
        ziyuan2.innerHTML = "资源2";
        ziyuan2.style.margin = "4px";
        ziyuan2.style.width = "50px";
        ziyuan2.style.height = "30px";
        // 在点击按钮时输出日志到文本框中
        ziyuan2.onclick = async function () {
            if (rewrite == 0) {
                showToast("请先开启监听！宝贝")
                return
            }
        ...
        battleFloor.onclick = async function () {
            ...
            await campaign_pet_fight()
        };
        div.appendChild(battleFloor);

        // var stopStatus = false
        var WaterDroplet = document.createElement("button");
        WaterDroplet.innerHTML = "刷水滴";
        WaterDroplet.style.margin = "4px";
        WaterDroplet.style.width = "45px";
        WaterDroplet.style.height = "30px";
        // 在点击按钮时输出日志到文本框中
        WaterDroplet.onclick = async function () {
            if (rewrite == 0) {
                showToast("请先开启监听！宝贝")
                return
            }
        ...
        div.appendChild(WaterDroplet);
        // var stopStatus = false
        var Lukebattle = document.createElement("button");
        Lukebattle.innerHTML = "鲁克大作战";
        Lukebattle.style.margin = "4px";
        Lukebattle.style.width = "45px";
        Lukebattle.style.height = "30px";
        // 在点击按钮时输出日志到文本框中
        Lukebattle.onclick = async function () {
            if (rewrite == 0) {
                showToast("请先开启监听！宝贝")
                return
            }
        ...
        div.appendChild(Lukebattle);

        async function lukebattle() {
            while (1) {
                await sendMsg(1176, {
                    "planetId": 6,
                })
                ...
                writeLog("当前时间：" + hours)
                if (hours < 5) {
                    writeLog("未到开放时间，当前时间："+ hours)
                    break
                }
                while (1) {
                    if (lukeReword.iffind == true) {
                        lukeReword.iffind = false
                        break
                    }
                    await wait(50)
                }
                if (hours >= 5 && hours < 12) {
                    if (lukeReword.morning == 10) {
                        writeLog("当前时段已完成"+ hours)
                        break
                    }
                    await doLukeBattle(322, lukeReword.morning)
                }
                if (hours >= 12 && hours < 18) {
                    if (lukeReword.afternoon == 10) {
                        writeLog("当前时段已完成"+ hours)
                        break
                    }
                    await doLukeBattle(323, lukeReword.afternoon)
                }

                if (hours >= 18 && hours < 24) {
                    if (lukeReword.night == 10) {
                        writeLog("当前时段已完成"+ hours)
                        break
                    }
                    await doLukeBattle(324, lukeReword.night)
                }
        ...
        async function doLukeBattle(levelId, times) {
            writeLog("当前已完成奖励次数："+ times)
                await sendMsg(1172, {
        ...
        // var stopStatus = false
        var BlackActivity = document.createElement("button");
        BlackActivity.innerHTML = "暗布瞬杀";
        BlackActivity.style.margin = "4px";
        BlackActivity.style.width = "45px";
        BlackActivity.style.height = "30px";
        // 在点击按钮时输出日志到文本框中
        BlackActivity.onclick = async function () {
            if (rewrite == 0) {
                showToast("请先开启监听！宝贝")
                return
            }
        ...
        div.appendChild(BlackActivity);

        async function blackActivity() {
            bossDamage = 0
            while (1) {
                if (rewrite == 0) {
                    break
                ...
                await sendMsg(10611, {
                    "activityCard": bossCard,
                    "activityType":1,
                    "bossLevel": 0,
                    "friendUid":0,
                    "fun":3,
                    "getTime":[1707358755,1703779698,1709663630],
                    "isSuper":0
                })

                await beforeSendSkill()
                ...
                let result = await checkSuccessAndRun()
                if (result == true) {
                    bossCard = 0
                    break
                }
        ...
        async function waterDroplet() {
            while (1) {
                try {
                    var cap = document.querySelector("iframe").contentWindow.ItemManager.getInstance().getItemNumById(100021)
                } catch (e) {
                    var cap = ItemManager.getInstance().getItemNumById(100021)
                }
                if (cap == 5) {
        ...
        var ptbattleFloor = document.createElement("button");
        ptbattleFloor.innerHTML = "阶梯";
        ptbattleFloor.style.margin = "4px";
        ptbattleFloor.style.width = "45px";
        ptbattleFloor.style.height = "30px";
        // 在点击按钮时输出日志到文本框中
        ptbattleFloor.onclick = async function () {
            if (rewrite == 0) {
                showToast("请先开启监听！宝贝")
                return
            }
        ...
        div.appendChild(ptbattleFloor);


        var actionPvpBtn = document.createElement("button");
        actionPvpBtn.innerHTML = "PVP";
        actionPvpBtn.style.margin = "4px";
        actionPvpBtn.style.width = "45px";
        actionPvpBtn.style.height = "30px";
        // 在点击按钮时输出日志到文本框中
        actionPvpBtn.onclick = async function () {
            if (rewrite == 0) {
                showToast("请先开启监听！宝贝")
                return
            }
            if (getSelectedOptionsPVP() == "dianfeng") {
                await dianfeng()
            }
            if (getSelectedOptionsPVP() == "luandou") {
                await luandou()
            }

            if (getSelectedOptionsPVP() == "jingji") {
                await jingji()
            }
            if (getSelectedOptionsPVP() == "wangzhe") {
                await wangzhe()
            }


        };
        div.appendChild(actionPvpBtn);

        // 创建单选按钮1
        var radio1 = document.createElement('input');
        radio1.type = 'radio';
        radio1.name = 'optionPVP';
        radio1.value = 'luandou';
        radio1.style.marginLeft = "10px";
        // 创建标签1
        var label1 = document.createElement('label');
        label1.innerHTML = '乱斗';
        ...
        var radio2 = document.createElement('input');
        radio2.type = 'radio';
        radio2.name = 'optionPVP';
        radio2.value = 'dianfeng';
        // 创建标签2
        var label2 = document.createElement('label');
        label2.innerHTML = '巅峰';
        label2.style.fontSize = "10px";
        ...
        radio4.type = 'radio';
        radio4.name = 'optionPVP';
        radio4.value = 'wangzhe';
        // 创建标签3
        var label4 = document.createElement('label');
        label4.innerHTML = '王者';
        label4.style.fontSize = "10px";
        // 添加单选按钮和标签到div
    ...
    async function checkSuccessAndRun() {
        try {
            await sendMsg(1057, {
                "groupId": ""
            })
            ...
            if (battleResult == 1) {
                ...
                await sendMsg(801, {})
                while(1) {
                    if (getEffortValue == true && querybattle == true) {
                        getEffortValue = false
                        querybattle = false
                        break
                    }
                    await wait(50)
    ...
    async function beforeSendSkill() {
        // writeLog("开始进入战斗")
        run = false
        querybattle = false
        getEffortValue = false
    ...
    async function sendSkillToSuccess(nowSkillId) {
        ...
        // 1172 8209 1042 1045 1057 点技能 1045 1176（切场景）
        doneRound = false
        let CountBattle = 0
        while (1) {
            if (roundEnd == true) {
    ...
    function sendMsg(nowCmd, nowBody) {
        return new Promise((resolve) => {
            ...
            // writeLog({ "指令": nowCmd, "发送体": nowBody });
            // writeLog({ "指令": nowCmd});
    ...
    function getSelectedOptionsPVP() {
        var radios = document.getElementsByName('optionPVP');
        var selectedOptions = [];
    ...
    async function campaign_pet_fight() {
        let date = new Date().getDay()
        let hour = new Date().getHours()
        await sendMsg(4865,
            {}
        )
        var r = xls.petTest.getItems();
        var configInfos = [];
        var maxLevelId = 0;
        while (1) {
            if (douziBattleInfo.bossType) {
                break
            }
            await wait(50)
        }
        writeLog(`今天的bossType： ` + douziBattleInfo.bossType)
        for (var i = 0; i < r.length; i++) {
            if (r[i].bossType == douziBattleInfo.bossType) {
                 configInfos.push(r[i]);
                 writeLog(`找到本日对应的bossId: `+ r[i].id)
                 if  (r[i].id > maxLevelId) {
                    maxLevelId = r[i].id
                 }
            }
        }
        writeLog(`找到本日最大的bossId: `+ maxLevelId)
        while (1) {
            douziBattleInfo = {}
            await sendMsg(4865,
                {}
            ...
            writeLog(`今天的bossType： ` + douziBattleInfo.bossType)
            writeLog(`本日最大的bossId: `+ maxLevelId)
            if (douziBattleInfo.times == 0) {
                break
            }
            if (date == 0 || date == 1 || date == 3 || date == 5) {
                if (hour < 5) {
                    await sendMsg(4868,
                            {
                                hirePetList:['1703779698'],
                                attachBattleSkill:[],
                                levelId:maxLevelId,
                            }
                    )
    ...
    async function battleF() {
        ...
        while (1) {
            ...
            while (1) {
                if (floorContent != "") {
                    nowfloor = floorContent.fragments.split("_")[1]
                    if (doneKing == 1 && (floorContent.fragments.split("_")[1] == 0)) {
                        writeLog("顺利击毙教皇！，脚本结束")
                        return
                    }
                    if ((floorContent.fragments.split("_")[1] == "21") || (floorContent.fragments.split("_")[1] == "15")){
    ...
    async function wangzhe() {
        ...
        while (1) {
            ...
            while (1) {
                ...
                try {
                    if (roundContent.result.roundNum >= 50) {
                        writeLog("到达十回合 发起投降")
                        //! !!发起投降
                        try {
                            document.querySelector("iframe").contentWindow.MFC.battleManager.surrenderPvp({
    ...
    function fixMsg() {
        ...
        socketPrototype.createMsg = function (t, e) {
            ...
            try {
                ...
                if (cmd == 1056) {
                    let processQueue = JSON.parse(raw).result.processQueue
                    processQueue.forEach(item => {
                        if (item.opInfo.attacker != 14619404) {
                            bossDamage = item.opInfo.damage
                        }
                    })
                }
                if (cmd == 1184) {
                ...
                if (cmd == 1176) {
                    var reword = JSON.parse(raw);
                    lukeReword.morning = reword.reward[322]
                    lukeReword.afternoon = reword.reward[323]
                    lukeReword.night = reword.reward[324]
                    lukeReword.iffind = true
                }
                if (cmd == 4865) {
                    douziBattleInfo = JSON.parse(raw);
                }
                if (cmd == 10611) {
                    let bossInfo = JSON.parse(raw).bossInfo
                    bossInfo.forEach(item => {
                        bossCard = item.activityCard
                    })
                }
                if (cmd == 303) {
                    run = true
                ...
                if (cmd != 279 && cmd != 2324 && cmd != 2322 && cmd != 4354 && cmd != 1044 && cmd != 1045 && cmd != 1056 && cmd != 1057 && cmd != 1109 && cmd != 4353 && cmd != 263 && cmd != 288) {
                    writeLog("cmd: " + cmd)
                }
            } catch (e) { }
    ...
    function checkMengDuoPet(skillList) {
        for (let index = 0; index < skillList.length; index++) {
            if (skillList[index].id == 10630) {
                skillStarId = "10630"
                return true
            }
        }
    ...
    function checkMoYanPet(skillList) {
        for (let index = 0; index < skillList.length; index++) {
    ...
    async function starTrek() {
        ...
        isOk = checkMoYanPet(StartJson.petList[0].useSkills)
        if (!isOk) {
            writeLog(`魔焰没带绝命`)
            return
        }
        isOk = checkMengDuoPet(StartJson.petList[1].useSkills)
        if (!isOk) {
            writeLog(`蒙多没带光闪击`)
            return
        }
        ...
        while (1) {
            ...
            while(1) {
                if (StartJson.levelId != undefined) {
                    break
                }
                await wait(50)
            }
    ...
    async function fightan() {
        ...
        for (let g = 1; g <= 2; g++) {
            if (g == 2) {
                try {
                    var ziyuan1 = document.querySelector("iframe").contentWindow.ItemManager.getInstance().getItemNumById(100314)
                } catch (e) {
                    ziyuan1 = ItemManager.getInstance().getItemNumById(100314)
                }
            ...
            if (g == 1) {
                try {
                    var ziyuan2 = document.querySelector("iframe").contentWindow.ItemManager.getInstance().getItemNumById(100194)
                } catch (e) {
                    ziyuan2 = ItemManager.getInstance().getItemNumById(100194)
                }
