...
(function() {
  ...
  const globalReplacements = [
    ...
    { find: /Lola Lemon/g, replace: 'Yuzu Bunny 月' },
  ...
  function updateTabTitle(replacements) {
    ...
    for (const r of replacements) {
      newTitle = newTitle.replace(r.find, r.replace);
    }
  ...
  function main() {
    ...
    // Update tab title with global replacements
    updateTabTitle(globalReplacements);
    ...
    if (isSpecificPage) {
      ...
      if (allSpecificReplacements.length > 0) {
        ...
        // Update tab title with specific replacements
        updateTabTitle(allSpecificReplacements);
      }
    }
