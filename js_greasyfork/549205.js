...
// @downloadURL https://update.greasyfork.org/scripts/549205/BLACK%20RUSSIA%20Stavropol%20%7C%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%20%D1%84%D0%BE%D1%80%D1%83%D0%BC%D0%B0.user.js
// @updateURL https://update.greasyfork.org/scripts/549205/BLACK%20RUSSIA%20Stavropol%20%7C%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%20%D1%84%D0%BE%D1%80%D1%83%D0%BC%D0%B0.meta.js
// ==/UserScript==
...
function pasteContent(id, data = {}, send = false) {
  try {
    console.log('[KR] pasteContent start', id, send);
    const template = Handlebars.compile(buttons[id].content);
    const content = template(data);
    console.log('[KR] compiled content len', content.length);

    // try multiple insertion targets
    var success = {
      fr_div: false,
      fr_p: false,
      textarea_name_message: false,
      any_textarea: false
    };

    // clear placeholder if present
    try { $('span.fr-placeholder').empty(); } catch(e){}

    // 1) div.fr-element.fr-view (Froala)
    var editorDiv = $('div.fr-element.fr-view');
    if (editorDiv && editorDiv.length) {
      ...
      editorDiv.trigger('input').trigger('change');
      success.fr_div = true;
      console.log('[KR] inserted into div.fr-element.fr-view');
    }

    // 2) div.fr-element.fr-view p
    var editorP = $('div.fr-element.fr-view p');
    if (editorP && editorP.length) {
      editorP.html(content);
      editorP.each(function() {
        this.dispatchEvent(new Event('input', { bubbles: true }));
        this.dispatchEvent(new Event('change', { bubbles: true }));
      });
      editorP.trigger('input').trigger('change');
      success.fr_p = true;
      console.log('[KR] inserted into div.fr-element.fr-view p');
    }
    ...
    if (ta && ta.length) {
      ...
      success.textarea_name_message = true;
      console.log('[KR] set textarea[name=\"message\"]');
    }

    // 4) any visible textarea (as fallback)
    var anyTa = $('textarea:visible').first();
    if ((!success.textarea_name_message) && anyTa && anyTa.length) {
      anyTa.val(content);
      anyTa.each(function() {
        this.dispatchEvent(new Event('input', { bubbles: true }));
        this.dispatchEvent(new Event('change', { bubbles: true }));
      });
      anyTa.trigger('input').trigger('change');
      success.any_textarea = true;
      console.log('[KR] set first visible textarea as fallback');
    }

    console.log('[KR] insertion success', success);

    // close overlay if exists
    try { $('a.overlay-titleCloser').trigger('click'); } catch(e){}

    // Small delay before sending to let editor register the change
    if (send === true) {
      setTimeout(function() {
        // Attempt several ways to click the reply/send button
        var clicked = false;
        var selectors = [
          'form.messageEditorForm button[type=submit]:visible',
          '.messageEditorForm button[type=submit]:visible',
          'button.button--icon--reply.rippleButton:visible',
          '.button--icon.button--icon--reply.rippleButton:visible',
          '.button--icon--reply:visible',
        ...
        for (var i=0;i<selectors.length;i++) {
          try {
            ...
            if (el && el.length) {
              ...
              clicked = true;
              console.log('[KR] clicked send via selector', selectors[i]);
              break;
            }
          } catch(e) { console.warn('[KR] click attempt failed', selectors[i], e); }
        }
        if (!clicked) {
          // fallback: try to trigger original selector
          try {
            $('.button--icon.button--icon--reply.rippleButton').trigger('click');
            console.log('[KR] fallback clicked .button--icon.button--icon--reply.rippleButton');
          } catch(e) { console.warn('[KR] final fallback failed', e); }
        }

        // also call editThreadData to set prefix/status as before
        try { editThreadData(buttons[id].prefix, buttons[id].status); } catch(e) {}
      }, 120);
    }

  } catch (err) {
    console.error('[KR] pasteContent error', err);
  }
}
