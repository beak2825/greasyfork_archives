...
// @author         Акрополь
// @version        0.16
// @description    Передача артов толпой
// @encoding       utf-8
...
// @downloadURL https://update.greasyfork.org/scripts/406078/hwmp_post_art.user.js
// @updateURL https://update.greasyfork.org/scripts/406078/hwmp_post_art.meta.js
// ==/UserScript==

// (c) 2008-2009, xo4yxa
// (c) 2020, Акрополь  ( https://www.heroeswm.ru/pl_info.php?id=962663 )
...
var kukla_tbl = document.querySelectorAll('table .wblight table')[0];
if( kukla_tbl )
{
	o1 = kukla_tbl;
	...
	td.setAttribute( 'colspan' , 2 ) ;
	td.setAttribute( 'align' , 'center' ) ;
	td.innerHTML = '<table><tr><td>\u0418\u043c\u044f</td><td>\u0414\u043d\u0438</td><td><a href="javascript:void(0);" id="day_1_battle">\u0411\u043e\u0438</a></td><td>\u0420\u0435\u043c.</td><td>1\u0437</td><td>\u04261</td><td>\u04262</td><td>\u04263</td><td></td></tr><tr><td><input id="trans_nick" value="" size="15"></td><td><input id="trans_time" value=0 size="1"></td><td><input id="trans_count" value=0 size="1"></td><td><input type=checkbox id="trans_rep"></td><td><input type="radio" name="sum" checked="true" value="0" id="sum0"></td><td><input type="radio" name="sum" value="1" id="sum1"></td><td><input type="radio" name="sum" value="2" id="sum2"></td><td><input type="radio" name="sum" value="3" id="sum3"></td></tr><tr><td id="place4return"></td><td colspan="7" align="right"><input type="button" id="trans_a" value="\u041f\u0435\u0440\u0435\u0434\u0430\u0442\u044c"></td></tr></table>' ;
	tr.appendChild( td ) ;
	o1.appendChild( tr ) ;
...
//+ Цена за арт
	if( el.href.indexOf( 'art_transfer.php' ) > -1 )
	{
		...
		if( ( price_art_id = GM_getValue( "hwm_takeoffon_price_"+art_id ) ) && price_art_id.indexOf( '0;0;0;' ) < 0 )
		{
			price_a.style.fontWeight = "bold"
			price_a.style.color = "#006400"
		} else if( ( price_art_name = GM_getValue( "hwm_takeoffon_price_"+name ) ) && price_art_name.indexOf( '0;0;0;' ) < 0 )
		{
			price_a.style.fontWeight = "bold"
    ...
    padding-left: 230px;
  	`;
		el.parentNode.parentNode.insertBefore( td , el.parentNode.parentNode.firstChild ) ;
...
// �� ������� ����
	...
	} else if( GM_getValue( "hwm_takeoffon_day1battle" ) )
	{
		var day_time = GM_getValue( "hwm_takeoffon_day1battle" ) * $('trans_count').value
	} else
	{
		var day_time = 0 ;
	}
	for( var i = 0; i < inputs.length; i++ )
	{
		...
		if( inp.type == "checkbox" && inp.name == "dress_id" && inp.checked )
		{
			...
			{
				if( ( price = GM_getValue( "hwm_takeoffon_price_"+art_id ) ) && price.indexOf( '0;0;0;' ) < 0 )
				{
					price_arr = price.split(';');
					sum = price_arr[gold]
				} else if( $("id_price_a"+art_id) && ( art_name = $("id_price_a"+art_id).getAttribute( "art_name" ) ) && ( price = GM_getValue( "hwm_takeoffon_price_"+art_name ) ) && price.indexOf( '0;0;0;' ) < 0 )
				{
					price_arr = price.split(';');
					sum = price_arr[gold]
				} else
			...
			({
				...
				data: 'id='+art_id+'&nick='+urlencode($('trans_nick').value)+'&gold='+( sum * ( $('trans_count').value > 0 ? $('trans_count').value : 1 ) )+'&wood=0&ore=0&mercury=0&sulphur=0&crystal=0&gem=0&sendtype=2&dtime='+day_time+'&bcount='+$('trans_count').value+( ( $('trans_rep') && $('trans_rep').checked ) ? '&rep=on' : '' )+'&art_id=&sign='+sign ,
				onload:function(res)
				{
					inp.type = "radio" ;
					inp.selected = true ;
...
function daybattle_form()
{
	...
	if( !bg ) 
	{
		...
		bg.addEventListener( "click", price_form_close , false );
	...
	var day1battle = GM_getValue( "hwm_takeoffon_day1battle" ) ;
	if( !day1battle ) day1battle = 0

	bgc.innerHTML = '<div style="border:1px solid #abc;padding:5px;margin:2px;"><center><table><tr><td colspan="2"><b>\u0412\u0440\u0435\u043c\u044f \u043d\u0430 1 \u0431\u043e\u0439 \u0432 \u0434\u043d\u044f\u0445 (1 \u0447\u0430\u0441 ~ 0,042)</b></td></tr><tr><td></td><td><input id="day_cnt" value="'+day1battle+'"></td></tr><tr><td colspan="2" align="right"><input type="button" id="form_o" value="\u0423\u0441\u0442\u0430\u043d\u043e\u0432\u0438\u0442\u044c"></td></tr></table></center></div>' ;
...
{
	GM_setValue( "hwm_takeoffon_day1battle" , $('day_cnt').value ) ;
	price_form_close() ;
}
...
function price_form()
{
	...
	var price_c1 = price_c2 = price_c3 = 0 ;
	var st_i = st_n = '' ;
	if( ( price = GM_getValue( "hwm_takeoffon_price_"+art_id ) ) && price.indexOf( '0;0;0;' ) < 0 )
	{
	...
	} else if( ( price = GM_getValue( "hwm_takeoffon_price_"+name ) ) && price.indexOf( '0;0;0;' ) < 0 )
	{
	...
	bgc.innerHTML = '<div style="border:1px solid #abc;padding:5px;margin:2px;"><center><table><tr><td colspan="2"><b id="art_title"'+st_i+'>'+title+'</b> <i id="art_name"'+st_n+'>('+name+')'+'</i></td></tr><tr><td>[\u04261] 1 \u0431\u043e\u0439</td><td><input id="art_c1" value="'+price_c1+'"></td></tr><tr><td>[\u04262] \u041e\u043f\u0442</td><td><input id="art_c2" value="'+price_c2+'"></td></tr><tr><td>[\u04263] \u041e\u0441\u043e\u0431\u0430\u044f&nbsp;</td><td><input id="art_c3" value="'+price_c3+'"></td></tr><tr><td colspan="2" align="center"><input type="button" art_id="'+art_id+'" id="form_oi" value="\u0418\u043d\u0434\u0438\u0432\u0438\u0434\u0443\u0430\u043b\u044c\u043d\u043e"> <input type="button" art_name="'+name+'" id="form_on" value="\u0412\u0441\u0435\u043c \u0442\u0430\u043a\u0438\u043c"></td></tr></table></center></div>' ;
...
function price_set_id()
{
	...
	price = $('art_title').innerHTML+';'+$('art_c1').value+';'+$('art_c2').value+';'+$('art_c3').value+';';
	...
	if( price.indexOf( ';0;0;0;' ) < 0 )
	{
		price_a_obj.style.fontWeight = "bold"
		price_a_obj.style.color = "#006400"
	} else
	{
		if( ( this_name = price_a_obj.getAttribute( 'art_name' ) ) && ( this_price = GM_getValue( "hwm_takeoffon_price_"+this_name ) ) && this_price.indexOf( '0;0;0;' ) < 0 )
		{
			price_a_obj.style.fontWeight = "bold"
...
function price_set_name()
{
	...
	if( price.indexOf( '0;0;0;' ) < 0 )
	{
		for(var i=0; i < need_a.snapshotLength; i++)
		{
			...
			if( ( this_id = this_a.getAttribute( 'art_id' ) ) && ( this_price =  GM_getValue( "hwm_takeoffon_price_"+this_id ) ) && this_price.indexOf( '0;0;0;' ) < 0 )
			{
