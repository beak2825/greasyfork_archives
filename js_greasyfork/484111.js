// ==UserScript==
// @name            赛尔号启航Pc修改版黄金天马透视
// @name:en         赛尔号启航Pc修改版黄金天马透视
// @namespace       赛尔号启航Pc修改版黄金天马透视
// @version         2.1.3
// @description     赛尔号启航Pc.
...
// @downloadURL https://update.greasyfork.org/scripts/484111/%E8%B5%9B%E5%B0%94%E5%8F%B7%E5%90%AF%E8%88%AAPc%E4%BF%AE%E6%94%B9%E7%89%88%E9%BB%84%E9%87%91%E5%A4%A9%E9%A9%AC%E9%80%8F%E8%A7%86.user.js
// @updateURL https://update.greasyfork.org/scripts/484111/%E8%B5%9B%E5%B0%94%E5%8F%B7%E5%90%AF%E8%88%AAPc%E4%BF%AE%E6%94%B9%E7%89%88%E9%BB%84%E9%87%91%E5%A4%A9%E9%A9%AC%E9%80%8F%E8%A7%86.meta.js
// ==/UserScript==
(function () {
  'use strict';
  //去除左侧小贴士
  ...
  if (topWindow == window) {
    ...
    logTextArea.style.height = "170px";
  ...
  function fixMsg() {
    ...
    socketPrototype.createMsg = function (t, e) {
      ...
      if(cmd == 10668){
        ...
        let s="===\n";
        for (let i=0;i<value.openList.length;i++){
          let no=value.openList[i].no
          if (value.openList[i].value.length==0){
            s+="next:("+parseInt(no/8+1)+","+((no%8)+1)+")\n"
          }
         // if (value.openList[i].value[0]==13886){
         //   s+="道具:("+parseInt(no/8+1)+","+((no%8)+1)+")\n"
         // }
          if (value.openList[i].value[0]==13883){
            s+="钥匙:("+parseInt(no/8+1)+","+((no%8)+1)+")\n"
          }
          if (value.openList[i].value[0]==13884){
            s+="眼睛:("+parseInt(no/8+1)+","+((no%8)+1)+")\n"
          }
          if (value.openList[i].value[0]==13885){
            s+="羽毛:("+parseInt(no/8+1)+","+((no%8)+1)+")\n"
          }
        }
        for (let i=0;i<value.enAblePointList.length;i++){
          let no=value.enAblePointList[i].no
          if (value.enAblePointList[i].value.length==0){
            s+="next:("+parseInt(no/8+1)+","+((no%8)+1)+")\n"
          }
         // if (value.enAblePointList[i].value[0]==13886){
         //   s+="道具:("+parseInt(no/8+1)+","+((no%8)+1)+")\n"
         // }
          if (value.enAblePointList[i].value[0]==13883){
            s+="钥匙:("+parseInt(no/8+1)+","+((no%8)+1)+")\n"
          }
          if (value.enAblePointList[i].value[0]==13884){
            s+="眼睛:("+parseInt(no/8+1)+","+((no%8)+1)+")\n"
          }
          if (value.enAblePointList[i].value[0]==13885){
            s+="羽毛:("+parseInt(no/8+1)+","+((no%8)+1)+")\n"
          }
        }
