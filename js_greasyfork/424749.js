...
// ==UserScript==
// @name         Steam Bundle Sites Extension(F站提取key修复版本)
...
// @downloadURL https://update.greasyfork.org/scripts/424749/Steam%20Bundle%20Sites%20Extension%28F%E7%AB%99%E6%8F%90%E5%8F%96key%E4%BF%AE%E5%A4%8D%E7%89%88%E6%9C%AC%29.user.js
// @updateURL https://update.greasyfork.org/scripts/424749/Steam%20Bundle%20Sites%20Extension%28F%E7%AB%99%E6%8F%90%E5%8F%96key%E4%BF%AE%E5%A4%8D%E7%89%88%E6%9C%AC%29.meta.js
// ==/UserScript==
  ...
  fanatical() {
    ...
    const productHandler = /*#__PURE__*/function () {
      var _ref5 = _asyncToGenerator(function* (APIData) {
        if (Object.keys(APIData).length > 0) {
          ...
          const $priceExt = $(`
          <div class="SBSE-priceExt SBSE-priceExt--portrait" style="position: relative;">
            <div>
        ...
        `);
          ...
          if (location.href.includes('/game/') || location.href.includes('/dlc/')) {
            ...
            Object.keys(APIData.price).sort().forEach(currency => {
              ...
              if (has.call(starDeal, 'promo-price')) value = starDeal.promo-price[currency];
    ...
    const handlers = {
      extract() {
        ...
        $('.account-content .new-order-item').each((i, orderItem) => {
          ...
          if (Object.keys(APIData).length > 0) {
            ...
            const matchGame = data => {
              if (has.call(data, 'steam') && data.steam.id) {
                const $gameTitle = $node.find(`.new-order-item .game-name:contains(${data.name})`).filter((index, name) => data.name === name.textContent.trim());
                const $orderItem = $gameTitle.closest('.new-order-item');
                const d = {
                  title: data.name,
    ...
    new MutationObserver(mutations => {
      mutations.forEach(mutation => {
        Array.from(mutation.addedNodes).filter(x => x.nodeType === 1).forEach(node => {
          ...
          if ($node.is('div.order-item') || $node.children('div.order-bundle-items-container, div.new-order-item').length > 0) {
