// ==UserScript==
// @name         phw nè
// @description  Based on original script by ANuro#9818BC (modified & extended)
// @version      1.01
...
// @grant        none
// @namespace    https://greasyfork.org/users/1560904
// @license All Rights Reserved
// @downloadURL https://update.greasyfork.org/scripts/562850/phw%20n%C3%A8.user.js
// @updateURL https://update.greasyfork.org/scripts/562850/phw%20n%C3%A8.meta.js
// ==/UserScript==
        ...
        if (user.alive) {
            ...
            if (enemy && !trap && user.alive) {
                ...
                if (Toggle.autoPush && distance <= 250) {
                    let trap = Entity.find(c => c && getDistance(c, enemy) <= 60 && c.type == 6 && c.id !== enemy.id);
                    ...
                    if (trap) {
                        let spikes = Entity.filter(c => c && [2, 7, 17].includes(c.type) && c.id !== enemy.id && getDistance(c, trap) <= 130);
                        if (wsPushing && !spikes.length) {
                            wsPushing = false;
                            Sploop.walk('stop');
                        }
                        if (spikes.length) {
                            let spike = spikes.sort((a, b) => getDistance(a, trap) - getDistance(b, trap))[0];
                            ...
                            angle = () => {
                                ...
                                if (distance > 100) {
                                    ...
                                    let angleDifference = Math.abs(getAngle(spike, position) - getAngle(spike, user))
                                    let message = `diffence [${angleDifference / (Math.PI / 180)}]`

                                    //Sploop.chat(`Smart-Push initiated... ${(angleDifference / (Math.PI / 180)).toFixed(3)}`)
                                    // Sploop.chat(message);
...
const __TOGGLERS__ = [{
    ...
    name: "Optimized Heal",
    desc: "tối ưu hóa hồi máu",
    id: "optheal",
    active: false,
    last: false
}, {
    name: "AutoPush",
    desc: "đẩy địch vào gai",
    id: "autopush",
    active: true,
    last: false
}, {
    name: "AutoPlace",
    desc: "tự động đặt gai , bẫy",
    id: "autoplace",
    active: true,
    last: false
}, {
    name: "AutoSync",
    desc: "Tấn công tối ưu & Tự động chuyển đổi mũ",
    id: "autosync",
    active: true,
    last: false
}, {
    name: "AutoBreak",
    desc: "tự động phá bẫy",
    id: "autobreak",
    active: true,
    last: false
}, {
...
<div class="centerMenu">
  <div class="pop-box popup-fade-in" id="script-changelog" style="display: flex; flex-direction: column;">
    <div class="pop-top">
...
#script-changelog .pop-list-content {
  flex: 1;
  overflow-y: auto;
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    cEl("#autoheal",  Toggle.autoHeal,  "AutoHeal")
cEl("#autopush",  Toggle.autoPush,  "AutoPush")
cEl("#autosync",  Toggle.autoSync,  "AutoSync")
cEl("#autoplace", Toggle.autoPlace, "AutoPlace")
cEl("#autobreak", Toggle.autoBreak, "AutoBreak")
cEl("#autochat",  Toggle.autoChat,  "AutoChat")
cEl("#tracers",   Toggle.tracers,   "Tracers")
cEl("#autorespawn", Toggle.autoRespawn, "AutoRespawn")
cEl("#optheal",   Toggle.optHeal,   "OptimizedAutoheal")
cEl("#ui",        Toggle.UI,        "UI")
    $("#hack").click(() => {
