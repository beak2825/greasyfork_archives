// ==UserScript==
// @name           Mission Queue
// @namespace      http://bitbucket/odahviing
// @description    Put "Mission Commands" in the fleet notes.
// @include        http://*.war-facts.com/fleet.php*
// @grant          GM_xmlhttpRequest
// @version        2.0
// @downloadURL https://update.greasyfork.org/scripts/13679/Mission%20Queue.user.js
// @updateURL https://update.greasyfork.org/scripts/13679/Mission%20Queue.meta.js
// ==/UserScript==

// Version 1.0, 1.1, 1.2 - Carabas Amazing Script
// Version 2.0 - Cohenman - Basic support in the new user interface, many bugs still
...
a) let you have other notes
b) give a place for the script to save the current "step".
...
The second, is to enter step:# into the current mission text box and press the button.
...
function updateMissionStatus() {
		...
		if (current_mission.indexOf('done') == 0) {
			b.value = "Restart";
		} else if (current_mission.indexOf('mission') == 0) {
			b.value = "Launch";
		} else if (current_mission.indexOf('trade') == 0) {
			b.value = "Trade";
		}
		...
		for (var i = 0, len = marray.length; i < len; i++) {
			...
			if (type == 'mission') {
				...
				case 'global':
					...
					break;
				case 'local':
					url += '&tpos=local&rawcoords=' + encodeURIComponent(m[1]);
					break;
				default:
					break;
				}
			...
			if (m[0] == 'trade') {
				...
				case 'sellall':
					break;
				default:
					s.value = "Syntax Error!";
					return;
				}
				subtype = m[1];
			}

			if ((outpost && ship) || colony) {
				type = 'trade';
			}
			...
			if (type != 'trade') {
				...
				case 'outpost':
					outpost = m[1];
					url += 'outposttrade.php?fleet=' + fleetid + '&outpost=' + m[1];
					outpost = 1;
					if (type == 'refuel') {
						execMission('trade', url + '&refuel=1');
						return;
					}
					break;
				case 'ship':
					ship = m[1];
					/*url += '&shipselect='+m[1]; ship=1;*/
					break;
				case 'colony':
					colony = m[1];
					break;
			...
			if (((outpost && ship) || colony) && subtype && subtype != 'load' && subtype != 'unload') {
				if (outpost) {
					url += 'outposttrade.php?fleet=' + fleetid + '&outpost=' + outpost + '&shipselect=' + ship;
				} else if (colony) {
					url += 'cargo_fleet.php?posted=1&fleet=' + fleetid + '&colony=' + colony;
				}

				if (mission.indexOf('all') != -1 || mission.indexOf('%') != -1) {
					getResources(subtype, url, mission);
					return;
				}
			...
			if (type == 'trade') {
				if (subtype == 'load' || subtype == 'unload') {
					...
					if (outpost) {
						execMission(type, url + '&cargo_type=' + m[0] + '&lu' + m[0] + '=' + subtype + '&' + m[0] + '=' + m[1]);
					} else if (colony) {
						url += '&lu' + m[0] + '=' + subtype + '&' + m[0] + '=' + m[1];
					}
				}
			}
		}
				...
				onload : function (responseDetails) {
					...
					if (res) {
						...
						for (var i = 0, len = res.length; i < len; i++) {
							...
							if (buysell == 'buyall' || buysell == 'sellall' || resources[thisres[1]] == 'all') {
								resources[thisres[1]] = thisres[2];
							}
							if (resources[thisres[1]] < 1 && resources[thisres[1]] > 0) {
								resources[thisres[1]] = Math.round(resources[thisres[1]] * thisres[2]);
							}
						}
						...
						for (i in resources) {
							if (resources[i] > 0) {
								res.push(i + ':' + resources[i]);
							}
						}

						if (res == '') {
							s.value = 'The station seems to be empty!   ';
							mnum++;
							updateMission();
							saveMission(mnum);
							return;
						...
						window.missionDo(mission);

					} else {
						s.value = 'Your cargo holds are empty!   ';
						mnum++;
						updateMission();
						saveMission(mnum);
						return
