...
// @version      0.93
// @description  该脚本用于翻译各类常用社交网站为中文，不会经过中间服务器，添加了bluesky的翻譯，現已可以翻譯網站https://bsky.app/（原作者HolynnChen）
// @author       mms (Origionally made by HolynnChen)
// @match        *://*.twitter.com/*
// @match        *://*.x.com/*
// @match        *://*.bsky.app/*
// @match        *://*.youtube.com/*
...
// @downloadURL https://update.greasyfork.org/scripts/513087/%E7%BF%BB%E8%AF%91%E6%9C%BA.user.js
// @updateURL https://update.greasyfork.org/scripts/513087/%E7%BF%BB%E8%AF%91%E6%9C%BA.meta.js
// ==/UserScript==
...
const rules = [
    ...
    },
    {
           ...
           name: 'bluesky通用',
        matcher: /https:\/\/([a-zA-Z.]*?.|)bsky\.app/,
        options: [
            {
                name: "帖子",
                selector: baseSelector('div[dir="auto"][data-word-wrap]'),
                textGetter: baseTextGetter,
                textSetter: options => {
                    options.element.style = options.element.style.cssText.replace(/-webkit-line-clamp.*?;/, '');
                    baseTextSetter(options).style.display = 'flex';
            ...
            {
                name: "背景信息",
                selector: baseSelector('background-info-selector'), // Adjust the selector to match Bluesky background info
                textGetter: baseTextGetter,
                textSetter: options => {
                    options.element.style = options.element.style.cssText.replace(/-webkit-line-clamp.*?;/, '');
                    baseTextSetter(options).style.display = 'flex'
