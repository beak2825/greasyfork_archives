...
// @downloadURL https://update.greasyfork.org/scripts/437970/mzblueplugin.user.js
// @updateURL https://update.greasyfork.org/scripts/437970/mzblueplugin.meta.js
// ==/UserScript==
...
var vanGmMz = {
    ...
    setSrc: function (transfer, img, skill, maxed, skillBallDay, pid, k) {
        ...
        let id = 'boll_' + k;
        let skill_bool = "" + skill;
        let maxed_bool = "" + maxed;

        let strdiv = "<div id=" + id + " skill_bool=" + skill_bool + " maxed_bool=" + maxed_bool + " class='skill' style='font-size:0;padding: 0 0 0 4px;'>";
    ...
    //显示到队的天数
    displayArriveDays: function (pid, arrivalTime, pdom) {//zhangxc add
        $("#GM_arrival_" + pid).remove();
        let now = (new Date()).valueOf();
        let days = Math.floor((now - arrivalTime) / (24 * 60 * 60 * 1000))

        let week = (new Date(arrivalTime)).getDay();
        if (week === 6 || week === 2) {
        ...
        let arrival = `<span id='GM_arrival_${pid}' style="color: ${dayColor}">[到队:${days}天]</span>`;
        arrival = $(arrival)[0];
        let d = pdom.find(`span#player_id_${pid}.floatRight`);
        pdom.find(`span#player_id_${pid}.floatRight`).before(arrival);
    },
    drawPlayerByTrainingGraphs: function (pid, data, pdom) {
        ...
        vanGmMz.displayArriveDays(pid, (plotBands === null) ? series[0].data[0].x : plotBands[0].to, pdom);
        ...
        vple.ajax(
            ...
            function (data) {
                ...
                if (srdivs.length == 3) {
                    ...
                    const array = ['速度', '耐力', '意识', '传球', '射门', '头球', '守门', '控球', '抢断', '传中', '定位'];
                    let scout = HS + "," + array.indexOf(HArr[0]) + "," + array.indexOf(HArr[1]) + "," + LS + "," + array.indexOf(LArr[0]) + "," + array.indexOf(LArr[1]);

                    let nsavgstat = "<span id='GM_scout_" + pid + "' scout=" + scout + ">[H" + HS + " " + HArr[0] + "," + HArr[1] + "] [L" + LS + " " + LArr[0] + "," + LArr[1] + "] S" + SS;
                    nsavgstat += "</span>";
    ...
    initgw: function () {
        ...
        if (tmpTacConf && tmpTacConf.trim() != "") {
            ...
            var json = JSON.stringify(vanGmMz.tacCof);
            // console.log(json)
        }
        ...
        document.onkeydown = function () {
            if (event.altKey) {
                ...
                else if (window.event.keyCode == 80) {
                    //alt + S
                    vanGmMz.gw_start(10);
                }
            }
    ...
    gw_start: function (GraphsType) {
        ...
        if (GraphsType == 10) {
            ...
            $("#thePlayers_0").style.backgroundColor = 0xFFFFFF;

            // var pid = $("#thePlayers_0").find(".player_id_span").text;
            // var scout = $("#thePlayers_0").find(".GM_scout_" + pid).text;
            // if (scout.indexOf('H4') >= 0) {
            //     $("#thePlayers_0").backgroundColor = 0xFFFFFF;
            // }
        }
...
';
    ...
    PLoadInternal: function () {
        ...
        if (ajaxSport && ajaxSport == "soccer") {
            ...
            if (location.href == "https://www.managerzone.com/?p=tactics") {
            ...
            else if (location.href == "https://www.managerzone.com/?p=event") {
                console.log('### event ###')
                ...
                setInterval(function () {
                    var claim_button = document.querySelector('a#claim')
                    claim_button.click()
                    //var xpathResult = document.evaluate('//*[@id="next-reset-clock"]', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
                    //var timeElement = xpathResult.singleNodeValue;
                    var date = new Date();
                    console.log(date)
                }, 5 * 1000);
            }
    ...
    DisplayInfo: function (pid, option, index) {
        PredictValue.parseTwoSeason(index, (data) => {

            let predictionStr = data.prediction.toString().split("").reverse().join("").match(/.{1,3}/g).join(" ").split("").reverse().join("");
            let predictionStr2 = data.prediction2.toString().split("").reverse().join("").match(/.{1,3}/g).join(" ").split("").reverse().join("");

            this.GetPlayerData(pid, option, (data, option) => {
                // console.log(data);
                // console.log(option);
                let ret = '';
                if (data.length <= 0) {
                    ret = `<p><font size="4">${pid}<font></p>
                        <p>无数据！</p>`;
                } else {
                    let item = data[0];

                    item.shots = item.shotsOnGoal + item.shotsWide;
                    item.passes = item.goodPasses + item.interceptedPasses;
                    item.tackles = item.tacklesWon + item.tacklesLost;

                    var tRate = (item.tackles === 0) ? 0 : (item.tacklesWon * 100 / item.tackles).toFixed(2);
                    var pRate = (item.passes === 0) ? 0 : (item.goodPasses * 100 / item.passes).toFixed(2);
                    var sogRate = (item.shots === 0) ? 0 : (item.shotsOnGoal * 100 / item.shots).toFixed(2);
                    var sRate = (item.opposingShotOnGoal === 0) ? 0 : (item.saves * 100 / item.opposingShotOnGoal).toFixed(2);
                    var gRate = (item.shots === 0) ? 0 : (item.goals * 100 / item.shots).toFixed(2);
                    var ptRate = (item.playingtime === 0) ? 0 : (item.playingtime / (item.starting + item.substitute)).toFixed(0);

                    item.shotOnGoalRate = sogRate + '%25';
                    item.passesRate = pRate + '%25';
                    item.tacklesRate = tRate + '%25';
                    item.savesRate = sRate + '%25';
                    item.goalsRate = gRate + '%25';

                    ret = `<p><font size="4">${item.name}(${item._id})<font></p>
                        <p><font size="4">${item.team}<font></p>
                        <p><font size="3">出场：${item.starting}, 替补：${item.substitute}, 场均：${ptRate}分钟，参赛级别：${item.level.toFixed(2)}<font></p>
                        <p><font size="3">射门：${item.shots}，射中：${item.shotsOnGoal}，效率：${item.shotOnGoalRate}<font></p>
                        <p><font size="3">进球：${item.goals}，效率：${item.goalsRate}<font></p>
                        <p><font size="3">传球：${item.passes}，成功：${item.goodPasses}，效率：${item.passesRate}<font></p>
                        <p><font size="3">抢断：${item.tackles}，成功：${item.tacklesWon}，效率：${item.tacklesRate}<font></p>
                        <p><font size="3">对方射正：${data[0].opposingShotOnGoal}，扑救：${data[0].saves}，效率：${item.savesRate}<font></p>
                        <p><font size="3">拦截：${item.interceptions}<font></p>
                        `;

                }
                ret += `<p><font size="3">当前估价：${predictionStr} MM<font></p>
                    <p><font size="3">下季估价：${predictionStr2} MM<font></p>`


                let css = document.createElement('style');
                css.type = 'text/css';
                css.innerHTML = ".clearfix2{display:flex;}";
                document.getElementsByTagName('head')[0].appendChild(css);

                let content;
                if (jQuery.isEmptyObject(option) == true) {
                    content = `<div class='clearfix2'><div id='datachart' style='height: 450px; width: 400px'>${ret}</div>`;
                } else {
                    content = `<div class='clearfix2'><div id='ldchart' style='height: 450px; width: 400px'></div><div id='datachart' style='height: 450px; width: 400px'>${ret}</div>`;
                }

                showHelpLayer(content, vanGmMz.now_language.Data, true);
    ...
    GetPlayerData: function (playerId, option, callback) {
        // console.log('GetPlayerData:' + playerId);
        GM_xmlhttpRequest({
            method: "POST",
            url: "http://www.bluemz.cn:3000/api/player/querybyplayer",
            data: "{\"pid\": \"" + playerId + "\"}",
            headers: { "Content-Type": "application/json" },
            responseType: "json",
            onload: function (result) {
                let dxml = JSON.parse(result.responseText);
                //console.log('dxml:', dxml);
                if (dxml.success == true) {
                    //console.log('data:', dxml.data);
                    callback(dxml.data, option);
                    //alert(vanGmMz.now_language.CopyXmlMsg);
                } else {
                    //alert(vanGmMz.now_language.CopyXmlMsgError);
                }
    ...
    showScore: function (pid, pdom) {
        ...
        if (tacPI == undefined) {
            ...
            // console.log('###### showScore.tacPI', tacPI)
        ...
        if (tacPI.tacPlayer.InitByTds(pdom.find("td.skillval"))) {
            ...
            this.DisplayInfo(pid, this.getRadarOption(tacPI, pdom), index);
        } else {
            this.DisplayInfo(pid, {}, index);
        }
    },
...
var PredictValue = {
    ...
    //         PredictValue.parseAll();
    //     }, PredictValue.interval);
    //     PredictValue.intervalIds.push(intervalId);
    // },
    // parseAll: function () {
    //     for (let i = 0; i < 100; i++) {
    //         var object = PredictValue.parseOne(i)
    //         if (object) {
    //             //console.log(object)
    //         } else {
    //             //console.log('null')
    ...
    //     }
    // },
    parseOne: function (index, callback) {
        ...
        PredictValue.predict(id, value, (prediction) => {
            callback({
                id,
                prediction
            })
    ...
    parseTwoSeason: function (index, callback) {
        var id = transferParse.getID(index);
        if (id === '') {
            return;
        }
        ...
        PredictValue.predict(id, value, (prediction) => {
            ...
            PredictValue.predict(id, value2, (prediction2) => {
                callback({
                    id,
                    prediction,
                    prediction2
    ...
    predict: function (id, value, callback) {
        // console.log(id, value)
        if (PredictValue.predictMap.has(id) == true) {
            PredictValue.addPrediction(id, PredictValue.predictMap.get(id), value)
        } else {
            ...
            GM_xmlhttpRequest({
                ...
                onload: function (response) {
                    if (response.status >= 200 && response.status < 400) {
                        ...
                        // PredictValue.predictMap.set(resp.id, resp.prediction)
                        // PredictValue.addPrediction(resp.id, resp.prediction, value)
                        callback(resp.prediction)
                    } else {
    ...
    addPrediction: function (pid, prediction, value) {
        if (prediction < 0) {
            prediction = 0;
        }
        $("#GM_prediction_" + pid).remove();
        let predictionStr = prediction.toString().split("").reverse().join("").match(/.{1,3}/g).join(" ").split("").reverse().join("");
        let predval = `<span id='GM_prediction_${pid}' style="color: clack" feature=${value}> [估价: ${predictionStr} MM] </span>`;
        predval = $(predval)[0];
