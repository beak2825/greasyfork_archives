...
function addToParty(){
    var ssToAdd=[];
$("#ss_Table tr.selected td:first-child").each(function()
{
log("enter success"+ $(this).text());
ssToAdd.push(($(this).text());
}
ssAddToAdventureParty($(this).text(),false);
});
}
//100++<<
*/
...
function addToParty(){
    ...
    ssAddToAdventureParty_2(ssToAdd,false);
}

function ssAddToAdventureParty_2(ssToAdd,a)
{
    if (ssToAdd.length===0) {
        return;
    }
    var   b=ssToAdd.pop();
    !1==a&&(closeAdventureDialog(),prepareAdvPartyTimeout());
    $.ajax({
        url:"/play/batch_add_sworn_sword?batch_type=1&ss_id="+b+"&nonce="+userContext.advparty_nonce,dataType:"JSON",
        success:function(a){
            showAdvPartyResponse(a);
            isWeb()&&chooseAvASwornSwordPopup(function(){ssAddToAdventureParty(itemCurrentSelection.id,!1)},"inventory",!0);
            ssAddToAdventureParty_2(ssToAdd,a);
        },
        error:function(a){
            ssToAdd.push(b);
            400==a.status?advPartyFail():409==a.status?setTimeout(function(){ssAddToAdventureParty_2(ssToAdd,!0)},5E3):spinTimeout()
        }
    });
}
//v101++<<
...
function getCommandPoints() {
    if(!userContext || !userContext.playerData || !userContext.playerData.stat)
        return 0;

    var commandPoints = userContext.playerData.stat.command - userContext.playerData.stat.current_command;

    log("Debugging get command points: command points: " + commandPoints + ", " +
    "continue then? " + (!isNaN(commandPoints) && commandPoints > 0));
...
log("pvpSendBatch pvpSendBatch pvpSendBatch");
    if (extender.options.sendAllAction === "none") {
        inform("This function is disabled. Please enable it from options.");
        return;
    }
    ...
    if(loops_new<0){
        inform("Nunber of loops done. <br>Action finished.");
        return;
    }
...
var questMan = (function ($) {
    var _this = {
        ...
        delay: 4E3,
        enabled: true,
        striveFor: ["old_ways", "truthful", "realm"],
        ...
        analyzeOptions: function(options){
            ...
            for(var i = 0; i < options.length; i ++){
                ...
                var loot = o.reward_loot || (o.change_reward != void 0 ? o.change_reward.reward_loot : "");
                var silver = o.reward_silver || (o.change_reward != void 0 ? o.change_reward.reward_silver : 0);
                var alignment = o.alignment || (o.change_reward != void 0 ? o.change_reward.alignment : "none");
                console.log("options analysis, loot:", loot, "silver:", silver, "alignment:", alignment);

                if(this.striveFor.contains(alignment)){
                    bestOption = i + 1;
                    break;
                }
            }

            console.log("Best option resolved: ", options[bestOption]);
            return bestOption;
        },
...
log(  pvpForm.target_id); 
log( userContext.currentActionLabel);  
log( fealtySymbol[pvpForm.target_faction_id]);   
log( userContext.playerData.character.faction_id);   
log( fealtySubRegions[b][pvpForm.sub_region_index].symbol);   
log( userContext.currentQuest.action_type[userContext.currentActionLabel].attacker_strength);   
log( userContext.currentQuest.action_type[userContext.currentActionLabel].defender_strength); 
...
function pvpLaunch_2(callback) {
    json = { pvp: {} };
log("pvpLaunch2 pvpLaunch2 pvpLaunch2") ;
    json.pvp.target_id = 25334790;
    json.pvp.sworn_sword_id = userContext.setSwornSword.id;
    json.pvp.pvp_action_symbol = "fight";
json.pvp.region_symbol = undefined;
    var Sub_region_index = Math.floor(3 * Math.random());
    var b = undefined ;
    void 0 == b && (b = userContext.playerData.character.faction_id);
    json.pvp.sub_region_symbol = fealtySubRegions[b][Sub_region_index].symbol;
    json.pvp.attack_value = 11111;
    json.pvp.defense_value = 11;
    false || (showSpinner(),
    $.ajax({
        type: "get",
        url: "/pvps/create",
