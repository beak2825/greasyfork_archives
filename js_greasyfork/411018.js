...
// @exclude            *://subscribestar.adult/age_confirmation_warning
// @exclude            *://subscribestar.adult/age_verification
...
// @downloadURL https://update.greasyfork.org/scripts/411018/ViewOnYP.user.js
// @updateURL https://update.greasyfork.org/scripts/411018/ViewOnYP.meta.js
// ==/UserScript==
...
(async () => {
  ...
  GM.registerMenuCommand("Populate cache", () => {
    for (const x of sites) {
      GM.xmlHttpRequest({
        ...
        onerror: () => {
          alert(`Failed to populate cache for ${x.name}.`);
        },
      });
    }
  });
    ...
    switch (host) {
      ...
      case "patreon":
        window.addEventListener("load", () => {
          ...
          if (textContent) {
            const { pageBootstrap } =
              JSON.parse(textContent).props.pageProps.bootstrapEnvelope;
            resolve(
              pageBootstrap.campaign.data.relationships.creator?.data.id ||
                pageBootstrap.post?.data.relationships.user.data.id,
            );
          } else {
            const x = structuredClone(unsafeWindow.__next_f);
            resolve(
              x
                .find((y) =>
                  y[1]?.includes("https://www.patreon.com/api/user/"),
                )[1]
                .match(/https:\/\/www\.patreon\.com\/api\/user\/(\d+)/)[1],
            );
          }
        });
        break;
  ...
  function show(site, host, user, status) {
        ...
        `<br><b>${name}:</b> ${
          status === 0
            ...
            : status === 1
              ...
              : status === 2
                ? `Failed to connect to platform, try again later.`
