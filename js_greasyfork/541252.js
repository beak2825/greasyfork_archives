// ==UserScript==
// @name        Milovana: Ultimate Hentai Lottery of Denial Helper
...
// @downloadURL https://update.greasyfork.org/scripts/541252/Milovana%3A%20Ultimate%20Hentai%20Lottery%20of%20Denial%20Helper.user.js
// @updateURL https://update.greasyfork.org/scripts/541252/Milovana%3A%20Ultimate%20Hentai%20Lottery%20of%20Denial%20Helper.meta.js
// ==/UserScript==
...
(function () {
    ...
    const TEASE_ID_1 = 37913;
    const TEASE_ID_2 = 41749;

    const TEASE_DATA = {
        [TEASE_ID_1]: {
            name: "Ultimate Hentai Lottery of Denial",
            start_page: 3,
            page_count: 111
        },
        [TEASE_ID_2]: {
            name: "Ultimate Hentai Lottery of Denial 2",
            start_page: 4,
            page_count: 203
        }
    };
    ...
    const DIFFICULTY_CONSTS = {
        ...
        Moderate: {
            ...
            ALT_LABELS: ["Chastity", "Standard"],
            EXCLUDE_DIFFICULTY: {[TEASE_ID_1]: ["Weak"]},
        },
        Hard: {
    ...
    // difficulty from the first version of Ultimate Hentai Lottery of Denial
    // const DIFFICULTY_CONSTS = {
    //     Weak: {
    //         CHASTITY_MAX_DEFAULT: 14,
    //         PAUSE_AFTER_EDGE: 60,
    ...
    const STORAGE = {
        ...
        COMMA_SEPARATED_ACTIVE_TAGS: `${STORAGE_PREFIX}active_tags`,
        RNG_HISTORY: `${STORAGE_PREFIX}rng_history`,
        SELECTED_TEASES: `${STORAGE_PREFIX}selected_teases`,
    };

    let selectedTeases = JSON.parse(localStorage.getItem(STORAGE.SELECTED_TEASES)) || [TEASE_ID_1, TEASE_ID_2];
    ...
    let _randomUrl = null;
    function getRandomURL(force_reload=false) {
        if (force_reload) {
            _randomUrl = null;
            selectedTeases = JSON.parse(localStorage.getItem(STORAGE.SELECTED_TEASES));
        }

        if (_randomUrl) return _randomUrl;
        ...
        for (const teaseId of selectedTeases) {
            const tease = TEASE_DATA[String(teaseId)];
            totalPages += tease.page_count;
        }

        let teaseId, page;
        do {
            let randomIndex = Math.floor(Math.random() * totalPages);
            for (const rolledTeaseId of selectedTeases) {
                const tease = TEASE_DATA[rolledTeaseId];
                if (randomIndex < tease.page_count) {
                    teaseId = parseInt(rolledTeaseId, 10);
                    page = tease.start_page + randomIndex;
                    break;
                }
                randomIndex -= tease.page_count;
            }
        } while (teaseId === currentTeaseId && page === currentPageNumber);

        _randomUrl = generateTeaseUrl(page, teaseId);

        // Preload
        fetch(_randomUrl, { method: "GET", mode: "no-cors" })
    ...
    function replacePageWithAnchors(node) {
        ...
        node.innerHTML = node.innerHTML.replace(/page (\d+)/g, (match, pageNumber) => {
            return `<a href="${generateTeaseUrl(pageNumber, teaseId)}">${match}</a>`;
        });
    }

    function generateTeaseUrl(pageNumber, teaseId) {
        let baseUrl;
    ...
    function getTeaseAndPageNumber() {
        return `${getCurrentTeaseId()}&p=${getCurrentPageNumber()}`
    }

    let cache_is_not_new_page = null;
    function isNotNewPage() {
        if (cache_is_not_new_page !== null) return cache_is_not_new_page;

        const teaseId = getCurrentTeaseId();
        const currentPageNumber = getCurrentPageNumber();
        const pageKey = getTeaseAndPageNumber();

        const teaseData = TEASE_DATA[teaseId];
        const startPage = teaseData.start_page;

        if (currentPageNumber < startPage) {
            cache_is_not_new_page = true;
        } else {
            const lastPage = localStorage.getItem(STORAGE.LAST_PAGE);
            cache_is_not_new_page = pageKey === lastPage;
        }

        localStorage.setItem(STORAGE.LAST_PAGE, pageKey);

        return cache_is_not_new_page;
    }
...
.modifier-history-list {
    list-style-type: none;
    padding-left: 0;
    margin: 0;
}

.modifier-history-list li {
    margin-bottom: 5px;
    font-size: 14px;
}
        `;
        ...
        function highlightDifficultyElements(node) {
            ...
            const exclusionMap = CONSTS.EXCLUDE_DIFFICULTY || {};
            const exclusionList = exclusionMap[teaseId] || [];
            ...
            node.innerHTML = node.innerHTML.replace(/\[(.*?)\]/g, (match, contentInside) => {
                ...
                if (matchedDifficulty) {
                    ...
                    const isExcluded = exclusionList.includes(matchedDifficulty);

                    if (isExcluded) {
                        return `<span style="color: ${inactiveColor}; font-weight: bold;" title="Excluded for this tease.">${match}</span>`;
                    }
        ...
        class RandomNumberManager {
            static rngHistory = []; // To store the history of generated numbers

            static init() {
                if (isNotNewPage()) {
                    const savedHistory = localStorage.getItem(STORAGE.RNG_HISTORY);
                    if (savedHistory) {
                        this.rngHistory = JSON.parse(savedHistory);
                    }
                } else {
                    this.resetHistory();
                }
            ...
            static showRandomDialog() {
                ...
                this.historyDisplay = document.createElement("p");
                this.historyDisplay.className = "modifier-history-display";
                ...
                dialog.appendChild(this.historyDisplay);
            ...
            static addToHistory(randomNumber) {
                this.rngHistory.push(randomNumber);
                this.updateHistoryDisplay();
                localStorage.setItem(STORAGE.RNG_HISTORY, JSON.stringify(this.rngHistory));
            }

            static updateHistoryDisplay() {
                this.historyDisplay.textContent = `History this page (${this.rngHistory.length}): ` + this.rngHistory.join(", ")
            }

            static resetHistory() {
                localStorage.removeItem(STORAGE.RNG_HISTORY);
                this.rngHistory = [];
            }
        }
            ...
            function addGoddessessPart() {
                ...
                function registerGoddess() {
                    if (!goddessPages[teaseId][currentPageNumber] && goddessNames.length > 0) {
                        ...
                        goddessCounter.textContent = Object.values(goddessPages).reduce(
                            (sum, teaseData) => sum + Object.keys(teaseData).length,
                            0
                        );
                        disableAllButtons();
        ...
        class ModifierManager {
            ...
            static countDownOnNewPage() {
                if (!isNotNewPage()) { // Check if it is not a page reload
                    const modifiers = JSON.parse(localStorage.getItem(STORAGE.MODIFIERS)) || [];
        ...
        class SessionActiveManager {
            ...
            static addSessionStartRow() {
                ...
                // Checkbox container for selecting sessions
                const checkboxContainer = document.createElement('div');
                checkboxContainer.style.margin = '10px 0';

                const selectedTeasesFromStorage = JSON.parse(localStorage.getItem(STORAGE.SELECTED_TEASES)) || Object.keys(TEASE_DATA).map(Number);

                Object.entries(TEASE_DATA).forEach(([teaseId, tease]) => {
                    ...
                    checkbox.value = teaseId;
                    checkbox.checked = selectedTeasesFromStorage.includes(Number(teaseId));

                    checkboxLabel.appendChild(checkbox);
                    checkboxLabel.appendChild(document.createTextNode(tease.name));
                ...
                startButton.addEventListener('click', () => {
                    this.startSession(dropdown, checkboxContainer);
                });
            ...
            static startSession(dropdown, checkboxContainer) {
                const selectedDifficulty = dropdown.value;

                // Gather checked tease IDs
                const checkedTeases = [];
                checkboxContainer.querySelectorAll('input[type=checkbox]').forEach(cb => {
                    if (cb.checked) checkedTeases.push(Number(cb.value));
                });

                if (checkedTeases.length === 0) {
                ...
                localStorage.setItem(STORAGE.SELECTED_TEASES, JSON.stringify(checkedTeases));
                localStorage.setItem(STORAGE.SESSION_START, new Date().toISOString());

                window.location.href = getRandomURL(true);
            }
