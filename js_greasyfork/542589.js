...
// @run-at      document-end
// @downloadURL https://update.greasyfork.org/scripts/542589/Qiuwei%20Chat%20Assistant.user.js
// @updateURL https://update.greasyfork.org/scripts/542589/Qiuwei%20Chat%20Assistant.meta.js
// ==/UserScript==
...
;(function() {
    ...
    const utils = {
        ...
        processLongText(text) {
            ...
            // 处理超长单词（如URL、长数字等）
            processed = processed.replace(/([^\s]{50,})/g, function(match) {
                // 对于超长单词，每50个字符插入一个换行符
                return match.match(/.{1,50}/g).join('\n');
            });
            
            // 处理长行，如果一行超过80个字符，在合适的位置换行
            processed = processed.replace(/(.{80,})/g, function(match) {
                // 尝试在标点符号处换行
                const breakPoints = [',', ';', ':', '!', '?', '.', '，', '；', '：', '！', '？', '。'];
                for (let i = 0; i < breakPoints.length; i++) {
                    const index = match.lastIndexOf(breakPoints[i], 60);
                    if (index > 0) {
                        return match.substring(0, index + 1) + '\n' + match.substring(index + 1);
                    }
                }
                // 如果没有合适的标点符号，在空格处换行
                const spaceIndex = match.lastIndexOf(' ', 60);
                if (spaceIndex > 0) {
                    return match.substring(0, spaceIndex) + '\n' + match.substring(spaceIndex + 1);
                }
                // 最后的选择：强制换行
                return match.substring(0, 60) + '\n' + match.substring(60);
            });
    ...
    function capturePageContent(forceCapture = false, retryCount = 0) {
        try {
            ...
            let isGroup = false;
            if (/群/.test(nickname) && /(\d+)$/.test(tail)) {
                isGroup = true;
            }
            ...
            } else if (lastRightMessageIndex >= 0) {
