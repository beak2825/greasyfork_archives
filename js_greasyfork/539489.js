// ==UserScript==
// @name         ShadConnect | Foreman Mode (James)
// @namespace    http://tampermonkey.net/
// @version      5.0
// @description  Foreman Mode: Sign-in (with multiple locations), Prestart Generator, PTA Helper, and a Preferences Panel
// @match        https://employee.shadconnect.com.au/*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/539489/ShadConnect%20%7C%20Foreman%20Mode%20%28James%29.user.js
// @updateURL https://update.greasyfork.org/scripts/539489/ShadConnect%20%7C%20Foreman%20Mode%20%28James%29.meta.js
// ==/UserScript==
...
// Injected Location Spoofer Submenu for Foreman Mode
const locationCSV = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQnB6xLIgt0uukKVO0y7KeysbVgNrzwix6doLoJniAQ_l82El9AXMEYh9UjlAsk8T6eKIQJSwdfxzN_/pub?output=csv';

async function loadCSVLocations() {
    const res = await fetch(locationCSV);
    const text = await res.text();
    const lines = text.trim().split('\n');
    const headers = lines[0].split(',');
    const locationIndex = headers.findIndex(h => h.trim().toLowerCase() === 'locations');
    if (locationIndex === -1) {
        console.error('[Foreman Mode] No "Locations" column found in CSV.');
        return [];
    }
    return lines.slice(1)
        .map((row, i) => {
            const cols = row.split(',');
            const loc = cols[locationIndex]?.trim();
            if (!loc) console.warn(`[Foreman Mode] Empty location on row ${i + 2}`);
            return loc;
        })
        .filter(Boolean);
}
...
async function geocodeLocation(address) {
    ...
    if (cache[address] && cache[address].lat && cache[address].lng) return cache[address];
    ...
    if (data[0]) {
        ...
        console.warn(`[Foreman Mode] Geocoding failed for: ${address}`);
    }
    return null;
}

function spoofCoords(lat, lng) {
    if (!lat || !lng) {
        console.error('[Foreman Mode] Invalid coordinates for spoofing:', lat, lng);
        return;
    }
    navigator.geolocation.getCurrentPosition = function(success) {
    ...
    navigator.geolocation.watchPosition = navigator.geolocation.getCurrentPosition;
    localStorage.setItem('siteLat', lat);
    localStorage.setItem('siteLon', lng);
    console.log(`[Foreman Mode] Spoofed to: ${lat}, ${lng}`);
}

async function injectSpoofSubmenu() {
    const signInBtn = document.getElementById('enterSite');
    if (!signInBtn) return;

    const subMenu = document.createElement('div');
    subMenu.style.margin = '-8px 0px 10px 0px';
    subMenu.style.fontSize = '12px';
    subMenu.style.padding = '10px';
    subMenu.style.maxHeight = '150px';
    subMenu.style.overflowY = 'auto';
    subMenu.style.backgroundColor = '#d3dc26';

    const addresses = await loadCSVLocations();
    for (const address of addresses) {
        ...
        item.style.fontFamily = 'Geomanist-Book,serif';
        ...
        item.setAttribute('data-lat', coords?.lat || '');
        item.setAttribute('data-lng', coords?.lng || '');

        item.addEventListener('click', () => {
            if (coords && coords.lat && coords.lng) {
                spoofCoords(coords.lat, coords.lng);
            } else {
                alert(`❌ Can't find: ${address}.  Try a different address or call Dale.`);
            }
        });
    ...
    signInBtn.insertAdjacentElement('afterend', subMenu);
}

// Call after dashboard is rendered
window.addEventListener('load', () => {
    setTimeout(injectSpoofSubmenu, 1500);
});
