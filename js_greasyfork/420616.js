// ==UserScript==
// @name         Auto_Award_Screenshot
// @namespace    https://blog.chrxw.com
// @version      2.5
// @description  Steam截图自动打赏
// @author       Chr_
...
// @downloadURL https://update.greasyfork.org/scripts/420616/Auto_Award_Screenshot.user.js
// @updateURL https://update.greasyfork.org/scripts/420616/Auto_Award_Screenshot.meta.js
// ==/UserScript==
...
function addPanel() {
    ...
    let panelArea = document.querySelector('#rightContents>.sidebar');
    let panel = genDiv('panel');
    panel.id = 'autoaward';
    let pTitle = genDiv('profile_count_link ellipsis');

    let pTips1 = genLabel('自动打赏截图 - V' + Version + ' - By Chr_');
    let pLink1 = genLink('打赏作者（点数）', 'https://steamcommunity.com/sharedfiles/filedetails/?id=2296172582', '感谢支持');
...
function autoAward() {
    ...
    function reviewAward() {
        let reward = document.querySelector('#ItemControls>span[onclick^="Published"]') || document.querySelector;
        if (reward) {
            reward.click();
            tries = 0;
            retry(waitLoad, 600);
...
function setGoal() {
    ...
    Vgoal = goal;
    Vleft = goal;
    Vname = document.querySelector('.creatorsBlock .friendBlockContent').firstChild.textContent.strip();
    Vpoint = Vpoint_now;
    saveConf();
    updateProcess();
...
function checkSelf() {
    return document.querySelector("#account_pulldown").textContent.strip() ==
        document.querySelector('.creatorsBlock .friendBlockContent').firstChild.textContent.strip();
}
// 判断是不是目标用户
function checkTarget() {
    return document.querySelector('.creatorsBlock .friendBlockContent').firstChild.textContent.strip() == Vname;
}
// 添加千分位
