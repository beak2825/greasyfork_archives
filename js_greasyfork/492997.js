...
// @version           2.2.19
// @updateDesc        观看付费视频
// @description       🔥观看付费视频，储存账号密码免输入，展示帖子是否有视频图片或付费(标题前缀)，复制播放链接，下载视频(日限)，去广告🔥
// @icon              https://vip.yotade.cc/static/haijiao.png
...
// @downloadURL https://update.greasyfork.org/scripts/492997/%E6%B5%B7%E8%A7%92%E7%A4%BE%E5%8C%BA%7C%E8%A7%82%E7%9C%8Bvip%E4%BB%98%E8%B4%B9%E8%A7%86%E9%A2%91.user.js
// @updateURL https://update.greasyfork.org/scripts/492997/%E6%B5%B7%E8%A7%92%E7%A4%BE%E5%8C%BA%7C%E8%A7%82%E7%9C%8Bvip%E4%BB%98%E8%B4%B9%E8%A7%86%E9%A2%91.meta.js
// ==/UserScript==
...
<div class="yota">
    ...
    <div class="yota-player-bottom" v-if="this.bottomVideoVisible">
        <video class="yota-video-bottom" ref="bottomVideo" controls>
            <source type="application/x-mpegURL" ref="bottomVideoSrc">
        </video>
        <button @click="closeBottomPlayer()">关闭</button>
    </div>
          ...
          `,
                ...
                data() {
                    ...
                    return {
                        ...
                        last_video_type: "",// 上一个解析成功的视频类型 防止重复解析同一个视频
                        ...
                        bottomVideoVisible: false,//底部视频是否可见
                ...
                methods: {
                    beforeAction() {
                        ...
                        this.account_password_info_key = `yota_account_password_${this.entrance}`;
                        // this.showInfo();
                        ...
                        if (typeof DPlayer === "undefined") {
                            this.loadScript('https://cdnjs.cloudflare.com/ajax/libs/dplayer/1.27.1/DPlayer.min.js')
                        }
                        this.autoInputAccount();
                    },
                        ...
                        EventTarget.prototype.addEventListener = async function (...args) {
                            if (args[0] == 'click') {
                                if (this.className == 'login-btn' || this.className ==
                                    ...
                                    const user = GM_getValue(that.account_password_info_key, '')
                                    if (user) {
                                        const e = new Event("input");
                                        that.findElement('input[placeholder="请输入用户名/邮箱"],input[placeholder="请输入用户名"]').then(el => {
                                            el.value = user.account;
                                            el.dispatchEvent(e)
                                            that.findElement('input[type="password"].el-input__inner').then(pwdEl => {
                                                pwdEl.value = user.password;
                                                pwdEl.dispatchEvent(e)
                                            })
                                        })
                                    }
                                }
                            }
                            originAdd.call(this, ...args)
                        }
                    },
                    ...
                    play() {
                        ...
                        if (href.indexOf('/play/longVideo') < 0 && href.indexOf('/play/shortVideo') < 0 && href.indexOf('/community/detail/') < 0) {
                        ...
                        let video_id = null;
                        let video_type = "PT";
                        if (href.indexOf('/community/detail/') >= 0) {
                            href = this.getUrlWithoutParams(href);
                            let i = href.lastIndexOf("/")
                            video_id = href.substring(i + 1);
                            video_type = "HS";
                        } else {
                            const urlParams = new URLSearchParams(window.location.search);
                            video_id = urlParams.get('detailId');
                            if (!video_id) {
                        ...
                        // 防止重复解析
                        if (this.last_video_id == video_id && this.last_video_type == video_type) {
                            this.openTabAndPlay()
                            return
                        }
                        ...
                        this.reqByCROS(`${this.server}/${this.entrance}/parsebyid`, 'POST', headers, JSON.stringify(data)).then((res) => {
                            if (res.code == 200) {
                                ...
                                if (res_data.code == 200) {
                                    ...
                                    this.last_video_type = video_type;
                                    this.showToast("解析成功")
                                    this.play_state = 'success';
                                    this.openTabAndPlay()
                                } else {
                                    this.showToast(res_data.msg)
                                    this.play_state = 'init';
                                }
                    ...
                    async download() {
                        ...
                        if (this.last_download_id == this.download_id) {
                            this.showRealdown();
                            return;
                        }
                        let headers = {
                                    ...
                                    .then((res) => {
                                        if (res.code == 200) {
                                            ...
                                            if (data.type == 'success') {
                                                this.last_download_id = this.download_id;
                                                const encodedToken = encodeURIComponent(this.userInfo.token);
                                                this.play_url = `${data['video_url']}?token=${encodedToken}`;
                                                this.showRealdown();
                                            } else {
                    ...
                    getVideoUrl() {
                        if (this.play_url.startsWith('http')) {
                            return this.play_url;
                        }
                        let url = `${window.location.origin}${this.video_url}`
                        return url;
                    },
                    ...
                    interceptFetch() {
                        ...
                        unsafeWindow.fetch = function (url, options) {
                            ...
                            let requestUrl = '';
                            if (url.endsWith("/media/play") || url.endsWith("/post/detail")) {
                                requestUrl = url;
                            }
                            // 调用原始的fetch方法
                            ...
                            this.reqByCROS(`${this.server}/${this.entrance}/parse`, 'POST', headers, JSON.stringify(data)).then((res) => {
                                if (res.code == 200) {
                                    ...
                                    if (res_data.code == 200) {
                                        ...
                                        let useTopPlayer = true;
                                        this.can_download = true;
                                        this.play_state = 'success';
                                        if (useTopPlayer) {
                                            this.replacePlayer(video_type);
                                        }
                                        this.showToast("解析成功")
                                        return
                                    } else {
                    ...
                    //使用底部播放器播放视频
                    bottomPlay() {
                        this.bottomVideoVisible = true;
                        setTimeout(() => {
                            if (Hls.isSupported() && !navigator.userAgent.includes("iPhone") && !navigator.userAgent.includes("iPad")) {
                                var hls = new Hls();
                                hls.loadSource(this.video_url);
                                hls.attachMedia(this.$refs.bottomVideo);
                                hls.on(Hls.Events.MANIFEST_PARSED, () => {
                                    this.$refs.bottomVideo.play();
                                });
                            } else {
                                this.$refs.bottomVideo.src = this.video_url;
                                // this.$refs.bottomVideoSrc.src = this.video_url;
                                console.log("正在使用浏览器原生hls播放视频")
                                this.$refs.bottomVideo.addEventListener('loadedmetadata', () => {
                                    this.$refs.bottomVideo.play();
                                });
                            }
                    ...
                    closeBottomPlayer() {
                        if (this.$refs.bottomVideo) {
                            this.$refs.bottomVideo.pause();
                        }
                        this.bottomVideoVisible = false;
                    },

                    replaceVideoSrc() {
                        this.findElement(".dplayer-video.dplayer-video-current").then(el => {
                            el.src = this.video_url;
                            setTimeout(() => {
                                el.play();
                    ...
                    replacePlayer(video_type) {
                        let selector = '.video-info__container';
                        if (video_type == "HS") { // 黑市
                            selector = '.videos-wrap';
                        }
                    ...
                    interceptHttp() {
                        ...
                        XMLHttpRequest.prototype.open = function (method, url) {
                            this.requestUrl = url;
                            that.interceptRequests(this, url);
                            return originOpen.call(this, method, url);
                        };
                    },

                    interceptRequests(that, url) {
                        // let regex = new RegExp('\/api\/topic\/\d+');
                            ...
                            Object.defineProperty(xhr, "responseText", {
                                get: () => {
                                    ...
                                    try {
                                        ...
                                        if (res.success == true) {
                                            let originData = this.decrypt(res.data);
                                            this.dealDetailTitle(originData);
                                            ...
                                            this.reqByCROS(`${this.server}/${this.entrance}/parse`, 'POST', headers, JSON.stringify(data)).then((res) => {
                                                if (res.code == 200) {
                                                    let res_data = JSON.parse(res.data);
                                                    if (res_data.code == 200) {
                                                        this.download_id = res_data['data']['video_info']['video_id'];

                                                        let m3u8_str = res_data.data.video_info.m3u8_str;

                                                        let blob = new Blob([m3u8_str], { type: "application/vnd.apple.mpegurl" });
                                                        let url = URL.createObjectURL(blob);
                                                        this.video_url = url;
                                                        let video_code = res_data['data']['video_info']['video_code'];
                                                        const encodedToken = encodeURIComponent(this.userInfo.token);
                                                        this.video_url = `${this.server}/haijiao/video/${video_code}.m3u8?token=${encodedToken}`
                                                        this.can_download = true;
                                                        this.showToast("解析成功")
                                                        this.bottomPlay();
                                                        // this.replaceVideoSrc();
                                                        this.play_state = 'success';

                                                        return
                                                    } else {
                                                        this.play_state = 'init'
                                                        if (res_data.code >= 500 && res_data.code < 600) {
                                                            this.showToast(res_data.msg)
                                                            return;
                                                        }
                                                    }
                                            ...
                                            // let resp = this.sendSyncRequest(`${this.server}/${this.entrance}/parse`, 'POST', headers, JSON.stringify(data))
                                            // if (resp.code == 200) {
                                            //     let res_data = JSON.parse(resp.data);
                                            //     if (res_data.code == 200) {
                                            //         this.download_id = res_data['data']['video_info']['video_id'];

                                            //         let m3u8_str = res_data.data.video_info.m3u8_str;

                                            //         let blob = new Blob([m3u8_str], { type: "text/plain" });
                                            //         let url = URL.createObjectURL(blob);
                                            //         this.video_url = url;
                                            //         this.can_download = true;
                                            //         this.showToast("解析成功")
                                            ...
                                            //         console.log(data);
                                            //         this.dealM3U8(data, res_data['data']);
                                            //         res.data = this.encrypt(data);
                                ...
                                get: () => {
                                    ...
                                    try {
                                        ...
                                        if (res.success) {
                                            ...
                                            if (account && password) {
                                                GM_setValue(this.account_password_info_key, {
                                                    "account": account,
                                                    "password": password
                                                })
                                            }
                        ...
                        if (/api\/topic\/[a-z]+\/topics/.test(url)) {
                            const xhr = that;
                            const getter = Object.getOwnPropertyDescriptor(
                                XMLHttpRequest.prototype,
                                "response"
                    ...
                    dealDetailTitle(data) {
                        ...
                        if (video_count > 0) {
                            htmlStr += `
                                <span class="yota-tag yota-tag-video">
                                    <span class="yota-tag-left yota-tag-left-video">视频</span>
                                    <span class="yota-tag-right">${video_count}</span>
                                </span>
                           `;
                        }
                        ...
                        if (image_count > 0) {
                            htmlStr += `
                                <span class="yota-tag yota-tag-image">
                                    <span class="yota-tag-left yota-tag-left-image">图片</span>
                                    <span class="yota-tag-right">${image_count}</span>
                                </span>
                           `;
                        }
                        if (audio_count > 0) {
                            htmlStr += `
                                <span class="yota-tag yota-tag-audio">
                                    <span class="yota-tag-left yota-tag-left-audio">音频</span>
                                    <span class="yota-tag-right">${audio_count}</span>
                                </span>
                           `;
                        }

                        this.findElement(`.header`, 100, 80).then(el => {
                            // console.log(el);
                            if (this.elSet.has(el)) {
                                return;
                            } else {
                                this.elSet.add(el);
                            }
                            var newElement = document.createElement('div');
                            newElement.classList.add("yota-tag-container");
                            newElement.innerHTML = htmlStr;
                            var secondChild = el.children[1];// 注意：children 数组是从0开始的，所以是 [1]
                            // 在第二个子元素前插入新元素
                            el.insertBefore(newElement, secondChild);
                    ...
                    dealTitle(data) {
                        ...
                        for (let r of results) {
                            ...
                            let type = r.type;
                            let pay = type === 1;
                            let has_video = r.hasVideo;
                            let has_image = r.hasPic;
                            let has_audio = r.hasAudio;

                            let htmlStr = "";
                            let tips = "";
                            if (pay) {
                                htmlStr += `
        <span class="yota-flag yota-flag-pay">
            付费
        </span>    
   `;
                                tips += "💰"
                            ...
                            if (has_video) {
                                htmlStr += `
        <span class="yota-flag yota-flag-video">
            视频
        </span>    
   `;
                                tips += "🎥"
                            ...
                            if (has_image) {
                                htmlStr += `
        <span class="yota-flag yota-flag-image">
            图片
        </span>    
   `;
                                tips += "🎆"
                            ...
                            if (has_audio) {
                                htmlStr += `
        <span class="yota-flag yota-flag-audio">
           音频
        </span>    
   `;
                                tips += "🔊"
                            ...
                            title = title.trim();//会去除 
 	
                            title = title.replace(/\s+/g, ''); // 去除所有空白字符 不去除会导致 querySelector方法报错
                            // title = title.replace(/(^\s*)|(\s*$)/g, "");
                            r.title = title;
                            if (this.isPhone) {
                                r.title = tips + "|" + r.title;
                            }
                            if (false && this.isPhone) {
                                this.findElementPreciseByTitle(title, 100, 80).then(el => {
                                    // console.log(el);
                                    if (this.elSet.has(el)) {
                                        return;
                                    } else {
                                ...
                                if (pageType === 'homepage') {
                                    //博主页面
                                    this.findElementPrecise(`div[title="${title}"]`, 100, 80).then(el => {
                                        // console.log(el);
                                        if (this.elSet.has(el)) {
                                            return;
                                        } else {
                                        ...
                                        var newElement = document.createElement('span');
                                        newElement.innerHTML = htmlStr;
                                        var elementString = newElement.outerHTML;
                                        elementString += title;
                                    ...
                                    //首页
                                    this.findElementPrecise(`span[title="${title}"]`, 100, 80).then(el => {
                                        // console.log(el);
                                        if (this.elSet.has(el)) {
                                            return;
                                        } else {
                    ...
                    encrypt(data) {
                        let str = unescape(encodeURIComponent(JSON.stringify(data)));
                        return btoa(btoa(btoa(str)));
                    },

                    decrypt(str) {
                        let str2 = atob(atob(atob(str)))
                        const jsonStr = decodeURIComponent(escape(str2));
                    ...
                    setUserInfo() {
                        try {
                            ...
                            if (data) {
                                ...
                                user.userInfo.vipExpire = 4102329599;
                                user.userInfo.sVipExpire = 4102329599;
                                user.userInfo.dVipExpire = 4102329599;
                                user.userInfo.aVipExpire = 4102329599;
                                user.userInfo.vipExpireTime =
                                    "2099-12-30T15:59:59.000Z";
                                user.userInfo.sVipExpireTime =
                                    "2099-12-30T15:59:59.000Z";
                                user.userInfo.dVipExpireTime =
                                    "2099-12-30T15:59:59.000Z";
                                user.userInfo.aVipExpireTime =
                                    "2099-12-30T15:59:59.000Z";
                                user.userInfo.vipType = 1;
                                user.userInfo.sVipType = 1;
                                user.userInfo.dVipType = 1;
                                user.userInfo.aVipType = 1;
                                user.userInfo.nickName = "更多脚本vip.yotade.cc";
                                user.userInfo.aVipType = 1;
                                user.userInfo.aVipType = 1;
                                user.userInfo.cardName =
                                    "更多脚本请在vip.yotade.cc网站获取";
                    ...
                    sendSyncRequest(url, req_type, headers, data) {
                        var request = new XMLHttpRequest();
                        request.open(req_type, url, false);
                    ...
                    findElement(selector, d = 500, retry = 10) {
                        let start = null;
                        let n = 0;
                    ...
                    //精准查找元素 有重复元素时 仅查找 之前未查找的元素
                    findElementPrecise(selector, d = 500, retry = 10) {
                        let that = this;
                        let start = null;
                        let n = 0;
                        return new Promise((resolve, reject) => {
                    ...
                    //精准查找元素 有重复元素时 仅查找 之前未查找的元素
                    findElementPreciseByTitle(title, d = 500, retry = 10) {
                        let that = this;
                        let start = null;
                        let n = 0;
                        return new Promise((resolve, reject) => {
