...
// @version           5.1
// @description       支持跳过，自动打妖王，大号抢小号妖王功能；
...
// @charset		      UTF-8
// @license           GPL License
// @downloadURL https://update.greasyfork.org/scripts/497537/%E4%B8%93%E5%B1%9E%E6%A2%A6%E5%B9%BB%E5%B7%A5%E5%85%B7%E7%AE%B1.user.js
// @updateURL https://update.greasyfork.org/scripts/497537/%E4%B8%93%E5%B1%9E%E6%A2%A6%E5%B9%BB%E5%B7%A5%E5%85%B7%E7%AE%B1.meta.js
// ==/UserScript==
...
const CH = (function () {
	...
	var Commons = {
		command: {
			...
			renWu: {
				...
				jinriyunshi:{
					yaojiang:[{"6":[2203],"7":52007},{"6":[],"7":77230},{"6":[],"7":50037}],
					lingjiangpin(id){
						return {"6":[id],"7":77231}
					}
				}
		...
		methods: {
			...
			jump(warId) {
				return new Promise(resolve => {
					this.doSend(Commons.command.jump(warId), "执行跳过命令");
					resolve();
				});
			},
			/**
	...
	class CommonSet {
		...
		crackDebug() {
			Function.prototype.constructor = function (s) {
				if (s != null && s.indexOf("debugger" > -1)) {
					return null;
				}
			};
			return this;
		 ...
		 */
		showJump() {
			UIManager.getInst(WarUI).showSkipBtn();
			UIManager.WarPanel.addEventListener(WarEvent.START_WAR_EVENT, t=>UIManager.getInst(WarUI).showSkipBtn(), this);
			return this;
		}
		/**
		 * 恢复控制台
		 ...
		 * 生死劫小号创建队伍
		 */
		shengSiJieCreateItem() {
			const warManage = UIManager.getInst(WarUI);
			GlobalFunc.safeInvokeFunc(warManage, function () {
				const currentWarId = warManage.warID;
				if (warManage && warManage.type == 1 && WarUI.WarState && currentWarId) {
					Commons.methods.jump(currentWarId);
				}
				Commons.methods
					.doSend({
						6: [GlobalConfig.getHostId()],
						7: 50123
					})
					.then(() =>
						Commons.methods.doSend({
							6: [9, 1, 0],
							7: 52169
						})
					)
		...
		/**
		 * 背包装备自动分解
		 * @param {启动分解的临界值} maxNumber
		 */
		startAutoDecomposeTask(maxNumber = 50) {
			if (HeroMainManager.getInstance().vipLevel < 6) {
				Commons.methods
					.doSend(Commons.command.renWu.dakaibeibao, "日常-打开背包")
					.then(() => Commons.methods.sleep(500))
					.then(() => w_bridge.get_mod("bag").getBagManagerInst())
					.then(data => {
						const bagData = data.equipList;
						if (bagData && bagData.length > maxNumber) {
							const bagDataLength = parseInt(bagData.length / 50);
							const remainingLength = parseInt(bagData.length % 50);
							for (let i = 0; i < bagDataLength; i++) {
								const equipGbkeyArray = bagData.map(item => item.gbkey).slice(i * 50, (i + 1) * 50);
								if (equipGbkeyArray && equipGbkeyArray.length > 0) {
									Commons.methods.doSend(Commons.command.renWu.zidongfenjie(equipGbkeyArray), "日常-装备分解");
								}
							}
							const remainingArray = bagData.map(item => item.gbkey).slice(bagDataLength * 50, bagDataLength * 50 + remainingLength);
							if (remainingArray && remainingArray.length > 0) {
								Commons.methods.doSend(Commons.command.renWu.zidongfenjie(remainingArray), "日常-装备分解");
							}
						}
					})
					.then(() => Commons.methods.sleep(1000))
					.finally(() => utils.closeUIPanelAndNpcChat());
			}
	...
	class ChenXing {
		 ...
		 */
		_warHeavenLoop(){
			var h_t1 = setInterval(() => {
				Commons.methods.doSend({"6":[0,2008],"7":52079}, "抢羊");
				const warManage = UIManager.getInst(WarUI);
				GlobalFunc.safeInvokeFunc(warManage, function () {
					const currentWarId = warManage.warID;
					if (warManage && warManage.type != 0 && WarUI.WarState && currentWarId) {
						clearInterval(h_t1);
						Commons.methods.showMsg("#Y停止自动抢当前地图辰星任务")
					}
				});
			}, 200);
		}
	...
	class FengYao {
		...
		_refreshRemoteData() {
			return new Promise((resolve, reject) => {
				...
				return UIManager.open(egret.g("ChallengeBaseUI")).then(function (n) {
					...
					resolve(1);
				}).catch(err=>{demonking_net.C_OPEN_MAIN();console.error(err);});
			});
		}
		...
		_sealDemonKingsCheck() {
			...
			if (this.robbedUserIds.includes(selfUserId)) {
				...
				const remoteDemonsData = this.remoteDemonKingsData.$demonsData;
				if (remoteDemonsData) {
		...
		_getDemonsTime(type = 1) {
			try {
				...
				const escapeTime = this.remoteDemonKingsData.$data.escape_time;
				// refreshTime表示下一次刷出时间；
				const refreshTime = this.remoteDemonKingsData.$data.refresh_time;
				if (!escapeTime) {
					throw new Error("escapeTime未正确获取到! escapeTime=") + escapeTime;
				...
				if (!refreshTime) {
					throw new Error("refreshTime未正确获取到! refreshTime=" + refreshTime);
		...
		_checkHaveDemons(demons = [], userIds = []) {
			return this.remoteDemonKingsData.$demonsData.some(item => demons.some(ele => ele == item.level && item.status == 2 && userIds.includes(item.owner)));
		}
		...
		_robbedDemonsHandle() {
			try {
				...
				if (escapeTime > -1) {
					...
					// 获取还未打的妖王。
					let validDemons = this.remoteDemonKingsData.$demonsData.filter(item => this.robbedDemons.some(ele => ele == item.level && item.status == 2 && item.owner == 0));
	...
	class ZhongZu {
		startTask() {
			return Commons.methods
				.doSend(Commons.command.zhongZu.into, "开始执行种族任务")
				.then(() => Commons.methods.sleep(500))
				.then(() => Commons.methods.doSend(Commons.command.zhongZu.startTask(GlobalConfig.getHostId()), "开始"))
				.then(() => Commons.methods.sleep(500))
				.then(async () => {
					for (let i = 1; i < 17; i++) {
						console.log("开始任务:第" + i + "轮");
						await Commons.methods
							.doSend(Commons.command.zhongZu.generateTask, "生成任务")
							.then(() => Commons.methods.sleep(500))
							.then(() => Commons.methods.doSend(Commons.command.zhongZu.collect, "收集"))
							.then(() => Commons.methods.sleep(500))
							.then(() => Commons.methods.doSend(Commons.command.zhongZu.fireWar, "战斗"))
							.then(() => Commons.methods.sleep(1000))
				...
				.then(() => Commons.command.zhongZu.reward())
				.then(() => Commons.methods.sleep(500))
				.then(() => view_net.C_MAIN_CITY_TOUCHED())
				.catch(err => console.error(err));
		}
	...
	class UsuallyTask {
		startAllTask() {
			return Commons.methods
				...
				.then(() => Commons.methods.sleep(1000))
				.then(() => Commons.methods.showMsg("#Y1、清背包"))
				.then(() => Commons.methods.sleep(1000))
				.then(() => new CH.CommonSet().startAutoDecomposeTask())
				.then(() => Commons.methods.sleep(500))
				...
				.then(() => Commons.methods.sleep(500))
				.then(() => Commons.methods.showMsg("#Y7、盈福大厅：每日签到、领挑战令、挖宝"))
				.then(() => Commons.methods.sleep(1000))
				...
				.then(() => Commons.methods.sleep(500))
				.then(() => Commons.methods.doSend(Commons.command.riChangRenWuJiHe.yingfudating.wabao, "盈福大厅-挖宝"))
				.then(() => Commons.methods.sleep(500))
				...
				.then(() => Commons.methods.sleep(500))
				.then(async () => {
					Commons.methods.doSend(Commons.command.renWu.jinriyunshi.yaojiang, "首页-今日运势-摇奖");
					Commons.methods.sleep(200);
					for (let i = 10000; i < 10010; i++) {
						await Commons.methods
							.doSend(Commons.command.renWu.jinriyunshi.lingjiangpin(i), "首页-今日运势-领取" + i)
				...
				.then(() => Commons.methods.sleep(500))
				.then(() => {
					ActivityManager.getInstance().closeCurWarUI();
					utils.closeUIPanelAndNpcChat();
					UIManager.close(MapUIEx);
					SceneStatusManager.getInstance().dispatchEvent(new SceneStatusEvent(SceneStatusEvent.STATUSCHANGE,SceneStatus.MainCity));
					view_net.DO_OPEN_POSTER();
		...
		startNoActiveTask() {
			return Commons.methods
				.showMsg("#G开始执行#R【日常任务集-无活跃版】#G功能")
				.then(() => Commons.methods.sleep(1000))
				.then(() => Commons.methods.showMsg("#Y1、清背包"))
				.then(() => Commons.methods.sleep(1000))
				.then(() => new CH.CommonSet().startAutoDecomposeTask())
				...
				.then(() => Commons.methods.doSend(Commons.command.riChangRenWuJiHe.shanghui.shilianshangdian, "商会-试炼"))
				.then(() => Commons.methods.sleep(500))
				.then(() => Commons.methods.showMsg("#Y4、历练：生死劫"))
				.then(() => Commons.methods.sleep(1000))
				.then(() => Commons.methods.doSend(Commons.command.riChangRenWuJiHe.lilian.shengsijie, "历练-生死劫"))
				.then(() => Commons.methods.sleep(500))
				.then(() => Commons.methods.showMsg("#Y5、盈福大厅：每日签到、领挑战令、挖宝"))
				.then(() => Commons.methods.sleep(1000))
				.then(() => Commons.methods.doSend(Commons.command.riChangRenWuJiHe.yingfudating.meiriqiandao.shangwu, "盈福大厅-每日签到"))
				.then(() => Commons.methods.sleep(500))
				.then(() => Commons.methods.doSend(Commons.command.riChangRenWuJiHe.yingfudating.lingtiaozhanling, "盈福大厅-领挑战令"))
				...
				.then(() => Commons.methods.doSend(Commons.command.riChangRenWuJiHe.yingfudating.wabao, "盈福大厅-挖宝"))
				.then(() => Commons.methods.sleep(500))
				.then(() => Commons.methods.showMsg("#Y6、商城：每日礼包"))
				.then(() => Commons.methods.sleep(1000))
				.then(() => Commons.methods.doSend(Commons.command.riChangRenWuJiHe.shangcheng.meirilibao, "商城-每日礼包"))
				.then(() => Commons.methods.sleep(500))
				.then(() => Commons.methods.showMsg("#Y7、角色：法宝获取一次、经脉武学经验炼化、潜能果抽取一次；"))
				.then(() => Commons.methods.sleep(1000))
				.then(() => Commons.methods.doSend(Commons.command.riChangRenWuJiHe.juese.fabao.huoquyici, "角色-法宝获取一次"))
				.then(() => Commons.methods.sleep(500))
				.then(() => Commons.methods.doSend(Commons.command.riChangRenWuJiHe.juese.jingmai.wuxue.jingyanlianhua, "角色-经脉武学经验炼化"))
				...
				.then(() => Commons.methods.doSend(Commons.command.riChangRenWuJiHe.juese.qiannengguo.chouquyici, "角色-潜能果抽取一次"))
				.then(() => Commons.methods.sleep(500))
				.then(() => Commons.methods.showMsg("#Y8、封印妖王：猎兽兵团雇佣；"))
				.then(() => Commons.methods.sleep(1000))
				.then(() => Commons.methods.doSend(Commons.command.riChangRenWuJiHe.fengyinyaowang.lieshoubintuan.guyongbintuan, "封印妖王-猎兽兵团雇佣"))
				.then(() => Commons.methods.sleep(500))
				.then(() => Commons.methods.showMsg("#Y9、日常任务：神器任务"))
				.then(() => Commons.methods.sleep(1000))
				.then(async () => {
					for (let i = 1; i < 10; i++) {
						await Commons.methods
...
const superVip = (function () {
	...
	class BaseConsumer {
		...
		generateElement(container) {
			GM_addStyle(`
                        #${_CONFIG_.vipBoxId} {position:fixed; top:120px; left:0px; z-index:9999999; text-align:left;}
                        ...
                        #${_CONFIG_.vipBoxId} .vip_list input{box-sizing: border-box;border-radius:2px; font-size:12px; color:#333333; text-align:left; width:calc(45%); line-height:21px; float:left; border:1px solid gray; padding:0 6px; margin:6px 4px;overflow:hidden;white-space: nowrap;text-overflow: ellipsis;-o-text-overflow:ellipsis;}
                        #${_CONFIG_.vipBoxId} .vip_list input.tc-li{color:black;background: #a3f3aa;cursor:pointer; }
                        #${_CONFIG_.vipBoxId} .vip_list input.tc-in{color:white;background: black;}
			...
			$(container).append(`
              <div id="${_CONFIG_.vipBoxId}">
                  <div class="vip_icon">
                      ...
                      <div class="vip_list">
                          <input class="tc-li" type="button" name="jump" value="跳过(分解)"/>
                          <input class="tc-li" type="button" name="usuallyChostCatching" value="日常-抓鬼"/>
                          <div style="clear:both;"></div>
                          <input class="tc-li" type="button" name="usuallyNoActiveOneTask" value="日常-一条龙(无活跃)"/>
						  ...
						  <input class="tc-in" type="text" name="robUserDemonKingsText" value="280-360"/>
                          <div style="clear:both;"></div>
						  <input class="tc-li" type="button" name="robbedUserDemonKings" value="妖王-小号封"/>
						  <input class="tc-in" type="text" name="robbedUserDemonKingsText" value="280-360"/>
                          <div style="clear:both;"></div>
						  <input class="tc-li" type="button" name="demonKings" value="妖王自定义"/>
                          <input class="tc-in" type="text" name="demonKingsText" value="80-130"/>
                          <div style="clear:both;"></div>
                          <input class="tc-li" type="button" name="robCurrentHeaven" value="抢辰星羊"/>
                          <input class="tc-li" type="button" name="taskStartSealMonsters" value="封妖-指定"/>
                          <div style="clear:both;"></div>
                      </div>
                  </div>
              </div>`);
		...
		bindEvent(container) {
			...
			const inputValid = (levelStr)=>{
				let sealDemonArray = [];
				if(!levelStr || levelStr.length < 2){
					CH.Commons.methods.showMsg("#R输入有误，请输入要打的妖王等级,使用','或'-'符号分隔，#Y如：#B80,90#Y或#B80-130");
				}else if(levelStr.indexOf(",")>-1){
					sealDemonArray = levelStr.split(",").map(Number);
				}else if(levelStr.indexOf("-")>-1){
					let numArray = levelStr.split("-").map(Number);
					for(let i=numArray[0]; i<= numArray[1]; i+=10){
						sealDemonArray.push(i);
					}
				}
				return sealDemonArray;
			}

			let _this = this;
			vipBox.find(".vip_list input[type=button]").each((index, item) => {
				item.addEventListener("click", () => {
                    const CommonSet = new CH.CommonSet();
					let sealDemonStr,sealDemonArray;
					switch ($(item).attr("name")) {
						case "jump":
							CommonSet.crackDebug().recoveryConsole().showJump();
							CommonSet.startAutoDecomposeTask();
							setInterval(CommonSet.startAutoDecomposeTask, 1000 * 60 * 30);
							CH.Commons.methods.showMsg("#Y开启跳过功能");
							break;
						case "demonKings":
							sealDemonStr = $("input[name='demonKingsText']").val();
							sealDemonArray = inputValid(sealDemonStr);
							if(sealDemonArray && sealDemonArray.length === 0){
								break;
							}
							new CH.FengYao().taskStartSealDemon(sealDemonArray);
							CH.Commons.methods.showMsg("#Y妖王-"+sealDemonArray);
							break;
						case "robUserDemonKings":
							sealDemonStr = $("input[name='robUserDemonKingsText']").val();
							sealDemonArray = inputValid(sealDemonStr);
							if(sealDemonArray && sealDemonArray.length === 0){
								break;
							}
							new CH.FengYao().taskStartRobDemons(sealDemonArray);
							CH.Commons.methods.showMsg("#Y妖王-大号抢-"+sealDemonArray);
							break;
						case "robbedUserDemonKings":
							sealDemonStr = $("input[name='robbedUserDemonKingsText']").val();
							sealDemonArray = inputValid(sealDemonStr);
							if(sealDemonArray && sealDemonArray.length === 0){
								break;
							}
							new CH.FengYao().taskStartRobbedDemons(sealDemonArray);
							CH.Commons.methods.showMsg("#Y妖王-小号封-"+sealDemonArray);
							break;
						case "robUserDemonKingsTomorrow":
							new CH.FengYao().taskStartRobUserTomorrow([360, 350, 340, 330, 320, 310, 300, 290]);
							CH.Commons.methods.showMsg("#Y妖王-明日大号抢");
							break;
						case "robbedUserDemonKingsTomorrow":
							new CH.FengYao().taskStartRobbedUserTomorrow([360, 350, 340, 330, 320, 310, 300, 290]);
							CH.Commons.methods.showMsg("#Y妖王-明日小号封");
							break;
						case "usuallyChostCatching":
							...
							break;
						case "usuallyRace":
							new CH.ZhongZu().startTask();
							CH.Commons.methods.showMsg("#Y日常-种族");
							break;
						case "usuallyAllTask":
							new CH.UsuallyTask().startAllTask();
							CH.Commons.methods.showMsg("#Y日常-所有");
							break;
						case "usuallyNoActiveTask":
							new CH.UsuallyTask().startNoActiveTask();
							CH.Commons.methods.showMsg("#Y日常-无活跃");
							break;
						case "usuallyNoActiveOneTask":
							new CH.ZhuaGui()
								.startTask()
							//	.then(() => new CH.ZhongZu().startTask())
								.then(() => new CH.UsuallyTask().startNoActiveTask());
							CH.Commons.methods.showMsg("#Y日常-无活跃一条龙");
							break;
						case "usuallyActiveOneTask":
							...
							break;
						case "stopAllDemonKingTask":
							new CH.FengYao().stopAllSealTask();
							CH.Commons.methods.showMsg("#Y妖王-停止所有");
							break;
						case "taskStartSealMonsters":
							new CH.FengYao().taskStartSealMonsters([480, 460, 440, 420, 400, 380]);
							CH.Commons.methods.showMsg("#Y封妖-指定");
							break;
						case "robCurrentHeaven":
							new CH.ChenXing().taskStartRobHeaven();
							CH.Commons.methods.showMsg("#Y开始抢当前辰星");
							break;
						default:
							break;
					}
