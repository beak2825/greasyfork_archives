...
// @downloadURL https://update.greasyfork.org/scripts/11781/AgarRussia.user.js
// @updateURL https://update.greasyfork.org/scripts/11781/AgarRussia.meta.js
// ==/UserScript==
...
window.jQuery.ajax({
    ...
    if (io[0] != version) {
        ...
        $('#mainPanel').html('<h3>Новое обновление!</h3><br>Внимание!<br>Вышло новое обновление расширения AgarRussia!<br>Версия: ' + io[0] + '<br>Что нового: ' + io[4] + '.<br>Загрузить обновление можно перейдя по ссылке: <a href="' + io[1] + '">AgarRussia.JS</a>');
            ...
            (function() {
                        ...
                        switch (packet.type) {
                            case 128:
                                for (var i = 0; i < packet.data.addition.length; ++i) {
                                    ...
                                    if (!miniMapIsRegisteredToken(cell.id)) {
                                        miniMapRegisterToken(cell.id, miniMapCreateToken(cell.id, cell.color));
                                    }
                            ...
                            case 130:
                                if (agar_server != packet.data.url) {
                                    ...
                                    var content = ('You are now at: <strong>' + agar_server + '</strong><br>Your team members are all at: <strong>' + packet.data.url + ', ' + region_name + ':' + gamemode_name + packet.data.party + '</strong>.<br>The minimap server has disconnected automatically.');
                ...
                function miniMapRender() {
                    ...
                    for (var id in window.mini_map_tokens) {
                        ...
                        ctx.fill();
                        if (options.enableCross && -1 != current_cell_ids.indexOf(token.id)) miniMapDrawCross(token.x, token.y, token.color);
                        if (options.enableAxes && -1 != current_cell_ids.indexOf(token.id)) miniMapDrawMiddleCross()
                ...
                function miniMapInit() {
                    ...
                    if (render_timer === null) render_timer = setInterval(miniMapRender, 1000 / 30);
                    ...
                    if ($('#mini-map-options').length === 0) {
                        ...
                        var setting_btn = $('<button>').addClass('btn').css({
                            ...
                            setting_btn.blur();
                            return false;
                        }).append($('<i>').addClass('glyphicon glyphicon-cog')).appendTo(form_group);
                    }
                    if ($('#mini-map-party').length === 0) {
                        var mini_map_party = window.mini_map_party = $('<div>').css({
                            top: 50,
                            left: 10,
                            width: 200,
                            color: '#FFF',
                            ...
                            textAlign: 'center',
                            padding: 10
                        }).attr('id', 'mini-map-party').appendTo(window.document.body).append($('<h3>').css({
                            margin: 0,
                            padding: 0
                        }).text('Party'));
                        var mini_map_party_list = $('<ol>').attr('id', 'mini-map-party-list').css({
                            listStyle: 'none',
                            padding: 0,
                            margin: 0
                        ...
                        mini_map_party.on('update-list', function(e) {
                            ...
                            for (var p in players) {
                                ...
                                $('<li>').text(player.no + 1 + '. ' + name).appendTo(mini_map_party_list);
                            }
                        });
                        mini_map_party.hide();
                    }
                ...
                Cell.prototype = {
                    ...
                    destroy: function() {
                        ...
                        id = current_cell_ids.indexOf(this.id); - 1 != id && current_cell_ids.splice(id, 1);
                        this.destroyed = true;
                        if (map_server === null || map_server.readyState !== window._WebSocket.OPEN) {
                ...
                function extractCellPacket(data, offset) {
                    ...
                    for (var e = 0; e < size; ++e) {
                        var p = cells[data.getUint32(c, true)],
                            ...
                            c = c + 8;
                        p && f && (f.destroy(), f.ox = f.x, f.oy = f.y, f.oSize = f.size, f.nx = p.x, f.ny = p.y, f.nSize = f.size, f.updateTime = I, dataToSend.destroyQueue.push(f.id));
                    }
                    for (e = 0;;) {
                        ...
                        c = c + 2;
                        for (var h = data.getUint8(c++), m = data.getUint8(c++), q = data.getUint8(c++), h = (h << 16 | m << 8 | q).toString(16); 6 > h.length;) h = "0" + h;
                        var h = "#" + h,
                            k = data.getUint8(c++),
                            m = !!(k & 1),
                            q = !!(k & 16);
                        ...
                        k = null;
                        var updated = false;
                        cells.hasOwnProperty(d) ? (k = cells[d], k.updatePos(), k.ox = k.x, k.oy = k.y, k.oSize = k.size, k.color = h, updated = true) : (k = new Cell(d, p, f, g, h, n), k.pX = p, k.pY = f);
                        k.isVirus = m;
                        k.isAgitated = q;
                        k.nx = p;
                        k.ny = f;
