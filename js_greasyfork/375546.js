// ==UserScript==
// @name         HIT For(Slac)ker
// @version      1.0.0.7
// @description  Monitors mturk.com for HITs
// @author       ThisPoorGuy And MODIFIED for slack friendly exports by TurkIngiNtheusa
// @icon         http://nopurpose.org/stuff/avatars/Lj21396.gif
// @include      https://worker.mturk.com/?finder_beta
// @include      https://worker.mturk.com/?hit_for_Slac_ker
// @grant        GM_log
...
// @namespace https://greasyfork.org/users/232726
// @namespace https://greasyfork.org/users/163167
// @downloadURL https://update.greasyfork.org/scripts/375546/HIT%20For%28Slac%29ker.user.js
// @updateURL https://update.greasyfork.org/scripts/375546/HIT%20For%28Slac%29ker.meta.js
// ==/UserScript==

// Acknowledgements (HIT For(Slac)ker)
// All but a dozen or so lines of this script are directly stolen from ThisPoorGuy's HIT Forker. He deserves all of the credit for this script.
// This version is simply a modified version of his 1.0.1 version that has been adapted for an export that is slack friendly.
// Thanks also have to be given to tjololo for the bug fix in his script that allows this export to work.
// Acknowledgements (HIT Forker)
...
// Changelog (HIT For(Slac)ker)
// 1.0.0.7 - Rolled back changes until proper dubugging can occur
// 1.0.0.4 - updated the changes in 1.0.0.3
// 1.0.0.3 - Returned vB export and removed IRC exort. Also, modified reward export code to hard code $ and remove it from variable.
// 1.0.0.2 - Fixed a bug in export
// 1.0.0.1 - Modified export to be slack friendly. There is no VB export. Only slack and IRC.
// Changelog (HIT Forker)
...
$('body').html(
  ...
  '<div style="position: absolute; top: 32px width: 100px; font-size: 14pt; font-weight: bold;" id="menu_title">HIT For(Slac)ker</div>'+
...
function _scrape_new (data, timeis) {
  ...
  for (var i = 0; i < hits.length; i ++) {
    var hit = hits[i],
        ...
        req_link  = hit.requester_url.replace(/\.json/, ''),
        ...
        prev_link = hit.project_tasks_url.replace(/\.json/, ''),
        //prev_link = 'https://www.mturk.com/mturk/preview?groupId=' + hit.hit_set_id,
        pand_link = hit.accept_project_task_url.replace(/\.json/, ''),
...
function _build (keys, log_keys, timeis) {
    ...
    hit_html +=
      ...
      '<button value="' + hit.key + '" class="irc">SLK</button>' +
  ...
  if (log_keys.length) {
    for (var j = 0; j < log_keys.length; j ++) {
      ...
      log_html +=
        ...
        '<button value="' + hit_log.key + '" class="irc">SLK</button>' +
