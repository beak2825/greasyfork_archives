// ==UserScript==
// @name         吉林继续教育--吉林农科专技
// @namespace    http://tampermonkey.net/zzzzzzys_吉林农科专技
// @version      1.0.0
// @copyright    zzzzzzys.All Rights Reserved.
// @description  吉林农科专技，可秒刷视频，也可常速自动化播放所有视频！
// @author       zzzzzzys
// @match        https://media.lllnet.cn/media/accessCourseInfo
...
// @run-at       document-end
// @license      
// @downloadURL https://update.greasyfork.org/scripts/532189/%E5%90%89%E6%9E%97%E7%BB%A7%E7%BB%AD%E6%95%99%E8%82%B2--%E5%90%89%E6%9E%97%E5%86%9C%E7%A7%91%E4%B8%93%E6%8A%80.user.js
// @updateURL https://update.greasyfork.org/scripts/532189/%E5%90%89%E6%9E%97%E7%BB%A7%E7%BB%AD%E6%95%99%E8%82%B2--%E5%90%89%E6%9E%97%E5%86%9C%E7%A7%91%E4%B8%93%E6%8A%80.meta.js
// ==/UserScript==
class Runner {
    constructor() {
        this.runner = null
    ...
    run() {
        unsafeWindow.onload = () => {
            ...
            if (url.includes("accessCourseInfo")) {
...
class Course {
        ...
        try {
            ...
            const confirmResult = await Swal.fire({
                ...
                html: `<div style="text-align:left">
                    <b>注意事项：</b>
                    <li>长视频有概率触发反作弊机制导致异常数据而失败：</li>
                    <li>多次刷取失败时，此时应获取课程进度查看是否刷完，也可休息一段时间再刷</li>
                   </div>`,
                icon: 'warning',
            ...
            console.error(error)
            Swal.fire({
                title: "已刷取部分时间(进度更新可能延迟)！",
                text: "长视频此平台有概率执行失败，且不宜高频刷课！您可休息一会儿刷新进度看是否完成或再次刷取！若一直失败，请联系进行售后处理！",
                icon: 'error',
                confirmButtonText: '确定',
                allowOutsideClick: false,
                timerProgressBar: true,
    ...
    async run() {
        try {
            const catalogSelector='#contents_list ul li'
            const catalogList=await Utils.getStudyNode(catalogSelector,'nodeList')
            for (let i = 0; i < catalogList.length; i++) {
                const catalog = catalogList[i];
                console.log(catalog.querySelector('.t-item').innerText)
                const status=this.checkStatus(catalog)
                if(status){
                    console.log("已学完，跳过！")
                    continue
                }
                if(!this.checkCurrent(catalog)){
                    catalog.querySelector('a').click()
                }
    ...
    checkStatus(dom) {
        return dom.querySelector('.yuanbox1')
    }
    checkCurrent(dom) {
        return dom.querySelector('.current')
    }
}
...
class Utils {
    ...
    static flag = 'lllnet_VIP'
    static jsFlag = 'lllnet_JsCode'
    ...
    static async validateCode(data) {
        try {
            ...
            data.website = "67c7d1548a5c78c37fa5dbae"
            console.log(data)
            // return
