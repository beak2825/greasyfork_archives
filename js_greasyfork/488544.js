...
// @downloadURL https://update.greasyfork.org/scripts/488544/%E5%BB%8A%E5%9D%8A%E5%B8%88%E8%8C%83.user.js
// @updateURL https://update.greasyfork.org/scripts/488544/%E5%BB%8A%E5%9D%8A%E5%B8%88%E8%8C%83.meta.js
// ==/UserScript==
...
//         if(window.location.host !== 'cws.edu-edu.com'){
...
//         }
    ...
    function juCourse(){
        if(courseIndex < courseCount){
            let ariaValuenow = document.getElementsByClassName("single-lists")[courseIndex].getElementsByClassName('red')[0].innerText;
            let ariaValuenowInt = parseInt(ariaValuenow);
            if(ariaValuenowInt < 98){
                document.getElementsByClassName("single-lists")[courseIndex].getElementsByClassName('learn-button')[0].children[0].children[0].children[0].click()
                window.close();
            }
            else{
                let courseName = document.getElementsByClassName("single-lists")[courseIndex].getElementsByTagName('h3')[1].innerText;
    ...
    function jianTing(){
        document.querySelector("#J_prismPlayer > video").volume = 0;
        try{
            let textMsg = document.getElementsByClassName('alert-modal v-transfer-dom')[1].getElementsByClassName('ivu-modal-wrap')[0].getElementsByTagName('span')[2].innerText;
            if(textMsg === '学员您好，当前系统已经停止计时，为了确认您是否在计算机面前，特此弹出对话框询问，如有带来不便，请您谅解！'){
                addMessage("挂机响应...");
                document.getElementsByClassName('alert-modal v-transfer-dom')[1].getElementsByClassName('ivu-modal-wrap')[0].getElementsByTagName('button')[0].click();
                setTimeout(function(){
                    jianTing();
                },8000);
            }else{
    ...
    var wait = setInterval(function (){
        ...
        else if(juUrlInt('courseware-player') != -1 && juUrlInt('courseware-player') != 0){
            try {
                addMessage("学习页...");
                document.querySelector("#app > div > dl > div > span.ivu-switch.ivu-switch-default").click();
                jianTing();
                clearInterval(wait);
            }catch(e){
                addMessage("发现异常："+e+"|程序已终止运行...");
        ...
        else if(juUrlInt('Login') != -1 && juUrlInt('Login') != 0){
            try {
                addMessage("请登录");
            }catch(e){
                addMessage("发现异常："+e+"|程序已终止运行...");
        ...
        else if(juUrlInt("IndexNew") != -1 && juUrlInt("IndexNew") != 0){
            addMessage("首页");
            setTimeout(function(){
                saveIndexUrl
                if(document.location.host === 'tstc.edu-cj.com'){
                    document.getElementsByClassName("online-left")[0].children[1].click();
                    setTimeout(function(){
                        courseCount = document.getElementsByClassName("single-lists").length;
                        courseIndex = 0;
                        juCourseCaztc();
                    },2000);
                    courseCount = document.getElementsByClassName("single-lists").length;
                    ...
                    window.open('https://lfnu.edu-cj.com/MyOnlineCourseNew/OnlineLearningNew/OnlineLearningNewIndex','_self');
                }
            },2000);
            clearInterval(wait);
        }
        else if(juUrlInt("OnlineLearningNewIndex") != -1 && juUrlInt("OnlineLearningNewIndex") != 0){
            addMessage("课程列表");
            setTimeout(function(){
                courseCount = document.getElementsByClassName("single-lists").length;
                courseIndex = 0;
        ...
        else if(juUrlInt("termCourseID") != -1 && juUrlInt("termCourseID") != 0){
            addMessage("简介");
            setTimeout(function(){
                document.querySelector("body > div.task_list.main > ul > li > div > div.titer-main-right > div > a").click();
            },2000);
            clearInterval(wait);
        }
        else{
