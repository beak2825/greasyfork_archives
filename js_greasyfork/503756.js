// ==UserScript==
// @name   🌱【免费版】上海理工大学|广州软件学院等|如需高级收费版本见文档：https://doc.zhanyc.cn/pages/shdxjyxyxy/
// @namespace    http://jb.zhanyc.cn/
// @icon    https://js.zhanyc.cn/img/js-logo.svg
// @version      2.0
// @description  【已适配新版】当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制功能。如需倍速、自动换课程、全自动无人值守等高级功能可升级付费版本，一杯咖啡钱，保你无忧学习，且永久使用|接各类脚本开发、代挂工作，微信：zhanyc_cn 备用微信:zhanfengkuo 个人网站：http://doc.zhanyc.cn。适用于：上海理工大学|韩山师范学院|广州软件学院|上海出版印刷高等专科学校|上海大学继续教育学院继续教育管理平台|上海立信会计金融学院|广东职业技术学院|华南农业大学继续教育学院|开封大学继续教育学院|东华大学网络教育学院考试系统|广州学涯|华南农业大学|东华大学|延安大学|上海应用技术大学等
// @author       zfk
// @include    *://*shu.edu.cn/*
...
// @antifeature 
// @license GPL
// @downloadURL https://update.greasyfork.org/scripts/503756/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E4%B8%8A%E6%B5%B7%E7%90%86%E5%B7%A5%E5%A4%A7%E5%AD%A6%7C%E5%B9%BF%E5%B7%9E%E8%BD%AF%E4%BB%B6%E5%AD%A6%E9%99%A2%E7%AD%89%7C%E5%A6%82%E9%9C%80%E9%AB%98%E7%BA%A7%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%E8%A7%81%E6%96%87%E6%A1%A3%EF%BC%9Ahttps%3Adoczhanyccnpagesshdxjyxyxy.user.js
// @updateURL https://update.greasyfork.org/scripts/503756/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E4%B8%8A%E6%B5%B7%E7%90%86%E5%B7%A5%E5%A4%A7%E5%AD%A6%7C%E5%B9%BF%E5%B7%9E%E8%BD%AF%E4%BB%B6%E5%AD%A6%E9%99%A2%E7%AD%89%7C%E5%A6%82%E9%9C%80%E9%AB%98%E7%BA%A7%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%E8%A7%81%E6%96%87%E6%A1%A3%EF%BC%9Ahttps%3Adoczhanyccnpagesshdxjyxyxy.meta.js
// ==/UserScript==
(function () {
    ...
    let freeTips = "当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制。如需倍速、自动换课程、全自动无人值守等高级功能，可点击下方按钮查看付费版本"
    let docUrl = "http://doc.zhanyc.cn/pages/shdxjyxyxy/";
    let plugMain = Object.assign(baseConfig, {
        ...
        async runByUrl(url) {
            if (url.includes('/learn/outline/courseware/item/')) {
                ...
                url.includes("index/course/learn/courseware/video") ||
                ...
                plugMain.page_video();
            } else if (url.includes("/StudentPortal/StudentSchedule")) {
        ...
        async page_video2() {
            ...
            await plugMain.waitOf((a) => $(`.catalogue-wrap [role="progressbar"]`).length > 0);
        ...
        async page_studentIndex() {
            console.log("%c page_studentIndex", "background:rgb(0,0,0);color:#fff");
            plugMain.closeWaitConfrimWin();
            ...
            await plugMain.waitTimeout(1000);
            let arr = $(".count").text().split("/");
            if (Number(arr[0]) > Number(arr[1])) {
                plugMain.logFinish();
                return;
            }
            plugMain.confirmRun().then((a) => {
                if (plugMain.getElByText($("button"), "开始学习") != null) {
                    plugMain.getElByText($("button"), "开始学习").click();
                } else if (plugMain.getElByText($("button"), "继续学习") != null) {
                    plugMain.getElByText($("button"), "继续学习").click();
                } else {
                    layer.alert("异常情况");
                    return;
                }
        ...
        async page_video() {
            ...
            await plugMain.waitOf((a) => $(".section_status").length > 0);
            if ($(".fa-adjust,.fa-circle-o").length == 0) {
                plugMain.confirmRun("当前课程已经学习完毕，准备前往下一门课程").then((a) => {
                    plugMain.logFinish();
                });
                return;
            }
            await plugMain.waitTimeout(1000);
            $(".fa-adjust,.fa-circle-o")
                .eq(0)
                .parents(".course_chapter_item:first")
                .find(".section_title")
                .click();
            plugMain.pageData.waitTime = 10;
            plugMain.pageData.waitTime = 3;
            layer.msg("10秒后继续操作");
            plugMain.pageData.video.index = setInterval(async () => {
                try {
                    if (plugMain.pageData.waitTime > 0) {
                    ...
                    let $tips = plugMain.getElByText(
                        $(".layui-layer-content"),
                        "您的视频课件观看时长未达到，请继续观看以完成学习"
                    );
                    if ($tips != null) {
                        $tips.parents(".layui-layer").find(".layui-layer-btn0")[0].click();
                        return
                    }
                    if (plugMain.getElByText(
                        $(".layui-layer-content"),
                        "该视频课件已观看完毕，是否继续学习下一课程节点？"
                    ) != null) {
                        plugMain.getElByText($(".layui-layer-content"),
                            "该视频课件已观看完毕，是否继续学习下一课程节点？"
                        ).parents(".layui-layer").find(".layui-layer-btn0")[0].click();
                        return
                    }
                    if (plugMain.getElByText(
                        $(".layui-layer-content"),
                    ...
                    if (plugMain.getElByText($(".complete"), "已完成") != null) {
                        plugMain.pageData.waitTime = 20
                        plugMain.confirmRun("当前视频已经完成，前往下一个任务").then(a => {
                            plugMain.nextVideo()
                        })
                        return
                    }
                    if (!plugMain.getVideo()) {
                        console.log("%c zfk no video", "background:rgb(0,0,0);color:#fff");
                        plugMain.pageData.waitTime = 30
                        plugMain.tipsMsg("等待确认操作", 10 * 1000)
                        await plugMain.waitTimeout(10 * 1000)
                        if (!plugMain.getVideo()) {

                            let index = $("#menu_tarr_content>div").index()
                            if ($("#menu_tarr_content>div.active").is($("#menu_tarr_content>div:last"))) {
                                plugMain.nextVideo();
                            } else {
                                $("#menu_tarr_content>div.active").next().click()
                                plugMain.pageData.waitTime = 0
                            }
                    ...
                    $("title").text(title);
                    console.log("%c video run", "background:rgb(255,0,0);color:#fff");
                    if (plugMain.getElByText($(".layui-layer-btn0"), "继续播放")) {
                        plugMain.getElByText($(".layui-layer-btn0"), "继续播放")[0].click();
                        return;
                    }
                    if (plugMain.getElByText($(".layui-layer-btn0"), "学习下一课节")) {
                        plugMain.getElByText($(".layui-layer-btn0"), "学习下一课节")[0].click();
                        plugMain.pageData.waitTime = 10;
                        layer.msg("10秒后继续操作");
                        return;
                    }
                    ...
                    if (!isPlay) {
                        if (!isFinish) {
                            ...
                            if (plugMain.getElByText($(".popup_title"), '视频弹题', 'like') == null) {
                                plugMain.play();
                            } else {
                                plugMain.pageData.waitTime = 30
                                plugMain.page_video_QA()
                            }
                        }
                    }
                    if (plugMain.getVideo().playbackRate != bs) {
                        plugMain.getVideo().playbackRate = bs;
                    }
                } catch (e) {
                    console.error("视频页面定时器出错", e);
                }
        ...
        async page_video_QA() {
            plugMain.showPaidContent(`免费版本无权查询题库，无法答题！如需使用请安装收费版本`);
        },
        checkVideoPlay() {
