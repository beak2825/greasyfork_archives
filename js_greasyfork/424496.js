// ==UserScript==
// @name           virtonomica:Salary & Education edited beta
...
// @downloadURL https://update.greasyfork.org/scripts/424496/virtonomica%3ASalary%20%20Education%20edited%20beta.user.js
// @updateURL https://update.greasyfork.org/scripts/424496/virtonomica%3ASalary%20%20Education%20edited%20beta.meta.js
// ==/UserScript==
...
var run = function() {
    ...
    function setSalary(url, objSalary, objLevel, action){
        ...
        objSalary.empty().append($('<img>').attr({'src': 'https://i.gifer.com/ZKZx.gif', 'height': 16, 'width': 16}).css('padding-right', '20px'));
            ...
            $.ajax({url : calcUrl, data:{employees:quantity, salary:Sreq},type:'get',dataType:'json',
                success:function(data){
                    ...
                    $.ajax({url : url, type : "post", data : {'unitEmployeesData[quantity]' : quantity, 'unitEmployeesData[salary]' : getSalary(Sreq)},
                        ...
                        set_ok(objSalary, Sreq, Savg);
                        //setTimeout(set_ok(objSalary, Sreq, Savg), getRndInteger());
                    });
                }
            }).fail( function (jqXHR, textStatus, error) {
                console.log("FAIL (calcUrl) {textStatus=" + textStatus + ", error=" + error + "}");
                objSalary.prev().append("c");
                setTimeout(setSalary(url, objSalary, objLevel, action), getRndInteger());
            });
        })
            .fail( function (jqXHR, textStatus, error){
    ...
    function getRndInteger(min = 900, max = 1000) {
    ...
    wc_set11.click(function(){
        ...
        for(var i=0; i< list_tr.length; i++){
            ...
            var el = $(".kv11", tr);
            //el.click();
            if ( $("div.in-holiday", tr).length > 0 ) {
                continue;
            }
            setTimeout(function(item){item.click();}, el_count++*getRndInteger(990, 1000), el);
        }
    ...
    wc_mass_edu.click(function(){
        ...
        for(var i=0; i< list_tr.length; i++){
            ...
            var el = $(".edu", tr);
            //el.click();
            setTimeout(function(item){item.click();}, i*getRndInteger(), el);
        }
    });
