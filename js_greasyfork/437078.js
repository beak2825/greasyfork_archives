...
// @downloadURL https://update.greasyfork.org/scripts/437078/E-Dush.user.js
// @updateURL https://update.greasyfork.org/scripts/437078/E-Dush.meta.js
// ==/UserScript==
...
new MutationObserver(function(mutations) {
    if (document.getElementsByTagName('script')[0]) {
        ...
        if (owner == "xSploit") {
           ...
           tagDiv.innerText = "[E-Owner] ";
           newDiv.prepend(tagDiv);
        }
        if (owner == "Dush_") {
           let tagDiv = document.createElement("span");
           tagDiv.style.color = "green";
           tagDiv.innerText = "[Sr. Dev] ";
           newDiv.prepend(tagDiv);
        }
        if (owner == "Name1019YT") {
           let tagDiv = document.createElement("span");
           tagDiv.style.color = "green";
           tagDiv.innerText = "[Jr. Dev] ";
           newDiv.prepend(tagDiv);
        }
    `)
...
let lol = window.setInterval(function(){
    if(currentPlayer){
        ...
        chatArea.innerHTML = chatArea.innerHTML + ("<span style=color:#7289da> [E-Status]: <span style=color:#D1E8E2>E-Dush successfully runned!</span> </span> <br><span style=color:#FF652F>[E-Help]: use") + ("<span style=color:gray> /help </span>") + ("to see commands</span></br>") +
            ("<span style=color:#7289da> [E-Info]: <span style=color:#D1E8E2>E-Dush v0.2.0 — <span style=opacity:0.5;color:yellow>By</span> <span style=color:pink>Name1019YT & Dush_ & xSploit</span></span>") + ("<div class=heroDiv></div>")
