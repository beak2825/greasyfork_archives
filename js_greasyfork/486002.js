...
// @downloadURL https://update.greasyfork.org/scripts/486002/%E7%BC%96%E7%A8%8B%E7%8C%AB%E4%BC%98%E5%8C%96.user.js
// @updateURL https://update.greasyfork.org/scripts/486002/%E7%BC%96%E7%A8%8B%E7%8C%AB%E4%BC%98%E5%8C%96.meta.js
// ==/UserScript==
...
if(window.location.href.startsWith('https://shequ.codemao.cn/work/')){
    //保存nemo作品草稿箱
    (async function(){
        let info = await (await fetch(
          "https://api.codemao.cn/creation-tools/v1/works/" + window.location.href.split("/")[4], 
          {
          "credentials": "include",
          "headers": {
        ...
        if(info.type != 'NEMO'){return}
        let source = await (await fetch(
          'https://api.codemao.cn/creation-tools/v1/works/'+info.id+'/source/public', 
          {
          "credentials": "include",
          "headers": {
            "Content-Type": "application/json;charset=UTF-8",
        ...
        while($('.r-work-c-work_interaction-component-fork_button--fork_work_button').length==0){
            await sleep(100)
        };
        $('.r-work-c-work_interaction-component-fork_button--fork_box_wrap').remove()
        $('.r-work-c-work_interaction-component-fork_button--disable').removeClass('r-work-c-work_interaction-component-fork_button--disable')
        $('.r-work-c-work_interaction-component-fork_button--fork_work_button .r-work-c-work_interaction-component-fork_button--data').html("保存至草稿箱")
        $('.r-work-c-work_interaction-component-fork_button--fork_work_button').click(async()=>{
          let uploadContent = {
            "bcm_version": "0.0.1",
            "cloud_variables": [],
            "n_blocks": 0,
          ...
          uploadContent.n_blocks = info.n_brick
          uploadContent.n_roles = info.n_roles
          uploadContent.name = info.work_name
          uploadContent.root_ids[0] = info.id
          uploadContent.preview = info.preview
          uploadContent.work_url = source.work_urls[0];
          
          let back = await (await fetch(
            'https://api.codemao.cn/nemo/v3/works/upload/1', 
            {
              "method": 'POST', 
          ...
          console.log(back)
          bconf('作品已保存至云端', '可在APP端「我的-云端作品」中查看')
        })
    })();
}
...
async function del_msg(type, offset, type_name) {
    let r=0;
    await fetch(
      "https://api.codemao.cn/web/message-record?query_type=" + type + "&limit=200&offset=" + offset, 
      {
      "credentials": "include",
      "headers": {
        "Content-Type": "application/json;charset=UTF-8",
    ...
    }).then(async function(res) {r=await res.json()}, async function() {r=-1});
    console.log(r)
    if(r==-1){
    ...
    ).then(async function(res) {r2=await res.json()}, async function(obj) {r2=-1});
    console.log(r2)
    if(r2==-1){
        return del_msg(type, offset)
    };
...
if(window.location.href.startsWith('https://shequ.codemao.cn/message')){
    var query_type=[
        'COMMENT_REPLY', 
        'LIKE_FORK', 
        'SYSTEM'
    ...
    (async function(){
        ...
        $('#clean_msg').click(function(){
            if($('.r-message--cur_nav').text().includes('评论与回复')){
                del_msg(query_type[0], 0, '评论与回复')
            }else if($('.r-message--cur_nav').text().includes('赞与被购买')){
                del_msg(query_type[1], 0, '赞与被购买')
            }else if($('.r-message--cur_nav').text().includes('系统消息')){
...
if(window.location.href.startsWith('https://shequ.codemao.cn/work/')){
    ...
    (async function(){
        ...
        if(view_fork){
            ...
            view_fork.innerHTML=`<img src='https://cdn-community.codemao.cn/47/community/d2ViXzMwMDFfODgwODU4MF8wXzE3MDc2NTc4ODU2ODlfYmI0ZjJjMmQ.png' style='height: 28px; margin-top: -3px; margin-left: -4px;'>查看原作品`;
            await get_fws(window.location.href.split("/")[4])
            if(fw_info!=-1){
                view_fork.innerHTML=`<img src='https://cdn-community.codemao.cn/47/community/d2ViXzMwMDFfODgwODU4MF8wXzE3MDc2NTc4ODU2ODlfYmI0ZjJjMmQ.png' style='height: 28px; margin-top: -3px; margin-left: -4px;'>${fw_info.user_info.nickname}`
            }
            console.log(view_fork)
...
<div class="event_target data_report" style="width: 450px !important"><div class="c-work_item--work_item" onclick="window.open('/work/${fw_info.id}')" style="width:450px;margin:0px"><div class="c-work_item--work_cover" style="background-image: url(${fw_info.preview});"></div><div class="c-work_item--work_detail"><p class="c-work_item--name">${fw_info.work_name}</p><p class="c-work_item--datas"><span class="c-work_item--data_span"><i class="c-work_item--icon_view"></i>${fw_info.view_times}</span><span class="c-work_item--data_span"><i class="c-work_item--icon_prise"></i>${fw_info.praise_times}</span></p><p class="c-work_item--author"><img src="${fw_info.user_info.avatar}" alt="" class="c-work_item--author_head"><span class="c-work_item--author_name" onclick="window.open('/user/${fw_info.user_info.id}')">${fw_info.user_info.nickname}</span></p></div></div></div>
            ...
            try{
            await get_fws(window.location.href.split("/")[4]);
            $('.r-work-c-work_interaction--labels_container').append(`<span class="r-work-c-work_interaction--label" id="view_fork"><img src='https://cdn-community.codemao.cn/47/community/d2ViXzMwMDFfODgwODU4MF8wXzE3MDc2NTkwNjY4MTZfZmZkYzM0MTg.png' style='height: 28px; margin-top: -3px; margin-left: -4px;'>再创作族谱</span>`)
...
setInterval(function(){
    let dangers = document.querySelectorAll('iframe, embed')
    if(dangers){
        //console.log(dangers)
        for(let i=0; i<dangers.length; i++){
            ...
            if(danger.src&&!(
                danger.src.startsWith('https://player.codemao.cn')||
                danger.src.startsWith('https://nemo.codemao.cn/w')||
                danger.src.startsWith('https://player.bilibili.com/player.html')
            )){
                console.log(danger, danger.src.startsWith('//player.codemao.cn'), danger.src)
                //danger.removeAttribute('src');
                danger.style.backgroundColor='#dddddd'
                danger.style.width='640px'
                danger.style.height='480px'
                console.log('removed');
                danger.remove()
...
window.bconf = async function(header='提示', content='content', cancel='取消', confirm='确认'){
    $('.c-dialog--dialog_cover').after(`<div class="c-dialog--content_box" id='confirm'><div class="c-dialog-c-confirm_box_center--dialog"><div class="c-dialog-c-confirm_box_center--title">${header}</div><div class="c-dialog-c-confirm_box_center--content">${content}</div><div class="c-dialog-c-confirm_box_center--btns"><a class="c-dialog-c-confirm_box_center--cancel">${cancel}</a><a class="c-dialog-c-confirm_box_center--confirm">${confirm}</a></div></div></div>`)
    document.querySelector('#confirm').offsetHeight
    $('.c-dialog--dialog_wrap').addClass('c-dialog--visiable').addClass('c-dialog--show')
    dialog_r = 0
    $('#confirm .c-dialog-c-confirm_box_center--btns .c-dialog-c-confirm_box_center--confirm').click(function(){dialog_r=2})
    $('#confirm .c-dialog-c-confirm_box_center--btns .c-dialog-c-confirm_box_center--cancel').click(function(){dialog_r=1})
    while(dialog_r==0){
        await sleep(100)
        //console.log(dialog_r)
    }
    $('.c-dialog--dialog_wrap').removeClass('c-dialog--visiable').removeClass('c-dialog--show')
    await sleep(300)
    $('#confirm').remove()
    return dialog_r
}
...
;(async function(){
    if(localStorage.getItem('codemao_optimization_data')){
        window.data=JSON.parse(localStorage.getItem('codemao_optimization_data'))
    }else{
        window.data={}
    }
    while(1){
        localStorage.setItem('codemao_optimization_data', JSON.stringify(data))
        await sleep(1000)
    }
})();
...
.r-work_manager-c-sidebar--selected{
...
.r-user-c-banner--banner .r-user-c-banner--background{
    background-image: url(https://cdn-community.codemao.cn/47/community/d2ViXzMwMDFfODgwODU4MF8wXzE3MDgxNDgxNDI1MjZfYjcxNGRiMDQ.png) !important;
}
/*nemo作品卡片修复*/
.r-work_manager-c-nemo_work_card--detail{
