...
// @downloadURL https://update.greasyfork.org/scripts/410133/AO3%3A%20Kudoshits%20ratio.user.js
// @updateURL https://update.greasyfork.org/scripts/410133/AO3%3A%20Kudoshits%20ratio.meta.js
// ==/UserScript==
...
if (found_stats.length>0) {
if (found_stats.parents('li').is('.work') || found_stats.parents('li').is('.bookmark') || found_stats.parents('.statistics').length) {
countable = true;
sortable = true;
//addRatioMenu();
}
...
//addRatioMenu();
                        ...
                        else if (found_stats.parents('li.work').length) {
 alert('found work li');
                                 countable = true;
//addRatioMenu();
}
...
var kudos_value = $(this).find('dd.kudos');
                                var chapter_value = $(this).find('dd.chapters');
                                var word_value = $(this).find('dd.words');
// if hits and kudos were found
if (kudos_value.length && chapter_value.length && hits_value.length && hits_value.text() !== '0') {
// get counts
var hits_count = parseInt(hits_value.text().replace(',',''));
var kudos_count = parseInt(kudos_value.text().replace(',',''));
                                        var chapter_count = parseInt((chapter_value.text().split('/'))[0]);
                                        var word_count = parseInt(word_value.text().replace(',',''));
// count percentage
var percents = 100*(kudos_count/(hits_count));
                                        var wordsPerChapter = word_count/chapter_count;
// get percentage with one decimal point
var percents_print = percents.toFixed(1);
                                        var wordsPerChapter_print = wordsPerChapter.toFixed(0);
// add ratio stats
var ratio_label = $('<dt class="kudoshits"></dt>').text('Kudos/Hits:');
var ratio_value = $('<dd class="kudoshits"></dd>').text(percents_print + '%');
                                        var wc_label = $('<dt class="wordsperchapter"></dt>').text('Words/Ch:');
                                        var wc_value = $('<dd class="wordsperchapter"></dd>').text(wordsPerChapter_print);
hits_value.after('\n', ratio_label, '\n', ratio_value, '\n', wc_label, '\n', wc_value);
if (colourbg) {
...
ratio_value.css('background-color', ratio_red);
                                                ...
                                                // colour background depending on percentage
if (wordsPerChapter>= lvl2_wc) {
wc_value.css('background-color', ratio_green);
}
else if (wordsPerChapter>= lvl1_wc) {
wc_value.css('background-color', ratio_yellow);
}
else {
wc_value.css('background-color', ratio_red);
}
}
                                       ratio_value.css('color', font_color);
                                       wc_value.css('color', font_color);
if (hide_hitcount) {
// hide hitcount label and value
$(this).find('.hits').css('display', 'none');
}
// add attribute to the blurb for sorting
$(this).parents('li').attr('kudospercent', percents);
}
else {
// add attribute to the blurb for sorting
$(this).parents('li').attr('kudospercent', 0);
}
});
}
}
