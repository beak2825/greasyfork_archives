// ==UserScript==
// @name         湖南人社-湖南农业大学课程助手
// @namespace    http://tampermonkey.net/zzzzzzys_湖南人社-湖南农业大学
// @version      1.0.0
// @copyright    zzzzzzys.All Rights Reserved.
// @description  湖南人社-湖南农业大学,全自动完成课程,自动化挂机播放，自动完成验证码
// @author       zzzzzzys
// @match        https://sypx.hunau.edu.cn/course/courseDetails/*
// @match        https://sypx.hunau.edu.cn/p/classroom/simple*
...
// @license      
// @downloadURL https://update.greasyfork.org/scripts/533537/%E6%B9%96%E5%8D%97%E4%BA%BA%E7%A4%BE-%E6%B9%96%E5%8D%97%E5%86%9C%E4%B8%9A%E5%A4%A7%E5%AD%A6%E8%AF%BE%E7%A8%8B%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/533537/%E6%B9%96%E5%8D%97%E4%BA%BA%E7%A4%BE-%E6%B9%96%E5%8D%97%E5%86%9C%E4%B8%9A%E5%A4%A7%E5%AD%A6%E8%AF%BE%E7%A8%8B%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
class Runner {
    constructor() {
        this.runner = null
    ...
    async dialogValid() {
        ...
        const data={
            code:"",
            coursewareid:obj.get("coursewareid"),
            courseid:obj.get("courseid")
        }
        for (let i = 0; i <= max; i++) {
            data.code=i
            console.log(data)
            let res=await fetch("https://sypx.hunau.edu.cn/p/check/captcha", {
                "headers": {
                    "accept": "*/*",
    ...
    initAjaxHooker() {
        ...
        ajaxHooker.hook(request => {
            if (request.url.includes('/action/get')) {
                request.response = res => {
                    const data=JSON.parse(res.responseText)
                    if(data.action==="4"){
                        console.log("准备进入请求验证！")
                        self.dialogValid().then(r => location.reload())
                    }
    ...
    run() {
        ...
        if (url.includes("classroom/simple")) {
            this.runner = new Course("channel-hunau")
            // this.runner.run()
        } else if (url.includes("courseDetails")) {
            this.runner = new Index("channel-hunau")
        }
    }
}
...
class Index {
    ...
    init() {
        ...
        try {
            let flag=GM_getValue(this.flag,0)
            if(!flag){
                flag=1
            }else {
                flag=parseInt(flag)+1
            }
            GM_setValue(this.flag,flag)
            Swal.fire({
                title: "提示",
                text: "脚本即将自动开始！",
                icon: 'info',
    ...
    async run() {
        // const cacheIndex=GM_getValue(this.flag,null)
        // if(!this.VIP ){
        //     Swal.fire({
        //         title: "当前是基础版！",
        //         text: "脚本只会自动连播两个课程中的所有视频！如需完全自动化请升级高级版！",
        //         icon: 'info',
        //         confirmButtonText: '确定',
        //         timerProgressBar:true,
        //         willClose: () => {
        ...
        // }
        try {
            const pageSelector='.el-pager li'
            const catalogSelector = '.mozs_list li'
            const btnSelector='.qd_but'
            let pageList=await this.getStudyNode(pageSelector, 'nodeList',undefined,5000)
            if(!pageList){
            ...
            for (let i = 0; i < pageList.length; i++) {
                ...
                let index=0
                // await sleep(500)
                let catalogList = await this.getStudyNode(catalogSelector, 'nodeList',undefined,3000)
                // if(!catalogList){
                //     try{
                //         document.querySelector('.years').querySelectorAll('div')[1].click()
                //     }catch (e){}
                //     catalogList=await this.getStudyNode(catalogSelector, 'nodeList',undefined,4000)
                // }
                for (let catalog of catalogList) {
                    console.log(catalog.querySelector('.mul_mc').innerText)
    ...
    checkFinish(dom) {
        return dom.querySelector('.jdt_bar').innerText.includes('100')
    }
...
class Course {
    ...
    async run() {
        try{
            document.querySelector('#checkDialogCloseBtn').click()
            await sleep(500)
        ...
        try {
            const catalogSelector = '.lcml_djj_list li'
            const catalogList = await this.getStudyNode(catalogSelector, 'nodeList')
            let index=0
            for (let catalog of catalogList) {
                console.log(catalog.querySelector('strong').innerText)
                if (this.checkFinish(catalog)) {
                    console.log("已学完，跳过！")
                    continue
                }
        ...
        return new Promise(resolve => {
            const checkInterval = setInterval(async () => {
                try {
                    ...
                    if(window.playConfig  && window.playConfig.studystatus==="03"){
                        console.log("检测到学完，准备进行下一项！")
                        clearInterval(checkInterval);
                        resolve()
                    }
                    try {
                        const breath=document.querySelector('.layui-layer-btn0')
                        if(breath){
                            breath.click()
                        }
                    }catch (e) {}
...
class Utils {
    ...
    static flag = 'hunau_VIP'
    static js_Flag = 'hunau_jsCode'
    static vipText = '高级功能已启用！(湖南人社-湖南农业大学)'
    static baseText = '您正在使用基础版本，功能可能存在限制'
    static vipSign = 'hunau_vipSign'
    ...
    static async validateCode(data) {
        try {
            let value = document.querySelector('#realNameHeader')?.innerText
            if(!value ){
                throw Error("无效的账号信息！")
            }
            data.bindInfo =  "username_" + value
            data.website = "68025754bd02209f5be0ceda"
            console.log(data);
            // return
