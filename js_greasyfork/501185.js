// ==UserScript==
// @name         Для Кураторов/Заместителей Кураторов by Archer (UPD)
// @namespace    https://forum.blackrussia.online
// @version      8.1
// @description  Для определенного круга лиц
// @author       Denny Archer, A. Flyweather
...
// @downloadURL https://update.greasyfork.org/scripts/501185/%D0%94%D0%BB%D1%8F%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%D0%97%D0%B0%D0%BC%D0%B5%D1%81%D1%82%D0%B8%D1%82%D0%B5%D0%BB%D0%B5%D0%B9%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%20by%20Archer%20%28UPD%29.user.js
// @updateURL https://update.greasyfork.org/scripts/501185/%D0%94%D0%BB%D1%8F%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%D0%97%D0%B0%D0%BC%D0%B5%D1%81%D1%82%D0%B8%D1%82%D0%B5%D0%BB%D0%B5%D0%B9%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%20by%20Archer%20%28UPD%29.meta.js
// ==/UserScript==
...
(function () {
	...
	const buttons = [
       ...
       '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
		...
		content:
       '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
            '[CENTER]*строчка*<br><br>На ваш банковский счёт были зачислены денежные средства с аккаунта - "НИК". <br>Этот аккаунт оказался вашим твинк-аккаунтом, что можно доказать с помощью системы логирования. <br>Переводы средств с одного аккаунта на другой запрещены и рассматриваются как нарушение правил проекта.<br>Нарушение пункта правил 4.05, которое устанавливает запрет на передачу денежных средств или ресурсов между основным и твинк-аккаунтом привело к блокировке аккаунта.<br>Пункт 4.05<br>[SPOILER]4.05. Запрещено передавать любые игровые ценности между игровыми аккаунтами, а также в целях удержания имущества | Ban 15 - 30 дней / PermBan<br>Пример: перекинуть бизнес, АЗС, дом или любые другие игровые материальные ценности с одного аккаунта игрока на другой / используя свой твинк / договорившись заранее с игроком и иные способы удержания.[/SPOILER]<br>[SIZE=4]Остались ли у вас вопросы касаемо данной блокировки? Ожидаю ваш ответ.',
            },
            {
        title: 'Приветствие',
        dpstyle: 'border-radius: 13px; margin-right: 5px; border: 2px solid; border-color: rgb(236, 124, 38, 0.5)',
		content:
		'[FONT=georgia][CENTER]Доброго времени суток, уважаемый(ая) {{ user.mention }}.<br><br>' +
    ...
    },
    {
        ...
        dpstyle: 'border-radius: 13px; margin-right: 5px; border: 2px solid; border-color: rgb(236, 124, 38, 0.5)',
		content:
		'[FONT=georgia]{{ greeting }}, уважаемый {{ user.name }}.<br><br>' +
		'[CENTER] Сюда текст [/CENTER]',
    },
        {
       ...
       '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
        ...
        '[CENTER][SIZE=4]Рассмотрено.[/FONT][/CENTER][/SIZE]<br>',
        prefix: WATCHED_PREFIX,
        status: false,
    },
    {
        title: 'ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠᅠ ᅠ Для умки Нади<3 ᅠ ᅠᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ᅠ ',
    ...
    },
      {
		...
		'[CENTER]Сожалеем, но доступ к аккаунту заблокирован.[/CENTER]<br><br>'+
          '[CENTER][SIZE=4]Отказано.[/FONT][/CENTER]<br><br>',
		...
		'[CENTER]Для блогеров, как публичных лиц, у нас существует отдельный регламент и своя система наказаний.<br>Все решения, касающиеся таких игроков, тщательно обсуждаются и согласовываются с отделом маркетинга.<br>Помимо этого, к блогерам частично применяются соответствующие пункты правил из общего регламента проекта.<br><br>Все наказания выносятся с точки зрения правил проекта и внутренних установок в отделах.[/CENTER]<br><br>'+
          '[CENTER][SIZE=4]Закрыто.[/FONT][/CENTER]',
		prefix: CLOSE_PREFIX,
		status: false,
	},
        {
      ...
      '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
        ...
        '[CENTER]Рекомендуем вам более детально ознакомиться с [URL= https://forum.blackrussia.online/threads/Общие-правила-серверов.312571/][COLOR=rgb(255,207,64)]правилами проекта.[/URL][/CENTER]<br>' +
	...
	},
    {
		...
		'[CENTER][SIZE=4]Передано руководству[/CENTER][/FONT][/SIZE]',
		prefix: COMMAND_PREFIXX,
		status: true,
	},
    {
        ...
        '[CENTER][SIZE=4]Остались ли у вас вопросы касаемо данной блокировки? Ожидаю ваш ответ.[/CENTER][/FONT][/SIZE]',
		prefix: PIN_PREFIX,
		status: true,
	},
        {
      ...
      '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
        ...
        {
            title: 'купил ИВ 2.28 через трейд',
            dpstyle: 'border-radius: 13px; margin-right: 5px; border: 2px solid; border-color: rgb(236, 124, 38, 0.5)',
            content:
            '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
            '[CENTER]На ваш счёт в системе трейда поступила N-ая сумма от игрока, который продает игровую валюту. До этого момента вы не взаимодействовали; договорились встретиться вне игры, что нельзя подтвердить. Встретились, получили деньги, что является нарушением правил. Никаких обсуждений в игре не было, поэтому можно предположить, что произошла покупка игровой валюты.[/CENTER]<br><br>'+
            '[CENTER]Пункт 2.28[/CENTER]<br><br>'+
            '[CENTER][SPOILER][/CENTER]<br>'+
            '[CENTER]2.28. Запрещена покупка/продажа внутриигровой валюты в любых ее проявлениях за реальные деньги | PermBan с обнулением аккаунта + ЧС проекта[/CENTER]<br>'+
            '[CENTER]Примечание: любые попытки покупки/продажи, попытки поинтересоваться о ней у другого игрока и прочее - наказуемы.[/CENTER]<br>'+
            '[CENTER]Примечание: также запрещен обмен доната на игровые ценности и наоборот;[/CENTER]<br>'+
            '[CENTER]Пример: пополнение донат счет любого игрока взамен на игровые ценности;[/CENTER]<br>'+
            '[CENTER]Исключение: официальная покупка через сайт.[/CENTER]<br>'+
            '[CENTER][/SPOILER][/CENTER]<br><br>'+
            '[CENTER][SIZE=4]Вы согласны с наказанием? Ожидаю ваш ответ.[/CENTER][/FONT][/SIZE]',
		prefix: PIN_PREFIX,
		status: true,
     },
        {
            ...
            '[CENTER][SIZE=4]Ваша жалоба принята к рассмотрению.[/CENTER][/FONT][/SIZE]',
		prefix: PIN_PREFIX,
		status: true,
     },
                {
    ...
    },
    {
       ...
       '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
        '[CENTER]К сожалению, обнаружена чужая привязка к вашему аккаунту, что может представлять угрозу для безопасности данных. В данной ситуации нам, к сожалению, не удастся разблокировать ваш аккаунт. Для обеспечения безопасности рекомендуется создать новый аккаунт и принять все возможные меры по защите данных. Наша команда технических специалистов настоятельно рекомендует вам установить дополнительные меры безопасности, такие как двухфакторную аутентификацию, сложные пароли и регулярное обновление паролей. [/CENTER] <br>' +
        '[CENTER]Пожалуйста, будьте внимательны при создании нового аккаунта и храните данные доступа в надежном месте. Мы приносим извинения за возможные неудобства, связанные с этой ситуацией, и стараемся обеспечить безопасность всех наших пользователей [/CENTER]<br><br>' +
        '[CENTER][SIZE=4]Передано руководству.[/FONT][/CENTER]',
		prefix: COMMAND_PREFIXX,
		status: true,
	},
	{
		...
		content:
       '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
            '[CENTER]*строчка*<br><br>Поступило обращение от игрока, о взломе его игрового аккаунта. После восстановления доступа к игровому аккаунту, игрок обнаружил, то-что его игровое имущество пропало. <br>После проверки системы логирования, выяснилось, что данная игровая валюта во время взлома была переведена на ваш аккаунт - *NickName*. <br>Думаете это совпадение?<br>Ваш интернет протокол (IP) абсолютно схож с (IP) взломанного игрока.<br>Игровые аккаунты обжалованию/апелляции не подлежат.<br><br>[SIZE=4]Остались ли у вас вопросы касаемо данной блокировки? Ожидаю ваш ответ.',
	},
	{
        title: 'Бан по IP',
        dpstyle: 'border-radius: 13px; margin-right: 5px; border: 2px solid; border-color: rgb(236, 124, 38, 0.5)',
        ...
        '[CENTER]Желаем приятного времяпровождения на нашем проекте.[/CENTER]<br><br>'+
        '[CENTER][SIZE=4]Рассмотрено.[/FONT][/CENTER][/SIZE]<br>',
        prefix: WATCHED_PREFIX,
        status: false,
    },
    ...
    },
    {
        ...
        '[CENTER]Приносим свои извинения за предоставленные неудобства.[/CENTER]<br><br>'+
		'[CENTER][SIZE=4]Рассмотрено.[/FONT][/CENTER][/SIZE]<br>',
		prefix: WATCHED_PREFIX,
		status: false,
     },
     ...
     },
     {
        ...
        '[CENTER]Технический специалист действовал согласно должностным инструкциям, вердикт и наказание верны. В независимости от того, хотели Вы купить-продать или просто поинтересоваться, наказание предусмотрено по пункту 2.28 правил, Выше приложил описание. Любые шутки на тему покупки/продажи игровой валюты также наказуемы.<br><br>' +
        '[CENTER]Помните, что ваше поведение в игре оказывает влияние на ее общую атмосферу и удовлетворение всех игроков.<br><br>' +
        '[CENTER][SIZE=4]Наказание остается без изменений.[/FONT][/CENTER]',
        prefix: CLOSE_PREFIX,
        status: false,
    },
        {
     ...
     },
    {
        ...
        '[CENTER]Вы нарушили правило пункта 2.21 общих правил проекта. [SPOILER="2.21"]2.21. Запрещено пытаться обходить игровую систему или использовать любые баги сервера | Ban 15 - 30 дней / PermBan (по согласованию с ГА, ЗГА, руководством тех. специалистов)[/SPOILER][/CENTER]<br><br>'+
        '[CENTER][SIZE=4]Закрыто.[/FONT][/CENTER]',
        prefix: CLOSE_PREFIX,
        status: false,
    },
    {
       ...
       '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
        '[CENTER]Технический специалист предоставил доказательства вашего нарушения. После проверки доказательств было принято решение, что наказание, выданное техническим специалистом, является верным.[/CENTER]<br>'+
        '[CENTER]Вы нарушили правило пункта 2.41 общих правил проекта. [SPOILER="2.41"]2.41. Передача своего личного игрового аккаунта третьим лицам | PermBan[/SPOILER][/CENTER]<br><br>'+
        '[CENTER][SIZE=4]Закрыто.[/FONT][/CENTER]',
        prefix: CLOSE_PREFIX,
        status: false,
    },
    {
        ...
        '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
        '[CENTER]Технический специалист предоставил доказательства вашего нарушения. После проверки доказательств было принято решение, что наказание, выданное техническим специалистом, является верным.[/CENTER]<br>'+
        '[CENTER]Вы нарушили правило пункта 4.05 общих правил проекта.[SPOILER="4.05"]4.05. Запрещено передавать любые игровые ценности между игровыми аккаунтами, а также в целях удержания имущества | Ban 15 - 30 дней / PermBan[/SPOILER][/CENTER]<br><br>'+
        '[CENTER][SIZE=4]Закрыто.[/FONT][/CENTER]',
        prefix: CLOSE_PREFIX,
        status: false,
    },
    ...
    },
	{
		...
		'[CENTER][SIZE=4]Отказано.[/FONT][/CENTER]',
		prefix: UNACCEPT_PREFIX,
		status: false,
	},
	{
	...
	},
	{
        ...
        '[CENTER]Пример: Lev_Kalashnikov | махинации<br>Форма заполнения темы:<br>[code]01. Ваш игровой никнейм:<br>02. Игровой никнейм технического специалиста:<br>03. Сервер, на котором Вы играете:<br>04. Описание ситуации (описать максимально подробно и раскрыто):<br>05. Любые скриншоты, которые могут помочь в решении проблемы (если таковые имеются):<br>06. Дата и время произошедшей технической проблемы (постарайтесь указать максимально точно):[/code][/CENTER]<br><br>' +
        '[CENTER]Примечание: все оставленные заявки обращения в данный раздел обязательно должны быть составлены по шаблону предоставленному немного выше. В ином случае, заявки обращения в данный раздел составленные не по форме — будут отклоняться. Касательно названия заголовка темы — четких правил нет, но, желательно чтобы оно содержало лишь никнейм и сервер технического специалиста. Заранее, настоятельно рекомендуем ознакомиться с [URL= https://forum.blackrussia.online/forums/%D0%98%D0%BD%D1%84%D0%BE%D1%80%D0%BC%D0%B0%D1%86%D0%B8%D1%8F-%D0%B4%D0%BB%D1%8F-%D0%B8%D0%B3%D1%80%D0%BE%D0%BA%D0%BE%D0%B2.231/][COLOR=rgb(225,204,79)]данным разделом.[/URL][/CENTER]<br><br>' +
        '[CENTER]Ваша жалоба также может быть отказана<br><br>А) Если в содержании темы присутствует оффтоп/оскорбления.<br> Б) Если в заголовке темы отсутствует никнейм технического специалиста.<br>В) С момента выдачи наказания прошло более 14 дней.[/CENTER]<br><br>' +
        '[CENTER]Благодарим вас за обращение![/CENTER]<br><br>'+
		'[CENTER][SIZE=4]Отказано.[/FONT][/CENTER]',
		prefix: UNACCEPT_PREFIX,
		status: false,
	},
	{
        ...
        '[CENTER]Пожалуйста, обратитесь в раздел «Жалобы на администрацию» вашего сервера.<br>Форма для подачи жалобы - [URL= https://forum.blackrussia.online/index.php?threads/Правила-подачи-жалоб-на-администрацию.3429349/]Кликабельно.[/URL]<br><br>'+
		'[CENTER][SIZE=4]Отказано.[/FONT][/CENTER]',
		prefix: UNACCEPT_PREFIX,
		status: false,
    },
	...
	},
	{
    ...
    },
     {
       ...
       '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
        ...
        '[CENTER]Рекомендуем вам ознакомиться с [URL= https://forum.blackrussia.online/threads/%D0%9E%D0%B1%D1%89%D0%B8%D0%B5-%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0-%D1%81%D0%B5%D1%80%D0%B2%D0%B5%D1%80%D0%BE%D0%B2.312571/#post-25703465][COLOR=rgb(255,207,64)]правилами проекта[/URL], чтобы такого больше не повторилось. Мы очень надеемся, что данная ситуация станет для вас уроком.[/CENTER]<br>' +
        '[CENTER]К сожалению, мы не всегда сможем пойти к вам на встречу и обжаловать/амнистировать вас.[/FONT][/CENTER]<br><br>' +
        '[CENTER][SIZE=4]Рассмотрено.[/CENTER][/SIZE]<br>',
		prefix: WATCHED_PREFIX,
		status: false,
    },
    {
      ...
      '[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br><br>' +
        ...
        '[CENTER]Рекомендуем вам ознакомиться с [URL= https://forum.blackrussia.online/threads/Общие-правила-серверов.312571/][COLOR=rgb(255,207,64)]правилами проекта.[/URL], чтобы такого больше не повторилось. Мы очень надеемся, что данная ситуация станет для вас уроком.[/CENTER]<br><br>'+
        '[CENTER][SIZE=4]В обжаловании отказано.[/FONT][/CENTER]',
		prefix: UNACCEPT_PREFIX,
		status: false,
	},
     {
	...
	},
    {
        ...
        '[CENTER]Иногда рассмотрение темы может занять определенное время.[/CENTER]<br><br>'+
        '[CENTER]Ваша тема взята на рассмотрение.[/FONT][/CENTER]<br>',
		...
		title: 'Передача тестерам',
            dpstyle: 'border-radius: 13px; margin-right: 5px; border: 2px solid; border-color: rgb(236, 124, 38, 0.5)',
		content:
		'[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER]<br><br>' +
...
'[CENTER][SIZE=4]Закрыто.[/FONT][/CENTER]',
	...
	dpstyle: 'border-radius: 13px; margin-right: 5px; border: 2px solid; border-color: rgb(236, 124, 38, 0.5)',
	content:
	'[FONT=georgia][SIZE=4][CENTER]{{ greeting }}, уважаемый {{ user.name }}.[/CENTER][/SIZE]<br>' +
	'[CENTER]Проблема с которой вы столкнулись не является багом, ошибкой сервера.<br><br>[CENTER][SIZE=4]Закрыто.[/FONT][/CENTER]<br>',
	prefix: UNACCEPT_PREFIX,
	status: false,
},
        {
	...
	"[CENTER]📹 Включение продемонстрировано на видео: https://youtu.be/Wft0j69b9dk<br>[CENTER][SIZE=4]Закрыто.[/FONT][/CENTER]<br>",
	prefix: CLOSE_PREFIX,
	status: false,
},
        {
