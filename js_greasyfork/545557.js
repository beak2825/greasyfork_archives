...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/545557/copy-markdown.user.js
// @updateURL https://update.greasyfork.org/scripts/545557/copy-markdown.meta.js
// ==/UserScript==
  ...
  const routes = [
    ...
    },
    {
      ...
      getContent: async () => {
        ...
        if (url.endsWith("/")) url = url.slice(0, -1);
        if (
          json.template !== "user.show" ||
          json.data.user.uid !== url.split("/").pop()
        ) {
          console.log("[DEBUG] retry");
          const res = await fetch(location.href, {
            credentials: "include",
        ...
        console.log(json);
        const content = json.data?.user?.introduction || "复制失败";
        return content;
      },
    },
    {
