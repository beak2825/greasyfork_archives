// ==UserScript==
// @name        Youtube Always Play in Highest Quality Everywhere (DEV)
...
// @version     0.1DEV
// @license     GNU GPL v3
// @downloadURL https://update.greasyfork.org/scripts/8870/Youtube%20Always%20Play%20in%20Highest%20Quality%20Everywhere%20%28DEV%29.user.js
// @updateURL https://update.greasyfork.org/scripts/8870/Youtube%20Always%20Play%20in%20Highest%20Quality%20Everywhere%20%28DEV%29.meta.js
// ==/UserScript==

//Tested with:
// Firefox 36.0 with Greasemonkey 3.1
// Chrome 40.0 with Tampermonkey  3.10
...
if ((/^(https?\:\/\/(www\.)?youtube(-nocookie)?.com\/(v|embed|e)\/[a-zA-Z0-9+\-\_]{11})/).test(window.location.href)){
	document.addEventListener('DOMContentLoaded', function(){
		...
		var interval = setInterval(function(){
			...
			if (player != undefined){ //TODO: TIMEOUT???
...
function replaceSrc(src){
	if ((/^(https?\:\/\/(www\.)?youtube(-nocookie)?\.com\/(v|e|embed)\/[a-zA-Z0-9+\-\_]{11})/).test(src)){
		...
		if ((new RegExp("(\\?|\\&)vq=[^#^&]*(\\#|\\&)?")).test(src))
			...
			src = src.replace(/^(https?\:\/\/(www\.)?youtube(-nocookie)?\.com\/(v|e|embed)\/[a-zA-Z0-9+\-\_]{11})\??/,"$1?vq=" +defaultQuality + "&");
