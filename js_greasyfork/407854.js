// ==UserScript==
// @name         MCBBS CreditAnalysis Mobile
// @namespace    https://fang.blog.miri.site
// @version      0.3
// @description  MCBBS用户积分成分分析（手机版）
// @author       Mr_Fang
// @match        https://*.mcbbs.net/home.php?mod=space&uid=*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/407854/MCBBS%20CreditAnalysis%20Mobile.user.js
// @updateURL https://update.greasyfork.org/scripts/407854/MCBBS%20CreditAnalysis%20Mobile.meta.js
// ==/UserScript==
...
(function() {
    ...
    console.log('[L] 正在运行的MCA为Mobile版！');

    // 获取用户uid（我觉得通过页面url获取更麻烦）
    var uid = jq("span.uid").html();
    uid = uid.split(':');
    uid = uid[1];
    ...
    jq.ajax({
        ...
        success:function(body,heads,status){
            ...
            jq('ul.tjxx').append(`
<li class="cl">
	<div height="` + div_h + `" width="100%" id="mca_chart" style="padding-top: 10px;">
...
<script type="text/javascript">
	function goChart(dataArr) {
		...
		function initChart() {
			...
			canvas.width = $('#mca_chart').width() * 2;
			...
			//饼状图信息
			radius = 200; //半径       cHeight * 2 / 6 高度的2/6
			ox = canvas.width / 2; //圆心
			oy = canvas.height * (2 / 3);
			tWidth = 60; //图例宽和高
			tHeight = 20;
			posX = cMargin;
			posY = cMargin; //
