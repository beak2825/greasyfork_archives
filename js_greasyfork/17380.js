...
// @name        Starting To Bloom TM - V1.1.2
...
// @grant        all
// @downloadURL https://update.greasyfork.org/scripts/17380/Starting%20To%20Bloom%20TM%20-%20V112.user.js
// @updateURL https://update.greasyfork.org/scripts/17380/Starting%20To%20Bloom%20TM%20-%20V112.meta.js
// ==/UserScript==
	...
	$("div.player_name > a").each(function(e, player) 
	{
		...
		{ 
			if (xhr.readyState == 4)
			{
                        ...
                        {var Age = parseInt(age);
                         ...
                         if ((Last_Season_Avg_Ti < 9) && (Last_Season_Avg_Ti > 0))
                             if ((Season_Avg > 10) && (Season_Avg > 0))
                                 return "⚔⚔⚔" ;
                         if ((Age == 18)&&(rec > 1.5)&&(rec <4.0)&&(Season_Avg >24)) {return "$$$"} 
                         if ((Age == 19)&&(rec > 2.0)&&(rec <4.0)&&(Season_Avg > 16)) {return "$$$"}
                         if ((Age == 20)&&(rec > 2.0)&&(rec <4.5)&&(Season_Avg > 14)) {return "$$$"}
                         if ((Age == 21)&&(rec > 2.0)&&(rec <4.5)&&(Season_Avg > 12)) {return "$$$"} 
                         if ((Age == 22)&&(rec > 2.0)&&(rec <4.5)&&(Season_Avg > 11)) {return "$$$"} else {return "----";}
                ...
                function AVG_Check(AVG_TI,Player_Months,session) {
                ...
                $("a[player_link=" + resultado.player.player_id + "]").closest("tr").append("<td class='right align_right AVGTI' style='padding-right:5px;'><span>" + AVG_Check (( Season_TI(player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary)),SI(resultado.player.skill_index),return_wage(resultado.player.wage),24.795)/session),parseInt(resultado.player.months),session) + "</span></td>");
                
                $("a[player_link=" + resultado.player.player_id + "]").closest("tr").append("<td class='right align_right SB' style='padding-right:5px;'><span>" + return_starting_blooming(Last_Season_Avg_Ti(return_TI((arrary[id_index(resultado.player.player_id,arrary)][1]),resultado.player.skill_index,player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary))),Season_TI(player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary)),SI(resultado.player.skill_index),return_wage(resultado.player.wage),24.795),session,parseInt(resultado.player.months)),return_TI((arrary[id_index(resultado.player.player_id,arrary)][1]),resultado.player.skill_index,player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary))),resultado.player.skill_index,resultado.player.age,resultado.player.rec_sort)  + "</span></td>");
