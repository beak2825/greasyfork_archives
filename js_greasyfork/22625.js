...
// @downloadURL https://update.greasyfork.org/scripts/22625/WME%20LevelReset.user.js
// @updateURL https://update.greasyfork.org/scripts/22625/WME%20LevelReset.meta.js
// ==/UserScript==
...
function LevelReset_init() {
    ...
    var cntryDB = {
        ...
        RS:     { str_lvl:0, pri_lvl:1,min_lvl:2, maj_lvl:3,  rmp_lvl:4, fwy_lvl:4},  //--------------------------------- Russian
    };
    // Country database --------------------------------------------------------------------------------------------------------------
    ...
    function scanArea() {
        ...
        $.each(Waze.model.segments.objects, function( k, v ) {
            if (count < 150 && v.type == "segment" && onScreen(v) && v.isGeometryEditable()) {
                ...
                if (v.attributes.roadType == 6) {
                    lck_lvl = userlevel < maj_lvl+1 ? userlevel-1 : maj_lvl;
                    if (v.attributes.lockRank < lck_lvl) {
                        relockObject.maj.push(new UpdateObject(v, {lockRank: lck_lvl}));
                        foundBadlocks = true;
                        count++;
                    }
                ...
                if (v.attributes.roadType == 4) {
                    lck_lvl = userlevel < rmp_lvl+1 ? userlevel-1 : rmp_lvl;
                    if (v.attributes.lockRank < lck_lvl) {
                        relockObject.rmp.push(new UpdateObject(v, {lockRank: lck_lvl}));
                        foundBadlocks = true;
                        count++;
                    }
                ...
                if (v.attributes.roadType == 3) {
                    lck_lvl = userlevel < fwy_lvl+1 ? userlevel-1 : fwy_lvl;
                    if (v.attributes.lockRank < lck_lvl) {
                        relockObject.fwy.push(new UpdateObject(v, {lockRank: lck_lvl}));
                        foundBadlocks = true;
                        count++;
                    }
        ...
        $.each(relockObject, function( key, value ) {
            ...
            var c_lvl = eval(key+'_lvl')+1;  // No eval is not evil :)
            lck_lvl = userlevel < c_lvl ? userlevel : c_lvl;
            __keyLeft.innerHTML = readable[key].replace('#', 'Lock ' + lck_lvl);
