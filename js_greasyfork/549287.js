// ==UserScript==
// @name         CRYSTAL'S PAWS OC Highlighter
// @namespace    http://tampermonkey.net/
// @version      3.0
// @description  Highlights empty OC roles; shows pass/fail badges
// @author       Crystal
...
// @downloadURL https://update.greasyfork.org/scripts/549287/CRYSTAL%27S%20PAWS%20OC%20Highlighter.user.js
// @updateURL https://update.greasyfork.org/scripts/549287/CRYSTAL%27S%20PAWS%20OC%20Highlighter.meta.js
// ==/UserScript==

(function () {
    'use strict';
    console.log("CRYSTAL: Script initializing...");

    const CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSgiNgdobg0X3YsMmYDdkL_LbtRZ_qNToIBQWUAGa3EUWHuVok7djtoO3t7sxFC5QlpQzsnmUOQiuIM/pub?gid=1635969979&single=true&output=csv";

    let roleMinimums = null;
    let alertsBlocked = [];
    let alertsNotes = [];

    let settings = JSON.parse(localStorage.getItem("crystal_oc_settings") || "{}");
    settings = Object.assign({
        ...
        showPayouts: true // NEW: show/hide payout info
    }, settings);

    function saveSettings() {
        localStorage.setItem("crystal_oc_settings", JSON.stringify(settings));
    }
    ...
    .crystal-button {
        ...
        background: linear-gradient(to bottom, #ffb3d0, #ff6f9b); border: none; border-radius:6px; cursor:pointer;
    ...
    .crystal-button.secondary {
        background: linear-gradient(to bottom, rgba(200,200,200,0.12), rgba(120,120,120,0.12));
    ...
    #crystal-oc-panel {
        ...
        background: rgba(64,64,64,0.96); padding:8px 10px; border-radius:6px; color:#fff;
    ...
    #crystal-oc-panel summary { cursor:pointer; font-weight:700; color:#ffd5ea; padding:6px; border-radius:4px; display:block; }
    #crystal-oc-panel summary:hover { background: rgba(255,128,176,0.06); }
    #crystal-oc-panel .section-header { background: rgba(255,128,176,0.06); padding:6px; border-radius:4px; color:#ffd5ea; font-weight:700; margin-bottom:6px; }
    ...
    #crystal-oc-panel .alerts-heading { font-weight:800; color:#ffd5ea; margin-bottom:6px; }
    `;
        ...
        wrappers.forEach(wrapper => {
            ...
            roles.forEach(container => {
                ...
                if (score !== null && threshold != null) {
                    ...
                    bgColor = isPass ? "rgba(0,200,81,0.12)" : "rgba(255,68,68,0.12)";
                } else {
    ...
    function updateAlertRowColors() {
        ...
        panel.querySelectorAll(".alerts-table tbody tr").forEach(row => {
            ...
            if (roleAlert) {
                ...
                bgColor = score >= min ? "rgba(0,200,81,0.12)" : "rgba(255,68,68,0.12)";
            } else {
                bgColor = "rgba(255,68,68,0.12)";
            }
    ...
    function buildUI() {
        ...
        pawBtn.className = "crystal-button secondary";
        pawBtn.style.padding = "0 6px";
        const pawImg = document.createElement("img");
        pawImg.src = "https://raw.githubusercontent.com/Crystallized22/torn-assets/main/paw-icon.png";
        pawImg.alt = "Guidelines";
        pawImg.style.width = "18px";
        pawImg.style.height = "18px";
        pawBtn.appendChild(pawImg);
        main.appendChild(pawBtn);
        ...
        bSum.textContent = "📘 PAWS OC GUIDELINES";
        ...
        blurb.innerHTML = `
            <p style="margin:0 0 8px;">
                Join <a href="http://discord.gg/RzrH85A" target="_blank" style="color:#ff80b0; text-decoration:none;">PAWS FAMILY DISCORD #oc-info</a>
                for the latest info (prioritized OCs, blocked OCs, etc).<br>
                Questions? Contact <a href="https://www.torn.com/profiles.php?XID=2669666" target="_blank" style="color:#ff80b0; text-decoration:none;">Daniel_A</a>.
            </p>
            <img src="https://drive.google.com/thumbnail?id=1nyR7eCL7omPx-S_saBLj8ko_j1Z0rD1y&sz=w1000"
                 style="max-width:100%; height:auto; border-radius:6px; margin-bottom:10px;">
            <p><b>Help us better organize OCs:</b><br>Install the <a href="https://greasyfork.org/en/scripts/538377-torn-stats-faction-cpr-tracker" target="_blank" style="color:#ff80b0; text-decoration:none;">Faction CPR Tracker by IceBlueFire</a> to automatically fetch and pass your rates into TornStats whenever you visit the Faction Crimes page.</p>
        `;
