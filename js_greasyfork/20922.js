...
// @author          Deparsoul & onisuly & Coxxs
// @namespace       https://greasyfork.org/users/50960
...
// @downloadURL https://update.greasyfork.org/scripts/20922/Enhanced%20Steam%20Community.user.js
// @updateURL https://update.greasyfork.org/scripts/20922/Enhanced%20Steam%20Community.meta.js
// ==/UserScript==
...
if (location.href.match(/^https?:\/\/steamcommunity\.com\/(id|profiles)\/.+\/gamecards\/[0-9]+/)) {
    ...
    Profile_CraftGameBadge = function ( profileUrl, appid, series, border_color )
    {
        ...
        $J.post(submitUrl,{ appid: appid, series: series, border_color: border_color, sessionid: g_sessionID } )
            ...
            ShowAlertDialog( '', 'Error when crafting the badge' );
        });
        ...
        g_CraftModal = ShowDialog( '', $Crafter, { bExplicitDismissalOnly: true } );
...
function escEnhanceBadges() {
    ...
    $J('#batch_buy_card').click(function () {
        if (batch_state == 0) {
            ...
            function matchCard(list, hashname) {
                for (var i = 0; i < list.length; ++i) {
                    let name = list[i].name
                    name = name.replace('卡牌', 'Card')
                    if (name == hashname) {
                        return i;
                    }
                }
                // 夏促卡牌的名字对不上，直接用最后的数字匹配
