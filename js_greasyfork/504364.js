// ==UserScript==
// @name         sciencedirect+ieeexplore+tandfonline+springer跳转HFUT_按钮
// @namespace    hanzhang
// @license Apache
// @version      1.2.3
...
// @grant        GM_openInTab
// @downloadURL https://update.greasyfork.org/scripts/504364/sciencedirect%2Bieeexplore%2Btandfonline%2Bspringer%E8%B7%B3%E8%BD%ACHFUT_%E6%8C%89%E9%92%AE.user.js
// @updateURL https://update.greasyfork.org/scripts/504364/sciencedirect%2Bieeexplore%2Btandfonline%2Bspringer%E8%B7%B3%E8%BD%ACHFUT_%E6%8C%89%E9%92%AE.meta.js
// ==/UserScript==
    ...
    function addButton() {
			...
			if (targetElement && !document.getElementById("new-button")) {
				...
				newButton.innerHTML = `
					...
					<span class="button-link-text-container">
						<span class="button-link-text">跳转HFUT</span>
					</span>
				`;
                // 设置按钮的颜色样式
		...
		else if(link==html_title[1])
		{
			...
			if (targetElement && !document.getElementById("new-button")) {
				...
				newButton.type = "button";
				newButton.innerHTML = "跳转HFUT";  // 按钮的文本内容
                // 设置按钮的颜色样式
                newButton.style.backgroundColor = "#007BFF";  // 设置背景颜色为蓝色
                newButton.style.color = "white";  // 设置文字颜色为白色
                newButton.style.border = "none";  // 移除按钮边框
