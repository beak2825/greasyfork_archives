// ==UserScript==
// @name   🌱【免费版】成学课堂|收费版本使用说明：http://doc.zhanyc.cn/course/cxkt/
// @namespace    http://jb.zhanyc.cn/
// @icon    https://js.zhanyc.cn/img/js-logo.svg
// @version      1.1
// @description  当前使用的是免费版本，仅包含视频页面自动换课的功能，如需极速秒过、自动换课、答题、考试等高级功能，可通过描述页安装付费版本，一杯咖啡钱，保你无忧学习，且永久使用|接各类脚本开发工作，微信：zhanyc_cn 备用微信:zhanfengkuo 个人网站：http://doc.zhanyc.cn
// @author       zfk
...
// @antifeature 
// @license GPL
// @downloadURL https://update.greasyfork.org/scripts/502449/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E6%88%90%E5%AD%A6%E8%AF%BE%E5%A0%82%7C%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E%EF%BC%9Ahttp%3Adoczhanyccncoursecxkt.user.js
// @updateURL https://update.greasyfork.org/scripts/502449/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E6%88%90%E5%AD%A6%E8%AF%BE%E5%A0%82%7C%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E%EF%BC%9Ahttp%3Adoczhanyccncoursecxkt.meta.js
// ==/UserScript==
(function () {
    ...
    let freeTips = "当前使用的是免费版本，仅包含视频页面自动换课的功能，如需极速秒过、自动换课、答题、考试等高级功能，可点击下方按钮查看付费版本"
    let docUrl = "http://doc.zhanyc.cn/pages/cxkt/";
    let plugMain = Object.assign(baseConfig, {
        config: {
            maxComment: 100,
        },
        pageData: {
            nextIndex: -1,// 视频页面标记当前是那个视频
            userNameIndex: null,
            loadTaskProgress: false, // 答题任务进度是否已经加载完成
            mgVideoRun: false,
            mgVideoIndex: 0,
            videoPageData: null, // 视频页面数据
            isRunWork: false,// 是否在跑作业（视频完成了）
            confirmRunIndex: null,
            videoList: null,//存放课程目录
            videoFinishList: null,// 存放课程完成情况
            lastMgVideoId: null,// 上一次秒过视频的ID
            mgVideoCheckIndex: null,// 上次秒过视频检测的timeoutIndex
            waitTime: 0,
            lastRunTime: -1,
            qaArr: [],
            index: {
        ...
        async runByUrl(url) {
            if (url.toLocaleLowerCase().includes("/layout/roomdetail")) {
                plugMain.page_video();
            } else if (url.includes("/Layout/classStudy") || url.includes('/Layout/home')) {
                plugMain.showPaidContent(`免费版本不包含自动换课、换学年功能，如需使用请安装收费版本`);
            } else if (url.includes("/Layout/examList")) {
                plugMain.showPaidContent(`免费版本不包含考试功能，如需使用请安装收费版本`);
            } else if (url.includes("/homeWork?")) {
                plugMain.showPaidContent(`免费版本不包含作业答题功能，如需使用请安装收费版本`);
            }
        },

        async page_video_work() {
            console.log("%c page_video_work", "background:rgb(0,0,0);color:#fff");
            plugMain.pageData.lastRunTime = plugMain.now();
            let config = plugMain.getConfig();
            if (config.czms == "2") return;
            await plugMain.gmAuthDownScore("9099284436654264bb5f1ac99baeb3c1");
            plugMain.tipsMsg("等待脚本执行")
            await plugMain.waitOf(a => plugMain.pageData.loadTaskProgress)
            await plugMain.waitTimeout(1000)
            let $el = null;
            let userData = await plugMain.getUserData()
            let courseId = plugMain.parseQueryString().courseId
            $(".chapter-item .fa-book").each((i, el) => {
                let $parent = $(el).parents(".chapter-item:first")
                if ($parent.find(".pass").length == 0) {
                    let has = userData.skipWordArr.find(a => a.courseId == courseId)
                    if (has) {
                        let title = $parent.text().trim()
                        if (has.skipList.includes(title)) {
                            plugMain.tipsMsg(`跳过作业：${title}`)
                            return true
                        }
                    }
                    $el = $(el).parents(".chapter-item:first");
                    return false;
                }
            });

            if ($el == null) {
                await plugMain.waitOf(a => plugMain.pageData.mgVideoRun == false, 1000, -1)
                isDev && console.log('等待秒过视频完成');
                plugMain.confirmRun("当前课程已经完成， 准备返回列表页面").then((a) => {
                    plugMain.getElByText($(".el-menu-item"), "我的课程").click();
                });
                return;
            }
            $el.click();
        },
        async page_video(callback = false) {
            console.log("%c page_video", "background:rgb(0,0,0);color:#fff");
            plugMain.pageData.isRunWork = false
            plugMain.pageData.lastRunTime = plugMain.now();

            clearInterval(plugMain.pageData.index.refreshIndex);
            // 5分钟没运行则刷新页面
            plugMain.pageData.index.refreshIndex = setTimeout(() => {
                if (plugMain.now() - plugMain.pageData.lastRunTime > 5 * 60 * 1000)
                    location.reload();
            }, 5 * 60 * 1000);
            if (!callback) {
                await plugMain.waitOf((a) => $(".fa-volume-up").length > 0);
                await plugMain.waitTimeout(2000);
            }
            await plugMain.waitOf(a => plugMain.pageData.videoList.length != null)

            let $el = null;
            $(".fa-volume-up").each((i, el) => {
                if ($(el).parents(".list-item:first").find(".finish").length == 0 && plugMain.pageData.nextIndex < i) {
                    $el = $(el).parents(".chapter-item:first");
                    plugMain.pageData.nextIndex = i
                    return false;
                }
            });
            if ($el == null) {
                plugMain.page_video_work();
                plugMain.pageData.isRunWork = true
                return;
            }
            $el.click();
            let timeout = 2;
            ...
            if (mg != "1") {
                layer.msg(
                    "脚本支持视频秒过，如需秒过请在右上角油猴脚本的【脚本菜单】中设置秒过后刷新页面生效"
                );
            } else {
                return
            }
            plugMain.pageData.video.index = setInterval(async () => {
                try {
                    ...
                    if (!location.href.toLocaleLowerCase().includes("/layout/roomdetail")) {
                        return;
                    }
