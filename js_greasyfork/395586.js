...
// @downloadURL https://update.greasyfork.org/scripts/395586/Steam%20Community%20-%20Group%20Members%20Kicker.user.js
// @updateURL https://update.greasyfork.org/scripts/395586/Steam%20Community%20-%20Group%20Members%20Kicker.meta.js
// ==/UserScript==
...
function getMembers(p) {
    $.get(g_strGroupURL + "/memberslistxml/?xml=1&p=" + p, function(xml) {
        ...
        console.log(xmlmembers);
        xmlmembers.forEach(function(member) {
            var steamid = member.innerHTML;
            if (steamid != g_steamID && !undefined) {console.log(steamid); members.push(steamid);}
        });
        modal.Dismiss();
        modal = ShowBlockingWaitDialog("Executing...", "Gathered " + members.length + "/" + total + " steamID's of " + g_strGroupName + " members.");
