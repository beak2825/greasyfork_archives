// ==UserScript==
// @name       Hybrid - Check Yahoo Finance
// @author		jawz
// @version    1.0
...
// @downloadURL https://update.greasyfork.org/scripts/23737/Hybrid%20-%20Check%20Yahoo%20Finance.user.js
// @updateURL https://update.greasyfork.org/scripts/23737/Hybrid%20-%20Check%20Yahoo%20Finance.meta.js
// ==/UserScript==

if ($('li:contains("Check Yahoo Finance")').length) {
...
if (document.URL.indexOf("finance.yahoo.com/quote/") > 0) {
    setTimeout(function(){
        $('span:contains("Quarterly")').click();

        var a,b,c,d,e,f,g,h,i,j;
        a = $('td:contains("Revenue")').eq(0).next().text();
        b = $('td:contains("Revenue")').eq(0).next().next().text();
        c = $('td:contains("Revenue")').eq(0).next().next().next().text();
        d = $('td:contains("Revenue")').eq(0).next().next().next().next().text();
        e = $('td:contains("Total Revenue")').eq(0).next().text();
        f = $('td:contains("Total Revenue")').eq(0).next().next().text();
        g = $('td:contains("Total Revenue")').eq(0).next().next().next().text();
        h = $('td:contains("Total Revenue")').eq(0).next().next().next().next().text();
