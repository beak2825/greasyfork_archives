...
// @grant          GM_addStyle
// @grant          GM_log
// @downloadURL https://update.greasyfork.org/scripts/440630/Estiah%20-%20Mind%20Tricks.user.js
// @updateURL https://update.greasyfork.org/scripts/440630/Estiah%20-%20Mind%20Tricks.meta.js
// ==/UserScript==
...
function List(init) {
  if (init == undefined || init.length < 2) this.array = new Array();
  else this.array = init.split(this.separator);
}
...
function Map(init, mainSeparator, semiSeparator) {
  ...
  if (mainSeparator != undefined) this.mainSeparator = mainSeparator;
  if (semiSeparator != undefined) this.semiSeparator = semiSeparator;
  if (init != undefined) {
...
Map.prototype.insert = function (val1, val2, val3, val4) {
  ...
  if ((val3 == undefined || val3 === "") && (val4 == undefined || val4 === ""))
    val = [val2];
  else if (val4 == undefined || val4 === "") val = [val2, val3];
  else val = [val2, val3, val4];
  if (this.container[val1] == val) return false;
...
Map.prototype.add = function (val1, val2) {
  ...
  if (this.container[val1].indexOf(val2) > -1) return false; // already there
...
Map.prototype.toString = function () {
  ...
  for (var key in this.container) {
    res.push(
      key + this.semiSeparator + this.container[key].join(this.semiSeparator)
    );
  }
...
Map.prototype.foreach = function (callback) {
  for (var key in this.container) callback(key, this.container[key]);
};
...
function GMList(gmName, autoupdate, clear) {
  ...
  if (clear != undefined && clear) this.list = new List();
  else this.list = new List(Storage.get(gmName));
}

GMList.prototype.add = function (val) {
  var added = this.list.add(val);
  if (added && this.autoupdate) this.save();
  return added;
};
...
GMList.prototype.remove = function (val) {
  ...
  if (removed && this.autoupdate) this.save();
  return removed;
};
...
function GMMap(gmName, autoupdate, clear) {
  this.name = gmName;
  this.autoupdate = autoupdate != undefined && autoupdate;
  if (clear != undefined && clear) this.map = new Map(undefined);
  else this.map = new Map(Storage.get(gmName));
}
  ...
  GM_xmlhttpRequest({
    ...
    onload: function (responseDetails) {
      ...
      var responseXML = parser.parseFromString(
        Web.SanitizeResponse(responseDetails.responseText),
        "text/xml"
      );
      if (debug) {
        ...
        GM_log(
          'url "' +
            url +
            '" loaded with ' +
            responseDetails.responseText.length +
            ' characters, title "' +
            title.innerHTML +
            '"'
        );
...
Web.SanitizeResponse = function (text) {
  return text
    ...
    .replace(
      new RegExp("<strong>([0-9]+)</span> </strong>", "g"),
      "<strong>$1</strong> </span>"
    ); // switch </span and </strong>
};

Web.SwitchVisibility = function (showClass, hide) {
  var show = hide.parentNode.getElementsByClassName(showClass);
  for (var d = 0; d < show.length; d++) show[d].style.display = "";
...
Web.CreatePageTitle = function (title, desc, wireFrame, page) {
  var titleFrame = Web.CreateElement(
    "div",
    "wireframe_" + wireFrame + " common_content common_wl",
    page
  );
  ...
  if (desc != undefined) Web.appendText(desc, descEl);
  return titleFrame;
};
...
Web.FindTagAfter = function (element, tagname) {
  ...
  while (el != undefined && el.tagName != tagname) el = el.nextSibling;
  return el;
};
...
Web.SanitizeXpathName = function (inp) {
  if (inp.indexOf('"') > -1) {
    ...
    for (var i = 0; i < inParts.length; i++) {
      if (inParts[i] != "") outParts.push('"' + inParts[i] + '"');
      if (i < inParts.length - 1) outParts.push("'" + '"' + "'");
    }
...
Estiah.GetPlayerLevel = function () {
  ...
  if (level.length > 0) return parseInt(level[0].innerHTML);
  return 1;
};
...
Estiah.IsItemShopUrl = function (url) {
  if (!Estiah.IsShopUrl(url)) return false;
...
Estiah.IsCraftShopUrl = function (url) {
  ...
  return (
    shop[0] == "4" || shop[0] == "5" || shop[0] == "6" || shop == "tracked"
  );
};
...
Estiah.Market.AddShop = function (
  shopId,
  className,
  shopName,
  shopDesc,
...
Estiah.Fetcher.SynchronizeCallbacks = function (
  syncKey,
  callback,
  func1,
  func2
...
Estiah.Fetcher._checkFinished = function (syncKey, callback) {
  if (
    Estiah.Fetcher[syncKey + "_data1"] != undefined &&
    Estiah.Fetcher[syncKey + "_data2"] != undefined
  )
    callback(
      Estiah.Fetcher[syncKey + "_data1"],
      Estiah.Fetcher[syncKey + "_data2"]
    );
};

Estiah.Fetcher.FetchItemAndCharmCount = function (callback) {
  var key = "_ItemCountAndCharmCount";
  Estiah.Fetcher._cache(key, callback, function (innerCallback) {
...
Estiah.Inventory.UrlTest = function (url) {
  return (
    Launcher.UrlTest.EndsWith("/character/inventory")(url) ||
...
Estiah.Inventory._getItemCounts = function (doc) {
  ...
  var items = doc.evaluate(
    '//*[starts-with(@id,"StorageItemCount")]',
    doc,
    null,
  ...
  for (var i = 0; i < items.snapshotLength; i++)
    map[items.snapshotItem(i).id.substring(16)] = parseInt(
      items.snapshotItem(i).innerHTML
    );

  items = doc.evaluate(
    '//*[starts-with(@id,"InventoryItemCount")]',
    doc,
    null,
    XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
    null
  ...
  for (var i = 0; i < items.snapshotLength; i++) {
    ...
    else map[id] = parseInt(items.snapshotItem(i).innerHTML);
  }
  return map;
};
...
Estiah.Charms.FetchCharmCounts = function (callback) {
  Estiah.Fetcher.SynchronizeCallbacks(
    "_CharmCount",
    function (map1, map2) {
      for (var i in map2) map1[i] = map2[i];
      callback(map1);
    },
    Estiah.Charms._fetchCharmCounts("http://www.estiah.com/character/card"),
    Estiah.Charms._fetchCharmCounts(
      "http://www.estiah.com/character/card/index/collection/archive"
    )
  );
};
...
DataUpdate.RemoveEmptyValues = function () {
  ...
  Storage.foreach(function (sKey) {
    ...
    map.foreach(function (key, valueArray) {
      ...
      if (valueArray.length == 0) map.remove(key);
    });
...
Settings.SettingsShopDesc =
  "Young man is showing off wonderful mind tricks. Maybe you can learn few of them too ..";
Settings.ImportShopName = "Soul Harvester's Tent";
Settings.ImportShopDesc =
  "This strange man can store your memory into a crystal or back. Be aware, it hurts ..";
...
Settings.AlterMarket = function (doc) {
  ...
  for (var i = 0; i < shopRoots.snapshotLength; i++) {
    ...
    var shopGroup = Estiah.Market.AddShopGroup(
      "staff",
      "Dark corner",
      shopRoot
    );
    Estiah.Market.AddShop(
      "settings",
      "item",
      Settings.SettingsShopName,
      Settings.SettingsShopDesc,
  ...
  cityShops.setAttribute(
    "class",
    cityShops.getAttribute("class") + " CityShops"
  );
};
...
Settings.CreateSettingsPage = function () {
  var settingsPage = Web.CreatePage(
    Settings.SettingsShopName,
    Settings.SettingsShopDesc,
    "market",
    "Settings",
  ...
  if (Storage.canUseLS && Storage.canUseGM) {
    ao = Web.CreateElement(
      "div",
      "wireframe_market common_content common_wl",
      settingsPage
    );
    ...
    var aoUseGM = Web.AddButton(
      "Use greasemonkeyStorage",
      href,
      3,
      Settings.UseGM,
      ao
    ...
    Web.appendText(
      "Here you can change where the data is stored. It may take a while to transfer all the data. Do not click if you do not know what you are doing.",
      aoDesc
    );
  }
  Settings.texts.foreach(function (addonName, addonDesc) {
    ...
    if (Settings.IsEnabled(addonName)) aoEnable.style.display = "none";
    else aoDisable.style.display = "none";
    var aoDesc = Web.CreateElement("div", "paragraph_text", ao);
...
Settings.CreateImportPage = function () {
  ...
  Web.CreateElement("option", undefined, select).appendChild(
    document.createTextNode("--Everything--")
  );
  Web.CreateElement("option", undefined, select).appendChild(
    document.createTextNode("--Static data--")
  );
  Web.CreateElement("option", undefined, select).appendChild(
    document.createTextNode("--Personal data--")
  );
...
Settings.Export = function () {
  ...
  switch (select.value) {
    case "--Everything--":
      Settings.ExportVariables(container, function () {
        return true;
      });
      break;
    case "--Static data--":
      ...
      break;
    case "--Personal data--":
      Settings.ExportVariables(container, function (name) {
        return !Settings.IsStaticVariableName(name);
      });
      break;
    default:
...
Settings.ExportVariables = function (container, include) {
  Storage.foreach(function (key) {
    ...
    if (include(key)) container.insert(key, Storage.get(key));
  });
};
...
Settings.Import = function () {
  ...
  if (containerVersion == undefined) {
    if (
      !confirm(
        "You are trying to import data from an unknown version. Are you sure you do want to continue?"
      )
    )
      return;
  ...
  if (containerVersion < dataVersion) {
    ...
    if (
      !confirm(
        "You are trying to import data from version " +
          containerVersion +
          " but you have version " +
          dataVersion +
          ". There probably were some updates in data structure in the meantime. Do you want to try your luck and import it anyway?"
      )
    )
      return;
  ...
  if (containerVersion > dataVersion) {
    alert(
      "Imported data are from newer version of this script. Please update it first."
    );
    return;
  }
...
ItemAvailability.GetCitiesSellingItem = function (itemId) {
  ...
  for (var shop = 0; shop < shops.length; shop++) {
    ...
    for (var cKey = 0; cKey < cityIds.length; cKey++) {
      ...
      if (this.cities.indexOf(cityId) == -1) this.cities.push(cityId); // avoid duplicities
...
ItemAvailability.AlterCraftShops = function (root) {
  var shops = root.getElementsByClassName(
    "wireframe_craft common_content common_wl"
  );
  if (shops.lenght == 0) ItemAvailability.AlterCraftShop(root);
  else
    for (var i = 0; i < shops.length; i++) {
      var shopCities = shops[i].getElementsByClassName("shop-city");
      var shopCity =
        shopCities.length > 0 ? shopCities[0].innerHTML : undefined;
...
ItemAvailability.AlterCraftShop = function (root, shopCity) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (cities.length > 0) {
      ...
      items[i].parentNode.insertBefore(
        insertPlaceholder,
        Web.FindBrAfter(items[i])
      );
      ...
      for (var cityIt = 0; cityIt < cities.length; cityIt++) {
        ...
        } else outerSpan.appendChild(document.createElement("br"));
...
Launcher.Register(
  "ItemAvailability",
  "Shows in which cities you can buy an item."
)
  .Read(Estiah.IsItemShopUrl, function (doc, url) {
    ItemAvailability.LoadItemShop(doc, Estiah.GetShopIdFromUrl(url));
  })
  .Modify(Estiah.IsCraftShopUrl, ItemAvailability.AlterCraftShops)
  .Modify(Launcher.UrlTest.EndsWith("/guild"), ItemAvailability.AlterCraftShop)
  .Modify(Estiah.Inventory.UrlTest, ItemAvailability.AlterInventory);
...
CraftHelper.UpdateCharmAndItemCounts = function (root, doneCallback) {
  Estiah.Fetcher.FetchItemAndCharmCount(function (itemCounts, charmCounts) {
    ...
    for (var key = 0; key < items.length; key++) {
      ...
      var toUpdate = item.parentNode.getElementsByClassName(
        "PT_update_count_t0i" + itemId
      );
...
CraftHelper.LoadVendorShop = function (doc, shopId) {
  // 3
  CraftHelper.LoadCharmShop(doc, shopId, "cardframe_lr", 9);
};

CraftHelper.LoadCraftShop = function (doc, shopId) {
  // 4, 6
...
CraftHelper.AlterMarket = function (doc) {
  ...
  backButton.setAttribute(
    "class",
    backButton.getAttribute("class") + " decision"
  );
  Web.AddButton(
    "Show all shops",
    "/market/all",
    5,
    CraftHelper.ShowAllShops,
...
CraftHelper.CreateAllShopsPage = function () {
  var allShops = Web.CreatePage(
    "All shops",
    "Shops from all visited cities",
    "market",
    "AllShops",
    "wireframe_2"
  );

  var shopsFrame = Web.CreateElement(
    "div",
    "wireframe_market common_content common_wl",
    allShops
  );
  ...
  var tracking = Estiah.Market.AddShopGroup(
    "bread",
    "Interesting Stuff",
    shopRoot
  );
  ...
  CraftHelper.GetShopNames().foreach(function (shopId, info) {
    switch (shopId[0]) {
      case "3":
        if (vendor == null)
          vendor = Estiah.Market.AddShopGroup(
            "staff",
            "Vendor Shops",
            shopRoot
          );
  ...
  Web.AddButton(
    ...
    "/market",
    5,
    CraftHelper.ShowCityShops,
    buttonsRoot
  );
};

CraftHelper.AddShop = function (shopId, className, shopName, shopDesc, root) {
  var shop = Estiah.Market.AddShop(
    shopId,
    className,
    shopName,
    shopDesc,
    CraftHelper.ShowShop,
    root
  );
...
CraftHelper.AddTrackingShop = function (root) {
  Estiah.Market.AddShop(
    ...
    "item",
    "Tracked Charms",
    "Charms you have had marked for tracking",
    CraftHelper.CreateAndShowTracked,
    root
  );
};
...
CraftHelper.CreateAndShowTracked = function () {
  ...
  CraftHelper.getCharmTracker().foreach(function (shopId, charmIds) {
    ...
    var title = Web.CreatePageTitle(
      shopDesc[0] + CraftHelper.GetCitiesWithShopHtml(shopId),
      shopDesc[1],
      "craft",
      page
    );
...
CraftHelper.CreateCraftShopPage = function (shopId, shopName, shopDesc) {
  ...
  var shop = new CraftHelper.CharmShop(shopId);
  var page = Web.CreatePage(
    shopName + CraftHelper.GetCitiesWithShopHtml(shopId),
    shopDesc,
    "craft",
    "Shop" + shopId,
    "wireframe_2"
  );
  var crafts = Web.CreateElement(
    "div",
    "craftlist format log",
    page.firstChild
  );
  var craftSection;
  var i = 0;
  shop.foreach(function (charmId, charmHTML) {
    if (charmHTML == undefined) return;
    if (i++ % 3 == 0)
      craftSection = Web.CreateElement("div", "section_text", crafts);
    var div = Web.CreateElement("div", "craft", craftSection);
    div.id = "Craft" + charmId;
...
CraftHelper.CreateVendorShopPage = function (shopId, shopName, shopDesc) {
  ...
  var items = Web.CreateElement(
    "div",
    "inventory shop format log",
    page.firstChild
  );
  var goodsSection = Web.CreateElement("div", "goodslist c2", items);
  var i = 0;
  shop.foreach(function (charmId, charmHTML) {
    var div = Web.CreateElement(
      "div",
      "cardframe_lr cardbg_lr_common",
      goodsSection
    );
    div.id = "ShopGoods" + charmId;
    div.innerHTML = charmHTML;
    var buys = div.getElementsByClassName("BV_shop_buy");
    for (var buy = 0; buy < buys.length; buy++) Web.removeNode(buys[buy]);
  });
...
CraftHelper.AddTrackButton = function (craft, shopId) {
  var functions = craft.getElementsByClassName("functions");
  var functs = null;
  if (functions.length > 0) functs = functions[0];
  else {
  ...
  var track = Web.AddInnerLink(
    "[Track]",
    href,
    "nolink track",
    CraftHelper.TrackCharm,
    functs
  );
  var untrack = Web.AddInnerLink(
    "[Untrack]",
    href,
    "nolink untrack",
    CraftHelper.UntrackCharm,
    functs
  );
  if (CraftHelper.getCharmTracker().contains(shopId, craftId))
    track.style.display = "none";
  else untrack.style.display = "none";
};
...
CraftHelper.UntrackCharm = function () {
  var ids = this.href.split("/");
  CraftHelper.getCharmTracker().untrack(
    ids[ids.length - 2],
    ids[ids.length - 1]
  );
...
CraftHelper.ReadShopNamesFromCity = function (doc) {
  ...
  for (var i = 0; i < eventShops.snapshotLength; i++) {
    ...
    var names = eventShops
      .snapshotItem(i)
      .parentNode.parentNode.getElementsByClassName("lhp");
...
CraftHelper.ReadShopNamesFromMarket = function (doc) {
  ...
  for (var i = 0; i < Shops.snapshotLength; i++) {
    ...
    // no item shops
    if (shopId[0] == "1" || shopId[0] == "2") continue;
    var descriptions =
      Shops.snapshotItem(i).parentNode.parentNode.getElementsByClassName(
        "description"
      );
    for (var desc = 0; desc < descriptions.length; desc++)
      CraftHelper.GetShopNames().addShop(
        shopId,
        Shops.snapshotItem(i).innerHTML,
        descriptions[desc].innerHTML
      );
  }
};
...
CraftHelper.AlterInventory = function (doc) {
  Estiah.Charms.FetchCharmCounts(function (charmCounts) {
    ...
    CraftHelper.GetShopNames().foreach(function (shopId, info) {
      ...
      shop.foreach(function (craftId, craftHTML) {
        ...
        for (var i = 0; i < ingredients.length; i++) {
          ...
          var itemCount = Web.FindTagAfter(
            ingredients[i],
            "SPAN"
          ).innerHTML.substr(1);
          ...
          if (charmId == undefined) itemUsageI.add(itemId, craftId);
          else itemUsageCh.add(itemId, craftId);
        }
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (usageCh.length == 0 && usageI.length == 0) continue;
      ...
      if (usageCh.length > 0) {
        for (var j = 0; j < usageCh.length; j++) {
          ...
          CraftHelper.AddInventoryTooltipItem(
            desc,
            itemId,
            craftId,
            receipts[craftId],
      ...
      if (usageI.length > 0) {
        if (usageCh.length > 0) desc.appendChild(document.createElement("br"));
        for (var j = 0; j < usageI.length; j++) {
          ...
          if (resultItemCount == undefined) resultItemCount = 0;
          var craftDesc = Web.CreateElement("span", "craft", desc);
          Web.appendText("x" + itemCount + " in ", craftDesc);
          var color = CraftHelper.getCharmTracker().contains(
            receipts[craftId].shopId,
            craftId
          )
            ? "iepic"
            : "c2";
          Web.appendText(
            craftName,
            Web.CreateElement("span", color, craftDesc)
          );
          ...
          if (resultInCharms.length > 0) {
            ...
            for (var k = 0; k < resultInCharms.length; k++) {
              ...
              needed |= CraftHelper.AddInventoryTooltipItem(
                desc,
                resultItemId,
                chCraftId,
                receipts[chCraftId],
          ...
          if (!needed) craftDesc.style.opacity = 0.5;
        }
      }
    }
  });
...
CraftHelper.AddInventoryTooltipItem = function (
  desc,
  itemId,
  craftId,
  receipt,
  ...
  var craftDesc = Web.CreateElement("span", "craft", desc);
  if (prefix != undefined) Web.appendText(prefix, craftDesc);
  Web.appendText("x" + itemCount + " in ", craftDesc);
  var color = CraftHelper.getCharmTracker().contains(receipt.shopId, craftId)
    ? "iepic"
    : "c2";
...
Launcher.Register(
  "CraftHelper",
  "Remembers all charms you could want to craft. Also allows to track interesting ones."
)
  .Read(Estiah.IsVendorShopUrl, function (doc, url) {
    CraftHelper.LoadVendorShop(doc, Estiah.GetShopIdFromUrl(url));
  })
  .Read(Estiah.IsCraftShopUrl, function (doc, url) {
    CraftHelper.LoadCraftShop(doc, Estiah.GetShopIdFromUrl(url));
  })
  .Modify(Estiah.IsCraftShopUrl, CraftHelper.AlterCraftShop)
  .Read(Launcher.UrlTest.EndsWith("/city"), CraftHelper.ReadShopNamesFromCity)
  .Read(
    Launcher.UrlTest.EndsWith("/market"),
    CraftHelper.ReadShopNamesFromMarket
  )
  .Modify(Launcher.UrlTest.EndsWith("/market"), CraftHelper.AlterMarket)
  .Modify(Estiah.Inventory.UrlTest, CraftHelper.AlterInventory);
...
CityLoader.LoadCity = function (doc) {
  ...
  for (var i = 0; i < eventShops.snapshotLength; i++) {
    Web.HttpRequest(
      eventShops.snapshotItem(i).getAttribute("href"),
      function () {}
    );
  }
  ...
  Web.HttpRequest("http://www.estiah.com/market", function (response) {
    ...
    for (var i = 0; i < Shops.snapshotLength; i++) {
      Web.HttpRequest(
        Shops.snapshotItem(i).getAttribute("href"),
        function () {}
      );
    }
  });
...
).Read(Launcher.UrlTest.EndsWith("/city"), CityLoader.LoadCity);
...
Launcher.Register(
  "AutoSelectPet",
  "Preselects your first pet for a race / fight."
)
  .Modify(Launcher.UrlTest.EndsWith("/city/race"), AutoSelectPet.Race)
  .Modify(Launcher.UrlTest.EndsWith("/city/fight"), AutoSelectPet.Fight);
  ...
  for (var i = 0; i < cards.length; i++) {
    ...
    if (!NewCharms.SeenCardsInstance.Contains(cardId)) {
      if (placeholder == null) {
        ...
        Web.AddAsFirstChild(
          placeholder,
          doc.getElementsByClassName("z2 format")[0]
        );

        Web.appendText(
          "New Charms",
          Web.CreateElement("div", "title", placeholder)
        );

        var info = Web.CreateElement("div", "info highlight", placeholder);
        Web.appendText(
          "You can use new charms!",
          Web.CreateElement("div", "total", info)
        );
      ...
      Web.AddInnerLink(
        ...
        "/card/" + cardId,
        "func_tier",
        Web.CreateMouseClickFunction(plusA),
        cardDiv
      );
...
Launcher.Register(
  "NewCharms",
  "Point out that there are new charms you can use."
).Modify(Launcher.UrlTest.Contains("/character/deck"), NewCharms.AlterDeckPage);
...
PenetratedDmg.PenetratedDmg = function (root, type) {
  ...
  for (var i = 0; i < descs.length; i++) {
    ...
    if (dmg == undefined) {
      ...
      if (descs[i].nextSibling != null) {
        ...
        if (timesText != undefined) times = parseInt(timesText);
      }
    }
    var penetration = String.getTextBetween(str, "(", "%");
    if (dmg == undefined || penetration == undefined) continue;
    penetratedDmg +=
      Math.floor((parseInt(dmg) * parseInt(penetration)) / 100) * times;
  ...
  if (penetratedDmg > 0) {
    ...
    Web.CreateElement("br", undefined, root);
    Web.appendText(
      "Penetrated " + type + " " + penetratedDmg,
      Web.CreateElement("span", type, root)
    );
  }
};
...
Launcher.Register(
  "PenetratedDmg",
  "Did you ever wondered how many damage will this charm do to heavely armored opponent?"
).Modify(
...
Launcher.Register(
  "HideUsedReceipts",
  "Hides receipts of charms you have already five of."
)
  .Modify(Estiah.IsCraftShopUrl, HideUsedReceipts.AlterShopPage)
  .Modify(
...
HideLowLevelBounties.AlterPvPPage = function (doc) {
  ...
  moblist.addEventListener(
    "DOMNodeInserted",
    HideLowLevelBounties.SomethingInserted,
    false
  );
};
...
HideLowLevelBounties.SomethingInserted = function (event) {
  ...
  if (cl == undefined || cl.indexOf("mob") == -1) return;
  HideLowLevelBounties.AlterPlayer(event.target);
};

HideLowLevelBounties.AlterPlayer = function (player) {
  if (HideLowLevelBounties.TypeSelect.value != "bounty") return;
  ...
  if (Estiah.Player.Level - level > 1) player.style.opacity = 0.3;
};

Launcher.Register(
  "HideLowLevelBounties",
  "Hides players among players with bounty with lower level than yours -1."
).Modify(Launcher.UrlTest.EndsWith("pvp"), HideLowLevelBounties.AlterPvPPage);
...
).Modify(Estiah.Inventory.UrlTest, ItemsCounter.AlterInventoryPage);
...
StorageItemsWatcher.setLastChecked = function () {
  Storage.set(
    "ExpiringItems_LastChecked_" + Estiah.Player.Id,
...
StorageItemsWatcher.CheckAndAlterPage = function (doc) {
  ...
  if (new Date().getTime() - lastChecked > 1000 * 60 * 30 /* 30 minutes */) {
    Web.HttpRequest(
      "http://www.estiah.com/character/inventory",
      function (inv) {
        StorageItemsWatcher.Update(inv);
        StorageItemsWatcher.AlterPage(doc);
      }
    );
  } else StorageItemsWatcher.AlterPage(doc);
};

StorageItemsWatcher.AlterPage = function (doc) {
  var storedItems = StorageItemsWatcher.getItems();
  var count = 0;
  for (k in storedItems.map.container) count++;
  if (count > 0) {
    ...
    Web.CreateElement("a", "nolink disabled", entry).innerHTML =
      "Expiring (" + count + ")";
    ...
    storedItems.foreach(function (name, info) {
      var row = Web.CreateElement(
        "div",
        "left",
        Web.CreateElement("div", "row", items)
      );
      Web.CreateElement("span", info[0], row).innerHTML = name;
      Web.appendText(" (" + info[1] + ") ", row);
      var timeRemaining =
        Math.floor((24 - (now - info[2]) / 1000 / 60 / 60) * 10) / 10;
      Web.CreateElement("span", "disabled", row).innerHTML =
        timeRemaining >= 0 ? timeRemaining + "+h" : "?";
    });
    if (unsafeWindow.System != undefined)
...
StorageItemsWatcher.MakeItAltFriendly = function () {
  ...
  if (items != null) Storage.set("ExpiringItems_" + Estiah.Player.Id, items);
...
Launcher.Register(
  "StorageItemsWatcher",
  "Watches for items that will expire soon. Check is performed once per 1/2h or by visiting inventory."
)
  .Read(Estiah.Inventory.UrlTest, StorageItemsWatcher.Update)
  .Modify(Launcher.UrlTest.EveryPage, StorageItemsWatcher.CheckAndAlterPage)
  .UpdateData("110301", StorageItemsWatcher.MakeItAltFriendly);
...
AutoRelogin.LogIn = function (doc) {
  ...
  AutoRelogin.TryLogIn = function () {
    ...
    for (var i = 0; i < inputs.length; i++) {
      if (!inputs[i].value.length) filled = false;
    }
    if (filled) {
      // console.log(inputs[0].value + ':' + inputs[1].value);
      Web.MouseClick(doc.querySelector("#AuthSubmit a"));
    } else setTimeout(AutoRelogin.TryLogIn, 100);
  };
...
Launcher.Register(
  "AutoRelogin",
  "If you have set autofill login/pass, it will refresh your lost session."
).Modify(Launcher.UrlTest.LogIn, AutoRelogin.LogIn);
