// ==UserScript==
// @name         SN's GLB Skill Point Page Enhancer (v6.1)
// @namespace    SeattleNiner
// @description  v6.1: Speed optimization & detection badge spacing fix.
// @version      6.1
// @match        https://glb.warriorgeneral.com/game/skill_points.pl?*
// @grant        none
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/563531/SN%27s%20GLB%20Skill%20Point%20Page%20Enhancer%20%28v61%29.user.js
// @updateURL https://update.greasyfork.org/scripts/563531/SN%27s%20GLB%20Skill%20Point%20Page%20Enhancer%20%28v61%29.meta.js
// ==/UserScript==
...
(function () {
    ...
    function fetchPlayerData(callback) {
        ...
        fetch("/game/player.pl?player_id=" + pid).then(r => r.text()).then(html => {
    ...
    style.innerHTML = `
        ...
        .sn_section_wrapper { display: flex; flex-direction: column; }
        .sn_header {
        ...
        .sn_planner_table th { background: #333; color: #fff; font-size: 10px; padding: 0; border: 1px solid #555; height: 20px; width: 33.333% !important; }
        ...
        .plus_button, .minus_button, .plus_button a, .minus_button a { display: flex !important; align-items: center !important; height: 100% !important; margin: 0 !important; }
        .plus_minus img { display: block !important; margin: 0 !important; }
        .sn_detect_badge { font-weight: bold; margin-left: 5px; }
    `;
    ...
    function updatePlannerRow(row, baseVal, currentLevel, buildData) {
        ...
        let mm = buildData.affectedAtts[shortName] || 0, totalGain = (mm === 0) ? 0 : calcALG(currentLevel, mm, buildData);
    ...
    function createPlannerRow(statName, map, currentLevel, buildData) {
        ...
        row.querySelector('input').addEventListener('input', () => updatePlannerRow(row, baseVal, currentLevel, buildData));
        updatePlannerRow(row, baseVal, currentLevel, buildData);
        return row;
    }

    function init(level, pos, archName) {
        ...
        let builds = getBuilds(pos), myBuildArgs = builds.find(b => b[0] === archName) || builds[0], buildData = new BuildData(myBuildArgs);
        ...
        physNames.forEach(n => { if (map[n]) { wrapper.querySelector('#sn_phys_body').appendChild(map[n]); wrapper.querySelector('#sn_phys_table').appendChild(createPlannerRow(n, map, level, buildData)); } });
        skillNames.forEach(n => { if (map[n]) { wrapper.querySelector('#sn_skill_body').appendChild(map[n]); wrapper.querySelector('#sn_skill_table').appendChild(createPlannerRow(n, map, level, buildData)); } });

        const headerTitle = root.querySelector('.medium_head');
        if (headerTitle) {
            headerTitle.innerHTML += ` <span class="sn_detect_badge">[Lvl ${level} ${pos} | ${archName}]</span>`;
    ...
    const run = () => { if (document.querySelector('.attribute_container') && !document.getElementById('sn_main_wrapper')) { fetchPlayerData((level, pos, arch) => { init(level, pos, arch); }); } };
    run();
