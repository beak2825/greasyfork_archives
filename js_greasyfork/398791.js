...
// @downloadURL https://update.greasyfork.org/scripts/398791/Same%20Style.user.js
// @updateURL https://update.greasyfork.org/scripts/398791/Same%20Style.meta.js
// ==/UserScript==
...
function initializePage(){
    ...
    var getDetailsInfo = api_url+'getDetailsInfo';
    GM_xmlhttpRequest({
        method: "POST",
        url: getDetailsInfo,
        dataType: "json",
        ...
        onload: function(response) {
            if(response.readyState==4&&response.status==200){
                ...
                var html_one = "<div class='item-content clearfix'>" +
                    ...
                    "                    <strong style='font-size: 18px;font-weight: 400;margin-bottom: 10px;'>"+jsondata.product_name+"</strong>" +
                    "                </div>" +
                    ...
                    "                        <div style='float: left;line-height:35px;width: 74px;'>网店价</div>" +
                    ...
                    "                                <strong style='font-size: 26px;font-weight: bold;'>"+jsondata.store_price+"</strong>" +
                    "                            </span>" +
                    "                        </div>" +
                    ...
                    "                        <div style='float: left;font-size:14px;color: #6c6c6c;width: 68px;'>采购数量</div>" +
                    "                        <div style='display: inline-block;color: #6c6c6c;'>"+jsondata.buy_num+"件</div>" +
                    "                        <div style='clear: both;'></div>" +
                    "                    </div>" +
                    "                </div>" +
                                      "<div style='position: relative;width: 530px;padding: 2px 0;'>" +
                    "                    <div style='position: relative;zoom: 1;line-height: 30px;margin: 10px 20px;'>" +
                    "                        <div style='float: left;font-size:14px;color: #6c6c6c;width: 68px;'>颜色</div>" +
                    "                        <div style='display: inline-block;color: #6c6c6c;'>"+jsondata.color+"</div>" +
                    "                        <div style='clear: both;'></div>" +
                    "                    </div>" +
                    "                </div>" +
                                       "<div style='position: relative;width: 530px;padding: 2px 0;'>" +
                    "                    <div style='position: relative;zoom: 1;line-height: 30px;margin: 10px 20px;'>" +
                    "                        <div style='float: left;font-size:14px;color: #6c6c6c;width: 68px;'>尺码</div>" +
                    "                        <div style='display: inline-block;color: #6c6c6c;'>"+jsondata.size+"件</div>" +
                    "                        <div style='clear: both;'></div>" +
                    "                    </div>" +
                    "                </div>" +
                                        "<div style='position: relative;width: 530px;padding: 2px 0;'>" +
                    "                    <div style='position: relative;zoom: 1;line-height: 30px;margin: 10px 20px;'>" +
                    "                        <div style='float: left;font-size:14px;color: #6c6c6c;width: 86px;'>最高采购价</div>" +
                    "                        <div style='display: inline-block;color: #6c6c6c;'>"+jsondata.ceiling_price+"元</div>" +
                    "                        <div style='clear: both;'></div>" +
                    "                    </div>" +
                    "                </div>" +
                    ...
                    "                                <a href="+jsondata.goods_url+" target='_blank'>" +
                    "                                    <img src="+jsondata.main_image+" alt='' class='jqzoom' style='max-height: 400px;max-width: 400px;cursor: pointer;'>" +
                    "                                </a>" +
                    "                            </div>" +
                    "                        </div>" +
                    "                    </div>" +
...
function collectData(params,obj){
    GM_xmlhttpRequest({
        method: "POST",
        url: api_url +"confirm?details_id="+details_id,
        dataType: "json",
        data: params,
        headers: {
            "Content-Type": "application/json"
...
function showVvicList(sort_by) {
    ...
    for (var i = 0; i < similarList.length; i++) {
        ...
        var trItemSame = trItemSame = '<tr  class="tr_'+i+'"><td width="14%" rowspan="2" class="lxSimilarImage"><a target="_blank" href="' + vvicUrl + itemId + '.html"><img src="' + imageUrl + '" style="width:100px;height:100px;"/></a></td><td width="50%" class="lxSimilarTitle"><a target="_blank" href="' + vvicUrl + itemId + '.html">' +
            ...
            '</td><td class="lxSimilarLocation">' + "" + '</td><td align="center" style="width:100px;position:relative"><input type="button" value="选用" class="cjvvic_btn" data=\''+datastr+'\'/><input type="button" value="获取供应商信息" class="get_supplier_info" data=\''+datastr+'\'/><input type="button" value="价格比对" data-price="'+price+'" data-itemid="'+itemId+'" class="price_compare" /></td></tr>';
        for (var j = 0; j < price_arr.length; j++) {
            if (price_arr[j].itemid == itemId) {
            ...
            title + '</a></td><td width="25%" class="lxSimilarSale">成交' + tradeNumber + '笔</td><td width="16%" class="lxSimilarPrice">' + price + '</td></tr><tr  class="tr_'+i+'"><td class="lxSimilarShop">' + shopName +
            '</td><td class="lxSimilarLocation">' + "" + '</td><td align="center" style="width:100px;position:relative"><input type="button" value="选用" class="cjvvic_btn" data=\''+datastr+'\'/><input type="button" value="获取供应商信息" class="get_supplier_info" data=\''+datastr+'\'/><input type="button" value="取消比对" data-price="'+price+'" data-itemid="'+itemId+'" class="price_compare has" /></td></tr>';
            }
        }
        similarHtml += trItemSame;
    }
