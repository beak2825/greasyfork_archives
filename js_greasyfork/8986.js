...
// @namespace   PSFD
// @description Skip waiting screen at Firedrive, Putlocker, Sockshare and 21+ more video hosting sites.
// @version     2.4
...
// @author      drhouse
// @icon        https://cdn2.iconfinder.com/data/icons/windows-8-metro-style/512/timer.png
// @downloadURL https://update.greasyfork.org/scripts/8986/Firedrive%20Putlocker%20Sockshare%20Skip.user.js
// @updateURL https://update.greasyfork.org/scripts/8986/Firedrive%20Putlocker%20Sockshare%20Skip.meta.js
// ==/UserScript==
...
$(document).ready(function () {
    ...
    if (location.href.toString().indexOf("billionuploads") != -1 || parent.location.href.toString().indexOf("bestreams") != -1 || parent.location.href.toString().indexOf("movpod") != -1 || parent.location.href.toString().indexOf("movreel") != -1){
    ...
    //movreel
    setInterval(function () {$('#vid_play').click();}, 3000);  
    ...
    if (location.href.toString().indexOf("uploadboy") != -1){
        ...
        //vidbull
	if (location.href.toString().indexOf("vidbull") != -1){
		setInterval(function () {$('#btn_download').click();}, 3000); 
	}

	//letwatch
	if (location.href.toString().indexOf("letwatch") != -1){
		setInterval(function () {$('#btn_download').click();}, 3000); 
	}

	//vidzi
	if (location.href.toString().indexOf("vidzi") != -1){
		setInterval(function () {$("#impo_box1").remove();}, 1000);
	}

	//vid.ag
	if (location.href.toString().indexOf("vid.ag") != -1){
		setInterval(function () {$('#btn_download').click();}, 5200); 
	}

	//streamin.to
	if (location.href.toString().indexOf("streamin.to") != -1){
		setInterval(function () {$('#btn_download').click();}, 5200); 
	}
