// ==UserScript==
// @name         百度云百度网盘下载助手直链
// @namespace    https://greasyfork.org/zh-CN/scripts/374961
// @version      11.1
// @description  去除广告
// @author       fxixi
...
// @namespace undefined
// @downloadURL https://update.greasyfork.org/scripts/374961/%E7%99%BE%E5%BA%A6%E4%BA%91%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E4%B8%8B%E8%BD%BD%E5%8A%A9%E6%89%8B%E7%9B%B4%E9%93%BE.user.js
// @updateURL https://update.greasyfork.org/scripts/374961/%E7%99%BE%E5%BA%A6%E4%BA%91%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E4%B8%8B%E8%BD%BD%E5%8A%A9%E6%89%8B%E7%9B%B4%E9%93%BE.meta.js
// ==/UserScript==
		...
		function start(ctx, dServ, allZip) {
			...
			if (currentProduct === 'pan') {
				requestMethod = function(e, cb) {
					dServ.getDlinkPan(dServ.getFsidListData(e), allZip ? 'batch' : e.isdir === 1 ? 'batch' : 'nolimit', cb,
			...
			Promise.all(promises).then(function(result) {
				...
				result.filter(function(e) {
					...
					if (typeof e.dlink === 'string') {
						var dlink = e.dlink + "&zipname=" + encodeURIComponent((e.isdir ? '【文件夹】' : '【文件】') + e.server_filename +
							'.zip');
				...
				var text = '<textarea id="bar" rows="' + ((dlinks.length > 20 ? 20 : dlinks.length) + 1) +
					'" style="width: 100%;white-space: nowrap;">' + dlinks.join('\n').replace('d.pcs.baidu.com',
						'c.pcs.baidu.com') + '</textarea>';
			...
			require.async(prefix + 'download/service/guanjiaConnector.js', function(gjC) {
				gjC.init = function() {
					setTimeout(function() {
						ctx.ui.tip({
							mode: 'caution',
							msg: '检测到正在调用百度网盘客户端，若脚本失效，请检查更新或提交issue',
							hasClose: true,
							autoClose: false
						});
					}, 5e3);
