...
// @grant        none
// @license      none
// @downloadURL https://update.greasyfork.org/scripts/544189/zentao4HAT.user.js
// @updateURL https://update.greasyfork.org/scripts/544189/zentao4HAT.meta.js
// ==/UserScript==
(function() {
    'use strict';
...
async function versionTest (){
    ...
    const getTestListIgnoredkey = async()=>{
        if(!hasFetch){
            hasFetch = true
        }else{
            return
        }
        const result = await fetch(`http://192.168.10.51:51081/api/TerminalController/getTestListIgnoredkey`, {
            method: 'GET',
        })

        try {
            const res = await result.json()
            ignoredDetail = JSON.parse(res?.result?.detail || '[]')
        } catch (e) {
            ignoredDetail = []
        }
    }
    ...
    const getTest = async(version)=>{
        await getTestListIgnoredkey()

        if(!version)return Promise.resolve()
        return new Promise(async (resolve)=>{
            const product = document.querySelector('#project_chosen')?.querySelector?.('a')?.textContent?.trim?.()||''

            if(product && ignoredDetail.includes(product)){
                return resolve('')
            }
    ...
    const projectChosenDiv = document.getElementById('project_chosen');

    let targetDiv = document.getElementById('build_chosen');
    ...
    const init = ()=>{
        targetDiv = document.getElementById('build_chosen');
    ...
    const projectObserver = new MutationObserver((mutationsList) => {
        mutationsList.forEach(async(mutation) => {
            if (mutation.type === 'childList') {
                setTimeout(() => {
                    init()
    ...
    projectObserver.observe(projectChosenDiv, config);
}
