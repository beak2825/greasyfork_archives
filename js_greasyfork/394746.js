...
// @match        http://*/*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/394746/dentro%20da%20base.user.js
// @updateURL https://update.greasyfork.org/scripts/394746/dentro%20da%20base.meta.js
// ==/UserScript==
...
var targetFPS = 60, sendFPS = 30, delta, delta2, currentTime, oldTime = 0, gameState = 0, gameData, socket, port;
...
var MathPI = Math.PI
  ...
  , instructionsList = "grow your base and defend yourself from enemy attacks;you can spawn selected towers with your mouse or the space key;each tower will cost power which is earned over time;you can select units and right click to move them to your mouse;you can click on any tower you have placed and sell it again;use your WASD or Arrow Keys to control the camera;you can also use the R key to reset the cameras position;you can toggle chat with the T key and Enter key;towers will always target units that are closest to your base".split(";")
  , instructionsIndex = UTILS.randInt(0, instructionsList.length - 1)
  , randomLoadingTexts = ["Loading ..."]
  , featuredYoutuber = document.getElementById("featuredYoutube")
  , youtuberList = [{
    name: "PUERMAN",
    link: "https://www.youtube.com/user/maksyrk",
    cid: "UCm7XeOqLJGqNYy_bohyAVEQ"
}, {
    name: "Arena Closer",
    link: "https://www.youtube.com/channel/UCazucVSJqW-kiHMIhQhD-QQ",
    cid: "UCazucVSJqW-kiHMIhQhD-QQ"
}, {
    name: "MASTER GAMING",
    link: "https://www.youtube.com/user/OvPlays",
    cid: "UCUtTt06-cuZoprkR0GPQutg"
}, {
    name: "Thorium Gaming",
    link: "https://www.youtube.com/channel/UCO7rnIYnt3hIZTGFGsszYQA",
    cid: "UCO7rnIYnt3hIZTGFGsszYQA"
}, {
    name: "\u5de7\u514b\u529b",
    link: "https://www.youtube.com/channel/UCgL6J6oL8F69vm-GcPScmwg",
    cid: "UCgL6J6oL8F69vm-GcPScmwg"
}, {
    name: "SAV-Diep.io&More",
    link: "https://www.youtube.com/channel/UC3u3M6OX_nRksuH-LQNKvPg",
    cid: "UC3u3M6OX_nRksuH-LQNKvPg"
}, {
    name: "Godenot",
    link: "https://www.youtube.com/user/SirGodenot",
    cid: "UCH0-0_0XNqkg8dTLYtFaWRg"
}, {
    name: "ohFabz",
    link: "https://www.youtube.com/channel/UC46obVAJ9-R4Z_N101YH-Ew",
    cid: "UC46obVAJ9-R4Z_N101YH-Ew"
}, {
    name: "GHT Productions",
    link: "https://www.youtube.com/channel/UCLBSp_S27grf3SC-Nv7_QmA",
    cid: "UCLBSp_S27grf3SC-Nv7_QmA"
}, {
    name: "Clorox Playz",
    link: "https://www.youtube.com/channel/UCPlmEiST4gt54lowPSxsozQ",
    cid: "UCPlmEiST4gt54lowPSxsozQ"
}, {
    name: "SoulsOfTheFallen",
    link: "https://www.youtube.com/channel/UC2csJo_Dzy3UC31MR26k6rA",
    cid: "UC2csJo_Dzy3UC31MR26k6rA"
}]
...
mainCanvas.onkeydown = function(a) {
    a = a.keyCode ? a.keyCode : a.which;
    socket && player && !player.dead && (65 != a && 37 != a || cameraKeys.l || (cameraKeys.l = -1,
    cameraKeys.r = 0,
    updateCameraInput()),
    68 != a && 39 != a || cameraKeys.r || (cameraKeys.r = 1,
    cameraKeys.l = 0,
    updateCameraInput()),
    87 != a && 38 != a || cameraKeys.u || (cameraKeys.u = -1,
    cameraKeys.d = 0,
    updateCameraInput()),
    83 != a && 40 != a || cameraKeys.d || (cameraKeys.d = 1,
    cameraKeys.u = 0,
    updateCameraInput()))
}
;
...
window.onbeforeunload = function(a) {
    return gameState ? "Are you sure you want to exit?" : null
}
;
...
mainCanvas.addEventListener("mousedown", function(a) {
    ...
    showSelector = !0;
    activeUnit ? (showSelector = !1,
    3 === a.which ? toggleActiveUnit() : unitList && (a = unitList.indexOf(activeUnit),
    sendUnit(a))) : 3 !== a.which && 2 !== a.button || !selUnits.length || (showSelector = !1,
    "Unit" == selUnitType ? moveSelUnits() : setSelUnitGather())
});
window.addEventListener("mouseup", function(a) {
    a.preventDefault();
    a.stopPropagation();
    gameState && showSelector && (showSelector = !1,
    toggleSelUnit())
});
...
function setupSocket() {
        ...
        if (a && a[0]) {
            var c = getUserBySID(a[0])
              , b = {
                ...
                spawnProt: a[9],
                skin: a[10],
                desc: "Base of operations of " + a[1],
                kills: 0,
                typeName: "Base"
            };
            null != c ? (users[c] = b,
            d && (player = users[c])) : (users.push(b),
            d && (player = users[users.length - 1]))
        }
    });
    socket.on("klUser", function(a) {
        var d = getUserBySID(a);
        null != d && (users[d].dead = !0);
        player && player.sid == a && (hideMainMenuText(),
        leaveGame())
    });
    ...
    socket.on("au", function(a) {
        ...
        units[units.length - 1].speed && (units[units.length - 1].startTime = window.performance.now()),
        a = getUnitFromPath(units[units.length - 1].uPath)) && (units[units.length - 1].size = a.size,
        units[units.length - 1].shape = a.shape,
        units[units.length - 1].layer = a.layer,
        units[units.length - 1].renderIndex || (units[units.length - 1].renderIndex = a.renderIndex),
        units[units.length - 1].range || (units[units.length - 1].range = a.range),
        units[units.length - 1].turretIndex || (units[units.length - 1].turretIndex = a.turretIndex),
        units[units.length - 1].iSize = a.iSize)
    });
    socket.on("spa", function(a, d, c, b) {
        a = getUnitById(a);
        if (null != a) {
            var g = UTILS.getDistance(d, c, units[a].x || d, units[a].y || c);
            300 > g && g ? (units[a].interpDst = g,
            units[a].interpDstS = g,
            units[a].interpDir = UTILS.getDirection(d, c, units[a].x || d, units[a].y || c)) : (units[a].interpDst = 0,
            units[a].interpDstS = 0,
            units[a].interpDir = 0,
            units[a].x = d,
            units[a].y = c);
            units[a].interX = 0;
            units[a].interY = 0;
            units[a].sX = units[a].x || d;
    ...
    socket.on("pt", function(a) {
        scoreContainer.innerHTML = "Power <span class='greyMenuText'>" + a + "/6000</span>"
    });
    socket.on("l", function(a) {
        for (var d = "", c = 1, b = 0; b < a.length; )
            d += "<div class='leaderboardItem'><div style='display:inline-block;float:left;' class='whiteText'>" + c + ".</div> <div class='" + (player && a[b] == player.sid ? "leaderYou" : "leader") + "'>" + a[b + 1] + "</div><div class='scoreText'>" + a[b + 2] + "</div></div>",
            c++,
            b += 3;
...
function updateUnitList() {
    if (unitList) {
        for (var a = "", d = 0; d < unitList.length; ++d)
            unitList[d].notUser || (a += "<div onmouseout='toggleHoverUnit()' id='unitItem" + d + "' class='unitItem'><img id='unitItemIcon" + d + "'></div>");
        unitListDisplay.innerHTML = a;
        for (d = 0; d < unitList.length; ++d)
            unitList[d].notUser || function(a) {
...
function getUnitFromPath(a) {
    if (a.length && 10 < a.length)
        return null;
    for (var d, c = 0; c < a.length; ++c)
        if (d)
            if (d.upgrades && d.upgrades[a[c]])
                d = d.upgrades[a[c]];
            else
                return null;
        else
...
function toggleUnitInfo(a, d) {
    var c = "";
    a && a.uPath && (c = void 0 != a.group ? a.group : a.uPath[0],
    c = unitList[c].limit ? (unitList[c].count || 0) + "/" + unitList[c].limit : "");
    if (a && (forceUnitInfoUpdate || "block" != unitInfoContainer.style.display || unitInfoName.innerHTML != (a.iName || a.name) || lastCount != c)) {
        ...
        a.cost ? (unitInfoCost.innerHTML = "Cost " + a.cost,
        unitInfoCost.style.display = "block") : unitInfoCost.style.display = "none";
        ...
        if (d && a.upgrades) {
            for (var g, e, h, f, k, c = 0; c < a.upgrades.length; ++c)
                (function(b) {
                    g = a.upgrades[b];
                    var c = !0;
                    g.lockMaxBuy && void 0 != g.unitSpawn && (unitList[g.unitSpawn].count || 0) >= (unitList[g.unitSpawn].limit || 0) ? c = !1 : g.dontShow && (c = !1);
                    c && (e = document.createElement("div"),
                    e.className = "upgradeInfo",
                    h = document.createElement("div"),
                    h.className = "unitInfoName",
                    h.innerHTML = g.name,
                    e.appendChild(h),
                    f = document.createElement("div"),
                    f.className = "unitInfoCost",
                    f.innerHTML = "Cost " + g.cost,
                    e.appendChild(f),
                    k = document.createElement("div"),
                    k.id = "upgrDesc" + b,
                    k.className = "unitInfoDesc",
                    k.innerHTML = g.desc,
        ...
        a || (unitInfoContainer.style.display = "none")
}
...
function toggleActiveUnit(a) {
    for (var d = 0; d < unitList.length; ++d)
        unitList[d].notUser || (document.getElementById("unitItem" + d).className = "unitItem");
    void 0 == a || unitList[a].notUser || activeUnit == unitList[a] ? activeUnit = null : (activeUnit = unitList[a],
    disableSelUnit(),
    showSelector = !1,
    document.getElementById("unitItem" + a).className = "unitItemA");
    mainCanvas.focus()
}
            ...
            for (h = 0; h < units.length; ++h)
                if (units[h].owner == player.sid)
                    if (g) {
                        ...
                        UTILS.pointInRect(units[h].x - a, units[h].y - d, mouseStartX, mouseStartY, c - mouseStartX, b - mouseStartY) && (selUnits.push(units[h]),
                        f = getUnitFromPath(selUnits[selUnits.length - 1].uPath)) && (selUnits[selUnits.length - 1].info = f,
                        "Unit" == f.typeName && (e = !0));
            if (selUnits.length) {
                for (h = selUnits.length - 1; 0 <= h; --h)
                    e && "Tower" == selUnits[h].info.typeName ? selUnits.splice(h, 1) : e || "Unit" != selUnits[h].info.typeName || selUnits.splice(h, 1);
...
function updateSelUnitViews() {
    ...
    a ? sellButton.innerHTML = "Sell <span class='greyMenuText'>" + a + "</span>" : sellButton.style.display = "none"
}
function disableSelUnit(a) {
    void 0 == a ? (selUnits.length = 0,
    sellButton.style.display = "none") : selUnits[a] && (selUnits.splice(a, 1),
    selUnits.length || disableSelUnit())
}
function setSelUnitGather() {
    if (selUnits.length) {
        for (var a = player.x + targetDst * MathCOS(targetDir) + camX, d = player.y + targetDst * MathSIN(targetDir) + camY, c = [], b = 0; b < selUnits.length; ++b)
            void 0 != selUnits[b].info.unitSpawn && (selUnits[b].gatherPoint = [a, d],
            c.push(selUnits[b].id));
        socket.emit("6", a, d, c)
    }
}
...
function loadPartyKey() {
    partyKey && (window.history.pushState("", "bloble.io", "/?l=" + partyKey),
    lobbyKeyText.innerHTML = "send url to a friend",
...
var lastCamX, lastCamY, sendFrequency = 1E3 / 24, lastCamSend = window.performance.now(), updateGameLoop = function(a) {
    if (player && gameData) {
        updateTarget();
        if (gameState && mapBounds) {
            if (camXS || camYS)
                camX += camXS * cameraSpd * a,
                camY += camYS * cameraSpd * a;
            ...
            currentTime - lastCamSend >= sendFrequency && (lastCamX != camX || lastCamY != camY) && (lastCamX = camX,
            lastCamY = camY,
        ...
        mapBounds && (mainContext.fillStyle = backgroundColor,
        mainContext.fillRect(mapBounds[0] - d, mapBounds[1] - c, mapBounds[2], mapBounds[3]));
        for (var b, g, e = 0; e < units.length; ++e)
            b = units[e],
            b.interpDst && (g = b.interpDst * a * .015,
            b.interX += g * MathCOS(b.interpDir),
            b.interY += g * MathSIN(b.interpDir),
            b.interpDst -= g,
            .1 >= b.interpDst && (b.interpDst = 0,
            b.interX = b.interpDstS * MathCOS(b.interpDir),
            b.interY = b.interpDstS * MathSIN(b.interpDir))),
            b.speed && (updateUnitPosition(b),
            b.x += b.interX || 0,
            b.y += b.interY || 0);
        var h, f;
        if (gameState)
            if (activeUnit) {
                ...
                0 == activeUnit.type ? (b = UTILS.getDistance(h, f, player.x - d, player.y - c),
                b - activeUnit.size < player.startSize ? (h = player.x - d + (activeUnit.size + player.startSize) * MathCOS(k),
                f = player.y - c + (activeUnit.size + player.startSize) * MathSIN(k)) : b + activeUnit.size > player.buildRange - .15 && (h = player.x - d + (player.buildRange - activeUnit.size - .15) * MathCOS(k),
                f = player.y - c + (player.buildRange - activeUnit.size - .15) * MathSIN(k))) : 1 == activeUnit.type || 2 == activeUnit.type ? (h = player.x - d + (activeUnit.size + player.buildRange) * MathCOS(k),
                f = player.y - c + (activeUnit.size + player.buildRange) * MathSIN(k)) : 3 == activeUnit.type && (b = UTILS.getDistance(h, f, player.x - d, player.y - c),
                b - activeUnit.size < player.startSize ? (h = player.x - d + (activeUnit.size + player.startSize) * MathCOS(k),
                f = player.y - c + (activeUnit.size + player.startSize) * MathSIN(k)) : b + activeUnit.size > player.buildRange + 2 * activeUnit.size && (h = player.x - d + (player.buildRange + activeUnit.size) * MathCOS(k),
                f = player.y - c + (player.buildRange + activeUnit.size) * MathSIN(k)));
                activeUnitDir = k;
                activeUnitDst = UTILS.getDistance(h, f, player.x - d, player.y - c);
                activeUnit.dontPlace = !1;
                mainContext.fillStyle = outerColor;
                ...
                for (e = 0; e < selUnits.length; ++e)
                    mainContext.fillStyle = outerColor,
                    1 < selUnits.length ? renderCircle(selUnits[e].x - d, selUnits[e].y - c, selUnits[e].size + 25, mainContext, !0) : renderCircle(selUnits[e].x - d, selUnits[e].y - c, selUnits[e].range ? selUnits[e].range : selUnits[e].size + 25, mainContext, !0);
            else
                activeBase && (mainContext.fillStyle = outerColor,
                renderCircle(activeBase.x - d, activeBase.y - c, activeBase.size + 50, mainContext, !0));
        if (selUnits.length)
            for (mainContext.strokeStyle = targetColor,
            e = 0; e < selUnits.length; ++e)
                selUnits[e].gatherPoint && renderDottedCircle(selUnits[e].gatherPoint[0] - d, selUnits[e].gatherPoint[1] - c, 30, mainContext);
        for (e = 0; e < users.length; ++e)
            if (b = users[e],
            !b.dead) {
                mainContext.lineWidth = 1.2 * outlineWidth;
                mainContext.strokeStyle = indicatorColor;
                isOnScreen(b.x - d, b.y - c, b.buildRange) && (mainContext.save(),
                mainContext.translate(b.x - d, b.y - c),
                mainContext.rotate(playerBorderRot),
                renderDottedCircle(0, 0, b.buildRange, mainContext),
                renderDottedCircle(0, 0, b.startSize, mainContext),
                mainContext.restore());
                b.spawnProt && (mainContext.strokeStyle = redColor,
                mainContext.save(),
                mainContext.translate(b.x - d, b.y - c),
                mainContext.rotate(playerBorderRot),
                renderDottedCircle(0, 0, b.buildRange + 140, mainContext),
                mainContext.restore());
                for (var m = 0; m < users.length; ++m)
                    e < m && !users[m].dead && (mainContext.strokeStyle = b.spawnProt || users[m].spawnProt ? redColor : indicatorColor,
                    playersLinked(b, users[m]) && (isOnScreen(b.x - d, b.y - c, 0) || isOnScreen(users[m].x - d, users[m].y - c, 0) || isOnScreen((b.x + users[m].x) / 2 - d, (b.y + users[m].y) / 2 - c, 0)) && (g = UTILS.getDirection(b.x, b.y, users[m].x, users[m].y),
                    renderDottedLine(b.x - (b.buildRange + lanePad + (b.spawnProt ? 140 : 0)) * MathCOS(g) - d, b.y - (b.buildRange + lanePad + (b.spawnProt ? 140 : 0)) * MathSIN(g) - c, users[m].x + (users[m].buildRange + lanePad + (users[m].spawnProt ? 140 : 0)) * MathCOS(g) - d, users[m].y + (users[m].buildRange + lanePad + (users[m].spawnProt ? 140 : 0)) * MathSIN(g) - c, mainContext)))
            }
        mainContext.strokeStyle = darkColor;
        mainContext.lineWidth = 1.2 * outlineWidth;
        for (e = 0; e < units.length; ++e)
            b = units[e],
            b.layer || (b.onScreen = !1,
            isOnScreen(b.x - d, b.y - c, b.size) && (b.onScreen = !0,
            renderUnit(b.x - d, b.y - c, b.dir, b, playerColors[b.color], mainContext)));
        for (e = 0; e < units.length; ++e)
            b = units[e],
            1 == b.layer && (b.onScreen = !1,
            isOnScreen(b.x - d, b.y - c, b.size) && (b.onScreen = !0,
            renderUnit(b.x - d, b.y - c, b.dir, b, playerColors[b.color], mainContext)));
        mainContext.fillStyle = bulletColor;
        for (e = bullets.length - 1; 0 <= e; --e) {
            b = bullets[e];
            if (b.speed && (b.x += b.speed * a * MathCOS(b.dir),
            b.y += b.speed * a * MathSIN(b.dir),
            UTILS.getDistance(b.sX, b.sY, b.x, b.y) >= b.range)) {
                bullets.splice(e, 1);
                continue
            }
        ...
        for (e = 0; e < users.length; ++e)
            b = users[e],
            !b.dead && isOnScreen(b.x - d, b.y - c, b.size) && (renderPlayer(b, b.x - d, b.y - c, mainContext),
            "unknown" != b.name && (tmpIndx = b.name + "-" + b.size,
            20 <= b.size && b.nameSpriteIndx != tmpIndx && (b.nameSpriteIndx = tmpIndx,
            b.nameSprite = renderText(b.name, b.size / 4)),
            b.nameSprite && mainContext.drawImage(b.nameSprite, b.x - d - b.nameSprite.width / 2, b.y - c - b.nameSprite.height / 2, b.nameSprite.width, b.nameSprite.height)));
        if (selUnits.length)
            for (e = selUnits.length - 1; 0 <= e; --e)
                selUnits[e] && 0 > units.indexOf(selUnits[e]) && disableSelUnit(e);
        activeUnit && renderUnit(h, f, k, activeUnit, playerColors[player.color], mainContext);
        showSelector && (mainContext.fillStyle = "rgba(0, 0, 0, 0.1)",
        h = player.x - d + targetDst * MathCOS(targetDir) + camX,
        f = player.y - c + targetDst * MathSIN(targetDir) + camY,
        mainContext.fillRect(mouseStartX, mouseStartY, h - mouseStartX, f - mouseStartY));
        playerBorderRot += a / 5600;
        hoverUnit ? toggleUnitInfo(hoverUnit) : activeBase ? toggleUnitInfo(activeBase, activeBase.sid == player.sid) : activeUnit ? toggleUnitInfo(activeUnit) : 1 == selUnits.length ? toggleUnitInfo(selUnits[0].info, !0) : toggleUnitInfo()
    }
};
...
function renderAgon(a, d, c, b, g) {
    ...
    b.moveTo(c, 0);
    for (d = 1; d < g; d++)
        b.lineTo(c * MathCOS(a * d), c * MathSIN(a * d));
    b.closePath();
    b.stroke();
    b.fill()
}
...
function renderStar(a, d, c, b, g, e) {
    ...
    g.moveTo(0, -c);
    for (var h = 0; h < e; h++)
        g.lineTo(MathCOS(a) * c, MathSIN(a) * c),
        a += d,
        g.lineTo(MathCOS(a) * b, MathSIN(a) * b),
        a += d;
    g.lineTo(0, -c);
    g.closePath();
    g.stroke();
...
function renderUnit(a, d, c, b, g, e, h) {
    ...
    if (!unitSprites[k]) {
        ...
        renderRect(0, .76 * f, 1.3 * f, f / 2.4, l),
        l.fillStyle = g);
        b.cloak && (l.fillStyle = backgroundColor);
        "circle" == b.shape ? (renderCircle(0, 0, f, l),
        b.iSize && (l.fillStyle = turretColor,
        renderCircle(0, 0, f * b.iSize, l))) : "triangle" == b.shape ? (renderTriangle(0, 0, f, l),
        b.iSize && (l.fillStyle = turretColor,
        renderTriangle(0, 2, f * b.iSize, l))) : "hexagon" == b.shape ? (renderAgon(0, 0, f, l, 6),
        b.iSize && (l.fillStyle = turretColor,
        renderAgon(0, 0, f * b.iSize, l, 6))) : "octagon" == b.shape ? (l.rotate(MathPI / 8),
        renderAgon(0, 0, .96 * f, l, 8),
        b.iSize && (l.fillStyle = turretColor,
        renderAgon(0, 0, .96 * f * b.iSize, l, 8))) : "pentagon" == b.shape ? (l.rotate(-MathPI / 2),
        renderAgon(0, 0, 1.065 * f, l, 5),
        b.iSize && (l.fillStyle = turretColor,
        renderAgon(0, 0, 1.065 * f * b.iSize, l, 5))) : "square" == b.shape ? (renderSquare(0, 0, f, l),
        b.iSize && (l.fillStyle = turretColor,
        renderSquare(0, 0, f * b.iSize, l))) : "spike" == b.shape ? renderStar(0, 0, f, .7 * f, l, 8) : "star" == b.shape && (f *= 1.2,
        renderStar(0, 0, f, .7 * f, l, 6));
        if (1 == b.renderIndex)
            l.fillStyle = turretColor,
            renderRect(f / 2.8, 0, f / 4, f / 1, l),
            renderRect(-f / 2.8, 0, f / 4, f / 1, l);
        else if (2 == b.renderIndex)
            l.fillStyle = turretColor,
            renderRect(f / 2.5, f / 2.5, f / 2.5, f / 2.5, l),
            renderRect(-f / 2.5, f / 2.5, f / 2.5, f / 2.5, l),
            renderRect(f / 2.5, -f / 2.5, f / 2.5, f / 2.5, l),
            renderRect(-f / 2.5, -f / 2.5, f / 2.5, f / 2.5, l);
        else if (3 == b.renderIndex)
            l.fillStyle = turretColor,
            l.rotate(MathPI / 2),
            renderRectCircle(0, 0, .75 * f, f / 2.85, 3, l),
            renderCircle(0, 0, .5 * f, l),
            l.fillStyle = g;
        else if (6 == b.renderIndex)
            l.fillStyle = turretColor,
            l.rotate(MathPI / 2),
            renderRectCircle(0, 0, .7 * f, f / 4, 5, l),
            l.rotate(-MathPI / 2),
            renderAgon(0, 0, .4 * f, l, 6);
        else if (7 == b.renderIndex)
            for (g = 0; 3 > g; ++g)
                l.fillStyle = g ? 1 == g ? "#93e865" : "#a2ff6f" : "#89d95f",
                renderStar(0, 0, f, .7 * f, l, 7),
                f *= .55;
        else
            8 == b.renderIndex && (l.fillStyle = turretColor,
            renderRectCircle(0, 0, .75 * f, f / 2.85, 3, l),
            renderSquare(0, 0, .5 * f, l));
    ...
    1 == b.type && b.turretIndex && renderTurret(0, 0, b.turretIndex, h ? iconSizeMult : 1, b.turRot - MathPI / 2 - c, e);
    e.restore()
}
function updateUnitPosition(a) {
    if (a.paths && a.paths.length) {
        ...
        void 0 != a.lastX && void 0 != a.lastY && (a.x = a.lastX,
        a.y = a.lastY)
}
function updatePath(a, d) {
    if (a.paths[1]) {
        ...
        var c = a.paths[1]
          , c = c - d;
        0 < c ? (a.x += d * MathCOS(a.dir),
        a.y += d * MathSIN(a.dir)) : (c *= -1,
        a.x += a.paths[1] * MathCOS(a.dir),
        a.y += a.paths[1] * MathSIN(a.dir),
        a.startTime += a.paths[1] / a.speed,
        a.paths.splice(0, 2),
        updatePath(a, c));
        a.lastX = a.x;
        a.lastY = a.y
...
function renderTurret(a, d, c, b, g, e) {
    ...
    if (!unitSprites[h]) {
        ...
        2 == c ? (c = 14 * b,
        k.strokeRect(0, -(c / 2.3), 1.9 * c, c / 1.15),
        k.fillRect(0, -(c / 2.3), 1.9 * c, c / 1.15),
        k.strokeRect(0, -(c / 2), 1.4 * c, c),
        k.fillRect(0, -(c / 2), 1.4 * c, c)) : 3 == c ? (c = 15 * b,
        k.strokeRect(0, -(c / 2), 2.2 * c, c),
        k.fillRect(0, -(c / 2), 2.2 * c, c)) : 4 == c ? (c = 17 * b,
        k.strokeRect(0, -(c / 2), 2.1 * c, c),
        k.fillRect(0, -(c / 2), 2.1 * c, c)) : 5 == c ? (c = 17 * b,
        k.strokeRect(0, -(c / 2.3), 2.2 * c, c / 1.15),
        k.fillRect(0, -(c / 2.3), 2.2 * c, c / 1.15),
        k.strokeRect(0, -(c / 2), 1.5 * c, c),
        k.fillRect(0, -(c / 2), 1.5 * c, c)) : 6 == c ? (c = 18 * b,
        k.strokeRect(0, -(c / 2), 2.1 * c, c),
        k.fillRect(0, -(c / 2), 2.1 * c, c),
        k.strokeRect(1.5 * c, -(c / 1.6), c / 1.4, c / 1.6 * 2),
        k.fillRect(1.5 * c, -(c / 1.6), c / 1.4, c / 1.6 * 2)) : 7 == c ? (c = 14 * b,
        k.strokeRect(0, -(c / 2), 2 * c, c),
        k.fillRect(0, -(c / 2), 2 * c, c),
        k.beginPath(),
        k.moveTo(0, 0),
        k.lineTo(2.1 * c, 0),
        k.stroke()) : 8 == c ? (c = 12 * b,
        k.strokeRect(0, -(c / 2), 1.8 * c, c),
        k.fillRect(0, -(c / 2), 1.8 * c, c)) : 9 == c ? (c = 20 * b,
        k.strokeRect(0, -(c / 2), 1.9 * c, c),
        k.fillRect(0, -(c / 2), 1.9 * c, c)) : 10 == c ? (c = 15 * b,
        k.strokeRect(0, -(c / 2), 2.6 * c, c),
        k.fillRect(0, -(c / 2), 2.6 * c, c),
        k.strokeRect(-7, 1.2 * -c, 14, 2.4 * c),
        k.fillRect(-7, 1.2 * -c, 14, 2.4 * c)) : (c = 14 * b,
        k.strokeRect(0, -(c / 2), 1.8 * c, c),
        k.fillRect(0, -(c / 2), 1.8 * c, c));
...
function renderPlayer(a, d, c, b, g) {
    ...
    a.skin && skinSprites[a.skin] && skinSprites[a.skin].readyToDraw ? (e = a.size - b.lineWidth / 4,
    b.drawImage(skinSprites[a.skin], d - e, c - e, 2 * e, 2 * e),
    b.lineWidth /= 2,
    renderCircle(d, c, a.size, b, !1, !0)) : g || (b.fillStyle = playerColors[a.color],
    renderCircle(d, c, a.size, b));
    b.restore()
}
...
function updateTarget() {
    player && !player.dead && (targetDir = MathATAN2(mouseY - screenHeight / 2, mouseX - screenWidth / 2),
    targetDst = MathSQRT(MathPOW(mouseY - screenHeight / 2, 2) + MathPOW(mouseX - screenWidth / 2, 2)),
    targetDst *= MathMIN(maxScreenWidth / screenWidth, maxScreenHeight / screenHeight))
}
...
function toggleMenuUI(a) {
    ...
    linksContainer.style.display = "block") : (menuContainer.style.display = "none",
