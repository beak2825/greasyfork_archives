...
const createToolboxContent = (options = { showFill: true, showConvert: true, showOutline: false }) => {
    const { showFill, showConvert, showOutline } = options;
    ...
    if (showOutline) {
        ...
        const renderTree = (filterText = '') => {
            ...
            forms.forEach(form => {
                ...
                // 匹配逻辑
                const matchForm = title.toLowerCase().includes(lowerFilter);
                let matchingFields = [];
    ...
    function switchTab(active) {
        const tabs = [fillTab, convertTab, outlineTab];
        const contents = [fillContent, convertContent, outlineContent];
        tabs.forEach(t => {
...
const buildUIbyDesignerPage = () => {
    ...
    const injectSidebar = async () => {
        ...
        // 获取内容
        const { tabHeader, contentArea, logTitle, logArea } = createToolboxContent({ showFill: false, showConvert: true, showOutline: true });
