// ==UserScript==
// @name         WME POIデータ取得
// @name:ja      WME 🇯🇵 POIデータ取得
// @version      0.1.0
// @description:ja POIに関する情報を外部ソースから取得します
// @license      MIT License
// @author       Anton Shevchuk (日本向け改変)
...
// @description 日本向けに外部ソースからPOI情報を取得します
// @downloadURL https://update.greasyfork.org/scripts/548654/WME%20POI%E3%83%87%E3%83%BC%E3%82%BF%E5%8F%96%E5%BE%97.user.js
// @updateURL https://update.greasyfork.org/scripts/548654/WME%20POI%E3%83%87%E3%83%BC%E3%82%BF%E5%8F%96%E5%BE%97.meta.js
// ==/UserScript==
...
(function () {
  ...
  class VisicomProvider extends Provider {
    ...
    async request (lon, lat) {
      ...
      let data = {
        ...
        order: 'distance',
        radius: 15, // Changed from 100 to 15 meters
        limit: 5,   // Changed from 10 to 5 results
        key: this.key,
      }
  ...
  class GisProvider extends Provider {
    ...
    async request (lon, lat) {
      ...
      let data = {
        point: lon + ',' + lat,
        radius: 15, // Changed from 20 to 15 meters
        type: 'building',
        fields: 'items.address,items.adm_div,items.geometry.centroid',
        locale: this.settings.locale,
        format: 'json',
        key: this.key,
        limit: 5, // Added to limit to 5 results
      }
  ...
  class GoogleProvider extends Provider {
    ...
    async request (lon, lat) {
      ...
      console.groupCollapsed(this.uid)
      let result = this.collection(response.slice(0, 5)) // Limit to top 5 results
      console.groupEnd()
      return result
    }
    ...
    async makeAPIRequest (lat, lon) {
      ...
      let request = {
        ...
        rankby: 'distance', // Added to prioritize closer results
      }
