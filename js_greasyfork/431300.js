...
// @namespace https://greasyfork.org/zh-CN/scripts/431300-pinterest-mobild-save-dailog-filter
// @downloadURL https://update.greasyfork.org/scripts/431300/pinterest_plus_for_mobile.user.js
// @updateURL https://update.greasyfork.org/scripts/431300/pinterest_plus_for_mobile.meta.js
// ==/UserScript==
GM_addStyle(`
...
function formatTime(s) {
    ...
    return _s1(h, ':') + _s(m, ':') + _s(parseInt(s), '');
}
    ...
    if (g_data[id]) {
        if (!btn1) {
            ...
            btn.onclick = (e) => {
                if (confirm('确定删除吗？')) {
                    delete g_data[id];
                    GM_setValue('data', g_data);
                    alert('删除成功');
                    e.srcElement.parentElement.remove();
                }
            }
            btn.innerHTML = `<button class="RCK Hsu USg adn CCY czT F10 xD4 fZz hUC a_A gpV hNT BG7 NTm KhY iyn" tabindex="0" type="button" style=""><div class="tBJ dyH iFc yTZ pBj tg7 mWe">删除记录</div></button>`;
            var old = document.querySelector('[data-test-id="board-follow-button"]');
            if (!old) old = document.querySelector('[data-test-id="board-unfollow-button"]');
...
setInterval(() => {
    ...
    if (!document.querySelector("#ftb")) {
        ...
        _['timer'] = setInterval(() => {
            _['time']++;
            ftb.innerText = formatTime(_['time']);
        }, 1000);
    ...
    var dialog = document.querySelector('.XbT.Zr3.hUC.zI7.iyn.Hsu');
    if (dialog) {
        if (!document.querySelector("#submitTask") && !document.querySelector("#historyList")) {
            var btn = document.createElement('div');
            btn.className = 'xuA';
            btn.id = 'submitTask';
            btn.onclick = () => {
                if (confirm('确定提交吗？')) {
                    g_data[cutString(location.href, 'https://www.pinterest.com/pin/', '/')] = {
                        img: document.querySelector('img[elementtiming="closeupImage"]').src,
                        time: _['add'] ? _['time'] : _['minute'] * 60 - _['time']
                    }
                    GM_setValue('data', g_data);
                    alert('提交成功');
                    checkRemoveButton();
            ...
            btn.innerHTML = `<div aria-disabled="false" class="CCY czT eEj iyn DI9 BG7" role="button" tabindex="0"><div class="tBJ dyH iFc yTZ pBj zDA IZT mWe">我已完成!</div></div>`;

            dialog.children[0].insertBefore(btn, dialog.children[0].children[0]);
            ...
            btn.onclick = () => {
                ...
                dialog.children[0].append(list);

                var btn_close = document.createElement('div');
                btn_close.style = 'position: absolute;right: 30px; top: 17px;';
                btn_close.onclick = () => {
                    if (confirm('确定清空吗？')) {
                        g_data = {};
                        GM_setValue('data', g_data);
                ...
                btn_close.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
  <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z"/>
  <path fill-rule="evenodd" d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z"/>
</svg>`;
                dialog.parentElement.children[0].append(btn_close);
            }
            btn.innerHTML = `<div aria-disabled="false" class="CCY czT eEj iyn DI9 BG7" role="button" tabindex="0"><div class="tBJ dyH iFc yTZ pBj zDA IZT mWe">查看列表</div></div>`;
    ...
    if (location.href.substr(-7) == '/repin/') {
        ...
        if (!document.querySelector("#searchInput")) {
            ...
            search.oninput = (e) => {
                var dir;
                var h = '';
                var word = e.srcElement.value.toLowerCase();
                _['matchs'] = [];

                if (word != '') {
                    for (var d of document.querySelectorAll(".tBJ.dyH.iFc.yTZ.pBj.zDA.IZT.swG")[1].parentNode.parentNode.querySelectorAll(".CCY.czT.DUt.DI9.BG7")) {
                        dir = d.innerText.toLowerCase();
                        if (dir == word || PinYinTranslate.start(dir) == word || dir.indexOf(word) != -1) {
                            _['matchs'].push(d);
                        }
                    }
                }
                var list = document.querySelector("#searchResult");
                list.innerHTML = '';
                for (var dom of _['matchs']) {
                    var d = document.createElement('div');
                    d.style = "text-align: center;padding: 10px;margin-bottom: 10px;";
...
var PinYinTranslate = {
    ...
    start: function(a) {
        ...
        for (var i = 0; i < l2; i++) {
            var val = a.substr(i, 1);
            var name = that.arraySearch(val, that.PinYin);
            if (reg.test(val)) {
                I1 += val;
            } else if (name !== false) {
                I1 += name;
