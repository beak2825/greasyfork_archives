...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/543646/Modern%20Roblox%20Blue.user.js
// @updateURL https://update.greasyfork.org/scripts/543646/Modern%20Roblox%20Blue.meta.js
// ==/UserScript==
...
(function() {
    ...
    function handleVisibilityChange() {
        if (document.hidden) {
            document.querySelector("link[rel='icon']").href = "https://www.roblox.com/favicon.ico";
        } else {
            document.querySelector("link[rel='icon']").href = "https://tr.rbxcdn.com/180DAY-d3df05db03bbfdf5d3146f83bec4dfbb/420/420/Decal/Webp/noFilter";
        }
    ...
    if (currentUrl === 'https://www.roblox.com/discover#/sortName/v2/Favorites' || currentUrl === 'https://www.roblox.com/charts#/sortName/v2/Favorites') {
        ...
        window.onload = function() {
            ...
            if (document.getElementById('versionid')) {
                needsupdate = document.getElementById('versionid').innerText != versionid;
            } else {
                needsupdate = false;
            }
    ...
    window.onload = function() {
        ...
        setTimeout(function () {
            ...
            const elements = Array.from(document.querySelectorAll('body *')).sort((a, b) => a.children.length - b.children.length);

            const changed = new Set();

            elements.forEach(el => {
                if (changed.has(el) || !/connections/i.test(el.textContent)) return;
                const hasChildWithMatch = Array.from(el.querySelectorAll('*')).some(child => /connections/i.test(child.textContent));
                if (!hasChildWithMatch) {
                    console.log('Element:', el);
                    el.innerHTML = el.innerHTML.replace(/connections/gi, match =>
                                                        match[0] === 'C' ? 'Friends' : 'friends'
                                                       );
                    changed.add(el);
                }
            });
