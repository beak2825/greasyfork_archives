...
// @description:en    Display the marker of page scroll in InoReader.
// @description:ja    InoReaderでページスクロールしたときにマーカーを表示します。
// @id    InoReaderScrollMarker
// @namespace    http://userscripts.org/scripts/show/173921
// @homepage    https://greasyfork.org/scripts/894-inoreader-scroll-marker
...
// @version    1.13
// @grant    GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/894/InoReader%20Scroll%20Marker.user.js
// @updateURL https://update.greasyfork.org/scripts/894/InoReader%20Scroll%20Marker.meta.js
// ==/UserScript==
...
var init = function() {
  var ismRp = document.createElement('div'),
    ismTwc = document.createElement('div'),
    nScrPos = 0,
    nCheck = 0,
    nTime = 750,
    shift = false,
  ...
  ismRp.id = 'irsm_scroll_marker_rp';
  ismTwc.id = 'irsm_scroll_marker_twc';
  ismRp.className = 'irsm_scroll_marker';
  ismTwc.className = 'irsm_scroll_marker';
  $id('reader_pane').insertBefore(ismRp, $id('reader_pane').firstChild);
  new MutationObserver(function() {
    var rp = $id('reader_pane');
    if (!$id('irsm_scroll_marker_rp') && rp && rp.childNodes.length) {
      rp.insertBefore(ismRp, rp.firstChild);
    }
  }).observe($id('reader_pane'), {
    childList: true
  });
  new MutationObserver(function() {
    var twcafc = document.querySelector('#three_way_contents > .article_full_contents:last-child');
    if (!$id('irsm_scroll_marker_twc') && twcafc && twcafc.childNodes.length) {
      twcafc.insertBefore(ismTwc, twcafc.firstChild);
    }
  }).observe($id('three_way_contents'), {
    childList: true
  });
  var CSS = '#irsm_settings { display: none; color: black; position: absolute; top: 68px; left: 68px; z-index: 90200; background: rgba(255, 255, 255, 0.98); border: 1px solid #999999; border-radius: 4px; box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2); -moz-user-select: none; -webkit-user-select: none; }' +
    '#irsm_s_titlebar { background-color: #666666; border-radius: 4px 4px 0 0; padding: 2px 0 0 4px; height: 2em; min-width: 20em; }' +
    ...
    '.irsm_scroll_marker { position: absolute; left: 0; width: 100%; height: 4px; margin-top: -2px; background: rgba(255,0,0,0.25); display: none; z-index: 9000; }' +
    '.irsm_s_hide { display: none; }' +
    '#three_way_contents .article_full_contents { padding: 4px 0; }' +
    '#three_way_contents .article_footer { display: none; box-shadow: 0 2px 4px 0 #333; width: auto; padding-left: 20px; padding-right: 20px; }' +
    '#subscriptions_buttons:hover + #splitter #three_way_contents .article_footer, #three_way_contents .article_footer:hover { display: block; }';
  GM_addStyle(CSS);
  ...
  var currentEntry = function() {
    if ($id('article_dialog')) {
      return document.querySelector('#article_dialog > .article_full_contents');
    }
    if ($id('three_way_contents') && $id('three_way_contents').style.display !== 'none' && $id('reader_pane').getElementsByClassName('article_current article_current_3way')[0]) {
      return document.querySelector('#three_way_contents > .article_full_contents:last-child');
    }
    if ($id('subscriptions_articles')) {
      return document.querySelector('#subscriptions_articles > .article_current');
    }
    return document.querySelector('#reader_pane .article_current');
  };
  ...
  $id('irsm_settings').innerHTML = '<div id="irsm_s_titlebar"><div id="irsm_s_title"><a href="https://greasyfork.org/scripts/894-inoreader-scroll-marker" target="_blank">InoReader Scroll Marker ' + loc[0] + '</a></div><div id="irsm_s_btn"><input type="button" id="irsm_s_ok" value="OK"><input type="button" id="irsm_s_cancel" value="' + loc[1] + '"></div></div><div id="irsm_s_body"><label>' + loc[2] + '<input id="irsm_s_time" type="text" maxlength="4"></label><fieldset><legend>' + loc[3] + ' : </legend><label><input id="irsm_s_key_page" type="checkbox">PageUp / PageDown</label><br><label><input id="irsm_s_key_space" type="checkbox">Space / Shift+Space</label></fieldset></div>';
  ...
  document.addEventListener('keydown', function(e) {
    ...
    if (eRp && !/^input|^textarea/i.test(e.target.tagName) && st.time !== 0) {
      if (
        ...
        ismRp.style.display = 'none';
        ismTwc.style.display = 'none';
        nScrPos = (eTwc.offsetParent) ? eTwc.scrollTop : eRp.scrollTop;
      ...
      } else if (ismRp.style.display !== 'none') {
        window.clearTimeout(iMarker);
        ismRp.style.display = 'none';
      } else if (ismTwc.style.display !== 'none') {
        window.clearTimeout(iMarker);
        ismTwc.style.display = 'none';
      }
    }
  ...
  document.addEventListener('keyup', function(e) {
    var eRp = $id('reader_pane'),
      ...
      sst = 0;
    if (eRp && !/^input|^textarea/i.test(e.target.tagName) && st.time !== 0) {
      var bTwc = (eTwc.offsetParent) ? true : false;
      soh = (bTwc) ? eTwc.offsetHeight : eRp.offsetHeight;
      sst = (bTwc) ? eTwc.scrollTop : eRp.scrollTop;
      if (
        (st.key_page && e.keyCode === 33) ||
        ...
        if (nScrPos !== sst) {
          ...
          if (bTwc) {
            ismTwc.style.display = 'block';
            ismTwc.style.top = (nScrPos - (ismTwc.scrollHeight / 2)) + 'px';
          } else {
            ismRp.style.display = 'block';
            ismRp.style.top = (nScrPos - (ismRp.scrollHeight / 2)) + 'px';
          }
          window.clearTimeout(iMarker);
          if (st.time > 0) {
        ...
        if (nScrPos !== sst) {
          ...
          if (bTwc) {
            ismTwc.style.display = 'block';
            ismTwc.style.top = (nScrPos + soh - (ismTwc.scrollHeight / 2)) + 'px';
          } else {
            ismRp.style.display = 'block';
            ismRp.style.top = (nScrPos + soh - (ismRp.scrollHeight / 2)) + 'px';
          }
          window.clearTimeout(iMarker);
          if (st.time > 0) {
            iMarker = window.setTimeout(function() {
