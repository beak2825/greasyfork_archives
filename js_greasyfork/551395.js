...
// @downloadURL https://update.greasyfork.org/scripts/551395/SOOP%20%ED%81%B4%EB%A6%BD%20%EB%B0%8F%20%EB%9D%BC%EC%9D%B4%EB%B8%8C%20%EA%B9%94%EB%81%94%ED%95%98%EA%B2%8C%20%EC%BA%A1%EC%B3%90.user.js
// @updateURL https://update.greasyfork.org/scripts/551395/SOOP%20%ED%81%B4%EB%A6%BD%20%EB%B0%8F%20%EB%9D%BC%EC%9D%B4%EB%B8%8C%20%EA%B9%94%EB%81%94%ED%95%98%EA%B2%8C%20%EC%BA%A1%EC%B3%90.meta.js
// ==/UserScript==
...
(function() {
    ...
    const configDesc = {
        ...
        shownUIs:{
            ...
            title: "잠금 상태에서 숨기지 않을 UI를 선택합니다. 이걸 어떻게 설정하든, 캡쳐 버튼을 누른 순간은 모든 방해요소가 숨겨집니다",
            type: "folder",
            items: {
                hideControl: {
                ...
                hideCatchUI: {
                    name: "Catch making UI(wip)",
                    title: "Catch 제작 시, 제외되는 영역을 어둡게 하는 요소입니다.",
                    type: "enum",
                    options: ["Hide", "Show"],
                    default: 1
                }
    ...
    function main(videoLayer, ctrlBox, titleElement, nicknameElement, playerInfo, viewCtrl) {
        ...
        config.addEventListener("set", (_e) => setLockState(isLocked));  // update UI when config changes
