...
// @downloadURL https://update.greasyfork.org/scripts/5091/labStartProjectFilter.user.js
// @updateURL https://update.greasyfork.org/scripts/5091/labStartProjectFilter.meta.js
// ==/UserScript==
...
// @include http://ru.virtonomics.*/*/main/unit/view/*/investigation
        //для определения изучаемой в открытой лабе технологии
        if (/http:\/\/*virtonomics\.\w+\/\w+\/main\/unit\/view\/\w+\/investigation/.test(window.location)) {
        ...
        if (/http:\/\/ru.virtonomics\.\w+\/\w+\/main\/management_action\/\d+\/investigations\/technologies[#]{0,1}/.test(window.location)) {
            var arr = new Array();
        ...
        if (/http:\/\/ru.virtonomics\.\w+\/\w+\/window\/unit\/view\/\d+\/project_create$/.test(window.location)) {
            var arr = StrToMultDimArr( $.Storage.get('labStartFilterArray') );
        ...
        if (/http:\/\/ru.virtonomics.\w+\/\w+\/main\/company\/view\/\d+\/unit_list$/.test(window.location)) {
