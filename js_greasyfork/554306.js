...
// @downloadURL https://update.greasyfork.org/scripts/554306/Missav%E5%AD%97%E5%B9%95%E5%8A%A0%E8%BD%BD%E5%A2%9E%E5%BC%BA%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/554306/Missav%E5%AD%97%E5%B9%95%E5%8A%A0%E8%BD%BD%E5%A2%9E%E5%BC%BA%E7%89%88.meta.js
// ==/UserScript==
...
(function () {
    ...
    GM_addStyle(`
        ...
        .custom-control-panel .button-group {
            ...
            text-align: center; /* Center buttons */
             display: flex;  /* Make button more flexible */
    ...
    function createControlPanel() {
        ...
        // --- Time Skip Buttons ---
        const timeSkips = [5, 10, 30]; // Seconds
        const minuteSkips = [5, 7, 12]; // Minutes

        const createTimeSkipButton = (seconds, direction) => {
            const text = `${direction === 'backward' ? '-' : '+'}${seconds}s`;
            const button = document.createElement('button');
            button.textContent = text;
            button.onclick = () => skipTimeHandler(seconds, direction);
            return button;
        };

        const createMinuteSkipButton = (minutes, direction) => {
            const text = `${direction === 'backward' ? '-' : '+'}${minutes}m`;
            const button = document.createElement('button');
            button.textContent = text;
            button.onclick = () => skipTimeHandler(minutes * 60, direction); // Convert minutes to seconds
            return button;
        };

        // Add the buttons directly to the buttonContainer
        const skipButtonsContainer = document.createElement('div'); // Container for skip buttons
        //skipButtonsContainer.style.display = 'flex'; // Align items horizontally


        // 后退按钮
        timeSkips.forEach(sec => {
            skipButtonsContainer.appendChild(createTimeSkipButton(sec, 'backward'));
        });
        minuteSkips.forEach(min => {
            skipButtonsContainer.appendChild(createMinuteSkipButton(min, 'backward'));
        });
        // 前进按钮
        timeSkips.forEach(sec => {
            skipButtonsContainer.appendChild(createTimeSkipButton(sec, 'forward'));
        });
        minuteSkips.forEach(min => {
            skipButtonsContainer.appendChild(createMinuteSkipButton(min, 'forward'));
        });
        skipButtonsContainer.style.display = 'block';

        buttonContainer.appendChild(skipButtonsContainer); // Append skip buttons to main button container
    ...
    function skipTimeHandler(seconds, direction) {
        if (!plyrInstance || typeof plyrInstance.currentTime !== 'number') return;
        ...
        try {
            if (direction === 'forward') {
                ...
                plyrInstance.currentTime = Math.max(0, plyrInstance.currentTime - seconds);
            }
        } catch (err) {
    ...
    function initializeScript() {
        console.log("Missav Script: Initializing v1.7...");
