...
// @downloadURL https://update.greasyfork.org/scripts/555733/OLM%20Helper.user.js
// @updateURL https://update.greasyfork.org/scripts/555733/OLM%20Helper.meta.js
// ==/UserScript==

(function () {
  "use strict";

  const OLM_API_CONFIG = {
    publicApiUrl: "https://cavoixanh1806.site/olmhelper/api_check_key.php",
    lanApiUrl: "http://192.168.1.8/olmhelper/api_check_key.php",
    devMode: false // true = luôn ưu tiên LAN API
  };
  ...
  function getApiUrl() {
    const cfg = OLM_API_CONFIG || {};
    if (cfg.devMode) return cfg.lanApiUrl || cfg.publicApiUrl || "olmhelper/api_check_key.php";
    if (isLanHostname(location.hostname) && cfg.lanApiUrl) return cfg.lanApiUrl;
    return cfg.publicApiUrl || cfg.lanApiUrl || "olmhelper/api_check_key.php";
  }

  const Config = {
    API_ENDPOINT: getApiUrl(),
    API_FALLBACKS: [
      OLM_API_CONFIG.lanApiUrl,
      OLM_API_CONFIG.publicApiUrl,
      "olmhelper/api_check_key.php"
    ].filter(Boolean), // thử thêm đường dẫn cục bộ nếu máy chủ không ra internet
    DEFAULT_KEY: "",
  ...
  const getApiEndpoints = () => {
    ...
    pushUnique(getApiUrl());
    pushUnique(OLM_API_CONFIG.lanApiUrl);
    pushUnique(OLM_API_CONFIG.publicApiUrl);
    if (Array.isArray(Config.API_FALLBACKS)) Config.API_FALLBACKS.forEach(pushUnique);
    if (!endpoints.length) pushUnique("olmhelper/api_check_key.php");
    return endpoints;
  };
  ...
  const AuthService = {
    ...
    init() {
      ...
      this.lastKeyCheck = Number(GM_getValue("lastKeyCheck", 0));
    ...
    async processValidation(key, isRevalidation = false) {
      ...
      for (const endpoint of endpoints) {
        ...
        if (result.ok) {
          ...
          GM_setValue("lastKeyCheck", this.lastKeyCheck);
          return true;
        }
        if (!result.retry) {
          this.markInvalid();
      ...
      if (isRevalidation) {
        console.warn("[OLM Helper] Re-validation failed (network), trusting cache.");
        return true;
      }
    ...
    async checkCachedKey() {
      ...
      if (this.currentKey && cachedStatus === "valid" && this.keyExpiresAt && this.keyExpiresAt > Date.now()) {
        // Re-validate if > 5 minutes have passed
        if (Date.now() - this.lastKeyCheck > 5 * 60 * 1000) {
          console.log("[OLM Helper] Re-validating key (5m interval)...");
          const stillValid = await this.processValidation(this.currentKey, true);
      ...
      if (this.keyExpiresAt && this.keyExpiresAt <= Date.now()) {
        this.markInvalid({ clearStoredKey: true });
        return false;
      }
      if (this.currentKey) {
    ...
    markInvalid(options = {}) {
      const { clearStoredKey = false } = options || {};
      if (!isKeySystemEnabled()) {
        this.applyBypassState(this.currentKey);
        this.notifyListeners();
        return;
  ...
  const CAVOIXANH_LINK = "https://cavoixanh1806.github.io/-/";
  ...
  const LS_PAGE_ZOOM = "olm_page_zoom"; // legacy key for panel zoom
  const LS_WEB_ZOOM = "olm_web_zoom";
  ...
  const PANEL_MIN_ZOOM = 0.6;
  const PANEL_MAX_ZOOM = 1.8;
  const PANEL_ZOOM_STEP = 0.1;
  const WEB_MIN_ZOOM = 0.5;
  const WEB_MAX_ZOOM = 2;
  const WEB_ZOOM_STEP = 0.1;
  ...
  // Override duration formatters to include days and years
  function formatDuration(ms) {
    const totalSeconds = Math.max(0, Math.floor(((ms || 0)) / 1000));
    if (totalSeconds === 0) return "0 giây";
  ...
  function parseDurationToMs(value) {
    ...
    let minutes = 0;
    let matched = false;
    normalized.replace(/(\d+(?:\.\d+)?)\s*(gi(?:ờ|o)|h)/g, (_, num) => {
      minutes += parseFloat(num) * 60;
      matched = true;
      return "";
    });
    normalized.replace(/(\d+(?:\.\d+)?)\s*(ph(?:ú|u)t|p|min)/g, (_, num) => {
      minutes += parseFloat(num);
      matched = true;
      return "";
    });
    if (!matched && /^\d+(?:\.\d+)?$/.test(normalized)) {
      minutes = parseFloat(normalized);
      matched = true;
    }
    if (!matched) return 0;
  ...
  class AnswerDisplay {
    constructor() {
      ...
      this.timeDisplays = new Set();
      this.panelZoom = this.loadStoredPanelZoom();
      this.webZoom = this.loadStoredWebZoom();
      this.webZoomIndicator = null;
      this.panelZoomIndicator = null;
      // Auto Search (integrated with Unlock Copy)
      this.lastSelectionText = "";
      this.autoSearchHandler = null;

      // Controls row drag scroll
      this.controlsScroll = { dragging: false, startX: 0, scrollLeft: 0, moved: false };
      this.controlsDragPreventClick = false;

      // Auto Solve (Auto Highlight) properties
      ...
      this.handleSelectionChangeForUnlockCopy = this.handleSelectionChangeForUnlockCopy.bind(this);
      ...
      this.scrollBlockDepth = 0;
    }

    getPanelScale() {
      const val = this.panelZoom;
      return Number.isFinite(val) && val > 0 ? val : 1;
    }

    getWebScale() {
      const z = Number(this.webZoom);
      if (Number.isFinite(z) && z > 0) return z;
      const htmlZoom = Number(document.documentElement?.style?.zoom);
      return Number.isFinite(htmlZoom) && htmlZoom > 0 ? htmlZoom : 1;
    }

    init() {
      this.injectCSS();
      this.createUI();
      this.applyPanelZoom(this.panelZoom, { save: false });
      this.applyWebZoom(this.webZoom, { save: false });
    ...
    applyStoredToggleStates() {
      this.setCopyUnlockEnabled(this.copyUnlockEnabled, { save: false, force: true });
      this.setFullscreenBypassEnabled(this.fullscreenBypassEnabled, { save: false, force: true });
    }
    ...
    injectCSS() {
      const css = `
        ...
        #olm-answers-container{
          ...
          transform: scale(var(--olm-panel-zoom, 1));
        }
        ...
        .panel-zoom-btn + .panel-zoom-btn{
          border-left:1px solid rgba(15,23,42,0.12);
        }
        #olm-answers-container.olm-dark .panel-zoom-buttons{
          border-color:rgba(255,255,255,0.15);
        }
        #olm-answers-container.olm-dark .panel-zoom-btn + .panel-zoom-btn{
          border-left-color:rgba(255,255,255,0.15);
        }
        .panel-zoom-btn:active{ transform: scale(0.95); }
        .panel-zoom-indicator{
          font-size:11px;
          font-weight:700;
          color:var(--muted);
          min-width:40px;
        ...
        .olm-controls-row::-webkit-scrollbar{
          height: 6px;
        }
        .olm-controls-row::-webkit-scrollbar-track{
          background: transparent;
        }
        .olm-controls-row::-webkit-scrollbar-thumb{
          background: rgba(0,0,0,0.2);
          border-radius: 3px;
        }
        .olm-controls-row::-webkit-scrollbar-thumb:hover{
        ...
        #olm-answers-container .olm-btn.is-locked{
          opacity: 0.6;
          ...
          background: linear-gradient(135deg, rgba(209, 213, 219, 0.1), rgba(229, 231, 235, 0.15)) !important;
          transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
          animation: lockedPulse 2s ease-in-out infinite;
        }
        #olm-answers-container .olm-btn.is-locked::before{
          content: "";
          position: absolute;
          inset: 0;
          ...
          background: repeating-linear-gradient(
            ...
            transparent,
            transparent 4px,
            rgba(209, 213, 219, 0.1) 4px,
            rgba(209, 213, 219, 0.1) 8px
          );
          opacity: 0.3;
          pointer-events: none;
        }
        #olm-answers-container .olm-btn.is-locked::after{
          content: "🔒";
          font-size: 10px;
          position: absolute;
          top: 2px;
        ...
        #olm-answers-container.olm-dark .olm-btn.is-locked{
          border-color: rgba(255, 255, 255, 0.2) !important;
          background: linear-gradient(135deg, rgba(255, 255, 255, 0.05), rgba(255, 255, 255, 0.08)) !important;
        }
        #olm-answers-container.olm-dark .olm-btn.is-locked::before{
          background: repeating-linear-gradient(
            45deg,
            transparent,
            transparent 4px,
        ...
        #olm-answers-container.olm-dark .olm-btn.is-locked::after{
          color: #f97316;
        }
        ...
        /* Auto Highlight, Unlock Copy, Anti FS buttons: blue color when active */
        #olm-answers-container .olm-btn.is-active{
          border-color: #3b82f6;
          color: #2563eb;
          animation: olm-blink 1.4s infinite;
        }
        #olm-answers-container.olm-dark .olm-btn.is-active{
          color: #60a5fa;
          border-color: #60a5fa;
        }
        ...
        /* Hover animation for toggle buttons (glow-toggle) when OFF */
        #olm-answers-container .olm-btn.glow-toggle:not(.is-active):not(.is-alert):not(.is-locked):hover{
          border-color: #3b82f6;
          color: #2563eb;
          box-shadow: 0 0 12px rgba(59, 130, 246, 0.4), 0 4px 8px rgba(59, 130, 246, 0.2);
          transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
          transform: translateY(-1px);
        }
        #olm-answers-container.olm-dark .olm-btn.glow-toggle:not(.is-active):not(.is-alert):not(.is-locked):hover{
          border-color: #60a5fa;
          color: #60a5fa;
          box-shadow: 0 0 12px rgba(96, 165, 250, 0.4), 0 4px 8px rgba(96, 165, 250, 0.2);
          transform: translateY(-1px);
        }
        ...
        .search-wrap{ display:flex; gap:8px; align-items:center; padding:6px 12px; border-bottom:1px solid rgba(0,0,0,0.06); background: var(--bg-sub); touch-action: pan-y; }
        #olm-answers-container.olm-dark .search-wrap{ border-bottom-color: rgba(255,255,255,0.06); }
        .search-field-group{ flex:1; display:flex; gap:6px; align-items:center; }
        .search-input{ flex:1; padding:6px 9px; border-radius:10px; border:1px solid rgba(0,0,0,0.06); outline:none; background: rgba(255,255,255,0.85); font-size:13px; color:#111827; }
        ...
        .search-btn-stack{ display:flex; flex-direction:column; gap:3px; align-self:stretch; justify-content:center; }
        .search-mini-btn{
          width:28px;
          height:22px;
          border-radius:8px;
        ...
        .search-mini-btn:active{ transform: translateY(1px); }
        .meta{ font-size:12px; color: var(--muted); min-width:74px; text-align:right; align-self:center; }
        .answer-copy-btn{
          margin-left:8px;
          padding:3px 8px;
          border-radius:6px;
        ...
        .answer-copy-btn:active{ transform: translateY(1px); }
        ...
        .footer-bar{ padding:8px 10px; display:flex; align-items:center; gap:8px; border-top:1px solid rgba(0,0,0,0.06); background: var(--bg-sub); touch-action: pan-y; }
        ...
        @media (max-width: 520px){
          #olm-answers-container{
            ...
            max-width: calc(100vw - 24px);
            height: min(66vh, calc(100vh - 24px));
            max-height: calc(100vh - 24px);
          }
      ...
      /* Khoảng cách giữa các câu */
          ...
          /* Auto highlight styles for main page */
          .olm-auto-highlight-correct{
          ...
          .olm-auto-highlight-correct::before{
            content: "✓ " !important;
            color: #16a34a !important;
            font-weight: bold !important;
            margin-right: 4px !important;
          }
          /* For new structure: highlight the qselect div */
          div.qselect.olm-auto-highlight-correct{
            background: #dcfce7 !important;
            border: 2px solid #16a34a !important;
            border-radius: 6px !important;
          ...
          div.qselect.olm-auto-highlight-correct .qsign{
            color: #16a34a !important;
            font-weight: 600 !important;
          }
          /* True/False question highlighting - only highlight clause text, not the yellow box */
          .true-false .tf-row td:first-child .clause.olm-tf-correct,
          .true-false .tf-row td:first-child.olm-tf-correct{
            background-color: #d4edda !important;
            border-radius: 4px !important;
            padding: 2px 4px !important;
          }
          /* Do NOT highlight the yellow box - keep original styling */
          .true-false .qselect.olm-auto-highlight-correct-tf{
            /* Remove previous highlighting of qselect */
          }
          .true-false .qselect.olm-auto-highlight-correct-tf .qiradio{
            /* Remove previous highlighting of qiradio */
          }
      `;
    ...
    createUI() {
      ...
      const panelZoomWrap = document.createElement("div");
      panelZoomWrap.className = "panel-zoom-wrap";
      const panelZoomButtons = document.createElement("div");
      panelZoomButtons.className = "panel-zoom-buttons";
      const panelZoomOutBtn = document.createElement("button");
      panelZoomOutBtn.type = "button";
      panelZoomOutBtn.className = "panel-zoom-btn";
      panelZoomOutBtn.textContent = "-";
      panelZoomOutBtn.title = "Thu nhỏ bảng đáp án";
      panelZoomOutBtn.addEventListener("click", () => this.adjustPanelZoom(-PANEL_ZOOM_STEP));
      const panelZoomResetBtn = document.createElement("button");
      panelZoomResetBtn.type = "button";
      panelZoomResetBtn.className = "panel-zoom-btn";
      panelZoomResetBtn.textContent = "R";
      panelZoomResetBtn.title = "Đưa bảng về kích thước mặc định";
      panelZoomResetBtn.addEventListener("click", () => this.resetPanelZoom());
      const panelZoomInBtn = document.createElement("button");
      panelZoomInBtn.type = "button";
      panelZoomInBtn.className = "panel-zoom-btn";
      panelZoomInBtn.textContent = "+";
      panelZoomInBtn.title = "Phóng to bảng đáp án";
      panelZoomInBtn.addEventListener("click", () => this.adjustPanelZoom(PANEL_ZOOM_STEP));
      panelZoomButtons.append(panelZoomOutBtn, panelZoomResetBtn, panelZoomInBtn);
      const panelZoomIndicator = document.createElement("span");
      panelZoomIndicator.className = "panel-zoom-indicator";
      panelZoomIndicator.textContent = "100%";
      panelZoomWrap.append(panelZoomButtons, panelZoomIndicator);
      const stopTopbarDrag = (evt) => evt.stopPropagation();
      panelZoomWrap.addEventListener("pointerdown", stopTopbarDrag);
      panelZoomWrap.addEventListener("mousedown", stopTopbarDrag);
      panelZoomWrap.addEventListener("touchstart", stopTopbarDrag);
      header.append(panelZoomWrap);
      this.panelZoomIndicator = panelZoomIndicator;
      this.updatePanelZoomIndicator();
      ...
      copyUnlockBtn.className = "olm-btn glow-toggle";
      ...
      fullscreenBypassBtn.className = "olm-btn glow-toggle";
      ...
      zoomGroup.className = "zoom-control";
      const zoomOutBtn = document.createElement("button");
      zoomOutBtn.type = "button";
      zoomOutBtn.className = "olm-btn zoom-btn";
      zoomOutBtn.textContent = "Thu nhỏ";
      zoomOutBtn.title = "Giảm tỷ lệ toàn bộ trang web";
      zoomOutBtn.addEventListener("click", () => this.adjustWebZoom(-WEB_ZOOM_STEP));
      const zoomResetBtn = document.createElement("button");
      zoomResetBtn.type = "button";
      zoomResetBtn.className = "olm-btn zoom-btn";
      zoomResetBtn.textContent = "100%";
      zoomResetBtn.title = "Đưa trang web về kích thước chuẩn";
      zoomResetBtn.addEventListener("click", () => this.resetWebZoom());
      const zoomInBtn = document.createElement("button");
      zoomInBtn.type = "button";
      zoomInBtn.className = "olm-btn zoom-btn";
      zoomInBtn.textContent = "Phóng to";
      zoomInBtn.title = "Phóng to toàn bộ trang web";
      zoomInBtn.addEventListener("click", () => this.adjustWebZoom(WEB_ZOOM_STEP));
      const zoomIndicator = document.createElement("span");
      zoomIndicator.className = "zoom-indicator";
      zoomIndicator.textContent = "100%";
      zoomIndicator.title = "Nhấn để nhập tỷ lệ zoom trang web";
      zoomIndicator.tabIndex = 0;
      const handleZoomIndicatorInput = () => {
        const current = Math.round((this.webZoom || 1) * 100);
        const input = prompt("Nhập phần trăm phóng to web (40-200):", String(current));
        if (!input) return;
        const value = Number(input);
        if (!Number.isFinite(value)) return;
        const normalized = Math.min(200, Math.max(40, value));
        this.applyWebZoom(normalized / 100);
      };
      zoomIndicator.addEventListener("click", handleZoomIndicatorInput);
      zoomIndicator.addEventListener("keydown", (evt) => {
        if (evt.key === "Enter" || evt.key === " ") {
          evt.preventDefault();
          handleZoomIndicatorInput();
        }
      });
      zoomGroup.append(zoomOutBtn, zoomResetBtn, zoomInBtn, zoomIndicator);
      this.webZoomIndicator = zoomIndicator;
      this.updateWebZoomIndicator();
      ...
      autoSolveBtn.textContent = "Auto Highlight";
      ...
      controlsRow.append(autoSolveBtn, darkBtn, collapseBtn, copyUnlockBtn, fullscreenBypassBtn, refreshBtn, zoomGroup, exportTxtBtn, exportPdfBtn, copyAnswersBtn, exportDocBtn, exportDocV2Btn, keyMenuBtn);

      // Enable drag to scroll on controls row
      ...
      const searchFieldGroup = document.createElement("div"); searchFieldGroup.className = "search-field-group";
      ...
      const searchBtnStack = document.createElement("div"); searchBtnStack.className = "search-btn-stack";
      const clearSearchBtn = document.createElement("button");
      clearSearchBtn.type = "button";
      clearSearchBtn.className = "search-mini-btn";
      clearSearchBtn.title = "Xóa nội dung tìm kiếm";
      clearSearchBtn.textContent = "X";
      clearSearchBtn.addEventListener("click", () => this.clearSearchInput());
      const pasteSearchBtn = document.createElement("button");
      pasteSearchBtn.type = "button";
      pasteSearchBtn.className = "search-mini-btn";
      pasteSearchBtn.title = "Dán nội dung đã sao chép";
      pasteSearchBtn.textContent = "Dán";
      pasteSearchBtn.addEventListener("click", () => this.pasteSearchFromClipboard());
      searchBtnStack.append(clearSearchBtn, pasteSearchBtn);
      searchFieldGroup.append(searchInput, searchBtnStack);
      const meta = document.createElement("div"); meta.className = "meta"; meta.id = "meta-info"; meta.textContent = "0 câu";
      searchWrap.append(searchFieldGroup, meta);
      searchWrap.addEventListener("pointerdown", this.onPointerDownDrag);
      ...
      footer.addEventListener("pointerdown", this.onPointerDownDrag);
      ...
      this.searchPasteBtn = pasteSearchBtn;
    ...
    buildInlineKeyPanel() {
      ...
      if (refs.activateBtn) {
        if (!refs.activateBtn.dataset.defaultLabel) {
          refs.activateBtn.dataset.defaultLabel = (refs.activateBtn.textContent || "").trim() || "Kích hoạt";
        }
        if (!refs.activateBtn.dataset.state) {
          refs.activateBtn.dataset.state = "idle";
        }
      }
    ...
    updateInlineKeyUI() {
      ...
      const { info, keySystemOn, hasUnlimitedTtl, label, remaining, isActive } = this.getKeyUiState();
      const isLocked = keySystemOn && isActive;
      if (this.keyPanelRefs.input) {
        const input = this.keyPanelRefs.input;
        input.disabled = !keySystemOn;
        input.readOnly = isLocked;
        if (isLocked || document.activeElement !== input) {
          input.value = AuthService.currentKey || "";
        }
        input.classList.toggle("is-locked", isLocked);
      }
      if (this.keyPanelRefs.activateBtn) {
        const btn = this.keyPanelRefs.activateBtn;
        const defaultLabel = btn.dataset.defaultLabel || "Kích hoạt";
        btn.disabled = !keySystemOn || isLocked;
        btn.dataset.state = isLocked ? "locked" : "idle";
        btn.textContent = isLocked ? "Đã kích hoạt" : defaultLabel;
      }

      this.keyPanelRefs.statusLabel.textContent = label;
      const remainingDisplay = this.formatKeyRemainingDisplay(info, keySystemOn);
      this.keyPanelRefs.statusRemaining.textContent = remainingDisplay;
    ...
    async handleInlineKeySubmit() {
      ...
      if (
        this.keyPanelRefs.activateBtn.dataset.state === "locked" &&
        AuthService.isKeyValid &&
        AuthService.getRemainingTimeMs() > 0
      ) {
        alert("Key đã kích hoạt. Vui lòng xóa key hoặc đợi hết hạn để nhập key mới.");
        return;
      }
      ...
      const btn = this.keyPanelRefs.activateBtn;
      const defaultLabel = btn.dataset.defaultLabel || btn.textContent || "Kích hoạt";
      btn.disabled = true;
    ...
    toggleCopyUnlock() {
      if (this.copyUnlockBtn?.disabled) return;
      this.setCopyUnlockEnabled(!this.copyUnlockEnabled);
    }

    setCopyUnlockEnabled(state, options = {}) {
      const next = !!state;
      const { save = true, force = false } = options;
      if (!force && this.copyUnlockEnabled === next) return;
      this.copyUnlockEnabled = next;
      this.syncCopyUnlockState();
      if (save) {
        try { localStorage.setItem(LS_COPY_UNLOCK, next ? "1" : "0"); } catch { }
      }
    }
    ...
    toggleFullscreenBypass() {
      if (this.fullscreenBypassBtn?.disabled) return;
      this.setFullscreenBypassEnabled(!this.fullscreenBypassEnabled);
    }

    setFullscreenBypassEnabled(state, options = {}) {
      const next = !!state;
      const { save = true, force = false } = options;
      if (!force && this.fullscreenBypassEnabled === next) return;
      this.fullscreenBypassEnabled = next;
      this.syncFullscreenBypassState();
      if (save) {
        try { localStorage.setItem(LS_FS_BYPASS, next ? "1" : "0"); } catch { }
      }
    }
    ...
    syncCopyUnlockState() {
      if (this.copyUnlockEnabled) {
        ...
        // Enable Auto Search when Unlock Copy is enabled
        if (!this.autoSearchHandler) {
          this.autoSearchHandler = this.handleSelectionChangeForUnlockCopy.bind(this);
          document.addEventListener("selectionchange", this.autoSearchHandler);
        }
      } else if (this.copyUnlockCleanup) {
        try { this.copyUnlockCleanup(); } catch { }
        this.copyUnlockCleanup = null;
        // Disable Auto Search when Unlock Copy is disabled
        if (this.autoSearchHandler) {
          document.removeEventListener("selectionchange", this.autoSearchHandler);
          this.autoSearchHandler = null;
          this.lastSelectionText = "";
        }
      }
    ...
    handleSelectionChangeForUnlockCopy() {
      // Auto Search functionality integrated with Unlock Copy
      if (!this.copyUnlockEnabled || !this.searchInput || !this.filterDebounced) return;
    ...
    onControlsPointerDown(e) {
      ...
      window.addEventListener("pointercancel", this.onControlsPointerUp);
    }
    ...
    onControlsPointerUp(e) {
      ...
      window.removeEventListener("pointermove", this.onControlsPointerMove);
      window.removeEventListener("pointerup", this.onControlsPointerUp);
      window.removeEventListener("pointercancel", this.onControlsPointerUp);
    ...
    handleAuthStateChange() {
      ...
      const canUseExtras = keySystemOn ? !locked : true;
      this.setExportButtonsState(canUseExtras);
      this.setUtilityButtonsState(canUseExtras);
      if (locked) {
    ...
    setExportButtonsState(enabled) {
      this.exportButtons.forEach((btn) => this.applyButtonGate(btn, enabled));
    }

    setUtilityButtonsState(enabled) {
      if (!enabled) {
        this.setCopyUnlockEnabled(false);
        this.setFullscreenBypassEnabled(false);
        // Turn off Auto Highlight if key is invalid
        if (this.autoSolveEnabled) {
          this.autoSolveEnabled = false;
          this.applyAutoSolveState(false);
        }
      }
      this.applyButtonGate(this.copyUnlockBtn, enabled);
      this.applyButtonGate(this.fullscreenBypassBtn, enabled);
      this.applyButtonGate(this.autoSolveBtn, enabled);
    }

    applyButtonGate(btn, enabled) {
      if (!btn) return;
    ...
    addEventListeners() {
      ...
      this.contentArea?.addEventListener("click", (evt) => this.handleContentAreaClick(evt));
    }

    startTimeTicker() {
      if (this.timeTicker) return;
      this.refreshLiveTimers();
      this.timeTicker = setInterval(() => this.refreshLiveTimers(), 1000);
    }

    refreshLiveTimers() {
      if (isKeySystemEnabled() && AuthService.isKeyValid && AuthService.getRemainingTimeMs() <= 0) {
        AuthService.markInvalid({ clearStoredKey: true });
      }
      this.updateKeyRemainingTicker();
      this.updateLiveTimeDisplays();
    }

    updateKeyRemainingTicker() {
      const info = AuthService.getDeviceInfo();
      const keySystemOn = isKeySystemEnabled();
      if (this.keyPanelRefs?.statusRemaining) {
        this.keyPanelRefs.statusRemaining.textContent = this.formatKeyRemainingDisplay(info, keySystemOn);
      }
    }

    updateLiveTimeDisplays() {
      if (!this.timeDisplays || !this.timeDisplays.size) return;
      const stale = [];
      this.timeDisplays.forEach((node) => {
        if (!node || !node.isConnected) {
          stale.push(node);
          return;
        }
        this.updateSingleTimeDisplay(node);
      });
      stale.forEach((node) => this.timeDisplays.delete(node));
    }

    updateSingleTimeDisplay(node) {
      if (!node) return;
      node.innerHTML = `<b>Time:</b> ${formatNowWithDate()}`;
    }

    getKeyUiState(info = AuthService.getDeviceInfo()) {
      const keySystemOn = isKeySystemEnabled();
      const hasUnlimitedTtl = info.ttlDays < 0 || info.expiresAt === UNLIMITED_TS;
      const isActive = keySystemOn ? (info.isValid && info.expiresAt && info.expiresAt > Date.now()) : true;
      let label;
      if (!keySystemOn) label = "Không yêu cầu key";
      else label = isActive ? "Đang hoạt động" : (info.status === "valid" ? "Hết hạn" : "Chưa kích hoạt");
      const remaining = this.getKeyRemainingText(info, keySystemOn);
      return { info, keySystemOn, hasUnlimitedTtl, isActive, label, remaining };
    }

    getKeyRemainingText(info = AuthService.getDeviceInfo(), keySystemOn = isKeySystemEnabled()) {
      if (!keySystemOn) return "Hệ thống key đang tắt";
      if (!info) return "Còn lại: --";
      const unlimited = info.ttlDays < 0 || info.expiresAt === UNLIMITED_TS;
      if (unlimited) return "Còn lại: Vô thời hạn";
      if (info.expiresAt) {
        const ms = Math.max(0, info.expiresAt - Date.now());
        const ttlHint = (info.ttlExpiresAt && info.ttlExpiresAt < UNLIMITED_TS)
          ? ` (tự hết hạn lúc ${formatTimestamp(info.ttlExpiresAt)})`
          : "";
        return `Còn lại: ${formatDuration(ms)}${ttlHint}`;
      }
      return "Còn lại: --";
    }

    getKeyRemainingMs(info = AuthService.getDeviceInfo(), keySystemOn = isKeySystemEnabled()) {
      if (!keySystemOn || !info) return null;
      const unlimited = info.ttlDays < 0 || info.expiresAt === UNLIMITED_TS;
      if (unlimited) return UNLIMITED_TS;
      if (info.expiresAt) return Math.max(0, info.expiresAt - Date.now());
      return null;
    }

    formatKeyRemainingDisplay(info = AuthService.getDeviceInfo(), keySystemOn = isKeySystemEnabled()) {
      if (!keySystemOn) return "Không yêu cầu key";
      if (!info) return "Còn lại: --";
      const ms = this.getKeyRemainingMs(info, keySystemOn);
      if (ms === UNLIMITED_TS) return "Còn lại: Vô thời hạn";
      if (!Number.isFinite(ms) || ms <= 0) return "Còn lại: 0 giây";
      return `Còn lại: ${formatHmsVerbose(ms)}`;
    }
    ...
    onPointerDownDrag(e) {
      ...
      const interactiveSelector = "button, input, textarea, select, a, [role='button'], .olm-controls-row, .search-field-group, .search-mini-btn, .panel-zoom-wrap, .panel-zoom-btn, .panel-zoom-indicator, .zoom-control, .zoom-btn, .zoom-indicator";
      ...
      const webScale = this.getWebScale() || 1;
      const effectiveScale = Math.max(0.001, scale * webScale);
      ...
      this.dragState = {
        ...
        startX: e.clientX,
        startY: e.clientY,
        initX: Number.isFinite(currentLeft) ? currentLeft : rect.left,
        initY: Number.isFinite(currentTop) ? currentTop : rect.top,
        scale,
        webScale,
        effectiveScale
      };
      this.container.style.transition = "none";
      if (e.pointerType === "touch") this.blockPageScroll();
      window.addEventListener("pointermove", this.onPointerMoveDrag);
      window.addEventListener("pointerup", this.onPointerUpDrag);
      window.addEventListener("pointercancel", this.onPointerUpDrag);
    }
    ensureDragAnchor() {
      if (!this.dragState || this.dragState.anchorApplied) return;
      this.container.style.right = "auto";
      this.container.style.left = `${this.dragState.initX}px`;
      this.container.style.top = `${this.dragState.initY}px`;
      this.dragState.anchorApplied = true;
    }
    onPointerMoveDrag(e) {
      if (!this.dragState || !this.dragState.dragging) return;
      const dxRaw = e.clientX - this.dragState.startX;
      const dyRaw = e.clientY - this.dragState.startY;
      const movement = Math.abs(dxRaw) + Math.abs(dyRaw);
      if (!this.dragState.active) {
        if (movement <= 2) return;
        this.dragState.active = true;
        this.ensureDragAnchor();
      }
      e.preventDefault();
      const scale = this.dragState.effectiveScale || Math.max(0.001, (this.dragState.scale || this.getPanelScale() || 1) * (this.dragState.webScale || this.getWebScale() || 1));
      const deltaX = dxRaw / scale;
      const deltaY = dyRaw / scale;
      let left = this.dragState.initX + deltaX;
      let top = this.dragState.initY + deltaY;
      const rect = this.container.getBoundingClientRect();
    ...
    onPointerUpDrag() {
      ...
      window.removeEventListener("pointercancel", this.onPointerUpDrag);
    ...
    onPointerDownResize(e) {
      ...
      const webScale = this.getWebScale();
      const effectiveScale = scale * webScale;
      this.resizeState = {
        startX: e.clientX,
        startY: e.clientY,
        startW: r.width / effectiveScale,
        startH: r.height / effectiveScale,
        scale,
        webScale,
        active: false
      };
      ...
      window.addEventListener('pointercancel', this.onPointerUpResize);
    }
    onPointerMoveResize(e) {
      if (!this.resizeState) return;
      const scale = this.resizeState.scale || this.getPanelScale();
      const webScale = this.resizeState.webScale || this.getWebScale();
      const effectiveScale = scale * webScale;
      const dx = (e.clientX - this.resizeState.startX) / effectiveScale;
      const dy = (e.clientY - this.resizeState.startY) / effectiveScale;
      if (!this.resizeState.active) {
        if (Math.abs(dx) + Math.abs(dy) < 6) return;
        this.resizeState.active = true;
      }
      const minW = 320, minH = 240;
      const maxW = Math.min(window.innerWidth - 16, 1200) / effectiveScale;
      const maxH = Math.min(window.innerHeight - 16, 1000) / effectiveScale;
      let newW = this.resizeState.startW + dx;
    ...
    onPointerUpResize() {
      ...
      window.removeEventListener('pointercancel', this.onPointerUpResize);
      this.releasePageScroll();
      const wasActive = this.resizeState?.active;
      const rect = wasActive ? this.container.getBoundingClientRect() : null;
      if (wasActive && rect) {
        const scale = this.getPanelScale();
        const webScale = this.getWebScale();
        const effectiveScale = scale * webScale;
        const baseWidth = rect.width / effectiveScale;
        const baseHeight = rect.height / effectiveScale;
        this.size = { w: Math.round(baseWidth), h: Math.round(baseHeight) };
    ...
    onPointerDownToggle(e) {
      ...
      window.addEventListener("pointercancel", this.onPointerUpToggle);
    }
    ...
    onPointerUpToggle(e) {
      ...
      window.removeEventListener("pointercancel", this.onPointerUpToggle);
      this.releasePageScroll();
      // lưu vị trí
      const rect = this.toggleBtn.getBoundingClientRect();
    ...
    loadStoredPanelZoom() {
      const stored = Number(localStorage.getItem(LS_PAGE_ZOOM));
      if (!Number.isFinite(stored) || stored <= 0) return 1;
      return Math.min(PANEL_MAX_ZOOM, Math.max(PANEL_MIN_ZOOM, stored));
    }

    loadStoredWebZoom() {
      const stored = Number(localStorage.getItem(LS_WEB_ZOOM));
      if (!Number.isFinite(stored) || stored <= 0) return 1;
      return Math.min(WEB_MAX_ZOOM, Math.max(WEB_MIN_ZOOM, stored));
    }

    updatePanelZoomStyle() {
      const zoomValue = this.panelZoom || 1;
      const valueStr = String(zoomValue);
      if (this.container?.style) {
        this.container.style.setProperty("--olm-panel-zoom", valueStr);
      }
      if (document.documentElement?.style) {
        document.documentElement.style.setProperty("--olm-panel-zoom", valueStr);
      }
    }

    updatePanelZoomIndicator() {
      if (!this.panelZoomIndicator) return;
      const percent = Math.round((this.panelZoom || 1) * 100);
      this.panelZoomIndicator.textContent = `${percent}%`;
    }

    applyPanelZoom(value, options = {}) {
      const { save = true } = options || {};
      let next = Number(value);
      if (!Number.isFinite(next) || next <= 0) next = 1;
      next = Math.min(PANEL_MAX_ZOOM, Math.max(PANEL_MIN_ZOOM, next));
      this.panelZoom = next;
      this.updatePanelZoomStyle();
      this.updatePanelZoomIndicator();
      if (save) {
        try { localStorage.setItem(LS_PAGE_ZOOM, String(next)); } catch { }
      }
    }

    adjustPanelZoom(delta) {
      const step = Number(delta) || 0;
      this.applyPanelZoom((this.panelZoom || 1) + step);
    }
    ...
    updateWebZoomIndicator() {
      if (!this.webZoomIndicator) return;
      const percent = Math.round((this.webZoom || 1) * 100);
      this.webZoomIndicator.textContent = `${percent}%`;
    }

    applyWebZoom(value, options = {}) {
      const { save = true } = options || {};
      let next = Number(value);
      if (!Number.isFinite(next) || next <= 0) next = 1;
      next = Math.min(WEB_MAX_ZOOM, Math.max(WEB_MIN_ZOOM, next));
      this.webZoom = next;
      this.updateWebZoomIndicator();
      const valueStr = String(next);
      if (document.documentElement?.style) {
        document.documentElement.style.zoom = valueStr;
        document.documentElement.style.setProperty("--olm-web-zoom", valueStr);
      }
      if (document.body?.style) {
        document.body.style.zoom = valueStr;
      }
      if (save) {
        try { localStorage.setItem(LS_WEB_ZOOM, String(next)); } catch { }
      }
    }

    adjustWebZoom(delta) {
      const step = Number(delta) || 0;
      this.applyWebZoom((this.webZoom || 1) + step);
    }
    ...
    getAnswersAsDOM(question) {
      ...
      if (fillInInput) {
        ...
        const li = document.createElement('li');
        li.className = 'correct-answer short-answer';
        const answerText = ans.join(' / ');
        const textSpan = document.createElement('span');
        textSpan.className = 'short-answer-text';
        textSpan.textContent = `Kết quả: ${answerText}`;
        const copyBtn = document.createElement('button');
        copyBtn.type = 'button';
        copyBtn.className = 'answer-copy-btn';
        copyBtn.dataset.role = 'answer-copy';
        copyBtn.dataset.copyValue = answerText;
        copyBtn.textContent = 'Copy';
        li.append(textSpan, copyBtn);
        listElement.appendChild(li);
        return listElement;
      }
      return null;
    ...
    renderData(data, options = {}) {
      ...
      timeLine.className = "olm-time-line";
      timeLine.style.cssText = "font-family:monospace;font-size:12px;background:rgba(0,0,0,0.06);padding:6px;border-radius:6px;margin:0 0 6px 0;";
      this.timeDisplays.add(timeLine);
      this.updateSingleTimeDisplay(timeLine);
      responseContainer.appendChild(timeLine);
      ...
      data.forEach((question) => {
        ...
        if (groups && groups.length) {
          ...
          groups.forEach((g, idx) => {
            ...
            // Register question meta for auto solve
            const meta = this.buildQuestionMeta(questionDisplayContainer, ul);
          ...
          if (answersElement) {
            ...
            // Register question meta for auto solve
            const meta = this.buildQuestionMeta(questionDisplayContainer, answersElement);
            if (meta) this.registerQuestionMeta(meta);
          }
    ...
    handleContentAreaClick(evt) {
      const copyBtn = evt.target.closest('[data-role="answer-copy"]');
      if (copyBtn && this.contentArea?.contains(copyBtn)) {
        evt.preventDefault();
        const value = copyBtn.dataset.copyValue || "";
        this.copyShortAnswerValue(value, copyBtn);
      }
    }

    copyShortAnswerValue(text, button) {
      if (!text) return;
      ...
      const fallbackAlert = () => alert("Không thể copy kết quả.");
      if (navigator.clipboard?.writeText) {
        navigator.clipboard.writeText(text).then(() => {
          showDone();
        }).catch(() => {
          if (this.copyTextViaTextarea(text)) showDone();
          else fallbackAlert();
        });
        return;
      }
    ...
    async pasteSearchFromClipboard() {
      if (!this.searchInput) return;
      let text = "";
      if (navigator.clipboard?.readText) {
        try {
          ...
          console.warn("[OLM Helper] Clipboard read failed:", err);
        }
      }
      if (!text) {
        const fallback = prompt("Dán nội dung cần tìm:", "");
    ...
    filterQuestions(keyword) {
      ...
      const normalizedQuery = stripDiacritics(q);
      const blocks = this.contentArea.querySelectorAll(".qa-block");
      let shown = 0;
      blocks.forEach((b) => {
        ...
        const match = !q || text.includes(q) || (!!normalizedQuery && normalizedText.includes(normalizedQuery));
    ...
    buildQuestionMeta(questionEl, answersElement, overrideQuestionText = "") {
      ...
      answerNodes.forEach((node) => {
        if (!(node instanceof Element)) return;
        const text = normalizeAnswerText(getMathAwareText(node));
        if (!text) return;
      ...
      return { signature, questionText, answersNormalized, correctSet };
    }
    ...
    // ===== Auto Solve (Auto Lam) Functions =====
    toggleAutoSolve() {
      // Check if key is valid
      if (!AuthService.isKeyValid) {
        alert("Vui lòng nhập key để sử dụng tính năng Auto Highlight");
        return;
      }
      this.applyAutoSolveState(!this.autoSolveEnabled);
    }

    applyAutoSolveState(force) {
      // Don't enable if key is not valid
      if (force && !AuthService.isKeyValid) {
        this.autoSolveEnabled = false;
      } else {
      ...
      if (this.autoSolveBtn) {
        ...
        this.autoSolveBtn.textContent = this.autoSolveEnabled ? "Auto Highlight ON" : "Auto Highlight";
    ...
    scanAndHighlightCurrentQuestion() {
      if (!this.autoSolveEnabled || !this.questionBank || !this.questionBank.length) return;
      try {
    ...
    findPageOptionLists() {
      ...
      // Also look for containers with .qselect children (not in quiz-list)
      ...
      document.querySelectorAll(".qselect, [data-ind].qselect").forEach((qselect) => {
        ...
        // Find the parent container that holds all qselect options
        let container = qselect.parentElement;
        if (!container) return;
        // Look for common quiz container classes
        ...
        // If no specific container found, use parent if it has multiple qselect children
        // Make sure container is valid and not document.body
    ...
    extractOptionItems(rootEl) {
      ...
      if (rootEl.matches(".true-false") || rootEl.querySelector(".tf-row")) {
        ...
        rows.forEach((row) => {
          ...
          options.push({ element: clause, normalized });
        });
      ...
      nodes.forEach((node) => {
        ...
        if (node.closest("#olm-answers-container")) return;
        if (!isElementVisible(node)) return;
        // For .qselect elements, prioritize .qsign for content extraction
        let contentEl = node;
        ...
        if (!normalized) return;
        options.push({ element: node, normalized });
      });
      return options;
    }
    ...
    // ===== Auto Highlight Functions (removed - using Auto Solve instead) =====
  }
  ...
  // Instantiate immediately to capture early requests
  answerUIInstance = new AnswerDisplay();
  installNetworkHooks();

  function startHelper() {
    // Only init UI when DOM is ready
