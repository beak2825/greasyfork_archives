// ==UserScript==
// @name         VIP视频解析
// @namespace    http://www.1688cloud.xyz/
// @version      1.0.0
// @license      AGPL-3.0
// @description  无需跳转新网址，打开官网直接看，超清 无广告 随机去水印。支持：腾讯，爱奇艺，优酷，哔哩哔哩，咪咕，乐视，搜狐，芒果，西瓜，PPTV，1905电影网，华数。支持解析失败自动切换推荐解析源。
// @author       Tenfond
// @match        http*://v.qq.com/*
...
// @include      http*://*.*/*v=http*://*.*.*/*
// @include      http*://*.*:*/*v=http*://*.*.*/*
// @match        http*://api.leduotv.com/*?vid=*
// @grant        GM_getValue
// @grant        GM_setValue
// @run-at       document-body
// @downloadURL https://update.greasyfork.org/scripts/441904/VIP%E8%A7%86%E9%A2%91%E8%A7%A3%E6%9E%90.user.js
// @updateURL https://update.greasyfork.org/scripts/441904/VIP%E8%A7%86%E9%A2%91%E8%A7%A3%E6%9E%90.meta.js
// ==/UserScript==
/**
 * 任务清单：
 */
    ...
    if (window === top) {
        ...
        */
        settings.NoAD解析 = {    // TODO by 17kyun.com/api.php?url=    // TODO by tv.hzwdd.cn
            ...
            "接口①": "https://okjx.cc/?url=",
            "接口②": "http://ckmov.vip/api.php?url=",
            "接口③": "https://jiexi.071811.cc/jx.php?url=",
            "接口④": "https://www.playm3u8.cn/jiexi.php?url=",
            "接口⑤": "https://api.jiexi.la/?url=",
            "接口⑥": "https://www.eggvod.cn/jx.php?zhm_jx=",
     // TODO 腾讯 [爱奇艺] 优酷 (乐视) 芒果 (1905电影网) [华数]
        };
        settings.AD解析 = {    // TODO (有赌博广告，请勿相信，这么简单的骗术不会有人上当吧)
            "广告解析": "https://www.mtosz.com/m3u8.php?url="    // TODO 无水印(但不稳定): 腾讯 爱奇艺 优酷 乐视 [芒果] (PPTV) (华数)
        };
        start();
    ...
    if (window === top) {
        ...
        if (!detectMobile()) {
            if (location.host.indexOf("v.qq.com") !== -1) {
                readyPlayerBox("已进入腾讯视频", ["#mask_layer", ".mod_vip_popup", "#mask_layer"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div#mod_player", null);
            } else if (location.host.indexOf("iqiyi.com") !== -1) {
                readyPlayerBox("已进入爱奇艺", ["#playerPopup", "div[class^=qy-header-login-pop]"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "iqpdiv.iqp-player[data-player-hook$=er]", null);
            } else if (location.host.indexOf("youku.com") !== -1) {
                readyPlayerBox("已进入优酷视频", ["#iframaWrapper"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div#player", null);
                ...
                doElement("svg[aria-hidden=true]", () => readyPlayerBox("已进入哔哩哔哩", null, [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div.bpx-player-video-area,div.mask-container", null));    // TODO || document.getElementById("bilibiliPlayer") || document.getElementById("live-player-ctnr")
                ...
                readyPlayerBox("已进入乐视TV", null, [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "#le_playbox", null);
            } else if (location.host.match(/(tv|film).sohu.com/)) {
                readyPlayerBox("已进入搜狐视频", null, [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "#player,#sohuplayer,.player-view", null);
            } else if (location.host.indexOf("mgtv.com") !== -1) {
                readyPlayerBox("已进入芒果TV", null, [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "#mgtv-player-wrap", null);
            } else if (location.host.indexOf("ixigua.com") !== -1) {
                readyPlayerBox("已进入西瓜视频", null, [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div.teleplayPage__playerSection", null);
            } else if (location.host.indexOf("pptv.com") !== -1) {
                readyPlayerBox("已进入PPTV", null, [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div.w-video", null);
            } else if (location.host.indexOf("vip.1905.com") !== -1) {
                readyPlayerBox("已进入1905电影网", null, [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div#playBox", null);
            } else if (location.host.indexOf("www.wasu.cn") !== -1) {
                readyPlayerBox("已进入华数TV", null, [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div#pcplayer", null);
            }
        } else {
            if (location.host.indexOf("v.qq.com") !== -1) {
                readyPlayerBox("已进入腾讯视频", [".mod_vip_popup", "[class^=app_],[class^=app-],[class*=_app_],[class*=-app-],[class$=_app],[class$=-app]", "div[dt-eid=open_app_bottom]", "div.video_function.video_function_new", "a[open-app]", "section.mod_source", "section.mod_box.mod_sideslip_h.mod_multi_figures_h,section.mod_sideslip_privileges,section.mod_game_rec"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div.mod_play:not([style*='display: none;']) section.mod_player", null, href => {
                ...
                readyPlayerBox("已进入爱奇艺", ["div.m-iqyGuide-layer", "a[down-app-android-url]", "[name=m-extendBar]", "[class*=ChannelHomeBanner]", "section.m-hotWords-bottom"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "section.m-video-player", null);
            } else if (location.host.indexOf("youku.com") !== -1) {
                readyPlayerBox("已进入优酷视频", ["#iframaWrapper", ".ad-banner-wrapper", ".h5-detail-guide,.h5-detail-vip-guide", ".brief-btm"], [], "#player", null);
            } else if (location.host.indexOf("bilibili.com") !== -1) {
                readyPlayerBox("已进入哔哩哔哩", ["div.fe-ui-open-app-btn,div.recom-wrapper,open-app-btn", "[class*=openapp]"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div#app.main-container div.player-wrapper", null, href => href.replace("m.bilibili.com", "www.bilibili.com"));
            } else if (location.host.indexOf("miguvideo.com") !== -1) {
                ...
                readyPlayerBox("已进入乐视TV", ["a.leapp_btn", "div.full_gdt_bits[id^=full][data-url]", "[class*=Daoliu],[class*=daoliu],[class*=game]", "div.m-start", "[class*=icon_user]"], [settings.NoAD解析["接口③"]], "div.column.play", null);
            } else if (location.host.indexOf("m.tv.sohu.com") !== -1) {
                readyPlayerBox("已进入搜狐视频", ["div[class^=banner]", "div.js-oper-pos", "div[id^=ad],div[id^=ad] *", "[id*=login],[class*=login]", "[class$=-app]", "div.app-vbox.ph-vbox,div.app-vbox.app-guess-vbox", "div.twinfo_iconwrap", "div[class$=banner],div[id$=banner]"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "#player,#sohuplayer,.player-view", null, async href => {
                ...
                readyPlayerBox("已进入芒果TV", ["div.adFixedContain,div.ad-banner,div.m-list-graphicxcy.fstp-mark", "div[class^=mg-app],div#comment-id.video-comment div.ft,div.bd.clearfix,div.v-follower-info", "div.ht.mgui-btn.mgui-btn-nowelt", "div.personal", "div[data-v-41c9a64e]"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div.video-poster,div.video-area", null);
            } else if (location.host.indexOf("ixigua.com") !== -1) {
                readyPlayerBox("已进入西瓜视频", ["div.xigua-download", "div.xigua-guide-button", "div.c-long-video-recommend.c-long-video-recommend-unfold"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div.xigua-detailvideo-video", null);
            } else if (location.host.indexOf("pptv.com") !== -1) {
                readyPlayerBox("已进入PPTV", ["[data-darkreader-inline-bgimage][data-darkreader-inline-bgcolor]", "div[class^=pp-m-diversion]", "section#ppmob-detail-picswiper", "section.layout.layout_ads", "div.foot_app", "div[modulename=导流位]", "a[class*=user]", "div.mod_video_info div.video_func"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "section.pp-details-video", null, href => href.replace("m.pptv.com", "v.pptv.com"));
            } else if (location.host.indexOf("vip.1905.com") !== -1) {
                (movie_info => {
                    movie_info.innerHTML = "section#movie_info{padding-top: 20px !important;}";
                    document.head.appendChild(movie_info);
                })(document.createElement("style"));
                readyPlayerBox("已进入1905电影网", ["a.new_downLoad[target=_blank]", "iframe[srcdoc^='<img src=']", "section.movieList_new.club_new", ".wakeAppBtn", "[class*=login]", "section.openMembershipBtn", ".ad", ".open-app,.openApp,ul.iconList li:not(.introduceWrap),div#zhichiBtnBox", "section#hot_movie,section#exclusive_movie,section#hot_telve"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div.area.areaShow.clearfix_smile", null);
            } else if (location.host.indexOf("www.wasu.cn") !== -1) {
                readyPlayerBox("已进入华数TV", ["div.ws_poster", "div.appdown,div.player_menu_con", "div#play_and_info_fix_adv"], [settings.NoAD解析["接口①"],settings.NoAD解析["接口②"],settings.NoAD解析["接口③"],settings.NoAD解析["接口④"]], "div#player,div#pop", null);
            }
        }
