// ==UserScript==
// @name			UnTaint - Wiki Improvement Script
...
// @copyright		2014
// @author			SurfingEagle
// @icon			http://wikifoundryattachments.com/6qzWeWGTKHMIw5rWPn0KwA98790
...
// @include			http://www.zombieprepwiki.com/thread*
// @include			http://www.zombieprepwiki.com/forum/*
// @include			http://www.zombieprepwiki.com/forum
// @include			http://www.zombieprepwiki.com/account/*/thread/*
// @include			http://www.zombieprepwiki.com/photo/*/thread/*
// @include			http://www.zombieprepwiki.com/video/*/thread/*
...
// @downloadURL https://update.greasyfork.org/scripts/3491/UnTaint%20-%20Wiki%20Improvement%20Script.user.js
// @updateURL https://update.greasyfork.org/scripts/3491/UnTaint%20-%20Wiki%20Improvement%20Script.meta.js
// ==/UserScript==

// =============================================================================
// ***** Editing this section and below is modifying code. *****
// TinyEditor integration and script adaptation to FF 4+ and Chrome 11+ by: SurfingEagle
// Thanks to the inhabitants of the tsccwiki. Credits: toasty2 and Gu1.
...
// For debugging purposes, set to true to display error messages in certain sections more prone to errors. False [default] avoids alert.
const debug = false;
// Convenient method "contains" used in "user posts filter".
Array.prototype.contains = function(obj){var i = this.length; while(i--){if (this[i] === obj) {return true;}} return false;}

// Switch to using localStorage. Functions putSetting, getSetting, deleteSetting, emptySetting.
...
// The following are additions from other code sources and are documented as such.
...
// The following block handles returning to end of thread after a new post
try {
...
GM_addStyle('#input {border:none; margin:0; padding:0; font:12px "Courier New", Verdana; border:0} ' +
	...
	'.teheader {height:31px; border-bottom:1px solid #bbb; background:url(http://wikifoundryattachments.com/xg-mfHLVZfdD7OErlv0NxQ55) repeat-x; padding-top:1px} ' +
	...
	'.tecontrol {float:left; width:34px; height:30px; cursor:pointer; background-image:url(http://wikifoundryattachments.com/jjINLqTU68Po418uA2wLPw18521)} ' +
	'.tecontrol:hover {background-color:#fff; background-position:30px 0} ' +
	'.tefooter {height:32px; border-top:1px solid #bbb; background:#f5f5f5} ' +
	'.toggle {float:left; background:url(http://wikifoundryattachments.com/jjINLqTU68Po418uA2wLPw18521) -34px 2px no-repeat; padding:9px 13px 0 31px; height:23px; border-right:1px solid #ccc; cursor:pointer; color:#666} ' +
	'.toggle:hover {background-color:#fff} ' +
	'.resize {float:right; height:32px; width:32px; background:url(http://wikifoundryattachments.com/W_Ipj_QMRSXNr7FEg3Sz8A78) 15px 15px no-repeat; cursor:s-resize}');
...
/* =============================================================================
	The following block "main" is inserted into the page and contains most of
	the core funtionality, primarily for the purpose of compatibility with 
	Chrome but Firefox didn't require this.
============================================================================= */
function main() 
{
	...
	posting_add = function(x)
	{
		...
		if(x=='youtube')
		{
			...
			x = '<object height="' + h + '" width="' + w + '">'
				+ '<param name="movie" value="http://wikifoundryattachments.com/IHOlZ-XE6QdUuBNdXdk9jA154555?null=http://'
				+ '<param name="allowFullScreen" value="true"><param name="allowscriptaccess" value="never"><param name="wmode" value="transparent">'
				+ '<embed allowfullscreen="true" flashvars="provider=video&amp;image=http://i.ytimg.com/vi/' + y + '/hqdefault.jpg&amp;file=http://ox-proxy.no-ip.org/youtube/' 
				+ y + '" src="http://wikifoundrytools.com/wiki/m7r-227/widget/unknown/d5a99526c41ad11592c12c7b323ca651af50909a?null=http://" type="application/x-shockwave-flash" wmode="transparent" height="' 
				+ h + '" width="' + w + '">'
				+ '<param name="flashvars" value="provider=video&amp;image=http://i.ytimg.com/vi/' 
				+ y + '/hqdefault.jpg&amp;file=http://ox-proxy.no-ip.org/youtube/' + y + '"></object>';
			x += '<br><a href="http://youtu.be/' + y + '">' + 'http://youtu.be/' + y + '</a><br><br>';
		}
	...
	// The modified TinyEditor control
		...
		if (textElement)
		{
			...
			{	
				...
				if(input.substr != undefined && input.substr(0, 20) == "<thread><posts><post") // are we posting a new post ?
				{
					...
					// Add terms to array that avoid censor as part of a regular expression if necessary.
					uncensor = ['damn[^a-z]', 'hell[^a-z]'];
					
					input = input.replace(new RegExp(uncensor.join('|'), 'gi'), function(str) {
	...
	activateTinyEditor = function()
	{
		...
		if (!teFrame)
		{
			...
			tcolors:['Black','Blue','Brown','Crimson','Cyan','FireBrick','Gold','Gray','Green','HotPink','LightCoral','Lime','Magenta','Maroon','Navy','Olive','Orange','OrangeRed','Pink','Purple','Red','Silver','Teal','Tomato','White','Yellow'],
...
// Function finds and returns an element in a new post or edit
...
{ 
	...
	cfg_show = function()
	{
		...
		var pnlcon = null;
		pnlcon = document.getElementById('pageContentInner');
		if (pnlcon)
		{
			pnlcon.innerHTML+='<div style="position:fixed;z-index:10;top:0px;left:50%;width:350px;margin-left:-175px;background-color:black;color:white;"><form id="cfg" name="cfg">'
			...
			+'<center><button type="button" onclick="cfg_save();window.location.reload();">Save</button><button type="button" onclick="window.location.reload();">Cancel</button></center>'
...
try 
{
	// Always add setting functions to page
	...
	if (reply[0])
	{
		...
		for (i in reply)
		{
			// loadInitialize() is critical to the post filter
...
// Added a show left navigation bar option for those who want to add new pages and/or use the navigation bar.
...
// Stripped the domain back to .com to accommodate custom wiki names and 
...
// Untaint Panel. Ternary operator. If page numbers exist, 
// then mirror them in the Untaint panel's innerHTML, else return an empty string to display nothing.
try {var pages = (getElementsByClassName('paging')[0]) ? getElementsByClassName('paging')[0].innerHTML : ''; /* Page #'s */ } 
catch (e) {if (debug) alert(e.source + '\n' + e.message)}
...
try {
	// Changed element from allContentInner to pageContentInner.
	// The outer frame and styling of the navigation panel. Plus, adding "bottom"
...
try {
	...
	if (utpanel)
	{
		// Inner components of the navigation panel
		utpanel.innerHTML=''
 		+'<b>UnTaint | <small><sup><a href="#top" onclick="cfg_show();">Settings</a></sup></small></b><small>'
		+'<div style="float:right;padding-right:5px;padding-top:2px;"><a href="#top">Top</a><br /><a href="#bottom" onclick="setTimeout(function(){window.scroll(0,'+getDocHeight()+')},100);">Bottom</a></div>'
