...
// @downloadURL https://update.greasyfork.org/scripts/501248/RickBlock.user.js
// @updateURL https://update.greasyfork.org/scripts/501248/RickBlock.meta.js
// ==/UserScript==
...
This is the source code for RickBlock. Feel
free to inspect it, but note that it might
feel like a dumpster fire - I pray for your
safety.

To add a blocklist/allowlist website, use
the `settings` variable.

Made by Tiago Rangel (tiagorangel.com).
Do not redestribute.
...
(function (GM) {
  ...
  const settings = {
    ...
    match: {
      ...
      links: [
        ...
        "realistic-texture-pack.glitch.me/download.html",
        "microsoftgithub.com",
        "news.rr.nihalnavath.com",
        "rickroll.villainsrule.xyz",
        ...
        "social.mtdv.me",
        "creations.mtdv.me",
        "r.motiondevelopment.top",
        "blogs.motiondevelopment.top",
        "social.motiondevelopment.top",
        "creations.motiondevelopment.top",
       ...
       "/watch?v=dQw4w9WgXcQ",
       "dQw4w9WgXcQ",
       "i1.sndcdn.com/artworks-XJdVplPCbvDvJlH7-jF9c4A-t500x500.jpg"
      ],
    },
  };

  const $ = (e) => (window || unsafeWindow).document.querySelector(e);
  const $$ = (e) => (window || unsafeWindow).document.querySelectorAll(e);
  ...
  const detect = {
    detect: function (link) {
      ...
      settings.match.youtube.split(", ").forEach(function (e) {
        if (link.includes(e) && (link.includes("you") || link.includes("yt"))) {
          detected = true;
        }
      });
    ...
    web: async function () {
      ...
      return detect.detect(location.href);
    },
  };

  const hasSkip = async function () {
    if (
      decodeURIComponent(new URL(location.href).searchParams.get("active")) ==
        "skip" ||
      (await GM.getValue("skip"))
    ) {
      let hasExpired = false;
      if (await GM.getValue("skip")) {
        if (JSON.parse(await GM.getValue("skip")).expires <= Date.now()) {
          hasExpired = true;
        }
      }

      if (hasExpired) {
        await GM.deleteValue("skip");
      } else {
        await GM.setValue(
          "skip",
          JSON.stringify({
  ...
  const main = async function () {
    ...
    const checkAll = async function () {
      ...
      if (!allowed && (await detect.web())) {
        document.body.innerHTML = `<div style="margin:3em;text-align:center;"><h1 style="font-family: Roboto, Arial, sans-serif;">Rickroll detected.<br>Loading...</h1></div>`;
        document.location.replace(
          "https://rblock.glitch.me/magic/block?origin=" + btoa(location.href)
        );
      }
    };
