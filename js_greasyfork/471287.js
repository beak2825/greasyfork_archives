...
// @downloadURL https://update.greasyfork.org/scripts/471287/VAPECLIENT.user.js
// @updateURL https://update.greasyfork.org/scripts/471287/VAPECLIENT.meta.js
// ==/UserScript==
/* jshint esversion:6 */
...
Function("(" + ((GM_info) => {
    ...
    isSkull: null
        ...
        const defaultSettings = {
            ...
            possibleShots: true,
            autochat: true,
            autochatMessages: [ "VAPE Client","What is it?","The most advanced hack for sploop","YouTube RektByMateX" ],
            kill: true,
            killMessage: ["Get Rekt {NAME}"],
            autospawn: false,
            smootZoom: false,
            skipUpgrades: false,
      ...
      const Hats = [ {
        ...
        const getDelay = health => {
            if (health < 34) return 31;
            if (health < 74) return 124;
            if (health < 98) return 153;
            return 200;
        };
        const healing = () => {
                ...
                for (let i = 0; i < VAPE.saves.entityList()[ELayer.TRAP].length; i++) {
                    ...
                    if (trapx && trapx[VAPE.props.ownerID] !== VAPE.myPlayer.ownerID && !Allies.find(a => a.pid == trapx[VAPE.props.ownerID]) && Math.hypot(trapx[VAPE.props.x] - VAPE.myPlayer.x, trapx[VAPE.props.y] - VAPE.myPlayer.y) <= 55) {
                        ...
                        if (!inTrap) {
                            ...
                            VAPE.controller.place(7, angle);
                            setTimeout(function() {
                                VAPE.controller.place(ItemType.SPIKE, toRad(toDegree(angle) - 72));
                                setTimeout(function() {
                                    VAPE.controller.place(ItemType.SPIKE, toRad(toDegree(angle) + 72));
                                }, 100);
                            }, 100);
                  ...
                  VAPE.controller.PacketManager.chat(attackbot ? 'Bot: antitrap done' : 'done')
                            ...
                            //VAPE.controller.PacketManager.chat('lol im trapped')
                            window.equip(11)
...
#KatanaSetButton:hover:active,
    ...
    if (Math.hypot(entity.x - player.x2, entity.y - player.y2) <= 150
       ...
       controller.PacketManager.chat('replace ' + entity.id + ' type:' + entity.type);
        ...
        setTimeout(() => {
            setInterval(() => {
                if(Settings.autoheal2 && VAPE.myPlayer.health < 30){
                window.equip(6)
                }
            }, 0)
        }, 0)
        setInterval(() => {
            if(Settings.autoheal2 && VAPE.myPlayer.health < 35) {
                window.equip(6)
            }
        }, 0);
        setTimeout(() => {
