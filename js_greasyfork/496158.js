// ==UserScript==
// @name         KRASNOYARSK | Скрипт для ГС/ЗГС by R.Winston
// @namespace    https://forum.blackrussia.online
// @version      1.3
// @description  Скрипт для упрощения работы ГС/ЗГС KRASNOYARSK.
// @author       Rayn_Winston
...
// @grant        none
// @license 	 none
// @downloadURL https://update.greasyfork.org/scripts/496158/KRASNOYARSK%20%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%93%D0%A1%D0%97%D0%93%D0%A1%20by%20RWinston.user.js
// @updateURL https://update.greasyfork.org/scripts/496158/KRASNOYARSK%20%7C%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%93%D0%A1%D0%97%D0%93%D0%A1%20by%20RWinston.meta.js
// ==/UserScript==
...
(function () {
    ...
    const buttons = [
        ...
        },
        {
            ...
            "[CENTER][FONT=georgia]Доброго времени суток, уважаемый(-ая) {{ user.name }}.<br><br>"+
            "[url=https://ibb.co/51gRYCr][img]https://i.ibb.co/grLRvQS/image.png[/img][/url]<br>"+
             `[CENTER] Лидер получит наказание.<br>`+
        ...
        },
        {
            ...
            "Ваше наказание будет снято в ближайшее время, если оно ещё не снято.<br>"+
         ...
         },
{
	  ...
	  content:
		"[CENTER][FONT=georgia]Доброго времени суток, уважаемый(-ая) {{ user.name }}.[/FONT][/CENTER]<br><br>"+
		"[CENTER][COLOR=lavender]Жалоба передана Заместителю Главного Администратора по направлению ГОСС/ОПГ - @Devid Diev, пожалуйста, ожидайте ответа.<br>"+
		"[CENTER][IMG width=695px]https://i.postimg.cc/qBmYpvQv/C0ffE.png[/IMG]<br>"+
		'[CENTER][COLOR=red][ICODE]Передано ЗГА ГОСС/ОПГ[/ICODE][/COLOR][/CENTER]',
	  prefix: PIN_PREFIX,
	  status: true,
},
        {
        ...
        },
        {
            ...
            content:
            `[center][font=georgia][size=5][color=rgb(124, 252, 0)]Еженедельный отчёт был успешно просмотрен.[/size]<br><br>`+
           "[url=https://ibb.co/51gRYCr][img]https://i.ibb.co/grLRvQS/image.png[/img][/url]<br>"+
        ...
        },
        {
            title: `Одобрено`,
            content:
            `[QUOTE][CENTER][COLOR=#00FF00]Одобрено.[/COLOR][/CENTER][/QUOTE]`
        },
    ];
  ...
  $(document).ready(() => {
    ...
    const button = `<button type="button" class="button--primary button rippleButton" id="${id}" style="border-radius: 30px; margin-right: 7px;">${name}</button>`;
    $('.button--icon--reply').before(button);
}
 
    function buttonsMarkup(buttons) {
  return `<div class="select_answer">${buttons.map((btn, i) => `<button id="answers-${i}" class="button--primary button rippleButton" style="margin:5px"><span class="button-text">${btn.title}</span></button>`).join('')}</div>`;
}
