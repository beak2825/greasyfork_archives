...
(function () {
  ...
  const storageItems =
    storageItemsString && storageItemsString !== "undefined"
      ? JSON.parse(storageItemsString)
      : "";
  const masterItemsTimestamp =
    localStorage.getItem("masterItemsTimestamp") || 0;
  const now = Date.now();
  const twoHoursAgo = now - 2 * 60 * 60 * 1000;

  if (!storageItems || storageItems.length <= 0) {
    console.log(
      "********No Storage items Found, Setting Initial Storage Items...",
    );
