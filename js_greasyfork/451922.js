...
// @grant        none
// @license      GPL
// @downloadURL https://update.greasyfork.org/scripts/451922/Bonkio%20IP%20Logger.user.js
// @updateURL https://update.greasyfork.org/scripts/451922/Bonkio%20IP%20Logger.meta.js
// ==/UserScript==
/* jshint esversion: 6 */
...
function resetBox() {
    ...
    ad.innerHTML += `<p id="pings" style="font-family: monospace; color: pink;">Pings to hosts will appear here</p>`
...
function addSdp(sdp, username) {
    ad.innerHTML += `<p style="font-family: monospace; color: black; user-select: text;">Got ${username}'s sdp info: ${sdp}</p>`;
    ad.innerHTML += `<p style="font-family: monospace; color: white; user-select: text;">----------------</p>`;
}

w.RTCPeerConnection.prototype.createDataChannel2 = w.RTCPeerConnection.prototype.createDataChannel;
w.RTCPeerConnection.prototype.createDataChannel = function(...args) {
    var dc = this.createDataChannel2(...args);
    ad.innerHTML += `<p style="font-family: monospace; color: magenta; user-select: text;">initialized webrtc data channel with label: ${dc.label}</p>`;
    return dc
}
...
w.RTCPeerConnection.prototype.addIceCandidate = function(...args) {
    if (!args[0].address.includes(".local")) {
        if (args[0].address != "159.69.125.43") {
            ...
            sleep(1000)
            addSdp(this.remoteDescription.sdp, username)
        }
    }
    document.getElementById("getpings").addEventListener ("click", getPingInfo, false);
