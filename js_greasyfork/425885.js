// ==UserScript==
// @name         WME Place Interface Enhancements (modified)
// @namespace    https://greasyfork.org/users/564132-lihtsaltmats
...
// @author       JustinS83, modified by LihtsaltMats
...
// @license      GPLv3
// @downloadURL https://update.greasyfork.org/scripts/425885/WME%20Place%20Interface%20Enhancements%20%28modified%29.user.js
// @updateURL https://update.greasyfork.org/scripts/425885/WME%20Place%20Interface%20Enhancements%20%28modified%29.meta.js
// ==/UserScript==
...
//"#edit-buttons > div > div.toolbar-submenu.toolbar-group.toolbar-group-venues.ItemInactive > menu";
  ...
  let GLE;
  var catalog = [];
  const updateMessage = 'Fixed finding info from the nearest segment';
  ...
  async function init() {
    ...
    PIEPlaceNameLayer = new OpenLayers.Layer.Vector('PIEPlaceNameLayer', {
    ...
    PLSpotEstimatorLayer = new OpenLayers.Layer.Vector('PIEPLSpotEstimatorLayer', {
      displayInLayerSwitcher: false,
    ...
    PLSpotEstimatorCalibrationLayer = new OpenLayers.Layer.Vector('PIEPLSpotEstimatorCalibrationLayer', {
      displayInLayerSwitcher: false,
    ...
    showStopPointsLayer = new OpenLayers.Layer.Vector('PIEShowStopPointsLayer', {
      displayInLayerSwitcher: false,
    ...
    closestSegmentLayer = new OpenLayers.Layer.Vector('PIEClosestSegment', {
      displayInLayerSwitcher: false,
...
//         }
    ...
    /********* SHORTCUTS *********/
    new WazeWrap.Interface.Shortcut('CreateResidentialPlaceShortcut', 'Creates a resdiential Place point', 'wmepie', 'Place Interface Enhancements', settings.CreateResidentialPlaceShortcut, function () {
      startPlacementMode(resCategory, true);
    }, null).add();

    new WazeWrap.Interface.Shortcut('CreateParkingLotShortcut', 'Creates a parking lot Place', 'wmepie', 'Place Interface Enhancements', settings.CreateParkingLotShortcut, function () {
    ...
    new WazeWrap.Interface.Shortcut('CreateItem1Shortcut', 'Create Item 1', 'wmepie', 'Place Interface Enhancements', settings.CreateItem1Shortcut, function () {
      startPlacementMode('OTHER', true);
    }, null).add();
    new WazeWrap.Interface.Shortcut('CreateItem2Shortcut', 'Create Item 2', 'wmepie', 'Place Interface Enhancements', settings.CreateItem2Shortcut, function () {
      PlaceMenuShortcut(2);
    }, null).add();
    new WazeWrap.Interface.Shortcut('CreateItem3Shortcut', 'Create Item 3', 'wmepie', 'Place Interface Enhancements', settings.CreateItem3Shortcut, function () {
      PlaceMenuShortcut(3);
    }, null).add();
    new WazeWrap.Interface.Shortcut('CreateItem4Shortcut', 'Create Item 4', 'wmepie', 'Place Interface Enhancements', settings.CreateItem4Shortcut, function () {
      PlaceMenuShortcut(4);
    }, null).add();
    new WazeWrap.Interface.Shortcut('CreateItem5Shortcut', 'Create Item 5', 'wmepie', 'Place Interface Enhancements', settings.CreateItem5Shortcut, function () {
      PlaceMenuShortcut(5);
    }, null).add();
    new WazeWrap.Interface.Shortcut('CreateItem6Shortcut', 'Create Item 6', 'wmepie', 'Place Interface Enhancements', settings.CreateItem6Shortcut, function () {
      PlaceMenuShortcut(6);
    }, null).add();
    new WazeWrap.Interface.Shortcut('CreateItem7Shortcut', 'Create Item 7', 'wmepie', 'Place Interface Enhancements', settings.CreateItem7Shortcut, function () {
      PlaceMenuShortcut(7);
    }, null).add();
    new WazeWrap.Interface.Shortcut('CreateItem8Shortcut', 'Create Item 8', 'wmepie', 'Place Interface Enhancements', settings.CreateItem8Shortcut, function () {
      PlaceMenuShortcut(8);
    }, null).add();
    new WazeWrap.Interface.Shortcut('CreateItem9Shortcut', 'Create Item 9', 'wmepie', 'Place Interface Enhancements', settings.CreateItem9Shortcut, function () {
      PlaceMenuShortcut(9);
    }, null).add();
    new WazeWrap.Interface.Shortcut('CreateItem10Shortcut', 'Create Item 10', 'wmepie', 'Place Interface Enhancements', settings.CreateItem10Shortcut, function () {
      PlaceMenuShortcut(10);
    }, null).add();
    new WazeWrap.Interface.Shortcut('CreateItem11Shortcut', 'Create Item 11', 'wmepie', 'Place Interface Enhancements', settings.CreateItem11Shortcut, function () {
      PlaceMenuShortcut(11);
    }, null).add();
    new WazeWrap.Interface.Shortcut('CreateItem12Shortcut', 'Create Item 12', 'wmepie', 'Place Interface Enhancements', settings.CreateItem12Shortcut, function () {
    ...
    if (WazeWrap.hasPlaceSelected()) {
      ...
      if (!$('#PIEHoursParserDiv').length) {
        ...
        var appendDiv = (function () {
          ...
          $('#PIEAppendHours').click(function () {
            addHours(false);
          });
          $('#PIEReplaceHours').click(function () {
            addHours(true);
          });
        });
  ...
  function drawNavPointClosestSegmentLines() {
    try {
      ...
      if (highlightedVenue !== null && WazeWrap.Model.getObjectModel(highlightedVenue) && highlighting === false && W.map.getZoom() >= 16) {
        ...
        let closestSeg = findClosestSegment(navPoint, false, false);
  ...
  function findNearestSegment(navPoint) {
    ...
    closestSegment = findClosestSegment(navPoint, false, false);
        ...
        if ((isPoint && W.map.getZoom() >= 17) || (!isPoint && W.map.getZoom() >= 15)) {
          if (WazeWrap.Geometry.isGeometryInMapExtent(venue.getOLGeometry())) {
            if ((isPoint && showPoint) || (!isPoint && showArea && !venue.isParkingLot()) || (!isPoint && showPLA && venue.isParkingLot())) {
              ...
              let placeNameLabel = new OpenLayers.Feature.Vector(textLoc, {
                display: 'block',
    ...
    if (markerType !== null) {
      ...
      if (W.map.getLayerByUniqueName('problems').markers[markerID].model.attributes.subType === 71) {
        ...
        setTimeout(function () {
          ...
          $('#PIECreatePLAButton').click(function () {
  ...
  function createPlace(pos, category, isPoint) {
    ...
    if (category === resCategory) {
      ...
      const closestSeg = findClosestSegment(W.userscripts.toGeoJSONGeometry(new OpenLayers.Geometry.Point(pos.lon, pos.lat)), false, false);
      if (closestSeg && settings.AddEntryExitPoint) {
        const seg = W.model.segments.getObjectById(closestSeg.attributes.id);
        const distanceToSegment = NewPlace.getOLGeometry().distanceTo(seg.geometry, {details: true});
        ...
        if (halfDistancePoint) {
          const eep = new NavigationPoint(W.userscripts.toGeoJSONGeometry(halfDistancePoint));
    ...
    const closestNamedSeg = findClosestSegment(W.userscripts.toGeoJSONGeometry(new OpenLayers.Geometry.Point(pos.lon, pos.lat)), settings.SkipPLR, settings.SkipPLR);
    if (closestNamedSeg) { //if we were able to find a segment, try to pull the city and/or street name if the options are enabled
      var newAttributes, UpdateFeatureAddress = require('Waze/Action/UpdateFeatureAddress'),
        address = closestNamedSeg.getAddress();
        ...
        if (settings.UseAltCity && cityName === '') {
          if (address.attributes.altStreets.length > 0) { //segment has alt names
            for (var j = 0; j < closestNamedSeg.attributes.streetIDs.length; j++) {
              var altCity = W.model.cities.getObjectById(W.model.streets.getObjectById(closestNamedSeg.attributes.streetIDs[j]).attributes.cityID).attributes;
      ...
      W.selectionManager.setSelectedModels([NewPlace]);
      if (isPoint) {
        const container = document.querySelector('#venue-edit-general > div.external-providers-control.form-group > wz-button');
        const nameField = document.querySelector('#venue-edit-general > div:nth-child(2) > wz-text-input');
        if (container) {
          container.click()
          setTimeout(() => {
            const box = document.querySelector('#venue-edit-general > div.external-providers-control.form-group > wz-list > wz-list-item > div > div.form-group > wz-autocomplete').shadowRoot.querySelector('#text-input');
            if (box) {
              nameField.addEventListener('keyup', function copyText() {
                box.value = nameField.value;
              });
            }
          }, 1000);
        }
