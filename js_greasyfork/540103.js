// ==UserScript==
// @name        bangumi-auto-relate-infobox-new
...
// @downloadURL https://update.greasyfork.org/scripts/540103/bangumi-auto-relate-infobox-new.user.js
// @updateURL https://update.greasyfork.org/scripts/540103/bangumi-auto-relate-infobox-new.meta.js
// ==/UserScript==
...
(function () {
    ...
    $('#columnCrtRelatedA .board').after(`
        ...
        <div id='staffs' hidden='hidden'>${typeof genPrsnStaffList == 'function' && genPrsnStaffList(-1)}</div>
        <div id="TB_load" style="display: none;"><img src="/img/loadingAnimation.gif"></div>
    `);
    $('#autoRelate').on('click', autoRelate);
    $('#staffs select').removeAttr('name');

    $('li.collect').prepend('<span style="color:#999; cursor:pointer; float:left" class="collect"><a id="addToMap"></a></span>');
    $('#addToMap').text(isCollect() ? '取消关联' : '加入关联').addClass(isCollect() ? 'break' : '').hover(() => $(this).css("color", "#F09199"), () => $(this).css("color", "#999")).on('click', addIntoMap);
    ...
    function autoRelate() {
        ...
        jobMap = new Map();
        $('#staffs option').each(function () {
            let text = $(this).text(), parts = text.split(' / ');
            let job = parts.length > 1 ? (parts[0].trim() || parts[parts.length - 1].trim()) : text.trim();
            jobMap.set(job, $(this).val());
        });

        idxMap = new Map();
        infoMap = new Map();
        $('#infobox li').each(function (idx, ele) {
            let job = ele.childNodes[0].textContent.replace(':', '').trim();
            let staffs = ele.childNodes[1]?.textContent.trim();
    ...
    function addIntoMap() {
        ...
        if ($('#addToMap').hasClass('break')) {
            delete map[name];
            $('#addToMap').text('加入关联').removeClass('break');
        } else {
            map[name] = id.substr(id.lastIndexOf('/') + 1);
            $('#addToMap').text('取消关联').addClass('break');
        }
