...
// @downloadURL https://update.greasyfork.org/scripts/495045/%D0%9B%D0%B5%D0%BC%D0%BE%D0%BD%D1%82%D0%B5.user.js
// @updateURL https://update.greasyfork.org/scripts/495045/%D0%9B%D0%B5%D0%BC%D0%BE%D0%BD%D1%82%D0%B5.meta.js
// ==/UserScript==
	...
	},
{
     ...
     },
    {
    ...
    },
     {
        ...
        content:
        '[CENTER]Доброго времени суток, уважаемый(ая) {{ user.mention }}.<br><br>' +
        '[CENTER]Проверив историю аккаунта игрока который переводил  вам игровую валюту, через систему логирования, я выяснил что он является продавцом игровой валюты а вы его очередной покупатель. <br><br>' +
	...
	},
{
		...
		content:
		'[CENTER]Доброго времени суток, уважаемый(ая) {{ user.mention }}.<br><br>' +
		"[CENTER]На вашем игровом аккаунте имеется чужая привязка, так как чужую привязку отвязать невозможно такие аккаунты блокируются за утерю доступа к аккаунту.<br>" +
        '[CENTER]В следующий раз привязывайте все доступные привязки во избежания таких ситуаций.<br>' +
		'[CENTER]Закрыто.[/CENTER]',
		 prefix: UNACCEPT_PREFIX,
		 status: false,
	},
