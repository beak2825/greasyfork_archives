// ==UserScript==
// @name BLACK RUSSIA TECH (MODDED)
// @namespace https://forum.blackrussia.online
// @version 1.0.0
// @description Для технических шоколадок BLACK RUSSIA
// @author Cyberalg & SoLukky
// @match https://forum.blackrussia.online/threads/*
// @include https://forum.blackrussia.online/threads/
// @grant t.me/cyberalg
// @license t.me/cyberalg
// @icon https://i.yapx.ru/ViO6c.png
// @downloadURL https://update.greasyfork.org/scripts/471972/BLACK%20RUSSIA%20TECH%20%28MODDED%29.user.js
// @updateURL https://update.greasyfork.org/scripts/471972/BLACK%20RUSSIA%20TECH%20%28MODDED%29.meta.js
// ==/UserScript==
...
document.querySelector("button.button--primary.button.button--icon.button--icon--reply.rippleButton").style.border="2px solid #fff";
(function () {
	'use strict';
	...
	const NO_PREFIX = 0;
    const PLAYER_YES_PREFIX = 8;
    const PLAYER_NO_PREFIX = 4;
    const consider = [
        {
		title: 'Рассмотрение',
		content:
	...
	},
	{
		...
		content:
		'[SIZE=4][FONT=Veranda][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
		'[CENTER]С момента выдачи наказания от технического специалиста прошло более 7-ми дней.<br>Пересмотр блокировки невозможен/изменения наказания, вы можете попробывать написать обжалвания через N-ый промежуток времени.[/center]<br><br>'+
	...
	},
	{
		...
		"[SIZE=4][FONT=Veranda][CENTER]Таким образом основания для возврата денежных средств отсутствуют.<br>Подробнееознакомьтесь по данным ссылкам<br><br>[URL='https://blackrussia.online/oferta.php']ДОГОВОР ОФЕРТЫ[/URL]  и [URL='https://blackrussia.online/politica.php']ПОЛИТИКА КОНФИДЕНЦИАЛЬНОСТИ [/URL]" +
    ...
    {
        title: 'Игрок согласен с наказанием',
        content:
        '[SIZE=4][FONT=Veranda][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
		"[CENTER][SIZE=4][FONT=Veranda]Так как вы согласны с выданным наказанием, оно аннулировано не будет.<br>Пожалуйста, ознакомьтесь с Общими Правилами Серверов и больше не нарушайте!<br><br>" +
		"[CENTER][SIZE=4][FONT=Veranda][I][COLOR=rgb(255, 0, 0)]Закрыто[/COLOR][/I].[/FONT][/CENTER]",
        prefix: CLOSE_PREFIX,
		status: false,
    },
...
/* 	{
	...
	addCheckedButton('Рассмотрено', 'watched');
    addDecidedButton('Решено', 'decided');
    addDeniedButton('Отказано', 'unaccept');
    addClosedButton('Закрыто', 'closed');
    addLiner();
    addCommandButton('Команде Проекта', 'teamProject');
	addTechButton('Техническому Специалисту', 'techspec');
    addLiner();
    addPlayersYesButton('Одобрено (игроки)', 'players_applied');
    addPlayersNoButton('Отказано (игроки)', 'players_denied');
	addAnswers();
    addConsider('Рассмотрение', 'consider');
    addAuthor(' ')
    addAuthor('Y')
    addAuthor('K')
    addAuthor('K')
    ...
    $('button#players_applied').click(() => pasteContent(23, threadData, false))
    $('button#players_applied').click(() => editThreadData(PLAYER_YES_PREFIX, false))
    $('button#players_denied').click(() => pasteContent(24, threadData, false))
    $('button#players_denied').click(() => editThreadData(PLAYER_NO_PREFIX, false))
    $(`button#consider`).click(() => pasteContent(2, threadData, true))
	...
	`<button type="button" class="button--primary button rippleButton" id="${id}" style="border-radius: 5px; margin-right: 5px; background: linear-gradient(0deg, rgb(255, 152, 0), rgb(43, 25, 0)); text-shadow: 0px 1px 2px #000; margin-bottom: 15px; flex-grow:1">${name}</button>`,
	);
	}
    function addCommandButton(name, id) {
	$('.button--icon--reply').before(
	`<button type="button" class="button--primary button rippleButton" id="${id}" style="border-radius: 5px; margin-right: 5px; background: linear-gradient(0deg, rgb(255, 235, 59), rgb(52, 47, 0)); text-shadow: 0px 1px 2px #000; margin-bottom: 15px; flex-grow:1">${name}</button>`,
	);
	}
    function addCheckedButton(name, id) {
	$('.button--icon--reply').before(
	`<button type="button" class="button--primary button rippleButton" id="${id}" style="border-radius: 5px; margin-right: 5px; background: linear-gradient(0deg, rgb(76, 175, 80), rgb(13, 29, 13)); text-shadow: 0px 1px 2px #000; margin-bottom: 15px; flex-grow:1">${name}</button>`,
	);
	}
    function addDecidedButton(name, id) {
	$('.button--icon--reply').before(
	`<button type="button" class="button--primary button rippleButton" id="${id}" style="border-radius: 5px; margin-right: 5px; background: linear-gradient(0deg, rgb(76, 175, 80), rgb(13, 29, 13)); text-shadow: 0px 1px 2px #000; margin-bottom: 15px; flex-grow:1">${name}</button>`,
	);
	}
    function addDeniedButton(name, id) {
	$('.button--icon--reply').before(
	`<button type="button" class="button--primary button rippleButton" id="${id}" style="border-radius: 5px; margin-right: 5px; background: linear-gradient(0deg, rgb(211, 47, 47), rgb(35, 8, 8)); text-shadow: 0px 1px 2px #000; margin-bottom: 15px; flex-grow:1">${name}</button>`,
	);
	}
    function addClosedButton(name, id) {
	$('.button--icon--reply').before(
	`<button type="button" class="button--primary button rippleButton" id="${id}" style="border-radius: 5px; background: linear-gradient(0deg, rgb(211, 47, 47), rgb(35, 8, 8)); text-shadow: 0px 1px 2px #000; margin-bottom: 15px; flex-grow:1">${name}</button>`,
	);
	}
    function addTechButton(name, id) {
	$('.button--icon--reply').before(
	`<button type="button" class="button--primary button rippleButton" id="${id}" style="border-radius: 5px; background: linear-gradient(0deg, rgb(13, 71, 161), rgb(2, 12, 27)); text-shadow: 0px 1px 2px #000; margin-bottom: 15px; flex-grow:1">${name}</button>`,
	);
	}
    function addPlayersYesButton(name, id) {
        $('.button--icon--reply').before(
	`<button type="button" class="button--primary button rippleButton" id="${id}" style="border-radius: 5px; margin-right: 5px; background: linear-gradient(0deg, rgb(76, 175, 80), rgb(13, 29, 13)); text-shadow: 0px 1px 2px #000; flex-grow:1">${name}</button>`,
	);
    }
    function addPlayersNoButton(name, id) {
        $('.button--icon--reply').before(
	`<button type="button" class="button--primary button rippleButton" id="${id}" style="border-radius: 5px; margin-right: 5px; background: linear-gradient(0deg, rgb(211, 47, 47), rgb(35, 8, 8)); text-shadow: 0px 1px 2px #000; flex-grow:1">${name}</button>`,
	);
    }
	function addAnswers() {
		$('.button--icon--reply').after(`<button type="button" class="button--primary button rippleButton" id="selectAnswer" style="margin-left: 5px; border-bottom-left-radius: 25px; border-top-left-radius: 25px; border-bottom-right-radius: 5px; border-top-right-radius: 5px; border: 2px solid #F30; background: #db2309">ОТВЕТЫ</button>`,
	);
	}
    function addLiner() {
        $('.button--icon--reply').before(`<button type="button" class="button--primary button rippleButton" id="selectAnswer" style="flex-basis: 100%; height: 0"></button>`)
    }
    function addConsider(name, id) {
		$('.button--icon--write').before(`<button type="button" class="button--primary button rippleButton" id="${id}" style="border-radius: 5px; margin-right: 5px; background: linear-gradient(0deg, rgb(255, 152, 0), rgb(43, 25, 0)); text-shadow: 0px 1px 2px #000;">${name}</button>`,
	);
	}
	function addAuthor(letter) {
        $('[class^="p-breadcrumbs"]').after(`<div class="author name" style="font-size: 26px; font-style: italic; text-shadow: -2px 2px 3px #f0f; color: #f0f"><span>${letter}</span></div>`)
    }
	...
	`rippleButton" style="margin:4px; border: 2px solid #000"><span class="button-text">${btn.title}</span></button>`,
	)
	.join('')}</div>`;
	}
