...
// @downloadURL https://update.greasyfork.org/scripts/552288/WME%20Smart%20Connect%20Fill.user.js
// @updateURL https://update.greasyfork.org/scripts/552288/WME%20Smart%20Connect%20Fill.meta.js
// ==/UserScript==
...
(async () => {
  ...
  const I_EN = Object.freeze({
    ...
    copy_road_type: 'Copy Road type',
    copy_speed: 'Copy Speed limit(s)',
    copy_unpaved: 'Copy Unpaved checkbox',
  ...
  const roadTypeProcessed = new Set();
  const speedProcessed    = new Set();
  const unpavedProcessed  = new Set();
  ...
  const bindUIFromSettings = () => {
    ...
    setChecked('scfInheritRoadTypeCheckBox', S.inheritRoadType);
    setChecked('scfInheritSpeedCheckBox', S.inheritSpeed);
    setChecked('scfInheritUnpavedCheckBox', S.inheritUnpaved);
  ...
  const initPanel = async () => {
    ...
    details.appendChild(checkbox('scfInheritRoadTypeCheckBox', 'inheritRoadType', I.copy_road_type));
    details.appendChild(checkbox('scfInheritSpeedCheckBox', 'inheritSpeed', I.copy_speed));
    details.appendChild(checkbox('scfInheritUnpavedCheckBox', 'inheritUnpaved', I.copy_unpaved));
    ...
    on(resetBtn, 'click', () => { 
      ...
      lockProcessed.clear(); 
      roadTypeProcessed.clear();
      speedProcessed.clear();
      unpavedProcessed.clear();
      dbg('caches cleared'); });
  ...
  const firstNeighborWithUnpaved = (id) => {
    const ns = getNeighbors(id);
    for (let i = 0; i < ns.length; i++) {
      const seg = sdk.DataModel.Segments.getById({ segmentId: ns[i] });
      if (!seg) continue;

      const u = seg?.flagAttributes?.unpaved;
      if (typeof u === 'boolean') {
        dbg('donor unpaved', ns[i], u);
        return seg;
      }
    }
    dbg('no donor unpaved');
    return null;
  };
  ...
  const inheritRoadType = (id) => {
    ...
    // Only touch likely-new segments so we don't surprise-edit existing roads
  ...
  // ---------- core: unpaved ----------
  const inheritUnpaved = (id) => {
    if (!S.inheritUnpaved) return false;

    const target = sdk.DataModel.Segments.getById({ segmentId: id });
    if (!target) return false;

    // Safety: only touch newly-created segments
    if (!isLikelyNew(id, target)) return false;

    const donor = firstNeighborWithUnpaved(id);
    if (!donor) return false;

    const donorValue =
      donor?.flagAttributes?.unpaved ??
      donor?.unpaved ??
      donor?.flags?.unpaved;

    const targetValue =
      target?.flagAttributes?.unpaved ??
      target?.unpaved ??
      target?.flags?.unpaved;

    // No change needed
    if (typeof targetValue === 'boolean' && targetValue === donorValue) {
      return false;
    }
    ...
    try {
      sdk.DataModel.Segments.updateSegment({
        segmentId: id,
        flagAttributes: { ...(target.flagAttributes || {}), unpaved: donorValue }
    });

      dbg('unpaved updated', { id, unpaved: donorValue });
      return true;
    } catch (e) {
      err('unpaved update failed', { id, donorValue, e });
      return false;
    }
  };
  ...
  const run = () => {
    ...
    ids.forEach((id) => {
      ...
      if (candidate && trulyNew) {
        if (!roadTypeProcessed.has(id)) {
          roadTypeProcessed.add(id);
          try { inheritRoadType(id); } catch (e) { err('roadType error', e); }
        }

        if (!speedProcessed.has(id)) {
          speedProcessed.add(id);
          try { inheritSpeed(id); } catch (e) { err('speed error', e); }
        }

        if (!unpavedProcessed.has(id)) {
          unpavedProcessed.add(id);
          try { inheritUnpaved(id); } catch (e) { err('unpaved error', e); }
        }
      }
