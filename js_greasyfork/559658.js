...
// @homepage    https://greasyfork.org/en/scripts/559658
// @downloadURL https://update.greasyfork.org/scripts/559658/Summarize%20with%20AI.user.js
// @updateURL https://update.greasyfork.org/scripts/559658/Summarize%20with%20AI.meta.js
// ==/UserScript==

// modified from: https://greasyfork.org/en/scripts/509192  (Hélio <open@helio.me>)
// see also:      https://greasyfork.org/en/scripts/529791
//                https://greasyfork.org/en/scripts/558384

(function () {
  'use strict'

  // --- Hardcoded Configuration (for iOS Safari compatibility) ---
  // Set these values to bypass the tap-and-hold model selection
  // Leave as empty strings to use saved settings instead
  const HARDCODED_GEMINI_API_KEY = ''  // Example: 'AIza...'
  const HARDCODED_OPENAI_API_KEY = ''
  const HARDCODED_MODEL = 'gpt-5-nano'           // Example: 'gpt-5-nano' or 'gemini-flash-lite-latest'
  const HARDCODED_GPT5_NANO_MAX_TOKENS = 4096    // Max completion tokens for GPT-5 nano (set to 0 to use default)
  ...
  const MODEL_GROUPS = {
    openai: {
      ...
      models       : [
        { id: 'gpt-5-nano', name: 'GPT-5 nano', params: HARDCODED_GPT5_NANO_MAX_TOKENS > 0 ? { max_completion_tokens: HARDCODED_GPT5_NANO_MAX_TOKENS } : {} },
  ...
  async function initialize() {
    ...
    if (articleData) {
      ...
      setupFocusListeners()
      // Use hardcoded model if set, otherwise load from storage
      activeModel = HARDCODED_MODEL || await GM.getValue('last_used_model', activeModel)
    }
  }

  /**
   * Extracts article data using Readability.js with fallback.
  ...
  function getArticleData() {
    ...
    try {
      ...
      // Wrap isProbablyReaderable in try-catch to handle conflicts with other scripts
      let isReaderable = false
      try {
        // eslint-disable-next-line no-undef
        isReaderable = isProbablyReaderable(docClone)
      } catch (e) {
        // Silent failure - conflicts with other scripts are common, just use fallback
        isReaderable = false
      }

      if (!isReaderable) {
        console.log('Summarize with AI: Page not detected as readerable, using fallback extraction.')
      ...
      if (article?.content && article.textContent?.trim()) {
        ...
        console.log('Summarize with AI: Readability extraction successful')
        console.log('Title:', article.title)
        console.log('Content length:', extractedContent.length)
        console.log('Content:', extractedContent)
        return { title: article.title, content: extractedContent }
      } else {
        console.log('Summarize with AI: Readability returned empty content, using fallback extraction.')
        return getFallbackArticleData()
      }
    }
    catch (error) {
      console.error('Summarize with AI: Article parsing failed:', error, '- using fallback extraction.')
      return getFallbackArticleData()
    }
  }

  /**
   * Fallback method to extract page content when Readability fails.
   * @returns {object|null} Article data or null.
   */
  function getFallbackArticleData() {
    try {
        try {
            ...
            var content = clone.innerText.replace(/\s+/g, ' ');
        } catch (error) {
      ...
      // Accept content with at least 50 characters (lowered threshold)
      if (content && content.length > 50) {
        console.log('Summarize with AI: Fallback extraction successful')
      ...
      console.log('Summarize with AI: Fallback extraction returned insufficient content (length:', content.length, ')')
      if (content.length > 0) {
        console.log('Insufficient content was:', content)
      }
      return null
    }
    catch (error) {
      console.error('Summarize with AI: Fallback extraction failed:', error)
      return null
    }
  }
  ...
  function addSummarizeButton() {
    ...
    const startLongPressTimer = (event) => {
      event.preventDefault() // Prevent text selection on iOS Safari
      isLongPress = false
...
Respond helpfully to questions about this article. Use ${lang} language. Use HTML formatting for responses (no markdown, no code blocks). Keep responses concise but informative.`
  ...
  async function getApiKey(service) {
    // Check for hardcoded API keys first
    if (service === 'openai' && HARDCODED_OPENAI_API_KEY) {
      return HARDCODED_OPENAI_API_KEY.trim()
    }
    if (service === 'gemini' && HARDCODED_GEMINI_API_KEY) {
  ...
  async function handleApiKeyReset(service) {
    ...
    const storageKey = `${service}_api_key`
    const currentKey = await GM.getValue(storageKey, '')
    console.log(currentKey ? `${service.toUpperCase()} API key found.` : `No ${service.toUpperCase()} API key set.`)
    const newKey     = prompt(`Enter the new ${service.toUpperCase()} API key (leave blank to clear):`, currentKey)
