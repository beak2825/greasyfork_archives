...
// @version      v.10.7.1
// @description  pancake 2025 unpatched
// @author       HaThu
// @match        *://*.moomoo.io/*
...
// @downloadURL https://update.greasyfork.org/scripts/545967/pancake%20mod%20%5B%20UNPATCHED%20%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/545967/pancake%20mod%20%5B%20UNPATCHED%20%5D.meta.js
// ==/UserScript==
// Unpatched by HaThu

//credit to W4IT for the pancake script
...
newImg.addEventListener("click", () => {
    let w = window.open("https://discord.gg/ckrvZfth", null, `height=650, width=1199, status=yes, toolbar=no, menubar=no, location=no`);
});
...
setInterval(function() {
    if (ws && ws.readyState === WebSocket.OPEN && clanToggle == 1) {
        newSend(['N', [null]]);
        newSend(['L', [animate(false, 5)]]);
    }
}, 200);
setInterval(function() {
    if (ws && ws.readyState === WebSocket.OPEN && messageToggle == 1) {
        newSend(["6", [animate(true, 5)]])
    }
}, 111);
setInterval(function() {
    if (ws && ws.readyState === WebSocket.OPEN && autosecondary == true) {
        newSend(["z", [secondary, true]]);
    }
}, 10);
setInterval(function() {
    if (ws && ws.readyState === WebSocket.OPEN && autoprimary == true) {
        newSend(["z", [primary, true]]);
    }
}, 10);
setInterval(function() {
    if (ws && ws.readyState === WebSocket.OPEN && q == true) {
        place(foodType);
    }
}, 50);
setInterval(() => {
    if(ws && ws.readyState === WebSocket.OPEN && document.getElementById("360hit").checked) { //this didnt work anymore sadsadada
        //newSend(["D", [7.8715926535897935e+270]]);
    }
}, 30);
setInterval ( () => {
    if(ws && ws.readyState === WebSocket.OPEN && nearestEnemy && nearestEnemy[5] == 9) {
        antibow = true
    } else {
        antibow = false
    }
...
setInterval (() => {
    if (ws && ws.readyState === WebSocket.OPEN && autobreak == true && intrap == true) {
        if (secondary == 10) {
            newSend(["z", [secondary, true]]);
        } else {
            newSend(["z", [primary, true]]);
...
setInterval(function() {
    if (autoaim == true) {
        newSend(['D', [nearestEnemyAngle]]);
        if (ws && ws.readyState === WebSocket.OPEN && silentaim == true) {
            aim(nearestEnemy[1]-myPlayer.x+window.innerWidth/2, nearestEnemy[2]-myPlayer.y+window.innerHeight/2);
        };
    }
}, 20);
setInterval(function() {
    if (ws && ws.readyState === WebSocket.OPEN && myPlayer.hat == 45) {
        newSend(['6', ['kill me now = noob']]);
        hat(13);
        acc(13);
    }
}, 100);
...
function handleMessage(_x17) {
    ...
    if(myPlayeroldy != myPlayer.y || myPlayeroldx != myPlayer.x){
        if (walkmillhaha==true) {
            if(Math.sqrt(Math.pow((myPlayer.y-automilly), 2) + Math.pow((myPlayer.x-automillx), 2)) > 100) {
                ...
                newSend(["D", [Math.atan2(mouseY - height / 2, mouseX - width / 2)]]);
                automillx = myPlayer.x;
                automilly = myPlayer.y;
            }
        }
    ...
    if(token == "Q" && document.getElementById('ar').checked/* && isEnemyNear*/){
        place(boostType);
    }
    if(token == "P") {
        intrap = false;
    ...
    if(node[0] == "6" && node[1] !== myPlayer.id && document.getElementById('cm').checked){
        newSend(["6", [node[2]]]);
    }
    if (token == 'O' && node[1] == myPlayer.id) {
...
function newSend(data) {
    if (ws && ws.readyState === WebSocket.OPEN && msgpack5) {
        ws.send(new Uint8Array(Array.from(msgpack5.encode(data))));
    } else {
        console.log('Blocked send packet: ws or msgpack5 not ready');
    }
}
function acc(id) {
    newSend(['c', [0, 0, 1]]);
...
function place(p__14702) {
    ...
    newSend(['z', [p__14702, null]]);
    newSend(['F', [1, angle]]);
    newSend(['F', [0, angle]]);
    newSend(['z', [myPlayer.weapon, true]]);
    newSend(['z', [p__14702, null]]);
    newSend(['F', [1, angle]]);
    newSend(['F', [0, angle]]);
    newSend(['z', [myPlayer.weapon, true]]);
    ...
    if (a.keyCode == 76 && document.activeElement.id.toLowerCase() !== 'chatbox') {//age 1 insta
        if(primary == 0){
            ...
            setTimeout( () => {
                ...
                newSend(["H", [17]]);//cookie
                newSend(["H", [31]]);//trap
                newSend(["H", [27]]);//better mill
                newSend(["H", [10]]);//great hammer
                newSend(["H", [38]]);//tp
            }, 35);

            setTimeout( () => {
                newSend(["H", [4]]);//katana bro this didnt work any lol idk need fixed or not but i think it can xdd
            }, 35);
...
menuChange.innerHTML = `
        ...
        <span class="checkmark"></span></label>
 <label type="radio" class="container">360° hit (didnt workign anymore)<input type="checkbox" id="360hit">
        <span class="checkmark"></span></label>
        ...
        <span class="checkmark"></span></label>
         <label class="container">AutoBreak Pit-Trap?<input type="checkbox" id="autobreaktrap">
            ...
            <h5 class="nothing">Katana Musket didnt work anymore so dont use this shit</h5>
