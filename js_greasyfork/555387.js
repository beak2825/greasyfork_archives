...
// @version      1.0.8
// @author       wellcoming
// @description  在 YouTube 字幕中，认识的单词显示英文，不认识的显示中文，并可以随时添加和修改。
// @license      AGPL-3.0
// @icon         https://www.google.com/s2/favicons?sz=64&domain=youtube.com
// @homepage     https://github.com/wellcoming/ytb-word-memo
// @homepageURL  https://github.com/wellcoming/ytb-word-memo
// @source       https://github.com/wellcoming/ytb-word-memo.git
// @supportURL   https://github.com/wellcoming/ytb-word-memo/issues
...
// @downloadURL https://update.greasyfork.org/scripts/555387/Youtube%20Word%20Memo%20-%20%E7%9C%8B%E8%A7%86%E9%A2%91%E5%AD%A6%E5%8D%95%E8%AF%8D.user.js
// @updateURL https://update.greasyfork.org/scripts/555387/Youtube%20Word%20Memo%20-%20%E7%9C%8B%E8%A7%86%E9%A2%91%E5%AD%A6%E5%8D%95%E8%AF%8D.meta.js
// ==/UserScript==
...
(function () {
  ...
  class CaptionProcessor {
    ...
    static WORD_SPLIT_REGEX = /([\w-']+)/;
    static WORD_TEST_REGEX = /^[a-zA-Z]+(?:['-][a-zA-Z]+)*$/;
...
cfg: RC4.cfg.extend({
  ...
  function createQueryPayload(text) {
  ...
  async function requestDefinition(payload) {
      ...
      _GM_xmlhttpRequest({
        ...
        onload: (response) => {
          if (response.status >= 200 && response.status < 300) {
            const payload2 = response.response ?? response.responseText;
            try {
              const json = typeof payload2 === "string" ? JSON.parse(payload2) : payload2;
              resolve(json);
            } catch (error) {
  ...
  function parseResponse(data) {
    ...
    const phonetics = {
      us: word?.usphone,
      uk: word?.ukphone,
      usSpeechUrl: word?.usspeech ? buildSpeechUrl(word.usspeech) : void 0,
      ukSpeechUrl: word?.ukspeech ? buildSpeechUrl(word.ukspeech) : void 0
    };
    const explains = word?.trs ?? [];
  ...
  class EventHandler {
    ...
    updateWordStyles(wordBase, isKnown) {
  ...
  class WordManager {
    knownWords;
    constructor() {
      this.knownWords = new Set(_GM_getValue("knownWords", DEFAULT_KNOWN_WORDS));
    }
    isWordKnown(word) {
    ...
    saveWords() {
      _GM_setValue("knownWords", Array.from(this.knownWords));
    }
  }
  class Popup {
    element;
    ...
    buildExplainList(explains) {
      ...
      explains.forEach(({ pos, tran }) => {
        ...
        const item = this.createElement("li", void 0, text);
        list.appendChild(item);
      });
      return list;
    }
    buildPhoneticsLine(result) {
  ...
  class App {
    ...
    startObserver(targetNode) {
      ...
      this.handler.attachTo(targetNode);
      new MutationObserver((mutations) => this.handleMutations(mutations)).observe(targetNode, { childList: true, subtree: true });
    ...
    collectSegments(mutations) {
      const segments = [];
      for (const mutation of mutations) {
        for (const node of Array.from(mutation.addedNodes)) {
          if (node.nodeType !== Node.ELEMENT_NODE) continue;
          const found = node.querySelectorAll(".ytp-caption-segment");
          segments.push(...Array.from(found).filter(
            (seg) => !seg.dataset.processed
          ));
        }
      }
      return segments;
  ...
  const styleCss = "#word-popup{position:fixed;display:none;z-index:99999;pointer-events:none;font-family:sans-serif;color:#333}#word-popup .popup-card{border:1px solid #ccc;border-radius:8px;padding:16px;max-width:350px;background-color:#fff;box-shadow:0 6px 16px #00000026;line-height:1.6}#word-popup .popup-title{margin:0 0 10px;font-size:24px;color:#111}#word-popup .popup-phonetics{margin:0 0 15px;color:#555;font-size:14px;white-space:pre-wrap}#word-popup .popup-section-title{font-size:16px;margin:10px 0 5px;color:#222}#word-popup .popup-list{margin:0;padding-left:20px;color:#333}#word-popup .popup-list li+li{margin-top:4px}#word-popup .popup-error{color:#d14343;font-size:14px}#word-popup .popup-error b{font-size:16px}.ytb-word-memo-word{cursor:pointer}.ytb-word-memo-word.unknown{color:#42e2f5;text-shadow:0 0 5px rgba(66,226,245,.8)}";
  importCSS(styleCss);
  const app = new App();
  app.start();
