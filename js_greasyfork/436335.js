...
// @version      1.11
// @description  不白嫖，为观看时间超过10s的视频自动点赞
// @author       kakasearch
...
// @grant        GM_getValue
// @run-at        document-end
// @downloadURL https://update.greasyfork.org/scripts/436335/B%E7%AB%99%E7%82%B9%E8%B5%9E%E5%A7%AC.user.js
// @updateURL https://update.greasyfork.org/scripts/436335/B%E7%AB%99%E7%82%B9%E8%B5%9E%E5%A7%AC.meta.js
// ==/UserScript==

(function() {
    'use strict';
    function fireKeyEvent(el, evtType, keyCode){
        //参考自 https://blog.csdn.net/lovelyelfpop/article/details/52471878
    ...
    function main(){
        ...
        let tmp = setInterval(function(){
            ...
            if(video){
                clearInterval(tmp)
                let bv = /BV[a-zA-Z0-9]+/.exec(window.location.href)[0]
                if(bv && GM_getValue(bv)){
                    console.log(bv,GM_getValue(bv))
                    console.log("之前点过赞")
                }else{
                    GM_setValue(bv,new Date().toLocaleString())
                    setTimeout(function(){
                        fireKeyEvent(window.document.body,"keydown",81)
                        fireKeyEvent(window.document.body,"keyup",81) //点击1次Q键，删除此行变为长按q，即三连
                        console.log("已点赞")},10000)
                }
            }
        },3000)
