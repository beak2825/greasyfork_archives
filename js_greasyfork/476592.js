...
// @version      0.5
// @description  Starlanivia faucet rotator
// @author       White
// @match        https://starlavinia.name.tr/*
// @icon         https://www.google.com/s2/favicons?sz=64&domain=name.tr
// @grant        GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/476592/starlavinia.user.js
// @updateURL https://update.greasyfork.org/scripts/476592/starlavinia.meta.js
// ==/UserScript==
...
const redirectToNextURL = async () => {
    ...
    for (let i = 0; i < claimUrls.length; i++) {
        if (currentURL.includes(claimUrls[i])) {
            ...
            try {
                await delay(5000);
                window.location.href = `https://starlavinia.name.tr/faucet/currency/${nextURL}`;
...
const claimUrlPrefix = "https://starlavinia.name.tr/faucet/currency/";

async function init() {
    const targetUrl = "https://starlavinia.name.tr/?r=8734";
...
function redirectToRandomURL() {
    ...
    const randomURL = `https://starlavinia.name.tr/faucet/currency/${claimUrls[randomIndex]}`;
...
async function Logic() {
  ...
  if (currentUrl === "https://starlavinia.name.tr/" || currentUrl === "https://starlavinia.name.tr") {
    window.location.href = "https://starlavinia.name.tr/?r=8734";
  }
}
...
window.addEventListener('load', () => {
    if (document.title.includes('just a moment') || document.title.includes('um momento')) {
