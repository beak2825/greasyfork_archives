...
(function () {
    ...
    */
    ...
    async function addDoubaoQueryParam() {
        const query = new URLSearchParams(window.location.search).get('q');
        if (!query || !query.trim()) {
            console.log('[豆包优化] URL中未检测到有效q参数，脚本终止');
            return;
        }
        console.log(`[豆包优化] 检测到查询参数：${query}`);

        // ================= 核心工具函数 =================
        /** 精准等待元素加载（支持多选择器、超时重试） */
        const waitForElement = async (selectors, timeout = 15000) => {
            ...
            while (Date.now() - startTime < timeout) {
                for (const selector of selectorList) {
                    try {
                        ...
                        if (element && element.offsetParent !== null) { // 确保元素可见
                            console.log(`[豆包优化] 成功定位元素：${selector}`);
                            return element;
                        }
                    } catch (err) {
                        lastError = err;
                ...
                await delay(200); // 缩短重试间隔，提高响应速度
            }

            console.error(`[豆包优化] 元素定位超时（${timeout}ms）：${selectorList.join(' / ')}`, lastError);
        ...
        /** 触发React/Vue框架识别的输入事件（关键优化） */
        const triggerFrameworkEvents = (element, eventTypes = ['input', 'change', 'blur', 'focus']) => {
            eventTypes.forEach(type => {
                const event = new Event(type, {
                    bubbles: true,
                    cancelable: true,
                    composed: true // 穿透Shadow DOM（如果有）
                });
                // 手动添加inputType（部分框架依赖）
                if (type === 'input') {
        ...
        try {
            ...
            textarea.focus();
            await delay(300);
            setNativeValue(textarea, ''); // 清空原生值
            triggerFrameworkEvents(textarea, ['input', 'change']); // 触发清空事件
            await delay(200);

            // 3. 模拟真实输入（分块填充，提高框架识别率）
            const maxChunkSize = 15; // 每块最大字符数
            const chunks = [];
            for (let i = 0; i < query.length; i += maxChunkSize) {
                chunks.push(query.substring(i, i + maxChunkSize));
            }

            console.log(`[豆包优化] 分块填充（共${chunks.length}块）`);
            for (const chunk of chunks) {
                setNativeValue(textarea, textarea.value + chunk); // 累加内容
                triggerFrameworkEvents(textarea, ['input']); // 每块触发一次输入事件
                await delay(80 + Math.random() * 50); // 模拟打字间隔
            }

            // 4. 验证输入内容（确保框架已同步）
            await delay(500);
            const actualValue = textarea.value.trim();
            const expectedValue = query.trim();
            if (actualValue !== expectedValue) {
                console.warn(`[豆包优化] 内容不一致，重新填充：期望="${expectedValue}"，实际="${actualValue}"`);
                setNativeValue(textarea, expectedValue);
                triggerFrameworkEvents(textarea);
                await delay(300);
            } else {
            ...
            if (textarea.value.trim() === '') {
                console.log('[豆包优化] ✅ 消息发送成功（输入框已清空）');
            } else {
                console.log('[豆包优化] ✅ 消息发送请求已提交（输入框未清空，可能是框架设计）');
            }

        } catch (error) {
            console.error('[豆包优化] ❌ 脚本执行失败:', error.message);
            // 异常兜底：直接通过URL跳转触发搜索（如果支持）
            try {
                const searchUrl = `https://www.doubao.com/chat?q=${encodeURIComponent(query)}`;
                if (window.location.href !== searchUrl) {
                    console.log(`[豆包优化] 兜底方案：跳转至搜索URL`, searchUrl);
                    window.location.href = searchUrl;
                }
            } catch (err) {
                console.error('[豆包优化] 兜底方案失败:', err);
            }
        }
    }
    // 仅在豆包聊天页面执行
    if (window.location.host === 'www.doubao.com' && window.location.pathname.startsWith('/chat')) {
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => {
                setTimeout(addDoubaoQueryParam, 1000); // 延迟1秒，避免框架未就绪
            });
        } else {
            setTimeout(addDoubaoQueryParam, 1000);
        }
    }
