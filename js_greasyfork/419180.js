// ==UserScript==
// @name          On Hover Link-Type Icons
// @namespace     OHLTIhal
// @description	  This inserts icons before or after links on a page, ON HOVER, in order to inform the user of whether these links open other programs, open new windows, use certain protocols, utilize javascript or plug-ins etc...
// @author        iralakaelah
// @homepage      https://greasyfork.org/en/scripts/
// @run-at        document-start
// @include       http*://*
// @version       1.20
// @downloadURL https://update.greasyfork.org/scripts/419180/On%20Hover%20Link-Type%20Icons.user.js
// @updateURL https://update.greasyfork.org/scripts/419180/On%20Hover%20Link-Type%20Icons.meta.js
// ==/UserScript==
(function() {var css = [
	"/*link hover icons*/",
	"",
	"/*created by iralakaelah on 11.3.08*/",
	"",
	"/*images*/",
	"",
	"a[href$=\".qtz\"]:hover:after, a[href$=\".targa\"]:hover:after, a[href$=\".tga\"]:hover:after, a[href$=\".tif\"]:hover:after, a[href$=\".tiff\"]:hover:after, a[href$=\".pict\"]:hover:after, a[href$=\".pic\"]:hover:after, a[href$=\".pct\"]:hover:after, a[href$=\".png\"]:hover:after, a[href$=\".sgi\"]:hover:after, a[href$=\".rgb\"]:hover:after, a[href$=\".bmp\"]:hover:after, a[href$=\".dib\"]:hover:after, a[href$=\".jp2\"]:hover:after, a[href$=\".pntg\"]:hover:after, a[href$=\".pnt\"]:hover:after, a[href$=\".mac\"]:hover:after, a[href$=\".qtif\"]:hover:after, a[href$=\".qti\"]:hover:after",
	"{",
	"padding:0px 3px; content:url(\"data:image/gif;base64,%3D%3D\");",
	"} ",
	"",
	"/*audio*/",
	"",
	"a[href$=\".mp3\"]:hover:after, a[href$=\".swa\"]:hover:after, a[href$=\".qcp\"]:hover:after, a[href$=\".gsm\"]:hover:after, a[href$=\".aiff\"]:hover:after, a[href$=\".aif\"]:hover:after, a[href$=\".aifc\"]:hover:after, a[href$=\".cda\"]:hover:after, a[href$=\".m1s\"]:hover:after, a[href$=\".m1a\"]:hover:after, a[href$=\".mp2\"]:hover:after, a[href$=\".mpa\"]:hover:after, a[href$=\".m2a\"]:hover:after, a[href$=\".wav\"]:hover:after, a[href$=\".bwf\"]:hover:after, a[href$=\".mid\"]:hover:after, a[href$=\".midi\"]:hover:after, a[href$=\".smf\"]:hover:after, a[href$=\".kar\"]:hover:after, a[href$=\".aac\"]:hover:after, a[href$=\".adts\"]:hover:after, a[href$=\".amr\"]:hover:after, /*a[href$=\".au\"]:hover:after,*/a[href$=\".snd\"]:hover:after, a[href$=\".ulw\"]:hover:after, a[href$=\".m4p\"]:hover:after, a[href$=\".m4a\"]:hover:after, a[href$=\".3gp\"]:hover:after, a[href$=\".3gpp\"]:hover:after, a[href$=\".m4b\"]:hover:after, a[href$=\".caf\"]:hover:after",
	"{",
	"padding:0px 3px; content:url(\"data:image/gif;base64,%3D\");",
	"}",
	"",
	"/*video*/",
	"",
	"a[href$=\".avi\"]:hover:after, a[href$=\".vfw\"]:hover:after, a[href$=\".mpeg\"]:hover:after, a[href$=\".mp4\"]:hover:after, a[href$=\".mpg\"]:hover:after, a[href$=\".m1v\"]:hover:after, a[href$=\".m75\"]:hover:after, a[href$=\".mpm\"]:hover:after, a[href$=\".m15\"]:hover:after, a[href$=\".mpv\"]:hover:after, a[href$=\".mov\"]:hover:after, a[href$=\".qt\"]:hover:after, a[href$=\".mqv\"]:hover:after, a[href$=\".sdv\"]:hover:after, a[href$=\".3g2\"]:hover:after, a[href$=\".3gp2\"]:hover:after, a[href$=\".m4v\"]:hover:after, a[href$=\".flc\"]:hover:after, a[href$=\".fli\"]:hover:after, a[href$=\".cel\"]:hover:after",
	"{",
	"padding:0px 3px; content:url(\"data:image/gif;base64,\");",
	"}",
	"",
	"/*stream*/",
	"",
	"a[href$=\".sdp\"]:hover:after, a[href$=\".amc\"]:hover:after, a[href$=\".rtsp\"]:hover:after, a[href$=\".rts\"]:hover:after, a[href^=\"sdp:\"]:hover:after, a[href^=\"rtsp:\"]:hover:after, a[href^=\"rts:\"]:hover:after, a[href^=\"udp:\"]:hover:after, a[href^=\"mms:\"]:hover:after, a[href^=\"rtp:\"]:hover:after, a[href^=\"vdownload:\"]:hover:after, a[href^=\"quicktime:\"]:hover:after",
	"{",
	"padding:0px 3px; content:url(\"data:image/gif;base64,%3D%3D\");",
	"}",
	"",
	"/*adobe*/",
	"",
	"a[href$=\".pdf\"]:hover:after, a[href$=\".spdf\"]:hover:after",
	"{",
	"padding:0px 3px; content:url(\"data:image/gif;base64,\");",
	"}",
	"",
	"/*java*/",
	"",
	"a[href$=\".jnlp\"]:hover:after, a[href$=\".javaapplet\"]:hover:after",
	"{",
	"padding:0px 3px; content:url(\"data:image/gif;base64,%3D\");",
	"}",
	"",
	"/*e-mail*/",
	"",
	"a[href^=\"mailto:\"]:hover:after, a[href^=\"mail:\"]:hover:after",
	"{",
	"padding:0px 3px; content:url(\"data:image/gif;base64,%3D\");",
	"}",
	"",
	"/*secure site*/",
	"",
	"a[href^=\"https:\"]:hover:before",
	"{",
	"padding:0px 3px;",
	"content:url(\"data:image/gif;base64,%3D%3D\");",
	"}",
	"",
	"/*javascript*/",
	"",
	"a[href^=\"javascript:\"]:hover:after",
	"{",
	"padding:0px 3px;",
	"content:url(\"data:image/gif;base64,\");",
	"}",
	"",
	"/*new windows*/",
	"",
	"a[target*=\"_blank\"]:hover:before, a[target*=\"_new\"]:hover:before, a[target*=\"new\"]:hover:before, a[target*=\"nw\"]:hover:before, a[target*=\"new_window\"]:hover:before, a[target*=\"blank\"]:hover:before, a[target*=\"_nw\"]:hover:before, a[target*=\"newwindow\"]:hover:before, a[target*=\"_newwindow\"]:hover:before, a[target*=\"_new_window\"]:hover:before, a[onclick*=\"window.open\"]:hover:before, a[href^=\"javascript:window.open\"]:hover:before, a[href^=\"javascript:newwindow\"]:hover:before, a[href^=\"javascript:new_window\"]:hover:before, a[href^=\"javascript:_new_window\"]:hover:before, a[href^=\"javascript:_newwindow\"]:hover:before",
	"{",
	"padding:0px 3px;",
	"content:url(\"data:image/gif;base64,\");",
	"}",
	"",
	"/*ical files*/",
	"",
	"a[href^=\"webcal:\"]:hover:after, a[href$=\".ics\"]:hover:after, a[href$=\".ical\"]:hover:after, a[href^=\"ical:\"]:hover:after",
	"{",
	"padding:0px 3px;",
	"content:url(\"data:image/gif;base64,%3D%3D\");",
	"}",
	"",
	"/*flash*/",
	"",
	"a[href$=\".swf\"]:hover:after, a[href$=\".spl\"]:hover:after, a[href$=\".flv\"]:hover:after",
	"{",
	"padding:0px 3px;",
	"content:url(\"data:image/gif;base64,\");",
	"}",
	"",
	"/*feeds and podcasts*/",
	"",
	"a[href^=\"feed:\"]:hover:after, a[href^=\"nntp:\"]:hover:after, a[href^=\"webnews:\"]:hover:after, a[href^=\"news:\"]:hover:after, a[href^=\"rss:\"]:hover:after, a[href^=\"atom:\"]:hover:after, /*a[href^=\"//rss.\"]:hover:after, a[href^=\"//atom.\"]:hover:after, a[href^=\"//feed.\"]:hover:after, a[href*=\".rss\"]:hover:after, a[href*=\".atom\"]:hover:after, */a[href^=\"http://phobos.apple.com\"]:hover:after, a[href^=\"snews:\"]:hover:after, a[href^=\"sherlock:\"]:hover:after, a[href^=\"photo:\"]:hover:after, a[href^=\"pcast:\"]:hover:after, a[href^=\"itpc:\"]:hover:after, a[href^=\"itmms:\"]:hover:after, a[href^=\"itms:\"]:hover:after, a[href^=\"feedsearch:\"]:hover:after, a[href^=\"feeds:\"]:hover:after, a[href^=\"daap:\"]:hover:after/*, a[href*=\"/feed/\"]:hover:after, a[href*=\"/rss/\"]:hover:after, a[href*=\"/atom/\"]:hover:after a[href$=\"atom.xml\"]:hover:after, a[href$=\"atom.php\"]:hover:after, a[href$=\"rss.xml\"]:hover:after, a[href$=\"rss.php\"]:hover:after, a[href$=\"/feed\"]:hover:after, a[href$=\"/rss\"]:hover:after, a[href$=\"/atom\"]:hover:after, a[href$=\"feed/\"]:hover:after, a[href$=\"rss/\"]:hover:after, a[href$=\"atom/\"]:hover:after*/",
	"{",
	"padding:0px 3px;",
	"content:url(\"data:image/gif;base64,%3D\");",
	"}",
	"",
	"/*messaging*/",
	"",
	"a[href^=\"aim:\"]:hover:after, a[href^=\"jabber:\"]:hover:after, a[href^=\"icq:\"]:hover:after, a[href^=\"irc:\"]:hover:after, a[href^=\"telnet:\"]:hover:after, a[href^=\"ssh:\"]:hover:after, a[href^=\"ichat:\"]:hover:after",
	"{",
	"padding:0px 3px;",
	"content:url(\"data:image/gif;base64,%3D\");",
	"}",
	"",
	"/*ftp*/",
	"",
	"a[href^=\"ftp:\"]:hover:before, a[href^=\"sftp:\"]:hover:before, a[href^=\"ftps:\"]:hover:before",
	"{",
	"padding:0px 3px;",
	"content:url(\"data:image/gif;base64,%3D%3D\");",
	"}"
].join("\n");
if (typeof GM_addStyle != "undefined") {
	GM_addStyle(css);
} else if (typeof PRO_addStyle != "undefined") {
	PRO_addStyle(css);
} else if (typeof addStyle != "undefined") {
	addStyle(css);
} else {
	var node = document.createElement("style");
	node.type = "text/css";
	node.appendChild(document.createTextNode(css));
	var heads = document.getElementsByTagName("head");
	if (heads.length > 0) {
		heads[0].appendChild(node);
	} else {
		// no head yet, stick it whereever
		document.documentElement.appendChild(node);
	}
}
})();
