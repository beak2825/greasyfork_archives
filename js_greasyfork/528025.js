// ==UserScript==
// @name        [editVersion]Twitter Media Downloader[2504fix]
// @name:ja     [editVersion]Twitter Media Downloader[2504fix]
// @name:zh-cn  [editVersion]Twitter 媒体下载[2504fix]
// @name:zh-tw  [editVersion]Twitter 媒體下載[2504fix]
...
// @compatible  Firefox
// @license     MIT
// @downloadURL https://update.greasyfork.org/scripts/528025/%5BeditVersion%5DTwitter%20Media%20Downloader%5B2504fix%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/528025/%5BeditVersion%5DTwitter%20Media%20Downloader%5B2504fix%5D.meta.js
// ==/UserScript==
/* jshint esversion: 8 */
...
function timestampToYMDHMS(timestamp) {
    ...
    const month = ('0' + (date.getUTCMonth() + 1)).slice(-2); // 月份是从0開始的
...
// let reversePreViewLeftRight = true
let imageSizeKey = "name=";
let imageSize = ["small", "medium", "Orig"]
let imageSizeIndex = 1;
// 就是让预览框偏向中间还是两侧,感觉两侧很多时候不点 , 而且中间的图片也就占1/3左右 , 放两侧还挺合适
let previewSwitch = true;
let previewOnSide = true;

// tag_ppEdit
let previewDiv = document.createElement("div");

previewDiv.id = "helloTwitterMedia";
...
const TMD = (function () {
    ...
    return {
        init: async function () {
            ...
            // tag_ppEdit_preview
            divWidth = GM_getValue('previewWidth', 700);
            divHeight = GM_getValue('previewHeight', 700);
            previewDiv.style.width = divWidth + "px";
            previewDiv.style.height = divHeight + "px";
            document.body.appendChild(previewDiv);

            previewSwitch = GM_getValue('previewSwitch', true);
            console.log(" 当前的预览开关 ", previewSwitch);
            previewOnSide = GM_getValue('previewOnSide', true);
            console.log(" 当前的预览位置 ", previewOnSide)
            ...
            document.addEventListener("mousemove", function (event) {
                ...
                let nodeSrc = node.src;
                if (node !== lastNode) {
                    console.log(" 鼠标焦点变化 ")
                    if (nodeSrc == null || !nodeSrc.includes("pbs.twimg.com/media")) {
                        previewDiv.style.display = "none";
                    } else {
                        previewDiv.style.display = "block";
                        // // 网上搜了,总共四种 &name=small 、 &name=medium 、 &name=Large 、 &name=Orig
                        previewImg.src = getNewUrl(event.target.src)
                    }
                }
                lastNode = node;
                movePreviewDiv(event.clientX, event.clientY, divWidth, divHeight);
            });

            function getNewUrl(url) {
                let index = url.indexOf(imageSizeKey);
                if (index < 0) {
                    return url;
                }
                let i = index + imageSizeKey.length;
                if (url.substring(i, i + 5) === "small") {
                    return url.substring(0, i) + "medium" + url.substring(i + 5);
                }
                let numArr = [];
                let numIndex = -1;
                for (; i < url.length; i++) {
                    let c = url.charAt(i) - '0';
                    if (c >= 0 && c <= 9) {
                        numArr[++numIndex] = 0;
                        for (; i < url.length && (c = url.charAt(i)) >= '0' && url.charAt(i) <= '9'; i++) {
                            numArr[numIndex] = numArr[numIndex] * 10 + (c - '0');
                        }
                        if (numIndex === 1) {
                            break;
                        }
                ...
                if (numIndex <= 0) {
                    ...
                    // url = url.substring(0, index) + "name=" + (numArr[0] * 2) + "x" + (numArr[1] * 2) + url.substring(i);
                    // 好像只能是固定的 4096*4096
                    url = url.substring(0, index) + "name=4096x4096" + url.substring(i);
                }
                return url;
            }

            function movePreviewDiv(clientX, clientY, divWidth, divHeight) {
                ...
                // 左右上下超出的距离
                let leftOutLen = clientX - divWidth;
                let topOutLen = clientY - divHeight;
                let rightOutLen = windowWidth - clientX - divWidth;
                let bottomOutLen = windowHeight - clientY - divHeight;

                let isOnRight = (leftOutLen < rightOutLen) ^ previewOnSide;
                let targetLeft = isOnRight ? clientX + 10 : clientX - divWidth - 10;
                previewImg.style.float = isOnRight ? "left" : "right";
                let targetTop = topOutLen < bottomOutLen ? clientY + 10 : clientY - divHeight - 10;
                // 上下和左右只能调一个 , 否则鼠标会和窗口重叠 , 鉴于一般窗口都是宽的 , 那么只调整上下
                targetTop = targetTop < 0 ? 0 : targetTop;
                let maxTop = windowHeight - divHeight;
                targetTop = targetTop > maxTop ? maxTop : targetTop;

                previewDiv.style.left = targetLeft + "px";
                previewDiv.style.top = targetTop + "px";
            }
        ...
        click: async function (btn, status_id, is_exist, index) {
            ...
            // tag_ppEdit001
            console.log(" 当前的推文id ", status_id)
            // 喜欢推文的接口
            let favoriteResult = await this.favoriteTweet(status_id, "lI07N6Otwv1PhnEgXILM7A");
            if (null == favoriteResult) {
                this.displayFavoriteResult_simp('http error while favorite twitter');
                console.log('http error while favorite twitter')
                return;
            }
            let res = this.displayFavoriteResult(favoriteResult, status_id);
            if (res == null || !res) {
                console.log(" 已经like过了 , 不下载 ")
        ...
        // tag_ppEdit , 有bug , 不知道为啥有时候会失败 , 404
        favoriteTweet: async function (tweet_id, queryId) {
            let base_url = `https://${host}/i/api/graphql/${queryId}/FavoriteTweet`;
            let variables = {
                "tweet_id": tweet_id
            };
            // let queryId = "lI07N6Otwv1PhnEgXILM7A";
            ...
            try {
                ...
                console.log("Favorite Tweet Response:", response);
                if (!response.ok) {
                    console.error("Favorite Tweet Response --- HTTP error!");
                    return null;
                }
                let result = await response.json();
                console.log("Favorite Tweet Response json:", result);
                return result;
            } catch (error) {
        ...
        settings: async function () {
            ...
            // tag_ppEdit
            let widthHeightDiv = $element(options, 'div', 'margin: 1px 2px;background: white;', lang.dialog.width + "," + lang.dialog.height);
            let preview_height_input = $element(widthHeightDiv, 'input', 'float: right;background: white;', GM_getValue('previewHeight'));
            let preview_width_input = $element(widthHeightDiv, 'input', 'float: right;background: white;', GM_getValue('previewWidth'));
            let previewSetDiv = $element(options, 'div', 'margin: 1px 2px;', "preOnOff,previewOnSide(other is on center)");
            let previewSwitchDiv = $element(previewSetDiv, 'input', 'float: right;', "checkbox");
            let previewOnSideDiv = $element(previewSetDiv, 'input', 'float: right;', "checkbox");
            ...
            preview_width_input.onchange = () => {
                let newPreviewWidth = preview_width_input.value;
                console.log(" 预览宽度变化 : ", newPreviewWidth);
                GM_setValue('previewWidth', newPreviewWidth);
                divWidth = newPreviewWidth;
                previewDiv.style.width = newPreviewWidth + "px";
            }
            preview_height_input.onchange = () => {
                let newPreviewHeight = preview_height_input.value;
                console.log(" 预览高度变化 : ", newPreviewHeight);
                GM_setValue('previewHeight', newPreviewHeight);
                divHeight = newPreviewHeight;
                previewDiv.style.height = newPreviewHeight + "px";
            }
            previewSwitchDiv.onchange = () => {
                previewSwitch = previewSwitchDiv.checked;
                console.log(" 是否开启预览 : ", previewSwitchDiv.checked);
                GM_setValue('previewSwitch', previewSwitch);
            };
            previewOnSideDiv.onchange = () => {
                previewOnSide = previewOnSideDiv.checked;
                console.log(" 预览框是在两侧还是在中央 : ", previewOnSideDiv.checked);
                GM_setValue('previewOnSide', previewOnSide);
            };
...
//let tweet_result = tweet_entrie.content.itemContent.tweet_results.result;
        ...
        // tag_ppEdit
        displayFavoriteResult: function (result, status_id) {
            let favoriteDiv = document.getElementById('favorite-result');
            if (!favoriteDiv) {
                favoriteDiv = document.createElement('div');
                favoriteDiv.id = 'favorite-result';
                favoriteDiv.style.position = 'fixed';
                // favoriteDiv.style.top = '10px';
                // favoriteDiv.style.left = '10px';
                favoriteDiv.style.top = '2px';
                favoriteDiv.style.left = '2px';
                favoriteDiv.style.backgroundColor = '#fff';
                // favoriteDiv.style.padding = '10px';
                favoriteDiv.style.border = '1px solid #ccc';
                favoriteDiv.style.zIndex = '1000';
                favoriteDiv.style.color = "black";
                favoriteDiv.style.fontSize = "10px";
                document.body.appendChild(favoriteDiv);
            }
            let data = {};
            data.result = result;
            data.time = timestampToYMDHMS(Date.now() + timeOffset);
            data.status_id = status_id;
            favoriteDiv.innerHTML = result ? JSON.stringify(data, null, 2) : 'Failed to favorite tweet';
            return result != null && result.data != null && result.data.favorite_tweet != null && result.data.favorite_tweet === 'Done';
        },

        displayFavoriteResult_simp: function (str) {
            let favoriteDiv = document.getElementById('favorite-result');
            if (!favoriteDiv) {
                favoriteDiv = document.createElement('div');
                favoriteDiv.id = 'favorite-result';
