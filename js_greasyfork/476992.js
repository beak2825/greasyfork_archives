...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/476992/Google%20Search%20Extra%20Buttons%20%28Position%20Slightly%20Right%29.user.js
// @updateURL https://update.greasyfork.org/scripts/476992/Google%20Search%20Extra%20Buttons%20%28Position%20Slightly%20Right%29.meta.js
// ==/UserScript==
...
var xLocStI = 0,
  xLocSto = [
    {
      origin: "https://spmbt.github.io",
      restHref: "/googleSearchExtraButtons/saveYourLocalStorage.html",
    },
    {
      origin: "https://www.gstatic.com",
      restHref: "/sites/p/b9356d/system/services/test.html",
      "//": "blank page",
    },
    {
      origin: "https://www.gstatic.com",
  ...
  window.addEventListener(
    ...
    function (ev) {
      if (/^https?:\/\/www\.google\./.test(ev.origin)) {
        var d =
          typeof ev.data == "string" && ev.data[0] == "{"
            ? JSON.parse(ev.data)
            : ev.data;
        if (!d.do) return;
        var tok = d.tok,
        ...
        try {
          switch (d.do) {
            ...
            case "get":
              ...
              prev =
                prev === undefined ||
                  (typeof prev == "string" && prev[0] != "{")
                  ? prev
                  : JSON.parse(prev);
              break;
            case "remove":
              prev = localStorage[key];
              if (prev !== undefined) localStorage.removeItem(key);
          }
        } catch (er) { }
    ...
    !1
  ); //console.log('[xLocSto-1]_'+ xLocStI);
} else
  (function (setts) {
    //lang, sites, lastHoursLess

    var $x = function (el, h) {
      if (h) for (var i in h) el[i] = h[i];
      return el;
    }, //===extend===
      $pd = function (ev) {
        ev.preventDefault();
      ...
      d = document,
      $q = function (q, el) {
        return (el || d).querySelector(q);
      },
      lh = location.href,
      $e = function (g, el) {
        //===create or use existing element=== //g={el|clone,cl,ht,cs,at,atRemove,on,apT}
        g.el = el || g.el || g.clone || "DIV";
        var o = (g.o =
          (g.clone && g.clone.cloneNode && g.clone.cloneNode(!0)) ||
        ...
        if (o) {
          ...
          if (g.atRemove)
            for (var i in g.atRemove) o.removeAttribute(g.atRemove[i]);
          if (g.on)
            for (var i in g.on) if (g.on[i]) o.addEventListener(i, g.on[i], !1);
      ...
      addRules = function (css) {
        $e(
          { apT: d.getElementsByTagName("head")[0], ap: d.createTextNode(css) },
          "style"
        );
      },
      ...
      }, //example: t:120, i:12, m: 1.6 => wait around 55 sec
      //for xLocStor:
      xLocStorOrigin =
        d.location.protocol + xLocSto[xLocStI].origin.replace(/[^/]*/, ""),
      qr,
      qrs = {}, //set of queries "key-calls" (ок, toutLitt, toutLong, noService, noStorage)
      qrI = 0, //queries counter
      qrN = 12, //max number of waiting queries
      errIMax = 120,
      errNMax = errIMax, //max number of errors
      ...
      xLocStor = function (h) {
        ...
        var ifr = d.getElementById("xLocStor"),
          query = function () {
            ...
            qrs[token] = $x(
              {
                //for wait of response
                wToutLitt: (function (h, qrI, errIMax) {
                  return setTimeout(function () {
                    qrI -= 1;
                    if ((errIMax -= 1) >= 0); //console.warn('toutLitt', h);
                    chkErrMax();
                  }, h.toutLitt);
                })(h, qrI, errIMax),
                wTout: (function (h, qrI) {
                  return setTimeout(function () {
                    qrI -= 1;
                    //xCatch('tout', null, h);
                    //xLocStor(h0);
            ...
            function (ev) {
              if (ev.origin == xLocStorOrigin) {
                ...
                if ((qr = qrs[resp.tok])) {
                  ...
                  var er = qr.err;
                  delete qrs[resp.tok];
                } // else ignore unsufficient token
                if (resp.err && (!er || er(resp.err)))
                  //individual or common error processing depends of er()
      ...
      chkErrMax = function () {
        if (!errIMax) console.error("Too many err messages:", errNMax);
      },
      ...
      meta = {
        ...
        Mailru: "",
        SlideS: "",
      }, //will create child-tabs (window names)
      imgFile = "SVG,JPG,GIF,PNG,BMP,webp,ICO,RAW".split(",").map(function (x) {
        return "&nbsp;" + x + "&nbsp; ";
      }), //will switch to Img Search
      ...
      $l = {
        ru: {
          ...
          "Less positions at the end of selects":
            "Меньше выбора в конце селектов",
        ...
        fr: {
          ...
          "Less positions at the end of selects":
            "Moins de choix les longues listes",
        ...
        de: {
          ...
          "Less positions at the end of selects":
            "Weniger Auswahl in langen Listen",
        ...
        es: {
          ...
          "Less positions at the end of selects":
            "Menos elección en listas largas",
    ...
    var bBack = /^(?:rgba?\((\d+)|#(.))/.exec(
      window.getComputedStyle(d.body).backgroundColor.replace(/gb/, "gba")
    ), // for Images tab
      mDark =
        (d.querySelectorAll('meta[name="color-scheme"]')[0] || {}).content ===
        "dark" ||
        (bBack && ((bBack[1] && bBack[1] < 96) || (bBack[2] && bBack[2] < 6)));
    addRules(
      ".hp .sfsbc,.sfsbc{display:inline-block}.siteList:hover button{display:block}" +
      ...
      ".UUbT9 >div.aajZCb{background-color:rgba(" +
      (mDark ? "40,44,48, 0.92" : "255,255,255, 0.75") +
      ");}" + //opacity for suggests
      ".UUbT9 ul li div span b{background-color:rgba(" +
      (mDark ? "88,93,99" : "237,242,248") +
      ", 0.9); margin:0 -6px 0 4px; padding:0 6px 0 0;border-radius:7px;}" + //white under suggest texts
      ".gb_kb{padding-left:10px; padding-right:7px}form .RNNXgb{position:relative; background:rgba(" +
      (mDark ? "40,44,48" : "255,255,255") +
      ", 0.92)}" +
    ...
    try {
      xLocStor({
        ...
        key: "sett",
        val: setts,
        cB: (cB = function (prev, undef) {
          S = prev || setts;
          S.dwmyh = S.dwmyh || setts.dwmyh; //temp. transitional expr.
          console.timeStamp = function () { };
          addRules(
            !(S.whiteMintOval || S.whiteMintOval === undefined) //blue old design
              ...
              (mDark ? "92,100,110" : "240,247,248") +
              "); opacity:0.75; color:rgb(" +
              (mDark ? "40,44,48" : "137,137,137") +
              ")}" +
              ".lsbb >.siteList   {border:1px solid rgb(" +
              (mDark ? "87,97,108" : "183,219,205") +
              "); border-radius:10px; background-color:rgba(" +
              (mDark ? "86,89,92" : "243,243,243") +
              ",0.7); color:rgb(" +
              (mDark ? "140,154,173" : "75,143,231") +
              ")}" +
              ".lsbb >.siteList .lsb   {font-weight:normal; border:1px solid rgb(" +
              (mDark ? "98,98,90" : "210,210,190") +
              "); border-radius:10px; background-color:rgb(" +
              (mDark ? "77,84,89" : "225,239,239") +
              "); color:rgb(140, 140, 140)}" +
              ".lsbb .lsb:hover   {opacity:1; color:rgb(152, 123, 43); cursor:default}" +
              ".lsbb >.siteList:hover   {background-color:rgb(" +
              (mDark ? "87,97,108" : "183,219,205") +
              ")}" +
              ".lsbb .xButt:hover   {background-color:rgb(221, 230, 228)}" +
              ".lsbb >.siteList >div:not([class]):hover span   {color:rgb(" +
              (mDark ? "233,140,19" : "170,108,28") +
              ")}" +
          ...
          CS({
            ...
            c: function () {
              return (
                d &&
                d.getElementsByName("q") &&
                !/[?&]tbm=(shop|bks|fin)/.test(lh) &&
                d.getElementsByName("q")[0]
              );
            },
            o: function (dat) {
              var lang = S.lang != null ? S.lang : setts.lang,
                sites =
                  (S.sites &&
                    ((S.sites.length && S.sites[0]) || S.sites.length > 1) &&
                    S.sites) ||
              ...
              if (!lang || !$l[lang] || lang == "en")
                for (var l in $L) {
                  //replace 'en' lang for default or substitution
                  if ($L[l] instanceof Array)
                    for (var l2 in $L[l]) $L[l][l2] = l;
                  else $L[l] = l;
                }
              var srch = $q(".RNNXgb"),
                startPg =
                  (srch && !$q("button", srch)) || /\/(web|img)hp/.test(lh);
              if (startPg) {
                ...
                $e({
                  ...
                  at: {
                    ...
                    jsname: "Tg7LZd",
                    innerHTML:
                      '<div class="gBCQ5d"><span class="z1asCe MZy1Rb">sr</span></div>',
                  },
                  apT: srch,
                });
              }
              var $LSettings = $L["Settings"];
              if (sites && sites.length) sites.push($LSettings);
              var mainPg = /\/search\?|&q=|#q=/.test(lh),
                inputSearch = dat,
                layout1811 =
                  $q(".Tg7LZd") ||
                  $q('button[aria-label="Google Search"]') ||
                  $q('button[jsname="Tg7LZd"]'),
                ...
                isBWShown =
                  imgTools && /[&?]tbs[^&]*?(=|,|%2C)ic(:|%3A)gray/i.exec(lh), // Black-White Images search
                buttSearcStart =
                  (startPg &&
                    layout1811 &&
                    ($q('input[name="btnK"]') ||
                      $q('input[aria-label="Google Search"]'))) ||
                  $q('button[jsname="Tg7LZd"]'), //for the start page
                buttSearch =
                  (d.getElementsByName("btnG") &&
                    d.getElementsByName("btnG")[0]) ||
                  design1612 ||
                  layout1811,
                buttS = {
                  Srch: { url: "", txt: "search" },
                  PDF: {
                    ...
                    txt:
                      $L[
                      imSrch
                        ? (isBWShown ? "hide" : "show") + " sizes"
                        : "search in PDF files"
                      ],
                  },
                  site: {
                    url: "site:" + S.sites[0],
                    txt: $L["search in"] + " " + S.sites[0],
                    one: "day",
                ...
                iD = -1;
              if (
                (design1612 || layout1811) &&
                !d16 &&
                buttSearch &&
                buttSearch.parentNode
              )
                buttSearch.parentNode.className += " lsbb";
              !sites && delete buttS.site;
              if (!layout1811 && buttSearch && buttSearch.parentNode) {
                buttSearch.parentNode.style.position = "relative";
                buttSearch.parentNode.style.zIndex = "1003";
              }
              if (buttSearch && top == self)
                for (var i in buttS)
                  if ((i == "site" && !S.sites) || !imSrch || i != "1H") {
                    ++ii; //buttons under search input line
                    if (i.length == 2) iD++;
                    else iD = -1;
                    var bI = buttS[i],
                      Gesch = { m: "letzter", f: "letzte", n: "letztes" }[
                        "m,f,m,n,f".split(",")[iD]
                      ],
                      hint = function (j) {
                        return (
                      ...
                      butt2 = $e({
                        clone:
                          i == "site" || i.length == 2 || i == "PDF"
                            ? $e({
                              cl: "siteList",
                              cs: { cursor: "default" },
                              at: { site: S.sites[0], date: bI.url },
                            })
                            : i != ".. : .." || mainPg
                              ? $e({
                                el: "button",
                                cl: "xButt " + (d16 ? "xButt2" : "lsb"),
                              })
                              : $e({ cl: "siteList hiddn" }),
                        at: {
                          value:
                          ...
                          innerHTML:
                            ...
                            "><s" +
                            (isBWShown2 ? "" : "pan") +
                            ' class=txt onclick=this.parentNode.click();return!1 title="' +
                            (lang || i == "site" || i == ".. : .."
                              ? (
                                (iD == -1 || S.dwmyh[iD] == 1
                                  ? bI.txt
                                  ...
                                  " " +
                                  hint(S.dwmyh[iD] - 1)) || ""
                              ).replace(/letzte/, Gesch)
                              : "") +
                            '" itrvNum="' +
                            (i == "site"
                              ? ""
                              : bI.url + (imSrch ? "" : S.dwmyh[iD])) +
                            '">' +
                            (iD != -1 && S.dwmyh[iD] != 1
                              ? S.dwmyh[iD] + bI.lett
                              : imSrch && i == "PDF"
                        ...
                        cs: $x(
                          {
                            position: "absolute",
                            top: startPg ? "40px" : "33px",
                            wordSpacing: "-1px",
                            visibility:
                              ii <= S.hiddenEdgeLeft ? "hidden" : "visible",
                          },
                          $x(
                            csLeft(ii),
                            ii === 2
                          ...
                          click: (function (bI, i, iD) {
                            ...
                            return /Srch|PDF|DOC|site/.test(i)
                              ? function (ev) {
                                ...
                                if (
                                  t &&
                                  (t.getAttribute("site") == $LSettings ||
                                    (t.parentNode &&
                                      t.parentNode.getAttribute("site") ==
                                      $LSettings)) &&
                                  !/Srch|PDF|DOC/.test(i)
                                )
                                  return;
                                if (
                                  t.classList.contains("settIn") ||
                                  t.parentNode.classList.contains("settIn")
                                ) {
                                  ev.stopPropagation();
                                  return;
                                }
                                ...
                                if (
                                  t &&
                                  t.getAttribute("site") == null &&
                                  t.parentNode &&
                                  t.parentNode.getAttribute("site") == null &&
                                  !/Srch|PDF|DOC/.test(i)
                                )
                                  return;
                                if (imSrch && i == "PDF") {
                                  ev.stopPropagation();
                                  saveLocStor();
                                  location.href = isBWShown
                                    ? lh.replace(
                                      new RegExp(imgTools[0]),
                                      imgTools &&
                                      imgTools[0]
                                        .replace(
                                          /(,|%2C)?ic(:|%3A)gray/gi,
                                    ...
                                    : imgTools
                                      ...
                                      : lh +
                                      (/\?/.test(lh) ? "&" : "?") +
                                      "tbs=ic:gray"; //new Tools-More_tools_Show_sizes
                                } else if (
                                  (t &&
                                    /xButt|txt/.test(t.className) &&
                                    !(
                                      i == "site" &&
                                      !(
                                        /list/.test(t.parentNode.className) ||
                                        /list/.test(
                                          t.parentNode.parentNode.className
                                        )
                                      )
                                    )) ||
                                  (t && /Srch|PDF|DOC/.test(t.value))
                                )
                                    /*console.log('==startSrch'),*/ (
                                    buttSearcStart || buttSearch
                                  ).click();
                              }
                              : !bI.url
                                ? function (ev) {
                                  //from-to date (! not used now)
                                  var el = $q("#cdrlnk"),
                                    o;
                                  el &&
                                    el.dispatchEvent(
                                      ((o = d.createEvent("Events")).initEvent(
                                        "click",
                                        !0,
                                        !1
                                      ),
                                ...
                                : function (ev) {
                                  ...
                                  if (
                                    tOvr &&
                                    tOvr.classList.contains("siteList") &&
                                    !ta.classList.contains("list")
                                  ) {
                                    //clicked by top button
                                    var elTop =
                                      $q("div:not(.list) >.txt", tOvr) || "",
                                      itrvNum =
                                        (elTop &&
                                          elTop.getAttribute("itrvNum")) ||
                                        "",
                                      newSrch = /[?&]q=/.test(l2)
                                        ? l2.replace(
                                          /(&|\?)q=([^&]*)(&|$)/g,
                                        ...
                                        (/\?/.test(l2) ? "&" : "?") +
                                        "q=" +
                                        encodeURIComponent(inputSearch.value); //set new value as &q=.+
                                    if (layout1811 && itrvNum !== "" && date2)
                                      location.href = /qdr(:|%3A)([dwmyh])\d*/.test(
                                        l2
                                      )
                                        ? newSrch.replace(
                                          /([?&]tbs=)?qdr(:|%3A)[dwmyh]\d*/,
                                          function (x) {
                                            return itrvNum.replace(
                                              /&/,
                                              /\?/.test(x) ? "?" : "&"
                                            );
                                          }
                                        ...
                                        : newSrch +
                                        (/\?/.test(newSrch) ? "&" : "?") +
                                        itrvNum; //add date in URL
                                    if (itrvNum != null)
                                      S.dwmyh[iD] = +(itrvNum || "").replace(
                                        /\D/g,
                                        ""
                                      );
                                  } else if (
                    ...
                    bI.el = butt2;
                    if (i == "site" || i.length == 2 || i == "PDF") {
                      //dropdown lists under some buttons
                      ...
                      for (
                        ...
                        (imSrch
                          ? 1
                          : bI.up -
                          (i == "1W" && S.lastHoursLess ? 4 : 0) -
                          (i == "1M" && S.lastHoursLess ? 9 : 0) -
                          (i == "1Y" && S.lastHoursLess ? 5 : 0));
                        j++
                      )
                              ...
                              at: {
                                ...
                                innerHTML:
                                  (/site|PDF/.test(i)
                                    ...
                                    '" data-val="' +
                                    sI.replace(/&nbsp; ?/g, "") +
                                    '" title="' + //multiselect mechanics
                                    (1
                                      ? (i == "PDF" ? "" : "sites ") +
                                      "multiselect" +
                                      (i == "PDF" ? " of types" : "")
                                      : "click to disable select") +
                                    '">' +
                                    (1 ? "OR" : "V") +
                                    "</span>"
                                    : "") +
                                  ...
                                  sI +
                                  "</span>" +
                                  (sI != $LSettings && !(!S.sites && i == "1H")
                                    ? ""
                                    : '<div class="settIn">' +
                                    $L.Settings +
                                    " " +
                                    $L["of userscript"] +
                                    '<br/>"Google Search Extra Buttons"<hr/>' +
                                    $L["Interface language"] +
                                    ': <select class="lang" style="width:70px">' +
                                    (function () {
                                      var s =
                                        "<option" +
                                        (lang == "en" ? " selected" : "") +
                                    ...
                                    "</select><br/>" +
                                    '<input type="checkbox" class="less" id="hoursLess"' +
                                    (S.lastHoursLess ? " checked" : "") +
                                    "/>" +
                                    '<label for="hoursLess" id="hoursLessL">' +
                                    $L[
                                    "Less positions at the end of selects"
                                    ] +
                                    "</label><br/>" +
                                    '<input type="checkbox" class="des16" id="design1612"' +
                                      /*!layout1811 &&*/ (S.design1612
                                      ? " checked"
                                      : "") +
                                    (1 ? " disabled" : "") +
                                    "/>" +
                                    '<input type="checkbox" class="des18" id="whiteMintOval"' +
                                    (S.whiteMintOval ||
                                      S.whiteMintOval === undefined
                                      ? " checked"
                                      : "") +
                                    "/>" +
                                    '<label for="whiteMintOval" id="whiteMintOvalL">' +
                                    $L["Gray design of buttons"] +
                                    "</label><br/>" +
                                    '<input type="checkbox" class="butTyp" id="butTypes"' +
                                    (S.hiddenEdgeLeft ? "" : " checked") +
                                    "/>" +
                                    '<label for="butTypes" id="butTypesL">' +
                                    $L["Show Filetype Button"] +
                                    "</label><br/>" +
                                    '<i><a href="#" class="defa" style="float: right">Default settings</a></i>' +
                                    $L.Sites +
                                    ': <br/><textarea class="sites" style="width:97%" rows=8>' +
                                    strSites +
                                    "</textarea><br/>" +
                                    '<a class="reload" href=# onclick="location.reload();return!1">' +
                                    $L["reload page for effect"] +
                                    "</a>" +
                                    "</div>"),
                              },
                              cs: Object.assign(
                                {
                                  ...
                                  width:
                                    sI != $LSettings
                                      ? "auto"
                                      : /en|es/.test(lang) || !lang
                                        ? "3em"
                                        : "4em",
                                  height: sI != $LSettings ? "18px" : "16px",
                                ...
                                fTMore ? { top: "18px", left: "47px" } : {}
                              ),
                              on: {
                                click: (function (fTyp, pdf, fTMore) {
                                  return function (ev) {
                                    var t = ev.target;
                                    var less = $q("#hoursLess"),
                                      ...
                                      itrv =
                                        t.getAttribute("date") ||
                                        t.parentNode.getAttribute("date") ||
                                        "",
                                      num = (
                                        t.getAttribute("site") ||
                                        t.parentNode.getAttribute("site") ||
                                    ...
                                    if (
                                      t.classList.contains("sett") ||
                                      t.parentNode.classList.contains("sett")
                                    ) {
                                      ev.stopPropagation();
                                      return;
                                    }
                                    if (less && /hoursLess/.test(t.id)) {
                                      less.outerHTML =
                                        '<input type="checkbox" class="less" id="hoursLess"' +
                                        (less.getAttribute("checked") != null
                                          ? ""
                                          : ' checked="checked"') +
                                        "/>";
                                      saveLocStor();
                                    ...
                                    if (des16 && /design1612/.test(t.id)) {
                                      des16.outerHTML =
                                        '<input type="checkbox" class="des16" id="design1612"' +
                                        (des16.getAttribute("checked") != null
                                          ? ""
                                          : ' checked="checked"') +
                                        "/>";
                                      saveLocStor();
                                    ...
                                    if (des18 && /whiteMintOval/.test(t.id)) {
                                      des18.outerHTML =
                                        '<input type="checkbox" class="des18" id="design1612"' +
                                        (des18.getAttribute("checked") != null
                                          ? ""
                                          : ' checked="checked"') +
                                        "/>";
                                      saveLocStor();
                                    ...
                                    if (butTyp && /butTypes/.test(t.id)) {
                                      butTyp.outerHTML =
                                        '<input type="checkbox" class="butTyp" id="butTypes"' +
                                        (butTyp.getAttribute("checked") != null
                                          ? ""
                                          : ' checked="checked"') +
                                        "/>";
                                      saveLocStor();
                                    ...
                                    if (
                                      ...
                                      buttSearcStart.click();
                                      return;
                                    }
                                    if ((layout1811 && num !== "") || pdf) {
                                      pdf && ev.stopPropagation();
                                      if (!fTMore)
                                        location.href = /qdr(:|%3A)([dwmyh])\d*/.test(
                                          l2
                                        )
                                      ...
                                      //add date in URL
                                        ...
                                        if (t.classList.contains("txt")) {
                                          if (
                                            ...
                                            t.parentNode.className =
                                              "xButt lsb more";
                                        } //'className' because bug of Chrome in upper line with .toggle
                                        else {
                                          if (t.classList.contains("more"))
                                            t.className = "xButt lsb moreShow";
                                          else t.className = "xButt lsb more";
                                        }
                                      }
                                    } //bug of Fx with .toggle,.toggle - list is changed slow (0.4 s)
                                    $pd(ev);
                                  };
                                })(fTyp, i == "PDF", fTMore),
                              }, //, mouseover: function(ev){ev.stopPropagation()}, mouseout: function(ev){ev.stopPropagation()}
                              apT: siteList,
                            });
                        }
                      siteList.style.height = "auto";
                      siteList.style.textAlign = "center";
                      siteList.style.marginRight = "-40px";
                    }
                  }
            },
          });
    ...
    var saveLocStor = function (ev, val, do2) {
      var aaa,
        aab,
        aac,
        aad,
        aae,
        aaf,
        t = (ev && ev.target.form) || document.documentElement || document.body;
      xLocStor({
        do: do2 || "set",
        key: "sett",
        val: {
          lang: (aaa = d.querySelectorAll(".lang", t))[aaa.length - 1].value,
          sites: (
            (aab = d.querySelectorAll(".sites", t))[aab.length - 1].value || ""
          )
            .replace(/^[ \t]*|[ \n\t]*$/g, "")
            .split("\n"),
          lastHoursLess: (aac = d.querySelectorAll(".less", t))[aac.length - 1]
            .checked,
          design1612: (aad = d.querySelectorAll(".des16", t))[aad.length - 1]
            .checked,
          whiteMintOval: (aae = d.querySelectorAll(".des18", t))[aae.length - 1]
            .checked,
          hiddenEdgeLeft: !(aaf = d.querySelectorAll(".butTyp", t))[
            aaf.length - 1
          ].checked,
    ...
    lang:
      ...
      navigator.language.substr(0, 2), //='' if hide hints, or 2 letters from $l{}
    sites: [
      //=array or one site in string
      "",
      "slashdot.org",
      "reddit.com",
      "techcrunch.com",
