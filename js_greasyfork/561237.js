...
// @version      1.2
// @description  域名列表管理/拖拽记忆/白瓷UI/自动回车
// @author       Gemini
// @match        *://*/*
// @grant        GM_download
...
// @downloadURL https://update.greasyfork.org/scripts/561237/%E5%9B%BE%E7%89%87%E4%B8%8B%E8%BD%BD%E5%99%A8.user.js
// @updateURL https://update.greasyfork.org/scripts/561237/%E5%9B%BE%E7%89%87%E4%B8%8B%E8%BD%BD%E5%99%A8.meta.js
// ==/UserScript==
...
(function() {
    ...
    function init() {
        ...
        const btnSelect = createBtn('wb-sel', '<svg viewBox="0 0 24 24"><path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"/></svg>', '选图');
        const btnDate = createBtn('wb-date', '<svg viewBox="0 0 24 24"><path d="M19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11zM7 10h5v5H7z"/></svg>', '按日期');
        const btnLast = createBtn('wb-last', '<svg viewBox="0 0 24 24"><path d="M16 13h-3V3h-2v10H8l4 4 4-4zM4 19v2h16v-2H4z"/></svg>', '最新一张');
        const btnAll = createBtn('wb-all', '<svg viewBox="0 0 24 24"><path d="M20 2H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-8.5 7.5c0 .83-.67 1.5-1.5 1.5H9v2H7.5V7H10c.83 0 1.5.67 1.5 1.5v1zm5 2c0 .83-.67 1.5-1.5 1.5h-2.5V7H15c.83 0 1.5.67 1.5 1.5v3zm4-3H19v1h1.5V11H19v2h-1.5V7h3v1.5zM9 9.5h1v-1H9v1zM4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm10 5.5h1v-3h-1v3z"/></svg>', '下载全部');
        const btnReply = createBtn('wb-reply', '<svg viewBox="0 0 24 24"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg>', `回复: ${state.replyText}`);
    ...
    function toggleSelectMode() {
        ...
        if (state.modeOn) {
            ...
            updateSelectBtn(); showToast('✨ 进入选图模式');
    ...
    function createSettings() {
        ...
        modal.innerHTML = `
            <div class="wb-box">
                ...
                <div class="wb-scroll-area">
                    ...
                    <div class="wb-field">
                        <label class="wb-label">⌨️ 快捷键设置</label>
                        <div id="wb-k-toggle" class="wb-key">选图模式: ${fmtKey(state.hkToggle)}</div>
                        <div id="wb-k-date" class="wb-key">一键下载: ${fmtKey(state.hkDate)}</div>
                    </div>
                </div>
        ...
        const toggle = document.getElementById('wb-toggle-confirm');
        toggle.checked = state.showConfirm;
        toggle.onchange = (e) => { state.showConfirm = e.target.checked; GM_setValue('date_show_confirm', e.target.checked); };

        document.getElementById('wb-save').onclick = () => {
            state.replyText = replyIn.value;
            GM_setValue('custom_reply', state.replyText);
            document.getElementById('wb-reply').dataset.tip = `回复: ${state.replyText}`;
        ...
        const bindRec = (id, type, storeKey) => {
            ...
            el.onclick = (e) => {
                e.stopPropagation(); el.innerText = '按键...'; el.classList.add('rec');
                state.recKey = { type, el, storeKey };
            };
        };
        bindRec('wb-k-toggle', 'toggle', 'hk_toggle');
        bindRec('wb-k-date', 'date', 'hk_date');
    }
    ...
    function bindKeys() {
        document.addEventListener('keydown', (e) => {
            if (state.recKey) {
                ...
                if(state.recKey.type==='toggle') state.hkToggle=nk; else state.hkDate=nk;
                GM_setValue(state.recKey.storeKey, nk);
                state.recKey.el.innerText=fmtKey(nk); state.recKey.el.classList.remove('rec');
                state.recKey=null; return;
            }
            if (matchKey(e, state.hkToggle)) toggleSelectMode();
            if (matchKey(e, state.hkDate)) downloadAll();
    ...
    document.addEventListener('mouseup', (e) => {
        if(state.modeOn || state.isDragging || (modal && modal.style.display === 'flex')) return;
        ...
        const txt = div.textContent; if(txt) { GM_setClipboard(txt); showToast(`✨ 已复制: ${txt.substring(0,8)}...`); }
    });
