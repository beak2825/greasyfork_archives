    ...
    function readDicePool() {
        ...
        dice.forEach((die) => {
            ...
            const amount = parseInt(
                die.querySelector(".amount")?.textContent.trim() || "0",
                10
            );
...
`;
    ...
    function combineTwoPMFs(pmfA, pmfB) {
        const result = [];
    ...
    function buildPoolPMF(tally) {
        let pmf = [{ s: 0, a: 0, d: 0, t: 0, tri: 0, des: 0 }]; // Start with no dice
    ...
    // Compute marginal probabilities for each symbol type
    // Note: These were used for the old system and are now kept for reference
    // The main display uses computeGameOutcomes() instead
    function computeProbabilities(tally) {
        ...
        for (const o of pmf) {
            ...
            if (c.netSuccess > 0) successCount++;
            if (c.netAdvantage > 0) advantageCount++;
            if (c.triumph > 0) triumphCount++;
            if (c.despair > 0) despairCount++;
        }

        return {
            success: successCount / N,
            advantage: advantageCount / N,
            triumph: triumphCount / N,
            despair: despairCount / N,
        };
    }
    ...
    function computeGameOutcomes(tally) {
        ...
        for (const o of pmf) {
            ...
            // Track specials
            if (tri > 0) outcomes.triumph++;
            if (des > 0) outcomes.despair++;

            // Classify primary outcome
            if (s > 0) {
                // Success
                if (a > 0) outcomes.success_advantage++;
                else if (a < 0) outcomes.success_threat++;
                else outcomes.success_clean++;
            } else {
                // Failure (net success <= 0)
                if (a > 0) outcomes.failure_advantage++;
                else if (a < 0) outcomes.failure_threat++;
                else outcomes.failure_clean++;
            }
        }
    ...
    function updateProbabilityPanel(tally) {
        ...
        const pSuccess =
              outcomes.success_advantage +
              outcomes.success_threat +
              outcomes.success_clean;
        const pFailure =
              outcomes.failure_advantage +
              outcomes.failure_threat +
              outcomes.failure_clean;
  ...
  <div class="prob-row"><span>Success:</span><span style="color:#0f0;">${pct(
        pSuccess
    )}</span></div>
  <div class="prob-row"><span>Failure:</span><span style="color:#f00;">${pct(
        pFailure
    )}</span></div>

  <div class="prob-section">Success Modifiers</div>
  <div class="prob-row"><span>+ Advantage:</span><span>${pct(
        outcomes.success_advantage
    )}</span></div>
  <div class="prob-row"><span>+ Threat:</span><span>${pct(
        outcomes.success_threat
    )}</span></div>
  <div class="prob-row"><span>Clean:</span><span>${pct(
        outcomes.success_clean
    )}</span></div>
  ...
  <div class="prob-row"><span>Triumph:</span><span style="color:#ff0;">${pct(
        outcomes.triumph
    )}</span></div>
  <div class="prob-row"><span>Despair:</span><span style="color:#f0f;">${pct(
        outcomes.despair
    )}</span></div>
`;
  }
