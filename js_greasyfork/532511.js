...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/532511/%E8%9D%89%E4%B9%8B%E5%8A%A9.user.js
// @updateURL https://update.greasyfork.org/scripts/532511/%E8%9D%89%E4%B9%8B%E5%8A%A9.meta.js
// ==/UserScript==
...
1. **语法与权重控制**
   - **权重调节**: 使用(关键词:权重值)精确控制权重，如(chan:1.2)将权重设为1.2倍，建议范围0.3-1.5
   - **【重要】权重使用应当克制**: 仅对核心元素适度使用权重，chan建议使用(chan:1.2)，不要过度使用高权重
   - **多层叠加**: 多层括号可累加权重，如((chan))提升至1.21倍（1.1×1.1）
...
3. **蝉蝉IP特定要求**
   - 保持提示词中"chan"相关元素不变，必须保持在提示词最前面，使用(chan:1.2)权重
   - 分析提示词的主题和意图，保持其核心内容
   - 识别用户的核心需求关键词，将其放在"chan"元素之后、提示词最前面
   - 仅在用户特别强调的核心需求上，适度增加权重(1.1)到(1.2)
...
返回的JSON格式示例：
{
  "positive": "(chan:1.2), (astronaut chan:1.1), space chan, cute, adorable, chibi, cartoon, anime style, [保持原有提示词，维持关键词优先级和权重]",
...
1. **语法与权重控制**
   - **权重调节**: 使用(关键词:权重值)精确控制权重，如(mail:1.2)将权重设为1.2倍，建议范围0.3-1.5
   - **【重要】权重使用应当克制**: 仅对用户特别强调或明确关注的核心图标主题使用权重，不要过度使用，权重值建议控制在1.1-1.2范围内
   - **多层叠加**: 多层括号可累加权重，如((icon))提升至1.21倍
...
2. **图标提示词要求**
   ...
   - 仅在用户特别强调的关键词上，适度增加权重(1.1)到(1.2)
...
返回的JSON格式示例：
{
  "positive": "(email icon:1.2), mail icon, icon, business icon, simple, clean background, studio lighting, 3D rendering, vector graphics, professional UI",
...
1. **语法与权重控制**
   - **权重调节**: 使用(关键词:权重值)精确控制权重，如(dog:1.2)将权重设为1.2倍，建议范围0.3-1.5
   - **【重要】权重使用应当克制**: 仅对用户特别强调或明确关注的核心元素使用权重，不要过度使用，权重值建议控制在1.1-1.2范围内
   - **多层叠加**: 多层括号可累加权重，如((dog))提升至1.21倍（1.1×1.1）
...
返回的JSON格式示例：
{
  "positive": "(rocket:1.2), space rocket, detailed character, full body, full shot, masterpiece, best quality, [保持原有提示词，维持关键词优先级和权重]",
  "negative": "cropped, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer digits, bad feet, fewer toes, extra toes, [其他负向提示词]"
}`;
            }
...
1. **语法与权重控制**
   - **权重调节**: 使用(关键词:权重值)精确控制权重，如(dog:1.2)，建议范围0.3-1.5
   - **【重要】权重使用应当克制**: 仅对用户特别强调或明确关注的核心元素使用权重，不要过度使用，权重值建议控制在1.1-1.2范围内
   - **多层叠加**: 多层括号可累加权重，如((dog))提升至1.21倍
...
3. **语义理解与转化策略**
   ...
   - 识别用户特别强调的元素，仅对这些元素适度增加权重(1.1)到(1.2)
...
4. **提示词结构与优先级**
   ...
   - 前75个词影响最大，关键词应放在前面
   - 特别处理以下常见语义场景：
     - 风格转换：识别目标风格，使用(style:1.1)等适度加权
...
**【重要提醒】请务必确保所有带权重的关键词都使用正确的 (keyword:weight) 格式，例如 (cat:1.2)。检查你的输出！**`;
...
4. **提示词生成要求**
   ...
   - 将用户提及的关键词放在提示词最前面，并仅对用户明确强调的元素适度增加权重(1.1)到(1.2)
   - 提示词顺序: 核心需求 > 重要特征 > 场景描述 > 风格 > 辅助描述

返回的JSON格式示例：
{
  "positive": "(rocket:1.2), (space rocket:1.1), detailed character, full body, full shot, [other positive prompts]",
...
3. **图标提示词要求**
   ...
   - 仅为用户特别强调的核心图标主题提供适度权重(1.1-1.2)
   - 负向提示词中加入"text, words, letters, ugly, grainy, noisy, blurry"等防止出现文字和低质量元素

4. **提示词结构**
   - 按"图标主题→图标类型→风格→渲染细节→效果"顺序排列
   - 例如: (email:1.2), (mail icon:1.1), business icon, minimalistic, clean background, vector
   - 前50个词影响最大，关键词应放在前面
...
返回的JSON格式示例：
{
  "positive": "(email:1.2), (mail icon:1.1), business icon, clean minimalistic design, clean background, studio lighting, 3D rendering, soft shadows, vector graphics, professional UI",
  "negative": "text, words, letters, ugly, grainy, noisy, blurry, low resolution, oversaturated, cartoon, anime, sketchy, hand drawn"
}`
        },
...
4. **提示词结构**
   ...
   - 例如: (email:1.2), (mail icon:1.1), flat design, minimal, simple line art
   - 前50个词影响最大，关键词应放在前面
...
返回的JSON格式示例：
{
  "positive": "(email:1.2), (mail icon:1.1), flat design, minimal, simple line art, clean geometric shapes, solid colors, white background, professional UI, vector graphics",
...
1. **语法与权重控制**
   - **权重调节**: 使用(关键词:权重值)精确控制权重，如(chan:1.2)将权重设为1.2倍，建议范围0.3-1.5
   - **【重要】权重使用应当克制**: 仅对核心元素适度使用权重，chan建议使用(chan:1.2)，不要过度使用高权重，权重值建议控制在1.1-1.2范围内
   - **多层叠加**: 多层括号可累加权重，如((chan))提升至1.21倍（1.1×1.1）
   - **下划线连接**: 使用happy_chan连接词组避免歧义，生成"快乐的蝉蝉"
   - **混合符号**: 使用[happy|sad]实现情绪变化效果
...
3. **蝉蝉角色特定要求**
   - 必须将"chan"作为第一关键词，使用适度权重(chan:1.2)
   - 必须生成完整的蝉蝉角色，正向提示词中加入"full body, full shot"
   - 必须根据用户描述添加适当的动作、场景、表情等
   - 如有用户指定的手持物品，适度使用(holding xxx:1.1)格式
   - 情绪表达适度使用(happy:1.1)等格式
   - 风格描述适度使用(cute:1.1), (cartoon:1.1)等格式

4. **提示词结构**
   - 按"chan→用户核心需求→角色描述→动作/表情→场景→风格→效果"顺序排列
   - 例如: (chan:1.2), (explorer chan:1.1), astronaut, full body, jumping, happy face, space station, cute, cartoon style
...
返回的JSON格式示例：
{
  "positive": "(chan:1.2), [(normal chan):(stone chan):0.5], full body, full shot, standing still, neutral expression, simple background, cute, cartoon style",
