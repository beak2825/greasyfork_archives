...
// @version      0.1
// @description  bifaucet!
// @author       LTW
// @license      none
// @match        https://bifaucet.com/*
// @icon         https://www.google.com/s2/favicons?sz=64&domain=bifaucet.com
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/502867/bifaucetcom.user.js
// @updateURL https://update.greasyfork.org/scripts/502867/bifaucetcom.meta.js
// ==/UserScript==
...
if (window.location.href === 'https://bifaucet.com/') {
window.location.href = 'https://bifaucet.com/login';}
if (window.location.href === 'https://bifaucet.com/dashboard') {
window.location.href = 'https://bifaucet.com/faucet';}
    ...
    if(window.location.href.includes('/login')) {
        ...
        setInterval(function() {
            ...
            if ((reCaptchaResponse && reCaptchaResponse.length !== 0) || (hCaptchaResponse && hCaptchaResponse.length !== 0)) {
                if (!buttonClicked) {
                    const submitButton = document.querySelector('.btn.btn.btn-primary.d-grid.w-100.waves-effect.waves-light');
     ...
     const checkAndClaim = async () => {
       let reCaptchaResponse = "";
        ...
        if (window.location.href.includes("/faucet") && mbsolver() && !wasButtonClicked()) {
            await sleep(3000);
            select('.btn.btn.btn-primary.waves-effect.claim-button.waves-light')?.click();
            setButtonClicked();
        }
         }
    };if (window.location.href.includes("/faucet")){setInterval(async () => {await checkAndClaim();}, 3000);}
