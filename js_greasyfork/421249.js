...
// @match        https://skins-table.xyz/table_test_*
// @grant        unsafeWindow
...
// @downloadURL https://update.greasyfork.org/scripts/421249/%5Btradeitgg%5D%20TradeBot%20v2.user.js
// @updateURL https://update.greasyfork.org/scripts/421249/%5Btradeitgg%5D%20TradeBot%20v2.meta.js
// ==/UserScript==
...
(function() {
    ...
    if (window.location.href.includes("://tradeit.gg")) {
        // настройки бота
        const botSettings =  {
            debug: true,
            // сюда вписываем предметы которые нужно игнорировать
            ignoredListItems: {
                // PUBG
                "578080": [],
                // RUST
                ...
                // CS:GO
                "730": [
                    "Предмет который не нужно покупать"
                ]
            },
            shoppingList: {}, // тут будут хранится предметы полученные из таблицы skins-table
        ...
        GM_registerMenuCommand("Очистить список предметов", () => {
            for(let key in botSettings.shoppingList) {
                botSettings.shoppingList[key] = [];
            }
        });

        // преобразовать название игры в id игры
        function gameNameToId(name) {
            name = name.toUpperCase();
            if (name == "CS:GO") return 730;
            else if (name == "DOTA2") return 570;
            else if (name == "H1Z1") return 433850;
            else if (name == "RUST") return 252490;
            else if (name == "PUBG") return 578080;
            else return null;
        }

        // создать команды запуска бота
        function makeStartCommands() {
            startCmdIds = [
                GM_registerMenuCommand("Запустить в режиме замены", () => botStart(1)),
                GM_registerMenuCommand("Запустить в режиме дополнения", () => botStart(2)),
            ];
        }

        // удалить команды запуска
        function removeStartCommands() {
            startCmdIds.map(GM_unregisterMenuCommand);
        }
        ...
        function reloadPage(timeout = 0, message = "") {
            setTimeout(() => {
                ...
                botStop(message);
                unsafeWindow.location.hash = `#autostart-${botMode}-${Date.now()}` // благодоря хэшу "#autostart" в url, скрипт поймер что нужно автоматически запустится
        ...
        async function botTick() {
            if (botIsStarted) {
                // список предметов которые нужно покупать
                let itemsName = botSettings.shoppingList[unsafeWindow.sgame] || [];
                // список игнорируемых предметов
                let ignoredNames = botSettings.ignoredListItems[unsafeWindow.sgame] || [];
                ...
                try {
                    if (itemsName.length > 0) {
                        ...
                        let foundItems = items.filter(item => !ignoredNames.includes(item.n) && itemsName.includes(item.n));
        ...
        // запуск бота
        function botStart(mode, autostart = false) {
            if (!botIsStarted) {
                botMode = mode;
        ...
        GM_onMessage("skins-table-items", (gameName, itemsNames) => {
            if (botSettings.debug) console.log("skins-table-items", gameName, itemsNames);
            let gameId = gameNameToId(gameName);
            if (gameId !== null) {
                if (botMode == 1) {
                    botSettings.shoppingList[gameId] = Array.from(new Set(itemsNames));
                } else if (botMode == 2) {
                    botSettings.shoppingList[gameId] = Array.from(new Set(itemsNames.concat(botSettings.shoppingList[gameId] || [])));
                }
            } else {
                notify(`Неизвестная игра "${gameName}"`);
            }
        });

        // ожидаем загрузки сайта, после чего инициализируем бота
        waitSiteInit().then((r) => {
            initFilters();
            makeStartCommands();
            // если в url присутсвует хэш #autostart то скрипт запустится автоматически
            if (unsafeWindow.location.hash.includes("#autostart")) {
                let mode = +location.hash.split("-")[1];
                if (!mode) return notify("Не удалось установить режим работы бота. Бот остановлен");
                botStart(+location.hash.split("-")[1], true);
            }
        });
    }
    else if (window.location.href.includes("://skins-table.xyz")) {
        let timer = setInterval(() => {
            let gameId = unsafeWindow.$(unsafeWindow.game_id_select.selOpts).filter(`[data-value='${unsafeWindow.game_id}']`).eq(0).text();

            if (!gameId) {
                alert("Не удалось получить идентификатор игры. Парсинг остановлен");
                return clearInterval(timer);
            }

            GM_sendMessage("skins-table-items", gameId, unsafeWindow.item_list.map(e => e.i), Date.now());
        }, 1000);
    }
})();
