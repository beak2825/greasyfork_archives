...
// @downloadURL https://update.greasyfork.org/scripts/449008/uzis.user.js
// @updateURL https://update.greasyfork.org/scripts/449008/uzis.meta.js
// ==/UserScript==
...
let zhpressCount = 0
    ...
    hacks.ares = function(){
            for (let i = 0; i < game.getTank().components_0.array.length; i++)
        try{game.getTank().components_0.array[i].speedCharacteristics_obegdm$_0.acceleration=1900
            game.getTank().components_0.array[i].speedCharacteristics_obegdm$_0.reverseAcceleration = 1900
            game.getTank().components_0.array[i].speedCharacteristics_obegdm$_0.turnMaxSpeed = 3
            game.getTank().components_0.array[i].speedCharacteristics_obegdm$_0.turnAcceleration = 2.5
            game.getTank().components_0.array[i].speedCharacteristics_obegdm$_0.deceleration = 180
            game.getTank().components_0.array[i].params_m76zm3$_0.tiltStabilityMaxAngle = 1
            game.getTank().components_0.array[i].tiltStabilityScale = 1
           } catch (error) {
           }}
           document.addEventListener('keydown', function (event) { if (event.key === '7'){
            zpressCount ++
            if(zpressCount%2==1){
            window.d = setInterval(hacks.ares,1)
...
hacks.shaft = function(){
                for (let i = 0; i < game.getTank().components_0.array.length; i++)
            try{game.getTank().components_0.array.at(89).shaftCC_0.chargeRate = 300
               } catch (error) {
               }}

let pressCount = 0
document.addEventListener('keydown', function (event) { if (event.key === 'o'){
pressCount ++
if(pressCount%2==1){
window.rz = setInterval(hacks.shaft,1)
...
let yhpressCount = 0
  ...
  document.addEventListener('keydown', function (event) { if (event.key === '9'){
            yhpressCount ++
            if(yhpressCount%2==1){
            window.bh = setInterval(hacks.rico,1)
...
document.addEventListener('keydown', (e) => { if (e.keyCode === 79 && game.isNotOpenChat()){
s7p+=1
if(s7p%2==1){
State7.innerText = "ON"
State7.style.color = "#00D000"
window.re = setInterval(hacks.shaft,1)
...
document.addEventListener('keydown', (e) => { if (e.keyCode === 55 && game.isNotOpenChat()){
s10p+=1
if(s10p%2==1){
State10.innerText = "ON"
State10.style.color = "#00D000"
window.hp = setInterval(hacks.ares,1)
...
clearInterval(window.hp)
State10.innerText = "OFF"
State10.style.color = "#FF0000"
}
