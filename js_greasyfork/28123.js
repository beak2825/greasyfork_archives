...
// @downloadURL https://update.greasyfork.org/scripts/28123/emjack.user.js
// @updateURL https://update.greasyfork.org/scripts/28123/emjack.meta.js
// ==/UserScript==
			...
			},
		{
			...
			callback: function(data) {
				if(!master) {
					...
					sock.chat("Back off! My master is "+master, data.user);
					}
				}
			},
		{
			...
			},
        {
			...
			callback: function(data) {
				if(data.user===master) {
					...
					sock.chat("Get away creep", data.user);
					}
				},
			responses: [
				"I feel so safe in master senpai's arms"
				]
			},
		{
			name: "Bow",
			...
			regex: /^splash/i,
			responses: [
				"I only love EliteMagikarps, hes one sexy fish!" ]
			},
		{
			name: "Vote",
			regex: /^vote (\w+)/i,
