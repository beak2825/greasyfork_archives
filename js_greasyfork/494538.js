// ==UserScript==
// @name         Test Script Whatsapp Tools
// @namespace    http://tampermonkey.net/
// @version      0.2
// @description  Test Script Whatsapp Tools Este script e um teste de publição
// @author       SleX - https://slex.com.br
// @match        https://web.whatsapp.com/
// @icon         https://www.google.com/s2/favicons?sz=64&domain=whatsapp.com
// @grant        none
// @license MIT 
// @downloadURL https://update.greasyfork.org/scripts/494538/Test%20Script%20Whatsapp%20Tools.user.js
// @updateURL https://update.greasyfork.org/scripts/494538/Test%20Script%20Whatsapp%20Tools.meta.js
// ==/UserScript==

(function() {
    'use strict';

var divTools, btnGetMembers, onClickGetMembers, audBeebSuccess, onInjectBtn, onCopyTextToClipboard;


onInjectBtn = function() {
	divTools = document.querySelector('#app > div > div > div > header > div > div > span');

	if(!divTools){
		window.setTimeout(onInjectBtn, 1000);
        console.log('🤔');
        return;
	}
	btnGetMembers = document.createElement('div');
	btnGetMembers.setAttribute('id', 'btn-get-members');
	btnGetMembers.innerHTML = '<div role="button" tabindex="0" title="Pegar Membros" ><span><img src="https://www.svgrepo.com/show/252715/folder-group.svg" width="24" height="24"/></span></div>';
	audBeebSuccess = new Audio('data:audio/ogg;base64,');

    onCopyTextToClipboard = function(text) {
        var textArea = document.createElement("textarea");
        textArea.value = text;

        textArea.style.top = "0";
        textArea.style.left = "0";
        textArea.style.position = "fixed";

        document.body.appendChild(textArea);
        textArea.focus();
        textArea.select();

        try {
            var successful = document.execCommand('copy');
            var msg = successful ? 'successful' : 'unsuccessful';
            console.log('Fallback: Copying text command was ' + msg);
        } catch (err) {
            console.error('Fallback: Oops, unable to copy', err);
        }

        document.body.removeChild(textArea);
    }


	onClickGetMembers = function(){
		try {
            var el, listM, textM, groupName;
            groupName = document.querySelector('#main > header > div > div > div > span').textContent.trim();
            el = document.querySelector('#main > header > div > div > span');
            listM = el.textContent.split(', ');
            listM = listM.filter(function(e) { return e.startsWith('+')});
            listM = listM.map(function(e){ return `${groupName}\t${e.replace('+', '')}`});
            if(listM.length === 0 ){
                console.log('😞');
                return;
            }
            textM = listM.join('\n');
            console.clear();
            console.log(textM);
            onCopyTextToClipboard(textM);
            audBeebSuccess.play();
        }
        catch (e) {
            console.log(e);
        }

	};
	btnGetMembers.addEventListener("click", onClickGetMembers , false);
	divTools.insertBefore(btnGetMembers, divTools.firstChild);
	audBeebSuccess.play();
    console.log('0.2');
}

onInjectBtn();

})();