...
// @license     MIT
// @downloadURL https://update.greasyfork.org/scripts/543650/OBG-Estado%20de%20patio.user.js
// @updateURL https://update.greasyfork.org/scripts/543650/OBG-Estado%20de%20patio.meta.js
// ==/UserScript==
...
(function() {
  ...
  const TASK_FETCH_INTERVAL = 30000; // 30 seconds for task data
...
function fetchCarrierData() {
  ...
  function fetchActivityLog(uuid) {
    return new Promise((resolve, reject) => {
      ...
      fetch('https://yard-visibility-na12.api.project44.com/v1/asset/activity-log/search?start=1&end=10&asset_class=trailer&asset_id=' + uuid + '&site=obregon&activity_type=arrival', {
        method: 'GET',
        headers: headers,
        credentials: 'include'
      })
...
function fetchProductTypes() {
  ...
  // Function to create departure modal (updated to use enriched data)
  function createDepartureModal(assetData, enrichedItem) {
    ...
    const numeroGafete = extractFromComment(comment, 'ga');
    const numeroIdentificacion = extractFromComment(comment, 'id');
    ...
    const numeroGafeteField = createField('NÚMERO DE GAFETE', numeroGafete);
    modalContent.appendChild(numeroGafeteField.fieldDiv);
 
    const numeroIdentificacionField = createField('NÚMERO DE IDENTIFICACIÓN', numeroIdentificacion);
    modalContent.appendChild(numeroIdentificacionField.fieldDiv);
        ...
        if (select.value) {
          processDeparture(assetData, true, formData)
            .then(() => {
              ...
              const cleanFolio = folio.replace(/_[12]\$/, ''); // Remove _1 or _2
              const url = 'https://script.google.com/macros/s/AKfycbz_vhCZhLUIvwjvbKfI2EA3mWOqiAS-k9A9eahN2JKRPXtwEFTIXTAwy-7lwfBOU1c9/exec?shipment=' + cleanFolio + '&reasoncode=' + select.value;
...
// Modified fetchTaskData function - fetch from both endpoints and throttle to every 30 seconds
function fetchTaskData() {
  return new Promise((resolve, reject) => {
    const now = Date.now();
    
    // Check if we've fetched within the last 30 seconds
    if (now - lastTaskDataFetch < TASK_FETCH_INTERVAL) { // Changed to 30 seconds
    ...
    console.log("Fetching task data from both endpoints...");
    ...
    // First endpoint
    const firstEndpoint = fetch('https://yard-visibility-na12.api.project44.com/v1/task-search?page_no=1&size=100&site=obregon', {
      method: 'GET',
      headers: headers,
      credentials: 'include'
    });
    ...
    // Second endpoint  
    const secondEndpoint = fetch('https://yard-visibility-na12.api.project44.com/v1/task-search?size=50&created_by_me=false&site=obregon&atlas_enabled=true&page_no=1&is_kanban=true&task_status=started', {
      method: 'GET',
      headers: headers,
      credentials: 'include'
    });
    ...
    Promise.all([firstEndpoint, secondEndpoint])
      .then(responses => {
        // Check if both responses are OK
        responses.forEach((response, index) => {
          if (!response.ok) {
            throw new Error('Task API endpoint ' + (index + 1) + ' responded with status ' + response.status);
          }
        });
      ...
      .then(([data1, data2]) => {
        console.log("Task data fetched successfully", { 
          endpoint1Items: data1.data?.length || 0,
          endpoint2Items: data2.data?.length || 0
        });
        
        // Check if both responses have the expected format
        if (!data1.data) {
          console.log("First API response missing data property", data1);
          throw new Error("First API response missing data property");
        }
        
        if (!data2.data) {
          console.log("Second API response missing data property", data2);
          throw new Error("Second API response missing data property");
        }
        
        // Combine data from both endpoints, removing duplicates by task ID
        const combinedTasks = [...(data1.data || [])];
        const existingTaskIds = new Set(combinedTasks.map(task => task.id || task.uuid));
        
        // Add tasks from second endpoint that aren't already in the first
        (data2.data || []).forEach(task => {
        ...
        taskData = combinedTasks;
        console.log("Combined task data: " + taskData.length + " total tasks");
        resolve(taskData);
      })
      .catch(error => {
  ...
  // Extract badge (gafete) from the comment field
  function extractBadge(item) {
    if (!item) return null;
    
    // Check for comment in trailer object first
    if (item.trailer && item.trailer.comment) {
      const trailerMatch = item.trailer.comment.match(/<ga>(.*?)<\/ga>/);
      if (trailerMatch) return trailerMatch[1];
    }
    
    // Then check for comment in the main object
    if (item.comment) {
      const match = item.comment.match(/<ga>(.*?)<\/ga>/);
      if (match) return match[1];
    }
  ...
  // Extract identification from the comment field
  function extractIdentification(item) {
    if (!item) return null;
    
    // Check for comment in trailer object first
    if (item.trailer && item.trailer.comment) {
      const trailerMatch = item.trailer.comment.match(/<id>(.*?)<\/id>/);
      if (trailerMatch) return trailerMatch[1];
    }
    
    // Then check for comment in the main object
    if (item.comment) {
      const match = item.comment.match(/<id>(.*?)<\/id>/);
      if (match) return match[1];
    }
  ...
  // Add or update badge (gafete) for a trailer
  function updateBadge(item, badgeText) {
    return new Promise((resolve, reject) => {
      if (!authorization || !cookieValue) {
        reject(new Error("Authorization or Cookie not captured yet."));
        return;
      ...
      console.log('Fetching asset details for UUID ' + item.uuid + ' to update badge');
      ...
      .then(data => {
        ...
        // Add the badge information to the comment
        let comment = '';
        ...
        // Add or update the badge in the comment
        if (comment.includes('<ga>')) {
          // Replace existing badge
          comment = comment.replace(/<ga>.*?<\/ga>/, '<ga>' + badgeText + '</ga>');
        } else {
          // Add new badge
          comment = comment + '<ga>' + badgeText + '</ga>';
        }
        ...
        // Set the comment with the badge
        updatePayload.comment = comment;
        ...
        console.log('Sending badge update for UUID ' + item.uuid);
      ...
      .then(result => {
        console.log('Badge updated for UUID ' + item.uuid, result);
        ...
        if (itemIndex !== -1) {
          // Update the comment in our local data structure
          if (yardData[itemIndex].trailer && yardData[itemIndex].trailer.comment) {
            if (yardData[itemIndex].trailer.comment.includes('<ga>')) {
              yardData[itemIndex].trailer.comment = yardData[itemIndex].trailer.comment.replace(
                /<ga>.*?<\/ga>/, 
                '<ga>' + badgeText + '</ga>'
              );
            } else {
              yardData[itemIndex].trailer.comment = yardData[itemIndex].trailer.comment + '<ga>' + badgeText + '</ga>';
            }
          } 
          else if (yardData[itemIndex].comment) {
            if (yardData[itemIndex].comment.includes('<ga>')) {
              yardData[itemIndex].comment = yardData[itemIndex].comment.replace(
                /<ga>.*?<\/ga>/, 
                '<ga>' + badgeText + '</ga>'
              );
            } else {
              yardData[itemIndex].comment = yardData[itemIndex].comment + '<ga>' + badgeText + '</ga>';
            }
          }
        }
      ...
      .catch(error => {
        console.log('Error updating badge for UUID ' + item.uuid + ':', error.message);
        reject(error);
      });
    });
  }
 
  // Add or update identification for a trailer
  function updateIdentification(item, identificationText) {
    return new Promise((resolve, reject) => {
      if (!authorization || !cookieValue) {
        reject(new Error("Authorization or Cookie not captured yet."));
        return;
      ...
      console.log('Fetching asset details for UUID ' + item.uuid + ' to update identification');
      ...
      .then(data => {
        ...
        // Add the identification information to the comment
        let comment = '';
        ...
        // Add or update the identification in the comment
        if (comment.includes('<id>')) {
          // Replace existing identification
          comment = comment.replace(/<id>.*?<\/id>/, '<id>' + identificationText + '</id>');
        } else {
          // Add new identification
          comment = comment + '<id>' + identificationText + '</id>';
        }
        ...
        // Set the comment with the identification
        updatePayload.comment = comment;
        ...
        console.log('Sending identification update for UUID ' + item.uuid);
      ...
      .then(result => {
        console.log('Identification updated for UUID ' + item.uuid, result);
        ...
        if (itemIndex !== -1) {
          // Update the comment in our local data structure
          if (yardData[itemIndex].trailer && yardData[itemIndex].trailer.comment) {
            if (yardData[itemIndex].trailer.comment.includes('<id>')) {
              yardData[itemIndex].trailer.comment = yardData[itemIndex].trailer.comment.replace(
                /<id>.*?<\/id>/, 
                '<id>' + identificationText + '</id>'
              );
            } else {
              yardData[itemIndex].trailer.comment = yardData[itemIndex].trailer.comment + '<id>' + identificationText + '</id>';
            }
          } 
          else if (yardData[itemIndex].comment) {
            if (yardData[itemIndex].comment.includes('<id>')) {
              yardData[itemIndex].comment = yardData[itemIndex].comment.replace(
                /<id>.*?<\/id>/, 
                '<id>' + identificationText + '</id>'
              );
            } else {
              yardData[itemIndex].comment = yardData[itemIndex].comment + '<id>' + identificationText + '</id>';
            }
          }
        }
      ...
      .catch(error => {
        console.log('Error updating identification for UUID ' + item.uuid + ':', error.message);
        reject(error);
      });
    });
  }
  ...
  function exportToExcel() {
    ...
    try {
      ...
      // Add headers - Updated to include new LOCALIZADOR column
      const headers = [
        ...
        'Gafete', 'Identificación', 'Localizador', 'Incidente Almacén', 'Razón de Rechazo',
      ...
      Promise.all(enrichPromises).then(enrichedItems => {
        enrichedItems.forEach(item => {
          ...
          // Get badge and identification
          const badge = extractBadge(item)?.toUpperCase() || '';
          const identification = extractIdentification(item)?.toUpperCase() || '';
  ...
  // Fallback CSV export - Updated to include new column and enriched data
...
function fetchYardData(manualFetch = false) {
  ...
  // Fetch yard data
  const yardDataPromise = fetch('https://yard-visibility-na12.api.project44.com/v1/asset/search?page_no=1&size=100&assetClass=trailer&in_yard=true&site=obregon', {
    method: 'GET',
    headers: headers,
    credentials: 'include'
  })
  ...
  function sortData(data) {
    ...
    return [...data].sort((a, b) => {
      ...
      // Extract values based on column (updated for new column count)
  ...
  // Filter data based on current filter values - Updated to use enriched data
      ...
      for (const columnIndex in filters) {
        ...
        switch(parseInt(columnIndex)) {
          ...
          case 19: // Incidente seguridad
            ...
            break;
          case 20: // Gafete
            itemValue = extractBadge(item) || '';
            break;
          case 21: // Identificación
            itemValue = extractIdentification(item) || '';
            break;
          case 22: // Localizador
  ...
  // Update the table with fetched yard data - Updated to use enriched data
  function updateTable() {
    ...
    if (yardData.length === 0) {
      ...
      emptyRow.innerHTML = '<td colspan="31" style="text-align: center; padding: 20px;">NO HAY DATOS DISPONIBLES</td>';
    ...
    if (displayData.length === 0) {
      const emptyRow = document.createElement('tr');
      emptyRow.innerHTML = '<td colspan="31" style="text-align: center; padding: 20px;">NO HAY RESULTADOS PARA LOS FILTROS ACTUALES</td>';
      tableBody.appendChild(emptyRow);
      return;
    }
    ...
    Promise.all(enrichPromises).then(enrichedItems => {
      enrichedItems.forEach((item, index) => {
        try {
          ...
          // Identification data
          const identification = extractIdentification(item);
          ...
          // Create cells (all existing cells remain the same)
          ...
          tdBadge.style.cssText = cellStyle;
          if (badge) {
            tdBadge.textContent = badge.toUpperCase();
            ...
            `;
            editButton.onclick = function() {
              const newBadge = prompt("Actualizar número de gafete:", badge);
              if (newBadge !== null) {
                updateBadge(item, newBadge)
                  .catch(err => {
                    alert('Error actualizando gafete: ' + err.message);
                  });
              }
            };
            tdBadge.appendChild(editButton);
          } else {
            const addButton = document.createElement('button');
            addButton.textContent = 'AÑADIR';
            addButton.className = 'badge-button';
            addButton.style.cssText = `
              padding: 4px 8px;
              background-color: #4CAF50;
              color: white;
            ...
            `;
            addButton.onclick = function() {
              const badgeText = prompt("Ingrese el número de gafete:");
              if (badgeText) {
                updateBadge(item, badgeText)
                  .catch(err => {
                    alert('Error añadiendo gafete: ' + err.message);
                  });
              }
            };
            tdBadge.appendChild(addButton);
          }
          
          // Identification column (Identificación)
          const tdIdentification = document.createElement('td');
          tdIdentification.style.cssText = cellStyle;
          if (identification) {
            tdIdentification.textContent = identification.toUpperCase();
            ...
            `;
            editButton.onclick = function() {
              const newIdentification = prompt("Actualizar identificación:", identification);
              if (newIdentification !== null) {
                updateIdentification(item, newIdentification)
                  .catch(err => {
                    alert('Error actualizando identificación: ' + err.message);
                  });
              }
            };
            tdIdentification.appendChild(editButton);
          } else {
            const addButton = document.createElement('button');
            addButton.textContent = 'AÑADIR';
            addButton.className = 'identification-button';
            addButton.style.cssText = `
              padding: 4px 8px;
            ...
            `;
            addButton.onclick = function() {
              const identificationText = prompt("Ingrese la identificación:");
              if (identificationText) {
                updateIdentification(item, identificationText)
                  .catch(err => {
                    alert('Error añadiendo identificación: ' + err.message);
                  });
              }
            };
            tdIdentification.appendChild(addButton);
          }
          ...
          // Append all cells to the row (including new LOCALIZADOR and SALIDA columns)
          ...
          row.appendChild(tdIdentification);
          row.appendChild(tdLocalizador); // Add new LOCALIZADOR column
          ...
          row.appendChild(tdSalida); // Add SALIDA column
  ...
  function showConsole() {
    ...
    // Create header row with sortable and filterable headers - Updated to include LOCALIZADOR
  ...
  // Create sample data for testing - Updated to include enriched data and localizador
    ...
    const samples = [
      ...
      },
      {
        trailer: {
          ...
          comment: "<ga>B54321</ga><lo>ZN-B-15</lo><y>Revisión de documentos pendiente</y>"
        },
        status: "loaded",
      ...
      },
      {
        trailer: {
          ...
          comment: "<id>BC987654</id><lo>ZN-C-08</lo><rr>R2-RECHAZO ALMACEN LLENO</rr>",
          load_information: [
            {
              sku: "000000000002010099-btl mod esp 12oz bulk premium",
              qty: 25
