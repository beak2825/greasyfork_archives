...
// @grant        all
// @downloadURL https://update.greasyfork.org/scripts/14083/Last%20SEASON%20TI.user.js
// @updateURL https://update.greasyfork.org/scripts/14083/Last%20SEASON%20TI.meta.js
// ==/UserScript==
			...
			if (xhr.readyState == 4)
			{
                ...
                function Last_Season_Avg_Ti(TI,Season_TI,session,months,playerID,wage) {
                    ...
                    { var This_Season = months + 1;
                      ...
                      if ((playerID > 108779965) || (wage < 30001))
                          return 0;
                      else{
                ...
                if (today.getDay() < 2){
                $("a[player_link=" + resultado.player.player_id + "]").closest("tr").append("<td class='right align_right AVGTI' style='padding-right:5px;'><span>" + (Season_TI(player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary)),SI(resultado.player.skill_index),return_wage(resultado.player.wage),24.795)/session).toFixed(2) + "</span></td>");
                }
