// ==UserScript==
// @name         WarScript - Player Attack Buttons (PDA Fork)
...
// @downloadURL https://update.greasyfork.org/scripts/545727/WarScript%20-%20Player%20Attack%20Buttons%20%28PDA%20Fork%29.user.js
// @updateURL https://update.greasyfork.org/scripts/545727/WarScript%20-%20Player%20Attack%20Buttons%20%28PDA%20Fork%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    console.log("WarScript (PDA Fork): Script starting (v3.7.3-pda)...");
            ...
            GM_xmlhttpRequest({
                ...
                onload: response => {
                    if (!response || typeof response.responseText === 'undefined') {
                        console.error("WarScript: onload callback received an invalid response object.");
                        resolve(null);
                        return;
                    }
                    try {
    ...
    function createNewRow(data = {}) {
        ...
        row.innerHTML = `
            ...
            <div class="settings-column stats"><span class="stats-display">-</span></div>
    ...
    function loadSettings() {
        ...
        settings.forEach(setting => createNewRow(setting));
    }
    ...
    async function forcePollAndUpdate() {
        ...
        if (factionId) {
            const data = await fetchFromServer('faction', { apiKey: apiKey, id: factionId });
            if (data && data.members) {
                 const membersObject = Array.isArray(data.members) ? data.members.reduce((obj, item) => { obj[item.id] = item; return obj; }, {}) : data.members;
                 liveData = { ...liveData, ...membersObject };
            }
        }
        ...
        for (const player of monitoredPlayers) {
            let data = await fetchFromServer('individual', { apiKey: apiKey, id: player.profileId, selections: 'profile,basic' });
            ...
            try {
                ...
                if (data && data.members) {
                    try {
                        ...
                        const membersObject = Array.isArray(data.members) ? data.members.reduce((obj, item) => { obj[item.id] = item; return obj; }, {}) : data.members;
                        const mergedData = {...currentData, ...membersObject};
                    ...
                    logDebug("Faction poll ran, but no data.members was found from server.");
                }
            } catch (e) {
    ...
    function initializeUI() {
        ...
        document.getElementById('faction-fetch-button').addEventListener('click', async () => {
            ...
            const data = await fetchFromServer('faction', { apiKey: apiKey, id: factionId.trim() });

            if (data && data.members) {
                let membersArray = Array.isArray(data.members) ? data.members : Object.keys(data.members).map(id => data.members[id]);

                membersArray.forEach(member => {
                    createNewRow({ profileId: member.id, name: member.name, enabled: false, source: 'faction' });
                });
