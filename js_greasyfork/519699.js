...
// @downloadURL https://update.greasyfork.org/scripts/519699/MouseHunt%20-%20Mapping%20Helper%2B.user.js
// @updateURL https://update.greasyfork.org/scripts/519699/MouseHunt%20-%20Mapping%20Helper%2B.meta.js
// ==/UserScript==
...
(function () {
  ...
  function render() {
    ...
    masterEl.appendChild(refreshSpan);
      ...
      function transferSB(snuid, name) {	// replace original function delcaration at line ~395
          // code chunk taken from WS' Profile+ tipping script
    ...
    if (cacheRaw) {
      ...
      if (cache[mapName] !== undefined) {
        ...
        const imgIDMap = {};
        const idNameMap = {};
		const hunters = cache[mapName].hunters.map(hunter => {if (hunter.is_active) return hunter}).filter(value => value !== undefined);
        hunters.forEach(el => {
          if (el.profile_pic) imgIDMap[el.profile_pic] = el.sn_user_id;
          idNameMap[el.sn_user_id] = el.name;
        });

		  // Function to generate Discord tags and copy to clipboard
		  function generateDiscordTags() {
			  // Section: Extract In-Game Names
			  const selectedKeys = ['name','user_id','sn_user_id','is_active','upgrader','captain'];
			  const mappers = {};
			  for (let i in hunters) {
				  mappers[i] = {};
				  for (let key in selectedKeys) {
					  // console.log(`${selectedKeys[key]}:${hunters[i][selectedKeys[key]]}`)
					  mappers[i][selectedKeys[key]] = hunters[i][selectedKeys[key]];
				  }
			  }

			  // Section: Extract Discord IDs from LocalStorage
			  function csvToObject(csvString) {
				  ...
				  for (let i = 1; i < rows.length; i++) {
					  ...
					  result[entry.snuid] = entry;
				  }

				  return result;
			  }
			  const cachedMappingCSV = localStorage.getItem('ast-gwh-chaos-mapping-discord-mapping');
			  const discordIDMapObj = csvToObject(cachedMappingCSV);

			  // Section: Generate Discord Tags
			  let copyPasteOutput = '';
			  for (let i in mappers) {
				  // console.log(`Testing ${mappers[i]['name']}, snuid:${mappers[i]['sn_user_id']}`);
				  if (discordIDMapObj[mappers[i]['sn_user_id']]) {
					  const discordID = discordIDMapObj[mappers[i]['sn_user_id']]['discord_id'];
					  mappers[i]['discord_id'] = discordID;
					  if (mappers[i]['captain']) copyPasteOutput += `Maptain: `;
					  if (!mappers[i]['upgrader']) copyPasteOutput += `<@${discordID}>, `;
				  }
			  }
			  // Remove trailing comma and space
			  copyPasteOutput = copyPasteOutput.slice(0, -2);
			  console.table(discordIDMapObj);
			  console.table(mappers);

			  // Section: Copy to Clipboard
			  const textArea = document.createElement('textarea');
			  textArea.value = copyPasteOutput;
        ...
        function imgHover(img) {
          ...
          if (snuid) {
            ...
            img.onclick = function () {
              transferSB(snuid, name); // asterios added name to work with updated transferSB function
            };
	  ...
	  // Add a button for triggering the script
	  // const buttonContainer = document.querySelector('.treasureMapView-mapMenu-group-actions');
	  const generateDiscordTagsButton = document.createElement('button');
	  generateDiscordTagsButton.id = 'generateDiscordTagsButton';
	  generateDiscordTagsButton.className = "tsitu-mapping";
	  generateDiscordTagsButton.textContent = 'Generate Discord Tags';
	  generateDiscordTagsButton.addEventListener('click', () => {
		  generateDiscordTags();
	  });
	  document.querySelector('.treasureMapPopup .suffix').insertBefore(generateDiscordTagsButton,document.querySelector('.treasureMapPopup .suffix .jsDialogClose'));
	...
	// Function to fetch the discord id mapping from Google Sheets for GWH chaos mapping
	async function updateMappingFromGoogleSheet() {
		const csvLink = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vRnQ0D77USsUBINKqaP_Y8_Anyv9yUNbhcX8Sp3RL101bFkdYxWjxMpkTQ2w4b_25VvSjkCMajsl4GD/pub?gid=841370847&single=true&output=csv';

		// Fetch CSV data
		fetch(csvLink)
			.then(response => response.text())
			.then(csvData => {
			// Process and use the CSV data in your script
			console.log('New discord id mapping cached in localStorage for GWH Chaos');
			// console.log(csvData);
			localStorage.setItem('ast-gwh-chaos-mapping-discord-mapping', csvData);
		})
			.catch(error => console.error('Error fetching CSV data:', error));
	}

	// Check for updates when the page loads
	updateMappingFromGoogleSheet();
