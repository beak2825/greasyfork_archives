...
// @connect        s.ytimg.com
// @version        2.44
// @downloadURL https://update.greasyfork.org/scripts/39420/YouTube%20Links.user.js
// @updateURL https://update.greasyfork.org/scripts/39420/YouTube%20Links.meta.js
// ==/UserScript==
...
decryptSig.extractScriptUrl = function(data) {
  if(data.match(/ytplayer.config\s*=.*"assets"\s*:\s*\{.*"js"\s*:\s*(".+?")[,}]/))
    ...
    return JSON.parse(RegExp.$1);
  else if(data.match(/"rootElementId":"movie_player","jsUrl":(".+?")[,}]/))
    return JSON.parse(RegExp.$1);
  else
    return false;
  };
...
decryptSig.fetchScript = function(scriptName, url) {
  ...
  function findDecodeCode(sourceCode) {
    ...
    /\.set\s*\("n"\s*,\s*\w+\),\s*[\$\w]+\.length\s*\|\|\s*([a-zA-Z0-9_$][\w$]*)\(/);
    if (decodeFunctionName !== null) {
      ...
      functionCode = findMatch(sourceCode, /(=\s*function\s*\([\w$]*\)\s*\{var\s+\w+=String.prototype.split.call[\w\W]+enhanced_except_[^\}]+\}return Array.prototype.join.call\(\w+\s*,\s*""\)\})/)
                  || findMatch(sourceCode, /(=\s*function\s*\([\w$]*\)\s*\{var\s+\w+=String.prototype.split.call[\w\W]+enhanced_except_[^\}]+\}return [\w$]*\.join\(""\)\})/);
    }
    if (functionCode === null) return;

    //window.eval("window.decodeN" + functionCode);
    return functionCode;
  }

  function success(data) {
    let decodeFn = findDecodeCode(data);
    data = data.replace(/\n|\r/g, "");
    ...
    decryptSig.store[scriptName] = { ver: relInfo.ver, ts: timeNowInSec(), fn: fullFn, decodeFn: decodeFn };
...
function deobfuscateVideoSig(scriptName, sig) {
  ...
  try {
    sig = eval("(" + decryptSig.store[scriptName].fn + ") (\"" + decodeURIComponent(sig) + "\")");
  } catch(e) {
    }
...
function decodeN(scriptName, n) {
  ...
  try {
    n = eval("(" + decryptSig.store[scriptName].decodeFn + ") (\"" + n + "\")");
  } catch(e) {
  }
...
function parseNewFormatsMap(map, str, unescSlashFlag) {
  ...
  var list = ((data) => {try {return JSON.parse(data);} catch (e) {return JSON.parse('{}');}})(str);
  ...
  forEach(list, function(idx, elm) {
    ...
    if(cipher) {
      ...
      if (obj.url.match(/&n=([^&]+)/)) {
        obj.url = obj.url.replace(/&n=([^&]+)/, `&n=${decodeN(map.scriptName, RegExp.$1)}`);
      }
...
function getVideoInfo(url, callback) {
  ...
  function success(data) {
    ...
    if (ytplayer && ytplayer.config && ytplayer.config.args) {
      let args=ytplayer.config.args;
      let jsonStreamData = (args.player_response == undefined ? args.raw_player_response : JSON.parse(args.player_response));
      streamData = JSON.stringify(jsonStreamData);
    }
    ...
    if(streamData.match(/[,{]"url_encoded_fmt_stream_map":\s?"([^"]+)"[,}]/)) {
    ...
    if(streamData.match(/[,{]"adaptive_fmts":\s?"(.+?)"[,}]/)) {
      parseAdaptiveStreamMap(map, RegExp.$1);
      oldFmtFlag = true;
      }

    if(streamData.match(/[,{]"fmt_list":\s?"([^"]+)"[,}]/))
      parseFmtList(map, RegExp.$1);

    // Is part of 'player_response' and is escaped
    if(!oldFmtFlag && streamData.match(/\\"formats\\":(\[{[^\]]*}\])[},]/)) {
      parseNewFormatsMap(map, RegExp.$1, /*unescSlash*/ true);
      newFmtFlag = true;
      }

    if(!oldFmtFlag && streamData.match(/\\"adaptiveFormats\\":(\[{[^\]]*}\])[},]/)) {
      parseNewFormatsMap(map, RegExp.$1, /*unescSlash*/ true);
      newFmtFlag = true;
      }
    ...
    if(!oldFmtFlag && !newFmtFlag) {
      for (let formats of streamData.match(/[,{]"formats":(\[{[^\]]*}\])[},]/g))
        if(formats.match(/[,{]"formats":(\[{[^\]]*}\])[},]/))
          parseNewFormatsMap(map, RegExp.$1);

      if(streamData.match(/[,{]"adaptiveFormats":(\[{[^\]]*}\])[},]/))
        parseNewFormatsMap(map, RegExp.$1);
      }

    if(streamData.match(/[,{]"dashmpd":\s?"(.+?)"[,}]/))
      map.dashmpd = decodeURIComponent(RegExp.$1.replace(/\\\//g, "/"));
    else if(streamData.match(/[,{]\\"dashManifestUrl\\":\s?\\"(.+?)\\"[,}]/))
      map.dashmpd = decodeURIComponent(RegExp.$1.replace(/\\\//g, "/"));
...
function condInsertHdr(divId) {
  ...
  divNode.style = 'position: relative;z-index: 1;';
...
Links.prototype.genUrl = function(map, elm) {
  ...
  if(ext)
    fname += "." + ext;
  var url = elm.url + "&title=" + encodeSafeFname(fname);
