...
// @version          1.0.0
// @author           improper.dev
// @license          CC-BY-NC-SA-4.0; https://creativecommons.org/licenses/by-nd/4.0/legalcode
...
// @match            https://*.chaturbate.com/*
// @match            https://*.chatur*.com/*
// @match            https://*.chaturbategirls.com/*
// @match            https://*.camvirt.*/*
// @match            https://*.chatubate.*/*
// @match            https://chaturbateeuu.chaturbate.com/*
// @match            https://*.chaturbate.*/*
// @match            https://*.camschedule.*/*
// @match            https://*.sexcamscentre.*/*
...
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/494329/Chaturbate%20Enhancer.user.js
// @updateURL https://update.greasyfork.org/scripts/494329/Chaturbate%20Enhancer.meta.js
// ==/UserScript==
...
(function() {
  ...
  function addElement(parent_node, tag_name, attributes) {
    ...
    // 将指定的父节点添加类名，以便识别消息来源
    $(parent_node).addClass('cb-enh-add-element-id-' + id);
    // 通过 postMessage 发送消息数据
    window.postMessage(JSON.stringify(msgData), '*');
  }
// 存储 XMLHttpRequest 请求的任务数组
let xhrTasks = [];

// 发起 XMLHttpRequest 请求的函数
function xmlhttpRequest(details) {
  // 如果存在 GM_xmlhttpRequest 函数，则使用它
...
$(document).ready(function() {
	...
	$(document).on("click", "#cb-enh-settings-use-intrf-def", function() {
		let enabled = $(this)[0].checked;
		setSetting('use-intrf-def', enabled);
		$('#cb-enh-video-controls-use-intrf-def')[0].checked = show;
	});
...
function updateSettingsWindow() {
	...
	$("#cb-enh-settings-use-intrf-def")[0].checked = getSetting('use-intrf-def');
...
function initBelowVideoButtons() {
	...
	$(document).on("click", "#cb-enh-video-controls-use-intrf-def", function() {
		let enabled = $(this)[0].checked;
		setSetting('use-intrf-def', enabled);
		$('#cb-enh-settings-use-intrf-def')[0].checked = enabled;
	});
...
let acre1 = 'Z3RhZ1woJ3NldCcsIHsnZGltZW5zaW';
...
let acre2 = '9uNCc6ICcoW2EtekEtWjAtOS1dKyknfVwpOw';
