...
// @downloadURL https://update.greasyfork.org/scripts/2252/Additional%20buttons%20for%20Lists%2C%20Posts%20and%20PMs.user.js
// @updateURL https://update.greasyfork.org/scripts/2252/Additional%20buttons%20for%20Lists%2C%20Posts%20and%20PMs.meta.js
// ==/UserScript==
...
function addButtons(prev) {
    ...
    if ($('#'+prev)) {
        ...
        if (document.URL.indexOf("~") < 0 && document.URL.indexOf("/list/") < 0) {
            ...
            document.getElementById('note').addEventListener('click', function(){editTextareaTwo('[note]','[/note]')}, false)
        }
        
        $('#'+prev).prev().before(btnTemplate.replace(/ID_HERE/g, 'spoiler').replace(/TEXT_HERE/g, 'spoiler'))
        document.getElementById('spoiler').addEventListener('click', function(){editTextareaTwo('[spoiler]','[/spoiler]')}, false)
        $('#'+prev).before(btnTemplate.replace(/ID_HERE/g, 'strikethrough').replace(/TEXT_HERE/g, '<s>S</s>'))
        document.getElementById('strikethrough').addEventListener('click', function(){editTextareaTwo('[s]','[/s]')}, false)                
        ...
        if (document.URL.indexOf("~") < 0 && document.URL.indexOf("/list/") < 0) {
            ...
            document.getElementById('colours').addEventListener('click', function(){editTextareaTwo('[color '+$('#colour_box').val()+']', '[/color]')}, false)
            ...
            document.getElementById('rand_wi').addEventListener('click', function(){editTextareaTwo('', '[wi:' + Math.random().toString(36).replace(/[^a-z]+/g, '') + ']')}, false)
