// ==UserScript==
// @name         zmpt-Torrent-Assistant
// @namespace    http://tampermonkey.net/
// @version      1.2
// @description  织梦种审助手
// @author       zmpt
// @include      http*://zmpt.cc/details.php*
// @include      http*://zmpt.cc/offers.php*off_details*
// @include      http*://zmpt.cc/torrents.php*
// @include      http*://zmpt.club/details.php*
// @include      http*://zmpt.club/offers.php*off_details*
// @include      http*://zmpt.club/torrents.php*
// @include      https://152.67.204.212/details.php*
...
// @connect      *
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/509632/zmpt-Torrent-Assistant.user.js
// @updateURL https://update.greasyfork.org/scripts/509632/zmpt-Torrent-Assistant.meta.js
// ==/UserScript==
(function() {
	'use strict';
	...
	//=====================================
	var isEditor;
	if (GM_info.script.name === "zmpt-Torrent-Assistant") {
	...
	if (window.location.href.includes("/details.php") || window.location.href.includes("/offers.php")) {
		...
		if (/[.| ](dts-hd|dtshd|dts-x|dts:x)/.test(title_lowercase)) {
			...
			title_audio = 19;
		} else if (/[.| ]ac3|[.| ]ac-3|[.| ]ddp|[.| ]dd\+|[.| ]dd2|[.| ]dd5|[.| ]dd\.2|[.| ]dd\.5/.test(
				title_lowercase)) {
			title_audio = 9;
		...
		mediainfo_s = document.getElementById("kdescr").innerText;
		...
		for (var i = 0; i < tdlist.length; i++) {
			...
			if (/字\s*幕.*?Chinese/i.test(mediainfo_s) || /字\s*幕.*?Mandarin/i.test(
					mediainfo_s) || /Subtitle:\s*?Chinese/i.test(mediainfo_s)||/Language: +|\s*?Chinese/i.test(mediainfo_s)) {
				sub_chinese = true;
			} else {
				sub_chinese = false;
			}
		...
		if (/-(FGT|NSBC|BATWEB|GPTHD|DreamHD|BlackTV|CatWEB|Xiaomi|Huawei|MOMOWEB|DDHDTV|SeeWeb|TagWeb|SonyHD|MiniHD|BitsTV|CTRLHD|ALT|NukeHD|ZeroTV|HotTV|EntTV|GameHD|SmY|SeeHD|VeryPSP|DWR|XLMV|XJCTV|Mp4Ba|GodDramas|FRDS|BeiTai|Ying|VCB-Studio)/
			.test(title_lowercase)) {
		...
		if ((type === 6 || type === 4 || type === 7 || type === 8 || type === 9 || type === 10) && $(
				'.mediainfo-short .codemain').text().replace(/\s+/g, '') === $('.mediainfo-raw .codemain').text()
			.replace(/\s+/g, '')) {
		...
		if (isEditor) {
			...
			if ((/x264/i.test(title_lowercase) && /10bit/i.test(title_lowercase)) || (/Bit\s+depth\s*:\s*10\s+bits/i
