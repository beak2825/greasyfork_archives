...
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/564149/Bilibili%E5%AD%97%E5%B9%95%E5%AE%9E%E6%97%B6%E6%98%BE%E7%A4%BA%E6%8F%92%E4%BB%B6%28%E5%88%86P%E7%89%88%29.user.js
// @updateURL https://update.greasyfork.org/scripts/564149/Bilibili%E5%AD%97%E5%B9%95%E5%AE%9E%E6%97%B6%E6%98%BE%E7%A4%BA%E6%8F%92%E4%BB%B6%28%E5%88%86P%E7%89%88%29.meta.js
// ==/UserScript==
...
(function () {
    ...
    const SubtitleFetcher = {
        ...
        async getVideoInfo() {
            ...
            const pIndex = urlParams.get('p');

            // 新增：从页面状态中获取分P列表
            const state = window.__INITIAL_STATE__;
            if (state && state.videoData && state.videoData.pages) {
                // 如果有分P列表
                if (pIndex && state.videoData.pages.length > 1) {
                    const pageIndex = parseInt(pIndex) - 1; // p参数从1开始，数组从0开始
                    if (pageIndex >= 0 && pageIndex < state.videoData.pages.length) {
                        // 使用对应分P的CID
                        info.cid = state.videoData.pages[pageIndex].cid;
                        console.log(`使用分P ${pIndex} 的CID: ${info.cid}`);
                    }
                } else if (state.videoData.cid) {
                    // 默认使用第一个分P的CID
                    info.cid = state.videoData.cid;
                }
            }
    ...
    const SubtitleUI = {
        injectStyles() {
            ...
            style.textContent = `
                ...
                .subtitle-function-btn:nth-child(2) {
                    flex: 0;
                    justify-content: flex-start;
                }
                ...
                /* 响应式布局 */
                @media (max-widcth: 400px) {
                    .subtitle-funtion-btn span {
                        font-size: 11px;
                    }
    ...
    async function main() {
        ...
        // 在主函数main()的末尾，添加分P切换监听
        // 分P切换监听模块
        const PageChangeDetector = {
            ...
            init(onPageChange) {
                ...
                this.lastCid = window.cid;

                // 方法1: 监听URL变化 (处理用户点击分P列表)
                ...
                // 方法2: 监听popstate事件 (处理浏览器前进后退)
                window.addEventListener('popstate', () => {
                    console.log("Popstate event detected");
                    setTimeout(() => onPageChange(), 500);
                });

                // 方法3: 监听视频cid变化 (更可靠的检测)
                const cidCheckInterval = setInterval(() => {
                    const currentCid = window.cid;
                    if (currentCid && currentCid !== this.lastCid) {
                        console.log("CID changed:", this.lastCid, "->", currentCid);
                        this.lastCid = currentCid;
                        onPageChange();
                    }
                }, 1000);

                // 方法4: 拦截history.pushState (处理B站的路由跳转)
        ...
        async function reloadSubtitles() {
            ...
            try {
                ...
                if (!subtitleConfig) {
                    console.log("该分P没有字幕");
                    content.innerHTML = `<div class="subtitle-item">该视频没有CC字幕或AI字幕</div>`;
                    subtitles = null;
                    return;
                }
                ...
                if (newSubtitles) {
                    ...
                    console.log("字幕重新加载完成");
                    showTempNotification("字幕已更新");
                }
            } catch (error) {
                console.error("重新加载字幕失败:", error);
                content.innerHTML = `<div class="subtitle-item">${error.message}</div>`;
                subtitles = null;
            }
        }
        ...
        // 初始化分P切换检测
        PageChangeDetector.init(reloadSubtitles);
    }
