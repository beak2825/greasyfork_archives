// ==UserScript==
// @name            Bilibili 按标签、标题、时长、UP主屏蔽视频 Plus
// @namespace       https://github.com/tjxwork
// @version         0.5.8-Plus
// @note            v0.5.8-Plus 修复低分辨率情况下设置面板高度超出问题，增加根据UP名称关键词匹配，增加规则订阅功能，增加播放页面一键屏蔽功能（按钮在右上角用户名旁边），增加低评论数屏蔽。
// @description     对Bilibili.com的视频卡片元素，以 标签、标题、时长、UP主名称、UP主UID 来判断匹配，添加一个屏蔽叠加层或者隐藏视频。
// @author          tjxwork,NoBB
// @license         CC-BY-NC-SA
// @icon            https://www.bilibili.com/favicon.ico
// @homepageURL     https://greasyfork.org/zh-CN/scripts/494277-bilibili-%E6%8C%89%E6%A0%87%E7%AD%BE-%E6%A0%87%E9%A2%98-%E6%97%B6%E9%95%BF-up%E4%B8%BB%E5%B1%8F%E8%94%BD%E8%A7%86%E9%A2%91-plus
// @supportURL      https://greasyfork.org/zh-CN/scripts/494277-bilibili-%E6%8C%89%E6%A0%87%E7%AD%BE-%E6%A0%87%E9%A2%98-%E6%97%B6%E9%95%BF-up%E4%B8%BB%E5%B1%8F%E8%94%BD%E8%A7%86%E9%A2%91-plus/feedback#post-discussion
...
// @downloadURL https://update.greasyfork.org/scripts/494277/Bilibili%20%E6%8C%89%E6%A0%87%E7%AD%BE%E3%80%81%E6%A0%87%E9%A2%98%E3%80%81%E6%97%B6%E9%95%BF%E3%80%81UP%E4%B8%BB%E5%B1%8F%E8%94%BD%E8%A7%86%E9%A2%91%20Plus.user.js
// @updateURL https://update.greasyfork.org/scripts/494277/Bilibili%20%E6%8C%89%E6%A0%87%E7%AD%BE%E3%80%81%E6%A0%87%E9%A2%98%E3%80%81%E6%97%B6%E9%95%BF%E3%80%81UP%E4%B8%BB%E5%B1%8F%E8%94%BD%E8%A7%86%E9%A2%91%20Plus.meta.js
// ==/UserScript==
...
function blockedMenuUi() {
    ...
    const blockedCommentCountInput = createInputModule(
        "按评论数少于指定数的视频屏蔽 (-1为不生效)",
        blockedParameter,
        "blockedCommentCount",
        "Number"
    );
    const blockedCommentRatioInput = createInputModule(
        "按评论率屏蔽 (0为不生效)",
        blockedParameter,
        "blockedCommentRatio",
        "Number"
    ...
    menuContent.appendChild(blockedCommentCountInput);
    menuContent.appendChild(blockedCommentRatioInput);
...
function saveButtonClickFunction(blockedParameterObject) {
    ...
    for (const item of inputIdItem) {
        ...
        if (item.type == "number") {
            ...
            if (item.id === "blockedCommentRatio") {
                blockedParameterObject[item.id] = parseFloat(item.value);
            } else {
            // 多行展示框 的数据存入 blockedParameter
    ...
    console.log("Block Data", blockedParameterObject);
...
function handleBlockedTitle(videoElement, videoBv) {
    ...
    const blockedTitleFind = blockedParameter.blockedTitleArray.find((blockedTitle) => {
        ...
        if (blockedTitleRegEx.test(filterStr(videoInfoDict[videoBv].videoTitle))) {
            return blockedTitle;
        }
    });
    if (blockedTitleFind) {
...
function handleBlockedUpNameOrUpUid(videoElement, videoBv, withinFetch = false) {
    ...
    if (blockedNameOrUidFind) {
        createOverlay(`屏蔽UP: ${videoInfoDict[videoBv].videoUpName} ${blockedNameOrUidFind}`, videoElement, `${videoBv} handleBlockedUpNameOrUpUid()`);
...
// 一键屏蔽功能
function blockOnClick(uid) {
    if (!blockedParameter.blockedNameOrUidArray.includes(uid)) {
        blockedParameter.blockedNameOrUidArray.push(uid);
        alert("已屏蔽：" + uid);
    } else {
        blockedParameter.blockedNameOrUidArray = blockedParameter.blockedNameOrUidArray.filter(item => item !== uid);
        alert("已解除屏蔽：" + uid);
    }
...
// 在播放页面增加快捷屏蔽按钮
var lastProcessUid = null;
function addBlockBtnInVideoPage() {
    if (location.pathname.startsWith("/video/")) {
        const checkDOM = document.getElementById("bilibili_blockplus_oneclick");
        const upDetailTopDOM = document.getElementsByClassName("up-detail-top");
        const spaceUrl = upDetailTopDOM[0].children[0].href;
        const uid = spaceUrl.substr(spaceUrl.lastIndexOf("/") + 1);
        if (lastProcessUid === uid && checkDOM) {
            return;
        }
        lastProcessUid = uid;
        if (checkDOM !== null) {
            checkDOM.innerText = blockedParameter.blockedNameOrUidArray.includes(uid) ? "解除屏蔽" : "一键屏蔽";
            checkDOM.onclick = () => {
                blockOnClick(uid);
            };
            return;
        };
        if (upDetailTopDOM.length > 0){
            const element = document.createElement("button");
            element.id = "bilibili_blockplus_oneclick";
            element.class = "send-msg";
            element.innerText = blockedParameter.blockedNameOrUidArray.includes(uid) ? "解除屏蔽" : "一键屏蔽";
            element.style = "margin-left: 8px;";
            element.onclick = () => {
                blockOnClick(uid);
            };
            upDetailTopDOM[0].appendChild(element);
        }
...
function convertToNumber(str) {
    // 移除所有非数字和非中文字符，只保留数字和单位
    const cleanedStr = str.replace(/[^\d\.\,万百千万亿]/g, '');
  
    // 正则表达式匹配数字和单位
    const regex = /(\d+(?:\.\d+)?)(万|百万|千万|亿)?/;
  
    // 匹配字符串
    const match = cleanedStr.match(regex);
  
    if (match) {
      let number = parseFloat(match[1]); // 转换为浮点数
      let unit = match[2]; // 获取单位
...
// 获取评论率
function getCommentRatio(videoElement) {
    const statusDOM = videoElement.querySelector(".bili-video-card__stats--left");
    const playCount = convertToNumber(statusDOM.children[0].innerText);
    const commentCount = convertToNumber(statusDOM.children[1].innerText);
    return commentCount / playCount;
}
// 获取评论数
function getCommentCount(videoElement) {
    const statusDOM = videoElement.querySelector(".bili-video-card__stats--left");
    // const playCount = convertToNumber(statusDOM.children[0].innerText);
    const commentCount = convertToNumber(statusDOM.children[1].innerText);
    return commentCount;
...
// 根据评论数屏蔽
function handleBlockedVideoCommentCount(videoElement, videoBv) {
    // 获取视频的评论数
    const commentCount = getCommentCount(videoElement);
  
    if (commentCount < blockedParameter.blockedCommentCount) {
        // 如果匹配成功，屏蔽该视频
        createOverlay(`评论数低: ${commentCount}`, videoElement, `${videoBv} handleBlockedVideoCommentCount()`);
        return true;
    }
    return false;
}
...
// 根据评论率屏蔽
function handleBlockedVideoCommentRatio(videoElement, videoBv) {
    // 获取视频的评论率
    const commentRatio = getCommentRatio(videoElement);
  
    if (commentRatio < blockedParameter.blockedCommentRatio) {
        // 如果匹配成功，屏蔽该视频
        createOverlay(`评论率低: ${commentRatio * 100}%`, videoElement, `${videoBv} handleBlockedVideoCommentRatio()`);
        return true;
    }
    return false;
}
...
function FuckYouBilibiliRecommendationSystem() {
    ...
    for (let videoElement of videoElementArray) {
        ...
        // 根据评论率屏蔽
        if (blockedParameter.blockedCommentRatio > 0) {
            if (handleBlockedVideoCommentRatio(videoElement, videoBv)) {
                continue;
            }
        }

        // 根据评论数屏蔽
        if (blockedParameter.blockedCommentCount > -1) {
            if (handleBlockedVideoCommentCount(videoElement, videoBv)) {
                continue;
            }
        }
    }
