...
// @name        GM dat.GUI
// @version     0.1.1
// @description dat.GUI modified to prioritize GM_getValue/GM_setValue over localStorage among other things.
// @license     Apache
// @author      dataarts @ GitHub, userscript by MadHero @ GreasyFork
      ...
      case 'MouseEvents':
        {
          ...
          evt.initMouseEvent(eventType, params.bubbles || false, params.cancelable || true, document.defaultView, params.clickCount || 1, 0,
          0,
          clientX,
          clientY,
      ...
      case 'KeyboardEvents':
        {
          ...
          init(eventType, params.bubbles || false, params.cancelable, document.defaultView, params.ctrlKey, params.altKey, params.shiftKey, params.metaKey, params.keyCode, params.charCode);
          break;
        }
      default:
        {
...
var css = {
  ...
  inject: function inject(cssContent, container) {
    ...
    try {
      if (!container && !document.head) {
        setTimeout(function () {
          (container || document.head).appendChild(injected);
        }, 200);
      } else {
        (container || document.head).appendChild(injected);
      }
...
var CenteredDiv = function () {
  function CenteredDiv(shadowDOMContainer) {
    ...
    var shadowDOM = shadowDOMContainer ? shadowDOMContainer.shadowRoot : false;
    if (shadowDOM && !document.body.contains(shadowDOMContainer)) {
      document.body.appendChild(shadowDOMContainer);
    }
    (shadowDOM || document.body).appendChild(this.backgroundElement);
    (shadowDOM || document.body).appendChild(this.domElement);
    var _this = this;
...
var setStorage = void 0;
var getStorage = void 0;
try {
  setStorage = GM_setValue;
  getStorage = GM_getValue;
} catch (e) {}
if (!setStorage || !getStorage) {
  try {
    setStorage = GM ? GM.setValue : undefined;
    getStorage = GM ? GM.getValue : undefined;
  } catch (e) {}
}
if (!setStorage || !getStorage) {
  try {
...
var SUPPORTS_STORAGE = !!setStorage && !!getStorage;
...
var GUI = function GUI(pars) {
  ...
  params = Common.defaults(params, {
    storageHashPrefix: getStorageHashPrefix(),
    useLocalStorage: false,
    autoSaveIfPossible: true
  });
  ...
  if (SUPPORTS_STORAGE) {
    var isLocal = getStorage(params.storageHashPrefix + 'isLocal');
    useLocalStorage = isLocal !== undefined ? isLocal : params.useLocalStorage;
  }
  var saveToStorage = void 0;
  var autoSaveIfPossible = void 0;
  var titleRow = void 0;
  Object.defineProperties(this,
  {
    ...
    useLocalStorage: {
      ...
      set: function set$$1(bool) {
        if (SUPPORTS_STORAGE) {
          ...
          setStorage(params.storageHashPrefix + 'isLocal', bool);
          var storageCheckBox = (shadowDOM || document).getElementById('dg-local-storage');
          if (storageCheckBox && storageCheckBox.checked !== useLocalStorage) {
            storageCheckBox.checked = useLocalStorage;
          }
        }
      }
    },
    storageHashPrefix: {
      get: function get$$1() {
        return params.storageHashPrefix;
      }
    },
    autoSaveIfPossible: {
      get: function get$$1() {
        return autoSaveIfPossible;
      },
      set: function set$$1(bool) {
        if (bool && autoSaveIfPossible !== bool) {
          dom.bind(window, 'beforeunload', saveToStorage);
        } else {
          dom.unbind(window, 'beforeunload', saveToStorage);
        }
        autoSaveIfPossible = bool;
      }
    }
  });
  if (Common.isUndefined(params.parent)) {
    this.autoSaveIfPossible = params.autoSaveIfPossible;
    dom.addClass(this.domElement, GUI.CLASS_MAIN);
    dom.makeSelectable(this.domElement, false);
    if (SUPPORTS_STORAGE) {
      if (useLocalStorage) {
        var savedGui = getStorage(params.storageHashPrefix + 'gui');
        if (savedGui) {
          params.load = savedGui;
        }
      }
    }
  ...
  if (params.autoPlace) {
    if (Common.isUndefined(params.parent)) {
      if (autoPlaceVirgin) {
        ...
        (shadowDOM || document.body).appendChild(autoPlaceContainer);
  ...
  saveToStorage = function saveToStorage() {
    if (SUPPORTS_STORAGE && useLocalStorage) {
      setStorage(params.storageHashPrefix + 'gui', _this.getSaveObject());
    }
  };
  this.saveToLocalStorageIfPossible = saveToStorage;
  function resetWidth() {
...
GUI.toggleHide = function () {
  ...
  Common.each(hideableGuis, function (gui) {
    gui.domElement.style.display = _hide ? 'none' : '';
  });
};
GUI.hide_key_code = 72;
...
GUI._keydownHandler = function (e) {
  if (e.which !== GUI.hide_key_code && e.keyCode !== GUI.hide_key_code) {
    return;
  }
  var elem = e.composedPath ? e.composedPath()[0] : e.target;
  var tag = elem.tagName;
  if (!elem.isContentEditable && (tag === 'BUTTON' || tag === 'FIELDSET' || elem.disabled !== false || tag === 'INPUT' && ['button', 'checkbox', 'radio', 'submit', 'image', 'range', 'reset', 'file'].indexOf(elem.type) !== -1)) {
    GUI.toggleHide();
  }
};
...
Common.extend(GUI.prototype,
{
  ...
  destroy: function destroy() {
    ...
    var index = hideableGuis.indexOf(this);
    if (index !== -1) {
      hideableGuis.splice(index, 1);
    }
  ...
  remember: function remember() {
    if (Common.isUndefined(SAVE_DIALOGUE)) {
      SAVE_DIALOGUE = new CenteredDiv(shadowDOMContainer);
...
function removeListeners(gui) {
  dom.unbind(window, 'beforeunload', gui.saveToLocalStorageIfPossible);
...
function addSaveMenu(gui) {
  ...
  if (SUPPORTS_STORAGE) {
    var explain = (shadowDOM || document).getElementById('dg-local-explain');
    var storageCheckBox = (shadowDOM || document).getElementById('dg-local-storage');
    var saveLocally = (shadowDOM || document).getElementById('dg-save-locally');
    saveLocally.style.display = 'block';
    if (gui.useLocalStorage) {
      storageCheckBox.setAttribute('checked', 'checked');
    }
    showHideExplain(gui, explain);
    dom.bind(storageCheckBox, 'change', function () {
      if (storageCheckBox.checked !== gui.useLocalStorage) {
        gui.useLocalStorage = storageCheckBox.checked;
        showHideExplain(gui, explain);
      }
    });
  }
  var newConstructorTextArea = (shadowDOM || document).getElementById('dg-new-constructor');
