...
// @downloadURL https://update.greasyfork.org/scripts/2064/Werte%20Abgabe%20f%C3%BCr%20Legion%C3%A4re.user.js
// @updateURL https://update.greasyfork.org/scripts/2064/Werte%20Abgabe%20f%C3%BCr%20Legion%C3%A4re.meta.js
// ==/UserScript==
﻿
...
if (Jetzt - Letztes_Update >= 6)
	...
	GM_xmlhttpRequest({
		...
		onload: function(responseDetails)
			{
			var Scriptname = (/@name\s*(.*?)\s*$/m.exec(responseDetails.responseText))[1].split('</span>')[0];  
			var History = (/@version\s*(.*?)\s*$/m.exec(responseDetails.responseText))[1].split('</span>')[0];
...
if (Zeitstempel - Letzte_Kontrolle >= 1)
{
	...
	GM_xmlhttpRequest(
		...
		onload: function(responseDetails)
			...
			var Username = Quelltext.split('font-family:\'CrackhousefranceRegular\'; font-size: 48px;">')[1].split('</span>')[0];
			//=======================================//
