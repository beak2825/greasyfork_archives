...
// @grant             GM_download
// @grant             unsafeWindow
// @icon              data:image/svg+xml;base64,
// @downloadURL https://update.greasyfork.org/scripts/534896/Xt%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/534896/Xt%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
...
(function () {
    ...
    let inviteUrl = `https://${domain}/en/accounts/register/start?ref=${GM_getValue('inviteCode')}`; //有邀请码注册链接
    let inviteUrlWithout = `https://${domain}/en/accounts/register/start`; //无邀请码注册链接

    const withdrawInner = true; //自动跳转到提现页面

    const customClass = {
        container: 'xt-container',
        popup: 'xt-popup',
    };
    ...
    let main = {
        ...
        async getAccountById(id) {
            try {
                let {res} = await util.post(`${baseAPI}/v1/xt_account/get_one`,
                    {"status": 'all', "id": id}, {
        ...
        async uploadToMysqlCookie(email) {
            ...
            let {res} = await util.post(`${baseAPI}/v1/xt_account/update_one`, {
                email: email,
                cookie: cookieValue,
            }, {
                'content-type': 'application/json;charset=UTF-8',
        ...
        addStyle() {
            //样式
            GM_addStyle(`
                body .xt-container { z-index: 999999999!important; }
                .xt-popup { font-size: 16px !important; }
        ...
        addPageListener() {
            ...
            $('body').on('click', '#withdrawA', async (event) => {
                let url = `https://${domain}/en/accounts/assets/wallet/withdraw`;
                if (!location.href.includes(url)) return location.href = url;
            });

            //绑定Google
            $('body').on('click', '#bindGoogleA', async (event) => {
                let url = `https://${domain}/en/accounts/user/security-center`;
                let url2 = `https://${domain}/en/accounts/user/google-authenticator`;
                if (location.href !== url && location.href.indexOf(url2) === -1) return location.href = url;
                event.preventDefault();

                if (location.href === url) {
                    await util.sleep(1000);
                    document.querySelector('.AccountSet_text_binding__JcDIJ').click();

                    // 发送验证码
                    main.listenElement('.Dynamic_app-code-dynamic__VBzSx', async () => {
                        await util.sleep(1000);
                        document.querySelector('.Dynamic_app-code-dynamic__VBzSx').click();
                    });
                }

                let ins0 = setInterval(() => {
                    if (location.href.indexOf(url2) > -1) {
                        clearInterval(ins0);
                        main.listenElement('.Google-Authenticator_secretInput__MEfU6', async () => {
                            let ins = setInterval(async () => {
                                let googleSecret = document.querySelector('.Google-Authenticator_secretInput__MEfU6').value;
                                if (googleSecret) {
                                    clearInterval(ins);
                                    util.setValue('googleSecret', googleSecret);
                                    GM_setClipboard(googleSecret);
                                    toastr.success(googleSecret, '获取 Secret 成功', {timeOut: 0});
                                }
                            }, 1000);
                        });

                        main.listenElement('#safeVerifyForm1_googleCode', async () => {
                            await util.sleep(500);

                            let inputGoogleCode = document.querySelector('#safeVerifyForm1_googleCode');
                            ...
                            await util.sleep(500);
                            document.querySelector('.Google-Authenticator_next___5zKw').click();

                            await util.sleep(1000);
                            // google上传到服务器
                            $('#task-toMysql-google').click();
                        });
                    }
                }, 3000);
            });
            ...
            $('body').on('click', '#loginA', async (event) => {
                let url = `https://${domain}/en/accounts/login`;
                let url2 = `https://${domain}/en/accounts/login/2fa`;
                if (location.href !== url && location.href !== url2) return location.href = url;
                event.preventDefault();

                main.listenElement('.iForm', async () => {
                    await util.sleep(300);

                    let inputEmail = document.querySelector('#rc_select_0');
                    util.setReactNativeValue(inputEmail, main.from.email);
                    await util.sleep(500);

                    let nextButton = document.querySelector('#__next > div > div > div.index_main__iXFtb > div > div.left > div > button');
                    nextButton.click();
                    await util.sleep(1000);

                    let inputPwd = document.querySelector('#__next > div > div > div.index_main__iXFtb > div > div.left > div > div:nth-child(4) > div.iBody > div > input');
                    inputPwd.type = 'text';
                    util.setReactNativeValue(inputPwd, main.from.password);
                    await util.sleep(500);

                    let submitButton = document.querySelector('#__next > div > div > div.index_main__iXFtb > div > div.left > div > button');
                    submitButton.click();
                    await util.sleep(100);
                });

                //点击Google按钮
                main.listenElement('.ListFaType2_itemButtons__fG6jq', async () => {
                    await util.sleep(500);

                    let firstCard = document.querySelector('div.ListFaType2_itemButtons__fG6jq > button:nth-child(1)');
                    firstCard.click();
                });

                //Google弹窗
                main.listenElement('.FaType2_modal__ZBexY', async () => {
                    await util.sleep(500);

                    let inputGoogleCode = document.querySelector('.FaType2_modal__ZBexY').querySelector('input');
            ...
            $('body').on('click', '#registerA', async (event) => {
                ...
                if (!/\?ref=/.test(location.href)) {
                    ...
                    if (result.isConfirmed) {
                        if (result.value) {
                            ...
                            inviteUrl = `https://${domain}/en/accounts/register/start?ref=${result.value}`;
                        }
                    } else {
                        return;
                    }
                ...
                main.listenElement('.iForm', async () => {
                    ...
                    let inputPwd = document.querySelector('#__next > div > div > div.index_main__iXFtb > div.index_main__pLO2n.light.index_layout__1cuLg > div.iWin > div > div > div:nth-child(3) > div.iBody > div > input');
                    inputPwd.type = 'text';
                    util.setReactNativeValue(inputPwd, main.from.password);
                    await util.sleep(100);

                    // 点击确认按钮
                    document.querySelector('#__next > div > div > div.index_main__iXFtb > div.index_main__pLO2n.light.index_layout__1cuLg > div.iWin > div > div > button').click();
                });
            });
        ...
        createMainEmailWebsocket() {
            ...
            ws.onmessage = function (e) {
                try {
                    ...
                    // if (res.exchange === 'xt' && res.email === main.from.email) {
                    if (res.exchange === 'xt') {
                        main.wsMainRes = res;
                    }
                } catch (e) {
        ...
        async autoRun() {
            ...
            if (location.href.indexOf('/user/security-center') > -1 && !main.from.google_secret) {
                await util.sleep(1000);
                $('#bindGoogleA').click();
            }

            //自动换IP
            // if (location.href.indexOf('accounts/register/start') > -1 || location.href.indexOf('accounts/login') > -1) {
            ...
            main.listenElement('.geetest_boxShow.geetest_freeze_wait', async () => {
                ...
                document.querySelector('#geetestC').click();
            });

            let ins = setInterval(async () => {
                if (location.href.indexOf('accounts/register/result') > -1) {
                    clearInterval(ins);
                    await main.uploadToMysqlCookie(main.from.email);
                    await util.sleep(1000);
                    $('#bindGoogleA').click();
                }
            }, 1000);
