// ==UserScript==
// @name        MaelstromTools Dev CCE
// @namespace   MaelstromTools
// @description C&C Tiberium Alliances - Just a set of statistics & summaries about repair time and base resources. Mainly for internal use, but you are free to test and comment it.
// @version     0.1.4.48.07 - cce
// @author      Maelstrom, HuffyLuf, KRS_L, Krisan and DLwarez, Chillchef
// @include     http*://prodgame*.alliances.commandandconquer.com/*/index.aspx*
// @include     http*://cncapp*.alliances.commandandconquer.com/*/index.aspx*
// @downloadURL https://update.greasyfork.org/scripts/37855/MaelstromTools%20Dev%20CCE.user.js
// @updateURL https://update.greasyfork.org/scripts/37855/MaelstromTools%20Dev%20CCE.meta.js
// ==/UserScript==
      ...
      function createMaelstromTools() {
        ...
        qx.Class.define("MaelstromTools.Language", {
          ...
          members: {
            ...
            loadData: function (language) {
              ...
              //var l = 0; // 0 = "de_DE" fest
              if (l < 0) {
                this.Data = null;
                return;
              }
              // upgrade-übersicht: UpgradePriorityGUI Z2730
              // produktion-übersicht : qx.Class.define("MaelstromTools.Production" Z1295
              this.Data = {};
              ...
              this.Data["Total / d"] = ["Gesamt / d", "Total / d", "Total / d", "Toplam / ??."][l];
              ...
              this.Data["Ø Level"] = ["Ø Stufe", "Ø Nível", "Ø Niveau", "Ø Seviye"][l];
              ...
              this.Data["City"] = ["Basis", "Base", "Base", "Sehir"][l];
          ...
          members: {
            ...
            initialize: function () {
              try {
                ...
                //Fenstergrößen siehe MaelstromTools.DefaultObject.openWindow 'alert' im 'beforeClose event'
                this.createNewWindow("MainMenu", "R", 125, 140, 120, 100, "B");
                this.createNewWindow("Production", "L", 120, 60, 1210, 820);
                this.createNewWindow("RepairTime", "L", 120, 60, 789, 436);
                this.createNewWindow("ResourceOverview", "L", 120, 60, 817, 336);
                this.createNewWindow("BaseStatusOverview", "L", 120, 60, 645, 396);
                this.createNewWindow("Preferences", "L", 120, 60, 440, 140);
                this.createNewWindow("UpgradePriority", "L", 120, 60, 1240, 480);
            ...
            addToDesktop: function (name, button) {
              try {
                ...
                app.getDesktop().add(button, {
                  ...
                  bottom: 104 - (42 * (desktopPosition - 1)) //bottom: 140 - (42 * (desktopPosition - 1))
                });
			...
			mcvResearchTimerLabel: null,
              ...
              try {
                ...
                this.mcvPopup.setCaption(Lang.gt("Next MCV") + " ($ " + MaelstromTools.Wrapper.FormatNumbersCompact(creditsNeeded) + ")");
                  ...
                  this.mcvCreditProcentageLabel.setTextColor("limeGreen");
                }
                  if (PercentageOfCredits < 100) {
                  ...
                  this.mcvResearchTimerLabel.setTextColor("limeGreen");
                }
                  if (PercentageOfResearchPoints < 100) {
        ...
        qx.Class.define("MaelstromTools.DefaultObject", {
          ...
          members: {
            ...
            openWindow: function (WindowName, WindowTitle) {
              try {
                var winSize = MaelstromTools.Base.getInstance().mWindows[WindowName];

                if (!this.Window) {
                  this.Window = new qx.ui.window.Window(WindowTitle).set({     // WINDOW-PROPS
                    resizable: true, 
                    showMaximize: false,
                    showMinimize: false,
                    allowMaximize: false,
                  ...
                  //this.Window.setLayout(new qx.ui.layout.VBox(1));
                  this.Window.setLayout(new qx.ui.layout.Dock());

                  this.Widget = new qx.ui.container.Composite(new qx.ui.layout.Grid());
                  ///this.Widget = new qx.ui.container.Composite(new qx.ui.layout.Dock());
                  this.Widget.setTextColor("white");
                  ...
                  var mem = MaelstromTools.LocalStorage.get(WindowName, {
                    w : winSize['w'],
                    h : winSize['h'],
                    x : winSize['x'],
                    y : winSize['y']
                  ...
                  this.Window.add(scroll, {edge:"center"});
                  this.Window.setOpacity(0);
                  this.Window.addListener("beforeClose", function(e) {
                    winSize['w'] = this.Window.getWidth();
                    winSize['h'] = this.Window.getHeight();
                    winSize['x'] = this.Window.getLayoutProperties().left;
                    winSize['y'] = this.Window.getLayoutProperties().top;
                    MaelstromTools.LocalStorage.set(WindowName, {
                      w : winSize['w'],
                      h : winSize['h'],
                      x : winSize['x'],
                      y : winSize['y']
                    });
                    //alert("w: " + String(winSize['w']) + "\r\nh: " + String(winSize['h']) ); //Fenstergröße einblenden
                    //e.preventDefault(); //standart close-event blocken
                    //this.Window.fadeOut();
                  }, this);
                ...
                //console.log(WindowName, MaelstromTools.Base.getInstance().mWindows);
                if (this.Window.isVisible()) {
                  this.Window.close();
                } else {
                  MT_Base.openWindow(this.Window, WindowName);
                  this.calc();
                  this.Window.setWidth(winSize['w']);
                  this.Window.setHeight(winSize['h']);
                  this.Window.moveTo(winSize['x'],winSize['y']);
        ...
        qx.Class.define("MaelstromTools.Production", {
          ...
          members: {
            RessiTech: null,
            ressiCounter: function (obCity, resTypeToCount, resName) {
                var retVal = 0;
                var techDn = "noDn";
                var lvl = 0;
                try
                {
                  for(var b in obCity.get_Buildings()['d'])
                  {
                    if(obCity.get_Buildings()['d'][b].get_CollectResourceType() === resTypeToCount)
                    {
                      retVal++;
                      techDn = obCity.get_Buildings()['d'][b].get_TechGameData_Obj()['dn'];
                      lvl += obCity.get_Buildings()['d'][b].get_CurrentLevel();
                      if(this.RessiTech === null || this.RessiTech === undefined) this.RessiTech = Object();
                      //this.RessiTech[resTypeToCount] = techDn;
                      this.RessiTech[resName] = techDn;
                    }
                  }
                }
                catch(ex)
                {
                  console.log("ressiCounter: " + ex.message);
                }
                var o = Object();
                o['count'] = retVal;
                o['lvl'] = lvl;
            ...
            updateCache: function (onlyForCity) {
              try {
                ...
                for (var cname in MT_Cache.Cities) {
                  ...
                  //this.Cache[cname]["CityObj"] = ncity;
                  var rc;
                  ...
                  rc = this.ressiCounter(ncity,ClientLib.Base.EResourceType.Tiberium,"Tiberium");
                  this.Cache[cname][MaelstromTools.Statics.Tiberium]["CollectBuildingCount"] = rc.count;
                  this.Cache[cname][MaelstromTools.Statics.Tiberium]["CollectBuildingLvlCount"] = rc.lvl;
                  ...
                  rc = this.ressiCounter(ncity,ClientLib.Base.EResourceType.Crystal,"Crystal");
                  this.Cache[cname][MaelstromTools.Statics.Crystal]["CollectBuildingCount"] = rc.count;
                  this.Cache[cname][MaelstromTools.Statics.Crystal]["CollectBuildingLvlCount"] = rc.lvl;
                  ...
                  rc = this.ressiCounter(ncity,ClientLib.Base.EResourceType.Power,"Power");
                  this.Cache[cname][MaelstromTools.Statics.Power]["CollectBuildingCount"] = rc.count;
                  this.Cache[cname][MaelstromTools.Statics.Power]["CollectBuildingLvlCount"] = rc.lvl;
                  ...
                  rc = this.ressiCounter(ncity,ClientLib.Base.EResourceType.Gold,"Dollar");
                  this.Cache[cname][MaelstromTools.Statics.Dollar]["CollectBuildingCount"] = rc.count;
                  this.Cache[cname][MaelstromTools.Statics.Dollar]["CollectBuildingLvlCount"] = rc.lvl;
            ...
            createProductionLabels2: function (rowIdx, colIdx, cityName, resourceType) {
              try {
                var colRed = "orangeRed";
                var colGreen = "springGreen";
                var colRed2 = "coral";

                var lbl;
                var cityCount = 0;
                var Totals = Object();
                Totals["Felder"] = 0; //sammler count
                Totals["Lvl"] = 0; //lvl count
                Totals["Delta"] = 0;
                Totals["ExtraBonusDelta"] = 0;
                Totals["POI"] = 0;
                Totals["Total"] = 0; // gesamt stunde
                Totals["Total2"] = 0; // gesamt tag
                Totals["Schnitt"] = 0; // gesamt tag sammler /felder
                Totals["SchnittLvl"] = 0;

                for (var cname in this.Cache) {
                  Totals["Felder"] += this.Cache[cname][resourceType]['CollectBuildingCount'];
                  Totals["Lvl"] += this.Cache[cname][resourceType]['CollectBuildingLvlCount'];
                ...
                Totals["Total2"] = Totals["Total"] * 24;
                Totals["Schnitt"] = ((Totals["Delta"] + Totals["ExtraBonusDelta"]) * 24) / Totals["Felder"];
                Totals["SchnittLvl"] = Totals["Lvl"] / Totals["Felder"];

                if (cityName == "-Total-") {

                  rowIdx++;
                  lbl = MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, MaelstromTools.Wrapper.FormatNumbersCompact(Totals['Felder']), 80, 'right', 'bold');
                  lbl.setToolTipText("Ø " + this.RessiTech[resourceType]+"-Count: " + String((Totals['Felder']/cityCount).toFixed(2)));
                  lbl = MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, parseFloat(Totals['SchnittLvl'].toFixed(2)), 80, 'right', 'bold');
                  lbl.setToolTipText("Ø " + this.RessiTech[resourceType]+"-Level");
                  lbl = MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, MaelstromTools.Wrapper.FormatNumbersCompact(Totals['Schnitt'] / Totals['SchnittLvl']), 80, 'right', 'bold', colGreen);
                  lbl.setToolTipText("Ø " + resourceType + "/" + this.RessiTech[resourceType]+"-Lvl/d");
                  lbl = MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, MaelstromTools.Wrapper.FormatNumbersCompact(Totals['Schnitt']), 80, 'right', 'bold', colGreen);
                  lbl.setToolTipText("Ø " + resourceType + "/" + this.RessiTech[resourceType]+"/d");
                  ...
                  lbl = MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, MaelstromTools.Wrapper.FormatNumbersCompact(Totals['Total2']), 80, 'right', 'bold', colGreen);
                  lbl.setToolTipText("Ø " + resourceType + "/Base/d: " + String(MaelstromTools.Wrapper.FormatNumbersCompact(Totals['Total2'] / cityCount)));

                } else if (cityName == "-Labels-") {
                  //console.log("-------->   lege Labels an!");
                  //console.log("rowIdx: " + String(rowIdx) + "   colIdx: " + String(colIdx) + "   cityName: " + cityName + "  resourceType: " + String(resourceType));
                  //console.log(this.RessiTech);
                  //console.log(this.Cache[cityName][resourceType]['CollectBuildingCount']);
                  MaelstromTools.Util.addImage(this.Widget, rowIdx++, colIdx, MaelstromTools.Util.getImage(resourceType));
                  //MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, this.Cache[1][resourceType]['CollectBuildingCount']['tech'], 100, 'left');
                  MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, this.RessiTech[resourceType], 100, 'left');
                  MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, "Ø Lvl " + this.RessiTech[resourceType], 100, 'left');
                  MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, "Ø/d/" + this.RessiTech[resourceType] + "-Lvl", 100, 'left');
                  MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, "Ø/d/" + this.RessiTech[resourceType], 100, 'left');
                  ...
                  MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, "Total / d", 100, 'left');
                  ...
                  MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, MaelstromTools.Wrapper.FormatNumbersCompact(cityCache[resourceType]['CollectBuildingCount']), 80, 'right'); //ANZAHL RESSI FELDER
                  var perFeld = ((cityCache[resourceType]['Delta'] + cityCache[resourceType]['ExtraBonusDelta']) * 24) / cityCache[resourceType]['CollectBuildingCount'];
                  var dsStufe = cityCache[resourceType]['CollectBuildingLvlCount'] / cityCache[resourceType]['CollectBuildingCount'];
                  var perStufe = perFeld/dsStufe;
                  MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, parseFloat(dsStufe).toFixed(2), 80, 'right');
                  lbl = MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, MaelstromTools.Wrapper.FormatNumbersCompact(perStufe), 80, 'right', null, (perStufe >= (Totals['Schnitt'] / Totals['SchnittLvl'])) ? colGreen : colRed);
                  lbl.setToolTipText(String(((perStufe*100)/(Totals['Schnitt'] / Totals['SchnittLvl'])).toFixed(2)) + " %");
                  lbl = MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, MaelstromTools.Wrapper.FormatNumbersCompact(perFeld), 80, 'right', null, (perFeld >= Totals["Schnitt"]) ? colGreen : colRed);
                  lbl.setToolTipText(String(((perFeld*100)/Totals["Schnitt"]).toFixed(2)) + " %");
                  ...
                  var proTag = (cityCache[resourceType]['Delta'] + cityCache[resourceType]['ExtraBonusDelta'] + cityCache[resourceType]['POI']) * 24;
                  lbl = MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, MaelstromTools.Wrapper.FormatNumbersCompact(proTag), 80, 'right', 'bold', (proTag >= (Totals["Total2"] / cityCount)) ? colGreen : colRed2);
                  lbl.setToolTipText(String(((proTag*100)/(Totals["Total2"] / cityCount)).toFixed(2)) + " %");
                }
                return rowIdx;
              } catch (e) {
            ...
            setWidgetLabels: function () {
              try {
                this.Widget.removeAll();
                //this.Widget.setBackgroundColor("black");
                this.Widget.setOpacity(0.9);
                this.Widget.setBackgroundColor("#404040");
                this.Widget.setMargin(0);
                this.Widget.setPadding(10);
                //this.Widget.setToolTipText("ToooolTip :)");

                //console.log("CityChache: ",this.Cache);

                var rowIdx = 1;
                var colIdx = 1;

                MaelstromTools.Util.addLabel(this.Widget, rowIdx, colIdx, "Base", 80, 'left', 'bold', "lightSkyBlue");
                MaelstromTools.Util.addLabel(this.Widget, rowIdx+1, colIdx, "Level", 80, 'left', 'bold', "lightSkyBlue");
                ...
                for (var cityName in this.Cache) {
                  rowIdx = 1;
                  cityCount++;
                  lbl = MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, cityName, 80, 'right', 'bold', "lightSkyBlue");
                  lbl.setToolTipText(this.Cache[cityName]['BaseLevel']);
                  MaelstromTools.Util.addLabel(this.Widget, rowIdx++, colIdx, this.Cache[cityName]['BaseLevel'], 80, 'right', 'bold', "lightSkyBlue");

                  baseLvlSum = baseLvlSum + parseFloat(this.Cache[cityName]['BaseLevel']);
                  ...
                  MaelstromTools.Util.addLabel(this.Widget, rowIdx+1, colIdx, " ... ", 80, 'right', 'bold', "transparent");//test

                  MaelstromTools.Util.addElement(this.Widget, rowIdx+2, colIdx++, MaelstromTools.Util.getAccessBaseButton(cityName));
                }

                rowIdx = 1;
                lbl = MaelstromTools.Util.addLabel(this.Widget, rowIdx, colIdx, "Total \t [" + String(cityCount) + "]", 80, 'right', 'bold', "lightSkyBlue ");
                lbl.setToolTipText("Cities: " + String(cityCount));
                lbl = MaelstromTools.Util.addLabel(this.Widget, rowIdx+1, colIdx, (baseLvlSum / cityCount).toFixed(2), 80, 'right', 'bold', "lightSkyBlue ");
                lbl.setToolTipText("Ø Base Lvl");
          ...
          members: {
            ...
            init: function () {
              ...
              try {
                ...
                // Nöööööö, ich will Tiberium !   ;P
                this.HT_TabView.setSelection([this.HT_TabView.getChildren()[0]]);
                this.HT_SelectedResourceType = ClientLib.Base.EResourceType.Tiberium;
              } catch (e) {
            ...
            createTable: function (eType) {
              try {
                ...
                this.HT_Models[eType].setColumns(["ID", Lang.gt("City"), Lang.gt("Type (coord)"), Lang.gt("to Level"), Lang.gt("Gain/h"), Lang.gt("Factor"), Lang.gt("Tiberium"), Lang.gt("Power"), Lang.gt("Tib/gain"), "ROI Tib. [d]", Lang.gt("Pow/gain"), "ROI Power [d]", Lang.gt("ETA"), Lang.gt("Upgrade"), "State"]);
                ...
                this.HT_Tables[eType].setColumnWidth(3, 70);
                this.HT_Tables[eType].setColumnWidth(4, 120);
                this.HT_Tables[eType].setColumnWidth(5, 60);
                this.HT_Tables[eType].setColumnWidth(6, 70);
                this.HT_Tables[eType].setColumnWidth(7, 70);
                this.HT_Tables[eType].setColumnWidth(8, 100);
                this.HT_Tables[eType].setColumnWidth(9, 90);
                this.HT_Tables[eType].setColumnWidth(10, 100);
                this.HT_Tables[eType].setColumnWidth(11, 90);
                this.HT_Tables[eType].setColumnWidth(12, 80);
                this.HT_Tables[eType].setColumnWidth(13, 45);
                this.HT_Tables[eType].setColumnWidth(14, 0);

                var tcm = this.HT_Tables[eType].getTableColumnModel();
                tcm.setColumnVisible(0, false);
                tcm.setColumnVisible(10, false);//power/zuwachs
                tcm.setColumnVisible(11, false);//power roi
                tcm.setColumnVisible(14, false);
                ...
                tcm.setDataCellRenderer(13, new HuffyTools.ImageRender(40, 20));
              } catch (e) {
            ...
            TabChanged: function (e) {                                               //TABELLENSPALTEN ANZEIGEN/AUSBLENDEN
              try {
                ...
                console.log(this);
                console.log(this[1]);
                console.log("tib: " + ClientLib.Base.EResourceType.Tiberium);
                console.log("pow: " + ClientLib.Base.EResourceType.Power);
                var tcm = this[0].HT_Tables[this[1]].getTableColumnModel();
                if(this[1] == ClientLib.Base.EResourceType.Tiberium){
                  tcm.setColumnVisible(8, true);
                  tcm.setColumnVisible(9, true);
                }
                else{
                  tcm.setColumnVisible(8, false);
                  tcm.setColumnVisible(9, false);
                }
                if(this[1] == ClientLib.Base.EResourceType.Power){
                  tcm.setColumnVisible(10, true);
                  tcm.setColumnVisible(11, true);
                }
                else{
            ...
            setWidgetLabels: function () {                                                   //HIIIIIER BERECHNUNG FÜR ROI ??????????????????????
              try {
                ...
                for (var eResourceType in UpgradeList) {
                  ...
                  for (var mCity in UpgradeList[eResourceType]) {
                    for (var mBuilding in UpgradeList[eResourceType][mCity]) {
                      ...
                      rowData.push([UpItem.ID, mCity, sType, UpItem.Level, UpItem.GainPerHour, UpItem.Ticks, iTiberiumCosts, iPowerCosts, iTiberiumPerGain, roiTib, iPowerPerGain,  roiPow, sETA, img, iState]);
                    }
                  }
