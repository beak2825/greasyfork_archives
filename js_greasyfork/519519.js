...
// @license      Botir_Soliev

// @description Theme Changer
// @downloadURL https://update.greasyfork.org/scripts/519519/%21%20%D0%94%D0%BB%D1%8F%20Logs%20BLACK-RUSSIA.user.js
// @updateURL https://update.greasyfork.org/scripts/519519/%21%20%D0%94%D0%BB%D1%8F%20Logs%20BLACK-RUSSIA.meta.js
// ==/UserScript==
(function () {
    'use strict';
    ...
    function createStyleButton(element) {
        ...
        styleToggle.textContent = 'Настройки';
    ...
    function createStyleContainerBg(element) {
        ...
        styleTitleBadge.textContent = 'Настройки';
        ...
        const fonts = ['Roboto, sans-serif', 'Times New Roman', 'Roboto', 'Trebuchet MS', 'Arial', 'Courier', 'Brush Script MT', 'Segoe UI', 'Arial Narrow', 'Geneva', 'Ubuntu', 'Lato', 'Open Sans', 'Montserrat',
'Comfortaa', 'Console Unicode', 'Microsoft Sans Serif', 'Sylfaen', 'Tahoma', 'Georgia', 'Verdana', 'Courier New', 'Palatino', 'Garamond', 'Georgia', 'Comic Sans MS', 'Arial Black', 'Impact', 'Lucida Console', 'Lucida Sans',
'Optima', 'Gill Sans', 'Helvetica', 'Futura', 'Arial Rounded MT Bold', 'Century Gothic', 'Frank Ruhl Libre', 'Chalkboard', 'Copperplate', 'Corbel', 'Comfortaa', 'Fira Sans', 'Marmelad', 'Montserrat', 'Neucha', 'Play', 'Roboto', 'Sofia Sans', 'Arial', 'Ubuntu', 'Bad Script', 'Comfortaa', 'Fira Sans', 'Marmelad', 'Montserrat', 'Neucha', 'Play', 'Roboto', 'Sofia Sans', 'Ubuntu'];
        fonts.forEach(font => {
...
const colorCodes = {
    'White': '#FFFFFF',
    'Fuchsia': '#FF00FF', // Добавлено
    'Purple': '#800080', // Добавлено
    'Maroon': '#800000', // Добавлено
    'Red': '#FF0000',
    'Lime': '#00FF00',
    'Olive': '#808000',
    'Yellow': '#FFFF00',
    ...
    'Ярко-желтый': '#FFB300',
        ...
        bugReportTG.href = 'https://t.me/botsol'
        ...
        bugReportVK.href = 'https://vk.com/botsol'
        ...
        bugReportGF.href = 'https://greasyfork.org/ru/scripts/519519-для-logs-br/feedback'
...
function formatNumbersInTable() {
    ...
    tableCells.forEach(cell => {
        if (cell.dataset.formattedByScript) return; // Если уже обработан, пропускаем
        ...
        if (!isNaN(text.replace(/,/g, ''))) {
            ...
            cell.dataset.formattedByScript = "true"; // Помечаем как обработанный
        }
    });
}
...
function darkenColor(hexCode, percent) {
    ...
    const darkColor80 = darkenColor(color, 80); // 80% darker
    const darkColor50 = darkenColor(color, 50); // 50% darker
    const darkColor20 = darkenColor(color, 20); // 20% darker
...
.table>:not(:last-child)>:last-child>* {
    color: ${color} !important;
    text-shadow: 0px 0px 2px ${color} !important;
    background-color: ${darkenColor(color, 80)} !important;
    border: 1px solid ${darkenColor(color, 60)} !important;
    border-bottom: 1px solid ${color} !important;
    transition: all 0.3s ease !important;
}

/* Hover-эффект для строк */
#log-table[data-v-2d76ca92]>:not(:last-child)>:last-child>*:hover,
.table>:not(:last-child)>:last-child>*:hover {
    background-color: ${darkenColor(color, 70)} !important;
    text-shadow: 0px 0px 4px ${color} !important;
}

#log-table[data-v-2d76ca92] .first-row[data-v-2d76ca92] td[data-v-2d76ca92] {
    border: 1px solid ${color} !important;
    text-align: center;
    background: ${darkenColor(color, 90)};
    color: ${color};
...
#log-table[data-v-2d76ca92] .second-row[data-v-2d76ca92] td[data-v-2d76ca92] {
    color: ${color} !important;
    text-shadow: 0px 0px 2px ${color} !important;
    background-color: ${darkenColor(color, 95)} !important; /* Темный вариант основного цвета */
}
                .form-control {
                    color: ${color} !important;
                    border: 1px solid ${color} !important;
    ...
    function applyTextGradient() {
        ...
        textGradient.textContent = `
                ...
                .td-category[data-v-2d76ca92] a[data-v-2d76ca92] {
                    background: linear-gradient(45deg, #FF0000, #00FF00, #00ccff);
    ...
    bodyStyle.textContent = `
        ...
        .navbar-dark .navbar-nav .nav-link {
            color: #ff0000;
            text-shadow: 0px 0px 10px #0d1113;
        }

        body {
            background-color: #0d1113; /* Добавлено */
            background-size: 100
        }

        /* Остальная часть стилей */
    `;
    ...
    function applyBodyStyle() {
        ...
        .td-player-name[data-v-2d76ca92] a[data-v-2d76ca92] {
            ...
            font-weight: 400;
            text-decoration: none;
            text-shadow: 0px 0px 0px #ffb0b0;
            padding-right: 3px;
        }
...
function replaceSpinnerImage() {
    ...
    if (!spinnerElement) return; // Если элемент не найден, выходим
