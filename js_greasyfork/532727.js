...
// @downloadURL https://update.greasyfork.org/scripts/532727/MWITools%20with%20Realtime%20Market.user.js
// @updateURL https://update.greasyfork.org/scripts/532727/MWITools%20with%20Realtime%20Market.meta.js
// ==/UserScript==
...
(() => {
    ...
    async function handleTooltipItem(tooltip) {
        ...
        if (settingsMap.itemTooltip_prices.isTrue) {
            ...
            let priceObj = marketJson?.marketData[itemHrid]?.[0];
            ask = marketJson?.marketData[itemHrid]?.[0].a;
            bid = marketJson?.marketData[itemHrid]?.[0].b;
            updateTime = marketJson?.marketData[itemHrid]?.[0].time;
            appendHTMLStr += `
        <div style="color: ${SCRIPT_COLOR_TOOLTIP};">${isZH ? "最新价: " : "Latest price: "}${numberFormatter(ask)} / ${numberFormatter(
                bid
            )} (${ask && ask > 0 ? numberFormatter(ask * amount) : ""} / ${bid && bid > 0 ? numberFormatter(bid * amount) : ""})<br/>[${updateTime ? new Date(updateTime * 1000).toLocaleString() : "Not Updated"}]</div>
        `;
        }
