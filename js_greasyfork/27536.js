// ==UserScript==
// @name            Maximize Netvibes Mod
// @description	    Hides netvibes top bar and bottom with modified icon location.
// @include         http://www.netvibes.com/*
// @include         https://www.netvibes.com/*
// @version 0.0.2
// @namespace https://greasyfork.org/users/814
// @downloadURL https://update.greasyfork.org/scripts/27536/Maximize%20Netvibes%20Mod.user.js
// @updateURL https://update.greasyfork.org/scripts/27536/Maximize%20Netvibes%20Mod.meta.js
// ==/UserScript==
...
//Mod by:          Bunta
//Updated:         10/01/2013
//Description:     Mod alters magnifying glass location to appear in tab bar to the left of the new tab '+' icon.

//Mod by:          DnAp
//Updated:         20/02/2017
//Description:     Hide readed news
(function()
{
    ...
    var el = document.getElementById("divTabs");
    ...
    var fHideRead = function(){
        if(hideRead){
            var sheet = document.createElement('style');
            sheet.innerHTML = "div.nv-item-renderer.no-tagging.read:not(.open){display:block;}";
            document.body.appendChild(sheet);
            //nv-line-item-renderer nv-line-item-renderer-root nv-item-renderer nv-item-renderer-root no-actions read no-tagging
            //nv-line-item-renderer nv-line-item-renderer-root nv-item-renderer nv-item-renderer-root no-actions no-tagging
        }else{
            var sheet = document.createElement('style');
            sheet.innerHTML = "div.nv-item-renderer.no-tagging.read:not(.open){display:none;}";
            document.body.appendChild(sheet);
        }
        hideRead = !hideRead;
    };
    dv.addEventListener("click", fHideRead, true);
    dv.style.cursor = "pointer";
    /*dv.style.textDecoration = "none";
    ...
    dv.style.marginTop = "4px";
    dv.style.marginRight = "4px";
    dv.id = "magnifier1";
    dv.calss = "header-button uwa-icon";
    dv.src = 'https://cdn4.iconfinder.com/data/icons/gnome-desktop-icons-png/PNG/64/Gnome-Mail-Unread-64.png';
    
    var f = function(){
        el = document.getElementsByClassName("nv-item-list-renderer-modal-header");
        if(el.length == 0) {
            setTimeout(f, 300);
        }else{
            el[0].appendChild(dv);
