...
// @downloadURL https://update.greasyfork.org/scripts/497798/Tech%3C3.user.js
// @updateURL https://update.greasyfork.org/scripts/497798/Tech%3C3.meta.js
// ==/UserScript==
(function () {
	'use strict';
	...
	const buttons = [
	{
		title: 'Приветствие',
		content:
		'[SIZE=4][FONT=Trebuchet MS][CENTER]Приветствую, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	...
	},
{
	...
	},
	{
		...
		content:
		'[SIZE=4][FONT=Trebuchet MS][CENTER]Доброе время суток, уважаемый[B] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	...
	},
{
	...
	},
	{
		...
		'[SIZE=4][FONT=Trebuchet MS][CENTER]Запуская игру, пользователь <u>соглашается с правилами её использования</u>, которые изложены в <u>Пользовательском соглашении</u>, что по смыслу ст. 435 и 438 Гражданского кодекса РФ является принятием (акцептом) оферты Компании https://blackrussia.online/oferta.php, а равно заключением договора.<br><br>Согласно Пользовательскому соглашению <u>«Внутриигровая валюта» – виртуальная внутриигровая валюта</u>, являющаяся неактивированными данными и командами, которая не имеет денежной стоимости и не подлежит денежной оценке, хотя и имеет цену на момент приобретения.<br>' +
		'[SIZE=4][FONT=Trebuchet MS][CENTER]Согласно п. Политика возврата платежей, Пользовательского соглашения денежные средства за внутриигровые товары не подлежат возврату с момента появления Внутриигровой валюты на счете аккаунта.<br>' +
		'[SIZE=4][FONT=Trebuchet MS][CENTER]Согласно п. Политика возврата платежей, Пользовательского соглашения пользователь самостоятельно следит за безопасностью своего аккаунта, сам несет ответственность за все действия, которые выполняются в сервисах с помощью его аккаунта, а также в нем самом.<br>' +
		...
		'[SIZE=4][FONT=Trebuchet MS][CENTER]Компания /u>не несет ответственности за возможный ущерб третьим лицам</u>, причиненный в результате использования пользователем не принадлежащих ему средств оплаты.<br><br> ' +
		'[SIZE=4][FONT=Trebuchet MS][CENTER]</u>Совершая покупки внутри игры</u>, а также предоставляя платежную информацию, Вы гарантируете, что являетесь законным владельцем платёжного средства и аккаунта, связанного с данным платежом.<br><br>' +
		'[SIZE=4][FONT=Trebuchet MS][CENTER]Все действия с картой считаются совершенными с Вашего ведома и согласия, то есть лично владельцем карты.<br><br>' +
		'[SIZE=4][FONT=Trebuchet MS][CENTER]Кроме того, в соответствии законодательством РФ родители несут имущественную ответственность по сделкам малолетнего, в том числе по сделкам, совершенным им самостоятельно.<br>' +
		'[SIZE=4][FONT=Trebuchet MS][CENTER]Таким образом, если Вы являетесь законными представителем, Вы отвечаете за действия ребёнка внутри игры. Компания не может отслеживать действия несовершеннолетнего и нести за них ответственность.<br><br>' +
		'[SIZE=4][FONT=Trebuchet MS][CENTER]Таким образом основания для возврата денежных средств отсутствуют.<br>' +
	...
	},
{
    ...
    addButton('ОТВЕТЫ', 'selectAnswer1', 'border-radius: 20px; margin-right: 5px; border: 2px solid; border-color: rgb(0, 250, 154, 0.9);');
	...
	function addButton(name, id, hex = "black") {
		...
		return `<div class="select_answer">${buttons
		  .map(
			(btn, i) =>
			  ...
			  `rippleButton" style="margin:5px; background-color: ${btn.color || "black"}"><span class="button-text">${btn.title}</span></button>`,
		  )
		  .join('')}</div>`;
		}
