...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/402082/%E5%AD%A6%E4%B9%A0%E9%80%9A%E5%88%B7%E8%AF%BE%EF%BC%88JS%E8%84%9A%E6%9C%AC%E6%9C%BA%E5%99%A8%E4%BA%BA%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/402082/%E5%AD%A6%E4%B9%A0%E9%80%9A%E5%88%B7%E8%AF%BE%EF%BC%88JS%E8%84%9A%E6%9C%AC%E6%9C%BA%E5%99%A8%E4%BA%BA%EF%BC%89.meta.js
// ==/UserScript==
...
if (url == '/mycourse/studentstudy') {
    ...
    _self.layui.use('layer', function() {
        this.layer.open({content: '邀请您关注公众号：我安静来存活，来获取内测版以及更多其他内容！', title: '超星网课助手提示', btn: '已关注', offset: 't', closeBtn: 0});
    });
    ...
    GM_xmlhttpRequest({
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if (obj.code) {
                    setting.div.children('div:eq(0)').text('操作正常...');
                    ...
                    setting.div.children('div:eq(0)').html(obj.data || setting.over + '出现异常，正在重试...');
                }
                setting.div.children('span').html(obj.msg || '');
            } else if (xhr.status == 403) {
                var html = xhr.responseText.indexOf('{') ? '出现异常，建议查阅公众号新闻' : $.parseJSON(xhr.responseText).data;
                setting.div.children('div:eq(0)').data('html', html).siblings('button:eq(0)').click();
            } else {
                setting.div.children('div:eq(0)').html(setting.over + '出现异常，正在重试...');
            }
        },
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').html(setting.over + '出现异常，建议查阅公众号新闻，正在重试...');
        }
    });
}
