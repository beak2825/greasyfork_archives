...
// @author       You
// @license MIT
// @match        https://x.yupoo.com/gallery/*
// @match        https://x.yupoo.com/gallery
// @match        https://x.zhidian-inc.cn/gallery/*
// @match        https://x.zhidian-inc.cn/gallery
// @icon         https://www.google.com/s2/favicons?sz=64&domain=yupoo.com
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/483818/yuppo%20copy.user.js
// @updateURL https://update.greasyfork.org/scripts/483818/yuppo%20copy.meta.js
// ==/UserScript==

(function () {
  'use strict';

  const defaultRate = 6.66;
  const defaultShippingCost = 86;
  const defaultProfit = 0.25;
  ...
  function loadAssets() {
    ...
    script.src =
      'https://cdn.jsdelivr.net/npm/vue@3.4.4/dist/vue.global.min.js';
    document.documentElement.appendChild(script);
    let link = document.createElement('link');
    link.setAttribute('rel', 'stylesheet');
    link.href =
      'https://cdn.jsdelivr.net/npm/element-plus@2.4.4/dist/index.min.css';
  ...
  function loadVue() {
    document.querySelector(
      '#main > main > form > div:nth-child(3) > div'
    ).style.display = 'none';

    let text = `<div id="app" style="position: absolute;top: -10px;left: 112px;width: 378px;">
           <el-tree-select
                      ref="tree"
                      v-model="value"
                      :data="data"
    ...
    const father = document.querySelector(
      '#main > main > form > div:nth-child(3)'
    );
    father.append(el);
    ...
    const App = {
      ...
      watch: {
        value() {
          ...
          if (node.parentId) {
            cateIds = node.parentId;
            childCateIds = node.value;
          } else {
            cateIds = node.value;
          }
        },
  ...
  function checkDem(str, str2) {
    let dem = str.match(/(?<=(size|尺寸|码数).*?)(\w+.*\w+)/i);
    if (dem) {
      return dem[0];
    }
    dem = str2.match(/(?<=(size|尺寸|码数).*?)(\w+.*\w+)/i);
    return dem ? dem[0] : 'none';
  }

  function setText(node, dem) {
    const text = `Code: \nWeight(Volume): \nDIM(Size): ${dem}\nQuality: normal`;
    node.value = text;
  }
    ...
    function getPrice(cost, weight) {
      const price = (cost / (1 - defaultProfit) + weight * defaultShippingCost) / defaultRate;
      return Math.ceil(price);
    }

  function setContent(des, name) {
    const dem = checkDem(name.value, des.value);
    setText(des, dem);
    des.onblur = function blue() {
      if (name.value.match(/\$\d+/)) {
        return;
      }
      let price = des.value.match(/Code:\s*[a-zA-Z]+(\d+)/);
      if (price) {
        price = price[1];
      }
      let weight = des.value.match(
        /.*\n*Weight\(Volume\):\s*(\d{1,2}\.?\d{0,2})/
      );
      if (weight) {
        weight = weight[1];
      }
      price = getPrice(+price, +weight);
      name.value = `$${price} ${name.value}`;
    };
  }

  let des, name, cateIds, childCateIds, catData;
  ...
  if (path === '/gallery') {
    window.onload = function () {
      const createBtn = document.querySelector(
        '#galleryPage > div.Tabs__sticky > div.toolbar__toolbar > button:nth-child(3)'
      );
      createBtn.onclick = function () {
        setTimeout(() => {
          des = document.querySelector(
            '#galleryPage > div.Modal__backdrop.undefined.backdrop.Modal__noafter.display > div > div.undefined.Modal__children > form > div.undefined.false.Input__tdtop.Input__main.gallery__input.false > div > textarea'
          );
          name = document.querySelector(
            '#galleryPage > div.Modal__backdrop.undefined.backdrop.Modal__noafter.display > div > div.undefined.Modal__children > form > div:nth-child(1) > div > input'
          );
          setContent(des, name);
        }, 100);
      };
    ...
    window.onload = function onLoad() {
      setTimeout(() => {
        ...
        des = document.querySelector('textarea#t_6');
        if (des) {
          name = 'input#t_5';
        } else {
          name = 'input#t_4';
          des = document.querySelector('textarea#t_5');
        }
    ...
    ajaxHooker.hook(request => {
      if (request.url === '//x.yupoo.com/api/albums' || request.url === '//x.zhidian-inc.cn/api/albums') {
        const data = JSON.parse(request.data);
        data.cateIds = cateIds + '' || '';
        if (childCateIds) {
          data.childCateIds = childCateIds + '';
        }
        request.data = JSON.stringify(data);
      }

      if (
        request.url === '//x.yupoo.com/api/category?_username=cassieinspried' || request.url === '//x.zhidian-inc.cn/api/category?_username=cassieinspried'
      ) {
        request.response = res => {
          const categories = JSON.parse(res.responseText).data;
          catData = categories
            .map(category => {
              const res = {
                value: category.id,
              ...
              if (category.child && category.child.length) {
                res.disabled = true;
                res.children = category.child.map(({ name, id, parentId }) => ({
                  value: id,
                  label: name,
                  parentId,
                }));
