...
// @downloadURL https://update.greasyfork.org/scripts/479949/%E6%90%9C%E7%B4%A2%E8%BE%BE%E4%BA%BA.user.js
// @updateURL https://update.greasyfork.org/scripts/479949/%E6%90%9C%E7%B4%A2%E8%BE%BE%E4%BA%BA.meta.js
// ==/UserScript==
...
(function() {
    ...
    let div = `
        <div class="cyOperate">
            <div class="cyInp">
                <input id="cyKeyword" placeholder="请输入搜索关键词"/>
                <input id="cyCount" placeholder="请输入搜索所需数量（整数）"/>
            </div>
            <div class="cyBtn">
                <button>搜索</button>
            </div>
                ...
                success: function (res) {
                    ...
                    if(res.user_list == undefined){
                        ...
                        for(let i of res.user_list){
                            let dyUid = i.user_info.sec_uid
                            // console.log(dyUid)
                            linkArr.push("https://www.douyin.com/user/"+dyUid)
                        }
                        // console.log(linkArr)

                        let urlData = {
                            platform_id: 1,
                            search_keyword: $("#cyKeyword").val(),
                            links: linkArr
                        }

                        req("https://spider.oa.cyek.com/script/addSearchKeyword",JSON.stringify(urlData),function(res){
                            if(has_more == 1 && currentCount < getCount){
                                setTimeout(function(){
                                    getDyData()
                                },1000)
                            }else{
                        ...
                        // $.ajax({
                        //     type: "post",
                        //     url: "https://spider.oa.cyek.com/script/addSearchKeyword",
                        //     data: {
                        //         platform_id: 1,
                        //         search_keyword: $("#cyKeyword").val(),
                        //         links: linkArr
