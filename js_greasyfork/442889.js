...
// @version      4.23
// @namespace    渔
// @description  开启即自动切换旧版，多服务器，题库高精准再度提升
// @author       渔
// @match        *://*.chaoxing.com/*
// @match        *://*.edu.cn/*
// @match        *://*.nbdlib.cn/*
...
// @downloadURL https://update.greasyfork.org/scripts/442889/%E8%B6%85%E6%98%9F%E6%98%9F%E7%AD%94%E9%A2%98.user.js
// @updateURL https://update.greasyfork.org/scripts/442889/%E8%B6%85%E6%98%9F%E6%98%9F%E7%AD%94%E9%A2%98.meta.js
// ==/UserScript==
...
var setting = {
    ...
    , rate: '0' // 视频播放默认倍率，参数范围0∪[0.0625,16]，'0'为秒过，倍速功能已恢复，请查看教程使用，然后修改这儿的速度
            ...
            if (xhr.status == 200) {
                ...
                if (is_bind){
                    _self.layui.use('layer', function () {
                        this.layer.open({ content: '拖动进度条、倍速播放、秒过会导致不良记录!<br>题库在慢慢补充，搜不到的题目系统会尽快进行自动补充!', title: '<span style="color: #f73131;"><strong>挂机助手</strong></span>提示', btn: ['确定'], offset: 't', closeBtn: 0});
                    });
                }else{
                    _self.layui.use('layer', function () {
        this.layer.open({ content: '拖动进度条、倍速播放、秒过会导致不良记录!<br>题库在慢慢补充，搜不到的题目系统会尽快进行自动补充!<br>绑定公众号可以获取精准查询结果和倍速功能', title: '<span style="color: #f73131;"><strong>挂机助手</strong></span>提示', btn: ['前往绑定','不需要'], offset: 't', closeBtn: 0,
...
function findAnswer() {
    ...
    GM_xmlhttpRequest({
        ...
        data: 'question=' + encodeURIComponent(question)+'&options='+encodeURIComponent(options) + '&cx_id=' + ansertype+ '&html=' + encodeURIComponent($TiMu.html())+ '&url=' + encodeURIComponent(top.document.URL)+'&course='+encodeURIComponent(course)+'&version=4.23',
        timeout: setting.time,
        onload: function (xhr) {
            if (!setting.loop) {
                ...
                setting.div.children('div:eq(0)').html('题库异常,可能被恶意攻击了...请等待恢复或者自行替换代码的73和74行');
            }
        },
        ontimeout: function () {
...
function submitAnswer($job, data) {
    ...
    data.length && GM_xmlhttpRequest({
        ...
        + '&fid=' + fid
        + '&url=' + encodeURIComponent(top.document.URL)+'&version=4.23'
    });
    //$('#right').click()
    //$('#right2').click()
