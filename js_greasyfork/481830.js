...
// @downloadURL https://update.greasyfork.org/scripts/481830/Dead%20Frontier%20-%20Selling%20Lists.user.js
// @updateURL https://update.greasyfork.org/scripts/481830/Dead%20Frontier%20-%20Selling%20Lists.meta.js
// ==/UserScript==
...
(function() {
	...
	// User Options
	const show_additional_place_price = true; // Shows additional place price.
	...
	function bulkPostSalesKeys() {
		...
		market_price.style.setProperty("position", "absolute");
		market_price.style.setProperty("top", "23px");
		market_price.style.setProperty("left", "230px");
		market_price.innerHTML = "Market Price";
		keys.appendChild(market_price);
		const undercut_price = document.createElement("span");
		undercut_price.style.setProperty("position", "absolute");
		undercut_price.style.setProperty("top", "23px");
		undercut_price.style.setProperty("left", "373px");
		undercut_price.innerHTML = "Undercut";
		keys.appendChild(undercut_price);
		const count = document.createElement("span");
		count.style.setProperty("position", "absolute");
		count.style.setProperty("top", "23px");
		count.style.setProperty("left", "444px");
		count.innerHTML = "Qty.";
		keys.appendChild(count);
		return keys;
	}
	...
	function bulkCancelSalesKeys() {
		...
		market_price.style.setProperty("position", "absolute");
		market_price.style.setProperty("top", "23px");
		market_price.style.setProperty("left", "225px");
		market_price.innerHTML = "Market Price";
		keys.appendChild(market_price);
		const my_price = document.createElement("span");
		my_price.style.setProperty("position", "absolute");
		my_price.style.setProperty("top", "23px");
		my_price.style.setProperty("left", "365px");
		my_price.innerHTML = "My Price";
		keys.appendChild(my_price);
		const marketplace_position = document.createElement("span");
		marketplace_position.style.setProperty("position", "absolute");
		marketplace_position.style.setProperty("top", "23px");
		marketplace_position.style.setProperty("left", "459px");
		marketplace_position.innerHTML = "Place";
		keys.appendChild(marketplace_position);
		const qty = document.createElement("span");
		qty.style.setProperty("position", "absolute");
		qty.style.setProperty("top", "23px");
		qty.style.setProperty("left", "510px");
		qty.innerHTML = "Qty.";
		keys.appendChild(qty);
		return keys;
	}
	...
	function bulkPostSalesNavigation() {
		const button = document.createElement("button");
		button.innerHTML = "Bulk Sell >>";
		button.style.setProperty("position", "absolute");
		button.style.setProperty("top", "5px");
		button.style.setProperty("left", "580px");
		return button;
	}

	function postSalesNavigation() {
		const button = document.createElement("button");
		button.innerHTML = "<< Post Sales";
		button.style.setProperty("position", "absolute");
		button.style.setProperty("top", "5px");
	...
	function bulkCancelSalesNavigation() {
		const button = document.createElement("button");
		button.innerHTML = "Bulk Cancel >>";
		button.style.setProperty("position", "absolute");
		button.style.setProperty("top", "5px");
		button.style.setProperty("left", "570px");
		return button;
	}

	function cancelSalesNavigation() {
		const button = document.createElement("button");
		button.innerHTML = "<< Cancel Sales";
		button.style.setProperty("position", "absolute");
		button.style.setProperty("top", "5px");
	...
	function countDatum(n) {
		const count = document.createElement("td");
		count.innerHTML = n.toString();
		count.style.setProperty("padding-left", "4px");
		count.style.setProperty("padding-top", "4px");
		count.style.setProperty("padding-bottom", "4px");
		count.style.setProperty("width", "25px");
	...
	function updatePostSales(table, player_items, market_items, cache, requested_place, requested_undercut, requested_datum) {
		...
		.then(function() {
			...
			for (const [slot, item] of player_items.inventoryItems()) {
				...
				const item_market_data = is_renamed ? cache.getItemMarketEntriesByRename(item.properties.get("rename")) : cache.getItemMarketEntriesByType(item.base_type);
			...
			headers.forEach((header) => header.style.setProperty("display", posting_item_category_data.get(header.children[0].innerHTML).length === 0 ? "none" : "block"));
			posting_item_category_data.forEach((category_data) => category_data.sort(sortingFunction));
			headers.forEach((header) => header.after(...posting_item_category_data.get(header.children[0].innerHTML).map((data) => data.element)));
			setLoadingHidden();
		});
	}

	function updateBulkPostSales(table, player_items, market_items, cache, requested_place, requested_undercut, requested_datum) {
		setLoadingVisible();
		return market_items.requestSelling()
		.then(function() {
			table.replaceChildren(); // Remove children.
			...
			const nselling = Array.from(market_items.sellingEntries()).length;
			const selling_list_full = nselling === parseInt(userVars["DFSTATS_df_invslots"]);
			// Making item groups for multiple selling.
			...
			for (const [slot, item] of player_items.inventoryItems()) {
				...
				const full_type = item.full_type;
				const quantity = item.quantity;
				const key = full_type + "_" + quantity.toString();
				if (!item_groups.has(key)) {
					item_groups.set(key, {
						items: [item],
						slots: [slot],
						count: 1
					...
					const group = item_groups.get(key);
					group.items.push(item);
					group.slots.push(slot);
					group.count += 1;
				}
			}
			for (const group of item_groups.values()) {
				...
				// Item name
				const name = itemNameDatum(displayName(item));
				name.style.setProperty("width", "182px");
				// Preparing price data
				let market_price_text = "";
				let undercut_price_text = "";
				let additional_place_market_price_text = "N/A";
				...
				function postSales(price) {
					...
					for (let i = 0; i < group.slots.length; i++) {
						current_promise = current_promise.then(function() {
							...
							const is_last_item = i === group.slots.length - 1;
							const update = is_last_item ? UiUpdate.YES : UiUpdate.NO;
							setLoadingText("Loading...<br>(Selling " + (i + 1).toString() + " of " + group.slots.length.toString() + ")");
							return market_items.sellInventoryItem(slot, item, price, update);
						});
					}
					return current_promise
					.then(function() {
						updateBulkPostSales(table, player_items, market_items, cache, requested_place, requested_undercut, requested_datum);
						setLoadingHidden();
						previous_sale_prices.set(item.full_type, price);
						previous_sale_information.set(item.full_type, {quantity: isStackable(item) ? item.quantity : 1, price: price})
					});
				...
				const undercut_price = undercutPriceDatum(undercut_price_text);
				// Count
				const count = countDatum(group.count);
				// Undercut button
				const undercut_button = undercutButton();
				undercut_button.addEventListener("mouseenter", function(e) {
					tr.style.setProperty("font-weight", "bold");
				...
				undercut_button.addEventListener("click", function(e) {
					if (suggested_undercut_price < scrap_value && requested_datum !== "scrap") {
						...
						setWarningVisible(message, function() {
							setWarningHidden();
							postSales(suggested_undercut_price);
						}, setWarningHidden);
					} else {
						postSales(suggested_undercut_price);
					}
				...
				post_sale_button.addEventListener("click", function(e) {
					...
					setPricePromptVisible(message, function(value) {
						...
						if (scrap_value > my_price && requested_datum !== "scrap") { // Scrap value is greater than proposed price.
							...
							setWarningHidden();
							return postSales(my_price);
						}
					}, setPricePromptHidden, initial_value);
				});
				const enough_market_space = market_items.availableSellingEntries() >= group.count;
				if (!enough_market_space) {
					undercut_button.disabled = true;
					post_sale_button.disabled = true;
				}
				if (suggested_undercut_price !== null && scrap_value > suggested_undercut_price && requested_datum !== "scrap") {
	...
	function updateCancelSales(table, market_items, cache) {
		...
		.then(function() {
			...
			headers.forEach((header) => header.style.setProperty("display", selling_item_category_data.get(header.children[0].innerHTML).length === 0 ? "none" : "block"));
			selling_item_category_data.forEach((category_data) => category_data.sort(sortingFunction));
			headers.forEach((header) => header.after(...selling_item_category_data.get(header.children[0].innerHTML).map((data) => data.element)));
			setLoadingHidden();
		});
	}

	function updateBulkCancelSales(table, market_items, cache, player_items) {
		setLoadingVisible();
		market_items.requestSelling()
		.then(function() {
			table.replaceChildren(); // Remove children.
			...
			selling_categories.forEach((category) => selling_item_category_data.set(category.name, []));
			headers.forEach((header) => table.appendChild(header));
			// Making selling entry groups for multiple cancellation.
			const selling_entry_groups = new Map();
			for (const [slot, entry] of market_items.sellingEntries()) {
				const market_entry = entry.market_entry;
				const item = market_entry.item;
				const full_type = item.full_type;
				const quantity = item.quantity;
				const price = market_entry.price;
				const key = full_type + "_" + quantity.toString() + "_" + price.toString();
				if (!selling_entry_groups.has(key)) {
					selling_entry_groups.set(key, {
						selling_entries: [entry],
						price: price,
						count: 1
					})
				} else {
					const group = selling_entry_groups.get(key);
					group.selling_entries.push(entry);
					group.count += 1;
				}
			}

			for (const group of selling_entry_groups.values()) {
				const selling_entry = group.selling_entries[0];
				const market_entry = selling_entry.market_entry;
				const item = market_entry.item;
				const is_renamed = item.properties.has("rename");
				if (!cache.hasItemType(item.base_type) && !(is_renamed && cache.hasRename(item.properties.get("rename")))) {
				...
				const inventory_space_available = player_items.emptyInventorySlots() >= group.count;
				if (!inventory_space_available) {
					cancel_sale_button.disabled = true;
				}
				cancel_sale_button.addEventListener("mouseenter", function(e) {
				...
				cancel_sale_button.addEventListener("click", function(e) {
					setLoadingVisible();
					let current_promise = Promise.resolve();
					for (let i = 0; i < group.selling_entries.length; i++) {
						current_promise = current_promise.then(function() {
							const is_last_entry = i === group.selling_entries.length - 1;
							const update = is_last_entry ? UiUpdate.YES : UiUpdate.NO
							setLoadingText("Loading...<br>(Cancelling " + (i + 1).toString() + " of " + group.selling_entries.length.toString() + ")");
							return market_items.cancelSellingEntry(group.selling_entries[i], update);
						});
					}
					current_promise
					.then(function() {
						updateBulkCancelSales(table, market_items, cache, player_items);
						setLoadingHidden();
					});
				});
				// Adding to arrays (to be sorted and added to table).
	...
	function addToSellingTab(player_items, market_items, market_cache) {
		...
		const bulk_post_sales_navigation = bulkPostSalesNavigation();
		...
		post_sales_container.appendChild(bulk_post_sales_navigation);
		...
		// Bulk post sales container
		const bulk_post_sales_container = salesContainer();
		const bulk_post_sales_title = titleBlock("Bulk Sell");
		const bulk_post_sales_keys = bulkPostSalesKeys();
		const bulk_post_sales_frame = scrollingFrame();
		const bulk_post_sales_table = document.createElement("table");
		bulk_post_sales_table.style.setProperty("margin-bottom", "4px");
		const bulk_post_sales_place_input = placeInput();
		bulk_post_sales_place_input.style.setProperty("margin-left", "100px");
		const bulk_post_sales_place_update = updatePlaceButton();
		bulk_post_sales_place_update.style.setProperty("margin-left", "100px");
		const close_bulk_post_sales_container_button = closeContainerButton();
		const post_sales_navigation = postSalesNavigation();
		const bulk_post_sales_market_scrap_dropdown = marketScrapDropdown();
		const bulk_post_sales_under_div = underDiv();
		const bulk_post_sales_undercut_input = undercutInput();
		bulk_post_sales_undercut_input.value = requested_undercut.toString();
		function updateBulkPlaceSales(e) {
			const new_place = parseInt(bulk_post_sales_place_input.value);
			if (!Number.isInteger(new_place) || new_place <= 0) {
				return;
			} else {
				requested_place = new_place;
			...
			setLoadingVisible();
			updateBulkPostSales(bulk_post_sales_table, player_items, market_items, market_cache, requested_place, requested_undercut, requested_datum);
			bulk_post_sales_place_input.value = requested_place.toString();
			setLoadingHidden();
		}
		bulk_post_sales_place_update.addEventListener("click", updateBulkPlaceSales);
		bulk_post_sales_place_input.onkeydown = function(e) {
			if (e.code === "Enter" || e.code === "NumpadEnter") {
				updateBulkPlaceSales();
			}
		}
		bulk_post_sales_place_update.addEventListener("click", updateBulkPlaceSales);
		bulk_post_sales_place_input.onkeydown = function(e) {
		...
		function bulkUpdateUndercut(e) {
			const new_undercut = parseInt(bulk_post_sales_undercut_input.value);
			if (!Number.isInteger(new_undercut) || new_undercut <= 0){
				return;
			} else {
				requested_undercut = new_undercut;
		...
		bulk_post_sales_undercut_input.addEventListener("input", bulkUpdateUndercut);
		function bulkUpdateDatum(e) {
			const new_datum = bulk_post_sales_market_scrap_dropdown.value;
			requested_datum = new_datum;
			setLoadingVisible();
			updateBulkPostSales(bulk_post_sales_table, player_items, market_items, market_cache, requested_place, requested_undercut, requested_datum);
			for (const child of bulk_post_sales_keys.children) {
				if (child.innerHTML === "Market Price" && requested_datum === "scrap") {
					child.innerHTML = "Scrap Price";
					child.style.setProperty("left", "235px");
					break;
				} else if (child.innerHTML === "Scrap Price" && requested_datum === "market") {
					child.innerHTML = "Market Price";
					child.style.setProperty("left", "227px");
					break;
				}
			}
			setLoadingHidden();
		}
		bulk_post_sales_market_scrap_dropdown.addEventListener("change", bulkUpdateDatum);
		close_bulk_post_sales_container_button.addEventListener("click", function(e) {
			bulk_post_sales_container.style.setProperty("display", "none");
		});
		bulk_post_sales_container.appendChild(bulk_post_sales_title);
		bulk_post_sales_container.appendChild(bulk_post_sales_keys);
		bulk_post_sales_container.appendChild(bulk_post_sales_frame);
		bulk_post_sales_frame.appendChild(bulk_post_sales_table);
		bulk_post_sales_container.appendChild(bulk_post_sales_place_input);
		bulk_post_sales_container.appendChild(bulk_post_sales_place_update);
		bulk_post_sales_container.appendChild(close_bulk_post_sales_container_button);
		bulk_post_sales_container.appendChild(post_sales_navigation);
		bulk_post_sales_container.appendChild(bulk_post_sales_market_scrap_dropdown);
		bulk_post_sales_container.appendChild(bulk_post_sales_under_div);
		bulk_post_sales_container.appendChild(bulk_post_sales_undercut_input);
		// Post Sale Navigation
		bulk_post_sales_navigation.addEventListener("click", function(e) {
			updateBulkPostSales(bulk_post_sales_table, player_items, market_items, market_cache, requested_place, requested_undercut, requested_datum)
			.then(function() {
				bulk_post_sales_place_input.value = requested_place.toString();
				bulk_post_sales_undercut_input.value = requested_undercut.toString();
				bulk_post_sales_market_scrap_dropdown.value = requested_datum;
				post_sales_container.style.setProperty("display", "none");
				bulk_post_sales_container.style.setProperty("display", "grid");
			});
		});
		post_sales_navigation.addEventListener("click", function(e) {
			updatePostSales(post_sales_table, player_items, market_items, market_cache, requested_place, requested_undercut, requested_datum)
			.then(function() {
				post_sales_place_input.value = requested_place.toString();
				undercut_input.value = requested_undercut.toString();
				market_scrap_dropdown.value = requested_datum;
				post_sales_container.style.setProperty("display", "grid");
				bulk_post_sales_container.style.setProperty("display", "none");
			});
		});
		// Adding elements to marketplace
		marketplace.appendChild(post_sales_button);
		marketplace.appendChild(post_sales_container);
		marketplace.appendChild(bulk_post_sales_container);
		...
		const bulk_cancel_sales_navigation = bulkCancelSalesNavigation();
		const bulk_cancel_sales_container = salesContainer();
		const bulk_cancel_sales_title = titleBlock("Bulk Cancel");
		const bulk_cancel_sales_keys = bulkCancelSalesKeys();
		const bulk_cancel_sales_frame = scrollingFrame();
		const bulk_cancel_sales_table = document.createElement("table");
		bulk_cancel_sales_table.style.setProperty("margin-bottom", "4px");
		const close_bulk_cancel_sales_container_button = closeContainerButton();
		const cancel_sales_navigation = cancelSalesNavigation();
		...
		close_bulk_cancel_sales_container_button.addEventListener("click", function(e) {
			bulk_cancel_sales_container.style.setProperty("display", "none");
		});
		...
		cancel_sales_container.appendChild(bulk_cancel_sales_navigation);
		bulk_cancel_sales_container.appendChild(bulk_cancel_sales_title);
		bulk_cancel_sales_container.appendChild(bulk_cancel_sales_keys);
		bulk_cancel_sales_frame.appendChild(bulk_cancel_sales_table);
		bulk_cancel_sales_container.appendChild(bulk_cancel_sales_frame);
		bulk_cancel_sales_container.appendChild(close_bulk_cancel_sales_container_button);
		bulk_cancel_sales_container.appendChild(cancel_sales_navigation);
		// Cancel Sales navigation
		bulk_cancel_sales_navigation.addEventListener("click", function(e) {
			setLoadingVisible();
			updateBulkCancelSales(bulk_cancel_sales_table, market_items, market_cache, player_items);
			promiseWait(100)
			.then(function() {
				bulk_cancel_sales_container.style.setProperty("display", "grid");
				cancel_sales_container.style.setProperty("display", "none");
				setLoadingHidden();
			});
		});
		cancel_sales_navigation.addEventListener("click", function(e) {
			setLoadingVisible();
			updateCancelSales(cancel_sales_table, market_items, market_cache);
			promiseWait(100)
			.then(function() {
		...
		marketplace.appendChild(bulk_cancel_sales_container);
	}
