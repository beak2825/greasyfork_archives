...
// @grant        window.open
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/538996/Animestars%20Trade%20Helper.user.js
// @updateURL https://update.greasyfork.org/scripts/538996/Animestars%20Trade%20Helper.meta.js
// ==/UserScript==
...
(function() {
    ...
    const ICONS = {
        ...
        // Remelt Helper Icons (from Test.txt)
    ...
    const SCRIPT_STATE = {
        ...
        // Remelt Helper State (from Test.txt)
            ...
            if (window.location.pathname.includes('/cards/pack/')) {
                const skipperInterval = setInterval(() => {
                    if (typeof unsafeWindow.doAnimLoot === 'function' && unsafeWindow.doAnimLoot.toString().length > 15) {
                        ...
                        console.log('[Super Helper] Анимация паков успешно отключена через unsafeWindow.');
    ...
    // =================================================================================
    // MODULE: REMELT HELPER (from Test.txt)
    // =================================================================================
            ...
            if (path.includes('/cards/') && path.endsWith('/trade/')) {
                ...
                while (hasMoreWantsPages && offeredData.length < offerCount) {
                    ...
                    if (cardsToLookFor.length === 0) {
                        console.log('[Stealth Worker] No more suitable cards from my list to look for in "Wants".');
                        break;
                    }
                    ...
                    console.log(`[Stealth Worker] Scanning page in "Wants" mode. Found ${inventoryItems.length} items.`);
                    for (const myCard of cardsToLookFor) {
                        ...
                        if (matchingItem) {
                            ...
                            if (newCount > initialCount) {
                                console.log(`[Stealth Worker] Found and added a match in "Wants" list: ${myCard.name}`);
                                offeredData.push(myCard);
                            } else {
                                console.warn(`[Stealth Worker] FAILED to add card from "Wants" list: ${myCard.name}. It might be untradeable or a website glitch. Skipping it.`);
                            }
                        }
                    }
                    ...
                    if (paginationEl) {
                        ...
                        if (currentPage >= totalPages) {
                            ...
                            console.log('[Stealth Worker] Last page of "Wants" list reached.');
                        } else {
                            const nextBtn = document.querySelector('#next_trade_page button');
                            if (nextBtn) {
                                console.log(`[Stealth Worker] Clicking next page (${currentPage}/${totalPages}) in "Wants" list.`);
                                nextBtn.click();
                                await wait(750);
                            } else {
                                hasMoreWantsPages = false;
                                console.log('[Stealth Worker] Next button not found, ending "Wants" search.');
                            }
                        }
                    } else {
                        hasMoreWantsPages = false;
                        console.log('[Stealth Worker] Pagination info not found, ending "Wants" search.');
                    }
                }

                console.log(`[Stealth Worker] Stage 1 finished. Found ${offeredData.length} of ${offerCount} cards.`);
                // --- STAGE 2: REGULAR SEARCH (if needed) ---
                if (offeredData.length < offerCount) {
                    console.log('[Stealth Worker] Starting Stage 2: Regular search.');
                    const activeWantBtn = document.querySelector('button.tabs__item.tabs__want__card.tabs__item__want--active');
                    if (activeWantBtn) {
                        console.log('[Stealth Worker] Deactivating "Wants" filter.');
                        activeWantBtn.click();
                        await wait(750);
                    }
                    ...
                    if (suitableAndUnusedCards.length < cardsStillNeeded) {
                        console.error(`[Stealth Worker] Not enough cards for regular search. Have ${suitableAndUnusedCards.length}, need ${cardsStillNeeded}.`);
                        ...
                        for (const image in groupedCards) {
                            ...
                            console.log(`[Stealth Worker] Searching for: ${searchName}`);
                            ...
                            console.log(`[Stealth Worker] Found ${availableItems.length} matching items in search results.`);
                            for (let i = 0; i < cardGroup.length && i < availableItems.length; i++) {
                                ...
                                const newCount = document.querySelectorAll('.trade__main-items[data-type="creator"] .trade__main-item').length;
                                if (newCount > initialCount) {
                                    console.log(`[Stealth Worker] Successfully added card from search: ${cardGroup[i].name}`);
                                    offeredData.push(cardGroup[i]);
                                } else {
                                    console.warn(`[Stealth Worker] FAILED to add card from search: ${cardGroup[i].name}. Skipping it.`);
                                }
                            }
                        }
                    }
                ...
                if (offeredData.length < offerCount) {
                    ...
                    console.log(`[Stealth Worker] Проверка успешна. Отправляю обмен с ${offeredData.length} карт(ой).`);
        ...
        HelperModule: {
            ...
            prepareOwnerLinks() {
                ...
                if (!cardId) {
                    ...
                    if (cardId) {
                        console.log(`[Helper Module] ID карты для обмена не был сохранен, используется ID с текущей страницы: ${cardId}`);
                    } else {
                        console.log('[Helper Module] Не удалось определить ID целевой карты для обмена.');
                        return;
                    }
                }
                ...
                document.querySelectorAll('a.card-show__owner').forEach(link => {
                    try {
                        ...
                        console.error('[Helper Module] Не удалось обработать ссылку:', link.href, e);
                    }
                });
            },
            initiateTradeOnUserPage() {
                ...
                waitForElement(baseCardSelector, () => {
                    ...
                    if (cardToClick) {
                        ...
                        console.log('[Helper Module] Найдена карта без замка. Кликаю...');
                        ...
                        console.warn(`[Helper Module] Карта (ID: ${cardId}) найдена, но все копии заблокированы. Авто-переход остановлен.`);
                    }
                });
            },
                ...
                if (loggedInUser && (new URLSearchParams(window.location.search).get('name') === null || new URLSearchParams(window.location.search).get('name') === loggedInUser)) {
                    ...
                    const startMarkingAsUnwantedProcess = async () => {
                        ...
                        if (processedPages.has(currentPageNum)) {
                            console.log('[AS Helper] Страница уже обработана, остановка во избежание цикла.');
                        ...
                        try {
                            ...
                            if (markUnwantedBtn) {
                                ...
                                console.log('[AS Helper] Нажата кнопка "Добавить карты на странице в ненужные".');
                                await wait(3000);
                                // Увеличенная задержка после нажатия
                            } else {
                                const otherBtn = document.querySelector('button.profile-cards__deck-btn');
                                if (otherBtn) {
                                    console.log('[AS Helper] Карты на странице уже отмечены или доступно другое действие. Пропускаю.');
                                } else {
                            ...
                            console.error('[AS Helper] Ошибка в процессе отметки карт:', error);
    ...
    const BoostHelper = {
        ...
        handleSkipToggle: function() {
            ...
            console.log(`[Boost Helper] Скрипт пропуска карт ЗАПУЩЕН (интервал ожидания 't': ${SCRIPT_STATE.boostSkipInterval} мс, частота проверки: 500 мс).`);
            } else {
                if (!this.skipInterval) return;
            ...
            if (newInterval !== null) {
                ...
                if (!isNaN(intervalMs) && intervalMs >= 200) {
                    ...
                    if (this.skipInterval) {
                        ...
                        console.log(`[Boost Helper] Интервал автоскипа обновлен: ${intervalMs} мс`);
                    }
                } else {
        ...
        scheduleAutoBoost: function() {
            ...
            console.log(`[Boost Helper] Автокликер запланирован на ${formatter.format(targetTime)} (МСК)`);
    ...
    const UpgradeHelper = {
        ...
        unfixCollection: async function(context, url, title, notificationId) {
            ...
            try {
                ...
                if (!fixButton) {
                    console.log('[Upgrade Helper] Кнопка фиксации не найдена. Пропускаю этап.');
                ...
                if (error.message && error.message.includes('не найдено')) {
                    console.log(`[Upgrade Helper] Коллекция "${title}" не найдена. Пропускаю этап фиксации.`);
        ...
        fixCollection: async function(context, url, title, notificationId) {
            ...
            const fixButton = animeLink.closest('.user-anime').querySelector('.fix-my-progress');
            if (!fixButton) {
                console.log('[Upgrade Helper] Кнопка фиксации не найдена. Возврат не требуется.');
                return;
            }
            ...
            waitForElement('body', async () => {
                ...
                try {
                    if (step === 'unlockCards' && path.includes('/user/cards/')) {
                        console.log('[Upgrade Worker] Step 1: Unlocking cards...');
                        ...
                        console.log('[Upgrade Worker] Step 2: Searching for anime...');
                        ...
                        console.log('[Upgrade Worker] Step 2.5 or 5: Handling collection fix...');
                        ...
                        console.log('[Upgrade Worker] Step 3: Selecting rank...');
                        ...
                        console.log('[Upgrade Worker] Step 5: Finding and upgrading card...');
    ...
    const RankBasedTradeLinker = {
        run: function() {
            ...
            if (path.includes('/cards/users/') && !path.includes('/need') && !path.includes('/trade')) {
                ...
                if (rankElement) {
                    ...
                    if (rankClass) {
                        ...
                        console.log(`[Rank Linker] Ранг карточки '${rank.toUpperCase()}' был сохранен.`);
                    }
                }
                const mainCardImage = document.querySelector('.ncard__img img');
                if (mainCardImage) {
                    ...
                    if (cardId) {
                        ...
                        console.log(`[Rank Linker] ID целевой карты для обмена '${cardId}' был сохранен.`);
                    }
                }
            }
            ...
            if (path.includes('/cards/users/need')) {
                ...
                if (savedRank) {
                    console.log(`[Rank Linker] Применяется сохраненный ранг '${savedRank.toUpperCase()}'.`);
                    ...
                    console.log(`[Rank Linker] Все ссылки на пользователей (${userLinks.length} шт.) были обновлены.`);
                } else {
                    console.warn('[Rank Linker] Сохраненный ранг карточки не найден. Сначала откройте основную страницу карточки.');
                }
            }
        }
    };
    const AutoUpgradeHelper = {
        ...
        startAutoUpgrade: function() {
            console.log('[Auto Upgrade] Запуск автоматического улучшения...');
            this.performClickAndScheduleNext();
        },

        stopAutoUpgrade: function() {
            console.log('[Auto Upgrade] Остановка автоматического улучшения.');
        ...
        performClickAndScheduleNext: function() {
            ...
            if (upgradeButton) {
                upgradeButton.click();
                console.log('[Auto Upgrade] Нажата кнопка "Начать повышение".');
                ...
                console.warn('[Auto Upgrade] Кнопка улучшения не найдена. Остановка.');
