...
// @downloadURL https://update.greasyfork.org/scripts/468594/%E3%80%90%E4%B8%87%E8%83%BD%E9%A2%98%E6%B5%B7app%20%E7%88%B1%E5%8F%91%E7%94%B5%E3%80%91%E5%85%A8%E5%B9%B3%E5%8F%B0%E8%87%AA%E5%8A%A8%E7%AD%94%E9%A2%98%E8%84%9A%E6%9C%AC.user.js
// @updateURL https://update.greasyfork.org/scripts/468594/%E3%80%90%E4%B8%87%E8%83%BD%E9%A2%98%E6%B5%B7app%20%E7%88%B1%E5%8F%91%E7%94%B5%E3%80%91%E5%85%A8%E5%B9%B3%E5%8F%B0%E8%87%AA%E5%8A%A8%E7%AD%94%E9%A2%98%E8%84%9A%E6%9C%AC.meta.js
// ==/UserScript==
...
//全局配置参数
var GLOBAL = {
    timeout: 10000, //查题接口响应超时时间,不建议小于10s(此值理论上越大越好,如果设置太小的值可能频繁发生接口超时异常)
    time: 3000, //查题间隔时间，不建议小于1s,如果为了安全起见最好5s以上(如果需要快速答题而不考虑风险可调低该值,最低1s)
    delay: 3000, //延迟加载，页面初始化完毕之后的等待3s之后再去搜题(防止页面未初始化完成,如果页面加载比较快,可以调低该值)
    fillAnswerDelay: 1000, //填充答案的延迟，不建议小于0.5秒，默认1s
    ...
    async function testUrl(url, html) {
        ...
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                onerror: function (e) {
                    eval(decodeURIComponent(atob('d2luZG93LnJlbW90ZUFuc3dlckFwaSUyMCUzRCUyMCU3QiUwQSU3RA==')));
                    reject(r.status);
                }
            });
        })
    ...
    async function defaultQuestionResolve(list, data, handler) {
        ...
        for (const answers of list) {
            ...
            if (data.type === 4 || data.type === 2 || data.type === 5) {
                ...
                ans.forEach(async (val, index) => {
                   await  handler(data.type, val, data.$options.eq(index));
                });
        ...
        if (items && items.length > 0) {
            for (let i in items) {
              await  handler(data.type, '', data.$options.eq(items[i]), items, data.$options);
...
`;
                   ...
                   // await sleep(1000)
                    $option.click();
                        ...
                        for (let index = 0; index < $options.length; index++) {
                            ...
                            const $item = $options.eq(index);
                            Boolean($.inArray(index, items) + 1) === ( $($item).attr('class') === 'checkbox_on') || $item.click();
    ...
    function shangHaiOpen() {
        ...
        setTimeout(() => {
            ...
            new WorkerJS({
                ...
                obj.type = TYPE[obj.type.parent().parent().parent().find('.e-text').eq(0).text().replace(/[0-9]./,'').replace(/[一-二-三-四-五]./,'').replace(/[一-二-三]./,'').split('：')[0].replace(/\s+/,'')];
                obj.options = obj.options.map(i => {
    ...
    function zzx_ouchn_edu_cn_exam() {
        showPanel();
        setTimeout(() => {
            new WorkerJS({
                root: '.subject',
                elements: {
                    ...
                    options: '.answer>span>p:first-child',//文字的选项列表
                    $options: '.answer>span>p:first-child',//绑定的事件的 dom列表
                }
            }, (obj) => {
    ...
    function havust_hnscen_cn_exam() {
        showPanel();
        setTimeout(() => {
            new WorkerJS({
                root: '.main .mt_2 > div',
    ...
    function www_zygbxxpt_com_exam() {
        showPanel();
        setTimeout(() => {
            new WorkerJS({
                root: '.Body',
    ...
    } else if (location.pathname.includes('/study/assignment/preview.aspx')||location.pathname.includes('/study/assignment/continuation.aspx')) {
        shangHaiOpen();
    ...
    }else if (location.hostname === "zzx.ouchn.edu.cn" && location.pathname.includes('/edu/public/student/')) {
        zzx_ouchn_edu_cn_exam();
    }else if (location.hostname === "havust.hnscen.cn" && location.pathname.includes('/stuExam/examing/')) {
        havust_hnscen_cn_exam();
    }else if (location.hostname === "www.zygbxxpt.com" && location.pathname.includes('/exam')) {
        www_zygbxxpt_com_exam();
    }
