...
// @homepage    https://greasyfork.org/zh-CN/scripts/441262
...
// @downloadURL https://update.greasyfork.org/scripts/441262/wsmud_funny2.user.js
// @updateURL https://update.greasyfork.org/scripts/441262/wsmud_funny2.meta.js
// ==/UserScript==
...
function say(text) {
  ...
  (function() {
    ...
    var onmessage = event => {
      ...
      if(ws_on_message==null){ws_on_message=unsafeWindow.funny.API.ws_on_message}
      //ws_on_message.apply(this, [event]);
    }
    let listeners = [];//监听
    let addListener = fn => {
    ...
    addListener(function (event, message) {
      if (message.type === "room" && !unsafeWindow.WG) {
        ...
        ws_on_message.apply(this, [event0]);
        $("#show").click(() => {
          $("#more").show();
          $("#show").hide();
          $("#hide").show();
      ...
      );
      {
        $(".left-hotkeys").append(
          $("<div></div>").append(
            ...
            $(`<span class="span-btn"></span>`).append("任务").click(() => {  $("span[command=\"tasks\"]").click(); }),
    ...
    function SendCommand(command) {
      if (command instanceof Array) {
        ...
        if (typeof cmd1 === "number") {
          ...
          if(websocket==null){websocket=unsafeWindow.funny.API.websocket}
