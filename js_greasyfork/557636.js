...
// @run-at      document-end
// @downloadURL https://update.greasyfork.org/scripts/557636/Qiuwei%20Chat%20Assistant.user.js
// @updateURL https://update.greasyfork.org/scripts/557636/Qiuwei%20Chat%20Assistant.meta.js
// ==/UserScript==
...
;(function() {
    ...
    const utils = {
        ...
        processLongText(text) {
            ...
            processed = processed.replace(/(https?:\/\/[^\s]{100,})/g, function(match) {
    ...
    function addEventListeners() {
        ...
        if (!submitBtn || !copyBtn || !captureBtn || !minimizeBtn) {
    ...
    function capturePageContent(forceCapture = false, retryCount = 0) {
        try {
            ...
            let nickname = '';
            const targetDiv = rootContainer.querySelector('div.inline-block.truncate.cursor-text.overflow-hidden');
            ...
            if (activeCard) {
                // 名称在包含 text-[12px] 类的 div 中的 div 元素内
                const nameDiv = activeCard.querySelector('[class*="text-[12px]"] div');
            ...
            if (belong_user_name && wx_nick_name) {
                if (belong_user_name !== state.lastBelongUserName || wx_nick_name !== state.lastWxNickName) {
                    // 切换用户，清理缓存
                    state.prerequisiteCache = null;
                    ...
                    if (container) {
                        container.innerHTML = '<div style="padding: 24px; text-align: center; color: #999;"><span class="ant-loading-dot"></span> 加载中...</div>';
                    }
                ...
                for (let i = lastRightMessageIndex + 1; i < msgItems.length; i++) {
                    ...
                    if (leftSideDiv && !leftSideDiv.classList.contains('creator')) {
                        ...
                        if (textDiv && textDiv.innerHTML.trim()) {
                            ...
                            // 使用新的表情包处理函数，保持原位置
                            const processedText = utils.processMessageWithEmojisInPosition(clonedTextDiv.innerHTML);
    ...
    function validatePrerequisiteCard(item) {
        // 示例校验逻辑：检查咨询详情是否有效
        // 可根据实际需求修改校验规则
        if (!item.consultation_details || item.consultation_details === '-' || !item.consultation_details.trim()) {
    ...
    function showBindConfirmation(item, onConfirm, onCancel) {
        ...
        body.innerHTML = `是否确认将“<span style="font-weight: bold;">${item.wx_nick_name || '-'}</span>”的信息绑定到“<span style="font-weight: bold;">${wxId}</span>”？`;
    ...
    function renderPrerequisiteCards(data, isSelectable = false) {
                    ...
                    <div style="padding: 16px;">
                        <div style="display: flex; align-items: center; margin-bottom: 12px;">
                            <div style="flex: 1; padding-right: 12px; border-right: 1px solid #f0f0f0;">
                                <p style="margin: 0; color: #262626; font-size: 16px; font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${item.wx_nick_name || item.nickname || '-'}">${item.wx_nick_name || '-'}</p>
                            </div>
                            <div style="flex: 1; padding-left: 12px;">
                                <p style="margin: 0; color: #262626; font-size: 16px; font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${item.phone || '-'}</p>
                            </div>
                        </div>

                        <div style="margin-bottom: 12px;">
                            <p style="margin: 0; color: #595959; font-size: 14px; line-height: 1.6; white-space: pre-wrap;">${item.consultation_details || '-'}</p>
                        </div>
                        
                        <div style="text-align: right;">
                            <span style="display: inline-block; background: #f5f5f5; color: #8c8c8c; font-size: 12px; padding: 4px 8px; border-radius: 4px;">${item.create_time || '-'}</span>
                        </div>
                    </div>
                </div> 
            `;
        ...
        if (isSelectable) {
            if (!container.dataset.hasScrollListener) {
                container.addEventListener('scroll', utils.debounce(function() {
        ...
        if (isSelectable) {
            cards.forEach(card => {
                card.addEventListener('click', function() {
                    ...
                    const belongUserNameInput = document.getElementById('belong_user_name');
                    const belongUserName = belongUserNameInput ? belongUserNameInput.value.trim() : '';
    ...
    function getPrerequisiteInformation() {
        ...
        setTimeout(() => {
            ajaxRequest({
                method: "POST",
                url: "https://plugin.fengyu-ai.com/api/prerequisite_information/getDetailForAssistant",
                data: JSON.stringify({
    ...
    function getPrerequisiteInformationList(pageNum = 1, pageSize = 10) {
        ...
        setTimeout(() => {
            ajaxRequest({
                method: "POST",
                url: "https://plugin.fengyu-ai.com/api/prerequisite_information/getListForAssistant",
                data: JSON.stringify({
                    pageNum: pageNum,
                    pageSize: pageSize,
                    belong_user_name: belong_user_name,
                        ...
                        try {
                            ...
                            if (result && result.code === 200) {
                                ...
                                if (pageNum === 1) {
                                    ...
                                    state.prerequisiteCache = (state.prerequisiteCache || []).concat(list);
                                }
    ...
    function bindConversation() {
        ...
        if (state.selectedPrerequisiteId == '' || state.selectedPrerequisiteId == null) {
            showToast('前置信息id不能为空');
            return;
        }
        if (wx_id == '' || wx_id == null) {
        ...
        setTimeout(() => {
            ajaxRequest({
                method: "POST",
                url: "https://plugin.fengyu-ai.com/api/prerequisite_information/bindConversation",
                data: JSON.stringify({
