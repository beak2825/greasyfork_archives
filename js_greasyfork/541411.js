...
// @run-at      document-end
// @downloadURL https://update.greasyfork.org/scripts/541411/Qiuwei%20Chat%20Assistant.user.js
// @updateURL https://update.greasyfork.org/scripts/541411/Qiuwei%20Chat%20Assistant.meta.js
// ==/UserScript==
...
;(function() {
    ...
    function createFloatingButton() {
        ...
        // 尝试查找聊天相关元素作为参考
        const chatContainer = document.querySelector('.chat-container, .message-list, .conversation-area');
        ...
        try {
            ...
            for (const item of spaceItems) {
                ...
                if (targetDiv && targetDiv.textContent.trim()) {
                    nickname = targetDiv.textContent.trim();
                    break;
                }
            }
            ...
            // 3. 消息内容抓取（根据demo-html.js规则优化）
            let messages = [];
            const msgItems = rootContainer.querySelectorAll('div.message-item');
            ...
            // 获取最后一个消息的div.w-full下的节点
            const wfullDiv = lastMessageItem.querySelector('div.w-full');
            if (!wfullDiv) {
                return;
            }
            
            // 获取div.w-full下的最后一个子节点
            const lastChild = wfullDiv.lastElementChild;
            if (!lastChild) {
                return;
            }
            
            // 根据demo-html.js规则判断消息类型
            // div.creator.item.pr-[6px] 属于右侧消息（AI消息）
            // div.item.pr-[6px] 属于左侧消息（用户消息）
            ...
            // 无论最后一条消息是什么类型，都要抓取昵称
            if (nicknameInput && nickname) {
                nicknameInput.value = nickname;
            }
            
            // 如果最后一个消息不是左侧消息，则只显示昵称，清空消息内容
            ...
            // 如果最后一个消息是左侧消息，则捕获最后一个右侧消息之后的所有左侧消息
            let lastRightMessageIndex = -1;
            
            // 从后往前遍历消息，找到最后一个右侧消息的位置
            for (let i = msgItems.length - 1; i >= 0; i--) {
                ...
                // 查找右侧消息 div.creator.item.pr-[6px]
                ...
                for (let i = lastRightMessageIndex + 1; i < msgItems.length; i++) {
                    ...
                    if (leftSideDiv && !leftSideDiv.classList.contains('creator')) {
                        ...
                        if (textDiv && textDiv.textContent.trim()) {
                            ...
                            // 移除ant-image-mask中的内容
                            const imageMasks = clonedTextDiv.querySelectorAll('.ant-image-mask');
                            imageMasks.forEach(mask => mask.remove());
                            
                            const cleanText = clonedTextDiv.textContent.trim();
                            if (cleanText) {
                                messages.push(cleanText);
                            }
                        }
                        
                        // 检查是否有图片消息（只抓取ant-image-img类的图片）
                        const contentDiv = leftSideDiv.querySelector('.content');
                        if (contentDiv) {
                            const contentImages = contentDiv.querySelectorAll('img.ant-image-img');
                            contentImages.forEach(img => {
                                if (img.src && img.src.trim()) {
                                    messages.push(`${img.src.trim()}`);
                                }
                            });
                        }
                    }
    ...
    function adjustButtonPosition() {
        ...
        const chatContainer = document.querySelector('.chat-container, .message-list, .conversation-area, .chat-box');
        if (chatContainer) {
            const chatContainerRect = chatContainer.getBoundingClientRect();
            floatingBtn.style.position = 'absolute';
            floatingBtn.style.top = (chatContainerRect.top - 50) + 'px';
            // 靠右对齐，距离右侧20px
