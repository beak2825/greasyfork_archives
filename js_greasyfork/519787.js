// ==UserScript==
// @name         WK Auto Commit Fixed
// @namespace    WKAUTOCOMMIT
// @version      0.5.1
// @description  Auto commit for Wanikani
// @author       Johannes Mikulasch, fix: Lockyard
...
// @run-at       document-end
// @license
// @downloadURL https://update.greasyfork.org/scripts/519787/WK%20Auto%20Commit%20Fixed.user.js
// @updateURL https://update.greasyfork.org/scripts/519787/WK%20Auto%20Commit%20Fixed.meta.js
// ==/UserScript==
 ...
 * 
 * Verion 0.5.1
 * Removed 'warned' and 'blocked' items from expected answer, which lead to incorrect auto commits. Also removed verbose log
 * Version 0.5.0 onward by Lockyard
 * Fixed fields renamed by wanikani which broke the script, filling expected_answers with broken stuff
 * 
 *
 * Version 0.4.8
...
window.addEventListener("willShowNextQuestion", function(event) {
    ...
    if (item.questionType === "meaning") {
        expected_answers = expected_answers.concat(subject.meanings.filter((e) => e?.kind == 'primary' || e?.kind == 'alternative' || e?.kind == 'allowed').map((e) => e.text));
        ...
        if (subject.type === 'Vocabulary') {
            expected_answers = expected_answers.concat(subject.readings.filter((e) => e?.kind == 'primary' || e?.kind == 'alternative' || e?.kind == 'allowed').map((e) => e.text));
        } else if (subject.type === 'Kanji') {
            expected_answers = expected_answers.concat(subject.readings.filter((e) => (e?.kind == 'primary' || e?.kind == 'alternative' || e?.kind == 'allowed') && e.type === subject.primary_reading_type)).map((e) => e.text);
        }
    }
