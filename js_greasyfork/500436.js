...
// @name            IWR
// @version         no
// @author          a wealthy aristocrat (ft. MIT Prof)
// @namespace       https://semver.com/
// @description     try to play with this mod
// @match           *://*.moomoo.io/*
// @icon            https://i.pinimg.com/564x/e1/e4/e9/e1e4e9c522b08c237bc4e2419d6575fa.jpg
// @grant           none
// @downloadURL https://update.greasyfork.org/scripts/500436/IWR.user.js
// @updateURL https://update.greasyfork.org/scripts/500436/IWR.meta.js
// ==/UserScript==

//SCRIPT_START_HEADER_END
var pps = 0, ppm = 0, key;
...
, fc = function () {
    this.init = function (e, t, i, s, n, r, o, notif) {
        this.x = e,
            this.y = t,
            this.originalX = e,
            this.originalY = t,
        ...
        if (!this.notif) {
            if (this.life) {
                this.life -= e;
                //this.y -= this.speed * e * this.acc;
                this.acc -= e / (this.maxLife / 2.5);
                if (this.life <= 200) {
                    if (this.alpha > 0) {
                        this.alpha = max(0, this.alpha - (e / 300));
                    }
                } else {
                    if (this.alpha < 1) {
                        this.alpha = min(1, this.alpha + (e / 100));
                    }
                }
                this.x += abs(this.x - this.originalX) + abs(this.ranX) >= 10 ? 0 : this.ranX;
                if (this.scale <= 25) this.scale += 1.2;
                if (this.life <= 0) {
                    this.life = 0;
                }
            }
    ...
    this.sid = e,
        ...
        for (let i = nearPlayers.length, weaponObj; i--;) {
            ...
            if (playerDist[i] < weaponObj.range + 35 + boostMovement) {
                health -= weaponObj.dmg * ((playerDist[i] < weaponObj.range + 35 + tankMovement) * 3.3 || 1)
            }
        }
...
function makeAngles(building, type) {
    const { player, enemies } = IWR;
    let buildings = building.filter(obj => Utils.getDist(player, obj, 2, 0) < player.scale + IWR.items.list[type].scale + obj.scale + 50);

    for (const object of buildings) {
        const index = buildings.indexOf(object);
        const dist = Utils.getDist(player, object, 2);
        const distE = Utils.getDist(enemies[0], object, 2);
    ...
    let allAngles = []
    let offset = player.scale + IWR.items.list[type].scale + (IWR.items.list[type].placeOffset || 0)
    for (let i = 0; i < buildings.length; i++) {
        ...
        let angles = []
        let dist = (IWR.items.list[type].scale + scale + 1)
...
class tphoftm {
    //wss servers
    static privateChatSocket = "haxxer is a skid"; //new WebSocket('wss://time-probable-magnesium.glitch.me');
...
!private, toggles private chat\n
    ...
    */
    ...
    place(item, dir, sC = false, real = false) {
        ...
        this.socket.send("G", this.player.weaponIndex, 1);
    ...
    */
    ...
    handleTick() {
        ...
        if (millplacer && this.tick % 3 === 0) {
            ...
            [ang, ang + Math.PI / 2.4, ang - Math.PI / 2.4].forEach(angle => secondaryCheck(this.player.items[3], angle) && this.place(this.player.items[3], angle));
        }
        ...
        if (this.clicks.right) {
            ...
            if (this.player.weaponIndex != this.player.weapons[0]) {
                this.socket.send("G", this.player.weapons[0], 1);
            };
...
setTimeout(() => {
    addMenuChText("Server", "Hello everyone, my name is HaXXer On YT, my new name is poopsack.", "lightgreen");
    addMenuChText("Private", "I made sandy mod", "brightpink", "lightgreen");
    addMenuChText("Server", "Im also a gay skid faggot", "red");
    addMenuChText("Server", "Barely any code here is written by me, and most of it is by spyder and dune!", 'red');
}, 500);
