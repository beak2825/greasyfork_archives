// ==UserScript==
// @name         Torn.com Enhanced Chat Buttons Beta
...
// @downloadURL https://update.greasyfork.org/scripts/541770/Torncom%20Enhanced%20Chat%20Buttons%20Beta.user.js
// @updateURL https://update.greasyfork.org/scripts/541770/Torncom%20Enhanced%20Chat%20Buttons%20Beta.meta.js
// ==/UserScript==
...
(function() {
    ...
    function getAutoSendSetting() {
        return localStorage.getItem('autoSendMessages') === 'true';
    }

    function saveAutoSendSetting(enabled) {
        localStorage.setItem('autoSendMessages', enabled.toString());
        showNotification('Auto-send setting saved.');
    }
    ...
    function createUIPanel() {
        ...
        panel.innerHTML = `
            ...
            <div id="config-settings-tab" class="tab-content">
                ...
                <div style="margin: 15px 0;">
                    <label for="auto-send-toggle" style="display: flex; align-items: center; cursor: pointer;">
                        <input type="checkbox" id="auto-send-toggle" ${getAutoSendSetting() ? 'checked' : ''} style="margin-right: 8px;">
                        Auto-send messages when clicking buttons
                    </label>
                    <small style="color: #666; display: block; margin-top: 5px;">
                        When enabled, clicking a button will automatically send the message by pressing Enter
                    </small>
                </div>
                <button id="save-auto-send-button">Save Auto-Send Setting</button>
        ...
        document.getElementById('save-auto-send-button').addEventListener('click', () => {
            const enabled = document.getElementById('auto-send-toggle').checked;
            saveAutoSendSetting(enabled);
        });
    ...
    function insertMessage(chatBox, message, targetButton) {
        ...
        if (getAutoSendSetting()) {
            // Use setTimeout to prevent UI freeze and ensure the input is properly registered
            setTimeout(() => {
    ...
    function exportConfig() {
        const config = {
            ...
            autoSendMessages: getAutoSendSetting()
        };
            ...
            reader.onload = (e) => {
                try {
                    ...
                    if (config && config.buttons) {
                        ...
                        if (config.autoSendMessages !== undefined) {
                            saveAutoSendSetting(config.autoSendMessages);
                            document.getElementById('auto-send-toggle').checked = config.autoSendMessages;
                        }
                        populateButtonConfigs();
