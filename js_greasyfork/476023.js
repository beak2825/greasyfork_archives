...
// @author       Xiguayaodade
// @license      MIT
// @match        *://web3.swufe-online.com/*
// @match        *://kc.swufe-online.com/*
// @grant        GM_info
// @grant        GM_getTab
// @grant        GM_saveTab
// @grant        GM_setValue
...
// @downloadURL https://update.greasyfork.org/scripts/476023/%E8%A5%BF%E8%B4%A2%E5%9C%A8%E7%BA%BF.user.js
// @updateURL https://update.greasyfork.org/scripts/476023/%E8%A5%BF%E8%B4%A2%E5%9C%A8%E7%BA%BF.meta.js
// ==/UserScript==
...
(function() {
    ...
    var addLisenner = function(){
        ...
        //获取播放器组件
        elevideo = window.frames[0][0].document.getElementsByTagName("video")[0];
        ...
        vdpause = function(){
            ...
            setTimeout(function(){
                ...
                }else if(document.getElementsByClassName("layui-layer layui-layer-dialog")[0] != null){
                    console.log("xigua:挂机监测，等待响应");
                    addMessage("xigua:挂机监测，等待响应");
                    setTimeout(function(){
                        document.getElementsByClassName("layui-layer layui-layer-dialog")[0].getElementsByClassName("layui-layer-btn0")[0].click();
                        setTimeout(function(){
                            elevideo.play();
                        },500);
                    },1000);
                }else{
                    let currentTime = elevideo.currentTime;
                    let duration = elevideo.duration;
                    let progress = (currentTime / duration) * 100;
                    if(progress >= 98){
                        console.log("xigua:播放完毕");
                        addMessage("xigua:播放完毕");
                        removeLisenner();
                        setTimeout(function(){
                            console.log("xigua:开始检索");
                            addMessage("xigua:开始检索");
                            teamId++;
                            searchTeam();
                        },1500);
                    }else{
        ...
        vdended = function(){
            clearInterval(speedIn);
            console.log("xigua:结束播放，等待10秒");
            addMessage("xigua:结束播放，等待10秒");
            setTimeout(function(){
    ...
    //------检索章节是否完成start------
    var search = function(){
        chapterCount = window.frames[0].document.getElementsByClassName("s_learnlist")[0].getElementsByClassName("s_chapter").length;
        if(chapterId <= chapterCount){
            sectionCount = window.frames[0].document.getElementsByClassName("s_sectionlist")[chapterId-1].getElementsByClassName("s_section").length;
            console.log("xigua:当前第"+chapterId+"章，共"+sectionCount+"讲。");
            addMessage("xigua:当前第"+chapterId+"章，共"+sectionCount+"讲。");
            searchSection();
        }else{
    ...
    var searchSection = function(){
        if(sectionId <= sectionCount){
            teamCount = window.frames[0].document.getElementsByClassName("s_sectionlist")[chapterId-1].getElementsByClassName("s_sectionwrap")[sectionId-1].getElementsByClassName("s_point").length;
            searchTeam();
        }else{
    ...
    var searchTeam = function(){
        if(teamId <= teamCount){
            let studyState = window.frames[0].document.getElementsByClassName("s_sectionlist")[chapterId-1].getElementsByClassName("s_sectionwrap")[sectionId-1].getElementsByClassName("s_point")[teamId-1].getElementsByTagName("div")[0].className;
            let studyType = window.frames[0].document.getElementsByClassName("s_sectionlist")[chapterId-1].getElementsByClassName("s_sectionwrap")[sectionId-1].getElementsByClassName("s_point")[teamId-1].getElementsByTagName("div")[2].innerText;
            //'视频：0402.mp4'
            if(studyType.substring(0,2) === '视频'){
                //未完成 === 'item_done_icon item_done_pos'
                //完成 === 'item_done_icon item_done_pos done_icon_show'
                if(studyState === 'item_done_icon item_done_pos'){
                    console.log("xigua:现在学习第"+chapterId+"章第"+sectionId+"讲资源"+teamId);
                    addMessage("xigua:现在学习第"+chapterId+"章第"+sectionId+"讲资源"+teamId);
                    window.frames[0].document.getElementsByClassName("s_sectionlist")[chapterId-1].getElementsByClassName("s_sectionwrap")[sectionId-1].getElementsByClassName("s_point")[teamId-1].click();
                    setTimeout(function(){
                        removeLisenner();
                        setTimeout(function(){
                            addLisenner();
                    ...
                    console.log("xigua:第"+chapterId+"章第"+sectionId+"讲资源"+teamId+"已完成");
                    addMessage("xigua:第"+chapterId+"章第"+sectionId+"讲资源"+teamId+"已完成");
                    teamId++;
                    searchTeam();
                }
            }else{
                console.log("xigua:跳过第"+chapterId+"章第"+sectionId+"讲资源"+teamId);
                addMessage("xigua:跳过第"+chapterId+"章第"+sectionId+"讲资源"+teamId);
                teamId++;
                searchTeam();
            }
        }else{
    ...
    var wait = setInterval(function (){
        ...
        // if(nowUrl.substring(0,77) === 'https://kc.swufe-online.com/learnspace/learn/learn/templateeight/index.action'){
        if(window.location.host === 'kc.swufe-online.com'){
            addMessage("学习页");
            console.log("学习页");
            setTimeout(function(){
                search();
            },500);
        }else if(nowUrl === 'https://web3.swufe-online.com/user/stu/index.jsp'){
            addMessage("课程列表");
            console.log("课程列表");
            // 获取数据
            courseCount = document.getElementsByClassName("main")[0].getElementsByTagName("ul")[0].getElementsByTagName("a").length;
            var ju = function(){
                   ...
                   // console.log("当前第1科,学习已记录");
                    ...
                    if(courseId <= courseCount){
                       ...
                       // console.log("当前第"+courseId+"科，共"+courseCount+"科");
                        ...
                        var courseName = document.getElementsByClassName("main")[0].getElementsByTagName("ul")[0].getElementsByTagName("a")[courseId-1].getElementsByClassName("cn")[0].innerText;
                        var courseHref = document.getElementsByClassName("main")[0].getElementsByTagName("ul")[0].getElementsByTagName("a")[courseId-1].href;
                        if(courseName != '毕业论文' && courseName != '大学英语B（统考）'){
                            setTimeout(function(){
                                // window.open(courseHref,'_self');
                            },1500);
                        }else{
                            addMessage("跳过"+courseName);
                            console.log("跳过"+courseName);
                            GM_setValue("courseId", (courseId+1));
                            setTimeout(function(){
                                ju();
                            },1000);
                        ...
                        GM_deleteValue("courseId");
                        //addMessage("所有科目学习完成,清空缓存,退出!刷新页面可再次运行");
                       // console.log("所有科目学习完成,清空缓存，退出!刷新页面可再次运行");
                    }
                }
            }
            ju();
