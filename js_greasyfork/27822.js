// ==UserScript==
// @name        百度网盘直接下载助手（改）
// @namespace   renne@gmail.com
// @description 增加推送aria2功能
// @author       Renne
...
// @downloadURL https://update.greasyfork.org/scripts/27822/%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E7%9B%B4%E6%8E%A5%E4%B8%8B%E8%BD%BD%E5%8A%A9%E6%89%8B%EF%BC%88%E6%94%B9%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/27822/%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E7%9B%B4%E6%8E%A5%E4%B8%8B%E8%BD%BD%E5%8A%A9%E6%89%8B%EF%BC%88%E6%94%B9%EF%BC%89.meta.js
// ==/UserScript==
...
(function() {
    ...
    function PanHelper(){
        ...
        var aria2RPC = "http://token:83376885@localhost:6800/jsonrpc";
        var aria2Header = "Cookie: BDUSS=hOMEtWRTJmZklkOUlhWEc5UG5iMkFTeE0wTXFTWX5MTWk5SkoxTlBHS2UzdTFYQVFBQUFBJCQAAAAAAAAAAAEAAAD0gRkCRXJpYzI1NDQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAJ5RxleeUcZXeF; STOKEN=6462b9bfba099fa5b6ebd2261020108b011e6927adb981a2250724f6ab09e4af; SCRC=c77e4683570fd3e2163a8156c6eb8762; __cfduid=daec6411df6d0d9d8b34c16b55220e48a1488265946; PANPSC=3157524027107904279%3AzKsuZfKzYZNTTUR8K35N3ytNuTbE3Vkvqnl51Q2UuOlKu8WyLCLL4a1wwlQMCvHkylUiKLAFl5U%2FR9kih9xSl1lZViRoO6nmfXOsWjuKC4FuWt7N7jFSoGVWcHP9mn3MPjCB1D%2FVxYyaB5R8hpvpEDWPC7NKcJhViVE530gHCys%3D";
        ...
        function addButton(){
            ...
            var $apibutton_pusharia2_button = $('<a id="pusharia2-api" class="g-button-menu" href="javascript:void(0);">发送至aria2</a>');
            ...
            $apibutton_menu.append($apibutton_download_button).append($apibutton_pusharia2_button).append($apibutton_link_button).append($apibutton_batchhttplink_button).append($apibutton_batchhttpslink_button);
            ...
            $apibutton_pusharia2_button.click(pusharia2Click);
            ...
            var $outerlinkbutton_pusharia2_button = $('<a id="pusharia2-outerlink" class="g-button-menu" href="javascript:void(0);">发送至aria2</a>');
            $outerlinkbutton_menu.append($outerlinkbutton_download_button).append($outerlinkbutton_link_button).append($outerlinkbutton_batchlink_button).append($outerlinkbutton_pusharia2_button);
            ...
            $outerlinkbutton_pusharia2_button.click(pusharia2Click);
        ...
        var ARIA2 = (function() {
            ...
            return function(jsonrpc_path) {
                ...
                this.addUri = function (uriList, options) {
                    request(this.jsonrpc_path, 'aria2.addUri', [uriList, options]);
                };
                return this;
            }
         })();
        
        function pusharia2Click(event){
            var id = event.target.id;
            var aria2 = new ARIA2(aria2RPC);
            if (id == 'pusharia2-api') {
                $.each(selectFileList,function(index,element){
                    var downloadLink = getDownloadLinkWithRESTAPIBaidu(element.path);
                    slog(downloadLink);
                    if (!downloadLink) return;
                    aria2.addUri([downloadLink, ], {out: element.filename, header: aria2Header});
                });
            }else if (id == 'pusharia2-outerlink') {
                $.each(selectFileList, function(index, element){
                    var uriList = [];
                    var result = getDownloadLinkWithClientAPI(element.path);
                    if (result.errno == 0) {
                        $.each(result.urls, function(i, url){
                            uriList.push(url.url);
                        });
                        slog(uriList);
                        if (uriList.length > 0)
                           aria2.addUri(uriList, {out: element.filename, header: aria2Header, "check-integrity": false});
                    }
                });                
            }
            alert('已成功发送至aria2');
