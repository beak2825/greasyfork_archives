// ==UserScript==
// @name           Orpheus Most Popular by Artist
// @namespace      orpheus.network
// @description    Lists the most snatched releases on artist pages
// @version        1.1.2
// @include        https://orpheus.network/artist.php*id=*
// @include        https://orpheus.network/collage*.php*id=*
// @include        https://redacted.ch/artist.php*id=*
// @include        https://redacted.ch/collage*.php*id=*
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/369925/Orpheus%20Most%20Popular%20by%20Artist.user.js
// @updateURL https://update.greasyfork.org/scripts/369925/Orpheus%20Most%20Popular%20by%20Artist.meta.js
// ==/UserScript==
    ...
    if (groupRows[i].classList.contains('group')) {
      ...
      while (row && !/\b(?:group|torrent)\b/.test(row.className)) {
        if (row.classList.contains('torrent_row')) {
          snatchCount += parseInt(row.cells[2].textContent.replace(',', ''), 10);
        }
      ...
      stats[groupId] = parseInt(groupRows[i].cells[4].textContent.replace(',', ''), 10);
    }
  }
...
function makeList(filt) {
  ...
  var sortedIds = Object.keys(groupStats).sort(function (a, b) {
