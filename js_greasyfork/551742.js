...
// @downloadURL https://update.greasyfork.org/scripts/551742/CmlPersonaUpworkAutomation.user.js
// @updateURL https://update.greasyfork.org/scripts/551742/CmlPersonaUpworkAutomation.meta.js
// ==/UserScript==
//const BASIC_URL = 'http://localhost:8080/';
//const PERSONA_AI_BASIC_URL = 'http://localhost:3000/';
...
async function prepareRequiredProposalDataForPersona() {
    ...
    //let upworkUserId = 'Serhii_Voloshyn';
...
async function enhanceJobDetailsPage() {
    ...
    let h1 = $("div[data-test=FlagButton]");
    console.log(h1, 'tumba jumba');
    if(h1.length === 0){
        console.log('entered here');
        const flagButton = $("button:contains('Flag as inappropriate')");
        h1 = flagButton.parent();
    }
...
function inferJobIdFromUrl() {
    if (isJobDetailsPage()) {
        ...
        const jobId = idPart.split("/")[0].split("?")[0];
        return jobId;
...
(async () => {
    if (location.host === "www.upwork.com") {
        ...
        //cloudflare page check
        function isCloudflarePage() {
            const bodyText = (document.body && document.body.innerText) || '';
            const title = document.title || '';
            if (title.includes('Checking your browser') || bodyText.includes('Checking your browser')) return true;
            if (document.querySelector('[id^="cf-challenge"]') || document.querySelector('.cf-browser-verification')) return true;
            return false;
        }
