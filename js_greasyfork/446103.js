// ==UserScript==
// @name           JVChat Mp Premium
// @description    Outil de discussion instantanée pour les messages de groupe privé sur le forum de Jeuxvideo.com
// @author         Blatomi
// @namespace      JVChatPrive
// @license        MIT
// @version        0.1.01
// @match          https://*.jeuxvideo.com/messages-prives/*
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/446103/JVChat%20Mp%20Premium.user.js
// @updateURL https://update.greasyfork.org/scripts/446103/JVChat%20Mp%20Premium.meta.js
// ==/UserScript==

/*
 Adaptation JVCHAT Premium pour qu'il marche avec les messages privés du forum Jeuxvideo.com
*/
...
#repondre-mp .jv-editor > .conteneur-editor {
    margin: 0;
    border: 0;
    padding: 0.5rem;
    line-height: normal;
...
let storageKey = "jvchat-prive-premium-configuration";
...
function tryCatch(func) {
    function wrapped(optArg) {
        try {
            ...
            let message = `Une erreur est survenue dans JVChat Mp Premium: '${err.message}' (function '${func.name}', line ${err.lineNumber})`;
...
function parseURL(url) {
    let regex = /^(.*?)(\?[\w=]*)([&#][\w=]*)([&#][\w=]*)?([&#][\w=]*)?$/i;
    let regexId = /id/i, regexFolder = /folder/i, regexLastmsg = /#?last_msg/i;
    let regexSpecial = /[\?&#]/i;
    let ids, offsetString, folder, lastmsg = "";
    let [_, domain, string1, string2] = url.match(regex);
    let search =  [string1, string2];

    search.forEach((item) => {
        if(item.match(regexId)) {
            ids = item.replace(regexSpecial, '');
        } else if(item.match(regexFolder)) {
            folder = item.replace(regexSpecial, '');
        }
    });

    offsetString = offsetNumberString();
    if (lastmsg = url.match(regexLastmsg))
        lastmsg = "#last_msg";
    
    return { domain: domain, ids: ids, offsetString: offsetString, folder: folder, lastmsg: lastmsg};
}
...
function buildURL(dict) {
    return `${dict.domain}?${dict.ids}&${dict.offsetString}&${dict.folder}${dict.lastmsg}`;
}
...
function clearPage(document) {
    ...
    let toolbar = document.getElementById("repondre-mp").getElementsByClassName("jv-editor-toolbar")[0];
    if (toolbar) {
...
function addJVChatButton(document) {
    ...
    document.head.insertAdjacentHTML("beforeend", css);
    let blocPreRight = document.getElementsByClassName("action-right")[1];
    let jvchatButton = makeJVChatButton();
    blocPreRight.insertAdjacentHTML('afterbegin', jvchatButton);
}
