...
// @downloadURL https://update.greasyfork.org/scripts/473564/%E6%98%BE%E7%A4%BARW%E5%A5%96%E5%8A%B1%E4%BB%B7%E5%80%BC.user.js
// @updateURL https://update.greasyfork.org/scripts/473564/%E6%98%BE%E7%A4%BARW%E5%A5%96%E5%8A%B1%E4%BB%B7%E5%80%BC.meta.js
// ==/UserScript==
    ...
    async function valueRow(row) {
        ...
        for (const item of awardsString.split(", ")) {
            if (item.includes("points")) {
                ...
                const resultString = item + " (" + numberFormatter(pointValue) + " each)<br>";
                ...
                totalBB += 60 * parseInt(item.substring(0, item.indexOf("x")));
                const resultString = item + " (" + numberFormatter(cacheValue) + " each)<br>";
                appendInnerHTML += resultString;
            } else if (item.includes("Melee Cache")) {
                const cacheValue = await valueCache(1119);
                rowTotalValue += cacheValue * parseInt(item.substring(0, item.indexOf("x")));
                totalBB += 30 * parseInt(item.substring(0, item.indexOf("x")));
                const resultString = item + " (" + numberFormatter(cacheValue) + " each)<br>";
                appendInnerHTML += resultString;
            } else if (item.includes("Small Arms Cache")) {
                const cacheValue = await valueCache(1120);
                rowTotalValue += cacheValue * parseInt(item.substring(0, item.indexOf("x")));
                totalBB += 20 * parseInt(item.substring(0, item.indexOf("x")));
                const resultString = item + " (" + numberFormatter(cacheValue) + " each)<br>";
                appendInnerHTML += resultString;
            } else if (item.includes("Medium Arms Cache")) {
                const cacheValue = await valueCache(1121);
                rowTotalValue += cacheValue * parseInt(item.substring(0, item.indexOf("x")));
                totalBB += 50 * parseInt(item.substring(0, item.indexOf("x")));
                const resultString = item + " (" + numberFormatter(cacheValue) + " each)<br>";
                appendInnerHTML += resultString;
            } else if (item.includes("Heavy Arms Cache")) {
                const cacheValue = await valueCache(1122);
                rowTotalValue += cacheValue * parseInt(item.substring(0, item.indexOf("x")));
                totalBB += 70 * parseInt(item.substring(0, item.indexOf("x")));
                const resultString = item + " (" + numberFormatter(cacheValue) + " each)<br>";
                appendInnerHTML += resultString;
            }
        }

        appendInnerHTML += "Total value: " + numberFormatter(rowTotalValue, 2) + " (" + totalBB + " Bunker Bucks)";
        appendInnerHTML += `</div>`;
