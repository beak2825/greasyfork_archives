// ==UserScript==
// @name         Pixelplace.fun BOT
// @namespace    pixelbot
// @version      1.1
// @description  A bot for Pixelplace.fun
// @author       Asuma & Matador & translation by Ajax
// @match        https://pixelplace.fun/*
// @match        http://pixelplace.fun/*
// @run-at       document-start
// @grant        GM_getValue
// @grant        GM_setValue
// @downloadURL https://update.greasyfork.org/scripts/377427/Pixelplacefun%20BOT.user.js
// @updateURL https://update.greasyfork.org/scripts/377427/Pixelplacefun%20BOT.meta.js
// ==/UserScript==
        ...
        if (fetchstat == 422) {
            ...
            if (p2 == 0)
                new Notification('PixelBot CAPTCHA!', {
                    body: "Place a pixel then solve a captcha"
                });
            captchaT = "(CAPTCHA)";
            console.log('CAPTCHA!!!');
            p2 = 1;
            ...
            cooldown = 15000;
            setStatus("Error, try again in 15s");
            setStatusColor("rgba(100, 10, 0, 0.75)");
            waiting(cooldown);
            timestampp = Date.now();
            return;
...
setInterval(function () {
    ...
    if (list.length == 0 && size != 0) {
        ...
        if (p1 == 0) {
            ...
            new Notification('PixelBot COMPLETE!', {
                body: "The bot has finished placing the pixels"
            });
        }
        p1 = 1;
        setStatusColor("rgba(0, 0, 0, 0.75)");
...
htmlFragment = '<div id="minimapbg"> <div class="posy" id="posyt"> <div id="minimap-text"></div><div id="minimap-box"> <canvas id="minimap"></canvas> <canvas id="minimap-board"></canvas> <canvas id="minimap-cursor"></canvas> </div><div id="minimap-config"> <span class="map-clickable" id="hide-map">Esconder</span> | <span class="map-clickable" id="follow-mouse">Seguir Mouse</span> | <span class="map-clickable" id="toggle-grid">Grade </span> | Zoom: <span class="map-clickable map-zoom" id="zoom-plus">+</span> | <span class="map-clickable map-zoom" id="zoom-minus">-</span> </div></div></div>';
window.addEventListener('load', function () {
    ...
    document.getElementById("minimap-box").style.display = "none";
    document.getElementById("minimap-text").style.display = "block";
    document.getElementById("minimap-text").innerHTML = "No template found";
}, false);
...
setInterval(function () {
    ...
    botTemplate.onload = function () {
        ...
        for (var i = 0; i < size; i++) {
            ...
            if (clr == -1) {
                alert('Cor não suportada em ' + xloop + ', ' + yloop + '. All unsupported pixels will be transparent');
                return;
            } else if (clr == 16)
                trnsprntPxls++;
            else
