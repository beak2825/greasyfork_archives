...
// @namespace    Shill Shocked
// @version      0.5
// @description  Download the currently playing song (or any song from the current tracklist) in any chosen format (standard, hq, etc) right to your computer for playing in any player (outside Deezer). Supports HQ even without Premium subscription. Also allows you to download an album at a time.
// @author       Shill Shocked
...
// @grant        GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/38796/Deezer%3A%20Download%2B.user.js
// @updateURL https://update.greasyfork.org/scripts/38796/Deezer%3A%20Download%2B.meta.js
// ==/UserScript==
...
// console.log(m.origin, m.data);
			var url = m.data[0], key = m.data[1], userdata = m.data[2];

			var rq = new XMLHttpRequest();
			rq.responseType = 'arraybuffer';
			rq.addEventListener('progress', function(e) { xhrProgress (e, userdata); }); // xhr.onprogress = xhrProgress;
			rq.addEventListener('load',     function(e) { xhrComplete (e, key, userdata); });
...
//		var name = generateName(obj, fmt, flags);
 ...
 console.log(msg);
  ...
  var downloadAll = D.createElement('a');
	
    var downloadAll = D.createElement('a');
	downloadAll.className = 'downAll';
	downloadAll.href = "#";
	downloadAll.innerHTML = 'Download all FLAC';
	downloadAll.onclick = function() {
	var flacLinks = window.document.querySelectorAll('#dltracklist .dl.flac');
	for (var i=0; i< flacLinks.length; i++){
		flacLinks[i].click();
	}
	return false; };
	...
	rootEl.appendChild(elH1);
  rootEl.appendChild(downloadAll);
