...
// @downloadURL https://update.greasyfork.org/scripts/530041/VoiceLinks-%E9%80%82%E9%85%8D%E6%9C%AC%E5%9C%B0.user.js
// @updateURL https://update.greasyfork.org/scripts/530041/VoiceLinks-%E9%80%82%E9%85%8D%E6%9C%AC%E5%9C%B0.meta.js
// ==/UserScript==
  ...
  `
    ...
    const Parser = {
        ...
        wrapRJCode: function (rjCode) {
            ...
            getXmlHttpRequest()({
                method: "GET",
                url: "http://127.0.0.1:21962/local/" + rjCode,
                responseType: "json",// 指定响应类型为 JSON
                onload: function (response) {
                    if (response.status === 200) {
                        // 处理响应数据（JSON 对象）
                        e.style.textDecoration = 'underline 3px green'
                        if ( response.response['media']==1) {
                            e.innerText='媒体 '+e.innerText

                        }
                        if (response.response['CHIT']==1) {
                            e.style.textDecoration = 'underline 3px violet'
                            e.innerText='繁中 '+e.innerText
                        }
                        if (response.response['CHIS']==1) {
                            e.style.textDecoration = 'underline 3px violet'
                            e.innerText='简中 '+e.innerText
                        }
                        if (response.response['CHIS']!=1 && response.response['CHIT']!=1 && response.response['subtitles']==1) {
                            e.style.textDecoration = 'underline 3px red'
                            e.innerText='字幕 '+e.innerText
                        }
