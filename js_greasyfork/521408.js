// ==UserScript==
// @name         RivalRegions auto script V3
// @namespace    http://tampermonkey.net/
// @version      3.2
// @description  HandsFree Script For RivalRegions!
// @author       TrapKingAstolDicc
// @match        https://rivalregions.com/
// @icon         https://www.google.com/s2/favicons?sz=64&domain=rivalregions.com
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/521408/RivalRegions%20auto%20script%20V3.user.js
// @updateURL https://update.greasyfork.org/scripts/521408/RivalRegions%20auto%20script%20V3.meta.js
// ==/UserScript==
...
// v2.12: add full auto option
// v3.0: add and refactor multiple experimental functions for marketing and working
// v3.1: fixing bug at working page
// v3.2: fixing bug when working at mars
...
(async function() {
    ...
    //Log in Option
    var logInOption = null;                      //Deprecated option due to facebook login failed, now only support alternative login
    ...
    var logInPassword = 'pa$$w0rd';              //Password that ramdomly create by game if you successfully add email with you acc, can be fondin your email, input should be with quote ''
    ...
    var upgradePerks = 1;                        //1 for str, 2 for edu, 3 for end
    ...
    var cycleTime = 300;                        //waiting time in each cycle (in seconds)
    var fullAuto = false;                       //for full auto war, set this option to true to override enableWork and enableTraining, this option WILL NOT help you hitting autoly, still need to click full auto in game manually
    ...
    async function doDrinks(limit){
        ...
        if(R>155){
            ...
            console.log('Steps:document.querySelector(\'[url="26"]\');');
            await sleep(1000);
            target.click();
            console.log('Steps:target.click();');
            await sleep(5000);
            var RPrice = parseInt(document.querySelectorAll('[class="dot"]')[1].innerHTML.replaceAll('.',''));
            console.log('Steps:var RPrice = parseInt(document.querySelectorAll(\'[class="dot"]\')[1].innerHTML.replaceAll(\'.\',\'\'));');
            target = document.getElementsByClassName('storage_sell dot');
            console.log('Steps:target = document.querySelector(\'[url="26"][class="storage_sell dot"]\');');
            console.log(target)
            if(target[0]!=null){
                ...
                target[0].click();
                await sleep(5000);
                console.log('RPrice=' + RPrice + 'LowestPrice=' + lowestRPrice);
                var input = document.getElementsByClassName('storage_sell_price')[0];
                if(input!=null){
                    if(RPrice > lowestRPrice){
                        input.value = RPrice-1;
                        console.log('Using RPrice-1');
                    }else{
                        input.value = lowestRPrice;
                        console.log('Using lowestRPrice');
                    }
                }else{
            ...
            if(target.length>0){
                ...
                if(target.length<1){
                    ...
                    if(enableDiggingRubbish && (loc == 'Mars' || loc == '火星')){
                        ...
                        document.querySelectorAll('[class="dot pointer hov2 list_name"]')[7].click();
                        console.log('picking R factory');
                        flag=0;
                        await sleep(3000);
                    }else{
                        console.log('location: ' + loc);
                        target=document.querySelectorAll('[class="dot pointer hov2 list_name"]')[rss];
                        if(target!=null){
                            target.click();
                            flag++;
                        }
                    ...
                    var tar = document.getElementsByClassName('button_green')[flag]
                    if(tar!=null){
                        tar.click();
                    }else{
                        console.log('Can\'t find button, often caused by wrong location, refreshing....')
                        await sleep(2000);
                        window.localStorage.setItem('step','perk');
                        location.href='https://rivalregions.com/#overview';
                        location.reload();
                    ...
                    await sleep(2000);
                    target = document.getElementsByClassName('work_w_autom')[0];
                    console.log('target = document.getElementsByClassName(\'work_w_autom\')[0];');
                    if(target!=null){
                        console.log('target!=null')
                        target.click();
                    }
