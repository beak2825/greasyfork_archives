...
// @version      2.5.0
// @description  在libulib网页上使用DeepSeek生成Stable Diffusion提示词，蝉蝉IP生成助手，支持高级提示词语法
// @author       You
...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/532772/%E8%9D%89%E4%B9%8B%E5%8A%A9.user.js
// @updateURL https://update.greasyfork.org/scripts/532772/%E8%9D%89%E4%B9%8B%E5%8A%A9.meta.js
// ==/UserScript==
...
(function() {
    ...
    function loadSystemPrompt() {
        ...
        // 使用通用模板作为默认系统提示词
        SYSTEM_PROMPT = SYSTEM_PROMPTS.general.systemPrompt;
    ...
    function createUserInputModal() {
        ...
        generateButton.addEventListener('click', () => {
            ...
            // 显示加载状态 - 使用与优化提示词一致的动画风格
            loadingContainer.style.display = 'block';
            loadingContainer.innerHTML = `
            <div class="loading-animation-container" style="
                text-align: center;
                margin: 20px 0;
                display: flex;
                ...
                width: 100%;
            ">
                <div class="loading-header" style="
                    display: flex;
                    align-items: center;
                    justify-content: center;
                    margin-bottom: 20px;
                ...
                ">
                    <div class="loading-spinner" style="
                        width: 30px;
                        height: 30px;
                        border: 3px solid rgba(255, 91, 46, 0.1);
                        border-radius: 50%;
                ...
                <div class="loading-steps" style="
                    ...
                    border-radius: 8px;
                ">
                    <div class="step step-1" style="margin: 5px 0; opacity: 1; font-weight: 500; color: #555;">✓ 解析用户输入</div>
                    <div class="step step-2" style="margin: 5px 0; opacity: 0.6;">⋯ 生成提示词中</div>
                    <div class="step step-3" style="margin: 5px 0; opacity: 0.4;">⋯ 优化关键词</div>
                    <div class="step step-4" style="margin: 5px 0; opacity: 0.2;">⋯ 返回最终结果</div>
                </div>
            </div>
            <style>
                @keyframes spinner {
            ...
            const updateLoadingStep = (step) => {
                const steps = loadingContainer.querySelectorAll('.loading-steps .step');
            ...
            // 更新步骤1（已在HTML中完成）
            setTimeout(() => updateLoadingStep(1), 500);  // 生成提示词中
            setTimeout(() => updateLoadingStep(2), 1500); // 优化关键词
            ...
            callDeepSeekAPI(finalInput, loadingContainer, activeTabId, apiOptions)
                .then(data => {
                    ...
                    // 添加完成动画
                    const loadingAnimContainer = loadingContainer.querySelector('.loading-animation-container');
                    if (loadingAnimContainer) {
                        const loadingText = loadingAnimContainer.querySelector('.loading-text');
                        if (loadingText) {
                            loadingText.textContent = '提示词生成成功！';
                            loadingText.style.color = '#4CAF50';
                            loadingText.style.fontWeight = 'bold';
                        }
                        
                        // 更改spinner为成功图标
                        const spinner = loadingAnimContainer.querySelector('.loading-spinner');
                        if (spinner) {
                            ...
                            spinner.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#4CAF50" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>`;
                ...
                .catch(error => {
                    ...
                    if (loadingAnimContainer) {
                        const loadingText = loadingAnimContainer.querySelector('.loading-text');
                        if (loadingText) {
                            loadingText.textContent = `生成失败: ${error.message}`;
                        ...
                        if (spinner) {
                            ...
                            spinner.style.alignItems = 'center';
                            spinner.style.justifyContent = 'center';
                            spinner.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#F44336" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>`;
                        }
                    } else {
                        loadingContainer.innerHTML += `
                        <div style="background-color: #fff2f2; padding: 10px; margin-top: 8px; border-radius: 8px; border-left: 3px solid #f44336;">
        ...
        return new Promise((resolve, reject) => {
            ...
            GM_xmlhttpRequest({
                ...
                onloadstart: function() {
                    // 不再直接添加到loadingContainer，等待onprogress创建响应容器
                },
                onprogress: function(response) {
                    ...
                    try {
                        // 确保有一个专门的响应内容容器
                        let responseContainer = loadingContainer.querySelector('.response-content');
                        if (!responseContainer) {
                            ...
                            responseContainer.style.fontSize = '13px';
                            responseContainer.style.color = '#555';
                            responseContainer.style.maxHeight = '150px';
                            responseContainer.style.overflowY = 'auto';
                            loadingContainer.appendChild(responseContainer);
                        }
                        ...
                        if (latestChunk && latestChunk !== '[DONE]') {
                            ...
                            if (parsedChunk.choices && parsedChunk.choices[0].delta.content) {
                                ...
                                span.textContent = parsedChunk.choices[0].delta.content;
...
1. **语法与权重控制**
   - **权重调节**: 使用(关键词:权重值)精确控制权重，如(chan:1.1)将权重设为1.1倍，建议范围0.3-1.3
   - **【重要】权重使用应当克制**: 仅对核心元素适度使用权重，chan建议使用(chan:1.1)，不要过度使用高权重，权重值建议控制在1.05-1.15范围内
   - **多层叠加**: 多层括号可累加权重，如((chan))提升至1.1025倍（1.05×1.05）
   - **下划线连接**: 使用happy_chan连接词组避免歧义，生成"快乐的蝉蝉"
   - **混合符号**: 使用[happy|sad]实现情绪变化效果

2. **流程控制**
   - **时间轴控制**: 使用[提示词:数值]指定生效阶段，例如[flower:0.7]表示采样70%后开始生成花朵
   - **多提示词交替**: [forest:beach:0.6]表示前60%在森林，后40%在海滩

3. **蝉蝉IP特定要求**
   - 保持提示词中"chan"相关元素不变，必须保持在提示词最前面，使用(chan:1.1)权重
   - 分析提示词的主题和意图，保持其核心内容
   - 识别用户的核心需求关键词，将其放在"chan"元素之后、提示词最前面
   - 仅对用户明确强调的1-2个最核心需求增加权重(1.05)，避免过度使用权重
   - 提示词顺序应按照相关性排序：chan > 用户核心需求 > 蝉蝉角色关键词 > 动作/情绪/场景 > 风格 > 技术描述
   - 移除冗余或效果不佳的关键词

4. **负向提示词处理策略**
   - 如果用户明确表示"不要X"或"我不希望出现X"，则在负向提示词中添加X并适当增加权重(1.05-1.1)
   - 如果是元素替换（如"将帽子换成头盔"），只需在正向提示词中替换并增加新元素权重，负向提示词无需添加被替换的元素
   - 除非用户特别强调，否则保持原有负向提示词，只添加用户明确指定不需要的元素
   - 确保负向提示词中保留防止生成低质量图像的常用词汇

5. **返回要求**
   - 返回格式必须是JSON，包含positive和negative字段
   - 不要添加解释，直接返回优化后的提示词
   - 所有提示词必须使用英文

返回的JSON格式示例：
{
  "positive": "(chan:1.1), (astronaut chan:1.05), space chan, cute, adorable, chibi, cartoon, anime style, [保持原有提示词，维持关键词优先级和权重]",
  "negative": "cropped, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer digits, bad feet, fewer toes, extra toes, [其他负向提示词]"
}`;
...
1. **语法与权重控制**
   - **权重调节**: 使用(关键词:权重值)精确控制权重，如(mail:1.1)将权重设为1.1倍，建议范围0.3-1.3
   - **【重要】权重使用应当克制**: 仅对用户特别强调或明确关注的核心图标主题使用权重，不要过度使用，权重值建议控制在1.05-1.15范围内
   - **多层叠加**: 多层括号可累加权重，如((icon))提升至1.1025倍
   - **下划线连接**: 使用email_icon连接词组避免歧义
   - **混合符号**: 使用[gradient|solid]实现渐变与纯色交替效果

2. **图标提示词要求**
   - 分析提示词的主题和意图，保持其核心内容
   - 识别用户的核心需求关键词（图标主题），将其放在提示词最前面
   - 仅对用户明确强调的1-2个最核心关键词增加权重(1.05-1.1)，避免过度使用权重
   - 提示词顺序应按照相关性排序：核心需求图标 > 商务图标关键词 > UI设计场景 > 风格描述 > 技术描述
   - 移除冗余或效果不佳的关键词
   - 确保提示词中包含以下UI设计常见场景提示词：
...
3. **负向提示词处理策略**
   - 如果用户明确表示"不要X"或"我不希望出现X"，则在负向提示词中添加X并适当增加权重(1.05-1.1)
   - 如果是元素替换（如"将邮件图标换成电话图标"），只需在正向提示词中替换并增加新元素权重，负向提示词无需添加被替换的元素
   - 除非用户特别强调，否则保持原有负向提示词，只添加用户明确指定不需要的元素
   - 确保负向提示词中保留防止图标出现文字和低质量元素的常用词汇
...
返回的JSON格式示例：
{
  "positive": "(email icon:1.1), mail icon, icon, business icon, simple, clean background, studio lighting, 3D rendering, vector graphics, professional UI",
  "negative": "text, words, letters, ugly, grainy, noisy, blurry, low resolution, oversaturated, cartoon, anime, sketchy, hand drawn"
}`;
            } else {
                // 通用优化提示词
                optimizeSystemPrompt = `你是一个专业的提示词优化专家，专注于优化Stable Diffusion的提示词。请遵循以下规则：
...
1. **语法与权重控制**
   - **权重调节**: 使用(关键词:权重值)精确控制权重，如(dog:1.1)将权重设为1.1倍，建议范围0.3-1.3
   - **【重要】权重使用应当克制**: 仅对用户特别强调或明确关注的核心元素使用权重，不要过度使用，权重值建议控制在1.05-1.15范围内
   - **多层叠加**: 多层括号可累加权重，如((dog))提升至1.1025倍（1.05×1.05）
   - **下划线连接**: 使用coffee_cake连接词组避免歧义，生成"咖啡蛋糕"而非分开的"咖啡和蛋糕"
   - **混合符号**: 使用[red|blue hair]实现红蓝发色交替效果

2. **流程控制**
   - **时间轴控制**: 使用[提示词:数值]指定生效阶段，例如[flower:0.7]表示采样70%后开始生成花朵
   - **多提示词交替**: [stone:flower:0.7]表示前70%生成石头，后30%生成花朵

3. **提示词结构与优化策略**
   - 分析提示词的主题和意图，保持其核心内容
   - 识别用户的核心需求关键词，将其放在提示词最前面
   - 仅对用户明确强调的1-2个最核心关键词增加权重(1.05-1.1)，避免过度使用权重
   - 提示词顺序应按照相关性排序：核心需求 > 重要特征 > 场景描述 > 风格 > 辅助描述
   - 移除冗余或效果不佳的关键词
   - 按"画质→风格→主体→场景"顺序排列，例如：masterpiece, best quality, cyberpunk style, 1girl in neon city
   - 前75个词影响最大，关键词应放在前面

4. **负向提示词处理策略**
   - 如果用户明确表示"不要X"或"我不希望出现X"，则在负向提示词中添加X并适当增加权重(1.05-1.1)
   - 如果是元素替换（如"将西装换成连衣裙"），只需在正向提示词中替换并增加新元素权重，负向提示词无需添加被替换的元素
   - 除非用户特别强调，否则保持原有负向提示词，只添加用户明确指定不需要的元素
   - 确保负向提示词中保留防止生成低质量图像的常用词汇
...
返回的JSON格式示例：
{
  "positive": "(rocket:1.1), space rocket, detailed character, full body, full shot, masterpiece, best quality, [保持原有提示词，维持关键词优先级和权重]",
  "negative": "cropped, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer digits, bad feet, fewer toes, extra toes, [其他负向提示词]"
}`;
            }

            const userPrompt = `请优化以下Stable Diffusion提示词，保持其主题和关键元素，但使用高级提示词语法使其更加精炼有效：

${currentPrompt}

请使用权重语法(keyword:weight)、多层括号((keyword))、下划线连接(coffee_cake)和混合符号[red|blue]等高级技巧，并确保使用适当的权重值(0.3-1.5)。`;
    ...
    function semanticOptimizePrompt(userDescription, currentPrompt, loadingContainer) {
        return new Promise((resolve, reject) => {
            try {
                // 检查用户输入
                if (!userDescription || !currentPrompt) {
                    return reject(new Error('用户描述或当前提示词为空'));
                }
                ...
                ">
                    ...
                    <div class="loading-steps" style="
                        ...
                        border-radius: 8px;
                    ">
                        <div class="step step-1" style="margin: 5px 0; opacity: 1; font-weight: 500; color: #555;">✓ 解析当前提示词</div>
                        <div class="step step-2" style="margin: 5px 0; opacity: 0.6;">⋯ 分析语义意图</div>
                        <div class="step step-3" style="margin: 5px 0; opacity: 0.4;">⋯ 优化关键词权重</div>
                        <div class="step step-4" style="margin: 5px 0; opacity: 0.2;">⋯ 生成最终结果</div>
                    </div>
                    ...
                    ">
                        <div style="font-weight: 500; margin-bottom: 5px;">您的原始提示词：</div>
                        <div style="color: #333; margin-bottom: 8px; white-space: pre-wrap;">${currentPrompt.positive || '无正向提示词'}</div>
                        <div style="font-weight: 500; margin-bottom: 5px; margin-top: 8px;">您的原始负向提示词：</div>
                        <div style="color: #333; margin-bottom: 8px; white-space: pre-wrap;">${currentPrompt.negative || '无负向提示词'}</div>
                        <div style="font-weight: 500; margin-bottom: 5px; margin-top: 8px;">您的用户描述：</div>
                        <div style="color: #333; white-space: pre-wrap;">${userDescription}</div>
                    </div>
                </div>
                <style>
                    @keyframes spinner {
                ...
                // 更新步骤1（已在HTML中完成）
                setTimeout(() => updateLoadingStep(1), 500); // 分析语义意图
                setTimeout(() => updateLoadingStep(2), 1500); // 优化关键词权重

                // 优化系统提示词，增强语义理解和权重分配能力
                const semanticOptimizeSystemPrompt = `你是一个AI提示词优化专家，擅长理解用户语义需求并转化为高质量的Stable Diffusion提示词。请遵循以下规则：

1. **语法与权重控制**
   - **权重调节**: 使用(关键词:权重值)精确控制权重，如(dog:1.1)，建议范围0.3-1.3
   - **【重要】权重使用应当极度克制**: 仅对用户明确强调的1-2个最核心元素使用权重，权重值建议控制在1.05-1.15范围内
   - **多层叠加**: 多层括号可累加权重，如((dog))提升至1.1025倍
   - **下划线连接**: 使用coffee_cake连接词组避免歧义
   - **混合符号**: 使用[red|blue hair]实现交替效果

2. **流程控制**
   - **时间轴控制**: 使用[提示词:数值]指定生效阶段。如[flower:0.7]表示采样70%后才开始出现花朵
   - **多提示词交替**: [stone:flower:0.7]表示前70%生成石头，后30%生成花朵

3. **语义理解与转化策略**
   - 分析用户描述中的语义意图，理解用户真正想要的是什么
   - 识别关键元素、特征、风格、场景等，转化为Stable Diffusion能理解的关键词
   - 识别用户特别强调的元素，仅对1-2个最核心元素适度增加权重(1.05-1.1)
   - 根据场景需求识别变化、转换或渐变的描述，使用适当的流程控制语法

4. **提示词结构与优先级**
   - 按相关性和重要性排序关键词
   - 前75个词影响最大，关键词应放在前面
   - 特别处理以下常见语义场景：
     - 风格转换：识别目标风格，使用(style:1.05)等适度加权
     - 主体替换：清晰定义新主体，保留原始上下文
     - 场景转换：检测时间/地点的转变，使用流程控制语法表达
...
6. **返回要求**
   - 返回优化后的提示词应保持与原始提示词的整体结构
   - 分析用户描述，将对应的高级语法要素融入现有提示词
   - 返回格式必须是JSON字符串，包含positive和negative两个字段

处理示例：
- 用户描述"我想要一张森林渐变成沙漠的图"→使用[forest:desert:0.5]
- 用户描述"让角色从开心变成悲伤"→使用[happy face:sad face:0.6]
- 用户描述"强调红色的头发"→使用(red hair:1.1)或((red hair))
- 用户描述"连接coffee和cake概念"→使用coffee_cake
- 用户描述"不要帽子"→在负向提示词中添加(hat:1.1)
- 用户描述"把西装换成连衣裙"→在正向提示词中替换suit为(dress:1.1)，负向提示词不变`;
...
请分析用户描述的语义意图，识别用户真正关心的核心元素，并有针对性地优化提示词。特别注意根据用户需求调整关键词的权重，并灵活运用括号语法、下划线连接、混合符号和时间轴控制等高级提示词技巧。

**【重要提醒】请务必确保所有带权重的关键词都使用正确的 (keyword:weight) 格式，例如 (cat:1.1)。检查你的输出！**`;
                    ...
                    onload: function(response) {
                        try {
                            ...
                            if (loadingAnimContainer) {
                                ...
                                const loadingText = loadingAnimContainer.querySelector('.loading-text');
                                if (loadingText) {
                                    loadingText.textContent = '正在处理优化结果...';
                                }
                                ...
                                setTimeout(() => {
                                    ...
                                    if (spinner) {
                                        ...
                                        spinner.style.animation = 'none';
                                        spinner.innerHTML = `
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#4CAF50" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
                            ...
                            if (!jsonData) {
                                try {
                                    ...
                                    // 尝试提取JSON对象
                                    // 首先，尝试捕获所有看起来像JSON对象的内容
                                    const potentialJson = fullContent.match(/\{[\s\S]*?\}/g);
                                    if (potentialJson) {
                                        // 从最长的匹配开始尝试，通常最复杂的JSON包含更多信息
                            ...
                            if (jsonData) {
                                ...
                                if (loadingAnimContainer) {
                                    const loadingText = loadingAnimContainer.querySelector('.loading-text');
                                    if (loadingText) {
                                        loadingText.textContent = '提示词优化成功！';
                                        loadingText.style.color = '#4CAF50';
                                        loadingText.style.fontWeight = 'bold';
                                    }
                                } else {
                                ...
                                if (loadingAnimContainer) {
                                    const loadingText = loadingAnimContainer.querySelector('.loading-text');
                                    if (loadingText) {
                                        loadingText.textContent = '无法提取有效的提示词数据';
                                        loadingText.style.color = '#F44336';
                                    }
                                    ...
                                    if (spinner) {
                                        ...
                                        spinner.style.animation = 'none';
                                        spinner.innerHTML = `
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#F44336" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
                                    ...
                                    loadingContainer.innerHTML += `
                                    <div style="color: #F44336; font-weight: bold; margin-top: 15px;">
                                        无法提取有效的提示词数据
                                    </div>`;
                                }
                                reject(new Error('无法从响应中提取有效的JSON数据'));
    ...
    const SYSTEM_PROMPTS = {
        ...
        general: {
            name: "通用模板",
            systemPrompt: `你是一个专业的提示词生成助手，需要为Stable Diffusion F.1模型生成高质量的提示词。请遵循以下规则：
...
3. **提示词结构**
   - 按"画质→风格→主体→场景"顺序排列
   - 例如: masterpiece, best quality, cyberpunk style, 1girl in neon city
   - 前75个词影响最大，关键词应放在前面

4. **提示词生成要求**
   - 提示词必须精炼，格式整齐，分成正向提示词和负向提示词两部分
   - 根据用户输入的需求自动发散提示词
   - 必须确保生成的是完整的全身形象，正向提示词中加入"full body, full shot"关键词
   - 在负向提示词中加入"cropped, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer digits, bad feet"
   - 返回格式必须是纯JSON字符串，包含positive和negative两个字段
   - 将用户提及的关键词放在提示词最前面，并仅对用户明确强调的元素适度增加权重(1.05-1.1)到(1.1)
   - 提示词顺序: 核心需求 > 重要特征 > 场景描述 > 风格 > 辅助描述

返回的JSON格式示例：
{
  "positive": "(rocket:1.1), (space rocket:1.05), detailed character, full body, full shot, [other positive prompts]",
  "negative": "cropped, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer digits, bad feet, fewer toes, extra toes, [other negative prompts]"
        ...
        bIcon: {
            name: "B端图标模板",
            systemPrompt: `你是一个专业的B端图标提示词生成助手，需要为Stable Diffusion生成高质量的商务图标提示词。遵循以下规则：
...
2. **流程控制 (虽然图标少用，但语法需知晓)**
   - **时间轴控制**: 使用[提示词:数值]指定生效阶段。例如[shining:0.8]表示采样80%后才开始闪耀。
   - **多提示词交替/渐变**: 使用[from:to:when]语法。例如，图标从草图变到最终形态: [sketch icon:final icon:0.5]

3. **图标提示词要求**
   - 必须生成简洁、商务风格的图标
   - 使用以下UI常见描述性术语：clean background, studio lighting, 3D rendering, minimalism, professional UI, vector graphics
   - 图标必须带有底座(base)和投影(shadow)
   - 仅为用户特别强调的核心图标主题提供适度权重(1.05-1.1)
   - 负向提示词中加入"text, words, letters, ugly, grainy, noisy, blurry"等防止出现文字和低质量元素

4. **提示词结构**
   - 按"图标主题→图标类型→风格→渲染细节→效果"顺序排列
   - 例如: (email:1.1), (mail icon:1.05), business icon, minimalistic, clean background, vector
   - 前50个词影响最大，关键词应放在前面
...
返回的JSON格式示例：
{
  "positive": "(email:1.1), (mail icon:1.05), business icon, clean minimalistic design, clean background, studio lighting, 3D rendering, soft shadows, vector graphics, professional UI",
  "negative": "text, words, letters, ugly, grainy, noisy, blurry, low resolution, oversaturated, cartoon, anime, sketchy, hand drawn"
}`
        },
        ...
        bIconNoBase: {
            name: "B端简约图标模板",
            systemPrompt: `你是一个专业的B端简约图标提示词生成助手，需要为Stable Diffusion生成高质量的简约商务图标提示词。遵循以下规则：
...
2. **流程控制 (虽然图标少用，但语法需知晓)**
   - **时间轴控制**: 使用[提示词:数值]指定生效阶段。例如[glowing:0.8]表示采样80%后才开始发光。
   - **多提示词交替/渐变**: 使用[from:to:when]语法。例如，图标从线框变到填充: [wireframe icon:filled icon:0.5]

3. **简约图标提示词要求**
   - 必须生成极简、现代风格的图标
   - 不带底座(no base)和复杂装饰
   - 使用flat design, minimal, clean lines等描述词
   - 仅为用户特别强调的核心图标主题提供适度权重(1.05-1.1)
   - 负向提示词中加入"text, base, pedestal, stand, shadow, complex, detailed, ornate"等

4. **提示词结构**
   - 按"图标主题→图标类型→极简风格→基本效果"顺序排列
   - 例如: (email:1.1), (mail icon:1.05), flat design, minimal, simple line art
   - 前50个词影响最大，关键词应放在前面
...
返回的JSON格式示例：
{
  "positive": "(email:1.1), (mail icon:1.05), flat design, minimal, simple line art, clean geometric shapes, solid colors, white background, professional UI, vector graphics",
  "negative": "text, words, letters, base, pedestal, stand, shadow, complex, detailed, ornate, ugly, grainy, noisy, blurry, low resolution"
}`
        },
        ...
        chanIp: {
            name: "蝉蝉IP模板",
            systemPrompt: `你是一个专业的蝉蝉IP提示词生成助手，需要为Stable Diffusion生成高质量的蝉蝉角色提示词。遵循以下规则：
...
2. **流程控制 (处理渐变/转变过程)**
   - **【重要】当用户描述包含"逐渐"、"慢慢"、"从A变成B"等渐变过程时，必须使用时间轴控制语法！**
   - **时间轴控制**: 使用[提示词:数值]指定生效阶段。例如[flower:0.7]表示采样70%后才开始出现花朵；[flower::0.7]则表示花朵只在前70%阶段生效。
   - **多提示词交替/渐变**: 使用[from:to:when]语法。例如，要实现"从普通蝉蝉在0.5步时变成石头蝉蝉"的效果，可以使用：[(normal chan):(stone chan):0.5]
   - 另一个例子：[forest:beach:0.6]表示前60%在森林，后40%在海滩。

3. **蝉蝉角色特定要求**
   - 必须将"chan"作为第一关键词，使用适度权重(chan:1.1)
   - 必须生成完整的蝉蝉角色，正向提示词中加入"full body, full shot"
   - 必须根据用户描述添加适当的动作、场景、表情等
   - 如有用户指定的手持物品，适度使用(holding xxx:1.05)格式
   - 情绪表达适度使用(happy:1.05)等格式
   - 风格描述适度使用(cute:1.05), (cartoon:1.05)等格式

4. **提示词结构**
   - 按"chan→用户核心需求→角色描述→动作/表情→场景→风格→效果"顺序排列
   - 例如: (chan:1.1), (explorer chan:1.05), astronaut, full body, jumping, happy face, space station, cute, cartoon style
   - 负向提示词中加入防止生成低质量图像的常用词
...
返回的JSON格式示例：
{
  "positive": "(chan:1.1), [(normal chan):(stone chan):0.5], full body, full shot, standing still, neutral expression, simple background, cute, cartoon style",
  "negative": "cropped, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer digits, bad feet, fewer toes, extra toes, ugly, deformed"
}`
        }
    };
