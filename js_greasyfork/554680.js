// ==UserScript==
// @name        Wanikani Open Framework - Menu module - test
// @namespace   rfindley
...
console.log('entering menu');
	...
	function install_scripts_header() {
        ...
        listener_history['scripts-menu'] = submenu_click;
	...
	function insert_script_link(config) {
        ...
        console.log('menu - insert_script_link', config);
        ...
        listener_history[link_id] = link_handler;
    ...
    window.addEventListener("turbo:load", (e) => {
        ...
        console.log('menu - in turbo load listener');
        listener_executing = true;
        retry = max_retry;
        setTimeout(reinstate_listeners, 0)
    });

    function reinstate_listeners(){
        let scripts_menu = document.getElementById('scripts-menu');
        if (scripts_menu === null) {
            retry--;
            if (retry > 0) {
                console.log('menu - scripts_menu null - retrying');
                setTimeout(reinstate_listeners, 100);
            } else {
                console.log('menu - scripts_menu null - retries exhausted');
                listener_executing = false;
            };
        }
        for (let key of listener_history.keys()){
            let submenu_element = document.querySelector('#'+key);
            if (submenu_element) {
                console.log('menu - processing element ',submenu_element);
                let event_listener = listener_history[key];
                submenu_element.removeEventListener(event_listener);
                submenu_element.addEventListener(event_listener);
            };
        };
        listener_executing = false;
    };
