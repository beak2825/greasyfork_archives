...
// @author       You
// @match        https://melbet.com/ru/office/history/
...
// @downloadURL https://update.greasyfork.org/scripts/408333/GetNames.user.js
// @updateURL https://update.greasyfork.org/scripts/408333/GetNames.meta.js
// ==/UserScript==
...
var done_counter = 0;
        ...
        for (var i = 0; i < 7; i++) {
          if (
            datas[i].getElementsByClassName("kof")[0].style.background == "rgb(85, 192, 20)" 
            )
            done_counter++;
            if (
              datas[i].getElementsByClassName("kof")[0].style.background != "rgb(85, 192, 20)" &&
              datas[i].getElementsByClassName("kof")[0].style.background != "rgb(249, 112, 112)"
              )
...
// alert(typeof(localStorage.getItem('CheckNotificationsSend')))
  ...
  {
    $.ajax({ type : "POST", url : "https://api.pushover.net/1/messages.json", data : 
  {   token: settings_gmc.get('token'), 
    user: settings_gmc.get('id'), 
    message: "Все ставки сыграли", 
...
clearInterval(timer);
       ...
       document.title = done_counter + ready_or_not;
        ...
        document.querySelector("body > div.contentWrap.clear > div > div > div.blockContent > div > div.colCont.fl > div > div.colContName.clear > div").textContent=
        `unplayed ${unplayed} completed ${completed} won ${won}  Live ${ready_or_not}`;
      
      else document.querySelector("body > div.contentWrap.clear > div > div > div.blockContent > div > div.colCont.fl > div > div.colContName.clear > div").textContent=
      `Live ${ready_or_not} (${done_counter})`;
  ...
  var gametype = "";
  if (typeof localStorage.getItem("gametype") !== 'undefined' || localStorage.getItem("gametype") !== null)
  ...
  function detectGame(str) {
    ...
    search = "fifa";
    if (str.includes(search)) result += " #киберспорт";
  ...
  function detectSmile(str) {
    ...
    if (str.includes(search)) result += "🏅 ";
    search = "убок";
    if (str.includes(search) || str.includes("League")) result += "🏆 ";

    search = "Англи";
    if (str.includes(search)) result += "🏴󠁧󠁢󠁥󠁮󠁧󠁿 ";

    search = "Германии";
    if (str.includes(search) || str.includes("BudnesLiga")) result += "🇩🇪 ";
    ...
    search = "ига Про";
    if (str.includes(search) || str.includes("Итали")) result += "🇮🇹 ";
...
if (str.length <= 29 ) {
  ...
  document.onkeyup = function (e) {
    var key = e.keyCode;
    console.log("%c 🚛: document.onkeyup -> e.keyCode ", "font-size:16px;background-color:#dd4a43;color:white;", e.keyCode)
    //alert(key);
    //-48
    if (key == 80){//p
      var arr = document.querySelector("#contentHistory > ul > li:nth-child(1) > div.more > table > tbody > tr:nth-child(1) > td.name").textContent.split("\n").map(function(item) {
        return item.trim();
      });
...
console.log("%c 🇲🇼: document.onkeyup -> time ", "font-size:16px;background-color:#5fa368;color:white;", time)

navigator.clipboard.writeText(`📌Сегодня играют #`+camelize(teams[0]).replace(" ","")+` 🆚 #`+camelize(teams[1]).replace(" ","").substring(0,camelize(teams[1]).replace(" ","").indexOf(".")-2)+` / Начало в ` +time+ ` (по мск) ⚽️🔥

✅Я ставлю на `+document.querySelector("#contentHistory > ul > li:nth-child(1) > div.more > table > tbody > tr:nth-child(1) > td:nth-child(2)").textContent+`! 💰

#новости #прогноз #футбол #ставки #`+arr[1].replace(/[^а-яА-Яa-zA-Z0-9.]/g, "_"));
...
//alert("c space!");
      ...
      result +=
        ...
        convertHTML(
          stavka +
            "\r\n\r\n#line #линия #ставки #футбол #" +game.replace(/[^а-яА-Яa-zA-Z0-9.]/g, "")
            //  +" #" +
    ...
    if ( key == 65)//a
    {
      navigator.clipboard.writeText(`🔶Получить 10400 рублей🔶 - https://beto4ka.generalmirror.site/ 
...
if (gametype=="баскетбол")
      teams_smile = "🏀";

if (gametype=="волейбол")
      teams_smile = "🏐";
if (gametype=="теннис")
      teams_smile = "🎾";      
if (gametype=="футбол")
  ...
  function lookForStakes() {
    ...
    arrStakes.forEach(stake => {
            ...
            for (var i = 0; i < datas.length; i++) {
              ...
              if(datas[i].getElementsByClassName("number")[0].textContent.indexOf(stake)>-1)
              { 
                if(datas[i].getElementsByClassName("kof")[0].style.background ==     "rgb(85, 192, 20)")
                {
                won++;
                completed++;
              }
              else if(datas[i].getElementsByClassName("kof")[0].style.background ==     "rgb(249, 112, 112)")
    ...
    console.log("%c 🎆: lookForStakes -> unplayed ", "font-size:16px;background-color:#b69ae8;color:white;", `Line: unplayed ${unplayed} completed ${completed} won ${won} `)
...
if (completed===arrStakes.length && JSON.stringify(arrStakes).length>3)
{
  ...
  {    token: settings_gmc.get('token'), 
  user: settings_gmc.get('id'), 
    message: `completed ${completed} won ${won} `, 
    device: settings_gmc.get('notificationsSend'), 
    title: "Линии готовы!!!", 
    priority: 0, 
    sound: "classical",
 ...
 settings_gmc = new GM_configStruct(
    {
      ...
      'fields': // Fields object
      {
        ...
        'listMonitor': // This is the id of the field
        {'labelPos': 'above',
          ...
          'default': 'введите номера купонов которые надо мониторить\r\nкаждый в отдельной строке' // Default value if user doesn't change it
        }
        ,
        'liveMonitor': // This is the id of the field
        {'labelPos': 'above',
      ...
      ,
        ...
        'events': {
            'init': function() {
                ...
                this.set('notificationsSend', localStorage.getItem('notificationsSend').toString());
                (localStorage.getItem('CheckNotificationsSend') == "true") ? this.set('CheckNotificationsSend', true) : this.set('CheckNotificationsSend', false)
// Object.keys(this)
console.log("%c 🍵: appendButton -> Object.keys(this) ", "font-size:16px;background-color:#3a953f;color:white;", Object.keys(this))
        ...
        'save': function(){
         ...
         localStorage.setItem('CheckNotificationsSend', this.get('CheckNotificationsSend'));
           
            localStorage.setItem('notificationsSend', this.get('notificationsSend'));
...
document.querySelector("#filters > div.filterDate.clear > input.but.greenBut.fl.del.delete_history").after(btn_open_settings);
}
