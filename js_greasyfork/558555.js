// ==UserScript==
// @name         Bitcointalk Board Stats 4 (con Download Cards e Top 3 Merit Sender)
// @namespace    http://tampermonkey.net/
// @version      4.3.3
// @description  Statistiche board Bitcointalk con cards colorate, download come immagine, percentuale post con Merit, grafico Merit giornalieri, top threads per merit e tutte le statistiche originali.
// @author       Ace
// @match        https://bitcointalk.org/*
// @grant        GM_xmlhttpRequest
// @grant        fetch
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/558555/Bitcointalk%20Board%20Stats%204%20%28con%20Download%20Cards%20e%20Top%203%20Merit%20Sender%29.user.js
// @updateURL https://update.greasyfork.org/scripts/558555/Bitcointalk%20Board%20Stats%204%20%28con%20Download%20Cards%20e%20Top%203%20Merit%20Sender%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    // Funzione per caricare html2canvas dinamicamente
    ...
    // Funzione per scaricare le cards come immagine
    function downloadCardsAsImage() {
    // Cerca il contenitore delle cards (modifica il selettore)
    const cardsContainer = document.querySelector('#fake-stat-page div[style*="grid-template-columns"]');
    if (!cardsContainer) {
        alert("Nessuna card trovata da scaricare! Assicurati che le statistiche siano state generate.");
        return;
    }

    // Nascondi temporaneamente il pulsante di download per evitare che venga catturato
    const downloadButton = document.querySelector('#download-cards');
    if (downloadButton) downloadButton.style.display = 'none';

    // Usa html2canvas per catturare le cards
    html2canvas(cardsContainer, {
        scale: 2,
        backgroundColor: '#ffffff', // Sfondo bianco per le cards
        logging: false,
        useCORS: true,
        allowTaint: true
    }).then(canvas => {
        ...
        link.download = `Cards_Statistiche_${new Date().toISOString().split('T')[0]}.png`;
        link.href = canvas.toDataURL('image/png');
        link.click();
    }).catch(err => {
        console.error("Errore durante la generazione dell'immagine:", err);
        alert("Errore durante il download. Controlla la console per dettagli.");
    ...
    async function fetchMeritBatchData(boardIds, startDate, endDate) {
        const payload = {
            ...
            "1": { "json": { "date_min": startDate, "date_max": endDate, "board_id": boardIds, "limit": 1000 } },
            "2": { "json": { "date_min": startDate, "date_max": endDate, "board_id": boardIds, "limit": 1000 } },
            "3": { "json": { "date_min": startDate, "date_max": endDate, "board_id": boardIds, "limit": 1000 } },
            "4": { "json": { "date_min": startDate, "date_max": endDate, "board_id": boardIds, "interval": "day", "limit": 1000 } },
            "5": { "json": { "date_min": startDate, "date_max": endDate, "board_id": boardIds, "interval": "day", "limit": 1000 } },
            "6": { "json": { "date_min": startDate, "date_max": endDate, "board_id": boardIds, "limit": 1000 } },
            "7": { "json": { "date_min": startDate, "date_max": endDate, "board_id": boardIds, "limit": 1000 } },
            "8": { "json": { "date_min": startDate, "date_max": endDate, "board_id": boardIds, "limit": 1000 } }
        };
    ...
    // Funzione per recuperare i top threads per merit ricevuti
    async function fetchTopThreadsByMerit(boardId, startDate, endDate, childBoards = true, limit = 10) {
        try {
            const payload = {
                "0": {
                    "json": {
                        "date_min": startDate,
                        "date_max": endDate,
                        "board_id": childBoards ? [28, 170, 115, 132, 144, 145, 153, 162, 165, 169, 175, 205, 46, 107, 171, 172, 173, 176, 200] : [boardId],
                        "limit": 10000
                    }
                }
            };

            const endpoint = `https://bitlist.co/trpc/merits.top_posts_by_merit_count?batch=1&input=${encodeURIComponent(JSON.stringify(payload))}`;
            ...
            const response = await new Promise((resolve) => {
                GM_xmlhttpRequest({
                    ...
                    url: endpoint,
                    onload: resolve,
                    onerror: (e) => resolve({ responseText: JSON.stringify({ error: e }) })
                });
            });

            const data = JSON.parse(response.responseText);
            const postsWithMerit = data[0]?.result?.data?.json || [];

            const threads = {};
            postsWithMerit.forEach(post => {
                ...
                threads[topicId].totalMerits += post.total_merits || 0;
                threads[topicId].postIds.add(post.post_id);
            });

            for (const topicId in threads) {
                threads[topicId].postCount = threads[topicId].postIds.size;
            }
            ...
            console.error("Errore fetchTopThreadsByMerit:", err);
            return [];
        }
    }
    ...
    function createStatPage() {
        ...
        page.innerHTML = `
            <div style="max-width: 950px; margin: 20px auto; background: white; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.2); border: 1px solid #ddd;">
                ...
                <div style="margin-bottom: 20px; border: 1px solid #ddd; padding: 10px; border-radius: 4px;">
                    ...
                    <label style="display: block; margin-bottom: 5px;">
                        <input type="checkbox" id="show-top-threads-by-merit" checked> Top 10 Thread per Merit Ricevuti
                    </label>
                    <label style="display: block; margin-bottom: 5px;">
                ...
                <div id="stats-output" style="margin-top: 20px; padding: 15px; background: #f5f5f5; border-radius: 4px; border: 1px solid #ddd; min-height: 100px; display: none;">
                    ...
                    <button id="download-cards" style="display: block; margin: 10px auto 0; padding: 8px 16px; background: #2e3b4e; color: white; border: none; border-radius: 4px; cursor: pointer;">Scarica Cards come Immagine</button>
                </div>
        ...
        document.querySelector('#download-cards').onclick = downloadCardsAsImage;
    ...
    async function generateStats() {
        ...
        try {
            ...
            const showTopThreadsByMerit = document.querySelector('#show-top-threads-by-merit').checked;
            ...
            if (showSummary) {
                ...
                summaryText += `
                    <div style="margin-top: 20px;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 15px; margin-bottom: 20px;">
                            ...
                            <!-- Card 6: Utenti Attivi -->
                            <div style="background: linear-gradient(135deg, #e0f7fa 0%, #b2ebf2 100%); border-radius: 8px; padding: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                                <h5 style="margin: 0 0 10px 0; color: #00838f; font-size: 14px; display: flex; align-items: center;">
                                    <span style="margin-right: 8px;">👥</span> Utenti Attivi
                                </h5>
                                <div style="text-align: center; font-size: 20px; font-weight: bold; color: #00838f; margin: 10px 0;">
                                    ${allAuthors.length}
                                </div>
                                <div style="text-align: center; font-size: 12px; color: #666;">
                                    (almeno 1 post in ${currentMonthName})
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="margin-top: 20px; padding: 15px; background: #f9f9f9; border-radius: 5px; border: 1px solid #ddd;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 10px;">
                                ...
                                <div>
                                    ...
                                    <p style="margin-bottom: 5px;"><strong>Utenti attivi (almeno 1 post):</strong> ${allAuthors.length}</p>
                                </div>
                            </div>
                ...
                // BBCode sintesi (con Utenti Attivi e Top 3 Merit Sender)
                summaryBBCode += `
...
${childBoards ? `
[b]Post per childboard (${currentMonthName}):[/b]
[table]
[tr]
[td][b]Board[/b][/td]
[td][b]Post[/b][/td]
...
[tr]
[td]${data.name}[/td]
[td]${data.currentPosts}[/td]
[td][color=${data.variation >= 0 ? 'green' : 'red'}]${data.variation >= 0 ? '+' : ''}${data.variation}[/color][/td]
[/tr]
`).join('')}
[/table]
` : ''}
...
[b]Top 3 Rapporto Merit:[/b] ${top3Ratio.map(u => u.user).join(', ')}
            ...
            let topThreadsByMeritPreview = ``;
            let topThreadsByMeritBBCode = ``;
...
[/tr]
            ...
            // Genera top threads per merit (se richiesto)
            if (showTopThreadsByMerit) {
                const topThreadsByMerit = await fetchTopThreadsByMerit(boardId, currentStart.split('T')[0], currentEnd.split('T')[0], childBoards, 10);

                topThreadsByMeritPreview = `
                    <div style="margin-top: 20px;">
                        <h4 style="text-align: center; margin-bottom: 10px;">Top 10 Thread per Merit Ricevuti</h4>
                        <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                            <tr style="background: #f0f0f0;">
                                <th style="padding: 8px; text-align: center; border: 1px solid #ddd;">Pos.</th>
                                <th style="padding: 8px; text-align: left; border: 1px solid #ddd;">Thread</th>
                                <th style="padding: 8px; text-align: center; border: 1px solid #ddd;">Merit Totali</th>
                                <th style="padding: 8px; text-align: center; border: 1px solid #ddd;">Post</th>
                                <th style="padding: 8px; text-align: center; border: 1px solid #ddd;">Link</th>
                            </tr>
                `;

                topThreadsByMerit.forEach((thread, index) => {
                    topThreadsByMeritPreview += `
                        <tr style="border: 1px solid #ddd;">
                            <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">${index + 1}.</td>
                            <td style="padding: 8px; border: 1px solid #ddd;">${thread.title}</td>
                            <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">${thread.totalMerits}</td>
                            <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">${thread.postCount}</td>
                            <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">
                                <a href="https://bitcointalk.org/index.php?topic=${thread.id}" target="_blank">Apri</a>
                            </td>
                        </tr>
                ...
                topThreadsByMeritPreview += `</table></div>`;

                // BBCode
                topThreadsByMeritBBCode = `
[center][b]Top 10 Thread per Merit Ricevuti[/b][/center]
[table]
[tr]
[td][b]Pos.[/b][/td]
[td][b]Thread[/b][/td]
[td][b]Merit Totali[/b][/td]
[td][b]Post[/b][/td]
[td][b]Link[/b][/td]
[/tr]
                `;
...
[/tr]
            ...
            setTimeout(() => {
                ...
                document.querySelector('#preview-table').innerHTML =
                    ...
                    topThreadsPreview +
                    topMeritedPostsPreview +
                    (showTopThreadsByMerit ? topThreadsByMeritPreview : '');
                ...
                document.querySelector('#bbcode-output').value =
                    ...
                    topThreadsBBCode +
                    topMeritedPostsBBCode +
                    (showTopThreadsByMerit ? topThreadsByMeritBBCode : '');
    ...
    window.downloadCardsAsImage = downloadCardsAsImage;
})();