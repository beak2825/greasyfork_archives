// ==UserScript==
// @name            FUQ Blacklister
// @version         0.9
// @description     Delete unwanted fuq.com videos with a list of blacklisted keywords (see code for the list)
// @description:de  Unerwünschte Videos mit einer Blacklist von fuq.com entfernen (siehe Code für die Liste)
// @description:fr  Supprimer les vidéos indésirables de fuq.com avec une liste noire (voir le code pour la liste)
// @description:it  Rimuovere i video indesiderati da fuq.com con una lista nera (vedi codice per la lista)
// @author          J.H
// @include         *fuq.com*
// @icon            https://www.google.com/s2/favicons?sz=64&domain=fuq.com
// @grant           none
// @license         MIT
// @run-at          document-end
// @namespace       https://greasyfork.org/users/448067
// @downloadURL https://update.greasyfork.org/scripts/445563/FUQ%20Blacklister.user.js
// @updateURL https://update.greasyfork.org/scripts/445563/FUQ%20Blacklister.meta.js
// ==/UserScript==
...
const colorPrint = function (videoTitle, ...args) { // debug info printing function
    ...
    console.log('%c[FUQ Blacklister]:%c ' + videoTitle + ' %c[removed]', clr_s[2], clr_s[1], ...args, clr_s[3]);
}, clr_s = ["color: red", "color: white", "color: #f90", "color: green"]; // color settings (for debug info)

const divs = document.querySelectorAll(".card-container.sub");
divs.forEach((div) => { // loop through all elements from the divs and find the title
    ...
    let videoTitle = div.querySelector("a").getAttribute("title");
