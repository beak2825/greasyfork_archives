// ==UserScript==
// @name         烟草网络学院完整循环学习助手 v8.0（目录扫描+视频播放+循环返回+隐藏目录展开）
...
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/550005/%E7%83%9F%E8%8D%89%E7%BD%91%E7%BB%9C%E5%AD%A6%E9%99%A2%E5%AE%8C%E6%95%B4%E5%BE%AA%E7%8E%AF%E5%AD%A6%E4%B9%A0%E5%8A%A9%E6%89%8B%20v80%EF%BC%88%E7%9B%AE%E5%BD%95%E6%89%AB%E6%8F%8F%2B%E8%A7%86%E9%A2%91%E6%92%AD%E6%94%BE%2B%E5%BE%AA%E7%8E%AF%E8%BF%94%E5%9B%9E%2B%E9%9A%90%E8%97%8F%E7%9B%AE%E5%BD%95%E5%B1%95%E5%BC%80%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/550005/%E7%83%9F%E8%8D%89%E7%BD%91%E7%BB%9C%E5%AD%A6%E9%99%A2%E5%AE%8C%E6%95%B4%E5%BE%AA%E7%8E%AF%E5%AD%A6%E4%B9%A0%E5%8A%A9%E6%89%8B%20v80%EF%BC%88%E7%9B%AE%E5%BD%95%E6%89%AB%E6%8F%8F%2B%E8%A7%86%E9%A2%91%E6%92%AD%E6%94%BE%2B%E5%BE%AA%E7%8E%AF%E8%BF%94%E5%9B%9E%2B%E9%9A%90%E8%97%8F%E7%9B%AE%E5%BD%95%E5%B1%95%E5%BC%80%EF%BC%89.meta.js
// ==/UserScript==
...
(function () {
    ...
    // 自动展开章节内容
    async function expandChapterSections() {
        currentOperation = '展开章节内容';
        ...
        try {
            ...
            // 查找所有章节箭头图标（未展开状态）
            const arrows = document.querySelectorAll('i.iconfont.icon-triangle-down.arrow');
            let clickedCount = 0;
            
            arrows.forEach(arrow => {
                // 确保箭头是可见的并可点击
                if (arrow.offsetParent !== null && typeof arrow.click === 'function') {
                    arrow.click();
                    clickedCount++;
                    log(`点击展开第 ${clickedCount} 个章节`);
                }
            });
            
            if (clickedCount > 0) {
                log(`已自动展开 ${clickedCount} 个章节`, 'success');
            ...
            log(`展开章节失败: ${error.message}`, 'error');
        }
    }

    // 扫描展开后的必修内容
    async function scanExpandedContent() {
        currentOperation = '扫描展开的必修内容';
        ...
        try {
            ...
            // 查找所有视频状态元素
            const videoItems = document.querySelectorAll('.video-status, .lesson-item, .chapter-item, .content-item');
            let foundRequiredUnfinished = false;
            ...
            for (let item of videoItems) {
                ...
                const isRequired = itemText.includes('[必修]') || 
                                 itemText.includes('必修') ||
                                 item.classList.contains('required') ||
                                 item.querySelector('.required');
                
                // 跳过选修内容
                const isElective = itemText.includes('[选修]') || 
                                 itemText.includes('选修') ||
                                 item.classList.contains('elective');
                
                if (isElective) {
                    log(`跳过选修内容: ${itemText.substring(0, 30)}...`);
                    continue;
                }
                
                if (isRequired) {
                    // 检查学习状态
                    const isReviewed = itemText.includes('复习') || 
                                     itemText.includes('已完成') || 
                                     itemText.includes('已学完') ||
                                     itemText.includes('100%') ||
                                     item.classList.contains('completed');
                    
                    const isLearning = itemText.includes('学习中') || 
                                     itemText.includes('未完成') ||
                                     item.classList.contains('learning');
                    
                    log(`必修内容: ${itemText.substring(0, 40)}... | 复习: ${isReviewed} | 学习中: ${isLearning}`);
                    ...
                    // 如果发现未完成的必修内容，点击进入学习
                    if (!isReviewed && (isLearning || !itemText.includes('已'))) {
                        log(`发现未完成的必修内容，准备点击学习: ${itemText.substring(0, 30)}...`, 'success');
                        ...
                        foundRequiredUnfinished = true;
                        log(`已点击必修学习内容`, 'success');
                        break; // 一次只处理一个
                    }
                }
            }
            
            if (!foundRequiredUnfinished) {
                log('所有展开的必修内容都已完成，检查整体课程状态');
                checkAllRequiredCompleted();
            }
            
        } catch (error) {
            log(`扫描展开内容失败: ${error.message}`, 'error');
        }
    }

    // 检查所有必修内容是否都已完成
    async function checkAllRequiredCompleted() {
        currentOperation = '检查所有必修完成状态';
        
        try {
            log('检查当前课程所有必修内容完成状态...');
            
            // 方法1: 检查视频状态
            const statusList = await getStatusList();
            if (statusList.length > 0 && allReviewed(statusList)) {
                log('通过状态列表确认：所有节次已复习', 'success');
                completedCourses.add(currentCourseTitle);
                jumpToCatalog();
                return;
            }
            ...
            // 方法2: 检查页面中的必修内容状态
            const allRequiredItems = document.querySelectorAll('.video-status, .lesson-item, .chapter-item, .content-item');
            const requiredItems = Array.from(allRequiredItems).filter(item => {
                const itemText = item.textContent || '';
                return (itemText.includes('[必修]') || itemText.includes('必修')) && 
                       !itemText.includes('[选修]') && !itemText.includes('选修');
            });
            
            if (requiredItems.length > 0) {
                const allRequiredReviewed = requiredItems.every(item => {
                    const itemText = item.textContent || '';
                    return itemText.includes('复习') || 
                           itemText.includes('已完成') || 
                           itemText.includes('已学完') ||
                ...
                log(`必修内容总数: ${requiredItems.length}, 全部完成: ${allRequiredReviewed}`);
                
                if (allRequiredReviewed) {
                    log('通过页面检查确认：所有必修内容已复习', 'success');
                    completedCourses.add(currentCourseTitle);
                    jumpToCatalog();
                    return;
                }
            ...
            // 如果检查结果不明确，等待一段时间后重新检查
            log('必修完成状态不明确，继续等待或重新检查');
            setTimeout(() => {
                expandChapterSections(); // 重新展开和检查
            }, 3000);
            
        } catch (error) {
            log(`检查必修完成状态失败: ${error.message}`, 'error');
        }
    }
