...
// @version       1.2.4
// @downloadURL https://update.greasyfork.org/scripts/7363/YouScrobbler.user.js
// @updateURL https://update.greasyfork.org/scripts/7363/YouScrobbler.meta.js
// ==/UserScript==
...
function updateUrl () {
	try {
		return (us_getValue("beta-channel") ?  "http://youscrobbler.lukash.de/currentversion/beta" : "http://youscrobbler.lukash.de/currentversion");
	} catch (e) {
...
function scriptDownloadUrl () {
	try {
		if (Math.random() >= 0.4) {
			return (us_getValue("beta-channel") ? "http://youscrobbler.lukash.de/youscrobbler_beta.user.js" : "http://youscrobbler.lukash.de/youscrobbler.user.js");
		} else {
			return (us_getValue("beta-channel") ? "http://youscrobbler.lukash.de/youscrobbler_beta.user.js" : "http://adf.ly/rNGQj");
		}
	} catch (e) {
...
function us_addButton() {
    ...
    el.innerHTML =	'.us_box { border-radius: 5px; border: 5px solid #333; background: #fff;'+
					...
					'#us_icon_small { vertical-align: text-bottom;}}'+ 
					'#foundInDBIcon { }'+
...
function us_boxcontent(title,content) {
	...
	loginbox.innerHTML = '<h3 id="us_box_head">'+title+'<ul><li><a href="#" title="Close" id="us_box_close"></a></li><li><a href="#" title="Settings" id="us_box_settings"></a></li><li><a href="#" title="Help" id="us_box_help"></a></li></ul></h3>'+
	'<div>'+content+'</div>';
...
function us_settings(updated) {
		...
		var cont =  '<div id="us_loginbox_form" style="text-align:left"><form name="us_settings_form" onSubmit="return false"><table style="table-layout:fixed"><tr><td class="us_settings_grp us_settings_grp_left">'+
					...
					'<span>Version: '+VERSION+'</span><br/><label for="beta-channel">Beta-Channel</label><input type="checkbox" name="beta-channel" id="beta-channel"/><br/><span id="us_manualupdate"><a href="#" id="us_manualupdate_link">Check for Update</a></span></td></tr></table> '+ notice +
		...
		if (us_getValue("beta-channel")){document.getElementById('beta-channel').setAttribute("checked", 'checked');}
		...
		function us_savesettings() {
			...
			us_saveValue("beta-channel", document.getElementById("beta-channel").checked);
...
function us_scrobble(artist,track,album,mbid,retry,queued,auto) {
	...
	"&timestamp=" + us_getTempData("us_playstart_s") + "&track=" + encodeURIComponent(track);
	if (album != 0 && album != "") {
		album==false;
...
function us_scrobblenp(retry) {
	...
	if ((formArtist!="") && (formTrack!="")) {
		if ((formArtist!=decodeURIComponent(us_getTempData("artist")) || formTrack!=decodeURIComponent(us_getTempData("track")))) {
		...
		if (!album) { var album = ''; } else {us_saveTempData("album", encodeURIComponent(album));}
		var mbid = document.forms[0].elements[3].value;
		if (!mbid) { var mbid = ''; } else {us_saveTempData("mbid", encodeURIComponent(mbid));}
		us_scrobble(decodeURIComponent(us_getTempData("artist")),decodeURIComponent(us_getTempData("track")),album,mbid,0,0,0);
	} //empty input
}
...
function us_ajax_scanner () {
	...
	if (leftToPlay <= 0 && us_getTempData("scrobbled") != 1 && TO1Helper) {
		...
		us_scrobble(decodeURIComponent(us_getTempData("artist")), decodeURIComponent(us_getTempData("track")), decodeURIComponent(us_getTempData("album")), decodeURIComponent(us_getTempData("mbid")), 0, 1);
	}
...
*/
	...
	{	
		if ((forced) || ((parseInt(us_getValue('us_last_update', '0')) + 86400000) <= parseInt(((new Date()).getTime())))) // Checks every day (24 h * 60 m * 60 s * 1000 ms)
		{
			try
			{
				GM_xmlhttpRequest(
					...
					onload: function(resp)
					{
						...
						if (remote_version > local_version)
						{
							...
							var cont =  '<div id="us_loginbox_form"><span>YouScrobbler '+ String(remote_version) +' is available.</span><br/>'+
										'<br/>'+
										'<span>Reload the page after updating for changes to take effect.</span><br/>'+
