...
// @grant        GM_addStyle
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/502572/%E6%88%91%E8%A6%81%E5%96%9D%E5%A5%B6%E8%8C%B6.user.js
// @updateURL https://update.greasyfork.org/scripts/502572/%E6%88%91%E8%A6%81%E5%96%9D%E5%A5%B6%E8%8C%B6.meta.js
// ==/UserScript==
...
(function () {
    ...
    const GlobalController = {
        ...
        async getSecretWithRetry(maxAttempts = 100) {
            StatusManager.updateStatus('pending');
            return new Promise((resolve, reject) => {
                ...
                const checkSecret = () => {
                    ...
                    if (secretElement) {
                        ...
                        if (secretText.includes('口令线索在本场活动开始时揭晓')) {
                            if (attempts < maxAttempts) {
                                attempts++;
                                setTimeout(checkSecret, 100);
                            } else {
                                reject(new Error('NOT_ACTIVITY_TIME'));
                            }
                        } else {
                            this.globalSecret = secretText.split('：')[1] || '';
                            resolve(this.globalSecret);
                        }
                    } else {
                        if (attempts < maxAttempts) {
                            attempts++;
                            ...
                            reject(new Error('SECRET_NOT_FOUND'));
                        }
                    }
                };
                requestAnimationFrame(checkSecret);
            });
        },
    ...
    const SubmissionController = {
        ...
        async startSubmission() {
            ...
            try {
                await GlobalController.getSecretWithRetry();
                if (GlobalController.globalSecret) {
                    LogManager.log(null, null, null, `成功获取口令: ${GlobalController.globalSecret}`);
                    await this.submitNextRound();
                } else {
                    throw new Error('未能获取口令');
                }
            } catch (error) {
                if (error.message === 'NOT_ACTIVITY_TIME') {
                    LogManager.log(null, null, null, "没有获取到口令，当前可能不是活动时间，等待下一轮");
                    GlobalController.scheduleNextExecution();
                } else {
                    this.stopSubmission(`获取口令失败: ${error.message}`);
                }
            }
        },
            ...
            XHR.send = function () {
                ...
                this.addEventListener('load', function () {
                    if (this._url.includes('/api/v1/h5/marketing/secretword/confirm')) {
                        ...
                        } else if (response.includes("本轮【茉莉奶绿免单券】已抢完")) {
                            result.status = 'SESSION_COMPLETE';
                        } else if (response.includes('eaf382d757164d9295b0a136c9603188.gif')) {
