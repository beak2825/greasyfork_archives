...
// @version     2.2
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/30727/Twitter%20video%20links.user.js
// @updateURL https://update.greasyfork.org/scripts/30727/Twitter%20video%20links.meta.js
// ==/UserScript==
$(document).on("dataPlayerReadyEvent", function(){
	...
	var vmap_url = ($pt=$("#playerContainer")).length == 1? ( $pt.data("config").vmap_url || '' ) : '',
		a_attrs = 'target="_blank" style="text-decoration: none;"',
		$vspan = $('<span>').addClass("myv1").css({ 'position': "fixed", 'top': "0px", 'right': "0px", 
				'padding': "0px 5px 2px", 'margin': "2px", }),
	...
	if(vmap_url !== '' || $.inArray(st, ['consumer', 'snap-reel', 'gif']) > -1 ){
		if( $.inArray(ct, ['video/mp4', 'application/x-mpegURL'] ) > -1 
				&& typeof (vu = $pt.data("config").scribe_playlist_url) !== 'undefined' )
$vspan.append('<form name="myDownloadForm" action="http://twittervideodownloader.com/download" method="post"><input type="hidden" name="csrfmiddlewaretoken" value="ctgdYfAhef3UYXR7HTB03E4ujhwobKIB"><input name="tweet" type="hidden" value="'+vu+'" required=""><input type="submit" value="?" style="visibility:hidden;"></form>&nbsp;');
		if( vmap_url !== '' ){
			var dlm = ($vspan.text() == '')? '' : '|&nbsp;';
