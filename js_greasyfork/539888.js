...
// @grant        none
// @license      TANGMING
// @downloadURL https://update.greasyfork.org/scripts/539888/Ozon%20%E8%87%AA%E5%8A%A8%E5%A1%AB%E5%86%99%E4%BB%93%E5%BA%93%E8%A1%A8%E5%8D%95%E5%AD%97%E6%AE%B5.user.js
// @updateURL https://update.greasyfork.org/scripts/539888/Ozon%20%E8%87%AA%E5%8A%A8%E5%A1%AB%E5%86%99%E4%BB%93%E5%BA%93%E8%A1%A8%E5%8D%95%E5%AD%97%E6%AE%B5.meta.js
// ==/UserScript==
...
async function createDeliveryMethod14(scCompanyId, warehouseDraftId) {
    const deliveryBody = {
        company_id: Number(scCompanyId),
        warehouse_id: warehouseDraftId,
        name:"UNI Standard Small UNY UNI Standard Small Pick Up Point (轻小件,快速,自提点) Putian",
        cutoff: "17:00",
        tpl_integration_type: "aggregator",
        provider_id: 1297,
        template_id: 1116,
...
async function createDeliveryMethod15(scCompanyId, warehouseDraftId) {
    const deliveryBody = {
        company_id: Number(scCompanyId),
        warehouse_id: warehouseDraftId,
        name:"UNI Standard Big UNQ UNI Standard Big  Pick Up Point (大件,快速,自提点) Putian",
        cutoff: "17:00",
        tpl_integration_type: "aggregator",
        provider_id: 1303,
        template_id: 1122,
...
async function createDeliveryMethod16(scCompanyId, warehouseDraftId) {
    const deliveryBody = {
        company_id: Number(scCompanyId),
        warehouse_id: warehouseDraftId,
        name:"UNI Standard Premium Small UNL UNI Standard Premium Small  Pick Up Point (高客单轻小件,快速,自提点) Putian",
        cutoff: "17:00",
        tpl_integration_type: "aggregator",
        provider_id: 1306,
        template_id: 1128,
...
async function createDeliveryMethod17(scCompanyId, warehouseDraftId) {
    const deliveryBody = {
        company_id: Number(scCompanyId),
        warehouse_id: warehouseDraftId,
        name:"UNI Standard Premium Big UNQ UNI Standard Premium Big Pick Up Point (高客单大件自提点) Putian",
        cutoff: "17:00",
        tpl_integration_type: "aggregator",
        provider_id: 1309,
        template_id: 1134,
...
(async function () {
    ...
    let warehouseDraftId = await createWarehouse(scCompanyId, legalName);

    if (warehouseDraftId) {
        await sleep(1000); // 稍等一下，确保草稿创建完毕
        async function tryCreateAndActivate(fnCreate, label, companyId, warehouseId) {
            try {
                const createRes = await fnCreate(companyId, warehouseId);
                console.log(`激活配送${label}`);
                warehouseDraftId = await activateCreate(companyId, createRes.id);
                // warehouseDraftId = createRes.id;
            } catch (e) {
                console.error(`激活配送${label}失败:`, e.response?.data || e.message);
            }
        }
        // 调用流程
        await tryCreateAndActivate(createDeliveryMethod, 1, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod2, 2, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod3, 3, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod4, 4, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod5, 5, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod6, 6, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod7, 7, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod8, 8, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod9, 9, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod10, 10, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod11, 11, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod12, 12, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod13, 13, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod14, 14, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod15, 15, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod16, 16, scCompanyId, warehouseDraftId);
        await tryCreateAndActivate(createDeliveryMethod17, 17, scCompanyId, warehouseDraftId);
...
alert("✅ 仓库信息已在自动填写，请关闭插件并且在一分钟后刷新此页面！");