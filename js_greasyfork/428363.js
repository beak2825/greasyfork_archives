// ==UserScript==
// @name        Asagi & Torako's Black Tech
// @namespace   Asagi & Torako's Black Tech
// @match       https://desuarchive.org/*
// @match       https://archive.wakarimasen.moe/*
// @match       https://archived.moe/*
// @version     4.1
// @description ALRIGHT THIRD TIME'S THE FUCKING CHARM YOU FAGGOT NIGGER
// @icon        https://desuarchive.org/favicon.ico
// @downloadURL https://update.greasyfork.org/scripts/428363/Asagi%20%20Torako%27s%20Black%20Tech.user.js
// @updateURL https://update.greasyfork.org/scripts/428363/Asagi%20%20Torako%27s%20Black%20Tech.meta.js
// ==/UserScript==

//==Initialization==
if(window.location.href.indexOf("thread")>-1){
if(document.querySelectorAll("[src*='banned']").length>0){
  for(a=document.querySelectorAll("[src*='banned']").length;a--;){
    document.querySelectorAll("[src*='banned']")[a].
...
if(window.location.href.indexOf("desu")>-1){
  ...
  document.querySelectorAll(".pull-right")[document.querySelectorAll(".pull-right").length-1].style.display="none"}}else{
...
img="this.style.display=`none`;this.parentNode.querySelector(`.";
exp="ost_image`).style.display=`inline`;";exz="inline";nxz="none";
if(window.location.href.indexOf("wakarimasen")>-1){suace="https://t"}else{suace="https://d"}
...
document.querySelector(".coc").addEventListener("click",function(){b=0;c=document.body.scrollHeight
  for(a=0;a<c*1.2;a=a+800){setTimeout(function(){window.scrollTo(0,b);b=b+800},a/20)}})
// ==/Fuckin' Buttons 'n Shiet==
...
//Videos & Spoilers
      ...
      if(entry.target.classList.contains("has_image")){
        ...
        if(entry.target.querySelectorAll(".spoiler_box").length>0){sfos="></video>"
          if(parseInt(entry.target.querySelector(".post_file_metadata")
              .innerHTML.split(" ")[2].split("x")[0])>
            parseInt(entry.target.querySelector(".post_file_metadata")
              .innerHTML.split(" ")[2].split("x")[1])){sps="width"}else{sps="height"}
          entry.target.querySelector(".thread_image_link").innerHTML+=pos+
            entry.target.querySelector("[download]").href+"' "+sps+
            "='125px' loading='lazy' class='post_image spoiled'"+sfos}
//Post Layout
