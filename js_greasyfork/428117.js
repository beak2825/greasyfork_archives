...
// @version      0.5.0
// @description  看得懂的用 0点、 3点、6点、9点、12点、15点、18点、21点刷新，开始自动学习(到处抄代码终于能用了)
// @author       lpjp01
...
// @match        https://pc.xuexi.cn/notFound.html
...
// @downloadURL https://update.greasyfork.org/scripts/428117/%E7%BB%88%E4%BA%8E%E5%8F%AF%E4%BB%A5%E5%AD%A6%E4%B9%A0%E5%BC%BA%E5%9B%BD%E4%BA%86.user.js
// @updateURL https://update.greasyfork.org/scripts/428117/%E7%BB%88%E4%BA%8E%E5%8F%AF%E4%BB%A5%E5%AD%A6%E4%B9%A0%E5%BC%BA%E5%9B%BD%E4%BA%86.meta.js
// ==/UserScript==
...
(async function() {
    ...
    setInterval(function () {
        ...
        topRightElement.textContent = GM_getValue("userBoxText",userBoxText)+"   正在学习~~~";//userBoxText：姓名
        topRightElement.style.position = 'fixed';
        topRightElement.style.top = '0';
        topRightElement.style.right = '0';
          topRightElement.style.fontSize = '50px';
    ...
    async function doDailyExam() {
        ...
        if (settings.daily.value) {
            ...
            while (scores.daily.dayScore < scores.daily.dayMaxScore ) {
                ...
                if (end) {
                    //await openDailyExam();https://pc.xuexi.cn/notFound.html
                    window.close();
                    break;
                }
            }
    ...
    if (window.location.href === "https://pc.xuexi.cn/points/exam-practice.html") {
      ...
      } else if (window.location.href==="https://pc.xuexi.cn/notFound.html" ) {
        window.close();
    ...
    } else if (window.location.href.startsWith("https://pc.xuexi.cn/points/exam-index.html")) {
      await sleep(3000);
      var userBoxText = document.querySelector('span.name').innerText;
localStorage.setItem('userBoxText', userBoxText);
      GMsetValue("userBoxText", userBoxText);
      console.log(userBoxText)


        window.close();

    } else if (window.location.href.startsWith("https://pc.xuexi.cn/points/my-points.html")) {
window.open('https://pc.xuexi.cn/points/exam-index.html');
    ...
    async function getExams() {
        ...
        while (buttons.length == 0) {
            ...
            buttons2=document.querySelectorAll("li");
        }
        var exams = Array.prototype.slice.call(buttons);
        // console.log(exams)
        exams = exams.filter(exams => {
        ...
        for (let mmm=1 ;mmm< buttons2.length-1;mmm++) {
            ...
            await delay();
            buttons2[mmm-1].click();
            await delay();
            buttons=document.querySelectorAll(".ant-btn");
            exams = Array.prototype.slice.call(buttons);
            exams = exams.filter(exams => {
                return exams.innerText === dati;
            });
            console.log("第"+mmm+"次"+exams.length);
    ...
    async function doExam() {
        ...
        try{
            //document.getElementById('nc_1__scale_text').classe="scale_text slidetounlock nc-align-center scale_text2;"
            //  obj.parentNode.removeChild(obj)
            //document.getElementById("J-risk-close-btn").click();
            //x(e)
    ...
    async function  parseScoreDocument() {
        ...
        var dailyScoreText = $(cards[3]).text();//每周答题没有了改成专项答题了
        var dayScore_daily = parseInt(dailyScoreText.split("/")[0].substring(0,1));
        var dayMaxScore_daily =5// parseInt(dailyScoreText.split("/")[1].substring(0,1));

        var weeklyScoreText = $(cards[3]).text();//每周答题没有了改成专项答题了
        var dayScore_weekly = parseInt(weeklyScoreText.split("/")[0].substring(0,1));
        var dayMaxScore_weekly = 5//parseInt(weeklyScoreText.split("/")[1].substring(0,1));

        var zhuanxiangScoreText = $(cards[3]).text();//每周答题没有了改成专项答题了
        console.log(zhuanxiangScoreText);
        var dayScore_zhuanxiang = parseInt(zhuanxiangScoreText.split("/")[0].substring(0,1));
        console.log(dayScore_zhuanxiang);

        var dayMaxScore_zhuanxiang =5// parseInt(zhuanxiangScoreText.split("/")[1].substring(0,zhuanxiangScoreText.split("/")[1].indexOf("分")));,zhuanxiangScoreText.split("/")[0].indexOf("分"))
        console.log(dayMaxScore_zhuanxiang);
...
`);
    ...
    var d=datev.getHours()
    var m=datev.getMinutes()
    console.log("现在时间"+d+":")
    if(d==0||d==3||d==6||d==9||d==12||d==15||d==18||d==21 ) {
        await sleep(6000)
        //window.open("https://pc.xuexi.cn/points/my-points.html");
        //doStudy()
        console.log("整点刷新")
        await sleep(3600000)
        location.reload();}//休息60分钟

    else{
        console.log("现在时间"+d+":"+m+"将休眠1小时")
        await sleep(3600000)
        location.reload();
    }
    function sleep (time) {
