...
// @downloadURL https://update.greasyfork.org/scripts/502649/jxzh%E8%87%AA%E5%8A%A8%E6%92%AD%E6%94%BE%E8%A7%86%E9%A2%91.user.js
// @updateURL https://update.greasyfork.org/scripts/502649/jxzh%E8%87%AA%E5%8A%A8%E6%92%AD%E6%94%BE%E8%A7%86%E9%A2%91.meta.js
// ==/UserScript==
    ...
    function getJxzh() {
        return {
            seeVideo: function () {
                ...
                // 5分钟重置一次验证码计时
                if (location.pathname.includes('course_study')) {
                    times = setInterval(() => {
                        processTimeCheck = 0
                    }, 1000 * 60 * 5)
                } else {
                ...
                setInterval(function () {
                    ...
                    try {
                        ...
                        var video = $('#J_prismPlayer').find("video")[0]
                        ...
                        // 判断是否暂停,如果暂停,就继续播放.
                        if (video.player) {
                            console.log("player状态: ", video.player._status)
                            // if ( $("video")[0].player.tag.paused ) {} 这个方法也可以.
                            if ("playing" != video.player._status) {
                                console.log("未开始播放!")
                                // 点击继续播放.
