// ==UserScript==
// @name        CosmosisT's TinyChat Script (CTS V1.8.57) with extra features
// @version     1.8.57
...
//              jshint esversion: 6
// @namespace https://greasyfork.org/users/1230074
// @downloadURL https://update.greasyfork.org/scripts/481636/CosmosisT%27s%20TinyChat%20Script%20%28CTS%20V1857%29%20with%20extra%20features.user.js
// @updateURL https://update.greasyfork.org/scripts/481636/CosmosisT%27s%20TinyChat%20Script%20%28CTS%20V1857%29%20with%20extra%20features.meta.js
// ==/UserScript==
...
(function() {
    ...
    function WordleCommandCheck() {
        ...
        if (arguments[1].match(/^!wordle$/i)) {
            //check no current player then adds player
            if (NewPlayer.length === 0) {
            NewPlayer.push(User.nick);
            //generate wordList if doesn't exist
            //User.username
            if (CTS.Game.Wordle.wordList.length === 0) {
                // Call WordleGenerateList with WordleGetAnswer as the callback
                WordleGenerateList(WordleGetAnswer);
            } else if (CTS.Game.Wordle.ActiveAnswer.length === 0) {
                // picks a random word from wordList and sets as ActiveAnswer
                WordleGetAnswer();
                }
            } else {
        ...
        } else if ((User.nick === NewPlayer[0]) && (AttemptsRemaining > 0) && arguments[1].match(/^!guess ([a-z]{5})$/i)) {
            var attempt = arguments[1].match(/^!guess ([a-z]{5})$/i);
            //check that attempt.toLowerCase() is in wordList
    ...
    function WordleGetAnswer() { 
        ...
        let resp = randomWord;
        let msg = "📖🔤[𝕎𝕆ℝ𝔻𝕃𝔼]🔤📖\n\nWordle game started\n\ntype !𝗴𝘂𝗲𝘀𝘀 [𝗔-𝗭]*𝟱\n\nto guess the answer.\n\ntype !𝘄𝗼𝗿𝗱𝗹𝗲𝗵𝗲𝗹𝗽 for game rules.";
        CTS.Game.Wordle.ActiveAnswer.push(randomWord.toUpperCase());
        if (resp !== null) Send("msg", msg);
    }
    
    function WordleVerifyAttempt(guess) {
        //CHECK THAT GUESS IS A VALID WORD IN wordList
        let msg = "📖🔤[𝕎𝕆ℝ𝔻𝕃𝔼]🔤📖\n\n";
        if (CTS.Game.Wordle.wordList.includes(guess)) {
            //CTS.Game.Wordle.AttemptList.push(CTS.Game.Wordle.CurrentAttempt[0]);
            CTS.Game.Wordle.AttemptList.push(guess);
            AttemptResults();
        } else {
            msg += (guess.toUpperCase() + " is not a valid word.");
            CTS.Game.Wordle.CurrentAttempt = [];
            Send("msg", msg);
        }    
    }            

    //WORDLE ATTEMPT FUNCTIONS
    function AttemptResults() {
        var msg = "📖🔤[𝕎𝕆ℝ𝔻𝕃𝔼]🔤📖\n\n";
        var wordleAnswer = CTS.Game.Wordle.ActiveAnswer[0];
        var wordleAttempt = CTS.Game.Wordle.CurrentAttempt[0];
        for (var i = 0; i < 5; i++) {
        CheckAttempt(wordleAnswer.charAt(i), wordleAttempt.charAt(i));
        }
        CTS.Game.Wordle.ProgressMsg += "\n\n";
        if (wordleAttempt == wordleAnswer) {
            msg += (CTS.Game.Wordle.ProgressMsg + "WINNER!\n\n" + CTS.Game.Wordle.ActivePlayer[0] + ", found the WORDLE.");
            Send("msg", msg)
            WordleReset() 
        } else {
            msg += CTS.Game.Wordle.ProgressMsg;
            if ((6 - CTS.Game.Wordle.AttemptList.length) < 1) {
                msg += (CTS.Game.Wordle.ActivePlayer[0] + ", lost.\n\nWORDLE was: " + wordleAnswer);
                Send("msg", msg);
                WordleReset();
            } else {
            msg += (CTS.Game.Wordle.Keyboard + "\n\nAttempts Remaining: " + (6 - CTS.Game.Wordle.AttemptList.length))
            Send("msg", msg);
            CTS.Game.Wordle.CurrentAttempt = [];
            }
        }
    ...
    function CheckAttempt(aLetter, gLetter) {
        var msg = "";
        var wordleAnswer = CTS.Game.Wordle.ActiveAnswer[0];
        msg += "[" + gLetter + "]" + (aLetter === gLetter ? "🟩" : (wordleAnswer.search(gLetter) !== -1 ) ? "🟨" : "⬛");
        msg = msg.replace("I", " I ");
        CTS.Game.Wordle.Keyboard = CTS.Game.Wordle.Keyboard.replace(gLetter, "");
        CTS.Game.Wordle.ProgressMsg += (msg)
    }
    ...
    var CommandList = {
        ...
        //!uba short inputt for !userbanadd
        uba: function() {
        ...
        nick: function() {
            ...
            if (user != -1) {
                ...
                if ((CTS.GreetList.includes(CTS.UserList[user].username) || (CTS.Host == CTS.Me.handle && CTS.GreetList.includes("-ALL"))) && CTS.NoGreet === false) {
                    ...
                    if (arguments[0].nick.match(/stumble|nig|3_b|nazi/i)) {
        ...
        AddPlayer: function() {
            if (!this.GetPlayer(arguments[0], false, false) && CTS.CanHostFishGames === true) {
                if (isSafeListed(arguments[1])) {
                    ...
                    Send("msg", "[FISHING BOAT]\n" + arguments[2].substr(0, 16) + "...\n has jumped aboard.\nType !fishhelp for command list.");
