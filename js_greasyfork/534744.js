...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/534744/Dobby2.user.js
// @updateURL https://update.greasyfork.org/scripts/534744/Dobby2.meta.js
// ==/UserScript==

(function initDobbyWhenReady() {
    const interval = setInterval(() => {
        if (
            typeof Bag !== "undefined" &&
            typeof Bag.search === "function" &&
            typeof Dobby !== "undefined" &&
            typeof Dobby.findAllConsumables === "function"
        ) {
            clearInterval(interval);
            console.log("Bag e Dobby prontos.");

            // Tenta detetar idioma do navegador
            const lang = (navigator.language || navigator.userLanguage || "en_DK").toLowerCase();

            // Suporte garantido para en_DK e pt-pt
            let chosenLang = "en_DK"; // default
            if (lang.startsWith("pt")) {
                chosenLang = "pt-pt";
            }

            Dobby.language = chosenLang;
            console.log(" Idioma detetado:", Dobby.language);

            // Adiciona palavras-chave se necessário
            if (!Dobby.searchKeys["pt-pt"]) {
                Dobby.searchKeys["pt-pt"] = {
                    energy: "Energia",
                    energyText: "Aumento de energia:",
                    motivation: "Motivação",
                    motivationText: "Aumento da motivação de trabalho:",
                    health: "Vida",
                    healthText: "Bonús de ponto de vida:"
                };
                console.log("Palavras-chave pt-pt adicionadas.");
            }

            //  Procura consumíveis
            Dobby.findAllConsumables();
            console.log("Consumíveis encontrados:", Dobby.allConsumables);
        } else {
            console.log("A aguardar que Bag e Dobby estejam disponíveis...");
        }
     ...
     Dobby = {
         ...
         searchKeys:{
             ...
             "pt-pt":{
                 ...
                 motivationText: "Aumento da motivação de trabalho:",
                 health: "Vida",
                 healthText: "Bónus de ponto de vida:"
             },
