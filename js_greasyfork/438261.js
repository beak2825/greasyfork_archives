// ==UserScript==
// @name         MA SHU SHU GuitarDownloadImage
// @namespace    http://tampermonkey.net/
// @version      0.1
// @description  下載嘖嘖馬叔叔和圖片
// @author       You
...
// @grant        unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/438261/MA%20SHU%20SHU%20GuitarDownloadImage.user.js
// @updateURL https://update.greasyfork.org/scripts/438261/MA%20SHU%20SHU%20GuitarDownloadImage.meta.js
// ==/UserScript==
...
/*waitForKeyElementsg啟動會失敗，先用暴力法等待時間加載完在執行。
(function() {
...
function CopyTitleContent(){
	//如果有新的類型 1.熱門單曲 2.FREE ，要修改CopyTitleContent()，getTitle()
	if($('h3:contains("熱門單曲")').length > 0)
	{
	...
	else if($('h3:contains("FREE")').length > 0)
	{
		var title = $('h3:contains("FREE")');
        title.append("<input id=\"copyTitle\" type=\"button\" value=\"複製標題\">");
			$("#copyTitle").click(function(){
            copyTitle();
        });
...
function getTitle(){
	...
	{
		var title = $('h3:contains("FREE")').text();
		var myRegexp = /【FREE】(.*)\(/g;
		var match = myRegexp.exec(title);
		return match[1];
	}
