...
// @description  斗鱼抢全站红包，用于升账号等级，使用者需拥有呆妹儿小霸王(92000)15级粉丝牌，才可使用插件功能「http://dy92000.cn/」
// @author       Dabiaodi
// @match			*://*.douyu.com/0*
// @match			*://*.douyu.com/1*
// @match			*://*.douyu.com/2*
...
function update_Script(x){
    let update_message = "";
    update_message += '<a href="https://greasyfork.org/zh-CN/scripts/427955" style="font-size:13px;" target="_blank">【更新】新版本已发布，点击立即我更新插件！</a>';
    if(x == true){
...
function init_Red_Envelope_Html(){
    ...
    html += `<section>
  	             ...
  	             <span id="red_envelope_text">🐨礼物红包设置⚙️</span>
                 <a href="http://api.bluanr.cn/dyapi/query_log?uid=${getUid()}" id="red_envelope_query" style="display:none;" target="_blank">🐨查询礼物记录</a>
            </section>`;
    a.innerHTML = html;
        ...
        if (retJson.is_Get_Red_Envelope == true) {
            verifyFans("2947432", 15).then(r => {
                if (r == true) {
                    ...
                    showMessage("抢红包礼物功能需拥有15级呆妹粉丝牌(2947432)可使用", "error");
                }
            })
        }
	}
...
function init_Setting_Html() {
    ...
    a.innerHTML = `
            <div class="check_setting">
                <p><input type="checkbox" id="yc_100"><label>100鱼翅红包</label></p>
                <p><input type="checkbox" id="yc_500"><label>500鱼翅红包</label></p>
                <p><input type="checkbox" id="yc_2000"><label>2000鱼翅红包</label></p>
			</div>
    `;
	let b = document.getElementsByClassName("PlayerToolbar-Wealth")[0];
    let c = document.getElementById("ex-point");
