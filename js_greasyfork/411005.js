...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/411005/Acfun%E7%9B%B4%E6%92%AD%E7%9B%91%E6%8E%A7pack1.user.js
// @updateURL https://update.greasyfork.org/scripts/411005/Acfun%E7%9B%B4%E6%92%AD%E7%9B%91%E6%8E%A7pack1.meta.js
// ==/UserScript==
  ...
  if (location.href.indexOf('/live/') === -1) {
    ...
    setTimeout(() => {
      ...
      newNode.innerHTML =
        ...
        '<div class="monitor_panel" :class="{monitor_panel_top: hideMonitor}"><span @click="handelHideMonitor" v-show="!hideMonitor">收起</span>' +
        ...
        `<span @click="setGrid(9)" title="九宫格">${_icon_js__WEBPACK_IMPORTED_MODULE_2__[/* default */ "a"].grid9}</span><span @click="setGrid(16)" title="十六宫格">${_icon_js__WEBPACK_IMPORTED_MODULE_2__[/* default */ "a"].grid16}</span></div>` +
      ...
      let monitorVue = new Vue({
        ...
        created() {
          ...
          this.divTop =
            document
              .querySelector('.category-wrapper')
              .querySelectorAll('div')[5].offsetTop +
            20 +
            'px'
    ...
    style_2.innerHTML =
      ...
      '.monitor_panel{position: absolute; z-index: 1000; left: 10px; top: 35px; width: 325px; height: 40px;display: flex; align-items: center; border-radius: 20px; opacity: 0; background: #000; color: #fff; padding-left: 20px;}' +
      '.monitor_panel:hover{opacity: 1;}' +
      '.monitor_panel_top{top: 2px;}' +
  ...
  if (location.href.indexOf('/live/') !== -1) {
    ...
    let appendInterval = setInterval(() => {
      ...
      if (containerEl) {
        ...
        let style = document.createElement('style')
        style.innerHTML =
          '#my_mark{position: absolute; right: 10px; top: -12px; z-index: 99; width: 80px; height: 30px; z-index: 100; text-align: right;}' +
