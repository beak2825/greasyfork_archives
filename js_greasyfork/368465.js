// ==UserScript==
// @name          Heise.de Forum: all comments on one page v3
// @namespace     Vorticon
// @author        marrr, edited by commander_keen, jn, mastacheata
// @version       3.2.0
...
// @grant         GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/368465/Heisede%20Forum%3A%20all%20comments%20on%20one%20page%20v3.user.js
// @updateURL https://update.greasyfork.org/scripts/368465/Heisede%20Forum%3A%20all%20comments%20on%20one%20page%20v3.meta.js
// ==/UserScript==
...
// GreaseMonkey/FF57+ script insertion iFrame
if (document.location.href === 'about:blank') {
 	baseUrl = window.parent.document.location.protocol + '//' + window.parent.document.location.host;
 	isNewForum = /^\/forum\//.test(window.parent.document.location.pathname) ? true : false;
 }
...
function processMessageDivNew_NoText(div, messageUrl, divPost) {
	...
	var ntDiv = '<div class="userbar clearfix"><ul class="author forum_userbar--author userbar_list"><li><span class="full_user_string"><span class="pseudonym">' + userH + '</span></span></li></ul><p><!--           xxx BeitrÃƒÂ¤ge seit xx.xx.xxxx  --></p><div class="user_interaction"><!-- <a href="Link nicht vorhanden /user-xxxxxx/posting-xxxxxxxx/ignore/" class="ignore" rel="nofollow">ignorieren</a>  --></div></div>';
...
function processMessageDivNew(div, messageUrl, replyURL, quoteURL) {
	// link username to search
	...
	if(replyURL != "") {
		...
		if(quoteLink != "") {
			quoteURL  = ensureAbsoluteUrl(quoteURL);
			quoteLink  = " | <a href=\"" + quoteURL + "\">Zitieren</a>";
			replyLink += quoteLink;
		}
		...
		if(userI !== null) {
			...
			if(enableIReply === 1)
				if (enableAutoQuote === 1) {
					document.getElementById("iReplyLink_" + div.getAttribute("replyFrameID"))
						.addEventListener("click", function() {
							iReplyNew(div.getAttribute ("replyFrameID"), quoteURL)
						}, false);					
				}
				else {
					document.getElementById("iReplyLink_" + div.getAttribute("replyFrameID"))
		...
		for(var i = 0; i < voteLinks.snapshotLength; i++) {
			...
			//voteLink.removeAttribute("href");	// modifying the element will invalidate the list element and the event listener will match nothing
 			voteLink.addEventListener("click", function(event) {
				if(GM.xmlHttpRequest || XMLHttpRequest)
...
function joinOverviewPages()
{
	...
	if(firstPostNr > overviewPagePostCount) {
		var li = document.createElement('li');
		li.innerHTML = "<a href=\"" + ensureAbsoluteUrl(insertPostStart(lastPageURL, firstPostNr - maxJoinedPosts - 3 * overviewPagePostCount)) + "\"><b>VorwÃƒÂ¤rts...</b></a>";
...
function joinOverviewPagesNew()
{
	...
	if(currentPageNr > 1) {
		var li = document.createElement('li');
		li.innerHTML = "<a href=\"" + ensureAbsoluteUrl(insertPostStart(lastPageURL, currentPageNr > (maxJoinedPosts / overviewPagePostCount) ? currentPageNr - maxJoinedPosts / overviewPagePostCount - 1 : 1)) + "\"><b>VorwÃƒÂ¤rts...</b></a>";
		threadTree.insertBefore(li, threadTree.childNodes[0]);
	}
	if(limited) {
		var li = document.createElement('li');
...
{
	// do not crop the regular answer page (e.g. after click on "Zitat einfÃƒÂ¼gen")
...
function iReplyNew(frameId, replyUrl) {
	...
	{
		var form = event.target.contentDocument.getElementById("reply_create_posting");
		event.target.contentDocument.body.innerHTML = form.innerHTML;
	};
}
...
function main()
{
	...
	if(enableIReply) {
		...
		// is reply page?
		if((isWriteUrl(document.location.href) || document.body.innerHTML.match(/<textarea name="message"/i) || document.body.innerHTML.match(/<textarea name="body" id="msg_body"/i) || window.location.href === "about:blank") &&
		  (top === undefined || !isWriteUrl(top.location.href)) || top.location.href === "about:blank") {
			cleanUpReplyPage();
			return;
		}
	}
	...
	if(!/\/reply\/$/.test(document.location.href) && window.location.href !== "about:blank") {
		joinThreadPosts();
	}
}
main();
