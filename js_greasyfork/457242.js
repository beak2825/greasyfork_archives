// ==UserScript==
// @name         CityU Check available seats of courses, no msg
...
// @downloadURL https://update.greasyfork.org/scripts/457242/CityU%20Check%20available%20seats%20of%20courses%2C%20no%20msg.user.js
// @updateURL https://update.greasyfork.org/scripts/457242/CityU%20Check%20available%20seats%20of%20courses%2C%20no%20msg.meta.js
// ==/UserScript==
...
<div class="addonBox" style="border: solid 1px rgb(238,238,238);position: absolute;width: 250px;border-radius: 15px;overflow: hidden;font-family: -apple-system, system-ui, BlinkMacSystemFont, &quot;Segoe UI&quot;, &quot;Roboto&quot;, &quot;Ubuntu&quot;, &quot;Helvetica Neue&quot;, sans-serif;z-index: 99999;background-color: white;top: -800px; left: 500px">
   ...
   <div class="addonBody2" style="padding: 10px 30px;display: none;text-align: center;">
    ...
    // Init
    var courseCode = document.querySelectorAll("form[action='/pls/PROD/hwscrssh_cityu.P_CrseSearch'] b")[0].innerHTML.split(" ")[2];
    var CRN = localStorage.getItem("addonCRN" + courseCode);
    var companyCRN = localStorage.getItem("companyCRN" + courseCode);
    if(CRN != void 0){
        inputBox.value = CRN;
        addonSelect.value = localStorage.getItem("addonInterval" + courseCode);
        hideBox();
    ...
    autoRegBtn.onclick = () => {
        ...
        localStorage.setItem("addonAutoReg" + courseCode, "true");
        localStorage.setItem("companyCRN" + courseCode, inputBox2.value);
        console.log("Auto Reg Start")
        startCheck()
    }

    submitBtn.onclick = () => {
        localStorage.setItem("addonAutoReg" + courseCode, "false");
        startCheck()
    }
    ...
    stopBtn.onclick = () => {
        showBox();
        localStorage.setItem("addonAutoReg" + courseCode, "false");
        localStorage.removeItem('addonCRN' + courseCode);
        localStorage.removeItem('companyCRN' + courseCode);
        localStorage.removeItem('addonInterval' + courseCode);
        clearTimeout(addonInterval);
    }

    function startCheck() {
        CRN = inputBox.value;
        localStorage.setItem("addonCRN" + courseCode, inputBox.value);
        localStorage.setItem("addonInterval" + courseCode, addonSelect.value);
        hideBox();
    }
    ...
    function hideBox(){
        ...
        if(statusOfChecking == "full"){
            ...
            if (isAutoRegChekced()) {
                autoReg(localStorage.getItem('addonCRN' + courseCode))
            }
    ...
    function isAutoRegChekced() {
        const isChecked = localStorage.getItem("addonAutoReg" + courseCode);
