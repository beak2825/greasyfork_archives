// ==UserScript==
// @name        CosmosisT's TinyChat Script (CTS V1.6.90)
// @version     1.6.90
...
// @downloadURL https://update.greasyfork.org/scripts/405542/CosmosisT%27s%20TinyChat%20Script%20%28CTS%20V1690%29.user.js
// @updateURL https://update.greasyfork.org/scripts/405542/CosmosisT%27s%20TinyChat%20Script%20%28CTS%20V1690%29.meta.js
// ==/UserScript==
(function() {
    "use strict";
    //DEBUGGER
    ...
    var CTS = {
        ...
        Room: [],
        ScriptInit: false,
        MainBackground: "url(https://i.imgur.com/aS5RCaX.jpg) rgb(0, 0, 0) no-repeat",
    ...
    if (CTS.StorageSupport) {
        ...
        CTS.Discord.Disabled = JSON.parse(Load("DiscordDisabled", JSON.stringify(true)));
        ...
        CTS.Favorited = JSON.parse(Load("Favorited", JSON.stringify([
            ["stonercircle", "https://avatars.tinychat.com/da/838b96/5f/medium/phpKXAdEn.jpeg"], null, null, null, null
        ])));
        ...
        CTS.ServerEvent = JSON.parse(Load("ServerEvent", JSON.stringify(true)));
    ...
    function CTSInit() {
        ...
        CTS.FullLoad = setInterval(function() {
            if (CTS.ScriptInit && CTS.SocketConnected) {
                ...
                for (var i = 0; i < len; i++) favorited_rooms += CTS.Favorited[i] !== null ? "<span>#" + (i + 1) + ' <a href="https://tinychat.com/room/' + CTS.Favorited[i][0] + '">' + CTS.Favorited[i][0] + "</a></span>" : "<span>#" + (i + 1) + ' <a href="#" onclick="return AddFavorite(this,' + i + ');">ADD ROOM</a></span>';
    ...
    function CTSRoomInject() {
        ...
        window.CTSChatCSS = [
            ...
            [ //STYLE #12
                ["#chat-wrapper{background: #fff!important;}.message>.systemuser{color:#FFFFFF}#cts-chat-content>.message{background:#00000000}.message{color:#000000;text-shadow: unset;}"],
                [".PMPopup .PMContent{background:#2d373ADB;}.PMPopup h2{background:linear-gradient(0deg,rgb(0, 0, 0)0%,rgba(255, 255, 255, 0.72)8px,rgba(255, 255, 255, 0.81)100%);}#videos-footer-broadcast-wrapper>.waiting{background:#53b6ef;}#videos-footer-broadcast-wrapper>#videos-footer-submenu-button{background:#53b6ef!important;}#videos-footer-broadcast-wrapper>#videos-footer-submenu-button:hover{background:#3d89b5!important;}#videos-footer-push-to-talk{background:#53b6ef}#videos-footer-push-to-talk:hover{background:#3d89b5}#videos-footer-broadcast:hover{background:#3d89b5}#videos-footer-broadcast{background:#53b6ef;}"],
        ...
        CTS.enableSound = window.TinychatApp.BLL.SettingsFeature.prototype.getSettings().enableSound;
        //TTS (TEXT-TO-SPEECH)
                ...
                if(username === "GUEST") {
                    ...
                    if (a.hidden) {
                        ...
                        if (index === -1) {
                            //ADD
                            if (arguments[1] === undefined) {
                                debug("HIDDENCAMERALIST::", "ADD USER " + username + " TO HIDDENCAMERALIST");
                                Alert(GetActiveChat(), "✓ Adding " + username + " to hiddencameralist!");
    ...
    function CTSRoomLoad() {
        ...
        ChatLogElement.querySelector("#chat-download").textContent = "📋Download";
    ...
    function BotCommandCheck() {
        ...
        if (isCommand(arguments[1])) {
            if (arguments[1].match(/^!added_to_queue$|^!\s|^!cz\s|^!clear$|^!skip$|^!queue$/i)) {
                BotCommandCheckYT(arguments[0], arguments[1]);
            } else if (arguments[1].match(/^!userkick\s|^!userban\s|^!userclose\s|^!nickkick\s|^!nickban\s|^!nickclose\s/i)) {
                BotCommandCheckJR(arguments[0], arguments[1]);
            } else if (arguments[1].match(/^!whoisbot$|^!8ball\s|^!vote\s|^!coin$|^!chuck$|^!urb\s|^!dad$|^!advice$/i)) {
                BotCommandCheckPUB(arguments[0], arguments[1]);
            } else if (CTS.UserList[arguments[0]].canGame) {
    ...
    function BotCommandCheckYT() {
        ...
        if (CTS.Room.YT_ON) {
            ...
            } else if (arguments[1].match(/^!\s/i)) {
                BotCommand(1, arguments[0], arguments[1]);
            } else if (arguments[1].match(/^!cz\s/i)) {
                BotCommand(6, arguments[0], arguments[1]);
            } else if (arguments[1].match(/^!clear$/i)) {
                BotCommand(2, arguments[0]);
            } else if (arguments[1].match(/^!skip$/i)) {
                BotCommand(3, arguments[0]);
            } else if (arguments[1].match(/^!queue$/i)) {
    ...
    function BotCommandCheckJR() {
        ...
        if (CTS.BotModList.includes(CTS.UserList[arguments[0]].username) || CTS.UserList[arguments[0]].mod) {
            if (arguments[1].match(/^!userkick\s/i)) {
                ModCommand("kick", arguments[1], true);
            } else if (arguments[1].match(/^!userban\s/i)) {
                ModCommand("ban", arguments[1], true);
            } else if (arguments[1].match(/^!userclose\s/i)) {
                ModCommand("stream_moder_close", arguments[1], true);
            } else if (arguments[1].match(/^!nickkick\s/i)) {
                ModCommand("kick", arguments[1], false);
            } else if (arguments[1].match(/^!nickban\s/i)) {
                ModCommand("ban", arguments[1], false);
            } else if (arguments[1].match(/^!nickclose\s/i)) {
    ...
    function BotCommandCheckPUB() {
        if (arguments[1].match(/^!check$/i)) BotCommand(5, arguments[0]);
        if (arguments[1].match(/^!vote\s/i)) Vote(arguments[0], arguments[1]);
        // PUBLIC COMMANDS
        if (CTS.PublicCommandToggle) {
            if (arguments[1].match(/^!8ball\s[\w\s]*\??/i)) {
                ...
                } else if (arguments[1].match(/^!urb\s/i)) {
    ...
    function Events() {
        ...
        CTS.Events.XHR.setRequestHeader("Accept", "application/json");
        CTS.Events.XHR.send();
    }
    //MESSAGE FUNCTION
    ...
    function CreateServerEvent() {
        ...
        if (CTS.enableSound === true && window.TinychatApp.getInstance().defaultChatroom.volume > 0) window.CTSSound.MENTION.play();
        UpdateScroll(1, true);
    }
    ...
    window.CreateGift = function() {
        ...
        if (CTS.enableSound === true && window.TinychatApp.getInstance().defaultChatroom.volume > 0) window.CTSSound.GIFT.play();
    ...
    function LoadMessage() {
        var ChatIndex, Chat = ChatLogElement.querySelector("#cts-chat-content");
        CTS.ChatScroll = true;
        Chat.innerHTML = "";
        CheckUnreadMessage();
    ...
    function Discord() {
        var element,
            button = ChatLogElement.querySelector(".chat-discord");
        if (!CTS.Discord.Disabled) {
            //DISCORD ON
            if (!button) {
                element = document.createElement("button");
                ...
                element.setAttribute("src","https://titanembeds.com/embed/532908572212723722?scrollbartheme=3d&theme=MetroEdge");
            ...
            if (button) {
                ChatLogElement.querySelector("#chat-discord").removeEventListener("click", DiscordToggle, {
                    passive: true
                });
                Remove(ChatLogElement, ".chat-discord");
                Remove(ChatLogElement, "#chat-discord");
            }
        }
    }
    ...
    function DiscordToggle() {
        CTS.Discord.Show = !CTS.Discord.Show;
        ChatLogElement.querySelector(".chat-discord").style.visibility = ((CTS.Discord.Show) ? "visible" : "hidden");
    }
    ...
    function Vote() {
        ...
        if (ChecksOut) {
            if (isSafeListed(CTS.UserList[arguments[0]].username.toUpperCase())) {
                var targetname = arguments[1].match(/^!vote\s([a-z0-9]{1,16})$/i);
                if (targetname !== null) {
    ...
    function ServerEvent() {
        var temp,
            i,
            len = CTS.ServerEventServerInList.length;
        for (i = 0; i < len; i++) clearTimeout(CTS.ServerEventServerInList[i]);
        CTS.ServerEventServerInList = [];
        if (CTS.ServerEvent === true) {
            var OffsetTime,
                event = true;
            len = CTS.ServerEventList.length;
            for (i = 0; i < len; i++) {
                temp = new Date(CTS.ServerEventList[i][1]);
                CTS.RecentTime = new Date();
                if (temp < CTS.RecentTime && CTS.ServerEventList[i][0]) {
                    temp.setDate(CTS.RecentTime.getDate());
                    temp.setYear(CTS.RecentTime.getFullYear());
                }
                OffsetTime = temp - CTS.RecentTime;
                if (OffsetTime >  0 && OffsetTime < 3600000) {
                    event = false;
                    CTS.ServerEventServerInList.push(setTimeout(AddServerEvent, OffsetTime, CTS.ServerEventList[i][2], CTS.ServerEventList[i][3], CTS.ServerEventList[i][4]));
                }
            }
    ...
    function Cameras() {
        ...
        for (var num = 0; num < Len; num++) {
            Camera[num].shadowRoot.querySelector(".video").style.padding = (CTS.CameraBorderToggle) ? "5px" : "0";
            ...
            if (CTS.UserList[arguments[0]].owner) {
                if (!CTS.Me.owner) {
                    ...
                    if (arguments[1].match(/^!raid\s/i)) {
                        ...
                        var raid = arguments[1].match(/^(?:!raid\s)(?:https?:\/\/)?tinychat\.com(?!\/#)\/(?!gifts|settings|coins|subscription|promote)(?:room\/)?([a-z0-9]{3,16})$/i);
                        if (raid !== null) {
                            if (CTS.RaidToggle) {
    ...
    function ModCommand() {
        var name = arguments[1].match(/^!(?:userkick|nickkick|userban|nickban|userclose|nickclose)\s(guest-[0-9]{1,32}|[a-z0-9_]{1,32})$/i),
            target;
        if (name !== null) {
        ...
        if (len <= 0) {
            ...
            if (check){
                ...
                if (VerifyYouTube(arguments[1])) {
                    console.log("true true true24525252552");
                            ...
                            if (videoid !== null) {
                                CTS.SocketTarget.send(JSON.stringify({
                                    ...
                                    "item": {
                                        ...
                                        "duration": 7200,
                                        "offset": 0,
                                        "title": "YOUTUBE IS BYPASSED BY A CZ MOD"
                                    }
                                }));
    ...
    var CommandList = {
        ...
        servereventtoggle: function() {
            CTS.ServerEvent = !CTS.ServerEvent;
            Save("ServerEvent", JSON.stringify(CTS.ServerEvent));
            ServerEvent();
            Settings();
            ...
            if (arguments[0] === "" || arguments[0].toUpperCase() === "GUEST") { // Can't protect guests;
                ...
                if (CheckUserNameStrict(arguments[0])) {
                    if (!CTS.SafeList.includes(arguments[0].toUpperCase())) {
                        ...
                        Alert(GetActiveChat(), "X Name already exists!");
                    }
                } else {
                    Alert(GetActiveChat(), "X Command Rejected!\nArgument passed didn't match criteria!");
                }
        ...
        allowdiscord: function() {
            if(CTS.Discord.Disabled) {
                CTS.Discord.Disabled = false;
                Save("DiscordDisabled", JSON.stringify(CTS.Discord.Disabled));
                Alert(GetActiveChat(), "✓ Command Accepted!\nDiscord is now on!");
                Discord();
            }
        },
    ...
    var ServerInList = {
        ...
        join: function() {
            ...
            if (CTS.HighlightList.includes(user) && CTS.enableSound === true && window.TinychatApp.getInstance().defaultChatroom.volume > 0) window.CTSSound.HIGHLIGHT.play();
            if (user === "COSMOSIST" && CTS.Room.Name.toUpperCase() !== "STONERCIRCLE" && !CTS.hasGreetedWC) { // Meet your creator
                CTS.hasGreetedWC = true;
                if (CTS.enableSound === true && window.TinychatApp.getInstance().defaultChatroom.volume > 0) window.CTSSound.C.play();
                        ...
                        if (GamePrevention(arguments[0].text, CTS.UserList[user].mod)) {
                            ...
                            if (!CheckUserIgnore(user) && !CheckUserTempIgnore(user) && IgnoreText(text)) {
                                ...
                                if (CTS.Me.handle !== arguments[0].handle) {
                                    ...
                                    if (CTS.enableSound === true && window.TinychatApp.getInstance().defaultChatroom.volume > 0) {
    ...
    CTS.Events.XHR.onload = function() {
        console.log("TEST");
        var resp = JSON.parse(CTS.Events.XHR.responseText);
        console.log(resp);
        if (resp !== null) {
            if (resp.files.CTSJson.content !== null) {
                try {
                    var parse = JSON.parse(resp.files.CTSJson.content);
                    CTS.ServerEventList = parse.serverevent;
                } catch (e) {
                    CTS.ServerEventList = [];
                }
                ServerEvent();
