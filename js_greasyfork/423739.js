// ==UserScript==
// @name        UPM Snow Helper 1.0
// @description Desk helper
// @namespace   https://upmprod.service-now.com/
// @match       https://upmprod.service-now.com/*
// @version     1.0
// @grant       none
// @require     http://ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js
// @charset "utf-8"
// @downloadURL https://update.greasyfork.org/scripts/423739/UPM%20Snow%20Helper%2010.user.js
// @updateURL https://update.greasyfork.org/scripts/423739/UPM%20Snow%20Helper%2010.meta.js
// ==/UserScript==
//
//
// Own name, team and signature to be used in messages sent via UPM Environment.
...
var defaultAssignmentGroup      = 'Metsä Group Service Desk Finland (24X7)';
...
var defaultCloseNotesEN         = 'Dear user, \n\nThis message is to confirm you that your request at UPM IT Service Desk has been resolved. \n\nSolution:  \n\nThank you for using UPM IT Service Desk.';
var defaultCloseNotesFI            = 'Hei,\n\nTämä viesti on vahvistus työpyyntönne ratkaisusta.\n\nRatkaisu: \n\nKiitos, että käytitte UPM IT Service Deskin palveluita.';
var defaultCloseNotesSE         = 'Hej,\n\nVill meddela att ditt ärende nu är löst och kommer att stängas.\n\nLösning: \n\nTack för att du kontaktat UPM IT Service Desk.';

var ServerCloseNotesEN         = 'Dear user, \n\nThis message is to confirm you that your request at UPM Service Desk has been resolved. \n\nSolution: No responsible person/group could be located and no other incidents have been raised regarding device hence closing the ticket as per process. \n\nThank you for using UPM Service Desk.';
...
var signatureEN                    = '\n\nBest Regards,\n' + ownName + '  | UPM IT Service Desk | itservicedesk@upm.com\nUPM IT Service Desk | P.O Box 380, FI-00101 Helsinki, Finland | www.upm.com\nUPM IT Service Desk contact number:\nTel. +358 2041 46000 (Finnish)\nTel. +358 9 876 6600 (English, Finnish, German, French, Russian and Polish)\nTel. +1 630 922 8650 (Option for users in the Americas)\nTel. +86 512 5229 5119 Ext. 3196 (Mandarin)\nhttps://intranet.upm.com/services/itservices/get_support/pages/four-ways-to-contact-it-service-desk.aspx';
var signatureFI                  = '\n\nYstävällisin Terveisin,\n' + ownName + '  | UPM IT Service Desk | itservicedesk@upm.com\nUPM IT Service Desk | P.O Box 380, FI-00101 Helsinki, Finland | www.upm.com\nUPM IT Service Desk contact number:\nTel. +358 2041 46000 (Finnish)\nTel. +358 9 876 6600 (English, Finnish, German, French, Russian and Polish)\nTel. +1 630 922 8650 (Option for users in the Americas)\nTel. +86 512 5229 5119 Ext. 3196 (Mandarin)\nhttps://intranet.upm.com/services/itservices/get_support/pages/four-ways-to-contact-it-service-desk.aspx';
...
var AntiLoopcookie = getCookie( "antiloopcookie" );
    ...
    var stateEl                 = $('#element\\.incident\\.sys_created_on'); //changed for INC
...
var OriginalDescVal1 = ''; // Set by a function below to be able to follow changes
    ...
    var contactType              = $('#label\\.incident\\.contact_type');
...
window.open('https://hclupm.service-now.com/nav_to.do?uri=%2Fsc_task.do%3Fsys_id%3D'+ gr.sys_id +'%26sysparm_stack%3D%26sysparm_view%3D', '_top');
...
var AntiLoopcookie = getCookie( "antiloopcookie" );
    ...
    var subStatus                    = $('select#sc_task\\.u_hold_reason');
    ...
    var attachmentButton         = $('a#header_add_attachment');
    var attachmentList           = $('li#attachment_list_items');
    var caller                   = $('input#sys_display\\.sc_task\\.u_created_for');
    var callerVal                  = $('input#sys_display\\.sc_task\\.u_created_for').val();
    var requestedFor                  = $('input#sys_display\\.sc_task\\.u_created_for').val();
...
var OriginalDescVal1 = ''; // Set by a function below to be able to follow changes
    ...
    var WatchlistElement             = $('#element\\.sc_task\\.watch_list'); //changed for Task
    ...
    var commentsToCaller         = $('textarea#activity-stream-comments-textarea');
...
var CallerValueSRQ = {}; // Globally scoped object following User inspect changes
    var TrasnsportState = $('#label\\.sc_task\\.u_transport_state');
       var MailAttachments = $('#label\\.control-label\\.col-xs-2');
...
100 */
            ...
            group.valWithDelay( 'HCL Service Desk Finland HD' );
 /*                        to.focus();
100
                         group.focus();
100 */
...
/*
 if ( groupLabel.length ) {
        ...
        $( '.me-button' ).click( function() {
            group.valWithDelay2( 'Metsä Group Service Desk Finland (24X7)' );
       //     group.focus();
            to.valWithDelay2( ownName );
         //   group.focus();
         //   to.focus();
...
|| originalDescription.val().match( /One+/i )
    ...
    if (r == true) {
        ...
        return;
            ...
            ) { alert("Firewall match found\n\nCygate is responsible for Metsä Firewall changes/modification\n\n17.3.2016");
            ...
            ) { alert("WAN match found\n\nSonera and Cygate are responsible for Metsä WAN changes/modification\n\n17.3.2016");
            ...
            } if ( originalDescription.val().match( /Metsä Web/i )
                    || shortDescription.val().match( /Metsä Web/i )
                    || originalDescription.val().match( /website/i )
                    || shortDescription.val().match( /website/i )

            ) { alert("Metsä Web/Website match found\n\nMetsä websites are handled by COMMS Digital Channels\n\n17.3.2016");
    ...
    if (r == true) {
        ...
        return;
            ...
            ) { alert('Identity match found\n\nThe identity manager for Metsä is: Jarkko Lehtola\n\n1/4/2016');
            ...
            ) { alert('License match found\n\nThe license manager for Metsä is: Alf Burge\n\n1/4/2016');
...
closeCode.valWithDelay( defaultCloseCode );
    ...
    */
       ...
       //ONSITE BUTTON // INDIA button

        if ( groupLabel.length ) {
        $( groupLabel ).append(
            '<div class="india-button" style="display:inline-block; border-radius: 3px; margin: 0px 0px 0px 145px; border: 1px solid black; background-color: #FFD8B1; padding: 0 9px; cursor: pointer;">India</div>'
        );
        $( '.india-button' ).click( function() {
           g_form.setValue('work_notes','Kindly assist the user.' );
           g_form.setValue('assignment_group','HCL Service Desk India HD' );
    ...
    || originalDescription.val().match( /griffin password/i )
           ...
           commentsToCaller.val( 'Access granted' ); //UPM Mailbox creation starts
            } else if ( internalWorkNotes.val().match( /type=/i )
            ) {            state.valWithDelay( '3' );
           commentsToCaller.val( 'Access Granted.' );
           g_form.setValue('work_notes','Access Granted.');

                } else if  ( shortDescription.val().match( /User Attribute Change for/i)
            ) {
                //    var userIDvalue = originalDescription;
                //    str.split('User ID: ').pop().split(' New location:')[0];
                    var userIDvalue1 = originalDescription.val();

var userIDvalue = userIDvalue1.split('User ID: ').pop().split('New location:')[0];
var userNEW_ADDRESS = userIDvalue1.split('New street:').pop().split('Old street:')[0];
var userOLD_ADDRESS = userIDvalue1.split('Old street:').pop().split('\n\n\n\n\n\nWith best regards,')[0];

var ShortDescriptionValue = shortDescription.val();
var ShortDescriptionWords = ShortDescriptionValue.split(' ');
var AttributeUserName = ShortDescriptionWords[4];

                state.val( '-5' );
                subStatus.val('Customer Action');
           g_form.setValue('work_notes','Email sent to the user.' );
           commentsToCaller.val( 'Email sent to the user.' );

             //                                      commentsToCaller.val( 'käyttäjänimi: ' + AttributeUserName + ' Käyttäjätili ' + userIDvalue + ' New Street: '+ userNEW_ADDRESS +' old street ' + userOLD_ADDRESS + '' );

                              window.location.href = "mailto: "+ userIDvalue +" ?subject="+ incidentNr +""+ shortDescriptionText +"&body=Hei%20"+ AttributeUserName +",%0A%0ATämä%20koskee%20UPM%20osoite%20tai%20organisaatio%20muutostasi%20vanhasta%20sijainnista:%20"+ userOLD_ADDRESS +"%20%20uuteen%20sijaintiin:%20"+ userNEW_ADDRESS +"%20.%0A%0AMeidän%20pitäisi%20konfiguroida%20muutamia%20asioita%20järjestelmässämme%20varmistaaksemme%20että%20kaikki%20toimii%20kuten%20pitää%20jatkossakin.%0AEli%20tarvitsemme%20hieman%20lisätietoa%20teiltä:%0A%0A1.%20Jonkin%20kollegan%20V-%20tai%20K-tunnus%20kuka%20työskentelee%20samassa%20tulevassa%20sijainnissasi,%20minne%20olet%20siirtymässä.%0A2.%20Käytätkö%20UPMn%20tietokonetta?%20Jos%20käytät,%20niin%20vahvistatko%20vielä%20jatkatko%20saman%20tietokoneen%20käyttämistä,%20mikä%20on%20vanhassa%20sijainnissa%20teille%20annettu%20(tämä%20koskee%20mahdollista%20tietojen%20varmuuskopiointia).%0A3.%20Mitä%20Windowsia%20käytät%20(7,%208%20vai%2010)?%20(Voit%20tarkistaa%20käyttöjärjestelmän%20sijainnista%20START%20->%20CONTROL%20PANEL%20->%20SYSTEM%20tai%20näpäyttämällä%20START%20->%20CONTROL%20PANEL%20->%20SYSTEM%20AND%20SECURITY%20tai%20vaan%20kirjoittamalla%0AJäämme%20odottamaan%20vastaustanne,%20jotta%20voimme%20tehdä%20muutoksen.%0A%0AJos%20tarvitsette%20apua,%20älkää%20epäröikö%20ottaa%20yhteyttä%20IT%20service%20deskiin.%20";
...
// NO longer disabled
                } else if ( originalDescription.val().match( /UPM External Full Suite User/i ) && CallerValueSRQ.CallerEmail.match( /tieto.com/i )
            ) {//DEFINE Height
...
EmailCreationName();
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nTieto EMAIL DETECTED assign F1 license NO YAMMER\n\n1.1 Change E-Mail to: \n'+ CallerValueSRQ.CallerEmail +'\n\n1.2  UPM Target Address:\nSMTP:'+ CallerValueSRQ.CallerEmail +'\n\n2. Proxy Address Collection:\nSMTP:'+ CallerValueSRQ.CallerEmail +'\nNOTES:'+ firstName +' '+ lastName +'TE/UPM External@\n\n3. UPM Mailbox Type:\ntype=DESKLESS;status=NEWLIC\n\n4. UPM Lync Entitlement:\n0' );
                ...
                } else if ( CallerValueSRQ.ManagerEmail.match( /marja.varri@upm.com/i )
            ) {//DEFINE Height
alert("Marja Värri detected as manager, please check if SD personel if not proceed as per process");
                    $(function () {
    $("textarea").each(function () {
        this.style.height = (this.scrollHeight+290)+'px';
    });
...
EmailCreationName();
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nHCL EMAIL DETECTED and Marja Varri is the Manager assign E3 License - [x] Microsoft Forms (Plan E3) - NO YAMMER\n\n1.1 Change E-Mail to: \n'+ CallerValueSRQ.CallerEmail +'\n\n1.2  UPM Target Address:\nUPM Target Address: [DO NOT TOUCH (if empty assign to MDS communications for investigation)]\n\n2. Proxy Address Collection:\nSMTP:'+ CallerValueSRQ.CallerEmail +'\nNOTES:'+ firstName +' '+ lastName +'/USC/UPM External@\n\n3. UPM Mailbox Type:\ntype=DESKLESS;status=NEWMBX\n\n4. UPM Lync Entitlement:\n15' );

                } else if ( originalDescription.val().match( /UPM External Deskless User/i ) && CallerValueSRQ.CallerEmail.match( /hcl.com/i ) && CallerValueSRQ.ManagerEmail.match( /marja.varri@upm.com/i )
            ) {//DEFINE Height
...
EmailCreationName();
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nHCL EMAIL DETECTED and Marja Varri is the Manager assign F1 license NO YAMMER\n\n1.1 Change E-Mail to: \n'+ CallerValueSRQ.CallerEmail +'\n\n1.2  UPM Target Address:\nSMTP:'+ CallerValueSRQ.CallerEmail +'\n\n2. Proxy Address Collection:\nSMTP:'+ CallerValueSRQ.CallerEmail +'\nNOTES:'+ firstName +' '+ lastName +'/USC/UPM External@\n\n3. UPM Mailbox Type:\ntype=DESKLESS;status=NEWLIC\n\n4. UPM Lync Entitlement:\n0' );
                ...
                } else if ( CallerValueSRQ.CallerEmail.match( /genpact.com/i )
            ) {
                //DEFINE Height
                    $(function () {
    $("textarea").each(function () {
...
EmailCreationName();
           ...
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nE3 License - Include Microsoft Forms (Plan E3). - NO YAMMER\n\n1.1 Change E-Mail to:\n'+ firstName +'.'+ lastName +'@upm.com\n\n1.2  UPM Target Address:\nUPM Target Address: [DO NOT TOUCH (if empty assign to MDS communications for investigation)]\n\n2. Proxy Address Collection:\nSMTP:'+ firstName +'.'+ lastName +'@upm.com\nsmtp:'+ firstName +'.'+ lastName +'@upm-kymmene.com\nNOTES:'+ firstName +' '+ lastName +'/GHO/UPM@\n\n3. UPM Mailbox Type:\ntype=FULL;status=NEWMBX\n\n4. UPM Lync Entitlement:\n15' );

                } else if ( originalDescription.val().match( /Office 365 Full Suite/i )
            ) {
                //DEFINE Height
                    $(function () {
    $("textarea").each(function () {
...
EmailCreationName();
           ...
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\ntype=FULL;status=NEWMBX\n\n4. UPM Lync Entitlement:\n15' );
               ...
               } else if ( originalDescription.val().match( /Office 365 Deskless/i ) && originalDescription.val().match( /v79|v78/i )
            ) {
                //DEFINE Height
                    $(function () {
    $("textarea").each(function () {
...
EmailCreationName();
           ...
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\ntype=DESKLESS;status=NEWLIC\n\n4. UPM Lync Entitlement:\n0' );
                 ...
                 } else if ( originalDescription.val().match( /Office 365 Deskless/i )
            ) {
                //DEFINE Height
                    $(function () {
    $("textarea").each(function () {
...
EmailCreationName();
           ...
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\ntype=DESKLESS;status=NEWMBX\n\n4. UPM Lync Entitlement:\n0' );
                ...
                } else if ( originalDescription.val().match( /UPM External Deskless User/i ) && CallerValueSRQ.CallerEmail.match( /genpact.com/i )
            ) {
 //DEFINE Height
                    $(function () {
    $("textarea").each(function () {
...
EmailCreationName();
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nAssign to Comms team (Deepak) for final evaluation\n\n1.1 Change E-Mail to:\n'+ firstName +'.'+ lastName +'@upm.com\n\n1.2  UPM Target Address:\nSMTP:'+ CallerValueSRQ.CallerEmail +'\n\n2. Proxy Address Collection:\nSMTP:'+ firstName +'.'+ lastName +'@upm.com\nsmtp:'+ firstName +'.'+ lastName +'@upm-kymmene.com\nNOTES:'+ firstName +' '+ lastName +'/GHO/UPM@\n\n3. UPM Mailbox Type:\ntype=DESKLESS;status=NEWLIC\n\n4. UPM Lync Entitlement:\n0' );
                ...
                }  else if ( originalDescription.val().match( /upm internal full suite/i )
            ) {
//DEFINE Height
                    $(function () {
    $("textarea").each(function () {
...
EmailCreationName();
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nE3 License - Include Microsoft Forms (Plan E3). -\n\n1. UPM Target Address:\nUPM Target Address: [DO NOT TOUCH (if empty assign to MDS communications for investigation)]\n\n2. Proxy Address Collection:\nSMTP:'+ firstName +'.'+ lastName +'@upm.com\nsmtp:'+ firstName +'.'+ lastName +'@upm-kymmene.com\nNOTES:'+ firstName +' '+ lastName +'*****@\n\n3. UPM Mailbox Type:\ntype=FULL;status=NEWMBX\n\n4. UPM Lync Entitlement:\n15' );

                } else if ( originalDescription.val().match( /UPM Internal 25GB Size Mailbox/i )
            ) {//DEFINE Height
                    $(function () {
    $("textarea").each(function () {
        this.style.height = (this.scrollHeight+310)+'px';
...
EmailCreationName();
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nE3 License - Include Microsoft Forms (Plan E3). -\n\n1. UPM Target Address:\nUPM Target Address: [DO NOT TOUCH (if empty assign to MDS communications for investigation)]\n\n2. Proxy Address Collection:\nSMTP:'+ firstName +'.'+ lastName +'@upm.com\nsmtp:'+ firstName +'.'+ lastName +'@upm-kymmene.com\nNOTES:'+ firstName +' '+ lastName +'*****@\n\n3. UPM Mailbox Type:\nMBX=50GB;TYPE=EP2D;\n\n4. UPM Lync Entitlement:\n15' );
                ...
                } else if ( originalDescription.val().match( /UPM Internal Deskless User/i )
            ) {//DEFINE Height
                    $(function () {
    $("textarea").each(function () {
        this.style.height = (this.scrollHeight+310)+'px';
...
EmailCreationName();
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nassign F1 license, NO YAMMER\n\n1. UPM Target Address:\nUPM Target Address: [DO NOT TOUCH (if empty assign to MDS communications for investigation)]\n\n2. Proxy Address Collection:\nSMTP:'+ firstName +'.'+ lastName +'@upm.com\nsmtp:'+ firstName +'.'+ lastName +'@upm-kymmene.com\nNOTES:'+ firstName +' '+ lastName +'*****@\n\n3. UPM Mailbox Type:\ntype=DESKLESS;status=NEWMBX\n\n4. UPM Lync Entitlement:\n0' );
                ...
                } else if ( originalDescription.val().match( /UPM External Full Suite User/i )
            ) {
                //DEFINE Height
                    $(function () {
    $("textarea").each(function () {
...
EmailCreationName();
           ...
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nE3 License - Include Microsoft Forms (Plan E3). - NO YAMMER\n\n1.1 Change E-Mail to:\n'+ firstName +'.'+ lastName +'@visitor.upm.com\n\n1.2  UPM Target Address:\nUPM Target Address: [DO NOT TOUCH (if empty assign to MDS communications for investigation)]\n\n2. Proxy Address Collection:\nSMTP:'+ firstName +'.'+ lastName +'@visitor.upm.com\nsmtp:'+ firstName +'.'+ lastName +'@visitor.upm-kymmene.com\nNOTES:'+ firstName +' '+ lastName +'/UPM External@\n\n3. UPM Mailbox Type:\ntype=FULL;status=NEWMBX\n\n4. UPM Lync Entitlement:\n15' );

                } else if ( originalDescription.val().match( /UPM External Deskless User/i )
            ) {
 //DEFINE Height
                    $(function () {
    $("textarea").each(function () {
...
EmailCreationName();
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nassign F1 license NO YAMMER\n\n1.1 Change E-Mail to: \n'+ CallerValueSRQ.CallerEmail +'\n\n1.2  UPM Target Address:\nSMTP:'+ CallerValueSRQ.CallerEmail +'\n\n2. Proxy Address Collection:\nSMTP:'+ CallerValueSRQ.CallerEmail +'\nNOTES:'+ firstName +' '+ lastName +'/UPM External@\n\nUPM Mailbox Type:\ntype=DESKLESS;status=NEWLIC' );
                ...
                }  else if ( originalDescription.val().match( /UPM Raflatac Internal Full Suite UserDISABLED/i )
                    || originalDescription.val().match( /UPM Raflatac External Full Suite UserDISABLED/i )
            ) {//DEFINE Height
                    $(function () {
    $("textarea").each(function () {
...
EmailCreationName();
                    g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nE3 License - Include Microsoft Forms (Plan E3). - - If External (v-id user) No Yammer\n\n1.1 Change E-Mail to: \n'+ firstName +'.'+ lastName +'@upmraflatac.com\n\n1.2 UPM Target Address: \nUPM Target Address: [DO NOT TOUCH (if empty assign to MDS communications for investigation)]\n\n2. Proxy Address Collection:\nNOTES:'+ firstName +' '+ lastName +'/RAF/UPM@\n\n3. UPM Mailbox Type:\ntype=FULL;status=NEWMBX\n\n4. UPM Lync Entitlement:\n15');

                } else if ( originalDescription.val().match( /UPM Raflatac 25GB Size Mailbox/i )
            ) {
                //DEFINE Height
                    $(function () {
    $("textarea").each(function () {
...
EmailCreationName();
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\n1.1 Change E-Mail to: \n'+ firstName +'.'+ lastName +'@upmraflatac.com\n\n1.2 UPM Target Address: \nUPM Target Address: [DO NOT TOUCH (if empty assign to MDS communications for investigation)]\n\n2. Proxy Address Collection:\nSMTP:'+ firstName +'.'+ lastName +'@upmraflatac.com\nsmtp:'+ firstName +'.'+ lastName +'@upm.com\nsmtp:'+ firstName +'.'+ lastName +'@upm-kymmene.com\nNOTES:'+ firstName +' '+ lastName +'*****@\n\n3. UPM Mailbox Type:\nMBX=50GB;TYPE=EP2D;\n\n4. UPM Lync Entitlement:\n15' );


                } else if ( originalDescription.val().match( /UPM Raflatac Internal Deskless User/i )
            ) {//DEFINE Height
...
EmailCreationName();
           g_form.setValue('work_notes','Copy the user account name to AD to check if there are duplicate accounts\n\nassign F1 license, - If External (v-id user) No Yammer\n\n1.1 Change E-Mail to: \n'+ firstName +'.'+ lastName +'@upmraflatac.com\n\n1.2 UPM Target Address: \nUPM Target Address: [DO NOT TOUCH (if empty assign to MDS communications for investigation)]\n\n2. Proxy Address Collection:\nSMTP:'+ firstName +'.'+ lastName +'@upmraflatac.com\nsmtp:'+ firstName +'.'+ lastName +'@upm.com\nsmtp:'+ firstName +'.'+ lastName +'@upm-kymmene.com\nNOTES:'+ firstName +' '+ lastName +'/RAF/UPM@\n\n3. UPM Mailbox Type:\ntype=FULL;status=NEWMBX\n\n4. UPM Lync Entitlement:\n0' );
                ...
                } else if ( originalDescription.val().match( /UPM Raflatac External Deskless User/i )
            ) {//DEFINE Height
    ...
    */
    /* NOT IN UPM USE
        ...
        $( '.BaSeEN-button' ).click( function() {
            ...
            window.location.href = "mailto: "+ callerVal +" ?subject="+ incidentNr +" "+ shortDescriptionText +"&body=Hi%20"+ callerFirstName +",%0D%0A%0D%0AYour new SAP password is: ---%0D%0A%0D%0AIf you still are experiencing issues with the login contact Servicedesk by calling +358204146000 or by contacting us using chat: +358204146000";
...
*/
    ...
    firstName = fullName[0];
            ...
            commentsToCaller.valWithDelay( 'Hei '+ firstName +',\n\n'+ signatureFI +'' );
                  ...
                  if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="AUFen-button"  style="display:inline-block; border-radius: 3px; margin: 0px 0px 0px -196px; border: 1px solid black; background-color: #99ddff; padding: 0 2px; cursor: pointer;">EN</div>'
        );
...
email_client_open.setBody('Dear Sir, ...');  }
            MsgCcUI.valWithDelay2( 'OMC-UPM-SD-Finland@hcl.com;' );
        });
    }
    ...
    firstName = fullName[0];
        ...
        $( '.FUmailFI-button' ).click( function() {
            ...
            state.valWithDelay( '-5' );
            subStatus.valWithDelay2('Customer Action');
            window.location.href = "mailto: "+ CallerValueINC.CallerEmail +" ?subject="+ incidentNr +" "+ shortDescriptionText +"&body=Hei%20"+ callerFirstName +",%0D%0A%0D%0AUPM Service Desk on yrittänyt ottaa teihin yhteyttä liittyen tapaukseen: "+ incidentNr +"%0D%0A%0D%0AHaluaisimme tutkia tapaustasi tarkemmin etäyhteyden avulla%0D%0A%0D%0AVoit ottaa meihin yhteyttä milloin vain soittamalla numeroon: +358 2041 46000 tai ottamalla yhteyttä Service Desk chattiin: itservicedeskchat@sdupmchat.com%0D%0AOlemme tavoitettavissa 24/7 ja autamme sinua mielellämme tämän tapauksen kanssa.%0D%0A%0D%0A";

            } else {
    var fullName = requestedFor.split(' ');
    firstName = fullName[0];
            state.valWithDelay( '-5' );
            window.location.href = "mailto: "+ CallerValueSRQ.CallerEmail +" ?subject="+ ritmNr +" - ["+ incidentNr +"] - "+ shortDescriptionText +"&body=Hei%20"+ firstName +",%0D%0A%0D%0AUPM Service Desk on yrittänyt ottaa teihin yhteyttä liittyen tapaukseen: "+ incidentNr +"%0D%0A%0D%0AHaluaisimme tutkia tapaustasi tarkemmin etäyhteyden avulla%0D%0A%0D%0AVoit ottaa meihin yhteyttä milloin vain soittamalla numeroon: +358 2041 46000 tai ottamalla yhteyttä Service Desk chattiin: itservicedeskchat@sdupmchat.com%0D%0AOlemme tavoitettavissa 24/7 ja autamme sinua mielellämme tämän tapauksen kanssa.%0D%0A%0D%0A";
            subStatus.valWithDelay2('Customer Action');
            }
             });
    }
        ...
        $( '.FUmailEN-button' ).click( function() {
            ...
            state.valWithDelay( '-5' );
            subStatus.valWithDelay2('Customer Action');
            window.location.href = "mailto: "+ CallerValueINC.CallerEmail +" ?subject="+ incidentNr +" "+ shortDescriptionText +"&body=Hi%20"+ callerFirstName +",%0D%0A%0D%0AUPMService Desk has tried to contact you Regarding ticket: "+ incidentNr +"%0D%0A%0D%0AWe would need to look into your issue remotely.%0D%0A%0D%0AYou can contact Service Desk at any time on +358 2041 46000 or by the Service Desk chat: itservicedeskchat@sdupmchat.com%0D%0AWe are available 24/7 and will gladly assist you with this issue.%0D%0A%0D%0A";

            } else {
     var fullName = requestedFor.split(' ');
    firstName = fullName[0];
            state.valWithDelay( '-5' );
                      window.location.href = "mailto: "+ CallerValueSRQ.CallerEmail +" ?subject="+ ritmNr +" - ["+ incidentNr +"] - "+ shortDescriptionText +"&body=Hi%20"+ firstName +",%0D%0A%0D%0AUPMService Desk has tried to contact you Regarding ticket: "+ incidentNr +"%0D%0A%0D%0AWe would need to look into your issue remotely.%0D%0A%0D%0AYou can contact Service Desk at any time on +358 2041 46000 or by the Service Desk chat: itservicedeskchat@sdupmchat.com%0D%0AWe are available 24/7 and will gladly assist you with this issue.%0D%0A%0D%0A";
            subStatus.valWithDelay2('Customer Action');
            }
                      ...
                      if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="2users-button"  style="display:inline-block; border-radius: 3px; margin: 0 5px; border: 1px solid black; background-color: #ffdb99; padding: 0 2px; cursor: pointer;">Mailbox Name Conflict</div>'
        );
        $( '.2users-button' ).click( function() {
            if ( $('#incident\\.do').length ) {
            setCallerNames();
            state.valWithDelay( '-5' );
            subStatus.valWithDelay2('UPM Internal');
            window.location.href = "mailto: Manager@upm.com ?subject="+ incidentNr +" "+ shortDescriptionText +"&body=INC detected. Mail Creation requests should be created in Task format.%20,\n\n";
    ...
    lastName = fullName[1];
       ...
       state.valWithDelay( '-5' );
            subStatus.valWithDelay2('UPM Internal');
            window.location.href = "mailto: "+ CallerValueSRQ.ManagerEmail +" ?subject="+ incidentNr +" Kaksi samannimistä henkilöä järjestelmässä: "+ firstName +" "+ lastName +"&body=Hei%20"+ CallerValueSRQ.ManagerFirstName +",%0D%0A%0D%0ASaatiin pyyntö jossa sähköposti tiliä pyydetään seuraavalle käyttäjälle: "+ Userid +"%0D%0A%0D%0AMilloin sähköposti tili tulisi luoda?%0D%0A%0D%0ALöysimme toisen tilin jolla on sama nimi, voitko varmistaa onko kyseessä sama henkilö? Jos on eri henkilöt kyseessä satutko tietämään heidän toisen nimensä jotta saadaan tilit eroteltua?%0D%0A%0D%0A"+ firstName +" "+ lastName +": "+ Userid +" - "+ CallerValueSRQ.CallerTitle +"%0D%0A"+ firstName +" "+ lastName +": VanhaID - Työnimike";   }
           commentsToCaller.valWithDelay( 'Two users with the same name were detected. Asked the users manager if both accounts are related to the same person and when the transfer should be initiated.' );
                      ...
                      if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="2usersEN-button"  style="display:inline-block; border-radius: 3px; margin: 3px 0px 0px 0px; border: 1px solid black; background-color: #ffdb99; padding: 0 2px; cursor: pointer;">EN</div>'
        );
        $( '.2usersEN-button' ).click( function() {
            if ( $('#incident\\.do').length ) {
            setCallerNames();
            state.valWithDelay( '-5' );
            subStatus.valWithDelay2('UPM Internal');
    ...
    lastName = fullName[1];
       ...
       state.valWithDelay( '-5' );
            ...
            window.location.href = "mailto: "+ CallerValueSRQ.ManagerEmail +" ?subject="+ incidentNr +" Two users with the same name were detected in our system: "+ firstName +" "+ lastName +"&body=Hi%20"+ CallerValueSRQ.ManagerFirstName +",%0D%0A%0D%0AWe received a request where a new email address for the following user account is requested: "+ Userid +"%0D%0A%0D%0AWhen Should the mailbox be created?%0D%0A%0D%0AWe found another user account with the same name. Could you confirm if this is the same person? If these are different persons do you happen to know what their middle names are so we can separate the accounts?%0D%0A%0D%0A"+ firstName +" "+ lastName +": "+ Userid +" - "+ CallerValueSRQ.CallerTitle +"%0D%0A"+ firstName +" "+ lastName +": olduserID - Role";   }
           commentsToCaller.valWithDelay( 'Two users with the same name were detected. Asked the users manager if both accounts are related to the same person and when the transfer should be initiated.' );
        });
    }
                  ...
                  if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="CheckSpheraCase-button"  style="display:inline-block; border-radius: 3px; margin: 0px 0px 0px 10px; border: 1px solid black; background-color: #ffd6d6; padding: 0 2px; cursor: pointer;">Sphere#</div>'
        );
        $( '.CheckSpheraCase-button' ).click( function() {
setShortDescription();
   window.open( 'https://scn.spherasolutions.com/client/ticketdetail.aspx?cbx=0&search=&product=Rivo&productID=&id='+ summaryLastWord +'');
            });
    }

/*                  if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="CheckRivoCase-button"  style="display:inline-block; border-radius: 3px; margin: 0px 0px 0px 10px; border: 1px solid black; background-color: #ffd6d6; padding: 0 2px; cursor: pointer;">Rivo#</div>'
        );
        $( '.CheckRivoCase-button' ).click( function() {
setShortDescription();
   window.open( 'https://support.rivosoftware.com/hc/en-gb/requests/'+ summaryLastWord +'');
            });
    }
*/
/*
                  if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="AUFfi-button"  style="display:inline-block; border-radius: 3px; margin: 0px 0px 0px 10px; border: 1px solid black; background-color:  #99ddff; padding: 0 2px; cursor: pointer;">AUF FI</div>'
        );
        $( '.AUFfi-button' ).click( function() {
            if ( $('#incident\\.do').length && commentsToCaller.val().match( /Hei/i ) ) {
                state.valWithDelay( '-5' );
    ...
    firstName = fullName[0];
            ...
            subStatus.valWithDelay2('Customer Action');
                  ...
                  if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="AUFen-button"  style="display:inline-block; border-radius: 3px; margin: 0px 0px 0px 5px; border: 1px solid black; background-color: #99ddff; padding: 0 2px; cursor: pointer;">EN</div>'
        );
        $( '.AUFen-button' ).click( function() {
            if ( $('#incident\\.do').length && commentsToCaller.val().match( /Hi/i ) ) {
                state.valWithDelay( '-5' );
...
*/
                      ...
                      if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="pwResetManager-button"  style="display:inline-block; border-radius: 3px; margin: 0 5px; border: 1px solid black; background-color: #ecff7f; padding: 0 2px; cursor: pointer;">PW Reset > Manager</div>'
        );
        $( '.pwResetManager-button' ).click( function() {
            if ( $('#incident\\.do').length ) {
            setCallerNames();
            window.location.href = "mailto: "+ CallerValueINC.CallerEmail +" ?subject="+ incidentNr +" "+ shortDescriptionText +"&body=Hei%20"+ callerFirstName +",";
   } else {
       var fullName = requestedFor.split(' ');
    firstName = fullName[0];
            window.location.href = "mailto: "+ CallerValueSRQ.ManagerEmail +" ?subject=Alaisesi "+ requestedFor +" uusi salasana&body=Hei%20"+ CallerValueSRQ.ManagerFirstName +",%0A%0AAlaisellasi%20ei%20ole%20merkitty%20puhelinnumeroa%20UPM:n%20järjestelmään%20joten%20lähetämme%20salasanan%20sinulle.%0A%0AAlaisesi%20uusi%20salasana%20on%20---%0A%0A";
   }
});
    }
                      ...
                      if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="SMSuser-button"  style="display:inline-block; border-radius: 3px; margin: 0 1px; border: 1px solid black; background-color: #ffffb2; padding: 0 2px; cursor: pointer;">SMS</div>'
        );
        $( '.SMSuser-button' ).click( function() {
            if ( $('#incident\\.do').length ) {
            setCallerNames();
            window.location.href = "mailto:@sms.upm.com?subject=Uusi salasana on:&body=%20";
   } else {
       var fullName = requestedFor.split(' ');
    firstName = fullName[0];
            window.location.href = "mailto: "+ CallerValueSRQ.MobilePhone +"@sms.upm.com?subject=Uusi salasana on:&body=%20";
   }
});
    }
                      ...
                      if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="LotusFI-button"  style="display:inline-block; border-radius: 3px; margin: 5px 0px 0px 5px; border: 1px solid black; background-color: #aa8cc5; padding: 0 2px; cursor: pointer;">Lotus Password Reset FI</div>'
        );
        $( '.LotusFI-button' ).click( function() {
            if ( $('#incident\\.do').length ) {
            setCallerNames();
            state.valWithDelay( '-5' );
            window.location.href = "mailto: "+ CallerValueINC.CallerEmail +" ?subject="+ incidentNr +" "+ shortDescriptionText +"&body=Hei%20"+ callerFirstName +",%0A%0ALotus Notes salasana on nyt resetoitu.%0A%0AUusi Salasana: --- %0A%0ASalasanan replikointiaika on noin: 30min-1h, jonka jälkeen voit kirjautua.%0A%0AIlmoitattehan toimiiko uusi salasana, niin voimme sulkea tapauksen.";
            subStatus.valWithDelay2('Customer Action');
   } else {
       var fullName = requestedFor.split(' ');
    firstName = fullName[0];
            state.valWithDelay( '-5' );
            subStatus.valWithDelay2('Customer Action');
            window.location.href = "mailto: "+ CallerValueSRQ.CallerEmail +" ?subject="+ ritmNr +" - ["+ incidentNr +"] - "+ shortDescriptionText +"&body=Hei%20"+ firstName +",%0A%0ALotus Notes salasana on nyt resetoitu.%0A%0AUusi Salasana: --- %0A%0ASalasanan replikointiaika on noin: 30min-1h, jonka jälkeen voit kirjautua.%0A%0AIlmoitattehan toimiiko uusi salasana, niin voimme sulkea tapauksen.";
   }
});
    }
                  ...
                  if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="LotusEN-button"  style="display:inline-block; border-radius: 3px; margin: 0 2px; border: 1px solid black; background-color: #aa8cc5; padding: 0 2px; cursor: pointer;">EN</div>'
        );
        $( '.LotusEN-button' ).click( function() {
            if ( $('#incident\\.do').length ) {
            setCallerNames();
            state.valWithDelay( '-5' );
            window.location.href = "mailto: "+ CallerValueINC.CallerEmail +"?subject="+ incidentNr +" "+ shortDescriptionText +"&body=Hi%20"+ callerFirstName +",%0A%0AYour Lotus Notes password has been reset.%0A%0ANew Password: ---%0A%0AThe Password replication time is approximately 30min-1h, then you should be able to login.%0A%0APlease let us know if you're able to login so that we can close the ticket. ";
            subStatus.valWithDelay2('Customer Action');
    ...
    firstName = fullName[0];
            state.valWithDelay( '-5' );
            window.location.href = "mailto: "+ CallerValueSRQ.CallerEmail +"?subject="+ ritmNr +" - ["+ incidentNr +"] - "+ shortDescriptionText +"&body=Hi%20"+ firstName +",%0A%0AYour Lotus Notes password has been reset.%0A%0ANew Password: ---%0A%0AThe Password replication time is approximately 30min-1h, then you should be able to login.%0A%0APlease let us know if you're able to login so that we can close the ticket.";
            subStatus.valWithDelay2('Customer Action');
                      ...
                      if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="Folderaccess-button"  style="display:inline-block; border-radius: 3px; margin: 0 5px; border: 1px solid black; background-color: #A9D0D1; padding: 0 2px; cursor: pointer;">Folder Access Approval</div>'
        );
        $( '.Folderaccess-button' ).click( function() {
            if ( $('#incident\\.do').length ) {
            setCallerNames();
            window.location.href = "mailto: "+ CallerValueINC.CallerEmail +" ?subject="+ incidentNr +" "+ shortDescriptionText +"&body=Hei%20"+ callerFirstName +",";
   } else {
       var fullName = requestedFor.split(' ');
    firstName = fullName[0];
            window.location.href = "mailto: "+ CallerValueSRQ.ManagerEmail +" ?subject="+ ritmNr +" - ["+ incidentNr +"] - "+ requestedFor +" Tarvitsee kansio oikeudet&body=Hei%20"+ CallerValueSRQ.ManagerFirstName +",%0A%0A"+ requestedFor +"%20on%20pyytänyt%20---%20oikeuksia%20seuraavaan%20kansioon%0A%0A---%0A%0AVoitteko%20ystävällisesti%20hyväksyä%20tai%20hylätä%20kyseisen%20oikeuspyynnön%20mahdollisimman%20pian%20jotta%20saamme%20sen%20pikaisesti%20käsittelyyn?";
   }
});
    }
                      ...
                      if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="FolderaccessEn-button"  style="display:inline-block; border-radius: 3px; margin: 0 5px; border: 1px solid black; background-color: #A9D0D1; padding: 0 2px; cursor: pointer;">EN</div>'
        );
        $( '.FolderaccessEn-button' ).click( function() {
            if ( $('#incident\\.do').length ) {
            setCallerNames();
            window.location.href = "mailto: "+ CallerValueINC.CallerEmail +" ?subject="+ incidentNr +" "+ shortDescriptionText +"&body=Hi%20"+ callerFirstName +",";
   } else {
       var fullName = requestedFor.split(' ');
    firstName = fullName[0];
            window.location.href = "mailto: "+ CallerValueSRQ.ManagerEmail +" ?subject="+ ritmNr +" - ["+ incidentNr +"] - Folder access has been requested for "+ requestedFor +"&body=Hi%20"+ CallerValueSRQ.ManagerFirstName +",%0A%0A"+ requestedFor +"%20has%20requested%20---%20access%20to%20the%20following%20folder%0A%0A---%0A%0ACan%20you%20kindly%20approve/disapprove%20this%20access%20request%20as%20soon%20as%20possible%20so%20that%20we%20may%20proceed%20accordingly.";
   }
});
    }
                      ...
                      if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="AttributeChangeFI-button"  style="display:inline-block; border-radius: 3px; margin: 0 5px; border: 1px solid black; background-color: #FF5733; padding: 0 2px; cursor: pointer;">Attribute Change</div>'
        );
        $( '.AttributeChangeFI-button' ).click( function() {
                    var userIDvalue1 = originalDescription.val();
...
var AttributeUserName = ShortDescriptionWords[4];
       ...
       //    g_form.setValue('work_notes','Email sent to the user.' );
           commentsToCaller.val( 'Email sent to the user.' );
                             ...
                             window.location.href = "mailto: "+ userIDvalue +" ?subject="+ incidentNr +""+ shortDescriptionText +"&body=Hei%20"+ AttributeUserName +",%0A%0ATämä%20koskee%20UPM%20osoite%20tai%20organisaatio%20muutostasi%20vanhasta%20sijainnista:%20"+ userOLD_ADDRESS +"%20%20uuteen%20sijaintiin:%20"+ userNEW_ADDRESS +"%20.%0A%0AMeidän%20pitäisi%20konfiguroida%20muutamia%20asioita%20järjestelmässämme%20varmistaaksemme%20että%20kaikki%20toimii%20kuten%20pitää%20jatkossakin.%0AEli%20tarvitsemme%20hieman%20lisätietoa%20teiltä:%0A%0A1.%20Jonkin%20kollegan%20V-%20tai%20K-tunnus%20kuka%20työskentelee%20samassa%20tulevassa%20sijainnissasi,%20minne%20olet%20siirtymässä.%0A2.%20Käytätkö%20UPMn%20tietokonetta?%20Jos%20käytät,%20niin%20vahvistatko%20vielä%20jatkatko%20saman%20tietokoneen%20käyttämistä,%20mikä%20on%20vanhassa%20sijainnissa%20teille%20annettu%20(tämä%20koskee%20mahdollista%20tietojen%20varmuuskopiointia).%0A3.%20Mitä%20Windowsia%20käytät%20(7,%208%20vai%2010)?%20(Voit%20tarkistaa%20käyttöjärjestelmän%20sijainnista%20START%20->%20CONTROL%20PANEL%20->%20SYSTEM%20tai%20näpäyttämällä%20START%20->%20CONTROL%20PANEL%20->%20SYSTEM%20AND%20SECURITY%20tai%20vaan%20kirjoittamalla%0AJäämme%20odottamaan%20vastaustanne,%20jotta%20voimme%20tehdä%20muutoksen.%0A%0AJos%20tarvitsette%20apua,%20älkää%20epäröikö%20ottaa%20yhteyttä%20IT%20service%20deskiin.%20%0A%0A";
        }
);
    }
                      ...
                      if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="AttributeChangeEN-button"  style="display:inline-block; border-radius: 3px; margin: 0 5px; border: 1px solid black; background-color: #FF5733; padding: 0 2px; cursor: pointer;">EN</div>'
        );
        $( '.AttributeChangeEN-button' ).click( function() {
                    var userIDvalue1 = originalDescription.val();
...
var AttributeUserName = ShortDescriptionWords[4];
           ...
           commentsToCaller.val( 'Email sent to the user.' );
            ...
            window.location.href = "mailto: "+ userIDvalue +" ?subject="+ incidentNr +""+ shortDescriptionText +"&body=Hi%20"+ AttributeUserName +",%0A%0AThis%20is%20regarding%20your%20UPM%20location%20that%20has%20been%20changed%20from%20old:%20"+ userOLD_ADDRESS +"%20to%20new%20:%20"+ userNEW_ADDRESS +"%0A%0AFew%20things%20we%20need%20to%20configure%20in%20our%20system%20to%20make%20sure%20that%20everything%20is%20working%20fine%20with%20your%20workstation%20and%20policies.%20%20%0AIn%20order%20to%20do%20that%20we%20need%20below%20information%20from%20you:%0A%0A1.%20%20%20%20%20%20%20%20%20%20A%20V-id%20or%20K-id%20of%20one%20of%20your%20colleague%20who%20is%20working%20at%20the%20same%20location%20which%20you%20are%20transferred%20to%20right%20now.%0A2.%20%20%20%20%20%20%20%20%20%20Are%20you%20provided%20with%20a%20UPM%20laptop/Workstation?%20If%20so,%20please%20confirm%20if%20you%20are%20continuing%20with%20the%20same%20laptop%20from%20your%20old%20location%20or%20a%20new%20one%20is%20given.%20(This%20is%20for%20data%20backup%20purpose.)%0A3.%20%20%20%20%20%20%20%20%20%20Is%20it%20a%20Windows%207%20/%208%20/%2010%20operation%20system?%20(You%20can%20check%20it%20by%20following%20the%20path:%20Click%20Start>%20Control%20Panel%20>%20System%20or%20Click%20Start>%20Control%20Panel%20>%20System%20and%20Security%20>%20System)%20%0AAwaiting%20your%20response%20to%20proceed%20further%20with%20the%20update.%0A%0APlease%20feel%20free%20to%20contact%20IT%20Service%20Desk%20for%20further%20assistance";
        }
);
    }
        ...
        if ( WatchlistElement.length ) {
        $( WatchlistElement ).append(
            '<div class="ProTip-button" style="display:inline-block; border-radius: 3px; margin: 0px 0px 0px 10px; border: 1px solid black; background-color: #7CFC00; padding: 0 4px; cursor: pointer;">ProTip</div>'
        );
        $( '.ProTip-button' ).click( function() {
    if ( shortDescription.val().match( /griffin password/i) && CallerValueSRQ.MobilePhone.length < 2 && CallerValueSRQ.CallerUsername.match( /k/i )
    || originalDescription.val().match( /griffin password/i ) && CallerValueSRQ.MobilePhone.length < 2 && CallerValueSRQ.CallerUsername.match( /k/i )

            ) {
     var fullName = requestedFor.split(' ');
    firstName = fullName[0];
            window.location.href = "mailto: "+ CallerValueSRQ.CallerEmail +" ?subject="+ incidentNr +" Puhelinnumeron rekisteröiminen Järjestelmään&body=Hei%20"+ firstName +",%0A%0ARekisteröi puhelinnumerosi Workdayhin seuraavan linkin ohjeiden mukaisesti:%0Ahttps://intranet.upm.com/NewsArchive/News/Pages/Update-your-photo-and-phone-number-in-OurWorkday!.aspx%0A%0ATämän jälkeen voit itse vaihtaa Griffin-salasanasi millä tahansa laitteella osoitteessa:%0Ahttps://passwordreset.upm.com";
                 }
    else if ( shortDescription.val().match( /griffin password/i) && CallerValueSRQ.MobilePhone.length > 2 && CallerValueSRQ.CallerUsername.match( /k/i )
    || originalDescription.val().match( /griffin password/i ) && CallerValueSRQ.MobilePhone.length > 2 && CallerValueSRQ.CallerUsername.match( /k/i )
            ) {           var fullName = requestedFor.split(' ');
    firstName = fullName[0];
            window.location.href = "mailto: "+ CallerValueSRQ.CallerEmail +" ?subject="+ incidentNr +" Tiesitkö että voit itse vaihtaa Griffin-salasanasi itse?&body=Hei%20"+ firstName +",%0A%0ATiesitkö että voit itse vaihtaa Griffin-salasanasi millä tahansa laitteella osoitteessa:%0Ahttps://passwordreset.upm.com"; //UPM Mailbox creation starts
            }
    else if ( shortDescription.val().match( /griffin password/i) && CallerValueSRQ.CallerUsername.match( /v/i )
    || originalDescription.val().match( /griffin password/i ) && CallerValueSRQ.CallerUsername.match( /v/i )
            ) {            var fullName = requestedFor.split(' ');
    firstName = fullName[0];
            window.location.href = "mailto: "+ CallerValueSRQ.CallerEmail +" ?subject="+ incidentNr +" Tiesitkö että voit itse vaihtaa Griffin-salasanasi itse?&body=Hei%20"+ firstName +",%0A%0ATiesittekö, että voitte itse vaihtaa Griffin-salasananne osoitteessa:%0Ahttps://passwordreset.upm.com";

                }     else if ( shortDescription.val().match( /SAP/i)
    || originalDescription.val().match( /SAP/i )
            ) {
            window.location.href = "mailto: "+ CallerValueSRQ.CallerEmail +" ?subject="+ incidentNr +" Tiesitkö että voit itse vaihtaa SAP-salasanasi?&body=Hei%20"+ firstName +",%0A%0AKaikki SAP-järjestelmät ovat nykyään MyAccessin puolella ja voitte itse vaihtaa salasananne.%0AOsoitteessa https://myaccess.upm.com%0AO%0AOValitkaa vasemmalta ylhäältä Manage Access >> Change Passwords ja sitten voitte valita mihin järjestelmään salasana tulisi vaihtaa.";
                } else {
               // alert("No other match found"); //+ originalDescription.val()
            }
...
// Automated close notes
        ...
        $( '.ResolveFin-button' ).click( function() {
           ...
           closeNotes.valWithDelay( defaultCloseNotesFI );
      //     closeCode.valWithDelay( 'Other' );
        ...
        $( '.ResolveEng-button' ).click( function() {
            setCallerNames();
                                   state.valWithDelay( '6' );
           closeNotes.valWithDelay( defaultCloseNotesEN );
           subStatus.valWithDelay( 'Permanently Resolved' );
       //    closeCode.valWithDelay( 'Other' );
    ...
    /* NOT IN USE IN UPM
        ...
        $( '.ResolveSwe-button' ).click( function() {
           ...
           closeCode.valWithDelay( 'Other' );
                   ...
                   if ( CC_Knowledge.length ) {
        $( CC_Knowledge ).prepend(
            '<div class="Resolve3Fin-button"  style="display:inline-block; border-radius: 3px; margin: 0 1px; border: 1px solid black; padding: 0 2px; cursor: pointer;">3sResFin</div>'
        );
        $( '.Resolve3Fin-button' ).click( function() {
           ...
           closeCode.valWithDelay( 'Other' );
                   ...
                   if ( CC_Knowledge.length ) {
        $( CC_Knowledge ).prepend(
            '<div class="Resolve3Eng-button"  style="display:inline-block; border-radius: 3px; margin: 1 1px; border: 1px solid black; padding: 0 2px; cursor: pointer;">3sResEng</div>'
        );
        $( '.Resolve3Eng-button' ).click( function() {
           ...
           closeCode.valWithDelay( defaultCloseCode );
                           ...
                           if ( CC_Knowledge.length ) {
        $( CC_Knowledge ).prepend(
            '<div class="Resolve3Swe-button"  style="display:inline-block; border-radius: 3px; margin: 0px 0px 0px -100px; border: 1px solid black; padding: 0 2px; cursor: pointer;">3sResSwe</div>'
        );
