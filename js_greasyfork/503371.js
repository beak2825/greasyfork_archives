// ==UserScript==
// @name   🌱【免费版】双卫网|收费版本见文档：https://doc.zhanyc.cn/pages/sww/
// @namespace    http://jb.zhanyc.cn/
// @icon    https://js.zhanyc.cn/img/js-logo.svg
// @version      1.1
// @description  当前是免费版本，只包含了简单基本功能。广告:付费版本可解锁全自动无人值守，欢迎选购，请我喝一杯咖啡，保你无忧学习|接各类脚本开发工作，微信：zhanyc_cn 备用微信:zhanfengkuo 个人网站：http://doc.zhanyc.cn
// @author       zfk
// @include    *://www.sww.com.cn/*
// @grant       GM_getValue
// @grant       GM_setValue
// @grant       GM_addStyle
...
// @antifeature 
// @license GPL
// @downloadURL https://update.greasyfork.org/scripts/503371/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E5%8F%8C%E5%8D%AB%E7%BD%91%7C%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%E8%A7%81%E6%96%87%E6%A1%A3%EF%BC%9Ahttps%3Adoczhanyccnpagessww.user.js
// @updateURL https://update.greasyfork.org/scripts/503371/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E5%8F%8C%E5%8D%AB%E7%BD%91%7C%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%E8%A7%81%E6%96%87%E6%A1%A3%EF%BC%9Ahttps%3Adoczhanyccnpagessww.meta.js
// ==/UserScript==
(function () {
    ...
    let docUrl = "https://doc.zhanyc.cn/pages/sww/";
    let plugMain = Object.assign(baseConfig, {
        ...
        async runByUrl(url) {
            ...
            if (url.includes("/html/studyVideo?")) {
                plugMain.page_video();
            } else if (url.includes("/html/projectLog")) {
                plugMain.showPaidContent(`当前是免费版本,不含自动换课功能，收费版本可实现全自动看课换课、秒过、过人脸识别、自动答题！`)
            } else if (url.includes("/html/projectDetail?")) {
                plugMain.showPaidContent(`当前是免费版本,不含自动换课功能，收费版本可实现全自动看课换课、秒过、过人脸识别、自动答题！`)
            } else if (url.includes("/html/studyExam?")) {
                plugMain.showPaidContent(`当前是免费版本,不含考试功能，收费版本可实现全自动看课换课、秒过、过人脸识别、自动答题！`)
            } else if (url.includes("/html/myBase")) {

            } else if (url.includes("/html/survey")) {
                plugMain.showPaidContent(`当前是免费版本，收费版本可实现全自动看课换课、秒过、过人脸识别、自动答题！`)
            }
        },

        async page_video() {
            await plugMain.gmAuthDownScore("c4c69cc154504092a60339aee5559d5f");
            ...
            plugMain.pageData.video.index = setInterval(async () => {
                try {
                    if (plugMain.getElByText($(".dati-wenti-con"), "您是否在线", "startsWith") != null) {
                        $(".icr-text").click()
                    ...
                    if (plugMain.getElByText($(".loginSucc-msg"), "学习时长检测异常，未能记录学习状态！") != null) {

                        clearInterval(plugMain.pageData.video.index)
                        plugMain.confirmMsg("秒过失败，请查看使用文档，按文档使用秒过功能！", {
                            btn: ["查看文档", "关闭"], fun1: function (index) {
                                window.open(docUrl)
                                return false
                            }
                        })
        ...
        nextVideo() {
            ...
            setTimeout(() => {
                plugMain.confirmRun("准备前往列表页面").then((a) => {
                    ...
                    let url = plugMain.getGMData("detailUrl", "/html/projectLog");
