...
// @downloadURL https://update.greasyfork.org/scripts/27831/Hybrid%20-%20Marchy%20callsy.user.js
// @updateURL https://update.greasyfork.org/scripts/27831/Hybrid%20-%20Marchy%20callsy.meta.js
// ==/UserScript==

if ($('li:contains("Call")').length) {
    var question1 = [], question2 = [], question3 = [], question5 = [], question4;
    ...
    createButton('No',0,0,2,2,'');
    createButton('Wrong Biz',0,1,3,3,'Wrong business');
    createButton('DC',2,2,3,3,'Disconnected');
    createButton('Fax',1,3,3,3,'Fax');
    createButton('Ringing',0,2,3,3,'Kept ringing','Kept ringing');
    createButton('Silence',0,2,3,3,'Rang until silence');
    createButton('Hung Up',0,2,3,3,'Hung up on me');
    createButton('Busy',0,2,3,3,'Busy signal');
    createButton('Generic VM',0,2,3,3,'Voicemail with no info.');
    createButton('Personal VM',0,2,3,3,'Personal voicemail');
    createButton('Wrong Contact',0,0,1,2,'');
...
function createButton(inner, a, b, c, text, d) {
    var btn = document.createElement("BUTTON");
    btn.innerHTML = inner;
    btn.type = "button";
    btn.onclick = function() { answer(a,b,c,text,d); };
    number.append (btn);
}

function answer(one, two, three, five, six) {
