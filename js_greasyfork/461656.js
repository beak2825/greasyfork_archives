// ==UserScript==
// @name         Site  search on Google
// @version      1.0.1.3
// @license      MIT
// @description  Adds a button to search results under specific websites  with Google
// @author       IcyBearLovesSleep
...
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/461656/Site%20%20search%20on%20Google.user.js
// @updateURL https://update.greasyfork.org/scripts/461656/Site%20%20search%20on%20Google.meta.js
// ==/UserScript==

// put in desire site searches
const siteData = [
    {
        site:"reddit.com",
        name:"Reddit"
...
(function () {
    const elements = document.getElementsByClassName("MUFPAc");

    if (elements.length > 0) {
        for (let i = 0; i < siteData.length; i++) {
            let Url = '+site%3A'+siteData[i].site;

            // Creating the element
            let el = document.createElement('div');
            el.className = 'hdtb-mitem';
            const link = document.createElement('a');
            if(siteData[i].name=="Reddit"){
                // Adding the svg icon
                if (useIcon) {
            ...
            link.appendChild(document.createTextNode(siteData[i].name));
    ...
    const elements2 = document.getElementsByClassName("MUFPAc");
    ...
    if (elements2.length > 0) {
        for (let i = 0; i < siteData.length; i++) {
            ...
            window.addEventListener("load", function () {
                ...
                button.innerText = siteData[i].name
