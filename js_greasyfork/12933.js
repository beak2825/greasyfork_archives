// ==UserScript==
// @name        Enhanced dmhy
// @description 为动漫花园增加多项实用功能：添加支持迅雷离线的磁链，自动重命名种子文件，批量导出磁链，新番列表增强（完整列表、封面、字幕组、最近更新）
...
// @version     20171100
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/12933/Enhanced%20dmhy.user.js
// @updateURL https://update.greasyfork.org/scripts/12933/Enhanced%20dmhy.meta.js
// ==/UserScript==
...
(function($){
    ...
    function Row(i){
        ...
        if(!this.tr.find('.title').text().trim())
            return;
        ...
        var btih = this.link_magnet.attr('href').substr(20, 32);
        btih = btih_base32_to_hex(btih);

        this.link_thunder.attr('href', 'magnet:?xt=urn:btih:'+btih);
        ...
        this.link_torrent.closest('td').mouseenter(function(){
            ...
            if(href){
                ...
                // 否则先加载数据
                this.loadData(0);
                this.link_torrent.animate({opacity:.8}).fadeToggle().fadeToggle().fadeToggle().fadeToggle().fadeToggle().fadeToggle(); // 野鸡闪烁动画
            }
        }.bind(this));
    }
    ...
    // 增强番组列表
    (function better_list($, list){
        var jmd = $('.jmd')
        jmd.html('<style>.cover,.cover>.text,.cover>img{width:150px}.cover>.text{position:absolute;opacity:.8}.cover a{border: 1px solid #ffa500;background-color:white;margin:1px;padding:1px;line-height:14px;display:inline-block}.cover .title{white-space:nowrap;overflow:hidden}a.new{color:#D00!important}</style>');
        var base = $('.jmd_base tr');
        base.each(function(day){
            $(this).find('a').each(function(i){
                var a = $(this);
                var d = list[day][i];
                var links = $('<div>'+d[3]+'</div>')
                var q = d[2];
                ...
                $('td.title').each(function(){
                    var title = $(this);
                    text = tranStr(title.text());
                    if(re.exec(text)){
                        ++torrents;
                        var tag = title.find('.tag>a');
                        if(tag.length){
                            var team = tag.attr('href').match(/\d+/);
                            links.find('a[href$=3A'+team+']').addClass('new');
                        }
                    }
                });
                if(torrents)
                    a.addClass('new');
                a.attr('data-cover', d[0]);
                a.attr('data-links', links.html());
            });
        });
        var today = (new Date()).getDay();
        for(var i=-2;i<5;++i){
            jmd.append( base.eq( (today+i+7)%7 ).clone().addClass( i==0?'today':'' ) );
        }
        jmd.find('tr:odd').addClass('odd');
        jmd.find('tr:even').addClass('even');
        var timer;
        jmd.find('a').hover(function(){
            var a = $(this);
            var cover = $('<div class="cover processing" style="position:absolute;display:none"><div class="text"><div class="title"><a href="'+a.attr('href')+'" class="'+a.attr('class')+'">'+a.text()+'</a></div><div>'+a.attr('data-links')+'</div></div><img src="'+a.attr('data-cover')+'"></div>');
        ...
        setInterval(function(){
            $('.cover:not(:hover):not(.processing)').slideUp(function(){$(this).remove()});
        }, 200)
    })($, [sunarray,monarray,tuearray,wedarray,thuarray,friarray,satarray]);

    // 单字简繁转换，来自 https://github.com/hustlzp/jquery-s2t/blob/master/jquery.s2t.js，基于 MIT 协议发布
    ...
    function btih_base32_to_hex(base32) {
        var chunk = base32.match(/(.{4})/g);
        chunk = chunk.map(function (base32) {
            var dict = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567";
            var x = 0;
            base32.split('').map(function (c) {
                x = x * 32 + dict.indexOf(c);
            });
            x = x.toString(16);
            x = '00000' + x;
            x = x.substr(x.length-5);
            return x;
        });
        return chunk.join('');
    }
