...
// @description  Checks all your threads for offline images
// @version      0.2
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/11722/HF%20Your%20Offline%20Images.user.js
// @updateURL https://update.greasyfork.org/scripts/11722/HF%20Your%20Offline%20Images.meta.js
// ==/UserScript==
...
var 
	...
	oic_url_list = [],
	oic_page_index,
	oic_uurl = 'search.php?action=finduserthreads&uid='+$('#panel strong a').attr('href').split("=")[2]+'&order=desc';
...
function oic_collect_thread_images ( ) {
	...
	oic_log("");
	oic_log("<strong>Collect images on thread "+oic_url_list[oic_thread_index].split("=")[1]+"</strong>");
	$.get( oic_url_list[oic_thread_index], function( data ) {
		get_post = data;
		var temp_post_id = oic_url_list[oic_thread_index].split("=")[1];
		$(get_post).find('#posts>table:first').find('.post_body').find('img').each(function() {
...
function oic_collect_threads ( link ) {
	...
	$.get( use_page, function( data ) {
		...
		$(cur_page).find(".quick_keys>table:eq(1)>tbody>tr+tr+tr").each(function() {
			oic_url_list.push($(this).find('td:eq(1)>div>span>a').attr('href'));
		});
