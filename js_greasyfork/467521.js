// ==UserScript==
// @name         PadThai
// @namespace    zero.pdathai.torn
// @version      0.3
// @description  checks hosp time, added revive check
// @author       -zero [2669774]
// @match        https://www.torn.com/factions.php?step=profile*
// @icon         https://www.google.com/s2/favicons?sz=64&domain=torn.com
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/467521/PadThai.user.js
// @updateURL https://update.greasyfork.org/scripts/467521/PadThai.meta.js
// ==/UserScript==
...
async function checkRevives(){
    var fac = $('.enemy-faction');
    $('.enemy', fac).each(async function(){
        var id = $('div[id$="-user"]', $(this)).attr('id').split('_')[0];
        var link = `https://api.torn.com/user/${id}?selections=&key=${api}&comment=PadRe`;
        var response = await $.getJSON(link);
        console.log('CHecking '+ id + " "+response.revivable);
        if (response.revivable == 1){
...
async function insert(){
    if ($('.enemy', $('.enemy-faction')).length > 0){
        ...
        const button = `<button id="checkRevives" class="torn-btn">Check Revives</button>`;
        if ($('#checkRevives').length == 0){
            $('.faction-names').append(button);
            $('#checkRevives').on('click', function(){
                checkRevives();
            });
        }

        //  console.log(memberD);c
