...
// @namespace    unrival
// @version      3.1 
// @description 支持超星视频、文档、答题、自定义正确率、掉线自动登录
// @author       Modify by Caramel
// @run-at       document-end
...
// @original-script https://blog.086520.xyz
...
//安全网址请填写在上方空白区域
// @downloadURL https://update.greasyfork.org/scripts/483975/%E3%80%90%E8%B6%85%E6%98%9F%E5%AD%A6%E4%B9%A0%E9%80%9A%E5%B0%8F%E5%8A%A9%E6%89%8B%E3%80%91.user.js
// @updateURL https://update.greasyfork.org/scripts/483975/%E3%80%90%E8%B6%85%E6%98%9F%E5%AD%A6%E4%B9%A0%E9%80%9A%E5%B0%8F%E5%8A%A9%E6%89%8B%E3%80%91.meta.js
// ==/UserScript==
(() => {
  //  var token = 'pcTfNDfECWvXsXbN', //关注微信公众号：一之哥哥，发送 “token” 领取你的token，填写在两个单引号中间并保存，可以提高答题并发数量。
                    ...
                    <div class="panel-body">
                        <div>
                            ...
                            <div  style=" margin-top: 10px;">
                                ...
                                <div class="panel-body">
                                    <img src="https://image.086520.xyz/file/914d107dfae0b404e4cdf.png" width="100%" height="100%" object-fit="conver";/>
                                    <center><p>Modify By Caramel<p/><center/>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            doVideo = (item) => {
                ...
                if (allowBackground && backGround) {
                    if (_w.top.document.getElementsByClassName('catalog_points_sa').length > 0 || _w.top.document
                        ...
                        logs.addLog('您已安装ONE-PIECE，但此课程可能为闯关模式，不支持后台挂机，将为您在线完成', 'blue');
                    } else {
                        item['userid'] = UID;
                        ...
                        logs.addLog(
                            '您已安装ONE-PIECE，已添加至后台任务，<a href="unrivalxxtbackground/" target="view_window">点我查看后台</a>',
                            'green');
                        missionList['m' + item['jobid']]['running'] = true;
                        setTimeout(function () {
                            missionList['m' + item['jobid']]['running'] = false;
                ...
                logs.addLog('如遇脚本使用异常情况，请检查脚本版本是否为最新版。', 'orange');
                if (disableMonitor) {
    ...
    `;
        ...
        setInterval(function () {
            if (Math.round(new Date() / 1000) - parseInt(GM_getValue('unrivalBackgroundVideoEnable', '6')) >
                15) {
                logs.addLog('ONE-PIECE可能运行异常，如页面无反应，请尝试重启脚本猫或重启浏览器(脚本猫0.9.0版本有此问题)');
            }
        }, 10000);
        var loopShow = () => {
            let jobList = GM_getValue('unrivalBackgroundList', '1');
            if (jobList == '1') {
                _d.getElementById('joblist').innerHTML = '请将“ONE-PIECE”升级到最新版并重启浏览器';
            } else {
                try {
                    let jobHtml = '';
                    for (let i = 0, l = jobList.length; i < l; i++) {
