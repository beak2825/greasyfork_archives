...
(function() {
    ...
    const CONFIG = {
      ...
      USER_AGENT: 'TampermonkeyScript/2.0.2', // User-Agent для запросов
    ...
    /**
     * @description Получает полные данные сущности (Аниме или Манга) используя GraphQL и REST.
    ...
    const getEntityData = async (id, type) => {
        ...
        const query = isAnime ? GRAPHQL_QUERY_ANIME : GRAPHQL_QUERY_MANGA;
        const missingImg = 'https://shikimori.one/assets/globals/missing_preview.jpg';

        // Вспомогательная функция для GraphQL запроса
        ...
        // GraphQL для основных данных, REST для Новостей (topics) и Похожих (similar)
        const [mainResult, newsResult, similarResult] = await Promise.allSettled([
            graphqlRequest(),
            ...
            apiRequest(`/${type}s/${id}/similar`) // Используем REST, similar есть только там
        ]);

        // 2. Валидация ответа GraphQL
        if (mainResult.status === 'rejected' || !mainResult.value.data) {
            error(`❌ КРИТИЧЕСКАЯ ОШИБКА: не удалось получить данные через GraphQL.`, mainResult.reason || mainResult.value?.errors);
            throw new Error('Entity not found or GraphQL error');
        }

        const dataRoot = mainResult.value.data;
        const entityList = isAnime ? dataRoot.animes : dataRoot.mangas;

        if (!entityList || entityList.length === 0) {
            throw new Error('404: Entity not found in GraphQL response');
        }

        const entity = entityList[0];

        // 3. Загрузка комментариев (если есть topic_id)
        ...
        // 4. Обработка Ролей (Character + Staff)
        // Преобразуем структуру GraphQL Poster в структуру Image для рендерера
        const processRoles = () => {
            ...
            if (entity.characterRoles) {
                entity.characterRoles.forEach(role => {
                    ...
                    const originalUrl = char.image ? char.image.originalUrl : missingImg;
                    const x48Url = char.image ? char.image.miniAltUrl : missingImg;
                    ...
                    const mappedRole = {
                        ...
                        character: {
                            ...
                            image: {
                                preview: imgUrl,
                                x96: imgUrl, // Используем mainUrl как x96
                                x48: x48Url,
                    ...
                    if (role.rolesEn.includes('Main')) result.main.push(mappedRole);
            ...
            if (entity.personRoles) {
                entity.personRoles.forEach(role => {
                    ...
                    const imgUrl = person.image ? person.image.mainUrl : missingImg;
                    const originalUrl = person.image ? person.image.originalUrl : missingImg;
                    const x48Url = person.image ? person.image.miniAltUrl : missingImg;
        ...
        // 5. Обработка Связанных произведений (Related)
        // Используем relationText (локализованное) и relationKind
        const processRelated = () => {
            if (!entity.related) return [];

            return entity.related.map(rel => {
                const item = rel.anime || rel.manga;
                if (!item) return null; // Если связанный объект удален или скрыт

                // Определяем постер. Берем mainUrl для списков.
                const posterUrl = item.poster ? item.poster.mainUrl : 'https://shikimori.one/assets/globals/missing_mini.png';
                const posterX48 = item.poster ? item.poster.miniAltUrl : 'https://shikimori.one/assets/globals/missing_mini.png';

                // Формируем объект, совместимый с renderRelatedBlock
                return {
                    id: rel.id,
                    relationKind: rel.relationKind,
                    relation_russian: rel.relationText, // Рендерер ожидает это поле
                    ...
                    anime: rel.anime ? {
                        ...
                        aired_on: rel.anime.airedOn ? `${rel.anime.airedOn.year}-01-01` : null,
                        image: {
                            preview: posterUrl,
                            x96: posterUrl, // Для ретины 2x
                            x48: posterX48 // Добавили, если вдруг пригодится
                        }
                    } : null,
                    ...
                    manga: rel.manga ? {
                        ...
                        aired_on: rel.manga.airedOn ? `${rel.manga.airedOn.year}-01-01` : null,
                        image: {
                            preview: posterUrl,
                            x96: posterUrl,
                            x48: posterX48
        ...
        // 6. Сборка финального объекта
        const similarData = similarResult.status === 'fulfilled' ? similarResult.value : [];

        const finalData = {
            TYPE: type, // Важно для рендерера (anime/manga)
            INFO: {
                ID: entity.id,
                ...
                DURATION_BLOCK: isAnime
                    ? `<div class='line-container'><div class='line'><div class='key'>Длительность:</div><div class='value'>${entity.duration || '?'} мин.</div></div></div>`
                    : '',

                // Организации (Студии vs Издатели)
                ORG_LABEL: isAnime ? 'Студия' : 'Издатель',
                ORGANIZATIONS: isAnime
                    ? (entity.studios || [])
                    : (entity.publishers || [])
            },
            // Основной постер страницы - берем Original (лучшее качество)
            ...
            // Медиа (Только для аниме)
            VIDEOS: {
                SUBTITLES: isAnime ? (entity.fansubbers || []).map(n => ({name: n})) : [],
                DUBBING: isAnime ? (entity.fandubbers || []).map(n => ({name: n})) : [],
                LIST: entity.videos || []
            },
            ...
            NEWS: newsResult.status === 'fulfilled' ? newsResult.value.map(t => ({
                id: t.id,
                topic_title: t.topic_title,
                link: `https://shikimori.one/forum/news/${t.id}`
            })) : [],

            EXTERNAL_LINKS: entity.externalLinks ? entity.externalLinks.map(l => ({
                url: l.url,
                kind: l.kind, // Keep the raw kind for CSS class
                site: l.kind.replace(/_/g, ' ') // Human readable label
            })) : [],
    ...
    */
    const renderRelatedBlock = (relatedData, currentUser) => {
      ...
      const renderItem = (item) => {
          ...
          const image = entry.image?.preview ? getFullUrl(entry.image.preview) : 'https://shikimori.one/assets/globals/missing_mini.png';
    ...
    const renderTemplate = (html, data) => {
      ...
      html = html.replaceAll('{{DURATION_BLOCK}}', data.INFO.DURATION_BLOCK || `? мин.`);
      ...
      if (data.USER) {
          ...
          html = html.replaceAll('{{USER_URL}}', getFullUrl(data.USER.USER_URL));
          html = html.replaceAll('{{USER_AVATAR}}', getFullUrl(data.USER.USER_AVATAR));
          html = html.replaceAll('{{USER_AVATAR_X16}}', getFullUrl(data.USER.USER_AVATAR_X16));
          html = html.replaceAll('{{USER_AVATAR_X32}}', getFullUrl(data.USER.USER_AVATAR_X32));
          html = html.replaceAll('{{USER_AVATAR_X48}}', getFullUrl(data.USER.USER_AVATAR_X48));
          html = html.replaceAll('{{USER_AVATAR_X64}}', getFullUrl(data.USER.USER_AVATAR_X64));
          html = html.replaceAll('{{USER_AVATAR_X80}}', getFullUrl(data.USER.USER_AVATAR_X80));
          html = html.replaceAll('{{USER_AVATAR_X148}}', getFullUrl(data.USER.USER_AVATAR_X148));
          html = html.replaceAll('{{USER_AVATAR_X160}}', getFullUrl(data.USER.USER_AVATAR_X160));
      }
      ...
      */
      const renderMainCharacters = (mainCharacters) => {
        ...
        const charactersHtml = mainCharacters.map(role => {
          ...
          const imagePreview = char.image?.preview ? getFullUrl(char.image.preview) : 'https://shikimori.one/assets/globals/missing_preview.jpg';
          const imageX96 = char.image?.x96 ? getFullUrl(char.image.x96) : imagePreview;
        ...
        return `
          <div class="cc">
              ${sortedStaff.map(role => {
                  ...
                  const img2x = p.image?.x96 ? getFullUrl(p.image.x96) : img4x;

                  const img4x = p.image?.x48 ? getFullUrl(p.image.x48) : '/assets/globals/missing/mini@4x.png';
      ...
      const orgsHtml = orgs.map(org =>
          `<a href="https://shikimori.one/${data.TYPE}s/${data.TYPE === 'anime' ? 'studio' : 'publisher'}/${org.id}-${encodeURIComponent(org.name)}"
              title="${org.name}">
              ${org.image ? `<img src="${org.image}" class="studio-logo">` : `<span class="b-tag">${org.name}</span>`}
           </a>`
      ).join(' ');
    ...
    let renderEntityPage  = async (id, type) => {
        ...
        try {
            ...
            // --- Если менять на подход запроса, раскоментировать соответствующие строки и заменить на htmlText
            ...
            // Инициализируем кнопки после рендера
            setTimeout(() => {
    ...
    const setupQuoteButtons = () => {
        ...
        document.addEventListener('click', e => {
            ...
            if (selectedText) {
                ...
                log(`Цитируется выделенный текст: ${quoteText.substring(0, 100)}...`);
            } else {
                // Если нет выделения - берем весь текст комментария
                const commentBody = comment.querySelector('.body');
                if (!commentBody) return;

                const commentText = commentBody.textContent || commentBody.innerText;
                const maxLength = 50000;
                quoteText = commentText.length > maxLength
            ...
            const quoteTag = `[quote=${commentId.replace('comment-', '')};${userId};${nickname}]${cleanText}[/quote]\n\n`;
            ...
            log(`Цитата добавлена: комментарий ${commentId}, пользователь ${nickname}`);
        });
    ...
    const setupAddToListButtons = () => {
        ...
        document.body.addEventListener('click', async (e) => {
            ...
            const option = e.target.closest('.b-add_to_list .expanded-options .option');
            if (option) {
                e.preventDefault();
                ...
                const targetId = form.querySelector('input[name="user_rate[target_id]"]').value;
                const targetType = form.querySelector('input[name="user_rate[target_type]"]').value;
                const userId = form.querySelector('input[name="user_rate[user_id]"]').value; // Если нужно
        ...
        function updateUI(container, statusKey) {
            const map = STATUS_MAP[statusKey] || { label: statusKey, class: 'planned' };
            
            // 1. Меняем класс контейнера (цвет кнопки)
            // Удаляем старые классы статусов
            Object.values(STATUS_MAP).forEach(s => container.classList.remove(s.class));
            // Добавляем новый
            ...
            const input = container.querySelector('input[name="user_rate[status]"]');
    ...
    window.restorePage = async (id, type) => {
        ...
        // }
        // ^ todo Оно выдавало ошибку просто потому-что
