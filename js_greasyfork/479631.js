// ==UserScript==
// @name         【test】全网VIP视频自动解析播放器(已适配手机)
...
(function () {
    ...
    if (new RegExp("[?&]\\w{1,5}=https?://(?:\\w+\.)?(?:v\\.qq|iqiyi|youku|bilibili|miguvideo|le|(?:tv|film)\\.sohu|mgtv|ixigua|pptv|1905)\\.com|wasu\\.cn/.+").test(location.search)) {
        ...
        console.log(settings.isParse, 'settings.isParse')
            ...
            if (window === top) {
                ...
                */
                ...
                for (let i=0;i< playLine.length;i++) {
                    console.log('加载其余解析网址')
                    settings.NoAD解析[playLine[i].name] = playLine[i].url
                }
                ...
                ready(function () {
                 ...
                 console.log(settings.isParse, 'settings.isParse',settings.key.encrypt("宝塔镇河妖\x00给予\x000\x00" + location.href))
                    ...
                    doElement(cssString, function (playerBox) {
                        ...
                        iframe.onload = function () {
                            if (iframe.contentWindow.length === 0 && settings.address.length === 0) {
                                console.log('【尝试切换被阻止】',"解析已失效，正在切换解析源")
