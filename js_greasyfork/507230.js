...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/507230/MWIFavorites.user.js
// @updateURL https://update.greasyfork.org/scripts/507230/MWIFavorites.meta.js
// ==/UserScript==
...
(function() {
    ...
    // 创建中文到英文的名称映射
    const zhToEnNameMap = {};
    for (const key in fullItemNames.en.translation.itemNames) {
        const chineseName = fullItemNames.zh.translation.itemNames[key];
        const englishName = fullItemNames.en.translation.itemNames[key];
        if (chineseName) {
            zhToEnNameMap[chineseName.trim()] = englishName.trim();
        }
    }
                                                ...
                                                <div class="MuiTooltip-tooltip MuiTooltip-tooltipPlacementBottom css-1spb1s5" style="opacity: 1; transition: opacity cubic-bezier(0.4, 0, 0.2, 1);">
                                                    <div class="Item_actionMenu__2yUcG">
                                                        <div class="Item_itemInfo__3zAGf">
                                                            <span class="Item_name__2C42x" script_translatedfrom=" ${name} "> ${fullItemNames.zh.translation.itemNames["/items/"+id]} </span>
                                                        </div>
                                                        <div class="Item_itemInfo__3zAGf">
                                                            <span class="Item_name__2C42x" script_translatedfrom=" Amount: ${numberFormatter(itemCount[id])}/${numberFormatter(itemFavoritesMaps[name])}"> 数量: ${numberFormatter(itemCount[id])}/${numberFormatter(itemFavoritesMaps[name])}</span>
                                                        </div>
            ...
            for (const added of mutation.addedNodes) {
                if (added.classList.contains("MuiTooltip-popper")) {
                    ...
                    if(item_amountInputContainer && item_input){
                        ...
                        let displayedName = added.querySelector('span.Item_name__2C42x').textContent.trim();
                        let originalName = getOriTextFromElement(added.querySelector('span.Item_name__2C42x')).trim();

                        let nameToUse = displayedName;

                        if (zhToEnNameMap[displayedName]) {
                            nameToUse = zhToEnNameMap[displayedName];
                        ...
                        if(itemFavoritesMaps[nameToUse]){
                            favoritesInput.value=Number(itemFavoritesMaps[nameToUse]);
                        };
                        ...
                        btnAdd.onclick = () => {
                            ...
                            itemFavoritesMaps[nameToUse]=Number(favoritesInput.value);
                            favoritesInput.value=Number(itemFavoritesMaps[nameToUse]);
                            itemFavoritesMapsSave();
                            itemFavoritesShow();
                        };
                        ...
                        btnDel.onclick = () => {
                            delete itemFavoritesMaps[nameToUse];
                            favoritesInput.value=0;
                            itemFavoritesMapsSave();
                            itemFavoritesShow();
                        };
