...
// @downloadURL https://update.greasyfork.org/scripts/505928/pixels%20mine%202.user.js
// @updateURL https://update.greasyfork.org/scripts/505928/pixels%20mine%202.meta.js
// ==/UserScript==
...
(() => {
  ...
  */
        ...
        setInterval(() => {
            if(document.querySelector(`[class*="MuiDialog-paperWidthSm"]`)) {
                if(document.querySelector(`[class*="MuiDialog-paperWidthSm"]`).querySelector(`[class*="commons_pushbutton__"]`)) {
                    document.querySelector(`[class*="MuiDialog-paperWidthSm"]`).querySelector(`[class*="commons_pushbutton__"]`).click()
                }
        ...
        let interval = setInterval(() => {
            ...
            if (unsafeWindow.spacepack) {
                ...
                unsafeWindow.move = function(x,y,vx = 0, vy = 0){
                    Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[4].selfPlayer.move({x:vx,y:vy},{x:x,y:y})
                }
                ...
                let position_y = 0
                let orders = []
                let mine_map = 'pixelsNFTFarm-2793'
                let apiary_time = {}
                let ElementsName = []
                let cur_coins = 0
                let SaunaInterior_time = 0
                let bed_covers_time = 0
                let retain = {"Queen Bee":4}
                let sell_info = ["Popberry", "Grainbow", "Queen Bee", "Honey", "Wax","Gravelglass Matrix","Ochrux Matrix","Clay Matrix","Copperite Ore"]
                let bug_seed = ["Popberry Seeds", "Grainbow Seeds"]
                let win_drank = {"Grainshine": 116, "Muckchuck Mead": 326, "Popberry Wine": 77, "Grumpkin Wine": 216}
                let win_drank_item = ['itm_popberrywine', 'itm_grumpkinwine', 'itm_grainbow_grainshine', 'itm_muckchuck_mead']
                ...
                function senddata(action, data) {
                    let room_ws = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[4].stateManager.room
...
//                  RoomPrototype.connection.send(encryptUtils.encryptSend(action, data));
                ...
                async function sendMssage() {
                    console.log("房间开始加载")
                    await sleep(7000);
                    while (!unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[4]){
                        console.log("test_canvas")
                        await sleep(3000);
                    }
                    console.log("进入房间")
                    let room = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[4].stateManager.room;
                    while (room){
                        senddata('timerCheck', undefined)
                        await sleep(3000);
                        let state = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[4].stateManager.room.state;
                        let player = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[4].selfPlayer;
                        let entities = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[4].entities;
                        let game = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game;
                        while(!player.body){
                            await sleep(1000)
                            player = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[4].selfPlayer;
                        }
                        ...
                        if(state.id.includes("terravilla")){
                            ...
                            let win_price = Object.keys(win_drank)
                            let win_price_entity = {}
                            while (win_price.length>0){
                                const name = win_price.shift()
                                ...
                                console.log(parseInt(Listings_price_input),win_drank[name])
                                win_price_entity[name] = (parseInt(Listings_price_input)/win_drank[name]).toFixed(2)
                            ...
                            let min_price = 100
                            let buy_win = 'Muckchuck Mead'
                            for (const win in win_price_entity){
                                if (win_price_entity[win] < min_price){
                                    min_price = win_price_entity[win]
                                    buy_win = win
                                }
                            }
                            let amount = 30
                            const getCurrentItemCountAll = (item_info) => {
                                return slotArray.reduce((count, slot) => {
                                    return count + (item_info.includes(slot.item) ? slot.quantity : 0);
                                }, 0);
                            };
                            let win_amount = getCurrentItemCountAll(win_drank_item)
                            amount = amount- win_amount
                            updateMarketplaceSearchKeyword(buy_win)
                            const Index = Array.from(document.querySelectorAll('[class*="Marketplace_itemName__"]')).findIndex(el => el.textContent === buy_win);
                            if (!room.connection.transport.isOpen) {
                                break;
                            }
                            const item = Array.from(document.querySelectorAll('[class*="Marketplace_viewListings__"]'))
                            ...
                            const Listings_price_input=Listings_amount[1].querySelector('input').value
                            if (Math.floor(cur_coins/Listings_price_input)<amount){
                                amount = Math.floor(cur_coins/Listings_price_input)
                            }
                            if (amount>0){
                            ...
                            console.log(Mail_box.querySelector("span"))
                            ...
                            await sleep(2000)
                            if (!map_list.includes(mine_map)){
                                map_list.unshift(mine_map)
                            }
                            ...
                            orders = []
                            entities = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[4].entities;
                            ...
                            let gostore=false
                            const winItems = slotArray.filter(item => win_drank_item.includes(item.item));
                                ...
                                if(value.gameEntity&&value.gameEntity.id ==='ent_mine_01'){
                                    ...
                                    while (true){
                                        ...
                                        if(value.currentState.state==='waiting'){
                                            if (pickaxeItem){
                                                await move_distance(value.propCache.position.x,value.propCache.position.y,80,3)
                                                senddata("ui", { "id": pickaxeItem.item, "type": "entity", "slot": pickaxeItem.slot, "mid": key });
                                            }
                                            await sleep(1000)
                                        }else if(value.currentState.state==='ready') {
                                            await move_distance(value.propCache.position.x,value.propCache.position.y)
                                            senddata("clickEntity",  { "mid": key, "entity": value.gameEntity.id, "impact": "click", "inputs": [value.propCache.position.x,value.propCache.position.y] });
                                            await sleep(1000)
                                        }else if(value.currentState.state==='loaded' && value.currentState.displayInfo.utcTarget!=0){
                                            break
                                        }else {
                                            await sleep(1000)
                                        }
                        ...
                        await sleep(3000);
                        room = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[4].stateManager.room
                        if(room){
                            break;
                        }
                    }
                ...
                RoomPrototype.dispatchMessage = function (m, R) {
                    ...
                    switch (T) {
                        case 'joinRoom':
                            ...
                            break;
                        case 'sellOrders':
                            playSchemaSerializer = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[4].stateManager.playerSerializer
                        ...
                        case 'updatePlayer':
                            ...
                            playSchemaSerializer.state.coinInventory.$items.forEach((value, key) => {
