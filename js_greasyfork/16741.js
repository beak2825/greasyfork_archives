...
// @downloadURL https://update.greasyfork.org/scripts/16741/Place%20Harmonizer%20Beta.user.js
// @updateURL https://update.greasyfork.org/scripts/16741/Place%20Harmonizer%20Beta.meta.js
// ==/UserScript==
(function () {
	// item = W.selectionManager.selectedItems[0].model
	var WMEPHversion = '1.0.63'; // pull version from header
	...
	function runPH() {
		...
		'1.0.63: Dupefinder labels are removed on edit save.',
		'1.0.62: Dupefinder labels are removed if a dupe is deleted.',
		'1.0.62: Enabled functionality in the territories.',
		'1.0.62: Hours parser can handle more cases, including dual hours (M-F9-12, 13-17).',
		'1.0.61: Tweaks of Dupefinder, HN rangefinder, hours parser and more.',
		'1.0.60: Zoom KB shortcut: Ctrl-Alt-z will zoom to (if no place is selected) 200ft level centered where mouse is hovering.  If a place is selected, it centers it and zooms to 50ft level.',
		'1.0.60: Fixing of spurious PNH matches.'
		];  
		var WMEPHWhatsNewMetaList = [  // New in this major version
		'Live integration with Place Name Harmonization data sheets',
		'Over 750 chains harmonized',
		...
		'Easy to add missing hours with copy and paste for many places',
		...
		var dupeWL = [], urlWL = false, phoneWL = false, HNWL = false, AvPWL = false, hmcFlagName = false, aCodeWL = false, hotelLocWL = false, HNRange = false; // initialize WL variables  ### rework and delete?
		...
		*/
		...
		W.model.venues.events.register('objectschanged', W.map, deleteDupeLabel);
		W.accelerators.events.register('save', null, destroyDupeLabels);
		$('.WazeControlSave').click(function(){destroyDupeLabels();});
		...
		var severity=0;  // error tracking to determine banner color (messages)
		var severityMess=0;  // error tracking to determine banner color (messages)
		var severityButt=0;  // error tracking to determine banner color (action buttons)
		var bannMess, bannButt, bannButt2, bannServ, bannDupl;  // Banner Buttons objects
		...
		function normalizeURL(s, lc) {
			if (!s) {  // Notify that url is missing and provide web search to find website and gather data (provided for all editors)
				if ($("#WMEPH-EnableWhitelisting" + devVersStr).prop('checked')) {
					if (urlWL) {
						bannButt.urlMissingNB.severity = 0;
						formBannButt('urlMissingNB');
					} else {
		...
		function harmonizePlaceGo() {
			...
			dupeWL = []; urlWL = false; phoneWL = false; HNWL = false; AvPWL = false; hmcFlagName = false; aCodeWL = false; hotelLocWL = false; HNRange = false;  // ### Rework and delete?
			...
			bannMess = {    // banner message array in order of display
				...
				localizeHotel: { active: false, severity: 1, message: 'Please check the hotel website and add the localized name: (e.g. Holiday Inn - Tampa North).' },
				formatUSPS: { active: false, severity: 1, message: 'Adjust the primary name according to your regional standards for USPS locations (i.e., "USPS - Tampa").' },
				...
				catHotel: { active: false, severity: 0, message: 'Please check hotel details, as names can often be unique (e.g. Holiday Inn - Tampa North).' },
				...
				sfAliases: { active: false, severity: 0, message: 'Unnecessary aliases were removed.' },
			...
			bannButt = {  // set up banner action buttons.  Structure:
				...
				HNRange: { 
					active: false, 
					severity: 2, 
					bannText: 'House number seems out of range for the street name. Verify.',
					id: "HNRange",  
					value: wlButtText, 
					title: 'Whitelist HN range',
					cLog: "HN out of range Whitelisted",  
					action: function() {
						wlKeyName = 'HNRange';
						...
						bannButt.HNRangeNB.severity = 0;
						bannButt.HNRangeNB.active = true;
					}
				},
				HNRangeNB: { 
					active: false, 
				...
				noHours: { 
					active: false, 
					severity: 0, 
					bannText: 'No hours: <input type="text" value="Paste Hours Here" id="WMEPH-HoursPaste'+devVersStr+'" style="width:170px;padding-left:3px;color:#AAA">',
					id: "noHours",  
					value: "Try hours", 
					title: 'Try to parse hours and add',
					cLog: "Hours added",  
					action: function() {
						...
						if (hoursObjectArray !== false) {
							...
							bannButt.noHours.bannText = 'Hours: <input type="text" value="Paste Hours Here" id="WMEPH-HoursPaste'+devVersStr+'" style="width:170px;padding-left:3px;color:#AAA">';
						} else {
				...
				changeHMC2Office: {  // append optional Alias to the name
					active: false, 
					bannText: "This doesn't look like a hospital or urgent care location.", 
					severity: 3,  
					id: "changeHMC2Office",  
					value: "Change to Offices", 
					title: 'Change to Office Category',
					cLog: "Changed hospital to Office Category",  
					action: function() {
						/*
						wlKeyName = 'hmcFlagName';
						if (!venueWhitelist.hasOwnProperty(itemID)) {  // If venue is NOT on WL, then add it.
							venueWhitelist[itemID] = { };
						}
						venueWhitelist[itemID][wlKeyName] = {active: true};  // WL the flag for the venue
						saveWL_LS();  // Save the WL to local storage
						bannButt2.clearWL.active = true;  // Activate the clear WL button
						*/
						// Change the HMC cat to Offices
				...
				changeHMC2PetVet: {  // append optional Alias to the name
					active: false, 
					bannText: "This looks like it should be a Pet/Veterinarian category. Change?", 
					severity: 3,  
					id: "changeHMC2PetVet",  
					value: "Yes", 
					title: 'Change to Pet/Veterinarian Category',
					cLog: "Changed hospital to Pet/Veterinarian Category",  
					action: function() {
						/*
						wlKeyName = 'hmcFlagName';
						if (!venueWhitelist.hasOwnProperty(itemID)) {  // If venue is NOT on WL, then add it.
				...
				changeSchool2Offices: {  // append optional Alias to the name
					active: false, 
					bannText: "This doesn't look like it should be School category.", 
					severity: 3,
					id: "changeSchool2Offices",
					value: "Change to Office", 
					title: 'Change to Offices Category',
					cLog: "Changed School to Offices category",  
					action: function() {
						// Change the HMC cat to Offices
				...
				pointNotArea: {  // Area 2 Point button
					active: false, 
					bannText: "This category should be an point place.", 
					severity: 3,  
					id: "pointNotArea",  
					value: "Change to point", 
					title: 'Change to point place',
					cLog: "Changed to point place",  
					action: function() {
				...
				areaNotPoint: {  // Point 2 Area button
					active: false, 
					bannText: "This category should be an area place.", 
					severity: 3,  
					id: "areaNotPoint",  
					value: "Change to area", 
				...
				catHotel: { 
					active: false, 
					severity: 1, 
					bannText: 'Check hotel website for any name localization (e.g. Hilton - Tampa North) ',
					id: "catHotel",  
					value: wlButtText, 
					title: 'Whitelist hotel localization',
					cLog: "Hotel Localization Whitelisted",  
					action: function() {
						...
						venueWhitelist[itemID][wlKeyName] = {active: true};  // WL the flag for the venue
						saveWL_LS();  // Save the WL to local storage
						WMEPH_WLCounter();  // increase WL count
						bannButt2.clearWL.active = true;  // Activate clear WL button
						
						bannButt.catHotel.active = false;
					}
				},
				catHotelNB: { 
					active: false, 
				...
				badAreaCode: { 
					active: false, 
					severity: 1, 
					bannText: "Area Code mismatch ", 
					id: "badAreaCode",
					value: wlButtText, 
					title: 'Whitelist the area code',
					cLog: "Area code Whitelisted",  
					action: function() {
						wlKeyName = 'aCodeWL';
						if (!venueWhitelist.hasOwnProperty(itemID)) {  // If venue is NOT on WL, then add it.
							venueWhitelist[itemID] = { };
						...
						bannButt.badAreaCodeNB.severity = 0;
						bannButt.badAreaCodeNB.active = true;
					}
				},
				badAreaCodeNB: { 
					active: false, 
				...
				urlMissing: { 
					...
					action: function() {
						...
						bannButt.urlMissingNB.severity = 0;
						bannButt.urlMissingNB.active = true;
					}
				},
				urlMissingNB: { 
					active: false, 
				...
				gasMkPrim: {  // if the gas brand and name don't match
					active: false, 
					bannText: "Gas category is not first", 
					severity: 3,  
					id: "gasMkPrim",  
					value: "Fix", 
					title: 'Make the Gas Station category the primary category.',
					cLog: "Made Gas Station primary category",  
					action: function() {
						newCategories = insertAtIX(newCategories,"GAS_STATION",0);  // Insert ATM category in the second position
						W.model.actionManager.add(new UpdateObject(item, { categories: newCategories }));
						//phlogdev(bannButt.gasMkPrim.cLog);
						bannButt.gasMkPrim.active = false;  // reset the display flag
						harmonizePlaceGo();
					}
				},  
				hotelMkPrim: {  // if the gas brand and name don't match
					active: false, 
					bannText: "Hotel category is not first", 
					severity: 3,  
					id: "hotelMkPrim",  
					value: "Fix", 
					title: 'Make the Hotel category the primary category.',
					cLog: "Made Hotel primary category",  
					action: function() {
						newCategories = insertAtIX(newCategories,"HOTEL",0);  // Insert ATM category in the second position
						W.model.actionManager.add(new UpdateObject(item, { categories: newCategories }));
						//phlogdev(bannButt.hotelMkPrim.cLog);
				...
				bankBranch: {
					...
					action: function() {
						...
						bannMess.bankType1.active = false;  // remove bank type warning
					}
				}, 
				standaloneATM: {
					active: false,
					bannText: "Or is this a standalone ATM? ",
					severity: 2,  
					id: "standaloneATM",
					value: "Yes",
				...
				bankCorporate: {
					active: false,
					bannText: "Or is this the bank's corporate offices?",
					severity: 1,  
					id: "bankCorporate",
					value: "Yes",
				...
				PlaceWebsite: {
					...
					action: function() {
						var openPlaceWebsiteURL = 'http:\/\/' + newURL, linkProceed = true;
			...
			for (var ix = 0; ix < W.selectionManager.selectedItems.length; ix++) {
				...
				if (venueWhitelist.hasOwnProperty(itemID) && $("#WMEPH-EnableWhitelisting" + devVersStr).prop('checked')) {
					...
					for (var WLKey in venueWhitelist[itemID]) {  // loop thru the venue WL keys
						if (venueWhitelist[itemID].hasOwnProperty(WLKey) && venueWhitelist[itemID][WLKey].active) {
							bannButt2.clearWL.active = true;
							break;
						}
					}
					...
					if ( venueWhitelist[itemID].hasOwnProperty('dupeWL') ) { dupeWL = venueWhitelist[itemID].dupeWL; }
					...
					if ( venueWhitelist[itemID].hasOwnProperty('hmcFlagName') ) { hmcFlagName = venueWhitelist[itemID].hmcFlagName.active; }
					if ( venueWhitelist[itemID].hasOwnProperty('aCodeWL') ) { aCodeWL = venueWhitelist[itemID].aCodeWL.active; }
					if ( venueWhitelist[itemID].hasOwnProperty('hotelLocWL') ) { hotelLocWL = venueWhitelist[itemID].hotelLocWL.active; }
					if ( venueWhitelist[itemID].hasOwnProperty('HNRange') ) { HNRange = venueWhitelist[itemID].HNRange.active; }
				}
					...
					if (inferredAddress.state && inferredAddress.country) {
						...
						if ( $("#WMEPH-AddAddresses" + devVersStr).prop('checked') ) {  // update the item's address if option is enabled
							...
							if (item.attributes.houseNumber && item.attributes.houseNumber.replace(/[^0-9A-Za-z]/g,'').length > 0 ) {
								formBannMess('fullAddressInference');
								lockOK = false;
							}
						}
				...
				if (item.attributes.residential) {   
					...
					if (PNHMatchData[0] !== "NoMatch" && PNHMatchData[0] !== "ApprovalNeeded" ) { // *** Replace place data with PNH data
						...
						if (["HOTEL"].indexOf(priPNHPlaceCat) > -1) { 
							if (newName.toUpperCase() === PNHMatchData[ph_name_ix].toUpperCase()) {  // If no localization
								bannMess.localizeHotel.message = 'Please check the hotel website and add the localized name: (e.g. '+ PNHMatchData[ph_name_ix] +' - Tampa Airport).';
								formBannMess('localizeHotel');
						...
						} else {  // if not a special category then update the name
							newName = PNHMatchData[ph_name_ix];
						...
						if ( newCategories.indexOf("HOTEL") > -1  && newName.indexOf(' - ') === -1 && newName.indexOf(': ') === -1) {
							if ($("#WMEPH-EnableWhitelisting" + devVersStr).prop('checked')) {
								if (hotelLocWL) {
									// if WL, then do nothing (for now)
								} else {
									formBannButt('catHotel');
								}
							} else {
								formBannButt('catHotelNB');
							}
						}
					...
					for (var iii=0; iii<CH_NAMES.length; iii++) {
						if (newCategories.indexOf(CH_NAMES[iii]) > -1 ) {
							...
							for (var psix=0; psix<servHeaders.length; psix++) {
								if (CH_DATA_Temp[servHeaders[psix]] === '1') {  // These are automatically added to all countries/regions
					...
					// Activate all icons that are checked in the tab or added by the script
					for (var slix=0; slix<servList.length; slix++) {
						if (item.attributes.services.indexOf(servList[slix]) > -1 || newServices.indexOf(servList[slix]) > -1) {
						...
						if (newCategories.indexOf(CH_NAMES[iii]) === 0 ) {  // Primary category
							...
							if (item.isPoint()) {
								if (pvaPoint === '' || pvaPoint === '0') {
									formBannButt('areaNotPoint');
									lockOK = false;
								} else if (pvaPoint === '2') {
									formBannMess('areaNotPointOpt');
								} else if (pvaPoint === '3') {
									bannMess.areaNotPointOpt.severity = 2;
									formBannMess('areaNotPointOpt');
									lockOK = false;
								}
							} else if (item.is2D()) {
								if (pvaArea === '' || pvaArea === '0') {
									formBannButt('pointNotArea');
									lockOK = false;
								} else if (pvaArea === '2') {
					...
					if (newURL !== item.attributes.url && newURL !== "" && newURL !== "0") {
						// if option is checked and place was harmonized, keep long URL for PNH place if domains match
						if ($("#WMEPH-PreserveLongURLs" + devVersStr).prop('checked') && PNHNameRegMatch && item.attributes.url !== null) {  
							var tempNormURL = normalizeURL(item.attributes.url,true);  // Normalize existing url
							if (tempNormURL.match(/\/.+/i) !== null) {  // If there's something after the domain/ ...
								...
								if ( tempNormURL.toLowerCase().indexOf(tempNewURL.toLowerCase()) > -1 ) {  // domain match check
									newURL = normalizeURL(item.attributes.url,false);  // Keep existing, normalized URL
					...
					if (countryCode === "USA" || countryCode === "CAN") {
						if (newPhone !== null && newPhone.match(/[2-9]\d{2}/) !== null) {
							...
							if ( areaCodeList.indexOf(areaCode) === -1 ) {
								if ($("#WMEPH-EnableWhitelisting" + devVersStr).prop('checked')) {
									if (hotelLocWL) {
										bannMess.badAreaCodeNB.severity = 0;
										formBannButt('badAreaCodeNB');
									} else {
										formBannButt('badAreaCode');
									}
								} else {
									formBannButt('badAreaCodeNB');
								}
				...
				if (newCategories.indexOf("HOSPITAL_MEDICAL_CARE") > -1 && hospitalPartMatch.length > 0) {
					...
					if (containsAny(testNameWords,animalFullMatch)) {
						lockOK = false;
						formBannButt('changeHMC2PetVet');
						bannMess.pnhCatMess.active = false;
					} else if (containsAny(testNameWords,hospitalFullMatch)) {
						lockOK = false;
						formBannButt('changeHMC2Office');
						bannMess.pnhCatMess.active = false;
					} else {
				...
				if (newCategories.indexOf("SCHOOL") > -1 && schoolPartMatch.length>0) {
					if (containsAny(testNameWords,schoolFullMatch)) {
						lockOK = false;
						formBannButt('changeSchool2Offices');
						bannMess.pnhCatMess.active = false;
					} else {
				...
				!bannButt.changeHMC2Office.active && !bannButt.changeHMC2PetVet.active &&  !bannButt.changeSchool2Offices.active &&  !bannButt.areaNotPoint.active && !bannMess.bankType1.active) {
				...
				// update Severity for messages and banners
				for (var bannKey in bannMess) {
					if (bannMess.hasOwnProperty(bannKey) && bannMess[bannKey].active) {
						severityMess = Math.max(bannMess[bannKey].severity, severityMess);
					}
				}
				for (bannKey in bannButt) {
				...
				var walmartFlag = 0;
				if (Math.max(severity, Math.max(severityButt, severityMess)) < 3) {
				...
				var dupeBannMess = '', dupesFound = false, dupeWLMatch = false, makeButtDupl;
				bannDupl = {};
					...
					if (duplicateName.length > 0) {
						if (duplicateName.length+1 !== dupeIDList.length && devUser) {  // If there's an issue with the data return, allow an error report
							...
							for (var ijx=1; ijx<duplicateName.length+1; ijx++) {
								if ($("#WMEPH-EnableWhitelisting" + devVersStr).prop('checked')) {
									makeButtDupl = true;
									if (venueWhitelist.hasOwnProperty(itemID)) {  // If the venue is in the whitelist then don't make it
										if (venueWhitelist[itemID].hasOwnProperty('dupeWL')) {
											if (venueWhitelist[itemID].dupeWL.indexOf(dupeIDList[ijx]) > -1) {
												makeButtDupl = false;
											}
										}
									}
									if (makeButtDupl) {
										bannDupl[dupeIDList[ijx]] = {
											active: true, 
											severity: 2, 
											bannText: "&nbsp-- " + duplicateName[ijx-1], 
											id: dupeIDList[ijx],
											value: wlButtText, 
											title: 'Whitelist Duplicate',
											cLog: "Duplicate Whitelisted",  
											action: function(dID) {
												wlKeyName = 'dupeWL';
												if (!venueWhitelist.hasOwnProperty(itemID)) {  // If venue is NOT on WL, then add it.
												...
												venueWhitelist[itemID][wlKeyName].push(dID);  // WL the id for the duplicate venue
												saveWL_LS();  // Save the WL to local storage
												WMEPH_WLCounter();
												bannButt2.clearWL.active = true;
								...
								} else {  // if WL is not turned on, display simple banners
									dupeBannMess = dupeBannMess + '<br> --- ' + duplicateName[ijx-1];
								}
							}  // END loop for duplicate venues
						
							if ($("#WMEPH-EnableWhitelisting" + devVersStr).prop('checked')) {
								// ??  Nothing needed at this point
							} else {  // if no WL on, then active basic message
				...
				if (dupeHNRangeList.length > 3) {
					...
					for (dhnix = 0; dhnix < dupeHNRangeListSorted.length; dhnix++) {
						...
						//sumHNRatio += Math.abs( (parseInt(item.attributes.houseNumber) - dupeHNRangeListSorted[dhnix]) / dupeHNRangeDistList[dhnix] );
					}
					//sumHNRatio = sumHNRatio / dupeHNRangeListSorted.length;
					sortWithIndex(arrayHNRatio);
					if (arrayHNRatio[Math.round(arrayHNRatio.length/2)] > 1.0) {
						if ($("#WMEPH-EnableWhitelisting" + devVersStr).prop('checked')) {
							if (HNRange) {
								bannButt.HNRangeNB.severity = 0;
								formBannButt('HNRangeNB');
							} else {
								formBannButt('HNRange');
							}
						} else {
							formBannButt('HNRangeNB');
						}
		...
		function assembleBanner(item) {
			...
			//if ( $("#WMEPH-EnableWhitelisting" + devVersStr).prop('checked') && dupesFound ) {
			var dupesFound = false;
			if ( $("#WMEPH-EnableWhitelisting" + devVersStr).prop('checked') ) {
				strButt1 = 'Possible duplicates: ';
				for ( tempKey in bannDupl ) {
					if (bannDupl.hasOwnProperty(tempKey) && bannDupl[tempKey].active) {
						dupesFound = true;
						strButt1 = strButt1 + '<br>' + bannDupl[tempKey].bannText + '<input class="PHbutton" id="' + bannDupl[tempKey].id + '" title="' + bannDupl[tempKey].title + '" type="button" value="' + bannDupl[tempKey].value + '">';
			...
			for (NHix = 0; NHix < Object.keys(bannButt).length; NHix++ ) {
				...
				if (bannButt[tempKey].active) {
					if (bannButt[tempKey].hasOwnProperty('action')) {
						...
						strButt1 = bannButt[tempKey].bannText;
					}
			...
			for (NHix = 0; NHix < Object.keys(bannButt2).length; NHix++ ) {
				...
				if (bannButt2[tempKey].active) {
					strButt1 = bannButt2[tempKey].bannText + '<input class="PHbutton2" id="' + bannButt2[tempKey].id + '" title="' + bannButt2[tempKey].title + '" style="" type="button" value="' + bannButt2[tempKey].value + '">';
					sidebarMessageEXT.push(strButt1);
			...
			displayBanners(sidebarMessageEXT.join("<li>"), Math.max(severity, Math.max(severityButt, severityMess)) );
			
			
			if ( $("#WMEPH-EnableWhitelisting" + devVersStr).prop('checked') && dupesFound ) {
				setupDuplButtons(item);
			}
			setupButtons(item);
			if (!$("#WMEPH-HideServices" + devVersStr).prop('checked')) {
		...
		function setupDuplButtons(item) {
			var ixButt = 0;
			var btn = [];
			for (var tempKey in bannDupl) {
				if (bannDupl.hasOwnProperty(tempKey) && bannDupl[tempKey].active) {
					btn[ixButt] = document.getElementById(bannDupl[tempKey].id); 
					btn[ixButt].onclick = (function(buttonId, item){
						return function() {
							bannDupl[buttonId].action(buttonId);
							assembleBanner(item);
						};
					})(tempKey, item)
					ixButt++;
		...
		function setupButtons(item) {
			...
			for (var NHix = 0; NHix < Object.keys(bannButt).length; NHix++ ) {
				...
				if (bannButt[tempKey].hasOwnProperty('action') && bannButt[tempKey].active) {
		...
		function displayCloneButton() {
			...
			setTimeout(function() {
				...
				if ($('#clonePlace').length === 0 ) {
					var strButt1 = '<hr width=90%><input class="PHbutton3" id="clonePlace" title="Copy the Place info." type="button" value="Copy info">'+
						'<input class="PHbutton3" id="pasteClone" title="Apply the Place info. (Ctrl-Alt-O)" type="button" value="Paste info">';
					$("#WMEPH_runButton").append(strButt1);
				}
		...
		function clonePlace() {
			...
			if (cloneMaster !== null && cloneMaster.hasOwnProperty('url')) {
				...
				W.model.actionManager.add(new UpdateObject(item, { 
					houseNumber: cloneMaster.houseNumber,
					url: cloneMaster.url,
					phone: cloneMaster.phone,
					services: cloneMaster.services,
				}));
		...
		function parseHours(inputHours) {
			...
			for (tsix=0; tsix<newDaysVec.length; tsix++) {
				...
				if (hoursObjectAdd.fromHour === hoursObjectAdd.toHour && hoursObjectAdd.toHour !== '00:00') {
					phlogdev('Open & Close hours are the same');
					return false;
				}
				hoursObjectArrayMinDay.push( ((daysObjArray[0]+6)%7)+1 );
		...
		function findNearbyDuplicate(itemName, itemAliases, item, recenterOption) {
			...
			if ( (itemNameNoNum.length>2 && noNumSkip.indexOf(itemNameNoNum) === -1) || allowedTwoLetters.indexOf(itemNameNoNum) > -1 ) {  //  only add de-numbered name if anything remains
			...
			if (itemAliases.length > 0) {
				for (aliix=0; aliix<itemAliases.length; aliix++) {
					...
					if ( (aliasNameNoNum.length>2 && noNumSkip.indexOf(aliasNameNoNum) === -1) || allowedTwoLetters.indexOf(aliasNameNoNum) > -1 ) {  //  only add de-numbered name if anything remains
			...
			for (var venix in venueList) {  // for each place on the map:
				if (venueList.hasOwnProperty(venix)) {  // hOP filter
					...
					if (item.getAddress().street !== null && item.getAddress().street.name !== null && item.attributes.houseNumber && item.attributes.houseNumber !== '' &&
					venueList[venix].getAddress().street !== null && venueList[venix].getAddress().street.name !== null && testVenueAtt.houseNumber && testVenueAtt.houseNumber !== '' && 
					venix !== item.attributes.id && item.getAddress().street.name === venueList[venix].getAddress().street.name) {
					...
					if ( !wlDupeMatch && dupeIDList.length<6 && pt2ptDistance < 1000 && !testVenueAtt.residential && venix !== item.attributes.id && 'string' === typeof testVenueAtt.id && testVenueAtt.name.length>1 && testVenueAtt.name !== null) {  // don't do res, the point itself, new points or no name
						...
						if ( (testNameNoNum.length>2 && noNumSkip.indexOf(testNameNoNum) === -1) || allowedTwoLetters.indexOf(testNameNoNum) > -1  ) {  //  only add de-numbered name if at least 2 chars remain
						...
						if (!nameMatch && testVenueAtt.aliases.length > 0) {
							for (aliix=0; aliix<testVenueAtt.aliases.length; aliix++) {
								...
								aliasNameNoNum = aliasNameRF.replace(/[^A-Z]/g, '');  // Clear non-letter characters for alternate match ( HOLLYIVYPUB23 --> HOLLYIVYPUB ) 
								if ((aliasNameNoNum.length>2 && noNumSkip.indexOf(aliasNameNoNum) === -1) || allowedTwoLetters.indexOf(aliasNameNoNum) > -1) {  //  only add de-numbered name if at least 2 characters remain
		...
		function removeSFAliases(nName, nAliases) {
			...
			for (var naix=0; naix<nAliases.length; naix++) {
				if ( !nName.startsWith( nAliases[naix].toUpperCase().replace(/'/g,'').replace(/-/g,' ').replace(/\/ /g,' ').replace(/ \//g,' ').replace(/ {2,}/g,' ') ) ) {
					...
					bannMess.sfAliases.active = true;
				}
			}
		...
		function add_PlaceHarmonizationSettingsTab() {
			...
			createSettingsCheckbox("PlaceHarmonizer" + devVersStr, "WMEPH-HideServices" + devVersStr,"Hide Services banner icons");
			...
			var phKBContentHtml = $('<div id="PlaceHarmonizerKB' + devVersStr + 
				'"><div id="PlaceHarmonizerKBWarn' + devVersStr + '"></div><form>Shortcut Letter (a-Z): <input type="text" maxlength="1" id="WMEPH-KeyboardShortcut'+devVersStr+
				'" style="width: 30px;padding-left:8px"></form><div id="PlaceHarmonizerKBCurrent' + devVersStr + '"></div></div>');
		...
		function getItemPL() {
			...
			} else if ( $(".WazeControlPermalink").children(".fa fa-link").length > 0 ) {
				return $(".WazeControlPermalink").children(".fa fa-link")[0].href;
			}
			return  '';
		}
		...
		function WMEPH_WLCounter() {
			...
			if (localStorage.WMEPH_WLAddCount > 100) {
		...
		function harmoList(itemName,state2L,region3L,country,itemCats) {
			...
			for (var phnum=1; phnum<ixendPNH_NAMES; phnum++) { 
				...
				if (specCases.indexOf('strMatchAny') > -1 || currMatchData[ph_category1_ix] === 'Hotel') {  // Match any part of WME name with either the PNH name or any spaced names
					...
					if (PNHMatchData.split("|")[ph_searchnameword_ix] !== '') {
						spaceMatchList.push( currMatchData[ph_searchnameword_ix].toUpperCase().replace(/, /g,',').split(',') );
					}
			...
			if (bannMess.placeMatched.active) {
