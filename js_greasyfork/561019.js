...
(function() {
    ...
    function detectUserRank() {
        const cardContent = document.querySelector('.card-content.pt-15');
        if (cardContent) {
            const rankElement = cardContent.querySelector('.tbdrank');
            if (rankElement) {
                const classes = rankElement.className;
                if (classes.includes('wizard') || classes.includes('mvp') || classes.includes('star-uploader') ||
                    classes.includes('staff-associate') || classes.includes('moderator') || classes.includes('super-moderator')) {
                    maxGiftAmount = 5000;
                    userRank = 'premium';
                } else {
                    maxGiftAmount = 1000;
    ...
    function logMessage(message) {
        ...
        if (progressInfo) {
            ...
            progressInfo.style.justifyContent = 'flex-start';
    ...
    async function startGifting(targetCount) {
        if (isGifting) return;
            ...
            if (manualLinksMode) {
                ...
                while (isGifting) {
                    ...
                    for (const torrent of pageTorrents) {
                        if (!isGifting) break;
                        if (targetCount !== -1 && targetMet >= targetCount) break;
                        ...
                        if (shouldGiftSB) {
                            if (result.success) {
                                actionSuccessful = true;
                                targetMet++;
                                logMsg = `✓ ${giftingStats.successful + 1} - ${result.amount} SB (Total: ${giftingStats.totalSeedbonus})`;
                            }
                        } else if (shouldAddRep) {
                            if (result.repAdded) {
                                actionSuccessful = true;
    ...
    function makeDraggable(element) {
        const header = element.querySelector('#gifter-header');
        if (!header) return;

        header.addEventListener('mousedown', startDrag);

        function startDrag(e) {
            if (e.target.id === 'close-gifter') return;
    ...
    function createUI() {
        ...
        const giftOptions = maxGiftAmount === 5000
            ? '<option value="50">50 SB</option><option value="100">100 SB</option><option value="500">500 SB</option><option value="1000">1000 SB</option><option value="5000">5000 SB</option>'
            : '<option value="50">50 SB</option><option value="100">100 SB</option><option value="500">500 SB</option><option value="1000">1000 SB</option>';

        container.innerHTML = `<div id="gifter-header" style="display: flex;justify-content: space-between;align-items: center;padding: 5px 12px;border-bottom: 1px solid #404040;background: linear-gradient(135deg, #2a2a2a, #1a1a1a);border-radius: 8px 8px 0 0;cursor: grab;user-select: none;"><div style="display: flex; align-items: center; font-size: 13px; font-weight: 600; color: #8e44ad;"><img src="https://www.torrentbd.net/images/seedbonus-icon-1.png" alt="Gift Icon" style="width: 14px; height: 14px; vertical-align: middle; margin-right: 4px;">Gifter</div><button id="close-gifter" style="background: none;border: none;color: #999;font-size: 16px;cursor: pointer;padding: 2px 4px;border-radius: 3px;line-height: 1;transition: background 0.2s;" onmouseover="this.style.background='#444'" onmouseout="this.style.background='none'">×</button></div><div style="padding: 12px; max-height: calc(85vh - 60px); overflow-y: auto;"><div id="auto-section"><div style="display: flex; align-items: flex-start; gap: 8px;"><label style="font-size: 12px; color: #ccc; min-width: 80px; padding-top: 6px;">Torrent Count</label><input type="number" id="gift-target" min="1" value="10" placeholder="e.g., 10" style="flex: 1;height: 28px;background: #222;border: 1px solid #555;color: #e0e0e0;font-size: 12px;padding: 4px 8px;outline: none;border-radius: 4px;box-sizing: border-box;"></div></div><div id="gift-amount-container" style="margin-bottom: 8px; margin-top: -6px;"><div style="display: flex; align-items: flex-start; gap: 8px;"><label style="font-size: 12px; color: #ccc; min-width: 80px; padding-top: 6px;">Gift Amount</label><select id="gift-amount-select" style="flex: 1;background: #333;color: #e0e0e0;border: 1px solid #555;border-radius: 4px;padding: 4px 8px;font-size: 11px;cursor: pointer;outline: none;display: block;box-sizing: border-box;appearance: menulist;-webkit-appearance: menulist;-moz-appearance: menulist;height: 28px;line-height: 1;"><option value="50">50 SB</option><option value="100">100 SB</option><option value="200">200 SB</option><option value="500">500 SB</option><option value="1000">1000 SB</option><option value="5000">5000 SB${maxGiftAmount < 5000 ? '' : ''}</option></select></div></div><div style="display:grid;gap:6px;margin-bottom:8px;"><div class="tbdaio-row"><span>Add reputation</span><label class="tbdaio-switch"><input type="checkbox" id="add-rep"><span class="tbdaio-slider"></span></label></div><div class="tbdaio-row"><span>Thank uploader</span><label class="tbdaio-switch"><input type="checkbox" id="add-thanks"><span class="tbdaio-slider"></span></label></div><div class="tbdaio-row"><span>Gift seedbonus</span><label class="tbdaio-switch"><input type="checkbox" id="gift-sb" checked><span class="tbdaio-slider"></span></label></div></div><div class="tbdaio-row" style="margin-bottom: 8px;"><span style="font-size: 11px; font-weight: 500;">Manual Mode</span><label class="tbdaio-switch"><input type="checkbox" id="manual-mode-toggle"><span class="tbdaio-slider"></span></label></div><div id="manual-links-section" style="display: none; margin-bottom: 12px;"><div style="margin-bottom: 8px;"><label style="font-size: 11px; color: #ccc; margin-bottom: 4px; display: block;">Torrent Links</label><textarea id="manual-links-input" placeholder="One link per line" style="width: 100%;height: 80px;background: #222;border: 1px solid #444;color: #e0e0e0;border-radius: 4px;padding: 6px;font-size: 10px;resize: vertical;box-sizing: border-box;"></textarea></div><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;"><button id="process-links" style="background: #4a90e2;color: white;border: none;padding: 4px 8px;border-radius: 3px;font-size: 10px;cursor: pointer;">Process Links</button><span id="manual-links-count" style="font-size: 10px; color: #888;">0 torrent(s) found</span></div></div><div style="display: grid; gap: 4px; margin-bottom: 8px;"><button id="start-gifting" style="width: 100%; padding: 6px; background: linear-gradient(135deg, #27ae60, #2ecc71); color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 11px; font-weight: 500; transition: all 0.2s;" onmouseover="this.style.transform='translateY(-1px)'" onmouseout="this.style.transform='translateY(0)'">Start Gifting</button><button id="stop-gifting" style="width: 100%; padding: 6px; background: linear-gradient(135deg, #e74c3c, #c0392b); color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 11px; font-weight: 500; display: none;" onmouseover="this.style.transform='translateY(-1px)'" onmouseout="this.style.transform='translateY(0)'">Stop</button></div><div id="progress-info" style="font-size: 10px; color: #aaa; margin-bottom: 8px; padding: 4px; background: #222; border-radius: 3px; border-left: 3px solid #6c3483; min-height: 25px; max-height: 25px; line-height: 1.2; display: flex; align-items: center; justify-content: flex-start;">Ready to start...</div><div id="stats" style="padding: 6px; background: #1f1f1f; border-radius: 4px; border: 1px solid #3a3a3a;"><div style="display: grid; grid-template-columns: 1fr 1fr; gap: 4px; font-size: 10px;"><div>Processed: <span id="stat-processed" style="color: #4a90e2; font-weight: 500;">0</span></div><div>Success: <span id="stat-successful" style="color: #27ae60; font-weight: 500;">0</span></div><div>Skipped: <span id="stat-skipped" style="color: #f39c12; font-weight: 500;">0</span></div><div>Total SB: <span id="stat-total" style="color: #e67e22; font-weight: 500;">0</span></div></div></div></div>`;
        ...
        giftAmountSelect.addEventListener('change', function() {
            // Fun Easter egg for non-premium users trying to select 5000
            if (maxGiftAmount < 5000 && this.value === '5000') {
                const progressInfo = document.getElementById('progress-info');
                const previousValue = loadGiftAmount() || '1000';

                if (progressInfo) {
                    progressInfo.style.display = 'flex';
                    progressInfo.style.alignItems = 'center';
                    progressInfo.style.justifyContent = 'center';
                    progressInfo.innerHTML = '<img src="https://www.torrentbd.net/images/smilies/an_slap.gif" style="width: 40px; height: 40px; object-fit: contain;">';

                    setTimeout(() => {
                        progressInfo.style.display = 'flex';
                        progressInfo.style.alignItems = 'center';
                        progressInfo.style.justifyContent = 'flex-start';
                        progressInfo.textContent = 'Ready to start...';
                    }, 3000);
                }
                // Revert selection back to previous valid value
                setTimeout(() => {
