...
// @description	Wersja przyspieszona
...
// @downloadURL https://update.greasyfork.org/scripts/3600/eRepublik%20Battle%20Master.user.js
// @updateURL https://update.greasyfork.org/scripts/3600/eRepublik%20Battle%20Master.meta.js
// ==/UserScript==
...
function letsJQuery() {
  ...
  unsafeWindow.battleFX.hit = exportFunction(function () {
    ...
    if (multiHitRunning) {
      ...
      multiHitLoopId = setTimeout("jQuery.fn.multiHIT()", 701); //1001
  ...
  unsafeWindow.battleFX.blow = exportFunction(function () {
    if (multiHitRunning) {
      multiHitEnemyKill++;
  ...
  unsafeWindow.battleFX.countNextBattle = exportFunction(function (time) {
  ...
  unsafeWindow.jQuery.fn.getWell = exportFunction(function () {
    ...
    if ($j('#heal_btn small').html() != '0' && !h.hasClass('disabled') && unsafeWindow.SERVER_DATA.onlySpectator == 0) {
      ...
      var newUrl = "http://" + unsafeWindow.erepublik.settings.hostname + "/" + unsafeWindow.erepublik.settings.culture + '/main/eat?format=json&_token=' + $j('#' + $j('div.user_health input[type=hidden]').attr('id')).val();
      if ($j('#fightSessionKey').length !== 0) {
        newUrl += "&fightSessionKey=" + $j('#fightSessionKey').val();
      }
      newUrl += "&buttonColor=";
      ...
      /*$j.getJSON(-*foodUrl*-newUrl, {}, function (data) {
console.log("success, wewnatrz");
      ...
      .done(function(){
        console.log("2nd success, zewnatrz");
      })
      ...
      GM_xmlhttpRequest({
        ...
        onload: function(response) {
          ...
          var koniec = response.responseText.lastIndexOf("}");
          var JASON = $j.parseJSON(response.responseText.substr(start, koniec-start+1));
          $j('#DailyConsumtionTrigger').removeClass('load');
          JASON.health = parseFloat(JASON.health);
          var wellInc = JASON.health - unsafeWindow.SERVER_DATA.health;
          currentHealth = JASON.health;
          unsafeWindow.energy.processResponse(JASON);

          clearTimeout(multiHitLoopId);
          multiHitLoopId = setTimeout("jQuery.fn.multiHIT()", 250);
        },
        onerror: function(response) {
          console.log("Blad przy jedzeniu");
        }
      });
  ...
  unsafeWindow.jQuery.fn.changeWeapon = exportFunction(function () {
  ...
  unsafeWindow.jQuery.fn.multiHIT = exportFunction(function () {
...
console.log("ustawianie heros_update");
...
console.log("ustawianie ajaxSuccess");
