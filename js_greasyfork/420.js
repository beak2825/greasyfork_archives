...
(function(){
	this.Graph = {
		...
		likePost: function(postID,params){try{
			//https://graph.facebook.com/v1.0/POST_ID/likes?access_token=		
			var req; req=GM_xmlhttpRequest({
				method: "POST",
				url: "https://graph.facebook.com/v1.0/"+postID+"/likes?access_token="+Graph.authToken,
		...
		commentPost: function(postID,comment){try{
			...
			var req; req=GM_xmlhttpRequest({
				method: "POST",
				url: "https://graph.facebook.com/v1.0/"+postID+"/comments?access_token="+Graph.authToken+"&message="+comment,
				timeout: Graph.fetchTimeout*1000,
				onload: function(response) {try{
					if (response.responseText=="true") {
						//comment successful
		...
		requestAuthCodeB: function(callback){try{
			...
			var req; req=GM_xmlhttpRequest({
				method: "GET",
				url: "http://developers.facebook.com/tools/explorer?method=GET&path=me%3Ffields%3Did%2Cname&version=v1.0",
				timeout: Graph.fetchTimeout*1000,
				onload: function(response) {try{
					var test=response.responseText;
					var auth=test.longestQuoteWithin(matchFunc_OnlyAlphaNumeric);
					if (auth!="") {
		...
		fetchUser: function(params){try{
			...
			var URL="https://graph.facebook.com/v1.0/me?access_token="+Graph.authToken;
			var req; req=GM_xmlhttpRequest({
				method: "GET",
				url: URL,
				timeout: Graph.fetchTimeout*1000,
		...
		fetchPostsFQL_B: function(params){try{
			...
			*/
			...
			var URL_prefix="https://graph.facebook.com/v1.0/fql?access_token={0}&q=";
			...
			if (params.friends||null) {
				//URL=URL.replace("{3}",params.friends.join(",")); //use single friend passes, not multi friend queries
				URL=URL.replace("{3}",params.friends);
			} else {
				URL=URL.replace("{3}","SELECT uid2 FROM friend WHERE uid1=me() LIMIT 5000");
			}

			//as of FQL 2.1, WM can no longer request data by specific friend UID
			//so by default we now ALWAYS just use a list of all of our friends
			//URL=URL.replace("{3}","SELECT uid2 FROM friend WHERE uid1=me() LIMIT 5000");
			//reinstated above params.friends parameter
			...
			//unless no apps were passed or we specified no app filtering
				URL=URL.replace("{1}",""); //temp fix, no app filtering ever
			...
			URL=URL.replace("{4}","+LIMIT+"+(params.limit||5000));
			...
			var req; req=GM_xmlhttpRequest({
					...
					if (response){
						...
						if (exists(data.data)) {							
							...
							if (gotMoreToDo) {
								...
								Graph.fetchPostsFQL_B(newParams);
							}
							...
							else if (data.error.message||null) log("Graph.fetchPostsFQL: "+data.error.message,{level:3});
				...
				ontimeout: function(response) {try{
					...
					if (params.timeouts<3) Graph.fetchPostsFQL_B(params);
		...
		fetchPostsFQL: function(params){try{
				...
				if (edge||null){
					...
					if (limit) {
						if ((timeNow-(edge*1000)) > limit) {
							log("Graph.fetchPostsFQL("+params.feed.url+"): the user-set older limit of this feed has been reached",{level:2});
			...
			var req; req=GM_xmlhttpRequest({
				...
				ontimeout: function(response) {try{
					...
					if (params.timeouts<3) Graph.fetchPostsFQL(params);
				}catch(e){log("Graph.fetchPostsFQL.ontimeout: "+e);}},
	...
	this.Graph2 = {
		...
		getGraphExplorerAuthToken : function(callback){try{
			...
			var req; req=GM_xmlhttpRequest({
				...
				onload: function(response) {try{
					var test=response.responseText;
					var searchString='<a href="https://graph.facebook.com/v1.0/me/home?access_token=';
					var auth = test.indexOf(searchString),authEnd;
					if (auth!=-1) {
						authEnd = test.indexOf('">',auth);
						var authCode = (test.substring(auth+(searchString.length), authEnd));
		...
		likePost: function(params){try{
			...
			*/
			var req; req=GM_xmlhttpRequest({
				method: "POST",
				url: "https://graph.facebook.com/v1.0/"+params.postID+"/likes?access_token="+Graph2.currentUser.authToken,
				timeout: Graph2.fetchTimeout*1000,
				onload: function(response) {try{
		...
		commentPost: function(params){try{
			...
			*/
			var req; req=GM_xmlhttpRequest({
				method: "POST",
				url: "https://graph.facebook.com/v1.0/"+params.postID+"/comments?access_token="+Graph2.currentUser.authToken+"&message="+params.comment,
				timeout: Graph2.fetchTimeout*1000,
				onload: function(response) {try{
					params.responseText = response.responseText
					if (response.responseText=="true") {
		...
		getUser : function(params){try{
			...
			*/
			...
			var URL="https://graph.facebook.com/v1.0/"+params.userID+"?access_token="+Graph2.currentUser.authToken;
			var req; req=GM_xmlhttpRequest({
				method: "GET",
				url: URL,
				timeout: Graph2.fetchTimeout*1000,
		...
		getFriends : function(params) {try{
			...
			var URL="https://graph.facebook.com/v1.0/me/friends?limit=5000&access_token="+Graph2.currentUser.authToken;
			var req; req=GM_xmlhttpRequest({
				method: "GET",
				url: URL,
				timeout: Graph2.fetchTimeout*1000,
		...
		getFriendLists : function(params) {try{
			...
			var URL="https://graph.facebook.com/v1.0/me/friendlists?limit=5000&access_token="+Graph2.currentUser.authToken;
			var req; req=GM_xmlhttpRequest({
				method: "GET",
				url: URL,
				timeout: Graph2.fetchTimeout*1000,
