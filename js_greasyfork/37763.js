// ==UserScript==
// @name        RefreshFinRequest
// @namespace   refreshyckfin
// @version      1.0.4
// @description  定时刷新待打款订单请求, 并生成alert窗口
// @author       Jack Li
// @include       http://finance.yinchengku.com/*
// @include       http://finance01.tst.yinchengku.com/*
// @include       http://finance01.uat.yinchengku.com/*
// @exclude       http://*/admin
// @exclude       http://*/admin/logout
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/37763/RefreshFinRequest.user.js
// @updateURL https://update.greasyfork.org/scripts/37763/RefreshFinRequest.meta.js
// ==/UserScript==
...
var OCRDynData = {
  ...
  notExist: function(key) {
    ...
    getQuoteStatus(domEle) {
        var statusStr = domEle.getElementsByTagName("th")[10].textContent.trim();
        if (statusStr === "打款成功") {
            return true;
        }
        return false;
  ...
  json: function(url, cb) {
      var that = this;
        this.fetchResponse(url, function(content) {
            ...
            var iter = document.evaluate( "//th/a", doc, null, XPathResult.UNORDERED_NODE_ITERATOR_TYPE, null );
            var item = iter.iterateNext();
            while (item) {
                var id = item.getAttribute('id') ;
                if (!!id && that.hasNoReply(that.getQuoteStatus(item.parentNode.parentNode))) {
    ...
    function notifyWithNewOCRData() {
        ...
        OCRDynData.fetch5PagesJson(function(totalNoReplyList, newNoReplyList) {
            ...
            if (!totalNoReplyList || totalNoReplyList.length === 0) {
                ...
                NotifyWindow.show("[新] 待打款", "您有(" + totalNoReplyList.length + ")条未打款(" + (newNoReplyList.length || 0) + "条新增), 点击查看");
                Sound.play();
            }
