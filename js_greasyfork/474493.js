...
// @description  快捷插件
// @author       You
// @match        https://ldq.ldmnq.com/mngt/sys/bwlist/app/load.do?page=sys/bwlist
// @match        https://ldq.ldmnq.com/mngt/cph/device-fault-list/load.do?page=cph/device-fault-list
// @icon         https://res.ldmnq.com/cz_ldq/assets/images/favicon-cph.ico
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/474493/ld%E6%8F%92%E4%BB%B6.user.js
// @updateURL https://update.greasyfork.org/scripts/474493/ld%E6%8F%92%E4%BB%B6.meta.js
// ==/UserScript==
...
(function() {
            ...
            for (var i = 0;i<data_objs.length;i++){
                ...
                if(id == data_objs[i].id){
                    ...
                    if(status == 1){
                        ...
                        formData.append('note', data_objs[i].note);
                    }
    ...
    var randomInterval = function(){
        var randomSeconds = Math.floor((Math.random() * (300 - 60 + 1) + 60))
        console.log(randomSeconds);
        return randomSeconds  * 1000;
    }

    //监控黑白名单
    var test_quantity = function(){
        get_page();
        var menu_texts = document.getElementsByClassName('menu-text');
        for(var i = 0;i<menu_texts.length;i++){
            if (menu_texts[i].innerText == "Active"){
                break;
            }
        }
        setInterval(function() {
            ...
            if (currentHour >= 8 && currentHour < 24) {
                console.log("工作时间,继续运行")
                get_page();
                var string_data;
                var xhr = new XMLHttpRequest();
                xhr.open("GET","/mngt/rest/sys/bwlist/app/page?packageName=&appName=&note=&bwFlag=-1&appType=&pool=&cardType=&status=&_search=false&size=9999&current=1&sidx=id&sord=desc",false);
                xhr.onreadystatechange = function() {
                    if (xhr.status === 200 && xhr.readyState === 4) {
                        string_data = JSON.parse(xhr.responseText);
                        console.log(string_data);
                    };

                };
                xhr.send();
                if (string_data.total != 0){
                    menu_texts[i].innerText = "Active      " + string_data.total
                }else{
                    menu_texts[i].innerText = "Active"
                }
            }else{
                console.log("非工作时间,停止运行");
            }
        },randomInterval());
