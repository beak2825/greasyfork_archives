...
// @downloadURL https://update.greasyfork.org/scripts/539057/Magapoke%20Ripper.user.js
// @updateURL https://update.greasyfork.org/scripts/539057/Magapoke%20Ripper.meta.js
// ==/UserScript==
...
(function () {
    ...
    // Simplified descrambling logic and tile dimension calculations
    ...
    const downloadChapter = async (progressBar) => {
        ...
        if (isLoggedIn) {
            alert(
                "UPDATE ABOUT THE PREVIOUS WARNINGS: After some more testing and obvservations, I'm now confident enough to say the ban risk I mentioned previously is unrelated to the use of this script, though there is still a chance you may be at risk depending on how you use the site - please see the updated description on the script's homepage for more details.\nI will remove this warning very shortly in a next update, sorry for taking a while!"
            );
        }
        ...
        try {
            await unsafeWindow.processZip(title, pageCount, unscrambledPageBlobs);
        } catch (error) {
            console.error(error);
