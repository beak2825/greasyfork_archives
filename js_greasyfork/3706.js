// ==UserScript==
// @name           Rutracker.org Torrents Re-Downloader
// @namespace      https://greasyfork.org/users/3348-xant1k
// @description    Scanning torrent release of change and notification when update. Also download again torrent if it updated
...
// // @grant       GM_setValue
// @version        1.0
// @downloadURL https://update.greasyfork.org/scripts/3706/Rutrackerorg%20Torrents%20Re-Downloader.user.js
// @updateURL https://update.greasyfork.org/scripts/3706/Rutrackerorg%20Torrents%20Re-Downloader.meta.js
// ==/UserScript==
...
var track = 'torrents.ru';
if (window.location.href.indexOf('rutracker.org', 1) > 0) {
	track = 'rutracker.org';
}
//Переменные
var pathes = [];
				...
				for (ii in pathes) {
					if (pathes[ii] == zchk[i].id.substr(3, zchk[i].id.length - 3)) {
						...
						if (torrdtostr(newinf[1]) != bb) {
							...
							asd.innerHTML = asd.innerHTML + '</td><td><a href="javascript:dnl(' + aaa + ')" >' + imgsave() + '</a>';
...
unsafeWindow.dnl = function(au) {
	...
	for (ii in pathes) {
		if (pathes[ii] == au) {
			if (confirm("Обновить дату торрента в памяти?")) {
				dates[ii] = newdates[ii];
				tmp2 = dates.join('*/*');
				window.setTimeout(UPDATE_, 0);
				TShow();
				TShow();
...
function torrdtostr(str) {
	...
	m1 = str.indexOf('Yanv', 1);
	if (m1 != (-1)) {
		m = '01';
	};
	m2 = str.indexOf('Phev', 1);
	if (m2 != (-1)) {
		m = '02';
	};
	m3 = str.indexOf('Mar', 1);
	if (m3 != (-1)) {
		m = '03';
	};
	m4 = str.indexOf('Apr', 1);
	if (m4 != (-1)) {
		m = '04';
	};
	m5 = str.indexOf('May', 1);
	if (m5 != (-1)) {
		m = '05';
	};
	m6 = str.indexOf('Iyun', 1);
	if (m6 != (-1)) {
		m = '06';
	};
	m7 = str.indexOf('Iyul', 1);
	if (m7 != (-1)) {
		m = '07';
	};
	m8 = str.indexOf('Avg', 1);
	if (m8 != (-1)) {
		m = '08';
	};
	m9 = str.indexOf('Sen', 1);
	if (m9 != (-1)) {
		m = '09';
	};
	m10 = str.indexOf('Okt', 1);
	if (m10 != (-1)) {
		m = '10';
	};
	m11 = str.indexOf('Noya', 1);
	if (m11 != (-1)) {
		m = '11';
	};
	m12 = str.indexOf('Dek', 1);
	if (m12 != (-1)) {
		m = '12';
	};
	dd = str.substr(1, 2);
...
function torrdtostr2(str) {
	var m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, dd, yy, time;
	m1 = str.indexOf('Jan', 1);
	if (m1 != (-1)) {
		m = '01';
	};
	m2 = str.indexOf('Feb', 1);
	if (m2 != (-1)) {
		m = '02';
	};
	m3 = str.indexOf('Mar', 1);
	...
	m6 = str.indexOf('Jun', 1);
	if (m6 != (-1)) {
		m = '06';
	};
	m7 = str.indexOf('Jul', 1);
	if (m7 != (-1)) {
		m = '07';
	};
	m8 = str.indexOf('Aug', 1);
	if (m8 != (-1)) {
		m = '08';
	};
	m9 = str.indexOf('Sep', 1);
	if (m9 != (-1)) {
		m = '09';
	};
	m10 = str.indexOf('Oct', 1);
	if (m10 != (-1)) {
		m = '10';
	};
	m11 = str.indexOf('Nov', 1);
	if (m11 != (-1)) {
		m = '11';
	};
	m12 = str.indexOf('Dec', 1);
	if (m12 != (-1)) {
		m = '12';
	};
	dd = str.substr(5, 2);
