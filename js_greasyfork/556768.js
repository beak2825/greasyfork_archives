// ==UserScript==
// @name         烟草网络学院通用智能学习助手 v10.0（无播放返回）
...
// @grant        GM_addStyle
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/556768/%E7%83%9F%E8%8D%89%E7%BD%91%E7%BB%9C%E5%AD%A6%E9%99%A2%E9%80%9A%E7%94%A8%E6%99%BA%E8%83%BD%E5%AD%A6%E4%B9%A0%E5%8A%A9%E6%89%8B%20v100%EF%BC%88%E6%97%A0%E6%92%AD%E6%94%BE%E8%BF%94%E5%9B%9E%EF%BC%89.user.js
// @updateURL https://update.greasyfork.org/scripts/556768/%E7%83%9F%E8%8D%89%E7%BD%91%E7%BB%9C%E5%AD%A6%E9%99%A2%E9%80%9A%E7%94%A8%E6%99%BA%E8%83%BD%E5%AD%A6%E4%B9%A0%E5%8A%A9%E6%89%8B%20v100%EF%BC%88%E6%97%A0%E6%92%AD%E6%94%BE%E8%BF%94%E5%9B%9E%EF%BC%89.meta.js
// ==/UserScript==
...
const CLEANUP = {
  ...
  clear() {
    ...
    this.observers = [];
    // 清理视频加载超时
    if (STATE.videoLoadTimeout) {
      clearTimeout(STATE.videoLoadTimeout);
      STATE.videoLoadTimeout = null;
    }
  }
};
...
const STATE = {
  ...
  isProcessing: false,
  currentVideo: null,
  videoLoadTimeout: null,  // 视频加载超时计时器
  videoLoadRetryCount: 0,  // 重试计数
  maxRetries: 2            // 最大重试次数
};
...
// --- 【新增】返回目录并刷新继续 ---
function returnAndRefreshCatalog() {
  CLEANUP.clear(); // 清理所有资源
  ...
  UI.update('返回目录刷新中...', 'search');
  
  setTimeout(() => {
    if (STATE.returnUrl && STATE.returnUrl !== location.href) {
      // 返回目录但不清空returnUrl，这样刷新后会继续扫描
...
async function handleVideo() {
  ...
  STATE.videoLoadTimeout = setTimeout(() => {
    UI.update('视频加载超时，返回目录重试...', 'error');
    STATE.videoLoadRetryCount++;
    
    if (STATE.videoLoadRetryCount >= STATE.maxRetries) {
      UI.update('多次加载失败，跳过此视频', 'warn');
      STATE.videoLoadRetryCount = 0;
      // 标记为已完成，避免死循环
      try {
        const title = document.querySelector('h1')?.textContent.trim() || document.title;
        STATE.completedCourses.add(title + '_FAILED');
        GM_setValue('completedCourses', JSON.stringify(Array.from(STATE.completedCourses)));
      } catch(e) {}
    }
    
    // 返回目录刷新
    returnAndRefreshCatalog();
  }, 15000);
  ...
  if (!video) {
    UI.update('无视频元素，尝试检查章节...', 'warn');
    // 再等3秒给页面加载时间
    await new Promise(r => setTimeout(r, 3000));
    video = document.querySelector('video');
    
    if (!video) {
      // 【新增】仍然没有视频，可能是页面问题，不立即跳转
      // 让超时机制来处理
      UI.update('页面可能异常，等待超时处理...', 'warn');
      return;
    }
  }
  ...
  STATE.videoLoadRetryCount = 0; // 重置重试计数
...
function init() {
  ...
  if (coursePatterns.some(k => url.includes(k))) {
    ...
    // 【优化】目录页刷新后自动继续扫描
    setTimeout(() => {
      if (STATE.returnUrl === url) {
        UI.update('刷新后继续扫描...', 'search');
        handleCatalog();
      } else {
        handleCatalog();
      }
