...
// @version      2.15
// @namespace    ErQi
// @description  挂机无止境的辅助脚本(自动秘境无法单独使用)
...
// @downloadURL https://update.greasyfork.org/scripts/389289/idleinfinity_.user.js
// @updateURL https://update.greasyfork.org/scripts/389289/idleinfinity_.meta.js
// ==/UserScript==
...
/*    if (config.dropNotification && location.href.indexOf('Map/Dungeon') === -1) {
    ...
    if (config.mapHack && location.href.indexOf('Map/Dungeon') >= 0) {
        ...
        if (map) {
            ...
            if (map[id] && map[id] === 'start') {
                ...
                } else if (bossLeft === 0 && config.mapHackType === 'mystery') {
        ...
        function mapMove() {
            ...
            if (blocks.length) {
                ...
                for (let i = 0; i < enemyBlocks.length; i++) {
                    ...
                    // 判断是否有打不过的怪
                    if ((itemDistance < distance) && failedBlocks.indexOf(enemyBlocks[i]) === -1) {
        ...
        function ajaxMove(block, a) {
            ...
            const g = f.attr("id");
            const k = $("#cid").val();
            let td = JSON.parse(localStorage.getItem("t"));
            let ti = 0;

            while(!td[0] || !td[0].A) {
            	setTimeout(td = JSON.parse(localStorage.getItem("t")),500);
            	ti++
            	if (ti == 6) {
            		//alert('获取token失败，点击确定将刷新页面重新获取\r\n如果重复弹出此提醒，请检查你的网络连接速度或联系作者\r\n（如果你能找到我的话）');
            		location.href = "/Map/Dungeon?id=" + k;
            		break;
            	}
            }
            ...
            if (f.hasClass("monster")) {
                ...
                $.ajax({
                    ...
                    success: function (a) {
                        ...
                        setTimeout(() => {
                            mapMove();
                        }, Math.round(config.userNumber * 500));
                    },
    ...
    if (config.showSetAttr) {
        ...
        function loadSetAttr() {
            ...
            if (location.href.indexOf('Auction/Query') < 0) {
                if (setDB && newSetDB) {
    ...
    if (config.oneKeyAgree && location.href.indexOf('Notice/Query') >= 0) {
        ...
        action += renderButton('idle-ui-agree-current', '同意当页');
        const def = $('a.btn.btn-xs.btn-default');
        def.eq(0).before(action);
        $('#idle-ui-agree-current').click(function () {
            agreeMessage();
        });
