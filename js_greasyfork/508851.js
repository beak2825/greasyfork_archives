...
// @version      2.5
// @description  answer machine unlearn v2
// @author       Your Name
// @match        https://web.uplearn.co.uk/*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/508851/answer%20machine.user.js
// @updateURL https://update.greasyfork.org/scripts/508851/answer%20machine.meta.js
// ==/UserScript==
// @license MIT
...
(function () {
    ...
    button.innerHTML = "Answers (Please Don't Abuse)";
    ...
    // Function to submit an answer and retrieve feedback
    async function submitAnswer(questionId, answerValue) {
        ...
        try {
            const response = await fetch("https://web.uplearn.co.uk/api/", {
                ...
                body: JSON.stringify({
                    ...
                    variables: {
                        ...
                        responses: [{
                            answer: JSON.stringify({ value: answerValue }),
                            timeSpent: "PT24.64S"
                        }]
                    },
                    query: `
            ...
            if (feedbackMessages && feedbackMessages.length > 0) {
                return feedbackMessages.map(message => formatTextWithBoldTags(message.content)).join("\n");
            } else {
                return null; // No feedback received
            }
        } catch (error) {
            console.error(`Error submitting answer for question ID ${questionId}:`, error);
            alert(`Error submitting answer for question ID ${questionId}. Check the console for details.`);
            return null;
        }
    }
    ...
    async function fetchAndDisplayFeedbackMessages() {
        ...
        for (const questionId of questionIds) {
            try {
                let feedbackMessage = await submitAnswer(questionId, ""); // Try with empty value first

                if (!feedbackMessage) {
                    feedbackMessage = await submitAnswer(questionId, 0); // Try with value "3" if no feedback
                }

                if (!feedbackMessage) {
                    feedbackMessage = "A"; // Default to "C" if still no feedback
                }

                const questionNumber = questionMap[questionId];
                allFeedbackMessages.push(`${questionNumber}. ${feedbackMessage}`);
            } catch (error) {
                console.error(`Error processing question ID ${questionId}:`, error);
                alert(`Error processing question ID ${questionId}. Check the console for details.`);
            }
        }

        if (allFeedbackMessages.length > 0) {
            const formattedMessages = allFeedbackMessages.join("\n\n");
