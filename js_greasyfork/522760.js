...
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/522760/Hunter%20Stumblechat%20Script.user.js
// @updateURL https://update.greasyfork.org/scripts/522760/Hunter%20Stumblechat%20Script.meta.js
// ==/UserScript==
let webSocket;
...
function isUserBanned(username) {
    // Retrieve the ban list from browser cache
    const storedBans = localStorage.getItem("bans");
    const banList = storedBans ? JSON.parse(storedBans) : [];
 
    console.log("Ban List:", banList);
 
    // Check if the username is in the ban list
    const isBanned = banList.includes(username);
    console.log(`Checking ban status for user ${username}. Banned: ${isBanned}`);
 
    if (isBanned) {
        // If the user is banned, send a ban message to the WebSocket server
    ...
    ws.addEventListener('message', event => {
        ...
        if (parsedMessage.stumble === "joined") {
            ...
            if (handle !== yourHandle) {
                ...
                // Check if the user is banned and ban if they are
...
function displayWebSocketMessage(message) {
    ...
    if (parsedMessage.stumble === "join") {
        ...
        if (text === "#join") {
            ...
            //Ai(word); // Adjust parameters as needed
        }
...
function ban(username) {
    // Check if the WebSocket is not null
...
// Function to create ban list div
function createBanListDiv(banList) {
    const banListDiv = document.createElement("div");
    banListDiv.id = "banList";
    banListDiv.style.position = "absolute"; // Change to absolute positioning
    banListDiv.style.top = "100px"; // Adjust top position as needed
    banListDiv.style.left = "10px"; // Adjust left position as needed
    banListDiv.style.height = "calc(100% - 100px)"; // Adjust height to fill remaining space
    banListDiv.style.overflowY = "auto";
    banListDiv.style.color = "#ffffff";
    banListDiv.style.padding = "10px";
    banListDiv.style.zIndex = "2"; // Set a higher z-index value
    banListDiv.style.display = "none"; // Hide ban list by default

    // Create list element
    const list = document.createElement("ul");

    // Populate list with bans
    banList.forEach(ban => {
        const listItem = document.createElement("li");
    ...
    // Append list to banListDiv
    banListDiv.appendChild(list);

    // Create button to toggle ban list visibility
    const toggleButton = document.createElement("button");
    ...
    fileInput.addEventListener("change", (event) => {
        ...
        reader.onload = (e) => {
            // Read file content and split by new lines
            const importedBans = e.target.result.split("\n").map(line => line.trim()).filter(line => line.length > 0);

            // Convert the list to JSON and update the ban list
            const jsonBans = JSON.stringify(importedBans);
            localStorage.setItem("bans", jsonBans);
            banList.splice(0, banList.length, ...importedBans); // Update the current ban list
    ...
    saveButton.addEventListener("click", () => {
        // Convert ban list to a plain text format (newline-separated)
...
// Function to refresh the ban list and update display
function refreshBanList(banList) {
    ...
    localStorage.setItem("bans", JSON.stringify(banList)); // Store as JSON
}
...
// Load the ban list from localStorage (if exists), otherwise initialize as an empty array
const storedBans = localStorage.getItem("bans");
const banList = storedBans ? JSON.parse(storedBans) : [];

// Create and display the ban list div
const banListDiv = createBanListDiv(banList);
document.body.appendChild(banListDiv);

// Function to toggle the visibility of the ban list
