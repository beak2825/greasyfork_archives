...
// @version        1.10.0
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/3991/Dinosaur%20Comics%20Overlay%20Chooser.user.js
// @updateURL https://update.greasyfork.org/scripts/3991/Dinosaur%20Comics%20Overlay%20Chooser.meta.js
// ==/UserScript==
...
const listOfOverlays = ["A Lesson Is Learned But The Damage Is Irreversible", "About Pirates", "Achewood", "Brodyquest", "Before The Special Effects Are Added", "Buttercup Festival", "Daisy Owl", "DC Comics", "Dr McNinja", "Just About T-Rex", "Mr Boop", "Nedroid",
                        "On April Fools Day 2014", "One Done In Watercolours", "One Where T-Rex Got Assimilated", "One Where T-Rex Swears More", "One Where T-Rex Wears More", "Penny Arcade", "Pokey", "Problem Sleuth", "Registered Weapon",
                        "Shortpacked", "Sister Claire", "Something More Historically Accurate", "Something That Will Destroy My Brain", "Sweet Bro And Hella Jeff", "The Last Dinosaur Comic Ever", "The RIAA Version Of The Internet|theRIAAversionoftheinternet",
                        "The Same Pictures", "webcomic name", "Wigu", "Winnie The Pooh", "With Less White", "Wondermark", "XKCD"];
...
function getValue(key, defaultValue) {
	let value = localStorage.getItem("DCOC_"+key);
	if (value) {
        return value;
    }
	return defaultValue;
...
function isValueStored() {
	for (let i = 0, len = localStorage.length; i < len; i++) {
		if (localStorage.key(i).indexOf("DCOC_") === 0) {
            return true;
        }
	}
	return false;
...
function isUrl(line) {
	return line !== null && line.search('http') === 0;
}
...
if (overlayName === 'custom' || isUrl(overlayName)) {
	...
	if (overlayName === 'custom') {
        overlayLocation = getValue('customOverlay', '');
    } else {
        overlayLocation = overlayName;
    }
...
selectBox.onchange = function() {
	...
	if (selectBox.value === 'none' || selectBox.value === '') {
        window.location.href = url.substring(0, url.length-1);
    } else {
        window.location.href = url + 'butiwouldratherbereading=' + selectBox.value;
    }
	//return false;
}
...
for (let i=0, len = listOfOverlays.length; i < len; i++) {
	let displayName = listOfOverlays[i];
	let valueName;
	//If there's a special value name, retrieve that
	let sepPos = displayName.indexOf('|');
	if (sepPos > -1) {
		valueName = displayName.substring(sepPos + 1);
		displayName = displayName.substring(0, sepPos);
	} else {
        valueName = displayName.toLowerCase().replace("-", "").split(" ").join("");
    }
...
//Custom overlay
if (getValue('customOverlay', '') !== '') {
    createLink('Change Custom Overlay', menu, true, function() {setCustomOverlay(); return false;});
} else {
    createLink('Set Custom Overlay', menu, true, function() {setCustomOverlay(); return false;});
}
...
//Display the dropdown between the header and the comic
	...
	let newRow = document.getElementById('header').getElementsByTagName('table')[0].insertRow(2); //Below the Utahr.-Rex conversation (1 = above)
...
//Display the dropdown below the comic
	...
	if (comic.length >= 1) {
		...
		if (comic.snapshotLength === 1) {
            comic = comic.snapshotItem(0);
        } else {
            comic = null;
        }
	}
	...
	if (comic) {
		...
		let comicParent = comic.parentNode;
