...
// @downloadURL https://update.greasyfork.org/scripts/28544/Hybrid%20-%20Calls%20-%20DCS.user.js
// @updateURL https://update.greasyfork.org/scripts/28544/Hybrid%20-%20Calls%20-%20DCS.meta.js
// ==/UserScript==
...
if ($('li:contains("Call - Contact")').length) {
    ...
    createButton('Nope',0,0,2,3,'This person does not work there, the person I spoke to was not aware of past employment');
    createButton('Wrong Biz',0,1,3,3,'This number connected to the wrong business');
    createButton('Biz on VM',0,0,3,3,'The business is listed on the voicemail but with no information pertaining to the contact');
    createButton('DC',2,2,3,3,'Disconnected- this number is not in service. ');
    createButton('DC - Directory',2,2,3,3,'Disconnected - Directory assistance offered');
    createButton('DC - Fast Busy',2,2,3,3,'Disconnected - fast busy');
    createButton('Fax',3,2,3,3,'Fax');
    createButton('Generic VM',0,2,3,3,'This  number connected to a voicemail with information that did not help to answer questions.');
    createButton('Personal VM',0,2,3,3,'Voice mail did identify the contact but no title information');
    createButton('Ringing', 1,2,3,3,'This call did connect but no one answered after extended ringing');
    createButton('Silence',0,2,3,3,'Rang until silence');
    createButton('Busy',0,2,3,3,'Busy signal repeatedly after multiple attemps at calling');
    createButton('No Contact VM',0,0,3,3,'Voicemail without contact');
    createButton('Hung Up',0,2,3,3,'Hung up on me');
