...
// @grant       GM_listValues
// @grant        God
// @downloadURL https://update.greasyfork.org/scripts/443639/ReaderTextOtp.user.js
// @updateURL https://update.greasyfork.org/scripts/443639/ReaderTextOtp.meta.js
// ==/UserScript==
...
if (GmailBox !== -1) {
    if (localStorage.getItem('OTP1') == undefined) {
        ...
        if (ActualTime - localStorage.getItem('OTP1At') > 1800000) {
            console.log('OTP1 a éxpiré \n Deleting...');
            localStorage.removeItem('OTP1At');
            localStorage.removeItem('OTP1');
        } else {
            console.log('OTP1 is saved since ' + Math.floor(((ActualTime - localStorage.getItem('OTP1At')) / 60000) + 1) + ' min');
        };
        console.log('Waiting for the second OTP....')
    }
};
/*Transfer OTP1 to BLS*/
if (GM_getValue("OTP1") !== null && GM_getValue("OTP1") !== undefined) {
                    ...
                    var OTP1RegistredAt0 = date.getTime();
                    console.log('OTP1 Transfered to bls Successfuly');


                };

/*Clear Storage In index*/
if (index !== -1) {console.log('Delete all storage....');
localStorage.removeItem('gmailOpening');
                   localStorage.removeItem('OTP1');
            GM_deleteValue('OTP1');
                   localStorage.removeItem('OTP1At');
}
