...
// @require      https://update.greasyfork.org/scripts/445293/1704677/TyprMd5.js
// @connect      vercel.app
// @connect      xmig6.cn
// @connect      lyck6.cn
// @connect      *
...
// 悬浮窗状态持久化（油猴存储）
const FLOAT_WIN_STORE_KEY = 'unrival_float_win_state_v1';
function loadFloatWinState() {
  try {
    const state = GM_getValue(FLOAT_WIN_STORE_KEY, null);
...
function saveFloatWinState(patch) {
  try {
    const prev = loadFloatWinState() || {};
    GM_setValue(FLOAT_WIN_STORE_KEY, Object.assign({}, prev, patch, { ts: Date.now() }));
  } catch (e) {}
}
// 根据已保存的状态初始化悬浮窗（在插入 DOM 之前调用，避免“跳动”）
function initFloatWinFromState() {
  const state = loadFloatWinState();
  if (!state) return;

  // 位置（仅在非最大化时应用，避免和居中 transform 冲突）
  if (!state.isMax) {
    if (typeof state.left === 'number') floatWin.style.left = state.left + 'px';
    if (typeof state.top === 'number') floatWin.style.top = state.top + 'px';
    floatWin.style.transform = 'none';
  }

  // 最小化状态
  if (state.isMinimized) {
    const content = floatWin.querySelector("#xxt-helper-content");
    if (content) {
  ...
  // 最大化状态（最后应用）
  if (state.isMax) {
    floatWin.style.width = "720px";
    floatWin.style.height = "560px";
...
`;
// 在插入到页面之前，先按上次的状态设置好位置和大小，避免刷新后“先到默认位置再跳到记忆位置”的感觉
initFloatWinFromState();
...
function setupDragging() {
  ...
  top.document.addEventListener("mouseup", () => {
    ...
    // 拖拽结束后保存位置（若当前是最大化，则不存坐标，避免还原冲突）
    try {
      if (!isMax) {
        const left = parseFloat(floatWin.style.left || floatWin.offsetLeft || 0);
        const topPos = parseFloat(floatWin.style.top || floatWin.offsetTop || 0);
        saveFloatWinState({ left, top: topPos });
      }
    } catch (e) {}
  });
  ...
  if (minBtn) {
    minBtn.onclick = () => {
      if (!isMinimized) {
        ...
        isMinimized = true;
        saveFloatWinState({ isMinimized: true });
      } else {
        // 恢复：显示完整内容
        ...
        isMinimized = false;
        saveFloatWinState({ isMinimized: false });
      }
    };
  }
  ...
  if (maxBtn) {
    maxBtn.onclick = () => {
      if (!isMax) {
        ...
        floatWin.style.width = "720px"; // 缩小为原来的80%
        floatWin.style.height = "560px"; // 缩小为原来的80%
        // 优先恢复到上次拖拽保存的位置；没有就用默认值
        const state = loadFloatWinState();
        if (state && typeof state.top === 'number' && typeof state.left === 'number') {
          floatWin.style.top = state.top + "px";
          floatWin.style.left = state.left + "px";
        } else {
                    ...
                    ">
                        ...
                        ">
                            ...
                            <textarea id="cx-copy-modal-textarea" style="
                                ...
                                resize: vertical;
                                outline: none;
                                box-sizing: border-box; /* 修复textarea宽度溢出问题 */
                            "></textarea>
