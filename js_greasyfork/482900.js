...
// @author       Exception & 7ommy & AGSV骄阳
// @match        *://*.agsvpt.com/details.php*
// @match        *://*.agsvpt.work/details.php*
// @match        *://*.agsvpt.cn/details.php*
// @match        *://*.agsvpt.com/web/torrent-approval-page?torrent_id=*
// @match        *://*.agsvpt.work/web/torrent-approval-page?torrent_id=*
// @match        *://*.agsvpt.cn/web/torrent-approval-page?torrent_id=*
...
(function() {
    ...
    var redleavesSeed = 0; //红叶官种
    var beitaiSeed = 0; //备胎官种
    var arkSeed = 0; //方舟种子
    ...
    if (title_lowercase.includes("-rl") || title_lowercase.includes("r²") || title_lowercase.includes("-vandoge@R²")) {
        redleavesSeed = 1;
        console.log("红叶官种");
    }


    if(title_lowercase.includes("beitai")) {
        beitaiSeed = 1;
    }

    if (beitaiSeed || redleavesSeed || haresSeed) {
        arkSeed = 1;
    }
    ...
    for (var i = 0; i < tdlist.length; i ++) {
        ...
        if (td.text() == '基本信息') {
            ...
            if (text.indexOf('GodDramas') >= 0) {
                ...
                category = 23;
            } else if (text.indexOf('RL') >= 0) {
                category = 25;
            } else if (text.indexOf('BeiTai') >= 0) {
                category = 26;
    ...
    if(/[^\x00-\xff]+/g.test(title) && !title.includes('￡') && !title.includes('™') && !/[\u2161-\u2169]/g.test(title) && !title.includes('Ⅰ') && !title.includes('白自在') && !title.includes('至尊宝') && cat !== 415) {
    ...
    if (redleavesSeed && category != 25) {
        $('#assistant-tooltips').append('制作组未选择RL<br>');
        error = true;
    }

    if (beitaiSeed && category != 26) {
        $('#assistant-tooltips').append('制作组未选择BeiTai<br>');
        error = true;
    }
...
//     }
    ...
    if (cat === 413 || cat === 418 || cat === 412 || cat === 411 || cat === 408  || cat === 406 || cat === 415) {
        $('#assistant-tooltips').empty();
        error = false;
...
//     });
    ...
    // 主页面操作
    if (/https:\/\/.*\.agsvpt\.(com|work)\/details\.php\?id=.*/.test(window.location.href)) {
        addApproveLink();
    ...
    // 弹出页的操作
    if (/https:\/\/.*\.agsvpt\.(com|work)\/web\/torrent-approval-page\?torrent_id=.*/.test(window.location.href)) {
        // 使用延迟来等待页面可能的异步加载
        setTimeout(function() {
            //console.log("autoFillErrorInfo:"+GM_getValue('autoFillErrorInfo'));
            //console.log("autoCheckAndConfirm:"+GM_getValue('autoCheckAndConfirm'));
...
//审核弹窗页面自动勾选“拒绝”但不自动点击确认按钮
  if (/https:\/\/.*\.agsvpt\.(com|work)\/web\/torrent-approval-page\?torrent_id=.*/.test(window.location.href)) {
    setTimeout(function() {
        // 如果autoReject标志为true，自动勾选“拒绝”
        if (GM_getValue('autoReject', false)) {
            var radioDenyButton = document.querySelector("body > div.form-comments > form > div:nth-child(3) > div > div:nth-child(6)");
            if (radioDenyButton) {
                radioDenyButton.click();
                // 只勾选，不自动提交，等待人工点击提交按钮
                console.log('已自动勾选拒绝，等待人工提交');
            } else {
                console.warn('拒绝选项按钮未找到');
            }
            // 不清除autoReject标志，人工提交后可手动清理或下次自动覆盖
        }
        // 你之前处理自动通过、自动填写错误信息等逻辑保留
    }, timeout);
}
...
//按 Ctrl + Alt + 1 快捷键，当前页自动点击“审核”按钮打开审核弹窗；审核页面打开后，自动勾选“拒绝”选项，但不自动提交，由人工确认后提交。
   document.addEventListener('keydown', function(e) {
    if (e.ctrlKey && e.altKey && e.key === '1') {
        GM_setValue('autoReject', true);    // 设置自动拒绝标志
        let approvalButton = document.querySelector('#approval'); // 审核按钮选择器
        if (approvalButton) {
            approvalButton.click();
        } else {
            console.warn("未找到审核按钮 #approval");
        }
    }
    // 保留你之前的 F3/F4 快捷键逻辑
});
