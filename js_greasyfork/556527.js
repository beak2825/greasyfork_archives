// ==UserScript==
// @name         Forum Script for Support Team (С IP)
    ...
    },
{
    ...
    // Добавляем стили для кнопок и модального окна
    ...
    style.textContent = `
        ...
        /* Горизонтальное расположение полей ввода для ПК */
        .ip-inputs-row {
            display: flex;
            gap: 15px;
            margin-bottom: 20px;
        ...
        /* Стили для подробного результата */
    ...
    // Функция для открытия модального окна IP
    ...
    async function compareIPAddresses() {
        ...
        if (!ip1 || !ip2) {
            showResult('Пожалуйста, введите оба IP-адреса', 'error');
            return;
        }

        if (!isValidIP(ip1) || !isValidIP(ip2)) {
            showResult('Один или оба IP-адреса имеют неверный формат', 'error');
            return;
        }

        showResult('<div class="ip-loading">🔄 Получаем геоданные...</div>', 'loading');

        try {
            // Получаем геоданные для обоих IP
            ...
            if (!geo1 || !geo2) {
                showResult('<div class="ip-error">❌ Не удалось получить геоданные для одного из IP-адресов</div>', 'error');
                return;
            }
            ...
            // Формируем подробный результат
               // В функции compareIPAddresses заменим блок формирования результата:
            const result = `
                <div class="ip-comparison-result">
                    ...
                    <div class="ip-comparison-grid">
                        <div class="ip-comparison-item">
                            ...
                            <div class="ip-comparison-details">
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">IP:</span>
                                    <span class="ip-detail-value">${ip1}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Страна:</span>
                                    <span class="ip-detail-value">${geo1.country}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Город:</span>
                                    <span class="ip-detail-value">${geo1.city}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Регион:</span>
                                    <span class="ip-detail-value">${geo1.region}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Провайдер:</span>
                                    <span class="ip-detail-value">${geo1.isp}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Координаты:</span>
                                    <span class="ip-detail-value">${typeof geo1.latitude === 'number' ? geo1.latitude.toFixed(6) : geo1.latitude}, ${typeof geo1.longitude === 'number' ? geo1.longitude.toFixed(6) : geo1.longitude}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Часовой пояс:</span>
                                    <span class="ip-detail-value">${geo1.timezone}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="ip-comparison-item">
                            ...
                            <div class="ip-comparison-details">
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">IP:</span>
                                    <span class="ip-detail-value">${ip2}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Страна:</span>
                                    <span class="ip-detail-value">${geo2.country}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Город:</span>
                                    <span class="ip-detail-value">${geo2.city}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Регион:</span>
                                    <span class="ip-detail-value">${geo2.region}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Провайдер:</span>
                                    <span class="ip-detail-value">${geo2.isp}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Координаты:</span>
                                    <span class="ip-detail-value">${typeof geo2.latitude === 'number' ? geo2.latitude.toFixed(6) : geo2.latitude}, ${typeof geo2.longitude === 'number' ? geo2.longitude.toFixed(6) : geo2.longitude}</span>
                                </div>
                                <div class="ip-detail-row">
                                    <span class="ip-detail-label">Часовой пояс:</span>
                                    <span class="ip-detail-value">${geo2.timezone}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="ip-distance-result">
                        ...
                        <div class="ip-distance-description">
                    ...
                    <div class="ip-comparison-summary">
                        <div class="ip-summary-row">
                            <span class="ip-summary-label">Совпадение страны:</span>
                            <span class="ip-summary-value ${geo1.country === geo2.country ? 'ip-match' : 'ip-no-match'}">
                                ${geo1.country === geo2.country ? '✅ Да' : '❌ Нет'}
                            </span>
                        </div>
                        <div class="ip-summary-row">
                            <span class="ip-summary-label">Совпадение города:</span>
                            <span class="ip-summary-value ${geo1.city === geo2.city ? 'ip-match' : 'ip-no-match'}">
                                ${geo1.city === geo2.city ? '✅ Да' : '❌ Нет'}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            showResult(`<div class="ip-error">❌ Ошибка при получении данных: ${error.message}</div>`, 'error');
        }
    }

        // Функция для получения описания расстояния
    ...
    // Функция для нормализации данных из разных API
    function normalizeGeoData(data, endpoint) {
        console.log('Raw data from', endpoint, ':', data); // Для отладки
        ...
        let normalized = {
            ...
            org: getValue(data, ['org', 'asn_org', 'company.name'])
        };

        // Обработка специальных случаев для разных API
        if (endpoint.includes('ipapi.co')) {
            normalized = normalizeIpApiCo(data, normalized);
        } else if (endpoint.includes('ipwhois.app')) {
            normalized = normalizeIpWhoisApp(data, normalized);
        } else if (endpoint.includes('ip-api.com')) {
            normalized = normalizeIpApiCom(data, normalized);
        } else if (endpoint.includes('freeipapi.com')) {
            normalized = normalizeFreeIpApi(data, normalized);
        }
        ...
        if (normalized.latitude !== 'Неизвестно') {
            normalized.latitude = parseFloat(normalized.latitude);
        }
        if (normalized.longitude !== 'Неизвестно') {
            normalized.longitude = parseFloat(normalized.longitude);
        }

        console.log('Normalized data:', normalized); // Для отладки
        return normalized;
    }

    // Вспомогательная функция для получения значения по приоритету ключей
    function getValue(obj, keys) {
        for (let key of keys) {
            // Обработка вложенных свойств (например, 'connection.isp')
            if (key.includes('.')) {
    ...
    // Специфичные нормализаторы для каждого API
    function normalizeIpApiCo(data, normalized) {
        // ipapi.co возвращает region_code для региона
        if (data.region_code && normalized.region === 'Неизвестно') {
            normalized.region = data.region_code;
        }
        // ipapi.co возвращает org для провайдера
        if (data.org && normalized.isp === 'Неизвестно') {
            normalized.isp = data.org;
        }
        return normalized;
    }

    function normalizeIpWhoisApp(data, normalized) {
        // ipwhois.app возвращает continent для региона
        if (data.continent && normalized.region === 'Неизвестно') {
            normalized.region = data.continent;
        }
        // ipwhois.app возвращает asn для провайдера
        if (data.asn && normalized.isp === 'Неизвестно') {
            normalized.isp = data.asn;
        }
        return normalized;
    }

    function normalizeIpApiCom(data, normalized) {
        // ip-api.com возвращает regionName для региона
        if (data.regionName && normalized.region === 'Неизвестно') {
            normalized.region = data.regionName;
        }
        // ip-api.com возвращает org для провайдера
        if (data.org && normalized.isp === 'Неизвестно') {
            normalized.isp = data.org;
        }
        // ip-api.com возвращает timezone
        if (data.timezone && normalized.timezone === 'Неизвестно') {
            normalized.timezone = data.timezone;
        }
        return normalized;
    }

    function normalizeFreeIpApi(data, normalized) {
        // freeipapi.com возвращает regionName для региона
        if (data.regionName && normalized.region === 'Неизвестно') {
            normalized.region = data.regionName;
        }
        // freeipapi.com возвращает ispService для провайдера
        if (data.ispService && normalized.isp === 'Неизвестно') {
            normalized.isp = data.ispService;
        }
        return normalized;
    }

    // Функция для получения геоданных IP
    async function getGeoData(ip) {
        ...
        for (const endpoint of endpoints) {
            try {
                console.log(`Trying ${endpoint.name} for IP ${ip}`);
                ...
                if (response.ok) {
                    ...
                    if (isValidGeoData(data)) {
                        ...
                        console.log(`Invalid data from ${endpoint.name}`);
                    }
                } else {
                    console.log(`HTTP error from ${endpoint.name}: ${response.status}`);
                }
            } catch (error) {
                console.log(`Error from ${endpoint.name}:`, error);
                continue;
            }
        }
    ...
    // Функция для проверки валидности геоданных
    function isValidGeoData(data) {
        // Проверяем, что есть хотя бы страна или город
        return (data.country || data.country_name || data.countryName) &&
               (data.city || data.cityName);
    }



    // Функция для вычисления расстояния между координатами (формула гаверсинуса)
    ...
    function showResult(message, type) {
        ...
        if (resultSection && resultText) {
            ...
            resultText.className = 'ip-result-text';
            if (type === 'error') {
                resultText.classList.add('ip-result-error');
            } else if (type === 'success') {
                resultText.classList.add('ip-result-success');
            } else if (type === 'loading') {
                resultText.classList.add('ip-result-loading');
            }
        }
    }
    ...
    // Функция для сохранения данных (теперь сохраняем введенные IP)
    function saveIPData() {
    ...
    function loadIPSavedData() {
        ...
        if (ipInput1 && savedIP1) {
            ipInput1.value = savedIP1;
        }

        if (ipInput2 && savedIP2) {
            ipInput2.value = savedIP2;
        }
    }
    ...
    // В функции createButtonsContainer() добавить новую кнопку в массив buttonsData
    function createButtonsContainer() {
        ...
        const buttonsData = [
            ...
            { text: "IP", href: "#", action: "openIPModal" } // Новая кнопка
        ];
        ...
        buttonsData.forEach(buttonInfo => {
            ...
            if (buttonInfo.action === "openIPModal") {
                button.addEventListener("click", openIPModal);
            } else {
