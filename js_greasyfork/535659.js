...
// @author       独爱冰淇淋 (重构 by Gemini, CSS fix by User)
// @match        https://chat.deepseek.com/*
// @grant        GM_setValue
// @grant        GM_getValue
// @grant        GM_addStyle
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/535659/%E7%BB%99DeepSeek%E6%B7%BB%E5%8A%A0%E5%8E%9F%E7%94%9F%E5%8C%96%E7%9A%84%E6%8F%90%E7%A4%BA%E8%AF%8D%E7%AE%A1%E7%90%86%E5%8A%9F%E8%83%BD.user.js
// @updateURL https://update.greasyfork.org/scripts/535659/%E7%BB%99DeepSeek%E6%B7%BB%E5%8A%A0%E5%8E%9F%E7%94%9F%E5%8C%96%E7%9A%84%E6%8F%90%E7%A4%BA%E8%AF%8D%E7%AE%A1%E7%90%86%E5%8A%9F%E8%83%BD.meta.js
// ==/UserScript==
...
(function() {
    ...
    function updateUsageDropdown() {
        ...
        savedPrompts.forEach(prompt => {
            ...
            item.addEventListener('click', async () => {
                try {
                    const textArea = $('textarea.ds-scroll-area');
    ...
    function toggleManagementPanel(forceState) {
        ...
        if (show) {
            // Initialize panel if it's empty
            if (!managementSidePanel.querySelector('.dspm-management-content-area')) {
    ...
    function setupPanelEventListeners() {
        ...
        const multiSelectToggleBtn = $(`#${SCRIPT_PREFIX}multi_select_toggle_btn`, managementSidePanel);
        ...
        // Setup add prompt button
        $(`#${SCRIPT_PREFIX}add_prompt_btn`, managementSidePanel).addEventListener('click', (e) => {
            e.stopPropagation();
            showEditPromptModal();
        });
    ...
    function updateManagementPanelContent() {
        ...
        // Only update the content area if it exists
        const contentArea = $('.dspm-management-content-area', managementSidePanel);
        if (contentArea) {
            ...
            if (isMultiSelectMode) {
                $$('.dspm-select-prompt-checkbox', contentArea).forEach(checkbox => {
            ...
            $$('.dspm-apply-prompt-btn', contentArea).forEach(button => {
            ...
            $$('.dspm-edit-prompt-btn', contentArea).forEach(button => {
                button.addEventListener('click', function(e) {
                    e.stopPropagation();
        ...
        const paginationContainer = $('.dspm-pagination-container', managementSidePanel);
        if (paginationContainer) {
            paginationContainer.innerHTML = totalPages > 1 ? `
                ...
                <div class="flex items-center space-x-2">
                    <button id="${SCRIPT_PREFIX}prev_page_btn" class="px-3 py-1.5 text-sm text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-md ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-200 dark:hover:bg-gray-600'}" ${currentPage === 1 ? 'disabled' : ''}>
                        <i class="fas fa-chevron-left mr-1"></i> 上一页
                    </button>
                    <button id="${SCRIPT_PREFIX}next_page_btn" class="px-3 py-1.5 text-sm text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-md ${currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-200 dark:hover:bg-gray-600'}" ${currentPage === totalPages ? 'disabled' : ''}>
            ...
            if (totalPages > 1) {
                const prevPageBtn = $(`#${SCRIPT_PREFIX}prev_page_btn`, paginationContainer);
                const nextPageBtn = $(`#${SCRIPT_PREFIX}next_page_btn`, paginationContainer);
        ...
        // Update selected prompts bar
        const selectedPromptsBar = $(`#${SCRIPT_PREFIX}selected_prompts_bar`, managementSidePanel);
        if (selectedPrompts.length > 0) {
            if (!selectedPromptsBar) {
                const bar = document.createElement('div');
                bar.id = SCRIPT_PREFIX + 'selected_prompts_bar';
                bar.className = 'flex items-center justify-between px-4 py-2 bg-indigo-50 dark:bg-indigo-900/20 border-b border-indigo-100 dark:border-indigo-800/30';
                bar.innerHTML = `
                ...
                `;
                const searchBar = $('.dspm-search-bar-container', managementSidePanel);
                if (searchBar) {
                    searchBar.after(bar);
                }
            } else {
                selectedPromptsBar.querySelector('span').textContent = `已选择 ${selectedPrompts.length} 项`;
            }

            // Setup selected prompts buttons
            const deselectAllBtn = $(`#${SCRIPT_PREFIX}deselect_all_btn`, selectedPromptsBar);
            const applySelectedBtn = $(`#${SCRIPT_PREFIX}apply_selected_btn`, selectedPromptsBar);
        ...
        } else if (selectedPromptsBar) {
            selectedPromptsBar.remove();
        }
    }
    ...
    function showEditPromptModal(prompt = null, index = null) {
        ...
        if (!isEditing && $('textarea.ds-scroll-area')?.value.trim()) $(`#${SCRIPT_PREFIX}prompt_content`, modalOverlay).value = $('textarea.ds-scroll-area').value.trim();
