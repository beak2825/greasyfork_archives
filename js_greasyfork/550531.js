// ==UserScript==
// @name            Bilibili 按标签、标题、时长、UP主屏蔽视频 个人自用
// @namespace       https://github.com/a11034
// @version         1.0.1
// @description     对 Bilibili.com 的视频卡片元素，以标题、UP 主、标签、双重标签、充电专属、收藏投币比、竖屏、时长、播放量、点赞率、视频分区、UP 主等级、UP 主粉丝数、UP 主简介、精选评论、置顶评论来判断匹配，添加覆盖叠加层或隐藏视频，隐藏或屏蔽热搜、附带去除广告等非视频元素的功能。本脚本为 tjxwork 开源作品（原脚本地址 https://greasyfork.org/scripts/481629）的衍生版本,由 a11034 修改并继续以 CC - BY - NC - SA 4.0 发布。
// @author          a11034
// @license         CC-BY-NC-SA
...
// @downloadURL https://update.greasyfork.org/scripts/550531/Bilibili%20%E6%8C%89%E6%A0%87%E7%AD%BE%E3%80%81%E6%A0%87%E9%A2%98%E3%80%81%E6%97%B6%E9%95%BF%E3%80%81UP%E4%B8%BB%E5%B1%8F%E8%94%BD%E8%A7%86%E9%A2%91%20%E4%B8%AA%E4%BA%BA%E8%87%AA%E7%94%A8.user.js
// @updateURL https://update.greasyfork.org/scripts/550531/Bilibili%20%E6%8C%89%E6%A0%87%E7%AD%BE%E3%80%81%E6%A0%87%E9%A2%98%E3%80%81%E6%97%B6%E9%95%BF%E3%80%81UP%E4%B8%BB%E5%B1%8F%E8%94%BD%E8%A7%86%E9%A2%91%20%E4%B8%AA%E4%BA%BA%E8%87%AA%E7%94%A8.meta.js
// ==/UserScript==
...
let blockedParameter = GM_getValue("GM_blockedParameter", {
    ...
    //屏蔽数据异常（投币占三连总和比例<=20%或>=50%）的视频
    blockedCoinRatio_Switch: false,
...
！！！对教程类视频可能会有严重误伤！！！"><input type="checkbox"
        ...
        <div class="menuOptions">
    <label title="只保留投币占比 20%~50% 的视频（like+coin+fav 总和为 100%）">
        <input type="checkbox" v-model="menuUiSettings.blockedCoinRatio_Switch" />
        投币占比过滤（20~50%）
    </label>
</div>
...
function handleBlockedCoinRatio(videoBv) {
    const d = videoInfoDict[videoBv];
    if (!d.videoLike || !d.videoCoin || !d.videoFavorite) return;
    if (d.videoView < 5000) return;
    if (d.videoFavorite < 50) return;
    const now = Math.floor(Date.now() / 1000);
    if (now - d.videoPubdate < 7200) return;
    const total = d.videoLike + d.videoCoin + d.videoFavorite;
    const ratio = (d.videoCoin / total * 100).toFixed(2);
    if (ratio < 20 || ratio > 50) {
        markAsBlockedTarget(videoBv, "投币占比异常", `${ratio}%`);
    }
}
...
function FuckYouBilibiliRecommendationSystem() {
    ...
    for (let videoElement of videoElements) {
        ...
        if (blockedParameter.blockedCoinRatio_Switch) handleBlockedCoinRatio(videoBv);
