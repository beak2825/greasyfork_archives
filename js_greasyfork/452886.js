...
// @downloadURL https://update.greasyfork.org/scripts/452886/%E5%BA%B7%E5%B8%88%E5%82%85%E8%8C%89%E8%8E%89%E8%8A%B1%E8%8C%B6.user.js
// @updateURL https://update.greasyfork.org/scripts/452886/%E5%BA%B7%E5%B8%88%E5%82%85%E8%8C%89%E8%8E%89%E8%8A%B1%E8%8C%B6.meta.js
// ==/UserScript==
...
var setting = {
    ...
    time: 2000,     // 答题时间间隔，默认5s=2000
    sub: 1,         // 测验自动提交，0为关闭,1为开启，当没答案时测验将不会提交，如需提交请设置force：1
    force: 1,       // 测验强制提交，0为关闭，1为开启，开启此功能将会强制提交测验（无论作答与否）
    ...
    phone: '123',      // 登录配置项：登录手机号/超星号
    password: '123'    // 登录配置项：登录密码
}
/************************************************************************************************** */
/**
...
/*
 ...
 /_/    \_\\__,_| \__||_| |_| \___/ |_|    |_| \_| \___|      |____||_|
  ...
  TG：https://t.me/beijiyulin  E-mail：nawlgzs@gmail.com  Date：20221103     
...
function missonWork(dom, obj) {
    ...
    if (isDo) {
        if (obj['jobid'] !== undefined) {
            ...
            // setTimeout(() => { startDoCyWork(0, dom) }, 2000)
            setTimeout(() => { startDoPhoneCyWork(0, dom, phoneWeb) }, 2000)
        } else {
            setTimeout(() => { startDoCyWork(0, dom) }, 2000)
        }
    } else {
...
function startDoPhoneCyWork(index, doms, phoneWeb) {
    ...
    getElement($(doms[index]).contents()[0], 'iframe').then(element => {
        ...
        if (workIframe.length == 0) {
            setTimeout(() => { startDoPhoneCyWork(index, doms) }, 2000)
        }
        ...
        if (setting.share && workStatus.indexOf("已完成") != -1) {
            ...
            getElement($(doms[index]).contents()[0], 'iframe[src="' + phoneWeb + '"]').then((element) => {
                setTimeout(() => { doPhoneWork($(element).contents()) }, 2000)
            })
        } else if (workStatus.indexOf('待批阅') != -1) {
            _mlist.splice(0, 1)
            _domList.splice(0, 1)
            logger('测验：' + (index + 1) + ',测验待批阅,跳过', 'red')
            setTimeout(() => { startDoPhoneCyWork(index + 1, doms, phoneWeb) }, 2000)
        } else {
            _mlist.splice(0, 1)
            _domList.splice(0, 1)
...
function startDoCyWork(index, doms) {
    ...
    getElement($(doms[index]).contents()[0], 'iframe').then(element => {
        let workIframe = element
        if (workIframe.length == 0) {
            setTimeout(() => { startDoCyWork(index, doms) }, 2000)
        }
        let workStatus = $(workIframe).contents().find('.CeYan .ZyTop h3 span:nth-child(1)').text().trim()
        if (!workStatus) {
            _domList.splice(0, 1)
        ...
        if (setting.share && workStatus.indexOf("已完成") != -1) {
            ...
            logger('测验：' + (index + 1) + ',准备处理此测验...', 'purple')
            setTimeout(() => { doWork(index, doms, workIframe) }, 2000)
        } else if (workStatus.indexOf('待批阅') != -1) {
            _mlist.splice(0, 1)
            _domList.splice(0, 1)
            logger('测验：' + (index + 1) + ',测验待批阅,跳过', 'red')
            setTimeout(() => { startDoCyWork(index + 1, doms) }, 2000)
        } else {
            _mlist.splice(0, 1)
            _domList.splice(0, 1)
            logger('测验：' + (index + 1) + ',未知状态或用户选择不收录答案,跳过', 'red')
