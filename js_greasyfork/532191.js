// ==UserScript==
// @name         广西广东干部学院课程助手
// @namespace    http://tampermonkey.net/zzzzzzys_广西广东干部学院
// @version      1.0.0
// @copyright    zzzzzzys.All Rights Reserved.
// @description  广西广东干部学院,视频自动静音播放，防暂停，单个视频可秒刷！支持考试自动答题、自动收录题目、题库保存在本题可导出，详细信息请查看文档
// @author       zzzzzzys
// @include      /^https:\/\/(?:[^/]+\.)*[^/]+\.gov\.cn(\/.*)?$/
...
// @license
// @downloadURL https://update.greasyfork.org/scripts/532191/%E5%B9%BF%E8%A5%BF%E5%B9%BF%E4%B8%9C%E5%B9%B2%E9%83%A8%E5%AD%A6%E9%99%A2%E8%AF%BE%E7%A8%8B%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/532191/%E5%B9%BF%E8%A5%BF%E5%B9%BF%E4%B8%9C%E5%B9%B2%E9%83%A8%E5%AD%A6%E9%99%A2%E8%AF%BE%E7%A8%8B%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
class Runner {
    constructor() {
        this.runner = null
    ...
    initAjaxHooker() {
        ...
        ajaxHooker.hook(request => {
            if (request.url.includes('AntiForgeryToken')) {

                request.response = res => {
                    const json = JSON.parse(res.responseText)
                    console.log("AntiForgeryToken：");
                    console.log(json);
                    window.AntiForgeryToken = json
                    // res.responseText += 'test';
                };
            } else if (request.url.includes('api/Home/Play') && !request.url.includes('Jwplay') && !request.url.includes('JY')) {
                request.response = res => {
                    const json = JSON.parse(res.responseText)
                    console.log("api/Home/Play：");
                    console.log(json);
                    window.PlayInfo = json
                    // res.responseText += 'test';
                };
            } else if (request.url.includes('api/Home/PlayJwplay')) {

                request.response = res => {
                    const json = JSON.parse(res.responseText)
                    console.log("api/Home/PlayJwplay：");
                    console.log(json);
                    window.VideoInfo = json
                    // res.responseText += 'test';
                };
            } else if (request.url.includes('api/Exam/Exam') && !request.url.includes('ExamReview')) {
                request.response = res => {
                    const json = JSON.parse(res.responseText)
                    console.log("api/Exam/Exam：");
                    console.log(json);
                    window.ExamData = json
                    // res.responseText += 'test';
                };
            } else if (request.url.includes('api/CourseProcess/SingleProcess')) {
                //拦截请求，并返回指定数据
                if(window.VIPRunning){
                    request.abort = true;
                    request.response = res => {
                        console.log("fake res");
                        res.responseText = '{"BrowseScore":66.66,"Content":"记录成功！"}';
                        console.log(res.responseText)
                    };
                }

            } else if (request.url.includes('api/Exam/ExamReview')) {
                request.response = res => {
                    console.log("ExamReview res");
                    const json = JSON.parse(res.responseText)
                    console.log(json);
                    window.ExamReviewInfo = json
                    try {
                        Exam.processAnswerData(window.ExamReviewInfo.Data)
                    }catch (e) {
                        console.error(e);
                    }
                };
    ...
    async runVIP() {
        try {
            ...
            if (window.isFinish) {
                ...
                Swal.fire({
                    title: "未完成！",
                    text: "请刷新页面继续，单次约刷取十分钟！一直出现此提示，请联系进行维护！",
                    icon: 'info',
                    confirmButtonText: '确定',
                    allowOutsideClick: false,
                    timerProgressBar: true,
    ...
    async run() {
        ...
        try {
            try {
                ...
                const maxMove = dragContainer.width() - handler.outerWidth() + 50;

                const mousedownEvent = $.Event('mousedown', {pageX: 0});
                handler.trigger(mousedownEvent);

                const mousemoveEvent = $.Event('mousemove', {pageX: maxMove});
...
class Utils {
    ...
    static flag = 'gdgx_gbzx_VIP'
    static js_Flag = 'gdgx_gbzx_jsCode'
    static vipText = '高级功能已启用！(广西广东干部中心)'
    ...
    static async validateCode(data) {
        try {
            console.log(data);
            let info = window.PlayInfo?.Data?.UserId
            if (!info) {
                throw new Error("无效的账号信息！")
            }
            data.bindInfo = "UserId_" + info
            data.website = "67de45ea816a3ff63b2a933d"
            console.log(data);
