// ==UserScript==
// @name         [Test] KIPSutian-autoplay
// @namespace    aiuanyu
// @version      4.44b-桌面 Chrome 載入進度也要按 modal
...
// @license      GNU GPLv3
// @downloadURL https://update.greasyfork.org/scripts/531909/%5BTest%5D%20KIPSutian-autoplay.user.js
// @updateURL https://update.greasyfork.org/scripts/531909/%5BTest%5D%20KIPSutian-autoplay.meta.js
// ==/UserScript==
...
(function () {
  ...
  function showInteractionPromptOverlay(callback) {
    ...
    interactionBox.innerHTML = '若欲繼續放送進度，請點一下畫面<br>Nā beh kè-sio̍k hòng-sàng tsìn-tōo, tshiánn tiám tsit-ê uē-bīn';
  ...
  function initialize() {
    ...
    if (!isMobile) { // 只有桌面才需要判斷
        const userAgent = navigator.userAgent;
        // 檢查是否包含 Chrome，但不包含 Edge (Edg/)
        // 注意：這可能也會包含其他 Chromium 核心瀏覽器，但通常行為類似
        if (userAgent.includes("Chrome")) { // 只要包含 Chrome 字樣就視為需要互動 (包含 Edge)
            isDesktopChromiumBased = true;
        }
    }
    console.log(`[自動播放] 初始化腳本 v4.43b ... isMobile: ${isMobile}, isDesktopChromiumBased: ${isDesktopChromiumBased}`);
    GM_addStyle(
    ...
    if (urlParams.has(LOAD_PROGRESS_PARAM)) {
      ...
      console.log(`[自動播放] 偵測到 ${AUTOPLAY_PARAM} 參數 (內部自動跳頁)，準備自動啟動...`);
    ...
    if (loadFromProgress || isInternalAutoNext) {
      ...
      const waitForContentAndStart = () => {
        ...
        if (contentExists) {
          ...
          const requiresInteraction = isMobile || (isDesktopChromiumBased && loadFromProgress); // ** 使用 loadFromProgress **

          if (requiresInteraction) {
            // ** 修改日誌訊息，更清晰 **
            console.log(`[自動播放] 需要使用者互動以啟動播放 (isMobile: ${isMobile}, isDesktopChromiumBased: ${isDesktopChromiumBased}, triggeredByURL: ${loadFromProgress})，顯示提示。`);
            const interactionCallback = () => initiateAutoPlayback(startIndexFromUrl);
            showInteractionPromptOverlay(interactionCallback);
          } else {
            // 不需要互動的情況 (例如：桌面 Firefox，或桌面 Chrome 但非外部 URL 觸發，或內部自動跳頁)
            // ** 修改日誌訊息，更清晰 **
            console.log(`[自動播放] 不需要使用者互動 (isMobile: ${isMobile}, isDesktopChromiumBased: ${isDesktopChromiumBased}, triggeredByURL: ${loadFromProgress}, isInternalAutoNext: ${isInternalAutoNext})，直接啟動播放。`);
