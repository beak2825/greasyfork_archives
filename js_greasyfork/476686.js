// ==UserScript==
// @name          네이버 맨 끝 게시물 열람
// @namespace     네이버 맨 끝 게시물 열람
// @include       *cafe.naver.com/*
// @version       0.1
// @description   Ctrl + Shift + E를 누르면 "?iframe_url="을 추가합니다.
// @icon          data:image/jpeg;base64,
// @author        mickey90427 <mickey90427@naver.com>
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/476686/%EB%84%A4%EC%9D%B4%EB%B2%84%20%EB%A7%A8%20%EB%81%9D%20%EA%B2%8C%EC%8B%9C%EB%AC%BC%20%EC%97%B4%EB%9E%8C.user.js
// @updateURL https://update.greasyfork.org/scripts/476686/%EB%84%A4%EC%9D%B4%EB%B2%84%20%EB%A7%A8%20%EB%81%9D%20%EA%B2%8C%EC%8B%9C%EB%AC%BC%20%EC%97%B4%EB%9E%8C.meta.js
// ==/UserScript==

(function() {
    'use strict';

    document.addEventListener('keydown', (event) => {
        if (event.ctrlKey && event.shiftKey && event.key === 'E') {
            event.preventDefault();
            appendIframeUrl();
        }
    });

    function appendIframeUrl() {
        const currentUrl = window.location.href;
        if (currentUrl.includes('?iframe_url=')) {
            return;
        }
        const targetUrl = currentUrl + '?iframe_url=/ArticleList.nhn?%26search.page=999';
        window.location.href = targetUrl;
    }
})();