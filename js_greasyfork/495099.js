...
// @run-at       document-end
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/495099/%F0%9F%92%8A%E5%A6%96%E7%81%AB%E7%BD%91%E5%A2%9E%E5%BC%BA%E6%8F%92%E4%BB%B6.user.js
// @updateURL https://update.greasyfork.org/scripts/495099/%F0%9F%92%8A%E5%A6%96%E7%81%AB%E7%BD%91%E5%A2%9E%E5%BC%BA%E6%8F%92%E4%BB%B6.meta.js
// ==/UserScript==
...
(function() {
  ...
  */
  ...
  const a3style =
    ...
    // ikun风格
const spanstyleikun =
  "color: #FFFFFF; font-weight: bold; padding: 6px 12px; font-size: 10px; background: #407088; border-radius: 5px; box-shadow: -2px -2px 3px #9ebdcd, 2px 2px 3px #9ebdcd;";
    ...
    if (postPage.includes(window.location.pathname) && isAddNewPostUBB) {
      ...
      bookContent.insertAdjacentHTML(
        ...
        <div class='more_ubb_tools' style='display: none'>
          ...
          <div class='btBox'>
            <div class='bt2'>
              <a style='width:25%' href='https://695402.xyz/ppx' target="_blank">皮皮虾解析</a>
    ...
    if (
      ...
      form.insertAdjacentHTML(
        ...
        <div class="ubb_wrap" style="text-align: center;overflow: hidden;">
          <span id='ubb_url' style="${a2styleikun}">链接</span>
          <span id='ubb_img' style="${a2styleikun}">图片</span>
          
          <span id='ubb_movie' style="${a2styleikun}">视频</span>
          <span id='ubb_nzgsa' style="${a2styleikun}">该死</span>
           <span id='ubb_bjdy' style="${a2styleikun}">卧槽</span>
           <span id='ubb_xngg' style="${a2styleikun}">不信</span>  
            <span id='ubb_tbs' style="${a2styleikun}">大叫</span>          
            <span id='ubb_jb' style="${a2styleikun}">举报</span>                          
          <br>                  
          <span id='ubb_text' style="${a2styleikun}">半角</span>
          <span id='ubb_br' style="${a2styleikun}">换行</span>
          <span id='ubb_b' style="${a2styleikun}">加粗</span>
          <span id='ubb_color' style="${a2styleikun}">彩字</span>
            <span id='ubb_lyltc' style="${a2styleikun}">天才</span>               
            <span id='ubb_ls' style="${a2styleikun}">乐色</span>      
             <span id='ubb_sscd' style="${a2styleikun}">插兜</span>           
            <span id='ubb_ngm' style="${a2styleikun}">坤坤</span>       
           <br>                      
          <span id='ubb_i' style="${a2styleikun}">斜体</span>                           
          <span id='ubb_u' style="${a2styleikun}">下划</span>
          <span id='ubb_strike' style="${a2styleikun}">删除</span>
          <span id='ubb_hr' style="${a2styleikun}">分割</span>
          <span id='ubb_sms' style="${a2styleikun}">短信</span>
          <span id='ubb_call' style="${a2styleikun}">拨号</span>
          <span id='ubb_now' style="${a2styleikun}">时间</span>
          <span id='ubb_codo' style="${a2styleikun}">倒数</span>
          <br>
          <a href='https://yaohuo.me/tuchuang/' target="_blank" style="${a2styleikun}">妖火图床</a>
          <a href='https://695402.xyz/ppx' target="_blank" style="${a2styleikun}">皮皮</a>
          <a href='https://695402.xyz/bili' target="_blank" style="${a2styleikun}">b站</a>
          <a href='https://695402.xyz/dy' target="_blank" style="${a2styleikun}">抖音</a>
          <a href='https://695402.xyz/ks' target="_blank" style="${a2styleikun}">快手</a>
          <a href='https://www.uhsea.com' target="_blank" style="${a2styleikun}">文件柜</a>
          <a href='https://urlify.cn/' target="_blank" style="${a2styleikun}">短链生成</a>
        </div>
        <hr>
        `
      );
  ...
  function handleAddReplyFace() {
    if (
      ...
      sendmsg.insertAdjacentHTML(
        "afterend",
        `<span 
          style="${a2styleikun}margin-left: 2px; display:${
  ...
  function handleEventListener(id, textarea, ubb, offset) {
    document.getElementById(id)?.addEventListener("click", (e) => {
      if (id === "ubb_nzgsa") {
        ...
        insertText(
          textarea,
          "[audio=X]https://aweme.snssdk.com/aweme/v1/play/?video_id=v0300fg10000ccd0glrc77ua2dbian5g&ratio=1080p[/audio]",
          0
        );
        // } else {
        //   alert("不要无意义灌水啦！");
