// ==UserScript==
// @name         agar.bio - Draw mouse line, weird mode, auto respawn, chat spam, dark theme, show mass and more!
// @description some weird stuff for the game lol
// @namespace    -
// @version      2
// @description  -
// @author       Stew
...
// @downloadURL https://update.greasyfork.org/scripts/428855/agarbio%20-%20Draw%20mouse%20line%2C%20weird%20mode%2C%20auto%20respawn%2C%20chat%20spam%2C%20dark%20theme%2C%20show%20mass%20and%20more%21.user.js
// @updateURL https://update.greasyfork.org/scripts/428855/agarbio%20-%20Draw%20mouse%20line%2C%20weird%20mode%2C%20auto%20respawn%2C%20chat%20spam%2C%20dark%20theme%2C%20show%20mass%20and%20more%21.meta.js
// ==/UserScript==
window.cells = []
var portlar = ['11005', '11022', '11023', '11024', '11025', '11004', '11026', '11030'];
var port = portlar[Math.floor(Math.random() * portlar.length)];
var CONNECTION_URL = "37.187.76.129:" + port;
(function(wHandle, wjQuery) {
        ...
        wHandle.onkeydown = function(event) {
            ...
            case 13:
                if (isTyping) {
                    ...
                    chattxt = chattxt.replace("5 Saniyede 1 kere yazabilirsin", "");
                    chattxt = chattxt.replace("www", "");
                    chattxt = chattxt.replace("Agario", "");
                    chattxt = chattxt.replace("agario", "");
                    chattxt = chattxt.replace("agarios", "");
                    chattxt = chattxt.replace("Agarios", "");
                    chattxt = chattxt.replace("Agarlove.Com", "");
                    chattxt = chattxt.replace("AgarLove.Com", "");
                    chattxt = chattxt.replace("New Address", "");
                    chattxt = chattxt.replace("agario.biz", "");
                    function getTime() {
                        var now = new Date()
                          , time = [now.getSeconds() + 3];
                        return time;
                    }
                    function getTime2() {
                        var now = new Date()
    ...
    function getServerList() {
        ...
        wjQuery.get("info.php", function(a) {
            ...
            for (var numplayer in numPlayers)
                wjQuery('#region option[value="' + numplayer + '"]').text(playerStat[numplayer] + " (" + numPlayers[numplayer] + " players)")
        }, "json")
    }
    function hideOverlays() {
        hasOverlay = false;
        wjQuery("#oluncereklam").hide();
    ...
    function showOverlays(arg) {
        ...
        setTimeout(reklamgoster, 0000);
        wjQuery("#oluncereklam").show();
    }
    function Ha() {
    ...
    function updateNodes(view, offset) {
        ...
        for (var i = 0; ; ) {
            ...
            if (-1 != nodesOnScreen.indexOf(nodeid) && -1 == playerCells.indexOf(node)) {
                ...
                document.getElementById("oluncereklam").style.display = "none";
    ...
    function drawGameScene() {
        ...
        if (0 != userScore) {
            ...
            scoreText.setValue(scorelng + ': ' + ~~(userScore / 100));
            c = scoreText.render();
            a = c.width;
            ctx.globalAlpha = .2;
    ...
    function skorkaydet() {
        var skor = "skor=" + ~~(userScore / 100) + "&oyuncuadi=" + userNickName;
        $.ajax({
            type: 'POST',
            url: 'skor.php',
            data: skor,
            success: function(cevap) {
                console.log("Add:" + ~~(userScore / 100) + cevap);
            }
        });
    }
    function drawLeaderBoard() {
        ...
        if (null != teamScores || 0 != leaderBoard.length)
            if (null != teamScores || showName) {
                ...
                var c = leaderboardlng;
                ctx.font = "30px Ubuntu";
                ctx.fillText(c, 100 - ctx.measureText(c).width / 2, 40);
                var b;
                if (null == teamScores) {
                    for (ctx.font = "20px Ubuntu",
                         b = 0; b < leaderBoard.length; ++b) {
                        c = leaderBoard[b].name || "agar.bio";
                        if (!showName) {
                            (c = "agar.bio");
                        }
