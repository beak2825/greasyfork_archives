...
// @downloadURL https://update.greasyfork.org/scripts/422616/%E6%B7%98%E5%AE%9D%E4%B9%B0%E5%AE%B6%E8%AE%A2%E5%8D%95%E5%AF%BC%E5%87%BA.user.js
// @updateURL https://update.greasyfork.org/scripts/422616/%E6%B7%98%E5%AE%9D%E4%B9%B0%E5%AE%B6%E8%AE%A2%E5%8D%95%E5%AF%BC%E5%87%BA.meta.js
// ==/UserScript==
...
function tbExportOrdersToCsv() {
    ...
    let csvHeader = ["订单号", "下单日期", "卖家", "商品明细", "单价", "数量", "实付金额", "状态"];
...
function tbParseOrder(order) {
    ...
    while (1) {
        ...
        let sellerPrefix = ".0.7:$order-" + orderNo + ".$" + orderNo + ".0.1:0.0.1.0.1";
        ...
        let amoutPrefix = ".0.7:$order-" + orderNo + ".$" + orderNo + ".0.1:1:0.$" + orderIndex + ".$4.0.0.2.0.1";

        let countQuery = order.querySelector("p[data-reactid='" + countPrefix + "']");
        let sellerQuery = order.querySelector("a[data-reactid='" + sellerPrefix + "']");
        ...
        let amoutQuery = order.querySelector("span[data-reactid='" + amoutPrefix + "']");
        if (orderIndex === 0) {
