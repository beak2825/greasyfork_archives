// ==UserScript==
// @name         MH: Valour Rift HUD Enhancer (edited by Chromatical)
...
// @downloadURL https://update.greasyfork.org/scripts/423884/MH%3A%20Valour%20Rift%20HUD%20Enhancer%20%28edited%20by%20Chromatical%29.user.js
// @updateURL https://update.greasyfork.org/scripts/423884/MH%3A%20Valour%20Rift%20HUD%20Enhancer%20%28edited%20by%20Chromatical%29.meta.js
// ==/UserScript==
...
function updateHUD(resultString) {
    ...
    let steps_Rem = document.getElementsByClassName("valourRiftHUD-stepsRemaining")[0].innerHTML
    var steps_2_eclispe = 0
    if((steps_Rem/stepsPerHunt).toFixed(2) <=1)
     {steps_2_eclispe = 0} else { steps_2_eclispe = (steps_Rem/stepsPerHunt).toFixed(2)};

    let floorNum = parseInt(document.getElementsByClassName("valourRiftHUD-stepsTaken-value")[0].innerText)
    let floorPct = parseInt(document.getElementsByClassName("valourRiftHUD-floorPercent")[0].innerText)
    let floorSet = 0
    let stepNum = 20
    let isEight = 0
    while (floorNum >= 0){
       floorNum = floorNum - stepNum;
       isEight ++;
    ...
    let steps2floor = (100-floorPct)/100 * stepNum
    var hunts2floor = 0
    if (steps_2_eclispe == 0)
       {hunts2floor = 'Catch Eclipse to proceed'} else {hunts2floor = (steps2floor/stepsPerHunt).toFixed(2)}

    let stepsString = 'You are averaging ' + stepsPerHunt + ' steps per hunt (' + steps_2_eclispe + ' hunts to Eclipse) (' + hunts2floor + ' hunts to next floor)';
