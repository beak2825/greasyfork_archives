...
// @match        *://*.qpic.cn/*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/449578/%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E6%BB%A1%E9%80%9F%E4%B8%8B%E8%BD%BD%E5%8A%A9%E6%89%8B%20QQ%E7%A9%BA%E9%97%B4%E7%9B%B8%E5%86%8C%E4%B8%8B%E8%BD%BD%E7%A5%9E%E5%99%A8.user.js
// @updateURL https://update.greasyfork.org/scripts/449578/%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E6%BB%A1%E9%80%9F%E4%B8%8B%E8%BD%BD%E5%8A%A9%E6%89%8B%20QQ%E7%A9%BA%E9%97%B4%E7%9B%B8%E5%86%8C%E4%B8%8B%E8%BD%BD%E7%A5%9E%E5%99%A8.meta.js
// ==/UserScript==
//
...
(() => {
    ...
    var 智能下载 = (url, filename = '') => {
        ...
        if ('使用IFRAME下载') {
            ...
            document.body.appendChild(
                新元素('<iframe style="visibility: hidden"></iframe>', {
    ...
    */
    ...
    var 照片列を取 = () => {
        ...
        return [...document.querySelectorAll('.j-pl-photoitem,.j-box-img')].map(
            (e, i) => ({
                src: e.querySelector('.j-pl-photoitem-img,.j-img').src,
                url: 高清地址を取(
                    e.querySelector('.j-pl-photoitem-img,.j-img').src
                ),
                filename:
                    (
                        相册名 +
                        ...
                        // (1 + i) + '-' +
                        试取内容(
                            e.querySelector('.j-pl-photoitem-title,span[title]')
                        ) +
                        '-' +
                        试取内容(
                            e.querySelector('.j-pl-photoitem-desc,j-photo-desc')
                        )
                    )
                        .replace(/\/|\?|\*|\:|\||\\|\<|\>|\r|\n/, '_')
                        .substr(0, 50) + '.jpg',
    ...
    var 下载当页 = (globalThis.下载当页 = () => {
        ...
        if (列有效)
            return 照片列を取().forEach(({ url, filename }) =>
                智能下载(url, filename)
            );
        // 自动滚动重试
    ...
    var 生成下载界面 = () => {
        ...
        var 下载按钮 = 新元素('<div></div>', {
            ...
            innerHTML:
                '<a class="c-tx2 bg3 bor gb-btn-nor" href="javascript:globalThis.下载当页()">下载当页</a>',
            title: '请滚动到底以确保当前页面所有照片都已加载',
            //onclick: 下载当页,
        });
        [
            ...document.querySelectorAll('.photo-op-item.photos-download'),
        ].forEach((e) => e.remove());
    ...
    // 被下载
    if (
        location.host.match(/.*\.photo\.store\.qq\.com/) ||
