...
// @grant          GM_info
// @grant          GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/490423/AI%20Prediction.user.js
// @updateURL https://update.greasyfork.org/scripts/490423/AI%20Prediction.meta.js
// ==/UserScript==
...
const openDown = async (url, e, name, img) => {
  ...
  try {
    ...
    const r = await GM.xmlHttpRequest({
      url: 'http://10.240.50.34:1099/', method: 'POST', headers: {
...
function setPredictionLabel(data, imageDom) {
  let St = data?.detection_classes;
  if (!St || 0 === St?.length) {
  ...
  data.detection_classes.forEach(function(item, index) {
    let pn = {
      x: data.detection_boxes[index][1] / Ii,
      y: data.detection_boxes[index][0] / si,
      ex: data.detection_boxes[index][3] / Ii,
      ey: data.detection_boxes[index][2] / si
    };
    const ki = item ? item + '(' + String(Math.round(100 * data.detection_scores[index])) + '%)' : '';
    Mt = $('<div class="imageLabel-imgdrop"><span style="font-size:12px;zoom:0.9" class="imageLabel-imgdrop-font">' + (ki || '') + '</span></div>'),
      Mt.css({
        left: 100 * (pn.ex - pn.x > 0 ? pn.x : pn.ex) + '%',
        top: 100 * (pn.ey - pn.y > 0 ? pn.y : pn.ey) + '%',
        width: 100 * Math.abs(pn.ex - pn.x) + '%',
        height: 100 * Math.abs(pn.ey - pn.y) + '%'
      });
