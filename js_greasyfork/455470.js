...
*/
// a couple of lines were modified...
...
const byteArrayToString = function(bytes) {
    return byteArrayToString.decoder.decode(bytes);
};

byteArrayToString.decoder = new TextDecoder();
...
class WailParser extends BufferReader {
    ...
    _readSection() {
        ...
        if (id != SECTION_DATACOUNT) {
            ...
            for (let missingId = 0; missingId < id; missingId++) {
                ...
                if (thisSectionRequired && !(thisSectionRequired & this._parsedSectionsState)) {
                    switch (missingId) {
                        case SECTION_TYPE:
                    ...
                    this._parsedSectionsState |= thisFlag;
        ...
        */
        ...
        switch(id) {
            case SECTION_TYPE: {
                const results = this._parseTypeSection();
                const data = { id };

                if (results) data.results = results;

                this._parsedSections.push(data);
                break;
            }
            case SECTION_IMPORT: {
                const results = this._parseImportSection();
                const data = { id };
            ...
            case SECTION_FUNCTION: {
                const results = this._parseFunctionSection();
                const data = { id };
            ...
            case SECTION_GLOBAL: {
                const results = this._parseGlobalSection();
                const data = { id };
            ...
            case SECTION_EXPORT: {
                const results = this._parseExportSection();
                const data = { id };
            ...
            case SECTION_START: {
                const results = this._parseStartSection();
                const data = { id };
            ...
            case SECTION_ELEMENT: {
                const results = this._parseElementSection();
                const data = { id };
            ...
            case SECTION_CODE: {
                const results = this._parseCodeSection();
                const data = { id };
            ...
            case SECTION_DATA: {
                const results = this._parseDataSection();
                const data = { id };
            ...
            default:
                this.readBytes(this.readVarUint32());

                return;
                throw new Error("Attempted to parse unhandled section");
        }

        this._parsedSectionsState |= 1 << id;
    }
    ...
    _addTypeSection() {
        ...
        for (let i = 0; i < newEntries.length; i++) {
            ...
            if (returnType === VALUE_TYPE_BLOCK) returnType = null;
    ...
    _parseTypeSection() {
        ...
        for (let typeIndex = 0; typeIndex < oldCount; typeIndex++) {
            ...
            let returnType = VALUE_TYPE_BLOCK;
            ...
            if (returnType === VALUE_TYPE_BLOCK) returnCount = 0;
    ...
    _parseImportSection() {
        ...
        for (let importIndex = 0; importIndex < oldCount; importIndex++) {
            ...
            results.push({
                kind: kindStr[kind],
                moduleStr: byteArrayToString(moduleStr),
                fieldStr: byteArrayToString(fieldStr),
                type
            })
        }
    ...
    _parseGlobalSection() {
        ...
        for (let globalIndex = 0; globalIndex < oldCount; globalIndex++) {
            ...
            results.push({
                contentType: valueTypeStr[contentType],
                mutable: !!mutability
            });
        }
    ...
    _parseCodeSection() {
        ...
        for (let i = 0; i < oldCount; i++) {
            ...
            results.push(this._readFunction(reader, funcIndex));
        }
    ...
    _readFunction(reader, funcIndex) {
        ...
        while (bodyReader.inPos < bodyReader.inBuffer.length) {
            instructions.push(this._readInstruction(bodyReader).parsed);
    ...
    _readInstruction(reader) {
        ...
        switch (opcode) {
            ...
            case OP_MEMORY_SIZE:
            case OP_MEMORY_GROW:
                immediates.push(reader.readUint8());
                break;
            case OP_BR:
            case OP_BR_IF:
            case OP_GET_LOCAL:
            ...
            case OP_I32_CONST:
            case OP_I64_CONST:
                immediates.push(reader.readVarUint32());
                break;
            case OP_GET_GLOBAL:
            case OP_SET_GLOBAL:
                reader.commitBytes();
                ...
                immediates.push(oldTarget);
                newTarget = this._getAdjustedGlobalIndex(oldTarget);

                reader.copyBuffer(VarUint32ToArray(newTarget));
                break;
            case OP_F32_CONST:
                immediates.push(new Float32Array(reader.readBytes(4).buffer)[0]);
                break;
            case OP_F64_CONST:
                immediates.push(new Float64Array(reader.readBytes(8).buffer)[0]);
                break;
            case OP_I32_LOAD:
            case OP_I64_LOAD:
            case OP_F32_LOAD:
            ...
            case OP_I64_STORE16:
            case OP_I64_STORE32:
                immediates.push(reader.readVarUint32(), reader.readVarUint32());
                break;
            case OP_BR_TABLE:
                const count = reader.readVarUint32();
                const entries = Array(count);
                for (let i = 0; i < count; i++) {
                    entries[i] = reader.readVarUint32();
                }

                immediates.push({count, entries, leftover: reader.readVarUint32()});
                break;
            case OP_CALL:
                reader.commitBytes();
            ...
            case OP_CALL_INDIRECT:
                immediates.push(reader.readVarUint32(), reader.readUint8());
                break;
            ...
            case OP_BULK_MEMORY:
                ...
                switch (arg) {
                    ...
                    case ARG_TABLE_COPY:
                        immediates.push(reader.readUint8(), reader.readUint8());
                        break;
        ...
        fullInstruction.parsed = {opcode, immediates};
