// ==UserScript==
// @name         Craftworks Visuals
// @version      1.0.25
// @description  Reorders Craftworks
// @author       ClientCoin
...
// @grant        none
// @namespace    welcometothejunglewehavefunand540games
// @downloadURL https://update.greasyfork.org/scripts/528989/Craftworks%20Visuals.user.js
// @updateURL https://update.greasyfork.org/scripts/528989/Craftworks%20Visuals.meta.js
// ==/UserScript==
...
function mainEdit() {
    ...
    //console.log("%c[Script] Formatting Craftworks display...", "color: cyan; font-weight: bold;");

    const locName = location.hash.slice(location.hash.search(/[^#!\/]/), location.hash.search(/.php/))
    if (locName == "craftworks") {
        formatCraftworksItems();
    } else {
        return;
    }
}

function formatCraftworksItems() {
    //console.log("Starting Craftworks Item Formatter...");

    // Locate the Craftworks items container
    let craftworkContainer = document.querySelector(".list-block.searchbar-found.cwitems.sortable");

    if (!craftworkContainer) {
        console.log("Craftworks item container not found. Exiting script.");
        return;
    }

    //console.log("Craftworks item container found.");

    // Get all `li` elements (each represents a craft item)
    let craftItems = craftworkContainer.querySelectorAll("li");

    if (craftItems.length === 0) {
        console.log("No craft items found. Exiting script.");
        return;
    }
    ...
    craftItems.forEach((item, index) => {
        try {
            ...
            let titleElem = item.querySelector(".item-title");
            if (!titleElem) {
                console.warn(`Item ${index + 1}: No title element found.`);
                return;
            }

            let title1 = titleElem.innerText.replaceAll("\n", "&");
            const title = title1.split("&");
            //console.log(title);

            let originalColor = "white";
            let inventoryElem = [...titleElem.childNodes].find(node => node.nodeType === 1 && node.tagName === "SPAN");
            let inventoryColor = inventoryElem ? window.getComputedStyle(inventoryElem).color : originalColor;

            let craftWork = title[0] ? title[0] : "a";
            let inventory = title[1] ? title[1] : "";
            let material = title[2] ? title[2].replace("Out of: ", "") : "";
            let materialColor = material ? "red" : originalColor;
            if (parseInt(inventory.match(/\d+/)?.[0] || '0', 10) === 0) inventoryColor = 'grey';


            if (material) {
                material = material.split(", ").map(item => {
                    let itemID = Object.keys(itemData).find(id => itemData[id] === item.trim());
                    let itemIDoutput = itemID ? `<a href='item.php?id=${itemID}'>` : "";
                    if (itemID && imgData[itemID]) {
                        let imgSrc = `/img/items/${imgData[itemID]}`;
                        // if (!imgSrc.endsWith(".PNG")) {imgSrc += ".png";}


                        itemIDoutput = `${itemIDoutput}<img src='${imgSrc}' class='itemimgsm' alt='${item}' title='${item}'></a>`;
                    } else {
                        itemIDoutput = `${itemIDoutput}<div style='color: red;'>${item}</div></a>`;
                    }
                    return itemIDoutput;
            ...
            //console.log(`Item ${index + 1} ${craftWork} count: ${inventory}, color: ${inventoryColor}`);
            //if (material) console.log(`Item ${index + 1} ${craftWork} is out of: ${material}`);

            let formattedText = `
                    <span style="display: inline-block; width: 30%; text-align: left; font-size: 11pt;">${craftWork}</span>
                    <span style="display: inline-block; width: 30%; text-align: left; font-size: 11pt; color: ${inventoryColor};">${inventory}</span>
                    <span style="display: inline-block; width: 40%; text-align: left; font-size: 11pt; color: ${materialColor};">${material}</span>
                `;
            //console.log(`Formatted text for item ${index + 1}: ${formattedText}`);

            titleElem.outerHTML = `<span style="font-family: monospace; display: flex; align-items: center; color: ${originalColor}; width: 100%;">${formattedText}</span>`;
        } catch (error) {
            console.error(`Error processing item ${index + 1}:`, error);
        }
    });

    console.log("Craftworks Item Formatter completed.");
}
...
$(document).ready( () => {
    ...
    const observer = new MutationObserver( mutation => {if (mutation[0]?.attributeName == "data-page") mainEdit()} )
    mainEdit();
