(function() {
    ...
    search = function(){
        if(tempTime >= 800){
            addMessage("课程完成"+tempTime+"/800，10秒后回主页");
            setTimeout(function(){
    ...
    function startTimer() {
        wait = setInterval(function (){
            ...
            if(window.location.href.split('?')[0] === 'https://ifang.hnteacher.net/HTML/WorkShops/workshop-courses.html'){
            ...
            else if(window.location.href.split('?')[0] === 'https://ifang.hnteacher.net/HTML/WorkShops/workshop-course-details.html'){
                addMessage("章节列表");
                saveChapterUrl();
                tipsWin().then((result) => {
                    if(!result){
            ...
            else if(window.location.href.split('?')[0] === 'https://ifang.hnteacher.net/HTML/WorkShops/workshop-course-play.html'){
                addMessage("学习页");
                ...
                tipsWin().then((result) => {
                    if(!result){
                        setTimeout(function(){
                            addMessage("启动");
                            let cTime = parseInt(document.querySelector("#_ctime").innerText) + 2;
                            listenTime(cTime);
                        },1000);
                    }else{
                        addMessage("取消");
    ...
    function xuanKe(index,count){
        if(index < count){
            if(!document.querySelector("#courseList").getElementsByTagName('input')[0].checked){
