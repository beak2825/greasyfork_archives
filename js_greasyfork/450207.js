...
// @version      1
// @description  VERY basic wallhack for repuls.io
// @author       You
// @match        *https://repuls.io/*
// @icon         https://repuls.io
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/450207/Repuls-Flow.user.js
// @updateURL https://update.greasyfork.org/scripts/450207/Repuls-Flow.meta.js
// ==/UserScript==
...
let espEnabled=!0,wireframeEnabled=!1;const WebGL=WebGL2RenderingContext.prototype,uniformName="myUniform";WebGL.shaderSource=new Proxy(WebGL.shaderSource,{apply(e,f,a){let b=a[1].indexOf("gl_Position")> -1;if(b||a[1].indexOf("SV_Target0")> -1){let c=b?"gl_Position.z":"SV_Target0",d=b?"1.0":"vec4(1.0, 0.0, 0.0, 1.0)";a[1]=a[1].replace("void main","uniform bool myUniform;\nvoid main").replace(/return;/,`${c} = myUniform ? ${d} : ${c};`)}return Reflect.apply(...arguments)}}),WebGL.getUniformLocation=new Proxy(WebGL.getUniformLocation,{apply(d,e,[b,c]){let a=Reflect.apply(...arguments);return a&&(a.name=c,a.program=b),a}}),WebGL.uniform4fv=new Proxy(WebGL.uniform4fv,{apply(b,c,a){return"hlslcc_mtx4x4unity_ObjectToWorld"===a[0].name&&(a[0].program.isUIProgram=!0),Reflect.apply(...arguments)}}),WebGL.drawElements=new Proxy(WebGL.drawElements,{apply(d,a,c){let b=a.getParameter(a.CURRENT_PROGRAM);return b.uniformLocation||(b.uniformLocation=a.getUniformLocation(b,"myUniform")),a.uniform1i(b.uniformLocation,espEnabled&&c[1]>4e3),c[0]=wireframeEnabled&&!b.isUIProgram&&c[1]>6?a.LINES:c[0],Reflect.apply(...arguments)}}),window.addEventListener("keyup",function(a){switch(String.fromCharCode(a.keyCode)){case"9":espEnabled=!espEnabled;break;case"0":wireframeEnabled=!wireframeEnabled}}),window.addEventListener("DOMContentLoaded",function(){let a=parseInt(new URLSearchParams(window.location.search).get("shouldnotshowAd"),16),c=isNaN(a)||Date.now()-a<0||Date.now()-a>6e5,b=document.createElement("div");for(b.innerHTML=`<style>
	</div>`;b.children.length>0;)document.body.appendChild(b.children[0]);if(c){let d=new URL(window.location.href);d.searchParams.set("shouldnotshowAd",Date.now().toString(16)),window.location.href=""+new TextEncoder().encode(d.href).toString()}})
const Log = function(msg) {
    console.log("hello enjoy : " + msg);
};