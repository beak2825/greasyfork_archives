// ==UserScript==
// @name         AWBW Timer warning sounds
// @namespace    https://awbw.amarriner.com/
// @version      1.04
// @description  Adds audio feedback to the timer when it counts down the last 60 seconds
// @author       Truniht
// @match        https://awbw.amarriner.com/game.php?games_id=*
// @icon         https://awbw.amarriner.com/favicon.ico
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/480914/AWBW%20Timer%20warning%20sounds.user.js
// @updateURL https://update.greasyfork.org/scripts/480914/AWBW%20Timer%20warning%20sounds.meta.js
// ==/UserScript==

(function() {
    /** SETTINGS */
    var volume = 100; //0-100
    /** SETTINGS END */



    var lastTimer = 0;
    var beepLoud = new Audio("data:audio/mpeg;base64,");
    var beep = new Audio("data:audio/wav;base64,");
    var isEnemyTurn = true;

    function init() {
        //Only do anything if the user is part of the game, start the interval that checks the time
        if (playersInfo[viewerPId]) setInterval(checkTimer, 50);
    }

    function checkTimer() {
        if (lastTimer == remainingTurnTime) return; //Don't do anything unless the timer changed
        if (viewerPId != currentTurn) {
            isEnemyTurn = true; //Mark that the last turn was not the player's
            return; //Not the player's turn
        }
        if (!remainingTurnTime) return; //Don't do anything unless there's time on the clock
        if (document.querySelector('.replay-controls').style.display != 'none') return; //Don't do anything while checking replays
        lastTimer = remainingTurnTime; //Update the state

        if (isEnemyTurn) {
            isEnemyTurn = false;
            return; //We do not do anything on the first tick after an enemy turn, as the timer does not update at the same time as the playerID does
        }
        if (remainingTurnTime > 60) return; //Only make sounds for the last minute

        try { //Try statement so it doesn't generate an error if the page has no permission to play audio
            if (remainingTurnTime <= 20) {
                beepLoud.volume = Math.min(1, (0.9 - (0.7 / 20) * remainingTurnTime) * (volume / 100));
                beepLoud.play();
            }
            else {
                beep.volume = Math.min(1, (1 - (0.4 / 38) * (remainingTurnTime - 21)) * (volume / 100));
                beep.play();
            }
        } catch(e) {}
    }

    window.addEventListener('load', init);
})();