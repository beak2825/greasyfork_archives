...
//
// Copyright 2010-2014, Michael Devore
...
// @version       4.6
// @downloadURL https://update.greasyfork.org/scripts/5439/HowlsOfOutrage.user.js
// @updateURL https://update.greasyfork.org/scripts/5439/HowlsOfOutrage.meta.js
// ==/UserScript==
//
// version 4.6: post favorite fix
// version 4.5: fix for MetaFilter redesign, add https support
// version 4.2: fixes a problem that Greasemonkey 2.1 has with GM_xmlhttpRequest and Windows
...
function howlMain()
{
	...
	favWork.id = 'howlDiv';
	favWork.style.display = "none";
	document.getElementsByTagName('body')[0].appendChild(favWork);
	...
	xpath = "//DIV[@class='copy']/SPAN[starts-with(text(),'posted by') and (@class='smallcopy' or @class='smallcopy postbyline')]/SPAN[starts-with(@id,'favcnt')]/A";
	postNodes = document.evaluate(
		xpath,
		document,
		null,
...
*/
	...
	request.open("GET", favURL, true);
//	request.setRequestHeader("User-Agent", "Mozilla/5.0");
	request.setRequestHeader("Accept", "text/xml");
	try
	{
		request.addEventListener("readystatechange", favCheckPageReady, false);
	}
	catch (e)
	{
		if (e.message.indexOf("NS_ERROR_ILLEGAL_VALUE" >= 0))
		{
			// readystatechange event not unsupported for this version browser
			request.onreadystatechange = function() {
				if (request.readyState == 4 && request.status == 200)
				{
					if (request.responseText)
					{
						processFav(request.responseText);
					}
				}
			};
		}
...
function favCheckPageReady()
{
	if (this.readyState == 4 && this.status == 200)
	{
		if (this.responseText)
		{
			processFav(this.responseText);
		}
	}
}
...
//	var xpath = "/DIV[@class='copy']/A";
	var xpath = "//DIV[@id='howlDiv']//DIV[@class='copy']/A";
