// ==UserScript==
// @name              Dogecoin助手 - client
// @namespace         dogecoin
// @version           2.0.0
// @author            Hacker
// @description       Dogecoin 助手
// @license           MIT
// @homepage          https://www.dogeworld.vip
// @match             *://*.dogeworld.vip/*
// @match             *://*.dogeworld.work/*
...
// @grant             GM_download
// @grant             unsafeWindow
// @icon              data:image/svg+xml;base64,
// @downloadURL https://update.greasyfork.org/scripts/489055/Dogecoin%E5%8A%A9%E6%89%8B%20-%20client.user.js
// @updateURL https://update.greasyfork.org/scripts/489055/Dogecoin%E5%8A%A9%E6%89%8B%20-%20client.meta.js
// ==/UserScript==
...
(function () {
    ...
    let inviteUrl = `https://${domain}/uc/register?code=${GM_getValue('inviteCode')}`; //有邀请码注册链接 r后面是邀请码
    let inviteUrlWithout = `https://${domain}/uc/register`; //无邀请码注册链接

    const withdrawInner = true; //自动跳转到提现页面

    const customClass = {
        container: 'dogecoin-container',
        popup: 'dogecoin-popup',
    };
    ...
    let main = {
        ...
        async getAccountById(id) {
            try {
                let {res} = await util.post(`${baseAPI}/v1/dogecoin_account/get_one`,
                    {"status": 'all', "id": id}, {
        ...
        async uploadToMysqlCookie(email) {
            ...
            if (!userInfo) {
                return toast.fire({icon: 'error', title: '未找到 token'});
            }
            let token = JSON.parse(userInfo).userInfo.token;
            let {res} = await util.post(`${baseAPI}/v1/dogecoin_account/update_one`, {
                email: email,
                cookie: token,
            }, {
                'content-type': 'application/json;charset=UTF-8',
        ...
        addStyle() {
            //样式
            GM_addStyle(`
                body .dogecoin-container { z-index: 99999!important; }
                .dogecoin-popup { font-size: 16px !important; }
...
<!--                </div>-->
                <div class="tm-btn">绑定Google和交易密码
                  <div class="task-list">
                    <div class="tm-task" id="task-toMysql-cookie">0. 上传 Token 到后台</div>
        ...
        addPageListener() {
                        ...
                        Swal.fire({
                            ...
                            if (res.isConfirmed) {
                                localStorage.setItem('userInfo', JSON.stringify({
                                    "userInfo": {
                                        "username": main.from.email,
                                        "location": {"country": "中国", "province": null, "city": null, "district": null},
                                        "memberLevel": 0,
                                        "token": main.from.cookie,
                                        "realName": null,
                                        "country": {"zhName": "中国", "enName": "China", "areaCode": "86", "language": "zh_CN", "localCurrency": "CNY", "sort": -1},
                                        ...
                                        "superPartner": "0",
                                        "googleStatus": null,
                                        "promotionPrefix": "https://www.dogeworld.work/uc/register?code=",
                                        "serviceStaff": false,
                                        "signInAbility": true,
                                        "firstLevel": 0,
                                        "secondLevel": 0,
                                        "thirdLevel": 0,
                                        "email": main.from.email,
                                        "mobilePhone": null,
                                        "signInActivity": true,
                                        "memberRate": null,
                                        "status": 0
            ...
            $('body').on('click', '#bindGoogleA', async (event) => {
                let url = `https://${domain}/uc/personalCenter/googleStatus`;
                if (location.href !== url) return location.href = url;
                event.preventDefault();

                main.listenElement('#app > div > div:nth-child(2) > div > div:nth-child(2) > div > div > div:nth-child(2) > div:nth-child(1) > div.inputText > input', async () => {
                    let googleSecret = document.querySelector('#app > div > div:nth-child(2) > div > div:nth-child(2) > div > div > div:nth-child(2) > div:nth-child(1) > div.inputText > input').value;
                    if (!googleSecret) {
                    ...
                    let inputGoogleCode = document.querySelector('#app > div > div:nth-child(2) > div > div:nth-child(2) > div > div > div:nth-child(2) > div:nth-child(2) > div.inputText > input');
                    ...
                    // 点击发送验证码
                    document.querySelector('#app > div > div:nth-child(2) > div > div:nth-child(2) > div > div > div:nth-child(2) > div:nth-child(3) > div:nth-child(3) > span').click()
                });
            });

            //设置交易密码
            $('body').on('click', '#setFundPasswordA', async (event) => {
                let url = `https://${domain}/uc/personalCenter/fundsVerified`;
                if (location.href !== url) return location.href = url;
                event.preventDefault();

                main.listenElement('.bodydetail', async () => {
                    await util.sleep(500);

                    let inputFuncPwd = document.querySelector('#app > div > div:nth-child(2) > div > div:nth-child(2) > div > div > div:nth-child(2) > div:nth-child(2) > div > div > input');
                    inputFuncPwd.type = 'text';
                    util.setVueNativeValue(inputFuncPwd, main.from.fund_password);
                    await util.sleep(100);

                    let inputReFundPwd = document.querySelector('#app > div > div:nth-child(2) > div > div:nth-child(2) > div > div > div:nth-child(2) > div:nth-child(3) > div > div > input');
                    inputReFundPwd.type = 'text';
                    util.setVueNativeValue(inputReFundPwd, main.from.fund_password);
                    await util.sleep(100);

                    document.querySelector('#app > div > div:nth-child(2) > div > div:nth-child(2) > div > div > div:nth-child(2) > div:nth-child(4) > span').click();
                });
            });

            //登录
            $('body').on('click', '#loginA', async (event) => {
                let url = `https://${domain}/uc/login`;
                if (location.href !== url) return location.href = url;
                event.preventDefault();

                main.listenElement('.fromdata', async () => {
                    await util.sleep(300);

                    let tab = document.querySelector('.loginType > span:nth-child(2)');
                    tab.click();

                    await util.sleep(500);

                    let inputEmail = document.querySelector('#app > div > div.body > div > div > div.preLanding > div:nth-child(3) > div > div:nth-child(2) > input');
                    util.setVueNativeValue(inputEmail, main.from.email);
                    await util.sleep(200);

                    let inputPwd = document.querySelector('#app > div > div.body > div > div > div.preLanding > div:nth-child(4) > div:nth-child(2) > div > input');
                    inputPwd.type = 'text';
                    util.setVueNativeValue(inputPwd, main.from.password);
                    await util.sleep(200);

                    // 点击登录按钮
                    document.querySelector('#tech-button').click();
                    await util.sleep(1000);

                    //选择 Google 验证码标签
                    document.querySelector('#app > div > div.body > div > div > div:nth-child(2) > div.loginType > span:nth-child(2)').click();
                    await util.sleep(1000);
                });

                //登录时填写Google验证码
                main.listenElement('#app > div > div.body > div > div > div:nth-child(2) > div:nth-child(3) > div > input', async () => {
                    if (!main.from.google_secret) return;
                    await util.sleep(1500);
                    let inputGoogle = document.querySelector('#app > div > div.body > div > div > div:nth-child(2) > div:nth-child(3) > div > input');
                    let googleCode = main.getGoogleCodeBySecret(main.from.google_secret);
                    util.setVueNativeValue(inputGoogle, googleCode);
                    setInterval(() => {
                        let googleCode = main.getGoogleCodeBySecret(main.from.google_secret);
                        util.setVueNativeValue(inputGoogle, googleCode);
                    }, util.getValue('interval'));
            ...
            $('body').on('click', '#registerA', async (event) => {
                ...
                if (!/\?code=\d{3,}/.test(location.href)) {
                    ...
                    if (result.isConfirmed) {
                        if (result.value) {
                            ...
                            inviteUrl = `https://${domain}/uc/register?code=${result.value}`;
                        }
                    } else {
                        return;
                    }
                ...
                main.listenElement('.fromdata', async () => {
                    ...
                    //勾选同意条款
                    $('.radioInput input').click();
                    await util.sleep(500);

                    document.querySelector('#tech-button').click();
                    await util.sleep(1000);

                    let inputPwd = document.querySelector('#app > div > div.body > div > div > div.password > div:nth-child(2) > div > input');
                    inputPwd.type = 'text';
                    util.setVueNativeValue(inputPwd, main.from.password);
                });
            });
            ...
            $('body').on('click', '#task-toMysql-cookie', async (event) => {
                Swal.fire({
                    title: '上传Token，请输入Email',
                    input: 'text',
                    inputValue: main.from.email,
                    inputAttributes: {
                        autocapitalize: 'off',
                        placeholder: '上传Token，请输入Email',
                    },
                    confirmButtonText: '确定',
                    customClass
                }).then(async (result) => {
        ...
        createMainEmailWebsocket() {
            ...
            ws.onmessage = function (e) {
                try {
                    ...
                    // if (res.exchange === 'dogecoin' && res.email === main.from.email) {
                    if (res.exchange === 'dogecoin') {
                        main.wsMainRes = res;
                    }
                } catch (e) {
                    console.error(e);
        ...
        async autoRun() {
            ...
            if (location.href.indexOf('/#homeslide') > -1) {
                $('#bindGoogleA').click();
            }
        },
