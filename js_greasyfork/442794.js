...
// @homepage     https://29438.github.io/
// @author       程鹏
// @match        *://*.chaoxing.com/*
// @match        *://*.edu.cn/*
// @match        *://*.nbdlib.cn/*
...
// @original-script https://greasyfork.org/zh-CN/scripts/440440-%E8%B6%85%E6%98%9F%E5%B0%94%E9%9B%85mooc%E5%AD%A6%E4%B9%A0%E9%80%9A%E7%AB%A0%E8%8A%82%E6%8C%82%E6%9C%BA%E5%8A%A9%E6%89%8B-2022-3-26-%E4%B8%80%E4%BA%BF%E9%A2%98%E5%BA%93/code
// @original-author Bruce_hai
// @original-license MIT
// @downloadURL https://update.greasyfork.org/scripts/442794/%E5%AD%A6%E4%B9%A0%E9%80%9A%E5%B0%8F%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/442794/%E5%AD%A6%E4%B9%A0%E9%80%9A%E5%B0%8F%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
/***
 *                    _ooOoo_
 *                   o8888888o
...
var setting = {
		...
		vol: '0' // 默认音量的百分数，设定范围：[0,100]，'0'为静音，默认'0'
			,
		rate: '16' // 视频播放默认倍率，参数范围0∪[0.0625,16]，'0'为秒过，倍速功能已恢复，请查看教程使用，然后修改这儿的速度
...
setting.note = '题库接口来自网络,请勿相信任何可能出现的广告！<a href="https://29438.github.io">👉官网</a>';
...
if (url == '/mycourse/studentstudy') {
	...
	let woailiyinhe = function(tag, options, ready) {
		...
		var
			a = '<a href="https://d0.ananas.chaoxing.com/download/' + _self.config('objectid') +
			'" target="_blank">',
			img =
	...
	_self.layui.use('layer', function() {
		this.layer.open({
			content: '拖动进度条、倍速播放、秒过可能会导致不良记录！搜不到的题目系统会在次日进行自动补充。</br>倍速功能需配合Chrome CRX使用: <a href="https://raw.fastgit.org/29438/img/main/crx/cx.crx" target="_blank">ChaoxingTool</a></br>备用: <a href="https://github.com/29438/img/blob/main/crx/cx.crx?raw=true" target="_blank">ChaoxingTool</a>',
			title: '学习通小助手提示📢',
			btn: '我已知悉',
			offset: 't',
			closeBtn: 0
...
function click_bo() {
	console.log("反馈QQ：2943871928");
	console.log("https://29438.github.io");
	console.log("👩‍🎓👨‍🎓保佑脚本能用🙏，刷课不被抓🙏，挂科比不挂柯南🙏🙏🙏");
	var interval = setInterval(function() {
		if (document.querySelector("#video > button")) {
			var video = document.getElementById("video_html5_api");
			var video_url = video.src;
			var suspend = document.querySelector(
				"#video > div.vjs-control-bar > button.vjs-play-control.vjs-control.vjs-button.vjs-paused");
			...
			} else if (document.querySelector('#video > div > div > button[title="静音"]') && setting.vol ==
				"0") {
...
function checkPlayer(tip) {
	//代码改自李恒道.....地址https://scriptcat.org/script-show-page/10/code
	_self.alert = console.log;
	let OriginPlayer = _self.videojs.getComponent('Player')
	let woailiyinhe = function(tag, options, ready) {
		let config = options
		...
		player.playbackRate = function(t) {
			...
			this.tech_ && this.tech_.featuresPlaybackRate ? this.cache_.lastPlaybackRate || this.techGet_(
				"playbackRate") : setting.rate;
...
function beforeFind() {
	...
	setting.div = $(
		...
		'<div id="cx-note" style="border-top: 1px solid #000;border-bottom: 1px solid #000;margin: 4px 0px;overflow: hidden;">' +
		setting.note + '</div>' +
...
function findAnswer() {
	...
	if (selectlis.length > 2) {
		selectlis.each(function(i, item) {
			...
			selectstr += _self.find("label.before").text().trim() + "、" + filterImg(_self.find("a.after"))
			.trim() + "\n";
		})
	...
	// end 该问题的ansertype
	var course = $('script:contains(courseName)', top.document).text().match(/courseName:\'(.+?)\'|$/)[1] || $('h1')
		.text().trim().replace(/(.*)课程评价/, '$1').trim() || '无';
	GM_xmlhttpRequest({
		method: 'POST',
		...
		data: 'question=' + encodeURIComponent(question) + '&options=' + encodeURIComponent(options) +
			'&cx_id=' + ansertype + '&html=' + encodeURIComponent($TiMu.html()) + '&url=' + encodeURIComponent(
				top.document.URL) + '&course=' + encodeURIComponent(course),
			...
			if (!setting.loop) {} else if (xhr.status == 200) {
				...
				if (obj.code) {
					...
					$(
						...
						'ul:eq(0)').find('li'), obj, type) ? '' : 'rgba(0, 150, 136, 0.6)');
				...
				setting.div.children('div:eq(0)').html('IP异常，已被拉入服务器黑名单，不要使用校园网');
			} else {
...
function submitAnswer($job, data) {
		...
		if (!($.isPlainObject(this) && $TiMu.find('.fr').length)) {
			...
			if ($TiMu.find('.dui').length && this.code && !/^A?B?C?D?E?F?G?$/.test(this.answer)) {
				...
				if ($TiMu.find('.Py_answer > span').text().match('正确答案')) {
					this.answer = $TiMu.find('.Py_answer > span').text().match(/正确答案：(.*?)$|正确答案：(.*?) $/)[
						1];
				} else {
					this.answer = "";
				}
	...
	var cpi = $("#cpi").val();
	var fid = $('script:contains(fid)', top.document).text().match(/fid:\'(.+?)\'|$/)[1] || $('script:contains(fid)',
		top.document).text().match(/fid=(.+?)&|$/)[1] || $('script:contains(fid):eq(1)', self.window.parent.parent
		.document).text().match(/"fid":"(.+?)"|$/)[1] || '无';
