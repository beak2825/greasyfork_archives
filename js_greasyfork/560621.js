// ==UserScript==
// @name         知乎问题屏蔽器-i
// @namespace    zhihu-question-blocker-i
// @version      1.3.0
...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/560621/%E7%9F%A5%E4%B9%8E%E9%97%AE%E9%A2%98%E5%B1%8F%E8%94%BD%E5%99%A8-i.user.js
// @updateURL https://update.greasyfork.org/scripts/560621/%E7%9F%A5%E4%B9%8E%E9%97%AE%E9%A2%98%E5%B1%8F%E8%94%BD%E5%99%A8-i.meta.js
// ==/UserScript==
...
(() => {
    ...
    GM_addStyle(`
        ...
        .zhihu-blocker-quick-tab::before {
            content: '知乎屏蔽';
            white-space: nowrap;
    ...
    // 检查文本是否包含屏蔽关键词，返回匹配的关键词数组
    function containsBlockedKeyword(text) {
        if (!text) return [];
        const lowerText = text.toLowerCase();
        const matched = blockedKeywords.filter(keyword =>
    ...
    // 屏蔽问题元素
    function blockElement(element, reason, fullTitle, matchedKeywords) {
        element.classList.add('zhihu-blocker-hidden');
        blockedCount++;
        const keywordsStr = matchedKeywords && matchedKeywords.length > 0 
        ...
        console.log(`[知乎屏蔽器] [${timestamp}] #${blockedCount} 屏蔽问题: ${fullTitle || reason}${keywordsStr}`);
        updateStats();
    }
        ...
        selectors.forEach(selector => {
            document.querySelectorAll(selector).forEach(item => {
                ...
                titleElements.forEach(titleEl => {
                    ...
                    const matched = containsBlockedKeyword(titleText);
                    if (matched.length > 0) {
                        shouldBlock = true;
                        fullTitle = titleText.trim();
                        blockReason = titleText.substring(0, 50) + '...';
                        matchedKeywords = [...new Set([...matchedKeywords, ...matched])];
                    }
                });
                ...
                descElements.forEach(descEl => {
                    ...
                    const matched = containsBlockedKeyword(descText);
                    if (matched.length > 0) {
                        shouldBlock = true;
                        if (!fullTitle) {
                            fullTitle = descText.trim();
                        }
                        if (!blockReason) {
                ...
                if (shouldBlock) {
                    blockElement(item, blockReason, fullTitle, matchedKeywords);
                }
            });
        });
    }
    ...
    function init() {
        ...
        // 创建快速输入框
        quickInputPanel = createQuickInputPanel();
