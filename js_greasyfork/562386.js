// ==UserScript==
// @name         HappyLand
// @name:zh-TW   HappyLand
// @namespace    http://tampermonkey.net/
// @version      v2.8
// @description  🚀新功能上线，不浪费一秒，会员随时停启💰。专为“极乐禁地”网站用户量身打造，助你解锁前所未有的观影自由。无需付费，即可畅享VIP专属特权，让你的每一次访问都物超所值。
// @description:zh-TW   🚀新功能上線，不浪費一秒，會員隨時停啟💰。專為「極樂禁地」網站用戶量身打造，助你解鎖前所未有的觀影自由。無需付費，即可暢享VIP專屬特權，讓你的每一次造訪都物超所值。🚀
// @author            lx
...
// @downloadURL https://update.greasyfork.org/scripts/562386/HappyLand.user.js
// @updateURL https://update.greasyfork.org/scripts/562386/HappyLand.meta.js
// ==/UserScript==
...
(function () {
  ...
  function clearModal() {
    const targetClasses = ["vue-nice-modal-root", "van-popup", "van-overlay"];

    // Helper to remove element
    const removeNode = (node) => {
      if (node.nodeType !== 1) return;

      // Check if the node itself matches any target class
      ...
      // Check children for all target classes
      targetClasses.forEach((cls) => {
        if (node.querySelectorAll) {
          const children = node.querySelectorAll(`.${cls}`);
          children.forEach((child) => {
            child.remove();
            console.log(`Removed .${cls} from child`);
          });
        }
      });
    };
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => removeNode(node));
      });
    });
  ...
  `;
	...
	function createPersonalInfoDialog() {
        ...
        <div class="info-dialog">
            ...
            <div style="text-align:center">
                <h2 style="margin:0;font-size:18px;color:gray">${vipUser.username || "未登录"}</h2>
            ...
            ${
                ...
                <div class="expired-notice">
                    ${new Date(vipUser.expireTime) > new Date() ? `会员有效期至：${new Date(vipUser.expireTime).toLocaleDateString()}` : "您的会员已过期，请重新开通"}
                </div>
            `
								: ""
						}
            ...
            <div class="script-settings">
                <div class="setting-item">
                    <span>脚本开关</span>
                    <label class="switch">
                        <input type="checkbox" id="featureToggle" ${featureEnabled ? "checked" : ""}>
            ...
            <div class="info-actions">
                <a href="https://www.haijiaom.cc/" class="vip-btn">
                    ...
                    ${vipUser.expireTime && new Date(vipUser.expireTime) > new Date() ? "续费会员" : "开通会员"}
                </a>
    ...
    `;
		...
		if (featureToggle) {
			featureToggle.addEventListener("change", async () => {
				...
				try {
					const result = await putAsync(baseUrl + "/business/member/script-operation", {
					...
					if (result.code == 200) {
						...
						showApiMessage(`功能已${isEnabled ? "开启" : "关闭"}，页面即将刷新...`);
						setTimeout(() => {
	...
	function getDownloadTools() {
		...
		const tools = [...baseTools, ...mobileTools].filter((tool) => tool.clients.includes(client));
  ...
  `;
			...
			// 关闭按钮事件
			secondDialog.querySelector(".dialog-close").addEventListener("click", () => {
				secondDialog.remove();
			});
		}
  ...
  `;
    ...
    `;
		...
		// Create hidden video element for download/sniffing
		const hiddenVideo = document.createElement("video");
		hiddenVideo.id = "hidden-download-video";
    ...
    `;
		...
		hiddenVideo.playsInline = true; // Important for iOS
			...
			window.hlsPlayer = {
				setContent: (input) => {
					if (isM3U8Content(input)) {
						...
						currentUrl = input; // createBlobUrl(input);
					} else {
						// Handle URL
						currentUrl = input;
					}
				...
				play: () => {
					...
					// Set source for hidden video using the original m3u8Url
					if (m3u8Url && m3u8Url.startsWith("http")) {
						console.log("Setting hidden video src to:", m3u8Url);
						hiddenVideo.src = m3u8Url;
						// We don't necessarily need to play it, just setting src might be enough for sniffers
						// But playing it briefly might help some sniffers detect it
						// hiddenVideo.play().catch(e => console.log("Hidden video play error:", e));
					}
	...
	async function togglePlay() {
		...
		// 优先使用原始 m3u8 URL 播放
		if (m3u8Url && m3u8Url.startsWith("http")) {
			console.log("Play directly with URL:", m3u8Url);
			window.hlsPlayer.setContent(m3u8Url);
			window.hlsPlayer.play();
			// toggleVisibility();
			return;
		}
		...
		tempM3u8 = tempM3u8.replace("/api/app/media/enkey", "https://d1ibyof3mbdf0n.cloudfront.net/api/app/media/enkey");

		tempM3u8 = tempM3u8.replace("/api/app/media/newenkey", "https://d1ibyof3mbdf0n.cloudfront.net/api/app/media/newenkey");
	...
	function toggleVisibility() {
		...
		navbar.style.display = isVisible ? "block" : "block";
	...
	function CustomXHR() {
		...
		const urlMappings = {
			"videos/v2/getUrl": (url) => {
				...
				return url.replace(url, "https://api.qianyuewenhua.xyz/videos/getPreUrl");
			},
		};
						...
						try {
							...
							Object.keys(urlPatterns).forEach((pattern) => {
								if (new RegExp(pattern).test(requestURL)) {
									...
									console.log("[Modified Response]:", requestURL, modifiedResponse);
								}
							});
							Object.keys(urlM3u8Patterns).forEach((pattern) => {
								if (new RegExp(pattern).test(requestURL)) {
									...
									if (checkToken() && hasPrev) {
										console.log("#", requestURL.lastIndexOf("_0001"));
										if (requestURL.lastIndexOf("_0001") > 0) {
											console.log("预览");
											requestURL = requestURL.replace("_0001", "");
	...
	const urlPatterns = {
		"api\\/video\\/checkVideoCanPlay(\\?[^/]*)?$": (response) => {
			...
			response.data = Object.assign({}, JSON.stringify(atob(atob(atob(decodeData))), `utf-8`), freeVideo);
			return response;
		},
		...
		"videos\\/v2\\/getUrl(\\?[^/]*)?$": (response) => {
			if (checkToken() && hasPrev) {
				response.data.url = response.data.url.replace(/start=\d+\&end=\d+\&/, "");
	...
	async function fetchUserInfo() {
		try {
			const response = await fetch(baseUrl + "/business/member/info?siteCode=" + siteCode, {
				method: "GET",
				headers: {
	...
	async function gcreateDownloadUrl() {
		if (slefVideo.m3u8Url) {
			...
			return await postAsync(baseUrl + "/business/download/generate-token", params);
		} else {
	...
	function removeAdds() {
		...
		const TARGETS = [".vue-nice-modal-root", ".wh-screen", ".app-list-pop", ".mine-ad", ".jg-list", ".sub-nav"];
		// 是否每个目标只处理一次
		const observeOnce = false;

		// 用 WeakSet 记录已处理的元素，防止重复
		...
		const observer = new MutationObserver((mutations, obs) => {
			...
			// 如果只监听一次，全部找到后断开
			if (observeOnce) {
				const allFound = TARGETS.every((target) => {
					return document.querySelector(target);
				});
				if (allFound) {
					observer.disconnect();
					console.log("✅ 所有目标已找到，停止监听");
				}
			}
		});
		...
		observer.observe(document.body, {
			childList: true,
			subtree: true,
			attributes: false, // 可改为 true 监听 class 变化
		});
	}
})();
