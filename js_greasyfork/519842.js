...
// @downloadURL https://update.greasyfork.org/scripts/519842/%E6%B0%B4%E6%BA%90%E5%8A%A9%E6%89%8B%20%28hotfix%20ver%29.user.js
// @updateURL https://update.greasyfork.org/scripts/519842/%E6%B0%B4%E6%BA%90%E5%8A%A9%E6%89%8B%20%28hotfix%20ver%29.meta.js
// ==/UserScript==
...
(async () => {
    ...
    const POST_VIOLATION_ANNOUNCEMENT_TOPIC_ID = new Set([12737, 294846]);
    ...
    const getRetortLib = () => {
        ...
        try {
            retortLib = window.Discourse.lookup('service:retort');
        } catch (e) {
            // eslint-disable-next-line no-console
            console.error(e);
        }
    ...
    const ALL_FEATURES = [
        ...
        },
        {
            ...
            onMatch: (element, optionValues) => {
                ...
                const postFor = id => (Discourse.lookup('route:topic').currentModel.postStream.posts || []).find(p => p.id === id)

                const scheduleRetortUpdate = () => {
                    let retorts = null;
                    try {
                        retorts = postFor(currentPostId).retorts;
                ...
                const updateOneRetort = (retortButton) => {
                    try {
                        ...
                        const retortUsernames = [...postFor(postId).retorts.find((item) => item.emoji === retortEmoji)?.usernames || []];
        ...
        },
        {
            ...
            onMatch: (element) => {
                const topicArea = element.closest('.topic-area');
                const currentTopicId = parseInt(topicArea.getAttribute('data-topic-id'), 10);
                if (!POST_VIOLATION_ANNOUNCEMENT_TOPIC_ID.has(currentTopicId)) {
                    return;
                }
        ...
        },
        {
            ...
            onMatch: (element) => {
                ...
                const getEndYMD = (durationContent) => {
                    // 尝试匹配原始格式
                    const originalMatch = durationContent.match(/(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*(?:日|号)/u);
                    if (originalMatch) {
                        return originalMatch.slice(1, 4).map((s) => parseInt(s, 10));
                    }

                    // 尝试匹配新格式
                    const newFormatMatch = durationContent.match(/data-date="(\d{4})-(\d{2})-(\d{2})"/);
                    if (newFormatMatch) {
                        return newFormatMatch.slice(1, 4).map((s) => parseInt(s, 10));
                    }
                ...
                const updateDurationNode = (durationNode) => {
                    const durationContent = durationNode.innerHTML; // 使用 innerHTML 而不是 textContent
                    ...
                    originalContentSpan.innerHTML = durationContent; // 使用 innerHTML 保留原始格式
                    ...
                    durationNode.innerHTML = ''; // 清空原始内容
                    durationNode.appendChild(parsedSpan); // 添加新内容
                };
