...
(function $123pan() {
	...
	*/
	...
	let info = {
		...
		version: (GM_info.script?.version || "1.0.6.4"),
		...
		async showSettings() {
			...
			let dialog = await Swal.fire({
				...
				html: `<div class="ym-settings">
					<div class="block">Tips: 设置自动保存，刷新页面后生效。</div>
					<div class="block">Tips: 您也可通过脚本管理器菜单再次打开本窗口。</div>
					<div class="block">
						<blockquote>
					...
					${generateHTML("隐藏广告|<small>仅 SVIP 及以上有效</small>", "dads", "boolean")}
	...
	let rules = [
		{
			...
			action: (res, url) => {
				...
				if (user.photo) res.data.HeadImage = user.photo + "#/123-pics/head-pic";
		...
		},
		{
			// 用户信息 基本
			runat: "end",
			match: (url) => url.pathname.includes('api/vip/buy_list'),
			condition: () => user.vip == true,
			action: (res, url) => {
				if (res.data?.OrderList) {
					function generateOrderNo(date = new Date(), prefix = 'VMH', typeCode = '01', channel = 'V2CS') {
						const year = date.getFullYear();
						const month = String(date.getMonth() + 1).padStart(2, '0');
						const day = String(date.getDate()).padStart(2, '0');
						const hours = String(date.getHours()).padStart(2, '0');
					...
					res.data.OrderList = [
						...
						}] : []),
						{
							...
							"originAmount": 3000,
							"itemType": 1,
							"memberLevel": user.svip == true ? "SVIP" : "VIP",
							"isCreditGoods": false,
							"signNo": "",
							"continuousPayment": 0
						},
				...
				if (res.data?.benefitList) {
					...
					res.data.benefitList = [
						...(user.pvip == true ? [{
							"benefitType": "会员权益",
							"desc": "长期VIP(模拟)",
							"startTime": new Date(1638288000 * 1000).toLocaleDateString(mount),
							"endTime": "",
						...
						{
							"benefitType": "会员权益",
							"desc": `${user.svip == true ? "SVIP" : "VIP"}(模拟)`,
							"startTime": new Date(1638288000 * 1000).toLocaleDateString(mount),
							"endTime": new Date(user.endtime * 1000).toLocaleDateString(mount),
							"sourceType": "123 云盘会员青春版",
							"isEffect": true
						},
						...res.data.benefitList
		...
		},
		{
			// 获取基础地址
			runat: "start",
			match: (url) => url.pathname.includes('api/getBaseUrl'),
			condition: () => true,
			action: (res, url) => {
				res = { "apiUrl": location.origin }
				return res;
			}
		},
		{
			// 获取服务器时间
			runat: "start",
			match: (url) => url.pathname.includes('api/get/server/time'),
			condition: () => true,
			action: (res, url) => {
				res = {
					"code": 0,
	...
	navigator.sendBeacon = base.hooker.sendBeacon;
	...
	base.waitForKeyElements(".contentBorder .cleanNet", (element) => {
		...
		let text = "感谢您选择会员青春版，喜欢的话就来给个 Star 吧~ >>>";
	...
	base.waitForKeyElements(".frontend-layout-header > .frontend-layout-header-left > .frontend-layout-header-left-cdn", (element) => {
		...
		element.children(':first').text("感谢您选择会员青春版，喜欢的话就来给个 Star 吧~");
	...
	base.waitForKeyElements(".FileList-CleanNet-cleanNet-container", (element) => {
		if (element.data("replaced")) return;
		element.children('span:first').text("感谢您选择会员青春版，喜欢的话就来给个 Star 吧~");
		element.data("replaced", true);
	...
	// 页脚恢复点击功能
	base.waitForKeyElements(".footer-area.pc", (element) => {
