...
// @author         adipiciu + maiert (based on Travian wave builder 0.5 by Serj_LV)
...
// @version        2.2
// @downloadURL https://update.greasyfork.org/scripts/394418/Travian%20wave%20builder.user.js
// @updateURL https://update.greasyfork.org/scripts/394418/Travian%20wave%20builder.meta.js
// ==/UserScript==
...
function allInOneOpera () {
    ...
    function getRandomWave ( x ) {
        if (x % 4 === 0) {
            return 100 + Math.round(Math.random() * 10)
        }
        const incrementalTime = x % 4 * 10 + 100;
        const randomInteger = incrementalTime + Math.round(Math.random() * 10);
    ...
    function sendWaves () {
        ...
        waitForStartTimer = setInterval(() => waitForStart(wCount), Math.round(Math.random()*10));
    }

    function waitForStart(wCount) {
        var d = new Date();
        if (d.getMilliseconds() > 0 && d.getMilliseconds() < 20) {
            clearInterval(waitForStartTimer);
            sendWaves2(wCount);
        }
    }

    function sendWaves2(wCount) {
        const d = new Date();
        let nextWave = 10;
        for( var i=0; i<wCount; i++ ) {
            nextWave += getRandomWave(i);
