// ==UserScript==
// @name         lanis 자동사냥 3.7.1
// @namespace    http://tampermonkey.net/
// @version      3.7.1
// @description  화면 좌측에 ON/OFF 버튼, 레벨제한 입력창, 골드제한 입력창, 숙련도제한 입력창, 버튼 텍스트 입력창, 제외 버튼 텍스트 입력창을 추가. 레벨제한 또는 숙련도제한 도달 시 버튼 클릭을 일시 중지하고 조건 변화 시 재개. 입력된 텍스트가 포함된 버튼을 골드제한 이하일 때 0.3초 간격으로 자동 클릭. 제외 버튼 텍스트가 포함된 버튼이 있으면 자동 클릭 중지. 입력창 크기 드래그로 조절 가능.
// @author       Grok
// @match        https://lanis.me/*
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/543113/lanis%20%EC%9E%90%EB%8F%99%EC%82%AC%EB%83%A5%20371.user.js
// @updateURL https://update.greasyfork.org/scripts/543113/lanis%20%EC%9E%90%EB%8F%99%EC%82%AC%EB%83%A5%20371.meta.js
// ==/UserScript==
...
(function() {
    ...
    // 제외 텍스트 확인 함수: 페이지의 모든 텍스트에서 제외 텍스트가 있는지 확인 (UI 제외)
    ...
    toggleButton.addEventListener('click', function() {
        ...
        if (isOn) {
            ...
            clickInterval = setInterval(function() {
                ...
                // 제외 텍스트가 페이지에 있으면 클릭 중지
                if (hasExcludeText()) {
                    console.log('제외 텍스트가 페이지에 감지되었습니다. 자동 클릭을 중지합니다.');
                    return;
                }
    ...
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            if (isOn) {
                ...
                if (hasExcludeText()) {
                    console.log('제외 텍스트가 동적으로 감지되었습니다.');
                }
            }
        });
    });
