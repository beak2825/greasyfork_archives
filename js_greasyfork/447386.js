...
// @downloadURL https://update.greasyfork.org/scripts/447386/202273.user.js
// @updateURL https://update.greasyfork.org/scripts/447386/202273.meta.js
// ==/UserScript==
...
//CALL CONSTANTS
var CALL_PON_CHI = 1.1; //Lower: Call Pon/Chi less often. Higher: Call Pon/Chi more often. Default: 1.0, Minimum: 0
...
function discardTile(tile) {
	...
	for (var i = 0; i < ownHand.length; i++) {
		if (isSameTile(ownHand[i], tile, true)) {
			...
			if (!isDebug()) {
				callDiscard(i);
				var weixiandu = getTileDanger(tile);
				currentActionOutput.value = `${tileName}:V${foldValue} D${weixiandu};`;
			}
			else {
				ownHand.splice(i, 1);
			}
