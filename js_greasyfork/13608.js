...
// @grant        all
// @downloadURL https://update.greasyfork.org/scripts/13608/Last%20SEASON%20TI.user.js
// @updateURL https://update.greasyfork.org/scripts/13608/Last%20SEASON%20TI.meta.js
// ==/UserScript==

var wage_rate = 24.7;

var today = new Date();
var s43 = new Date("09 07 2015 10:30:00 GMT");				// season start
var s43t = new Date("09 08 2015 00:30:00 GMT");				// first training
	...
	$("div.player_name > a").each(function(e, player) 
	{
		...
		{ 
			if (xhr.readyState == 4)
			{
                ...
                function Last_Season_Avg_Ti(TI,Season_TI,session,months) {
                    if ((months + 1) >=  session)
                    { return 0 ;
                    }
                    else
				...
				$("a[player_link=" + resultado.player.player_id + "]").closest("tr").append("<td class='right align_right LSTI' style='padding-right:5px;'><span>" + Last_Season_Avg_Ti(return_TI((arrary[id_index(resultado.player.player_id,arrary)][1]),resultado.player.skill_index,player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary))),Season_TI(player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary)),SI(resultado.player.skill_index),return_wage(resultado.player.wage),24.795),session,parseInt(resultado.player.months))   + "</span></td>");
                ...
                if (today.getDay() < 2){
                $("a[player_link=" + resultado.player.player_id + "]").closest("tr").append("<td class='right align_right AVGTI' style='padding-right:5px;'><span>" + Season_TI(player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary)),SI(resultado.player.skill_index),return_wage(resultado.player.wage),24.795)/session + "</span></td>");
                }
                $("a[player_link=" + resultado.player.player_id + "]").closest("tr").append("<td class='right align_right SB' style='padding-right:5px;'><span>" + return_starting_blooming(Last_Season_Avg_Ti(return_TI((arrary[id_index(resultado.player.player_id,arrary)][1]),resultado.player.skill_index,player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary))),Season_TI(player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary)),SI(resultado.player.skill_index),return_wage(resultado.player.wage),24.795),session,parseInt(resultado.player.months)),return_TI((arrary[id_index(resultado.player.player_id,arrary)][1]),resultado.player.skill_index,player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary))))  + "</span></td>");
