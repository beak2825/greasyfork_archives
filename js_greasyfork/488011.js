...
// @require      https://update.greasyfork.org/scripts/469732/1397327/ImageDownloaderLibla.js
// @grant        GM_info
// @grant        GM_xmlhttpRequest
// @grant        unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/488011/ComicWalkerDownloader.user.js
// @updateURL https://update.greasyfork.org/scripts/488011/ComicWalkerDownloader.meta.js
// ==/UserScript==
...
(async function(axios, JSZip, saveAs, ImageDownloader) {
  ...
  const title1 = document.getElementsByTagName('meta')['description'].content.split('読むならカドコミ')[0];
  const title2 = title;
  const title0 = `${title1} ${title2}`;
  let title00 = title0.replace(/(\?|\~|\/)/gi,  function ($0, $1) {
        return {
            '?':'？',
            '~':'～',
            ':':'：',
