// ==UserScript==
// @name         Special Gym Ratios (with goals)
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/375635/Special%20Gym%20Ratios%20%28with%20goals%29.user.js
// @updateURL https://update.greasyfork.org/scripts/375635/Special%20Gym%20Ratios%20%28with%20goals%29.meta.js
// ==/UserScript==

// Based off of Torn Gym Pony by Zanoab (http://puu.sh/jFtro/1af393771e.user.js).

// Kivou [2000607] I just added this from original script
var dex = 300000000;
var goals = {
    strength: 2*(dex+4*dex/5.)/5.,
    defense: 4*dex/5.,
    speed:  2*(dex+4*dex/5.)/5.,
    dexterity: dex,
}
	...
	setInterval(function() {
        ...
        $statContainers.each(function(index, element) {
            ...
            if (stats[stat]) {
                ...
                var remains = goals[stat]-stats[stat];
                if (remains > 0) {
                    gymStatus += '<spa class="gymstatus t-red"><br />' + FormatAbbreviatedNumber(remains, 1) + ' below ' + FormatAbbreviatedNumber(goals[stat], 1) + ' goal. </span>';
                } else {
                    gymStatus += '<spa class="gymstatus t-green"><br />' + FormatAbbreviatedNumber(-remains, 1) + ' above ' + FormatAbbreviatedNumber(goals[stat], 3) + ' goal.</span>';
                }
                var $statInfoDiv = $element.find('[class^="description__"], [class*=" description__"]');
                var $insertedElement = $statInfoDiv.find('.gymstatus');
                $insertedElement.remove();
