...
// @version        1.204
// @author         Agor71 (edited by UnclWish and Chippa)
...
// @downloadURL https://update.greasyfork.org/scripts/29974/Virtonomica%3Akvala_personal_indicator_zarplaty_by_Agor71.user.js
// @updateURL https://update.greasyfork.org/scripts/29974/Virtonomica%3Akvala_personal_indicator_zarplaty_by_Agor71.meta.js
// ==/UserScript==

var run = function() {
	$( document ).ready(function() {
		//Следующие две строки убирают "обои" со всех страниц. Для отключения функционала поставьте "//" в следующих двух строках - аналогично с тем, как эти два символа стоят в начале этой строки
		$('body').attr('class','');
		$('#wrapper > div.metro_header > div > div.picture').remove();
		...
		function calcTopGet(){
				...
				maxq=f2(calcQualTop1(kv,cur_pers, type));
					maxq100 = f2(calcQualTop100(kv, type));
//		console.log(maxq100);
		$("a:contains('Сотрудники и зарплата')").attr("href", $("a:contains('Сотрудники и зарплата')").attr("href") + '?maxq=' + maxq100);
	...
	// топ-1
		...
		$("td:contains('Уровень квалификации')").next().each(function() { 
			...
			if (new_interface) {
				...
				var str = '<tr><td>Квалификация для 100%/120%/144% топ-1</td>';
				...
				this.innerHTML = this.innerHTML + '<br><font color=gray>Максимальная квала для 100%/120%/144% по топ-1: <b>' + maxq + '/' + f2(calcQualTop1(kv, cur_pers / 1.2, type)) + '/' + f2(calcQualTop1(kv, cur_pers / 1.44, type)) + '</b></font>';
	...
	// максмальное количество поситетилей по персоналу
		...
		function calcQualTop1(q, p, type){		
			...
			return Math.log(0.2*14*getK1(type)*q*q/p)/Math.log(1.4);	
				...
				///////////////////////////////////////////////////////////////////////////
	//function calcQualTop100(q, type)
	// q - квалификация игрока
	//вычисляет максимальное квалификацию работающих при заданных их численности и квалификации игрока (обратна calcPersonalTop1())
	///////////////////////////////////////////////////////////////////////////// 
	function calcQualTop100(q,type){		
		return 0.2*14*getK1(type)*q*q;	
	}//end calcQualTop100()
