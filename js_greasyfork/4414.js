// ==UserScript==
// @name        MTurk Great HIT Export (Mills' special edition)
...
// @version     1.90006
// @downloadURL https://update.greasyfork.org/scripts/4414/MTurk%20Great%20HIT%20Export%20%28Mills%27%20special%20edition%29.user.js
// @updateURL https://update.greasyfork.org/scripts/4414/MTurk%20Great%20HIT%20Export%20%28Mills%27%20special%20edition%29.meta.js
// ==/UserScript==
...
//
// 2014-08-21  cribbed (by Tribune) from Tjololo's version 1.9 at https://greasyfork.org/scripts/911-mturk-great-hit-export
//
// 2014-10-01 changed from (c. line 42) var API_PROXY_BASE = 'https://api.turkopticon.istrack.in/';
//                      to var API_PROXY_BASE = 'http://turkopticon.ucsd.edu/api/';
//
(function () {
    var EDIT = false;
    var HITS = [];
    ...
    DEFAULT_TEMPLATE = 'Title: [url={link}][COLOR=blue]{title}[/COLOR][/url]\n';
...
// not for Mills toStuff += (txt.indexOf('{to_stuff}') >= 0 ? (txt.indexOf('{to_text}') >= 0 ? "" : toText) : "");
            toStuff += "\nNumber of Reviews: "+numResp;
// not for Mills            toStuff += "[URL=\""+TO_BASE+"report?requester[amzn_id]=" + hit_data['requesterId'] + "&requester[amzn_name]=" + hit_data['requester'] + "\"]";
// not for Mills            toStuff += "\n(Submit a new TO rating for this requester)[/URL]";
        }
        ...
        if (linkElements != null)
            for (var i = 0; i < linkElements.length; i++) {
                ...
                if (linkElements[i] != null && linkElements[i].textContent == '(Why?)'){
                    var url = EASYLINK + linkElements[i].href.replace("notqualified?hit","preview?group");
                    hit_data.adfly = url;
                    hit_data.link = linkElements[i].href.replace("notqualified?hit","preview?group");
                }
            }
