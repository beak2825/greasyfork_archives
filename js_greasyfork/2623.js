...
// @description    HWM Transfer Search
// @homepage       http://userscripts.org/scripts/show/113693
// @include        http://www.heroeswm.ru/pl_transfers.php*
// @version        1.12
// @author         Alex_2oo8
// @downloadURL https://update.greasyfork.org/scripts/2623/%5BHWM%5D%20TransferSearch.user.js
// @updateURL https://update.greasyfork.org/scripts/2623/%5BHWM%5D%20TransferSearch.meta.js
// ==/UserScript==
...
//							if ( td_arr[i].innerHTML.match(/&nbsp;&nbsp;\d{2}-\d{2}-\d{2} \d{2}:\d{2}/) )
	...
	function search2( id, type, elem, pgCount )
	{
		...
		elem.appendChild( center );
		elem.appendChild( document.createElement( 'br' ) );
		startpage=Number(getstartpage());
//  GM_log("startpage is " + startpage +", pgCount is " + pgCount);
		startSearch( startpage, id, reg, pgCount, elem, type, search_str );
	}
	
	function startSearch( pg, id, reg, lastPg, elem, type, search_str )
	{
// GM_log("startSearch Invoked with startpage " + pg + " , lastPg " + lastPg + " , stop is " + document.getElementById('stop').value );
		if ( document.getElementById('stop').value != '1' && pg <= lastPg )
		{
			var pg = pg - 1;

// GM_log("goint to xmlhhtpreqest");
		GM_xmlhttpRequest(
			{
				method: 'GET',
...
//								if ( td_arr[i].innerHTML.match(/&nbsp;&nbsp;\d{2}-\d{2}-\d{2} \d{2}:\d{2}/) )
	...
	{
		var id = location.href.match( /\?(?:.*=.*&)*page=([0-9]*)(?:&.*=.*)*/ );
		if (id)	return id[1];
		else return 1
	}