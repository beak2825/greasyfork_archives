// ==UserScript==
// @name         Theme & Font Changer castom
// @namespace    https://logs.blackrussia.online/
// @version      1.0
// @description  Theme Changer + Font Changer for Black Logs with Custom Color Picker
// @author       novikov
...
// @license      novikov
// @downloadURL https://update.greasyfork.org/scripts/563800/Theme%20%20Font%20Changer%20castom.user.js
// @updateURL https://update.greasyfork.org/scripts/563800/Theme%20%20Font%20Changer%20castom.meta.js
// ==/UserScript==
(function () {
    'use strict';

    // Глобальная переменная для cleanup функции
    let formatCleanup = null;
    ...
    function createStyleContainerBg(element) {
        ...
        // Добавляем переключатель для форматирования чисел
        const switchFormatNumbersBlock = document.createElement('label');
        switchFormatNumbersBlock.className = 'switch';
        switchFormatNumbersBlock.innerHTML = `
            <input type="checkbox" id="formatNumbersCheck">
            <span class="slider round" style="padding-right: 20px;"></span>
            <div class="addingText tooltip-style" style="display: block; width: max-content; margin: 5px; margin-left: 50px">Форматировать числа
            <span class="tooltip-style-text">Добавляет пробелы в большие числа для лучшей читаемости (5 384 374 вместо 5384374)</span></div>
            </span>
        `;
        switchFormatNumbersBlock.style.marginTop = '20px';
        styleContBody.appendChild(switchFormatNumbersBlock);

        const formatNumbersCheck = document.getElementById('formatNumbersCheck');
        if (localStorage.getItem('formatNumbersEnabled') === 'true') {
            formatNumbersCheck.checked = true;
        }
        formatNumbersCheck.addEventListener('change', function () {
            if (formatNumbersCheck.checked) {
                localStorage.setItem('formatNumbersEnabled', 'true');
                if (formatCleanup) {
                    formatCleanup();
                }
                formatCleanup = applyNumberFormatting();
            } else {
                localStorage.setItem('formatNumbersEnabled', 'false');
                if (formatCleanup) {
                    formatCleanup();
                    formatCleanup = null;
                }
                // Перезагружаем страницу для отмены форматирования
                location.reload();
            }
        });
        ...
        const colors = ['-------МЯГКИЕ-------', 'PINK', 'KHAKI', 'SKYBLUE', 'PALEGREEN', '', '-------ЯРКИЕ-------', 'RED', 'LIME', 'CYAN', 'WHITE', 'YELLOW', 'MAGENTA', 'DEEPPINK', '', 'CUSTOM'];
        const colorCodes = {
            'PINK': '#FFC0CB',
            'KHAKI': '#F0E68C',
            'SKYBLUE': '#87CEEB',
        ...
        colors.forEach(color => {
            ...
            if (color === storedColor) {
                option.selected = true;
                if (color !== 'CUSTOM' && color !== '-------МЯГКИЕ-------' && color !== '' && color !== '-------ЯРКИЕ-------' && colorCodes[color]) {
                    applySelectedStyle(colorCodes[color]);
                } else if (color === 'CUSTOM') {
                    const customColor = localStorage.getItem('selectedCustomColor') || '#FFFFFF';
                    applySelectedStyle(customColor);
                }
            }
        ...
        colorSelector.addEventListener('change', function () {
            if (this.value === 'CUSTOM') {
                // При выборе CUSTOM используем сохраненный кастомный цвет
                const customColor = localStorage.getItem('selectedCustomColor') || '#FFFFFF';
                localStorage.setItem('selectedColor', 'CUSTOM');
                applySelectedStyle(customColor);
            ...
            } else if (this.value !== '-------МЯГКИЕ-------' && this.value !== '' && this.value !== '-------ЯРКИЕ-------') {
        ...
        // Добавляем кастомный color picker для выбора любого цвета
        const customColorBlock = document.createElement('label');
        customColorBlock.className = 'custom-color-block';
        customColorBlock.style.marginTop = '20px';
        styleContBody.appendChild(customColorBlock);

        const customColorPicker = document.createElement('input');
        customColorPicker.type = 'color';
        customColorPicker.className = 'color-picker';
        customColorPicker.id = 'custom-color-picker';
        customColorPicker.style.marginRight = '10px';

        // Получаем сохраненный кастомный цвет или используем белый по умолчанию
        const storedCustomColor = localStorage.getItem('selectedCustomColor') || '#FFFFFF';
        customColorPicker.value = storedCustomColor;

        customColorPicker.addEventListener('input', function() {
            const selectedColor = this.value;
            localStorage.setItem('selectedCustomColor', selectedColor);
            localStorage.setItem('selectedColor', 'CUSTOM');
            applySelectedStyle(selectedColor);

            // Обновляем селектор цветов чтобы показать "CUSTOM"
            colorSelector.value = 'CUSTOM';
        });

        customColorBlock.appendChild(customColorPicker);

        const customColorText = document.createElement('span');
        customColorText.className = 'addingText';
        customColorText.textContent = 'Выбор любого цвета';
        customColorBlock.appendChild(customColorText);
    ...
    function applyNumberFormatting() {
        ...
        // Форматируем все суммы в таблице
        function formatNumbersInTable() {
            ...
            tableCells.forEach(cell => {
                ...
                // Проверяем, является ли числом (целым или с дробной частью)
                if (/^-?\d+(\.\d+)?$/.test(cleanedText)) {
                    // Сохраняем оригинальное значение для копирования
                    if (!cell.dataset.originalNumber) {
                        cell.dataset.originalNumber = cleanedText;
                    }
                    ...
                    if (cleanedText.includes('.')) {
                        ...
                        const formattedInteger = formatNumber(Math.abs(parseInt(integerPart)));
                        cell.textContent = (integerPart[0] === '-' ? '-' : '') + formattedInteger + '.' + decimalPart;
                    } else {
                        // Для целых чисел
                    ...
                    // Добавляем обработчик копирования для сохранения оригинального формата
                    if (!cell.hasAttribute('data-copy-handler-added')) {
                        cell.addEventListener('copy', function(event) {
                            const originalNumber = this.dataset.originalNumber;
                            if (originalNumber) {
                                event.clipboardData.setData('text/plain', originalNumber);
                                event.preventDefault();
                            }
                        });
                        cell.setAttribute('data-copy-handler-added', 'true');
                    }
                }
            });
        }

        // Инициализируем форматирование сразу
        formatNumbersInTable();

        // Создаем более надежный Observer для динамического контента
        let observerTimeout;
        const tableObserver = new MutationObserver(function(mutationsList) {
            // Отменяем предыдущий таймаут
            clearTimeout(observerTimeout);

            // Устанавливаем новый таймаут для объединения мутаций
            observerTimeout = setTimeout(() => {
                let shouldFormat = false;

                for (const mutation of mutationsList) {
                    if (mutation.type === 'childList') {
                        // Проверяем, были ли добавлены узлы с классами таблицы
                        if (mutation.addedNodes.length > 0) {
                            for (let i = 0; i < mutation.addedNodes.length; i++) {
                                const node = mutation.addedNodes[i];
                                if (node.nodeType === 1) { // Element node
                                    // Проверяем разные сценарии обновления таблицы
                                    if (node.classList &&
                                        (node.classList.contains('first-row') ||
                                         node.classList.contains('second-row') ||
                                         node.classList.contains('td-transaction-amount') ||
                                         node.classList.contains('td-balance-after'))) {
                                        shouldFormat = true;
                                        break;
                                    }
                                    ...
                                    if (node.querySelector &&
                                        (node.querySelector('.first-row') ||
                                         node.querySelector('.second-row') ||
                                         node.querySelector('.td-transaction-amount') ||
                                         node.querySelector('.td-balance-after'))) {
                                        shouldFormat = true;
                                        break;
                                    }
                                }
        ...
        // Наблюдаем за всей таблицей логов и её родителями
        const logTable = document.querySelector('#log-table[data-v-2d76ca92]');
        const logTableContainer = document.querySelector('main') || document.body;
        ...
        if (logTable) {
            tableObserver.observe(logTable, config);
        }

        if (logTableContainer && logTableContainer !== logTable) {
            tableObserver.observe(logTableContainer, config);
        }

        // Также наблюдаем за document.body для случаев полной замены контента
        tableObserver.observe(document.body, {
            childList: true,
            subtree: false
        });

        // Дополнительно: форматируем при клике на кнопки пагинации
        const paginationButtons = document.querySelectorAll('#prev-page-btn[data-v-2d76ca92], #next-page-btn[data-v-2d76ca92], .btn-secondary');
        paginationButtons.forEach(button => {
            if (!button.hasAttribute('data-format-handler')) {
                button.addEventListener('click', function() {
                    setTimeout(() => {
                        formatNumbersInTable();
                    }, 300);
                });
                button.setAttribute('data-format-handler', 'true');
            }
        });

        // Также форматируем при любом клике (на всякий случай)
        const clickHandler = function(e) {
            // Проверяем, не кликнули ли мы на элемент пагинации или фильтра
            const target = e.target;
            if (target.closest &&
                (target.closest('#prev-page-btn[data-v-2d76ca92]') ||
                 target.closest('#next-page-btn[data-v-2d76ca92]') ||
        ...
        let checkInterval = setInterval(() => {
            const cells = document.querySelectorAll('td.td-transaction-amount, td.td-balance-after');
            const hasUnformattedNumbers = Array.from(cells).some(cell => {
                const text = cell.textContent.trim();
                const cleaned = text.replace(/\s/g, '');
                return /^-?\d+(\.\d+)?$/.test(cleaned) && !text.includes(' ');
            });

            if (hasUnformattedNumbers) {
                formatNumbersInTable();
            }
        }, 2000);

        // Очистка интервала при отключении функции
        return function cleanup() {
            clearInterval(checkInterval);
            tableObserver.disconnect();
            document.removeEventListener('click', clickHandler, true);

            // Удаляем обработчики с кнопок пагинации
            const buttons = document.querySelectorAll('[data-format-handler]');
            buttons.forEach(button => {
                button.removeAttribute('data-format-handler');
                button.replaceWith(button.cloneNode(true));
            });

            // Удаляем обработчики копирования
            const cells = document.querySelectorAll('[data-copy-handler-added]');
            cells.forEach(cell => {
                cell.removeAttribute('data-copy-handler-added');
    ...
    function applySelectedStyle(color) {
        // Если передана строка (название цвета из списка), получаем hex код
        if (typeof color === 'string') {
            const colorCodes = {
                ...
                'MAGENTA': '#FF00FF',
                'DEEPPINK': '#FF1493',
                'CUSTOM': localStorage.getItem('selectedCustomColor') || '#FFFFFF'
            };
...
--- Ububntu --- */
    ...
    function scriptInit() {
        ...
        // Проверяем, включено ли форматирование чисел
        if (localStorage.getItem('formatNumbersEnabled') === 'true') {
            formatCleanup = applyNumberFormatting();
        }
