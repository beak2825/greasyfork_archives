...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/466772/Dobby2.user.js
// @updateURL https://update.greasyfork.org/scripts/466772/Dobby2.meta.js
// ==/UserScript==
...
(function() {
    ...
    ConsumablePrototype.prototype = {
     ...
     Dobby = {
         ...
         searchKeys:{
             ...
             "es_ES":{
                 energy:"Energía",
                 energyText:"Incrementa Energía:",
                 motivation:"Motivación trabajo",
                 motivationText:"Incrementa Motivación trabajo:",
                 health: "Bonificación Puntos Salud",
                 healthText:"Bonificación Puntos Salud:"
             },
    ...
    Dobby.canAddMissing = function(result) {
        if(!Dobby.settings.addMotivation && Dobby.jobsBelowMotivation(result) && !Dobby.isStopMotivationZero()) {
            alert("No puedo seguir a causa de la motivación");
            return false;
        }
        if(!Dobby.settings.addEnergy && Character.energy == 0) {
            alert("No puedo seguir a causa de la energía");
            return false;
        }
        if(!Dobby.settings.addHealth && Dobby.isHealthBelowLimit()) {
            alert("No puedo seguir a causa de la salud");
            return false;
        }
        return true;
    };
    ...
    Dobby.createSetGui = function() {
        if(Dobby.sets.length == 0) {
            return $("<span style=\'font-size:20px'\>Conjuntos no disponibles</span>");
        }
