...
// @include        https://*.kingdomofloathing.com/choice.php*
// @include        https://*.kingdomofloathing.com/adventure.php?snarfblat=*
// @include        https://*.kingdomofloathing.com/account.php*
// @version        1.1.9
// @grant          GM_getValue
// @grant          GM_setValue
// @downloadURL https://update.greasyfork.org/scripts/4205/NC%20Actionbar.user.js
// @updateURL https://update.greasyfork.org/scripts/4205/NC%20Actionbar.meta.js
// ==/UserScript==
...
//                    '#debug { background-color:#CCCCFF; display:block; font-size:10px; left:0; position:absolute; top:0; }\n'+
    ...
    for (var i=1;i<=nc;i++) {
        ...
        if (b) {
            var cf = document.getElementsByName('choiceform'+i)[0];
            if (cf) {
                cf.addEventListener('submit',recordChoice,true);
			    var cftext = document.evaluate( './/input[@class="button"]', cf, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
                if (cftext.singleNodeValue) {
                    var t = cftext.singleNodeValue.getAttribute('value');
                    b.setAttribute('title',t);
                } else {
                    b.setAttribute('title','Unknown option text');
                }
            } else {
                b.setAttribute('title','Option not available');
            }
