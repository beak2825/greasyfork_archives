...
// @downloadURL https://update.greasyfork.org/scripts/552595/MWITools.user.js
// @updateURL https://update.greasyfork.org/scripts/552595/MWITools.meta.js
// ==/UserScript==
...
(() => {
    ...
    let settingsMap = {
        ...
        forceMWIToolsDisplayZH: {
   		...
   		// Jossyu_01
		excludeBillionUnitInNetWorth: {
			id: "excludeBillionUnitInNetWorth",
			desc: isZH ? "No Chinese Translation - *caution* unofficial revision" : "NetWorth에서 최대 M단위까지만 사용: *주의* 비공식 수정",
			isTrue: true,
		},
    };
    readSettings();
        ...
        const addInventorySummery = async (invElem) => {
            ...
            invElem.insertAdjacentHTML(
                ...
                `<div style="text-align: left; color: ${SCRIPT_COLOR_MAIN}; font-size: 0.875rem;">
                    ...
                    <!-- Jossyu_02 -->
                         ${isZH ? "+ 总NetWorth：" : "+ Total NetWorth: "}${numberFormatter(totalNetworth, undefined, settingsMap.excludeBillionUnitInNetWorth.isTrue)}
                    </div>
                    ...
                    <div id="netWorthDetails" style="display: none; margin-left: 20px;">
                        ...
                        <div id="currentAssets" style="display: none; margin-left: 20px;">
                           <!-- Jossyu_03 -->
							<div>${isZH ? "装备价值：" : "Equipment value: "}${numberFormatter(equippedNetworthAsk, undefined, settingsMap.excludeBillionUnitInNetWorth.isTrue)}</div>
                            <div>${isZH ? "库存价值：" : "Inventory value: "}${numberFormatter(inventoryNetworthAsk, undefined, settingsMap.excludeBillionUnitInNetWorth.isTrue)}</div>
                            <div>${isZH ? "订单价值：" : "Market listing value: "}${numberFormatter(marketListingsNetworthAsk, undefined, settingsMap.excludeBillionUnitInNetWorth.isTrue)}</div>
                        </div>
                        ...
                        <!-- Jossyu_04 -->
                            <div>${isZH ? "房子价值：" : "Houses value: "}${numberFormatter(totalHouseScore * 1000000, undefined, settingsMap.excludeBillionUnitInNetWorth.isTrue)}</div>
                            <div>${isZH ? "技能价值：" : "Abilities value: "}${numberFormatter(abilityScore * 1000000, undefined, settingsMap.excludeBillionUnitInNetWorth.isTrue)}</div>
                        </div>
                    </div>
                </div>`
            );
            ...
            toggleButton.addEventListener("click", () => {
                ...
                toggleButton.textContent =
                    // Jossyu_05
                    (isCollapsed ? "↓ " : "+ ") + (isZH ? "总NetWorth：" : "Total NetWorth: ") + numberFormatter(totalNetworth, undefined, settingsMap.excludeBillionUnitInNetWorth.isTrue);
            ...
            if (targetNode) {
                targetNode.insertAdjacentHTML(
                    ...
                    `<div style="font-size: 13px; font-weight: 500; color: ${SCRIPT_COLOR_MAIN}; text-wrap: nowrap;">Current Assets: ${numberFormatter(
                        networthAsk, undefined, settingsMap.excludeBillionUnitInNetWorth.isTrue
                    )} / ${numberFormatter(networthBid, undefined, settingsMap.excludeBillionUnitInNetWorth.isTrue)}${`<div id="script_api_fail_alert" style="color: ${SCRIPT_COLOR_ALERT};">${
	...
	// Jossyu_07
	function numberFormatter(num, digits = 1, disableBillionUnit = false) {
		...
		// Jossyu_08
		const lookup = disableBillionUnit
			? [
				{ value: 1, symbol: "" },
				{ value: 1e3, symbol: "k" },
				{ value: 1e6, symbol: "M" },
