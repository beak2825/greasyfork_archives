// ==UserScript==
// @name         网课助手
// @namespace    3139849074@qq.com
// @version      1.6.1
// @description  点击下面的 安装此脚本， 安装完成后，打开智慧树登录账号，打开网课脚本会自动运行。请使用前仔细看完下面的说明谢谢！（最底下有惊喜！！！！！！）
// @author       纹煜丶神
// @match        *://*.zhihuishu.com/*
// @connect      http://wpa.qq.com/msgrd?v=3&uin=3139849074&site=qq&menu=yes
// @run-at       document-end
// @grant        unsafeWindow
// @grant        GM_xmlhttpRequest
// @grant        GM_setClipboard
// @supportURL   http://wpa.qq.com/msgrd?v=3&uin=3139849074&site=qq&menu=yes
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/390158/%E7%BD%91%E8%AF%BE%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/390158/%E7%BD%91%E8%AF%BE%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
...
function findAnswer(tip) {
    if (setting.queue.length) {
        ...
        return setting.div.children('div:eq(0)').data('html', tip).siblings('button:eq(0)').hide().click();
    ...
    question = $dom.map(function() {
        return $('img', this).length ? $(this).html() : $(this).text().trim();
    })[0] || $TiMu.find('.subject_describe').map(function() {
        return $('img', this).length ? $(this).html() : $(this).text().trim();
    })[0] || '',
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if (obj.code) {
                    ...
                    obj.data = $('<p></p>').append(obj.data).not('script').text().trim();
                    $(
                        '<tr>' +
...
function fillAnswer($TiMu, obj, type) {
    ...
    obj.code > 0 && $div.each(function() {
        ...
        tip = $('.node_detail', this).text().trim() || new Date().toString();
        if (tip.match(/^(正确|是|对|√)$/)) {
