...
// @description  Collect and earn satoshi ( btc ) via Adbtc , best site to earn
// @version      1.37
// @author       CryptoBR
...
// @noframes
// @license      ElonMuskBR
// @downloadURL https://update.greasyfork.org/scripts/470167/Adbtctop%20collector%20and%20claim%20satoshi%20%28%20BTC%20%29.user.js
// @updateURL https://update.greasyfork.org/scripts/470167/Adbtctop%20collector%20and%20claim%20satoshi%20%28%20BTC%20%29.meta.js
// ==/UserScript==
(function() {
            ...
            if (!stopSolvingCaptcha || !window.location.href.includes("login")) {
                ...
                for (var hc = 0; hc < document.querySelectorAll("iframe").length; hc++) {
                    if (!clicked && document.querySelectorAll("iframe")[hc] &&
                        document.querySelectorAll("iframe")[hc].getAttribute("data-reCaptcha-response") &&
                        document.querySelectorAll("iframe")[hc].getAttribute("data-reCaptcha-response").length > 0) {
                        for (let i = 0; i < websiteDataValues.captchaButtonSubmitSelector.length; i++) {
                            if (document.querySelector(websiteDataValues.captchaButtonSubmitSelector[i])) {
                                document.querySelector(websiteDataValues.captchaButtonSubmitSelector[i]).click();
                            }
