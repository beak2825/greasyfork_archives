...
// @version      2.8
// @description  Quality-of-life tools for the web game Zed City. English-only UI, safer hooks, optional server uploads. Tools for the web game Zed City. English translation from the works of bot7420(yangleda)
// @author       mightyMONTY
...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/524550/ZedTools.user.js
// @updateURL https://update.greasyfork.org/scripts/524550/ZedTools.meta.js
// ==/UserScript==
...
(function() {
  ...
  const getCheckboxStates = () => lsGetJSON("script_gymCheckboxs", {});
  const saveCheckboxStates = (s) => lsSetJSON("script_gymCheckboxs", s);
  ...
  function addGymLocks() {
    ...
    elements.forEach((element, index) => {
      ...
      maxbtn.addEventListener("click", () => {
        ...
        if (!input) return;
        let times = Math.floor(lsGetNumber("script_energy", 0) / 5);
        if (times < 1) times = 1;
  ...
  setInterval(addGymLocks, 1000);
   ...
   // Estimated level-up display
  function updatePlayerXpDisplay() {
    const playerXp = lsGetNumber("script_playerXp_current", 0);
    const xpMax = lsGetNumber("script_playerXp_max", 0);
 
    let levelUpText = "";
    if (localStorage.getItem("script_estimate_levelup_time_switch") === "enabled") {
      const sec = Math.max(0, ((xpMax - playerXp)/72) * 3600); // 72 XP/h heuristic
      levelUpText = `${timeReadableNoSec(sec)} to level up`;
    }
 
    const levelElem = document.body.querySelectorAll(".level-up-cont")[1];
    let wrap = document.getElementById("script_player_level");
    if (levelElem && !wrap) {
      levelElem.insertAdjacentHTML("beforeend",
        `<div id="script_player_level" class="script_do_not_translate">
           <span id="script_player_level_inner" class="script_do_not_translate"></span>
         </div>`);
      wrap = document.getElementById("script_player_level");
    }
    if (wrap) {
      const inner = document.getElementById("script_player_level_inner");
      if (inner) setText(inner, `${Math.floor(playerXp)} / ${Math.floor(xpMax)} ${levelUpText}`);
    }
