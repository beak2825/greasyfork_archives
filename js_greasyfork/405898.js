...
// @downloadURL https://update.greasyfork.org/scripts/405898/yokoto123.user.js
// @updateURL https://update.greasyfork.org/scripts/405898/yokoto123.meta.js
// ==/UserScript==
(function() {
    'use strict';
    //debugger;
    ...
    var openCasenumber = function () {
            ...
            executeOrDelayUntilConditionMeet(
                ...
                function(){
                    if (!$('#RaveEx-OpenCasenumber').length) {
                        $('rct-case-summary label:contains("Case number")').after(`<button id="RaveEx-OpenCasenumber" class="align-right btn btn-success" onclick="RaveEx.openCase()"><span translate="" class="ng-scope">Purus</span></button>`)
                    }1
                }
            )
        }
        var exec = function (tag) {
            var caseController = angular.element($('#breadcrumb')).data().$scope.caseController
            var Casenumber = caseController.Request.RequestData.CaseNumber
