...
// @homepageURL     https://greasyfork.org/zh-CN/users/306515
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/498488/V2EX%20Pro%20Max.user.js
// @updateURL https://update.greasyfork.org/scripts/498488/V2EX%20Pro%20Max.meta.js
// ==/UserScript==
...
(function() {
    ...
    //回复内容
    var replyText = $('div.cell > table > tbody > tr > td:nth-child(3) > div.reply_content').get().map(i=>i.innerHTML);
    var ids = $('div.cell > table > tbody > tr > td:nth-child(3) > strong > a').get().map(i=>i.innerHTML);

    var replyContent = $('div.cell > table > tbody > tr > td:nth-child(3) > div.reply_content');
    var cells = $('#Main > div:nth-child(4) > div[id].cell');

    var user = $('div.cell > table > tbody > tr > td:nth-child(3) > strong > a');
    var author = $('#Main > div:nth-child(2) > div.header > small > a')[0].innerHTML;
    ...
    var replyLink = new Object();
    var replyDeep = new Object();

    function getRootFloor(currentFloor, user1, user2) {
        var pre = replyLink[currentFloor];
        if (pre == undefined) {
            return currentFloor;
        }
        var deep = replyDeep[currentFloor] || 1;
        if (deep >= 6) {
            return getRootFloor(pre, user1, user2);
        }

        if (ids[pre] != user1 && ids[pre] != user2) {
            return currentFloor;
        }
        return getRootFloor(pre, user1, user2);
    }
    ...
    while(i< cells.length){
        if( ids[i] == loginUser )
        {
            user[i].setAttribute('style','color:purple');
        }

        if(replyText[i].match("楼上")){
            replyContent[i-1].append(cells[i]);
        }

        for(var j=i-1;j>=0;j--){
            if(replyText[i].match(ids[j])){
                var root = getRootFloor(j, ids[j], ids[i]);

                var deep = replyDeep[root] || 1;
                replyDeep[i] = deep + 1;

                replyLink[i] = j;
                replyContent[root].append(cells[i]);
                break;
            }
        }
        i++;
    }
    console.log(replyDeep);
})();