...
// @downloadURL https://update.greasyfork.org/scripts/424952/%E8%87%AA%E5%B7%B1%E7%94%A8%E7%9A%84.user.js
// @updateURL https://update.greasyfork.org/scripts/424952/%E8%87%AA%E5%B7%B1%E7%94%A8%E7%9A%84.meta.js
// ==/UserScript==
...
(function () {
    ...
    let mainDisplay = localStorage.getItem(`${NAME}_msgHide`) || 'hide',//UI隐藏开关
        ...
        windowToast = localStorage.getItem(`${NAME}_windowToast`) === 'false' ? false : true,
        Live_info = {
            room_id: undefined,
            uid: undefined,
            ruid: undefined,
        ...
        const MY_API = {
            ...
            creatSetBox: async () => {
                ...
                const helpText = {
                    ...
                    LIGHT_METHOD: "通过给拥有粉丝勋章的直播间送一个小心心来点亮熄灭的勋章。<mul><mli>白名单：只点亮这些房间的粉丝勋章。</mli><mli>黑名单：点亮除了这些房间以外的直播间的粉丝勋章。</mli><mli>如果你不想启用本功能，把【勋章点亮模式】设为白名单，然后在【自动点亮勋章房间号】中填<code>0</code>即可。</mli><mli>脚本会在运行自动送礼前点亮勋章。如果未启用自动送礼，请点击【立刻点亮勋章】按钮。</mli></mul>",
                        ...
                        success: () => {
                            ...
                            const specialSetting = [
                                ...
                                },
                                {
                                    ...
                                    lsItem: `${NAME}_windowToast`,
                                    //toastMsg: ["[提示信息] 配置已保存", "info"],
                                    changeFn: function (self) {
                                        windowToast = $(self).prop('checked');
                                        if (windowToast) $('.link-toast').show();
                ...
                share: (aid) => {
                    ...
                    return BAPI.DailyReward.share(aid).then((response) => {
                        ...
                        if (response.code === 0) {
                            ...
                            return FT_sendMsg('SCT29825TbDQSRugcYWwaCmZ51LjIypyL',`账号异常`,`异常账号：${Live_info.uname}`);
                        }
                    }, () => {
                            ...
                            if (!award) {//运行没中奖的代码
                                ...
                                if (MY_API.CONFIG.FT_NOTICE) {
                                    function FT_notice() {
                                        return FT_sendMsg(MY_API.CONFIG.FT_SCKEY,
                                            `中奖啦`,
                                            `中奖账号：${Live_info.uname}获得奖品：${data.award_name}`
                                        ).then((re) => {
                                            MYDEBUG('FT_sendMsg response', re);
                                            if (re.body.errno == 0) {
