...
// @grant        unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/17930/GetDataFromTests.user.js
// @updateURL https://update.greasyfork.org/scripts/17930/GetDataFromTests.meta.js
// ==/UserScript==
...
var savelink = 'http://gpro.hajnr.linuxpl.info/prt/testing/save';
var checklink = 'http://gpro.hajnr.linuxpl.info/prt/testing/check';
				...
				jq.get(facilities).done(function(data){
					...
					if(team.td>0) {
						...
						jq.get(tdprofile).done(function(data){
							...
							if(typeof unsafeWindow.localStorage.getItem('wasClicked') !== 'undefined') {
								if((ref.localeCompare(url) === 0) && (unsafeWindow.localStorage.getItem('wasClicked') == 1)) {
		...
		var prior = '&stint_priority_text=' + p.text;
			...
			GM_xmlhttpRequest({
	            ...
	            onload: function(data) {
	                ...
	                unsafeWindow.localStorage.setItem('wasClicked', 1);
	                unsafeWindow.localStorage.setItem('raceNumber', info.race);
	                jq("#formQual").submit();             
	            } // zamyka onload
	        });
...
function autosave(thisdata) {
	...
	GM_xmlhttpRequest({
	            ...
	            onload: function(data) {
	                ...
	                unsafeWindow.localStorage.setItem('wasClicked', 0);                
	            } // zamyka onload
	});
}
...
function manualsave(thisdata, info) {
	...
	if(typeof unsafeWindow.localStorage.getItem('raceNumber') === 'undefined') {
		...
		if(unsafeWindow.localStorage.getItem('raceNumber') != info.race) {
...
function getStaffID(data) {
	...
	var driver = jq(".manager-tables:eq(4) tr:eq(3) a:eq(0)", data).attr('href');
	var td = jq(".manager-tables:eq(4) tr:eq(4) a:eq(0)", data).attr('href');

	//console.log(data);
	//console.log(jq("#item-5"), jq("#item-5 table:eq(0)"));
