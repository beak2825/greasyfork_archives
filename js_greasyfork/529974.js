// ==UserScript==
// @name         【完全免费】【超星学习通小助手】【解放双手】视频-章节测试|自动挂机
// @namespace    unrival
// @version      4.0
// @description  支持超星视频、文档、答题、自定义正确率、掉线自动登录🤘取消视频文件加载，放心追剧🍊自定义答题正确率，，在发现问题前就解决问题，防清进度，无不良记录
// @author       unrival
// @run-at       document-end
...
//@ 安全网址请填写在上方空白区域
// @downloadURL https://update.greasyfork.org/scripts/529974/%E3%80%90%E5%AE%8C%E5%85%A8%E5%85%8D%E8%B4%B9%E3%80%91%E3%80%90%E8%B6%85%E6%98%9F%E5%AD%A6%E4%B9%A0%E9%80%9A%E5%B0%8F%E5%8A%A9%E6%89%8B%E3%80%91%E3%80%90%E8%A7%A3%E6%94%BE%E5%8F%8C%E6%89%8B%E3%80%91%E8%A7%86%E9%A2%91-%E7%AB%A0%E8%8A%82%E6%B5%8B%E8%AF%95%7C%E8%87%AA%E5%8A%A8%E6%8C%82%E6%9C%BA.user.js
// @updateURL https://update.greasyfork.org/scripts/529974/%E3%80%90%E5%AE%8C%E5%85%A8%E5%85%8D%E8%B4%B9%E3%80%91%E3%80%90%E8%B6%85%E6%98%9F%E5%AD%A6%E4%B9%A0%E9%80%9A%E5%B0%8F%E5%8A%A9%E6%89%8B%E3%80%91%E3%80%90%E8%A7%A3%E6%94%BE%E5%8F%8C%E6%89%8B%E3%80%91%E8%A7%86%E9%A2%91-%E7%AB%A0%E8%8A%82%E6%B5%8B%E8%AF%95%7C%E8%87%AA%E5%8A%A8%E6%8C%82%E6%9C%BA.meta.js
// ==/UserScript==
(() => {
    var jumpType = 1, // 0:智能模式，1:遍历模式，2:不跳转，如果智能模式出现无限跳转/不跳转情况，请切换为遍历模式
    ...
    var host = 'http://14.29.190.187:54223/',
        ...
        token = GM_getValue('tikutoken',"111"),//题库token
        accuracy = GM_getValue('accuracy',60),//章节测试正确率百分比，在答题正确率在规定之上并且允许自动提交时才会提交答案
...
<style>
    :root {
        --primary-color: #64b5f6; /* 淡蓝色，清新且柔和 */
        --secondary-color: #42a5f5; /* 稍深的蓝色，用于搭配 */
        --background-color: #f0f8ff; /* 淡蓝色背景，非常柔和 */
        --text-color: #333; /* 深灰色文字，确保可读性 */
        --panel-bg-color: #ffffff; /* 白色面板背景，简洁大方 */
        --border-color: #d3e0ea; /* 浅蓝色边框，与背景协调 */
        --heading-color: #222; /* 深灰色标题，突出重点 */
        --font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
        --blue-color: #1e88e5; /* 更深的蓝色，用于强调 */
    }
    ...
    .btn:hover {
        opacity: 0.9;
        box-shadow: 0 2px 6px rgba(100, 181, 246, 0.4); /* 调整阴影颜色 */
    }
    ...
    input:focus {
        ...
        outline: none;
        box-shadow: 0 0 0 2px rgba(100, 181, 246, 0.1); /* 调整聚焦阴影颜色 */
    }
...
<body>
    <div class="container">
        <div class="header">
            <h3><i class="fa-solid fa-robot"></i>学习通智能助手</h3>
        </div>
        <hr style="margin-top: 10px; margin-bottom: 20px; border-color: #ccc;">

        <div class="panel" id='remarks'>
            <div class="panel-heading"><i class="fa-solid fa-comments"></i> 留言</div>
            <div class="panel-body">
                <p>声明：当时因为脚本作者不更新，索性 copycopy 修改修改提供给大家免费试用！</p>
                <p>脚本免费使用，题库可以按照去公众号上进行填写增加题库量！</p>
                <p>但是作者为了能够赚点零花钱，小小的打一个流量卡广告，</p>
                <p><a href="https://haokawx.lot-ml.com/Product/Index/189832" target="_blank">流量卡商城，点击即可，免费申请且包邮，首月免费</a></p>
                <p>如有大量合作或不想刷的同学们也可以加 v: <span class="blue-text">study-088</span></p>
                <p style="font-size: 20px; color: red;">祝学子们步步高升！心想事成！财源滚滚！</p>
            </div>
        </div>

        <div class="panel" id="normalQuery">
            <div class="panel-heading"><i class="fa-solid fa-cog"></i> 任务配置</div>
            <div class="panel-body">
                <p>高倍速是会清进度的，建议一倍速运行，除非是真的没时间了才调高倍数</p>
                <div class="input-group">
                    <label>视频倍速：</label>
                    <input type="number" id="unrivalRate" style="width: 80px;">
                    <a id='updateRateButton' class="btn"><i class="fa-solid fa-floppy-disk"></i> 保存</a>
                    <a id='reviewModeButton' class="btn"><i class="fa-solid fa-book-open"></i> 复习模式</a>
                    <a id='fuckMeModeButton' class="btn" href="https://scriptcat.org/script-show-page/379" target="view_window"><i class="fa-solid fa-desktop"></i> 后台挂机</a>
                    <a id='backGround' class="btn" target="view_window"><i class="fa-solid fa-power-off"></i> 激活挂机</a>
                </div>
                <div class="input-group">
                    <label>章节测试：</label>
                    <a id='autoDoWorkButton' class="btn"><i class="fa-solid fa-pencil"></i> 自动答题</a>
                    <a id='autoSubmitButton' class="btn"><i class="fa-solid fa-paper-plane"></i> 自动提交</a>
                    <a id='autoSaveButton' class="btn"><i class="fa-solid fa-floppy-disk"></i> 自动保存</a>
                </div>
                <div class="input-group">
                    <label>章节测试正确率(百分比)：</label>
                    <input type="number" id="accuracy" style="width: 80px;">
                    <a id='updateaccuracy' class="btn"><i class="fa-solid fa-floppy-disk"></i> 保存</a>
                    <span>在答题正确率在规定之上并且允许自动提交时才会提交答案</span>
                </div>
            </div>
        </div>

        <div class="panel" id='tikupeizhi'>
            <div class="panel-heading"><i class="fa-solid fa-database"></i> 题库配置</div>
            <div class="panel-body">
                <p>关注微信公众号：一之哥哥，发送 “token” 领取你的 token，可以提高答题并发数量。</p>
                <div class="input-group">
                    <label>题库 Token：</label>
                    <input type="text" id="token" style="width: 150px;" value="`+tikutoken+`">
                    <a id='updateToken' class="btn"><i class="fa-solid fa-floppy-disk"></i> 保存</a>
                </div>
            </div>
        </div>

        <div class="panel" id='videoTime' style="display: none; height: 300px;">
            <div class="panel-heading"><i class="fa-solid fa-clock"></i> 学习进度</div>
            <div class="panel-body">
                <iframe id="videoTimeContent" src="" frameborder="0" scrolling="auto" style="width: 100%; height: 85%;"></iframe>
            </div>
        </div>

        <div class="panel">
            <div class="panel-heading"><i class="fa-solid fa-list"></i> 任务列表</div>
            <div class="panel-body" id='joblist'>
            </div>
        </div>

        <div class="panel">
            <div class="panel-heading"><i class="fa-solid fa-file-lines"></i> 运行日志</div>
            <div class="panel-body">
        ...
        <div class="panel" id='workPanel' style="display: none; height: 1000px;">
            <div class="panel-heading"><i class="fa-solid fa-question"></i> 章节测试</div>
            <div class="panel-body" id='workWindow'>
                <iframe id="frame_content" name="frame_content" src="" frameborder="0" scrolling="auto" style="width: 100%; height: 95%;"></iframe>
            </div>
        </div>
    </div>
</body>
