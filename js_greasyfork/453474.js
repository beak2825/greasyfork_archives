// ==UserScript==
// @name         forDianpingPOI评论信息采集工具
// @namespace    http://tampermonkey.net/
// @version      0.10
// @description  获取大众点评网页评论(用户名、发布时间、评论内容),解决动态字体加密
// @author       You
...
// @downloadURL https://update.greasyfork.org/scripts/453474/forDianpingPOI%E8%AF%84%E8%AE%BA%E4%BF%A1%E6%81%AF%E9%87%87%E9%9B%86%E5%B7%A5%E5%85%B7.user.js
// @updateURL https://update.greasyfork.org/scripts/453474/forDianpingPOI%E8%AF%84%E8%AE%BA%E4%BF%A1%E6%81%AF%E9%87%87%E9%9B%86%E5%B7%A5%E5%85%B7.meta.js
// ==/UserScript==
...
(function () {
    ...
    const getResult3 = async () => {
        ...
        const times = [...$('.time')];
        var timeIndex = 0;
        const names = [...$('.name')];//包含点评网登录用户名，每页15个评论的话，返回16个，其中第一个是登录用户名
        ...
        comments.forEach(cmt => {
            ...
            foo = foo.replace(/,/ig,'，');
            result.push([time, name, foo.trim()]);
            timeIndex++;
            nameIndex++;
        });
    ...
    //测试SVG加密字体文件格式为<path>和<textpath>组合出现的情况
    ...
    getUserCommentTime.style.bottom = '120px';
    ...
    getUserTimeCommentScoreUrl.style.bottom = '160px';
    getUserTimeCommentScoreUrl.id = "UserTimeCommentScoreUrl";

    getUserTimeCommentScoreUrl2.innerHTML = '采集用户名、时间、评论和打分(模式2)';
    getUserTimeCommentScoreUrl2.style.position = 'fixed';
    getUserTimeCommentScoreUrl2.style.right = '20px';
    getUserTimeCommentScoreUrl2.style.bottom = '180px';
    ...
    getUserCommentTime.onclick = async () => {
        const moreBtns = $(moreBtnClass);
        moreBtns.forEach(b => {
            b.click();
            b.style.opacity = 0;
