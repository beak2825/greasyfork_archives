// ==UserScript==
// @name         Trivia Winner
// @version      0.1.4.1
// @description  Win trivias as fast as possible
// @author       Lokilo
// @match        https://gamdom.com/roulette
// @grant        GM_notification
// @grant        none
// @namespace http://tampermonkey.net/
// @downloadURL https://update.greasyfork.org/scripts/368811/Trivia%20Winner.user.js
// @updateURL https://update.greasyfork.org/scripts/368811/Trivia%20Winner.meta.js
// ==/UserScript==

(function() {
    var match = ["k alattam","c alattam","k under me","c under me"];
    var nomatch = ["\w alattam \w","alattam \w","^alattam$"];
    var matches = new RegExp(match.join("|"), "i");
    var nomatches = new RegExp(nomatch.join("|"), "i");
    var text="",rtext="",on=0,percentage=70,textmatch,notextmatch,s=1,statusfield="",defstatus="",loaded=false,spam=0;
    const audio = new Audio("data:audio/mp3;base64,");
    function switcher (status) {
        var onbutton = document.getElementsByClassName("checkbox")[0];
        if(status=="on") {onbutton.checked=true;}
        if(status=="off") {onbutton.checked=false;}
    if(onbutton.checked) {on=1;} else {on=0;}
    }
    function percentagechange(value) {
    percentage=value;
    }
    function check () {
    var checkelement = document.querySelector('.chat-input');
    var checktxt = checkelement.placeholder;
    if(checktxt=="Type here...") {
        loaded = true;
        return true;
    }
        else
        {            
            return false;
        }
    }
    function ui () {
    var body=document.getElementsByTagName("body")[0];
    var style=document.createElement("style");
        body.appendChild(style);
        style.innerHTML='.onbuttonlabel {dispaly:inline-block;position: relative;padding-left: 35px;margin-left:5px;margin-bottom: 18px;cursor: pointer;font-size: 22px;-webkit-user-select: none;-moz-user-select: none;-ms-user-select: none;user-select: none;}.onbuttonlabel input {position: absolute;opacity: 0;cursor: pointer;}.onbutton {position: absolute;top: 0;left: 0;height: 25px;width: 25px;background-color: red;}'
        style.innerHTML+='.onbuttonlabel:hover input ~ .onbutton {background-color: #850000;}.onbuttonlabel input:checked ~ .onbutton {background-color: #08b96f;}';
        style.innerHTML+='.title {color:#fff;font-size:20px;}';
        style.innerHTML+='.scriptcont {width:100%;height:100px;overflow:scroll;background:#101014;z-index:100;position:absolute;margin-top:35px;border-bottom:#08974f solid 3px;}';
        style.innerHTML+='.scriptcont .ondiv {width:auto;color:#fff;display:inline-block;}';
        style.innerHTML+='.scriptcont a {color:#fff;cursor:default;}'
        style.innerHTML+='.scriptcont .pdiv {display:inline-block;float:left;margin-top:10px;margin-left:10px;}'
        style.innerHTML+='.scriptcont .sdiv {display:inline-block;float:right;margin-top:10px;margin-right:10px;}'
    var uibg = document.createElement("div");
    var chatbar = document.getElementsByClassName("tab-container noselect")[0];
        var uibgatt = document.createAttribute("class");
        uibgatt.value = "scriptcont";
        uibg.setAttributeNode(uibgatt);
        chatbar.appendChild(uibg);
    var center = document.createElement("center");
        uibg.appendChild(center);
    var title = document.createElement("h1");
        center.appendChild(title);
        title.innerHTML="Trivia Winner Script";
        var titleatt = document.createAttribute("class");
        titleatt.value = "title";
        title.setAttributeNode(titleatt);
    var pdiv = document.createElement("div");
        uibg.appendChild(pdiv);
        var pdivatt = document.createAttribute("class");
        pdivatt.value = "pdiv";
        pdiv.setAttributeNode(pdivatt);
    var ondiv = document.createElement("div");
        pdiv.appendChild(ondiv);
        var ondivatt = document.createAttribute("class");
        ondivatt.value = "ondiv";
        ondiv.setAttributeNode(ondivatt);    
    var ontext = document.createElement("a");
    var onbutton = document.createElement("input");
        var realonbuttonatt = document.createAttribute("class");
        realonbuttonatt.value = "checkbox";
        onbutton.setAttributeNode(realonbuttonatt);
        var onbuttonlabel = document.createElement("label");
        var onbuttonattlabel = document.createAttribute("class");
        onbuttonattlabel.value = "onbuttonlabel";
        onbuttonlabel.setAttributeNode(onbuttonattlabel);
        var onbuttonspan = document.createElement("span");
        var onbuttonatt = document.createAttribute("class");
        onbuttonatt.value = "onbutton";
        onbuttonspan.setAttributeNode(onbuttonatt);
        ondiv.appendChild(ontext);
        ontext.innerHTML="Active: ";
        pdiv.appendChild(onbuttonlabel);
        onbuttonlabel.appendChild(onbutton);
        onbuttonlabel.appendChild(onbuttonspan);
        onbutton.type="checkbox";
    var sdiv = document.createElement("div");
        uibg.appendChild(sdiv);
        var sdivatt = document.createAttribute("class");
        sdivatt.value = "sdiv";
        sdiv.setAttributeNode(sdivatt);
    var perdiv = document.createElement("div");
        sdiv.appendChild(perdiv);
    var perdivatt = document.createAttribute("class");
        perdivatt.value = "perdiv";
        perdiv.setAttributeNode(perdivatt);
        if(on==1) {onbutton.checked=true;} else {onbutton.checked=false;}
    var pera= document.createElement("a");
        perdiv.appendChild(pera);
    var peraatt = document.createAttribute("class");
        peraatt.value = "pera";
        pera.setAttributeNode(peraatt);
        pera.innerHTML="Winrate: "+percentage+"%";
    }
    function random () {
        var rand = Math.floor(Math.random() * 10)
        if(rand<5) {
            rtext="";
            for(var i=0;i<=Math.floor(Math.random() * 5);i++) {
          var chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
          rtext+=chars.charAt(Math.floor(Math.random() * chars.length));
            }
        }
        else
        {
            var words = ["Ohh","<3","asd","lol",":D",":P"]
            rtext=words[Math.floor(Math.random() * words.length)];
        }
        return rtext;
    }    
function chatwriter () {
		var input = document.querySelector('.chat-input');

		if (input) {
			input.value = random();

			var inputSend = document.querySelector('.chatInputSend');
			if (inputSend) {
				inputSend.click();
			}
		}
	};
    function chatreader () {
    var msgs = document.querySelectorAll('.messages>li>span');
    //console.log(Math.floor(Math.random()*100)<=70);
    for(var i=0;i<=msgs.length-1;i++) {
    var lastmsg=msgs[i];
    }
        if(on==1) {
            text=lastmsg.innerHTML;
            textmatch = matches.test(text);
            notextmatch = nomatches.test(text);
            if(textmatch && !notextmatch && spam == 0 ) {
            audio.play();
                var writerand = Math.floor(Math.random()*100);
                var wrt = 2000+Math.floor(Math.random()*5000);
                if(writerand>=percentage && spam==0) {
                    console.log("Chance in "+wrt+" milliseconds");
                    setTimeout(function() {                   
                    chatwriter();                    
                    },wrt);
                    spam=1;
                }
                else
                {
                    console.log("Immediately");
                    spam=1;
                    chatwriter();
                }
        }
            if(!textmatch) {
            spam=0;
            }
    //console.log("Utolsó üzenet: "+lastmsg.innerHTML);
        }
        else
        {
            text=lastmsg.innerHTML;
            textmatch = matches.test(text);
            notextmatch = nomatches.test(text);
            if(textmatch && !notextmatch) {
            audio.play();
        }
        //console.log("The script isn't running!");
        }
}
    function commands() {
        var chatfield = document.querySelector('.chat-input');
        var chattext=chatfield.value;        
        //var commandmatch = ["/winrate"];
        //var commandmatches = new RegExp(match.join("|"), "i");
        var commandmatches = new RegExp('\/winrate', "gmi");
        if(chattext=="/toggle") {
            //console.log("toggled")
            if(on==1){on=0;console.log("The script has been stopped!");switcher("off");}else{on=1;console.log("The script has been started!");switcher("on");}
            chatfield.value="";
        }       
        var cm=commandmatches.test(chattext);
        if(cm!=false) {            
            var splitct=chattext.split(" ");            
            if(splitct[1]>=10 && splitct[1]<=100) {
            percentagechange(splitct[1]);                
            chatfield.value="";
            }
        }
    }
    function uirefresh() {
    var pertext=document.getElementsByClassName("pera")[0];
    pertext.innerHTML="Winrate: "+percentage+"%";
    }
    function statustext() {
    statusfield = document.querySelector('.chat-input');
    defstatus= statusfield.placeholder;
    }
    function status() {
        s++;
        if(s%2) {
            statusfield.placeholder=defstatus;
    }
        else
        {
        if(on==1) {statusfield.placeholder="Trivia winning script is ruinning...";} else {statusfield.placeholder="Trivia winning script isn't running...";}
        }
    }
    //setInterval(function() {console.log(random());},100);
    setInterval(check,1000);
    setTimeout(function() {
        if(loaded) {
        setTimeout(ui,1000);
        setTimeout(statustext,4990);
        setInterval(uirefresh,100);
        setInterval(switcher,500);
        setInterval(commands,100);
        setInterval(status,5000);
        setInterval(chatreader,100);
        }
        else
        {
        alert("Refresh the site please.");
        }
    },10000)
})();