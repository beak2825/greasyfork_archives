...
// ==UserScript==
// @name         Glovia Updater OEO / ON PROGRESS
...
// @downloadURL https://update.greasyfork.org/scripts/542017/Glovia%20Updater%20OEO%20%20ON%20PROGRESS.user.js
// @updateURL https://update.greasyfork.org/scripts/542017/Glovia%20Updater%20OEO%20%20ON%20PROGRESS.meta.js
// ==/UserScript==
...
processButton.addEventListener('click', async () => {
    try {
        ...
        console.log('[Delivery Date Updater] Starting to process lines:', lines);
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // Try to match both date formats and handle both space and tab separators
            // Format examples: "13/06/25 80114 7 1" or "13-6-2025\t80114\t7\t1"
            const match = line.match(/(\d{1,2}[-\/]\d{1,2}[-\/]\d{2,4})[\s\t]+([\w\d]+)[\s\t]+(\d+)(?:[\s\t]+(\d+))?/);
            console.log('[Delivery Date Updater] Parsing line:', line, 'Match result:', match);
            ...
            console.log('[Delivery Date Updater] Extracted values:', {
                rawDateStr,
                orderNumber,
                lineNumber,
                deliveryLine
            ...
            // Process date format (convert from either format to MM/DD/YY)
            let dateStr = rawDateStr;
            let convertedDate = '';
            
            if (rawDateStr.includes('-')) {
                try {
                    // Handle D-M-YYYY format (e.g., 13-6-2025)
                    const dateParts = rawDateStr.split('-');
                    
                    // Ensure we have valid parts
                    if (dateParts.length !== 3) {
                        throw new Error(`Invalid date format: ${rawDateStr}`);
                    }
                    ...
                    // The convertedDate should be without slashes for typing
                    convertedDate = `${month}${day}${year}`;
                    console.log(`[Delivery Date Updater] Converted date format from ${rawDateStr} to ${dateStr}, for typing: ${convertedDate}`);
                } catch (error) {
                    console.error(`[Delivery Date Updater] Error converting date: ${error.message}`);
                    updateStatus(`Error converting date format: ${rawDateStr}`, 'error');
                    dateStr = rawDateStr; // Fall back to original format
                    convertedDate = rawDateStr.replace(/\//g, '');
                }
            } else {
            ...
            try {
                ...
                // Update regex for next order detection to handle tabs too
                const nextLineMatch = !isLastLine ? lines[i + 1].match(/\d{1,2}[-\/]\d{1,2}[-\/]\d{2,4}[\s\t]+([\w\d]+)/) : null;
                const nextOrderNumber = nextLineMatch ? nextLineMatch[1] : null;
                console.log('[Delivery Date Updater] Next order check:', nextOrderNumber, 'Current:', orderNumber, 'Needs return:', nextOrderNumber && nextOrderNumber !== orderNumber);
                const needsReturn = isLastLine || (nextOrderNumber && nextOrderNumber !== orderNumber);

                // Log what we're about to call the function with
                console.log('[Delivery Date Updater] Calling processLineUpdate with params:', {
                    orderNumber,
                    lineNumber,
                    dateStr,
                    convertedDate,
                ...
                const success = await processLineUpdate(
                    orderNumber, 
                    lineNumber, 
                    dateStr,           // This is the MM/DD/YY format with slashes 
                    convertedDate,     // This is the MMDDYY format without slashes for typing
                    speedMultiplier, 
                    previousOrderNumber, 
                    deliveryLine, 
...
///////////
/**
 * Creates a confirmation dialog for user interaction.
 * @returns {Object} Object containing dialog, confirmButton, cancelButton, and message elements.
 */
function createConfirmationDialog() {
    ...
    message.id = 'confirmationDialogMessage';
    ...
    return { dialog, confirmButton, cancelButton, message };
}

/**
 * Converts a YYYYMMDD date string to DD/MM/YYYY format with validation.
 * @param {string} yyyymmdd - Date in YYYYMMDD format.
 * @returns {Object} Object with normalized (YYYYMMDD) and display (DD/MM/YYYY) formats, or null if invalid.
 */
function convertYYYYMMDDtoDDMMYYYY(yyyymmdd) {
    if (!yyyymmdd || yyyymmdd.length !== 8) return { normalized: null, display: null };
    ...
    if (!/^\d{4}$/.test(year) || !/^\d{2}$/.test(month) || !/^\d{2}$/.test(day)) {
        console.log(`[Date Converter] Invalid YYYYMMDD format: ${yyyymmdd}`);
        return { normalized: null, display: null };
    }
    
    // Validate as an actual date
    const date = new Date(`${year}-${month}-${day}`);
    if (isNaN(date.getTime())) {
        console.log(`[Date Converter] Invalid date: ${year}-${month}-${day}`);
        return { normalized: null, display: null };
    }
    ...
    console.log(`[Date Converter] Converted YYYYMMDD → DD/MM/YYYY: ${yyyymmdd} → ${display}`);
    return { normalized, display };
}

/**
 * Converts a DD/MM/YY date string to DD/MM/YYYY by prepending "20" to the year.
 * @param {string} dateStr - Date in DD/MM/YY format (e.g., "27/02/26").
 * @returns {string|null} Date in DD/MM/YYYY format (e.g., "27/02/2026"), or null if invalid.
 */
function convertDDMMYYtoDDMMYYYY(dateStr) {
    if (!/^\d{2}\/\d{2}\/\d{2}$/.test(dateStr)) {
        console.error(`[Date Converter] Invalid DD/MM/YY format: ${dateStr}`);
        return null;
    }
    ...
    if (parseInt(day) < 1 || parseInt(day) > 31 || parseInt(month) < 1 || parseInt(month) > 12) {
        console.error(`[Date Converter] Invalid day or month: ${dateStr}`);
        return null;
    }
...
/**
 * Normalizes various date formats to YYYYMMDD for internal use.
 * @param {string} dateStr - Date string in various formats.
 * @returns {string|null} Date in YYYYMMDD format, or null if invalid.
 */
function normalizeDate(dateStr) {
    ...
    // CASE 1: If already in YYYYMMDD format (8 digits)
    if (/^\d{8}$/.test(cleaned)) {
        console.log(`[Date Normalizer] Already normalized: ${cleaned}`);
        return cleaned;
    }
    
    // CASE 2: Handle DD/MM/YYYY or DD/MM/YY formats
    if (cleaned.includes('/')) {
        const parts = cleaned.split('/');
        if (parts.length !== 3) {
            console.log(`[Date Normalizer] Invalid format (wrong number of parts): ${cleaned}`);
            return null;
        }
        ...
        // Handle 2-digit years (YY → YYYY)
        if (year.length === 2) {
            year = `20${year}`;
            console.log(`[Date Normalizer] Expanded 2-digit year: ${parts[2]} → ${year}`);
        } else if (year.length !== 4) {
            console.log(`[Date Normalizer] Invalid year format: ${year}`);
            return null;
        }
        ...
        const date = new Date(`${year}-${month}-${day}`);
        if (isNaN(date.getTime())) {
            console.log(`[Date Normalizer] Invalid date: ${day}/${month}/${year}`);
            return null;
        }
        
        const normalized = `${year}${month}${day}`;
        console.log(`[Date Normalizer] Normalized DD/MM/YYYY → YYYYMMDD: ${cleaned} → ${normalized}`);
        return normalized;
    }
    
    // CASE 3: Handle YYYY-MM-DD
    if (cleaned.includes('-')) {
        const parts = cleaned.split('-');
        if (parts.length !== 3) {
            console.log(`[Date Normalizer] Invalid hyphen format: ${cleaned}`);
            return null;
        }
        ...
        if (year.length !== 4) {
            console.log(`[Date Normalizer] Invalid year in YYYY-MM-DD: ${year}`);
            return null;
        }
        ...
        const date = new Date(`${year}-${month}-${day}`);
        if (isNaN(date.getTime())) {
            console.log(`[Date Normalizer] Invalid YYYY-MM-DD date: ${cleaned}`);
            return null;
        }
        
        const normalized = `${year}${month}${day}`;
        console.log(`[Date Normalizer] Normalized YYYY-MM-DD → YYYYMMDD: ${cleaned} → ${normalized}`);
        return normalized;
    }
    
    console.log(`[Date Normalizer] Unrecognized date format: ${cleaned}`);
    return null;
}

/**
 * Processes a delivery line update with date comparison and user confirmation.
...
async function processLineUpdate(orderNumber, lineNumber, dateStr, newDate, speedMultiplier, previousOrderNumber, deliveryLine, skipConfirmationCheckbox, inputLine, failedOrders, updateFailedOrders, autoSkipCheckbox) {
    return new Promise(async (resolve) => {
        try {
            // Check if navigation to a new order is needed
            const needsNavigation = !previousOrderNumber || orderNumber !== previousOrderNumber;
            if (needsNavigation) {
                updateStatus(`🌐 Navigating to Deliveries for order ${orderNumber}`, 'processing');
                ...
                // Attempt to navigate to the Deliveries tab
                ...
                updateStatus(`🔄 Reusing order ${orderNumber}, skipping navigation`, 'processing');
            }
            ...
            // Add stop delay before checking date
            updateStatus(`⏳ Pausing to inspect delivery date field...`, 'processing');
            await humanDelay(config.humanDelayBeforeDateCheck || 2000, speedMultiplier);

            // Check if current date matches the input date
            updateStatus(`🔍 Checking current delivery date...`, 'processing');
            const currentDate = deliveryDateField.value.trim(); // e.g., "27/02/2026"
            let inputDateFull = dateStr;

            // Convert input date from DD/MM/YY to DD/MM/YYYY if needed
            if (/^\d{2}\/\d{2}\/\d{2}$/.test(dateStr)) {
                inputDateFull = convertDDMMYYtoDDMMYYYY(dateStr); // e.g., "27/02/26" → "27/02/2026"
                if (!inputDateFull) {
                    updateStatus(`❌ Invalid input date format: ${dateStr}`, 'error');
                    if (!failedOrders.includes(inputLine)) {
                        failedOrders.push(inputLine);
                        updateFailedOrders(failedOrders);
                    }
            ...
            console.log(`[Delivery Date Updater] Current field value: ${currentDate}, Input date (full): ${inputDateFull}`);
            const datesMatch = currentDate === inputDateFull;

            console.log(`[Delivery Date Updater] Comparison - Current: ${currentDate}, Input: ${inputDateFull}, Match: ${datesMatch}`);

            if (datesMatch) {
                updateStatus(`⚠️ Date already matches: Current=${currentDate}, Input=${dateStr}`, 'warning');
                deliveryDateField.style.backgroundColor = '#ffffcc';
                deliveryDateField.style.border = '2px solid #ffc107';

                window.scriptIsPaused = true;
                const shouldSkip = await new Promise((confirmResolve) => {
                    let dialog = document.getElementById(config.confirmationDialogId);
                    let confirmButton, cancelButton, messageElement;
                    ...
                    if (!dialog) {
                        ...
                        messageElement = dialogElements.message;
                    } else {
                        confirmButton = dialog.querySelector('button:nth-child(2)');
                        cancelButton = dialog.querySelector('button:nth-child(1)');
                        messageElement = dialog.querySelector('#confirmationDialogMessage');
                    }

                    messageElement.textContent = `Delivery date matches for line ${lineNumber}${deliveryLine ? `, Delivery ${deliveryLine}` : ''}: ${currentDate}. Skip saving?`;
                    confirmButton.textContent = 'Skip';
                    cancelButton.textContent = 'Save Anyway';
                    dialog.style.display = 'flex';

                    confirmButton.onclick = () => {
                        dialog.style.display = 'none';
                        window.scriptIsPaused = false;
                        confirmResolve(true); // Skip
                    };

                    cancelButton.onclick = () => {
                        dialog.style.display = 'none';
                        window.scriptIsPaused = false;
                        confirmResolve(false); // Proceed
                    };
                });
                ...
                if (shouldSkip) {
                    updateStatus(`⏩ SKIPPED: Line ${lineNumber}${deliveryLine ? `, Delivery ${deliveryLine}` : ''} already has date ${currentDate}`, 'success');
                    ...
                    updateStatus(`🔄 Proceeding with update despite matching dates...`, 'processing');
                }
            } else {
                updateStatus(`ℹ️ Dates do not match: Current=${currentDate}, Input=${dateStr}. Proceeding with update...`, 'processing');
            }

            // Convert newDate to YYYYMMDD if necessary for system input
            let formattedNewDate = newDate;
            const normalizedNewDate = normalizeDate(newDate);
            if (normalizedNewDate) {
                formattedNewDate = normalizedNewDate; // e.g., "20260206"
            }
            ...
            // Type new date
            updateStatus(`⌨️ Typing new date: ${formattedNewDate}`, 'processing');
            await humanTypeInto(deliveryDateField, formattedNewDate, speedMultiplier);
            ...
            if (!skipConfirmation) {
                ...
                shouldSave = await new Promise((confirmResolve) => {
                    ...
                    messageElement.textContent = `Do you want to save the adjusted delivery date for line ${lineNumber}${deliveryLine ? `, Delivery ${deliveryLine}` : ''}?`;
                    ...
                    confirmButton.onclick = () => {
                        dialog.style.display = 'none';
                        window.scriptIsPaused = false;
                        confirmResolve(true); // Save
                    };

                    cancelButton.onclick = () => {
                        dialog.style.display = 'none';
                        window.scriptIsPaused = false;
                        confirmResolve(false); // Cancel
                    };
                });
            }

            if (!shouldSave) {
                updateStatus(`❌ Save cancelled by user, attempting to navigate to order number field`, 'warning');

                // First try clicking the Cancel button if it exists
                const cancelButton = document.querySelector('#SORDS_mn_active, li.k-item[text="Cancel"]');
                let cancelButtonClicked = false;

                if (cancelButton && cancelButton.offsetParent !== null) {
                    updateStatus(`🖱️ Found Cancel button, attempting to click it...`, 'processing');
                    try {
                        cancelButton.click();
                        await humanDelay(1500, speedMultiplier);
                        cancelButtonClicked = true;
                        updateStatus(`✓ Cancel button clicked successfully`, 'success');
                    } catch (err) {
                        updateStatus(`⚠️ Failed to click Cancel button: ${err.message}`, 'warning');
                        console.error('[Delivery Date Updater] Cancel button click error:', err);
                    }
                } else {
                    updateStatus(`ℹ️ No Cancel button found, will use Escape key sequence`, 'processing');
                }

                // If button click failed or button wasn't found, try ESC key sequence
                if (!cancelButtonClicked) {
                    ...
                    if (!orderNumberField.offsetParent || orderNumberField.disabled) {
                        updateStatus(`⚠️ Order number field is not visible or focusable`, 'warning');
                        if (!failedOrders.includes(inputLine)) {
                            failedOrders.push(inputLine);
                            updateFailedOrders(failedOrders);
                        }
                    ...
                    while (document.activeElement !== orderNumberField && attempts < maxAttempts) {
                        ...
                        const activeElementInfo = activeElement.id || activeElement.tagName || 'unknown';

                        updateStatus(`🔄 Pressing Escape to reach order number field (attempt ${attempts + 1}/${maxAttempts}, active: ${activeElementInfo})`, 'processing');

                        if (activeElement === lastActiveElement) {
                            consecutiveNoChangeCount++;
                            if (consecutiveNoChangeCount >= 3) {
                                updateStatus(`⚠️ Stuck on same element after multiple ESC presses, trying alternative approach`, 'warning');
                                break;
                            }
                        } else {
                        ...
                        const errorDialogResult = await handleErrorDialog(inputLine, failedOrders, updateFailedOrders, speedMultiplier, autoSkipCheckbox);
                        if (errorDialogResult.shouldSkip) {
                            updateStatus(`⏩ Skipped line due to error during cancellation: ${inputLine}`, 'warning');
                            if (!failedOrders.includes(inputLine)) {
                                failedOrders.push(inputLine);
                                updateFailedOrders(failedOrders);
                            }
                            ...
                            updateStatus(`🛑 Processing stopped by user during cancellation`, 'warning');
                            resolve(false);
                            return;
                        }

                        if (document.activeElement === orderNumberField) {
                            updateStatus(`✓ Order number field is now focused`, 'success');
                            break;
                        }
                ...
                // Check if order number field is now focused
                const orderNumberField = document.getElementById(config.orderNumberFieldId);
                if (document.activeElement === orderNumberField) {
                    updateStatus(`✓ Reached order number field after cancellation`, 'success');
                    orderNumberField.focus();
                    await humanDelay(500, speedMultiplier);
                } else {
                    updateStatus(`🔙 Order number field not reached, attempting fallback navigation`, 'processing');
                    const returnSuccess = await returnToOrderNumberField(speedMultiplier, inputLine, failedOrders, updateFailedOrders, autoSkipCheckbox);
                    if (returnSuccess && document.activeElement === orderNumberField) {
                        updateStatus(`✓ Fallback navigation succeeded, reached order number field`, 'success');
                        orderNumberField.focus();
                        await humanDelay(500, speedMultiplier);
                    } else {
                        updateStatus(`❌ All navigation attempts failed, marking order as failed`, 'error');
                        if (!failedOrders.includes(inputLine)) {
                            failedOrders.push(inputLine);
                            updateFailedOrders(failedOrders);
                        }
                ...
                // Mark order as failed due to cancellation
                if (!failedOrders.includes(inputLine)) {
                    failedOrders.push(inputLine);
                    updateFailedOrders(failedOrders);
                    console.log(`[Delivery Date Updater] Marked as failed due to cancellation: ${inputLine}`);
                }
                resolve(false);
                return;
            }
            ...
            if (verifyDateField) {
                const savedDate = verifyDateField.value.trim();
                const expectedDate = inputDateFull; // Compare in DD/MM/YYYY format
                if (savedDate === expectedDate) {
                    updateStatus(`✓ Date change verified successfully`, 'success');
                } else {
                    updateStatus(`⚠️ Could not verify date change: Saved=${savedDate}, Expected=${expectedDate}`, 'warning');
                }
            } else {
                updateStatus(`⚠️ Could not verify date change: Date field not found`, 'warning');
            }
    ...
    autoSkipCheckbox
        ...
        function checkPageAndInitialize() {
            if (document.readyState === 'complete' || document.readyState === 'interactive') {
                log('Page is ready, initializing script');
                initialize();
            } else {
