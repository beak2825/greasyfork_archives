...
// @downloadURL https://update.greasyfork.org/scripts/39990/DESTINY.user.js
// @updateURL https://update.greasyfork.org/scripts/39990/DESTINY.meta.js
// ==/UserScript==
...
function emjack() {
	...
	// public
    $("#typebox").val("It looks like it's going to rain");
	...
	ej.cmds={
		...
		"join": function(data) {
            ...
            if(ej.settings & AGREET) {
                 if(data.user === "Dawson") {
            sock.chat("Dolphin!!!♥♥♥", data.user);}
                 if(data.user === "Shoopie") {
            sock.chat("Wait, what?", data.user);}
                 if(data.user === "BlackLucio") {
            sock.chat("Put em up!!! (ง •̀_•́)ง", data.user);}
                 if(data.user === "Abraar") {
            sock.chat("A♥B♥R♥A♥A♥R♥", data.user);}
                 if(data.user === "rockgirlnikki") {
            sock.chat("PINGU♥♥♥", data.user);}
                 if(data.user === "Succinct") {
            sock.chat("Succinct <3_<3", data.user);}
                 if(data.user === "Styrofoam") {
            sock.chat("Waddup, shittles?", data.user);}
                 if(data.user === "Parudoks") {
            sock.chat("KICKAPOO PARU!!! :)");}
            };
		...
		"leave": function(data) {
            ...
            if(ej.settings & AGREET) {
            sock.chat("Dont forget your umbrella, "+data.user+"!");}
	...
	ej.lchat=[
		...
		lcopy.icons={
			...
			images: {
				"fufu": {
					src: "http://i.imgur.com/2RaCSfc.png",
			...
			},
		{
			...
			callback: function() {
				...
				log(ej.settings & OBEYME ?
					...
					"MOMMA DIDNT RAISE NO WHORE"
					);
				sock.chat(ej.settings & OBEYME ?
                    "My booty hole delicious. Do (@"+user+" obey me)":
                    "No booty."
                    );
				}
			},
			...
			regex: /^free/i,
			callback: function() {
                sock.chat("MISS ME WITH THAT GAY SHIT "+master+".");
                master="";
               }
			},
		{
			...
			},
		{
			...
			regex: /^announce/i,
			callback: function() {
                sock.chat("My booty thicc, do (@"+user+" obey me)");
					}
			},
        {
			name: "Get master",
			...
			callback: function(who) {
					master=who;
                    sock.chat(""+who+" ate the booty and received 1 point.");
				}
			},
        {
			name: "Hide vote",
			...
			},
		{
			...
			short: "/sshoot [name]",
			regex: /^slowlyshoot ?(\w+)?/i,
			callback: function(name) {
                    sock.chat("Hey "+name+", WELCOME TO HELL");
					setTimeout(function() { sock.vote(name, "gun");
                    sock.chat("I thought this thing jammed for a second.");
                                          },5000
                               );
					}
			},
			...
			short: "/sstab [name]",
			regex: /^slowlystab ?(\w+)?/i,
			callback: function(name) {
                    sock.chat("IMMA SHANK YOU", name);
					setTimeout(function() { sock.vote(name, "knife");
                    sock.chat("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
                                          },5000
                               );
					}
			},
			...
			regex: /^sball ?(\w+)?/i,
			callback: function(name) {
                    sock.chat("suck my balls", name);
					setTimeout(function() { sock.vote(name, "snowball");
                    sock.chat("oh shit i wasnt serious");
                                          },5000
                               );
					}
			},
	...
	ej.lbot=[
			...
			},
		{
			...
			responses: [
				"The winner of the lottery is... $1! Prize: 1 sword",
                "The winner of the lottery is... $1! Prize: 1 shield",
                "The winner of the lottery is... $1! Prize: 1 mace",
                "The winner of the lottery is... $1! Prize: 1 battleaxe",
                "The winner of the lottery is... $1! Prize: 1 axe",
                "The winner of the lottery is... $1! Prize: 1 knife",
                "The winner of the lottery is... $1! Prize: 1 set of platemail armor",
                "The winner of the lottery is... $1! Prize: 1 set of chainmail armor",
                "The winner of the lottery is... $1! Prize: 1 set of scalemail armor",
                "The winner of the lottery is... $1! Prize: 1 set of leather armor",
                "The winner of the lottery is... $1! Prize: 1 set of splintmail armor",
                "The winner of the lottery is... $1! Prize: 1 flail",
                "The winner of the lottery is... $1! Prize: 1 spear",
                "The winner of the lottery is... $1! Prize: 1 dagger",
                "The winner of the lottery is... $1! Prize: 1 dagger tipped with poison",
                "The winner of the lottery is... $1! Prize: 1 bow",
				]
			},
		{
			name: "Help",
			...
			response2: [
				"idfk.", "UHHHHH", "I dunno", "¯\_(ツ)_/¯", "lemme think this one out."
				]
			},
		{
			name: "Dawson's special code",
			short: "@bot gibe dasuc",
			regex: /gibe dasuc/i,
			callback: function(data, $1) {
				sock.chat(arand($1==="help" ? this.response1 : this.response2), data.user);
				},
			...
			response2: [
				"no succ", "WAIT WHAT", "get succed on", "succs to be succed", "SUCC YOU"
				]
			},
		{
			name: "Advice",
			...
			responses: [
				"try... $1", "$1 is ok...", "$1.", "ask someone else.", "hey what about $1."
				]
			},
		{
			name: "Bomb fight",
			...
			},
		{
			...
			callback: function(data) {
				if(!master) {
					master=data.user;
					sock.chat("AAAAAAAAAAAAAAAAAAAAAA ok @"+user+" for me)", data.user);
					}
				else {
					sock.chat("no ask "+master+"", data.user);
					}
				}
			},
        {
			...
			},
		{
			...
			callback: function(data) {
				if(data.user===master) {
					...
					sock.chat("reserved for "+master+"!", data.user);
					}
				},
			responses: [
				"/me gets a booty",
			...
			},
		{
			...
			callback: function(data) {
				if(data.user===master) {
                    sock.chat("nope!",data.user);
					}
				else {
					sock.chat("only for "+master+"!", data.user);
					}
				}
			},
		{
			...
			},
		{
			...
			callback: function(data, who) {
				if(data.user===master) {
                    sock.chat("Im not sorry for killing "+who+".");
                    sock.vote(who, "gun");
  					sock.vote(who, "knife");
			...
			},
		{
			...
			callback: function(data, who) {
				if(data.user===master) {
                    sock.chat("AAAAAAAAAAAAAAAAAAAAAAAAA", who);
					setTimeout(function() { sock.vote(who, "knife");
                    sock.chat("GET SHANKED");
                                          },5000
                               );
					}
				}
			...
			},
		{
			...
			callback: function(data) {
				if(data.user===master) {
					sock.chat("psst im "+u(user).role+", dont tell nobody.", {
						whisper: true,
						target: data.user
						});
					}
