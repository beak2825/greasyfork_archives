...
// @version			2014-03-19 13:00
// @downloadURL https://update.greasyfork.org/scripts/12037/Feuerwachenet%20AAO%20Script%20by%20Amtsleiter%20-%20Hogsmeade.user.js
// @updateURL https://update.greasyfork.org/scripts/12037/Feuerwachenet%20AAO%20Script%20by%20Amtsleiter%20-%20Hogsmeade.meta.js
// ==/UserScript==
...
var Einsatzklassen = {
  ...
  'Wohnungsbrand'               :  'F2',	
  'Wohnwagenbrand'              :  'F1',	
  'Großbrand in Zuckerfabrik'   :  'Zucker',	
  'Brand in Polizeistation'     :  'Polizei',	
  'Brand in Streichholzfabrik'  :  'Streichholz',
  'Brand in Telefonshop'        :  'F2',
...
// Flughafen-Einsätze
  ...
  'Grasnarbenbrand'             :  'F1 N',  
  'Mülleimer Brand_n'           :  'F1 N',
...
// Altstadteinsätze in der Neustadt
  ...
  'Brand in Erholungsbad_n'         :  'F3-Bad N',
  ...
  'Großbrand in Zuckerfabrik_n'     :  'Zucker N',	
  'Brand in Polizeistation_n'       :  'Polizei N',	
  'Brand in Streichholzfabrik_n'    :  'Streichholz N',
  'Brand in Telefonshop_n'          :  'F2 N',
...
// Brandeinsätze
  ...
  'Zucker'         :  'LF,LF,LF,LF,LF,LF,ELW 1,DLK,GW-L2,GW-M,GW-A',
  'Zucker N'       :  'LF/LF-S,LF/LF-S,LF/LF-S,LF/LF-S,LF/LF-S,LF/LF-S,ELW 1,DLK,GW-L2,GW-M,GW-A',
  'Polizei'        :  'LF,LF,ELW 1,RW 2/AB-Rü,DLK', 
  'Polizei N'      :  'LF/LF-S,LF/LF-S,LF/LF-S,ELW 1,RW 2/AB-Rü,DLK', 
  'Streichholz'    :  'LF,LF,ELW 1,GW-A,DLK',
  'Streichholz N'  :  'LF/LF-S,LF/LF-S,ELW 1,GW-A,DLK',
  ...
  'F3-Bad'         :  'LF,LF,LF,ELW 1,GW-L2,GW-A,DLK',
  'F3-Bad N'       :  'LF/LF-S,LF/LF-S,LF/LF-S,ELW 1,GW-L2,GW-A,DLK',
...
//Alle Fahrzeugtypen aufgelistet, nur mit Zahlenindex
var fahrzeugabkuerzungen = new Array('AB-S','AB-WF','AB-Öl','AB-Rü','AB-AS','AB-EL','LF','RTW','NEF','ELW 1','TLF','DLK','RW 2','FwK','GW-A','GW-L2','GW-Ö','GW-S','GW-T','GW-M','GW-G','FLF','RTF','GT','GW-H','Feuerlöschboot','Rettungsboot','ULF','LF-S','RTZ','WLF','AB EL');
...
function listener_schule() {
	pupil_onclick("false",this);
}

function listener_uebung() {
	pupil_onclick("true",this);
}
	...
	for (var i=1; i<DT.length; i++) { 
		...
		for ( var j=0;j<TR.length;j++) {
			...
			if (TDs.length==7) { 
				...
				TDs[0].getElementsByTagName('input')[0].removeEventListener( "click" , listener_schule, true );
				TDs[0].getElementsByTagName('input')[0].addEventListener( "click" , listener_schule, true );
...
function pupil_onclick(uebung,element) {
	...
	document.getElementById("count_schueler").innerHTML=checked_count;
	if ( checked_count==parseInt(max_schueler) && uebung == "true") Markiere_FWLeute();
...
/////////////////////// EINSATZGEBIET ENDE /////////////////////////////////////////////
	...
	if ( Einsatzstichwort == ermittleEigenerAuftrag()) {
		document.getElementById("content").getElementsByTagName("h1")[0].style.color=colour;
		document.getElementById("content").getElementsByTagName("h1")[0].style.textDecoration="blink";
	}
	else if ( auftraege.match(Einsatzstichwort) ) document.getElementById("content").getElementsByTagName("h1")[0].style.color=colour;
...
function init() {
	...
	//// Bei Bewerbungen im Verband färben
	if ( document.getElementById('root').getElementsByClassName('level2')[4].getElementsByTagName('li')[4] ) {
		var bewerbungen=document.getElementById('root').getElementsByClassName('level2')[4].getElementsByTagName('li')[4].getElementsByTagName('a')[0];
		if ( bewerbungen.href.match("/verband_bewerbungsliste") ) {
			var anzahl=bewerbungen.innerHTML.replace("Bewerbungen: ","");
			//if ( anzahl > 0 ) document.getElementById('root').getElementsByTagName('li')[4].style.backgroundColor='green';
			if ( anzahl > 0 ) document.getElementById('root').getElementsByClassName('level2')[4].parentNode.style.backgroundColor='green';
		}
	}
	...
	var welt=ermittleWelt();
	if ( welt == 1 ) {
		if ( unsafeWindow.feuerwache_layout=='black') {
			document.getElementById("footer").innerHTML+='<a href="" style="position:fixed;top:75px;left:20px;"><input type="button" class="button" value="Welt 1 aktiv!" /></a>';
		}
		else {
			document.getElementById("footer").innerHTML+='<a href="" style="position:fixed;top:75px;left:20px;" class="button">Welt 1 aktiv!</a>';
		}
	}
	else {
		if ( unsafeWindow.feuerwache_layout=='black') {
			document.getElementById("footer").innerHTML+='<a href="/welten/1" style="position:fixed;top:75px;left:20px;"><input type="button" class="button" value="zu Welt 1 wechseln" /></a>';
		}
		else {
			document.getElementById("footer").innerHTML+='<a href="/welten/1" style="position:fixed;top:75px;left:20px;" class="button">zu Welt 1 wechseln</a>';
		}
	}
	
	if ( welt == 2 ) {
		if ( unsafeWindow.feuerwache_layout=='black') {
			document.getElementById("footer").innerHTML+='<a href="" style="position:fixed;top:100px;left:20px;"><input type="button" class="button" value="Welt 2 aktiv!" /></a>';
		}
		else {
			document.getElementById("footer").innerHTML+='<a href="" style="position:fixed;top:100px;left:20px;" class="button">Welt 2 aktiv!</a>';
		}
	}
	else {
		if ( unsafeWindow.feuerwache_layout=='black') {
			document.getElementById("footer").innerHTML+='<a href="/welten/2" style="position:fixed;top:100px;left:20px;"><input type="button" class="button" value="zu Welt 2 wechseln" /></a>';
		}
		else {
			document.getElementById("footer").innerHTML+='<a href="/welten/2" style="position:fixed;top:100px;left:20px;" class="button">zu Welt 2 wechseln</a>';
		}
	}
	
	if ( welt == 3 ) {
		if ( unsafeWindow.feuerwache_layout=='black') {
			document.getElementById("footer").innerHTML+='<a href="" style="position:fixed;top:125px;left:20px;"><input type="button" class="button" value="Welt 3 aktiv!" /></a>';
		}
		else {
			document.getElementById("footer").innerHTML+='<a href="" style="position:fixed;top:125px;left:20px;" class="button">Welt 3 aktiv!</a>';
		}
	}
	else {
		if ( unsafeWindow.feuerwache_layout=='black') {
			document.getElementById("footer").innerHTML+='<a href="/welten/3" style="position:fixed;top:125px;left:20px;"><input type="button" class="button" value="zu Welt 3 wechseln" /></a>';
		}
		else {
			document.getElementById("footer").innerHTML+='<a href="/welten/3" style="position:fixed;top:125px;left:20px;" class="button">zu Welt 3 wechseln</a>';
		}
	}
...
function bearbeiteSingleGebaeude() {
	document.getElementById("footer").innerHTML+='<div style="position:fixed;top:50px;left:20px;color:#AB59FF;">Ausgewählte FW-Leute:</div>';
	document.getElementById("footer").innerHTML+='<div id="count_schueler" style="position:fixed;top:50px;left:160px;color:#AB59FF;">0</div>';
...
function Markiere_FWLeute(){
	...
	for (var i=2; i<DT.length; i++) {
		...
		for ( var j=0;j<TR.length;j++) {
			...
			if (TDs.length==7) {
				//Haken bleibt, trotz neu sortieren bei wache öffnen
				TDs[0].getElementsByTagName('input')[0].removeEventListener( "click" , listener_uebung, true );
				TDs[0].getElementsByTagName('input')[0].addEventListener( "click" , listener_uebung, true );
...
function bearbeiteAuftragsliste() {
	...
	var liste3=GM_getValue("auftragsliste3","--");
	var content = document.getElementById('content');
	var len = content.childNodes.length;
	...
	var NewDiv = document.createElement("div");
	var Kram="<br><table><tr><th>Einsatz</th><th>Welt 1</th><th>Welt 2</th><th>Welt 3</th></tr>";
	...
	//	if ( einsatz.match("\\(")) alert(einsatz_suche);
		...
		Kram+="></td><td><input type='checkbox' name='AuftragBox3' id='"+einsatz+"'";
		if ( liste3.match(","+einsatz_suche+",") ) Kram+=" checked";
		Kram+="></td></tr>";
	}
...
function AddAuftragEventlisteners() { 
	...
	var Boxes = document.getElementsByName("AuftragBox3");
	for ( var i=0;i < Boxes.length;i++) {
		Boxes[i].addEventListener("click",function () { AuftragBox_clicked("3"); },false);
	}
}
...
function ermittleWelt() {
	...
	else if ( welt.match("Welt 3") ) return "3";
}

function ermittleEigenerAuftrag() {
	return document.getElementById("navigation_mission_quest_full").title;
}
