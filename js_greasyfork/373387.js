...
// @grant		GM_info
// @run-at		document-end
// @downloadURL https://update.greasyfork.org/scripts/373387/MouseHunt%20AutoBot%20ENHANCED%20%2B%20REVAMP.user.js
// @updateURL https://update.greasyfork.org/scripts/373387/MouseHunt%20AutoBot%20ENHANCED%20%2B%20REVAMP.meta.js
// ==/UserScript==
...
var objBestTrap = {
    weapon: {
        arcane: ['Golem Guardian Arcane Trap','New Horizon Trap', 'Event Horizon', 'Grand Arcanum Trap', 'Chrome Arcane Capturing Rod', 'Arcane Blast Trap', 'Arcane Capturing Rod Of Nev'],
        draconic: ['Dragonvine Ballista Trap', 'Harrowing Holiday Harpoon Harp', 'Dragon Lance', 'Ice Maiden'],
        forgotten: ['Infinite Labyrinth Trap', 'Golem Guardian Forgotten Trap','Endless Labyrinth Trap', 'Crystal Crucible Trap', 'Stale Cupcake Golem Trap', 'Tarannosaurus Rex Trap', 'Crystal Mineral Crusher Trap', 'The Forgotten Art of Dance'],
        hydro: ['School of Sharks', 'Golem Guardian Hydro Trap','Rune Shark Trap', 'Chrome Phantasmic Oasis Trap', 'Phantasmic Oasis Trap', 'Oasis Water Node Trap', 'Steam Laser Mk. III', 'Steam Laser Mk. II', 'Steam Laser Mk. I', 'Ancient Spear Gun'],
        ...
        physical: ['Chrome MonstroBot', 'Golem Guardian Physical Trap','Sandstorm MonstroBot', 'Sandtail Sentinel', 'Enraged RhinoBot'],
        ...
        tactical: ['Chrome Sphynx Wrath', 'Golem Guardian Tactical Trap','Sphynx Wrath', 'Zugzwang\'s Ultimate Move', 'Zugzwang\'s First Move']
    },
    base: {
...
function sandCrypts(obj) {
    checkThenArm('best', 'weapon', objBestTrap.weapon.shadow);
    checkThenArm(null, 'bait', 'Graveblossom');
    var sandCryptsCharm = ['Shattering','Grub Scent'];
    ...
    if (salt >= obj.SC.maxSaltCharged) {
        ...
        checkThenArm('best', 'trinket',sandCryptsCharm);
    }
    else {
...
function MountainBoulder(){
	if(GetCurrentLocation().indexOf('Mountain') < 0)
		return;
    var health=document.getElementsByClassName("mountainHUD-boulder-health-percent")[0].innerText;
    if(health=="0 hp")
        window.setTimeout(function () { fireEvent(document.getElementsByClassName("mousehuntActionButton small")[0], 'click'); }, 150);
}
//birthday2019HUD-claimButton mousehuntTooltipParent
function Birthday2019(){
	if(GetCurrentLocation().indexOf('SUPER') < 0)
		return;
   var test=getPageVariable('user.quests.QuestBirthday2019.factory_atts.can_claim');
   // var test2=getPageVariable('user.quests.QuestBirthday2019.factory_atts.boss_warning');
    ...
    if( test=="true")
        window.setTimeout(function () { fireEvent(document.getElementsByClassName("birthday2019HUD-claimButton mousehuntTooltipParent")[0], 'click'); }, 150);
}
function sortNumber(a,b)
{
    return a-b;
}
function Birthday2019V2(){
	if(GetCurrentLocation().indexOf('SUPER') < 0)
		return;
    var test=getPageVariable('user.quests.QuestBirthday2019.factory_atts.can_claim');
   // var test2=getPageVariable('user.quests.QuestBirthday2019.factory_atts.boss_warning');
...
factory_room_stats[0] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_mixing_room_stat_item.quantity'));
factory_room_stats[1] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_break_room_stat_item.quantity'));
factory_room_stats[2] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_pumping_room_stat_item.quantity'));
factory_room_stats[3] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_quality_assurance_room_stat_item.quantity'));
 var factory_room_stats_2=new Array(4);
factory_room_stats_2[0] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_mixing_room_stat_item.quantity'));
factory_room_stats_2[1] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_break_room_stat_item.quantity'));
factory_room_stats_2[2] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_pumping_room_stat_item.quantity'));
factory_room_stats_2[3] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_quality_assurance_room_stat_item.quantity'));
...
var buttonToFire=document.getElementsByClassName('birthday2019HUD-factoryRoom')[lowestIndex].getElementsByClassName('mousehuntActionButton tiny')[0];
var currentLocation=getPageVariable('user.quests.QuestBirthday2019.factory_atts.current_room');
    if(currentLocation!=locationIndex[lowestIndex])
window.setTimeout(function () { fireEvent(buttonToFire, 'click'); }, 150);
}
function HalloweenBrutal(){
	var isFire=document.getElementsByClassName("halloweenHUD-campBanner-cannonBall-button selected").length;
	window.setTimeout(function () { fireEvent(document.getElementsByClassName("mousehuntActionButton small halloweenHUD-campBanner-claimReward")[0], 'click'); }, 150);
	if(isFire==0)
    {
    window.setTimeout(function () { fireEvent(document.getElementsByClassName("halloweenHUD-campBanner-cannonBall-button default")[0], 'click'); }, 150);
    }
    //Experimental: Minimize the missing cannon
	window.setTimeout(function(){
									var isFire2=document.getElementsByClassName("halloweenHUD-campBanner-cannonBall-button selected").length;
									if(isFire2==0)
									{
									window.setTimeout(function () { fireEvent(document.getElementsByClassName("halloweenHUD-campBanner-cannonBall-button default")[0], 'click'); }, 150);
									}
    ...
    //
    window.setTimeout(function () { fireEvent(document.getElementsByClassName("jsDialogClose button")[0], 'click'); }, 15000);
}
/* V1 Halloween 2017 Mountain Rush*/
function HalloweenNew(){
	if(GetCurrentLocation().indexOf('Mountain') < 0)
		return;
    var baitName = getPageVariable('user.bait_name');
    console.log("current bait:"+baitName);
    if(!isNullOrUndefined(baitName)){
    if (baitName.indexOf('Lockbox')>-1)return;}
    var isFire=document.getElementsByClassName("halloweenHUD-campBanner-cannonBall-button selected").length;
    var regularCannonballCount=document.getElementsByClassName("halloweenHUD-campBanner-cannonBall-quantity quantity")[0].innerText;
    var ghostfireCannonballCount=document.getElementsByClassName("halloweenHUD-campBanner-cannonBall-quantity quantity")[1].innerText;
    console.log("ballcount:"+regularCannonballCount+"|"+ghostfireCannonballCount);
    if(regularCannonballCount==0&&ghostfireCannonballCount==0)
    {
        checkThenArm('best', 'bait', ['Ghastly Galleon Gouda', 'Brie Cheese'],true);
        return;
    }
	checkThenArm('best', 'bait', ['Abominable Asiago Cheese','Chedd-Ore Cheese', 'Brie Cheese'],true);
    //console.log(objTrapList['bait']);
    baitName = getPageVariable('user.bait_name');
    console.log("current bait:"+baitName);
     window.setTimeout(function () { fireEvent(document.getElementsByClassName("mousehuntActionButton small halloweenHUD-campBanner-claimReward")[0], 'click'); }, 150);
    //
    window.setTimeout(function () { fireEvent(document.getElementsByClassName("jsDialogClose button")[0], 'click'); }, 15000);
    ...
    if(baitName.indexOf('Brie')>-1)
    {
        window.setTimeout(function () { fireEvent(document.getElementsByClassName("halloweenHUD-campBanner-cannonBall-button selected")[0], 'click'); }, 150);
    }
	// window.setInterval(function () { fireEvent(document.getElementsByClassName("mousehuntActionButton small halloweenHUD-campBanner-claimReward")[0], 'click'); }, 150000);

    //window.setInterval(function () { fireEvent(document.getElementsByClassName("halloweenHUD-campBanner-cannonBall-button default")[0], 'click'); }, 150000);
}
/* V2 SG + ZTower */
...
function fieryWarpath(superCharm) {
    ...
    if (currentLocation.indexOf("Fiery Warpath") > -1) {
        ...
        var wave4PhysicalTrap = ['Warden Slayer', 'Chrome MonstroBot', 'Sandstorm MonstroBot', 'Sandtail Sentinel','Enraged Rhinobot'];
...
function checkThenArm(sort, category, name, isForcedRetry)   //category = weapon/base/charm/trinket/bait
{
    ...
    if(category=='bait')
	{
	var baitWorkaround=getPageVariable("user.bait_quantity");
	if(!isNullOrUndefined(userVariable)&&baitWorkaround==0)
	{
		console.log("OOC detected,trying workaround");
		var baitList=localStorage.getItem("TrapBestBait");
        console.log(baitList);
		baitList.replace(userVariable+",","");
		baitList.replace(","+userVariable,"");
		console.log(baitList);
		localStorage.setItem("TrapBestBait",baitList);
	}
	}
    if (sort == 'best') {
        ...
        if (showTimerInPage) {
            ...
            if (headerElement) {
                ...
                */
                ...
                preferenceHTMLStr += '<option value="HalloweenNew">Halloween Event 2017+</option>';
				preferenceHTMLStr += '<option value="HalloweenBrutal">Halloween Event 2017+ Brutal</option>';
				preferenceHTMLStr += '<option value="Mountain">MountainBoulder Smasher</option>';
				preferenceHTMLStr += '<option value="Birthday2019">Birthday2019</option>';
                preferenceHTMLStr += '<option value="Birthday2019V2">Birthday2019V2</option>';
...
function getPageVariable(name) {
    ...
    try {
        ...
        if (browser == 'chrome') {
            ...
            } else if (name == "user.bait_name") {
                return unsafeWindow.user.bait_name;
            } else if (name == "user.quests.QuestTrainStation.on_train") {
                return unsafeWindow.user.quests.QuestTrainStation.on_train;
                } else if (name == "user.quests.QuestBirthday2019.factory_atts.hunts_remaining") {
                return unsafeWindow.user.quests.QuestBirthday2019.factory_atts.hunts_remaining;
                     } else if (name == "user.quests.QuestBirthday2019.items.birthday_factory_mixing_room_stat_item.quantity") {
                return unsafeWindow.user.quests.QuestBirthday2019.items.birthday_factory_mixing_room_stat_item.quantity;
                          } else if (name == "user.quests.QuestBirthday2019.items.birthday_factory_break_room_stat_item.quantity") {
                return unsafeWindow.user.quests.QuestBirthday2019.items.birthday_factory_break_room_stat_item.quantity;
                               } else if (name == "user.quests.QuestBirthday2019.items.birthday_factory_pumping_room_stat_item.quantity") {
                return unsafeWindow.user.quests.QuestBirthday2019.items.birthday_factory_pumping_room_stat_item.quantity;
                                    } else if (name == "user.quests.QuestBirthday2019.items.birthday_factory_quality_assurance_room_stat_item.quantity") {
                return unsafeWindow.user.quests.QuestBirthday2019.items.birthday_factory_quality_assurance_room_stat_item.quantity;
                                          } else if (name == "user.quests.QuestBirthday2019.factory_atts.current_room") {
                return unsafeWindow.user.quests.QuestBirthday2019.factory_atts.current_room;
            } else {
                if (debug) console.log('GPV firefox: ' + name + ' not found.');
            }
            /*user.quests.QuestBirthday2019.factory_atts.current_room
            factory_room_stats[0] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_mixing_room_stat_item.quantity'));
factory_room_stats[1] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_break_room_stat_item.quantity'));
factory_room_stats[2] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_pumping_room_stat_item.quantity'));
factory_room_stats[3] = parseInt(getPageVariable('user.quests.QuestBirthday2019.items.birthday_factory_quality_assurance_room_stat_item.quantity'));*/
