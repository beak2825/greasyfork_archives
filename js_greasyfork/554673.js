...
// @version      3.5.0
// @description  Skanuje oferty Allegro z autoryzacją i limitem raportów przez Google Sheets API
...
evaluateProductRating() {
	...
	if (this.productRating > 0) {
		if (this.productRating < 4.00) {
			...
			this.ratingValueEvaluation = {
				rating: '🌟 Bardzo dobrze',
				color: '#10b981', // wyraźny zielony (emerald)
	...
	return {
		...
		bestPrice: {
			...
			recommendation: this.hasBestPriceGuarantee
				...
				: 'Rozważ dodanie Gwarancji najniższej ceny - daje klientom pewność najlepszej oferty.'
		},
		allegroPay: {
		...
		coins: {
			...
			recommendation: this.coinsAmount > 0
				...
				: 'Rozważ dodanie monet Smart! - to doskonałe narzędzie promocyjne do kampanii czasowych.'
		},
		coupons: {
...
scanSuggestionsSection() {
	...
	if (hasBrandTab && (brandMatches || !this.hasBrand)) {
		...
		// Brak zakładki z marką, tylko "Pokrewne" = średnio
		qualityRating = '⚠️ Średnio';
		qualityColor = '#fb923c'; // pomarańczowy
		qualityMessage = 'Sekcja zawiera tylko produkty pokrewne, brak dedykowanej zakładki z marką.';
		recommendation = 'Rozważ wzmocnienie obecności produktów swojej marki, aby Allegro mogło utworzyć dedykowaną zakładkę.';
		this.suggestionsQualityScore = 50;
		console.log('📊 Ocena: ŚREDNIO - Brak zakładki z marką (tylko Pokrewne)');
	}
...
async analyzeImageWithAI() {
	...
	try {
		...
		this.aiImageAnalysis.summary = `❌ Błąd połączenia: ${error.message}`;
	}
}
...
async analyzeDescriptionWithAI() {
	...
	try {
		...
		this.descriptionAiAnalysis = `❌ Błąd połączenia: ${error.message}`;
	}
}
			...
			<!-- TABELA: Funkcje Allegro -->
				...
				<div class="section">
					...
					<div style="margin-top:16px; padding:12px; background:#fee2e2; border:2px solid #dc2626; border-radius:8px;">
						...
						<div style="color:#7f1d1d; font-size:13px; line-height:1.6;">
							<strong>Brak własnych sekcji promocyjnych!</strong><br>
							Tylko sekcje sponsorowane przez Allegro (promują konkurencję).<br>
							Skonfiguruj promocję na dodatkowe produkty!
						</div>
					</div>
					` : ''}
					`}
				...
				<div class="section">
					...
					<div class="card" style="border:2px solid ${this.suggestionsSection.qualityColor}; background:${this.suggestionsSection.hasBrandTab ? '#f0fdf4' : '#fff7ed'};">
						...
						<div class="row">
							<div class="label">Ma zakładkę Pokrewne:</div>
							<div class="value" style="font-weight:700;">${this.suggestionsSection.hasRelatedTab ? '✅ TAK' : '❌ NIE'}</div>
						</div>
						<div class="row">
							<div class="label">Ocena:</div>
				...
				<div class="section">
					...
					<div class="card">
						...
						<div style="margin-top: 16px;">
							...
							<div style="color:#6b7280; font-size:12px; margin-top:8px; padding:8px; background:#f9fafb; border-radius:4px;">
								<strong>Rekomendacja:</strong> Białe tło poprawia czytelność i wygląd oferty. Optymalnie: ≥80%
							</div>
						</div>
						...
						<div style="margin-top: 16px; padding: 12px; background: #fffbeb; border-left: 3px solid #f59e0b; border-radius: 6px;">
							<strong style="color: #d97706;">⚠️ Informacje o analizie AI:</strong>
							<ul style="margin: 8px 0 0 20px; color: #6b7280; font-size: 12px;">
							...
							<div style="margin-top: 8px; font-size: 11px; color: #78716c; font-style: italic;">
								💡 Analiza AI wymaga połączenia z internetem i poprawnego klucza API OpenAI.
							</div>
						</div>
						` : ''}
					</div>
	...
	const fileName = this.generateFileName(offerName);
		...
		<!-- TABELA: Funkcje Allegro -->
				...
				<div style="margin-bottom:24px;">
					...
					${this.promotionalSections.filter(s => !s.isSponsored).length === 0 && this.promotionalSections.length > 0 ? `
					<div style="margin-top:16px; padding:12px; background:#fee2e2; border:2px solid #dc2626; border-radius:8px;">
						<div style="font-weight:700; color:#991b1b; margin-bottom:8px;">❌ KRYTYCZNE: Brak własnych sekcji promocyjnych!</div>
						<div style="color:#7f1d1d; font-size:13px; line-height:1.6;">
							<strong>Ta aukcja NIE MA żadnych promocji sprzedawcy na dodatkowe produkty.</strong><br>
							Wyświetla się tylko sekcja sponsorowana przez Allegro, która promuje oferty konkurencji.<br><br>
							<strong>Zalecenie:</strong> Natychmiast skonfiguruj promocję typu "-50% na piąty przedmiot" lub podobną, dodając minimum 5 produktów. To zwiększy wartość koszyka i zapobiegnie utracie klientów na rzecz konkurencji promowanej przez Allegro.
						</div>
					</div>
					` : ''}
					`}
				...
				<div style="margin-bottom:24px;">
					...
					<div style="border:2px solid ${this.suggestionsSection.qualityColor}; border-radius:8px; padding:16px; background:${this.suggestionsSection.hasBrandTab ? '#f0fdf4' : '#fff7ed'};">
						...
						<div style="display:flex; justify-content:space-between; margin-bottom:12px; padding:8px 0; border-bottom:1px dashed #e5e7eb;">
							<div style="color:#6b7280;">Ma zakładkę Pokrewne:</div>
							<div style="font-weight:700;">${this.suggestionsSection.hasRelatedTab ? '✅ TAK' : '❌ NIE'}</div>
						</div>
						${this.suggestionsSection.recommendation ? `
