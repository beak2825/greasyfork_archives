...
// @version      1.0
// @description  Javct enhancer...
// @author       HappyTreeFriends
// @namespace    https://greasyfork.org/users/317047
// @match        https://javct.net/v/*
// @match        https://javdc.net/*
// @icon         https://www.google.com/s2/favicons?domain=javct.net
// @grant        GM_xmlhttpRequest
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/434768/Javct%20enhancer.user.js
// @updateURL https://update.greasyfork.org/scripts/434768/Javct%20enhancer.meta.js
// ==/UserScript==

(function() {
    'use strict';

    document.querySelectorAll('.server-list .switch-source').forEach(item => {
        if(item.innerText.includes('ST')){
            proceed_streamtape(item.innerText, item.dataset.source, item.dataset.episode);
        }else if(item.innerText.includes('FE')){
            proceedfe(item.innerText, item.dataset.source, item.dataset.episode);
        }
    });
...
async function proceed_streamtape(name, source, episode) {
    ...
    (async () => {
        const element = document.querySelector('.switch-player');
...
async function proceedfe(name, source, episode) {
            ...
            GM_xmlhttpRequest({
                ...
                onload: function(response) {
                    ...
                    if(items != null){
                        ...
                        document.querySelector('.switch-player').appendChild(buttonDiv);
                    }
                }
            });
        }
