...
// @author       You
// @include      http://oa.eccjt.com:88/spa/workflow/index_mobx.jsp*
// @include      http://oa.eccjt.com:88/wui/index.html*
// @match        *://oa.eccjt.com:88/spa/workflow/static/index.html*
// @require      http://libs.baidu.com/jquery/1.11.1/jquery.min.js
// @run-at       document-end
// @grant        GM_xmlhttpRequest
// @downloadURL https://update.greasyfork.org/scripts/466339/oafor.user.js
// @updateURL https://update.greasyfork.org/scripts/466339/oafor.meta.js
// ==/UserScript==
...
(function init1(){
    ...
    if (counter.length == 0) {
        counter=document.querySelectorAll('div[title*="投资集团-办公室"]');
    }
    ...
    if (counter.length == 0) {
        counter=document.querySelectorAll('div[title*="JY"]');
    }
    if (counter.length == 0) {
        counter=document.querySelectorAll('div[title*="CT"]');
    }
    if (counter.length == 0) {
        counter=document.querySelectorAll('div[title*="TZ"]');
    }
...
//    var counter=document.querySelectorAll('span[class="wea-url"] a');
//    if (counter.length>0) {
//        counter.forEach((info, index) => { if(index%2==0){info.click(); }});
//        //counter[0].click()
//    } else {
//        if(x<5){
//            x=x+1;
...
var login1 = document.querySelectorAll('input[id="validatecode"]')
var dic = new Array();
...
if (flag > 0) {
  GM_xmlhttpRequest({
    method: 'GET',
    url: 'http://oa.eccjt.com:88/weaver/weaver.file.MakeValidateCode?seriesnum_=3',
        ...
        GM_xmlhttpRequest({
          ...
          onload: function(response) {
            if (response.status === 200) {
              ...
              setTimeout(function() {
                  window.location.href = 'http://oa.eccjt.com:88/spa/workflow/static/index.html#/main/workflow/listDoing'; // 将URL替换为你想要跳转的地址
              }, 3000); // 延迟时间，单位为毫秒
            }
          }
        });
