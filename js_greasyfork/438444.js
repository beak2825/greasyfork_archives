...
// @execlude     http*://*.*/*url=http*://*.*.*/*
// @execlude  url=http
// @execlude jx=http
// @execlude     http*://*.*/*jx=http*://*.*.*/*
// @execlude      http*://*.*:*/*url=http*://*.*.*/*
// @execlude      http*://*.*:*/*jx=http*://*.*.*/*
// @execlude      http*://*.*/*v=http*://*.*.*/*
// @execlude      http*://*.*:*/*v=http*://*.*.*/*


// @grant        none
// @run-at       document-body
// @downloadURL https://update.greasyfork.org/scripts/438444/2022VIP.user.js
// @updateURL https://update.greasyfork.org/scripts/438444/2022VIP.meta.js
// ==/UserScript==
// 读取配置后执行
function readSettings() {
    // CPU性能好的设为100，CPU不好的可设置为1000
    settings.getElementTimes = 800;
    // log输出字体布局
    settings.fontStyle = {
    ...
    if (window === top) {
        ...
        */
        settings.NoAD解析 = {    // TODO by 17kyun.com/api.php?url=    // TODO by tv.hzwdd.cn
            // 定义解析接口。格式如下：
            // "解析名称": "接口链接",             // TODO 注意 : 和 " 以及 , 都是英文符号。
            "艾特解析": "https://jsap.attakids.com/?url=",    // TODO 腾讯 
            "OK解析": "https://api.okjx.cc:3389/jx.php?url=" || "https://okjx.cc/?url=" || "https://m2090.com/?url=",    // TODO 优质: 腾讯 
            "拍伟解析": "https://vip.parwix.com:4433/player/?url=",    // TODO  腾讯 
            "猪蹄解析": "https://jx.iztyy.com/svip/?url=" || "https://chaxun.truechat365.com/?url=",    // TODO 腾讯
            "爱豆解析": "https://jx.aidouer.net/?url=" || "https://jx.ppflv.com/?url=",    // TODO 腾讯 
            "虾米解析": "https://jx.xmflv.com/?url=",    // TODO 土豆
            "夜幕解析": "https://www.yemu.xyz/?url=",    // (Parwix解析: "https://jx.parwix.com:4433/player/analysis.php?v=" || "https://vip.parwix.com:4433/player/?url=" || )    // TODO 西瓜
          "m1907解析": "https://z1.m1907.cn?jx=",
            "友谊解析": "https://jx.youyitv.com/?url="    // TODO (B站)
        };
        settings.AD解析 = {    // TODO (有广告)
    ...
    if (window === top) {
        ...
        if (!detectMobile()) {
            if (location.host.indexOf("v.qq.com") !== -1) {
                readyPlayerBox("进入腾讯视频", ["#mask_layer", ".mod_vip_popup", "#mask_layer"], [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]], "div#mod_player", null);
            } else if (location.host.indexOf("iqiyi.com") !== -1) {
                readyPlayerBox("进入爱奇艺", ["#playerPopup", "div[class^=qy-header-login-pop]"],[settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]],"iqpdiv.iqp-player[data-player-hook=mainlayer]", null);
            } else if (location.host.indexOf("youku.com") !== -1) {
                readyPlayerBox("进入优酷视频", ["#iframaWrapper"], [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]], "#player", null);
                ...
                doElement("svg[aria-hidden=true]", () => readyPlayerBox("已进入哔哩哔哩", null, [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]], "div.bpx-player-video-area,div.mask-container", null));    // TODO || document.getElementById("bilibiliPlayer") || document.getElementById("live-player-ctnr")
            } else if (location.host.indexOf("miguvideo.com") !== -1) {
                readyPlayerBox("进入咪咕视频", null, [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]], "section#mod-player", null);
            } else if (location.host.indexOf("le.com") !== -1) {
                readyPlayerBox("已进入乐视TV", null, [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]], "#le_playbox", null);
            } else if (location.host.match(/(tv|film).sohu.com/)) {
                readyPlayerBox("已进入搜狐视频", null, [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]], "#player,#sohuplayer,.player-view", null);
            } else if (location.host.indexOf("mgtv.com") !== -1) {
                readyPlayerBox("已进入芒果TV", null, [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]], "#mgtv-player-wrap", null);
            } else if (location.host.indexOf("ixigua.com") !== -1) {
                readyPlayerBox("进入西瓜视频", null, [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]], "div.teleplayPage__playerSection", null);
            } else if (location.host.indexOf("pptv.com") !== -1) {
                readyPlayerBox("进入PPTV", null,[settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]], "div.w-video", null);
            } else if (location.host.indexOf("vip.1905.com") !== -1) {
                readyPlayerBox("进入1905电影网", null, [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]], "div#playBox", null);
            } else if (location.host.indexOf("www.wasu.cn") !== -1) {
                readyPlayerBox("进入华数TV", null, [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"]], "div#pcplayer", null);
            }
        } else {
            if (location.host.indexOf("v.qq.com") !== -1) {
                readyPlayerBox("进入腾讯视频", [".mod_vip_popup", "[class^=app_],[class^=app-],[class*=_app_],[class*=-app-],[class$=_app],[class$=-app]", "div[dt-eid=open_app_bottom]", "div.video_function.video_function_new", "a[open-app]", "section.mod_source", "section.mod_box.mod_sideslip_h.mod_multi_figures_h,section.mod_sideslip_privileges,section.mod_game_rec"], [settings.NoAD解析["OK解析"]], "div.mod_play:not([style*='display: none;']) section.mod_player", null, href => {
                ...
                readyPlayerBox("进入爱奇艺", ["div.m-iqyGuide-layer", "a[down-app-android-url]", "[name=m-extendBar]", "[class*=ChannelHomeBanner]", "section.m-hotWords-bottom"], [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"]], "section.m-video-player", null);
            } else if (location.host.indexOf("youku.com") !== -1) {
                readyPlayerBox("进入优酷视频", ["#iframaWrapper", ".ad-banner-wrapper", ".h5-detail-guide,.h5-detail-vip-guide", ".brief-btm"], [settings.NoAD解析["OK解析解析"]], "#player", null);
            } else if (location.host.indexOf("bilibili.com") !== -1) {
                readyPlayerBox("进入哔哩哔哩", ["div.fe-ui-open-app-btn,div.recom-wrapper,open-app-btn", "[class*=openapp]"], [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"]], "div#app.main-container div.player-wrapper", null, href => href.replace("m.bilibili.com", "www.bilibili.com"));
            } else if (location.host.indexOf("miguvideo.com") !== -1) {
                readyPlayerBox("进入咪咕视频", ["[class^=app_],[class^=app-],[class*=_app_],[class*=-app-],[class$=_app],[class$=-app]", ".openClient", "div.group-item.programgroup .data-rate-01,div.group-item.programgroup .max-rate-01,div.group-item.programgroup .p-common"], [settings.NoAD解析["OK解析"]], "section#mod-player", null, href => href.replace("m.miguvideo.com", "www.miguvideo.com").replace("msite", "website"));
            } else if (location.host.indexOf("le.com") !== -1) {
                (block_show => {
                ...
                readyPlayerBox("进入乐视TV", ["a.leapp_btn", "div.full_gdt_bits[id^=full][data-url]", "[class*=Daoliu],[class*=daoliu],[class*=game]", "div.m-start", "[class*=icon_user]"], [settings.NoAD解析["OK解析"]], "div.column.play", null);
            } else if (location.host.indexOf("m.tv.sohu.com") !== -1) {
                readyPlayerBox("进入搜狐视频", ["div[class^=banner]", "div.js-oper-pos", "div[id^=ad],div[id^=ad] *", "[id*=login],[class*=login]", "[class$=-app]", "div.app-vbox.ph-vbox,div.app-vbox.app-guess-vbox", "div.twinfo_iconwrap", "div[class$=banner],div[id$=banner]"], [settings.NoAD解析["OK解析"]], "#player,#sohuplayer,.player-view", null, async href => {
                ...
                readyPlayerBox("进入芒果TV", ["div.adFixedContain,div.ad-banner,div.m-list-graphicxcy.fstp-mark", "div[class^=mg-app],div#comment-id.video-comment div.ft,div.bd.clearfix,div.v-follower-info", "div.ht.mgui-btn.mgui-btn-nowelt", "div.personal", "div[data-v-41c9a64e]"], [settings.NoAD解析["OK解析"], settings.NoAD解析["parwix解析"], settings.NoAD解析["艾特解析"], settings.NoAD解析["人我解析"], settings.NoAD解析["云解析"], settings.NoAD解析["猪蹄解析"], settings.NoAD解析["夜幕解析"]], "div.video-poster,div.video-area", null);
            } else if (location.host.indexOf("ixigua.com") !== -1) {
                readyPlayerBox("已进入西瓜视频", ["div.xigua-download", "div.xigua-guide-button", "div.c-long-video-recommend.c-long-video-recommend-unfold"], [settings.NoAD解析["OK解析"]], "div.xigua-detailvideo-video", null);
            } else if (location.host.indexOf("pptv.com") !== -1) {
                ...
                readyPlayerBox("进入1905电影网", ["a.new_downLoad[target=_blank]", "iframe[srcdoc^='<img src=']", "section.movieList_new.club_new", ".wakeAppBtn", "[class*=login]", "section.openMembershipBtn", ".ad", ".open-app,.openApp,ul.iconList li:not(.introduceWrap),div#zhichiBtnBox", "section#hot_movie,section#exclusive_movie,section#hot_telve"], [settings.NoAD解析["OK解析"]], "div.area.areaShow.clearfix_smile", null);
            } else if (location.host.indexOf("www.wasu.cn") !== -1) {
                readyPlayerBox("进入华数TV", ["div.ws_poster", "div.appdown,div.player_menu_con", "div#play_and_info_fix_adv"], [settings.NoAD解析["OK解析"]], "div#player,div#pop", null);
            }
        }
            ...
            doElement(cssString, async playerBox => {
                ...
                settings.randomSeleceParse = async () => {
                    ...
                    if (typeof doHref === "function") {
                        let href = await doHref(location.href);
                    ...
                    if (settings.parseDB.DIY解析栏) {
                        if (!DIY_iframe || DIY_iframe.constructor.name !== "HTMLDivElement") {
                            ...
                            async function DIY_iframe_src() {
                                ...
                                if (typeof doHref === "function") {
                                    let href = await doHref(DIY_iframe_text.value ? DIY_iframe_text.value : location.href);
        ...
        async function setParseBox() {
            ...
            if (!settings.parseDB.弹幕开关) {
                ...
                removeElements(['document.querySelector("div[class$=player-video-wrap]").getElementsByTagName("div")', 'document.querySelector("div[class$=playr-danmu]")', 'document.querySelector("div[class$=player-danmaku]")', 'document.querySelector("div[class*=player-comment-box]")', 'document.querySelector("div[class*=player-controller-mask]")', 'document.querySelector("[class*=player-list-icon]")', 'document.querySelector("div[class$=player-menu]")']);
            }
        }
