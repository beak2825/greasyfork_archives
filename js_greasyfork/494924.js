...
// @license      GPL
// @downloadURL https://update.greasyfork.org/scripts/494924/HTML5%E6%92%AD%E6%94%BE%E5%99%A8%E5%A2%9E%E5%BC%B7%E8%85%B3%E6%9C%AC371.user.js
// @updateURL https://update.greasyfork.org/scripts/494924/HTML5%E6%92%AD%E6%94%BE%E5%99%A8%E5%A2%9E%E5%BC%B7%E8%85%B3%E6%9C%AC371.meta.js
// ==/UserScript==
...
//設定跳秒skipStep:
  ...
  */
   ...
   * @param concatArr {boolean} -可选 合并数组，默认遇到数组的时候，直接以另外一个数组替换当前数组，将此設置true则，遇到数组的时候一律合并，而不是直接替换
   * @returns {*|void}
   */
  ...
  /**
   * 根据文本路径設置对象里面的值，如需支持数组请使用lodash的set方法
   * @param obj {Object} -必选 要操作的对象
   * @param path {String} -必选 路径信息
   * @param val {Any} -必选 如果不传该参，最终结果会被設置为undefined
   * @returns {Boolean} 返回true表示設置成功，否则設置失败
   */
  ...
  */
  ...
  const taskConf = {
    ...
    'demo.demo': {
      ...
      switchPlayStatus: '.player-play',
      playbackRate: function () {},
      // playbackRate: true, // 当给某个功能設置true时，表示使用网站自身的能力控制视频，而忽略插件的能力
      currentTime: function () {},
      addCurrentTime: '.add-currenttime',
      subtractCurrentTime: '.subtract-currenttime',
      // 自定义快捷键的执行方式，如果是组合键，必须是 ctrl-->shift-->alt 这样的顺序，没有可以忽略，键名必须全小写
  ...
  */
  ...
  class HookJs {
      ...
      if (useProxy && Proxy) {
        ...
        keys.forEach(keyName => {
          try {
            ...
            // 設置defineProperty的时候出现异常，可能导致hookMethod部分功能确实，也可能不受影响
    ...
    isAllowHook (parentObj, keyName) {
      /* 有些对象会設置getter，让读取值的时候就抛错，所以需要try catch 判断能否正常读取属性 */
  ...
  */
  ...
  const h5Player = {
    ...
    /**
     * 加强版的倍速调节，当短时间内設置同一个值时，会认为需更快的跳速能力
    ...
    palyerTrigger: function (player, event) {
      ...
      if (event.ctrlKey && keyCode === 39) {
        t.setCurrentTimeUp(t.skipStep * 10);
      }
      // ctrl+方向键左←：後退30秒
      if (event.ctrlKey && keyCode === 37) {
        t.setCurrentTimeDown(-t.skipStep * 10);
      }
