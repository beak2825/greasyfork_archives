// ==UserScript==
// @name         clan_sklad_protocols
// @namespace    http://tampermonkey.net/
// @version      0.3
// @description  clan and warehouse analyzing
// @author       SalmonGOD
// @match        /^https{0,1}:\/\/((www|my)\.(heroeswm|lordswm)\.(ru|com)|178\.248\.235\.15)\/(sklad_log|clan_log).php*/
// @include      /^https{0,1}:\/\/((www|my)\.(heroeswm|lordswm)\.(ru|com)|178\.248\.235\.15)\/(sklad_log|clan_log).php*/
// @icon         https://www.google.com/s2/favicons?sz=64&domain=lordswm.com
// @license      MIT
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/526539/clan_sklad_protocols.user.js
// @updateURL https://update.greasyfork.org/scripts/526539/clan_sklad_protocols.meta.js
// ==/UserScript==
...
(function() {
    ...
    let protocol_type = '';
    const protocols_type = ['clan_log', 'sklad_log'];
    if (location.href.includes('clan_log')) protocol_type = 'clan_log';
    if (location.href.includes('sklad_log')) protocol_type = 'sklad_log';
    ...
    const gold_img = create_el('img', 'width: 15px; height: 15px; margin-left: 3px;');
    ...
    const show_data = (data) => {
        let char_id = create_el('div', 'font-weight: bold', `ID of protocol: ${data.id}`);
        ...
        for (let i = 0; i < data.data.length; i++) {
            ...
            const create_stats = (information, lab) => {
                ...
                information.forEach(el => {
                    ...
                    let element = create_el('div', 'text-align: left; width: 250px', `${el[0]}`);
    ...
    const check_all_pages_count = async (id) => {
        let res = await fetch(`${link}/${protocol_type}.php?id=${id}&page=100050000`);
    ...
    const fetch_xml = (page) => {
        ...
        xhr.open('get', `${link}/${protocol_type}.php?id=${warehouse_id}&page=${page}`);
        ...
        xhr.addEventListener('load', () => {
            ...
            if (protocol_type === 'sklad_log') {
                list = global_elem.innerHTML.split('\n&nbsp;&nbsp;');
                list.shift();
            } else if (protocol_type === 'clan_log') {
                list = global_elem.innerHTML.split('<br>');
                list.pop();
            }

            for (let i = 0; i < list.length; i++) {
                let obj = {};
                let element = list[i].match(/Куплена 1 часть артефакта \S+ \S+/gi);
                let price = list[i].match(/за \d+/gi);
                let hero = list[i].match(/<b>.+<\/b>/gi);
                let heroLink = list[i].match(/pl_info.php?id=\d+/gi);
                let amount = 1;

                if (element !== null && element[0].includes('часть')) {
                    element = element[0];
                    element = element.replace('Куплена 1 часть артефакта', '');
                }
                price === null ? price = 0 : price = +price[0].match(/\d+/gi)[0];
                if (hero !== null) {
                    let index = hero[0].indexOf('/') - 1;
                    hero = hero[0].slice(0, index);
                    hero = hero.replace('<b>', (''));
                }
                ...
                if (protocol_type === 'clan_log') list[i] += '<br>';
                if (protocol_type === 'sklad_log') {
                    let art_id = list[i].match(/--.+/gi);
                    if (art_id !== null) {
                        art_id = art_id[0].match(/\d+/gi);
                        art_id = art_id.join(',');
                    } else {
                        art_id = '';
                    }
            ...
            if (xhr.status === 200 && start_stop && current_page > stop_page) {
                if (my_data) {
                    ...
                    add_data_to_db(warehouse_id, current_page, parsed_pages, stats);
                }
                current_page_amount.innerText = `${current_page}`;
                current_page = 0;
                stop_page = 0;
