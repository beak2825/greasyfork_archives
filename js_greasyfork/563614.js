...
// @match        https://www.pistonheads.com/gassing/forum.asp*
...
(function() {
    ...
    function toggleBlock(username) {
        ...
        if (isBlocked(username)) {
            ...
            //alert(`Unblocked: ${username} → posts will now reappear`);
        } else {
            blockedUsers.push(username);
            //alert(`Blocked: ${username} → posts will now be hidden`);
        }
    ...
    function hideBlockedQuotes(container) {
        container.querySelectorAll('.forumQuote').forEach(quote => {
            ...
            if (shouldHide && !isHidden) {
                ...
                quote.innerHTML = '';
                quote.appendChild(createPlaceholder(username, true, originalContent, quote));
    ...
    function updateVisibility() {
        document.querySelectorAll('.topic-reply').forEach(post => {
            ...
            if (shouldHide && !isHidden) {
                ...
                body.innerHTML = '';
                body.appendChild(createPlaceholder(username, false, originalContent, body));
    ...
    function createMenu(username, x, y) {
        ...
        item.addEventListener('mouseover', () => item.style.background = '#e6f0ff');
