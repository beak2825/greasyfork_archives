...
// @grant   	  none
// @version       1.7
// @downloadURL https://update.greasyfork.org/scripts/3954/Custom%20Salesforce%20UI.user.js
// @updateURL https://update.greasyfork.org/scripts/3954/Custom%20Salesforce%20UI.meta.js
// ==/UserScript==
...
/*DetailPage.prototype.handleRLAjaxResponse = function (a) {
    ...
    if (b.exElem) this.handleRLAjaxException(b.exElem);
    else {
        if (b.rls) {
            var a = [],
                c;
            for (c in b.rls) {
                ...
                f && (e && g) && (b.rls[c].title && Sfdc.Dom.updateHTML(f, b.rls[c].title), b.rls[c].buttons && (e.currentStyle && (b.rls[c].buttons = '<span style="display:none" id="__REMOVE">.</span>' + b.rls[c].buttons), Sfdc.Dom.updateHTML(e, b.rls[c].buttons), e.currentStyle && Sfdc.Dom.removeChild(document.getElementById("__REMOVE"), !0), this.relatedListPanel && this.relatedListPanel.getIFrameNode() && DomUtil.copyScripts(e, this.relatedListPanel.getIFrameNode().contentWindow.document, this.scriptsToNotCopy), Util.evalScriptsUnderElement(e)), f = document.createElement("div"), Sfdc.Dom.updateHTML(f, b.rls[c].content), g.parentNode.replaceChild(f.firstChild, g), this.relatedListsById[c].visibleRowCount = null, this.relatedListsById[c].listHasMore = null, this.relatedListsById[c].shouldLoad = !1, a.push(this.relatedListsById[c]))
        ...
        if (b.devFooterHTML && (c = document.getElementById("ajaxPlSql"))) c.innerHTML = b.devFooterHTML;
...
unsafeWindow.sendEmail2 = function() {
	...
	if (confirmSend) {
		...
		if (document.getElementById("Call").checked) {
			...
			SFDCurl = "/00T/e?ent=Task&nooverride=1&RecordType=01280000000cYz1&who_id=%20"+ document.getElementById("p2_lkid").value +"&retURL="+ document.getElementById("p3_lkid").value+"&what_id="+ document.getElementById("p3_lkid").value +"&tsk10=Call%20Manual&tsk5=Call&followup=1";
			...
			//https://cs12.salesforce.com/servlet/servlet.Integration?lid=00b80000001HgzP&eid=500V0000003UaBz&ic=1&retURL=/500V0000003UaBz&wrapMassAction=1&scontrolCaching=1
			// /00T/e?ent=Task&nooverride=1&RecordType=01280000000cYz1&who_id=%2000380000016Y6if&retURL=500V0000003UaBz&what_id=500V0000003UaBz&tsk10=Call%20Manual&tsk5=Call&followup=1 HTTP/1.1
			GM_xmlhttpRequest({
			  method: "GET",
			  url: SFDCurl,
			  synchronous:true,
...
(function(){
	...
	if(document.getElementById("ep") != null){
		...
		sf += '<input class="inpt" type="checkbox" name="openCTI">&nbsp;Resize OpenCTI element<br>';
	...
	if(GM_getValue("sidebarHide","false")=="true" || GM_getValue("sidebarFloat","false")=="true"){
 	...
 	//OpenCTI smaller
	if(GM_getValue("openCTI","false")=="true" && document.getElementById("sidebarDiv") != null){
		document.getElementById("sidebarDiv").firstChild.setAttribute("style", "max-height:200px;overflow:auto;");
	}
	...
	if(GM_getValue("tightLook","false")=="true"){
		if(document.getElementById("ep") != null){
			...
			//Case Feed resize
			css+= ".publishercontainer {padding-top:0px;padding-bottom:0px;padding-right:0px} .detailPage .feedpage .feedmain {line-height: 12px;} .feedpage .feedmain {padding-bottom: 0px;} body .quickActionPanel { margin-top: 0px; padding-left: 0px; padding-right: 0px;padding-bottom: 0px;} .publishercontainer .quickActionPanel .quickActionField {padding-bottom: 0px;} .publishercontainer .quickActionPanel .quickActionFieldLabel {padding-bottom: 0px;padding-top: 0px;} .publishercontainer .quickActionPanel .quickActionFieldLabel, .publishercontainer .quickActionPanel .quickActionFieldElement {margin-left: 0px;  margin-right: 0px;} .publishercontainer .bottomBar {padding: 0px; height: 20px;} .feedcontainer {padding-bottom: 0px; line-height: 12px;} .zen .comboMenu.feedsupermenu { padding-bottom: 0px;} .feedcontainer .feeditem {margin: 0 0px 0px 0; padding: 0 0px 0px 0;border-top: 1px solid #9dbaed;} .feedcontainer .feeditem .preamblecontainer {padding-bottom: 0px;} .feeditembody .feeditemaux { padding: 0px 0px 0px 0px;}"
		}
		...
		//add word-wrap into related lists
		css+="body .pbBody table.list tr.last > th, body .pbBody table.list tr.last > td, .bPageBlock.bEditBlock .bRelatedList tr.dataRow.last > td, .bPageBlock.bEditBlock .bRelatedList tr.dataRow.last > th { border-bottom-width: 0px;max-width: 300px;overflow-wrap:break-word;word-break: break-all;}"

		//the new global header minimizing
		css+=".globalHeaderBar {height:0px;} .globalHeaderNameMenuContainer {width: 450px;}"
		
		//scroll so the top menu is not visible
		window.scrollTo(0,50);


		//add color on alternative feed items
		if(document.getElementById("feedwrapper")!=null) {
			var ParentFeed = document.getElementById("feedwrapper").childNodes[1];
			var q = 0;
			var z=0;
			while (q < ParentFeed.childNodes.length) {
				if(ParentFeed.childNodes[q].nodeName=="DIV"){
					if (z==0){
						ParentFeed.childNodes[q].setAttribute("style","background-color:#fafafa;");
						z=1;
					}
					else {
						ParentFeed.childNodes[q].setAttribute("style","background-color:#f0f0f0;");
						z=0;
					}
				}
				q+=1;
