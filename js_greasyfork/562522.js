// ==UserScript==
// @name         Lolzteam Multiaccount Finder by iney
// @version      3.0.1
// @license MIT
...
// @downloadURL https://update.greasyfork.org/scripts/562522/Lolzteam%20Multiaccount%20Finder%20by%20iney.user.js
// @updateURL https://update.greasyfork.org/scripts/562522/Lolzteam%20Multiaccount%20Finder%20by%20iney.meta.js
// ==/UserScript==
...
function findMenuElements() {
    ...
    if (accountMenu.length) {
        linksList = accountMenu.find(".blockLinksList");
        if (linksList.length) {
            logWithPrefix("Найдено старое меню AccountMenu с blockLinksList");
            return true;
        }
    }
 
    const navigation = $("#navigation .account-links");
    if (navigation.length) {
        manageItems = navigation;
        logWithPrefix("Найдена навигация account-links");
        return true;
    }
...
function initializeMenu() {
    ...
    const checkInterval = setInterval(() => {
        ...
        if (findMenuElements()) {
            ...
            clearInterval(checkInterval);
            logWithPrefix("Не удалось найти меню после 50 попыток (5 секунд)");
        }
    }, 100);
}
...
function setupMenuButton() {
    ...
    if (linksList && linksList.length) {
        ...
        logWithPrefix("Кнопка добавлена в старое меню");
    } else if (manageItems && manageItems.length) {
        const multiaccountFinderItem = `
            <li class="navTab Popup PopupControl PopupClosed" id="${buttonId}-container">
                <a href="javascript:void(0)" class="navLink" id="${buttonId}">
                    <div class="counter-container">
                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 20 20" fill="none">
                            ...
                            <path d="M7.5 9.16667C7.5 8.66663 7.83333 8.33329 8.33333 8.33329C8.83333 8.33329 9.16667 7.99996 9.16667 7.49996C9.16667 6.99996 8.83333 6.66663 8.33333 6.66663C7.33333 6.66663 6.66667 7.33329 6.66667 8.33329" stroke="currentColor" stroke-width="1.66667" stroke-linecap="round" stroke-linejoin="round"/>
                            <circle cx="8.75" cy="10.4167" r="0.41667" fill="currentColor"/>
                        </svg>
                    </div>
                </a>
            </li>
        ...
        const accountTab = manageItems.find('li.navTab.account').first();
        if (accountTab.length) {
            accountTab.before(multiaccountFinderItem);
            logWithPrefix("Кнопка добавлена перед аккаунтом в навигации");
        } else {
            // Если аккаунта нет, добавляем в конец навигации
            manageItems.append(multiaccountFinderItem);
            logWithPrefix("Кнопка добавлена в конец навигации");
        }
    }
...
function setupProfileLink() {
    if (linksList && linksList.length) {
        const profileLinkEl = $("#AccountMenu > ul > li:nth-child(1) > a");
        if (profileLinkEl.length) {
            profileLink = profileLinkEl.attr("href").split(`${domain}`)[1];
        }
    } else {
        let profileLinkElement = $("#navigation .account-links .avatar img, #AccountMenu .image-block a, #AccountMenu .about-user a.username").first();
 
        if (profileLinkElement.length && profileLinkElement.is('img')) {
            profileLinkElement = profileLinkElement.closest('span').parent('span').closest('li').find('span[id*="account-style"]');
            if (!profileLinkElement.length) {
                const usernameEl = $("#NavigationAccountUsername");
                if (usernameEl.length) {
                    const username = usernameEl.text().trim();
                    profileLink = '/' + username.toLowerCase() + '/';
                    logWithPrefix("Найдена ссылка на профиль по имени пользователя: " + profileLink);
                    return;
                }
            }
        }
        ...
        if (profileLinkElement.length && !profileLinkElement.is('img')) {
            const href = profileLinkElement.attr("href");
            if (href) {
                profileLink = href.replace(`https://${domain}`, '').replace(`${domain}`, '');
                if (!profileLink.startsWith('/')) {
                    profileLink = '/' + profileLink;
                }
                logWithPrefix("Найдена ссылка на профиль: " + profileLink);
            }
        }
 
        if (!profileLink) {
            const currentPath = window.location.pathname;
            if (currentPath && currentPath !== '/' && !currentPath.includes('/threads/')) {
                profileLink = currentPath;
                logWithPrefix("Ссылка на профиль получена из текущего URL: " + profileLink);
            }
        }
    }
}
...
async function openSettings(page = 1, lastId = generateRandomString(10)) {
    ...
    XenForo.alert(content, 'Lolzteam Multiaccount Finder by iney');
    ...
    for (let i = 0; i < sharedItems.length; i++) {
        ...
        if (makeClaimLink) {
            ...
            for (let j = 0; j < bannedModule.length; j++) {
                ...
                let sympathies = lastElement.querySelector('a.counter:nth-child(1) span.count').textContent.replace(/ /g, "");
                let messages = lastElement.querySelector('a.counter:nth-child(3) span.count').textContent.replace(/ /g, "");
...
async function AutoCheckOnlineRegistered() {
    if ((await checkKeyValue("autocheck-online-registered")) === 'true') {
        if (window.location.href.indexOf(`https://${domain}/online/?type=registered`) === 0) {
            const currentPageElement = document.querySelector('a.currentPage');
            const pageNavElement = document.querySelector('.PageNav');
            const maxPage = pageNavElement ? parseInt(pageNavElement.getAttribute('data-last')) : null;
            const ulElement = document.querySelector('#content > div > div > div.mainContainer > div > ul');
 
            if (!currentPageElement || !maxPage) {
                if (await checkKeyValue("CircularNavigation") === 'true') {
                    ...
                    window.location.href = `https://${domain}/online/?type=registered`;}} else {
                        ...
                        const members = document.querySelectorAll('.primaryContent.memberListItem');
                        ...
                        const checkNextMember = async () => {
                            ...
                            const usernameLink = member.querySelector('.member h3.username a.username');
                            const usernameStyle = member.querySelector('.member h3.username a.username > span')
                            const usernameHref = usernameLink.getAttribute('href');
                            const userStatCounters = member.querySelector('.member .userStatCounters');
                            ...
                            if (usernameStyle.classList.contains('banned') || usernameStyle.classList.contains('style18') || usernameStyle.classList.contains('style360')) {
                                ...
                                if (usernameLink.textContent !== hrefSearchUsers) {
                                    const sympathies = member.querySelector(".userStatCounters a.counter:nth-child(1) span.count").textContent.replace(/ /g, "")
                                    const messages = member.querySelector(".userStatCounters a.counter:nth-child(3) span.count").textContent.replace(/ /g, "")
...
function logWithPrefix(text) {
    console.log(`[Lolzteam Multiaccount Finder by iney] ${text}`)
}
...
async function template(name, description, link, open = true, source = undefined, bannedPercent) {
    ...
    if (await checkKeyValue("AutomaticFraudComplaint") === "true") {
        if (await checkKeyValue("ForumComplaintHide") === "true") {
            message = `[CLUB]1. Никнейм нарушителя и ссылка на профиль: ${link.replace('/shared-ips', '')}\n2. Краткое описание жалобы: ${description}\n3. Доказательства: ${link}[/CLUB]\nЖалоба была сформирована с использованием Lolzteam Multiaccount Finder by iney ([URL]https://${domain}/posts/59576374/[/URL])[CLUB]Создано автоматически\nДата и время: ${moscowTime}[/CLUB]`
        }
        else {
    ...
    if (source === "members" || source === "registered") {
        if (await checkKeyValue("AutomaticFraudComplaint") === "true") {
            if (parseInt(await checkKeyValue("AutomaticFraudComplaintPercentage")) <= bannedPercent) {
...
async function checkUser(link, source, gifId, style = null) {
    ...
    if (notToCheckPreviouslyChecked && checkedItem) {
        ...
        if (gifElement) {
            ...
            xenforoLogAndAlert(`${output}`, `Lolzteam Multiaccount Finder by iney`);
        }
        return;
    }
    ...
    const [bannedPercent, nonBannedPercent] = [bannedUsersCount, nonBannedUsersCount].map(x => totalUsers ? parseFloat((x / totalUsers * 100).toFixed(2)) : 0);
    ...
    if (htmlDocument.body.textContent.includes("Пользователей по заданным параметрам не найдено.") || htmlDocument.body.textContent.includes("No matching users were found.")) {
        ...
        const cases = createCases(bannedPercent, nonBannedUsersCount, bannedUsersCount, nonBannedPercent);
        let caseFound = false;
