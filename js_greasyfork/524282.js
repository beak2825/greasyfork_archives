...
(function () {
    ...
    reg_menu();
    function inject_deps() {
        var get_elem = document.querySelector('[http-equiv=Content-Security-Policy]')
        if (get_elem != null && get_elem != undefined) {
            get_elem.remove()
        }
        document.head.insertAdjacentHTML("beforeend", `<meta http-equiv="Content-Security-Policy" content="script-src 'unsafe-inline' 'unsafe-eval' *;">`)
        setTimeout(() => {
            ...
            script2.innerHTML = htmltoimage_tmpl;
            document.head.appendChild(script2);
        }, 1000)
    }
    setTimeout(() => { inject_deps() }, 200)
