// ==UserScript==
// @name         壳壳杂货铺
// @icon         https://bbs.kkok.cc/favicon.ico
// @namespace    https://greasyfork.org/zh-CN/scripts/401359-壳壳杂货铺
// @supportURL   https://greasyfork.org/zh-CN/scripts/401359-壳壳杂货铺/feedback
// @description  壳壳杂货铺效果增强，如自动签到、自动展开帖子、滚动加载评论、显示UID、屏蔽用户、手机版小黑屋、编辑器优化、在线用户查看、便捷式图床等
// @version      2.8.2
// @author       WhiteSevs
// @match        http*://bbs.kkok.cc/*
...
// @downloadURL https://update.greasyfork.org/scripts/460912/%E5%A3%B3%E5%A3%B3%E6%9D%82%E8%B4%A7%E9%93%BA.user.js
// @updateURL https://update.greasyfork.org/scripts/460912/%E5%A3%B3%E5%A3%B3%E6%9D%82%E8%B4%A7%E9%93%BA.meta.js
// ==/UserScript==
...
(function () {
	...
	let MT_CONFIG = {
		...
		regexp: {
			bbs: /bbs.kkok.cc/ /* 论坛 */,
			searchUrl: /bbs.kkok.cc\/search.php/g /* 搜索页 */,
			...
			homeUrlWithAt: /bbs.kkok.cc\/space-uid-/g /* 个人空间页的@点进去 */,
			homeKmisignUrl:
				/bbs.kkok.cc\/(forum.php\?mod=guide&view=hot(|&mobile=2)|k_misign-sign.html)/g /* 主页和签到页链接 */,
			homeSpaceUrl:
				/bbs\.kkok\.cc\/home\.php\?mod=space&do=profile&mycenter/g /* 【我的】 个人信息页链接 */,
			homeSpacePCUidUrl: /space-uid-(.*?).html/ /* PC 个人空间链接uid */,
			replyForum:
				/bbs.kkok.cc\/forum.php\?mod=post&action=reply/g /* 回复的界面url */,
			signUrl: "" /* 签到url */,
			navigationUrl:
			...
			forumPost:
				/(bbs.kkok.cc\/thread-|bbs.kkok.cc\/forum.php\?mod=viewthread)/g /* 帖子链接 */,
			forumPostPC: /.*:\/\/bbs.kkok.cc\/thread.*/ /* 资料设置 */,
			dataSettingUrl:
				/mod=space&do=profile&set=comiis&mycenter=1/ /* 帖子链接-PC */,
			forumGuideUrl: /bbs.kkok.cc\/forum.php\?mod=guide/g /* 导航链接 */,
			forumPostReply:
	...
	/*  mt全屏遮罩调用
        popup.open('<img src="https://bbs.kkok.cc/template/comiis_app/comiis/img/imageloading.gif" class="comiis_loading comiis_noloadimage">');
        关闭方式
        popup.close()
    */
	...
	const pc = {
		...
		collectionForumPost() {
			...
			var collect_href =
				"https://bbs.kkok.cc/home.php?mod=spacecp&ac=favorite&type=thread&id=" +
				collect_href_id +
				"&formhash=" +
				own_formhash;
			...
			function run() {
				document.querySelectorAll("#postlist .comiis_vrx").forEach((item) => {
					...
					item.querySelectorAll("img").forEach((_item_) => {
						...
						if (
							IMG_URL_HOSTNAME.indexOf("bbs.kkok.cc") != -1 ||
		...
		latestReleaseForumPost() {
			...
			var latestReleaseNode = $jq(
				`<li id="latest_publication"><a href="https://bbs.kkok.cc/forum.php?mod=guide&view=newthread" hidefocus="true" title="最新发表">最新发表</a></li>`
			);
			$jq("#comiis_nv .wp.comiis_nvbox.cl ul").append(latestReleaseNode);
			if (
				window.location.href ==
				"https://bbs.kkok.cc/forum.php?mod=guide&view=newthread"
			) {
	...
	const mobileRepeatFunc = {
			...
			if (
				...
				(window.location.href.match(MT_CONFIG.regexp.forumPostGuideUrl) ||
					...
					window.location.href.match(
						/bbs.kkok.cc\/home.php\?mod=space&do=thread&view=me/
					) ||
					window.location.href.match(
		...
		pageSmallWindowBrowsingForumPost() {
			...
			async function resumeBack() {
				...
				while (1) {
					if (window.location.href == "https://bbs.kkok.cc/#") {
						console.log("back！");
	...
	const mobile = {
		...
		blackHome() {
			async function showBlackHomeView() {
                ...
                <div class="blackhome-user-avatar-name">
									<img src="https://bbs.kkok.cc/uc_server/avatar.php?uid=${value["uid"]}&size=small" loading="lazy">
			...
			async function getBlackList(cid = "") {
				...
				return new Promise((resolve) => {
					GM_xmlhttpRequest({
						url:
							"https://bbs.kkok.cc/forum.php?mod=misc&action=showdarkroom&cid=" +
							cid +
							"&t=&ajaxdata=json",
						timeout: 5000,
						method: "GET",
						...
						<div class="msgdivmain" data="shieldUser">
							<div class="msgdivflex">
								<a href="home.php?mod=space&uid=${item["uid"]}&do=profile">
									<img src="https://bbs.kkok.cc/uc_server/avatar.php?uid=${item["uid"]}&size=small" loading="lazy">
								</a>
								<div>
						...
						btn1: function () {
							...
							html2canvas(pageHTML, html2canvasOptions).then((canvas) => {
								...
								var data = {
									...
									title: document.title.replace(" - 壳壳杂货铺", ""),
									data: base64Image,
						...
						Promise.all(
							Array.from(chooseImageFiles).map(async (item, index) => {
								if (item.type === "image/gif") {
									...
									watermark.addText({
										text: [GM_getValue("chartBedWaterMarkText", "壳壳杂货铺")],
			...
			function chartbedByMT() {
				...
				$jq(document).on("change", "#filedata_mt", async (e) => {
					...
					function getCSRFToken() {
						...
						return new Promise((resolve) => {
							GM_xmlhttpRequest({
								url: "https://img.kkok.cc/",
								method: "get",
								async: false,
								timeout: 5000,
								headers: {
									"user-agent": Utils.getRandomPCUA(),
									referer: "https://img.kkok.cc/",
									origin: "https://img.kkok.cc",
								},
								onload: (response) => {
									console.log(response);
		...
		editorOptimization() {
			...
			function chartbedWaterMarkEvent() {
				...
				$jq("#watermarktext")
					.val(GM_getValue("chartBedWaterMarkText", "壳壳杂货铺"))
		...
		loadNextComments() {
			...
			if (
				GM_getValue("v21") &&
				window.location.href.match(MT_CONFIG.regexp.forumPost) &&
				document.title.indexOf("提示信息 - 壳壳杂货铺") == -1
			) {
				let tip_html = `
		...
		onlineUserView() {
			...
			function getOnlineUsers() {
				...
				return new Promise((resolve) => {
					GM_xmlhttpRequest({
						url: "https://bbs.kkok.cc/forum.php?showoldetails=yes",
						method: "GET",
						timeout: 5000,
						headers: {
							"User-Agent": Utils.getRandomPCUA(),
						...
						onload: function (response) {
							...
							onlineList.each((index, item) => {
								...
								let avatar = `https://bbs.kkok.cc/uc_server/avatar.php?uid=${uid}&size=middle`; /* 头像 */
		...
		pageAfterDOMChangeRunFunction() {
			/* 当本页面动态加载帖子需要重复加载的东西 */
			if (
				window.location.href.match(/bbs.kkok.cc\/forum/) ||
				window.location.href.match(
					/bbs.kkok.cc\/home.php\?mod=space&do=thread&view=me/
				) ||
				window.location.href.match(
				...
				if (qq) {
					qq.forEach((item) => {
						...
						let content = match_content
							? match_content[match_content.length - 1]
							: "";
						/* 这个是以前的wpa协议，现在是tencent协议，mt的discuz没有更新，如：tencent://message/?uin=xxx&site=bbs.kkok.cc&menu=yes  */
						text = text.replace(
							item,
		...
		repairUnableToEnterOtherSpaceCorrectly() {
			...
			if (window.location.href.match(MT_CONFIG.regexp.homeUrlWithAt)) {
				...
				href_params = href_params[href_params.length - 1];
				window.location.href = `https://bbs.kkok.cc/home.php?mod=space&uid=${href_params}&do=profile`;
			}
		},
		async searchHistory() {
			/* 搜索历史 */
		...
		setDynamicAvatar() {
            ...
            <img loading="lazy" style="
                ...
                border-radius: 50%;
                overflow: hidden;
            " src="https://bbs.kkok.cc/uc_server/avatar.php?uid=${uid}&size=big&ts=1" class="kmtximg">
            <img loading="lazy" style="
                float: right;
                width: 30px;
                height: 30px;
                ...
                margin-left: 10px;
                margin-right: 10px;
            " src="https://bbs.kkok.cc/uc_server/avatar.php?uid=${uid}&size=middle&ts=1" class="kmtximg">
            <img loading="lazy" style="
                float: right;
                width: 30px;
                height: 30px;
                border-radius: 50%;
                overflow: hidden;
            " src="https://bbs.kkok.cc/uc_server/avatar.php?uid=${uid}&size=small&ts=1" class="kmtximg">
			...
			function getPCUploadNewAvater() {
				return new Promise((resolve) => {
					GM_xmlhttpRequest({
						url: "https://bbs.kkok.cc/home.php?mod=spacecp&ac=avatar",
						method: "GET",
						timeout: 5000,
						headers: {
							"User-Agent": Utils.getRandomPCUA(),
					...
					ok: {
						...
						callback: async () => {
							...
							GM_xmlhttpRequest({
								...
								data: formData,
								headers: {
									origin: "https://bbs.kkok.cc",
									referer:
										"https://bbs.kkok.cc/home.php?mod=spacecp&ac=avatar",
									accept:
		...
		showLatestPostForm() {
			...
			function getLatestPostForm() {
				...
				return new Promise((resolve) => {
					GM_xmlhttpRequest({
						url: "https://bbs.kkok.cc/forum.php?mod=guide&view=hot",
						method: "GET",
						timeout: 8000,
						async: false,
						responseType: "html",
			...
			if (window.location.href.match(MT_CONFIG.regexp.kMiSignSign)) {
				...
				let getMaxPage = (urlextra) => {
					return new Promise((resolve) => {
						GM_xmlhttpRequest({
							url:
								"https://bbs.kkok.cc/k_misign-sign.html?operation=" + urlextra,
							async: false,
							responseType: "html",
							timeout: 5000,
							headers: {
				...
				let getPagePeople = (page) => {
					return new Promise((resolve) => {
						GM_xmlhttpRequest({
							url:
								"https://bbs.kkok.cc/k_misign-sign.html?operation=list&op=&page=" +
								page,
							async: false,
							timeout: 5000,
							responseType: "html",
							...
							onload: function (response) {
								...
								for (let i = 1; i <= peoples.length - 2; i++) {
									...
									ret_json["uid"] = uid;
									ret_json["avatar"] =
										"https://bbs.kkok.cc/uc_server/avatar.php?uid=" +
										uid +
										"&size=small";
	...
	function firstUseTip() {
		...
		if (GM_getValue("firstUse", true) && envIsMobile()) {
			var tipImage = "https://kkok.cc/zykkok/8182751.gif";
			popup2.confirm({
				text: `<p>如果您是第一次使用，请看演示GIF</p><image src="${tipImage}" alt="演示图-4MB" loading="eager" crossoriginNew="anonymous" height="400"></image>`,
				cancel: {
					text: "不再提醒",
					callback: () => {
