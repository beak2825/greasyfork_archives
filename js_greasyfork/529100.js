// ==UserScript==
// @name         [Beta] BetterLZT By LolzNews
// @namespace    betterbeta
// @version      0.0.0
// @author       monkey
// @description  Не распространять лол
...
// @downloadURL https://update.greasyfork.org/scripts/529100/%5BBeta%5D%20BetterLZT%20By%20LolzNews.user.js
// @updateURL https://update.greasyfork.org/scripts/529100/%5BBeta%5D%20BetterLZT%20By%20LolzNews.meta.js
// ==/UserScript==
...
`;
  ...
  function exuiOnPage() {
    if (document.querySelector(".titleBar h1").innerHTML === "LN BetterLZT - Настройки") return;
    let htmlall = `<br>
    <details>
    ...
    <br>
        <details>
        <summary>BetterLZT AI<br><span>Исскуственный интелект и фактор доверия</span></summary>
        <div>
            <p>ВАЖНО! Функции ИИ находятся на этапе разработки и могут допускать ошибки</p>
            <br>
            <div class="btns-l" id="trustFactor">Фактор доверия (OLD)</div>
            <div class='btns-l' id='rai'> ИИ Ассистент по Розыгрышам</div>
            <div class='btns-l' id='rai_textgen'> Генерация текста <span style="color: red">Отвественность лежит на вас</span></div>
            <div class='btns-l' id='rai_report'> Поиск нарушение в сообщениях <span style="color: red">Отвественность лежит на вас</span></div>
         </div>
    </details>
    <br>
    <details>
  ...
  function threadRender() {
    ...
    button.tabIndex = -1;
    button.setAttribute("style", "font-weight: bold; padding: 3px 10px; background: rgb(45, 45, 45); border-radius: 50px; margin-right: 5px; cursor: pointer;");
        ...
        try {
          ...
          try {
            ...
            if (!response.ok) {
              ...
              if (jsonData.error === "429_2") return xfAlert("Слишком часто! Попробуйте через час");
              if (jsonData.error === "429_3") return xfAlert("Слишком часто! Попробуйте завтра");
              if (jsonData.error === "500" || jsonData.error === "501") return xfAlert("RAI не может обработать этот запрос");
              if (jsonData.error === "403") return xfAlert("Доступ запрещен");
              if (jsonData.error === "Prompt is required") return xfAlert("Клиентская ошибка");
            }
            if (responseText == 1) {
              let span2 = document.createElement("span");
              span2.innerHTML = '<i class="fas fa-sparkles"></i> Обнаружено нарушение';
              span2.className = "raiReport-button";
              span2.setAttribute("style", "font-weight: bold; padding: 3px 10px; background: red; border-radius: 50px; margin-right: 5px; cursor: pointer;");
              if (block.querySelector(".publicControls")) block.querySelector(".publicControls").prepend(span2);
              return;
            } else if (responseText == 2) {
              let span2 = document.createElement("span");
              span2.innerHTML = '<i class="fas fa-sparkles"></i> Нарушений не найдено';
              span2.className = "raiReport-button";
              span2.setAttribute("style", "font-weight: bold; padding: 3px 10px; background: green; border-radius: 50px; margin-right: 5px; cursor: pointer;");
              if (block.querySelector(".publicControls")) block.querySelector(".publicControls").prepend(span2);
              return;
            }
          } catch (error) {
