// ==UserScript==
// @name         Moonliteco.in (Pescador de Cripto)
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/415320/Moonlitecoin%20%28Pescador%20de%20Cripto%29.user.js
// @updateURL https://update.greasyfork.org/scripts/415320/Moonlitecoin%20%28Pescador%20de%20Cripto%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    function roll(){
        ...
        if($('.close').eq(1).is(':visible')){
            message = "MoonLiteco.in\n- Sucesso: Recebeu " + $('[data-bind="text: claimedAmount().toFixed(8)"]').eq(0).text() + " LTC\n- CoinPot balance: " + $('[data-bind="text: balance().toFixed(8)"]').eq(1).text() + " LTC";
            ntb(message);
    ...
    function open(){
        if (window.location.href.indexOf("pescadordecripto.com/dashboard") > -1) {
            var win_moonlitecoin = window.open("https://moonliteco.in/faucet", "MoonLitecoin","width=10,height=10,left=-3000,top=-3000");
        }
    ...
    $(document).ready(function(){
        setTimeout(function(){
            if (window.location.href.indexOf("moonliteco.in/faucet") > -1) {
                moonBtc();
            }
        },10000);
    });
    if (window.location.href.indexOf("pescadordecripto.com/install") > -1) {
        document.getElementById('moonliteco-in').classList.add("faucet-active");
    }
})();