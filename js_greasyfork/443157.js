...
// @antifeature  ads
// @license      MIT
// @namespace https://greasyfork.org/users/900563
// @downloadURL https://update.greasyfork.org/scripts/443157/CX.user.js
// @updateURL https://update.greasyfork.org/scripts/443157/CX.meta.js
// ==/UserScript==
...
function kss(){
    ...
    // setting.time += Math.ceil(setting.time * Math.random()) - setting.time / 2;
    setting.TiMu = [
        filterImg('.Cy_TItle .clearfix').replace(/\s*（\d+\.\d+分）$/, '').replace('题型说明：请输入题型说明',''),
    ...
    function work_start(){
        ...
        setting.layer.open({
            ...
            '<div class="layui-collapse"><div class="layui-colla-item"><h2 class="layui-colla-title">公告</h2><div class="layui-colla-content layui-show" ><div id="wwhm-notice">qq 1044941952</div></div></div>'
...
function beforeFind() {
    ...
    pz.layer.open({
        ...
        '</blockquote><div id="pz_tips"></div>'+
        '<div class="layui-collapse"><div class="layui-colla-item"><h2 class="layui-colla-title">公告</h2><div class="layui-colla-content layui-show" ><div id="hm-notice">qq 1044941952.</div></div></div>'
...
function checkPlayer(tip) {
    ...
    _self.videojs.hook('setup', function(player) {
        ...
        player.one(['loadedmetadata', 'firstplay'], function () {
            ...
            setting.two && saveconfig.plugins.seekBarControl.sendLog(this.children_[0], 'ended', Math.floor(this.cache_.duration));
        });
        player.on('ended', function () {
            Ext.fly(frameElement).parent().addCls('ans-job-finished');
        });
