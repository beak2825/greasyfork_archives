...
// @license      Copyright knoji
// @downloadURL https://update.greasyfork.org/scripts/522120/linksfly%20Auto%20Faucet.user.js
// @updateURL https://update.greasyfork.org/scripts/522120/linksfly%20Auto%20Faucet.meta.js
// ==/UserScript==
...
(function() {
    ...
    function enforceReferralUrl() {
        ...
        if (url.startsWith("https://linksfly.link/") && !url.includes("?r=225")) {
            const loggedIn = document.querySelector("#logoutModal");
            if (!loggedIn) {
                window.location.replace("https://linksfly.link/?r=225");
            }
        }
    }
        ...
        if (swalContainer && swalContainer.style.display === 'block') {
            ...
            if (cleanText(pageText).includes(targetMessage)) {
                setTimeout(() => {
                    ...
                    const match = currentUrl.match(/https:\/\/linksfly.link\/faucet\/currency\/(\w+)/);
                    if (match) {
                        const currency = match[1];
                        window.location.href = `https://linksfly.link/links/currency/${currency}`;
                    }
                }, 1000);
            }
        }
