...
// @license      MIT
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/538213/BUSTR%3A%20Busting%20Reminder%20%2B%20PDA.user.js
// @updateURL https://update.greasyfork.org/scripts/538213/BUSTR%3A%20Busting%20Reminder%20%2B%20PDA.meta.js
// ==/UserScript==
...
const PDA_API_KEY = 'cmIkq3IsrclcGPrf';
function isPDA() {
...
function renderHardnessScore(playerEl, hardnessScore) {
  const hardnessSpan = playerEl.querySelector('.bustr-hardness-score');
  hardnessSpan.textContent = hardnessScore;

  const penaltyScore = getPenaltyScore();
  const penaltyThreshold = getPenaltyThreshold();
  const projectedScore = penaltyScore + hardnessScore;

  if (projectedScore < penaltyThreshold) {
    hardnessSpan.style.color = '#85b200'; // Green (safe)
    hardnessSpan.title = 'Safe: Will not exceed penalty threshold';
  } else if (projectedScore === penaltyThreshold) {
    hardnessSpan.style.color = '#ffbf00'; // Yellow (risky)
    hardnessSpan.title = 'Risky: Will exactly hit your threshold';
  } else {
    hardnessSpan.style.color = '#e64d1a'; // Red (dangerous)
    hardnessSpan.title = 'Dangerous: Will exceed your threshold';
  }
}
