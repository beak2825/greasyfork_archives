...
// @downloadURL https://update.greasyfork.org/scripts/474884/Twitter%20Click%27n%27Save%20Sa.user.js
// @updateURL https://update.greasyfork.org/scripts/474884/Twitter%20Click%27n%27Save%20Sa.meta.js
// ==/UserScript==
// ---------------------------------------------------------------------------------------------------------------------
// ---------------------------------------------------------------------------------------------------------------------

const settings = loadSettings();
const filenameGlobal = `twitter @{author}, name. 【{authorName}】, twitter id. {status-id}, mediaName. {media-name}.{extension}`;
...
function makeName(author, authorName, stausId, mediaName, ext)
{
    return filenameGlobal.replace(/\.?{author}/, author).replace(/\.?{authorName}/, authorName).replace(/\.?{status-id}/, stausId).replace(/\.?{media-name}/, mediaName).replace(/\.?{extension}/, "." + ext);
}

function getFullImage(btn)
{   let url = btn.dataset.url;
    ...
    // sa code
    console.log('_________________full image url = ' + url);
    return url;
}
...
function hoistFeatures() {
    class Features {
        ...
        static rightClickHandler(ev, btn) {
            ev.preventDefault();
            let fullImageUrl = getFullImage(btn);

            if (window.event.ctrlKey)
            {   console.log("control right click");
...
<div class="ujs-btn-common ujs-btn-error-text"></div>`.slice(1);
           ...
           //btn.classList.add("buttonParent");
        ...
        static async _downloadBanner(url, btn) {
            ...
            const filename = `twitter[bg] @${username}—${id}—${seconds}—.${extension}`;
        ...
        static async imagesHandler(preventBlinking) {
            ...
            for (const img of images) {
                ...
                btn.addEventListener('contextmenu', (ev) => Features.rightClickHandler(ev, btn));
        ...
        static async _imageClickHandler(event) {
            ...
            // image filename
            // const filename = `twitter${sampleText}—${author}—${id}—${name}—.${extension}`;
            console.log("___url : " + url);
            let authorName = await API.getAuthorName(btn);
            let filename = makeName(author, authorName, id, name, extension);
            console.log("_____________out: " + filename);
        ...
        static async _videoClickHandler(event) {
            ...
            let authorName;
            try {
                ({video, tweetId: id, screenName: author, vidNumber, authorName} = await API.getVideoInfo(id, author, posterUrl));
            ...
            // video filename
            //const filename = `twitter ${author}——${id}—${name}—.${extension}`;
            //const filename = `twitter @${author}, ${authorName}, ${id}, ${name}.${extension}`;
            let filename = makeName(author, authorName, id, name, extension);
            console.log("_____________out: " + filename);
        ...
        */
        ...
        static async _downloadVideoMediaEntry(mediaEntry, btn, id /* of original tweet */) {
            ...
            //const filename = `twitter @${author}, ${authorName}, ${id}, ${name}.${extension}`;
            let video; // {bitrate, content_type, url}
            let authorName = API.getAuthorName(btn);
                ...
                if (anchor.dataset?.testid === "UserUrl") {
                    ...
                    if (anchor.textContent.endsWith("…")) {
                        ...
                        try {
                            ...
                            console.log("___author: ");
                            console.log("___author: " + author);
...
.ujs-progress {
  background-image: linear-gradient(to right, var(--ujs-yellow) var(--progress), transparent 0%);
}
...
function hoistTweet() {
    class Tweet {
        ...
        get author() {
            console.log("_____________URL: " + this.url);
...
function hoistAPI() {
    class API {
        ...
        static async apiRequest(url) {
            ...
            console.log("[ujs][apiRequest]", _url);
            ...
            try {
                ...
                //console.log("_____________[apiRequest]" + JSON.stringify(json, null, 2));
            } catch (e) {
                console.error(e, _url);
                throw e;
            }

            //console.log("____JSON: " + JSON.stringify(json, null, 2));
        ...
        static async getAuthorName(button)
        {
            let status_id = Tweet.of(button).id;
            //console.log("___getAuthorName status_id " + status_id);
            let url = ' https://x.com/i/api/2/timeline/conversation/' + status_id + '.json?tweet_mode=extended&include_entities=false&include_user_entities=false';
            //console.log("___getAuthorName url " + url);
            const json = await API.apiRequest(url);
            //console.log("___JSON " + JSON.stringify(json, null, 2));

            let tweet = json.globalObjects.tweets[status_id];
            let user = json.globalObjects.users[tweet.user_id_str];
            let invalid_chars = {'\\': '＼', '\/': '／', '\|': '｜', '<': '＜', '>': '＞', ':': '：', '*': '＊', '?': '？', '"': '＂', '\u200b': '', '\u200c': '', '\u200d': '', '\u2060': '', '\ufeff': '', '🔞': ''};

            let authorName = user.name.replace(/([\\/|*?:"]|[\u200b-\u200d\u2060\ufeff]|🔞)/g, v => invalid_chars[v]);

            console.log("___authorName : " + authorName);
            return authorName;
        }

        // @return {bitrate, content_type, url, vidNumber, authorName}
        static async getVideoInfo(tweetId, screenName, posterUrl) {
            console.log("___getVideoInfo");
            const url = API.createVideoEndpointUrl(tweetId);

            console.log('cute url  = ' + url);
            const json = await API.apiRequest(url);
            ...
            let authorName = json.data.threaded_conversation_with_injections_v2.instructions[0].entries[0].content.itemContent.tweet_results.result.core.user_results.result.legacy.name;
            console.log("___authorName: " + authorName);
            return {video, tweetId, screenName, vidNumber, authorName};
        }
