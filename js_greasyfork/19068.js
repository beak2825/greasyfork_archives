...
// @downloadURL https://update.greasyfork.org/scripts/19068/WaveSynchro.user.js
// @updateURL https://update.greasyfork.org/scripts/19068/WaveSynchro.meta.js
// ==/UserScript==
...
notRunYet = false;
var defInterval = 35;
var defIntervalBeforeKinder = 860;
var defDecalage = 250;
...
{								// exemple pour      25 ms    50 ms   100 ms   500 ms	1000 ms
...
function addWave () {
	...
	if (needC == '2') textMouvement = 'Assistance';
	
	if ( needC == '3') textMouvement = 'Attaque';

	if (needC == '4') textMouvement = 'Pillage';
...
function getSec(chaine)
{
	...
	if  (detailTime.length != 3 ) return null;
	...
	if (htemp == undefined  || mtemp == undefined || stemp == undefined) return null;
...
function adjustSecInTime(bsecTemp)
{
	...
	if (valTemp == undefined) return null;
	
	while(valTemp > 86400) valTemp -= 86400; //enleve 24h tant que ça fait plus de 24h
	
	while(valTemp < 0) valTemp += 86400; // ajoute 24h tant que ça fait moins de 0
...
function lanceSetTimeout(){
	var newTimeServ = getSec(document.getElementById('tp1').innerHTML) ;
...
function afficheHeures()
{
	...
	if (timerOn) // timer is On when there is one wave at less
	{
		...
		servTimeSec = getSec(document.getElementById('tp1').innerHTML) ;
...
function checkChangeVivi(){
	...
	if (changeViviAfter) bDivForSelectVivi.appendChild(selectViviForChangeAfter);
...
function sendWaves () {
	...
	for( var i=0; i<wCount; i++ ) {
		...
		window.setTimeout(function(x){return function(){ sendTroops(x); }}(i), nextWave);
...
function backupStart () {
	if(notRunYet) {
		...
		if( l4 ) allInOneOpera();
		else window.setTimeout(backupStart, 500);
	}
}
