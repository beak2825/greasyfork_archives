// ==UserScript==
// @name        Freds-FB-unsponsored-Mobile
// @namespace   namespace herpaderp facebook sucks Mobile Version
// @description Removes Facebook Ads On English Only Facebook On Mobile
// @version     1.01.12
...
// @downloadURL https://update.greasyfork.org/scripts/382461/Freds-FB-unsponsored-Mobile.user.js
// @updateURL https://update.greasyfork.org/scripts/382461/Freds-FB-unsponsored-Mobile.meta.js
// ==/UserScript==
 
//The initial code injection point after testing for the window being loaded
window.onload = (event) => {
  console.log('Facebook mobile unsponsored 1.01.12 loaded');
  codestart();
};
	...
	//Loops that we need to make it go through the selectors array
		...
		if (story.querySelector(textRoots[k]) != null){
			...
			if (story.querySelector(textRoots[k]).innerText.trim().substr(sIndex, slength) == searchWords[j]){
				...
				//var elr2 = story.closest('article');
