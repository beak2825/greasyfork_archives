// ==UserScript==
// @name         Ironwood RPG - Guild Market Search
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/503299/Ironwood%20RPG%20-%20Guild%20Market%20Search.user.js
// @updateURL https://update.greasyfork.org/scripts/503299/Ironwood%20RPG%20-%20Guild%20Market%20Search.meta.js
// ==/UserScript==
...
function clearGuildFilter(){
    getListingElems().forEach((elem) => {if (elem.classList.contains('guildListing')) elem.classList.remove('guildListing'); })
...
async function syncFilter() {
  ...
  $('market-listings-component').find('div.sticky').first().find('button.close')[0].click();
}
async function eachListing(){
    let Q = $('market-listings-component').find('div.sticky').first().find('span:contains("Seller")').closest('div')[0];
    if(Q){
        return Q.childNodes[1].textContent.trim();
    } else {
        let P = $('market-listings-component').find('div.sticky').first().find('span:contains("Buyer")').closest('div')[0];
        if(P){
...
function listingsCount(){
    return $('market-listings-component').find('div.card').first().find('div.count:first').text().replace(/\D/g,'').trim();
}
...
function getMembers(){
    ...
    let e = localStorage.getItem('gms extra members'); //get member list from localStorage
    if(g){
        let p;
        if(e){
            let main = {group: 'main', data: JSON.parse(g), color: safeColor()};
            let additional = JSON.parse(e); //extra members
