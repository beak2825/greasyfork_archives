...
// @version        102.1
// @grant          AntiStream (Edited by 4t)
// @description Quick installation prices of goods in the trading hall.
// @downloadURL https://update.greasyfork.org/scripts/370156/Virtonomica%3A%20Quick%20Price.user.js
// @updateURL https://update.greasyfork.org/scripts/370156/Virtonomica%3A%20Quick%20Price.meta.js
// ==/UserScript==
...
debugger;
                        ...
                        let currency_ = ''
                        if (/(nika)|(fast)|(crypto)/.test(window.location.href))
                        {currency_ = '©'}
                        else {currency_ = '$'}
                        // закупочная цена товара
						let itemCost = $tr.children("td:eq(8)").text().trim().replace(" ", "").replace(currency_, "") - 0;
                        // цена продажи товара в среднем по городу
						const cityPrice = $tr.children("td:eq(11)").text().trim().replace(" ", "").replace(currency_, "") - 0;
