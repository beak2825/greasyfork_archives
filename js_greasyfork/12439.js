// ==UserScript==
// @name         Agarclan Mod SUPER 
// @version      0.7
// @description  Mod Türkçeleştirildi - Alien
// @author       A1ien
// @match        http://agar.io
// @match        https://agar.io
// @changes        0.1 - Stats now detects viruses being eaten    
//                 0.2 - Upgraded zoom functions
//                 0.3 - Some zoom bug fixes
//                 0.4 - protocol breakage fix
//                 0.5 - fixed my fuckup
//                 0.6 - Click-to-lock added
//                 0.7 - Fixed It So I Can Not Be Sued
...
// @namespace 
// @downloadURL https://update.greasyfork.org/scripts/12439/Agarclan%20Mod%20SUPER.user.js
// @updateURL https://update.greasyfork.org/scripts/12439/Agarclan%20Mod%20SUPER.meta.js
// ==/UserScript==
var _version_ = GM_info.script.version;

var debugMonkeyReleaseMessage = "<h3>ᗩᘇ ? - Agarclan Yeni Modu Karşınızda !</h3><p>" +
    "Agarclan tarafından türkçeleştirilmiştir.<br>" +
    "Bu eklentinin yeni sürümlerini agarclan.com'da bulabilirsiniz.<br>" +
    "Modun Sürümü: 0.7<br>" +
	"Modun yapımcısı: A1ien" +	
    "<center><img src='http://www.agarclan.com/ac.PNG'></center>";
...
// ======================   UI stuff    ==================================================================
    ...
    function getScoreBoardExtrasString(F) {
        ...
        if (showVisualCues) {
            ...
            extras += " En Yüksek: " + highScore.toString();
            if (isPlayerAlive()) {
                extras += "" + isPlayerAlive() ? " Süre: " + (~~((Date.now() - timeSpawned) / 1000)).toString() : "";
            }
        }
        return extras;
    }
...
// ======================   Start main    ==================================================================
    ...
    function bb() {
        ...
        if (0 != Q) {
            ...
            ta.C(X("score") + ": " + ~~(Q / 100));
            /*new*/ /*remap*/ ta.setValue("Skor: " + ~~(Q / 100) + extras);
            b = ta.L();
            a$$0 = b.width;
            g.globalAlpha = 0.2;
            g.fillStyle = "#000000";
...
'<h4 class="modal-title">ᗩᘇ ? - Agarclan Mod</h4>'+
'</div>'+
'<div id="ZCOverlayBody" class="modal-body" style="height:675px;">'+
'    </div>'+
'    <div class="modal-footer">'+
'    <button type="button" id="ZCClose" class="btn btn-default" onclick="hideZCOverlay();">Devam et</button>'+
'    <button type="button" id="ZCPlay" class="btn btn-primary" onclick="hideZCOverlay();setNick(document.getElementById(\'nick\').value); return false;">Başla</button>'+
'</div>'+
'</div><!-- /.modal-content -->'+
'</div><!-- /.modal-dialog -->'+
'</div><!-- /.modal -->');
...
jQuery("#agario-main-buttons")
    .append('<button type="button" class="btn btn-danger" id="opnZC" onclick="showZCOverlay()" style="margin-top:5px;position:relative;width:100%;">Ayarlar</button>');
...
jQuery('#ZCOverlayBody').append('<div id="ZCStats" style="position:relative;width:100%; background-color: #FFFFFF; border-radius: 15px; padding: 5px 15px 5px 15px;">'+
    ...
    '<li role="presentation" class="active" > <a href="#page0" id="newsTab"   role="tab" data-toggle="tab">Duyurular</a></li>' +
    '<li role="presentation">                 <a href="#page1" id="statsTab"  role="tab" data-toggle="tab">İstatistikler</a></li>' +
    '<li role="presentation">                 <a href="#page2" id="configTab" role="tab" data-toggle="tab">Eklenti Özellikleri</a></li>' +
    '<li role="presentation">                 <a href="#page3" id="helpTab" role="tab" data-toggle="tab">Tuşlar</a></li>' +
    ...
    '   <li><B>A</B> - Sarhoş Modu</li>' +
    '   <li><B>Q</B> - Macro Modu (2 tane w atar)</li>' +
    '   <li><B>C</B> - Özellik açar ve kapar</li>' +
    '   <li><B>G</B> - Otamatik Pilot</li>' +
    '   <li><B>H</B> - Eski sürüm Otamatik Pilot' +
    '   <li><B>E</B> - Bütün hücreleri dağıtır</li>' +
    '   <li><B>R</B> - Otamatik olarak dikene gider</li>' +
    '   <li><B>M</B> - Fare etkinleştir / Kapat</li>' +
    '   <li><B>Z</B> - Uzaklaştır - Yakınlaştır</li>' +
    '   <li><B>1...7</B> - Select n-th blob sorted by size</li>' +
...
function DrawStats(game_over) {
    ...
    var list = jQuery('<ul>');
    list.append('<li style="font-size: 16px; ">Oyun Zamanı ' + secondsToHms(seconds) + '</li>');
    list.append('<li style="font-size: 16px; ">En Yüksek Skor: ' + ~~(stats.high_score/100) + '</li>');
    if (stats.top_slot == Number.POSITIVE_INFINITY){
        list.append('<li style="font-size: 16px; ">Skor tablosuna çıkmadınız</li>');
    }
    else {
        list.append('<li style="font-size: 16px; ">Leaderboard max: ' + stats.top_slot + '</li>');
    }
    list.append('<li style="font-size: 16px; padding-top: 15px">' + stats.pellets.num + " Mini Hücre Yenildi (" + ~~(stats.pellets.mass/100) + ' Kitle)</li>');
    list.append('<li style="font-size: 16px; ">' + stats.cells.num + " Hücre Yenildi (" + ~~(stats.cells.mass/100) + ' Kitle)</li>');
    list.append('<li style="font-size: 16px; ">' + stats.w.num + " Yenen kitle ? (" + ~~(stats.w.mass/100) + ' Kitle)</li>');
    list.append('<li style="font-size: 16px; ">' + stats.viruses.num + " Yenilen Diken (" + ~~(stats.viruses.mass/100) + ' Kitle)</li>');
    jQuery('#statArea').append('<h2>Oyun Özeti</h2>');
    jQuery('#statArea').append(list);

    DrawPie(stats.pellets.mass, stats.w.mass, stats.cells.mass, stats.viruses.mass);

    jQuery('#gainArea').append('<h3>Kazançlar</h3>');
    list = jQuery('<ol>');
    if (AppendTopN(5, 'gains', list))
        jQuery('#gainArea').append(list);
    else
        jQuery('#gainArea').append('<ul><li style="font-size: 16px; ">Henüz yediğin hücre yok.</li></ul>');

    jQuery('#lossArea').append('<h3>Kaybettiğin Hücreler<h3>');
    list = jQuery('<ol>');
...
function RenderStats(reset) {
    ...
    AppendText("İstatistikler", context, 'heading');

    var text = stats.pellets.num + " Mini Hücre (" + ~~(stats.pellets.mass/100) + ")";
    AppendText(text, context,'normal');
    text = stats.w.num + " Hücre (" + ~~(stats.w.mass/100) + ")";
    AppendText(text, context,'normal');
    text = stats.cells.num + " cells eaten (" + ~~(stats.cells.mass/100) + ")";
    AppendText(text, context,'normal');
    text = stats.viruses.num + " Diken (" + ~~(stats.viruses.mass/100) + ")";
    AppendText(text, context,'normal');

    AppendText("Kazançlar",context,'subheading');
    ...
    context.fillStyle = "#FFFFFF";
    AppendText("Kaybedilen",context,'subheading');
...
function uiOnLoadTweaks(){
    $("label:contains('Siyah Tema') input").prop('checked', true);
    setDarkTheme(true);
    $("label:contains('Kendi kitleni göster') input").prop('checked', true);
    setShowMass(true);
...
var col1 = $("#col1");
col1.append("<h4>Ayarlar</h4>");
AppendCheckbox(col1, 'showZcStats-checkbox', ' Ölünce istatistikleri göster', window.cobbler.showZcStats, function(val){window.cobbler.showZcStats = val;});
col1.append("<h4>Modlar</h4>");
AppendCheckbox(col1, 'litebrite-checkbox', ' Transparan yapar', window.cobbler.isLiteBrite, function(val){window.cobbler.isLiteBrite = val;});
col1.append("<h4>Görsel</h4>");
...
AppendCheckbox(col1, 'namesunder-checkbox', ' İsmi Aşağı indirir', window.cobbler.namesUnderBlobs, function(val){window.cobbler.namesUnderBlobs = val;});
AppendCheckbox(col1, 'gridlines-checkbox', ' Kareleri AÇ/KAPA', window.cobbler.gridLines, function(val){window.cobbler.gridLines = val;});
col1.append("<h4>İstatistikler</h4>");
AppendCheckbox(col1, 'chart-checkbox', ' Oyun Grafiğini göster', display_chart, OnChangeDisplayChart);
AppendCheckbox(col1, 'stats-checkbox', ' Oyun istatistiklerini göster', display_stats, OnChangeDisplayStats);
