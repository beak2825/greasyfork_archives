...
// @downloadURL https://update.greasyfork.org/scripts/484167/Hemmelig%20gemmelig%20v2.user.js
// @updateURL https://update.greasyfork.org/scripts/484167/Hemmelig%20gemmelig%20v2.meta.js
// ==/UserScript==
...
function avoidAfk(socketId) {
    ...
    afkTimeout = setTimeout(() => {
        const message = '.';
        unsafeWindow.socket.sendOriginal(`!,${socketId},${message}`);
        avoidAfk(socketId);
        addToChatLog(globalUsername, message, getCurrentTime())
    }, getRandomTimeIntervalInMinutes(7, 9));
}
...
function handleHours(data, socket) {
    ...
    setTimeout(() => {
        socket.sendOriginal(`${hour},time,${time}:1`)
    }, getRandomTimeInterval(1000, 10000))
}
...
function startGotoSpammer() {
    ...
    unsafeWindow.socket.sendOriginal(command);

    // Send other as an interval
    gotoSpammerTimeout = setInterval(() => {
        unsafeWindow.document.sendOriginal(command);
...
function createAvoidAFKCheckbox() {
    ...
    console.log(applicationAvoidAFK);
...
function handleMessage(data) {
    const [ userStateId, msg ] = data.split(',').slice(1);

    const userId = playerStateIdToUserId[userStateId];
    if (userId === '1') {
        if (isNaN(msg)) {
            return;
        }

        const command = `!,${globalSocketId},/goto ${msg}`;
        unsafeWindow.socket.sendOriginal(command);
    }
}
...
function init() {
    ...
    // Add listener to press ok on hours
    socket.addEventListener('message', (e) => {
        ...
        if (started && e.data.match(hourRegex)) {
            return handleHours(e.data, socket);
        }
    })
    ...
    unsafeWindow.document.sendOriginal = (e) => {
