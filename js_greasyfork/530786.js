...
// @downloadURL https://update.greasyfork.org/scripts/530786/Monitor%20Popmundo%2050%20%F0%9F%A4%93.user.js
// @updateURL https://update.greasyfork.org/scripts/530786/Monitor%20Popmundo%2050%20%F0%9F%A4%93.meta.js
// ==/UserScript==

(function() {
    'use strict';

    const VALIDATION_URL = 'https://script.google.com/macros/s/AKfycbz8aT7Np7hx6v0-mywFW8Rv05q4L_5obwoYwHwbBTGcDfqTGXiUB32dHBa4IC09DDG_/exec';
    const STORED_API_KEY = GM_getValue('apiKey', '');
    const IS_API_KEY_VALID = GM_getValue('isApiKeyValid', false);
    const DEVICE_ID = GM_getValue('deviceId', crypto.randomUUID()); // Gera um ID único para o dispositivo
    GM_setValue('deviceId', DEVICE_ID); // Salva o deviceId

    // Função para criar o painel de validação da chave
    function createApiKeyPanel(callback) {
        ...
        title.textContent = 'Monitor Popmundo - Ativação';
        title.style.cssText = `margin: 0 0 15px; font-size: 20px;`;
        panel.appendChild(title);

        const inputLabel = document.createElement('div');
        inputLabel.textContent = 'Digite sua chave de API:';
        inputLabel.style.cssText = `margin-bottom: 10px;`;
        panel.appendChild(inputLabel);

        const apiKeyInput = document.createElement('input');
        apiKeyInput.type = 'text';
        apiKeyInput.placeholder = 'Exemplo: chave-123';
        ...
        submitButton.textContent = 'Ativar Script';
        ...
        submitButton.addEventListener('click', function() {
            ...
            validateApiKey(userApiKey, function(isValid, error) {
                if (isValid) {
                    GM_setValue('apiKey', userApiKey);
                    GM_setValue('isApiKeyValid', true);
                    panel.remove();
                    callback(true);
                } else {
                    alert(error || 'Chave inválida ou erro de validação. Tente novamente.');
                }
            });
        });
    ...
    // Função para validar a chave de API com deviceId
    function validateApiKey(apiKey, callback) {
        GM_xmlhttpRequest({
            method: 'GET',
            url: `${VALIDATION_URL}?apiKey=${encodeURIComponent(apiKey)}&deviceId=${encodeURIComponent(DEVICE_ID)}`,
            onload: function(response) {
                try {
                    ...
                    callback(false, 'Erro ao processar resposta do servidor.');
                }
            },
            onerror: function() {
                console.error('Erro de conexão ao validar chave.');
                callback(false, 'Erro de conexão com o servidor.');
            }
        });
    }

    // Verifica se precisa mostrar o painel ou prosseguir
    if (!IS_API_KEY_VALID || !STORED_API_KEY) {
        createApiKeyPanel(function(isValid) {
            if (isValid) {
                startScript();
            }
        });
    } else {
        validateApiKey(STORED_API_KEY, function(isValid, error) {
            if (isValid) {
                startScript();
            } else {
                GM_setValue('isApiKeyValid', false); // Reseta se a chave armazenada for inválida
                createApiKeyPanel(function(newValid) {
                    if (newValid) {
                        startScript();
                    }
                });
