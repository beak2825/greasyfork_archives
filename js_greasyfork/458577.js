// ==UserScript==
// @name         Advanced Multiboxㄷㄷ2
// @version      1.4
// @namespace    https://tampermonkey.net/
// @description  q for auto respawn
// @author       Shädam(MstJK add some)
// @match        https://diep.io
// @grant        GM_setValue
// @grant        GM_getValue
// @downloadURL https://update.greasyfork.org/scripts/458577/Advanced%20Multibox%E3%84%B7%E3%84%B72.user.js
// @updateURL https://update.greasyfork.org/scripts/458577/Advanced%20Multibox%E3%84%B7%E3%84%B72.meta.js
// ==/UserScript==
(function() {
    var delay = 1000;
    (function loop() {
...
function builde() {
            setTimeout(() => {
             input.execute('game_stats_build 8888888');
            }, 400);
}
builde()
...
let AR = false;
    function autoRespawn() {
        if (AR == true) {
     window.input.execute('game_spawn MstJK#0864^') // edit name
        }
    }
...
setTimeout(() => {
    let gui = () => {
         ctx.fillStyle = "white";
         ctx.lineWidth = 10;
         ctx.font = 5 + "Comic Sans MS";
         ctx.strokeStyle = "black";
        ctx.strokeText(`pushbot by MstJK#0864`, 40, 500);
        ctx.fillText(`pushbot by MstJK#0864/ :`,40, 500);
      ...
      switch(posCount) {
        ...
        case 2: {
          ...
          if(withinMinimap(x, y) == true && d < 15 * scale * getScale()) {
            ...
            playerPos = [xx, yy];
            window._updateous_playros_posos = [playerPos[0]/(r * 0.2), playerPos[1]/(r * 0.2)];
            realMouse = [playerPos[0] + (mouse[0] - canvas.width / 2) / getScale() / FoV / scaling,
                         playerPos[1] + (mouse[1] - canvas.height / 2) / getScale() / FoV / scaling];
          }
          posCount = 0;
  ...
  window.input.execute = new Proxy(window.input.execute, {
    apply: function(to, what, args) {
      args[0] = args[0].trim();
