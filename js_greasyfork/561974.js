...
// @license     MIT
// @downloadURL https://update.greasyfork.org/scripts/561974/Live%20Percentile%20Ranges.user.js
// @updateURL https://update.greasyfork.org/scripts/561974/Live%20Percentile%20Ranges.meta.js
// ==/UserScript==
(function () {
  "use strict";
...
*/
  ...
  const PercentileRanges = (() => {
    ...
    const extendedSummaryStatsConfig = [
      ...
      [
        [/rapier/, /shortsword/, /wakizashi/],
        ["ADB", "Parry", "Str", "Dex", "Agi"],
      ],

      //2h
      [
        [/estoc/, /longsword/, /mace/, /katana/],
        ["ADB", "Str", "Dex", "Agi"],
      ],
      ...
      [
        [/\sshield\s/, /buckler/],
        ["BLK", "Pmit", "Mmit", "Str", "Dex", "End", "Agi", "Crus", "Slas", "Pier"],
      ],
      ...
      [
        [/cotton/, /phase/],
        ["MDB", "Int", "Wis", "Cast Speed", "Mag CD", "Evd", "Agi", "Pmit", "Mmit", "Crus"],
      ],

      //Light
      [
        [/leather/, /shade/],
        ["ADB", "Attack Speed", "Pmit", "Mmit", "Evd", "Res", "Str", "Dex", "End", "Agi", "Int", "Wis", "Crus", "Slas", "Pier"],
      ],
      [[/leather/], ["Burden"]],
      ...
      [
        [/\splate/, /power/],
        ["Str", "Dex", "End", "Pmit", "Mmit", "Phy CC", "Phy CD", "Crus", "Slas", "Pier", "Burden", "Interf"],
      ],
    ].map((line) => ({ nameTests: line[0], statsToShow: line[1] }));

    const weightedValueConfig = [].map((line) => ({
      tag: line[0],
      nameTest: line[1],
      statsWeight: Object.entries(line[2]).map((entry) => ({ name: entry[0], value: entry[1] })),
    }));
    ...
    const statNames = [
      ...
      abbr: statNameArray[0],
      forgeName: statNameArray[1],
      htmlName: statNameArray[2],
      baseMultiplier: statNameArray[3],
      levelScalingFactor: statNameArray[4],
    }));
    ...
    function addEquipLink(link, useQuality) {
      ...
      get(link, (equipDocument) => {
        ...
        if (delayedDone && Object.keys(parsingDone).every((parsingEid) => parsingDone[parsingEid] === true)) run(delayedDone, delayedstatusElm);
      });
    }
    ...
    function getName(equipDocument) {
      ...
      const name =
        nameDiv.children.length === 3 ? nameDiv.children[0].textContent + " " + nameDiv.children[2].textContent : nameDiv.children[0].textContent;
      return name;
    }
    function getEquipInfo(equipDocument, eid, useQuality) {
      ...
      equipInfo.maxForging =
        forgingKeys.length === 0
          ? 0
          : forgingKeys.reduce((maxForgeLevel, forgeObjKey) => Math.max(maxForgeLevel, equipInfo.forging[forgeObjKey].forgeLevel), 0);
      return equipInfo;
    }
    ...
    function getForging(equipDocument) {
      const forging = {};
      [{ textContent: "Physical Damage Lv.0" }, { textContent: "Magical Damage Lv.0" }, ...equipDocument.querySelectorAll("#eu > span")].forEach(
        (span) => {
    ...
    function getEquipStats(equipDocument, equipInfo) {
      ...
      equipDocument.querySelectorAll("div[title]").forEach((div) => {
        ...
        if (spans[0].textContent !== "+")
          //+s currently in a text node here will be turned into spans shortly - this will let us parse the HTML again if the user asks for percentiles for a certain quality
        ...
        if (isHtmlNameFromIW(htmlName, equipInfo)) return; //throws out IW-only stats
        ...
        if (htmlName === "Attack Damage" || htmlName === "Magic Damage") {
          ...
          if (equipInfo.forging[htmlName] || iwLevel)
            equipStatObj.baseUnforgedValue = reverseForgeMultiplierDamage(
              equipStatObj.baseUnforgedValue,
              equipInfo.forging[htmlName],
              equipInfo.pxp0,
              iwCoeff
            );
        } else if (equipInfo.forging[htmlName])
          equipStatObj.baseUnforgedValue = reverseForgeMultiplierNondamage(
            equipStatObj.baseUnforgedValue,
            equipInfo.forging[htmlName],
            equipInfo.pxp0
          );
    ...
    function parseEquip(equipDocument, eid, useQuality) {
      ...
      if (/(Shield\s)|(Buckler)/.test(equipInfo.name)) equipInfo.infoStr += ", " + equipInfo.pabs.map((pab) => pab.substring(0, 3)).join(" ");
      ...
      [
        ...
        if (equipInfo.quality === qualityObj[0]) equipInfo.infoStr += ", " + qualityObj[1] + " ranges: ";
      });
    ...
    function validateRanges(type, slot, quality, htmlNames) {
      ...
      if (equipsByQuality === false) return true; //bad equip types: don't need to ping reasoningtheory again
      if (quality === "Peerless") quality = "Legendary"; //when userscript asks reasoningtheory for Peerless info, reasoningtheory will return Leg info, and Leg info will go into userscript database
      if (!Object.keys(equipsByQuality).includes(quality)) return false;
      if (
        !htmlNames.every((htmlName) => {
        ...
        return false;
      if (updateCheckDays && Date.now() / 1000 - (equipsByQuality[quality].lastCheck || 0) > 3600 * 24 * updateCheckDays) return false;
    ...
    function updateDatabase(done, statusElm) {
      post(
        "https://reasoningtheory.net/gethvitems.php",
        "rangesNeeded=" + JSON.stringify(rangesNeeded),
        (newRangesStr) => {
          let newRanges;
          try {
      ...
      if (!thisMatchingRange) {
        debuglog(
          "Failed to find a range for " +
            ...
            htmlName +
            " in list " +
            JSON.stringify(Object.keys(statArray))
        );
        return false;
      }
    ...
    function getAbsolutePercentile(equipInfo, htmlName, baseValue) {
      ...
      const percentile =
        ...
        "<br>" +
        ((100 * baseValue) / thisMatchingRange.max).toFixed(2) +
        "%</span>";
      return percentile;
    }
    function getFullyForgedBase(equip, equipStat) {
      ...
      if (htmlName === "Attack Damage" || htmlName === "Magic Damage") {
        ...
        forgeLevel = 100;
        if (equipInfo.type === "Phase" && equipInfo.prefix === "Radiant") {
          // Magic Damage Lv.50: Radiant Phase
          forgeLevel = 50;
        }
        if (!equipInfo.slot && equipInfo.suffix === "Slaughter") {
          // Butcher Lv.5: Weapon of Slaugher
          iwLevel = 5;
        }
        if (!equipInfo.slot && equipInfo.suffix === "Destruction") {
          // Archmage Lv.5: Staff of Destruction
          //iwLevel = 5;
        }
      }
    ...
    function equipValid(equip) {
      const equipsByQuality = equip.equipInfo.slot
        ? EquipmentRanges[equip.equipInfo.type][equip.equipInfo.slot]
        : EquipmentRanges[equip.equipInfo.type];
    ...
    function makeSummaryAndPriorities(config, equip) {
      ...
      config.forEach((line) => {
        line.nameTests.forEach((nameTest) => {
          if (!nameTest.test(equip.equipInfo.nameLower)) return;
          line.statsToShow.forEach((abbrevName) => {
            ...
            if (percentile < 0) return; //lower-than-0 percentiles are not displayed in summary, but they're still accessible via "percentiles" object
      ...
      weightedValueConfig.forEach((line) => {
        ...
        const [baseValue, maxValue] = line.statsWeight.reduce(
          (prev, entry) => {
        ...
        weightedValues.push(
          "[" + line.tag + "] " + ((100 * baseValue) / maxValue).toFixed(2) + "% (" + baseValue.toFixed(2) + " / " + maxValue.toFixed(2) + ")"
        );
      });
    ...
    function makeTransformations(equip) {
      ...
      transformations.showUnforgedPercentiles = function () {
        ...
        if (equip.equipStats.plusAndPercentNodes) equip.equipStats.plusAndPercentNodes.forEach((node) => (node.style.display = "none"));
      ...
      transformations.showUnforgedBase = function () {
        ...
        if (equip.equipStats.plusAndPercentNodes) equip.equipStats.plusAndPercentNodes.forEach((node) => (node.style.display = "inline"));
  ...
  function DocumentInteractor() {
    if (!document.querySelector("#showequip")) return; // "No such item"
    if (window.innerHeight < 650)
      //this will only work on windows created via window.open, whose source is almost surely pressing "c" while hovering over an equip
...
.eq span > span {display:inline-block; vertical-align:middle; text-align:center}
  `;
    ...
    const { optionsDiv, select, wikiLink, sendRangeDiv } = (function buildSelect() {
      ...
      ["Legendary", "Magnificent", "Exquisite", "Superior"].forEach(
        (quality) => (select.appendChild(document.createElement("option")).textContent = quality)
      );
    ...
    function addResultsToDocument(resultsParam) {
      ...
      if (type === "Cotton" || type === "Phase") wikiLink.href = "https://ehwiki.org/wiki/Equipment_Ranges_Clothes#" + type + "_" + slot;
      else if (type === "Leather" || type === "Shade") wikiLink.href = "https://ehwiki.org/wiki/Equipment_Ranges_Light#" + type + "_" + slot;
      else if (type === "Plate" || type === "Power") wikiLink.href = "https://ehwiki.org/wiki/Equipment_Ranges_Heavy#" + type + "_" + slot;
      ...
      summaries.weighted.forEach((text) => {
        summaryDiv.append(document.createElement("br"), text);
      });
      ...
      const promptForumLink = (() => {
        const formattedLink = (() => {
          ...
          if (hrefReg) return hrefBase + hrefIsk + "equip/" + hrefReg[1] + "/" + hrefReg[2];
      ...
      select.childNodes.forEach((option) => {
        ...
        } else option.style.backgroundColor = null;
      });
      ...
      if (mobile && !document.querySelector("#mobileButtons")) buildMobileButtons(transformations, promptForumLink);
    ...
    function sendThisPageToHvitems(responseDiv) {
      ...
      const [, eid, key] = /\/equip\//.test(url) ? url.match(/\/equip\/(\d+)\/([0-9a-f]+)/) : url.match(/\.php\?eid=(\d+)&key=([0-9a-f]+)/);
      ...
      r.onload = function () {
        ...
        if (r.response === "1") responseDiv.textContent = "Done!";
        else responseDiv.textContent = 'Error, response was: "' + r.response + '"';
      };
    }
    function buildMobileButtons(transformations, promptForumLink) {
      ...
      buttonConfigs.forEach((buttonConfig, buttonIndex) => {
        if (buttonIndex === 3 || buttonIndex === 6 || buttonIndex === 9) mobileButtons.appendChild(document.createElement("br"));
    ...
    function doCompare(compareDiv, checkboxContainer, compareResultsPacked) {
      ...
      if (!compareResultsUnpacked || !compareResultsUnpacked.infos) {
        if (!compareResultsUnpacked) compareButton.textContent = "Equip parsing failed (obsolete? mistyped link?)";
        else compareButton.textContent = "No data available for this equip type";
        compareResults = null;
        buildGraphic(checkboxContainer);
        return;
      }
    ...
    function buildGraphic(checkboxContainer) {
      ...
      const qualitiesToDisplay = !checkboxContainer
        ...
        : Array.from(checkboxContainer.children).reduce((accum, checkboxDiv) => {
            if (!checkboxDiv.children[0].checked) return accum;
      ...
      if (svgDiv) svgDiv.remove(); //might have already been called by reset(), but sometimes buildGraphic is called without reset()
      else pageJustLoaded = true;
      ...
      const equipTypeStr = `${infos.type}, ${infos.slot ? infos.slot + ", " : ""}${infos.quality} [${infos.prefix ? infos.prefix : ""} | ${
        infos.suffix ? infos.suffix : ""
      }]`;
      addInnerHTML(
        svg,
        "text",
      ...
      htmlNames.forEach((htmlName) => {
        const [graphMin, graphMax] = (() => {
          let min, max;
          Object.keys(rangesByQuality)
            .filter((quality) => qualitiesToDisplay.includes(quality))
            .forEach((quality) => {
              if (!Object.keys(rangesByQuality[quality]).includes(htmlName)) return;
              const thisStatRange = rangesByQuality[quality][htmlName];
              if (!thisStatRange) return;
              if (!min || thisStatRange.min < min) min = Number(thisStatRange.min);
              if (!max || thisStatRange.max > max) max = Number(thisStatRange.max);
            });
          if (!max && compareResults && compareResults.bases.unforged[htmlName]) return [0, compareResults.bases.unforged[htmlName]];
          return [min, max];
        })();
        if (!graphMax) return;
        ...
        if (compareResults && compareResults.bases.unforged[htmlName]) {
          ...
          if (
            higherX - lowerX > 12 &&
            !((lowerX < axisStart && higherX < axisStart) || (lowerX > axisStart + axisWidth && higherX > axisStart + axisWidth))
          ) {
            ...
            addInnerHTML(
              g,
              "rect",
              `x="${startX}" y="${nativeQualityY - 9}" width="${width}" height="10" fill="${origX > compareX ? "red" : "#00ff11"}"/>`
            );
          }
        ...
        function drawStatCircle(unforgedStat, below) {
          ...
          if (!unforgedStat && min === 0) {
            addInnerHTML(
              ...
              `x="${axisStart}" y="${
                y + (below ? 13 : -13)
              }" stroke="#DFEBED" stroke-width="8" paint-order="stroke" fill="black" text-anchor="middle">0 = 0%</text>`
            );
          ...
          addInnerHTML(
            ...
            `x="${visibleX}" y="${
              y + (below ? 13 : -13)
            }" stroke="#DFEBED" stroke-width="8" paint-order="stroke" fill="black" text-anchor="middle">${statStrToDraw}</text>`
          );
    ...
    function addInnerHTML(parent, elmName, continuedString) {
      //adds elements without dereferencing variable/listener assignments for earlier elements in parent - unlike plain .innerHTML +=
    ...
    function restrictToGraphRange(x, axisStart, axisWidth) {
      if (x > axisStart + axisWidth) return axisStart + axisWidth;
      if (x < axisStart) return axisStart;
      return x;
    }
