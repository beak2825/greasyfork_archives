...
// @grant        unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/386236/%E7%9A%87%E5%B8%9D%E6%88%90%E9%95%BF%E8%AE%A1%E5%88%92H5%E7%89%88MOD.user.js
// @updateURL https://update.greasyfork.org/scripts/386236/%E7%9A%87%E5%B8%9D%E6%88%90%E9%95%BF%E8%AE%A1%E5%88%92H5%E7%89%88MOD.meta.js
// ==/UserScript==
...
var mypage=0;
   ...
   function on_feizi(){
       unsafeWindow.GameManager.SceneManager.jianZhuMoudle.getMingFeiRate=function(){return 100};
       alert('赶快选秀试试吧~');
   };
   function on_tongyi(){
    ...
    function on_bingzhongcheat(){

        if($('.bingzhonginput[name="rolename"]').attr('value')!=""){
            var rolename=$('.bingzhonginput[name="rolename"]').attr('value')

        }else{alert('请填写将领名称!');return -1;};
        var check=false;
        var myset=unsafeWindow.GameManager.RoleManager;
        myset.roleTypeDataMap[1].forEach(function(myset){
            if(myset.name==rolename){
                check=true;
                myset=unsafeWindow.GameManager.FightManager.mapModule.getJunTuanByRoleId(myset.id);
                $('.bingzhonginput').each(function(){

                    if($(this).attr('value')!="")
                    {
                        if($(this).attr('name')!="jundui")
                        {
                            myset[$(this).attr('name')]=Number($(this).attr('value'));
                            myset['maxjundui']=Number($(this).attr('value'));
                        }
                        else if($(this).attr('name')!="rolename"){
                            myset[$(this).attr('name')]=Number($(this).attr('value'));
                        }
        ...
        if(check==false){alert('没有找到该将领,请检查将领名称是否正确!');return;};
    };
...
unsafeWindow.GameManager.UIManager.showTipsDialog(locktimeUI);
    ...
    function on_mingchen(){
        unsafeWindow.UILayer.ExamResultUI.prototype.onViewShow=function() {
            var e = unsafeWindow.GameManager.DataManager.mingChenRate;
            unsafeWindow.GameManager.EventSocketListen.lockMap.dataCfg.keJvNum++;
            var t = MathUtils.indexRandom(2)
              , n = 3+MathUtils.indexRandom(3)
              , a = unsafeWindow.GameManager.CardManager.cardToOtherAttrUpdate(CardOtherAttrType.keJuNumAdd)
              , i = 12 + a
              , r = unsafeWindow.GameManager.RoleManager.officerRoleCreater.filterRenCai()
              , o = !1;
            this.roleNum = 0;
            var s = unsafeWindow.GameManager.MapManager.getCoutryById(1)._wenhua;
            if (a = unsafeWindow.GameManager.CardManager.cardToOtherAttrUpdate(CardOtherAttrType.keJuMingChenRate)) {
                i -= 1;
                var u = void 0;
                MathUtils.indexRandom(100) < a[0] && (u = MathUtils.ArrayRandom(unsafeWindow.GameManager.DataManager.mingChenDB.getExamChaoDaiMingChenMap(MathUtils.minAndMaxRandom(1, 10), MING_CHEN_TYPE.EXAM)),
                o = !0),
                this.createNewRole(s, u)
            }
            if (a = unsafeWindow.GameManager.CardManager.cardToOtherAttrUpdate(CardOtherAttrType.kejuAdd)) {
                i -= a;
                for (var g = 0; a > g; g++) {
                    var u = MathUtils.ArrayRandom(unsafeWindow.GameManager.DataManager.mingChenDB.getExamChaoDaiMingChenMap(s, MING_CHEN_TYPE.EXAM));
                    this.createNewRole(s, u),
                    o = !0
                }
            }
            for (var c = 0; 18 > c; c++) {
                MathUtils.indexRandom(1e3) < 1e3 && unsafeWindow.GameManager.DataManager.mingChenDB.getExamChaoDaiMingChenMap(s, MING_CHEN_TYPE.EXAM).length > 0 && n > 0 || !o && 0 == unsafeWindow.GameManager.UserInfoManager.userInfo.KeJuChuxianMingChenHuiHe ? (n--,
                o = !0,
                this.createNewRole(s, MathUtils.ArrayRandom(unsafeWindow.GameManager.DataManager.mingChenDB.getExamChaoDaiMingChenMap(s, MING_CHEN_TYPE.EXAM)))) : r.length > 0 ? this.createNewRole(MathUtils.ArrayRandom(r)) : this.createNewRole(s)
            }
            this.mingchenHuiHeUpdate(o),
            this.zhuangyaun(),
            this.table0.tableGroupToCenter(),
            this.table0.source = this.examList0,
            this.createTable(this.table1),
            this.table1.tableGroupToCenter(),
            this.table1.source = this.examList1
        }
        alert('修改成功,快去科举吧~');
    };
    ...
    function(){
        ...
        $("#panel").append('<div id="box3" class="box" style="display:none;"><br><a class="tab" href="javascript:void(0);" style="padding-left:10px;padding-right:10px;color:black;text-decoration:none;background-color:#96b97d;border-radius:15px;">切换到兵种修改</a><br><br></div>');
        $("#panel").append('<div id="box4" class="box" style="display:none;"><br><a class="tab" href="javascript:void(0);" style="padding-left:10px;padding-right:10px;color:black;text-decoration:none;background-color:#96b97d;border-radius:15px;">切换到属性修改</a><br><br></div>');
//box1里的各种元素
        ...
        $("#box1 div.cheatbtn").append('<br><br><a href="javascript:void(0);" id="mingchen" style="text-align:center;color:white;padding: 5px 15px;background-color:green;border-radius:15px;text-decoration:none;">必得名臣</a>');
        $("#box1 div.cheatbtn").append('<br><br><a href="javascript:void(0);" id="feizi" style="text-align:center;color:white;padding: 5px 15px;background-color:green;border-radius:15px;text-decoration:none;">必得名妃</a>');
...
//box4里的各种元素
        $("#box4").append('<a>填写将领名称以及要修改的属性</a>');
        $("#box4").append('<div class="bingzhonginfo"></div>');
        $("div.bingzhonginfo").append('<div style="color:red;">将领名称:<input class="bingzhonginput" name="rolename" value="" type="text" ></div>');
        $("div.bingzhonginfo").append('<div>兵种:<input class="bingzhonginput" name="bingzhong" value="" type="number" ></div>');
        $("div.bingzhonginfo").append('<div>所在城市:<input class="bingzhonginput" name="chengshi_id" value="" type="number" ></div>');
        $("div.bingzhonginfo").append('<div>归属(0-18):<input class="bingzhonginput" name="guishu" value="" type="number" ></div>');
        $("div.bingzhonginfo").append('<div>军队人数:<input class="bingzhonginput" name="jundui" value="" type="number" ></div>');
        $("div.bingzhonginfo").append('<div>训练值:<input class="bingzhonginput" name="xunlian" value="" type="number" ></div>');
        $("div.bingzhonginfo").append('<div>甲胄:<input class="bingzhonginput" name="jiazhou" value="" type="number" ></div>');
        $("div.bingzhonginfo").append('<div>朴刀:<input class="bingzhonginput" name="pudao" value="" type="number" ></div>');
        $("div.bingzhonginfo").append('<div>战马:<input class="bingzhonginput" name="zhanma" value="" type="number" ></div>');
        $("div.bingzhonginfo").append('<div>弓箭:<input class="bingzhonginput" name="gongjian" value="" type="number" ></div>');
        $("div.bingzhonginfo").append('<div>火炮:<input class="bingzhonginput" name="huopao" value="" type="number" ></div>');
        $("div.bingzhonginfo").append('<div>长枪:<input class="bingzhonginput" name="changqiang" value="" type="number" ></div>');
        $("div.bingzhonginfo").append('<div>重甲:<input class="bingzhonginput" name="zhongjia" value="" type="number" ></div>');
        $("#box4").append('<br><div class="cheatbtn"></div>');
        $("#box4 div.cheatbtn").append('<br><a href="javascript:void(0);" id="changebingzhong" style="text-align:center;color:white;padding: 5px 15px;background-color:green;border-radius:15px;text-decoration:none;">修改兵种</a>');
        ...
        document.getElementById("changebingzhong").onclick =on_bingzhongcheat;
        document.getElementById("danyao").onclick =on_danyao;
        document.getElementById("mingchen").onclick=on_mingchen;
        document.getElementById("feizi").onclick =on_feizi;
