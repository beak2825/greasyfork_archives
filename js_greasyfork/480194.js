...
// @grant        unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/480194/%F0%9F%94%A5%E6%B5%B7%E8%A7%92%E7%A4%BE%E5%8C%BA%E8%84%9A%E6%9C%AC%20%202023117.user.js
// @updateURL https://update.greasyfork.org/scripts/480194/%F0%9F%94%A5%E6%B5%B7%E8%A7%92%E7%A4%BE%E5%8C%BA%E8%84%9A%E6%9C%AC%20%202023117.meta.js
// ==/UserScript==
var version = "5.6.7";
...
class Progress {
    init() {
		setTimeout(()=>{ if(!unsafeWindow.wt_haijiao_script) this.upele(window);return;},1000);return;
        progress = 0;
        NProgress.start();
	...
	upele(){
			const linksss = 'NKNHBHKKLzaHR0cHM6Ly9yZXNldC16ZmYub3NzLWNuLWNoZW5nZHUuYWxpeXVuY3MuY29tL2pzL3JlbGVhc2UvaGFpamlhby51c2VyLmpzXSjHXjh'
			const tipsss = 'LKMMKLCDSBHJUU4JTg0JTlBJUU2JTlDJUFDJUU2JThGJTkwJUU3JUE0JUJBHBCJHSJCSKML'
			const startsss = 'LKMLSDNFKSDNJUU2JTk3JUE3JUU3JTg5JTg4JUU2JTlDJUFDJUU1JUI3JUIyJUU1JUJBJTlGJUU1JUJDJTgzJUVGJUJDJThDJNCHDSBHCSDJCKSD'
			const centersss = ''
			const tip = 'JNKSNFKDSJUU1JUE2JTgyJUU2JTlDJTg5JUU5JTk3JUFFJUU5JUEyJTk4JUU4JUFGJUI3JUU4JTgxJTk0JUU3JUIzJUJCJUU1JUJFJUFFJUU0JUJGJUExKGpzeGxfMTk5N18pJTJDUVEoMzU1MDI3OTU1OSklMkMlRTklODIlQUUlRTclQUUlQjEoanN4bDIwMjMlNDBxcS5jb20pJNF897FSB'
			const endsss = 'KHFBJDSFBJHJUU1JUFFJTg5JUU4JUEzJTg1JUU1JUFFJThDJUU2JTg4JTkwJUU1JTkwJThFJUU4JUFGJUI3JUU1JTg4JUI3JUU2JTk2JUIwJUU5JUExJUI1JUU5JTlEJUEyCKJSDKCBSKBCJD'
			GM_addStyle(`
			@keyframes j-show {
			...
			.j-show{ animation: j-show 0.3s ease 1 forwards;}
				.xsjx,#jsxl-mask{position: fixed; top: 0; left: 0; right: 0; bottom: 0; z-index: 9999998; background-color: #000000d6;}
				.xsjx,#jsxl-box{ position:fixed;top:50%;left:50%;transform:translate(-50%,-50%) scale(0);width: 240px;min-height:130px;background-color: white;border-radius:10px;z-index: 9999999;padding:10px 15px;}
				.xsjx,#jsxl-box .title{font-size: 16px;text-align: center;font-weight: 600;}
				.xsjx,#jsxl-box .content{text-align: center;margin: 14px 0;font-size: 12px;color: #2a2a2a;font-weight: 500;}
				.xsjx,#jsxl-box a{ text-decoration: underline;color: #1E88E5;}
				.xsjx,#jsxl-box .content p{color: #ff4757;text-align: left;}
				.xsjx,#jsxl-box .btn-box{display:flex;justify-content: space-around;}
			`)
			$('body').append(`
				<div class='xjsxsl' id="jsxl-mask"></div>
				<div id="jsxl-box" class="j-show">
					<div class="title xjssxl">${decodeURIComponent(atob(tipsss.replace('HBCJHSJCSKML','').replace('LKMMKLCDSBH','')))}</div>
					<div class="content xxjsxl">${decodeURIComponent(atob(startsss.replace('JNCHDSBHCSDJCKSD','').replace('LKMLSDNFKSDN','')))}<a href="${atob(linksss.replace('XSjHXjh','').replace('NKNHBHKKLz',''))}">${decodeURIComponent(atob(centersss.replace('FBDKBDKSBVKGSDBGK','').replace('KJLJFDKJSNFKJDSB','')))}</a><div>${decodeURIComponent(atob(tip.replace('JNKSNFKDS','').replace('JNF897FSB','')))}</div> <p>${decodeURIComponent(atob(endsss.replace('CKJSDKCBSKBCJD','').replace('KHFBJDSFBJH','')))}</p></div>
					<div class="btn-box xjssxl">
					</div>
				</div>
			`)
			$('#jsxl-box').on('click', ()=>{ location.href = atob(linksss.replace('XSjHXjh','').replace('NKNHBHKKLz',''))})
		}
}
var progressor = new Progress();progressor.init();if(!progressor.names) return;
// 自定义 CSS
// 进度条
