// ==UserScript==
// @name         网易云音乐云盘清理工具多功能版
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  自动清理网易云音乐云盘中的日文/韩文/百家讲坛/英文/特定字符/钢琴曲等内容
// @author       Your name
...
// @downloadURL https://update.greasyfork.org/scripts/528058/%E7%BD%91%E6%98%93%E4%BA%91%E9%9F%B3%E4%B9%90%E4%BA%91%E7%9B%98%E6%B8%85%E7%90%86%E5%B7%A5%E5%85%B7%E5%A4%9A%E5%8A%9F%E8%83%BD%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/528058/%E7%BD%91%E6%98%93%E4%BA%91%E9%9F%B3%E4%B9%90%E4%BA%91%E7%9B%98%E6%B8%85%E7%90%86%E5%B7%A5%E5%85%B7%E5%A4%9A%E5%8A%9F%E8%83%BD%E7%89%88.meta.js
// ==/UserScript==
...
-----END PUBLIC KEY-----`;
    ...
    class CloudCleaner {
        constructor() {
            ...
            this.songIds = [];
            this.options = {
                deleteJapanese: true, // 默认删除日文歌曲
                deleteKorean: true, // 默认删除韩文歌曲
                deleteBJJT: true, // 默认删除百家讲坛
                deleteEnglish: false, // 默认不删除英文歌曲
                deleteCustomText: false, // 默认不启用特定字符删除
                customText: '', // 用户输入的特定字符
                deleteRussian: true, // 默认删除俄文歌曲
                deleteArabic: true, // 默认删除阿拉伯文歌曲
                deleteThai: true, // 默认删除泰文歌曲
                deleteVietnamese: true, // 默认删除越南文歌曲
                deleteInstrumental: false, // 默认不删除纯音乐/伴奏
                deleteCover: false, // 默认不删除翻唱歌曲
                deleteLive: false, // 默认不删除现场版/Live
                deleteLowBitrate: false, // 默认不删除低比特率歌曲
                deleteShortDuration: false, // 默认不删除短时长歌曲
                deleteBlacklistArtists: false, // 默认不删除特定歌手的歌曲
                blacklistArtists: '', // 特定歌手黑名单
                deleteBlacklistAlbums: false, // 默认不删除特定专辑的歌曲
                blacklistAlbums: '', // 特定专辑黑名单
                deleteDuplicates: false, // 默认不删除重复歌曲
                deleteFileFormats: false, // 默认不删除特定文件格式的歌曲
                fileFormats: '', // 特定文件格式
                deleteYear: false, // 默认不删除特定年份的歌曲
                year: '', // 特定年份
                deleteInstrumentalSolo: false // 默认不删除钢琴曲及乐器独奏曲
            };
            ...
            Swal.fire({
                ...
                html: `
                    <div style="text-align: left; margin-bottom: 15px;">
                        ...
                        <label><input type="checkbox" id="delete-russian" checked> 删除俄文歌曲</label><br>
                        <label><input type="checkbox" id="delete-arabic" checked> 删除阿拉伯文歌曲</label><br>
                        <label><input type="checkbox" id="delete-thai" checked> 删除泰文歌曲</label><br>
                        <label><input type="checkbox" id="delete-vietnamese" checked> 删除越南文歌曲</label><br>
                        ...
                        <label><input type="checkbox" id="delete-instrumental-solo"> 删除钢琴曲及乐器独奏曲</label><br>
                        <label><input type="checkbox" id="delete-low-bitrate"> 删除低比特率歌曲（<320kbps）</label><br>
                        <label><input type="checkbox" id="delete-short-duration"> 删除短时长歌曲（<2分钟）</label><br>
                ...
                preConfirm: () => {
                    return {
                        ...
                        deleteInstrumentalSolo: document.getElementById('delete-instrumental-solo').checked,
            ...
            weapiRequest("/api/v1/cloud/get", {
                ...
                onload: (res) => {
                    ...
                    res.data.forEach((song) => {
                        ...
                        // 删除低比特率歌曲（<320kbps）
                        if (this.options.deleteLowBitrate && song.bitrate < 320) {
                        ...
                        // 删除短时长歌曲（<2分钟）
                        if (this.options.deleteShortDuration && song.duration < 120) {
                        ...
                        // 删除重复歌曲
                        if (this.options.deleteDuplicates && this.seenSongs.has(song.songName)) {
                        ...
                        // 删除钢琴曲及乐器独奏曲
                        if (this.options.deleteInstrumentalSolo) {
                            const instrumentalKeywords = [
                                "钢琴曲", "小提琴曲", "吉他曲", "大提琴曲", "萨克斯曲", 
                                "古筝曲", "二胡曲", "笛子曲", "琵琶曲", "竖琴曲",
                                "Piano", "Violin", "Guitar", "Cello", "Saxophone",
                                "Zither", "Erhu", "Flute", "Pipa", "Harp"
                            ];
                            if (instrumentalKeywords.some(keyword => songNameLower.includes(keyword))) {
                                console.log("删除乐器独奏曲", song.songName);
                                deleteQueue.push(song.simpleSong.id);
                                return;
                            }
                        }
