// ==UserScript==
// @name Cкрипт для ГС ГОСС, ЖБ by Anton_Bazalev || Purple (06)
// @namespace http://tampermonkey.net/
// @version 1.3 (Ibra_Sheyh)
// @description Полный cкрипт для ЗГС ГОСС Ibra_Sheyh, ЖБ 06 PURPLE
...
// @license MIT
// @downloadURL
// @downloadURL https://update.greasyfork.org/scripts/552474/C%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%93%D0%A1%20%D0%93%D0%9E%D0%A1%D0%A1%2C%20%D0%96%D0%91%20by%20Anton_Bazalev%20%7C%7C%20Purple%20%2806%29.user.js
// @updateURL https://update.greasyfork.org/scripts/552474/C%D0%BA%D1%80%D0%B8%D0%BF%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%93%D0%A1%20%D0%93%D0%9E%D0%A1%D0%A1%2C%20%D0%96%D0%91%20by%20Anton_Bazalev%20%7C%7C%20Purple%20%2806%29.meta.js
// ==/UserScript==
...
const TotleEnd = "[B][CENTER][COLOR=lavender]С уважением[COLOR=Blue] ЗГС ГОСС - Ibra_Sheyh[/COLOR].<br><br>";
...
},
{
 ...
 "[B][CENTER][COLOR=#CCCCCC][ICODE]{{ greeting }}, уважаемый {{ user.name }} [/ICODE][/COLOR][/CENTER][/B]<br><br>"+
 TotlePhotoTxt1 +
 "[B][CENTER][COLOR=white][FONT=courier new]Спасибо за обращение, Ваша жалоба [COLOR=#00FF00]одобрена[/COLOR],<br><br>"+
 "[B][CENTER][COLOR=white][FONT=courier new]Лидер допустил ошибку, приносим свои извинения.[/COLOR]<br><br>" +
 TotlePhotoTxt1 +
 TotleEnd,
 prefix: ACCСEPT_PREFIX,
 status: false,
...
{
 title: '| К лд будут применены меры |',
 content:
 "[B][CENTER][COLOR=#CCCCCC][ICODE]{{ greeting }}, уважаемый {{ user.name }} [/ICODE][/COLOR][/CENTER][/B]<br><br>"+
 TotlePhotoTxt1 +
 "[B][CENTER][COLOR=white][FONT=courier new]Спасибо за обращение, Ваша жалоба [COLOR=#00FF00]одобрена[/COLOR],<br><br>"+
 "[B][CENTER][COLOR=white][FONT=courier new]В сторону лидера будут применены меры.[/COLOR]<br><br>" +
 TotlePhotoTxt1 +
 TotleEnd,
 prefix: ACCСEPT_PREFIX,
 status: false,
...
{
	  title: '| Правельный вердикт |',
	  content:
		"[B][CENTER][COLOR=#CCCCCC][ICODE]{{ greeting }}, уважаемый {{ user.name }} [/ICODE][/COLOR][/CENTER][/B]<br><br>"+
    	   TotlePhotoTxt1 +
		"[B][CENTER][COLOR=white][FONT=courier new]Ваша жалоба [COLOR=#FF0000]отказана[/COLOR], так как Лидер вынес правильный вердикт. <br><br>"+
              TotlePhotoTxt1 +
			TotleEnd,
     prefix: UNACCСEPT_PREFIX,
	  status: false,
},
{
	  ...
	  content:
		"[B][CENTER][COLOR=#CCCCCC][ICODE]{{ greeting }}, уважаемый {{ user.name }} [/ICODE][/COLOR][/CENTER][/B]<br><br>"+
    	   TotlePhotoTxt1 +
		"[B][CENTER][COLOR=white][FONT=courier new]Ваша жалоба [COLOR=#FF0000]отказана[/COLOR], так как Нарушений со стороны Лидера не обнаружено.<br><br>"+
              TotlePhotoTxt1 +
			TotleEnd,
     prefix: UNACCСEPT_PREFIX,
	  status: false,
