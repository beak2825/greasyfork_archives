...
// @downloadURL https://update.greasyfork.org/scripts/455491/Forum%20Curators%20%7C%20%20for%20Black%20Russia%20.user.js
// @updateURL https://update.greasyfork.org/scripts/455491/Forum%20Curators%20%7C%20%20for%20Black%20Russia%20.meta.js
// ==/UserScript==
...
const buttons = [
    ...
    },
    {
      ...
      content:
        '[Color=rgb(222, 143, 255)][FONT=Georgia][CENTER][I]{{ greeting }}, уважаемый {{ user.mention }}.[/color][/CENTER]<br>' +
        "[CENTER]Вы используете не оригинальные файлы игры (сборку), поэтому ваша жалоба не подлежит рассмотрению.[/CENTER]<br>" +
...
const butt = [
  ...
  $(document).ready(() => {
    ...
    $(`button#greenserver`).click(() => {
      XF.alert(buttMarkup(butt), null, 'Выберите ответ:');
      butt.forEach((btnd, id) => {
        if (id > 0) {
          $(`button#answers-${id}`).click(() => pasteConten(id, threadData, true));
        }
        else {
          $(`button#answers-${id}`).click(() => pasteConten(id, threadData, false));
        }
      });
    });
  ...
  function buttMarkup(butt) {
    return `<div class="select_answer">${buttons
  .map(
  (btnd, i) =>
    `<butt id="answers-${i}" class="butt--primary butt ` +
    `rippleButt" style="margin:5px"><span class="button-text">${btnd.title}</span></butt>`,
  )
  .join('')}</div>`;
  }
  ...
  function pasteConten(id, data = {}, send = false) {
    const template = Handlebars.compile(butt[id].content);
    if ($('.fr-element.fr-view p').text() === '') $('.fr-element.fr-view p').empty();
    ...
    if (send == true) {
      editThreadData(butt[id].prefix, butt[id].status);
      $('.butt--icon.button--icon--reply.rippleButton').trigger('click');
    }
  }
