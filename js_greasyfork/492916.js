...
// @downloadURL https://update.greasyfork.org/scripts/492916/%E5%80%8D%E9%80%9F%E6%92%AD%E6%94%BE%E4%BC%98%E5%8C%96VisJoker.user.js
// @updateURL https://update.greasyfork.org/scripts/492916/%E5%80%8D%E9%80%9F%E6%92%AD%E6%94%BE%E4%BC%98%E5%8C%96VisJoker.meta.js
// ==/UserScript==
/* jshint esversion: 6 */
      ...
      "CSS": `
       ...
       /* 调速圈样式 */
        #speedDiv{
          ...
          overflow:hidden;
          width:30px;/*调速圈长度*/
          padding:5px; border:1px solid #FF0;/*调速圈描边*/
          border-radius:30px;/*调速圈圆角*/
          display:grid;/*项目排列方式 grid平行inline-block垂直*/
          grid-template-columns: 30px 30px 300px 30px;/*调速圈图标间距*/
          grid-template-rows: 30px;/*调速圈高度*/
          grid-column-gap: 5px;/*图标间距*/
          justify-content:flex-start;/*启动方式*/
          align-items:center;/*启动方式 居中*/
          opacity:0.5;/*不透明度*/
        }
        #speedDiv:hover,#speedDiv:focus,#speedDiv:active{
          width:auto;/*调速框展开尺寸*/
          padding-right:5px;/*右侧填充*/
          opacity:1;/*不透明度*/
        }
        #speedDiv span,#speedDiv input,.speedText{
          font-weight: Bold;/*字体粗细normal正常 Bold加粗*/
          background: #FF0;/*背景颜色*/
          display: grid;
        ...
        #speedDiv .speedText{
          height: 30px;/*背景高度*/
          width: 30px;/*背景长度*/
          line-height: 30px;/*字体垂直位置*/
          vertical-align: middle; /*垂直对齐*/
          font-size: 18px;/*字体大小*/
          border-radius: 30px;/*背景圆角*/
          cursor:move;
        }
        ...
        #speedDiv span{
          font-size: 25px;/*调速图标大小*/
          height: 30px;/*调速图标背景高度*/
          width: 30px;/*调速图标背景长度*/
          border-radius:30px;/*调速图标背景圆角*/
          cursor:pointer;
        }
...
(function () {
    ...
    function getPageType() {
        ...
        if (path.startsWith("/video/") || path.startsWith("/watch")) {
            return "video";
        } else if (path.startsWith("/list/") || path.startsWith("/playlist?")) {
            return "list";
        } else if (path.startsWith("/bangumi/play/") || path.startsWith("/watch")) {
            return "bangumi";
    ...
    let hideTimer;
    let playerControlEntity;
    let playerControlWrap;
    let videoPlayer;

    function initializeElementsForBilibili() {
        playerControlEntity = document.querySelector(".bpx-player-control-entity");
        playerControlWrap = document.querySelector(".bpx-player-control-wrap");
        videoPlayer = document.querySelector("video");
    }

    function initializeElementsForYouTube() {
        playerControlEntity = document.querySelector(".ytp-right-controls");
        playerControlWrap = document.querySelector(".ytp-chrome-bottom");
        videoPlayer = document.querySelector("video");
    }

    function showTip(title, time) {
        let h;
        if (playerControlEntity.getAttribute("data-shadow-show") == "true") {
            h = 0;
        } else {
            h = playerControlWrap.clientHeight;
        }
    ...
    class SettingPopup {
        // Your existing code for SettingPopup class goes here
    }

    let setting = new SettingPopup();

    class PlayRateMenu {
        // Your existing code for PlayRateMenu class goes here
    }
    ...
    let _interval = setInterval(function () {
        ...
        if (element) {
            if (pageType === "video" || pageType === "list" || pageType === "bangumi" || pageType === "cheese" || pageType === "festival") {
                initializeElementsForBilibili();
            } else if (pageType === "watch" || pageType === "playlist") {
                initializeElementsForYouTube();
            }

            if (menu.init(element)) {
                menu.render();
                menu.setRate(getRate());
                let playerVideoWrap = document.querySelector(".bpx-player-video-wrap");
                playerVideoWrap.appendChild(tipDisplay);
                playerVideoWrap.appendChild(timeDisplay);

                if (setting.GlobalVolumeAdjustment) {
                    playerVideoWrap.addEventListener("mousewheel", (e) => {
                        e.preventDefault();
