// ==UserScript==
// @name         Microsoft Bing Rewards每日任务脚本KERUA
// @version      V3.1
...
// @namespace    https://greasyfork.org/zh-CN/scripts/517707/
// @downloadURL https://update.greasyfork.org/scripts/517707/Microsoft%20Bing%20Rewards%E6%AF%8F%E6%97%A5%E4%BB%BB%E5%8A%A1%E8%84%9A%E6%9C%ACKERUA.user.js
// @updateURL https://update.greasyfork.org/scripts/517707/Microsoft%20Bing%20Rewards%E6%AF%8F%E6%97%A5%E4%BB%BB%E5%8A%A1%E8%84%9A%E6%9C%ACKERUA.meta.js
// ==/UserScript==

// 每次执行4次搜索后插入暂停时间,解决账号被监控不增加积分的问题
var pause_time = Math.floor(Math.random() * (900000 - 600000 + 1)) + 600000; // 暂停时长建议为16分钟,也就是960000(60000毫秒=1分钟)
var search_words = []; //搜索词
var max_count = 50; //最多搜索次数
var min_count = 40; //最少搜索词素
...
let menu3 = GM_registerMenuCommand('重置搜索次数', function () {
GM_setValue('MaxRewards', Math.floor(Math.random() * (max_count - min_count + 1)) + min_count); // 重新生成随机搜索次数
});
// 自动将字符串中的字符进行替换
...
// 初始化max_rewards
let max_rewards = GM_getValue('MaxRewards');
if (max_rewards === undefined || max_rewards === null) {
    max_rewards = Math.floor(Math.random() * (50 - 40 + 1)) + 40; // 重复执行的次数
...
function restartScript() {
    GM_setValue('Cnt', 0); // 将计数器重置为0
    GM_setValue('MaxRewards', Math.floor(Math.random() * (max_count - min_count + 1)) + 40); // 重新生成随机搜索次数
    location.href = "https://www.bing.com/?br_msg=Please-Wait"; // 跳转到Bing首页
}
