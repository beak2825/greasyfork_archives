...
// @version      20260129
// @description  在微博网页端，为每个帖子创建一个收藏。
// @author       ok406lhq
// @license      MIT
...
// @downloadURL https://update.greasyfork.org/scripts/564400/%E5%BE%AE%E5%8D%9A%E5%B8%96%E5%AD%90%E4%B8%80%E9%94%AE%E6%94%B6%E8%97%8F.user.js
// @updateURL https://update.greasyfork.org/scripts/564400/%E5%BE%AE%E5%8D%9A%E5%B8%96%E5%AD%90%E4%B8%80%E9%94%AE%E6%94%B6%E8%97%8F.meta.js
// ==/UserScript==
...
class ClassName {
    ...
    static buttonBlock = 'button_a656_block';
}
...
class BlogView {
    ...
    static getUserID(articleNode) {
        let userA = articleNode.querySelector(Selector.userASector);
        if (!userA) {
            return null;
        }
        let url = userA.href;
    ...
    static getUsername(articleNode) {
        let userTitle = articleNode.querySelector(Selector.userTitleSector);
        if (!userTitle) {
            return null;
        }
...
class Button {
    ...
    static addBaseClass(node) {
        if (Button.buttonClassList == null) {
            ...
            if (!settingButton) {
                // 如果找不到展开按钮，尝试其他按钮获取样式
                settingButton = document.querySelector('button[class*="woo-like-main"]');
            }
            if (!settingButton) {
                // 仍然找不到，使用默认样式
                console.warn('未找到参考按钮，使用默认样式');
                return;
            }
            Button.buttonClassList = settingButton.classList;
        }
        if (Button.buttonClassList) {
    ...
    static createFavorite(type = 'default', like = false) {
        ...
        buttonNode.addEventListener('click', ev => {
            ...
            if (!articleNode) {
                console.error('无法找到文章节点');
                return;
            }
            let blogID = BlogView.getBlogID(articleNode);
            if (!blogID) {
                console.error('无法获取博客ID');
                return;
            }
    ...
    static createBlock(type = 'default') {
        ...
        buttonNode.classList.add(ClassName.button);
        buttonNode.classList.add(ClassName.buttonBlock);
        Button.addBaseClass(buttonNode);
        ...
        if (Domain.domain === Domain.WEIBO || Domain.domain === Domain.WEIBO_3W) {
            return GM_.get(Const.BLOCK_SELECT_LEVEL).then(tags => {
                ...
                if (Domain.domain === Domain.WEIBO) {
                    ...
                    url = `https://www.weibo.com/ajax/statuses/filterUser`;
                }
    ...
    static remove(ID, username) {
        if (Domain.domain === Domain.WEIBO || Domain.domain === Domain.WEIBO_3W) {
            ...
            if (Domain.domain === Domain.WEIBO) {
                ...
                url = `https://www.weibo.com/ajax/setting/deleteFilteredUser`;
            }
    ...
    setInterval(() => {
        if (onScrollFlag) {
            ...
            for (let [articleNode, config] of blockConfig) {
                ...
                if (isVisible) {
                    if (needUpdateNodeList.indexOf(articleNode) === -1) {
...
// 清理已经不在DOM中的节点
function cleanupBlockConfig() {
    for (let [articleNode, config] of blockConfig) {
        if (!document.body.contains(articleNode)) {
            blockConfig.delete(articleNode);
        }
    }
    // 同时清理needUpdateNodeList
    needUpdateNodeList = needUpdateNodeList.filter(node => document.body.contains(node));
}
...
function updateFavoriteButton3(articleNode) {
    ...
    // 如果按钮不存在，可能被虚拟滚动移除了，需要重新创建
    if (!buttonNode) {
        // 移除标记以便重新创建按钮
        articleNode.removeAttribute('data_a656_value1');
        placeButtons(articleNode);
        buttonNode = articleNode.querySelector(`button[class*="${ClassName.buttonFavorite}"]`);
        if (!buttonNode) {
            return true; // 仍然无法创建按钮，跳过
        }
    }
...
function listenRootBlock() {
    ...
    setInterval(() => {
        ...
        if (currentRootNode !== rootNode) {
            ...
            // 移除旧的标记（如果存在）
            rootNode.removeAttribute('data_a656_is_load_event');
        }
        ...
        if (Domain.domain === Domain.WEIBO || Domain.domain === Domain.WEIBO_3W) {
            ...
            currentObserver = new MutationObserver((mutationsLi) => {
            ...
            currentObserver.observe(rootNode, {childList: true, subtree: true});
            onScrollFlag = true;
...
function placeButtons(node) {
    ...
    // 关键修复：检查按钮是否真的存在，而不仅仅依赖标记
    let existingFavButton = node.querySelector(`button[class*="${ClassName.buttonFavorite}"]`);
    let existingOpenButton = node.querySelector(`button[class*="${ClassName.buttonOpenNewTab}"]`);
    let existingBlockButton = node.querySelector(`button[class*="${ClassName.buttonBlock}"]`);

    // 如果所有按钮都存在，则不需要重新创建
    if (existingFavButton && existingOpenButton) {
        // 对于 weibo.com，还需要检查屏蔽按钮
        if (Domain.domain === Domain.WEIBO) {
    ...
    // 移除旧标记（节点可能被复用）
    node.removeAttribute('data_a656_value1');
    ...
    if (!existingFavButton) {
        ...
        if (Domain.domain === Domain.WEIBO || Domain.domain === Domain.WEIBO_3W) {
            ...
            let wrap = node.querySelector('.button_a656_wrap') || createWrapForS(targetNode);
    ...
    if (Domain.domain === Domain.WEIBO && !existingBlockButton) {
    ...
    if (forwardNode != null) {
        let existingForwardFav = forwardNode.querySelector(`button[class*="${ClassName.buttonFavorite}"]`);
        let existingForwardOpen = forwardNode.querySelector(`button[class*="${ClassName.buttonOpenNewTab}"]`);

        if (!existingForwardFav || !existingForwardOpen) {
            let forwardTargetNode = forwardNode.querySelector('a');
            if (forwardTargetNode && forwardTargetNode.parentNode) {
                let targetParent = forwardTargetNode.parentNode;

                if (!existingForwardFav) {
                    let favoriteButtonNode = Button.createFavorite('forward');
                    favoriteButtonNode.style.marginRight = '26px';
                    if (Domain.domain === Domain.WEIBO || Domain.domain === Domain.WEIBO_3W) {
                        targetParent.appendChild(favoriteButtonNode);
                    }
                }

                if (!existingForwardOpen) {
                    let openButton = Button.createOpen('forward');
                    openButton.style.marginLeft = 'auto';
                    if (Domain.domain === Domain.WEIBO || Domain.domain === Domain.WEIBO_3W) {
                        targetParent.appendChild(openButton);
                    }
                }

                let existingForwardBlock = forwardNode.querySelector(`button[class*="${ClassName.buttonBlock}"]`);
                if (!existingForwardBlock && Domain.domain === Domain.WEIBO) {
                    let blockButton = Button.createBlock('forward');
                    targetParent.appendChild(blockButton);
                }
            }
        }
    }
...
// 为 s.weibo.com 创建按钮容器
function createWrapForS(targetNode) {
    let wrap = document.createElement('div');
    wrap.className = 'button_a656_wrap';
