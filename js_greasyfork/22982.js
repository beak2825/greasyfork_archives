// ==UserScript==
// @name          Jacob's WME Tools - modified
// @version       0.1.9.2tm
...
// @grant         none
// @author        JJohnston84
// @namespace https://greasyfork.org/users/25536
// @downloadURL https://update.greasyfork.org/scripts/22982/Jacob%27s%20WME%20Tools%20-%20modified.user.js
// @updateURL https://update.greasyfork.org/scripts/22982/Jacob%27s%20WME%20Tools%20-%20modified.meta.js
// ==/UserScript==
...
(function () {   
  ...
  function jjwme_convertUnknownToTwoWay() {
    ...
    for (var segIndex in segmentsToModify) {
      // Check that we don't have too many pending changes.
        ...
        console.log("Pending Changes " + jjwme_pendingChangeCount() );
      ...
      var segment = segmentsToModify[segIndex];
      console.log("Segment Number " + segIndex + " is updated by " + segment.updatedBy);
      if (segment.getDirection() === 0 && jjwme_onScreen(segment) && segment.isGeometryEditable()) {
  ...
  function jjwme_convertOneWayToTwoWay() {
    ...
    for (var segIndex in segmentsToModify) {
      ...
      if (jjwme_pendingChangeCount() >= jjwme_MAX_BASE_EDITS) {
        ...
        console.log("Made it through segment checks for 1>2 segments");
        
      var segment = segmentsToModify[segIndex];
      if (segment.isOneWay() && jjwme_onScreen(segment) && segment.isGeometryEditable()) {
        W.model.actionManager.add(new UpdateObject(segment, { fwdDirection: true, revDirection: true }));
  ...
  function jjwme_convertAllToTwoWay() {
    ...
    for (var segIndex in segmentsToModify) {
      ...
      var segment = segmentsToModify[segIndex];
      if (segment.lockRank === 0 && (segment.isOneWay() || segment.getDirection() === 0) && jjwme_onScreen(segment) && segment.isGeometryEditable()) {
        W.model.actionManager.add(new UpdateObject(segment, { fwdDirection: true, revDirection: true }));
  ...
  function jjwme_clearUndo() {
    while (W.model.actionManager.canUndo()) {
      ...
      console.log(jjwme_pendingChangeCount());
    }
    console.log("Final count " + W.model.actionManager.actions.length);
  } 
