// ==UserScript==
// @name         Moonbit.co.in (Pescador de Cripto)
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/415317/Moonbitcoin%20%28Pescador%20de%20Cripto%29.user.js
// @updateURL https://update.greasyfork.org/scripts/415317/Moonbitcoin%20%28Pescador%20de%20Cripto%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    function checkRoll(){
        if($('[data-bind="text: claimAmount().toFixed(claimAmount() >= 0.001 ? 4 : 8)"]').text().indexOf('0.00000000') > -1){
            setTimeout(function(){
                if($('[data-bind="text: claimAmount().toFixed(claimAmount() >= 0.001 ? 4 : 8)"]').text().indexOf('0.00000000') < 0){
                    window.history.go(0);
                }
            },2000);
        }
    ...
    function roll(){
        ...
        if($('.close').eq(1).is(':visible')){
            message = "MoonBit.co.in\n- Sucesso: Recebeu " + $('[data-bind="text: claimedAmount().toFixed(8)"]').eq(0).text() + " BTC\n- CoinPot balance: " + $('[data-bind="text: balance().toFixed(8)"]').eq(1).text() + " BTC";
            ntb(message);
    ...
    function open(){
        if (window.location.href.indexOf("pescadordecripto.com/dashboard") > -1) {
            var win_moonbitcoin = window.open("https://moonbit.co.in/faucet", "MoonBitcoin","width=10,height=10,left=-3000,top=-3000");
        }
    ...
    $(document).ready(function(){
        setTimeout(function(){
            if (window.location.href.indexOf("moonbit.co.in/faucet") > -1) {
                moonBtc();
            }
        },10000);
    });
    if (window.location.href.indexOf("pescadordecripto.com/install") > -1) {
        document.getElementById('moonbit-co-in').classList.add("faucet-active");
    }
})();