...
// @match        https://*.humanatic.com/pages/humfun/login.cfm?invalid=ip&CFID=*&CFTOKEN=*
// @match        https://*.humanatic.com/pages/humfun/login.cfm
// @match        http://*.humanatic.com/pages/humfun/review/?category=*
// @match        http://*.humanatic.com/pages/humfun/login.cfm?invalid=ip&CFID=*&CFTOKEN=*
// @match        http://*.humanatic.com/pages/humfun/login.cfm
// @grant        none
// @run-at       document-start
// @connect      https://joseeayalav.000webhostapp.com
// @require      https://cdnjs.cloudflare.com/ajax/libs/crypto-js/3.1.9-1/crypto-js.min.js
// @downloadURL https://update.greasyfork.org/scripts/373287/HumFun%20Tools.user.js
// @updateURL https://update.greasyfork.org/scripts/373287/HumFun%20Tools.meta.js
// ==/UserScript==
console.log('Listo');
...
if(location.href.indexOf('login.cfm') !== -1)
{sessionStorage.removeItem('um');sessionStorage.removeItem('umc');}
prepare();
function prepare(){
        ...
        if(typeof $._data(document,"events").ajaxSend === 'undefined'){
            $(document).ajaxSend(function( event, request, settings ) {
                ...
                if( settings.url === "cfc/call.cfc?method=getCall") {
                    ...
                    request.abort();
                    normallyAjax = settings;
                }
                //console.log(settings.url + ":::::::::::::::::::",event,request,settings);
                //if( settings.url === "reviewer_module.cfm"){}
            });
        }
        if(typeof $._data(document,"events").ajaxError === 'undefined'){
            $(document).ajaxError(function( event, request, settings, err ) {
                settings.nerr = settings.nerr ? (settings.nerr+1) : 1;
                if(settings.nerr<3) {
                    console.log('retry '+settings.nerr);
                    $.ajax(settings);
                }
            });
        }
...
function check(){
    ...
    if(localStorage.getItem('dataTool') !== null){
        var data = JSON.parse(localStorage.getItem('dataTool'));
        if(data.date != new Date().getDate()){
            localStorage.removeItem('dataTool');
            if(sessionStorage['um'] !== undefined) checkNew(data.lic);
            else bg(checkNew.bind(null,data.lic));
        } else {
            bg(checkReady);
        }
    } else {
        if(sessionStorage['um'] !== undefined) checkNew();
        else bg(checkNew);
    }
}
...
function bg(cb){
    $.ajax({
        url: atob('aHR0cHM6Ly93d3cuaHVtYW5hdGljLmNvbS9wYWdlcy9odW1mdW4vcHJvZmlsZS9jZmMvcHJvZmlsZV9zZXR0aW5ncy5jZmM/bWV0aG9kPXB1bGxVc2VyRGF0YQ=='),
        cb: cb,
        dataType: 'json',
        success: function(res){
            var um = res[atob('VVNFUk5BTUU=')].substr(0,24).toLowerCase();
            sessionStorage['umc'] = res[atob('VVNFUk5BTUU=')].toLowerCase();
            um = um.length < 24 ? (um+"0".repeat(24 - um.length)) : um;
            sessionStorage['um'] = um;
            if(this.cb !== undefined) cb();
        },
        error: function(){setTimout(bg.bind(null,this.cb),1000);}
    });
}

function checkNew(lik){
    var um=sessionStorage['um'];
    var umc=sessionStorage['umc'];
    var lic = lik ? lik : '';
    while(lic === ''){
        lic = prompt(atob('SW5ncmVzZSBsYSBsaWNlbmNpYSBwcm9wb3JjaW9uYWRhLCAoUHJlY2lvbmUgQ2FuY2VsYXIgcGFyYSBtb2RvIG5vcm1hbCk='));
    }
    if(lic === null) {
        console.log('normally');
        $.ajax(normallyAjax); return false;
    }
    var key = um+lic;
    $.ajax({
        url: atob('aHR0cHM6Ly9qb3NlZWF5YWxhdi4wMDB3ZWJob3N0YXBwLmNvbS9hZG1pbmhtL2RhdGEucGhw'),
        type: 'post',
        dataType: 'json',
        key: key,
        lic: lic,
        data: {um: btoa(umc), uid: uid},
        success: function(res){
            if(res.status === 200){
                var key = CryptoJS.enc.Utf8.parse(this.key);
                var iv = CryptoJS.enc.Base64.parse(res.iv);
                var ct = res.data;
                var pt = null;
                ...
                if(pt === null) {window._alert(atob('TGljZW5jaWEgaW5jb3JyZWN0YSwgbyB1c3VhcmlvIG5vIHBlcm1pdGlkbw=='));setTimeout(checkNew,500);return false;}
                localStorage.setItem('dataTool',JSON.stringify({iv: res.iv, lic: this.lic, ct: ct, date: new Date().getDate()}));
...
function checkReady(){
    ...
    if(pt !== null){
        ...
        window._alert(atob('TGljZW5jaWEgaW5jb3JyZWN0YSwgbyB1c3VhcmlvIG5vIHBlcm1pdGlkbw=='));
        localStorage.removeItem('dataTool');
    }
}
...
function ready(){
    ...
    sc2.src = 'https://cdn.bootcss.com/wavesurfer.js/2.0.5/plugin/wavesurfer.timeline.min.js';
                ...
                success: function(response) {
                    ...
                    if(response.status == "success") {
                        if(response.review.duration_seconds > this.minTime && this.minTime !== undefined) {
                            let tiempoEspera = localStorage['tiempoEspera'] || false;
                            $('.processing-overlay').show();
                            hideOptions();
                            if(tiempoEspera){
                                updateLoaderStatus("Esperando " + tiempoEspera + " Segundos...");
                                setTimeout((function(minTime,duration){
                                    updateLoaderStatus("Omitiendo llamada de " + duration + " Segundos...");
                                    betaSkipCall(true, getCall.bind(null,minTime));
                                }).bind(null,this.minTime,response.review.duration_seconds),tiempoEspera * 1000);
...
`;
    ...
    document.body.onkeydown = function(e) {
        ...
        if(e.key === 'ArrowUp' && e.ctrlKey) { audioZoom = typeof(audioZoom) === 'undefined' ? 10 : ( audioZoom+10 ) ; wavesurfer.zoom(audioZoom); }
        if(e.key === 'ArrowDown' && e.ctrlKey) { audioZoom = typeof(audioZoom) === 'undefined' ? 0 : ( (audioZoom-10) < 0  ? 0 : (audioZoom-10) ) ; wavesurfer.zoom(audioZoom)}
    };
    ...
    addContextMenu('#omitir',[
        ...
        {
            name: function(){return 'Set WaitTime' + ( localStorage['tiempoEspera'] === undefined ? '':(', Currenly ['+localStorage['tiempoEspera']+']') );},
            callback: function(){
                let vWait;
                do {
                     vWait = prompt('Tiempo a esperar, entre cada llamada, al omitir (segundos) ');
                } while(isNaN(vWait) );
                if(vWait !== 0) localStorage['tiempoEspera'] = vWait;
                else localStorage.removeItem('tiempoEspera');
            }
        }
    ]);
