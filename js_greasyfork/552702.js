// ==UserScript==
// @name              大人的Greasyfork
// @version           0.0.1
// @author            youzi
// @description       脚本会在本网站页面顶部创建大人版本的Greasyfork(Sleazyfork)与Greasyfork快捷跳转按钮
// @icon              data:image/png;base64,
// @namespace         大人
// @include           */greasyfork.*/*
// @include           */sleazyfork.*/*
// @require           https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js
// @run-at 			  document-start
// @grant             unsafeWindow
// @grant             GM_addStyle
// @license           MIT
// @charset		      UTF-8
// @downloadURL https://update.greasyfork.org/scripts/552702/%E5%A4%A7%E4%BA%BA%E7%9A%84Greasyfork.user.js
// @updateURL https://update.greasyfork.org/scripts/552702/%E5%A4%A7%E4%BA%BA%E7%9A%84Greasyfork.meta.js
// ==/UserScript==

const fd = function(t){
	const b = window.document;
	let c;
	let ti = 0;
	let s;
	return new Promise((resolve, reject) => {
		function fe(tp) {
			if (!s) {
				s = tp;
			}
			const et = tp - s;
			if (et >= 500) {
				c = b.querySelector(t)
				if (c) {
					resolve(c)
				} else if (++ti === 15) {
					reject()
				} else {
					s = tp
				}
			}
			if (!c && ti < 15) {
				requestAnimationFrame(fe);
			}
		}
		requestAnimationFrame(fe);
	});
}

GM_addStyle(`
	@keyframes annn {
		0% {
			transform: scale(1);
		}
		50% {
			transform: scale(1.1);
		}
		100% {
			transform: scale(1);
		}
	}
	.mybtn-anima{
		animation: annn 2s infinite ease;
	}
`)

fd('#main-header').then(res =>{
	$(res).after(`
		<div class='mydiy-btn mybtn-anima' style="width: 80%;margin: 0 auto;background-color: #e3473c;text-align: center;font-size: 25px;color: white;padding: 10px 0;margin-top: 15px;border-radius: 5px;cursor: pointer;">跳转到${location.origin.includes('greasyfork')?'大人Sleazyfork': 'Greasyfork'}</div>
	`)
	$('.mydiy-btn').on("click", () => {
		location.href = location.origin.includes('greasyfork')?'https://sleazyfork.org/scripts': 'https://greasyfork.org/scripts'
	})
})

if(location.pathname.endsWith('/scripts')){
	fd('#browse-script-list li').then(res =>{
		const lis = document.querySelectorAll('#browse-script-list li');
		lis.forEach(item =>{
			if(item.getAttribute('data-script-id').includes('42833') || item.getAttribute('data-script-id').includes('42640')){
				item.remove();
			}
		})
	})
}