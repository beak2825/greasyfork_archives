...
// @downloadURL https://update.greasyfork.org/scripts/498128/%E7%8F%8D%E8%97%8F.user.js
// @updateURL https://update.greasyfork.org/scripts/498128/%E7%8F%8D%E8%97%8F.meta.js
// ==/UserScript==
...
function config() {
    ...
    if (window === top) {
        settings.NoAD解析 = {
            "麒麟": "https://t1.qlplayer.cyou/player/?url=",
        ...
        settings.AD解析 = {
            ...
            "BL": "https://svip.bljiex.cc/?v=",
            "B站": "https://jx.jsonplayer.com/player/?url=",
...
// 核心驱动代码
(function () {
    if (location.search.match(new RegExp(/[?&]url=https?:\/\/.+\..+\..+\/.+/i)) ||
        location.search.match(new RegExp(/[?&]v=https?:\/\/.+\..+\..+\/.+/i)) ||
        location.href.match(new RegExp(/https?:\/\/api\.leduotv\.com\/.+?vid=/i))) {
        settings.isParse = true;
    }
    if (settings.isParse || location.host.indexOf(new RegExp(/(v.qq.com|iqiyi.com|youku.com|bilibili.com|miguvideo.com|le.com|tv.sohu.com|film.sohu.com|mgtv.com|ixigua.com|pptv.com|vip.1905.com|www.wasu.cn)/) === -1)) {
        // 对符合条件的域名执行脚本
        // 调用自写加密算法，生成实例类
        ...
        if (window === top) {
            settings.parseDB = {
                解析开关: "\x01", 自动全屏: "\x01", 弹幕开关: "\x01", DIY解析栏: "\x01", 自动解析: "\x01"
            };
    ...
    if (window === top) {
        ...
        if (!isMobile) {
            if (location.host.indexOf("v.qq.com") !== -1) {
                ...
                readyPlayerBox("爱奇艺", ["#playerPopup", "#modal-vip-cashier-scope", "div[class^=qy-header-login-pop]", "div[element.style]", "div[class^=covers_cloudCover__Dd-AN ]"], [],
                ...
                doElement("div.bpx-player-video-area,svg[aria-hidden=true],div.list-wrapper.simple>ul.clearfix", function () {
                    return readyPlayerBox("哔哩哔哩", ["div.login-panel-popover,div.vip-panel-popover", "div.login-tip"], [],
            ...
            forElements("video", async function (videos, thisInterval) {
                for (const video of videos) {
                    ...
                    if (video.src && video.duration > 7) {
                        ...
                        top.postMessage(key.ENCRYPT("宝塔镇河妖\x00给予\x001", settings.k1, settings.k2), "*");
...
function showTip(msg, style = "") {
    ...
    if (window === top) {
        ...
        tip.style = style + "pointer-events: none; opacity: 0; background-color: #222a; color: #fff; font-family: 微软雅黑,黑体,Droid Serif,Arial,sans-serif; font-size: 17px; text-align: center; padding: 6px; border-radius: 16px; position: fixed; transform: translate(-50%, -50%); left: 50%; bottom: 15%; z-index: 2147483647;";
