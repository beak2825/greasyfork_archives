...
// @downloadURL https://update.greasyfork.org/scripts/390265/bro3_beyond2.user.js
// @updateURL https://update.greasyfork.org/scripts/390265/bro3_beyond2.meta.js
// ==/UserScript==
// version	date
// 0.01		2016/05/**	試験的に着手
					...
					.done(function(res) {
						...
						for (var i = 0; i < area.length; i++) {
							...
							//	match = q$(area[i]).attr("onmouseover").match(/rewriteAddRemoving\('\d+','.*', '(.*)', '.*', '(.*)', '(.*)', '[★]+', '.*', '(\d+)', '(\d+)', '(\d+)', '(\d+)', '', '(\d+)', \w+\); over.*$/);
                                match = q$(area[i]).attr("onmouseover").match(/rewriteAddRemoving\('\d+','(.*)', '(.*)', '(.*)', '(.*)', '(.*)', '(.*)', '(.*)', '(.*)', '(.*)', '(.*)', '(.*)', '', '(.*)', \w+\); over.*$/);
 							} else {
...
// 								}
                                ...
                                var _num = i+1;
                                if ( _num < 10) {
                                    var _ret = ( '00' + _num ).slice( -2 );
                                 } else{
                                     var _ret = String(_num);
                                 }

                                var img2 = q$("#mapsAll .mapAll"+_ret, resp);
                                var imgc = q$(img2[0] ).attr("src");

                                var capital_flag=''
                                try{
                                    if ( imgc.indexOf('capital_') != -1) {
                                        capital_flag='C'
                                    }
                                } catch(e) {
                                    capital_flag=''
                                }
                                list.push(parseInt(pos[1]) + "," + parseInt(pos[2]) + ",\"" + match[1] + "\"," + match[2] + "," + match[3] + ",\"" + match[4] +  "\"," + match[5] + "," + match[6] + "," + match[8] + "," + match[9]+ "," + match[10]+ "," + match[11]+ "," + match[12] + "," + capital_flag );
								listcnt ++;
