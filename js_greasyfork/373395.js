...
// @downloadURL https://update.greasyfork.org/scripts/373395/TEST%20Triton.user.js
// @updateURL https://update.greasyfork.org/scripts/373395/TEST%20Triton.meta.js
// ==/UserScript==
...
//waitForElementToDisplay("[aria-label='Show hidden characters']", 500);
...
var matchChecks = [
    ...
    [/{\/\d+}[^\s,!\?:;！\.？；。\-'\|।؟]+/, "Missing space after placeholder", null, 1, /Thai|Chinese|Korean|Japanese/],
    [/[^\s\*,!\?:;！\.？；。\-'\|।؟]+\{\d+}/, "Missing space before placeholder", null, 1, /Thai|Chinese|Korean|Japanese/],
    ...
    [/[^\S\n]+[!\?:;！\.？；。\|।؟]/g, "Space(s) before punctuation", 1],
...
// INIT - load content
var messages = document.querySelectorAll('div[data-tu-seq-no]');
...
for (var i = 0; i < messages.length; i++) {
    ...
    var sourcequeries = messages[i].querySelectorAll('[lang=en] > ul > li'); // Seems relatively safe for en > XY direction
    ...
    TritonObject.content.push({ "segmentID": segmentName, "source": APIsource, "target": APItarget, "intentFlags": [] });
...
function checkSegment(source, text, html, i) {
    ...
    for (var t = 0; t < queriesArray.length; t++) {
        ...
        if (!query.replace(/\{\d+\}.*?(?=\{\/\d+\}){\/\d+\}|\s/g, "").length && query.length) {
            // replace content wrapped in placeholders and whitespace. If the result is empty, it's annotation-only. Skip empty annotaitons
        ...
        // Untranslated queries
        var queryPattern = new RegExp(escapeRegExp(queryPlainText.trim()), "gi");
...
function capitalizeFirstLetter(string) {
    ...
    for (var x = 0; x < stringArray.length; x++) {
        ...
        if (!originalString.match(/([^\u0000-\u007F¿]|[a-zA-Z])/)) continue;
