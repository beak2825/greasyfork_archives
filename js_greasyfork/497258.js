...
// @downloadURL https://update.greasyfork.org/scripts/497258/%E6%90%9C%E7%B4%A2%E8%A7%86%E9%A2%91%E6%B5%8B%E8%AF%95.user.js
// @updateURL https://update.greasyfork.org/scripts/497258/%E6%90%9C%E7%B4%A2%E8%A7%86%E9%A2%91%E6%B5%8B%E8%AF%95.meta.js
// ==/UserScript==
...
(function() {
    ...
    if(rex.match(/https:\/\/www.douyin.com\/search\/*/) != null){
        ...
        // let currentCount = 0
        const getNinety = (time,callBack)=>{
            // 获取当前时间戳
            let currentTimeStamp = Date.now();
            // 获取90天前的时间戳
            let ninetyDaysAgo = currentTimeStamp - (90 * 24 * 60 * 60);
            // 只抓小于九十天的视频
            if (time > ninetyDaysAgo) {
                console.log("时间戳大于90天");
            } else {
                callBack()
            }
        }
        function getDyData(keyword,search_keyword_id){
            ...
            let dataVideo = {
                ...
                enable_history: 1,
                keyword: keyword,
                filter_selected: {sort_type:"2",publish_time:"180"},
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if(res.data == undefined){
                        ...
                        // 使用reduce()和findIndex()去除重复数据
                       const uniqueArr = [...new Set(res.data.filter(item => item.type == 1).map(item => item.aweme_info.author.sec_uid))].map(id => res.data.find(item => item.aweme_info.author.sec_uid == id));
                        console.log(uniqueArr,'uniqueArr')
                       uniqueArr.forEach((item,index)=>{
                            getNinety(item?.aweme_info?.create_time,()=>{
                                if(linkArr?.[index]?.aweme_info?.author?.sec_uid != item.aweme_info.author.sec_uid){
                                    linkArr.push(item)
                                }
                            })
                        })
                        ...
                        if(res.has_more == 1 && linkArr.length <maxNum){
                            ...
                            let linkItem = linkArr.slice(0,maxNum)
                             const newArr = linkItem.map(e=>({outside_video_id:e.aweme_info.aweme_id,sec_uid:e.aweme_info.author.sec_uid}))
                            let urlData = {
                                search_keyword_id,
                                keyword: keyword,
                                platform_id: 1,
                            ...
                            req(apiHost+"spider/browser/saveSearchKeywordResult",urlData,function(res){
                                ...
                                search_id = null
                                linkArr = []
                                console.log(`总共${linkItem.length}条数据信息抓取完毕`)
                                //同步每条视频信息的数据
                                openLink(linkItem)
                            },function(res){
                                console.log(`数据信息抓取失败,${res}`)
                                alert("抓取失败")
                                $(".cyloading").hide()
                                dyoffset = 0
                                need_filter_settings = 1
                            ...
                            function openLink(linkList,linkKey=0){
                                // 定义每次处理的数据量
                              let index = 0;
                                function processData() {
                                    // 取出下一批数据进行处理
                                    let batch = linkList.slice(index, index + 10);
                                    console.log(`${index}~${index+10}条数据抓取完毕`)
                                    index += 10;
                                    // 如果还有数据需要处理
                                    if (index < linkList.length) {
                                        ...
                                        for (let i = 0; i <= batch.length; i++) {
                                            setTimeout(function(){
                                                window.open("https://www.douyin.com/user/"+linkList[i].aweme_info.author.sec_uid+"?type=keyword")
                                            },300)
                                        }
                                        ...
                                        // 等待一分钟后继续处理下一批数据
                                     //   setTimeout(processData, 60000); // 60000毫秒等于一分钟
                                    } else {
                                        console.log(`==============${keyword}关键词抓取完毕==============`)
                                        // 数据处理结束
    ...
    }else if(rex.match(/https:\/\/www.kuaishou.com\/search\/video*/) != null){
        ...
        function getDyData(keyword){
            ...
            let dataVideo = {
                ...
                "variables": {"keyword": keyword,"page": "search","pcursor": pcursor,"searchSessionId":searchSessionId}
            }
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    pcursor = res.data.visionSearchPhoto.pcursor

                    if(res.data.visionSearchPhoto.feeds == undefined){
                        repeatNum++
                        setTimeout(function(){
                            if(repeatNum <= 10){
                                console.log("接口返回失败，重跑接口")
                        ...
                        for(let i in res.data.visionSearchPhoto.feeds){
                            linkArr.push(res.data.visionSearchPhoto.feeds[i])
                        }
                        // console.log(linkArr)

                        if(res.data.visionSearchPhoto.pcursor != "no_more" && linkArr.length < 40){
                            setTimeout(function(){
                                getDyData(keyword)
                            },2000)
                        }else{
...
//         // })
//     }
    else if(rex.match(/https:\/\/search.bilibili.com\/video*/) != null){

        let biliArr = ["健身娱乐","一人之下","古典音乐","街头运动"]
        let linkArr = []

        function getKey(biliArr,index=0){
            if(index >= biliArr.length){
                console.log("已完成")
                $(".loading").hide()
                return;
            }

            let biliKey = biliArr[index]
            getDyData(biliKey,index)
        }

        // B站 哔哩哔哩
        function getDyData(biliKey,index){
            ...
            let starData = {
                ...
                page_size: 40,
                from_source: "",
                from_spmid: unsafeWindow.__spm_prefix,
                platform: "pc",
                highlight: 1,
                single_column: 0,
                keyword: biliKey,
                qv_id: unsafeWindow.__pinia.searchResponse.qvId,
                source_tag: 3,
                gaia_vtoken: "",
                dynamic_offset: 0,
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    for(let i in res.data.result){
                        linkArr.push(res.data.result[i])
                    }
                    console.log(linkArr)

                    // for(let k in linkArr){
                    //     setTimeout(function(linkArr){
                    //         window.open(linkArr[k])
                    //     },2000)
                    // }
                    function openLink(linkArr,linkKey=0){
                        if(linkKey >= linkArr.length){
                            console.log("已完成")
                            if(linkArr.length < 40){
                                getDyData(biliKey,index)
                            }else{
                                linkArr=[]
                                setTimeout(function(){
                                    getKey(biliArr,index+1)
                                },3000)
                            }
                        }else{
                            setTimeout(function(){
                                window.open(linkArr[linkKey].arcurl)
                                openLink(linkArr,linkKey+1)
                            },2000)
                        }
                    }
...
//                     });
    ...
    }else if(rex.match(/https:\/\/www.bilibili.com\/video*/) != null){
        // alert(unsafeWindow.__playinfo__.data.dash.audio[1].baseUrl)
        // alert(unsafeWindow.__playinfo__.data.dash.audio[1].baseUrl)
        // window.close()
    }
