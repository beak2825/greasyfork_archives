// ==UserScript==
// @name        Auto Reload F5(Traduzido PT-BR)
// @version     0.58
// @author      paulonkey(Traduzido por:Chocodubabr)
// @include 	*
// @grant		GM_deleteValue 
// @grant		GM_getValue
// @grant		GM_setValue 
// @description Recarrega a pagina de tempo em tempo. Configuração: (Shift + T)
// @namespace http://userscripts.org/scripts/show/41891
// @downloadURL https://update.greasyfork.org/scripts/390539/Auto%20Reload%20F5%28Traduzido%20PT-BR%29.user.js
// @updateURL https://update.greasyfork.org/scripts/390539/Auto%20Reload%20F5%28Traduzido%20PT-BR%29.meta.js
// ==/UserScript==
...
reload = [	'Tempo para reload?  Esse numero em segundos.',	'In welchem Intervall -in Sekunden- soll die Seite neugeladen werden?',	'Cada cuantos segundos quiere actualizar la página?',	'Hoe vaak wilt u de pagina verversen?, Interval is in seconden',	'À quelle fréquence diriez-vous recharger votre page ? Ce nombre est en secondes.',	'Tempo para reload?  Esse numero em segundos.'	];
...
statusbar_behind_seconds = [	' Segundos restantes...',	' Sekunden neugeladen...',	' segundos...',	' seconden over...',	'secondes restantes ...',	' 时间还剩...秒'	];
statusbar_behind_minutes = [	' Minutos restantes...',	' Minuten neugeladen...',	' minutos...',	' minuten over...',	'minutes restantes ...',	' 时间还剩...分钟'	];
statusbar_behind_hours = [	' Horas restantes...',	' Stunden neugeladen...',	' horas...',	' uren over...',	'heures restantes ...',	'  时间还剩...小时'	];
statusbar_stop_refresh = [	'A contagem regressiva foi cancelada...',	'Die Aktualisierung wurde abgebrochen...',	'La cuenta atrás ha sido cancelada...',	'Het aftellen is geannuleerd...',	'Le compte à rebours a été annulé ...',	' 倒计时已经取消...'	];
offline_box_message = [	'Conexão perdida com o site. A pagina ira ser recarregada, quando a conexão for restaurada...',	'Verbindung fehlgeschlagen. Die Seite wird neugeladen, sobald wieder eine Verbindung besteht... ',	'Pérdida de conexion. La página será recargada cuando se recupere la conexion...',	'Verbinding met site verbroken. Als de verbinding is hersteld, zal de pagina opnieuw worden geladen...',	'la connexion au site est perdu. La page sera rechargée, lorsque la connexion sera rétablie ...',	'与网站失去连接，当连接恢复时，页面将被重新加载...'	];
...
options_default_timeout = [	'Tempo padrão',	'Standardtimeout',	'Tiempo de recarga estándar',	'Standaard timeout',	'délai par défaut',	'默认的超时时间'	];
options_timeout = [	'Tempo nesta pagina',	'Timeout der aktuellen Seite',	'Tiempo de recarga de esta página',	'Timeout voor deze pagina',	' delai pour cette page',	'此网页超时时间'	];
options_random = [	'Adicione um valor randomico para esgotar o tempo entre 0 e',	'Zufallswert zu Timeout hinzufügen. Zwischen 0 und',	'Añadir número aleatorio entre 0 y',	'Voeg een willekeurige waarde aan timeout toe, tussen 0 en',	'Ajout de la valeur du délai d\'attente aléatoire compris entre 0 et',	'添加超时随机值，介于0和'	];
options_hotKeys = [	'Mostrar e esconder menu ',	'Menü ein- und ausblenden ',	'Mostrar y esconder menu ',	'Toon en verberg menu ',	'Afficher et masquer le menu',	'唤醒菜单快捷键'	];
options_separators = [	'Ignore tudo após esses simbolos na URL(separe com speaços)',	'Ignoriere URLs nach folgenden Symbolen(mit Leerzeichen trennen)',	'Ignorar todo despues de estos símbolos en URLs (separar con espacios)',	'Negeer alles na deze symbolen in URL(gescheiden door spatie\'s)',	'Tout ignorer après ces symboles dans l\'URL (séparés par des espaces)',	'忽略网址这些符号之后的一切（用空格分隔）'	];
...
options_autoclose = [	'Esconder menu ao clicar fora dele',	'Bei Click außerhalb des Menüs, Menü ausblenden',	'Ocultar menú al clickear fuera del menú',	'Verberg menu als er buiten het menu wordt geklikt',	'Masquer le menu lorsque vous cliquez en dehors du menu',	'单击菜单外时隐藏菜单'	];
options_click_extend = [	'Prolongar tempo ao clicar ou digitar',	'Bei Mausklicks oder Tastatureingaben Timeout hinauszögern',	'Prolongar cuenta atras cliqueando o tecleando',	'Verleng timeouts muis klik of toetsenbord invoer',	'Prolongez le délai sur les clics de souris ou du clavier',	'当本页有鼠标和键盘活动时超时刷新'	];
options_refresh_all = [	'Recarregar todas as paginas com tempo padrão, que estão sem tempo definido',	'Aktualisiere alle Seiten mit Standardtimeout, für die kein Timeout gesetzt wurde',	'Actualiza todas las páginas con tiempo de recarga estándar, sin propio tiempo de recarga',	'Herlaad alle pagina\'s met standaard time-out, welke niet ingesteld zijn op standaard',	'Recharger toutes les pages avec temporisation par défaut, ce qui n\'a pas un délai défini',	'重新装入默认的超时时间，它没有超时设置所有页面'	];
options_btn_ok = [	'OK',	'OK',	'OK',	'OK',	'OK',	'确定'	];
options_btn_cancel = [	'Cancelar',	'Abbrechen',	'Cancelar',	'Annuleren',	'annuler',	'取消'	];
options_btn_abort = [	'Cancelar Reload',	'Neuladen verhindern',	'Abortar recarga',	'Herladen afbreken',	'Abandonner le rechargement',	'重置本页'	];
options_number_format = [	'Exemplos de formatos: hh:mm:ss, h:m:ss, m:sss, s etc. like 1:20:30 oder 500 oder 3:100',	'Formatbeispiele: hh:mm:ss, h:m:ss, m:sss, s, also 1:20:30 oder 500 oder 3:100',	'Ejemplos de formato: hh:mm:ss, h:m:ss, m:sss, s etc. como 1:20:30 oder 500 oder 3:100',	'Formaat voorbeelden: hh:mm:ss, h:m:ss, m:sss, s etc. zoals 1:20:30 of 500 of 3:100',	'Format exemples: hh:mm:ss, h:m:ss, m:sss, s etc. comme 1:20:30 ou 500 ou 3:100',	'超时时间格式表达，例如 时时:分分:秒, 时:分:秒, 分:秒, 秒 等。就像 1:20:30 或者 500 或者3:100'	];
num_err_default = [	'Entre um valor positivo para o tempo padrão.',	'Bitte eine positive ganze Zahl für Standardtimeout eingeben.',	'Introducir número entero y postivo de tiempo de recarga estándar por favor',	'A.u.b. voer een positief getal in voor standaard time-out',	'S\'il vous plaît entrer une valeur numérique positive de delai par défaut.',	'请输入一个缺省的超时数字值。'	];
num_err_timeout = [	'Coloque um valor positivo para o tempo.',	'Bitte eine positive ganze Zahl für Timeout eingeben.',	'Introducir número entero y postivo de tiempo de recarga por favor',	'A.u.b. voer een positief getal in voor time-out',	'S\'il vous plaît entrer une valeur numérique positive de delai.',	'请输入一个超时数字值。'	];
num_err_random = [	'Coloque um numero positivo para um tempo randomico.',	'Bitte eine positive ganze Zahl für Zufallszahl eingeben.',	'Introducir número entero y postivo de número aleatorio por favor',	'A.u.b. voer een positief getal in voor een willekeurige waarde',	'S\'il vous plaît entrer une valeur numérique positive pour la valeur aléatoire.',	'请输入一个随机值数值。'	];
time_format_err = [	'Não coloque mais que 3 \':\'',	'Bitte nicht mehr als drei \':\' eingeben',	'No poner mas de tres \':\' por favor',	'A.u.b. voer niet meer dan drie \':\'',	'S\'il vous plaît ne pas entrer plus de trois \':\'',	'请不要输入超过三个 \':\''	];
