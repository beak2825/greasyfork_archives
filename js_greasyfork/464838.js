// ==UserScript==
// @name           RJ_preview_and_asmr_one_redirect
// @author         SettingDust
...
// @name:zh-CN     RJ预览+asmr.one转跳
// @description:zh-cn    让RJ更有用
...
// @downloadURL https://update.greasyfork.org/scripts/464838/RJ_preview_and_asmr_one_redirect.user.js
// @updateURL https://update.greasyfork.org/scripts/464838/RJ_preview_and_asmr_one_redirect.meta.js
// ==/UserScript==

// src/fetch-rj.ts

const URL = window.location.href
if (URL.search('www.dlsite.com')!=-1){
    var dlsite_items=document.querySelectorAll('.work_name');
    if (dlsite_items){
        dlsite_items.forEach(i=>{
            var a=i.querySelector('a')
            var RJ=document.createElement('RJ')
            RJ.innerText=a.getAttribute('href').split('/')[7].split('.')[0]
            a.parentNode.insertBefore(RJ,a)
        })
    }
}
...
function wrapRJCode(rj) {
    ...
    return a.href = productPage(rj), a.innerHTML = rj, a.target = "_blank", a.rel = "noreferrer", a.dataset[RJ_CODE_ATTRIBUTE] = rj.toUpperCase(), a;
}
function injectRJCode(node) {
  var _a;
  let text = node.nodeValue, matches = [], match;
  const RAPIDUPLOAD_REGEXP=new RegExp("[A-Za-z0-9]*#[A-Za-z0-9]*#.*", "gi")
  if ( text.search('pan.baidu.com')!=-1 || RAPIDUPLOAD_REGEXP.exec(text))
      return
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    let nextIndex = (_a = matches[i + 1]) == null ? void 0 : _a.index, afterText = text.substring(
      ...
      var RJ=match2.value
      const URL = window.location.href
      if (URL.search('asmr.one')==-1){
          const ASMR_SEARCH_BUTTON = createElementFromHTML("<a id='xiccnd_asmr_search_button' style='margin:2px' target='_blank' rel='noreferrer' href='https://asmr.one/work/" + RJ +" '>asmr</a>")
          $(ASMR_SEARCH_BUTTON).hide()
          a.parentNode.insertBefore(ASMR_SEARCH_BUTTON,a)         
          GM_xmlhttpRequest({
              method: "GET",
              url: 'https://api.asmr.one/api/workInfo/'+RJ.slice(2),
              responseType: "json",
              onload: function(resp) {
                  if(resp.readyState === 4 && resp.status === 200) {
                      if(resp.response['has_subtitle']){
                          ASMR_SEARCH_BUTTON.style.textDecoration='underline 3px red'
                      }
                      else{
                          ASMR_SEARCH_BUTTON.style.textDecoration='underline 3px green'
                      }
                      $(ASMR_SEARCH_BUTTON ).show()
                  }
              }
          });
      }
