// ==UserScript==
// @name         HIT Forker (PoC Broken Req)
// @version      0.8.1
...
// @downloadURL https://update.greasyfork.org/scripts/39295/HIT%20Forker%20%28PoC%20Broken%20Req%29.user.js
// @updateURL https://update.greasyfork.org/scripts/39295/HIT%20Forker%20%28PoC%20Broken%20Req%29.meta.js
// ==/UserScript==
...
$('body').on('click', '.request_qual', function() {
    _requestQual( $(this).data('reqlink') )
});
        ...
        if (quals[k] !== '') {
            if( quals[k].indexOf('~!~') != -1 ) {
                ...
                if (temp[1].indexOf('request') == -1 ) {
                    ...
                    //Request Qual
                    quals[k] += '<button class="request_qual" data-reqlink="' + temp[1] +'">Request</button>';
                }
            }
...
function _requestQual( reqURL ) {
    var postreq = "https://worker.mturk.com" + reqURL;
    console.log( postreq );
    location.assign('javascript:$.post( "' + postreq + '");');
    fetch( postreq, {
        method: 'post'
    });
    /*GM_xmlhttpRequest ({
   ...
   // $.post( "https://worker.mturk.com" + reqURL, function (data) { alert (data); } );
   */
    alert( "Qualification requested!" );
}
...
function _export_irc (key) {
  ...
  $.get('https://ns4t.net/yourls-api.php?action=bulkshortener&title=MTurk&signature=39f6cf4959&urls[]=' + hit.prevlink + '&urls[]=' + hit.pandlink, function (data) {
    ...
    var exportcode = 'Req: ' + hit.reqname + ' ■ Title: ' + hit.title + ' ■ Reward: ' + hit.reward;
