...
// @downloadURL https://update.greasyfork.org/scripts/540550/Voxhack%20by%20Lox.user.js
// @updateURL https://update.greasyfork.org/scripts/540550/Voxhack%20by%20Lox.meta.js
// ==/UserScript==
...
chamsFolder.add(espConfig, 'showOres').name('Show Ores (,)');
...
// замените старый document.addEventListener('keydown', …) этим
document.addEventListener('keydown', (e) => {
  if (e.target && (e.target.isContentEditable || e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement)) {
        return;
    }
  ...
  if (e.key === ",") {
    espConfig.showOres = !espConfig.showOres
  }
  if (e.key === 'e') {
...
(function injectShowOres() {
  ...
  WebGL.shaderSource = new Proxy(WebGL.shaderSource,{
    apply(target,gl,[shader,src]){
      if(src.includes('vRealUv = realUv;')){
        ...
        `).replace('vRealUv = realUv;',`
          vRealUv = realUv;
          float atlasDim = 16.0;
          float tilePosX = clamp(fract(vRealUv.z),0.01,0.99);
          float tilePosY = clamp(fract(vRealUv.w),0.01,0.99);
          vec2 uv = vec2(vRealUv.x*(1.0/atlasDim)+tilePosX*(1.0/atlasDim),
                         vRealUv.y*(1.0/atlasDim)+tilePosY*(1.0/atlasDim));
          if(showOres){
      ...
      return Reflect.apply(target,gl,[shader,src]);
    }
  });

  WebGL.attachShader = new Proxy(WebGL.attachShader,{
    apply(target,gl,[program,shader]){
      if(shader.isChunkShader) program.isChunkProgram = true;
      return Reflect.apply(target,gl,[program,shader]);
    }
  });

  WebGL.useProgram = new Proxy(WebGL.useProgram,{
    apply(target,gl,[program]){
      const res = Reflect.apply(target,gl,[program]);
      if(program && program.isChunkProgram){
        if(!program.__showOresLoc){
          program.__showOresLoc = {
            uShow: gl.getUniformLocation(program,'showOres'),
            uTime: gl.getUniformLocation(program,'currTime')
          };
        }
        if(program.__showOresLoc.uShow) gl.uniform1i(program.__showOresLoc.uShow, espConfig.showOres?1:0);
        if(program.__showOresLoc.uTime) gl.uniform1f(program.__showOresLoc.uTime, performance.now()/1000);
      }
      return res;
    }
  });
