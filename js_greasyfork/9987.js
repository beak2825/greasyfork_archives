// ==UserScript==
// @name LabaBot+SH
// @description Бот для лабы Fcity.su  - быстрый  + СХ
// @author Catchme
// @license MIT
// @version 1.1
// @include http://www.fcity.su/*
// @namespace https://greasyfork.org/users/10566
// @downloadURL https://update.greasyfork.org/scripts/9987/LabaBot%2BSH.user.js
// @updateURL https://update.greasyfork.org/scripts/9987/LabaBot%2BSH.meta.js
// ==/UserScript==
...
(function (window, undefined) {  // [2] нормализуем window
    ...
    doc.write(
        ...
        '<body bgcolor=#e2e0e0><nobr><img src="http://www.fcity.su/i/klan/Eon.gif"><input type="button" id="autopilot_enable" name="autopilot_enable" value="Старт" onclick="autopilotEnable();" /> Бот для Лабиринта + СХ Fcity.su &copy; Catchme</nobr></body>' +
        '</html>');
    ...
    setInterval(function(){
        if (top.frames['plugin'].window.autopilot) {
			...
			battle.find(':contains("50(11-11)")').click();
			...
			battle.find('input[value="Принять участие в мясорубке"]').click();
			   ...
			   battle.find(':contains("против 50(2-50)")').each(function() {
		...
		battle.find(':contains("Колодец жизни")').each(function() {
				...
				//mainFrame.location.href = 'http://www.fcity.su/quests.php?new=1';
			    //mainFrame.location.href = 'http://www.fcity.su/quests.php?end=1';
				...
				battle.find(':contains("Доступные задания")').each(function() {
				 battle.find('input[name="get_quest"]').each(function() {
                $(this).click();				
				});
				mainFrame.location.href = 'http://www.fcity.su/quests.php?end=1';
				//battle.find('input[value="Назад"]').click();
			    });		
				
				battle.find(':contains("Доступные задания")').each(function() {
					battle.find(':contains("Вы уже взяли задание")').each(function() {
				 mainFrame.location.href = 'http://www.fcity.su/quests.php?end=1';
              				});
			    });		
				
				battle.find(':contains("Текущие задания")').each(function() {
				 battle.find('input[value="Назад"]').each(function() {
                $(this).click();
				});
			    });		
				
				battle.find(':contains("Завершенные задания")').each(function() {
				 battle.find('input[name="get_priz"]').each(function() {
                $(this).click();
				});
				mainFrame.location.href = 'http://www.fcity.su/vxod_lab.php';
				battle.find('input[name="buyexit"]').click();
				//battle.find('input[value="Назад"]').each(function() {
                //$(this).click();
				//});
			    });		
				...
				battle.find(':contains("Завершенные задания")').each(function() {
				 battle.find(':contains("Приз получен")').each(function() {
               	mainFrame.location.href = 'http://www.fcity.su/vxod_lab.php';
				 battle.find('input[name="buyexit"]').click();
							    });		
								});	
