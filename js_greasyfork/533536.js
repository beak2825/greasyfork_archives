// ==UserScript==
// @name         湖南人社-湖南云畔课程助手
// @namespace    http://tampermonkey.net/zzzzzzys_湖南人社-湖南云畔
// @version      1.0.2
// @copyright    zzzzzzys.All Rights Reserved.
// @description  湖南人社-湖南云畔,自动化脚本，自动化完成所有已选课程视频，自动填写验证码！
// @author       zzzzzzys
// @match        https://www.hnedu123.com.cn/personal-center/my-course*
// @match        https://www.hnedu123.com.cn/my-course-details*
...
// @license      
// @downloadURL https://update.greasyfork.org/scripts/533536/%E6%B9%96%E5%8D%97%E4%BA%BA%E7%A4%BE-%E6%B9%96%E5%8D%97%E4%BA%91%E7%95%94%E8%AF%BE%E7%A8%8B%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/533536/%E6%B9%96%E5%8D%97%E4%BA%BA%E7%A4%BE-%E6%B9%96%E5%8D%97%E4%BA%91%E7%95%94%E8%AF%BE%E7%A8%8B%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
class Runner {
    constructor() {
        this.runner = null
    ...
    run() {
        ...
        if (url.includes("my-course-details")) {
            this.runner = new Course("channel-hunau")
            // this.runner.run()
        } else if (url.includes("my-course")) {
...
class Index {
    ...
    async run() {
        ...
        try {
            ...
            const catalogSelector = '.listBox li'
            const btnSelector='.studyBtn'
            ...
            for (let i = 0; i < pageList.length; i++) {
                ...
                for (let catalog of catalogList) {
                    console.log(catalog.querySelector('.courseTitle').innerText)
    ...
    async CourseRun() {
        ...
        try {
            const courseSelector = '.el-tree-node__children .el-tree-node'
            const btnSelector=".studyBtn"
            const courseList = await this.getStudyNode(courseSelector, 'nodeList')
            let index=0
            for (let i = 0; i < courseList.length; i++) {
                // const course = (await this.getStudyNode(courseSelector, 'nodeList'))[i]
                const course = courseList[i]
                await sleep(1000)
                try{
                    console.log(course.querySelector('.treeJie').innerText)
                    if (this.CourseCheckFinish(course)) {
                        console.log("已学完，跳过！")
                        continue
                    }
                    // course.querySelector(btnSelector).click()
                    course.click()
                    await sleep(2000)
                }catch (e) {
                    console.error(e)
    ...
    CourseCheckFinish(dom) {
        return dom.querySelector('.numberPercent').innerText.includes('100')
    }
...
class Utils {
    ...
    static vipText = '高级功能已启用！(湖南人社-湖南云畔)'
    ...
    static async validateCode(data) {
        try {
            let value = document.querySelector(".p-title").innerText
            if(!value){
                throw Error("无效的账号信息！")
            }
            data.bindInfo =value
            data.website = "68045ad43f1a470b6e29a521"
            console.log(data);
            // return
