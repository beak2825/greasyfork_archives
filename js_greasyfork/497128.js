// ==UserScript==
// @name         Для Кураторов/Заместителей Кураторов
...
// @downloadURL https://update.greasyfork.org/scripts/497128/%D0%94%D0%BB%D1%8F%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%D0%97%D0%B0%D0%BC%D0%B5%D1%81%D1%82%D0%B8%D1%82%D0%B5%D0%BB%D0%B5%D0%B9%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2.user.js
// @updateURL https://update.greasyfork.org/scripts/497128/%D0%94%D0%BB%D1%8F%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2%D0%97%D0%B0%D0%BC%D0%B5%D1%81%D1%82%D0%B8%D1%82%D0%B5%D0%BB%D0%B5%D0%B9%20%D0%9A%D1%83%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%BE%D0%B2.meta.js
// ==/UserScript==
...
(function () {
    ...
    },
    {
     ...
     },
     {
        ...
        '[CENTER]Доброго времени суток, уважаемый(ая) {{ user.mention }}.<br><br>' +
        '[CENTER]Технический специалист предоставил доказательства вашего нарушения. После проверки доказательств было принято решение, что наказание, выданное техническим специалистом, является верным.[/CENTER]<br>'+
        '[CENTER]Вы нарушили пункт 2.28 общих правил проекта. [SPOILER="2.28"][COLOR=rgb(255, 0, 0)]2.28.[/COLOR] Запрещена покупка/продажа внутриигровой валюты в любых ее проявлениях за реальные деньги | [COLOR=rgb(255, 0, 0)]PermBan с обнулением аккаунта + ЧС проекта[/COLOR][/SPOILER][/CENTER] <br>' +
        '[CENTER]Наказание выдано верно, [COLOR=rgb(255, 0, 0)]закрыто.[/COLOR][/CENTER]<br>',
        prefix: CLOSE_PREFIX,
        status: false,
    },
    ...
    },
    {
        ...
        content:
        '[CENTER]Доброго времени суток, уважаемый(ая) {{ user.mention }}.<br><br>' +
        '[CENTER]Проверив доказательства Технического специалиста, я пришел к выводу, что наказание действительно выдано верно. [/CENTER] <br>' +
        '[CENTER]На вашем аккаунте присутствует привязка злоумышленника. К сожалению, обжаловать выданное наказание никак. [/CENTER]<br>' +
	...
	},
	{
        ...
        content:
        '[CENTER]Доброго времени суток, уважаемый(ая) {{ user.mention }}.<br><br>' +
        '[CENTER]Ваш аккаунт будет разблокирован в течении 24х часов.[/CENTER]<br>' +
        '[CENTER]Приносим свои извинения за предоставленные неудобства.[/CENTER]<br>' +
		'[COLOR=rgb(127, 255, 0)]Рассмотрено.[/COLOR][/CENTER]',
		prefix: WATCHED_PREFIX,
		status: false,
     },
     {
		...
		content:
		'[CENTER]Доброго времени суток, уважаемый(ая) {{ user.mention }}.<br><br>' +
		"[CENTER]Блокировка вашего аккаунта содержит серьёзное нарушение правил сервера.<br>Вы получили блокировку за серьезное нарушение, мы не в силах снизить срок вашего наказания/обнулить.<br>" +
	...
	},
	{
		...
		content:
		'[CENTER]Доброго времени суток, уважаемый(ая) {{ user.mention }}.<br><br>' +
		'[CENTER]С момента выдачи наказания от технического специалиста прошло более 7-ми дней.<br>Пересмотр/изменение меры наказания невозможно, вы можете попробовать написать обжалование через N-ый промежуток времени.<br><br>Обращаем ваше внимание на то, что некоторые наказания не подлежат не обжалованию,амнистии.[/center]<br><br>'+
		'[CENTER][COLOR=rgb(255, 0, 0)]Отказано.[/COLOR][/CENTER]',
		prefix: UNACCEPT_PREFIX,
		status: false,
    },
