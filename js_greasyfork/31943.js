// ==UserScript==
// @id WhutBBCode
// @name WhutBBCode? - Bj-Share.info
// @namespace hateradio)))
// @author hateradio & updated by Felipe Amaral
// @version 99.999
// @description Versão modificada do WhutBBCode? para funcionar com Bj-Share.info, apollo.rip, redacted, waffles, btn, indietorrents, cg, mtv, ggn e ptp
// @homepage https://greasyfork.org/en/scripts/31943-whutbbcode-bj-share-info
...
// @include http*://*bj-share.info/*
// @include http*://*bj-share.info/wiki.php*
// @exclude http*://*bj-share.info/reportsv2.php*
// @exclude http*://*bj-share.info/user.php?action=notify*
...
// @downloadURL https://update.greasyfork.org/scripts/31943/WhutBBCode%20-%20Bj-Shareinfo.user.js
// @updateURL https://update.greasyfork.org/scripts/31943/WhutBBCode%20-%20Bj-Shareinfo.meta.js
// ==/UserScript==
/**
* Updates
* 3.6
...
(function () {
   ...
   *     emoticons: WhutBB.db.emoticons.gazelle.slice(0, 29),
   *     blueprint: [
   *       ['b', 'i', 'u', 's'], ['code'],
   *       ['color', 'size'], ['*'],
   *       ['url', 'img'], ['quote'],
    ...
    WhutBB.db = {
        sites: {
            ':default': function () {
                return {
                    name: '',
                    link: 'https://greasyfork.org/en/scripts/31943-whutbbcode-bj-share-info',
                    beneath: true,
                    blueprint: [],
                    width: 550,
                    emoticonDir: '',
            ...
            ':generic': function () {
                return {
                    emoticonDir: 'https://apollo.rip/static/common/smileys/',
                    emoticons: WhutBB.db.emoticons.gazelle.slice(0, 40),
                    blueprint: [
                        ['b', 'i', 'u', 's'], ['important', 'heading', 'code'],
                        ['color', 'size'], ['gz_left', 'gz_center', 'gz_right'],
                        ['#', '*'], ['url', 'img', 'video'], ['quote', 'pre'], ['hide', 'mature'],
                        ['artist', 'torrent', 'user', 'wiki', 'gz_rule'], ['tex', 'plain'], ['erase'],
                        ['emoticon', 'settings', 'shortcut']
                    ]
                };
            },
            ...
            indietorrents: function () {
                return {
                    ...
                    blueprint: [
                        ['b', 'i', 'u', 's'], ['color', 'size'],
                        ['gz_left', 'gz_center', 'gz_right'], ['*'], ['url', 'img', 'video'],
        ...
        buttons: {
            b: {title: 'Negrito', icon: 'bold'},
            i: {title: 'Itálico', icon: 'italic'},
            u: {title: 'Sublinhado', icon: 'text-width'}, //underline
            s: {title: 'Tachado', icon: 'minus'},
            code: {display: 'c', title: 'Inserir código', icon: 'leaf'},
            important: {display: '!', title: 'Texto Importante', icon: 'exclamation-sign'},
            color: {type: 1, display: '\u25ee', prompt: 'Coloque uma cor #hexadecimal ou o nome em inglês.', title: 'Cor', val: '#', icon: 'tint'},
            size: {type: 1, display: '\u00b1', prompt: 'Coloque o tamanho da fonte.', title: 'Tamanho', val: 3, icon: 'text-height'},
            align: {type: 1, display: '-', title: 'Alinhar texto', icon: 'align-left'},
            left: {display: '<', title: 'Alinhar Esquerda', icon: 'align-left'},
            center: {display: '\u2013', title: 'Alinhar Centro', icon: 'align-center'},
            right: {display: '>', title: 'Alinhar Direita', icon: 'align-right'},
            '#': {type: 3, title: 'Lista Ordenada', icon: 'list-alt'},
            '*': {type: 3, title: 'Lista Desordenada', icon: 'list'},
            url: {type: 1, prompt: 'Coloque um Link', title: 'Link Internet', val: 'http://', icon: 'globe'},
            img: {title: 'Imagem', icon: 'picture'},
            quote: {type: 1, display: 'q', prompt: 'Coloque um autor ou nome.', title: 'Citar', placeholder: 'author', icon: 'comment'},
            pre: {title: 'Texto formatado/Código-fonte', icon: 'asterisk'},
            hide: {display: 'h', title: 'Esconder conteúdo/Spoilers', icon: 'warning-sign'},
            spoiler: {display: '_', title: 'Spoilers!', icon: 'exclamation-sign'},
            mature: {display: 'm', title: 'Esconder conteúdo adulto', icon: 'eye-open'},
            artist: {display: 'a', title: 'Linkar um artista/banda no site site', icon: 'music'},
            user: {display: 'p', title: 'Linkar um usuário', icon: 'user'},
            wiki: {type: 4, tag: ['[[', ']]'], display: 'w', title: 'Linkar um artigo', icon: 'share'},
            tex: {display: 't', title: 'LaTeX', icon: 'pencil'},
            plain: {display: 'x', title: 'Desabilitar BBCode.', icon: 'ban-circle'},
            video: {type: 0, display: 'yt', title: 'Inserir vídeo', icon: 'film'},
            font: {type: 1, display: 'f', prompt: 'Digite o nome da fonte', title: 'Font', val: 'Arial', icon: 'font'},
            torrent: {display: 'id', title: 'Linkar um torrent pelo ID.', icon: 'download'},
            search: {type: 1, display: '@', prompt: 'Digite uma palavra para buscar', title: 'Linkar para uma busca.', val: 'keywords', icon: 'search'},
            table: {display: 'tbl', title: 'Insere uma tabela.', icon: 'th-large'},
            tr: {display: 'tr', title: 'Insere uma linha na tabela.', icon: 'th-list'},
            th: {display: 'th', title: 'Insere um título na tabela.', icon: 'th'},
            td: {display: 'td', title: 'Insere uma célula na tabela.', icon: 'pencil'},
            heading: {type: 4, tag: '=', display: '=', title: 'Insere um cabeçalho', icon: 'arrow-right'},
            // Gazelle
            gz_left: {tag: 'align', val: 'left', type: 1, noPrompt: true, display: '<', title: 'Alinhar Esquerda', icon: 'align-left'},
            gz_center: {tag: 'align', val: 'center', type: 1, noPrompt: true, display: '\u2013', title: 'Alinhar Centro', icon: 'align-center'},
            gz_right: {tag: 'align', val: 'right', type: 1, noPrompt: true, display: '>', title: 'Alinhar Direita', icon: 'align-right'},
            gz_src: {macro: ['quote', 'pre'], type: -3, display: '<>', title: 'Inserir Código-fonte', icon: 'tasks'},
            gz_rule: {tag: 'rule', title: 'Linkar para uma regra no site', icon: 'info-sign', display: 'r' },
            // Panels
            emoticon: {display: ':]', toggle: ';]', title: 'Emoticons', type: -1, icon: 'fire'},
            settings: {display: '%', title: 'Ajustes', type: -1, icon: 'wrench'},
            shortcut: {display: '?', title: 'Atalhos', type: -1, icon: 'question-sign'},
            erase: {display: '-', title: 'Deletar Conteúdo', type: -2, icon: 'remove-sign'}
        },
        emoticons: {
