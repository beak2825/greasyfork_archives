    ...
    // ==UserScript==
    // @name        CosmosisT's TinyChat Script (CTS V1.8.64) - With SpamBan
    // @version     1.8.64
...
// @downloadURL https://update.greasyfork.org/scripts/489739/CosmosisT%27s%20TinyChat%20Script%20%28CTS%20V1864%29%20-%20With%20SpamBan.user.js
// @updateURL https://update.greasyfork.org/scripts/489739/CosmosisT%27s%20TinyChat%20Script%20%28CTS%20V1864%29%20-%20With%20SpamBan.meta.js
    // ==/UserScript==
    ...
    (function() {
        ...
        function CTSRoomInject() {
            ...
            VideoListElement.querySelector("#videolist").insertAdjacentHTML("afterbegin", '<div class="ctsdrop"><button class="ctsoptions" title="CTS Options">🔧</button><div class="ctsdrop-content"><div style="height:6px;background:#624482;"></div><button id="BackgroundUpdateRight" class="ctsoptions" title="Background">➖</button><button id="BackgroundUpdateLeft" class="ctsoptions" title="Background">➕</button><div style="height:6px;background:#624482;"></div><button id="FontSizeUpdate" class="ctsoptions" title="Font Size">🔍</button><button id="ChatCompact" class="ctsoptions" title="Compact Chat">💬</button><div style="height:6px;background:#624482;"></div>' + ((!CTS.ThemeChange) ? '<button id="ChatWidthToggled" class="ctsoptions" title="Chat Resize">↔</button><button id="ChatHeightToggled" class="ctsoptions" title="Chat Resize">↕</button><div style="height:6px;background:#624482;"></div>' : '') + '<button id="ChatColor" class="ctsoptions" title="Chat Style">🔨</button><button id="CameraBorderToggled" class="ctsoptions" title="Camera Border">📷</button><button id="FeaturedToggled" class="ctsoptions" title="Featured Resize">📺</button><button id="PerformanceModeToggled" class="ctsoptions" title="Performance Mode">🎮</button>' + ((!CTS.Project.isTouchScreen) ? '<div style="height:6px;background:#624482;"></div><button id="ThemeChange" class="ctsoptions" title="Switch CTS Theme Mode">🔄</button><button id="SpamBan" class="ctsoptions" title="SpamBan Toggle">🪓</button></div></div>' : ''));
        ...
        function CTSRoomPrepare() {
           ...
           window.TinychatApp.BLL.ChatRoom.prototype.tcPkt_MsgParse = function(a) {
                let b = new window.TinychatApp.DAL.ChatLogItemEntity;
               console.log(a.text);
               if (a.text == "test"){
                   a.text = "a";
        ...
        function CreateGift() {
            ...
            CTS.Message[0].push({
                ...
                "msg": "<br><div class=\"gift\"><center>" + gift.name + "</center><br><a href=\"" + gift.store_url + "\" target=\"_blank\"><img style=\"display: block;margin-left: auto;margin-right: auto;width: 50%;\" src=\"" + gift.url + "\"></a><center><br>" + comment + "<br>From:<br>" + from + "<br>To:<br>" + to + "</center></div><br>",
                "mention": true
            });
        ...
        function LineSpam() {
            let SafeUser = isSafeListed(arguments[2]);
            var LineBreaks = (arguments[0].match(/\n|\r/g) || []).length;
            if (arguments[1] === false & !SafeUser) {
                if (LineBreaks >= 14) return true;
                else if (arguments[0].length >= 300) return true;
                else if ((CTS.LineSpamCheck.filter(item => item === arguments[0]).length >= 2)) return true;
                else {
                    CTS.LineSpamCheck.push(arguments[0]);
                    if (CTS.LineSpamCheck.length > 3) {
                        CTS.LineSpamCheck = CTS.LineSpamCheck.slice(-3);
                    }
                }
            } else {
            return false;
        ...
        function CheckUserAbuse() {
            ...
            if (CTS.Me.mod) {
                ...
                if (!action) {
                    let underscoreCheck = /_.*_.*_.*_.*_/;
                    if (CTS.UserBanList.includes(arguments[1]) || CTS.NickBanList.includes(arguments[2].toUpperCase()) || arguments[2].includes("th3") || arguments[2].includes("brain") || arguments[2].includes("t_h_3") || underscoreCheck.test(arguments[2])) {
                        CTS.NoGreet = true;
        ...
        var ServerInList = {
            ...
            join: function() {
                ...
                if (user === "COSMOSIST" && CTS.Room.Name.toUpperCase() !== "STONERCIRCLE" && !CTS.hasGreetedWC) { // Meet your creator
                    ...
                    Send("msg", arguments[0].nick.toUpperCase());
                } else {
            ...
            msg: function() {
                if (CTS.ScriptInit) {
                    var user = HandleToUser(arguments[0].handle);
                    if (user != -1) {
                        if (!LineSpam(arguments[0].text, CTS.UserList[user].mod, CTS.UserList[user].username)) {
        ...
        function SetLocalValues() {
            if (CTS.StorageSupport) {
                ...
                CTS.MainBackground = Load("MainBackground", "url(https://i.imgur.com/MB3vwHI.png) rgb(0, 0, 0) no-repeat");
