...
// @require      https://cdn.jsdelivr.net/npm/naive-ui@2.40.1/dist/index.min.js
// @require      https://update.greasyfork.org/scripts/517274/1483694/ElementGetter_xzf.js
// @grant        unsafeWindow
// @grant        GM_setValue
// @grant        GM_getValue
// @grant        GM_addValueChangeListener
// @downloadURL https://update.greasyfork.org/scripts/516575/%E6%B5%B7%E5%A4%96OTA%E5%B9%B3%E5%8F%B0%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/516575/%E6%B5%B7%E5%A4%96OTA%E5%B9%B3%E5%8F%B0%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==
...
function init() {
    let text = `
         <div id="xzf">
            <div
                id="xzfapp"
                :style="{right: position.right + 'px', top: position.top + 'px', position: 'fixed' ,backgroundColor: ThemeColor,zIndex:1024,width: '220px',padding:'4px'}">
    ...
    const App = {
        ...
        methods: {
            ...
            winZoomlevelChange() {
                let min = this.Zoomlevel[0] > this.Zoomlevel[1] ? this.Zoomlevel[1] : this.Zoomlevel[0];
                let max = this.Zoomlevel[0] > this.Zoomlevel[1] ? this.Zoomlevel[0] : this.Zoomlevel[1];
            ...
            readConfig() {
                ...
                this.vin = config.vin.toString() ?? this.vin;
                this.strategyid = config.strategyid.toString() ?? this.strategyid;
                this.taskid = config.taskid.toString() ?? this.taskid;
            ...
            async vinChange() {
                ...
                if (res.data.list.length == 0) {
                    ...
                    notify({
                        title: "搜索vin失败",
                        message: "未找到对应vin,请重新输入",
                        type: "error",
                        duration: this.NotifTime,
                    });
                    return;
                }
            ...
            hookUrl() {
                this.urlHooks = [
                    ...
                    {
                        url: "/#/OTA/StrategyInfoManager/StrategyConfig/StrategyDetail/",
                        init: this.init_StrategyDetail,
                        hookfun: this.urlhook_StrategyDetail,
                        observers: [],
                    },
                ];
                this.urlHooks.forEach((x) => x.init(x));
                console.debug("hookUrl", this.urlHooks);
            ...
            async urlhook_AddStrategyInfo(urlhook) {
                ...
                let but = await elmGetter.get("div.part-one > div:nth-child(9)> div > button", 30_000);
                // if (but)
                // but.addEventListener("click", async () => {
                // let but1 = await elmGetter.get(
                //     ".part-one+div> div:nth-child(3) >.content-main >div:nth-child(1)>div:nth-child(1)",
                //     5_000
                // );
                // console.debug("点击", but1);
                // but1?.click();
                // but1 = await elmGetter.get(".el-popper[x-placement^=bottom] li:nth-child(1)", 3_000);
                // but1?.click();
                // document
                //     .querySelector(".part-one+div> div:nth-child(4) >.content-main >div:nth-child(1)>div")
                //     ?.click();
                // but1 = await elmGetter.get(".el-popper[x-placement^=bottom] li:nth-child(1)", 3_000);
                // but1?.click();
                // await elmGetter.get(".el-popper[x-placement^=bottom] li:nth-child(1)", 500);
                // document.querySelector(".part-one+div> div:nth-child(5) button")?.click();
                // });
                urlhook.context = { ecuMap: ecuMap, packageMap: packageMap };
                return urlhook.context;
            },
            async urlhook_AddSoftwares(urlhook) {
                let ecu = "";
                var carECU99 = await this.GetCarECU99(this.vin);
                let ecuMap = new Map(carECU99.map((ecu) => [ecu.ecuName + ecu.hwPartNumber, ecu]));
                let ele_version = await elmGetter.get("div.content-main > form > div > div:nth-child(4) input", 30_000);
                if (ele_version) ele_version.maxLength = 10;
                let ele_ecu = document.querySelector("div.content-main > form > div > div:nth-child(1) input");
                if (ele_ecu) ecu = ele_ecu.value;
                let ele_des = await elmGetter.get("div.content-main > form > div > div:nth-child(5) textarea", 30_000);
                let vin4 = this.vin.slice(-4);
                if (ele_des) ele_des.value = "_vin:" + vin4;
                ele_des?.dispatchEvent(new Event("input"));
                urlhook.context = { ecuMap: ecuMap, ecu: ecu };
                return urlhook.context;
            },
            async urlhook_StrategyDetail(urlhook) {
                var carECU99 = await this.GetCarECU99(this.vin);
                var hw = [...new Set(carECU99.map((ecu) => ecu.hwPartNumber))];
                let packageMap = await this.GetPackageInfoList(hw);
                urlhook.context = { packageMap: packageMap };
                this.showStrategyDetailPackagesDes(urlhook);
                return urlhook.context;
            },
            ...
            init_AddSoftwares(urlhook) {
                ...
                let data = returnobservers(
                    "div.content-main > form > div > div:nth-child(2) input",
                    (mutations, observer) => {
                        if (!urlhook.context) return;
                        let ele_ecu = document.querySelector("div.content-main > form > div > div:nth-child(1) input");
                        if (!ele_ecu) return;
                        if (ele_ecu.value == urlhook.context.ecu) return;
                        urlhook.context.ecu = ele_ecu.value;
                        this.showUploadPackageEcu(urlhook);
                    },
                    { attributes: true, subtree: true }
                );
                urlhook.observers.push(data);
            },
            init_StrategyDetail(urlhook) {
                console.debug("init_StrategyDetail=====***====");
                let data = returnobservers(
                    ".main-box .content-box:nth-child(3) tbody",
                    (mutations, observer) => {
                        if (!urlhook.context) return;
                        this.showStrategyDetailPackagesDes(urlhook);
                    },
            ...
            async showUploadPackageEcu(urlhook) {
                let ecuMap = urlhook.context.ecuMap;
                let ecu = urlhook.context.ecu;
                let display = this.PackageVisibility == "visible" ? "block" : "none";
                console.debug("showUploadPackageEcu====显示上传包对应ECU====");
                await elmGetter.get(".el-popper[x-placement^=bottom]");
                const elements = document.querySelectorAll(".el-popper[x-placement^=bottom] ul li");
                elements.forEach((element) => {
                    let ele_version = element.querySelector("span");
                    if (!ele_version) return;
                    const packageInfo = ecuMap.get(ecu + ele_version.innerHTML);
                    if (!packageInfo) {
                    ...
                    if (this.PackageDes)
                        return (ele_version.innerHTML = `${ele_version.innerHTML}--${packageInfo.vin}`);
                    ele_version.innerHTML = ele_version.innerHTML;
                });
            },
            showEcuDes(urlhook) {
                ...
                const elements = document.querySelectorAll("div.part-one  tbody tbody >tr");
                elements.forEach((element) => {
                    element.style.visibility = "visible";
                    let ecu = element.querySelector("td:nth-child(2) div").innerHTML;
                    let versionele = element.querySelector("td:nth-child(3) div ");
                    let version = versionele.innerHTML;
                    let index = version.indexOf("<br>备注:");
                    version = index !== -1 ? version.slice(0, index) : version;
                    let ele = element.querySelector("td div td:nth-child(3) > .cell");
                    if (!ele) return;
                    const ecuInfo = ecuMap.get(ecu + version);
                    if (!ecuInfo) return (element.style.visibility = this.PackageVisibility);
                    element.style.visibility = "visible";
                    if (this.PackageDes) return (ele.innerHTML = `${ecuInfo.hwPartNumber}<br>备注:${ecuInfo.vin}`);
                    ele.innerHTML = ecuInfo.hwPartNumber;
                });
                let ele = document.querySelector("div.part-one  div:nth-child(3)  div.table-box > div");
                ele.classList.add("el-table--scrollable-y");
            },
            ...
            showCheckedPackagesDes(urlhook) {
                console.debug("showCheckedPackagesDes====选中目标升级包备注====", urlhook.context);
                ...
                for (const tr of trs) {
                    ...
                    let versionele = tr.querySelector("td:nth-child(5) div div");
                    let version = versionele.innerHTML;
                    let index = version.indexOf("<br>备注:");
                    version = index !== -1 ? version.slice(0, index) : version;
            ...
            showStrategyDetailPackagesDes(urlhook) {
                console.debug("showStrategyDetailPackagesDes====显示策略升级包备注====", urlhook.context);
                let packageMap = urlhook.context.packageMap;
                let eles = document.querySelectorAll(".main-box .content-box:nth-child(3) tbody tr");
                eles.forEach((ele) => {
                    let ecuele = ele.querySelector("td:nth-child(2) div");
                    if (!ecuele) return;
                    let ecu = ecuele.innerHTML;
                    let moudleele = ele.querySelector("td:nth-child(4) div");
                    if (!moudleele) return;
                    let moudle = moudleele.innerHTML;
                    let versionele = ele.querySelector("td:nth-child(5) div");
                    if (!versionele) return;
            ...
            inputUpdateTime() {
                console.debug("inputUpdateTime====填写策略升级时间====");
            ...
            Showdes(element, packageMap, checkboxedPackages) {
                ...
                if (substringAfter.includes(vin4) || moudle == "99") {
                    element.style.visibility = "visible";
                    if (this.PackageDes)
                        return (versionele.innerHTML = `${version}<br>备注: ${packageInfo.description}`);
            ...
            hookHTTP() {
                let hookurls = new Map([
                    [
                        ...
                        {
                            gethook: (url) => {
                                if (this.PackageDes || this.PackageVisibility == "collapse")
                                    return url.replaceAll("pageSize=10", "pageSize=1000");
                                return url;
                            },
            ...
            async hookMain(hookurl, __url, __data, result) {
                ...
                try {
                    ...
                    if (__data != null) data = JSON.parse(__data);
                } catch (error) {
                    console.error("hookMain", __url, error);
                    return;
                }
                let url = this.getUrlData(__url);
            ...
            async GetPackageInfoList(hwPartNumbers) {
                ...
                if (!this.PackageDes) return new Map();
                //关闭备注时,不进行api请求
            ...
            async strategyCreatestatus(url, data, json) {
                console.debug("strategyCreatestatus====创建策略提交====");
            ...
            async strategyStatusSwitch() {
                ...
                setTimeout(this.strategyStatusSwitch, 3000);
            },
            ...
            async TaskStatusSwitch() {
                ...
                switch (status[task.status]) {
                    ...
                    case "审核通过":
                        ...
                        break;
                    case "待发布":
                        setTimeout(this.TaskStatusSwitch, 3000);
                        return;
                    case "已发布":
                        ...
                        return;
                    case "任务结束":
                        res = await this.TaskRepublish(task);
                        if (!res) return;
                        break;
                    case "任务异常":
                    case "已失效":
            ...
            async TaskRepublish(task) {
                let url = "/mgapi/upgrade/task/republish";
                let data = { id: task.id };
                let json = await this.httpRequset(url, { json: data });
                return json.code == 1000;
            },
