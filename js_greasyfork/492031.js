...
// @grant        GM_openInTab
// @grant        window.close
// @downloadURL https://update.greasyfork.org/scripts/492031/%E4%B8%8D%E5%AD%A6%E4%B9%A0%E4%BD%95%E4%BB%A5%E5%BC%BA%E5%9B%BDfix.user.js
// @updateURL https://update.greasyfork.org/scripts/492031/%E4%B8%8D%E5%AD%A6%E4%B9%A0%E4%BD%95%E4%BB%A5%E5%BC%BA%E5%9B%BDfix.meta.js
// ==/UserScript==
...
//颜色配置
const Colors = { "primary": "#409EFF", "success": "#67C23A", "warning": "#E6A23C", "danger": "#d90609", "info": "#909399" };
$(document).ready(function () {
    let itv = setInterval(() => {
        let divs = document.querySelectorAll("div")
        if (!divs || divs.length < 5) {
            return;
        }
    ...
    if (url == "https://www.xuexi.cn" || url == "https://www.xuexi.cn/" || url == "https://www.xuexi.cn/index.html") {
        ...
        let ready = setInterval(function () {
            if (document.getElementsByClassName("title")[0]) {
                ...
                let examInterval = setInterval(function () {
        ...
        console.log('不处理页面', location.href);
    }
...
async function slideVerification() {
    ...
    if (xc_window) {
        ...
        //先关掉
        document.querySelector("#J-risk-close-btn").click();
        await waitingTime(1000);
        //回到上一题
        document.querySelector(".pre-btn").click();
        await waitingTime(1000);
        //回到这题
        ...
        try {
            ...
            mousedown = new MouseEvent("mousedown", {
                bubbles: true,
                cancelable: true,
                view: unsafeWindow,//控制台用window,脚本要用unsafeWindow
                detail: 0,
                screenX: 0,
                screenY: 0,
                clientX: 0,
...
function waitingTime(time) {
    ...
    time += 2 + Math.floor(Math.random() * 3000);
    return new Promise(resolve => {
        setTimeout(function () {
            resolve('done');
        }, time);
...
function createStartButton() {
    ...
    startButton.innerText = logged() ? "开始学习" : "登录学习";
...
function saveSetting() {
    ...
    settings[0] = (formData.get('0') != null);//文章
    settings[1] = (formData.get('1') != null);//视频
    settings[3] = (formData.get('3') != null);//每日答题
    console.log("保存配置")
    GM_setValue('studySetting', JSON.stringify(settings));
    showTip("保存成功");
}
...
function logged() {
    ...
    return loggedBox && loggedBox.innerText
}
//开始
    ...
    if (logged()) {
        ...
        while (continueToDo) {
            ...
            if (taskProgress != null) {
                ...
                if (settings[1] && temp != 0) {
                    tasks[1] = false;//只要还有要做的，就当做没完成
                    videoNum = temp;//还需要看多少个视频
                    console.log("2.看视频");
                    await watchVideo();
                } else {
                    tasks[1] = true;
                ...
                //检查每日答题
                if (settings[3] && taskProgress[3].currentScore != taskProgress[3].dayMaxScore) {
        ...
        //提醒登录
        console.log("未登录");
        top.location.href = "https://pc.xuexi.cn/points/login.html?ref=https%3A%2F%2Fwww.xuexi.cn%2Findex.html";
    }
    return false;
}