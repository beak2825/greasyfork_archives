...
// @downloadURL https://update.greasyfork.org/scripts/556491/vanmzplayerAdvanced.user.js
// @updateURL https://update.greasyfork.org/scripts/556491/vanmzplayerAdvanced.meta.js
// ==/UserScript==
...
// ===============================================================
// [MZ%] AUTOEXEC MOBILE PATCH — van.mz.playerAdvanced v4.27
// ===============================================================
...
(function() {
    ...
    if (isMobile) {
        console.log("[MZ%] Modo Mobile detectado — iniciando execução automática.");

        window.addEventListener('load', function() {
            try {
                // --- Executa auto colorir (equivalente ao ALT + A)
                if (typeof vanGmMz.getMax === "function" && typeof vanGmMz.showMax === "function") {
                    vanGmMz.getMax(function() {
                        vanGmMz.showMax(0); // 0 = padrão: colorir atributos
                        console.log("[MZ%] AutoColor (ALT+A) executado.");
                    });
                }

                // --- Executa análise de treino (equivalente ao ALT + S)
                if (typeof vanGmMz.getScoutReport === "function") {
                    const players = document.querySelectorAll(".playerContainer");
                    players.forEach(p => {
                        const match = p.innerHTML.match(/player_id_(\d+)/);
                        if (match) {
                            const pid = match[1];
                            vanGmMz.getScoutReport(pid, $(p), false, true);
                        }
                    });
                    console.log("[MZ%] Análise de treino (ALT+S) executada.");
                }

            } catch (err) {
                console.error("[MZ%] Erro na execução automática mobile:", err);
            }
        });
    }
})();
