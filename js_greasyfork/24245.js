// ==UserScript==
// @id             WhutBBCodeForkBeta
// @name           WhutBBCodeForkBeta?
// @namespace      hateradio)))
// @author         hateradio forked by Tesla
// @version        5.3
...
// @downloadURL https://update.greasyfork.org/scripts/24245/WhutBBCodeForkBeta.user.js
// @updateURL https://update.greasyfork.org/scripts/24245/WhutBBCodeForkBeta.meta.js
// ==/UserScript==
...
if (location.pathname == "/shoutbox.php")
	SHOUT_BOX=true;
...
(function () {
	...
	update = {
		...
		version: 5300,
		key: 'ujs_WBB_UPDT_HR',
		urij: 'https://dl.dropboxusercontent.com/s/2ho6xhyar761lhi/wbb.json',
	...
	function WhutBB(textarea, id) {
		...
		{
			//Hide next row
			//var nextRow = document.getElementsByClassName("shoutbox_messageboxback")[0].parentNode.rows[ document.getElementsByClassName("shoutbox_messageboxback")[0].rowIndex + 1 ];
			//nextRow.style.display="none";

			//Add Whut in row
			this.insertAfterRow(document.getElementsByClassName("shoutbox_messageboxback")[0]);
		}
		else
		{
	...
	WhutBB.factory = function () {
		var document_elem = null;
		if (SHOUT_BOX)
		{
			document_elem = document.getElementsByName('message');
		}
		else
		{
			document_elem = document.getElementsByTagName('textarea');
		}

		dom.aEach(document_elem, function (textarea) {
	...
	/**
	 * Creates a WhutBBCodeForkBeta? instance for a textarea
	 ...
	 * Inserts after row
	 */
	WhutBB.prototype.insertAfterRow = function (target) {
		var node = document.createElement('tr');
		node.setAttribute("id", "WhutShout");

		target.parentNode.insertBefore(this.wrap, target.nextSibling);
		//target.parentNode.appendChild(this.wrap);


		this.textarea.parentNode.insertBefore(node, this.textarea.nextSibling);
	};
	...
	/**
	 * The WhutBBCodeForkBeta? initializer
	 *
	...
	WhutBB.init = function (config) {
		if (WhutBB.gazelle && WhutBB.gazelleBlacklist.test(document.location.href)) {
			console.log('WhutBBCodeForkBeta?: Will not run in ' + RegExp.lastMatch + '!');
			return;
		}

		WhutBB.config = new WhutBB.Settings(config || WhutBB.db.getSiteSettings(WhutBB.$.findSite()));
		try {
			console.info('WhutBBCodeForkBeta? mode ' + WhutBB.config.name);
	...
	WhutBB.user = {
		...
		options: {
			...
			/*link: {
				txt: 'WhutBBCodeForkBeta? Link',
				title: 'Show WhutBBCodeForkBeta? link',
				value: true
			}*/
		},
		load: function () {
	...
	WhutBB.db = {
		sites: {
			':default': function () {
				return {
					name: '',
					link: 'https://greasyfork.org/en/scripts/24200-whutbbcodefork',
					beneath: true,
					blueprint: [],
					width: 430,
					emoticonDir: '',
			...
			abnormal: function () {
				if (SHOUT_BOX)
					return {
						link: '/wiki.php?action=article&name=BBCode',
						emoticonDir: 'https://abnormal.ws/static/common/smileys/',
						emoticons: 'abnormal',
						blueprint: 'gazelle_shout'
					};
				else
					return {
						link: '/wiki.php?action=article&name=BBCode',
		...
		emoticons: {
			...
			abnormal: [[":angry:", "angry.gif"], [":D", "biggrin.gif"], [":|", "blank.gif"], [":blush:", "blush.gif"], [":cool:", "cool.gif"], [":'(", "crying.gif"], [">.>", "eyesright.gif"], [":creepy:", "creepy.gif"], [":frown:", "frown.gif"], ["<3", "heart.gif"], [":unsure:", "hmm.gif"], [":whatlove:", "ilu.gif"], [":lol:", "laughing.gif"], [":loveflac:", "loveflac.gif"], [":ninja:", "ninja.gif"], [":no:", "no.gif"], [":nod:", "nod.gif"], [":ohno:", "ohnoes.gif"], [":omg:", "omg.gif"], [":o", "ohshit.gif"], [":paddle:", "paddle.gif"], [":(", "sad.gif"], [":shifty:", "shifty.gif"], [":sick:", "sick.gif"], [":)", "smile.gif"], [":-)", "smile.gif"], [":sorry:", "sorry.gif"], [":thanks:", "thanks.gif"], [":P", "tongue.gif"], [":wave:", "wave.gif"], [":wink:", "wink.gif"], [":worried:", "worried.gif"], [":wtf:", "wtf.gif"], [":wub:", "wub.gif"]]
		},
		blueprints: {
			...
			gazelle_shout: [ // pretty standard gazelle blueprint as of ... 2012 or 2013?
				['b', 'i', 'u', 's'], ['important', 'heading', 'code'],
				['color', 'size'], ['gz_left', 'gz_center', 'gz_right'],
				['#', '*'], ['url', 'img'], ['quote', 'pre'], ['hide', 'mature'],
				['artist', 'torrent', 'user', 'wiki', 'gz_rule'], ['tex', 'plain'],
				[ 'erase'], ['settings', 'shortcut']
			]
		},
		shortcuts: {
			alt: {
		...
		delegate: {
			...
			settings: { // settings events
				...
				fn: {
					...
					link: function () { // toggles WhutBBCodeForkBeta? link
	...
	/**
	 * WhutBBCodeFork Settings Class
	 * Intended to be a singleton used within WhutBB.init()
	 *
	 * This class is used to store site configurations for WhutBBCodeForkBeta?
	 ...
	 *
	 *  link:
	 *    link to information about the site's BBCode or WhutBBCodeForkBeta? itself (default)
	 *
	...
	WhutBB.Panel = (function () {
		...
		Panel.construct = function () {
			Panel.factory('link', dom.dom('div', {className: 'wbblink' + (WhutBB.user.settings.link ? '' : ' wbbhide') },
				dom.dom('a', {href: WhutBB.config.link, title: 'Version r.' + update.version, txt: 'WhutBBCodeForkBeta?', target: '_blank'})), true);

			var classNamePanel = ""
			if (SHOUT_BOX)
				classNamePanel = "wbbcode wbbbuttons";
			else
				classNamePanel = "wbbbuttons";
			Panel.factory('button', dom.dom('div', {className: classNamePanel}), true);
