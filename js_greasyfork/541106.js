...
class WmeCommentIcons {
  constructor() {
    ...
    const defaults = {
      ...
      label: true,
      rules: {
        VAO_KDC: { legend: "Biển vào KDC", enable: true, regex: '^(?!.*\\b(ngo.i|ra|h.t|h.*c)\\b).*?(.*kdc|.*kdc|.*khu d.n c.|.*d.n c.|.*b.t ..u k|dc|n.i th.*|th.nh th.)' },
        HET_KDC: { legend: " Biển hết KDC", enable: true, regex: '^(?!.*\\b(v.o|b.t ..u|v.)\\b).*?(ngo.i.*|ra\\S.*k.*|ra n.*|h.t.*cư|h.*dc|h.t.*kdc|th.nh th.|h.t th.*)' },
        C_RT: { legend: "Biển cấm rẽ trái", enable: true, regex: '^(?!.*\\b(v..t)\\b)(bi.n|c.m|k.*g|cr).*?(tr.i|t)$' },
        C_RP: { legend: "Biển cấm rẽ phải", enable: true, regex: '^(bi.n|.*c.m|k.*g|cr).*?(ph.i|p)$' },
        C_QD: { legend: "Biển cấm quay đầu", enable: true, regex: '^(bi.n|.*c.m|k.*g)\W(quay ..u)$' }, //(biển, cấm, không) (quay đầu)
        C_RTQD: { legend: "Biển cấm rẽ trái & quay đầu", enable: true, regex: '^(bi.n|.*c.m) (r. tr.i|rt).* (quay ..u|qd)' },
        C_VUOT: { legend: "Biển cấm vượt", enable: true, regex: '^(bi.n|.*c.m|k.*g|c).*?(v..t)$' },
        HET_C_VUOT: { legend: "Biển hết cấm vượt", enable: true, regex: '^(h.t|bi.n|hc).*?(v..t)$' },
        C_OTO_RT_QD: { legend: "Biển cấm oto rẽ trái & quay đầu", enable: true, regex: '^(bi.n|.*c.m) .t. (r. tr.i|rt).* (quay ..u|qd)' },
        C_OTO_RT: { legend: "Biển cấm oto rẽ trái", enable: true, regex: '^(bi.n|.*c.m|k.*g|) .t. (r. tr.i|rt)$' },
        C_OTO_RP: { legend: "Biển cấm oto rẽ phải", enable: true, regex: '^(bi.n|.*c.m|k.*g|) .t. (r. ph.i|rp)$' },
        C_OTO: { legend: "Biển cấm oto", enable: true, regex: '^(bi.en|c.m) (.t.)$' },
        CAMERA_TOC_DO: { legend: "Camera tốc độ", enable: true, regex: '^(cam.* t.c ..|speed.*|cam.*speed)$' },
        CAMERA_DEN_DO: { legend: "Camera đèn đỏ", enable: true, regex: '^(cam.* ph.t ngu.i|cam.* ..n ..|..n ..|traffic light)$' },
        LAN_OTO: { legend: "Biển đường dành oto", enable: true, regex: '^(...ng|l.n).*(oto|ôtô)$' }, //(làn, đường).*(oto)
        LAN_XEMAY: { legend: "Biển làn dành xe máy", enable: true, regex: '^(l.n|...ng).*(m.y)$' }, // (làn, đưởng).*(máy)
        HL_DT: { legend: "Biển hiệu lệnh đi thẳng", enable: true, regex: '^(.i).*(th.ng)' }, // đi thẳng
        HL_RP: { legend: "Biển hiệu lệnh rẽ phải", enable: true, regex: '^(r.).*(ph.i)' },  // rẽ phải
        HL_RT: { legend: "Biển hiệu lệnh đi trái", enable: true, regex: '^(r.).*(tr.i)' },  // rẽ trái
        HET_MOI_LENH_C: { legend: "Biển hết mọi lệnh cấm", enable: true, regex: '^(h.t|bi.n).*?(bi.n|c.m|l.c|t.m th.i|l..h|h.n)$' },
        GH_TOC_DO: { legend: "Biển giới hạn tốc độ", enable: true, regex: '^(?!.*\\b(h.t|n.a)\\b).*?(\\d{2,3})' },
        HET_CAM_TOC_DO: { legend: "Biển hết cấm giới hạn tốc độ", enable: true, regex: '^(?:h.t|h.t bi.n)\\s+.*?(\\d+)' },
      }
    };
    this.options = Object.assign(defaults, JSON.parse(localStorage.getItem(this.storageKey) || '{}'));
  ...
  async updateDisplay() {
    ...
    try {
      ...
      for (const comment of mapComments) {
        ...
        // Offset ngẫu nhiên áp dụng cho vị trí cơ sở
        const { iconSize, opacity, offsetRange, isSubsign, label } = this.options;
        const baseOffsetX = Math.floor(Math.random() * (offsetRange * 2 + 1)) - offsetRange;
        const baseOffsetY = Math.floor(Math.random() * (offsetRange * 2 + 1)) - offsetRange;
        const feature = new OpenLayers.Feature.Vector(pointGeometry);
        if (iconSvg) {
          const base64Svg = this.utf8ToBase64(iconSvg);

          // Đảm bảo Feature chính được gán style đầy đủ
          feature.style = {
            graphic: true,
            externalGraphic: 'data:image/svg+xml;base64,' + base64Svg,
            graphicHeight: iconSize, // Kích thước cố định cho icon chính
            graphicWidth: iconSize,
            graphicXOffset: baseOffsetX,
            graphicYOffset: baseOffsetY,
            graphicOpacity: opacity,
            // Có thể thêm label nếu cần:
        ...
        if (isSubsign) {
          ...
          if (textSubsign) {
            // ƯỚC TÍNH CHIỀU RỘNG DỰA TRÊN ĐỘ DÀI TEXT
            const CHAR_UNIT_WIDTH = 10; // Đơn vị viewBox ước tính cho 1 ký tự
            const PADDING_UNITS = 25; // Tổng padding

            let widthUnits = PADDING_UNITS + (textSubsign.length * CHAR_UNIT_WIDTH);
            widthUnits = Math.max(30, widthUnits); // Đảm bảo chiều rộng tối thiểu là 80 units

            const SUB_SIGN_VIEWBOX_HEIGHT = 50;
            const SUB_SIGN_PIXEL_HEIGHT = 30; // Biển phụ cùng chiều cao với biển chính

            // Tính toán graphic width dựa trên tỷ lệ chiều cao
            const graphicWidth = Math.round(widthUnits * (SUB_SIGN_PIXEL_HEIGHT / SUB_SIGN_VIEWBOX_HEIGHT));
            const subSignSvg = this.subSignIcon(textSubsign, widthUnits);
            const base64SubSignSvg = this.utf8ToBase64(subSignSvg);

            // Điều chỉnh offset: đặt biển phụ dịch sang phải 35px so với vị trí cơ sở
            featureSub.style = {
              graphic: true,
              externalGraphic: 'data:image/svg+xml;base64,' + base64SubSignSvg,
              graphicWidth: graphicWidth,
              graphicHeight: SUB_SIGN_PIXEL_HEIGHT,
              graphicXOffset: baseOffsetX - 15,
              graphicYOffset: baseOffsetY + 30,
              graphicOpacity: 0.9,
            };
  ...
  createConfigRow(label, type, key, subKey = null) {
    const container = this.createElem('div', { style: 'margin-bottom: 8px; display: flex; align-items: center; justify-content: space-between;' });
    ...
    input.addEventListener('change', (e) => {
      const newV = type === 'checkbox' ? e.target.checked : (type === 'number' ? parseFloat(e.target.value) : e.target.value);
      ...
      // this.arrayRegex = this.buildCombinedRegex(); // Re-build regex ngay lập tức
      // this.updateDisplay();
      this.applySettings();
    });

    container.append(this.createElem('span', { textContent: label, style: 'font-size: 11px;' }), input);
    return container;
  }
  createElem(type = '', attrs = {}, eventListener = []) {
    const el = this._elems[type]?.cloneNode(false) || this._elems.div.cloneNode(false),
  ...
  async tabUI() {
    ...
    this.wmeSdk.Sidebar.registerScriptTab()
      .then(({ tabLabel, tabPane }) => {
        ...
        docFrags.appendChild(this.createElem('span', { class: 'WMECI-spanTitle', textContent: this.scriptName }));
        docFrags.appendChild(this.createElem('span', { class: 'WMECI-spanVersion', textContent: this.version }));
        let liElem = this.createElem('li', { class: 'active' });
        liElem.appendChild(this.createElem('a', {
          'data-toggle': 'tab', href: '#panel-wmeci-configs', 'aria-expanded': true, textContent: 'Configs'
        }));
        const ulElem = this.createElem('ul', { class: 'nav nav-tabs' });
        ulElem.appendChild(liElem);
        liElem = this.createElem('li');
        liElem.appendChild(this.createElem('a', {
          'data-toggle': 'tab', href: '#panel-wmeci-settings', 'aria-expanded': true, textContent: 'Settings'
        }));
        ulElem.appendChild(liElem);
        liElem = this.createElem('li');
        liElem.appendChild(this.createElem('a', {
          'data-toggle': 'tab', href: '#panel-wmeci-others', 'aria-expanded': true, textContent: 'Others'
        }));
        ulElem.appendChild(liElem);

        let divElemRoot = this.createElem('div', { class: 'WmeCommentIcons-navTabs' });
        divElemRoot.appendChild(ulElem);
        docFrags.appendChild(divElemRoot);
        divElemRoot = this.createElem('div', { class: 'tab-content WmeCommentIcons-divTabs', style: '--height-offset:0px;' });
        divElemRoot.appendChild(this.createElem('div', { id: 'panel-wmeci-configs', class: 'tab-pane active' }));
        divElemRoot.appendChild(this.createElem('div', { id: 'panel-wmeci-settings', class: 'tab-pane' }));
        divElemRoot.appendChild(this.createElem('div', { id: 'panel-wmeci-others', class: 'tab-pane' }));
        docFrags.appendChild(divElemRoot);
        ...
        configPanel.append(
          ...
          this.createElem('hr'),
          this.createElem('p', { textContent: 'Regex Rules:', style: 'font-weight: bold' })
        );
        Object.entries(this.options.rules).forEach(([ruleKey, ruleData]) => {
          configPanel.append(
            this.createConfigRow(ruleData.legend, 'text', 'regex', ruleKey)
          );
        });
        const settingsPanel = divElemRoot.querySelector('#panel-wmeci-settings');
        settingsPanel.append(
          this.createConfigRow('Bật biển phụ', 'checkbox', 'isSubsign'),
          this.createConfigRow('Bật tên', 'checkbox', 'label'),
        );
        Object.entries(this.options.rules).forEach(([key, data]) => {
          settingsPanel.append(this.createConfigRow(data.legend, 'checkbox', 'enable', key));
        });
        tabPane.appendChild(docFrags);
        tabPane.id = 'sidepanel-wme-ci';
        W.userscripts.waitForElementConnected(tabPane);
        Object.assign(document.getElementById('sidepanel-wme-ci').parentElement.style, { width: 'auto', padding: '0 15px' });
        this.updateDisplay();
      })
      .catch((error) => {
  ...
  applySettings() {
    ...
    // 2. Build lại regex mới dựa trên options vừa sửa
    this.arrayRegex = this.buildCombinedRegex();

    // 3. Cập nhật lại giao diện bản đồ ngay lập tức
    this.updateDisplay();
  }
  findTopLayer() {
  ...
  subSignRegex(subject) {
    ...
    if (match) {
      return match[1] // get inside bracket
    }
  }
  ...
  buildCombinedRegex() {
    ...
    // Ánh xạ icon gốc (để lại logic chọn icon vì nó chứa SVG phức tạp)
    const iconLibrary = {
      VAO_KDC: () => this.residentialZoneIcon(false),
      HET_KDC: () => this.residentialZoneIcon(true),
      C_RT: () => this.prohibitTurnIcon('left'),
      C_RP: () => this.prohibitTurnIcon('right'),
      C_QD: () => this.prohibitTurnIcon('u-turn'),
      C_RTQD: () => this.prohibitIcon('left-uturn'),
      C_VUOT: () => this.prohibitIcon('cam_vuot'),
      HET_C_VUOT: () => this.prohibitIcon('het_cam_vuot'),
      C_OTO_RT_QD: () => this.prohibitTurnIcon('oto-left-uturn'),
      C_OTO_RT: () => this.prohibitTurnIcon('oto-left'),
      C_OTO_RP: () => this.prohibitTurnIcon('oto-right'),
      C_OTO: () => this.prohibitIcon('oto'),
      CAMERA_TOC_DO: () => this.cameraIcon('speed'),
      LAN_OTO: () => this.mandatoryIcon('only-oto'),
      LAN_XEMAY: () => this.mandatoryIcon('only-moto'),
      CAMERA_DEN_DO: () => this.cameraIcon('red_light'),
      HL_DT: () => this.mandatoryIcon('straight'),
      HL_RP: () => this.mandatoryIcon('right'),
      HL_RT: () => this.mandatoryIcon('left'),
      HET_MOI_LENH_C: () => this.createEndOfProhibitionsIcon(),
      HET_CAM_TOC_DO: (m) => this.speedIcon(m?.[1] || "??", true),
      GH_TOC_DO: (m) => this.speedIcon(m?.[2] || "??", false)
    }

    Object.entries(this.options.rules).forEach(([name, config]) => {
      if (!config.enable) return;
      this.ruleMap.set(name, { ...config, icon: iconLibrary[name], regexObj: new RegExp(config.regex, 'ims') });
      allRegexParts.push(`(?<${name}>${config.regex})`);
    });

    return new RegExp(`^(${allRegexParts.join('|')})`, 'ims');
  }
  getCommentIcon(subject) {
    ...
    // Duyệt theo thứ tự đã định nghĩa trong options.rules
    for (const ruleName of Object.keys(this.options.rules)) {
      if (match.groups[ruleName] !== undefined) {
        const rule = this.ruleMap.get(ruleName);
        if (rule) {
          const localMatch = match.groups[ruleName].match(rule.regexObj);
          if (!localMatch) continue; // Thử rule tiếp theo
  ...
  subSignIcon(content, widthUnits) {
    return `<svg width="${widthUnits}" height="50" viewBox="0 0 ${widthUnits} 50" xmlns="http://www.w3.org/2000/svg"><rect style="fill:#fff;fill-opacity:1;stroke:#000;stroke-width:2.89086;stroke-linecap:square;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1;paint-order:fill markers stroke" width="${widthUnits - (widthUnits * 0.05)}" height="47.109" x="1.445" y="1.445" ry="5.419"/><text xml:space="preserve" style="font-style:normal;font-variant:normal;font-weight:700;font-stretch:normal;font-size:20;line-height:2;letter-spacing:.326827px;fill:#000;fill-opacity:1;stroke:none;stroke-width:1.23525;stroke-linecap:square;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1;paint-order:fill markers stroke" x="7.665" y="31.348"><tspan style="fill:#000;fill-opacity:1;stroke:none;stroke-width:1.23525" x="7.665" y="31.348" font-family="Arial" font-weight="bold">${content}</tspan></text></svg>`
  }
