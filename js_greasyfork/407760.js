...
// @grant        GM_setValue
// @grant        GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/407760/%E5%8D%A1%E7%89%B9%E4%BB%8B%E9%9D%A2%E7%BE%8E%E5%8C%96Plus.user.js
// @updateURL https://update.greasyfork.org/scripts/407760/%E5%8D%A1%E7%89%B9%E4%BB%8B%E9%9D%A2%E7%BE%8E%E5%8C%96Plus.meta.js
// ==/UserScript==

"use strict";

const DefaultHeroColor = [28, 93, 153];
...
var CurrentHeroColor = [28, 93, 153];
...
function currentBodyClass(e) {
    if (document.getElementById("app"))
        return document.getElementById("app").classList.contains(e)
    else
        return false;
}
...
function PageCheck() {
    ...
    if (currentBodyClass("App--index") || currentBodyClass("App--discussion")) {
        if (LoadedFunctionSetting["DiscussionLimit"]) DiscussionListLimit();
    }

    if (currentBodyClass("App--discussion") || currentBodyClass("App--user")) {
    ...
    if (currentBodyClass("App--discussion")) {
...
function ColoredMention() {
    ...
    document.querySelectorAll(".CommentPost:not(.editing) .UserMention:not(.Colored)").forEach(function (usermention) {
        let username = usermention.childNodes.length > 1 ? Array.from(usermention.childNodes).map(x => x.nodeName == "IMG" ? x.alt : x.textContent).join("").slice(1) : usermention.textContent.slice(1);
        GetUserColor(username).then(x => {
...
//現在顯示的是displayName
async function GetUserColor(displayName) {
    let userdata = Object.values(app.store.data.users).find(x => x.data.attributes.displayName === displayName);
...
function DiscussionListLimit() {
    ...
    if (loadMore) {
        ...
        if (app.previous && (currentBodyClass("App--index") && !(app.previous.data.routeName && app.previous.data.routeName.includes("discussion")))) {
            OffsetReset();
        }
...
function LoadDiscussions() {
    if (currentBodyClass("App--index"))
        window.scrollTo(0, 0);
    app.discussions.discussions = [];
    app.discussions.loading = true;
    m.redraw();
    app.discussions.loadResults(DiscussionOffset * 20).then(function (x) {
        //如果載入沒東西就重整跳回第一頁
        if (x.payload.data.length == 0) {
            console.log("道生一，一生二，二生三，三生萬物");
            app.discussions.loading = false;
            OffsetReset();
            PageOffsetUpdate();
            app.discussions.refresh();
            return;
        }
        app.discussions.parseResults(x);
...
function AddUsernameHistoryButton() {
    ...
    if (postuser.length > 0) {
        postuser.forEach(function (val) {
            ...
            let user = Object.values(app.store.data.users).find(x => x.data.attributes.username === userid);
            if (user && user.data.attributes.usernameHistory) {
                let historyNames = user.data.attributes.usernameHistory;
                //如果3天內有改名的會標記訊息
                ...
                val.querySelector(".username-history").addEventListener("click", function (e) {
                    ...
                    if (!dropdown.classList.contains("Loaded")) {
                        let currentUsername = user.data.attributes.username;
...
function PreviewCheck(flag) {
    ...
    if (flag) {
        ...
        PreviewChecker = setInterval(function () {
            ...
            if (app.composer.editor && previewpostbody && PreviewSource !== app.composer.editor.el.value) {
                PreviewSource = app.composer.editor.el.value;
...
function AddSettingButton() {
    if (document.querySelector("#header-secondary .Header-controls") && !document.getElementById("item-customstyle")) {
        ...
        if (document.querySelector("#header-secondary .Header-controls .item-logIn"))
            document.querySelector("#header-secondary .Header-controls .item-logIn").insertAdjacentHTML("afterend", ItemCustomStyle);
        else
            document.querySelector("#header-secondary .Header-controls .item-session").insertAdjacentHTML("afterend", ItemCustomStyle);
