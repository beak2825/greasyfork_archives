// ==UserScript==
// @name         Zed汉化 & ZedTools & Edit by Owen
...
// @downloadURL https://update.greasyfork.org/scripts/560407/Zed%E6%B1%89%E5%8C%96%20%20ZedTools%20%20Edit%20by%20Owen.user.js
// @updateURL https://update.greasyfork.org/scripts/560407/Zed%E6%B1%89%E5%8C%96%20%20ZedTools%20%20Edit%20by%20Owen.meta.js
// ==/UserScript==
...
/*Start Owen 调整经验显示 */
        if (levelElem && !insertElem) {
            levelElem.insertAdjacentHTML(
                "beforeend",
                `<div id="script_player_level" class="script_do_not_translate"><span id="script_player_level_inner" class="script_do_not_translate"><strong class="script_do_not_translate">
                ${Math.floor(playerXp)} / ${Math.floor(currentLevelMaxXP)} (${Math.floor(currentLevelMaxXP)-Math.floor(playerXp)})
                </strong></span></div>`
            );
        } else if (levelElem && insertElem) {
            insertElem.querySelector("#script_player_level_inner").innerHTML = `<strong class="script_do_not_translate">
            ${Math.floor(playerXp)} / ${Math.floor(currentLevelMaxXP)} (${Math.floor(currentLevelMaxXP)-Math.floor(playerXp)})
            </strong>`;
        }
/*End Owen 调整经验显示 */
        ...
        if (levelElem?.parentElement?.parentElement?.parentElement && !insertElem) {
            levelElem?.parentElement?.parentElement?.parentElement.insertAdjacentHTML(
                "beforeend",
                `<div id="script_countdowns_container" class="script_do_not_translate" style="display: flex; align-items: center; justify-content: center; gap: 12px;"></div>
 ...
 /*Start Owen 添加市场 */
        logoElem = document.body.querySelector("#script_market_logo");
        if (!logoElem) {
            insertToElem.insertAdjacentHTML(
                "beforeend",
                `<div id="script_market_logo" style="order: 2; cursor: pointer; width:30px" class="script_do_not_translate"><a class="script_do_not_translate" style="font-size: 12px; text-decoration: none;" href="#" >市场</a></div>`
            );
...
/*End Owen 远征图开门*/
    ...
    function updateFuelOpenDoorDisplay() {
        //const insertToElemFuel = document.body.querySelector("#script_countdowns_container_3");
    ...
    */
    ...
    function hideItems() {
        if (window.location.href.includes("zed.city/store/junk") &&
            ...
            document.querySelectorAll(".q-item").forEach((item) => {
                ...
                // ----------- 保留永遠要顯示的項目（不受互相影響） -----------
                const alwaysShow = ["Nails", "Unrefined Plastic"];
    ...
    const dictsEquip = {
        ...
        "Sweat Pants": "运动裤",
        "Swim Shorts": "游泳短裤",
        "Heavy Reinforced Legs": "重型加固护腿",
        "Kevlar Pants": "凯夫拉纤维长裤",
...
/*Start Owen 资源相关字典加英文*/
    ...
    const dictEnhance = {
        ...
        "Booster (Medical)": "增强剂（医疗）(Booster (Medical))",
        "Booster (Medical": "增强剂（医疗）(Booster (Medical)",
        "Boosters (Medical": "增强剂（医疗）(Boosters (Medical)",
        "Booster (Energy Drink": "增强剂（能量饮料）(Booster (Energy Drink)",
        "Boosters (Energy Drink)": "增强剂（能量饮料）(Boosters (Energy Drink))",
        "Boosters (energy": "增强剂（能量饮料）(Boosters (energy)",
        "Booster (Special": "增强剂（特殊）(Booster (Special)",
        "Booster (Easter": "增强剂（复活节）(Booster (Easter)",
        "Booster (Food": "增强剂（食物）(Booster (Food)",
        "Booster (Food)": "增强剂（食物）(Booster (Food))",
        "Booster (Alcohol": "增强剂（酒精）(Booster (Alcohol)",
        "Boosters (Alcohol)": "增强剂（酒精）(Boosters (Alcohol))",
        "Boosters (Alcohol": "增强剂（酒精）(Boosters (Alcohol)",
        "Booster (Fish": "增强剂（鱼类）(Booster (Fish)",
...
/*End Owen 资源相关字典加英文*/
    ...
    const dictMission = {
        ...
        "Objective: Find all the grub in the Abandoned Cabin Cellar": "任务目标：在（拾荒）废弃小屋的地窖里找到所有的食物",
        "":"",
        "":"",
    ...
    function dict(oriText, ignoreUnmatchDueToBeingPossiblePlayerID = false,node) {
        ...
        */
        ...
        if (/^([\w\s-']+) bought (\d+)x ([\w\s-']+) and you gained \$(\d+), your market listing has sold (?:(\d+)\/(\d+)|out)$/.test(text)) {
            let res = /^([\w\s-']+) bought (\d+)x ([\w\s-']+) and you gained \$(\d+), your market listing has sold (?:(\d+)\/(\d+)|out)$/.exec(text);
            // 精准匹配翻译：部分售出显示 "数字/数字"，售罄显示 "已售罄"
            let soldPart = res[5] && res[6] ? `${res[5]} / ${res[6]}` : "已售罄";
            return `${res[1]} 购买了 ${res[2]}x ${dict(res[3])}，你获得了 $${res[4]}，你的市場上架${soldPart}`;
        }
