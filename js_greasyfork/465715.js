// ==UserScript==
// @name         [Bilibili] 关注管理器 (0.2.20.添加排序版本)
// @namespace    ckylin-bilibili-foman-special1
// @version      0.2.20
...
// @downloadURL https://update.greasyfork.org/scripts/465715/%5BBilibili%5D%20%E5%85%B3%E6%B3%A8%E7%AE%A1%E7%90%86%E5%99%A8%20%280220%E6%B7%BB%E5%8A%A0%E6%8E%92%E5%BA%8F%E7%89%88%E6%9C%AC%29.user.js
// @updateURL https://update.greasyfork.org/scripts/465715/%5BBilibili%5D%20%E5%85%B3%E6%B3%A8%E7%AE%A1%E7%90%86%E5%99%A8%20%280220%E6%B7%BB%E5%8A%A0%E6%8E%92%E5%BA%8F%E7%89%88%E6%9C%AC%29.meta.js
// ==/UserScript==
(function () {
    'use strict';
    const s = {
                                ...
                                openModal("更多...", await makeDom("div", async select => {
                                    ...
                                    [
                                        ...
                                        await makeDom("button", btn => {
                                            ...
                                            refreshChecked();
                                            if (datas.checked.length > 0)
                                                btn.innerHTML = "导出所有被选中的UP的当前缓存数据(最早->最新)..."
                                            else
                                                btn.innerHTML = "导出所有关注的UP的当前缓存数据(最早->最新)...";
                                            btn.onclick = async e => {
                                                    let list;
                                                    if (datas.checked.length > 0)
                                                        list = datas.checked;//listdom;
                                                    ...
                                                    let infoList = list.map(it=>mapToObj(it)).filter(it=>!!it).sort((a,b) =>a.mtime - b.mtime);
                                                    let copyList = JSON.stringify(infoList);
                                                    let mtitle = "";
                                                    if(await copy(copyList)){
                                                        mtitle+="✅ 内容已经自动复制到剪贴板, 你可以粘贴到别处";
                                        ...
                                        await makeDom("button", btn => {
                                            ...
                                            refreshChecked();
                                            if (datas.checked.length > 0)
                                                btn.innerHTML = "导出所有被选中的UP的当前缓存数据(最新->最早)..."
                                            else
                                                btn.innerHTML = "导出所有关注的UP的当前缓存数据(最新->最早)...";
                                            btn.onclick = async e => {
                                                    let list;
                                                    if (datas.checked.length > 0)
                                                        list = datas.checked;//listdom;
                                                    ...
                                                    let infoList = list.map(it=>mapToObj(it)).filter(it=>!!it).sort((a,b) =>b.mtime - a.mtime);
                                                    let copyList = JSON.stringify(infoList);
                                                    let mtitle = "";
                                                    if(await copy(copyList)){
                                                        mtitle+="✅ 内容已经自动复制到剪贴板, 你可以粘贴到别处";
                                        ...
                                        await makeDom("button", btn => {
                                            ...
                                            refreshChecked();
                                            if (datas.checked.length > 0)
                                                btn.innerHTML = "导出所有被选中的UP的基础数据(最新->最早)..."
                                            else
                                                btn.innerHTML = "导出所有关注的UP的基础数据(最新->最早)...";
                                            btn.onclick = async e => {
                                                    let list;
                                                    if (datas.checked.length > 0)
                                                        list = datas.checked;//listdom;
                                                    ...
                                                    const mapToObj = (uid) => {
                                                        try {
                                                            console.log(1001,{uid})
                                                            if (datas.mappings.hasOwnProperty(+uid)) {
                                                                const { mid,uname,mtime,sign, tag} = datas.mappings[+uid];
                                                                let tags = tag?.map(t => datas.tags[t]?.name ?? null).filter(t => !!t);
                                                                let regdate = new Date((+mtime) * 1000);
                                                                let followDateText = `${regdate.getFullYear()}年${regdate.getMonth() + 1}月${regdate.getDate()}日`;
                                                                return { mid,uname,mtime,follow:followDateText,sign, tag, tag: tags ?? [] };
                                                            } else return null;
                                                        } catch (err) {
                                                            console.error('e!!',err);
                                                            return null;
                                        ...
                                        await makeDom("button", btn => {
                                            ...
                                            refreshChecked();
                                            if (datas.checked.length > 0)
                                                btn.innerHTML = "导出所有被选中的UP的基础数据(最早->最新)..."
                                            else
                                                btn.innerHTML = "导出所有关注的UP的基础数据(最早->最新)...";
                                            btn.onclick = async e => {
                                                    let list;
                                                    if (datas.checked.length > 0)
                                                        list = datas.checked;//listdom;
