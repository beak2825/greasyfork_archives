...
// @downloadURL https://update.greasyfork.org/scripts/370424/Ch%E1%BA%A7uL%C3%B4n%20Script.user.js
// @updateURL https://update.greasyfork.org/scripts/370424/Ch%E1%BA%A7uL%C3%B4n%20Script.meta.js
// ==/UserScript==
...
/* VI */	["Hậu vệ Trung tâm", "Hậu vệ Trái", "Hậu vệ Phải", "Tiền vệ Phòng ngự Trung tâm", "Tiền vệ Phòng ngự Trái", "Tiền vệ Phòng ngự Phải", "Tiền vệ Trung tâm", "Tiền vệ Trái", "Tiền vệ Phải", "Tiền vệ Tấn công Trung tâm", "Tiền vệ Tấn công Trái", "Tiền vệ Tấn công Phải", "Tiền đạo", "Thủ môn"],
];
...
if (location.href.indexOf("/players/") != -1){
    ...
    if (!retiring)
    {
			...
			if (year >= 18)
      {
        var ageD = 25 / (year * 1 + (month / 12));
          if (!isGK) {
            Result = addCommas(Math.round(si * 500 * Math.pow(ageD, 2.5)));
          } else {
            Result = addCommas(Math.round(si * 0.75 * 500 * Math.pow(ageD, 2.5)));
          }
      }
    }
    return Result;
  ...
  function CalcAgeMonths(age) {
    ...
    var isGK = positionArray[0] == "Goalkeeper";
  ...
  var RetireStatus = gettr[7].getElementsByTagName("td")[0].innerHTML.indexOf("retire.gif") > 0;
	...
	rectBankPrcV.innerHTML = CalcBankPrice(AgeMonths, SI, RetireStatus, isGK);
  ...
  if (RetireStatus == true)
	...
	function DoCalcNewASI() {
		...
		if (posIdx.value != 13) {
		  ...
		  FinalBP.innerHTML = CalcBankPrice(AgeNow.value*1 + NumTr.value*1, FinSi);
        } else {
          FinSi = Math.pow(AllSkills / 11 * 14, 7) /(Math.pow(2, 9) * Math.pow(5, 4) * Math.pow(7, 7));
		  FinTi.innerHTML = addCommas(Math.round(FinSi));
		  FinalBP.innerHTML = CalcBankPrice(AgeNow.value*1 + NumTr.value*1, FinSi, false, true);
        }
