// ==UserScript==
// @name        TinyChatScript Extra Features 12_14_23
// @version     1.8.57.5
...
//              jshint esversion: 6
// @namespace https://greasyfork.org/users/1234068
// @downloadURL https://update.greasyfork.org/scripts/482268/TinyChatScript%20Extra%20Features%2012_14_23.user.js
// @updateURL https://update.greasyfork.org/scripts/482268/TinyChatScript%20Extra%20Features%2012_14_23.meta.js
// ==/UserScript==
...
(function() {
    ...
    function BotCommandCheck() {
        ...
        if (isCommand(arguments[1])) {
            ...
            } else if (arguments[1].match(/^!wordle$|^!wordlehelp$|^!wordlereset$|^!guess /i) && CTS.UserList[arguments[0]].canGame && CTS.CanHostWordleGames) {
    ...
    function WordleCommandCheck() {
        ...
        if (arguments[1].match(/^!wordle$/i)) {
            if (newPlayer.length === 0) {
                ...
                Send("msg", `[WORDLE]\n\nPlease wait for ${newPlayer[2]} to finish their game\n\nAttempts Remaining: ${attemptsRemaining}`);
            }
        ...
        } else if (arguments[1].match(/^!guess /i)) {
            if (user.handle === newPlayer[0] && attemptsRemaining > 0) {
                ...
                // Check if the guessed word has exactly 5 letters
                    ...
                    } else if (arguments[1].match(/^!guess [a-z]+$/i)){
                        Send("msg", "[WORDLE]\n\nTry again, not a 5 letter word.");
                    }
                } else if (user.handle !== newPlayer[0]) {
                Send("msg", `[WORDLE]\n\nPlease wait for ${newPlayer[2]}, to finish their game\n\nAttempts Remaining: ${attemptsRemaining}`);
            }
        } else if ((CTS.Host === user.handle || (CTS.BotModList.includes(user.username) && user.mod)) && arguments[1].match(/^!wordlereset$/i)) {
            WordleReset();
        }
    }
                ...
                if (!isCommand(arguments[1])) {
                    ...
                    if (arguments[1].length == 1 && CTS.Game.Trivia.ANum.includes(arguments[1].toUpperCase()) && !Guessed && !CTS.Game.Trivia.Waiting) {
                        if (CTS.Game.Trivia.Correct === arguments[1].toUpperCase()) {
                            ...
                            //set wrong guess value.  subtracts 1/3 value for easy, medium and 1/4 for hard questions.
                            const wrongGuessValue = CTS.Game.Trivia.Worth >= 70 ? Math.ceil(CTS.Game.Trivia.Worth / 4) : Math.ceil(CTS.Game.Trivia.Worth / 3);
                            // Save Progress
    ...
    async function WordleGenerateList(callback) {
        try {
            // Fetch the word list
            // downloads valid wordle attempt list and uses it verify guess.
    ...
    function attemptResults() {
        ...
        if (wordleAttempt === wordleAnswer) {
            const winnerMessage = `${CTS.Game.Wordle.ProgressMsg}${CTS.Game.Wordle.ActivePlayer[2].substr(0, 12)}, WINNER!\n`;
    ...
    function LineSpam() {
        ...
        if ((LineBreaks >= 10 || arguments[0].match(/\n\n\n\n/g) || arguments[0].length > 250) && arguments[1] === false) return true;
        return false;
    }
    ...
    function CheckUserWordAbuse() {
        if (CTS.UserList[arguments[0]].handle != CTS.Me.handle && !CTS.UserList[arguments[0]].mod) {
            ...
            const keywordTargetNick = CTS.UserList[arguments[0]].nick 
            const keywordTargetUser = CTS.UserList[arguments[0]].username
            for (var i = 0; i < len; i++) {
    ...
    function FishUpgradeStatus() {
        ...
        if (arguments[1] != 6) msg += `Balance: $${arguments[0].Points}`;
        Send("msg", msg);
    }
    ...
    var CommandList = {
        ...
        undo: function() {
            const userBanListNumber = CTS.UserBanList.length - 1;
        ...
        namealert: function() {
            CTS.NameAlert = !CTS.NameAlert;
            Save("NameAlert", JSON.stringify(CTS.NameAlert));
            Alert(GetActiveChat(), "Command Accepted!\n" + ((CTS.NameAlert) ? "NAMEALERT IS NOW ON!" : "NAMEALERT IS NOW OFF!"));
        },
        clr: function() {
            CTS.Message[GetActiveChat()] = [];
    ...
    var ServerInList = {
        ...
        nick: function() {
            ...
            var isMod = CTS.UserList[user].mod;
            var isSafe = isSafeListed(arguments[0].username);
            ...
            if (user != -1) {
                ...
                if ((CTS.Host == CTS.Me.handle) && CTS.NameAlert) {
                    //prevent automated name change bot message from including tinychat|room set auto-kick words.
                    //does not check or post alert for mods
                    if (!isMod && !isSafe) {
                            if (arguments[0].nick.match(/^stumblechat$|nigg|n1gg|3_b|nazi/i) || CTS.UserList[user].username.match(/^stumblechat$|nigg|n1gg|3_b|nazi/i)) {
                                Alert(GetActiveChat(), `✓ Banned for bad name.\nNick: ${arguments[0].nick}\nUser: ${CTS.UserList[user].username}`);
                                Send("ban", arguments[0].handle);
                            } else if (arguments[0].nick.toUpperCase() !== CTS.UserList[user].username) {
                                Send("msg", "⚠️[NAME CHANGE]⚠️\nNick: " + arguments[0].nick + "\nUser: " + CTS.UserList[user].username);
                            }
                        }
                    }
    ...
    const Fish = {
        ...
        GetPlayer() {
            const len = CTS.Game.Fish.Player.length;
            if (arguments[0] !== undefined) {
                for (const player of CTS.Game.Fish.Player) {
        ...
        Winner() {
            ...
            const len = CTS.Game.Fish.Player.length - 1;
        ...
        PriceList() {
            const { Upgrades } = arguments[0];
            const netPrice = 1000 * Upgrades.Net ** 3;
            const radarPrice = 1000 * Upgrades.Radar * 2 + 3500;
            const storePrice = Upgrades.Store ** 2 * 25000;
            const insurancePrice = 20000;
            const robPrice = 15000; // default 10000
            const slapPrice = 150000; // default 50000
            const gamblePrice = 5000; // default 1000
                    ...
                    if (CTS.Game.Fish.Round < 10) {
                        ...
                        for (let cast = 0; cast <= 2; cast++) {
                            ...
                            if (Rand(0, 100) <= Rand(10, 70)) {
                                const net = Rand(1, CTS.Game.Fish.Player[id].Upgrades.Net);
    ...
    const FishList = {
        ...
        fishrob() {
            if (arguments[0] !== -1 && FishTimerCheck(arguments[0])) {
                const canEliminate = Fish.GetPlayer(UsernameToHandle(arguments[1].toUpperCase()), false, true);
            ...
            if (arguments[0] !== -1 && FishTimerCheck(arguments[0])) {
                if (FishTransaction(arguments[0], Fish.PriceList(arguments[0], 6))) {
                    ...
                    if (Rand(1, 10) === 7) { // 10% chance
                        winnings = Rand(5000, 25000);
                    } else if (Rand(1, 7) === 4) { // 15%
                        winnings = Rand(5000, 15000);
                    }
                    if (winnings) {
                        arguments[0].Points += winnings;
                        Send("msg", `[FISHING BOAT]\n${arguments[0].Nickname}, won $${winnings}\nBalance: $${arguments[0].Points}`);
                    } else {
                        Send("msg", `[FISHING BOAT]\n${arguments[0].Nickname}, lost $5000!\nBalance: $${arguments[0].Points}`);
                    }
                }
            }
        },
        ...
        fishslap() {
            if (arguments[0] !== -1 && FishTimerCheck(arguments[0])) {
                const user = UsernameToUser(arguments[1].toUpperCase());
        ...
        fishupgradenet() {
            if (arguments[0] !== -1 && FishTimerCheck(arguments[0])) {
                if (arguments[0].Upgrades.Net >= 10) {
                    Send("msg", `${arguments[0].Nickname}, you own all net upgrades.`);
                } else {
        ...
        fishupgraderadar() {
            if (arguments[0] !== -1 && FishTimerCheck(arguments[0])) {
                if (arguments[0].Upgrades.Radar >= 20) {
                    Send("msg", `${arguments[0].Nickname}, you own all radar upgrades.`);
                } else {
                    if (FishTransaction(arguments[0], Fish.PriceList(arguments[0], 1))) {
                        arguments[0].Upgrades.Radar += 3;
        ...
        fishupgradeshop() {
            if (arguments[0] !== -1 && FishTimerCheck(arguments[0])) {
                if (arguments[0].Upgrades.Store >= 6) {
                    Send("msg", `${arguments[0].Nickname}, you own all shop upgrades.`);
                } else {
        ...
        fishupgradeinsurance() {
            if (arguments[0] !== -1 && FishTimerCheck(arguments[0])) {
                if (arguments[0].Upgrades.Insurance === true) {
                    Send("msg", `${arguments[0].Nickname}, you already own insurance.`);
                } else {
    ...
    function SetLocalValues() {
        if (CTS.StorageSupport) {
            ...
            CTS.NameAlert = JSON.parse(Load("NameAlert", JSON.stringify(false)));
