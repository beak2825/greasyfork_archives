// ==UserScript==
// @name       Japscan Loader
// @description Lecteur de manga specifique a Japscan (derivée du lecteur de Fuzetsu)
// @match *://www.japscan.co/lecture-en-ligne/*
// @version 0.0.1.20190914160852
// @namespace https://greasyfork.org/users/196270
// @downloadURL https://update.greasyfork.org/scripts/390069/Japscan%20Loader.user.js
// @updateURL https://update.greasyfork.org/scripts/390069/Japscan%20Loader.meta.js
// ==/UserScript==

// short reference to unsafeWindow (or window if unsafeWindow is unavailable e.g. bookmarklet)
var W = (typeof unsafeWindow === 'undefined') ? window : unsafeWindow;

var scriptName = 'Japscan Loader';
...
var implementations = [{
  name: 'japscan',
  match: "^https?://www\\.japscan\\.co/lecture-en-ligne/[^/]*/[0-9]*",
  img: function(ctx){
    let url = (ctx || document).querySelector('#image img');
   
    
    if(!url){
      url = (ctx || document).querySelector('#image');
      
      url = url.getAttribute("data-src");
  ...
  next: function(ctx){
    let url = (ctx || document).querySelector('#image a');
    
    if(!url){
      url = (ctx || document).querySelector('#image').getAttribute("data-next-link");
  ...
  numpages: '#pages',
  curpage: '#pages',
  nextchap: 'div.card:nth-child(3) > div:nth-child(2) > div:nth-child(1) > p:nth-child(4) > a:nth-child(2)',
  prevchap: 'div.card:nth-child(3) > div:nth-child(2) > div:nth-child(1) > p:nth-child(3) > a:nth-child(2)'
}];
// END OF IMPL
      ...
      getPageInfo = function() {
        ...
        try {
          // find image and link to next page
         	//log("*****"+ex('img', imp.imgmod, page)+ " " + ex('next', null, page))
