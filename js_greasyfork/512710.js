...
// @version      1.5.1
// @description  反诈中心脚本，检测并提示疑似诈骗链接（支持关键字和正则表达式两种模式）
// @author       _s_z_y_ & Murasame
// @match        https://www.luogu.com.cn/*
// @match        https://www.luogu.com/*
// @icon         https://www.google.com/s2/favicons?sz=64&domain=luogu.com.cn
// @license      MIT
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/512710/%E6%B4%9B%E8%B0%B7%E5%8F%8D%E8%AF%88%E4%B8%AD%E5%BF%83%20Plus.user.js
// @updateURL https://update.greasyfork.org/scripts/512710/%E6%B4%9B%E8%B0%B7%E5%8F%8D%E8%AF%88%E4%B8%AD%E5%BF%83%20Plus.meta.js
// ==/UserScript==
...
**功能简介**：
洛谷反诈中心脚本用于检测并提示洛谷网站中的疑似诈骗链接。用户可以自定义两种模式的关键词列表：
1. **匹配关键字模式**：使用简单的字符串进行匹配。
2. **高级模式**：使用正则表达式进行匹配。

**关键词配置**：
1. **打开编辑界面**：在洛谷页面，按下 `Ctrl + M` 快捷键将打开关键词编辑界面。
2. **编辑关键词**：
   - **匹配关键字模式**：
     - 每个关键词以独立的块展示，块与块之间有清晰的分割线。
     - 点击 `🖊` 按钮可直接在原位编辑关键词。
     - 点击 `×` 按钮可删除关键词。
     - 点击 `+ 添加关键词` 按钮可添加新的关键词。
   - **高级模式（正则表达式）**：
     - 每个正则表达式以独立的块展示，块与块之间有清晰的分割线。
     - 点击 `🖊` 按钮可直接在原位编辑正则表达式。
     - 点击 `×` 按钮可删除正则表达式。
     - 点击 `+ 添加正则表达式` 按钮可添加新的正则表达式。
     - 正则表达式需用斜杠 `/` 包裹，例如：`/^https?:\/\/(www\.)?example\.com/`
3. **保存设置**：编辑完成后，点击 `保存` 按钮，新的关键词和正则表达式列表将被保存并立即生效。

**使用说明**：
- 当鼠标悬停在包含疑似诈骗关键词或符合正则表达式的链接上时，将显示一个提示框。
- 点击该链接时，将在鼠标旁边弹出确认窗口，显示链接地址。
- 您可以通过以下方式操作：
  - 点击 `确定 (Enter)` 或按下 `Enter` 键以确认打开链接。
  - 点击 `取消 (Esc)` 或按下 `Esc` 键以取消操作。

**注意事项**：
- 请确保正则表达式的语法正确，否则可能导致匹配不准确。
- 关键词列表支持动态修改，您可以根据需要随时更新。
*/
...
(function() {
    ...
    let targetRegexes = JSON.parse(localStorage.getItem('targetRegexes')) || ['/^https?:\/\/([a-zA-Z0-9-]+\.)?mihoyo\.(com|cn)\/.*/', '/^https?:\\/\\/malicious\\.site\\//'];
