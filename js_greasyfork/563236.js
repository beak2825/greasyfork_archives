...
// + 기타 @ (뭔가 고쳤는데 기억이 안남)
// @downloadURL https://update.greasyfork.org/scripts/563236/SOOP%20%28%EC%88%B2%29%20-%20%EC%82%AC%EC%9D%B4%EB%93%9C%EB%B0%94%20UI%20%EB%B3%80%EA%B2%BD%20260119.user.js
// @updateURL https://update.greasyfork.org/scripts/563236/SOOP%20%28%EC%88%B2%29%20-%20%EC%82%AC%EC%9D%B4%EB%93%9C%EB%B0%94%20UI%20%EB%B3%80%EA%B2%BD%20260119.meta.js
// ==/UserScript==
...
// [수정 제안: 더 강력한 예외 처리 로직]

// 1. 현재 URL 가져오기
const currentUrl = window.location.href;

// 2. 사이드바를 표시하면 안 되는 페이지들 정의 (블랙리스트)
const isStation = currentUrl.includes('/station/') || currentUrl.includes('ch.sooplive.co.kr'); // 개인 방송국
const isVod = currentUrl.includes('vod.sooplive.co.kr') || currentUrl.includes('/player/');   // 다시보기 플레이어
const isEsports = currentUrl.includes('play.sooplive.co.kr');                                 // Esports 페이지

// 3. 하나라도 해당되면 '특수 페이지'로 간주
const isSpecialPage = isStation || isVod || isEsports;

if (isSpecialPage) {
    // [특수 페이지 상태] -> 사이드바 절대 금지

    // (1) 여백 클래스 제거
    document.body.classList.remove('customSidebar');

    // (2) 기존 사이드바가 있다면 즉시 처형(삭제)
    const sidebar = document.querySelector('.left_navbar');
    if (sidebar) {
        sidebar.remove();
    }

    // (3) 혹시라도 상단바 함수가 실행되지 않도록 방어
    // (이 부분은 필요 시 추가)

} else {
    // [홈/일반 목록 페이지] -> 사이드바 활성화

    // (1) 여백 클래스 추가 (설정에 따라)
    if (isCustomSidebarEnabled && !document.querySelector('.left_navbar')) {
        document.body.classList.add('customSidebar');

        // (2) 사이드바 생성 함수 실행
        if (typeof makeTopNavbarAndSidebar === 'function') {
            makeTopNavbarAndSidebar("main");
        }

        // (3) 기타 공통 함수 실행
        if (typeof runCommonFunctions === 'function') {
            runCommonFunctions();
        }
    }
}
