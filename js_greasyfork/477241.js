...
// @downloadURL https://update.greasyfork.org/scripts/477241/%E7%9B%91%E7%90%86%E5%B7%A5%E7%A8%8B%E5%B8%88%E7%BB%A7%E7%BB%AD%E6%95%99%E8%82%B2.user.js
// @updateURL https://update.greasyfork.org/scripts/477241/%E7%9B%91%E7%90%86%E5%B7%A5%E7%A8%8B%E5%B8%88%E7%BB%A7%E7%BB%AD%E6%95%99%E8%82%B2.meta.js
// ==/UserScript==
...
(function() {
    ...
    function search(){
        if (courseIndex <= courseCount) {
            let na = document.getElementsByTagName("tbody")[0].children[courseIndex-1].children[1].innerText;
            const ofIndex = na.indexOf('(');
            const coursName = na.substring(2,ofIndex);
            let courseStatu = document.getElementsByTagName("tbody")[0].children[courseIndex-1].children[2].innerText.substring(0,3);
            if(courseStatu !== '已完成'){
                addMessage(coursName+"|未完成");
                const coursHref = document.getElementsByTagName("tbody")[0].children[courseIndex-1].children[4].children[0].href;
                const nowUrl = window.location.href;
                saveNowPage(nowUrl);
                setTimeout(function(){
                    window.open(coursHref,'_self');
                },1000);
            }
            else{
                addMessage(coursName+"|已完成");
                courseIndex++;
                search();
            }
        }
    ...
    var wait = setInterval(function (){
        ...
        if(nowUrl.substring(0,50) === 'https://jlgcs.ok99ok99.com/stu/cls_courselist.aspx'){
            addMessage("课程列表,开始检索");
            courseCount = document.getElementsByTagName("tbody")[0].childElementCount;
            search();
        }
        else if(nowUrl.substring(0,48) === 'https://jlgcs.ok99ok99.com/stu/study_new_v3.aspx'){
            try {
                addMessage("学习页,无需操作...");
                let testx = setInterval(function(){
                    ...
                    if(document.getElementsByClassName("text_center study_unfinished pl_20 pr_20 pb_10")[0].getElementsByTagName("button")[0].className === 'btn btn_lg btn_fluid submit_btn'){
                        addMessage("开始提交");
                        clearInterval(testx);
                        document.getElementsByClassName("text_center study_unfinished pl_20 pr_20 pb_10")[0].getElementsByTagName("button")[0].click();
                        setTimeout(function(){
                            window.location.reload();
                        },1000);
                    }
                    if(document.getElementsByClassName("carry_prompt text_center study_finished")[0].innerText === '恭喜您!\n\n已完成本课程学习'){
                        clearInterval(testx);
                        addMessage("已完成本课程学习");
                        const oldHref = getNowPage();
                        setTimeout(function(){
                            window.open(oldHref,'_self');
                        },1000);
                    }
                },1000);
            }catch(e){
