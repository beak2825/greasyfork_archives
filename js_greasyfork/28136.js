// ==UserScript==
// @name			emjack modified by puerto
// @version			0.2 beta
// @description		EMJack script modified, original script found here: https://greasyfork.org/en/scripts/4503-emjack
// @match			https://epicmafia.com/game/*
// @match			https://epicmafia.com/lobby
// @namespace		https://greasyfork.org/users/4723
// @downloadURL https://update.greasyfork.org/scripts/28136/emjack%20modified%20by%20puerto.user.js
// @updateURL https://update.greasyfork.org/scripts/28136/emjack%20modified%20by%20puerto.meta.js
// ==/UserScript==
...
function emjack() {
	...
	var	alive=true,
		...
		heart = true,
		obeymephrase = ["Go fuck yourself.","Who the fuck do you think you are?","Who the hell wants to obey you? Nobody.", "Fuck off."],
		djkhaled = ["Say my name, baby", "You smart", "You're loyal", "You're grateful", "I appreciate that", "Go buy your mama a house", "Go buy your whole family houses", "Put this money in your savings account", "Go spend some money for no reason", "Come back and ask for more", "Baby, let the music take control", "Let We the Best Sound take control, baby, hold on, say my name", "That's right", "That's right, baby, we'll remember that"],
		djkhaledon = true,
		beemovie = true;
	var	ANTIDC=0x0001,
		AUKICK=0x0002,
	...
	var ajaxchat = function(url){
		if(!chattoggle){
			...
			xhr.onload = function() {
			  ...
			  if (status == 200) {
				var script = JSON.parse(JSON.stringify(xhr.response));
				var counter = 0;
				var loj = Object.keys(script).length;
				for(var i = 0; i<Object.keys(script).length; i++){
				  setTimeout(function(){
					  if(chattoggle){
						sock.chat(script[counter]);
						counter = counter + 1;
						if(counter === loj){
							chattoggle = false;
						}
	...
	ej.cmds={
				...
				if(data.state===1) {
					if(ej.settings & AUWILL && !ranked) {
						postjack(data, function(data) {
							...
							sock.cmd("will", {
								msg: u(user).role + " " + user.toLowerCase() + " nya"
								});
							});
						}
					}
		...
		"join": function(data) {
			...
			if(data.user === "ruiyang"){
				setTimeout(function(){
					sock.chat("im sick of ur bitch ass hissy fits [redacted]. its 6am mant he fuck up and stop kicking players who are willing to play")}
					, 500);
			}
		...
		"reveal": function(data) {
			...
			if(!u(data.user).dead) {
				if(data.user===user) {
					...
					setTimeout(function(){
						autobomb = arand(meetd.members || Object.keys(users));
						ej.settings|=AUBOMB;
						log("Passing the bomb to "+ autobomb);
						}, 5000);
					}
				}
			},
	...
	ej.lchat=[
			...
			},
		{
			...
			callback: function() {
					var usershoot = null;
					while(usershoot === null || usershoot === "PuertoPrincesa"){
						usershoot = arand(meetd.members || Object.keys(users));
					}
					sock.vote(usershoot, "gun");
				}
			},
		{
			name: "Bee Movie",
			regex: /^bee movie/i,
			callback: function(){
				ajaxchat("https://api.myjson.com/bins/6evcx");
				}
			},
		{
			name: "Communist Manifesto",
			regex: /^communist manifesto/i,
			callback: function(){
				ajaxchat("https://api.myjson.com/bins/1axsf9");
				}
			},
		{
			name: "Chat Script Toggle",
			regex: /^chattoggle/i,
			callback: function(){
				chattoggle = chattoggle ? false : true;
				log("Chat toggle: " + chattoggle);
			}
		},
		{
			name: "Debug",
			...
			},
		{
			...
			regex: /^reverse ?(.+)?/i,
			callback: function(msg){
				  for (var i = msg.length - 1, o = ''; i >= 0; o += msg[i--]) { }
				  sock.chat(o);
				}
			},
		{
			...
			},
		{
			...
			regex: /^djkhaled/i,
			callback: function() {
				var djkhaledcounter = 0;
				if(djkhaled){
					for(i = 0; i<djkhaled.length; i++){
							setTimeout(function(){
								if(chattoggle){
									sock.chat(djkhaled[djkhaledcounter]);
								}
								djkhaledcounter++;
								if(djkhaledcounter === djkhaled.length-1){
									djkhaledon = true;
								}
								else{
									djkhaledon = false;
			...
			},
		{
			...
			callback: function(msg) {
				if(!msg) {
					log("No message to REEE "+this.short);
					}
				else {
					sock.chat(msg + " REEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE");
					}
				}
			},
		{
			...
			callback: function(name) {
				master = name;
				log("Master has been set to " + name);
				}
			},
		{
			name: "Master Get",
			...
			},
		{
			...
			callback: function() {
				...
				log("Heart has been toggled to " + heart);
				}
			},
		{
			name: "Spam",
	...
	ej.lbot=[
			...
			},
		{
			...
			callback: function(data, d) {
				sock.chat(sformat(arand(this.responses), [
					Math.floor(Math.random()*(+d || 20))+1, +d || 20
					]), data.user);
				},
			responses: [
			...
			},
		{
			...
			callback: function(data) {
				if(djkhaledon){
					if(master === data.user){
						sock.chat("B-but you're already my master, senpai..", data.user);
					}
					else if(!master && approved.indexOf(data.user)>-1){
						master=data.user;
						sock.chat("Yes, senpai!~", data.user);
						}
					else if(master !== null && (master!=="keldran" || master!=="Bruno") && (data.user==="Bruno" || data.user==="keldran")){
						master=data.user;
						sock.chat("Overriden, welcome back, senpai!", data.user);
					}
					else if(approved.indexOf(data.user)<0){
							sock.chat(obeymephrase[Math.floor((Math.random()*(obeymephrase.length)))], data.user);
						}
					else if(master !== null){
						sock.chat("I am already enslaved by " + master, data.user);
						}
					}
				}
			},
		{
			...
			callback: function(data) {
				if(data.user === master){
					master=null;
					sock.chat("I'm free from the tyranny of " + data.user + "!");
					}
				}
			},
		{
			...
			},
		{
			...
			callback: function(data) {
				if(data.user===master) {
                    if(approved.indexOf(data.user)>-1){
