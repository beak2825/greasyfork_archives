...
// @downloadURL https://update.greasyfork.org/scripts/503599/Paragon%E9%A1%B5%E9%9D%A2%E7%9B%B4%E6%8E%A5%E5%BC%80%E5%87%BAWinston%20case.user.js
// @updateURL https://update.greasyfork.org/scripts/503599/Paragon%E9%A1%B5%E9%9D%A2%E7%9B%B4%E6%8E%A5%E5%BC%80%E5%87%BAWinston%20case.meta.js
// ==/UserScript==

(function() {
    'use strict';

    const emailBodyElements = document.getElementsByClassName('body_37kMP');
    ...
    const FAT_text = {
        ...
        "JP015": "JP021_Attribute Update for Retail ASINs | JP021_Retail ASINカタログAttributeの更新",
        "JP020": "JP021_Attribute Update for Retail ASINs | JP021_Retail ASINカタログAttributeの更新",
        ...
        "JP068": "JP068_Missing Buybox PB14 | JP068_異常Buyboxの確認と修正",
    ...
    function parseParagonPage() {
        try {
            const titleElement = document.querySelector('h1.caseSubject_blOTB');
            ...
            const tasklist = ["JP006","JP011","JP015","JP020","JP021","JP031","JP036", "JP051","JP059","JP068","JP077","JP107","JP108","JP119","JP184",'JP015'];
            ...
            //let match = (document.querySelector('.cc_-vtP2').textContent).match(/\b\w+(?=@amazon\.co\.jp)/);
            //let BS_login = '';
            //if (match) {
            //    BS_login = match[0];
            //    console.log('Found email content before @amazon.co.jp:', BS_login);
            //} else {
            //    console.log('No matching email content found');
            //}

            GM_setValue("vendorCode", vendorCode[0]);
            //GM_setValue("BS", BS_login);
...
async function fillWinstonForm() {
    try {
        ...
        await handleDropdown('.awsui-dropdown-trigger.awsui-select-trigger.awsui-select-trigger-no-option.awsui-select-trigger-variant-label', '#awsui-select-2-dropdown-options', FAT, '#awsui-select-2-textbox > span',);
        await waitForSeconds(3);
        await handleDropdown('.awsui-dropdown-trigger.awsui-select-trigger.awsui-select-trigger-no-option.awsui-select-trigger-variant-label', '#awsui-select-14-dropdown-options', '0 - 49', '#awsui-select-14-textbox > span');
        await handleDropdown('#awsui-select-6', '#awsui-select-6-dropdown-options', 'Paragon', '#awsui-select-6-textbox > span',);
...
async function handleDropdown(triggerSelector, optionsSelector, optionText, targetTextSelector, sourceTypeSelector = null, sourceTypeText = null) {
    try {
        ...
        const dropdownOptionsContainer = await waitForElement(optionsSelector);
        if (dropdownOptionsContainer) {
            let optionToSelect;
            if (optionText === "JP119_SL Auto variation broken fix | JP119_SL バリエーションばらけの修正") {
                optionToSelect = Array.from(dropdownOptionsContainer.querySelectorAll('div.awsui-select-option')).find(option => option.textContent.trim() === optionText);
            } else {
                optionToSelect = Array.from(dropdownOptionsContainer.querySelectorAll('div.awsui-select-option')).find(option => option.textContent.includes(optionText));
            }
            ...
            if (optionToSelect) {
                ...
                const targetElement = await waitForElement(targetTextSelector);
                targetElement.textContent = optionToSelect.textContent;

                if (sourceTypeSelector && sourceTypeText) {
                    const sourceTypeElement = await waitForElement(sourceTypeSelector);
    ...
    if (currentUrl === 'https://fe.winston.a2z.com/JP/task') {
        fillWinstonForm();
