...
// @downloadURL https://update.greasyfork.org/scripts/530210/WME%20Desa%20Overlay.user.js
// @updateURL https://update.greasyfork.org/scripts/530210/WME%20Desa%20Overlay.meta.js
// ==/UserScript==
...
(function() {
  ...
  var _color = '#E6E6E6';
  var _settingsStoreName = '_wme_desa';
  var _settings;
  var _features;
  var _kml;
  ...
  async function updateAllMaps(){
      ...
      let KMLinfoArr = await fetch(`https://api.github.com/repos/hardian-n/Waze_Cityname_Indonesia/contents/DESA/${countryAbbr}`);
          ...
          for(let j=0; j<KMLinfoArr.length; j++){
              if(KMLinfoArr[j].name === `${state}_Cities.kml`){ //check the size in db against server - if different, update db
                  ...
                  if(stateObj.kmlsize !== KMLinfoArr[j].size){
                      let kml = await fetch(`https://raw.githubusercontent.com/${repoOwner}/Waze_Cityname_Indonesia/master/DESA/${countryAbbr}/${state}_Cities.kml`);
  ...
  function init() {
      ...
      $section.html([
          `<h4 style="margin-bottom:0px;"><i id="citiesPower" class="fa fa-power-off" aria-hidden="true" style="color:${_settings.layerVisible ? 'rgb(0,180,0)' : 'black'}; cursor:pointer;"></i> <b>WME Desa</b></h4>`,
      ...
      if(currState != W.model.getTopState().attributes.name)
      {
          ...
          if(typeof stateAbbr !== "undefined"){
              if(typeof kmlCache[stateAbbr] == 'undefined'){
                  ...
                  if(!request){
                      let kml = await fetch(`https://raw.githubusercontent.com/${repoOwner}/Waze_Cityname_Indonesia/master/DESA/${countryAbbr}/${stateAbbr}_Cities.kml`);
                      _kml = kml;
                      updatePolygons();
  ...
  function bootstrap(tries = 1) {
      if (W && W.loginManager && W.loginManager.user && W.model.getTopState() && WazeWrap.Ready) {
          init();
          console.log('WME Desa:', 'Initialized');
      } else if(tries < 1000){
          console.log('WME Desa: ', 'Bootstrap failed.  Trying again...');
