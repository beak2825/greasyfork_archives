...
// @downloadURL https://update.greasyfork.org/scripts/511656/CosmosisT%27s%20TinyChat%20Script%20%28CTS%20V1865%29.user.js
// @updateURL https://update.greasyfork.org/scripts/511656/CosmosisT%27s%20TinyChat%20Script%20%28CTS%20V1865%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    var CTS = {
        ...
        Favorited: ["enragedhippo", null, null, null, null],
        SafeList: [],
        GreenRoomIgnoreList: [],
        GreenRoomList: [],
        WatchList: [],
    ...
    function CTSInit() {
        ...
        CTS.ScriptLoading = setInterval(function() {
            ...
            if (document.querySelector("#welcome-wrapper")) {
                document.querySelector("#welcome-wrapper h1").innerHTML = `<span style="display: inline-block; font-family: Arial, sans-serif; color: #fff; background: linear-gradient(to right, #1a1a1a, #444); padding: 20px 30px; border-radius: 20px; transition: background-color 0.3s;">Looking for an epic online video chat experience?<br><span style="font-size: 1.5em;">Tinychat is an online video chat community that went to shit.<br>Try <a href="https://stumblechat.com/" style="text-decoration: none; color: #f7b32d!important;">Stumblechat.com</a> today!</span></span>`;
                CTSHomeInject();
    ...
    function BotCommandCheck() {
        ...
        if (isCommand(arguments[1])) {
            ...
            } else if (arguments[1].match(/^!whoisbot$|^!8ball |^!vote |^!coin$|^!chuck$|^!urb |^!wea |^!dad$|^!advice$/i)) {
    ...
    function BotCommandCheckPUB() {
        ...
        if (CTS.PublicCommandToggle) {
            if (arguments[1].match(/^!8ball [\w\s]*\??/i)) {
                ...
                } else if (arguments[1].match(/^!wea /i)) {
                    Weather(arguments[1], CTS.UserList[arguments[0]].username);
    ...
    function Weather() {
        //OPEN REQUEST
        if (isSafeListed(arguments[1])) {
            var wea = arguments[0].match(/^!wea ([\w ]*)/i);
            if (wea !== null) {
               CTS.Weather.XHR.open("GET", "https://api.openweathermap.org/data/2.5/weather?q="+wea[1]+"&appid=dd73c6a7e159ac97b9cf49a77c37274a&units=metric");
               CTS.Weather.XHR.send();
            }
        }
    }
    ...
    var CommandList = {
        ...
        publiccommandtoggle: function() {
            ...
            Settings();
            Alert(GetActiveChat(), "✓ Command Accepted!\n" + ((CTS.PublicCommandToggle) ? "Public commands (8Ball,  Advice, Chuck, Coin, Dad, Urb, Weather) are enabled." : "Public commands (8Ball,  Advice, Chuck, Coin, Dad, Urb, Weather) are disabled."));
        },
        ...
        ping: function() {
            if (CTS.ScriptInit) {
                ...
                if (CTS.WatchList.length > 0) {
                    ...
                    debug("WATCHLIST::VERIFYING", CTS.WatchList[0][0] + " " + verify + "/7000");
                    if (CTS.SafeList.indexOf(CTS.WatchList[0][0]) === -1) { //LET'S NOT ADD TWICE
                        if (verify > 7000) {
                            debug("WATCHLIST::VERIFIED", CTS.WatchList[0][0] + " " + verify + "/7000");
    ...
    CTS.Weather.XHR.onload = function() {
        var resp = JSON.parse(CTS.Weather.XHR.responseText),
    ...
    function SetLocalValues() {
        if (CTS.StorageSupport) {
            ...
            CTS.FavoritedRooms = JSON.parse(Load("FavoritedRooms", JSON.stringify(["enragedhippo", null, null, null, null])));
