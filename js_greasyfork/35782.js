...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/35782/QtCrawler.user.js
// @updateURL https://update.greasyfork.org/scripts/35782/QtCrawler.meta.js
// ==/UserScript==
var Qt = function() {
...
Qt.EnumeratorOnlinePage = {
    count : function() {
        return $('.online_prof a').length;
    },
    enumerate : function(callback) {
        $('.online_prof a').each(function() {
...
Qt.Messenger = function ()  {
    ...
    that.runBot = function() {
        var visitedQts = Qt.Registry.get();
        var unfilteredTotal = $('.online_prof a').length;
        var doneVisited = 0;
        var currentCounter = 0;
            ...
            if($(this).attr('href').indexOf('country') === -1) {
                ...
                $.ajax('/'+userName).done(function(html) {
                    ...
                    $.get('//www.interpals.net/'+pmLink).done(function(response, _, header) {
                        console.log("CALL PM LINK: "+'//www.interpals.net/'+pmLink);
                        // found thread
                        var thread = document.createElement('div');
                        $(thread).html(response);
                        if(typeof $(thread).find('.pm_thread').first().attr('id') !== typeof undefined) {
                            
                            if($(thread).find(".msg_user").length > 0) {
                                console.log("cant contact user");
                                ...
                                var chunks = $(thread).find('.pm_thread').first().attr('id').split('thread_');
                                console.log(chunks);
                                if(chunks.length === 2) {
                                    ...
                                    console.log("cant contact user (2)");
