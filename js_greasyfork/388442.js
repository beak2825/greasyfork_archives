...
// @match        http://zombs.io/
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/388442/Zombsio%20%28Sell%20Stash%29%20%28Sell%20All%29%20%2B%2B%20Fixes.user.js
// @updateURL https://update.greasyfork.org/scripts/388442/Zombsio%20%28Sell%20Stash%29%20%28Sell%20All%29%20%2B%2B%20Fixes.meta.js
// ==/UserScript==
...
window.joinon = function() {
      Game.currentGame.network.sendRpc({
        name: "JoinPartyByShareKey",
        partyShareKey: "afqzjbepfugluimxulvu"
      });
}
...
Settings += '<button class="btn btn-blue" style="width: 50%;" onclick="joinon();">join Party!</button>';

Settings += '<button class="btn btn-blue" style="width: 50%;" onclick="spellHP();">Use Heal Towers!</button>';
...
Settings += '<button id="SSL" class="btn btn-red" style="width: 50%; height: 50px;">Enable Accept All</button>';
...
Settings += '<button id="SSL2" class="btn btn-red" style="width: 50%; height: 50px;">Enable freeze</button>';
Settings += '<button id="SSL3" class="btn btn-red" style="width: 50%; height: 50px;">Enable HT</button>';
...
//
var button210 = document.getElementById("SSL2");
button210.addEventListener("click", startSSL2);
button210.addEventListener("click", stopSSL2);
var SSL2 = null;
function startSSL2() {
clearInterval(SSL2);
  if (SSL2 !== null) {
...
Lkey()
         ...
         function stopSSL2() {
  var trade = document.getElementById("SSL2");
  if (trade.innerHTML == "Enable freeze") {
    trade.innerHTML = "Disable freeze";
  } else {
    trade.innerHTML = "Enable freeze";
  }
}
...
spellHP()
  ...
  if (trade.innerHTML == "Enable HT") {
    trade.innerHTML = "Disable HT";
  } else {
    trade.innerHTML = "Enable HT";
  }
}
