// ==UserScript==
// @name         Exchange parse
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  Парсинг танків для фільтру в розширенні
// @author       wandersher
// @match        https://st-roster.win/personal/vehicle-exchange
// @icon         data:image/png;base64,
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/430757/Exchange%20parse.user.js
// @updateURL https://update.greasyfork.org/scripts/430757/Exchange%20parse.meta.js
// ==/UserScript==


function getItem(key) {
	return new Promise((resolve) => chrome.runtime.sendMessage("mpjhblapicilagkfkldbjcilghbnmpgj", { action: 'get', key }, resolve))
}

function setItem(key, data) {
	return new Promise((resolve) => chrome.runtime.sendMessage("mpjhblapicilagkfkldbjcilghbnmpgj", { action: 'set', key, data }, resolve))
}

(async function() {
    'use strict';
   	const date = new Date().getTime()
	const regexp = new RegExp('<div class="col-md-3 vehicle-item vehicle-type-(.*?) vehicle-level-(.*?)".*?<div class="title">(.*?)</div>', 'g')
	const source = document.querySelector('.row.vehicles').innerHTML
	const vehicles = await getItem("exchange")
	let matches = null
	while (matches = regexp.exec(source)) {
		const found = vehicles.find((vehicle) => vehicle.name === matches[3])
		if (found) {
			found.date = date
		} else {
			vehicles.push({name: matches[3], type: parseInt(matches[1]), level: parseInt(matches[2]), date})
		}
	}
	await setItem("exchange", vehicles)
})();