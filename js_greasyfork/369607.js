...
// @downloadURL https://update.greasyfork.org/scripts/369607/%D0%91%D1%8B%D1%81%D1%82%D1%80%D1%8B%D0%B9%20%D0%BF%D0%BE%D0%B8%D1%81%D0%BA%20%D0%93%D0%9D.user.js
// @updateURL https://update.greasyfork.org/scripts/369607/%D0%91%D1%8B%D1%81%D1%82%D1%80%D1%8B%D0%B9%20%D0%BF%D0%BE%D0%B8%D1%81%D0%BA%20%D0%93%D0%9D.meta.js
// ==/UserScript==
...
let info = $('#info');
let btns = $('#show-car').parent()
btns.css({'color': 'black', 'padding': '0'})
...
checkButton.clone().prop('id', 'av-r').css('background-color', 'rgba(255, 188, 0, 0.71)').text('АвтоРапорт').prependTo(bttnContainer);
checkButton.clone().prop('id', 'mos').css('background-color', 'rgba(205, 54, 51, 0.71)').text('МосРег').prependTo(bttnContainer);
...
checkButton.clone().prop('id', 'sts').css('background-color', 'rgba(47, 4, 145, 1)').text('История СТС').appendTo(bttnContainer);
checkButton.clone().prop('id', 'dkk').css('background-color', 'rgba(20, 20, 255, 0.71)').text('История ДКК').appendTo(bttnContainer);
checkButton.clone().prop('id', 'dkb').css('background-color', 'rgba(242, 42, 117, 0.71)').text('История ДКБ').appendTo(bttnContainer);
...
if (!url.includes('qc/history?exam=') && !url.includes('qc?exam=branding')) {
    ...
    const btnBlacklist = document.getElementById('btn-blacklist'),
          ...
          btnRemarks = document.getElementById('btn-ok-remarks')
