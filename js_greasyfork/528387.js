// ==UserScript==
// @name         WestForts.com Import Link
// @version      2.3
// @description  A new link to launch the WestForts import script
// @author       WestForts Staff
// @website      https://www.westforts.com/
// @include  http://*.the-west.*/game.php*
// @include  https://*.the-west.*/game.php*
// @namespace    https://greasyfork.org/users/8213
// @downloadURL https://update.greasyfork.org/scripts/528387/WestFortscom%20Import%20Link.user.js
// @updateURL https://update.greasyfork.org/scripts/528387/WestFortscom%20Import%20Link.meta.js
// ==/UserScript==

if (location.href.indexOf(".the-west.") != -1 && location.href.indexOf("game.php") != -1) {

  if (document.getElementById('westforts_js')) { return; }

  var westforts_js = document.createElement('script');

  westforts_js.setAttribute('type', 'text/javascript');
  westforts_js.setAttribute('id', 'westforts_js');

  westforts_js.innerHTML = "("+(function() {

    var westforts_icon = 'data:image/png;base64,';
    var westforts_icon_bn = 'data:image/png;base64,';
    var westforts_link = document.createElement('div');
    westforts_link.setAttribute('id', 'westforts_link_div');
    westforts_link.setAttribute('title', '<b>Start West Forts Import</b>');
    westforts_link.setAttribute('style', 'position:absolute; z-index:10; cursor:pointer; text-align:center; color:#fff; font-size:12px; padding: 0px 3px 1px 9px; right:0px; top:0px; background:url(\"/images/interface/minimap/minimapbg.png\") no-repeat scroll 0px -7px');
    westforts_link.innerHTML = '<img id="westforts_icon_img" src="' + westforts_icon_bn + '" /><div style="float:right;margin-left:5px;margin-top:1px;">West Forts</div>';

    var westfortsArmed = false; // Track script state
    var origFortBattleMakeStats = FortBattle.makeStats; // Store original function
    var origCemeteryShowStatUpdateTable = CemeteryWindow.showStatUpdateTable; // Store original function
    var existingScript;

    westforts_link.onclick = function() {
        if (westfortsArmed) {
            // **Disarm the script**
            this.style.color = 'rgb(255, 255, 255)'; // Turn white
            document.getElementById('westforts_icon_img').src = westforts_icon_bn;
            westfortsArmed = false;

            existingScript = document.getElementById('wfScript');
            if (existingScript) {
                existingScript.remove(); // Remove script tag
            }

            // **Restore the original functions to stop auto-import**
            FortBattle.makeStats = origFortBattleMakeStats;
            CemeteryWindow.showStatUpdateTable = origCemeteryShowStatUpdateTable;

            // **Ensure that any lingering import form is removed**
            if (document.getElementById('wfForm')) {
                document.getElementById('wfForm').remove();
            }

        } else {
            // **Arm the script**
            existingScript = document.getElementById('wfScript');
            if (!existingScript) {
                var westforts_script = document.createElement('script');
                westforts_script.setAttribute('type', 'text/javascript');
                westforts_script.setAttribute('id', 'wfScript');
                westforts_script.setAttribute('src', 'https://www.westforts.com/js/import.js?' + new Date().getTime());
                document.body.appendChild(westforts_script);
            }

            this.style.color = '#01DF00'; // Turn green
            document.getElementById('westforts_icon_img').src = westforts_icon;
            westfortsArmed = true;
        }
    };

    document.body.appendChild(westforts_link);

  }).toString()+")();";

  document.body.appendChild(westforts_js);
}