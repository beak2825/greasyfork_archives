// ==UserScript==
// @name           Check Yth Pulls Feed Local
// @version        4.0.1
...
// @downloadURL https://update.greasyfork.org/scripts/432991/Check%20Yth%20Pulls%20Feed%20Local.user.js
// @updateURL https://update.greasyfork.org/scripts/432991/Check%20Yth%20Pulls%20Feed%20Local.meta.js
// ==/UserScript==

let div = "";
let div_area = "";
const Promises_Array = [];
const Promises_Array_Players = [];
...
const pulls_time = new Map();

const LastSaturday = getLastSaturday();

const leauges_array = [
  { country: "il", division: "1", group: "1" },
  { country: "il", division: "2", group: "1" },
  { country: "il", division: "2", group: "2" },
  { country: "il", division: "2", group: "3" },
  { country: "il", division: "2", group: "4" },
  { country: "il", division: "3", group: "1" },
  { country: "il", division: "3", group: "2" },
  { country: "il", division: "3", group: "3" },
  { country: "il", division: "3", group: "4" },
];
...
function request_player_info(id) {
  return $.ajax({
    ...
    success: function (data) {
      ...
      potenitel = players_map.get(player.player_id);
      if (
        player.age < 18 &&
        ((favposition == "gk" && potenitel > 6) ||
          (favposition != "gk" && potenitel > 8))
      ) {
        players.push({
          id: player.player_id,
          name: player.name,
        ...
        ids_array.push(player.player_id);
        pullTime = pulls_time.get(player.player_id);
        checkPullWeek(pullTime) ? last_week.push(player.player_id) : older.push(player.player_id);
...
function getIdsFromFeed(feed) {
  ...
  for (var i = 0; i < array_list.length; i++) {
    ...
    if (yth_val > -1) {
      pot = feed[i].attributes.extra[1];
      if (parseInt(pot) > 6) {
        id = feed[i].attributes.extra[0];
        if (players_map.has(id) == false) {
          players_map.set(id, parseInt(pot));
          pulls_time.set(id, feed[i].full_time)
        }
      }
      ...
      if (array_list[i].hasOwnProperty(myProp)) {
        ...
        for (var k = 0; k < array_subs.length; k++) {
          pot = array_subs[k].attributes.extra[1];
          if (parseInt(pot) > 6) {
            id = array_subs[k].attributes.extra[0];
            if (players_map.has(id) == false) {
              players_map.set(id, parseInt(pot));
              pulls_time.set(id, array_subs[k].full_time)
            }
          }
        }
      }
...
function BuildResult(last_week, older) {
  div_area.innerHTML =
    '<div style="position: absolute; z-index: 1;background-color: #4e7525 ; width: 186px; height: 400px; margin-top: 190px; color: white;  outset; display:inline;"><table id = \'table\' style="margin-top: -1em; margin-bottom: 1em; position:relative; top:0px;left:5px">&nbsp;<tr><td>Done: </td><td>' +
    "YES" +
    " </td></tr><tr><td>Last Week: </td><td> " +
    "</td></tr>      </table></b></div>";
...
div.onclick = function () {
  ...
  Promise.all(Promises_Array).then(() => {
    getIDs(feeds_array);
    runCheck(
      Promises_Array_Players,
      [...players_map.keys()],
      request_player_info
    );
    Promise.all(Promises_Array_Players).then(() => {
...
function clear_data() {
  Promises_Array.splice(0, Promises_Array.length);
  Promises_Array_Players.splice(0, Promises_Array_Players.length);
  ids_array.splice(0, ids_array.length);
  feeds_array.splice(0, feeds_array.length);
  players.splice(0, feeds_array.length);
  players_map.clear();
}
