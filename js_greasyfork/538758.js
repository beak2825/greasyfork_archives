// ==UserScript==
// @name         Faction & War Member Filter test
...
// @downloadURL https://update.greasyfork.org/scripts/538758/Faction%20%20War%20Member%20Filter%20test.user.js
// @updateURL https://update.greasyfork.org/scripts/538758/Faction%20%20War%20Member%20Filter%20test.meta.js
// ==/UserScript==
...
(function() {
    ...
    const TORN_API_KEY = 'AIK9ZdfA1KQ5eirH';
    const TS_API_KEY = 'TS_M3xTcYuMDCAVWp7x';
    ...
    function getStatus(node) {
        ...
        if (c.includes('okay'))      return OKAY;
        if (c.includes('hospital'))  return HOSPITAL;
        if (c.includes('traveling')) return TRAVELING;
        if (c.includes('abroad'))    return ABROAD;
        return ABROAD;
    }
    ...
    function sortStatus(node, sort) {
        ...
        const sortIcon = node.parentNode.querySelector(".status > [class*='sortIcon']");
        if (sort !== undefined)      node.finallySort = sort;
        ...
        if (sortIcon) {
            if (node.finallySort > 0) {
                const active = node.parentNode.querySelector("[class*='activeIcon']:not(.finally-status-activeIcon)");
                if (active) {
                    const cls = active.className.match(/(?:\s|^)(activeIcon[^\s]+)(?:\s|$)/)?.[1];
                    if (cls) active.classList.remove(cls);
                }
        ...
        nodes.sort((a, b) => {
            ...
            const statusA = getStatus(a.querySelector('.status'));
            const posA    = a.finallyPos;
            const idB     = b.querySelector('a[href*="XID"]').href.replace(/.*?XID=(\d+)/i, '$1');
            const statusB = getStatus(b.querySelector('.status'));
            ...
            switch (node.finallySort) {
                case 1:
                    ...
                    return (hospTime[idA] - Date.now()/1000) - (hospTime[idB] - Date.now()/1000);
                case 2:
                    if (statusA !== HOSPITAL || statusB !== HOSPITAL) {
                        return statusOrder[statusB] - statusOrder[statusA];
                    }
                    return (hospTime[idB] - Date.now()/1000) - (hospTime[idA] - Date.now()/1000);
                default:
                    return posA - posB;
            }
        });
        ...
        // propagate sort to other lists
        if (sort === undefined) {
            document.querySelectorAll('.members-list').forEach(e => {
                if (e !== node) sortStatus(e, node.finallySort);
            });
        }
    }

    function updateHospTimers() {
        for (const [id, node] of hospNodes) {
            if (!node || !hospTime[id]) continue;
            let total = hospTime[id] - Date.now()/1000;
            if (total <= 0) continue;
            // throttle updates
            if ((total >= 600 && total % 10 > 1) || (total >= 300 && total % 5 > 1)) continue;
    ...
    function collectStatusNode(node) {
        ...
        const id        = link.href.replace(/.*?XID=(\d+)/i, '$1');
        const inWarPage = node.classList.contains("your") || node.classList.contains("enemy");
        if (!hospNodes.some(h => h[0]===id && h[2]===inWarPage)) {
            hospNodes.push([id, statusNode, inWarPage]);
        }
    }

    function collectStatusNodeAll(root) {
        if (!root) root = document;
        const items = root.querySelectorAll('.your:not(.row-animation-new), .enemy:not(.row-animation-new), .table-body > .table-row');
        items.forEach(collectStatusNode);
    }

    function watchWall(list) {
        if (!list) return;
        const title = list.parentNode.querySelector('.title, .c-pointer');
        const old   = title.querySelector('.status');
        if (old && !old.classList.contains('custom-hosp-timer')) {
            const cloned = old.cloneNode(true);
            cloned.classList.add('custom-hosp-timer');
            const arrowClass = cloned.childNodes[1].className.match(/(asc|desc)[^\s]*/)?.[0];
            if (arrowClass) cloned.childNodes[1].classList.remove(arrowClass);
            old.replaceWith(cloned);
            cloned.addEventListener('click', () => sortStatus(list));
            // restore previous sort
            const stored = parseInt(localStorage.getItem('finally.torn.factionSort')) || 1;
            setTimeout(() => title.children[Math.abs(stored)-1].click(), 50);
        }
        collectStatusNodeAll(list);
        new MutationObserver(muts => {
            muts.forEach(m => {
                m.addedNodes.forEach(n => {
                    if (n.classList && (n.classList.contains('your') || n.classList.contains('enemy'))) {
                        collectStatusNode(n);
                    }
                });
            });
    ...
    function watchWalls(root) {
        if (!root) return;
        root.querySelectorAll('.members-list').forEach(watchWall);
        new MutationObserver(muts => {
            muts.forEach(m => {
                m.addedNodes.forEach(n => {
                    if (n.querySelectorAll) {
                        n.querySelectorAll('.members-list').forEach(watchWall);
                    }
                });
            });
    ...
    new MutationObserver(muts => {
        muts.forEach(m => {
            m.addedNodes.forEach(n => {
                if (n.querySelector && n.querySelector('.f-war-list')) {
                    watchWalls(n.querySelector('.f-war-list'));
                }
            });
        });
    }).observe(document.body, { childList: true, subtree: true });

    // intercept fetch for live war updates
    const tW = isPDA() ? window : unsafeWindow;
    const oldFetch = tW.fetch;
    tW.fetch = async function(...args) {
        const url = args[0]?.url || args[0];
        if (!url.includes('getwarusers') && !url.includes('getProcessBarRefreshData')) {
            return oldFetch.apply(this, args);
        }
        const resp = await oldFetch.apply(this, args);
        const clone = resp.clone();
        clone.json().then(json => {
            const members = json.warDesc?.members || json.userStatuses || null;
            if (!members) return;
            Object.entries(members).forEach(([id, data]) => {
                const st = data.status?.text || data.text;
                if (st === 'Hospital') hospTime[id] = data.status?.updateAt || data.updateAt;
    ...
    // intercept WebSocket for live status
    const oldWS = tW.WebSocket;
    tW.WebSocket = function(...args) {
        const sock = new oldWS(...args);
        sock.addEventListener('message', evt => {
            const msg = JSONparse(evt.data);
            const update = msg?.result?.data?.data?.message?.namespaces?.users?.actions?.updateStatus
            || msg?.push?.pub?.data?.message?.namespaces?.users?.actions?.updateStatus;
            if (!update?.status) return;
            const id = update.userId;
            if (update.status.text === 'Hospital') hospTime[id] = update.status.updateAt;
            else delete hospTime[id];
            collectStatusNodeAll();
        });
        return sock;
    ...
    addStyle(`
      ...
      .finally-status-activeIcon { display: block !important; }
      .finally-status-asc {
        border-bottom:6px solid var(--sort-arrow-color);
        border-left:6px solid transparent;
        border-right:6px solid transparent;
        height:0; top:-8px; width:0;
      }
      .finally-status-desc {
        border-top:6px solid var(--sort-arrow-border-color);
        border-left:6px solid transparent;
        border-right:6px solid transparent;
        height:0; top:-1px; width:0;
      }
      .finally-status-col { text-overflow: clip !important; }
      .faction-side-swap {
