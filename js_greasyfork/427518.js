...
// @require     https://cdnjs.cloudflare.com/ajax/libs/crypto-js/3.1.2/rollups/aes.jsy
// @grant       GM_getValue
// @grant       GM_setValue
...
// @grant       GM_addStyle
// @version 2022.09.03
// @downloadURL https://update.greasyfork.org/scripts/427518/UnMod_cf.user.js
// @updateURL https://update.greasyfork.org/scripts/427518/UnMod_cf.meta.js
// ==/UserScript==
var UM_VER = '2022.09.03';
//
var conowego = 'Co nowego?\n\n\'';
//2021.06.04: poprawki by xGreatx\n\
//2021.06.04: podsumowanie walka runda po rundzie itd by xGreatx\n\
...
function UNMOD() {
    ...
    var wyprawkastartbutton = document.getElementsByClassName('button actionButton')[0]

    if(wyprawkastartbutton && a =="?a=quest"){
        var menu = document.getElementsByClassName('menu')[13].outerText
        console.log(menu)
        if(menu == "Pamiątki"){
            menu = document.getElementsByClassName('menu')[14].outerText
            }
        if(menu == "Zbrojownia · 16"){
        wyprawkastartbutton.click();
        }
        else{
            console.log("zły zestaw")
        }
    }
    ...
    function notification(msg) {
        if (GM_getValue(id + 'UM_OP_notify', true)) {
            if (!('Notification' in window)) {
                ...
                var audio2 = new Audio('https://www.soundjay.com/buttons/beep-24.mp3');
                audio2.volume = 0.6;
                audio2.play();
    ...
    function sellScrapForPR(){
        if(GM_getValue(id + 'UM_OP_sellscrap', true)){
        var count = GM_getValue(id + 'UM_countValue', false)
        var price = GM_getValue(id + 'UM_priceValue', false)
        var scraps = document.getElementById('listItem_-1')
        scraps.click()
        var selljunk = document.getElementById('sell_junk')
        selljunk.value = count
        addItem(-1, $$('sell_junk').value)
        var minimalPrizeCheckbox = document.getElementById('mpCheck')
        var sixhours = document.getElementById('auEnd_1')
        var minPrizeValue = document.getElementById('minPrizeValue')
        sixhours.click()
        minimalPrizeCheckbox.click()
        minPrizeValue.value = (count * price)
        console.log(scraps)
        }
    }
    ...
    function bidAuction() {
        var items = {
            "Legendarny Doskonały Hipnotyczny Łańcuch Sztuki": 500000,
            "Legendarny Doskonały Twardy Sygnet Celności": 45000
        };
        var auctionRow = document.getElementsByClassName('auctionRow');
        if (auctionRow.length > 0) {
            var winningSon = auctionRow[0].children[0].innerHTML.includes("Twoja oferta wygrywa");
            var name = auctionRow[0].children[1].outerText;
            var highestbid = auctionRow[0].children[3].outerText.replace(/\s/g, '');
            highestbid = parseInt(highestbid);
            var timer = auctionRow[0].children[4].children[3].outerText;
            var bidButton = auctionRow[0].children[6].children[0];
            timer = timer.match("00:00:[0-4][0-9]")
            var rand = Math.round(Math.random() * (34 - 31)) + 31;
            if(!sessionStorage.getItem("rand") || sessionStorage.getItem("rand") == 0){
                sessionStorage.setItem("rand", "00:00:"+rand)
            }
            for (var key in items) {
                var limit = items[key];
                if (!winningSon) {
                    if (timer !== null) {
                        var timerFromRandom = sessionStorage.getItem("rand")
                        if (highestbid <= limit && key == name && (timer[0] == timerFromRandom || timer[0] == "00:00:28" || timer[0] == "00:00:24" || timer[0] == "00:00:15" || timer[0] == "00:00:08")) {
                            bidButton.onclick()
                            var buttonGreen = document.getElementsByClassName('buttonGreen');
                            buttonGreen = buttonGreen[0]
                            console.log("Bid")
                            buttonGreen.click()
                            sessionStorage.setItem("rand", 0)
                        } else {
                            console.log("Limit")
                        }
                    } else {
                        console.log("Timer")
                        console.log(sessionStorage.getItem("rand"))
                    }
                } else {
                    console.log("Winning")
                    if (timer[0] == "00:00:35" || timer[0] == "00:00:32" || timer[0] == "00:00:26" || timer[0] == "00:00:16" || timer[0] == "00:00:01") {
                        window.location = 'https://r14.bloodwars.interia.pl/?a=auction';
                    }
                }
            }
        }
    ...
    function scanForPLN()
    {
     var matchnickregex ='<b>.+</b>'
     var table = document.getElementsByClassName('rank hoverTable')
     for (row = 1; row < table[0].rows.length; row++) {
  //for (cell = 0; cell < table[0].rows[row].cells.length; cell++) {
      var nick = table[0].rows[row].cells[1].innerHTML.trim()
      var nick = nick.match(matchnickregex)
      var nick = nick[0].substring(3)
      nick = nick.substring(0,nick.length-4);
      var address = table[0].rows[row].cells[5].innerHTML.trim();
      var clan = table[0].rows[row].cells[6].innerHTML.trim();
      var uid = table[0].rows[row].cells[1].innerHTML.match('uid=[0-9]+');
      //console.log(clan)
      if((address.startsWith('<a href="?a=townview&amp;strefa=4') || address.startsWith('<a href="?a=townview&amp;strefa=5')) && clan.startsWith('-')){
          console.log('https://r14.bloodwars.interia.pl/?a=ambush&opt=spy&targetName='+nick);
          //window.open('https://r14.bloodwars.interia.pl/?a=ambush&opt=spy&targetName='+nick);
      }
   //}
     }
    }
    ...
    if (a == '?a=settings') {
        ...
        opcje = '<br /><br /><span style="color: #fff; text-shadow: 0px -1px 4px white, 0px -2px 10px yellow, 0px -10px 20px #ff8000, 0px -18px 40px red; font: 20px \'BlackJackRegular\'";><b>UnMod</b> ver: ' + UM_VER + ' - simply made by JUEN, poprawki czapka & FaZiR</span><br /><br />';
        ...
        //autoczyszczenie
        opcje += '<tr><td><input type="checkbox"';
        if (GM_getValue(id + 'UM_OP_autoczyszczenie', true)) opcje += ' checked="checked"';
        opcje += ' id="UM_OP_autoczyszczenie"> autoczyszczenie śmieci z zk (półka 1)</td></tr>';

        //wystawienie złomu
        opcje += '<tr><td><input type="checkbox"';
        if (GM_getValue(id + 'UM_OP_sellscrap', false)) opcje += ' checked="checked"';
        opcje += ' id="UM_OP_sellscrap"> Ilość złomu: <input type="text" id="UM_countValue" value="' + GM_getValue(id + 'UM_countValue', '0') + '"></td><td> Cena złomu: <input type="text" id="UM_priceValue" value="' + GM_getValue(id + 'UM_priceValue', '0') + '"></td></tr>';
        ...
        opcje += ' id="UM_OP_donesound"> odgrywaj dźwięk po zakończonej wyprawie/ataku/szpiegowaniu <input type="text" id="UM_urlsound" value="' + GM_getValue(id + 'UM_urlsound', 'https://media.geeksforgeeks.org/wp-content/uploads/20190531135120/beep.mp3') + '"></td></tr>';
        ...
        // opcje += '- STATYSTYKI MOBÓW Z II MAPY: <a target="_new" href="https://docs.google.com/spreadsheets/d/1x1YTmImhj6a_g3OeKXXJrWlh_BtvmIDJcMr5DFkdRtk/edit#gid=0">https://docs.google.com/spreadsheets/d/1x1YTmImhj6a_g3OeKXXJrWlh_BtvmIDJcMr5DFkdRtk/edit#gid=0</a><br>';
        // opcje += '- TABELA UMAGÓW: <a target="_new" href="https://docs.google.com/spreadsheets/d/10hylzzMRHe8f-nkr1UMLge0sx6NEMvBkZhnveXKPEpg/edit#gid=969554585">https://docs.google.com/spreadsheets/d/10hylzzMRHe8f-nkr1UMLge0sx6NEMvBkZhnveXKPEpg/edit#gid=969554585</a><br>';
        // opcje += '</span></td></tr>';
        ...
        /* autoczyszczenie */
        document.getElementById('UM_OP_autoczyszczenie').addEventListener('click', function() {
            GM_setValue(id + 'UM_OP_autoczyszczenie', this.checked);
        }, false);

        /* sprzedaż złomu */
        document.getElementById('UM_OP_sellscrap').addEventListener('click', function() {
            GM_setValue(id + 'UM_OP_sellscrap', this.checked);
        }, false);

        document.getElementById('UM_countValue').addEventListener('keyup', function() {
            GM_setValue(id + 'UM_countValue', this.value);
        }, false);

         document.getElementById('UM_priceValue').addEventListener('keyup', function() {
            GM_setValue(id + 'UM_priceValue', this.value);
        }, false);
    ...
    if (GM_getValue(id + 'UM_OP_unmodon', true)) {
        if (a.search('&unmod=true') != -1) {
            ...
            if (GM_getValue(id + 'UM_OP_questItemListOn', true) && a.search('msg&do=view') != -1 && isQuest()) {
                ...
                if (itemsCol.length) {
                    ...
                    for (var i = 0; i < itemsArr.length; i++) {
                        ...
                        if (itemsArr[i].innerText.search("Epick") > -1) {
                            epic.push(itemsArr[i]);
                        } else if (itemsArr[i].innerText.search("Legendarn") > -1) {
                            if (itemsArr[i].innerText.search("Dosk") > -1) {
                                legdsk.push(itemsArr[i]);
                            } else if (itemsArr[i].innerText.search("Dobr") > -1) {
                                legdbr.push(itemsArr[i]);
                            } else {
                                leg.push(itemsArr[i]);
                            }
                        } else {
            ...
            if (a.match('.+a=equip.*')) {
                cl = document.getElementsByClassName('equip')[0];
                var loadItems = document.getElementById('loadArmoryItemsButton');
                loadItems.click();
                var loadClanItems = document.getElementById('loadClanArmoryItemsButton');
                loadClanItems.click();
                div = cl.getElementsByTagName('div');
                z = 20;
                if (div.length < 40) {
            ...
            if (a.substring(0, 8) == '?a=equip') {
                ...
                if (GM_getValue(id + 'UM_OP_fastzk', true)) {
                    ...
                    for (var i = 0; i < itemS.length; i++) {
                        ...
                        i//f (!krew) ta[0].addEventListener('mousedown', function() {
                ...
                // mod clean-zk
                polka = document.getElementById('hc_c19');
                if (document.getElementById('hc_c19')) {
                    ...
                    var rand = Math.round(Math.random() * (500 - 2000)) + 2000;
                    if(GM_getValue(id + 'UM_OP_autoczyszczenie', true)){
                        window.addEventListener('load', function() {
                                        setTimeout(function() {
                        polka = document.getElementById('hc_c0');
                        itemS = polka.getElementsByClassName('item');
                        var itemsCountToDelete = 40
                        for (i = 0; i < itemS.length; i++) {
                            ...
                            nazwa = (itemS[i].innerText).split('\t')[0].trim()
                            console.log(nazwa)
                            koszt = (itemLink.substring((itemLink.search('<b>') + 12), itemLink.search('</b>')).replace(/ /gi, ''));
                            if (parseInt(koszt) < GM_getValue(id + 'UM_zkclean', '25000') && parseInt(koszt) > 49 && nazwa.match('Epi.+') === null && itemsCountToDelete > 0) {
            ...
            if (a.substring(0, 9) == '?a=ambush') {
                if (document.getElementById('atkTimeLeft')) {
                    ...
                    if (document.getElementsByClassName('error')) {} else {
                ...
                if (table) {
                    ...
                    for (i = 1; i < tr.length; i++) {
                        ...
                        if (kw) {
                            ...
                            var teraz = new Date();
                            teraz = Date.now(); //teraz.getTime()+unsafeWindow.timeDiff*1000);
                            ...
                            if ((pozniej.getTime() - teraz) < roznica) {
                                roznica = pozniej.getTime() - teraz
                            }
                            var ijk = setInterval(function() {
                                roznica -= 1000;
                                if (roznica <= 0) {
            ...
            var now = new Date();
            var arena = false
            if (now.getDay() == 1 || now.getDay() == 4 || now.getDay() == 0) {
                arena = true
                var a1 = setInterval(function() {
                    ...
                    //{
                    var roznica = ((21 - hours) * 60 + 10 - minutes) * 60 - seconds
                    if (roznica == 300) {
                        notification('Uwaga: Koniec zapisów areny za 5 minut!');
                    } else if (roznica == 3*60) {
                        notification('Uwaga: Koniec zapisów areny za 3 minuty!');
                        //document.getElementById('unmodarena').innerHTML = roznica
                    } else if (roznica == 2*60) {
                        notification('Uwaga: Koniec zapisów areny za 2 minuty!');
                        //document.getElementById('unmodarena').innerHTML = roznica
                    } else if (roznica < 60 && roznica % 2 == 0 && roznica > 0) {
                        var audio2 = new Audio('https://www.soundjay.com/buttons/beep-24.mp3');
                        audio2.volume = 0.6;
                        audio2.play();
                    } else if (roznica > 0) {
                        document.getElementById('unmodarena').innerHTML = new Date(roznica * 1000).toISOString().substr(11, 8)
                    } else {
                        document.getElementById('unmodarena').innerHTML = "21:10 KONIEC ZAPISÓW"
                        clearInterval(a1);
                    }
                    //}
                }, 1000);

                //if (hours == 21 && minutes > 5 && minutes < 10) {
                //notification('Uwaga: Arena dzisiaj!');
                //}
            }
            ...
            if (a.substring(0, 22) == '?a=auction&do=new'){
                sellScrapForPR()
            }
            ...
            function zaznaczacz(txt, check) {
                ...
                for (i = 0; i < tr.length; i++) {
                    ...
                    if (td.length == 4)
                        if (td[3].innerHTML.length == 19)
                            if (td[1].innerHTML.search(txt) > 0) td[0].getElementsByTagName('input')[0].checked = check;
                }
            }
            ...
            function zaznaczacz_res(check) {
                ...
                for (i = 0; i < tr.length; i++) {
                    ...
                    if (td.length == 4)
                        if (td[3].innerHTML.length == 19)
                            if (td[1].innerHTML.search('zasadzkę') < 1 && td[1].innerHTML.search('Arena - ') < 1 && td[1].innerHTML.search(' ofertę handlową') < 1 && td[1].innerHTML.search('Wygrana licytacja!') < 1 && td[1].innerHTML.search('Twoja aukcja została') < 1 && td[1].innerHTML.search('Twoja oferta została') < 1 && td[1].innerHTML.search('Raport z wyprawy.') < 1 && td[1].innerHTML.search('Raport z ekspedycji - ') < 1 && td[1].innerHTML.search('Król Wzgórza - ') < 1 && td[1].innerHTML.search('Oblężenie na ') < 1 && td[1].innerHTML.search('Walka na Arenie ') < 1) td[0].getElementsByTagName('input')[0].checked = check;
                }
            }
            ...
            if ('?a=msg' == a.substring(0, 6) || '?mid=' == a.substring(0, 5) || document.getElementsByClassName('rlc')) {
                               rlcAll = document.getElementsByClassName('rlc');
                for (rlcInt = 0; rlcInt < rlcAll.length; rlcInt++) {
                    var roundPlayers = [];
                    rundy = rlcAll[rlcInt].querySelectorAll('ul.round')
                    links = rlcAll[rlcInt].querySelectorAll('div.round');
                    for (z = 0; z < rundy.length; z++) {
                        rlc = rlcAll[rlcInt];
                        rlc2 = rlcAll[rlcInt];
                        links[z].innerHTML = '<p class="flip" onclick="showToggleStats()">' + links[z].innerHTML + '</p>'
                        if (rlc) {
                            for (zmiana = 0; zmiana < 1; zmiana++) {
                                var stan = new Array();
                                var wyprowadzonych = new Array();
                                var kolor = new Array();
                                var unik = new Array();
                                ...
                                var otrzymane = new Array();
                                var uniknione = new Array();
                                var otrzymanydmg = new Array();
                                var zadanydmg = new Array();
                                var zadanydmgsojusznikom = new Array();
                                var trafione = new Array();
                                var zgon = new Array();
                                s = -1;
                                rlc = rlc2[zmiana];

                                var zgony = rundy[z].querySelectorAll('li.playerDeath')
                                for (i = 0; i < zgony.length; i++) {
                                    if (zgony[i].innerText.match(".+na polu walki.$")) {
                                        zgon[i] = zgony[i].getElementsByTagName('B')[0].innerHTML;
                                    }
                                }
                                var whoattack =  document.getElementsByClassName('amblist rlr fll')[0]
                                if(!whoattack) {
                                    whoattack = document.getElementsByClassName('clanSecOwner')[0].innerText
                                    stan[0]= whoattack
                                    kolor[0] = 'atkHit'
                                }
                                else{
                                    whoattack = whoattack.querySelectorAll('ul')[0].children;
                                    for (i = 0; i < whoattack.length; i++) {
                                        stan[i] = whoattack[i].innerText.replace(" (s)","")
                                        kolor[i] = 'atkHit'
                                    }
                                }
                                var whodefend =  document.getElementsByClassName('amblist rll flr')[0]
                               if(!whodefend) {
                                    whodefend = document.getElementsByClassName('clanSecOwner')[1].innerText
                                    stan+= ","+whodefend
                                    kolor+= ","+'defHit'
                                }
                                else{
                                    whodefend = whodefend.querySelectorAll('ul')[0].children;
                                    for (i = 0; i < whodefend.length; i++) {
                                        stan += ","+whodefend[i].innerText.replace(" (s)","")
                                        kolor += ',defHit'
                                    }
                                }
                                ...
                                kolor = kolor.split(',')
                                //walka = rlc.getElementsByClassName('atkHit');
                                walka = rundy[z].querySelectorAll('li.atkHit')
                                for (i = 0; i < walka.length; i++) {
                                    kto = walka[i].getElementsByTagName('B')[0].innerHTML;
                                    //kto
                                    s = stan.indexOf(kto)
                                    if (walka[i].innerHTML.search('Żar Krwi') < 1) {
                                        ...
                                        if (walka[i].innerHTML.search(' zostaje zranion') > 0) {
                                            ...
                                            ile = walka[i].innerText.match('[0-9]+ PKT')[0].match('[0-9]+')[0]
                                            //stan[d] = komu;
                                            if(kolor[d]  == undefined) kolor[d] = 'defHit';
                                            if (wyprowadzonych[d] == undefined) wyprowadzonych[d] = 0;
                                            if (kryty[d] == undefined) kryty[d] = 0;
                                            if (unik[d] == undefined) unik[d] = 0;
                                            if (otrzymane[d] == undefined) otrzymane[d] = 0;
                                            if (otrzymanydmg[d] == undefined) otrzymanydmg[d] = 0;
                                            if (zadanydmg[s] == undefined) zadanydmg[s] = 0;
                                            if (zadanydmgsojusznikom[s] == undefined) zadanydmgsojusznikom[s] = 0;
                                            if (trafione[s] == undefined) trafione[s] = 0;
                                            otrzymane[d]++;
                                            otrzymanydmg[d] += parseInt(ile);
                                            if(kolor[s] != kolor[d])
                                            {
                                            trafione[s]++;
                                            zadanydmg[s] += parseInt(ile);
                                            if (kryty[s] == undefined) kryty[s] = 0;
                                            if (walka[i].innerHTML.search('cios krytyczny') > 0) kryty[s]++;
                                            }
                                            else{
                                            zadanydmgsojusznikom[s] += parseInt(ile);
                                            }
                                        } else
                                        if (walka[i].innerHTML.search(' trafi') > 0 && walka[i].innerHTML.search('luzję') == -1) {
                                            komu = walka[i].getElementsByTagName('b')[walka[i].getElementsByTagName('b').length - 1].innerHTML;
                                ...
                                //walka = rlc.getElementsByClassName('defHit');
                                //walka = document.querySelectorAll('ul.round')
                                walka = rundy[z].querySelectorAll('li.defHit')
                                for (i = 0; i < walka.length; i++) {
                                    kto = walka[i].getElementsByTagName('B')[0].innerHTML;
                                    //kto
                                    s = stan.indexOf(kto)
                                ...
                                sum = rundy[z]
                                var child = document.createElement("ui");
                                child.style.display = "none";
                                child.setAttribute("id", "sumround");
                                if (!sum.length) {
                                    //sum = document.getElementsByClassName('result') [document.getElementsByClassName('result').length - 1];
                                    raport = '<ui><table border="0" style="border-collapse: collapse; background: black; text-align: center;" width="100%"><tr>';
                                } else {
                                    raport = '<ui><table border="0" style="border-collapse: collapse; text-align: center;" width="95%"><tr>';
                                    sum = sum[zmiana];
                                }
                                ...
                                for (i = 0; i < stan.length; i++) {
                                    ...
                                    player.nazwa = stan[i];
                                    if (wyprowadzonych[i]) {
                                        player.wyprowadzonych = parseInt(wyprowadzonych[i]);
                                    } else player.wyprowadzonych = 0
                                    //if(player.nazwa.includes("*")){
                                    //player.kolor = 0
                                    //}
                                    //else{
                                    player.kolor = kolor[i];
                                    //}
                                    if (unik[i]) {
                                        player.unik = parseInt(unik[i]);
                                    } else player.unik = 0
                                    if (kryty[i]) {
                                        player.kryty = parseInt(kryty[i]);
                                    } else player.kryty = 0
                                    if (otrzymane[i]) {
                                        player.otrzymane = parseInt(otrzymane[i]);
                                    } else player.otrzymane = 0
                                    if (uniknione[i]) {
                                        player.uniknione = parseInt(uniknione[i]);
                                    } else player.uniknione = 0
                                    if (otrzymanydmg[i]) {
                                        player.otrzymanydmg = parseInt(otrzymanydmg[i]);
                                    } else player.otrzymanydmg = 0
                                    if (zadanydmg[i]) {
                                        player.zadanydmg = parseInt(zadanydmg[i]);
                                    } else player.zadanydmg = 0
                                    if (zadanydmgsojusznikom[i]) {
                                        player.zadanydmgsojusznikom = parseInt(zadanydmgsojusznikom[i]);
                                    } else player.zadanydmgsojusznikom = 0
                                    if (trafione[i]) {
                                        player.trafione = parseInt(trafione[i]);
                                    } else player.trafione = 0


                                    if (zgon.includes(player.nazwa)) player.zgon = true;
                                    if (document.getElementsByClassName('amblist rlr fll').length) {
                                        var attackers = document.getElementsByClassName('amblist rlr fll')[rlcInt].children[0].children;
                                        var defenders = document.getElementsByClassName('amblist rll flr')[rlcInt].children[0].children;

                                        for (v = 0; v < attackers.length; v++) {
                                            var attacker = attackers[v]
                                            var nazwa = attackers[v].outerText
                                            if (nazwa.includes(player.nazwa)) {
                                                var hpline = attackers[v].outerHTML.match("PKT ŻYCIA:.+<br/>")
                                                var inicjatywaline = attackers[v].outerHTML.match("INICJATYWA:.+</b>")
                                                var poziomline = attackers[v].outerHTML.match("Poziom:.+<br/>")
                                                var HP = hpline[0].match("[0-9]+")
                                                var inicjatywa = inicjatywaline[0].match("[0-9]+")
                                                var poziom = poziomline[0].match("[0-9]+")
                                                player.StartHP = parseInt(HP[0])
                                                player.Inicjatywa = parseInt(inicjatywa[0])
                                                player.Poziom = parseInt(poziom[0])
                                                if (roundPlayers.length == 0) {
                                                    player.ActualHP = player.StartHP - player.otrzymanydmg
                                                } else {
                                                    for (dj = 0; dj < roundPlayers[z - 1].length; dj++) {
                                                        if (player.nazwa.includes(roundPlayers[z - 1][dj].nazwa)) {
                                                            player.StartHP = roundPlayers[z - 1][dj].ActualHP
                                                            player.ActualHP = roundPlayers[z - 1][dj].ActualHP - player.otrzymanydmg
                                                        }
                                                    }
                                        ...
                                        for (v = 0; v < defenders.length; v++) {
                                            var attacker = defenders[v]
                                            var nazwa = defenders[v].outerText
                                            if (nazwa.includes(player.nazwa)) {
                                                var hpline = defenders[v].outerHTML.match("PKT ŻYCIA:.+<br/>")
                                                var inicjatywaline = defenders[v].outerHTML.match("INICJATYWA:.+</b>")
                                                var poziomline = defenders[v].outerHTML.match("Poziom:.+<br/>")
                                                var HP = hpline[0].match("[0-9]+")
                                                var inicjatywa = inicjatywaline[0].match("[0-9]+")
                                                var poziom = poziomline[0].match("[0-9]+")
                                                player.StartHP = parseInt(HP[0])
                                                ...
                                                if (roundPlayers.length == 0) {
                                                    ...
                                                    for (djd = 0; djd < roundPlayers[z - 1].length; djd++) {
                                                        if (player.nazwa.includes(roundPlayers[z - 1][djd].nazwa)) {
                                                            player.StartHP = roundPlayers[z - 1][djd].ActualHP
                                                            player.ActualHP = roundPlayers[z - 1][djd].ActualHP - player.otrzymanydmg
                                                        }
                                                    }
                                ...
                                roundPlayers.push(playerList);
                                playerList.sort(function(a, b) {
                                    var keyA = a.zadanydmg,
                                        keyB = b.zadanydmg;
                                    // Compare the 2 dates
                                ...
                                for (i = 0; i < stan.length; i++) {
                                    ...
                                    if (playerList[i].nazwa.includes("*")) niceOne = 'border: 2px white dotted;';
                                    if (playerList[i].zgon == true) raport += '<td style="' + niceOne + ' padding: 6px; text-align: center;"><table style="margin-left: auto; margin-right: auto;"><tr><td style = "border: 2px white dotted;border-color: red;padding: 6px; text-align: center;"><b class="' + playerList[i].kolor + '">' + playerList[i].nazwa + '</b><BR></td></tr></table>';
                                    else {
                                        raport += '<td style="' + niceOne + ' padding: 6px; text-align: center;"><b class="' + playerList[i].kolor + '">' + playerList[i].nazwa + '</b><BR>';
                                    }
                                    raport += 'Trafione: <b>' + playerList[i].trafione + '</b> / <b>' + playerList[i].wyprowadzonych + '</b> ';
                                    if ((playerList[i].wyprowadzonych - playerList[i].unik) / playerList[i].wyprowadzonych * 100) raport += '(' + ((playerList[i].trafione / playerList[i].wyprowadzonych * 100).toFixed(2)) + '%)';
                                    raport += '<br>Krytycznych: <b>' + playerList[i].kryty + '</b> ';
                                    if (playerList[i].kryty) raport += '(' + (((playerList[i].kryty) / (playerList[i].trafione) * 100).toFixed(2)) + '%)';
                                    if (playerList[i].uniknione == undefined) playerList[i].uniknione = 0;
                                    if (playerList[i].otrzymane == undefined) playerList[i].otrzymane = 0;
                                    raport += '<br>Otrzymane: <b>' + playerList[i].otrzymane + '</b> / <b>' + (playerList[i].otrzymane + playerList[i].uniknione) + '</b>';
                                    if (playerList[i].otrzymane / (playerList[i].otrzymane + playerList[i].uniknione) * 100) raport += ' (' + (playerList[i].otrzymane / (playerList[i].otrzymane + playerList[i].uniknione) * 100).toFixed(2) + '%)';
                                    if (playerList[i].otrzymanydmg)
                                        raport += '<br>Otrzymany dmg: <b>' + Intl.NumberFormat().format(playerList[i].otrzymanydmg) + '</b>';
                                    if (playerList[i].zadanydmg)
                                        raport += '<br>Zadany dmg: <b>' + Intl.NumberFormat().format(playerList[i].zadanydmg) + '</b>';
                                    if (playerList[i].zadanydmgsojusznikom)
                                        raport += '<br>Zadany dmg sojusznikom: <b>' + Intl.NumberFormat().format(playerList[i].zadanydmgsojusznikom) + '</b>';
                                    if (roundPlayers[z][i].StartHP)
                                        raport += '<br>Startowy HP: <b>' + new Intl.NumberFormat().format(roundPlayers[z][i].StartHP) + '</b>';
                                    if (roundPlayers[z][i].ActualHP)
                                        raport += '<br>Aktualny HP: <b>' + new Intl.NumberFormat().format(roundPlayers[z][i].ActualHP) + '</b>';
                                    raport += '<br>Inicjatywa: <b>' + new Intl.NumberFormat().format(playerList[i].Inicjatywa) + '</b>';
                                    raport += '<br>Poziom: <b>' + new Intl.NumberFormat().format(playerList[i].Poziom) + '</b>';
                                    raport += '</td>';

                                }
                                //sum.innerHTML += raport + '</tr></table>';
                                sum.before(child);
                                child.innerHTML = (raport + '</tr></table><hr>');
                            }
                        }
                    }
                }
            ...
            if (a.substring(0, 11) == '?a=auction&' && a.search('&t=69') > 0) {
                ...
                for (i = 1; i < tr.length; i++) {
                    td = tr[i].getElementsByTagName('TD');
                    sztuk = td[1].innerHTML.substring(td[1].innerHTML.search(':') + 1).replace('</span>                <div style="margin: 5px;">\n                                    </div>', '');
            ...
            if (krok != -1) {
                ...
                if (roznicak > 0) {
                    ...
                    var i2 = setInterval(function() {
                        roznicak -= 1000;
                        if (roznicak == 1800000 || roznicak == 300000 || roznicak == 150000 || roznicak == 50000) {
                            notification('Przebierz się jeśli trzeba');
                        }
                ...
                if (roznicaa > 0) {
                    ...
                    var i4 = setInterval(function() {
                        ...
                        if (roznicaa <= 0) {
                            ...
                            audio.volume = 0.3;
                            if (przypomnienie == 10 * 60) {
                                notification('Uwaga: prawdopodobny koniec aukcji za 10 minut!');
                            } else
                            ...
                            if (przypomnienie < 1 * 60) {
                                ...
                                //console.log(div)
                                if (przypomnienie % 2 == 0 && przypomnienie > 40) {

                                    //audio.play();
                                } else {
                                    bidAuction();
                                    var winningSon = document.getElementsByClassName('auctionRow');
                                    if (winningSon.length > 0) {
                                        winningSon = winningSon[0].children[0].innerHTML.includes("Twoja oferta wygrywa");
                                        if (przypomnienie < 15 && !winningSon) {
                                            audio.play();
                                        }
                                    } else {
                                        if (przypomnienie < 40) {
            ...
            if (arena) div.innerHTML += '<a href="?a=newarena"><span style="color: red;">ARENA:</span> <span class="enabled" id="unmodarena">00:00:00</span></a>&nbsp;&nbsp;';
                ...
                if (test - Math.floor(Date.now() / 1000) > 0) {
                    setTimeout(function() {
                        if (GM_getValue(id + 'UM_wyprawa', 0)) {
                            GM_setValue(id + 'UM_wyprawa', 0);
                            var menu = document.getElementsByClassName('menu')[13].outerText
                            var wyprawy = document.getElementsByClassName('menu')[19].outerText
                            console.log(menu)
                            console.log(wyprawy)
                            if(menu == "Pamiątki"){
                                menu = document.getElementsByClassName('menu')[14].outerText
                                wyprawy = document.getElementsByClassName('menu')[20].outerText
                            }
                            if(wyprawy == "Idź na wyprawę"){
                                notification('Wyprawy zakończone');
                            }
                            if(menu == "Zbrojownia · 16"){
                                window.location = 'https://r14.bloodwars.interia.pl/?a=quest';
                            }
                        }
                    }, (test - Math.floor(Date.now() / 1000)) * 1000);
                } else {
            ...
            if (czas > 0) {
                ...
                var si = setInterval(function() {
                    ...
                    if (val > 0) {
                        ...
                        if (GM_getValue(id + 'UM_OP_donesound', false)) {
                            document.body.appendChild(document.createElement('div'));
                            document.body.lastChild.innerHTML = '<audio src="' + GM_getValue(id + 'UM_urlsound', 'https://media.geeksforgeeks.org/wp-content/uploads/20190531135120/beep.mp3') + '" autoplay=true></audio>';
                        }
                        clearInterval(si);
                    }
                }, 1000);
...
function showToggleStats() {
    var x = document.querySelectorAll("#sumround");
