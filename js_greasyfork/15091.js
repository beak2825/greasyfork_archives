// ==UserScript==
// @name        C&C:TA Attack Formation Saver
// @namespace   CnC_TA_Attack_Formation_Saver   
// @author  	Vulcano
// @version     1.0.0.2
// @date        2015-07-25
...
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/15091/CC%3ATA%20Attack%20Formation%20Saver.user.js
// @updateURL https://update.greasyfork.org/scripts/15091/CC%3ATA%20Attack%20Formation%20Saver.meta.js
// ==/UserScript==

(function (){
    console.log("C&C:TA Attack Formation Saver loading ...");    
    var AFS_main = function() {
    var windowSaver;
      ...
      qx.Class.define("webfrontend.gui.PlayArea.FormationSaver", {
        ...
        members: {
          ...
          init: function() {          
            ...
            labelTitle.setTextColor("#FF0000");
            ba.add(labelTitle);
...
//ALT       this.containerMain=new qx.ui.container.Composite(new qx.ui.layout.VBox().set({alignX:"right"})).set({maxHeight:webfrontend.gui.PlayArea.FormationSaver.SaverExpandedHeight,width:75,minHeight:32,allowShrinkY:true,decorator:new qx.ui.decoration.VBox().set({baseImage:"webfrontend/ui/common/bgr_mission_tracker.png"})});
          ...
          update: function() {
            ...
            for(var id in formations[cityID][ownCityID]) {
              if(id != 0) {
                ...
                labelLoad.setTextColor("#009B00");
                labelLoad.addListener("click", this.clickLoad(formation), this);
                labelLoad.addListener("mouseover", this.mouseover(labelLoad, "#00FF00"), this);
                labelLoad.addListener("mouseout", this.mouseout(labelLoad, "#009B00"), this);
                ...
                labelDelete.addListener("mouseover", this.mouseover(labelDelete, "#FF0000"), this);
          ...
          save: function() {
            try {
              ...
              if(armyUnits == null) {
                console.log("AFS Error: You must move a unit befor saving!");
                return;
              }
          ...
          load: function(newFormation) {
            try {
              ...
              var armyUnits = formation.get_ArmyUnits();
              if(armyUnits == null) {
                console.log("AFS Error: You must move a unit befor loading!");
                return;
              }
              armyUnits = armyUnits.l;
      ...
      if(!ClientLib.Data.MainData.GetInstance().get_Cities().__AFS__set_CurrentOwnCityId) {
        ClientLib.Data.MainData.GetInstance().get_Cities().__AFS__set_CurrentOwnCityId = ClientLib.Data.MainData.GetInstance().get_Cities().set_CurrentOwnCityId;
      }
      ClientLib.Data.MainData.GetInstance().get_Cities().set_CurrentOwnCityId = function(a) {
        this.__AFS__set_CurrentOwnCityId(a); 
        updateView();
      }
      
      if(!ClientLib.Data.MainData.GetInstance().get_Cities().__AFS__set_CurrentCityId) {
        ClientLib.Data.MainData.GetInstance().get_Cities().__AFS__set_CurrentCityId = ClientLib.Data.MainData.GetInstance().get_Cities().set_CurrentCityId;
      }
      ClientLib.Data.MainData.GetInstance().get_Cities().set_CurrentCityId = function(a) {
        this.__AFS__set_CurrentCityId(a); 
        updateView();
      }
      ...
      try {
        if (typeof qx != 'undefined') {
          ...
          if (a && mb) {
            ...
            window.setTimeout(AFS_checkIfLoaded, 1000);
        } else {
          window.setTimeout(AFS_checkIfLoaded, 1000);
        }
      } catch (e) {
  ...
  var AFSScript = document.createElement("script");
  AFSScript.innerHTML = "(" + AFS_main.toString() + ")();";
  AFSScript.type = "text/javascript";
  if (/commandandconquer\.com/i.test(document.domain)) {
    document.getElementsByTagName("head")[0].appendChild(AFSScript);
  }
  console.log("C&C:TA Attack Formation Saver loaded successfully !!!");    
})();