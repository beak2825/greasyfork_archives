// ==UserScript==
// @name         XJTLU xpcyjgy 自动登录
// @namespace    http://tampermonkey.net/
// @version      1.1
// @description  XJTLU(TC)线上自助服务自动登录 (支持移动端)
// @author       wujinjun
// @license      MIT
// @match        https://www.xpcyjgy.com/*
// @grant        none
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/551035/XJTLU%20xpcyjgy%20%E8%87%AA%E5%8A%A8%E7%99%BB%E5%BD%95.user.js
// @updateURL https://update.greasyfork.org/scripts/551035/XJTLU%20xpcyjgy%20%E8%87%AA%E5%8A%A8%E7%99%BB%E5%BD%95.meta.js
// ==/UserScript==
...
(function() {
    ...
    // --- 元素选择器定义 ---
    const USERNAME_SELECTOR = 'body > div.page-admin > div > div:nth-child(1) > div.nui-box-bd > div:nth-child(1) > input';
    const PASSWORD_SELECTOR = 'body > div.page-admin > div > div:nth-child(1) > div.nui-box-bd > div:nth-child(2) > input';
    const CHECKBOX_SELECTOR = 'body > div.page-admin > div > div:nth-child(1) > div.nui-box-bd > div.wjt-remember > label > input[type=checkbox]'; // 确保是checkbox
    const AGREE_SELECTOR = 'body > div.page-admin > div > div:nth-child(1) > div.nui-box-bd > div:nth-child(4) > div.wjt-mt30.wjt-agreement > label > input';
    const LOGIN_BUTTON_SELECTOR = 'body > div.page-admin > div > div:nth-child(1) > div.nui-box-bd > div:nth-child(4) > div.nui-mt10 > button';

    // --- 进度显示逻辑 ---
    let progressIndicator = null;
    const HELPER_PREFIX = "[XJTLU xpcyjgy login helper] "; // 新增前缀常量
    ...
    function main() {
        ...
        updateProgress('脚本初始化中(或不在登录页)...', 'info');
        ...
        .then((inputStable) => {
            ...
            const agreeEl = document.querySelector(AGREE_SELECTOR);
            const loginButtonEl = document.querySelector(LOGIN_BUTTON_SELECTOR);
            let checkInterval = null;
            let initialUrl = window.location.href;

            updateProgress('账号和密码输入稳定，准备勾选复选框 (1/3 完成)', 'info');

            // 步骤 2：处理单选框/复选框
            if (checkboxEl) {
                if (!checkboxEl.checked) {
                    console.log('步骤 2：勾选复选框1(记住密码)。');
                    checkboxEl.click();
                } else {
                    console.log('步骤 2：复选框1已勾选，跳过。');
                }
                updateProgress('复选框1处理完毕', 'info');
            } else {
                console.warn('复选框1元素未找到。');
                updateProgress('警告：复选框1未找到 (2/3)', 'info');
            }
            if (agreeEl) {
                if (!agreeEl.checked) {
                    console.log('步骤 2：勾选复选框2(我已阅读并同意《住宿管理协议》)。');
                    agreeEl.click();
                } else {
                    console.log('步骤 2：复选框2已勾选，跳过。');
                }
                updateProgress('复选框2处理完毕', 'info');
            } else {
                console.warn('复选框2元素未找到。');
                updateProgress('警告：复选框2未找到 (2/3 完成)', 'info');
            }
