...
// @downloadURL https://update.greasyfork.org/scripts/1610/TMD%20posters.user.js
// @updateURL https://update.greasyfork.org/scripts/1610/TMD%20posters.meta.js
// ==/UserScript==
...
var filterWords = ["MEM", "\\[ENG AUDIO\\]", "XViD\\W\\w*", "AC3\\W\\w*", "DVDRip", "AC3", "HDTVRip", "TS", "HQ", "\\[NO RAR\\]", "READNFO", "BDRip", "Hive\\]", "DVDScr", "1CDRip", "\\[DDR\\]", "x264", "Feel-Free", "PPVRIP", "IFLIX", "NFO", "LKRG", "\\.", "DVD-RERip", "\\W", "R5", "PPRIP", "NYDIC", "BRRiP", "T0XiCiNK", "befree", "CM8", "NEW\\WHQ\\WVIDEO", "NO\\WRAR", "UNRATED", "BR2DVD", "DD5\\.1", "NL", "Subs", "DMT", "\\(", "\\)"];

// "i" is to ignore case
var rgx = new RegExp(filterWords.join("|"), "gi");
var years = new RegExp("\\d\\d\\d\\d", "gi");

var thumb_height = 116;
$("#torrents > table > tbody > tr > td:nth-child(2) > a").each(function (index) {
    var link = $(this);
    //link.parent().parent().css("height",thumb_height+9+"px");
    link.css("vertical-align", "middle");
    var index1 = index;
    var torrentUrl = $(this).attr("href");
    $.get(torrentUrl, function (data) {
        var img = $("tbody > tr > td:nth-child(2) > img", $(data))
        var link1 = link;
        var index2 = index1;
        if (img.length != 0) {
            img.css({
            ...
            img.css("max-height", thumb_height + "px");
            img.attr("class", "thumb");
            img.attr("alt", "");
            link.prepend(img);
            if (index2 == $("tbody > tr > td:nth-child(2) > img").length - 1)
                done();
        } else {
            //no image found
            var imdbElm = $("a[href*='.imdb.com/title/']", $(data));
            if (imdbElm.length != 0) {
                //look for imdb link
                var imdbUrl = imdbElm.attr("href");
                $.getJSON("http://www.ig.gmodules.com/gadgets/proxy/refresh=90000000&container=ig/http://pipes.yahoo.com/pipes/pipe.run?_id=67ffda63df0c4b934bd4bebf1c40e4fb&_render=json&urlinput1=" + imdbUrl, function (data) {
                    if (index2 == $("div.td:nth-child(2) > img").length - 1)
                        done();
                    console.log(data);
                    if (data.count > 0) {
                        try {
                            var imdbThumb = data.value.items[0].content;
                            link1.prepend("<img class='thumb' style='max-height:" + thumb_height + "px;float:left' src='http://www.ig.gmodules.com/gadgets/proxy/refresh=3600&container=ig/" + imdbThumb + "'>");
                            console.log("prepended imdb image to(1)", link1);
                        } catch (err) {
                            console.error(err, imdbUrl, data);
                        }
                    } else {
                        console.error("0 items for", imdbUrl, torrentUrl);
                    }
                });
            } else {
                //no imdb link found, get title and search imdb and get poster from imdb
                var orgTitle, title;
                title = orgTitle = $("#title", $(data)).text();
                //title = jQuery.trim(title.substring(0,title.regexLastIndexOf(years)).replace(/\.|\_|\s|\(|\)/g, " ")).replace(/[ ]/g, "+");
                title = (jQuery.trim(title.toLowerCase().replace(/\./g, " ").replace(/(s[0-9]+e[0-9]+)|([0-9][0-9][0-9][0-9]).*?\s|(spa|sbs|chi|eng|ru)[\s,.\]\)]|(hdtv|dvd|720|3d|h-sbs|hardsu|bluray|hsbs|dts).*?\s|(x264|aac|xvid|ac3).*?\s|(avi|mkv|mpeg|mpg|mov|mp4)|(\[.*\])|(\.)/g, " ")));
                $.getJSON("http://www.ig.gmodules.com/gadgets/proxy/refresh=36&container=ig/http://pipes.yahoo.com/pipes/pipe.run?_id=c9af0c1883656031f96733a9fbfe0152&_render=json&title=" + title, function (data) {
                    if (data.count > 0) {
                        try {
                            var imdbUrl = data.value.items[0].content;
                            console.error("No imdb link found, title:", title, " original:", orgTitle, "\nGot imdb URL", imdbUrl);
                            imdbUrl = encodeURIComponent(imdbUrl.replace(/&#92;/g, ""));
                            var link2 = link1;
                            var index3 = index2;
                            $.getJSON("http://www.ig.gmodules.com/gadgets/proxy/refresh=3600&container=ig/http://pipes.yahoo.com/pipes/pipe.run?_id=67ffda63df0c4b934bd4bebf1c40e4fb&_render=json&urlinput1=" + imdbUrl, function (data) {
                                if (data.count > 0) {
                                    try {
                                        var imdbThumb = data.value.items[0].content;
                                        var link3 = link2;
                                        link3.prepend("<img class='thumb' style='max-height:" + thumb_height + "px;float:left' src='http://www.ig.gmodules.com/gadgets/proxy/refresh=3600&container=ig/" + imdbThumb + "'>");
                                        console.log("prepended imdb image to(2)", link3);
                                        if (index3 == $("div.td:nth-child(2) > img").length - 1)
                                            done();
                                    } catch (err) {
                                        console.error(err, "Failed to get image from ", imdbUrl)
                                    }
                                } else {
                                    console.error("0 items from imdb query, title:", title, "orgTitle:", orgTitle, "url:", imdbUrl);
                                }
                            });
                        } catch (err) {
                            console.error(err, "No imdb link found, title:", title, " original:", orgTitle, "\nGot imdb URL", imdbUrl);
                        }
                    } else {
                        console.error("No imdb entry found for query:", title, " original title:", orgTitle, "http://pipes.yahoo.com/pipes/pipe.run?_id=c9af0c1883656031f96733a9fbfe0152&_render=json&title=" + title, data);
                    }
                });
            }
        }
...
GM_addStyle("" +
    ...
    "   -moz-transition: -moz-transform 1000ms, opacity 1000ms, box-shadow 1000ms;" +
    "   -webkit-transition: -webkit-transform 1000ms, opacity 1000ms, box-shadow 1000ms" +
    "}" +
    ".thumb:hover {" +
