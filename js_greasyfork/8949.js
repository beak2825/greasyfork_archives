...
// @namespace		KoCDomSel
// @description		Ce script recupere vos jetons merlins tous les jours depuis les publis Facebook!
...
// @downloadURL https://update.greasyfork.org/scripts/8949/KoC%20Merlin%20Token%20Domain%20Selector%20FRANCAIS.user.js
// @updateURL https://update.greasyfork.org/scripts/8949/KoC%20Merlin%20Token%20Domain%20Selector%20FRANCAIS.meta.js
// ==/UserScript==
...
function AddMainTabLink(text, eventListener, mouseListener) {
	...
	a.className='tab';
	a.title='Avez vous collectes vos jetons merlin aujourdhui?';
	...
	unsafeWindow.FB.login(function (o) {
		if (o.authResponse) {
			...
			unsafeWindow.FB.api('/me/friends', p, function(result) {
				...
				var markup = '';
				var FriendList = {0:'(Tout le monde)'};
...
function TokenPopup (){
	...
	if (TokenPop) {
		...
		n += '<tr><td width=50 class=xtab>Domaine de reception des Jetons:&nbsp;</td><td class=xtab><input type=text id=tkdomain size=2 maxlength=3 class=btInput value="'+KOCAutoAcceptGifts.options.UserDomain+'"></td><td class=xtab align=right><b>Activer&nbsp;<input type=checkbox id=tkenable '+(KOCAutoAcceptGifts.options.Enabled?'CHECKED':'')+'></b></td></tr>';
		n += '<tr><td class=xtab>Lien des Domaines pour coffres:&nbsp;</td><td class=xtab colspan=2><input type=text id=tkchestdomainlist size=47 class=btInput value="'+KOCAutoAcceptGifts.options.ChestDomainList+'" title="Liste des domaines separes par une virgule."></td></tr>';
		n += '</table>';
		n += '<br><table align=center width=95% cellspacing=0 cellpadding=0><tr><td colspan=2 class=xtab align=left><b><i>Collecte des Jetons</i></b></td></tr>';
		n += '<tr><td colspan=2 class=xtab align=right><a id=tkgrabchest class="inlineButton btButton brown8"><span>Chercher Coffres</span></a>&nbsp;&nbsp;Nbr de Posts<input title="Entrer le numbre de posts ou vous desirez rechercher." type=text id=tksearchnum size=2 maxlength=3 class=btInput value="'+KOCAutoAcceptGifts.options.SearchNum+'">&nbsp;&nbsp;Votre Mur<input title="Cocher pour rechercher sur votre mur. Decocher pour rechercher sur toutes les publis FB." type=checkbox id=tkyourwall '+(KOCAutoAcceptGifts.options.YourWall?'CHECKED':'')+'>&nbsp;Rev<input title="Tick to search posts in reverse order, i.e. oldest first." type=checkbox id=tkrev '+(KOCAutoAcceptGifts.options.Reversed?'CHECKED':'')+'></td></tr>';
		n += '<tr><td class=xtab colspan=2 align=right><a id=RefreshFriendList title="Click to refresh friends list">Recherche Amis</a>&nbsp;'+htmlSelector(FriendList,KOCAutoAcceptGifts.options.SelectedFriendId,'id=selectfriend class=btInput style="width:150px;" title="Selection amis pour vos recherches"')+'&nbsp;-&nbsp;Fb Id:&nbsp;<input type=text id=tksearchuser size=20 class=btInput value="'+KOCAutoAcceptGifts.options.SearchUser+'" title="Laisser vide pour rechercher chez tous vos amis"></td>';
		n += '<tr><td class=xtab>Lien:&nbsp;</td><td class=xtab align=right><input type=text id=tklink size=80 class=btInput title="Copier et coller votre Construction, depuis votre lien FB!"></td></tr>';
		n += '<tr><td class=xtab align=right colspan=2><a id=tkcleanwall class="inlineButton btButton brown8" title="Efface les coffres deja recuperes  (et les autres posts Kabam) de votre mur FB"><span>Nettoyer votre mur</span></a>&nbsp;<a id=tktokenbmk class="inlineButton btButton brown8"><span>Sauver Jeton</span></a>&nbsp;<a id=tkbuildbmk class="inlineButton btButton brown8"><span>Sauver Construct</span></a></td></tr>';
		n += '</table>';
		n += '<br><table align=center width=95% cellspacing=0 cellpadding=0>';
		n += '<tr><td width=33% class=xtab align=center><a id=tktokenlink><img height=40 style="vertical-align:text-top;" src="'+TokenImage+'" title="'+KOCAutoAcceptGifts.options.TokenLink+'"></a><br><span id=tktokencollected>&nbsp;</span></td><td width=33% class=xtab align=center><a id=tkbuildlink><img height=40 style="vertical-align:text-top;" src="'+BuildImage+'" title="'+KOCAutoAcceptGifts.options.BuildLink+'"></a><br><span id=tkbuildcollected>&nbsp;</span></td><td width=33% class=xtab align=center><a id=tkchestlink><img height=40 style="vertical-align:text-top;" src="'+ChestImage+'" title="nvoyer pour recuperer en fonction du lien demander ci dessous..."></a><br><a id=tkprior><<</a>&nbsp;<input type=text id=tkchestdomain size=2 maxlength=3 class=btInput value="'+KOCAutoAcceptGifts.options.ChestDomain+'" title="Enter a domain you do not play to collect chests from your own wall!">&nbsp;<a id=tknext>>></a></td></tr>';
		n += '</table>';
		n += '<div align="center" style="font-size:10px;opacity:0.6;">Barbarrossa KoC Domain Selector '+Version+'<br>'+KOCAutoAcceptGifts.options.TokenCount+' Jetons collectes aujourdhui.<br>Vous possedez actuellement <span id=tknum>'+NumTokens+'</span> jetons.</div>';
		...
		TokenPop.getTopDiv().innerHTML = '<DIV align=center><B>COLLECTEUR JETONS Traduit par LastTemplar/KnightCathar</B></DIV>';
		TokenPop.getMainDiv().innerHTML = n;

		if (KOCAutoAcceptGifts.options.TokenCollected) {
			document.getElementById('tktokencollected').innerHTML = '<span style="color:#080;"><b>Collecte</b></span>';
		}
		if (KOCAutoAcceptGifts.options.BuildCollected) {
			document.getElementById('tkbuildcollected').innerHTML = '<span style="color:#080;"><b>Collecte</b></span>';
		}
		...
		document.getElementById('tkbuildbmk').addEventListener('click', function () {
			if (document.getElementById('tklink').value != "") { 
				...
				document.getElementById('tkbuildcollected').innerHTML = '<span style="color:#f00;"><b>Aucun Lien</b></span>';			
			}
		}, false);
		document.getElementById('tktokenbmk').addEventListener('click', function () {
			if (document.getElementById('tklink').value != "") { 
				...
				document.getElementById('tktokencollected').innerHTML = '<span style="color:#f00;"><b>Aucun Lien</b></span>';			
			}
		}, false);
		...
		document.getElementById('tkgrabchest').addEventListener('click', function () {
			...
			if (FBUser != '' && !KOCAutoAcceptGifts.options.YourWall) {
				document.getElementById('tkmessage').innerHTML = 'Recherche coffre par '+FBUser+'...';
			}
			else {
				document.getElementById('tkmessage').innerHTML = 'Recherche des coffres...';
			}
		...
		document.getElementById('tkcleanwall').addEventListener('click', function () {
			document.getElementById('tkmessage').innerHTML = 'Recherche les posts Kabam sur votre mur...';
			document.getElementById('tklink').value = '';
			...
			if (!likes_found) {
				if (KOCAutoAcceptGifts.options.YourWall) {
					...
					unsafeWindow.FB.api('/' + post.id + '/likes', "POST", ClaimChest.p, function (result) {
						if (result && !result.error) {
							document.getElementById('tklink').value = post.link;
							document.getElementById('tkmessage').innerHTML = '<span style="color:#080;"><b>Coffre trouver provenant de '+post.from.name+'! :)</b></span>';
							KOCAutoAcceptGifts.Log(JSON2.stringify(post));
						}
						else {
							ClaimChest.CheckNext();
...
var KOCAutoAcceptGifts = {
	...
	GetOptions : function () {
		...
		var defOptions = {
			UserDomain : 457,
			Enabled : true,
			ChestDomain : '457,415,454,453,452,451,450,448,348',
			BuildLink : '',
			TokenLink : '',
			TokenDate : 0,
			TokenCount : 0,
					...
					if (domain_selector && (next_button1 || next_button2)) {
						...
						if (next_button3) {
							...
							if (next_button2 || back_button) {
								t.giftAccepted = true;
								t.Log("Le jeton Merlin a pas pu etre collecte :(");

								var a = document.createElement('div');
								a.innerHTML = '<div align=center><br><i>Jeton Merlin non collecte.<br>(KoC sera recharge dans 10 secondes)</i></div>';
								...
								if (domain_selector == null && (typeof unsafeWindow.checkServer == 'function')) {
									...
									if (FeedID !='n/a'){
										...
										t.Log("Jetons Merlins collectes :)");
										CheckTokenDay();
										t.options.TokenCount = t.options.TokenCount + 1;
										t.SetOptions(t.options);
										...
										var a = document.createElement('div');
										a.innerHTML = '<div align=center><br><b>ID Jeton non trouvee.</b><br><br><i>Le jeton merlin ne peut etre collecte.<br>(KoC sera recharge dans 10 secondes)</i></div>';
										claim_gift.appendChild(a);	
