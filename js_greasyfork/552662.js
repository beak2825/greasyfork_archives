// ==UserScript==
// @name         Geo-Bundesliga-Hintergrund (Game Master) - auch für Pinpointing Duels
// @namespace    http://tampermonkey.net/
// @version      2.0.10
// @description  Adaptation of the original pinpointing duels game master mode script for the league of the german discord server. For non-pinpointing duels, only show the Geo-Bundesliga background.
...
// @grant        GM_getValue
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/552662/Geo-Bundesliga-Hintergrund%20%28Game%20Master%29%20-%20auch%20f%C3%BCr%20Pinpointing%20Duels.user.js
// @updateURL https://update.greasyfork.org/scripts/552662/Geo-Bundesliga-Hintergrund%20%28Game%20Master%29%20-%20auch%20f%C3%BCr%20Pinpointing%20Duels.meta.js
// ==/UserScript==
...
function showSettingsPanel() {
    ...
    const tieRangeValue = GM_getValue('tieRange', 2);
    ...
    for (let x = 3; x <= 30; x += 1) {
        ...
        option.value = x;
        option.innerText = x + ' (set max. rounds to ' + 2*x + ')';
    ...
    const dachInfo = document.createElement('div');
        dachInfo.innerText = "You are using the Geo-Bundesliga\nversion of the Pinpointing script!\n\nWhen changing the 'First to' value, please adjust the 'max rounds' number in the party accordingly."
...
function showPenaltyDisplay(message) {
    ...
    const customStyles = `
        ...
        .views_activeRoundWrapper__1_J5M {
            background-image: url('https://i.imgur.com/NgGNlwW.png') !important;
    ...
    if (
      ...
      response.options.maxNumberOfRounds > 0 &&
      ...
      isPPD = true;
      console.log("PPD is set to true");
    } else {
      isPPD = false;
      console.log("PPD is set to false");
    }
    ...
    const updateActiveRoundBackground = () => {
        let bgUrl = "https://i.imgur.com/NgGNlwW.png"; // default background
        // If game is over, use default background.
        if (leftScore >= winThreshold || rightScore >= winThreshold) {
            bgUrl = "https://i.imgur.com/NgGNlwW.png";
        }
