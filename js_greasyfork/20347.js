// ==UserScript==
// @name        E-Comm ShipCalc - Dev - DEPRECATED
...
// @include     *infocenter/shipping.html*
// @version     1.8.1
// @description Implements a shipping calculator on the current page.
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/20347/E-Comm%20ShipCalc%20-%20Dev%20-%20DEPRECATED.user.js
// @updateURL https://update.greasyfork.org/scripts/20347/E-Comm%20ShipCalc%20-%20Dev%20-%20DEPRECATED.meta.js
// ==/UserScript==
...
if (url.indexOf('infocenter/shipping.html') > 0) {
...
if (url.indexOf('infocenter/shipping.html') <= 0) {
  makeButton().addClass('upsButton').text('Use this (UPS)').appendTo($('#myDimWeight'));
  makeButton().addClass('uspsButton').text('Use this (Post Office)').css({'color' : '#000', 'margin-top' : '5px'}).appendTo($('#uspsDiv'));
  makeButton().addClass('pickupOnlyButton').text('Use this (Pickup Only)').appendTo($('#myPickupDiv'));
}
...
//			boxNoteString = boxName + cutString;
      return {'weight' : boxWeight, 'name' : boxName, 'boxNoteString' : boxNoteString}; // TODO: Return cut down size!
    } else {
        ...
        if (pickup === true) {
          ...
          $('.upsButton').text('Use this (UPS)');   
			...
			if (shipType == 'override') {
				  ...
				  $('#shipCalcShippingWeight').html(shipping['shippingWeight']);
				  $('#myDimWeight').show();
          $('.upsButton').text('Use this (UPS)').css('background-color', '#f00');
				  $('#calc-override').css('background-color', '#f00');
			} else {
				$('#calc-override').css('background-color', '#eee');
				$('.upsButton').css('background-color', '#eee');
			}
      if ($('#ownBox:checked:visible').length > 0) {
          pickup = false;
        }
