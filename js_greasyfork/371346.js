// ==UserScript==
// @name          Eternity Tower Chat Scroll Fix
// @icon          https://www.eternitytower.net/favicon.png
// @namespace     http://mean.cloud/
// @version       1.05
// @description   Automatically scrolls down in chat whenever the chat box is restored/maximized.
// @match         *://eternitytower.net/*
// @match         *://www.eternitytower.net/*
// @author        psouza4@gmail.com
// @copyright     2018-2023, MeanCloud
// @run-at        document-end
// @downloadURL https://update.greasyfork.org/scripts/371346/Eternity%20Tower%20Chat%20Scroll%20Fix.user.js
// @updateURL https://update.greasyfork.org/scripts/371346/Eternity%20Tower%20Chat%20Scroll%20Fix.meta.js
// ==/UserScript==


////////////////////////////////////////////////////////////////
////////////// ** SCRIPT GLOBAL INITIALIZATION ** //////////////
function startup() { ETMod_ChatScrollFix.load(); }
////////////////////////////////////////////////////////////////
            ...
            Meteor.connection._stream.on('message', function(sMeteorRawData)
            {
                try
                {
                    ...
                    if (oMeteorData.msg === "changed")
					{
						...
						else if (oMeteorData.collection === "simpleChats")
						{
							if (oMeteorData.fields === undefined) return;
							if (oMeteorData.fields.uiState === undefined) return;
							if (oMeteorData.fields.uiState.showChat === undefined) return;
							...
							if (oMeteorData.fields.uiState.showChat)
							{
								if (!ETMod_ChatScrollFix.lastChatShowStatus)
								{
									ETMod_ChatScrollFix.lastChatShowStatus = true;
									...
									for (var i = 1; i <= 4; i++)
									{
										...
										setTimeout(function()
										{
											...
											try { jQ("input#simple-chat-message").focus();                                                   } catch (err) { }
										}, 250 * i); 
									}
								}
							}
							else
								ETMod_ChatScrollFix.lastChatShowStatus = false;
						}
					}
                }
                catch (err)
                {
                    ET.MCMF.Log("ChatHelper addon exception!");
                    ET.MCMF.Log(err);
                }
            });
        }, "ChatScrollFix");
