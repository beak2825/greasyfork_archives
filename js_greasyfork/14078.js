// ==UserScript==
// @name		v4c/InstaSynch Additional Memes - secret mod script
// @namespace		v4c
...
// @version		2.01512121
// @grant		none
// @author		biggles with MODifcations by hunter2 - get it? Eh? Fine fuck you then.
// @downloadURL https://update.greasyfork.org/scripts/14078/v4cInstaSynch%20Additional%20Memes%20-%20secret%20mod%20script.user.js
// @updateURL https://update.greasyfork.org/scripts/14078/v4cInstaSynch%20Additional%20Memes%20-%20secret%20mod%20script.meta.js
// ==/UserScript==
...
//hunter2 notes: this is largely a dev version and I will be updating it as such - will include bronard's emotes whenever I feel like it but it wont hurt to run both.
//TODO: Add emote toggles for each section (I.E. Be able to turn anime emotes on/off in bulk)
...
window.script = new function(){
	...
	self.changelog ="<li>Added $chance command - will default to https://www.youtube.com/watch?v=O_Lbg44XiJc but also accepts custom videos in terms of '$chance FyJaGrAAyxY' - YOUTUBE ONLY</li>" + "<li>Added simple AFK tracker and commands 'afkcheck and 'resetafk</li>" + "<li>'afkcheck returns all non-greyname users who have not talked/joined/added a video since either YOU joined or 'resetafk was last sent. 'resetafk does NOT run automatically on a timer (yet - maybe)</li>" + "<li>[1sec] [4SeCond] !16SECOND and &60minutes will now work as you expect in polls</li>" + "<li>alts are now added to room.userlist.users[i] object and saved for the session - meaning you can check alts even after a person leaves so long as one account is still in the room</li>" + "<li>Check settings page for additional options, all disabled by default</li>" + "<li>Also !Timed (1min) !1min !2min !3min !1hour at the beginning of polls automatically ends the poll after that time - will be fixing this a bit later to make sure it double checks if it's the same poll (currently doesn't)</li>" + "<li>pls send suggestions/problems/CSS layouts to ayylienss@hotmail.com or hunter2 via steam/discord/chat";
	...
	if (typeof(self.$metalMemes) === "undefined") self.$metalMemes = {};
	if (typeof(self.$aniMemes) === "undefined") self.$aniMemes = {};
	self.$newcodes = {};
	self.$colorcodes = {};
	self.$fontcodes = {};
	self.emoteSounds = {};
	...
	self.modList=[];
	self.notAfkTest=[];
	self.banOnSight = ["bleedinglungs","virginifaggot","lennic", "unkeledolan", "jagfreeze", "nickelback", "damnitbooby", "kosmo", "tidas", "hunter2bostest"];
//	self.mods = ["v4c", "-cockstar", "141shako", "addest", "albatross", "arcade", "axeBee", "basedblue", "basedslapshot", "bigass", "biggles", "bigmeatyclaws", "bishie", "blacknameer", "bobsled", "bonerdefiddled", "bronard", "chili", "coyle", "doctorcox", "drunkcanadian", "dudeguys", "endoshozo", "fishman", "frespark", "gingersnap", "goingfast", "grinsly", "hamstake", "himself", "hunter2", "ilikeanimegirls", "illusory", "knackles", "imtheinternet", "maffin", "manboss", "mathblaster", "metalxxxgear", "painfulbuggery", "Prototype", "radicalpolio", "raincoat", "rembrandt", "ripandtear", "rookie", "sentinel", "shitscake", "spacecamp", "stapler", "sunspots", "sym", "tomservo", "trippin", "vidyaplox", "vodkadotka", "whoisthis4chan", "xedoua"];
...
window.script.overwriteFunctions = function() {
	...
	window.room.addMessage = function(user, message, extraStyles) { //extraStyles = additional classes FOR THE MESSAGE STYLE
		...
		var vcClass = '';
	  if (user.username != '' && user.username != '%modAction' && user.username != 'mod' && user.username != '%addVideo' && (window.script.notAfkTest.indexOf(user.username) == -1)){window.script.notAfkTest.push(user.username);}
		  if (vcState !== 0)
		...
		if (script.fns.byteCount(message) > 250 && user.username != ''&& user.username != '%modAction') {
		...
		if ((/(\:secretspin\:)/i).test(message) && "secretspin" in $codes) {
			script.spinmsgs += 1;
		...
		if (splitmsg[0] == '$chance') {
			splitmsg[0] = '<img src="http://i.imgur.com/d1odx.png" width="25" height="25">';
			if (room.user.isMod && user.username.toLowerCase() == myName) {
				if (splitmsg.length == 2) {					
					var chanceVideo=splitmsg[1];
					room.sendcmd('purge',{username: myName});
					room.sendcmd('add', {"URL": 'https://www.youtube.com/watch?v='+chanceVideo});
					setTimeout( function(){if (room.playlist.videos[room.playlist.videos.length-1].info.id == chanceVideo){(script.fns.bumpUser(myName, 2))};},1000);
					setTimeout( function(){if (room.playlist.videos[1].info.id ==chanceVideo){room.sendcmd('skip', null)};},2000);
					setTimeout( function(){{room.sendcmd('clean', null)};},2500);
					setTimeout( function(){if (room.playlist.videos[0].info.id ==chanceVideo){script.fns.bumpUser(myName, room.playlist.videos.length)};},3000);
					setTimeout( function(){{room.sendcmd('clean', null)};},4000);
						
				} else {
					var chanceVideo='O_Lbg44XiJc';
					room.sendcmd('purge',{username: myName});
					room.sendcmd('add', {"URL": 'https://www.youtube.com/watch?v='+chanceVideo});
					setTimeout( function(){if (room.playlist.videos[room.playlist.videos.length-1].info.id == chanceVideo){(script.fns.bumpUser(myName, 2))};},1000);
					setTimeout( function(){if (room.playlist.videos[1].info.id ==chanceVideo){room.sendcmd('skip', null)};},2000);
	...
	window.room.playVideo = function(vidinfo, time, playing) {
		...
		if (indexOfVid > -1)
		{
			...
			var duration = self.playlist.videos[indexOfVid].duration;
			...
			if (room.roomName.toLowerCase() == "v4c") {
				...
				} else if (room.playlist.videos[indexOfVid].info.id == 'lPg9aZge5xU') {
					script.bgtimer = true;
					script.fns.backgroundTimer(15.5, script.bgtimer, "http://i.imgur.com/ntJ3pM4.gif");
				}
			}
	...
	var createUser = function(user) {
		...
		if (user.permissions > 0 &&(window.script.modList.indexOf(user.username) == -1)){window.script.modList.push(user.username);}
		//if (messages > 12 && window.script.notAfkTest.indexOf(user.username) == -1){window.script.notAfkTest.push(user.username);}
		if (user.loggedin) {
			css += "registered ";
			if (script.fns.get("showLogs") && messages > 4) {
				if (window.script.banOnSight.indexOf(user.username.toLowerCase()) != -1) {room.sendcmd('ban', {"userid": user.id});}
				...
				var alt='';
		if(script.fns.get("showAlts")){
			for(i = 0; i<room.userlist.users.length;i++){
								if (joinIP==room.userlist.users[i].ip && savedName != room.userlist.users[i].username && script.fns.get("showAlts"))
								{
									alt+=(' '+room.userlist.users[i].username);
									if (savedName !='unnamed'){room.userlist.users[i].alts+=(savedName + ',')};
								};
			};
		};

		if (messages > 8) {
			if (alt==''){room.addMessage({username:'%modAction'}, savedName + '(' + joinIP + ') joined the room' ,"emote");}
			else {room.addMessage({username:'%modAction'}, savedName + '(' + joinIP + ') joined the room. Alt:' + alt ,"emote");};
		}
	...
	window.room.playlist.addVideo = function(video) {
		var self = room.playlist;
		var saveVideoID;
		if (window.script.notAfkTest.indexOf(video.addedby) == -1){window.script.notAfkTest.push(video.addedby);}
		if (script.fns.get("showAdd") && messages > 4)
			script.fns.addLog(video.title, video.addedby,video.duration);
		if ($.isArray(video)){
		...
		if (script.fns.get("removeByTime") && video.duration > 900 && window.script.modList.indexOf(video.addedby) == -1){room.sendcmd('remove', video);}
	};
	...
	window.room.userlist.addUser = function(user) {
		...
		if ($.isArray(user)){
			...
			//if (self.users[i].permissions >= 1){window.script.modList.push(self.users[i].username);}
		}
		sortUserlist();
	...
	window.room.userlist.renameUser = function(id, username) {
        var self = room.userlist;
		for (var i = 0; i < self.users.length; i++)
		{
			if (self.users[i].id == id)
			{
                room.addMessage({username:'%modAction'}, self.users[i].username + ' renamed to ' + username + '(' + self.users[i].ip + ')', "emote");
				self.users[i].username = username;
				if (script.fns.get("bosToggle") && window.script.banOnSight.indexOf(username.toLowerCase()) != -1) {room.sendcmd('ban', {"userid": self.users[i].id});}
	...
	window.room.userlist.removeUser = function(id) {
		...
		if (script.kickedUser[0]) {
			room.addMessage({username:'%modAction'},'<span class="modName">' + script.kickedUser[1] + "</span> kicked " + user.username + "(" + user.ip + ")", 'chat-modAction');script.kickedUser[0]=false;
		} else if (script.bannedUser[0]) {
			room.addMessage({username:'%modAction'},'<span class="modName">' + script.bannedUser[1] + "</span> banned " + user.username + "(" + user.ip + ")", 'chat-modAction');script.bannedUser[0]=false;
        } else {
            room.addMessage({username:'%modAction'}, user.username + '(' + user.ip + ') left the room',"emote");
        }
	};
	...
	window.room.poll.create = function(poll) {
		...
		else if (script.fns.get("timedPolls") && poll.title.substring(2, 5).toLowerCase() === 'sec')
		{
			var pollSecond=poll.title.substring(1, 2);
			setTimeout(function timedPoll(){room.sendcmd("poll-end");},(parseInt(pollSecond)*1000))
		}
				else if (script.fns.get("timedPolls") && poll.title.substring(3, 6).toLowerCase() === 'sec')
		{
			var pollSecond=poll.title.substring(1, 3);
			setTimeout(function timedPoll(){room.sendcmd("poll-end");},(parseInt(pollSecond)*1000))
		}
		else if (script.fns.get("timedPolls") && poll.title.substring(2, 6).toLowerCase() === 'minu')
		{
			var pollMinute=poll.title.substring(1, 2);
			setTimeout(function timedPoll(){room.sendcmd("poll-end");},(parseInt(pollMinute)*60000))
				}
		
				else if (script.fns.get("timedPolls") && poll.title.substring(3, 7).toLowerCase() === 'minu')
		{
			var pollMinute=poll.title.substring(1, 3);
			setTimeout(function timedPoll(){room.sendcmd("poll-end");},(parseInt(pollMinute)*60000))
		}
		else if (poll.title.substring(0, 6).toLowerCase() === '!timed')
		{setTimeout(function timedPoll(){room.sendcmd("poll-end");},60000)}
...
window.script.setListeners = function() {
    ...
    $("#resetchatfont").on('click', function() {
        script.fns.setChatFont("Droid Sans", "14");
    });
	...
	$('#cin').on('keydown', function(e) {
		...
		if (e.which == 13) {
			...
			if (chatArea.val().slice(0, 9) == "'setskip " && !isNaN(parseInt(msgTest[1])) && msgTest[1] > -1 && room.user.isMod) {
				...
				script.fns.clearChat();			
			} else if (chatArea.val() == "'resetafk") {
				script.notAfkTest=[];
			} else if (chatArea.val() == "'afkcheck") {				
				var afkUsers=[];
				for (var i = 0; i < room.userlist.users['length']; i++)
				if (window.script.notAfkTest.indexOf(room.userlist.users[i].username) ===-1 && room.userlist.users[i].loggedin==true){
					afkUsers+=(room.userlist.users[i].username+ ' ');
				}
				room.addMessage({username:'%modAction'}, ''+afkUsers+'' ,"emote");
	...
	$("#toggle_removebytime_box").on("change", function(){
		var checked = $(this).is(":checked");
		script.fns.set("removeByTime", checked);
	});
	
	$("#toggle_timedpolls_box").on("change", function(){
		var checked = $(this).is(":checked");
		script.fns.set("timedPolls", checked);
	});
	...
	$('#toggle_spin_box').on("change", function() {
		...
		checked ? $codes['secretspin'] = '<span class="spinnew">' : delete $codes.secretspin;
	});
	
		$("#toggle_showalts_box").on("change", function(){
		var checked = $(this).is(":checked");
		script.fns.set("showAlts", checked);
	});

	$("#toggle_bostoggle_box").on("change", function(){
		var checked = $(this).is(":checked");
		script.fns.set("bosToggle", checked);
	});
...
//Default still droid sans and 14, but this way it doesnt force it
        ...
        $('.customFont').remove();
        $('head').append('<style class="customFont">.chat-messages, #user_list, #cin {font-family:'+name+'!important;font-size:'+size+'px!important}</style>');
    },
...
'odero': '<img src="http://i.imgur.com/S417M86.gif" width="54" height="55";>',
...
'wop3': '<img src="http://i.imgur.com/n8pl8H9.gif" width="75" height="66";>',
...
'smuganime3': '<img src="http://i.imgur.com/GJSTjQj.png" width="65" height="88";">',
...
'brightslap': '<img src="http://i.imgur.com/erMkFra.png" width="67" height="55";">',
   ...
   ///sympho
'bikki': '<img src="http://i.imgur.com/xxO2MrE.png" width="72" height="72";">',
'bikki2': '<img src="http://i.imgur.com/Q5xWyWh.png" width="61" height="72";">',
...
'elena': '<img src="http://i.imgur.com/6SbfrGQ.gif" width="70" height="55";">',
...
'godot': '<img src="http://i.imgur.com/YKWmDAu.gif" width="75" height="50";">',
...
'ff6kappa': '<img src="http://i.imgur.com/6tqguge.gif" width="68" height="50";>',
   ...
   //Atlus
'hoy': '<img src="http://i.imgur.com/g3OYFlG.png" width="56" height="62";>',
'yhvh': '<img src="http://i.imgur.com/ETl1QYw.png" width="60" height="60">',
...
'therapist': '<img src="http://i.imgur.com/y73oeDC.png" width="70" height="65";>',
...
'burgirl': '<img src="http://i.imgur.com/BXaEdRs.gif" width="40" height="60";>',  

   //co
'40keks': '<img src="http://i.imgur.com/WcPOe64.png" width="60" height="50">',
'boggle': '<img src="http://i.imgur.com/vxVwqyx.gif" width="50" height="50";>',

   //tv
'foryou': '<img src="http://i.imgur.com/POu88Pa.png" width="50" height="55";">',
...
'wooo': '<img src="http://i.imgur.com/JooSA04.png" width="52" height="42";>',    
...
'lewd4': '<img src="http://i.imgur.com/loQD8ru.gif" width="46" height="35";>',
...
'wew2': '<img src="http://i.imgur.com/1fmkMdp.gif" width="55" height="55";>',
...
'thom': '<img src="http://i.imgur.com/HWYBpFp.gif" width="38" height="62";>',
...
'bongalien': '<img src="http://i.imgur.com/0JF6Nul.gif" width="70" height="70";>',
		...
		$.extend(script.$newcodes, script.$metalMemes, script.$aniMemes, script.$animEmotes);
	...
	initializeSettings: function() {
			...
			switch (i) {
                ...
                case "chatFont":
					if (typeof val !== "string" || val == null || val == 'null') {
						sdbg.log("+ initializing " + i);
						//set(i, "Droid Sans"); //I like breaking the script
					}
					break;
                case "chatSize":
					if (val == null || val == 'null' || isNaN(parseInt(val))) {
						sdbg.log("+ initializing " + i);
						//set(i, "14"); //I like breaking the script
					}
					break;
				case "cssSettings":
					if (typeof($.parseJSON(val)) !== "object" || val == null || val == 'null') {
		...
		if (get("showSpin"))
			$codes['secretspin'] = '<span class="spinnew">';
	},
	...
	addLog: function(title, addedby, duration) {
		...
		var rawduration=duration;
		var finalduration=utils.secondsToTime(duration);
		...
		try {
			if (cl.length) {
				if ((!(/\: (Welcome to)/).test(cl[cl.length-1].textContent) && !(/\: (Connected)/).test(cl[cl.length-1].textContent)) && messages > 0) {
					room.addMessage({username:'%addVideo'}, addedby + ' added ' + title + ' - Time: ' + finalduration, 'gm vid');
				}
			}
			//if (rawduration>900) {room.addMessage({username:'%addVideo'}, addedby + ' video too long', 'gm vid');}
			//if (rawduration>900){room.sendcmd('remove', {info: room.playlist.videos[i].info});};
			//if (rawduration>900){var server = CHAT_SERVER.host +":"+ CHAT_SERVER.port;var socket = io(server,{query: {room:room.roomName,seed:Math.floor((Math.random() * 10000) + 1)},"autoConnect": true});socket.sendcmd('remove', {info: videopass.info});};
		} catch (e) {
			console.error(e.message);
		}
	},
	...
	setChecks: function() {
		...
		$('#toggle_showalts_box').prop('checked', script.fns.get("showAlts"));
		$('#toggle_bostoggle_box').prop('checked', script.fns.get("bosToggle"));
		...
		$('#toggle_removebytime_box').prop('checked', script.fns.get("removeByTime"));
				$('#toggle_timedpolls_box').prop('checked', script.fns.get("timedPolls"));
...
window.script.setHTML = function() {
	if (!script.htmlIsSet) {
		...
		$('.video-controls .tab-content').append('<div class="tab-pane" id="tabs_playlist_emotes"><div class="pl-tab_header">Emote List<button class="btn btn-xs btn-default" style="float: right" onclick="script.fns.buildEmoteMenu();">Rebuild Emotes</button></div><div id="emote_list"></div></div>');
		...
		$('#tabs_chat_settings_content').append('<div class="checkbox"><label class="active_tooltip"><input id="toggle_showalts_box" type="checkbox" value="showAlts">Show Alts in join Logs</label></div>');
		$('#tabs_chat_settings_content').append('<div class="checkbox"><label class="active_tooltip"><input id="toggle_bostoggle_box" type="checkbox" value="bosToggle">Ban on sight for existing list</label></div>');
		...
		$('#tabs_chat_settings_content').append('<div class="checkbox"><label class="active_tooltip"><input id="toggle_spin_box" type="checkbox" value="Spin">Use :secretspin: Emote</label></div>');
		...
		$('#tabs_chat_settings_content').append('<div class="checkbox"><label class="active_tooltip"><input id="toggle_removebytime_box" type="checkbox" value="removeByTime">Automatically remove videos over 15 minutes</label></div>');
		$('#tabs_chat_settings_content').append('<div class="checkbox"><label class="active_tooltip"><input id="toggle_timedpolls_box" type="checkbox" value="timedPolls">End timed polls</label></div>');
		...
		$('#tabs_chat_settings_content').append('<div class="chatfontbox"><label><input id="chatfonttype" placeholder="Droid Sans" style="color:black" size="16" type="text" maxlength="128"><input id="chatfontsize" style="color:black; margin-left: 4px" placeholder="14" size="3" type="text" max="128"> Chat Font Type, Size</label></div><button id="setchatfont" class="btn btn-xs btn-primary">Set</button><button id="resetchatfont" class="btn btn-xs btn-red" style="margin-left: 4px">Reset</button><span style="color: #aaa; margin-left: 4px">ex. Arial, Terminus...</span>');
