...
(async function(axios, JSZip, saveAs, ImageDownloader) {
  ...
  const title1 = document.querySelector('title').textContent.split('|')[0].trim().split('】 ')[1].trim();
  const title2 = `${content.seriesEpisodeEdge.node.notation}${content.seriesEpisodeEdge.node.title ? ' ' + content.seriesEpisodeEdge.node.title : ''}`;
