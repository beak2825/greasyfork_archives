// ==UserScript==
// @name         Letterboxd → FLAM (Fenlight) [MOBILE • Speedy]
// @namespace    http://tampermonkey.net/
// @version      3.5.0-mobile
// @description  Mobile-friendly: Send Letterboxd lists & nanogenre pages to Kodi (Fenlight) via TMDB with smart matching. Touch-first UI, overlays, larger tap targets. Now faster: no revalidate on send, dirty-only writes, deduped base load, and optional perf marks.
...
// @downloadURL https://update.greasyfork.org/scripts/546586/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29%20%5BMOBILE%20%E2%80%A2%20Speedy%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/546586/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29%20%5BMOBILE%20%E2%80%A2%20Speedy%5D.meta.js
// ==/UserScript==
...
(async function () {
  ...
  // New perf defaults
  const PERF_ENABLE_DEFAULT = false;                 // off by default
  const PERF_AUTO_RUNS_DEFAULT = 2;                  // show for first two runs automatically
  const BASE_REVALIDATE_INTERVAL_MS = 30 * 24 * 60 * 60 * 1000; // ~30 days
  ...
  // ─── Binary base cache loader (no revalidate during sends) ──────────────
  function baseBinUrl() { return GM_GetOrSet('baseBinUrl', BASE_BIN_URL_DEFAULT); }
  let BASE_PAIRS_U32 = null;  let BASE_COUNT = 0;

  // Ensures in-memory arrays are initialized from IDB; no network unless explicitly asked
  async function loadBaseBinOnce({ allowRevalidate = false } = {}) {
    ...
    // Skip revalidate unless explicitly allowed (we never allow during sends)
  ...
  // Force refresh, silent by default; caller decides UX messaging
  async function refreshBaseBinNow({ silent = true } = {}) {
    await new Promise((resolve) => {
      GM_xmlhttpRequest({
        method: 'GET',
        url: baseBinUrl(),
          ...
          try {
            if (res.status >= 200 && res.status < 300 && res.response) {
              ...
              if (u32.length % 2 === 0) {
                ...
                const newEtag = m ? m[1].trim() : '';
                try { await putBaseToIDB(newBuf, newEtag); } catch {}
                GM_setValue('baseLastChecked', Date.now());
              } else if (!silent) { alert('❌ Base cache corrupt (odd length).'); }
            } else if (!silent) { alert(`❌ Base cache HTTP ${res.status}.`); }
          } finally { resolve(); }
        },
        onerror: () => { if (!silent) alert('❌ Base cache network error.'); resolve(); },
        ontimeout: () => { if (!silent) alert('❌ Base cache request timed out.'); resolve(); },
      });
    });
  }

  // Background monthly revalidate (never during a send)
  async function maybeMonthlyRevalidateInBackground() {
    try {
      const last = Number(GM_GetOrSet('baseLastChecked', 0)) || 0;
      if (Date.now() - last > BASE_REVALIDATE_INTERVAL_MS) {
        // Quietly revalidate after a short delay so we don't block page settle
        await sleep(2500);
  ...
  // Overrides + dirty tracking
  let LOCAL_OVERRIDES = null;
  let OVERRIDES_DIRTY = false;
  ...
  function overrideSetPacked(filmIdStr, tmdbId, isTv) {
    ...
    const nextPacked = (tmdbId << 1) | (isTv ? 1 : 0);
    const prevPacked = LOCAL_OVERRIDES[filmIdStr];
    if (!Number.isFinite(prevPacked) || prevPacked !== nextPacked) {
      LOCAL_OVERRIDES[filmIdStr] = nextPacked;
      OVERRIDES_DIRTY = true;
    }
  }
  async function persistOverridesIfDirty() {
    if (OVERRIDES_DIRTY && LOCAL_OVERRIDES) {
      const snapshot = JSON.parse(JSON.stringify(LOCAL_OVERRIDES));
      await setCache(snapshot, CACHE_KEY);
      // keep backup aligned only when we actually changed
      try { await setCache(snapshot, CACHE_BACKUP_KEY); } catch {}
  ...
  // ─── Review Panel (unchanged UX; dirty-only saves) ──────────────────────
  async function openReviewPanelPromise() {
    ...
    return new Promise(async (resolve) => {
      ...
      async function applySavesToCache(doc) {
        ...
        if (changesApplied > 0) await persistOverridesIfDirty();
  ...
  // ─── Cache Editor (dirty-only persistence) ──────────────────────────────
  async function openCacheEditor() {
    ...
    saveBtn.onclick = async () => {
      ...
      try {
        ...
        await setCache(packed, CACHE_BACKUP_KEY);
  ...
  // Core processing — no base revalidate and dirty-only persistence
  async function processItems(items, providedOverrides = null) {
    // Base cache load is handled by caller once per run; don't re-load/revalidate here
    await loadLocalOverridesOnce();
    ...
    // Final retry pass (only if list is large enough)
  ...
  // ─── Perf marks (lightweight) ───────────────────────────────────────────
  function shouldShowPerf() {
    const forced = GM_GetOrSet('perfEnable', PERF_ENABLE_DEFAULT);
    let autoLeft = GM_GetOrSet('perfAutoRunsLeft', PERF_AUTO_RUNS_DEFAULT);
    if (forced) return { show: true, fromAuto: false };
    if (autoLeft > 0) {
      GM_setValue('perfAutoRunsLeft', autoLeft - 1);
      return { show: true, fromAuto: true };
    }
    return { show: false, fromAuto: false };
  }
  function mkPerf() {
    const marks = [];
    const t0 = performance.now();
    function mark(label) { marks.push([label, performance.now()]); }
    function summary() {
      const lines = [];
      let prev = t0;
      for (const [label, t] of marks) {
        const dt = t - prev; prev = t;
        lines.push(`${label}: ${dt.toFixed(1)}ms`);
      }
      lines.push(`Total: ${(prev - t0).toFixed(1)}ms`);
      return lines;
    }
    return { mark, summary };
  }

  // ─── Show URL (overlay) — uses no revalidate and perf marks ─────────────
  async function handleShowUrl(btnEl) {
    if (btnEl) btnEl.disabled = true;
    const perfGate = shouldShowPerf();
    const perf = mkPerf();

    // Ensure caches for this run (no revalidate)
    perf.mark('start');
    await loadBaseBinOnce({ allowRevalidate: false });
    await loadLocalOverridesOnce();
    perf.mark('caches loaded');

    let items;
    try { items = await scrapeItemsSmart(); perf.mark('scraped'); }
  ...
  // ─── FLAM action — fast path: no revalidate, deduped base load, perf marks ─
  flamBtn.addEventListener('click', async function () {
    ...
    // Load base + overrides ONCE for this run; no network revalidation
    await loadBaseBinOnce({ allowRevalidate: false });
    await loadLocalOverridesOnce();
    perf.mark('caches loaded');
  ...
  // ─── Settings (tabs; includes perf toggle; refresh is on-demand) ────────
  function showSettings() {
    ...
    general.innerHTML = `
      ...
      <div>
        ...
        <div class="kb-row" style="margin-top:10px">
          <div>
            <label><input type="checkbox" id="perfEnable"/> Enable Perf Marks</label>
            <div style="color:#bbb;font-size:12px;margin-top:4px">Shows phase timings in a toast. Also auto-shown for first two runs.</div>
            <div id="perfAutoInfo" style="color:#bbb;font-size:12px;margin-top:4px"></div>
          </div>
        </div>
    ...
    // Perf controls
    body.querySelector('#perfEnable').checked = GM_GetOrSet('perfEnable', PERF_ENABLE_DEFAULT);
    const perfAutoInfo = body.querySelector('#perfAutoInfo');
    const runsLeft = GM_GetOrSet('perfAutoRunsLeft', PERF_AUTO_RUNS_DEFAULT);
    perfAutoInfo.textContent = `Auto perf runs remaining: ${runsLeft}`;
    ...
    body.querySelector('#refreshBaseCacheBtn').onclick = async () => {
      ...
      await refreshBaseBinNow({ silent: true });
    ...
    saveBtn.onclick = () => {
      ...
      // Perf save
      GM_setValue('perfEnable', body.querySelector('#perfEnable').checked);
    ...
    body.querySelector('#btnClearCache').onclick = async () => { await clearCache(); LOCAL_OVERRIDES = {}; OVERRIDES_DIRTY = false; alert('Overrides cleared'); };
  ...
  // ─── Start background monthly revalidation (quiet) ──────────────────────
  // Runs once per page load, never blocks, never runs during FLAM clicks
  maybeMonthlyRevalidateInBackground();
