// ==UserScript==
// @name            wsmud_Trigger_ex
// @namespace       cqv3
// @version         0.0.2
// @date            08/06/2019
// @modified        08/06/2019
// @homepage        https://greasyfork.org/zh-CN/scripts/388499
...
// @downloadURL https://update.greasyfork.org/scripts/388499/wsmud_Trigger_ex.user.js
// @updateURL https://update.greasyfork.org/scripts/388499/wsmud_Trigger_ex.meta.js
// ==/UserScript==
...
// 刷新人物名称：(name)`;
    ...
    (function() {
        const name = new InputFilter("长老获取", InputFilterFormat.text, "", KeyAssert);
        let filters = [name];
        const intro = `// 长老获取触发器
// 长老ids：(ids)
// 长老names：(names)`;
        const t = new TriggerTemplate("长老获取", filters, intro);
        TriggerTemplateCenter.add(t);
        ...
        const run = function() {
            WG.add_hook("items", function(data){
				...
				for (const item of data.items) {
					...
					if(item != 0)
					{
						if(item.name.indexOf("派长老") != -1 || item.name.indexOf("副帮主") != -1)
						{
							Ids.push("'"+item.id+"'");
				...
				params["ids"] = Ids;
				params["names"] = Names;
                const n = new Notification("长老获取", params);
                NotificationCenter.post(n);
            });
        };
        const monitor = new Monitor(run);
    ...
    (function() {
        const name = new InputFilter("查看尸体", InputFilterFormat.text, "", KeyAssert);
        let filters = [name];
        const intro = `// 查看尸体触发器
// 尸体id：(id)`;
        const t = new TriggerTemplate("查看尸体", filters, intro);
        TriggerTemplateCenter.add(t);

        const run = function() {
            WG.add_hook("item", function(data){
                if(data.desc.indexOf("尸体") == -1) return;
				let params = {
					"查看尸体": data.desc.substr(data.desc.indexOf("。")+1,data.desc.lenght).replace(/\n|\s/g,""),
				};
				params["id"] = data.id;
                const n = new Notification("查看尸体", params);
                NotificationCenter.post(n);
            });
        };
        const monitor = new Monitor(run);
    ...
    (function() {
	    const name = new InputFilter("选中叫杀", InputFilterFormat.text, "", KeyAssert);
        let filters = [name];
        const intro = `// 选中叫杀触发器
// 人物id：(id)
kill (id)`;
        const t = new TriggerTemplate("选中叫杀", filters, intro);
        TriggerTemplateCenter.add(t);

        const run = function() {
            WG.add_hook("item", function(data){
                if(data.desc.indexOf("尸体") != -1) return;
				for(var index in data.commands) {
					var name = data.commands[index].name;
					if(name == "重新开始挑战" || name == "快速挑战" || name == "武道记录" || name == "提升境界" || name == "结婚" || name == "兑换物资" || name == "刷新货物" || name == "兑换" || name == "师门任务" || name == "学习" || name == "追捕" || name == "快速追捕" || name == "离婚" || name == "解除师徒" || name == "邀请组队" || name == "购买")
					{
						return;
					}
				};
				...
				params["id"] = data.id;
                const n = new Notification("选中叫杀", params);
                NotificationCenter.post(n);
            });
        };
        const monitor = new Monitor(run);
