...
// @downloadURL https://update.greasyfork.org/scripts/479123/Rizz%20Your%20Waifu.user.js
// @updateURL https://update.greasyfork.org/scripts/479123/Rizz%20Your%20Waifu.meta.js
// ==/UserScript==
(function() {
    ...
    const NEO_TURNS_URL = /https:\/\/neo\.character\.ai\/turns\/.*/g;
    const SENTRY_URL = "sentry.io";
    const E_VERSION = "1.3.6";
            ...
            switch (eventType) {
                case "update_turn": {
                    if (data.hasOwnProperty("turn")) {
                        ...
                        self.lastdata = data;
                        self.botname.innerText = data.turn.author.name;
                        ...
                        if (data.turn.candidates[0].hasOwnProperty("safety_truncated")) {
                            self.status = 3;
                            self.reqstatus.innerText = "Filtered";
                            this.dom.classList.add("warned");
                        }

                        if (data.turn.candidates[0].hasOwnProperty("raw_content")) {
                            self.botmsg.value = data.turn.candidates[0].raw_content;
                            self.rawmsg.innerText = data.turn.candidates[0].raw_content;
                        } else {
                            this.dom.classList.remove("warned");
                            this.dom.classList.add("errored");
                            self.errored = true;
