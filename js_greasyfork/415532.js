// ==UserScript==
// @name         MouseHunt - Mapping Helper (Badly Fixed)
...
// @downloadURL https://update.greasyfork.org/scripts/415532/MouseHunt%20-%20Mapping%20Helper%20%28Badly%20Fixed%29.user.js
// @updateURL https://update.greasyfork.org/scripts/415532/MouseHunt%20-%20Mapping%20Helper%20%28Badly%20Fixed%29.meta.js
// ==/UserScript==
...
(function () {
    ...
    this.addEventListener("load", function () {
      ...
      if (rhMap && rhMap.maps.length > 0) {
        ...
        if (currentMap.name.indexOf("Scavenger") < 0) {
          const remainStr = `${currentMap.map_id}~${currentMap.num_found}`;
  ...
  function render() {
    ...
    refreshButton.addEventListener("click", function () {
      ...
      let mapId = user.quests.QuestRelicHunter.default_map_id;
    ...
    if (cacheRaw) {
      ...
      var checkMap = document.querySelector(
        ".treasureMapManagerView-task.active"
      );
      checkMap = user.quests.QuestRelicHunter.maps[0].name
      if (checkMap == "Rare Gilded Treasure Map") checkMap = "Rare Gilded Birthday Event Map"
      ...
      if (checkPreview) {
        ...
        mapName = user.quests.QuestRelicHunter.maps[0].name
      if (cache[mapName] !== undefined) {
         var mapId = cache[mapName].map_id
        ...
        if (mapIdEl|| true) {
          // Abstract equality comparison because map ID can be number or string
            console.log(cache[mapName].map_id)
          if (cache[mapName].map_id) {
