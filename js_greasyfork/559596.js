// ==UserScript==
// @name         ChatGPT auto select GPT-5.1 Instant
// @namespace    http://tampermonkey.net/
// @version      11.0
// @description  自動選擇舊的 GPT-5.1 Instant 模型
// @author       You
// @match        https://chat.openai.com/*
// @match        https://chatgpt.com/*
// @grant        none
// @run-at       document-idle
// @downloadURL https://update.greasyfork.org/scripts/559596/ChatGPT%20auto%20select%20GPT-51%20Instant.user.js
// @updateURL https://update.greasyfork.org/scripts/559596/ChatGPT%20auto%20select%20GPT-51%20Instant.meta.js
// ==/UserScript==
...
(function() {
    ...
    async function selectGPT51Instant() {
        await wait(500);
        const current = findModelButton()?.textContent?.trim() || '';
        if (current.includes('5.1') && current.includes('Instant')) return true;
        ...
        for (let i = 0; i < 3; i++) {
            if (findAndClick('GPT-5.1 Instant', true)) {
                await wait(200);
                const model = findModelButton()?.textContent?.trim() || '';
                return model.includes('5.1') && model.includes('Instant');
            }
            await wait(80);
        }
        return false;
    ...
    new MutationObserver(() => {
        if (location.href !== lastUrl && !processing) {
            ...
            setTimeout(async () => {
                ...
                if (!m.includes('5.1') || !m.includes('Instant')) {
                    processing = true;
                    await selectGPT51Instant();
                    processing = false;
                }
            }, 1000);
        }
    ...
    // 全域指令
    window.selectGPT51 = async () => { processing = true; const r = await selectGPT51Instant(); processing = false; return r; };
})();