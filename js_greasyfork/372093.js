// ==UserScript==
// @name        Shin WaniKani SRS Level Progress
// @namespace   chooban
...
// @run-at      document-end
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/372093/Shin%20WaniKani%20SRS%20Level%20Progress.user.js
// @updateURL https://update.greasyfork.org/scripts/372093/Shin%20WaniKani%20SRS%20Level%20Progress.meta.js
// ==/UserScript==
...
(function() {
    ...
    if (window.location.pathname == '/review') {
        ...
        get_api_key().then(function() {
            ...
            if (api_key) {
                ajax_retry('https://wk-stats.herokuapp.com/srs/status?api_key='+api_key, 3, 120000).then(function(json) {
    ...
    var update = function(json) {
        ...
        json.levels.order.forEach(function(level) {
            ...
            if (leech_total) {
                var html = '<span class="leech-count" title="'+leech_total+' '+(leech_total > 1 ? 'leeches' : 'leech')+'">'+
                    '<a href="https://wk-stats.herokuapp.com/leeches?sort_by=srs&api_key='+api_key+'#'+level+'" target="_blank">';
        ...
        if (!json.review_order || json.review_order.length === 0) {
            ...
            json.review_order.forEach(function(review_leech) {
                review_html += '<tr class="'+review_leech.subject_type+'-'+review_leech.subject_id+'"><td><a href="/'+review_leech.subject_type+'/'+review_leech.name+'"><span lang="ja">'+review_leech.name+'</span><span class="pull-right">'+review_leech.worst_score.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })+'</span></a></td></tr>';
            });
        }
        ...
        if (!json.review_order || json.review_order.length === 0) {
            ...
            review_html += '<div class="see-more">'+
                '<a class="small-caps" href="https://wk-stats.herokuapp.com/leeches?sort_by=wrong&api_key='+api_key+'" target="_blank">See More Leeches...</a>'+
                '</div>';
        }
