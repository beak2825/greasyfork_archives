...
// @version     1
// @grant       none
// @downloadURL https://update.greasyfork.org/scripts/472228/Pesquisa%20de%20Palavras-chave%20Extensas.user.js
// @updateURL https://update.greasyfork.org/scripts/472228/Pesquisa%20de%20Palavras-chave%20Extensas.meta.js
// ==/UserScript==
...
window.addEventListener('load', function() {
  ...
  function myFunction() {
    ...
    bodyTextNodes.forEach(function(node) {
      ...
      var startIndex = 0, searchStrLen = filter.length;
      var index, newContent = '', prevIndex = 0;

      if (content.toLowerCase().includes(filter)) {
        while ((index = content.toLowerCase().indexOf(filter, startIndex)) > -1) {
          newContent += content.substring(prevIndex, index) + '<mark>' + content.substr(index, searchStrLen) + '</mark>';
          startIndex = index + searchStrLen;
          prevIndex = startIndex;
        }
        newContent += content.substring(prevIndex);
        node.textContent = '';
        node.insertAdjacentHTML('beforeend', newContent);
