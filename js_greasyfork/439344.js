...
// @include /^(https?:\/\/)(.+)?(cashurl|linkad|bitcoinly|crazyblog|zolomix|technologylover|earnfort|spidermods|expertlinks|linksfire|largestpanel|linkres|rsrlink|linkstream|xmod|short2url|99links|rplinks|enit|adloot|url2go|tnvalue|hpshort|hblinks|pvidly|apurl|linkshor|earnspace|urlinked|sniply|mdlink|gamerfang)(\.in)(\/.*)?/
...
// @include /^(https?:\/\/)(profitlink|zagl|scratch247|siyn|portablesusb|children-learningreading|azmath|mobilereview|cryptofly|inform3tion|mobilism|machicon-akihabara|cooklike|videoclip|bitcrypto|shortlinkto|uptobhai|brbushare)(\.info)(\/.*)/
...
// @match      *://inplastore.online/*
// @downloadURL https://update.greasyfork.org/scripts/439344/1%20Shortlinks%20Helper.user.js
// @updateURL https://update.greasyfork.org/scripts/439344/1%20Shortlinks%20Helper.meta.js
// ==/UserScript==
(function() {
    'use strict';
    ...
    function redirect(url, blog = true) {location = blog ? 'https://inplastore.online/?url=' + url : url;}
    ...
    case 'dogeearn.com': case 'adarima.org': case 'mcrypto.club': if ( h.searchParams.has('go')) {location.href = 'https://adarima.org/redirect?/' + h.searchParams.get('go');} break;
    ...
    let $ = window.jQuery; let respect = 'https://inplastore.online/?url='; // Don't use My Scripts if You Change/Remove My Blogs, Except You Make Donations!!!.
    ...
    switch (h.host) {case 'inplastore.online': case 'inplastore.online': if (h.pathname === '/' && h.searchParams.has('url') && h.searchParams.has('sn')) {
        ...
        result.link = h.searchParams.get('url').replace('&m=1', '').replace('<br />', ''); result.redirectDelay = 2; result.isNotifyNeeded = true; return result; } break;
    ...
    case 'acortame.xyz': if (window.location.hash) {location.href ="https://inplastore.online/?url="+(atob(window.location.hash.substr(1)));} break;
    ...
    BloggerPemula('sport4you.net|nadersoft7.com|manga2day.com|pickhopes.com|gam3ah.com|cars-1year.xyz', 'link', '', false);
    BloggerPemula('kienthucrangmieng.com|coindoog.com|tieutietkiem.com|askpaccosi.com|shopsixseven.com|hoclamdaubep.com|thuocdangian.net|chinhnhacoban.com|coin-free.com', 'wpsafelink', '', false);
    ...
    BypassedByBloggerPemula(/coinsparty.com|coinsurl.com/, function() {if (elementExists('#link-view')) {}else {clickIfElementExists("button[type='submit']", 2);}});
    ...
    BypassedByBloggerPemula(/faucet.work|techlearningtube.in|wildblog.me|bebodybuilder.com|blogginglass.com|technoflip.in|studyranks.in|brixarena.tech|crypto-blog.xyz|jiotech.net|saqercoin.xyz|crypto4tun.com|mrsaifi.xyz|manthans.in|adshorturl.com|digitalnaz.net|welovecrypto.xyz/, function() {var el = document.querySelector("input[name=newwpsafelink]");redirect(JSON.parse(atob(el.value)).linkr);});
    BypassedByBloggerPemula(/99faucet.com|readi.online|cempakajaya.com|tribuncrypto.com|bitzite.com|intmagz.com|nivaprofit.xyz|videoclip.info|freebitcoin.fun|blog-forall.com|punyablog.my.id|punyamerk.top|lycoslink.com|aduzz.com|soltoshindo.com|crypto-news-hub.com|onlineteori.my.id|xtrabits.click|onlineincoms.com|crypto-fi.net|claimcrypto.cc|ourcoincash.xyz|9-animie.co|sapica.xyz|techleets.xyz|moddingzone.in|chuksblog.fun|liongiv.xyz/, function(){var bypasslink = atob(`aH${bp("#landing [name='go']").value.split("aH").slice(1).join("aH")}`); redirect(bypasslink, false);});
