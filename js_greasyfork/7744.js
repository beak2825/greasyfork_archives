...
// @downloadURL https://update.greasyfork.org/scripts/7744/HIT%20Scraper%20testing.user.js
// @updateURL https://update.greasyfork.org/scripts/7744/HIT%20Scraper%20testing.meta.js
// ==/UserScript==
...
var COMM_WEIGHT = (localStorage["scraper_comm"]) ? parseFloat(localStorage["scraper_comm"]) : 1;
var PAY_WEIGHT  = (localStorage["scraper_comm"]) ? parseFloat(localStorage["scraper_comm"]) : 3;
var FAIR_WEIGHT = (localStorage["scraper_comm"]) ? parseFloat(localStorage["scraper_comm"]) : 3;
var FAST_WEIGHT = (localStorage["scraper_comm"]) ? parseFloat(localStorage["scraper_comm"]) : 1;
...
btn02.onclick = function() { 
    ...
    localStorage.removeItem("scraper_fast");
    localStorage.removeItem("scraper_comm");
    localStorage.removeItem("scraper_pay");
    localStorage.removeItem("scraper_fair");
    COMM_WEIGHT = 1;
    PAY_WEIGHT  = 3;
    FAIR_WEIGHT = 3;
    FAST_WEIGHT = 1;
...
for (var i=0; i<thin.length; i++) { 
    ...
    if ((i+1)%4 === 0 && i <= 22) eddiv.appendChild(document.createElement("BR"));
    if (i === 22) {
        var tempSpan = document.createElement("SPAN");
        tempSpan.className = "edspan"+sufTheme;
        tempSpan.innerHTML = "<br /><br />TO weightings:<br />Color Coded Search: 1, 3, 3, 1<br />feihtality: 1, 6, 3.5, 1<br />COMM, PAY, FAIR, FAST<br />";
        eddiv.appendChild(tempSpan);
    }
    thin[i].size = '6'; 
    if (i <= 22)
...
function saveState() {
    ...
    var lastState = {"refreshTime":time_input.value,"numPages":page_input.value,"minHits":min_input.value,"newHitHighlight":new_time_display_input.value,
                     ...
                     "customcustom":ucucolors,"customclassic":uclcolors};
    ...
    for (var i=0; i<thin.length; i++) { 
        ...
        switch (i) {
            ...
            case 22: new jscolor.color(thin[i], {}).fromString(col.hover);
                ...
                break;
            case 23: 
                thin[i].title = "TO Communication weighting";
                thin[i].value = COMM_WEIGHT;
                thin[i].onkeyup = function() { var weight = parseFloat(this.value); if (weight >= 0) { COMM_WEIGHT = weight; localStorage["scraper_comm"] = weight; } };
                
                break;
            case 24: 
                thin[i].title = "TO Pay weighting";
                thin[i].value = PAY_WEIGHT;
                thin[i].onkeyup = function() { var weight = parseFloat(this.value); if (weight >= 0) { PAY_WEIGHT = weight; localStorage["scraper_pay"] = weight; } };
                
                break;
            case 25: 
                thin[i].title = "TO Fairness weighting";
                thin[i].value = FAIR_WEIGHT;
                thin[i].onkeyup = function() { var weight = parseFloat(this.value); if (weight >= 0) { FAIR_WEIGHT = weight; localStorage["scraper_fair"] = weight; } };
                
                break;
            case 26: 
                thin[i].title = "TO Fastness weighting";
                thin[i].value = FAST_WEIGHT;
                thin[i].onkeyup = function() { var weight = parseFloat(this.value); if (weight >= 0) { FAST_WEIGHT = weight; localStorage["scraper_fast"] = weight; } };
