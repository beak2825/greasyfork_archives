...
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/563755/%E5%BE%AE%E5%8D%9A%E7%9F%A5%E4%B9%8EB%E7%AB%99%E5%B0%8F%E7%BA%A2%E4%B9%A6%E5%85%B3%E9%94%AE%E8%AF%8D%E5%B1%8F%E8%94%BD%E5%99%A8.user.js
// @updateURL https://update.greasyfork.org/scripts/563755/%E5%BE%AE%E5%8D%9A%E7%9F%A5%E4%B9%8EB%E7%AB%99%E5%B0%8F%E7%BA%A2%E4%B9%A6%E5%85%B3%E9%94%AE%E8%AF%8D%E5%B1%8F%E8%94%BD%E5%99%A8.meta.js
// ==/UserScript==
 ...
 * 3. 微博/B站/小红书屏蔽器
 *    作者/Author: 化纤粒
 *    来源/Source: https://www.zhihu.com/question/1920892831981106398/answer/1930304825150669292
 *
 * 感谢原作者的贡献！
...
(function () {
    ...
    const NOTIFICATION_MODE_KEY = 'keyword_blocker_notification_mode';
    // 通知模式：'toast' 或 'console'
    ...
    const siteConfigs = {
        ...
        bilibili: {
            containerSelector: '.bili-video-card:not(.bili-video-card__skeleton)',
    ...
    function createManagementUI() {
        ...
        style.textContent = `
            ...
            .kb-close-btn:hover {
             ...
             .kb-clear-all-btn:hover {
                 background: #ff7875;
             }

             .kb-toggle-notification-btn {
                 width: 100%;
                 padding: 8px 16px;
                 background: #52c41a;
                 color: white;
             ...
             .kb-toggle-notification-btn:hover {
                 background: #73d13d;
             }
             ...
             .kb-toast {
                 ...
                 font-size: 14px;
                 line-height: 1.5;
                 animation: kb-toast-slide-in 0.3s ease;
                 word-break: break-word;
             }

             .kb-toast.kb-toast-hide {
                 animation: kb-toast-slide-out 0.3s ease;
             }
        ...
        const notificationBtnText = notificationMode === 'toast' ? '切换到日志' : '切换到通知';
        ...
        panel.innerHTML = `
            ...
            <div class="kb-panel-header">
                ...
                <div class="kb-action-row">
                    <button id="kb-clear-all-btn" class="kb-clear-all-btn">清空关键词</button>
                    <button id="kb-toggle-notification-btn" class="kb-toggle-notification-btn">${notificationBtnText}</button>
                </div>
            </div>
    ...
    function showToast(message, duration = 3000) {
        ...
        toast.innerHTML = `已屏蔽内容：<span class="kb-toast-keyword">${message}</span>`;
    ...
    // 通知函数
    function notifyBlocked(keyword, title) {
        const mode = getNotificationMode();
        const msg = `[${keyword}]: ${title}`

        if (mode === 'toast') {
            // Toast通知模式
            // const msg = `已屏蔽内容：<span class="kb-toast-keyword">关键词：${keyword}\n标题：${title}</span>`;
            showToast(msg, 1500);
        } else {
            // 控制台日志模式
            console.log(msg);
    ...
    function initUIEvents() {
        ...
        const toggleNotificationBtn = document.getElementById('kb-toggle-notification-btn');
        ...
        clearAllBtn.addEventListener('click', (e) => {
            e.stopPropagation(); // 阻止事件冒泡
            if (BLOCK_KEYWORDS.length === 0) {
                alert('当前没有屏蔽词');
                return;
            }

            if (confirm(`确定要删除所有 ${BLOCK_KEYWORDS.length} 个关键词吗？\n\n此操作不可恢复！`)) {
                BLOCK_KEYWORDS = [];
                saveKeywords(BLOCK_KEYWORDS);
                renderKeywordList();
                console.log('已清空所有屏蔽词');
                alert('已成功清空所有关键词');
            }
        });
        ...
        toggleNotificationBtn.addEventListener('click', (e) => {
            ...
            const newMode = currentMode === 'toast' ? 'console' : 'toast';
            setNotificationMode(newMode);

            const newBtnText = newMode === 'toast' ? '切换到日志' : '切换到通知';
            toggleNotificationBtn.textContent = newBtnText;

            const modeText = newMode === 'toast' ? 'Toast通知' : '控制台日志';
            showToast(`通知模式已切换到：${modeText}`, 2000);
            console.log(`通知模式已切换到：${modeText}`);
        });
    }
    ...
    function processContentElement(element, config) {
        ...
        let title = titleElement ? titleElement.textContent.trim() : element.textContent.trim();

        // 追加作者
        const authorEl = element.querySelector('.bili-video-card__info--author');
        const author = authorEl ? authorEl.textContent.trim() : '';
        const textToMatch = author ? `${title} ${author}` : title;
        const hitKeyword = BLOCK_KEYWORDS.find(keyword => textToMatch.includes(keyword));

        if (hitKeyword) {
            let containerRemoved = false;

            // 构造日志信息，包含命中的关键词
            // const logMsg = `${config.logPrefix}: [命中: ${hitKeyword}]\n标题: ${title}`;

            if (site === 'zhihu') {
                let cardElement = element.closest('.Card.TopstoryItem.TopstoryItem-isRecommend');
                if (cardElement) {
                    cardElement.remove();
                    // console.log(`${logMsg} (删除整个卡片)`);
            ...
            if (!containerRemoved) {
                ...
                // console.log(`${logMsg} (隐藏元素)`);
            }

            // 发送通知
            notifyBlocked(hitKeyword, title);
        }
    }
