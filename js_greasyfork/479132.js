// ==UserScript==
// @name         MyBot Parallel Connections
// @description  MyBot Pixel Place Parallel Connections
// @version      1.6.2
// @author       SamaelWired
...
(() => {
    ...
    window.MyBot = MyBot;
    if ('Connect' in MyBot.modules) return;
        ...
        async connect(username, boardId) {
            ...
            const restart = () => {
                ...
                user.onmessage = ({ data }) => {
                    ...
                    if (event == 'p') user.count = MyBot.config.packetCount;
                };
                user.onclose = async () => {
