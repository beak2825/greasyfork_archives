...
// @downloadURL https://update.greasyfork.org/scripts/504945/pixels-vip.user.js
// @updateURL https://update.greasyfork.org/scripts/504945/pixels-vip.meta.js
// ==/UserScript==
  ...
  */
        ...
        let interval = setInterval(() => {
            ...
            if (unsafeWindow.spacepack) {
                ...
                let sell_info = ["Popberry", "Grainbow", "Queen Bee", "Honey", "Wax", "Egg", "Eggsplosive", "Seltsam Egg"]
...
//                  RoomPrototype.connection.send(encryptUtils.encryptSend(action, data));
                ...
                async function sendMssage() {
                    ...
                    while (room){
                        ...
                        selfPlayer = unsafeWindow.Phaser.Display.Canvas.CanvasPool.pool[0].parent.game.scene.scenes[1].stateManager.selfPlayer
                        ...
                        if (state.id.includes("shareRent")) {
                            ...
                            while (energy > 100) {
                                ...
                                let seed_info = []
                                const farm_seed = ['itm_popberrySeeds', 'itm_grainseeds']
                                ...
                                if ((!shearsItem) || (!rustyWateringCanItem)){
                                    go_store = true
                                    if (!map_list.includes("generalStore")){
                                        map_list.unshift("generalStore")
                                        map_list.unshift("terravilla")
...
//                             await move_distance(3492, 3000)
                            ...
                            let metalore = getCurrentItemCount('itm_metalore_01');
                            if (metalore < 30){
                                const name ="Copperite Ore"
                                const amount = 90-metalore
                                updateMarketplaceSearchKeyword(name)
                                await sleep(1000)
                                const Index = Array.from(document.querySelectorAll('[class*="Marketplace_itemName__"]')).findIndex(el => el.textContent === name);
                                if (!room.connection.transport.isOpen) {
                                ...
                                if (bug_seed.includes(result['title'])){
                                    ...
                                    if (item_result){
                                        if (item_result<297){
                                            ...
                                            emitKeyboardEvent(buy_amount_input, 297-item_result)
                                            await sleep(2000)
