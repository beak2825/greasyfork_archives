...
// @downloadURL https://update.greasyfork.org/scripts/560275/Linuxdo%20Agent.user.js
// @updateURL https://update.greasyfork.org/scripts/560275/Linuxdo%20Agent.meta.js
// ==/UserScript==
...
`;
  ...
  function toolResultToContext(name, result) {
    ...
    if (name === "discourse.search") {
      ...
      const header = `【TOOL_RESULT discourse.search | q=${result?.q ?? ""
    ...
    if (name === "discourse.getUserRecent") {
      ...
      const header = `【TOOL_RESULT discourse.getUserRecent | @${result?.username ?? ""
    ...
    if (name === "discourse.getTopicAllPosts") {
      ...
      const midGap =
        tailLines.length && count > head.length + tailLines.length
          ? `\n\n…（中间省略 ${count - head.length - tailLines.length
    ...
    if (name === "discourse.getUserSummary") {
      ...
      const base = [
        `【TOOL_RESULT discourse.getUserSummary | @${r.username || ""
        } | Rich】`,
      ...
      const badgeHint =
        r.badges?.user_badges && r.badges?.badges
          ? `\n--- 徽章（Badges）---\nuser_badges: ${Array.isArray(r.badges.user_badges)
            ? r.badges.user_badges.length
            : "n/a"
          } | badges: ${Array.isArray(r.badges.badges) ? r.badges.badges.length : "n/a"
          }`
          : "";
    ...
    if (name === "discourse.listLatestPosts") {
      ...
      const header = `【TOOL_RESULT discourse.listLatestPosts | before=${result?.before ?? "null"
  ...
  async function callOpenAIChat(messages, conf, cancelToken, confStore) {
    // 获取所有启用的 API
    const enabledApis = confStore ? confStore.getEnabledApis() : [];
    const activeApi = confStore ? confStore.getActiveApi() : null;

    // 向后兼容：如果没有 confStore 或 apiConfigs，使用旧的单 API 模式
    if (!activeApi || !enabledApis.length) {
      const base = String(conf.baseUrl || "").replace(/\/+$/, "");
      const url = base.endsWith("/chat/completions") ? base : base + "/chat/completions";
      const payload = { model: conf.model, temperature: conf.temperature ?? 0.2, messages };
      const json = await gmPostJson(url, { Authorization: `Bearer ${conf.apiKey}` }, payload, {
        retries: 3, onlyStatus200: true, cancelToken, timeoutMs: conf.requestTimeoutMs || 120000
      });
      return String(json?.choices?.[0]?.message?.content ?? "");
    }

    // 构建尝试顺序：先尝试 activeApi，再尝试其他启用的 API
    const tryOrder = [activeApi, ...enabledApis.filter(a => a.id !== activeApi.id)];

    let lastErr = null;
    for (const api of tryOrder) {
      if (cancelToken?.cancelled) throw new Error("Cancelled");
      if (!api.enabled) continue;

      const base = String(api.baseUrl || "").replace(/\/+$/, "");
      const url = base.endsWith("/chat/completions") ? base : base + "/chat/completions";
      const payload = { model: api.model, temperature: conf.temperature ?? 0.2, messages };
      ...
      try {
        const json = await gmPostJson(
          ...
          payload,
          { retries: 2, onlyStatus200: true, cancelToken, timeoutMs: conf.requestTimeoutMs || 120000 }
        );
        const text = json?.choices?.[0]?.message?.content ?? "";
        return String(text);
      } catch (e) {
        lastErr = e;
        console.warn(`[Linux.do Agent] API "${api.name}" 请求失败:`, e?.message || e);

        // 如果不启用 fallback，直接抛出错误
        if (!conf.apiFallback) {
          throw new Error(`API "${api.name}" 请求失败: ${e?.message || e}`);
        }
        // 否则继续尝试下一个 API
        continue;
      }
    ...
    // 所有 API 都失败
    throw new Error(`所有 API 均请求失败。最后错误: ${lastErr?.message || lastErr}`);
  }
  ...
  function parseModelJsonWithRepair(raw, sessionId, store) {
    ...
    try {
      ...
      return { ok: true, obj, repaired: false, jsonText: text };
    } catch { }

    // Step 2: 剥离 Markdown 代码块 (```json ... ``` 或 ``` ... ```)
    ...
    if (codeBlockMatch && codeBlockMatch[1]) {
      ...
      try {
        ...
        store.updateLastAgent(
          ...
          (m) => ({ ...m, kind: "model_json_repaired", content: text, repairedFrom: raw })
        );
        return { ok: true, obj, repaired: true, jsonText: text };
      } catch { }
    }
  ...
  function buildLLMMessagesFromSession(session, conf) {
    ...
    if (conf.includeToolContext) {
      for (const a of session.agent || []) {
        ...
        // ✅ 新增：如果模型输出了无效 JSON，注入错误反馈让它自我纠正
        if (a.kind === "model_parse_error") {
          events.push({
            ts: Number(a.ts || 0),
            role: "user",
            content: `【SYSTEM ERROR】你上一次的响应不是有效的 JSON 格式，解析失败！\n错误详情：${String(a.content).slice(0, 500)}\n\n请务必严格遵守协议，输出纯 JSON（不要包裹在 Markdown 代码块中，不要添加任何自然语言解释）。\n如果你已获取足够信息，请直接输出 type="final" 的回答。`,
          });
        }
  ...
  async function runAgentTurn(sessionId, store, conf, ui, cancelToken, confStore, options = {}) {
    ...
    // 🚨 Force Final: 在最后一轮强制模型输出 final
    if (forceFinal) {
      llmMessages.push({
        role: "user",
        content: `【SYSTEM URGENT - 最后一轮】你已经达到最大工具调用次数限制！此轮你**必须**输出 type="final" 的最终回复。
即使信息不完整，也请根据已获取的工具结果给出尽可能好的回答。
严禁再调用任何工具（type="tool"），否则任务将失败！
请立即输出：{"type":"final","answer":"...","refs":[...]}`,
      });
    }
    const raw = await callOpenAIChat(llmMessages, conf, cancelToken, confStore);
    ...
    if (!parsed.ok) {
      ...
      store.setFSM(sessionId, {
        state: FSM.RUNNING, // 保持运行状态，而不是 ERROR
        isRunning: true,
        lastError: "JSON 解析失败，将提示模型重试",
      });
      ui?.renderAll?.();
      // 返回 done: false 以继续循环，让模型有机会在下一轮纠正
      return { done: false, obj: { type: "parse_error" } };
    }
  ...
  async function runAgent(sessionId, store, conf, ui, confStore) {
    const session = store.all().find((s) => s.id === sessionId);
    if (!session) throw new Error("session not found");
    // 检查是否有可用的 API Key
    const activeApi = confStore?.getActiveApi?.();
    if (!activeApi?.apiKey && !conf.apiKey) throw new Error("请先在设置中填写 API Key");
    ...
    try {
      for (let i = 0; i < maxTurns; i++) {
        if (cancelToken.cancelled) throw new Error("Cancelled");
        // 在最后一轮时强制要求模型输出 final
        const isLastTurn = i === maxTurns - 1;
        const r = await runAgentTurn(sessionId, store, conf, ui, cancelToken, confStore, { forceFinal: isLastTurn });
        if (r.done) {
          CANCEL.delete(sessionId);
          return r.obj;
        }
  ...
  class UI {
    ...
    _renderShell() {
      ...
      drawer.innerHTML = `
        ...
        <div class="${APP_PREFIX}body">
          <div class="${APP_PREFIX}sidebar" id="${APP_PREFIX}sidebar">
            ...
            <!-- Hack: Dummy inputs to trap browser autofill -->
            <div style="height:0;overflow:hidden;position:absolute;z-index:-1;">
              <input type="text" name="fake_user_name" tabindex="-1" />
              <input type="password" name="fake_password" tabindex="-1" />
            </div>
            <input type="search" autocomplete="off" name="random_search_field" class="${APP_PREFIX}filter" id="${APP_PREFIX}sessSearch" placeholder="过滤会话（标题）" />
      ...
      overlay.innerHTML = `
        <div class="${APP_PREFIX}modal" style="max-width:700px;max-height:85vh;overflow-y:auto;">
          <h3 style="margin:0 0 15px 0;">⚙️ 设置</h3>

          <!-- ========== 多 API 管理 ========== -->
          <!-- ========== 多 API 管理 ========== -->
          <div style="margin-bottom:20px;padding:15px;border:1px solid var(--a-border);border-radius:12px;background:rgba(127,127,127,.03);">
            
            <!-- View 1: 列表视图 -->
            <div id="${APP_PREFIX}apiListView">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                <span style="font-weight:900;font-size:14px;">🔗 API 配置</span>
                <button class="${APP_PREFIX}btn" id="${APP_PREFIX}cfgAddApi" style="padding:6px 12px;font-size:12px;">+ 添加 API</button>
              </div>
              <div id="${APP_PREFIX}apiList" style="display:flex;flex-direction:column;gap:8px;max-height:240px;overflow-y:auto;"></div>
              <div style="margin-top:10px;display:flex;align-items:center;gap:8px;">
                <label style="font-weight:700;font-size:12px;color:var(--a-sub);">
                  <input type="checkbox" id="${APP_PREFIX}cfgApiFallback" />
                  启用失败自动切换（Fallback）
                </label>
              </div>
            </div>
            ...
            <!-- View 2: 编辑视图 (默认隐藏) -->
            <div id="${APP_PREFIX}apiEditView" style="display:none;flex-direction:column;gap:12px;">
              <h4 id="${APP_PREFIX}apiEditTitle" style="margin:0;font-size:14px;border-bottom:1px solid var(--a-border);padding-bottom:8px;">编辑 API</h4>
              
              <div class="${APP_PREFIX}formRow">
                <label>从预设快速填充</label>
                <select id="${APP_PREFIX}editPreset" style="width:100%;padding:6px;border-radius:6px;border:1px solid var(--a-border);background:var(--a-bg);">
                  <option value="">-- 选择预设 (可选) --</option>
                  <option value="oa_pro,https://api.openai.com/v1,gpt-4-turbo-preview">OpenAI Official</option>
                  <option value="groq,https://api.groq.com/openai/v1,llama3-70b-8192">Groq</option>
                  <option value="deepseek,https://api.deepseek.com,deepseek-chat">DeepSeek</option>
                  <option value="openrouter,https://openrouter.ai/api/v1,openai/gpt-3.5-turbo">OpenRouter</option>
                </select>
              </div>

              <div class="${APP_PREFIX}formRow">
                <label>名称 (备注)</label>
                <input id="${APP_PREFIX}editName" placeholder="例如：My OpenAI" />
              </div>
              <div class="${APP_PREFIX}formRow">
                <label>Base URL</label>
                <input id="${APP_PREFIX}editBaseUrl" placeholder="https://api...." />
              </div>
              <div class="${APP_PREFIX}formRow">
                <label>Model</label>
                <input id="${APP_PREFIX}editModel" placeholder="gpt-4o" />
              </div>
              <div class="${APP_PREFIX}formRow">
                <label>API Key</label>
                <input id="${APP_PREFIX}editKey" type="password" placeholder="sk-..." />
              </div>

              <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:5px;">
                <button id="${APP_PREFIX}editCancel" class="${APP_PREFIX}btn ${APP_PREFIX}btnGhost">取消</button>
                <button id="${APP_PREFIX}editSave" class="${APP_PREFIX}btn">保存</button>
              </div>
            </div>
          ...
          <div style="display:flex;gap:10px;flex-wrap:wrap;">
            ...
            <div class="${APP_PREFIX}formRow" style="flex:1;min-width:120px;">
              ...
              <input id="${APP_PREFIX}cfgMaxTurns" type="number" step="1" min="1" max="100" />
            </div>
            <div class="${APP_PREFIX}formRow" style="flex:1;min-width:150px;">
              <label>maxContextChars</label>
              <input id="${APP_PREFIX}cfgMaxCtx" type="number" step="500" min="4000" max="800000" />
            </div>
            <div class="${APP_PREFIX}formRow" style="flex:1;min-width:150px;">
              <label>请求超时(ms)</label>
              <input id="${APP_PREFIX}cfgTimeout" type="number" step="1000" min="10000" max="600000" />
            </div>
          </div>
      ...
      this.dom = {
        ...
        sessionFilter: drawer.querySelector(`#${APP_PREFIX}sessSearch`), // Updated ID
        ...
        // 多 API 管理
        cfgApiList: overlay.querySelector(`#${APP_PREFIX}apiList`),
        cfgAddApi: overlay.querySelector(`#${APP_PREFIX}cfgAddApi`),
        cfgApiFallback: overlay.querySelector(`#${APP_PREFIX}cfgApiFallback`),
        // API 编辑器
        apiListView: overlay.querySelector(`#${APP_PREFIX}apiListView`),
        apiEditView: overlay.querySelector(`#${APP_PREFIX}apiEditView`),
        apiEditTitle: overlay.querySelector(`#${APP_PREFIX}apiEditTitle`),
        editPreset: overlay.querySelector(`#${APP_PREFIX}editPreset`),
        editName: overlay.querySelector(`#${APP_PREFIX}editName`),
        editBaseUrl: overlay.querySelector(`#${APP_PREFIX}editBaseUrl`),
        editModel: overlay.querySelector(`#${APP_PREFIX}editModel`),
        editKey: overlay.querySelector(`#${APP_PREFIX}editKey`),
        editCancel: overlay.querySelector(`#${APP_PREFIX}editCancel`),
        editSave: overlay.querySelector(`#${APP_PREFIX}editSave`),
        // 通用配置
        ...
        cfgTimeout: overlay.querySelector(`#${APP_PREFIX}cfgTimeout`),
    ...
    _bind() {
      ...
      d.cfgAddApi.addEventListener("click", () => this.showApiEditor(null));

      // API 编辑器绑定
      d.editPreset.addEventListener("change", () => {
        const val = d.editPreset.value;
        if (!val) return;
        const [name, url, model] = val.split(",");
        if (name && !d.editName.value) d.editName.value = name;
        if (url) d.editBaseUrl.value = url;
        if (model) d.editModel.value = model;
      });
      d.editCancel.addEventListener("click", () => this.showApiList());
      d.editSave.addEventListener("click", () => {
        const id = d.editSave.dataset.id;
        const name = d.editName.value.trim() || "未命名 API";
        const baseUrl = d.editBaseUrl.value.trim() || "https://api.openai.com/v1";
        const model = d.editModel.value.trim() || "gpt-4o-mini";
        const apiKey = d.editKey.value.trim();

        if (id) {
          const old = this.confStore.getApiById(id);
          this.confStore.updateApi(id, { name, baseUrl, model, apiKey: apiKey || old.apiKey });
          this.toast("API 更新成功");
        } else {
          if (!apiKey) return this.toast("请填写 API Key");
          this.confStore.addApi({ name, baseUrl, model, apiKey, enabled: true });
          this.toast("API 添加成功");
        }
        this.showApiList();
        this.renderApiList();
      ...
      // close overlay on ESC, toggle drawer on Alt+A
    ...
    loadConfToUI() {
      ...
      this.dom.cfgMaxTurns.value = String(c.maxTurns ?? 10);
      this.dom.cfgMaxCtx.value = String(c.maxContextChars ?? 60000);
      this.dom.cfgTimeout.value = String(c.requestTimeoutMs ?? 120000);
      this.dom.cfgSys.value = c.systemPrompt || DEFAULT_CONF.systemPrompt;
      this.dom.cfgToolCtx.checked = !!c.includeToolContext;
      this.dom.cfgApiFallback.checked = c.apiFallback !== false;
    ...
    renderApiList() {
      const c = this.confStore.get();
      const apis = c.apiConfigs || [];
      const activeId = c.activeApiId;

      this.dom.cfgApiList.innerHTML = apis.map(api => `
        <div class="${APP_PREFIX}apiCard" data-id="${api.id}" style="
          padding:10px 12px;
          border:2px solid ${api.id === activeId ? 'var(--a-primary)' : 'var(--a-border)'};
          border-radius:10px;
          background:${api.enabled ? 'var(--a-card)' : 'rgba(127,127,127,.1)'};
        ...
        ">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <div style="display:flex;align-items:center;gap:8px;">
              <input type="radio" name="${APP_PREFIX}activeApi" value="${api.id}" ${api.id === activeId ? 'checked' : ''} style="margin:0;"/>
              <strong style="font-size:13px;color:var(--a-text);">${api.name || '未命名'}</strong>
              ${api.id === activeId ? '<span style="font-size:10px;padding:2px 6px;background:var(--a-primary);color:#fff;border-radius:4px;">当前</span>' : ''}
            </div>
            <div style="display:flex;gap:4px;">
              <button class="${APP_PREFIX}apiTest" data-id="${api.id}" style="padding:3px 8px;font-size:11px;cursor:pointer;border:1px solid #4a9;border-radius:4px;background:transparent;color:#4a9;" title="测试连接">测试</button>
              <button class="${APP_PREFIX}apiEdit" data-id="${api.id}" style="padding:3px 8px;font-size:11px;cursor:pointer;border:1px solid var(--a-border);border-radius:4px;background:transparent;color:var(--a-text);">编辑</button>
              <button class="${APP_PREFIX}apiToggle" data-id="${api.id}" style="padding:3px 8px;font-size:11px;cursor:pointer;border:1px solid var(--a-border);border-radius:4px;background:transparent;color:var(--a-sub);">${api.enabled ? '禁用' : '启用'}</button>
              ${apis.length > 1 ? `<button class="${APP_PREFIX}apiDelete" data-id="${api.id}" style="padding:3px 8px;font-size:11px;cursor:pointer;border:1px solid #f55;border-radius:4px;background:transparent;color:#f55;">删除</button>` : ''}
            </div>
          </div>
          <div style="font-size:11px;color:var(--a-sub);word-break:break-all;">
            <span>${api.baseUrl || '-'}</span> · <span>${api.model || '-'}</span> · <span>${api.apiKey ? '***' + api.apiKey.slice(-4) : '无Key'}</span>
          </div>
        </div>
      `).join('');

      // 绑定事件
      this.dom.cfgApiList.querySelectorAll('input[type="radio"]').forEach(r => {
        r.addEventListener('change', () => {
          this.confStore.setActiveApi(r.value);
          this.renderApiList();
        });
      });
      this.dom.cfgApiList.querySelectorAll(`.${APP_PREFIX}apiEdit`).forEach(btn => {
        btn.addEventListener('click', () => this.showApiEditor(btn.dataset.id));
      });
      this.dom.cfgApiList.querySelectorAll(`.${APP_PREFIX}apiToggle`).forEach(btn => {
        btn.addEventListener('click', () => {
          this.confStore.toggleApiEnabled(btn.dataset.id);
          this.renderApiList();
        });
      });
      this.dom.cfgApiList.querySelectorAll(`.${APP_PREFIX}apiDelete`).forEach(btn => {
        btn.addEventListener('click', () => {
          if (confirm('确定删除此 API 配置？')) {
            this.confStore.removeApi(btn.dataset.id);
            this.renderApiList();
          }
        });
      });
      // 测试按钮
      this.dom.cfgApiList.querySelectorAll(`.${APP_PREFIX}apiTest`).forEach(btn => {
        btn.addEventListener('click', () => this.testApi(btn.dataset.id));
      });
    }

    showApiList() {
      this.dom.apiListView.style.display = 'block';
      this.dom.apiEditView.style.display = 'none';
    }

    showApiEditor(id) {
      this.dom.apiListView.style.display = 'none';
      this.dom.apiEditView.style.display = 'flex';

      // 如果是添加模式，清理表单；编辑模式，填充表单
      const api = id ? this.confStore.getApiById(id) : null;
      this.dom.apiEditTitle.textContent = api ? "编辑 API" : "添加新 API";
      this.dom.editPreset.value = "";
      this.dom.editName.value = api?.name || "";
      this.dom.editBaseUrl.value = api?.baseUrl || "";
      this.dom.editModel.value = api?.model || "";
      this.dom.editKey.value = api?.apiKey || "";

      // 保存 ID 以便保存时使用
      this.dom.editSave.dataset.id = id || "";
    }

    async testApi(id) {
      const api = this.confStore.getApiById(id);
      if (!api) return this.toast('API 不存在');
      if (!api.apiKey) return this.toast('请先设置 API Key');

      this.toast(`正在测试 "${api.name}"...`);
      ...
      try {
        const response = await new Promise((resolve, reject) => {
          const timeout = setTimeout(() => reject(new Error("请求超时 (15s)")), 15000);
          GM_xmlhttpRequest({
            method: "POST",
            url,
            headers: {
        ...
        if (response.status === 200) {
          ...
          if (json?.choices?.[0]) {
            this.toast(`✅ "${api.name}" 连接成功！`);
          } else {
            this.toast(`⚠️ "${api.name}" 响应格式异常`);
          }
        } else {
          const errText = response.responseText?.slice(0, 100) || `HTTP ${response.status}`;
          this.toast(`❌ "${api.name}" 失败: ${errText}`);
        }
      } catch (e) {
        this.toast(`❌ "${api.name}" 错误: ${e?.message || e}`);
      }
    }

    saveConfFromUI() {
      const temperature = Math.max(0, Math.min(1, parseFloat(this.dom.cfgTemp.value) || 0.2));
      const maxTurns = Math.max(1, Math.min(200, parseInt(this.dom.cfgMaxTurns.value, 10) || 10));
      const maxContextChars = Math.max(4000, Math.min(8000000, parseInt(this.dom.cfgMaxCtx.value, 10) || 60000));
      const requestTimeoutMs = Math.max(10000, Math.min(600000, parseInt(this.dom.cfgTimeout.value, 10) || 120000));
      ...
      const apiFallback = !!this.dom.cfgApiFallback.checked;
    ...
    renderSessions() {
      ...
      console.log('[Linux.do Agent] renderSessions()', {
        total: all.length,
        filter: q,
        filterId: this.dom.sessionFilter.id,
        filterVal: this.dom.sessionFilter.value
      });
      ...
      wrap.innerHTML = filtered
        .map((s) => {
          ...
          return `
          <div class="${APP_PREFIX}session ${s.id === activeId ? "active" : ""
            }" data-id="${s.id}">
    ...
    renderTools() {
      ...
      wrap.innerHTML = `
        <div class="${APP_PREFIX}toolCard">
          ...
          .map(
            (n) =>
              `<option value="${n}" ${n === name ? "selected" : ""
              }>${n}</option>`
          )
          .join("")}
              </select>
    ...
    async send() {
      ...
      const activeApi = this.confStore.getActiveApi();
      if (!activeApi?.apiKey && !conf.apiKey) {
      ...
      try {
        await runAgent(s.id, this.store, conf, this, this.confStore);
        this.toast("完成");
      } catch (e) {
  ...
  function init() {
    ...
    const ui = new UI(store, confStore);

    // 注册油猴菜单命令
    try {
      GM_registerMenuCommand("🤖 打开 Agent 面板 (Alt+A)", () => ui.toggleDrawer(true));
      GM_registerMenuCommand("⚙️ 打开设置", () => {
        ui.loadConfToUI();
        ui.dom.overlay.classList.add("open");
      });
    } catch (e) {
      console.warn("[Linux.do Agent] GM_registerMenuCommand 不可用:", e);
    }
  }
