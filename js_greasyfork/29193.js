...
// @grant        GM_listValues
// @downloadURL https://update.greasyfork.org/scripts/29193/bv7_bbc_helper.user.js
// @updateURL https://update.greasyfork.org/scripts/29193/bv7_bbc_helper.meta.js
// ==/UserScript==
...
//				console.log(page.idHrefTripOffersActive);
		...
		function ToolSendMessagePageTripOffersActive() { // Class
			...
			if (ok) {
				...
				this.changeStart = function() {
					var disabled = (nodeStart.checked ? 'disabled' : '');
					...
					for(var i = 0; i < page.trips.length; i++) if(page.trips[i].actual) {
						var disabled2                          = ((nodeStart.checked || page.trips[i].nodeDefault.checked)? 'disabled' : '')
...
//				console.log(this.plan);
		...
		if (regHrefTripOffersActive.test(location.href)) {
			...
			if (ok) tools.sendMessage = new ToolSendMessagePageTripOffersActive();
...
//		console.log(ok);
//		console.log(tools.sendMessage);
//		console.log(tools.sendMessage.plan);
		...
		this.elChangeStart = function(event) {
			event.preventDefault();
			if (nodeStart.checked)
				this.runPlan();
			else {
			...
			GM_setValue(idStart, nodeStart.checked);
			this.changeStart();
		};

		this.changeStart = function() {
			nodeReset.disabled = (nodeStart.checked ? 'disabled' : '');
		...
		this.counter = function() {
			...
			idTimeoutCounter = setTimeout(function(me) {return function() {me.counter();};} (this), timeoutCounter);
		};
...
//		console.log(ok);
		if (ok) {
			...
			nodeStart = document.createElement('input');
			nodeStart.setAttribute('type', 'checkbox');
			nodeStart.checked =  GM_getValue(idStart, false);
			nodeTimer = document.createTextNode('');
			nodeStart.addEventListener('change', function(me) {return function(event) {me.elChangeStart(event);};}(this));
			...
			nodeTool.appendChild(nodeStart);
