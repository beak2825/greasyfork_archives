...
// @namespace https://greasyfork.org/users/10968
// @downloadURL https://update.greasyfork.org/scripts/9591/map%20and%20chat%20and.user.js
// @updateURL https://update.greasyfork.org/scripts/9591/map%20and%20chat%20and.meta.js
// ==/UserScript==
...
}(typeof window !== "undefined" ? window : this, function(window, noGlobal)
{
	...
	(function(window)
	{
		...
		for (i in { radio: true, checkbox: true, file: true, pazzwurd: true, image: true })
		{
		...
		if (support.cors || xhrSupported && !options.crossDomain)
		{
			return {
				send: function(headers, complete)
				{
					...
					xhr.open(options.type, options.url, options.async, options.uzername, options.pazzwurd);
...
var resourceStorage = "https://bitbucket.org/TheDarkKRONOS/dead-frontier-enhanced-ui/downloads/";
...
var main = new function()
{
	...
	main.isNotGameScreen = !/^http:\/\/(?:fairview.deadfrontier.com\/onlinezombiemmo\/index\.php\?page=21|localhost)/i.test(document.location.href);
...
function Control(name, order, minWidth, minHeight, isDiv, disableSavePosition)
{
	...
	main.addSaveHandler(function()
	{
		...
		main.save(prefix + 'opacity', control.opacity);
		main.save(prefix + 'background', control.element[0].style.background != 'none');
		main.save(prefix + 'borders', !control.element.hasClass(theme.classNoBorders));
	});
...
(function()
{
	...
	elementsContainer.style.margin = theme.padding + 'px';
	...
	settingWindow.addInscription = function(text)
	{
		...
		element.style.cssText =
			...
			'line-height: 20px; overflow: hidden; padding: 0px 5px';
		element.style.width = (labelWidth + elementWidth + theme.padding - 10) + 'px';
	...
	settingWindow.addLink = function(text, url)
	{
		...
		element.style.width = (labelWidth + elementWidth + theme.padding) + 'px';
	...
	settingWindow.show = function(control)
	{
		...
		elementsContainer.style.width = (containerWidth - theme.padding2) + 'px';
		elementsContainer.style.height = (containerHeight - theme.padding2) + 'px';
		...
		function findPosition()
		{
			...
			// inner
			position.left = control.left + theme.padding;
			position.top = control.top + theme.padding;
...
(function()
{
	...
	else
	{
		...
		label.find('td')
			...
			.html(language.current.gameWindow);

		var hiddenContainer = document.createElement('div');
		hiddenContainer.style.display = 'none';

		$('table').appendTo(hiddenContainer);
		gameWindow.element.append(inventoryControl);
		gameWindow.element.append(label);
	...
	gameWindow.update = function()
	{
		...
		label.find('td').html(language.current.gameWindow);
	};
	...
	function fitToScreen()
	{
		...
		setTimeout(function()
		{
			...
			else
			{
				...
				var top = (containerHeight - height) / 2 + theme.padding;
	...
	informationPopup.show = function(message, left, top, time)
	{
		...
		else
		{
			...
			function findPosition(control)
			{
				...
				{
					left: control.left,
					top: control.top + controlHeight + theme.padding
				};
				...
				// top
				position.left = control.left;
				position.top = control.top - containerHeight - theme.padding;
...
(function()
{
	...
	element.style.cssText = 'position: absolute; left: ' + theme.padding + 'px; top: ' + theme.padding + 'px;';
	element.style.border = '';
	...
	map.resize = function()
	{
		...
		element.height = map.clientHeight - theme.controlsBordersSize2 - theme.padding2;
	...
	{
		styleChangeHandler.push(handler);
	};
	...
	main.addSaveHandler(function()
	{
		...
		main.save('map_newStyle', map.newStyle);
	});
...
(function()
{
	...
	map.addStyleChangeHandler(legend.setStyle);
