...
// @downloadURL https://update.greasyfork.org/scripts/430796/Steam%E6%B7%BB%E5%8A%A0.user.js
// @updateURL https://update.greasyfork.org/scripts/430796/Steam%E6%B7%BB%E5%8A%A0.meta.js
// ==/UserScript==
    ...
    */});
    ...
    let batchredeemscript=getText(function(){/*
        ...
        var s_s_s_myTexts = {
            ...
            redeeming: '激活中',
            waiting: '等待中',
            showUnusedKey: '顯示未使用的Key',
            hideUnusedKey: '隱藏未使用的Key',
        };
        ...
        function s_s_s_redeemKeys() {
            ...
            if (keys.length <= 0) {
                ShowAlertDialog("失敗","沒有找到符合格式的key！");
                return;
            }
    ...
    */});
    ...
    let batchredeemkey=getText(function(){/*
        ...
        <div style="float: left;">
            <textarea class="form-control" cols="80" rows="10" name="inputKey" id="inputKey" class="registerkey_input_box_text" placeholder="支持批量激活，可以把整個網頁文字複製過來&#10;若一次激活的Key的數量超過9個則會自動分批激活（等待20秒）"></textarea>
        </div>
        ...
        <br>
                <div class="table-responsive table-condensed">
                    <table class="table table-hover" style="display: none">
                        <caption><h2>激活記錄</h2></caption><thead><th>No.</th><th>Key</th>
                            <th>結果</th><th>詳情</th><th>Sub</th></thead><tbody></tbody>
        </table></div><br><br><br>
...
`;
    ...
    let addtowishlistscript=getText(function(){/*
            ...
            function s_s_s_addtowishlistfunction(){
                ...
                if(!args[0].trim().match(/^[\d]+$/)){
                    ShowAlertDialog("失敗","請輸入APP ID，多個APP ID用“,”隔開");
                    return;
                }
    ...
    let addtowishlist=getText(function(){/*
            ...
            <br>
                <br>
                <h2>輸入APP ID添加鎖偏好遊戲至願望單，輸入SUB ID在Steam上添加免費產品。多個ID之間用“,”隔開。 (<span style="color:#c6d4df;">鎖國區的遊戲需要切換賬單。切換帳單會有點延遲，取決於網速，一開始點不動的話，等一會就能點了。</span>)</h2>
                <div>輸入APP ID</div>
    ...
    var addfreescript=getText(function(){/*
        ...
        function s_s_s_addfreefunction(){
            let args=arguments[0].split(",");
            if(!args[0].trim().match(/^[\d]+$/)){
                ShowAlertDialog("失敗","請輸入SUB ID，多個SUB ID用“,”隔開");
                return;
            }
            ...
            if (location.hostname !== 'store.steampowered.com') {
                alert('請在steam商店運行本代碼！');
                return;
            } else if (typeof jQuery !== 'function') {
                ShowAlertDialog('失敗', '腳本需要jQuery庫！');
                return;
            } else if (document.getElementById('header_notification_area') === null) {
                ShowAlertDialog('失敗', '你必須處於登錄狀態.');
                return;
            }
            ...
            let loaded = 0,
                ...
                modal = ShowBlockingWaitDialog('正在添加...', '請等待所有請求結束。'),
            ...
            function addfreelicense(){
                if(GDynamicStore.s_rgOwnedPackages[freePackages[i]]){
                    ...
                    if(addFreeOwnedEle.getElementsByClassName("s_s_s_s_free_game_list_row").length==0){
                        addFreeOwnedEle.innerHTML+=`<span class="s_s_s_s_free_game_list_index">索引</span>&nbsp;&nbsp;<span class="s_s_s_s_free_game_list_subid">SUB ID</span>&nbsp;&nbsp;<span class="s_s_s_s_free_game_list_status">狀態</span><br>`;
                    }
                    let freegamelistownedItem=`<span class="s_s_s_s_free_game_list_row"><span class="s_s_s_s_free_game_list_index">${ownedCount}</span>&nbsp;&nbsp;<span class="s_s_s_s_free_game_list_subid"><a target="_blank" href="https://steamdb.info/sub/${freePackages[i]}">${freePackages[i]}</a></span>&nbsp;&nbsp;<span class="s_s_s_s_free_game_list_status">已擁有</span></span><br>`;
                    addFreeOwnedEle.innerHTML+=freegamelistownedItem;
                    modal.Dismiss();
 
                    if (loaded === total) {
                        ShowAlertDialog('完成！', '已全部添加完畢。');
                    } else {
                        modal = ShowBlockingWaitDialog('正在添加...', '載入 ' + loaded + '/' + total);
                        ++i;
                        addfreelicense();
                    }
                }else{
                    jQuery.post(
                        ...
                        function(data) {
                            ...
                            if(afs.length>0){
                                ...
                                if(freegamelistEle.getElementsByClassName("s_s_s_s_free_game_list_row").length==0){
                                    freegamelistEle.innerHTML+=`<span class="s_s_s_s_free_game_list_index">索引</span>&nbsp;&nbsp;<span class="s_s_s_s_free_game_list_subid">SUB ID</span>&nbsp;&nbsp;<span class="s_s_s_s_free_game_list_status">狀態</span>&nbsp;&nbsp;<span class="s_s_s_s_free_game_list_name">遊戲名稱</span><br>`;
                                }
                                let addFreeSuccess=afs[0];
                                let gameName=/([^\n\r]*) 已被綁定至您的 Steam 帳戶。 /.exec(addFreeSuccess.firstElementChild.innerHTML)[1];
                            ...
                            if(errs.length>0){
                                ...
                                if(addFreeFailedEle.getElementsByClassName("s_s_s_s_free_game_list_row").length==0){
                                    addFreeFailedEle.innerHTML+=`<span class="s_s_s_s_free_game_list_index">索引</span>&nbsp;&nbsp;<span class="s_s_s_s_free_game_list_subid">SUB ID</span>&nbsp;&nbsp;<span class="s_s_s_s_free_game_list_status">狀態</span><br>`;
                                }
                        ...
                        if (loaded === total) {
                            ...
                            let freegamefailedlistItem=`<span class="s_s_s_s_free_game_list_row"><span class="s_s_s_s_free_game_list_index">${failCount}</span>&nbsp;&nbsp;<span class="s_s_s_s_free_game_list_subid"><a target="_blank" href="https://steamdb.info/sub/${freePackages[i]}">${freePackages[i]}</a></span>&nbsp;&nbsp;<span class="s_s_s_s_free_game_list_status">發送請求錯誤！ </span></span><br>`;
                            addFreeFailedEle.innerHTML+=freegamefailedlistItem;
                            setTimeout(addfreelicense,200);
                        }
                    });
    ...
    */});
    ...
    let addfree=getText(function(){/*
        ...
        <br>
                <br>
                <h2>輸入SUB ID在Steam上添加免費產品。多個ID之間用“,”隔開。 (<span style="color:#c6d4df;">鎖國區的遊戲需要切換賬單。切換帳單會有點延遲，取決於網速，一開始點不動的話，等一會就能點了。<spans style="color:#d55;display:none;">注意：已經添加成功的SUB ID，如果在短時間內重複添加相同的SUB ID，因為網頁緩存的關係請按下Ctrl+F5快捷鍵強制刷新，否則腳本無法判斷該SUB ID是否已擁有！</span></span>)</h2>
            <div>輸入SUB ID</div>
        <div style="float: left;" class="block_content checkout_content_box">
