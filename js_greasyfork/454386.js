// ==UserScript==
// @name         DianpingToolTest20221107
// @namespace    http://tampermonkey.net/
// @version      0.01
// @description  Tool for Dianping comments
// @author       unknown
...
// @downloadURL https://update.greasyfork.org/scripts/454386/DianpingToolTest20221107.user.js
// @updateURL https://update.greasyfork.org/scripts/454386/DianpingToolTest20221107.meta.js
// ==/UserScript==
...
(function () {
    ...
    const getResult2 = async () => {
        ...
        const getFileViaUrl2 = (url,duration) => {
            return new Promise((resolve, reject) => {
                GM.xmlHttpRequest({
                    method: 'GET',
                    url: url,
                    ...
                    onload: response => {
                        ...
                        { 
                            setTimeout(()=>{
                                resolve(response.responseXML)//获取xmldom对象
                            },duration)
                            }           
                        else return resolve('');
                    }
        ...
        comments.forEach(async cmt => {
            ...
            var gender = "未知";
            if (namehref != "") {
                const userinfo = await getFileViaUrl2(namehref,2500);
                console.log(userinfo);
                iplocation = userinfo.getElementsByTagName("span")[24].nodeValue;
                gender = userinfo.getElementsByTagName("span")[24].firstElementChild.className;
            }
            print(gender, iplocation)
            var rank = 0.0;//初始化评分制
            //提取用户评价星级（span的class属性值为“sml-rank-stars sml-str50 star”，str50即为5颗星，str30为3颗星）
            if (rankDivs[timeIndex].getElementsByTagName("span").length > 0) {//div没有span子节点的话，说明该条评论中没有星级打分数据，打分值设为0;如果有的话，提取第1个子节点的数据即可；
                var rankstring = parseFloat(rankDivs[timeIndex].firstElementChild.className.split(' ')[1].split('-')[1].substring(3, 5));
            ...
            //因此，需要替换为中文逗号，保证一条评论内容被当作一列处理。
            foo = foo.replace(/,/ig, '，');
            result.push([time, name, foo.trim(), rank, namehref, gender, iplocation]);
            timeIndex++;
            nameIndex++;
        });
    ...
    getUserTimeCommentScoreUrl2.innerHTML = '采集用户名、时间、评论、打分、URL及用户注册信息(测试)';
