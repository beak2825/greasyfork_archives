// ==UserScript==
// @name   🌱【免费版】轻松在线学习平台|湛江科技学院|东莞城市学院|如需秒过、无人值守等高级收费版本见文档：https://doc.zhanyc.cn/pages/qszxxx/
// @namespace    http://jb.zhanyc.cn/
// @icon    https://js.zhanyc.cn/img/js-logo.svg
// @version      1.1
// @description  当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制、自动下一集功能。如需秒过、自动换课程、全自动无人值守等高级功能可升级付费版本，一杯咖啡钱，保你无忧学习，且永久使用|接各类脚本开发、代挂工作，微信：zhanyc_cn 备用微信:zhanfengkuo 个人网站：http://doc.zhanyc.cn
// @author       zfk
// @include    *://*qsxueli.com/*
// @grant       GM_getValue
// @grant       GM_setValue
...
// @antifeature 
// @license GPL
// @downloadURL https://update.greasyfork.org/scripts/502705/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E8%BD%BB%E6%9D%BE%E5%9C%A8%E7%BA%BF%E5%AD%A6%E4%B9%A0%E5%B9%B3%E5%8F%B0%7C%E6%B9%9B%E6%B1%9F%E7%A7%91%E6%8A%80%E5%AD%A6%E9%99%A2%7C%E4%B8%9C%E8%8E%9E%E5%9F%8E%E5%B8%82%E5%AD%A6%E9%99%A2%7C%E5%A6%82%E9%9C%80%E7%A7%92%E8%BF%87%E3%80%81%E6%97%A0%E4%BA%BA%E5%80%BC%E5%AE%88%E7%AD%89%E9%AB%98%E7%BA%A7%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%E8%A7%81%E6%96%87%E6%A1%A3%EF%BC%9Ahttps%3Adoczhanyccnpagesqszxxx.user.js
// @updateURL https://update.greasyfork.org/scripts/502705/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E8%BD%BB%E6%9D%BE%E5%9C%A8%E7%BA%BF%E5%AD%A6%E4%B9%A0%E5%B9%B3%E5%8F%B0%7C%E6%B9%9B%E6%B1%9F%E7%A7%91%E6%8A%80%E5%AD%A6%E9%99%A2%7C%E4%B8%9C%E8%8E%9E%E5%9F%8E%E5%B8%82%E5%AD%A6%E9%99%A2%7C%E5%A6%82%E9%9C%80%E7%A7%92%E8%BF%87%E3%80%81%E6%97%A0%E4%BA%BA%E5%80%BC%E5%AE%88%E7%AD%89%E9%AB%98%E7%BA%A7%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%E8%A7%81%E6%96%87%E6%A1%A3%EF%BC%9Ahttps%3Adoczhanyccnpagesqszxxx.meta.js
// ==/UserScript==
(function () {
    ...
    let freeTips = " 当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制、自动下一集功能。如需秒过、无人值守等高级功能，可点击下方按钮查看付费版本"
    let docUrl = "http://doc.zhanyc.cn/pages/qszxxx/";
    let plugMain = Object.assign(baseConfig, {
        ...
        async runByUrl(url) {
            ...
            if (url.includes("/web/courseInfo?")) {
                plugMain.page_study();
            } else if (url.includes("/#/web/index")) {
                plugMain.page_list();
            }
        },
        ...
        async page_study() {
            ...
            plugMain.closeWaitConfrimWin();
            let timeout = 2;
            plugMain.tipsMsg("请等待脚本操作", 10 * 1000)
            let hasZl = await plugMain.waitOf(a => $(".ziliaoList>ul>li").length > 0, 500, 10, true)
            await plugMain.waitOf(a => $(".jindu").text().includes('平时分'))
            let jdArr = $(".jindu").text().replace(/[^/\.0-9]/g, '').trim().split('/').map(a => Number(a))
            await plugMain.waitTimeout(1000)
            if (jdArr[0] >= 80) {
                plugMain.showPaidContent(`免费版本不包含秒过、自动换课、无人值守功能，如需使用请安装收费版本`);
                return
            }
            if (hasZl) {
                if ($(".ziliaoList>ul>li").length > 0) {
                    plugMain.page_video_zl()
                    return
                }
            } else {
                plugMain.page_video()
        ...
        async page_video_zl() {
            console.log("%c page_video_zl", "background:rgb(0,0,0);color:#fff");
            if ($(".ke_menu li.act").text().trim() != '课程资料') {
                plugMain.getElByText($(".ke_menu li"), '课程资料', 'like').click()
            }
            let $el = null
            let index = 0
            $(".ziliaoList>ul>li").each((i, el) => {
                let jd = Number($(el).find('[aria-valuenow]').attr('aria-valuenow'))
                if (jd < 100) {
                    $el = $(el)
                    index = i
                    return false
            ...
            if ($el == null) {
                plugMain.alertMsg("资料学习完成")
                return;
            }
            $el.find('.zl_rightBox_btn .see_btn_icn').parent().click()

            let timeout = 0
            if (plugMain.pageData.index.zk == null) {
                plugMain.pageData.index.zk = setInterval(() => {
                    if (timeout > 0) {
                        timeout -= 2
                        return
                    }
                    let jd = Number($(".ziliaoList>ul>li.act").find('[aria-valuenow]').attr('aria-valuenow'))
                    if (jd == 100) {
                        plugMain.page_video_zl()
                        timeout = 6
                        return
        ...
        async page_video() {
            if ($(".ke_menu li.act").text().trim() != '视频课程') {
                plugMain.getElByText($(".ke_menu li"), '视频课程', 'like').click()
            }
            $(".shipinList ul>li").each((i, el) => {
                let jd = Number($(el).find('[aria-valuenow]').attr('aria-valuenow'))
                if (jd < 100) {
                    $el = $(el)
                    index = i
            ...
            plugMain.pageData.video.index = setInterval(async () => {
                try {
                    ...
                    // plugMain.triggerMouseMove()
                    if (!plugMain.getVideo()) {
                        console.log("%c zfk no video", "background:rgb(0,0,0);color:#fff");
                        return;
                    }
                    let jd = Number($(".shipinList ul>li.act").find('[aria-valuenow]').attr('aria-valuenow'))
                    if (jd == 100) {
                        // plugMain.page_video()

                        plugMain.pageData.waitTime = 6
                        location.reload()
                        return
                    }
