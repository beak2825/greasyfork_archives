// ==UserScript==
// @name         AutoInteract - Footmundo só pros de verdade
...
// @downloadURL https://update.greasyfork.org/scripts/464645/AutoInteract%20-%20Footmundo%20s%C3%B3%20pros%20de%20verdade.user.js
// @updateURL https://update.greasyfork.org/scripts/464645/AutoInteract%20-%20Footmundo%20s%C3%B3%20pros%20de%20verdade.meta.js
// ==/UserScript==
...
$("head").append(style);
    ...
    const selecionarLigacaoAmigavel = async function() {
      for (let i = 1; i <= 3; i++) {
        await aguardarCarregamentoIframe();
        const iframeDoc = iframe.contentDocument || iframe.contentWindow.document;

        const selectInteracao = iframeDoc.querySelector("select[name='interacao_select']");
        if (!selectInteracao) {
    ...
    const selecionarInteracaoAmigavel = async function() {
      for (let i = 1; i <= 3; i++) {
        ...
        const selectInteracao = iframeDoc.querySelector("select[name='select_interacao']");
        if (!selectInteracao) {
          iframe.contentWindow.close();
          return;
        }
    ...
    // ligação amorosa
    const selecionarLigacaoAmorosa = async function() {
      for (let i = 1; i <= 3; i++) {
        await aguardarCarregamentoIframe();
        const iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
  ...
  // Interação amorosa
    ...
    const iniciarInteracao = async function() {
      ...
      if (isProibido & jogadorAtual.selectedOption === "Amigáveis") {
          ...
          await selecionarLigacaoAmigavel();
      } else if (isProibido & jogadorAtual.selectedOption === "Amorosas") {
          await carregarUrl(jogadorAtual.ligacao);

          await selecionarLigacaoAmorosa();
      }
        else {
