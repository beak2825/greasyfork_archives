...
// @grant       GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/513395/SendToClient-Tags.user.js
// @updateURL https://update.greasyfork.org/scripts/513395/SendToClient-Tags.meta.js
// ==/UserScript==
...
const addTorrent = async (torrentUrl, clientUrl, username, password, client, path, category, tags) => {
  let implementations = {
    qbit: async () => {
    ...
    rutorrent: async () => {
      ...
      XFetch.post(`${clientUrl}/php/addtorrent.php?json=1`, form, headers);
    }
  };
...
async function testClient(clientUrl, username, password, client) {
  let clients = {
    ...
    rutorrent: async () => {
      ...
      let res = await XFetch.post(`${clientUrl}/php/addtorrent.php?json=1`, null, headers);
        console.log(res);
      if (res.raw.status !== 401) {
        return true;
      }
...
class Profile {
  constructor(id, name, host, username, password, client, saveLocation, category, tags, linkedTo = []) {
    this.id = id;
    this.name = name;
    this.host = host;
  ...
  async addTorrent(torrent_uri) {
    return await addTorrent(torrent_uri, this.host, this.username, this.password, this.client, this.saveLocation, this.category, this.tags);
  }
}
  ...
  load: async function () {
    ...
    if (profiles) {
      this.profiles = JSON.parse(profiles).map(p => {
        var _p$category, _p$tags, _p$linkedTo;
        return new Profile(p.id, p.name, p.host, p.username, p.password, p.client, p.saveLocation, (_p$category = p.category) != null ? _p$category : '', (_p$tags = p.tags) != null ? _p$tags : '', (_p$linkedTo = p.linkedTo) != null ? _p$linkedTo : []);
      });
    }
...
const profileOnSave = (e, shadow) => {
  ...
  profile.tags = shadow.querySelector('#tags').value;
...
function profileSelectHandler(e, shadow) {
  ...
  shadow.querySelector('#tags').value = profile.tags;
