// ==UserScript==
// @name        CosmosisT's TinyChat Script (CTS V1.8.18) (CRACKED)
// @version     1.8.18
...
// @downloadURL https://update.greasyfork.org/scripts/422666/CosmosisT%27s%20TinyChat%20Script%20%28CTS%20V1818%29%20%28CRACKED%29.user.js
// @updateURL https://update.greasyfork.org/scripts/422666/CosmosisT%27s%20TinyChat%20Script%20%28CTS%20V1818%29%20%28CRACKED%29.meta.js
// ==/UserScript==
(function() {
    "use strict";
    //DEBUGGER
    ...
    function CTSRoomPrepare() {
        ...
        window.YouTube = {
            ...
            Init: function() {
                if (CTS.YouTube.CurrentTrack.ID != undefined) {
                    if (window.YouTube.Ready) {
                        ...
                        element.innerHTML = '<div class="volume"></div><div class="duration"></div><button id="yt_voldown">🔉</button><button id="yt_volup">🔊</button><button id="yt_volmute">🔇</button>' + ((!CTS.Project.isTouchScreen) ? '<button id="yt_fullscreen">⛶</button>' + ((!CTS.ThemeChange) ? '<button id="yt_behindchat">↸</button>' : '') : '') + ((CTS.Me.mod) ? '<button id="yt_close" style="float:right;">X</button>' : '');
            ...
            onPlayerReady: function(event) {
                event.target.setVolume(window.YouTube.Volume);
                event.target.seekTo(CTS.YouTube.CurrentTrack.offset);
    ...
    function YouTubeMute (reset) {
        var elem = (!CTS.ThemeChange) ? SideMenuElement.querySelector("#sidemenu-content") : ChatListElement;
        if(reset === true) {
            if (window.YouTube.Muted) {
                window.YouTube.Player.mute();
            } else {
                window.YouTube.Player.unMute();
            }
        } else {
            if (window.YouTube.Player.isMuted()) {
                window.YouTube.Player.unMute();
                window.YouTube.Muted = false;
            } else {
                window.YouTube.Player.mute();
                window.YouTube.Muted = true;
            }
        }
    }
    function YouTubeVolumeUp() {
        var vol = window.YouTube.Volume;
        vol = (vol < 100) ? (vol >= 50) ? vol + 10 : (vol >= 20) ? vol + 5 : vol + 1 : vol;
        window.YouTube.Player.setVolume(vol);
    ...
    function YouTubeVolumeDown() {
        var vol = window.YouTube.Volume;
        vol = (vol > 1) ? (vol >= 60) ? vol - 10 : (vol >= 25) ? vol - 5 : vol - 1 : vol;
        window.YouTube.Player.setVolume(vol);
        window.YouTube.Volume = vol;
        YouTubeVolumeIndicator();
    }
    ...
    var ServerInList = {
        ...
        yut_play: function() {
            if (CTS.ScriptInit) {
                ...
                if (window.TinychatApp.BLL.SettingsFeature.prototype.getSettings().enableYoutube) window.YouTube.Init(arguments[0].item.id, CTS.YouTube.CurrentTrack.offset);
