...
// @require https://update.greasyfork.org/scripts/518420/1532441/MaxShowBox.js
// @require https://update.greasyfork.org/scripts/522187/1532433/Kquery.js
// @downloadURL https://update.greasyfork.org/scripts/528465/hitomi%2030%20pc%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/528465/hitomi%2030%20pc%E7%89%88.meta.js
// ==/UserScript==
...
function NewHome2(h){
	//2025-02-06 修改Pc的显示
	const OldStart = h.Start.bind(h);
	h.Start = ()=>{
		OldStart();
		if($(window).height()>$(window).width()){return;}
		$('.get-all-books-bu').css({
			'font-size':'100%',
			'width':'auto'
		})
		if(window.location.href.indexOf('reader')<0){
			...
			`
			$('body').append(thcss)
			//WaitingElement(()=>$('ul.thumbnail-list a')).then(atag=>atag.attr('target',"_blank"));
		}
...
class Home{
	Start(){
		...
		bu.on('touchstart click',function(event){
			event.stopPropagation();
			if($(window).height()>$(window).width() && event.type=="click"){return;}
	...
	Add_sameKard(item){
		if($(item).find('.dj-img-cont img:first').length==0){return;}
...
function NewList(){
	...
	//return ll;
	//2025-02-05 增加pc的逻辑
	const ll2 = NewList2(ll);
	//return ll2;
	//2025-2-14 修改reader page初始页的逻辑
	const ll3 = NewList3(ll2);
	return ll3;
}
function NewList1(l){
	let ll = l;
	...
	ll.Auto_download = ()=>{
		const img = ()=>$('.lt-showbox img:visible').first()
...
function NewList2(lt){
	//2025-02-05 增加pc的逻辑
	//复写GetPageCount，增加#single-page-select option
	lt.GetPageCount = ()=>{
		return new Promise(resolve=>{
			const option = ()=>{return $('#mobile-single-page-select option,#single-page-select option')};
			lt.WaitingElement(option)
				.then(ele=>{
	...
	//复写Get_iframe_img，增加#comicImages .lillie
	lt.Get_iframe_img = (iframe)=>{
		return new Promise(resolve=>{
			let check = setInterval(function(){
				let img = $(iframe.contentWindow.document).find('#mobileImages .lillie,#comicImages .lillie');
	...
	lt.Change_pb_pc = ()=>{
		...
		$('.lt-showbox .big-and-small').hide();
		$('.lt-showbox .page').css({'bottom':'97vh','left':'60%'});
		$('.thumb-bu').css({
			'z-index':9999,
			'bottom':0,
			'opacity':'.1'
		})
		$('.thumb-box').css('height','100%');
		$('ul.thumbnail-list').css({
			'height':'100vh',
			'padding':'10px',
			'grid-template-columns':'1fr 1fr 1fr 1fr 1fr'
		})
	}
	lt.StartBuAfterEvent = ()=>{
		lt.Change_pb_pc();
	}
	//改写pc显示图片的逻辑
	lt.tw = new TwoPartImg();
	lt.tw.box.css({
		'top':'40px',
		'position':'fixed',
	...
	lt.tw.box.on('click',function(){
		const img = $('.lt-showbox img:visible');
	...
	const OldShowBox = lt.NewShowbox;
	lt.Change_NewShowbox(()=>{
		const sb = OldShowBox();
		const ob = sb.Add_show_box.bind(sb);
		sb.Add_show_box = ()=>{
			ob();
			if($(window).height()>$(window).width()){return;}
			lt.tw.box.show();
		}
		const OldeShowimg = sb.Show_img.bind(sb);
		sb.Show_img = (index)=>{
			OldeShowimg(index);
			if($(window).height()>$(window).width()){return;}
			lt.WaitingElement(()=>$('.lt-showbox img').eq(index).filter(function(){return this.src}))
				.then(img=>{
					lt.tw.Add(img);
				})
		}
	...
	const prevbu = $("<button></button>").click(function(){
			// 创建一个触摸事件对象
			var touchEvent = new TouchEvent('touchstart', {
			    bubbles: true,
			    cancelable: true
	...
	const nextbu = $("<button></button>").click(function(){
			// 创建一个触摸事件对象
			var touchEvent = new TouchEvent('touchstart', {
				bubbles: true,
				cancelable: true
...
function NewList3(lt){
	//2025-2-14 修改reader page初始页的逻辑
	let startPage = window.location.href.match(/\d+$/g)[0];
	if(startPage){startPage = Number(startPage) - 1;}else{startPage = 0;}
	//alert(startPage);
	lt.startPage = startPage;
	
	//2025-2-15 修改pc排版，避免head被挡
	const oldstart = lt.Start.bind(lt);
	lt.Start = ()=>{
		oldstart();
		if($(window).width()>$(window).height()){
			WaitingElement(()=>$('.up-and-down')).then(tag=>tag.hide());
		}
	};
	return lt;
}
...
function List(){
	...
	lt.NewShowbox = NewShowbox;
	lt.Change_NewShowbox = foo=>{
		NewShowbox = foo;
	}
	...
	lt.Showbox = Showbox;
	lt.Change_Showbox_class = foo=> Showbox = foo;
...
function TwoPartImg(){
	...
	let pc_css = `
		<style>
			...
			.two-part-img-box div{
				background-size: cover;
				width: 100%;
				height: 100%;
				background-position: center top;
			}
			.two-part-img-box div:last-of-type{
	...
	$('body').append(box);
	$('body').append(pc_css);
	this.box = $('.two-part-img-box');
	this.Add = (oneimg)=>{
		this.box.find('.img').css("background-image",`url("${oneimg[0].src}")`);
	}
}
window.TwoPartImg = TwoPartImg
