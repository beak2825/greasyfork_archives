...
// @author       minjie.chen
// @match        http://192.168.10.2:82/secure/RapidBoard.jspa?rapidView=16*
// @icon         https://www.google.com/s2/favicons?sz=64&domain=10.2
// @grant        none
// @license      phoenixylf
// @downloadURL https://update.greasyfork.org/scripts/499715/Emb%20Jira%20Tools.user.js
// @updateURL https://update.greasyfork.org/scripts/499715/Emb%20Jira%20Tools.meta.js
// ==/UserScript==
...
(function() {
    ...
    function doGetDataAndShowWorkload(){
        // 页面及所有资源加载完成后执行的代码
        $.get('http://192.168.10.2:82/rest/greenhopper/1.0/xboard/work/allData.json?rapidViewId=16&selectedProjectKey=EMB', function(data) {
            // 请求成功，打印响应数据
            showWorkload(data);
        ...
        for(var i = 0;i<timeData.thisMonthResult.length;i++){
            ...
            for(var j = 0;j<timeData.thisMonthResult[i].extraFields.length;j++){
                if(timeData.thisMonthResult[i].extraFields[j].label == "到期日" || timeData.thisMonthResult[i].extraFields[j].label == "Due Date" || allData[i].extraFields[j].label == "Target end"){
                     ...
                     var regexStr1 = /<time[^>]*>([^<]*)<\/time>/;
                     var match = dueDate.match(regexStr1);
                     if (match && match[1]) {
                         dueDate = match[1];
                     }
                }
        ...
        for(var i = 0;i<timeData.nextMonthResult.length;i++){
            ...
            for(var j = 0;j<timeData.nextMonthResult[i].extraFields.length;j++){
                if(timeData.nextMonthResult[i].extraFields[j].label == "到期日" || timeData.nextMonthResult[i].extraFields[j].label == "Due Date" || allData[i].extraFields[j].label == "Target end"){
                     ...
                     var regexStr2 = /<time[^>]*>([^<]*)<\/time>/;
                     var match= dueDate.match(regexStr2);
                     if (match && match[1]) {
                         dueDate = match[1];
                     }
                }
    ...
    function getUserName(){
        ...
        if (currentURL.includes('quickFilter=')) {
            ...
            if(quickFilterValue == 114){
                return "youfu.wu"
            }else if(quickFilterValue == 115){
                return "zaitian.cai"
            }else if(quickFilterValue == 116){
                return "chuanfang.lv"
            }else if(quickFilterValue == 117){
                return "chunlin.tang"
            }else if(quickFilterValue == 118){
                return "songsong.yan"
            }else if(quickFilterValue == 119){
                return "peiyi.tang"
            }else if(quickFilterValue == 126){
                return "phoenixylf"
            }else if(quickFilterValue == 135){
                return "linhui.lan"
            }else if(quickFilterValue == 136){
                return "wenrui.zhong"
            }else if(quickFilterValue == 185){
                return "bill"
            }else if(quickFilterValue == 193){
                return "wenyi.liu"
            }else if(quickFilterValue == 194){
                return "yuehui.xu"
            }else if(quickFilterValue == 195){
                return "boyi.wang"
            }else if(quickFilterValue == 214){
                return "huilong.wang"
            }else{
