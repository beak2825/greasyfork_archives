// ==UserScript==
// @name         民用无人驾驶航空器管理平台-安全操控理论培训-学习助手
// @namespace    http://tampermonkey.net/zzzzzzys_民用无人驾驶航空器管理平台-安全操控理论培训
// @version      1.0.0
// @copyright    zzzzzzys.All Rights Reserved.
// @description  民用无人驾驶航空器管理平台-安全操控理论培训，培训视频可秒刷时长，考试离开限制解除！可随意缩小切屏隐藏网页！
// @author       zzzzzzys
// @match        https://uom.caac.gov.cn/*
// @match        https://uom.caac.gov.cn/output/@nvwa/3rd-party-embed/index.html*
...
// @license      
// @downloadURL https://update.greasyfork.org/scripts/532190/%E6%B0%91%E7%94%A8%E6%97%A0%E4%BA%BA%E9%A9%BE%E9%A9%B6%E8%88%AA%E7%A9%BA%E5%99%A8%E7%AE%A1%E7%90%86%E5%B9%B3%E5%8F%B0-%E5%AE%89%E5%85%A8%E6%93%8D%E6%8E%A7%E7%90%86%E8%AE%BA%E5%9F%B9%E8%AE%AD-%E5%AD%A6%E4%B9%A0%E5%8A%A9%E6%89%8B.user.js
// @updateURL https://update.greasyfork.org/scripts/532190/%E6%B0%91%E7%94%A8%E6%97%A0%E4%BA%BA%E9%A9%BE%E9%A9%B6%E8%88%AA%E7%A9%BA%E5%99%A8%E7%AE%A1%E7%90%86%E5%B9%B3%E5%8F%B0-%E5%AE%89%E5%85%A8%E6%93%8D%E6%8E%A7%E7%90%86%E8%AE%BA%E5%9F%B9%E8%AE%AD-%E5%AD%A6%E4%B9%A0%E5%8A%A9%E6%89%8B.meta.js
// ==/UserScript==

unsafeWindow.onload = () => {
    unsafeWindow.addEventListener('onblur', function () {
        console.log(' 新的失焦事件处理');
    });
}
window.onload = () => {
...
class Runner {
    ...
    initAjaxHooker() {
        ...
        ajaxHooker.hook(request => {
            if (request.url.includes('trained/video/list')) {

                window.Authorization = request.headers['Authorization']
                console.log("Authorization:", window.Authorization)
            ...
            } else if (request.url.includes('uomExam/leaveCountCut')) {
                request.abort = true
                request.response = res => {
                    res.responseText = JSON.stringify({code: 0, msg: "成功", data: null})
                }
            } else if (request.url.includes('uomExam/getLeaveCount')) {
                const pid = request.data.pid
                console.log("pid:", pid)
                request.abort = true
                request.response = res => {
                    res.responseText = JSON.stringify({"msg": "操作成功", "code": 0, "count": 0, "pid": pid})
                }
            }
    ...
    run() {
        ...
        this.runner = new Course("channel-caac")
        if (url.includes("caac") && !url.includes("3rd-party-embed")) {
            this.runner = new Course("channel-caac")
        } else if (url.includes("3rd-party-embed")) {
            // this.runner = new Index("channel-csu")
            setInterval(() => {
...
class Course {
    ...
    async runVIP() {
        try {
            ...
            /**
             * 计算两个视频时间点之间的时间差（秒）
             * @param {string} startTime - 开始时间，格式为"mm:ss"或"hh:mm:ss"
             * @param {string} endTime - 结束时间，格式为"mm:ss"或"hh:mm:ss"
             * @returns {number} 时间差（秒），总是返回正数
             */
            function calculateVideoTimeDifference(startTime, endTime) {
                // 将时间字符串转换为秒数
                function timeToSeconds(timeStr) {
                    const parts = timeStr.split(':');

                    // 处理hh:mm:ss和mm:ss两种情况
                    if (parts.length === 3) {
                        // hh:mm:ss格式
            ...
            const update = async (id, end) => {
                const start = "00:00"
                const data = {
                    trainedVideoId: id,
                    videoStartLocation: start,
                    videoEndLocation: end,
                    durationSecond: calculateVideoTimeDifference(start, end),

                }
                let res = await fetch("https://uom.caac.gov.cn/api/uom-uavdriver/trained/video/connect", {
                    "headers": {
                    ...
                    "referrer": "https://uom.caac.gov.cn/",
                ...
                if (res.ok) {
                    ...
                    console.log(res)
                    if (res.code !== 0) {
                        throw Error("刷取失败：" + res.msg)
                    }
                }
            }
            if (window.list && window.list.rows) {
                for (const item of window.list.rows) {

                    await update(item.id, item.duration)
                    await sleep(1000)
                }
            } else {
                throw Error("未获取到视频信息列表")
            }
            Swal.fire({
                title: "刷课成功！",
                text: "刷课成功！",
    ...
    async run() {
        ...
        try {
            const catalogSelector = '.ivu-tree-children li'
            const catalogList = await this.getStudyNode(catalogSelector, 'nodeList')
            let index = 0
            for (let catalog of catalogList) {
                console.log(catalog.querySelector('.titles').innerText)
                if (this.checkFinish(catalog)) {
                    console.log("已学完，跳过！")
                    continue
                }
                catalog.querySelector('.tree-node').click()
                await sleep(1000)
    ...
    checkFinish(dom) {
        return dom.querySelector('.videoRatio').innerText.includes('100%')
    }
...
class Utils {
    ...
    static flag = 'caac_VIP'
    static js_Flag = 'caac_jsCode'
    static vipText = '高级功能已启用！(民用无人驾驶,视频秒刷，考试离开限制接触)'
    static baseText = '您正在使用基础版本，功能可能存在限制'
    static vipSign = 'caac_vipSign'
    ...
    static async validateCode(data) {
        try {
            const tokenValue = document.querySelectorAll('.fullname')
            // console.log('动态键名值:', tokenValue);
            let info = {
            ...
            data.bindInfo = info.jti + "_" + info.name
            data.website = "67ecf9af2eea65fc6c093e09"
            console.log(data);
            // return
