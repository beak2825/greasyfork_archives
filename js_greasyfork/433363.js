...
// @downloadURL https://update.greasyfork.org/scripts/433363/%E3%80%90%E7%94%B5%E8%84%91%20%20%E6%89%8B%E6%9C%BA%20%E5%85%A8%E7%BD%91%E5%94%AF%E4%B8%80%E5%85%A8%E8%87%AA%E5%8A%A8%E8%84%9A%E6%9C%AC%E3%80%91VIP%E4%BC%9A%E5%91%98%E8%A7%86%E9%A2%91%E8%87%AA%E5%8A%A8%E8%A7%A3%E6%9E%90.user.js
// @updateURL https://update.greasyfork.org/scripts/433363/%E3%80%90%E7%94%B5%E8%84%91%20%20%E6%89%8B%E6%9C%BA%20%E5%85%A8%E7%BD%91%E5%94%AF%E4%B8%80%E5%85%A8%E8%87%AA%E5%8A%A8%E8%84%9A%E6%9C%AC%E3%80%91VIP%E4%BC%9A%E5%91%98%E8%A7%86%E9%A2%91%E8%87%AA%E5%8A%A8%E8%A7%A3%E6%9E%90.meta.js
// ==/UserScript==
...
/* ************** 根据以下格式，自己添加接口**************** */
{name:"蓝猫",url:"https://jiexi.daidaitv.top:43810/api/?key=7WEdqCJSkvy7kPEggA&url=",vip:"强制弹大窗"},
...
(async function() {
	try {
		if (self != top) {
			return false;
		} else if (document.title.match(/^\s*?[45]0[0234]\s/i) != null) {
			return false;
		} else {
			if (location.href.match(/\.163\..+?\/store/)) {} else {
				try {
					...
					function jxbvipobj() {
						...
						var arra = [],
							arrb = [],
							jsonvipjk, arrahttpsjk, arrbhttpsjk, httpsjk, vipzdjx, vipjxtb, vipjxss, vipjxkjj, gbdmobj, vipvideotg, gbkjjjobj, jstgggobj, pdssgjcobj, mgzdgq, lkzdzt, zdbfqp, zdwbfobj, sjtzjx, jxbzdjxdsqobj, jishu, elevideo, jxbzdws, byjxbzdws, jxbzdwstitle, jxbzdwsjs;
						...
						setTimeout(function() {
							if (((localStorage.getItem('\u7535\u8111Json\u5168\u5c40\u64ad\u653e\u5668') != null) && (localStorage.getItem('\u7535\u8111Json\u5168\u5c40\u64ad\u653e\u5668').match(/^https:\/\//))) || ((localStorage.getItem('\u624b\u673aJson\u5168\u5c40\u64ad\u653e\u5668') != null) && (localStorage.getItem('\u624b\u673aJson\u5168\u5c40\u64ad\u653e\u5668').match(/^https:\/\//)))) {
								...
								if (document.querySelector('江小白电脑网页')) {
									jsonvipjk = 'https://api.ldjx.cc/wp-api/ifr.php?isDp=1&vid=';
								} else {
									jsonvipjk = 'https://player.laoyam3u8.com/aliplayer/?url=';
								}
							}
						}, 1400);
						const jsonvipbybfqa = "https://www.ikukk.com/?ac=0&url=", jsonvipthb = /{[\s\S]*?["']\s*?urls?\s*?["']\s*?:\s*?["'](?![\s\S]+?(?:\.html|(?:&llq|\/\?[^=#]+?[=#])["']|\.[a-z]{2,5}\?[a-z]{1,10}=))\s*?(http[\s\S]+?)["']/i, jsonvipthc = /{[\s\S]*?["']\s*?urls?\s*?["']\s*?:\s*?["'](?![\s\S]+?(?:&llq|\/\?[^=#]+?[=#]|\/[^\/]+?\.php\?[\s\S]+?)["'])\s*?(http(?![^"']+?\.html)[\s\S]+?\.[a-z]{2,5}[\s\S]+?)["']/i, jsonvipthd = /{[\s\S]*?["']\s*?urls?\s*?["']\s*?:\s*?["'](?![\s\S]+?(?:&llq|\/\?[^=#]+?[=#])["'])\s*?(http[\s\S]+?(?:\/[^\/]+?\.php\?|=http)[\s\S]+?)["']/i, jsonvipthe = /^\s*?http(?!.+?http).+?\.m(?:3u8|p4)/i, jsonvipthf = /(?:{[\s\S]*?["']\s*?(?:code|m)\s*?["']\s*?:\s*?["']?\s*?[45]0[0234]|{[\s\S]*?["']\s*?urls?\s*?["']\s*?:\s*?["'](?:\s*?|http(?!.+?http).+?shouquan[^"']*?\.m(?:3u8|p4)[^"']*?)["']|["']\s*?msg\s*?["']\s*?:\s*?["']?[^"']*?(?:没有|\\u6ca1\\u6709|下载|\\u4e0b\\u8f7d|失败|\\u5931\\u8d25|丢失|\\u4e22\\u5931|修复|\\u4fee\\u590d|盗|\\u76d7|调|\\u8c03|未|\\u672a|封|\\u5c01|禁|\\u7981|对不起|\\u5bf9\\u4e0d\\u8d77|客户端|\\u5ba2\\u6237\\u7aef))/img, jsonvipthg = /^\s*?<\s*?[^>]*?html\s*?>[\s\S]+?<script[\s\S]+?(?:\s+?urls?\s*?=|,\s*?urls?\s*?:|=\s*?{[\s\S]*?"urls?"\s*?:)\s*?["']\s*?(https?:\/\/(?![^"']+?\.php\?)[^"']+?\.m(?:3u8|p4)[^"']*?)["']/i, jsonvipthh = /^\s*?<\s*?[^>]*?html\s*?>[\s\S]+?<h\d+?>[\s\S]*?解[^析]*?析[^失]*?失[^败]*?败/i, jsonvipthi = /^\s*?<\s*?[^>]*?html\s*?>[\s\S]+?<script/i, jsonvipthj = /^\s*?http.+?(?:\/[^\/]+?\.php\?|=http)/i, jsonvipthk = /["']\s*?urls?\s*?["']\s*?:\s*?["']([^"']+?)["']/i, jsonvipthl = /{(?![\s\S]*?["']\s*?urls?\s*?["']\s*?:\s*?["']http)/i, jsonvipthn = /^\s*?<\s*?[^>]*?html\s*?>[\s\S]+?<script[\s\S]+?(?:\s+?urls?\s*?=|,\s*?urls?\s*?:|=\s*?{[\s\S]*?"urls?"\s*?:)\s*?["'](?:\s*?|https?:\/\/[^"']+?\/https?:\/\/[^"']+?\.m(?:3u8|p4)[^"']*?)["']/i, jsonvipthm = /^\s*?(?:11?1?|(?:原|(?:镶?嵌|内?置)入?|当前)网?页?播?放?)\s*?$/, jsonvipjkcsa = "&type=auto&autoplay=1&metareferer=undefined", jsonvipjkcsb = "&type=auto&autoplay=1&dpuser=%E6%88%91%E6%98%AF%E6%B1%9F%E5%B0%8F%E7%99%BD&href=https%3A%2F%2Fqm.qq.com%2Fcgi-bin%2Fqm%2Fqr%3Fk%3D1einiDT3MfcxrYknT5hS0vxO63ELOSAi&metareferer=undefined", jsonvipjkcsc = '<ul id="小白" style="overflow:auto;margin:0;padding:0px;border:0;font-size:large;"><h1 style="text-align:center;overflow:auto;margin:0;padding:0px;border:0;font-size:large;color:forestgreen;height:33px;">提取结果</h1><h2 style="text-align:center;"><h2 style="text-align:center;"><a href="', jsonvipjkcsd = '" style="color:lightsalmon;font-size:large;text-decoration:none;font-family:monospace!important;" title="点击 打开该资源" target="_blank">', jsonvipjkcse = '<ul style="text-align:center;"><a target="_blank" href="', jsonvipjkcsf = '" rel="noreferrer" title="点击 打开该资源" style="color:whitesmoke;">该→', jsonvipjkcsg = '←不支持当前视频</a></ul>', jsonvipjkcsh = '←无法访问</a></ul>', jsonvipjklxa = /^\s*?(?:https|aHR0cH)/, jxbjxqptck = "allowfullscreen=true,allowfullscreen=allowfullscreen,esizable=1,scrollbars=1,toolbar=0,status=0,width=1050,height=600,left=" + (screen.availWidth - 1050) / 2 + ",top=" + (screen.availHeight - 600) / 2, jxbjxqptdc = "allowfullscreen=true,allowfullscreen=allowfullscreen,esizable=1,scrollbars=1,toolbar=0,status=0,width=" + (screen.availWidth - screen.availWidth * 0.0066) + ",height=" + (screen.availHeight - 55);
						...
						if (jxjkobj || objj.match(/(?:m1907|isyour)\./) || obj.match(/&autoplay=1&metareferer=/)) {
							...
							} else {} if (objj.match(/(?:jx\.2bjx\.com|4321s\.cc|wfy\d+?\.|xjqxz\.top|dm\.2g\d+?\.)/)) {
							...
							if (objj.match(/(?:eggvod|eofelis)\./)) {
								...
								if ((document.querySelector("body>div") == null) && (document.querySelector("video") == null) && (document.querySelector("iframe") == null)) {
									setTimeout(function() {
										...
										try {
											if ((xmlaa.match(jsonvipthf) != null) || (xmlaa.match(jsonvipthl) != null && xmlaa.match(jsonvipthi) == null)) {
								...
								try {
									(function() {
										if (!document.querySelector("\u6c5f\u5c0f\u767d\u76d1\u63a7\u89c6\u9891\u64ad\u653e\u7ed3\u675f")) {
											if (objj.match(/(?:m1907\.|jx\.2bjx\.com|4321s\.cc|wfy\d+?\.|xjqxz\.top|dm\.2g\d+?\.)/)) {} else {
						...
						function jxbmmanzxobj() {
							...
							if (obj.match(/^https?:\/\/(?:www\.wbdy\.tv|www\.ikukk\.com)\//)) {
								cssobj('[class$=" hidden-xs"],[id$=" hidden-xs"],body>[class^="cont"]~:not(maomibtn):not([class^="cont"]):not([id^="cont"]),body>[id^="cont"]~:not(maomibtn):not([class^="cont"]):not([id^="cont"]),[classname]');
								...
								if (document.querySelector("\u6c5f\u5c0f\u767d\u81ea\u5b9a\u4e49\u63a5\u53e3") != null) {} else {
									...
									if (localStorage.getItem("jkqhmsobj") == '2') {
										...
										httpsaobj = [{
											...
											name: "樱花",
											url: "https%3A%2F%2Fbuaon.xyz%2F%3Furl%3D",
											ys: "2",
											jj: "1"
										}, {
											name: "瓜皮",
											...
											name: "庄勇",
											url: "https%3A%2F%2Fjiexi.jiexizhuanyong.com%2F%3Furl%3D",
											jj: "1"
										}, {
											name: "叻簇",
											...
											name: "初心",
											url: "https%3A%2F%2Fplayer.5znn.pro%2Fjiexi.php%3Furl%3D",
											jj: "1"
										}, {
											name: "段友",
											...
											if (jxbpcobj) {} else {
												...
												httpsvipli.onclick = function() {
													try {
														...
														function vipobj(vip) {
															if (https[num].json != null) {} else {
																jsonimgb = setTimeout(function() {
																	try {
																		if (document.querySelector('div#江小白>div#小白>img')) {
																			document.querySelector('div#江小白>div#小白>img').remove();
																		} else {}
																	} catch (err) {}
																}, 5555);
															}
															...
															try {
																...
																function vipjxdmaobj() {
																	setTimeout(function() {
																		...
																		try {
																			if (jxb.match(new RegExp("^https:\/\/(?!.+?强制(?:弹[小大]窗|跳转|搜索))")) && !(jxb.match(new RegExp("(?:透明无接口|布米米|视频选集|Youtube|麻花影视|旋风视频)")))) {
																				if (objj.match('acfun') || (pcliwaiobj && localStorage.getItem("jxywyfsgbobj") != null & !(sessionStorage.getItem("jxywygdfsgbobj") == '1'))) {
																					...
																					if (document.querySelector(vip)) {
																						function tcthcobj() {
																							...
																							if (https[num].json != null && localStorage.getItem("videobfjhobj") == '0') {
																								tcthcobj();
																								window.open(jxb.replace(/强制.*?$/, ""), "bkmk_popup");
																							} else {
																...
																if (https[num].json == null || https[num].json == undefined || https[num].json == '' || https[num].json == '0' || https[num].json.match(new RegExp("(?:跳转?|^\s*?(?<![非不])套壳浏?览?器?|^\s*?(?<!开)关\s*?$)")) || localStorage.getItem("objjbo") != '1') {
																	jxb = jjxxaa.replace(/^(.+?)undefined\s*?$/img, "$1").replace(/^[^:\/]*?(https?:\/\/)/igm, "$1").replace(/%20/gm, "").replace(/\s{1,}/gm, "");
																	vipjxdmaobj();
																} else {
																	sessionStorage.setItem('jsonyjdjcs', https[num].name);
																	jsonyjdjcs = sessionStorage.getItem('jsonyjdjcs');
																	if (https[num].json != null) {
																		jsonimga = setTimeout(function() {
																			try {
																				if ((document.querySelector('div#江小白>div#小白>img') && (jsonyjdjcs == https[num].name))) {
																					try {
																						document.querySelector("div#江小白>div#小白").innerHTML = jsonvipjkcse + atob(https[num].url) + jxbjxurl + jsonvipjkcsf + https[num].name + jsonvipjkcsg;
																					} catch (e) {
																						document.querySelector("div#江小白>div#小白").innerHTML = jsonvipjkcse + unescape(https[num].url) + jxbjxurl + jsonvipjkcsf + https[num].name + jsonvipjkcsg;
																					}
																				} else {}
																			} catch (err) {}
																		}, 12345);
																										...
																										try {
																											if (sessionStorage.getItem("解析失败") != null) {
																												...
																												if (sessionStorage.getItem("失败接口") != null) {
																													if (jsonyjdjcs == https[num].name) {
																														if ((document.querySelector('div#江小白>div#小白>img') && (jsonyjdjcs == https[num].name))) {
																															try {
																																document.querySelector("div#江小白>div#小白").innerHTML = jsonvipjkcse + atob(https[num].url) + jxbjxurl + jsonvipjkcsf + sessionStorage.getItem("失败接口") + jsonvipjkcsh;
																															} catch (e) {
																																document.querySelector("div#江小白>div#小白").innerHTML = jsonvipjkcse + unescape(https[num].url) + jxbjxurl + jsonvipjkcsf + sessionStorage.getItem("失败接口") + jsonvipjkcsh;
																															}
																														} else {}
																													...
																													if (jsonyjdjcs == https[num].name) {
																														if ((document.querySelector('div#江小白>div#小白>img') && (jsonyjdjcs == https[num].name))) {
																															try {
																																document.querySelector("div#江小白>div#小白").innerHTML = jsonvipjkcse + atob(https[num].url) + jxbjxurl + jsonvipjkcsf + https[num].name + jsonvipjkcsh;
																															} catch (e) {
																																document.querySelector("div#江小白>div#小白").innerHTML = jsonvipjkcse + unescape(https[num].url) + jxbjxurl + jsonvipjkcsf + https[num].name + jsonvipjkcsh;
																															}
																														} else {}
																													} else {}
																												}
																		...
																		GM_xmlhttpRequest({
																			...
																			onload: function(bxmla) {
																						...
																						if (bxmla.responseText.match(/["']\s*?urls?\s*?["']\s*?:\s*?["']/i)) {
																							try {
																								if (bxmla.responseText.match(jsonvipthk)[1].replace(/\\\//g, "/").match(jsonvipthj)) {
																									sessionStorage.setItem('解析失败', jsonvipjkcsc + jsonvipbybfqa + encodeURIComponent(bxmla.responseText.match(jsonvipthk)[1].replace(/\\\//g, "/")) + jsonvipjkcsd + bxmla.responseText.match(jsonvipthk)[1].replace(/\\\//g, "/") + '</a></h2></ul>');
																								} else {
																									sessionStorage.setItem('解析失败', jsonvipjkcsc + bxmla.responseText.match(jsonvipthk)[1].replace(/\\\//g, "/") + jsonvipjkcsd + bxmla.responseText.match(jsonvipthk)[1].replace(/\\\//g, "/") + '</a></h2></ul>');
																								}
																							} catch (e) {}
																							jxqzsxobj();
																						}
																				...
																				if (bxmla.status === 200) {
																					...
																					if ( !! bxmlc) {
																						if ((bxmlc.match(jsonvipthf) != null) || (bxmlc.match(jsonvipthl) != null && bxmlc.match(jsonvipthi) == null) || (bxmlc.match(jsonvipthn) != null)) {
																							if (jsonyjdjcs == https[num].name) {
																								byjxpd.byjxpaobj();
																							} else {}
																						} else if (bxmlc.match(jsonvipthh) != null) {
																							if (jsonyjdjcs == https[num].name) {
																								byjxpd.byjxpaobj();
																							} else {}
																						} else {
																							...
																							setTimeout(function() {
																								if (bxmlc.match(jsonvipthg) == null && bxmlc.match(jsonvipthi) != null) {
																									if (https[num].url.match(jsonvipjklxa)) {
																										...
																										if (jsonyjdjcs == https[num].name) {
																											try {
																												document.querySelector(vip).innerHTML = jxbvipjxyyy + atob(https[num].url) + jxbjxurl + '" ' + jxbvipjximg + '></iframe></div>';
																											} catch (e) {
																												document.querySelector(vip).innerHTML = jxbvipjxyyy + unescape(https[num].url) + jxbjxurl + '" ' + jxbvipjximg + '></iframe></div>';
																											}
																											return;
																										} else {}
																									} else {
																										if (jsonyjdjcs == https[num].name) {
																											if (document.querySelector("img#江小白") != null) {} else {
																												if (document.querySelector(vip)) {
																													document.querySelector(vip).innerHTML = '<div id="江小白" ' + jxbvipjxstyle + '><img id="江小白" src="' + jxbimg + '" ' + jxbvipjximg + '></img></div>'
																												} else {}
																												jxqzsxobj();
																											}
																											try {
																									...
																									if (unescape(bxmlc.match(jsonvipthg)[1].replace(/\\\//g, "/")).match(jsonvipthe)) {
																										if (jsonyjdjcs == https[num].name) {
																											sessionStorage.setItem('解析失败', jsonvipjkcsc + bxmlc.match(jsonvipthg)[1].replace(/\\\//g, "/") + jsonvipjkcsd + bxmlc.match(jsonvipthg)[1].replace(/\\\//g, "/") + '</a></h2></ul>');
																											if (https[num].json == '小白') {
																												byjxpd.byjxpbobj();
																												if (pcliwaiobj) {
																													GM_openInTab(bxmlc.match(jsonvipthg)[1].replace(/\\\//g, "/"), false);
																												} else {
																													GM_openInTab(jsonvipjk + encodeURIComponent(bxmlc.match(jsonvipthg)[1].replace(/\\\//g, "/")), false);
																												}
																											} else {
																												if ((https[num].json.match(jsonvipthm)) && (https[num].bfq == null || https[num].bfq == undefined || https[num].bfq == '')) {
																													document.querySelector(vip).innerHTML = jxbvipjxyyy + jsonvipjk + encodeURIComponent(bxmlc.match(jsonvipthg)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")) + '" ' + jxbvipjximg + '></iframe></div>';
																													jxqzsxobj();
																												} else if ((https[num].json.match(jsonvipthm)) && ((https[num].bfq != null || https[num].bfq != undefined) && https[num].bfq.match(jsonvipjklxa))) {
																													document.querySelector(vip).innerHTML = jxbvipjxyyy + https[num].bfq + encodeURIComponent(bxmlc.match(jsonvipthg)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")) + '" ' + jxbvipjximg + '></iframe></div>';
																													jxqzsxobj();
																												} else {
																													byjxpd.byjxpbobj();
																													if (https[num].bfq == null || https[num].bfq == undefined || https[num].bfq == '') {
																														GM_openInTab(jsonvipjk + encodeURIComponent(bxmlc.match(jsonvipthg)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")), false);
																													} else {
																														GM_openInTab(https[num].bfq + encodeURIComponent(bxmlc.match(jsonvipthg)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")), false);
																													}
																												}
																											}
																											return;
																									...
																									if (bxmlc.match(jsonvipthd)[1].replace(/\\\//g, "/").match(jxjkwzobj)) {
																										if (jsonyjdjcs == https[num].name) {
																											if (bxmlc.match(jsonvipthd)[1].replace(/\\\//g, "/").match(/^https:\/\//)) {
																												document.querySelector(vip).innerHTML = jxbvipjxyyy + bxmlc.match(jsonvipthd)[1].replace(/\\\//g, "/") + '" ' + jxbvipjximg + '></iframe></div>';
																												return;
																											} else {
																												byjxpd.byjxpbobj();
																												GM_openInTab(bxmlc.match(jsonvipthd)[1].replace(/\\\//g, "/"), false);
																												return;
																											}
																										} else {}
																									} else {
																										if (jsonyjdjcs == https[num].name) {
																											if (https[num].json == '小白') {
																												byjxpd.byjxpbobj();
																												GM_openInTab(jsonvipbybfqa + encodeURIComponent(bxmlc.match(jsonvipthd)[1].replace(/\\\//g, "/")), false);
																											} else {
																												if ((https[num].json.match(jsonvipthm)) && (https[num].bfq == null || https[num].bfq == undefined || https[num].bfq == '')) {
																													...
																													byjxpd.byjxpbobj();
																													if (https[num].bfq == null || https[num].bfq == undefined || https[num].bfq == '') {
																														GM_openInTab(jsonvipjk + encodeURIComponent(bxmlc.match(jsonvipthd)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")), false);
																													} else {
																														GM_openInTab(https[num].bfq + encodeURIComponent(bxmlc.match(jsonvipthd)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")), false);
																													}
																												}
																											}
																											return;
																									...
																									if (jsonyjdjcs == https[num].name) {
																										if (https[num].json == '小白') {
																											byjxpd.byjxpbobj();
																											if (pcliwaiobj) {
																												GM_openInTab(bxmlc.match(jsonvipthb)[1].replace(/\\\//g, "/"), false);
																											} else {
																												GM_openInTab(jsonvipjk + encodeURIComponent(bxmlc.match(jsonvipthb)[1].replace(/\\\//g, "/")), false);
																											}
																										} else {
																											if ((https[num].json.match(jsonvipthm)) && (https[num].bfq == null || https[num].bfq == undefined || https[num].bfq == '')) {
																												document.querySelector(vip).innerHTML = jxbvipjxyyy + jsonvipjk + encodeURIComponent(bxmlc.match(jsonvipthb)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")) + '" ' + jxbvipjximg + '></iframe></div>';
																												jxqzsxobj();
																											} else if ((https[num].json.match(jsonvipthm)) && ((https[num].bfq != null || https[num].bfq != undefined) && https[num].bfq.match(jsonvipjklxa))) {
																												document.querySelector(vip).innerHTML = jxbvipjxyyy + https[num].bfq + encodeURIComponent(bxmlc.match(jsonvipthb)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")) + '" ' + jxbvipjximg + '></iframe></div>';
																												jxqzsxobj();
																											} else {
																												byjxpd.byjxpbobj();
																												if (https[num].bfq == null || https[num].bfq == undefined || https[num].bfq == '') {
																													GM_openInTab(jsonvipjk + encodeURIComponent(bxmlc.match(jsonvipthb)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")), false);
																												} else {
																													GM_openInTab(https[num].bfq + encodeURIComponent(bxmlc.match(jsonvipthb)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")), false);
																												}
																											}
																										}
																										return;
																									...
																									if (jsonyjdjcs == https[num].name) {
																										if (https[num].json == '小白') {
																											...
																											if (pcliwaiobj) {
																												...
																												GM_openInTab(jsonvipjk + encodeURIComponent(bxmlc.match(jsonvipthc)[1].replace(/\\\//g, "/")), false);
																											}
																										} else {
																											if ((https[num].json.match(jsonvipthm)) && (https[num].bfq == null || https[num].bfq == undefined || https[num].bfq == '')) {
																												document.querySelector(vip).innerHTML = jxbvipjxyyy + jsonvipjk + encodeURIComponent(bxmlc.match(jsonvipthc)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")) + '" ' + jxbvipjximg + '></iframe></div>';
																												jxqzsxobj();
																											} else if ((https[num].json.match(jsonvipthm)) && ((https[num].bfq != null || https[num].bfq != undefined) && https[num].bfq.match(jsonvipjklxa))) {
																												document.querySelector(vip).innerHTML = jxbvipjxyyy + https[num].bfq + encodeURIComponent(bxmlc.match(jsonvipthc)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")) + '" ' + jxbvipjximg + '></iframe></div>';
																												jxqzsxobj();
																											} else {
																												byjxpd.byjxpbobj();
																												if (https[num].bfq == null || https[num].bfq == undefined || https[num].bfq == '') {
																													GM_openInTab(jsonvipjk + encodeURIComponent(bxmlc.match(jsonvipthc)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")), false);
																												} else {
																													GM_openInTab(https[num].bfq + encodeURIComponent(bxmlc.match(jsonvipthc)[1].replace(/\\\//g, "/").replace(/undefined\s*?$/, "")), false);
																												}
																											}
																										}
																										return;
									...
									if (jxbpcobj) {
										...
										(function() {
											try {
												...
												if (localStorage.getItem("jkqhmsobj") == '2') {
													...
													sjqjpbzdylwaobj = /(?:M19|邦宁|网吧|B站)/i, sjyyyzdypbobj = /(?:PAR|虾米|地衣|吾吾|爱跟|老版|达人|[淘白][a-z0-9]|扫描|月亮|看剧|神马|酷看|久播|思[云念]|速云)/i, sjqjpbzdylwbobj = /(?:民俗|玻璃|四零|小蒋|学习|猪蹄|七彩|扫描|神马|白玉|依托|法克|垫片|依齐|乐多|思古|[三八][一二]|17云|1717|8090|哔哩|九八|CKP|OK|Youtube|和巴|[方文学提教]案|布米米|视频选集|透明.*?接口)/i;
													for (let jxbqcsjjki = 0; jxbqcsjjki < jxbqcsjjk.length; jxbqcsjjki++) {
										...
										(function() {
											...
											function createSelectew(jxbhttpsew) {
												...
												document.body.insertAdjacentHTML('beforeend', '<style type="text/css">div[id^="jjxxkk"]{margin:0px;padding:0px;position:fixed;width:max-content;bottom:75px;left:50%;transform:translateX(-50%);z-index:102030;overflow:auto hidden;text-align:center;}div[id^="jjxxkk"]>span,div[id^="jjxxkk"]>a{font-weight:bold;float:left;border-radius:25%;padding:10px 5px 10px 5px;font-size:large;border:4px outset buttonface;color:midnightblue;text-decoration:none;background-color:violet;}</style><div id="jjxxkka" style="display:none;"><span id="sjzdws">备用解析</span></div>');
								...
								if (document.querySelector("\u6c5f\u5c0f\u767d\u7535\u8111\u7f51\u9875") != null) {
									...
									for (let kongjiekoui = 0; kongjiekoui < kongjiekou.length; kongjiekoui++) {
										if (kongjiekou[kongjiekoui].innerText == '') {
											kongjiekou[kongjiekoui].remove();
											console.log("%c[江小白 kongjiekou-01 已经删除] ✂", "border-left:5px solid #A0B;color:#A0B;padding:3px", kongjiekou[kongjiekoui]);
											break;
										}
									};
									try {
										...
										function zdpddyobj() {
											...
											function jiangxiaobaiviphyzdpd() {
												if (obj.match(/&autoplay=1&metareferer=/)) {
													...
													if (vipzdjx == 1) {
														document.querySelector("jiangxiaobai").remove();
														if (document.querySelector("#江小白")) {} else {
															(function() {
																setTimeout(function() {
																	try {
													...
													if (pcliwaiobj || ttbkjjliobj) {
														if (pcliwaiobj) {
															(function() {
																...
																try {
																	if (document.querySelector("#江小白:not(div):not(img):not(ul)")) {
																		try {
																						...
																						(function() {
																							try {
																								...
																								let sdjxyywzdqp_jiankong = setInterval(function() {
																									let sdjxyywzdqp_btn = document.querySelector("#江小白:not(div):not(img):not(ul)");
																									if (sdjxyywzdqp_btn) {
																										try {
																											qpobj(document.querySelector('#江小白:not(div):not(img):not(ul)'))
																										} catch (err) {}
																										clearInterval(sdjxyywzdqp_jiankong);
																										return false
																									}++sdjxyywzdqp_counter;
