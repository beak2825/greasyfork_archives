...
// @downloadURL https://update.greasyfork.org/scripts/431639/mydownload.user.js
// @updateURL https://update.greasyfork.org/scripts/431639/mydownload.meta.js
// ==/UserScript==
  ...
  function getContent(pageId) {
    ...
    $.get(pathname + chapId)
      .done(function (response) {
        ...
        if (endDownload) return;
        let test = $data.find('.heading-font')[0];
        if (test){
        chapTitle = 'Chương ' + chapId.match(/\d+/)[0] + ': '+ $data.find('.heading-font')[0].outerText.trim();
        }else{
            chapTitle === '';
        }
