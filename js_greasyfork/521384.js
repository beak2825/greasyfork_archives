...
// @version      0.0.1
// @description  今日头条文章舆情抓取
// @author       daben
// @match        https://www.toutiao.com/article/*
// @icon         https://www.google.com/s2/favicons?sz=64&domain=toutiao.com
// @license      MIT
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/521384/%E4%BB%8A%E6%97%A5%E5%A4%B4%E6%9D%A1%E6%96%87%E7%AB%A0%E8%88%86%E6%83%85.user.js
// @updateURL https://update.greasyfork.org/scripts/521384/%E4%BB%8A%E6%97%A5%E5%A4%B4%E6%9D%A1%E6%96%87%E7%AB%A0%E8%88%86%E6%83%85.meta.js
// ==/UserScript==
...
(function() {
  ...
  window.addEventListener('load', function() {
    ...
    var authorElement = document.querySelector('.article-meta .name');
    var author = authorElement? authorElement.textContent.trim() : '未获取到作者';
    // 获取发布时间，假设在class为"publish-time"的元素内，按需修改选择器
    var timeElement = document.querySelector('.article-meta span');
    var time = timeElement? timeElement.textContent.trim() : '未获取到发布时间';
    // 获取文章内容所在元素，假设文章内容在id为"article-content"的元素内，按实际调整
    var contentElement = document.querySelector('.tt-article-content');
    var wordCount = contentElement? contentElement.textContent.trim().length : 0;
    // // 阅读数
    // var readnum = getArticleNum('.read-num em', '未获取到阅读数')
    // 点赞数
    var likeNum = getArticleNum('.detail-like span', '未获取到点赞数')
    // 评论数
    var commentNum = getArticleNum('.detail-interaction-comment span', '未获取到评论数')
    // 收藏数
    var collectNum = getArticleNum('.detail-interaction-collect span', '未获取到收藏数')
    // // 分享数
