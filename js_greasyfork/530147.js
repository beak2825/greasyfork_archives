...
// @downloadURL https://update.greasyfork.org/scripts/530147/ShopeeLAB%3A%20COMBO%20cnb%20%20hosted.user.js
// @updateURL https://update.greasyfork.org/scripts/530147/ShopeeLAB%3A%20COMBO%20cnb%20%20hosted.meta.js
// ==/UserScript==
...
const msEarlier         = -20;      // milliseconds earlier adjustment before eventTime
const lastRefresh       = 20; // in seconds
var targetPrice         = '19.80';   // string formatted || Actual PROMO PRICE on EVENT TIME
var targetPrice2        = '10.99';   // string formatted || Actual PROMO PRICE on EVENT TIME || press [ALT] + [/]
var targetPrice3        = '6.56';   // string formatted || Actual PROMO PRICE on EVENT TIME || press [ALT] + [.]
var targetPrice4        = '42.00';   // string formatted || Actual PROMO PRICE on EVENT TIME || press [ALT] + [,]
...
unsafeWindow.fetch = async (...args) => {
    ...
    if(resource.includes('pdp/get_pc?shop_id=')) {
    ...
    .then((data) => {
        ...
        if(response.url == 'https://shopee.com.my/api/v4/checkout/place_order') {
        ...
        if(response.url == 'https://shopee.com.my/api/v4/cart/checkout') {
            data.error = 0
        ...
        if(response.url.startsWith('https://shopee.com.my/api/v4/recommend/product_detail_page')) {
...
// RESPONSE INTERCEPTOR : start
   response.json = json;
    ...
    if(response.url.startsWith('https://shopee.com.my/api/v4/account/address/get_user_address_list?')) {
    ...
    if(response.url == 'https://shopee.com.my/api/v4/cart/add_to_cart') {
    ...
    if(response.url.startsWith('https://shopee.com.my/api/v4/cart/mini')) {
...
hotkeys('alt+1,alt+2,alt+3,alt+4,alt+5,alt+7,alt+8,alt+9,alt+0,alt+c,alt+v,alt+b,alt+[,alt+],alt+\\,alt+\',alt+/,alt+.,alt+\,,alt+f5,alt+f6', function (event, handler){
    switch (handler.key) {
