...
// @grant          unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/394445/bilibili%20Video%20Skip.user.js
// @updateURL https://update.greasyfork.org/scripts/394445/bilibili%20Video%20Skip.meta.js
// ==/UserScript==
...
(function() {
    ...
    var App = function App() {
        ...
        var _useGmValue = useGmValue("triggerKey", "j"), triggerKey = _useGmValue.data, updateTriggerKey = _useGmValue.updateData;
        var _useGmValue2 = useGmValue("OPTime", 90), OPTime = _useGmValue2.data, updateOPTime = _useGmValue2.updateData;
        useGmMenu("設置觸發按鈕", (function() {
            var setkey = prompt("請輸入觸發鍵", triggerKey);
            if (setkey) {
                if (setkey.length > 1) {
                    alert("只能輸入一個字");
                    return;
                ...
                alert("已設定觸發鍵為 ".concat(setkey));
            }
        }));
        useGmMenu("設置跳過時間", (function() {
            ...
            if (OPTimeString) {
                ...
                var parseOPTime = parseInt(OPTimeString, 10);
                updateOPTime(parseOPTime);
                alert("已設定Buffer時間為 ".concat(parseOPTime));
            }
        }));
        reactExports.useEffect((function() {
            function playerSeek(event) {
                if (dashPlayer && event.key.toLocaleLowerCase() === triggerKey && dashPlayer) {
                    var currentTime = dashPlayer.getCurrentTime();
