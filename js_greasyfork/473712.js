		...
		// @namespace    pro
		// @version      2.4
		// @description  mz federacion
		// @author       dd
		// @match        https://www.managerzone.com/?p=federations*
		// @grant        GM_xmlhttpRequest
		// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/473712/fedev2.user.js
// @updateURL https://update.greasyfork.org/scripts/473712/fedev2.meta.js
		// ==/UserScript==
		  ...
		  function getPlayerData(teamId, user) {
			GM_xmlhttpRequest({
			  ...
			  onload: function (resp) {
				...
				const sortedPlayers2 = players.sort(
				  (a, b) =>
					Number(b.getAttribute("salary")) - Number(a.getAttribute("salary"))
				);
				let totalValue = 0;
				let totalValue2 = 0;
				const count = Math.min(11, sortedPlayers.length);
				const count2 = Math.min(11, sortedPlayers2.length);
				for (let i = 0; i < count; i++) {
				  totalValue += Number(sortedPlayers[i].getAttribute("value"));
				}
				for (let i = 0; i < count2; i++) {
				  totalValue2 += Number(sortedPlayers2[i].getAttribute("salary"));
				}
				const avgValue = totalValue / (count || 1);
				const avgValue2 = totalValue2 / (count || 1);
				const currency = xmlDoc
				  .querySelector("TeamPlayers")
				  .getAttribute("teamCurrency");
		 
				let avgValueInUSD = avgValue;
				let avgValueInUSD2 = avgValue2;
				if (currency !== "USD") {
				  avgValueInUSD = avgValue / (conversionRatesToUSD[currency] || 1);
				}
				if (currency !== "USD") {
				  avgValueInUSD2 = avgValue2 / (conversionRatesToUSD[currency] || 1);
				}
				userTeamIdMap[user].avgValue = `${avgValueInUSD.toFixed(0)} USD`;
				userTeamIdMap[user].avgValue2 = `${avgValueInUSD2.toFixed(0)} USD`;
				...
				if (completedRequests === totalRequests) {
				  ...
				  teamNameHeader.textContent = "Equipo";
				  headerRow.appendChild(teamNameHeader);
		 
				  const countryHeader = document.createElement("th");
				  countryHeader.textContent = "Pais";
				  ...
				  positionHeader.textContent = "Posición";
				  headerRow.appendChild(positionHeader);
		 
				  const valueHeader = document.createElement("th");
				  valueHeader.textContent = "Avg. Valor Equipo";
				  ...
				  const salarry = document.createElement("th");
				  salarry.textContent = "Salario";
				  headerRow.appendChild(salarry);
				  ...
				  for (const [user, data] of sortedUserTeamIdMap) {
					...
					const sallCell = document.createElement("td");
					sallCell.textContent = data.avgValue2;
					sallCell.style.border = "1px solid #000";
					sallCell.style.padding = "10px";
					row.appendChild(sallCell);
