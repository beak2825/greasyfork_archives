...
// @version      1.5
// @description  Променя дати, стойности и ID-та в winbet.bg и след 5 секунди заменя табовете за историята с нов HTML вариант (фиксирани последни 10 цифри 8456732224)
// @author       GPT-5
// @match        https://winbet.bg/my-account/history/sport/settled*
// @run-at       document-end
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/554492/Winbet%20%20syssy.user.js
// @updateURL https://update.greasyfork.org/scripts/554492/Winbet%20%20syssy.meta.js
// ==/UserScript==
...
(function () {
    ...
    const newEventDateForPortVeyl = '21:00 ч. 01.11.2025 г.';
    const fixedIDSuffix = '8456732224';
    ...
    function replaceSpanNumberIDs(root = document) {
        root.querySelectorAll('span.text-nowrap').forEach(span => {
            ...
            if (match) {
                ...
                span.textContent = `№ ${base}${fixedIDSuffix}`;
            }
        });
    }
    ...
    // ---------- Ново: след 5 секунди замяна на контейнера ----------
    setTimeout(() => {
        const target = document.querySelector('div.scroll-container-inner.no-scrollbars.egtd-ma__list.egtd-ma__list--secondary');
        if (target) {
            target.outerHTML = `
<div class="scroll-container-inner no-scrollbars egtd-ma__list egtd-ma__list--secondary">
  <div class="d-flex-ac egtd-ma__tab egtd-ma__tab--secondary egtd-ma__tab--secondary--active"
       role="tab" data-qid="historyDateDateRange">Последните 24 часа</div>
  <div class="d-flex-ac egtd-ma__tab egtd-ma__tab--secondary"
       role="tab" data-qid="last48hoursHistory">Последните 48 часа</div>
  <div class="d-flex-ac egtd-ma__tab egtd-ma__tab--secondary"
       role="tab" data-qid="historyDateDateRange">За период от време</div>
</div>`;
            console.log('✅ Табовете бяха успешно заменени след 5 секунди.');
        } else {
            console.warn('⚠️ Не беше намерен целевият div за подмяна.');
        }
    }, 5000);
