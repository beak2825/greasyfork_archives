// ==UserScript==
// @name        Warbase Helper
// @namespace   warbaseHelper
// @description War Base display, tagging, and filters
...
// @grant       GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/26699/Warbase%20Helper.user.js
// @updateURL https://update.greasyfork.org/scripts/26699/Warbase%20Helper.meta.js
// ==/UserScript==
/* jshint -W097 */
...
/**
 * Gets a particular cookie from the document cookie. Not all browsers have this existing function
 * available at script time (*cough* Firefox).
 * @param {string} cookieName the name of the cookie to search for
 * @returns a string with the value of the stored cookie, null if it doesn't exist
 **/
function GetCookie(cookieName) {
    var cookieStart = cookieName + "=";
    ...
    for (var i = 0; i < cookieArray.length; i++) {
        ...
        if (thisCookie.indexOf(cookieStart) === 0) {
            return thisCookie.substring(cookieStart.length, thisCookie.length);
        }
    }
    return null;
}
...
function enableWarBaseLayout() {
    ...
    var $enemyAttackLinks = $('ul.member-list > li').find('div.act-cont > a');
    var $enemyStatusElements = $('div.status');
    for (var i = 0; i < $enemyList.length ; ++i) {
        // Remove the attack button from the right-most column. Put it back in place of the "Okay" status if it exists.
        var $rightColumnDiv = $enemyAttackLinks.eq(i).parent();  // Capture the right-most column for later.
        $enemyAttackLinks.eq(i).remove();
        if ($enemyStatusElements.eq(i).text().trim() == "Okay") {
            $enemyStatusElements.eq(i).empty();
            $enemyStatusElements.eq(i).append($enemyAttackLinks.eq(i));
        }
    }
    ...
    GM_addStyle(
        ...
        '.f-war-list #icon6, .f-war-list #icon8, .f-war-list #icon23, .f-war-list #icon10, ' +
            '.f-war-list #icon26, .f-war-list #icon9, .f-war-list #icon35, .f-war-list #icon4, ' +
            '.f-war-list #icon7 { display: none !important }'
    );
}
...
function applyFilter() {
    ...
    $factionList.each(function(index, node) {
        ...
        var $factionEnemyDifficultySelects = $(node).find('.vinkuun-enemeyDifficulty');
...
function FilterIndividualEnemy(playerLevel, $enemyElement, $enemyStatusIcon, enemyStatusText, $enemyDifficultySelect, $enemyUserLink,
    ...
    if (warBaseFilter.status.showOnline) {
...
function addFilterPanel($panel) {
    ...
    var $hospitalTextfield = $('<input>', {type: 'number', style: 'width: 35px'}).on('change', function() {
        if (isNaN(this.value)) {
    ...
    $panel.append($hospitalElement).append(', ');

    // status: level filter
    var $levelTextfield = $('<input>', {type: 'number', step: '5', style: 'width: 35px'}).on('change', function() {
        if (isNaN(this.value)) {
...
*/
    $panel.append($('<br>')).append('Show enemies: ');
    ...
    var $levelHolderElement = $('<label>', {text: 'level holders'}).prepend($levelHolderCheckbox);
    $panel.append($levelHolderElement).append(', ');

    // status: show online people
    var $onlineCheckbox = $('<input>', {type: 'checkbox'}).on('change', function() {
        reapplyFilter({status: {showOnline: this.checked}});
    });
    var $onlineElement = $('<label>', {text: 'online'}).prepend($onlineCheckbox);
    $panel.append($onlineElement);
...
var TAGS = {
    ...
    possible: {  // This tag name is linked to the default setting for an enemy whose tag no longer exists.
    ...
    fluffy: {
        text: 'Weak High-Level to Spank!',
        color: 'rgba(233, 197, 233, 1)'
    },


    impossible8: {
        text: 'Impossible, maybe May 2017?',
        color: 'rgba(139, 026, 026, 1)'
    },
    impossible9: {
        text: 'Impossible, maybe Sep 2017?',
        color: 'rgba(139, 026, 026, 1)'
    }
};
...
function LoadEnemyTags() {
    enemyTags = JSON.parse(localStorage.vinkuunEnemyTags || '{}');
}
LoadEnemyTags();
...
function createDropdown($li, id) {
    var $dropdown = $('<select>', {'class': 'vinkuun-enemeyDifficulty'}).on('change', function() {
        ...
        localStorage.getItem('vinkuunEnemyTags');  // Maybe needed to force persistence? http://stackoverflow.com/questions/13292744/why-isnt-localstorage-persisting-in-chrome
...
/**
 * Highlights gold any factions in the war base that are linked in the faction announcement.
...
function HighlightBonusFactions() {
    try {
        ...
        $('.f-war-list > li > .status-desc > .f-right > .cont > .bold > a').each(function(index, node) {
        ...
        $bonusFactions.each(function(index, node) {
            ...
            if ($factionId) {
                ...
                bonusFactionHeader.css("background-color", "gold")
...
/**
 * Highlights grey any factions in the war base that are already defeated.
 **/
    ...
    try {
        ...
        $('.f-war-list > li > .status-desc').each(function(index, node) {
            if ($(node).find('div[title="100%"]').length > 0) {
                $(node).css("background-color", "#909090");
