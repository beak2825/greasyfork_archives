// ==UserScript==
// @name         bilibili-GetVideoCover
// @namespace    https://greasyfork.org/users/135090
// @version      3.00
// @description  脚本将封面按钮放在了网页底部
// @author       ZWB
// @license      CC
...
// @downloadURL https://update.greasyfork.org/scripts/438376/bilibili-GetVideoCover.user.js
// @updateURL https://update.greasyfork.org/scripts/438376/bilibili-GetVideoCover.meta.js
// ==/UserScript==
(function(){
    if (unsafeWindow.location.href.includes("www.bilibili.com")){
        (function loop() {
            var link = unsafeWindow.document.createElement("span");
            var data = unsafeWindow.__INITIAL_STATE__.videoData;
            var avid = unsafeWindow.__INITIAL_STATE__.videoData.aid;
            if(avid){
                var cover = data.pic.substring(data.pic.indexOf(":")+1);
                var container = document.body;
                if (!container) return setTimeout(loop, 1500);
                link.style.display = "inline-block";
                link.style.color = "#23ade5";
                var coverhtml= "查看封面".link("javascript:void(0)");
                link.innerHTML = coverhtml;
                link.querySelector("a").addEventListener("click",function(){
                    void(window.open(cover));
                });
                link.style="position:fixed;bottom:0;left:0";
                console.log("封面地址:".concat(cover));
                container.append(link);
            }
        })();
    }
    /*
    if (unsafeWindow.location.href.includes("live.bilibili.com")){
        (function loop() {
            var link = unsafeWindow.document.createElement("div");
            var data = unsafeWindow.__NEPTUNE_IS_MY_WAIFU__.roomInfoRes.data.room_info;
            var roomid = data.room_id;
            if (roomid){
                var user_cover=data.cover;
                var container = unsafeWindow.document.querySelector("#sidebar-vm");
                if (!container) return setTimeout(loop, 500);
                link.style.display = "inline-block";
                link.style.marginLeft = link.style.marginRight = "1em";
                var coverhtml= "<hr /><a href=\""+user_cover+"\" target=_blank >查看封面</a><hr />";
                link.innerHTML = coverhtml;
                link.style="position:fixed;bottom:0;left:0";
                container.insertBefore(link, container.firstChild);
                console.log("封面地址:".concat(user_cover));
            }
        })();
    }
    */
