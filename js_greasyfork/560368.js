...
(function () {
	...
	let hasInsertedProfileSelector = false;
	...
	class Profile {
		constructor(id, name, clientId, saveLocation, category, tags, skipChecking, stopped, rootFolder) {
			...
			this.rootFolder = rootFolder !== undefined ? rootFolder : 'Original';
		}
	...
	const profileManager = {
		...
		async setCurrentProfile(profileId) {
			...
			this.currentProfile = new Profile(
				...
				data.skipChecking,
				data.stopped,
				data.rootFolder !== undefined ? data.rootFolder : 'Original'
			);
		...
		async getProfile(id) {
			...
			return new Profile(
				...
				parsed.skipChecking,
				parsed.stopped,
				parsed.rootFolder !== undefined ? parsed.rootFolder : 'Original'
			);
		}
	};
		...
		let implementations = {
			qbit: async () => {
				...
				if (rootFolder !== undefined) {
					tor_data.append('contentLayout', rootFolder);
				}
...
`);
	...
	function openProfileModal(profileId = null) {
		...
		<!-- REPLACE THE rootFolder CHECKBOX WITH THIS DROPDOWN: -->
		<div class="stc-form-group" id="rootFolder-group">
			<label class="stc-form-label">Content Layout:</label>
			<select class="stc-form-select" id="profile-rootFolder">
	...
	`;
		...
		function getCurrentState() {
			return {
				...
				rootFolder: content.querySelector('#profile-rootFolder').value
			};
		}
		...
		async function loadProfileIntoForm(profileId) {
			...
			let layoutValue = profile.rootFolder;
			if (typeof layoutValue === 'boolean') {
				layoutValue = layoutValue ? 'Original' : 'NoSubfolder';
			}
			content.querySelector('#profile-rootFolder').value = layoutValue || 'Original';
		...
		function switchToAddMode() {
			...
			content.querySelector('#profile-rootFolder').value = 'Original';
		...
		async function saveCurrentProfile(closeModal) {
			...
			const rootFolder = content.querySelector('#profile-rootFolder').value;
...
`;
	...
	const STBTN = ({ torrentUrl }) => {
		...
		btn.dataset.torrentUrl = torrentUrl;
		btn.onclick = async (e) => {
			e.preventDefault();
			...
			try {
				...
				btn.onclick = null;
				// Hide options button if it exists
				const optionsBtn = container.querySelector('.stc-options-btn');
				if (optionsBtn) {
		...
		// Only add options button if no profile selector was inserted on page
		if (!hasInsertedProfileSelector) {
			const optionsBtn = document.createElement('a');
			optionsBtn.href = '#';
			optionsBtn.className = 'stc-options-btn';
			optionsBtn.textContent = '⚙️';
			optionsBtn.title = 'Select profile';
			optionsBtn.onclick = async (e) => {
				e.preventDefault();
				await openInlineProfileModal(optionsBtn);
			};
	...
	const handlers = [
		{
			...
			run: async () => {
				...
				window.addEventListener('profileChanged', () => {
					document.querySelectorAll('a.sendtoclient').forEach(btn => {
						if (btn.textContent === 'FST') {
							btn.title = `Freeleechize and add to ${profileManager.currentProfile.name}`;
						} else if (btn.textContent !== 'Added!') {
							btn.title = `Add to ${profileManager.currentProfile.name}`;
						}
					});
					document.querySelectorAll('a.stc-options-btn').forEach(btn => {
						btn.title = 'Select profile';
					});
				});
			}
		},
	...
	(async () => {
		await profileManager.init();
		await updateProfileMenuCommand();
		hasInsertedProfileSelector = insertProfileSelector();
		await createButtons();
	})();
