...
// @namespace   https://greasyfork.org/en/users/732314-skillsboy
// @author      SkillsBoY
// @version     0.3.0
// @license     GPLv3
// @description Villages, Troops, Resources Overview, Quick links. More to come, feel free to give ideas...
// @include     *://*.travian.*
// @include     *://*/*.travian.*
...
// @grant       GM_addStyle
// @grant       GM_info
// @downloadURL https://update.greasyfork.org/scripts/447572/Travian%20Legends%2B%2B.user.js
// @updateURL https://update.greasyfork.org/scripts/447572/Travian%20Legends%2B%2B.meta.js
// ==/UserScript==
...
// travian utilities
{
    ...
    function getResources() {
        return {
            lumber: parseInt(document.querySelector("#l1").textContent.replace(/[^\x00-\x7F]|,/g, "")),
            clay: parseInt(document.querySelector("#l2").textContent.replace(/[^\x00-\x7F]|,/g, "")),
            iron: parseInt(document.querySelector("#l3").textContent.replace(/[^\x00-\x7F]|,/g, "")),
            crop: parseInt(document.querySelector("#l4").textContent.replace(/[^\x00-\x7F]|,/g, ""))
        };
    }
    ...
    function getHourlyProduction() {
        ...
        return {
            lumber: parseInt(lumberTitle.substring(lumberTitle.indexOf(":") + 2, lumberTitle.indexOf("<"))),
            clay: parseInt(clayTitle.substring(clayTitle.indexOf(":") + 2, clayTitle.indexOf("<"))),
            iron: parseInt(ironTitle.substring(ironTitle.indexOf(":") + 2, ironTitle.indexOf("<"))),
            crop: parseInt(spanTemp.textContent.match(/[−\d]+/g).join("").replace("−", "-"))
        };
    }
    ...
    function createSettingsHTML() {
        ...
        const html = `<div style="background-color: #FFFFFF; margin: 10px; padding: 10px;">
                ...
                <div>
                    <ul>
                        <li style="margin-bottom: 10px;">
                            <a href="https://greasyfork.org/en/scripts/447572-travian-legends" target="_blank">Features</a>
                        </li>
                    </ul>
                </div>
            </div>
            <div>
                ...
                <div>
                    <ul>
                        <li style="margin-bottom: 10px;"><strong>Name:</strong> Travian Legends++</li>
                        <li style="margin-bottom: 10px;"><strong>Version:</strong> ${myVersion}</li>
                        <li style="margin-bottom: 10px;"><strong>Author:</strong> SkillsBoY</li>
                        <li style="margin-bottom: 10px;"><strong>Want new features?</strong> <a href="https://greasyfork.org/en/scripts/447572-travian-legends/feedback" target="_blank">Feedback</a></li>
                        <li style="margin-bottom: 10px;"><strong><a href="https://www.buymeacoffee.com/skillsboy" target="_blank">Donate</a></strong></li>
                    </ul>
                </div>
            </div>
...
// main code
{
    ...
    if (/karte.php/.test(window.location.pathname)) {
        ...
        // css
        {
            ...
            #us_mapData > .us__map-content > div {
                display: none;
            }

            #us_mapData > .us__map-content > div.active {
                display: block;
            }

            #us_mapData > .us__map-content thead th {
                font-weight: bold;
            }

            #us_mapData > .us__tabs > .active {
                color: #283308!important;
                background-image: none!important;
                border-color: #506030!important;
                box-shadow: inset 0 20px 25px -10px rgba(255,255,255,.7)!important;
        ...
        // functions
        {
            // parse travian cookie property name (until now the only type, inside the Travian object, that is a string is the cookie)
            function getCookieProperty() {
                for (let key in Travian) {
                    if (typeof Travian[key] === "string") return key;
                }
            }

            function calcDistance(origin, destination) {
                const b = Math.abs(origin.x - destination.x);
                const a = Math.abs(origin.y - destination.y);
                const c = Math.sqrt(a * a + b * b);

                return Math.round(c * 10) / 10;
            }

            function getActiveVillageCoordinates() {
                const coordinateX = parseInt(
                    document
                        .querySelector(
                            "#sidebarBoxVillagelist > div.content > div.villageList .listEntry.active .coordinateX"
                        )
                ...
                const coordinateY = parseInt(
                    document
                        .querySelector(
                            "#sidebarBoxVillagelist > div.content > div.villageList .listEntry.active .coordinateY"
                        )
                        .textContent.match(/[−\d]+/g)
                        .join("")
                        .replace("−", "-")
            ...
            function mapApi(url, t) {
                if (url === "/api/v1/map/position") {
                    const activeVillageCoordinates =
                        getActiveVillageCoordinates();
                    ...
                    t.tiles.forEach((tile) => {
                        const { position, text, title, did, uid } = tile;

                        // if (position.x === -30 && position.y === 61) console.log(tile);

                        const titleTranslated = title
                            ? Travian.Translation.translate(title)
                            : "";

                        const textTranslated = text
                            ? Travian.Translation.translate(text)
                            : "";

                        const distance = calcDistance(
                            activeVillageCoordinates,
                            position
                        );

                        if (!titleTranslated) {
                            // wilderness
                            us_mapData.wilderness.push({
                                position,
                                textTranslated,
                            });
                        } else if (
                            titleTranslated.includes(
                                Travian.Translation.keys["k.vt"]
                            ) // abandoned valley
                        ) {
                            us_mapData.abandonedValleys.push({
                                position,
                                valley: titleTranslated.split(" ")[2],
                                distance,
                            });
                        } else if (
                            titleTranslated.includes(
                                Travian.Translation.keys["k.bt"]
                            ) // occupied oasis
                        ) {
                            let player;
                            const resources = [];

                            textTranslated.split("<br />").forEach((line) => {
                                if (
                                    line.includes(
                                        Travian.Translation.keys["k.spieler"]
                                    ) // player
                                ) {
                                    player = line.split(" ")[1];
                                } else if (
                                    line.includes("r4") &&
                                    line.includes(
                                        Travian.Translation.keys["a.r4"]
                                    ) // crop
                                ) {
                                    resources.push(line);
                                } else if (
                                    line.includes(
                                        Travian.Translation.keys["a.r1"]
                                    ) // lumber
                                ) {
                                    resources.push(line);
                                } else if (
                                    line.includes(
                                        Travian.Translation.keys["a.r2"]
                                    ) // clay
                                ) {
                                    resources.push(line);
                                } else if (
                                    line.includes(
                                        Travian.Translation.keys["a.r3"]
                                    ) // iron
                                ) {
                                    resources.push(line);
                                }
                            ...
                            const animalsNode =
                                div.querySelectorAll(".tooltipUnit");

                            animalsNode.forEach((el) =>
                                animals.push(el.outerHTML)
                            );

                            us_mapData.unoccupiedOasis.push({
                                position,
                                resources: resources.join("<br>"),
                                distance,
                            ...
                            textTranslated.split("<br />").forEach((line) => {
                                if (
                                    line.includes(
                                        Travian.Translation.keys["k.einwohner"] // population
                                    )
                                ) {
                    ...
                    us_mapData.villages.sort((a, b) => a.distance - b.distance);
                    us_mapData.abandonedValleys.sort(
                        (a, b) => a.distance - b.distance
                    );
                    us_mapData.unoccupiedOasis.sort(
                        (a, b) => a.distance - b.distance
                    );
                    us_mapData.occupiedOasis.sort(
                        (a, b) => a.distance - b.distance
                    );

                    const villagesHtml = us_mapData.villages
                        .map((tile) => {
                            const {
                                position,
                                population,
                            ...
                            return `<tr>
                                <td>${distance}</td>
                                <td><a href="/position_details.php?x=${position.x}&y=${position.y}" target="_blank" rel="noreferrer">(${position.x}|${position.y})</a></td>
                                <td>${population}</td>
                                <td><a href="/profile/${uid}" target="_blank" rel="noreferrer">${player}</a></td>
                                <td>${titleTranslated}</td>
                            </tr>`;
                        })
                        .join("");

                    const abandonedValleysHtml = us_mapData.abandonedValleys
                        .map((tile) => {
                            const { position, valley, distance } = tile;
                            let crop = "";

                            if (/9|15/.test(valley))
                                crop = "background-color: green";

                            return `<tr>
                                    <td>${distance}</td>
                                    <td><a href="/position_details.php?x=${position.x}&y=${position.y}" target="_blank" rel="noreferrer">(${position.x}|${position.y})</a></td>
                                    <td style="${crop}">${valley}</td>
                                </tr>`;
                        })
                        .join("");

                    const unoccupiedOasisHtml = us_mapData.unoccupiedOasis
                        .map((tile) => {
                            const { position, resources, distance, animals } =
                                tile;
                    ...
                    const occupiedOasisHtml = us_mapData.occupiedOasis
                        .map((tile) => {
                            const {
                                position,
                                uid,
                    ...
                    t1.innerHTML = villagesHtml;
                    t2.innerHTML = abandonedValleysHtml;
                    t3.innerHTML = unoccupiedOasisHtml;
                    t4.innerHTML = occupiedOasisHtml;
                }
            }
        }
        ...
        {
            Travian.api = function (t, e = {}, i = "POST", n = "json") {
                const a = function (t = {}, e, i) {
                        return t.reload
                            ? Travian.Autoreload.autoreload()
                            : t.redirectTo
                            ? Travian.redirectTo(t.redirectTo)
                            : void e(t, i);
                    },
                    o = e.success || Travian.emptyFunction,
                    r = e.error || Travian.defaultErrorFunction,
                    s = Object.assign(e.headers || {}, {
                        "X-Version": "2206.6",
                        Authorization: "Bearer " + Travian[getCookieProperty()],
                    }),
                    l = Object.assign(
                        {},
                        Travian.Variables.javascript.ajaxOptions || {},
                        e,
                        {
                            url: "/api/v1/" + t,
                            method: i,
                            ...
                            headers: s,
                            complete: e.complete || Travian.emptyFunction,
                            success: function (t, e, i) {
                                a(t, o, i);
                                mapApi(this.url, t);
                            },
                            error: function (t, e, i) {
                                a(t && t.responseJSON, r, t);
                            },
                        }
                    );
        ...
        // create map tabs
        {
            const mapContentContainer = document.querySelector(
                "#contentOuterContainer > div.contentContainer"
            );

            mapContentContainer.insertAdjacentHTML(
                "beforeend",
                `<div id="us_mapData">
                <div class="us__tabs">
                    <button type="button" data-tab="us__t1" class="textButtonV1 green active">Villages</button>
                    <button type="button" data-tab="us__t2" class="textButtonV1 green">Abandoned Valleys</button>
                    <button type="button" data-tab="us__t3" class="textButtonV1 green">Unoccupied Oasis</button>
                    <button type="button" data-tab="us__t4" class="textButtonV1 green">Occupied Oasis</button>
                </div>
                <div class="us__map-content">
                    <div id="us__t1" class="active">
                        <table>
                            <thead>
                                <tr>
                                    <th>Distance</th>
            ...
            const tabs = document.querySelector("#us_mapData > .us__tabs");
            const content = document.querySelector(
                "#us_mapData > .us__map-content"
            );
            ...
            tabs.addEventListener("click", (e) => {
                ...
                if (tab) {
                    ...
                    tabs.querySelectorAll("button.active").forEach((el) =>
