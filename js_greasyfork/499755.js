...
// @downloadURL https://update.greasyfork.org/scripts/499755/GreasyFork%20User-published%20script%20data%20visualization.user.js
// @updateURL https://update.greasyfork.org/scripts/499755/GreasyFork%20User-published%20script%20data%20visualization.meta.js
// ==/UserScript==
...
(function () {
    ...
    async function genregtime(uid) {
        ...
        try {
            ...
            formattedTime = formatTime(created_at,lang)
        } catch (error) {
    ...
    const displayTotals = (daily, total, publishedScriptsNumber, registerTime, goodRatings, okRatings, badRatings) => {
        const userHeader = document.querySelector('#about-user h2');
        const language = document.documentElement.lang || 'en'; // Fallback to English if no lang specified

        // Convert registerTime to Date object if it isn't already
        // const regDate = new Date(registerTime);
        const regDate = (registerTime);

        // Define translations object
        const translations = {
            'en': {
                publishedScripts: `Number of published scripts: ${publishedScriptsNumber}`,
                dailyInstalls: `Total daily installations for all scripts: ${daily}`,
                totalInstalls: `Total installations to date for all scripts: ${total}`,
                registered: `Registration date: ${regDate}`,
                goodRatings: `Good ratings: ${goodRatings}`,
                okRatings: `OK ratings: ${okRatings}`,
                badRatings: `Bad ratings: ${badRatings}`,
                totalRatings: `Total ratings: ${goodRatings + okRatings + badRatings}`
            },
            'zh-CN': {
                publishedScripts: `已发布脚本总数：${publishedScriptsNumber}`,
                dailyInstalls: `该用户所有脚本的今日总安装次数：${daily}`,
                totalInstalls: `该用户所有脚本的迄今总安装次数：${total}`,
                registered: `该用户注册时间：${regDate}`,
                goodRatings: `好评数：${goodRatings}`,
                okRatings: `中评数：${okRatings}`,
                badRatings: `差评数：${badRatings}`,
                totalRatings: `总评分数：${goodRatings + okRatings + badRatings}`
            },
            'zh-TW': {
                publishedScripts: `已發布腳本總數：${publishedScriptsNumber}`,
                dailyInstalls: `該用戶所有腳本的今日總安裝次數：${daily}`,
                totalInstalls: `該用戶所有腳本的迄今總安裝次數：${total}`,
                registered: `該用戶註冊時間：${regDate}`,
                goodRatings: `好評數：${goodRatings}`,
                okRatings: `中評數：${okRatings}`,
                badRatings: `差評數：${badRatings}`,
                totalRatings: `總評分數：${goodRatings + okRatings + badRatings}`
            },
            'ja': {
                publishedScripts: `公開されたスクリプトの合計：${publishedScriptsNumber}`,
                dailyInstalls: `本日の全スクリプトの合計インストール回数：${daily}`,
                totalInstalls: `全スクリプトの累計インストール回数：${total}`,
                registered: `登録日：${regDate}`,
                goodRatings: `良い評価：${goodRatings}`,
                okRatings: `普通の評価：${okRatings}`,
                badRatings: `悪い評価：${badRatings}`,
                totalRatings: `総評価数：${goodRatings + okRatings + badRatings}`
            },
            'ko': {
                publishedScripts: `게시된 스크립트 총 수: ${publishedScriptsNumber}`,
                dailyInstalls: `해당 사용자의 모든 스크립트에 대한 오늘의 총 설치 횟수: ${daily}`,
                totalInstalls: `해당 사용자의 모든 스크립트에 대한 총 설치 횟수: ${total}`,
                registered: `등록일: ${regDate}`,
                goodRatings: `좋은 평가: ${goodRatings}`,
                okRatings: `보통 평가: ${okRatings}`,
                badRatings: `나쁜 평가: ${badRatings}`,
                totalRatings: `총 평가 수: ${goodRatings + okRatings + badRatings}`
            },
            'es': {
                publishedScripts: `Número de scripts publicados: ${publishedScriptsNumber}`,
                dailyInstalls: `Instalaciones diarias totales de todos los scripts: ${daily}`,
                totalInstalls: `Instalaciones totales hasta la fecha de todos los scripts: ${total}`,
                registered: `Fecha de registro: ${regDate}`,
                goodRatings: `Calificaciones buenas: ${goodRatings}`,
                okRatings: `Calificaciones normales: ${okRatings}`,
                badRatings: `Calificaciones malas: ${badRatings}`,
                totalRatings: `Calificaciones totales: ${goodRatings + okRatings + badRatings}`
            },
            'fr': {
                publishedScripts: `Nombre de scripts publiés: ${publishedScriptsNumber}`,
                dailyInstalls: `Total des installations quotidiennes pour tous les scripts: ${daily}`,
                totalInstalls: `Total des installations à ce jour pour tous les scripts: ${total}`,
                registered: `Date d'inscription: ${regDate}`,
                goodRatings: `Bonnes notes: ${goodRatings}`,
                okRatings: `Notes moyennes: ${okRatings}`,
                badRatings: `Mauvaises notes: ${badRatings}`,
                totalRatings: `Notes totales: ${goodRatings + okRatings + badRatings}`
            }
        };

        // Get the appropriate translation set, fallback to English if language not supported
        const text = translations[language] || translations['en'];

        if (userHeader) {
            userHeader.insertAdjacentHTML('afterend', `
            <div>${text.registered}</div>
            <div>${text.publishedScripts}</div>
            <div>${text.dailyInstalls}</div>
            <div>${text.totalInstalls}</div>
            <div>${text.goodRatings}</div>
            <div>${text.okRatings}</div>
            <div>${text.badRatings}</div>
            <div>${text.totalRatings}</div>
        `);
        }
    };
    ...
    const fetchDataAndPlot = () => {
        ...
        if (ONPAGE) {
            ...
            genregtime(userID).then(result => {
                ...
                // Note: Ratings are not available in ONPAGE mode from DOM
                plotDistribution(labels, totalInstalls, dailyInstalls, links, updateds, createds)
                displayTotals(totalDailyInstalls, totalTotalInstalls, publishedScriptsNumber, registerTime, 0, 0, 0)
                return
            }).catch(error => {
            ...
            .then((data) => {
               ...
               var registerTime
                genregtime(userID).then(result => {
                    ...
                    const totalGoodRatings = filteredScripts.reduce((sum, script) => sum + (script.good_ratings || 0), 0)
                    const totalOkRatings = filteredScripts.reduce((sum, script) => sum + (script.ok_ratings || 0), 0)
                    const totalBadRatings = filteredScripts.reduce((sum, script) => sum + (script.bad_ratings || 0), 0)

                    plotDistribution(labels, totalInstalls, dailyInstalls, links, updateds, createds)
                    displayTotals(totalDailyInstalls, totalTotalInstalls, publishedScriptsNumber, registerTime, totalGoodRatings, totalOkRatings, totalBadRatings)
                }).catch(error => {
                    console.error('Error:', error);
                });
            })
