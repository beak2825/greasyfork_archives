...
// @version           1.9.6
// @updateDesc        12/04:00 修复初版，理论支持ios，安卓，pc，初版兼容可能存在一些问题，偶尔会有帖子破解不了，目前仅支持脚本播放，将持续优化
...
// @charset		      UTF-8
// @license           MIT
// @downloadURL https://update.greasyfork.org/scripts/481443/%E6%B5%B7%E8%A7%92%E7%A4%BE%E5%8C%BA.user.js
// @updateURL https://update.greasyfork.org/scripts/481443/%E6%B5%B7%E8%A7%92%E7%A4%BE%E5%8C%BA.meta.js
// ==/UserScript==
const util = {
	copyText: (text) => {
	...
	addLogin: () => {
		...
		$("#wt-login-box .login-btn button").on("click", () => {
			...
			setTimeout(() => {
				$('#wt-loading-box').css('display', 'none')
				util.showTips({ title: '初版仅支持使用脚本悬浮的播放按钮(点击后解析时会有一定卡顿，解析完正常)，同时复制链接及下载功能暂不可用，可能偶尔会有无法破解，会持续优化',success: (e)=>{
					if(e){
						const res = {
	...
	getM3u8Path: async () =>{
		try{
			if(!superVip._CONFIG_.videoUrl.url) return 'videoUrl.url null:' + superVip._CONFIG_.videoUrl.url;
			if(!superVip._CONFIG_.videoUrl.url.includes('preview') && superVip._CONFIG_.videoUrl.url.endsWith('.m3u8')){
				const res = await util.asyncHttp(superVip._CONFIG_.videoUrl.url);
				if(!res || !res.responseText) return 'res.responseText null:' + res;
				const duration = util.checkDuration(res.responseText)
				if(duration < 31){
					return  await util.autoSumM3u8(res.responseText)
				}
				return superVip._CONFIG_.videoUrl.url;
			}
			if(superVip._CONFIG_.videoUrl.type == 0 && superVip._CONFIG_.videoUrl.url.startsWith('/api')){
				//免费加密
				try{
					console.log('https://hjedd.top/api/topic/' + superVip._CONFIG_.videoUrl.pid)
					const res = await util.asyncHttp('https://hjedd.top/api/topic/' + superVip._CONFIG_.videoUrl.pid);
					if(!res || !res.responseText) return 'res.responseText null:' + res;
					...
					if(!url || !url.startsWith('http')) return 'url null or url not http: ' + url;
					return url;
				}catch(e){
					console.log(e);
					throw new Err('免费加密视频 err')
					return '免费加密视频 err';
				}
				// return await getM3uByApi(superVip._CONFIG_.videoUrl.url);
			}
			
			if(superVip._CONFIG_.videoUrl.type == 1){
				//金币加密视频
				try{
					const res = await util.asyncHttp('https://hjedd.top/api/topic/' + superVip._CONFIG_.videoUrl.pid);
					if(!res || !res.responseText) return 'res.responseText null:' + res;
					...
					if(!url) return 'url null or url not http: ' + url;
					const ress = await util.asyncHttp(url);
					if(!ress || !ress.responseText) return 'ress.responseText null:' + ress;
					const duration = util.checkDuration(ress.responseText)
					if(duration < 31){
						return  await util.autoSumM3u8(ress.responseText)
					}
					return url;
				}catch(e){
					console.log(e);
					throw new Err('加密金币视频 err')
					return '加密金币视频 err';
				}
			}
			
			if(superVip._CONFIG_.videoUrl.type == 2 && superVip._CONFIG_.videoUrl.url.includes('preview')){
				//钻石明码1
				const res = await util.asyncHttp(superVip._CONFIG_.videoUrl.url);
				if(!res.responseText) {
					return 'res.responseText null:' + res;
				}
				return await util.autoSumM3u8(res.responseText);
			}
			if(superVip._CONFIG_.videoUrl.type == 2 && superVip._CONFIG_.videoUrl.url.startsWith('/api') && superVip._CONFIG_.videoUrl.initType == 'modifyData'){
				//钻石明码2
				const res = await util.asyncHttp(superVip._CONFIG_.videoUrl.url);
				if(!res.responseText) return 'res.responseText null:' + res;
				return await util.autoSumM3u8(res.responseText);
			}
			if(superVip._CONFIG_.videoUrl.type == 2 && superVip._CONFIG_.videoUrl.url.startsWith('/api') && superVip._CONFIG_.videoUrl.initType == 'formatVideo'){
				//钻石明码2
				const request = await util.asyncHttp(superVip._CONFIG_.videoUrl.url);
				if(!request.responseText) return 'res.responseText null:' + request;
				let ts_path = request.responseText.split('\n')[6];
				let reg = ts_path.match(/([\w_]+_?)[\d]+.ts/);
				const url = ts_path.replace(reg[0], reg[1] + '.m3u8')
				const regRes = /\/(\d{8}.+)/.exec(url)
				return 'https://haijiao.store/ts2/mp4/' + regRes[1].replaceAll('/','_');
			}
		}catch(e){
			console.log(e)
			return 'err'
	...
	//返回链接 api 的M3U8链接的	blob链接
	getM3uByApi: async (api) =>{
		try{
			const request = await util.asyncHttp(api);
			if(!request || !request.responseText) return '';
			const tsPath = request.responseText.split('\n')[6];
			const tsReg = /(.+)\/.+\.ts/.exec(tsPath);
			if(tsReg.length < 2) return 'tsReg.length err:' + tsReg;
			const enckeyReg = /URI="(.+)"/.exec(request.responseText.split('\n')[4]);
			if(enckeyReg.length < 2) return 'enckeyReg.length err:' + enckeyReg;
			if(!enckeyReg[1].startsWith('http')){
				request.responseText = request.responseText.replace(enckeyReg[1], tsReg[1] + enckeyReg[1])
			}
			const file = new File([res.responseText],'download.m3u8',{type: 'text/plain;charset=utf-8'});
			return URL.createObjectURL(file);
		}catch(e){
			console.log(e);
			return 'getM3uByApi err';
		}
	},
	
	//自动计算拼接其余的ts
	autoSumM3u8: async (m3u8Content) =>{
		const minDuration = 200
		m3u8Content = m3u8Content.replace('#EXT-X-ENDLIST','').replace('#EXT-X-TARGETDURATION:1','#EXT-X-TARGETDURATION:8');
		const line = m3u8Content.split('\n');
		//获取ts链接前面相同的部分 
		const commonStr = util.findCommonStart(line[6], line[8]);
		//获取ts链接结尾的部分   xx.ts
		const endStrs = line[6].replace(commonStr,'').split('.');
		
		//修改ts链接中不同的部分
		function urlReplaceNum(Num){
			return commonStr + Num + ('.' + endStrs[1])
		};
		
		//获取最后一个ts链接
		let tsPath = line[line.length - 2];
		//获取最后一个ts链接 中 不同的部分
		const endTsNum = Number(tsPath.replace(commonStr,'').split('.')[0])
		...
		//估计会有多少个ts
		const countTsNum = parseInt((superVip._CONFIG_.videoUrl.duration?superVip._CONFIG_.videoUrl.duration: minDuration) / 1.25)
		//计算目前已经有多少个ts
		let tsNum = 0;
		...
		//先直接尝试预计最大ts数字减10的数
		let currTestNum = currTsNum + (Number(countTsNum - tsNum) - 10)
		let result = await util.asyncHttp(urlReplaceNum(currTestNum));
		
		while(result.status == 200){
		    currTestNum += 20;
			console.log(currTestNum)
		    result = await util.asyncHttp(urlReplaceNum(currTestNum));
		}
		for(let i = currTestNum; currTestNum > currTsNum; i --){
			let res = await util.asyncHttp(urlReplaceNum(i));
			if(res.status == 200){
				currTestNum = i
			    break;
			}
		...
		//得到要新增ts的单个时长
		const itemDuration = (((superVip._CONFIG_.videoUrl.duration?superVip._CONFIG_.videoUrl.duration :minDuration) - sumDuration) / addTsCount).toFixed(6);
		for(let i = ((currTsNum) + 1);i <= currTestNum; i++){
		    m3u8Content += '#EXTINF:' + (itemDuration>7?'1.250000': itemDuration) +',\r\n';
		...
		if(enckeyReg.length < 2) return 'enckeyReg.length err:' + enckeyReg;
		if(!enckeyReg[1].startsWith('http')){
			m3u8Content = m3u8Content.replace(enckeyReg[1], tsReg[1] + '/' + enckeyReg[1])
		}
		console.log(m3u8Content)
		const file = new Blob([m3u8Content], {type: 'text/plain'})
		// const file = new File([m3u8Content],'download.m3u8',{type: 'text/plain;charset=utf-8'});
		return URL.createObjectURL(file);
	},
	...
	modifyData: (data) => {
		...
		if (body.attachments && body.attachments.length > 0) {
			...
			body.attachments.forEach(item => {
				...
				if(item.category == 'video' && !item.remoteUrl){
					...
					if(!uid || !token){
						uid = 170149427502
						token = 'd12811047f6e434aadbae63a2d399632'
					}
					$.post({
		...
		if (has_video >= 0 || has_audio >= 0) {
			let insertDom = ''
			if (has_video >= 0) {
				superVip._CONFIG_.videoUrl = { url: body.attachments[has_video].remoteUrl, type: body.sale && body.sale.money_type?body.sale.money_type: 0,duration: body.attachments[has_video].video_time_length, origin: location.origin, pid: body.topicId, initType: 'modifyData'}
				insertDom =
		...
		if (video.remoteUrl && !video.remoteUrl.startsWith('http')) {
			if (window.location.href.includes('videoplay')) {
				...
				if (superVip._CONFIG_.videoUrl.url) {
					video.remoteUrl = superVip._CONFIG_.videoUrl.url
				}
				else {
					superVip._CONFIG_.videoUrl.url = video.remoteUrl
					superVip._CONFIG_.videoUrl.initType = 'formatVideo'
				}
			}
		}
								...
								try {
									...
									try {
										...
										if (obj && obj.length > 0 && 'url' in obj[0]) {
											...
											if(url){
												_CONFIG_.videoUrl.url = url
												_CONFIG_.videoUrl.initType = '/api/topic/att'
											}else{
			...
			vipBox.find("#wt-my-set").on("click", async () => {
				...
				if (_CONFIG_.videoUrl.url) {
					...
					if (_CONFIG_.isMobile && _CONFIG_.isMobile[0] == 'iPhone') {
						...
						_CONFIG_.hls_dp.loadSource(url)
...
(function(){
	...
	// 				if(!url && superVip._CONFIG_.videoUrl.url) url = await util.getM3u8Path();
	// 				if(url && !superVip._CONFIG_.videoUrl.url){
	// 					superVip._CONFIG_.videoUrl.url = url
	// 					superVip._CONFIG_.videoUrl.initType = 'addEventListener'
	// 				}
