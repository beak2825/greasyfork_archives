...
// @namespace    Ruthuvan Dream Info
// @version      5.7
// @description  Brings many enhancements to the MTurk Worker Dashboard.
// @author       RUTHUVAN
// @icon         http://i.imgur.com/oGRQwPN.png
// @include      https://worker.mturk.com/dashboard*
// @downloadURL https://update.greasyfork.org/scripts/381425/%5BMTurk%20Worker%5D%20Dashboard%20Enhancer.user.js
// @updateURL https://update.greasyfork.org/scripts/381425/%5BMTurk%20Worker%5D%20Dashboard%20Enhancer.meta.js
// ==/UserScript==


//$(document).ready(function(){
//$(".bonus-rewards-column").find("span").text("$0.00");
//var ah = $('.mturk-table td').eq(1).html().replace("$", "");
//var tb = $('.mturk-table td').eq(3).html().replace("$", "");
//$('.mturk-table td').eq(3).html("$" + (tb /10).toFixed(2));
//$('.mturk-table td').eq(5).html("$" + ah);

//$( ".desktop-row, .fa-plus-circle, .table-expand-collapse-button" ).click(function() {
//      setTimeout(function() {
//$(".bonus-rewards-column").find("span").text("$0.00");
//$('.earnings-column').hide();
//$('.bonus-rewards-column').hide();
...
function getvalue()
{
var e = document.getElementById("dtselect");
 selecteddt= e.options[e.selectedIndex].value;
//     alert(selecteddt)
    latestActivity(selecteddt);
}
...
function formatDate(inDate) {
    ...
    if (mm < 10) mm = '0' + mm
    var yy = inDate.getFullYear().toString();
    return yy+'-'+ mm + '-' + dd ;
}
//var selectdate="";
// function latestActivity1() {

//   const container1 = document.createElement(`div`);
//   container1.className = `row m-b-xl`;

//   const coldt = document.createElement(`div`);
//   coldt.className = `col-xs-12`;
//   container1.appendChild(coldt);

//   const dtdropdown = document.createElement(`div`);
//   dtdropdown.className = `row`;
//   dtdropdown.id = `dtdropdown`;
//   coldt.appendChild(dtdropdown);

//     var selectList = document.createElement("select");
//     selectList.setAttribute("id", "dtselect");
//     dtdropdown.appendChild(selectList);

//     for (var i = 0; i < 5; i++) {
//     var d = new Date();
//     var option = document.createElement("option");
//     d.setDate(d.getDate() - i)
//     var todaydt= formatDate(d);
//     option.value = todaydt;
//     option.text = todaydt;
//     selectList.appendChild(option);
//     }

//     setInterval(function(){ document.getElementById("dtselect").onchange = function() {getvalue()}; }, 3000);
// }

function latestActivity(parmdt) {

//     alert ("parm:"+parmdt)
  const latest = statusDetailsArr[0];

       var date = toDate(latest.date);
    if(parmdt)
    {
         date = parmdt;
    }

//    const date="2019-03-25";
  ...
  async function get(page, rescan) {
    try {
      ...
      if (json.results.length === 20) {
        ...
        const sort = Object.keys(reqLog).sort((b, a) => reqLog[b] - reqLog[a]).reverse();
        ...
        earningsValue.textContent = `$${projectedEarnings.toLocaleString(`en-US`, { minimumFractionDigits: 2 })}`;
          setInterval(function(){ document.getElementById("dtselect").onchange = function() {date=getvalue()}; }, 3000);
      }
    } catch (error) {
