...
// @namespace    NightSwan
// @version      0.39
// @description  B站评论区自动标注三相玩家，依据是动态里是否有三相相关内容（基于原神指示器和三相一些小的修改）
...
// @license MIT
// @run-at document-end
// @downloadURL https://update.greasyfork.org/scripts/451395/%E6%B2%A1%E6%9C%89%E6%94%BB%E5%87%BB%E6%80%A7%E7%9A%84%E4%B8%89%E7%9B%B8%E4%B9%8B%E5%8A%9B%E6%8C%87%E7%A4%BA%E5%99%A8.user.js
// @updateURL https://update.greasyfork.org/scripts/451395/%E6%B2%A1%E6%9C%89%E6%94%BB%E5%87%BB%E6%80%A7%E7%9A%84%E4%B8%89%E7%9B%B8%E4%B9%8B%E5%8A%9B%E6%8C%87%E7%A4%BA%E5%99%A8.meta.js
// ==/UserScript==
...
(function () {
  ...
  //成分，可自定义
  const luyou = new Set()//撸
  const no_luyou = new Set()

  const daoyou = new Set()//刀
  const no_daoyou = new Set()
  ...
  const sai = new Set()
  const no_sai = new Set()
  const zs = new Set()
  const no_zs = new Set()
  const bl = new Set()
  const no_bl = new Set()
  const yxw = new Set()
  const no_yxw = new Set()
  const sq = new Set()
  const no_sq = new Set()
  const bb = new Set()
  const no_bb = new Set()
  ...
  const keyword_sai = '塞尔达'
  const keyword_zs = '战双'
  const keyword_bl = '碧蓝'
  const keyword_yxw = '游戏王'
  const keyword_sq = '少前'
  const keyword_bb = '崩坏'



  const keyword_lu = 'lol'
  const keyword_dao = 'dota'
  ...
  const tag_zhou = " 【明日方舟】"
  const tag_nong = " 【王者荣耀】"
  const tag_yuanzhou = " 【二次元双象限】"
  const tag_yuannong = " 【双厨狂喜】"
  const tag_zhounong = " 【稀有的存在】"
  const tag_sanxiang = " 【三相之力】"
  const tag_cj = " 【隐藏】"
  const tag_jxt = " 嘉心糖"
  const tag_ccj = " 雏草姬"
  const tag_gcb = " 棺材板"
  ...
  //标签颜色，可自定义，默认为B站会员色
  const tag_sai_Inner = "<b style='color: #48c9b0'>" + tag_sai + "</b>"
  const tag_zs_Inner = "<b style='color: #dc143c'>" + tag_zs + "</b>"
  const tag_bl_Inner = "<b style='color: #b22222'>" + tag_bl + "</b>"
  const tag_yxw_Inner = "<b style='color: #ff8c00'>" + tag_yxw + "</b>"
  const tag_sq_Inner = "<b style='color: #cd853f'>" + tag_sq + "</b>"
  const tag_bb_Inner = "<b style='color: #00ffff'>" + tag_bb + "</b>"


  const tag_daoyou_Inner = "<b style='color: #7b68ee'>" + tag_daoyou + "</b>"
  const tag_luyou_Inner = "<b style='color: #adff2f'>" + tag_luyou + "</b>"
  ...
  let jiance = setInterval(() => {
      ...
      if (commentlist.length != 0) {
          ...
          commentlist.forEach(c => {
              ...
              if (sai.has(pid)) {//塞尔达
              if (c.textContent.includes(tag_sai) === false) {
                  c.innerHTML += tag_sai_Inner
              }
              return
              } else if (no_sai.has(pid)) {
                  // do nothing
                  return
              }
              
                if (zs.has(pid)) {//战双
                  if (c.textContent.includes(tag_zs) === false) {
                      c.innerHTML += tag_zs_Inner
                  }
                  return
                  } else if (no_zs.has(pid)) {
                      // do nothing
                      return
                  }
                  if (bl.has(pid)) {//碧蓝
                    ...
                    if (yxw.has(pid)) {//游戏王
                      if (c.textContent.includes(tag_yxw) === false) {
                          c.innerHTML += tag_yxw_Inner
                      }
                      return
                      } else if (no_yxw.has(pid)) {
                          // do nothing
                          return
                      }
                      if (sq.has(pid)) {//少前
                        if (c.textContent.includes(tag_sq) === false) {
                            c.innerHTML += tag_sq_Inner
                        }
                        return
                        } else if (no_sq.has(pid)) {
                            // do nothing
                            return
                        }
                        if (bb.has(pid)) {//崩坏
                          if (c.textContent.includes(tag_bb) === false) {
                              c.innerHTML += tag_bb_Inner
                          }
                          return
                          } else if (no_bb.has(pid)) {
                              // do nothing
                              return
                          }

              if (daoyou.has(pid)) {
                  if (c.textContent.includes(tag_daoyou) === false) {
                      c.innerHTML += tag_daoyou_Inner
                  }
                  return
              } else if (no_daoyou.has(pid)) {
                  // do nothing
                  return
              }

              if (luyou.has(pid)) {
                  if (c.textContent.includes(tag_luyou) === false) {
                      c.innerHTML += tag_luyou_Inner
                  }
                  return
              } else if (no_luyou.has(pid)) {
                  // do nothing
                  return
              }
              ...
              GM_xmlhttpRequest({
                  ...
                  onload: function (res) {
                      if (res.status === 200) {
                          ...
                          if (st.includes(keyword_yxw)) {//小写
                            c.innerHTML += tag_yxw_Inner
                            yxw.add(pid)
                          } else {
                              no_yxw.add(pid)
                          }
                          if (st.includes(keyword_sq)) {//小写
                            c.innerHTML += tag_sq_Inner
                            sq.add(pid)
                          } else {
                              no_sq.add(pid)
