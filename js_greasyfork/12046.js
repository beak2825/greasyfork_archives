...
// @namespace https://greasyfork.org/users/14523
// @downloadURL https://update.greasyfork.org/scripts/12046/%5B%C5%BD%D3%BE%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/12046/%5B%C5%BD%D3%BE%5D.meta.js
// ==/UserScript==
...
(function () {
    ...
    var overwriting = function (evt) {
        if (evt.keyCode === 70) { // KEY_F
            for (var i = 0; i < amount; ++i) {
                setTimeout(function () {
                    window.onkeydown({ keyCode: 32 }); // Spacebar
...
var debugMonkeyReleaseMessage = "<h3>Namen mit Tribal Skins (Copy + Paste):</h3><p>" +
    "[ŽӾ] Isoquante<br>" +
    "[ŽӾ] ✩ĤiฎฅŽž✩ツ<br>" +
    "[ŽӾ] Sara<br>" +
    "[ŽӾ] Skundales<br>" +
    ...
    "<h3>Masse schießen = w</h3><p>" +
    "<h3>Masse schießen = d (Automatikschuss)</h3><p>" +
    "<h3>Fast Split = f</h3><p>" +
    "<h3>Blobs wechseln = Tab</h3><p>" +
    "<h3>Blob nach Größe wählen = 1 (größtes) bis 7 (klein)</h3><p>";
...
(function (d, f) {
    ...
    var Huge_Color = "#FF3C3C",
        ...
        Small_Color = "#00AA00",
        Tiny_Color = "#CC66FF",
        myColor = "#0066ff",//3371FF,b400ff
        virusColor = "#ededed";
    var lastMouseCoords = { x: 0, y: 0 };
    var ghostBlobs = [];


    var miniMapCtx = jQuery('<canvas id="mini-map" width="175" height="175" style="background-image: url(http://isoquant.bplaced.net/live%20access/agaraddon/gridneu.png);background-size:contain;border:2px solid #0096FF;text-align:center;position:fixed;bottom:5px;right:5px;"></canvas>')
    ...
    var extendedSkins = {
        ...
        "[žӿ] isoquante": "http://isoquant.bplaced.net/live%20access/agaraddon/tribals/isoquantee.png",
        "[žӿ] ✩ĥiฎฅžž✩ツ": "http://isoquant.bplaced.net/live%20access/agaraddon/tribals/erik.png",
        "[žӿ] sara": "http://isoquant.bplaced.net/live%20access/agaraddon/tribals/sara.png",
        "[žӿ] skundales": "http://isoquant.bplaced.net/live%20access/agaraddon/tribals/skundales.png",
        "[žӿ] serpentius": "http://isoquant.bplaced.net/live%20access/agaraddon/tribals/serpentius.png",
        "[žӿ] ℜøℓɐχ♕": "http://isoquant.bplaced.net/live%20access/agaraddon/tribals/rolex.png",
        "[žӿ] ✧ɯαѕняσσм✧": "http://isoquant.bplaced.net/live%20access/agaraddon/tribals/washroom.png",
        "[žӿ] silly": "http://isoquant.bplaced.net/live%20access/agaraddon/tribals/silly.png",
        "✧ɯαѕняσσм✧": "http://isoquant.bplaced.net/live%20access/agaraddon/tribals/washroom.png",
    ...
    function drawCellName(isMyCell, kbIndex, itemToDraw) {
        ...
        if ((zeach.hasNickname || isMyCell) && (this.name && (nameCache && (null == itemToDraw || -1 == zeach.textBlobs.indexOf(kbIndex))))) {
            ...
            if (shouldRelocateName.call(this)) {
                ...
                //yBasePos += itemToDraw.height / 1 / scale + 4;
...
'<h4 class="modal-title">[ŽӾ] ' + GM_info.script.version + '</h4>' +
'</div>' +
...
jQuery("#agario-main-buttons")
    .append('<button type="button" class="btn btn-danger" id="opnZC" onclick="showZCOverlay()" style="margin-top:5px;position:relative;width:100%;">Options</button>');
jQuery("#agario-main-buttons")
...
jQuery('#ZCOverlayBody').append('<div id="ZCStats" style="position:relative;width:100%; background-color: #d7d7d7; border-radius: 15px; padding: 5px 15px 5px 15px;">' +
