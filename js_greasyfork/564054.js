...
// @license        MIT
// @version        0.1.125.v038
// @match          http://*.jeuxvideo.com/forums/42-*
// @match          https://*.jeuxvideo.com/forums/42-*
// @match          http://*.jeuxvideo.com/forums/1-*
// @match          https://*.jeuxvideo.com/forums/1-*
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/564054/JVChat%20Premium.user.js
// @updateURL https://update.greasyfork.org/scripts/564054/JVChat%20Premium.meta.js
// ==/UserScript==
...
.jvchat-reduced #forums-post-message-editor .messageEditor__containerEdit > *:not(.messageEditor__edit):not(#jvchat-buttons-main) {
    display: none;
}
...
#bloc-formulaire-forum #message_topic {
    grid-column: 1;
}
#bloc-formulaire-forum .jvchat-buttons {
    grid-column: 2;
}
.jvchat-reduced #message_topic {
    min-height: 2em;
...
function getPayload(doc) {
    let scriptTags = [...doc.scripts].find(s => s.textContent?.includes('forumsAppPayload'));
    if (!scriptTags) return undefined;
    let payload = scriptTags.textContent.match(/forumsAppPayload\s*=\s*["']([^"']+)["']/);
    if (!payload) {
        return undefined;
    }
...
function postMessage() {
    ...
    let formData = normalizePayload(freshPayload);
    ...
    function onSuccess(res) {
        ...
        if (res.errors) {
            for (let key in res.errors) {
                addAlertbox("danger", res.errors[key]);
            }
            //Rehydratation PayLoad
            freshPayload.formSession = res["formSession"];
        } else {
            reactAreaText(textarea, '');
            tryCatch(forceUpdate)();
        }
...
function editMessage(bloc) {
    ...
    let contentClasses = bloc.getElementsByClassName("jvchat-content")[0];
    let formData = normalizePayload(freshPayload);
    ...
    function onSuccess(res) {
        ...
        //Temp_Dom
        contentClasses.innerHTML = `<div class="txt-msg text-enrichi-forum">${res.html}</div>`;
        fixMessage(contentClasses);
        detectMosaic(contentClasses);
        improveImages(contentClasses);

        //State_Edit
        lastEditionTime[messageId] ??= [, , false];
        lastEditionTime[messageId][0] = getTimestamp();
        lastEditionTime[messageId][1] = "ToPush";

        contentClasses.classList.remove("jvchat-hide");
        blocEdition.classList.add("jvchat-hide");

        // Force_Update
        const suppressEditAnimation = document.createElement('style');
        suppressEditAnimation.textContent = `
        .jvchat-bloc-message:has(> [jvchat-id="${messageId}"]) { 
            animation-duration: 0s; 
        }`;
        document.head.appendChild(suppressEditAnimation);
        setTimeout(forceUpdate, 150);

        // Cleanup after DOM refresh window
        setTimeout(() => suppressEditAnimation.remove(), 2000);
...
function requestEdit(bloc) {
    ...
    let url = `/forums/ajax_edit_message.php?id_message=${id}&ajax_hash=${freshHash}&action=get`;
...
function normalizePayload(payload) {
    ...
    dict["text"] = "";
    // IDs
    dict["topicId"] = payload["topicId"];
    dict["forumId"] = payload["forumId"];
    // Rang
    const aliasRang = document.getElementById('form_alias_rang');
    dict["group"] = aliasRang?.value || "1";
    // Default Values 2
    dict["messageId"] = "undefined";
    // FS : valeurs fs à la racine du dictionnaire
    const fs = payload["formSession"];
    for (const key in fs) {
        dict[key] = fs[key];
    }
    // Ajax
    dict["ajax_hash"] = payload["ajaxToken"];
    return dict;
}
...
function submitSondageAnswer(event) {
    ...
    if (target.classList.contains("click-sondage")) {
        ...
        let url = `/forums/ajax_topic_sondage_vote.php?id_topic=${topicId}&id_sondage_reponse=${reponseId}&id_sondage=${sondageId}&ajax_hash=${freshHash}`;
