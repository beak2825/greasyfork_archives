// ==UserScript==
// @name         爱丽丝书屋 TTS 阅读器 (CosyVoice)
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  调用本地 CosyVoice API 朗读爱丽丝书屋小说，支持全文朗读、选段朗读、进度控制
// @author       Moear
...
// @icon         https://www.google.com/s2/favicons?sz=64&domain=alicesw.com/
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/560778/%E7%88%B1%E4%B8%BD%E4%B8%9D%E4%B9%A6%E5%B1%8B%20TTS%20%E9%98%85%E8%AF%BB%E5%99%A8%20%28CosyVoice%29.user.js
// @updateURL https://update.greasyfork.org/scripts/560778/%E7%88%B1%E4%B8%BD%E4%B8%9D%E4%B9%A6%E5%B1%8B%20TTS%20%E9%98%85%E8%AF%BB%E5%99%A8%20%28CosyVoice%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    let currentSessionId = 0; // 会话ID，防止多线程竞争
    ...
    function generateAudio(text, speaker, callback) {
        ...
        // 移除默认的 mode: "指令控制"，让后端使用角色配置的默认模式
        // 如果需要指定模式，可以在这里添加，例如 payload.mode = "零样本复制";
    ...
    function analyzePage() {
        const readContent = document.querySelector('.read-content.j_readContent.user_ad_content');
        if (!readContent) return;
        const paragraphs = readContent.querySelectorAll('p');
    ...
    // 生产者：负责连续生成音频
    function generateNext(sessionId) {
        // 如果会话ID不匹配，说明是旧的生成任务，直接退出
        ...
        if (generationIndex - playbackIndex > 5) {
            setTimeout(() => generateNext(sessionId), 1000);
            return;
        }
        ...
        if (globalAudioCache.has(cacheKey)) {
            ...
            setTimeout(() => generateNext(sessionId), 0);
            return;
        }
        ...
        generateAudio(text, speaker, (audioUrl) => {
            if (!isPlaying) return; 
            if (sessionId !== currentSessionId) return; // 再次检查
    ...
    function playAudio(url) {
        ...
        currentAudio.onended = () => {
            if (currentAudio !== thisAudio) return; // 防止旧音频的回调干扰
            isPlayingAudio = false;
            // 不再 revoke，以便复用
        ...
        currentAudio.play().catch(e => {
            if (currentAudio !== thisAudio) return; // 忽略旧音频的错误
    ...
    // extractTextContent 已被 getCleanText 替代，但为了兼容旧代码或清理，保留或移除
    // 这里我们移除它，因为不再使用了
