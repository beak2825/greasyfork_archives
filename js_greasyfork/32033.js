// ==UserScript==
// @name        Literotica Downloader Improved
...
// @version     2.1
// @author      Improved by a random redditor, originally by Patrick Kolodziejczyk
// @grant       GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/32033/Literotica%20Downloader%20Improved.user.js
// @updateURL https://update.greasyfork.org/scripts/32033/Literotica%20Downloader%20Improved.meta.js
// ==/UserScript==
...
$(document).ready(function () {
  ...
  function getContentOfStoie(baseURL) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if ($(data).find('.b-pager-next').size()) {
          remote = $(data).find('.b-story-body-x').html() + getContentOfStoie($(data).find('.b-pager-next') [0].href);
        } else {
          remote = $(data).find('.b-story-body-x').html();
        }
      }
    });
    return remote;
  ...
  function getABookForSerieDiv(myDiv) {
    ...
    book += '</head>\n<body style="background-color:#333333; color: #EEEEEE; font-family: Helvetica,Arial,sans-serif; width: 50%; margin: 0 auto; line-height: 1.5em; font-size:2.2em; padding: 50px 0 50px 0;">';
