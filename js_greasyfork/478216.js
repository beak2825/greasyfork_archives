// ==UserScript==
// @name         Ubits Batch Claim
// @namespace    http://tampermonkey.net/
// @version      0.1
// @description  Ubits批量认领脚本
// @author       CosmoGao
// @match        https://*.ubits.club/special.php*
// @match        https://*.ubits.club/torrents.php*
// @match        https://*.ubits.club/userdetails.php*
...
// @downloadURL https://update.greasyfork.org/scripts/478216/Ubits%20Batch%20Claim.user.js
// @updateURL https://update.greasyfork.org/scripts/478216/Ubits%20Batch%20Claim.meta.js
// ==/UserScript==
...
(async function () {
  ...
  console.log(url);
  let btn = `<input class="btn" type="button" value="批量认领" id="_ub_auto_claim" style="margin-left:8px;margin-right:4px;"/>`;
  if (url.includes("torrents") || url.includes("special")) {
    ...
    newTR.innerHTML = `<td width="1%" class="rowhead nowrap" valign="top" align="right">批量认领脚本</td><td><input class="btn" type="button" value="认领此账号所有做种的种子" id="_ub_claim_all" style="margin-left:16px;margin-right:4px;transform:scale(1.2);"/></td>`;
    table.append(newTR);
  } else if (url.includes("claim")) {
    btn = `<input class="btn" type="button" value="批量取消认领" id="_ub_auto_claim" style="margin-top:1em;margin-bottom:1em;transform:scale(1.2);"/>`;
    ...
    console.log("Ubits batch remove claim injected");
  }
  let claim_btn = document.querySelector("#_ub_auto_claim");
  let claim_all_btn = document.querySelector("#_ub_claim_all");
...
}</style><p id="_ub_label" style="margin-top:10px;color:black;font-weight:bold;font-size:1.3em;text-shadow:0px 2px 5px rgba(255,255,255,0.6);">正在认领 第1页...</p><textarea style="padding: 10px;
    overflow-y: scroll;
    width: 257px;
    color: red;
    height: 80px;
    margin-top: 1em;background:rgab(255,255,255,0.38);box-shadow:2px 2px 6px rgba(0,0,0,0.3);border-radius:6px;border:none;" id="_ub_res"></textarea>`,
          { lock: true }
        );
        const sleep = (delay) =>
        ...
        let s = 0,
          f = 0;
        let a = document.querySelector("#_ub_res");
        while (true) {
          let i = 0;
          timeout = 0;
          var res = await fetch(
            `https://ubits.club/getusertorrentlistajax.php?userid=${userid}&type=seeding&page=${page}`
          );
          res = await res.text();
          ...
          for (let t of torrents_link) {
            claimTorrent(t).then((res) => {
              ...
              i++;
              document.querySelector("#_ub_label").innerText = `正在认领...第${
                page + 1
...
async function claimCurrentPage() {
  ...
  xtip.confirm(
    `确定${is_remove ? "取消" : ""}认领本页 ${torrents_link.length} 个种子吗？`,
    {
      ...
      btn1: async () => {
        let loadid = xtip.load(
          `<p id="_ub_label" style="margin-top:10px;color:black;font-weight:bold;font-size:1.3em;text-shadow:0px 2px 5px rgba(255,255,255,0.6);">正在${
            is_remove ? "取消" : ""
          }认领...</p>`,
          { lock: true }
        );
