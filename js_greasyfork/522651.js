// ==UserScript==
// @name         PTerClub Torrent Checker 先行版
...
// @downloadURL https://update.greasyfork.org/scripts/522651/PTerClub%20Torrent%20Checker%20%E5%85%88%E8%A1%8C%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/522651/PTerClub%20Torrent%20Checker%20%E5%85%88%E8%A1%8C%E7%89%88.meta.js
// ==/UserScript==
/*
1.0.22: 修正片名中带 WEB 文字时无法分析标题的 bug，修正 ADWEB 的 NFO 识别，修正国内片子的国粤语标签判断。优化 DVD 的标题判断，优化 AV1 的质量判断逻辑。增加播出日期、AV3A 等编码的识别，增加合作组提示，增加第二种图床（已关闭）黑名单，增加 REPACK 的识别提示。
1.0.21: 增加多余文件 pad 的判断，增加图床黑名单的判断
...
//             }
    ...
    if (TORRENT_INFO.results.title.match(/Blu-?ray/i)) {
        ...
        //获取标题媒介3
    } else if (TORRENT_INFO.results.title.match(/WEB-DL/i)) {//无法在此处理媒介为 WEB 的问题
        ...
        if (infosp.match(/(参.*数.*:|中上英下).*/i)) {//CMCT
            ...
            match.forEach((r) => {
                if (r.match(/(帧.*率|Frame.?Rate).*/i)) {
                    ...
                    if (r.match(/(帧.*率|Frame.?Rate).*23.*/i)) {
                        ...
                        TORRENT_INFO.mediainfo.video.fps = '120FPS';
                    } else if (!r.match(/(帧.*率|Frame.?Rate).*\)\sFPS/i)){
            ...
            match.forEach((r) => {
                ...
                if (r.match(/视频编码/i)) {
                    if (r.match(/HEVC/i)) {
                        ...
                        TORRENT_INFO.mediainfo.video.format = 'AVC';
                    } else if (r.match(/C?AVS[23]?/i)) {
                        TORRENT_INFO.mediainfo.video.format = r.match(/C?AVS[23]?/i)[0];
                    }
                    if (r.match('Main 10')) {
                        TORRENT_INFO.mediainfo.video.bitdepth = '10';
                    }
                ...
                if (r.match(/音频编码/i)) {
                    ...
                    if (r.match(/AV3A/i)) {
                        audio_x.format = 'AV3A';
                    } else if (r.match(/Dolby TrueHD/i)) {
                        audio_x.format = 'TrueHD';
                    } else if (r.match(/DTS-HD MA/i)) {
                        audio_x.format = 'DTS-HD MA';
                    } else if (r.match(/FLAC/i)) {
                    ...
                    if (r.match(/[1-7]\.[01](\.[4])?/)) {
                        audio_x.channels = r.match(/[1-7]\.[01](\.[4])?/)[0];
            ...
            match.forEach((r) => {
                ...
                if (r.match(/(视频信息|VIDEO.CODEC|ViDEO BiTRATE).*/i)) {//HDH:ViDEO BiTRATE
                    if (r.match(/x26[45].*Base\sMedia/i)) {
                    } else if (r.match(/x264/i)) {
                        TORRENT_INFO.mediainfo.video.format = 'AVC';
                        TORRENT_INFO.mediainfo.video.codec = 'x264';
                ...
                if (r.match(/Audio.*(kbps|Mbps).*/i)) {
                    ...
                    if (r.match(/TrueHD/i)) {
                        ...
                        audio_x.format = 'AAC';
                    } else if (r.match(/E-AC-3|DD Plus/i)) {
                        audio_x.format = 'DDP';
                    } else if (r.match(/(AC3|DD|Dolby Digital Audio)/i)) {
                        audio_x.format = 'DD';
                    }
