...
// @run-at       document-end
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/477793/%F0%9F%94%A5%E6%8B%93%E5%B1%95%E5%A2%9E%E5%BC%BA%F0%9F%94%A5%E5%A6%96%E7%81%AB%E7%BD%91%E6%8F%92%E4%BB%B6%203194%E2%86%91%20%E5%A2%9E%E5%8A%A0%E5%8A%9F%E8%83%BD.user.js
// @updateURL https://update.greasyfork.org/scripts/477793/%F0%9F%94%A5%E6%8B%93%E5%B1%95%E5%A2%9E%E5%BC%BA%F0%9F%94%A5%E5%A6%96%E7%81%AB%E7%BD%91%E6%8F%92%E4%BB%B6%203194%E2%86%91%20%E5%A2%9E%E5%8A%A0%E5%8A%9F%E8%83%BD.meta.js
// ==/UserScript==
...
(function () {
  ...
  // 等级和对应称号同时相似 大郎增强
  MY_addStyle(`.dengji,.touxian{display:unset !important;} .dengji::after{content:"丨";}`);

  // https://yaohuo.me/bbs-1312684.html
  MY_addStyle(`.list {column-count: 2;column-gap: 20px; /* 可以根据需要调整列之间的间距 */}.list {width: 100%;box-sizing: border-box;}`);

  MY_addStyle(`.plus-one-btn{color: rgb(255, 255, 255); padding: 2px 0px 2px 4px; font-size: 12px; margin: 0px 2px 0px 5px; background-color: rgb(204, 204, 204); border-radius: 10%; cursor: pointer; display: inline-block; height: 16px; line-height: 17px; width: 40px;}`)

  // H5移动端禁止链接点击出现蓝色背景 大郎增强
  MY_addStyle(`*{-webkit-tap-highlight-color: rgba(0,0,0,0);} textarea{word-break: break-all;}`);
  ...
  let settingData = {
    ...
    loadNextPageType: isMobile() ? "more" : "more",
    ...
    settingIconResponsiveSize: 14,
    // 按钮最大的大小，单位px，默认100px
    settingIconMaxSize: 65,
  ...
  */
  ...
  let addEventAry = [
    ...
    {
      id: "ubb_movie",
      ubb: "[movie=100%*100%]视频直链地址|https://tucdn.wpon.cn/2024/04/30/04e178483a8f1.gif[/movie]",
      offset: 8,
    },
    {
      id: "ubb_nzgsa",
    ...
    {
      id: "ubb_nzsgtc",
      ubb: "[audio=X]https://aweme.snssdk.com/aweme/v1/play/?video_id=v0d00fg10000cco09b3c77ufcav057og&ratio=1080p[/audio]",
      offset: 0,
    },
    {
      // 大朗增强
    ...
    {
      id: "ubb_bjdy",
      ubb: "[audio=X]https://aweme.snssdk.com/aweme/v1/play/?video_id=v0300fg10000c39llkp400egrpaivp30&ratio=1080p[/audio]",
      offset: 0,
    },
    {
      id: "ubb_xngg",
      ubb: "[audio=X]https://aweme.snssdk.com/aweme/v1/play/?video_id=v0200ff10000bop9e9evld7780im485g&ratio=1080p[/audio]",
      offset: 0,
    },
    {
      id: "ubb_tbs",
      ubb: "[audio=X]https://aweme.snssdk.com/aweme/v1/play/?video_id=v0300fg10000c27pgb9j1vdnlib21bmg&ratio=1080p[/audio]",
      offset: 0,
    },
    {
      id: "ubb_jb",
      ubb: "[audio=X]https://aweme.snssdk.com/aweme/v1/play/?video_id=v0200fg10000clgc3l3c77u37ec4o06g&ratio=1080p[/audio]",
      offset: 0,
    },
    {
      id: "ubb_sscd",
      ubb: "[audio=X]https://aweme.snssdk.com/aweme/v1/play/?video_id=v0d00fg10000cl3317rc77uboqgts6s0&ratio=1080p[/audio]",
      offset: 0,
    },
    {
      id: "ubb_ngm",
      ubb: "[audio=X]https://aweme.snssdk.com/aweme/v1/play/?video_id=v0300fg10000ckbjafrc77uc4heo85r0&ratio=1080p[/audio]",
      offset: 0,
    },
    {
      id: "ubb_lyltc",
    ...
    {
      id: "ubb_ls",
      ubb: "[audio=X]https://aweme.snssdk.com/aweme/v1/play/?video_id=v0300fg10000ckei8ejc77u3qug3fnc0&ratio=1080p[/audio]",
      offset: 0,
    },
  ];
  ...
  const spanstyle = "color: #fff; padding: 2px 4px; font-size: 14px; margin: 0 2px; background-color: #ccc; border-radius: 10%; cursor: pointer;";
  const a2style =
  ...
  // ==通知楼主==
  function tongzhi_up() {
    let tzup = document.getElementById('cbx');
    if (tzup && document.querySelector('[name=myuserid]').value != document.querySelector('[name=touserid]').value) tzup.checked = true;
  }
  // ==阻止==
  ...
  function initSetting() {
    ...
    // 查看本地是否存在旧数据  大郎增强→让设置存储失效
    //if (!0 || !yaohuo_userData) {
    if (!yaohuo_userData) {
            ...
            <ul>
              ...
              <li>
                ...
                <select data-key="imageBedType" id="imageBedType">
                  ...
                  <option value="限时图床">限时图床(限时删除)</option>
                </select>
              </li>
              <li>
      ...
      throttle(() => {
        ...
        if (isPage) {
          ...
          if (loadNextPageType === "more" || !nextPageWrap) {
            // 加载更多加载下一页
            nextBtn = document.querySelector("span[id$=show_tip]") || document.querySelector("span[id=ual_status_text]");
  ...
  function handleAddNewPostUBB() {
    if (postPage.includes(window.location.pathname.toLowerCase()) && isAddNewPostUBB) {
          ...
          <div class='more_ubb_tools' style='display: none'>
            ...
            <div class='btBox'>
                <div class='bt2'>
                    ...
                    <a style='width:25%' href='https://695402.xyz/dy' target="_blank">抖音解析</a>
                    <a style='width:25%' href='https://695402.xyz/ks' target="_blank">快手解析</a>
                </div>
            </div>
            <div class='btBox'>
              <div class='bt2'>
                <a style='width:25%' href='https://aapi.eu.org/ppx' target="_blank">皮皮虾解析</a>
                <a style='width:25%' href='https://695402.xyz/bili' target="_blank">b站解析</a>
  ...
  function handleAddReplyUBB() {
    if (
      ...
      fileTag.insertAdjacentHTML(
        "afterend",
        `<input id="ubb_unfold" type="submit" value="${isUnfoldUbb ? "折叠UBB" : "展开UBB"
        }" style="float:right"/>`
      );
      ...
      form.insertAdjacentHTML(
        ...
        `
          ...
          <div class="ubb_wrap" style="text-align: center;overflow: hidden;">
            ...
            <br>
            <span id='ubb_nzgsa' style="${a2style}">该死</span>
            <span id='ubb_nzsgtc' style="${a2style}">天才</span>
            <span id='ubb_bjdy' style="${a2style}">卧槽</span>
            <span id='ubb_xngg' style="${a2style}">不信</span>
            <span id='ubb_tbs' style="${a2style}">大叫</span>
            <span id='ubb_ls' style="${a2style}">乐色</span>
            <span id='ubb_sscd' style="${a2style}">插兜</span>
            <span id='ubb_ngm' style="${a2style}">坤坤</span>
            <span id='ubb_jb' style="${a2style}">举报</span>
            <br>
            <span id='ubb_text' style="${spanstyle}">半角</span>
            <span id='ubb_br' style="${spanstyle}">换行</span>
            <span id='ubb_b' style="${spanstyle}">加粗</span>
            <span id='ubb_color' style="${spanstyle}">彩字</span>
            ...
            <br>
            <span id='ubb_del' style="${spanstyle}">清空输入框</span>   <!--大郎增强-->
            <br>
          </div>
          <hr>
          `
  ...
  // 在 handleAddReplyColorful 函数中添加彩字按钮
  function handleAddReplyColorful() {
    // 找到快速回复按钮
    const quickReplyBtn = document.querySelector('.kuaisuhuifu input[name="g"]');
    if (quickReplyBtn) {
        // 添加彩字按钮
        quickReplyBtn.insertAdjacentHTML(
        'afterend',
        `<input type="submit" id="colorful_reply" value="彩字回复" style="margin-left: 5px;">`
        );

        // 获取原始的提交事件
        const originalOnclick = quickReplyBtn.onclick;

        // 处理彩字回复点击事件
        document.getElementById('colorful_reply').addEventListener('click', (e) => {
            const textarea = document.querySelector('.retextarea');
            ...
            // 定义彩虹色数组
const colors = [
    '#FF0000', '#FF7F00', '#00FF00',
    '#00FFFF', '#0000FF', '#8B00FF', '#FF69B4',
    '#FF1493', '#FF6347', '#FFD700', '#32CD32',
    '#00CED1', '#1E90FF', '#9370DB', '#FF00FF'
];
...
chars.forEach((char, index) => {
    // 跳过已有的 UBB 标签（处理未闭合标签的情况）
    if (char === '[') {
        const ubbEnd = content.indexOf(']', index);
        if (ubbEnd !== -1) {
            // 直接拼接整个UBB标签（包括起始和结束符号）
            colorfulContent += content.substring(index, ubbEnd + 1);
            // 跳过已处理的标签部分（避免重复处理标签内的字符）
            return;
        } else {
            // 处理未闭合的UBB标签（直接拼接当前字符）
            colorfulContent += char;
            return;
        }
    }

    // 普通字符：随机选取颜色（允许重复，自然循环）
    const randomIndex = Math.floor(Math.random() * colors.length);
    colorfulContent += `[forecolor=${colors[randomIndex]}]${char}[/forecolor]`;
});

            // 更新文本框内容
            textarea.value = colorfulContent;
            // 调用原始的提交事件
            if (originalOnclick) {
                originalOnclick.call(quickReplyBtn, e);
            } else {
                quickReplyBtn.click();
            }
        });
  ...
  function handleAddReplyFace() {
    if (
      ...
      sendmsg.insertAdjacentHTML(
        "afterend",
        `<span
            style="${spanstyle}margin-left: 2px; display:${isUnfoldFace ? "display: block" : "display: none"
        }" id="unfold"
            >表情${isUnfoldFace ? "折叠" : "展开"}</span>`
      );

      //  添加表情包预览框 大郎增强
      document.querySelector('#unfold').insertAdjacentHTML(
        "afterend",
        `<img id="bq_preview" style="max-height:26px;vertical-align:sub;margin-left: 6px;" />`
      );
      ...
      facearea.onclick = function (event) {
        if (event.target.tagName.toLowerCase() === "img") {
          ...
          document.querySelector(`#faceselect .select_options`).style.display = "block";  // 大郎增强

          if (diySrc) {
            document.querySelector('#bq_preview').setAttribute('src', diySrc);    // 大郎增强

            const emojiTag = `[img]${diySrc}[/img]`;
            textarea.focus();
            ...
            // 重新设置光标位置
            const newPos = startPos + emojiTag.length;
            textarea.focus();
            ...
            document.querySelector(`input.select_input[value="${val}"]`).checked = true;
            document.querySelector('#bq_preview').setAttribute('src', event.target.getAttribute("src"));
  ...
  function handleUploadImage() {
    if (isUploadImage) {
      ...
      let isReplyPage = isBbsViewPage || viewPage.includes(window.location.pathname);
      ...
      async function uploadFile(file) {
        let uploadConfig = {
          ...
          限时图床: {
            url: "https://p.921069.xyz/api/image/upload",
            name: "file"
          }
        };
        let {
        ...
        try {
          ...
          let { code, url, data, msg, directUrl } = res;

          if (code === 200 || code === 0 || url || directUrl) {
  ...
  function handleAddLoadMoreBtnClick() {
    ...
    if (isPage) {
      ...
      loadMoreBtn?.addEventListener("click", (e) => {
            ...
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
              ...
              mutation.addedNodes.forEach(node => {
                if (node.nodeType === 1) {  // 元素节点
                  ...
                  [...newImages, ...newVideos].forEach(handleMedia);
        ...
        // 开始观察目标节点
        recontent && observer.observe(recontent, {
          childList: true,
          subtree: true
        });
      });
  ...
  function handleEventListener(id, textarea, ubb, offset) {
    document.getElementById(id)?.addEventListener("click", (e) => {
      if (id === "ubb_nzgsa") {
        ...
        //   alert("不要无意义灌水啦！");
        // }
      } else if (id === "ubb_nzsgtc") {
        insertText(
          textarea,
          "[audio=X]https://aweme.snssdk.com/aweme/v1/play/?video_id=v0d00fg10000cco09b3c77ufcav057og&ratio=1080p[/audio]",
          0
        );
      }
      else if (id == "ubb_del") {      // 大郎增强 清空输入框
        textarea.value = "";
      } else {
        e.preventDefault();
  ...
  function ReAdd1() {
   ...
   // 添加+1按钮的函数
function addPlusOneBtn() {

    // 获取父容器 .recontent
    const recontentDiv = document.querySelector('.recontent');
    if (!recontentDiv) return;

    // 为父容器添加一次性的事件委托
    if (!recontentDiv.hasAttribute('data-plus-one-initialized')) {
        recontentDiv.setAttribute('data-plus-one-initialized', 'true');

        recontentDiv.addEventListener('click', (e) => {
            // 检查点击的是否是"回复+1"按钮
            if (e.target.classList.contains('plus-one-btn')) {
                const item = e.target.closest('span.retext');
                if (!item) return;
                ...
                content = content.replace(/<span class="plus-one-btn">.*?<\/span>/g, '');
                ...
                links.forEach(link => {
                    ...
                    link.outerHTML = `[url=${href}]${text}[/url]`;
                });
                ...
                videos.forEach(video => {
                    ...
                    const poster = video.poster || 'https://tucdn.wpon.cn/2024/04/30/04e178483a8f1.gif'; // 默认封面
                    video.outerHTML = `[movie=100%*100%]${videoSrc}|${poster}[/movie]`;
                });
                ...
                audios.forEach(audio => {
                    ...
                    audio.outerHTML = `[audio=X]${audioSrc}[/audio]`;
                });
    ...
    // 只添加按钮，不绑定事件
    var allreplys = document.querySelectorAll("span.retext");
    allreplys.forEach(item => {
        if (!item.querySelector('.plus-one-btn')) {
            var add1Elem = document.createElement("span");
            add1Elem.className = 'plus-one-btn';
            add1Elem.innerText = "回复+1";
            item.appendChild(add1Elem);
        }
    });
}
  ...
  function addTopButton() {
    ...
    commentBtn.id = 'to-comment';
    commentBtn.className = 'float-btn';
    ...
    commentBtn.addEventListener('click', () => {
      // 获取评论区元素
      const commentArea = document.querySelector('.recontent');
      if (commentArea) {
    ...
    window.addEventListener('scroll', throttle(() => {
      // 获取评论区位置
      const commentArea = document.querySelector('.recontent');
      const commentTop = commentArea?.getBoundingClientRect().top;
  ...
  function handleMedia(media) {
    if (!media.style.maxHeight) {  // 避免重复处理
      // 设置最大预览尺寸
      const maxPreviewSize = /Mobi|Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ? 580 : 680;
      media.style.maxHeight = `${maxPreviewSize}px`;


      if (media.parentNode.tagName === 'A') {
        media.parentNode.onclick = (e) => {
          e.preventDefault();
          return false;
        };
      }
      ...
      media.onclick = function (e) {
        ...
        if (this.style.maxHeight) {
          ...
          this.style.maxHeight = `${maxPreviewSize}px`;
          const offTop = offSet(this).top;
          window.scrollTo({
            top: offTop + 50,
            behavior: "smooth"
          });
  ...
  // 修改 handleImageEnhance 函数
  function handleImageEnhance() {
    // 添加视频样式
    MY_addStyle(`.content video{max-width: max-content;}`);

    // 处理所有媒体元素的函数
    function processAllMedia() {
      const contentImages = document.querySelectorAll('.content img');
      const recontentImages = document.querySelectorAll('.recontent img');
      const recontentVideos = document.querySelectorAll('.content video');

      const allMedia = [...contentImages, ...recontentImages, ...recontentVideos];
      allMedia.forEach(handleMedia);
    }
  ...
  function handleSendBtn(){
    const textarea = document.querySelector('.retextarea');
    const sendBtn = document.querySelector('.kuaisuhuifu>input[name=g]');
