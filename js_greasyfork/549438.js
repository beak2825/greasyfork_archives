// ==UserScript==
// @name         Bing Rewards Auto Searcher (Custom Merge) — Type-Only Submit + Auto-Start + Pre-Start Delay
// @namespace    brx-custom
// @version      0.4.2
// @description  Added customizable pre-start delay countdown on auto-start only. All previous features preserved, typing-only submit with real URLs, auto-scroll, random iframe link, cooldowns, single-tab lock, UI. Runs on bing.com and cn.bing.com.
...
// @grant        GM_setValue
// @grant        GM_addStyle
// @downloadURL https://update.greasyfork.org/scripts/549438/Bing%20Rewards%20Auto%20Searcher%20%28Custom%20Merge%29%20%E2%80%94%20Type-Only%20Submit%20%2B%20Auto-Start%20%2B%20Pre-Start%20Delay.user.js
// @updateURL https://update.greasyfork.org/scripts/549438/Bing%20Rewards%20Auto%20Searcher%20%28Custom%20Merge%29%20%E2%80%94%20Type-Only%20Submit%20%2B%20Auto-Start%20%2B%20Pre-Start%20Delay.meta.js
// ==/UserScript==
...
(function() {
  ...
  const DEFAULTS = {
    autoStart: true,
    preStartDelay: 6,                // seconds before 1st search on auto-start
    autoScroll: true,
    openRandomLink: true,
    autoCooldown: true,
    cooldownThreshold: 3,
  ...
  const SEARCH_INPUT_SELECTORS = ['#sb_form_q','.b_searchbox','input[name="q"]','#searchboxinput','textarea[name="q"]'];
  const RESULTS_READY_SELECTORS = ['ol#b_results','#b_results','main#b_content','.b_results','li.b_algo'];
  ...
  const rewards = { current:0, total:0, lastChecked:0, completed:false, noProgressCount:0 };
  const run = { done:0, target:capToDevice(settings.totalSearches), startedAt:0 };
  ...
  ui.btnStartStop.addEventListener('click', () => {
    if (!running) startRun(false); else stopRun('Stopped by user');
  });
  ...
  ui.chkAutoStart.addEventListener('change', () => {
    settings.autoStart = ui.chkAutoStart.checked;
    saveSettings();
  });
  ui.inPreStartDelay.addEventListener('change', () => {
    const val = clampInt(ui.inPreStartDelay.value, 0, 60);
    settings.preStartDelay = val;
    ui.inPreStartDelay.value = val;
    saveSettings();
  });
  ui.chkAutoScroll.addEventListener('change', () => { settings.autoScroll=ui.chkAutoScroll.checked; saveSettings(); });
  ui.chkOpenRandom.addEventListener('change', () => { settings.openRandomLink=ui.chkOpenRandom.checked; saveSettings(); });
  ui.chkCooldown.addEventListener('change', () => { settings.autoCooldown=ui.chkCooldown.checked; saveSettings(); });
  ...
  async function autoStartIfEnabled() {
    ...
    if (!running) {
      if (acquireLock()) {
        if(settings.preStartDelay > 0) {
          await countdown(settings.preStartDelay,'Starting');
        }
        startRun(true);
      } else {
        updateStatus('Another Bing tab active. Auto-start waiting for lock...');
        const t0=Date.now();
        const poll= setInterval(() => {
          if(running) { clearInterval(poll); return; }
          if(Date.now()-t0 > 20000) { clearInterval(poll); return; }
          if(acquireLock()) {
            clearInterval(poll);
            if(settings.preStartDelay > 0) {
              countdown(settings.preStartDelay,'Starting').then(() => startRun(true));
            } else { startRun(true); }
          }
        },1500);
      }
  ...
  async function startRun(isAuto=false) {
    if (!hasLock() && !acquireLock()) {
      updateStatus('Active lock not available; start aborted.');
      return;
    }
    running=true;
    run.done=0;
    run.target=capToDevice(settings.totalSearches);
    rewards.current=0; rewards.total=0; rewards.lastChecked=0; rewards.completed=false; rewards.noProgressCount=0;
  ...
  async function nextCycle() {
    ...
    if(!input) {
      stopRun('Search box not found. Open a Bing search or homepage.');
      return;
    }
  ...
  function buildUI() {
      ...
      <div id="brx-body">
        ...
        <div id="brx-settings">
          <div class="row">
            ...
            <input type="checkbox" id="brx-auto-start">
          </div>
          <div class="row">
            <label>Pre-Start Delay (sec)</label>
            <input type="number" id="brx-prestart-delay" min="0" max="60" step="1">
          </div>
          <div class="row">
    ...
    const chkAutoStart = box.querySelector('#brx-auto-start');
    const inPreStartDelay = box.querySelector('#brx-prestart-delay');
    ...
    inPreStartDelay.value = settings.preStartDelay;
    ...
    return {
      ...
      chkAutoStart, inPreStartDelay, chkAutoScroll, chkOpenRandom, chkCooldown,
  ...
  function hasLock() {
    try {
      ...
      return obj && obj.id === instanceId && Date.now() - obj.ts < 20000;
    } catch { return false; }
  }
  ...
  function loadSettings() {
    ...
    s.preStartDelay = clampInt(s.preStartDelay || 6, 0, 60);
