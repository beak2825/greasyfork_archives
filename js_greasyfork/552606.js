// ==UserScript==
// @name         Trophy Manager - R5 + Filtro + Status + NT
// @namespace    http://tampermonkey.net/
// @version      16.1
// @description  Carrega dados de 15 temporadas, filtra jogadores por país, calcula o R5, cria lista de melhores por idade, marca jogadores NT e times inativos/banidos.
...
// @match        https://trophymanager.com/national-teams/*/nt-shortlist/*
// @grant        GM_xmlhttpRequest
// @grant        GM_setValue
// @grant        GM_getValue
// @connect      trophymanager.com
// @downloadURL https://update.greasyfork.org/scripts/552606/Trophy%20Manager%20-%20R5%20%2B%20Filtro%20%2B%20Status%20%2B%20NT.user.js
// @updateURL https://update.greasyfork.org/scripts/552606/Trophy%20Manager%20-%20R5%20%2B%20Filtro%20%2B%20Status%20%2B%20NT.meta.js
// ==/UserScript==

(function() {
    'use strict';

    // --- SEÇÃO PARA SALVAR SHORTLIST NT ---
    if (window.location.href.includes('nt-shortlist')) {
        const btn = document.createElement('button');
        btn.textContent = '💾 Salvar Lista NT';
        btn.style.cssText = "position: fixed; top: 10px; left: 10px; z-index: 9999; padding: 6px; background: #337ab7; color: white; border: none; border-radius: 4px; cursor: pointer;";
        btn.addEventListener('click', () => {
            const rows = document.querySelectorAll('table.zebra.hover tbody tr');
            const listaNT = [];

            rows.forEach(row => {
                const link = row.querySelector('td:first-child a[href*="/players/"]');
                if (link) {
                    const match = link.href.match(/\/players\/(\d+)/);
                    if (match) listaNT.push(match[1]); // Salva ID
                }
            });

            GM_setValue('listaNT', JSON.stringify(listaNT));
            alert(`✅ Lista NT salva com ${listaNT.length} jogadores!`);
        });
        document.body.appendChild(btn);
        return; // Para não executar o resto do script nesta página
    }

    // --- SEÇÃO DE ESTILOS GLOBAIS ---
    const styles = `
        #tm-best-players-panel {
            position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
            width: 450px; max-height: 80vh; overflow-y: auto; background-color: #2d2d2d;
            border: 1px solid #555; border-radius: 8px; z-index: 10001;
            color: #f1f1f1; box-shadow: 0 5px 15px rgba(0,0,0,0.5);
        }
        #tm-best-players-panel h2 {
            margin: 0; padding: 12px; background-color: #3a3a3a;
            font-size: 1.1em; text-align: center; border-bottom: 1px solid #555;
        }
        #tm-best-players-panel h3 {
            margin: 10px 12px 5px; font-size: 1em; color: #4CAF50;
            border-bottom: 1px solid #444; padding-bottom: 4px;
        }
        #tm-best-players-panel ol {
            margin: 0 12px 15px 30px; padding: 0;
        }
        #tm-best-players-panel li {
            margin-bottom: 6px;
        }
        #tm-best-players-panel a {
            color: #8ab4f8; text-decoration: none;
        }
        #tm-best-players-panel a:hover {
            text-decoration: underline;
        }
        #tm-best-players-panel strong {
            color: #f1f1f1; font-weight: bold;
        }
        #tm-best-players-panel .nt-player {
            color: #00FF00;
            font-weight: bold;
        }
        #tm-close-panel {
            position: absolute; top: 8px; right: 10px; background: transparent;
            border: none; color: #ccc; font-size: 1.4em; cursor: pointer;
        }
    `;
    ...
    // --- PESO E FUNÇÕES DE CÁLCULO DE R5 ---
    ...
    let ratingType = 'R5';
    let bestPlayersData = []; // Armazena dados dos jogadores para a lista de melhores

    async function getFullPlayerInfo(playerID) {
    ...
    async function calculateR5ForFiltered() {
        bestPlayersData = []; // Limpa a lista antes de um novo cálculo
        const listaNT = JSON.parse(GM_getValue('listaNT', '[]'));
        ...
        for (const table of tables) {
            ...
            for (let i = 0; i < rows.length; i += batchSize) {
                ...
                const promises = batch.map(async (row) => {
                    ...
                    // Aplica a marcação visual [NT] na tabela principal
                    const isNT = listaNT.includes(playerID);
                    if (isNT) {
                        if (!playerLink.textContent.startsWith('[NT]')) {
                           playerLink.textContent = `[NT] ` + playerLink.textContent;
                        }
                        playerLink.style.color = '#00FF00';
                    }
                    ...
                    try {
                        ...
                        // Define o nome do jogador para a lista, adicionando [NT] se necessário
                        let playerNameForPanel = playerInfo.name;
                        if (isNT) {
                            playerNameForPanel = `[NT] ${playerInfo.name}`;
                        }

                        bestPlayersData.push({
                            id: playerID,
                            name: playerNameForPanel, // Usa a variável com o nome corrigido
                            age: playerInfo.age,
                            r5: highestR5,
                            link: playerLink.href,
                            isNT: isNT // Adiciona um campo para facilitar a estilização
                        });
    ...
    function mostrarTodos() {
        ...
        bestPlayersData = []; // Limpa a lista ao remover o filtro
    }

    function displayBestPlayersPanel() {
        const existingPanel = document.getElementById('tm-best-players-panel');
        if (existingPanel) {
            existingPanel.remove();
            return;
        }

        if (bestPlayersData.length === 0) {
            alert('Nenhum dado de jogador para exibir. Por favor, filtre e calcule o R5 primeiro.');
            return;
        }

        const groupedByAge = bestPlayersData.reduce((acc, player) => {
            (acc[player.age] = acc[player.age] || []).push(player);
            return acc;
        }, {});

        const sortedAges = Object.keys(groupedByAge).sort((a, b) => a - b);
        sortedAges.forEach(age => {
            groupedByAge[age].sort((a, b) => b.r5 - a.r5);
        });

        const panel = document.createElement('div');
        panel.id = 'tm-best-players-panel';

        let content = `<h2>🏆 Melhores R5 por Idade</h2><button id="tm-close-panel">×</button>`;
        sortedAges.forEach(age => {
            content += `<h3>Idade: ${age}</h3>`;
            content += '<ol>';
            groupedByAge[age].forEach(player => {
                const ntClass = player.isNT ? 'class="nt-player"' : '';
                content += `<li><a href="${player.link}" target="_blank" ${ntClass}>${player.name}</a> - <strong>R5: ${player.r5}</strong></li>`;
            });
            content += '</ol>';
        });
        ...
        document.getElementById('tm-close-panel').addEventListener('click', () => panel.remove());
    }
    ...
    function criarInterfaceFiltro() {
        ...
        topRow.style.cssText = "display: flex; gap: 10px; width: 100%;";
        ...
        loadSeasonsButton.style.cssText = "padding: 5px 10px; color: #fff; border: none; border-radius: 3px; cursor: pointer; background-color: #337ab7; flex-grow: 1;";
        loadSeasonsButton.addEventListener('click', () => carregarTemporadasAnteriores(isFiltered));

        const showBestButton = document.createElement('button');
        showBestButton.id = 'tm-show-best-button';
        showBestButton.textContent = '🏆 Melhores por Idade';
        showBestButton.style.cssText = "padding: 5px 10px; color: #fff; border: none; border-radius: 3px; cursor: pointer; background-color: #6f42c1;";
        showBestButton.addEventListener('click', displayBestPlayersPanel);

        topRow.appendChild(loadSeasonsButton);
        topRow.appendChild(showBestButton);
        ...
        countryInput.placeholder = 'ex: ao';
        ...
        filterButton.style.cssText = "padding: 5px 10px; color: #fff; border: none; border-radius: 3px; cursor: pointer; min-width: 150px;";
        const updateButtonText = () => {
            const countryCode = countryInput.value.toUpperCase();
            filterButton.textContent = `Filtrar de ${countryCode}`;
        };
