// ==UserScript==
// @name         CustomChat H&M V1
// @namespace    http://hackandmodz.net/
// @version      1.1
// @description  enter something useful
// @include     http://hackandmodz.net/
// @include     http://www.hackandmodz.net/
// @include     http://hackandmodz.fr/taigachat/
// @include     http://hackandmodz.fr/taigachat/
// @include     http://www.hackandmodz.net/taigachat/
// @include     http://www.hackandmodz.net/taigachat/
// @author       Wayz / Adapted and modified by Marentdu93
// @match        http://hackandmodz.net/
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/12235/CustomChat%20HM%20V1.user.js
// @updateURL https://update.greasyfork.org/scripts/12235/CustomChat%20HM%20V1.meta.js
// ==/UserScript==
// CustomChat v1 by Wayz / Adapted and modified by Marentdu93 !
...
var customChat = {
	...
	template_Button: '<div id="customChat_op" style="font-style:none;cursor:pointer;" class="fa fa-bars"></div>',
	template_Html: '<div id="customChat_op" data-role="blk" style="width:100%;height:100%;position:fixed;z-index:2147483645;background-color:rgba(0,0,0,0.2);opacity:0;display:none;"></div><div style="right: -800px;position: fixed;" class="customChat"><div class="customChat_header" style="background-color:#2F78A0"><div class="customChat_close" style="background-image:url()"></div><div class="customChat_header_text" style="margin-top: 40px;">CustomChat</div><div class="customChat_header_message">by Wayz</div><div class="customChat_header_message">Adapted and modified By Marentdu93</div></div><div class="customChat_middle_wrapper"><div class="customChat_middle"><div><input class="customChat_btn" id="hello" type="button"/><label for="hello"> Hello ! </label></div><br/><div><input class="customChat_btn" id="Good" type="button"/><label for="Good"> Good Night ! </label></div></br><div><input class="customChat_btn" id="rep_pred" type="button"/><label for="rep_pred"> Message prédéfini </label></div><br/><div><input class="customChat_cb" id="notif" type="checkbox" checked=""/><label for="notif"><span class="ui"></span> Activer les notifications </label></div><br/><br/><div><input class="customChat_cb" id="bot" type="checkbox"/><label for="bot"><span class="ui"></span> Activer le pseudo Fondateur </label></div><br></br><div><input class="customChat_cb" id="admin" type="checkbox" checked=""/><label for="admin"><span class="ui"></span>  Activer le pseudo Admin  </label></div><br></br><div><input class="customChat_cb" id="ok" type="checkbox" checked=""/><label for="ok"><span class="ui"></span>  Activer le pseudo Partenaires  </label></div><br></br><div><input class="customChat_cb" id="VIP" type="checkbox" checked=""/><label for="VIP"><span class="ui"></span>  Activer le pseudo VIP </label></div><br></br><div><input class="customChat_cb" id="Assistant" type="checkbox" checked=""/><label for="Assistant"><span class="ui"></span>  Activer le pseudo Assistant </label></div></div></div></div><div class="customChat_reponse" style="right:-800px;"><div class="customChat_rep_wrapper"><ul class="customChat_pred"><li class="customChat_a"><a id="customChat_rP" data-msg="" data-num="1" href="javascript:void(0);">Message 1</a></li><li class="customChat_a"><a id="customChat_rP" data-msg="" data-num="2" href="javascript:void(0);">Message 2</a></li><li class="customChat_a"><a id="customChat_rP" data-msg="" data-num="3" href="javascript:void(0);">Message 3</a></li><li class="customChat_a"><a id="customChat_rP" data-msg="" data-num="4" href="javascript:void(0);">Message 4</a></li><li class="customChat_a"><a id="customChat_rP" data-msg="" data-num="5" href="javascript:void(0);">Message 5</a></li><li class="customChat_a"><a id="customChat_del-rP" href="javascript:void(0);">Supprimer un message</a></li><li class="customChat_a"><a id="customChat_timer-rP" href="javascript:void(0);">Dernier message envoyé il y a NaN</a></li></ul></div></div>',
	...
	init: function(){
		if(location.host == "dreamgaming.fr"){
			...
			return location.href = "http://hackandmodz.net/";
		}
        ...
        var $admin = $('#admin[type=checkbox]');
		$admin.on('change', function(){
			scc.admin = $(this).is(':checked');
			customChat.save();
		});
        var $VIP = $('#VIP[type=checkbox]');
		$VIP.on('change', function(){
			scc.VIP = $(this).is(':checked');
			customChat.save();
		});
        var $Assistant = $('#Assistant[type=checkbox]');
		$Assistant.on('change', function(){
			scc.Assistant = $(this).is(':checked');
			customChat.save();
		});
        var $ok = $('#ok[type=checkbox]');
		$ok.on('change', function(){
			scc.ok = $(this).is(':checked');
			customChat.save();
		});
	...
	load: function(){
        ...
        $('#admin[type=checkbox]').prop('checked', scc.admin);
        $('#VIP[type=checkbox]').prop('checked', scc.VIP);
        $('#Assistant[type=checkbox]').prop('checked', scc.Assistant);
        $('#ok[type=checkbox]').prop('checked', scc.ok);
		setInterval(function(){customChat.lastRP();}, 2000);
	},
	notify: function(msg) {
		if(scc.notif){
			if(!("Notification"in window)){console.error("Ce navigateur ne supporte pas les notifications !")}else if(Notification.permission==="granted"){var notification=new Notification(msg, {icon: 'http://hackandmodz.net/data/avatars/l/0/771.jpg?1395444758'})}else if(Notification.permission!=="denied"){Notification.requestPermission(function(e){if(!("permission"in Notification)){Notification.permission=e}if(e==="granted"){var t=new Notification(msg, {icon: 'http://hackandmodz.net/data/avatars/l/0/771.jpg?1395444758'})}})}
		}
		else {
	...
	checkNewMsg: function(){
		...
		for(var i in msg) {
			...
			if(this.msgID.indexOf(id) == -1 && msg_text.indexOf(this.username) != -1) {
				...
				$('#taigachat_box').children('ol').children('li[data-messageid="' + id + '"]').children('span').eq(1).children('a').children('span').attr('class', 'style30');
				this.__msgID.unshift(id);
			}
            else if(this.__msgID.indexOf(id) == -1 && msg_sender == this.username && scc.admin){
				$('#taigachat_box').children('ol').children('li[data-messageid="' + id + '"]').children('span').eq(1).children('a').children('span').attr('class', 'style3');
				this.__msgID.unshift(id);
			}
            else if(this.__msgID.indexOf(id) == -1 && msg_sender == this.username && scc.VIP){
				$('#taigachat_box').children('ol').children('li[data-messageid="' + id + '"]').children('span').eq(1).children('a').children('span').attr('class', 'style23');
				this.__msgID.unshift(id);
			}
            else if(this.__msgID.indexOf(id) == -1 && msg_sender == this.username && scc.Assistant){
				$('#taigachat_box').children('ol').children('li[data-messageid="' + id + '"]').children('span').eq(1).children('a').children('span').attr('class', 'style6');
				this.__msgID.unshift(id);
			}
            else if(this.__msgID.indexOf(id) == -1 && msg_sender == this.username && scc.ok){
				$('#taigachat_box').children('ol').children('li[data-messageid="' + id + '"]').children('span').eq(1).children('a').children('span').attr('class', 'style16');
				this.__msgID.unshift(id);
			}
		}
	},
	newMsg: function(id) {
		$('#taigachat_box').children('ol').children('li[data-messageid="' + id + '"]').css('backgroundColor', "#15445F");
    ...
    Good: function() {
		if((Date.now() - scc.lastH) >= 30){
			scc.lastH = Date.now();
			var user1 = $('#taigachat_box').children('ol').children('li').eq(5).children('span').eq(1).children('a').text();
			var user2 = $('#taigachat_box').children('ol').children('li').eq(17).children('span').eq(1).children('a').text();
			var user3 = $('#taigachat_box').children('ol').children('li').eq(23).children('span').eq(1).children('a').text();
			var userinchat = $('#taigachat_count').text();
			$("#taigachat_message").val("Bonne nuit à @" + user1 + " , @" + user2 + " , @" + user3 + " et aux " + userinchat + " membres actuellement sur le chat ;)");
			this.save();
		}
		else{
			alert('Vous avez utilisé cette fonction il y a moins de 30 sec !');
		}
	},
	lastRP: function(){
