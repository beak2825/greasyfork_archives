...
// @downloadURL https://update.greasyfork.org/scripts/420551/IdleScape%20-%20Lootify%20Beta.user.js
// @updateURL https://update.greasyfork.org/scripts/420551/IdleScape%20-%20Lootify%20Beta.meta.js
// ==/UserScript==
...
(function() {
    ...
    let initInterval = setInterval(()=> {
        if (window.D4IS) {
            ...
            lib = window.D4IS.init('lootify', '1.4.4');
            app = lib['lootify'];
    ...
    function main() {
            ...
            $.each(expTypes, function(k, type) {
                ...
                if (lib.user[type]) {
                    if(!lib.user.isFighting()) {
                        ...
                        $expTimeInfo.append($('<span>').text('to '+lib.user.levelGoal[type][skill]));
                        let $arrows = $('<div>', {
        ...
        function addLogEntry(entry, info) {
            ...
            let $logEntry = $('.log-entry-'+sanitizeEntry(entry));
            $.each(lib.general.sortObject(info), function(loot, lootinfo) {
                if (isNaN(lootinfo.count)) { return false; }
        ...
        function canSubmit(logs) {
            ...
            if (lib.user.getEnchantment('35') > 0) {
                can = false;
            }
