// ==UserScript==
// @name         Shikimori.one GIF inserter
// @namespace    http://tampermonkey.net/
// @version      2026-01-01.4
// @description  Come to Anime moments club 🎊 https://shikimori.one/clubs/5686-klub-anime-momentov 🎉! Add convenient Tenor.com gif image inserter into Shikimori.one comment editor. Initially a fork of "Gifs" script by Pirate~, but eventually rewritten entirely.
// @author       AlexDEV.pro
// @match        *://shikimori.one/*
...
// @grant        GM_setValue
// @grant        GM_getValue
// @downloadURL https://update.greasyfork.org/scripts/552074/Shikimorione%20GIF%20inserter.user.js
// @updateURL https://update.greasyfork.org/scripts/552074/Shikimorione%20GIF%20inserter.meta.js
// ==/UserScript==
...
(async function() {
    ...
    const lastUsedImagesStorageKey = 'sgiLastUsedImages';
    const insertOptionWidthPxStorageKey = 'sgiInsertOptionWidthPx';
    const displayOptionSquareViewStorageKey = 'sgiDisplayOptionSquareView';
    ...
    const i18n = {
        en: {
            ...
            dialogOpenButtonLegacyTitle: 'GIFs & stickers'
        },
        ru: {
            ...
            dialogOpenButtonLegacyTitle: 'Гифки и стикеры'
        }
    };

    const currentUserLocale = window.I18n?.locale || document.body.dataset.locale || window.LOCALE || 'en';
    const t = i18n[currentUserLocale] || i18n.en;

    const popupSpacingPx = 2;
    const popupSpacingLegacyPx = 5;
    ...
    let isLegacyEditorDetected = false;
    ...
    style.textContent = `
        ...
        @media(max-width: 375px) {
            #sgi-toolbar-container #sgi-search-input { width: 1em }
        }
        @media(max-width: 450px) {
            #sgi-toolbar-container #sgi-search-filter-select { width: 5em }
        }
    `;
    ...
    const initPopupElements = () => {
        ...
        popupContainerEl.id = 'sgi-popup-container';

        const toolbarContainerEl = document.createElement('div');
        toolbarContainerEl.id = 'sgi-toolbar-container';

        const searchGroupEl = document.createElement('div');
        searchGroupEl.className = 'sgi-left-border-stack';

        searchQueryPrefixSelectEl = document.createElement('select');
        searchQueryPrefixSelectEl.id = 'sgi-search-query-prefix-select';
        ...
        searchFilterSelectEl.id = 'sgi-search-filter-select';
        ...
        searchInputEl.id = 'sgi-search-input';
        ...
        searchClearButtonEl.id = 'sgi-search-clear-button';
        ...
        searchButtonEl.id = 'sgi-search-button';
        ...
        displayOptionsGroupEl.id = 'sgi-display-options-container';
        displayOptionsGroupEl.className = 'sgi-flex-wrap';
        ...
        isCoverObjectFitCheckboxLabelEl.className = 'sgi-checkbox-label';
        ...
        imagesContainerEl.id = 'sgi-images-container';
        imagesContainerEl.className = 'sgi-no-selection';
        if (!displayOptionSquareDefault) {
            imagesContainerEl.classList.add('sgi-object-fit-contain');
        }

        resultsContainerEl = document.createElement('div');
        resultsContainerEl.id = 'sgi-results-container';

        lastUsedImagesContainerEl = document.createElement('div');
        lastUsedImagesContainerEl.id = 'sgi-last-used-img-container';

        const loadMoreButtonContainerEl = document.createElement('div');
        loadMoreButtonContainerEl.id = 'sgi-load-more-button-container';

        loadMoreButtonEl = document.createElement('button');
        loadMoreButtonEl.id = 'sgi-load-more-button';
    ...
    const initButtonAndAnchorForEveryEditor = () => {
        let targets = getInjectionTargets();
        if (!targets.length) return; // No editor toolbars found on the current page, nothing to do here.

        // Ensure the targets are fully loaded.
        setTimeout(() => {
            for(const target of targets) {
                // Skip the target if it was processed before.
                //if (target.dataset.sgiProcessed) continue;
                if (target.querySelector('.sgi-popup-anchor')) continue;

                const popupAnchorEl = document.createElement('div');
                popupAnchorEl.className = 'sgi-popup-anchor';

                let buttonTargetEl = target.querySelector('.menu_group-block'); // New editor type.
                if (!buttonTargetEl) {
                    buttonTargetEl = target.querySelector('.shiki_editor-selector .controls .editor-controls'); // Old editor type.
                    if (buttonTargetEl) isLegacyEditorDetected = true;
                }
                if (buttonTargetEl) {
                    // Insert anchor element only if button target element is also found.
                    target.insertBefore(popupAnchorEl, target.firstChild);

                    const dialogOpenButtonEl = document.createElement('button');
                    dialogOpenButtonEl.className = 'icon';
                    ...
                    if (!isLegacyEditorDetected) {
                        dialogOpenButtonEl.classList.add('sgi-dialog-open-button');
                    } else {
                        dialogOpenButtonEl.classList.add('sgi-dialog-open-button-legacy');

                        // Add the same type of tooltip as the other buttons have.
                        if (typeof $ === 'function' && typeof $.fn.tipsy === 'function') {
                            $(dialogOpenButtonEl).tipsy({
                                gravity: 's',
                                title: function() {
                                    return t.dialogOpenButtonLegacyTitle;
                                }
                            });
                        }
                    }
                ...
                //target.dataset.sgiProcessed = true;
            }
        }, 0);
    }

    const getEditorEl = () => {
        return document.querySelector('.ProseMirror') // New editor type.
        || document.querySelector('.shiki_editor-selector textarea'); // Old editor type.
    }
...
></span>`;
    ...
    const onDialogOpenButtonClick = (popupAnchorEl) => {
        ...
        const elementToEnlarge = document.querySelector('.l-page') || document.body;
        if (popupContainerEl.style.display !== 'none') {
            const headerCalculatedHeightPx = document.querySelector('header')?.offsetHeight || 0;
            const spaceAboveTheAnchorPx = popupAnchorEl.getBoundingClientRect().top + window.scrollY - headerCalculatedHeightPx;
            const popupTopOffsetPx = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--sgi-popup-height'));

            let actualPopupSpacingPx;
            if (!isLegacyEditorDetected) {
                actualPopupSpacingPx = popupSpacingPx;
            } else {
                actualPopupSpacingPx = popupSpacingLegacyPx;
            }
            document.documentElement.style.setProperty('--sgi-popup-spacing', `${actualPopupSpacingPx}px`); // Triggers the popup animation and moves it to the height corresponding to the editor type.
            ...
            if (deltaPx > 0) {
                ...
                document.documentElement.style.setProperty('--sgi-popup-extra-height', `${Math.ceil(deltaPx + enlargementBeautifyDeltaPx)}px`);
                elementToEnlarge.classList.add('sgi-popup-extra-height');
            } else {
                elementToEnlarge.classList.remove('sgi-popup-extra-height');
            }

            // If the popup element is visible then focus the search input element.
            searchInputEl.focus();
        } else {
            document.documentElement.style.setProperty('--sgi-popup-spacing', '0px'); // Resets the popup position to prepare for its show animation.
            elementToEnlarge.classList.remove('sgi-popup-extra-height');
        }
    }
    ...
    const addLastUsedImageEl = (url, tenorPageUrl) => {
        ...
        imgEl.className = 'sgi-last-used-img';
        imgEl.src = url;
    ...
    const setObjectFitCover = (isCoverMode) => {
        if (isCoverMode) imagesContainerEl.classList.remove('sgi-object-fit-contain');
        else imagesContainerEl.classList.add('sgi-object-fit-contain');
    ...
    const legacyLastUsedImagesLocalStorageKey = 'sgiPopularImages';
    const legacyInsertOptionWidthLocalStorageKey = 'sgiInsertOptionWidth';
    const legacyDisplayOptionSquareLocalStorageKey = 'sgiDisplayOptionSquare';
    ...
    const init = async () => {
        initButtonAndAnchorForEveryEditor();

        if (!window.sgiBaseInitializationComplete) {
            initPopupElements();
            await restoreLastUsedImages();
            window.sgiBaseInitializationComplete = true;
        }
    }
    ...
    const getInjectionTargets = () => {
        return document.querySelectorAll('.shiki_editor-selector');
    }
    ...
    mo.observe(document.documentElement, { childList: true, subtree: true }); // Only observing document.documentElement allows tracking all editor appearances during Shikimori SPA navigation.
