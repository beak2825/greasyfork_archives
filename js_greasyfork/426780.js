// ==UserScript==
// @name         Market vs Facilities
// @namespace    http://tampermonkey.net/
// @version      0.3
// @description  Shows Current Market Prices in Economic Stats
// @author       Naturef
// @match        https://www.lordswm.com/ecostat.php
// @icon         https://www.google.com/s2/favicons?domain=lordswm.com
// @grant        none
// @require      https://code.jquery.com/jquery-3.3.1.min.js
// @downloadURL https://update.greasyfork.org/scripts/426780/Market%20vs%20Facilities.user.js
// @updateURL https://update.greasyfork.org/scripts/426780/Market%20vs%20Facilities.meta.js
// ==/UserScript==

var marketURL = window.location.origin + '/auction.php' ;
var spacingHTML = '<code>&nbsp;&nbsp;&nbsp;</code>' ;
const artStart = 16 ; 

function addButton()
{
    var ecoStatsText = document.getElementsByClassName('global_container_block_header')[0];
    var center = document.createElement('center');
    center.append(document.createElement('br'));
    var btn = document.createElement("BUTTON");
    var sortbtn = document.createElement('BUTTON');
    btn.innerText = "Get market details";
    btn.addEventListener('click',main);
    sortbtn.innerText = 'Sort by Profit';
    sortbtn.addEventListener('click',sortTable); // USING ARTROWS HERE //
    center.appendChild(btn);
    center.appendChild(sortbtn);
    ecoStatsText.parentNode.insertBefore(center, ecoStatsText.nextSibling)
    // x.parentNode.insertAfter(btn,x);
}

function getTable()
{
    return document.querySelector('#tableDiv').childNodes[0].childNodes[3] ;
}

async function main()
{
    var table = getTable() ; 
    var artRows = table.childNodes ;
    let artMap = loadArts() ;
    for(let i = artStart ; i < artRows.length - 1 ; i++)
    {
        var artRow = artRows[i] ;
        let artName = artRow.childNodes[0].textContent.trim() ;
        let artCost = artRow.childNodes[2].textContent.trim() ;
        let artCategory = artMap[artName][0];
        let artID = artMap[artName][1];

        let marketLink = getMarketLink(artCategory, artID) ;
        getMarketPrice(marketLink, artName).then(function (priceAndDura){
            let price = Number(priceAndDura[0]) ;
            let dura = priceAndDura[1];
            let cost = artCost.replaceAll(',', '');
            let profit = price * 0.99 - cost ;
            // console.log(profit);
            let htmlCode = spacingHTML + '<code>' + (price).toLocaleString() + '</code>' ;
            let color = profit > 0 ? 'green' : 'red';
            profit = profit.toFixed(2);
            htmlCode = htmlCode + spacingHTML + '<a href="'+ marketLink + '" target="_blank"><code class=\'profit\' value=\'' + profit + '\'style="color:' + color + '">(' + profit.toLocaleString() + ')</code></a>'
            artRows[i].childNodes[3].innerHTML = htmlCode ;
            artRows[i].childNodes[0].innerHTML += '&nbsp&nbsp (' + dura + ')';
        }) ;
    }
}

async function getMarketPrice(link, artName)
{
...
function getPrice(row)
{
    ...
    var durability = row.childNodes[0].textContent.split('#')[0];
...
function sortTable()
{
  ...
  while (switching)
  {
    ...
    for (i = artStart; i < (rows.length - 2); i++) {
      // Start by saying there should be no switching:
      shouldSwitch = false;
      /* Get the two elements you want to compare,
      one from current row and one from the next: */
      x = rows[i].getElementsByClassName('profit')[0];
      y = rows[i + 1].getElementsByClassName("profit")[0];
      if(!x || !y) continue ;
