// ==UserScript==
// @name          BiLi-dongtai
// @namespace     https://greasyfork.org/zh-CN/users/863179
// @version       0.8
// @description   将稍后再看的视频改成视频直链
// @author        zwb
// @icon          data:image/png;base64,
// @match         https://www.bilibili.com/watchlater/*
// @grant         none
// @run-at        document-end
// @license       CC
// @downloadURL https://update.greasyfork.org/scripts/464280/BiLi-dongtai.user.js
// @updateURL https://update.greasyfork.org/scripts/464280/BiLi-dongtai.meta.js
// ==/UserScript==

(function () {
    'use strict';
    /*
    //// @match         https://t.bilibili.com/*
    if (location.href.includes("t.bilibili.com")){
        if (document.querySelector("aside.right")?.textContent.length >0){
            document.querySelector("aside.right")?.remove();
            document.querySelector(".bili-dyn-up-list")?.remove();
            document.querySelector("aside.left").style.width = "20%";
            document.querySelector("main").style.width = "75%";
            document.querySelector(".bili-dyn-sidebar").style.right = "3%";
            console.log("OK");
        }
    }
    */
    if (location.pathname.includes("/watchlater/")) {
        if (location.search.length > 0) {
            location.href = "https://" + location.hostname + location.pathname + location.hash;
        } else {
            setTimeout(function () {
                    var len=document.querySelector("a.bili-cover-card").href.indexOf("&oid=");
                    var sbvid=document.querySelector("a.bili-cover-card").href.indexOf("bvid=")+"bvid=".length;
                    document.querySelectorAll("div.bili-video-card__title>a").forEach(i => { i.href = "/video/".concat(i.href.substring(sbvid,len)) });
                    document.querySelectorAll("a.bili-cover-card").forEach(i => { i.href = "/video/".concat(i.href.substring(sbvid,len)) });
            }, 2000);
        }
    }
})();