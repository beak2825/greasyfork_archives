...
// @namespace    https://greasyfork.org/zh-CN/scripts/504748
// @downloadURL https://update.greasyfork.org/scripts/504748/B%20Plus.user.js
// @updateURL https://update.greasyfork.org/scripts/504748/B%20Plus.meta.js
// ==/UserScript==
...
let menu2 = GM_registerMenuCommand('停止', function () {
    GM_setValue('Cnt', max_rewards + 10); // 将计数器设置为超过最大搜索次数，以停止搜索
        ...
        function exec() {
            ...
            //    console.log('当前词', search_words[currentSearchCount], '当前次数', currentSearchCount)
            // 根据计数器的值选择搜索引擎
            if (currentSearchCount <= max_rewards / 2) {

                document.title = "[" + currentSearchCount + " / " + max_rewards + "] "
            ...
            if (currentSearchCount >= max_rewards) {
                console.log('超过最大次数，清零')
                GM_setValue('Cnt', 0); // 将计数器加1
    ...
    setInterval(() => {
        ...
        if (tag.date != new Date().toLocaleDateString() && !localStorage.running) {
            GM_setValue('Cnt', 0); // 将计数器加1
    ...
    // 定义菜单命令：开始
    let menu1 = GM_registerMenuCommand('开始', function () {
        if (localStorage.running) return console.log('正在运行')
        GM_setValue('Cnt', 0); // 将计数器重置为0
        start()
        location.href = "https://www.bing.com/?br_msg=Please-Wait"; // 跳转到Bing首页
    }, 'o');
