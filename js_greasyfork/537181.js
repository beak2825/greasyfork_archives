...
// @version      1.3.1-mod-1.0.7
// @description  基于@Bao-qing （2025-01-29）， @微風子 （1.3.1）版本的分支，补充额外功能并修复bug
// @author       Cursor
// @match        *://*.123pan.com/*
// @match        *://*.123pan.cn/*
// @match        *://*.123865.com/*
...
// @grant        GM_getValue
// @grant        GM_setValue
// @downloadURL https://update.greasyfork.org/scripts/537181/123FastLink.user.js
// @updateURL https://update.greasyfork.org/scripts/537181/123FastLink.meta.js
// ==/UserScript==
...
(function() {
    ...
    const SCRIPT_VERSION = "1.3.1-mod-1.0.7";
    ...
    const DOM_SELECTORS = {
        TARGET_BUTTON_AREA: '.home-operator-button-group',
    ...
    const settingsManager = {
        settings: {
            ...
            fastlinkButtonBg: '#fff',
            fastlinkButtonText: '#333',
            fastlinkButtonRandom: false
        },
        init: function() {
            ...
            if (savedSettings) {
                try {
                    ...
                    if (!this.settings.fastlinkButtonBg) this.settings.fastlinkButtonBg = '#fff';
                    if (!this.settings.fastlinkButtonText) this.settings.fastlinkButtonText = '#333';
                    if (typeof this.settings.fastlinkButtonRandom !== 'boolean') this.settings.fastlinkButtonRandom = false;
                } catch (e) {
    ...
    const uiManager = {
        ...
        applyStyles: function() {
            ...
            style.textContent = `
                ...
                .fastlink-modal-buttons { display: flex; justify-content: center; gap: 10px; }
                ...
                #fastlink-dropdown-menu-container{position:absolute;background:#fff;border:1px solid #e4e7ed;padding:0;box-shadow:0 4px 12px rgba(0,0,0,.12);margin-top:5px;z-index:10002 !important;max-height:calc(100vh - 80px);overflow-y:auto;top:100%;left:0;min-width:180px;font-size:14px;border-radius:8px;}
                #fastlink-dropdown-menu-container ul.ant-dropdown-menu{padding: 6px 0;}
                #fastlink-dropdown-menu-container li.ant-dropdown-menu-item{font-size:14px;line-height:22px;padding: 6px 20px;white-space:nowrap;}
                #fastlink-dropdown-menu-container li.ant-dropdown-menu-item:hover{background:#f5f5f5;}
                #fastlink-dropdown-menu-container li[role="separator"]{height:1px;background:#f0f0f0;margin:4px 0;padding:0;}
                ...
                .fastlink-menu-btn:focus, .fastlink-menu-btn:active {
                    border-color: #4096ff;
                    box-shadow: 0 0 0 2px rgba(24, 144, 255, 0.2);
                    outline: none;
                }
                .home-operator-button-group .ant-btn, .home-operator-button-group .fastlink-menu-btn {
                    margin-right: 8px;
                }
            `;
            style.textContent += `
        ...
        createDropdownButton: function() {
            ...
            if (targetElement && targetElement.parentNode) { 
                ...
                buttonContainer.className = 'fastlink-main-button-container';
                buttonContainer.style.display = 'inline-block';
                buttonContainer.style.marginLeft = '4px';
                buttonContainer.style.position = 'relative';
                // Use a button styled like Ant Design, with dynamic background color
                let btnBg = settingsManager.get('fastlinkButtonBg') || '#fff';
                let btnText = settingsManager.get('fastlinkButtonText') || '#333';
                if (settingsManager.get('fastlinkButtonRandom')) {
                    // Get all preset color pairs except random
                    const presetColors = [
                        {bg:'#1677ff', fg:'#fff'},
                        {bg:'#52c41a', fg:'#fff'},
                        {bg:'#faad14', fg:'#fff'},
                    ...
                    btnBg = randomPreset.bg;
                    btnText = randomPreset.fg;
                    // Save the chosen random color for this session
                    settingsManager.set('fastlinkButtonBg', btnBg);
                    settingsManager.set('fastlinkButtonText', btnText);
                }
                buttonContainer.innerHTML = `<button type="button" class="ant-btn ant-btn-default ant-btn-color-default ant-btn-variant-outlined mfy-button fastlink-menu-btn" style="background: ${btnBg}; color: ${btnText};"><span role="img" aria-label="menu" class="anticon anticon-menu"><svg viewBox="64 64 896 896" focusable="false" data-icon="menu" width="1em" height="1em" fill="currentColor" aria-hidden="true"><path d="M120 300h720v60H120zm0 180h720v60H120zm0 180h720v60H120z"></path></svg></span><span>秒传</span></button>`; 
                ...
                dropdownMenu.style.boxShadow = '0 4px 12px rgba(0,0,0,0.12)';
                dropdownMenu.style.background = '#fff';
                dropdownMenu.style.border = '1px solid #e4e7ed';
                ...
                // Only open menu when clicking the button, not the whole container
                const menuBtn = buttonContainer.querySelector('.fastlink-menu-btn');
                menuBtn.addEventListener('click', (e) => { e.stopPropagation(); dropdownMenu.style.display = dropdownMenu.style.display === 'none' ? 'block' : 'none'; });
                ...
                // Insert after the upload button using its class selector
                let inserted = false;
                const uploadBtn = targetElement.querySelector('.upload-button');
                if (uploadBtn && uploadBtn.parentNode) {
                    uploadBtn.parentNode.insertBefore(buttonContainer, uploadBtn.nextSibling);
                    inserted = true;
                }
                if (!inserted) {
                    targetElement.appendChild(buttonContainer);
                }
                // Always append the dropdownMenu to the buttonContainer
        ...
        showModal: function(title, content, type = 'info', closable = true, pureLinkForClipboard = null, jsonDataForExport = null, preprocessingFailuresForLog = null) {
            ...
            else if (type === 'showLink') { if (pureLinkForClipboard || jsonDataForExport) { htmlContent += `<button id="fl-m-copy" class="copy-btn">📋 复制链接</button>`; if (jsonDataForExport) htmlContent += `<button id="fl-m-export-json" class="export-btn">📄 导出为 JSON</button>`; } htmlContent += `<button id="fl-m-cancel" class="close-btn">关闭</button>`; }
...
fileInputEl.addEventListener('change', () => { if (fileInputEl.files && fileInputEl.files.length > 0) { statusDiv.textContent = `已选中文件: ${fileInputEl.files[0].name}。请点击下方"转存"按钮。`; if(linkInputEl) linkInputEl.value = ''; } else statusDiv.textContent = ''; }); ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => dropArea.addEventListener(eventName, (e) => { e.preventDefault(); e.stopPropagation(); }, false)); ['dragenter', 'dragover'].forEach(eventName => dropArea.addEventListener(eventName, () => dropArea.classList.add('drag-over-active'), false)); ['dragleave', 'drop'].forEach(eventName => dropArea.addEventListener(eventName, () => dropArea.classList.remove('drag-over-active'), false)); dropArea.addEventListener('drop', (e) => {
        ...
        showSettingsDialog: function() {
            const settingsContent = `
                <div class="settings-dialog">
                    <div class="settings-group">
                        ...
                        <div class="settings-item">
                            <input type="color" id="setting-fastlink-btn-bg" value="${settingsManager.get('fastlinkButtonBg') || '#fff'}" style="width: 32px; height: 32px; border: none; margin-right: 8px; vertical-align: middle;">
                            <label for="setting-fastlink-btn-bg">秒传按钮背景色</label>
                            <div class="settings-description">自定义秒传按钮的背景色</div>
                        </div>
                        <div class="settings-item">
                            <input type="color" id="setting-fastlink-btn-text" value="${settingsManager.get('fastlinkButtonText') || '#333'}" style="width: 32px; height: 32px; border: none; margin-right: 8px; vertical-align: middle;">
                            <label for="setting-fastlink-btn-text">秒传按钮文字颜色</label>
                            <div class="settings-description">自定义秒传按钮的文字颜色</div>
                        </div>
                        <div class="settings-item">
                            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                                <span style="margin-right: 8px;">颜色预设：</span>
                                <span class="fastlink-btn-preset" data-bg="#1677ff" data-fg="#fff" title="蓝色"><span style="background:#1677ff;color:#fff;">蓝</span></span>
                                <span class="fastlink-btn-preset" data-bg="#52c41a" data-fg="#fff" title="绿色"><span style="background:#52c41a;color:#fff;">绿</span></span>
                                <span class="fastlink-btn-preset" data-bg="#faad14" data-fg="#fff" title="橙色"><span style="background:#faad14;color:#fff;">橙</span></span>
                                <span class="fastlink-btn-preset" data-bg="#f5222d" data-fg="#fff" title="红色"><span style="background:#f5222d;color:#fff;">红</span></span>
                                <span class="fastlink-btn-preset" data-bg="#595959" data-fg="#fff" title="深灰"><span style="background:#595959;color:#fff;">灰</span></span>
                                <span class="fastlink-btn-preset" data-bg="#fff" data-fg="#1677ff" title="白底蓝字"><span style="background:#fff;color:#1677ff;border:1px solid #1677ff;">白</span></span>
                                <span class="fastlink-btn-preset" data-bg="#fff" data-fg="#333" title="白底黑字"><span style="background:#fff;color:#333;border:1px solid #ccc;">白</span></span>
                                <span class="fastlink-btn-preset" data-bg="#722ed1" data-fg="#fff" title="紫色"><span style="background:#722ed1;color:#fff;">紫</span></span>
                                <span class="fastlink-btn-preset" data-bg="#13c2c2" data-fg="#fff" title="青色"><span style="background:#13c2c2;color:#fff;">青</span></span>
                                <span class="fastlink-btn-preset" data-bg="#eb2f96" data-fg="#fff" title="粉色"><span style="background:#eb2f96;color:#fff;">粉</span></span>
                                <span class="fastlink-btn-preset" data-bg="#fadb14" data-fg="#222" title="亮黄"><span style="background:#fadb14;color:#222;">黄</span></span>
                                <span class="fastlink-btn-preset" data-bg="#36cfc9" data-fg="#fff" title="亮蓝绿"><span style="background:#36cfc9;color:#fff;">蓝绿</span></span>
                                <span class="fastlink-btn-preset fastlink-btn-preset-random" data-bg="random" data-fg="random" title="随机"><span style="background:linear-gradient(90deg,#1677ff,#52c41a,#faad14,#eb2f96,#36cfc9,#722ed1,#fff);color:#333;border:1px solid #ccc;">随机</span></span>
                            </div>
                            <div class="settings-description">点击预设快速应用现代风格配色</div>
                        </div>
                    </div>
            ...
            const fastlinkBtnBgInput = this.modalElement.querySelector('#setting-fastlink-btn-bg');
            const fastlinkBtnTextInput = this.modalElement.querySelector('#setting-fastlink-btn-text');
            const fastlinkBtnPresets = this.modalElement.querySelectorAll('.fastlink-btn-preset');
            ...
            if (fastlinkBtnBgInput) {
                fastlinkBtnBgInput.oninput = () => {
                    settingsManager.set('fastlinkButtonBg', fastlinkBtnBgInput.value);
                    settingsManager.set('fastlinkButtonRandom', false);
                    // Update the button color live if present
                    const btn = document.querySelector('.fastlink-menu-btn');
                    if (btn) btn.style.background = fastlinkBtnBgInput.value;
                };
            }

            if (fastlinkBtnTextInput) {
                fastlinkBtnTextInput.oninput = () => {
                    settingsManager.set('fastlinkButtonText', fastlinkBtnTextInput.value);
                    settingsManager.set('fastlinkButtonRandom', false);
                    // Update the button text color live if present
                    const btn = document.querySelector('.fastlink-menu-btn');
                    if (btn) btn.style.color = fastlinkBtnTextInput.value;
                };
            }

            if (fastlinkBtnPresets && fastlinkBtnPresets.length > 0) {
                // Helper to update selected indicator
                function updatePresetSelectionIndicator(selectedBg, selectedFg, isRandom) {
                    fastlinkBtnPresets.forEach(preset => {
                        const bg = preset.getAttribute('data-bg');
                        const fg = preset.getAttribute('data-fg');
                        if (isRandom) {
                            if (bg === 'random' && fg === 'random') preset.classList.add('fastlink-btn-preset-selected');
                            else preset.classList.remove('fastlink-btn-preset-selected');
                        } else {
                            if (bg === selectedBg && fg === selectedFg) preset.classList.add('fastlink-btn-preset-selected');
                            else preset.classList.remove('fastlink-btn-preset-selected');
                        }
                    });
                }
                // On dialog open, set indicator
                if (settingsManager.get('fastlinkButtonRandom')) {
                    updatePresetSelectionIndicator(null, null, true);
                } else {
                    updatePresetSelectionIndicator(settingsManager.get('fastlinkButtonBg'), settingsManager.get('fastlinkButtonText'), false);
                }
                fastlinkBtnPresets.forEach(preset => {
                    preset.style.cursor = 'pointer';
                    preset.style.display = 'inline-block';
                    preset.style.borderRadius = '4px';
                    preset.style.padding = '2px 8px';
                    preset.style.margin = '2px 0';
                    preset.style.border = '1px solid #eee';
                    preset.addEventListener('click', () => {
                        let bg = preset.getAttribute('data-bg');
                        let fg = preset.getAttribute('data-fg');
                        if (bg === 'random' && fg === 'random') {
                            settingsManager.set('fastlinkButtonRandom', true);
                            updatePresetSelectionIndicator(null, null, true);
                            // On next reload, a random color will be chosen
                        } else {
                            settingsManager.set('fastlinkButtonRandom', false);
                            settingsManager.set('fastlinkButtonBg', bg);
                            settingsManager.set('fastlinkButtonText', fg);
                            updatePresetSelectionIndicator(bg, fg, false);
                            // Update color pickers
                            if (fastlinkBtnBgInput) fastlinkBtnBgInput.value = bg;
                            if (fastlinkBtnTextInput) fastlinkBtnTextInput.value = fg;
    ...
    function extractShareInfoFromText(text) {
        let shareKey = null;
        let sharePwd = null;
        ...
        try {
            ...
            if (sIndex !== -1 && pathSegments.length > sIndex + 1) {
                // Get everything after /s/ up to the next / or end of string
                shareKey = pathSegments[sIndex + 1].split(/[\/?#]/)[0];
                const searchParams = new URLSearchParams(url.search);
                const possiblePwdParams = ['pwd', '提取码', 'password', 'extract', 'code'];
                for (const paramName of possiblePwdParams) {
                    if (searchParams.has(paramName)) {
                        sharePwd = searchParams.get(paramName);
                        break;
                    }
                }
            }
        } catch (e) {
            // If URL parsing fails, try to extract from text
            const sMatch = text.match(/\/s\/([^\/\s?#]+)/);
            if (sMatch) {
                shareKey = sMatch[1].split(/[\/?#]/)[0];
            }
        }

        // If not found as URL, try to extract share key and password from text
        if (!shareKey) {
            // Try to find share key in various formats, including those with special characters
            const keyMatch = text.match(/\/s\/([^\/\s?#]+)/) || text.match(/([A-Za-z0-9\-_%:]{8,})/);
            if (keyMatch) {
                shareKey = keyMatch[1].split(/[\/?#]/)[0];
            }
        }
        ...
        if (!sharePwd) {
            const pwdRegexes = [
                /提取码[:=]([A-Za-z0-9]+)/,
                /[:=]([A-Za-z0-9]+)$/,  // Match password at the end after : or =
                /[?&]提取码[:=]([A-Za-z0-9]+)/,
                /[?&]pwd[:=]([A-Za-z0-9]+)/,
                /[?&]password[:=]([A-Za-z0-9]+)/,
                /提取码[：:]([A-Za-z0-9]+)/,
    ...
    generatePublicBatchBtn.onclick = async function() {
        const batchInputEl = this.modalElement.querySelector('#fl-m-public-share-batch-input');
        if (!batchInputEl || !batchInputEl.value.trim()) {
            this.showAlert("请输入至少一个分享链接。");
            return;
