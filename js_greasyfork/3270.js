...
// @grant       GM_info
// @grant       unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/3270/zbiorkowy.user.js
// @updateURL https://update.greasyfork.org/scripts/3270/zbiorkowy.meta.js
// ==/UserScript==
...
var engine = {
    ...
    addonurl: 'https://greasyfork.org/scripts/3270-zbiorkowy',
	...
	checkVersion : function()
	{
	   ...
	   {
	       var code = '<div style="background: #8b2d21; border: 2px #2e170f solid; border-left: 0px; border-right: 0px; margin-bottom: 3px; text-align: center; padding: 10px; font-size: 16px; color: white; cursor: pointer;" onclick="window.location=\''+engine.addonurl+'\'">Dostępna nowa wersja automatu zbiórkowego MON</div>';
	...
	{
    register_event('zbiorkowy_myevent', function() { engine.modifiedFunction(unsafeWindow.zbiorkowy_myevent_data.data, unsafeWindow.zbiorkowy_myevent_data.status, unsafeWindow.zbiorkowy_myevent_data.jq); }, function()
			{
				window.$j(document).ajaxSuccess(function(data, status, jq)
					 {
						 window.zbiorkowy_myevent_data = {data : data, status : status, jq : jq};
						 var event = document.createEvent("Event");
						 event.initEvent("zbiorkowy_myevent", true, true);
