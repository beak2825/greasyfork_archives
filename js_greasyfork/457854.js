// ==UserScript==
// @name           GGn YADG
// @description    This script provides integration with online description generator YADG (http://yadg.cc) - Credit to Slack06 and SavageCore
// @homepageURL    https://greasyfork.org/en/scripts/457854-ggn-yadg
// @version        1.0.0
// @license        MIT
// @namespace      yadg
...
// @require        https://yadg.cc/static/js/jsandbox.min.js#md5=95d2a40bb24b5779a572cf9e4b4f570e
// @match          http*://*gazellegames.net/upload.php*
// @match          http*://*gazellegames.net/requests.php*
// @match          http*://*gazellegames.net/torrents.php*
// @downloadURL https://update.greasyfork.org/scripts/457854/GGn%20YADG.user.js
// @updateURL https://update.greasyfork.org/scripts/457854/GGn%20YADG.meta.js
// ==/UserScript==
...
*/
const defaultGGnFormat = 914;
const defaultGGnTarget = 'other';
...
factory = {
    ...
    locations: [
        {
            name: 'ggn_upload',
            regex: /http(s)?:\/\/(.*\.)?gazellegames\.net\/upload\.php.*/i,
        },
        {
            name: 'ggn_edit',
            regex: /http(s)?:\/\/(.*\.)?gazellegames\.net\/torrents\.php\?action=editgroup&groupid=.*/i,
        },
        {
            name: 'ggn_request',
            regex: /http(s)?:\/\/(.*\.)?gazellegames\.net\/requests\.php\?action=new/i,
        }
    ],
    ...
    init() {
        const pageHasLoaded = () => document.getElementById('bitrate') != null || document.getElementById('bitrates_tr') != null
        ...
        else if (this.currentLocation === 'ggn_upload' || this.currentLocation === 'ggn_request') {

            if (!pageHasLoaded() && !this.hasInitialized) {
                function waitForLoad(){
                    if (document.getElementById('categories').options[document.getElementById('categories').selectedIndex].value == 'OST') {
                        const intervalHolder = setInterval(() => {
                            if(pageHasLoaded()) {
                                clearInterval(intervalHolder);
                                document.getElementById('categories').removeEventListener('change', waitForLoad)
                                if (factory.init()) {
    ...
    setDefaultFormat() {
        ...
        if (defaultFormat !== null && defaultFormat in formatOffsets) {
            ...
            formatSelect.selectedIndex = formatOffsets[defaultGGnFormat];
        }
    },
    ...
    setDefaultTarget() {
        ...
        targetSelect.selectedIndex = defaultTarget !== null && defaultTarget in targetOffsets ? targetOffsets[defaultTarget] : targetOffsets[defaultGGnTarget];
    },
    ...
    setFormatSelect(templates) {
        ...
        const saveTemplates = [];
        const ggn_template = {
            "url": "https://yadg.cc/api/v2/templates/914/",
            "id": 914,
            "owner": "GameFuzzy",
            "name": "GazelleGames",
            "nameFormatted": "GazelleGames",
            "code": "[align=center][size=3][u][b]Album Information\n{{ data.title }}[/b][/u][/size][/align]\r\n\r\n{% if data.labelIds.length > 0 %}{% set labelId=data.labelIds|first %}{% if labelId.catalogueNrs.length > 0 %}[*][b]Catalog Number:[/b] {{ labelId.catalogueNrs|join(\" or \") }}\r\n{% endif %}{% endif %}{% if data.releaseEvents.length > 0 %}{% set releaseEvent=data.releaseEvents|first %}{% if releaseEvent.date %}[*][b]Release Date:[/b] {{ releaseEvent.date|replace(\"th\", \"\")|date(\"M j, Y\") }}\r\n{% endif %}{% endif %}{% if data.format|replace(\",.*$\", \"\") %}[*][b]Media Format:[/b] {{ data.format|replace(\",.*$\", \"\") }}\r\n{% endif %}{% if data.format|replace(\".*, \", \"\") %}[*][b]Classification:[/b] {{ data.format|replace(\".*, \", \"\") }}\r\n{% endif %}{% if data.labelIds.length > 0 %}[*][b]Published by:[/b] {% set labelId=data.labelIds|first %}{{ labelId.label }}\r\n{% endif %}{% if data.format %}[*][b]Composed by:[/b] {% set main_artists=data.artists|artistsbytype(\"main\") %}{{ main_artists|wrap(\"%s\",\", \", \" & \") }}\r\n{% endif %}{% for disc in data.discs %}{% for track in disc.tracks %}{% if track.title.length > longest_title.length %}{% set longest_title = track.title %}{% endif %}{% if track.number.length > longest_number.length %}{% set longest_number = track.number %}{% endif %}{% endfor %}{% endfor %}\r\n[align=center][pre]Tracklist\r\n{% for disc in data.discs %}{% if data.discs.length > 1 %}{% if not loop.first %}\r\n\r\n{% endif %}Disc {{ disc.number }}\r\n{% endif %}{% for track in disc.tracks %}{% set replace_string = \".{\" + track.number.length + \"}\" %}{% set length = longest_number|replace(replace_string, \"\") %}{% for i in length %}0{% endfor %}{{ track.number }}\t{{ track.title }}{% set length_subtrahend = longest_title.length * 7 / 8 + \"\" %}{% set length_subtrahend = length_subtrahend|replace(\"\\..*\", \"\") %}{% set replace_string = \".{\" + length_subtrahend + \"}\" %}{% set longest_title_over_8 = longest_title|replace(replace_string, \"\") %}{% set length_subtrahend = track.title.length * 7 / 8 %}{% set length_subtrahend_str = track.title.length * 7 / 8 + \"\" %}{% set length_subtrahend_stripped = length_subtrahend_str|replace(\"\\..*\", \"\") %}{% if length_subtrahend_str != length_subtrahend_stripped %}{% set length_subtrahend += 1 %}{% endif %}{% set length_subtrahend += \"\" %}{% set length_subtrahend = length_subtrahend|replace(\"\\..*\", \"\") %}{% set replace_string = \".{\" + length_subtrahend + \"}\" %}{% set title_length = track.title|replace(replace_string, \"\") %}{% set replace_string = \".{\" + title_length.length + \"}\" %}{% set length = longest_title_over_8|replace(replace_string, \"\") %}{% for i in length %}\t{% endfor %}{% if track.length %}{{ track.length|formatseconds(true) }}{% endif %}{% if not loop.last %}\r\n{% endif %}{% endfor %}{% endfor %}[/pre][/align]",
            "isUtility": false,
            "default": true,
        }
        yadgTemplates.addTemplate(ggn_template)
        nonUtility.push(ggn_template);
        ...
        switch (this.currentLocation) {
            case 'ggn_upload': {
                ...
                const f = function (rawData) {
                    ...
                    if (data.artist_keys) {
                        title = data.title + ' by ' + data.artist_keys.join(' & ')

                        if (data.artist_keys.length > 2) {
                            title = data.title + ' by Various Artists'
                        }
                    }
