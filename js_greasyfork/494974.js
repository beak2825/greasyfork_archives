...
// @downloadURL https://update.greasyfork.org/scripts/494974/%EF%BC%88%E4%BF%9D%E5%9B%BD%E9%80%9B%E8%89%B2%E8%8A%B1%EF%BC%89%E6%97%A7%E7%89%88%E5%AD%98%E6%A1%A3240512.user.js
// @updateURL https://update.greasyfork.org/scripts/494974/%EF%BC%88%E4%BF%9D%E5%9B%BD%E9%80%9B%E8%89%B2%E8%8A%B1%EF%BC%89%E6%97%A7%E7%89%88%E5%AD%98%E6%A1%A3240512.meta.js
// ==/UserScript==
...
(function () {
  ...
  function getSettings() {
    ...
    return {
      logoText: GM_getValue("logoText", "保国爱你"), // 评分文字和特效文字
      tipsText: GM_getValue("tipsText", "马保国友情提示"), // 评分文字和特效文字
      ...
      excludeOptions: GM_getValue("excludeOptions", ["度盘", "夸克", "内容隐藏", "搬运", "SHA1"]), // 要排除的选项
      excludePostOptions: GM_getValue("excludePostOptions", ["度盘", "夸克"]), // 要排除的选项
      blockedUsers: GM_getValue("blockedUsers", ["老灯法师", "树根520", "豪大大鸡排", "林哲", "aa142150", "如果有重来", "Lcc123456cc"]), // 被屏蔽的用户
    ...
    <div class='bgsh-setting-box'>
        <div class='bgsh-setting-box-container '>
            <div class="bgsh-setting-first">
                ...
                <input type="number" id="maxGradeThread" value="${settings.maxGradeThread}">
                <br>
                <!--勋章隐藏设置-->
                <fieldset>
                    <label>隐藏勋章</label>
                ...
                <fieldset>
                    ...
                    <br>
                    <label>
                        <input type="radio" name="resizeMedals" value="0" ${settings.resizeMedals === 0 ? "checked" : ""}>
                        不修改
                    </label>
                    <label>
                        <input type="radio" name="resizeMedals" value="1" ${settings.resizeMedals === 1 ? "checked" : ""}>
                        修改所有
                    </label>
                    <label>
                        <input type="radio" name="resizeMedals" value="2" ${settings.resizeMedals === 2 ? "checked" : ""}>
                        修改女优勋章
                    </label>
                </fieldset>
                <!--勋章替换设置-->
                <fieldset>
                    ...
                    <br>
                    <label>
                        <input type="radio" name="replaceMedals" value="0" ${settings.replaceMedals === 0 ? "checked" : ""}>
                        不替换
                    </label>
                    <label>
                        <input type="radio" name="replaceMedals" value="1" ${settings.replaceMedals === 1 ? "checked" : ""}>
                        替换所有
                    </label>
                    <label>
                        <input type="radio" name="replaceMedals" value="2" ${settings.replaceMedals === 2 ? "checked" : ""}>
                        替换女优勋章
                    </label>
                </fieldset>
                ...
                <label class='bgsh-setting-checkbox-label' for="enableTitleStyleCheckbox">
                    <input type='checkbox' id="enableTitleStyleCheckbox" ${settings.enableTitleStyle ? "checked" : ""}>
                    <span class='bgsh-setting-checkbox-custom'></span>
                    <span>启用帖子标题样式</span>
                </label>
                <br>
                ...
                <input type='checkbox' id="isShowWatermarkMessageCheckbox" ${settings.isShowWatermarkMessage ? "checked" : ""}>
                    <span class='bgsh-setting-checkbox-custom'></span>
                    <span>启用点击特效</span>
                </label>
                <br>
                ...
                <input type='checkbox' id="showPayLogCheckbox" ${settings.showPayLog ? "checked" : ""}>
                    <span class='bgsh-setting-checkbox-custom'></span>
                    <span>启用购买记录</span>
                </label>
                <br>
                ...
                <input type='checkbox' id="qiandaoTipCheckbox" ${settings.qiandaoTip ? "checked" : ""}>
                    <span class='bgsh-setting-checkbox-custom'></span>
                    <span>签到提示</span>
                </label>
                <br>
...
`;
  ...
  function convertCurrency() {
    ...
    if (!moneyInput || !rmbInput || moneyInput <= 0 || rmbInput <= 0) {
      document.getElementById("conversionResult").innerHTML = "请输入有效的金钱和人民币值";
      return;
    }
    ...
    document.getElementById("conversionResult").innerHTML = `1金钱=${(1 * baseRate).toFixed(4)}元<br/>1元=${(1 / baseRate).toFixed(
      4
  ...
  `;
    ...
    const otherCheckboxes = Array.from(groupDiv.querySelectorAll('input[type="checkbox"]')).filter((cb) => cb !== selectAllCheckbox);

    function checkIndeterminateStatus() {
      const checkedCheckboxes = otherCheckboxes.filter((cb) => cb.checked).length;

      selectAllCheckbox.checked = checkedCheckboxes === otherCheckboxes.length;
      selectAllCheckbox.indeterminate = checkedCheckboxes > 0 && checkedCheckboxes < otherCheckboxes.length;
    }
  ...
  async function checkForUpdates() {
    ...
    const updateURL = "https://sleazyfork.org/zh-CN/scripts/474766-%E4%BF%9D%E5%9B%BD%E9%80%9B%E8%89%B2%E8%8A%B1/code";
  ...
  function showUpdateDialog() {
    ...
    <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.7); z-index: 9999; display: flex; justify-content: center; align-items: center;">
        <div style="background: #fff; padding: 20px; border-radius: 5px;">
            <p>有新版本的保国逛色花脚本可用！</p>
            <a href="https://sleazyfork.org/zh-CN/scripts/474766-%E4%BF%9D%E5%9B%BD%E9%80%9B%E8%89%B2%E8%8A%B1" target="_blank">点击这里更新</a>
  ...
  async function sign(userid) {
    ...
    let responseText = await postSignData({ baseURL, formhash, signtoken, secanswer, signhash });
  ...
  function extractSignParametersFromHTML(responseText) {
    ...
    const signhash = extractValueFromDOM(doc, 'form[name="login"]', "id").replace("signform_", "");
  ...
  function extractValidationText(resultText, contentType) {
    const text = resultText.replace("sectplcode[2] + '", "前").replace("' + sectplcode[3]", "后");
  ...
  function updateSignButton(responseText, userid) {
    ...
    if (responseText.includes("已经签到过") || responseText.includes("重复签到")) {
      if (settings.qiandaoTip) {
  ...
  async function topicadmin(pid, stickreply) {
    ...
    try {
      ...
      if (responseText.includes("操作成功 ")) {
        showTooltip(stickreply == "1" ? ` 置顶成功，并通知了楼主!` : ` 取消置顶成功，并通知了楼主!`);
      } else {
        showTooltip("抱歉，置顶失败！");
        console.error(responseText);
      }
  ...
  function searchSehuatang(query) {
    ...
    formData.append("srchtxt", query);
    formData.append("searchsubmit", "yes");

    fetch(`${baseURL}/search.php?mod=forum`, {
      method: "POST",
      body: formData,
      credentials: "include",
      headers: {
      ...
      .then((response) => {
        response.url ? window.open(response.url, "_blank") : copyToClipboard(query).then(openSearch);
        return response.text();
      })
      .then((data) => {
        // Process data if needed
      })
      .catch((err) => console.error("SearchSehuatang error", err));
  }
  ...
  function initInfiniteScroll(pageName) {
    ...
    async function loadNextPage() {
      ...
      newNextPageLink ? nextPageLink.setAttribute("href", newNextPageLink.getAttribute("href")) : (noMoreData = true);
      ...
      processPageContentBasedOnSettings(pageName, newSettings);
    ...
    function appendNewContent(newContent) {
      ...
      newContent.childNodes.forEach((child) => currentContent.appendChild(child.cloneNode(true)));
    }
    ...
    function updatePagination(newPgElement) {
      ...
      currentPageElements.forEach((pg) => (pg.innerHTML = newPgElement.innerHTML));
    }
     ...
     */
    function processPageContentBasedOnSettings(pageName, settings) {
  ...
  function shouldElementBeDisplayed(element, settings) {
    ...
    if (settings.excludeGroup && settings.excludeGroup.length) {
      ...
      if (isExcludedByKeyword(pElement, settings.excludeGroup) || isExcludedByKeyword(xs3Element, settings.excludeGroup)) {
        return false;
      }
    }
  ...
  function addDateRangeSelectorAndButton(targetElementOrId) {
    ...
    // 创建并绑定点击事件到“打开”按钮
    const openButton = createButton("openAllUrlButton", "批量打开帖子", () => filterAndOpenThreadsByDate(), "bgsh-openAllUrlBtn", "#f6211c");
  ...
  function createTimeSortButton(settings, buttonContainer) {
    ...
    const timeSortButton = createButton("timeSortButton", initialButtonText, function () {
      if (isFidInOrder) {
  ...
  async function displayThreadImages(settings) {
    ...
    for (let link of postLinks) {
      ...
      try {
        ...
        if (tbodyRef.nextElementSibling && tbodyRef.nextElementSibling.id === "imagePreviewTbody") {
          continue;
        }
  ...
  async function isOnlyShowMoneyAction(settings) {
    if (settings.isOnlyShowMoney) {
      ...
      tbodies.forEach((tbody) => {
        const containsECard = tbody.textContent.includes("E卡") || tbody.textContent.includes("e卡") || tbody.textContent.includes("话费");
  ...
  function processContent(content) {
    ...
    var cleanedHtml = removeElementsByClass(html, ["pstatus", "tip_4"], ["font", "div", "ignore_js_op", "br", "ol", "li", "strong", "a", "i", "table", "tbody", "tr", "td", "blockquote"], ["em"]);
  ...
  function removeElementsByClass(htmlString, classList, tagsToRemove, tagsToAllRemove) {
    ...
    imgElements.forEach(function (img) {
      ...
      if (fileAttr) {
        var fileText = (fileAttr.includes("static/image") ? "" : fileAttr) + "\\baoguo";
        var textNode = document.createTextNode(fileText);
        img.parentNode.replaceChild(textNode, img);
      } else {
        var srcAttr = img.getAttribute("src");
        if (srcAttr) {
          var srcText = (srcAttr.includes("static/image") ? "" : srcAttr) + "\\baoguo";
  ...
  function createFastPMButton(pid, touid) {
    return createButton(
      ...
      function () {
        let fid = getFidFromElement();
        const tid = extractTid(window.location.href);
        showWindow("sendpm", `home.php?mod=spacecp&ac=pm&op=showmsg&handlekey=showmsg_${touid}&touid=${touid}&pmid=0&daterange=2&pid=${pid}&tid=${tid}`);
      },
      "bgsh-fastPMButtonBtn",
      "#7f8c8d"
    );
  ...
  function createDownButton() {
    return createButton("downButton", "下载附件", function () {
      ...
      const lockedDivs = Array.from(document.querySelectorAll("div.locked")).filter((div) => div.textContent.includes("购买"));
      const dls = Array.from(document.querySelectorAll("dl.tattl")).filter((dl) => dl.querySelector("p.attnm"));
  ...
  function createGirlFriendButton(buttonContainer) {
    var girlFriendButton = createButton("girlFriendButton", "挑选女友", () => createPopup());
  ...
  function createADButton(buttonContainer) {
    var ADButton = createButton("ADButton", "广告招租", () => togglePopup());
   ...
   * 挑选女友的script。
   */
  function addGirlFriendScript() {
    var script = document.createElement("script");
    script.id = "popupScript_baoguo";
  ...
  function togglePopup() {
    ...
    if (!popup) {
      ...
      iframe.src = "https://ad1.caomeistore.com/";
  ...
  function delegatePageHandlers(settings, buttonContainer) {
    const isPostPage = () => /forum\.php\?mod=viewthread|\/thread-\d+-\d+-\d+\.html/.test(window.location.href);
    const isSearchPage = () => /search\.php\?mod=forum/.test(window.location.href);
    ...
    const isMySpacePage = () =>
      /(forum|home)\.php\?mod=(guide|space|misc)&(view=(hot|digest|new|newthread|sofa|my)|action=showdarkroom|do=favorite)(&type=(thread|reply|postcomment))?/.test(window.location.href);
    const isMyfavoritePage = () => /home\.php\?mod=space&do=favorite&view=me/.test(window.location.href);
    const isShowdarkroomPage = () => /forum\.php\?mod=misc&action=showdarkroom/.test(window.location.href);
  ...
  function createExchangeRateButton(buttonContainer) {
    var exchangeRateButton = createButton("exchangeRateButton", "金钱汇率", () => createFloatingWindow());
  ...
  function createBaoguoButton(buttonContainer) {
    var baoguoButton = createButton("baoguoButton", "功能设置", () => createSettingsUI(getSettings()));
  ...
  async function handleUserSign(buttonContainer) {
    ...
    const signButton = createButton("signButton", signButtonText, () => (window.location.href = `${baseURL}/plugin.php?id=dd_sign:index`));
  ...
  function saveSettings(settings) {
    ...
    // 对excludeGroup进行过滤
    settings.excludeGroup = settings.excludeGroup.filter((group) => settings.excludeOptions.includes(group));
    ...
    if (
      ...
      oldSettings.excludeOptions.toString() !== settings.excludeOptions.toString() ||
      oldSettings.excludePostOptions.toString() !== settings.excludePostOptions.toString() ||
  ...
  function createAdvancedSearchDiv(settings, TIDOptions = DEFAULT_TID_OPTIONS) {
    ...
    advancedSearchDiv.appendChild(createCheckboxGroup("excludeGroup", "排除关键字", excludeOptionsFormatted));
    advancedSearchDiv.appendChild(createCheckboxGroup("TIDGroup", "只看板块", TIDOptions));
    ...
    div.addEventListener("change", function (e) {
      const handleCheckboxChange = (group) => {
        if (e.target.closest(`#${group}`)) {
          const selectedValues = [...document.querySelectorAll(`#${group} input:checked`)].map((input) => input.value);
