...
// @match        *.swordz.io
// @grant        ur mum
// @downloadURL https://update.greasyfork.org/scripts/438984/swordzIO%20lol.user.js
// @updateURL https://update.greasyfork.org/scripts/438984/swordzIO%20lol.meta.js
// ==/UserScript==
...
function onRender() {
    ...
    ctx.fillText("SpeedHack [E]", 80, 180);
    ...
    if (intervalTimer % 8 == 0 && Player['list'][selfId]['map'] !== 'menu') {
        if (Math['abs'](angleStatus - mouseAngle) > 1 || Math['abs'](distanceStatus - mouseDistance) > 0.1) {
            if(!isAimbotActive) {
                socket.emit('keyPress', {
                    inputId: 'angle',
                    state: speedhack ? (mouseAngle + 360) % 360 : mouseAngle
                });
            }
            socket.emit('keyPress', {
                inputId: 'mouseDistance',
                state: speedhack ? 45 : (staying ? 15 : mouseDistance)
            });
