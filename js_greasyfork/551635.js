// ==UserScript==
// @name         Lolzteam Multiaccount Finder Pro
// @version      1.0.0
// @license MIT
...
// @match        https://lolz.live/online*
// @match        https://lolz.live/members*
...
// @downloadURL https://update.greasyfork.org/scripts/551635/Lolzteam%20Multiaccount%20Finder%20Pro.user.js
// @updateURL https://update.greasyfork.org/scripts/551635/Lolzteam%20Multiaccount%20Finder%20Pro.meta.js
// ==/UserScript==
...
let telegram_bot_token = null; //Токен телеграм бота
let telegram_chat_id = null; //Id пользователя
...
// Добавьте эту переменную в начало скрипта, после определения domain
let hrefSearchUsers = "user"; // Замените на ваш никнейм на форуме
 ...
 */
async function getCheckboxes(telegramBotTokenText = telegram_bot_token, telegramUserIDText = telegram_chat_id, lastId = null, showParameters = false) {
    let checkboxes = [
    ...
    let parameters = [
        ...
        {id: 'telegram-bot-token', value: telegram_bot_token === null ? "" : telegram_bot_token},
        {id: 'telegram-user-id', value: telegram_chat_id === null ? "" : telegram_chat_id},
...
function waitFor(selector, { timeout = 10000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      obs.disconnect();
      reject(new Error(`Не дождался селектора: ${selector}`));
    }, timeout);
  });
}


(function exposeOpenSettings() {
  try {

    if (typeof window.__lmfOpenSettings === 'function') return;


    if (typeof openSettings === 'function') {
      window.__lmfOpenSettings = (...args) => {
        try {
          return openSettings.apply(null, args);
        } catch (e) {
          console.error('LMF: ошибка в openSettings:', e);
        }
      };
    } else {

      Object.defineProperty(window, '__lmfOpenSettings', {
        configurable: true,
        enumerable: false,
        writable: true,
        value: (...args) => {
          console.error('LMF: openSettings пока недоступна');
        }
      });

      const check = setInterval(() => {
        if (typeof openSettings === 'function') {
          window.__lmfOpenSettings = (...a) => openSettings.apply(null, a);
          clearInterval(check);
        }
      }, 200);
    ...
    console.error('LMF: не удалось экспортировать openSettings:', err);
  }
})();

(async () => {
  try {
    const menu = await waitFor('.manageItems');
    const buttonId = 'lmfsettings';


    const old = menu.querySelector('#' + buttonId);
    if (old) old.remove();
    ...
    a.addEventListener('click', (e) => {
      e.preventDefault();
      try {
        const fn = window.__lmfOpenSettings || window.openSettings;
        if (typeof fn === 'function') fn();
        else console.error('LMF: функция openSettings недоступна.');
      } catch (err) {
        console.error('LMF: ошибка при открытии настроек:', err);
      }
    });


    a.setAttribute('onclick', 'if (window.__lmfOpenSettings) { window.__lmfOpenSettings(); return false; }');


    document.addEventListener('click', (ev) => {
      const t = ev.target.closest && ev.target.closest('#' + buttonId);
      if (t) {
        ev.preventDefault();
        if (typeof window.__lmfOpenSettings === 'function') window.__lmfOpenSettings();
      }
    }, { capture: true });
    ...
    if (typeof window.XenForo === 'undefined' || typeof window.XenForo.alert !== 'function') {
      window.XenForo = window.XenForo || {};
      window.XenForo.alert = function (html, title) {
        try {
          const wrap = document.createElement('div');
          wrap.style.cssText = 'position:fixed;inset:0;display:flex;align-items:center;justify-content:center;z-index:99999;background:rgba(0,0,0,.5)';
          wrap.innerHTML = `
            <div style="max-width:430px;width:100%;background:#111;border:1px solid #333;border-radius:10px;padding:20px;">
              <div style="font-weight:700;text-align:center;font-size:25px;margin-bottom:15px;">${title || 'Настройки'}</div>
              <div>${html}</div>
              <div class="mn-15-0-0" style="text-align:right;margin-top:15px;">
      <a href="javascript:void(0)" id="lmf-close" class="button red" style="padding:inherit;border:1px solid #ff565624;background:#ff565616;color:#f5f5f5;border-radius:10px;cursor:pointer;text-decoration:none;display: inherit;">Закрыть</a>
           </div>
           </div>`;
          document.body.appendChild(wrap);
          wrap.querySelector('#lmf-close').onclick = () => wrap.remove();
        } catch (e) {
          console.error('LMF: резервная модалка упала:', e);
          alert(title ? `${title}\n\n${html}` : html.replace(/<[^>]+>/g, ''));
        }
      };
    }
  } catch (err) {
    console.error('LMF: не получилось добавить пункт меню:', err);
  }
})();
...
async function openSettings(page = 1, lastId = generateRandomString(10)) {
    ...
    document.querySelectorAll('div.modal.fade, div.modal-backdrop, div[style*="position:fixed"][style*="inset:0"]').forEach(el => el.remove());
    ...
    const generateSpanContent = (id, value, isToken) => {
        ...
        if (value === null) {
            return `<span className={prompt-${spanClass}-${id}}>не указан</span>`;
        }
        return !isEmptyOrWhitespaces(value)
            ? `<span id="set-${spanClass}-${id}">${value}</span>`
        : `<span className={prompt-${spanClass}-${id}}>не указан</span>`;
    };

    const [telegramBotTokenText, telegramUserIDText] = [generateSpanContent(lastId, telegram_bot_token, true), generateSpanContent(lastId, telegram_chat_id, false)];
    ...
    modalWrap.style.cssText = 'position:fixed;inset:0;display:flex;align-items:center;justify-content:center;z-index:99999;background:rgba(0,0,0,.5)';
    modalWrap.innerHTML = `
        <div style="max-width:620px;width:90%;max-height:90vh;overflow-y:auto;background:#111;border:1px solid #333;border-radius:10px;padding:20px;">
            <div style="font-weight:700;text-align:center;font-size:25px;margin-bottom:15px;">Lolzteam Multiaccount Finder Pro</div>
            <div>${content}</div>
            <div class="mn-15-0-0" style="text-align:right;margin-top:15px;">
                <a href="javascript:void(0)" id="lmf-close" class="button red" style="padding: inherit;border:1px solid #ff565624;background:#ff565616;color:#f5f5f5;border-radius:10px;cursor:pointer;text-decoration:none;display: inherit;">Закрыть</a>
            </div>
    ...
    // Обработчик закрытия - удаляем ТОЛЬКО эту модалку
    modalWrap.querySelector('#lmf-close').onclick = () => {
        modalWrap.remove();
    };
    ...
    // Обработчики для кнопок навигации - закрываем текущую модалку перед открытием новой
    if (prevButton) {
        prevButton.onclick = () => {
            modalWrap.remove();
            openSettings(page - 1);
...
function createButtonElement(buttonId, currentUrl) {
    ...
    // Добавляем текст
    const textNode = document.createTextNode("Multiaccount Finder");
...
async function AutoCheckOnlineRegistered() {
    console.log('AutoCheckOnlineRegistered started');

    if ((await checkKeyValue("autocheck-online-registered")) !== 'true') {
        console.log('AutoCheckOnlineRegistered: настройка отключена');
        return;
    }

    if (window.location.href.indexOf(`https://${domain}/online/?type=registered`) !== 0) {
        console.log('AutoCheckOnlineRegistered: не на странице online registered');
        return;
    }

    console.log('AutoCheckOnlineRegistered: запускаем проверку');

    const currentPageElement = document.querySelector('.currentPage');
    const maxPageElement = document.querySelector(".PageNav a:last-child");
    const ulElement = document.querySelector('.memberList') || document.querySelector('#content ul');
    ...
    const currentPage = currentPageElement ? parseInt(currentPageElement.innerText.trim()) : 1;
    const maxPage = maxPageElement ? parseInt(maxPageElement.innerText.trim()) : 1;

    // Добавляем обработчик быстрого переключения
    ...
    // Ищем всех пользователей - используем более гибкие селекторы
    const members = document.querySelectorAll('.memberListItem, .primaryContent.memberListItem, li.primaryContent');

    if (members.length === 0) {
        console.log('AutoCheckOnlineRegistered: пользователи не найдены');
        return;
    }

    console.log(`AutoCheckOnlineRegistered: найдено ${members.length} пользователей`);

    // Создаем панель статистики если её нет
    const visitorCountDl = document.querySelector('dl.visitorCount, .visitorCount');
    if (visitorCountDl && !visitorCountDl.querySelector('dl.clean')) {
       ...
       newFootnoteDiv.innerHTML = `
            <div style="font-weight:700;margin-bottom:10px;text-align:center;font-size:18px;">Lolzteam Multiaccount Finder Pro</div>
            <dl class="clean"><dt>Не заподозрены:</dt><dd>0</dd></dl>
            <dl class="vpn"><dt>VPN:</dt><dd>0</dd></dl>
            <dl class="scammers"><dt>Мошенники:</dt><dd>0</dd></dl>
            <dl class="errors"><dt>Ошибки:</dt><dd>0</dd></dl>
            ${await checkKeyValue("autocheck-only-parameters") === "true" ? `<dl class="skipped"><dt>Не подошли под параметры:</dt><dd>0</dd></dl>` : ''}
            ${await checkKeyValue("not-to-check-previously-checked") === "true" ? `<dl class="checked"><dt>Ранее проверялись:</dt><dd>0</dd></dl>` : ''}
            <dl class="remained"><dt>Осталось проверить:</dt><dd>${members.length}</dd></dl>
        `;
    ...
    const checkNextMember = async () => {
        if (index >= members.length) {
            const scammersCount = parseInt(document.querySelector("dl.scammers dd")?.innerText || "0");
            ...
            console.log(`AutoCheckOnlineRegistered: проверка завершена. Найдено мошенников: ${scammersCount}`);

            if (switchPageAutomatically && currentPage < maxPage && (scammersCount < 1 || continueAfterScammerDetected)) {
                await waitWithRandomDelay();
                window.location.href = `https://${domain}/online/?type=registered&page=${currentPage + 1}`;
            } else if (circularNavigation && currentPage >= maxPage) {
                await waitWithRandomDelay();
                window.location.href = `https://${domain}/online/?type=registered`;
        ...
        if (!usernameLink) {
            console.log(`AutoCheckOnlineRegistered: пользователь ${index} - не найден usernameLink`);
            OnlineChangeTable('dl.clean', 0);
            index++;
        ...
        const usernameStyle = usernameLink.querySelector('span');
        const userStatCounters = member.querySelector('.userStatCounters, .memberInfo, .userInfo');
        const gifId = `gif-${Date.now()}-${index}`;

        // Проверяем забаненных пользователей
        if (usernameStyle && (usernameStyle.classList.contains('banned') ||
            usernameStyle.classList.contains('style18') ||
            usernameStyle.classList.contains('style360'))) {
            console.log(`AutoCheckOnlineRegistered: пользователь ${usernameLink.textContent} - заблокирован, пропускаем`);
            OnlineChangeTable('dl.clean', 0);
            index++;
            await checkNextMember();
            return;
        ...
        if (usernameLink.textContent === hrefSearchUsers) {
            console.log(`AutoCheckOnlineRegistered: пользователь ${usernameLink.textContent} - это вы, пропускаем`);
        ...
        // Проверяем параметры если включена настройка
        if (await checkKeyValue("autocheck-only-parameters") === "true") {
            let sympathies = 0;
            let messages = 0;

            const sympathyElement = member.querySelector('[href*="/likes"] .count, .counter .count');
            const messageElement = member.querySelector('[href*="search?users"] .count, .counter .count');

            if (sympathyElement) sympathies = parseInt(sympathyElement.textContent.replace(/ /g, "")) || 0;
            if (messageElement) messages = parseInt(messageElement.textContent.replace(/ /g, "")) || 0;

            const minSympathies = parseInt(await checkKeyValue("autocheck-only-parameters-sympathies")) || 20;
            const minMessages = parseInt(await checkKeyValue("autocheck-only-parameters-messages")) || 50;

            if (sympathies >= minSympathies && messages >= minMessages) {
                console.log(`AutoCheckOnlineRegistered: пользователь ${usernameLink.textContent} - не подходит под параметры (${sympathies} симпатий, ${messages} сообщений)`);
        ...
        if (userStatCounters) {
            ...
            console.log(`AutoCheckOnlineRegistered: начинаем проверку пользователя ${usernameLink.textContent}`);

            try {
                await checkUser(`https://${domain}/${usernameHref}/shared-ips`, 'registered', gifId, usernameStyle);
            } catch (error) {
                console.error(`AutoCheckOnlineRegistered: ошибка при проверке пользователя ${usernameLink.textContent}:`, error);
                OnlineChangeTable('dl.errors', 1);
                const gifElement = document.getElementById(gifId);
                if (gifElement) {
                    gifElement.src = 'https://i.imgur.com/wqXWudH.png';
                    gifElement.title = 'Ошибка при проверке';
                }
            }
        }
    ...
    checkNextMember().catch(error => {
        console.error('AutoCheckOnlineRegistered: критическая ошибка:', error);
    });
}
...
async function CreateComplaint(title, message_html, retries = 3, delay = 60000) {
    try {
        ...
        console.log('Requst Data:', params);
...
function logWithPrefix(text) {
    console.log(`[Lolzteam Multiaccount Finder Pro] ${text}`)
}
...
async function checkUser(link, source, gifId, style = null) {
    ...
    if (notToCheckPreviouslyChecked && checkedItem) {
        ...
        if (gifElement) {
            ...
            xenforoLogAndAlert(`${output}`, `Lolzteam Multiaccount Finder Pro`);
        }
        return;
    }
