...
// @namespace gdst.cc
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/37226/gdst-study.user.js
// @updateURL https://update.greasyfork.org/scripts/37226/gdst-study.meta.js
// ==/UserScript==
    ...
    if (list.length > 0) {
      ...
      for (var i = list.length - 1; i >= 0; i--) {
        if (list[i].parentNode.style['background-color'] != 'rgb(112, 240, 106)') {
          ...
          if (v_id == '') continue;
          var length=100000;
          var url = '/gdst/VideoPlay/StudyRecode?is_elective=&end='+length+ '&start=0';
          url = url + '&student_id=' + stu_id;
          url = url + '&course_id=' + cour_id;
          url = url + '&courseware_id=' + v_id;
          url = url + '&timestamp=' + timestamp;
          $.get(url);
          url=url.replace(/start=0/i,"start="+length);$.get(url);
