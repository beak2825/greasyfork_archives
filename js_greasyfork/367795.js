...
// @grant       GM_getValue
// @grant       GM_setValue
// @downloadURL https://update.greasyfork.org/scripts/367795/BelaPenta.user.js
// @updateURL https://update.greasyfork.org/scripts/367795/BelaPenta.meta.js
// ==/UserScript==
...
function SyncMouse()
{
    if(override)
    {
        var timeCog = ((timeNow - overrideStart) / cog) % 6;
        if(timeCog >= 3)
        {
            timeCog = -timeCog + 3;
        }
        /**var timeCog = ((timeNow - overrideStart) / (cog / 5 * 3)) % 11;
        if(
            (timeCog < 2 && timeCog >= 1) ||
            (timeCog < 4 && timeCog >= 3) ||
            (timeCog < 6 && timeCog >= 5) ||
            (timeCog < 8 && timeCog >= 7) ||
            (timeCog < 10 && timeCog >= 9)
        )
        {
            timeCog = -timeCog - 1;
        }
        timeCog /= 2;**/
        var newXY = rotateAngle(canvasMouseX - window.innerWidth / 2, canvasMouseY - window.innerHeight / 2, timeCog * 22.5);
