// ==UserScript==
// @name         wcdn batch job transfer back to alive
...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/483776/wcdn%20batch%20job%20transfer%20back%20to%20alive.user.js
// @updateURL https://update.greasyfork.org/scripts/483776/wcdn%20batch%20job%20transfer%20back%20to%20alive.meta.js
// ==/UserScript==
        ...
        methods: {
            ...
            showBatchPotentialExTransferDiv() {
                ...
                this.$confirm(`1.需要变更潜力的龙玉请提前放入背包中<br/>
                               2.支持龙玉潜力<strong>单独设置</strong>;支持潜力联动选择(戒指不支持),若潜力联动有误,关闭关联潜力选项<br/>
                               3.免责声明：本人是第一只小白鼠,在使用中没遇到什么问题;但无法保证100%无误,<strong>因使用本脚本造成损失概不负责</strong><br/>
                               4.有建议和bug联系我,虽然不一定改<br/>
                               <strong>~用的愉快~</strong><br/>`,
                    '潜力转换须知', {
                        dangerouslyUseHTMLString: true,
                        distinguishCancelAndClose: true,
            ...
            getTransferBalance() {
                axios.get(`/api/character/GetCharacterProfile?bear=${this.token}&characterID=${this.characterID}`).then((response) => {
                    if (response.data.success) {
            ...
            async initPotentialExTable() {
                ...
                await axios.get(`/api/character/GetCharacterMaterializedItems?bear=${this.token}&itemID=0&characterID=${this.characterID}&localCode=2&itemName=龙玉&pageIndex=1&pageSize=300`).then(async (response) => {
                    if (response.data.success) {
                        this.potentialExTableData = response.data.data.list.filter(item => { if (item.canChangePotentialEx) { return item } });
            ...
            async setChangePotentialExs(item) {
                await axios.get(`/api/character/GetPotentialExList?bear=${this.token}&materializedItemID=${item.materializedItemID}&characterID=${this.characterID}`).then((response) => {
                    if (response.data.success) {
            ...
            async doJobTransfer(changeitemID, changeitemName, targetTransferId, materializedItemID, changeitePotential, transferType) {
                ...
                await $.ajax({
                    type: 'get',
                    url: `/api/character/UseFrequency?bear=${this.token}&characterID=${this.characterID}&t=${transferType}&itemID=${changeitemID}&itemName=${changeitemName}&local=2&id=${targetTransferId}&materializedItemID=${materializedItemID}&potential=${changeitePotential}`,
                    async: true,
                    data: {},
            ...
            async doChangePotentialEx(materializedItemID, potentialExId) {
                ...
                await $.ajax({
                    type: 'get',
                    url: `/api/character/ChangePotentialEx?bear=${this.token}&materializedItemID=${materializedItemID}&potentialExId=${potentialExId}&characterID=${this.characterID}`,
                    async: true,
                    data: {},
                    success: function (response) {
                        // console.info(JSON.stringify(response));
