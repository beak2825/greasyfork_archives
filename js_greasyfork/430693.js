// ==UserScript==
// @name         Roster vehicles socket fix
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  Исправление автоматического занятия мест другими игроками
// @author       wandersher
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/430693/Roster%20vehicles%20socket%20fix.user.js
// @updateURL https://update.greasyfork.org/scripts/430693/Roster%20vehicles%20socket%20fix.meta.js
// ==/UserScript==

function SocketListener() {

	window.owner = document.querySelector('.glyphicon.glyphicon-user').parentElement.innerText.trim()
	window.coordinator = false
	window.type_test = "Regular_test"
	window.test_id = document.querySelector('input#test_id').value
	window.test_uuid = location.pathname.split('/')[location.pathname.split('/').length - 1]
	var socket = null

    function onVehicleAdd(slot, { roster, active, user }) {
		let button = document.createElement('a')
		button.setAttribute('role', 'button')
		button.href = active ? "/rt/vehicleloss/" + window.test_uuid + "/" + roster : '/#'
		button.className = active ? "btn btn-sm btn-danger" : "btn btn-sm btn-warning disabled"
		button.innerText = user
		slot.innerHTML = button.outerHTML
	}

	function onVehicleLoss(slot, { roster, active }) {
		let button = document.createElement('a')
		button.setAttribute('role', 'button')
		button.href = active ? "/rt/vehicleadd/" + window.test_uuid + "/" + roster : '/#'
		button.className = active ? "btn btn-sm btn-success" : "btn btn-sm btn-success disabled"
		button.innerText = "Занять"

		if (window.coordinator) {
			let hold = document.createElement('a')
			hold.setAttribute('role', 'button')
			hold.href = "/rt/vehicleaddr/" + window.test_uuid + "/" + roster
			hold.className = "btn btn-sm btn-danger"
			hold.innerHTML = '<span class="glyphicon glyphicon-lock" aria-hidden="true"></span>'
			slot.innerHTML = button.outerHTML + hold.outerHTML
		} else {
			slot.innerHTML = button.outerHTML
		}
	}

	function onVehicleReserve(slot, { roster, active }) {
		let button = document.createElement('a')
		button.setAttribute('role', 'button')
		button.href = active ? "/rt/vehicleloss/" + window.test_uuid + "/" + roster : '/#'
		button.className = active ? "btn btn-sm btn-danger" : "btn btn-sm btn-warning disabled"
		button.innerText = "Разработчик"
		slot.innerHTML = button.outerHTML
	}

	function onMessage({ data }) {
		const { roster, user, type, type_test, test_id } = JSON.parse(data)
		console.log(data)
		const slot = document.querySelector("#id-" + roster);
		if (!window.test_uuid) return console.log('Error:', 'Undefined test UUID!')
		if (type_test !== window.type_test) return null
		if (test_id !== window.test_id) return null
		const user_already_reserve = !Array.from(document.querySelectorAll('.btn.btn-sm.btn-danger[role="button"]')).some(a => a.innerText.includes(window.owner))
		switch (type) {
			case 'vehicle_add':
				return onVehicleAdd(slot, { roster, active: user == window.owner, user })
			case 'vehicle_loss':
				return onVehicleLoss(slot, { roster, active: user_already_reserve })
			case 'vehicle_add_r':
				return onVehicleReserve(slot, { roster, active: window.coordinator })
			default:
				return null
		}
	}
	...
	function connect() {
		socket = new WebSocket("wss://st-roster.win/services/websocket");
		socket.onmessage = function (event) { onMessage(event) };
		console.log('socket opened', new Date().toTimeString())

		socket.onclose = function (e) {
			console.log('socket closed', new Date().toTimeString())
			connect()
		};
