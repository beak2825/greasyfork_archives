// ==UserScript==
// @name         Forum Master・Discuz! Revision by auralife
// @name:en      Forum Master・Discuz! Revision by auralife
// @name:zh-CN   论坛大师・Discuz！修改版 by auralife
// @name:zh-TW   論壇大師・Discuz！修改版 by auralife
// @namespace    Forum Master・Discuz!-mxdh (Update by auralife)
// @version      1.5.6
...
// @author       hostname,mxdh,wwwab,auralife
...
// @match        https://mce.msi.cn/*
// @match        9dmgamemod.com/*
// @match        www.9dmgamemod.com/*
// @match        http://9dmgamemod.com/*
...
// @connect      *
// @license GPL-3.0
// @downloadURL https://update.greasyfork.org/scripts/558649/Forum%20Master%E3%83%BBDiscuz%21%20Revision%20by%20auralife.user.js
// @updateURL https://update.greasyfork.org/scripts/558649/Forum%20Master%E3%83%BBDiscuz%21%20Revision%20by%20auralife.meta.js
// ==/UserScript==
...
(function () {
    ...
    const GLOBAL_CONFIG = {
        ...
        // 主题美化: true/false
        // 主題美化: true/false
        theme_beautification: true,

        // 主题美化开启后 是否随机帖子中用户背景板图片
        // 设置模式: 'random' | 'on' | 'off'
        // 随机模式：每个用户不同但固定
        // 开启模式：所有用户相同
        randomFavatarBackgroud: 'random',
        favatarBackgroudUrls: [
                    //'https://www.onexiaolaji.cn/RandomPicture/api/?key=qq249663924', // 写真/套图
                    //'http://img.krkr.vip/pcapi.php',//二次元大尺度  横屏
                    //'http://img.krkr.vip/mpapi.php',//二次元大尺度  竖屏
                    'https://api.miaomc.cn/image/other/bing',// bing每日一图
                    'https://t.alcy.cc/ai',//二次元横屏
                    'https://t.alcy.cc/mp',//二次元竖屏
                    'https://imgapi.xl0408.top/index.php'//随机图（走的是github raw 接口 较慢）
                ],
    ...
    if (document.getElementsByClassName('prev').length === 0) {
        ...
        !!replyfast && skip_bottom(replyfast);
    }
    ...
    if (GLOBAL_CONFIG.theme_beautification) {
        ...
        `);
        if (member) {
            const backgroundMode = GLOBAL_CONFIG.randomFavatarBackgroud;
            if (backgroundMode !== 'off') {
                const apis = GLOBAL_CONFIG.favatarBackgroudUrls;

                const userDivs = document.querySelectorAll('div.pls.cl.favatar');
                ...
                if (backgroundMode === 'on') {
                    GM_xmlhttpRequest({
                        ...
                        nocache: true,
                        onload: function(response) {
                            const imageUrl = response.finalUrl || apis[0];
                            userDivs.forEach(div => {
                                div.style.cssText += `background:url('${imageUrl}')center/cover no-repeat!important;`;
                            });
                        },
                        onerror: function() {
                            console.error('固定背景加载失败');
                        }
                    });
                    ...
                    userDivs.forEach((div, index) => {
                        ...
                        GM_xmlhttpRequest({
                            ...
                            onload: function(response) {
                                // 获取实际的图片URL（可能是重定向后的）
                                const imageUrl = response.finalUrl || apiUrl;
                            ...
                            onerror: function() {
                                // 失败时使用默认
                                div.style.cssText += `background:url('${apis[0]}')center/cover no-repeat!important;`;
                            }
                        });
                    });
                }
