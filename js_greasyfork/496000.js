// ==UserScript==
// @name           𝑺𝑷𝑰𝑫𝑬𝑹 𝑩𝑶𝑻 𝑽1
// @name:tr        made by  leader script
// @name:az        𝐥𝐞𝐚𝐝𝐞𝐫 𝐬𝐜𝐫𝐢𝐩𝐭
// @description    Bot Panel for gartic.io
// @description:tr Bot Panel for gartic.io (in العربية)
// @description:az Bot Panel for gartic.io (in English)
// @version        1.0
// @author         𝐥𝐞𝐚𝐝𝐞𝐫 𝐬𝐜𝐫𝐢𝐩𝐭
// @license        Anonymous
// @match          *://gartic.io/*
// @match          *://*/*?__cpo=aHR0cHM6Ly9nYXJ0aWMuaW8
// @icon           https://cdn.discordapp.com/attachments/1226163651149566135/1233905485187514468/LogoSpiderbot.png?ex=6651bb48&is=665069c8&hm=c2a0a6235d673d02e4eb7073ec47ef210a5ed37c7106d940f41e46782cd8ad31&~
// @grant          GM_setValue
// @grant          GM_getValue
// @grant          GM_addValueChangeListener
// @grant          GM_addStyle
// @namespace https://youtube.com/@leaderscript.?si=7l3lNKrsBB-gbM3i
// @downloadURL https://update.greasyfork.org/scripts/496000/%F0%9D%91%BA%F0%9D%91%B7%F0%9D%91%B0%F0%9D%91%AB%F0%9D%91%AC%F0%9D%91%B9%20%F0%9D%91%A9%F0%9D%91%B6%F0%9D%91%BB%20%F0%9D%91%BD1.user.js
// @updateURL https://update.greasyfork.org/scripts/496000/%F0%9D%91%BA%F0%9D%91%B7%F0%9D%91%B0%F0%9D%91%AB%F0%9D%91%AC%F0%9D%91%B9%20%F0%9D%91%A9%F0%9D%91%B6%F0%9D%91%BB%20%F0%9D%91%BD1.meta.js
// ==/UserScript==
(function() {
    'use strict';
    ...
    image.src = 'https://media.discordapp.net/attachments/1184975344294445108/1243657990528303304/spiderbanner.png?ex=66524642&is=6650f4c2&hm=d6f1f35296763d1caed433fcccf10d2078a453e6470fa878fd61700df48c054b&=&format=webp&quality=lossless&width=1025&height=234';


    image.style.position = 'absolute';
    image.style.top = '50px';
    image.style.left = '60px';
    image.style.height = '51.0px';
    image.style.zIndex = '9999';


    document.body.appendChild(image);
})();
let kickonjoin=false,theme=0
...
if (customkick) {
    ...
    console.log("running")
    WebSocket.prototype.send=function(aa){
        originalSend.apply(this, arguments)
        console.log("this is the ws function" + " " + aa)
    ...
    function updatespeckicks(){
        ...
        roomusers.forEach(user=>{
             ; // (السكربت مجهز في أساسيات الطبع والنشر )
            user.nick.split("‏").join("")!="ضع اسمك هنا"?f(".userkickmenu").innerHTML+=`<input type="submit" class="kickmenubtn" value="`+user.nick+`" onclick="window.postMessage('kickuser.`+user.id+`','*')">`:0
        })
    }
...
if(antiafkcheckbox.checked){
    ...
    <input type="submit" style="width:90px; background:red" onclick="window.postMessage('kickall','*')" value="طرد الكل">
        <input type="checkbox" class="kickonjoin">&nbsp;طرد تلقائي<br>
    </div>
    <div class="option">
    <input type="submit" class="hidemenu" onclick="window.postMessage('hidemenu','*')" value="اخفاء">
    ...
    <h2 style="color:white;">̲𝑺𝑷𝑰𝑫𝑬𝑹 𝑩𝑶𝑻 𝑽1</h2>
  ...
  <path d="m22.34 10.642-.007-.003-20.02-8.303a1.104 1.104 0 0 0-1.04.1 1.156 1.156 0 0 0-.523.966v5.31a1.125 1.125 0 0 0 .915 1.105l10.919 2.02a.187.187 0 0 1 0 .368L1.665 14.224a1.125 1.125 0 0 0-.915 1.104v5.31a1.105 1.105 0 0 0 .496.924 1.123 1.123 0 0 0 1.066.097l20.02-8.256.008-.004a1.5 1.5 0 0 0 0-2.757Z"></path>
    ...
    <input type="submit" class="exit" onclick="window.postMessage('exit','*')" value="خروج">
    <input type="submit" class="kickall" onclick="window.postMessage('kickall','*')" value="KICK ALL">
    <input type="submit" class="kickall" onclick="window.postMessage('kickall','*')" value="طرد الكل">
    <input type="submit" class="report" onclick="window.postMessage('report','*')" value="REPORT">
    <input type="submit" class="report" onclick="window.postMessage('report','*')" value="ريبورت">
    <input type="submit" class="jump" onclick="window.postMessage('jump','*')" value="JUMP">
    <input type="submit" class="jump" onclick="window.postMessage('jump','*')" value="تخطي الرسمة">
    <input type="submit" class="acceptdraw1" onclick="window.postMessage('acceptdraw1','*')" value="DRAW 1">
    <input type="submit" class="acceptdraw1" onclick="window.postMessage('acceptdraw1','*')" value="الرسمة ¹">
    <input type="submit" class="acceptdraw2" onclick="window.postMessage('acceptdraw2','*')" value="DRAW 2">
    <input type="submit" class="acceptdraw2" onclick="window.postMessage('acceptdraw2','*')" value="الرسمة ²">
    <input type="submit" class="tips" onclick="window.postMessage('tips','*')" value="DETECT"><br>
    <input type="submit" class="tips" onclick="window.postMessage('tips','*')" value="كشف احرف"><br>
    <input type="checkbox" id="autoreport" class="autoskip"><span>  ريبورت تلقائي</span>
    <input type="checkbox" id="autoskip" class="autoskip"><span>  سكب تلقائي</span><br>
    <input type="checkbox" id="antikick" class="antikick"><span>  كك تلقائي</span>
    <input type="checkbox" id="antiafk" class="antiafk"><span>  مانع تلقائي</span></br>
...
(function() {
  ...
  function injectMessage() {
    if (!executed) {
      const message = '<div class="msg system" style="background-color: white ;"><strong style="cursor: pointer; color:#FF0000;">spiderbot Injected</strong></div>';
...
(function() {
  ...
  function injectMessage() {
    if (!executed) {
      const message = '<div class="msg system" style="background-color: white ;"><strong style="cursor: pointer; color:#0022b1;">spiderbot Injected</strong></div>';
      const chatElement = document.querySelector("#chat > div > div > div.scrollElements");
      chatElement.insertAdjacentHTML('beforeend', message);
      executed = true;
    }
