...
// @version             1.12.1
// @grant               none
// @downloadURL https://update.greasyfork.org/scripts/10661/WME%20Route%20Checker.user.js
// @updateURL https://update.greasyfork.org/scripts/10661/WME%20Route%20Checker.meta.js
// ==/UserScript==

// globals
var wmerc_version = "1.12.1";
...
function showInstructions(nav_json, r) {
  ...
  var continueImage = '<img src="'+imgRoot+'/images/vectors/routeInstructions/big_direction_forward.png" style="float: left; top: 0; padding-right: 4px" width="16" height="16" />';
  var beginImage = continueImage;
  ...
  for (var i = 0; i < route.results.length; i++) {
    ...
    var dirImageSrc = '';
    if (dirImage != '')
      dirImageSrc = '<img src="'+imgRoot+'/images/vectors/routeInstructions/'+dirImage+'.png" style="float: left; top: 0; padding-right: 4px" width="16" height="16" />';
    ...
    if ((streetName != prevStreet && streetName != '') || i == 0) {
      if (currentItem == null) {
        ...
        addTurnImage(lat, lon, i == 0 ? "big_direction_forward" : dirImage);
    ...
    if (opcode.match(/keep.*/) && i+1 < route.results.length &&
