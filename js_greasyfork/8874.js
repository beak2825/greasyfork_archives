// ==UserScript==
// @name                WME Color Highlights Unlocked
...
// @version             1.81.2y
// @grant               none
// @downloadURL https://update.greasyfork.org/scripts/8874/WME%20Color%20Highlights%20Unlocked.user.js
// @updateURL https://update.greasyfork.org/scripts/8874/WME%20Color%20Highlights%20Unlocked.meta.js
// ==/UserScript==

var wmech_version = "1.81.2y";
...
function highlightSegments() {
  ...
  var showLevels = getId('_cbShowLevels').checked;
  ...
  var showLandmarks = getId('_cbHighlightLandmarks').checked;
    ...
    //var H = WM.getSize().h;
    //var zoom = WM.getZoom();
    //getId('mapsize').innerHTML = 'w: ' + W + '<br>h: ' + H+ '<br>zoom: ' + zoom;
  ...
  if (specificRoadType) {
    ...
    if (showRecent) {
        ...
        numHours = getId('_numRecentHours').value;
    }

    if (numDays  === undefined) numDays  = 0;
    if (numHours === undefined) numHours = 0;
  ...
  for (var seg in Waze.model.segments.objects) {
    ...
    if (line !== null) {
      ...
      var createdBy   = attributes.createdBy;
      var roundabout  = attributes.junctionID !== null;
      var tDif = (tNow.getTime() - updatedOn.getTime()) / 3600000;
      var cDif = (tNow.getTime() - createdOn.getTime()) / 3600000;
      ...
      if (showLevels) {
          newWidth = 8;
          if (level <= -3)   newColor = "#000080";
          if (level == -2)   newColor = "#0000C0";
          if (level == -1)   newColor = "#0000FF";
          if (level == 0)  { newColor = "#808080"; newWidth = 4; }
          if (level == 1)    newColor = "#FF8000";
          if (level == 2)    newColor = "#FF0000";
          if (level == 3)    newColor = "#FF00FF";
          if (level >= 4)    newColor = "#800080";
          newOpacity = 0.75;
      }      

        var ok = false;
        
        if (showRecent && (numDays*24 + numHours) >= 0 && cDif <= (numDays*24 + numHours) ) {
            if ((createdBy == selectedUserId) || (!specificEditor)) {
                var heatScale = 0.75 / (numDays*24 + numHours);
                newColor = "#f00000";
                newOpacity = Math.min(0.999999, 1 - (cDif * heatScale));
                newOpacity = 0.80;
                ok = true;
            }
        }
        ...
        if (showRecent && (numDays*24 + numHours) >= 0 && tDif <= (numDays*24 + numHours) && ok==false) {
      ...
      else if (specificRoadType && (attributes.roadType == selectedRoadType || attributes.roadType==8 || attributes.roadType==20 || attributes.roadType==17) ) {
          newColor = "#909";
          newOpacity = 0.5;
          if (attributes.roadType == 8)  { newColor = "#FF8000"; newOpacity = 0.5; }
          if (attributes.roadType == 20) { newColor = "#404040"; newOpacity = 0.5; }
          if (attributes.roadType == 17) { newColor = "#FF0000"; newOpacity = 0.5; }
          newWidth = 6;
      }
...
function highlightSelectedNodes() {
  ...
  var showTurns = getId('_cbHighlightTurns').checked;
...
function initialiseHighlights()
{
  ...
  //addon.innerHTML = '<b id=mapsize style="font-size:200%"></b><br>';
  ...
  section.innerHTML  = '<b>Highlight Segments</b><br>'
                     + '<input type="checkbox" id="_cbShowLevels" /> Road Levels<br>'
  ...
  section.innerHTML  = '<b>Advanced Options</b><br>'
             ...
             + '  <input type="number" min="0" max="999"  size="3" id="_numRecentDays"   style="margin: 0 0 4px 16px"/> days '
             + '  <input type="number" min="0" max="9999" size="4" id="_numRecentHours"  style="margin: 0 0 4px 16px"/> hours<br>'
  ...
  getId('_cbShowLevels').onclick = highlightSegments;
