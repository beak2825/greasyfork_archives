...
- changed 'xhr', 'server', 'margin' and 'game_url' from const to let
  ...
  1.6.4 (775049):
  - changed skill_disable_debuffs to disable_debuffs
  1.6.5:
  - added countRaid
  - fixed raids.t3.light.name
*/
...
const raids = {
	...
	't3': {
		...
		'light': {'name': 'Hector (Impossible)', 'tweet': 'Lvl 120 Hector'},
			...
			if (skill_list.length>0) {
				...
				if ("debuff" in char.condition) {
					...
					for (let i = 0; i < char.condition.debuff.length; i++) {
						...
						if (disable_debuffs.indexOf(debuff.status) >= 0) {
							can_use_skill = false;
						}
					}
				}
...
function countRaid() {
	const raid_count = window.localStorage.getItem('raid_count');
	let updated_raid_count;
	if (raid_count === null){
		updated_raid_count = start_raid_count+1;
	} else {
		updated_raid_count = parseInt(raid_count)+1;
	}
	window.localStorage.setItem('raid_count',updated_raid_count);
	return updated_raid_count;
}
