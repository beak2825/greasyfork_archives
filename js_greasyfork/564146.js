...
// @downloadURL https://update.greasyfork.org/scripts/564146/Funr3g.user.js
// @updateURL https://update.greasyfork.org/scripts/564146/Funr3g.meta.js
// ==/UserScript==
...
(() => {
  ...
  class Nuker extends Module {
    constructor() {
        ...
        this.toggleKey = "KeyZ"; // bật / tắt nuker

        // ID KIM CƯƠNG MINEFUN (ĐÃ CHỐT)
        this.DIAMOND_ORE_ID = 581;
        ...
        this._onKeyDown = (e) => {
            ...
            if (e.code === this.toggleKey) {
                this.toggle();
                console.log(`[Nuker] Enabled = ${this.isEnabled}`);
            }
        };
        ...
        for (let dx = -radius; dx <= radius; dx++) {
            for (let dy = -radius; dy <= radius; dy++) {
                for (let dz = -radius; dz <= radius; dz++) {
                    ...
                    // không phá block dưới chân
                    if (x === px && z === pz && y === py - 1) continue;
                    ...
                    try {
                        ...
                        // 🚫 CHỈ NÉ KIM CƯƠNG
                        if (block === this.DIAMOND_ORE_ID) continue;
