...
// @grant        GM_addStyle
// @license      AGPL
// @downloadURL https://update.greasyfork.org/scripts/438297/91pu%E5%85%8D%E8%B2%BB%E7%A7%BB%E8%AA%BF%20%E6%B8%AC%E8%A9%A6%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/438297/91pu%E5%85%8D%E8%B2%BB%E7%A7%BB%E8%AA%BF%20%E6%B8%AC%E8%A9%A6%E7%89%88.meta.js
// ==/UserScript==
...
midi鍵盤升降key快捷鍵(目前暫時只支援88鍵)：
同時按下「最高」三個音 -> 升key
同時按下「最低」三個音 -> 降key
同時按下「最高」與「最低」音 -> 回到原調
同時按下「最低」兩個音與「F#」鍵 -> 直接切換至「F#」調(以此類推...)
左踏板 -> 往下滾
中踏板 -> 回到標題

v0.3 增大移調按鈕，且去除惱人的廣告
v0.4 再把按鈕變超級大（避免有人找不到），放置在譜上方
v0.5 新增同音異名調的變換功能，點擊灰色按鈕即可，例如 F# -> Gb, Db -> C#
v0.6 把按鈕字體加大，精簡一點語法
v0.7 擋電腦版首頁廣告
v0.8 支援 網頁版(首頁/song頁)、手機版(首頁/song頁)都能運行且去廣告
v0.9 新增鍵盤快捷鍵操作升降Key
v1.0 新增調性提示，會淡出
v1.1 調整調性提示字體大小&顏色，避色按住ctrl或command時觸發移調快捷鍵
v1.2 讓調性提示向右橫移再消失
v1.3 調整調性提示向右橫的加速度
v1.4 連動midi鍵盤，可用midi鍵盤升降key
v1.5 修復部份midi鍵盤無法使用的bug
v1.6 修復「原調：A-F#」時會出錯的bug
v1.7 新增腳踏板能滾動頁面(左踏板 -> 往下滾，中踏板 -> 回到標題)
v1.8 修正滾動頁面bug
v1.9 支援中英輸入法都能使用鍵盤升降key快捷鍵，新增歌詞繁/簡體轉換、拼音顯示按鈕
v1.9.1 修正midi event事件若與上次相同，則不重複觸發
v1.9.2 新增midi inputs選單、鋼琴鍵盤顯示、隱藏懸浮的首頁列
v1.9.3 新增簡單和弦辨識但暫時不開放，修正midi event事件忽略254，支援雙頁顯示功能，去掉漏網之魚的廣告
v1.9.4 去掉漏網之魚的廣告
v2.0 換一種去除廣告的寫法，新增去除直播中的廣告
v2.0.1 新增下載樂譜按鈕，若放大頁面，下載的樂譜畫質會更好
v2.0.2 去除譜面浮水印
v2.0.3 去除譜面浮水印(網頁版&手機版)
*/
...
if (url.indexOf('https://www.91pu.com.tw/m/tone.shtml?id=') != -1) {
    ...
    //-------------------------------
    // 得到歌手 & 曲名 + 從網頁載入html2canvas.js
    let get_singer_song_timer = setInterval(function() {
        let get_singer_song = document.querySelector('div.place')
        if (get_singer_song) {
            let singer_song = get_singer_song.innerText
            // console.log(`singer_song: ${singer_song}`);
            singer_song = singer_song.split(' > ')
            singer = singer_song[1]
            song = singer_song[2]
            console.log(`singer: ${singer}`);
            console.log(`song: ${song}`);
            console.log('get_singer_song_timer停止');
            // 從網頁載入html2canvas.js
            (function(d, script) {
            ...
            clearInterval(get_singer_song_timer)
        } else {
            console.log('無singer_song');
        }
    }, 1000)

    // 移除原本的"下載樂譜雙頁"按鈕，重新新增一個並新增下載功能
    let down_btn_timer = setInterval(function() {
        let down_btn = document.querySelector('button.down')
        if (down_btn) {
            let down2page = document.createElement("button")
            down2page.innerHTML = "下載雙頁樂譜"
            down2page.className = 'down'
            down2page.onclick = function(){
                showhint("下載雙頁樂譜")
                console.log("下載雙頁樂譜")
                html2canvas(document.querySelector('.stitems')).then(function (canvas) {
                    a = document.createElement('a');
                    a.href = canvas.toDataURL("image/png", 1).replace("image/png", "image/octet-stream");
                    a.download = `${singer}_${song}_雙頁.png`;
                    a.click();
                });
            }
            document.querySelector('.splitTone').append(down2page)

            console.log('down_btn_timer停止');
            clearInterval(down_btn_timer)
        }
    }, 1000)
}

// v1.0 提示調性or指定文字，會消失
let showkey = document.createElement("div")
showkey.style.position = "fixed"
showkey.style.zIndex = "100001"
showkey.style.top = "0px"
showkey.style.left = "0px"
showkey.style.color = "#55558e"
showkey.style.fontSize = "60px"
showkey.id = "hint"
showkey.innerHTML = ''
document.body.prepend(showkey)

//提示chord，會消失
let showchord = document.createElement("div")
showchord.style.position = "fixed"
showchord.style.zIndex = "100001"
showchord.style.top = "0px"
showchord.style.left = "0px"
showchord.style.color = "red"
showchord.style.fontSize = "60px"
showchord.id = "showchord"
showchord.innerHTML = ''
document.body.prepend(showchord)

// 創midi_settings的div
let midi_settings = document.createElement("div")
midi_settings.id = "midi_settings"
midi_settings.innerHTML = ''
document.body.prepend(midi_settings)

// 創piano的div
let piano = document.createElement("div")
// piano.style.position = "fixed"
piano.style.display = "none"
piano.style.top = "160px"
piano.style.left = "160px"
piano.style.zIndex = "100002"
piano.id = "piano"
piano.innerHTML = ''
document.body.prepend(piano)
// document.querySelector(".wmask").insertBefore(piano)
appendPainoCss()
...
var is_read = false
var st = "0"  // 用來結束fadeout
var st_chord = "0"  // 用來結束和弦顯示fadeout
var lyric = []  // 歌詞[]
...
btn3.onclick = function(){
    write()
    for (let i = 0; i < tone_z_num_half.length; i++) {
        document.getElementsByClassName("music")[i].innerHTML = tone_z_num_half[i]
    }
    btn4.innerHTML = nowkey
    showhint(nowkey)
    console.log("級數")
...
btn5.onclick = function(){
    ...
    if (isTraditional) {
        for (let i = 0; i < lyric.length; i++) {
            document.getElementsByClassName("lyric")[i].innerHTML = t2s(lyric[i])
        }
        btn4.innerHTML = nowkey
        btn5.innerHTML = "簡->繁"
        showhint("簡體歌詞")
        ...
        for (let i = 0; i < lyric.length; i++) {
            document.getElementsByClassName("lyric")[i].innerHTML = lyric[i]
        }
        btn4.innerHTML = nowkey
        btn5.innerHTML = "繁->簡"
        showhint("繁體歌詞")
...
btn6.onclick = function(){
    write()
    if (!isPinyin) {
        for (let i = 0; i < document.getElementsByClassName("pinyin").length; i++) {
            document.getElementsByClassName("pinyin")[i].style.display = ""
        }
        btn4.innerHTML = nowkey
        btn6.innerHTML = "隱藏拼音"
        showhint("顯示拼音")
        ...
        for (let i = 0; i < document.getElementsByClassName("pinyin").length; i++) {
            document.getElementsByClassName("pinyin")[i].style.display = "none"
        }
        btn4.innerHTML = nowkey
        btn6.innerHTML = "顯示拼音"
        showhint("隱藏拼音")
...
btn7.onclick = function(){
    write()
    if (!isPiano) {
        document.getElementById("piano").style.display = ""
        btn4.innerHTML = nowkey
        btn7.innerHTML = "隱藏鍵盤"
        showhint("顯示鍵盤")
        console.log("顯示鍵盤")
...
btn8.onclick = function(){
    ...
    console.log("下載樂譜")
    // 下載樂譜截圖
    html2canvas(document.querySelector('.showToneInfo')).then(function (canvas) {
        let a = document.createElement('a');
        a.href = canvas.toDataURL("image/png", 1).replace("image/png", "image/octet-stream");
        a.download = `${singer}_${song}.png`;
        a.click();
    });
}
...
btn6.style.width = "70px"
btn7.style.height = "40px"
btn7.style.width = "70px"
btn8.style.height = "40px"
btn8.style.width = "70px"
...
if (is_mobile) {
    ...
    // 加入「繁簡」「拼音」按鈕
    pos2 = document.querySelector(".tfunc2 > div:nth-child(2)")
    pos2.parentElement.append(btn5, pos2)
    pos2.parentElement.append(btn6, pos2)
    pos2.parentElement.append(btn7, pos2)
    pos2.parentElement.append(btn8, pos2)
}
...
window.addEventListener('keydown', function(e){
    console.log(e)
    if (e['metaKey'] == false && e['ctrlKey'] == false && (e['key'] == "+" || e['key'] == "]")) {
        button1.onclick()
    }
    else if (e['metaKey'] == false && e['ctrlKey'] == false && (e['key'] == "-" || e['key'] == "[")) {
        button2.onclick()
    }
    else if (e['metaKey'] == false && e['ctrlKey'] == false && (e['key'] == "0" || e['key'] == "\\")) {
        button3.onclick()
    }
    else if (e['metaKey'] == false && e['ctrlKey'] == false && (e['key'] == "Enter")) {
        button4.onclick()
    }
})
*/
...
window.addEventListener('keydown', function(e){
    console.log(e)
    if (e['metaKey'] == false && e['ctrlKey'] == false && (e['key'] == "+" || e['code'] == "Equal" || e['code'] == "BracketRight")) {
        button1.onclick()
    }
    else if (e['metaKey'] == false && e['ctrlKey'] == false && (e['key'] == "-" || e['code'] == "Minus" || e['code'] == "BracketLeft")) {
        button2.onclick()
    }
    else if (e['metaKey'] == false && e['ctrlKey'] == false && (e['code'] == "Digit0" || e['code'] == "Numpad0" || e['code'] == "Backslash")) {
        button3.onclick()
    }
    else if (e['metaKey'] == false && e['ctrlKey'] == false && (e['key'] == "Enter")) {
        button4.onclick()
...
// v1.4 start v1.5有修改 v1.6有修改
// 紀錄每個鍵是否被按下
let mididown = []
for (let i = 0; i < 88; i++) {
    mididown.push(false)
    // console.log(mididown)
}

let footkey = []  // footkey[0]=左踏板 footkey[1]=中踏板 footkey[2]=右踏板
for (let i = 0; i < 3; i++) {
    footkey.push(false)
    // console.log(footkey)
}
...
function appendPainoCss() {
    ...
    width: 100%;
    height: 100%;
    /* background: url(https://i.imgur.com/JaQwfDz.png) no-repeat center 0; */
    /* background: url(https://i.imgur.com/xoruQhm.png) no-repeat center 0; */
...
function onMessage(event) {
    ...
    if (channel === 144) {
        mididown[number - 21] = true
    } else if (channel === 128) {
        mididown[number - 21] = false
    } else if (channel === 176) {
        if (value === 0) {
            footkey = false
        } else if (value > 0) {
    ...
    */
    ...
    console.log("channel", event.data[0], "number", event.data[1], "value", event.data[2])


    // 先判斷midi訊號是否與上次相同，若是，則不重複觸發
    if (last_event_list[0] === event.data[0] && last_event_list[1] === event.data[1] && last_event_list[2] === event.data[2]) {
        // 此時midi訊號與上次相同，直接return不觸發
        // console.log("event與上次相同")
        return
    } else {
        last_event_list[0] = event.data[0]
        last_event_list[1] = event.data[1]
        last_event_list[2] = event.data[2]
        // console.log("event與上次不同")
    }
    ...
    switch (true) {
        ...
        case (event.data[0] < 208):
            ...
            // 若要寫踏板key.classList.add("pedaled")可能可以寫在這？
            break
    ...
    if (message.type === 'noteoff') {
        ...
        if (event.data[1] === 67) {
            ...
            if (event.data[2] === 0) {
                ...
                // 回到title
                if (is_mobile) {
                    document.querySelector(".wmask").scrollIntoView({behavior: "smooth"})
                }
                else {
                    document.querySelector(".wmask").scrollIntoView()
                    // 往上滾動
                    window.scrollBy({
                        top: -47
                    })
            ...
            if (event.data[2] === 0) {
                ...
                footkey[2] = false
                isPedaled = false
                // 要把每一個音的pedaled屬性都要拿掉
                for (let i = 21; i < 109; i++) {
                    document.querySelector(`[data-number="${i}"]`).classList.remove("pedaled")
                }
            } else if (event.data[2] === 127) {
                // 左踏板「踩」
                footkey[2] = true
                isPedaled = true
                // 要把已經pressed的音，都加上pedaled屬性
                for (let i = 21; i < 109; i++) {
                    if (document.querySelector(`[data-number="${i}"]`).classList.contains("pressed")) {
                        document.querySelector(`[data-number="${i}"]`).classList.add("pedaled")
                    }
    ...
    // console.log("mididown", mididown)

    // 用「midi鍵盤」實現移調功能，目前只支援88鍵
    let mididown_count = 0  // 計算已按下的鍵的數量
    for (let i = 0; i < 88; i++) {
        if (mididown[i]) {
            mididown_count++
        }
    }
    // console.log(mididown_count)
    if (mididown[0] && mididown[1] && mididown[2] && mididown_count === 3) {
        // 降key：按下最低三個音
        button2.onclick()
        console.log("降key")
    } else if (mididown[85] && mididown[86] && mididown[87] && mididown_count === 3) {
        // 升key：按下最高三個音
        button1.onclick()
        console.log("升key")
    } else if (mididown[0] && mididown[87] && mididown_count === 2) {
        // 回到原調：按下最高與最低音
        write()
        nowkey = originalkey
        changeKey()
        btn4.innerHTML = nowkey
        showhint("原調："+`${nowkey}`)
        console.log("回到原調", originalkey)
    } else if (mididown[0] && mididown[1] && !mididown[2] && mididown_count === 3) {
        // 到指定key：按下最低兩個音+任意(Bb)鍵 -> 切換到(Bb)調
        write()
        ...
        if (originalkey.slice(-1) != "m") {
            nowkey = chromatic_scale_major[key_index]
        } else if (originalkey.slice(-1) === "m") {
            nowkey = chromatic_scale_minor[key_index]
        }

        changeKey()
        btn4.innerHTML = nowkey
        showhint(nowkey)
        console.log("到指定key", nowkey)
    }
    ...
    */
    ...
    // 琴鍵放開不只128
    // 踏板踩下不只176
    // 踏板踩下放開可能不只176，value放開是0，踩滿是127
}

function whatChord() {
    let chord_list = [false, false, false, false, false, false, false, false, false, false, false, false]
    let chord = []  // 例如[0, 4, 7]是Cmaj
    let bass = ""
    let chromatic = ["C", "Db", "D", "Eb", "E", "F", "Gb", "G", "Ab", "A", "Bb", "B"]
    // let chromatic_samenote = [["B#", "C"], ["C#", "Db"], ["D", "D"], ["D#", "Eb"], ["E", "Fb"], ["E#", "F"], ["F#", "Gb"], ["G", "G"], ["G#", "Ab"], ["A", "A"], ["A#", "Bb"], ["B", "Cb"]]
    let chromatic_samenote = [{'6':'B#', '0':'C'}, {'0':'C#', '1':'Db'}, {'1':'D'}, {'1':'D#', '2':'Eb'}, {'2':'E', '3':'Fb'}, {'2':'E#', '3':'F'}, {'3':'F#', '4':'Gb'}, {'4':'G'}, {'4':'G#', '5':'Ab'}, {'5':'A'}, {'5':'A#', '6':'Bb'}, {'6':'B', '0':'Cb'}]
    let d = []  // 紀錄和弦音之間的距離
    for (let i = 0; i < 88; i++) {
        if (mididown[i]) {
            if (bass === "") {
    ...
    if (chord.length === 12) {
    ...
    } else if (chord.length === 11) {

    } else if (chord.length === 10) {

    } else if (chord.length === 9) {

    } else if (chord.length === 8) {
        ...
        // i 是用來轉位的
        // i = 0 原位，i = 1 一轉，i = 2 二轉
        // let j = 0
        for (let i = 0; i < chord.length; i++) {
            for (let j = 0; j < chord.length - 1; j++) {
                d[j] = (chord[(j + 1 + i) % chord.length] - chord[(j + i) % chord.length] + 12) % 12
            }
            console.log("dif", d[0], d[1])
            if (d[0] === 4 && d[1] === 3) {
                // maj
                if (chromatic[bass] === chromatic[chord[(i) % chord.length]]) {
                    showChord(chromatic[chord[(i) % chord.length]] + "")
                    console.log(chromatic[chord[(i) % chord.length]] + "")
                } else {
                    showChord(chromatic[chord[(i) % chord.length]] + "/" + chromatic[bass])
                    console.log(chromatic[chord[(i) % chord.length]] + "/" + chromatic[bass])
                }
            } else if (d[0] === 3 && d[1] === 4) {
                // min
                if (chromatic[bass] === chromatic[chord[(i) % chord.length]]) {
                    showChord(chromatic[chord[(i) % chord.length]] + "m")
                    console.log(chromatic[chord[(i) % chord.length]] + "m")
                } else {
                    showChord(chromatic[chord[(i) % chord.length]] + "m/" + chromatic[bass])
                    console.log(chromatic[chord[(i) % chord.length]] + "m/" + chromatic[bass])
                }
            } else if (d[0] === 4 && d[1] === 4 && i === 0) {
                // aug
                // 不需要轉位的和弦
                showChord(chromatic[bass] + "aug")
                console.log(chromatic[bass] + "aug")
                // if (chromatic[bass] === chromatic[chord[(i) % chord.length]]) {
                //     showChord(chromatic[chord[(i) % chord.length]] + "aug")
                //     console.log(chromatic[chord[(i) % chord.length]] + "aug")
                // } else {
                //     showChord(chromatic[chord[(i) % chord.length]] + "aug/" + chromatic[bass])
                //     console.log(chromatic[chord[(i) % chord.length]] + "aug/" + chromatic[bass])
                // }
            } else if (d[0] === 3 && d[1] === 3) {
                // dim
                // 要轉位的和弦
                if (chromatic[bass] === chromatic[chord[(i) % chord.length]]) {
                    showChord(chromatic[chord[(i) % chord.length]] + "dim")
                    console.log(chromatic[chord[(i) % chord.length]] + "dim")
                } else {
                    showChord(chromatic[chord[(i) % chord.length]] + "dim/" + chromatic[bass])
                    console.log(chromatic[chord[(i) % chord.length]] + "dim/" + chromatic[bass])
                }
            } else if (d[0] === 2 && d[1] === 5) {
                // sus2
                if (chromatic[bass] === chromatic[chord[(i) % chord.length]]) {
                    showChord(chromatic[chord[(i) % chord.length]] + "sus2")
                    console.log(chromatic[chord[(i) % chord.length]] + "sus2")
                }
            } else if (d[0] === 5 && d[1] === 2) {
                // sus4
                if (chromatic[bass] === chromatic[chord[(i) % chord.length]]) {
                    showChord(chromatic[chord[(i) % chord.length]] + "sus4")
                    console.log(chromatic[chord[(i) % chord.length]] + "sus4")
                }
            } else if (d[0] === 5 && d[1] === 5) {
                // 7sus4
                if (chromatic[bass] === chromatic[chord[(i) % chord.length]]) {
                    showChord(chromatic[chord[(i) % chord.length]] + "7sus4")
                    console.log(chromatic[chord[(i) % chord.length]] + "7sus4")
                }
            } else if (d[0] === 4 && d[1] === 7) {
                // maj7
                if (chromatic[bass] === chromatic[chord[(i) % chord.length]]) {
                    showChord(chromatic[chord[(i) % chord.length]] + "maj7")
                    console.log(chromatic[chord[(i) % chord.length]] + "maj7")
                } else {
                    showChord(chromatic[chord[(i) % chord.length]] + "maj7/" + chromatic[bass])
                    console.log(chromatic[chord[(i) % chord.length]] + "maj7/" + chromatic[bass])
                }
            } else if (d[0] === 3 && d[1] === 7) {
                // min7
                if (chromatic[bass] === chromatic[chord[(i) % chord.length]]) {
                    showChord(chromatic[chord[(i) % chord.length]] + "m7")
                    console.log(chromatic[chord[(i) % chord.length]] + "m7")
                }
            } else if (d[0] === 4 && d[1] === 6) {
                // 7
                if (chromatic[bass] === chromatic[chord[(i) % chord.length]]) {
                    showChord(chromatic[chord[(i) % chord.length]] + "7")
                    console.log(chromatic[chord[(i) % chord.length]] + "7")
                } else {
                    showChord(chromatic[chord[(i) % chord.length]] + "7/" + chromatic[bass])
                    console.log(chromatic[chord[(i) % chord.length]] + "7/" + chromatic[bass])
                }
            }
        }
    ...
    // console.log("chord_list", chord_list)
    // if (chord_list[0] && chord_list[4] && chord_list[7]) {
    //     showChord("Cmaj")
    //     console.log("Cmaj")
    // } else {
    //     console.log("不是Cmaj")
...
function renderSettings(el, inputs) {
    ...
    inputs.forEach((input, index) => {
        ...
        // )
        options.push(
            `<option value="${index}">${input.id} - ${input.name}</option>`
        )
    })
...
async function mainMidi() {
    ...
    const midi_settings = document.getElementById("midi_settings")
    ...
    try {
        ...
        listenEvent(inputs)
        renderPiano(piano)
        renderSettings(midi_settings, inputs)
    } catch (e) {
        alert(e)
    }
}
...
function showhint(nowkey) {
    clearInterval(st)
    // 提示從這邊開始滑
    showkey.style.top = "160px"
    showkey.style.left = "160px"

    let leftnum = 160
    showkey.innerHTML = nowkey
    showkey.style.opacity = 1
    if(showkey.style.opacity != 0){
        let num = 500
        ...
        st = setInterval(function(){
            ...
            showkey.style.left = `${leftnum}px`
            num--
            showkey.style.opacity = num / 100
            if (num <= 0) {
                clearInterval(st)
                // 提示結束時，回到0,0
                showkey.style.top = "0px"
...
function showChord(chord) {
    /*
    和弦辨識開發好，再開放
    clearInterval(st_chord)
    // 提示從這邊開始滑
    showchord.style.top = "240px"
    showchord.style.left = "160px"

    let leftnum = 160
    showchord.innerHTML = chord
    showchord.style.opacity = 1
    if(showchord.style.opacity != 0){
        let num = 500
        // let st = setInterval(function(){
            st_chord = setInterval(function(){
            // leftnum = leftnum + 0.5
            if (leftnum <= 200) {
                leftnum = leftnum + 320 / leftnum
            } else {
            ...
            showchord.style.left = `${leftnum}px`
            num--
            showchord.style.opacity = num / 100
            if (num <= 0) {
                clearInterval(st_chord)
                // 提示結束時，回到0,0
                showchord.style.top = "0px"
...
function write() {
    if (is_read == false) {
        ...
        // 先存原始歌詞
        for (let i = 0; i < document.getElementsByClassName("lyric").length; i++) {
            lyric.push(document.getElementsByClassName("lyric")[i].innerHTML)
        }
        // 製作pinyin，但不顯示
        for (let i = 0; i < document.getElementsByClassName("lyric").length; i++) {
            let cln = document.getElementsByClassName("lyric")[i].cloneNode(true)
            if (cln.innerHTML === "&nbsp;") {
                // 空白歌詞行，避免複製一行
                continue
            }
            cln.innerHTML = onespace(cln.innerHTML)
            cln.innerHTML = trans(cln.innerHTML)
            cln.classList.remove("lyric")
            cln.classList.add("pinyin")
            cln.style.display = "none"
            // document.getElementsByClassName("lyric")[i].appendChild(cln)
            // document.getElementsByClassName("lyric")[i].append(cln)
            insertAfter(cln, document.getElementsByClassName("lyric")[i])
        }

        // originalkey = document.querySelector("div.tkinfo > p:nth-child(1) > span").innerHTML  // 原調：A-F#時會出錯，例如：對等關係
        originalkey = document.querySelector("div.tkinfo > p:nth-child(1) > span").innerHTML.split("-")[0]
        oldkey = capo  // 紀錄初始調
        nowkey = oldkey
        // v1.8.3
        // 紀錄原和弦
        for (let i = 0; i < document.getElementsByClassName("music").length; i++) {
            tone_z_list.push(document.getElementsByClassName("music")[i].innerHTML)
        }
        numFunction()  // 做出級數，存於tone_z_num_full[i]及tone_z_num_half[i]
        is_read = true
    }
}
...
function numFunction() {
    for (let i = 0; i < tone_z_list.length; i++) {
        let str1 = tone_z_list[i]
        let reg = /<span class="tf">.*?\<\/span>/g
        str1 = str1.replace(reg, num01)
        tone_z_num_full.push(str1)
    }
...
function fullToHalf() {
    for (let i = 0; i < tone_z_num_full.length; i++) {
        let str1 = tone_z_num_full[i]
        let reg = /<span class="tf">.*?\<\/span>/g
        str1 = str1.replace(reg, FTH01)
        tone_z_num_half.push(str1)
    }
...
function changeKey() {
    for (let i = 0; i < tone_z_num_full.length; i++) {
        ...
        document.getElementsByClassName("music")[i].innerHTML = str1
    }
...
function onespace(str){
	...
	// let after_str = str
	// let space = "<span>&nbsp;</span><span>&nbsp;</span>"
	while (true) {
		let temp_str = str
		str = str.replace(/<span>&nbsp;<\/span><span>&nbsp;<\/span>/g, '<span>&nbsp;</span>')
		if (temp_str === str) {
			break
		}
	}
	// 若原本開頭有三個空白，則加上三個空白
	if (original_str.substr(0, 57) === '<span>&nbsp;</span><span>&nbsp;</span><span>&nbsp;</span>') {
		// 原本3個已被弄成1個，所以補2個就好
		return "<span>&nbsp;</span><span>&nbsp;</span>" + str
	}
	return str
}

function sstr(){
	return "锕皑蔼碍爱嗳嫒瑷暧霭谙铵鹌肮袄奥媪骜鳌坝罢钯摆败呗颁办绊钣帮绑镑谤剥饱宝报鲍鸨龅辈贝钡狈备惫鹎贲锛绷笔毕毙币闭荜哔滗铋筚跸边编贬变辩辫苄缏笾标骠飑飙镖镳鳔鳖别瘪濒滨宾摈傧缤槟殡膑镔髌鬓饼禀拨钵铂驳饽钹鹁补钸财参蚕残惭惨灿骖黪苍舱仓沧厕侧册测恻层诧锸侪钗搀掺蝉馋谗缠铲产阐颤冁谄谶蒇忏婵骣觇禅镡场尝长偿肠厂畅伥苌怅阊鲳钞车彻砗尘陈衬伧谌榇碜龀撑称惩诚骋枨柽铖铛痴迟驰耻齿炽饬鸱冲冲虫宠铳畴踌筹绸俦帱雠橱厨锄雏础储触处刍绌蹰传钏疮闯创怆锤缍纯鹑绰辍龊辞词赐鹚聪葱囱从丛苁骢枞凑辏蹿窜撺错锉鹾达哒鞑带贷骀绐担单郸掸胆惮诞弹殚赕瘅箪当挡党荡档谠砀裆捣岛祷导盗焘灯邓镫敌涤递缔籴诋谛绨觌镝颠点垫电巅钿癫钓调铫鲷谍叠鲽钉顶锭订铤丢铥东动栋冻岽鸫窦犊独读赌镀渎椟牍笃黩锻断缎簖兑队对怼镦吨顿钝炖趸夺堕铎鹅额讹恶饿谔垩阏轭锇锷鹗颚颛鳄诶儿尔饵贰迩铒鸸鲕罚阀珐矾钒烦贩饭访纺钫鲂飞诽废费绯镄鲱纷坟奋愤粪偾丰枫锋风疯冯缝讽凤沣肤辐抚辅赋负讣妇缚凫驸绂绋赙麸鲋鳆钆该钙盖赅杆赶秆赣尴擀绀冈刚钢纲岗戆镐睾诰缟锆搁鸽阁铬个纥镉颍给亘赓绠鲠龚宫巩贡钩沟苟构购够诟缑觏蛊顾诂毂钴锢鸪鹄鹘剐挂鸹掴关观馆惯贯诖掼鹳鳏广犷规归龟闺轨诡贵刽匦刿妫桧鲑鳜辊滚衮绲鲧锅国过埚呙帼椁蝈铪骇韩汉阚绗颉号灏颢阂鹤�
埡婭椏氬閹煙鹽嚴巖顏閻艷厭硯彥諺驗厴贗儼兗讞懨閆釅魘饜鼴鴦楊揚瘍陽癢養樣煬瑤搖堯遙窯謠藥軺鷂鰩爺頁業葉靨謁鄴曄燁醫銥頤遺儀蟻藝億憶義詣議誼譯異繹詒囈嶧飴懌驛縊軼貽釔鎰鐿瘞艤蔭陰銀飲隱銦癮櫻嬰鷹應纓瑩螢營熒蠅贏穎塋鶯縈鎣攖嚶瀅瀠瓔鸚癭頦罌喲擁傭癰踴詠鏞優憂郵鈾猶誘蕕銪魷輿魚漁娛與嶼語獄譽預馭傴俁諛諭蕷崳飫閾嫗紆覦歟鈺鵒鷸齬鴛淵轅園員圓緣遠櫞鳶黿約躍鑰粵悅閱鉞鄖勻隕運蘊醞暈韻鄆蕓惲慍紜韞殞氳雜災載攢暫瓚趲鏨贓臟駔鑿棗責擇則澤賾嘖幘簀賊譖贈綜繒軋鍘閘柵詐齋債氈盞斬輾嶄棧戰綻譫張漲帳賬脹趙詔釗蟄轍鍺這謫輒鷓貞針偵診鎮陣湞縝楨軫賑禎鴆掙睜猙爭幀癥鄭證諍崢鉦錚箏織職執紙摯擲幟質滯騭櫛梔軹輊贄鷙螄縶躓躑觶鐘終種腫眾鍾謅軸皺晝驟紂縐豬諸誅燭矚囑貯鑄駐佇櫧銖專磚轉賺囀饌顳樁莊裝妝壯狀錐贅墜綴騅縋諄準著濁諑鐲茲資漬諮緇輜貲眥錙齜鯔蹤總縱傯鄒諏騶鯫詛組鏃鉆纘躦鱒翺並沈醜澱叠鬥範幹臯矽櫃後夥稭傑訣誇淩麽黴撚淒扡聖屍擡塗窪污鍁鹹蠍彜湧遊籲禦願雲竈紮劄築於雕訁譾氹阪壟堖垵墊檾蕒葤蓧蒓菇槁摣咤唚哢噝噅撅劈謔襆嶴脊仿僥獁麅餘餷饊饢楞怵懍爿漵灩瀦糸絝緔瑉梘棬橰櫫軲軤賫膁腖飈糊煆溜湣渺碸滾瞘鈈鉕鋣銱鋥鋶鐦鐧鍩鍀鍃錇鎄鎇鎿鐝鑥鑹鑔穭鶓鶥鸌癧屙瘂臒襇繈耮顬蟎麯鮁鮃鮎鯗鯝鯴鱝鯿鰠鰵鱅鞽韝齇麽覆讚發髮"
}

 // 簡 -> 繁
function s2t(input_str) {
	// input_str = document.biggbks.text.value
	s = ''
	for (var i = 0; i < input_str.length; i++) {
		if (sstr().indexOf(input_str.charAt(i)) != -1)
			s += tstr().charAt(sstr().indexOf(input_str.charAt(i)))
		else
			s += input_str.charAt(i)
	}
	//    document.biggbks.text.value = s
	return s
}

 // 繁 -> 簡
function t2s(input_str) {
	// input_str = document.biggbks.text.value
	s = ''
	for (var i = 0; i < input_str.length; i++) {
		if (tstr().indexOf(input_str.charAt(i)) != -1)
			s += sstr().charAt(tstr().indexOf(input_str.charAt(i)))
		else
			s += input_str.charAt(i)
	}
	//    document.biggbks.text.value = s
