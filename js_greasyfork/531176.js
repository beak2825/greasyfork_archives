...
// @description  很喜欢115Master的播放器，所以将JAV老司机也改为调用115Master的播放器，具体就是点击“115在线播放”时调用115Master的播放器播放！需要配合115Master脚本使用（https://greasyfork.org/zh-CN/scripts/528348-115master）
// @author       Hobby
...
// @downloadURL https://update.greasyfork.org/scripts/531176/JAV%E8%80%81%E5%8F%B8%E6%9C%BA%E9%85%8D%E5%90%88115Master%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/531176/JAV%E8%80%81%E5%8F%B8%E6%9C%BA%E9%85%8D%E5%90%88115Master%E7%89%88.meta.js
// ==/UserScript==
/* jshint -W097 */
(function () {
    'use strict';
                    ...
                    if (resultJson.count > 0) {
                        ...
                        for (let i = 0; i < resultJson.data.length; i++) {
                            ...
                            if (row.play_long && (row.n.search(reg) >= 0)) { //iv vdi ico play_long
                                let pickcode = row.pc;
                                let cid = row.cid;
                                let avNumber = javId_Key;
                                let title = encodeURIComponent(row.n);
                                let videoUrl = `https://115.com/web/lixian/master/video/?cid=${cid}&pick_code=${pickcode}&avNumber=${avNumber}&title=${title}`;
                                callback(true, videoUrl, pickcode);
                                return;
                            }
                        }
                    }
