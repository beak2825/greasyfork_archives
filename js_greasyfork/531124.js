// ==UserScript==
// @name         XingYunGe-Torrent-Assistant
// @namespace    http://tampermonkey.net/
// @version      1.4.9.1
// @description  星陨阁审种助手
// @author       Exception & 7ommy & wiiii
// @match        *://*.xingyunge.top/details.php*
// @match        *://*.xingyunge.top/web/torrent-approval-page?torrent_id=*
// @match        *://*.xingyungept.org/details.php*
// @match        *://*.xingyungept.org/web/torrent-approval-page?torrent_id=*



// @icon         https://xingyunge.top/favicon.ico
...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/531124/XingYunGe-Torrent-Assistant.user.js
// @updateURL https://update.greasyfork.org/scripts/531124/XingYunGe-Torrent-Assistant.meta.js
// ==/UserScript==
...
(function() {
    ...
    // 配置信息
    //     const config = {
    //         tmpt: "https://xingyungept.org/details.php",
    //         tmpt_dialog: "https://xingyunge.top/web/torrent-approval-page?torrent_id=",
    //         official_tags:["starfallweb", "starfall"]
    //     };
    const config = {
        tmpt: "https://pt.xingyungept.org/details.php",
        tmpt_dialog: "https://pt.xingyungept.org/web/torrent-approval-page?torrent_id=",
        official_tags:["starfallweb", "starfall"]
    };
    // 构建正则表达式
    const regexPattern = new RegExp(`^${config.tmpt}`);
    const dialogRegexPattern = new RegExp(`^${config.tmpt_dialog}`);
    ...
    //bySt
    //音轨
    var title_multiAudios = false
    var matchAudios = title.match(/(\d+)Audios/)
    var titleAudioNumber = 1
    if (matchAudios) {
        title_multiAudios = true;
        titleAudioNumber = matchAudios[1];
    }
    //季数年份顺序
    var yearSeason =false
    if (title.match(/\d{4} S\d{1,2}/)){
        yearSeason = true
    }
    //      console.log("==============================${yearSeason}");
    //      console.log(title.match(/\d+Audios/));

    //有无年份信息
    var yearExists =false
    if (title.match(/(17|18|19|20)\d{2}/)){
        yearExists = true
    }
    ...
    for (var i = 0; i < tdlist.length; i ++) {
        ...
        if (td.text() == "MediaInfo"){
            ...
            //bySt
            //检测多音轨
            const multiAudioMatch = mediainfo.match(/Audio#\d+/g);
            var mediaAudioNumber = 1;
            if (multiAudioMatch!== null && multiAudioMatch.length > 1) {
                isMultiAudios = true;
                mediaAudioNumber = multiAudioMatch.length;
            }

            //             console.log("============media Audios");
            //             console.log(multiAudioMatch.length);
            //             console.log(isMultiAudios);
    ...
    const imagePromises = imagesArray.map(function(img) {
        // 获取每个图片的源链接（src属性）
        var src = img.getAttribute('src');
    ...
    Promise.all(imagePromises)
        ...
        //         console.log('加载成功的图片数量:', loadedImages);
        //         console.log('加载失败的图片数量:', failedImages);
        //         console.log('是否所有图片都显示:', imageShow);
        if (!imageShow) {
            var tempStr = failedImages + "张海报或截图无法正常显示<br>"
            $('#assistant-tooltips').append(tempStr);
            error = true;
        }
    })
        .catch((error) => {
        console.error('发生错误:', error);
        $('#assistant-tooltips-warning').append("图片显示检查遇到错误，请人工检查<br>");
        warning = true;
    });
    ...
    if(yearSeason){
        $('#assistant-tooltips').append('季数年份顺序颠倒<br>');
        error = true;
        //         console.log("=======集");
    }

    if (!isEpisode && !is_complete && (cat === 402 || cat === 403 || cat === 404|| cat === 405)) {
        $('#assistant-tooltips-warning').append("剧集可能未选择分集/完结标签<br>");
        warning = true;
    }

    if ((isMultiAudios != title_multiAudios)||(mediaAudioNumber!=titleAudioNumber)) {
        $('#assistant-tooltips-warning').append("主标题多音轨可能不正确或不规范，示例2Audios<br>");
        warning = true;
    }

    if (!yearExists) {
        $('#assistant-tooltips').append("主标题无年份信息<br>");
        error = true;
    }

    if (!title_is_episode && (subtitle_is_episode||isEpisode) && (cat === 402 || cat === 403 || cat === 404|| cat === 405)) {
        $('#assistant-tooltips-warning').append("主标题无分集信息，如S01E01-E05<br>");
        warning = true;
    }
        ...
        if (targetElement) {
            ...
            actionLink.addEventListener('click', async () => {
                try {
                    ...
                    const elementError = document.getElementById('assistant-tooltips');
                    const elementWarning = document.getElementById('assistant-tooltips-warning');


                    let errorContent = elementError.innerHTML;
                    let warningContent = elementWarning.innerHTML;

                    // 将 <br> 标签替换为换行符
                    errorContent = errorContent.replace(/<br\s*[\/]?>/gi, '\n');
                    warningContent = warningContent.replace(/<br\s*[\/]?>/gi, '\n');

                    checkStr = errorContent + warningContent;

                    checkStr=checkStr.replace("【错误】: ", "");
                    checkStr=checkStr.replace("【警告】: ", "");
                    checkStr=checkStr.replace("此种子未检测到错误", "");

                    await navigator.clipboard.writeText(checkStr);
                    console.log('字符串已成功复制到剪贴板');
                    var popup = document.createElement('div');
                    popup.id = "popup";
                    popup.style.fontSize = "20pt";
                    popup.style.position = "fixed";
                    ...
                    popup.style.left = "10%";
                    popup.style.transform = "translate(-50%, -50%)";
                    popup.style.backgroundColor = "rgb(144, 238, 144)";
                    popup.style.color = "white";
                    popup.style.padding = "15px";
                    popup.style.borderRadius = "10px";
                    popup.style.display = "none";
    ...
    // 添加复制报错按钮到页面
    function addCopyLink2() {
        const targetElement = document.querySelector('#approval');
        if (targetElement) {
            const $targetElement = $(targetElement);
            // 在目标元素后插入 HTML 内容
            $targetElement.after(' | <a href="javascript:;" id="copylink" class="small"><b><font><svg t="1743176849970" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2607" width="16" height="16"><path d="M672 832 224 832c-52.928 0-96-43.072-96-96L128 160c0-52.928 43.072-96 96-96l448 0c52.928 0 96 43.072 96 96l0 576C768 788.928 724.928 832 672 832zM224 128C206.368 128 192 142.368 192 160l0 576c0 17.664 14.368 32 32 32l448 0c17.664 0 32-14.336 32-32L704 160c0-17.632-14.336-32-32-32L224 128zM800 960 320 960c-17.664 0-32-14.304-32-32s14.336-32 32-32l480 0c17.664 0 32-14.336 32-32L832 256c0-17.664 14.304-32 32-32s32 14.336 32 32l0 608C896 916.928 852.928 960 800 960zM544 320 288 320c-17.664 0-32-14.336-32-32s14.336-32 32-32l256 0c17.696 0 32 14.336 32 32S561.696 320 544 320zM608 480 288.032 480c-17.664 0-32-14.336-32-32s14.336-32 32-32L608 416c17.696 0 32 14.336 32 32S625.696 480 608 480zM608 640 288 640c-17.664 0-32-14.304-32-32s14.336-32 32-32l320 0c17.696 0 32 14.304 32 32S625.696 640 608 640z" fill="#1296db" p-id="2608"></path></svg>复制意见</font></b></a>'); // Add new hyperlink and separator
    ...
    //         const dialogElement = document.querySelector("#layui-layer1 > div.layui-layer-title");
    //         if (dialogElement) {
    //             console.log("弹框元素存在");

    //             console.log("autoFillErrorInfo:"+GM_getValue('autoFillErrorInfo'));
    //             console.log("autoCheckAndConfirm:"+GM_getValue('autoCheckAndConfirm'));
    //             if (GM_getValue('autoCheckAndConfirm', true)) {
    //                 //通过
