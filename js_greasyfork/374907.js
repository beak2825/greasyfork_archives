// ==UserScript==
// @name           hwonk's Rocket League Trader (editing)
// @version        1.02.0
// @namespace      hwonk_rl_trader
// @description    Торговый бот для RL (с редактированием)
// @include        https://rocket-league.com/player/*
// @run-at         document-end
// @require        https://code.jquery.com/jquery-1.12.4.min.js
// @downloadURL https://update.greasyfork.org/scripts/374907/hwonk%27s%20Rocket%20League%20Trader%20%28editing%29.user.js
// @updateURL https://update.greasyfork.org/scripts/374907/hwonk%27s%20Rocket%20League%20Trader%20%28editing%29.meta.js
// ==/UserScript==
...
(function () {
    ...
    let pause_state = 0;//Инициируем флаг для паузы
    ...
    $('.rlg-header-main').before('<div id="scriptDiv" style="font-family: arial; font-size: 11px;"></div>');
    ...
    function initScript() {
        ...
        var buttonTrade = $('<button id="buttonTrade">').append('Начать торговлю').on('click', function() {
            if (localStorage.getItem('isWorking') == 'no') {
                ...
                if (stop_timer > 0){
                    setTimeout(function() {
                        localStorage.setItem('isWorking', 'no');
                        scriptState.text('Торговля автоматически остановлена');
                        deleteTrades();//Удаляем все активные лоты
                    }, stop_timer * 1000);
                }
            }
        });
        ...
        //Кнопка удаления активных лотов
        ...
        //Кнопка паузы
        let button_pause = $('<button id="button_pause">').append('Остановить торговлю').on('click', function() {
            pause_trades();
        });

        //Функция паузы скрипта
        function pause_trades() {
            if (localStorage.getItem('isWorking') == 'yes') {
                localStorage.setItem('isWorking', 'no');
                scriptState.text('Торговля остановлена');
                button_pause.text('Возобновить торговлю');
                return;
            }
            if (localStorage.getItem('isWorking') == 'no') {
                localStorage.setItem('isWorking', 'yes');
                scriptState.text('Торговля запущена');
                button_pause.text('Остановить торговлю');
                startTrading();
            }
        }
        ...
        if (!localStorage["edit_trades"]) {
            localStorage.setItem('edit_trades','false');
        }

        //Область управления настройками
        var settingsArea = $(`<div id="settingsDiv" style="border: 1px solid gray; padding: 15px; margin: 15px;">
        <div style="float: left;"><span>Редактировать лоты</span></div>&nbsp
        <input type="checkbox" class="edit-settings" id="edit_trades" value="` + +JSON.parse(localStorage["stop_timer"]) + `">
        <br>
        ...
        <br>
        <div style="float: left;"><span>Интервал между лотами, в сек</span></div>&nbsp
        ...
        <br>
        <div style="float: left;"><span>Рандом между лотами, в сек</span></div>&nbsp
        ...
        $('#buttonStart').after(buttonTrade,button_pause,buttonDeleteTrades,settingsArea).before(scriptState,'<br>');


        //Галка редактирования выставляется в соответствии с настройками
        if (JSON.parse(localStorage["edit_trades"]) == false) {
            $('#edit_trades').eq(0).prop('checked', false);
        }
        if (JSON.parse(localStorage["edit_trades"]) == true) {
            $('#edit_trades').eq(0).prop('checked', true);
        }


        //Записываем в хранилище состояние галки о редактировании
        $('#edit_trades').on('click', function() {
            localStorage.setItem('edit_trades', $('#edit_trades').prop('checked'));
        });
        ...
        //Перед началом удалением все активные сделки
        deleteTrades();
    ...
    function toTradeList(id, flag) {
        if (flag) {
            ...
            $('#tradeTable > [id="tradeSectionBody-' + returnObj['o_section'] + '"]').append(`<div id="tradingListDiv-` + id + `" style="padding: 15px; /*text-align: center;*/ width: 100%; /*min-width: 400px; display: flex;*/">
            <div id="hideItem-` + id + `" isHidden="false" style="width: 130px; float: left;">
            <span style="font-weight: bold;">` + returnObj["o_name"] + `</span>
            <img src="` + returnObj["o_img_src"] + `" style="height: 110px; width: 110px;">
            <span style="display: none;" class="id-span">` + id + `</span>
            <br><button class="delete_from_tradelist">Убрать из списка</button>
            <br><span><b>Цены</b>
            <br>Покупают: ` + returnObj["o_minPrice"] + `
            <br>Продают: ` + returnObj["o_maxPrice"] + `</span><br></div></div>`);

            $('#tradingListDiv-' + id).append('<div id="tradingListTable-' + id + '" style="display: flex; /*width: 100%;*/"><div class="left" style="min-width:182px; margin-left: 15px;"></div><div class="right" style="display: flex; width: 100%; flex-wrap: wrap;"></div></div>');
            ...
            for (var key in colors){
                ...
                <div style="width: 50%; padding: 5px; border: 1px solid gray">
                <div style="min-width: 50px; float: left;"><span>Продажа</span></div>&nbsp
                <input class="checkbox-trade ` + id + ` ` + key + `" item-id="` + id + `" item-key="` + key + `"  checkbox-type="sell" type="checkbox">
                <br>
                <span>Цена</span>&nbsp
                <input type="text" class="inputPrice" item-id="` + id + `" item-key="` + key + `" trade-type="sell" size="3" value="` + returnObj[key]["sellPrice"] + `">
                ...
                <div style="width: 50%; padding: 5px; border: 1px solid gray">
                <div style="min-width: 50px; float: left;"><span>Покупка</span></div>&nbsp
                <input class="checkbox-trade ` + id + ` ` + key + `" item-id="` + id + `" item-key="` + key + `" checkbox-type="buy" type="checkbox">
                <br>
                <span>Цена</span>&nbsp
                <input type="text" class="inputPrice" item-id="` + id + `" item-key="` + key + `" trade-type="buy" size="3" value="` + returnObj[key]["buyPrice"] + `">
                ...
                if (key == 'none') {
                    ...
                    //Для предмета цвета none делаем карточку вертикальной, а не горизонтальной
                    //keyBlock.find('.divKeyTrading').children().css('width', '100%');
                    //keyBlock.find('.divKeyTrading').css('display', 'block');
                }
                else {
            ...
            //Добавляем удаление предмета из трейдлиста по кнопке
            $('.delete_from_tradelist').on('click', function() {
                let id = +$(this).parent().attr('id').replace('hideItem-','');

                //Изменяем чекбокс из первой таблицы
                $('#checkBox-' + id).prop('checked', false);
                $('#checkBox-' + id).parent().css('font-weight','normal');

                //Отправляем на удаление из второй таблицы
                traderCheckboxClick(false, id);
            })
    ...
    function createLot(haveArray, wantArray){
        ...
        let timer2 = +JSON.parse(localStorage.getItem('timer_delete')) * 1000 - 0.5 * +JSON.parse(localStorage.getItem('timer_delete_random')) * 1000 + Math.random() * 1000 * +JSON.parse(localStorage.getItem('timer_delete_random'));


        //Если активных сделок 15, то переходим к редактированию
        if (active_trades_counter == 15 && JSON.parse(localStorage["edit_trades"]) == true) {
            $.ajax({
                url: 'https://rocket-league.com/trade/edit?trade=' + last_trade_id + '&source=mytrades',
                type: 'get',
                success: function() {
                    sender += '&alias=' + last_trade_id;
        ...
        //В ином случае создаём новый лот
        else {
            $.ajax({
                ...
                success: function(html,status,xhr){
                    if (JSON.parse(localStorage["edit_trades"]) == false) {
                        let tradeID = xhr.responseText.match(/trade\/.*'/)[0].replace('trade/','').replace(/'/g,'');//вытаскиваем из ответа айди лота
    ...
    //Получение токена
    //getToken();//В случае, если уже есть 15 сделок, сразу же получим эти данные и перейдём к редактированию

    let last_trade_id,//айдишка последнего лота, используется для редактирования
        active_trades_counter,//число активных сделок
        get_token_timer;
    ...
    function getToken(){
        $.ajax({
            ...
            success: function(html){
                ...
                active_trades_counter = $(html).find('.rlg-trade-display-container').length;
                if (active_trades_counter > 0) { last_trade_id = $(html).find('.rlg-trade-display-container:last > * > a').attr('href').replace('/trade/',''); }

                get_token_timer = +JSON.parse(localStorage["timer_between_trades"]) - +JSON.parse(localStorage["timer_between_trades_random"]) / 2 - 5;

                if (get_token_timer < 10) { get_token_timer = 10; }

                setTimeout(function() {getToken(0)}, get_token_timer * 1000);
            },
            error: function(){
                setTimeout(function() {getToken(0)}, 3000);
            }
            /*statusCode: {
                500: function() {
    ...
    function deleteTrades() {
        $.ajax({
            url: 'https://rocket-league.com/disablealltrades?id=' + playerID,
            async: false,
            type: 'get',
            success: function(html) {
