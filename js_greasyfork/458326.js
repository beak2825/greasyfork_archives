// ==UserScript==
// @name         Ebay/Search/EmbedSamples2023
...
// @downloadURL https://update.greasyfork.org/scripts/458326/EbaySearchEmbedSamples2023.user.js
// @updateURL https://update.greasyfork.org/scripts/458326/EbaySearchEmbedSamples2023.meta.js
// ==/UserScript==
        ...
        onload: function(response) {
            ...
            if (desc_text.includes('youtube.com')){
                ...
                for(var i = 1; i < links.length; i++){
                    ...
                    console.log("for");
                    temp_src = links[i].split('<')[0].split('&')[0].replace('watch?v=','').split('?')[0].split('"')[0].replace('embed/','').split(']')[0].split(')')[0];
                    temp_src = "https://www.youtube.com/embed/"+temp_src;
                    if(!temp_src.includes('.jpg') && temp_src != last_src && !audio_urls.includes(temp_src)){
                        temp_iframe = document.createElement('iframe');
                        temp_iframe.setAttribute('id', 'audio_sample'+i);
                        temp_iframe.setAttribute('class', 'audio');
                        temp_iframe.setAttribute("controls", "controls");
