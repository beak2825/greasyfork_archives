...
// @grant        GM_getValue
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/556638/HDSky%20%E4%BD%93%E8%82%B2%E6%B2%99%E9%BE%99%E7%AE%A1%E7%90%86%E9%9D%A2%E6%9D%BF.user.js
// @updateURL https://update.greasyfork.org/scripts/556638/HDSky%20%E4%BD%93%E8%82%B2%E6%B2%99%E9%BE%99%E7%AE%A1%E7%90%86%E9%9D%A2%E6%9D%BF.meta.js
// ==/UserScript==
...
(function() {
    ...
    // 归零结算功能：计算每个人需要增加或扣减多少分才能让总分变为0
    function calculateZeroSettlement() {
        console.log('=== 开始归零结算计算 ===');
        ...
        let processedCount = 0; // 处理的帖子数量
        let adjustCount = 0; // 需要调整的帖子数量
        ...
        postsData.forEach(function(postData) {
            ...
            // 获取该用户的历史评分总和（使用RatingManager）
            const ratingSum = RatingManager.getRatingSum(postData.postBody);
            
            // 计算需要的调整：目标是0，所以需要的调整就是当前评分的相反数
            // 如果当前评分是+100（加了100分），那么需要-100（减去100分）才能归零
            // 如果当前评分是-50（扣了50分），那么需要+50（加50分）才能归零
            const adjustmentNeeded = -ratingSum;
            
            // 确定操作类型（增加还是扣减）
            let netProfit = adjustmentNeeded;
            let resultType = 'lose'; // 默认类型
            
            if (adjustmentNeeded > 0) {
                resultType = 'win'; // 需要增加分数
                adjustCount++;
            } else if (adjustmentNeeded < 0) {
                resultType = 'lose'; // 需要扣减分数
                adjustCount++;
            } else {
                resultType = 'push'; // 已经是0，不需要调整
            }
            ...
            const postResult = {
                ...
                netProfit: netProfit,
                winBets: adjustmentNeeded > 0 ? [`归零调整: 当前评分${ratingSum > 0 ? '+' : ''}${ratingSum}，需要${adjustmentNeeded > 0 ? '+' : ''}${adjustmentNeeded}`] : [],
                loseBets: adjustmentNeeded < 0 ? [`归零调整: 当前评分${ratingSum > 0 ? '+' : ''}${ratingSum}，需要${adjustmentNeeded}`] : [],
                pushBets: adjustmentNeeded === 0 ? ['已归零，无需调整'] : [],
                winCount: adjustmentNeeded > 0 ? 1 : 0,
                loseCount: adjustmentNeeded < 0 ? 1 : 0,
                pushCount: adjustmentNeeded === 0 ? 1 : 0,
                totalCount: 1,
                exceedMagic: false,
                bestBonus: 0,
                isZeroMode: true  // 标记为归零模式，告诉autoFillRatingForm不要再次调整
            };

            console.log(`楼层${postData.floorNumber} - ${postData.author}: 当前评分=${ratingSum > 0 ? '+' : ''}${ratingSum}, 调整=${adjustmentNeeded > 0 ? '+' : ''}${adjustmentNeeded}`);
            processedCount++;
        ...
        console.log(`=== 归零结算计算完成 ===`);
        console.log(`共处理 ${processedCount} 个帖子，其中 ${adjustCount} 个需要调整，${processedCount - adjustCount} 个已归零`);
    }
    ...
    function calculateBets() {
        ...
        // 特殊处理：如果关键词是"0"，执行归零操作
        if (keywordsInputValue === '0') {
            calculateZeroSettlement();
            return;
        }
        ...
        // 解析关键词（用空格分隔），并提取每个关键词的赔率
        const rawKeywords = keywordsInputValue.split(/\s+/).filter(k => k.trim() !== '');
        const keywords = []; // 存储解析后的关键词和赔率
        const keywordOdds = []; // 存储每个关键词对应的赔率
        
        rawKeywords.forEach(function(rawKeyword) {
            const trimmed = rawKeyword.trim();
            if (!trimmed) return;
            
            // 检查是否包含[数字]格式的赔率（支持前后有空格的情况）
            // 匹配格式：[数字]关键词 或 [数字] 关键词
            const oddsMatch = trimmed.match(/\[([\d.]+)\]\s*(.+)$/);
            if (oddsMatch) {
                // 有关键词赔率
                const keywordOddsValue = parseFloat(oddsMatch[1]);
                const keywordText = oddsMatch[2].trim(); // 提取赔率后的关键词文本，去除赔率部分
                if (!isNaN(keywordOddsValue) && keywordOddsValue > 0 && keywordText) {
                    // 存储去除赔率后的关键词文本，用于匹配
                    keywords.push(keywordText);
                    keywordOdds.push(keywordOddsValue);
                } else {
                    // 解析失败，使用默认赔率（保留原始文本，但去除可能的赔率标记）
                    // 如果解析失败，可能是格式问题，尝试去除[数字]部分
                    const cleanedKeyword = trimmed.replace(/\[[\d.]+\]\s*/g, '').trim();
                    keywords.push(cleanedKeyword || trimmed);
                    keywordOdds.push(null); // null表示使用默认赔率
                }
            } else {
                // 没有关键词赔率，使用默认赔率（确保去除可能的[数字]标记）
                const cleanedKeyword = trimmed.replace(/\[[\d.]+\]\s*/g, '').trim();
                keywords.push(cleanedKeyword || trimmed);
                keywordOdds.push(null); // null表示使用默认赔率
            }
        ...
        if (deadline) {
            // 给截止时间加6分钟（宽限时间）
            deadline = new Date(deadline.getTime() + 6 * 60 * 1000);
        }
        ...
        postsData.forEach(function(postData) {
            ...
            const matchKeywords = [];
            const matchKeywordOdds = []; // 存储每个匹配关键词对应的赔率
            for (let i = 0; i < betCount; i++) {
                if (i < keywords.length) {
                    matchKeywords.push(keywords[i]);
                    matchKeywordOdds.push(keywordOdds[i]); // 对应的赔率（可能为null）
                } else {
                    matchKeywords.push('');
                    matchKeywordOdds.push(null);
                }
            }
            ...
            const winBetOdds = []; // 每个胜场对应的赔率（用于计算）

            // 检查所有下注，每个下注对应一个关键词
            for (let i = 0; i < betCount; i++) {
                const betText = postData.bets[i];
                const keyword = matchKeywords[i];
                const keywordOddsValue = matchKeywordOdds[i]; // 该关键词的赔率（可能为null）
                ...
                if (checkSingleBetWin(betText, keyword)) {
                    winCountForPost++;
                    // 构建显示文本（如果有关键词赔率，显示赔率）
                    let displayKeyword = keyword;
                    if (keywordOddsValue !== null) {
                        displayKeyword = `[${keywordOddsValue}]${keyword}`;
                    }
                    winBets.push(`第${i + 1}场: ${betText} (关键词: ${displayKeyword})`);
                    // 保存该胜场的赔率（如果有关键词赔率就用关键词赔率，否则用默认赔率）
                    winBetOdds.push(keywordOddsValue !== null ? keywordOddsValue : odds);
                } else {
            ...
            // 计算净收益：每个胜场使用自己的赔率，输场不减赔率
            // 公式：Σ(胜场i * 下注点数 * 赔率i) - 输场数 * 下注点数
            let totalWinProfit = 0;
            for (let i = 0; i < winBetOdds.length; i++) {
                totalWinProfit += postData.betNum * winBetOdds[i];
            }
            const netProfit = Math.round(totalWinProfit - (loseCountForPost * postData.betNum));
    ...
    function autoCheckViolations() {
        ...
        postsData.forEach(post => {
            const violationTypes = ViolationChecker.getPostViolations(post, postsData, deadline);
            
            if (violationTypes.length > 0) {
                violations.push({
                    replyId: post.replyId,
                    floorNumber: post.floorNumber || 0,
        ...
        if (violations.length > 0) {
            ...
            violations.forEach(v => {
                v.violationTypes.forEach(type => {
                    violationTypesCount[type] = (violationTypesCount[type] || 0) + 1;
                });
            });
            
            let message = `⚠️ 发现 ${violationCount} 个违规下注！\n\n`;
            message += '违规类型统计：\n';
            for (const [type, count] of Object.entries(violationTypesCount)) {
            ...
            if (confirm(message)) {
                // 用户选择查看详细信息，打开违规检查弹窗
                ViolationChecker.openDialog(violations);
            }
        }
    }
    ...
    const ViolationChecker = {
        ...
        checkViolationBets() {
            ...
            if (!deadline) {
                ...
                // 给截止时间加6分钟（宽限时间）
                deadline = new Date(deadline.getTime() + 6 * 60 * 1000);
                console.log('提取的截止时间（加6分钟宽限）:', deadline);
            }
    ...
    function batchSettleAll() {
        ...
        calculationResults.forEach(function(calcData, pid) {
            ...
            if (isPostSettled(pid)) {
                ...
                // 归零模式下，netProfit已经是最终需要的调整值，不需要再次调整
                if (!result.isZeroMode && ratingSum !== 0 && ratingSum !== result.netProfit) {
                    // 如果已有评分，需要调整：最终结算 = 计算结果 - 已有评分总和
                    finalAmount = result.netProfit - ratingSum;
                    console.log(`帖子 #${pid}: 非归零模式调整 - 计算结果=${result.netProfit}, 评分总和=${ratingSum}, 最终金额=${finalAmount}`);
                } else if (result.isZeroMode) {
                    console.log(`帖子 #${pid}: 归零模式 - 评分总和=${ratingSum}, 归零调整=${finalAmount}`);
                }
    ...
    function submitBatchesInSequence(formData, batches, submitIframe, options) {
        ...
        function submitNextBatch() {
            ...
            // 重置iframe并等待加载完成后再提交表单
            // 这样可以确保每次都能正确触发load事件
            let loadTimeout;
            let isSubmitted = false;
            
            // 第一步：重置iframe为空白页
            const resetHandler = function() {
                submitIframe.removeEventListener('load', resetHandler);
                
                // 第二步：设置提交后的load监听器
                const submitLoadHandler = function() {
                    if (loadTimeout) {
                        clearTimeout(loadTimeout);
                    }
                    submitIframe.removeEventListener('load', submitLoadHandler);
                    batchIndex++;
                    // 等待150ms后提交下一批（适当减慢速度，确保服务器处理完成）
                    setTimeout(submitNextBatch, 150);
                };
                submitIframe.addEventListener('load', submitLoadHandler);

                // 设置超时处理（如果800ms内没有响应，继续下一批）
            ...
            // 监听iframe重置完成
            submitIframe.addEventListener('load', resetHandler);
            
            // 重置iframe到空白页
            submitIframe.src = 'about:blank';
        }
    ...
    function processBatchSettleForms(forms, currentIndex, totalForms, initialSettledCount) {
        ...
        const onComplete = function() {
            ...
            // 等待150ms后处理下一个表单（适当减慢速度，确保服务器处理完成）
            setTimeout(function() {
    ...
    function displayPostResult(postBody, result) {
        ...
        // 自动将结算分数填入评分表单（归零模式下不进行二次调整）
        const fillResult = RatingManager.autoFillRatingForm(postBody, result.netProfit, result.isZeroMode);
        
        // 如果有调整，显示调整提示（归零模式下不显示）
        if (fillResult && fillResult.adjustment !== 0 && !result.isZeroMode) {
    ...
    const RatingManager = {
        ...
        // 自动将结算分数填入评分表单
        autoFillRatingForm(postBody, netProfit, isZeroMode = false) {
            ...
            // 计算调整值：
            // 归零模式下，netProfit已经是最终需要的调整值，不需要再次调整
            // 非归零模式下，如果评分总和与计算结果不一致，需要加回来
            let adjustedProfit = netProfit;
            let adjustment = 0;
            
            if (!isZeroMode && ratingSum !== 0 && ratingSum !== netProfit) {
                // 计算差值并加回来
    ...
    function autoFillRatingForm(postBody, netProfit, isZeroMode = false) {
        return RatingManager.autoFillRatingForm(postBody, netProfit, isZeroMode);
    }
    ...
    function handleBatchRating(form, markInput) {
        ...
        // 从表单的隐藏字段中获取 pid 和 forumid
        const pidInput = form.querySelector('input[name="pid"]');
        const forumidInput = form.querySelector('input[name="forumid"]');
        ...
        // 构建表单数据对象
        const formData = {
            markdes: markdesSelect.value, // 'add' 或 'sub'
            pid: pidInput.value,
            forumid: forumidInput ? forumidInput.value : getForumId(),
            reason: reason
        };
        ...
        // 使用通用的批次提交函数
        submitBatchesInSequence(formData, batches, submitIframe, {
            onComplete: onComplete,
