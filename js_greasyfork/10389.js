...
// @version        0.3.4
// @date           2011-01-15
// @downloadURL https://update.greasyfork.org/scripts/10389/Cross%20Site%20Torrent%20Search.user.js
// @updateURL https://update.greasyfork.org/scripts/10389/Cross%20Site%20Torrent%20Search.meta.js
// ==/UserScript==
...
/*		Narrow your search queries by removing the // from the beginning of the following lines: */
//		query += '+is:flac';// Search Waffles for FLAC only
//		query += '+is:320';// Search Waffles for 320 MP3 only
//		query += '+is:v0';// Search Waffles for V0 MP3 only
//		query += '+is:scene';// Search Waffles for Scene only
/*		only enable one of the format lines at a time, obviously */
...
/*		Narrow your search queries by removing the // from the beginning of the following lines: */
//		search_url += '&encoding=Lossless&format=FLAC';// Search What for FLAC only
//		search_url += '&encoding=320&format=MP3';// Search What for 320 MP3 only
//		search_url += '&encoding=V0+(VBR)&format=MP3';// Search What for V0 MP3 only
//		search_url += '&scene=1';// Search What for Scene only
/*		only enable one of the format lines at a time, obviously */
		...
		whatcd_proxy.get(search_url, 'text/xml',
			function(response) {
				...
				if (m) {
					m[0]=m[0].replace(/&raquo;/g, '');
					m[0]=m[0].replace(/&minus;/g, '');
					doc = $(new DOMParser().parseFromString('<html xmlns="http://www.w3.org/1999/xhtml"><head><title></title></head><body>'+m[0]+'</body></html>', 'text/xml'));
	...
	if (!GM_getValue('whatcd_url_base')) {
		if (/^https:\/\/what\.cd\//.test(document.URL)) GM_setValue('whatcd_url_base', 'https://what.cd');
	else if (/^https:\/\/ssl.what\.cd\//.test(document.URL)) GM_setValue('whatcd_url_base', 'https://ssl.what.cd');
	}
