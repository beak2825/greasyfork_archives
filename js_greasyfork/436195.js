...
// @match        *://fa.y3api.cn/jssg/h5/zhuzhu.html?*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/436195/jueshi5.user.js
// @updateURL https://update.greasyfork.org/scripts/436195/jueshi5.meta.js
// ==/UserScript==
...
(function () {
    ...
    window.xzhsj = function (e, i, a) {
        if (void 0 === i && (i = null),
            ...
            var n = CryptoJS.enc.Utf8.parse(e)
                ...
                , r = CryptoJS.enc.Base64.stringify(o.ciphertext);
            return r
        }
        return console.log("Encrypt is null"),
            null
    }
    //排序
    ...
    function jszr (ljsy) {
        ...
        window.hook_websock.oksock[ljsy].onmessage = function (msg) {
            var newmsg = msg;
            let strmsg = kit.Decrypt(window.Uint8ArrayToString(new Uint8Array(msg.data)));
            strmsg = window.hook_websock.sjcl(strmsg);
            if (window.tssc == 1) {
                console.log('接收', strmsg);
            }
             let xmsg = window.xzhsj(strmsg, null, null);
             let n1 = new egret.ByteArray();
             n1.writeUTFBytes(xmsg);
             newmsg.data=n1.buffer;
            try {
                mjs.apply(this, [newmsg]);
            } catch (error) {
            }
        }
        let mp3 = "https://134.175.38.223/ts.mp3";
        ...
        cjysock.send = function (msg) {
            if (typeof (msg) == 'string') {
                try {
                    ...
                    let xmsg = window.xzhsj(msg, null, null);
                    let n1 = new egret.ByteArray();
                    n1.writeUTFBytes(xmsg);
                    cjysock.cxsend(n1.buffer);
                    if (window.tssc == 1) {
                        console.log('发送', msg);;
                    }
                } catch (error) {
                ...
                if (window.tssc == 1) {
                    console.log('发送', kit.Decrypt(window.Uint8ArrayToString(new Uint8Array(msg))));;
                }
                cjysock.cxsend(msg);
            }
        };
            ...
            try {
                switch (dx.cmd) {
                    ...
                    case 1:
                        ...
                        $("body").append(
                            '<div class="ui-row-flex ui-whitespace ui-tooltips-action"><button class="ui-btn ui-btn-primary ui-col " type="submit" id="kg">攻击</button><button class="ui-btn ui-btn-primary ui-col " type="submit" id="zdy">运行</button><button class="ui-btn ui-btn-primary ui-col " type="submit" id="kt">开启调试</button><button class="ui-btn ui-btn-primary ui-col " type="submit" id="tp">逃跑</button><button class="ui-btn ui-btn-primary ui-col " type="submit" id="lq">领</button></div></div>'
                        );
                        $("#kg").click(function () {
                        ...
                        $("#zdy").click(function () {
                            let code=as=prompt('code');
                            let yc=prompt('延迟',1000);
                            if ($("#zdy").text() == "运行") {
                                window.jssg.xhzjid = setInterval(function () {
                                    window.hook_websock.oksock[window.dqsy].send(code);
                                }, yc);
                                console.log(window.jssg.xhzjid);
                                $("#kzj").text("停止");
                            } else {
