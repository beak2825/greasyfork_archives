...
// @grant        all
// @downloadURL https://update.greasyfork.org/scripts/390109/Check%20Yth%20Pulls.user.js
// @updateURL https://update.greasyfork.org/scripts/390109/Check%20Yth%20Pulls.meta.js
// ==/UserScript==
		...
		xhr.onreadystatechange = function()
		{
			if (xhr.readyState == 4)
			{
                ...
                return ((Math.pow(2,Math.log(weight*SI)/Math.log(Math.pow(2,7))) - Math.pow(2,Math.log(weight*wage/(wage_rate*0.8))/Math.log(Math.pow(2,7)))).toFixed(1)*10) - 1 ;
                 ...
                 function return_starting_blooming(Last_Season_Avg_Ti,Season_Avg,SI,age,rec,player)
                        {var Age = parseInt(age);
                         var Months =parseInt(player.months);
                         var ID = parseInt(player.player_id);
                         if ((Age == 18)&&(Months >0) && (Months < 4) && (ID > 127657894)) {return "$$$";}
                         if ((Age == 19)&&(Months >0) && (Months < 4) && (ID > 127657894)) {return "$$$";}
                         if ((Age == 20)&&(Months >0) && (Months < 4) && (ID > 127657894)) {return "$$$";}
                         else {return "----";}
                        }
                ...
                $("a[player_link=" + resultado.player.player_id + "]").closest("tr").append("<td class='right align_right SB' style='padding-right:5px;'><span>" + return_starting_blooming(Last_Season_Avg_Ti(return_TI((arrary[id_index(resultado.player.player_id,arrary)][1]),resultado.player.skill_index,player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary))),Season_TI(player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary)),SI(resultado.player.skill_index),return_wage(resultado.player.wage),19.836),session,parseInt(resultado.player.months),parseInt(resultado.player.player_id),return_wage(resultado.player.wage)),return_TI((arrary[id_index(resultado.player.player_id,arrary)][1]),resultado.player.skill_index,player_postion(transfer_info_ar,id_index(resultado.player.player_id,arrary))),resultado.player.skill_index,resultado.player.age,resultado.player.rec_sort,resultado.player)  + "</span></td>");
