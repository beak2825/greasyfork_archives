...
(function() {
    ...
    const PinyinMatch = (function() {
        ...
        function g(i, o) {
            if (!i || !o) return !1;
            i = i.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""), o = function(n) {
                var i = n.replace(/\s+/g, "").toLowerCase();
                return t.some((function(n) {
                    return i.includes(n)
                })) && (i = i.replace("ü", "v")), i.normalize("NFD").replace(/[\u0300-\u036f]/g, "")
            }(o);
            var r = i.indexOf(o);
            if (-1 !== r) return [r, r + o.length - 1];
            var g = h(i.split(""), [o.split("")], o);
            if (g) return g;
            var f, l, c = function(n) {
            ...
            return h(c, u[o] || (l = [], e(f = o).forEach((function(n) {
                var i = n.split(" "),
                    a = i.length - 1;
                i[a].indexOf(",") ? i[a].split(",").forEach((function(n) {
                    i.splice(a, 1, n), l.push(JSON.parse(JSON.stringify(i)))
                })) : l.push(i)
            })), 0 !== l.length && l[0].length === f.length || l.push(f.split("")), u = n({}, f, l), l), o)
        }
        ...
        var f = {
            match: function(n) {
                ...
                return Object.keys(n).forEach((function(a) {
                    ...
                    var i = {};
                    for (var a in n)
                        for (var u = n[a], e = 0, o = u.length; o > e; e++) i[u[e]] = i[u[e]] ? i[u[e]] + " " + a : a;
                    return i
                }(u), g
            }({
                a: "阿啊呵腌嗄吖锕",
    ...
    function parseEhTagDB(rawObj) {
        ...
        const nsList = Array.isArray(rawObj.data) ? rawObj.data :
            (rawObj.data ? Object.entries(rawObj.data).map(([k, v]) => ({
                namespace: k,
                data: v
            })) : []);
    ...
    function loadDBViaJSONP() {
        ...
        jsonpLoading = true;
        if (translationDB && jsonpLoadedOnce) {
            jsonpLoading = false;
            return;
        }

        try {
            delete window.load_ehtagtranslation_db_text;
        } catch (e) {
            window.load_ehtagtranslation_db_text = undefined;
        }
    ...
    function setupSearchSuggestions() {
        ...
        let hideTimeout;
        let debounceTimeout; // 新增：防抖定时器

        searchInput.addEventListener('input', (e) => {
            const val = e.target.value;
            if (!translationDB) return;

            // 性能优化1：添加防抖，避免每次击键都触发全库搜索
            ...
            debounceTimeout = setTimeout(() => {
                ...
                // 性能优化2：检测输入是否包含中文
                // 如果输入已经包含中文，说明用户不是在拼音检索，跳过昂贵的 PinyinMatch 计算
                const hasChinese = /[\u4e00-\u9fa5]/.test(currentTerm);
                ...
                for (const ns in translationDB) {
                    ...
                    for (const key in nsObj) {
                        ...
                        // 优化：仅在不包含中文时才进行拼音匹配
                        else if (!hasChinese && PinyinMatch.match(trans, currentTerm)) matchType = 5;
