...
// @grant        none
// @license      GNU GPLv3
// @downloadURL https://update.greasyfork.org/scripts/459258/EasyWJX-nginx.user.js
// @updateURL https://update.greasyfork.org/scripts/459258/EasyWJX-nginx.meta.js
// ==/UserScript==

(function() {
    'use strict';
    // Config
    var EasyWJX_version = '2.0.9';
    ...
    async function initElement() {
        ...
        setInterval(function () {
            changeWechatLoginUrl();
        },1000)

        // 修改getwxcode页面的script
        checkAndReplaceGetwxcode();
    ...
    // 修改微信验证后的跳转地址
    function changeWechatLoginUrl() {
        var all_a = $('a[href]')
        for (var i=0; i<all_a.length; i++) {
            if (all_a[i].href.indexOf('ks.wjx.top')) {
                all_a[i].href = all_a[i].href.replace('ks.wjx.top','easywjx.kzw.ink').replace('www.wjx.cn','easywjx.kzw.ink')
            }
        }
    }

    // 检查当前是否在getwxcode页面并修改替换script
    function checkAndReplaceGetwxcode() {
        if (location.pathname == '/html/getwxcode.html'){
            $('script').remove()
            $('body').append($(`<script>var GWC={urlParams:{},appendParams:function(params,url){var baseWithSearch=url.split('#')[0];var hash=url.split('#')[1];for(var i=0;i<params.length;i++){if(params[i].value!==undefined){var newParam=params[i].key+"="+params[i].value;if(baseWithSearch.indexOf('?')>0){var oldParamReg=new RegExp(params[i].key+'=[-\\w]{0,40}','g');if(oldParamReg.test(baseWithSearch)){baseWithSearch=baseWithSearch.replace(oldParamReg,newParam)}else{baseWithSearch+="&"+newParam}}else{baseWithSearch+="?"+newParam}}}if(hash){url=baseWithSearch+'#'+hash}else{url=baseWithSearch}return url},getUrlParams:function(){var pairs=location.search.substring(1).split('&');for(var i=0;i<pairs.length;i++){var pos=pairs[i].indexOf('=');if(pos===-1){continue}GWC.urlParams[pairs[i].substring(0,pos)]=decodeURIComponent(pairs[i].substring(pos+1))}},doRedirect:function(){var code=GWC.urlParams['code'];var key="code";if(!code){code=GWC.urlParams['auth_code'];key="auth_code"}var appId=GWC.urlParams['appid'];var scope=GWC.urlParams['scope']||'snsapi_base';var state=GWC.urlParams['state']||"";var redirectUri;if(!code)return;if(code=="authdeny"||code.indexOf("this_is")>-1){return}redirectUri=GWC.appendParams([{key:key,value:code},{key:'state',value:state}],GWC.urlParams['redirect_uri']);if(redirectUri.indexOf("https://")!=0&&redirectUri.indexOf("http://")!=0)return;location.href=redirectUri}};GWC.getUrlParams();GWC.doRedirect();</script>`))
        }
    }
