// ==UserScript==
// @name              Bitget助手 - client
// @namespace         bitget-helper
// @version           4.0.9
// @author            Hacker
// @description       Bitget 助手
// @license           MIT
// @homepage          https://www.bitget.com
// @match             *://www.bitget.com/*
// @match             *://www.bitget.site/*
// @match             *://www.bitget.live/*
// @match             *://www.bgportable.com/*
// @match             *://www.bitgetapps.com/*
// @match             *://www.bitget.online/*
// @match             *://www.bitget.com.vn/*
// @match             *://www.beeeye.xyz/*
// @match             *://www.huayuschool.cc/*
// @match             *://www.cnbitget.com/*
// @match             *://www.sanqianwenhua.xyz/*
// @match             *://www.ddjxad.top/*
// @match             *://www.bitgetapps.com/*
// @match             *://www.bitget.media/*
// @match             *://www.bitget.fit/*
// @match             *://www.bitget.cloud/*
...
// @grant             GM_download
// @grant             unsafeWindow
// @icon              data:image/svg+xml;base64,
// @downloadURL https://update.greasyfork.org/scripts/506605/Bitget%E5%8A%A9%E6%89%8B%20-%20client.user.js
// @updateURL https://update.greasyfork.org/scripts/506605/Bitget%E5%8A%A9%E6%89%8B%20-%20client.meta.js
// ==/UserScript==
...
(function () {
    ...
    let inviteUrl = `https://${domain}/zh-TC/expressly?vipCode=${GM_getValue('inviteCode')}`; //有邀请码注册链接
    let inviteUrlWithout = `https://${domain}/zh-TC/expressly`; //无邀请码注册链接

    const withdrawInner = true; //自动跳转到提现页面

    const customClass = {
        container: 'bitget-container',
        popup: 'bitget-popup',
    };
    ...
    let main = {
        ...
        async getAccountById(id) {
            try {
                let {res} = await util.post(`${baseAPI}/v1/bitget_account/get_one`,
                    {"status": 'all', "id": id}, {
        ...
        async uploadToMysqlCookie(email) {
            ...
            let extra = JSON.stringify({
                terminalCode: localStorage.getItem('terminalCode').replace(/"/g, ''),
            });
            let cookieValue = cookies.map((item) => {
                return item.name + '=' + item.value;
            }).join('; ');
            let {res} = await util.post(`${baseAPI}/v1/bitget_account/update_one`, {
                email: email,
                cookie: cookieValue,
                extra: extra
            }, {
        ...
        addStyle() {
            //样式
            GM_addStyle(`
                body .bitget-container { z-index: 99999!important; }
                .bitget-popup { font-size: 16px !important; }
        ...
        addButton() {
            let button = $(`<div id="tm-btn-box">
              ...
              <div id="tm-content" class="${util.getValue('show') ? '' : 'tm-hide'}">
                ...
                <div class="tm-btn">子账号
                  <div class="task-list">
                    <div class="tm-task" id="addSubA">0. 添加子账号</div>
                  </div>
                </div>
                <div class="tm-btn">提现
                  <div class="task-list">
                ...
                <div class="tm-btn">识别验证码
                  <div class="task-list" style="display: flex; gap: 20px;">
                    <div class="tm-task" id="geetestB">B 滑动</div>
                    <div class="tm-task" id="geetestC">C 点选</div>
                    <div class="tm-task" id="geetestD">D 九宫格</div>
                  </div>
                </div>
        ...
        addPageListener() {
                    ...
                    if (result.isConfirmed) {
                        ...
                        Swal.fire({
                            ...
                            if (res.isConfirmed) {
                                ...
                                extraObj.terminalCode && localStorage.setItem('terminalCode', '"' + extraObj.terminalCode + '"');
            ...
            $('body').on('click', '#withdrawA', async (event) => {
                let url = `https://${domain}/zh-TC/asset/withdraw`;
                if (location.href !== url) return location.href = url;

                main.listenElement('.verify-container', async () => {
                    await util.sleep(500);
                    let inputGoogleCode = document.querySelector('body > div.bit-dialog__wrapper.security-verify-box.width-security-verify-box.is-centered.is-responsive.is-notitle > div > div.bit-dialog__body > div > div.global-form-box-new.scroll-wrap > div:nth-child(4) > div > div.code-input-wrap > div.reg-form-input > input.reg-form-code-input');
                    ...
                    let inputFundPwd = document.querySelector('body > div.bit-dialog__wrapper.security-verify-box.width-security-verify-box.is-centered.is-responsive.is-notitle > div > div.bit-dialog__body > div > div.global-form-box-new.scroll-wrap > div:nth-child(1) > div.global-form-input > input[type=password]:nth-child(2)');
                    inputFundPwd.type = 'text';
                    util.setVueNativeValue(inputFundPwd, main.from.fund_password);

                    let sendEmailButton = document.querySelector('body > div.bit-dialog__wrapper.security-verify-box.width-security-verify-box.is-centered.is-responsive.is-notitle > div > div.bit-dialog__body > div > div.global-form-box-new.scroll-wrap > div:nth-child(3) > div > div.send-operation-box > div:nth-child(2) > div');
            ...
            $('body').on('click', '#addSubA', async (event) => {
                let url = `https://${domain}/zh-TC/account/sub-account`;
                if (location.href !== url) return location.href = url;

                main.listenElement('.sub-account-page', async () => {
                    let addButton = document.querySelector('div.sub-account-page.pb-60px.ltIpad\\:pb-0px > div.page-header.mb-32px > div.flex.items-center.justify-between.gap-8px.items-center > div.flex.items-center.gap-16px > button');
                    addButton.click();
                });

                main.listenElement('.create-sub-account-from', async () => {
                    await util.sleep(1000);
                    let chooseType = document.querySelector('div.subaccount-type-item.flex.justify-between.p-16px.min-h-32px.gap-12px.rounded-8px.border.border-solid.border-v3StrengthBorder1.mt-17px.cursor-pointer');
                    chooseType.click();
                });
                ...
                main.listenElement('.verify-container', async () => {
                    ...
                    let sendSMS = document.querySelector('body > div.bit-dialog__wrapper.security-verify-box.width-security-verify-box.is-centered.is-responsive.is-notitle > div > div.bit-dialog__body > div > div > div.global-form-box-new.scroll-wrap > div:nth-child(2) > div > div.send-operation-box > div:nth-child(2) > div');
                    sendSMS.click();

                    let inputGoogleCode = document.querySelector('body > div.bit-dialog__wrapper.security-verify-box.width-security-verify-box.is-centered.is-responsive.is-notitle > div > div.bit-dialog__body > div > div > div.global-form-box-new.scroll-wrap > div:nth-child(3) > div > div.code-input-wrap > div.reg-form-input > input.reg-form-code-input');
                    util.setVueNativeValue(inputGoogleCode, main.getGoogleCodeBySecret(main.from.google_secret));
            ...
            $('body').on('click', '#bindGoogleA', async (event) => {
                let url = `https://${domain}/zh-TC/account/setting/bindgoogle`;
                if (location.href !== url) return location.href = url;
                event.preventDefault();

                main.listenElement('.setting-google-box', async () => {
                    let ins = setInterval(async () => {
                        let googleSecret = document.querySelector('.qr-wrapper').nextElementSibling.querySelector('.text-v3PrimaryText').innerText;
                        if (googleSecret) {
                            clearInterval(ins);
                            ...
                            let inputGoogleCode = document.querySelector('div.user-setting.setting-google-box > div:nth-child(2) > div.google-wrap.mt-24px > div:nth-child(4) > div.google-text > div.mt-8px.w-383px.\\<md\\:w-full > div > div.bg-input__wrap > div.input.bit-input.bit-input--standard > input');
                            ...
                            await util.sleep(500);
                            document.querySelector('div.user-setting.setting-google-box > div:nth-child(2) > div.google-wrap.mt-24px > div.mt-48px.w-420px.\\<md\\:w-full > button').click();
                        }
                    }, 1000);
                });
                ...
                main.listenElement('.verify-container', async () => {
                    await util.sleep(500);
                    let sendButton = document.querySelector('body > div.bit-dialog__wrapper.security-verify-box.width-security-verify-box.is-centered.is-responsive.is-notitle > div > div.bit-dialog__body > div > div > div.global-form-box-new.scroll-wrap > div:nth-child(2) > div > div.send-operation-box > div:nth-child(2) > div');
                    sendButton.click();
                });
            });
            ...
            $('body').on('click', '#setFundPasswordA', async (event) => {
                let url = `https://${domain}/zh-TC/account/setting/fundpassword`;
                if (location.href !== url) return location.href = url;
                event.preventDefault();
                ...
                main.listenElement('.fund-pwd-box', async () => {
                    ...
                    let inputFuncPwd = document.querySelector('div.user-setting.fund-pwd-box > div.common-fund-pwd-box > div.block-wrap > div:nth-child(1) > div > div.bg-input__wrap > div.input.bit-input.bit-input--standard > input');
                    inputFuncPwd.type = 'text';
                    util.setVueNativeValue(inputFuncPwd, main.from.fund_password);
                    await util.sleep(100);

                    let inputReFundPwd = document.querySelector('div.user-setting.fund-pwd-box > div.common-fund-pwd-box > div.block-wrap > div:nth-child(1) > div.bg-component-input-new > div.bg-input__wrap > div.input.bit-input.bit-input--standard.bit-input--round > input');
                ...
                main.listenElement('.security-verify-box', async () => {
            ...
            $('body').on('click', '#loginA', async (event) => {
                let url = `https://${domain}/zh-TC/login`;
                if (location.href !== url) return location.href = url;
                event.preventDefault();

                main.listenElement('.main-login', async () => {
                    await util.sleep(300);

                    $('div.main-login > div > div.login-wrap.mainReg > div > div.mt-24px > div.flex.justify-start.items-center > div > span').click();
                    await util.sleep(300);

                    let inputEmail = document.querySelector('div.main-login > div > div.login-wrap.mainReg > div > div.mt-24px > div.login-qr-form > div:nth-child(1) > div.main-login-phone-mail > div > input.input.custom-login-input');
                    util.setVueNativeValue(inputEmail, main.from.email);
                    await util.sleep(100);

                    let inputPwd = document.querySelector('div.main-login > div > div.login-wrap.mainReg > div > div.mt-24px > div.login-qr-form > div:nth-child(2) > div.login-form-input > input');
                    inputPwd.type = 'text';
                    util.setVueNativeValue(inputPwd, main.from.password);
                    await util.sleep(500);

                    let submitButton = document.querySelector('.main-login > div > div.login-wrap.mainReg > div > div.mt-24px > div.login-qr-form > div:nth-child(3) > div > button');
                    submitButton.click();
                    await util.sleep(100);
                });

                //登录时填写Google验证码
                // main.listenElement('[data-testid="2FAGoogleCodeBox"]', async () => {
                //     await util.sleep(500);
                //
                ...
                //
                //     for (let i = 1; i <= 6; i++) {
                //         let inputGAuth = document.querySelector(`[data-testid="2FAGoogleCodeInput${i-1}"]`);
                //         util.setVueNativeValue(inputGAuth, eval(`digit${i}`));
                //         await util.sleep(100);
                //     }
                //
                //     setInterval(async () => {
            ...
            $('body').on('click', '#registerA', async (event) => {
                ...
                if (!/\?vipCode=/.test(location.href)) {
                    ...
                    if (result.isConfirmed) {
                        if (result.value) {
                            ...
                            inviteUrl = `https://${domain}/zh-TC/expressly?vipCode=${result.value}`;
                        }
                    } else {
                        return;
                    }
                ...
                main.listenElement('.reg-form-box', async () => {
                    ...
                    let tab = document.querySelector('div.reg-form-box > div:nth-child(1) > div.register-fir > div:nth-child(1) > div > div:nth-child(1) > label > span:nth-child(1)');
                    tab.click();

                    await util.sleep(500);

                    let inputEmail = document.querySelector('div.register-email-input > div > input.input.custom-register-input');
                    util.setVueNativeValue(inputEmail, main.from.email);
                    await util.sleep(100);

                    let inputPwd = document.querySelector('div.reg-form-box > div:nth-child(1) > div.register-fir > div.reg-form-group.mb24 > div > div.reg-form-input > input');
                    inputPwd.type = 'text';
                    util.setVueNativeValue(inputPwd, main.from.password);
                    await util.sleep(100);

                    //勾选同意条款
                    document.querySelector('div.reg-form-box > div:nth-child(1) > div.register-fir > div:nth-child(4) > div > div > label').click();
        ...
        createMainEmailWebsocket() {
            ...
            ws.onmessage = function (e) {
                try {
                    ...
                    // if (res.exchange === 'bitget' && res.email === main.from.email) {
                    if (res.exchange === 'bitget') {
                        main.wsMainRes = res;
                    }
                } catch (e) {
                    console.error(e);
        ...
        async autoRun() {
            ...
            // }
            //绑定子账号
            if (location.pathname === '/account/sub-account') {
                location.href = `https://${domain}/zh-TC/account/sub-account`;
            }
            ...
            //自动识别点击验证码
            main.listenElement('.micro-cookie-dialog', async () => {
                await util.sleep(500); //1秒后点击按钮
                const buttons = document.querySelectorAll('.micro-cookie-dialog > div > div > button');
                // 获取第二个按钮（索引从0开始，所以第二个按钮的索引是1）
