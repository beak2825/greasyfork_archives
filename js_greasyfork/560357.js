...
// @downloadURL https://update.greasyfork.org/scripts/560357/openplace%20Overlay%20Pro.user.js
// @updateURL https://update.greasyfork.org/scripts/560357/openplace%20Overlay%20Pro.meta.js
// ==/UserScript==
(() => {
  // src/core/gm.ts
  ...
  var DEFAULT_FREE_KEYS = openplace_FREE.map(([r, g, b]) => `${r},${g},${b}`);
  ...
  // src/core/overlay.ts
  var ALL_COLORS = [...openplace_FREE, ...openplace_PAID];
  ...
  function matchTileUrl(urlStr) {
    try {
      ...
      if (u.hostname !== "openplace.live" || !u.pathname.startsWith("/files/")) return null;
  ...
  function matchPixelUrl(urlStr) {
    try {
      const u = new URL(urlStr, location.href);
      if (u.hostname !== "openplace.live") return null;
      const m = u.pathname.match(/\/s0\/pixel\/(\d+)\/(\d+)$/);
      if (!m) return null;
      const sp = u.searchParams;
      return { normalized: `https://openplace.live/s0/pixel/${m[1]}/${m[2]}?x=${sp.get("x") || 0}&y=${sp.get("y") || 0}` };
    } catch {
      return null;
    }
  }
  ...
  function renderPaletteGrid() {
    ...
    for (const [r, g, b] of openplace_FREE) {
      const key = `${r},${g},${b}`;
      const cell = document.createElement("div");
      cell.className = "op-cc-cell";
      cell.style.background = `rgb(${r},${g},${b})`;
      cell.title = openplace_NAMES[key] || key;
    ...
    for (const [r, g, b] of openplace_PAID) {
      const key = `${r},${g},${b}`;
      const cell = document.createElement("div");
      cell.className = "op-cc-cell";
      cell.style.background = `rgb(${r},${g},${b})`;
  ...
  function isAllPaidActive() {
    const allPaidKeys = openplace_PAID.map(([r, g, b]) => `${r},${g},${b}`);
  ...
  function setAllActive(type, active) {
    if (type === "free") {
      ...
      const keys = openplace_PAID.map(([r, g, b]) => `${r},${g},${b}`);
        ...
        <div class="op-section" id="op-editor-section">
          ...
          <div id="op-editor-body">
            ...
            <div class="op-row" id="op-cc-btn-row" style="display:none; justify-content:space-between; gap:8px; flex-wrap:wrap;">
              ...
              <button class="op-button" id="op-open-cc" title="Match colors to openplace palette">Color Match</button>
            </div>
