// ==UserScript==
// @name        DTF-Album 2.0 (Firefox. Бета-версия скрипта)
...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/443976/DTF-Album%2020%20%28Firefox%20%D0%91%D0%B5%D1%82%D0%B0-%D0%B2%D0%B5%D1%80%D1%81%D0%B8%D1%8F%20%D1%81%D0%BA%D1%80%D0%B8%D0%BF%D1%82%D0%B0%29.user.js
// @updateURL https://update.greasyfork.org/scripts/443976/DTF-Album%2020%20%28Firefox%20%D0%91%D0%B5%D1%82%D0%B0-%D0%B2%D0%B5%D1%80%D1%81%D0%B8%D1%8F%20%D1%81%D0%BA%D1%80%D0%B8%D0%BF%D1%82%D0%B0%29.meta.js
// ==/UserScript==
/* jshint esversion:8 */
...
class AlbumItem{
  constructor({imgUrl, imgSize, imgLinks, imgTitle, target}){
    ...
    this.i.loading = 'lazy';
    if(!mode.browser.isFirefox) this.i.style.zoom = '100%';
    if(mode.browser.isFirefox) this.i.style.transform = 'scale(1.00)';
    this.i.onwheel = function(s){
        if(focused && button1Pressed && !mode.browser.isFirefox){
            s.preventDefault();
            s.stopPropagation();
            s.stopImmediatePropagation();
              ...
              console.log(s)
                ...
                if(s.deltaY < 0 && button1Pressed){
                    if(!s.target.style.transform){
                        ...
                        if(mode.smartZoom) focused.scrollTo(s.x, s.y);
                        imagePreviewer.zoom.textContent = `${imagePreviewerElements.zoomLevel}${s.target.style.transform.replace(firefoxZoomFilter, '$1')}x`;
                    }else
                    if(s.target.style.transform){
                        console.log('Scale', s.target.style.transform, s.target.style.transform.replace(firefoxZoomFilter, '$1'))
                        s.target.style.transform = `scale(${+s.target.style.transform.replace(firefoxZoomFilter, '$1') + 0.25})`;
                        if(mode.smartZoom) focused.scrollTo(s.x, s.y);
                        imagePreviewer.zoom.textContent = `${imagePreviewerElements.zoomLevel}${s.target.style.transform.replace(firefoxZoomFilter, '$1')}x`;
                    }
                }else
                if(s.deltaY > 0 && button1Pressed){
                    ...
                    if(s.target.style.transform){
                        if(+s.target.style.transform.replace(firefoxZoomFilter, '$1') > 0.25){
                            s.target.style.transform = `scale(${+s.target.style.transform.replace(firefoxZoomFilter, '$1') - 0.25})`;
                            if(mode.smartZoom) focused.scrollTo(s.x, s.y);
                            imagePreviewer.zoom.textContent = `${imagePreviewerElements.zoomLevel}${s.target.style.transform.replace(firefoxZoomFilter, '$1')}x`;
                        }
...
class AlbumPreview{
  constructor({previewer, target}){
    ...
    this.bL.onclick = function(){
        if(focused && !main.browser.isFirefox){
        ...
        if(focused && main.browser.isFirefox){
            if(focused.previousElementSibling){
                if(focused.classList.value.match(/zoomed/)){
                    focused.scrollTo(0, 0);
                    focused.children[1].style.transform = 'scale(1)';
                    focused.classList.remove('zoomed');
                }
                imagePreviewer.zoom.textContent = `${imagePreviewerElements.zoomLevel}${focused.children[1].style.transform.replace(firefoxZoomFilter, '$1')}x`;
                focused.classList.remove('picked');
                focused.previousElementSibling.focus();
            }else
            if(!focused.previousElementSibling){
    ...
    this.bR.onclick = function(){
        if(focused && !mode.browser.isFirefox){
        ...
        if(focused && mode.browser.isFirefox){
            if(focused.nextElementSibling){
                ...
                imagePreviewer.zoom.textContent = `${imagePreviewerElements.zoomLevel}${focused.children[1].style.transform.replace(firefoxZoomFilter, '$1')}`;
                focused.classList.remove('picked');
                focused.nextElementSibling.focus();
            }else
            if(!focused.nextElementSibling){
    ...
    this.buttonClose.onclick = function(e){
      if(focused){
        ...
        if(document.querySelector(`div[class='albumPreview-field']`)){
          ...
          if(focused.classList.value.match(/zoomed/)){
              focused.scrollTo(0, 0);
              if(!mode.browser.isFirefox) focused.children[1].style.zoom = '100%';
              if(mode.browser.isFirefox) focused.children[1].style.transform = 'scale(1)';
              focused.classList.remove('zoomed');
          }
          focused.blur();
          focused = false;
  ...
  if(e.code === mode.buttonEsc){
    if(focused){
      ...
      if(document.querySelector(`div[class='albumPreview-field']`)){
        ...
        focused.parentNode.parentNode.classList.remove('preview-opened');
        document.body.classList.remove('blockScroll');
        if(focused.classList.value.match(/zoomed/) && !mode.browser.isFirefox){
            focused.scrollTo(0, 0);
            focused.children[1].style.zoom = '100%';
            focused.classList.remove('zoomed');
        }else
