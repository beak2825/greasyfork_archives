// ==UserScript==
// @name   🌱【免费版】中医健康管理学院学习管理系统|如需自动下一集、自动换课程、自动答题等高级功能见收费版本：https://doc.zhanyc.cn/pages/zyjk/
// @namespace    http://jb.zhanyc.cn/
// @icon    https://js.zhanyc.cn/img/js-logo.svg
// @version      1.1
// @description  当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制功能。如需自动下一集、自动换课程、自动答题、全自动无人值守高级功能可升级付费版本，一杯咖啡钱，保你无忧学习，且永久使用|接各类脚本开发、代挂工作，微信：zhanyc_cn 备用微信:zhanfengkuo 个人网站：http://doc.zhanyc.cn
// @author       zfk
// @include    *://*yxlearning.com/*
// @grant       GM_getValue
// @grant       GM_setValue
// @grant       GM_addStyle
...
// @antifeature 
// @license GPL
// @downloadURL https://update.greasyfork.org/scripts/512019/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E4%B8%AD%E5%8C%BB%E5%81%A5%E5%BA%B7%E7%AE%A1%E7%90%86%E5%AD%A6%E9%99%A2%E5%AD%A6%E4%B9%A0%E7%AE%A1%E7%90%86%E7%B3%BB%E7%BB%9F%7C%E5%A6%82%E9%9C%80%E8%87%AA%E5%8A%A8%E4%B8%8B%E4%B8%80%E9%9B%86%E3%80%81%E8%87%AA%E5%8A%A8%E6%8D%A2%E8%AF%BE%E7%A8%8B%E3%80%81%E8%87%AA%E5%8A%A8%E7%AD%94%E9%A2%98%E7%AD%89%E9%AB%98%E7%BA%A7%E5%8A%9F%E8%83%BD%E8%A7%81%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%EF%BC%9Ahttps%3Adoczhanyccnpageszyjk.user.js
// @updateURL https://update.greasyfork.org/scripts/512019/%F0%9F%8C%B1%E3%80%90%E5%85%8D%E8%B4%B9%E7%89%88%E3%80%91%E4%B8%AD%E5%8C%BB%E5%81%A5%E5%BA%B7%E7%AE%A1%E7%90%86%E5%AD%A6%E9%99%A2%E5%AD%A6%E4%B9%A0%E7%AE%A1%E7%90%86%E7%B3%BB%E7%BB%9F%7C%E5%A6%82%E9%9C%80%E8%87%AA%E5%8A%A8%E4%B8%8B%E4%B8%80%E9%9B%86%E3%80%81%E8%87%AA%E5%8A%A8%E6%8D%A2%E8%AF%BE%E7%A8%8B%E3%80%81%E8%87%AA%E5%8A%A8%E7%AD%94%E9%A2%98%E7%AD%89%E9%AB%98%E7%BA%A7%E5%8A%9F%E8%83%BD%E8%A7%81%E6%94%B6%E8%B4%B9%E7%89%88%E6%9C%AC%EF%BC%9Ahttps%3Adoczhanyccnpageszyjk.meta.js
// ==/UserScript==
(function () {
    ...
    let freeTips = " 当前是免费版本，只包含了视频页面自动播放、解除播放暂停限制功能。如需自动下一集、自动换课程、自动答题、全自动无人值守等高级功能，可点击下方按钮查看付费版本"
    let docUrl = "https://doc.zhanyc.cn/pages/zyjk/";
    let plugMain = Object.assign(baseConfig, {
        ...
        async page_video() {
            ...
            const checkTimeout = 1 * 60;
            let checkTimedown = checkTimeout;
            let checkVideo = -1;
            plugMain.pageData.video.index = setInterval(async () => {
                try {
                    ...
                    //   return;
                    // }
                    let curItem = plugMain.getCurNavItem();
                    if (curItem.find(".video-icon").length == 0) {
                        if (lastNavItem == null || lastNavItem != curItem[0]) {
                            layer.msg("检测到进入非视频页面，等待15秒继续");
                            plugMain.pageData.waitTime = 15;
                            lastNavItem = curItem[0];
                            return;
                        } else {
                            lastNavItem = null;
                            plugMain.clickNextVideoNavItem();
                        }
                    }
                    $("title").text(title);
                    ...
                    if (curVideoQT && curVideoQT != "流畅") {
                        zfk
                            .getElByText($(".vjs-menu-item-text"), "流畅", "eq", false)
                            .click();
                    }
                    checkTimedown -= timeout;
                    if (checkTimedown < 0) {
                        checkTimedown = checkTimeout;
                        if (checkVideo == plugMain.getCurTime()) {
                            plugMain.pageData.waitTime = 10;
                            plugMain.writeOnlineInfoLog("卡死了:" + title, true);
                            plugMain.confirmRun("卡死了？3秒后返回").then((a) => {
                                $(".return-icon-a")[0].click();
                            });
                        } else {
                            checkVideo = plugMain.getCurTime();
                        }
                    }
                } catch (e) {
        ...
        getCurNavItem() {
            let $el = null;
            $("#tital_vue li").each((i, el) => {
                if (!!$(el).attr("style") && $(el).attr("style").length > 1) {
                    $el = $(el);
                    return false;
                }
            });
        ...
        async runByUrl(url) {
            if (url.includes("pc/course/viewCourseVideoTemple.do")) {
                plugMain.page_video();
            } else if (url.includes("/web/studycenter/studyCenter.html")) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
            } else if (
                url.includes("/web/coursestudy/study/courseStudy.html") &&
                url.includes("homework")
            ) {
                plugMain.showPaidContent(`免费版本不包含答题功能，如需使用请安装收费版本`);

            } else if (url.includes("/web/index.html")) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
            } else if (url.includes("/web/coursedetail/courseDetailJsp.html")) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
            } else if (url.includes("/#/indexList")) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
            } else if (
                location.href.includes("/web/institutionCorrect/work/workSee.html")
            ) {
                plugMain.showPaidContent(`免费版本不包含答题功能，如需使用请安装收费版本`);

            } else if (
                location.href.includes(
                    "/web/questions/homework/showWorkByWorkId.html"
                ) ||
                location.href.includes(
                    "/web/questions/homework/showWorkByWorkIdNoAnswer.html"
                )
            ) {
                plugMain.showPaidContent(`免费版本不包含答题功能，如需使用请安装收费版本`);
                ;
            } else if (location.href.includes("/homeworkDetails.html")) {
                plugMain.showPaidContent(`免费版本不包含自动换课、无人值守功能，如需使用请安装收费版本`);
            }
