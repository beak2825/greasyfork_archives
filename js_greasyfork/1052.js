...
// @include        http*://*atlantis.pennergame.de/profil/bande*
// @include        http*://*vatikan.pennergame.de/profil/bande*
// @include        http*://*malle.pennergame.de/profil/bande*
...
// @version        4.6.1 Korrektur: Hamburg lief nicht ehr
// @downloadURL https://update.greasyfork.org/scripts/1052/Bandenprofil%20Deluxe%20%20PG%2040%20Hamburg%20%2B%20Berlin%20%2B%20Muenchen.user.js
// @updateURL https://update.greasyfork.org/scripts/1052/Bandenprofil%20Deluxe%20%20PG%2040%20Hamburg%20%2B%20Berlin%20%2B%20Muenchen.meta.js
// ==/UserScript==

// @version        4.6.0 Umstellung auf https
// @version        4.5.0 Grafiken auf anderen Server verlegt
// @version        4.4.4 Erweiterung um Vatikan und Atlantis
// @version        4.4.3 Erweiterung um Malle; Update-Verfahren geändert
// @version        4.4.2 Skript lief nicht mehr
// @version        4.4.1 Umzug auf greasyfork.org; Problem  mit Gästebuch behoben
...
// @version        4.1.0 Skript nach Patch wieder zum laufen gebracht
...
// @version        $Id: bandenprofil_deluxe_pg_4.user.js 200 2018-04-10 22:17:56Z mkl $
...
var THISSCRIPTINSTALL_URLGF = "https://greasyfork.org/scripts/1052-bandenprofil-deluxe-pg-4-0-hamburg-berlin-muenchen";
            ...
            onload:function(responseDetails) {
                ...
                if (responseDetails.status == 200) {
                    ...
                    if (scriptversion != THISSCRIPTVERSION) {
                        ...
                        window.location.href = THISSCRIPTINSTALL_URLGF+'/code/Bandenprofil%20Deluxe%20%20PG%2040%20Hamburg%20+%20Berlin%20+%20Muenchen.user.js';
                    }
                }
            }
        });
...
function imglink(pic) {
	var linkbase = 'http://fs5.directupload.net/images/user/170725/';
	var images = ['bgeven', '5rx52ij4.png',
				  'bghead', '8mr8sww6.png', 
				  'bgodd', '6xp6y3x8.png',
				  'bgred', '8lt6t2w6.png',
				  'female', 'ciyhi3d7.png',
				  'male', 'ca879vpx.png',
				  'offline', 'kk4ee98u.png',
				  'premium', '6slcd7g9.png',
				  'urlaub', '8wdyzm3m.png'];

	var i = images.indexOf(pic);
	if (i < 0)
		return '';
	return linkbase + images[i+1];
}
...
var statics = wlp + '//static.pennergame.de/img/pv4/'
...
var sStrings = [wlp + '//pennergame', 'berlin.pennergame', 'muenchen.pennergame',
				'halloween.pennergame', 'koeln.pennergame', 'reloaded.pennergame', 
				'sylt.pennergame', 'malle.pennergame', 'vatikan.pennergame', 'atlantis.pennergame'];
var towns = ['HH', 'BL', 'MU', 'HW', 'KL', 'HR', 'SY', 'ML', 'VT', 'AT'];

var links = [wlp + "//www.pennergame.de", wlp + "//berlin.pennergame.de",
			 wlp + "//muenchen.pennergame.de", wlp + "//halloween.pennergame.de",
			 wlp + "//koeln.pennergame.de", wlp + "//reloaded.pennergame.de", 
			 wlp + "//sylt.pennergame.de", wlp + "//malle.pennergame.de",
			 wlp + "//vatikan.pennergame.de", wlp + "//atlantis.pennergame.de"];
var langs = ['de_DE', 'bl_DE', 'mu_DE', '', 'kl_DE', 'hr_DE', 'sy_DE', 'ml_DE', 'vt_DE', 'at_DE'];

var url = document.location.href;

var index = -1;
if (url.indexOf(wlp + '//www.pennergame')>=0)
    index = 0;
else
    for (var i = 0; i < sStrings.length; i++)
		if (url.indexOf(sStrings[i]) >=0) {
			index = i;
			break;
		}
...
if (lang != '')
    var sig = wlp + '//inodes.pennergame.de/'+lang+'/signaturen/';
else
    var sig = '';
...
for (x = tables.length-1; x > 0; x--) {
   if (document.getElementsByTagName("table")[x].innerHTML.indexOf("profil/id") != -1 &&
       document.getElementsByTagName("table")[x].innerHTML.indexOf('id="gb_posts"') == -1)
      break;
}
...
GM_xmlhttpRequest({
  ...
  onload: function(responseDetails) {
      ...
      var myid = dom.getElementsByTagName('uid');
	  if (myid.length == 0) {
		  myid = document.getElementById("my-profile-new").getElementsByTagName("a")[0].href.split("id:")[1].split("/")[0];
	  }
	  else
          myid = myid[0].getAttribute('value');
      ...
      GM_xmlhttpRequest({
          ...
          onload: function(responseDetails) {
            ...
            for (var x = 1; x < tr.length; x++) {
                ...
                if (xx%2 == 0){
                  tr[x].setAttribute('style', 'background-image: url('+imglink('bgeven')+');');
                }
                else {
                  tr[x].setAttribute('style', 'background-image: url('+imglink('bgodd')+');');
                }
				...
				if (town != "SY" && town != "ML" && town != "VT" && town != "AT") {
					var newtd = tr[x].appendChild(document.createElement('td'));
            ...
            var newtr = document.createElement('tr');
            newtr.setAttribute('style', 'background-image: url('+imglink('bghead')+');');
...
function Geldladen(myid, pts, minPts, maxPts, id, x) {
    GM_xmlhttpRequest({
        ...
        onload: function(responseDetails) {
            ...
            document.getElementById('nachricht'+x).innerHTML = "<a href=\"/messages/write/?to="+id+"\"><img style\"margin:5px;\" src=\"" + statics + "icons/new_msg.gif\" title=\""+username+" eine Nachricht schreiben.\"></a>";

            document.getElementById('freundesch'+x).innerHTML = "<a href=\"/friendlist/?friend="+username+"\"><img style\"margin:5px;\" src=\"" + statics + "icons/add.gif\" title=\""+username+" in die Freundschaftliste mit aufnehmen.\"></a>";

            document.getElementById('poebel'+x).innerHTML = "<a onclick=\"ProvokeForm(3410, "+id+",1,'popup');\" style=\"text-decoration: none;\"><img src=\"" + statics + "vcc/premium.gif\"  title=\""+username+" anp&ouml;beln.\"border=\"0\"></a>";
            if (myid == id){
    ...
    onload: function(responseDetails) {
        ...
        try {
            ...
            for (var i = 0; i < tiere.length; i++)
                if (hausi2.replace(/static\d*/, 'static') == statics+'shop/'+lang+'/tiere/'+tiere[i][0]+'.jpg') {
                    petname =       tiere[i][1];
                    var att = ' ' + tiere[i][2] + ' ';
                    var def = ' ' + tiere[i][3] + ' ';
                    var dex = ' ' + tiere[i][4] + ' ';
    ...
    //              tr[x].setAttribute('style', 'background:red;');
                    document.getElementsByName('tr'+x)[0].setAttribute('style', 'background-image: url('+imglink('bgred')+');');
            }
            else {
    ...
    //            tr[x].setAttribute('style', 'background:red;');
        ...
        try {
            ...
            if(geschlecht.indexOf('icons/female.jpg') != -1) {
				var geschpic = imglink("female");
				var geschpictooltip = "Frau";
			}
            else if(geschlecht.indexOf('icons/male.jpg') != -1) {
				var geschpic = imglink("male");
				var geschpictooltip = "Mann";
			}
            else if(geschlecht.indexOf('plunder/nikolaus.png') != -1) {
				var geschpic = statics+"plunder/nikolaus.png";
				var geschpictooltip = "Rote Schleife";
			}
            else if(geschlecht.indexOf('plunder_new/old/valday.png') != -1) {
				var geschpic = statics+"plunder_new/old/valday.png";
				var geschpictooltip = "Be my Valentine!";
			}
            else if(geschlecht.indexOf('plunder/cake.png') != -1) {
				var geschpic = statics+"plunder/cake.png";
				var geschpictooltip = "Happy Birthday!";
			}
            else if(geschlecht.indexOf('premiummedia/img/js/swfobject.js') != -1) {
				var geschpic = imglink("premium");
				var geschpictooltip = "Premium Profil";
			}
            else if(geschlecht.indexOf('events/easter/profil_wiesendeko.png') != -1) {
				var geschpic = statics+"events/easter/profil_wiesendeko.png";
				var geschpictooltip = "Ostergammel";
			}
            else if(geschlecht.indexOf('plunder_new/blumenkette.png') != -1) {
				var geschpic = statics+"plunder_new/blumenkette.png";
				var geschpictooltip = "Blumenkette";
			}
            else if(geschlecht.indexOf('plunder_new/eiskristall.png') != -1) {
				var geschpic = statics+"plunder_new/eiskristall.png";
				var geschpictooltip = "Eiskristall";
			}
            else if(geschlecht.indexOf('events/may2012') != -1) {
				var geschpic = statics+"events/may2012"+geschlecht.split("may2012")[1];
				var geschpictooltip = "Tag und Nacht";
			}
        }
        catch(err) {
            if (content.indexOf("buch_s.png") != -1) {
				var geschpic = statics+"events/buch_s.png";
				var geschpictooltip = "Goldenes Buch";
			}
		}
        ...
        try{
            if (suche != -1) {
                var online2a = statics + "icons/on.png";
                var onoff = "Spieler ist online"
            }
            else {
                var online2a = imglink("offline");
        ...
        try{
            if (suche != -1) {
                ...
                document.getElementById('urlaub'+x).innerHTML ="<img src=\""+imglink("urlaub")+"\" style=\"width:20px; height:20px\" title=\""+urlaub+"\"></img>";
            }
            else {
                var urlaub ="&nbsp;"
