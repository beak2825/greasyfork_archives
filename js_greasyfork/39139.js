...
// @include        https://*virtonomic*.*/*/main/unit_market/list
// @downloadURL https://update.greasyfork.org/scripts/39139/Virtonomica%3AUnitMarkets.user.js
// @updateURL https://update.greasyfork.org/scripts/39139/Virtonomica%3AUnitMarkets.meta.js
// ==/UserScript==
...
var run = function() {
  ...
  */
  ...
  var wc_view = $("<img src=http://www.iconsearch.ru/uploads/icons/freeapplication/24x24/load.png title='Показать новые поступления'>").click( function() {
	console.log("view");
	...
	for(i=0; i<link.length; i+=2){
		...
		var ee = link.eq(i).parent().parent().next().next().next();
		console.log(ee.html());
  ...
  var wc = $("<img alt='Добавить Идентификаторы подразделений в локальное хранилище' src=http://www.iconsearch.ru/uploads/icons/ull_icons/24x24/message_add.png title='Добавить Идентификаторы подразделений в локальное хранилище'>").click( function() {
	console.log("add");
        // Маркировка
        unit_mark = JSON.parse( window.localStorage.getItem('unit_mark') );
        if ( unit_mark == null ) unit_mark = new Object();
