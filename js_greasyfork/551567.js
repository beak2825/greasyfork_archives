// ==UserScript==
// @name         Мотыга by Unblock
// @namespace    https://lolz.live/
// @author       Unblock
// @version      2.2.8
// @description  Автозамена на мотыгу
// @match        https://lolz.live/*
// @match        https://*.lolz.live/*
// @run-at       document-idle
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/551567/%D0%9C%D0%BE%D1%82%D1%8B%D0%B3%D0%B0%20by%20Unblock.user.js
// @updateURL https://update.greasyfork.org/scripts/551567/%D0%9C%D0%BE%D1%82%D1%8B%D0%B3%D0%B0%20by%20Unblock.meta.js
// ==/UserScript==

(function () {
  'use strict';

  const SELECTOR = '.thankAuthorBox';

  // Построение НОВОГО HTML на основе старого href (сохраняем уникальные параметры)
  function buildNewHTML(oldHref) {
    // ⚠️ Здесь подставь свои ТЕКСТЫ, но без призывов к насилию/оскорблений.
    // Всё остальное — как ты просил: структура, иконка, классы, ссылка — сохраняются.
    return `
<div class="thankAuthorBox">
  <div class="thankAuthorTitle">
    Этот материал оказался бесполезным?
  </div>
  <div class="thankAuthorDiscrip">
    Вы можете оттарабанить автора темы путём сувания фалоса ему в анус.
  </div>
  <a href="${oldHref}" class="btnThanksAuthor mn-15-0-0 OverlayTrigger">
    <span class="icon leftIcon thankAuthorButtonIcon">
      <svg xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" height="18" viewBox="0 0 379.416 379.416" width="12" xml:space="preserve" aria-hidden="true" focusable="false">
        <g fill="#FFFFFF" stroke="#FFFFFF">
          <path d="M366.082,195.332l-0.495-0.538c-22.455-24.592-48.208-48.927-78.729-74.395c-4.614-3.851-25.51-20.384-26.486-21.215 s-1.989-1.555-1.775-2.666c0.254-1.319,0.624-2.743,1.06-4.243c0.388-1.336,0.788-2.719,1.094-4.184 c0.557-2.661,0.91-5.032,1.08-7.251l0.01-0.12c0.19-2.474,0.546-7.078-2.327-10.145c-1.008-1.074-2.857-2.356-5.955-2.356 c-2.282,0-4.521,0.689-5.857,1.103l-0.147,0.046c-3.634,1.114-6.873,2.817-9.29,4.177c-1.399,0.787-2.796,1.614-4.146,2.46 c-0.404,0.253-0.81,0.53-1.218,0.812c-1.05,0.725-1.545,1.392-3.079,0.084c-2.122-1.867-25.89-22.144-89.825-51.953 c-10.255-4.781-20.893-9.172-31.615-13.05C97.866,8.096,86.273,4.504,72.946,0.918l-0.407-0.11C70.99,0.389,69.311,0,67.54,0 C64,0,61.036,1.63,59.188,4.59c-1.523,2.441-1.906,5.412-1.049,8.148c0.896,2.866,3.021,5.201,5.825,6.408l7.161,3.083 c28.388,12.212,56.542,29.121,86.072,51.693c10.122,7.738,19.275,15.194,27.981,22.795c4.114,3.592,8.22,7.237,12.203,10.835 c0.354,0.318,3.372,3.191,3.372,3.191L17.508,328.184c-5.761,6.833-8.54,14.704-8.039,22.76c0.479,7.696,4.049,15.149,9.796,20.446 c5.617,5.177,12.942,8.026,20.628,8.026c0.962,0,1.936-0.046,2.892-0.138c8.684-0.831,16.688-5.458,21.96-12.695 c1.593-2.185,156.703-215.057,165.4-227.015c24.118,24.903,46.882,51.772,67.655,79.86c4.086,5.524,11.317,15.82,11.644,16.317 c1.397,2.141,2.982,4.565,5.224,6.351c1.693,1.346,3.663,2.059,5.697,2.059c3.176,0,5.756-1.713,7.42-3.002l37.582-29.109 c4.483-3.896,4.615-7.84,4.615-7.84C370.266,199.869,367.651,197.033,366.082,195.332z"></path>
        </g>
      </svg>
    </span>
    Отпиздить автора мотыгой
  </a>
</div>
`.trim();
  }

  function processBox(box) {
    if (box.dataset._replaced === '1') return;

    // Пытаемся найти исходную кнопку и взять её href
    const a = box.querySelector('a.btnThanksAuthor, a.OverlayTrigger, a[href*="/payment/balance-transfer"]');
    const oldHref = a ? a.getAttribute('href') || a.href : null;

    // Если кнопка есть — строим новый блок с тем же href (сохранит уникальные параметры темы/автора)
    const html = buildNewHTML(oldHref || '#');

    const wrapper = document.createElement('div');
    wrapper.innerHTML = html;
    const replacement = wrapper.firstElementChild || wrapper;

    box.replaceWith(replacement);
  }

  function scan(root = document) {
    root.querySelectorAll(SELECTOR).forEach(processBox);
  }

  const mo = new MutationObserver((muts) => {
    for (const m of muts) {
      for (const n of m.addedNodes) {
        if (!(n instanceof HTMLElement)) continue;
        if (n.matches && n.matches(SELECTOR)) {
          processBox(n);
        } else {
          scan(n);
        }
      }
    }
  });

  scan();
  mo.observe(document.documentElement || document.body, { childList: true, subtree: true });
  setTimeout(scan, 1200);
  document.addEventListener('DOMContentLoaded', () => scan());
})();
