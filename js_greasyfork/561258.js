...
(function() {
  class GM_ConfigUI {
    constructor({config, onSave, defaultData = {}, prefix} = {}) {
      ...
      this.css = null;
      if (this.defaultData) this.data = JSON.parse(JSON.stringify(defaultData)); else this.initData();
      this.initStyles();
    }
    open() {
    ...
    render() {
      ...
      header.innerHTML = `<h3 class="${this.prefix}-title">${this.config.title || 'Settings'}</h3>\n<span class="${this.prefix}-close">×</span>`;
