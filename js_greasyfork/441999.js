...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/441999/taobao%E5%B8%A6%E6%8A%BC%E5%90%8E.user.js
// @updateURL https://update.greasyfork.org/scripts/441999/taobao%E5%B8%A6%E6%8A%BC%E5%90%8E.meta.js
// ==/UserScript==
...
(function() {
    ...
    function hotkey() {
        ...
        if (a == 38) {
            ...
            if (tblocalHostPathName.indexOf("imgfile") == -1 && localHost.indexOf("taobao.com") > -1) {
                ...
                for (var i = 1; i < 49; i++) {
                    ...
                    var xl = document.querySelector("#mainsrp-itemlist > div > div > div:nth-child(1) > div:nth-child("+ i +") > div.ctx-box.J_MouseEneterLeave.J_IconMoreNew > div.row.row-1.g-clearfix > div.deal-cnt")
                    if(xl.innerText.indexOf("0人付款") != -1){
                        var btnas = document.querySelector("#mainsrp-itemlist > div > div > div:nth-child(1) > div:nth-child("+ i +")")
                        btnas.classList="333"
            ...
            if (tblocalHostPathName.indexOf("image") == -1 && localHost.indexOf("jd.com") > -1) {
                for (var c = 1; c < 61; c++) {
                    ...
                    var btnjds =document.querySelector("#J_goodsList > ul > li:nth-child("+ c +") > div > div.p-focus > a").dataset.sku
                    var btnjdss =document.querySelector("#J_comment_" + btnjds)
                    console.log(btnjdss.innerText);
                    if(btnjdss.innerText == "0"){
                        console.log("屏蔽");
                        var btnjdas = document.querySelector("#J_goodsList > ul > li:nth-child("+ c +")")
                        btnjdas.style.display="none"
                    }
                    if(btnjd != null) {
        ...
        if(a == 32){
            var kgtj = document.querySelector("#app > div:nth-child(4) > div > div > div > div.work-process-footer > div > div > div:nth-child(5) > button")
            kgtj.click()
        }
        if (a == 39) {
            // 模拟点击押后
            var ju_动态id="17"
            var btntcs = document.querySelector("#app > div:nth-child(4) > div > div > div > div.work-process-footer > div > div > button.tcs-new-button.verify-page-button.ivu-btn.ivu-btn-default")
            btntcs.click();
            // 模拟输入押后原因
