...
// @downloadURL https://update.greasyfork.org/scripts/406572/MyKirito%20Helper%20Unofficial%20version%20for%2008.user.js
// @updateURL https://update.greasyfork.org/scripts/406572/MyKirito%20Helper%20Unofficial%20version%20for%2008.meta.js
// ==/UserScript==
...
// config
var helperConfig = GM_getValue('MyKiritoHelper', {
  'moreinfo': true,
  'yuukiMod': false,
  'lisbethMod': false,
  'deadMod': false,
...
var fielddata = {
  ...
  'background': {
    'label': '背景圖片網址:',
    'type': 'text',
    'default': 'https://i.imgur.com/7a3r191.png'
  },
  'deadpic': {
    'label': '鯊魚上香圖片:',
    'type': 'text',
  ...
  'cleardata': {
    'label': '網頁重設(行動計算或MOD出現問題時使用) 註:需要重新登入',
    'type': 'button',
    'size': 100,
    'click': function() {
      localStorage.clear();
  ...
  'botpic': {
    'label': '通緝頭貼:',
    'type': 'text',
    'default': "https://i.imgur.com/EJqzi4Q.png"
  },
  'botcolor': {
    'label': '通緝顏色:',
    'type': 'text',
...
GM_config.init({
  ...
  'title': 'MOD設定',
  'fields': fielddata,
  'css': '#modconfig { background: #2c2f33; color:white;} #modconfig_section_3 .config_var { width: 10%; display: inline; } #modconfig_section_2 .config_var { width: 10%; display: inline; } #modconfig .config_var{margin: 0 0 20px;} #modconfig .field_label {font-size: 16px;} #modconfig * { font-family:Helvetica Neue,Helvetica,Arial,微軟正黑體,sans-serif} #modconfig .reset, #modconfig .reset a, #modconfig_buttons_holder { color: white; }',
  'events': // Callback functions object
  {
    'init': function() {},
    'open': function() {
      this.frame.setAttribute('style', 'top: 120px; left: 239px; right: auto; position: fixed; width: 30%; height: 70%;text-align: left; background: white; margin: 30px 0; box-shadow: 0 0 30px black; border-width: 0; transition: transform .2s linear; transform: translateY(-20px);')
    },
    'save': function() {
      location.reload();
    },
...
var 導覽列常駐上方 = GM_config.get('shouldbetop');
...
var 找紅色 = GM_config.get('redbutton');
var 找黃色 = GM_config.get('yellowbutton');
...
function init() {
  ...
  if (helperConfig.id) {
    ...
    inputDiv.className = 'sc-fzomuh gHzJiR';
  ...
  button.className = 'sc-fzomuh gHzJiR';
  ...
  tippy(button, {
    content: `<div style="text-align: center;">MyKirito Helper</div>` +
      `<p><label title="開關後請重新整理畫面"><input type="checkbox" id="more_info" title="開關後請重新整理畫面" ${(helperConfig.moreinfo)?"checked":""}> 額外設定</label></p>` +
      ...
      `<p><label title="開關後請重新整理畫面"><input type="checkbox" id="show_action" title="開關後請重新整理畫面" ${(helperConfig.actionRecode)?"checked":""}> 行動紀錄器</label></p>` +
      `<p><label title="開關後請重新整理畫面"><input type="checkbox" id="show_effort" title="開關後請重新整理畫面" ${(helperConfig.effortRecode)?"checked":""}> 努力值紀錄器</label></p>` +
      `<p><label title="開關後請重新整理畫面"><input type="checkbox" id="ach_sort" title="開關後請重新整理畫面" ${(helperConfig.achSort)?"checked":""}> 重新排序成就(強迫症專用)</label></p>` +
    ...
    onShown() {
      if (!inited) {
        ...
        document.getElementById("more_info").addEventListener('input', () => {
          (document.getElementById("more_info").checked) ? helperConfig.moreinfo = true: helperConfig.moreinfo = false;
        ...
        document.getElementById('show_action').addEventListener('input', () => {
          (document.getElementById("show_action").checked) ? helperConfig.actionRecode = true: helperConfig.actionRecode = false;
          GM_setValue('MyKiritoHelper', helperConfig);
        });
        document.getElementById('show_effort').addEventListener('input', () => {
          (document.getElementById("show_effort").checked) ? helperConfig.effortRecode = true: helperConfig.effortRecode = false;
          GM_setValue('MyKiritoHelper', helperConfig);
        });
        document.getElementById('ach_sort').addEventListener('input', () => {
          (document.getElementById("ach_sort").checked) ? helperConfig.achSort = true: helperConfig.achSort = false;
          GM_setValue('MyKiritoHelper', helperConfig);
        });
...
function myKirito(url, response) {
  ...
  async function addTimetip() {
    ...
    function clearAtTime(tip, timetoclear) {
      setTimeout(() => {
        tip.destroy();
      }, timetoclear - now.getTime() - 100 * 1000);
    }
  }

  async function addChartBtn() {
    if (!helperConfig.chart) {
      return;
    }
    await sleep(helperConfig.delay);
...
async function otherKirito(url, response) {
  ...
  async function reportbutton() {
    ...
    var bodydata = {
      ...
      "embeds": [{
        ...
        "author": {
          name: "通緝令發起人: " + myK.nickname,
          url: "https://mykirito.com/profile/" + myK._id
        },
        "fields": [{
          name: "職業:",
          value: otherK.character,
          inline: true
        }, {
          name: "稱號:",
          value: otherK.title,
          ...
          name: "血量:",
          value: otherK.hp + "(+" + otherK.rattrs.hp + ")",
          inline: true
        }, {
          name: "攻擊:",
          value: otherK.atk + "(+" + otherK.rattrs.atk + ")",
          inline: true
        }, {
          name: "防禦:",
          value: otherK.def + "(+" + otherK.rattrs.def + ")",
          inline: true
        }, {
          name: "體力:",
          value: otherK.stm + "(+" + otherK.rattrs.stm + ")",
          inline: true
        }, {
          name: "敏捷:",
          value: otherK.agi + "(+" + otherK.rattrs.agi + ")",
          inline: true
        }, {
          name: "反應速度:",
          value: otherK.spd + "(+" + otherK.rattrs.spd + ")",
          inline: true
        }, {
          name: "技巧:",
          value: otherK.tec + "(+" + otherK.rattrs.tec + ")",
          inline: true
        }, {
          name: "智力:",
          value: otherK.int + "(+" + otherK.rattrs.int + ")",
          inline: true
        }, {
          name: "幸運:",
          value: otherK.lck + "(+" + otherK.rattrs.lck + ")",
          inline: true
        }, ],
        "color": 通緝顏色
      }]
  ...
  async function updateMyK() {
    if (response.gained.prevLV === response.gained.nextLV) {
      return;
    }
    //重置行動紀錄
    resetActionRecode(1);
...
async function playerFilter() {
  ...
  if (helperConfig.wanted) {
    ...
    for (let i = 1; i < allPlayer.length; i++) {
      ...
      if (找紅色) {
        if (allPlayer[i].style.color == 'var(--red)') {
          wanted = true;
        }
      }
      if (找黃色) {
        if (allPlayer[i].style.color == 'rgb(255, 152, 0)') {
          wanted = true;
        }
      }
      if (!wanted) {
...
async function NOTIMY() {
  if (helperConfig.NotificationMOD) {
    ...
    if (tt.length == '4') actionbuttom = document.querySelectorAll('#root > div > div.sc-fzokOt.hLgJkJ > div:nth-child(3) > button.llLWDd');
...
async function NOTIOT() {
  if (helperConfig.NotificationMOD) {
    var battlebuttom = document.querySelectorAll('#root > div > div:nth-child(2) > div:nth-child(2) > div > div > button');
...
function cleanObjs() {
  ...
  if (isExist(actToLvUp)) {
    cleanTips(actToLvUp);
  }
  if (isExist(pkBtns)) {
    cleanTips(pkBtns);
  }
  if (isExist(nextTimetip)) {
    cleanTips(nextTimetip);
  }
  if (isExist(myChart)) {
    myChart.destroy();
...
function createTipGroup(btns, config, sglt = true) {
  ...
  if (sglt) {
    tippy.createSingleton(tippyBtns, config);
  }
  return tippyBtns;
}
...
function updateActionStatus() {
  var headings = document.evaluate("//h3[contains(., '行動')]", document, null, XPathResult.ANY_TYPE, null);
  var actionTitle = headings.iterateNext();
  if (helperConfig.actionRecode) {
    if (actionTitle != null) {
      let actionBar = document.querySelector('#actionBar');
      let actionObject = JSON.parse(localStorage.getItem("action"));
      let actionContent = '';
      actionContent += `<div id="actionBar"><button id="resetBtn" class="sc-AxgMl llLWDd">重置計算</button>
調整行動數值：
<select id="acts">
<option value="hunt2">兔肉</option>
<option value="train2">自主</option>
<option value="eat2">野餐</option>
<option value="girl2">汁妹</option>
<option value="good2">善事</option>
<option value="sit2">坐下</option>
<option value="fish2">釣魚</option>
<option value="1h">1小</option>
<option value="2h">2小</option>
<option value="4h">4小</option>
<option value="8h">8小</option>
<option value="floor">樓層</option>
<option value="pw">P勝</option>
<option value="pl">P敗</option>
<option value="pk">超渡</option>
</select>
<select id="actsType">
<option value="original">當前等級</option>
<option value="additional">角色生涯</option>
</select>
<input id="inputAct" maxlength="6" type="number" value="">
<button id="editActBtn" class="sc-AxgMl llLWDd">儲存</button>
<hr/>
兔肉:${actionObject.hunt2}<span class="sc-fzoLsD fYZyZu">(${actionObject.hunt2x})</span>|
自主:${actionObject.train2}<span class="sc-fzoLsD fYZyZu">(${actionObject.train2x})</span>|
野餐:${actionObject.eat2}<span class="sc-fzoLsD fYZyZu">(${actionObject.eat2x})</span>|
汁妹:${actionObject.girl2}<span class="sc-fzoLsD fYZyZu">(${actionObject.girl2x})</span>|
善事:${actionObject.good2}<span class="sc-fzoLsD fYZyZu">(${actionObject.good2x})</span>|
坐下:${actionObject.sit2}<span class="sc-fzoLsD fYZyZu">(${actionObject.sit2x})</span>|
釣魚:${actionObject.fish2}<span class="sc-fzoLsD fYZyZu">(${actionObject.fish2x})</span>`;
      actionContent += `<hr/>1小:${actionObject["1h"]}<span class="sc-fzoLsD fYZyZu">(${actionObject["1hx"]})</span>|
2小:${actionObject["2h"]}<span class="sc-fzoLsD fYZyZu">(${actionObject["2hx"]})</span>|
4小:${actionObject["4h"]}<span class="sc-fzoLsD fYZyZu">(${actionObject["4hx"]})</span>|
8小:${actionObject["8h"]}<span class="sc-fzoLsD fYZyZu">(${actionObject["8hx"]})</span>|
樓層:${actionObject.floor}<span class="sc-fzoLsD fYZyZu">(${actionObject.floorx})</span>|
P勝:${actionObject.pw}<span class="sc-fzoLsD fYZyZu">(${actionObject.pwx})</span>|
P敗:${actionObject.pl}<span class="sc-fzoLsD fYZyZu">(${actionObject.plx})</span>|
超渡:${actionObject.pk}<span class="sc-fzoLsD fYZyZu">(${actionObject.pkx})</span><hr/></div>`;
      if (actionBar != null) {
        actionBar.outerHTML = actionContent;
      } else {
        actionTitle.insertAdjacentHTML('afterend', actionContent);
      }
      document.getElementById("resetBtn").addEventListener("click", function() {
        resetActionRecode(2);
      });
      document.getElementById("editActBtn").addEventListener("click", function() {
        editActionRecode();
      });
    }
  }
...
function updateEffortStatus() {
  var headings = document.evaluate("//h3[contains(., '行動')]", document, null, XPathResult.ANY_TYPE, null);
  var effortTitle = headings.iterateNext();
  if (helperConfig.effortRecode) {
    if (effortTitle != null) {
      let effortBar = document.querySelector('#effortBar');
      let effortObject = JSON.parse(localStorage.getItem("effort"));
      let effortContent = '';
      effortContent += `<div id="effortBar"><button id="resetEffortBtn" class="sc-AxgMl llLWDd">重置計算</button>
調整努力值：
<select id="efforts">
<option value="hp">血量</option>
<option value="atk">攻擊</option>
<option value="def">防禦</option>
<option value="stm">體力</option>
<option value="agi">敏捷</option>
<option value="spd">反應速度</option>
<option value="tec">技巧</option>
<option value="int">智力</option>
<option value="lck">幸運</option>
</select>
<input id="inputEffort" maxlength="6" type="number" value="">
<button id="editEffortBtn" class="sc-AxgMl llLWDd">儲存</button>
<hr/>
血量:${effortObject.hp}<span class="sc-fzoLsD fYZyZu"></span>|
攻擊:${effortObject.atk}<span class="sc-fzoLsD fYZyZu"></span>|
防禦:${effortObject.def}<span class="sc-fzoLsD fYZyZu"></span>|
體力:${effortObject.stm}<span class="sc-fzoLsD fYZyZu"></span>|
敏捷:${effortObject.agi}<span class="sc-fzoLsD fYZyZu"></span>|
反應速度:${effortObject.spd}<span class="sc-fzoLsD fYZyZu"></span>|
技巧:${effortObject.tec}<span class="sc-fzoLsD fYZyZu"></span>|
智力:${effortObject.int}<span class="sc-fzoLsD fYZyZu"></span>|
幸運:${effortObject.lck}<span class="sc-fzoLsD fYZyZu"></span>
<hr/></div>`;
      if (effortBar != null) {
        effortBar.outerHTML = effortContent;
      } else {
        effortTitle.insertAdjacentHTML('afterend', effortContent);
      }
      document.getElementById("resetEffortBtn").addEventListener("click", function() {
        resetEffortRecode();
      });
      document.getElementById("editEffortBtn").addEventListener("click", function() {
        editEffortRecode();
      });
    }
  }
...
//修改行動紀錄
function editActionRecode() {
  action = JSON.parse(localStorage.getItem("action"));
  let act = document.getElementById("acts");
  let actType = document.getElementById("actsType");
  var actSelect = act.options[act.selectedIndex].value;
  var actTypeSelect = actType.options[actType.selectedIndex].value;
  let setValue = parseInt(document.getElementById("inputAct").value, 10);
  switch (actSelect) {
    case "1h":
    case "2h":
    case "4h":
    case "floorBonus":
    case "8h":
      action[actSelect] = setValue;
      break;
    default:
      if (actTypeSelect == "original") {
        action[actSelect] = setValue;
      } else if (actTypeSelect == "additional") {
        action[actSelect + "x"] = setValue;
      }
      break;
  }
...
//修改努力值紀錄
function editEffortRecode() {
  effort = JSON.parse(localStorage.getItem("effort"));
  let efforts = document.getElementById("efforts");
  var effortSelect = efforts.options[efforts.selectedIndex].value;
  let setValue = parseInt(document.getElementById("inputEffort").value, 10);
  effort[effortSelect] = setValue;
  localStorage.setItem("effort", JSON.stringify(effort));
  updateEffortStatus();
}
...
//重置行動紀錄 1是升級 2是轉生
function resetActionRecode(type) {
  if (type == 1) {
    action.hunt2 = 0;
    action.train2 = 0;
    action.eat2 = 0;
...
var actionArray = ["hp", "atk", "def", "stm", "agi", "spd", "tec", "int", "lck"];
//type 1一般行動 2 x4 3 x6
function dealEffortCount(action, type, level) {
  if (level < 70) {
    if (type == 1) {
      let data = actionEffortData[action];
      if (action == "train2") {
        if (myK.hp < myK.tec * 10) {
          let val = checkCharaMax();
          data[val] = 4;
        } else {
          data[0] = 4;
        }
        for (let i = 0; i < 9; i++) {
          effort[actionArray[i]] += parseInt(data[i]);
        }
      } else {
        for (let i = 0; i < 9; i++) {
          effort[actionArray[i]] += parseInt(data[i]);
      ...
      let data = actionEffortData.x4;
      for (let i = 0; i < 9; i++) {
        effort[actionArray[i]] = parseInt(effort[actionArray[i]]) * 4 + parseInt(data[i]);
      }
    } else if (type == 3) {
      let data = actionEffortData.x6;
      for (let i = 0; i < 9; i++) {
        effort[actionArray[i]] = parseInt(effort[actionArray[i]]) * 6 + parseInt(data[i]);
      }
    }
  }
}
//取得角色最大能力值
function checkCharaMax() {
  let valIndex = 0;
  let tempMax = 0;
  for (let i = 0; i < 9; i++) {
    if (i == 0 && myK[actionArray[i]] > tempMax * 10) {
      tempMax = myK[actionArray[i]];
      valIndex = i;
    } else if (i != 0 && myK[actionArray[i]] * 10 > tempMax) {
      tempMax = myK[actionArray[i]] * 10;
      valIndex = i;
    }
  }
  return valIndex;
...
var actionEffortData = {
  "hunt2": [0, 1, 0, 2, 2, 0, 1, 1, 1],
  "train2": [0, 2, -1, 2, 1, -1, 2, 0, -1],
  "eat2": [3, 1, 2, 3, -2, -2, 0, -1, 0],
  "girl2": [0, 0, 2, 0, 0, 1, 3, 2, 0],
  "good2": [0, 0, 1, 1, 0, 2, 0, 2, 3],
  "sit2": [1, -1, 1, -1, 0, 0, 1, 3, 0],
  "fish2": [1, 2, 0, 1, 0, 3, 0, 0, 0],
  "pw": [0, 2, 1, 1, 2, 0, 1, 0, 0],
  "pl": [3, -1, 3, 1, 0, 1, 0, 2, 1],
  "pk": [-1, 3, -1, -1, 2, -1, 2, 0, -1],
  "x4": [2, 2, 2, 2, 2, 2, 2, 2, 2],
  "x6": [3, 3, 3, 3, 3, 3, 3, 3, 3]
};

function getBattleLog(report) {
  var player1 = '';
  var player2 = '';
  var boss = report.b.nickname;
  var player1dmg = 0;
  var player2dmg = 0;
  var nowplayer = '';
  player1 = report.a.nickname;
  if (report.a2 != null) {
    player2 = report.a2.nickname;
  }
  nowplayer = player1;
  for (var i = 0; i < report.messages.length; i++) {
    var msg = report.messages[i].m.match(/[0-9]+[\s]{0,1}點傷害/i);
    if (msg != null) {
      var dmg = parseInt(msg[0].replace(" 點傷害", "").replace("點傷害", ""));
      if (report.messages[i].m.indexOf(`${boss}使出了猛撞`) > -1) {
        var msg2 = report.messages[i].m.match(/自身受到[\s]{0,1}[0-9]+[\s]{0,1}點傷害/i);
        dmg = parseInt(msg2[0].replace(" 點傷害", "").replace("點傷害", "").replace("自身受到 ", "").replace("自身受到", ""));
      }
      if (report.messages[i].m.indexOf(`對${nowplayer}造成`) > -1 &&
        report.messages[i].m.indexOf(`${boss}使出了猛撞`) == -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${nowplayer}受到`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${nowplayer}被`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`中${nowplayer}`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${nowplayer}的精神遭到侵蝕`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${nowplayer}滑倒自摔`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${nowplayer}踩到`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${nowplayer}直接被`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${nowplayer}攻擊自己`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${nowplayer}的眼前爆炸`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${nowplayer}及時閃開了仍被手榴彈的碎片命中`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${nowplayer}往後退後腦撞到樹`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`遭到${boss}反擊`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${boss}趁機攻擊`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${boss}趁機反擊`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${boss}趁勢攻擊`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`被 ${boss}攻擊造成`) > -1) {
        continue;
      }
      if (report.messages[i].m.indexOf(`${player1}受到了`) > -1 && report.messages[i].m.indexOf(`結冰了`) > -1) {
        continue;
      }
      if (player2 != '') {
        if (report.messages[i].m.indexOf(`${player2}受到了`) > -1 && report.messages[i].m.indexOf(`結冰了`) > -1) {
          continue;
        }
      }
      ...
      if (nowplayer == player1) {
        player1dmg += dmg;
        console.log(`第${i}條 ${report.messages[i].m} ${player1}:${player1dmg}`);
      } else if (nowplayer == player2) {
        player2dmg += dmg;
        console.log(`第${i}條 ${report.messages[i].m} ${player2}:${player2dmg}`);
      }
    }
    if (report.messages[i].m.indexOf(`剛才所有的戰鬥都是幻覺`) > -1) {
      player1dmg = 0;
      player2dmg = 0;
    }
    if (report.messages[i].m == `${player1}倒下了`) {
      nowplayer = player2;
    }
    if (report.messages[i].m == `${player2}倒下了`) {
      nowplayer = player1;
    }
    if (report.messages[i].m == "Switch!") {
      if (nowplayer == player1) {
        nowplayer = player2;
      } else if (nowplayer == player2) {
        nowplayer = player1;
  ...
  var headings = document.evaluate("//p[contains(., '提示：可直接複製分享此網址')]", document, null, XPathResult.ANY_TYPE, null);
  var battleTitle = headings.iterateNext();
  if (battleTitle != null) {
    let battleBar = document.querySelector('#battleBar');
    let battleContent = '';
    battleContent += `<div id="battleBar">${player1}輸出值:${player1dmg}<br/>`;
    if (player2 != '') {
      battleContent += `${player2}輸出值:${player2dmg}<br/>`;
    }
    battleContent += `總輸出值:${player1dmg+player2dmg}</div>`;
    if (battleBar != null) {
      battleBar.outerHTML = battleContent;
    } else {
      battleTitle.insertAdjacentHTML('afterend', battleContent);
    }
  }
}

//重新排列成就表 造福強迫症患者
function reSortAch(achText) {
  if (helperConfig.achSort) {
    var headings = document.evaluate("//p[contains(., '目前總點數')]", document, null, XPathResult.ANY_TYPE, null);
    var achTitle = headings.iterateNext();
    if (achTitle != null) {
      document.querySelectorAll(".sc-AxjAm.bcMPWx")[0].remove();
      let ach = JSON.parse(achText);
      ach.list = sortByProperty(ach.list, "name");
      let achContent = `<table class="sc-AxjAm bcMPWx"><tbody class="sc-AxirZ bJCmFu">`;
      achContent += `<tr class="sc-AxiKw eSbheu"><th class="sc-AxhCb lnpmAI" style="width: 30px;">#</th><th class="sc-AxhCb lnpmAI">名稱</th><th class="sc-AxhCb lnpmAI" style="max-width: 50px;">點數</th></tr>`;
      for (var i = 0; i < ach.list.length; i++) {
        achContent += `<tr class="sc-AxiKw eSbheu"><td class="sc-AxhUy dRdZbR" style="text-align: center;">${i+1}</td><td class="sc-AxhUy dRdZbR">${ach.list[i].name}</td><td class="sc-AxhUy dRdZbR" style="text-align: center;">${ach.list[i].point}</td></tr>`;
      }
      achContent += `</tbody></table>`;
      achTitle.insertAdjacentHTML('afterend', achContent);
    }
  }
}
...
(function() {
  ...
  if (localStorage.getItem("effort") == null) {
    resetEffortRecode()
  } else {
    effort = JSON.parse(localStorage.getItem("effort"));
  }
  ...
  XMLHttpRequest.prototype.open = function(method, url) {
    ...
    this.addEventListener('load', function() {
      action = JSON.parse(localStorage.getItem("action"));
      effort = JSON.parse(localStorage.getItem("effort"));
      if (recordurl.indexOf("api/challenge") > -1) {
        let pvp = JSON.parse(this.responseText);
        let level = pvp.gained.prevLV;
        if (lastchallengeType < 3) {
          if (pvp.result == "勝利") {
            action.pw += 1;
            action.pwx += 1;
            dealEffortCount("pw", 1, level);
          } else if (pvp.result == "戰敗") {
            action.pl += 1;
            action.plx += 1;
            dealEffortCount("pl", 1, level);
          }
        } else if (lastchallengeType == 3) {
          if (pvp.result == "勝利") {
            if (Math.abs(killlv - pvp.gained.prevLV) < 10) {
              action.pk += 1;
              action.pkx += 1;
              dealEffortCount("pk", 1, level);
            } else {
              action.pw += 1;
              action.pwx += 1;
              dealEffortCount("pw", 1, level);
        ...
        if (pvp.gained.nextLV > pvp.gained.prevLV) {
          resetActionRecode(1);
          resetEffortRecode();
        } else {
          localStorage.setItem("action", JSON.stringify(action));
      ...
      } else if (recordurl.indexOf("doaction") > -1) {
        let actionResult = JSON.parse(this.responseText);
        if (actionResult.message == "行動成功！" || actionResult.message == "領取成功！") {
          if (lastAcction != "floorBonus") {
            action[lastAcction] += 1;
          }
          let level = actionResult.gained.prevLV;
          switch (lastAcction) {
            case "1h":
            case "2h":
            case "4h":
              dealEffortCount(lastAcction, 2, level);
              action[lastAcction + "x"] += 1;
              break;
            case "floorBonus":
              action.floor += 1;
            case "8h":
              dealEffortCount(lastAcction, 3, level);
              action[lastAcction + "x"] += 1;
              break;
            default:
              dealEffortCount(lastAcction, 1, level);
              action[lastAcction + "x"] += 1;
              break;
          }
        }
        if (actionResult.gained.nextLV > actionResult.gained.prevLV) {
          resetActionRecode(1);
          resetEffortRecode();
        } else {
          localStorage.setItem("action", JSON.stringify(action));
      ...
      } else if (recordurl.indexOf("reincarnation") > -1) {
        resetActionRecode(2);
        resetEffortRecode();
      } else if (recordurl.indexOf("/api/my-kirito") > -1) {
        updateActionStatus();
        updateEffortStatus();
      } else if (recordurl.indexOf("achievements") > -1) {
        reSortAch(this.responseText);
      }
    });
  ...
  var newSend = function(vData) {
    lastAcction = '';
    if (recordurl.indexOf("doaction") > -1) {
      lastAcction = JSON.parse(vData).action;
    } else if (recordurl.indexOf("api/challenge") > -1) {
      let pvpData = JSON.parse(vData);
      lastchallengeType = pvpData.type;
      killlv = pvpData.lv;
    }
    this.realSend(vData);
  };
