// ==UserScript==
// @name       Hybrid - Calls - Contact (No Title)
// @version    2.01
// @author	   jawz
...
// @downloadURL https://update.greasyfork.org/scripts/28027/Hybrid%20-%20Calls%20-%20Contact%20%28No%20Title%29.user.js
// @updateURL https://update.greasyfork.org/scripts/28027/Hybrid%20-%20Calls%20-%20Contact%20%28No%20Title%29.meta.js
// ==/UserScript==
...
if ($('li:contains("Call - Contact")').length) {
    ...
    createButton('Biz on VM',0,0,2,'Business, but no contact on voicemail');
    createButton('Wrong Biz',0,1,2,'Wrong business');
    createButton('Closed Biz',0,1,1,'Out of business');
    createButton('DC',1,2,2,'Disconnected');
    createButton('DC - Directory',1,2,2,'Disconnected - Directory message');
    createButton('DC - No Complete',1,2,2,'Disconnected - Could not be completed as dialed');
    createButton('Fax',2,2,2,'Fax');
    createButton('Generic VM',0,2,2,'Generic voicemail with no info.');
    createButton('Personal VM',0,2,2,'Personal voicemail with no info.');
    createButton('Cant tell VM',0,2,2,'Cant tell if businesses or not from voicemail');
    createButton('No Contact VM',0,0,2,'Voicemail without contact');
    createButton('Ringing', 3,2,2,'Kept ringing');
