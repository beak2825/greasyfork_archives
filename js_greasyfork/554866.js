...
// @run-at      document-end
// @downloadURL https://update.greasyfork.org/scripts/554866/Qiuwei%20Chat%20Assistant.user.js
// @updateURL https://update.greasyfork.org/scripts/554866/Qiuwei%20Chat%20Assistant.meta.js
// ==/UserScript==
...
;(function() {
    ...
    function capturePageContent(forceCapture = false, retryCount = 0) {
        try {
            ...
            // // 检测昵称变化
            // const nicknameChanged = nickname && nickname !== state.previousNickname;
            // if (nicknameChanged) {
            //     state.previousNickname = nickname;
            //     // 如果昵称发生变化，清空微信号
            //     const wxIdInput = document.getElementById('wx_id');
            //     if (wxIdInput) {
            //         wxIdInput.value = '';
            ...
            //     // 清空该昵称的微信号记录
            //     if (nickname) {
            //         state.nicknameWxIdMap[nickname] = '';
            //     }
            // }
    ...
    function handleSubmit() {
        ...
        setTimeout(() => {
            ajaxRequest({
                method: "POST",
                url: "http://47.97.11.56:8000/api/n8n/assistantChat",
                data: JSON.stringify({
                ...
                success: function(response) {
                        ...
                        try {
                            ...
                            if (result && result.code === 200) {
                                ...
                                // const currentWxId = document.getElementById('wx_id')?.value?.trim();
                                // if (currentNickname && currentWxId) {
                                //     state.nicknameWxIdMap[currentNickname] = currentWxId;
                                // }
                            } else {
    ...
    function handleSave() {
        ...
        ajaxRequest({
            method: "POST",
            url: "http://47.97.11.56:8000/api/n8n/assistantChatWrite",
            data: JSON.stringify({
    ...
    function handleReCapture() {
        ...
        // const currentWxId = wxIdInput.value.trim();
        ...
        // // 检查昵称是否发生变化
        // if (currentNickname && currentNickname !== state.previousNickname) {
        //     // 昵称发生变化，清空微信号
        //     wxIdInput.value = '';
        //     state.nicknameWxIdMap[currentNickname] = ''; // 清空该昵称的微信号记录
        //     showToast('昵称已变化，已清空微信号');
        // } else if (currentNickname && state.nicknameWxIdMap[currentNickname] && !currentWxId) {
        //     // 昵称未变化且当前微信号为空，恢复该昵称对应的微信号
        //     wxIdInput.value = state.nicknameWxIdMap[currentNickname];
        //     showToast('已恢复该昵称对应的微信号');
        // } else if (currentNickname && currentNickname === state.previousNickname && currentWxId) {
        //     // 昵称未变化且当前有微信号，保持当前微信号不变
        //     showToast('昵称未变化，保持当前微信号');
        // } else if (currentNickname && currentNickname === state.previousNickname && !currentWxId) {
        //     // 昵称未变化但当前微信号为空，尝试恢复
        //     if (state.nicknameWxIdMap[currentNickname]) {
        //         wxIdInput.value = state.nicknameWxIdMap[currentNickname];
        //         showToast('已恢复该昵称对应的微信号');
        //     } else {
        //         showToast('该昵称暂无对应的微信号记录');
        //     }
        // }
    ...
    // // 调试函数：查看当前状态
    // window.debugQiuweiState = function() {
    //     console.log('当前状态:', {
    //         previousNickname: state.previousNickname,
    //         currentNickname: state.currentNickname,
    //         nicknameWxIdMap: state.nicknameWxIdMap,
    //         lastCapturedNickname: state.lastCapturedNickname
    //     });
    // };
})();