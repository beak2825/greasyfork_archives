// ==UserScript==
// @name           TrapDesuAvisDiff
// @description    Outil de discussion instantanée permet de voir la couleur des pseudos (modo, admins, etc) + Liens Msg
...
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/563538/TrapDesuAvisDiff.user.js
// @updateURL https://update.greasyfork.org/scripts/563538/TrapDesuAvisDiff.meta.js
// ==/UserScript==
...
async function postJvcMessage() {
    ...
    try {
        ...
        if (messageId) {
            ...
            /* [RMV : CONFIDENTIALITE (version moderateur)]
...
async function submitEditedMessage(messageBloc, messageId, newText, formSession, ajaxHash) {
    ...
    try {
        ...
        if (data.html) {
            originalContentDiv.innerHTML = `<div class="txt-msg  text-enrichi-forum ">${data.html}</div>`;
...
function makeMessage(message) {
    ...
    /* [ADD ModeColo] */
    let mggHref = exists ? `href="/forums/message/${id}" target="_blank"` : "";
    /*change la couleur si le pseudo à une classe AUTRE QUE TEXT USER */
    let classUser = message.classUser;
    author = `<a ${mggHref} class="${classUser}">${author}</a>`;
    /* [ADD END] */
