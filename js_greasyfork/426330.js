...
// @author       Sakaiyo & Chandog#6373 & Cullen & RevoGen
// @match        https://hordes.io/play
// @grant        GM_addStyle
// @namespace https://greasyfork.org/users/160017
// @downloadURL https://update.greasyfork.org/scripts/426330/Hordes%20UI%20Mod.user.js
// @updateURL https://update.greasyfork.org/scripts/426330/Hordes%20UI%20Mod.meta.js
// ==/UserScript==
...
function addLockItemContextMenu() {
  ...
  setTimeout(() => {
    ...
    $contextMenu.appendChild((0, _misc.makeElement)({
      ...
      //note
    //if (!document.querySelector('.js-lock-item')) return; // quit if does not exist
...
function handleMerchantFilterInputChange() {
  ...
  // NOTE
  var filters = value.split(',').map(v => v.trim()) || [''];
    if (filters[0] === "default") {
        filters = ["3stat", "nowrongbloodline", "nocrap", "properif"].concat(filters.slice(1))
    }
  ...
  $items.forEach($item => {
    ...
    var tooltipTextContent = tooltipContent.innerHTML.replace(/<[^>]*>/g, '\n').replace(/\n\s*\n/g, '\n').substring(1).toLowerCase();
    var type_regex = /(?:common|uncommon|rare|epic|legendary)(?:\s)(\w+)/g
    var type = type_regex.exec(tooltipTextContent);
    if (type == null) {
        type = "";
    } else {
        type = type[1];
    }
    if (tooltipTextContent.includes(" - ")){
        // treat min max combination (like 1-2 damage) as 2 stats, except when it is a weapon
        // check if item has Damage
        var is_weapon = ["hammer", "bow", "staff", "sword"].includes(type);
        var dmg_regex = /(\d+)(?:\s+-\s+)(\d+)/g

        if (is_weapon) {
            var replacement_string = "$1 min dmg.\n$2 max dmg.";
        } else {
            var replacement_string = "+ $1 min dmg.\n+ $2 max dmg."
        }
        tooltipTextContent = tooltipTextContent.replace(dmg_regex, replacement_string);
    }

    var plusTooltipTextContent = tooltipTextContent.split("\n").filter((line) => line.startsWith("+")).join("\n");


    let filterMatchCount = 0;
    filters.forEach(filter => {
        // since newlines are not preserved, recreate newlines from innerHTML
        // tag strip, then remove consecutive newlines, substring removes leading newline

        if (filter === ''){
            filterMatchCount++;
        } else if (filter === '3stat'){
            // only show 3 stat-4 stat items
            if ((plusTooltipTextContent.split('+ ').length - 1) >= 3) {
                filterMatchCount++;
            }
        } else if (filter === 'nowrongbloodline') {
            var bloodlines = ["strength", "intelligence", "dexterity", "wisdom"];
            var bloodline_counters = [0, 0, 0, 0];
            var counter = 0;

            for (let i = 0; i < bloodlines.length; i++) {
                if (plusTooltipTextContent.includes(bloodlines[i])) {
                    bloodline_counters[i] += 1;
                    counter += 1;
                }
            }
            if (counter == 0) {
                ...
                var bloodline_dict = {"sword": 0, "shield": 0, "staff": 1, "orb": 1, "bow": 2, "quiver": 2, "hammer": 3, "totem": 3};
                if (Object.keys(bloodline_dict).includes(type)) {
                    if (bloodline_counters[bloodline_dict[type]] == 1) {
                        filterMatchCount++;
                    }
                } else {
                    filterMatchCount++;
        ...
        } else if (filter === "nocrap") {
            // HPr is included in HP, MPr is included in MP
            var crap = ["hp", "mp"];
            var counter = 0;

            for (let i = 0; i < crap.length; i++) {
                if (plusTooltipTextContent.includes(crap[i])) {
                    counter += 1;
                }
            }
            if (counter == 0) {
        ...
        }  else if (filter === "properif"){
            // i.e., valid only if it has both IF & luck, or neither, doesn't account for str if haste
            var stats = ["luck", "item find"];
            var counter = 0;

            for (let i = 0; i < stats.length; i++) {
                if (plusTooltipTextContent.includes(stats[i])) {
                    counter += 1;
                }
            }
            ...
            var matchesFilter;
            var tempTooltipTextContent = tooltipTextContent;
            var opposite = false;
            var filter_counter = 0;

            if (filter.substring(0,2).includes("+")) {
                tempTooltipTextContent = plusTooltipTextContent;
                filter_counter += 1;
            }
            if (filter.substring(0,2).includes("-")) {
                opposite = true;
            ...
            filter = filter.substring(filter_counter);

            matchesFilter = filter.toLowerCase().split('/').map(_ => tempTooltipTextContent.includes(_)).includes(true);
            if (opposite) {
