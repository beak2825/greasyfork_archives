// ==UserScript==
// @name          URL aware Text Highlight and Seek
// @author        guebosch, erosman and Jefferson "jscher2000" Scher
// @namespace     guebosch
// @version       2026-01-23
// @description   Automatically highlight user-defined text with Seek function on specified url patterns.
...
// @copyright     Copyright 2019 Jefferson Scher. Portions created by erosman. Portions created by guebosch & claude in 2026
// @license       BSD-3-clause
// @downloadURL https://update.greasyfork.org/scripts/563781/URL%20aware%20Text%20Highlight%20and%20Seek.user.js
// @updateURL https://update.greasyfork.org/scripts/563781/URL%20aware%20Text%20Highlight%20and%20Seek.meta.js
// ==/UserScript==
/* --------- Note ---------
...
async function THS_init(){
  ...
  if (!hljson || hljson.length == 0){
    ...
    try {
      ...
      console.error("Error parsing stored keyword styles:", err);
      hlobj = hlobjDefault;
    }
  }
  // global keys array
  ...
  kwhibar.innerHTML = "<form id=\"thdtopform\" onsubmit=\"return false\"><p id=\"thdtopbarhome\"><span style=\"display:block;width:22px;height:22px;\"></span></p>" +
    ...
    "> Match in pre/code</label></li><li><label style=\"padding-left:4px\">iFramed pages:</label><br><select id=\"hlframeselect\" size=\"3\"><option value=\"none\">No highlighting</option><option value=\"same\">Same site only</option>" +
    "<option value=\"any\">Any site</option></select></li><li><button id=\"btnthsreread\" title=\"Update from and apply stored settings\">Re-Read Saved Prefs</button></li></ul></div>" +
    ...
    "#thdtopdrop td{padding:4px 4px; vertical-align:top;border-right:1px solid #000;border-bottom:1px solid #000;} #thdtopdrop td div{word-wrap:break-word;font-size:12px} #thdtopdrop p{margin-top:8px;margin-bottom:0;} " +
  ...
  kwhied.innerHTML = "<form onsubmit=\"return false;\"><p style=\"margin-top:0\"><b>Edit/Add Keywords/Highlighting</b>" +
    ...
    "<table cellspacing=\"0\" style=\"table-layout:fixed\"><tbody><tr kwhiset=\"new\"><td style=\"width:calc(100% - 500px)\">" +
    ...
    "<td style=\"width:480px\" id=\"stylecontrols\"><p><span>Text color:</span> <input id=\"txtcolorinput\" type=\"color\" value=\"#000000\" title=\"Pop up color picker\"> " +
    ...
    "<p><span>URL pattern:</span> <input type=\"text\" id=\"kwhiurlpattern\" style=\"width:65%\" title=\"Semicolon-separated URL patterns (e.g., https://example.com/*; https://test.com/*). Leave empty to apply to all sites.\"> </p>" +
    "<p><button id=\"btnkwhisave\">Save Changes</button> <button id=\"btnkwhicancel\">Discard Changes</button> " +
    "<button id=\"btnkwhiremove\">Delete Set</button> <button id=\"btnkwhirevert\" disabled>Revert Last Keyword Edit</button></p></form><style type=\"text/css\">" +
    "#kwhiedit{position:fixed;top:1px;left:150px;width:950px;height:400px;border:1px solid #000;border-radius:6px;padding:1em;color:#000;" +
...
// Check if current URL matches the set's URL patterns
function checkURLPatterns(urlpatterns){
  if (!urlpatterns || urlpatterns.length === 0) return true; // No patterns = apply to all
  var patterns = urlpatterns.split(";").map(function(p){ return p.trim(); });
  ...
  for (var i = 0; i < patterns.length; i++){
    ...
    var escapedParts = parts.map(function(p){
      return p.replace(/[.+^${}()|[\]\\]/g, "\\$&");
    });
    var regexPattern = escapedParts.join(".*");
...
function THmo_doHighlight(el,subset){
  ...
  for (var j = 0; j < keyset.length; ++j) {
    ...
    if (hlobj[hlset].visible == "true" && hlobj[hlset].enabled == "true"){
      // Check if current URL matches the set's URL patterns
      if (!checkURLPatterns(hlobj[hlset].urlpatterns)) continue;
...
function refreshSetList(e){
  ...
  for (var j = 0; j < hlkeys.length; ++j){
    ...
    if (hlobj[hlset].visible == "true"){
      ...
      if (j == 0){
        newrow.innerHTML = '<td style=\"width:272px\"><div class=\"' + hlset + '\">' + escapeHtml(hlobj[hlset].keywords) + '</div></td>' +
          '<td style=\"width:195px\"><button kwhiset=\"' + hlset + '\" title=\"Bring matches into view\">⌕</button> ' +
          '<button kwhiset=\"' + hlset + '\" title=\"Edit\">✎</button> ' +
          '<button kwhiset=\"' + hlset + '\" title=\"Clone this set\">⎘</button> ' +
          '<label><input type=\"checkbox\" kwhiset=\"' + hlset +
          '\"' + strchk + '\"></label></td>';
      } else {
        newrow.innerHTML = '<td><div class=\"' + hlset + '\">' + escapeHtml(hlobj[hlset].keywords) + '</div></td>' +
          '<td><button kwhiset=\"' + hlset + '\" title=\"Bring matches into view\">⌕</button> ' +
          '<button kwhiset=\"' + hlset + '\" title=\"Edit\">✎</button> ' +
          '<button kwhiset=\"' + hlset + '\" title=\"Clone this set\">⎘</button> ' +
          '<label><input type=\"checkbox\" kwhiset=\"' + hlset +
          '\"' + strchk + '\"></label></td>';
...
async function kwhiformevent(e){
  ...
  if (e.target.nodeName == "BUTTON"){ // Call up edit form or find bar
    ...
    if (e.target.textContent == "✎"){
      ...
      // set URL pattern field
      document.getElementById("kwhiurlpattern").value = hlobj[hlset].urlpatterns || "";
...
function kwhinewset(e,kwtext){ // call up new set form
  ...
  document.getElementById("kwhipattype").selectedIndex = 0;
  // clear URL pattern field for new set
  document.getElementById("kwhiurlpattern").value = "";
  // set style editing to defaults
  kwhieditstyle = ["rgb(0,0,255)","rgb(255,255,0)","inherit",""];
  kwhiShowEditForm();
}
function kwhiduplicate(sourceSet){ // duplicate an existing set
  // set set number attribute for temporary edit
  document.querySelector('#kwhiedit tr').setAttribute('kwhiset', 'new');
  // Copy all properties from source set
  document.querySelector('#kwhiedit td:nth-of-type(1) p:nth-of-type(1)').className = "";
  document.querySelector('#kwhiedit td:nth-of-type(1) p:nth-of-type(1)').textContent = hlobj[sourceSet].keywords;
  document.getElementById("kwhipattype").selectedIndex = 0;
  if (hlobj[sourceSet].type == "word") document.getElementById("kwhipattype").selectedIndex = 1;
  if (hlobj[sourceSet].type == "regex") document.getElementById("kwhipattype").selectedIndex = 2;
  // Set URL pattern field
  document.getElementById("kwhiurlpattern").value = hlobj[sourceSet].urlpatterns || "";
  // set style editing from source set
  kwhieditstyle = [
    hlobj[sourceSet].textcolor || "rgb(0,0,255)",
    hlobj[sourceSet].backcolor || "rgb(255,255,0)",
    hlobj[sourceSet].fontweight || "inherit",
    hlobj[sourceSet].custom || ""
  ];
  kwhiShowEditForm();
}
  ...
  if (arc.toLowerCase() == "t"){ // Total replacement
    ...
    for (var j = 0; j < keysImport.length; ++j){ // Add/replace individual sets
      ...
      if(keysString.indexOf("|"+impset+"|") > -1 && arc.toLowerCase() == "r"){ // replace
        ...
        hlobj[hlset].urlpatterns = objImport[impset].urlpatterns || "";
        hlobj[hlset].updated = (new Date()).toJSON();
      } else { // add
        ...
        hlobj[hlset] = {
          ...
          urlpatterns : objImport[impset].urlpatterns || "",
...
function kwhicancelipe(setno){
  ...
  if (setno && setno != ''){
    ...
    var divs = document.querySelectorAll('#kwhitbod div');
...
async function kwhisavechg(e){
  ...
  if (hlset == ""){
    ...
    hlobj[hlset] = {
      ...
      enabled : "true",
      visible : "true",
      urlpatterns : document.getElementById("kwhiurlpattern").value,
      updated : ""
    }
    // Update the global key array
    hlkeys = Object.keys(hlobj);
    ...
    hlobj[hlset].urlpatterns = document.getElementById("kwhiurlpattern").value;
    // Set updated date/time
    hlobj[hlset].updated = (new Date()).toJSON();
  }
  // Persist the object
...
async function kwhiremove(e){
  var hlset = document.querySelector('#kwhiedit td:nth-of-type(1) p:nth-of-type(1)').className;
  if (hlset == ""){
    alert("This set has not been saved and therefore does not need to be deleted, you can just close the dialog to discard it.");
  } else {
    if (confirm("Are you sure you want to permanently delete this set? This cannot be undone.")){
      // Delete the set from the object
      delete hlobj[hlset];
      // Update the global key array
      hlkeys = Object.keys(hlobj);
      // Persist the object
