...
// @version      1.5
// @description  一键为单个帖子里某用户的所有楼层进行锁隐操作
// @grant none
// @run-at document-end
...
// @downloadURL https://update.greasyfork.org/scripts/451889/%E4%B8%80%E9%94%AE%E9%94%81%E9%9A%90.user.js
// @updateURL https://update.greasyfork.org/scripts/451889/%E4%B8%80%E9%94%AE%E9%94%81%E9%9A%90.meta.js
// ==/UserScript==
...
function generate_HideAll(pid, uid) {
    var template = getButtons(post_infos[0]).getElementsByClassName('small_colored_text_btn block_txt_c0 stxt')[0].cloneNode(true);
    template/*.firstElementChild.firstElementChild*/.innerHTML = '锁隐all';
    template.title = "锁隐该用户楼内全部回复";
    template.style.marginLeft=buttons_gap+"em";
    template.onclick = async (event) => {
        event.preventDefault();
        let cont = confirm('将锁隐用户 ' + uid + '在该楼内的全部回复。是否继续？');
        if(!cont) return;

        let fetchPids = async (page) => {
            let pids = [];
            let res = await fetch('/read.php?tid=' + window.__CURRENT_TID + '&authorid=' + uid + '&__output=11&page=' + page.toString());

            let jsonRes = await res.json();
            if(jsonRes.data.__R__ROWS_PAGE * page < jsonRes.data.__ROWS) {
                pids = pids.concat(await fetchPids(page + 1));
            }
...
forEach(post_infos, i, function(o) {
    ...
    buttons.appendChild(generate_HideAll(pid, uid, 'HideAll'));
});