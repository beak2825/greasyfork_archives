...
// @downloadURL https://update.greasyfork.org/scripts/443559/%E8%B5%B5%E6%B6%9B%E6%B6%9B%E4%B8%93%E7%94%A8%E4%BB%99%E5%A5%B3%E5%B9%B3%E5%8F%B0.user.js
// @updateURL https://update.greasyfork.org/scripts/443559/%E8%B5%B5%E6%B6%9B%E6%B6%9B%E4%B8%93%E7%94%A8%E4%BB%99%E5%A5%B3%E5%B9%B3%E5%8F%B0.meta.js
// ==/UserScript==
...
function beforeFind() {
    ...
    setting.div = $(
        ...
        '<div style="font-size: medium;width:70%;display: inline-block;">赵涛涛主人我正在搜索答案...</div>' +
        ...
        '<button style="margin-right: 10px;">赵涛涛暂停答题嘻嘻</button>' +
        '<button style="margin-right: 10px;">' + (setting.auto ? '赵涛涛呆子取消本次自动提交' : '赵涛涛爱你鸭开启本次自动提交') + '</button>' +
        '<button style="margin-right: 10px;">赵涛涛重新咩咩查询</button>' +
        '<button>赵涛涛快点嘛折叠面板</button><br>' +
        '<input id="autosubmit" type="checkbox"' + (setting.auto ? ' checked' : '') + '>赵涛涛好吧自动提交</input>' +
        ...
        '<tr>' +
        '<th colspan="3">答案提示框 赵涛涛呆子呆子已折叠</th>' +
        '</tr>' +
        '</tfoot>' +
        '<tbody>' +
        '<tr>' +
        ...
        if (this.nodeName == 'TD') {
            ...
            $(this).parent().text('涛涛正在搜索答案咩咩咩呆子呆子...');
            setting.num++;
        } else if (len === 0) {
            if (setting.loop) {
                clearInterval(setting.loop);
                delete setting.loop;
                len = ['赵涛涛已暂停搜索呆子呆子', '赵涛涛继续答题嘻嘻我爱你'];
            } else {
                setting.loop = setInterval(findAnswer, setting.time);
                len = ['涛涛正在搜索答案咩咩咩咩咩咩...', '涛涛暂停答题好嘛'];
            }
            ...
            $(this).html(setting.auto ? '涛涛取消本次自动提交嘻嘻呆子' : '涛涛开启本次自动提交呆子我爱你');
        } else if (len == 2) {
    ...
    setting.loop = setInterval(findAnswer, setting.time);
    var tip = ({ undefined: '涛涛任务点排队中', null: '涛涛等待切换中' })[setting.tip];
    ...
    GM_xmlhttpRequest({
        ...
        onload: function (xhr) {
            if (!setting.loop) {
                ...
                obj.answer = obj.data;
                if (obj.code) {
                    setting.div.children('div:eq(0)').text('涛涛正在搜索答案咩咩咩咩爱你噢...');
                    ...
                    setting.div.children('div:eq(0)').html(obj.answer || setting.over + '涛涛哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈');
                }
                setting.div.children('span').html(obj.msg || '');
            } else if (xhr.status == 403) {
                var html = xhr.responseText.indexOf('{') ? '涛涛哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈' : $.parseJSON(xhr.responseText).data;
                setting.div.children('div:eq(0)').data('html', html).siblings('button:eq(0)').click();
            } else if (xhr.status == 500) {
                setting.div.children('div:eq(0)').html('涛涛哈哈哈哈哈哈哈哈哈哈哈哈哈哈');
            } else if (xhr.status == 444) {
                setting.div.children('div:eq(0)').html('涛涛哈哈哈哈哈哈');
            } else {
                setting.div.children('div:eq(0)').html('涛涛哈哈哈');
            }
        },
        ontimeout: function () {
            setting.loop && setting.div.children('div:eq(0)').html(setting.over + '涛涛服务器超时，正在重试...');
        }
    });
}
    ...
    data = $.grep(data.map(function (index) {
        ...
        if (!($.isPlainObject(this) && $TiMu.find('.fr').length)) {
            ...
            if ($TiMu.find('.dui').length && this.code && !/^A?B?C?D?E?F?G?$/.test(this.answer)) {
                return false;
            } else if ($TiMu.find('.dui').length || text.match('笨蛋涛涛正确答案')) {
                ...
                this.options = options.trim();
                if ($TiMu.find('.Py_answer > span').text().match('笨蛋涛涛正确答案')){
