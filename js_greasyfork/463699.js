...
// @downloadURL https://update.greasyfork.org/scripts/463699/Test.user.js
// @updateURL https://update.greasyfork.org/scripts/463699/Test.meta.js
// ==/UserScript==
...
const buttons = [
    ...
    },
    {
      ...
      content:
        ...
        "[CENTER][size=1][ISPOILER]@Angel_Sakurai, @Roman_Kiriluyk [/ISPOILER][/size][/CENTER]<br>" +
    ...
    },
    {
    ...
    
     {
      ...
      content:
        ...
        "[CENTER][ISPOILER]@Stas_Mikhnyk[/ISPOILER][/CENTER]<br>" ,
      prefix: PIN_PREFIX,
	  status: true,
    },
    {
...
const xp = [
    ...
    {
        title: '________В общем я старался_________'
    },
    {
        title: '_________сделать скрипт более приятным для использованием...________'
    },

    {
        title: '_________Делал скрипт в соло 2 дня..._________'
    },

    {
        title: '_________________________________Скрипт был обновлен___________________________'
    },
    {
        title: '___________________________11.04.2023 в 12:27___________________________',
    },
...
$(document).ready(() => {
    ...
    addButton('💎 Биографии и т.д. 💎', 'selectAnswer2');
    addButton('♥Скрипт от Angel_Sakurai♥ ', 'xp');
...
return `<div class="xp">${xp
  .map(
	(btn, i) =>
	  `<button id="xp-${i}" src="https://www.youtube.com/watch?v=dQw4w9WgXcQ" class="button--primary button ` +
	  `rippleButton"  src="https://www.youtube.com/watch?v=dQw4w9WgXcQ" style="margin:5px"><span class="button-text">${btn.title}</span></button>`,
  )
  .join('')}</div>`;
}
...
function pasteContent(id, data = {}, send = false) {
	const template = Handlebars.compile(buttons2[id].content2);
	if ($('.fr-element.fr-view p').text() === '') $('.fr-element.fr-view p').empty();
