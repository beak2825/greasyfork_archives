...
// @version      0.0.5
// @license GPL
// @description  使用sd、novelai3、comfyui、std 配合ai在酒馆生成插图 以及酒馆正则管理
// @author       123
// @grant        unsafeWindow
// @match        *://*/*
...
// @namespace https://greasyfork.org/users/1495206
// @downloadURL https://update.greasyfork.org/scripts/542650/%E9%80%9A%E7%94%A8ai%E6%8F%92%E5%9B%BE%E8%84%9A%E6%9C%AC.user.js
// @updateURL https://update.greasyfork.org/scripts/542650/%E9%80%9A%E7%94%A8ai%E6%8F%92%E5%9B%BE%E8%84%9A%E6%9C%AC.meta.js
// ==/UserScript==
...
( function() {
 ...
 let json3=`{
  ...
  // 初始化并启动定时器
  function initialize() {
    console.log('初始化AI插图脚本...');
    ster = setInterval(addNewElement, 2000);
    // 设置定时器检查并替换文本
    setInterval(chenk, 2000);
    // 立即执行一次
    setTimeout(chenk, 1000);
    console.log('AI插图脚本初始化完成！');
  }
  ...
  const dbName = 'tupian';
  const dbVersion = 1;
  const objectStoreName = 'tupianstdncun';
...
async function Storedelete(id){//使用数据库
  ...
  const defaultSettings = {
      ...
      stdApi: {
        sq: '',  // 网站授权key
        naiKey: '', // nai官方key
        model: 'nai-diffusion-3', // 默认模型
        artist: '', // 画师
        size: '竖图', // 默认尺寸
        scale: '5', // 提示词引导值
        steps: '23', // 步数
        cfgRescale: '0', // 缩放提示词引导值
        sampler: 'k_euler', // 采样器
        noiseSchedule: 'native', // 噪声调度
  ...
  for (const [key, defaultValue] of Object.entries(defaultSettings)) {
      ...
      if (settings[key] === defaultValue) {
          if(key=="yushe"||key=="workers"||key=="worker"||key=="stdApi"){
              GM_setValue(key, JSON.stringify(defaultValue));//使用字符串保存
          }else{
              GM_setValue(key, defaultValue);
          }
      }else if(key=="yushe"||key=="workers"||key=="stdApi"){
          ...
          try {
            ...
            if(key=="workers"){
              ...
              settings.stdApi = defaultSettings.stdApi;
            }
          }
...
`;
  ...
  <label>模式：
    <select id="mode">
      ...
      <option value="std" ${settings.mode === 'std' ? 'selected' : ''}>STD API</option>
    </select>
  </label><br>
...
`
 ...
 html +=`
  ...
  `
    if(settings.mode === 'std') {
      html += `
        <div class="std-settings">
          <h3>STD API 设置</h3>
          <label>授权Key：<input type="text" id="stdApiSq" value="${settings.stdApi.sq || ''}"></label><br>
          <label>NAI官方Key：<input type="text" id="stdApiNaiKey" value="${settings.stdApi.naiKey || ''}"></label><br>
          <label>模型：
            <select id="stdApiModel">
              <option value="nai-diffusion-3" ${settings.stdApi.model === 'nai-diffusion-3' ? 'selected' : ''}>nai-diffusion-3</option>
              <option value="nai-diffusion-furry-3" ${settings.stdApi.model === 'nai-diffusion-furry-3' ? 'selected' : ''}>nai-diffusion-furry-3</option>
              <option value="nai-diffusion-4-full" ${settings.stdApi.model === 'nai-diffusion-4-full' ? 'selected' : ''}>nai-diffusion-4-full</option>
              <option value="nai-diffusion-4-curated-preview" ${settings.stdApi.model === 'nai-diffusion-4-curated-preview' ? 'selected' : ''}>nai-diffusion-4-curated-preview</option>
              <option value="nai-diffusion-4-5-full" ${settings.stdApi.model === 'nai-diffusion-4-5-full' ? 'selected' : ''}>nai-diffusion-4-5-full</option>
              <option value="nai-diffusion-4-5-curated" ${settings.stdApi.model === 'nai-diffusion-4-5-curated' ? 'selected' : ''}>nai-diffusion-4-5-curated</option>
            </select>
          </label><br>
          <label>画师：<input type="text" id="stdApiArtist" value="${settings.stdApi.artist || ''}"></label><br>
          <label>尺寸：
            <select id="stdApiSize">
              <option value="竖图" ${settings.stdApi.size === '竖图' ? 'selected' : ''}>竖图</option>
              <option value="横图" ${settings.stdApi.size === '横图' ? 'selected' : ''}>横图</option>
              <option value="方图" ${settings.stdApi.size === '方图' ? 'selected' : ''}>方图</option>
            </select>
          </label><br>
          <label>提示词引导值：<input type="number" id="stdApiScale" value="${settings.stdApi.scale}" step="0.1"></label><br>
          <label>步数：<input type="number" id="stdApiSteps" value="${settings.stdApi.steps}"></label><br>
          <label>缩放提示词引导值：<input type="number" id="stdApiCfgRescale" value="${settings.stdApi.cfgRescale}" step="0.01"></label><br>
          <label>采样器：
            <select id="stdApiSampler">
              <option value="k_euler_ancestral" ${settings.stdApi.sampler === 'k_euler_ancestral' ? 'selected' : ''}>k_euler_ancestral</option>
              <option value="k_euler" ${settings.stdApi.sampler === 'k_euler' ? 'selected' : ''}>k_euler</option>
              <option value="k_dpmpp_2s_ancestral" ${settings.stdApi.sampler === 'k_dpmpp_2s_ancestral' ? 'selected' : ''}>k_dpmpp_2s_ancestral</option>
              <option value="k_dpmpp_2m_sde" ${settings.stdApi.sampler === 'k_dpmpp_2m_sde' ? 'selected' : ''}>k_dpmpp_2m_sde</option>
              <option value="k_dpmpp_2m" ${settings.stdApi.sampler === 'k_dpmpp_2m' ? 'selected' : ''}>k_dpmpp_2m</option>
              <option value="k_dpmpp_sde" ${settings.stdApi.sampler === 'k_dpmpp_sde' ? 'selected' : ''}>k_dpmpp_sde</option>
              <option value="ddim_v3" ${settings.stdApi.sampler === 'ddim_v3' ? 'selected' : ''}>ddim_v3</option>
            </select>
          </label><br>
          <label>噪声调度：
            <select id="stdApiNoiseSchedule">
              <option value="native" ${settings.stdApi.noiseSchedule === 'native' ? 'selected' : ''}>native</option>
              <option value="exponential" ${settings.stdApi.noiseSchedule === 'exponential' ? 'selected' : ''}>exponential</option>
              <option value="polyexponential" ${settings.stdApi.noiseSchedule === 'polyexponential' ? 'selected' : ''}>polyexponential</option>
              <option value="karras" ${settings.stdApi.noiseSchedule === 'karras' ? 'selected' : ''}>karras</option>
            </select>
          </label><br>
          <label>使用缓存：
            <select id="stdApiUseCache">
              <option value="true" ${settings.stdApi.useCache ? 'selected' : ''}>是</option>
              <option value="false" ${!settings.stdApi.useCache ? 'selected' : ''}>否</option>
            </select>
          </label><br>
        </div>
      `
...
`;
      ...
      document.getElementById('mode').addEventListener('change', function() {
          ...
          // 隐藏所有模式特定的设置
          document.querySelectorAll('.std-settings, .novelai-settings, .sd-settings, .comfyui-settings, .free-settings').forEach(el => {
              el.style.display = 'none';
          });

          // 显示当前模式的设置
          if(mode === 'std' && document.querySelector('.std-settings')) {
              document.querySelector('.std-settings').style.display = 'block';
          } else if(mode === 'sd' && document.querySelector('.sd-settings')) {
              document.querySelector('.sd-settings').style.display = 'block';
          } else if(mode === 'comfyui' && document.querySelector('.comfyui-settings')) {
              document.querySelector('.comfyui-settings').style.display = 'block';
          } else if(mode === 'free' && document.querySelector('.free-settings')) {
              document.querySelector('.free-settings').style.display = 'block';
          }
...
function stylishConfirm(message) {
 ...
 async function clearCache() {
  ...
  function saveSettings() {
      ...
      if(key=="stdApi"){
          const stdApiSettings = {
              sq: document.getElementById('stdApiSq')?.value || '',
              naiKey: document.getElementById('stdApiNaiKey')?.value || '',
              model: document.getElementById('stdApiModel')?.value || 'nai-diffusion-3',
              artist: document.getElementById('stdApiArtist')?.value || '',
              size: document.getElementById('stdApiSize')?.value || '竖图',
              scale: document.getElementById('stdApiScale')?.value || '5',
              steps: document.getElementById('stdApiSteps')?.value || '23',
              cfgRescale: document.getElementById('stdApiCfgRescale')?.value || '0',
              sampler: document.getElementById('stdApiSampler')?.value || 'k_euler',
              noiseSchedule: document.getElementById('stdApiNoiseSchedule')?.value || 'native',
              useCache: document.getElementById('stdApiUseCache')?.value === 'true'
          };
          settings[key] = stdApiSettings;
          GM_setValue(key, JSON.stringify(stdApiSettings));
          continue;
      }
...
async function nai3(button1,Xwidth=null,Xheight=null) {
  ...
  let Divide_roles=false;
      if(button1.dataset.link.includes("Scene Composition")&&(settings.novelaimode=="nai-diffusion-4-curated-preview"||settings.novelaimode=="nai-diffusion-4-full")){
        Divide_roles=true;
      }
      ...
      for (var i = 0; i < ps.length; i++) {
          ...
          if(matches){
                ...
                imgSpan.addEventListener('dblclick', (event) => {
                  addSmoothShakeEffect(event.target);
                      if(event.target && event.target.dataset && event.target.dataset.name) {
                          let targetButton = document.getElementById(event.target.dataset.name);
                          if(targetButton) {
                              nai3(targetButton);
                          }
                      }
              ...
              p.addEventListener('click', async function(event) {
                      if (event.target.tagName !== 'BUTTON' || !event.target.id.includes("image")) {
                    return;
                    }
                      nai3(event.target);
                  });
              ...
              if(newbutton && newbutton.parentNode) {
                  // 替换原按钮位置为容器
                  newbutton.parentNode.replaceChild(container, newbutton);
                  // 将按钮添加到容器中
              ...
              if(loadimg && document.getElementById(button.name)){
                ...
                const lastMesText = mesTextElements[mesTextElements.length - 1];
                  if(lastMesText==p && settings.zidongdianji=="true" && newbutton){
                   nai3(newbutton);
                }
          }
          }
...
async function chenk(){
    ...
    }else if(settings.mode=="std"){
        replaceWithStd();
    }
}

async function replaceWithStd() {
    if(!settings.scriptEnabled){
        return;
    }
    if(checkSendBuClass()){
...
`;
  ...
  document.body.appendChild(panel);
  document.head.appendChild(style);
  document.getElementById('save-settings2').addEventListener('click', saveSettings2);
  ...
  hideSettingsPanel2();
    ...
    async function processGenerateImageRequest(requestId,prompt,width,height) {
      ...
      if(settings.mode=="novelai"){
        ...
        console.log('油猴使用STD API生成图片');
        let loadimg=await std(button,width,height);
        console.log('油猴使用STD API生成图片成功');
        await sendGenerateImageResponse(requestId,loadimg);
      }
  }
...
async function std(button1, Xwidth=null, Xheight=null) {
  button1.textContent = "加载中";
  ...
  // 添加参数
  let params = {
    sq: settings.stdApi.sq || '',
    'nai-key': settings.stdApi.naiKey || '',
    model: settings.stdApi.model || 'nai-diffusion-3',
    artist: settings.stdApi.artist || '',
    tag: prompt,
    negative: negative_prompt,
    size: settings.stdApi.size || '竖图',
    scale: settings.stdApi.scale || '5',
    steps: settings.stdApi.steps || '23',
    cfg_rescale: settings.stdApi.cfgRescale || '0',
    sampler: settings.stdApi.sampler || 'k_euler',
    noise_schedule: settings.stdApi.noiseSchedule || 'native'
  };

  // 如果启用缓存，添加nocache参数
  if (settings.stdApi.useCache) {
    params.nocache = 1;
  }
  ...
  try {
    ...
    // 保存图片到缓存
    await setItemImg(prompt, base64);
    ...
    console.error('Error generating image with STD API:', error);
    button1.textContent = "生成图片";
    return null;
  }
}
