...
// @grant          none
// @downloadURL https://update.greasyfork.org/scripts/416852/JVChatPremium%20fix.user.js
// @updateURL https://update.greasyfork.org/scripts/416852/JVChatPremium%20fix.meta.js
// ==/UserScript==
...
#content.jvchat-night-mode #jvchat-leftbar > .panel {
    background-color: #2F3136!important;
}

#content.jvchat-night-mode {
    color: #dcddde!important;
    scrollbar-color: #191A1C #2F3136!important;
}

#content.jvchat-night-mode .jvchat-ruler {
    border-color: #2e3035!important;
    border-top: 1px solid transparent !important;
}

#content.jvchat-night-mode .conteneur-editor,
#content.jvchat-night-mode .bloc-editor-forum,
#content.jvchat-night-mode .jvchat-bloc-message,
#content.jvchat-night-mode .conteneur-messages-pagi {
    background-color: #36393F!important;
}

#content.jvchat-night-mode #message_topic,
#content.jvchat-night-mode .btn-group {
    background: #484C52!important;
}

#content.jvchat-night-mode .jvchat-bloc-avatar {
...
#content.jvchat-night-mode .jvchat-edition-textarea {
    background-color: #2a2a2a!important;
}

#content.jvchat-night-mode .jvchat-buttons button {
    background: #484c52!important;
}

#content.jvchat-night-mode .text-enrichi-forum blockquote.blockquote-jv {
    border-left-color: #484C52!important;
}

#content.jvchat-night-mode .text-enrichi-forum .nested-quote-toggle-box::after {
	background-color: #484c52!important;
	border-color: #737373!important;
	color: #737373!important;
}

#content.jvchat-night-mode .text-enrichi-forum .nested-quote-toggle-box:hover::before {
...
let lastEditionTime = {};  // id => [timestamp, edition] (Value should be set to 'false' for deleted messages)
let messagesByPage = {};
...
function clearPage(document) {
	...
	document.querySelector('.layout__content').style = "position: absolute !important;right: -64px !important;width: 927px !important;grid-row: none !important;";
    ...
    document.getElementById("page-messages-forum").insertAdjacentHTML("afterbegin", "<div id='jvchat-alerts'><div id='jvchat-fixed-alert' class='jvchat-hide'><div class='alert-row'></div></div><div id='jvchat-turbo-warning' class='jvchat-hide'><button class='close jvchat-alert-hide' aria-hidden='true' data-dismiss='alert' type='button'>×</button><div class='alert-row'></div></div><div id='jvchat-degraded-refresh-warning' class='jvchat-hide'><div class='alert-row'></div></div></div>");

     document.getElementById("page-messages-forum").insertAdjacentHTML("afterbegin", PANEL);
     document.getElementById("page-messages-forum").insertAdjacentHTML("beforeend", "<div id='jvchat-right-padding'></div>");
    ...
    if (configuration["night_mode"]) {
        document.getElementById("content").classList.add("jvchat-night-mode");
    }
	...
	setTimeout(function()
	{
		document.querySelector("#page-messages-forum").scrollTo(0, document.querySelector("#page-messages-forum").scrollHeight);
	}, 1000);
...
function toggleNightModeOption(event) {
    ...
    saveConfig();
    document.getElementById("content").classList.toggle("jvchat-night-mode");
}
...
function findDeletedMessages(res, requestTimestamp) {
    ...
    for (let i = 0; i < regIds.length; i++) {
        ...
        // Pas enregistré => blacklist, false => supprimé
        if (!lastEditionTime.hasOwnProperty(id) || lastEditionTime[id] === false) {
            continue;
        }
        ...
        let [timestamp, _] = lastEditionTime[id];
...
function addMessages(messages, editing, requestTimestamp, onMobile) {
    ...
    for (let message of messages) {
        ...
        if (referenced) {
            ...
            let [timestamp, edition] = lastEditionTime[id];
        ...
        if (!referenced || !onMobile) {
            lastEditionTime[id] = [requestTimestamp, edited];
        }
    ...
    if (hasNewMessages) {
        ...
        for (let newMessageId of newMessagesIds) {
            let event = new CustomEvent('jvchat:newmessage', { 'detail': { id: newMessageId } });
            dispatchEvent(event);
        }
    } else {
        if (!turboActivated && !refreshDegraded) {
...
function checkDeleted(id) {
    ...
    function onError(_, status) {
        if (status === 410 && lastEditionTime[id] !== false) {
            let [timestamp, _] = lastEditionTime[id];
