    // ==UserScript==
    // @name           Transfer List CN
    // @version        1.0.2023020305
    // @author         Shomi
    // @description    You can use this script to see rating R4、R5, REC and TI on TL.
    // @match	   https://trophymanager.com/transfer/
    // @namespace https://greasyfork.org/users/721529
// @downloadURL https://update.greasyfork.org/scripts/459364/Transfer%20List%20CN.user.js
// @updateURL https://update.greasyfork.org/scripts/459364/Transfer%20List%20CN.meta.js
    // ==/UserScript==
    ...
    const resizeTable = () => {
        ...
        thead.childNodes[1].innerText = '姓名'
        thead.childNodes[1].style.textAlign = 'left'
        thead.childNodes[2].innerText = '年龄'
        thead.childNodes[6].innerText = '到期时间'
        thead.childNodes[7].innerText = '当前价格'
        if (thead.childNodes.length == '10') {
            thead.lastChild.remove();
            thead.lastChild.remove();
            addColumn(thead, 'Rating评分', '#fff');
            addColumn(thead, 'REC评星', '#fff');
        ...
        document.querySelectorAll('.transfer_list_outer')[0].style.width = '800px';
    }
    ...
    (function () {
        ...
        const addFilters = () => {
            ...
            labelPosEl.innerText = '选择Rating评分位置'
