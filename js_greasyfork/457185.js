// ==UserScript==
// @name         json-viewer-updated
// @namespace    http://tampermonkey.net/
// @version      0.4.2
// @description  在原作者 0.4 版本上加入支持每行单独为 json 的文本输出
// @description  修复 0.4.1 版本 bug
// @author       sgd/robin.cai
// @match        *://*/*
// @grant        none
// @require      https://code.jquery.com/jquery-3.4.1.min.js
// @icon         https://www.easyicon.net/api/resizeApi.php?id=501159&size=128
// @downloadURL https://update.greasyfork.org/scripts/457185/json-viewer-updated.user.js
// @updateURL https://update.greasyfork.org/scripts/457185/json-viewer-updated.meta.js
// ==/UserScript==
...
(function() {
    ...
    if(source.length > 0 && source.first().html().includes("}\n{")){
        var rawStr = source.first().html()
        var newStr = rawStr.replace(/}\n{/g, "},{")
        newStr = "[" + newStr + "]"
        if (isJSON(newStr)) {
            source.first().html(newStr)
        } else {
            source.first().html(rawStr)
        }
    }
    ...
    if(source.length > 0 && isJSON(source.first().html())){
        ...
        // 添加一个格式化显示的per元素
        jq("body").append(jq('<per id="json-renderer"></per>'))
        console.log(src);
        // 将内容用eval函数处理下
        var input = eval('(' + src + ')');
