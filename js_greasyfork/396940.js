// ==UserScript==
// @name        挊 - EMBY - OFFCLOUD
// @namespace   撸
// @description 自动获取磁链接并自动离线下载，由于原作者停止更新网页，遂发布此版本更新过期网址
...
// @include     http*://www.v22q.com/*
// @include     http*://www.d21b.com/*
// @include     http*://www.w24j.com/*
// @include      /^https?://www\.[a-z]\d\d[a-z]\.com/.*$/
...
// @include     http*://avhd101.com/watch*
...
// @downloadURL https://update.greasyfork.org/scripts/396940/%E6%8C%8A%20-%20EMBY%20-%20OFFCLOUD.user.js
// @updateURL https://update.greasyfork.org/scripts/396940/%E6%8C%8A%20-%20EMBY%20-%20OFFCLOUD.meta.js
// ==/UserScript==
...
var main = {
  ...
  jav: {
    type: 0,
    re: /(javmoo|javlog|avio|avso|avxo|av28).*movie.*/,
  ...
  avhd101: {
    type: 0,
    re: /avhd101/,
    insert_where: ".video-info",
    vid: function () {
        var txt= document.querySelector(".video-info").children[3].children[1].textContent;
        txt=txt.replace("CBM-","");
        txt=txt.replace("CBMPR-","");
  ...
  javlibrary: {
    type: 0,
    re: /(javlibrary|\d\dlib|jav\d\db|j*a|d*b|v*q|o23g|w24j|[a-z]\d\d[a-z]).*\?v=.*/,
  ...
  javdb: {
    type: 0,
    re: /javdb/,
    insert_where: ".preview-images",
    vid: function () {
      return document.querySelector(".copy-to-clipboard").getAttribute("data-clipboard-text");
    }
  },
  ...
  jav101: {
    type: 0,
    re: /play\/video/,
    insert_where: ".videoInfo",
    vid: function () {
      return document.querySelector("title").textContent.match(/[a-zA-Z0-9\-]+/)[0];
    }
  },
...
var magnet_table = {
  template: {
    ...
    create_row: function (data) {
      ...
      var list = [this.create_info(data.title, data.mag), this.create_size(data.size, data.src), this.create_operation(data.mag), this.create_offline(data.mag)];
    ...
    create_offline: function (mag) {
      var a = document.createElement("a");

      //a.href = mag;
      var regex = /magnet:\?xt=urn:btih:(\w+)/gm;

      var m = regex.exec(mag);

      if(m!== null){
          var hash= m[1];
          GM_xmlhttpRequest({
            method: "POST",
            url: "https://offcloud.com/api/cache?key=dA28P0zH5aFAl6eyyM0DEdo6MIvL3s8X",
            data: "hashes[]=" + hash,
            headers: {
            ...
            onload: function (result) {
                ...
                console.log(obj);
                if(obj.cachedItems.length>0){
                    a.textContent+="offcloud cached";
                }else{
                    a.textContent+="offcloud miss";
                }
            },
            onerror: function (e) {
                a.textContent+=" error";
          ...
          a.addEventListener("click", function (e) {
            GM_xmlhttpRequest({
                method: "POST",
                url: "https://offcloud.com/api/remote/download?key=dA28P0zH5aFAl6eyyM0DEdo6MIvL3s8X",
                data: "remoteOptionId=5dcbd4b35ca39b3e12598dbb&folderId=&url=" + encodeURIComponent(mag),
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded"
                },
                onload: function (result) {
                    var obj = JSON.parse(result.responseText);
                    console.log(obj);
                    a.textContent+=" download "+ obj.status;
                ...
                if (t.length!==0) {
                    for (let elem of t) {
                        data.push({
                            ...
                            "mag": elem.querySelector("td:nth-child(3)>a:nth-child(2)").getAttribute("href"),
                            ...
                            "src": "https://nyaa.si" + elem.querySelector("td:nth-child(2)>a:nth-child(1)").getAttribute("href"),
                        });
                    }
                }
                else {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (result) {
                ...
                if (t) {
                    ...
                    for (let elem of t) {
                        if (elem.querySelector(".action>a:nth-child(2)")==null){
                            data.push({
                                "title": "没有找到磁链接",
                                "mag": "",
                                "size": "0",
...
var display_table = function (vid, insert_where) {
  ...
  if (!document.querySelector("#nong-head")) {
    ...
    tab_with_head.appendChild(tr);
      ...
      th.appendChild(search_box);

      /*var a=document.createElement("a");
      a.setAttribute("href", "https://avhd101.com/search?q="+vid);
      a.setAttribute("target", "_blank");
      a.textContent = "AVHD101";
      tab_with_head.appendChild(a);*/

      var a=document.createElement("a");
      a.setAttribute("href", "http://www.javlibrary.com/cn/vl_searchbyid.php?keyword="+vid);
      a.setAttribute("target", "_blank");
      a.textContent = "Javlibrary";
      th.appendChild(a);

      a=document.createElement("a");
      a.setAttribute("href", "https://javdb.com/search?f=all&q="+vid);
      a.setAttribute("target", "_blank");
      a.textContent = "JavDB";
      th.appendChild(a);
      ...
      GM_xmlhttpRequest({
            method: "GET",
            url: "http://loning.bonus.vip:8096/emby/Search/Hints?api_key=16c6b4c9e9be458790aca61bee89bd65&SearchTerm=" + vid,
            onload: function (result) {
                var obj = JSON.parse(result.responseText);
                console.log(obj);
                if(obj.SearchHints.length>0){
                    a.textContent ="!!!Already on emby!!!";
                }else{
                    a.textContent ="Not on emby";

                }
            },
            onerror: function (e) {
                a.textContent ="error on emby server";
    ...
    console.log("tab",tab_with_head,"where",insert_where);
