// ==UserScript==
// @name          Pixiv Old Manga Viewer (renew)
...
// @grant         GM_log
// @downloadURL https://update.greasyfork.org/scripts/392000/Pixiv%20Old%20Manga%20Viewer%20%28renew%29.user.js
// @updateURL https://update.greasyfork.org/scripts/392000/Pixiv%20Old%20Manga%20Viewer%20%28renew%29.meta.js
// ==/UserScript==

(function() {
    // Config: Always just go straight to the manga page (even in medium mode)
    var alwaysLoadManga = true;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var metas = document.getElementsByTagName('meta');
          for (let i = 0; i < metas.length; i++) {
                if (metas[i].getAttribute('name') === 'preload-data') {
                  //GM_log(metas[i].getAttribute('name')+' : '+metas[i].getAttribute('content'));
                  globalInitData = JSON.parse('{ "preload":'+metas[i].getAttribute('content')+'}');
                }
          }
        ...
        function apply(){
            ...
            function allFullURLs(original){
                ...
                function pixivOriginalURL(createDate,id,page,url){
                  //GM_log('createDate='+createDate+', id='+id+', page='+page+', url='+url);
                ...
                {
                    results.push(pixivOriginalURL(globalInitData.preload.illust[id].userIllusts[id].updateDate,id,i,urlBase));
                }
                return results;
            }
            ...
            result += `<span>`+globalInitData.preload.user[user].name+`</span></a>`;
            // Manga images:
            for (var i = 0; i < images.length; i++) {
                result += `<div style="height:2.5vh;width:100vw;" id="pomv-`+i+`"></div><div style="z-index:0;position:relative;height:95vh;width:100vw;">`;
                let href = images[i];
                result += `<img onclick="document.getElementById('pomv-`+(i+1)+`').scrollIntoView()" style="width:100%;height:100%;object-fit:scale-down;object-position:center center;" src="`+href+`"/>`;
                ...
                result += `</div><div style="width:100vw;height:2.5vh;"></div>`;
            }
            result += `<div style="width:100vw;height:75vh;" id="pomv-`+(images.length)+`"></div></div>`;
