...
// @downloadURL https://update.greasyfork.org/scripts/440674/%E5%B0%8F%E8%AF%B4%E4%B8%8B%E8%BD%BD%E5%99%A8GL.user.js
// @updateURL https://update.greasyfork.org/scripts/440674/%E5%B0%8F%E8%AF%B4%E4%B8%8B%E8%BD%BD%E5%99%A8GL.meta.js
// ==/UserScript==
...
const rules = new Map([
   ...
   ["www.xxbiquge.net", {
        bookname() { return document.querySelector('#info > h1:nth-child(1)').innerText.trim() },
        author() { return document.querySelector('#info > p:nth-child(2)').innerText.replace(/作s+者：/, '').trim() },
        intro() { return convertDomNode(document.querySelector('#intro'))[0] },
        linkList() { return includeLatestChapter('#list > dl') },
        coverUrl() { return document.querySelector('#fmimg > img').src },
        chapterName: function(doc) { return doc.querySelector('.bookname> h1:nth-child(1)').innerText.trim() },
    ...
    ["www.qu-la.com", {
        ...
        author() { return document.querySelector('.book-text > span:nth-child(1)').innerText.replace(/\s+著/, '').trim() },
        intro() { return convertDomNode(document.querySelector('.intro'))[0] },
        linkList() { return includeLatestChapter2('.book-chapter-list > ul') },
        coverUrl() { return document.querySelector('#fmimg > img').src },
...
function includeLatestChapter2(selector) {
    ...
    let rli = dl.querySelector('li:nth-child(1)')
    if (rli.innerText.includes('最新章节')) {
        let p = null;
        let n = rli;
        while (true) {
            if (n.nodeName == 'li') {
                ...
                n = n.nextSibling;
                p.classList.add('not_download')
            } else if (n.nodeName == 'ul' && !n.innerText.includes('最新章节')) {
                break;
            } else {
                p = n;
                n = n.nextSibling;
    ...
    else if(rli.innerText.includes('最新')){
        let p = null;
        let n = rli;
        while (true) {
            if (n.nodeName == 'li') {
                ...
                n = n.nextSibling;
                p.classList.add('not_download')
            } else if (n.nodeName == 'ul' && !n.innerText.includes('最新')) {
                break;
            } else {
                p = n;
                n = n.nextSibling;
    ...
    return dl.querySelectorAll('li:not(.not_download) > a')
}
