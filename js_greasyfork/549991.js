...
(function() {
    ...
    // Use GM_log and console.error to ensure visibility in all environments
    const logDebug = (message, ...args) => {
        if (DEBUG_MODE) {
            const msg = `[WarScript DEBUG]: ${message}`;
            console.error(msg, ...args);
            GM_log(msg + " " + JSON.stringify(args));
        }
    };

    console.log("WarScript (PDA Fork): Script starting (v3.8.8-pda)...");
    ...
    async function fetchFromServer(endpoint, body = {}, extraHeaders = {}, method = "POST") {
        return new Promise(resolve => {
            const requestOptions = {
                ...
                timeout: 15000,
                // REMOVED anonymous: true, to ensure custom cookies are sent if permitted
                onload: response => {
                    if (!response || typeof response.responseText === 'undefined') {
                        logDebug("fetchFromServer: onload callback received an invalid response object.");
                        resolve(null);
                        return;
                    }
                    try {
                        ...
                        if (data.error) {
                            logDebug(`Server Error for ${endpoint}:`, data.error);
                            resolve(null);
                        } else {
                            resolve(data);
                        }
                    } catch (e) {
                        logDebug("Failed to parse server response.", e);
                        resolve(null);
                    }
                },
                onerror: error => {
                    logDebug("GM_xmlhttpRequest error:", error);
                    resolve(null);
                },
                ontimeout: () => {
                    logDebug(`GM_xmlhttpRequest timeout for endpoint: ${endpoint}`);
                    resolve(null);
                }
            };

            // PDA Fix: Explicitly set the 'cookie' property AND header if a 'Cookie' header exists
            if (extraHeaders && extraHeaders['Cookie']) {
                requestOptions.cookie = extraHeaders['Cookie'];
                // Ensure header key is capitalized 'Cookie' for consistency
                requestOptions.headers['Cookie'] = extraHeaders['Cookie'];
            }
            ...
            if (DEBUG_MODE) {
                logDebug(`Fetching ${endpoint} [${method}]`);
                logDebug(`Request Headers:`, requestOptions.headers);
                if (requestOptions.cookie) logDebug(`Request Cookie Prop:`, requestOptions.cookie);
            }
    ...
    async function saveSettings() {
        ...
        logDebug("Settings saved:", sortedSettings);
    ...
    function updateArrowStates() {
        ...
        rows.forEach((row, index) => {
            ...
            upArrow.classList.toggle('disabled', index === 0);
            downArrow.classList.toggle('disabled', index === rows.length - 1);
        });
    }
    ...
    function createNewRow(data = {}, initialStats = '-') {
        ...
        upArrow.addEventListener('click', function() {
            ...
            const currentRow = this.closest('.settings-row');
            const prevRow = currentRow.previousElementSibling;
            if (prevRow && prevRow.classList.contains('settings-row')) {
    ...
    function loadSettings() {
        ...
        try {
            ...
            logDebug("Failed to parse settings, starting with empty list.", e);
        }
    ...
    async function forcePollAndUpdate() {
        ...
        try {
            ...
            logDebug("Could not parse stored live data, starting fresh.", e);
            liveData = {};
        }
        ...
        if (factionId) {
            // NEW ENDPOINT: warmonitor-data/{factionId} with apiKey in URL for GET
    ...
    function startAllPolling() {
            ...
            try {
                ...
                if (data && !data.error) {
                    ...
                    try {
                        ...
                        logDebug("Faction poll successful. Merged data.");
                    } catch (e) {
                        logDebug("Error processing faction data.", e);
                    }
                } else {
                    logDebug("Faction poll ran, but no valid data returned or error.", data);
                }
            } catch (e) {
                logDebug("CRITICAL ERROR in factionPoll loop.", e);
            }
        };
        ...
        try {
            ...
            logDebug("Could not parse settings for individual polling.", e);
            monitoredPlayers = [];
        }
            ...
            try {
                ...
                if (data && data.name && data.last_action && data.status) {
                    try {
                        ...
                        GM_setValue(LIVE_DATA_KEY, JSON.stringify(mergedData));
                        renderOrUpdateButtons(mergedData);
                        logDebug(`Individual poll successful for ${player.profileId}. Merged data.`);
                    } catch (e) {
                        logDebug("Error processing individual player data.", e);
                    }
                } else {
                ...
                playerIndex++;
                 ...
                 logDebug("CRITICAL ERROR in individualPoll loop.", e);
            }
        };
    ...
    function initializeUI() {
            ...
            <div id="settings-modal-content">
                ...
                <div id="main-settings-view">
                    <div id="settings-modal-header">
                        <h2>Settings</h2>
                        <div>
                            <button id="reorder-button" class="modal-header-button">Reorder</button>
                            <button id="show-api-management-button" class="modal-header-button">API Key</button>
                    ...
                    <div id="profile-list-view">
                        ...
                        <div id="settings-options-container">
                             <div id="lock-bar-container">
                                <label for="lock-bar-checkbox" style="margin-right: 5px;">Lock bar:</label>
                                <input type="checkbox" id="lock-bar-checkbox">
                            </div>
                            <div id="polling-rate-container">
                                <label for="polling-rate-input" style="margin-right: 5px;">Poll (s):</label>
                ...
                <div id="api-management-view">
                     <div id="settings-modal-header">
                        ...
                        <button id="back-to-settings-from-mgmt-button" class="modal-header-button">Back</button>
                     </div>
        ...
        function toggleReordering() {
            ...
            const controls = document.querySelectorAll('#settings-rows .reorder-controls');

            reorderButton.classList.toggle('active', isReordering);
            reorderButton.textContent = isReordering ? 'Lock' : 'Reorder';

            controls.forEach(control => {
                control.style.display = isReordering ? 'flex' : 'none';
            });
        ...
        sortableHeaders.forEach(header => {
            header.addEventListener('click', () => {
                ...
                if (isReordering) updateArrowStates();
            });
        });
        ...
        settingsButton.onclick = () => {
            ...
            if (apiKey) {
                ...
                const liveData = JSON.parse(GM_getValue(LIVE_DATA_KEY, '{}'));
                const rows = document.querySelectorAll('#settings-rows .settings-row');
                rows.forEach(row => {
                    const profileId = row.dataset.profileId;
                    if (liveData[profileId] && liveData[profileId].spy) {
                        const statsDisplay = row.querySelector('.stats-display');
                        if (statsDisplay) {
                            statsDisplay.textContent = formatStats(liveData[profileId].spy.total);
                        }
                    }
                });
            } else {
        ...
        document.getElementById('faction-fetch-button').addEventListener('click', async () => {
            ...
            // NEW ENDPOINT CALL: warmonitor-data/{factionId} with apiKey in URL for GET
            const data = await fetchFromServer(`warmonitor-data/${factionId.trim()}?apiKey=${apiKey}`, null, {}, "GET");
        ...
        async function runInitialValidation() {
            ...
            const accessData = await fetchFromServer('checkAccess', { factionId: factionData.faction_id });
            if (accessData && accessData.access === 1) {
                logDebug("WarScript (PDA): Access validated. Starting services.");
                await forcePollAndUpdate();
                startAllPolling();
            } else {
                logDebug("WarScript: Faction access denied. Polling disabled.");
    ...
    const waitForElementInterval = setInterval(() => {
        // MODIFIED: Added guard to prevent script duplication
        if (document.getElementById('script-bar-wrapper')) {
