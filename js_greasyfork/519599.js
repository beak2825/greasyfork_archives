// ==UserScript==
// @name         Exibir Youth Development em Todos os Lugares
// @namespace    http://trophymanager.com/
// @version      1.5
// @description  Exibe o Youth Development ao lado do nome dos jogadores em todos os lugares da página de táticas
// @author       Você
// @match        https://trophymanager.com/tactics/*
// @match        https://trophymanager.com/tactics/reserves
...
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/519599/Exibir%20Youth%20Development%20em%20Todos%20os%20Lugares.user.js
// @updateURL https://update.greasyfork.org/scripts/519599/Exibir%20Youth%20Development%20em%20Todos%20os%20Lugares.meta.js
// ==/UserScript==

(function() {
    'use strict';

    function exibirYouthDevelopment() {
        // Seleciona jogadores no campo, na lista lateral e no banco de reservas
        const jogadoresCampo = document.querySelectorAll('.field_player');
        const jogadoresLista = document.querySelectorAll('.player_name'); // Lista lateral
        const jogadoresBanco = document.querySelectorAll('.bench_player_name'); // Banco de reservas

        // Função para adicionar Youth Development ao lado do nome do jogador
        function adicionarYouthDev(elemento, playerId, isListaLateral = false) {
            const youthDevelopment = localStorage.getItem(`youthDevelopment_${playerId}`);

            if (youthDevelopment && !elemento.querySelector('.youth-dev')) {
                const youthDevSpan = document.createElement('span');
                youthDevSpan.className = 'youth-dev';
                youthDevSpan.style.color = 'yellow';
                youthDevSpan.style.fontSize = '12px';
                youthDevSpan.textContent = ` (${youthDevelopment})`;

                // Ajusta a posição dependendo de onde está sendo adicionado
                if (isListaLateral) {
                    youthDevSpan.style.marginLeft = '5px'; // Espaço para ficar ao lado na lista lateral e banco
                    elemento.append(youthDevSpan); // Adiciona ao lado do nome
                } else {
                    elemento.appendChild(youthDevSpan); // Adiciona ao final no campo (tática)
                }
            }
        }

        // Adiciona Youth Development nos jogadores posicionados no campo (tática)
        jogadoresCampo.forEach(jogador => {
            const playerId = jogador.getAttribute('player_id');
            const nomeJogadorElemento = jogador.querySelector('.field_player_name');
            if (playerId && nomeJogadorElemento) {
                adicionarYouthDev(nomeJogadorElemento, playerId);
            }
        });

        // Adiciona Youth Development na lista lateral de jogadores
        jogadoresLista.forEach(jogadorLista => {
            const playerId = jogadorLista.getAttribute('player_id'); // Obtém o ID do jogador da lista
            if (playerId) {
                adicionarYouthDev(jogadorLista, playerId, true); // `true` indica que é na lista lateral
            }
        });

        // Adiciona Youth Development nos jogadores do banco de reservas
        jogadoresBanco.forEach(jogadorBanco => {
            const parentElement = jogadorBanco.closest('.draggable'); // Pega o elemento pai que contém o ID
            const playerId = parentElement?.getAttribute('player_id'); // ID do jogador no banco

            if (playerId) {
                adicionarYouthDev(jogadorBanco, playerId, true); // `true` indica que é similar à lista lateral
            }
        });
    }

    // Executa a função após a página carregar completamente
    window.addEventListener('load', function() {
        setTimeout(exibirYouthDevelopment, 2000); // Aguarda 2 segundos para garantir o carregamento
    });

    // Observa mudanças no DOM para atualizar dinamicamente
    const observer = new MutationObserver(() => exibirYouthDevelopment());
...
(function() {
    ...
    // Função para verificar se o elemento de Youth Development está presente na página
    function checkYouthDevelopment() {
        const youthDevelopmentElement = document.querySelector('#tmvn_script_scout_table_body tr:nth-child(1) td:nth-child(2)');

        if (youthDevelopmentElement && youthDevelopmentElement.textContent.trim() !== '') {
            saveYouthDevelopment();
        } else {
            // Tenta novamente após um pequeno intervalo caso o elemento não esteja disponível ainda
            setTimeout(checkYouthDevelopment, 500);
        }
    }

    // Função para salvar o Youth Development no localStorage
    function saveYouthDevelopment() {
        const playerId = window.location.pathname.split('/')[2]; // Pega o ID do jogador pela URL
        const youthDevelopmentElement = document.querySelector('#tmvn_script_scout_table_body tr:nth-child(1) td:nth-child(2)');
        const youthDevelopment = youthDevelopmentElement.textContent.trim(); // Obtém o valor de Youth Development

        // Salva a informação no localStorage com o ID do jogador como chave
        localStorage.setItem(`youthDevelopment_${playerId}`, youthDevelopment);
        console.log(`Youth Development (${youthDevelopment}) do jogador ${playerId} foi salvo.`);
    }

    // Executa a verificação quando a página é carregada
    window.addEventListener('load', function() {
        setTimeout(checkYouthDevelopment, 500); // Inicia a verificação com um pequeno atraso
    });
})();
...
(function () {
    ...
    function present() {
        ...
        if (playerId != '') {
            ...
            tdYouthDevelopment.innerText = reportObj.YouthDevelopment == undefined ? '' : (reportObj.YouthDevelopment + '[' + reportObj.BornAge + ']');
            ...
            tdPotential.innerText = reportObj.Potential == undefined ? '' : (reportObj.Potential + '[' + reportObj.PotentialAge + ']');
            ...
            /*LastWeekScout*/
            let trLastWeekScout = document.createElement('tr');
            trLastWeekScout.className = 'odd';

            let tdLastWeekScoutLabel = document.createElement('td');
            tdLastWeekScoutLabel.innerText = 'Last Week Scout: ';

            let tdLastWeekScout = document.createElement('td');
            if (reportObj.LastWeekScout && reportObj.LastWeekScout > 0 && (tdBloomStatus.innerText != BLOOM_STATUS_TEXT.BLOOMED || tdDevStatus.innerText != DEVELOPMENT_STATUS_TEXT.DONE)) {
                tdLastWeekScout.innerHTML = '<span style="color:Darkred">' + reportObj.LastWeekScout + '</span>';
            } else {
                tdLastWeekScout.innerText = reportObj.LastWeekScout == undefined ? '' : reportObj.LastWeekScout;
            }

            trLastWeekScout.appendChild(tdLastWeekScoutLabel);
            trLastWeekScout.appendChild(tdLastWeekScout);
            tbody.appendChild(trLastWeekScout);
