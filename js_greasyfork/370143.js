// ==UserScript==
// @name           Virtonomica: управление закупками на склад 2
// @version        1.11
...
// @downloadURL https://update.greasyfork.org/scripts/370143/Virtonomica%3A%20%D1%83%D0%BF%D1%80%D0%B0%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5%20%D0%B7%D0%B0%D0%BA%D1%83%D0%BF%D0%BA%D0%B0%D0%BC%D0%B8%20%D0%BD%D0%B0%20%D1%81%D0%BA%D0%BB%D0%B0%D0%B4%202.user.js
// @updateURL https://update.greasyfork.org/scripts/370143/Virtonomica%3A%20%D1%83%D0%BF%D1%80%D0%B0%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5%20%D0%B7%D0%B0%D0%BA%D1%83%D0%BF%D0%BA%D0%B0%D0%BC%D0%B8%20%D0%BD%D0%B0%20%D1%81%D0%BA%D0%BB%D0%B0%D0%B4%202.meta.js
// ==/UserScript==
...
var run = function() {
  ...
  $( document ).ready(function() {
	...
	function remainToFloat(spNum, last){
        console.log(spNum);
        ...
        var svNum;
		if(nvBrPos > -1){
			if (last) svNum = spNum.substr(nvBrPos+5, spNum.indexOf("</span>")-1);
            else svNum = spNum.substr(nvSpanCloseTagPos+1, nvBrPos-nvSpanCloseTagPos-1);
	...
	function changeOrderQty(opInput, nvRemain){
        if (nvRemain) {
            if (nvRemain <= 1000) {
                if (nvRemain % 100 > 0) nvRemain = nvRemain - nvRemain % 100 + 100;
            } else if (nvRemain > 1000 && nvRemain <= 10000) {
                if (nvRemain % 500 > 0) nvRemain = nvRemain - nvRemain % 500 + 500;
            } else if (nvRemain > 10000 && nvRemain <= 200000) {
                if (nvRemain % 1000 > 0) nvRemain = nvRemain - nvRemain % 1000 + 1000;
            } else if (nvRemain > 200000 && nvRemain <= 500000) {
                if (nvRemain % 5000 > 0) nvRemain = nvRemain - nvRemain % 5000 + 5000;
            } else nvRemain = nvRemain - nvRemain % 10000 + 10000;
        }
		opInput.val(nvRemain);
	}
	...
	collectData();
	sortTable();
    // Extension by Alexander Galitsky //
    var maxAvailableBtn, allCheckBtn, allZeroCheckBtn;
    $("table.list > tbody > tr[class]").each( function() {
        var row = $(this);
        if (row.attr('class') == 'p_title') { // Отгрузки по контрактам
            //var nvQtyNeed = toFloat($('> td.p_title_l > div:nth-child(2) > table > tbody > tr:nth-child(3) > td:nth-child(2) > strong', row).first().text());
            //var nvQtyOrdered = toFloat($('> td:nth-child(2) > nobr > strong', row).first().text());
            //var ovAddContras = $('> td.p_title_l > div:nth-child(1) > div:nth-child(2) > a:nth-child(2)', row);
            //svProductTitle = ovProductImg.attr('title');
            //addProduct(svProductTitle, nvQtyNeed, nvQtyOrdered, ovAddContras);
            maxAvailableBtn = $('<button>Max</button>');
            row.children('.p_title_l').append(maxAvailableBtn);
            allCheckBtn = $('<button>All</button>');
            row.children('.p_title_l').append(allCheckBtn);
            allZeroCheckBtn = $('<button>Zero</button>');
            row.children('.p_title_l').append(allZeroCheckBtn);
        } else if (row.attr('class') == 'odd' || row.attr('class') == 'even') {
            var ovInputOrderQty = $('> td:nth-child(2) > input[type="text"]:nth-child(1)', row);
            ovInputOrderQty.attr('old_val', ovInputOrderQty.val());
            var nvOrderedQty = toFloat($('> td:nth-child(3)', row).first().text());
            var nvPrice = priceToFloat($('> td:nth-child(4)', row).first().text());
            var nvQuality = toFloat($('> td:nth-child(6)', row).first().text());
            var remain = $('> td:nth-child(9)', row).first().html();
            var nvRemain = remainToFloat(remain);
            var nvRemianLast = remainToFloat(remain, true);
            if ($(' > td:nth-child(1) > div:nth-child(3) > img[src="/img/unit_types/seaport.gif"]', row).length > 0) {
                nvRemain = Number.POSITIVE_INFINITY;
            }
            if (maxAvailableBtn) {
                maxAvailableBtn.click(function() {
                    var nvRemain2 = nvRemain;
                    if (nvRemain <= 1000) {
                        if (nvRemain % 100 > 0) nvRemain2 = nvRemain - nvRemain % 100 + 100;
                    } else if (nvRemain > 1000 && nvRemain <= 10000) {
                        if (nvRemain % 500 > 0) nvRemain2 = nvRemain - nvRemain % 500 + 500;
                    } else if (nvRemain > 10000 && nvRemain <= 200000) {
                        if (nvRemain % 1000 > 0) nvRemain2 = nvRemain - nvRemain % 1000 + 1000;
                    } else if (nvRemain > 200000 && nvRemain <= 500000) {
                        if (nvRemain % 5000 > 0) nvRemain2 = nvRemain - nvRemain % 5000 + 5000;
                    } else nvRemain2 = nvRemain - nvRemain % 10000 + 10000;
                    ovInputOrderQty.val(nvRemain2);
                    return false;
                });
                ...
                allZeroCheckBtn.click(function () {
                    row.find('input[type="checkbox"]').each(function () {
                        if (!nvRemianLast) {
                            console.log(nvRemain);
                            this.checked = true;
                        }
                    });
                    return false;
