...
// @grant         none
// @license       MIT
// @namespace https://greasyfork.org/users/98974
// @downloadURL https://update.greasyfork.org/scripts/415488/Discord%20cleaner.user.js
// @updateURL https://update.greasyfork.org/scripts/415488/Discord%20cleaner.meta.js
// ==/UserScript==
...
async function deleteMessages(authToken, authorId, guildId, channelId, minId, maxId, content,hasLink, hasFile, includeNsfw, includePinned, extLogger, stopHndl, onProgress) {
    ...
    async function recurse() {
        let API_SEARCH_URL;
        if (guildId === '@me') {
            API_SEARCH_URL = `https://discord.com/api/v8/channels/${channelId}/messages/`; // DMs
        }
        else {
            API_SEARCH_URL = `https://discord.com/api/v8/guilds/${guildId}/messages/`; // Server
        }
        ...
        if (messagesToDelete.length > 0) {
            ...
            for (let i = 0; i < messagesToDelete.length; i++) {
                ...
                try {
                    const s = Date.now();
                    const API_DELETE_URL = `https://discord.com/api/v8/channels/${message.channel_id}/messages/${message.id}`;
...
function initUI() {
    ...
    <div id="undiscord" style="display:none;">
        <div class="header">
            Discord message cleaner © 2020 louga31
        </div>
        <div class="form">
