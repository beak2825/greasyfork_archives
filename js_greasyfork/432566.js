...
// @version      0.1
// @description  SUpport for deposits
// @author       Dimas
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/432566/Deposit%20Support.user.js
// @updateURL https://update.greasyfork.org/scripts/432566/Deposit%20Support.meta.js
// ==/UserScript==
...
const request = "https://managment.io/admin/changesubagent/editsubagenttoorder"+global_order+"\",\"subagent_id\":"+global_subagent+",\"bank_id\":"+global_bank+",\"ref_id\":"+global_ref_id

              if(prev_request == request) {
  
              console.log("%c 🏳️‍🌈: REQUEST CANCELLED ",
               "font-size:16px;background-color:#a7c8c6;color:black;", 
               request)
...
const request = "https://managment.io/admin/report/recountingwebreplenishment"+'{"status":10,"summa":' +
amount +
...
.btn{
  ...
  #snackbar.show {
    ...
    -webkit-animation: fadein 0.5s, fadeout 0.5s 59.5s;
...
if(tocheck.startsWith('3')||tocheck.startsWith('2')||tocheck.startsWith('1')){    
  if(tocheck.startsWith('3')) 
  {
    set_fields({trans:tocheck})
  }

  if(tocheck.startsWith('2')|| tocheck.startsWith('1')) 
  {
...
// let search_button ;
  function clip_changed() {
    ...
    if(search.startsWith('3') || search.startsWith('2')|| search.startsWith('1')&&search.length<=15) {
...
if(search.startsWith('2')||search.startsWith('1')){
...
function select_li() {
      ...
      async function getbanks() {
        ...
        /////
        const request = "https://managment.io/admin/changesubagent/getbanksbysubagentid"+"{\"id\":"+element.id+"}"
      ...
      approve_button.addEventListener(
        ...
        function (
          ...
          async function getsubs() {
            // Default options are marked with *
            const request = "https://managment.io/admin/changesubagent/getsubagnetbytransactionid"+"{\"id\":\""+id+"\",\"ref_id\":"+ref+"}"
        ...
        approve_button.addEventListener(
          ...
          function (
            ...
            async function postData(order, refID, stat, amount) {
              ...
              const request = "https://managment.io/admin/report/recountingwebreplenishment"+'{"status":' +
              stat +
              ',"summa":' +
              amount +
              ',"new_number":null,"order_ids":"' +
              ...
              refID +
              ',"field_id":null,"field":""}'
              if(prev_request != request) request_count=0;
              if(prev_request == request && request_count > 1) {
