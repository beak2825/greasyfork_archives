// ==UserScript==
// @name        aio-fs-functions
// @namespace   aio-fs-functions 1
...
// @downloadURL https://update.greasyfork.org/scripts/448512/aio-fs-functions.user.js
// @updateURL https://update.greasyfork.org/scripts/448512/aio-fs-functions.meta.js
// ==/UserScript==
 
// var NEXT_TIME_TO_WAIT = 250;
// var TIME_TO_ADD = 400;

var STORE_INFO_URL = 'https://mmonj.pythonanywhere.com/stores_data.json';
...
function complete_survey() {
  let product_nodes = get_product_nodes();
  
  let store_label = get_store_detail()
  let store_data = {};
  
  for (let [key, value] of Object.entries(STORES_DATA)) {
    if ( store_label.startsWith(key) ) {
      store_data = value;
      break;
    }
  }
  
  let client_name = get_client_name().toUpperCase();
  
  var barcode_img_urls = [];
  let total_ordered = 0;
  let total_instock = 0;
  let total_tags_placed = 0;
  for (let node of product_nodes) {
    let upc = get_upc_from_question_node(node);
    if (upc == null) {
      continue;
    }
    
    upc = get_full_upc(upc, client_name);
    ret = process_product_question(node, store_data, upc);
    
    if (ret['ordered']) {
      total_ordered += 1;
    }
    if (!ret['instock']) {
      let img_url = get_upc_image_url(node);
      if (img_url != null) {
        barcode_img_urls.push(img_url);
      }
    }
    else {
      total_instock += 1;
  ...
  alert(`${total_instock} in-stock of ${product_nodes.length} items, Ordered: ${total_ordered}`);
  // console.log(`${total_instock} in-stock of ${product_nodes.length} items, Ordered: ${total_ordered}`);

  window.scrollTo(0,document.body.scrollHeight);
  
  // --------- Fill preset questions -------------------
  FILL_INFO['Decision Maker Name*'] = get_manager_names();
  let all_question_nodes = document.querySelectorAll('div[class="fsSection fs1Col"] div[class="fsRow fsFieldRow fsLastRow"]');
  
  for (let node of all_question_nodes) {
    fill_from_presets(node);
  }
}

function process_product_question(node, store_data, upc) {
  ret = {
    'ordered': false, 
    'instock': false,     
  };
  
  let chance_for_oos = 10;
  // 2nd param in is_within_target is the perc chance for the function to be true
  if ( upc in store_data && is_within_target(rand_percent(), chance_for_oos) ) {
    check_radio(node, 'What is the Status upon arrival?*', 'Not On Shelf | Tag Presesnt');
    check_radio(node, 'What Action did you take on Status "Not on Shelf | Tag Present"?*', 'Placed Order');
    ret['ordered'] = true;
  }
  else if (upc in store_data) {
    check_radio(node, 'What is the Status upon arrival?*', 'On Shelf | Tag Present');
    check_radio(node, 'What Action did you take on Status "On Shelf | Tag Present"?*', 'Added Stock');
    ret['instock'] = true;
  }
  else {
    check_radio(node, 'What is the Status upon arrival?*', 'Not On Shelf | No tag');
    check_radio(node, 'What Action did you take on Status "Not on Shelf | No Tag"?*', 'Decision Maker Refused');
  }
  
  return ret;
}

function fill_start_info(user_email) {
  // populate user email
  document.querySelector("input[type='email']").value = user_email;

  check_radio(document, 'Did you Verify the Store Information ABOVE is Correct?*', 'I Affirm');

  // select 'Yes' to store is operational
  document.querySelector("button[class='fsImageOptionButton']").click();
}

function __main(user_email) {
  console.log('  Filling prelim questions');
  fill_start_info(user_email);
    
  _data_promise = get_json();
  _data_promise.then(
    _data => {
        STORES_DATA = _data;
        console.log('  Filling survey questions');f
        complete_survey();
    }
  )
}
...
async function get_json() {
  const resp = await fetch(STORE_INFO_URL);
