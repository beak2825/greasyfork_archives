// ==UserScript==
// @name         123FastLink (最新侧边菜单栏固定按钮适配新UI)
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  基于@Bao-qing (2025-01-29)，@微風子 (1.3.1)，@Cursor (1.0.6) 版本的分支。最新侧边菜单栏固定按钮适配新UI
...
// @grant        GM_getValue
// @grant        GM_setValue
// @downloadURL https://update.greasyfork.org/scripts/542552/123FastLink%20%28%E6%9C%80%E6%96%B0%E4%BE%A7%E8%BE%B9%E8%8F%9C%E5%8D%95%E6%A0%8F%E5%9B%BA%E5%AE%9A%E6%8C%89%E9%92%AE%E9%80%82%E9%85%8D%E6%96%B0UI%29.user.js
// @updateURL https://update.greasyfork.org/scripts/542552/123FastLink%20%28%E6%9C%80%E6%96%B0%E4%BE%A7%E8%BE%B9%E8%8F%9C%E5%8D%95%E6%A0%8F%E5%9B%BA%E5%AE%9A%E6%8C%89%E9%92%AE%E9%80%82%E9%85%8D%E6%96%B0UI%29.meta.js
// ==/UserScript==
        ...
        const checkInterval = setInterval(() => {
            ...
            if (sidebarMenu && !document.getElementById('fastlink-tool-li')) {
                ...
                fastLinkItem.onclick = (e) => {
                    e.preventDefault();
                    // 先移除已存在的自定义菜单
                    document.getElementById('fastlink-sidebar-popup-menu')?.remove();
                    // 创建小巧悬浮菜单
                    ...
                    menu.innerHTML = `
                        <div style="padding:2px 0;">
                            <div class="fastlink-menu-item" id="fl-menu-generateShare">📄 生成秒传链接</div>
                            <div class="fastlink-menu-item" id="fl-menu-generateFromPublicShare">🌐 从分享链接生成</div>
                            <div class="fastlink-menu-item" id="fl-menu-generateFromPublicShareBatch">🌐 批量解析分享链接</div>
                            <div class="fastlink-menu-item" id="fl-menu-receiveDirect">📥 链接/文件转存</div>
                            <div class="fastlink-menu-item" id="fl-menu-splitJsonFile">✂️ 拆分JSON文件</div>
                            <div class="fastlink-menu-item" id="fl-menu-convert123Share">🔄 转换.123share</div>
                            <div class="fastlink-menu-item" id="fl-menu-filterSettings">🔍 元数据过滤设置</div>
                            <div class="fastlink-menu-item" id="fl-menu-settings">⚙️ 设置</div>
                        </div>
                        <style>
                            .fastlink-menu-item{padding:7px 18px;cursor:pointer;transition:background 0.18s;border-radius:5px;margin:0 4px;}
                            .fastlink-menu-item:hover{background:#e6f7ff;}
                        </style>
                    `;
                    document.body.appendChild(menu);
                    // 点击外部关闭菜单
                    const closeMenu = () => { menu.remove(); document.removeEventListener('mousedown', onDocClick, true); };
                    function onDocClick(ev) {
                        if (!menu.contains(ev.target) && ev.target !== fastLinkItem) closeMenu();
                    }
                    setTimeout(()=>document.addEventListener('mousedown', onDocClick, true), 10);
                    // 菜单项事件
                    ...
                    document.getElementById('fl-menu-generateFromPublicShare')?.addEventListener('click', () => { closeMenu(); uiManager.showModal("🌐 从分享链接中生成链接", "", 'inputPublicShare'); });
                    document.getElementById('fl-menu-generateFromPublicShareBatch')?.addEventListener('click', () => { closeMenu(); uiManager.showModal("🌐 批量生成", "", 'inputPublicShareBatch'); });
                    document.getElementById('fl-menu-receiveDirect')?.addEventListener('click', () => { closeMenu(); uiManager.showModal("📥 文件转存/粘贴链接", "", 'inputLink'); });
                    document.getElementById('fl-menu-splitJsonFile')?.addEventListener('click', () => { closeMenu(); uiManager.showModal("✂️ 拆分工具", "", 'splitJsonTool'); });
                    document.getElementById('fl-menu-convert123Share')?.addEventListener('click', () => {
                        closeMenu();
                        const input = document.createElement('input');
                        input.type = 'file';
                        input.accept = '.123share,.json';
                    ...
                    document.getElementById('fl-menu-filterSettings')?.addEventListener('click', () => { closeMenu(); uiManager.showModal("🔍 元数据过滤设置", "", 'filterSettings'); });
                    document.getElementById('fl-menu-settings')?.addEventListener('click', () => { closeMenu(); uiManager.showSettingsDialog(); });
                };
