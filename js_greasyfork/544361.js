// ==UserScript==
// @name         Enhanced Torn Poker HUD - Tribute
...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/544361/Enhanced%20Torn%20Poker%20HUD%20-%20Tribute.user.js
// @updateURL https://update.greasyfork.org/scripts/544361/Enhanced%20Torn%20Poker%20HUD%20-%20Tribute.meta.js
// ==/UserScript==
...
(function() {
    ...
    function isPlayerAction(text) {
        const actionWords = ['posted', 'folded', 'called', 'raised', 'checked', 'bet'];
        return actionWords.some(word => text.includes(word)) && !text.includes('Game ') && !text.includes('The ');
    }
    ...
    function parseAction(text) {
        ...
        if (text.includes('posted small blind')) {
            const match = text.match(/(.+?)\s+posted small blind \$([0-9,]+)/);
            if (match) {
                playerName = match[1].trim();
                action = 'small_blind';
                amount = match[2].replace(/,/g, '');
            }
        } else if (text.includes('posted big blind')) {
            const match = text.match(/(.+?)\s+posted big blind \$([0-9,]+)/);
            if (match) {
                playerName = match[1].trim();
                action = 'big_blind';
                amount = match[2].replace(/,/g, '');
            }
        } else if (text.includes('folded')) {
            const match = text.match(/(.+?)\s+folded/);
            if (match) {
                playerName = match[1].trim();
                action = 'fold';
            }
        } else if (text.includes('called')) {
            const match = text.match(/(.+?)\s+called \$([0-9,]+)/);
            if (match) {
                playerName = match[1].trim();
                action = 'call';
                amount = match[2].replace(/,/g, '');
            }
        } else if (text.includes('raised')) {
            const match = text.match(/(.+?)\s+raised \$([0-9,]+) to \$([0-9,]+)/);
            if (match) {
                playerName = match[1].trim();
                action = 'raise';
