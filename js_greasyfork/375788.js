// ==UserScript==
// @name         KissCartoon Downloader
// @namespace    https://greasyfork.org/en/users/135934-Cartoon-bro1
// @version      3.3.1
// @description  This is a userscript that will download multi episodes form KissCartoon.
// @author       CartoonBro1
// @homepage     https://github.com/Eltion/KissCartoon-Downloader
// @match        http://kimcartoon.to/Cartoon/*
// @include      http://kimcartoon.to/Cartoon/*
// @exclude      http://kimcartoon.to/Cartoon/*/*
// @grant        GM_getValue
// @grant        GM_listValues
// @run-at       document-end
...
// @require      https://cdn.rawgit.com/Eltion/KissCartoon-Downloader/ee154d713ce5af9c031b4abdd20fae8bb7cc2dc5/css.js
// @require      https://cdn.rawgit.com/Eltion/KissCartoon-Downloader/4fc64d92baba62fb52de03a3472464c2b6466ed9/vr2.js
// @require      https://cdn.rawgit.com/Stuk/jszip/579beb1d45c8d586d8be4411d5b2e48dea018c06/dist/jszip.min.js
// @require      https://cdn.rawgit.com/Eltion/KissCartoon-Downloader/b24ffcadd00a4f3eda526e213f4d4c8d5196af6c/FlieSaver.js

// @downloadURL https://update.greasyfork.org/scripts/375788/KissCartoon%20Downloader.user.js
// @updateURL https://update.greasyfork.org/scripts/375788/KissCartoon%20Downloader.meta.js
// ==/UserScript==
...
(function() {
    ...
    //curretEP = "http://kimcartoon.to/Cartoon/Sonic-Mania-Adventures/Episode-4?id=84979";
    //getEp(curretEP);
     //getE();
    ...
    $("#startscript").on('click',function(){
        ...
        nameOfCartoon = location.href.split("/")[4].replace(/-/g," ");
        //alert(nameOfCartoon);
...
function setUI(){
    ...
    var imgSrc = "https://cdn.rawgit.com/Eltion/KissCartoon-Downloader/024c2d98b5580a14d1eaf74276d641c88f04764a/Download%20ButtonGreen.png";
    var html = '<div id="adownloader" style="position:fixed; bottom:10px; left:10px; z-index: 99999999;"><img id="startscript" style="cursor:pointer;float:left;position: relative; top:5px;margin-right:10px;" width="90px" src="'+imgSrc+'"><div style="background: #14dd3edb; position: relative;height:90px;padding:10px;border-radius: 10px;/* text-align:center; */color: white;float: left;"><div style="display: inline-block;float: left;margin-top: 13px;margin-left: 10px;">Start: <input value="1" id="astart" style="color:white;text-align:center;width: 30px;background: transparent;border: 0.5px solid white;border-radius:5px;" type="text"><input id="aend" value="7" style="color:white;text-align:center;width: 30px;background: transparent;border: 0.5px solid white;border-radius:5px;float: right;" type="text"><span style=" float: right; margin-right: 5px; ">End:</span><br><br>Quality: <input id="aquality" value="1080,720,480,360" style="color:white;text-align:center;width: 120px;background: transparent;border: 0.5px solid white;border-radius:5px;" type="text"></div><div style="display:inline-block;float:left;margin: 0px 10px;vertical-align: middle;"><input id="atxt" type="checkbox" style=" vertical-align: middle; ">Text<br><input id="ahtml" type="checkbox" style=" vertical-align: middle; ">Html<br><input id="am3u8" type="checkbox" style=" vertical-align: middle; ">M3u8<br><input id="idm" type="checkbox" style=" vertical-align: middle; ">IDM</div><div style="display:inline-block;float:left;margin: 0px 10px;vertical-align: middle;"><input id="abeta" type="checkbox" style=" vertical-align: middle; ">Beta<br><input id="arapid" type="checkbox" style=" vertical-align: middle; ">RapidVideo <br /><div style="text-align: center; font-size: 14pt;margin-top:10px;"><a href="https://github.com/Eltion/KissCartoon-Downloader" target="_blank" >Help?</a></div></div></div></div>';
    var html2 = "<div id='ainfo' style=' padding:10px;z-index: 99999999; border-radius:20px;position:fixed; display: none; bottom:10px; right:10px; background:#14dd3edb;height:100px;width:400px;'><h3 style='text-align: center'>KissCartoon Downloader</h3><p style='width:100%; word-wrap: break-word;' id='aoutput'></p><p id='aprogress'></p></div>";
...
function MobileFallBack(){
    ...
    var x = $.ajax({type: "POST", url:"https://kissCartoon.ru/Mobile/GetEpisode", data:{eID:id},async: false}).responseText;
    x = x.split("|||")[0];
...
function postdata(answer){
    ...
    var msg = $.ajax({type: "POST", url: "https://kissCartoon.ru/Special/AreYouHuman2",data: data ,async: false}).responseText;
        if(server == 1){
            beta(msg);
        }else if(server == 2){
            rapidvideo(msg);
...
function createTxtList(){
    ...
    $("#CaptchaInfo").show();
    $("#CaptchaInfo").find("p").html("You need to download <a href='https://cdn.rawgit.com/Eltion/KissCartoon-Downloader/040e60bfcfc57c1b27e3ca7faf65204abf435056/KissCartoon%20Downloader.zip'>KissCartoon Downloader.zip</a><br /><br /> <a href='https://cdn.rawgit.com/Eltion/KissCartoon-Downloader/5f62b6848a62d208ee799d6a8b256741fd7b9229/README.md'>Read this.</a>");
...
function createIDM(){
    ...
    for(var i = 0; i < EpisodesVideoLinks.length; i++){
        //list += encodeURI(EpisodesVideoLinks[i]) + "[" +  EpisodesName[i].replace(/[\s:\|\[\]\{\}]+/g,"_") + ".mp4\n";
        list += 'IDMan.exe  /n /p "%UserProfile%\\Downloads\\Cartoon\\'+ nameOfCartoon+ '" /a /f "' + EpisodesName[i].replace(/[\s:\|\[\]\{\}]+/g,"_")+'.mp4" /d "'+encodeURI(EpisodesVideoLinks[i])+'"\n';
    }
    list += "IDMan.exe /s \n";
    list += "IDMan.exe";
    var zip = new JSZip();
    ...
    zip.generateAsync({type:"blob"}).then(function(content) {
        saveAs(content, nameOfCartoon+".zip");
    });
    $("#CaptchaInfo").show();
...
function isBasicJson(){
    return GM_getValue("CartoonBro3",false);
}
...
function getBasicJson(){
    ...
    var msg='';
    //msg = $.ajax({type: "GET", url: "https://cdn.rawgit.com/Eltion/KissCartoon-Chaptcha-Auto-Complete/111255eebd4ee25aaa2ad6d072b75ae446217d97/KissCartoon.Downloader.Chaptcha.Database.json", async: false}).responseText;
    GM_xmlhttpRequest({
        method: "GET",
        url: "https://cdn.rawgit.com/Eltion/KissCartoon-Chaptcha-Auto-Complete/eee0c4153d22cec5a5327efc8d7d8ba3b37394d9/KissCartoon.Downloader.Chaptcha.Database2.json",
        synchronous: true,
        onload: function(response) {
...
function toDataURL(url, callback) {
  ...
  xhr.onload = function() {
    ...
    //alert("https://kissCartoon.ru/Special/"+url);
  xhr.open('GET', "https://kissCartoon.ru/Special/"+url);
