...
// @downloadURL https://update.greasyfork.org/scripts/488036/Lolzteam%20Multiaccount%20Finder.user.js
// @updateURL https://update.greasyfork.org/scripts/488036/Lolzteam%20Multiaccount%20Finder.meta.js
// ==/UserScript==
                ...
                if (await checkKeyValue("CircularNavigation") === 'true') {
                    ...
                    window.location.href = `https://${domain}/online/?type=registered`;}} else {
                        ...
                        const checkNextMember = async () => {
                            ...
                            if (usernameStyle.classList.contains('banned') || usernameStyle.classList.contains('style18')) {
...
async function CreateComplaint(title, message_html, retries = 3, delay = 60000) {
    try {
        let xfTokenValue = (document.querySelector('input[name="_xfToken"]') || {}).value || 'Элемент не найден';
...
async function checkUser(link, source, gifId, style = null) {
    link = cleanURL(domain, link); logWithPrefix(`${link}`);
