...
function showDbInfo() {
    ...
    const hywztk = correctAnswersDB.get() || {}; // 原 correct
    const huwctk = wrongAttemptsDB.get() || {};  // 原 wrong

    contentEl.innerHTML = `
        <h4>✔️ 正确答案库（hywztk）</h4>
        <textarea id="hywztk-editor" style="width:100%;height:200px;">${JSON.stringify(hywztk, null, 2)}</textarea>
        <div style="margin:10px 0;">
            <button id="btn-save-hywztk">💾 保存修改</button>
            <button id="btn-export-hywztk">📤 导出 hywztk.json</button>
            <button id="btn-import-hywztk">📥 导入 hywztk.json</button>
        </div>
        ...
        <div style="margin:10px 0;">
            <button id="btn-save-huwctk">💾 保存修改</button>
            <button id="btn-export-huwctk">📤 导出 huwctk.json</button>
            <button id="btn-import-huwctk">📥 导入 huwctk.json</button>
        </div>
    `;
