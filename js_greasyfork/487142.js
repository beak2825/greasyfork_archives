// ==UserScript==
// @name         Namu Hot Now debug
// @name:ko      나무위키 실검 알려주는 스크립트 디버그
...
// @downloadURL https://update.greasyfork.org/scripts/487142/Namu%20Hot%20Now%20debug.user.js
// @updateURL https://update.greasyfork.org/scripts/487142/Namu%20Hot%20Now%20debug.meta.js
// ==/UserScript==
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                url: url,
                headers: {
                    "User-Agent": "net.umanle.arca.android.playstore/0.9.80",
...
function makeArcaHotKeywordSide(){
    ...
    fetch('https://search.namu.wiki/api/ranking')
        ...
        data.slice(0, 10).forEach((searchTerm, index) => {
            ...
            const [resultLink, resultBadges] = findLinkByPartialMatch(pairs, searchTerm.trim());
