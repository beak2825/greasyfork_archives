...
// @downloadURL https://update.greasyfork.org/scripts/563903/Race%20Optimizer.user.js
// @updateURL https://update.greasyfork.org/scripts/563903/Race%20Optimizer.meta.js
// ==/UserScript==
...
(function () {
    ...
    const RACE_RESPONSE_CACHE_KEY = 'raceResponseCache';
    const CACHE_EXPIRY_MS = 14 * 24 * 60 * 60 * 1000; // 2 weeks

    function getCachedRaceResults() {
        try {
            const cached = localStorage.getItem(RACE_RESPONSE_CACHE_KEY);
            return cached ? JSON.parse(cached) : null;
        } catch (e) {
            console.error('[Race Optimizer] Failed to parse race results cache:', e);
            return null;
        }
    }

    function saveCachedRaceResults(results, userId) {
        try {
            const cacheData = { results, userId };
            localStorage.setItem(RACE_RESPONSE_CACHE_KEY, JSON.stringify(cacheData));
            console.log('[Race Optimizer] Cached race results');
        } catch (e) {
            console.error('[Race Optimizer] Failed to save race results cache:', e);
        }
    }

    function removeCachedRaceResults() {
        try {
            localStorage.removeItem(RACE_RESPONSE_CACHE_KEY);
            console.log('[Race Optimizer] Removed cached race results');
        } catch (e) {
            console.error('[Race Optimizer] Failed to remove race results cache:', e);
        }
    }
    ...
    async function fetchInitialRaceData(raceID) {
        ...
        const cachedData = getCachedRaceResults();
        if (cachedData && cachedData.results) {
            console.log('[Race Optimizer] Using cached race results');
            current_user = cachedData.userId;
            displayRaceResults(cachedData.results);
            return;
        }
    }
    ...
    function parseRaceData(data) {
        ...
        try {
            ...
            for (const playername in carsData) {
                ...
                const binaryData = carsData[playername];
                if (!binaryData) continue;

                const intervals = atob(binaryData).split(',');
                let raceTime = 0;
            ...
            console.error('[Race Optimizer] Failed to parse race data:', e);
            return null;
        }
    }
    ...
    function generateTableHtml(title, headers, rowsHtml) {
        ...
        return `
            ...
            <div class="control-buttons">
                ...
                <button id="btn-clear-race-cache">Clear Race Cache</button>
    ...
    async function displayParticipants(participants) {
        ...
        for (let i = 0; i < participants.length; i++) {
            ...
            const isCurrentUser = current_user && racer.userId == current_user;
        ...
        resultsContainer.innerHTML = generateTableHtml('Participants', ['#', 'Racer', 'Racing Skill'], rowsHtml);
    ...
    async function displayRaceResults(results) {
        ...
        resultsContainer.innerHTML = generateTableHtml('Race Results', ['Pos', 'Racer', 'Time', 'Racing Skill'], rowsHtml);
    ...
    function setupButtonListeners(results) {
        ...
        // Clear Race Cache button
        const clearRaceCacheBtn = document.querySelector('#btn-clear-race-cache');
        if (clearRaceCacheBtn) {
            clearRaceCacheBtn.addEventListener('click', () => {
                removeCachedRaceResults();
                alert('Race results cache cleared. Reload the page to see changes.');
            });
        }
    ...
    async function fetchRacingSkillsForRacers(racers, forceRefresh = false) {
        ...
        for (const racer of racers) {
            const cachedEntry = cache[racer.userId];
            const isCached = cachedEntry && (Date.now() - cachedEntry.timestamp <= CACHE_EXPIRY_MS);
            ...
            try {
                ...
                if (data && data.timeData) {
                    if (data.timeData.status >= 3) {
                        // Race finished - cache parsed results and show
                        const results = parseRaceData(data);
                        if (results && results.length > 0) {
                            saveCachedRaceResults(results, current_user);
                            displayRaceResults(results);
                        }
                    } else {
                        // Race not finished - remove cached results and show participants
                        removeCachedRaceResults();
