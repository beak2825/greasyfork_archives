...
// @license      MIT
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/475142/Bunkrr%20video%20show%20thumbnail.user.js
// @updateURL https://update.greasyfork.org/scripts/475142/Bunkrr%20video%20show%20thumbnail.meta.js
// ==/UserScript==
...
function insertThumbnail() {
  ...
  const domain = 'bunkrr';
  const domainExt = (new URL(window.location)).hostname;
  ...
  let thumbnail1 = thumbnailBase.replace(`//${domainExt}/v/`, `//i-${prefix}.${thumbDomain}.ru/thumbs/`);
  let thumbnail2 = thumbnailBase.replace(`//${domainExt}/v/`, `//${prefix}img.${thumbDomain}.ru/thumbs/`);
  ...
  let place = document.querySelector('#czmDownload');
  if (!place) {
