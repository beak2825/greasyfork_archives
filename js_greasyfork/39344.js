...
// @downloadURL https://update.greasyfork.org/scripts/39344/%E4%B9%9D%E5%A4%A9%E4%BF%AE%E6%94%B9.user.js
// @updateURL https://update.greasyfork.org/scripts/39344/%E4%B9%9D%E5%A4%A9%E4%BF%AE%E6%94%B9.meta.js
// ==/UserScript==
...
var tianjianButton = document.createElement('button');
	...
	tianjianButton.style.right = '0px';

	tianjianButton.style.top = currentPos + 'px';
		...
		if ((roominfo==undefined||tjroomclear==0)&&tianjianTrigger==1){//房间信息没有刷新，或者在战斗，或者房间内还有npc
			...
			for (var i=0;i<8;i++){
				...
				if (roominfo.get(direction[i])!=undefined){
					...
					if (roominfo.get(direction[i]).match("峽谷")!=null&&(path.length<=10||Math.random()>0.4)){//不包含峡谷两个字，为特殊房间
...
function force() {
			...
			var m=g_obj_map.get("msg_attrs");

			if (m.get("force") < setMp)
	...
	function tianjianGu(){
		...
		this.dispatchMessage=function(b){
			...
			if (type=="vs"&&subType=="vs_info"){ //這是進入戰鬥的提示

				clickButton("playskill 1",0);//放個絕學先

			}else if (type=="vs"&&subType=="combat_result"){//戰鬥結束 繼續調取擊
