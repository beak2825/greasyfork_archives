// ==UserScript==
// @name         Roster slot reservation
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  Резервування обраних машин в ростері
// @author       wandersher
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/430755/Roster%20slot%20reservation.user.js
// @updateURL https://update.greasyfork.org/scripts/430755/Roster%20slot%20reservation.meta.js
// ==/UserScript==

function unique(tank, index, list) {
    return list.findIndex(it => it.name === tank.name) === index
}

function getItem(key) {
	return new Promise((resolve) => chrome.runtime.sendMessage("mpjhblapicilagkfkldbjcilghbnmpgj", { action: 'get', key }, resolve))
}

function setItem(key, data) {
	return new Promise((resolve) => chrome.runtime.sendMessage("mpjhblapicilagkfkldbjcilghbnmpgj", { action: 'set', key, data }, resolve))
}

(async function () {

	'use strict';
	const data = await getItem('roster')
	const date = new Date().getTime();
	const slots = [];
	const closed = !!document.querySelector('.rating-container');
	const coordinators = /^.*? (.*?), (.*?)$/.exec(document.querySelector('h2').innerText);
	Array.from(document.querySelectorAll('tbody tr')).map(row => {
		let set = new RegExp('<td id="id-(.*?)"><a.*?href="(.*?)".*?>(.*?)</a>.*?</td><td><img.*?/img/(.*?).png.*?>(.*?)</td><td>.*?</td><td>.*?</td><td><b.(.*?)([0-9]{1,2})</b></td><td>.*?</td><td>.*?</td><td.*?>(.*?)<img.*?/img/(.*?).png.*?></td><td id="id-(.*?)"><a.*?href="(.*?)".*?>(.*?)</a>.*?</td>').exec(row.innerHTML)
		if (set) {
			const id_1 = set[1], id_2 = set[10], player_1 = set[3], player_2 = set[12], action_1 = set[2], action_2 = set[11];
			slots.push({ id: id_1, player: player_1, action: action_1, country: set[4], name: set[5].trim(), category: set[6], level: parseInt(set[7]), closed, date })
			slots.push({ id: id_2, player: player_2, action: action_2, country: set[9], name: set[8].trim(), category: set[6], level: parseInt(set[7]), closed, date })
			const coordinator_1 = !coordinators || coordinators.indexOf(player_1) === -1 ? 0 : coordinators.indexOf(player_1)
			if (data.find(it => it.id === id_1) == null && closed) data.push({ id: id_1, name: player_1, coordinator: coordinator_1, date })
			const coordinator_2 = !coordinators || coordinators.indexOf(player_2) === -1 ? 0 : coordinators.indexOf(player_2)
			if (data.find(it => it.id === id_2) == null && closed) data.push({ id: id_2, name: player_2, coordinator: coordinator_2, date })
		} else {
			return null
		}
	})
	await setItem('roster', data)


	let tanks = await getItem('vehicles');
	const new_slots = slots.filter(unique).filter(({ name }) => !tanks.find(it => it.name === name))
	const new_tanks = new_slots.map(({ name, country, category, level }) => ({ name, country, category, level, priority: 0 }))
	if (new_tanks.length > 0) {
		tanks = [...tanks, ...new_tanks].filter(unique).sort((a, b) => a.priority < b.priority ? 1 : -1)
		await setItem('vehicles', tanks)
	}

	if (!await getItem('auto_reserve_state')) return
	if (slots.some(({ action }) => action.includes('/rt/vehicleloss'))) return

	const vehicles = tanks.filter(it => it.priority).sort((a, b) => a.priority < b.priority ? 1 : -1);

	for (let i = 0; i < vehicles.length; i++) {
		const vehicle = vehicles[i]
		const available = slots.filter(({ name, country, action }) => name === vehicle.name && country === vehicle.country && action.includes('/rt/vehicleadd'))
		for (let j = 0; j < available.length; j++) {
			return chrome.tabs.update({ url: `https://st-roster.win${available[j].action}` });
		}
	}
