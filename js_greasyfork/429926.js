...
// @supportURL   https://keylol.com/t331397-1-1

// @downloadURL https://update.greasyfork.org/scripts/429926/%E9%A5%B0%E5%93%81%E6%AF%94%E4%BE%8B%E8%AE%A1%E7%AE%97%E8%84%9A%E6%9C%AC%28%E9%98%BF%E5%8C%BA%E6%94%B9%29.user.js
// @updateURL https://update.greasyfork.org/scripts/429926/%E9%A5%B0%E5%93%81%E6%AF%94%E4%BE%8B%E8%AE%A1%E7%AE%97%E8%84%9A%E6%9C%AC%28%E9%98%BF%E5%8C%BA%E6%94%B9%29.meta.js
// ==/UserScript==
const $ = window.jQuery;
var site;
var low;
...
function start() {
    ...
    GM_xmlhttpRequest({
        method: "get",
        url: "https://esapi.isthereanydeal.com/v01/rates/?to=ARS",
        responseType: "json",
        onload: function (r) {
            if (r && r.response) {
                ratio = r.response.data.CNY.ARS;
                console.log(ratio + "b比例为CNY/ARS");
            }
            else {
                ratio = 14.5
            }
                    ...
                    onload: function (data) {
                        ...
                        if (obj) {
                            ...
                            GM_addStyle(`.price > div{ margin-top:4px;font-size:16px; }
                                        .ls, .hb, .ls-c, .hb-c {color:#e46409; }
                                        .lsnf, .hbnf, .lsnf-c, .hbnf-c {color:#7ccc35; }
                                        .lsr, .hbr, .lsr-c, .hbr-c {color:#1ee44a; }
                                        ...
                                        .title > strong:nth-child(3) { margin-left:60px; }
                                        .title > strong:nth-child(4) { margin-left:60px; }
                                        .afkout { float:left;width:560px; }
                                        .afkout strong { color:#afb0b2;font-size: 15px}
                                       `);

                            var $pinfo = $(`<div class="afkout">
                                            <div class="title">
                                            <strong>出售</strong><strong>求购</strong><strong>出售(国)</strong><strong>求购(国)</strong>
                                            </div>
                                            <div class="price">
                                            <div><strong>价格：</strong><span class="ls"></span><span class="hb"></span><span class="ls-c"></span><span class="hb-c"></span></div>
                                            <div><strong>税后：</strong><span class="lsnf"></span><span class="hbnf"></span><span class="lsnf-c"></span><span class="hbnf-c"></span></div>
                                            <div><strong>比例：</strong><span class="lsr"></span><span class="hbr"></span><span class="lsr-c"></span><span class="hbr-c"></span></div>
                                            </div></div>
                                         `);
                            ...
                            if (obj.lowest_sell_order) {
                                ...
                                $("span.ls-c").text("¥ " + (lowest_sell_order / 100 / ratio).toFixed(2));
                                var lsnofee = calcfee(lowest_sell_order);
                                $("span.lsnf").text(obj.price_prefix + " " + lsnofee / 100);
                                $("span.lsnf-c").text("¥ " + (lsnofee / 100 / ratio).toFixed(2));
                            }
                            ...
                            if (obj.highest_buy_order) {
                                ...
                                $("span.hb-c").text("¥ " + (highest_buy_order / 100 / ratio).toFixed(2));
                                var hbnofee = calcfee(highest_buy_order);
                                $("span.hbnf").text(obj.price_prefix + " " + hbnofee / 100);
                                $("span.hbnf-c").text("¥ " + (hbnofee / 100 / ratio).toFixed(2));
                            }
                            ...
                            if (GM_getValue("version", "0") != GM_info.script.version) {
                                ...
                                GM_notification(details, function () { GM_openInTab("https://keylol.com/t331397-1-1", { active: true }); });
                            }
                            low = lsnofee;
                            high = hbnofee;
                            calcratio();
...
function steam302() {
    var $302 = $('<div style="color:#FF0000;margin-top:15px"><span class="glyphicon glyphicon-remove"></span><strong>查询超时，建议使用<a target="_blank" href="https://keylol.com/t339527-1-1" style="color:#0b84d3">Steam302</a></strong></div>');
    if (site == "c5") {
        $("div.hero").before($302);
    } else if (site == "buff") {
...
function calcratio() {
    var t = setInterval(function () {
        ...
        if (siteprice != 0) {
            if (low) {
                $("span.lsr").text((siteprice * ratio * 100 / low).toFixed(2));
                $("span.lsr-c").text((siteprice * ratio * 100 / low).toFixed(2));
            }
            if (high) {
                $("span.hbr").text((siteprice * ratio * 100 / high).toFixed(2));
                $("span.hbr-c").text((siteprice * ratio * 100 / high).toFixed(2));
            }
            clearInterval(t);
        }
    }, 200);
