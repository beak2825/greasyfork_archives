...
// @version   1.2
// @description Allows drawing on websites when the control key and mouse button are pressed. Includes settings for color, line width, opacity, eraser tool, circle distance, softness, and shadow. Also includes undo, redo, clear all, save color, and clear color history buttons.
// @author    Heptatron
// @license MIT
// @match    *://*/*
// @grant    GM_setValue
// @grant    GM_getValue
// @downloadURL https://update.greasyfork.org/scripts/486818/WebPadOnline%21%20Draw%20on%20Websites%21%20Take%20Notes%21%20Doodle%21.user.js
// @updateURL https://update.greasyfork.org/scripts/486818/WebPadOnline%21%20Draw%20on%20Websites%21%20Take%20Notes%21%20Doodle%21.meta.js
// ==/UserScript==
...
(function() {
  ...
  function createOverlay() {
    ...
    overlay.style.zIndex = '9998'; // Below canvas, but above everything else
  ...
  function draw(e) {
    ...
    if (settings.eraser) {
        ...
        ctx.globalCompositeOperation = 'source-over'; // Set the composite operation to draw
    }
  ...
  function init() {
    createCanvas();
    createOverlay(); // Add overlay initialization
  ...
  function createDraggableCircle() {
    ...
    circle.style.backgroundColor = '#888'; // Gray color
    circle.style.cursor = 'move';
    circle.style.zIndex = '10000'; // Ensure it's on top
    circle.style.left = '10px'; // Initial position
    circle.style.top = '10px'; // Initial position
    ...
    circle.addEventListener('mousedown', function(event) {
      if (settings.touchscreenMode) {
        isDrawing = false; // Disable drawing while dragging if touchscreen mode is enabled
      }
      isDragging = true;
    ...
    document.addEventListener('mouseup', function() {
      isDragging = false;
      if (settings.touchscreenMode) {
        isDrawing = false; // Disable drawing when mouse is released if touchscreen mode is enabled
      }
    });

    circle.addEventListener('click', function() {
      toggleControlsWithTouchscreenMode(); // Toggle controls when the circle is clicked
    });
  }
  ...
  function loadSettings() {
    ...
    document.getElementById('touchscreenMode').checked = savedTouchscreenMode; // Update checkbox state
  }

  function toggleControls(e) {
    if (e.ctrlKey && e.altKey) {
      toggleControlsWithTouchscreenMode(); // Use the same function to toggle controls
    }
  }
  ...
  function createColorHistoryButtons() {
    ...
    colorHistory.forEach((color, index) => {
      ...
      colorButton.style.marginBottom = '5px'; // Add bottom margin for better spacing
