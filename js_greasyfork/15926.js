// ==UserScript==
// @name        HIT Scraper - Saqfish style
// @author      Kerek, Tjololo, clickhappier, and feihtality, saqfish
...
// @downloadURL https://update.greasyfork.org/scripts/15926/HIT%20Scraper%20-%20Saqfish%20style.user.js
// @updateURL https://update.greasyfork.org/scripts/15926/HIT%20Scraper%20-%20Saqfish%20style.meta.js
// ==/UserScript==
...
function testie(name,mode,inter){
    var title = " ";
    var prevlink = " ";
    var n2 = " ";
    if (mode == "pwr"){   
        title = name["title"];
        prevlink = name["prev_link"].split("=");
        n2 = name["requester"]+"-"+name["title"];
        var watcher = {
            id          : prevlink[1],
            duration    : 1000,
            type        : 'hit',
...
function Push(text){
    console.log(settings.options.extrastoken);
    $.ajax({  
        url: "https://api.pushbullet.com/v2/pushes",  
        headers: {
            'Access-Token': settings.options.extrastoken,
            'Content-Type':'application/json'
        },
        type: "POST",  
        dataType: "json",  
        contentType: "json",  
        data: JSON.stringify({"type": "note", "body": text}),  
        success: function(){              
            console.log("PushBullet Sucess");  
        },  
        error: function(){  
            console.log("PushBullet Fail");  
        }  
    });
}
...
var panda_list = [];
if (!GM_getValue("scraper_panda_list"))
    GM_setValue("scraper_panda_list","nothing pandalisted yet");
if (GM_getValue("scraper_panda_list"))
    panda_list = GM_getValue("scraper_panda_list").split('^');
...
var ta = setInterval(function(){ DAL = []; console.log("DAL cleared");}, 100000);
var ta2 = setInterval(function(){ DAL2 = []; console.log("DAL2 cleared");}, 300000);
//var ta2 = setInterval(function(){ DAL3 = []; console.log("DAL3 cleared");}, 8000);

//For hitbox
var hitbox = document.createElement('DIV');
$(hitbox).html('<div style="left : 20%; top: 200px; position: fixed; background-color: black; overflow: hidden;             overflow-x: scroll; height: 200px; width: 500px; z-index: 100;">     <table id="table"             style="border-collapse:collapse;width: 100%;padding:10px;border:10px solid green;"             class="blackborder" >         <thead>             <tr class="HITS">                 <th style="color: white;">HITS</th>             </tr>         </thead>          <tbody style="1px solid orange"">             <tr id="hitsbox" style="color: white; border: 1px solid #ff0000;"></tr>         </tbody>     </table> </div>');
//$('body').append(hitbox);
document.body.insertBefore(hitbox, document.body.firstChild);
...
//For editing the panda list
var shouldPanda = false;
var div3 = document.createElement('div');
var textarea3 = document.createElement('textarea');

div3.style.position = 'fixed';
div3.style.width = '500px';
div3.style.height = '235px';
div3.style.left = '50%';
div3.style.right = '50%';
div3.style.margin = '-250px 0px 0px -250px';
div3.style.top = '300px';
div3.style.padding = '5px';
div3.style.border = '2px';
div3.style.backgroundColor = 'black';
div3.style.color = 'white';
div3.style.zIndex = '100';
div3.setAttribute('id','panda_div');

textarea3.style.padding = '2px';
textarea3.style.width = '500px';
textarea3.style.height = '180px';
textarea3.title = 'panda list';
textarea3.setAttribute('id','panda_text');

div3.textContent = 'PANDA - Every entry will be sent to TM with an active 1sec timer - TESTING';
...
save_button3.textContent = 'Save';
save_button3.setAttribute('id', 'save_pandalist');
save_button3.style.height = '18px';
save_button3.style.width = '100px';
...
cancel_button2.textContent = 'Cancel';
cancel_button2.setAttribute('id', 'cancel_pandalist');
cancel_button2.style.height = '18px';
cancel_button2.style.width = '100px';
cancel_button2.style.fontSize = '10px';
cancel_button2.style.paddingLeft = '3px';
...
div3.appendChild(save_button3);
div3.appendChild(cancel_button2);
document.body.insertBefore(div3, document.body.firstChild);
$("#panda_div").hide();


// save and cancel for includelist
function save_pandalist() {
    //console.log("Save");
    var textarea = $("#panda_text");
    var text = textarea.val();
    var pandas = text.split("^");
    //console.log(pandas);
    var trimmed_list = [];
    for (var requester in pandas){
        if (pandas[requester].trim().length !== 0)
            trimmed_list.push(pandas[requester].toLowerCase().trim());
    }
    GM_setValue("scraper_panda_list",trimmed_list.join('^'));
    panda_list = GM_getValue("scraper_panda_list").split('^');
    //console.log(panda_list);
    $("#panda_div").hide();
}
save_button3.addEventListener("click", function(){ save_pandalist(); }, false);
cancel_button2.addEventListener("click", function(){ 
    // reset textarea contents upon cancel
    panda_list = GM_getValue("scraper_panda_list").split('^');
    var textarea = $("#panda_text");
    var text = "";
    for (var i = 0; i < panda_list.length; i++){
        text += panda_list[i]+"^";
    }
    textarea.val(text.substring(0, text.length - 1));
    // close editor
...
var pandaOnly = document.createElement("INPUT");
pandaOnly.type = "checkbox";
...
saveState.init = function() {
    ...
    pandaOnly.checked = this.lastState.useInclude || false;
...
css += " #hitsbox td{border: 1px solid #ff0000;}\n";
css += "</style>";
...
settings.sidebar = { mainDiv:      document.createElement("DIV"),
                    ...
                    extrasSpan: document.createElement("SPAN"),
                    init: function() {
                        ...
                        this.mainDiv.appendChild(this.extrasSpan);
                        ...
                        this.extrasSpan.className = "settings settingsSidebarSpans settingsSidebarExtrasSpan";
                        this.extrasSpan.textContent = "Extras";
                        this.extrasSpan.addEventListener("click", function() { settings.sidebar._click("extrasSpan", "extras") });
                    },
...
settings.general = { mainDiv:          document.createElement("DIV"),
                    ...
                    commWeight2:        document.createElement("DIV"),
                    ...
                    commLabel2:         document.createElement("LABEL"),
                    ...
                    init: function() {
                        ...
                        var weightContainer2 = settings.panel.containers.cloneNode(false);
                        var weightSection2 = settings.panel.sections.cloneNode(false);
                        ...
                        this.mainDiv.appendChild(weightContainer2);
                        ...
                        ///SAQFISH BLAH			
                        this.commWeight2.className = "settings settingsDivInput settingsGeneralCommWeight";
                        this.commWeight2.contentEditable = true;
                        this.commWeight2.title = "default value: .5";
                        this.commWeight2.addEventListener("keyup", function() {
                            var num = Math.abs(Number(settings.general.commWeight2.textContent));
                            if (!isNaN(num)){
                                settings.options.commit("comm2", "commWeight2", num);

                                console.log(num);
                            }

                        });
                        this.commLabel2.textContent = "Seconds: ";
                        this.commLabel2.className = "settings settingsLabelSA";

                        var tempDiv4 = tempDiv.cloneNode(false);
                        tempDiv4.innerHTML = "<span style='position: relative; left: -8px;'><b>Panda Settings</b></span><br>";
                        tempDiv4.appendChild(this.commLabel2);
                        tempDiv4.appendChild(this.commWeight2);
                        weightContainer2.appendChild(tempDiv4);
                        weightContainer2.appendChild(weightSection2);
                        weightSection2.innerHTML = "<p>Panda timeout (seconds).<br>";
                        //weightSection2.innerHTML += "The default values respect backwards compatibility; recommended settings are [1, 6, 3.5, 1]."
                        ///END SAQFISH BLAH xD
                    },
                    _init: function() {
                        ...
                        this.commWeight2.textContent = settings.options.toWeighting.comm2;
...
settings.extras = { mainDiv:      document.createElement("DIV"),
                   useNotifications:  document.createElement("INPUT"),
                   usePushbullet:  document.createElement("INPUT"),
                   usePanda:  document.createElement("INPUT"),
                   notificationLabel: document.createElement("LABEL"),
                   pandaLabel: document.createElement("LABEL"),
                   pushbulletLabel: document.createElement("LABEL"),
                   pdelay:        document.createElement("DIV"),
                   pLabel:         document.createElement("LABEL"),
                   tokenn: document.createElement("DIV"),
                   tokennLabel: document.createElement("LABEL"),

                   init: function() {
                       this.mainDiv.className = "settings settingsInnerDiv settingsMainContainer settingsExtrasMainDiv";
                       this.mainDiv.dataset.category = "extras";
                       var notificationContainer = settings.panel.containers.cloneNode(false);
                       var listEditorContainer = settings.panel.containers.cloneNode(false);
                       var notificationSection = settings.panel.sections.cloneNode(false);
                       var tempDiv = document.createElement("DIV");
                       tempDiv.style.float = "left";
                       var pbContainer = settings.panel.containers.cloneNode(false);
                       var pbSection = settings.panel.sections.cloneNode(false);
                       var tempDiv2 = document.createElement("DIV");
                       tempDiv2.style.float = "left";
                       var pandaContainer = settings.panel.containers.cloneNode(false);
                       var pandaSection = settings.panel.sections.cloneNode(false);
                       var tempDiv3 = document.createElement("DIV");
                       tempDiv3.style.float = "left";

                       this.mainDiv.appendChild(notificationContainer);
                       this.mainDiv.appendChild(pbContainer);
                       this.mainDiv.appendChild(pandaContainer);

                       this.useNotifications.className = "settings settingsCheckbox settingsExtrasuseNotifications";
                       this.useNotifications.type = "checkbox";
                       this.useNotifications.id = "useNotifications";
                       this.useNotifications.addEventListener("click", function() { settings.options.commit("extrasnotifications", "ens", settings.extras.useNotifications.checked); });
                       this.usePushbullet.className = "settings settingsCheckbox settingsExtrasuseNotifications";
                       this.usePushbullet.type = "checkbox";
                       this.usePushbullet.id = "useNotifications";
                       this.usePushbullet.addEventListener("click", function() { settings.options.commit("extraspushbullet", "epb", settings.extras.usePushbullet.checked); });
                       this.usePanda.className = "settings settingsCheckbox settingsExtrasusePanda";
                       this.usePanda.type = "checkbox";
                       this.usePanda.id = "usePanda";
                       this.usePanda.addEventListener("click", function() { settings.options.commit("extraspanda", "epanda", settings.extras.usePanda.checked); });

                       this.notificationLabel.className = "settings settingsLabelExtras";
                       this.notificationLabel.htmlFor = "useNotifications";
                       this.notificationLabel.textContent = "Use Notifications";
                       this.pushbulletLabel.className = "settings settingsLabelExtras";
                       this.pushbulletLabel.htmlFor = "usePushbullet";
                       this.pushbulletLabel.textContent = "Use PushBullet";
                       this.pandaLabel.className = "settings settingsLabelExtras";
                       this.pandaLabel.htmlFor = "usePanda";
                       this.pandaLabel.textContent = "Use Panda";

                       notificationContainer.innerHTML = "<span style='position: relative; left: 7px;'><b>Notifications</b></span><p></p>";
                       notificationContainer.appendChild(tempDiv);
                       notificationContainer.appendChild(notificationSection);
                       pbContainer.innerHTML = "<span style='position: relative; left: 7px;'><b>PushBullet</b></span><p></p>";
                       pbContainer.appendChild(tempDiv2);
                       pbContainer.appendChild(pbSection);
                       pandaContainer.innerHTML = "<span style='position: relative; left: 7px;'><b>Panda Settings</b></span><p></p>";
                       pandaContainer.appendChild(tempDiv3);
                       pandaContainer.appendChild(pandaSection);

                       this.pdelay.className = "settings settingsDivInput settingsextraspdelay";
                       this.pdelay.contentEditable = true;
                       this.pdelay.title = "default value: 0.5";
                       this.pdelay.addEventListener("keyup", function() {
                           var num = Math.abs(Number(settings.extras.pdelay.textContent));
                           if (!isNaN(num)){
                               settings.options.commit("extrasdelay", "delay", num);

                               console.log(num);
                           }

                       });
                       this.pLabel.textContent = "Seconds: ";
                       this.pLabel.className = "settings settingsLabelSA";

                       this.tokenn.className = "settings settingsDivInput settingsextrastoken";
                       this.tokenn.contentEditable = true;
                       this.tokenn.title = "Pushbullet Token";

                       this.tokenn.addEventListener("keyup", function() { settings.options.commit("extrastoken", "etoken", settings.extras.tokenn.textContent);});
                       this.tokennLabel.textContent = "Token: ";
                       this.tokennLabel.className = "settings settingsLabelSA";

                       tempDiv.appendChild(this.notificationLabel);
                       tempDiv.appendChild(this.useNotifications);
                       tempDiv2.appendChild(this.pushbulletLabel);
                       tempDiv2.appendChild(this.usePushbullet);
                       tempDiv2.appendChild(document.createElement("BR"));
                       tempDiv2.appendChild(document.createElement("BR"));
                       tempDiv2.appendChild(this.tokennLabel);
                       tempDiv2.appendChild(this.tokenn);

                       tempDiv3.appendChild(this.pandaLabel);
                       tempDiv3.appendChild(this.usePanda);
                       tempDiv3.appendChild(document.createElement("BR"));
                       tempDiv3.appendChild(document.createElement("BR"));
                       tempDiv3.appendChild(this.pLabel);
                       tempDiv3.appendChild(this.pdelay);


                       notificationSection.style.marginLeft = "150px";
                       pbSection.style.marginLeft = "150px";
                       //notificationSection.innerHTML = "Enable notifications all around.";

                   },
                   _init: function() {
                       this.useNotifications.checked = settings.options.extrasnotifications;
                       this.usePushbullet.checked = settings.options.extraspushbullet;
                       this.usePanda.checked = settings.options.extraspanda;
                       this.pdelay.textContent = settings.options.extrasdelay;
                       this.tokenn.textContent = settings.options.extrastoken;

                   }
                  };
settings.options = { resultsColorType:  null, 
                    resultsSortType:    null, 
                    toWeighting:        { comm: null, pay: null, fair: null, fast: null, comm2: null },
                    blocklistWildcards: null,
                    extrasnotifications: null,
                    extraspushbullet: null,
                    extraspanda: null,
                    extrasdelay: null,
                    extrastoken: null,
                    ...
                    init: function() {
                        ...
                        this.toWeighting = {comm: saveState.getItem("commWeight") || 1,
                                            ...
                                            comm2: saveState.getItem("commWeight2") || 0.5,
                                           };
                        this.blocklistWildcards = saveState.getItem("wildblocks") || false;
                        this.extrasnotifications = saveState.getItem("ens") || false;
                        this.extraspushbullet = saveState.getItem("epb") || false;
                        this.extrasdelay = saveState.getItem("delay") || 0.5;
                        this.extraspanda = saveState.getItem("epanda") || false;
                        this.extrastoken = saveState.getItem("etoken") || "None";
...
settings.display = function() {
    ...
    this.extras.mainDiv.style.height = String(sheight-59).concat("px");
    this.extras.mainDiv.style.width = String(swidth-126).concat("px");
...
var panda_button = document.createElement("BUTTON");
    ...
    if (global_run == true)
    {
        ...
        else
        {
            ...
            for (var j = 0; j < found_key_list.length; j++)
            {
                ...
                var pandalisted = panda_check(obj.requester.replace(" (search)", ""),obj.title);
                if (!(obj.masters == true && masters_hide.checked) && (ignored && obj.found_this_time) || (!ignored && obj.found_this_time)){
                    ...
                    if(obj.req_link == obj.rid)
                    {
                        ...
                        //col_heads.push('<a href=\"javascript:void(0);\" onClick=\"testie(this);"\> --TM-- </a>');
                    }
                    else
                    {
                        if (useTO)
                    ...
                    if (!includelisted && obj.found_this_time && matchOnly.checked){
                        //Old alert stuff for new hits in include list
                        //if (confirm("Requester: "+obj.requester +"\n"+ "Title: "+obj.title+"\n"+"Hits: "+obj.hits+"\n"+"Reward: "+obj.reward) == true) {window.open(obj.prev_link);}
                        ...
                        if(DAL.indexOf(obj.rid) >= 0){

                        }else{
                            console.log(settings.options.extrasnotifications);
                            if(settings.options.extrasnotifications){
                                console.log("DELAY: " +delay);
                                var delay = settings.options.toWeighting.comm2 * 1000
                                var holder2 = obj;
                                var noti2 = new Notification(obj.requester + "-" + obj.title +" Found!");                         
                                noti2.onclick = function() {testie(holder2,"w",delay);};
                                console.log(obj);

                                document.getElementById("ding_noise"+audio_index).play();
                                DAL.push(obj.rid);
                                DAL2.push(obj.rid);
                                if(DAL2.indexOf(obj.rid) != -1){
                                    if(settings.options.extraspushbullet){
                                        Push(obj.requester + " " + obj.title + " " + obj.reward);
                                    }
                                }
                            }
                        }
                    } else if (!pandalisted && obj.found_this_time && settings.options.extraspanda){

                        //THIS IS WHERE THE MAGIC WILL HAPPEN :D

                        if(DAL.indexOf(obj.rid) != -1){

                        }else{
                            var holder = obj;
                            //var noti = new Notification(obj.title +" Found! Hoard it?");
                            //noti.onclick = function() {testie(holder,"w",500);};
                            var delay = settings.options.extrasdelay * 1000

                            testie(holder,"w",delay);
                            console.log("DELAY: " +delay);
                            console.log(obj);
                            snd.play();
                            DAL.push(obj.rid);
                    ...
                    var data222 = localStorage.getItem('notifier_watchers');
                    data3 = JSON.parse(data222);
                    var objarray = [];

                    //new_time_display_input.value = 1;
                    //saveState.setItem("newHitHighlight", new_time_display_input.value);
                    //saveState.save();
                    var objarray2 = eliminateDuplicates(objarray);
                    for (var o = 0, leno = objarray2.length; o<leno; o++){
                    ...
                    if (Object.keys(scraper_history).length > 0)
                    {
                        ...
                        if (obj.new_result < 1000*save_new_results_time)
                        {
                            if (ignored){
                                ...
                                if(aon==true){
                                    $('#hitsbox').append('<td>' +col_heads+ '</td>');
                                    $('#hitsbox').scrollTop = $('#hitsbox').scrollHeight;
                                    eeee = col_heads[5].split('=');
                                    var newlink = eeee[2]+"="+eeee[3];
                                    var nnewlink = newlink.replace(/'/g, "");
                                    var nnnewlink = nnewlink.substring(0,nnewlink.length - 7);
                                    console.log(nnnewlink);
                                    $.ajax({
                                        url: nnnewlink,
                    ...
                    //Old reddit button for whoever wants to re-add it
                    ...
                    var button5 = document.createElement('button'); // TM EXPORT BUTTON
                    button5.textContent = 'TM';
                    button5.style.width = '33px';
                    button5.className = "redditButton";
                    button5.className += opt_exportreddit.checked ? (" taButtons"+sufTheme) : (" taButtonsOff"+sufTheme);
                    button5.title = 'Export to TM.'

                    var button6 = document.createElement('button'); // TM EXPORT BUTTON 2
                    button6.textContent = '[$$]';
                    button6.style.width = '33px';
                    button6.className = "redditButton";
                    button6.className += opt_exportreddit.checked ? (" taButtons"+sufTheme) : (" taButtonsOff"+sufTheme);
                    button6.title = 'Export and Auto on TM on a 10 sec timer - BE CAREFUL!'
                    ...
                    button5.addEventListener("click", (function (obj,j) { return function() {testie(obj,"nrm");}})(obj,j));
                    row.cells[1].appendChild(document.createTextNode(" "));
                    row.cells[1].appendChild(button5);

                    button6.addEventListener("click", (function (obj,j) { return function() {testie(obj,"pwr");}})(obj,j));
                    row.cells[1].appendChild(document.createTextNode(" "));
                        ...
                        if (reviews > 4)
                        {
                            ...
                            if (copt == "sim") {                       
                                ...
                                if (quality > 4){
                                    ...
                                    //console.log(obj);
                                    //console.log("C-"+comm+"F-"+fair+"F2-"+fast); 
...
function ignore_check(r,t){
    ...
    if (shouldPanda){  // if in Use Includelist mode, treat all HITs not matching the includelist as if they were on the blocklist
        //console.log(include_list);
        var temp = panda_list.map(function(item) { return item.toLowerCase().replace(/\s+/g," "); }).indexOf(r.toLowerCase().replace(/\s+/g," "));
        console.log(temp);
        if (temp != -1)
            foundR = -1;
        else
...
function panda_check(r,t){
    var tempList = panda_list.map(function(item) { return item.toLowerCase().replace(/\s+/g," "); });
    var foundR = -1;
    var foundT = -1;
    foundR = tempList.indexOf(r.toLowerCase().replace(/\s+/g," "));
    foundT = tempList.indexOf(t.toLowerCase().replace(/\s+/g," "));
    var found = foundR == -1 && foundT == -1;
    return found;  // returns false (making !(panda_check(x,y)) true) if HIT should be highlighted, returns true if it shouldn't be highlighted
}
...
function start_running()
{
    if (big_red_button.textContent == "Start")
    {
        ...
        if (correctForSkips.checked)
        {
            if (pandaOnly.checked)
            {
                status_array.push("Use pandalist (panda only) checked, so ignoring Correct For Skips to prevent issues");
                correct_for_skips = false;
            }
            else{
                correct_for_skips = true;
        ...
        if (pandaOnly.checked)
        {
            if (panda_list.length == 0){
                status_array.push("No items in Pandalist, so ignoring Use Pandalist checkbox");
                shouldPanda = false;
            }
            else
            {
                ...
                if (!useBlock.checked)
                {
                    status_array.push("Use Pandalist checked, so forcing Use Blocklist for intended output behavior");
                    useBlocklist = true;
                }
            }
        }
...
function show_interface()
{
    ...
    redditspan.appendChild(document.createTextNode(" TM "));
    ...
    //Temp fix. Will make this better later. Gotta make that $$$
    var x = document.createElement("INPUT");
    x.setAttribute("type", "checkbox");

    var pandalistspan = document.createElement("SPAN");
    pandalistspan.appendChild(document.createTextNode("Accept next"));
    pandalistspan.className += " cpSpans"+sufTheme;
    pandalistspan.title = "Open new hit in window.\n";
    if (pandaOnly.checked) pandalistspan.className += " cpSpansOn"+sufTheme;
    pandalistspan.addEventListener("click", function() {
        if (x.checked) {
            x.checked = false;
            //pandalistspan.className = pandalistspan.className.replace(/(?:^| +)cpSpansOn_[a-zA-Z]+/g, "");
            aon = false;
            $(hitbox).hide();
            ...
            aon = true;
            $(hitbox).show();
            //pandalistspan.className += " cpSpansOn"+sufTheme;
        }
        saveState.setItem("usepanda", pandaOnly.checked)
        saveState.save();
    }, false);
    if (show_checkboxes.checked) pandaOnly.className = "checkboxes";
    else pandaOnly.className = "checkboxesOff";
    pandalistspan.appendChild(x);

    pandaOnly.onclick = function () { UIAux(pandalistspan, pandaOnly, "usepanda"); };
    control_panel.appendChild(spacer.cloneNode(true));
    control_panel.appendChild(pandalistspan);
    ...
    // open pandalist editor
    panda_button.textContent = "Edit pandalist";
    panda_button.className += " cpButtons"+sufTheme;
    panda_button.id = "pandas_reset_button";
    panda_button.onclick = function() {
        panda_list = GM_getValue("scraper_panda_list").split('^');
        var textarea = $("#panda_text");
        var text = "";
        for (var i = 0; i < panda_list.length; i++){
    ...
    table_div.appendChild(document.createTextNode("   "));
    table_div.appendChild(panda_button); // includelist editor
    table_div.appendChild(document.createTextNode("   "));
