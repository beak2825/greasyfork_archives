...
// @name Pixiv Muter
// @name:ja Pixiv Muter
// @description Mute unwanted authors or tags on Pixiv's illustration and novel tag search pages
// @description:ja pixivのイラストおよび小説のタグ検索画面で、表示したくない作者またはタグをミュートします
// @include https://www.pixiv.net/*
// @version 2025-05-26
// @author w5xZm
// @license MIT
// @namespace https://greasyfork.org/users/1471228
// @downloadURL https://update.greasyfork.org/scripts/536299/Pixiv%20Muter.user.js
// @updateURL https://update.greasyfork.org/scripts/536299/Pixiv%20Muter.meta.js
// ==/UserScript==
...
(function() {
  ...
  const EVENT_MUTE_STATE_CHANGED = "pixivMuteStateChanged";
  const EVENT_REQUEST_TAG_MODAL_SHOW = "requestTagModalShow";
  const PM_SUGGESTIONS_LIST_ID = "pm-suggestions-list";
  const PM_CURRENTLY_MUTED_LIST_ID = "pm-currently-muted-list";
  const PM_EXPORT_TEXT_AREA_ID = "pm-export-text-area";
  const PM_IMPORT_TEXT_AREA_ID = "pm-import-text-area";
  const PM_EXPORT_BUTTON_ID = "pm-export-button";
  const PM_IMPORT_BUTTON_ID = "pm-import-button";
  const PM_TOGGLE_IMPORT_EXPORT_BUTTON_ID = "pm-toggle-import-export-button";
  const PM_EXPORT_SECTION_ID = "pm-export-section";
  const PM_IMPORT_SECTION_ID = "pm-import-section";
  const PM_IMPORT_EXPORT_CONTAINER_ID = "pm-import-export-container";
  const MUTE_USER_STORAGE_KEY = "pixivMutedUserIds";
  const MUTE_TAGS_STORAGE_KEY = "pixivMutedTags";
  const MUTE_TAG_MODAL_ID = "pixiv-mute-tag-modal";
  const MEDIA_ITEM_SELECTOR = ".list-item.is-list:not(.ads):not(.column-3)";
  const USER_INFO_SELECTOR_COMBINED = "a.user, a.illust-author";
  class StorageService {
    constructor() {
      __publicField(this, "userKey");
      __publicField(this, "tagKey");
      this.userKey = MUTE_USER_STORAGE_KEY;
      this.tagKey = MUTE_TAGS_STORAGE_KEY;
    }
    // --- User Mute Methods ---
    getMutedUsers() {
      const mutedUsersJson = localStorage.getItem(this.userKey);
      let users = [];
      try {
        const parsedData = mutedUsersJson ? JSON.parse(mutedUsersJson) : [];
        if (Array.isArray(parsedData)) {
          if (parsedData.every((item) => typeof item === "object" && item !== null && "id" in item && typeof item.id === "string" && (item.name === null || typeof item.name === "string"))) {
            users = parsedData.filter((user) => user.id.trim() !== "");
          } else if (parsedData.length > 0) {
            console.warn("StorageService: Muted users data is an array but not in the expected format. Clearing.");
            users = [];
            this.saveMutedUsers(users);
          }
        }
      } catch (e) {
        console.error("Error parsing muted users from localStorage:", e);
        users = [];
      }
      return users;
    }
    saveMutedUsers(usersArray) {
      try {
        localStorage.setItem(this.userKey, JSON.stringify(usersArray));
        return true;
      } catch (e) {
        console.error("Error saving muted users to localStorage:", e);
        return false;
      }
    }
    addMutedUser(userId, userName = null) {
      if (!userId || typeof userId !== "string" || userId.trim() === "") return false;
      const normalizedUserId = userId.trim();
      const mutedUsers = this.getMutedUsers();
      if (mutedUsers.some((user) => user.id === normalizedUserId)) {
        return false;
      }
      mutedUsers.push({ id: normalizedUserId, name: userName ? userName.trim() : null });
      if (this.saveMutedUsers(mutedUsers)) {
        console.log(`StorageService: User ${normalizedUserId} (Name: ${userName}) added to mute list.`);
        return true;
      }
      return false;
    }
    isUserMuted(userId) {
      ...
      const mutedUsers = this.getMutedUsers();
      return mutedUsers.some((user) => user.id === userId.trim());
    }
    removeMutedUser(userId) {
      if (!userId || typeof userId !== "string") return false;
      const normalizedUserId = userId.trim();
      let mutedUsers = this.getMutedUsers();
      const initialLength = mutedUsers.length;
      mutedUsers = mutedUsers.filter((user) => user.id !== normalizedUserId);
      if (mutedUsers.length < initialLength) {
        if (this.saveMutedUsers(mutedUsers)) {
          console.log(`StorageService: User ${normalizedUserId} removed from mute list.`);
          return true;
        }
      }
      return false;
    ...
    // --- Tag Mute Methods ---
    getMutedTags() {
      const mutedTagsJson = localStorage.getItem(this.tagKey);
      try {
        const tags = mutedTagsJson ? JSON.parse(mutedTagsJson) : [];
        return Array.isArray(tags) ? tags.filter((tag) => typeof tag === "string" && tag.trim() !== "") : [];
      } catch (e) {
        console.error("Error parsing muted tags from localStorage:", e);
        return [];
      }
    }
    saveMutedTags(tagsArray) {
      try {
        localStorage.setItem(this.tagKey, JSON.stringify(tagsArray));
        return true;
      } catch (e) {
        console.error("Error saving muted tags to localStorage:", e);
        return false;
      }
    }
    addMutedTag(tag) {
      ...
      const mutedTags = this.getMutedTags();
      if (mutedTags.includes(normalizedTag)) {
        return false;
      }
      mutedTags.push(normalizedTag);
      if (this.saveMutedTags(mutedTags)) {
        console.log(`StorageService: Tag "${normalizedTag}" added to mute list.`);
        return true;
      }
      return false;
    }
    removeMutedTag(tag) {
      ...
      const normalizedTag = tag.trim();
      let mutedTags = this.getMutedTags();
      const index = mutedTags.indexOf(normalizedTag);
      if (index === -1) {
        return false;
      }
      mutedTags.splice(index, 1);
      if (this.saveMutedTags(mutedTags)) {
        console.log(`StorageService: Tag "${normalizedTag}" removed from mute list.`);
        return true;
      }
      return false;
    }
  }
  function getUserInfo(mediaElement) {
    var _a;
    let userId = null;
    let userName = null;
    const userInfoElement = mediaElement.querySelector(USER_INFO_SELECTOR_COMBINED);
    if (userInfoElement) {
      if (userInfoElement.matches("a.user")) {
        userId = userInfoElement.dataset.gtmUserId || null;
        userName = userInfoElement.textContent ? userInfoElement.textContent.trim() : null;
      } else if (userInfoElement.matches("a.illust-author")) {
        const href = userInfoElement.href;
        const userIdMatch = href ? href.match(/\/users\/(\d+)/) : null;
        userId = userIdMatch ? userIdMatch[1] : null;
        const userNameElement = userInfoElement.querySelector(".ellipsis");
        userName = userNameElement ? ((_a = userNameElement.textContent) == null ? void 0 : _a.trim()) || null : null;
        if (!userName) {
          userName = userInfoElement.textContent ? userInfoElement.textContent.trim() : null;
        }
      }
  ...
  function getItemTags(mediaElement) {
    const tags = [];
    const tagElements = mediaElement.querySelectorAll("div.display-tags a.tag");
    tagElements.forEach((tagElement) => {
      const tagName = tagElement.dataset.gtmTagName || tagElement.textContent;
      if (tagName) {
  ...
  class MediaMuter {
    ...
    /**
     * 現在のミュート状態に基づいてUIを適用する
     * @param {Element} mediaElement - 対象の作品コンテナ要素
     */
    applyUI(mediaElement) {
      const userInfo = getUserInfo(mediaElement);
      const userId = userInfo.userId;
      const itemTags = getItemTags(mediaElement);
      mediaElement.style.display = "";
      if (userId && this.storageService.isUserMuted(userId)) {
        mediaElement.style.display = "none";
        return;
      }
      const mutedTags = this.storageService.getMutedTags();
      const foundMutedTag = itemTags.find((itemTag) => mutedTags.includes(itemTag));
      if (foundMutedTag) {
        mediaElement.style.display = "none";
      }
    }
    _dispatchMuteChangeEvent(detail) {
      document.dispatchEvent(new CustomEvent(EVENT_MUTE_STATE_CHANGED, { detail }));
    }
  }
  function isMuteCustomEvent(event) {
    return "detail" in event;
  }
  class ContentProcessor {
    constructor(storageService) {
      __publicField(this, "storageService");
      __publicField(this, "mediaMuter");
      __publicField(this, "boundHandleMuteStateChange");
      this.storageService = storageService;
      this.mediaMuter = new MediaMuter(this.storageService);
      this.boundHandleMuteStateChange = this.handleMuteStateChange.bind(this);
      document.addEventListener(EVENT_MUTE_STATE_CHANGED, this.boundHandleMuteStateChange);
    }
    handleMuteStateChange(event) {
      if (isMuteCustomEvent(event)) {
        console.log("ContentProcessor: Detected pixivMuteStateChanged event", event.detail);
        this.processAllMediaItems();
      }
    }
    processAllMediaItems() {
      console.log("ContentProcessor: Processing all media items...");
      const allMediaElements = document.querySelectorAll(MEDIA_ITEM_SELECTOR);
      allMediaElements.forEach((mediaElement) => {
        this.mediaMuter.applyUI(mediaElement);
      });
      console.log(`ContentProcessor: Finished UI processing for ${allMediaElements.length} media items.`);
    }
    processSingleMediaItem(mediaElement) {
      if (mediaElement && mediaElement.matches(MEDIA_ITEM_SELECTOR)) {
        console.log("ContentProcessor: Processing single media item...");
        this.mediaMuter.applyUI(mediaElement);
      }
    }
    destroy() {
      document.removeEventListener(EVENT_MUTE_STATE_CHANGED, this.boundHandleMuteStateChange);
      console.log("ContentProcessor: Event listener removed.");
    }
  }
  class UIManager {
    constructor() {
      __publicField(this, "muteSettingButtonId");
      __publicField(this, "stylesInjected");
      this.muteSettingButtonId = "pm-setting-button";
      this.stylesInjected = false;
    }
    injectStyles() {
      if (this.stylesInjected || document.getElementById("pixiv-mute-style")) {
        this.stylesInjected = true;
        return;
      }
      const style = document.createElement("style");
      style.id = "pixiv-mute-style";
      style.textContent = `
            .pm-ui-element {
                font-family: sans-serif;
                font-size: 13px;
            }
            .pixiv-unmute-message, .pixiv-tagmute-message {
                padding: 15px;
                text-align: center;
                color: #333;
                font-size: 14px;
            ...
            .pm-button {
                ...
                padding: 5px 10px;
                cursor: pointer;
                margin-left: 5px; /* ボタン間のマージン調整 */
            }
            .pm-unmute-button, .pm-tag-management-button { /* MediaMuter内のボタン */
                margin-top: 10px;
                color: #43a047;
                border-color: #43a047;
            }
            .pm-mute-button { /* MediaMuter内のボタン */
                position: absolute;
            ...
            .pm-modal {
                ...
                opacity: 0;
                display: flex;
                justify-content: center; /* For horizontal centering of content if not full width */
                align-items: flex-end; /* Align content to the bottom */
                z-index: ${Number.MAX_SAFE_INTEGER};
                transition: opacity 0.5s cubic-bezier(0.25, 1, 0.5, 1); /* Transition for overlay opacity */
                pointer-events: none;
            }
            .pm-modal.show {
                opacity: 1;
            ...
            .pm-modal-content {
                ...
                /* max-width: 500px; /* Removed for full-width bottom sheet */
                max-height: 80vh; /* Max height for the bottom sheet */
                overflow-y: auto;
                box-shadow: 0 -4px 15px rgba(0,0,0,0.15); /* Shadow on top */
                text-align: left;
                transform: translateY(100%); /* Initially off-screen (slid down) */
                transition: transform 0.5s cubic-bezier(0.25, 1, 0.5, 1); /* Animation for sliding */
            }
            .pm-modal.show .pm-modal-content {
                transform: translateY(0); /* Slide in when modal is shown */
            }
            .pm-modal-section {
                margin-bottom: 20px;
                max-height: 30vh;
            ...
            .pm-modal-content h2 { /* モーダル全体のタイトル */
                font-size: 20px; /* 画像に合わせて調整 */
                font-weight: bold;
                color: #333;
                margin-top: 0;
                margin-bottom: 20px; /* 調整 */
                /* border-bottom: 1px solid #eee; */ /* 画像では線なし */
                /* padding-bottom: 15px; */
                text-align: center; /* 画像では中央 */
            }
            .pm-modal-content h3 { /* 各セクションのタイトル */
                font-size: 16px; /* 画像に合わせて調整 */
                font-weight: bold; /* 画像では太字ではないかも */
                color: #555;
                margin-top: 0;
                margin-bottom: 10px;
            }

            /* New Toggle List Styles */
            .pm-toggle-list {
            ...
            .pm-toggle-list li {
                ...
                align-items: center;
                padding: 10px 5px; /* Adjust padding to match image */
                border-bottom: 1px solid #f0f0f0; /* Light border like in image */
            }
            .pm-toggle-list li:last-child {
                border-bottom: none;
            }
            .pm-toggle-list li span {
                flex-grow: 1;
                font-size: 14px; /* Adjust to match image */
                color: #333;
            }
            .pm-clickable-list-item {
                cursor: pointer;
            }
            ...
            .pm-import-export-container.pm-sections-visible {
                max-height: 335px; /* Adjust as needed to fit content */
                padding-top: 10px; /* Space above the first section inside */
                padding-bottom: 10px; /* Space below the last section inside */
            }
            /* Ensure sections inside the container don't add excessive bottom margin if it's the last one */
            .pm-import-export-container > .pm-modal-section:last-child {
                margin-bottom: 0; /* Remove bottom margin from the last section if container has padding-bottom */
            }
            /* Styles for Import/Export Textareas */
            .pm-modal-section textarea {
            ...
            .pm-modal-section button.pm-export-button,
            .pm-modal-section button.pm-import-button {
                /* Add any specific styling for these buttons if needed */
                /* For example, to make them full width: */
                /* width: 100%; */
                /* display: block; */
                /* margin-top: 8px; */
            }
            .pm-toggle-import-export-button {
                width: 100%;
                display: block;
                margin-top: 10px;
                margin-bottom: 15px;
            ...
            .pm-export-text-area {
                /* Add any specific export textarea styles if needed */
            }
            .pm-import-text-area {
                /* Add any specific import textarea styles if needed */
            }
            .works-item-illust.list .illust-thumbnail[data-v-222e998f] {
                width: 50vw;
                height: 50vw;
            }
      ...
      document.head.insertAdjacentHTML("afterbegin", '<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200&icon_names=preview_off" />');
      this.stylesInjected = true;
      console.log("UIManager: Styles injected.");
    }
    /**
     * Adds a button next to the second child of .pager-view-nav to show the mute modal.
     */
    addMuteModalButtonToPagerNav() {
      const target = document.querySelector(".pager-view-nav");
      if (!target || document.getElementById("pm-pager-mute-modal-btn")) {
        return;
      }
      const div = document.createElement("div");
      div.style.textAlign = "right";
      div.className = "pm-ui-element";
      const btn = document.createElement("button");
      btn.id = "pm-pager-mute-modal-btn";
      btn.className = "pm-button pm-ui-element";
      btn.insertAdjacentHTML("afterbegin", '<span class="material-symbols-outlined">preview_off</span>Pixiv Muter');
      btn.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        document.dispatchEvent(new CustomEvent(EVENT_REQUEST_TAG_MODAL_SHOW));
      });
      div.appendChild(btn);
      if (target.parentNode) {
        target.parentNode.insertBefore(div, target.nextSibling);
      }
      console.log("UIManager: Pager mute modal button added.");
    }
    initializeAllUI() {
      this.injectStyles();
    }
  ...
  class PageObserver {
    constructor(options2 = {}) {
      ...
      __publicField(this, "onNewMediaItem");
      this.onContentChange = options2.onContentChange || (() => {
      });
      this.onNewMediaItem = options2.onNewMediaItem || (() => {
      });
      this.observer = null;
    }
    _processMutations(mutationList) {
      let contentMightHaveChanged = false;
      for (const mutation of mutationList) {
        if (mutation.type === "attributes" && mutation.attributeName === "data-intersection-key") {
          console.log("PageObserver: data-intersection-key changed.");
          contentMightHaveChanged = true;
        }
        if (mutation.type === "childList") {
          mutation.addedNodes.forEach((node) => {
            if (isElement(node)) {
              if (node.matches(MEDIA_ITEM_SELECTOR)) {
                this.onNewMediaItem(node);
              }
            }
          });
        }
      }
      if (contentMightHaveChanged) {
        this.onContentChange();
      }
    }
    _observerCallback(mutationList) {
      this._processMutations(mutationList);
    }
    start() {
      if (this.observer) {
      ...
      const config = { childList: true, subtree: true, attributes: true, attributeFilter: ["data-intersection-key"] };
  ...
  const ImportExportUI = ({
    storageService,
    dispatchMuteChangeEvent,
    populateLists
  }) => {
    const { t } = useTranslation();
    const [visible, setVisible] = reactExports.useState(false);
    const [exportValue, setExportValue] = reactExports.useState("");
    const [importValue, setImportValue] = reactExports.useState("");
    const exportValueRef = React.useRef(null);
    React.useEffect(() => {
      if (exportValue && exportValueRef.current) {
        exportValueRef.current.select();
      }
    }, [exportValue]);
    ...
    const handleExport = () => {
      const mutedUsers = storageService.getMutedUsers();
      const mutedTags = storageService.getMutedTags();
      const settings2 = { mutedUsers, mutedTags };
      setExportValue(JSON.stringify(settings2, null, 2));
    };
    const handleImport = () => {
      if (!importValue.trim()) {
        alert(t("importNoData", "インポートするデータがありません"));
        return;
      }
      try {
        const settings2 = JSON.parse(importValue);
        if (typeof settings2 !== "object" || settings2 === null) throw new Error(t("importInvalidJson", "無効なJSON形式です"));
        const { mutedUsers, mutedTags } = settings2;
        if (!Array.isArray(mutedUsers) || !Array.isArray(mutedTags)) throw new Error(t("importInvalidFormat", "データ形式が正しくありません (mutedUsers または mutedTags が配列ではありません)"));
        if (!mutedUsers.every((user) => typeof user === "object" && user !== null && "id" in user && (user.name === null || typeof user.name === "string"))) throw new Error(t("importInvalidUsers", "mutedUsers のデータ形式が正しくありません"));
        if (!mutedTags.every((tag) => typeof tag === "string")) throw new Error(t("importInvalidTags", "mutedTags のデータ形式が正しくありません"));
        mutedUsers.forEach((user) => storageService.addMutedUser(user.id, user.name));
        mutedTags.forEach((tag) => storageService.addMutedTag(tag));
        populateLists();
        dispatchMuteChangeEvent({ type: "settings", action: "imported" });
        setImportValue("");
        alert(t("importSuccess", "設定がインポートされました"));
      } catch (error) {
        alert(t("importFailed", "インポートに失敗しました") + ": " + error.message);
      }
    };
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          id: PM_TOGGLE_IMPORT_EXPORT_BUTTON_ID,
          className: "pm-button pm-toggle-import-export-button",
          style: { width: "100%", display: "block", marginTop: 10, marginBottom: 15 },
          onClick: toggleVisibility,
          children: visible ? t("hideImportExport", "インポート/エクスポート設定を隠す") : t("showImportExport", "インポート/エクスポート設定を表示")
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          id: PM_IMPORT_EXPORT_CONTAINER_ID,
          className: `pm-import-export-container${visible ? " pm-sections-visible" : ""}`,
          style: { display: visible ? void 0 : "none" },
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: PM_EXPORT_SECTION_ID, className: "pm-modal-section", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: t("export", "エクスポート") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "textarea",
                {
                  id: PM_EXPORT_TEXT_AREA_ID,
                  className: "pm-export-text-area",
                  readOnly: true,
                  rows: 5,
                  style: { width: "100%", boxSizing: "border-box" },
                  placeholder: t("exportPlaceholder", "「エクスポート」ボタンを押すとここに設定が表示されます"),
                  value: exportValue,
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  id: PM_EXPORT_BUTTON_ID,
                  className: "pm-button pm-export-button",
                  style: { marginTop: 5 },
                  onClick: handleExport,
                  children: t("export", "エクスポート")
                }
              )
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: PM_IMPORT_SECTION_ID, className: "pm-modal-section", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: t("import", "インポート") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "textarea",
                {
                  id: PM_IMPORT_TEXT_AREA_ID,
                  className: "pm-import-text-area",
                  rows: 5,
                  style: { width: "100%", boxSizing: "border-box" },
                  placeholder: t("importPlaceholder", "ここにエクスポートした設定データを貼り付けてください"),
                  value: importValue,
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  id: PM_IMPORT_BUTTON_ID,
                  className: "pm-button pm-import-button",
                  style: { marginTop: 5 },
                  onClick: handleImport,
                  children: t("importRun", "インポート実行")
                }
              )
            ] })
          ]
  ...
  const MuteModal = ({ storageService }) => {
    const { t } = useTranslation();
    const [show, setShow] = reactExports.useState(false);
    const [suggestions, setSuggestions] = reactExports.useState({ users: [], tags: [] });
    const [muted2, setMuted] = reactExports.useState({ users: [], tags: [] });
    const extractCandidates = () => {
      const mutedUsers = storageService.getMutedUsers();
      const mutedTags = storageService.getMutedTags();
      const mutedUserIds = new Set(mutedUsers.map((u) => u.id));
      const mutedTagSet = new Set(mutedTags);
      const allMediaElements = document.querySelectorAll(MEDIA_ITEM_SELECTOR);
      const userMap = /* @__PURE__ */ new Map();
      const tagSet = /* @__PURE__ */ new Set();
      allMediaElements.forEach((mediaElement) => {
        const userInfo = getUserInfo(mediaElement);
        if (userInfo.userId && !mutedUserIds.has(userInfo.userId)) {
          userMap.set(userInfo.userId, { userId: userInfo.userId, userName: userInfo.userName });
        }
        const tags = getItemTags(mediaElement);
        tags.forEach((tag) => {
          if (tag && !mutedTagSet.has(tag)) {
            tagSet.add(tag);
          }
        });
      });
    ...
    const refreshLists = () => {
      ...
      setSuggestions(candidates2);
      setMuted({ users: storageService.getMutedUsers(), tags: storageService.getMutedTags() });
    };
    const dispatchMuteChangeEvent = (detail) => {
      document.dispatchEvent(new CustomEvent(EVENT_MUTE_STATE_CHANGED, { detail }));
    };
    const handleShow = () => {
      refreshLists();
    ...
    reactExports.useEffect(() => {
      ...
      document.addEventListener(EVENT_REQUEST_TAG_MODAL_SHOW, handler);
      return () => document.removeEventListener(EVENT_REQUEST_TAG_MODAL_SHOW, handler);
    }, []);
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: MUTE_TAG_MODAL_ID, className: `pm-ui-element pm-modal pm-mute-settings-modal ${show && "show"}`, onClick: (e) => {
      if (e.target === e.currentTarget) handleHide();
    }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: MUTE_TAG_MODAL_ID + "-content", className: "pm-ui-element pm-modal-content", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: t("title") }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pm-modal-section", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: t("candidates", "候補") }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("ul", { id: PM_SUGGESTIONS_LIST_ID, className: "pm-toggle-list", children: [
          suggestions.users.map((user) => /* @__PURE__ */ jsxRuntimeExports.jsx(ToggleListItem, { item: user, type: "user", isMuted: false, storageService, dispatchMuteChangeEvent, refreshLists }, user.userId)),
          suggestions.tags.map((tag) => /* @__PURE__ */ jsxRuntimeExports.jsx(ToggleListItem, { item: tag, type: "tag", isMuted: false, storageService, dispatchMuteChangeEvent, refreshLists }, tag)),
          suggestions.users.length === 0 && suggestions.tags.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("li", { style: { justifyContent: "center", color: "#777" }, children: t("noCandidates", "ミュート可能な候補はありません") })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pm-modal-section", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: t("muted", "設定中") }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("ul", { id: PM_CURRENTLY_MUTED_LIST_ID, className: "pm-toggle-list", children: [
          muted2.users.map((user) => /* @__PURE__ */ jsxRuntimeExports.jsx(ToggleListItem, { item: { userId: user.id, userName: user.name }, type: "user", isMuted: true, storageService, dispatchMuteChangeEvent, refreshLists }, user.id)),
          muted2.tags.map((tag) => /* @__PURE__ */ jsxRuntimeExports.jsx(ToggleListItem, { item: tag, type: "tag", isMuted: true, storageService, dispatchMuteChangeEvent, refreshLists }, tag)),
          muted2.users.length === 0 && muted2.tags.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("li", { style: { justifyContent: "center", color: "#777" }, children: t("noMuted", "ミュート中の項目はありません") })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        ImportExportUI,
  ...
  const ToggleListItem = (props) => {
    ...
    let itemName = null;
    if (props.type === "user") {
      displayText = props.item.userName ? `${props.item.userName} (ID: ${props.item.userId})` : `ID: ${props.item.userId}`;
      itemId = props.item.userId;
      itemName = props.item.userName;
    } else {
      displayText = `#${props.item}`;
      itemId = props.item;
    }
    if (!itemId) return null;
    const [checked, setChecked] = React.useState(props.isMuted);
    const handleChange = () => {
      if (!itemId) return;
      if (!checked) {
        if (props.type === "user") {
          props.storageService.addMutedUser(itemId, itemName);
          props.dispatchMuteChangeEvent({ type: "user", action: "muted", userId: itemId, userName: itemName });
        } else {
          props.storageService.addMutedTag(itemId);
          props.dispatchMuteChangeEvent({ type: "tag", action: "added", tag: itemId });
        }
      } else {
        if (props.type === "user") {
          props.storageService.removeMutedUser(itemId);
          props.dispatchMuteChangeEvent({ type: "user", action: "unmuted", userId: itemId });
        } else {
          props.storageService.removeMutedTag(itemId);
          props.dispatchMuteChangeEvent({ type: "tag", action: "removed", tag: itemId });
        }
      }
      setChecked(!checked);
      props.refreshLists();
    };
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("li", { className: "pm-clickable-list-item", onClick: handleChange, style: { cursor: "pointer" }, children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "checkbox",
          checked,
          "aria-label": `Mute toggle for ${displayText}`,
          onChange: (e) => {
            e.stopPropagation();
            handleChange();
          },
  ...
  const title$1 = "Pixiv Muter";
  const mute$1 = "Mute";
  const unmute$1 = "Unmute";
  const settings$1 = "Settings";
  const save$1 = "Save";
  const cancel$1 = "Cancel";
  const candidates$1 = "Candidates";
  const muted$1 = "Muted";
  const noCandidates$1 = "No candidates available for mute";
  const noMuted$1 = "No muted items";
  const showImportExport$1 = "Show Import/Export Settings";
  const hideImportExport$1 = "Hide Import/Export Settings";
  const importRun$1 = "Run Import";
  const exportPlaceholder$1 = "Settings will appear here after pressing the Export button";
  const importPlaceholder$1 = "Paste exported settings data here";
  const importNoData$1 = "No data to import";
  const importInvalidJson$1 = "Invalid JSON format";
  const importInvalidFormat$1 = "Invalid data format (mutedUsers or mutedTags is not an array)";
  const importInvalidUsers$1 = "Invalid format for mutedUsers";
  const importInvalidTags$1 = "Invalid format for mutedTags";
  const importSuccess$1 = "Settings imported successfully";
  const importFailed$1 = "Import failed";
  const en = {
    title: title$1,
    mute: mute$1,
    unmute: unmute$1,
    ...
    noMuted: noMuted$1,
    showImportExport: showImportExport$1,
    hideImportExport: hideImportExport$1,
    "export": "Export",
    "import": "Import",
    importRun: importRun$1,
    exportPlaceholder: exportPlaceholder$1,
    importPlaceholder: importPlaceholder$1,
    importNoData: importNoData$1,
    importInvalidJson: importInvalidJson$1,
    importInvalidFormat: importInvalidFormat$1,
    importInvalidUsers: importInvalidUsers$1,
    importInvalidTags: importInvalidTags$1,
    importSuccess: importSuccess$1,
    importFailed: importFailed$1
  };
  const title = "Pixiv Muter";
  const mute = "ミュート";
  const unmute = "ミュート解除";
  const settings = "設定";
  const save = "保存";
  ...
  const candidates = "候補";
  const muted = "設定中";
  const noCandidates = "ミュート可能な候補はありません";
  const noMuted = "ミュート中の項目はありません";
  const showImportExport = "インポート/エクスポート設定を表示";
  const hideImportExport = "インポート/エクスポート設定を隠す";
  const importRun = "インポート実行";
  const exportPlaceholder = "「エクスポート」ボタンを押すとここに設定が表示されます";
  const importPlaceholder = "ここにエクスポートした設定データを貼り付けてください";
  const importNoData = "インポートするデータがありません";
  const importInvalidJson = "無効なJSON形式です";
  const importInvalidFormat = "データ形式が正しくありません (mutedUsers または mutedTags が配列ではありません)";
  const importInvalidUsers = "mutedUsers のデータ形式が正しくありません";
  const importInvalidTags = "mutedTags のデータ形式が正しくありません";
  const importSuccess = "設定がインポートされました";
  const importFailed = "インポートに失敗しました";
  const ja = {
    title,
    mute,
    unmute,
  ...
  window.addEventListener("load", () => {
    if (document.getElementById("spa-contents") === null) return;
    console.log("Window loaded. Initializing Pixiv Filter...");
    let reactRoot = document.getElementById("pixiv-muter-react-root");
    if (!reactRoot) {
      reactRoot = document.createElement("div");
      reactRoot.id = "pixiv-muter-react-root";
      document.body.appendChild(reactRoot);
    }
    const storageService = new StorageService();
    const contentProcessor = new ContentProcessor(storageService);
    const uiManager = new UIManager();
    const addMuteModalButton = () => {
      if (/^https:\/\/www\.pixiv\.net\/tags\//.test(location.href)) {
        uiManager.addMuteModalButtonToPagerNav();
      }
    };
    uiManager.initializeAllUI();
    ReactDOM.createRoot(reactRoot).render(
      /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(MuteModal, { storageService }) })
    );
    contentProcessor.processAllMediaItems();
    addMuteModalButton();
    const pageObserver = new PageObserver({
      ...
      onNewMediaItem: (mediaElement) => {
        contentProcessor.processSingleMediaItem(mediaElement);
      }
    });
    pageObserver.start();
  });
