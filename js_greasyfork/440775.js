// ==UserScript==
// @name         Nub Navy - Assists
// @namespace    kivou-nub
// @version      1.3
// @grant        GM_addStyle
// @description  Sends assist calls to the Nub Navy discord server
// @author       Kivou [2000607]
// @grant        GM.xmlHttpRequest
// @match        https://www.torn.com/loader.php?sid=attack*
// @run-at       document-end
// @license      WTFPL
// @downloadURL https://update.greasyfork.org/scripts/440775/Nub%20Navy%20-%20Assists.user.js
// @updateURL https://update.greasyfork.org/scripts/440775/Nub%20Navy%20-%20Assists.meta.js
// ==/UserScript==
...
const DISCORD_SERVER_ID = "432226682506575893"
const WEBSOCKET_URL = "https://torn.yata.yt/apiflkmizbkdzmwp"
const LOCAL_PORT = "8467"

// factions
const FACTIONS = ["nub navy"]
...
if($("#dark-mode-state")[0].checked) {
    console.log('[nub assists script] dark mode')
    ftcolor_a = '#eee'
    ftcolor_b = '#999'
    bgcolor_a = '#555'
    bgcolor_b = '#333'
...
const change_message = (txt, type)=>{
    $("#nub-result").removeClass("valid error warning").addClass(type);
    $("#nub-result").html(txt);
}
...
(function() {
    'use strict';
    let html = `
        <div id="nub-assist" class="nub-assist m-bottom10">
            Nub Navy\'s discord assist for
            <select id="nub-number">
    ...
    html += `
            ...
            member<span id="nub-member-plural">s</span>
            <span id="nub-call" class="nub-call">CALL</span>
            <span id="nub-result" class="nub-result"></span>
            <div class="clear"></div>
        </div>
    `;
    ...
    document.querySelector('#nub-number').addEventListener("change", function() {
        if (this.value == "1") {
            $("#nub-member-plural").html("")
        }else{
            $("#nub-member-plural").html("s")
        }
    });
    ...
    $("#nub-call").on('click', e => {
        ...
        if(dt < RATELIMIT){
            // ratelimit
            console.log(`[nub assists script] Delay between calls: ${dt}s`)
            ...
            data.faction = $('#nub-faction')[0].value
            data.number = $('#nub-number')[0].value
