// ==UserScript==
// @name         Технический Специалист MAKHACHKALA | №42
// @namespace    http://tampermonkey.net/
// @version      1.5
// @description  для  MAKHACHKALA | №42
// @author       Taylor Extazzy
// @match        https://forum.blackrussia.online/*
// @include      https://forum.blackrussia.online/threads/
// @grant        none
// @license  MIT
// @collaborator Taylor Extazzy
// @icon https://i.postimg.cc/sX3j7X8K/93-QIj-u-Mot-F-q-P2-B3-WJKj-Qd-SBZRYPqys-Ci-Qv-V-m-Jf-Ha-rt-G0brf-GKBA1-PWQIln-OI3-Hf-RJp-YXUYn-Tzh-Fjj-Pk502-Iq.jpg
// @downloadURL https://update.greasyfork.org/scripts/523061/%D0%A2%D0%B5%D1%85%D0%BD%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B9%20%D0%A1%D0%BF%D0%B5%D1%86%D0%B8%D0%B0%D0%BB%D0%B8%D1%81%D1%82%20MAKHACHKALA%20%7C%20%E2%84%9642.user.js
// @updateURL https://update.greasyfork.org/scripts/523061/%D0%A2%D0%B5%D1%85%D0%BD%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B9%20%D0%A1%D0%BF%D0%B5%D1%86%D0%B8%D0%B0%D0%BB%D0%B8%D1%81%D1%82%20MAKHACHKALA%20%7C%20%E2%84%9642.meta.js
// ==/UserScript==
  ...
  const TRANSFER_PREFIXXXXXX = 24 ; //  префикс передачи в жб на тех
	...
	const NO_PREFIX = 0; // (Без префикса)
...

{
	title: 'Расписываем',
	content:
	'[SIZE=4][FONT=Times New Roman][CENTER][COLOR=rgb(255, 0, 0)]Доброго времени суток, уважаемый[B][/COLOR] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	'[CENTER]Вы получили блокировку за нарушение [COLOR=rgb(255, 0, 0)]пункта правил[/COLOR]:<br><br>' +
	'[SPOILER="пункт "]<br>'+
	'[/SPOILER]<br><br>'+
	'[COLOR=rgb(255, 0, 0)]А именно из-за данных действий:[/COLOR][/CENTER]<br><br>' +
	'[TABLE][TR][TD][CENTER][/CENTER][/TD][TD][CENTER][/CENTER][/TD][/TR][TR][TD][CENTER][/CENTER][/TD][TD][CENTER][/CENTER][/TD][/TR][/TABLE]<br><br>[CENTER]'+
    '[COLOR=rgb(255, 0, 0)]Поясню причину и суть вашей блокировки:[/COLOR]<br><br>'+
	'[COLOR=rgb(255, 0, 0)]Передано Куратору Технических специалистов / Заместителю Куратора Технических Специалистов[/COLOR] для выноса окончательного вердикта.<br><br>' +
...
},
{
	...
	content:
	'[SIZE=4][FONT=Times New Roman][CENTER][COLOR=rgb(255, 0, 0)]Доброго времени суток, уважаемый[B][/COLOR] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	"[CENTER]Данная тема является [COLOR=rgb(255, 0, 0)]<u>дубликатом вашей предыдущей темы, ссылка на тему - </u>[/COLOR].<br>Пожалуйста, [COLOR=rgb(255, 0, 0)]<u><b>прекратите создавать идентичные или похожие темы[/COLOR] - иначе Ваш форумный аккаунт может быть [COLOR=rgb(255, 0, 0)]заблокирован</b></u>.[/COLOR]<br><br>" +
...
},
{
	...
	'[SIZE=4][FONT=Times New Roman][CENTER]<u>Создавать подобные темы не нужно</u>.<br>[CENTER][COLOR=rgb(255, 255, 0)]На рассмотрении.[/CENTER]',
	prefix: PIN_PREFIX,
	status: true,
},
{
...
},
{
	...
	content:
	'[SIZE=4][FONT=Times New Roman][CENTER][COLOR=rgb(255, 0, 0)]Доброго времени суток, уважаемый[B][/COLOR] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	"[CENTER]Пожалуйста, убедительная просьба, ознакомиться с назначением данного раздела в котором Вы создали тему, так как Ваш запрос не относится к жалобам на технических специалистов.<br>Что принимается в данном разделе:<br>Жалобы на технических специалистов, оформленные по форме подачи и не нарушающие правила подачи:<br> [FONT=Times New Roman][SIZE=7][COLOR=rgb(226, 80, 65)][U]Правила подачи жалобы на технических специалистов[/U][/COLOR][/SIZE][/FONT][/CENTER]<br><br>[QUOTE][FONT=Times New Roman][SIZE=4][COLOR=rgb(226, 80, 65)]01.[/COLOR] Ваш игровой никнейм:[FONT=Times New Roman][SIZE=4][COLOR=rgb(226, 80, 65)]<br>02.[/COLOR] Игровой никнейм технического специалиста:[COLOR=rgb(226, 80, 65)]<br>03.[/COLOR] Сервер, на котором Вы играете:[COLOR=rgb(226, 80, 65)]<br>04.[/COLOR] Описание ситуации (описать максимально подробно и раскрыто):[COLOR=rgb(226, 80, 65)]<br>05.[/COLOR] Любые скриншоты, которые могут помочь в решении проблемы (если таковые имеются):[COLOR=rgb(226, 80, 65)]<br>06.[/COLOR] Дата и время произошедшей технической проблемы (постарайтесь указать максимально точно):[/SIZE][/FONT][/SIZE][/QUOTE]<br><br>[FONT=Times New Roman][SIZE=4][FONT=Times New Roman][SIZE=4][COLOR=rgb(226, 80, 65)]Примечание:[/COLOR] все оставленные заявки обращения в данный раздел обязательно должны быть составлены по шаблону предоставленному немного выше.<br>В ином случае, заявки обращения в данный раздел составленные не по форме — будут отклоняться.<br>Касательно названия заголовка темы — четких правил нет, но, желательно чтобы оно содержало лишь никнейм технического специалиста и причину.<br>Заранее, настоятельно рекомендуем ознакомиться [U][B][URL='https://forum.blackrussia.online/index.php?forums/faq.231/']с данным разделом[/URL][/B][/U][/SIZE][/FONT][/SIZE][/FONT]<br>[CENTER][FONT=Times New Roman][SIZE=7][COLOR=rgb(226, 80, 65)][U]Какие жалобы не проверяются?[/U][/COLOR][/SIZE][/FONT][/CENTER]<br><br>[FONT=Times New Roman][SIZE=4][COLOR=rgb(226, 80, 65)]—[/COLOR] Если в содержании темы присутствует оффтоп/оскорбления.<br>[COLOR=rgb(226, 80, 65)]—[/COLOR] С момента выдачи наказания прошло более 14 дней.[/SIZE][/FONT]<br>" +
	'[CENTER][COLOR=rgb(255, 0, 0)]Закрыто.[/COLOR][/CENTER][/FONT][/SIZE]',
	prefix: CLOSE_PREFIX,
	status: false,
},
...
},
{
	...
	content:
	'[SIZE=4][FONT=Times New Roman][CENTER][COLOR=rgb(255, 0, 0)]Доброго времени суток, уважаемый[B][/COLOR] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	"[CENTER][B][COLOR=rgb(255, 255, 255)][FONT=Times New Roman]1. Укажите ваш Telegram ID, если ваш игровой аккаунт был привязан к Telegram. Узнать его можно здесь: t.me/getmyid_bot<br>[/FONT][/COLOR][/B][COLOR=rgb(255, 255, 255)][FONT=Times New Roman][B]2. Укажите ваш оригинальный ID страницы ВКонтакте, которая привязана к аккаунту (взять его можно через данный сайт - https://regvk.com/ )<br>[/B][/FONT][/COLOR][B][COLOR=rgb(255, 255, 255)][FONT=Times New Roman]3. Укажите почту, которая привязана к аккаунту[/FONT][/COLOR][/B][/CENTER]<br><br>" +
	"[CENTER][I][COLOR=rgb(247, 218, 100)][FONT=Times New Roman]Ожидаю ваш ответ.[/FONT][/COLOR][/I][/CENTER]",
  prefix: TECHADM_PREFIX,
	status: true,
},
{
	...
	content:
	'[SIZE=4][FONT=Times New Roman][CENTER][COLOR=rgb(255, 0, 0)]Доброго времени суток, уважаемый[B][/COLOR] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	"[CENTER][SIZE=4][FONT=Times New Roman]После проверки доказательств и системы логирования вердикт:<br><br>[COLOR=rgb(65, 168, 95)][FONT=Times New Roman]Игрок будет заблокирован<br><br>[/COLOR]Спасибо за обращение[/CENTER]<br><br>" +
	"[CENTER][SIZE=4][FONT=Times New Roman][I][COLOR=rgb(0, 255, 0)]Одобрено[/COLOR][/I][/FONT][/CENTER]",
...
},
{
	...
	"[CENTER][SIZE=4][FONT=Times New Roman][I][COLOR=rgb(255, 0, 0)]Отказано[/COLOR][/I][/FONT][/CENTER]",
...
},
{
	...
	"[CENTER]📹 Включение продемонстрировано на видео: https://youtu.be/Wft0j69b9dk<br>[CENTER][COLOR=rgb(255, 0, 0)]Закрыто[/COLOR][/CENTER][/FONT][/SIZE]",
	prefix: CLOSE_PREFIX,
	status: false,
},
{
...
},
{
	...
	content:
	'[SIZE=4][FONT=Times New Roman][CENTER][COLOR=rgb(255, 0, 0)]Доброго времени суток, уважаемый[B][/COLOR] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	'[CENTER]Если вы выставили свои вещи на аукцион а их никто не купил, то воспользуйтесь командой [COLOR=rgb(251, 160, 38)]/reward[/COLOR]<br> В случае отсутствии вещей там, приложите скриншоты с + /time в новой теме<br>[CENTER][I][COLOR=rgb(255, 0, 0)]Закрыто[/COLOR][/I][/CENTER][/FONT][/SIZE]',
	prefix: CLOSE_PREFIX,
	status: false,
},
{
...
},
{
  ...
  "[CENTER][IMG width=695px]https://i.postimg.cc/mrhcH5vR/1621526767066.png[/IMG]<br>"+
  "[COLOR=rgb(209, 213, 216)][FONT=Arial][size=15px][CENTER]Доброго времени суток, уважаемый(-ая) {{ user.mention }}[/CENTER]<br><br>" +
  '[CENTER][B][COLOR=rgb(255, 255, 255)][FONT=Times New Roman]К сожалению, обнаружена чужая привязка к вашему аккаунту, что может представлять угрозу для безопасности данных. В данной ситуации нам, к сожалению, не удастся разблокировать ваш аккаунт. Для обеспечения безопасности рекомендуется создать новый аккаунт и принять все возможные меры по защите данных. Наша команда технических специалистов настоятельно рекомендует вам установить дополнительные меры безопасности, такие как двухфакторную аутентификацию, сложные пароли и регулярное обновление паролей.<br>[/FONT][/COLOR][/B][COLOR=rgb(255, 255, 255)][FONT=Times New Roman][B]Пожалуйста, будьте внимательны при создании нового аккаунта и храните данные доступа в надежном месте.[/B][/FONT][/COLOR][B][COLOR=rgb(255, 255, 255)][FONT=Times New Roman]Мы приносим извинения за возможные неудобства, связанные с этой ситуацией, и стараемся обеспечить безопасность всех наших пользователей.[/FONT][/COLOR][/B][/CENTER]<br><br>' +
	'[CENTER][COLOR=rgb(255, 0, 0)]Закрыто.[/COLOR][/CENTER][/FONT][/SIZE]',
	prefix: CLOSE_PREFIX,
	status: false,
},
{
  ...
  "[CENTER][IMG width=695px]https://i.postimg.cc/mrhcH5vR/1621526767066.png[/IMG]<br>"+
  "[COLOR=rgb(209, 213, 216)][FONT=Arial][size=15px][CENTER]Доброго времени суток, уважаемый(-ая) {{ user.mention }}[/CENTER]<br><br>" +
  '[CENTER][B][FONT=Times New Roman]Компенсация и возврат имущества после взлома аккаунта невозможны. За безопасность аккаунта отвечаете лично вы, следовательно за потерянное имущество компенсация согласно общему регламенту правил, а также правил восстановлений имущества не подлежит возврату. Технические специалисты лишь отслеживают переданное имущество на аккаунт злоумышленника и фиксируют нарушение.[/FONT][/B][/CENTER]<br><br>' +
	'[CENTER][COLOR=rgb(255, 0, 0)]Закрыто.[/COLOR][/CENTER][/FONT][/SIZE]',
	prefix: CLOSE_PREFIX,
	status: false,
},
{
  ...
  dpstyle: 'oswald: 3px;     color: #fff; background: #db2309; box-shadow: 0 0 2px 0 rgba(0,0,0,0.14),0 2px 2px 0 rgba(0,0,0,0.12),0 1px 3px 0 rgba(0,0,0,0.2); border: none; border-color: #f53317',
        ...
        {
	title: 'В жалобы  на адм 42',
	color: '',
	content:
	'[SIZE=4][FONT=Times New Roman][CENTER][COLOR=rgb(255, 0, 0)]Доброго времени суток, уважаемый[B][/COLOR] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	'[CENTER][COLOR=rgb(209, 213, 216)][FONT=Arial][size=15px]Вы ошиблись разделом. Переношу вашу тему в раздел [COLOR=rgb(255,207,64)]"Жалобы на администрацию"[/COLOR] сервера [COLOR=rgb(0, 255, 127)]MAKHACHKALA | №42. [/color][/center]',
	prefix: TRANSFER_PREFIXX,
	status: false,
},
        {
	title: 'В обжалование наказаний 42',
	color: '',
	content:
	'[SIZE=4][FONT=Times New Roman][CENTER][COLOR=rgb(255, 0, 0)]Доброго времени суток, уважаемый[B][/COLOR] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	'[CENTER][COLOR=rgb(209, 213, 216)][FONT=Arial][size=15px]Вы ошиблись разделом. Переношу вашу тему в раздел [COLOR=rgb(255,207,64)]"Обжалование наказаний"[/COLOR] сервера [COLOR=rgb(0, 255, 127)]MAKHACHKALA | №42. [/color][/center]',
	prefix: TRANSFER_PREFIXXX,
	status: false,
},
       {
	title: 'В тех раздел 42',
	color: '',
	content:
	'[SIZE=4][FONT=Times New Roman][CENTER][COLOR=rgb(255, 0, 0)]Доброго времени суток, уважаемый[B][/COLOR] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	'[CENTER][COLOR=rgb(209, 213, 216)][FONT=Arial][size=15px]Вы ошиблись разделом. Переношу вашу тему в раздел [COLOR=rgb(255,207,64)]"Технический раздел"[/COLOR] сервера [COLOR=rgb(0, 255, 127)]MAKHACHKALA | №42. [/color][/center]',
	prefix: TRANSFER_PREFIXXXX,
	status: false,
},
{
	title: 'В жалобы на тех 42',
	color: '',
	content:
	'[SIZE=4][FONT=Times New Roman][CENTER][COLOR=rgb(255, 0, 0)]Доброго времени суток, уважаемый[B][/COLOR] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	'[CENTER][COLOR=rgb(209, 213, 216)][FONT=Arial][size=15px]Вы ошиблись разделом. Переношу вашу тему в раздел [COLOR=rgb(255,207,64)]"Жалобы на технических специалистов"[/COLOR] сервера [COLOR=rgb(0, 255, 127)]MAKHACHKALA | №42. [/color][/center]',
	prefix: TRANSFER_PREFIXXXXXX,
	status: false,
},
	{
	title: 'В жб на игроков 42',
	color: '',
	content:
	'[SIZE=4][FONT=Times New Roman][CENTER][COLOR=rgb(255, 0, 0)]Доброго времени суток, уважаемый[B][/COLOR] {{ user.mention }}[/B]![/CENTER]<br><br>' +
	'[CENTER][COLOR=rgb(209, 213, 216)][FONT=Arial][size=15px]Вы ошиблись разделом. Переношу вашу тему в раздел [COLOR=rgb(255,207,64)]"Жалобы на игроков"[/COLOR] сервера [COLOR=rgb(0, 255, 127)]MAKHACHKALA | №42. [/color][/center]',
	...
	addButton('На рассмотрение', 'pin','border-radius: 13px; margin-right: 5px; border: 2px solid; border-color: rgb(105, 105, 105, 0.5);');
...
(function() {
    ...
    ButtonRep.textContent = "Правила Серверов";
    ButtonWarning.textContent = "Технический Раздел";
    ButtonWarningP.textContent = "Жалобы на ТС";
    ButtonNakaz.textContent = "Жалобы на Игроков";
    ...
    function BWarning() {
        window.location.href = "https://forum.blackrussia.online/forums/%D0%A2%D0%B5%D1%85%D0%BD%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B9-%D1%80%D0%B0%D0%B7%D0%B4%D0%B5%D0%BB-makhachkala.1884/";
    };

    function BWarningP() {
        window.location.href = "https://forum.blackrussia.online/forums/%D0%A1%D0%B5%D1%80%D0%B2%D0%B5%D1%80-%E2%84%9642-makhachkala.1883/";

};

    function BNakaz() {
        window.location.href = "https://forum.blackrussia.online/forums/%D0%96%D0%B0%D0%BB%D0%BE%D0%B1%D1%8B-%D0%BD%D0%B0-%D0%B8%D0%B3%D1%80%D0%BE%D0%BA%D0%BE%D0%B2.1912/";
    };


    ButtonRep.addEventListener("click", () => {
        BRep();
        ButtonRep.style.background = "#FF0000"
    });

    ButtonWarning.addEventListener("click", () => {
        BWarning();
        ButtonWarning.style.background = "#FF0000"
    });

    ButtonWarningP.addEventListener("click", () => {
        BWarningP();
        ButtonWarningP.style.background = "#FF0000"
    });

    ButtonNakaz.addEventListener("click", () => {
        BNakaz();
        ButtonNakaz.style.background = "#FF0000"
    });
...
function countElements() {
  ...
  // Получаем все элементы с классом 'structItem structItem--thread is-prefix10'
  var elements2 = document.querySelectorAll('.structItem.structItem--thread.is-prefix10');
     // Получаем все элементы с классом 'structItem structItem--thread is-prefix13'
  var elements3 = document.querySelectorAll('.structItem.structItem--thread.is-prefix13');
     // Получаем все элементы с классом 'structItem structItem--thread is-prefix2'
  var elements4 = document.querySelectorAll('.structItem.structItem--thread.is-prefix2');
     // Получаем все элементы с классом 'structItem structItem--thread is-prefix21'
  var elements5 = document.querySelectorAll('.structItem.structItem--thread.is-prefix0');
  ...
  if (filterBar) {
    ...
    filterBar.insertAdjacentElement('beforebegin', createCountElement('.structItem.structItem--thread.is-prefix14', count1, 'Темы на Ожидании'));
       filterBar.insertAdjacentElement('beforebegin', createCountElement('.structItem.structItem--thread.is-prefix10', count2, 'Темы Команде Проекта'));
      filterBar.insertAdjacentElement('beforebegin', createCountElement('.structItem.structItem--thread.is-prefix13', count3, 'Темы Логисту'));
    filterBar.insertAdjacentElement('beforebegin', createCountElement('.structItem.structItem--thread.is-prefix2', count4, 'Темы на Рассмотрении'));
      filterBar.insertAdjacentElement('beforebegin', createCountElement('.structItem.structItem--thread.is-prefix0', count5, 'Темы без Префикса'));
