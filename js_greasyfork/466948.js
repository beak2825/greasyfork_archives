...
// @version      1.1
// @description  Scrapes all Btv leagues in current open league
// @author       Beat2er
...
// @downloadURL https://update.greasyfork.org/scripts/466948/BTV%20Scraper.user.js
// @updateURL https://update.greasyfork.org/scripts/466948/BTV%20Scraper.meta.js
// ==/UserScript==
...
(function() {
    ...
    function btv_scraper_main() {
        ...
        let league_name3_doms = jQuery('.step.previous.z-div> .step-label.z-label');
        league_name3_doms = league_name3_doms.slice(0, 4);
        ...
        let i=0;
        jQuery(league_name3_doms).each(function() {
            league_name += " " + jQuery(this).text().trim().replace(/[\(\)]/g, "");
            if (i++ % 2 == 0) league_name += ':';
        });

        league_name = league_name.trim();
        league_name = [...new Set(league_name.split(" "))].join(" ");
        league_name = league_name.replace("Archiv ", "");
        league_name = league_name.replace("Aktuelle Saison ", "");
    ...
    async function btv_scraper_loop_scrape_ids(ids_list, data, league_name) {
        for (let i = 0; i < ids_list.length; i++) {
            await btv_scraper_screape_id(ids_list[i], data, ids_list[ids_list.length - 1]);
        }
        //sort
        data = Object.keys(data).sort().reduce(function (result, key) {
            result[key] = data[key];
            return result;
          }, {});
    ...
    function btv_scraper_create_table(data, league_name) {
        ...
        for (const [key, value] of Object.entries(data)) {
            ...
            value['teams'].forEach(element => {
                if (first_team)
                    t += "<tr><td rowspan='" + value['teams'].length + "'><a href='" + value['link'] + "'>" + key + "</a></td><td>" + element + "</td></tr>";
                else
                    t += "<tr><td>" + element + "</td></tr>";
                first_team = false;
            });
             t += "<td bgcolor='white' style='line-height:10px;' colspan='2'>&nbsp;</td>";
        }
    ...
    async function btv_scraper_screape_id(id, data, max_nr) {
        console.log('scraping id ' + id + '/' + max_nr);
        let ids_dom = btv_scraper_get_ids_dom();
