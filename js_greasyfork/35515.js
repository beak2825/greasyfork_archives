...
// @downloadURL https://update.greasyfork.org/scripts/35515/Blank%20Contacts.user.js
// @updateURL https://update.greasyfork.org/scripts/35515/Blank%20Contacts.meta.js
// ==/UserScript==
...
if ($('li:contains("Call")').length) {
    ...
    number.append('<br>');
    createButton('Yes (Direct) NT',0,0,0,2,0,'');
    createButton('Yes (Can Reach) NT',0,0,0,2,1,'');
    createButton('Yes (Cant Reach) NT',0,0,0,2,2,'');
    number.append('<br>');

    createButton('Generic VM',2,2,3,2,3,'Did not give indentifying information or data to answer questions');
    createButton('Personal VM',2,2,3,2,3,'Voice mail did identify the contact but no title information');
    createButton('Biz on Voicemail',0,0,3,2,3,'The business is listed on the voicemail but with no information pertaining to the contact');
    number.append('<br>');
    createButton('No',0,0,2,1,2,'This person does not work there, the person I spoke to was not aware of past employment');
    createButton('No (Previous)',0,0,1,1,2,'This person does not work there anymore');
    createButton('Wrong Biz',0,1,3,2,3,'This number connected to the wrong business');
    number.append('<br>');
    createButton('DC',4,2,3,2,3,'Disconnected - this number is not in service.');
    createButton('Fax',5,2,3,2,3,'Fax');
    createButton('Ringing',2,2,3,2,3,'Kept ringing','Kept ringing');
    createButton('Silence',2,2,3,2,3,'Rang until silence');
