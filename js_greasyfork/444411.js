...
// @grant        unsafeWindow
// @downloadURL https://update.greasyfork.org/scripts/444411/bmr4.user.js
// @updateURL https://update.greasyfork.org/scripts/444411/bmr4.meta.js
// ==/UserScript==
...
$.extend(G, config), function (t) {
        ...
        this.shengxiao = [], this.weishu = [], this.number = $("input[name=number]", this.d), this.number2 = $("input[type=radio]", this.d), this.form = $("form", this.d), this.form.validate({
            ...
            this.d.on("click", ".fn_check_items", function (t) {
                var e = this.getAttribute("t")
                    , i = this.checked
                    , n = this.getAttribute("titles");
                if (0 == e) {
                    if (i) {
                        if (9 < a.selected1.length)
                            return t.preventDefault(),
                                void $.alert("A区最多能选择10组号码");
                        a.selected1.push(n)
                    } else {
                    ...
                    if (i) {
                        if (9 < a.selected2.length)
                            return t.preventDefault(),
                                void $.alert("B区最多能选择10组号码");
                        a.selected2.push(n)
                    } else {
                        for (s = 0,
                                 o = a.selected2.length; s < o && a.selected2[s] != n; s++)
                            ;
            ...
            if (0 == e) {
                if (i) {
                    //任意对碰的选择号码
                    if (9 < a.selected1.length)
                        return t.preventDefault(), void $.alert("A区最多能选择10组号码");
                    // a.selected1.push(n)
                } else {
                    for (var s = 0, o = a.selected1.length; s < o && a.selected1[s] != n; s++) ;
                    a.selected1.splice(s, 1)
        ...
        if (e) {
            ...
            switch (o = {
                ...
                case"5":
                    ...
                    break;
                case"6":
                    window.bmrSelect1 = g.id;
                    window.bmrSelect2 = b.id;
                    o.GroupName = [g.id.join(","), b.id.join(",")];
                    o.BetItems = this.select;
                    this.countBetNumber(this.number, o, r, t, m);
                    $.each(g.id, function (t, i) {
                        $.each(b.id, function (t, e) {
                            i != e && (d[n = e < i ? e + "-" + i : i + "-" + e] || (o.List.push([r[i], r[e]]), d[n] = !0))
                        })
                    });
    ...
    if (bmr.length == 0) {
        ...
        $(".dialog-customft").append('<div id="bmrIndex1Dev1" style="width:100%;color:white">\n' +
            '    开启从指定组合开始 <input type="checkbox" id="bmrIndex1Checkbox1">\n' +
            '    <label id="bmrIndex1Label1" hidden="true">\n' +
            '        <input type="text" id="bmrIndex1Text1" placeholder="请输入组合" style="background:white;">\n' +
            '        <input type="button" id="bmrIndex1Button1" value="确定" style="background:white;">\n' +
            '    </label>\n' +
            '</div>');

        $('#bmrIndex1Checkbox1').click(function () {
            if (this.checked == true) {
                $('#bmrIndex1Label1').show();
            } else {
                $('#bmrIndex1Label1').hide();
                t.bmrAppoint = undefined;
            }
            $('#bmrIndex1Text1').val("");
        })

        $('#bmrIndex1Button1').click(function () {
            let bmrIndex1Text1Vale = $("#bmrIndex1Text1").val();
            /*let bmrIndex1Text1Array = bmrIndex1Text1Vale.split(",");
            for (let i = 0; i < bmrIndex1Text1Array.length; i++) {
                if(bmrIndex1Text1Array[i].substr(0, 1) == "0") {
                    bmrIndex1Text1Array[i] = bmrIndex1Text1Array[i].slice(1);// 删除第一个字符
                }
            }
            t.bmrAppoint = bmrIndex1Text1Array.join(",");*/
            t.bmrAppoint = bmrIndex1Text1Vale;
            alert(t.bmrAppoint)
        })
    }
    ...
    var subBetsObj = {};
    $.each(subBets, function (t, i) {
        subBetsObj[i.BetItems.join(",")] = i;
    });
    console.log(subBetsObj)
    ...
    /*else if (bmrCurType == "任意对碰") {
        alert("任意对碰");
        // let arr = [1, 2, 3, 4, 5, 6, 7, 8, 9]
        // console.log(spArr(arr, 4))
        console.log(window.bmrSelect1);
        console.log(window.bmrSelect2);

        let bmrSelect1SpArr = spArr(window.bmrSelect1.sort(), 10);
        let bmrSelect2SpArr = spArr(window.bmrSelect2.sort(), 10);

        console.log(bmrSelect1SpArr);
        console.log(bmrSelect2SpArr);
...
/!*        $.each(g.id, function (t, i) {
        ...
        for (let i = 0; i < bmrSelect1SpArr.length; i++) {

            for (let j = 0; j < bmrSelect1SpArr[i].length; j++) {


                // console.log(""+bmrSelect1SpArr[i][j]+"-"+bmrSelect2SpArr[k][l])

                for (let k = 0; k < bmrSelect2SpArr.length; k++) {

                    for (let l = 0; l < bmrSelect2SpArr[k].length; l++) {
        ...
        /!*        $.each(bmrSelect1SpArr, function (t, i) {
                    $.each(bmrSelect2SpArr, function (t, e) {
                        i != e && (d[n = e < i ? e + "-" + i : i + "-" + e] || (o.List.push([r[i], r[e]]), d[n] = !0))
                    })
                });*!/
    ...
    if (typeof (e.bmrAppoint) != "undefined" && $('#bmrIndex1Checkbox1')[0].checked) {
        for (let i = 0; i < allObj.length; i++) {
            /*        if (e.bmrAppoint != allObj[i][0].BetItems.join(",")) {
                        allObj.splice(i--, 1);
                    }else {
                        allObj.splice(i, 1);
                        break;
                    }*/
            if (e.bmrAppoint != allObj[i][0].BetName) {
                allObj.splice(i--, 1);
            } else {
                allObj.splice(i, 1);
                break;
    ...
    var myFunction = function () {
        ...
        e.postdata2(JSON.stringify(allObj[curIndex]), function (parmRes, bmrMessage) {
            ...
            if (resStatus == null) {
                ...
                appendStr(e.bmrTextArea, "投注失败。。。原因:" + bmrMessage);
            ...
            curIndex++;
            counter = Math.round(Math.random() * 2000 + 2000);
