...
// @downloadURL https://update.greasyfork.org/scripts/518302/Luna%20Tools.user.js
// @updateURL https://update.greasyfork.org/scripts/518302/Luna%20Tools.meta.js
// ==/UserScript==
...
(() => {
    ...
    function openMainSettingsModal() {
        ...
        <div class="modal-content">
            ...
            <div id="general-tab" class="tab-content active" style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; color:#fff; padding: 10px; font-size: 11px;">
                ...
                <div class="toggle-row">
                    <label>No Profile Cards (NEW):<i style="margin-left:5px;" class="fa-solid fa-circle-info tooltip-icon" data-placement="right" data-title="Hides profile cards when hovering names in chat."></i></label>
                    <div class="toggle-switch" id="toggle-pfcards"></div>
                </div>
            </div>
    ...
    `;
        ...
        initializeToggleSwitch(document.getElementById('toggle-pfcards'), settings.hidePFCards || false);
        ...
        document.getElementById('save-settings').addEventListener('click', () => {
            const newSettings = {
                ...
                hidePFCards: document.getElementById('toggle-pfcards').classList.contains('active')
            };
    ...
    function applyMainSettings(settings) {
        ...
        const checkModal = () => {
            ...
            const hideModalsStyle = `
                ...
                ${settings.hidePFCards ? '.user-card_user-card__nebD_ { display: none !important; }' : '.profile-card_profile-card__3b1a7 { display: flex !important; }'}
            `;
    ...
    const checkForUpdates = async () => {
        try {
            const targetUrl = 'https://update.greasyfork.org/scripts/518302/Luna%20Tools.meta.js';
    ...
    const displayUpdateToast = (latestVersion) => {
        ...
        toast.innerHTML = `
            ...
            <div class="custom-toast-icon">
                ...
                <span>New Luna Tools available: ${latestVersion} <a href="https://greasyfork.org/scripts/518302" target="_blank">Update now</a></span>
...
(() => {
    ...
    const ftAudio_styles = `
        ...
        #ft-audio-copy-btn:hover,
        #ft-audio-generate-btn:hover,
        #ft-audio-import-btn:hover {
            background-color: #a70a00;
        }
    ...
    <div class="ft-audio-control-group">
        <h3>Share / Import</h3>
        <div class="ft-control-row">
            <label for="ft-audio-share-code">Share Code:</label>
            <input type="text" id="ft-audio-share-code" placeholder="Share code will appear here" readonly style="width: 270px;">
            <button id="ft-audio-generate-btn">Generate</button>
            <button id="ft-audio-copy-btn">Copy</button>
        </div>
        <div class="ft-control-row">
            <label for="ft-audio-import-code">Import Code:</label>
            <input type="text" id="ft-audio-import-code" placeholder="Paste code to import" style="width: 270px;">
            <button id="ft-audio-import-btn">Import</button>
        </div>
    </div>
    ...
    function ftAudio_insertButton() {
        ...
        if (navButtons && camKeyButton && !document.getElementById('ft-audio-controls-btn')) {
            try {
                navButtons.appendChild(ftAudio_controlBtn);
    ...
    function ftAudio_init() {
        ...
        function tryInsertWithRetry() {
            ...
            if (retryCount < maxRetries) {
                setTimeout(tryInsertWithRetry, 1000);
            } else {
                console.warn("ftAudio: Failed to insert button after multiple retries.");
            }
        }
    ...
    function ftAudio_setupEventListeners() {
        ...
        ftAudio_controlPanel.querySelectorAll('input').forEach(input => {
            ...
            if (ftAudio_currentSettings.hasOwnProperty(settingKey)) {
                input.value = ftAudio_currentSettings[settingKey];
            }
            input.addEventListener('input', () => {
                const val = parseFloat(input.value);
                if (!isNaN(val)) {
                    ftAudio_currentSettings[settingKey] = val;
                    if (ftAudio_system) ftAudio_system.updateSettings(ftAudio_currentSettings);
                    ftAudio_saveSettings();
                }
            });
        ...
        const shareCodeInput = document.getElementById('ft-audio-share-code');
        const generateBtn = document.getElementById('ft-audio-generate-btn');
        const copyBtn = document.getElementById('ft-audio-copy-btn');
        const importCodeInput = document.getElementById('ft-audio-import-code');
        const importBtn = document.getElementById('ft-audio-import-btn');

        shareCodeInput.placeholder = "Share code will appear here";
        shareCodeInput.value = "";
        importCodeInput.placeholder = "Paste code to import";
        importCodeInput.value = "";

        function generateShareCode() {
            const minimized = {};
            for (const [shortKey, fullKey] of Object.entries(keyMap)) {
                minimized[shortKey] = ftAudio_currentSettings[fullKey];
            }
        ...
        generateBtn.addEventListener('click', () => {
            ...
            try {
                ...
                shareCodeInput.value = "Error generating code";
            }
        });
        ...
        importBtn.addEventListener('click', () => {
            ...
            if (!code) {
                alert("Paste a share code to import.");
                return;
            }
            try {
                ...
                const importedMin = JSON.parse(decompressed);
                const imported = {};

                for (const [shortKey, val] of Object.entries(importedMin)) {
                    if (keyMap[shortKey]) {
                        imported[keyMap[shortKey]] = val;
                    }
                }

                ftAudio_currentSettings = { ...ftAudio_defaultSettings, ...imported };

                Object.entries(ftAudio_currentSettings).forEach(([key, val]) => {
                    if (key === 'enabled') return;
                    const input = document.querySelector(`#ft-${key}`);
...
(() => {
    ...
    const mentionSound = new Audio('https://github.com/luna-mae/ClippingTools/blob/main/media/notification%20sound.mp3?raw=true');
    ...
    .mention-style {
        fill:rgb(185, 0, 0);
        filter: drop-shadow(0 0 2px rgba(153, 0, 0, 0.7));
    }
    .mention-style:hover {
        fill:rgb(255, 0, 0);
    ...
    function setHomeRightZIndex(isModalOpen) {
        ...
        function getCurrentUsername() {
            try {
                const usernameSpan = document.querySelector('.top-bar-user_display-name__bzlpw');
                if (usernameSpan) return usernameSpan.textContent.trim();
                
                const userData = JSON.parse(localStorage.getItem('ftUserData'));
        ...
        function logMention(messageElement) {
            const username = getCurrentUsername();
            if (!username) {
                console.log('Mention check: No username found');
                return;
            }

            const mentionSpans = messageElement.querySelectorAll('span[class*="mention__"]');
            if (!mentionSpans.length) return;

            let mentionFound = false;
            
            mentionSpans.forEach(span => {
                const mentionText = span.textContent.trim().toLowerCase();
                if (mentionText === `@${username.toLowerCase()}`) {
                    mentionFound = true;
                }
            });
            ...
            console.log('Mention detected! Logging...');
            
            const senderElement = messageElement.querySelector('[class*="user__"]');
            const messageContent = messageElement.querySelector('[class*="message__"]') || 
                                messageElement.querySelector('[class*="content__"]');
            const timestampElement = messageElement.querySelector('[class*="timestamp__"]') || 
                                    messageElement.querySelector('[class*="time__"]');

            const messageText = messageContent?.textContent.trim() || '';
            const timestamp = timestampElement?.textContent.trim() || new Date().toLocaleString();
            
            let clanTag = '';
            let usernameOnly = 'Unknown';
            
            if (senderElement) {
                const clanTagElement = senderElement.querySelector('[class*="clan__"]');

                const nodes = senderElement.childNodes;
                ...
                if (clanTagElement) {
                    clanTag = clanTagElement.textContent.trim();
                    usernameOnly = fullText.replace(clanTag, '').trim();
                } else {
                    usernameOnly = fullText.trim();
                }
            }
            ...
            const mentionData = {
                ...
                message: messageText,
                timestamp: timestamp,
                fullMessage: messageElement.outerHTML
            };

            const mentions = JSON.parse(localStorage.getItem('chatMentions') || '[]');
            if (!Array.isArray(mentions)) {
                localStorage.setItem('chatMentions', '[]');
            }
            
            const updatedMentions = Array.isArray(mentions) ? mentions : [];
            updatedMentions.unshift(mentionData);
            localStorage.setItem('chatMentions', JSON.stringify(updatedMentions));

            const settings = JSON.parse(localStorage.getItem('chatSettings') || '{}');
            if (settings.mentionSoundEnabled) {
                if (!window.mentionSound) {
                    window.mentionSound = new Audio('https://raw.githubusercontent.com/luna-mae/ClippingTools/refs/heads/main/media/notification%20sound.mp3');
                }
                window.mentionSound.play().catch(e => console.log('Audio play failed:', e));
            }
        }

        function initializeMentionObserver() {
            const chatContainer = document.querySelector('[class*="messages__"], [class*="chat_messages__"]');
            
            if (chatContainer) {
                console.log('Mention observer initialized');
                ...
                const observer = new MutationObserver((mutations) => {
                    mutations.forEach(mutation => {
                        mutation.addedNodes.forEach(node => {
                            ...
                            if (node.nodeType === 1 && !node.matches('[class*="message__"], [class*="chat-message-"]')) {
                                messageElement = node.querySelector('[class*="message__"], [class*="chat-message-"]');
                            }
                            
                            if (messageElement && messageElement.nodeType === 1) {
                                logMention(messageElement);
                            }
                        });
                    });
                });
                ...
                const existingMessages = chatContainer.querySelectorAll('[class*="message__"], [class*="chat-message-"]');
                existingMessages.forEach(logMention);
                
            } else {
                console.log('Chat container not found, retrying...');
                setTimeout(initializeMentionObserver, 1000);
            }
        }
        ...
        function openMentionsModal() {
            if (document.querySelector('.mentions-modal')) return;

            const backdrop = document.createElement('div');
            backdrop.className = 'modal-backdrop-chat';

            const modal = document.createElement('div');
            modal.className = 'mentions-modal';
            modal.style.position = 'fixed';
            modal.style.top = '50%';
            modal.style.left = '50%';
            ...
            modal.innerHTML = `
                <div class="modal-content">
                    <h2 style="color:#fff;background-color:#740700;padding-left:8px;padding-top:8px;padding-bottom:8px;">Mentions</h2>
                    <div style="padding: 10px; display: flex; justify-content: space-between; align-items: center;">
                        <div class="control-button ${settings.mentionSoundEnabled ? 'active' : ''}" id="mention-sound-toggle">
                            <svg class="mention-style tooltip-icon" data-placement="right" data-title="Toggle notification sound for mentions" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" style="width: 33.33%; height: auto;"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path d="M224 0c-17.7 0-32 14.3-32 32l0 19.2C119 66 64 130.6 64 208l0 18.8c0 47-17.3 92.4-48.5 127.6l-7.4 8.3c-8.4 9.4-10.4 22.9-5.3 34.4S19.4 416 32 416l384 0c12.6 0 24-7.4 29.2-18.9s3.1-25-5.3-34.4l-7.4-8.3C401.3 319.2 384 273.9 384 226.8l0-18.8c0-77.4-55-142-128-156.8L256 32c0-17.7-14.3-32-32-32zm45.3 493.3c12-12 18.7-28.3 18.7-45.3l-64 0-64 0c0 17 6.7 33.3 18.7 45.3s28.3 18.7 45.3 18.7s33.3-6.7 45.3-18.7z"/></svg>
                        </div>
                        <button class="closer" id="delete-all-mentions">Delete All</button>
                    </div>
                    <div id="mentions-list" style="padding: 10px;">
                        ${mentions.length ? 
                            mentions.map(mention => `
                                <div class="mention-item" data-id="${mention.id}" style="border-bottom: 1px solid #505050; padding: 10px; margin-bottom: 10px;">
                                    <div style="display: flex; justify-content: space-between; align-items: center;">
                                        <div style="display: flex; align-items: center; gap: 5px;">
                                            ${mention.clanTag ? `<span style="background-color: #7a008a; color: white; padding: 2px 4px; border-radius: 3px; margin-right: 5px; font-size: 0.85em;">${mention.clanTag}</span>` : ''}
                                            <span style="color: #f30e00;">${mention.sender}</span>
                                        </div>
                                        <small style="color: #ccc;">${mention.timestamp}</small>
                                    </div>
                                    <div style="margin: 5px 0; color: #fff;">${mention.message}</div>
                                    <div style="display: flex; justify-content: flex-end;">
                                        <button class="closer" style="padding: 5px 10px; font-size: 11px;" data-delete="${mention.id}">Delete</button>
                                    </div>
                                </div>
                            `).join('') 
                            : 
                            '<div style="text-align: center; color: #ccc; padding: 20px;">No mentions....</div>'
                        }
                    </div>
                </div>
            `;
            ...
            document.getElementById('mention-sound-toggle').addEventListener('click', function() {
                this.classList.toggle('active');
                const settings = JSON.parse(localStorage.getItem('chatSettings') || '{}');
                settings.mentionSoundEnabled = this.classList.contains('active');
                localStorage.setItem('chatSettings', JSON.stringify(settings));
            });

            document.getElementById('delete-all-mentions').addEventListener('click', function() {
                localStorage.setItem('chatMentions', JSON.stringify([]));
                document.getElementById('mentions-list').innerHTML = '<div style="text-align: center; color: #ccc; padding: 20px;">No mentions yet</div>';
            });
            ...
            document.querySelectorAll('[data-delete]').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    localStorage.setItem('chatMentions', JSON.stringify(updatedMentions));
                    document.querySelector(`.mention-item[data-id="${id}"]`)?.remove();
    ...
    function insertMentionsButton() {
        const chatInputActions = document.querySelector('[class*="chat-input_actions__"]');
        
        const ttsButton = document.querySelector('.chat-input_action__qw4PQ.chat-input_tts__G8nFc');
        
        if (chatInputActions && ttsButton && !document.getElementById('mentions-button')) {
            const mentionsButton = document.createElement('button');
            mentionsButton.id = 'mentions-button';
            mentionsButton.type = 'button';
            mentionsButton.className = 'chat-input_action__qw4PQ';
            mentionsButton.innerHTML = '@';
            mentionsButton.style.fontWeight = 'bold';
            mentionsButton.addEventListener('click', openMentionsModal);
            
            chatInputActions.insertBefore(mentionsButton, ttsButton);
        }
    }
    ...
    function initializeButtonObserver() {
        ...
        if (!checkAndInsert()) {
            observer = new MutationObserver((mutations, obs) => {
                if (checkAndInsert()) obs.disconnect();
            });
    ...
    function initializeUIElements() {
        ...
        const settingsObserver = new MutationObserver(() => {
            const chatHeader = document.querySelector('[class*="header__"]');
            if (chatHeader && !document.querySelector('.settings-cog')) {
                attachSettingsIcon();
    ...
    setTimeout(() => {
        ...
        setInterval(() => {
            if (!document.getElementById('mentions-button')) {
                initializeButtonObserver();
            }
        }, 5000);
    }, 2500);
