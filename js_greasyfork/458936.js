...
// @grant        GM_getValue
// @connect      weibo.com
// @require https://update.cn-greasyfork.org/scripts/435697/1534947/mylib.js
// require      https://files.cnblogs.com/files/blogs/689299/sweetalert2.all.min.js?t=1720363564&download=true
// @icon         https://i0.hdslb.com/bfs/live/143f5ec3003b4080d1b5f817a9efdca46d631945.png
// noframes
// @version 0.0.1.20250210160502
// @namespace http://tampermonkey.net/
// @description .js
// @downloadURL https://update.greasyfork.org/scripts/458936/%E2%9D%97Menu.user.js
// @updateURL https://update.greasyfork.org/scripts/458936/%E2%9D%97Menu.meta.js
// ==/UserScript==
const gsv=GM_setValue,ggv=GM_getValue,gxhr=GM.xmlHttpRequest,gsc=GM.setClipboard,uw=unsafeWindow;//右击:拽-切换固定;顶-找到菜单;底-切换编辑
if(top===self) main(uw);
else {//iframe
    if(location.protocol.includes('http')){
        const btns=uw.myaddbtns(
            function 展(e){e.target.remove()&main()},
            function 删(){uw.top.postMessage({'删':location.href},'*');}// 向顶级窗口发送消息
        );
        uw.myaddas(location.href);
        uw.mywaitele('video',5000).then(v => {//视频找到
            v.pause();
            const rate=uw.mybtns.myappend(`input`,'','placeholder','倍速','value',ggv('pbRate',1)).ael("click",e=>{
                const n= Number(prompt('playbackRate：',rate.value));
                if(n>0){
                    v.playbackRate=e.target.value=n;
                    gsv('pbRate',n);
                }
            }),eles=uw.dqa('*');let fs;
            ...
            for (const ele of eles) {
                for (const attr of ele.attributes) {
                    if (attr.value === "全屏") fs=ele;
                }
            }
            v.onplay=()=>{
                v.playbackRate=rate.value;
                if(!document.fullscreen)fs.click();
            }
            document.addEventListener('keydown',function (ev) {//快进20s
                if (ev.key === ']')v.currentTime+=20;
                else if (ev.key === '[')v.currentTime-=20;
            });
            uw.mysleep(1000).then(()=>{
                v.play();
            });
        }).catch(err => {
            // al(document.URL+err)
...
function main(win){
    const title0=document.title,host=location.host,href=location.href,path=location.pathname,body=document.body,z=win.myz;
    let selt;
    if(!host.includes('mp.weixin')) Object.defineProperty(document,'title',{set:newTitle=>title0,get:newTitle=>title0});// 防瞎改标题
    if(host.includes('docs.q')){
        win.myaddbtns(function date(){
            let date=new Date(),year=date.getFullYear(),
                month=date.getMonth() + 1,//0-11,0代表1月
                day=date.getDate() // 1-31
            if (month < 10) month=`0${month}` // 月前面补0
            if (day < 10) day=`0${day}` // 日前面补0
            win.gsc(`${year}${month}${day}`)
        })
    }
    win.myaddstyle(`my-ul{
            z-index:290390444;
            position:absolute;
            user-select:none;
            list-style:none;
            ...
            font:13px/*/1 NeXT_Roman,Helvetica,Helvetica Neue,Arial,sans-serif*/;
            display:block;
        }.my-li:hover{
...
filter: invert(100%);
    ...
    if(host.includes("tieba")&path.includes("/p")) win.dq('#ueditor_replace').innerText=`挽尊`;
    const ul=z.myappend(`my-ul`,'',`style`,'display:none',`className`,'rwf').ael("click",()=>{ul.style.display='none'}),
          bing=ul.myappend(`li`,'bing',`className`,"my-li").ael("click",()=>{
              open('https://bing.com/search?q='+tarea.value,'_blank')
          }).myappend(`img`,'','src',"http://bing.com/favicon.ico",`className`,'my-ico'),
          copy=ul.myappend(`li`,'copy',`className`,"my-li").ael("click",()=>{
              tarea.select()&document.execCommand('copy')
          }),
          url=ul.myappend(`li`,'url',`className`,"my-li").ael("click",()=>{
              let t=tarea.value.replace(/\s+/g,'');
              if(!(/^h/.test(t))) t="http://"+t;// else if((/^data/.test(t)));
              open(t,'_blank');
          }),
          next1=ul.myappend(`li`,'',`className`,'my-li my_next').ael("mouseover",()=>{
              nav1.style.display=''
          }).ael("mouseout",()=>{
              nav1.style.display='none'
          }),
          next2=ul.myappend(`li`,'',`className`,'my-li my_next').ael("mouseover",()=>{
              nav2.style.display=''
          }).ael("mouseout",()=>{
              nav2.style.display='none'
          }),
          tarea=ul.myappend(`textarea`,'',`className`,'my-ta my-li').ael("mouseover",e=>{
              ul.style.display='none';
              getSelection().removeAllRanges();
          }),
          nav1=next1.myappend(`nav`,'',`style`,'display:none',`className`,'my-nav'),
          nav2=next2.myappend(`nav`,'',`style`,'display:none',`className`,'my-nav'),
          baidu=nav1.myappend(`li`,'baidu',`className`,"my-li").ael("click",()=>{
              open('https://baidu.com/s?wd='+tarea.value,'_blank')
          }).myappend(`img`,'','src',"http://baidu.com/favicon.ico",`className`,'my-ico'),
          zhihu=nav1.myappend(`li`,'zhihu',`className`,"my-li").ael("click",()=>{
              open(`https://www.zhihu.com/search?q=${tarea.value}&sort=upvoted_count&type=content`,'_blank')
          }).myappend(`img`,'','src',"http://zhihu.com/favicon.ico",`className`,'my-ico'),
          bili=nav1.myappend(`li`,'bilibili',`className`,"my-li").ael("click",()=>{
              open(`https://search.bilibili.com/all?keyword=${tarea.value}`,'_blank')
          }).myappend(`img`,'','src',"http://bilibili.com/favicon.ico",`className`,'my-ico'),
          //google=nav1.myappend(`li`,'google',`className`,"my-li").ael("click",()=>{open(`https://www.google.com/search?q=${tarea.value}`,'_blank')}).myappend(`img`,'','src',"http://google.cn/favicon.ico",`className`,'my-ico'),
          doubb=nav1.myappend(`li`,'doubb',`className`,"my-li").ael("click",()=>{
              open(`https://search.douban.com/book/subject_search?search_text=${tarea.value}`,'_blank')
          }).myappend(`img`,'','src',"http://douban.com/favicon.ico",`className`,'my-ico'),
          selnewin=nav1.myappend(`li`,'selnewin',`className`,"my-li").ael("click",()=>{
              const op=confirm('html:text?'),nwin=open('',"_blank");
              if(op) nwin.document.write(getseldom().innerHTML);
              else nwin.document.write(selt.toString().replace(/(\r\n|\r|\n)+/g,'<br>'));
              nwin.document.close();
          }),selhtml=nav1.myappend(`li`,'selhtml',`className`,"my-li").ael("click",()=>{//选中转html
              gsc(getseldom().innerHTML);
          }),events=['mouseup','mousedown'],
          share=nav1.myappend(`li`,'share',`className`,"my-li my-lr").ael("mouseup",ev=>{//al(2)//似乎不能是contextmenu
              ev.preventDefault();ev.stopPropagation();//ev.preventDefault()是阻止默认行为,ev.stopPropagation()才是阻断冒泡/捕获传播
              selt=getSelection().toString().replace(/(\r|\n)+/g,'\n');
              if(!selt)sharewb();
              else if (ev.button===0){//左键释放,首行缩进2
                  selt=selt.replace(/(\r|\n)+/g,'\n').split('\n').map(line => line.trimStart()).map(line => '　　' + line).join('\n');
                  sharewb(selt);
              }else if(ev.button===2){//右键释放,处理知乎
                  selt=selt.replace(/[\r\n]+/g,'\n').replace(/([^.。])\n/g,'$1').split('\n').map(line => line.trimStart()).map(line => '　　' + line).join('\n');
                  sharewb(selt);
              }
          }).myappend(`img`,'','src',"http://weibo.com/favicon.ico",`className`,'my-ico'),
          getitle=nav2.myappend(`li`,'getitle',`className`,"my-li").ael("click",()=>{win.gsc(title0)}),
          rmvCss=nav2.myappend(`li`,'rmvCss',`className`,"my-li").ael("click",()=>{
              win.dqa('style,link[rel="stylesheet"]').forEach(item => item.remove())
          }),
          copylink=nav2.myappend(`li`,'copylink',`className`,"my-li").ael("click",()=>{win.gsc(`[${title0}](${href})`)}),
          saveAll=nav2.myappend(`li`,'saveAll',`className`,"my-li my-lr").ael("mouseup",function(ev){//保存页面all
              const fileName=title0?decodeURIComponent(title0+'&t='+new Date().getTime()+'.htm'):decodeURIComponent(path.replace(/\//g,'')+'&t='+new Date().toJSON()+'.htm'),
                    finalName=prompt('\n文件名:',fileName);//4、download
              if(finalName){
                  win.dqa(".rwf").forEach(e=>{e.remove()})
                  removeRelativePaths();//2、replace relative path to absolute
                  if(body.hasAttribute("contentEditable")) body.removeAttribute("contentEditable");//千古神坑:必须关闭编辑状态!!!
              }else return;/*win.gsc(htm)&n({text:'copy成功！'})*/;
              if (ev.button===0){//左键释放saveAll
                  removeOn(win.dqa('*'),'on');//移除所有dom0事件
                  win.dqa("script,meta").forEach(e=>{e.remove()})//1、remove useless tags
              }
              const ahref=decodeURIComponent(href),a=body.myafter('a','','href','data:text/htm;charset=utf-8,\ufeff'+encodeURIComponent(document.documentElement.innerHTML) ,'download',finalName,'style','visibility:hidden')
              a.click();
              body.myappend('a',ahref,'href',ahref,'style','z-index:0;position:fixed;bottom:0;left:0;opacity:0.7;width:100%');
          }),getfrms=nav2.myappend(`li`,'getfrms',`className`,"my-li").ael("click",e=>{
              const ifrs=win.dqa('iframe');
              if(ifrs.length){
                  const select=z.qs('#my-btns').myappend(`select`,'',`className`,'mb_li'),
                        df=document.createDocumentFragment();
                  ifrs.forEach((e,id)=>{
                      const option=df.myappend(`option`,'iframe'+id,`h`,e.src).addEventListener('click',e=>{
              ...
              e.target.remove()
          }),clickAll=nav2.myappend(`li`,'clickAll',`className`,"my-li").ael('click',()=>{
              let kw=prompt('请输入queryStr');
              if(kw) win.dqa(kw).forEach(e=>{e.click()})
          }),rolldown=nav2.myappend(`li`,'rolldown',`className`,"my-li").ael('click',()=>{
              let s=win.si;
              if(s){
                  win.si=''&clearInterval(s)
              }else{
                  let ele=prompt('请输入下滚元素选择器语句，否则window');
                  ele?ele=win.eval(ele):ele=window;
                  win.si=setInterval(()=>{
                      ele.scrollBy(0,10000);
                      ele.scrollBy(0,-100);
                  },10)
              }
          }),繁转简=nav2.myappend(`li`,'繁转简',`className`,"my-li").onclick=()=>{
              document.title=simplized(title0);
              try {findTextNode(document.documentElement)}catch(e){}
          },getdivd=nav1.myappend(`li`,'getdivd',`className`,"my-li").ael("click",()=>{
              const t=getSelection().toString()||prompt('内容:',''),char=prompt('分隔符:','');
              win.gsc(divideStr(t,char));
          }),rmv隐藏域=nav2.myappend(`li`,'rmv隐藏域',`className`,"my-li").ael("click",()=>{
              win.dqa('input[type="hidden"]').forEach(e=>{e.type=''});
          }),funcs=e=>{e.stopImmediatePropagation()};
    next1.insertAdjacentText(`beforeend`,'next1▹');
    next2.insertAdjacentText(`beforeend`,'next2▹');
    const btns=win.myaddbtns(
        function 顶(){
            const winy0=win.scrollTop;
            win.scrollTo(0,0);
            const winy1=win.scrollTop;
            if(winy0===winy1) body.scrollTo(0,0);
        },
        function 底(){
            const winy0=win.scrollTop;
            win.scrollTo(0,body.scrollHeight);
            const winy1=win.scrollTop;
            if(winy0===winy1) body.scrollTo(0,body.scrollHeight);
        },
        function X(){win.myz.remove()}
    );
    btns.children[1].oncontextmenu=ev=>{//顶 右键召唤菜单
        ev.preventDefault();//捕获阶段,阻止右键菜单
        ul.style=`position:fixed;left:50%;top:0;display:block`;
    }
    btns.children[2].oncontextmenu=ev=>{//底 turnEdit
        ev.preventDefault();
        if(body.hasAttribute("contentEditable")){
            body.removeAttribute("contentEditable");
            events.forEach(event=>{win.removeEventListener(event,funcs,!0)})
        }else{
            body.contentEditable=!0;
            events.forEach(event=>{win.addEventListener(event,funcs,!0)})
        }
    };
    document.addEventListener("mouseup",ev => {//al(1)
        // ev.preventDefault();
        tarea.value=getSelection().toString().replace(/(\r\n|\r|\n)+/g,'\n');
        if(tarea.value){
            ul.style = `left:${ev.pageX}px;top:${ev.pageY}px;display:block`;
        }else{
            ul.style.display='none';
        }
        if(ev.altKey){//实现按住alt键在当前页打开a标签
            ev.target.dispatchEvent(new CustomEvent('altdown',{
                bubbles:false,
                cancelable:!0
            }));
            ev.stopImmediatePropagation();//停止分发
        }
    });//默认false冒泡,从内向外
    document.addEventListener("keydown",ev=>{
        if(ev.altKey){
            const k=ev.key.toLowerCase();
            if (k==='n') win.mysohe(win.myz)//alt+N    显/隐我的z中tag
            else if (k=== 'm') win.myeods()//alt+m    开/关我的z中styletag
            else if (k=== 'p') ul.style=`position:fixed;left:50%;top:0;display:block`;//alt+P    复位
        }
    },true);
    document.addEventListener('altdown',ev=>{// JSON.stringify(1);//锁定线程,效果同alert但不弹窗
        let a= ev.target,ahref=a.href;
        while (a && a.tagName !== 'A' && a.tagName !== 'BODY'){
            a=a.parentNode;//向外找
        }
        if (a && a.tagName==='A'){
            ahref?location.href=a.href:a.setAttribute('target','_self')&a.click()
        }else{//向里找
            a=ev.target.qs('A');
            a?ahref?location.href=ahref:a.setAttribute('target','_self')&a.click():alert("no a found");
        }
    },true);
    win.addEventListener('message',ev=>{//cl(ev) 监听message事件,移除对应iframe;缺点是只能通过唯一src定位
        if(ev.data.hasOwnProperty('删')) document.querySelector(`iframe[src="${ev.data.删}"]`).remove();
    });
}
function divideStr(str,char){return Array.from(str).join(char);}//以特定字符分隔字符串
function removeRelativePaths(){//除了移除改变以下的相对路径为绝对路径外,还要替换含锚的本站点的绝对路径>相对路径;不含css内的路径!
    return function(...props){
        const l=location.href.length,href=location.href;//根路径
        props.forEach(prop=>{
            dqa(`[${prop}]`).forEach(e=>{
                const eprop=e[prop]||e.getAttribute(prop);//得到完整路径
                if(eprop.startsWith("#"));//保留锚点
                else{
                    if(eprop.startsWith(href+"#")) e[`${prop}`]=eprop.substr(l)//替换原有的完整路径的锚点
                    else e[`${prop}`]=eprop;//替换相对路径为绝对路径
                }
            });
        })
    }('href','src','url')
}
function removeOn(domArr,prefix){//比如removeOn(win.dqa('*'),'on')移除所有元素的所有on开头的属性
    domArr.forEach(e=>{
...
function getseldom(){
    const tempDiv=document.createElement('div'),selt=getSelection();;
    for(var i=0;i<selt.rangeCount;i++){
        const range=selt.getRangeAt(i);
        tempDiv.appendChild(range.cloneContents());
    }
    return tempDiv;
}
function sharewb(txt){//必须登录
    let memo=prompt('解说/分隔符:','');
    if(memo===null) return;//取消
    else{
        if(memo.length>1){
            memo=`「${memo}」`;
        }else{//分隔符
            // href=divideStr(href,memo);
            txt=divideStr(txt,memo);//含1字符和0字符
        }
    }
    if(txt){
        txt=`❗${txt.replaceAll(/\n+/g,"\n")}`;
    ...
    gxhr({
        method:'POST',url:"https://weibo.com/aj/mblog/add",
        headers:{
            'Host':'weibo.com',
        ...
        },data:"text="+encodeURIComponent(`${location}\n#${document.title}#\n${memo}${txt}\n——————sharewbd by GM.xmlHttpRequestค้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้้`),
        onload:res=>{// console.log(res)
            Swal.fire({
                title:'前往微博？',
                text:JSON.parse(res.responseText).msg,
                timer:5000,
                toast:true,
                position:'top-end',
            }).then((result) => {
                if (result.isConfirmed)location="https://weibo.com/6126516289";
            });
        },onerror:res=>{
            Swal.fire({
                position:'top-end',
