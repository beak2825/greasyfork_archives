// ==UserScript==
// @name        zZoink Hacks (DGamer edition)
...
// @downloadURL https://update.greasyfork.org/scripts/539901/zZoink%20Hacks%20%28DGamer%20edition%29.user.js
// @updateURL https://update.greasyfork.org/scripts/539901/zZoink%20Hacks%20%28DGamer%20edition%29.meta.js
// ==/UserScript==
...
let friends = JSON.parse(localStorage.getItem('aimbotBlacklist')) || ["zZoink", "dominicguppyrel"];
const GUI = lil.GUI;
const gui = new GUI({ container: lilGuiContainer, title: 'DGamer Hacks' });
...
advancedAimbotFolder.add({ editBlacklist: editAimbotBlacklist }, 'editBlacklist').name('Edit Aimbot Blacklist (' + Object.keys(friends).length + ' ' +(Object.keys(friends).length !== 1 ? 'users' : 'user')+' currently blacklisted)').listen();
...
function updateNames() {
    ...
    if (n > 0) name = `${name} (${n} user${n === 1 ? '' : 's'} currently blacklisted)`;

    // Update the GUI element
    advancedAimbotFolder.controllers.forEach(controller => {
        if (controller.property === 'editBlacklist') {
            controller.name(name);
        }
    })
}

function editAimbotBlacklist() {
    const currList = friends.join(', ');
    const string = prompt('Enter usernames of players for whom aimbot should be disabled.\nSeparated by a single comma & space:', currList);
    ...
    if (string !== null) {
        ...
        localStorage.setItem('aimbotBlacklist', JSON.stringify(friends));
console.log(friends);
        updateNames();
    }
}
  ...
  entities.forEach(entity => {
    ...
    if (!entity.logged && isPlayer(entity)) {
      ...
      entity.ennemy = isEnnemy(entity);
        ...
        // Determine isTarget state
        entity.isTarget = !entity.isFriend && (entity.ennemy || espConfig.allEnnemies);
    ...
    if (entity.isPlayer) {
        ...
        const isTarget = !entity.isFriend && (entity.ennemy||espConfig.allEnnemies);
