...
(function () {
    ...
    class AlchemyHandler {
        ...
        static replaceAlchemyAtlasData(data) {
            ...
            data.data.data = configs.recipes;
            return data;
        }
    }
    ...
    class DevHelper {
        // 在强化页面，点开所有物品后执行
        // 依赖火龙果插件的 itemCnEn 可以直接在该脚本中定义
        static exportEnhanceItemNames() {
            // 选择所有包含物品名称的 div 元素
            const nameElements = document.querySelectorAll('div.absolute.text-center.left-0.-bottom-5\\.5');

            // 提取所有物品名称
            const itemNames = Array.from(nameElements).map(element => element.textContent).filter((name) => {
                return !name.includes('+')
            });

            let itemEnCn = {};
            for (const cnName of itemNames) {
                const enName = itemCnEn[cnName];
                if (enName) {
                    itemEnCn[enName] = cnName;
                }
            }

            itemEnCn['starEssence'] = "星辉精华"
            console.log(itemEnCn)
        }
    }
    ...
    class FloatingPanel {
        ...
        `;
            header.innerHTML = `
            <span style="font-weight:700; letter-spacing: -0.5px; font-size: 15px;">${this.title}</span>
    ...
    let gmc = newGMC({
        ...
        devToolsEnhanceExport: {
            'label': '开发使用：打印强化插件格式的中文映射',
            'type': 'button',
            'title': '需要在强化页面->查看可强化物品点开后再使用',
            'click': function () {
                DevHelper.exportEnhanceItemNames()
            }
        }
    });
