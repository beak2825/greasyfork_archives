...
// @grant        none
// @license      GNU AGPLv3
// @namespace    https://greasyfork.org/users/1340274
// @downloadURL https://update.greasyfork.org/scripts/501855/%E5%90%90%E5%8F%B8%E9%AD%94%E6%94%B9.user.js
// @updateURL https://update.greasyfork.org/scripts/501855/%E5%90%90%E5%8F%B8%E9%AD%94%E6%94%B9.meta.js
// ==/UserScript==
...
setTimeout(function() {
    ...
    // 模拟点击删除按钮
    let AutoDeleteButton = null;
	console.log('初始化AutoDeleteButton函数');
 
    async function startAutoDeleteButton() {
		AutoDeleteButton = setTimeout(async function() {
			const deleteButton = document.querySelector('button.__button-zm13ws-lsme.n-button.n-button--error-type.n-button--small-type');
			if (deleteButton) {
				deleteButton.click();
			}
			startAtuoDeleteButton();
		}, 300);
    }
    ...
    async function startInfiniteTimer() {
		...
		InfiniteTimer = setInterval(async function() {
		    const powerOutBtn = document.querySelector('button.vi-button.vi-button--size-large.vi-button--type-primary.flex-1');
		    if (powerOutBtn) {
		        console.log('算力不足了');
		        return;
		    }
		    
			const queueWarningButton = document.querySelector('button.__button-dark-1mkpae0-lsmw.n-button.n-button--warning-type.n-button--small-type');
            if (queueWarningButton) {
                console.log('检测到队列弹窗');
				queueWarningButton.click();
				console.log('click queue full');
			}
    ...
    function main() {
        ...
        checkboxDelete.addEventListener('change', function() {
            ...
            if (enabled) {
                startAutoDeleteButton();
                console.log('开启自动确认删除');
            } else {
                clearInterval(AutoDeleteButton);
