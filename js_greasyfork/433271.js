// ==UserScript==
// @name        processon-SVG下载辅助
// @namespace   https://github.io/ok-dok/
// @match       https://www.processon.com/diagraming/*
// @grant       none
// @version     0.1.1
// @author      ok-dok
// @description 2021/10/2 上午2:28:46
// @description:zh-cn 此脚本用于修复processon下载svg图片时，解决组合图片没有分组的问题
// @downloadURL https://update.greasyfork.org/scripts/433271/processon-SVG%E4%B8%8B%E8%BD%BD%E8%BE%85%E5%8A%A9.user.js
// @updateURL https://update.greasyfork.org/scripts/433271/processon-SVG%E4%B8%8B%E8%BD%BD%E8%BE%85%E5%8A%A9.meta.js
// ==/UserScript==

$('.header_right_btn').prepend(
  '<div class="toolbar_button active selected" id="custom_export_btn" style="padding: 0px 10px;background-color: green" menu="export_file">下载SVG</div>'
)
              ...
              if (f.result == 'size_wrong') {
                ...
                if (f.result == 'exception') {
                  ...
                  if (f.result == 'svg_error') {
                    ...
                    var g = f.image
                    UI.setShapeImage(g.fileId, g.imageW || 100, g.imageH || 100)
                  }
                }
              }
            }
          ...
          },
          {
            ...
            onChange: function (active) {
              ...
              if (activeIndex != active - 1) {
                shape.path[activeIndex].fillStyle = {
                  color: 'r-20,g-20,b-20',
                }
                shape.path[activeIndex].actions.push({
                  action: 'close',
                })
  ...
  c.colorpicker = function (f) {
    ...
    var d = {
      ...
      changeViewByHsb: function () {
        ...
        k.find('.picker_pin').css({
          left: Math.round((h.h / 360) * l),
        })
      },
      HSBToRGB: function (g) {
        var j = {}
...
var flow2svg = {
  init: function (o) {
    ...
    for (var r in a) {
      ...
      if (!!a[r].group && !!!groupDraw[a[r].group]) {
        gr = q.group()
        gr.id = a[r].group
        groupDraw[a[r].group] = gr
      }
    }
    ...
    for (var g = 0, h = p.length; g < h; g++) {
      var m = p[g]
      var gr = groupDraw[m.group]
      if (m.name == 'linker') {
        this.linker2svg(gr, m, q, s, l)
      } else {
        if (m.parent == '' || m.parent == null) {
          this.shape2svg(gr, m, q, s)
          if (m.children != null && m.children.length > 0) {
            var d = m.children
            for (var f = 0; f < d.length; f++) {
              var e = d[f]
              var c = Utils.copy(a[e])
              this.shape2svg(gr, c, q, s, l)
            }
          }
        }
      }
  ...
  shape2svg: function (gbox, shape, draw, canvasSize, hasTheme) {
    if (!shape || shape.name == 'linker') {
      return
    }
    var props = shape.props,
      x = 25,
    ...
    var group = null
    if (!!gbox) {
      group = gbox.group().attr('transform', 'translate(' + x + ',' + y + ')')
    } else {
      group = draw.group().attr('transform', 'translate(' + x + ',' + y + ')')
    }
      ...
      if (fillStyle.type == 'none') {
        ...
        if (fillStyle.type == 'image') {
          ...
          if (fillStyle.display == 'stretch' && flow2svg.type == 'pdfHD') {
            var image = group.image(image).size(imgW, imgH).attr({
  ...
  linker2svg: function (gbox, l, j, c, i) {
    var p = l.points
    var n = g(l.from)
    var d = g(l.to)
    ...
    if (Math.abs(n.y - d.y) < 1 && Math.abs(n.x - d.x) < 1) {
      return
    }
    var o = Utils.getEndpointAngle(l, 'from')
    var f = Utils.getEndpointAngle(l, 'to')
    var e = Utils.getLinkerLineStyle(l.lineStyle, i)
    if (e.lineWidth == 0) {
      e.lineWidth = 1
    }
    if (l.linkerType == 'broken') {
      var r = Model.intersection[l.id]
      if (r) {
        p = m(l.from, l.points, r.points, l.to)
      }
    }
    b()
    function b() {
      ...
      if (l.linkerType == 'curve') {
        ...
        if (l.linkerType == 'broken') {
          ...
          for (var v = 0; v < p.length; v++) {
            var y = g(p[v])
            if (y.type == 'intersectionR') {
              var u = y.x,
                t = y.y
              A +=
                'L' + (u - 6) + ' ' + t + 'A 3 3, 0, 0, 1,' + (u + 6) + ' ' + t
            } else {
              if (y.type == 'intersectionL') {
                var u = y.x,
                  t = y.y
                A +=
                  'L' +
    ...
    function q(s, v) {
      var u = null
      switch (s.beginArrowStyle) {
        case 'solidArrow':
          var w = 'M15,1.5 L2,6 L15,10.5',
            t = 0
          if (s.lineWidth == '1') {
            w = 'M10,3 L0,6 L10,9'
            t = 0
          ...
          u = j
            .marker(16, 12, function (y) {
              y.path(w)
                .fill('rgb(' + s.lineColor + ')')
                .stroke({
                  width: s.lineWidth,
                  color: 'rgb(' + s.lineColor + ')',
                })
            })
        ...
        case 'normal':
          u = j
            .marker(16, 20, function (y) {
              y.path('M15,1 L1,10 L15,19')
                .fill('none')
                .stroke({
                  width: s.lineWidth,
                  color: 'rgb(' + s.lineColor + ')',
        ...
        case 'solidDiamond':
          u = j
            .marker(20, 12, function (y) {
              y.path('M2,6 L10,2 L18,6 L10,10L2,6')
                .fill('rgb(' + s.lineColor + ')')
                .stroke({
                  width: s.lineWidth,
                  color: 'rgb(' + s.lineColor + ')',
      ...
      var x = null
      switch (s.endArrowStyle) {
        case 'solidArrow':
          var w = 'M0,2 L14,6 L0,11 L0,2',
            t = 16
          if (s.lineWidth == '1') {
            w = 'M0,3 L10,6 L0,9 L0,3'
            t = 10
          }
          x = j
            .marker(16, 12, function (y) {
      ...
      if (u != null) {
        if (typeof s.lineStyle != 'undefined' || s.lineStyle !== 'solid') {
          u.attr('stroke-dasharray', '0,0')
        }
        v.attr('marker-start', 'url(#' + u.node.id + ')')
      }
      if (x != null) {
        if (typeof s.lineStyle != 'undefined' || s.lineStyle !== 'solid') {
          x.attr('stroke-dasharray', '0,0')
        }
        v.attr('marker-end', 'url(#' + x.node.id + ')')
      }
    }
    function k(C) {
      ...
      if (l.textPos != null) {
        ...
        var u = $('#' + l.id).children('.linker_text')
        if (l.textPos.pos == 'bottom') {
          D.y += u.outerHeight() / 2 + 4
        } else {
      ...
      var w = Utils.getLinkerFontStyle(l.fontStyle)
      var x = w.fontFamily
      var s =
      ...
      flow2svg.richText2svg(v, C, D.x - y / 2, D.y - t / 2, 0)
      v.remove()
    }
    function a() {
      var z = {}
      if (l.linkerType == 'normal') {
        ...
        if (l.linkerType == 'curve') {
          ...
          var y = d
          z = {
            x: G.x * 0.125 + E.x * 0.375 + A.x * 0.375 + y.x * 0.125,
            y: G.y * 0.125 + E.y * 0.375 + A.y * 0.375 + y.y * 0.125,
          }
        } else {
          var H = []
          H.push(n)
          ...
          for (var w = 1; w < H.length; w++) {
            var E = H[w - 1]
            var A = H[w]
            var x = Utils.measureDistance(E, A)
            D += x
          }
          var C = D / 2
          var v = 0
    ...
    function m(E, G, v, F) {
      for (var z = 0; z < G.length - 1; z++) {
        var D = G[z].x,
          ...
          u = G[z + 1].x,
          C = G[z + 1].y
        if (Math.abs(A - C) < 1 && Math.abs(D - u) < 1) {
          G[z + 1].type = 'equal'
          G[z].type = 'equal'
        }
      }
        ...
        if (Math.abs(A - C) < 1) {
          ...
          if (u > D) {
            for (var w = 0; w < v.length; w++) {
              var t = v[w].x,
                B = v[w].y
              if (Math.abs(A - B) < 1 && t > D && t < u) {
                v[w].type = 'intersectionR'
                G[z].intersectionArr.push(v[w])
              }
            }
            G[z].intersectionArr = H(G[z].intersectionArr, 'x')
          } else {
            for (var w = 0; w < v.length; w++) {
              var t = v[w].x,
                B = v[w].y
              if (Math.abs(A - B) < 1 && t < D && t > u) {
                v[w].type = 'intersectionL'
                G[z].intersectionArr.push(v[w])
              }
            }
            G[z].intersectionArr = H(G[z].intersectionArr, 'x').reverse()
          }
        }
      }
      var I = []
    ...
    for (var w = 0; w < s.length; w++) {
      ...
      if (A.style.indexOf('underline') > -1) {
        ...
        if (!(flow2svg.type == 'svg' || flow2svg.type == 'pdfHD')) {
          var l = k.line(z, d, x, b).stroke({
            width: Math.round(parseInt(q.size) / 20),
            color: u,
          })
        }
      } else {
