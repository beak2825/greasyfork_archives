...
// @downloadURL https://update.greasyfork.org/scripts/39590/NyQueue.user.js
// @updateURL https://update.greasyfork.org/scripts/39590/NyQueue.meta.js
// ==/UserScript==
...
$(document).on('click','#startNyq',function(){//Begins NyQueue when 'GO!' button is pressed
    ...
    $('body').append(`<iframe src=${source} height=${wHeight} width=${eWidth-5} align ="right" name="queframe" allow="microphone; camera"></frame>`);
    if(gmGet(`colorShow`)){
...
function renderFrame(href){//draws the iframes to the NyQueue window
    $('body').append(`<iframe src="${href}" height=${eHeight} width=${eWidth} name="${href}" allow="microphone; camera"></frame>`);
