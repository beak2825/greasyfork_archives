// ==UserScript==
// @name         Auto download in drive.google.com
// @namespace    Auto download Google Drive
// @description  A userscript help automatic download files in Google Drive
// @version      1.0
// @icon         data:image/jpeg;base64,
// @match        https://drive.google.com/file/d/*
// @match        https://drive.usercontent.google.com/download*
// @license      GPL-3.0
// @downloadURL https://update.greasyfork.org/scripts/523674/Auto%20download%20in%20drivegooglecom.user.js
// @updateURL https://update.greasyfork.org/scripts/523674/Auto%20download%20in%20drivegooglecom.meta.js
// ==/UserScript==

window.onload = function() {
    var url = document.location.href;
    if (url.includes('https://drive.google.com/file/d/')) {
        if (url.endsWith('view')) {
            window.location.href = url.replace('drive.google.com/file/d/', 'drive.usercontent.google.com/download?id=').replace('/view', '&export=download');
        }
    } else if (url.includes('https://drive.usercontent.google.com/')) {
        const downloadLink = document.querySelector('#uc-download-link');
        if (downloadLink) {
            downloadLink.click();
        } else {
            window.location.href = url.replace('https://drive.usercontent.google.com/download?id=', 'https://drive.google.com/file/d/')
                                      .replace(/([&?])export=download.*?$/, '');
        }
    }
};