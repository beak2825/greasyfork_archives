// ==UserScript==
// @name         SIHF Hockey Minutaz Tracker
// @namespace    http://tampermonkey.net/
// @version      1.2
// @description  Sleduje hokejovou minutáž podle třetiny, možnost ruční úpravy, podporuje pauzu
// @author       Michal
// @match        https://www.sihf.ch/de/game-center//game/*
// @grant        none
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/528632/SIHF%20Hockey%20Minutaz%20Tracker.user.js
// @updateURL https://update.greasyfork.org/scripts/528632/SIHF%20Hockey%20Minutaz%20Tracker.meta.js
// ==/UserScript==

(function () {
    'use strict';

    console.log("🚀 Hockey Minutaz Tracker se spustil v Tampermonkey!");

    setTimeout(() => {
        console.log("⏳ Spuštěn po 3 sekundách...");
        ...
        function adjustMinute(change) {
            let minuteDisplay = document.getElementById("hockeyMinute");

            
            if (minuteDisplay.innerText === "-") {
                hockeyMinute = 1;
            } else {
                hockeyMinute = Math.min(20, Math.max(1, hockeyMinute + change));
            }

            minuteDisplay.innerText = hockeyMinute;
        ...
        function detectPeriod() {
            let periodElement = document.querySelector(".icon.-text.-disabled");
            if (!periodElement) {
                console.warn("⚠️ Element .icon.-text.-disabled nenalezen!");
                return null;
            }

            let periodText = periodElement.innerText.trim();
            console.log(`🔍 Detekován text: ${periodText}`);
        ...
        function waitForElement(selector, callback) {
            let interval = setInterval(() => {
                ...
                if (element) {
                    ...
                    console.log(`✅ Element ${selector} nalezen!`);
                    callback(element);
                }
            }, 1000);
        }

        waitForElement(".icon.-text.-disabled", () => {
            console.log("📢 Spouštím interval pro kontrolu třetiny každých 5 sekund...");
