// ==UserScript==
// @name        Lowes Inventory & Price Checker w/Ship Availability w/Ship Availability
// @namespace   myLPCw/shipnf
// @id          My Lowes Price Checker
// @description Check inventory and prices at a set of Lowes stores or all.
...
// @version     1.4
// @run-at document-end
// @downloadURL https://update.greasyfork.org/scripts/16249/Lowes%20Inventory%20%20Price%20Checker%20wShip%20Availability%20wShip%20Availability.user.js
// @updateURL https://update.greasyfork.org/scripts/16249/Lowes%20Inventory%20%20Price%20Checker%20wShip%20Availability%20wShip%20Availability.meta.js
// ==/UserScript==
...
(function ($) {
    ...
    GM_addStyle(
        '#icDialog { padding: 1em; z-index: 777; overflow-y: scroll;} \
         ...
         #myTableFooter h4 {text-align: center;}                           \
         #myTableHeader h4 {text-align: center;}                           \
         #myTableHeader h4 {text-align: center;}                           \
    ...
    $('#tabs-price-check').html(
         '<div id="myTableHeader" data-role="header"><h3> </h3></div> \
          <div><table data-role="table" data-mode="columntoggle" class="ui-responsive ui-shadow pc-tab" id="myTable">    \
            <thead>       \
              <tr><th>Store</th><th data-priority="1">Quantity</th><th data-priority="2">Price</th><th data-priority="3">Ship</th></tr>  \
            </thead>               \
            <tbody id="myTabBody"></tbody>  \
          </table></div>                    \
        <div id="myTableFooter" data-role="footer"><h4> </h4></div>');
    ...
    $('#tabs-options').html(
         '<div id="myOptDiv"></div>   \
            ...
            <tr><td><div style="text-align: left;"><input type="checkbox" id="hideInput2" name="hideEmptyShip"  ' + (options.hideEmptyShip ? "checked" : "" ) + ' /> Hide empty stores with no shipping</div></td></tr> \
    ...
    function goAction() {
        ...
        $('#myExportCSV').click( function() {
            debug('export');
            var csvContent = 'Store, Quantity, Price, Ship' + "\n";
            ...
            if (a.download !== undefined ) {
               ...
               a.target      = '_blank';
               a.download    = 'Lowes_' + unsafeWindow.Lowes.ProductDetail.product.description + ' ' + unsafeWindow.Lowes.ProductDetail.product.id +'.csv';
    ...
    function parseQty(storeId) {
        ...
        return function (respObject, status) {
            ...
            // find the div with the unavailStatus.ship
            var ship = $(responseDoc).find('div.unavailStatus.ship').text() || 0;
               //$(responseDoc).find('div.availStatus.store').attr('data-avlqty') || 0;
             //$(responseDoc).$('div.unavailStatus.ship').text();
            ...
            */
            ...
            if (availStatus !=0 || !options.hideEmpty) {
                // Creata a new row and add the data to the table
                addResult(storeName, availStatus, price, ship);
            }
            
        }
    }
    
    function addResult(storeName, qty, price, ship) {
        ...
        // insert
        if (ind == 0) {
            $('#myTabBody').prepend('<tr><td>' + storeName + '</td><td>' + qty + '</td><td>$' + price + '</td><td>' + ship + '</td></tr>');
        } else if (ind >= sortedRes.length) {
            // add to the end
            $('#myTabBody').append('<tr><td>' + storeName + '</td><td>' + qty + '</td><td>$' + price + '</td><td>' + ship + '</td></tr>');
        } else { 
            // add to the correct position
            var rowBefore = $('#myTabBody').children('tr').get(ind-1);
            $(rowBefore).after('<tr><td>' + storeName + '</td><td>' + qty + '</td><td>$' + price + '</td><td>' + ship + '</td></tr>');
        }
    }
    ...
    function parsePrice(storeId) {
        ...
        return function (respObject, status) {
            ...
            if (resHolder.hasOwnProperty(storeId) && resHolder[storeId]!= null)
            {
               ...
               // Creata a new row and add the data to the table
               $('#myTabBody').append('<tr><td>' + storeName + '</td><td>' + sQty + '</td><td>$' + price + '</td><td>' + sQty + '</td></tr>');
