// ==UserScript==
// @name         Youtube 字幕下载TXT格式(PayneV)
// @description  Youtube 播放右下角的齿轮选项中，提供了自动字幕，这个工具可以下载这个字幕
...
// @run-at document-end
// @downloadURL https://update.greasyfork.org/scripts/400517/Youtube%20%E5%AD%97%E5%B9%95%E4%B8%8B%E8%BD%BDTXT%E6%A0%BC%E5%BC%8F%28PayneV%29.user.js
// @updateURL https://update.greasyfork.org/scripts/400517/Youtube%20%E5%AD%97%E5%B9%95%E4%B8%8B%E8%BD%BDTXT%E6%A0%BC%E5%BC%8F%28PayneV%29.meta.js
// ==/UserScript==
...
// 转载于
// 作者  : Cheng Zheng 新浪微博 : 糖醋陈皮 Email   : guokrfans@gmail.com

// 改了的地方：
// 获取字幕的格式,只保留文本，序号，时间戳都被删除了。
// 下载按钮显示的文本
// 保存文件的格式为txt文本
...
function normal_subtitle_button(){
    if(new_material_design_version()){
        var title_element = document.querySelectorAll('.title.style-scope.ytd-video-primary-info-renderer');
        if (title_element){
            $(title_element[0]).after('<a id="YT_auto">' + '下载 『' + auto_subtitle_language_name + '』 字幕</a>');
        }
        $("#YT_auto").css('margin-top','2px')
            .css('position','relative')
            .css('top','-3px')
        ...
        $('#watch-headline-title').after('<a id="YT_auto">' + '下载 『' + auto_subtitle_language_name + '』 字幕</a>');
        $('#YT_auto').addClass('start yt-uix-button yt-uix-button-text yt-uix-tooltip'); // 样式是 Youtube 自带的.
...
function parseYoutubeXMLToSRT(youtube_xml_string){
    ...
    var len = text.length;
    //  分隔关键词数组
    var NewlineWords = ["."," and "," so "," now "," another "];
 ...
 */
        ...
        var new_line = "%0D%0A";
        //  遍历NewlineWords数组，判断 content内是否有需要换行的关键字
        for(var j=0;j<NewlineWords.length;j++){
            let wordsIndex = content.indexOf(NewlineWords[j]);
            if(wordsIndex != -1){
                content = content.substring(0,wordsIndex)+new_line+NewlineWords[j]+content.substring(wordsIndex+NewlineWords[j].length);
            }
        }
...
function get_file_name(){
    if(new_material_design_version()){
        ...
        return get_auto_subtitle_name() + video_name + '.txt';
    } else {
        var TITLE = unsafeWindow.ytplayer.config.args.title;
        return get_auto_subtitle_name() + TITLE + '..txt';
    }
}
