...
// @license GPL
// @downloadURL https://update.greasyfork.org/scripts/448727/Goatlings%20Accessibility%20Mod.user.js
// @updateURL https://update.greasyfork.org/scripts/448727/Goatlings%20Accessibility%20Mod.meta.js
// ==/UserScript==
...
if (page.includes("battle")) {
    ...
    if (page === "https://www.goatlings.com/battle") {
        ...
        let runAway = findText("Run Away From Battle");
        if (runAway) {
            runAway.innerHTML += " (R)";
        }
...
} else if (page.includes("viewa")) {
    var priceText = document.body.innerHTML;
    var startIndex = document.body.innerHTML.indexOf("close to ") + 12;

    priceText = document.body.innerHTML.substring(startIndex);
    var endIndex = priceText.indexOf(" Sugar");
    priceText = priceText.substring(0, endIndex);
    var priceTextNoComma = priceText.replace("<b>", "");
    priceTextNoComma = priceText.replace("</b>", "");
    priceTextNoComma = priceText.replace(" ", "");
    priceTextNoComma = priceText.replace(",", "");
    var priceInt = parseInt(priceTextNoComma);
    document.body.innerHTML = document.body.innerHTML.replace(priceText, priceInt * .8);
...
function battle_function(key) {
    if (page === "https://www.goatlings.com/battle") {
        ...
        if (key === ' ' || key === 'Spacebar' || key == "q") {
            ...
            findText("Run Away From Battle (R)").click()
        }
    } else if (page === "https://www.goatlings.com/battle/over") {
        ...
        } else if (key === ' ' || key === "Spacebar" || key == "q") {
...
function explore_function(key) {
    if (page.includes("view")) {
        if (findText("Explore Again! (Spacebar)") && (key === ' ' || key === 'Spacebar' || key == "q")) {
            let exploreAgain = findText("Explore Again! (Spacebar)")
            exploreAgain.click()
        } else if (key === ' ' || key === 'Spacebar' || key == "q") {
