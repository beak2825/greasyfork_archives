...
// @downloadURL https://update.greasyfork.org/scripts/550345/%E5%AE%9E%E6%88%98%E6%B5%8B%E8%AF%953.user.js
// @updateURL https://update.greasyfork.org/scripts/550345/%E5%AE%9E%E6%88%98%E6%B5%8B%E8%AF%953.meta.js
// ==/UserScript==
...
async function shotFourLayersCompose() {
  ...
  async function captureUploadAndAct(el){
    ...
    // 发送 b64
    const dataUrl=await shotFourLayersCompose(); const idx=dataUrl.indexOf('base64,'); const b64_raw=idx>=0?dataUrl.slice(idx+7):dataUrl;
  ...
  /*** 仅在“领取/验证”按钮点击后触发 ***/
  const TRIGGER_WORDS = ['领取','立即领取','去领取','刷新验证'];
  ...
  async function scanOnce(){
    ...
    for (const div of tipsB) {
      const txt=(div.innerText||div.textContent||'').trim();
      if (txt.includes('依次点击'))        {CLICK.intervalMs=400;       CFG.captchaType = 2316;}
      else if (txt.includes('符合右图')) {CFG.captchaType = 2302;CLICK.intervalMs=100;}
      const box = div.closest('div[class*="geetest_box_"]') || div.closest('div[class*="captcha"]');
      if (box) { await captureUploadAndAct(box); return true; }
    }
    // 兜底：常见类名
  ...
  document.addEventListener('click', (e)=>{
    ...
    setTimeout(()=>{ scanAfterClick().catch(()=>{}); }, 2000);
  }, true);
