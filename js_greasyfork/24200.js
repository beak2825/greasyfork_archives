...
// @downloadURL https://update.greasyfork.org/scripts/24200/WhutBBCodeFork.user.js
// @updateURL https://update.greasyfork.org/scripts/24200/WhutBBCodeFork.meta.js
// ==/UserScript==
...
(function () {
	...
	/**
	 * Creates a WhutBBCodeFork? instance for a textarea
	...
	/**
	 * The WhutBBCodeFork? initializer
	 *
	...
	WhutBB.init = function (config) {
		if (WhutBB.gazelle && WhutBB.gazelleBlacklist.test(document.location.href)) {
			console.log('WhutBBCodeFork?: Will not run in ' + RegExp.lastMatch + '!');
			return;
		}

		WhutBB.config = new WhutBB.Settings(config || WhutBB.db.getSiteSettings(WhutBB.$.findSite()));
		try {
			console.info('WhutBBCodeFork? mode ' + WhutBB.config.name);
	...
	WhutBB.user = {
		...
		options: {
			...
			/*link: {
				txt: 'WhutBBCodeFork? Link',
				title: 'Show WhutBBCodeFork? link',
				value: true
			}*/
		},
		load: function () {
		...
		delegate: {
			...
			settings: { // settings events
				...
				fn: {
					...
					link: function () { // toggles WhutBBCodeFork? link
	 ...
	 *
	 * This class is used to store site configurations for WhutBBCodeFork?
	 ...
	 *
	 *  link:
	 *    link to information about the site's BBCode or WhutBBCodeFork? itself (default)
	 *
	...
	WhutBB.Panel = (function () {
		...
		Panel.construct = function () {
			Panel.factory('link', dom.dom('div', {className: 'wbblink' + (WhutBB.user.settings.link ? '' : ' wbbhide') },
				dom.dom('a', {href: WhutBB.config.link, title: 'Version r.' + update.version, txt: 'WhutBBCodeFork?', target: '_blank'})), true);
