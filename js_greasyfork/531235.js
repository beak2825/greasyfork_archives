...
// @version      0.0.1
// @description  Xuetang auto play!
// @author       You
// @match        https://www.xuetangx.com/*
// @license MIT
// @icon         https://www.google.com/s2/favicons?sz=64&domain=xuetangx.com
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/531235/XuetangAuto.user.js
// @updateURL https://update.greasyfork.org/scripts/531235/XuetangAuto.meta.js
// ==/UserScript==
    ...
    class XMLHttpRequestHijacked extends original_XHR
    {
        ...
        get responseText()
        {
            ...
            else if(this.isList)
            {
                ...
                let cid=JSON.parse(super.response).data.course_id;
                console.log(lst);
                ...
                console.log(all_lst);
                localStorage.setItem(cid,JSON.stringify(all_lst));
                return super.responseText;
            }
            /*else if(this.hijack && false)
            {
            ...
            else if(this.hijack)
            {
                if(this.fin)
                {
                    let all_lst=JSON.parse(localStorage.getItem(this.fid));
                    let newID=all_lst[all_lst.indexOf(this.currID)+1]
                    let newUrl=window.location.href.replace(this.currID,newID);
                    console.log("redirect to next video",newUrl);
                    window.location.assign(newUrl);
        ...
        {
            if(url.search("api/v1/lms/learn/course/chapter")!=-1)
            {
                this.isList=true;
            }
            if(url.search("video-log/heartbeat")!=-1)
            {
                let currPath=window.location.pathname;
                this.currID=parseInt(currPath.substring(currPath.lastIndexOf('/')+1));
                //currPath.replace("/"+this.currID,"");
                //currPath.replace("/video","");
