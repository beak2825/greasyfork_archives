// ==UserScript==
// @name         Letterboxd → FLAM (Fenlight) [Icon Launcher + Picker + Bubble Status + Click Sound]
// @namespace    http://tampermonkey.net/
// @version      6.9.0
// @description  Send Letterboxd lists and nanogenre pages to Kodi (Fenlight) using TMDB
...
// @noframes
// @downloadURL https://update.greasyfork.org/scripts/547592/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29%20%5BIcon%20Launcher%20%2B%20Picker%20%2B%20Bubble%20Status%20%2B%20Click%20Sound%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/547592/Letterboxd%20%E2%86%92%20FLAM%20%28Fenlight%29%20%5BIcon%20Launcher%20%2B%20Picker%20%2B%20Bubble%20Status%20%2B%20Click%20Sound%5D.meta.js
// ==/UserScript==

/*
  Changes (5.1.0):
  - Optional click sound when pressing the Fenlight icon.
  - Sound is fetched once from GitHub (raw) and cached in IndexedDB for instant playback.
  - Monthly revalidation via ETag; manual "Refresh Click Sound" button in Settings → Cache Tools.
  - Toggle "Enable click sound" in Settings (default ON).
*/

/*
  Changes (5.0.2):
  - Restrict script to user list pages only (e.g., /jack/list/official-top-250-films-with-the-most-fans/).
  - Prevent running in iframes/ads via @noframes and a top-window guard.
*/
...
(async function () {
  ...
  // ────────────────────────────────────────────────────────────────────────
  // NEW: Hard guards so we only run on real user list pages and never in frames
  // ────────────────────────────────────────────────────────────────────────
  if (window.top !== window.self) return; // extra safety even with @noframes

  // Accept URLs like:
  //   /{user}/list/{slug}/
  //   /{user}/list/{slug}/page/2/
  function isTargetListPage() {
    const p = location.pathname.replace(/\/+$/, '/'); // normalize trailing slash
    return /^\/[^/]+\/list\/[^/]+\/(?:page\/\d+\/)?$/.test(p);
  }
  ...
  const DEFAULT_FINALRETRY_MIN_LIST_SIZE = 5001;
  ...
  // NEW: Click sound defaults
  const CLICK_WAV_URL_DEFAULT = 'https://raw.githubusercontent.com/hcgiub001/letterboxd-tmdb-cache/main/click.wav';
  const CLICK_SOUND_ENABLE_DEFAULT = true;
  ...
  // ────────────────────────────────────────────────────────────────────────
  // IndexedDB (v2) – overrides + base + click sound
  // ────────────────────────────────────────────────────────────────────────
  function openDB() {
  ...
  // NEW: Click sound storage in IDB
  async function getClickFromIDB() {
    const db = await openDB();
    const tx = db.transaction('tmdbBase', 'readonly');
    const store = tx.objectStore('tmdbBase');
    const bufP = new Promise((res, rej) => { const r = store.get('click_wav_buf'); r.onsuccess = () => res(r.result || null); r.onerror = () => rej(r.error); });
    const etgP = new Promise((res, rej) => { const r = store.get('click_wav_etag'); r.onsuccess = () => res(r.result || ''); r.onerror = () => rej(r.error); });
    const [buf, etag] = await Promise.all([bufP, etgP]);
    return { buf, etag };
  }
  async function putClickToIDB(buf, etag) {
    const db = await openDB();
    const tx = db.transaction('tmdbBase', 'readwrite');
    await Promise.all([
      new Promise((res, rej) => { const r = tx.objectStore('tmdbBase').put(buf, 'click_wav_buf'); r.onsuccess = res; r.onerror = () => rej(r.error); }),
      new Promise((res, rej) => { const r = tx.objectStore('tmdbBase').put(etag || '', 'click_wav_etag'); r.onsuccess = res; r.onerror = () => rej(r.error); }),
    ]);
  }
  ...
  // ────────────────────────────────────────────────────────────────────────
  // NEW: Click sound loader + monthly revalidation
  // ────────────────────────────────────────────────────────────────────────
  let CLICK_SOUND_URL = null;          // object URL for the sound blob
  const clickAudio = new Audio();      // persistent audio element for low-latency playback
  clickAudio.preload = 'auto';

  function setClickAudioFromBuf(buf) {
    try {
      if (CLICK_SOUND_URL) { try { URL.revokeObjectURL(CLICK_SOUND_URL); } catch {} }
      const blob = new Blob([buf], { type: 'audio/wav' });
      CLICK_SOUND_URL = URL.createObjectURL(blob);
      clickAudio.src = CLICK_SOUND_URL;
    } catch {}
  }

  async function loadClickSoundOnce({ allowRevalidate = false } = {}) {
    // load from IDB first
    if (!CLICK_SOUND_URL) {
      const { buf } = await getClickFromIDB();
      if (buf && buf.byteLength) setClickAudioFromBuf(buf);
    }
    if (!allowRevalidate) return;

    const { etag } = await getClickFromIDB();
    await new Promise((resolve) => {
      GM_xmlhttpRequest({
        method: 'GET',
        url: CLICK_WAV_URL_DEFAULT,
        headers: etag ? { 'If-None-Match': etag } : {},
        responseType: 'arraybuffer',
        onload: async (res) => {
          try {
            if (res.status === 304) {
              GM_setValue('clickLastRevalidateTs', Date.now());
              resolve(); return;
            }
            if (res.status >= 200 && res.status < 300 && res.response) {
              const newBuf = res.response;
              const m = String(res.responseHeaders || '').match(/etag:\s*(.+)/i);
              const newEtag = m ? m[1].trim() : '';
              try { await putClickToIDB(newBuf, newEtag); } catch {}
              setClickAudioFromBuf(newBuf);
              GM_setValue('clickLastRevalidateTs', Date.now());
            }
          } finally { resolve(); }
        },
  ...
  async function refreshClickSoundNow() {
    await new Promise((resolve) => {
      GM_xmlhttpRequest({
        method: 'GET',
        url: CLICK_WAV_URL_DEFAULT,
        ...
        onload: async (res) => {
          try {
            if (res.status >= 200 && res.status < 300 && res.response) {
              ...
              setClickAudioFromBuf(newBuf);
              GM_setValue('clickLastRevalidateTs', Date.now());
              alert('✅ Click sound refreshed.');
            } else {
              alert(`❌ Click sound HTTP ${res.status}.`);
            }
          } finally { resolve(); }
        },
        onerror: () => { alert('❌ Click sound network error.'); resolve(); },
        ontimeout: () => { alert('❌ Click sound request timed out.'); resolve(); },
      });
    });
  }

  async function maybeRevalidateClickMonthly() {
    try {
      const last = GM_GetOrSet('clickLastRevalidateTs', 0);
      const now = Date.now();
      if (now - last >= THIRTY_DAYS_MS) {
        await loadClickSoundOnce({ allowRevalidate: true });
        GM_setValue('clickLastRevalidateTs', Date.now());
      }
    } catch {}
  }
  // Preload from IDB immediately; schedule monthly revalidate
  await loadClickSoundOnce({ allowRevalidate: false });
  setTimeout(() => { maybeRevalidateClickMonthly(); }, 0);

  function playClickSound() {
    if (!GM_GetOrSet('clickSoundEnable', CLICK_SOUND_ENABLE_DEFAULT)) return;
    try {
      // ensure src set even if loaded later
      if (!clickAudio.src && CLICK_SOUND_URL) clickAudio.src = CLICK_SOUND_URL;
  ...
  function showSideInfoNearEl(el, lines, ms = 5000) {
    ...
    Object.assign(box.style, {
      position: 'fixed',
      top: '0px',                        // set then adjust after measuring
      right: '130px',
      background: '#1b1b1b',
      color: '#eee',
    ...
    try {
      ...
      // Try placing below the icon first.
      topPx = rect.top + 40;
      ...
      // If "below" would overflow, place above the icon.
  ...
  mainIcon.addEventListener('click', async function () {
    ...
    // Play click sound immediately on user gesture
    playClickSound();
  ...
  // ────────────────────────────────────────────────────────────────────────
  // Settings UI (Tabbed + Scrollable)  — with Icon Picker section + Click Sound toggle
  // ────────────────────────────────────────────────────────────────────────
  ...
  function showSettings() {
    ...
    panel.innerHTML = `
      ...
      <div id="kodi-tab-general" class="kodi-tab" style="display:block">
        ...
        <label style="color:#fff"><input type="checkbox" id="clickSoundEnable"/> Enable click sound</label>

        <label style="color:#fff;margin-top:8px;display:block"><input type="checkbox" id="descEnable"/> Add Description</label>
        ...
        <div>
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:6px">
            ...
            <button id="refreshClickSoundBtn">Refresh Click Sound</button>
          </div>
    ...
    panel.querySelector('#clickSoundEnable').checked = GM_GetOrSet('clickSoundEnable', CLICK_SOUND_ENABLE_DEFAULT);
    ...
    panel.querySelector('#refreshClickSoundBtn').onclick = async () => {
      setCacheMsg('Refreshing click sound…');
      await refreshClickSoundNow();
      setCacheMsg('Click sound refreshed.', true);
    };
    ...
    panel.querySelector('#kodisave').onclick = () => {
      ...
      GM_setValue('clickSoundEnable', panel.querySelector('#clickSoundEnable').checked);
