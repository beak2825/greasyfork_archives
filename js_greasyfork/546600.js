// ==UserScript==
// @name         知乎UI优化
// @namespace    http://tampermonkey.net/
// @version      v1.0.1
// @description  知乎UI界面优化
// @author       明天不知在哪里
// @match         *://*.zhihu.com/*
// @icon         data:image/x-icon;base64,
// @require      https://cdn.jsdelivr.net/npm/lodash@4.17.21/lodash.min.js
// @grant        none
// @run-at       window-load
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/546600/%E7%9F%A5%E4%B9%8EUI%E4%BC%98%E5%8C%96.user.js
// @updateURL https://update.greasyfork.org/scripts/546600/%E7%9F%A5%E4%B9%8EUI%E4%BC%98%E5%8C%96.meta.js
// ==/UserScript==

(function () {
  "use strict";
  setTimeout(() => {
    // 关闭登录提示弹窗
    const closeBtns = document.querySelectorAll(".Modal-closeButton");
    closeBtns.forEach((closeBtn) => {
      closeBtn.click();
    });

    // 删除右下角悬浮登录弹窗
    const closeFloatLogins = _.throttle(() => {
      const floatLogins = document.querySelectorAll(".css-woosw9");
      if (floatLogins.length > 0) {
        floatLogins.forEach((floatLogin) => {
          floatLogin.remove();
        });
        document.removeEventListener("scroll", closeFloatLogins);
      }
    }, 16);

    closeFloatLogins();

    // 监听滚动, 关闭右下角悬浮登录弹窗
    document.addEventListener("scroll", closeFloatLogins);

    // 关闭右侧栏
    const asideColumn = document.querySelector(".Question-sideColumn");
    if (asideColumn) {
      asideColumn.remove();
    }

    // 调整主体部分宽度
    const mainColumn = document.querySelector(".Question-mainColumn");
    if (mainColumn) {
      mainColumn.style.width = "100%";
    }

    // 关闭知乎专栏底部的推荐阅读
    const postSub = document.querySelector(".Post-Sub");
    if (postSub) {
      postSub.remove();
    }
    // 关闭专栏右侧栏
    const postRightColumn = document.querySelector(".Post-Row-Content-right");
    if (postRightColumn) {
      postRightColumn.remove();
    }

    // 专栏主体部分宽度调整
    const postMain = document.querySelector(".Post-Row-Content-left");
    if (postMain) {
      postMain.style.width = "100%";
    }
  }, 500);
})();
