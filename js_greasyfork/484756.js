...
// @grant        none
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/484756/MH%20-%20AnyTrapAnySkin.user.js
// @updateURL https://update.greasyfork.org/scripts/484756/MH%20-%20AnyTrapAnySkin.meta.js
// ==/UserScript==

var base_skin_button;
var trap_skin_button;
...
buttonstyle.innerHTML = '.buttonstyle {display:block;width:auto;height:20px;margin:5px 36px 0;background:#f6f3eb;border:1px solid #534023;-webkit-box-shadow:-1px -1px 1px #d3cecb inset;box-shadow:-1px -1px 1px #d3cecb inset;border-radius:4px;padding:0;z-index:15}';
...
function inject_skin_buttons(){
    let tsv_summary = document.getElementsByClassName("trapSelectorView__summary");
    if (tsv_summary.length==0){
        return;
    }
    let codex_container = document.getElementsByClassName("trapSelectorView__activeCodexContainer");
...
function steal(){
    ...
    let item_view_container = document.getElementsByClassName('itemViewContainer')[0];
...
function generate_steal_button(){
    ...
    button.classList.add('mousehuntActionButton','itemView-action-component-actionButton','available');
    button.onclick = function(click_event){steal()};
...
function inject_steal_button(){
    let itemviewcontainer = document.getElementsByClassName('itemViewContainer')[0]
    let item_id = itemviewcontainer.getAttribute('data-item-id');
    let itemview_actioncontainer = document.getElementsByClassName('itemView-actionContainer')[0];
    ...
    let itemView_action_weapon = document.getElementsByClassName('itemView-action weapon')[0];
    let wb_button = generate_steal_button();
    wb_button.id = 'wb-steal';
    let s_button = generate_steal_button();
    s_button.id = 's-steal';
    if (item_id == localStorage.getItem('stolen-weapon') || item_id == localStorage.getItem('stolen-base')){
        wb_button.classList.replace('available','selected');
        s_button.classList.replace('available','selected');
    }
...
function move_stolen_layers(){
    ...
    let trap_layers = document.getElementsByClassName('trapImageView-layerWrapper')[0];
    let real_base_layer = document.getElementsByClassName('trapImageView-layer base')[0];
    let real_weapon_layer = document.getElementsByClassName('trapImageView-layer weapon')[0];
    let real_bait_layer = document.getElementsByClassName('trapImageView-layer bait')[0];
    switch (to_show){
        case 'none':
...
function populate_stolen_layers(){
    ...
    hg.utils.UserInventory.getItems([stolen_base_id,stolen_weapon_id,real_weapon_id],(e)=>{
        let stolen_base=e[0];
        ...
        if (real_weapon.classification=='weapon'){
            if(real_weapon.type=='sphynx_weapon'||real_weapon.type=='chrome_sphynx_weapon'){
                real_weapon_layer.classList.add('sphynx-weapon');
            }
        } else if (real_weapon.classification=='skin'){
            if(real_weapon.component_type=='sphynx_weapon'||real_weapon.component_type=='chrome_sphynx_weapon'){
                real_weapon_layer.classList.add('sphynx-weapon');
            }
        }
        if (stolen_weapon.classification=='weapon'){
            if(stolen_weapon.type=='sphynx_weapon'||stolen_weapon.type=='chrome_sphynx_weapon'){
                stolen_weapon_layer.classList.add('sphynx-weapon');
            }
        } else if (stolen_weapon.classification=='skin'){
            if(stolen_weapon.component_type=='sphynx_weapon'||stolen_weapon.component_type=='chrome_sphynx_weapon'){
                stolen_weapon_layer.classList.add('sphynx-weapon');
            }
        }
        stolen_base_layer.style.backgroundImage='url('+stolen_base.image_trap+')';
        stolen_weapon_layer.style.backgroundImage='url('+stolen_weapon.image_trap+')';
        stolen_bait_layer.style.backgroundImage=real_bait_layer.style.backgroundImage;
        move_stolen_layers();
    });
}
...
function watch_weapon(){
    ...
    new MutationObserver(function(mutationList){
        if (mutationList[0].target.className == mutationList[0].oldValue) {return;}
        hg.utils.UserInventory.getItem(user.weapon_item_id,function(e){
            let real_weapon_layer = document.getElementsByClassName('trapImageView-layer weapon')[0];
            if (e.weapon_image_large && real_weapon_layer.getAttribute('large-weapon')==null){
                real_weapon_layer.setAttribute('large-weapon',true);
            } else {
    ...
    }).observe(document.getElementsByClassName('trapImageView-layer weapon')[0],{attributeFilter:['class'],attributeOldValue:true});
}
...
(function() {
    ...
    if (localStorage.getItem('stolen-base') == null){localStorage.setItem('stolen-base','31')};
    if (localStorage.getItem('stolen-weapon') == null){localStorage.setItem('stolen-weapon','51')};
    if (localStorage.getItem('stolen-show') == null){localStorage.setItem('stolen-show','both')};
    ...
    let modify_popup = function(mutationList){
        if (document.getElementById('overlayPopup').classList.contains('itemViewPopup')){
            inject_steal_button();
        }
    }
    new MutationObserver(modify_popup).observe(document.getElementById('overlayPopup'),{attributeFilter:['class']});
