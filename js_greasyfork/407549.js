...
// @run-at           document-end
// @downloadURL https://update.greasyfork.org/scripts/407549/IO%20lukuhelpotin.user.js
// @updateURL https://update.greasyfork.org/scripts/407549/IO%20lukuhelpotin.meta.js
// ==/UserScript==
...
(function () {
    ...
    'fi-FI': {
        ...
        'ignoreThread': 'Jätä viestiketju huomioimatta',
        'unIgnoreThread': 'Huomioi viestiketju'
    }
    };
    ...
    document.addEventListener('keydown', (kbEvent) => {
        if (kbEvent.isComposing) {
            console.log("__ COMPOSING")
        }
        if (kbEvent.target.tagName == "INPUT" || kbEvent.target.tagName == "TEXTAREA" || kbEvent.target.isContentEditable) {
            return;
        }

        if (kbEvent.ctrlKey && (kbEvent.code == forumkey.ReadAll || kbEvent.code == forumkey.ReadWatched || kbEvent.code == forumkey.ToggleIgnore || kbEvent.code == forumkey.ToggleWatch)) {

            switch ($('html').attr('data-template')) {

                case 'forum_view':

                    if (kbEvent.ctrlKey && (kbEvent.code == forumkey.ReadAll || kbEvent.code == forumkey.ReadWatched)) {
                        ...
                        while (i >= 0 && threadClickCount < maxThreadClickCount) {
                            ...
                            if (kbEvent.code == forumkey.ReadWatched) {
                            ...
                            jeaIOLH_debuglog("Open: " + linkelement.href);
                ...
                case "thread_view":
                    ...
                    if (kbEvent.code == forumkey.ToggleIgnore) {
                        ...
                        if (typeof (linkelementIgnore) != 'undefined' && linkelementIgnore.style.display != "none") {
                            jeaIOLH_debuglog("Thread: "+uiStrings[xfLang].ignoreThread+': '+window.location.href);
                            ...
                            if (typeof (linkelementUnignore) != 'undefined') {
                                jeaIOLH_debuglog("Thread: "+uiStrings[xfLang].unIgnoreThread+': '+window.location.href);
                                linkelementUnignore.click();
                            } else {
                                jeaIOLH_debuglog("Thread: "+uiStrings[xfLang].unIgnoreThread+': '+window.location.href);
                                jeaIOLH_getElementFromUrl(function (dom) {
                    ...
                    if (kbEvent.code == forumkey.ToggleWatch) {
                        ...
                        if ($('a:contains('+uiStrings[xfLang].unWatchThread+')').length > 0) {
                            jeaIOLH_debuglog("Thread: "+uiStrings[xfLang].unWatchThread);
                            $('a:contains('+uiStrings[xfLang].unWatchThread+')').click();
                        } else if ($('a:contains('+uiStrings[xfLang].watchThread+')').length > 0) {
                            jeaIOLH_debuglog("Thread: "+uiStrings[xfLang].watchThread);
    ...
    function jeaIOLH_getElementFromUrl(callback, href) {
        GM_xmlhttpRequest({
            method: 'GET',
            url: href,
    ...
    function jeaIOLH_debuglog(foo) {
        if (jea_debug === true) {
            console.log('__ jeaIOLH: ' + foo);
        }
    }
