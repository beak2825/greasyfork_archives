...
// @grant			none
// @run-at 			document-end
// @downloadURL https://update.greasyfork.org/scripts/2951/HackForums%20Moderation%20Packs.user.js
// @updateURL https://update.greasyfork.org/scripts/2951/HackForums%20Moderation%20Packs.meta.js
// ==/UserScript==
...
if(pageTitle.contains("showthread.php")) {
	...
	var inputs = inputs + '<input type="button" class="bitButton xeromove" value="Marketplace Discussions" /><br />';
	...
	var inputs = inputs + '<input type="button" class="bitButton xeromove" value="RANF" />';
	...
	$j('.xeromove').click(function(e) {
		...
		} else if(inputval == 'Marketplace Discussions') {
			$j('select[name="moveto"]').val('163');
		...
		} else if(inputval == 'RANF') {
			$j('select[name="moveto"]').val('2');
