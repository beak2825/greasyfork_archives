...
// @grant        GM_getValue
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/469045/get_tweet_card.user.js
// @updateURL https://update.greasyfork.org/scripts/469045/get_tweet_card.meta.js
// ==/UserScript==
...
(function () {
  ...
  class Notice {
    constructor() {
      ...
      this.notice.innerHTML = `
        ...
        ">
          <div id="notice-text" style="
            font-size: 16px;
            color: #333;
            margin-bottom: 10px;
          "></div>
          <button id="notice-btn" style="
            width: 100px;
            height: 30px;
            border: none;
            outline: none;
      ...
      `;
      this.noticeText = this.notice.querySelector("#notice-text");
      this.noticeBtn = this.notice.querySelector("#notice-btn");
      this.noticeBtn.addEventListener("click", () => {
    ...
    setMessage(text) {
      this.noticeText.innerText = text;
    }
    hide() {
      this.notice.style.display = "none";
    }
  ...
  async function get_tweet_card(tp, input) {
    ...
    /** @type {string} 获取输入 */
    const inputContent = await tp.system.prompt(
      "输入内容",
      selectedText,
      false,
      true
    ...
    if (opt.writeToClipboard) {
      await new Promise(async (reslove) => {
        canvas.toBlob(async (blob) => {
          // debugger
          let res = await navigator.clipboard
            .write([
              new ClipboardItem({
                [blob.type]: blob,
              }),
  ...
  button.textContent = "Share Tweet";

  // Set button style
  button.style.position = "fixed";
  button.style.top = "50px";
  button.style.right = "0";
  button.style.zIndex = "9999"; // Set z-index
