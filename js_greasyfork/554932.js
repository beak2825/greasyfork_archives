// ==UserScript==
// @name        Discourse ReadBoost3.0
// @namespace   discourse_ReadBoost3.0
// @match       *://*/*
// @grant       GM_setValue
// @grant       GM_getValue
...
// @version     3.0
// @author      Do
// @description Discourse ReadBoost是一个刷取已读帖量脚本，支持所有Discourse论坛（改LINUXDO ReadBoost）
// @description:zh-TW Discourse ReadBoost是一個刷取已讀帖量腳本，支持所有Discourse論壇
// @description:en Discourse ReadBoost is a script to boost the number of read posts. It supports all Discourse forums.
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/554932/Discourse%20ReadBoost30.user.js
// @updateURL https://update.greasyfork.org/scripts/554932/Discourse%20ReadBoost30.meta.js
// ==/UserScript==
...
(function () {
    ...
    function isDiscourseForum() {
        return !!document.querySelector('meta[name="discourse-base-url"]') ||
               !!document.querySelector('meta[name="generator"][content*="Discourse"]') ||
               !!document.body.classList.contains('discourse-default-styles');
    }

    // 如果不是Discourse论坛，直接退出
    if (!isDiscourseForum()) {
        return;
    }
    ...
    if (!hasAgreed) {
        const userInput = prompt("[ Discourse ReadBoost ]\n检测到这是你第一次使用Discourse ReadBoost，使用前你必须知晓：使用该第三方脚本可能会导致包括并不限于账号被限制、被封禁的潜在风险，脚本不对出现的任何风险负责，这是一个开源脚本，你可以自由审核其中的内容，如果你同意以上内容，请输入'明白'")
        if (userInput !== "明白") {
    ...
    function isTopicPage() {
        // 通用的Discourse帖子页面检测
        // Discourse的帖子页面格式通常为：/t/topic-slug/topic-id 或 /t/topic-id
        const pathPattern = /\/t\/[^/]+\/\d+/;
    ...
    function getPageInfo() {
        ...
        const topicID = pathParts[pathParts.length - 1] || pathParts[pathParts.length - 2];
    ...
    async function processReading(pageInfo) {
        ...
        async function sendBatch(startId, endId, retryCount = 3) {
            ...
            try {
                ...
                const response = await fetch(`${baseUrl}/topics/timings`, {
                    method: "POST",
                    headers: {
    ...
    function init() {
        ...
        try {
            const pageInfo = getPageInfo()
            console.log("Discourse ReadBoost 已加载")
            console.log(`帖子ID: ${pageInfo.topicID}, 总回复: ${pageInfo.totalReplies}`)
            console.log(`当前域名: ${getCurrentBaseUrl()}`)
