...
// @match           https://www.erepublik.com/*/main/citizen-friends/*
// @include         http://www.erepublik.com/*/main/citizen-friends/*
// @include         http://www.erepublik.com/*/citizen/profile/*
// @include         https://www.erepublik.com/*/citizen/profile/*
// @require         https://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery.min.js
// @downloadURL https://update.greasyfork.org/scripts/6632/Remove%20Dead%20Friends.user.js
// @updateURL https://update.greasyfork.org/scripts/6632/Remove%20Dead%20Friends.meta.js
// ==/UserScript==
...
function friends_get_list() {
    ...
    friends_config.amount   = parseFloat($('.citizen_activity h4.friends_title a').text()
        ...
        .replace('ĞŸÑ€Ğ¸Ñ˜Ğ°Ñ‚ĞµĞ»Ğ¸ (', "").replace(')', "") // Macedonian version
        .replace('ĞŸÑ€Ğ¸Ñ�Ñ‚ĞµĞ»Ğ¸ (', "").replace(')', "") // Bulgarian version
        ...
        .replace('Î¦Î¯Î»Î¿Î¹ (', "").replace(')', "") // Greek version
        .replace('Prijatelji (', "").replace(')', "") // Croatian, Slovenian version 
        .replace('BarÃ¡tok (', "").replace(')', "") // Hungarian version 
        ...
        .replace('Ğ”Ñ€ÑƒĞ·ÑŒÑ� (', "").replace(')', "") // Russian version 
        .replace('ShokÃ«t (', "").replace(')', "") // Albanian version 
        .replace('ĞŸÑ€Ğ¸Ñ˜Ğ°Ñ‚ĞµÑ™Ğ¸ (', "").replace(')', "") // Serbian version
        .replace('ArkadaÅŸlar (', "").replace(')', "") // Turkish version
        .replace('å¥½å�‹ (', "").replace(')', "") // Taiwanian version
        .replace('Ğ”Ñ€ÑƒĞ·Ñ– (', "").replace(')', "") // Ukranian version
        .replace('æœ‹å�‹ä»¬ (', "").replace(')', "") // Chinese version
        .replace('Ø§Ù„Ø£ØµØ¯Ù‚Ø§Ø¡ (', "").replace(')', "") // Arabic version
        .replace('Ø¯ÙˆØ³ØªØ§Ù† (', "").replace(')', "") // Iranian version
    );
