...
class JableLinker {
  constructor() {
    this.SITES = {
      ...
      AVSUBTITLES: "https://www.avsubtitles.com",
      SUBTITLECAT: "https://www.subtitlecat.com",
      HAIJIAO: "https://www.haijiao.com",
    };
    this.SELECTORS = {
      // JAV Site Selectors
      JAVDB_CODE: ".value",
    ...
    this.STYLES = {
      SUKEBEI: `table.torrent-list td:nth-child(4) {white-space: wrap;}.navbar-form .input-group>.form-control {width: 100%;margin-right: 50vw;}.container div:has(video),.container div:has(iframe),.ts-im-container,div:has(>.mn-related-container),td:has(img){display:none !important;} #navbar > form {position: fixed; top:30px;width: 1vw;}.table {width: 100%;} .torrent-list>tbody>tr>td { white-space: normal; max-width:90vw;}`,
      ...
      JAVDB: `.container:not(.is-max-desktop):not(.is-max-widescreen) { max-width: 100vw; } .movie-list .item .video-title { white-space: normal; } .moj-content{display:none}.fancybox-slide--video .fancybox-content,.fancybox-video {height: auto !important;width: auto !important;/*left: 25%;top: 25%;*/}`,
      ...
      HAIJIAO: `* {-webkit-user-select:auto;}.containeradvertising,.haslist{display:none !important;}`  ,
    };
    const commonButtons = {
      javdb: true,
      javbus: true,
    ...
    this.SITE_CONFIG = {
      [this.SITES.JAVDB]: {
        ...
        pathCheck: "/v/",
        buttons: { ...commonButtons, javdb: false },
        magnetSelectors: {
      ...
      [this.SITES.JAVBUS]: {
        ...
        pathCheck: /[A-Za-z]+-\d+/,
        buttons: { ...commonButtons, javbus: false },
        magnetSelectors: {
      ...
      [this.SITES.JABLE]: {
        ...
        pathCheck: "/videos/",
        buttons: { ...commonButtons, jable: false },
        isVideoSite: true,
      },
      [this.SITES.AVJOY]: {
        ...
        pathCheck: "/video/",
        buttons: { ...commonButtons, avjoy: false },
        isVideoSite: true,
      },
      [this.SITES.AV01]: {
        ...
        pathCheck: "/video/",
        buttons: { ...commonButtons, av01: false },
        isVideoSite: true,
      },
      [this.SITES.SLADKOTV]: {
        ...
        pathCheck: "/watch/",
        buttons: { ...commonButtons, sladkotv: false },
        isVideoSite: true,
      },
      [this.SITES.MISSAV]: {
        ...
        pathCheck: "/cn/",
        buttons: { ...commonButtons, missav: false },
        isVideoSite: true,
      },
      // Magnet Site Configs
      [this.SITES.SUKEBEI]: {
  ...
  utils = {
    ...
    getCodeFromCurrentSite: (domain, selector) => {
      ...
      let codeMatch =
        title.match(/[A-Za-z]+-\d+/) || document.title.match(/[A-Za-z]+-\d+/);
    ...
    gmFetch: (url, options = {}) =>
      new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
          ...
          onerror: (error) =>
            reject(new Error("Network error during request", { cause: error })),
        });
      }),
  ...
  dom = {
    ...
    linkButtonDefs: {
      ...
      javlibrary: {
        ...
        color: "#6027ae",
        url: (code) =>
          `${this.SITES.JAVLIBRARY}/cn/vl_searchbyid.php?keyword=${code}`,
      },
      hayav: {
        text: "HAYAV",
        color: "#8e44ad",
      ...
      avsubtitles: {
        ...
        color: "#e91e63",
        url: (code) =>
          `${this.SITES.AVSUBTITLES}/search_results.php?search=${code}`,
      },
      subtitlecat: {
        text: "SubtitleCat",
        color: "#c0392b",
    ...
    initBtdigFilesCollapse: () => {
      ...
      const expandAll = (folder) => {
        expand(folder);
        const childrenFolders = getChildren(folder).filter((el) =>
          el.classList?.contains("fa-folder-closed"),
        );
        childrenFolders.forEach((f) => expandAll(f));
      };

      // 初始化：所有文件夹折叠 + 点击事件
      document.querySelectorAll(".fa.fa-folder-open").forEach((folder) => {
        folder.style.cursor = "pointer";
        collapse(folder);
        folder.addEventListener("click", (e) => {
          e.stopPropagation();
          folder.classList.contains("fa-folder-closed")
            ? expand(folder)
            : collapse(folder);
        });
      });
      ...
      btn.style.cssText =
        "position:fixed;top:10px;right:10px;z-index:999;padding:5px 10px;font-size:14px;";
      btn.addEventListener("click", () => {
        document
          .querySelectorAll(".fa.fa-folder-closed")
          .forEach((folder) => expandAll(folder));
      });
          ...
          events: {
            click: async (e) => {
              ...
              try {
                ...
                const stylesheetLink = doc.querySelector(
                ...
                if (videoDetailElement /*&& stylesheetLink*/) {
                  ...
                  this.drawer.openWithContent(
                    videoDetailElement,
                    codeElement.textContent.trim() /*, cssUrl*/,
                  );
                } else {
                  console.error(
                    "JableLinker: Preview content or stylesheet not found.",
                  );
                  alert("无法加载预览内容。");
                }
              } catch (err) {
                ...
                if (errorMsg.includes("403")) {
                  alert(
                    "加载预览失败: 访问被拒绝 (403)。可能是网站阻止了请求，请尝试直接打开链接。",
                  );
                } else {
    ...
    addLinkButtons: async (code, selector, buttonsConfig) => {
      const titleElement = await this.utils
      ...
      if (buttonsConfig?.javdb) {
        try {
          const response = await this.utils.gmFetch(
            `${this.SITES.JAVDB}/search?q=${encodeURIComponent(code)}`,
          );
          const doc = new DOMParser().parseFromString(
            response.responseText,
            "text/html",
          ...
          const url = videoLink
            ? `${this.SITES.JAVDB}${videoLink.getAttribute("href")}`
            : `${this.SITES.JAVDB}/search?q=${encodeURIComponent(code)}`;
          const btn = this.dom.createSingleButton(
            "JAVDB",
            videoLink ? "#e74c3c" : "#c0392b",
            url,
          );
    ...
    createSingleButton: (text, bgColor, url, target = "_self") => {
      return this.utils.NewElement("a", {
        textContent: text,
        attributes: { href: url, target: target },
        style: {
          /*marginRight: "8px",border: "none", borderRadius: "3px",*/ padding:
            "4px 8px",
          color: "#fff",
          background: bgColor,
          cursor: "pointer",
    ...
    enhanceMagnetLinks: (config, detail = false) => {
      const processNode = (node) => {
        //console.log( document.querySelectorAll(config.magnetSelectors.list).length);
        if (!node || node.__tm_magnet_enhanced__) return;
        const selectors = detail
          ? config.magnetDetailsSelectors
          : config.magnetSelectors;
        //console.log(/*detail,*/selectors);
        const hashLinkEl = node.querySelector(selectors.hashLink);
        if (!hashLinkEl) return;
        const hashMatch = hashLinkEl.href
          ? hashLinkEl.href.match(/[0-9a-fA-F]{40}/)
          : hashLinkEl.innerText;
        //const hashMatch = hashLinkEl.href.match(/[0-9a-fA-F]{40}/)||hashLinkEl.innerText;
        if (!hashMatch) return;
        //console.log(hashMatch,hashLinkEl);
        const hash = hashLinkEl.href ? hashMatch[0].toLowerCase() : hashMatch.toLowerCase();
        console.log(hash);

        const title =
          node.querySelector(selectors.title)?.innerText ||
        ...
        const copyBtn = this.utils.NewElement("button", {
          textContent: "📋",
          attributes: { title: "Copy Magnet Link" },
          style: {
            height: "auto",
        ...
        const previewBtn = this.utils.NewElement("button", {
          textContent: "👁️",
          attributes: { title: "Preview Torrent Content" },
          style: {
            height: "auto",
            backgroundColor: "transparent",
            borderColor: "transparent",
      ...
      const run = () =>
        document
          .querySelectorAll(
            detail
              ? config.magnetDetailsSelectors.list
              : config.magnetSelectors.list,
          )
          .forEach(processNode);
      run(); // Initial run
    ...
    replacePreviewImagesFromJAVBUS: async () => {
      try {
        const code = document
          .querySelector(this.SELECTORS.JAVDB_CODE)
          ?.textContent.trim();
        if (!code) {
          console.log(
            "JableLinker: JAVDB code not found for image replacement.",
          );
          return;
        }
        ...
        const anchorElements = Array.from(
          doc.querySelectorAll(this.SELECTORS.JAVBUS_SAMPLE_BOX),
        );
        ...
        if (javbusHrefs.length > 0) {
          if ((javbusHrefs[0] || "").includes("javdb")) {
            console.log(
              "JableLinker: JAVBUS links back to JAVDB, skipping image replacement.",
            );
            return;
          }
          ...
          if (previewImagesContainer) {
            Array.from(
              document.querySelectorAll(this.SELECTORS.JAVDB_PREVIEW_IMAGES),
            ...
            console.log(
              `JableLinker: Replaced preview images with ${javbusHrefs.length} items from JAVBUS for code: ${code}`,
            );
          }
        } else {
          console.log(
            `JableLinker: No preview images found on JAVBUS for code: ${code}`,
          );
        }
      } catch (error) {
        console.error(
          "JableLinker: Error during replacePreviewImagesFromJAVBUS:",
          error,
        );
      }
    },
  ...
  drawer = (() => {
    ...
    const create = (options) => {
      ...
      const header = this.utils.NewElement("div", {
        style: {
          ...
          alignItems: "center",
          padding:
            "16px" /* ,borderBottom: '1px solid #eee', position: 'sticky', top: '0', backgroundColor: 'white', zIndex: '1'*/,
        },
      });
      const titleEl = this.utils.NewElement("a", {
        textContent: title,
        attributes: { href: options.url },
        style: { /* 'font-size': '24px'*/ "word-break": "break-all" },
      });
    ...
    const open = (url, title, width, direction) => {
      const instance = create({ url, title, width, direction });
    ...
    const openWithContent = (element, title, cssHref) => {
      ...
      const wrapper = this.utils.NewElement("div", {
        style: { padding: "1em", backgroundColor: "#f5f5f5", color: "#363636" },
      });
  ...
  subtitles = {
    ...
    _convertAssToVtt: (assText) => {
      ...
      dialogueLines.forEach((line) => {
        ...
        let cleanText = textContent
          ...
          .trim();
        if (cleanText)
          vttContent += `${formatTime(startTime)} --> ${formatTime(
            endTime,
          )}\n${cleanText}\n\n`;
      });
    ...
    _toVtt: (text) => {
      ...
      if (this.subtitles._isLikelyAss(text))
        return this.subtitles._convertAssToVtt(text);
    ...
    _removeExistingLocalTracks: (video) => {
      video
        .querySelectorAll('track[data-local-subtitle="1"]')
        .forEach((t) => t.remove());
    },
    _attachSubtitleToVideo: (video) => {
      ...
      try {
        for (const tt of video.textTracks) {
          tt.mode =
            tt.label === track.label || tt.language === "en"
              ? "showing"
              : "disabled";
        }
      } catch (e) {}
    ...
    _attachToAllVideos: () => {
      ...
      let maxWidthVideo = uniqueVideos.reduce(
        (max, vid) =>
          vid.getBoundingClientRect().width > max.getBoundingClientRect().width
            ? vid
            : max,
        uniqueVideos[0],
      );
    ...
    _ensureUi: () => {
      ...
      const button = this.utils.NewElement("button", {
        ...
        textContent: "本地字幕",
        attributes: { title: "为页面视频加载本地字幕 (.srt/.vtt/.ass)" },
        style: {
          padding: "6px 10px",
          fontSize: "12px",
          color: "#fff",
  ...
  handlers = {
    handleGenericSite: (siteKey, hooks = {}) => {
      ...
      if (siteKey === this.SITES.JAVBUS) {
        if (document.querySelector(".masonry-brick:has(.avatar-box)"))
          this.utils.moveElementToNextSiblingFirstChild(
            ".masonry-brick:has(.avatar-box)",
          );
      ...
      if (
        config.magnetSelectors &&
        (siteKey === this.SITES.JAVDB || siteKey === this.SITES.JAVBUS)
      ) {
        this.dom.enhanceMagnetLinks(config);
      }
      if (isDetailPage) {
        const code = this.utils.getCodeFromCurrentSite(
          siteKey,
    ...
    setupCookieMenu: () => {
      GM_registerMenuCommand("设置站点与Cookie(多行表格)", async () => {
        ...
        const savedPromises = siteEntries.map(([k]) =>
          GM_getValue(`SITES.${k}`, ""),
        );
        const [savedValues, savedJavdbCookie, savedJavbusCookie] =
          await Promise.all([
        ...
        // Cookie rows
        tbody.appendChild(
          makeRow("JAVDB Cookie", "javdb_cookie", savedJavdbCookie, ""),
        );
        tbody.appendChild(
          makeRow("JAVBUS Cookie", "javbus_cookie", savedJavbusCookie, ""),
        );
          ...
          for (const input of inputs) {
            ...
            if (key.startsWith("SITES.")) {
              ...
              if (this.SITES.hasOwnProperty(siteKey))
                this.SITES[siteKey] = val || this.SITES[siteKey];
            }
          }
          overlay.remove();
  ...
  init() {
    ...
    const currentSiteKey = Object.keys(this.SITE_CONFIG).find((site) =>
      this.utils.isIncludes(site),
    );
    if (currentSiteKey) {
