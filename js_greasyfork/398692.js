// ==UserScript==
// @name 51cto_get
// @match        *://edu.51cto.com/center/course/lesson/index?id=*
// @match        *://edu.51cto.com/course/*
// @language zh_cn
// @description  获取51cto的m3u8，建议配合N_m3u8dl-cli用，下载地址https://github.com/nilaoda/N_m3u8DL-CLI/releases
// @grant        none
// @version 20210412
// @run-at document-idle
// @supportURL https://greasyfork.org/zh-CN/scripts/398692-51cto-get/feedback
// @namespace https://greasyfork.org/users/70322
// @downloadURL https://update.greasyfork.org/scripts/398692/51cto_get.user.js
// @updateURL https://update.greasyfork.org/scripts/398692/51cto_get.meta.js
// ==/UserScript==

'use strict';
var map = new Map([
    [/edu\.51cto\.com\/center\/course\/lesson\/index.*/, function(){
        $('div.btns.downs').click(function(){
            var key = getKey(
                WordArray.create(new Uint8Array(edu_h5_player.hls.abrController.fragCurrent.decryptdata.key)).toString(Utf8).match(/\w/g).join(''),
                edu_h5_player.hls.abrController.fragCurrent.decryptdata._lid.replace(/_\d*/, "")
            );
            $.ajax({
                url: edu_h5_player.config.dispatch[0].url,
                dataType: 'text',
                success(s){
                    s = s.replace(/URI=".*?"/, `URI="base64:${key}"`);
                    var blob = new Blob([s], {type:'text/plain'}),
                        a = document.createElement('a');
                    a.download = $('li.lesson.cur>a span').slice(0,2).text() + '.m3u8';
    ...
    [/edu\.51cto\.com\/course/, function(){/*批量下载失效，暂屏蔽
        typeof(showTab)!='undefined' && showTab(1);
        var i = setInterval(() => {LoadList.loading.css('display')!=='none'?LoadList.loadNext():(clearInterval(i), replaceDiv())}, 1000);
        var btn_down = $('div.download.fl').find('p'),
            div = $('div.qrcode.tc'), chkBox;
        if(!div.length){
            btn_down = $('span.s2.fl.hasMore');
            div = btn_down.find('ol');
            btn_down = btn_down.find('i');
        }
        function replaceDiv(){
            var lesson_list = $('#syList>.lesson').not(['作业','练习'].map(e=>`:has(span:contains(${e}))`).join(',')),
                title_list = lesson_list.children('div').map((i,e)=>$(e).children('p').not('.look.green').text()),
                url_list = lesson_list.find('a').map((i,e)=>e.href);
            div.css({'width':'max-content','text-align':'left'}).empty();
            for(i=0;i<url_list.length;i++)
                $('<input type=checkBox />').add(`<a href="${url_list[i]}" target=_blank>${title_list[i]}</a><br/>`).appendTo(div);
            chkBox = div.find('input');
            $('<button>反选</button>').click(()=>chkBox.click()).add('<br/>').prependTo(div);
        }
        btn_down.click(async function(){
            var checked_url = chkBox.filter((i,e)=>e.checked).next('a').map((i,e)=>e.href).get(),
                checked_title = chkBox.filter((i,e)=>e.checked).next('a').map((i,e)=>e.innerText).get(),
                cli = 'chcp 65001\r\n', cli_path = '.\\N_m3u8DL-CLI_v2.6.0.exe';
            if(!checked_url.length) return;
            for(let [i,u] of checked_url.entries()){
                let m3u8_url, key, lid = u.match(/id=(\d+)/)[1], title = checked_title[i];
                cli += cli_path + ` "${title}.m3u8" --saveName "${title}" --enableDelAfterDone\r\n`;
                await new Promise(function(resolve){
                    $.ajax({
                        url:'https://edu.51cto.com/center/player/play/get-lesson-info?type=course&lesson_type=course&sign=&lesson_id=' + lid,
                        dataType: 'json',
                        success(res){
                            m3u8_url = res.dispatch[0].url;
                            key = getKey(res.key, lid);
                            resolve();
                        },
                        error(xhr, msg, e){
                            alert(`下载${this.url}出错了：\n ${msg}`);
                            throw e;
                        }
                    });
                });
                $.ajax({
                    ...
                    dataType: 'text',
                    success(res){
                        var blob = new Blob([res.replace(/URI=".*?"/, `URI="base64:${key}"`)], {type:'text/plain'});
                        $('<a/>').attr({
                    ...
                    error(xhr, msg, e){
                        alert(`下载文件${this.url}出错了:\n`+msg);
                        throw e;
                    }
                })
                document.title = `${i+1}/${checked_url.length}`;
            }
            $('<a/>').attr({
                'href': URL.createObjectURL(new Blob([cli], {type:'text/plain;charset=cp866'})),
                'download': ($('div.title')[0].title || CourseTitle.textContent || 'run') + '.bat',
                'target': '_blank'
            })[0].click();
        });
    */}]
...
window.onload = function(){
    map.get([...map.keys()].find(e => e.test(location.href)))();
};
