...
// @copyright           2014 FZ69617
// @downloadURL https://update.greasyfork.org/scripts/5403/WME%20GPS%20Track%20View.user.js
// @updateURL https://update.greasyfork.org/scripts/5403/WME%20GPS%20Track%20View.meta.js
// ==/UserScript==

var wmech_version = "1.2.4";

/*
 * Version history:
 *
 * 1.2.4 (20141005)
 * - New: Tracks layer summary with clean tracks button.
 * - Revert: Temporary from previous version removed.
 *
 * 1.2.3 (20141004)
 * - Improvement: Map panning to the center of added points.
 * - Temporary: Tracks layer cleaning before track addition.
 * - Other: Minor code changes.
 */



var gps_num_tracks = 0;
var gps_num_points = 0;
var gps_total_distance = 0;
...
function bootstrap_WME_GPS_Track()
{
  ...
  if (typeof unsafeWindow === "undefined" || ! bGreasemonkeyServiceDefined) {
    ...
    console.log("WME GPS Track View " + wmech_version + " loaded.");
}
//---------------------------------------------------------------------------------------
...
function gps_parse()
{
    ...
    var obj2 = document.getElementById('wme_gps_layer_info');
    ...
    if(layers.length != 0) {
        ...
        if (lineFeatures.length > 0 || pointFeatures.length > 0) {
            ...
            for (var i = 0; i < pointFeatures.length; ++i) {
                var point = pointFeatures[i];
                bounds.extend(point.geometry.bounds);
            }

            var center = bounds.getCenterPixel();
            var zoom = unsafeWindow.Waze.map.getZoom();
            unsafeWindow.Waze.map.setCenter([center.x, center.y], zoom);
        }
    }
    ...
    if (lineFeatures.length > 0) {
        ...
        gps_num_tracks += 1;
        gps_num_points += numPoints;
        gps_total_distance += sumDist;
    }
    
    obj2.innerHTML = 'Processed '

        + numPoints + ' point' + (numPoints != 1 ? 's' : '') + ', '
        + parseInt(sumDist/10) / 100 + ' km';
...
function updateSummary() {
    var msg = '';
    var clear_display = 'inline';
    
    if (gps_num_tracks == 0) {
        msg = 'No tracks';
        clear_display = 'none';
    } else {
        msg = gps_num_tracks + ' track' + (gps_num_tracks != 1 ? 's' : '') + ', '
            + gps_num_points + ' point' + (gps_num_points != 1 ? 's' : '') + ', '
            + parseInt(gps_total_distance/10) / 100 + 'km';
    }
    
    document.getElementById('wme_gps_layer_summary').innerHTML = msg;
    document.getElementById("wme_gps_layer_clear_button").style.display = clear_display;
}
//---------------------------------------------------------------------------------------
function Draw_WME_GPS_Track()
{
    if(document.getElementById('wme_gps_layer') == null) {
        ...
        var add_btn = $('<button id="wme_gps_layer_add_button" title="" >Add track</button>');
        add_btn.click(gps_parse);

        var clear_btn = $('<button id="wme_gps_layer_clear_button" title="" style="display:none;" >Clear tracks</button>');
        clear_btn.click(gps_clear);
        ...
        var info = '<p style="font-family: Tahoma; font-size:9px; color:#808080; " id="wme_gps_layer_info" ></p>';

        var summary = '<p id="wme_gps_layer_summary" ></p>';

        $("#sidepanel-drives").append(cnt);
        $("#sidepanel-drives").append(add_btn);
        $("#sidepanel-drives").append(spd);
        $("#sidepanel-drives").append(info);
        $("#sidepanel-drives").append(summary);
        $("#sidepanel-drives").append(clear_btn);
