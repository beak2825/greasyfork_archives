// ==UserScript==
// @name         蜜蜂收代充
// @namespace    http://tampermonkey.net/
// @version      0.3
// @description  try to take over the world!
// @author       You
// @match        http://www.mf178.cn/customer/qpay/mytasks*
// @match        http://www.mf178.cn/customer/order/pools
// @match        http://www.mf178.cn/customer/qpay/pools
// @match        http://www.mf178.cn/customer/qpay/union*
// @match        http://www.mf178.cn/customer/order/mytasks
// @match        http://www.mf178.cn/customer/qpay/pools_union
// @grant        GM_getValue
// @grant        GM_setValue
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/372177/%E8%9C%9C%E8%9C%82%E6%94%B6%E4%BB%A3%E5%85%85.user.js
// @updateURL https://update.greasyfork.org/scripts/372177/%E8%9C%9C%E8%9C%82%E6%94%B6%E4%BB%A3%E5%85%85.meta.js
// ==/UserScript==

(function() {
    'use strict';

    // Your code here...

    const str_url = "http://www.mf178.cn/customer/qpay/mytasks";

    var noti_sound = new Audio("data:audio/wav;base64,");
    //var noti_sound = new Audio("data:audio/wav;base64,");
    //var noti_sound = new Audio("data:audio/x-wav;base64,");

    var BEEP_INTERVAL=13000;//ms

    function play_sound(left){
        noti_sound.play();
        if(left>1){
            setTimeout(play_sound, BEEP_INTERVAL, --left);
        }
    }

    //const value = "1"; //移动微信支付宝100元
    //const value = "2"; //移动微信支付宝200元
    //const value = "3"; //移动微信支付宝300元
    //const value = "5"; //移动微信支付宝500元
    //const value = "11"; //电信微信支付宝100
    //const value = "12"; //电信微信支付宝200
    //const value = "13"; //电信微信支付宝300
    //const value = "15"; //电信微信支付宝500

    //const value = "42"; //移动微信支付宝200-500元
    //const value = "41"; //移动微信支付宝500-2500元

    //const value = "31"; //无损移动微信支付宝90-100

    //const value = "21"; //联通微信支付宝100
    //const value = "22"; //联通微信支付宝200
    //const value = "23"; //联通微信支付宝300
    //const value = "25"; //联通微信支付宝500

    const value = "200";

    // var str = "javascript:getTask(" + value + ");";
    //$('a.btn.btn-default.btn-info').attr("href",str);

    //return;

    //play_sound(1);

    $(document).scrollTop(($(document).height()-$(window).height()*1.15));

    var result = 1;
    if($('a.btn.btn-info').length>0){
        result = 2;
    }

    if($('a.btn.btn-danger').length>0){
       result = 3;
    }
    //alert(result);
    if(result == 2 || result == 1 ){
         //var ind4=self.setInterval(function () {
         //clearInterval(ind4);
        getTask(value);
        var ind0=self.setInterval(function () {
            clearInterval(ind0);
            //$('button.btn.btn-info').click();
            //$('#id').focus();
            //$('#count').focus();
            //$('[name=contract[]]:checkbox').prop('checked', this.checked);
            $('#submit').click();
        }, 4650);
        //}, 3500);
        //$('button.btn.btn-info').click();
        /*if($('#submit').length == 0){
           // $('a.btn.btn-default.btn-info')[0].click();
            getTask(6);
        } else {
            $('#submit').click();
            $(document).scrollTop($(document).height()-$(window).height());
            $(document).scrollTop(0);

            if($('a.btn.btn-xs.btn-success').length == 0){
                var ind1=self.setInterval(function () {
                    window.location.href = str_url;
                    clearInterval(ind1);
                }, 5000);
            }
            return;
        }

        $(document).scrollTop($(document).height()-$(window).height());
        $(document).scrollTop(0);
        $(document).scrollTop($(document).height()-$(window).height());
        $(document).scrollTop(0);*/

        if($('a.btn.btn-danger').length == 0){
            var ind1=self.setInterval(function () {
                clearInterval(ind1);
                window.location.href = str_url;
            }, 6000);
        }
    } else if (result == 3){
        var idx = $('a.btn.btn-success').length -1;
        //alert(idx);
        $('a.btn.btn-success')[idx].click();
        document.title = '付钱！快付钱！';
        play_sound(30);
        //alert("--------------------------------------");
    }
    /* else if(result == 1){
         var ind2=self.setInterval(function () {
             clearInterval(ind2);
             window.location.href = str_url;
         }, 7000);
   }*/

})();