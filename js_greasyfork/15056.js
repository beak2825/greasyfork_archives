// ==UserScript==
// @name        C&C:TA AutoUpgrade
// @namespace   CnC_TA_AutoUpgrade
// @author  	Vulcano
// @version     1.1.0.3
// @date        2015-10-02
...
// @description	Autoupgrade with one Click
// @include		http*://prodgame*.alliances.commandandconquer.com/*/index.aspx*
// @grant		none
// @downloadURL https://update.greasyfork.org/scripts/15056/CC%3ATA%20AutoUpgrade.user.js
// @updateURL https://update.greasyfork.org/scripts/15056/CC%3ATA%20AutoUpgrade.meta.js
// ==/UserScript==

(function (){
	var AutoUpgrade_main =  function() {
		try {
			/*function CCTAWrapperIsInstalled() {
				return (typeof (CCTAWrapper_IsInstalled) != 'undefined' && CCTAWrapper_IsInstalled);
			}*/
			
			function createAutoUpgrade() {
                console.log('C&C:TA AutoUpgrade loading ...');
				
				qx.Class.define("AutoUpgrade.Main", {
					type: "singleton",
					...
					members: {
						...
						initialize: function() {
							var _this = window.AutoUpgrade.Main.getInstance();
						
							console.log('AutoUpgrade initialize start');
							...
							button = new qx.ui.form.Button("Auto-Upgrade: AUS", "http://goo.gl/s1Gvne").set({
								width: 40,
								height: 40,
							...
							_this.FehlerWindow.addListener("close", function(e) {
								var _this = window.AutoUpgrade.Main.getInstance();
								if (window.AutoUpgrade.Main.getInstance().autoUpdateHandleBuilding != null) {
									_this.geb = 0;
									window.AutoUpgrade.Main.getInstance().BstopAutoUpdate();
									BuildingsButton.setLabel("Gebäude AUS");
									BuildingsButton.setAppearance("button-text-small");
								} 
								if (window.AutoUpgrade.Main.getInstance().autoUpdateHandleDefense != null) {
									_this.def = 0;
									window.AutoUpgrade.Main.getInstance().DstopAutoUpdate();
									DefenseButton.setLabel("Verteidigung AUS");
									DefenseButton.setAppearance("button-text-small");
								} 
								if (window.AutoUpgrade.Main.getInstance().autoUpdateHandleOffense != null) {
									_this.off = 0;
									window.AutoUpgrade.Main.getInstance().OstopAutoUpdate();
							...
							BuildingsButton.addListener("click", function (e) {
								...
								if (window.AutoUpgrade.Main.getInstance().autoUpdateHandleBuilding != null) {
									...
									_this.geb = 1;
									window.AutoUpgrade.Main.getInstance().BuildingstartAutoUpdate();
							...
							DefenseButton.addListener("click", function (e) {
								...
								if (window.AutoUpgrade.Main.getInstance().autoUpdateHandleDefense != null) {
									...
									_this.def = 1;
									window.AutoUpgrade.Main.getInstance().DefensestartAutoUpdate();
							...
							OffenseButton.addListener("click", function (e) {
								...
								if (window.AutoUpgrade.Main.getInstance().autoUpdateHandleOffense != null) {
									...
									_this.off = 1;
									window.AutoUpgrade.Main.getInstance().OffensestartAutoUpdate();
							...
							CommandBuildingChoice.addListener("click", function (e) {
								...
								if (_this.x != 0) {
									_this.x = 0;
									window.AutoUpgrade.Main.getInstance().OffFunction();
									...
									_this.x = 1;
									window.AutoUpgrade.Main.getInstance().OnFunction();
							...
							worldResBuildingChoice.addListener("click", function (e) {
								  var _this = window.AutoUpgrade.Main.getInstance();
								if (window.AutoUpgrade.Main.getInstance().AautoUpdateHandle != null) {
									window.AutoUpgrade.Main.getInstance().OffFunction();
...
//							app.getDesktop().add(button1, { // Option-Button
//								right: 123,
//								bottom: 90
//							});		
						...
						autoUpgradePopupOeffnen : function(){
							...
							var lblINFO = new qx.ui.basic.Label("rewritten by Vulcano @ <a href='http://Vulcanion.com\' target='_blank'>Vulcanion.com</a><br><br><div align='center'><a href='http://Vulcanion.com'><img src='http://goo.gl/mNx5ds' width='126' height='130' /></a></div>").set({rich: "true", alignX: "center", alignY: "center", font: "font_size_14_bold"});
                        ...
                        EinAusLabel : function (sender) {
							...
							button.setIcon("http://goo.gl/s1Gvne");
							...
							button.setIcon("http://goo.gl/HF0Xzf");
							button.set({toolTipText:ttt});
						...
						canUpgradeBuilding: function (building, city) {
							var _this = AutoUpgrade.Main.getInstance();
...
**************************************************************************************************************************************************************************************************************************************************************************************************************************************							  
	*/								
			...
			console.log("createAutoUpgrade: ", e);
		}
		
		function AutoUpgrade_checkIfLoaded() {
			try {
			if (typeof qx != 'undefined' && qx.core.Init.getApplication() && qx.core.Init.getApplication().getUIItem(ClientLib.Data.Missions.PATH.BAR_NAVIGATION) && qx.core.Init.getApplication().getUIItem(ClientLib.Data.Missions.PATH.BAR_NAVIGATION).isVisible()) {
					...
					if (typeof ClientLib.API.Util.GetUnitMaxHealth == 'undefined'){
							for (var key in ClientLib.Base.Util)
							{
								...
								{
										AutoUpgrade.Main.getInstance().GetUnitMaxHealth = ClientLib.Base.Util[key];
										console.log("AutoUpgrade.Main.getInstance().GetUnitMaxHealth = ClientLib.Base.Util["+key+"]");
										break;
								}
							}
						}else{
							AutoUpgrade.Main.getInstance().GetUnitMaxHealth = ClientLib.API.Util.GetUnitMaxHealth;	
                ...
                console.log("ClientLib.Data.CityUnits.prototype.get_DefenseUnits = function(){var $createHelper;return this." + fn_name + ";}");
					...
					AutoUpgrade.Main.getInstance();
					window.AutoUpgrade.Main.getInstance().initialize();
				} else {
					window.setTimeout(AutoUpgrade_checkIfLoaded, 1000);
				}
			} catch (e) {
				console.log("AutoUpgrade_checkIfLoaded: ", e);
			}
		}
	...
	try
	{
		...
		FlunikScript.innerHTML = "(" + AutoUpgrade_main.toString() + ")();";
		FlunikScript.type = "text/javascript";
		if (/commandandconquer\.com/i.test(document.domain)) {
			document.getElementsByTagName("head")[0].appendChild(FlunikScript);
		}
	} catch (e) {
		console.log("AutoUpgrade: init error: ", e);
	}
	console.log('C&C:TA AutoUpgrade loaded successfully !!!');
})();