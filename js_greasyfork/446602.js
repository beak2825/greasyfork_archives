...
// @downloadURL https://update.greasyfork.org/scripts/446602/Level4.user.js
// @updateURL https://update.greasyfork.org/scripts/446602/Level4.meta.js
// ==/UserScript==
...
var EFFICIENCY = 1.1; // Lower: Slower and more expensive hands. Higher: Faster and cheaper hands. Default: 1.0, Minimum: 0
var SAFETY = 0.9; // Lower: The bot will not pay much attention to safety. Higher: The bot will try to play safer. Default: 1.0, Minimum: 0
var SAKIGIRI = 1.0; //Lower: Don't place much importance on Sakigiri. Higher: Try to Sakigiri more often. Default: 1.0, Minimum: 0

//CALL CONSTANTS
var CALL_PON_CHI = 1.25; //Lower: Call Pon/Chi less often. Higher: Call Pon/Chi more often. Default: 1.0, Minimum: 0
var CALL_KAN = 0.9; //Lower: Call Kan less often. Higher: Call Kan more often. Default: 1.0, Minimum: 0

//STRATEGY CONSTANTS
var RIICHI = 1.2; //Lower: Call Riichi less often. Higher: Call Riichi more often. Default: 1.0, Minimum: 0
var CHIITOITSU = 4; //Number of Pairs in Hand to go for chiitoitsu. Default: 5
var THIRTEEN_ORPHANS = 9; //Number of Honor/Terminals in hand to go for 13 orphans. Default: 10
...
function makeCall(type) {
	if (MODE === AIMODE.AUTO) {
		...
		showCrtStrategyMsg(`${getCallNameByType(type)}寄りかな`);
	}
}
...
function makeCallWithOption(type, option) {
	if (MODE === AIMODE.AUTO) {
		...
		showCrtStrategyMsg(`${option}: ${getCallNameByType(type)}寄りかな`);
	}
}
...
function sendRiichiCall(tile, moqie) {
	if (MODE === AIMODE.AUTO) {
		...
		showCrtStrategyMsg(`${tileName}リーチ！`);
	}
}
...
function callDiscard(tileNumber) {
	if (MODE === AIMODE.AUTO) {
		...
		showCrtStrategyMsg(`${tileName}かな〜`);
