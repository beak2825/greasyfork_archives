...
// @name         IQOOZ BYPASSER
// @author       IQOOZ
// @description  All In One Bypass

// @match        https://mobile.codex.lol/*
// @match        https://iqoozdev.tech/*
...
// @license      MIT
// @supportURL   https://discord.com/users/987184688324030474
// @icon         https://files.catbox.moe/zr11gj.jpg
// @namespace    https://greasyfork.org/en/users/1257398-iqooz
// @downloadURL https://update.greasyfork.org/scripts/486919/IQOOZ%20BYPASSER.user.js
// @updateURL https://update.greasyfork.org/scripts/486919/IQOOZ%20BYPASSER.meta.js
// ==/UserScript==
...
const { handleError, sleep, linkvertiseSpoof, getTurnstileResponse, getGrecaptchaResponse, notifier } = __webpack_require__(223)
...
async function codex() {
    ...
    try {
        while (stagesCompleted < stages.length) {
            ...
            notifier.info(`${stagesCompleted + 1}/${stages.length} stages completed`, 1000);
        ...
        if (authenticate(validatedTokens)) {
            notifier.success('bypassed successfully');
            await sleep(3000);
...
async function fluxus() {
    ...
    try {
        document.getElementById('noUserscript').innerHTML = '<h1>Bypass Now:</h1>';
        ...
        if (fluxusLink && fluxusLinkRegex.test(fluxusLink)) {
            ...
            document.getElementById('noUserscript').innerHTML = '<h1>Always Use This Page For Fluxus Link</h1><h2>Bypassing. . .<br>(Come To This Page For No Delay)</h2>';
            await sleep(16000);
        }
        ...
        document.getElementById('noUserscript').innerHTML = '<h1>Bypassing. . .</h1>';
        inputBox.disabled = true;
...
async function arceus() {
    try {
        ...
        while (checkpointsDone <= 3) {
            ...
            notifier.info(`${checkpointsDone}/3 stages completed`, 1900);
            await sleep(2000);
            checkpointsDone++;
        }
        notifier.success('bypassed successfully');
...
function handleError(error) {
    ...
    GM_notification({
        text: errorText,
        title: "ERROR",
        url: 'https://discord.com/users/987184688324030474',
        silent: true,
    });
    GM.openInTab('https://discord.com/users/987184688324030474');
}
...
async function getTurnstileResponse() {
    let notif = setInterval(notification, 3000, 'please solve the captcha', 5000)
    let res = '';
    while (true) {
        try {
...
const { sleep, notifier } = __webpack_require__(223);
...
async function start() {
    ...
    switch (window.location.hostname) {
        ...
        case 'keysystem.fluxteam.net': {
            window.location.assign('https://iqoozdev.tech/?link=' + encodeURIComponent(window.location.href));
            break;
        }
        case 'spdmteam.com': {
            await arceus();
        ...
        default: {
            notifier.alert('unsupported keysystem');
            break;
        }
    }
}
