...
// @downloadURL https://update.greasyfork.org/scripts/10675/Hack-Free%20QuickAnalyst.user.js
// @updateURL https://update.greasyfork.org/scripts/10675/Hack-Free%20QuickAnalyst.meta.js
// ==/UserScript==
...
var customText = [
  ...
  '[align=center][size=medium][b][color=#1E90FF]MD5 :[/color][/b] [color=#FFFFFF]',
  '[/color] [/size][/align]',
  '[align=center][size=medium][b][color=#1E90FF]Scan :[/color][/b] [url]',
  '[/url] [/size][/align]'
];
var customTextTitle = [
  'Clean',
...
function insertLinkId(i) {
  var prevMessage = document.getElementById('message').value;
  document.getElementById('message').value = prevMessage + customText[i] + customText[5] + document.createElement("<br />") + document.getElementById('LinkId').value + document.createElement("<br />") + customText[6];
  if (sendFor[i]) {
    document.getElementById('quick_reply_submit').click()
  }
}
exportFunction(insertLinkId, unsafeWindow, {
  defineAs: 'insertLinkId'
});
if (document.URL.indexOf('Thread-') >= 0) {
  ...
  if (customTextTitle[5] !== '') {
    buttonsHTML += '<form name=\'LinkArea\'> <input type=\'button\' onClick=\'insertLinkId(1)\' value=\'Clean\'/> <input type=\'text\' name=\'Link\' id=\'LinkId\' value=\'http://\'/> </form>';
  }
  buttonsHTML += '</div>';
