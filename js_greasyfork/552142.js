...
// @grant        none
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/552142/Multitool%20Supervisor%20-%202.user.js
// @updateURL https://update.greasyfork.org/scripts/552142/Multitool%20Supervisor%20-%202.meta.js
// ==/UserScript==
...
width: '200px', // Условная ширина модального окна
   ...
   //     container.appendChild(infoIcon);
    ...
    checkboxContainer.appendChild(createCheckbox('Автоклик виджет', 'autoClickWidget', 'Автоматический клик по кнопке виджета при загрузке страницы.'));
    ...
    // Контейнер для настроек позиции диалога
    const positionContainer = document.createElement('div');
    Object.assign(positionContainer.style, { marginBottom: '15px' });

    const positionLabel = document.createElement('label');
    positionLabel.textContent = 'Позиция диалога';
    ...
    const positionInputsContainer = document.createElement('div');
    Object.assign(positionInputsContainer.style, {
        display: 'flex',
        gap: '10px',
        alignItems: 'center'
    });
    ...
    topInput.type = 'text';
    topInput.id = 'dialogTop';
    topInput.value = localStorage.getItem('dialogTop') || '100';
    topInput.placeholder = '100';
    topInput.addEventListener('input', () => localStorage.setItem('dialogTop', topInput.value));
    ...
    leftInput.type = 'text';
    leftInput.id = 'dialogLeft';
    leftInput.value = localStorage.getItem('dialogLeft') || '100';
    leftInput.placeholder = '100';
    leftInput.addEventListener('input', () => localStorage.setItem('dialogLeft', leftInput.value));
    ...
    applyButton.addEventListener('click', () => {
        const dialogPositionEnabled = localStorage.getItem('dialogPosition') === 'true';

        if (!dialogPositionEnabled) {
            console.log('Функция позиции диалога отключена');
            return;
        }

        const topValue = topInput.value || '100';
        const leftValue = leftInput.value || '100';

        // Применяем позицию к диалогу
        const dialog = document.querySelector('body > div.ui-dialog.ui-widget.ui-widget-content.ui-corner-all.ui-front.ui-dialog-buttons.ui-draggable.ui-resizable');
        if (dialog) {
            dialog.style.top = topValue + 'px';
            dialog.style.left = leftValue + 'px';
            console.log(`Позиция диалога изменена: top=${topValue}px, left=${leftValue}px`);
        } else {
    ...
    positionInputsContainer.appendChild(topLabel);
    positionInputsContainer.appendChild(topInput);
    positionInputsContainer.appendChild(leftLabel);
    positionInputsContainer.appendChild(leftInput);
    positionInputsContainer.appendChild(applyButton);

    positionContainer.appendChild(positionInputsContainer);
    ...
    function autoClickWidget() {
        ...
        if (autoClickEnabled) {
            ...
            if (onlyInActiveTask) {
                ...
                const hasActiveTask = activeTaskMessage && activeTaskMessage.textContent.trim() === 'Product is in your active task';

                // Проверяем статус ModerationComplete
                const hasModerationCompleteStatus = statusSelect && statusSelect.querySelector('option[selected="selected"][value="12"]');
...
// Автоматическое применение позиции диалога
(function() {
    ...
    function applyDialogPosition() {
        ...
        if (!dialogPositionEnabled) {
            return; // Если функция отключена, ничего не делаем
        }

        const savedTop = localStorage.getItem('dialogTop');
        const savedLeft = localStorage.getItem('dialogLeft');

        if (savedTop || savedLeft) {
            const dialog = document.querySelector('body > div.ui-dialog.ui-widget.ui-widget-content.ui-corner-all.ui-front.ui-dialog-buttons.ui-draggable.ui-resizable');
            if (dialog) {
                const currentPosition = `${savedTop || 'default'},${savedLeft || 'default'}`;

                // Логируем только при изменении позиции
                if (currentPosition !== lastAppliedPosition) {
                    console.log(`Применена сохраненная позиция виджета: top=${savedTop || 'default'}px, left=${savedLeft || 'default'}px`);
                    lastAppliedPosition = currentPosition;
                }

                if (savedTop) {
                    dialog.style.top = savedTop + 'px';
                }
                if (savedLeft) {
                    dialog.style.left = savedLeft + 'px';
                }
            }
        }
    }

    // Применяем позицию при загрузке страницы
    if (document.readyState === 'complete') {
        applyDialogPosition();
    } else {
        window.addEventListener('load', applyDialogPosition);
    }

    // Также применяем позицию при появлении диалога (с задержкой)
    const observer = new MutationObserver(() => {
        const dialog = document.querySelector('body > div.ui-dialog.ui-widget.ui-widget-content.ui-corner-all.ui-front.ui-dialog-buttons.ui-draggable.ui-resizable');
        if (dialog) {
            setTimeout(applyDialogPosition, 100); // Небольшая задержка для корректного применения
        }
    });
