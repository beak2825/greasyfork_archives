...
(function () {
    ...
    function saveSettings() {
        try {
            ...
            GM_setValue("bazaarLayoutMode", scriptSettings.layoutMode || "default");
        } catch (e) {
    ...
    style.textContent = `
        ...
        .bazaar-info-container.compact-layout .bazaar-scroll-arrow {
            display: none;
        }
        .bazaar-info-container.compact-layout .bazaar-scroll-wrapper {
            overflow-x: hidden;
            overflow-y: auto;
            max-height: 200px;
            height: auto;
        ...
        .bazaar-info-container.compact-layout .bazaar-card-container {
            flex-direction: column;
            align-items: stretch;
            gap: 6px;
            height: auto;
        }
        .bazaar-info-container.compact-layout .bazaar-listing-card {
            position: static;
            display: flex;
            flex-direction: row;
            flex-wrap: wrap;
        ...
        .bazaar-info-container.compact-layout .card-info-item {
            white-space: nowrap;
        }
        .bazaar-info-container.compact-layout .bazaar-player-link {
            font-weight: bold;
            text-decoration: underline;
            white-space: nowrap;
            overflow: hidden;
        ...
        .bazaar-info-container.compact-layout .bazaar-listing-footnote {
            margin-left: auto;
            text-align: left;
        }
    `;
    ...
    function createListingCard(listing, index) {
        ...
        const priceComparisonHTML = getPriceComparisonHtml(listing.price, listing.quantity);

        if (scriptSettings.layoutMode === 'compact') {
            card.innerHTML = `
                <a href="https://www.torn.com/bazaar.php?userId=${listing.player_id}&itemId=${listing.item_id}&highlight=1#/"
                    ...
                    class="bazaar-player-link" style="color:${visitedColor};">
                    ${displayName}
                </a>
                <div class="card-info-item"><strong>Price:</strong> <span>$${listing.price.toLocaleString()}</span></div>
                <div class="card-info-item"><strong>Qty:</strong> <span>${listing.quantity}</span></div>
                ${priceComparisonHTML}
                <div class="bazaar-listing-footnote">${getRelativeTime(listing.updated)}</div>
            `;
        } else {
                ...
                <div>
                    ...
                    <div>
                        ...
                        <div style="display:flex; align-items:center">
                            ...
                            <span style="margin-left:auto">${priceComparisonHTML}</span>
                        </div>
                    </div>
                </div>
    ...
    function renderVirtualCards(infoContainer) {
        ...
        try {
            ...
            if (scriptSettings.layoutMode === 'compact') {
                infoContainer.classList.add('compact-layout');
            } else {
                infoContainer.classList.remove('compact-layout');
            }
            ...
            if (scriptSettings.layoutMode === 'compact') {
                ...
                if (countElement) {
                    if (minQty > 0 && filteredListings.length < allListings.length) {
                        countElement.textContent = `Showing ${filteredListings.length} of ${allListings.length} bazaars (${totalQuantity.toLocaleString()} items, min qty: ${minQty})`;
                    } else {
                        countElement.textContent = `Showing ${filteredListings.length} bazaars (${totalQuantity.toLocaleString()} items total)`;
                    }
                }
            } else {
    ...
    function createInfoContainer(itemName, itemId) {
        ...
        sortControls.innerHTML = `
            ...
            <button class="bazaar-button bazaar-layout-toggle" title="Toggle Card Layout">
                ${scriptSettings.layoutMode === 'compact' ? 'Compact': 'Default'}
            </button>
            <span style="margin-left: 8px;">Min Qty:</span>
            <input type="number" class="bazaar-min-qty" style="width: 60px; padding: 3px; border: 1px solid #ccc; border-radius: 4px;" min="0" placeholder="" value="1">
        `;
    ...
    if (window.location.href.includes("bazaar.php")) {
        ...
        if (targetItemId && highlight === "1") {
            ...
            function highlightItem() {
                ...
                const card = img ? img.closest('.item___GYCYJ') : null;
                if (card && !card.classList.contains('green-outline')) {
            ...
            if (container) {
                ...
                const itemObserver = new MutationObserver(() => {
                    if (highlightItem()) itemObserver.disconnect();
                });
    ...
    document.body.addEventListener('click', event => {
        ...
        if (event.target.matches('.bazaar-layout-toggle')) {
            const button = event.target;
            scriptSettings.layoutMode = scriptSettings.layoutMode === 'default' ? 'compact' : 'default';
            button.textContent = scriptSettings.layoutMode === 'compact' ? 'Compact' : 'Default';
            saveSettings();
            performSort(container); // is this the right func?
        }
    });
                ...
                <div class="bazaar-settings-group">
                    ...
                    <div class="bazaar-settings-item">
                        ...
                        <select id="bazaar-layout-mode">
                            <option value="default" ${scriptSettings.layoutMode === 'default' ? 'selected' : ''}>Default (Horizontal Scroll)</option>
                            <option value="compact" ${scriptSettings.layoutMode === 'compact' ? 'selected' : ''}>Compact (Vertical List)</option>
                        </select>
                        <div class="bazaar-api-note">
                            Choose the display style for bazaar listings.
                        </div>
                    </div>
                </div>
            </div>
    ...
    function saveSettingsFromModal(modal) {
        ...
        scriptSettings.layoutMode = modal.querySelector('#bazaar-layout-mode').value;
