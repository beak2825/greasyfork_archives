...
// @downloadURL https://update.greasyfork.org/scripts/407807/BrokenStones%20-%20Upload%20App%20Helper.user.js
// @updateURL https://update.greasyfork.org/scripts/407807/BrokenStones%20-%20Upload%20App%20Helper.meta.js
// ==/UserScript==

//config (default)
var imgur_clientid = ""; // "97642ecbe2c74b6" personal imgur key
...
function get_date(str) {
    str = str.replace(/([0-9]{4})\-([0-9]{2})\-([0-9]{2}).*/, " ($3.$2.$1)");
    return str;
}
...
function fill_search(response) {
    ...
    results.forEach(function(result, index) {
        ...
        var title = macupdate ? get_title(result.querySelector("div.mu_card_line_info_name").innerText): get_title(result.trackName),
            title_hash = title.toLowerCase().replace(/(\s|\-|:|#|'|!|&|\?|\/|\.)/g, "").replace(/^(\d+)/g, "n$1"),
    ...
    if (ah_search_loaded == 0) {
        var hash = ah_input.value.toLowerCase().replace(/(\s|\-|:|#|'|!|&|\?|\/|\.)/g, "").replace(/^(\d+)/g, "n$1");
...
function fill_release_desc() {
    if (!ah_release_desc_lock) {
        var release_source = ah_source.value != "" ? ah_source.value + " " : "",
            ...
            release_desc = ah_instructions + release_languages + label_version + release_source + release_version + release_changelog + release_cure;
...
function fill_form(response) {
    ...
    var macupdate = response.responseText.includes("DOCTYPE") ? true : false,
        ...
        bit64 = macupdate ? sysreq.split("<li>")[1].split("</li>")[0] : "",
    ...
    if(macupdate && itunesid) {
        var itunes_request = new GM.xmlHttpRequest({
            method: "GET",
        ...
        if (ah_store_id.value != "") {
            if (ah_store_id.value == "0") {
                var itunesid = ah_store_id.getAttribute("itunesid"),
                    itunes_request = new GM.xmlHttpRequest({
                        method: "GET",
                        url: "https://itunes.apple.com/lookup?id=" + itunesid,
                        responseType: "json",
                        onload: fill_form
                ...
                var appid = ah_store_id.value,
                    mu_request = new GM.xmlHttpRequest({
                    method: "GET",
