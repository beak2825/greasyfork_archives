// ==UserScript==
// @name        HIT Scraper WITH EXPORT AND Notifications - TM addon
// @author      Kerek, Tjololo, clickhappier, and feihtality, also yo momma
...
// @downloadURL https://update.greasyfork.org/scripts/11500/HIT%20Scraper%20WITH%20EXPORT%20AND%20Notifications%20-%20TM%20addon.user.js
// @updateURL https://update.greasyfork.org/scripts/11500/HIT%20Scraper%20WITH%20EXPORT%20AND%20Notifications%20-%20TM%20addon.meta.js
// ==/UserScript==
...
function testie(name,mode){
var title = name["title"]
var prevlink = name["prev_link"].split("=");
    if (mode == "pwr"){   
        var watcher = {
						id          : prevlink[1],
						duration    : 1000,
    ...
    if (global_run == true)
    {
        ...
        else
        {
            ...
            for (var j = 0; j < found_key_list.length; j++)
            {
                ...
                var pandalisted = include_check(obj.requester.replace(" (search)", ""),obj.title);
                if (!(obj.masters == true && masters_hide.checked) && (ignored && obj.found_this_time) || (!ignored && obj.found_this_time)){
                    ...
                    if (!pandalisted && obj.found_this_time){
                        
                        //alert(obj.requester + obj.description);
