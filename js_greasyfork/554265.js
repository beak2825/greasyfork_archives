...
// @version      2.2.2
// @description  Augment注册流程完全自动化 - 一键开始，全程自动（集成Duck.com隐私邮箱API）
// @author       Trace
...
// @connect      quack.duckduckgo.com
// @run-at       document-idle
// @downloadURL https://update.greasyfork.org/scripts/554265/Augment%E8%87%AA%E5%8A%A8%E6%B3%A8%E5%86%8C.user.js
// @updateURL https://update.greasyfork.org/scripts/554265/Augment%E8%87%AA%E5%8A%A8%E6%B3%A8%E5%86%8C.meta.js
// ==/UserScript==
...
(function() {
    ...
    GM_addStyle(`
        ...
        #augment-menu-container.collapsed {
        ...
        .aug-settings-btn.primary {
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%) !important;
            color: white !important;
        }

        .aug-settings-btn.primary:hover {
            transform: translateY(-2px) !important;
            box-shadow: 0 4px 12px rgba(102, 126, 234, 0.4) !important;
        }

        .aug-settings-btn.secondary {
            background: #f3f4f6 !important;
            color: #374151 !important;
        }

        .aug-settings-btn.secondary:hover {
            background: #e5e7eb !important;
        }

        .aug-settings-btn.danger {
            background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%) !important;
            color: white !important;
        }

        .aug-settings-btn.danger:hover {
            transform: translateY(-2px) !important;
    ...
    // TempMail.Plus 配置（私有服务器配置 - 用于接收验证码）
    ...
    /**
     * Duck.com API 客户端 (简化版)
     * 所有复杂逻辑(Token管理、代理配置)都在服务器端处理
     * 客户端只需要知道服务器地址即可
     */
    class DuckAPITampermonkey {
        /**
         * 构造函数
         * @param {string} serverUrl - 服务器地址,默认为 http://home.chaogei.asia:32582
         */
        constructor(serverUrl = '') {
            // 从 GM_getValue 读取服务器地址,如果不存在则使用默认值
            this.serverUrl = GM_getValue('duck_server_url', serverUrl || 'http://home.chaogei.asia:32582');

            // 检查是否在油猴环境中
            if (typeof GM_xmlhttpRequest === 'undefined') {
                console.error('DuckAPITampermonkey: GM_xmlhttpRequest 未定义,请确保在油猴脚本中使用并添加 @grant GM_xmlhttpRequest');
            }

            console.log('🦆 Duck API 客户端已初始化');
            console.log('   服务器地址:', this.serverUrl);
        }

        /**
         * 更新服务器地址
         * @param {string} newServerUrl - 新的服务器地址
         */
        updateServerUrl(newServerUrl) {
            this.serverUrl = newServerUrl;
            GM_setValue('duck_server_url', newServerUrl);
            console.log('✅ 服务器地址已更新:', newServerUrl);
        }
        ...
        /**
         * 生成 Duck.com 隐私邮箱
         * 调用服务器 API,所有复杂逻辑在服务器端处理
         * @returns {Promise<Object>} 包含生成结果的Promise对象
         */
        async generateEmail() {
            console.log('🦆 开始调用服务器 API 生成 Duck.com 邮箱...');
            console.log('   服务器地址:', this.serverUrl);

            return new Promise((resolve, reject) => {
                GM_xmlhttpRequest({
                    method: 'POST',
                    url: `${this.serverUrl}/api/duck/generate-email`,
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    data: JSON.stringify({}),  // 空对象,不需要传递任何参数
                    timeout: 30000,
                    onload: (response) => {
                        try {
                            ...
                            if (response.status === 200) {
                                ...
                                if (data.success) {
                                    console.log('✅ 生成邮箱成功');
                                    console.log('   邮箱地址:', data.full_email);
                                ...
                                resolve({
                                    success: false,
                                    error: `服务器返回错误状态码: ${response.status}`
                                });
                            }
                        } catch (error) {
                    ...
                    onerror: (error) => {
                        ...
                        resolve({
                            success: false,
                            error: '无法连接到服务器,请检查服务器地址是否正确或服务器是否运行'
                        });
                    },
                    ontimeout: () => {
        ...
        /**
         * 测试服务器连接
         * @returns {Promise<Object>} 包含测试结果的Promise对象
         */
    ...
    // 实例化 Duck API 客户端
    const duckAPI = new DuckAPITampermonkey();
    ...
    const TempEmailAPI = {
        // 生成随机邮箱（使用Duck.com API）
        async generateRandomEmail() {
            try {
                console.log('🦆 开始调用服务器 API 生成 Duck.com 隐私邮箱...');

                // 调用服务器 API 生成邮箱
                const result = await duckAPI.generateEmail();

                if (result.success && result.full_email) {
                    console.log('✅ 服务器 API 调用成功');
                    console.log('📧 生成的隐私邮箱:', result.full_email);
                    return result.full_email;
                } else {
                    // API 调用失败，抛出错误
                    const errorMsg = `Duck.com API 调用失败: ${result.error || '未知错误'}`;
        ...
        isValidMail(fromMail, toMail, subject) {
            ...
            // 2. 检查收件人域名（Duck.com 隐私邮箱）
            if (toLower.includes('duck.com') || toLower.includes('@duck.com')) {
                console.log('   ✅ 匹配 Duck.com 隐私邮箱域名');
                return true;
            }

            // 3. 检查发件人域名（转发邮件特征）
            // Duck.com 转发的邮件可能包含特定特征
            if (fromLower.includes('duck.com') || fromLower.includes('duckduckgo')) {
                console.log('   ✅ 匹配 Duck.com 转发邮件特征');
                return true;
            }
    ...
    function getRandomCardBin() {
        ...
        if (enabled.length === 0) {
            return DataManager.getCardBins()[0];
        }
    ...
    // 切换菜单显示/隐藏
    function toggleMenu() {
        const menuContainer = document.getElementById('augment-menu-container');
        const toggleBtn = document.getElementById('btn-toggle-menu');

        if (!menuContainer || !toggleBtn) {
            console.error('❌ 未找到菜单容器或切换按钮');
            return;
        }

        // 切换折叠状态
        const isCurrentlyExpanded = !menuContainer.classList.contains('collapsed');
        ...
        if (newExpandedState) {
            ...
            toggleBtn.querySelector('span:last-child').textContent = '折叠菜单';
            console.log('📂 菜单已展开');
        } else {
            // 折叠菜单
            menuContainer.classList.add('collapsed');
            toggleBtn.querySelector('.icon').textContent = '📂';
            toggleBtn.querySelector('span:last-child').textContent = '展开菜单';
            console.log('📁 菜单已折叠');
        }

        // 保存用户偏好
        GM_setValue('augment_menu_expanded', newExpandedState);
    }
    ...
    function showSettingsModal() {
        // 检查是否已存在弹窗
        const existingModal = document.querySelector('.aug-settings-modal');
        if (existingModal) {
            existingModal.remove();
        }

        // 获取当前配置
        const currentServerUrl = duckAPI.getServerUrl();
        const defaultServerUrl = 'http://home.chaogei.asia:32582';

        console.log('⚙️ 打开设置弹窗');
        console.log('   当前服务器地址:', currentServerUrl);

        // 创建弹窗
        const modal = document.createElement('div');
        modal.className = 'aug-settings-modal';
        modal.innerHTML = `
            <div class="aug-settings-modal-content" onclick="event.stopPropagation()">
                <div class="aug-cookie-modal-header">
                    <div class="aug-cookie-modal-title">⚙️ 服务器设置</div>
                    <button class="aug-cookie-modal-close">×</button>
                </div>

                <div class="aug-settings-group">
                    <label class="aug-settings-label">
                        🌐 服务器地址
                    </label>
                    <input
                        type="text"
                        class="aug-settings-input"
                        id="server-url-input"
                        value="${currentServerUrl}"
                        placeholder="http://home.chaogei.asia:32582"
                    />
                    <div class="aug-settings-hint">
                        💡 Duck.com 邮箱生成服务器地址。所有 Token 和代理配置都在服务器端管理。
                    </div>
                </div>

                <div class="aug-settings-actions">
                    <button class="aug-settings-btn secondary" id="settings-cancel">
                        取消
                    </button>
                    <button class="aug-settings-btn secondary" id="settings-test">
                        🔍 测试连接
                    </button>
                    <button class="aug-settings-btn primary" id="settings-save">
                        保存设置
                    </button>
                </div>
            </div>
        ...
        modal.querySelector('#settings-test').addEventListener('click', async () => {
            ...
            const testServerUrl = serverUrlInput.value.trim();
            ...
            if (!testServerUrl.startsWith('http://') && !testServerUrl.startsWith('https://')) {
                showMessage('❌ 服务器地址必须以 http:// 或 https:// 开头', 'error');
                serverUrlInput.focus();
                return;
            }

            // 临时更新服务器地址进行测试
            const originalServerUrl = duckAPI.getServerUrl();
            duckAPI.serverUrl = testServerUrl;

            showMessage('🔍 正在测试服务器连接...', 'info', 2000);

            try {
                const result = await duckAPI.testConnection();

                if (result.success) {
                    showMessage('✅ 服务器连接成功！', 'success', 3000);
                    console.log('✅ 服务器连接测试成功');
                    console.log('   服务器状态:', result.data);
                } else {
                    showMessage(`❌ 服务器连接失败: ${result.error}`, 'error', 5000);
                    console.error('❌ 服务器连接测试失败:', result.error);
                }
            } catch (error) {
                showMessage(`❌ 测试失败: ${error.message}`, 'error', 5000);
                console.error('❌ 服务器连接测试异常:', error);
            } finally {
                // 恢复原始服务器地址
                duckAPI.serverUrl = originalServerUrl;
            }
        });
        ...
        modal.querySelector('#settings-save').addEventListener('click', () => {
            ...
            // 保存服务器地址
            duckAPI.updateServerUrl(newServerUrl);

            showMessage('✅ 设置已保存，请刷新页面使设置生效', 'success', 3000);

            console.log('✅ 服务器设置已更新');
            console.log('   新服务器地址:', newServerUrl);
    ...
    function createUI() {
        ...
        // 读取菜单展开/折叠状态,默认为展开(true)
        const isMenuExpanded = GM_getValue('augment_menu_expanded', true);
        ...
        container.setAttribute('data-version', '2.2.0');

        // 创建按钮：折叠/展开按钮 + 设置按钮 + 菜单容器(包含所有功能按钮)
        container.innerHTML = `
            <button class="aug-step-btn toggle" id="btn-toggle-menu">
                <span class="icon">${isMenuExpanded ? '📁' : '📂'}</span>
                <span>${isMenuExpanded ? '折叠菜单' : '展开菜单'}</span>
            </button>

            <button class="aug-step-btn settings" id="btn-settings">
                <span class="icon">⚙️</span>
                <span>设置</span>
            </button>

            <div id="augment-menu-container" class="${isMenuExpanded ? '' : 'collapsed'}">
        ...
        console.log('⚙️ 包含设置按钮');
        console.log(`📋 菜单默认状态: ${isMenuExpanded ? '展开' : '折叠'}`);
