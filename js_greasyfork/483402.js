// ==UserScript==
// @name         JAV老司机(磁链排序优化版)
// @namespace    https://sleazyfork.org/zh-CN/users/1071071
// @version      3.8.4
// @supportURL   https://sleazyfork.org/zh-CN/scripts/1071071/feedback
// @source       https://github.com/hobbyfang/javOldDriver
// @description  因原作者脚本很长一段时间没有更新，目前也暂时尚未与原作者取得联系，故临时将增加了一个按磁链资源大小排序的版本发布上来，有任何疑问欢迎联系
// @author       slslsl

// @require      https://cdn.bootcdn.net/ajax/libs/jquery/2.2.4/jquery.min.js
// @require      https://cdn.bootcdn.net/ajax/libs/lovefield/2.1.12/lovefield.min.js
// @require      https://cdn.bootcdn.net/ajax/libs/sweetalert2/9.17.4/sweetalert2.all.min.js
...
// @downloadURL https://update.greasyfork.org/scripts/483402/JAV%E8%80%81%E5%8F%B8%E6%9C%BA%28%E7%A3%81%E9%93%BE%E6%8E%92%E5%BA%8F%E4%BC%98%E5%8C%96%E7%89%88%29.user.js
// @updateURL https://update.greasyfork.org/scripts/483402/JAV%E8%80%81%E5%8F%B8%E6%9C%BA%28%E7%A3%81%E9%93%BE%E6%8E%92%E5%BA%8F%E4%BC%98%E5%8C%96%E7%89%88%29.meta.js
// ==/UserScript==
/* jshint -W097 */
(function () {
    'use strict';
 ...
 */
    ...
    class Jav {
        ...
        /**
         * javbus详情页磁链列表增加复制、115离线快捷键功能函数、顺带将磁链按大小排序
         */
        static javbusUs() {
         ...
         * 按磁链大小排序方法
         */
        static javbusUs_sort() {
            let table = $('#magnet-table')
            let tr_array = $('#magnet-table tr[height="35px"]');
            tr_array.sort((a,b) => {
                let magnetSize = $(a).find("td:nth-child(2) a")[0].innerText;
                let unit = 1024
                let lower_size = magnetSize.toLowerCase()
                if(lower_size.includes("mb")){
                    unit = 1
                } else if(lower_size.includes("tb")){
                    unit = 1024 * 1024
                }
                let size1 = parseFloat(lower_size.replace(/(mb|gb|tb| )/g, "")) * unit

                magnetSize = $(b).find("td:nth-child(2) a")[0].innerText;
                unit = 1024
                lower_size = magnetSize.toLowerCase()
                if(lower_size.includes("mb")){
                    unit = 1
                } else if(lower_size.includes("tb")){
                    unit = 1024 * 1024
                }
                let size2 = parseFloat(lower_size.replace(/(mb|gb|tb| )/g, "")) * unit

                return size1 < size2 ? 1 : -1
            })
            tr_array.detach().appendTo(table)
        }
