// ==UserScript==
// @name         NodeSeek 编辑器增强个人修改版
// @namespace    https://www.nodeseek.com/
// @version      0.0.10
// @description  为 NodeSeek 编辑器增加图片上传功能，增加了16图床
// @author       TomyJan
...
// @downloadURL https://update.greasyfork.org/scripts/507110/NodeSeek%20%E7%BC%96%E8%BE%91%E5%99%A8%E5%A2%9E%E5%BC%BA%E4%B8%AA%E4%BA%BA%E4%BF%AE%E6%94%B9%E7%89%88.user.js
// @updateURL https://update.greasyfork.org/scripts/507110/NodeSeek%20%E7%BC%96%E8%BE%91%E5%99%A8%E5%A2%9E%E5%BC%BA%E4%B8%AA%E4%BA%BA%E4%BF%AE%E6%94%B9%E7%89%88.meta.js
// ==/UserScript==
...
(function () {
    ...
    const imgHost = {
        type: "16best", // 图床类型, 支持 LskyPro / Telegraph / Chevereto / EasyImages
        url: "https://i.111666.best/image", // 图床地址, 带上协议头
        token: "aYDXetX-j4RfjmNF2ZE4M", // 图床 token, 可选, 不填则为游客上传, LskyPro 在 /user/tokens 生成, Chevereto 必填, 在 /settings/api 生成, EasyImages 填写则使用后端接口上传, 不填写则使用前端接口上传
    ...
    async function uploadImage(items) {
        ...
        if (imageFiles.length > 0) {
            ...
            for (let i = 0; i < imageFiles.length; i++) {
                ...
                } else if (imgHost.type === '16best') {
                    await uploadTo16Images(file)
                } else {
    ...
    async function uploadTo16Images(file) {
        return new Promise((resolve, reject) => {
            let url = imgHost.url;
            let formData = new FormData();
            ...
            GM_xmlhttpRequest({
                ...
                onload: (rsp) => {
                    ...
                    if (rspJson.ok) {
                        // 图片上传成功
                        if (rspJson?.src)
                            insertToEditor(`![${(mdImgName === 0 ? rspJson.srcName : mdImgName)}](https://i.111666.best${rspJson.src})`);
                        else {
                            log('图片上传成功, 但接口返回有误, 原始返回已粘贴到编辑器', 'red');
                            insertToEditor(`图片上传成功, 但接口返回有误: ${JSON.stringify(rspJson)})`);
                        }
