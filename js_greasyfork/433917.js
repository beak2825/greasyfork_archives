...
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/433917/Redmine%20%E4%B8%8B%E6%8B%89%20select2.user.js
// @updateURL https://update.greasyfork.org/scripts/433917/Redmine%20%E4%B8%8B%E6%8B%89%20select2.meta.js
// ==/UserScript==
...
(function() {
    ...
    function insertSelect2() {
        ...
        const observer = new ACMO(() => {
            SelectRender();
        })
        observer.observe(document.body, {'childList':true,'subtree':true});
    }

    safeWaitFunc('#values_assigned_to_id_1 + .toggle-multiselect:not([hide])', 1000).then(nodes => {
        nodes.forEach(one => {
            one.setAttribute('hide', 1)
            one.click()
            $(one).hide()
        });
        insertSelect2();
    ...
    const SelectRender = () => {
        document.querySelectorAll('#values_assigned_to_id_1 + .toggle-multiselect:not([hide])').forEach(one => {
            one.setAttribute('hide', 1)
            one.click()
            $(one).hide()
        })
        $('#issue_assigned_to_id, #values_assigned_to_id_1').not('.select2-hidden-accessible, #add_filter_select').select2({ width: '100%' });
    }
    ...
    GM_addStyle('.contextual{ min-width: 150px; margin-right: 50px; } .query-columns .select2-selection{ min-width: 200px; }')
