...
// @copyright          GPL-3.0
// @license            GPL-3.0
// @downloadURL https://update.greasyfork.org/scripts/474651/Search%20By%20Image%28%E4%BB%A5%E5%9B%BE%E6%90%9C%E5%9B%BE%29.user.js
// @updateURL https://update.greasyfork.org/scripts/474651/Search%20By%20Image%28%E4%BB%A5%E5%9B%BE%E6%90%9C%E5%9B%BE%29.meta.js
// ==/UserScript==
...
var default_setting = {
  site_list: {
    ...
    Sogou:
      "https://pic.sogou.com/ris?query=https%3A%2F%2Fimg03.sogoucdn.com%2Fv2%2Fthumb%2Fretype_exclude_gif%2Fext%2Fauto%3Fappid%3D122%26url%3D{%ss}&flag=1&drag=0",
...
var lang = i18n[navigator.language]
  ? navigator.language
  : navigator.languages
  ? navigator.languages.filter(function (elem) {
      return i18n[elem];
    })[0]
  : null;
...
function init() {
  return Promise.all([
    getValue("setting"),
    GM_getValue("version", 0),
    GM_getValue("timestamp", 0),
  ]).then(function (res) {
    var s = res[0],
      v = res[1],
      t = res[2];
    ...
    var applyMap = {
      3: function () {
        ...
        for (var i = 0; i < setting.site_option.length; i++) {
          if (
            (setting.site_option[i] === "Baidu ShiTu" ||
              setting.site_option[i] === "Baidu Image") &&
            !/,?Baidu,?/.test(new_site_option.join(","))
          ) {
      ...
      5: function () {
        if (setting.site_list["WhatAnime"]) {
          setting.site_list["WhatAnime"] =
            default_setting.site_list["WhatAnime"];
        }
      },
      6: function () {
...
function create_panel() {
  ...
  search_panel.style.cssText =
    'width: 198px; font-size: 14px; text-align: center; position: absolute; color: #000; z-index: 9999999999; box-shadow: 2px 2px 3px rgba(0, 0, 0, 0.5); border: 1px solid #CCC; background: rgba(255, 255, 255, 0.9); border-top-right-radius: 2px; border-bottom-left-radius: 2px; font-family: "Arial"; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none;';
  document.body.appendChild(search_panel);
  var search_top = document.createElement("div");
  search_top.style.cssText =
    "height: 24px; line-height: 24px; font-size: 12px; overflow: hidden; margin: 0 auto; padding: 0 5px;";
  search_top.className = "image-search-top";
  search_top.innerHTML =
    '<div class="search_top_url" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis; width: 100%; height: 24px;"></div><div class="search_top_file" style="width: 100%; height: 24px; line-height: 24px;" draggable="true"><label for="image-search-file">' +
    i18n[lang]["u2s"] +
  ...
  search_item.style.cssText =
    "width: 100%; height: 24px; line-height: 24px; cursor: pointer;";
  ...
  search_panel.ondrop = function (event) {
    ...
    if (files[files.length - 1].type.indexOf("image") >= 0)
      reader.readAsDataURL(files[files.length - 1]);
  };
  search_top.getElementsByTagName("progress")[0].onclick = function () {
    if (xhr.readyState != 0 && confirm(i18n[lang]["ca"]) == true) {
      xhr.abort();
      search_panel.getElementsByClassName("search_top_url")[0].style.marginTop =
        "-24px";
    }
  };
  if (navigator.userAgent.indexOf("Firefox") >= 0) {
    ...
    paste_node_firefox.style.cssText =
      "width: 0!important; height: 0!important; position: absolute; overflow: hidden;";
...
function call_setting() {
  ...
  setting_item.style.cssText =
    "width: 100%; height: 24px; line-height: 24px; margin: 1px 0;";
  setting_item.className = "image-search-setting-title";
  setting_item.innerHTML =
    '<div style="text-align: center; display: inline-block; width: 30px;">' +
    i18n[lang]["a"] +
    '</div><div style="width: 100px; text-align: center; display: inline-block;">' +
    i18n[lang]["n"] +
    '</div><div style="width: 350px; text-align: center; display: inline-block;">' +
    i18n[lang]["l"] +
  ...
  for (var i in setting.site_list) {
    ...
    setting_item_child.innerHTML =
      '<div style="text-align: center; display: inline-block; width: 30px; vertical-align: middle;"><input type="checkbox"' +
      (setting.site_option.join("\n").indexOf(i) >= 0
        ? ' checked="checked"'
        : "") +
      '></div><div style="width: 100px; text-align: center; display: inline-block;"><input style="width: 90px;" type="text" value="' +
      i +
      '"></div><div style="width: 350px; text-align: center; display: inline-block;"><input style="width: 340px;" type="text" value="' +
      setting.site_list[i] +
      '"></div><div style="text-align: center; display: inline-block; cursor: pointer; width: 20px;">×</div>';
  ...
  setting_server.innerHTML =
    'Upload Server <input type="text" value="' +
    setting["server_url"] +
    '" placeholder="//sbi.ccloli.com/img/upload.php" style="width: 350px;">';
  setting_panel.appendChild(setting_server);
  var setting_footer = document.createElement("div");
  setting_footer.style.cssText =
    "width: 100%; height: 32px; line-height: 32px; margin-top: 5px; text-align: right;";
  ...
  setting_hotkey.style.cssText =
    "height: 32px; display: inline-block; text-align: left; float: left;";
  setting_add.style.cssText =
    "width: 90px; height: 32px; margin: 0 5px; background: #666; color: #FFF; display: inline-block; text-align: center; cursor: pointer;";
  setting_reset.style.cssText =
    "width: 90px; height: 32px; background: #666; color: #FFF; display: inline-block; text-align: center; cursor: pointer;";
  setting_save.style.cssText =
    "width: 90px; height: 32px; margin: 0 5px; background: #666; color: #FFF; display: inline-block; text-align: center; cursor: pointer;";
  setting_cancel.style.cssText =
    "width: 90px; height: 32px; background: #666; color: #FFF; display: inline-block; text-align: center; cursor: pointer;";
  ...
  setting_hotkey.innerHTML =
    i18n[lang]["sh"] +
    ' <select><option value="ctrlKey"' +
    (setting.hot_key == "ctrlKey" ? " selected" : "") +
    '>Ctrl</option><option value="shiftKey"' +
    (setting.hot_key == "shiftKey" ? " selected" : "") +
    '>Shift</option><option value="altKey"' +
    (setting.hot_key == "altKey" ? " selected" : "") +
    ">Alt</option></select>";
  ...
  setting_add.onclick = function () {
    ...
    setting_item_child.className = "image-search-setting-item";
    setting_item_child.innerHTML =
      '<div style="text-align: center; display: inline-block; width: 30px; vertical-align: middle;"><input type="checkbox"></div><div style="width: 100px; text-align: center; display: inline-block;"><input style="width: 90px;" type="text"></div><div style="width: 350px; text-align: center; display: inline-block;"><input style="width: 340px;" type="text"></div><div style="text-align: center; display: inline-block; cursor: pointer; width: 20px;">×</div>';
  ...
  setting_save.onclick = function () {
    var setting_items = document.getElementsByClassName(
      "image-search-setting-item",
    );
    var setting_data = {
      site_list: {},
      site_option: [],
    ...
    for (var i = 0; i < setting_items.length; i++) {
      if (setting_items[i].getElementsByTagName("input")[1].value != "") {
        if (setting_items[i].getElementsByTagName("input")[0].checked)
          setting_data.site_option.push(
            setting_items[i].getElementsByTagName("input")[1].value,
          );
        setting_data.site_list[
          setting_items[i].getElementsByTagName("input")[1].value
        ] = setting_items[i].getElementsByTagName("input")[2].value;
      }
    }
    setting_data.hot_key =
      setting_hotkey.getElementsByTagName("select")[0].value;
    setting_data.server_url = document
      .getElementsByClassName("image-search-setting-server")[0]
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4) {
      if (xhr.status == 200) {
        ...
        search_panel.getElementsByClassName(
          "search_top_url",
        )[0].style.marginTop = "0px";
        search_panel.getElementsByClassName("search_top_url")[0].textContent =
          i18n[lang]["us"];
      }
    }
  };
  xhr.upload.onprogress = function (event) {
    search_panel.getElementsByTagName("progress")[0].value =
...
function get_clipboard(event) {
  ...
  if (items[items.length - 1].type.indexOf("image") >= 0)
    reader.readAsDataURL(items[items.length - 1].getAsFile());
}
...
document.addEventListener(
  ...
  function (event) {
    ...
    if (GM_getValue("enabled", false)) {
        ...
        if (!asyncGMAPI) {
          if (last_update != GM_getValue("timestamp", 0)) {
            ...
            setting = GM_getValue("setting")
              ? JSON.parse(GM_getValue("setting"))
              : default_setting;
            create_panel();
      ...
      search_panel.style.left =
        (document.documentElement.offsetWidth +
          (document.documentElement.scrollLeft || document.body.scrollLeft) -
          event.changedTouches[0].pageX >=
        200
          ? event.changedTouches[0].pageX
          : event.changedTouches[0].pageX >= 200
          ? event.changedTouches[0].pageX - 200
          : 0) + "px";
      search_panel.style.top =
        (event.pageY + search_panel.offsetHeight <
        (document.documentElement.scrollTop || document.body.scrollTop) +
          document.documentElement.clientHeight
          ? event.changedTouches[0].pageY
          : event.changedTouches[0].pageY >= search_panel.scrollHeight
          ? event.changedTouches[0].pageY - search_panel.offsetHeight
          : 0) + "px";
      ...
      if (
        ...
        search_panel.getElementsByClassName("search_top_url")[0].textContent =
          event.target.src;
        if (/^data:\s*.*?;\s*base64,\s*/.test(event.target.src))
          upload_file(event.target.src);
        else if (/^(?:blob:|filesystem:)/.test(event.target.src))
          upload_blob_url(event.target.src);
        else img_src = event.target.src;
      } else {
        search_panel.getElementsByClassName(
          "search_top_url",
        )[0].style.marginTop = "-24px";
        var firefoxPasteNode = document.getElementsByClassName(
          "image-search-paste-node-firefox",
        )[0];
      ...
      if (event.target.className == "image-search-item") {
        switch (event.target.getAttribute("search-option")) {
          case "all":
            if (img_src != null) {
              for (var i = setting.site_option.length - 1; i >= 0; i--) {
                ...
                GM_openInTab(turl.replace(/{%s+}/, rsrc));
              }
              hide_panel();
            }
            break;
          ...
          default:
            if (img_src != null) {
              var rsrc = img_src;
              var turl =
                setting.site_list[event.target.getAttribute("search-option")];
              if (turl.split(/{%s+}/).shift().indexOf("?") >= 0) {
                var token = turl.match(/{%s+}/)[0];
                for (var j = 0; j < token.length - 3; j++) {
                  rsrc = encodeURIComponent(rsrc);
...
function enabletouch() {
  ...
  GM_unregisterMenuCommand(gm_callenable);
  gm_callenable = GM_registerMenuCommand("关闭 Search By Image", disabletouch);
}

function disabletouch() {
  GM_setValue("enabled", false);
  GM_unregisterMenuCommand(gm_callenable);
  gm_callenable = GM_registerMenuCommand("打开 Search By Image", enabletouch);
}

if (typeof GM_registerMenuCommand !== "undefined") {
  gm_callenable = GM_registerMenuCommand("打开 Search By Image", enabletouch);
  var gm_callsetting = GM_registerMenuCommand(
    "Search By Image 设置",
    call_setting,
  );
}
