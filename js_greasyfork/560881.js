...
// @grant        GM_addStyle
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/560881/%F0%9F%94%A5%20Compass%20Pathfinder%20%20FINAL.user.js
// @updateURL https://update.greasyfork.org/scripts/560881/%F0%9F%94%A5%20Compass%20Pathfinder%20%20FINAL.meta.js
// ==/UserScript==
...
(function () {
  ...
  function detectCurrentFloor() {
    ...
    // Then check for actual floor names
    const floorMatch = text.match(/Floor\s+(\d+)/i);
    if (floorMatch) return `floor_${floorMatch[1]}`;
    ...
    if (state.waitingForNormal) {
        ...
        if (detectedFloor === 'elevator_access') {
            ...
            if (state.isLeavingFloor) {
                ...
                } else if (/Floor [12]|First|Second/i.test(h1Text)) {
                    actualFloorKey = 'floor'; // Floors 1-2
                }
                // If h1 detection fails, fall back to page text with same order
                    ...
                    actualFloorKey = 'collapsed_office'; // Floor 3
                } else if (/Floor\s*[12]|First|Second/i.test(pageText)) {
                    actualFloorKey = 'floor'; // Floors 1-2
                }
      ...
      if (state.history.length > 0) {
        ...
        if (elevatorCall && elevatorCall.btn) {
          ...
          if (state.autoCycleActive) {
            ...
            // Set leaving floor flag BEFORE saving
