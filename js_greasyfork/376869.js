// ==UserScript==
// @name        Virtonomica: Управляющий медициной
...
// @downloadURL https://update.greasyfork.org/scripts/376869/Virtonomica%3A%20%D0%A3%D0%BF%D1%80%D0%B0%D0%B2%D0%BB%D1%8F%D1%8E%D1%89%D0%B8%D0%B9%20%D0%BC%D0%B5%D0%B4%D0%B8%D1%86%D0%B8%D0%BD%D0%BE%D0%B9.user.js
// @updateURL https://update.greasyfork.org/scripts/376869/Virtonomica%3A%20%D0%A3%D0%BF%D1%80%D0%B0%D0%B2%D0%BB%D1%8F%D1%8E%D1%89%D0%B8%D0%B9%20%D0%BC%D0%B5%D0%B4%D0%B8%D1%86%D0%B8%D0%BD%D0%BE%D0%B9.meta.js
// ==/UserScript==
var run = function()
{
...
function get_Setting( type )
{
    ...
    set['minimal_price'] = 0;
		set['minimal_price2'] = 0;
    set['maximal_price'] = 0;
    set['min_market'] = 0.25;
    set['reklama'] = 10000000;

    if ( type == "Поликлиника" ) {
        set['minimal_price'] = 24000;
				set['minimal_price2'] = 29000;
        set['maximal_price'] = 100000;
    } else if ( type == "Больница" ) {
        set['minimal_price'] = 250000;
				set['minimal_price2'] = 350000;
        set['maximal_price'] = 1000000;
    } else if ( type == "Диагностический центр" ) {
        set['minimal_price'] = 10000;
        set['minimal_price2'] = 15000;
        set['maximal_price'] = 25000;
    } else if ( type == "Стоматологическая клиника" ) {
        set['minimal_price'] = 50000;
        set['minimal_price2'] = 100000;
        set['maximal_price'] = 250000;
    } else if ( type == "Кардиологическая клиника" ) {
        set['minimal_price'] = 150000;
        set['maximal_price'] = 400000;
    }else if ( type == "Косметологический центр ") {
        set['minimal_price'] = 200000;
        set['maximal_price'] = 500000;     
    ...
    if ( type == "Больница") {
        ...
        k = 5;
    } else if ( type == "Центр иммунологии") {
        k = 1;
    } else if ( type == "Профессиональный спорт" ){
       k = 5;
    } else if ( type == "Танцы" ){
      ...
      k = 80;
        ...
        k = 40;
    } else if ( type == "Вегетарианский ресторан") {
      k = 50;
...
if ( end == "unit_list" ) {
    ...
    for( i=0; i<el.length; i++){
          ...
          td = $("td.spec", tr);
          //console.log( td.html() );
          td.html( "<div style='float:right'><img alt='skip' width=16px title='Автоматическая обработка приостановлена' src=https://cdn4.iconfinder.com/data/icons/24x24-free-application-icons/24/Stop_sign.png></div>" + td.html() );
    }
    return;
}
...
function checkType()
{
    ...
    if ( title == 'Прачечная' ) return true;
    if ( title == 'Фитнес-центр' ) return true;
    if ( title == 'Парикмахерская' ) return true;
    /*
...
var wc = $("<li id=medical_manager style='cursor:pointer; color: white;'> <img width=20px title='Анализ и изменение цен' alt='Анализ и изменение цен' src=https://cdn1.iconfinder.com/data/icons/KDE_Crystal_Diamond_2.5_Classical_Mod/22x22/apps/file-manager.png>").click( function() {
    // Это медцентр
    // определим его тип
...
//---------------------------------------------------------------------
    ...
    if ( proc > 90 && proc < 100 ) {
    		addNotes( "<font color=green>Уровень продаж: "+ Math.round(proc*100)+"%</font>" );  
    }
    ...
    if (new_price > price) {
      if ( (new_price-price) > 30000 ) new_price = price +30000;
    }
    if (new_price < price) {
            ...
            if ( uSet['minimal_price'] > 0 ){
                ...
                if ( uSet['minimal_price'] < Math.round(price) ) {
                    //console.log("minimal_price 444");
					          if ( uSet['minimal_price2'] == 0 || 
					             ( uSet['minimal_price2'] > 0 && uSet['minimal_price2'] < Math.round(price) ) 
					          )  {
						            //addNotes( "<font color=brawn>Можно включить изменение цены: " + Math.round(price) + "</font> " + (Math.round(proc*1000)/10) + "%"  );
						            $("#medical_info").html( "Можно включить изменение цены: " + Math.round(price) ).css('color', 'red');
					          }
                }
            }
            if ( uSet['minimal_price'] > 0 && uSet['minimal_price'] > price) {
                addNotes( "<font color=brawn>Цена ниже минимума: " + Math.round(price)/1000 + "К</font>, продажи:"+ Math.round(proc*100)+"%" );
                //console.log("minimal_price");
            }
            if (uSet['maximal_price'] > 0 && uSet['maximal_price'] < price){
                addNotes( "<font color=brawn>Хорошая цена: " + Math.round(price)/1000 + "К</font>, продажи:"+ Math.round(proc*100)+"%" );
    ...
    addNotes( "<font color=brawn>Цена ниже минимума: " + Math.round(new_price) + "</font>" );
        //console.log("minimal_price");
    }
...
$("#childMenu").before("<br><span id=medical_info>");
    ...
    $("#medical_settings").html("<input id=chk_medic type=checkbox>Обрабатывать");
    if ( medic[id] != null ) {
       $("#medical_info").html( medic[id]['date'] + "<br>" + medic[id]['peaple'] + "<br>" + medic[id]['price'] + " [" + $("input[name*='servicePrice']").val() + "]<br>" + medic[id]['skip']);

      medic[id]['price'] = parseFloat( $("input[name*='servicePrice']").val() );
      try {
          window.localStorage.setItem( 'medic',  JSON.stringify( medic ) );
      } catch(e) {
          out = "Ошибка добавления в локальное хранилище";
          //console.log(out);
      }
