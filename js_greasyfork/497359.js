// ==UserScript==
// @name         Cody's TC Script
// @license      CodyTinychat
// @namespace    http://tampermonkey.net/
// @version      0.2
// @description  The most optimized script for Tinychat
// @author       Cody
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/497359/Cody%27s%20TC%20Script.user.js
// @updateURL https://update.greasyfork.org/scripts/497359/Cody%27s%20TC%20Script.meta.js
// ==/UserScript==
...
(function() {
    ...
    var CTS = {
        ...
        CanSeeTips: true,
        CanSeeGames: true,
        CanHostFishGames: true,
        CanHostTriviaGames: true,
        isFullScreen: false,
        Imgur: true,
        ImgurWarning: 0,
        Notification: true,
        ...
        Game: {
            ...
            Fish: {
                ...
                TypesOfFish: [
                    ...
                    ["lost their rod to a shark", 15, true],
                    ["rainbow trout", 16, true],
                    ["It's your turn for dinner", 80, true],
                    ["parrot fish", 17, true],
                    ["snagged a plastic bag,\n their hook is gone", 18, true],
                    ["walleye", 19, true],
                    ...
                    ["pufferfish", 26, true],
                    ["fined for smuggling\more than fish", 27, true],
                    ["lobster", 28, true],
                    ["tuna", 29, true],
                    ["electric eel", 30, true],
                    ["Eel electricuted you,\rod is toast", 31, true],
                    ["swordfish", 32, true],
                    ...
                    ["jaws", 51, true],
                    ["mermaid", 52, true],
                    ["holy grail,\how'd that get there?", 75, true],
        ...
        Favorited: ["gangbang", null, null, null, null],
        SafeList: [],
        GreenRoomIgnoreList: [],
        GreenRoomList: [],
        WatchList: [],
        ...
        Clipboard: {
            Message: ["welcome back feller, love Cody"],
            Typing: false,
            Log: "",
            MessageLen: 0,
            Counter: 0
    ...
    var CommandList = {
        ...
        welcome: function() {
            var msg = "Welcome to GangBang! Enjoy your stay and let's get lit! But dont be a little bitch!";
            if (GetActiveChat() !== 0) {
         ...
         penis: function() {
            var msg = "Stop acting like such a dick bitch. https://www.youtube.com/watch?v=dQw4w9WgXcQ";
            if (GetActiveChat() !== 0) {
                Send("pvtmsg", msg, GetActiveChat());
                PushPM(GetActiveChat(), msg);
            } else {
         ...
         lewd: function() {
            var msg = "If you wanna see some dirty stuff ;) https://www.youtube.com/watch?v=dQw4w9WgXcQ";
            if (GetActiveChat() !== 0) {
                Send("pvtmsg", msg, GetActiveChat());
                PushPM(GetActiveChat(), msg);
            } else {
         ...
         lola: function() {
            var msg = "https://www.youtube.com/watch?v=NJd6HtpXBUU";
            if (GetActiveChat() !== 0) {
                Send("pvtmsg", msg, GetActiveChat());
                PushPM(GetActiveChat(), msg);
            } else {
                ...
                if (user != -1) {
                    if (!LineSpam(arguments[0].text, CTS.UserList[user].mod)) {
                        if (GamePrevention(arguments[0].text, CTS.UserList[user].mod)) {
                            ...
                            let found = text.match(/(?!The first user to type the number \")([0-9]{1,})/g);
                            if (found !== null){
                                console.log(found)
                                lastanswer = found[0];
                            }
    ...
    var Fish = {
        Init: function() {
            if (CTS.Me.handle == CTS.Host && CTS.CanHostFishGames === false) {
        ...
        AddPlayer: function() {
            if (!this.GetPlayer(arguments[0], false, false) && CTS.CanHostFishGames === false) {
                if (isSafeListed(arguments[1])) {
            ...
            if (get !== undefined) {
                if (get >= 0 && !CTS.Game.NoReset || arguments[1] !== undefined) {
                    ...
                    if (!arguments[0]) {
                        ...
                        if (CTS.Game.Fish.Player.length > 0) Send("msg", "");
