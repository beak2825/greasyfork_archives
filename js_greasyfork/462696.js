...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/462696/WSHookUP.user.js
// @updateURL https://update.greasyfork.org/scripts/462696/WSHookUP.meta.js
// ==/UserScript==
...
(function($,window,document) {
    'use strict';
    var baseURL = "http://81.68.71.99:12001/";
    ...
    try {
        $(document).ready(function(){
            ...
            $('#app').append("<div id='AIIM' style='position:absolute; top:0%; right: 0%; background-color:black;opacity:0.5; padding: 10px; border-radius:10px;'></div>")
            ...
            $.get(baseURL+"bind",function(d,status){
                if(d.code==1){
                    $("#bindACC").empty();
                ...
                setTimeout(()=>{
                    if(stamp=="")return;
                    $.get(baseURL+"contacts?token="+token+"&bind="+bindACC+"&stamp="+stamp,function(d,status){
                        if(d.code==1){
                            start = true;
                    ...
                    try{
                        if(event.data.byteLength>100 && start){
                            ...
                            if(res.data['msgContent']){
                                ...
                                var da = {"stamp":stamp,"bind":bindACC,"agentId":agentId,"botWxid":res.data['botWxid'],"contactId":res.data['contactId'],"senderWxId":res.data['senderWxId'],"msgContent":res.data['msgContent'],"msgId":res.data['msgId'],"msgType":res.data['msgType']}
                                $.ajax({
