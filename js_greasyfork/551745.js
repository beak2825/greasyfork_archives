...
// @downloadURL https://update.greasyfork.org/scripts/551745/DjinniAutomation.user.js
// @updateURL https://update.greasyfork.org/scripts/551745/DjinniAutomation.meta.js
// ==/UserScript==

const BASIC_URL = 'https://personin4-backend.cml.team/';
//const BASIC_URL = 'https://personin4-test-backend.cml.team/';
...
class Component {
    ...
    static createInput(labelText, type, inputValue, size = "full") {
        ...
        Object.assign(input.style, {
            ...
            borderRadius: '6px',
            boxSizing: 'border-box',
            width: size === "small" ? "50px" : size ==="medium" ? "150px": "100%"
        });
...
function toExistingTechnology(inputString) {
    ...
    console.log(inputString,'ahahah');
    const lowerCaseInput = inputString.toLowerCase();
    ...
    const customMappings = [
        ...
        { keywords: ['fullstak', 'full stack', 'fullstack'], tech: 'Full Stack' }
...
function prepareApplicantData() {
    ...
    const smallText = document.querySelector(selectors.jobTitle).innerText;
    console.log(smallText,'batumba jumba');
    ...
    listItemsInfo.forEach(item => {
        ...
        if(experienceElement) {
            ...
            const experienceFloat = match ? parseFloat(match[0]) : 0;
            experience = Math.floor(experienceFloat);
            console.log(experienceFloat);
        }
    });
    ...
    const getBreadCrumbTitle = document.querySelector('[href^="/home/inbox?job="]')?.innerText.trim() || '';
    console.log(getBreadCrumbTitle);
    const allTechnologies = [
    ...toExistingTechnology(smallText),
    ...(getBreadCrumbTitle ? toExistingTechnology(getBreadCrumbTitle) : [])
    ];
...
async function clipToPersona(existingApplicant) {
    ...
    console.log(newApplicant.experience, 'asjkfsdkfj');
    const request = {
        firstName,
        lastName,
        ...
        projectId,
        vacancyId,
        technologies: newApplicant.technologies,
...
async function showApplicantClipModal(newApplicant, existingApplicant) {
    ...
    let experienceInput = Component.createInput('Experience', 'text', newApplicant.experience ?? '', 'small');
