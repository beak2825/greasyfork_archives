...
// @grant        none
// @license      TANGMING
// @downloadURL https://update.greasyfork.org/scripts/539884/Ozon%20%E6%B7%BB%E5%8A%A0UNI.user.js
// @updateURL https://update.greasyfork.org/scripts/539884/Ozon%20%E6%B7%BB%E5%8A%A0UNI.meta.js
// ==/UserScript==
...
async function createDeliveryMethod8(scCompanyId, warehouseDraftId) {
    const deliveryBody = {
        company_id: Number(scCompanyId),
        warehouse_id: warehouseDraftId,
        name:"UNI Standard Extra Small UNI UNI Standard Extra Small Pick Up Point (超级轻小件,快速,自提点) Putian1",
        cutoff: "17:00",
        tpl_integration_type: "aggregator",
        provider_id: 1266,
        template_id: 1109,
...
(async function () {
    ...
    const response = await fetch("https://seller.ozon.ru/app/crossborder/warehouse", {
        "headers": {
    ...
    let warehouseDraftId = null;
    // 用正则提取 warehouseId（即你说的 warehouseDraftId）
    const match =  html.match(/"warehouseId"\s*:\s*(\d+)/);
    if (match) {
        warehouseDraftId =   Number(match[1]);
        console.log("✅ 提取的 warehouseDraftId:", warehouseDraftId);
    } else {
        warehouseDraftId = false
        console.warn("未匹配到 warehouseId");
    }
    if (warehouseDraftId) {
        ...
        async function tryCreateAndActivate(fnCreate, label, companyId, warehouseId) {
            try {
                ...
                await activateCreate(companyId, createRes.id);
            } catch (e) {
