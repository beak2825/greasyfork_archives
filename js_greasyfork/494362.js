...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/494362/%E9%A3%93%E9%A3%8E%E7%9F%A9%E9%98%B5%E6%95%B0%E6%8D%AE%E5%A4%A7%E5%B1%8F.user.js
// @updateURL https://update.greasyfork.org/scripts/494362/%E9%A3%93%E9%A3%8E%E7%9F%A9%E9%98%B5%E6%95%B0%E6%8D%AE%E5%A4%A7%E5%B1%8F.meta.js
// ==/UserScript==
...
(function() {
    ...
    function modifyElementValue() {
        ...
        // 检查缓存是否存在且数据是否在24小时内生成
        if (dataCache && dataTimestamp && (now - parseInt(dataTimestamp) < 86400000)) {
            dataCache = JSON.parse(dataCache);
            pathsAndRanges.forEach((item, index) => {
                const element = document.evaluate(item.xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
                if (element) {
                    element.innerText = dataCache[index];
                }
            });
        } else {
            dataCache = [];
            pathsAndRanges.forEach(function(item, index) {
                const elements = document.evaluate(item.xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
                if(elements) {
                    const newValue = getRandomInt(item.min, item.max);
                    elements.innerText = newValue;
                    dataCache[index] = newValue;
                }
            });
    ...
    // 首次尝试修改目标元素
    modifyElementValue(observer);  // 传递observer，以便可以控制其状态
