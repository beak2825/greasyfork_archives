...
// @downloadURL https://update.greasyfork.org/scripts/499048/javdb%E6%B8%85%E5%8D%951.user.js
// @updateURL https://update.greasyfork.org/scripts/499048/javdb%E6%B8%85%E5%8D%951.meta.js
// ==/UserScript==
...
显示清单
链接打开
清单页面删除 x不更新调用函数 页面删除元素
x分页不能跳转页面
listssettings() 更新一遍newArry新增移除清单数字更新
清单和关闭span元素固定宽度inline-block
x备注 标记
导出清单页面 添加清单页面全部导出 导入
番号排序 发布时间排序 添加时间排序
修改名字一样不修改
系列 链接x
女优 图片 链接x
...
添加删除清单数更新
删除番号页面数更新
移除添加清单后加载listssettings() 显示慢 数字设置更新 页面显示设置更新为obj.name
*/
...
(function() {
    ...
    //替换数组元素名字
    /*let arr = {'tag1': [{ 'id': 0, 'name': '，小雪' },{ 'id': 1, 'name': '杨过' }], 'tag2': [{ 'id': 2, 'name': '过儿' }]}

    let obj = {};  //创建一个临时对象
    let newData = [];  //创建一个新数组接收
    Object.keys(arr).forEach(function (value, i){
        obj[value] = arr[value];
        obj['tag0'] = arr['tag1'];  //替换键
        delete obj['tag1'];  //替换后删除，也可以不删除
    ...
    /* 获取清单列表数量
    var a = ''
    document.querySelectorAll('.checkbox').forEach(function (value, i) {//数组
        a = a + value.childNodes[2].textContent.trim() + '  ' + value.childNodes[3].innerHTML.split('(')[1].split(')')[0] + '\n'
    });
    console.log(a)*/

    //查找数组对象重复内容
    function findRepeat(array){
        let result=[]; //新建数组来放置重复的元素
        for(let i=0;i<array.length-1;i++){ //遍历数组
            let isRepeat=false;	//标志变量，用来判断后边是否有重复
            for(let k=0;k<result.length;k++){ //遍历resule数组，
                if(array[i].url===result[k].url){ //先判断array里边有没有result存在的元素
                    isRepeat=true;	//如果有存在的就把标志变量设为true用来下边判断，循环怎样执行，
                    break;
                }
            }
            if(isRepeat) continue; //如果标志变量是true 那么就退出此次循环，执行下一次循环。
            for(let j=i+1;j<array.length;j++){ //然后再对比array里面的元素，
                if(array[i].url===array[j].url){
                    result.push(array[i]); //如果有一样的就把这个一样的数组成员放入result数组中
                    break;
                }
            }
        }
    ...
    //findRepeat(GM_getValue('丝袜'));
    // console.log(findRepeat(GM_getValue('多人')));
    // console.log(findRepeat(GM_getValue('丝袜')));
    // console.log(findRepeat(GM_getValue('舔脚')));
    // console.log(findRepeat(GM_getValue('封面')));
    // console.log(findRepeat(GM_getValue('高跟')));
    // console.log(findRepeat(GM_getValue('远古')));

    findRepeat(GM_getValue('丝袜')).forEach(function (obj, i) {//数组
        console.log(obj.url);
    });

    // GM_getValue('连裤袜').forEach(function (obj, i) {//数组
    //     // console.log(obj.url);
    //     if(obj.url == 'https://javdb.com/v/9DexR') {
    //             console.log(1)
    //         }
    // });
...
//     // const duplicatedItems = findByItems((a, b) => a.fanhao === b.fanhao);
//     const duplicatedItems = findByItems((a, b) => a.url === b.url);


    // console.log(duplicatedItems(GM_getValue('连裤袜')))
    // console.log(GM_getValue('丝袜').length)

    //数组元素去重
    // const uniqueArr = GM_getValue('高跟').reduce((result, item) => {
    //     const ids = result.map(item => item.url);
    ...
    //     return result;
    // }, []);
    // GM_setValue('高跟', uniqueArr)

    // var newArr = [];

    var arr = GM_getValue('西服');
    var newArr = [];
    arr.forEach((item) => {
        var check = newArr.every((b) => {
            return item.fanhao !== b.fanhao;
        })
        check ? newArr.push(item) : '';
    })

    // console.log(newArr);
    // GM_setValue('西服', newArr);
    ...
    let videoFanhao = document.querySelector('[data-clipboard-text]').attributes[2].value.trim();
    let videoTitle = document.getElementsByClassName('current-title')[0].innerHTML.split('[已收藏]')[0];
    ...
    // var tagList = [];
    var newArray = [];
    var origninfo = document.querySelectorAll('.is-size-7.has-text-grey')[0].innerHTML.trim();
    ...
    //         Object.values(GM_getValue(obj)).forEach(function (value, index) {//数组
    ...
    //         if(value.fanhao == videoFanhao) {
    //             tagEle['name'] = obj;
    //             tagEle['count'] = Object.values(GM_getValue(obj)).length;;
    //         }
    //         tagList.push(tagEle);
    //     });
    // });

    // tagList.sort(compare( "count"));
    //console.log(newArray)
    // tagList.forEach(function (obj, i) {//数组
    newArray.forEach(function (obj, i) {//数组
        // GM_getValue(obj).forEach(function (value, index) {//数组
        Object.values(GM_getValue(obj.name)).forEach(function (value, index) {//数组
            // console.log(value.fanhao);
            // if(value.fanhao == 'SW-506') {
            //     console.log(obj.name + ' ' + value.fanhao)
            // }
            if(value.fanhao == videoFanhao) {
                if(inLists == '') {
                    inLists = obj.name;
    ...
    buttonG.addEventListener('click', () => {
        ...
        window.setTimeout(function(){
            listsShow = true;
            // document.querySelectorAll(".count").forEach(function (value) {
            //     value.addEventListener('click', () => {
            //         var lname = value.previousSibling.innerHTML;
            //         var lcount = value.innerHTML.split('(')[1].split(')')[0];
            //         // console.log(lname + ' ' + lcount);
            //         showList(lname, lcount);
            //     });
            // });
        }, 300);
    ...
    function listssettings() {
        ...
        nameDiv.innerHTML = '清单 (' + GM_listValues().length + ')';
        ...
        addlist.style.marginRight = '145px';
        ...
        // inputSpan.style.fontSize = 'initial';
        ...
        //close.style.position = "fixed";
        // outputSpan.style.display = 'inline-block';
        // outputSpan.style.fontSize = 'initial';
        ...
        allValue.forEach(function (obj, i) {//数组
            ...
            modify.addEventListener('click', () => {
                var newobj = prompt('修改 ' + obj);
                let newele = [];
                var arr = GM_getValue(obj);
                newele = arr;
                //console.log(arr);
                //console.log(newele);
                //delete obj['tag1'];
                if(newobj) {
                    ...
                    showclipboard('修改清单 ' + obj + '为 ' + newobj);
            ...
            del.addEventListener('click', () => {
                ...
                if(arr.length == 0) {
                    var result = confirm("是否确认删除清单 " + obj);
                    if (result) {
                        GM_deleteValue(obj);
                        showclipboard('删除清单 ' + obj);
                        listssettings();
                    }
                } else {
                    //alert('清单番号页面数量不为零，无法删除');
                    showclipboard('清单番号页面数量不为零，无法删除');
        ...
        newArray.forEach(function (value, i) {//数组
            ...
            // list.className = 'curListEle';
            list.style.width = '80px';
            list.style.textAlign = 'left';
            list.style.cursor = "pointer";
            list.style.display = 'inline-block';
            // GM_getValue(value.name).forEach(function (value1, index1) {//数组
            Object.values(GM_getValue(value.name)).forEach(function (value1, index1) {//数组
                ...
                if(value1.fanhao == videoFanhao) {
                    ...
                    list.style.color = '#48C78E';
                    list.setAttribute('id', '1');
                    list.title = value1.adddate;
                    //console.log(list.id)
                }
            });
            var ele = GM_getValue(value.name);
            list.addEventListener('click', () => {
                newinLists = ''; //未执行listssettings() 设置变量为空
                if(list.id == 1) {
                    list.style.color = '#000';
                    list.title = '';
                    list.setAttribute('id', '0');
                    ...
                    // list.style.color = '#3D9140';
                    list.style.color = '#48C78E';
                    list.title = videoaddDate;
                    count.innerHTML = '(' + ( value.count + 1 ) + ')';
                    // document.getElementsByClassName('curListEle')[0].style.color = '#48C78E';
                    // console.log(1)
                    var x = event.clientX;
                    var y = event.clientY;
                    // console.log(x + ' ' + y)
            ...
            count.addEventListener('click', () => {
                ...
                // data = GM_getValue('丝袜');
                data.sort(compare1( "fanhao"));
                // console.log(data);
                // showList(value.name, value.count);
                showList(data, value.name, value.count);
            });
        ...
        nodeDiv.addEventListener('dblclick', function (e) {
            const settingnodeDiv = document.getElementById("Lists");
            settingnodeDiv.style.display = 'none';
            listsShow = false;
        });
    ...
    function compare1( propertyName) {//番号排序逆序
        return function( object1, object2) {
            var value1 = object1[propertyName];
            var value2 = object2[propertyName];
            if(value1 > value2) {
    ...
    var showList = function(data, listname, n) {
        //var data = GM_getValue('丝袜');
        // console.log(listname + ' ' + n);
        ...
        nodeEle1.addEventListener('dblclick', function (e) {
            document.getElementById("listpage").style.display = 'none';
            // document.getElementsByClassName('button is-info is-outlined button-c')[0].click();
            // listssettings();
            // window.setTimeout(function(){
            //     listsShow = true;
            //     // document.querySelectorAll(".count").forEach(function (value) {
        ...
        //nodeEle.appendChild(newnodeDiv);
        ...
        //divc.style.paddingLeft = '803px';
        divc.style.marginLeft = '680px';
        ...
        //newnodeDiv.appendChild(div);


        //div = document.getElementsByClassName('newDiv')[0],
                        ...
                        if (result1) {
                            ...
                            Object.values(ele).forEach(function (value, index) {
                                ...
                                if(i.fanhao == value.fanhao) {
                                    ...
                                    let newdivl = document.getElementById("divl");
                                    var m = newdivl.innerText.split(' (')[0];
                                    var n = newdivl.innerText.split(' (')[1].split(')')[0];
                                    newdivl.innerText = m + ' (' + (n - 1) + ')';
    ...
    document.addEventListener('mouseover', function (e) {
        if(videoUrl.indexOf('/v/') > -1) {
                //r = e.target.href;
                // s = e.target.textContent.split('[')[0];
    ...
    document.addEventListener('keydown',function(event) {
        ...
        if(keynum==76 && event.altKey){
            flag = false;
            xln.forEach(function (value, index) {
            // Object.values(ign).forEach(function (value, index) {
                if(s.split('[')[0] == value.split('[')[0]) {
                    idx = index;
                    flag = true;
                }
            });
            if(s != null && !flag) {
                try {
                    ...
                    if (s != '' && s != null) {
                        ...
                        document.querySelectorAll('a').forEach(function (value) {
                            if(value.href.indexOf('series/') > -1) {
                                if(s.indexOf('[') > -1)
                                    value.innerHTML = s.split('[')[0] + '[已收藏]' + s.slice(s.indexOf('['));
                                else
                                    value.innerHTML = s.split('[')[0] + '[已收藏]';
                            }
                        });
                        showclipboard('添加系列 '+ s);
                    } else {
                        showclipboard('未添加', 1);
                    }
                } catch (error) {
                ...
                try {
                    var p = s;
                    s = prompt('请输入信息', s);
                    if(s == '' || s == null) {//取消会移除 不另外设置快捷键删除
                        xln.splice(idx, 1);
                        GM_setValue('系列', xln);
                        showclipboard('移除系列 '+ p, 0);
                        document.querySelectorAll('a').forEach(function (value) {
                            if(value.href.indexOf('series/') > -1) {
                                value.innerHTML = p.split('[')[0];
                            }
                        });
                        s = p;
                    } else {
                        ...
                        if(s.indexOf('已收藏') > -1) {
                            ...
                            q = s.split('[')[0] + '[已收藏]' + s.slice(s.indexOf('['));
                        } else {//清除标签
                            q = s + '[已收藏]';
                        }
                        xln.push(s);
                        ...
                        showclipboard('修改系列 '+ s, 1);
                    }
                } catch (error) {
                    showclipboard(error);
                }
                // showclipboard('移除系列 '+ s, 0);
            } else if(s == null) {
                showclipboard('失败', 1);
            }
    ...
    //收藏的系列
    if(videoUrl.indexOf('collection_series') > -1) {
        xln.forEach(function (x, i) {//数组
            document.querySelectorAll('strong').forEach(function (value) {
                if(x.split('[')[0] == value.innerHTML) {
                    value.innerHTML = x;
                }
            });
        });
    ...
    if(videoUrl.slice(18, 20) == 'v/' || videoUrl.slice(17, 24) == '/series') {
        xln.forEach(function (x, i) {//数组

        // Object.values(ign).forEach(function (x, i) {//数组
            document.querySelectorAll('a').forEach(function (value) {
                if(value.href.indexOf('series/') > -1) {
                    if(x.split('[')[0] == value.innerHTML) {
                        value.innerHTML += '[已收藏]' + x.split(value.innerHTML)[1];
                    }
                }
            });
            document.querySelectorAll('span').forEach(function (value) {
                if(x.split('[')[0] == value.innerHTML) {
                    value.innerHTML += x.split(value.innerHTML)[1];
                }
            });
        //}
        });
