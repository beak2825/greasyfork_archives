// ==UserScript==
// @version       1.3.1
// @name          CNCOpt Link Button (unxix)
// @namespace     http://cncopt.com/
// @icon          http://cncopt.com/favicon.ico
// @description   lien vers CnCopt.com. Adapté pour l'outil d'optimisation de base d'unxix. le code ne vient pas de moi, j'ai RIP l'intégralité et y ait greffé mes besoins.Script pratique également pour créer un tableau excel des CnCopt.com avec récapitulatif (niv armée/def, type de soutien ...) des bases ennemies.
...
// @downloadURL https://update.greasyfork.org/scripts/25413/CNCOpt%20Link%20Button%20%28unxix%29.user.js
// @updateURL https://update.greasyfork.org/scripts/25413/CNCOpt%20Link%20Button%20%28unxix%29.meta.js
// ==/UserScript==
/*
        ...
        var cncopt = {
          ...
          make_sharelink: function (baseUrl) {
            try {
              ...
              var link5 =  "&ally="+AllianzName+"|"+serverName+"|"+PlayerName+"|"+AllianzID  ;
              ...
              var pName = city.get_OwnerName?city.get_OwnerName():"Forgotten" ;
			  var pAName = city.get_OwnerAllianceName?city.get_OwnerAllianceName():"Forgotten" ;
			  link += city.get_Name()+"("+city.get_PosX()+"-"+city.get_PosY()+")|";
              defense_units = []
              for (var i = 0; i < 20; ++i) {
                var col = [];
                for (var j = 0; j < 9; ++j) {
              ...
              var joueurconnecte = apc.get_CurrentOwnCity().get_PlayerName();
			  var allianceconnecte = apc.get_CurrentOwnCity().get_AllianceName();
              var nomjoueur = city.get_OwnerName?city.get_OwnerName():"Forgotten" ;
			  var nomalliance = city.get_OwnerAllianceName?city.get_OwnerAllianceName():"Forgotten" ;
              var d = new Date();
                var j = d.getDate();
                var m = d.getMonth()+1;
                var h = d.getHours();
                var date = j+"."+m+" a "+h+"h";
              var link5 =  "&ally="+nomalliance+"|"+serverName+"|"+nomjoueur+"|"+allianceconnecte+"|"+joueurconnecte+"|"+city.get_PosX()+"-"+city.get_PosY()+"|"+city.get_Name()+"|"+date;
                  link += "|newEconomy" + link5;
              }
              
              link += "|" + Math.floor(ClientLib.Data.MainData.GetInstance().get_Player().get_ResearchPoints());
              link += "|" + Math.floor(ClientLib.Data.MainData.GetInstance().get_Player().GetCreditsCount());
              
              var prodCredits =   Math.floor((ClientLib.Data.MainData.GetInstance().get_Player().get_Credits().Delta + ClientLib.Data.MainData.GetInstance().get_Player().get_Credits().ExtraBonusDelta) *  ClientLib.Data.MainData.GetInstance().get_Time().get_StepsPerHour());
              var packCredits = Math.floor(ClientLib.Data.MainData.GetInstance().get_Player().get_Credits().ExtraBonusDelta *  ClientLib.Data.MainData.GetInstance().get_Time().get_StepsPerHour());
              
              link += "|" + prodCredits;
              link += "|" + packCredits;
              link += "|" + ClientLib.Data.MainData.GetInstance().get_Cities().get_AllCities().c;
