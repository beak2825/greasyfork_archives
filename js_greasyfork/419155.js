// ==UserScript==
// @name         chrome menu
// @namespace    your face
// @version      your mom
// @icon         data:image/png;base64,
// @description  Special thanks for dunk, for the idea!
// @author       DCC'S CAVE xD
// @match        https://dino-chrome.com/*
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/419155/chrome%20menu.user.js
// @updateURL https://update.greasyfork.org/scripts/419155/chrome%20menu.meta.js
// ==/UserScript==

(function() {
    var menu = document.createElement('div');
    menu.innerHTML='<button style="background: cyan;" onclick="Runner.instance_.gameOver=function(){}">Dead Off</button><button onclick="Runner.instance_.gameOver=Runner.prototype.gameOver">Dead On</button><button style="background: red;" onclick="Runner.instance_.setSpeed (100);">Speed Up</button><button style="background: orange;" onclick="Runner.instance_.setSpeed (5);">Normal Speed</button><button style="background: pink;" onclick="Runner.instance_.playingIntro = false">Pause: Off</button><button style="background: pink;" onclick="Runner.instance_.playingIntro = true">Pause: On</button><p style="color: red;">Enjoy!</p><h3 style="color: blue;">DCC Script for DINO</h3>';
    document.body.insertBefore(menu, document.body.firstChild);
})();