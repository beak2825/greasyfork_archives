...
// @downloadURL https://update.greasyfork.org/scripts/432055/XDR%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84%E7%94%9F%E6%88%90.user.js
// @updateURL https://update.greasyfork.org/scripts/432055/XDR%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84%E7%94%9F%E6%88%90.meta.js
// ==/UserScript==
...
(function() {
    ...
    // 表格链接
    let TABLE_URL = "https://lgpznaxp6u.feishu.cn/base/bascn9Mje7aT32C7PLHtqdpYGNb?table=tblxRQtyUzIU4AuF&view=vewvETUjm8"
    // 表格对应列名
    let COLUMN = '数据库编号'
    // ====================需要输入的地方====================
        ...
        return new Promise(function(resolve, reject) {
            (function loop(page_token) {
                getTableContentByPageToken(tenant_access_token, page_token).then(function(data) {
                    ...
                    var database_id_list = all_list.map(x => x[COLUMN]).filter(x => !!x).map(x => x[0]).map(x => x.text);
                    console.log(database_id_list)
                    results.push(database_id_list)
                    if (has_more) {
    ...
    runAsync(feishu_token_url, "POST", feishu_token_headers, feishu_token_data).then((result) => {
        ...
        getAllTableContent(token).then((result) => {
            console.log("最终，所有的内容如下")
            console.log(result)
            result = result[0].join(" ")
            console.log(result)
            return result
        }).then((databaseNum) => {
            //let databaseNum = "659 660";

            // 最终调用
            var xdr_url = "https://10.0.0.106/api/datamanager/field/getlist?offset=0&limit=1000&filters=%7B%22search_val%22:%22%22%7D"
            var xdr_headers = {
            ...
            console.log("等待生成数据结构")
            runAsync(xdr_url, "GET", xdr_headers, null).then((result) => {
                console.log("3. 数据结构生成");
                console.log(result);
                var fields_raw = eval('(' + result + ')').data.list;
                //var fields_raw = JSON.stringify(listDict)
                console.log(fields_raw);
                let fields = fields_raw.map(x => {
                    return {
                ...
                console.log("fields：");
                console.log(fields);
                console.log("4. 转换成数据库语句");
                let ids2conf = (idstr) => {
                    let ids = idstr.split(/\s+/)
                    let jsondata = ids.map(x => Object.values(fields).filter(y => y.key == x)[0])
                    document.writeln(JSON.stringify(jsondata) + '<br>')
                    //console.log(JSON.stringify(jsondata) + '<br>');
                }
                ids2conf(databaseNum);
            });
        });
    })
})();
