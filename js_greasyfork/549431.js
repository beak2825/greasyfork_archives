// ==UserScript==
// @name         斗鱼全民星推荐自动领取 (优化版)
// @namespace    http://tampermonkey.net/
// @version      1.4
// @description  自动打开、领取并切换直播间处理全民星推荐活动红包，并尝试自动暂停视频。已优化页面堆积问题。
// @author       ysl & ChatGPT
...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/549431/%E6%96%97%E9%B1%BC%E5%85%A8%E6%B0%91%E6%98%9F%E6%8E%A8%E8%8D%90%E8%87%AA%E5%8A%A8%E9%A2%86%E5%8F%96%20%28%E4%BC%98%E5%8C%96%E7%89%88%29.user.js
// @updateURL https://update.greasyfork.org/scripts/549431/%E6%96%97%E9%B1%BC%E5%85%A8%E6%B0%91%E6%98%9F%E6%8E%A8%E8%8D%90%E8%87%AA%E5%8A%A8%E9%A2%86%E5%8F%96%20%28%E4%BC%98%E5%8C%96%E7%89%88%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    async function openOneNewTab() {
        ...
        try {
            const apiRoomList = await getRoomsFromApi(10);
    ...
    function setupLauncherUI() {
        ...
        button.innerHTML = '打开一个房间 <span class="count">(已开: 0)</span>';
        button.onclick = openOneNewTab;
    ...
    async function autoPauseVideo() {
        ...
        if (isWaitingForPopup || isSwitchingRoom) return;
        ...
        if (pauseBtn) {
            ...
            if (await safeClick(pauseBtn, "暂停按钮", false, false)) {
    ...
    async function waitForPopupAndClick() {
        ...
        if (await safeClick(openBtn, "红包弹窗的打开按钮")) {
            ...
            log("尝试关闭领取结果弹窗...");
            const finalPopup = document.querySelector(SELECTORS.popupModal);
            if (finalPopup) {
                const closeBtn = finalPopup.querySelector(SELECTORS.closeButton);
                if (!await safeClick(closeBtn, "领取结果弹窗的关闭按钮", true, false)) {
                    log("关闭按钮未找到或点击失败。");
                } else {
                    log("关闭按钮已点击。");
                }
    ...
    async function handleSwitchRoom() {
        ...
        try {
            ...
            const roomList = await getRoomsFromApi(5);
            let nextUrl = null;
            ...
            if (nextUrl) {
                ...
                log("未能从 API 获取到合适的下一个房间链接。可能没有其他活动房间了。");
                log("尝试关闭这个最后的标签页...");
                await closeCurrentTab();
            }
        } catch (error) {
