...
// @version      4.20
// @icon         
// @description  final version, no more updates
// @author       YO MOMA
...
// @downloadURL https://update.greasyfork.org/scripts/373177/NITROTYPE%20BOt.user.js
// @updateURL https://update.greasyfork.org/scripts/373177/NITROTYPE%20BOt.meta.js
// ==/UserScript==
...
(() => {
    ...
    checkIfBanned = callback => {
        if (userInfo.username) {
            ...
            xhr.onload = () => {
                ...
                if (status !== 200 || (res.includes("<title>Suck my dicky dick | Competitive Typing Game | Race Your Friends</title>"))) {
                    // I'm banned!
                    showBan();
                } else {
                    // Everything normal
    ...
    updateStats = () => {
        if (userInfo.username) {
            ...
            stm1.textContent = "$" + userInfo.money + " (Spent: $1,000" + userInfo.moneySpent + ")";
    ...
    generateTypeDecision = offset => {
        ...
        */
        ...
        setTimeout(() => {
            let dipRate = 0.80;
            let WRONG = false;
            timeout = tgen(12000000 / wordsPerMinute);
            if (inDip) {
