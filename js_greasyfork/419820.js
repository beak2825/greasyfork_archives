...
// @namespace    Unknown
// @version      1.2.1
// @description  Support extension for Swordz
// @author       UnknownScripter
// @match        *.swordz.io
// @grant        random
// @downloadURL https://update.greasyfork.org/scripts/419820/SwordzHack.user.js
// @updateURL https://update.greasyfork.org/scripts/419820/SwordzHack.meta.js
// ==/UserScript==
    ...
    if(tracers && Player.list[selfId] != undefined && Player.list[selfId].map != undefined && Player.list[selfId].map != 'menu') {
        for(var playerId in Player.list) {
            if(playerId != selfId) {
                ...
                var currentDistance = Math.sqrt(Math.pow(diffX, 2) + Math.pow(diffY, 2));
                if(currentDistance != undefined && !isNaN(currentDistance) && currentDistance <= 1500) {
                    ctx.beginPath();
