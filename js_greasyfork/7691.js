...
// @downloadURL https://update.greasyfork.org/scripts/7691/WoTExtendedStat.user.js
// @updateURL https://update.greasyfork.org/scripts/7691/WoTExtendedStat.meta.js
// ==/UserScript==
var lang,
    scriptVersion = '0.9.4.0',
    tanksarr = [],
    gServ, api_key;
...
function main() {
    ...
    function create_expanders(bA, lang, settings) {
        ...
        if (blBool) {
            ...
            document.getElementsByClassName("b-profile-link")[0].setAttribute('style', 'display:none;');
        ...
        script.textContent =
            "function popup(message, autoHide, autoReload) {var maskHeight = $(document).height();var maskWidth = $(window).width();var dialogTop =  (maskHeight/3) - ($('#dialog-box').height());var dialogLeft = (maskWidth/2) - ($('#dialog-box').width()/2);$('#dialog-overlay').css({height:maskHeight, width:maskWidth}).show();$('#dialog-box').show();$('#dialog-message').html(message); if (autoHide) setTimeout(function(){$('#dialog-overlay, #dialog-box').hide();} , 2000); if (autoReload) setTimeout(function(){window.location.reload();} , 2000);}"
        		...
        		+ " var wcount=parseInt($($(me).children()[2]).attr('wcount')), bcount=$($(me).children()[1]).attr('data-battle_count'), tDamage=toFl($(me).next().find('td[data-field=\"damage_dealt\"]')[0].innerHTML), lev=0, tnaSumm=0, nominalDamage=eval(GetLSData(\"nominalDamage\")),"
                ...
                + " if ($(this).attr('curStText') === '\"Рейтинги\"' || $(this).attr('curStText') === '\"Ratings\"') {$('.b-composite-heading').hide(); $('.b-profile-link').hide(); $($('.b-hr-layoutfix')[2]).hide()}"
                ...
                + " if ($(this).attr('curStText') === '\"Рейтинги\"' || $(this).attr('curStText') === '\"Ratings\"') {$('.b-composite-heading').show(); $('.b-profile-link').show(); $($('.b-hr-layoutfix')[2]).show();}"
    ...
    function CalcAvgLev(tanksarr) {
		...
		if (wn8Data) {
			...
			xdr.xget("http://www.wnefficiency.net/exp/expected_tank_values_17.json", saveWN8Data);
		}
    ...
    function CalcEffColor(effval, type) {
        ...
        if (type === "eff") {
            ...
            if (effval < 355)
                return ['FE0E00', (355 - effval).toFixed(2) + (lang === "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
            if (effval < 820)
                return ['FE7903', (820 - effval).toFixed(2) + (lang === "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
            if (effval < 1370)
                return ['F8F400', (1370 - effval).toFixed(2) + (lang === "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
            if (effval < 2020)
                return ['60FF00', (2020 - effval).toFixed(2) + (lang === "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
            if (effval < 2620)
                return ['02C9B3', (2620 - effval).toFixed(2) + (lang === "ru" ? ' баллов до следующего ранга' : ' balls to next level')];
            if (effval < 99999)
                return ['D042F3', (lang === "ru" ? 'Максимальный уровень. Круче только вареные яйца.' : 'Max. level')];
        }
        return ['white', ''];
