// ==UserScript==
// @name WebGames Client [Sploop.io]
// @author Murka
...
// @downloadURL https://update.greasyfork.org/scripts/461120/WebGames%20Client%20%5BSploopio%5D.user.js
// @updateURL https://update.greasyfork.org/scripts/461120/WebGames%20Client%20%5BSploopio%5D.meta.js
// ==/UserScript==
/* jshint esversion:6 */
...
Function("(" + ((GM_info) => {
    ...
    (() => {
        ...
        const defaultSettings = {
            ...
            possibleShots: true,
            kill: true,
            killMessage: "WebGame Client Is The Best!! x{KILL}",
            autospawn: false,
            smoothZoom: true,
            skipUpgrades: true,
        ...
        Storage.set("WebGame-settings", settings);
        const Settings = settings;
        class Formatter {
            static object(target) {
                const layer = LayerData[target.type];
                return {
                    id: target[WebGame.props.id],
                    type: target.type,
                    x: target[WebGame.props.x],
                    y: target[WebGame.props.y],
                    x1: target[WebGame.props.x1],
                    y1: target[WebGame.props.y1],
                    x2: target[WebGame.props.x2],
                    y2: target[WebGame.props.y2],
                    angle: target[WebGame.props.angle],
                    angle1: target[WebGame.props.angle1],
                    angle2: target[WebGame.props.angle2],
                    ownerID: target[WebGame.props.ownerID],
                    radius: layer.radius,
                    layerData: layer,
                    target
                };
            ...
            static projectile(target) {
                ...
                return {
                    ...object,
                    range: target.range,
                    projectileType: target[WebGame.props.projectileType]
                };
            }
            static entity(target) {
                const object = this.object(target);
                const healthValue = target[WebGame.props.health];
                ...
                return {
                    ...
                    entityValue: target[WebGame.props.entityValue]
                };
            }
            static player(target) {
                ...
                return {
                    ...entity,
                    hat: target[WebGame.props.hat],
                    isClown: entity.entityValue === 128,
                    currentItem: target[WebGame.props.currentItem]
                };
            }
        }
        ...
        const GM = (property, value) => {
            if (!WebGame.PRODUCTION) return true;
            try {
        ...
        const resetSkin = () => {
            const player = WebGame.myPlayer.target;
            if (!player) return;
            delete player[WebGame.props.skin];
            delete player[WebGame.props.accessory];
            delete player[WebGame.props.back];
        };
        const updateSkin = () => {
            if (!Settings.customSkins) return resetSkin();
            const player = WebGame.myPlayer.target;
            if (!player) return;
            Common_define(player, WebGame.props.skin, Settings.skin);
            Common_define(player, WebGame.props.accessory, Settings.accessory);
            Common_define(player, WebGame.props.back, Settings.back);
        };
        const Scale = {
            Default: {
                w: 1824,
        ...
        const createMenu = () => {
            ...
            IFRAME.onload = () => {
                ...
                const popups = [ {
                    ...
                    links: "https://www.youtube.com/channel/UCtCT9P2JcUoJlF3KCr5etjA",
                    prevs: "https://i.imgur.com/agwC9na.png"
                }, {
                    indexs: 1,
                    titles: "YouTube",
                    descriptions: "Nothing...",
                    ...
                    titles: "Nothing...",
                    descriptions: "Nothing...",
                    links: "https://www.instagram.com/_ma_te_x_/",
                    prevs: "https://i.imgur.com/7QFa9F6.png"
                } ];
                const pickPopup = () => {
                ...
                const update = () => {
                    ...
                    headerVersion.textContent = "v" + WebGame.version;
                    ...
                    closeMenu.onclick = WebGame.toggleMenu;
                ...
                downloadSettings.onclick = () => {
                    download(Settings, "WebGameSettings" + WebGame.version);
                };
        ...
        class EntityManager {
            ...
            static animals() {
                const layers = WebGame.saves.entityList();
                const animals = [];
            ...
            static enemies() {
                const players = WebGame.saves.entityList()[ELayer.PLAYER];
                const enemies = [];
            ...
            static sortDistance(a, b, sorted) {
                const target = sorted || WebGame.myPlayer;
            ...
            static predict(entity) {
                ...
                const distance = pos1.distance(pos2) * (entity === WebGame.myPlayer ? 1 : 2.2);
            ...
            static entityIn(entity, layer, extraRadius = 0) {
                const targets = WebGame.saves.entityList()[layer];
            ...
            static fuckme(number){
                const targets = WebGame.saved.entityList()[ELayer.PLAYER];
                return targets.some((target =>{
                    const my = WebGame.myPlayer;
                    const nEy = EntityManager.enemies()[0];
            ...
            static nearestLayer(entity, layer) {
                const objects = WebGame.saves.entityList()[layer].map((target => Formatter.object(target)));
                return objects.sort(((a, b) => this.sortDistance(a, b, entity)))[0];
            }
        }
        const attackAnimation = (entity, sorted) => {
            const b = WebGame.saves.buffer;
            const len = WebGame.saves.byteLength();
            const players = WebGame.saves.players();
                ...
                if (type === ELayer.PLAYER && target) {
                    ...
                    if (Settings.autosync && controller.canAutosync()) {
                        ...
                        if (controller.isTeammate(player) && controller.isPrimary(weapon)) {
                            ...
                            if (nearest !== null && EntityManager.inWeaponRange(WebGame.myPlayer, nearest, controller.itemBar[0])) {
                                const previousWeapon = controller.weapon;
                                controller.whichWeapon(false);
                                controller.attack(EntityManager.angle(WebGame.myPlayer, nearest));
        ...
        class PacketManager {
            ...
            send(...args) {
                WebGame.saves.send(new Uint8Array(args));
            }
        ...
        class Controller {
            ...
            updateWeapon(type) {
                const weapon = WebGame.saves.defaultData[WebGame.props.itemBar][type];
                if (this.isWeapon(weapon) && this.itemBar[type] !== weapon) {
                    this.itemBar[type] = weapon;
                }
            }
            isMyPlayer(entity) {
                return entity.id === WebGame.saves.myPlayerID();
            }
            isTeammate(entity) {
                return entity.id !== WebGame.saves.myPlayerID() && teammates.includes(entity.ownerID);
            }
            isEnemy(entity) {
            ...
            currentCount(type) {
                return WebGame.saves.defaultData[WebGame.props.currentCount][type];
            }
            hasCount(type) {
            ...
            upgradeScythe() {
                const target = WebGame.saves.entityList()[ELayer.GOLDENCOW][0];
                if (target !== undefined) {
                    this.PacketManager.upgradeScythe(target[WebGame.props.id]);
                }
            }
            buyHat(id) {
            ...
            hatReloaded() {
                return WebGame.myPlayer.target.hatReload.current === TargetReload.HAT;
            }
            ...
            accept(which) {
                ...
                const acceptList = WebGame.saves.clanData[WebGame.props.acceptList];
                acceptList.shift();
            }
            async spawn() {
                await sleep(100);
            ...
            place(type, angle, placementType) {
                if (this.wasAutoboost) {
                    ...
                    if (nearest !== undefined) {
                        angle = EntityManager.angle(WebGame.myPlayer, nearest);
            ...
            invisibleHit() {
                ...
                if (nearest && (Settings.meleeAim && !shoot || Settings.bowAim && shoot)) {
                    const pos1 = EntityManager.predict(WebGame.myPlayer);
            ...
            goku(){
                WebGame.controller.place(7, angle);
                setTimeout(function() {
                    WebGame.controller.place(7, toRad(toDegree(angle) - 72));
                    setTimeout(function() {
                        WebGame.controller.place(7, toRad(toDegree(angle) + 72));
                    }, 100);
                }, 40)
            ...
            handleKeydown(event, code) {
                ...
                if (code === Settings.toggleMenu && !isInput(event.target)) {
                    if (typeof WebGame.toggleMenu === "function") WebGame.toggleMenu();
                }
                if (!this.inGame) return;
                if (code === Settings.openChat) {
                    if (!isInput()) event.preventDefault();
                    WebGame.saves.toggleChat();
                }
                ...
                if (code === Settings.lockRotation) {
                    ...
                    WebGame.saves.toggleRotation(this.rotation);
                }
        ...
        const createEntity = target => {
            const id = target[WebGame.props.id];
            const type = target.type;
            const entities = WebGame.saves.entityList();
            if (type === ELayer.PLAYER) {
                if (id === WebGame.saves.myPlayerID()) {
                    WebGame.myPlayer.target = target;
                    updateSkin();
                }
                const player = Formatter.player(target);
                target.hatReload = {
        ...
        class RenderManager {
            ...
            static markerColor(target, ownerID) {
                let color = null;
                const object = Formatter.object(target);
                const isMyPlayer = WebGame.myPlayer.ownerID === ownerID;
            ...
            static renderHP(ctx, entity, height = 0) {
                ...
                const {x, y, health, radius} = entity;
                this.renderText(ctx, `${health}`, (width => [ x - width / 2, y + radius + 55 + height ]));
            }
            ...
            static windmillRotation(target) {
                ...
                if (target[WebGame.props.rotateSpeed] !== speed) {
                    target[WebGame.props.rotateSpeed] = speed;
                }
            }
            ...
            static renderTracer(ctx, entity, isTeammate) {
                const player = Formatter.player(WebGame.myPlayer.target);
        ...
        const drawEntityInfo = (target, ctx, isTeammate) => {
            ...
            const id = WebGame.saves.myPlayerID();
            if (id === entity.id) {
                if (Settings.rainbow) {
                    WebGame.controller.hsl = (WebGame.controller.hsl + .3) % 360;
                }
                if (controller.aimTarget !== null) {
                    ...
                    WebGame.myPlayer.target[WebGame.props.angle] = dir;
                }
            }
            let height = 0;
            ...
            if (id === entity.id || !WebGame.myPlayer.target) return;
        ...
        const drawItemBar = (ctx, imageData, index) => {
            ...
            const id = WebGame.saves.defaultData[WebGame.props.itemBar][index];
            const type = Items[id].itemType;
            const currentCount = WebGame.saves.defaultData[WebGame.props.currentCount][type];
            const maxCount = controller.maxCount[type];
            if (maxCount === 0) return;
            const x = imageData[WebGame.props.x] - 10;
            const y = imageData[WebGame.props.y] + 10;
        ...
        const healing = () => {
            const {health, maxHealth, isClown} = WebGame.myPlayer;
            if (Settings.autoheal && health < maxHealth && !isClown && controller.inGame) controller.heal();
            setTimeout(healing, getDelay(health));
        };
        function animalss(){
            const nearest = EntityManager.nearestLayer(WebGame.myPlayer, ELayer.WOLF);
            if(nearest){
                console.log('wolf')
            }
            if (Settings.anims && controller.hasCount(ItemType.TRAP) && EntityManager.entityIn(WebGame.myPlayer, ELayer.WOLF, 2) && !EntityManager.entityIn(WebGame.myPlayer, ELayer.WOLF)) {
                controller.place(ItemType.TRAP, EntityManager.angle(WebGame.myPlayer, nearest), PlacementType.INVISIBLE);
            }
        }
        ...
        window.equip = (e) => {
            if (WebGame.myPlayer.hat !== e) {
                WebGame.controller.PacketManager.equip(e)
            }
        }
        let nearestEnemy;
        let currentHat = 0;
        ...
        function autobreaker(){
            ...
            if (Settings.autobreak2) {
                for (let i = 0; i < WebGame.saves.entityList()[ELayer.TRAP].length; i++) {
                    const trapx = WebGame.saves.entityList()[ELayer.TRAP][i]
                    if (trapx && trapx[WebGame.props.ownerID] !== WebGame.myPlayer.ownerID && !Allies.find(a => a.pid == trapx[WebGame.props.ownerID]) && Math.hypot(trapx[WebGame.props.x] - WebGame.myPlayer.x, trapx[WebGame.props.y] - WebGame.myPlayer.y) <= 55) {
                        trapy = trapx;

                        if (!inTrap) {
                            inTrap = true;
                            currentHat = WebGame.myPlayer.hat
                            currentWeapon = WebGame.controller.weapon
                            // WebGame.controller.PacketManager.chat(attackbot ? 'Bot: trapped' : 'trapped')
                            window.equip(11)
                            let angle = Math.atan2(-(trapy[WebGame.props.y] - WebGame.myPlayer.y), -(trapy[WebGame.props.x] - WebGame.myPlayer.x));
                            WebGame.controller.place(7, angle);
                            setTimeout(function() {
                                WebGame.controller.place(7, toRad(toDegree(angle) - 72));
                                setTimeout(function() {
                            ...
                            /*setTimeout(() => {
                  WebGame.controller.PacketManager.chat(attackbot ? 'Bot: antitrap done' : 'done')
                }, 500)*/
                        } else {
                            let e = Math.atan2(
                                trapy[WebGame.props.y] - WebGame.myPlayer.y,
                                trapy[WebGame.props.x] - WebGame.myPlayer.x
                            )
                            //   WebGame.controller.PacketManager.chat('lol im trapped'.shuffle())
                            window.equip(11)
                            WebGame.controller.PacketManager.attack(e)
                            WebGame.controller.PacketManager.stopAttack()

                            if (WebGame.controller.itemBar[1] == 15) {
                                WebGame.controller.whichWeapon(true)
                            } else {
                                WebGame.controller.whichWeapon(false)
                            }
                        }
                        trapId = trapy[WebGame.props.id];
                        break
                    } else

                        if (trapx[WebGame.props.ownerID] == WebGame.myPlayer.ownerID && !teammates.includes(trapx[WebGame.props.ownerID])) {
                            let d = EntityManager.nearestPossible(WebGame.controller.itemBar[0])
                            let nearest = EntityManager.enemies()[0];
                            if (d !== null && trapx && Math.hypot(trapx[WebGame.props.x] - d.x, trapx[WebGame.props.y] - d.y) <= 55) {
                                enemyIsTrapped = true;
                                enemyTrap = trapx;
                                if(Settings.hitloehp && enemyIsTrapped){
                                    controller.PacketManager.chat
                                    if(d.health < 70){
                                        controller.PacketManager.chat
                                        //log('hits')
                                        controller.attack(EntityManager.angle(nearest, WebGame.myPlayer))
                                        controller.PacketManager.stopAttack();
                                    }
                                }
                            }else{
                                let aspike = null;
                                for (let i = 0; i < WebGame.saves.entityList()[ELayer.SPIKE].length; i++) {
                                    const spiker = WebGame.saves.entityList()[ELayer.SPIKE][i]
                                    if (spiker && spiker[WebGame.props.ownerID] == WebGame.myPlayer.ownerID && Allies.find(a => a.pid == spiker[WebGame.props.ownerID]) && Math.hypot(spiker[WebGame.props.x] - WebGame.myPlayer.x, spiker[WebGame.props.y] - WebGame.myPlayer.y) <= 200) {
                                        aspike = spiker;
                                        let tospike = Math.atan2(-(aspike[WebGame.props.y] - WebGame.myPlayer.y), -(aspike[WebGame.props.x] - WebGame.myPlayer.x));

                                        if(Settings.hitloehp && spiker){
                                            this.PacketManager.chat('nearest spikes fr')
        ...
        function autpush(){
            for (let i = 0; i < WebGame.saves.entityList()[ELayer.SPIKE].length; i++) {
                let spikex = WebGame.saves.entityList()[ELayer.SPIKE][i];
                let psuh = EntityManager.nearestPossible(WebGame.controller.itemBar[0])
                let topushto = Math.atan2(spikex[WebGame.props.y] - WebGame.myPlayer.y,spikex[WebGame.props.x] - WebGame.myPlayer.x)
                let spike = Math.atan2(-(spikex[WebGame.props.y] - psuh.y), -(spikex[WebGame.props.x] - psuh.x));
                if(enemyIsTrapped && psuh !== null){
                    angle = EntityManager.angle(WebGame.myPlayer, spike);
                    this.PacketManager.moveAngle(angle);
                ...
                case ELayer.PLAYER:
                    {
                        ...
                        if (player.id === WebGame.saves.myPlayerID()) {
                            WebGame.myPlayer = {
                                ...WebGame.myPlayer,
                                ...player
                            };
                            const {x2, y2, health, maxHealth, isClown, hat} = WebGame.myPlayer;
                            ...
                            const onPlatform = EntityManager.entityIn(WebGame.myPlayer, ELayer.PLATFORM);
                            ...
                            if (Settings.autoAccept && WebGame.saves.clanData[WebGame.props.acceptList].length) {
                            ...
                            animalss()
                            if (!trapy && inTrap) {
                                WebGame.controller.PacketManager.equip(currentHat);
                                // WebGame.controller.PacketManager.chat(attackbot ? 'Bot: untrapped' : 'detrapped')
                                if (WebGame.myPlayer.hat != currentHat) window.equip(currentHat)
                                self.activeweapon = currentWeapon
                                WebGame.controller.whichWeapon(currentWeapon)
                                currentWeapon = 0;
                            ...
                            if (Settings.antiFireball && hasPlatform && controller.hasCount(ItemType.PLATFORM) && EntityManager.entityIn(WebGame.myPlayer, ELayer.FIREBALL, 23) && !EntityManager.entityIn(WebGame.myPlayer, ELayer.PLATFORM)) {
                                const nearest = EntityManager.nearestLayer(WebGame.myPlayer, ELayer.FIREBALL);
                                controller.place(ItemType.PLATFORM, EntityManager.angle(nearest, WebGame.myPlayer), PlacementType.INVISIBLE);
                            }
                        }
                        const hatReload = target.hatReload;
                        if (target.prevHat !== player.hat) {
                            target.prevHat = player.hat;
                            hatReload.current = -WebGame.step;
                            hatReload.lerp = 0;
                        }
                        hatReload.current = Math.min(hatReload.current + WebGame.step, hatReload.max);
                        if (Settings.weaponReloadBar) {
                            const weaponReload = target.weaponReload;
                            weaponReload.current = Math.min(weaponReload.current + WebGame.step, weaponReload.max);
                        }
                        break;
                    }
                ...
                case ELayer.TURRET:
                    {
                        if (Settings.turretReloadBar) {
                            ...
                            turretReload.current = Math.min(turretReload.current + WebGame.step, turretReload.max);
                        }
                        break;
                    }
                ...
                case ELayer.DRAGON:
                    {
                        if (Settings.fireballReloadBar) {
                            ...
                            fireballReload.current = Math.min(fireballReload.current + WebGame.step, fireballReload.max);
                        }
                        break;
                    }
            }
        ...
        const applyHooks = code => {
            ...
            Hook.replace("buffer", /((\w+)=new \w+\(NUMBER{4096}\).+?(\w+)=.+?)function/, `$1WebGame.saves.buffer=$2;WebGame.saves.byteLength=()=>$3;function`);
            Hook.append("toggleRotation", /return (\w+)\?\w+:.+?\}/, `WebGame.saves.toggleRotation=(value)=>{$2=value};`);
            const acceptList = Hook.append("clanData", /(\w+)\.(\w+)\.\w+\(\)\);.+?\}/, `WebGame.saves.clanData=$2;`)[3];
            WebGame.props.acceptList = acceptList;
            Hook.append("upgradeItem", /\.001.+?for\(let \w+=0,.+?\w+\(new \w+\(\[.+?,(\w+)\]\)(,|;)?/, `,WebGame.controller.upgradeItem($2)$3`);
            Hook.replace("zoom", /(\w+):NUMBER{1824},(\w+):NUMBER{1026}/, "get $1(){return WebGame.scale.lerp.w},get $2(){return WebGame.scale.lerp.h}");
            Hook.insert("send", /=NUMBER{9999}.+?\(null\).+?{INSERT}function (\w+)\(\w+\)\{/, `WebGame.saves.send=$3;`);
            Hook.replace("toggleChat", /(return \(?(\w+&&\w+.+?)(?:,)?(?:\))?void.+?)function/, `$1WebGame.saves.toggleChat=()=>{$2};function`);
            Hook.replace("updatePlayer", /(\w+\(ARGS{16}\).+?(\w+)\.\w+=0[,;]?)\}function/, `$1;WebGame.hooks.updatePlayer($2)}function`);
            Hook.replace("createEntity", /(function \w+\((\w+),ARGS{16}\).+?\})(\}\w+\(\))/, `$1WebGame.hooks.createEntity($2)$3`);
            Hook.append("drawEntityInfo", /-NUMBER{50},.+?function \w+\((ARGS{3})\)\{/, `WebGame.hooks.drawEntityInfo($2);WebGame.hooks.ally($2);`);
            ...
            WebGame.props.y1 = y1;
            WebGame.props.y2 = y2;
            WebGame.props.angle = angle;
            WebGame.props.angle1 = angle1;
            WebGame.props.angle2 = angle2;
            const ownerID = Hook.match("ownerID", /\|\|\w+&&\w+===\w+\.(\w+)\)/)[1];
            WebGame.props.ownerID = ownerID;
            const health = Hook.match("health", /\w+\.(\w+)\/NUMBER{255}\*/)[1];
            WebGame.props.health = health;
            const entityValue = Hook.match("entityValue", /!\(\w+\.(\w+)&/)[1];
            WebGame.props.entityValue = entityValue;
            const [, currentItem, hat] = Hook.match("hat", /\(\w+\.(\w+)\|\w+\.(\w+)<<NUMBER{8}\)/);
            WebGame.props.hat = hat;
            WebGame.props.currentItem = currentItem;
            const projectileType = Hook.match("projectileType", /,\w+\[\w+\]\.(\w+),/)[1];
            WebGame.props.projectileType = projectileType;
            Hook.prepend("myPlayerID", /function \w+\(\)\{return \w+!==(\w+)/, `WebGame.saves.myPlayerID=()=>$2;`);
            const itemBar = Hook.replace("defaultData", /(\W\w+>NUMBER{1}\W.+?(\w+)\.(\w+).+?)function/, "$1WebGame.saves.defaultData=$2;function")[3];
            WebGame.props.itemBar = itemBar;
            const currentCount = Hook.match("currentCount", /(\w+):\[ARGS{11}\],/)[1];
            WebGame.props.currentCount = currentCount;
            Hook.replace("entityList", /(\(this,.+?typeof window.+?(\w+)=\[\].+?)function/, `$1WebGame.saves.entityList=()=>$2;function`);
            const rotateSpeed = Hook.match("rotateSpeed", /\w+\(ARGS{17}\)\{.+?\/NUMBER{4}.+?\/NUMBER{4}.+?\w+\.(\w+)=/)[1];
            WebGame.props.rotateSpeed = rotateSpeed;
            Hook.append("showHoods", /\w+\.\w+!==\w+\)/, `||WebGame.settings.showHoods`);
            Hook.append("itemCounter", /AGE 0.+?\[(\w+)\][,;](\w+)\.\w+\((\w+)\)([,;])/, `WebGame.hooks.drawItemBar($4,$3,$2)$5`);
            Hook.replace("renderItems", /(\(\w+\.\w+\+\w+,\w+\.\w+\+\w+\).+?\w+\(\).+?\w+\.\w+\.\w+\)([,;]))/, `$1WebGame.hooks.renderItems(...arguments)$2`);
            Hook.replace("mousemove", /(\+NUMBER{110}.+?)(const \w+=\w+\(\).+?\w+!==\w+.+?\w+\(\w+\))/, `$1if(WebGame.controller.mousemove){$2}`);
            Hook.replace("chatMessage", /(\.NUMBER{18},.+?)(\w+\.\w+\((\w+),\w+\))/, `$1if(pingCount!==$3&&!WebGame.settings.hideMessages){$2}`);
            Hook.replace("clanMessage", /(NUMBER{1006}.+?)(\w+\.\w+\(\w+,.+?\+(\w+)\))/, `$1if(pingCount!==$3&&!WebGame.settings.hideMessages){$2}`);
            Hook.replace("players", /(\)\)\(\).+?(\w+)=new.+?)function/, `$1WebGame.saves.players=()=>$2;function`);
            const [skin, accessory, back] = Hook.matchAll("skins", /=\w+\.(\w+)\|\|NUMBER{0}/).map((a => a[1]));
            WebGame.props.skin = skin;
            WebGame.props.accessory = accessory;
            WebGame.props.back = back;
        ...
        window.WebGame = {
            ...
            hooks: {
                ...
                ally: (entity, p2, p3) => {
                    if(p3 && entity[WebGame.props.id] !== WebGame.controller.myPlayerID && entity.type === 0 && !Enemys.find(A=> A.id == entity[WebGame.props.id])){
                        unload({
                            x: entity[WebGame.props.x],
                            y: entity[WebGame.props.y],
                            id: entity[WebGame.props.id],
                            type: entity.type
                        });
                    }
                    if (p3 && entity[WebGame.props.id] !== WebGame.controller.myPlayerID && entity.type === 0 && !Allies.find(A => A.id == entity[WebGame.props.id])) {
                        Allies.push({
                            x: entity[WebGame.props.x],
                            y: entity[WebGame.props.y],
                            type: entity.type,
                            hp: entity[WebGame.props.health] / 255 * 100,
                            hat: entity[WebGame.props.hat],
                            pid: entity[WebGame.props.ownerID],
                            id: entity[WebGame.props.id]
                        })
                    }
                    if (!p3 && entity[WebGame.props.id] !== WebGame.controller.myPlayerID && entity.type === 0 && !Enemys.find(A => A.id == entity[WebGame.props.id])) {
        ...
        // function getTrapType() {return WebGame.items.length>5?WebGame.items.length>6?WebGame.items[6]:WebGame.items[5]:WebGame.items[0]};
        function getTrapType() {
            let i = this.currentItem;
            let j = i.indexOf(9);
            return i[j+1?j:6];
        };
        function getPrimary(){return WebGame.items[0]};
        ...
        function unload(e) {
            ...
            //  this.mouse.angle(WebGame.myPlayer.angle)
        }
    ...
    if (Math.hypot(entity.x - player.x2, entity.y - player.y2) <= 150
        ...
        // WebGame[chatMessage]('replace ' + entity.id + ' type:' + entity.type);
        let angle = ang(entity);
        function PLACE() {
            let st = getSpikeType();
            let tt = getTrapType();

            this.place(!spikes.includes(entity.type) || tt == getPrimary() ? st : getTrapType(), angle);
        };
...
*/
        ...
        setInterval(()=>{
            const Players2 = WebGame.saves.entityList()[ELayer.PLAYER]
            const enemies2 = []
            const nearest = EntityManager.enemies()[0];
            const nEy = EntityManager.enemies()[0];
            if(Settings.autoplacerr && controller.hasCount(ItemType.TRAP) && EntityManager.distance(nearest, WebGame.myPlayer) /*&& EntityManager.fgdo(WebGame.myPlayer, nearest) <= 200*/){
                controller.place(ItemType.TRAP,EntityManager.angle(WebGame.myPlayer, nearest), PlacementType.INVISIBLE)
            }
        }, 1e3/9+45)
        let primaryWeapon = 1;
        ...
        setTimeout(() => {
            setInterval(() => {
                if(Settings.autoheal2 && WebGame.myPlayer.health < 20){
                    controller.heal();
                }
            }, 175)
        }, 200)
        setInterval(() => {
            if(Settings.autoheal2 && WebGame.myPlayer.health < 100) {
                controller.heal();
            }
        }, 1e3/9+35);
