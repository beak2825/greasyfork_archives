...
// @match        *://agma.io/
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/405809/Fav%20script.user.js
// @updateURL https://update.greasyfork.org/scripts/405809/Fav%20script.meta.js
// ==/UserScript==
  ...
  window.miracleScripts = {
    ...
    config: function() {
      ...
      setTimeout(function () {
        ...
        var $settingsButton = $('<button class="spec" style="width: 100px; margin-left: 7px; text-align: center; padding: 10px 0 20px 0" title="Miracle Scripts Settings">Settings</button>');
      ...
      window.addEventListener('miracleCommand', function(commandEvent) {
        ...
        if (commandEvent.command === 'fps' || commandEvent.command === '/fps') {
          ...
          if (parseInt(fps) > 0) {
            ...
            if (fps > 117) {  fpsRating = 'Tony is handsome ✔️'; }
          } else {
            fpsRating = '';
          }
    ...
    commands: function () {
      ...
      $('#chtbox').keydown(function (event) {
        if (event.keyCode === 13) {
          ...
          if (command === '/script') {
            var miracleInfo =  'is using 𝘍𝘢𝘷 𝘚𝘤𝘳𝘪𝘱𝘵, version ';
            ...
            $('#chtbox').val(miracleInfo + ', made by ℓαzαяσ' + self.watermark).focus();
          }
          ...
          if (command === '/powerups' || command === '/powers') {
            var map = {
                ...
                invGrowth: 'growth',
                invSpawnVirus: 'virus',
                invSpawnMothercell: 'mothercells',
